...
//			  quads that are oriented to fit the terrain and updated
//when object 			  moves.
//-----------------------------------------------------------------------------
...
//=============================================================================
void TerrainTracksRenderObjClass::Get_Obj_Space_Bounding_Sphere(
    SphereClass &sphere)
...
//=============================================================================
void TerrainTracksRenderObjClass::Get_Obj_Space_Bounding_Box(
...
//=============================================================================
void TerrainTracksRenderObjClass::init(Real width, Real length,
  ...
  m_boundingBox.Extent.Set(400.0f * MAP_XY_FACTOR, 400.0f * MAP_XY_FACTOR,
                           1.0f);
  m_width = width;
  m_length = length;
...
/** Cap the current track (adding an feathered edge) so we're ready to resume
                the track at a new location.  Used by objects entering FOW where
   we need to stop adding edges to the track when they enter the fog boundary
   but resume elsewhere if they become visible again.
*/
//=============================================================================
void TerrainTracksRenderObjClass::addCapEdgeToTrack(Real x, Real y) {
  /// @todo: Have object pass its height and orientation so we can remove extra
  /// calls.

  if (m_haveCap) {  // we already have a cap or there are no segments to cap
    return;
  }

  if (m_activeEdgeCount == 1) {  // if we only have one edge, then it must be
                                 // the current anchor edge.
  ...
  if (m_ownerDrawable &&
      (objectLayer = m_ownerDrawable->getObject()->getLayer()) != LAYER_GROUND)
    eHeight = BRIDGE_OFFSET_FACTOR +
              TheTerrainLogic->getLayerHeight(x, y, objectLayer, &vZTmp);
  else
  ...
  if (vDir.Length2() <
      sqr(m_length)) {  // not far enough from anchor to add track
    // since this is a  cap, we'll force the previous segment to transparent
    Int lastAddedEdge = m_topIndex - 1;
    if (lastAddedEdge < 0) lastAddedEdge = maxEdgeCount - 1;
    m_edges[lastAddedEdge].alpha =
        0.0f;  // force the last added edge to transparent.
    m_haveCap = TRUE;
    m_haveAnchor = false;  // recreate a new anchor when track resumes.
    return;
  }

  if (m_activeEdgeCount >=
      maxEdgeCount) {  // no more room in buffer so release oldest edge
    m_bottomIndex++;
    m_activeEdgeCount--;
  ...
  if (m_topIndex >= maxEdgeCount) m_topIndex = 0;  // roll around buffer
  ...
  topEdge.endPointPos[0] =
      vPos - (m_width * 0.5f * vX);  ///@todo: try getting height at endpoint
  topEdge.endPointPos[0].Z +=
      0.2f * MAP_XY_FACTOR;  // raise above terrain slightly
  ...
  topEdge.endPointPos[1] =
      vPos + (m_width * 0.5f * vX);  ///@todo: try getting height at endpoint
  topEdge.endPointPos[1].Z +=
      0.2f * MAP_XY_FACTOR;  // raise above terrain slightly
...
void TerrainTracksRenderObjClass::addEdgeToTrack(Real x, Real y) {
  ...
  if (!m_haveAnchor) {  // no anchor yet, make this point an anchor.
    PathfindLayerEnum objectLayer;
    if (m_ownerDrawable &&
        (objectLayer = m_ownerDrawable->getObject()->getLayer()) !=
            LAYER_GROUND)
      m_lastAnchor =
          Vector3(x, y,
                  TheTerrainLogic->getLayerHeight(x, y, objectLayer) +
    ...
    m_haveAnchor = true;
    m_airborne = true;
    m_haveCap = true;  // single segment tracks are always capped because
                       // nothing is drawn.
    return;
  }

  m_haveCap = false;  // have more than 1 segment now so will need to cap if
                      // it's interrupted.
 ...
 */
//=============================================================================
void TerrainTracksRenderObjClass::Render(
    RenderInfoClass &rinfo) {  ///@todo: After adding track mark visibility
                               ///tests, add visible marks to another list.
...
/**Find distance between the "trackfx" bones of the model.  This tells us the
   correct width for the trackmarks.
*/
static Real computeTrackSpacing(RenderObjClass *renderObj) {
  ...
  if ((leftTrack = renderObj->Get_Bone_Index("TREADFX01")) != 0 &&
      (rightTrack = renderObj->Get_Bone_Index("TREADFX02")) !=
          0) {  // both bones found, determine distance between them.
    ...
    trackSpacing =
        rightPos.Length() + DEFAULT_TRACK_WIDTH;  // add width of each track
...
/** Grab a track from the free store. If no free tracks exist, return NULL.
   ...
   return to the free store once all tracks have faded out.
        ...
        Input: width in world units of each track edge (should probably width of
   vehicle). length in world units between edges.  Shorter lengths produce more
   edges and smoother curves. texture to use for the tracks - image should be
   symetrical and include alpha channel.
*/
//=============================================================================
...
*/
//=============================================================================
void TerrainTracksRenderObjClassSystem::unbindTrack(
    TerrainTracksRenderObjClass *mod) {
 ...
 */
void TerrainTracksRenderObjClassSystem::releaseTrack(
    TerrainTracksRenderObjClass *mod) {
  if (mod == NULL) return;
...
void TerrainTracksRenderObjClassSystem::ReAcquireResources(void) {
  ...
  DEBUG_ASSERTCRASH(numModules * m_maxTankTrackEdges * 2 < 65535,
                    ("Too many terrain track edges"));

  m_vertexBuffer =
      NEW_REF(DX8VertexBufferClass,
              (DX8_FVF_XYZDUV1, numModules * m_maxTankTrackEdges * 2,
...
void TerrainTracksRenderObjClassSystem::init(SceneClass *TerrainTracksScene) {
  ...
  m_shaderClass = ShaderClass::
      _PresetAlphaShader;  //_PresetATestSpriteShader;//_PresetOpaqueShader;
...
void TerrainTracksRenderObjClassSystem::update() {
  ...
  while (mod) {
    ...
    for (i = 0, index = mod->m_bottomIndex; i < mod->m_activeEdgeCount;
         i++, index++) {
      ...
      if (iDiff == 0.0f) {  // this edge was invisible, we can remove it
        mod->m_bottomIndex++;
      ...
      if (mod->m_activeEdgeCount == 0 && !mod->m_bound) releaseTrack(mod);
    }
    mod = nextMod;
  }  // end while
}
...
void TerrainTracksRenderObjClassSystem::flush() {
  ...
  */
  ...
  if (!mod) return;  // nothing to render
    ...
    while (mod) {
      ...
      if (mod->m_activeEdgeCount >= 2 && mod->Is_Really_Visible()) {
        for (i = 0, index = mod->m_bottomIndex; i < mod->m_activeEdgeCount;
          ...
          if ((mod->m_activeEdgeCount - 1 - i) >=
              m_maxTankTrackOpaqueEdges)  // && i <
                                          // (MAX_PER_TRACK_EDGE_COUNT-FORCE_FADE_AT_EDGE))
          {  // we're getting close to the limit on the number of track pieces
             // allowed
            // so force it to fade out.
            distanceFade = 1.0f - (float)((mod->m_activeEdgeCount - i) -
          ...
          distanceFade *=
              mod->m_edges[index]
                  .alpha;  // adjust fade with distance from start of track
          ...
          verts->diffuse =
              diffuseLight | (REAL_TO_INT(distanceFade * 255.0f) << 24);
          verts++;
...
/**Adjust various paremeters which affect the cost of rendering tracks on the
map. Parameters are passed via GlobalData*/
void TerrainTracksRenderObjClassSystem::setDetail(void) {
  ...
  // We changed the maximum number of visible edges so re-allocate our resources
  // to match.
  ReAcquireResources();
};

TerrainTracksRenderObjClassSystem *TheTerrainTracksRenderObjClassSystem =
