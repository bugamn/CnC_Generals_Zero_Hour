...
// 30 fps
Int TheW3DFrameLengthInMsec =
    1000 / LOGICFRAMES_PER_SECOND;  // default is 33msec/frame == 30fps. but we
                                    // may change it depending on sys config.
static const Int MAX_REQUEST_CACHE_SIZE =
    40;  // Any size larger than 10, or examine code below for changes. jkmcd.
static const Real DRAWABLE_OVERSCAN =
    75.0f;  ///< 3D world coords of how much to overscan in the 3D screen region
...
static Real getHeightAroundPos(Real x, Real y) {
  ...
  terrainHeightMax = max(terrainHeightMax,
                         TheTerrainLogic->getGroundHeight(
                             x + TERRAIN_SAMPLE_SIZE, y - TERRAIN_SAMPLE_SIZE));
  terrainHeightMax = max(terrainHeightMax,
                         TheTerrainLogic->getGroundHeight(
                             x - TERRAIN_SAMPLE_SIZE, y - TERRAIN_SAMPLE_SIZE));
  terrainHeightMax = max(terrainHeightMax,
...
W3DView::W3DView() {
  ...
  m_cameraOffset.y =
      -(m_cameraOffset.z / tan(TheGlobalData->m_cameraPitch * (PI / 180.0)));
  m_cameraOffset.x =
      -(m_cameraOffset.y * tan(TheGlobalData->m_cameraYaw * (PI / 180.0)));
  ...
  m_locationRequests.reserve(
      MAX_REQUEST_CACHE_SIZE +
      10);  // This prevents the vector from ever re-allocing
...
//-------------------------------------------------------------------------------------------------
/** Sets the height of the viewport, while maintaining original camera
 * perspective. */
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
/** Sets the width of the viewport, while maintaining original camera
 * perspective. */
//-------------------------------------------------------------------------------------------------
void W3DView::setWidth(Int width) {
  ...
  m_3DCamera->Set_View_Plane(
      (Real)width / (Real)TheDisplay->getWidth() * DEG_TO_RADF(50.0f), -1);
}
...
void W3DView::setOrigin(Int x, Int y) {
  ...
  // bottom-right border was also moved my this call, so force an update of
  // extents.
  setWidth(m_width);
  setHeight(m_height);
}
...
//-------------------------------------------------------------------------------------------------
/** @todo This is inefficient. We should construct the matrix directly using
 * vectors. */
//-------------------------------------------------------------------------------------------------
void W3DView::buildCameraTransform(Matrix3D *transform) {
  ...
  // construct a matrix to rotate around the horizontal vector by the given
  // angle
...
void W3DView::calcCameraConstraints() {
  ...
  if (TheTerrainLogic) {
    ...
    /*
            ...
            -- they assume that all maps are height 'm_groundLevel' at the
       edges. (since you need to add some "buffer" around the edges of your map
                            anyway, this shouldn't be an issue.)

            -- for angles/pitches other than zero, it may show boundaries.
                            since we currently plan the game to be restricted to
    ...
    */
    ...
    if (TheGlobalData->m_debugAI) {
      offset =
          -1000;  // push out the constraints so we can look at staging areas.
    }
...
//-------------------------------------------------------------------------------------------------
void W3DView::getPickRay(const ICoord2D *screen, Vector3 *rayStart,
                         Vector3 *rayEnd) {
  Real logX, logY;

  // W3D Screen coordinates are -1 to 1, so we need to do some conversion:
  PixelScreenToW3DLogicalScreen(screen->x - m_originX, screen->y - m_originY,
                                &logX, &logY, getWidth(), getHeight());

  *rayStart = m_3DCamera->Get_Position();                // get camera location
  m_3DCamera->Un_Project(*rayEnd, Vector2(logX, logY));  // get world space
                                                         // point
...
void W3DView::setCameraTransform(void) {
  ...
  if ((TheGlobalData && TheGlobalData->m_drawEntireTerrain) ||
      (m_FXPitch < 0.95f ||
       m_zoom > 1.05)) {  // need to extend far clip plane so entire terrain can
                          // be visible
    farZ *= MAP_XY_FACTOR;
  }
...
#endif
  {
    ...
    DEBUG_ASSERTLOG(m_cameraConstraintValid,
                    ("*** cam constraints are not valid!!!\n"));
  ...
  if (TheTerrainRenderObject) {
    RefRenderObjListIterator *it =
        W3DDisplay::m_3DScene->createLightsIterator();
...
void W3DView::reset(void) {
  ...
  // Just move the camera to 0, 0, 0. It'll get repositioned at the beginning of
...
#if defined(_DEBUG) || defined(_INTERNAL)
void drawDrawableExtents(Drawable *draw,
                         void *userData);  // FORWARD DECLARATION
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
static void drawContainedDrawable(Object *obj, void *userData) {
  Drawable *draw = obj->getDrawable();

  if (draw) drawDrawableExtents(draw, userData);
...
static void drawDrawableExtents(Drawable *draw, void *userData) {
  ...
  switch (draw->getDrawableGeometryInfo().getGeomType()) {
    ...
    case GEOMETRY_CYLINDER: {
      ...
      TheTacticalView->worldToScreen(&pnt, &start);
      pnt.z = draw->getPosition()->z +
              draw->getDrawableGeometryInfo().getMaxHeightAbovePosition();
...
static void drawablePostDraw(Drawable *draw, void *userData) {
  ...
  if (draw->isDrawableEffectivelyHidden() || FXPitch < 0.0f) return;
...
#endif
  if (ss > OBJECTSHROUD_PARTIAL_CLEAR) return;

  // draw the any "icon" UI for a drawable (health bars, veterency, etc);

  //*****
  //@TODO: Create a way to reject this call easily -- like objects that have no
  //compatible modules.
  //*****
  ...
  if (TheGlobalData->m_showTerrainNormals) drawTerrainNormal(draw, userData);
...
/** This function performs all actions which affect the camera transform or 3D
   objects rendered in this frame.

   MW: I moved this code out out W3DView::draw() so that we can get final camera
   and object positions before any rendering begins.  This was necessary so that
   reflection textures (which update before the main rendering loop) could get a
   correct version of the scene. Without this change, the reflections were
   always 1 frame behind the non-reflected view.
*/
...
void W3DView::update(void) {
  ...
  //	Int elapsedTimeMs = TheW3DFrameLengthInMsec; // Assume a constant time
  //flow.  It just works out better.  jba.
    ...
    if (loseLock) {
      ...
      if (getCameraLockDrawable() != NULL) {
        ...
        if (!cameraLockDrawable) {
          ...
          Matrix3D transform;
          // this method must ONLY be called from the client, NEVER From the
          // logic, not even indirectly.
          if (cameraLockDrawable->clientOnly_getFirstRenderObjInfo(
                  &pos, &boundingSphereRadius, &transform)) {
            Vector3 zaxis(0, 0, 1);
            ...
            // get position of top of object, assuming world z roughly along
            // local z.
            objPos += boundingSphereRadius * 1.0f * zaxis;
            Vector3 objview =
                transform.Get_X_Vector();  // get view vector of object
            // move camera back behind object far enough not to intersect
            // bounding sphere
            Vector3 camtran = objPos - objview * boundingSphereRadius * 4.5f;

            Vector3 prevCamTran =
                m_3DCamera->Get_Position();  // get current camera position.

            Vector3 tranDiff =
                (camtran -
                 prevCamTran);  // vector old position to new position.

            camtran = prevCamTran +
                      tranDiff * 0.1f;  // slowly move camera to new position.
        ...
        if (m_lockType == LOCK_FOLLOW) {
          ...
          if (cameraLockObj->isUsingAirborneLocomotor() &&
              cameraLockObj->isAboveTerrainOrWater()) {
            Matrix3D camXForm;
  ...
  if (!(TheScriptEngine->isTimeFrozenDebug()/* || TheScriptEngine->isTimeFrozenScript()*/) && !TheGameLogic->isGamePaused()) {
    // If we aren't frozen for debug, allow the camera to follow scripted
    // movements.
    ...
    if (m_doingRotateCamera || m_doingMoveCameraOnWaypointPath ||
        m_doingPitchCamera || m_doingZoomCamera || m_doingScriptedCameraLock) {
  ...
  /*
   * In order to have the camera follow the terrain in a non-dizzying way, we
   * will have a "desired height" value that the user sets.  While scrolling,
   * the actual height (set by zoom) will not get updated unless we are
   * scrolling uphill and our view either goes underground or higher than the
   * max allowed height.  When the camera is at rest (not scrolling), the zoom
   * will move toward matching the desired height.
   */
  ...
  if (TheTerrainLogic && TheGlobalData && TheInGameUI && m_okToAdjustHeight &&
    ...
    if (didScriptedMovement ||
        (TheGameLogic->isInReplayGame() && TheGlobalData->m_useCameraInReplay))
#else
    if (didScriptedMovement)
#endif
    {
      // if we are in a scripted camera movement, take its height above ground
      // as our desired height.
      m_heightAboveGround = m_currentHeightAboveGround;
      // DEBUG_LOG(("Frame %d: height above ground: %g %g %g %g\n",
      // TheGameLogic->getFrame(), m_heightAboveGround, 	m_cameraOffset.z,
      //m_zoom, m_terrainHeightUnderCamera));
    }
    if (TheInGameUI->isScrolling()) {
      ...
      if (m_scrollAmount.length() < m_scrollAmountCutoff ||
          (m_currentHeightAboveGround < m_minHeightAboveGround) ||
          (TheGlobalData->m_enforceMaxCameraHeight &&
           m_currentHeightAboveGround > m_maxHeightAboveGround)) {
        Real zoomAdj =
            (desiredZoom - m_zoom) * TheGlobalData->m_cameraAdjustSpeed;
      ...
      Real zoomAdj =
          (m_zoom - desiredZoom) * TheGlobalData->m_cameraAdjustSpeed;
  ...
  if (recalcCamera) setCameraTransform();
  ...
  if (WW3D::Get_Frame_Time())  // make sure some time actually elapsed
    TheGameClient->iterateDrawablesInRegion(&axisAlignedRegion, drawDrawable,
                                            this);
}
...
void W3DView::getAxisAlignedViewRegion(Region3D &axisAlignedRegion) {
  //
  // get the 4 points in 3D space of the 4 corners of the view, we will use a z
  // = 0.0f value so that we can get everything ... even stuff below the terrain
  //
  Coord3D box[4];
  getScreenCornerWorldPointsAtZ(&box[0], &box[1], &box[2], &box[3], 0.0f);

  //
  // take those 4 corners projected into the world and create an axis aligned
  // bounding box, we will use this box to iterate the drawables in 3D space
  //
  ...
  // we want to overscan a little bit so that we get objects that are partially
  // offscreen
...
Bool W3DView::setViewFilter(enum FilterTypes filter) {
  FilterTypes oldFilter =
      m_viewFilter;  // save previous filter in case setup fails.
...
//-------------------------------------------------------------------------------------------------
/** Calculates how many pixels we scrolled since last frame for motion blur
 * calculations. */
//-------------------------------------------------------------------------------------------------
void W3DView::calcDeltaScroll(Coord2D &screenDelta) {
  screenDelta.x = 0;
  screenDelta.y = 0;
  Vector3 prevPos(m_previousLookAtPosition.x, m_previousLookAtPosition.y,
                  m_groundLevel);
  Vector3 prevScreen;
...
void W3DView::draw(void) {
  ...
  if (m_viewFilterMode && m_viewFilter > FT_NULL_FILTER &&
      m_viewFilter < FT_MAX) {
    // Most likely will redirect rendering to a texture.
    W3DShaderManager::filterPreRender(m_viewFilter, skipRender,
                                      customScenePassMode);
  ...
  if (!skipRender) {
    ...
    if (m_isWireFrameEnabled)
      W3DDisplay::m_3DScene->Set_Extra_Pass_Polygon_Mode(
          SceneClass::EXTRA_PASS_CLEAR_LINE);
    W3DDisplay::m_3DScene->doRender(m_3DCamera);
    W3DDisplay::m_3DScene->Set_Extra_Pass_Polygon_Mode(
        SceneClass::EXTRA_PASS_DISABLE);
  ...
  if (m_viewFilterMode && m_viewFilter > FT_NULL_FILTER &&
    ...
    continueTheEffect = W3DShaderManager::filterPostRender(
        m_viewFilter, m_viewFilterMode, deltaScroll, doExtraRender);
    if (!skipRender && getCameraLock()) {
      Object *cameraLockObj = TheGameLogic->findObjectByID(getCameraLock());
      if (cameraLockObj) {
        Drawable *drawable = cameraLockObj->getDrawable();
  ...
  // Some effects require that we render a modified version of the scene into a
  // texture but also require an unaltered version in the framebuffer.  So we
  // re-render again into framebuffer after texture rendering was turned off by
  // filterPostRender().
  if (doExtraRender) {
    // Reset to normal scene rendering.
    // The pass that rendered into a texture may have left the z-buffer in a
    // weird state so clear it before rendering normal scene.
    ///@todo: Don't clear z-buffer unless shader uses z-bias or anything else
    ///that would cause <= z to fail on normal render.
    DX8Wrapper::Clear(
        false, true, Vector3(0.0f, 0.0f, 0.0f),
        TheWaterTransparency->m_minWaterOpacity);  // Clear z but not color
    W3DDisplay::m_3DScene->setCustomPassMode(SCENE_PASS_DEFAULT);
    W3DDisplay::m_3DScene->doRender(m_3DCamera);
    Coord2D deltaScroll;
    W3DShaderManager::filterPostRender(m_viewFilter, m_viewFilterMode,
                                       deltaScroll, doExtraRender);
  }
  ...
  if (TheGlobalData->m_debugAI) {
    if (TheAI->pathfinder()->getDebugPath()) {
      ...
      if (worldToScreen(prevNode->getPosition(), &start)) {
        TheDisplay->drawLine(start.x - 3, start.y - 3, start.x + 3, start.y - 3,
                             1.0f, color);
        TheDisplay->drawLine(start.x + 3, start.y - 3, start.x + 3, start.y + 3,
                             1.0f, color);
        TheDisplay->drawLine(start.x + 3, start.y + 3, start.x - 3, start.y + 3,
      ...
      for (PathNode *node = prevNode->getNext(); node; node = node->getNext()) {
        ...
        if (node->getNext()) {
          if (worldToScreen(node->getPosition(), &start)) {
            TheDisplay->drawLine(start.x - 4, start.y, start.x + 3, start.y,
                                 1.0f, color);
          }
        }
      }
      if (prevNode && worldToScreen(prevNode->getPosition(), &start)) {
        TheDisplay->drawLine(start.x - 4, start.y, start.x + 3, start.y, 1.0f,
                             color);
        TheDisplay->drawLine(start.x, start.y - 4, start.x, start.y + 3, 1.0f,
                             color);
      }
      color = 0xFFFF0000;  // 0xAARRGGBB
      if (worldToScreen(TheAI->pathfinder()->getDebugPathPosition(), &start)) {
        TheDisplay->drawLine(start.x - 3, start.y, start.x + 3, start.y, 1.0f,
                             color);
        TheDisplay->drawLine(start.x, start.y - 3, start.x, start.y + 3, 1.0f,
                             color);
      }
    }
  ...
  //
  // there are several things we might want to do as a post pass on the objects
  // after they are all drawn
  /// @todo we might want to consider wiping this iterate out if there is
  /// nothing to post draw
  //
  TheGameClient->resetRenderedObjectCount();
  TheGameClient->iterateDrawablesInRegion(&axisAlignedRegion, drawablePostDraw,
                                          this);
...
void W3DView::scrollBy(Coord2D *delta) {
  ...
  if (delta && (delta->x != 0 || delta->y != 0)) {
    ...
    // m_cameraConstraintValid = false;	// pos change does NOT invalidate cam
    // constraints
...
void W3DView::setDefaultView(Real pitch, Real angle, Real maxHeight) {
  // MDC - we no longer want to rotate maps (design made all of them right to
  // begin with)
...
void W3DView::setHeightAboveGround(Real z) {
  ...
  // if our zoom is limited, we will stay within a predefined distance from the
  // terrain
  if (m_zoomLimited) {
...
/** Using the W3D camera translate the world coordinate to a screen coord.
        Screen coordinates returned in absolute values relative to full display
   resolution.  */
//-------------------------------------------------------------------------------------------------
Bool W3DView::worldToScreen(const Coord3D *w, ICoord2D *s) {
  ...
  if (m_3DCamera) {
    ...
    enum CameraClass::ProjectionResType projection =
        m_3DCamera->Project(screen, world);
    if (projection != CameraClass::INSIDE_FRUSTUM &&
        projection != CameraClass::OUTSIDE_FRUSTUM) {
    ...
    // coords now
    //
    W3DLogicalScreenToPixelScreen(screen.X, screen.Y, &s->x, &s->y, getWidth(),
                                  getHeight());
    s->x +=
        m_originX;  // convert viewport coordinates to full screen coordinates
    s->y += m_originY;
...
/** all the drawables in the view, that fall within the 2D screen region
        ...
        * the test are returned.
        Screen coordinates assumed in absolute values relative to full display
   resolution. */
//-------------------------------------------------------------------------------------------------
Int W3DView::iterateDrawablesInRegion(IRegion2D *screenRegion,
                                      Bool (*callback)(Drawable *draw,
                                                       void *userData),
                                      void *userData) {
  Bool inside = FALSE;
  Int count = 0;
  ...
  if (screenRegion) {
    ...
    normalizedRegion.lo.x =
        ((Real)(screenRegion->lo.x - m_originX) / (Real)getWidth()) * 2.0f -
        1.0f;
    normalizedRegion.lo.y =
        -(((Real)(screenRegion->hi.y - m_originY) / (Real)getHeight()) * 2.0f -
          1.0f);
    normalizedRegion.hi.x =
        ((Real)(screenRegion->hi.x - m_originX) / (Real)getWidth()) * 2.0f -
        1.0f;
    normalizedRegion.hi.y =
        -(((Real)(screenRegion->lo.y - m_originY) / (Real)getHeight()) * 2.0f -
          1.0f);
  ...
  if (regionIsPoint) {
    ...
    onlyDrawableToTest = pickDrawable(
        &screenRegion->lo, TRUE,
  ...
  for (draw = TheGameClient->firstDrawable(); draw;
    ...
    if (inside) {
      if (callback(draw, userData)) ++count;

    }  // end if

    // If onlyDrawableToTest, then we should bail out now.
    if (onlyDrawableToTest != NULL) break;
...
Drawable *W3DView::pickDrawable(const ICoord2D *screen, Bool forceAttack,
  ...
  while (window) {
    ...
    // select anything.
    if (!BitTest(window->winGetStatus(), WIN_STATUS_SEE_THRU)) return NULL;
  ...
  if (forceAttack) result.ComputeContactPoint = true;
  ...
  // for right now there is no drawable data in a render object which is
  // // if we've found a render object, return our drawable associated with it,

  // the terrain, therefore the userdata is NULL
  /// @todo terrain and picking!
  if (renderObj) drawInfo = (DrawableInfo *)renderObj->Get_User_Data();
...
void W3DView::screenToTerrain(const ICoord2D *screen, Coord3D *world) {
  // sanity
  if (screen == NULL || world == NULL || TheTerrainRenderObject == NULL) return;
  ...
  // We insert them at the end for speed (no copies needed), but using the princ
  // of locality, we should start searching where we most recently inserted
  for (int i = m_locationRequests.size() - 1; i >= 0; --i) {
    if (m_locationRequests[i].first.x == screen->x &&
        m_locationRequests[i].first.y == screen->y) {
  ...
  req.first = (*screen);
  req.second = (*world);
  m_locationRequests.push_back(
      req);  // Insert this request at the end, requires no extra copies
...
void W3DView::lookAt(const Coord3D *o) {
  ...
  if (o->z >
      PATHFIND_CELL_SIZE_F + TheTerrainLogic->getGroundHeight(pos.x, pos.y)) {
    ...
    rayStart = m_3DCamera->Get_Position();  // get camera location
    m_3DCamera->Un_Project(rayEnd,
                           Vector2(0.0f, 0.0f));  // get world space point
    rayEnd -= rayStart;                 // vector camera to world space point
    rayEnd.Normalize();                 // make unit vector
    rayEnd *= m_3DCamera->Get_Depth();  // adjust length to reach far clip plane
...
void W3DView::initHeightForMap(void) {
  ...
  const Real MAX_GROUND_LEVEL =
      120.0;  // jba - starting ground level can't exceed this height.
  ...
  m_cameraOffset.x =
      -(m_cameraOffset.y * tan(TheGlobalData->m_cameraYaw * (PI / 180.0)));
  m_cameraConstraintValid =
      false;  // possible ground level change invalidates cam constraints
  setCameraTransform();
}
 ...
 * evaluated in draw(). */
//-------------------------------------------------------------------------------------------------
void W3DView::moveCameraTo(const Coord3D *o, Int milliseconds, Int shutter,
                           Bool orient) {
...
//-------------------------------------------------------------------------------------------------
void W3DView::rotateCameraTowardObject(ObjectID id, Int milliseconds,
...
void W3DView::cameraModFinalZoom(Real finalZoom) {
  if (m_doingRotateCamera) {
    ...
    zoomCamera(
        finalZoom * maxZoom,
        (m_rcInfo.numFrames + m_rcInfo.numHoldFrames - m_rcInfo.curFrame) *
  ...
  if (m_doingMoveCameraOnWaypointPath) {
    ...
    zoomCamera(finalZoom * maxZoom, m_mcwpInfo.totalTimeMilliseconds -
                                        m_mcwpInfo.elapsedTimeMilliseconds);
  }
}
...
void W3DView::cameraModFinalTimeMultiplier(Int finalMultiplier) {
  if (m_doingZoomCamera) m_zcInfo.endTimeMultiplier = finalMultiplier;
  if (m_doingPitchCamera) m_pcInfo.endTimeMultiplier = finalMultiplier;
  if (m_doingRotateCamera) {
    ...
    for (i = 0; i < m_mcwpInfo.numWaypoints; i++) {
      ...
      m_mcwpInfo.timeMultiplier[i + 1] =
          REAL_TO_INT_FLOOR(0.5 + m_mcwpInfo.timeMultiplier[i + 1] * factor1 +
                            finalMultiplier * factor2);
    }
  } else {
...
void W3DView::cameraModFinalPitch(Real finalPitch) {
  if (m_doingRotateCamera) {
    pitchCamera(finalPitch, ((m_rcInfo.numFrames + m_rcInfo.numHoldFrames) -
                             m_rcInfo.curFrame) *
                                TheW3DFrameLengthInMsec);
  }
  if (m_doingMoveCameraOnWaypointPath) {
    pitchCamera(finalPitch, m_mcwpInfo.totalTimeMilliseconds -
                                m_mcwpInfo.elapsedTimeMilliseconds);
  }
}

// ------------------------------------------------------------------------------------------------
/** Move camera to a waypoint, resetting the default angle, pitch & zoom along
 * the way.. */
// ------------------------------------------------------------------------------------------------
void W3DView::resetCamera(const Coord3D *location, Int milliseconds) {
  ...
  DEBUG_LOG(
      ("W3DView::resetCamera() Current zoom: %g  Desired zoom: %g  Current "
       "pitch: %g  Desired pitch: %g\n",
...
// ------------------------------------------------------------------------------------------------
/** Move camera along a waypoint path in an interesting fashion.  Sets up
 * parameters that get evaluated in draw(). */
// ------------------------------------------------------------------------------------------------
void W3DView::moveCameraAlongWaypointPath(Waypoint *pWay, Int milliseconds,
                                          Int shutter, Bool orient) {
  ...
  while (pWay && m_mcwpInfo.numWaypoints < MAX_WAYPOINTS) {
    ...
    Vector3 dir(m_mcwpInfo.waypoints[m_mcwpInfo.numWaypoints].x -
                    m_mcwpInfo.waypoints[m_mcwpInfo.numWaypoints - 1].x,
                m_mcwpInfo.waypoints[m_mcwpInfo.numWaypoints].y -
                    m_mcwpInfo.waypoints[m_mcwpInfo.numWaypoints - 1].y,
                0);
    if (dir.Length() < MIN_DELTA) {
      if (pWay) {
        m_mcwpInfo.numWaypoints--;  // drop this one.
      } else {
        m_mcwpInfo.waypoints[m_mcwpInfo.numWaypoints - 1] =
            m_mcwpInfo.waypoints[m_mcwpInfo.numWaypoints];
...
// ------------------------------------------------------------------------------------------------
/** Calculates angles and distances for moving along a waypoint path.  Sets up
 * parameters that get evaluated in draw(). */
// ------------------------------------------------------------------------------------------------
void W3DView::setupWaypointPath(Bool orient) {
  ...
  for (i = 1; i < m_mcwpInfo.numWaypoints; i++) {
    Vector3 dir(m_mcwpInfo.waypoints[i + 1].x - m_mcwpInfo.waypoints[i].x,
                m_mcwpInfo.waypoints[i + 1].y - m_mcwpInfo.waypoints[i].y, 0);
  ...
  m_mcwpInfo.cameraAngle[m_mcwpInfo.numWaypoints] =
      m_mcwpInfo.cameraAngle[m_mcwpInfo.numWaypoints - 1];
  for (i = m_mcwpInfo.numWaypoints - 1; i > 1; i--) {
    m_mcwpInfo.cameraAngle[i] =
        (m_mcwpInfo.cameraAngle[i] + m_mcwpInfo.cameraAngle[i - 1]) / 2;
  }
  m_mcwpInfo.waySegLength[m_mcwpInfo.numWaypoints + 1] =
      m_mcwpInfo.waySegLength[m_mcwpInfo.numWaypoints + 1];
  if (m_mcwpInfo.totalDistance < 1.0) {
    m_mcwpInfo.waySegLength[m_mcwpInfo.numWaypoints - 1] +=
        1.0 - m_mcwpInfo.totalDistance;
  ...
  for (i = 0; i <= m_mcwpInfo.numWaypoints + 1; i++) {
    ...
    // m_mcwpInfo.cameraAngle[i]*180/PI));
  }
...
static Real makeQuadraticS(Real t) {
  // for t = linear 0-1, convert to quadratic s where 0==0, 0.5==0.5 && 1.0
  // == 1.0.
  Real tPrime = t;
  if (t < 0.5) {
    tPrime = 0.5 * (2 * t * 2 * t);
...
void W3DView::rotateCameraOneFrame(void) {
  ...
  if (m_rcInfo.curFrame <= m_rcInfo.numFrames) {
    ...
    if (m_rcInfo.trackObject) {
      ...
      Vector3 dir(m_rcInfo.targetObjectPos.x - m_pos.x,
                  m_rcInfo.targetObjectPos.y - m_pos.y, 0);
      if (dir.Length() >= 0.1) {
        dir.Normalize();
        Real angle = WWMath::Acos(dir.X);
        if (dir.Y < 0) {
    ...
    // m_zoom = m_rcInfo.startZoom +
    // (m_rcInfo.endZoom-m_rcInfo.startZoom)*factor; m_FXPitch =
    // m_rcInfo.startPitch + (m_rcInfo.endPitch-m_rcInfo.startPitch)*factor;
    normAngle(m_angle);
    // DEBUG_LOG(("\tm_angle:%g\n", m_angle));
    m_timeMultiplier = m_rcInfo.startTimeMultiplier +
                       REAL_TO_INT_FLOOR(0.5 + (m_rcInfo.endTimeMultiplier -
                                                m_rcInfo.startTimeMultiplier) *
                                                   factor);
  } else if (m_rcInfo.curFrame <= m_rcInfo.numFrames + m_rcInfo.numHoldFrames &&
             m_rcInfo.trackObject) {
    const Object *obj = TheGameLogic->findObjectByID(m_rcInfo.targetObjectID);
    if (obj) {
      // object has not been destroyed
      m_rcInfo.targetObjectPos = *obj->getPosition();
...
void W3DView::pitchCameraOneFrame(void) {
  ...
  if (m_pcInfo.curFrame <= m_pcInfo.numFrames) {
    ...
    m_FXPitch = m_pcInfo.startPitch +
                (m_pcInfo.endPitch - m_pcInfo.startPitch) * factor;
  }
...
void W3DView::moveAlongWaypointPath(Int milliseconds) {
  ...
  if (m_mcwpInfo.elapsedTimeMilliseconds > m_mcwpInfo.totalTimeMilliseconds) {
    ...
    /////////////////////m_cameraOffset.z =
    ///m_groundLevel+TheGlobalData->m_cameraHeight;
    m_cameraOffset.y =
        -(m_cameraOffset.z / tan(TheGlobalData->m_cameraPitch * (PI / 180.0)));
    m_cameraOffset.x =
        -(m_cameraOffset.y * tan(TheGlobalData->m_cameraYaw * (PI / 180.0)));
    ...
    pos.z = 0;
    setPosition(&pos);
    // Note - assuming that the scripter knows what he is doing, we adjust the
    // constraints so that the scripted action can occur.
  ...
  Real distance =
      m_mcwpInfo.totalDistance * deltaTime / m_mcwpInfo.totalTimeMilliseconds;
  m_mcwpInfo.curSegDistance += distance;
  while (m_mcwpInfo.curSegDistance >
         m_mcwpInfo.waySegLength[m_mcwpInfo.curSegment]) {
  ...
  m_mcwpInfo.curShutter = m_mcwpInfo.shutter;
  Real factor = m_mcwpInfo.curSegDistance /
                m_mcwpInfo.waySegLength[m_mcwpInfo.curSegment];
  ...
  */
  ...
  Real timeMultiplier =
      m_mcwpInfo.timeMultiplier[m_mcwpInfo.curSegment] * factor1 +
  ...
  /*
          ...
          DEBUG_LOG(("Dx %.2f, dy %.2f, DeltaANgle = %.2f, %.2f DeltaGround
     %.2f\n", m_pos.x-result.x, m_pos.y-result.y, deltaAngle, m_groundLevel,
     m_groundLevel-prevGround)); prevGround = m_groundLevel;
  */
  setPosition(&result);
  // Note - assuming that the scripter knows what he is doing, we adjust the
  // constraints so that the scripted action can occur.
...
void W3DView::shake(const Coord3D *epicenter, CameraShakeType shakeType) {
  ...
  if (dist > TheGlobalData->m_maxShakeRange) return;
...
//-------------------------------------------------------------------------------------------------
/** Transformt he screen pixel coord passed in, to a world coordinate at the
 * specified z value */
//-------------------------------------------------------------------------------------------------
