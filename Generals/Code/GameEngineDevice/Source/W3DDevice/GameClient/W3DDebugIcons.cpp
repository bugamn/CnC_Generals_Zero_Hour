...
void addIcon(const Coord3D *pos, Real width, Int numFramesDuration,
             RGBColor color) {
...
bool W3DDebugIcons::Cast_Ray(RayCollisionTestClass &raytest) { return false; }

//@todo: MW Handle both of these properly!!
W3DDebugIcons::W3DDebugIcons(const W3DDebugIcons &src) { *this = src; }
...
void W3DDebugIcons::addIcon(const Coord3D *pos, Real width,
                            Int numFramesDuration, RGBColor color) {
  if (pos == NULL) {
  ...
  m_debugIcons[m_numDebugIcons].endFrame =
      TheGameLogic->getFrame() + numFramesDuration;
  m_numDebugIcons++;
}
...
void W3DDebugIcons::Render(RenderInfoClass &rinfo) {
  ...
  for (k = 0; k < m_numDebugIcons;) {
    ...
    DX8Wrapper::Draw_Triangles(0, curIndex / 3, 0,
                               numVertex);  // draw a quad, 2 triangles, 4 verts
  }
