...
// #define SHROUD_COLOR	0x00ffffff //temporary test of gray shroud instead of
// pure black. #define MIN_SHROUD_LEVEL	0		//for gray fog

// Int SHROUD_COLOR=0x00808080; //temporary test of gray shroud instead of pure
// black. Int MIN_SHROUD_LEVEL=50;		//for gray fog

// #define SHROUD_COLOR	0x00eeeebff //temporary test of gray shroud instead of
// pure black. #define MIN_SHROUD_LEVEL	254		//for gray fog

// #define SHROUD_COLOR	0x00bbbbbb //temporary test of gray shroud instead of
// pure black. #define SHROUD_COLOR	0x00004080 //temporary test of blue
// shroud instead of pure black.

// #define MIN_SHROUD_LEVEL	100		//for black fog

// #define MAX_MAP_SHROUDSIZE	1024	//maximum number of shroud cells across
// entire map. #define MAX_VISIBLE_SHROUDSIZE	(MAX_MAP_SHROUDSIZE+1)
// //maximum number of shroud vertices visible at any given time
#define DEFAULT_SHROUD_CELL_SIZE \
  MAP_XY_FACTOR  // assume shroud at same resolution as terrain cells.
#define DEFAULT_TERRAIN_SIZE \
  1024  // assumed size of largest terrain possible (in vertices)
...
W3DShroud::W3DShroud(void) {
  ...
  m_boderShroudLevel =
      (W3DShroudLevel)TheGlobalData->m_shroudAlpha;  // assume border is black
...
W3DShroud::~W3DShroud(void) {
  ReleaseResources();
  if (m_pSrcTexture) m_pSrcTexture->Release();
  m_pSrcTexture = NULL;
  if (m_finalFogData) delete[] m_finalFogData;
  if (m_currentFogData) delete[] m_currentFogData;
  m_drawFogOfWar = FALSE;
}

//-----------------------------------------------------------------------------
/**Called to initialize a new shroud for a new map.  Should be done after the
   map is loaded into the terrain object.  worldCellSize is the world-space
   dimensions of each shroud cell. The system will generate enough cells to
   cover the full map.
*/
void W3DShroud::init(WorldHeightMap *pMap, Real worldCellSizeX,
                     Real worldCellSizeY) {
  DEBUG_ASSERTCRASH(m_pSrcTexture == NULL,
                    ("ReAcquire of existing shroud textures"));
  ...
  if (pMap) {
    m_numCellsX = REAL_TO_INT_CEIL(
        (Real)(pMap->getXExtent() - 1 - pMap->getBorderSize() * 2) *
        MAP_XY_FACTOR / m_cellWidth);
    m_numCellsY = REAL_TO_INT_CEIL(
        (Real)(pMap->getYExtent() - 1 - pMap->getBorderSize() * 2) *
        MAP_XY_FACTOR / m_cellHeight);

    // Maximum visible cells will depend on maximum drawable terrain size plus 1
    // for partial cells (since shroud cells are larger than terrain cells).
    dstTextureWidth = m_numMaxVisibleCellsX =
        REAL_TO_INT_FLOOR((Real)(pMap->getDrawWidth() - 1) * MAP_XY_FACTOR /
                          m_cellWidth) +
        1;
    dstTextureHeight = m_numMaxVisibleCellsY =
        REAL_TO_INT_FLOOR((Real)(pMap->getDrawHeight() - 1) * MAP_XY_FACTOR /
                          m_cellHeight) +
        1;
    dstTextureWidth += 2;  // enlarge by 2 pixels so we can have a border color
                           // all the way around.
    dstTextureHeight += 2;  // enlarge by 2 pixels so we can have border color
                            // all the way around.
    TextureLoader::Validate_Texture_Size((unsigned int &)dstTextureWidth,
                                         (unsigned int &)dstTextureHeight);
  }

  UnsignedInt srcWidth, srcHeight;

  srcWidth = m_numCellsX;
  // vertical size is larger by 1 pixel so that we have some unused pixels to
  // use in clearing the video texture. To clear the video texture, I will copy
  // pixels from this unused area.  There is no other way to clear a video
  // memory texture to a known value because you can't lock it - only copy into
  // it.
  ...
  if (TheGlobalData && TheGlobalData->m_fogOfWarOn)
    m_pSrcTexture = DX8Wrapper::_Create_DX8_Surface(srcWidth, srcHeight,
  ...
  DEBUG_ASSERTCRASH(m_pSrcTexture != NULL,
                    ("Failed to Allocate Shroud Src Surface"));
  ...
  if (TheGlobalData && TheGlobalData->m_fogOfWarOn)
    fillShroudData(
        TheGlobalData->m_shroudAlpha);  // initialize shroud to a known value
#endif

  if (dstTextureWidth != m_dstTextureWidth ||
      dstTextureHeight != m_dstTextureHeight)  ///@todo: Check if size has
                                               ///changed - probably never
  ...
  if (ThePartitionManager) ThePartitionManager->refreshShroudForLocalPlayer();
}
...
void W3DShroud::reset() {
  ...
  m_currentFogData = NULL;
  m_clearDstTexture =
      TRUE;  // always refill the destination texture after a reset
}

//-----------------------------------------------------------------------------
/// Release any resources that can't survive a D3D device reset.
void W3DShroud::ReleaseResources(void) { REF_PTR_RELEASE(m_pDstTexture); }

//-----------------------------------------------------------------------------
/// Restore resources that are lost on D3D device reset.
Bool W3DShroud::ReAcquireResources(void) {
  if (!m_dstTextureWidth)
    return TRUE;  // nothing to reaquire since shroud was never initialized with
                  // valid data

  DEBUG_ASSERTCRASH(m_pDstTexture == NULL,
                    ("ReAcquire of existing shroud texture"));

  // Create destination texture (stored in video memory).
  // Since we control the video memory copy, we can do partial updates more
  // efficiently. Or do shift blits.
#if defined(_DEBUG) || defined(_INTERNAL)
  if (TheGlobalData && TheGlobalData->m_fogOfWarOn)
    m_pDstTexture = MSGNEW("TextureClass") TextureClass(
        m_dstTextureWidth, m_dstTextureHeight, WW3D_FORMAT_A4R4G4B4,
        TextureClass::MIP_LEVELS_1, TextureClass::POOL_DEFAULT);
  else
#endif
    m_pDstTexture = MSGNEW("TextureClass")
        TextureClass(m_dstTextureWidth, m_dstTextureHeight, WW3D_FORMAT_R5G6B5,
                     TextureClass::MIP_LEVELS_1, TextureClass::POOL_DEFAULT);

  DEBUG_ASSERTCRASH(m_pDstTexture != NULL,
                    ("Failed ReAcquire of shroud texture"));

  if (!m_pDstTexture) {  // could not create a valid texture
    m_dstTextureWidth = 0;
  ...
  m_clearDstTexture =
      TRUE;  // force clearing of destination texture first time it's used.
...
W3DShroudLevel W3DShroud::getShroudLevel(Int x, Int y) {
  ...
  if (x < m_numCellsX && y < m_numCellsY) {
    UnsignedShort pixel = *(UnsignedShort *)((Byte *)m_srcTextureData + x * 2 +
                                             y * m_srcTexturePitch);
...
//-----------------------------------------------------------------------------
void W3DShroud::setShroudLevel(Int x, Int y, W3DShroudLevel level,
                               Bool textureOnly) {
  DEBUG_ASSERTCRASH(
      m_pSrcTexture != NULL,
      ("Writing empty shroud.  Usually means that map failed to load."));

  if (!m_pSrcTexture) return;
  ...
  if (x < m_numCellsX && y < m_numCellsY) {
    ...
    if (TheGlobalData && TheGlobalData->m_fogOfWarOn) {
      ...
      UnsignedShort pixel =
          ((blueVal >> 4) & 0xf) | (((greenVal >> 4) & 0xf) << 4) |
          (((redVal >> 4) & 0xf) << 8) | (((alphaVal >> 4) & 0xf) << 12);
      *(UnsignedShort *)((Byte *)m_srcTextureData + x * 2 +
                         y * m_srcTexturePitch) = pixel;
    } else
#endif
    {
#ifdef DO_FOG_INTERPOLATION
      if (!textureOnly) m_finalFogData[x + y * m_numCellsX] = level;
#endif
      UnsignedInt bluepixel =
          (UnsignedInt)((Real)level *
                        ((Real)(TheGlobalData->m_shroudColor.getAsInt() &
                                0xff) /
                         255.0f));
      UnsignedInt greenpixel =
          (UnsignedInt)((Real)level *
                        ((Real)((TheGlobalData->m_shroudColor.getAsInt() &
                                 0xff00) >>
                                8) /
                         255.0f));
      UnsignedInt redpixel =
          (UnsignedInt)((Real)level *
                        ((Real)((TheGlobalData->m_shroudColor.getAsInt() &
                                 0xff0000) >>
                                16) /
                         255.0f));
      //			UnsignedInt bluepixel =
      //(UnsignedInt)((Real)level*((Real)(SHROUD_COLOR&0xff)/255.0f));
      //			UnsignedInt greenpixel =
      //(UnsignedInt)((Real)level*((Real)((SHROUD_COLOR&0xff00)>>8)/255.0f));
      //			UnsignedInt redpixel =
      //(UnsignedInt)((Real)level*((Real)((SHROUD_COLOR&0xff0000)>>16)/255.0f));
      if (level == 255) {  // unshrouded pixels should be fully lit
        redpixel = 255;
        greenpixel = 255;
        bluepixel = 255;
      }

      UnsignedShort *texel = (UnsignedShort *)((Byte *)m_srcTextureData +
                                               x * 2 + y * m_srcTexturePitch);

      //      For those interested, MLorenzen has this bock commented out until
      //      he gets back on Mon, Sept. 30 2002
      ...
      //			if (texelRed < texelGreen && texelGreen >
      //texelBlue)
      //			{
      //				bluepixel += redpixel;
      //				bluepixel -= redpixel;
      ...
      *texel = (((bluepixel & 0xf8) >> 3) | ((greenpixel & 0xfc) << 3) |
                ((redpixel & 0xf8) << 8));
    }
    return;
  }
...
void W3DShroud::fillShroudData(W3DShroudLevel level) {
  ...
  if (TheGlobalData && TheGlobalData->m_fogOfWarOn) {
    ...
    pixel = ((blueVal >> 4) & 0xf) | (((greenVal >> 4) & 0xf) << 4) |
            (((redVal >> 4) & 0xf) << 8) | (((alphaVal >> 4) & 0xf) << 12);
  } else
#endif
  {
    UnsignedInt bluepixel =
        (UnsignedInt)((Real)level *
                      ((Real)(TheGlobalData->m_shroudColor.getAsInt() & 0xff) /
                       255.0f));
    UnsignedInt greenpixel =
        (UnsignedInt)((Real)level *
                      ((Real)((TheGlobalData->m_shroudColor.getAsInt() &
    ...
    UnsignedInt redpixel =
        (UnsignedInt)((Real)level *
                      ((Real)((TheGlobalData->m_shroudColor.getAsInt() &
                       ...
                       255.0f));
    //		UnsignedInt bluepixel =
    //(UnsignedInt)((Real)level*((Real)(SHROUD_COLOR&0xff)/255.0f)); 		UnsignedInt
    //greenpixel =
    //(UnsignedInt)((Real)level*((Real)((SHROUD_COLOR&0xff00)>>8)/255.0f));
    //		UnsignedInt redpixel =
    //(UnsignedInt)((Real)level*((Real)((SHROUD_COLOR&0xff0000)>>16)/255.0f));
    ...
    pixel = (((bluepixel & 0xf8) >> 3) | ((greenpixel & 0xfc) << 3) |
             ((redpixel & 0xf8) << 8));
  }
  ...
  for (y = 0; y < m_numCellsY; y++) {
    for (x = 0; x < m_numCellsX; x++) ptr[x] = pixel;
    ptr += pitch;
  }

#ifdef DO_FOG_INTERPOLATION
  // Set the final shroud state.  May differe from current state because of time
  // interpolation.
  W3DShroudLevel *cptr = m_finalFogData;
  pitch = m_numCellsX;
  for (y = 0; y < m_numCellsY; y++) {
    for (x = 0; x < m_numCellsX; x++) cptr[x] = level;
    ptr += pitch;
  }
#endif
}
...
void W3DShroud::fillBorderShroudData(W3DShroudLevel level,
  ...
  UnsignedShort *ptr = (UnsignedShort *)m_srcTextureData +
                       m_numCellsY * (m_srcTexturePitch >> 1);
  ...
  for (y = 0; y < m_dstTextureHeight; y++) {
    ...
    for (x = 0; x < numFullCopies; x++) {
      ...
      DX8Wrapper::_Copy_DX8_Rects(m_pSrcTexture, &srcRect, 1,
                                  pDestSurface->Peek_D3D_Surface(), &dstPoint);
    }
    if (numExtraPixels) {
...
void W3DShroud::render(CameraClass *cam) {
  ...
  DEBUG_ASSERTCRASH(m_pSrcTexture != NULL,
                    ("Updating unallocated shroud texture"));
  ...
  if (doInit) {
    ...
    DummyTexture =
        WW3DAssetManager::Get_Instance()->Get_Texture("shroud1024.tga");

    Short *dataDest =
        (Short *)((char *)m_srcTextureData);  // offset to correct row of full
                                              // sysmem shroud
    Int pitchDest =
        m_srcTexturePitch >> 1;  // 2 bytes per pixel so divide byte count by 2.

    // Copy the dummy shroud into our game shroud.
    SurfaceClass *pSurface = DummyTexture->Get_Surface_Level(0);
    Int pitch;
    Int *dataSrc = (Int *)((char *)pSurface->Lock(
        &pitch));  // offset to correct row of full sysmem shroud
    ...
    for (Int y = 0; y < desc.Height; y++) {
      for (Int x = 0; x < desc.Width; x++) {
        ...
        if (TheGlobalData && TheGlobalData->m_fogOfWarOn) {
          dataDest[x] =
              ((TheGlobalData->m_shroudColor.getAsInt() >> 4) & 0xf) |
              (((TheGlobalData->m_shroudColor.getAsInt() >> 12) & 0xf) << 4) |
              (((TheGlobalData->m_shroudColor.getAsInt() >> 20) & 0xf) << 8) |
              ((((255 - (dataSrc[x] & 0xff)) >> 4) & 0xf) << 12);
          //					dataDest[x]=((SHROUD_COLOR>>4)&0xf)
          //| (((SHROUD_COLOR>>12)&0xf)<<4) | (((SHROUD_COLOR>>20)&0xf)<<8) |
          //((((255-(dataSrc[x] & 0xff))>>4)&0xf)<<12);
        } else
#endif
        {
          dataDest[x] = ((dataSrc[x] >> 3) & 0x1f) |
                        (((dataSrc[x] >> 10) & 0x3f) << 5) |
                        (((dataSrc[x] >> 19) & 0x1f) << 11);
        }
      }
  ...
  Int visStartX = REAL_TO_INT_FLOOR(
      (Real)(hm->getDrawOrgX() - hm->getBorderSize()) * MAP_XY_FACTOR /
      m_cellWidth);  // start of rendered heightmap rectangle
  if (visStartX < 0)
    visStartX =
        0;  // no shroud is applied in border area so it always starts at > 0
  Int visStartY =
      REAL_TO_INT_FLOOR((Real)(hm->getDrawOrgY() - hm->getBorderSize()) *
                        MAP_XY_FACTOR / m_cellHeight);
  if (visStartY < 0)
    visStartY =
        0;  // no shroud is applied in border area so it always starts at > 0
  Int visEndX = visStartX +
                REAL_TO_INT_FLOOR((Real)(hm->getDrawWidth() - 1) *
                                  MAP_XY_FACTOR / m_cellWidth) +
                1;  // size of rendered heightmap rectangle
  Int visEndY = visStartY +
                REAL_TO_INT_FLOOR((Real)(hm->getDrawHeight() - 1) *
                                  MAP_XY_FACTOR / m_cellHeight) +
                1;
  ...
  if (visEndX > m_numCellsX) {
    visStartX -=
        visEndX -
        m_numCellsX;  // shift visible rectangle to fall within terrain bounds
    if (visStartX < 0) visStartX = 0;
    visEndX = m_numCellsX;
  }
  ...
  if (visEndY > m_numCellsY) {
    visStartY -=
        visEndY -
        m_numCellsY;  // shift visible rectangle to fall within terrain bounds
    if (visStartY < 0) visStartY = 0;
    visEndY = m_numCellsY;
  }
  ...
  // If system memory usage becomes too large, we should store shroud as Bytes.
  // Update a system memory texture with data.  Then copy this to video memory.
  // For now we're holding shroud data directly in a texture to avoid the extra
  // copy.
  /*	pSurface=m_pSrcTexture->Get_Surface_Level(0);
          data=(Short *)((char *)pSurface->Lock(&pitch) + visStartY*pitch);
     //offset to correct row of full sysmem shroud pitch >>= 1;	//we have 2
     bytes per pixel, so divide pitch by 2

          Byte *sd=shroudData+visStartY*MAX_MAP_SHROUDSIZE;	//pointer to
     first shroud row
  ...
  */
  ...
  if (m_clearDstTexture) {  // we need to clear unused parts of the destination
                            // texture to a known
...
#define FOG_INTERPOLATION_RATE \
  (255.0f / 1000.0f)  // take one second to go from black to fully lit.
//-----------------------------------------------------------------------------
void W3DShroud::interpolateFogLevels(RECT *rect) {
  ...
  if (!timeDiff) return;  // no time has elapsed

  prevTime += timeDiff;  // update for next frame

  Int maxFogChange =
      FOG_INTERPOLATION_RATE *
      (Real)timeDiff;  // maximum amount of fog change allowed in frame.
  if (maxFogChange > 255) maxFogChange = 255;
  ...
  for (Int j = 0; j < m_numCellsY; j++) {
    for (Int i = 0; i < m_numCellsX; i++, startLevel++, finalLevel++)
      if (*startLevel != *finalLevel) {  // fog needs fading.
        if (*startLevel == *finalLevel)
          continue;
        else if (*finalLevel < *startLevel) {
          if ((*startLevel - *finalLevel) < levelDelta)
            *startLevel =
                *finalLevel;  // change too large so clamp to final value.
          else
