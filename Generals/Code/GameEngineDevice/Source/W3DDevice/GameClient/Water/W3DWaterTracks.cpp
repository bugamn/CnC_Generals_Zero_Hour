...
//			  some simple animation : dynamic uv coordinates,
//scaling, scrolling, 			  and alpha.
//-----------------------------------------------------------------------------
...
// while one is being rendered, another is being updated.  Improves HW
// parallelism.
...
// #define DEFAULT_SECOND_WAVE_TIME_OFFSET 6267	//should always be half of
// totalMs

WaterTracksRenderSystem *TheWaterTracksRenderSystem =
    NULL;  ///< singleton for track drawing system.

static Bool pauseWaves = FALSE;

enum waveType : unsigned int {
  WaveTypeFirst,
...
struct waveInfo {
  ...
  Real m_initialWidthFraction;  // fraction of m_finalWidth when wave first
                                // appears.
  Real m_initialHeightWidthFraction;  // fraction of initial width to use as the
                                      // initial height.
  Int m_timeToCompress;  // time for back of wave to continue moving forward
                         // after front starts retreating.
  Int m_secondWaveTimeOffset;  // time for second wave to start.  Should always
                               // be half of first wave's TotalMs.
...
waveInfo waveTypeInfo[WaveTypeMax] = {
    {28.0f, 18.0f, 25.0f, 0.018f, 900, 0.01f, 0.18f, 1500, 0, "wave256.tga",
     "Pond"},  // pond
    {55.0f, 36.0f, 80.0f, 0.015f, 2000, 0.5f, 0.18f, 1000, 6267, "wave256.tga",
     "Ocean"},  // ocean
    {55.0f, 36.0f, 80.0f, 0.015f, 2000, 0.05f, 0.18f, 1000, 6267, "wave256.tga",
     "Close Ocean"},
    {55.0f, 36.0f, 80.0f, 0.015f, 4000, 0.01f, 0.18f, 2000, 6267, "wave256.tga",
     "Close Ocean Double"},
    {55.0f, 27.0f, 80.0f, 0.015f, 2000, 0.01f, 8.0f, 2000, 5367, "wave256.tga",
     "Radial"},
};
...
//=============================================================================
WaterTracksObj::~WaterTracksObj(void) { freeWaterTracksResources(); }
...
//=============================================================================
void WaterTracksObj::Get_Obj_Space_Bounding_Sphere(SphereClass &sphere)
    const {  /// @todo: Add code to cull track marks to screen by constantly
 ...
 */
//=============================================================================
void WaterTracksObj::init(Real width, Real length, Vector2 &start, Vector2 &end,
                          Char *texturename, Int waveTimeOffset) {
  ...
  m_perpDir.Rotate(-1.57079632679f);  // get vector perpendicular to wave
                                      // motion.
  m_perpDir.Normalize();
  ...
  m_waveDistance =
      waveTypeInfo[m_type]
          .m_waveDistance;  // total distance traveled by wave front

  m_waveDir *= m_waveDistance;
  m_startPos -= m_waveDir;  // move start point down away from shoreline

  m_initialVelocity = waveTypeInfo[m_type].m_initialVelocity;  // velocity per
                                                               // ms
  m_totalMs =
      m_waveDistance /
      m_initialVelocity;  // amount of time for wave to travel complete distance

  m_fadeMs =
      waveTypeInfo[m_type]
          .m_fadeMs;  // time for wave to fade out after it stops on beach

  m_waveInitialWidth =
      length *
      waveTypeInfo[m_type].m_initialWidthFraction;  ///< width of wave segment
                                                    ///< when it first appears
  m_waveInitialHeight =
      m_waveInitialWidth *
      waveTypeInfo[m_type]
          .m_initialHeightWidthFraction;  ///< height of wave segment when it
                                          ///< first appears
  ...
  m_frontSlowDownAcc =
      -(m_initialVelocity * m_initialVelocity) /
      (2 * m_waveFinalHeight);  // deceleration of wave after it hits land
  m_timeToStop = -m_initialVelocity / m_frontSlowDownAcc;
  m_timeToRetreat = sqrt(fabs(2.0f * m_waveFinalHeight / m_frontSlowDownAcc));
  m_totalMs = m_timeToReachBeach + m_timeToStop +
              m_timeToRetreat;  // total time that wave front is on screen
  m_backSlowDownAcc =
      (2.0f * m_waveInitialHeight /
       (m_timeToStop *
        m_timeToStop));  //((m_waveInitialHeight -
                         //m_velocity*m_timeToStop)*2.0f)/(m_timeToStop*m_timeToStop);
  m_timeToCompress =
      waveTypeInfo[m_type]
          .m_timeToCompress;  // time for back of wave to continue moving
                              // forward after front starts retreating.

  if (m_type == WaveTypeStationary) {  // this is a stationary wave slightly
                                       // behind starting point
    m_timeToRetreat = 1000;  // time to fade out.
    m_totalMs = m_timeToReachBeach + m_timeToStop + m_fadeMs +
                m_timeToRetreat;  // trigger when other wave stops.
    m_startPos = start;
    m_fadeMs = 1000;  // time for wave to fade out after it stops on beach
  }

  m_stageZeroTexture =
      WW3DAssetManager::Get_Instance()->Get_Texture(texturename);
}
 ...
 *	(start, end) define a vector perpendicular to wave travel.  The length
 *of this vector is used as the length of the wave segment.  The line between
 *start-end defines the maximum distance the wave will reach.
 */
//=============================================================================
void WaterTracksObj::init(Real width, Vector2 &start, Vector2 &end,
                          Char *texturename) {
  freeWaterTracksResources();  // free old resources used by this track
  m_boundingSphere.Init(Vector3(0, 0, 0), 400);
  m_boundingBox.Center.Set(0.0f, 0.0f, 0.0f);
...
//	else
  ...
  ///@todo: Should check in here if we are done with this object are return
  ///FALSE to free it.
  //	return FALSE;
...
Int WaterTracksObj::render(DX8VertexBufferClass *vertexBuffer, Int batchStart) {
  ...
  if (batchStart <
      (WATER_VB_PAGES * WATER_STRIP_X * WATER_STRIP_Y -
       m_x * m_y)) {  // we have room in current VB, append new verts
    if (vertexBuffer->Get_DX8_Vertex_Buffer()->Lock(
            batchStart * vertexBuffer->FVF_Info().Get_FVF_Size(),
            m_x * m_y * vertexBuffer->FVF_Info().Get_FVF_Size(),
            (unsigned char **)&vb, D3DLOCK_NOOVERWRITE) != D3D_OK)
      return batchStart;
  } else {  // ran out of room in last VB, request a substitute VB.
    if (vertexBuffer->Get_DX8_Vertex_Buffer()->Lock(
            0, m_x * m_y * vertexBuffer->FVF_Info().Get_FVF_Size(),
            (unsigned char **)&vb, D3DLOCK_DISCARD) != D3D_OK)
      return batchStart;
    batchStart = 0;  // reset start of page to first vertex
  }

  // Adjust wave position in a non-linear way so that it slows down as it hits
  // the target.  Using 1/4 sine wave seems to work okay since it maxes out
  // at 1.0 at our final position. Real
  // timeFrac=(Real)m_elapsedMs/(Real)m_totalMs;//sinf(0.5f*3.14159f*(Real)m_elapsedMs/(Real)m_totalMs);

  // Real
  // displacement=m_elapsedMs*waveInitialV+0.5*waveAcceleration*m_elapsed*m_elapsed;

  heightFrac = 1.0f;
  widthFrac = 1.0f;

  if (m_type == WaveTypeStationary) {  // stationary wave
    waveFrontOrigin = m_startPos;
    waveFrontOrigin -=
        m_perpDir * m_waveFinalWidth * 0.5f;  // offset to left edge of wave
    waveTailOrigin =
        waveFrontOrigin - m_waveFinalHeight * ooWaveDirLen * m_waveDir;
    waveAlpha = 0.0f;

    if (m_elapsedMs >= m_totalMs) m_elapsedMs = 0;  // done with effect*/
    if (m_elapsedMs >
        (m_timeToReachBeach + m_timeToStop - 1000 + m_fadeMs)) {  // fading out
      waveAlpha =
          m_elapsedMs - (m_timeToReachBeach + m_timeToStop - 1000 +
                         m_fadeMs);  //(m_totalMs-m_timeToRetreat -m_fadeMs -
                                     //m_elapsedMs)/m_fadeMs;
      waveAlpha = waveAlpha / m_timeToRetreat;
      waveAlpha = 1.0f - waveAlpha;
      if (waveAlpha < 0.0f) waveAlpha = 0.0f;
    } else if (m_elapsedMs >
               (m_timeToReachBeach + m_timeToStop - 1000)) {  // start fading up

      waveAlpha = m_elapsedMs - (m_timeToReachBeach + m_timeToStop -
                                 1000);  //(m_totalMs-m_timeToRetreat -m_fadeMs
                                         //- m_elapsedMs)/m_fadeMs;
    ...
    if (m_elapsedMs <
        m_timeToReachBeach) {  // wave has not reached beach yet so position
                               // only depends on velocity
      waveAlpha = m_elapsedMs / m_timeToReachBeach;
      widthFrac = waveAlpha;
      widthFrac = (m_waveInitialWidth +
                   widthFrac * (m_waveFinalWidth - m_waveInitialWidth)) /
                  m_waveFinalWidth;

      waveFrontOrigin = m_startPos + m_initialVelocity * m_elapsedMs *
                                         ooWaveDirLen * m_waveDir;
      waveFrontOrigin -= m_perpDir * m_waveFinalWidth * 0.5f *
                         widthFrac;  // offset to left edge of wave
      // Tail of wave will be behind the front by fixed amount.
      waveTailOrigin =
          waveFrontOrigin - m_waveInitialHeight * ooWaveDirLen * m_waveDir;
    } else  // wave has reached beach and is decelerating
      if (m_elapsedMs < m_totalMs) {
        waveAlpha = 1.0f;
        widthFrac = 1.0f;
        // Get position of wave when it hit the beach
        waveFrontOrigin = m_startPos + m_initialVelocity * m_timeToReachBeach *
                                           ooWaveDirLen * m_waveDir;
        waveTailOrigin =
            waveFrontOrigin;  // store position for calculating tail position
        // Add movement after it hit the beach
        Real elapsedMs = m_elapsedMs - m_timeToReachBeach;
        waveFrontOrigin += (m_initialVelocity * elapsedMs +
                            0.5f * m_frontSlowDownAcc * elapsedMs * elapsedMs) *
                           ooWaveDirLen * m_waveDir;
        waveFrontOrigin -= m_perpDir * m_waveFinalWidth * 0.5f *
                           widthFrac;  // offset to left edge of wave

        Real timeSinceBacktrack =
            m_elapsedMs - m_timeToReachBeach - m_timeToStop;
        ...
        if (m_elapsedMs >
            (m_timeToReachBeach + m_timeToStop + m_timeToCompress)) {
          elapsedMs = elapsedMs;  ///@todo: bug?
          waveTailOrigin +=
              (0.5f * m_backSlowDownAcc * (m_timeToStop + m_timeToCompress) *
               (m_timeToStop + m_timeToCompress)) *
              ooWaveDirLen * m_waveDir;
          // get time since wave should have stopped moving forward
          Real newElapsed = m_elapsedMs - (m_timeToReachBeach + m_timeToStop +
                                           m_timeToCompress);
          waveTailOrigin +=
              (0.5f * m_frontSlowDownAcc * newElapsed * newElapsed) *
              ooWaveDirLen * m_waveDir;
        } else
          //		if (m_elapsedMs < (m_totalMs-m_timeToRetreat))
          // find position of tail including slowdown after it hit the beach
          //		waveTailOrigin +=
          //(m_initialVelocity*elapsedMs+0.5f*m_backSlowDownAcc*elapsedMs*elapsedMs)*ooWaveDirLen*m_waveDir;
          waveTailOrigin += (0.5f * m_backSlowDownAcc * elapsedMs * elapsedMs) *
                            ooWaveDirLen * m_waveDir;

        waveTailOrigin -= m_perpDir * m_waveFinalWidth * 0.5f *
                          widthFrac;  // offset to left edge of wave
      } else {
        m_elapsedMs = 0;
        waveAlpha = m_elapsedMs / m_timeToReachBeach;
  ...
  DX8Wrapper::Set_Index_Buffer(TheWaterTracksRenderSystem->m_indexBuffer,
                               batchStart);
  DX8Wrapper::Draw_Strip(
      0, idxCount - 2, 0,
      m_x * m_y);  // there are always n-2 primitives for n index strip.

  return batchStart +
         m_x * m_y;  // return new offset into unused area of vertex buffer
}
...
/** Grab a track from the free store. If no free tracks exist, return NULL.
        As long as a track is bound to an object (like a tank) it is ready to
   accept updates with additional edges.  Once it is unbound, it will expire and
   return to the free store once all tracks have faded out.
*/
//=============================================================================
WaterTracksObj *WaterTracksRenderSystem::bindTrack(waveType type) {
  ...
  if (mod) {
    ...
    for (nextmod = m_usedModules; nextmod;
         prevmod = nextmod, nextmod = nextmod->m_nextSystem) {
      if (nextmod->m_type == type) {  // found start of other shadows using same
                                      // texture, insert new shadow here.
    ...
    if (nextmod == NULL) {  // shadow with new texture. Add to top of list.
...
void WaterTracksRenderSystem::ReAcquireResources(void) {
  ...
  // Will also need 2 extra indices to connect each strip to next one (except
  // last strip) Total index buffer size = (m_y-1)*(m_x*2+2) - 2 (drop the extra
  // 2 indices from last strip)
  ...
  // Fill up the IB
  {
    ...
    for (i = 0, j = 0, k = 0; i < idxCount; j++) {
      ...
      // Generate 4 degenerate triangle to connect current strip to next
      // strip/row of map To do this, we just repeat the last index of first
      // strip and first index of new strip. Any triangles with repeated
      // vertices will be skipped during rendering.
  ...
  m_vertexBuffer =
      NEW_REF(DX8VertexBufferClass,
              (DX8_FVF_XYZDUV1, m_stripSizeX * m_stripSizeY * WATER_VB_PAGES,
...
void WaterTracksRenderSystem::shutdown(void) {
  ...
  while (mod) {
    ...
    if (!mod->m_bound) releaseTrack(mod);
...
void WaterTracksRenderSystem::update() {
  ...
  while (mod) {
    ...
    if (!mod->m_bound ||
        (!mod->update(timeDiff) &&
         !mod->m_bound)) {  // object is not longer updating and is unbound so
                            // ok to release it.
      releaseTrack(mod);
    }
...
void WaterTracksRenderSystem::flush(RenderInfoClass &rinfo) {
  /** @todo: Optimize system by drawing tracks as triangle strips and use
  dynamic vertex buffer access. May also try rendering all tracks with one call
  to W3D/D3D by grouping them by texture. Try improving the fit to vertical
  surfaces like cliffs.
  */
  Int diffuseLight;

  if (TheGlobalData->m_usingWaterTrackEditor) TestWaterUpdate();
  ...
  diffuseLight = REAL_TO_INT(shadeB) | (REAL_TO_INT(shadeG) << 8) |
...
WaterTracksObj *WaterTracksRenderSystem::findTrack(Vector2 &start, Vector2 &end,
  ...
  while (mod) {
    if (mod->m_initEndPos == end && mod->m_initStartPos == start &&
        mod->m_type == type)
      return mod;
    mod = mod->m_nextSystem;
  }  // while (mod)
...
void WaterTracksRenderSystem::saveTracks(void) {
  ...
  if (fp) {
    umod = m_usedModules;
    while (umod) {
      if (umod->m_initTimeOffset ==
          0) {  // only save the primary wave front, second layer is added
                // automatically.
...
void WaterTracksRenderSystem::loadTracks(void) {
  ...
  if (file) {
    ...
    tryagain:
      ...
      if (umod) {  // umod->init(1.5f*MAP_XY_FACTOR,Vector2(0,0),Vector2(1,1),"wave256.tga");
        ...
        if (waveTypeInfo[wtype]
                .m_secondWaveTimeOffset)  // check if we need a second wave to
                                          // follow
        {
          umod = TheWaterTracksRenderSystem->bindTrack(wtype);
          if (umod) {
...
/**@todo: this is a quick hack for adding/removing/testing breaking waves inside
the client. Will need to move this code to an external editor at some pont. */
#include "GameClient/Display.h"

extern HWND ApplicationHWnd;

// TODO: Fix editor so it actually draws the wave segment instead of line while
// editing Could freeze all the water while editing?  Or keep setting elapsed
// time on current segment. Have to make it so seamless merge of segments at
// final position.
static void TestWaterUpdate(void) {
  ...
  if (GetAsyncKeyState(VK_F5) & 0x8001)  // check if F5 pressed since last call
  {
    if (trackEditModeReset) {
      if (trackEditMode) {
        ...
        string.format(L"Wave Type: %hs",
                      waveTypeInfo[currentWaveType].m_waveTypeName);
        TheInGameUI->message(string);
      }

      trackEditMode ^= 1;  // toggle editor on/off

      if (trackEditMode == 0) {  // editor was turned off, save changes
        haveStart = 0;
        haveEnd = 0;
      }
  ...
  if (trackEditMode) {  // we are in wave edit mode
    ...
    if (GetCursorPos(&screenPoint))  // read mouse position
    {
      ...
      if (GetAsyncKeyState(VK_F6) & 0x8001) {
        if (addPointReset) {
          if (!haveStart) {
            ...
            if (track) {  //	track->init(1.5f*MAP_XY_FACTOR,Vector2(terrainPointStart.x,terrainPointStart.y),Vector2(terrainPointEnd.x,terrainPointEnd.y),"wave256.tga");
              ...
              m_perpDir.Rotate(
                  1.57079632679f);  // get vector perpendicular to wave motion.
              m_perpDir.Normalize();
              midPoint = startPoint + (midPoint) * 0.5f;
              Vector2 dirMidPoint = midPoint + m_perpDir;

              track->init(waveTypeInfo[currentWaveType].m_finalHeight,
                          waveTypeInfo[currentWaveType].m_finalWidth,
                          Vector2(midPoint.X, midPoint.Y),
                          Vector2(dirMidPoint.X, dirMidPoint.Y),
                          waveTypeInfo[currentWaveType].m_textureName, 0);
              ...
              if (waveTypeInfo[currentWaveType].m_secondWaveTimeOffset) {
                ...
                if (track2) {
                  track2->init(
                      waveTypeInfo[currentWaveType].m_finalHeight,
                      waveTypeInfo[currentWaveType].m_finalWidth,
                      Vector2(midPoint.X, midPoint.Y),
                      Vector2(dirMidPoint.X, dirMidPoint.Y),
                      waveTypeInfo[currentWaveType].m_textureName,
                      waveTypeInfo[currentWaveType].m_secondWaveTimeOffset);
                }
              }
      ...
      if (GetAsyncKeyState(VK_DELETE) & 0x8001) {  // delete last segment added
        if (deleteTrackReset && track) {
          deleteTrackReset = 0;
          TheWaterTracksRenderSystem->unbindTrack(track);
          if (track2) TheWaterTracksRenderSystem->unbindTrack(track2);
          haveStart = 0;  // reset for next segment
          haveEnd = 0;
          track = NULL;
          track2 = NULL;
      ...
      if (GetAsyncKeyState(VK_INSERT) & 0x8001) {  // change current wave type
        if (changeTypeReset) {
          changeTypeReset = 0;
          currentWaveType = (waveType)((Int)currentWaveType + 1);
          if (currentWaveType > WaveTypeLast) currentWaveType = WaveTypeFirst;
      ...
      if (GetAsyncKeyState(VK_F7) & 0x8001) {  // save all segments added
        if (saveTracksReset) {
          saveTracksReset = 0;
      ...
      if (GetAsyncKeyState(VK_F8) & 0x8001) {  // load tracks for map
        if (loadTracksReset) {
          loadTracksReset = 0;
    ...
    if (haveStart && !haveEnd) {  // draw a guide line
      //			View *tacticalView = TheDisplay->getFirstView();
      //			tacticalView->worldToScreen( &m_moveHint[i].pos,
      //&pos );
      ...
      if (sqrt(xdiff * xdiff + ydiff * ydiff) <=
          waveTypeInfo[currentWaveType].m_finalWidth) {
        TheDisplay->drawLine(mouseAnchor.x, mouseAnchor.y, screenPoint.x,
                             screenPoint.y, 1, 0xffccccff);
      ...
      //			char buffer[64];
      //			sprintf(buffer,"\n%d,%d,%d,%d",mouseAnchor.x,
      //mouseAnchor.y, screenPoint.x, screenPoint.y); 			OutputDebugString
      //(buffer);
    }
  }
}
