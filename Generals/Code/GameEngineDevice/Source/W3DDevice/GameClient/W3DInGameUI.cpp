...
DebugHintObject::~DebugHintObject(void) { freeMapResources(); }
...
Bool DebugHintObject::Cast_Ray(RayCollisionTestClass &raytest) { return false; }

DebugHintObject::DebugHintObject(const DebugHintObject &src) { *this = src; }
...
void DebugHintObject::initData(void) {
  ...
  m_vertexBufferTile =
      NEW_REF(DX8VertexBufferClass,
              (DX8_FVF_XYZDUV1, 3, DX8VertexBufferClass::USAGE_DEFAULT));
...
void DebugHintObject::setLocAndColorAndSize(const Coord3D *loc, Int argb,
                                            Int size) {
  m_myLoc = *loc;
  m_myColor = argb;
  m_mySize = size;
  ...
  if (m_myLoc.z < 0 && TheTerrainRenderObject) {
    m_myLoc.z =
        TheTerrainRenderObject->getHeightMapHeight(m_myLoc.x, m_myLoc.y, NULL);
  }
...
static void loadText(char *filename, GameWindow *listboxText) {
  ...
  while (fgets(buffer, 1024, fp) != NULL) {
    ...
    line.trim();
    if (line.isEmpty()) line = UnicodeString(L" ");
...
void W3DInGameUI::init(void) {
  ...
  InGameUI::init();
  // for the beta, they didn't want the help menu showing up, but I left this as
  // a bock comment because we'll probably want to add this back in.
  /*
                  // create the MOTD
                  GameWindow *motd = TheWindowManager->winCreateFromScript(
     AsciiString("MOTD.wnd") ); if( motd )
                  {
                          NameKeyType listboxTextID =
     TheNameKeyGenerator->nameToKey( "MOTD.wnd:ListboxMOTD" ); GameWindow
     *listboxText = TheWindowManager->winGetWindowFromId(motd, listboxTextID);
...
void W3DInGameUI::draw(void) {
  ...
  if (m_isDragSelecting) drawSelectionRegion();
  ...
  if (TheDisplay) {
    ...
    for (view = TheDisplay->getFirstView(); view;
...
void W3DInGameUI::drawSelectionRegion(void) {
  ...
  TheDisplay->drawOpenRect(m_dragSelectRegion.lo.x, m_dragSelectRegion.lo.y,
...
void W3DInGameUI::drawMoveHints(View *view) {
  ...
  for (i = 0; i < MAX_MOVE_HINTS; i++) {
    ...
    if (elapsed <= 40) {
      ...
      //			if( view->pointInView( &m_moveHint[ i ].pos ==
      //FALSE ) 				continue;
      ...
      if (m_moveHintRenderObj[i] == NULL) {
        ...
        // create hint object
        hint = W3DDisplay::m_assetManager->Create_Render_Obj(
            TheGlobalData->m_moveHintName.str());

        AsciiString animName;
        animName.format("%s.%s", TheGlobalData->m_moveHintName.str(),
                        TheGlobalData->m_moveHintName.str());
        ...
        // note that 'anim' is returned from Get_HAnim with an AddRef, so we
        // don't need to addref it again. however, we do need to release the
        // contents of moveHintAnim (if any)
        REF_PTR_RELEASE(m_moveHintAnim[i]);
      ...
      if (m_moveHintRenderObj[i]->Is_Hidden() == 1) {
        ...
        if (m_moveHintAnim[i])
          m_moveHintRenderObj[i]->Set_Animation(m_moveHintAnim[i], 0,
                                                RenderObjClass::ANIM_MODE_ONCE);
      }

      // move this hint render object to the position and align with terrain
      Matrix3D transform;
      PathfindLayerEnum layer = TheTerrainLogic->alignOnTerrain(
          0, m_moveHint[i].pos, true, transform);

      Real waterZ;
      if (layer == LAYER_GROUND &&
          TheTerrainLogic->isUnderwater(m_moveHint[i].pos.x,
                                        m_moveHint[i].pos.y, &waterZ)) {
...
//-------------------------------------------------------------------------------------------------
void W3DInGameUI::drawAttackHints(View *view) {}  // end drawAttackHints
...
void W3DInGameUI::drawPlaceAngle(View *view) {
  ...
  if (!m_buildingPlacementAnchor) {
    m_buildingPlacementAnchor =
        W3DDisplay::m_assetManager->Create_Render_Obj("Locater01");

    // sanity
    if (!m_buildingPlacementAnchor) {
      DEBUG_CRASH(
          ("Unable to create BuildingPlacementAnchor (Locator01.w3d) -- cursor "
  ...
  if (!m_buildingPlacementArrow) {
    m_buildingPlacementArrow =
        W3DDisplay::m_assetManager->Create_Render_Obj("Locater02");

    // sanity
    if (!m_buildingPlacementArrow) {
      DEBUG_CRASH(
          ("Unable to create BuildingPlacementArrow (Locator02.w3d) -- cursor "
           "for placing buildings"));
      return;
    }
  }
  ...
  if (isPlacementAnchored() == FALSE) {
    if (anchorInScene) {
      // If our anchor is in the scene, remove it from the scene but don't
      // delete it.
      W3DDisplay::m_3DScene->Remove_Render_Object(m_buildingPlacementAnchor);
    }
    if (arrowInScene) {
      // If our arrow is in the scene, remove it from the scene but don't delete
      // it.
  ...
  // The proper way to orient the placement arrow is to copy the matrix from the
  // m_placeIcon[0]!
  if (anchorInScene) {
    if (m_placeIcon[0])
      m_buildingPlacementAnchor->Set_Transform(
          *m_placeIcon[0]->getTransformMatrix());
