...
// A W3D shader that does alpha, texturing, tests zbuffer, doesn't update
// zbuffer.
#define SC_ALPHA_DETAIL                                                        \
  (SHADE_CNST(                                                                 \
      ShaderClass::PASS_LEQUAL, ShaderClass::DEPTH_WRITE_DISABLE,              \
...
// The radius of the center of the road is 1.5 - outer radius 2.0, inner
// radius 1.0.
static const Real CORNER_RADIUS = 1.5f;

// The radius of the center of the road is 0.5 - outer radius 1.0, inner radius
// 0.0.
...
#define SC_ALPHA_2D ( SHADE_CNST(PASS_ALWAYS, DEPTH_WRITE_DISABLE,
COLOR_WRITE_ENABLE, \
        SRCBLEND_SRC_ALPHA, DSTBLEND_ONE_MINUS_SRC_ALPHA, FOG_DISABLE,
GRADIENT_DISABLE, \
        SECONDARY_GRADIENT_DISABLE, TEXTURING_ENABLE, DETAILCOLOR_DISABLE,
DETAILALPHA_DISABLE, \ ALPHATEST_DISABLE, CULL_MODE_ENABLE, DETAILCOLOR_DISABLE,
DE AILALPHA_DISABLE) ) ShaderClass
ShaderClass::_PresetAlpha2DShader(SC_ALPHA_2D);
*/

/** Calculate the sign of the cross product.  If the tails of the vectors are
both placed at 0,0, then the cross product can be interpreted as -1 means v2 is
to the right of v1, 1 means v2 is to the left of v1, and 0 means v2 is parallel
to v1. */
...
void RoadType::loadTexture(AsciiString path, Int ID) {
  ...
  m_vertexRoad = NEW_REF(DX8VertexBufferClass,
                         (DX8_FVF_XYZDUV1, TheGlobalData->m_maxRoadVertex + 4,
                          DX8VertexBufferClass::USAGE_DYNAMIC));
  m_indexRoad = NEW_REF(
      DX8IndexBufferClass,
      (TheGlobalData->m_maxRoadIndex + 4, DX8IndexBufferClass::USAGE_DYNAMIC));
  m_numRoadVertices = 0;
  m_numRoadIndices = 0;
...
void RoadType::loadTestTexture(void) {
  if (m_isAutoLoaded && m_uniqueID > 0 && !m_texturePath.isEmpty()) {
    /// @todo - delay loading textures and only load textures referenced by map.
    m_roadTexture = NEW_REF(
        TextureClass,
        (m_texturePath.str(), m_texturePath.str(), TextureClass::MIP_LEVELS_3));
    m_roadTexture->Set_Mip_Mapping(TextureClass::FILTER_TYPE_BEST);
...
//=============================================================================
Int RoadSegment::GetIndices(UnsignedShort *destination_ib, Int numToCopy,
                            Int offset) {
...
void RoadSegment::updateSegLighting(void) {
  ...
  for (i = 0; i < m_numVertex; i++) {
    ...
    m_vb[i].diffuse =
        (255 << 24) | TheTerrainRenderObject->getStaticDiffuse(x, y);
  }
}
...
//=============================================================================
void W3DRoadBuffer::loadTee(RoadSegment *pRoad, Vector2 loc1, Vector2 loc2,
                            Bool is4Way, Real scale) {
  Vector2 roadVector(loc2.X - loc1.X, loc2.Y - loc1.Y);
  Vector2 roadNormal(-roadVector.Y, roadVector.X);
  //	Real roadLen = scale;
  if (abs(roadVector.X) < MIN_ROAD_SEGMENT &&
      abs(roadVector.Y) < MIN_ROAD_SEGMENT) {
  ...
  loadFloatSection(pRoad, loc1, loc2 - loc1, teeFactor, left, teeFactor,
                   uOffset, vOffset, scale);
}
...
void W3DRoadBuffer::loadAlphaJoin(RoadSegment *pRoad, Vector2 loc1,
  ...
  loadFloat4PtSection(pRoad, loc1, roadNormal, roadVector, corners, uOffset,
                      vOffset, scale, uScale);
}
...
//=============================================================================
void W3DRoadBuffer::loadY(RoadSegment *pRoad, Vector2 loc1, Vector2 loc2,
                          Real scale) {
  Vector2 roadVector(loc2.X - loc1.X, loc2.Y - loc1.Y);
  Vector2 roadNormal(-roadVector.Y, roadVector.X);
  //	Real roadLen = scale;
...
//=============================================================================
void W3DRoadBuffer::loadH(RoadSegment *pRoad, Vector2 loc1, Vector2 loc2,
                          Bool flip, Real scale) {
  Vector2 roadVector(loc2.X - loc1.X, loc2.Y - loc1.Y);
  Vector2 roadNormal(-roadVector.Y, roadVector.X);
  //	Real roadLen = scale;
  if (abs(roadVector.X) < MIN_ROAD_SEGMENT &&
  ...
  if (flip) {
    corners[bottomLeft] =
        loc1 - roadNormal * 0.20f - roadVector * pRoad->m_widthInTexture / 2;
  } else {
    corners[bottomLeft] =
        loc1 - roadNormal * 0.8f - roadVector * pRoad->m_widthInTexture / 2;
  }
...
void W3DRoadBuffer::loadFloatSection(RoadSegment *pRoad, Vector2 loc,
                                     Vector2 roadVector, Real halfHeight,
                                     Real left, Real right, Real uOffset,
                                     Real vOffset, Real scale) {
  if (m_map == NULL) {
    return;
  }
  ...
  loadFloat4PtSection(pRoad, loc, roadNormal, roadVector, corners, uOffset,
                      vOffset, scale, scale);
}
...
the road vector gives the direction of the road, and the road normal is
perpendicular to the road normal.  */
//=============================================================================
void W3DRoadBuffer::loadFloat4PtSection(RoadSegment *pRoad, Vector2 loc,
                                        Vector2 roadNormal, Vector2 roadVector,
                                        Vector2 *cornersP, Real uOffset,
                                        Real vOffset, Real uScale,
                                        Real vScale) {
  ...
  for (i = 0; i <= uCount; i++) {
    ...
    if (i < uCount) {
      ...
      for (j = 0; j < vCount; j++) {
        ...
        nextColumn.vtx[j] = origin + (uVector1 * jBarFactor * iFactor) +
                            (uVector2 * jFactor * iFactor) +
                            (vVector1 * iBarFactor * jFactor) +
                            (vVector2 * iFactor * jFactor);
        Real z = TheTerrainRenderObject->getMaxCellHeight(nextColumn.vtx[j].X,
                                                          nextColumn.vtx[j].Y);
      ...
      if (i < 2) {
        ...
        if (prevColumn.collapsed && curColumn.collapsed &&
            nextColumn.collapsed) {
          Bool okToDelete = false;

          Real theZ =
              prevColumn.vtx[0].Z * (curColumn.uIndex - prevColumn.uIndex) +
              nextColumn.vtx[0].Z * (nextColumn.uIndex - curColumn.uIndex);
          theZ /= nextColumn.uIndex - prevColumn.uIndex;
          if (theZ >= curColumn.vtx[0].Z &&
              theZ < curColumn.vtx[0].Z + MAX_ERROR) {
            theZ =
                prevColumn.vtx[1].Z * (curColumn.uIndex - prevColumn.uIndex) +
                nextColumn.vtx[1].Z * (nextColumn.uIndex - curColumn.uIndex);
            theZ /= nextColumn.uIndex - prevColumn.uIndex;
            if (theZ >= curColumn.vtx[1].Z &&
                theZ < curColumn.vtx[1].Z + MAX_ERROR) {
              okToDelete = true;
            }
          }
          if (okToDelete) {
    ...
    if (!curColumn.deleted && i != 1) {
      ...
      for (j = 0; j < vCount; j++) {
        ...
        vb[numRoadVertices].v1 =
            vOffset - V / (vScale * 4);  // Road is 1/16 texture height.
...
perpendicular to the road normal.  */
//=============================================================================
void W3DRoadBuffer::loadLit4PtSection(
    RoadSegment *pRoad, UnsignedShort *ib, VertexFormatXYZDUV1 *vb,
  ...
  for (i = 0; i <= uCount; i++) {
    ...
    if (!curColumn.deleted && i != 1) {
      ...
      for (j = 0; j < vCount; j++) {
        ...
        curVector.Set(curColumn.vtx[j].X - info.loc.X,
                      curColumn.vtx[j].Y - info.loc.Y);
        V = Vector2::Dot_Product(info.roadNormal, curVector);
        U = Vector2::Dot_Product(info.roadVector, curVector);
        Int diffuse =
            (255 << 24) | TheTerrainRenderObject->getStaticDiffuse(
                              curColumn.vtx[j].X / MAP_XY_FACTOR + 0.5,
                              curColumn.vtx[j].Y / MAP_XY_FACTOR + 0.5);
        ...
        for (k = 0; k < numLights; k++) {
          ...
          if (lights[k]->Get_Type() == LightClass::POINT) {
            ...
            if (fabs(range - midRange) < 1e-5) {
              // if the attenuation range is too small assume uniform with
              // cutoff
              factor = 1.0;
            } else {
        ...
        shadeG *= 255;
        shadeB *= 255;
        diffuse = REAL_TO_INT(shadeB) | (REAL_TO_INT(shadeG) << 8) |
        ...
        vb[m_curNumRoadVertices].v1 =
            info.vOffset -
            V / (info.scale * 4);  // Road is 1/16 texture height.
...
//=============================================================================
void W3DRoadBuffer::loadCurve(RoadSegment *pRoad, Vector2 loc1, Vector2 loc2,
                              Real scale) {
...
void W3DRoadBuffer::preloadRoadSegment(RoadSegment *pRoad) {
  ...
  loadFloat4PtSection(pRoad, pRoad->m_pt1.loc, roadNormal, roadVector, corners,
                      uOffset, vOffset, pRoad->m_scale, pRoad->m_scale);
}
...
void W3DRoadBuffer::preloadRoadsInVertexAndIndexBuffers() {
  ...
  for (curRoad = 0; curRoad < m_numRoads; curRoad++) {
    if (m_roads[curRoad].m_type == CURVE) {
      loadCurve(&m_roads[curRoad], m_roads[curRoad].m_pt1.loc,
                m_roads[curRoad].m_pt2.loc, m_roads[curRoad].m_scale);
    }
  }
  // Do tees.
  for (curRoad = 0; curRoad < m_numRoads; curRoad++) {
    if (m_roads[curRoad].m_type == THREE_WAY_Y) {
      loadY(&m_roads[curRoad], m_roads[curRoad].m_pt1.loc,
            m_roads[curRoad].m_pt2.loc, m_roads[curRoad].m_scale);
    }
  }
  // Do tees.
  for (curRoad = 0; curRoad < m_numRoads; curRoad++) {
    if (m_roads[curRoad].m_type == THREE_WAY_H ||
        m_roads[curRoad].m_type == THREE_WAY_H_FLIP) {
      loadH(&m_roads[curRoad], m_roads[curRoad].m_pt1.loc,
            m_roads[curRoad].m_pt2.loc,
            m_roads[curRoad].m_type == THREE_WAY_H_FLIP,
  ...
  for (curRoad = 0; curRoad < m_numRoads; curRoad++) {
    if (m_roads[curRoad].m_type == TEE || m_roads[curRoad].m_type == FOUR_WAY) {
      loadTee(&m_roads[curRoad], m_roads[curRoad].m_pt1.loc,
              m_roads[curRoad].m_pt2.loc, (m_roads[curRoad].m_type == FOUR_WAY),
              m_roads[curRoad].m_scale);
    }
  }
  // Do joins.
  for (curRoad = 0; curRoad < m_numRoads; curRoad++) {
    if (m_roads[curRoad].m_type == ALPHA_JOIN) {
      loadAlphaJoin(&m_roads[curRoad], m_roads[curRoad].m_pt1.loc,
                    m_roads[curRoad].m_pt2.loc, m_roads[curRoad].m_scale);
    }
  }
  updateLighting();
...
void W3DRoadBuffer::loadRoadsInVertexAndIndexBuffers() {
  ...
  DX8IndexBufferClass::WriteLockClass lockIdxBuffer(
      m_roadTypes[m_curRoadType].getIB());
  DX8VertexBufferClass::WriteLockClass lockVtxBuffer(
      m_roadTypes[m_curRoadType].getVB());
...
/** Loads the roads into the vertex buffer for drawing. */
//=============================================================================
void W3DRoadBuffer::loadLitRoadsInVertexAndIndexBuffers(
    RefRenderObjListIterator *pDynamicLightsIterator) {
  if (!m_initialized) {
    return;
  }
...
/** Loads a road segment into the vertex buffer for drawing. */
//=============================================================================
void W3DRoadBuffer::loadRoadSegment(UnsignedShort *ib, VertexFormatXYZDUV1 *vb,
                                    RoadSegment *pRoad) {
  if (pRoad->m_uniqueID != m_curUniqueID) {
    return;
  }
  ...
  m_curNumRoadVertices +=
      pRoad->GetVertices(vb + curVertex, pRoad->GetNumVertex());
  m_curNumRoadIndices += pRoad->GetIndices(ib + m_curNumRoadIndices,
                                           pRoad->GetNumIndex(), curVertex);
}
...
void W3DRoadBuffer::moveRoadSegTo(Int fromNdx, Int toNdx) {
  if (fromNdx < 0 || fromNdx >= m_numRoads || toNdx < 0 ||
      toNdx >= m_numRoads) {
#ifdef _DEBUG
...
#ifdef _DEBUG
  DEBUG_ASSERTLOG(m_roads[ndx].m_pt1.loc == m_roads[ndx + 1].m_pt2.loc,
                  ("Bad link\n"));
  if (ndx > 0) {
    DEBUG_ASSERTLOG(m_roads[ndx].m_pt2.loc != m_roads[ndx - 1].m_pt1.loc,
                    ("Bad Link\n"));
  }
#endif
...
#ifdef _DEBUG
      DEBUG_ASSERTLOG(m_roads[checkNdx].m_pt1.count == 1,
                      ("Wrong m_pt1.count.\n"));
...
#define CHECK_SEGMENTS                                                        \
  {                                                                           \
    if (m_numRoads >= m_maxRoadSegments) {                                    \
      if (warnSegments)                                                       \
        DEBUG_LOG(                                                            \
            ("****** Too many road segments.  Need to increase ini values.  " \
             "See john a.\n"));                                               \
      warnSegments = false;                                                   \
      return;                                                                 \
    }                                                                         \
...
void W3DRoadBuffer::addMapObjects() {
  ...
  for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
    ...
    if (pMapObj->getFlag(FLAG_ROAD_POINT1)) {
...
#ifdef _DEBUG
      DEBUG_ASSERTLOG(pMapObj2 && pMapObj2->getFlag(FLAG_ROAD_POINT2),
                      ("Bad Flag\n"));
#endif
      ...
      curRoad.m_curveRadius = pMapObj->getFlag(FLAG_ROAD_CORNER_TIGHT)
                                  ? TIGHT_CORNER_RADIUS
                                  : CORNER_RADIUS;
...
void W3DRoadBuffer::insertTee(Vector2 loc, Int index1, Real scale) {
  ...
  // The greatest negative dot product is the pair that is heading most opposite
  // each other.
  Bool do12 = false;
  Bool do13 = false;
  Bool do32 = false;
  ...
  if (dot > cos60) {
    ...
    Real xpdct = Vector3::Cross_Product_Z(Vector3(upVector.X, upVector.Y, 0),
                                          Vector3(decider.X, decider.Y, 0));
    Bool mirror = false;
    if (xpdct < 0) {
      angle = -angle;
      mirror = true;
    ...
    if (do12) {
      ...
      offsetH(pc1, pc2, pc3, loc, upVector, teeVector, flip, mirror,
              m_roads[index1].m_widthInTexture);
    }
    if (do13) {
      flip = xpSign(teeVector, v2) == 1;
      offsetH(pc1, pc3, pc2, loc, upVector, teeVector, flip, mirror,
              m_roads[index1].m_widthInTexture);
    }
    if (do32) {
      flip = xpSign(teeVector, v1) == 1;
      offsetH(pc3, pc2, pc1, loc, upVector, teeVector, flip, mirror,
              m_roads[index1].m_widthInTexture);
    }
    ...
    m_roads[m_numRoads].m_pt1.last =
        true;  // if not, that one will clear flag in prior loop.
    m_roads[m_numRoads].m_pt2.last =
        true;  // if not, that one will clear flag in prior loop.
    ...
    if (do12) {
      offset3Way(pc1, pc2, pc3, loc, upVector, teeVector,
                 m_roads[index1].m_widthInTexture);
    }
    if (do13) {
      offset3Way(pc1, pc3, pc2, loc, upVector, teeVector,
                 m_roads[index1].m_widthInTexture);
    }
    if (do32) {
      offset3Way(pc3, pc2, pc1, loc, upVector, teeVector,
                 m_roads[index1].m_widthInTexture);
    }
    pc1->last = true;
    pc1->count = 0;
...
Bool W3DRoadBuffer::insertY(Vector2 loc, Int index1, Real scale) {
  ...
  if (dot12 < (-cos30))
    return false;  // Too close to a straigh line, do a straight side tee.
  if (dot13 < (-cos30))
    return false;  // Too close to a straigh line, do a straight side tee.
  if (dot32 < (-cos30))
    return false;  // Too close to a straigh line, to a straight side tee.
...
//=============================================================================
void W3DRoadBuffer::offset3Way(TRoadPt *pc1, TRoadPt *pc2, TRoadPt *pc3,
                               Vector2 loc, Vector2 upVector, Vector2 teeVector,
                               Real widthInTexture) {
  ...
  Real xpdct = Vector3::Cross_Product_Z(Vector3(upVector.X, upVector.Y, 0),
                                        Vector3(teeVector.X, teeVector.Y, 0));
  ...
  xpdct = Vector3::Cross_Product_Z(
      Vector3(upVector.X, upVector.Y, 0),
      Vector3(pc1->top.X - pc1->loc.X, pc1->top.Y - pc1->loc.Y, 0));
  if (xpdct > 0) {
  ...
  xpdct = Vector3::Cross_Product_Z(
      Vector3(upVector.X, upVector.Y, 0),
      Vector3(pc2->top.X - pc2->loc.X, pc2->top.Y - pc2->loc.Y, 0));
  if (xpdct > 0) {
  ...
  xpdct = Vector3::Cross_Product_Z(
      Vector3(rightTee.X, rightTee.Y, 0),
      Vector3(pc3->top.X - pc3->loc.X, pc3->top.Y - pc3->loc.Y, 0));
  if (xpdct < 0) {
...
the join points of the 3 way intersection. */
//=============================================================================
void W3DRoadBuffer::offsetH(TRoadPt *pc1, TRoadPt *pc2, TRoadPt *pc3,
                            Vector2 loc, Vector2 upVector, Vector2 teeVector,
...
the join points of the 3 way intersection. */
//=============================================================================
void W3DRoadBuffer::offsetY(TRoadPt *pc1, TRoadPt *pc2, TRoadPt *pc3,
                            Vector2 loc, Vector2 upVector,
                            Real widthInTexture) {
...
//=============================================================================
void W3DRoadBuffer::offset4Way(TRoadPt *pc1, TRoadPt *pc2, TRoadPt *pc3,
                               TRoadPt *pr3, TRoadPt *pc4, Vector2 loc,
                               Vector2 alignVector, Real widthInTexture) {
  pc1->loc = loc - alignVector;
  pc2->loc = loc + alignVector;

  Vector2 v3 = pr3->loc - loc;
  Real angle = (PI / 2);  // 90 degrees.
  Real xpdct = Vector3::Cross_Product_Z(
      Vector3(alignVector.X, alignVector.Y, 0), Vector3(v3.X, v3.Y, 0));
  if (xpdct < 0) angle = -angle;
  ...
  xpdct = Vector3::Cross_Product_Z(
      align3, Vector3(pc1->top.X - pc1->loc.X, pc1->top.Y - pc1->loc.Y, 0));
  if (xpdct > 0) {
  ...
  xpdct = Vector3::Cross_Product_Z(
      align3, Vector3(pc2->top.X - pc2->loc.X, pc2->top.Y - pc2->loc.Y, 0));
  if (xpdct > 0) {
  ...
  xpdct = Vector3::Cross_Product_Z(
      tee3, Vector3(pc3->top.X - pc3->loc.X, pc3->top.Y - pc3->loc.Y, 0));
  if (xpdct < 0) {
  ...
  xpdct = Vector3::Cross_Product_Z(
      tee3, Vector3(pc4->top.X - pc4->loc.X, pc4->top.Y - pc4->loc.Y, 0));
  if (xpdct < 0) {
...
void W3DRoadBuffer::insert4Way(Vector2 loc, Int index1, Real scale) {
  ...
  // The most negative dot product is the pair that is heading most opposite
  // each other.
  ...
  if (do12) {
    offset4Way(pc1, pc2, pc3, pr3, pc4, loc, alignVector,
               m_roads[index1].m_widthInTexture);
  }
  if (do13) {
    offset4Way(pc1, pc3, pc2, pr2, pc4, loc, alignVector,
               m_roads[index1].m_widthInTexture);
  }
  if (do14) {
    offset4Way(pc1, pc4, pc3, pr3, pc2, loc, alignVector,
               m_roads[index1].m_widthInTexture);
  }
  if (do23) {
    offset4Way(pc2, pc3, pc1, pr1, pc4, loc, alignVector,
               m_roads[index1].m_widthInTexture);
  }
  if (do24) {
    offset4Way(pc2, pc4, pc1, pr1, pc3, loc, alignVector,
               m_roads[index1].m_widthInTexture);
  }
  if (do34) {
    offset4Way(pc3, pc4, pc1, pr1, pc2, loc, alignVector,
               m_roads[index1].m_widthInTexture);
  }
  if (alignVector.X < 0) {
  ...
  m_roads[m_numRoads].m_pt2.last =
      true;  // if not, that one will clear flag in prior loop.
  m_roads[m_numRoads].m_scale = m_roads[index1].m_scale;
  m_roads[m_numRoads].m_widthInTexture =
      TEE_WIDTH_ADJUSTMENT;  // It is symmetrical.
...
void W3DRoadBuffer::insertCurveSegments(void) {
  ...
  Int segmentStartIndex = -1;
  for (i = 0; i < numRoadSegments; i++) {
    if (i < numRoadSegments - 1 &&
        m_roads[i].m_pt1.loc == m_roads[i + 1].m_pt2.loc) {
      ...
      if (m_roads[i].m_pt1.loc == m_roads[segmentStartIndex].m_pt2.loc) {
        if (m_roads[segmentStartIndex].m_pt2.count == 1 &&
            m_roads[i].m_pt1.count == 1) {
...
//=============================================================================
Int W3DRoadBuffer::findCrossTypeJoinVector(Vector2 loc, Vector2 *joinVector,
                                           Int uniqueID) {
  ...
  for (i = 0; i < numRoadSegments; i++) {
    ...
    if (loc.X >= bounds.lo.x && loc.Y >= bounds.lo.y && loc.X <= bounds.hi.x &&
        loc.Y <= bounds.hi.y) {
...
void W3DRoadBuffer::insertCrossTypeJoins(void) {
  ...
  for (i = 0; i < numRoadSegments; i++) {
    ...
    Int otherID =
        findCrossTypeJoinVector(loc1, &joinVector, m_roads[i].m_uniqueID);
    if (!otherID) {
      joinVector *= 100;
    }
    Vector2 roadNormal(-roadVector.Y, roadVector.X);
    ...
    Vector2 p1 = loc1 + roadNormal * m_roads[i].m_scale *
                            m_roads[i].m_widthInTexture / 2;
    Vector2 p2 = loc2 + roadNormal * m_roads[i].m_scale *
                            m_roads[i].m_widthInTexture / 2;
    ...
    p1 = loc1 -
         roadNormal * m_roads[i].m_scale * m_roads[i].m_widthInTexture / 2;
    p2 = loc2 -
         roadNormal * m_roads[i].m_scale * m_roads[i].m_widthInTexture / 2;
    v1.Set(p1.X, p1.Y, 0);
    v2.Set(p2.X, p2.Y, 0);
    ...
    bottom = bottom - top;
    Real scaleAdjustment =
        bottom.Length() / (m_roads[i].m_scale * m_roads[i].m_widthInTexture);
    if (otherID) {
...
void W3DRoadBuffer::insertCurveSegmentAt(Int ndx1, Int ndx2) {
  const Real DOT_LIMIT =
      0.5f;  // If the dot product of the new line is less than this, abort.
  Real radius = m_roads[ndx1].m_curveRadius * m_roads[ndx1].m_scale;
  Vector2 originalPt = m_roads[ndx1].m_pt1.loc;
  // we got a segment.
  LineSegClass line1(
      Vector3(m_roads[ndx1].m_pt1.loc.X, m_roads[ndx1].m_pt1.loc.Y, 0),
      Vector3(m_roads[ndx1].m_pt2.loc.X, m_roads[ndx1].m_pt2.loc.Y, 0));
  LineSegClass line2(
      Vector3(m_roads[ndx2].m_pt1.loc.X, m_roads[ndx2].m_pt1.loc.Y, 0),
      Vector3(m_roads[ndx2].m_pt2.loc.X, m_roads[ndx2].m_pt2.loc.Y, 0));
  ...
  if (offsetLine1.Find_Intersection(offsetLine2, &pInt1, &nu, &pInt2, &nu)) {
    ...
    Real theDot = Vector3::Dot_Product(line2.Get_Dir(),
                                       pInt1 - Vector3(pr3->X, pr3->Y, 0));
    if (theDot < DOT_LIMIT) {
      *pr1 = originalPt;
      *pr4 = originalPt;
      miter(ndx1, ndx2);
    ...
    theDot = Vector3::Dot_Product(line1.Get_Dir(),
                                  Vector3(pr2->X, pr2->Y, 0) - pInt3);
    if (theDot < DOT_LIMIT) {
      *pr1 = originalPt;
      *pr4 = originalPt;
      miter(ndx1, ndx2);
    ...
    *pr1 = Vector2(pInt3.X, pInt3.Y);
    ...
    roadNormal.Normalize();
    roadNormal *=
        (m_roads[ndx1].m_scale * m_roads[ndx1].m_widthInTexture / 2.0f);
    ...
    roadNormal.Normalize();
    roadNormal *=
        (m_roads[ndx2].m_scale * m_roads[ndx2].m_widthInTexture / 2.0f);
...
void W3DRoadBuffer::allocateRoadBuffers(void) {
  ...
  i = 0;
  for (road = TheTerrainRoads->firstRoad(); road;
...
/** Sets the height map. */
//=============================================================================
void W3DRoadBuffer::setMap(WorldHeightMap *pMap) { REF_PTR_SET(m_map, pMap); }
...
/** Draws the roads.   */
//=============================================================================
void W3DRoadBuffer::drawRoads(
    CameraClass *camera, TextureClass *cloudTexture, TextureClass *noiseTexture,
    Bool wireframe, Int minX, Int maxX, Int minY, Int maxY,
    RefRenderObjListIterator *pDynamicLightsIterator) {
  ...
  W3DShaderManager::ShaderTypes st =
      W3DShaderManager::ST_ROAD_BASE;  // set default shader
  if (cloudTexture) {
    st = W3DShaderManager::ST_ROAD_BASE_NOISE1;
    if (noiseTexture) st = W3DShaderManager::ST_ROAD_BASE_NOISE12;
  ...
  for (stacking = 0; stacking <= maxStacking; stacking++) {
    for (i = 0; i < m_maxRoadTypes; i++) {
      ...
      for (Int pass = 0; pass < devicePasses; pass++) {
        if (!wireframe) W3DShaderManager::setShader(st, pass);
