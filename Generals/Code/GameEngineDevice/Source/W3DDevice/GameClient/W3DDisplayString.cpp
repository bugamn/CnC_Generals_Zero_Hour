...
//						 double byte characters and all
//the data we need to render 						 those strings to the screen.
//
//-----------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////
...
/** */
//=============================================================================
W3DDisplayString::~W3DDisplayString(void) {}  // end ~W3DDisplayString
...
void W3DDisplayString::draw(Int x, Int y, Color color, Color dropColor,
  ...
  // sanity
  if (getTextLength() == 0) return;  // nothing to draw
  ...
  if (m_fontChanged || m_textChanged) {
    if (m_useHotKey) {
      ...
      m_textRenderer.Build_Sentence(getText().str(), &m_hotKeyPos.x,
                                    &m_hotKeyPos.y);
  ...
  //
  if (needNewPolys || x != m_textPos.x || y != m_textPos.y ||
      color != m_currTextColor || dropColor != m_currDropColor) {
    ...
    // draw the shadow
    m_textRenderer.Set_Location(
        Vector2(m_textPos.x + xDrop, m_textPos.y + yDrop));
    ...
    if (m_useHotKey) {
      ...
      m_textRendererHotKey.Set_Location(
          Vector2(m_textPos.x + m_hotKeyPos.x, m_textPos.y + m_hotKeyPos.y));
  ...
  if (TheGameClient) usingResources(TheGameClient->getFrame());
...
void W3DDisplayString::setFont(GameFont *font) {
  ...
  m_textRendererHotKey.Set_Font(static_cast<FontCharsClass *>(
      TheFontLibrary->getFont(font->nameString, font->pointSize, TRUE)
          ->fontData));
...
void W3DDisplayString::setClipRegion(IRegion2D *region) {
  ...
  if (region->lo.x != m_clipRegion.lo.x || region->lo.y != m_clipRegion.lo.y ||
      region->hi.x != m_clipRegion.hi.x || region->hi.y != m_clipRegion.hi.y) {
    // assign new region
    m_clipRegion = *region;

    // set new region in renderer
    m_textRenderer.Set_Clipping_Rect(
        RectClass(m_clipRegion.lo.x, m_clipRegion.lo.y, m_clipRegion.hi.x,
                  m_clipRegion.hi.y));
    m_textRendererHotKey.Set_Clipping_Rect(
        RectClass(m_clipRegion.lo.x, m_clipRegion.lo.y, m_clipRegion.hi.x,
                  m_clipRegion.hi.y));
  }  // end if
...
void W3DDisplayString::computeExtents(void) {
  ...
  if (len == 0 || m_font == NULL) {
    ...
    Vector2 extents = m_textRenderer.Get_Formatted_Text_Extents(
        getText().str());  // Get_Text_Extents( getText().str() );
    m_size.x = extents.X;
    m_size.y = extents.Y;
...
void W3DDisplayString::setWordWrap(Int wordWrap) {
  // set the Word Wrap
  if (m_textRenderer.Set_Wrapping_Width(wordWrap)) notifyTextChanged();
...
void W3DDisplayString::setWordWrapCentered(Bool isCentered) {
  // set the Word Wrap
  if (m_textRenderer.Set_Word_Wrap_Centered(isCentered)) notifyTextChanged();
}  // void setWordWrap( Int wordWrap )