...
class W3DPrototypeClass : public MemoryPoolObject, public PrototypeClass {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(W3DPrototypeClass,
                                          "W3DPrototypeClass")
...
TextureClass *W3DAssetManager::Get_Texture(
    const char *filename, TextureClass::MipCountType mip_level_count,
  ...
  */
  ...
  if (tex && texture_format != WW3D_FORMAT_UNKNOWN) {
    WWASSERT_PRINT(
        tex->Get_Texture_Format() == texture_format,
        ("Texture %s has already been loaded with different format", filename));
  }
  ...
  */
  if (!tex) {
    tex = NEW_REF(TextureClass, (lower_case_name, NULL, mip_level_count,
...
static inline void Munge_Render_Obj_Name(char *newname, const char *oldname,
                                         float scale, const int color,
...
//---------------------------------------------------------------------
static inline void Munge_Texture_Name(char *newname, const char *oldname,
                                      const int color) {
  char lower_case_name[255];
  strcpy(lower_case_name, oldname);
  _strlwr(lower_case_name);
...
//---------------------------------------------------------------------
int W3DAssetManager::replaceAssetTexture(RenderObjClass *robj,
                                         TextureClass *oldTex,
                                         TextureClass *newTex) {
...
//---------------------------------------------------------------------
Int W3DAssetManager::replaceHLODTexture(RenderObjClass *robj,
                                        TextureClass *oldTex,
                                        TextureClass *newTex) {
  int didReplace = 0;
...
//---------------------------------------------------------------------
Int W3DAssetManager::replaceMeshTexture(RenderObjClass *robj,
                                        TextureClass *oldTex,
                                        TextureClass *newTex) {
  int i;
  int didReplace = 0;
...
//---------------------------------------------------------------------
/** Replaces all references to old texture with new texture.  Operation is
   performed on asset prototype so it will affect most instances of this object.
   Objects which have been customized with house color will not be affected
   unless they are created after this function is called.
*/
...
//---------------------------------------------------------------------
/** Generals specific version that looks for a texture which has been
   house-color tinted. Don't use this unless you really need it.  For normal
   textures, use Get_Texture().
*/
...
//---------------------------------------------------------------------
TextureClass *W3DAssetManager::Recolor_Texture(TextureClass *texture,
                                               const int color) {
...
//---------------------------------------------------------------------
static void remapPalette16Bit(SurfaceClass::SurfaceDescription *sd,
                              UnsignedShort *palette, unsigned int color) {
  UnsignedShort pal[TEAM_COLOR_PALETTE_SIZE];
  Vector3 rgb, v_color((float)((color >> 16) & 0xff) / 255.0f / 255.0f,
                       (float)((color >> 8) & 0xff) / 255.0f / 255.0f,
                       (float)(color & 0xff) / 255.0f / 255.0f);
...
//---------------------------------------------------------------------
static void remapTexture16Bit(Int dx, Int dy, Int pitch,
                              SurfaceClass::SurfaceDescription *sd,
                              UnsignedShort *palette, UnsignedShort *data,
                              unsigned int color) {
  UnsignedShort pal[TEAM_COLOR_PALETTE_SIZE];
  Vector3 rgb, v_color((float)((color >> 16) & 0xff) / 255.0f / 255.0f,
                       (float)((color >> 8) & 0xff) / 255.0f / 255.0f,
  ...
  for (y = 0; y < dy; y++) {
    for (Int x = 0; x < dx;
         x++) {  // check if this pixel is part of team color palette
      for (Int p = 0; p < TEAM_COLOR_PALETTE_SIZE; p++) {
        if (palette[p] == data[x]) {
...
static void remapAlphaTexture16Bit(Int dx, Int dy, Int pitch,
                                   SurfaceClass::SurfaceDescription *sd,
                                   UnsignedShort *data, unsigned int color) {
  UnsignedShort pixel;
  UnsignedShort pixelAlpha;
#ifndef DO_HUE_SHIFT
  float fpixelAlpha, fpixelAlphaInv;
#endif
  Vector3 rgb, v_color((float)((color >> 16) & 0xff) / 255.0f,
  ...
  for (y = 0; y < dy; y++) {
    for (x = 0; x < dx; x++) {
      ...
      if (pixelAlpha) {  // some house color needs to show through
                         ///@todo: optimize this alpha blend to use fixed point
                         ///math.
#ifdef DO_HUE_SHIFT
        RGB_To_HSV(
            hsv, Vector3(((pixel >> 8) & 0xf) / 15.0f,
                         ((pixel >> 4) & 0xf) / 15.0f, (pixel & 0xf) / 15.0f));
        hsv.X = hsv_color.X;
        hsv.Y *= hsv_color.Y;
        HSV_To_RGB(rgb, hsv);
#else
        ...
        rgb.X = fpixelAlpha * v_color.X +
                fpixelAlphaInv * (Real)((pixel >> 8) & 0xf) / 15.0f;  // red
        rgb.Y = fpixelAlpha * v_color.Y +
                fpixelAlphaInv * (Real)((pixel >> 4) & 0xf) / 15.0f;  // green
        rgb.Z = fpixelAlpha * v_color.Z +
                fpixelAlphaInv * (Real)(pixel & 0xf) / 15.0f;  // blue
#endif
        data[x] = REAL_TO_INT(rgb.X * 15.0f) << 8 |
                  REAL_TO_INT(rgb.Y * 15.0f) << 4 | REAL_TO_INT(rgb.Z * 15.0f);
      }
...
//---------------------------------------------------------------------
static void remapPalette32Bit(SurfaceClass::SurfaceDescription *sd,
                              UnsignedInt *palette, unsigned int color) {
...
//---------------------------------------------------------------------
static void remapTexture32Bit(Int dx, Int dy, Int pitch,
                              SurfaceClass::SurfaceDescription *sd,
                              UnsignedInt *palette, UnsignedInt *data,
                              unsigned int color) {
  UnsignedInt pal[TEAM_COLOR_PALETTE_SIZE];
  Vector3 rgb, v_color((float)((color >> 16) & 0xff) / 255.0f / 255.0f,
                       (float)((color >> 8) & 0xff) / 255.0f / 255.0f,
...
//---------------------------------------------------------------------
static void remapAlphaTexture32Bit(Int dx, Int dy, Int pitch,
                                   SurfaceClass::SurfaceDescription *sd,
                                   UnsignedInt *data, unsigned int color) {
  UnsignedInt pixel;
  ...
  for (y = 0; y < dy; y++) {
    for (x = 0; x < dx; x++) {
      ...
      if (pixelAlpha) {  // some house color needs to show through
#ifdef DO_HUE_SHIFT
        RGB_To_HSV(hsv, Vector3(((pixel >> 16) & 0xff) / 255.0f,
                                ((pixel >> 8) & 0xff) / 255.0f,
                                (pixel & 0xff) / 255.0f));
        hsv.X = hsv_color.X;
        hsv.Y *= hsv_color.Y;
        ...
        fpixelAlphaInv = 1.0f - fpixelAlpha;
        rgb.X = fpixelAlpha * v_color.X +
                fpixelAlphaInv * (Real)((pixel >> 16) & 0xff) / 255.0f;  // red
        rgb.Y = fpixelAlpha * v_color.Y +
                fpixelAlphaInv * (Real)((pixel >> 8) & 0xff) / 255.0f;  // green
        rgb.Z = fpixelAlpha * v_color.Z +
                fpixelAlphaInv * (Real)(pixel & 0xff) / 255.0f;  // blue
#endif
        data[x] = REAL_TO_INT(rgb.X * 255.0f) << 16 |
                  REAL_TO_INT(rgb.Y * 255.0f) << 8 |
                  REAL_TO_INT(rgb.Z * 255.0f);
      }
...
*/
void W3DAssetManager::Remap_Palette(SurfaceClass *surface, const int color,
                                    Bool doPaletteOnly, Bool useAlpha) {
  //	unsigned int x;
  SurfaceClass::SurfaceDescription sd;
  ...
  if (doPaletteOnly) {  // only recolor the palette which is stored in top row.
                        // Model only references these pixels.
    if (size == 2)
    ...
    if (useAlpha) {
      if (size == 2)
        remapAlphaTexture16Bit(sd.Width, sd.Height, pitch >> 1, &sd,
                               (UnsignedShort *)bits, color);
      else if (size == 4)
        remapAlphaTexture32Bit(sd.Width, sd.Height, pitch >> 2, &sd,
                               (UnsignedInt *)bits, color);
    } else {  // Recolor the image using the palette stored in top row
      if (size == 2)
        remapTexture16Bit(sd.Width, sd.Height - 1, pitch >> 1, &sd,
                          (UnsignedShort *)bits,
                          (UnsignedShort *)(bits + pitch), color);
      else if (size == 4)
        remapTexture32Bit(sd.Width, sd.Height - 1, pitch >> 2, &sd,
                          (UnsignedInt *)bits, (UnsignedInt *)(bits + pitch),
                          color);
    }
  }
...
//---------------------------------------------------------------------
TextureClass *W3DAssetManager::Recolor_Texture_One_Time(TextureClass *texture,
                                                        const int color) {
  const char *name = texture->Get_Texture_Name();
  ...
  if (!texture->Is_Initialized())
    TextureLoader::Request_High_Priority_Loading(
        texture, (TextureClass::MipCountType)texture->Get_Mip_Level_Count());
  ...
  DEBUG_ASSERTCRASH(psize == 2 || psize == 4,
                    ("Can't Recolor Texture %s", name));
  ...
  if (*(name + 3) == 'D' || *(name + 3) == 'd')
    Remap_Palette(newsurf, color, true,
                  false);  // texture only contains a palette stored in top row.
  else if (*(name + 3) == 'A' || *(name + 3) == 'a')
    Remap_Palette(newsurf, color, false,
                  true);  // texture only contains a palette stored in top row.

  TextureClass *newtex = NEW_REF(
      TextureClass,
      (newsurf, (TextureClass::MipCountType)texture->Get_Mip_Level_Count()));
...
#endif
//---------------------------------------------------------------------
/** Generals specific code to generate customized render objects for each team
   color Scale==1.0, color==0x00000000, and oldTexure==NULL are defaults that do
   nothing.
*/
RenderObjClass *W3DAssetManager::Create_Render_Obj(const char *name,
                                                   float scale, const int color,
  ...
  if (pext) isGranny = (strnicmp(pext, ".GR2", 4) == 0);
#endif
  Bool reallyscale = (WWMath::Fabs(scale - ident_scale) > scale_epsilon);
  Bool reallycolor =
      (color & 0xFFFFFF) !=
      0;  // black is not a valid color and assumes no custom coloring.
...
#ifdef INCLUDE_GRANNY_IN_BUILD
  if (isGranny) {  // Granny objects share the same prototype since they allow
                   // instance scaling.
    strcpy(newname, name);  // use same name for all granny objects at any
                            // scale.
  }
#endif
  ...
  if (rendobj) {  // store the color that we used to create asset so we can read
                  // it back out
...
#endif
  {
    ...
    if (reallycolor) Recolor_Asset(rendobj, color);
  }
#ifdef INCLUDE_GRANNY_IN_BUILD
  else {
    ///@todo Granny objects are realtime scaled - fix to scale like W3D.
  ...
  W3DPrototypeClass *w3dproto =
      newInstance(W3DPrototypeClass)(rendobj, newname);
  rendobj->Release_Ref();
...
int W3DAssetManager::Recolor_Mesh(RenderObjClass *robj, const int color) {
  ...
  for (i = 0; i < material->Texture_Count(); i++) {
    ...
    if (_strnicmp(oldtex->Get_Texture_Name(), "ZHC", 3) ==
        0) {  // This texture needs to be adjusted for housecolor
 ...
 * color
 */
void W3DAssetManager::Recolor_Vertex_Material(VertexMaterialClass *vmat,
                                              const int color) {
  Vector3 rgb, rgb2;
  ...
  // We ignore the existing ambinent/diffuse and assume they were 1.0.  We can
  // change to scaling them if required.
...
bool W3DAssetManager::Load_3D_Assets(const char *filename) {
  ...
  if (pext) *pext = '\0';               // drop the extension
  ...
  */
  if (newproto != NULL) {
    if (!Render_Obj_Exists(newproto->Get_Name())) {
      ...
      */
      WWDEBUG_SAY(
          ("Render Object Name Collision: %s\r\n", newproto->Get_Name()));
      delete newproto;
      newproto = NULL;
      return false;
    }
    ...
    */
    WWDEBUG_SAY(
        ("Could not generate Granny prototype!  File  = %d\r\n", filename));
    return false;
  }
  return true;
#endif
...
HAnimClass *W3DAssetManager::Get_HAnim(const char *name) {
...
#endif
  {
    HAnimClass *anim = WW3DAssetManager::Get_HAnim(
        name);  // we only do custom granny processing.
#ifdef DUMP_PERF_STATS
  ...
  if (WW3D_Load_On_Demand &&
      anim == NULL) {  // If we didn't find it, try to load on demand
    if (!m_GrannyAnimManager->Is_Missing(
            name)) {  // if this is NOT a known missing anim
      ...
      if (anim == NULL) {
        //				WWDEBUG_SAY(("WARNING: Animation %s not
        //found!\n", name));
        m_GrannyAnimManager->Register_Missing(
            name);  // This is now a KNOWN missing anim
      }
    }
  }
  return anim;
 ...
 * color
 */
void W3DAssetManager::Make_HLOD_Unique(RenderObjClass *robj, Bool geometry,
                                       Bool colors) {
  int num_sub = robj->Get_Num_Sub_Objects();
  for (int i = 0; i < num_sub; i++) {
    RenderObjClass *sub_obj = robj->Get_Sub_Object(i);
 ...
 * color
 */
void W3DAssetManager::Make_Unique(RenderObjClass *robj, Bool geometry,
                                  Bool colors) {
  switch (robj->Class_ID()) {
    case RenderObjClass::CLASSID_MESH:
...
static Bool getMeshColorMethods(MeshClass *mesh, Bool &vertexColor,
                                Bool &textureColor) {
  vertexColor = false;
  textureColor = false;
  ...
  if (material) {
    for (int j = 0; j < material->Texture_Count(); j++)
      if (_strnicmp(material->Peek_Texture(j)->Get_Texture_Name(), "ZHC", 3) ==
          0) {
        textureColor = true;
        break;
      }
  ...
  // Check if mesh is using a custom mesh which contains house color in
  // material. Meshes which are part of another model have names in the form
  // "name.name" while isolated meshes are just "name".  We check for both
  // starting with "HOUSECOLOR".
  const char *meshName;
  if (((meshName = strchr(mesh->Get_Name(), '.')) != 0 && *(meshName++)) ||
      ((meshName = mesh->Get_Name()) !=
       NULL)) {  // Check if this object has housecolors on mesh
    if (_strnicmp(meshName, "HOUSECOLOR", 10) == 0) vertexColor = true;
  }
 ...
 * color
 */
void W3DAssetManager::Make_Mesh_Unique(RenderObjClass *robj, Bool geometry,
                                       Bool colors) {
  int i;
  MeshClass *mesh = (MeshClass *)robj;
  ...
  if ((colors && getMeshColorMethods(mesh, isVertexColor, isTextureColor)) ||
      geometry) {  // mesh has some house color applied so make those components
                   // unique to mesh.
...
void W3DAssetManager::Report_Used_Prototypes(void) {
  ...
  while (count-- > 0) {
    ...
    if (proto->Get_Class_ID() == RenderObjClass::CLASSID_HLOD ||
        proto->Get_Class_ID() == RenderObjClass::CLASSID_MESH) {
...
void W3DAssetManager::Report_Used_Textures(void) {
  /*
  ** for each texture in the list, get it, check it's refcount, and and release
  *ref it if the
  ** refcount is one.
  */
    ...
    if (tex->Num_Refs() <= 1) {
      /*		temp_textures[count++]=tex;
                      if (count==256) {
                              ...
                              count=0;
                              ite.First();	// iterator doesn't support
         modifying the hash table while iterating, so start from the
                                                                      //
         beginning.
                      }*/
    } else {
...
void W3DAssetManager::Report_Used_Font3DDatas(void) {
  /*
  ** for each font data in the list, get it, check it's refcount, and and
  *release ref it if the
  ** refcount is one.
  */
...
void W3DAssetManager::Make_Mesh_Unique(RenderObjClass *robj, Bool geometry, Bool
colors)
{
        int i;
        MeshClass *mesh=(MeshClass*) robj;
...
void W3DAssetManager::Make_HLOD_Unique(RenderObjClass *robj, Bool geometry, Bool
colors)
{
        int num_sub = robj->Get_Num_Sub_Objects();
        for(int i = 0; i < num_sub; i++) {
...
void W3DAssetManager::Make_Unique(RenderObjClass *robj, Bool geometry, Bool
colors)
{
        switch (robj->Class_ID())	{
        case RenderObjClass::CLASSID_MESH:
...
static inline void Munge_Render_Obj_Name(char *newname, const char *oldname,
float scale, const Vector3 &hsv_shift)
{
        char lower_case_name[255];
        strcpy(lower_case_name, oldname);
        _strlwr(lower_case_name);
        sprintf(newname,"#%s!%gH%gS%gV%g", lower_case_name, scale, hsv_shift.X,
hsv_shift.Y, hsv_shift.Z);
}

static inline void Munge_Texture_Name(char *newname, const char *oldname, const
Vector3 &hsv_shift)
{
        char lower_case_name[255];
        strcpy(lower_case_name, oldname);
        _strlwr(lower_case_name);
        sprintf(newname,"#%s!H%gS%gV%g", lower_case_name, hsv_shift.X,
hsv_shift.Y, hsv_shift.Z);
}

RenderObjClass * W3DAssetManager::Create_Render_Obj(const char * name,float
scale, const Vector3 &hsv_shift)
{
        Bool isGranny = false;
#ifdef	INCLUDE_GRANNY_IN_BUILD
        char *pext=strrchr(name,'.');	//find file extension
...
#endif
        Bool reallyscale = (WWMath::Fabs(scale - ident_scale) > scale_epsilon);
        Bool reallyhsv_shift = (WWMath::Fabs(hsv_shift.X - ident_HSV.X) >
H_epsilon || WWMath::Fabs(hsv_shift.Y - ident_HSV.Y) > S_epsilon ||
WWMath::Fabs(hsv_shift.Z - ident_HSV.Z) > V_epsilon);

        // base case, no scale or hue shifting
        if (!reallyscale && !reallyhsv_shift) return
WW3DAssetManager::Create_Render_Obj(name);
        ...
        if (isGranny)
        {	//Granny objects share the same prototype since they allow
instance scaling. strcpy(newname,name);	//use same name for all granny objects
at any scale.
        }
        Set_WW3D_Load_On_Demand(false); // munged name will never be found in a
file. rendobj=WW3DAssetManager::Create_Render_Obj(newname); if (rendobj) {
if (isGranny)
                        ///@todo Granny objects are realtime scaled - fix to
scale like W3D. rendobj->Set_ObjectScale(scale); Set_WW3D_Load_On_Demand(true);
// Auto Load. return rendobj;
        }
        ...
        Set_WW3D_Load_On_Demand(true); // Auto Load.
        if (WW3D_Load_On_Demand && proto == NULL) {	// If we didn't find
one, try to load on demand char filename [MAX_PATH]; char *mesh_name = ::strchr
(name, '.'); if (mesh_name != NULL) {
                        ::lstrcpyn (filename, name, ((int)mesh_name) -
((int)name) + 1); if (isGranny)
                                ::lstrcat (filename, ".gr2");
                        else
                                ::lstrcat (filename, ".w3d");
                } else {
                ...
                // If we can't find it, try the parent directory
                if ( Load_3D_Assets( filename ) == false ) {
                        StringClass	new_filename = StringClass("..\\") +
filename; if (Load_3D_Assets( new_filename ) == false)
                        {
                                char *mesh_name = ::strchr (filename, '.');
                                ::lstrcpyn (mesh_name, ".gr2",5);
                                Load_3D_Assets( filename );
        ...
        if (proto == NULL) {
                static int warning_count = 0;
                if (++warning_count <= 20) {
                        WWDEBUG_SAY(("WARNING: Failed to create Render Object:
%s\r\n",name));
                }
                return NULL;		// Failed to find a prototype
        }
        ...
        {	///@todo Granny objects are realtime scaled - fix to scale like
W3D. rendobj->Set_ObjectScale(scale); return rendobj;
        }
...
TextureClass * W3DAssetManager::Get_Texture_With_HSV_Shift(const char *
filename, const Vector3 &hsv_shift, TextureClass::MipCountType mip_level_count)
{
        WWPROFILE( "W3DAssetManager::Get_Texture with HSV shift" );

        Bool is_hsv_shift = (WWMath::Fabs(hsv_shift.X - ident_HSV.X) > H_epsilon
|| WWMath::Fabs(hsv_shift.Y - ident_HSV.Y) > S_epsilon ||
WWMath::Fabs(hsv_shift.Z - ident_HSV.Z) > V_epsilon);
        ...
        if (!is_hsv_shift) {
                ...
                if (!newtex) {
                        ...
                        if (!oldtex) {
                                oldtex = NEW_REF(TextureClass,(lower_case_name,
NULL, mip_level_count)); TextureHash.Insert(oldtex->Get_Texture_Name(), oldtex);
                        }

                        newtex = Recolor_Texture_One_Time(oldtex, hsv_shift);

                        // If the recolorization failed, return the original
texture if (!newtex) { newtex = oldtex; newtex->Add_Ref();
                        }
...
void W3DAssetManager::Recolor_Vertex_Material(VertexMaterialClass *vmat, const
Vector3 &hsv_shift)
{
        Vector3 rgb;
...
void W3DAssetManager::Recolor_Vertices(unsigned int *color, int count, const
Vector3 &hsv_shift)
{
        int i;
        Vector4 rgba;
...
TextureClass * W3DAssetManager::Recolor_Texture(TextureClass *texture, const
Vector3 &hsv_shift)
{
        const char *name=texture->Get_Texture_Name();
...
const Vector3 &hsv_shift)
{
        ...
        // make sure texture is loaded
        if (!texture->Is_Initialized())
                TextureLoader::Request_High_Priority_Loading(texture,
(TextureClass::MipCountType)texture->Get_Mip_Level_Count());
...
TextureClass * W3DAssetManager::Find_Texture(const char * name, const Vector3
&hsv_shift)
{
        char newname[512];
        Munge_Texture_Name(newname, name, hsv_shift);
...
void W3DAssetManager::Recolor_Mesh(RenderObjClass *robj, const Vector3
&hsv_shift)
{
        int i;
...
void W3DAssetManager::Recolor_HLOD(RenderObjClass *robj, const Vector3
&hsv_shift)
{
        int num_sub = robj->Get_Num_Sub_Objects();
        for(int i = 0; i < num_sub; i++) {
...
void W3DAssetManager::Recolor_ParticleEmitter(RenderObjClass *robj, const
Vector3 &hsv_shift)
{
        unsigned int i;
...
void W3DAssetManager::Recolor_Asset(RenderObjClass *robj, const Vector3
&hsv_shift)
{
        switch (robj->Class_ID())	{
        case RenderObjClass::CLASSID_MESH:
