...
Bool W3DFontLibrary::loadFontData(GameFont *font) {
  ...
  if ((UnsignedInt)font->pointSize >
      100)  // sanity check the size - anything over 100 is probably wrong. -MW
    fontChar = NULL;
  else {  // get the font data from the asset manager
    fontChar = WW3DAssetManager::Get_Instance()->Get_FontChars(
  ...
  if (fontChar == NULL) {
    DEBUG_LOG(("W3D load font: unable to find font '%s' from asset manager\n",
               font->nameString.str()));
    DEBUG_ASSERTCRASH(
        fontChar, ("Missing or Corrupted Font.  Pleas see log for details"));
    return FALSE;
  ...
  if (TheGlobalLanguageData)
    unicodeFontChar = WW3DAssetManager::Get_Instance()->Get_FontChars(
        TheGlobalLanguageData->m_unicodeFontName.str(), font->pointSize,
        font->bold ? true : false);
  else
    unicodeFontChar = WW3DAssetManager::Get_Instance()->Get_FontChars(
        "Arial Unicode MS", font->pointSize, font->bold ? true : false);
