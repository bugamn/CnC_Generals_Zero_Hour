...
//=============================================================================
void W3DGadgetHorizontalSliderDraw(GameWindow *window,
  ...
  if (backBorder != WIN_COLOR_UNDEFINED) {
    ...
    TheWindowManager->winOpenRect(backBorder, WIN_DRAW_LINE_WIDTH, start.x,
                                  start.y, end.x, end.y);
  ...
  if (backColor != WIN_COLOR_UNDEFINED) {
    ...
    TheWindowManager->winFillRect(backColor, WIN_DRAW_LINE_WIDTH, start.x,
                                  start.y, end.x, end.y);
...
void W3DGadgetHorizontalSliderImageDraw(GameWindow *window,
  ...
  Real selectedPercent =
      (s->position - s->minVal) / INT_TO_REAL((s->maxVal - s->minVal));
  ...
  if (BitTest(instData->getState(), WIN_STATE_HILITED)) {
    ...
    for (i = 0; i < numHighlightBoxes; ++i) {
      backgroundStart.x =
          origin.x + highlightOffset.x + i * (boxWidth + boxPadding);
      backgroundEnd.x = backgroundStart.x + boxWidth + boxPadding;
      TheWindowManager->winDrawImage(highlightSquare, backgroundStart.x,
                                     backgroundStart.y, backgroundEnd.x,
                                     backgroundEnd.y);
    }
  }
  ...
  for (i = 0; i < numSelectedBoxes; ++i) {
    ...
    TheWindowManager->winDrawImage(fillSquare, start.x, start.y, end.x, end.y);
  }
  for (i = numSelectedBoxes; i < numBoxes; ++i) {
    start.x = origin.x + i * (boxWidth + boxPadding);
    end.x = start.x + boxWidth;
    TheWindowManager->winDrawImage(blankSquare, start.x, start.y, end.x, end.y);
  }
}
...
void W3DGadgetHorizontalSliderImageDrawB(GameWindow *window,
  ...
  tooltip.format(L"mult:%g/%g, img offset:%d,%d", xMulti, yMulti, xOffset,
                 yOffset);

  tmp.format(L"\norigin: %d,%d size:%d,%d", origin.x, origin.y, size.x, size.y);
  tooltip.concat(tmp);

  tmp.format(L"\ns= %d <--> %d, numTicks=%g, pos = %d", s->minVal, s->maxVal,
             s->numTicks, s->position);
  tooltip.concat(tmp);
  ...
  if (BitTest(instData->getState(), WIN_STATE_HILITED)) {
    ...
    ICoord2D backgroundStart, backgroundEnd;
    backgroundStart.x =
        origin.x - (highlightSquare->getImageWidth() * xMulti) / 2;
    backgroundStart.y =
        origin.y + (highlightSquare->getImageHeight() * yMulti) / 3;
    backgroundEnd.y =
        backgroundStart.y + highlightSquare->getImageHeight() * yMulti;
    backgroundEnd.x =
        backgroundStart.x + highlightSquare->getImageWidth() * xMulti;

    tmp.format(L"\nHighlighted: (%d,%d) -> (%d,%d), step %d/%g, full %d/%d",
               backgroundStart.x, backgroundStart.y, backgroundEnd.x,
               backgroundEnd.y, highlightSquare->getImageWidth(),
               highlightSquare->getImageWidth() * xMulti, origin.x, size.x);
    tooltip.concat(tmp);
    ...
    tmp.format(L"\n  bsX = %d, beX = %d (%d < %d+%d or %d?)", backgroundStart.x,
               backgroundEnd.x, backgroundStart.x, origin.x, size.x,
               origin.x + size.x);
    tooltip.concat(tmp);
  }
  ...
  while (start.x <= origin.x + (s->numTicks * (s->position - s->minVal)) &&
         end.x < origin.x + size.x && s->position != s->minVal) {
    TheWindowManager->winDrawImage(fillSquare, start.x, start.y, end.x, end.y);
    start.x = end.x + 2;
    end.x = start.x + fillSquare->getImageWidth() * xMulti;
  }
  ...
  //	{
  //		progressArrow				=
  //GadgetSliderGetHiliteImageRight( window ); 		if(!progressArrow) 			return; 		Int
  //transPos = (s->numTicks * (s->position - s->minVal)) -
  //progressArrow->getImageWidth() /2; 		start.x = origin.x + transPos; 		start.y =
  //origin.y + fillSquare->getImageHeight()/3*2; 		end.y = start.y +
  //progressArrow->getImageHeight(); 		end.x	= start.x +
  //progressArrow->getImageWidth(); 		TheWindowManager->winDrawImage(
  //progressArrow, 																			start.x, start.y, 																			end.x, end.y );
  //	}
}
...
void W3DGadgetHorizontalSliderImageDrawA(GameWindow *window,
                                         WinInstanceData *instData) {
  const Image *leftImageLeft, *rightImageLeft, *centerImageLeft,
      *smallCenterImageLeft;
  const Image *leftImageRight, *rightImageRight, *centerImageRight,
      *smallCenterImageRight;
  ICoord2D origin, size, start, end;
  Int xOffset, yOffset;
  Int i;
  ...
  SliderData *s = (SliderData *)window->winGetUserData();
  Int transPos = (s->numTicks * (s->position - s->minVal)) +
  ...
  if (BitTest(window->winGetStatus(), WIN_STATUS_ENABLED) == FALSE) {
    ...
    rightImageRight = rightImageLeft =
        GadgetSliderGetDisabledImageRight(window);
    //		centerImageRight = centerImageLeft =
    //GadgetSliderGetDisabledImageCenter( window ); 		smallCenterImageRight =
    //smallCenterImageLeft	= GadgetSliderGetDisabledImageSmallCenter(
    //window );
  ...
  for (i = 0; i < pieces; i++) {
    ...
    TheWindowManager->winDrawImage(centerImageLeft, start.x, start.y, end.x,
                                   end.y);
    TheDisplay->setClipRegion(&clipRight);
    TheWindowManager->winDrawImage(centerImageRight, start.x, start.y, end.x,
                                   end.y);
  ...
  for (i = 0; i < pieces; i++) {
    ...
    TheDisplay->setClipRegion(&clipLeft);
    TheWindowManager->winDrawImage(smallCenterImageLeft, start.x, start.y,
                                   end.x, end.y);
    TheDisplay->setClipRegion(&clipRight);
    TheWindowManager->winDrawImage(smallCenterImageRight, start.x, start.y,
                                   end.x, end.y);
  ...
  TheDisplay->setClipRegion(&clipRight);
  TheWindowManager->winDrawImage(leftImageRight, start.x, start.y, end.x,
                                 end.y);
  // draw right end
  start = rightStart;
  ...
  TheDisplay->setClipRegion(&clipLeft);
  TheWindowManager->winDrawImage(rightImageLeft, start.x, start.y, end.x,
                                 end.y);
  TheDisplay->setClipRegion(&clipRight);
  TheWindowManager->winDrawImage(rightImageRight, start.x, start.y, end.x,
                                 end.y);
