...
//
// File name:
// projects\RTS\code\gameenginedevice\Source\W3DDevice\GameClient\GUI\Gadget\W3DTabControl.cpp
//
...
void W3DGadgetTabControlDraw(GameWindow *tabControl,
  ...
  if (BitTest(tabControl->winGetStatus(), WIN_STATUS_BORDER) == TRUE &&
      !BitTest(tabControl->winGetStatus(),
               WIN_STATUS_SEE_THRU)) {  // draw border if desired
  ...
  if (tabData->tabCount >= 1)  // Does exist
  {
    ...
    if (border != WIN_COLOR_UNDEFINED) {
      TheWindowManager->winOpenRect(border, WIN_DRAW_LINE_WIDTH, tabX, tabY,
                                    tabX + tabWidth, tabY + tabHeight);
    }
    if (color != WIN_COLOR_UNDEFINED) {
      TheWindowManager->winFillRect(color, WIN_DRAW_LINE_WIDTH, tabX + 1,
                                    tabY + 1, tabX + tabWidth - 1,
                                    tabY + tabHeight - 1);
    }
  }
  ...
  {
    if (tabData->subPaneDisabled[3]) {  // Disabled
    ...
    } else if (tabData->activeTab == 3) {  // Hilited/Active
  ...
  {
    if (tabData->subPaneDisabled[7]) {  // Disabled
    ...
    } else if (tabData->activeTab == 7) {  // Hilited/Active
...
void W3DGadgetTabControlImageDraw(GameWindow *tabControl,
  ...
  if (tabData->tabCount >= 1)  // Does exist
  {
    ...
    if (image != NULL) {
      TheWindowManager->winDrawImage(image, tabX, tabY, tabX + tabWidth,
                                     tabY + tabHeight);
    }
  }
