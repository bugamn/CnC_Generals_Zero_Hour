...
void W3DGadgetRadioButtonDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  TheWindowManager->winDrawLine(backBorder, WIN_DRAW_LINE_WIDTH, start.x,
                                start.y, end.x, end.y);
  ...
  TheWindowManager->winFillRect(boxColor, WIN_DRAW_LINE_WIDTH, start.x, start.y,
                                end.x, end.y);
  ...
  if (instData->getTextLength()) drawRadioButtonText(window, instData);
...
void W3DGadgetRadioButtonImageDraw(GameWindow *window,
  ...
  if (leftImage == NULL || centerImage == NULL || rightImage == NULL) return;
