...
//
// Desc:			W3D implementation for the push button control
// element
//
//-----------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////
...
void W3DGadgetPushButtonImageDrawThree(GameWindow *window,
...
void W3DGadgetPushButtonDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  if (border != WIN_COLOR_UNDEFINED) {
    TheWindowManager->winOpenRect(border, WIN_DRAW_LINE_WIDTH, start.x, start.y,
                                  end.x, end.y);
  ...
  if (color != WIN_COLOR_UNDEFINED) {
    ...
    TheWindowManager->winFillRect(color, WIN_DRAW_LINE_WIDTH, start.x, start.y,
                                  end.x, end.y);

  }  // end if

  // draw the button text
  if (instData->getTextLength()) drawButtonText(window, instData);

  // if we have a video buffer, draw the video buffer
  if (instData->m_videoBuffer) {
    TheDisplay->drawVideoBuffer(instData->m_videoBuffer, origin.x, origin.y,
  ...
  if (pData) {
    if (pData->overlayImage) {
      ...
      TheDisplay->drawImage(pData->overlayImage, origin.x, origin.y,
                            origin.x + size.x, origin.y + size.y);
    }

    if (pData->drawClock) {
      if (pData->drawClock == NORMAL_CLOCK) {
        TheDisplay->drawRectClock(origin.x, origin.y, size.x, size.y,
                                  pData->percentClock, pData->colorClock);
      } else if (pData->drawClock == INVERSE_CLOCK) {
        TheDisplay->drawRemainingRectClock(origin.x, origin.y, size.x, size.y,
                                           pData->percentClock,
                                           pData->colorClock);
      }
    ...
    if (pData->drawBorder && pData->colorBorder != GAME_COLOR_UNDEFINED) {
      TheDisplay->drawOpenRect(origin.x - 1, origin.y - 1, size.x + 2,
...
void W3DGadgetPushButtonImageDraw(GameWindow *window,
                                  WinInstanceData *instData) {
  // if we return NULL then we'll call the one picture drawing code, if we
  // return a value then we'll call the 3 picture drawing code
  if (GadgetButtonGetMiddleEnabledImage(window)) {
    if (BitTest(instData->getState(), WIN_STATUS_USE_OVERLAY_STATES)) {
      ...
      DEBUG_CRASH(
          ("Button at %d,%d is attempting to render with "
           "W3DGadgetPushButtonImageDrawThree(), but is using overlay states! "
           "Forcing the code to use W3DGadgetPushButtonImageDrawOne() instead.",
           start.x, start.y));
...
void W3DGadgetPushButtonImageDrawOne(GameWindow *window,
  ...
  if (!BitTest(window->winGetStatus(), WIN_STATUS_USE_OVERLAY_STATES)) {
    ...
    // altered states. If they do, then we won't render the auto-overlay
    // versions.
    if (BitTest(window->winGetStatus(), WIN_STATUS_ENABLED) == FALSE) {
      if (BitTest(instData->getState(), WIN_STATE_SELECTED))
  ...
  if (image) {
    ...
    if (BitTest(window->winGetStatus(), WIN_STATUS_USE_OVERLAY_STATES)) {
      // we're using a new drawing system which does "grayscale" disabled
      // buttons using original color artwork.
      if (!BitTest(window->winGetStatus(), WIN_STATUS_ENABLED)) {
        if (!BitTest(window->winGetStatus(), WIN_STATUS_NOT_READY)) {
          // The button is disabled -- but if the button isn't "ready", we don't
          // want to do this because we want to show the button in color with
          // just the clock overlay.
          if (!BitTest(window->winGetStatus(), WIN_STATUS_ALWAYS_COLOR)) {
            drawMode = Display::DRAW_IMAGE_GRAYSCALE;
          } else {
            colorMultiplier = 0xff909090;  // RGB values are 144/255 (90) --
                                           // Alpha is opaque (ff) --> ff909090;
          }
        }
      }
    }
    TheDisplay->drawImage(image, start.x, start.y, end.x, end.y,
  ...
  if (BitTest(window->winGetStatus(), WIN_STATUS_FLASHING)) {
    ...
    static const Image *hilitedOverlayIcon =
        TheMappedImageCollection->findImageByName("Cameo_push");
    TheDisplay->drawImage(hilitedOverlayIcon, start.x, start.y,
                          start.x + size.x, start.y + size.y);
  }

  if (BitTest(window->winGetStatus(), WIN_STATUS_USE_OVERLAY_STATES)) {
    image = NULL;
    static const Image *pushedOverlayIcon =
        TheMappedImageCollection->findImageByName("Cameo_push");
    static const Image *hilitedOverlayIcon =
        TheMappedImageCollection->findImageByName("Cameo_hilited");
    if (pushedOverlayIcon && hilitedOverlayIcon) {
      if (BitTest(window->winGetStatus(), WIN_STATUS_ENABLED)) {
        if (BitTest(instData->getState(), WIN_STATE_HILITED)) {
          if (BitTest(instData->getState(), WIN_STATE_SELECTED)) {
            // The button is hilited and pushed
            TheDisplay->drawImage(pushedOverlayIcon, start.x, start.y,
                                  start.x + size.x, start.y + size.y);
          } else {
