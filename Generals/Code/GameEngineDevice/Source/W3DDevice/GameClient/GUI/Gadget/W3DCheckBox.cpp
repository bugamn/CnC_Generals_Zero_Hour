...
void W3DGadgetCheckBoxDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  TheWindowManager->winOpenRect(boxBorder, WIN_DRAW_LINE_WIDTH, start.x,
                                start.y, end.x, end.y);

  // draw "x" for button
  if (boxColor != WIN_COLOR_UNDEFINED) {
    TheWindowManager->winDrawLine(boxColor, WIN_DRAW_LINE_WIDTH, start.x,
                                  start.y, end.x, end.y);
    TheWindowManager->winDrawLine(boxColor, WIN_DRAW_LINE_WIDTH, start.x, end.y,
                                  end.x, start.y);

  }  // end if

  // draw the button text
  if (instData->getTextLength()) drawCheckBoxText(window, instData);
...
void W3DGadgetCheckBoxImageDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  //	if( backgroundImage )
  //		TheWindowManager->winDrawImage( backgroundImage, start.x,
  //start.y, 																		end.x, end.y );
