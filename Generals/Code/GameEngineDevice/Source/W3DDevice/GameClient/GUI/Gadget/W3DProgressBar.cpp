...
void W3DGadgetProgressBarDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  if (progress) {
    ...
    if (barBorder != WIN_COLOR_UNDEFINED) {
      ...
      if (end.x - start.x > 1) {
        TheWindowManager->winOpenRect(barBorder, WIN_DRAW_LINE_WIDTH, start.x,
                                      start.y, end.x, end.y);
      }
    ...
    if (barColor != WIN_COLOR_UNDEFINED) {
      ...
      //			TheWindowManager->winOpenRect( barColor,
      //WIN_DRAW_LINE_WIDTH, 																		 start.x, start.y, end.x, end.y );

      if (end.x - start.x > 1) {
        TheWindowManager->winFillRect(barColor, WIN_DRAW_LINE_WIDTH, start.x,
                                      start.y, end.x, end.y);

        TheWindowManager->winDrawLine(GameMakeColor(255, 255, 255, 255),
                                      WIN_DRAW_LINE_WIDTH, start.x, start.y,
                                      end.x, start.y);
...
//=============================================================================
void W3DGadgetProgressBarImageDrawA(GameWindow *window,
  ...
  if (!barCenter || !barRight || !left || !right || !center) return;
  ...
  Int x = origin.x;
  for (i = 0; i < pieces; i++) {
    TheWindowManager->winDrawImage(barCenter, x, origin.y, x + width,
                                   origin.y + size.y);
    x += width;
  }
}

void W3DGadgetProgressBarImageDraw(GameWindow *window,
                                   WinInstanceData *instData) {
  ICoord2D origin, size, start, end;
  const Image *backLeft, *backRight, *backCenter, *barRight,
      *barCenter;  //*backSmallCenter,*barLeft,, *barSmallCenter;
  Int progress = (Int)window->winGetUserData();
  Int xOffset, yOffset;
  Int i;
  ...
  if (BitTest(window->winGetStatus(), WIN_STATUS_ENABLED) == FALSE) {
    ...
    // ); barSmallCenter	=
    // GadgetProgressBarGetDisabledBarImageSmallCenter( window );

  }  // end if, disabled
  else if (BitTest(instData->getState(), WIN_STATE_HILITED)) {
    backLeft = GadgetProgressBarGetHiliteImageLeft(window);
    // barLeft					=
    // GadgetProgressBarGetHiliteBarImageLeft( window );
    ...
    // barLeft					=
    // GadgetProgressBarGetEnabledBarImageLeft( window );
  ...
  // sanity
  if (backLeft == NULL || backRight == NULL || backCenter == NULL ||
      barRight == NULL)
    // backSmallCenter == NULL ||barLeft == NULL ||barCenter == NULL ||
    // barSmallCenter == NULL )
    return;
  ...
  for (i = 0; i < pieces; i++) {
    ...
    TheWindowManager->winDrawImage(backCenter, start.x, start.y, end.x, end.y);
  ...
  //																		start.x,
  //start.y, 																		end.x, end.y ); 		start.x += backCenter->getImageWidth();
  //
  //	}  // end for i
  //
  IRegion2D reg;
  ...
  for (i = 0; i < pieces; i++) {
    ...
    TheWindowManager->winDrawImage(barCenter, start.x, start.y, end.x, end.y);
  ...
  for (i = 0; i < pieces; i++) {
    ...
    TheWindowManager->winDrawImage(barRight, start.x, start.y, end.x, end.y);
