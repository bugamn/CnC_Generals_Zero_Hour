...
static void drawHiliteBar(const Image *left, const Image *right,
  ...
  if (centerWidth) {
    ...
    for (i = 0; i < pieces; i++) {
      ...
      TheWindowManager->winDrawImage(smallCenter, start.x, start.y, end.x,
                                     end.y);
...
static void drawListBoxText(GameWindow *window, WinInstanceData *instData,
  ...
  for (i = 0;; i++) {
    ...
    if (list->multiSelect) {
      ...
      if (i == list->selectPos) selected = TRUE;
    }
    ...
    if (selected) {
      if (useImages) {
        ...
        if (BitTest(window->winGetStatus(), WIN_STATUS_ENABLED) == FALSE) {
          ...
          smallCenter =
              GadgetListBoxGetDisabledSelectedItemImageSmallCenter(window);
          ...
          smallCenter =
              GadgetListBoxGetHiliteSelectedItemImageSmallCenter(window);
          ...
          smallCenter =
              GadgetListBoxGetEnabledSelectedItemImageSmallCenter(window);
        ...
        if (end.y > clipRegion.hi.y) end.y = clipRegion.hi.y;
        if (start.y < clipRegion.lo.y) start.y = clipRegion.lo.y;

        if (left && right && center && smallCenter)
          drawHiliteBar(left, right, center, smallCenter, start.x + 1, start.y,
                        end.x, end.y);
        ...
        if (BitTest(window->winGetStatus(), WIN_STATUS_ENABLED) == FALSE) {
          ...
          selectBorder =
              GadgetListBoxGetDisabledSelectedItemBorderColor(window);
        }  // end if, disabled
        else if (BitTest(instData->getState(), WIN_STATE_HILITED)) {
        ...
        if (selectBorder != WIN_COLOR_UNDEFINED)
          TheWindowManager->winOpenRect(selectBorder, WIN_DRAW_LINE_WIDTH,
        ...
        if (selectColor != WIN_COLOR_UNDEFINED)
          TheWindowManager->winFillRect(selectColor, WIN_DRAW_LINE_WIDTH,
                                        start.x, start.y, end.x, end.y);
      ...
      for (Int j = 0; j < list->columns; j++) {
        ...
        if (cells[j].cellType == LISTBOX_TEXT) {
          ...
          if (BitTest(window->winGetStatus(), WIN_STATUS_ONE_LINE) == TRUE) {
            ...
            string->draw(columnX + TEXT_X_OFFSET, drawY, textColor, dropColor);
          ...
          offsetY++;
          if (offsetX < x + 1) offsetX = x + 1;
          TheDisplay->setClipRegion(&columnRegion);
          TheWindowManager->winDrawImage((const Image *)cells[j].data, offsetX,
                                         offsetY, offsetX + width,
                                         offsetY + height, cells[j].color);
...
void W3DGadgetListBoxDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  if (border != WIN_COLOR_UNDEFINED)
    TheWindowManager->winOpenRect(border, WIN_DRAW_LINE_WIDTH, x, y, x + width,
                                  y + height);

  // draw background
  if (background != WIN_COLOR_UNDEFINED)
    TheWindowManager->winFillRect(background, WIN_DRAW_LINE_WIDTH, x + 1, y + 1,
