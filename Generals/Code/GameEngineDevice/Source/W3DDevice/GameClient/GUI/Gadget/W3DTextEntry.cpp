...
static void drawTextEntryText(GameWindow *window, WinInstanceData *instData,
  ...
  if (e->constructText->getTextLength() > 0) {
    e->constructText->draw(x + textWidth, y, compositeColor,
                           compositeDropColor);
  ...
  if (parent && !BitTest(parent->winGetStyle(), GWS_COMBO_BOX)) parent = NULL;

  if ((window == TheWindowManager->winGetFocus() ||
       (parent && parent == TheWindowManager->winGetFocus())) &&
      ((drawCnt++ >> 3) & 0x1))
    TheWindowManager->winFillRect(textColor, WIN_DRAW_LINE_WIDTH, cursorPos,
                                  origin.y + 3, cursorPos + 2,
                                  origin.y + size.y - 3);
...
void W3DGadgetTextEntryDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  Color backBorder, backColor, textColor, textBorder, compositeColor,
      compositeBorder;
  ...
  // draw the edit text
  drawTextEntryText(window, instData, textColor, textBorder, compositeColor,
                    compositeBorder, start.x, start.y, width, fontHeight);
...
//=============================================================================
void W3DGadgetTextEntryImageDraw(GameWindow *window,
  ...
  for (i = 0; i < pieces; i++) {
    ...
    TheWindowManager->winDrawImage(smallCenterImage, start.x, start.y, end.x,
                                   end.y);
