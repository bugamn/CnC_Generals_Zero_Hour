...
static void advancePosition(GameWindow *window, const Image *image,
                            UnsignedInt posX, UnsignedInt posY,
                            UnsignedInt sizeX, UnsignedInt sizeY) {
  if (!image) return;
  ...
  TheDisplay->drawImage(image, x, y, x + image->getImageWidth(),
...
void W3DMainMenuDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  IRegion2D topHorizontal2 = {pos.x, pos.y + (size.y * .1), pos.x + size.x,
                              pos.y + (size.y * .1)};
  IRegion2D topHorizontal2drop = {pos.x, pos.y + (size.y * .12), pos.x + size.x,
                                  pos.y + (size.y * .12)};

  IRegion2D bottomHorizontal1 = {pos.x, pos.y + (size.y * .9), pos.x + size.x,
                                 pos.y + (size.y * .9)};
  IRegion2D bottomHorizontal1drop = {pos.x, pos.y + (size.y * .92),
                                     pos.x + size.x, pos.y + (size.y * .92)};

  IRegion2D bottomHorizontal2 = {pos.x, pos.y + size.y, pos.x + size.x,
                                 pos.y + size.y};
  IRegion2D bottomHorizontal2drop = {pos.x, pos.y + size.y + 1, pos.x + size.x,
                                     pos.y + size.y + 1};

  IRegion2D verticle1 = {pos.x + (size.x * .225), pos.y,
                         pos.x + (size.x * .225), height};
  IRegion2D verticle2 = {pos.x + (size.x * .445), pos.y,
                         pos.x + (size.x * .445), height};
  IRegion2D verticle3 = {pos.x + (size.x * .6662), pos.y,
                         pos.x + (size.x * .6662), height};
  IRegion2D verticle4 = {pos.x + (size.x * .885), pos.y,
                         pos.x + (size.x * .885), height};
  //	static IRegion2D	verticle5 ={pos.x + (size.x * .7250), pos.y +
  //(size.y * .12), pos.x + (size.x * .7250), pos.y + (size.y * .86) 	};
  //	static IRegion2D	verticle6 ={pos.x + (size.x * .9062), pos.y +
  //(size.y * .12), pos.x + (size.x * .9062), pos.y + (size.y * .86) 	};

  TheDisplay->drawLine(topHorizontal1.lo.x, topHorizontal1.lo.y,
                       topHorizontal1.hi.x, topHorizontal1.hi.y, 2, color);
  TheDisplay->drawLine(topHorizontal1drop.lo.x, topHorizontal1drop.lo.y,
                       topHorizontal1drop.hi.x, topHorizontal1drop.hi.y, 2,
                       colorDrop);
  TheDisplay->drawLine(topHorizontal2.lo.x, topHorizontal2.lo.y,
                       topHorizontal2.hi.x, topHorizontal2.hi.y, 1, color);
  TheDisplay->drawLine(topHorizontal2drop.lo.x, topHorizontal2drop.lo.y,
                       topHorizontal2drop.hi.x, topHorizontal2drop.hi.y, 1,
                       colorDrop);
  TheDisplay->drawLine(bottomHorizontal1.lo.x, bottomHorizontal1.lo.y,
                       bottomHorizontal1.hi.x, bottomHorizontal1.hi.y, 1,
                       color);
  TheDisplay->drawLine(bottomHorizontal1drop.lo.x, bottomHorizontal1drop.lo.y,
                       bottomHorizontal1drop.hi.x, bottomHorizontal1drop.hi.y,
                       1, colorDrop);
  TheDisplay->drawLine(bottomHorizontal2.lo.x, bottomHorizontal2.lo.y,
                       bottomHorizontal2.hi.x, bottomHorizontal2.hi.y, 2,
                       color);
  TheDisplay->drawLine(bottomHorizontal2drop.lo.x, bottomHorizontal2drop.lo.y,
                       bottomHorizontal2drop.hi.x, bottomHorizontal2drop.hi.y,
                       2, colorDrop);

  TheDisplay->drawLine(verticle1.lo.x, verticle1.lo.y, verticle1.hi.x,
                       verticle1.hi.y, 3, color);
  TheDisplay->drawLine(verticle2.lo.x, verticle2.lo.y, verticle2.hi.x,
                       verticle2.hi.y, 3, color);
  TheDisplay->drawLine(verticle3.lo.x, verticle3.lo.y, verticle3.hi.x,
                       verticle3.hi.y, 3, color);
  TheDisplay->drawLine(verticle4.lo.x, verticle4.lo.y, verticle4.hi.x,
                       verticle4.hi.y, 3, color);
  ...
  advancePosition(NULL,
                  TheMappedImageCollection->findImageByName("MainMenuPulse"),
...
void W3DMainMenuFourDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  IRegion2D verticle1 = {pos.x + (size.x * .295), pos.y,
                         pos.x + (size.x * .295), height};
  IRegion2D verticle2 = {pos.x + (size.x * .59), pos.y, pos.x + (size.x * .59),
                         height};
  // IRegion2D	verticle3 ={pos.x + (size.x * .6662), pos.y, pos.x + (size.x *
  // .6662), height 	};
  IRegion2D verticle4 = {pos.x + (size.x * .885), pos.y,
                         pos.x + (size.x * .885), height};
  //	static IRegion2D	verticle5 ={pos.x + (size.x * .7250), pos.y +
...
//
//	const Image *image =
//TheMappedImageCollection->findImageByName("LogoGlow");
//
//	if(!image)
//		return;
//
...
//
//	TheDisplay->drawImage(image,x, y , x + image->getImageWidth(), y +
//image->getImageHeight(), GameMakeColor(255,255,255,alpha));
//	//TheDisplay->drawImage(image,x, y , x + image->getImageWidth(), y +
//image->getImageHeight(), GameMakeColor(255,200,250,alpha));
//	TheDisplay->enableClipping(FALSE );
//	//TheDisplay->drawLine();
//
...
void W3DClockDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  dString->draw(textPos.x, textPos.y, GameMakeColor(255, 255, 255, 255),
...
void W3DMainMenuMapBorder(GameWindow *window, WinInstanceData *instData) {
  ...
  if (image) {
    for (x = (originalX + 10); x <= x2; x += BORDER_LINE_SIZE) {
      TheDisplay->drawImage(image, x, y, x + size, y + size);
      TheDisplay->drawImage(image, x, y2, x + size, y2 + size);
    }
    ...
    if ((x2 - x) >= (BORDER_LINE_SIZE / 2)) {
      // Blit Half piece
      TheDisplay->drawImage(image, x, y, x + halfSize, y + size);
      TheDisplay->drawImage(image, x, y2, x + halfSize, y2 + size);

      x += (BORDER_LINE_SIZE / 2);
    }

    // x2 - x ... must now be less than a half piece
    // check for equals and if not blit an adjusted half piece border pieces
    // have a two pixel repeat so we will blit one pixel over if necessary to
    // line up the art, but we'll cover-up the overlap with the corners
    if (x < x2) {
  ...
  if (image) {
    ...
    for (y = (originalY + 10); y <= y2; y += BORDER_LINE_SIZE) {
      TheDisplay->drawImage(image, x, y, x + size, y + size);
      TheDisplay->drawImage(image, x2, y, x2 + size, y + size);
    }
    ...
    if ((y2 - y) >= (BORDER_LINE_SIZE / 2)) {
      // Blit Half piece
      TheDisplay->drawImage(image, x, y, x + size, y + halfSize);
      TheDisplay->drawImage(image, x2, y, x2 + size, y + halfSize);
  ...
  TheDisplay->drawImage(
      TheMappedImageCollection->findImageByName("FrameCornerUL"), x, y,
      x + size, y + size);
  x = maxX - BORDER_CORNER_SIZE;
  y = originalY - BORDER_CORNER_SIZE;
  TheDisplay->drawImage(
      TheMappedImageCollection->findImageByName("FrameCornerUR"), x, y,
      x + size, y + size);
  x = originalX - BORDER_CORNER_SIZE;
  y = maxY - BORDER_CORNER_SIZE;
  TheDisplay->drawImage(
      TheMappedImageCollection->findImageByName("FrameCornerLL"), x, y,
      x + size, y + size);
  x = maxX - BORDER_CORNER_SIZE;
  y = maxY - BORDER_CORNER_SIZE;
  TheDisplay->drawImage(
      TheMappedImageCollection->findImageByName("FrameCornerLR"), x, y,
      x + size, y + size);
...
void W3DMainMenuButtonDropShadowDraw(GameWindow *window,
  ...
  if (leftImage == NULL || rightImage == NULL || centerImage == NULL) return;
  ...
  if (centerWidth <= 0) {
    ...
    start.y = origin.y + yOffset;
        ...
        start.y + size.y + yOffset;  // centerImage->getImageHeight() + yOffset;
    //		TheDisplay->setClipRegion(&clipRegion);
    for (i = 0; i < pieces; i++) {
      end.x = start.x + centerImage->getImageWidth();
      TheWindowManager->winDrawImage(centerImage, start.x, start.y, end.x,
                                     end.y);
  ...
  if (instData->getTextLength()) drawText(window, instData);
  ...
  if (instData->m_videoBuffer) {
    TheDisplay->drawVideoBuffer(instData->m_videoBuffer, start.x, start.y,
                                start.x + size.x, start.y + size.y);
  }
  PushButtonData *pData = (PushButtonData *)window->winGetUserData();
  if (pData) {
    if (pData->overlayImage) {
      TheDisplay->drawImage(pData->overlayImage, start.x, start.y,
                            start.x + size.x, start.y + size.y);
    }
    ...
    if (pData->drawClock) {
      if (pData->drawClock == NORMAL_CLOCK) {
        ...
        TheDisplay->drawRemainingRectClock(start.x, start.y, size.x, size.y,
                                           pData->percentClock,
                                           pData->colorClock);
      }
    ...
    if (pData->drawBorder && pData->colorBorder != GAME_COLOR_UNDEFINED) {
      TheDisplay->drawOpenRect(start.x - 1, start.y - 1, size.x + 2, size.y + 2,
                               1, pData->colorBorder);
    }
  }
...
void W3DMainMenuRandomTextDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  if (!(tData->text && (textColor != WIN_COLOR_UNDEFINED))) return;
...
void W3DThinBorderDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  //	TheDisplay->drawOpenRect(start.x - 1, start.y - 1, size.x + 2, size.y +
  //2, 1, BrownishColor);
  TheDisplay->enableClipping(FALSE);
}
...
void W3DMainMenuInit(WindowLayout *layout, void *userData) {
  ...
  //	NameKeyType buttonWorldBuilderID = TheNameKeyGenerator->nameToKey(
  "MainMenu.wnd:ButtonWorldBuilder" );
  //	GameWindow *buttonWorldBuilder = TheWindowManager->winGetWindowFromId(
  parent, buttonWorldBuilderID );
  //	if (buttonWorldBuilder)
  //
  buttonWorldBuilder->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);

          NameKeyType staticTextRandom1ID = TheNameKeyGenerator->nameToKey(
  "MainMenu.wnd:StaticTextRandom1" ); NameKeyType staticTextRandom2ID =
  TheNameKeyGenerator->nameToKey( "MainMenu.wnd:StaticTextRandom2" ); GameWindow
  *staticTextRandom1 = TheWindowManager->winGetWindowFromId( parent,
  staticTextRandom1ID); GameWindow *staticTextRandom2 =
  TheWindowManager->winGetWindowFromId( parent, staticTextRandom2ID); if
  (staticTextRandom1)
  ...
  //	//NameKeyType getUpdateID = TheNameKeyGenerator->nameToKey(
  "MainMenu.wnd:ButtonGetUpdate" ); NameKeyType buttonUSAID =
  TheNameKeyGenerator->nameToKey( "MainMenu.wnd:ButtonUSA" ); NameKeyType
  buttonGLAID = TheNameKeyGenerator->nameToKey( "MainMenu.wnd:ButtonGLA" );
          NameKeyType buttonChinaID = TheNameKeyGenerator->nameToKey(
  "MainMenu.wnd:ButtonChina" ); NameKeyType skirmishID =
  TheNameKeyGenerator->nameToKey( AsciiString("MainMenu.wnd:ButtonSkirmish") );
          NameKeyType onlineID = TheNameKeyGenerator->nameToKey(
  AsciiString("MainMenu.wnd:ButtonOnline") ); NameKeyType networkID =
  TheNameKeyGenerator->nameToKey( AsciiString("MainMenu.wnd:ButtonNetwork") );

          GameWindow *button = TheWindowManager->winGetWindowFromId( parent,
  skirmishID ); if (button)
  ...
  //GameWindow *getUpdate = TheWindowManager->winGetWindowFromId( parent,
  getUpdateID );
                  //	if (getUpdate)
                  //
  getUpdate->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);

          GameWindow *buttonUSA = TheWindowManager->winGetWindowFromId( parent,
  buttonUSAID ); if (buttonUSA)
                  buttonUSA->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);
          GameWindow *buttonGLA = TheWindowManager->winGetWindowFromId( parent,
  buttonGLAID ); if (buttonGLA)
                  buttonGLA->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);
          GameWindow *buttonChina = TheWindowManager->winGetWindowFromId(
  parent, buttonChinaID ); if (buttonChina)
                  buttonChina->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);

          GameWindow *win = NULL;
          win = TheWindowManager->winGetWindowFromId(parent,
  TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonMultiBack")); if(win)
                  win->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);
          win = TheWindowManager->winGetWindowFromId(parent,
  TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonSingleBack")); if(win)
                  win->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);
          win = TheWindowManager->winGetWindowFromId(parent,
  TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonExit")); if(win)
                  win->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);
          win = TheWindowManager->winGetWindowFromId(parent,
  TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonOptions")); if(win)
                  win->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);
          win = TheWindowManager->winGetWindowFromId(parent,
  TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonMultiplayer")); if(win)
                  win->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);
          win = TheWindowManager->winGetWindowFromId(parent,
  TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonSinglePlayer")); if(win)
                  win->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);
          win = TheWindowManager->winGetWindowFromId(parent,
  TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonReplay")); if(win)
                  win->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);
          win = TheWindowManager->winGetWindowFromId(parent,
  TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonLoadGame")); if(win)
                  win->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);
          win = TheWindowManager->winGetWindowFromId(parent,
  TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonLoadReplay")); if(win)
                  win->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);
          win = TheWindowManager->winGetWindowFromId(parent,
  TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonLoadReplayBack")); if(win)
                  win->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);

          win = TheWindowManager->winGetWindowFromId(parent,
  TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonTRAINING")); if(win)
                  win->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);

          win = TheWindowManager->winGetWindowFromId(parent,
  TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonCredits")); if(win)
                  win->winSetDrawFunc(W3DMainMenuButtonDropShadowDraw);

          GameWindow *clipRegionWin = TheWindowManager->winGetWindowFromId(
  parent, TheNameKeyGenerator->nameToKey( AsciiString("MainMenu.wnd:MapBorder")
  )); Int x,y,width,height; clipRegionWin->winGetScreenPosition(&x, &y);
