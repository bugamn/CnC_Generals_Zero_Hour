...
void W3DCameoMovieDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  if (video) {
    ...
    TheDisplay->drawVideoBuffer(video, pos.x, pos.y, pos.x + size.x,
                                pos.y + size.y);
  }
}  // end W3DLeftHUDDraw
...
void W3DLeftHUDDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  if (video) {
    ...
    window->winGetScreenPosition(&pos.x, &pos.y);
    window->winGetSize(&size.x, &size.y);
    // TheDisplay->drawOpenRect(pos.x, pos.y, size.x,size.y,
    // 1,GameMakeColor(100,100,255,255));
...
//-------------------------------------------------------------------------------------------------
/** Temporary power meter drawing ... there are some problems with this meter,
 * that is what to do when the player doesn't have power, builds a building
 * before a power plant etc, but I don't want to spend time on this since it's
 * not a task right now and I'm sure it will be in the future.  This is just
 * down and dirty to see SOMETHING  (CD) */
//-------------------------------------------------------------------------------------------------
void W3DPowerDraw(GameWindow *window, WinInstanceData *instData) {
  // static const Image *endBarYellow =
  // TheMappedImageCollection->findImageByName("PowerBarYellowEndR"); static
  // const Image *beginBarYellow =
  // TheMappedImageCollection->findImageByName("PowerBarYellowEndL");
  static const Image *centerBarYellow =
      TheMappedImageCollection->findImageByName("PowerPointY");
  // static const Image *endBarRed =
  // TheMappedImageCollection->findImageByName("PowerBarRedEndR"); static const
  // Image *beginBarRed =
  // TheMappedImageCollection->findImageByName("PowerBarRedEndL");
  static const Image *centerBarRed =
      TheMappedImageCollection->findImageByName("PowerPointR");
  // static const Image *endBarGreen =
  // TheMappedImageCollection->findImageByName("PowerBarGreenEndR"); static
  // const Image *beginBarGreen =
  // TheMappedImageCollection->findImageByName("PowerBarGreenEndL");
  static const Image *centerBarGreen =
      TheMappedImageCollection->findImageByName("PowerPointG");
  // const Image *endBar = NULL;
  // const Image *beginBar = NULL;
  const Image *centerBar = NULL;
  static const Image *slider =
      TheMappedImageCollection->findImageByName("PowerBarSlider");
  Player *player = NULL;
  ...
  if (!player || !TheGlobalData) return;
  ...
  if ((consumption > energy->getProduction() - delta) &&
      (consumption <= energy->getProduction())) {
  ...
  if (!slider || !centerBar) return;

  Int range;
  range = logN(production, TheGlobalData->m_powerBarBase) *
          (size.x / TheGlobalData->m_powerBarIntervals);
  if (range >= size.x) range = size.x;
  ...
  if (centerWidth > 0) {
    ...
    //		start.y = pos.y;
    //		end = leftEnd;
    //		TheWindowManager->winDrawImage(beginBar, start.x, start.y,
    //end.x, end.y);
    //
    //		// draw right end
    //		start = rightStart;
    ...
    //		TheWindowManager->winDrawImage(endBar, start.x, start.y, end.x,
    //end.y);
  }
  Int posXstart;
  Int posXend;
  Real consumptionForNeedle =
      (consumption == 1)
          ? 1.5f
          : INT_TO_REAL(consumption);  // Log(1) == 0, but we need to show
                                       // something for 1 power used.
  range = logN(consumptionForNeedle, TheGlobalData->m_powerBarBase) *
          (size.x / TheGlobalData->m_powerBarIntervals);
  if (centerWidth <= 0 && range <= 0) return;
  if (range >= size.x) {
  ...
  TheWindowManager->winDrawImage(slider, posXstart,
                                 pos.y + size.y - slider->getImageHeight(),
                                 posXend, pos.y + size.y);
}

void W3DPowerDrawA(GameWindow *window, WinInstanceData *instData) {
  static const Image *endBarYellow =
      TheMappedImageCollection->findImageByName("PowerBarYellowEndR");
  static const Image *beginBarYellow =
      TheMappedImageCollection->findImageByName("PowerBarYellowEndL");
  static const Image *centerBarYellow =
      TheMappedImageCollection->findImageByName("PowerBarYellow");
  static const Image *endBarRed =
      TheMappedImageCollection->findImageByName("PowerBarRedEndR");
  static const Image *beginBarRed =
      TheMappedImageCollection->findImageByName("PowerBarRedEndL");
  static const Image *centerBarRed =
      TheMappedImageCollection->findImageByName("PowerBarRed");
  static const Image *endBarGreen =
      TheMappedImageCollection->findImageByName("PowerBarGreenEndR");
  static const Image *beginBarGreen =
      TheMappedImageCollection->findImageByName("PowerBarGreenEndL");
  static const Image *centerBarGreen =
      TheMappedImageCollection->findImageByName("PowerBarGreen");
  const Image *endBar = NULL;
  const Image *beginBar = NULL;
  const Image *centerBar = NULL;
  static const Image *slider =
  ...
  // slider = TheMappedImageCollection->findImageByName("PowerBarSlider");
  if (!slider || !endBar || !beginBar || !centerBar) return;
...
void W3DCommandBarGridDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  TheDisplay->drawLine(pos.x, pos.y + size.y * .33, pos.x + size.x,
                       pos.y + size.y * .33, 1, color);
  TheDisplay->drawLine(pos.x, pos.y + size.y * .66, pos.x + size.x,
                       pos.y + size.y * .66, 1, color);
  TheDisplay->drawLine(pos.x + size.x * .33, pos.y, pos.x + size.x * .33,
                       pos.y + size.y, 1, color);
  TheDisplay->drawLine(pos.x + size.x * .66, pos.y, pos.x + size.x * .66,
                       pos.y + size.y, 1, color);
}
...
void W3DCommandBarGenExpDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  static const Image *endBar =
      TheMappedImageCollection->findImageByName("GenExpBarTop1");
  static const Image *beginBar =
      TheMappedImageCollection->findImageByName("GenExpBarBottom1");
  static const Image *centerBar =
      TheMappedImageCollection->findImageByName("GenExpBar1");
  Int progress;
  progress =
  ...
  // GS This should never be necessary, but scripts can change the points
  // required or even disable a level. A disabled level will be -1 for points
  // required.  Just be totally safe and bind to 100, and we will fix the
  // scripts to bind the points gained later.
  ...
  if (!endBar || !beginBar || !centerBar) return;
...
void W3DCommandBarTopDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  //	TheDisplay->drawImage(TheControlBar->getStarImage(), imageStart.x,
  //imageStart.y, imageEnd.x, imageEnd.y);
}

void W3DCommandBarBackgroundDraw(GameWindow *window,
                                 WinInstanceData *instData) {
  ControlBarSchemeManager *man = TheControlBar->getControlBarSchemeManager();
  if (!man) return;
  static NameKeyType winNamekey = TheNameKeyGenerator->nameToKey(
...
void W3DCommandBarForegroundDraw(GameWindow *window,
                                 WinInstanceData *instData) {
  ControlBarSchemeManager *man = TheControlBar->getControlBarSchemeManager();
  if (!man) return;
...
void W3DDrawMapPreview(GameWindow *window, WinInstanceData *instData) {
  ...
  if (!BitTest(window->winGetStatus(), WIN_STATUS_IMAGE) ||
      !window->winGetEnabledImage(0))
    TheDisplay->drawFillRect(ul.x, ul.y, lr.x - ul.x, lr.y - ul.y, lineColor);
  else
    TheDisplay->drawImage(window->winGetEnabledImage(0), ul.x, ul.y, lr.x,
                          lr.y);
  ...
  if (image)
    while (it != TheSupplyAndTechImageLocations.m_techPosList.end()) {
      TheDisplay->drawImage(image, pixelX + it->x, pixelY + it->y,
                            pixelX + it->x + SUPPLY_TECH_SIZE,
                            pixelY + it->y + SUPPLY_TECH_SIZE);
      it++;
    }
...
void drawSkinnyBorder(Int x, Int y, Int width, Int height) {
  ...
  for (x = (originalX + 3); x <= x2; x += BORDER_LINE_SIZE) {
    TheDisplay->drawImage(image1, x, y, x + size, y + size);
    TheDisplay->drawImage(image2, x, y2, x + size, y2 + size);
  }
  ...
  if ((x2 - x) >= (BORDER_LINE_SIZE / 2)) {
    // Blit Half piece
    TheDisplay->drawImage(image1, x, y, x + halfSize, y + size);
    TheDisplay->drawImage(image2, x, y2, x + halfSize, y2 + size);
  ...
  for (y = (originalY + 3); y <= y2; y += BORDER_LINE_SIZE) {
    TheDisplay->drawImage(image1, x, y, x + size, y + size);
    TheDisplay->drawImage(image2, x2, y, x2 + size, y + size);
  }
  ...
  if ((y2 - y) >= (BORDER_LINE_SIZE / 2)) {
    // Blit Half piece
    TheDisplay->drawImage(image1, x, y, x + size, y + halfSize);
    TheDisplay->drawImage(image2, x2, y, x2 + size, y + halfSize);
...
void W3DCommandBarHelpPopupDraw(GameWindow *window, WinInstanceData *instData) {
  static const Image *endBar =
      TheMappedImageCollection->findImageByName("Helpbox-top");
  static const Image *beginBar =
      TheMappedImageCollection->findImageByName("Helpbox-bottom");
  static const Image *centerBar =
      TheMappedImageCollection->findImageByName("Helpbox-middle");
