...
void W3DGameWindow::blitBorderRect(Int x, Int y, Int width, Int height) {
  ...
  if (bordersInit == FALSE) initBorders();
  ...
  for (x = (originalX + OffsetLower); x <= x2; x += BORDER_LINE_SIZE) {
    TheDisplay->drawImage(borderPieces[BORDER_HORIZONTAL_TOP], x, y, x + size,
                          y + size);
    TheDisplay->drawImage(borderPieces[BORDER_HORIZONTAL_BOTTOM], x, y2,
                          x + size, y2 + size);
  }
  ...
  if ((x2 - x) >= (BORDER_LINE_SIZE / 2)) {
    // Blit Half piece
    TheDisplay->drawImage(borderPieces[BORDER_HORIZONTAL_TOP_SHORT], x, y,
                          x + halfSize, y + size);
    TheDisplay->drawImage(borderPieces[BORDER_HORIZONTAL_BOTTOM_SHORT], x, y2,
                          x + halfSize, y2 + size);
  ...
  for (y = (originalY + OffsetLower); y <= y2; y += BORDER_LINE_SIZE) {
    TheDisplay->drawImage(borderPieces[BORDER_VERTICAL_LEFT], x, y, x + size,
                          y + size);
    TheDisplay->drawImage(borderPieces[BORDER_VERTICAL_RIGHT], x2, y, x2 + size,
                          y + size);
  }
  ...
  if ((y2 - y) >= (BORDER_LINE_SIZE / 2)) {
    // Blit Half piece
    TheDisplay->drawImage(borderPieces[BORDER_VERTICAL_LEFT_SHORT], x, y,
                          x + size, y + halfSize);
    TheDisplay->drawImage(borderPieces[BORDER_VERTICAL_RIGHT_SHORT], x2, y,
                          x2 + size, y + halfSize);
  ...
  TheDisplay->drawImage(borderPieces[BORDER_CORNER_UL], x, y, x + size,
                        y + size);
  x = maxX - 5;  // BORDER_CORNER_SIZE;
  y = originalY - BORDER_CORNER_SIZE;
  TheDisplay->drawImage(borderPieces[BORDER_CORNER_UR], x, y, x + size,
                        y + size);
  x = originalX - BORDER_CORNER_SIZE;
  y = maxY - 5;  // BORDER_CORNER_SIZE;
  TheDisplay->drawImage(borderPieces[BORDER_CORNER_LL], x, y, x + size,
                        y + size);
  x = maxX - 5;  // BORDER_CORNER_SIZE;
  y = maxY - 5;  // BORDER_CORNER_SIZE;
  TheDisplay->drawImage(borderPieces[BORDER_CORNER_LR], x, y, x + size,
                        y + size);
...
// W3DGameWindow::~W3DGameWindow ==============================================
//=============================================================================
W3DGameWindow::~W3DGameWindow(void) {}  // end ~W3DGameWindow
...
void W3DGameWinDefaultDraw(GameWindow *window, WinInstanceData *instData) {
  ...
  if (BitTest(window->winGetStatus(), WIN_STATUS_IMAGE)) {
    ...
    if (borderColor != WIN_COLOR_UNDEFINED)
      TheWindowManager->winOpenRect(borderColor, borderWidth, origin.x,
                                    origin.y, origin.x + size.x,
                                    origin.y + size.y);

    // draw filled background
    if (color != WIN_COLOR_UNDEFINED)
      TheWindowManager->winFillRect(
          color, borderWidth, origin.x + borderWidth, origin.y + borderWidth,
          origin.x + size.x - borderWidth, origin.y + size.y - borderWidth);
  ...
  if (instData->m_videoBuffer) {
    ...
    TheDisplay->drawVideoBuffer(instData->m_videoBuffer, pos.x, pos.y,
      ...
      switch (m_instData.getStyle() & bits) {
        ...
        case GWS_ENTRY_FIELD: {
          ...
          // an entry box so I am removing this.
                                                  // Adjust entry box if an
          entryWidth is provided if( (e->entryWidth > 0) && (width >
          e->entryWidth) )
                                                  {
                                                          width = e->entryWidth;
                                                          x = originalX +
          (m_size.x - e->entryWidth);
                                                  }
          */
        ...
        case GWS_HORZ_SLIDER:
          //					blitBorderLine( originalX,
          //originalY, window->size.x, window->size.y );
        ...
        case GWS_SCROLL_LISTBOX: {
          ...
          if (m_instData.getTextLength()) labelAdjustment = 4;

          blitBorderRect((originalX - 3), (originalY - (3 + labelAdjustment)),
                         (m_size.x + 3 - sliderAdjustment), (m_size.y + 6));
...
void W3DGameWindow::getTextSize(Int *width, Int *height) {
  ...
  if (width) *width = extents.X;
  if (height) *height = extents.Y;
