...
static granny_pnt332_vertex
    g_blendingBuffer[4096];  ///< temporary workspace for granny (all models <
                             ///< 4096 verts).

/** Local function used to find animation matching the given model */
static int FindTrackGroupFor(granny_animation *Animation,
                             granny_model_instance *ModelInstance) {
  if (Animation && ModelInstance) {
    {
      for (Int TrackGroupIndex = 0;
           TrackGroupIndex < Animation->TrackGroupCount; ++TrackGroupIndex) {
...
/** Local function used to modify the original granny data - flip coordintes,
 * scale, etc. */
static void TransformFile(granny_file_info *FileInfo) {
  ...
  GrannyAutoComputeBasisConversion(
      FileInfo, 1.0f, Origin, RightVector, UpVector, BackVector, Affine3,
      (float *)Linear3x3, (float *)InverseLinear3x3);

  GrannyAutoTransformFile(FileInfo, Affine3, (float *)Linear3x3,
...
GrannyRenderObjClass::~GrannyRenderObjClass(void) {
  if (m_animationControl) GrannyFreeControl(m_animationControl);
  if (m_modelInstance) GrannyFreeModelInstance(m_modelInstance);
  freeResources();
}
...
GrannyRenderObjClass::GrannyRenderObjClass(const GrannyPrototypeClass &proto)
  ...
  if (fileInfo) {
    ...
    if (m_modelInstance) {
      ...
      GrannyAutoBindModel(m_modelInstance, _GrannyLoader.Get_Material_Library(),
                          1);
      //        GrannyAddModelToScene(Global.Scene, ModelInstance);
      //        ///@todo: Create a granny scene for quicker updates.
...
//=============================================================================
void GrannyRenderObjClass::Get_Obj_Space_Bounding_Sphere(
...
/** WW3D method that returns intersection point between ray and model.
        We will only return results of a simple 'pick box' - not full triangle
   level detail.
*/
//=============================================================================
 ...
 */
void GrannyRenderObjClass::Set_Animation(HAnimClass *motion, float frame,
                                         int anim_mode) {
  ...
  if (Animation) {
    ...
    if (trackIndex != -1) {
      ...
      m_animationControl = GrannyPlayControlledAnimation(
          frame / gAnim->FrameRate, Animation, trackIndex, m_modelInstance, 0);
      if (m_animationControl) {
        GrannySetControlSpeed(m_animationControl, 1.0f);  // play at normal
                                                          // speed
...
void GrannyRenderObjClass::Render(RenderInfoClass &rinfo) {
  ...
  GrannySampleModelAnimations(m_modelInstance, 0,
  ...
  for (Int MeshIndex = 0; MeshIndex < MeshCount; ++MeshIndex) {
    granny_mesh_instance *Mesh =
        (granny_mesh_instance *)GrannyGetModelMeshCookie(m_modelInstance,
                                                         MeshIndex);
    granny_mesh_model_binding *Binding = GrannyGetMeshModelBinding(Mesh);

    granny_pnt332_vertex *Vertices = 0;
    if (GrannyMeshIsRigid(Mesh)) {
      assert(0);  // have not coded support for rigid meshes yet - only soft
                  // skinned supported.
      ...
      GrannyDeformMesh(Binding, GrannyGetModelWorldPose(m_modelInstance), 0,
                       g_blendingBuffer);
    ...
    int GroupCount = GrannyGetMeshTriangleGroupCount(Mesh);
    {
      for (Int GroupIndex = 0; GroupIndex < GroupCount; ++GroupIndex) {
        granny_material_instance *Material =
            (granny_material_instance *)GrannyGetMeshMaterialCookie(
                Mesh,
                GrannyGetMeshTriangleGroupMaterialIndex(Mesh, GroupIndex));
        if (Material) {
          Int TextureIndex;
          if (GrannyGetMaterialTextureIndexByType(
            ...
            if (TextureInstance) {
              DX8Wrapper::Set_Texture(
                  0, (TextureClass *)GrannyGetTextureCookie(TextureInstance));
            } else
              DX8Wrapper::Set_Texture(0, NULL);
          }
        }

        DynamicVBAccessClass vb_access(BUFFER_TYPE_DYNAMIC_DX8,
                                       dynamic_fvf_type, m_vertexCount);
        {
        ...
        UnsignedInt *indices =
            (UnsignedInt *)GrannyGetMeshTriangleGroupIndices(Mesh, GroupIndex);
        ...
        DX8Wrapper::Draw_Triangles(
            0, indexCount / 3, 0,
            m_vertexCount);  // draw a quad, 2 triangles, 4 verts
      }
    }
  }
}
...
/**
GrannyLoaderClass::Load -- reads in a granny model and creates a prototype for
it
*/
PrototypeClass *GrannyLoaderClass::Load_W3D(const char *filename) {
  ...
  if (File) {
    ...
    if (fileInfo) {
      ...
      for (int TextureIndex = 0; TextureIndex < fileInfo->TextureCount;
           ++TextureIndex) {
        _splitpath(fileInfo->Textures[TextureIndex]->FromFileName, drive, dir,
                   fname, ext);
        TextureClass *TextureHandle =
            WW3DAssetManager::Get_Instance()->Get_Texture(
                strncat(fname, ".tga", 4));
        granny_texture_instance *TextureInstance =
            GrannyInstantiateTexture(fileInfo->Textures[TextureIndex]);
      ...
      // Calculate bounding box and find maximum number of vertices needed to
      // render mesh.
      for (Int modelIndex = 0; modelIndex < fileInfo->ModelCount;
           modelIndex++) {
        granny_model *sourceModel = fileInfo->Models[modelIndex];
        if (stricmp(sourceModel->Name, "AABOX") ==
            0) {  // found a collision box, copy out data
          int MeshCount = sourceModel->MeshBindingCount;
          if (MeshCount == 1) {
            granny_mesh *sourceMesh = sourceModel->MeshBindings[0].Mesh;
            granny_pn33_vertex *Vertices =
                (granny_pn33_vertex *)sourceMesh->PrimaryVertexData->Vertices;
            Vector3 points[24];

            assert(sourceMesh->PrimaryVertexData->VertexCount <= 24);

            for (Int boxVertex = 0;
                 boxVertex < sourceMesh->PrimaryVertexData->VertexCount;
                 boxVertex++) {
          ...
          for (Int meshIndex = 0; meshIndex < meshCount; meshIndex++) {
            ...
            if (sourceMesh->PrimaryVertexData) {
              ...
              meshData[meshIndex].vertex =
                  (granny_pnt332_vertex *)
                      sourceMesh->PrimaryVertexData->Vertices;
              meshData[meshIndex].vertexCount =
                  sourceMesh->PrimaryVertexData->VertexCount;
            }
            meshData[meshIndex].index = NULL;
            meshData[meshIndex].indexCount = NULL;
            if (sourceMesh->PrimaryTopology) {
              assert(sourceMesh->PrimaryTopology->GroupCount ==
                     1);  // should only have 1 material per mesh!
              granny_tri_material_group &sourceGroup =
                  sourceMesh->PrimaryTopology->Groups[0];
              ...
              if (sourceMesh->PrimaryTopology->IndexCount) {
                meshData[meshIndex].index =
                    (const UnsignedInt *)&sourceMesh->PrimaryTopology
                        ->Indices[3 * sourceGroup.TriFirst];
                // These are the indices for this group
                meshData[meshIndex].indexCount = 3 * sourceGroup.TriCount;
                indexCount +=
                    meshData[meshIndex].indexCount;  // keep track of total
                                                     // indices in this model
              }
            }
          }
        }
      }
      ///@todo: Convert granny materials into W3D/D3D materials - now using
      ///default.
      //            for(int MaterialIndex = 0; MaterialIndex <
      //            fileInfo->MaterialCount; ++MaterialIndex)
      //            {
      //			    granny_material_instance *MaterialInstance =
      //GrannyInstantiateMaterial(fileInfo->Materials[MaterialIndex]);
    ...
    for (Int i = 0; i < meshCount; i++) hproto->setMeshData(meshData[i], i);
    return hproto;
  }  // FILE
...
int GrannyAnimManagerClass::Load_Anim(const char *name) {
  ...
  if (newanim->Load_W3D(name) != GrannyAnimClass::OK) {  // load failed!
    newanim->Release_Ref();
    goto Error;
  } else if (Peek_Anim(newanim->Get_Name()) != NULL) {  // duplicate exists!
...
GrannyAnimClass::GrannyAnimClass(void)
    : NumFrames(0), FrameRate(0), File(0), Animation(0) {
  Name[0] = '\0';
}

/** GrannyAnimClass::~GrannyAnimClass -- Destructor */
GrannyAnimClass::~GrannyAnimClass(void) { GrannyFreeFile(File); }
...
/** Adds render object to a queue with other objects using the same material.
   These queues will be flushed at the end of the frame.
*/
void GrannyRenderObjSystem::AddRenderObject(RenderInfoClass &rinfo,
  ...
  for (Int i = 0; i < m_renderStateCount;
       i++) {  // search for other objects with same render state
    // right now we can just sort by prototype since prototypes have fixed
    // materials.
    if (m_renderStateModelList[i].list->m_prototype.m_file ==
        robj->m_prototype.m_file) {  // found model with same prototype.  Add
                                     // new model to same list.
...
void GrannyRenderObjSystem::Flush(void) {
  ...
  for (Int i = 0; i < m_renderStateCount; i++) {
    robj = m_renderStateModelList[i]
               .list;    // set to start of objects using this material
    ...
    while (robj) {
      ...
      robj->LastSyncTime = WW3D::Get_Sync_Time() * 0.001f;  // convert to
                                                            // seconds

      GrannySampleModelAnimations(modelInstance, 0,
                                  GrannyGetModelBoneCount(modelInstance),
                                  GrannyGetModelLocalPose(modelInstance));
      GrannyBuildModelWorldTransforms(
          modelInstance, 0, GrannyGetModelBoneCount(modelInstance),
          GrannyGetModelLocalPose(modelInstance),
      ...
      for (Int MeshIndex = 0; MeshIndex < MeshCount; ++MeshIndex) {
        granny_mesh_instance *Mesh =
            (granny_mesh_instance *)GrannyGetModelMeshCookie(modelInstance,
                                                             MeshIndex);
        granny_mesh_model_binding *Binding = GrannyGetMeshModelBinding(Mesh);
        ...
        if (GrannyMeshIsRigid(Mesh)) {
          ...
          GrannyDeformMesh(Binding, GrannyGetModelWorldPose(modelInstance), 0,
                           g_blendingBuffer);
          Vertices = g_blendingBuffer;
        }
        ...
        for (Int GroupIndex = 0; GroupIndex < GroupCount; ++GroupIndex) {
          ...
          DynamicVBAccessClass vb_access(BUFFER_TYPE_DYNAMIC_DX8,
                                         dynamic_fvf_type, robj->m_vertexCount);
          {
            DynamicVBAccessClass::WriteLockClass lock(&vb_access);
            VertexFormatXYZNDUV2 *vb = lock.Get_Formatted_Vertex_Array();
            if (vb) {
          ...
          if (setMaterial) {  // we started using a new material queue - really
                              // a new model prototype
            // so reset all relevant data.
            indexCount = GrannyGetMeshTriangleGroupIndexCount(Mesh, GroupIndex);
            UnsignedInt *indices =
                (UnsignedInt *)GrannyGetMeshTriangleGroupIndices(Mesh,
                                                                 GroupIndex);
          ...
          DX8Wrapper::Draw_Triangles(
              0, indexCount / 3, 0,
              robj->m_vertexCount);  // draw a quad, 2 triangles, 4 verts
        }
      }
