...
//			  which of our rendering features are supported.  The
//system allows 			  setting up a few custom shaders that are selected based on video
//			  card features.
//
...
//			  3) Create list of alternate shaders sorted by order of
//preference. 				 The first shader which passes hardware validation will be
//selected. 			  4) Add list from step 3) to MasterShaderList[].
//
//-----------------------------------------------------------------------------
...
/** Interface definition for custom shaders we define in our app.  These shaders
   can perform more complex operations than those allowed in the WW3D2 shader
   system.
*/
class W3DShaderInterface {
 public:
  ...
  virtual Int init(
      void) = 0;  ///< perform any one time initialization and validation
...
// this table will contain custom versions of each shader tuned for specific
// video card and user options.
static W3DFilterInterface *W3DFilters[FT_MAX];
static W3DShaderInterface *W3DShaders[W3DShaderManager::ST_MAX];
static Int
    W3DShadersPassCount[W3DShaderManager::ST_MAX];  // number of passes for each
                                                    // of the above shaders
TextureClass *W3DShaderManager::m_Textures[8];
W3DShaderManager::ShaderTypes W3DShaderManager::m_currentShader;
FilterTypes W3DShaderManager::m_currentFilter =
    FT_NULL_FILTER;  ///< Last filter that was set.
...
IDirect3DSurface8 *W3DShaderManager::m_oldRenderSurface =
    NULL;  ///< previous render target
IDirect3DTexture8 *W3DShaderManager::m_renderTexture =
    NULL;  ///< texture into which rendering will be redirected.
IDirect3DSurface8 *W3DShaderManager::m_newRenderSurface =
    NULL;  ///< new render target inside m_renderTexture
IDirect3DSurface8 *W3DShaderManager::m_oldDepthSurface =
    NULL;  ///< previous depth buffer surface
/*===========================================================================================*/
/*=========      Screen Shaders
 * =============================================================*/
/*===========================================================================================*/
...
ScreenBWFilterDOT3 screenBWFilterDOT3;  // slower version for older cards
...
W3DFilterInterface *ScreenBWFilterList[] = {
    &screenBWFilter,
    &screenBWFilterDOT3,  // slower version for older cards without pixel
                          // shaders.
    NULL};
...
Int ScreenBWFilter::init(void) {
  ...
  if ((res = W3DShaderManager::getChipset()) != 0) {
    if (res >= DC_GENERIC_PIXEL_SHADER_1_1) {
      ...
      // shader decleration
      DWORD Declaration[] = {
          (D3DVSD_STREAM(0)), (D3DVSD_REG(0, D3DVSDT_FLOAT3)),  // Position
          (D3DVSD_REG(1, D3DVSDT_D3DCOLOR)),                    // Diffuse
          (D3DVSD_REG(2, D3DVSDT_FLOAT2)),  //  Texture Coordinates
          (D3DVSD_END())};

      // Monochrome pixel shader.
      hr = W3DShaderManager::LoadAndCreateD3DShader("shaders\\monochrome.pso",
                                                    &Declaration[0], 0, false,
                                                    &m_dwBWPixelShader);
...
Bool ScreenBWFilter::postRender(enum FilterModes mode, Coord2D &scrollDelta,
                                Bool &doExtraRender) {
  ...
  DX8Wrapper::_Get_D3D_Device8()->SetTexture(
      0, tex);  // previously rendered frame inside this texture
  ...
  pDev->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, v,
...
Int ScreenBWFilter::set(enum FilterModes mode) {
  ...
  if (mode > FM_NULL_MODE) {  // rendering a quad with redirected rendering
                              // surface tinted by pixel shader

    if (m_fadeDirection > 0) {  // turning effect on
      m_curFadeFrame++;
    ...
    } else if (m_fadeDirection < 0) {  // turning effect off
      m_curFadeFrame++;
      Int fade = m_curFadeFrame;
      if (fade < m_fadeFrames) {
    ...
    DX8Wrapper::_Get_D3D_Device8()->SetPixelShaderConstant(
        0, D3DXVECTOR4(0.3f, 0.59f, 0.11f, 1.0f), 1);

    D3DXVECTOR4 color(1.0f, 1.0f, 1.0f, 1.0f);  // multiply color

    if (mode == FM_VIEW_BW_BLACK_AND_WHITE) {  // back & white mode
      color.x = 1.0f;
      color.y = 1.0f;
      color.z = 1.0f;
    }
    if (mode == FM_VIEW_BW_RED_AND_WHITE) {  // red is on
      color.x = 1.0f;
      color.y = 0.0f;
      color.z = 0.0f;
      // inverse red is on
    ...
    DX8Wrapper::_Get_D3D_Device8()->SetPixelShaderConstant(
        2, D3DXVECTOR4(m_curFadeValue, m_curFadeValue, m_curFadeValue, 1.0f),
        1);
    /*		DX8Wrapper::_Get_D3D_Device8()->SetPixelShaderConstant(2,
       D3DXVECTOR4(150.0f/255.0f, 150.0f/255.0f, 150.0f/255.0f, 0.0f), 1);
                    DX8Wrapper::_Get_D3D_Device8()->SetPixelShaderConstant(3,
       D3DXVECTOR4((765.0f/450.0f)/3, (765.0f/450.0f)/3,
       (765.0f/450.0f)/3, 1.0f), 1);
                    DX8Wrapper::_Get_D3D_Device8()->SetPixelShaderConstant(4,
       D3DXVECTOR4(0.5f, 0.5f, 0.5f, 0), 1);
                    DX8Wrapper::_Get_D3D_Device8()->SetPixelShaderConstant(5,
       D3DXVECTOR4((60.0f)/255.0f, (60.0f)/255.0f, (60.0f)/255.0f, 0), 1);
                    DX8Wrapper::_Get_D3D_Device8()->SetPixelShaderConstant(6,
       D3DXVECTOR4((157.0f)/255.0f, (157.0f)/255.0f, (157.0f)/255.0f, 0), 1);
                    DX8Wrapper::_Get_D3D_Device8()->SetPixelShaderConstant(7,
       D3DXVECTOR4((30.0f)/255.0f, (30.0f)/255.0f, (30.0f)/255.0f, 0), 1);
    */
    return true;
  }
  return false;
...
void ScreenBWFilter::reset(void) {
  DX8Wrapper::_Get_D3D_Device8()->SetTexture(
      0, NULL);  // previously rendered frame inside this texture
...
/**Alternate version of the above filter which does not require pixel shaders -
...
Bool ScreenBWFilterDOT3::postRender(enum FilterModes mode, Coord2D &scrollDelta,
                                    Bool &doExtraRender) {
  IDirect3DTexture8 *tex = W3DShaderManager::endRenderToTexture();
  DEBUG_ASSERTCRASH(tex, ("Require rendered texture."));
  if (!tex) return false;
  ...
  DWORD currentFade = (((Int)((1.0f - m_curFadeValue) * 255.0f)) << 24) |
                      0x00ffffff;  // store alpha value
  ...
  if (DX8Caps::Support_DOT3()) {  // Override W3D states with customizations for
                                  // grayscale
    DX8Wrapper::Set_DX8_Render_State(D3DRS_TEXTUREFACTOR, 0x80A5CA8E);
    DX8Wrapper::Set_DX8_Texture_Stage_State(
        0, D3DTSS_COLORARG0, D3DTA_TFACTOR | D3DTA_ALPHAREPLICATE);
    DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_COLORARG1, D3DTA_TEXTURE);
    DX8Wrapper::Set_DX8_Texture_Stage_State(
        0, D3DTSS_COLORARG2, D3DTA_TFACTOR | D3DTA_ALPHAREPLICATE);
    ...
    DX8Wrapper::Set_DX8_Texture_Stage_State(1, D3DTSS_COLOROP,
                                            D3DTOP_DOTPRODUCT3);
  } else {  // doesn't have DOT3 blend mode so fake it another way.
...
Int ScreenBWFilterDOT3::shutdown(void) { return TRUE; }

/*=========  ScreenCrossFadeFilter
 * =============================================================*/
/// Fades screen between 2 different views of the scene with both being visible
/// at once.
...
W3DFilterInterface *ScreenCrossFadeFilterList[] = {&screenCrossFadeFilter,
                                                   NULL};

Int ScreenCrossFadeFilter::init(void) {
  if (!TheDisplay)
    return FALSE;  // effect is useless without a view so no point initializing
                   // for the WB, etc.
  ...
  m_fadePatternTexture =
      WW3DAssetManager::Get_Instance()->Get_Texture("exmask_g.tga");
  if (!m_fadePatternTexture) return FALSE;
...
Bool ScreenCrossFadeFilter::preRender(Bool &skipRender,
                                      CustomScenePassModes &scenePassMode) {
  if (updateFadeLevel()) {  // if fade has not completed
    W3DShaderManager::startRenderToTexture();
    scenePassMode = SCENE_PASS_ALPHA_MASK;
    skipRender = false;
    m_skipRender =
        true;  // tell the postRender function not to draw into framebuffer yet.
    return true;
  }
...
Bool ScreenCrossFadeFilter::postRender(enum FilterModes mode,
                                       Coord2D &scrollDelta,
                                       Bool &doExtraRender) {
  IDirect3DTexture8 *tex;

  if (m_skipRender) {
    // don't render anything to frame buffer because we still need to draw the
    // new scene that we're fading into.  Okay to render on the next call.
    m_skipRender = false;
    doExtraRender = TRUE;
  ...
  if (mode == FM_VIEW_CROSSFADE_CIRCLE) {
    DX8Wrapper::_Get_D3D_Device8()->SetTexture(
        1, m_fadePatternTexture->Peek_DX8_Texture());
    // Use the current fade level to scale the mask texture, for other modes the
    // texture comes pre-scaled so doesn't require uv scaling.
  ...
  */
  ...
  DWORD diffuse =
      0xffffffff;  //((Int)((m_curFadeValue) * 255.0f) << 24) | 0x00ffffff;
                   ////store alpha value in vertex diffuse
...
Int ScreenCrossFadeFilter::set(enum FilterModes mode) {
  if (mode >
      FM_NULL_MODE) {  // rendering a quad with redirected rendering surface
    VertexMaterialClass *vmat =
        VertexMaterialClass::Get_Preset(VertexMaterialClass::PRELIT_DIFFUSE);
    DX8Wrapper::Set_Material(vmat);
    REF_PTR_RELEASE(vmat);  // no need to keep a reference since it's a preset.
    ...
    if (mode == FM_VIEW_CROSSFADE_CIRCLE) {  // cross-fading using circle mask
                                             // stored in stage 1
      ...
      DX8Wrapper::Set_DX8_Texture_Stage_State(1, D3DTSS_ADDRESSV,
                                              D3DTADDRESS_CLAMP);
      DX8Wrapper::Set_DX8_Texture_Stage_State(1, D3DTSS_MIPFILTER,
                                              D3DTEXF_NONE);
    }
...
ScreenMotionBlurFilter::ScreenMotionBlurFilter()
    : m_decrement(false), m_maxCount(0), m_lastFrame(0), m_skipRender(false) {}
/// List of different motion blur implementations in order of preference
W3DFilterInterface *ScreenMotionBlurFilterList[] = {&screenMotionBlurFilter,
                                                    NULL};
...
Bool ScreenMotionBlurFilter::postRender(enum FilterModes mode,
  ...
  if (mode >= FM_VIEW_MB_PAN_ALPHA) {
    Real len =
        sqrt(scrollDelta.x * scrollDelta.x + scrollDelta.y * scrollDelta.y);
    ...
    Real len =
        sqrt(m_priorDelta.x * m_priorDelta.x + m_priorDelta.y * m_priorDelta.y);
...
Bool ScreenMotionBlurFilter::setup(enum FilterModes mode) {
  ...
  m_skipRender = false;
  if (mode != FM_VIEW_MB_END_PAN_ALPHA) m_maxCount = 0;
  m_decrement = false;
  m_skipRender = false;
  switch (mode) {
...
Int ScreenMotionBlurFilter::shutdown(void) { return TRUE; }
...
class ShroudTextureShader : public W3DShaderInterface {
  virtual Int set(Int pass);  ///< setup shader for the specified rendering
                              ///< pass.
  virtual Int init(
      void);  ///< perform any one time initialization and validation
  virtual void reset(
      void);  ///< do any custom resetting necessary to bring W3D in sync.
  Int m_stageOfSet;
} shroudTextureShader;

/// List of different shroud shader implementations in order of preference
W3DShaderInterface *ShroudShaderList[] = {&shroudTextureShader, NULL};

// #define SHROUD_STRETCH_FACTOR	(1.0f/MAP_XY_FACTOR)	//1 texel per
// heightmap cell width
...
Int ShroudTextureShader::set(Int stage) {
  // force WW3D2 system to set it's states so it won't later overwrite our
  // custom settings.
  VertexMaterialClass *vmat =
      VertexMaterialClass::Get_Preset(VertexMaterialClass::PRELIT_DIFFUSE);
  DX8Wrapper::Set_Material(vmat);
  ...
  if (stage < 2)
    ...
    DX8Wrapper::Set_DX8_Texture(
        stage, W3DShaderManager::getShaderTexture(0)->Peek_DX8_Texture());
  ...
  DX8Wrapper::Set_DX8_Texture_Stage_State(stage, D3DTSS_TEXCOORDINDEX,
  ...
  // We need to scale so shroud texel stretches over one full terrain cell. Each
  // texel is 1/128 the size of full texture. (assuming 128x128 vid-mem
  // texture).
  W3DShroud *shroud;
  if ((shroud = TheTerrainRenderObject->getShroud()) !=
      0) {  ///@todo: All this code really only need to be done once per
            ///camera/view.  Find a way to optimize it out.
    D3DXMATRIX inv;
    float det;
    ...
    // We need to make all world coordinates be relative to the heightmap data
    // origin since that is where the shroud begins.
    ...
    if (TheTerrainRenderObject
            ->getMap()) {  // subtract origin position from all coordinates.
                           // Origin is shifted by 1 cell width/height to allow
                           // for unused border texels.
...
void ShroudTextureShader::reset(void) {
  ...
  DX8Wrapper::Set_DX8_Texture_Stage_State(m_stageOfSet, D3DTSS_TEXCOORDINDEX,
                                          m_stageOfSet);
  DX8Wrapper::Set_DX8_Texture_Stage_State(
      m_stageOfSet, D3DTSS_TEXTURETRANSFORMFLAGS, D3DTTFF_DISABLE);
}
...
/// List of different shroud shader implementations in order of preference
W3DShaderInterface *MaskShaderList[] = {&maskTextureShader, NULL};
...
Int MaskTextureShader::set(Int pass) {
  ...
  // For now we're always going to project the texture coming from the crossfade
  // effect
  ...
  DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_TEXCOORDINDEX,
                                          D3DTSS_TCI_CAMERASPACEPOSITION);
  DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_TEXTURETRANSFORMFLAGS,
                                          D3DTTFF_COUNT2);

  D3DXMATRIX inv;
  float det;

  // Get inverse view matrix so we can transform camera space points back to
  // world space
  D3DXMatrixInverse(&inv, &det, (D3DXMATRIX *)&curView);
  ...
  // Find center of projection (this should be returned from some other filter,
  // etc. but for now assume terrain location at center of screen.
  if (TheTacticalView) {
    Int xpos, ypos;
  ...
  D3DXMatrixTranslation(
      &offsetTextureCenter, 0.5f, 0.5f,
      0);  // shift coordinates so center of projection falls at uv 0.5,0.5

  Real worldTexelWidth =
      (1.0f - fadeLevel) * 25.0f;  // 9 worked well for circle but weird shape
                                   // requires more stretch to cover.
  ...
  if (worldTexelWidth != 0 && worldTexelHeight != 0) {
    ...
    *((D3DXMATRIX *)&curView) = ((inv * offset) * scale) * offsetTextureCenter;
    ...
    D3DXMatrixScaling(&scale, 0, 0,
                      1);  // scaling by 0 will set uv coordinates to 0,0
    *((D3DXMATRIX *)&curView) = ((inv * offset) * scale);
  }
...
/// regular terrain shader that should work on all multi-texture video cards
/// (slowest version)
class TerrainShader2Stage : public W3DShaderInterface {
 public:
  ...
  virtual void reset(
      void);  ///< do any custom resetting necessary to bring W3D in sync.
  void updateNoise1(D3DXMATRIX *destMatrix, D3DXMATRIX *curViewInverse,
                    Bool doUpdate = true);  ///< generate the uv coordinates for
                                            ///< Noise1 (i.e clouds)
  void updateNoise2(D3DXMATRIX *destMatrix, D3DXMATRIX *curViewInverse,
                    Bool doUpdate = true);  ///< generate the uv coordinates for
                                            ///< Noise2 (i.e lightmap)
} terrainShader2Stage;
...
class TerrainShaderPixelShader : public W3DShaderInterface {
  ...
  DWORD m_dwBaseNoise1PixelShader;  ///< handle to terrain/single noise D3D
                                    ///< pixel shader
  DWORD m_dwBaseNoise2PixelShader;  ///< handle to terrain/double noise D3D
                                    ///< pixel shader
...
W3DShaderInterface *TerrainShaderList[] = {&terrainShaderPixelShader,
                                           &terrainShader8Stage,
                                           &terrainShader2Stage, NULL};
...
Int TerrainShader2Stage::init(void) {
  ...
  // no special device validation needed - anything in our min spec should
  // handle this.
...
void TerrainShader2Stage::reset(void) {
  ...
  DX8Wrapper::Set_DX8_Texture_Stage_State(1, D3DTSS_TEXTURETRANSFORMFLAGS,
                                          D3DTTFF_DISABLE);
  DX8Wrapper::Set_DX8_Texture_Stage_State(1, D3DTSS_TEXCOORDINDEX,
...
#define STRETCH_FACTOR \
  ((float)(1 / (63.0 * MAP_XY_FACTOR / 2))) /* covers 63/2 tiles */
...
Int TerrainShader2Stage::set(Int pass) {
  ...
  if (TheGlobalData && TheGlobalData->m_bilinearTerrainTex ||
      TheGlobalData->m_trilinearTerrainTex) {
    DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_MINFILTER,
                                            D3DTEXF_LINEAR);
    DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_MAGFILTER,
                                            D3DTEXF_LINEAR);
  ...
  if (TheGlobalData && TheGlobalData->m_trilinearTerrainTex) {
    DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_MIPFILTER,
                                            D3DTEXF_LINEAR);
    DX8Wrapper::Set_DX8_Texture_Stage_State(1, D3DTSS_MIPFILTER,
                                            D3DTEXF_LINEAR);
  } else {
  ...
  switch (pass) {
    case 0:
      DX8Wrapper::_Get_D3D_Device8()->SetTexture(
          0, W3DShaderManager::getShaderTexture(0)->Peek_DX8_Texture());
      DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_ADDRESSU,
                                              D3DTADDRESS_CLAMP);
      DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_ADDRESSV,
                                              D3DTADDRESS_CLAMP);

      // Modulate the diffuse color with the texture as lighting comes from
      // diffuse.
    ...
    case 1:
      DX8Wrapper::_Get_D3D_Device8()->SetTexture(
          0, W3DShaderManager::getShaderTexture(1)->Peek_DX8_Texture());
      DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_ADDRESSU,
                                              D3DTADDRESS_CLAMP);
      DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_ADDRESSV,
                                              D3DTADDRESS_CLAMP);
    ...
    case 2:
      ...
      if (W3DShaderManager::getCurrentShader() ==
          W3DShaderManager::ST_TERRAIN_BASE_NOISE12) {
        // setup cloud pass
        DX8Wrapper::_Get_D3D_Device8()->SetTexture(
            0, W3DShaderManager::getShaderTexture(2)->Peek_DX8_Texture());

        updateNoise1(((D3DXMATRIX *)&curView),
                     &inv);  // update curView with texture matrix
        ...
        // setup noise pass
        DX8Wrapper::_Get_D3D_Device8()->SetTexture(
            1, W3DShaderManager::getShaderTexture(3)->Peek_DX8_Texture());
      ...
      else {  // only 1 noise or cloud texture
        // Now setup the texture pipeline.
        if (W3DShaderManager::getCurrentShader() ==
            W3DShaderManager::ST_TERRAIN_BASE_NOISE1) {  // setup cloud pass
          DX8Wrapper::_Get_D3D_Device8()->SetTexture(
              0, W3DShaderManager::getShaderTexture(2)->Peek_DX8_Texture());
          updateNoise1(((D3DXMATRIX *)&curView),
                       &inv);  // update curView with texture matrix
          ...
          // setup noise pass
          DX8Wrapper::_Get_D3D_Device8()->SetTexture(
              0, W3DShaderManager::getShaderTexture(3)->Peek_DX8_Texture());
          updateNoise2(((D3DXMATRIX *)&curView),
                       &inv);  // update curView with texture matrix
          DX8Wrapper::Set_DX8_Texture_Stage_State(1, D3DTSS_MINFILTER,
                                                  D3DTEXF_POINT);
          DX8Wrapper::Set_DX8_Texture_Stage_State(1, D3DTSS_MAGFILTER,
...
Int TerrainShader8Stage::init(void) {
  ...
  // this shader will also use the 2Stage shader for some of the passes so
  // initialize it too.
  if (terrainShader2Stage.init() &&
      (res = W3DShaderManager::getChipset()) >= DC_TNT && res <= DC_GEFORCE2) {
    ...
    W3DShaders[W3DShaderManager::ST_TERRAIN_BASE_NOISE12] =
        &terrainShader8Stage;
...
Int TerrainShader8Stage::set(Int pass) {
  if (pass == 0) {
    ...
    DX8Wrapper::_Get_D3D_Device8()->SetTexture(
        0, W3DShaderManager::getShaderTexture(0)->Peek_DX8_Texture());
    DX8Wrapper::_Get_D3D_Device8()->SetTexture(
        1, W3DShaderManager::getShaderTexture(1)->Peek_DX8_Texture());
    ...
    DX8Wrapper::Set_DX8_Texture_Stage_State(3, D3DTSS_COLOROP,
                                            D3DTOP_SELECTARG1);
    DX8Wrapper::Set_DX8_Texture_Stage_State(3, D3DTSS_TEXCOORDINDEX, 3);
    DX8Wrapper::Set_DX8_Texture_Stage_State(
        3, D3DTSS_COLORARG1, D3DTA_DIFFUSE | 0 | D3DTA_ALPHAREPLICATE);
    DX8Wrapper::Set_DX8_Texture_Stage_State(3, D3DTSS_COLORARG2, D3DTA_DIFFUSE);
    DX8Wrapper::Set_DX8_Texture_Stage_State(3, D3DTSS_ALPHAOP,
                                            D3DTOP_SELECTARG1);
    ...
    DX8Wrapper::Set_DX8_Texture_Stage_State(5, D3DTSS_ALPHAARG1,
                                            D3DTA_TFACTOR | D3DTA_COMPLEMENT);
    ...
    DX8Wrapper::Set_DX8_Texture_Stage_State(7, D3DTSS_COLOROP,
                                            D3DTOP_SELECTARG1);
    ...
    DX8Wrapper::Set_DX8_Texture_Stage_State(7, D3DTSS_ALPHAOP,
                                            D3DTOP_SELECTARG1);
...
Int TerrainShaderPixelShader::shutdown(void) {
  ...
  if (m_dwBaseNoise1PixelShader)
    DX8Wrapper::_Get_D3D_Device8()->DeletePixelShader(
  ...
  if (terrainShader2Stage.init() &&
    ...
    if (res >= DC_GENERIC_PIXEL_SHADER_1_1) {
      ...
      HRESULT hr = W3DShaderManager::LoadAndCreateD3DShader(
          "shaders\\terrain.pso", &Declaration[0], 0, false,
          &m_dwBasePixelShader);
      if (FAILED(hr)) return FALSE;

      // version which blends 1 noise texture.
      hr = W3DShaderManager::LoadAndCreateD3DShader("shaders\\terrainnoise.pso",
                                                    &Declaration[0], 0, false,
                                                    &m_dwBaseNoise1PixelShader);
      if (FAILED(hr)) return FALSE;
      ...
      hr = W3DShaderManager::LoadAndCreateD3DShader(
          "shaders\\terrainnoise2.pso", &Declaration[0], 0, false,
          &m_dwBaseNoise2PixelShader);
      if (FAILED(hr)) return FALSE;

      W3DShaders[W3DShaderManager::ST_TERRAIN_BASE] = &terrainShaderPixelShader;
      W3DShaders[W3DShaderManager::ST_TERRAIN_BASE_NOISE1] =
          &terrainShaderPixelShader;
      W3DShaders[W3DShaderManager::ST_TERRAIN_BASE_NOISE2] =
          &terrainShaderPixelShader;
      W3DShaders[W3DShaderManager::ST_TERRAIN_BASE_NOISE12] =
          &terrainShaderPixelShader;
      W3DShadersPassCount[W3DShaderManager::ST_TERRAIN_BASE] = 1;
...
Int TerrainShaderPixelShader::set(Int pass) {
  ...
  if (W3DShaderManager::getCurrentShader() >=
      W3DShaderManager::ST_TERRAIN_BASE_NOISE1) {
    Matrix4 curView;
    DX8Wrapper::_Get_DX8_Transform(D3DTS_VIEW, curView);
    ...
    if (W3DShaderManager::getCurrentShader() ==
        W3DShaderManager::ST_TERRAIN_BASE_NOISE12) {  // full shader
      DX8Wrapper::Set_DX8_Texture_Stage_State(3, D3DTSS_ADDRESSU,
                                              D3DTADDRESS_WRAP);
      DX8Wrapper::Set_DX8_Texture_Stage_State(3, D3DTSS_ADDRESSV,
                                              D3DTADDRESS_WRAP);
      DX8Wrapper::_Get_D3D_Device8()->SetTexture(
          2, W3DShaderManager::getShaderTexture(2)->Peek_DX8_Texture());
      DX8Wrapper::_Get_D3D_Device8()->SetTexture(
          3, W3DShaderManager::getShaderTexture(3)->Peek_DX8_Texture());
      DX8Wrapper::_Get_D3D_Device8()->SetPixelShader(m_dwBaseNoise2PixelShader);

      DX8Wrapper::Set_DX8_Texture_Stage_State(2, D3DTSS_MINFILTER,
                                              D3DTEXF_LINEAR);
      DX8Wrapper::Set_DX8_Texture_Stage_State(2, D3DTSS_MAGFILTER,
                                              D3DTEXF_LINEAR);

      DX8Wrapper::Set_DX8_Texture_Stage_State(3, D3DTSS_MINFILTER,
                                              D3DTEXF_POINT);
      DX8Wrapper::Set_DX8_Texture_Stage_State(3, D3DTSS_MAGFILTER,
                                              D3DTEXF_LINEAR);

      terrainShader2Stage.updateNoise1(
          ((D3DXMATRIX *)&curView),
          &inv);  // update curView with texture matrix
      DX8Wrapper::_Set_DX8_Transform(D3DTS_TEXTURE2, curView);

      terrainShader2Stage.updateNoise2(
          ((D3DXMATRIX *)&curView),
          &inv);  // update curView with texture matrix
      DX8Wrapper::_Set_DX8_Transform(D3DTS_TEXTURE3, curView);

      DX8Wrapper::Set_DX8_Texture_Stage_State(3, D3DTSS_TEXCOORDINDEX,
                                              D3DTSS_TCI_CAMERASPACEPOSITION);
      // Two output coordinates are used.
      DX8Wrapper::Set_DX8_Texture_Stage_State(3, D3DTSS_TEXTURETRANSFORMFLAGS,
                                              D3DTTFF_COUNT2);
    } else {  // single noise texture shader
      DX8Wrapper::_Get_D3D_Device8()->SetPixelShader(m_dwBaseNoise1PixelShader);

      if (W3DShaderManager::getCurrentShader() ==
          W3DShaderManager::ST_TERRAIN_BASE_NOISE1) {  // cloud map
        DX8Wrapper::_Get_D3D_Device8()->SetTexture(
            2, W3DShaderManager::getShaderTexture(2)->Peek_DX8_Texture());
        terrainShader2Stage.updateNoise1(
            ((D3DXMATRIX *)&curView),
        ...
        DX8Wrapper::_Get_D3D_Device8()->SetTexture(
            2, W3DShaderManager::getShaderTexture(3)->Peek_DX8_Texture());
        terrainShader2Stage.updateNoise2(
            ((D3DXMATRIX *)&curView),
            &inv);  // update curView with texture matrix
        DX8Wrapper::Set_DX8_Texture_Stage_State(2, D3DTSS_MINFILTER,
...
void TerrainShaderPixelShader::reset(void) {
  DX8Wrapper::_Get_D3D_Device8()->SetTexture(
      2, NULL);  // release reference to any texture
  DX8Wrapper::_Get_D3D_Device8()->SetTexture(
      3, NULL);  // release reference to any texture
...
/// Cloud layer rendering shader - used for objects similar to terrain which
/// only need the cloud layer.
class CloudTextureShader : public W3DShaderInterface {
  virtual Int set(
      Int stage);  ///< setup shader for the specified rendering pass.
  virtual Int init(
      void);  ///< perform any one time initialization and validation
  virtual void reset(
      void);  ///< do any custom resetting necessary to bring W3D in sync.
...
W3DShaderInterface *CloudShaderList[] = {&cloudTextureShader, NULL};
...
Int CloudTextureShader::set(Int stage) {
  ...
  terrainShader2Stage.updateNoise1(
      ((D3DXMATRIX *)&curView), &inv,
      false);  // update curView with texture matrix
  ...
  DX8Wrapper::Set_DX8_Texture_Stage_State(stage, D3DTSS_TEXTURETRANSFORMFLAGS,
                                          D3DTTFF_COUNT2);
  DX8Wrapper::_Set_DX8_Transform(
      (D3DTRANSFORMSTATETYPE)(D3DTS_TEXTURE0 + stage), curView);
  DX8Wrapper::Set_DX8_Texture_Stage_State(stage, D3DTSS_MINFILTER,
                                          D3DTEXF_LINEAR);
  ...
  DX8Wrapper::Set_DX8_Texture_Stage_State(stage, D3DTSS_COLORARG1,
                                          D3DTA_TEXTURE);
  DX8Wrapper::Set_DX8_Texture_Stage_State(stage, D3DTSS_COLORARG2,
                                          D3DTA_CURRENT);
  DX8Wrapper::Set_DX8_Texture_Stage_State(stage, D3DTSS_COLOROP,
                                          D3DTOP_MODULATE);
  DX8Wrapper::Set_DX8_Texture_Stage_State(stage, D3DTSS_ALPHAARG1,
                                          D3DTA_TEXTURE);
  DX8Wrapper::Set_DX8_Texture_Stage_State(stage, D3DTSS_ALPHAARG2,
                                          D3DTA_CURRENT);
  DX8Wrapper::Set_DX8_Texture_Stage_State(stage, D3DTSS_ALPHAOP,
                                          D3DTOP_MODULATE);

  DX8Wrapper::_Get_D3D_Device8()->SetTexture(
      stage, W3DShaderManager::getShaderTexture(stage)->Peek_DX8_Texture());
...
void CloudTextureShader::reset(void) {
  ...
  DX8Wrapper::Set_DX8_Texture_Stage_State(
      m_stageOfSet, D3DTSS_TEXTURETRANSFORMFLAGS, D3DTTFF_DISABLE);
  DX8Wrapper::Set_DX8_Texture_Stage_State(m_stageOfSet, D3DTSS_TEXCOORDINDEX,
                                          D3DTSS_TCI_PASSTHRU | m_stageOfSet);

  DX8Wrapper::Set_DX8_Texture_Stage_State(m_stageOfSet, D3DTSS_COLOROP,
                                          D3DTOP_DISABLE);
  DX8Wrapper::Set_DX8_Texture_Stage_State(m_stageOfSet, D3DTSS_ALPHAOP,
                                          D3DTOP_DISABLE);
}
...
class RoadShaderPixelShader : public W3DShaderInterface {
  DWORD m_dwBaseNoise2PixelShader;  ///< handle to road/double noise D3D pixel
                                    ///< shader
...
class RoadShader2Stage : public W3DShaderInterface {
  friend class RoadShaderPixelShader;  // pixel shader version uses some of the
                                       // same features.
...
/// List of different terrain shader implementations in order of preference
W3DShaderInterface *RoadShaderList[] = {&roadShaderPixelShader,
                                        &roadShader2Stage, NULL};
...
Int RoadShaderPixelShader::init(void) {
  ...
  if (roadShader2Stage.init() &&
    ...
    if (res >= DC_GENERIC_PIXEL_SHADER_1_1) {
      ...
      // version which blends 2 noise textures.
      HRESULT hr = W3DShaderManager::LoadAndCreateD3DShader(
          "shaders\\roadnoise2.pso", &Declaration[0], 0, false,
          &m_dwBaseNoise2PixelShader);
      if (FAILED(hr)) return FALSE;

      // Only set this shader for use in dual noise mode.  The 2Stage shader
      // will take care of all the other modes.
      W3DShaders[W3DShaderManager::ST_ROAD_BASE_NOISE12] =
          &roadShaderPixelShader;
...
Int RoadShaderPixelShader::set(Int pass) {
  // after we're done with this shader we're going to reset to NULL texture, so
  // set W3D same way.
  DX8Wrapper::Set_Texture(0, NULL);
  DX8Wrapper::Set_Texture(1, NULL);
  // force WW3D2 system to set it's states so it won't later overwrite our
  // custom settings.
  DX8Wrapper::Apply_Render_State_Changes();
  // Directly set texture without W3D knowing... but we'll restore to NULL
  // later.
  ...
  DX8Wrapper::Set_DX8_Render_State(D3DRS_ALPHABLENDENABLE,
                                   true);  // blend roads into terrain
  DX8Wrapper::Set_DX8_Render_State(D3DRS_SRCBLEND, D3DBLEND_SRCALPHA);
  DX8Wrapper::Set_DX8_Render_State(D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA);
  ...
  terrainShader2Stage.updateNoise1(((D3DXMATRIX *)&curView), &inv,
                                   false);  // get texture projection matrix
  DX8Wrapper::_Set_DX8_Transform(D3DTS_TEXTURE1, curView);
...
Int RoadShader2Stage::set(Int pass) {
  ...
  if (pass == 0) {
    ...
    if (W3DShaderManager::getCurrentShader() >=
        W3DShaderManager::ST_ROAD_BASE_NOISE1) {  // second texture unit will
                                                  // contain a noise pass
      Matrix4 curView;
      DX8Wrapper::_Get_DX8_Transform(D3DTS_VIEW, curView);
      ...
      if (W3DShaderManager::getCurrentShader() ==
          W3DShaderManager::ST_ROAD_BASE_NOISE12) {  // full shader, apply noise
                                                     // 1 in pass 0.
        DX8Wrapper::_Get_D3D_Device8()->SetTexture(
            1, W3DShaderManager::getShaderTexture(1)->Peek_DX8_Texture());
        DX8Wrapper::Set_DX8_Texture_Stage_State(1, D3DTSS_MINFILTER,
        ...
        if (W3DShaderManager::getCurrentShader() ==
            W3DShaderManager::ST_ROAD_BASE_NOISE1) {  // cloud map
          DX8Wrapper::_Get_D3D_Device8()->SetTexture(
              1, W3DShaderManager::getShaderTexture(1)->Peek_DX8_Texture());
          terrainShader2Stage.updateNoise1(
              ((D3DXMATRIX *)&curView), &inv,
          ...
          DX8Wrapper::_Get_D3D_Device8()->SetTexture(
              1, W3DShaderManager::getShaderTexture(2)->Peek_DX8_Texture());
          terrainShader2Stage.updateNoise2(
              ((D3DXMATRIX *)&curView), &inv,
              false);  // update curView with texture matrix
          DX8Wrapper::Set_DX8_Texture_Stage_State(1, D3DTSS_MINFILTER,
  ...
  else {  // pass 1, apply additional noise pass
    Matrix4 curView;
    DX8Wrapper::_Get_DX8_Transform(D3DTS_VIEW, curView);
    ...
    // Copy alpha channel into stage 1 but mask out color channel by replacing
    // with white.
    DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_COLORARG1, D3DTA_TEXTURE);
    // Force color channel to white by copying the alpha into RGB
    DX8Wrapper::Set_DX8_Texture_Stage_State(
        0, D3DTSS_COLORARG2, D3DTA_DIFFUSE | D3DTA_ALPHAREPLICATE);
    DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_COLOROP,
                                            D3DTOP_SELECTARG2);
    DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE);
    DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_ALPHAARG2, D3DTA_DIFFUSE);
    ...
    // Modulate into existing roads with clouds applied. - only apply where
    // roads are transparent by using road texture as a mask.
...
/** List of all custom shader lists - each list in this list contains variations
   of the same shader to allow it to work on different hardware configurations.
*/
W3DShaderInterface **MasterShaderList[] = {TerrainShaderList, ShroudShaderList,
                                           RoadShaderList,    MaskShaderList,
                                           CloudShaderList,   NULL};

/** List of all custom filter lists - each list in this list contains variations
   of the same filter to allow it to work on different hardware configurations.
*/
W3DFilterInterface **MasterFilterList[] = {ScreenBWFilterList,
                                           ScreenMotionBlurFilterList,
                                           ScreenCrossFadeFilterList, NULL};
...
void W3DShaderManager::init(void) {
  ...
  Int res = 0;
  if ((res = W3DShaderManager::getChipset()) != 0) {
    // Some of our effects require an offscreen render target, so try creating
    // it here.
    HRESULT hr =
        DX8Wrapper::_Get_D3D_Device8()->GetRenderTarget(&m_oldRenderSurface);
    ...
    hr = DX8Wrapper::_Get_D3D_Device8()->CreateTexture(
        ...
        D3DPOOL_DEFAULT, &m_renderTexture);
    ...
    if (hr != S_OK) {
      ...
      if (hr != S_OK) {
        ...
        hr = DX8Wrapper::_Get_D3D_Device8()->GetDepthStencilSurface(
            &m_oldDepthSurface);
        if (hr != S_OK) {
  ...
  for (i = 0; MasterShaderList[i] != NULL; i++) {
    ...
    for (j = 0; shaders[j] != NULL; j++) {
      if (shaders[j]->init()) break;  // found a working shader
    }
  }
  ...
  for (i = 0; MasterFilterList[i] != NULL; i++) {
    ...
    for (j = 0; filters[j] != NULL; j++) {
      if (filters[j]->init()) break;  // found a working shader
    }
  }
...
void W3DShaderManager::shutdown(void) {
  ...
  m_currentShader = ST_INVALID;
  m_currentFilter = FT_NULL_FILTER;
  // release any assets associated with a shader (vertex/pixel shaders,
  // textures, etc.)
...
// =======================================================
/** Return number of renderig passes required in perform the desired shader on
   current hardware.  App will need to re-render the polygons this many times to
   complete the effect.
 */
//=============================================================================
...
// =======================================================
/** Must call this method before each rendering pass in order to perform proper
   D3D setup for each shader.
 */
//=============================================================================
Int W3DShaderManager::setShader(ShaderTypes shader, Int pass) {
  ...
  if (W3DShaders[shader]) return W3DShaders[shader]->set(pass);
  return FALSE;
}

// W3DShaderManager::resetShader
// =======================================================
/** Must call this method after all polygons and rendering passes have been
   submitted. This method allows D3D to reset itself to a default state that
   doesn't conflict with the WW3D2 Shader system.
 */
//=============================================================================
void W3DShaderManager::resetShader(ShaderTypes shader) {
  if (m_currentShader == ST_INVALID) return;  // last shader is already reset.
  if (W3DShaders[shader]) W3DShaders[shader]->reset();
  m_currentShader = ST_INVALID;
}
...
Bool W3DShaderManager::filterPreRender(FilterTypes filter, Bool &skipRender,
  ...
  if (W3DFilters[filter]) {
    ...
    if (result) m_currentFilter = filter;
    return result;
  }
  return FALSE;
}
...
Bool W3DShaderManager::filterSetup(FilterTypes filter, enum FilterModes mode) {
  if (W3DFilters[filter]) return W3DFilters[filter]->setup(mode);
  return FALSE;
}
...
void W3DShaderManager::startRenderToTexture(void) {
  DEBUG_ASSERTCRASH(!m_renderingToTexture,
                    ("Already rendering to texture - cannot nest calls."));

  if (m_renderingToTexture || m_newRenderSurface == NULL ||
      m_oldDepthSurface == NULL)
    return;
  HRESULT hr = DX8Wrapper::_Get_D3D_Device8()->SetRenderTarget(
      m_newRenderSurface, m_oldDepthSurface);
  DEBUG_ASSERTCRASH(hr == S_OK, ("Set target failed unexpectedly."));
  if (hr != S_OK) return;
  m_renderingToTexture = true;
  if (TheGlobalData->m_showSoftWaterEdge) {  // Soft water edges use frame
                                             // buffer destination alpha so we
                                             // must clear it to a known value.
    if (m_currentFilter == FT_VIEW_MOTION_BLUR_FILTER ||
        m_currentFilter ==
            FT_VIEW_CROSSFADE) {  // these filters rely on the previous frame
                                  // being visible so we must be careful about
                                  // clearing
      // frame buffer.  Only clear the alpha channel
      DX8Wrapper::Set_DX8_Render_State(
          D3DRS_COLORWRITEENABLE,
          D3DCOLORWRITEENABLE_ALPHA);  // only clear alpha
      ...
      VertexMaterialClass *vmat =
          VertexMaterialClass::Get_Preset(VertexMaterialClass::PRELIT_DIFFUSE);
      DX8Wrapper::Set_Material(vmat);
      REF_PTR_RELEASE(vmat);  // no need to keep a reference since it's a
                              // preset.

      drawViewport(
          0x00ffffff |
          (((Int)(TheWaterTransparency->m_minWaterOpacity * 255.0f)) << 24));
      DX8Wrapper::Set_DX8_Render_State(
          D3DRS_COLORWRITEENABLE,
          D3DCOLORWRITEENABLE_RED | D3DCOLORWRITEENABLE_GREEN |
              D3DCOLORWRITEENABLE_BLUE);  // disable writes to alpha
...
IDirect3DTexture8 *W3DShaderManager::endRenderToTexture(void) {
  ...
  HRESULT hr = DX8Wrapper::_Get_D3D_Device8()->SetRenderTarget(
      m_oldRenderSurface, m_oldDepthSurface);  // restore original render target
  DEBUG_ASSERTCRASH(hr == S_OK, ("Set target failed unexpectedly."));
  if (hr == S_OK) {
    // assume render target texure will be in stage 0.  Most hardware has
    // "conditional" support for non-power-of-2 textures so we must force some
    // required states:
    DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_ADDRESSU,
                                            D3DTADDRESS_CLAMP);
    DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_ADDRESSV,
                                            D3DTADDRESS_CLAMP);
    DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_ADDRESSW,
                                            D3DTADDRESS_CLAMP);
    DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_MAGFILTER,
                                            D3DTEXF_LINEAR);
    DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_MINFILTER,
...
/**Returns texture containing the image that was last rendered using any of the
effects requiring render target textures.  Used mostly for cross-fading effects
that need an unmodified version of the view before the effect was applied. NOTE:
This texture does not survive device reset.. so quit effect on reset!*/
...
// =======================================================
/** Returns the chipset used by the currently active rendering device.  Can be
   useful for coding around specific driver bugs.
 */
//=============================================================================
ChipsetType W3DShaderManager::getChipset(void) {
  ...
  if (d3d8Interface && DX8Wrapper::_Get_D3D_Device8()) {
    ...
    /*	HRESULT res = */ d3d8Interface->GetAdapterIdentifier(
        0, D3DENUM_NO_WHQL_LEVEL, &did);

    if (did.VendorId == DC_NVIDIA_VENDOR_ID) {
      if (did.DeviceId == 0x20) return DC_TNT;

      if (did.DeviceId >= 0x28 && did.DeviceId < 0x100) return DC_TNT2;
      ...
      if (did.DeviceId >= 0x200 && did.DeviceId < 0x250) return DC_GEFORCE3;

      if (did.DeviceId >= 0x250) return DC_GEFORCE4;
    } else if (did.VendorId == DC_3DFX_VENDOR_ID) {
      if (did.DeviceId == 0x0002) return DC_VOODOO2;
      if (did.DeviceId == 0x0005) return DC_VOODOO3;
      if (did.DeviceId == 0x0008)  ///@todo: Just guessing on this one - find
                                   ///actual Voodoo4 deviceID.
        return DC_VOODOO4;
      if (did.DeviceId == 0x0009) return DC_VOODOO5;
    } else if (did.VendorId == DC_ATI_VENDOR_ID) {
      if (did.DeviceId == 0x5144) return DC_RADEON;
      if (did.DeviceId == 0x514C) return DC_RADEON_8500;
      if (did.DeviceId == 0x4e44) return DC_RADEON_9700;
    }

    // None of the vendor specific ID's matched so use generic means to classify
    // the card
    ...
    sprintf(buf, "%d.%d", DX8Caps::Get_Pixel_Shader_Majon_Version(),
            DX8Caps::Get_Pixel_Shader_Minor_Version());
    sscanf(buf, "%f", &pixelShaderVersion);

    if (maxTextures >= 4) {
      if (pixelShaderVersion >= 1.1f) chip = DC_GENERIC_PIXEL_SHADER_1_1;
      if (pixelShaderVersion >= 1.4f) chip = DC_GENERIC_PIXEL_SHADER_1_4;
...
HRESULT W3DShaderManager::LoadAndCreateD3DShader(char *strFilePath,
  ...
  try {
    ...
    const DWORD *pShader =
        (DWORD *)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, dwFileSize);
    if (!pShader) {
    ...
    {
      hr = DX8Wrapper::_Get_D3D_Device8()->CreateVertexShader(
          pDeclaration, pShader, pHandle, Usage);
...
// For the MP test, we're enforcing high min-spec requirements that need to be
// verified.
...
Bool testMinimumRequirements(ChipsetType *videoChipType, CpuType *cpuType,
                             Int *cpuFreq, Int *numRAM, Real *intBenchIndex,
                             Real *floatBenchIndex, Real *memBenchIndex) {
...
Bool W3DShaderManager::testMinimumRequirements(ChipsetType *videoChipType,
                                               ...
                                               Int *numRAM, Real *intBenchIndex,
                                               Real *floatBenchIndex,
                                               Real *memBenchIndex) {
  if (videoChipType) *videoChipType = getChipset();

  if (cpuType) {
    *cpuType = XX;  // unknown

    // Check if it's an Athlon
    if (CPUDetectClass::Get_Processor_Manufacturer() ==
            CPUDetectClass::MANUFACTURER_AMD &&
        CPUDetectClass::Get_AMD_Processor() >=
            CPUDetectClass::AMD_PROCESSOR_ATHLON_025)
      *cpuType = K7;

    // Check if it's a P3
    if (CPUDetectClass::Get_Processor_Manufacturer() ==
            CPUDetectClass::MANUFACTURER_INTEL &&
        CPUDetectClass::Get_Intel_Processor() >=
            CPUDetectClass::INTEL_PROCESSOR_PENTIUM_III_MODEL_7)
      *cpuType = P3;
    // Check if it's a P4
    if (CPUDetectClass::Get_Processor_Manufacturer() ==
            CPUDetectClass::MANUFACTURER_INTEL &&
        CPUDetectClass::Get_Intel_Processor() >=
            CPUDetectClass::INTEL_PROCESSOR_PENTIUM4)
      *cpuType = P4;
  }

  if (cpuFreq) *cpuFreq = CPUDetectClass::Get_Processor_Speed();

  if (numRAM) *numRAM = CPUDetectClass::Get_Total_Physical_Memory();
...
/**Try to guess how well the video card will handle the game assuming very fast
 * CPU*/
StaticGameLODLevel W3DShaderManager::getGPUPerformanceIndex(void) {
  ChipsetType chipType;
  StaticGameLODLevel detailSetting =
      STATIC_GAME_LOD_LOW;  // assume lowest settings for now.

  if ((chipType = getChipset()) !=
      DC_UNKNOWN) {  // a known video card so we can make some assumptions
    if (chipType >= DC_GEFORCE2)
      detailSetting =
          STATIC_GAME_LOD_LOW;  // these cards need multiple terrain passes.
    if (chipType >= DC_GENERIC_PIXEL_SHADER_1_1)  // these cards can do terrain
                                                  // in single pass.
...
/**We need a hardware independent method to compare different CPU's.  For lack
of anything better, we'll use time to calculate PIE using a slow random number
algorithm.*/

/**Used to test function call overhead*/
void add(float *sum, float *addend) { *sum = *sum + *addend; }
...
Real W3DShaderManager::GetCPUBenchTime(void) {
  ...
  ymult = 1307.0;
  ymod = 5471.0;
  itot = 560000;  // total iterations. This value ends up running at ~30 fps on
                  // our P4-2.2Ghz.
