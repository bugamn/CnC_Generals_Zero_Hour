...
//
// Desc:      TextureClass overrides to perform custom texturing for the
// terrain.
//
//-----------------------------------------------------------------------------
...
TerrainTextureClass::TerrainTextureClass(int height, MipCountType mipLevelCount)
    : TextureClass(TEXTURE_WIDTH, height, WW3D_FORMAT_A1R5G5B5, mipLevelCount) {
}
...
/** Sets the tile bitmap data into the texture.  The tiles are placed with 4
        pixel borders around them, so that when the tiles are scaled and
   bilinearly interpolated, you don't get seams between the tiles.  */
//=============================================================================
...
#ifdef _DEBUG
  // DEBUG_ASSERTCRASH(tilesPerRow*numRows >= htMap->m_numBitmapTiles, ("Too
  // many tiles."));
  DEBUG_ASSERTCRASH((Int)surface_desc.Width >= tilePixelExtent * tilesPerRow,
                    ("Bitmap too small."));
#endif
  if (surface_desc.Format == D3DFMT_A1R5G5B5) {
    ...
    for (tileNdx = 0; tileNdx < htMap->m_numBitmapTiles; tileNdx++) {
      ...
      for (j = 0; j < tilePixelExtent; j++) {
        ...
        pBGR += (tilePixelExtent - 1 - j) * TILE_BYTES_PER_PIXEL *
                tilePixelExtent;  // invert to match.
    ...
    for (texClass = 0; texClass < htMap->m_numTextureClasses; texClass++) {
      ...
      for (j = 0; j < width; j++) {
        ...
        pBGRX += column * pixelBytes;
        // copy before
        memcpy(pBGRX - (4) * pixelBytes, pBGRX + (width - 4) * pixelBytes,
               4 * pixelBytes);
        // copy after
      ...
      for (j = 0; j < 4; j++) {
        ...
        memcpy(target, target + width * surface_desc.Width * pixelBytes,
               (width + 8) * pixelBytes);
        // copy after.
        row = origin.y + j;
        ...
        memcpy(target + width * surface_desc.Width * pixelBytes, target,
               (width + 8) * pixelBytes);
      }
    }
  }
...
#if ADD_EXTRA_TILES  // Fills in an extra 2 columns and 1 row of tiles if there
                     // is room.
...
//=============================================================================
/** Sets the tile bitmap data into the texture.  Handles the special case for
   voodoos and the like where the max texture size is 256. The tiles are placed
   with 1 pixel borders around them, so that when the tiles are scaled and
   bilinearly interpolated, you don't get seams between the tiles.  */
//=============================================================================
int TerrainTextureClass::update256(WorldHeightMap *htMap) {
  ...
  Int tilePixelExtent =
      TILE_PIXEL_EXTENT / 4;  // We are going from 1024 to 256, so need to
                              // divide all the constants by 4.
  ...
  if (surface_desc.Format == D3DFMT_A1R5G5B5) {
    ...
    for (cellY = 0; cellY < numRows; cellY++) {
      for (cellX = 0; cellX < tilesPerRow; cellX++) {
        ...
        if (!(cellY & 1)) tileNdx += 2;
                     ...
                     // is room.
        if (!htMap->getSourceTile(tileNdx) &&
            htMap->getSourceTile(tileNdx - 4)) {
          tileNdx -= 4;
        }
        if (!htMap->getSourceTile(tileNdx) &&
            htMap->getSourceTile(tileNdx - 8)) {
          tileNdx -= 8;
        }
        if (!htMap->getSourceTile(tileNdx) &&
            htMap->getSourceTile(tileNdx - 2 * tilesPerRow)) {
          tileNdx -= 2 * tilesPerRow;
        }
#endif
        if (htMap->getSourceTile(tileNdx)) {
          ...
          for (j = 0; j < tilePixelExtent; j++) {
            UnsignedByte *pBGR =
                htMap->getSourceTile(tileNdx)->getRGBDataForWidth(
                    tilePixelExtent);
            pBGR += (tilePixelExtent - 1 - j) * TILE_BYTES_PER_PIXEL *
                    tilePixelExtent;  // invert to match.
            Int row = cellY * tilePixelExtent + j;
      ...
      for (cellX = 0; cellX < tilesPerRow; cellX++) {
        ...
        for (j = 0; j < tilePixelExtent; j++) {
          ...
          if (cellX & 1) {
            ...
            if (cellX == tilesPerRow - 1) {
              // last cell, so fill to the end of the texture.
              Int usedWidth = tilesPerRow * tilePixelExtent +
                              tileOffset * (tilesPerRow / 2);
              usedWidth *= pixelBytes;
              bytesToCopy += surface_desc.Width * pixelBytes - usedWidth;
            }
            memcpy(pBGRX + (2 * tilePixelExtent * pixelBytes), pBGRX,
                   bytesToCopy);
            // Copy after.
            pBGRX -= tilePixelExtent * pixelBytes;
            pBGRX += (TILE_OFFSET / 2) * pixelBytes;
            ...
            // copy before
            memcpy(pBGRX, pBGRX + (2 * tilePixelExtent) * pixelBytes,
                   pixelBytes);
          }
        }
      }
    ...
    for (cellY = 0; cellY < numRows; cellY++) {
      ...
      if ((cellY & 1) == 0) {
        UnsignedByte *pBGRX =
            ((UnsignedByte *)locked_rect.pBits) + (row)*rowBytes;
        row += 2 * tilePixelExtent;
        UnsignedByte *pBase =
            ((UnsignedByte *)locked_rect.pBits) + (row)*rowBytes;
...
void AlphaTerrainTextureClass::Apply(unsigned int stage) {
  ...
  // Since we are using multiple distinct tiles, the textures doesn't wrap, so
  // clamp it.
  DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_ADDRESSU,
                                          D3DTADDRESS_CLAMP);
  DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_ADDRESSV,
  ...
  if (stage == 0) {
    ...
    if (TheGlobalData && !TheGlobalData->m_multiPassTerrain) {
      ...
      // This method is a backdoor specific to Nvidia based cards.  It will fail
      // on other hardware.  Allows single pass blend of 2 textures and post
      // modulate diffuse.
      ...
      DX8Wrapper::Set_DX8_Texture_Stage_State(2, D3DTSS_COLOROP,
                                              D3DTOP_MODULATE);
      DX8Wrapper::Set_DX8_Texture_Stage_State(2, D3DTSS_TEXCOORDINDEX, 2);
      DX8Wrapper::Set_DX8_Texture_Stage_State(2, D3DTSS_COLORARG1,
                                              D3DTA_TEXTURE);
      DX8Wrapper::Set_DX8_Texture_Stage_State(2, D3DTSS_COLORARG2,
                                              D3DTA_TEXTURE);
      DX8Wrapper::Set_DX8_Texture_Stage_State(2, D3DTSS_ALPHAOP,
                                              D3DTOP_MODULATE);
      DX8Wrapper::Set_DX8_Texture_Stage_State(2, D3DTSS_ALPHAARG1,
                                              D3DTA_TFACTOR);
      DX8Wrapper::Set_DX8_Texture_Stage_State(2, D3DTSS_ALPHAARG2,
                                              D3DTA_TFACTOR);
      ...
      DX8Wrapper::Set_DX8_Texture_Stage_State(3, D3DTSS_COLORARG2,
                                              D3DTA_DIFFUSE);
      DX8Wrapper::Set_DX8_Texture_Stage_State(3, D3DTSS_ALPHAOP,
                                              D3DTOP_SELECTARG1);
      DX8Wrapper::Set_DX8_Texture_Stage_State(3, D3DTSS_ALPHAARG1,
                                              D3DTA_TFACTOR);
      DX8Wrapper::Set_DX8_Texture_Stage_State(3, D3DTSS_ALPHAARG2,
                                              D3DTA_TFACTOR);

      DX8Wrapper::Set_DX8_Texture(4, NULL);
      DX8Wrapper::Set_DX8_Texture_Stage_State(4, D3DTSS_COLOROP,
                                              D3DTOP_MODULATE);
      DX8Wrapper::Set_DX8_Texture_Stage_State(4, D3DTSS_TEXCOORDINDEX, 4);
      DX8Wrapper::Set_DX8_Texture_Stage_State(4, D3DTSS_COLORARG1,
                                              D3DTA_CURRENT);
      DX8Wrapper::Set_DX8_Texture_Stage_State(4, D3DTSS_COLORARG2,
                                              D3DTA_DIFFUSE);
      DX8Wrapper::Set_DX8_Texture_Stage_State(4, D3DTSS_ALPHAOP,
                                              D3DTOP_MODULATE);
      DX8Wrapper::Set_DX8_Texture_Stage_State(4, D3DTSS_ALPHAARG1,
                                              D3DTA_CURRENT);
      DX8Wrapper::Set_DX8_Texture_Stage_State(4, D3DTSS_ALPHAARG2,
                                              D3DTA_DIFFUSE);
      ...
      DX8Wrapper::Set_DX8_Texture_Stage_State(5, D3DTSS_COLORARG1,
                                              D3DTA_DIFFUSE);
      DX8Wrapper::Set_DX8_Texture_Stage_State(5, D3DTSS_COLORARG2,
                                              D3DTA_DIFFUSE);
      ...
      DX8Wrapper::Set_DX8_Texture_Stage_State(5, D3DTSS_ALPHAARG1,
                                              D3DTA_TFACTOR | D3DTA_COMPLEMENT);
      DX8Wrapper::Set_DX8_Texture_Stage_State(5, D3DTSS_ALPHAARG2,
                                              D3DTA_TFACTOR);

      DX8Wrapper::Set_DX8_Texture(6, NULL);
      DX8Wrapper::Set_DX8_Texture_Stage_State(6, D3DTSS_COLOROP,
                                              D3DTOP_MODULATE);
      DX8Wrapper::Set_DX8_Texture_Stage_State(6, D3DTSS_TEXCOORDINDEX, 6);
      DX8Wrapper::Set_DX8_Texture_Stage_State(6, D3DTSS_COLORARG1,
                                              D3DTA_TFACTOR);
      DX8Wrapper::Set_DX8_Texture_Stage_State(6, D3DTSS_COLORARG2,
                                              D3DTA_TFACTOR);
      DX8Wrapper::Set_DX8_Texture_Stage_State(6, D3DTSS_ALPHAOP,
                                              D3DTOP_MODULATE);
      DX8Wrapper::Set_DX8_Texture_Stage_State(6, D3DTSS_ALPHAARG1,
                                              D3DTA_TFACTOR);
      DX8Wrapper::Set_DX8_Texture_Stage_State(6, D3DTSS_ALPHAARG2,
                                              D3DTA_TFACTOR);
      ...
      DX8Wrapper::Set_DX8_Texture_Stage_State(7, D3DTSS_COLORARG1,
                                              D3DTA_TFACTOR);
      DX8Wrapper::Set_DX8_Texture_Stage_State(7, D3DTSS_COLORARG2,
                                              D3DTA_TFACTOR);
      DX8Wrapper::Set_DX8_Texture_Stage_State(7, D3DTSS_ALPHAOP,
                                              D3DTOP_SELECTARG1);
      DX8Wrapper::Set_DX8_Texture_Stage_State(7, D3DTSS_ALPHAARG1,
                                              D3DTA_TFACTOR);
      DX8Wrapper::Set_DX8_Texture_Stage_State(7, D3DTSS_ALPHAARG2,
                                              D3DTA_TFACTOR);
    } else {
      DX8Wrapper::Set_DX8_Texture_Stage_State(0, D3DTSS_COLORARG1,
                                              D3DTA_TEXTURE);
...
//=============================================================================
LightMapTerrainTextureClass::LightMapTerrainTextureClass(
    AsciiString name, MipCountType mipLevelCount)
 ...
 *
 */
AlphaEdgeTextureClass::AlphaEdgeTextureClass(int height,
                                             MipCountType mipLevelCount)
    :  //	TextureClass("EdgingTemplate.tga","EdgingTemplate.tga",
       //mipLevelCount )
      TextureClass(TEXTURE_WIDTH, height, WW3D_FORMAT_A8R8G8B8, mipLevelCount) {

}

int AlphaEdgeTextureClass::update256(WorldHeightMap *htMap) { return 1; }
...
int AlphaEdgeTextureClass::update(WorldHeightMap *htMap) {
  ...
  if (surface_desc.Format == D3DFMT_A8R8G8B8) {
    ...
    for (cellX = 0; (UnsignedInt)cellX < surface_desc.Width; cellX++) {
      for (cellY = 0; cellY < surface_desc.Height; cellY++) {
        UnsignedByte *pBGR = ((UnsignedByte *)locked_rect.pBits) +
    ...
    for (tileNdx = 0; tileNdx < htMap->m_numEdgeTiles; tileNdx++) {
      ...
      for (j = 0; j < tilePixelExtent; j++) {
        Int row = position.y + j;
        UnsignedByte *pBGR =
            htMap->getEdgeTile(tileNdx)->getRGBDataForWidth(tilePixelExtent);
        pBGR += (tilePixelExtent - 1 - j) * TILE_BYTES_PER_PIXEL *
                tilePixelExtent;  // invert to match.
        UnsignedByte *pBGRX = ((UnsignedByte *)locked_rect.pBits) +
                              (row)*surface_desc.Width * pixelBytes;
...
//=============================================================================
//@todo - Allow adjustment of the cloud slide rate, and lose the hard coded
//"cloudmap.tga"
CloudMapTerrainTextureClass::CloudMapTerrainTextureClass(
    MipCountType mipLevelCount)
...
void CloudMapTerrainTextureClass::restore(void) {
  ...
  if (TheGlobalData && !TheGlobalData->m_multiPassTerrain) {
    ...
    Int i;
    for (i = 0; i < 8; i++) {
      DX8Wrapper::Set_DX8_Texture_Stage_State(i, D3DTSS_COLOROP,
                                              D3DTOP_DISABLE);
      DX8Wrapper::Set_DX8_Texture_Stage_State(i, D3DTSS_TEXCOORDINDEX, i);
      DX8Wrapper::Set_DX8_Texture_Stage_State(i, D3DTSS_COLORARG1,
                                              D3DTA_TEXTURE);
      DX8Wrapper::Set_DX8_Texture_Stage_State(i, D3DTSS_COLORARG2,
                                              D3DTA_DIFFUSE);
      DX8Wrapper::Set_DX8_Texture_Stage_State(i, D3DTSS_ALPHAOP,
                                              D3DTOP_DISABLE);
      DX8Wrapper::Set_DX8_Texture_Stage_State(i, D3DTSS_ALPHAARG1,
                                              D3DTA_TEXTURE);
      DX8Wrapper::Set_DX8_Texture_Stage_State(i, D3DTSS_ALPHAARG2,
                                              D3DTA_DIFFUSE);
