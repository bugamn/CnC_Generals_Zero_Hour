...
/** Frees objects.  */
//=============================================================================
W3DBridge::~W3DBridge(void) { clearBridge(); }
...
void W3DBridge::renderBridge(Bool wireframe) {
  if (m_visible) {
    ...
    DX8Wrapper::Draw_Triangles(m_firstIndex, m_numPolygons, m_firstVertex,
                               m_numVertex);
  }
}
...
//=============================================================================
void W3DBridge::init(Vector3 fromLoc, Vector3 toLoc,
...
Bool W3DBridge::load(enum BodyDamageType curDamageState) {
  ...
  for (i = 0; i < numVertex; i++) {
    ...
    if (vert.Y > m_maxY)
      m_maxY = vert.Y;  // Note - we assume all sections are the same width, so
                        // we only do maxY for first section.
  }
...
Int W3DBridge::getModelVertices(VertexFormatXYZNDUV1 *destination_vb,
                                Int curVertex, Real xOffset, Vector3 &vec,
                                Vector3 &vecNormal, Vector3 &vecZ,
                                Vector3 &offset, const Matrix3D &mtx,
                                MeshClass *pMesh,
...
//=============================================================================
Int W3DBridge::getModelVerticesFixed(
    VertexFormatXYZNDUV1 *destination_vb, Int curVertex, const Matrix3D &mtx,
  ...
  return (getModelVertices(destination_vb, curVertex, xOffset, vec, vecNormal,
                           vecZ, m_start, mtx, pMesh, pLightsIterator));
}
...
void W3DBridge::getIndicesNVertices(UnsignedShort *destination_ib,
  ...
  if (m_sectionMesh == NULL) {
    numV = getModelVerticesFixed(destination_vb, *curVertexP, m_leftMtx,
                                 m_leftMesh, pLightsIterator);
  ...
  vec /= bridgeLength;
  numV =
      getModelVertices(destination_vb, *curVertexP, xOffset, vec, vecNormal,
                       vecZ, m_start, m_leftMtx, m_leftMesh, pLightsIterator);
  numI = getModelIndices(destination_ib, *curIndexP, *curVertexP, m_leftMesh);
  *curIndexP += numI;
  *curVertexP += numV;
  m_numVertex += numV;
  ...
  for (i = 0; i < numSpans; i++) {
    numV = getModelVertices(
        destination_vb, *curVertexP, xOffset + i * spanLength, vec, vecNormal,
        vecZ, m_start, m_sectionMtx, m_sectionMesh, pLightsIterator);
    numI =
        getModelIndices(destination_ib, *curIndexP, *curVertexP, m_sectionMesh);
    *curIndexP += numI;
    *curVertexP += numV;
    m_numVertex += numV;
    m_numPolygons += numI / 3;
  ...
  // Draw the right end.
  numV = getModelVertices(
      destination_vb, *curVertexP, xOffset + (numSpans - 1) * spanLength, vec,
      vecNormal, vecZ, m_start, m_rightMtx, m_rightMesh, pLightsIterator);
...
//=============================================================================
Int W3DBridge::getModelIndices(UnsignedShort *destination_ib, Int curIndex,
                               Int vertexOffset, MeshClass *pMesh) {
  if (pMesh == NULL) return (0);
...
//=============================================================================
/** Culls the bridges, marking the visible flag.  If a bridge changes
visibility, it sets m_anythingChanged */
//=============================================================================
...
//=============================================================================
W3DBridgeBuffer::~W3DBridgeBuffer(void) { freeBridgeBuffers(); }
...
void W3DBridgeBuffer::allocateBridgeBuffers(void) {
  m_vertexBridge =
      NEW_REF(DX8VertexBufferClass, (DX8_FVF_XYZNDUV1, MAX_BRIDGE_VERTEX + 4,
                                     DX8VertexBufferClass::USAGE_DYNAMIC));
  m_indexBridge =
      NEW_REF(DX8IndexBufferClass,
              (MAX_BRIDGE_INDEX + 4, DX8IndexBufferClass::USAGE_DYNAMIC));
  m_vertexMaterial =
...
void W3DBridgeBuffer::loadBridges(W3DTerrainLogic *pTerrainLogic,
  ...
  for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
    ...
    if (pMapObj->getFlag(FLAG_BRIDGE_POINT1)) {
      ...
      from.Z =
          TheTerrainRenderObject->getHeightMapHeight(from.X, from.Y, NULL) +
          BRIDGE_FLOAT_AMT;
      to.Set(pMapObj2->getLocation()->x, pMapObj2->getLocation()->y, 0);
      to.Z = TheTerrainRenderObject->getHeightMapHeight(to.X, to.Y, NULL) +
             BRIDGE_FLOAT_AMT;
      addBridge(from, to, pMapObj->getName(), pTerrainLogic,
                pMapObj->getProperties());
      pMapObj = pMapObj2;
    }
  }
  if (pTerrainLogic) {
...
static RenderObjClass *createTower(SimpleSceneClass *scene,
                                   ...
                                   MapObject *mapObject, BridgeTowerType type,
                                   BridgeInfo *bridgeInfo) {
  RenderObjClass *tower = NULL;

  // sanity
  if (scene == NULL || assetManager == NULL || mapObject == NULL ||
      bridgeInfo == NULL || type < 0 || type >= BRIDGE_MAX_TOWERS)
    return NULL;

  // get template for this bridge
  DEBUG_ASSERTCRASH(TheTerrainRoads,
                    ("createTower: TheTerrainRoads is NULL\n"));
  TerrainRoadType *bridgeTemplate =
      TheTerrainRoads->findBridge(mapObject->getName());
  if (bridgeTemplate == NULL) return NULL;

  // given the type of tower (corner position) find the appropriate spot to put
  // the tower
  Coord3D towerPos;
  switch (type) {
    case BRIDGE_TOWER_FROM_LEFT:
      towerPos = bridgeInfo->fromLeft;
      break;
    case BRIDGE_TOWER_FROM_RIGHT:
      towerPos = bridgeInfo->fromRight;
      break;
    case BRIDGE_TOWER_TO_LEFT:
      towerPos = bridgeInfo->toLeft;
      break;
    case BRIDGE_TOWER_TO_RIGHT:
      towerPos = bridgeInfo->toRight;
      break;
    default:
      return NULL;
  ...
  towerPos.z =
      TheTerrainRenderObject->getHeightMapHeight(towerPos.x, towerPos.y, NULL);
  ...
  DEBUG_ASSERTCRASH(TheThingFactory,
                    ("createTower: TheThingFactory is NULL\n"));
  const ThingTemplate *towerTemplate =
      TheThingFactory->findTemplate(towerTemplateName);
  if (towerTemplate == NULL) return NULL;

  // find the name of the render object to show
  const ModuleInfo &mi = towerTemplate->getDrawModuleInfo();
  if (mi.getCount() <= 0) return NULL;
...
//=============================================================================
//=============================================================================
static void updateTowerPos(RenderObjClass *tower, BridgeTowerType type,
                           BridgeInfo *bridgeInfo) {
  // sanity
  if (tower == NULL || type < 0 || type >= BRIDGE_MAX_TOWERS ||
      bridgeInfo == NULL)
    return;

  //
  // compute the angle of the bridge ... we consider the angle of the bridge to
  // be from 'from' to 'to' in the bridge info ... and so does the game
  //
  Coord2D v;
  ...
  //
  // given the type of tower (corner position) find the appropriate spot to put
  // the tower NOTE that we're also adjusting the angle for the from side to
  // point the opposite way the "bridge is pointing"
  //
  Coord3D towerPos;
  switch (type) {
    case BRIDGE_TOWER_FROM_LEFT:
...
/** loadBridges.  When loaded, tell the terrain logic where the bridge is. */
//=============================================================================
void W3DBridgeBuffer::worldBuilderUpdateBridgeTowers(
  ...
  for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
    ...
    if (pMapObj->getFlag(FLAG_BRIDGE_POINT1)) {
      ...
      //
      // now that we've got the two map objects that are bridge point 1 and 2,
      // get the bridge info that has been stored
      //
      for (Int i = 0; i < m_numBridges; ++i) {
        //
        // find the bridge with the matching name and position ... note we're
        // just matching (x,y) here cause name and location (without the
        // additional complication of Z) is really all we have to match bridges.
        ...
        if (m_bridges[i].getTemplateName() == pMapObj->getName() &&
          ...
          for (Int j = 0; j < BRIDGE_MAX_TOWERS; ++j) {
            ...
            if (towerRenderObj == NULL) {
              towerRenderObj = createTower(scene, assetManager, pMapObj,
                                           (BridgeTowerType)j, &bridgeInfo);
              created = TRUE;

            }  // end if

            // sanity
            DEBUG_ASSERTCRASH(towerRenderObj != NULL,
                              ("worldBuilderUpdateBridgeTowers: unable to "
                               "create tower for bridge '%s'\n",
            ...
            if (created) REF_PTR_RELEASE(towerRenderObj);
...
//=============================================================================
void W3DBridgeBuffer::addBridge(Vector3 fromLoc, Vector3 toLoc,
                                AsciiString name,
...
void W3DBridgeBuffer::drawBridges(CameraClass *camera, Bool wireframe,
  ...
  if (TheTerrainLogic) {
    ...
    for (Bridge *bridge = TheTerrainLogic->getFirstBridge(); bridge;
      ...
      if (m_bridges[info.bridgeIndex].getDamageState() != info.curDamageState) {
        changed = true;
        enum BodyDamageType curState =
            m_bridges[info.bridgeIndex].getDamageState();
  ...
  if (!wireframe &&
      cloudTexture) {  // Force a cloud texture projection into stage 1
  ...
  if (!wireframe && TheTerrainRenderObject->getShroud()) {
    ...
    for (curBridge = 0; curBridge < m_numBridges; curBridge++) {
      if (m_bridges[curBridge].isEnabled() &&
          m_bridges[curBridge].isVisible()) {
        // Pretend we're in wireframe so function doesn't reset the shroud
        // texture.
