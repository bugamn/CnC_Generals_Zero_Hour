...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
W3DGameClient::W3DGameClient() {}  // end W3DGameClient
...
//-------------------------------------------------------------------------------------------------
/** Reset this device client system.  Note we are extending reset functionality
 * from the device independent client */
//-------------------------------------------------------------------------------------------------
...
void W3DGameClient::setTimeOfDay(TimeOfDay tod) {
  ...
  if (TheWaterRenderObj) TheWaterRenderObj->setTimeOfDay(tod);
  if (TheW3DShadowManager) TheW3DShadowManager->setTimeOfDay(tod);
...
void W3DGameClient::adjustLOD(Int adj) {
  ...
  if (TheWritableGlobalData->m_textureReductionFactor > 4)
    TheWritableGlobalData->m_textureReductionFactor =
        4;  // 16x less resolution is probably enough.
  ...
  if (WW3D::Get_Texture_Reduction() !=
      TheWritableGlobalData->m_textureReductionFactor) {
    WW3D::Set_Texture_Reduction(TheWritableGlobalData->m_textureReductionFactor,
                                6);
    TheGameLODManager->setCurrentTextureReduction(
        TheWritableGlobalData->m_textureReductionFactor);
  }
