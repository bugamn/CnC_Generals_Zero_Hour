...
void StatDumpClass::dumpStats() {
  ...
  // AI stats
  UnsignedInt numAI, numMoving, numAttacking, numWaitingForPath,
      overallFailedPathfinds;
  TheGameLogic->getAIMetricsStatistics(&numAI, &numMoving, &numAttacking,
                                       &numWaitingForPath,
                                       &overallFailedPathfinds);
  fprintf(m_fp, "\n");
  ...
  AsciiString slowScripts =
      TheScriptEngine->getStats(&timeLastFrame, &slowScript1, &slowScript2);
  fprintf(m_fp, "\n");
  ...
  // setup texture stats
  Debug_Statistics::Record_Texture_Mode(
      Debug_Statistics::RECORD_TEXTURE_SIMPLE /*RECORD_TEXTURE_NONE*/);
  ...
  fprintf(m_fp, "  3-Way Blends: %d, Shoreline Blends: %d\n",
          TheTerrainRenderObject->getNumExtraBlendTiles(),
  ...
  // Report memory usage.
  TheMemoryPoolFactory->debugMemoryReport(REPORT_FACTORYINFO | REPORT_POOLINFO,
                                          0, 0, m_fp);
#else
  fprintf(m_fp,
          "Memory Report -- unavailable (build doesn't have MEMORYPOOL_DEBUG "
          "defined)\n");
#endif
  fprintf(m_fp, "\n");
...
W3DDisplay::W3DDisplay() {
...
#endif
  for (i = 0; i < LightEnvironmentClass::MAX_LIGHTS; i++) m_myLight[i] = NULL;
  m_2DRender = NULL;
  ...
  for (i = 0; i < DisplayStringCount; i++) m_displayStrings[i] = NULL;
...
Int W3DDisplay::getDisplayModeCount(void) {
  ...
  /*	Bool needStencil=false;
          ...
          //Walk through all resolutions and determine which ones are compatible
     with other settings
          //chosen by user.  For example, 32-bit may be required for shadows,
     occlusion, soft water edge, etc. if (TheGlobalData->m_useShadowVolumes ||
     (TheGlobalData->m_enableBehindBuildingMarkers &&
     TheGameLogic->getShowBehindBuildingMarkers())) needStencil=true;
  ...
  */
  for (int res = 0; res < resolutions.Count(); res++) {
    ...
    if (resolutions[res].BitDepth >= 24 && resolutions[res].Width >= 800 &&
        (fabs((Real)resolutions[res].Width / (Real)resolutions[res].Height -
              1.3333f)) < 0.01f)  // only accept 4:3 aspect ratio modes.
    {
      numResolutions++;
    }
  }
...
void W3DDisplay::getDisplayModeDescription(Int modeIndex, Int *xres, Int *yres,
                                           Int *bitDepth) {
  Int numResolutions = 0;
  const RenderDeviceDescClass &devDesc = WW3D::Get_Render_Device_Desc(0);
  const DynamicVectorClass<ResolutionDescClass> &resolutions =
  ...
  for (int res = 0; res < resolutions.Count(); res++) {
    ...
    if (resolutions[res].BitDepth >= 24 && resolutions[res].Width >= 800 &&
        (fabs((Real)resolutions[res].Width / (Real)resolutions[res].Height -
              1.3333f)) < 0.01f)  // only accept 4:3 aspect ratio modes.
    {
      if (numResolutions == modeIndex) {  // found the mode
...
void W3DDisplay::setGamma(Real gamma, Real bright, Real contrast,
                          Bool calibrate) {
  if (m_windowed)
    return;  // we don't allow gamma to change in window because it would affect
             // desktop.
...
void Reset_D3D_Device(bool active) {
  if (TheDisplay && WW3D::Is_Initted() && !TheDisplay->getWindowed()) {
    if (active) {
      ...
      WW3D::Set_Render_Device(WW3D::Get_Render_Device(), TheDisplay->getWidth(),
                              TheDisplay->getHeight(),
                              TheDisplay->getBitDepth(),
                              TheDisplay->getWindowed(), true, true);
      OSVERSIONINFO osvi;
      osvi.dwOSVersionInfoSize = sizeof(OSVERSIONINFO);
      if (GetVersionEx(&osvi)) {  // check if we're running Win9x variant since
                                  // they have buggy alt-tab that requires
        // reloading all textures.
        if (osvi.dwPlatformId ==
            VER_PLATFORM_WIN32_WINDOWS) {  // only do this on Win9x boxes
                                           // because it makes alt-tab very
                                           // slow.
          WW3D::_Invalidate_Textures();
        }
      }
    } else {
      // switch to windowed mode whenever the user alt-tabs out of game. Don't
      // restore assets after reset since we'll do it when returning.
      WW3D::Set_Render_Device(WW3D::Get_Render_Device(), TheDisplay->getWidth(),
                              TheDisplay->getHeight(),
                              TheDisplay->getBitDepth(),
                              TheDisplay->getWindowed(), true, true, false);
    }
  }
}
...
//=============================================================================
Bool W3DDisplay::setDisplayMode(UnsignedInt xres, UnsignedInt yres,
                                UnsignedInt bitdepth, Bool windowed) {
  if (WW3D_ERROR_OK ==
      WW3D::Set_Device_Resolution(xres, yres, bitdepth, windowed, true)) {
  ...
  WW3D::Set_Device_Resolution(getWidth(), getHeight(), getBitDepth(),
                              getWindowed(), true);
  Render2DClass::Set_Screen_Resolution(
      RectClass(0, 0, getWidth(), getHeight()));
  Display::setDisplayMode(getWidth(), getHeight(), getBitDepth(),
                          getWindowed());
...
/** */
//=============================================================================
void W3DDisplay::initAssets(void) {}  // end initAssets

// W3DDisplay::init3DScene ====================================================
/** */
//=============================================================================
void W3DDisplay::init3DScene(void) {}  // end init3DScene

// W3DDisplay::init2DScene ====================================================
/** This is the 2D scene, you can use it to draw on a 2D plane over the
 * 3D background */
//=============================================================================
void W3DDisplay::init2DScene(void) {}  // end init2DScene
...
void W3DDisplay::init(void) {
...
#if defined(_DEBUG) || defined(_INTERNAL)
  if (TheGlobalData->m_wireframe) m_3DScene->Set_Polygon_Mode(SceneClass::LINE);
#endif
  //============================================================================
  // m_myLight = NEW_REF
  //============================================================================
  ...
  setTimeOfDay(
      TheGlobalData
          ->m_timeOfDay);  // set each light to correct values for given time
  ...
  if (WW3D::Init(ApplicationHWnd) != WW3D_ERROR_OK)
    throw ERROR_INVALID_D3D;  // failed to initialize.  User probably doesn't
                              // have DX 8.1

  WW3D::Set_Prelit_Mode(WW3D::PRELIT_MODE_LIGHTMAP_MULTI_PASS);
  WW3D::Set_Collision_Box_Display_Mask(
      0x00);  ///< set to 0xff to make collision boxes visible
  ...
  if (WW3D::Set_Render_Device(0, getWidth(), getHeight(), getBitDepth(),
                              getWindowed(), true) != WW3D_ERROR_OK) {
  ...
  // Check if level was never set and default to setting most suitable for
  // system.
  if (TheGameLODManager->getStaticLODLevel() == STATIC_GAME_LOD_UNKNOWN)
    TheGameLODManager->setStaticLODLevel(
        TheGameLODManager->findStaticLODLevel());
  else {  // Static LOD level was applied during GameLOD manager init except for
          // texture reduction
  ...
  if (m_nativeDebugDisplay) {
    ...
    if (TheGlobalLanguageData &&
        TheGlobalLanguageData->m_nativeDebugDisplay.name.isNotEmpty()) {
...
void W3DDisplay::reset(void) {
  ...
  /// @todo really need that "scene abstraction", having this stuff in the
  /// display is icky
  m_assetManager->Release_Unused_Assets();

  if (TheWritableGlobalData) TheWritableGlobalData->m_drawSkyBox = 0;
}

const UnsignedInt START_CUMU_FRAME =
    LOGICFRAMES_PER_SECOND / 2;  // skip first half-sec

/** Update a moving average of the last 30 fps measurements.  Also try to filter
   out temporary spikes. This code is designed to be used by the GameLOD sytems
   to determine the correct dynamic LOD setting.
*/
void W3DDisplay::updateAverageFPS(void) {
  const Real MaximumFrameTimeCutoff =
      0.5f;  // largest frame interval (seconds) we accept before ignoring it as
             // a momentary "spike"
  ...
  if (numSamples) {
    ...
    for (Int i = 0, j = historyOffset - 1; i < numSamples; i++, j--) {
      if (j < 0) j = FPS_HISTORY_SIZE - 1;  // wrap around to front of buffer
...
#if defined(_DEBUG) || \
    defined(_INTERNAL)  // debug hack to view object under mouse stats
...
void W3DDisplay::gatherDebugStats(void) {
  ...
  if (m_displayStrings[0] == NULL) {
    ...
    for (int i = 0; i < DisplayStringCount; i++) {
      if (m_displayStrings[i] == NULL) {
        ...
        DEBUG_ASSERTCRASH(m_displayStrings[i],
                          ("Failed to create DisplayString"));
  ...
  if (m_benchmarkDisplayString == NULL) {
    GameFont *thisFont =
        TheFontLibrary->getFont(AsciiString("FixedSys"), 8, FALSE);
    m_benchmarkDisplayString = TheDisplayStringManager->newDisplayString();
    DEBUG_ASSERTCRASH(m_benchmarkDisplayString,
                      ("Failed to create DisplayString"));
  ...
  s_timeSinceLastUpdateInSecs =
      ((double)(time64 - s_lastUpdateTime64) / (double)(freq64));
  ...
  if (pListFile && lastFrameTime && samples < 100) {
    float timeSinceLastFrame =
        (float)((double)(time64 - lastFrameTime) / (double)(freq64));
  ...
  if (s_timeSinceLastUpdateInSecs >= UPDATE_RATE_SECS ||
      TheGlobalData->m_constantDebugUpdate) {
    ...
    // frames per second
    double fps =
        (Real)s_framesRenderedSinceLastUpdate / s_timeSinceLastUpdateInSecs;
    double drawsPerFrame = Debug_Statistics::
        Get_Draw_Calls();  //(Real)s_drawCallsSinceLastUpdate /
                           //(Real)s_framesRenderedSinceLastUpdate;
    double sortPolysPerFrame = Debug_Statistics::
        Get_Sorting_Polygons();  //(Real)s_sortedPolysSinceLastUpdate /
                                 //(Real)s_framesRenderedSinceLastUpdate;
...
#if defined(_DEBUG) || defined(_INTERNAL)
    double cumuTime =
        ((double)(time64 - m_timerAtCumuFPSStart) / (double)(freq64));
    if (cumuTime < 0.0) cumuTime = 0.0;
    Int numFrames = (Int)TheGameLogic->getFrame() - (Int)START_CUMU_FRAME;
    double cumuFPS =
        (numFrames > 0 && cumuTime > 0.0) ? (numFrames / cumuTime) : 0.0;
    double skinPolysPerFrame = Debug_Statistics::Get_DX8_Skin_Polygons();

    // Int LOD = TheGlobalData->m_terrainLOD;
    // unibuffer.format( L"FPS: %.2f, %.2fms mapLOD=%d [cumu FPS=%.2f] draws:
    // %.2f sort: %.2f", fps, ms, LOD, cumuFPS,
    // drawsPerFrame,sortPolysPerFrame);
    if (TheGlobalData->m_useFpsLimit)
      unibuffer.format(L"%.2f/%d FPS, ", fps,
                       TheGameEngine->getFramesPerSecondLimit());
    else
      unibuffer.format(L"%.2f FPS, ", fps);

    unibuffer2.format(
        L"%.2fms [cumuFPS=%.2f] draws: %d skins: %d sortP: %d skinP: %d", ms,
        cumuFPS, (Int)drawsPerFrame, (Int)skinDrawsPerFrame,
        (Int)sortPolysPerFrame, (Int)skinPolysPerFrame);
    unibuffer.concat(unibuffer2);
#else
    // Int LOD = TheGlobalData->m_terrainLOD;
    // unibuffer.format( L"FPS: %.2f, %.2fms mapLOD=%d draws: %.2f sort %.2f",
    // fps, ms, LOD, drawsPerFrame,sortPolysPerFrame);
    unibuffer.format(L"FPS: %.2f, %.2fms draws: %.2f skins: %.2f sort %.2f",
                     fps, ms, drawsPerFrame, skinDrawsPerFrame,
                     sortPolysPerFrame);
    if (TheGlobalData->m_useFpsLimit) {
    ...
    const int SHOW_STATS_TIME =
        12;  // show extended stats for 5 cycles == 10 seconds.
    static enum {
      disabled,
      sync,
      gameOverhead,
    ...
    if (statMode != disabled) {
      unibuffer.format(L"FPS: %.2f, %.2fms - Collecting extended stats.", fps,
                       ms);
    } else if (extendedStats > 0) {
      extendedStats--;
      unibuffer.format(
          L"FPS: %.2f, %.2fms - OH %.2fms, Console %.2fms, 3D OH %.2fms, "
          L"Terrain %.2fms, Obs %.2fms, CPU %.2fms",
          fps, ms, gameOverheadMS, consoleMS, threeDOverheadMS, terrainMS,
          objectMS, overlapMS);
      if (extendedStats == SHOW_STATS_TIME - 2) {
        char bufferA[256];
        sprintf(bufferA,
                "FPS: %.2f, %.2fms - OH %.2fms, Console %.2fms, 3D OH %.2fms, "
                "Terrain %.2fms, Obs %.2fms, CPU %.2fms\n",
                fps, ms, gameOverheadMS, consoleMS, threeDOverheadMS, terrainMS,
                objectMS, overlapMS);
        ...
        sprintf(bufferA, "Polygons: per frame %d, per second %d\n",
                polyPerFrame, (Int)(polyPerFrame * fps));
        ::OutputDebugString(bufferA);
        if (pListFile) {
    ...
    // out the terrain alpha texture (since it's really == terrain texture).
    //
    unibuffer.format(L"Video RAM: %d",
                     Debug_Statistics::Get_Record_Texture_Size() - 1376256);
    ...
    // terrain stats
    unibuffer.format(L"3-Way Blends: %d, Shoreline Blends: %d",
                     TheTerrainRenderObject->getNumExtraBlendTiles(),
                     TheTerrainRenderObject->getNumShoreLineTiles());
    ...
    Real actualHeightAboveGround =
        TheTacticalView->getCurrentHeightAboveGround();

    unibuffer.format(
        L"Camera zoom: %g, pitch: %g/%g, yaw: %g, pos: %g, %g, %g, FOV: %g\n   "
        L"    Height above ground: %g Terrain height: %g",
        zoom, pitch, FXPitch, angle, camPos.x, camPos.y, camPos.z, FOV,
        /*
        zoom,
        pitch * 180.0f / PI,
        FXPitch * 180.0f / PI,
...
#if defined(_DEBUG) || \
    defined(_INTERNAL)  // debug hack to view object under mouse stats
    Drawable *draw = TheTacticalView->pickDrawable(&TheMousePos, FALSE,
                                                   (PickType)0xffffffff);
#else
    Drawable *draw =
        TheGameClient->findDrawableByID(TheInGameUI->getMousedOverDrawableID());
#endif
    if (draw) object = draw->getObject();
    if (object) {
      unibuffer2.format(L"Moused over object: %S (%d) ",
    ...
    unibuffer.format(L"Mouse position: screen: (%d, %d), world: (%g, %g, %g)",
                     mouseIO->pos.x, mouseIO->pos.y, worldPos.x, worldPos.y,
                     worldPos.z);
    m_displayStrings[MousePosition]->setText(unibuffer);

    // display the number of particles in the world and being displayed on
    // screen
    Int totalParticles = TheParticleSystemManager->getParticleCount();
    Int onScreenParticleCount =
        TheParticleSystemManager->getOnScreenParticleCount();
    unibuffer.format(L"Particles: %d in world, %d being displayed",
    ...
    unibuffer.format(L"Objects: %d in world, %d being displayed", objCount,
                     objScreenCount);
    ...
    if (TheNetwork != NULL) {
      unibuffer.format(L"IN: %.2f bytes/sec, %.2f packets/sec",
                       TheNetwork->getIncomingBytesPerSecond(),
                       TheNetwork->getIncomingPacketsPerSecond());
      ...
      unibuffer.format(L"OUT: %.2f bytes/sec, %.2f packets/sec",
                       TheNetwork->getOutgoingBytesPerSecond(),
                       TheNetwork->getOutgoingPacketsPerSecond());
      m_displayStrings[NetOutgoing]->setText(unibuffer);

      // Network performance stats
      unibuffer.format(
          L"Run Ahead: %d, Net FPS: %d, Packet arrival cushion: %d",
          TheNetwork->getRunAhead(), TheNetwork->getFrameRate(),
          TheNetwork->getPacketArrivalCushion());
      m_displayStrings[NetStats]->setText(unibuffer);

      // Client frame rate averages for all players in the game.  This only
      // works right for the packet router.
      unibuffer.clear();
      Int numPlayers = TheNetwork->getNumPlayers();
      for (Int i = 0; i < numPlayers; ++i) {
        UnicodeString tempstr;
        tempstr.format(L"%s: %d ", TheNetwork->getPlayerName(i).str(),
                       TheNetwork->getSlotAverageFPS(i));
        unibuffer.concat(tempstr);
      }
      m_displayStrings[NetFPSAverages]->setText(unibuffer);
    } else {
      //			unibuffer.format(L"IN: 0.0 bytes/sec, 0.0
      //packets/sec"); 			m_displayStrings[NetIncoming]->setText( unibuffer );

      // Network outgoing bandwidth stats
      //			unibuffer.format(L"OUT: 0.0 bytes/sec, 0.0
      //packets/sec"); 			m_displayStrings[NetOutgoing]->setText( unibuffer );
      unibuffer.format(L"");
    ...
    unibuffer.format(L"Select Info: '%d' drawables selected",
                     TheInGameUI->getSelectCount());

    // Sorry, guys. I need a special kluge here to get constantdebug results for
    // angry mob. Do no be cross with me. if there is not exactly one drawable
    // selected it will report on the moused-over drawable
    ...
    if (draw) {
      ...
      if (obj && obj->getName().isEmpty() == FALSE) objectName = obj->getName();

      unibuffer.format(L"Select Info: '%S'(%S) at (%.3f,%.3f,%.3f)",
                       draw->getTemplate()->getName().str(), objectName.str(),
                       draw->getPosition()->x, draw->getPosition()->y,
                       draw->getPosition()->z);
      ...
      if (rcost.getDrawCallCount() > 0) {
        unibuffer2.format(
            ...
            rcost.getDrawCallCount(), rcost.getShadowDrawCount(),
            rcost.getSortedMeshCount(), rcost.getSkinMeshCount(),
            rcost.getBoneCount());
        unibuffer.concat(unibuffer2);
      }
#endif
...
void StatDebugDisplay(DebugDisplayInterface *, void *, FILE *fp) {
  DEBUG_CRASH(
      ("This should never be called directly, but is just a placeholder for "
       "drawDebugStats()"));
}
...
void W3DDisplay::calculateTerrainLOD(void) {
  ...
  do {
    ...
    for (i = 0; i < NUM_SAMPLES; i++) {
      ...
      // start render block
      updateViews();
      if (WW3D::Begin_Render(true, true, Vector3(0.0f, 0.0f, 0.0f)) ==
          WW3D_ERROR_OK) {  // draw all views of the world
        drawViews();
        // render is all done!
        WW3D::End_Render();
      }
...
void W3DDisplay::draw(void) {
  ...
  if (TheGlobalData->m_enableDynamicLOD && TheGameLogic->getShowDynamicLOD()) {
    DynamicGameLODLevel lod =
        TheGameLODManager->findDynamicLODLevel(m_averageFPS);
    TheGameLODManager->setDynamicLODLevel(lod);
  } else {  // if dynamic LOD is turned off, force highest LOD
...
AGAIN:
  ...
  /// @todo Make more explicit drawing layers(ground, ground UI, objects, object
  /// UI, overlay UI)
  ...
  // hack to let client spin fast in network games but still do effects at the
  // same pace. -MDC
  ...
  Debug_Statistics::Begin_Statistics();  // reset all counters (polygons,
                                         // vertices, etc) before drawing
  ...
  if (TheGlobalData->m_loadScreenRender != TRUE) {
    ...
    if (TheTerrainRenderObject) {
      ...
      if (TheTerrainRenderObject
              ->getMap())  // make sure a valid map is loaded into terrain.
      {
        if (TheTerrainRenderObject->getShroud()) {
          TheTerrainRenderObject->getShroud()->render(
              primaryW3DView->get3DCamera());
        }
      }
    }
  }
  ...
  if (!freezeTime) {
    ...
    // for now, use constant time steps to avoid animations running independent
    // of framerate
    syncTime += TheW3DFrameLengthInMsec;
  ...
  if (TheTacticalView->getTimeMultiplier() > 1) {
    ...
    if (timeMultiplierCounter > 1) return;
    timeMultiplierCounter = TheTacticalView->getTimeMultiplier();
    // limit the framerate, because while fast time is on, the game logic is
  ...
  do {
    ...
    if (DX8Wrapper::_Get_D3D_Device8() &&
        (DX8Wrapper::_Get_D3D_Device8()->TestCooperativeLevel()) ==
            D3D_OK) {  // Checking if we have the device before updating views
                       // because the heightmap crashes otherwise while trying
                       // to refresh the visible terrain geometry.
      //			if(TheGlobalData->m_loadScreenRender != TRUE)
      updateViews();

      if (TheWaterRenderObj && TheGlobalData->m_waterType == 2)
        TheWaterRenderObj->updateRenderTargetTextures(
            primaryW3DView->get3DCamera());  // do a render into each texture

      // Can't render into textures while rendering to screen so these textures
      // need to be updated before we enter main rendering loop.
      if (TheW3DProjectedShadowManager)
        TheW3DProjectedShadowManager->updateRenderTargetTextures();
    }

    Debug_Statistics::End_Statistics();  // record number of polygons rendered
                                         // in RenderTargetTextures.
    ...
    // start render block
    {
      ...
      if ((TheGlobalData->m_breakTheMovie == FALSE) &&
        ...
        if (TheGlobalData->m_loadScreenRender == TRUE) {
          TheInGameUI->draw();
          if (TheMouse)
            TheMouse->draw();  // keep applying the current cursor style so it
                               // remains hidden if needed.
          WW3D::End_Render();
          continue;
        }
        couldRender = true;
        ...
        if (numRenderTargetPolygons || numRenderTargetVertices)
          Debug_Statistics::Record_DX8_Polys_And_Vertices(
              numRenderTargetPolygons, numRenderTargetVertices,
        ...
        if (m_copyrightDisplayString) {
          ...
          x = (getWidth() / 2) - (dX / 2);
          y = getHeight() - dY - 20;
          m_copyrightDisplayString->draw(x, y, GameMakeColor(0, 0, 0, 255),
                                         GameMakeColor(0, 0, 0, 0), 0, 0);
        }
        // render letter box before debug display so debug info isn't hidden
        renderLetterBox(now);

        // display cinematicText over the black
        if (m_cinematicText != AsciiString::TheEmptyString &&
    ...
    if (TheScriptEngine->isTimeFrozenDebug() ||
        TheScriptEngine->isTimeFrozenScript() || TheGameLogic->isGamePaused()) {
      freezeTime = false;  // We're frozen for debug or for pause, and need to
  ...
  if (m_letterBoxEnabled) {
    if (m_letterBoxFadeLevel != 1.0f) {
      m_letterBoxFadeLevel =
          (currentTime - m_letterBoxFadeStartTime) / LETTER_BOX_FADE_TIME;
      if (m_letterBoxFadeLevel > 1.0f) m_letterBoxFadeLevel = 1.0f;
    }
...
#else
    drawFillRect(0, 0, m_width, (m_height - (9.0f / 16.0f * m_width)) * 0.5f,
                 lbcolor);
    drawFillRect(0, m_height - (m_height - (9.0f / 16.0f * m_width)) * 0.5f,
                 m_width, m_height, lbcolor);
#endif
  } else {  // letter box is disabled, but may still be fading out
    if (m_letterBoxFadeLevel != 0.0f) {
      m_letterBoxFadeLevel = 1.0f - (currentTime - m_letterBoxFadeStartTime) /
                                        LETTER_BOX_FADE_TIME;
      if (m_letterBoxFadeLevel < 0.0f) m_letterBoxFadeLevel = 0.0f;
      ...
      // m_width, m_height, lbcolor );
#endif
    } else {  // box has finished fading out
#ifdef SLIDE_LETTERBOX
...
Bool W3DDisplay::isLetterBoxFading(void) {
  if (m_letterBoxEnabled && m_letterBoxFadeLevel != 1.0f) return TRUE;
  if (!m_letterBoxEnabled && m_letterBoxFadeLevel != 0.0f) return TRUE;
  return FALSE;
}
...
void W3DDisplay::createLightPulse(const Coord3D *pos, const RGBColor *color,
  ...
  theDynamicLight->Set_Far_Attenuation_Range(innerRadius,
                                             innerRadius + attenuationWidth);
...
void W3DDisplay::enableLetterBox(Bool enable) {
  if (enable) {
    if (!m_letterBoxEnabled) {  // letterbox mode not previously enabled
      m_letterBoxEnabled = TRUE;
      m_letterBoxFadeStartTime = timeGetTime();
    }
  } else {
    if (m_letterBoxEnabled) {  // letterbox mode no previously disabled
      m_letterBoxEnabled = FALSE;
      m_letterBoxFadeStartTime = timeGetTime();
    }
  }
...
void W3DDisplay::setTimeOfDay(TimeOfDay tod) {
  const GlobalData::TerrainLighting *ol =
      &TheGlobalData->m_terrainObjectsLighting[tod][0];

  if (m_3DScene) {
    m_3DScene->Set_Ambient_Light(
        Vector3(ol->ambient.red, ol->ambient.green, ol->ambient.blue));
  }
  ...
  for (Int i = 0; i < LightEnvironmentClass::MAX_LIGHTS; i++) {
    if (m_myLight[i]) {
      ...
      m_myLight[i]->Set_Diffuse(
          Vector3(ol->diffuse.red, ol->diffuse.green, ol->diffuse.blue));
...
void W3DDisplay::drawLine(Int startX, Int startY, Int endX, Int endY,
  ...
  m_2DRender->Add_Line(Vector2(startX, startY), Vector2(endX, endY), lineWidth,
                       lineColor);
  m_2DRender->Render();
...
//=============================================================================
void W3DDisplay::drawLine(Int startX, Int startY, Int endX, Int endY,
                          Real lineWidth, UnsignedInt lineColor1,
...
void W3DDisplay::drawOpenRect(Int startX, Int startY, Int width, Int height,
  ...
  if (m_isClippedEnabled) {
    ...
    if (ClipLine2D(&start, &end, &returnStart, &returnEnd, &m_clipRegion))
      drawLine(returnStart.x, returnStart.y, returnEnd.x, returnEnd.y,
               lineWidth, lineColor);
    ...
    m_2DRender->Add_Outline(
        RectClass(startX, startY, startX + width, startY + height), lineWidth,
        lineColor);
...
void W3DDisplay::drawFillRect(Int startX, Int startY, Int width, Int height,
  ...
  m_2DRender->Enable_Texturing(FALSE);
  m_2DRender->Add_Rect(
      RectClass(startX, startY, startX + width, startY + height), 0, 0, color);
...
void W3DDisplay::drawRectClock(Int startX, Int startY, Int width, Int height,
  ...
  if (percent == 100) {
    m_2DRender->Add_Rect(
        RectClass(startX, startY, startX + width, startY + height), 0, 0,
        color);
  } else if (percent > 75) {
    // rectangle #1 & 2
    m_2DRender->Add_Rect(
        RectClass(startX + width / 2, startY, startX + width, startY + height),
        0, 0, color);
    // rectangle #3
    ...
    if (remain > 12) {
      ...
      m_2DRender->Add_Tri(
          Vector2(startX, startY + height / 2 - (height / 2 * percentDraw)),
          Vector2(startX, startY + height / 2),
          Vector2(startX + width / 2, startY + height / 2), Vector2(0, 0),
          Vector2(0, 0), Vector2(0, 0), color);
    }
    ...
    if (remain > 12) {
      ...
      Real percentDraw = (Real)(remain - 12) / 13;
      m_2DRender->Add_Tri(
          Vector2(startX, startY + height - (height / 2 * percentDraw)),
          Vector2(startX, startY + height),
          Vector2(startX + width / 2, startY + height / 2), Vector2(0, 0),
          Vector2(0, 0), Vector2(0, 0), color);
    } else {
      ...
      m_2DRender->Add_Tri(
          Vector2(startX + width / 2, startY + height),
          Vector2(startX + width / 2, startY + height / 2),
          Vector2(startX + width / 2 - (width / 2 * percentDraw),
                  startY + height),
          Vector2(0, 0), Vector2(0, 0), Vector2(0, 0), color);
    }
  } else if (percent > 25) {
    // rectangel #1
    m_2DRender->Add_Rect(RectClass(startX + width / 2, startY, startX + width,
                                   startY + height / 2),
                         0, 0, color);
    ...
    if (remain > 12) {
      ...
      m_2DRender->Add_Tri(
          ...
          Vector2(startX + width, startY + height), Vector2(0, 0),
          Vector2(0, 0), Vector2(0, 0), color);
    } else {
      // draw the part of triangle
      Real percentDraw = (Real)(remain) / 12;
      m_2DRender->Add_Tri(
          Vector2(startX + width, startY + height / 2),
          Vector2(startX + width / 2, startY + height / 2),
          Vector2(startX + width,
                  startY + height / 2 + (height / 2 * percentDraw)),
          Vector2(0, 0), Vector2(0, 0), Vector2(0, 0), color);
    }
  } else {
    ...
    if (percent > 12) {
      ...
      m_2DRender->Add_Tri(Vector2(startX + width / 2, startY),
                          Vector2(startX + width / 2, startY + height / 2),
                          Vector2(startX + width, startY), Vector2(0, 0),
                          Vector2(0, 0), Vector2(0, 0), color);
...
// This version will overlay a clock progress from the specified percentage to
// 100%. Essentially, this function will "reveal" an icon as it progresses
// towards completion.
//--------------------------------------------------------------------------------------------------------------------
void W3DDisplay::drawRemainingRectClock(Int startX, Int startY, Int width,
                                        Int height, Int percent,
                                        UnsignedInt color) {
  // sanity
  ...
  if (percent == 0) {
    ...
    if (percent < 13) {
      ...
      m_2DRender->Add_Tri(Vector2(midX, midY), Vector2(endX, midY),
                          Vector2(endX, startY), Vector2(0, 0), Vector2(0, 0),
                          Vector2(0, 0), color);

      // Draw the last part of the 1st portion of rectangle #1
      Real percentDraw = (Real)(13 - percent) / 13;
      m_2DRender->Add_Tri(Vector2(midX, midY), Vector2(endX, startY),
                          Vector2(endX - halfWidth * percentDraw, startY),
                          Vector2(0, 0), Vector2(0, 0), Vector2(0, 0), color);
    } else {
      // 13-24%
      //------
      ...
      m_2DRender->Add_Tri(Vector2(midX, midY), Vector2(endX, midY),
                          Vector2(endX, startY + halfHeight * percentDraw),
                          Vector2(0, 0), Vector2(0, 0), Vector2(0, 0), color);
    }
  } else if (percent < 50) {
    // 25-49%
    ...
    if (percent < 38) {
      ...
      m_2DRender->Add_Tri(Vector2(midX, midY), Vector2(midX, endY),
                          Vector2(endX, endY), Vector2(0, 0), Vector2(0, 0),
                          Vector2(0, 0), color);

      // Draw the last part of the 1st portion of rectangle #2
      Real percentDraw = (Real)(percent - 25) / 13;
      m_2DRender->Add_Tri(Vector2(midX, midY), Vector2(endX, endY),
                          Vector2(endX, midY + halfHeight * percentDraw),
                          Vector2(0, 0), Vector2(0, 0), Vector2(0, 0), color);
    } else {
      // 38-49%
      //------
      ...
      m_2DRender->Add_Tri(Vector2(midX, midY), Vector2(midX, endY),
                          Vector2(endX - halfWidth * percentDraw, endY),
                          Vector2(0, 0), Vector2(0, 0), Vector2(0, 0), color);
    }
  } else if (percent < 75) {
    // 50-74%
    ...
    if (percent < 63) {
      ...
      m_2DRender->Add_Tri(Vector2(midX, midY), Vector2(startX, midY),
                          Vector2(startX, endY), Vector2(0, 0), Vector2(0, 0),
                          Vector2(0, 0), color);

      // Draw the last part of the 1st portion of rectangle #3
      Real percentDraw = (Real)(percent - 50) / 13;
      m_2DRender->Add_Tri(Vector2(midX, midY), Vector2(startX, endY),
                          Vector2(midX - halfWidth * percentDraw, endY),
                          Vector2(0, 0), Vector2(0, 0), Vector2(0, 0), color);
    } else {
      // 62-74%
      //------
      ...
      m_2DRender->Add_Tri(Vector2(midX, midY), Vector2(startX, midY),
                          Vector2(startX, endY - halfHeight * percentDraw),
                          Vector2(0, 0), Vector2(0, 0), Vector2(0, 0), color);
    }
  } else {
    // 75-99%
    ...
    if (percent < 87) {
      ...
      m_2DRender->Add_Tri(Vector2(midX, midY), Vector2(midX, startY),
                          Vector2(startX, startY), Vector2(0, 0), Vector2(0, 0),
                          Vector2(0, 0), color);

      // Draw the last part of the 1st portion of rectangle #4
      Real percentDraw = (Real)(percent - 75) / 13;
      m_2DRender->Add_Tri(Vector2(midX, midY), Vector2(startX, startY),
                          Vector2(startX, midY - halfHeight * percentDraw),
                          Vector2(0, 0), Vector2(0, 0), Vector2(0, 0), color);
    } else {
      // 88-99%
      //------
      ...
      m_2DRender->Add_Tri(Vector2(midX, midY), Vector2(midX, startY),
                          Vector2(startX + halfWidth * percentDraw, startY),
                          Vector2(0, 0), Vector2(0, 0), Vector2(0, 0), color);
    }
  }
...
//=============================================================================
void W3DDisplay::drawImage(const Image *image, Int startX, Int startY, Int endX,
                           Int endY, Color color, DrawImageMode mode) {
  // sanity
  ...
  // if we have raw texture data we will use it, otherwise we are referencing
  // filenames
  ...
  if (m_isClippedEnabled) {  // need to clip this quad to clip rectangle

    //
    //	Check for completely clipped
    //
    if (endX <= m_clipRegion.lo.x || endY <= m_clipRegion.lo.y) {
  ...
  if (BitTest(image->getStatus(), IMAGE_STATUS_ROTATED_90_CLOCKWISE)) {
    m_2DRender->Add_Tri(Vector2(screen_rect.Left, screen_rect.Top),
                        ...
                        Vector2(uv_rect.Right, uv_rect.Bottom), color);
    ...
    m_2DRender->Add_Tri(Vector2(screen_rect.Right, screen_rect.Bottom),
                        ...
                        Vector2(uv_rect.Left, uv_rect.Top), color);
  ...
  if (doAlphaReset) m_2DRender->Enable_Alpha(true);
...
VideoBuffer *W3DDisplay::createVideoBuffer(void) {
  ...
  /// @todo query video player for supported formats - we assume bink formats
  /// here
  ...
  // on low mem machines, render every video in 16bit except for the EA Logo
  // movie
  if (!TheGlobalData
           ->m_playIntro)  //&& TheGameLODManager &&
...
void W3DDisplay::setBorderShroudLevel(UnsignedByte level) {
  if (TheTerrainRenderObject && TheTerrainRenderObject->getShroud()) {
    TheTerrainRenderObject->getShroud()->setBorderShroudLevel(
  ...
  if (TheTerrainRenderObject && TheTerrainRenderObject->getShroud()) {
    ...
    if (setting == CELLSHROUD_SHROUDED)
      TheTerrainRenderObject->getShroud()->setShroudLevel(
          x, y, (W3DShroudLevel)TheGlobalData->m_shroudAlpha);
    else if (setting == CELLSHROUD_FOGGED)
      TheTerrainRenderObject->getShroud()->setShroudLevel(
          x, y,
          (W3DShroudLevel)TheGlobalData
              ->m_fogAlpha);  ///< @todo placeholder to get feedback on logic
                              ///< work while graphic side being decided
    else
      TheTerrainRenderObject->getShroud()->setShroudLevel(
          x, y, (W3DShroudLevel)TheGlobalData->m_clearAlpha);
    // Logic is saying shroud.  We can add alpha levels here in client if
    // needed.
    //  W3DShroud is a 0-255 alpha byte.  Logic shroud is a double reference
    //  count.
  }
}
...
static void CreateBMPFile(LPTSTR pszFile, char *image, Int width, Int height) {
  ...
  pbmi->bmiHeader.biSizeImage =
      (pbmi->bmiHeader.biWidth + 7) / 8 * pbmi->bmiHeader.biHeight * 24;
...
void W3DDisplay::takeScreenShot(void) {
  ...
  while (!done) {
    ...
    if (_access(pathname, 0) == -1) done = true;
  }
  ...
  TheInGameUI->message(TheGameText->fetch("GUI:ScreenCapture"),
                       ufileName.str());
}
...
void dumpHLODAssets(HLodClass *hlod) {
  if (hlod) {
    ...
    for (Int i = 0; i < hlod->Get_Num_Sub_Objects(); i++) {
      ...
      else if (subObj->Class_ID() == RenderObjClass::CLASSID_MESH)
...
//-------------------------------------------------------------------------------------------------
/** Preload using the W3D asset manager the model referenced by the string
 * parameter */
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
/** Preload using the W3D asset manager the texture referenced by the string
 * parameter */
//-------------------------------------------------------------------------------------------------
...
void W3DDisplay::doSmartAssetPurgeAndPreload(const char *usageFileName) {
  if (!m_assetManager || !usageFileName || !*usageFileName) return;
  ...
  if (f) {
    for (;;) {
      AsciiString tmp;
      if (f->scanString(tmp) == FALSE) break;

      // allow for comments in the file. Note that this doesn't allow for
      // comments with spaces! doh. oh well. better than nothing.
  ...
  // just free everything if there's no exclusion list file (send in an empty
  // list)
...
void W3DDisplay::dumpAssetUsage(const char *mapname) {
  if (!m_assetManager || !mapname || !*mapname) return;
  ...
  while (true) {
    ...
    if (_access(buf, 0) != 0) break;  // it exists, we're good
    ++idx;
  }
...
static void drawFramerateBar(void) {
  ...
  Real percTime = (1000.0f / (now - prevTime)) /
                  (1000.0f / TheGlobalData->m_framesPerSecondLimit);
  ...
  UnsignedInt colorToUse =
      GameMakeColor(REAL_TO_UNSIGNEDBYTE((1.0f - percTime) * 255),
                    REAL_TO_UNSIGNEDBYTE(percTime * 255), 0, 0x7F);
