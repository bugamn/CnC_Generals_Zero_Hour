...
//-------------------------------------------------------------------------------------------------
/** Game file access.  At present this allows us to access test assets, assets
 * from legacy GDI assets, and the current flat directory access for textures,
 * models etc */
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
GameFileClass::~GameFileClass() { Close(); }

//-------------------------------------------------------------------------------------------------
/** Gets the file name */
//-------------------------------------------------------------------------------------------------
char const *GameFileClass::File_Name(void) const { return m_filename; }
...
// if file is still not found, try the test art folders
  ...
  // We allow the user to load their own images for various assets (like the
  // control bar)
  if (m_fileExists == FALSE && TheGlobalData) {
    if (fileType == FILE_TYPE_W3D) {
      sprintf(m_filePath, USER_W3D_DIR_PATH,
    ...
    if (isImageFileType(fileType)) {
      sprintf(m_filePath, USER_TGA_DIR_PATH,
              TheGlobalData->getPath_UserData().str());
  ...
  // We Need to be able to "temporarily copy over the map preview for whichever
  // directory it came from
  if (m_fileExists == FALSE && TheGlobalData) {
    if (fileType == FILE_TYPE_TGA)  // just TGA, since we don't dds previews
    {
      sprintf(m_filePath, MAP_PREVIEW_DIR_PATH,
              TheGlobalData->getPath_UserData().str());
      // strcpy( m_filePath, USER_TGA_DIR_PATH );
      strcat(m_filePath, filename);
  ...
  // We need to be able to grab images from a localization dir, because Art has
  // a fetish for baked-in text.  Munkee.
  if (m_fileExists == FALSE) {
    if (isImageFileType(fileType)) {
...
bool GameFileClass::Is_Available(int forced) {
  // not maintaining any GDF compatibility, all files should be where the
  // m_filePath says
  return m_fileExists;
}
...
/** Is the file open. */
//-------------------------------------------------------------------------------------------------
bool GameFileClass::Is_Open(void) const { return m_theFile != NULL; }
...
//-------------------------------------------------------------------------------------------------
void W3DFileSystem::Return_File(FileClass *file) { delete file; }
