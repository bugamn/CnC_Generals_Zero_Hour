...
void W3DCustomEdging::loadEdgingsInVertexAndIndexBuffers(WorldHeightMap *pMap,
  ...
  for (row = minY; row < maxY - 1; row++) {
    for (column = minX; column < maxX - 1; column++) {
      ...
      if (pMap->m_blendedTiles[blend].customBlendEdgeClass < 0)
        continue;  // alpha blend.
      Int i, j;
      Real uOffset;
      Real vOffset;
      ...
      Region2D range;
      pMap->getUVForBlend(pMap->m_blendedTiles[blend].customBlendEdgeClass,
                          &range);
      ...
      float UA[4], VA[4];
      Bool flipForBlend;
      pMap->getAlphaUVData(column - pMap->getDrawOrgX(),
                           row - pMap->getDrawOrgY(), UA, VA, alpha,
                           &flipForBlend, false);
      ...
      for (j = 0; j < 2; j++) {
        for (i = 0; i < 2; i++) {
          ...
          Int diffuse =
              TheTerrainRenderObject->getStaticDiffuse(column + i, row + j);
          curVb->diffuse =
              0x80000000 + (diffuse & 0x00FFFFFF);  // set alpha to 5b.
          Real theZ;
...
//=============================================================================
W3DCustomEdging::~W3DCustomEdging(void) { freeEdgingBuffers(); }
...
void W3DCustomEdging::allocateEdgingBuffers(void) {
  m_vertexEdging =
      NEW_REF(DX8VertexBufferClass, (DX8_FVF_XYZDUV2, MAX_EDGE_VERTEX + 4,
                                     DX8VertexBufferClass::USAGE_DYNAMIC));
  m_indexEdging =
      NEW_REF(DX8IndexBufferClass,
              (2 * MAX_EDGE_INDEX + 4, DX8IndexBufferClass::USAGE_DYNAMIC));
  m_curNumEdgingVertices = 0;
  m_curNumEdgingIndices = 0;
  // m_edgeTexture = MSGNEW("TextureClass")
...
//=============================================================================
void W3DCustomEdging::drawEdging(WorldHeightMap *pMap, Int minX, Int maxX,
                                 Int minY, Int maxY,
  ...
  DX8Wrapper::Set_DX8_Render_State(
      D3DRS_ALPHAFUNC,
      D3DCMP_LESSEQUAL);  // pass pixels who's alpha is not zero
  DX8Wrapper::Set_DX8_Render_State(
      D3DRS_ALPHATESTENABLE,
      true);  // test pixels if transparent(clipped) before rendering.
  DX8Wrapper::Draw_Triangles(m_curEdgingIndexOffset, m_curNumEdgingIndices / 3,
                             0, m_curNumEdgingVertices);
  ...
  DX8Wrapper::Set_DX8_Render_State(
      D3DRS_ALPHAFUNC,
      D3DCMP_GREATEREQUAL);  // pass pixels who's alpha is not zero
  DX8Wrapper::Set_DX8_Render_State(
      D3DRS_ALPHATESTENABLE,
      true);  // test pixels if transparent(clipped) before rendering.
  DX8Wrapper::Draw_Triangles(m_curEdgingIndexOffset, m_curNumEdgingIndices / 3,
  ...
  if (cloudTexture) {
    ...
    DX8Wrapper::Draw_Triangles(m_curEdgingIndexOffset,
                               m_curNumEdgingIndices / 3, 0,
                               m_curNumEdgingVertices);
  }
  if (noiseTexture) {
    DX8Wrapper::Set_Texture(1, NULL);
