...
#define SC_DETAIL_BLEND                                                      \
  (SHADE_CNST(ShaderClass::PASS_LEQUAL, ShaderClass::DEPTH_WRITE_ENABLE,     \
              ...
              ShaderClass::CULL_MODE_ENABLE, ShaderClass::DETAILCOLOR_SCALE, \
...
/** Entry point so that trees can be drawn at the appropriate point in the
   rendering pipe for transparent objects. */
...
#define DEFAULT_MAX_FRAME_EXTRABLEND_TILES \
  256  // default number of terrain tiles rendered per call (must fit in one VB)
#define DEFAULT_MAX_MAP_EXTRABLEND_TILES \
  2048  // default size of array allocated to hold all map extra blend tiles.
#define DEFAULT_MAX_BATCH_SHORELINE_TILES \
  512  // maximum number of terrain tiles rendered per call (must fit in one VB)
#define DEFAULT_MAX_MAP_SHORELINE_TILES \
  4096  // default size of array allocated to hold all map shoreline tiles.

#define ADJUST_FROM_INDEX_TO_REAL(k) \
  ((k - m_map->getBorderSize()) * MAP_XY_FACTOR)
...
//=============================================================================
/** Calculates the diffuse lighting for a vertex in the terrain, taking all of
the static lights into account as well.  It is possible to just use the normal
in the vertex and let D3D do the lighting, but it is slower to render, and can
only handle 4 lights at this point. */
//=============================================================================
void HeightMapRenderObjClass::doTheLight(
    VERTEX_FORMAT *vb, Vector3 *light, Vector3 *normal,
    RefRenderObjListIterator *pLightsIterator, UnsignedByte alpha) {
#ifdef USE_NORMALS
  vb->nx = normal->X;
  vb->ny = normal->Y;
  vb->nz = normal->Z;
  ...
  Real shade;
  shadeR = TheGlobalData->m_terrainAmbient[0]
               .red;  // only the first terrain light contributes to ambient
      ...
      switch (pLight->Get_Type()) {
        ...
        case LightClass::SPOT: {
          ...
          if (fabs(range - midRange) < 1e-5) {
            ...
            factor = 1.0f / (0.1 + dist / midRange +
                             5.0f * dist * dist / (range * range));
          }
#endif
  ...
  for (Int lightIndex = 0; lightIndex < TheGlobalData->m_numGlobalLights;
       lightIndex++) {
  ...
  if (m_useDepthFade &&
      vb->z <=
          TheGlobalData->m_waterPositionZ) {  // height is below water level
    // reduce lighting values based on light fall off as it travels through
    // water.
  ...
  shadeG *= 255.0f;
  shadeB *= 255.0f;
  vb->diffuse = REAL_TO_INT(shadeB) | (REAL_TO_INT(shadeG) << 8) |
                (REAL_TO_INT(shadeR) << 16) | ((Int)alpha << 24);
#endif
}
...
//=============================================================================
UnsignedInt HeightMapRenderObjClass::doTheDynamicLight(
    VERTEX_FORMAT *vb, VERTEX_FORMAT *vbMirror, Vector3 *light, Vector3 *normal,
    W3DDynamicLight *pLights[], Int numLights) {
#ifdef USE_NORMALS
  return;
#else
  Real shadeR, shadeG, shadeB;
...
#ifdef _DEBUG
  // vbMirror->diffuse += 30;	// Shows which vertexes are geting touched by
  // dynamic light. debug only.
#endif

  // (gth) avoiding the extra divides (compiler unfortunately didn't do this
  // automatically...)
  ...
  //	(gth) faster float to int conversion, return the result so we can re-use
  //it. 	vb->diffuse=REAL_TO_INT(shadeB) | (REAL_TO_INT(shadeG) << 8) |
  //(REAL_TO_INT(shadeR) << 16) | ((int)alpha << 24);
  UnsignedInt light_val = WWMath::Float_To_Int_Chop(shadeB) |
                          (WWMath::Float_To_Int_Chop(shadeG) << 8) |
                          (WWMath::Float_To_Int_Chop(shadeR) << 16) |
                          ((int)alpha << 24);
...
Int HeightMapRenderObjClass::updateVB(
    DX8VertexBufferClass *pVB, char *data, Int x0, Int y0, Int x1, Int y1,
  ...
  REF_PTR_SET(m_map, pMap);  // update our heightmap pointer in case it changed
                             // since last call.
  if (m_vertexBufferTiles && pMap) {
#ifdef _DEBUG
    assert(x0 >= originX && y0 >= originY && x1 > x0 && y1 > y0 &&
    ...
    VERTEX_FORMAT *vbHardware =
        (VERTEX_FORMAT *)lockVtxBuffer.Get_Vertex_Array();
    VERTEX_FORMAT *vBase = (VERTEX_FORMAT *)data;
    // Note that we are building the vertex buffer data in the memory buffer,
    // data. At the bottom, we will copy the final vertex data for one cell into
    ...
    for (j = y0; j < y1; j++) {
      ...
      if (m_halfResMesh) {
        if (j & 1) continue;
        vb += ((j - originY) / 2) * vertsPerRow /
              2;                         // skip to correct row in vertex buffer
        vb += ((x0 - originX) / 2) * 4;  // skip to correct vertex in row.
      } else {
        vb += (j - originY) *
              vertsPerRow;         // skip to correct row in vertex buffer
        vb += (x0 - originX) * 4;  // skip to correct vertex in row.
      }
      vn0 = getYWithOrigin(j) - cellOffset;
      if (vn0 < -pMap->getDrawOrgY()) vn0 = -pMap->getDrawOrgY();
      ...
      for (i = x0; i < x1; i++) {
        ...
        if (un0 < -pMap->getDrawOrgX()) un0 = -pMap->getDrawOrgX();
        ...
        float UA[4], VA[4];
        Bool flipForBlend =
            false;  // True if the blend needs the triangles flipped.

        if (pMap) {
          pMap->getUVData(getXWithOrigin(i), getYWithOrigin(j), U, V,
                          m_halfResMesh);
          pMap->getAlphaUVData(getXWithOrigin(i), getYWithOrigin(j), UA, VA,
                               alpha, &flipForBlend, m_halfResMesh);
        }
        ...
        // top-left sample
        l2r.Set(2 * MAP_XY_FACTOR, 0,
                MAP_HEIGHT_SCALE *
                    (pMap->getDisplayHeight(getXWithOrigin(i) + cellOffset,
                                            getYWithOrigin(j)) -
                     pMap->getDisplayHeight(un0, getYWithOrigin(j))));
        n2f.Set(0, 2 * MAP_XY_FACTOR,
                MAP_HEIGHT_SCALE *
                    (pMap->getDisplayHeight(getXWithOrigin(i),
                                            (getYWithOrigin(j) + cellOffset)) -
                     pMap->getDisplayHeight(getXWithOrigin(i), vn0)));
        ...
        vb->x = xCoord;
        vb->y = yCoord;
        vb->z = ((float)pMap->getDisplayHeight(getXWithOrigin(i),
                                               getYWithOrigin(j))) *
                MAP_HEIGHT_SCALE;
        ...
        l2r.Set(
            2 * MAP_XY_FACTOR, 0,
            MAP_HEIGHT_SCALE *
                (pMap->getDisplayHeight(up1, getYWithOrigin(j)) -
                 pMap->getDisplayHeight(getXWithOrigin(i), getYWithOrigin(j))));
        n2f.Set(
            0, 2 * MAP_XY_FACTOR,
            MAP_HEIGHT_SCALE *
                (pMap->getDisplayHeight(getXWithOrigin(i) + cellOffset,
                                        (getYWithOrigin(j) + cellOffset)) -
                 pMap->getDisplayHeight(getXWithOrigin(i) + cellOffset, vn0)));
        ...
        vb->y = yCoord;
        vb->z = ((float)pMap->getDisplayHeight(getXWithOrigin(i) + cellOffset,
                                               getYWithOrigin(j))) *
                MAP_HEIGHT_SCALE;
        vb->x = ADJUST_FROM_INDEX_TO_REAL(vb->x);
        vb->y = ADJUST_FROM_INDEX_TO_REAL(vb->y);
        ...
        l2r.Set(
            2 * MAP_XY_FACTOR, 0,
            MAP_HEIGHT_SCALE *
                (pMap->getDisplayHeight(up1, (getYWithOrigin(j) + cellOffset)) -
                 pMap->getDisplayHeight(getXWithOrigin(i),
                                        (getYWithOrigin(j) + cellOffset))));
        n2f.Set(
            0, 2 * MAP_XY_FACTOR,
            MAP_HEIGHT_SCALE *
                (pMap->getDisplayHeight(getXWithOrigin(i) + cellOffset, vp1) -
                 pMap->getDisplayHeight(getXWithOrigin(i) + cellOffset,
                                        getYWithOrigin(j))));
        ...
        vb->z =
            ((float)pMap->getDisplayHeight(getXWithOrigin(i) + cellOffset,
                                           getYWithOrigin(j) + cellOffset)) *
            MAP_HEIGHT_SCALE;
        vb->x = ADJUST_FROM_INDEX_TO_REAL(vb->x);
        vb->y = ADJUST_FROM_INDEX_TO_REAL(vb->y);
        ...
        l2r.Set(2 * MAP_XY_FACTOR, 0,
                MAP_HEIGHT_SCALE *
                    (pMap->getDisplayHeight(getXWithOrigin(i) + cellOffset,
                                            (getYWithOrigin(j) + cellOffset)) -
                     pMap->getDisplayHeight(un0,
                                            (getYWithOrigin(j) + cellOffset))));
        n2f.Set(
            0, 2 * MAP_XY_FACTOR,
            MAP_HEIGHT_SCALE *
                (pMap->getDisplayHeight(getXWithOrigin(i), vp1) -
                 pMap->getDisplayHeight(getXWithOrigin(i), getYWithOrigin(j))));
        ...
        vb->z = ((float)pMap->getDisplayHeight(
                    getXWithOrigin(i), getYWithOrigin(j) + cellOffset)) *
                MAP_HEIGHT_SCALE;
        vb->x = ADJUST_FROM_INDEX_TO_REAL(vb->x);
        vb->y = ADJUST_FROM_INDEX_TO_REAL(vb->y);
        vb->u1 = U[3];
...
#ifdef FLIP_TRIANGLES  // jba - reduces "diamonding" in some cases, not others.
                       // Better cliffs, though.
        ...
        if (m_showImpassableAreas) {
          ...
          DEBUG_ASSERTCRASH(PATHFIND_CELL_SIZE_F == MAP_XY_FACTOR,
                            ("Pathfind must be terrain cell size, or this code "
                             "needs reworking.  John A."));
          Real borderHiX =
              (pMap->getXExtent() - 2 * pMap->getBorderSize()) * MAP_XY_FACTOR;
          Real borderHiY =
              (pMap->getYExtent() - 2 * pMap->getBorderSize()) * MAP_XY_FACTOR;
          Bool border = pCurVertices[0].x == -MAP_XY_FACTOR ||
                        pCurVertices[0].y == -MAP_XY_FACTOR;
          Bool cliffMapped =
              pMap->isCliffMappedTexture(getXWithOrigin(i), getYWithOrigin(j));
          ...
          Bool isCliff =
              pMap->getCliffState(getXWithOrigin(i) + pMap->getDrawOrgX(),
                                  getYWithOrigin(j) + pMap->getDrawOrgY()) ||
              showAsVisibleCliff(getXWithOrigin(i) + pMap->getDrawOrgX(),
                                 getYWithOrigin(j) + pMap->getDrawOrgY());
          ...
          if (isCliff || border || cliffMapped) {
            ...
            for (cellX = 0; cellX < 2; cellX++) {
              for (cellY = 0; cellY < 2; cellY++) {
                ...
                if (border) {
                  Bool doBorder = false;
                  if (pCurVertices[vertex].y >= 0 &&
                      pCurVertices[vertex].y <= borderHiY) {
                    if (pCurVertices[vertex].x == 0 ||
                        pCurVertices[vertex].x == borderHiX) {
                      doBorder = true;
                    }
                  }
                  if (pCurVertices[vertex].x >= 0 &&
                  ...
                  if (doBorder) {
                    pCurVertices[vertex].diffuse &=
                        0xFF0000ff;  // blue with alpha.
                  }
                } else if (isCliff) {
                  pCurVertices[vertex].diffuse &=
                      0xFFFF0000;  // red with alpha.
                }
        ...
        // Note - We have been building the vertex buffer in the memory
        // location. Now copy the set of vertices into the hardware buffer. We
        // don't copy the whole vertex buffer because we often update only a
        // couple of rows and its a lot faster to just copy the ones that
        // change.
...
//=============================================================================
/** Update the dynamic lighting values only in a rectangular block of the given
Vertex Buffer. The vertex locations and texture coords are unchanged.
*/
Int HeightMapRenderObjClass::updateVBForLight(
    DX8VertexBufferClass *pVB, char *data, Int x0, Int y0, Int x1, Int y1,
    Int originX, Int originY, W3DDynamicLight *pLights[], Int numLights) {
#if (OPTIMIZED_HEIGHTMAP_LIGHTING)  // (gth) if optimizations are enabled, jump
                                    // over to the "optimized" version of this
                                    // function.
  return updateVBForLightOptimized(pVB, data, x0, y0, x1, y1, originX, originY,
                                   pLights, numLights);
#endif
  ...
  if (m_vertexBufferTiles && m_map) {
    ...
    for (j = y0; j < y1; j++) {
      ...
      Int yCoord =
          getYWithOrigin(j) + m_map->getDrawOrgY() - m_map->getBorderSize();
      Bool intersect = false;
      for (k = 0; k < numLights; k++) {
        if (pLights[k]->m_minY <= yCoord + 1 && pLights[k]->m_maxY >= yCoord) {
          intersect = true;
        }
      ...
      if (vn0 < -m_map->getDrawOrgY()) vn0 = -m_map->getDrawOrgY();
      ...
      for (i = x0; i < x1; i++) {
        ...
        Int xCoord =
            getXWithOrigin(i) + m_map->getDrawOrgX() - m_map->getBorderSize();
        Bool intersect = false;
        for (k = 0; k < numLights; k++) {
        ...
        // The important point is that we can read out of our copy to get the
        // original diffuse color, and xyz location.  It is VERY SLOW to read
        // out of the hardware vertex buffer, possibly worse... jba.
        VERTEX_FORMAT *vbMirror = ((VERTEX_FORMAT *)data) + offset;
        un0 = getXWithOrigin(i) - 1;
        if (un0 < -m_map->getDrawOrgX()) un0 = -m_map->getDrawOrgX();
        ...
        l2r.Set(2 * MAP_XY_FACTOR, 0,
                MAP_HEIGHT_SCALE *
                    (m_map->getDisplayHeight(getXWithOrigin(i) + 1,
                                             getYWithOrigin(j)) -
                     m_map->getDisplayHeight(un0, getYWithOrigin(j))));
        n2f.Set(0, 2 * MAP_XY_FACTOR,
                MAP_HEIGHT_SCALE *
                    (m_map->getDisplayHeight(getXWithOrigin(i),
                                             (getYWithOrigin(j) + 1)) -
                     m_map->getDisplayHeight(getXWithOrigin(i), vn0)));
        ...
        doTheDynamicLight(vb, vbMirror, &lightRay, &normalAtTexel, pLights,
                          numLights);
        vb++;
        vbMirror++;
        ...
        l2r.Set(2 * MAP_XY_FACTOR, 0,
                MAP_HEIGHT_SCALE *
                    (m_map->getDisplayHeight(up1, getYWithOrigin(j)) -
                     m_map->getDisplayHeight(getXWithOrigin(i),
                                             getYWithOrigin(j))));
        n2f.Set(0, 2 * MAP_XY_FACTOR,
                MAP_HEIGHT_SCALE *
                    (m_map->getDisplayHeight(getXWithOrigin(i) + 1,
                                             (getYWithOrigin(j) + 1)) -
                     m_map->getDisplayHeight(getXWithOrigin(i) + 1, vn0)));
        ...
        l2r.Set(2 * MAP_XY_FACTOR, 0,
                MAP_HEIGHT_SCALE *
                    (m_map->getDisplayHeight(up1, (getYWithOrigin(j) + 1)) -
                     m_map->getDisplayHeight(getXWithOrigin(i),
                                             (getYWithOrigin(j) + 1))));
        n2f.Set(0, 2 * MAP_XY_FACTOR,
                MAP_HEIGHT_SCALE *
                    (m_map->getDisplayHeight(getXWithOrigin(i) + 1, vp1) -
                     m_map->getDisplayHeight(getXWithOrigin(i) + 1,
                                             getYWithOrigin(j))));
        ...
        l2r.Set(2 * MAP_XY_FACTOR, 0,
                MAP_HEIGHT_SCALE *
                    (m_map->getDisplayHeight(getXWithOrigin(i) + 1,
                                             (getYWithOrigin(j) + 1)) -
                     m_map->getDisplayHeight(un0, (getYWithOrigin(j) + 1))));
        n2f.Set(0, 2 * MAP_XY_FACTOR,
                MAP_HEIGHT_SCALE *
                    (m_map->getDisplayHeight(getXWithOrigin(i), vp1) -
                     m_map->getDisplayHeight(getXWithOrigin(i),
                                             getYWithOrigin(j))));
...
Int HeightMapRenderObjClass::updateVBForLightOptimized(
    DX8VertexBufferClass *pVB, char *data, Int x0, Int y0, Int x1, Int y1,
    Int originX, Int originY, W3DDynamicLight *pLights[], Int numLights) {
  Int i, j, k;
  Int vn0, un0, vp1, up1;
  ...
  if (m_vertexBufferTiles && m_map) {
    ...
    //
    // (gth) the optimization in this function is to take advantage of verts in
    // the same x,y position who have already computed their lighting.  To do
    // this, we need to set up some offsets in the vertex buffer.  I've computed
    // these offsets to be consistent with the formula's that Generals is using
    // but in the case of the "half-res-mesh" I'm not sure things are correct...
    //
    ...
    //
    // i,j loop over the quads affected by the light.  Each quad has its *own* 4
    // vertices.  This means that for any vertex position on the map, there are
    // actually 4 copies of the vertex.
    //
    for (j = y0; j < y1; j++) {
      if (m_halfResMesh) {
        if (j & 1) continue;
      ...
      for (i = x0; i < x1; i++) {
        ...
        //
        // (gth) Following the set of rules below lets us take advantage of
        // lighting values that have been previously computed.  The idea is to
        // copy them ahead to future quads that will need them and then not
        // compute them when we get to those quads.  This also avoids having to
        // read-back from the vertex buffer but we do jump around in memory...
        // probably bad anyway, maybe we should compute into a temporary buffer
        // and copy all at once...
        //
          ...
          n2f.Set(0, 2 * MAP_XY_FACTOR,
                  MAP_HEIGHT_SCALE *
                      (m_map->getDisplayHeight(getXWithOrigin(i) + 1,
                       ...
                       m_map->getDisplayHeight(getXWithOrigin(i) + 1, vn0)));
          Vector3::Normalized_Cross_Product(l2r, n2f, &normalAtTexel);
          light_copy = doTheDynamicLight(vb, vbMirror, &lightRay,
                                         &normalAtTexel, pLights, numLights);

          if (i < x1 - 1) {
            // copy light to (right,0)
            (vBase + offset + quad_right_offset)->diffuse =
                (light_copy & 0x00FFFFFF) |
                ((vbaseMirror + offset + quad_right_offset)->diffuse &
                 0xff000000);
          }
        }
        vb++;
        ...
        // bottom-right sample -> always compute, then copy to (right,3),
        // (down,1), (down+right,0)
        l2r.Set(2 * MAP_XY_FACTOR, 0,
                MAP_HEIGHT_SCALE *
                    (m_map->getDisplayHeight(up1, (getYWithOrigin(j) + 1)) -
                ...
                MAP_HEIGHT_SCALE *
                    (m_map->getDisplayHeight(getXWithOrigin(i) + 1, vp1) -
                     m_map->getDisplayHeight(getXWithOrigin(i) + 1,
                                             getYWithOrigin(j))));
        Vector3::Normalized_Cross_Product(l2r, n2f, &normalAtTexel);
        light_copy = doTheDynamicLight(vb, vbMirror, &lightRay, &normalAtTexel,
                                       pLights, numLights);

        if (i < x1 - 1) {
          // copy light to (right,3)
          //(vBase + offset + quad_right_offset + 3)->diffuse = light_copy;
          (vBase + offset + quad_right_offset + 3)->diffuse =
              (light_copy & 0x00FFFFFF) |
              ((vbaseMirror + offset + quad_right_offset + 3)->diffuse &
               0xff000000);
        }
        if (j < y1 - 1) {
          // copy light to (down,1)
          //(vBase + offset + quad_below_offset + 1)->diffuse = light_copy;
          (vBase + offset + quad_right_offset + 1)->diffuse =
              (light_copy & 0x00FFFFFF) |
              ((vbaseMirror + offset + quad_right_offset + 1)->diffuse &
               0xff000000);
        }
        if ((i < x1 - 1) && (j < y1 - 1)) {
        ...
        if (i == x0) {
          ...
          if (j < y1 - 1) {
            ...
            (vBase + offset + quad_below_offset)->diffuse =
                (light_copy & 0x00FFFFFF) |
                ((vbaseMirror + offset + quad_below_offset)->diffuse &
                 0xff000000);
          }
        }
        vb++;
...
The coordinates in partialRange are map cell coordinates, relative to the entire
map. The vertex coordinates and texture coordinates, as well as static lighting
are updated.
*/
void HeightMapRenderObjClass::doPartialUpdate(
    const IRegion2D &partialRange, WorldHeightMap *htMap,
    RefRenderObjListIterator *pLightsIterator) {
  ...
  if (m_originX == 0 && m_originY == 0) {
    // simple case.
    updateBlock(minX, minY, maxX, maxY, htMap, pLightsIterator);
  } else {
    minY = minY + m_originY;
    maxY = maxY + m_originY;
  ...
  if (!m_extraBlendTilePositions) {  // Need to allocate memory
  ...
  // Find list of all extra blend tiles used on map.  These are tiles with 3
  // materials/textures over the same tile and require an extra render pass.
  ...
  for (j = 0; j < m_numExtraBlendTiles; j++) {
    ...
    if (x >= partialRange.lo.x && x < partialRange.hi.x &&
        y >= partialRange.lo.y &&
        y < partialRange.hi.y) {  // this tile is inside region being updated so
                                  // remove it by shifting tile array
      memcpy(m_extraBlendTilePositions + j, m_extraBlendTilePositions + j + 1,
             (m_numExtraBlendTiles - 1 - j) * sizeof(Int));
  ...
  for (j = partialRange.lo.y; j < partialRange.hi.y; j++)
    for (i = partialRange.lo.x; i < partialRange.hi.x; i++) {
      ...
      if (htMap->getExtraAlphaUVData(i, j, U, V, alpha, &flipState,
                                     &cliffState)) {
        if (m_numExtraBlendTiles >=
            m_extraBlendTilePositionsSize) {  // no more room to store extra
                                              // blend tiles so enlarge the
                                              // buffer.
          Int *tempPositions = NEW Int[m_extraBlendTilePositionsSize + 512];
          memcpy(tempPositions, m_extraBlendTilePositions,
                 m_extraBlendTilePositionsSize * sizeof(Int));
        ...
        // Pack x and y position into single integer since maps are limited in
        // size
        m_extraBlendTilePositions[m_numExtraBlendTiles] = i | (j << 16);
        m_numExtraBlendTiles++;
      }
    }
  updateShorelineTiles(partialRange.lo.x, partialRange.lo.y, partialRange.hi.x,
                       partialRange.hi.y, htMap);
...
The vertex coordinates and texture coordinates, as well as static lighting are
updated.
*/
Int HeightMapRenderObjClass::updateBlock(
    Int x0, Int y0, Int x1, Int y1, WorldHeightMap *pMap,
    RefRenderObjListIterator *pLightsIterator) {
#ifdef _DEBUG
  DEBUG_ASSERTCRASH(
      x0 >= 0 && y0 >= 0 && x1 < m_x && y1 < m_y && x0 <= x1 && y0 <= y1,
      ("Invalid updates."));
#endif
  ...
  for (j = 0; j < m_numVBTilesY; j++) {
    originY = j * VERTEX_BUFFER_TILE_LENGTH;  // location of this VB on the
                                              // large full-size heightmap
    Int yMin, yMax;
    yMin = originY;
    if (y0 > yMin) yMin = y0;
    ...
    for (i = 0; i < m_numVBTilesX; i++) {
      originX = i * VERTEX_BUFFER_TILE_LENGTH;  // location of this VB on the
                                                // large full-size heightmap
      Int xMin, xMax;
      xMin = originX;
      if (xMin < x0) xMin = x0;
      ...
      pVB = m_vertexBufferTiles + j * m_numVBTilesX +
            i;  // point to correct row/column of vertex buffers
...
//=============================================================================
/** Calculate the bounding box for given terrain tile.  Right now each terrain
tile is equivalent to one of the vertex buffers holding heightmap vertices. */
//=============================================================================
AABoxClass &HeightMapRenderObjClass::getTileBoundingBox(AABoxClass *aabox,
                                                        Int x, Int y) {
  Vector3 vmin;
  Vector3 vmax;
  Int xOffset = 0;
  ...
  vmin.Set(x * VERTEX_BUFFER_TILE_LENGTH + xOffset,
           y * VERTEX_BUFFER_TILE_LENGTH + yOffset, m_minHeight);
  vmax.Set((x + 1) * VERTEX_BUFFER_TILE_LENGTH + xOffset,
           (y + 1) * VERTEX_BUFFER_TILE_LENGTH + yOffset, m_maxHeight);
...
void HeightMapRenderObjClass::drawScorches(void) {
  ...
  if (Is_Hidden() == 0) {
    DX8Wrapper::Draw_Triangles(0, m_curNumScorchIndices / 3, 0,
...
HeightMapRenderObjClass::~HeightMapRenderObjClass(void) {
  ...
  if (m_extraBlendTilePositions) delete[] m_extraBlendTilePositions;
  if (m_shoreLineTilePositions) delete[] m_shoreLineTilePositions;
}
...
void HeightMapRenderObjClass::adjustTerrainLOD(Int adj) {
  if (adj > 0 && TheGlobalData->m_terrainLOD < TERRAIN_LOD_MAX)
    TheWritableGlobalData->m_terrainLOD =
        (TerrainLOD)(TheGlobalData->m_terrainLOD + 1);
  if (adj < 0 && TheGlobalData->m_terrainLOD > TERRAIN_LOD_MIN)
    TheWritableGlobalData->m_terrainLOD =
        (TerrainLOD)(TheGlobalData->m_terrainLOD - 1);
  ...
  switch (TheGlobalData->m_terrainLOD) {
    ...
    case TERRAIN_LOD_HALF_CLOUDS:
      ...
      TheWritableGlobalData->m_useHalfHeightMap = true;
      break;
    case TERRAIN_LOD_STRETCH_NO_CLOUDS:
      TheWritableGlobalData->m_useCloudMap = false;
      TheWritableGlobalData->m_useLightMap = false;
      TheWritableGlobalData->m_useWaterPlane = false;
      ...
      break;
    case TERRAIN_LOD_STRETCH_CLOUDS:
      TheWritableGlobalData->m_useCloudMap = true;
      TheWritableGlobalData->m_useLightMap = true;
      TheWritableGlobalData->m_useWaterPlane = false;
      TheWritableGlobalData->m_stretchTerrain = true;
  ...
  if (m_shroud)
    m_shroud->reset();  // need reset here since initHeightData will load new
                        // shroud.
  this->initHeightData(m_map->getDrawWidth(), m_map->getDrawHeight(), m_map,
                       NULL);
...
void HeightMapRenderObjClass::ReleaseResources(void) {
  ...
  if (TheWaterRenderObj) TheWaterRenderObj->ReleaseResources();
  if (TheTerrainTracksRenderObjClassSystem)
    TheTerrainTracksRenderObjClassSystem->ReleaseResources();
  if (TheW3DShadowManager) TheW3DShadowManager->ReleaseResources();
  if (m_shroud) {
    m_shroud->reset();
...
void HeightMapRenderObjClass::ReAcquireResources(void) {
  W3DShaderManager::init();  // reaquire resources which may be needed by custom
                             // shaders

  if (TheWaterRenderObj) TheWaterRenderObj->ReAcquireResources();

  if (TheTerrainTracksRenderObjClassSystem)
    TheTerrainTracksRenderObjClassSystem->ReAcquireResources();

  if (TheW3DShadowManager) TheW3DShadowManager->ReAcquireResources();
  if (m_shroud) m_shroud->ReAcquireResources();
  ...
  // Waypoint buffers are done dynamically. One line, one node (just rendered
  // multiple times accessing other data). Internally creates it if needed.
  ...
  if (TheTacticalView) {
    TheTacticalView->forceRedraw();  // force map to update itself for the
                                     // current camera position.
    // for some reason we need to do it twice otherwise we sometimes end up with
    // a black map until the player moves.
...
void HeightMapRenderObjClass::reset(void) {
  ...
  if (m_shroud) {
    m_shroud->reset();
    m_shroud->setBorderShroudLevel(
        (W3DShroudLevel)TheGlobalData
            ->m_shroudAlpha);  // assume border is always black at start.
  }
}
...
bool HeightMapRenderObjClass::Cast_Ray(RayCollisionTestClass &raytest) {
  ...
  if (!m_map) return false;  // need valid pointer to heightmap samples
  ...
  Int StartCellY = 0;
  Int EndCellY = 0;
  const Int overhang = 2 * VERTEX_BUFFER_TILE_LENGTH +
                       m_map->getBorderSize();  // Allow picking past the edge
                                                // for scrolling & objects.
  Vector3 minPt(MAP_XY_FACTOR * (-overhang), MAP_XY_FACTOR * (-overhang),
                -MAP_XY_FACTOR);
  Vector3 maxPt(MAP_XY_FACTOR * (m_map->getXExtent() + overhang),
                MAP_XY_FACTOR * (m_map->getYExtent() + overhang),
                MAP_HEIGHT_SCALE * m_map->getMaxHeightValue() + MAP_XY_FACTOR);
  ...
  for (p = 0; p < 3; p++) {
    ...
    result.Reset();
    result.ComputeContactPoint = true;
    if (CollisionMath::Collide(
            lineseg, hbox,
            &result)) {  // ray intersects terrain or starts inside the terrain.
      if (!result.StartBad)        // check if start point inside terrain
        P0 = result.ContactPoint;  // make intersection point the new start of
                                   // the ray.
      ...
      if (CollisionMath::Collide(lineseg2, hbox, &result)) {
        if (!result.StartBad)        // check if end point inside terrain
          P1 = result.ContactPoint;  // make intersection point the new end pont
                                     // of ray
      }
    } else {
      if (p == 0) return (false);
    ...
    for (j = StartCellY; j <= EndCellY; j++) {
      for (i = StartCellX; i <= EndCellX; i++) {
        Short cur = getClipHeight(i + m_map->getBorderSize(),
                                  j + m_map->getBorderSize());
        if (cur < minHt) minHt = cur;
        if (maxHt < cur) maxHt = cur;
      }
    }
    Vector3 minPt(MAP_XY_FACTOR * (StartCellX - 1),
                  MAP_XY_FACTOR * (StartCellY - 1),
                  MAP_HEIGHT_SCALE * (minHt - 1));
    Vector3 maxPt(MAP_XY_FACTOR * (EndCellX + 1),
                  MAP_XY_FACTOR * (EndCellY + 1),
                  MAP_HEIGHT_SCALE * (maxHt + 1));
    MinMaxAABoxClass mmbox(minPt, maxPt);
    hbox.Init(mmbox);
  }

  raytest.Result->ComputeContactPoint =
      true;  // tell CollisionMath that we need point.
  ...
  for (offset = 1; offset < 5; offset *= 3) {
    for (Y = StartCellY - offset; Y <= EndCellY + offset; Y++) {
      ...
      for (X = StartCellX - offset; X <= EndCellX + offset; X++) {
        ...
        hit = hit ||
              (Bool)CollisionMath::Collide(raytest.Ray, tri, raytest.Result);
        ...
        if (hit)
          raytest.Result->SurfaceType =
              SURFACE_TYPE_DEFAULT;  ///@todo: WW3D uses this to return dirt,
                                     ///grass, etc.  Do we need this?
      }
      // Don't break.  It is possible to intersect 2 triangles, and the second
      // is closer. if (hit) break;
    }
    // Don't break.  It is possible to intersect 2 triangles, and the second is
    // closer. if (hit) break;
  }
  return hit;
}
...
//=============================================================================
/** return the height and normal of the triangle plane containing given location
 * within heightmap. */
//=============================================================================
Real HeightMapRenderObjClass::getHeightMapHeight(Real x, Real y,
  ...
  // since ixf & iyf are already floor'ed, we can use the fastest f->i
  // conversion we have...
  ...
  // Check for extent-3, not extent-1: we go into the next row/column of data
  // for smoothed triangle points, so extent-1 goes off the end...
  if (ix > (xExtent - 3) || iy > (m_map->getYExtent() - 3) || iy < 1 ||
      ix < 1) {
...
//=============================================================================
Bool HeightMapRenderObjClass::isClearLineOfSight(
    const Coord3D &pos, const Coord3D &posOther) const {
  if (m_map == NULL) return false;  // doh. should not happen.

#define DO_BRESENHAM
#ifdef DO_BRESENHAM

  /*
          this is WAY faster, though not quite as accurate... however, the
     inaccuracy is pretty minimal, so we really should force other code to live
     with it. (srj)
  */
  const Real MAP_XY_FACTOR_INV = 1.0f / MAP_XY_FACTOR;
  ...
  for (Int curpixel = 0; curpixel < numpixels; curpixel++) {
    if (x < 0 || y < 0 || x >= xExtent - 1 || y >= yExtent - 1) {
    ...
    // we're above the max height of the terrain and still looking up, so we're
    // done. (don't bother for reverse test, since that doesn't generally
    // happen)
...
Real HeightMapRenderObjClass::getMaxCellHeight(Real x, Real y) const {
  ...
  if (m_map == NULL) {  // sample point is not on the heightmap
  ...
  p2 = data[(iX + offset) + (iY + offset) * m_map->getXExtent()] *
       MAP_HEIGHT_SCALE;
...
//=============================================================================
//=============================================================================
Bool HeightMapRenderObjClass::evaluateAsVisibleCliff(
    Int xIndex, Int yIndex, Real valuesGreaterThanRad) {
  ...
  // Note: getHeight will protect us from going out of bounds by returning 0 if
  // we give it a value outside of its bounds.
...
void HeightMapRenderObjClass::oversizeTerrain(Int tilesToOversize) {
  ...
  if (tilesToOversize > 0 && tilesToOversize < 5) {
    ...
    if (width > m_map->getXExtent()) width = m_map->getXExtent();
    if (height > m_map->getYExtent()) height = m_map->getYExtent();
  }
...
//=============================================================================
void HeightMapRenderObjClass::Get_Obj_Space_Bounding_Sphere(
    SphereClass &sphere) const {
  Vector3 ObjSpaceCenter(
      (float)m_x * 0.5f * MAP_XY_FACTOR, (float)m_y * 0.5f * MAP_XY_FACTOR,
      (float)m_minHeight + (m_maxHeight - m_minHeight) * 0.5f);
...
//=============================================================================
void HeightMapRenderObjClass::Get_Obj_Space_Bounding_Box(
    AABoxClass &box) const {
  Vector3 minPt(0, 0, m_minHeight);
  Vector3 maxPt((float)m_x * MAP_XY_FACTOR, (float)m_y * MAP_XY_FACTOR,
                (float)m_maxHeight);
  if (m_map) {
...
/** Get the 3D extent of the terrain visible through the camera.  Return value
        is false if no part of terrain is visible.  This function returns a
   worse case bounding volume based on lowest/highest points in entire terrain.
   It does not optimize the volume to heights actually visible.  Unlike some of
        the other methods, this function is guaranteed not to miss any visible
        polygons.  The ignoreMaxHeight flag is used to return a box that uses
   the camera position as the maximum height instead of the terrain - good for
   getting a volume enclosing things that can float above terrain.
 */
//-------------------------------------------------------------------------------------------------
Bool HeightMapRenderObjClass::getMaximumVisibleBox(const FrustumClass &frustum,
                                                   AABoxClass *box,
  ...
  for (Int i = 0; i < 4; i++) {
    ...
    if (groundPlane.Compute_Intersection(
            frustum.Corners[i], frustum.Corners[i + 4],
            &clipFraction)) {  // edge intersects the terrain
      ClippedCorners[i + 4] =
          frustum.Corners[i] +
          (frustum.Corners[i + 4] - frustum.Corners[i]) * clipFraction;
    } else
      ClippedCorners[i + 4] = frustum.Corners[i + 4];
  }

  if (box) box->Init(ClippedCorners, 8);
...
//=============================================================================
/** returns the class id, so the scene can tell what kind of render object it
 * has. */
//=============================================================================
...
//=============================================================================
void HeightMapRenderObjClass::loadRoadsAndBridges(
    W3DTerrainLogic *pTerrainLogic, Bool saveGame) {
...
// ============================================================================
/** The worldbuilder has it's own method here to update the visual
 * representation of the bridge towers */
// ============================================================================
void HeightMapRenderObjClass::worldBuilderUpdateBridgeTowers(
    W3DAssetManager *assetManager, SimpleSceneClass *scene) {
  if (m_bridgeBuffer)
...
/**Scan through our map and record all tiles which cross a water plane and are
within visible depth under water.*/
void HeightMapRenderObjClass::updateShorelineTiles(Int minX, Int minY, Int maxX,
                                                   Int maxY,
                                                   WorldHeightMap *pMap) {
  ...
  if (minX < 0) minX = 0;
  if (minY < 0) minY = 0;
  if (maxX > (pMap->getXExtent() - 1)) maxX = (pMap->getXExtent() - 1);
  if (maxY > (pMap->getYExtent() - 1)) maxY = (pMap->getYExtent() - 1);

  if (!m_shoreLineTilePositions) {  // Need to allocate memory
    m_shoreLineTilePositions =
        NEW shoreLineTileInfo[DEFAULT_MAX_MAP_SHORELINE_TILES];
  ...
  for (Int j = 0; j < m_numShoreLineTiles; j++) {
    ...
    if (x >= minX && x < maxX && y >= minY &&
        y < maxY) {  // this tile is inside region being updated so remove it by
                     // shifting tile array
      memcpy(m_shoreLineTilePositions + j, m_shoreLineTilePositions + j + 1,
             (m_numShoreLineTiles - 1 - j) * sizeof(shoreLineTileInfo));
      m_numShoreLineTiles--;
      j--;  // look at current tile again since it was removed.
    }
  }

  if (TheWaterTransparency->m_transparentWaterDepth == 0 ||
  ...
  // Figure out maximum depth of water before we reach the m_minWaterOpacity
  // value.  Depths greater than this don't need custom shoreline tiles because
  // they will get their opacity from the default value stored in the frame
  // buffer during a screen clear operation.
  Real transparentDepth = TheWaterTransparency->m_transparentWaterDepth *
  ...
  for (j = minY; j < maxY; j++)
    for (Int i = minX; i < maxX; i++) {
      waterSide =
          (waterZ0 = TheWaterRenderObj->getWaterHeight(
               (i - border) * MAP_XY_FACTOR, (j - border) * MAP_XY_FACTOR)) >
          ((terrainZ0 = MAP_HEIGHT_SCALE * pMap->getHeight(i, j)));
      waterSide |=
          ((waterZ1 = TheWaterRenderObj->getWaterHeight(
                (i - border + 1) * MAP_XY_FACTOR,
                (j - border) * MAP_XY_FACTOR)) >
           ((terrainZ1 = MAP_HEIGHT_SCALE * pMap->getHeight(i + 1, j))))
          << 1;
      waterSide |=
          ((waterZ2 = TheWaterRenderObj->getWaterHeight(
                (i - border + 1) * MAP_XY_FACTOR,
                (j - border + 1) * MAP_XY_FACTOR)) >
           ((terrainZ2 = MAP_HEIGHT_SCALE * pMap->getHeight(i + 1, j + 1))))
          << 2;
      waterSide |=
          ((waterZ3 = TheWaterRenderObj->getWaterHeight(
                (i - border) * MAP_XY_FACTOR,
                (j - border + 1) * MAP_XY_FACTOR)) >
           ((terrainZ3 = MAP_HEIGHT_SCALE * pMap->getHeight(i, j + 1))))
          << 3;

      if (!waterSide || (waterZ0 * waterZ1 * waterZ2 * waterZ3) <= 0)
        continue;  // all verts are on positive (surface) side of water so don't
                   // need blending.  Or one of them is outside the water plane
                   // bounds (waterHeight <= 0!)

      // Check if mix of under/over water vertices or some vertices within depth
      // fade region.
      if (waterSide < 0xf || (waterZ0 - terrainZ0) < transparentDepth ||
          (waterZ1 - terrainZ1) < transparentDepth ||
          (waterZ2 - terrainZ2) < transparentDepth ||
          (waterZ3 - terrainZ3) <
              transparentDepth) {  // add tile to set that needs shoreline
                                   // blending.
        if (m_numShoreLineTiles >=
            m_shoreLineTilePositionsSize) {  // no more room to store extra
                                             // blend tiles so enlarge the
                                             // buffer.
          shoreLineTileInfo *tempPositions =
              NEW shoreLineTileInfo[m_shoreLineTilePositionsSize + 512];
          memcpy(tempPositions, m_shoreLineTilePositions,
                 m_shoreLineTilePositionsSize * sizeof(shoreLineTileInfo));
        ...
        shoreLineTileInfo *shoreInfo =
            &m_shoreLineTilePositions[m_numShoreLineTiles];
...
void HeightMapRenderObjClass::updateViewImpassableAreas(Bool partial, Int minX,
                                                        Int maxX, Int minY,
                                                        Int maxY) {
  Int xSize = m_map->getXExtent();
  ...
  for (Int j = minY; j < maxY; ++j) {
    for (Int i = minX; i < maxX; ++i) {
      m_showAsVisibleCliff[i + j * xSize] =
...
void HeightMapRenderObjClass::initDestAlphaLUT(void) {
  ...
  if (surf) {
    ...
    if (pData) {
      ...
      for (Int x = 0; x < 256; x++) {
        alpha = x;
        if (alpha > maxOpacity) alpha = maxOpacity;
...
//=============================================================================
Int HeightMapRenderObjClass::initHeightData(
    Int x, Int y, WorldHeightMap *pMap,
    RefRenderObjListIterator *pLightsIterator) {
  Int i, j;
  ...
  if (m_shroud)
    m_shroud->init(m_map, TheGlobalData->m_partitionCellSize,
  ...
  if (pMap) {
    // Find min/max values for all terrain heights, useful for rendering
    // optimization
    ...
    for (j = 0; j < (m_mapDY - 1); j++)
      for (i = 0; i < (m_mapDX - 1); i++) {
        ...
        UnsignedByte alpha[4];
        Bool flipState, cliffState;
        if (pMap->getExtraAlphaUVData(i, j, U, V, alpha, &flipState,
                                      &cliffState)) {
          if (m_numExtraBlendTiles >=
              m_extraBlendTilePositionsSize) {  // no more room to store extra
                                                // blend tiles so enlarge the
  ...
  // If the textures aren't allocated (usually because of a hardware reset) need
  // to allocate.
  if (m_stageOneTexture == NULL) {
    needToAllocate = true;
  }
  if (data && needToAllocate) {  // requested heightmap different from old one.
    // allocate a new one.
    freeMapResources();        // free old data and ib/vb
    REF_PTR_SET(m_map, pMap);  // update our heightmap pointer in case it
                               // changed since last call.
    m_stageTwoTexture = NEW CloudMapTerrainTextureClass;
    m_stageThreeTexture = NEW LightMapTerrainTextureClass(m_macroTextureName);
    m_destAlphaTexture = MSGNEW("TextureClass")
        TextureClass(256, 1, WW3D_FORMAT_A8R8G8B8, TextureClass::MIP_LEVELS_1);
    initDestAlphaLUT();
#ifdef DO_SCORCH
    allocateScorchBuffers();
#endif

    // Create static index buffers.  These will index the vertex buffers holding
    // the map.
    m_indexBuffer = NEW_REF(
        DX8IndexBufferClass,
    ...
    for (j = 0; j < (VERTEX_BUFFER_TILE_LENGTH * VERTEX_BUFFER_TILE_LENGTH * 4);
         j += VERTEX_BUFFER_TILE_LENGTH * 4) {
      for (i = j; i < (j + VERTEX_BUFFER_TILE_LENGTH * 4);
           i += 4)  // 4 vertices per 2x2 block
      {
        ib[0] = i;
        ib[1] = i + 2;
        ib[2] = i + 3;
    ...
    // First round dimensions to next multiple of VERTEX_BUFFER_TILE_LENGTH
    // since that's our blocksize
    m_numVBTilesX = 1;
    ...
    m_numBlockColumnsInLastVB =
        (x - 1) % VERTEX_BUFFER_TILE_LENGTH;  // right border within last VB
    m_numBlockRowsInLastVB =
        (y - 1) % VERTEX_BUFFER_TILE_LENGTH;  // bottom border within last VB
    ...
    Int numVertex =
        VERTEX_BUFFER_TILE_LENGTH * 2 * VERTEX_BUFFER_TILE_LENGTH * 2;

#ifdef PRE_TRANSFORM_VERTEX
    D3DDEVICE_CREATION_PARAMETERS parms;
    DX8Wrapper::_Get_D3D_Device8()->GetCreationParameters(&parms);
    Bool softwareVertexProcessing =
        0 != (parms.BehaviorFlags & D3DCREATE_SOFTWARE_VERTEXPROCESSING);
    if (m_xformedVertexBuffer == NULL && softwareVertexProcessing) {
      m_xformedVertexBuffer =
          NEW IDirect3DVertexBuffer8 *[m_numVertexBufferTiles];
    }
#endif

    for (i = 0; i < m_numVertexBufferTiles; i++) {
#ifdef USE_NORMALS
      m_vertexBufferTiles[i] = NEW_REF(
          DX8VertexBufferClass,
          (DX8_FVF_XYZNUV2, numVertex, DX8VertexBufferClass::USAGE_DEFAULT));
#else
      m_vertexBufferTiles[i] = NEW_REF(
          DX8VertexBufferClass,
          (DX8_VERTEX_FORMAT, numVertex, DX8VertexBufferClass::USAGE_DEFAULT));
#endif
      ...
      if (m_xformedVertexBuffer) {
        DX8Wrapper::_Get_D3D_Device8()->CreateVertexBuffer(
            ...
            D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_TEX2, D3DPOOL_DEFAULT,
    ...
    m_shaderClass =
        detailOpaqueShader;  //		ShaderClass::_PresetOpaqueShader;
  }
...
void HeightMapRenderObjClass::allocateScorchBuffers(void) {
  m_vertexScorch =
      NEW_REF(DX8VertexBufferClass, (DX8_FVF_XYZDUV1, MAX_SCORCH_VERTEX,
  ...
  m_scorchesInBuffer =
      0;  // If we just allocated the buffers, we got no scorches in the buffer.
  m_curNumScorchVertices = 0;
  m_curNumScorchIndices = 0;
#ifdef _DEBUG
...
void HeightMapRenderObjClass::updateScorches(void) {
  ...
  Int diffuse = REAL_TO_INT(shadeB) | (REAL_TO_INT(shadeG) << 8) |
                (REAL_TO_INT(shadeR) << 16) | ((int)255 << 24);
  m_scorchesInBuffer = 0;
  for (curScorch = m_numScorches - 1; curScorch >= 0; curScorch--) {
    ...
    for (j = minY; j < maxY; j++) {
      for (i = minX; i < maxX; i++) {
        ...
        Real theZ;
        theZ = amtToFloat + ((float)getClipHeight(i + m_map->getBorderSize(),
                                                  j + m_map->getBorderSize()) *
                             MAP_HEIGHT_SCALE);
        if (m_halfResMesh) {
        ...
        curVb->u1 = (uOffset + 0.5f + (X - loc.X) / (2 * radius)) /
                    (SCORCH_PER_ROW + 1);
        curVb->v1 = (vOffset + 0.5f + (Y - loc.Y) / (2 * radius)) /
                    (SCORCH_PER_ROW + 1);
        curVb->x = X;
        curVb->y = Y;
        curVb->z = theZ;
...
/** Adds a scorch mark. */
//=============================================================================
void HeightMapRenderObjClass::addScorch(Vector3 location, Real radius,
                                        Scorches type) {
#ifdef DO_SCORCH
...
Int HeightMapRenderObjClass::getStaticDiffuse(Int x, Int y) {
  if (x < 0) x = 0;
  if (y < 0) y = 0;
  if (x >= m_map->getXExtent()) x = m_map->getXExtent() - 1;
  if (y >= m_map->getYExtent()) y = m_map->getYExtent() - 1;
  if (m_halfResMesh) {
    x &= 0xffffffe;
    y &= 0xffffffe;
  }
  ...
  vn0 = y - cellOffset;
  vp1 = y + cellOffset;
  if (vp1 >= m_map->getYExtent()) vp1 = m_map->getYExtent() - 1;
  if (vn0 < 0) vn0 = 0;
  un0 = x - cellOffset;
  up1 = x + cellOffset;
  if (un0 < 0) un0 = 0;
  if (up1 >= m_map->getXExtent()) up1 = m_map->getXExtent() - 1;
  ...
  l2r.Set(
      2 * MAP_XY_FACTOR, 0,
      MAP_HEIGHT_SCALE * (m_map->getHeight(up1, y) - m_map->getHeight(un0, y)));
  n2f.Set(
      0, 2 * MAP_XY_FACTOR,
      MAP_HEIGHT_SCALE * (m_map->getHeight(x, vp1) - m_map->getHeight(x, vn0)));
...
#define not_VERTS_MATCH
  ...
  VERTEX_FORMAT *vbMirror =
      ((VERTEX_FORMAT *)(*pData)) + (y)*vertsPerRow + 4 * (x);
...
#pragma MESSAGE("*** WARNING *** DOING DO_UNIT_TIMINGS!!!!")
  ...
  for (pDynamicLightsIterator.First(); !pDynamicLightsIterator.Is_Done();
       pDynamicLightsIterator.Next()) {
    W3DDynamicLight *pLight =
        (W3DDynamicLight *)pDynamicLightsIterator.Peek_Obj();
    ...
    if (pLight->m_enabled || pLight->m_priorEnable) {
      ...
      if (pLight->m_minX < xCoordMax && pLight->m_minY < yCoordMax &&
          pLight->m_maxX > xCoordMin && pLight->m_maxY > yCoordMin) {
  ...
  if (numDynaLights > 0) {
    ...
    for (j = 0; j < m_numVBTilesY; j++) {
      ...
      Bool intersect = false;
      Int yCoordMin =
          getYWithOrigin(yMin) + m_map->getDrawOrgY() - m_map->getBorderSize();
      Int yCoordMax = getYWithOrigin(yMax - 1) + m_map->getDrawOrgY() + 1 -
                      m_map->getBorderSize();
      ...
      for (i = 0; i < m_numVBTilesX; i++) {
        ...
        Bool intersect = false;
        Int xCoordMin = getXWithOrigin(xMin) + m_map->getDrawOrgX() -
                        m_map->getBorderSize();
        Int xCoordMax = getXWithOrigin(xMax - 1) + m_map->getDrawOrgX() + 1 -
                        m_map->getBorderSize();
        ...
        pVB = m_vertexBufferTiles + j * m_numVBTilesX +
              i;  // point to correct row/column of vertex buffers
        char **pData = m_vertexBufferBackup + j * m_numVBTilesX + i;
        updateVBForLight(*pVB, *pData, xMin, yMin, xMax, yMax, originX, originY,
...
//=============================================================================
void HeightMapRenderObjClass::addTerrainBib(Vector3 corners[4], ObjectID id,
                                            Bool highlight) {
...
static Bool check(const FrustumClass &frustum, WorldHeightMap *pMap, Int x,
                  Int y) {
  ...
  Vector3 loc((x - pMap->getBorderSize()) * MAP_XY_FACTOR,
              (y - pMap->getBorderSize()) * MAP_XY_FACTOR,
              height * MAP_HEIGHT_SCALE);
...
static void calcVis(const FrustumClass &frustum, WorldHeightMap *pMap, Int minX,
                    Int minY, Int maxX, Int maxY, Int limit) {
  if (maxX - minX < 2) return;
  if (maxY - minY < 2) return;
  if (minX >= visMinX && minY >= visMinY && maxX <= visMaxX &&
      maxY <= visMaxY) {
    return;
  }
...
is rendered. */
//=============================================================================
void HeightMapRenderObjClass::updateCenter(
    CameraClass *camera, RefRenderObjListIterator *pLightsIterator) {
  if (m_map == NULL) {
    return;
  }
  if (m_updating) {
  ...
  if (m_vertexBufferTiles == NULL) return;  // did not initialize resources yet.
  ...
  // determine the ray corresponding to the camera and distance to projection
  // plane
  ...
  float zmod = -1.0;  // Scene->vpd; // Note: view plane distance is now
                      // always 1.0 from the camera
  float minX = 200000;
  float maxX = -minX;
  float minY = 200000;
  float maxY = -minY;
  for (i = 0; i < 2; i++) {
    for (j = 0; j < 2; j++) {
      ...
      // transform the screen coordinates by the camera's matrix into world
      // coordinates.
      float x = zmod * camera_matrix[0][2] + xmod * camera_matrix[0][0] +
                ymod * camera_matrix[0][1];
      float y = zmod * camera_matrix[1][2] + xmod * camera_matrix[1][0] +
                ymod * camera_matrix[1][1];
      float z = zmod * camera_matrix[2][2] + xmod * camera_matrix[2][0] +
                ymod * camera_matrix[2][1];
  ...
  calcVis(frustum, m_map, minX - WIDE_STEP / 2, minY - WIDE_STEP / 2,
          maxX + WIDE_STEP / 2, maxY + WIDE_STEP / 2, limit);
  ...
  if (m_map) {
    ...
    if (abs(deltaX) > CENTER_LIMIT || abs(deltaY) > CENTER_LIMIT) {
      if (abs(deltaY) >= CENTER_LIMIT) {
        ...
        // It is much more efficient to update a cople of columns one frame, and
        // then a couple of rows.  So if we aren't "jumping" to a new view, and
        // have done X recently, return.
...
#pragma MESSAGE("*** WARNING *** DOING DO_UNIT_TIMINGS!!!!")
  ...
  if (((RTS3DScene *)rinfo.Camera.Get_User_Data())->getCustomPassMode() ==
          SCENE_PASS_ALPHA_MASK ||
      ((SceneClass *)rinfo.Camera.Get_User_Data())
              ->Get_Extra_Pass_Polygon_Mode() ==
          SceneClass::EXTRA_PASS_CLEAR_LINE) {
    if (WW3D::Is_Texturing_Enabled()) {  // first pass where we just fill the
                                         // z-buffer
    ...
    if (!ShaderClass::Is_Backface_Culling_Inverted()) {  // not reflection pass
      if (TheGlobalData->m_useLightMap && doCloud) {
        st = W3DShaderManager::ST_TERRAIN_BASE_NOISE12;
      } else if (TheGlobalData->m_useLightMap) {  // lightmap only
        st = W3DShaderManager::ST_TERRAIN_BASE_NOISE2;
      } else if (doCloud) {  // cloudmap only
        st = W3DShaderManager::ST_TERRAIN_BASE_NOISE1;
      }
    } else {  // reflection pass, just do base texture
    ...
    if (m_disableTextures) devicePasses = 1;  // force to 1 lighting-only pass
  ...
  for (pass = 0; pass < devicePasses; pass++) {
#ifdef TIMING_TESTS
#endif
    if (!doMultiPassWireFrame)  // multi-pass wireframe doesn't use regular
                                // shaders.
    {
    ...
    for (j = 0; j < m_numVBTilesY; j++)
      for (i = 0; i < m_numVBTilesX; i++) {
        ...
        count++;
        Int numPolys =
            VERTEX_BUFFER_TILE_LENGTH * VERTEX_BUFFER_TILE_LENGTH * 2;
        Int numVertex =
            (VERTEX_BUFFER_TILE_LENGTH * 2) * (VERTEX_BUFFER_TILE_LENGTH * 2);
        if (m_halfResMesh) {
          numPolys /= 4;
        ...
        DX8Wrapper::Set_Vertex_Buffer(
            m_vertexBufferTiles[j * m_numVBTilesX + i]);
#ifdef PRE_TRANSFORM_VERTEX
        if (m_xformedVertexBuffer && pass == 0) {
          // Note - m_xformedVertexBuffer should only be used for non T&L
          // hardware.  jba.
          DX8Wrapper::Apply_Render_State_Changes();
          int code = DX8Wrapper::_Get_D3D_Device8()->ProcessVertices(
              0, 0, numVertex, m_xformedVertexBuffer[j * m_numVBTilesX + i], 0);
        ...
        if (m_xformedVertexBuffer) {
          ...
          DX8Wrapper::_Get_D3D_Device8()->SetStreamSource(
              0, m_xformedVertexBuffer[j * m_numVBTilesX + i],
              D3DXGetFVFVertexSize(D3DFVF_XYZRHW | D3DFVF_DIFFUSE |
                                   D3DFVF_TEX2));
  ...
  if (!doMultiPassWireFrame) {
    ...
    Int xCoordMin = m_map->getDrawOrgX();
    Int xCoordMax = m_x + m_map->getDrawOrgX() - 1;
    m_customEdging->drawEdging(
        m_map, xCoordMin, xCoordMax, yCoordMin, yCoordMax, m_stageZeroTexture,
    ...
    if (!ShaderClass::Is_Backface_Culling_Inverted()) {
      ...
      if (Scene) {
        ...
        RefRenderObjListIterator pDynamicLightsIterator(
            pMyScene->getDynamicLights());
        m_roadBuffer->drawRoads(
            &rinfo.Camera, doCloud ? m_stageTwoTexture : NULL,
            TheGlobalData->m_useLightMap ? m_stageThreeTexture : NULL,
            m_disableTextures, xCoordMin - m_map->getBorderSize(),
            xCoordMax - m_map->getBorderSize(),
            yCoordMin - m_map->getBorderSize(),
            yCoordMax - m_map->getBorderSize(), &pDynamicLightsIterator);
      }
    }
#endif
...
/**Render parts of terrain that are along the coast line and have vertices
directly under the water plane.  Applying a custom render to these polygons
allows for a smoother land->water transition*/
void HeightMapRenderObjClass::renderShoreLines(CameraClass *pCamera) {
  ...
  while (j != m_numShoreLineTiles) {
    {  // Need to put this in another code block so vb/ib gets automatically
       // locked/unlocked by destructors
      DynamicVBAccessClass vb_access(BUFFER_TYPE_DYNAMIC_DX8, dynamic_fvf_type,
                                     DEFAULT_MAX_BATCH_SHORELINE_TILES * 4);
      ...
      // Loop over visible terrain and extract all the tiles that need shoreline
      // blend
      for (; j < m_numShoreLineTiles; j++) {
        ...
        if (x >= drawStartX && x < drawEdgeX && y >= drawStartY &&
            y < drawEdgeY) {  // this tile is inside visible region
    ...
    if (indexCount > 0 && vertexCount > 0)
      DX8Wrapper::Draw_Triangles(
          0, indexCount / 3, 0,
          vertexCount);  // draw a quad, 2 triangles, 4 verts
    vertexCount = 0;
    indexCount = 0;
  }  // for all shore tiles
...
/// Performs additional terrain rendering pass, blending in the black shroud
/// texture.
...
/** Renders an additoinal terrain pass including only those tiles which have
more than 2 textures blended together.  Used primarily for corner cases where 3
...
void HeightMapRenderObjClass::renderExtraBlendTiles(void) {
  ...
  if (!m_numExtraBlendTiles) return;  // nothing to draw

  if (maxBlendTiles >
      10000)  // we can only fit about 10000 tiles into a single VB.
    maxBlendTiles = 10000;

  DynamicVBAccessClass vb_access(BUFFER_TYPE_DYNAMIC_DX8, DX8_FVF_XYZNDUV2,
                                 maxBlendTiles * 4);
  DynamicIBAccessClass ib_access(BUFFER_TYPE_DYNAMIC_DX8, maxBlendTiles * 6);
  {
    ...
    for (Int j = 0; j < m_numExtraBlendTiles; j++) {
      if (vertexCount >= (maxBlendTiles * 4)) break;  // no room in vertex
                                                      // buffer
      ...
      if (x >= drawStartX && x < drawEdgeX && y >= drawStartY &&
          y < drawEdgeY &&
          m_map->getExtraAlphaUVData(
              x, y, U, V, alpha, &flipState,
              &cliffState)) {  // this tile is inside visible region and has 3rd
                               // blend layer.
        ...
        Real p3 = data[idx + xExtent] * MAP_HEIGHT_SCALE;
        if (cliffState &&
            abs(p0 - p2) > abs(p1 - p3))  // cliffs sometimes force a flip
          flipState = TRUE;
        ...
        vb->z = p1;
        vb->diffuse =
            (alpha[1] << 24) | (getStaticDiffuse(x + 1, y) & 0x00ffffff);
        vb->u1 = U[1];
        vb->v1 = V[1];
        vb++;
        ...
        vb->z = p2;
        vb->diffuse =
            (alpha[2] << 24) | (getStaticDiffuse(x + 1, y + 1) & 0x00ffffff);
        vb->u1 = U[2];
        vb->v1 = V[2];
        vb++;
        ...
        vb->z = p3;
        vb->diffuse =
            (alpha[3] << 24) | (getStaticDiffuse(x, y + 1) & 0x00ffffff);
        vb->u1 = U[3];
        vb->v1 = V[3];
        vb++;
  ...
  if (vertexCount) {
    // Check if we couldn't fit all blend tiles into vertex buffer so we can
    // enlarge it for next frame.
    if (vertexCount == (maxBlendTiles * 4))
      maxBlendTiles += 16;  // enlarge by 16 to reduce trashing.

    ShaderClass::Invalidate();  // invalidate to force shader to reset since we
                                // directly changed states
    DX8Wrapper::Set_Index_Buffer(ib_access, 0);
    DX8Wrapper::Set_Vertex_Buffer(vb_access);
    VertexMaterialClass *vmat =
        VertexMaterialClass::Get_Preset(VertexMaterialClass::PRELIT_DIFFUSE);
    ...
    shader.Set_Depth_Mask(
        ShaderClass::DEPTH_WRITE_DISABLE);  // disable writes to z
    DX8Wrapper::Set_Shader(shader);

    if (TheGlobalData->m_use3WayTerrainBlends == 2) {
      shader.Set_Primary_Gradient(
          ShaderClass::GRADIENT_DISABLE);  // disable lighting.
      shader.Set_Texturing(
          ShaderClass::TEXTURING_DISABLE);  // disable texturing.
      DX8Wrapper::Set_Shader(shader);
      DX8Wrapper::Set_Texture(
          0, NULL);  // debug mode which draws terrain tiles in white.
      if (Is_Hidden() == 0) {
        DX8Wrapper::Draw_Triangles(
            0, indexCount / 3, 0,
            vertexCount);  // draw a quad, 2 triangles, 4 verts
