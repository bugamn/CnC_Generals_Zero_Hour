...
//            structures and units on top of paths will render above it.
//            Waypoints are only shown for selected units while in waypoint
//            plotting mode.
//
//-----------------------------------------------------------------------------
...
W3DWaypointBuffer::W3DWaypointBuffer(void) {
  m_waypointNodeRobj =
      WW3DAssetManager::Get_Instance()->Create_Render_Obj("SCMNode");
  ...
  m_line->Set_Shader(lineShader);  // pick the alpha blending mode you want -
                                   // see shader.h for others.
  m_line->Set_Width(1.5f);
  m_line->Set_Color(Vector3(0.25f, 0.5f, 1.0f));
  m_line->Set_Texture_Mapping_Mode(
      SegLineRendererClass::TILED_TEXTURE_MAP);  // this tiles the texture
                                                 // across the line
}
...
void W3DWaypointBuffer::drawWaypoints(RenderInfoClass &rinfo) {
  if (TheInGameUI && TheInGameUI->isInWaypointMode()) {
    ...
    //@todo: Fix later by copying default scene light environement from
    //W3DScene.cpp.
    ...
    for (DrawableListCIt it = selected->begin(); it != selected->end(); ++it) {
      ...
      Int numPoints = 1;
      if (obj &&
          !obj->isKindOf(KINDOF_IGNORED_IN_GUI))  // so mobs and stuff sont make
                                                  // a gazillion lines
      {
          ...
          for (int i = gpIdx; i < goalSize; i++) {
            ...
            if (waypoint) {
              ...
              if (numPoints < MAX_DISPLAY_NODES + 1) {
                points[numPoints].Set(
                    Vector3(waypoint->x, waypoint->y, waypoint->z));
                numPoints++;
              }

              m_waypointNodeRobj->Set_Position(
                  Vector3(waypoint->x, waypoint->y, waypoint->z));
    ...
    if (TheInGameUI) {
      // Create a default light environment with no lights and only full
      // ambient.
      //@todo: Fix later by copying default scene light environement from
      //W3DScene.cpp.
      LightEnvironmentClass lightEnv;
      ...
      for (DrawableListCIt it = selected->begin(); it != selected->end();
        ...
        Int numPoints = 0;
        if (obj) {
          if (!obj->isLocallyControlled()) continue;
          ...
          if (exitInterface) {
            ...
            points[numPoints].Set(
                Vector3(exitPoint.x, exitPoint.y, exitPoint.z));
            numPoints++;

            Bool boxWrap = TRUE;
            Coord3D naturalRallyPoint;
            if (exitInterface->getNaturalRallyPoint(
                    naturalRallyPoint,
                    FALSE))  // FALSE means "without the extra offset"
            {
              if (!naturalRallyPoint.equals(exitPoint)) {
                points[numPoints].Set(Vector3(naturalRallyPoint.x,
                                              naturalRallyPoint.y,
                                              naturalRallyPoint.z));
                numPoints++;
              } else {
            ...
            if (rallyPoint) {
              if (boxWrap) {
                // test to se whether the rally point flanks the side of the
                // natural rally point to do this we find the two corners of the
                // geometry extents that are nearest the natural rally point
                //  these define the natural rally point edge
                //  an intermediate point should be inserted at the corner
                //  nearest the rally point
                ...
                // This is a quick idiot test to se whether the rally line needs
                // to wrap the box at all
                ...
                // if the rallypoint is closer to the wayoutpoint than it is to
                // the natural rally point then we definitely do not wrap
                Coord3D rallyToWayOutDelta = wayOutPoint;
                rallyToWayOutDelta.sub(rallyPoint);
                if ((100.0f + rallyToWayOutDelta.length()) > wayOutLength) {
                  // if we passed the above idiot test, now lets be sure by
                  // testing the dotproduct of the rp against the wayoutpoint
                  wayOutPoint
                      .normalize();  // a normal shooting straight out the door
                  ...
                  Real dot = NRPToRPDelta.x * wayOutPoint.x +
                             NRPToRPDelta.y * wayOutPoint.y;
                  if (dot > 0) {
                    ...
                    Coord2D *pNearElbow =
                        NULL;  // find the closest corner to the rallyPoint same
                               // end as door
                    Coord2D *pFarElbow = NULL;  // find the closest corner to
                                                // the rallypoint away from door
                    ...
                    for (UnsignedInt cornerIndex = 0; cornerIndex < 4;
                         ++cornerIndex) {
                      nearCandidate =
                          &corners[cornerIndex];  // for quicker array access
                      ...
                      dot = cornerToExitDelta.x * wayOutPoint.x +
                            cornerToExitDelta.y * wayOutPoint.y;
                      if (dot < 0.0f) {
                    ...
                    {
                      m_waypointNodeRobj->Set_Position(
                          Vector3(pNearElbow->x, pNearElbow->y, ctr->z));
                      WW3D::Render(*m_waypointNodeRobj,
                                   localRinfo);  // The little hockey puck
                      points[numPoints].Set(
                          Vector3(pNearElbow->x, pNearElbow->y, ctr->z));
                      numPoints++;
                      ...
                      {
                        // but let's test the dot of the first elbow against the
                        // rally point to find out
                        ...
                        dot = firstToRPDelta.x * firstElbowDelta.x +
                              firstToRPDelta.y * firstElbowDelta.y;
                        if (dot < 0)  // we have a second elbow
                        {
                          m_waypointNodeRobj->Set_Position(
                              Vector3(pFarElbow->x, pFarElbow->y, ctr->z));
                          WW3D::Render(*m_waypointNodeRobj,
                                       localRinfo);  // The little hockey puck
                          points[numPoints].Set(
                              Vector3(pFarElbow->x, pFarElbow->y, ctr->z));
              ...
              points[numPoints].Set(
                  Vector3(rallyPoint->x, rallyPoint->y, rallyPoint->z));
              numPoints++;

            } else
              continue;

            m_waypointNodeRobj->Set_Position(Vector3(
                naturalRallyPoint.x, naturalRallyPoint.y, naturalRallyPoint.z));
            WW3D::Render(*m_waypointNodeRobj,
                         localRinfo);  // The little hockey puck
