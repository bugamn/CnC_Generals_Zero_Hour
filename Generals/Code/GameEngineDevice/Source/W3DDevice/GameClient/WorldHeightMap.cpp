...
// -----------------------------------------------------------
static AsciiString validateName(AsciiString n, Int flags) { return n; }
...
MapObject::MapObject(Coord3D loc, AsciiString name, Real angle, Int flags,
  ...
  if (props) {
    m_properties = *props;
    if (thingTemplate &&
        !props->known(TheKey_objectSelectable, Dict::DICT_BOOL)) {
...
MapObject *MapObject::duplicate(void) {
  MapObject *pObj =
      newInstance(MapObject)(m_location, m_objectName, m_angle, m_flags,
...
RenderObjClass *MapObject::getBridgeRenderObject(BridgeTowerType type) {
  if (type >= 0 && type < BRIDGE_MAX_TOWERS) return m_bridgeTowers[type];
  return NULL;
}
...
void MapObject::verifyValidTeam(void) {
  ...
  // if it has an invalid team, the place it on the default neutral team, (by
  // clearing the existing team name.)
  Bool exists;
  AsciiString teamName =
      getProperties()->getAsciiString(TheKey_originalOwner, &exists);
  if (exists) {
    Bool valid = false;
    ...
    for (int i = 0; i < numSides; ++i) {
      ...
      Bool itBetter;
      AsciiString testAgainstTeamName =
          teamInfo->getDict()->getAsciiString(TheKey_teamName, &itBetter);
      if (itBetter) {
...
void MapObject::verifyValidUniqueID(void) {
  ...
  AsciiString uniqueID =
      getProperties()->getAsciiString(TheKey_uniqueID, &exists);
  ...
  while (obj) {
    ...
    Bool iterateExists;
    AsciiString tempStr =
        obj->getProperties()->getAsciiString(TheKey_uniqueID, &iterateExists);
...
void MapObject::fastAssignAllUniqueIDs(void) {
  // here's what we do. Take all of them, push them onto a stack. Then, pop each
  // one, setting its id. should be much faster than what we currently do.
...
WaypointID MapObject::getWaypointID() {
  return (WaypointID)getProperties()->getInt(TheKey_waypointID);
}
...
void MapObject::setWaypointID(Int i) {
  getProperties()->setInt(TheKey_waypointID, i);
}
void MapObject::setWaypointName(AsciiString n) {
  getProperties()->setAsciiString(TheKey_waypointName, n);
}

/*static */ Int MapObject::countMapObjectsWithOwner(const AsciiString &n) {
  Int count = 0;
  for (MapObject *pMapObj = MapObject::getFirstMapObject(); pMapObj;
       pMapObj = pMapObj->getNext()) {
...
#ifdef EVAL_TILING_MODES
static Bool ParseFunkyTilingDataChunk(DataChunkInput &file, DataChunkInfo *info,
                                      void *userData) {
...
WorldHeightMap::WorldHeightMap(ChunkInputStream *pStrm, Bool logicalDataOnly)
  ...
  if (logicalDataOnly) {
    ...
    file.registerParser(AsciiString("ObjectsList"), AsciiString::TheEmptyString,
                        ParseObjectsDataChunk);
    freeListOfMapObjects();  // just in case.
    file.registerParser(AsciiString("PolygonTriggers"),
    ...
    file.registerParser(AsciiString("HeightMapData"),
                        AsciiString::TheEmptyString, ParseHeightMapDataChunk);
    file.registerParser(AsciiString("BlendTileData"),
                        AsciiString::TheEmptyString, ParseBlendTileDataChunk);
#ifdef EVAL_TILING_MODES
    file.registerParser(AsciiString("FUNKY_TILING"),
                        AsciiString::TheEmptyString, ParseFunkyTilingDataChunk);
#endif
    file.registerParser(AsciiString("GlobalLighting"),
...
/** Optimized version of method to get triangle flip state of a terrain cell.
 *Use this instead of getAlphaUVData() whenever possible.
 */
Bool WorldHeightMap::getFlipState(Int xIndex, Int yIndex) const {
  ...
  return m_cellFlipState[yIndex * m_flipStateWidth + (xIndex >> 3)] &
         (1 << (xIndex & 0x7));
}
...
Bool WorldHeightMap::getCliffState(Int xIndex, Int yIndex) const {
  ...
  return m_cellCliffState[yIndex * m_flipStateWidth + (xIndex >> 3)] &
         (1 << (xIndex & 0x7));
}
...
void WorldHeightMap::setCliffState(Int xIndex, Int yIndex, Bool state) {
  ...
  UnsignedByte flagByte =
      m_cellCliffState[yIndex * m_flipStateWidth + (xIndex >> 3)];
...
Bool WorldHeightMap::ParseLightingDataChunk(DataChunkInput &file,
  ...
  // initialize the directions of the lights to not be totally invalid, in case
  // old maps are read
  for (i = 0; i < 4; i++) {
    for (Int j = 0; j < MAX_GLOBAL_LIGHTS; j++) {
      TheWritableGlobalData
          ->m_terrainObjectsLighting[i + TIME_OF_DAY_FIRST][j] =
          initLightValues;
      TheWritableGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][j] =
          initLightValues;
    }
  }

  for (i = 0; i < 4; i++) {
    TheWritableGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0]
        .ambient.red = file.readReal();
    TheWritableGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0]
        .ambient.green = file.readReal();
    TheWritableGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0]
        .ambient.blue = file.readReal();
    TheWritableGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0]
        .diffuse.red = file.readReal();
    TheWritableGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0]
        .diffuse.green = file.readReal();
    TheWritableGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0]
        .diffuse.blue = file.readReal();
    TheWritableGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0]
        .lightPos.x = file.readReal();
    TheWritableGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0]
        .lightPos.y = file.readReal();
    TheWritableGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0]
        .lightPos.z = file.readReal();

    TheWritableGlobalData->m_terrainObjectsLighting[i + TIME_OF_DAY_FIRST][0]
        .ambient.red = file.readReal();
    TheWritableGlobalData->m_terrainObjectsLighting[i + TIME_OF_DAY_FIRST][0]
        .ambient.green = file.readReal();
    TheWritableGlobalData->m_terrainObjectsLighting[i + TIME_OF_DAY_FIRST][0]
...
void WorldHeightMap::readTexClass(TXTextureClass *texClass,
  ...
  if (terrain == NULL) {
    ...
    theFile = TheFileSystem->openFile(texClass->name.str(),
                                      File::READ | File::BINARY);
#endif
  } else {
    sprintf(texturePath, "%s%s", TERRAIN_TGA_DIR_PATH,
            terrain->getTexture().str());
...
Bool WorldHeightMap::ParseBlendTileData(DataChunkInput &file,
  ...
  m_cliffInfoNdxes =
      MSGNEW("WorldHeightMap_ParseBlendTileData") Short[m_dataSize];
  m_blendTileNdxes =
      MSGNEW("WorldHeightMap_ParseBlendTileData") Short[m_dataSize];
  ...
  m_cellFlipState = MSGNEW("WorldHeightMap_getTerrainTexture")
      UnsignedByte[numBytesX * numBytesY];
  m_cellCliffState = MSGNEW("WorldHeightMap_getTerrainTexture")
      UnsignedByte[numBytesX * numBytesY];
  ...
  if (info->version >= K_BLEND_TILE_VERSION_6) {
    file.readArrayOfBytes((char *)m_extraBlendTileNdxes,
  ...
  if (info->version >= K_BLEND_TILE_VERSION_7) {
    file.readArrayOfBytes((char *)m_cellCliffState,
                          m_height * m_flipStateWidth);
  } else {
    initCliffFlagsFromHeights();
  }
  m_numBitmapTiles = file.readInt();
  DEBUG_ASSERTCRASH(m_numBitmapTiles > 0 && m_numBitmapTiles < 2048,
                    ("Unlikely numBitmapTiles."));
  m_numBlendedTiles = file.readInt();
  DEBUG_ASSERTCRASH(
      m_numBlendedTiles > 0 && m_numBlendedTiles < NUM_BLEND_TILES + 1,
      ("Unlikely numBlendedTiles."));
  if (info->version >= K_BLEND_TILE_VERSION_5) {
  ...
  DEBUG_ASSERTCRASH(m_numTextureClasses > 0 && m_numTextureClasses < 200,
                    ("Unlikely m_numTextureClasses."));
  ...
  for (i = 1; i < m_numBlendedTiles; i++) {
    ...
    // Useful for flushing out initial maps made with buggy 3-way blending.
    if (!TheGlobalData->m_use3WayTerrainBlends)
      m_blendedTiles[i].inverted &=
          ~FLIPPED_MASK;  // filter out extra flips from 3-way
...
Bool WorldHeightMap::ParseObjectDataChunk(DataChunkInput &file,
  ...
  return pThis->ParseObjectData(file, info, userData,
                                info->version >= K_OBJECTS_VERSION_2);
}
 ...
 *
 */
Bool WorldHeightMap::ParseObjectData(DataChunkInput &file, DataChunkInfo *info,
                                     void *userData, Bool readDict) {
  ...
  if (pThisOne->getProperties()->getType(TheKey_lightHeightAboveTerrain) ==
      Dict::DICT_REAL)
    pThisOne->setIsLight();
  ...
  if (pPrevious) {
    DEBUG_ASSERTCRASH(
        MapObject::TheMapObjectListPtr != NULL && pPrevious->getNext() == NULL,
        ("Bad linkage."));
...
typedef struct {
  ...
  UnsignedByte
      flags;  //  &0x0F = alpha channel bits, &0x10 is right to left flag,
              // 0x20 is top to bottom flag.  (0x0? is left to right, bottom to
              // top) 0x3? is top to bottom, right to left.
} TTargaHeader;
...
/*Break down a .tga file into a collection of tiles.  numRows * numRows total
 * tiles.*/
Bool WorldHeightMap::readTiles(InputStream *pStr, TileData **tiles,
                               Int numRows) {
  TTargaHeader hdr;
  ...
  for (row = 0; row < numRows * TILE_PIXEL_EXTENT; row++) {
    for (column = 0; column < hdr.imageWidth; column++) {
      ...
      int pixelNdx = (column % TILE_PIXEL_EXTENT) +
                     TILE_PIXEL_EXTENT * (row % TILE_PIXEL_EXTENT);
...
Int WorldHeightMap::updateTileTexturePositions(Int *edgeHeight) {
  ...
  for (tileWidth = tilesPerRow; tileWidth > 0; tileWidth--) {
    for (texClass = 0; texClass < m_numTextureClasses; texClass++) {
      ...
      Int xOrigin =
          TILE_OFFSET / 2 + column * (TILE_PIXEL_EXTENT + TILE_OFFSET);
 ...
 */
void WorldHeightMap::getUVForNdx(Int tileNdx, float *minU, float *minV,
                                 float *maxU, float *maxV, Bool fullTile) {
...
/// Get whether something is cliff indexed with the offset that
/// HeightMapRenderObjClass uses built in.
...
/** getUVData - Gets the texture coordinates to use.  See getTerrainTexture.
                xIndex and yIndex are the integer coorddinates into the height
   map. U and V are the texture coordiantes for the 4 corners of a height map
   cell. fullTile is true if we are doing 1/2 resolution height map, and require
   a full tile to texture  a cell.  Otherwise, we use quarter tiles per cell.
*/
...
#define dont_SHOW_THE_TEXTURE_FOR_DEBUG 1
  ...
  // This is debug code that just shows the generated texture laid on the
  // terrain. For debugging ;) jba.
...
/** getUVForTileIndex - Gets the texture coordinates to use.  See
   getTerrainTexture. ndx is the index into the linear height array. tileNdx is
   the index into the texture tiles array. U and V are the texture coordiantes
   for the 4 corners of a height map cell. fullTile is true if we are doing 1/2
   resolution height map, and require a full tile to texture  a cell. Otherwise,
   we use quarter tiles per cell.
*/
  ...
  if ((ndx < m_dataSize) && m_tileNdxes) {
    ...
    if (m_cliffInfoNdxes[ndx]) {
      ...
      for (i = 0; i < this->m_numTextureClasses; i++) {
        if (ndx1 >= m_textureClasses[i].firstTile &&
          ...
          tilesMatch = ndx2 >= m_textureClasses[i].firstTile &&
                       ndx2 < m_textureClasses[i].firstTile +
      ...
      if (tilesMatch) {
        ...
        Real maxV = m_textureClasses[i].positionInTexture.y +
                    m_textureClasses[i].width * TILE_PIXEL_EXTENT;
        minU /= TEXTURE_WIDTH;
        maxV /= m_terrainTexHeight;
...
#define DO_OLD_UV
    ...
    static Real TILE_LIMIT = 4.0;  // Our tiles are currently 4 cells wide &
                                   // tall, so dont' adjust to more than 4.0.
    ...
    if (ndx < this->m_dataSize - m_width - 1) {
      ...
      for (texClass = 0; texClass < m_numTextureClasses; texClass++) {
        ...
        // see if the blend tile is in a texture class, and get the right tile
        // for xIndex, yIndex.
        if (baseNdx >= m_textureClasses[texClass].firstTile &&
            baseNdx < m_textureClasses[texClass].firstTile +
                          m_textureClasses[texClass].numTiles) {
          break;
        }
      }
      ...
      if (below == 1 || above > below) {  //(avgH > minH + (2*deltaH+2)/3)
        ...
        if (dx < STRETCH_LIMIT)
          dx = 1.0f;  // don't make a seam unless there is great stretch.
        if (dy < STRETCH_LIMIT)
          dy = 1.0f;  // don't make a seam unless there is great stretch.
        if (dx > TILE_LIMIT)
          dx = TILE_LIMIT;  // don't tile past the texture's edge.
        if (dy > TILE_LIMIT)
          dy = TILE_LIMIT;  // don't tile past the texture's edge.
        dx *= xU - nU;
        dy *= xV - nV;
        U[0] = nU;
        U[1] = nU + dx;
...
///@todo: Are the different "if" cases mutually exclusive?  If so, should add
///else statements.
Bool WorldHeightMap::getExtraAlphaUVData(Int xIndex, Int yIndex, float U[4],
                                         float V[4], UnsignedByte alpha[4],
                                         Bool *needFlip, Bool *cliff) {
  Int ndx = (yIndex * m_width) + xIndex;
  *needFlip = FALSE;
  *cliff = FALSE;
...
/** getUVData - Gets the texture coordinates to use with the alpha texture.
   ...
   cell. fullTile is true if we are doing 1/2 resolution height map, and require
   a full tile to texture  a cell.  Otherwise, we use quarter tiles per cell.
                flip is set if we need to flip the diagonal across the cell to
   make the alpha coordinates blend properly.  Filling a square with 2 triangles
   is not symmetrical :)
*/
void WorldHeightMap::getAlphaUVData(Int xIndex, Int yIndex, float U[4],
                                    float V[4], UnsignedByte alpha[4],
                                    Bool *flip, Bool fullTile) {
  xIndex += m_drawOriginX;
  yIndex += m_drawOriginY;
  Int ndx = (yIndex * m_width) + xIndex;
  ...
  if ((ndx < m_dataSize) && m_tileNdxes) {
    ...
    if (blendNdx == 0) {
      stretchedForCliff =
          getUVForTileIndex(ndx, m_tileNdxes[ndx], U, V, fullTile);
      alpha[0] = alpha[1] = alpha[2] = alpha[3] = 0;
      // No alpha blend, so never need to flip.
      needFlip = false;
    } else {
      stretchedForCliff = getUVForTileIndex(
          ndx, m_blendedTiles[blendNdx].blendNdx, U, V, fullTile);
      alpha[0] = alpha[1] = alpha[2] = alpha[3] = 0;
      if (m_blendedTiles[blendNdx].horiz) {
        // Horizontals don't need flipping unless forced because of 3way blend.
        needFlip = m_blendedTiles[blendNdx].inverted & FLIPPED_MASK;
...
TextureClass *WorldHeightMap::getTerrainTexture(void) {
  if (m_terrainTex == NULL) {
    ...
    REF_PTR_RELEASE(m_terrainTex);
    m_terrainTex = MSGNEW("WorldHeightMap_getTerrainTexture")
        TerrainTextureClass(pow2Height);
    m_terrainTexHeight = m_terrainTex->update(this);
    char buf[64];
    sprintf(buf, "Base tex height %d\n", pow2Height);
    DEBUG_LOG((buf));
    REF_PTR_RELEASE(m_alphaTerrainTex);
    m_alphaTerrainTex = MSGNEW("WorldHeightMap_getTerrainTexture")
        AlphaTerrainTextureClass(m_terrainTex);
    ...
    REF_PTR_RELEASE(m_alphaEdgeTex);
    m_alphaEdgeTex = MSGNEW("WorldHeightMap_getTerrainTexture")
        AlphaEdgeTextureClass(pow2Height);
    m_alphaEdgeHeight = m_alphaEdgeTex->update(this);

    // Generate lookup table for determining triangle order in each terrain
    // cell. Not the best place to put this but getAlphaUVData() requires a
    // valid terrain texture to return valid values.
    ...
    for (Int y = 0; y < (m_height - 1); y++)
      for (Int x = 0; x < (m_width - 1); x++) {
        ...
        m_cellFlipState[y * m_flipStateWidth + (x >> 3)] |= flipForBlend
                                                            << (x & 0x7);
        DEBUG_ASSERTCRASH(
            (y * m_flipStateWidth + (x >> 3)) < (m_flipStateWidth * m_height),
            ("Bad range"));
      }
  }
...
Bool WorldHeightMap::setDrawOrg(Int xOrg, Int yOrg) {
  ...
  if (newY < 0) newY = 0;
  Bool anythingDifferent = (m_drawOriginX != newX) || (m_drawOriginY != newY) ||
