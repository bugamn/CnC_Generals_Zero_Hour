...
static TextureClass
    *cursorTextures[Mouse::NUM_MOUSE_CURSORS]
                   [MAX_2D_CURSOR_ANIM_FRAMES];  ///< Textures for each cursor
                                                 ///< type
static const Image
    *cursorImages[Mouse::NUM_MOUSE_CURSORS];  ///< Images for use with the
                                              ///< RM_POLYGON method.
static RenderObjClass
    *cursorModels[Mouse::NUM_MOUSE_CURSORS];  ///< W3D models for each cursor
                                              ///< type
static HAnimClass
    *cursorAnims[Mouse::NUM_MOUSE_CURSORS];  ///< W3D animations for each cursor
                                             ///< type
  ...
  if (m_currentRedrawMode == RM_POLYGON && cursorImages[1] == NULL) {
    for (Int i = 0; i < NUM_MOUSE_CURSORS; i++) {
      ...
      if (!m_cursorInfo[i].imageName.isEmpty())
        cursorImages[i] = TheMappedImageCollection->findImageByName(
            m_cursorInfo[i].imageName);
    }
  }
}
...
Bool W3DMouse::loadD3DCursorTextures(MouseCursor cursor) {
  ...
  if (!animFrames) return FALSE;  // no animation frames defined.
  ...
  if (animFrames == 1) {  // single animation frame without trailing numbers
    ...
    for (Int i = 0; i < animFrames; i++) {
      ...
      if ((cursorTextures[cursor][i] = am->Get_Texture(FrameName)) != NULL) {
        m_currentD3DSurface[m_currentFrames] =
            cursorTextures[cursor][i]->Get_Surface_Level();
        m_currentFrames++;
      }
    }
  return TRUE;
...
void W3DMouse::initD3DAssets(void) {
  // Nothing to do here unless we want to preload all possible cursors which
  // would probably not be practical for memory reasons.
  ...
  if (m_currentRedrawMode == RM_DX8 && cursorTextures[1] == NULL && am) {
    for (Int i = 0; i < NUM_MOUSE_CURSORS; i++) {
      for (Int j = 0; j < MAX_2D_CURSOR_ANIM_FRAMES; j++) {
        cursorTextures[i][j] =
            NULL;  // am->Get_Texture(m_cursorInfo[i].textureName.str());
...
void W3DMouse::initW3DAssets(void) {
  ...
  if ((cursorModels[1] == NULL && W3DDisplay::m_assetManager)) {
    for (Int i = 1; i < NUM_MOUSE_CURSORS; i++) {
      if (!m_cursorInfo[i].W3DModelName.isEmpty()) {
        if (m_orthoCamera)
          cursorModels[i] = W3DDisplay::m_assetManager->Create_Render_Obj(
              m_cursorInfo[i].W3DModelName.str(),
              m_cursorInfo[i].W3DScale * m_orthoZoom, 0);
        else
          cursorModels[i] = W3DDisplay::m_assetManager->Create_Render_Obj(
              m_cursorInfo[i].W3DModelName.str(), m_cursorInfo[i].W3DScale, 0);
        if (cursorModels[i]) {
  ...
  if ((cursorAnims[1] == NULL && W3DDisplay::m_assetManager)) {
    for (Int i = 1; i < NUM_MOUSE_CURSORS; i++) {
      if (!m_cursorInfo[i].W3DAnimName.isEmpty()) {
        ...
        cursorAnims[i] = W3DDisplay::m_assetManager->Get_HAnim(
            m_cursorInfo[i].W3DAnimName.str());
        if (cursorAnims[i] && cursorModels[i]) {
          cursorModels[i]->Set_Animation(cursorAnims[i], 0,
                                         (m_cursorInfo[i].loop)
  ...
  if (m_orthoCamera) m_camera->Set_Projection_Type(CameraClass::ORTHO);
}
...
void W3DMouse::init(void) {
  ...
  isThread = FALSE;
  if (m_currentRedrawMode == RM_DX8) thread.Execute();
...
void W3DMouse::setCursor(MouseCursor cursor) {
  ...
  m_directionFrame = 0;
  if (m_currentRedrawMode ==
      RM_WINDOWS) {  // Windows default cursor needs to refreshed whenever we
                     // get a WM_SETCURSOR
    m_currentD3DCursor = NONE;
    m_currentW3DCursor = NONE;
    m_currentPolygonCursor = NONE;
    setCursorDirection(cursor);
    if (m_drawing)  // only allow cursor to change when drawing the cursor (once
                    // per frame) to fix flickering.
  ...
  // if we're already on this cursor ignore the rest of code to stop cursor
  // flickering.
  if (m_currentCursor == cursor && m_currentD3DCursor == cursor) return;
  ...
  if (m_currentRedrawMode == RM_DX8) {
    ...
    if (m_pDev != NULL) {
      ...
      if (m_currentD3DSurface[0]) doImageChange = TRUE;
    }
    ...
    if (doImageChange) {
      ...
      res = m_pDev->SetCursorProperties(
    ...
    if (cursor != m_currentW3DCursor) {
      ...
      if (!cursorModels[1]) initW3DAssets();

      if (cursorModels[1]) {
        if (cursorModels[m_currentW3DCursor]) {
          W3DDisplay::m_3DInterfaceScene->Remove_Render_Object(
              cursorModels[m_currentW3DCursor]);
        }

        m_currentW3DCursor = cursor;

        if (cursorModels[m_currentW3DCursor]) {
          W3DDisplay::m_3DInterfaceScene->Add_Render_Object(
              cursorModels[m_currentW3DCursor]);
          if (m_cursorInfo[m_currentW3DCursor].loop == FALSE &&
              cursorAnims[m_currentW3DCursor]) {
            cursorModels[m_currentW3DCursor]->Set_Animation(
                cursorAnims[m_currentW3DCursor], 0,
                RenderObjClass::ANIM_MODE_ONCE);
          }
        }
      }
...
void W3DMouse::draw(void) {
  ...
  if (m_currentRedrawMode == RM_DX8 && m_currentD3DCursor != NONE) {
    ...
    if (m_pDev) {
      ...
      if (TheDisplay &&
          !TheDisplay->getWindowed()) {  // if we're full-screen, need to
                                         // manually move cursor image
        POINT ptCursor;

        GetCursorPos(&ptCursor);
        ScreenToClient(ApplicationHWnd, &ptCursor);
        m_pDev->SetCursorPosition(ptCursor.x, ptCursor.y,
    ...
    if (image) {
      TheDisplay->drawImage(
          image, m_currMouse.pos.x - m_currentHotSpot.x,
          m_currMouse.pos.y - m_currentHotSpot.y,
          m_currMouse.pos.x + image->getImageWidth() - m_currentHotSpot.x,
          m_currMouse.pos.y + image->getImageHeight() - m_currentHotSpot.y);
    }
  } else if (m_currentRedrawMode == RM_WINDOWS) {
  } else if (m_currentRedrawMode == RM_W3D) {
    if (W3DDisplay::m_3DInterfaceScene && m_camera && m_visible) {
      if (cursorModels[m_currentW3DCursor]) {
        Real xPercent = (1.0f - (TheDisplay->getWidth() - m_currMouse.pos.x) /
                                    (Real)TheDisplay->getWidth());
        Real yPercent = ((TheDisplay->getHeight() - m_currMouse.pos.y) /
        ...
        if (m_orthoCamera) {
          ...
          // W3D Screen coordinates are -1 to 1, so we need to do some
          // conversion:
          Real logX, logY;
          PixelScreenToW3DLogicalScreen(
              m_currMouse.pos.x - 0, m_currMouse.pos.y - 0, &logX, &logY,
              TheDisplay->getWidth(), TheDisplay->getHeight());

          Vector3 rayStart;
          Vector3 rayEnd;
          rayStart = m_camera->Get_Position();  // get camera location
          m_camera->Un_Project(rayEnd,
                               Vector2(logX, logY));  // get world space point
          rayEnd -= rayStart;  // vector camera to world space point
          rayEnd.Normalize();  // make unit vector
          rayEnd *=
              m_camera->Get_Depth();  // adjust length to reach far clip plane
          rayEnd +=
              rayStart;  // get point on far clip plane along ray from camera.
  ...
  if (!isThread) drawCursorText();

  // draw tooltip text
  if (m_visible && !isThread) drawTooltip();
...
void W3DMouse::setRedrawMode(RedrawMode mode) {
  ...
  switch (mode) {
    case RM_WINDOWS: {  // Windows mouse doesn't need an update thread.
    ...
    case RM_W3D: {  // Model mouse updated only at render time so doesn't
      // require thread.
      if (thread.Is_Running()) thread.Stop();
    ...
    case RM_POLYGON: {  // Polygon mouse updated only at render time so doesn't
      // require thread.
      if (thread.Is_Running()) thread.Stop();
      freeD3DAssets();  // using packed Image data, not textures.
      freeW3DAssets();
    ...
    case RM_DX8: {  // this cursor type is drawn by DX8 and can be refreshed
      // independent of rendering rate.  Uses another thread to do
      // position updates.
      initD3DAssets();  // make sure textures loaded.
      freeW3DAssets();
      freePolygonAssets();
      if (!thread.Is_Running()) thread.Execute();
      m_currentW3DCursor = NONE;
      m_currentPolygonCursor = NONE;
      break;
    }
...
void W3DMouse::setCursorDirection(MouseCursor cursor) {
  Coord2D offset = {0, 0};
  // Check if we have a directional cursor that needs different images for each
  // direction
  if (m_cursorInfo[cursor].numDirections > 1 && TheInGameUI &&
      TheInGameUI->isScrolling()) {
    offset = TheInGameUI->getScrollAmount();
    if (offset.x || offset.y) {
      ...
      // actual cursor direction.  Frame 0 is assumed to point right and
      // continue clockwise.
      m_directionFrame =
          (Int)(theta / (2.0f * M_PI / (Real)numDirections) + 0.5f);
      if (m_directionFrame >= numDirections) m_directionFrame = 0;
    } else {
      m_directionFrame = 0;
    }
  } else
