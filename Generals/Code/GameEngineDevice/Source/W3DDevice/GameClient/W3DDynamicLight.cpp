...
W3DDynamicLight::W3DDynamicLight(void) : LightClass(LightClass::POINT) {
  m_priorEnable = false;
  m_enabled = true;
}
...
void W3DDynamicLight::On_Frame_Update(void) {
  ...
  if (m_curIncreaseFrameCount > 0 && m_increaseFrameCount > 0) {
    ...
    factor = (m_increaseFrameCount - m_curIncreaseFrameCount) /
             (Real)m_increaseFrameCount;
...
void W3DDynamicLight::setFrameFade(UnsignedInt frameIncreaseTime,
