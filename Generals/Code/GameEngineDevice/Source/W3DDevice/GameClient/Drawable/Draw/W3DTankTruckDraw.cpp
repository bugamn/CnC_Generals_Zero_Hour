...
// Draw TankTrucks.  Actually, this draws quad cannon which has both treads and
// wheels. Author: Mark Wilczynski, August 2002
...
void W3DTankTruckDrawModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"Dust", INI::parseAsciiString, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_dustEffectName)},
      {"DirtSpray", INI::parseAsciiString, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_dirtEffectName)},
      {"PowerslideSpray", INI::parseAsciiString, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_powerslideEffectName)},
      {"LeftFrontTireBone", INI::parseAsciiString, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_frontLeftTireBoneName)},
      {"RightFrontTireBone", INI::parseAsciiString, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_frontRightTireBoneName)},
      {"LeftRearTireBone", INI::parseAsciiString, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_rearLeftTireBoneName)},
      {"RightRearTireBone", INI::parseAsciiString, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_rearRightTireBoneName)},
      {"MidLeftFrontTireBone", INI::parseAsciiString, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_midFrontLeftTireBoneName)},
      {"MidRightFrontTireBone", INI::parseAsciiString, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_midFrontRightTireBoneName)},
      {"MidLeftRearTireBone", INI::parseAsciiString, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_midRearLeftTireBoneName)},
      {"MidRightRearTireBone", INI::parseAsciiString, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_midRearRightTireBoneName)},
      {"TireRotationMultiplier", INI::parseReal, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_rotationSpeedMultiplier)},
      {"PowerslideRotationAddition", INI::parseReal, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_powerslideRotationAddition)},
      {"TreadDebrisLeft", INI::parseAsciiString, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_treadDebrisNameLeft)},
      {"TreadDebrisRight", INI::parseAsciiString, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_treadDebrisNameRight)},
      {"TreadAnimationRate", INI::parseVelocityReal, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_treadAnimationRate)},
      {"TreadPivotSpeedFraction", INI::parseReal, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_treadPivotSpeedFraction)},
      {"TreadDriveSpeedFraction", INI::parseReal, NULL,
       offsetof(W3DTankTruckDrawModuleData, m_treadDriveSpeedFraction)},
      {0, 0, 0, 0}};
...
W3DTankTruckDraw::W3DTankTruckDraw(Thing *thing, const ModuleData *moduleData)
    : W3DModelDraw(thing, moduleData),
      ...
      m_frontWheelRotation(0),
      m_rearWheelRotation(0),
  ...
  // Truck Data
  m_landingSound =
      *(thing->getTemplate()->getPerUnitSound("TruckLandingSound"));
  m_powerslideSound =
      *(thing->getTemplate()->getPerUnitSound("TruckPowerslideSound"));

  // Tank data
  m_treadDebrisLeft = NULL;
  m_treadDebrisRight = NULL;

  for (Int i = 0; i < MAX_TREADS_PER_TANK; i++) m_treads[i].m_robj = NULL;
  ...
  if (getW3DTankTruckDrawModuleData()) {
    ...
    sysTemplate = TheParticleSystemManager->findTemplate(
        getW3DTankTruckDrawModuleData()->m_treadDebrisNameLeft);
    if (sysTemplate) {
      m_treadDebrisLeft =
          TheParticleSystemManager->createParticleSystem(sysTemplate);
      m_treadDebrisLeft->attachToDrawable(getDrawable());
      DEBUG_CRASH(("test me, may not work (srj)"));
      // important: mark it as do-not-save, since we'll just re-create it when
      // we reload.
      m_treadDebrisLeft->setSaveable(FALSE);
    }

    sysTemplate = TheParticleSystemManager->findTemplate(
        getW3DTankTruckDrawModuleData()->m_treadDebrisNameRight);
    if (sysTemplate) {
      m_treadDebrisRight =
          TheParticleSystemManager->createParticleSystem(sysTemplate);
...
W3DTankTruckDraw::~W3DTankTruckDraw() {
  ...
  for (Int i = 0; i < MAX_TREADS_PER_TANK; i++)
    if (m_treads[i].m_robj) REF_PTR_RELEASE(m_treads[i].m_robj);
}
...
void W3DTankTruckDraw::startMoveDebris(void) {
  if (getDrawable()->isDrawableEffectivelyHidden()) return;
  if (m_treadDebrisLeft) m_treadDebrisLeft->start();
  if (m_treadDebrisRight) m_treadDebrisRight->start();
}
...
void W3DTankTruckDraw::stopMoveDebris(void) {
  if (m_treadDebrisLeft) m_treadDebrisLeft->stop();
  if (m_treadDebrisRight) m_treadDebrisRight->stop();
}
...
void W3DTankTruckDraw::createEmitters(void) {
  ...
  if (getW3DTankTruckDrawModuleData()) {
    ...
    if (!m_dustEffect) {
      sysTemplate = TheParticleSystemManager->findTemplate(
          getW3DTankTruckDrawModuleData()->m_dustEffectName);
      if (sysTemplate) {
        m_dustEffect =
            TheParticleSystemManager->createParticleSystem(sysTemplate);
        ...
        if (!getW3DTankTruckDrawModuleData()->m_dustEffectName.isEmpty()) {
          DEBUG_LOG(
              ("*** ERROR - Missing particle system '%s' in thing '%s'\n",
               getW3DTankTruckDrawModuleData()->m_dustEffectName.str(),
    ...
    if (!m_dirtEffect) {
      sysTemplate = TheParticleSystemManager->findTemplate(
          getW3DTankTruckDrawModuleData()->m_dirtEffectName);
      if (sysTemplate) {
        m_dirtEffect =
            TheParticleSystemManager->createParticleSystem(sysTemplate);
        ...
        if (!getW3DTankTruckDrawModuleData()->m_dirtEffectName.isEmpty()) {
          DEBUG_LOG(
              ("*** ERROR - Missing particle system '%s' in thing '%s'\n",
               getW3DTankTruckDrawModuleData()->m_dirtEffectName.str(),
               getDrawable()->getObject()->getTemplate()->getName().str()));
        }
      }
    }
    if (!m_powerslideEffect) {
      sysTemplate = TheParticleSystemManager->findTemplate(
          getW3DTankTruckDrawModuleData()->m_powerslideEffectName);
      if (sysTemplate) {
        m_powerslideEffect =
            TheParticleSystemManager->createParticleSystem(sysTemplate);
        ...
        if (!getW3DTankTruckDrawModuleData()
                 ->m_powerslideEffectName.isEmpty()) {
          DEBUG_LOG(
              ("*** ERROR - Missing particle system '%s' in thing '%s'\n",
               getW3DTankTruckDrawModuleData()->m_powerslideEffectName.str(),
               getDrawable()->getObject()->getTemplate()->getName().str()));
        }
      }
    }
...
void W3DTankTruckDraw::enableEmitters(Bool enable) {
  // don't check... if we are hidden the first time thru, then we'll never
  // create the emitters. eg, if we are loading a game and the unit is in a
  // tunnel, he'll never get emitteres even when he exits.
  ...
  if (m_powerslideEffect) {
    if (!enable) m_powerslideEffect->stop();
  }
}
//-------------------------------------------------------------------------------------------------
void W3DTankTruckDraw::updateBones(void) {
  if (getW3DTankTruckDrawModuleData()) {
    ...
    if (!getW3DTankTruckDrawModuleData()->m_frontLeftTireBoneName.isEmpty()) {
      m_frontLeftTireBone = getRenderObject()->Get_Bone_Index(
          getW3DTankTruckDrawModuleData()->m_frontLeftTireBoneName.str());
      DEBUG_ASSERTCRASH(
          m_frontLeftTireBone,
          ("Missing front-left tire bone %s in model %s\n",
           getW3DTankTruckDrawModuleData()->m_frontLeftTireBoneName.str(),
           getRenderObject()->Get_Name()));

      m_frontRightTireBone = getRenderObject()->Get_Bone_Index(
          getW3DTankTruckDrawModuleData()->m_frontRightTireBoneName.str());
      DEBUG_ASSERTCRASH(
    ...
    if (!getW3DTankTruckDrawModuleData()->m_rearLeftTireBoneName.isEmpty()) {
      m_rearLeftTireBone = getRenderObject()->Get_Bone_Index(
          getW3DTankTruckDrawModuleData()->m_rearLeftTireBoneName.str());
      DEBUG_ASSERTCRASH(
          m_rearLeftTireBone,
      ...
      m_rearRightTireBone = getRenderObject()->Get_Bone_Index(
          getW3DTankTruckDrawModuleData()->m_rearRightTireBoneName.str());
      DEBUG_ASSERTCRASH(
          m_rearRightTireBone,
          ("Missing rear-left tire bone %s in model %s\n",
           getW3DTankTruckDrawModuleData()->m_rearRightTireBoneName.str(),
    ...
    // midFront tires
    if (!getW3DTankTruckDrawModuleData()
             ->m_midFrontLeftTireBoneName.isEmpty()) {
      m_midFrontLeftTireBone = getRenderObject()->Get_Bone_Index(
          getW3DTankTruckDrawModuleData()->m_midFrontLeftTireBoneName.str());
      DEBUG_ASSERTCRASH(
      ...
      m_midFrontRightTireBone = getRenderObject()->Get_Bone_Index(
          getW3DTankTruckDrawModuleData()->m_midFrontRightTireBoneName.str());
      DEBUG_ASSERTCRASH(
    ...
    if (!getW3DTankTruckDrawModuleData()->m_midRearLeftTireBoneName.isEmpty()) {
      m_midRearLeftTireBone = getRenderObject()->Get_Bone_Index(
          getW3DTankTruckDrawModuleData()->m_midRearLeftTireBoneName.str());
      DEBUG_ASSERTCRASH(
      ...
      m_midRearRightTireBone = getRenderObject()->Get_Bone_Index(
          getW3DTankTruckDrawModuleData()->m_midRearRightTireBoneName.str());
      DEBUG_ASSERTCRASH(
...
void W3DTankTruckDraw::updateTreadPositions(Real uvDelta) {
  ...
  for (Int i = 0; i < m_treadCount; i++) {
    ...
    else if (pTread->m_type ==
             TREAD_LEFT)  // this tread needs to scroll forwards
      offset_u = pTread->m_materialSettings.customUVOffset.X + uvDelta;
    else if (pTread->m_type ==
             TREAD_RIGHT)  // this tread needs to scroll backwards
      offset_u = pTread->m_materialSettings.customUVOffset.X - uvDelta;
...
void W3DTankTruckDraw::updateTreadObjects(void) {
  ...
  for (Int i = 0; i < m_treadCount; i++) REF_PTR_RELEASE(m_treads[i].m_robj);
  m_treadCount = 0;

  // Make sure this object has defined a speed for tread scrolling.
  if (getW3DTankTruckDrawModuleData() &&
      getW3DTankTruckDrawModuleData()->m_treadAnimationRate && robj) {
    for (Int i = 0;
         i < robj->Get_Num_Sub_Objects() && m_treadCount < MAX_TREADS_PER_TANK;
         i++) {
      ...
      if (subObj && subObj->Class_ID() == RenderObjClass::CLASSID_MESH &&
          subObj->Get_Name() &&
          ((meshName = strchr(subObj->Get_Name(), '.')) != 0 &&
           *(meshName++)) &&
          _strnicmp(meshName, "TREADS", 6) ==
              0) {  // check if sub-object has the correct material to do
                    // texture scrolling.
        ...
        if (mat) {
          for (Int j = 0; j < mat->Vertex_Material_Count(); j++) {
            ...
            if (mapper && mapper->Mapper_ID() ==
                              TextureMapperClass::MAPPER_ID_LINEAR_OFFSET) {
              mapper->Set_UV_Offset_Delta(
                  Vector2(0, 0));  // disable automatic scrolling
              subObj->Add_Ref();   // increase reference since we're storing the
                                  // pointer
              m_treads[m_treadCount].m_robj = subObj;
              m_treads[m_treadCount].m_type = TREAD_MIDDLE;  // default type
              subObj->Set_User_Data(
                  &m_treads[m_treadCount]
                       .m_materialSettings);  // tell W3D about custom material
                                              // settings
              m_treads[m_treadCount].m_materialSettings.customUVOffset =
                  Vector2(0, 0);
              // Commented out since on vehicles with wheels, it makes no sense
              // to turn with treads.
              /*							switch
                 (meshName[6])	//check next character after 'TREADS'
                                                                      {
                                                                              case
                 'L': case 'l':	m_treads[m_treadCount].m_type = TREAD_LEFT;
                                                                                              break;
                                                                              case
                 'R': case 'r':	m_treads[m_treadCount].m_type = TREAD_RIGHT;
                                                                                              break;
                                                                      }*/
              m_treadCount++;
            }
...
void W3DTankTruckDraw::doDrawModule(const Matrix3D *transformMtx) {
  ...
  if (!TheGlobalData->m_showClientPhysics) return;
  ...
  const TWheelInfo *wheelInfo =
      getDrawable()->getWheelInfo();  // note, can return null!
  if (wheelInfo && (m_frontLeftTireBone || m_rearLeftTireBone)) {
    static Real rotation = 0;
    const Real rotationFactor =
        getW3DTankTruckDrawModuleData()->m_rotationSpeedMultiplier;
    m_frontWheelRotation += rotationFactor * speed;
    if (m_isPowersliding) {
      m_rearWheelRotation +=
          rotationFactor *
          (speed +
           getW3DTankTruckDrawModuleData()->m_powerslideRotationAddition);
    } else {
  ...
  if (!wasPowersliding && m_isPowersliding) {
    ...
    m_powerslideSound.setPlayingHandle(
        TheAudio->addAudioEvent(&m_powerslideSound));
  ...
  Real velMag = vel->x * vel->x +
                vel->y * vel->y;  // only care about moving on the ground

  if (velMag > DEBRIS_THRESHOLD &&
      !getDrawable()->isDrawableEffectivelyHidden() &&
  ...
  if (velMult.x > 1.0f) velMult.x = 1.0f;

  velMult.y = velMult.x;

  velMult.z = velMag + 0.1f;
  if (velMult.z > 1.0f) velMult.z = 1.0f;
  ...
  if (m_treadCount) {
    Real offset_u;
    Real treadScrollSpeed =
        getW3DTankTruckDrawModuleData()->m_treadAnimationRate;
    TreadObjectInfo *pTread = m_treads;
    ...
    /* Commented out because these vehicles are presumed not to turn via treads.
                    ...
                    //For optimization sake, we only do complex tread scrolling
       when tank
                    //is mostly stationary and turning
                    if ((turn=physics->getTurning()) != TURN_NONE &&
       physics->getSpeed()/maxSpeed <
       getW3DTankTruckDrawModuleData()->m_treadPivotSpeedFraction)
                    {
    ...
    if (physics->isMotive() &&
        physics->getVelocityMagnitude() / maxSpeed >=
            getW3DTankTruckDrawModuleData()
                ->m_treadDriveSpeedFraction) {  // do simple scrolling based
                                                // only on speed when tank is
                                                // moving straight at high
                                                // speed.
      // we stop scrolling when tank slows down to reduce the appearance of
      // sliding tread scrolling speed was not directly tied into tank velocity
      // because it looked odd under certain situations when tank moved
      // sideways.
      for (Int i = 0; i < m_treadCount; i++) {
        offset_u =
            pTread->m_materialSettings.customUVOffset.X - treadScrollSpeed;
        // ensure coordinates of offset are in [0, 1] range:
        offset_u = offset_u - WWMath::Floor(offset_u);
        pTread->m_materialSettings.customUVOffset.Set(offset_u, 0);
        pTread++;
