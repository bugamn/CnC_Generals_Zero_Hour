...
W3DDebrisDraw::W3DDebrisDraw(Thing* thing, const ModuleData* moduleData)
  ...
  m_renderObject = NULL;
  for (int i = 0; i < STATECOUNT; ++i) m_anims[i] = NULL;
  m_fxFinal = NULL;
  m_state = INITIAL;
  m_frames = 0;
  m_shadow = NULL;
...
void W3DDebrisDraw::setModelName(AsciiString name, Color color, ShadowType t) {
  if (m_renderObject == NULL && !name.isEmpty()) {
    Int hexColor = 0;
    if (color != 0) hexColor = color | 0xFF000000;
    m_renderObject = W3DDisplay::m_assetManager->Create_Render_Obj(
        name.str(), getDrawable()->getScale(), hexColor);
    DEBUG_ASSERTCRASH(m_renderObject,
                      ("Debris model %s not found!\n", name.str()));
    if (m_renderObject) {
      ...
      Matrix3D transform;
      ///@todo: Change back to identity once we figure out why objects show up
      ///at 0,0,0
      /// OBJECT_PILE
...
//-------------------------------------------------------------------------------------------------
void W3DDebrisDraw::setAnimNames(AsciiString initial, AsciiString flying,
  ...
  m_anims[INITIAL] = initial.isEmpty()
                         ? NULL
                         : W3DDisplay::m_assetManager->Get_HAnim(initial.str());
  m_anims[FLYING] = flying.isEmpty()
                        ? NULL
                        : W3DDisplay::m_assetManager->Get_HAnim(flying.str());
  ...
  m_anims[FINAL] = final.isEmpty()
                       ? NULL
                       : W3DDisplay::m_assetManager->Get_HAnim(final.str());
  m_state = 0;
  m_frames = 0;
  m_fxFinal = finalFX;
...
void W3DDebrisDraw::doDrawModule(const Matrix3D* transformMtx) {
  if (m_renderObject) {
    ...
    if (getDrawable()->getInstanceScale() !=
        1.0f) {  // do custom scaling of the W3D model.
    ...
    static const RenderObjClass::AnimMode TheAnimModes[STATECOUNT] = {
        RenderObjClass::ANIM_MODE_ONCE, RenderObjClass::ANIM_MODE_LOOP,
        RenderObjClass::ANIM_MODE_ONCE};

    Int oldState = m_state;
    Object* obj = getDrawable()->getObject();
    const Int MIN_FINAL_FRAMES = 3;
    if (m_state != FINAL && obj != NULL && !obj->isAboveTerrain() &&
        m_frames > MIN_FINAL_FRAMES) {
      m_state = FINAL;
    ...
    if (hanim != NULL &&
        (hanim != m_renderObject->Peek_Animation() || oldState != m_state)) {
      ...
      if (m_state == FINAL) {
        ...
        if (m_finalStop) m = RenderObjClass::ANIM_MODE_MANUAL;
      }
