...
// Desc: Draw module reacts to SupplyStatus setting by hiding an equal number of
// the specified bone array.
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void W3DSupplyDrawModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"SupplyBonePrefix", INI::parseAsciiString, NULL,
       offsetof(W3DSupplyDrawModuleData, m_supplyBonePrefix)},
...
void W3DSupplyDraw::updateDrawModuleSupplyStatus(Int maxSupply,
                                                 Int currentSupply) {
  ...
  if (m_totalBones == -1) {
    m_totalBones = getDrawable()->getPristineBonePositions(
        boneName.str(), 1, NULL, NULL,
        INT_MAX);  // The last arg is to guard the size of the arrays.  I am not
                   // passing any in, I am just counting bones.
    m_lastNumberShown = m_totalBones;
  }

  // Figure the % of our bones we should show, and if it is a different % than
  // last time start showing and hiding them.
  ...
  if (bonesToShow != m_lastNumberShown) {
    // Show/hide the bones that are now different, the indices between last and
    // now (low, high].
