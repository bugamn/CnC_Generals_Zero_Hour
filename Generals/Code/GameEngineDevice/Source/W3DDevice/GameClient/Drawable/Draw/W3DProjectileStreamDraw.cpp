...
// //////////////////////////////////////////////////////////// Tile a texture
// strung between Projectiles Graham Smallwood, May 2002
/////////////////////////////////////////////////////////////////////////////////////////////////
...
void W3DProjectileStreamDrawModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"Texture", INI::parseAsciiString, NULL,
       offsetof(W3DProjectileStreamDrawModuleData, m_textureName)},
      {"Width", INI::parseReal, NULL,
       offsetof(W3DProjectileStreamDrawModuleData, m_width)},
      {"TileFactor", INI::parseReal, NULL,
       offsetof(W3DProjectileStreamDrawModuleData, m_tileFactor)},
      {"ScrollRate", INI::parseReal, NULL,
       offsetof(W3DProjectileStreamDrawModuleData, m_scrollRate)},
      {"MaxSegments", INI::parseInt, NULL,
       offsetof(W3DProjectileStreamDrawModuleData, m_maxSegments)},
      {0, 0, 0, 0}};
...
W3DProjectileStreamDraw::W3DProjectileStreamDraw(Thing *thing,
  ...
  const W3DProjectileStreamDrawModuleData *d =
      getW3DProjectileStreamDrawModuleData();
  m_texture =
      WW3DAssetManager::Get_Instance()->Get_Texture(d->m_textureName.str());
...
void W3DProjectileStreamDraw::setFullyObscuredByShroud(Bool fullyObscured) {
  if (fullyObscured) {  // we need to remove all our lines from the scene
                        // because they are hidden
    for (Int lineIndex = 0; lineIndex < m_linesValid; lineIndex++) {
      SegmentedLineClass *deadLine = m_allLines[lineIndex];
      if (deadLine && deadLine->Peek_Scene()) deadLine->Remove();
    }
  } else {  // we need to restore lines into scene
    for (Int lineIndex = 0; lineIndex < m_linesValid; lineIndex++) {
      SegmentedLineClass *deadLine = m_allLines[lineIndex];
...
void W3DProjectileStreamDraw::doDrawModule(const Matrix3D *) {
  ...
  ProjectileStreamUpdate *update =
      (ProjectileStreamUpdate *)me->findUpdateModule(
          key_ProjectileStreamUpdate);

  const W3DProjectileStreamDrawModuleData *data =
      getW3DProjectileStreamDrawModuleData();
  ...
  if (data->m_maxSegments) {
    // If I have a drawing cap, I need to increase the start point in the array.
    // The furthest (oldest) point from the tank is in spot zero.
    currentMasterPoint = pointsUsed - data->m_maxSegments;
    currentMasterPoint =
        max(0, currentMasterPoint);  // (but if they say to draw more than
                                     // exists, draw all)
  }

  // Okay.  I have an array of ordered points that may have blanks in it.  I
  // need to copy to the staging area until I hit a blank or the end.  Then if I
  // have a line made, I'll overwrite it, otherwise I'll make a new one. I'll
  // keep doing this until I run out of valid points.
  while (currentMasterPoint < pointsUsed) {
    while (currentMasterPoint < pointsUsed &&
           allPoints[currentMasterPoint] != zeroVector) {
      // While I am not looking at a bad point (off edge or zero)
      stagingPoints[currentStagingPoint] =
          allPoints[currentMasterPoint];  // copy to the staging
    ...
    currentMasterPoint++;  // I am either pointed off the edge anyway, or I am
                           // pointed at a zero I want to skip
...
void W3DProjectileStreamDraw::makeOrUpdateLine(Vector3 *points,
  ...
  if (newLine) {
    // This is one time stuff we only need to do if this is a new and not a
    // change
    const W3DProjectileStreamDrawModuleData *data =
        getW3DProjectileStreamDrawModuleData();
    line->Set_Texture(m_texture);  // set the texture
    line->Set_Shader(
        ShaderClass::_PresetAdditiveSpriteShader);  // pick the alpha blending
                                                    // mode you want - see
                                                    // shader.h for others.
    ...
    line->Set_Texture_Mapping_Mode(
        SegLineRendererClass::TILED_TEXTURE_MAP);  // this tiles the texture
                                                   // across the line
    line->Set_Texture_Tile_Factor(
        data->m_tileFactor);  // number of times to tile texture across each
                              // segment
    line->Set_UV_Offset_Rate(Vector2(
        0.0f, data->m_scrollRate));  // amount to scroll texture on each draw
    W3DDisplay::m_3DScene->Add_Render_Object(
        line);  // add it to our scene so it gets rendered with other objects.
  }
}
