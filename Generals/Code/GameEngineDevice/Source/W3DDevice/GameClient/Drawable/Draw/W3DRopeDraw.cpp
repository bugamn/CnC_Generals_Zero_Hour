...
void W3DRopeDraw::tossSegments() {
  ...
  for (std::vector<SegInfo>::iterator it = m_segments.begin();
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void W3DRopeDraw::initRopeParms(Real length, Real width, const RGBColor& color,
                                Real wobbleLen, Real wobbleAmp,
                                Real wobbleRate) {
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void W3DRopeDraw::setRopeCurLen(Real length) { m_curLen = length; }
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
W3DRopeDraw::~W3DRopeDraw() { tossSegments(); }
...
void W3DRopeDraw::doDrawModule(const Matrix3D* transformMtx) {
  ...
  if (!m_segments.empty()) {
    ...
    for (std::vector<SegInfo>::iterator it = m_segments.begin();
         it != m_segments.end(); ++it) {
      Vector3 end(pos->x + deflection * it->wobbleAxisX,
                  pos->y + deflection * it->wobbleAxisY, start.Z - eachLen);
      if (it->line) (it->line)->Reset(start, end);
      if (it->softLine) (it->softLine)->Reset(start, end);
      start = end;
    }
  }

  m_curWobblePhase += m_wobbleRate;
  if (m_curWobblePhase > 2 * PI) m_curWobblePhase -= 2 * PI;
...
void W3DRopeDraw::xfer(Xfer* xfer) {
  ...
  if (xfer->getXferMode() == XFER_LOAD) tossSegments();
