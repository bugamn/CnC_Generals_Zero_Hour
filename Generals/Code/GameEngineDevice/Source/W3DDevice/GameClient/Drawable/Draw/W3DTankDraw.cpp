...
void W3DTankDrawModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"TreadDebrisLeft", INI::parseAsciiString, NULL,
       offsetof(W3DTankDrawModuleData, m_treadDebrisNameLeft)},
      {"TreadDebrisRight", INI::parseAsciiString, NULL,
       offsetof(W3DTankDrawModuleData, m_treadDebrisNameRight)},
      {"TreadAnimationRate", INI::parseVelocityReal, NULL,
       offsetof(W3DTankDrawModuleData, m_treadAnimationRate)},
      {"TreadPivotSpeedFraction", INI::parseReal, NULL,
       offsetof(W3DTankDrawModuleData, m_treadPivotSpeedFraction)},
      {"TreadDriveSpeedFraction", INI::parseReal, NULL,
       offsetof(W3DTankDrawModuleData, m_treadDriveSpeedFraction)},
      {0, 0, 0, 0}};
...
void W3DTankDraw::setFullyObscuredByShroud(Bool fullyObscured) {
  if (fullyObscured != getFullyObscuredByShroud()) {
    if (fullyObscured) stopMoveDebris();
  }
...
void W3DTankDraw::updateTreadObjects(void) {
  ...
  if (getW3DTankDrawModuleData() &&
      getW3DTankDrawModuleData()->m_treadAnimationRate && robj) {
    for (Int i = 0;
...
//-------------------------------------------------------------------------------------------------
void W3DTankDraw::onRenderObjRecreated(void) { updateTreadObjects(); }
  ...
  if (m_treadCount) {
    ...
    if (turn != TURN_NONE &&
        physics->getVelocityMagnitude() / maxSpeed <
            getW3DTankDrawModuleData()->m_treadPivotSpeedFraction) {
      ...
      if (fabs(1.0f - angleToGoal) >
          0.00001f)  // check if difference in angle cosines is greater than
                     // some cutoff.
      {
    ...
    } else if (physics->isMotive() &&
               physics->getVelocityMagnitude() / maxSpeed >=
                   getW3DTankDrawModuleData()
                       ->m_treadDriveSpeedFraction) {  // do simple scrolling
                                                       // based only on speed
                                                       // when tank is moving
                                                       // straight at high
                                                       // speed.
...
void W3DTankDraw::loadPostProcess(void) {
  ...
  // toss any existing ones and re-create 'em (since this module expects 'em to
  // always be around)
  tossEmitters();
  createEmitters();
