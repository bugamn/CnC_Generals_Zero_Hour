...
// Desc: The Overlord has a super specific special need.  He needs his rider to
// draw explicitly after him, and he needs direct access to get that rider when
// everyone else can't see it because of the OverlordContain.
///////////////////////////////////////////////////////////////////////////////////////////////////
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
W3DOverlordTankDraw::W3DOverlordTankDraw(Thing *thing,
...
void W3DOverlordTankDraw::doDrawModule(const Matrix3D *transformMtx) {
  ...
  // Our big thing is that we get our specific passenger (the turret thing) and
  // then wake it up and make it draw It depends on us because our renderObject
  // is only made correct in the act of drawing.
  Object *me = getDrawable()->getObject();
  if (me && me->getContain() && me->getContain()->friend_getRider() &&
    ...
    riderDraw->draw(
        NULL);  // What the hell?  This param isn't used for anything
  }
}
...
void W3DOverlordTankDraw::setHidden(Bool h) {
  ...
  // We need to hide our rider, since he won't realize he's being contained in a
  // contained container
  Object *me = getDrawable()->getObject();
  if (me && me->getContain() && me->getContain()->friend_getRider() &&
      me->getContain()->friend_getRider()->getDrawable()) {
