...
// Desc: Draw module just like Model, except it can't draw unless somebody else
// explicitly says to, since they have to draw first.
//
// This draw module can be used in a general case (although I don't see why),
// m_attachToDrawableBoneInContainer is for the one present and main reason to
// use this module.  Our transport needs to tell us it is okay to draw after he
// draws.
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void W3DDependencyModelDrawModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"AttachToBoneInContainer", INI::parseAsciiString, NULL,
       offsetof(W3DDependencyModelDrawModuleData,
                m_attachToDrawableBoneInContainer)},
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
W3DDependencyModelDraw::W3DDependencyModelDraw(Thing *thing,
...
void W3DDependencyModelDraw::adjustTransformMtx(Matrix3D &mtx) const {
  ...
  // We have an additional adjustment to make, we want to use a bone in our
  // container if there is one
  const Object *me = getDrawable()->getObject();
  const W3DDependencyModelDrawModuleData *md =
      getW3DDependencyModelDrawModuleData();

  if (md->m_attachToDrawableBoneInContainer.isNotEmpty() && me &&
      me->getContainedBy() &&
      !me->getContainedBy()->getContain()->isEnclosingContainerFor(me)) {
    // If we are currently "riding on", then our client position is determined
    // by the client position of a particular bone in our container object.  Our
    // logic position is updated by OpenContain.
    ...
    if (theirDrawable) {
      ...
      if (theirDrawable->getCurrentWorldspaceClientBonePositions(
              md->m_attachToDrawableBoneInContainer.str(), theirBoneMtx)) {
        mtx = theirBoneMtx;
      } else {
        DEBUG_LOG(("m_attachToDrawableBoneInContainer %s not found\n",
                   getW3DDependencyModelDrawModuleData()
                       ->m_attachToDrawableBoneInContainer.str()));
      }
    }
  }
}
