...
// Desc:   W3DLaserDraw
// Updated: Kris Morness July 2002 -- made it data driven and added new features
// to make it flexible.
///////////////////////////////////////////////////////////////////////////////////////////////////
...
W3DLaserDrawModuleData::W3DLaserDrawModuleData() {
  ...
  m_numBeams = 1;  // Number of overlapping cylinders that make the beam. 1 beam
                   // will just use inner data.
  m_maxIntensityFrames =
      0;             // Laser stays at max intensity for specified time in ms.
...
void W3DLaserDrawModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"NumBeams", INI::parseUnsignedInt, NULL,
       offsetof(W3DLaserDrawModuleData, m_numBeams)},
      {"InnerBeamWidth", INI::parseReal, NULL,
       offsetof(W3DLaserDrawModuleData, m_innerBeamWidth)},
      {"OuterBeamWidth", INI::parseReal, NULL,
       offsetof(W3DLaserDrawModuleData, m_outerBeamWidth)},
      {"InnerColor", INI::parseColorInt, NULL,
       offsetof(W3DLaserDrawModuleData, m_innerColor)},
      {"OuterColor", INI::parseColorInt, NULL,
       offsetof(W3DLaserDrawModuleData, m_outerColor)},
      {"MaxIntensityLifetime", INI::parseDurationUnsignedInt, NULL,
       offsetof(W3DLaserDrawModuleData, m_maxIntensityFrames)},
      {"FadeLifetime", INI::parseDurationUnsignedInt, NULL,
       offsetof(W3DLaserDrawModuleData, m_fadeFrames)},
      {"Texture", INI::parseAsciiString, NULL,
       offsetof(W3DLaserDrawModuleData, m_textureName)},
      {"ScrollRate", INI::parseReal, NULL,
       offsetof(W3DLaserDrawModuleData, m_scrollRate)},
      {"Tile", INI::parseBool, NULL, offsetof(W3DLaserDrawModuleData, m_tile)},
      {"Segments", INI::parseUnsignedInt, NULL,
       offsetof(W3DLaserDrawModuleData, m_segments)},
      {"ArcHeight", INI::parseReal, NULL,
       offsetof(W3DLaserDrawModuleData, m_arcHeight)},
      {"SegmentOverlapRatio", INI::parseReal, NULL,
       offsetof(W3DLaserDrawModuleData, m_segmentOverlapRatio)},
      {"TilingScalar", INI::parseReal, NULL,
       offsetof(W3DLaserDrawModuleData, m_tilingScalar)},
      {0, 0, 0, 0}};
...
W3DLaserDraw::W3DLaserDraw(Thing *thing, const ModuleData *moduleData)
  ...
  m_texture =
      WW3DAssetManager::Get_Instance()->Get_Texture(data->m_textureName.str());
  if (m_texture) {
  ...
  Real innerRed, innerGreen, innerBlue, innerAlpha, outerRed, outerGreen,
      outerBlue, outerAlpha;
  GameGetColorComponentsReal(data->m_innerColor, &innerRed, &innerGreen,
                             &innerBlue, &innerAlpha);
  GameGetColorComponentsReal(data->m_outerColor, &outerRed, &outerGreen,
                             &outerBlue, &outerAlpha);

  // Make sure our beams range between 1 and the maximum cap.
#ifdef I_WANT_TO_BE_FIRED
  // srj sez: this data is const for a reason. casting away the constness
  // because we don't like the values isn't an acceptable solution. if you need
  // to constrain the values, do so at parsing time, when it's still legal to
  // modify these values. (In point of fact, there's not even really any reason
  // to limit the numBeams or segments anymore.)
  ...
  for (int segment = 0; segment < data->m_segments; segment++) {
    // We don't care about segment positioning yet until we actually set the
    // position
    ...
    for (i = data->m_numBeams - 1; i >= 0; i--) {
      ...
      if (data->m_numBeams == 1) {
        ...
        width = data->m_innerBeamWidth +
                scale * (data->m_outerBeamWidth - data->m_innerBeamWidth);
      ...
      if (line) {
        ...
        line->Set_Shader(
            ShaderClass::_PresetAdditiveShader);  // pick the alpha blending
                                                  // mode you want - see
                                                  // shader.h for others.
        line->Set_Width(width);
        line->Set_Color(Vector3(red, green, blue));
        line->Set_UV_Offset_Rate(Vector2(
            0.0f,
            data->m_scrollRate));  // amount to scroll texture on each draw
        if (m_texture) {
          line->Set_Texture_Mapping_Mode(
              SegLineRendererClass::
                  TILED_TEXTURE_MAP);  // this tiles the texture across the line
        }

        // add to scene
        W3DDisplay::m_3DScene->Add_Render_Object(
            line);  // add it to our scene so it gets rendered with other
                    // objects.
...
void W3DLaserDraw::doDrawModule(const Matrix3D *transformMtx) {
  ...
  if (!update) {
    DEBUG_ASSERTCRASH(
        0, ("W3DLaserDraw::doDrawModule() expects its owner drawable %s to "
            "have a ClientUpdate = LaserUpdate module.",
  ...
  if (update->isDirty() || m_selfDirty) {
    ...
    for (int segment = 0; segment < data->m_segments; segment++) {
      if (data->m_arcHeight > 0.0f && data->m_segments > 1) {
        // CALCULATE A CURVED LINE BASED ON TOTAL LENGTH AND DESIRED HEIGHT
        // INCREASE To do this we will use a portion of the cos wave ranging
        // between -0.25PI and +0.25PI. 0PI is 1.0 and 0.25PI is 0.70 --
        // resulting in a somewhat gentle curve depending on the line height and
        // length. We also have to make the line *level* for this phase of the
        // calculations.
        ...
        // This is critical -- in the case we have sloped lines (at the end,
        // we'll fix it)
        ...
        //--------------------------------------------------------------------------------
        // Now at this point, we have our segment line in the level positions
        // that we want. Calculate the raised height for the start/end segment
        // positions using cosine.
        //--------------------------------------------------------------------------------
        ...
        laserPoints[0].Set(
            ...
            MAX(segmentStart.z, 2.0f + TheTerrainLogic->getGroundHeight(
                                           segmentStart.x, segmentStart.y)));
        laserPoints[1].Set(
            segmentEnd.x, segmentEnd.y,
            MAX(segmentEnd.z, 2.0f + TheTerrainLogic->getGroundHeight(
                                         segmentEnd.x, segmentEnd.y)));

      } else {
        // No arc -- way simpler!
        laserPoints[0].Set(update->getStartPos()->x, update->getStartPos()->y,
                           update->getStartPos()->z);
        laserPoints[1].Set(update->getEndPos()->x, update->getEndPos()->y,
                           update->getEndPos()->z);
      }
      ...
      for (Int i = data->m_numBeams - 1; i >= 0; i--) {
        ...
        // Calculate the number of times to tile the line based on the height of
        // the texture used.
        if (m_texture && data->m_tile) {
          ...
          // Adjust tile factor so texture is NOT stretched but tiled equally in
          // both width and length.
          Real tileFactor =
              length / width * m_textureAspectRatio * data->m_tilingScalar;

          // Set the tile factor
          m_line3D[index]->Set_Texture_Tile_Factor(
              tileFactor);  // number of times to tile texture across each
                            // segment
        }
