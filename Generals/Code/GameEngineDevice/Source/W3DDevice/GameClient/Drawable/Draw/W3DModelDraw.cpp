...
class LogClass {
 public:
  ...
  void dumpMatrix3D(const Matrix3D* m, AsciiString name, AsciiString fname,
                    Int line);
...
void LogClass::log(const char* fmt, ...) {
  DEBUG_ASSERTCRASH(isValidTimeToCalcLogicStuff(),
                    ("Calc'ing logic bone pos in client!!!"));
  if (!m_fp /*|| !isValidTimeToCalcLogicStuff()*/) return;
...
void LogClass::dumpMatrix3D(const Matrix3D* m, AsciiString name,
                            AsciiString fname, Int line) {
  fname.toLower();
  ...
  for (Int i = 0; i < 3; ++i)
    log("      0x%08X 0x%08X 0x%08X 0x%08X\n", AS_INT(matrix[(i << 2) + 0]),
        AS_INT(matrix[(i << 2) + 1]), AS_INT(matrix[(i << 2) + 2]),
        AS_INT(matrix[(i << 2) + 3]));
}
...
#define BONEPOS_DUMPMATRIX3DNAMED(x, y) \
  BonePosLog.dumpMatrix3D(x, y, __FILE__, __LINE__)
#define BONEPOS_DUMPREAL(x) BONEPOS_DUMPREALNAMED(x, #x)
#define BONEPOS_DUMPREALNAMED(x, y) \
  BonePosLog.dumpReal(x, y, __FILE__, __LINE__)
...
// flags that aren't read directly from INI, but set in response to various
// other situations
...
static const char* ACBitsNames[] = {"RANDOMSTART",
                                    "START_FRAME_FIRST",
                                    "START_FRAME_LAST",
                                    "ADJUST_HEIGHT_BY_CONSTRUCTION_PERCENT",
                                    "PRISTINE_BONE_POS_IN_FINAL_FRAME",
...
static const Int ALL_MAINTAIN_FRAME_FLAGS =
    (1 << MAINTAIN_FRAME_ACROSS_STATES) | (1 << MAINTAIN_FRAME_ACROSS_STATES2) |
    (1 << MAINTAIN_FRAME_ACROSS_STATES3) | (1 << MAINTAIN_FRAME_ACROSS_STATES4);
...
//-------------------------------------------------------------------------------------------------
W3DAnimationInfo::W3DAnimationInfo(const AsciiString& name, Bool isIdle,
...
//-------------------------------------------------------------------------------------------------
// note that this now returns an ADDREFED handle, which must be released by the
// caller!
HAnimClass* W3DAnimationInfo::getAnimHandle() const {
  ...
  if (m_handle == NULL) {
    ...
    DEBUG_ASSERTCRASH(
        m_handle, ("*** ASSET ERROR: animation %s not found\n", m_name.str()));
    if (m_handle) {
      m_naturalDurationInMsec =
          m_handle->Get_Num_Frames() * 1000.0f / m_handle->Get_Frame_Rate();
    }
  }
  ...
  DEBUG_ASSERTCRASH(
      handle, ("*** ASSET ERROR: animation %s not found\n", m_name.str()));
  if (handle != NULL && m_naturalDurationInMsec < 0) {
    m_naturalDurationInMsec =
        handle->Get_Num_Frames() * 1000.0f / handle->Get_Frame_Rate();
  }
...
void ModelConditionInfo::addPublicBone(const AsciiString& boneName) const {
  if (boneName.isEmpty() || boneName.isNone()) return;

  AsciiString tmp = boneName;
  tmp.toLower();
  if (std::find(m_publicBones.begin(), m_publicBones.end(), tmp) ==
      m_publicBones.end()) {
...
Bool ModelConditionInfo::matchesMode(Bool night, Bool snowy) const {
  for (std::vector<ModelConditionFlags>::const_iterator it =
           m_conditionsYesVec.begin();
       it != m_conditionsYesVec.end(); ++it) {
...
static Bool findSingleBone(RenderObjClass* robj, const AsciiString& boneName,
  ...
  if (boneName.isNone() || boneName.isEmpty()) return false;
  ...
  if (childObject) {
    ...
    for (Int subObj = 0; subObj < robj->Get_Num_Sub_Objects(); subObj++) {
      ...
      if (test == childObject) {
        ...
        DEBUG_ASSERTCRASH(test != NULL && test == childObject,
                          ("*** ASSET ERROR: Hmm, bone problem"));
#endif
      }
...
//-------------------------------------------------------------------------------------------------
static Bool doSingleBoneName(RenderObjClass* robj, const AsciiString& boneName,
  ...
  boneNameTmp
      .toLower();  // convert to all-lowercase to avoid case sens issues later
...
//-------------------------------------------------------------------------------------------------
void ModelConditionInfo::validateStuff(
    RenderObjClass* robj, Real scale,
    const std::vector<AsciiString>& extraPublicBones) const {
  // srj sez: hm, this doesn't make sense; I think we really do need to validate
  // transition states.
  ...
  if (!(m_validStuff & PUBLIC_BONES_VALID) && isValidTimeToCalcLogicStuff()) {
    for (std::vector<AsciiString>::const_iterator bone_it =
             extraPublicBones.begin();
         bone_it != extraPublicBones.end(); ++bone_it) {
...
//-------------------------------------------------------------------------------------------------
void ModelConditionInfo::validateCachedBones(RenderObjClass* robj,
                                             Real scale) const {
  // DEBUG_ASSERTCRASH(isValidTimeToCalcLogicStuff(), ("calling
  // validateCachedBones() from in GameClient!"));
  if (m_validStuff & PRISTINE_BONES_VALID) return;
  ...
  BONEPOS_LOG(("Validating bones for %s: %s", m_modelName.str(),
               getDescription().str()));
  ...
  if (robj == NULL) {
    ...
    robj = W3DDisplay::m_assetManager->Create_Render_Obj(m_modelName.str(),
                                                         scale, 0);
    DEBUG_ASSERTCRASH(
        robj, ("*** ASSET ERROR: Model %s not found!\n", m_modelName.str()));
    if (!robj) {
  ...
  // if we have any animations in this state, always choose the first, since the
  // animations vary on a per-client basis.
  HAnimClass* animToUse;
  if (m_animations.size() > 0) {
    animToUse =
        m_animations.front().getAnimHandle();  // return an AddRef'ed handle
  } else {
    animToUse = curAnim;  // Peek_Animation_And_Info does not addref, so we must
                          // do so here
    if (animToUse) animToUse->Add_Ref();
  }
  if (animToUse != NULL) {
    // make sure we're in frame zero.
    Int whichFrame = testFlagBit(m_flags, PRISTINE_BONE_POS_IN_FINAL_FRAME)
                         ? animToUse->Get_Num_Frames() - 1
                         : 0;
    robj->Set_Animation(animToUse, whichFrame,
  ...
  if (TheGlobalData) {
    for (std::vector<AsciiString>::const_iterator it =
             TheGlobalData->m_standardPublicBones.begin();
         it != TheGlobalData->m_standardPublicBones.end(); ++it) {
      if (!doSingleBoneName(robj, *it, m_pristineBones)) {
        // don't crash here, since these are catch-all global bones and won't be
        // present in most models.
        // DEBUG_CRASH(("public bone %s (and variations thereof) not found in
        // model %s!\n",it->str(),m_modelName.str()));
      }
      // else
      //{
      //	DEBUG_LOG(("global bone %s (or variations thereof) found in
      //model %s\n",it->str(),m_modelName.str()));
      // }
    }
  }
  for (std::vector<AsciiString>::const_iterator it = m_publicBones.begin();
       it != m_publicBones.end(); ++it) {
    if (!doSingleBoneName(robj, *it, m_pristineBones)) {
      // DO crash here, since we specifically requested this bone for this model
      DEBUG_CRASH(
          ("*** ASSET ERROR: public bone '%s' (and variations thereof) not "
           "found in model %s!\n",
           it->str(), m_modelName.str()));
    }
    // else
    //{
    //	DEBUG_LOG(("extra bone %s (or variations thereof) found in model
    //%s\n",it->str(),m_modelName.str()));
    // }
  }
...
void ModelConditionInfo::validateWeaponBarrelInfo() const {
  // DEBUG_ASSERTCRASH(isValidTimeToCalcLogicStuff(), ("calling
  // validateWeaponBarrelInfo() from in GameClient!"));
  if (m_validStuff & BARRELS_VALID) return;
  ...
  for (int wslot = 0; wslot < WEAPONSLOT_COUNT; ++wslot) {
    ...
    if (fxBoneName.isNotEmpty() || recoilBoneName.isNotEmpty() ||
        mfName.isNotEmpty() || plbName.isNotEmpty()) {
      Int prevFxBone = 0;
      char buffer[256];
      for (Int i = 1; i <= 99; ++i) {
...
#if defined(_DEBUG) || defined(_INTERNAL)
          if (info.m_muzzleFlashBone) info.m_muzzleFlashBoneName = buffer;
#endif
        }
        if (!fxBoneName.isEmpty()) {
          sprintf(buffer, "%s%02d", fxBoneName.str(), i);
          findPristineBone(NAMEKEY(buffer), &info.m_fxBone);
          // special case: if we have multiple muzzleflashes, but only one
          // fxbone, use that fxbone for everything.
        ...
        if (!plbName.isEmpty()) {
          ...
          const Matrix3D* mtx =
              findPristineBone(NAMEKEY(buffer), &plbBoneIndex);
          if (mtx != NULL) info.m_projectileOffsetMtx = *mtx;
        }

        if (info.m_fxBone == 0 && info.m_recoilBone == 0 &&
            info.m_muzzleFlashBone == 0 && plbBoneIndex == 0)
          break;
      ...
      if (m_weaponBarrelInfoVec[wslot].empty()) {
...
#if defined(_DEBUG) || defined(_INTERNAL)
        if (info.m_muzzleFlashBone) info.m_muzzleFlashBoneName = mfName;
#endif

        const Matrix3D* plbMtx =
            plbName.isEmpty() ? NULL : findPristineBone(NAMEKEY(plbName), NULL);
        if (plbMtx != NULL)
        ...
        if (info.m_fxBone != 0 || info.m_recoilBone != 0 ||
            info.m_muzzleFlashBone != 0 || plbMtx != NULL) {
          CRCDEBUG_LOG(
              ("validateWeaponBarrelInfo() - model name %s (unadorned) wslot "
               "%d\n",
               m_modelName.str(), wslot));
          DUMPMATRIX3D(&(info.m_projectileOffsetMtx));
          BONEPOS_LOG(
          ...
          CRCDEBUG_LOG(
              ("validateWeaponBarrelInfo() - model name %s (unadorned) found "
               "nothing\n",
               m_modelName.str()));
          BONEPOS_LOG(
              ("validateWeaponBarrelInfo() - model name %s (unadorned) found "
      ...
      DEBUG_ASSERTCRASH(
          !(m_modelName.isNotEmpty() && m_weaponBarrelInfoVec[wslot].empty()),
...
void ModelConditionInfo::validateTurretInfo() const {
  // DEBUG_ASSERTCRASH(isValidTimeToCalcLogicStuff(), ("calling
  // validateTurretInfo() from in GameClient!"));
  if (m_validStuff & TURRETS_VALID) return;
  ...
  for (int tslot = 0; tslot < MAX_TURRETS; ++tslot) {
    ...
    if (tur.m_turretAngleNameKey != NAMEKEY_INVALID) {
      if (findPristineBone(tur.m_turretAngleNameKey, &tur.m_turretAngleBone) ==
          NULL) {
        DEBUG_CRASH(("*** ASSET ERROR: TurretBone %s not found! (%s)\n",
                     KEYNAME(tur.m_turretAngleNameKey).str(),
                     m_modelName.str()));
        tur.m_turretAngleBone = 0;
      }
    } else {
    ...
    if (tur.m_turretPitchNameKey != NAMEKEY_INVALID) {
      if (findPristineBone(tur.m_turretPitchNameKey, &tur.m_turretPitchBone) ==
          NULL) {
        DEBUG_CRASH(("*** ASSET ERROR: TurretBone %s not found! (%s)\n",
                     KEYNAME(tur.m_turretPitchNameKey).str(),
                     m_modelName.str()));
        tur.m_turretPitchBone = 0;
      }
    } else {
...
const Matrix3D* ModelConditionInfo::findPristineBone(NameKeyType boneName,
                                                     Int* boneIndex) const {
  DEBUG_ASSERTCRASH((m_validStuff & PRISTINE_BONES_VALID),
                    ("*** ASSET ERROR: bones are not valid"));
  ...
  if (it != m_pristineBones.end()) {
    if (boneIndex) *boneIndex = it->second.boneIndex;
...
#ifdef RETAIN_ANIM_HANDLES
  for (W3DAnimationVector::const_iterator it2 = m_animations.begin();
       it2 != m_animations.end(); ++it2) {
    HAnimClass* h = it2->getAnimHandle();  // just force it to get loaded
    REF_PTR_RELEASE(h);
    h = NULL;
  }
#else
  // srj sez: I think there is no real reason to preload these all anymore.
  // things that need the anims (for bones) will force an implicit load anyway,
  // but there's no point in forcibly loading the anims that don't contain
  // interesting logical bones.
#endif
}
...
//-------------------------------------------------------------------------------------------------
void W3DModelDrawModuleData::validateStuffForTimeAndWeather(
    const Drawable* draw, Bool night, Bool snowy) const {
  if (!isValidTimeToCalcLogicStuff()) return;
  ...
  if (m_validated & mode) return;

  m_validated |= mode;

  for (ModelConditionVector::iterator c_it = m_conditionStates.begin();
       c_it != m_conditionStates.end(); ++c_it) {
  ...
  for (TransitionMap::iterator t_it = m_transitionMap.begin();
       t_it != m_transitionMap.end(); ++t_it) {
    // here's the tricky part: only want to load the anims for this one if there
    // is at least one source AND at least one dest state that matches the
    // current mode
    ...
    Bool a = false;
    Bool b = false;
    for (c_it = m_conditionStates.begin(); c_it != m_conditionStates.end();
         ++c_it) {
    ...
    if (a && b) {
      ...
      // srj sez: hm, this doesn't make sense; I think we really do need to
...
void W3DModelDrawModuleData::preloadAssets(TimeOfDay timeOfDay,
  ...
  for (ModelConditionVector::iterator it = m_conditionStates.begin();
       it != m_conditionStates.end(); ++it) {
    it->preloadAssets(timeOfDay, scale);
  }
}

//-------------------------------------------------------------------------------------------------
AsciiString W3DModelDrawModuleData::getBestModelNameForWB(
    const ModelConditionFlags& c) const {
  const ModelConditionInfo* info = findBestInfo(c);
  if (info) return info->m_modelName;
  return AsciiString::TheEmptyString;
}

#ifdef CACHE_ATTACH_BONE
//-------------------------------------------------------------------------------------------------
const Vector3* W3DModelDrawModuleData::getAttachToDrawableBoneOffset(
    const Drawable* draw) const {
  if (m_attachToDrawableBone.isEmpty()) {
    ...
    if (!m_attachToDrawableBoneOffsetValid) {
      ...
      Matrix3D boneMtx;
      if (draw->getPristineBonePositions(m_attachToDrawableBone.str(), 0, NULL,
                                         &boneMtx, 1) == 1) {
...
//-------------------------------------------------------------------------------------------------
static void parseAsciiStringLC(INI* ini, void* /*instance*/, void* store,
                               const void* /*userData*/) {
...
void W3DModelDrawModuleData::buildFieldParse(MultiIniFieldParse& p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"InitialRecoilSpeed", INI::parseVelocityReal, NULL,
       offsetof(W3DModelDrawModuleData, m_initialRecoil)},
      {"MaxRecoilDistance", INI::parseReal, NULL,
       offsetof(W3DModelDrawModuleData, m_maxRecoil)},
      {"RecoilDamping", INI::parseReal, NULL,
       offsetof(W3DModelDrawModuleData, m_recoilDamping)},
      {"RecoilSettleSpeed", INI::parseVelocityReal, NULL,
       offsetof(W3DModelDrawModuleData, m_recoilSettle)},
      {"OkToChangeModelColor", INI::parseBool, NULL,
       offsetof(W3DModelDrawModuleData, m_okToChangeModelColor)},
      {"AnimationsRequirePower", INI::parseBool, NULL,
       offsetof(W3DModelDrawModuleData, m_animationsRequirePower)},
      {"MinLODRequired", INI::parseStaticGameLODLevel, NULL,
       offsetof(W3DModelDrawModuleData, m_minLODRequired)},
      {"ProjectileBoneFeedbackEnabledSlots", INI::parseBitString32,
       TheWeaponSlotTypeNames,
       offsetof(W3DModelDrawModuleData, m_projectileBoneFeedbackEnabledSlots)},
      {"DefaultConditionState", W3DModelDrawModuleData::parseConditionState,
       (void*)PARSE_DEFAULT, 0},
      {"ConditionState", W3DModelDrawModuleData::parseConditionState,
       (void*)PARSE_NORMAL, 0},
      {"AliasConditionState", W3DModelDrawModuleData::parseConditionState,
       (void*)PARSE_ALIAS, 0},
      {"TransitionState", W3DModelDrawModuleData::parseConditionState,
       (void*)PARSE_TRANSITION, 0},
      {"TrackMarks", parseAsciiStringLC, NULL,
       offsetof(W3DModelDrawModuleData, m_trackFile)},
      {"ExtraPublicBone", INI::parseAsciiStringVectorAppend, NULL,
       offsetof(W3DModelDrawModuleData, m_extraPublicBones)},
      {"AttachToBoneInAnotherModule", parseAsciiStringLC, NULL,
       offsetof(W3DModelDrawModuleData, m_attachToDrawableBone)},
      {"IgnoreConditionStates", ModelConditionFlags::parseFromINI, NULL,
       offsetof(W3DModelDrawModuleData, m_ignoreConditionStates)},
      {0, 0, 0, 0}};
  p.add(dataFieldParse);
}

//-------------------------------------------------------------------------------------------------
enum AnimParseType { ANIM_NORMAL, ANIM_IDLE };

//-------------------------------------------------------------------------------------------------
static void parseAnimation(INI* ini, void* instance, void* /*store*/,
                           const void* userData) {
  ...
  Real distanceCovered =
      distanceCoveredToken ? INI::scanReal(distanceCoveredToken) : 0;
  DEBUG_ASSERTCRASH(
      !(animType == ANIM_IDLE && distanceCovered != 0),
  ...
  if (self->m_iniReadFlags & (1 << ANIMS_COPIED_FROM_DEFAULT_STATE)) {
    self->m_iniReadFlags &= ~((1 << ANIMS_COPIED_FROM_DEFAULT_STATE) |
                              (1 << GOT_IDLE_ANIMS) | (1 << GOT_NONIDLE_ANIMS));
  ...
  if (!animName.isEmpty() && !animName.isNone()) {
    while (timesToRepeat--) self->m_animations.push_back(animInfo);
  }
}

//-------------------------------------------------------------------------------------------------
static void parseShowHideSubObject(INI* ini, void* instance, void* store,
                                   const void* userData) {
  std::vector<ModelConditionInfo::HideShowSubObjInfo>* vec =
      (std::vector<ModelConditionInfo::HideShowSubObjInfo>*)store;
  ...
  while (subObjName.isNotEmpty()) {
    Bool found = false;
    for (std::vector<ModelConditionInfo::HideShowSubObjInfo>::iterator it =
             vec->begin();
         it != vec->end(); ++it) {
...
void W3DModelDraw::showSubObject(const AsciiString& name, Bool show) {
  if (name.isNotEmpty()) {
    Bool found = false;
    for (std::vector<ModelConditionInfo::HideShowSubObjInfo>::iterator it =
             m_subObjectVec.begin();
         it != m_subObjectVec.end(); ++it) {
...
//-------------------------------------------------------------------------------------------------
static void parseWeaponBoneName(INI* ini, void* instance, void* store,
                                const void* /*userData*/) {
  ModelConditionInfo* self = (ModelConditionInfo*)instance;
  AsciiString* arr = (AsciiString*)store;

  WeaponSlotType wslot = (WeaponSlotType)INI::scanIndexList(
      ini->getNextToken(), TheWeaponSlotTypeNames);
  arr[wslot] = ini->getNextAsciiString();
  arr[wslot].toLower();
  if (arr[wslot].isNone()) arr[wslot].clear();

  if (self) self->addPublicBone(arr[wslot]);
}

//-------------------------------------------------------------------------------------------------
static void parseParticleSysBone(INI* ini, void* instance, void* store,
                                 const void* /*userData*/) {
  ParticleSysBoneInfo info;
  info.boneName = ini->getNextAsciiString();
  info.boneName.toLower();
  ini->parseParticleSystemTemplate(ini, instance,
                                   &(info.particleSystemTemplate), NULL);
  ModelConditionInfo* self = (ModelConditionInfo*)instance;
  self->m_particleSysBones.push_back(info);
}

//-------------------------------------------------------------------------------------------------
static void parseRealRange(INI* ini, void* instance, void* store,
                           const void* /*userData*/) {
  ModelConditionInfo* self = (ModelConditionInfo*)instance;
...
//-------------------------------------------------------------------------------------------------
static void parseLowercaseNameKey(INI* ini, void* instance, void* store,
                                  const void* /*userData*/) {
...
//-------------------------------------------------------------------------------------------------
static void parseBoneNameKey(INI* ini, void* instance, void* store,
                             const void* /*userData*/) {
  ModelConditionInfo* self = (ModelConditionInfo*)instance;
  NameKeyType* key = (NameKeyType*)store;
  ...
  if (self) self->addPublicBone(tmp);
...
//-------------------------------------------------------------------------------------------------
static Bool doesStateExist(const ModelConditionVector& v,
                           const ModelConditionFlags& f) {
  for (ModelConditionVector::const_iterator it = v.begin(); it != v.end();
       ++it) {
    for (Int i = it->getConditionsYesCount() - 1; i >= 0; --i) {
      if (f == it->getNthConditionsYes(i)) return true;
    }
  }
  return false;
}
...
//-------------------------------------------------------------------------------------------------
void W3DModelDrawModuleData::parseConditionState(INI* ini, void* instance,
                                                 void* /*store*/,
                                                 const void* userData) {
  static const FieldParse myFieldParse[] = {
      {"Model", parseAsciiStringLC, NULL,
       offsetof(ModelConditionInfo, m_modelName)},
      {"Turret", parseBoneNameKey, NULL,
       offsetof(ModelConditionInfo, m_turrets[0].m_turretAngleNameKey)},
      {"TurretArtAngle", INI::parseAngleReal, NULL,
       offsetof(ModelConditionInfo, m_turrets[0].m_turretArtAngle)},
      {"TurretPitch", parseBoneNameKey, NULL,
       offsetof(ModelConditionInfo, m_turrets[0].m_turretPitchNameKey)},
      {"TurretArtPitch", INI::parseAngleReal, NULL,
       offsetof(ModelConditionInfo, m_turrets[0].m_turretArtPitch)},
      {"AltTurret", parseBoneNameKey, NULL,
       offsetof(ModelConditionInfo, m_turrets[1].m_turretAngleNameKey)},
      {"AltTurretArtAngle", INI::parseAngleReal, NULL,
       offsetof(ModelConditionInfo, m_turrets[1].m_turretArtAngle)},
      {"AltTurretPitch", parseBoneNameKey, NULL,
       offsetof(ModelConditionInfo, m_turrets[1].m_turretPitchNameKey)},
      {"AltTurretArtPitch", INI::parseAngleReal, NULL,
       offsetof(ModelConditionInfo, m_turrets[1].m_turretArtPitch)},
      {"ShowSubObject", parseShowHideSubObject, (void*)0,
       offsetof(ModelConditionInfo, m_hideShowVec)},
      {"HideSubObject", parseShowHideSubObject, (void*)1,
       offsetof(ModelConditionInfo, m_hideShowVec)},
      {"WeaponFireFXBone", parseWeaponBoneName, NULL,
       offsetof(ModelConditionInfo, m_weaponFireFXBoneName[0])},
      {"WeaponRecoilBone", parseWeaponBoneName, NULL,
       offsetof(ModelConditionInfo, m_weaponRecoilBoneName[0])},
      {"WeaponMuzzleFlash", parseWeaponBoneName, NULL,
       offsetof(ModelConditionInfo, m_weaponMuzzleFlashName[0])},
      {"WeaponLaunchBone", parseWeaponBoneName, NULL,
       offsetof(ModelConditionInfo, m_weaponProjectileLaunchBoneName[0])},
      {"WeaponHideShowBone", parseWeaponBoneName, NULL,
       offsetof(ModelConditionInfo, m_weaponProjectileHideShowName[0])},
      {"Animation", parseAnimation, (void*)ANIM_NORMAL,
       offsetof(ModelConditionInfo, m_animations)},
      {"IdleAnimation", parseAnimation, (void*)ANIM_IDLE,
       offsetof(ModelConditionInfo, m_animations)},
      {"AnimationMode", INI::parseIndexList, TheAnimModeNames,
       offsetof(ModelConditionInfo, m_mode)},
      {"TransitionKey", parseLowercaseNameKey, NULL,
       offsetof(ModelConditionInfo, m_transitionKey)},
      {"WaitForStateToFinishIfPossible", parseLowercaseNameKey, NULL,
       offsetof(ModelConditionInfo, m_allowToFinishKey)},
      {"Flags", INI::parseBitString32, ACBitsNames,
       offsetof(ModelConditionInfo, m_flags)},
  ...
  switch (cst) {
    case PARSE_DEFAULT: {
      if (self->m_defaultState >= 0) {
        DEBUG_CRASH(
            ("*** ASSET ERROR: you may have only one default state!\n"));
        ...
        if (!self->m_conditionStates.empty()) {
          DEBUG_CRASH(
              ("*** ASSET ERROR: when using DefaultConditionState, it must be "
               "the first state listed (%s)\n",
               TheThingTemplateBeingParsedName.str()));
          throw INI_INVALID_DATA;
        }

        // note, this is size(), not size()-1, since we haven't actually
        // modified the list yet
    ...
    case PARSE_TRANSITION: {
      ...
      if (firstKey == secondKey) {
        DEBUG_CRASH(
            ("*** ASSET ERROR: You may not declare a transition between two "
             "identical states\n"));
        throw INI_INVALID_DATA;
      }
    ...
    case PARSE_ALIAS: {
      if (self->m_conditionStates.empty()) {
        DEBUG_CRASH(
            ("*** ASSET ERROR: AliasConditionState must refer to the previous "
             "state!\n"));
        throw INI_INVALID_DATA;
      }

      ModelConditionInfo& prevState =
          self->m_conditionStates.at(self->m_conditionStates.size() - 1);
      ...
      if (conditionsYes.anyIntersectionWith(self->m_ignoreConditionStates)) {
        DEBUG_CRASH(
            ("You should not specify bits in a state once they are used in "
             "IgnoreConditionStates (%s)\n",
             TheThingTemplateBeingParsedName.str()));
        throw INI_INVALID_DATA;
      }
      ...
      if (doesStateExist(self->m_conditionStates, conditionsYes)) {
        DEBUG_CRASH(
            ("*** ASSET ERROR: duplicate condition states are not currently "
             "allowed"));
        throw INI_INVALID_DATA;
      }
      ...
      if (!conditionsYes.any() && self->m_defaultState >= 0) {
        DEBUG_CRASH(
            ("*** ASSET ERROR: you may not specify both a Default state and a "
    ...
    case PARSE_NORMAL: {
      ...
      // no, we do not currently require a default state, cuz it would break the
      // exiting INI files too badly. maybe someday.
      //	else
      //	{
      //		DEBUG_CRASH(("*** ASSET ERROR: you must specify a
      //default state\n")); 		throw INI_INVALID_DATA;
      //	}
      ...
      if (self->m_defaultState < 0 && self->m_conditionStates.empty() &&
          conditionsYes.any()) {
        // it doesn't actually NEED to be first, but it does need to be present,
        // and this is the simplest way to enforce...
        DEBUG_CRASH(
            ("*** ASSET ERROR: when not using DefaultConditionState, the first "
             "ConditionState must be for NONE (%s)\n",
             TheThingTemplateBeingParsedName.str()));
        throw INI_INVALID_DATA;
      }
      ...
      DEBUG_ASSERTCRASH(
          info.m_conditionsYesVec.size() == 0,
          ("*** ASSET ERROR: nonempty m_conditionsYesVec.size(), see srj"));
      info.m_conditionsYesVec.clear();
  ...
  if ((info.m_iniReadFlags & (1 << GOT_IDLE_ANIMS)) &&
    ...
    DEBUG_CRASH(
        ("*** ASSET ERROR: you should not specify both Animations and "
         "IdleAnimations for the same state"));
    throw INI_INVALID_DATA;
  }

  if ((info.m_iniReadFlags & (1 << GOT_IDLE_ANIMS)) &&
      (info.m_mode != RenderObjClass::ANIM_MODE_ONCE &&
       info.m_mode != RenderObjClass::ANIM_MODE_ONCE_BACKWARDS)) {
    DEBUG_CRASH(
        ("*** ASSET ERROR: Idle Anims should always use ONCE or ONCE_BACKWARDS "
         "(%s)\n",
         TheThingTemplateBeingParsedName.str()));
    throw INI_INVALID_DATA;
  }
  ...
  if (cst == PARSE_TRANSITION) {
    ...
    if (info.m_mode != RenderObjClass::ANIM_MODE_ONCE &&
        info.m_mode != RenderObjClass::ANIM_MODE_ONCE_BACKWARDS) {
      DEBUG_CRASH(
          ("*** ASSET ERROR: Transition States should always use ONCE or "
           "ONCE_BACKWARDS"));
      throw INI_INVALID_DATA;
    }

    if (info.m_transitionKey != NAMEKEY_INVALID ||
        info.m_allowToFinishKey != NAMEKEY_INVALID) {
      DEBUG_CRASH(
          ("*** ASSET ERROR: Transition States must not have transition keys "
           "or m_allowToFinishKey"));
      throw INI_INVALID_DATA;
    }
...
//-------------------------------------------------------------------------------------------------
const ModelConditionInfo* W3DModelDrawModuleData::findBestInfo(
    const ModelConditionFlags& c) const {
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
W3DModelDraw::W3DModelDraw(Thing* thing, const ModuleData* moduleData)
  ...
  getW3DModelDrawModuleData()->validateStuffForTimeAndWeather(
      getDrawable(), TheGlobalData->m_timeOfDay == TIME_OF_DAY_NIGHT,
  ...
  if (!info) {
    DEBUG_CRASH(
        ("*** ASSET ERROR: all draw modules must have an IDLE state\n"));
    throw INI_INVALID_DATA;
  }
...
void W3DModelDraw::doStartOrStopParticleSys() {
  ...
  for (std::vector<ParticleSysTrackerType>::const_iterator it =
           m_particleSystemIDs.begin();
       it != m_particleSystemIDs.end(); ++it)
  // for (std::vector<ParticleSystemID>::const_iterator it =
  // m_particleSystemIDs.begin(); it != m_particleSystemIDs.end(); ++it)
  {
    ParticleSystem* sys =
        TheParticleSystemManager->findParticleSystem((*it).id);
    if (sys != NULL) {
      // this can be NULL
      if (hidden) {
        sys->stop();
...
void W3DModelDraw::setHidden(Bool hidden) {
  ...
  if (m_shadow) m_shadow->enableShadowRender(!hidden);

  m_shadowEnabled = hidden;

  if (m_terrainDecal) m_terrainDecal->enableShadowRender(!hidden);
...
/**Free all data used by this model's shadow.  This is used to dynamically
 * enable/disable shadows by the options screen*/
void W3DModelDraw::releaseShadows(
    void)  ///< frees all shadow resources used by this module - used by Options
           ///< screen.
{
...
/** Create shadow resources if not already present. This is used to dynamically
 * enable/disable shadows by the options screen*/
void W3DModelDraw::allocateShadows(void) {
  const ThingTemplate* tmplate = getDrawable()->getTemplate();

  // Check if we don't already have a shadow but need one for this type of
  // model.
  if (m_shadow == NULL && m_renderObject && TheW3DShadowManager &&
      tmplate->getShadowType() != SHADOW_NONE) {
    Shadow::ShadowTypeInfo shadowInfo;
    strcpy(shadowInfo.m_ShadowName, tmplate->getShadowTextureName().str());
    DEBUG_ASSERTCRASH(shadowInfo.m_ShadowName[0] != '\0',
                      ("this should be validated in ThingTemplate now"));
    shadowInfo.allowUpdates = FALSE;  // shadow image will never update
    shadowInfo.allowWorldAlign =
        TRUE;  // shadow image will wrap around world objects
...
void W3DModelDraw::getRenderCost(RenderCost& rc) const {
  ...
  if (m_shadow) m_shadow->getRenderCost(rc);
}
#endif  //_DEBUG || _INTERNAL

/**recurse through sub-objs to collect stats about the rendering cost of this
 * draw module */
#if defined(_DEBUG) || defined(_INTERNAL)
void W3DModelDraw::getRenderCostRecursive(RenderCost& rc,
                                          RenderObjClass* robj) const {
  if (robj == NULL) return;
  ...
  // Only consider visible sub-objects.  Some vehicles have hidden headlight
  // cones for example that are not normally rendered.
  if (robj->Is_Not_Hidden_At_All()) {
    ...
    if (robj->Class_ID() == RenderObjClass::CLASSID_MESH) {
      ...
      if (model != NULL) {
        if (model->Get_Flag(MeshGeometryClass::SORT)) rc.addSortedMeshes(1);
        if (model->Get_Flag(MeshGeometryClass::SKIN)) rc.addSkinMeshes(1);
      }
  ...
  if (dist > 0.0f) {
    ...
    if (obj) {
      ...
      if (physics) {
        ...
        if (speed > 0.0f) {
          // if distance-covered is specified for this anim, adjust the frame
          // rate so it appears to sync with our unit speed
...
void W3DModelDraw::adjustTransformMtx(Matrix3D& mtx) const {
  ...
  if (d->m_attachToDrawableBone.isNotEmpty()) {
    // override the mtx in this case. yes, call drawable, since the bone in
    // question is likely to be in another module!
    Matrix3D boneMtx;
    if (getDrawable()->getCurrentWorldspaceClientBonePositions(
            d->m_attachToDrawableBone.str(), boneMtx)) {
      mtx = boneMtx;
    } else {
...
void W3DModelDraw::doDrawModule(const Matrix3D* transformMtx) {
  ...
  setPauseAnimation(!getDrawable()->getShouldAnimate(
      getW3DModelDrawModuleData()->m_animationsRequirePower));
  ...
  if (isAnimationComplete(m_renderObject)) {
    if (m_curState != NULL && m_nextState != NULL) {
      ...
      if (nextDuration != NO_NEXT_DURATION) {
        // DEBUG_LOG(("restoring pending duration of %d
        // frames\n",nextDuration));
        setAnimationLoopDuration(nextDuration);
      }
    }

    if (m_renderObject && m_curState != NULL && m_whichAnimInCurState != -1) {
...
//-------------------------------------------------------------------------------------------------
const ModelConditionInfo* W3DModelDraw::findTransitionForSig(
    TransitionSig sig) const {
  const TransitionMap& transitionMap =
      getW3DModelDrawModuleData()->m_transitionMap;
...
Real W3DModelDraw::getCurrentAnimFraction() const {
  if (m_curState != NULL && isAnyMaintainFrameFlagSet(m_curState->m_flags) &&
      m_renderObject != NULL &&
...
//-------------------------------------------------------------------------------------------------
void W3DModelDraw::adjustAnimation(const ModelConditionInfo* prevState,
                                   Real prevAnimFraction) {
  ...
  if (numAnims > 0) {
    ...
    const W3DAnimationInfo& animInfo =
        m_curState->m_animations[m_whichAnimInCurState];

    HAnimClass* animHandle =
        animInfo
            .getAnimHandle();  // note that this now returns an ADDREFED handle,
    ...
    if (m_renderObject && animHandle) {
      ...
      // order is important here: MAINTAIN_FRAME_ACROSS_STATES is overridden by
      // the other bits, above.
      else if (isAnyMaintainFrameFlagSet(m_curState->m_flags) && prevState &&
               prevState != m_curState &&
               isAnyMaintainFrameFlagSet(prevState->m_flags) &&
               isCommonMaintainFrameFlagSet(m_curState->m_flags,
                                            prevState->m_flags) &&
               prevAnimFraction >= 0.0) {
        startFrame =
            REAL_TO_INT(prevAnimFraction * animHandle->Get_Num_Frames() - 1);
      }
      ...
      if (m_renderObject->Class_ID() == RenderObjClass::CLASSID_HLOD) {
        HLodClass* hlod = (HLodClass*)m_renderObject;
        Real factor = GameClientRandomValueReal(
            m_curState->m_animMinSpeedFactor, m_curState->m_animMaxSpeedFactor);
...
Bool W3DModelDraw::setCurAnimDurationInMsec(Real desiredDurationInMsec) {
  if (m_renderObject &&
    ...
    if (anim) {
      Real naturalDurationInMsec =
          anim->Get_Num_Frames() * 1000.0f / anim->Get_Frame_Rate();
...
Real W3DModelDraw::getCurAnimDistanceCovered() const {
  if (m_curState != NULL && m_whichAnimInCurState >= 0) {
    const W3DAnimationInfo& animInfo =
        m_curState->m_animations[m_whichAnimInCurState];
#if defined(_DEBUG) || defined(_INTERNAL)
    if (TheSkateDistOverride != 0.0f) return TheSkateDistOverride;
#endif
...
//-------------------------------------------------------------------------------------------------
/**
        Utility function to make it easier to recursively hide all objects
   connected to a certain bone. We will hide all objects connected to bones
   which are children of boneIdx
*/
static void doHideShowBoneSubObjs(Bool state, Int numSubObjects, Int boneIdx,
                                  RenderObjClass* fullObject,
...
//-------------------------------------------------------------------------------------------------
void ModelConditionInfo::WeaponBarrelInfo::setMuzzleFlashHidden(
    RenderObjClass* fullObject, Bool hide) const {
  if (fullObject) {
    RenderObjClass* childObject =
        fullObject->Get_Sub_Object_On_Bone(0, m_muzzleFlashBone);
    if (childObject) {
      childObject->Set_Hidden(hide);
      childObject->Release_Ref();
    } else {
      DEBUG_CRASH(
          ("*** ASSET ERROR: childObject %s not found in "
...
//-------------------------------------------------------------------------------------------------
void W3DModelDraw::doHideShowSubObjs(
    const std::vector<ModelConditionInfo::HideShowSubObjInfo>* vec) {
  if (!m_renderObject) return;

  if (!vec->empty()) {
    for (std::vector<ModelConditionInfo::HideShowSubObjInfo>::const_iterator
             it = vec->begin();
         it != vec->end(); ++it) {
      Int objIndex;
      RenderObjClass* subObj;

      if ((subObj = m_renderObject->Get_Sub_Object_By_Name(
               it->subObjName.str(), &objIndex)) != NULL) {
        subObj->Set_Hidden(it->hide);

        const HTreeClass* htree = m_renderObject->Get_HTree();
        if (htree) {
          // get the bone of this subobject so we can hide all other child
          // objects that use this bone as a parent.
          Int boneIdx = m_renderObject->Get_Sub_Object_Bone_Index(0, objIndex);
          doHideShowBoneSubObjs(it->hide, m_renderObject->Get_Num_Sub_Objects(),
                                boneIdx, m_renderObject, htree);
        }
        subObj->Release_Ref();
      } else {
  ...
  // Kris (added Aug 2002)
  // This is really important as it allows a person to override modelcondition
  // show/hide objects. Used by the A10 strike September 2002 -- now used by the
  // UpgradeSubObject system.
...
/*
        ...
        This function must not EVER do ANYTHING which can in any way, shape, or
   form affect GameLogic in any way; if it does, net desyncs will occur and we
        will all lose our jobs. This must remain pure-client-only, and ensure
        that nothing it does can be detected, even in read-only form, by
   GameLogic!
...
void W3DModelDraw::handleClientTurretPositioning() {
  if (!m_curState ||
      !(m_curState->m_validStuff & ModelConditionInfo::TURRETS_VALID))
    return;
  ...
  for (int tslot = 0; tslot < MAX_TURRETS; ++tslot) {
    ...
    if (tur.m_turretAngleBone || tur.m_turretPitchBone) {
      ...
      if (obj) {
        ...
        if (ai)
          ai->getTurretRotAndPitch((WhichTurretType)tslot, &turretAngle,
                                   &turretPitch);
      }

      // do turret, if any
      if (tur.m_turretAngleBone != 0) {
        if (m_curState) turretAngle += tur.m_turretArtAngle;
      ...
      if (tur.m_turretPitchBone != 0) {
        if (m_curState) turretPitch += tur.m_turretArtPitch;
...
//{
//	const ModelConditionInfo::TurretInfo& tur =
//m_curState->m_turrets[tslot]; 	Real turretAngle =
//TheGlobalData->m_downwindAngle; 	if (tur.m_flagAngleBone )
//	{
//		Matrix3D turretXfrm(1);
//		turretXfrm.Rotate_Z(turretAngle);
//		if (m_renderObject)
//		{
//			m_renderObject->Capture_Bone( tur.m_turretAngleBone );
//			m_renderObject->Control_Bone( tur.m_turretAngleBone,
//turretXfrm );
//		}
//	}
// }
...
//-------------------------------------------------------------------------------------------------
/*
        Note that, strictly speaking this code is WRONG, since it assumes it
   will get called every frame, but in fact, will only get called every frame
   that it is visible. In practice, this isn't a big problem, but...
...
void W3DModelDraw::handleClientRecoil() {
  ...
  // do recoil, if any
  for (int wslot = 0; wslot < WEAPONSLOT_COUNT; ++wslot) {
    if (!m_curState->m_hasRecoilBonesOrMuzzleFlashes[wslot]) continue;

    const ModelConditionInfo::WeaponBarrelInfoVec& barrels =
        m_curState->m_weaponBarrelInfoVec[wslot];
    ...
    for (Int i = 0; i < count; ++i) {
      if (barrels[i].m_muzzleFlashBone != 0) {
        ...
        // DEBUG_LOG(("adjust muzzleflash %08lx for Draw %08lx state %s to %d at
        // frame
      ...
      if (barrels[i].m_recoilBone != 0) {
        switch (recoils[i].m_state) {
          ...
          case WeaponRecoilInfo::RECOIL:
            ...
            recoils[i].m_recoilRate *=
                d->m_recoilDamping;  // recoil decelerates
        ...
        // DEBUG_ASSERTLOG(recoils[i].m_shift==0.0f,("adjust bone %d by
      ...
      if (m_curState != NULL &&
        ...
        for (std::vector<ParticleSysBoneInfo>::const_iterator it =
          ...
          if (sys != NULL) {
            ...
            Int boneIndex =
                m_renderObject
                    ? m_renderObject->Get_Bone_Index(it->boneName.str())
                    : 0;
            if (boneIndex != 0) {
              // ugh... kill the mtx so we get it in modelspace, not world space
              Matrix3D originalTransform =
                  m_renderObject->Get_Transform();  // save the transform
              Matrix3D tmp(true);
            ...
            // and the direction, so that the system is oriented like the bone
            // is
            ...
            // important: mark it as do-not-save, since we'll just re-create it
            // when we reload.
            sys->setSaveable(FALSE);

            if (drawable->isDrawableEffectivelyHidden() ||
                m_fullyObscuredByShroud) {
              sys->stop();  // don't start the systems for drawables that are
                            // hidden.
            }
  ...
  if (drawable != NULL && m_curState != NULL && m_renderObject != NULL) {
    for (std::vector<ParticleSysTrackerType>::const_iterator it =
      ...
      if ((sys != NULL) && (boneIndex != 0)) {
        ...
        const Matrix3D boneTransform = m_renderObject->Get_Bone_Transform(
            boneIndex);  // just a little worried about state changes
        Vector3 vpos = boneTransform.Get_Translation();
...
void W3DModelDraw::setTerrainDecal(TerrainDecalType type) {
  if (m_terrainDecal) m_terrainDecal->release();
  ...
  if (TheProjectedShadowManager)
    m_terrainDecal =
        TheProjectedShadowManager->addDecal(m_renderObject, &decalInfo);
  if (m_terrainDecal) {
    m_terrainDecal->enableShadowInvisible(m_fullyObscuredByShroud);
...
void W3DModelDraw::nukeCurrentRender(Matrix3D* xform) {
  // this needs to be "dirtied" so that the new pausing of the animation will be
  // triggered.
  m_pauseAnimation = false;
  ...
  if (m_renderObject) {
    ...
    if (xform) *xform = m_renderObject->Get_Transform();
...
//-------------------------------------------------------------------------------------------------
#if defined(_DEBUG) || \
    defined(_INTERNAL)  // art wants to see buildings without flags as a test.
void W3DModelDraw::hideGarrisonFlags(Bool hide) {
  ...
  if ((subObj = m_renderObject->Get_Sub_Object_By_Name("POLE", &objIndex)) !=
      NULL) {
    subObj->Set_Hidden(hide);

    const HTreeClass* htree = m_renderObject->Get_HTree();
    if (htree) {
      // get the bone of this subobject so we can hide all other child objects
      // that use this bone as a parent.
      Int boneIdx = m_renderObject->Get_Sub_Object_Bone_Index(0, objIndex);
      doHideShowBoneSubObjs(hide, m_renderObject->Get_Num_Sub_Objects(),
                            boneIdx, m_renderObject, htree);
    }
    subObj->Release_Ref();
  }
...
//-------------------------------------------------------------------------------------------------
/** Hides all subobjects which are headlights.  Used to disable lights on models
 * during the day.*/
void W3DModelDraw::hideAllHeadlights(Bool hide) {
  if (m_renderObject) {
    for (Int subObj = 0; subObj < m_renderObject->Get_Num_Sub_Objects();
         subObj++) {
...
//-------------------------------------------------------------------------------------------------
void W3DModelDraw::hideAllMuzzleFlashes(const ModelConditionInfo* state,
  ...
  for (int wslot = 0; wslot < WEAPONSLOT_COUNT; ++wslot) {
    if (!state->m_hasRecoilBonesOrMuzzleFlashes[wslot]) continue;

    const ModelConditionInfo::WeaponBarrelInfoVec& barrels =
        state->m_weaponBarrelInfoVec[wslot];
    for (ModelConditionInfo::WeaponBarrelInfoVec::const_iterator it =
             barrels.begin();
         it != barrels.end(); ++it) {
...
//-------------------------------------------------------------------------------------------------
static Bool turretNamesDiffer(const ModelConditionInfo* a,
                              const ModelConditionInfo* b) {
  if (!(a->m_validStuff & ModelConditionInfo::TURRETS_VALID) ||
      !(b->m_validStuff & ModelConditionInfo::TURRETS_VALID)) {
    return true;
  }
  for (int i = 0; i < MAX_TURRETS; ++i)
    if (a->m_turrets[i].m_turretAngleNameKey !=
            b->m_turrets[i].m_turretAngleNameKey ||
        a->m_turrets[i].m_turretPitchNameKey !=
            b->m_turrets[i].m_turretPitchNameKey)
      return true;
...
//-------------------------------------------------------------------------------------------------
/** Change the model for this drawable. If color is non-zero, it will also apply
   team color to the model */
//-------------------------------------------------------------------------------------------------
void W3DModelDraw::setModelState(const ModelConditionInfo* newState) {
  DEBUG_ASSERTCRASH(newState,
                    ("invalid state in W3DModelDraw::setModelState\n"));

#ifdef DEBUG_OBJECT_ID_EXISTS
  if (getDrawable() && getDrawable()->getObject() &&
      getDrawable()->getObject()->getID() == TheObjectIDToDebug) {
    DEBUG_LOG(("REQUEST switching to state %s for obj %s %d\n",
               newState->m_description.str(),
               getDrawable()->getObject()->getTemplate()->getName().str(),
               getDrawable()->getObject()->getID()));
  }
#endif
  ...
  if (m_curState != NULL && newState != NULL) {
    ...
    // weird, but is deliberate: nextState and newState will always be "real"
    // states (or null), leaving us these possibilities:
    //
    // -- curState is a transition state
    //		--in this case, new state should take precedence (unless there's
    //a wait-to-finish marking, 		which would be handled by the else clause)
    //
    // -- curState is a real state, nextState is a real state with a
    // wait-to-finish clause
    //		-- if curState == newState, presume that we should restart
    //curState, thus punting thepending nextState
    //		-- if curState != newState, presume we should switch to it, thus
    //punting curState and nextState
    //
    // -- curState is a real state, nextState is a real state without a
    // wait-to-finish clause
    ...
    // check for allow-to-finish implicit transitions
             ...
             m_renderObject && !isAnimationComplete(m_renderObject)) {
#ifdef DEBUG_OBJECT_ID_EXISTS
      if (getDrawable() && getDrawable()->getObject() &&
          getDrawable()->getObject()->getID() == TheObjectIDToDebug) {
    ...
    // check for a normal->normal state transition
      ...
      TransitionSig sig = buildTransitionSig(m_curState->m_transitionKey,
                                             newState->m_transitionKey);
      ...
      if (transState != NULL) {
        ...
        if (getDrawable() && getDrawable()->getObject() &&
            getDrawable()->getObject()->getID() == TheObjectIDToDebug) {
          DEBUG_LOG(
              ("using TRANSITION state %s before requested state %s for obj %s "
               "%d\n",
               transState->m_description.str(), newState->m_description.str(),
               getDrawable()->getObject()->getTemplate()->getName().str(),
               getDrawable()->getObject()->getID()));
        }
#endif
  ...
  // get this here, before we change anything... we'll need it to pass to
  // adjustAnimation (srj)
  Real prevAnimFraction = getCurrentAnimFraction();

  //
  // Set up any particle effects for the new model.  Also stop the old ones,
  // note that we will never allow the placement of new ones if the status bit
  // is set that tells us we should not automatically create particle systems
  // for the model condition states
  //
  if (getDrawable()->testDrawableStatus(DRAWABLE_STATUS_NO_STATE_PARTICLES) ==
      FALSE)
  ...
  // and always hide muzzle flashes, in case modelstate and model have crossed
  // at the same time
  hideAllMuzzleFlashes(newState, m_renderObject);

  // note that different states might use the same model; for these, don't go
  // thru the expense of creating a new render-object. (exception: if color is
  // changing, or subobjs are changing, or a few other things...)
  if (m_curState == NULL || newState->m_modelName != m_curState->m_modelName ||
      turretNamesDiffer(newState, m_curState)
      // srj sez: I'm not sure why we want to do the "hard stuff" if we have
      // projectile bones; I think it is a holdover from days gone by when bones
      // were handled quite differently, rather than being cached. but doing
      // this hard stuff is a lot more work, and I think it's unnecessary, so
      // let's remove this.
    ...
    if (newState->m_modelName.isEmpty()) {
      ...
      m_renderObject = W3DDisplay::m_assetManager->Create_Render_Obj(
          newState->m_modelName.str(), draw->getScale(), m_hexColor);
      DEBUG_ASSERTCRASH(m_renderObject,
                        ("*** ASSET ERROR: Model %s not found!\n",
                         newState->m_modelName.str()));
    }

    // BONEPOS_LOG(("validateStuff() from within
    // W3DModelDraw::setModelState()\n")); BONEPOS_DUMPREAL(draw->getScale());

    newState->validateStuff(m_renderObject, draw->getScale(),
    ...
    if (m_renderObject && TheGlobalData->m_makeTrackMarks &&
        !m_trackRenderObject && TheTerrainTracksRenderObjClassSystem != NULL &&
        !getW3DModelDrawModuleData()->m_trackFile.isEmpty()) {
      m_trackRenderObject = TheTerrainTracksRenderObjClassSystem->bindTrack(
          m_renderObject, 1.0f * MAP_XY_FACTOR,
          getW3DModelDrawModuleData()->m_trackFile.str());
      if (draw && m_trackRenderObject)
        m_trackRenderObject->setOwnerDrawable(draw);
    }

    // set up shadows
    if (m_renderObject && TheW3DShadowManager &&
        tmplate->getShadowType() != SHADOW_NONE) {
      Shadow::ShadowTypeInfo shadowInfo;
      strcpy(shadowInfo.m_ShadowName, tmplate->getShadowTextureName().str());
      DEBUG_ASSERTCRASH(shadowInfo.m_ShadowName[0] != '\0',
      ...
      shadowInfo.m_offsetY = tmplate->getShadowOffsetY();
      m_shadow =
          TheW3DShadowManager->addShadow(m_renderObject, &shadowInfo, draw);
      if (m_shadow) {
        m_shadow->enableShadowInvisible(m_fullyObscuredByShroud);
    ...
    if (m_renderObject) {
      ...
      if (obj) {
        ...
        if (obj->isKindOf(KINDOF_BRIDGE) == FALSE &&
          ...
          if (obj->isKindOf(KINDOF_STRUCTURE) &&
              draw->getModelConditionFlags().test(MODELCONDITION_RUBBLE)) {
            // A dead building, -- don't allow the user to click on rubble!
            // Treat it as a location instead.
            m_renderObject->Set_Collision_Type(0);
          } else if (obj->isEffectivelyDead()) {
            // A dead object, -- don't allow the user to click on rubble/hulks!
            // Treat it as a location instead.
            m_renderObject->Set_Collision_Type(0);
          }
        }
      ...
      // We created a new render object so we need to preserve the visibility
      // state of the previous render object.
      if (draw->isDrawableEffectivelyHidden()) {
        m_renderObject->Set_Hidden(TRUE);
        if (m_shadow) m_shadow->enableShadowRender(FALSE);
        m_shadowEnabled = FALSE;
      }

      //
      // set the transform for the new model to that we saved before, we do this
      // so that the model transition is smooth and will immediately appear at
      // the same orientation and location as the previous one
      //
    ...
    newState->validateStuff(m_renderObject, getDrawable()->getScale(),
                            getW3DModelDrawModuleData()->m_extraPublicBones);
    rebuildWeaponRecoilInfo(newState);

    // ensure that any muzzle flashes from the *previous* state, are hidden...
    //		hideAllMuzzleFlashes(m_curState, m_renderObject);// moved to
    //above
  ...
  const ModelConditionInfo* prevState =
      m_curState;  // save, to pass to adjustAnimation (could be null)
  m_curState = newState;
...
void W3DModelDraw::replaceIndicatorColor(Color color) {
  if (!getW3DModelDrawModuleData()->m_okToChangeModelColor) return;
...
//-------------------------------------------------------------------------------------------------
// this method must ONLY be called from the client, NEVER From the logic, not
// even indirectly.
Bool W3DModelDraw::clientOnly_getRenderObjInfo(Coord3D* pos,
                                               Real* boundingSphereRadius,
                                               Matrix3D* transform) const {
  if (!m_renderObject) return false;
...
Bool W3DModelDraw::getProjectileLaunchOffset(
    const ModelConditionFlags& condition, WeaponSlotType wslot,
    Int specificBarrelToUse, Matrix3D* launchPos, WhichTurretType tur,
    Coord3D* turretRotPos, Coord3D* turretPitchPos) const {
  /*
          ...
          to get the pristine bone(s) for the state that logic believes to be
     current, not the one the client might currently be using...
  */
  ...
  for (Int i = 0; i < condition.size(); ++i) {
    if (condition.test(i)) {
      if (flags.isNotEmpty()) flags.concat(',');
  ...
  const W3DModelDrawModuleData* d = getW3DModelDrawModuleData();
  // CRCDEBUG_LOG(("validateStuffs() from within
  // W3DModelDraw::getProjectileLaunchOffset()\n"));
  // DUMPREAL(getDrawable()->getScale());
  // BONEPOS_LOG(("validateStuffs() from within
  // W3DModelDraw::getProjectileLaunchOffset()\n"));
  // BONEPOS_DUMPREAL(getDrawable()->getScale());
  stateToUse->validateStuff(NULL, getDrawable()->getScale(),
                            d->m_extraPublicBones);

  DEBUG_ASSERTCRASH(stateToUse->m_transitionSig == NO_TRANSITION,
                    ("It is never legal to getProjectileLaunchOffset from a "
                     "Transition state (they vary on a per-client basis)... "
                     "however, we can fix this (see srj)\n"));

  DEBUG_ASSERTCRASH(specificBarrelToUse >= 0,
                    ("specificBarrelToUse should now always be explicit"));
  ...
  Matrix3D pivot;
  if (d->m_attachToDrawableBone.isNotEmpty() &&
      getDrawable()->getPristineBonePositions(d->m_attachToDrawableBone.str(),
                                              0, NULL, &pivot, 1) == 1) {
  ...
  const ModelConditionInfo::WeaponBarrelInfoVec& wbvec =
      stateToUse->m_weaponBarrelInfoVec[wslot];
  if (wbvec.empty()) {
    // Can't find the launch pos, but they might still want the other info they
    // asked for
...
#ifdef CACHE_ATTACH_BONE
#else
      launchPos->Adjust_Translation(
          Vector3(techOffset.x, techOffset.y, techOffset.z));
#endif
  ...
  if (tur != TURRET_INVALID) {
    ...
    if (turretRotPos) {
      if (turInfo.m_turretAngleNameKey != NAMEKEY_INVALID &&
          !stateToUse->findPristineBonePos(turInfo.m_turretAngleNameKey,
                                           *turretRotPos)) {
    ...
    if (turretPitchPos) {
      if (turInfo.m_turretPitchNameKey != NAMEKEY_INVALID &&
          !stateToUse->findPristineBonePos(turInfo.m_turretPitchNameKey,
                                           *turretPitchPos)) {
        DEBUG_CRASH(("*** ASSET ERROR: TurretBone %s not found!\n",
...
Int W3DModelDraw::getPristineBonePositionsForConditionState(
  ...
  */
  const ModelConditionInfo* stateToUse = findBestInfo(condition);
  if (!stateToUse) return 0;

  //	if (isValidTimeToCalcLogicStuff())
  //	{
  //		CRCDEBUG_LOG(("W3DModelDraw::getPristineBonePositionsForConditionState()
  //- state = '%s'\n", 			stateToUse->getDescription().str()));
  ...
  // BONEPOS_LOG(("validateStuff() from within
  // W3DModelDraw::getPristineBonePositionsForConditionState()\n"));
  // BONEPOS_DUMPREAL(getDrawable()->getScale());
  //  we must call this every time we set m_nextState, to ensure cached bones
  //  are happy
  ...
  if (maxBones > MAX_BONE_GET) maxBones = MAX_BONE_GET;

  if (transforms == NULL) transforms = tmpMtx;
...
Int W3DModelDraw::getCurrentBonePositions(const char* boneNamePrefix,
                                          Int startIndex, Coord3D* positions,
                                          Matrix3D* transforms,
                                          Int maxBones) const {
  const int MAX_BONE_GET = 64;
  Matrix3D tmpMtx[MAX_BONE_GET];
...
#endif
    // DEBUG_ASSERTCRASH(!m_renderObject->Is_Bone_Captured(boneIndex), ("bone is
    // captured!"));
...
//-------------------------------------------------------------------------------------------------
const ModelConditionInfo* W3DModelDraw::findBestInfo(
    const ModelConditionFlags& c) const {
...
Int W3DModelDraw::getBarrelCount(WeaponSlotType wslot) const {
  return (m_curState &&
          (m_curState->m_validStuff & ModelConditionInfo::BARRELS_VALID))
             ? m_curState->m_weaponBarrelInfoVec[wslot].size()
             : 0;
}
...
Bool W3DModelDraw::handleWeaponFireFX(WeaponSlotType wslot,
                                      ...
                                      const FXList* fxl, Real weaponSpeed,
  ...
  if (wbvec.empty()) {
    ...
    // FXList::doFXPos(fxl, getDrawable()->getPosition(),
    // getDrawable()->getTransformMatrix(), weaponSpeed, victimPos);
    return false;
  }
  ...
  if (fxl) {
    if (info.m_fxBone && m_renderObject) {
      const Object* logicObject =
          getDrawable()->getObject();  // This is slow, so store it
      if (!m_renderObject->Is_Hidden() || (logicObject == NULL)) {
        // I can ask the drawable's bone position if I am not hidden (if I have
        // no object I have no choice)
        ...
        // Things in transports regularly fire from inside (hidden), so this is
        // not weird.
        const Matrix3D* mtx;
        Coord3D pos;
        mtx = logicObject->getTransformMatrix();
        pos = *(logicObject->getPosition());
        /** @todo Once Firepoint bones are actually implemented, this matrix
        will become correct.
        // Unless of course they decide to not have the tracers come out of the
        windows, but rather go towards the target.
        // In that case, tracers will have to be rewritten to be "point towards
  ...
  if (info.m_recoilBone || info.m_muzzleFlashBone) {
    // DEBUG_LOG(("START muzzleflash %08lx for Draw %08lx state %s at frame
    // %d\n",info.m_muzzleFlashBone,this,m_curState->m_description.str(),TheGameLogic->getFrame()));
    WeaponRecoilInfo& recoil =
        m_weaponRecoilInfoVec[wslot][specificBarrelToUse];
...
// this is never defined -- srj
  ...
  m_nextStateAnimLoopDuration = NO_NEXT_DURATION;
  DEBUG_ASSERTCRASH(
      m_curState != NULL && m_curState->m_transition == NO_TRANSITION,
      ("Hmm, setAnimationLoopDuration on a transition state is probably not "
       "right... see srj"));
#else
  m_nextStateAnimLoopDuration = NO_NEXT_DURATION;
#endif
...
void W3DModelDraw::setAnimationCompletionTime(UnsignedInt numFrames) {
  if (m_curState != NULL && m_curState->m_transitionSig != NO_TRANSITION &&
      m_curState->m_animations.size() > 0 && m_nextState != NULL &&
      m_nextState->m_transitionSig == NO_TRANSITION &&
      m_nextState->m_animations.size() > 0) {
    ...
    const Real t2 =
        m_nextState->m_animations.front().getNaturalDurationInMsec();
...
void W3DModelDraw::setPauseAnimation(Bool pauseAnim) {
  ...
  if (m_renderObject &&
    ...
    HLodClass* hlod = (HLodClass*)m_renderObject;
    HAnimClass* anim =
        hlod->Peek_Animation_And_Info(framenum, numFrames, mode, dummy);
    if (anim) {
      if (m_pauseAnimation) {
...
void W3DModelDraw::rebuildWeaponRecoilInfo(const ModelConditionInfo* state) {
  ...
  if (state == NULL) {
    ...
    for (wslot = 0; wslot < WEAPONSLOT_COUNT; ++wslot) {
      ...
      for (WeaponRecoilInfoVec::iterator it =
               m_weaponRecoilInfoVec[wslot].begin();
           it != m_weaponRecoilInfoVec[wslot].end(); ++it) {
        it->clear();
      }
    }
  }
...
void W3DModelDraw::preloadAssets(TimeOfDay timeOfDay) {
  ...
  if (modData) modData->preloadAssets(timeOfDay, getDrawable()->getScale());
}
...
void W3DModelDraw::updateProjectileClipStatus(UnsignedInt shotsRemaining,
                                              ...
                                              WeaponSlotType slot) {
  if ((getW3DModelDrawModuleData()->m_projectileBoneFeedbackEnabledSlots &
       (1 << slot)) == 0)
    return;  // Only if specified.

  doHideShowProjectileObjects(shotsRemaining, maxShots,
                              slot);  // Means effectively, show m of n.
}
...
//-------------------------------------------------------------------------------------------------
void W3DModelDraw::doHideShowProjectileObjects(UnsignedInt showCount,
                                               UnsignedInt maxCount,
                                               WeaponSlotType slot) {
  // Loop through the projectile bones, and start hiding off the front.  That
  // is, 8,9 means to hide the first only.
  ...
  if (m_curState->m_weaponProjectileHideShowName[slot].isEmpty()) {
    for (Int projectileIndex = 0; projectileIndex < maxCount;
         projectileIndex++) {
      oneEntry.subObjName.format(
          "%s%02d", m_curState->m_weaponProjectileLaunchBoneName[slot].str(),
          (projectileIndex + 1));
  ...
  // Rather than duplicate recursive utility stuff, I will build a vector like
  // those used by the main ShowHide setting
...
void W3DModelDraw::xfer(Xfer* xfer) {
  ...
  for (Int i = 0; i < WEAPONSLOT_COUNT; ++i) {
    ...
    if (xfer->getXferMode() == XFER_SAVE) {
      ...
      for (it = m_weaponRecoilInfoVec[i].begin();
           it != m_weaponRecoilInfoVec[i].end(); ++it) {
        // state
        weaponRecoilInfo.m_state = (*it).m_state;
        xfer->xferUser(&weaponRecoilInfo.m_state,
                       sizeof(WeaponRecoilInfo::RecoilState));
  ...
  if (version >= 2) {
    if (xfer->getXferMode() == XFER_SAVE) {
      ...
      if (m_renderObject &&
          m_renderObject->Class_ID() == RenderObjClass::CLASSID_HLOD &&
          m_curState && m_curState->m_transitionSig == NO_TRANSITION) {
        // cast to HLod
        HLodClass* hlod = (HLodClass*)m_renderObject;
        ...
        Real frame, dummy;
        HAnimClass* anim =
            hlod->Peek_Animation_And_Info(frame, numFrames, mode, dummy);
      ...
      if (present) {
        ...
        //
        // cast render object to HLod, if this is no longer possible we have
        // read the data already and can just ignore it
        //
        if (m_renderObject &&
            m_renderObject->Class_ID() == RenderObjClass::CLASSID_HLOD) {
          HLodClass* hlod = (HLodClass*)m_renderObject;
          ...
          // set animation data
          if (anim) {
            // figure out frame number given percent written in file and total
            // frames on anim
            ...
            // srj sez: do not change the animation mode. it's too risky, since
            // if you change (say) a nonlooping to a looping, something might
            // break since it could rely on that anim terminating.
...
// ------------------------------------------------------------------------------------------------
void W3DModelDrawModuleData::crc(Xfer* x) { xfer(x); }
...
void W3DModelDrawModuleData::xfer(Xfer* x) {
  ...
  for (ModelConditionVector::iterator it = m_conditionStates.begin();
    ...
    if (info->m_validStuff) {
      for (PristineBoneInfoMap::iterator bit = info->m_pristineBones.begin();
           bit != info->m_pristineBones.end(); ++bit) {
      ...
      for (i = 0; i < WEAPONSLOT_COUNT; ++i) {
        for (ModelConditionInfo::WeaponBarrelInfoVec::iterator wit =
                 info->m_weaponBarrelInfoVec[i].begin();
             wit != info->m_weaponBarrelInfoVec[i].end(); ++wit) {
