...
// ////////////////////////////////////////////////////////////// Created: Colin
// Day, July 2001 Desc:    Display string Manager for W3D
///////////////////////////////////////////////////////////////////////////////////////////////////
...
W3DDisplayStringManager::~W3DDisplayStringManager(void) {
  for (Int i = 0; i < 10; ++i) {
    if (m_groupNumeralStrings[i]) freeDisplayString(m_groupNumeralStrings[i]);
    m_groupNumeralStrings[i] = NULL;
  }
...
void W3DDisplayStringManager::postProcessLoad(void) {
  // Get the font.
  GameFont *font = TheFontLibrary->getFont(TheDrawGroupInfo->m_fontName,
...
DisplayString *W3DDisplayStringManager::newDisplayString(void) {
  ...
  if (newString == NULL) {
    DEBUG_LOG(
        ("newDisplayString: Could not allcoate new W3D display string\n"));
    assert(0);
    return NULL;
  ...
  if (TheGlobalLanguageData &&
      TheGlobalLanguageData->m_defaultDisplayStringFont.name.isNotEmpty()) {
...
void W3DDisplayStringManager::freeDisplayString(DisplayString *string) {
  ...
  // if the string happens to fall where our current checkpoint was, set the
  // checkpoint to null
...
void W3DDisplayStringManager::update(void) {
  ...
  // if the m_currentCheckpoint is valid, use it for the starting point for the
  // search
  ...
  const UnsignedInt w3dCleanupTime =
      60; /** any string not rendered after
