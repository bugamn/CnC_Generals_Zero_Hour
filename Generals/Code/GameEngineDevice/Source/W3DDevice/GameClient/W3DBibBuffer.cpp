...
void W3DBibBuffer::allocateBibBuffers(void) {
  m_vertexBib =
      NEW_REF(DX8VertexBufferClass, (DX8_FVF_XYZDUV1, m_vertexBibSize + 4,
                                     DX8VertexBufferClass::USAGE_DYNAMIC));
  m_indexBib =
      NEW_REF(DX8IndexBufferClass,
              (m_indexBibSize + 4, DX8IndexBufferClass::USAGE_DYNAMIC));
  m_curNumBibVertices = 0;
  m_curNumBibIndices = 0;
}
...
/** Adds a bib.   */
//=============================================================================
void W3DBibBuffer::addBibDrawable(Vector3 corners[4], DrawableID id,
                                  Bool highlight) {
  Int bibIndex;
  for (bibIndex = 0; bibIndex < m_numBibs; bibIndex++) {
...
void W3DBibBuffer::renderBibs() {
  ...
  if (m_curNumNormalBibIndices) {
    ...
    DX8Wrapper::Draw_Triangles(0, m_curNumNormalBibIndices / 3, 0,
