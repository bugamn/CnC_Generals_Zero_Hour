...
ProjectedShadowManager
    *TheProjectedShadowManager;  // global singleton with simpler interface.
extern const FrustumClass *shadowCameraFrustum;  // defined in W3DShadow.
///@todo: Externs from volumetric shadow renderer - these need to be moved into
///W3DBufferManager
...
LPDIRECT3DVERTEXBUFFER8 shadowDecalVertexBufferD3D =
    NULL;                                                 ///< D3D vertex buffer
...
class W3DProjectedShadow : public Shadow {
 ...
 public:
  ...
  void update(void);  ///< updates the texture and/or projection parameters when
                      ///< the object or light moves.
  void init(void);  ///< allocates local member variables used for projection
  void updateTexture(
      Vector3 &lightPos);  ///< updates the shadow texture image using render
                           ///< object and given light position.
  void updateProjectionParameters(
      const Matrix3D &cameraXform);  ///< recompute projection matrix - needed
                                     ///< when light or object moves.
 ...
 protected:
  ...
  TexProjectClass *m_shadowProjector;  ///< object used to generate texture and
                                       ///< projection matrix.
  RenderObjClass *m_robj;     ///< render object used to cast the shadow.
  Vector3 m_lastObjPosition;  ///< position of  object at time of projection
                              ///< matrix update.
  W3DProjectedShadow *m_next;  /// for the shadow manager list
  Bool m_allowWorldAlign;  /// wrap shadow around world geometry - else align
                           /// perpendicular to local z-axis.
  Real m_decalOffsetU;  /// texture coordinate offset so not centered at object
                        /// origin.
  Real m_decalOffsetV;  /// texture coordinate offset so not centered at object
                        /// origin.
...
/** This class will manage shadow textures for each render object.  Shadow
textures may be based on render geometry but don't need to be.  This allows
lower detail 'blob' textures to be substituted to improve performance.*/
...
class W3DShadowTexture : public RefCountClass, public HashableClass {
 public:
  ...
  void setLightPosHistory(Vector3 &pos) {
    m_lastLightPosition = pos;
  }  ///< updates the last position of light
  Vector3 &getLightPosHistory(void) { return m_lastLightPosition; }
  void setObjectOrientationHistory(Matrix3 &mat) {
    m_lastObjectOrientation = mat;
  }  ///< updates the last position of light
  ...
  void updateBounds(Vector3 &lightPos,
                    RenderObjClass *robj);  ///< update extent of shadow
 ...
 private:
  ...
  TextureClass *m_texture;  ///< texture holding the shadow for this
                            ///< renderobject
  Vector3 m_lastLightPosition;  ///< position of light source at time of last
                                ///< texture update.
  Matrix3 m_lastObjectOrientation;  ///< orientation of shadow casting object
                                    ///< when texture was generated.
  AABoxClass m_areaEffectBox;  ///< boundary defining object-space volume
                               ///< affected by shadow.
  SphereClass m_areaEffectSphere;  ///< boundary defining object-space volume
                                   ///< affected by shadow.
  Vector3 m_shadowUV[2];  /// world-space vectors defining the u and v texture
                          /// coordinate axis.
};
...
class W3DShadowTextureManagerIterator : public HashTableIteratorClass {
 public:
  W3DShadowTextureManagerIterator(W3DShadowTextureManager &manager)
      : HashTableIteratorClass(*manager.texturePtrTable) {}
  W3DShadowTexture *getCurrentTexture(void) {
    return (W3DShadowTexture *)Get_Current();
  }
};
...
W3DProjectedShadowManager::~W3DProjectedShadowManager(void) {
  ...
  DEBUG_ASSERTCRASH(m_shadowList == NULL,
                    ("Destroy of non-empty projected shadow list"));
  DEBUG_ASSERTCRASH(m_decalList == NULL,
                    ("Destroy of non-empty projected decal list"));
}

void W3DProjectedShadowManager::reset(void) {
  DEBUG_ASSERTCRASH(m_shadowList == NULL,
                    ("Reset of non-empty projected shadow list"));
  DEBUG_ASSERTCRASH(m_decalList == NULL,
                    ("Reset of non-empty projected decal list"));
...
Bool W3DProjectedShadowManager::init(void) {
  ...
  m_shadowContext = NEW SpecialRenderInfoClass(
      *m_shadowCamera, SpecialRenderInfoClass::RENDER_SHADOW);
...
Bool W3DProjectedShadowManager::ReAcquireResources(void) {
  ...
  DEBUG_ASSERTCRASH(m_dynamicRenderTarget == NULL,
                    ("Acquire of existing shadow render target"));

  m_renderTargetHasAlpha = TRUE;
  if ((m_dynamicRenderTarget = DX8Wrapper::Create_Render_Target(
           DEFAULT_RENDER_TARGET_WIDTH, DEFAULT_RENDER_TARGET_HEIGHT, true)) ==
      NULL) {
    m_renderTargetHasAlpha = FALSE;
  ...
  DEBUG_ASSERTCRASH(m_pDev, ("Trying to ReAquireResources on "
                             "W3DProjectedShadowManager without device"));
  DEBUG_ASSERTCRASH(
      shadowDecalIndexBufferD3D == NULL && shadowDecalIndexBufferD3D == NULL,
      ("ReAquireResources not released in W3DProjectedShadowManager"));

  if (FAILED(m_pDev->CreateIndexBuffer(SHADOW_DECAL_INDEX_SIZE * sizeof(WORD),
                                       D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC,
                                       D3DFMT_INDEX16, D3DPOOL_DEFAULT,
                                       &shadowDecalIndexBufferD3D)))
    return FALSE;

  if (shadowDecalVertexBufferD3D == NULL) {  // Create vertex buffer
...
void W3DProjectedShadowManager::ReleaseResources(void) {
  ...
  if (shadowDecalIndexBufferD3D) shadowDecalIndexBufferD3D->Release();
  if (shadowDecalVertexBufferD3D) shadowDecalVertexBufferD3D->Release();
...
void W3DProjectedShadowManager::updateRenderTargetTextures(void) {
  ...
  if (!TheGlobalData->m_useShadowDecals) return;

  if (m_numProjectionShadows)
    for (shadow = m_shadowList; shadow;
         shadow = shadow->m_next) {  // decals don't need any updates on a
                                     // per-frame basis since
      // the image never changes.
      if (shadow->m_type != SHADOW_DECAL) shadow->update();
    }
}

/// Renders shadow on part of terrain covered by world-space bounding box.
Int W3DProjectedShadowManager::renderProjectedTerrainShadow(
...
#define SHADOW_VOLUME_FVF D3DFVF_XYZ
  ...
  if (TheTerrainRenderObject) {
    ...
    if (vertsPerRow == 1 || vertsPerColumn == 1) return 0;  // nothing to render
    ...
    {  // flush the buffer by drawing the contents and re-locking again
      if (shadowVertexBufferD3D->Lock(
              0, numVerts * sizeof(SHADOW_VOLUME_VERTEX),
    ...
    Int numIndex = (endX - startX) * (endY - startY) *
                   6;  // 6 indices per terrain cell (2 triangles).
    ...
    if (pvIndices) {  // fill each cell's vertex indices
      Int rowStart;
      for (j = startY, rowStart = 0; j < endY; j++, rowStart += vertsPerRow) {
        for (i = rowStart, k = startX; k < endX;
             i++, k++) {  ///@todo: Fix this to deal with flipped triangles
          ...
          /*						if (flipForBlend)
                                                          ...
                                                          {
             pvIndices[0]=i+vertsPerRow; pvIndices[4]=pvIndices[1]=i;
                                                                  pvIndices[3]=pvIndices[2]=i+vertsPerRow+1;
                                                                  pvIndices[5]=i+1;
                                                          }*/
          ///@todo: fix the winding order in heightmap to be in strip order like
          ///above!
          if (flipForBlend) {
            pvIndices[0] = i + 1;
    ...
    Int numPolys =
        (endX - startX) * (endY - startY) * 2;  // 2 triangles per cell

    m_pDev->SetRenderState(D3DRS_ALPHATESTENABLE,
                           TRUE);  // should reject background pixels
    ...
    //    m_pDev->SetRenderState( D3DRS_ALPHABLENDENABLE, FALSE );	//useful
    //    to see bounds
    ...
    m_pDev->SetRenderState(D3DRS_ALPHATESTENABLE,
                           FALSE);  // should reject background pixels
...
void W3DProjectedShadowManager::flushDecals(W3DShadowTexture *texture,
  ...
  if (nShadowDecalVertsInBatch == 0 &&
      nShadowDecalPolysInBatch == 0) {  // nothing to render
    return;
  }
  ...
  //	DX8Wrapper::Set_Shader(ShaderClass::_PresetOpaqueShader);	//good
  //for debugging, draws without alpha
  switch (type) {
    case SHADOW_DECAL:
  ...
  */
  ...
  m_pDev->SetStreamSource(0, shadowDecalVertexBufferD3D,
                          sizeof(SHADOW_DECAL_VERTEX));
  ...
  /*	m_pDev->SetRenderState( D3DRS_SRCBLEND,  D3DBLEND_ZERO);
          ...
          m_pDev->SetRenderState(D3DRS_ALPHATESTENABLE, TRUE);	//should reject
     background pixels m_pDev->SetRenderState( D3DRS_STENCILENABLE, TRUE );
  */
  /*	m_pDev->SetRenderState( D3DRS_STENCILFUNC,     D3DCMP_ALWAYS );
          m_pDev->SetRenderState( D3DRS_STENCILREF,      0x1 );
          m_pDev->SetRenderState( D3DRS_STENCILMASK,     0xffffffff );
          ...
          m_pDev->SetRenderState( D3DRS_STENCILPASS,  D3DSTENCILOP_INCR );
  */
  // m_pDev->SetRenderState( D3DRS_ALPHABLENDENABLE, FALSE );	//useful to see
  // bounds
  ...
  if (DX8Wrapper::_Is_Triangle_Draw_Enabled()) {
    ...
    m_pDev->DrawIndexedPrimitive(
        D3DPT_TRIANGLELIST, 0, nShadowDecalVertsInBatch,
        nShadowDecalStartBatchIndex, nShadowDecalPolysInBatch);
  }

  //	m_pDev->SetRenderState(D3DRS_ALPHATESTENABLE, FALSE);	//should reject
  //background pixels 	m_pDev->SetRenderState( D3DRS_STENCILENABLE, FALSE );
  // m_pDev->SetRenderState( D3DRS_ALPHABLENDENABLE, TRUE );

  // Restore multiplicative sprite shader
  //	m_pDev->SetRenderState(D3DRS_DESTBLEND,D3DBLEND_SRCCOLOR);
  ////restore W3D state 	m_pDev->SetRenderState(D3DRS_SRCBLEND, D3DBLEND_ZERO);
...
void testShadowDecal(void)
{
        ...
        decalInfo.allowWorldAlign = TRUE;	//shadow image will wrap around
world objects decalInfo.m_type = SHADOW_ALPHA_DECAL;
...
/**Decals have a low poly count so its better to render large numbers at once.
This system will queue them up until the buffers fill up.  It will then flush
the buffer (draw decals) and be ready for new decals.  This is an optimized
system that only uses the render objects bounding box to determine shadow
visibility.
*/
  ...
  if (TheTerrainRenderObject) {
    ...
    if (robj) {
      ...
      if (robj->Get_User_Data()) {
        ...
        if (object &&
            (objectLayer = object->getLayer()) !=
                LAYER_GROUND) {  // check if object that this decal belongs to
                                 // is not on the ground (bridge?)
          layerHeight =
              BRIDGE_OFFSET_FACTOR +
              TheTerrainLogic->getLayerHeight(objPos.X, objPos.Y, objectLayer);
        }
      }
    } else {  // no render object so use shadow's local position and default
              // orientation
    ...
    objPos.Z = 0.0f;  // we don't care about object height since shadows project
                      // top-down
    ...
    if (vecLength != 0.0f)  // prevent divide by zero
    {
      ...
      vVector = uVector;
      vVector.Rotate_Z(
          -1.0f, 0.0f);  // rotate u vector by -90 degress to get v vector.
    } else {
      ...
      uVector = vVector;
      uVector.Rotate_Z(1.0f,
                       0.0f);  // rotate v vector by 90 degress to get u vector.
    }
    ...
    /*		{	//This version will stretch to fit orientation of object
                            ///@todo: Most of the values below can be cached in
    shadow object shadow->m_robj->Get_Obj_Space_Bounding_Box(box);
                            decalSizeX=box.Extent.X*2.0f;	//use local
    space bounding box to determine shadow size decalSizeY=box.Extent.Y*2.0f;
    //			box=shadow->m_robj->Get_Bounding_Box();	//get
    world-space bounding box objPos = box.Center;
                            box.Init(objCenter,Vector3(decalSizeX*0.5f,decalSizeY*0.5f,1.0f));
                            box.Transform(objXform);	//transform box from
    object space to world space
                            box.Translate(objPos=objXform.Rotate_Vector(objPos));
                            objPos += shadow->m_robj->Get_Position();
                    }
    */
    //	  Experimental code to try and get a better fitting bounding box around
    //shadow
    /*  Experimental code to try and get a better fitting bounding box around
shadow {	//use the object's bounding box to determine shadow extent
            ///@todo: Most of the values below can be cached in shadow object
            uVector=objXform.Get_X_Vector();
            uVector.Z=0;
            uVector.Normalize();

            vVector=objXform.Get_Y_Vector();	//invert direction since v axis
runs right relative to u. vVector.Z=0; vVector.Normalize();

            shadow->m_robj->Get_Obj_Space_Bounding_Box(box);
            decalSizeX = box.Extent.X * 2.0f;
            decalSizeY = box.Extent.Y * 2.0f;

            Real newExtentX = fabs(uVector * box.Extent) + fabs(uVector *
box.Center);	//get new extent for object orientation Real newExtentY =
fabs(vVector * box.Extent) + fabs(vVector * box.Center);  //get new extent for
object orientation
...
*/
    ...
    // Get terrain cell index for area with shadow
    Int startX =
        REAL_TO_INT_FLOOR(((objPos.X + min_x) * mapScaleInv)) + borderSize;
    Int endX =
        REAL_TO_INT_CEIL(((objPos.X + max_x) * mapScaleInv)) + borderSize;
    Int startY =
        REAL_TO_INT_FLOOR(((objPos.Y + min_y) * mapScaleInv)) + borderSize;
    Int endY =
        REAL_TO_INT_CEIL(((objPos.Y + max_y) * mapScaleInv)) + borderSize;
    ...
    if (numExtraX >
        0) {  // figure out how much to clip out at each edge of decal
    ...
    if (vertsPerRow <= 1 || vertsPerColumn <= 1) return;  // nothing to render
    ...
    if (nShadowDecalVertsInBuf >
        (SHADOW_DECAL_VERTEX_SIZE - numVerts))  // check if room for model verts
    {  // flush the buffer by drawing the contents and re-locking again
      flushDecals(shadow->m_shadowTexture[0], shadow->m_type);
      if (shadowDecalVertexBufferD3D->Lock(
              0, numVerts * sizeof(SHADOW_DECAL_VERTEX),
              (unsigned char **)&pvVertices, D3DLOCK_DISCARD) != D3D_OK)
        return;
    ...
    // code to deal with rotated shadows based on sun direction, fix this later.
    // For now shadow rotates with object rotation.
    // shadow->m_shadowTexture[0]->getDecalUVAxis(&uVector,&vVector);
    // uVector *= 20.0f/dx;//shadow->m_decalRadius;	//scale texture to fit
    // object vVector *= 20.0f/dy;//1.2f;//shadow->m_decalRadius;	//scale
    // texture to fit object
    /*		uVector=objXform.Get_X_Vector();
                    uVector.Normalize();
                    uVector /= decalSizeX + (1.0f+4.0f/64.0f);	//texture has 1
       pixel transparent border so we strtech up to make sure solid pixels reach
       extent.. vVector=objXform.Get_Y_Vector() * -1.0f;	//invert
       direction since v axis runs right relative to u. vVector.Normalize();
                    vVector /= decalSizeY + (1.0f+4.0f/64.0f);
    */
    DEBUG_ASSERTCRASH(numVerts == ((endY - startY + 1) * (endX - startX + 1)),
                      ("queueDecal VB size mismatch"));
      ...
      if (layerHeight)
        for (j = startY; j <= endY; j++) {
          ...
          for (i = startX; i <= endX; i++) {
            ...
            hmapVertex.Z = __max(
                (float)hmap->getHeight(i, j) * MAP_HEIGHT_SCALE, layerHeight);
            pvVertices->x = hmapVertex.X;
            pvVertices->y = hmapVertex.Y;
            pvVertices->z = hmapVertex.Z;
            pvVertices->diffuse = shadow->m_diffuse;
            pvVertices->u =
                Vector3::Dot_Product(uVector, (hmapVertex - objPos)) + uOffset;
            pvVertices->v =
                Vector3::Dot_Product(vVector, (hmapVertex - objPos)) + vOffset;
            pvVertices++;
          }
        }
      else
    ...
    if (nShadowDecalIndicesInBuf >
        (SHADOW_DECAL_INDEX_SIZE - numIndex))  // check if room for model verts
    {  // flush the buffer by drawing the contents and re-locking again
      flushDecals(shadow->m_shadowTexture[0], shadow->m_type);

      if (shadowDecalIndexBufferD3D->Lock(0, numIndex * sizeof(short),
                                          (unsigned char **)&pvIndices,
                                          D3DLOCK_DISCARD) != D3D_OK)
        return;
      ...
      if (shadowDecalIndexBufferD3D->Lock(
              nShadowDecalIndicesInBuf * sizeof(short),
              numIndex * sizeof(short), (unsigned char **)&pvIndices,
...
/**Simpler/faster decal system that always uses 2 triangles that are roughly
oriented to terrain.  Since they are not projected onto terrain, there may be
clipping artifacts in certain situations.
TODO: Too much clipping.  Need to check terrain heights at all 4 corners and
adjust tilt to match*/
///@todo: We should have a pre-made static filled index buffer since we always
///send down 2 triangles.
void W3DProjectedShadowManager::queueSimpleDecal(W3DProjectedShadow *shadow) {
  ...
  if (TheTerrainRenderObject) {
    ...
    Real groundHeight =
        TheTerrainRenderObject->getHeightMapHeight(objPos.X, objPos.Y, &normal);
    Vector3 groundNormal(normal.x, normal.y, normal.z);

    // Find new tu vector parallel to terrain by projecting existing x_vector
    // onto terrain normal and subtracting the result.
    uVector = objXform.Get_X_Vector();
    Real uVectorAlongNormal = Vector3::Dot_Product(uVector, groundNormal);
    uVector -= uVectorAlongNormal * groundNormal;
    uVector.Normalize();
    // Find new tv vector parallel to terrain by crossing new tu vector with
    // terrain normal.
    ...
    objPos +=
        groundNormal *
        1.0f;  // offset decal slightly above terrain to reduce z-fighting.
    Vector3 vertex;

    if (pvVertices) {
      // Top-left
      vertex = objPos + vVector * shadow->m_decalSizeY * -0.5f -
               uVector * shadow->m_decalSizeX * 0.5f;
...
Int W3DProjectedShadowManager::renderShadows(RenderInfoClass &rinfo) {
  ...
  if (TheGlobalData->m_useShadowDecals) {
      ...
      if (shadow->m_isEnabled && !shadow->m_isInvisibleEnabled) {
        if (shadow->m_type & SHADOW_DECAL) {
          ...
          if (shadow->m_shadowTexture[0] != lastShadowDecalTexture ||
            ...
            flushDecals(lastShadowDecalTexture,
                        lastShadowType);  // switched to a new texture, need to
                                          // render polys using last texture.
            lastShadowDecalTexture = shadow->m_shadowTexture[0];
            lastShadowType = shadow->m_type;
          }
          ///@todo: may need to fix this if shadows are large enough to be seen
          ///while object is not visible
          if (shadow->m_robj
                  ->Is_Really_Visible()) {  // queueSimpleDecal(shadow);
            queueDecal(shadow);  // only draw shadow if casting object is
                                 // visible
            projectionCount++;
          }
          continue;
        ...
        if (result == CollisionMath::OVERLAPPED) {  // do a more accurate test
          ...
          aaBox.Translate(
              shadow->m_robj
                  ->Get_Position());  // translate bounding box to world space.
          if (CollisionMath::Overlap_Test(*shadowCameraFrustum, aaBox) ==
              CollisionMath::OUTSIDE)
            continue;
        } else if (result == CollisionMath::OUTSIDE)
          continue;

        // Shadow is visible on screen.  Figure out which visible objects it may
        // affect.
        ...
        if (shadow->m_type == SHADOW_PROJECTION) {
          ...
          // terrain is always visible and affected by all shadows so must
          // render
          projector->Peek_Material_Pass()->Install_Materials();
          DX8Wrapper::Apply_Render_State_Changes();  // force update of view and
                                                     // projection matrices
          if (renderProjectedTerrainShadow(shadow, aaBox)) projectionCount++;
          ...
          iter = ThePartitionManager->iterateObjectsInRange(
              (const Coord3D *)&sphere.Center, sphere.Radius, FROM_CENTER_3D);
          ...
          for (obj = iter->first(); obj; obj = iter->next()) {
            ...
            for (DrawModule **dm = draw->getDrawModules(); *dm; ++dm) {
              ...
              if (di) {
                ...
                ///@todo: don't apply shadows to translcuent objects unless they
                ///are MOBILE - hack to get tanks to work.
                if ((robj = w3dDraw->getRenderObject()) != 0 &&
                    (!robj->Is_Alpha() || !obj->isKindOf(KINDOF_IMMOBILE)) &&
                    robj != shadow->m_robj && robj->Is_Really_Visible()) {
                  // do a more accurate test against W3D render bounding boxes.
                  if (robj->Intersect_AABox(boxtest)) {
                    // Shadow reached a visible object so it needs to be
                    // rendered with shadow applied.
                    ...
                    projectionCount++;  // keep track of number of shadow
                                        // projections
                  }
                }  // robj
              }  // di
    ...
    flushDecals(lastShadowDecalTexture,
                lastShadowType);  // make sure there are not any unrendered
                                  // decals left over.
  ...
  if (m_decalList) {
    ...
    for (shadow = m_decalList; shadow; shadow = shadow->m_next) {
      if (shadow->m_isEnabled && !shadow->m_isInvisibleEnabled) {
        ...
        if (lastShadowType == SHADOW_NONE) lastShadowType = m_decalList->m_type;
...
/** Generic function which can be used to create arbitrary decals that don't
have to be used for shadows. Some examples: Scorch marks, blood, stains,
selection/status indicators, etc.*/
Shadow *W3DProjectedShadowManager::addDecal(
    Shadow::ShadowTypeInfo *shadowInfo) {
  W3DShadowTexture *st = NULL;
  ShadowType shadowType = SHADOW_NONE;  /// type of projection
  Bool allowWorldAlign = FALSE;  /// wrap shadow around world geometry - else
                                 /// align perpendicular to local z-axis.
  ...
  if (st == NULL) {
    ...
    TextureClass *w3dTexture =
        WW3DAssetManager::Get_Instance()->Get_Texture(texture_name);
    ...
    DEBUG_ASSERTCRASH(w3dTexture != NULL,
                      ("Could not load decal texture: %s\n", texture_name));

    if (!w3dTexture) return NULL;
  ...
  shadow->setTexture(
      0, st);  ///@todo: Fix projected shadows to allow multiple lights
  shadow->m_type = shadowType;  /// type of projection
  shadow->m_allowWorldAlign =
      allowWorldAlign;  /// wrap shadow around world geometry - else align
                        /// perpendicular to local z-axis.
  ...
  // add to our shadow list through the shadow next links, insert next to other
  // shadows using same texture

  W3DProjectedShadow *nextShadow = NULL, *prevShadow = NULL;
  for (nextShadow = m_decalList; nextShadow;
       prevShadow = nextShadow, nextShadow = nextShadow->m_next) {
    if (nextShadow->m_shadowTexture[0] ==
        st) {  // found start of other shadows using same texture, insert new
               // shadow here.
  ...
  if (nextShadow == NULL) {  // shadow with new texture. Add to top of list.
...
/** Generic function which can be used to create arbitrary decals that follow
the renderObject but don't have to be used for shadows. Some examples: Scorch
marks, blood, stains, selection/status indicators, etc.*/
Shadow *W3DProjectedShadowManager::addDecal(
    RenderObjClass *robj, Shadow::ShadowTypeInfo *shadowInfo) {
  W3DShadowTexture *st = NULL;
  ShadowType shadowType = SHADOW_NONE;  /// type of projection
  Bool allowWorldAlign = FALSE;  /// wrap shadow around world geometry - else
                                 /// align perpendicular to local z-axis.
  ...
  if (!decalSizeY)
    decalSizeY =
        box.Extent.Y * 2.0f;  // world space distance to stretch full texture
  shadow->m_oowDecalSizeY = 1.0f / decalSizeY;  // one over height

  if (decalOffsetX) decalOffsetX = decalOffsetX * shadow->m_oowDecalSizeX;

  if (decalOffsetY) decalOffsetY = decalOffsetY * shadow->m_oowDecalSizeY;
...
W3DProjectedShadow *W3DProjectedShadowManager::addShadow(
  ...
  if (shadowInfo) {
    ...
    if (shadowInfo->m_type ==
        SHADOW_DECAL) {  // simple decal using the premade texture specified.
      // can be always perpendicular to model's z-axis or projected
      // onto world geometry.
      nameLen = strlen(shadowInfo->m_ShadowName);
      ...
      decalOffsetX = shadowInfo->m_offsetX;
      decalOffsetY = shadowInfo->m_offsetY;
    } else if (shadowInfo->m_type ==
               SHADOW_PROJECTION) {  // projection of object geometry into a
                                     // texture.
      // can be applied on a plane horizontal to model's z-axis or
      // projected onto world geometry.
      if (shadowInfo->m_ShadowName[0] !=
          '\0') {  // the shadow will be based on another render object
        ...
        strcpy(texture_name,
               robj->Get_Name());  // not texture name give, assume model name.

      st = m_W3DShadowTextureManager->getTexture(texture_name);
      if (st == NULL) {  // texture doesn't exist, use current render object to
                         // create it
        ...
        if (st == NULL) return NULL;  // could not create the shadow texture
      }
      shadowType = SHADOW_PROJECTION;
    }  // SHADOW_PROJECTION
  } else {  // no shadow info, assume user wants a projected shadow
    strcpy(texture_name, robj->Get_Name());

    st = m_W3DShadowTextureManager->getTexture(texture_name);

    if (st == NULL) {  // did not find a cached copy of the shadow geometry,
                       // create a new one
      m_W3DShadowTextureManager->createTexture(robj, texture_name);
      // try loading again
      st = m_W3DShadowTextureManager->getTexture(texture_name);
  ...
  if (decalSizeX)
    decalSizeX =
        1.0f /
        decalSizeX;  // world space distance to stretch full texture scale
  else
    decalSizeX =
        1.0f / (box.Extent.X * 2.0f);  // use bounding box to determine size
  ...
  W3DProjectedShadow *nextShadow = NULL, *prevShadow = NULL;
  for (nextShadow = m_shadowList; nextShadow;
       prevShadow = nextShadow, nextShadow = nextShadow->m_next) {
    if (nextShadow->m_shadowTexture[0] ==
        st) {  // found start of other shadows using same texture, insert new
               // shadow here.
...
void W3DProjectedShadowManager::removeShadow(W3DProjectedShadow *shadow) {
  ...
  if (shadow->m_type & (SHADOW_ALPHA_DECAL | SHADOW_ADDITIVE_DECAL)) {
    for (next_shadow = m_decalList; next_shadow;
...
W3DProjectedShadow::W3DProjectedShadow(void) {
  ...
  m_allowWorldAlign = FALSE;  /// wrap shadow around world geometry - else align
                              /// perpendicular to local z-axis.
  m_isEnabled = TRUE;
  m_isInvisibleEnabled = FALSE;
  for (Int i = 0; i < MAX_SHADOW_LIGHTS; i++) m_shadowTexture[i] = NULL;
}
...
void W3DProjectedShadow::init(void) {
  DEBUG_ASSERTCRASH(m_shadowProjector == NULL,
                    ("Init of existing shadow projector"));
...
#define DECAL_TEXELS_PER_WORLD_UNIT \
  (64.0f / 20.0f)  // 64 texels per 2 terrain cells (20 units)
...
void W3DProjectedShadow::updateTexture(Vector3 &lightPos) {
  ...
  static Vector2 uvData[4] = {Vector2(-0.5, -0.5f), Vector2(-0.5, 0.5f),
                              Vector2(0.5f, 0.5f), Vector2(-0.5f, 0.5f)};

  // force light always 2000 units from object - for some reason projection
  // fails if light is too far.
  ///@todo: See why infinite light sources don't project shadows correctly.

  if (m_type ==
      SHADOW_PROJECTION) {  // projected shadows use custom runtime generated
                            // textures based on object geometry
    ...
    SurfaceClass *oldSurface =
        m_shadowTexture[0]->getTexture()->Get_Surface_Level();
    SurfaceClass *newSurface =
        TheW3DProjectedShadowManager->getRenderTarget()->Get_Surface_Level();

    // Copy shadow from temporary video-memory surface into a permanent texture
    oldSurface->Copy(0, 0, 0, 0, DEFAULT_RENDER_TARGET_WIDTH,
                     DEFAULT_RENDER_TARGET_HEIGHT, newSurface);
    REF_PTR_RELEASE(newSurface);
    REF_PTR_RELEASE(oldSurface);
    m_shadowTexture[0]->updateBounds(TheW3DShadowManager->getLightPosWorld(0),
                                     m_robj);  // update local shadow bounds
  } else if (m_type ==
             SHADOW_DECAL) {  // decal shadows use artist supplied textures.  We
                              // just need to tweak the uv coordinates to match
    // the light direction.
    Vector3 objPos = m_robj->Get_Position();
    ...
    // default shadow texture points along world -x axis (west).  Rotate uv
    // coordinates to fit actual light direction
    Vector3 uVec =
        objectToLight * DECAL_TEXELS_PER_WORLD_UNIT / (float)surface_desc.Width;
    objectToLight.Rotate_Z(
        -1.0f, 0.0f);  // rotate u vector by -90 degress to get v vector.
    Vector3 vVec = objectToLight * DECAL_TEXELS_PER_WORLD_UNIT /
                   (float)surface_desc.Height;
    ...
    AABoxClass box;
    SphereClass sphere;
    m_robj->Get_Obj_Space_Bounding_Box(
        box);  // shadow uses same bounding box as object
  ...
  m_shadowTexture[0]->setLightPosHistory(
      lightPos);  // store position of light at time of texture update.
}

void W3DProjectedShadow::updateProjectionParameters(
    const Matrix3D &cameraXform) {
  m_shadowProjector->Pre_Render_Update(cameraXform);
}

void W3DProjectedShadow::update(void) {
  if (m_shadowTexture[0]->getLightPosHistory() !=
      TheW3DShadowManager->getLightPosWorld(
          0)) {  // light has moved since last time this shadow was calculated.
                 // Need update
    updateTexture(TheW3DShadowManager->getLightPosWorld(0));
  }
  if (m_lastObjPosition !=
      m_robj->Get_Position()) {  // object has moved.  Texture stays the same
                                 // but projection matrix needs updating.
    // force light always 2000 units from object - for some reason projection
    // fails if light is too far.
    ///@todo: See why infinite light sources don't project shadows correctly.
    if (m_type == SHADOW_PROJECTION) {
      Vector3 objToLight =
          TheW3DShadowManager->getLightPosWorld(0) - m_robj->Get_Position();
      objToLight.Normalize();
...
Int W3DShadowTexture::init(RenderObjClass *robj) {
  ...
  TheW3DProjectedShadowManager->getRenderTarget()->Get_Level_Description(
      surface_desc);

  TextureClass *new_texture = MSGNEW("TextureClass")
      TextureClass(surface_desc.Width, surface_desc.Height, surface_desc.Format,
...
void W3DShadowTexture::updateBounds(Vector3 &lightPos, RenderObjClass *robj) {
  ...
  // project each box corner to base of object to determine rough extent of
  // shadow Get vertices of top of bounding box
  ...
  box.Init(Corners,
           8);  // generate a new bounding box to fit the shadow projection
...
void W3DShadowTextureManager::invalidateCachedLightPositions(void) {
  // step through each of our shadow textures and update previous light
  // position.
  Vector3 idVec(0, 0, 0);
...
/*
** An entry for a table of textures not found, so we can quickly determine their
*loss
*/
...
** Missing Textures
**
** The idea here, allow the system to register which textures are determined to
*be missing
...
int W3DShadowTextureManager::createTexture(RenderObjClass *robj,
  ...
  if (res != TRUE) {  // load failed!
    ...
    goto Error;
  } else if (peekTexture(newTexture->Get_Name()) !=
             NULL) {            // duplicate exists!
