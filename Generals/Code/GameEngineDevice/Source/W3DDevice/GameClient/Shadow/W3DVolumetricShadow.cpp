...
///@todo: Find better way to determine when shadow volumes need updating -
///lights move, objects move.
...
#define MAX_SILHOUETTE_EDGES \
  1024  // maximum number of shadov volume sides or edges in silhoutte
#define SHADOW_EXTRUSION_BUFFER \
  0.1f  // amount to extend shadow volume beyond what's required to hit ground.
#define AIRBORNE_UNIT_GROUND_DELTA 2.0f
#define MAX_SHADOW_LENGTH_SCALE_FACTOR \
  1.0f  // amount shadow can extend beyond the objects normal bounding sphere
#define MAX_SHADOW_LENGTH_EXTRA_AIRBORNE_SCALE_FACTOR \
  1.5f  // scales MAX_SHADOW_LENGTH_SCALE_FACTOR a little more for flying units
#define MAX_EXTRUSION_LENGTH \
  (512.0f * MAP_XY_FACTOR)  // maximum length of a shadow extrusion - assumed as
                            // 512x512 cell map for now.
#define MAX_SHADOW_EXTRUSION_UNDER_OBJECT_BEFORE_CLAMP \
  5.0f  // maximum amount that shadow can reach below object base (z-position)
        // before we clamp it's length to reduce artifacts.
#define SHADOW_SAMPLING_INTERVAL \
  (MAP_XY_FACTOR * 2.0f)  // stepsize along ray used to find lowest point on
                          // terrain within shadow's reach.
#define OVERHANGING_OBJECT_CLAMP_ANGLE \
  (80.0f / 180.0f * PI)  // for objects that are right on a cliff edge, clamp
                         // light angle to cast a nearly vertical shadow.
...
#ifdef SV_DEBUG  // in debug mode, dynamic shadows are rendered with random
                 // diffuse color
...
/** A simple structure to hold random geometry (vertices, polygons, etc.). We'll
 * use this to store shadow volumes. */
struct Geometry {
  enum VisibleState {
  ...
  Int Create(Int numVertices, Int numPolygons) {
    if (numVertices)
      if ((m_verts = NEW Vector3[numVertices]) == 0) return FALSE;
    if (numPolygons)
      if ((m_indices = NEW UnsignedShort[numPolygons * 3]) == 0) return FALSE;
  ...
  Int SetNumActivePolygon(Int numPolygons) {
    return m_numActivePolygon = numPolygons;
  }
  Int SetNumActiveVertex(Int numVertices) {
    return m_numActiveVertex = numVertices;
  }
  UnsignedShort *GetPolygonIndex(long dwPolyId, short *psIndexList,
                                 int dwNSize) const {
  ...
  Int FindVertexInRange(Int start, Int end, Vector3 *pvVertex) {
    for (Int i = start; i < end; i++) {
      if ((m_verts[i] - *pvVertex).Length2() == 0) return i;
    }
    return -1;
  }
 ...
 private:
  ...
  SphereClass
      m_boundingSphere;  ///< object space bounding sphere of shadow volume
  VisibleState
      m_visibleState;  ///< flag if this geometry was visible in this frame.
};
...
/**This class holds original mesh specific data and geometry.  The meshes stored
in this class have been cleaned to remove replicated vertices and also cache
mesh data needed for faster silhouette computation.  A model can contain many
meshes for which we need to store separate data so they can move relative to
each other.*/
class W3DShadowGeometryMesh {
 ...
 public:
  ...
  /// @todo: Cache/Store face normals someplace so they are not recomputed when
  /// lights move.
  Vector3 *GetPolygonNormal(long dwPolyNormId, Vector3 *pvNorm) {
    if (m_polygonNormals) return &(*pvNorm = m_polygonNormals[dwPolyNormId]);
    short indexList[3];
  ...
  void buildPolygonNormals(void) {
    if (!m_polygonNormals) {  // need to allocate storage
 ...
 protected:
  ...
  virtual int GetPolygonIndex(long dwPolyId, short *psIndexList,
                              int dwNSize) const {
  ...
  UnsignedShort
      *m_parentVerts;  ///< array of parent vertex indices for each vertex.
  /// the neighbor info indexed by polygon id
  PolyNeighbor *m_polyNeighbors;
  Int m_numPolyNeighbors;  // length of m_polyNeighbors and the number of
                           // polygons in our current geometry.
...
class W3DShadowGeometryHeightmapMesh : public W3DShadowGeometryMesh {
 public:
  ...
  void setPatchOrigin(Int x, Int y) {
    m_patchOriginX = x;
    m_patchOriginY = y;
  }
  void getPatchOrigin(Int *x, Int *y) {
    *x = m_patchOriginX;
    *y = m_patchOriginY;
  }
  void setPatchSize(Int size) {
    m_width = size;
    m_numPolygons = (size - 1) * (size - 1) * 2;
  }
  Int getPatchSize(void) { return m_width; }

 protected:
  Int m_heightmapPitch;  ///< width of full heightmap of which this mesh is a
                         ///< sub-rectangle
...
int W3DShadowGeometryHeightmapMesh::GetPolygonIndex(long dwPolyId,
                                                    short *psIndexList,
                                                    int dwNSize) const {
  // Find top left vertex of cell containing polygon
  WorldHeightMap *map = NULL;
  if (TheTerrainRenderObject) map = TheTerrainRenderObject->getMap();
  if (!map) return 0;
  ...
  float UA[4], VA[4];
  Bool flipForBlend;
  map->getAlphaUVData(column + m_patchOriginX, row + m_patchOriginY, UA, VA,
                      alpha, &flipForBlend, false);
  if (flipForBlend) {
    if (dwPolyId & 1) {
...
Vector3 *W3DShadowGeometryHeightmapMesh::GetVertex(int dwVertId,
                                                   Vector3 *pvVertex) {
  WorldHeightMap *map = NULL;
  ...
  pvVertex->Z = (Real)data[(m_patchOriginX + column) +
                           (m_patchOriginY + row) * map->getXExtent()] *
                MAP_HEIGHT_SCALE;
...
Bool isPatchShadowed(W3DShadowGeometryHeightmapMesh *hm_mesh) {
  ...
  //
  if (Vector3::Dot_Product(lightVector, normal) < 0.0f) firstVisible = 1;
  ...
  for (Int i = 1; i < hm_mesh->GetNumPolygon(); i++) {
    ...
    //
    testVisible = 0;
    if (Vector3::Dot_Product(lightVector, normal) < 0.0f) testVisible = 1;

    //		if (testVisible ^ firstVisible)
    //			return TRUE;	//found polys facing different
    //directions to sun, will cast shadow
    if (!testVisible)
      return TRUE;  // some part of mesh not facing light, so it could cast a
                    // shadow
  }
  return FALSE;
}
  ...
  for (Int y = 0; y < map->getYExtent(); y += patchSize - 1) {
    for (Int x = 0; x < map->getXExtent(); x += patchSize - 1) {
      W3DShadowGeometryHeightmapMesh *hm_mesh =
          &terrainMeshes[numTerrainMeshes];
      hm_mesh->setPatchOrigin(x, y);
      hm_mesh->setPatchSize(patchSize);

      if (isPatchShadowed(hm_mesh)) {  // some polygons in this patch cast
                                       // shadows, need to generate a mesh
...
data needed for efficient shadow volume generation.  The
W3DVolumetricShadowManager will allocate these structures and hash them for
quick re-use on other models sharing the same geometry.*/
class W3DShadowGeometry : public RefCountClass, public HashableClass {
 public:
  ...
  Int initFromHLOD(RenderObjClass *robj);  ///< initialize the geometry from a
                                           ///< W3D HLOD object.
  Int initFromMesh(RenderObjClass *robj);  ///< initialize the geometry from a
                                           ///< W3D Mesh object.
  ...
  void Set_Name(const char *name) {
    memset(
        m_namebuf, 0,
        sizeof(
            m_namebuf));  // pad with zero so always ends with null character.
 ...
 private:
  ...
  W3DShadowGeometryMesh
      m_meshList[MAX_SHADOW_CASTER_MESHES];  ///< collection of meshes for this
                                             ///< geometry.
...
Int W3DShadowGeometry::initFromHLOD(RenderObjClass *robj) {
  ...
  for (i = 0; i < hlod->Get_Lod_Model_Count(top); i++) {
    if (hlod->Peek_Lod_Model(top, i) &&
        hlod->Peek_Lod_Model(top, i)->Class_ID() ==
            RenderObjClass::CLASSID_MESH) {
      DEBUG_ASSERTCRASH(m_meshCount < MAX_SHADOW_CASTER_MESHES,
                        ("Too many shadow sub-meshes"));

      geomMesh->m_mesh = (MeshClass *)hlod->Peek_Lod_Model(top, i);
      geomMesh->m_meshRobjIndex = i;

      if ((geomMesh->m_mesh->Is_Alpha() ||
           geomMesh->m_mesh->Is_Translucent()) &&
          !geomMesh->m_mesh->Peek_Model()->Get_Flag(
              MeshGeometryClass::CAST_SHADOW))
        continue;  // transparent meshes that don't have forced shadows will not
      ...
      for (j = 0; j < geomMesh->m_numVerts; j++) {
        ...
        for (k = j + 1; k < geomMesh->m_numVerts; k++) {
          ...
          if (len.Length2() == 0) {  // found duplicate vertex
            vertParent[k] = j;
      ...
      memcpy(geomMesh->m_parentVerts, vertParent,
             sizeof(UnsignedShort) * geomMesh->m_numVerts);
      geomMesh->m_numVerts =
          newVertexCount;  // adjust actual vertex count to ignore duplicates
...
Int W3DShadowGeometry::initFromMesh(RenderObjClass *robj) {
  ...
  if (((geomMesh->m_mesh->Is_Alpha() || geomMesh->m_mesh->Is_Translucent()) &&
       !geomMesh->m_mesh->Peek_Model()->Get_Flag(
           MeshGeometryClass::CAST_SHADOW)))
    return FALSE;  // transparent meshes that don't have forced shadows will not
                   // cast volumetric shadows
...
Int W3DShadowGeometry::init(RenderObjClass *robj) {
  return TRUE;
  //	m_robj=robj;
  /*	//code to deal with granny - don't think we'll use shadow volumes on
     these!? granny_file *fileInfo=robj->getPrototype().m_file;
          ...
          for (Int modelIndex=0; modelIndex<fileInfo->ModelCount; modelIndex++)
          {
                  ...
                  {	//found a collision box, copy out data
                          ...
                          if (MeshCount==1)
                          {
                                  granny_mesh *sourceMesh =
     sourceModel->MeshBindings[0].Mesh; granny_pn33_vertex *Vertices =
     (granny_pn33_vertex *)sourceMesh->PrimaryVertexData->Vertices; Vector3
     points[24];

                                  assert
     (sourceMesh->PrimaryVertexData->VertexCount <= 24);

                                  for (Int boxVertex=0;
     boxVertex<sourceMesh->PrimaryVertexData->VertexCount; boxVertex++) {
     points[boxVertex].Set(Vertices[boxVertex].Position[0],
                  ...
                  {	//mesh is part of model
                          int meshCount = sourceModel->MeshBindingCount;
                          for (Int meshIndex=0; meshIndex<meshCount;
     meshIndex++)
                          {
                                  granny_mesh *sourceMesh =
     sourceModel->MeshBindings[meshIndex].Mesh; if
     (sourceMesh->PrimaryVertexData)
...
W3DShadowGeometryMesh::~W3DShadowGeometryMesh(void) {
  ...
  if (m_polygonNormals) delete[] m_polygonNormals;
...
void W3DShadowGeometryMesh::buildPolygonNeighbors(void) {
  ...
  if (numPolys == 0) {
    ...
    //
    if (m_numPolyNeighbors != 0) deleteNeighbors();
  ...
  if (numPolys != m_numPolyNeighbors) {
    ...
    if (allocateNeighbors(numPolys) == FALSE) return;
  ...
  for (i = 0; i < m_numPolyNeighbors; i++) {
    ...
    for (j = 0; j < m_numPolyNeighbors; j++) {
      Int a, b;
      Int index1, index2;
      Int index1Pos[2];  // positions of shared edge vertices in triangle list.
                         // (0,1 or 2)
      Int diff1, diff2;
          ...
          if (poly[a] == otherPoly[b]) {
            if (index1 == -1) {
              ...
              index1Pos[1] = b;
                       ...
                       -1) {  // Check direction of edge in each polygon.  If
                              // they are same direction skip it.
              diff1 = a - index1Pos[0];
              diff2 = b - index1Pos[1];
              if (((diff1 & 0x80000000) ^ ((abs(diff1) & 2) << 30)) !=
                ...
                // Check if the 2 polygons face in exactly opposite directions -
                // don't allow this type of neighbor.
                if (fabs(Vector3::Dot_Product(vOtherNorm, vNorm) + 1.0f) <=
                    0.01f)
                  continue;
                ...
                continue;
            } else {  // This is the same poly facing opposite direction.
                      // //assert( 0 );  // should never match 3 vertices!
              index1 = index2 = -1;
              continue;
            }
          }  // end if
      if (index1 != -1 && index2 != -1) {
        ...
        if (a == MAX_POLYGON_NEIGHBORS) {
          ...
          //					sprintf(errorText,"%s: Shadow
          //Polygon with too many neighbors at
          //%f,%f,%f",m_parentGeometry->Get_Name(),pv[0].X,pv[0].Y,pv[0].Z);
          //					DEBUG_LOG(("****%s Shadow
          //Polygon with too many neighbors at
          //%f,%f,%f\n",m_parentGeometry->Get_Name(),pv[0].X,pv[0].Y,pv[0].Z));
          //					DEBUG_ASSERTCRASH(a !=
          //MAX_POLYGON_NEIGHBORS,(errorText));
        }
...
void W3DVolumetricShadow::updateOptimalExtrusionPadding(void) {
  if (m_robj) {
    ...
    //			draw = draw;	//debug code for China06 wacky bridge
    //shadow
    ...
    if (m_shadowLengthScale) {  // Find light's distance from origin in xy plane
      Real lightXYDistance = sqrt(lightPosWorld.X * lightPosWorld.X +
                                  lightPosWorld.Y * lightPosWorld.Y);
      Real newZ = lightXYDistance * m_shadowLengthScale;

      if (newZ >
          lightPosWorld.Z) {  // clamped z component is higher than actual light
                              // position allows so adjust it.
        lightPosWorld.Z = newZ;
      }
    }

    // find maximum shadow length which will not cause any corners of the
    // object's bounding box to cast shadows that drop significantly below the
    // object's base.  This will help avoid artifacts when we have an object on
    // a cliff/hill casting shadows onto the ground below. We need this hack
    // because the terrain does not cast shadows and looks weird when objects
    ...
    // Get vertices of top of bounding box since they will generate the longest
    // shadow
    ...
    for (Int i = 0; i < 4; i++) {
      ...
      if (TheTerrainRenderObject->Cast_Ray(raytest) &&
          !raytest.Result->StartBad) {  // Found intersection point where shadow
                                        // has its maximum length.  Do a quick
        // search to see if terrain falls significantly below the height of the
        // object anywhere between the base and the intersection point.  If so,
        // we either need to extend shadow extrusion or make the light angle
        // more vertical.

        shadowRay.Set(
            result.ContactPoint -
            Corners[i]);  // vector from object corner to terrain intersection.
        shadowRay.Z = 0;  // remove z-component since we'll be sampling along
                          // the xy plane.

        // walk along the shadow/light direction vector looking for large dips -
        // indicating object is on hill or cliff.
        ...
        for (Int j = 0; j < numSteps; j++) {
          ...
          Real terrainHeight = TheTerrainRenderObject->getHeightMapHeight(
              terrainPoint.X, terrainPoint.Y, NULL);
          if (terrainHeight <
              (objPos.Z -
               MAX_SHADOW_EXTRUSION_UNDER_OBJECT_BEFORE_CLAMP))  // check if
                                                                 // terrain dips
                                                                 // more than 10
                                                                 // units under
                                                                 // object.
          {
            if (j == 0)  // this is the initial point so object must be right on
                         // the edge of a cliff.
            {
              baseGroundHeight =
                  terrainHeight;  // force extrusion all the way down cliff.
              Real tanAngle =
                  tan(OVERHANGING_OBJECT_CLAMP_ANGLE);  // clamp to about 89
                                                        // degrees or close to
                                                        // vertical lightpos.
              setShadowLengthScale(
                  tanAngle);  // update the clamp angle to shorted shadow enough
                              // so this corner on flat ground.
              break;
            }

            // Find ray from last valid terrain contact point to object box
            // corner.
            ...
            if (clampAngle >= (PI / 2.0f) || clampAngle <= 0)
              clampAngle =
                  OVERHANGING_OBJECT_CLAMP_ANGLE;  // clamp to about 89 degrees
          ...
          if (terrainHeight < baseGroundHeight) {
            baseGroundHeight =
                terrainHeight;  // point was below object but within safety
                                // margin so record it's position.
    ...
    m_extraExtrusionPadding =
        objPos.Z - baseGroundHeight + SHADOW_EXTRUSION_BUFFER;

    DEBUG_ASSERTCRASH(
        m_extraExtrusionPadding <= (255.0f * MAP_HEIGHT_SCALE),
        ("Warning: Volumetric Shadow UpdateOptimalExtrusionPadding too large"));
  }
}
...
void W3DVolumetricShadow::getRenderCost(RenderCost &rc) const {
  ...
  if (m_geometry && m_isEnabled && !m_isInvisibleEnabled &&
    ...
    for (i = 0; i < MAX_SHADOW_LIGHTS; i++) {
      for (j = 0; j < m_geometry->getMeshCount(); j++) {
        ...
        if (mesh && mesh->Is_Not_Hidden_At_All()) drawCount++;
      }
    }
  }
...
void W3DVolumetricShadow::RenderMeshVolume(Int meshIndex, Int lightIndex,
  ...
  ///@todo: W3D always does transpose on all of matrix sets.  Slow???  Better to
  ///hack view matrix.
  ...
  if (!vbSlot) return;
  if (vbSlot->m_VB->m_DX8VertexBuffer->Get_DX8_Vertex_Buffer() !=
      lastActiveVertexBuffer) {
    lastActiveVertexBuffer =
        vbSlot->m_VB->m_DX8VertexBuffer->Get_DX8_Vertex_Buffer();
    m_pDev->SetStreamSource(0, lastActiveVertexBuffer,
                            vbSlot->m_VB->m_DX8VertexBuffer->FVF_Info()
                                .Get_FVF_Size());  // 12 bytes per vertex.
  }

  DEBUG_ASSERTCRASH(vbSlot->m_size >= numVerts,
                    ("Overflowing Shadow Vertex Buffer Slot"));

  W3DBufferManager::W3DIndexBufferSlot *ibSlot =
      m_shadowVolumeIB[lightIndex][meshIndex];
  if (!ibSlot) return;

  DEBUG_ASSERTCRASH(ibSlot->m_size >= numIndex,
                    ("Overflowing Shadow Index Buffer Slot"));
  ...
  if (DX8Wrapper::_Is_Triangle_Draw_Enabled()) {
    Debug_Statistics::Record_DX8_Polys_And_Vertices(
        numPolys, numVerts, ShaderClass::_PresetOpaqueShader);
    m_pDev->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, numVerts,
                                 ibSlot->m_start, numPolys);
  }
}

void W3DVolumetricShadow::RenderDynamicMeshVolume(Int meshIndex, Int lightIndex,
                                                  const Matrix3D *meshXform) {
  Geometry *geometry;
  Int numVerts, numPolys, numIndex;
  ...
  {  // flush the buffer by drawing the contents and re-locking again
    if (shadowVertexBufferD3D->Lock(
            0, numVerts * sizeof(SHADOW_DYNAMIC_VOLUME_VERTEX),
    ...
    if (shadowVertexBufferD3D->Lock(
            nShadowVertsInBuf * sizeof(SHADOW_DYNAMIC_VOLUME_VERTEX),
  ...
  if (pvVertices) {
    ...
    for (Int i = 0; i < numVerts; i++) {
      (*((Vector3 *)pvVertices)) =
          *geometry->GetVertex(i);  // cast is valid since both start with xyz
      pvVertices->diffuse =
          (rand() % 255) | ((rand() % 255) << 8) | ((rand() % 255) << 16);
      pvVertices++;
    }
#else
    memcpy(pvVertices, geometry->GetVertex(0),
           numVerts * sizeof(SHADOW_DYNAMIC_VOLUME_VERTEX));
#endif
  }
  ...
  if (nShadowIndicesInBuf >
      (SHADOW_INDEX_SIZE - numIndex))  // check if room for model verts
  {  // flush the buffer by drawing the contents and re-locking again
    if (shadowIndexBufferD3D->Lock(0, numIndex * sizeof(short),
                                   (unsigned char **)&pvIndices,
  ...
  if (pvIndices) {
    memcpy(pvIndices, geometry->GetPolygonIndex(0, (short *)pvIndices, 3),
           numPolys * 3 * sizeof(short));
  }
  ...
  if (shadowVertexBufferD3D != lastActiveVertexBuffer) {
    m_pDev->SetStreamSource(0, shadowVertexBufferD3D,
                            sizeof(SHADOW_DYNAMIC_VOLUME_VERTEX));
  ...
  if (DX8Wrapper::_Is_Triangle_Draw_Enabled()) {
    Debug_Statistics::Record_DX8_Polys_And_Vertices(
        numPolys, numVerts, ShaderClass::_PresetOpaqueShader);
    m_pDev->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, numVerts,
                                 nShadowStartBatchIndex, numPolys);
  }
...
void W3DVolumetricShadow::RenderMeshVolumeBounds(Int meshIndex, Int lightIndex,
                                                 const Matrix3D *meshXform) {
  Geometry *geometry;
  Int numVerts, numPolys, numIndex;
  SHADOW_DYNAMIC_VOLUME_VERTEX *pvVertices;
  ...
  static Vector3i _BoxFaces[12] = {
      ...
      Vector3i(0, 2, 3), Vector3i(4, 7, 6),  // -z faces
      Vector3i(4, 6, 5), Vector3i(0, 3, 7),  // +x faces
      Vector3i(0, 7, 4), Vector3i(1, 5, 6),  // -x faces
      Vector3i(1, 6, 2), Vector3i(4, 5, 1),  // +y faces
      Vector3i(4, 1, 0), Vector3i(3, 2, 6),  // -y faces
      Vector3i(3, 6, 7)};
  ...
  Matrix4 mWorld(1);  // identity since boxes are pre-transformed to world
                      // space.
...
W3DVolumetricShadow::~W3DVolumetricShadow(void) {
  ...
  for (j = 0; j < MAX_SHADOW_CASTER_MESHES; j++) deleteSilhouette(j);

  // free any shadow volume data
  for (i = 0; i < MAX_SHADOW_LIGHTS; i++) {
    for (j = 0; j < MAX_SHADOW_CASTER_MESHES; j++) {
      if (m_shadowVolume[i][j]) delete m_shadowVolume[i][j];
  ...
  if (m_geometry) REF_PTR_RELEASE(m_geometry);
...
void W3DVolumetricShadow::SetGeometry(W3DShadowGeometry *geometry) {
  ...
  for (Int i = 0; i < MAX_SHADOW_CASTER_MESHES; i++) {
    if (m_geometry) numPrevVertices = m_geometry->getMesh(i)->GetNumVertex();

    // now many vertices does our new geometry have
    if (geometry) numNewVertices = geometry->getMesh(i)->GetNumVertex();
    ...
    if (numNewVertices > numPrevVertices) {
      deleteSilhouette(i);
      if (allocateSilhouette(i, numNewVertices) == FALSE) return;
...
void W3DVolumetricShadow::Update() {
  ...
  //	if( currentTime - lastTime >= delay )
  {
    ...
    if (pos == originCompareVector) {  // the transform on this object was never
                                       // set so we can't make any determination
      // if it's visible or what the shadow looks like.
      return;
    }
    // Check if this is a "flying" unit.  Flying units are defined as anything
    // that moves more than AIRBORNE_UNIT_GROUND_DELTA world units above ground.
    // This will allow "jumping" units like rocket buggies from being flagged as
    // "flying".  We minimize the number of flying units because their shadow
    // volumes are extruded longer to reach the lowest point on the map. Regular
    // shadows only extend far enough to reach the base of the model (presumes
    // base is already touching the ground).
    Real groundHeight;
    if (TheTerrainLogic)
      groundHeight = TheTerrainLogic->getGroundHeight(
          pos.X, pos.Y);  // logic knows about bridges so use if available.
    else
      groundHeight =
          TheTerrainRenderObject->getHeightMapHeight(pos.X, pos.Y, NULL);
    if (fabs(pos.Z - groundHeight) >= AIRBORNE_UNIT_GROUND_DELTA) {
      Real extent =
          MAX_SHADOW_LENGTH_EXTRA_AIRBORNE_SCALE_FACTOR * m_robjExtent;
      ...
      // this unit is above ground, extend shadow volume to reach lowest point
      // on the terrain plus extra bit to make sure shadow goes under ground.
      updateVolumes(fabs(pos.Z - TheTerrainRenderObject->getMinHeight()) +
                    SHADOW_EXTRUSION_BUFFER);
    } else {  // normal object that is not floating above ground so we don't
              // need to extend the shadow lower than the object's
      ...
      // check if this object has never had it's extrusion length updated.  Will
      // only be true for immobile objects because finding an optimal extrusion
      // length is expensive.
      if (!m_extraExtrusionPadding) updateOptimalExtrusionPadding();
...
void W3DVolumetricShadow::updateVolumes(Real zoffset) {
  ...
  for (i = 0; i < MAX_SHADOW_LIGHTS; i++) {
    for (j = 0; j < m_geometry->getMeshCount(); j++) {
      ...
      if (mesh) {
        if (!mesh->Is_Not_Hidden_At_All()) continue;
        /**@todo: Getting the transform of the mesh may be forcing a full
           hierarchy evaluation. Expensive for off-screen models... do we really
           need this?	*/
        // Extend floor of model by 'zoffset' to compensate for flying units.
        updateMeshVolume(j, i, &mesh->Get_Transform(), mesh->Get_Bounding_Box(),
                         m_robj->Get_Position().Z - zoffset);
        // update visibility if not set yet
        if (m_shadowVolume[i][j]) {
          if (m_shadowVolume[i][j]->getVisibleState() ==
              Geometry::STATE_UNKNOWN) {  // Updating the mesh volume didn't
                                          // update the visibility, must do it
                                          // here.
            // First check against bounding sphere
            if (parentVis) {  // parent is visible so most likely all
                              // sub_objects are also visible so probably all
                              // shadows also visible. skip additional
                              // visibility tests
              ...
              if (result ==
                  CollisionMath::OVERLAPPED) {  // do a more accurate test
                                                // against bounding box.
                aaBox = m_shadowVolume[i][j]->getBoundingBox();
                aaBox.Translate(mesh->Get_Transform()
                                    .Get_Translation());  // translate bounding
                                                          // box to world space.
                if (CollisionMath::Overlap_Test(*shadowCameraFrustum, aaBox) !=
                    CollisionMath::OUTSIDE)
                  m_shadowVolume[i][j]->setVisibleState(
                      Geometry::STATE_VISIBLE);
                else
                  m_shadowVolume[i][j]->setVisibleState(
                      Geometry::STATE_INVISIBLE);
              } else
                m_shadowVolume[i][j]->setVisibleState(
                    (Geometry::VisibleState)result);
            }
          }
          if (m_shadowVolume[i][j]->getVisibleState() ==
              Geometry::STATE_VISIBLE) {  // shadow volume is visible.  Add it
                                          // to list of rendertasks.
            W3DBufferManager::W3DVertexBufferSlot *vbSlot =
                m_shadowVolumeVB[i][j];
            if (vbSlot) {  // add to static mesh volume list.
              W3DBufferManager::W3DRenderTask *oldTask =
                  vbSlot->m_VB->m_renderTaskList;
...
/*floorZ is the assumed ground height below the model.  The code will try to
extrude shadows just long enough to hit this point in order to reduce fill rate
usage.*/
void W3DVolumetricShadow::updateMeshVolume(Int meshIndex, Int lightIndex,
                                           const Matrix3D *meshXform,
  ...
  // Figuring out if mesh has rotated is cheaper (no normalization) than
  // figuring out if light angle has changed. So we divide the 2 tests.  Also,
  // our light (sun) almost never moves so no second test needed at all.
  Bool isMeshRotating =
      false;  // flag if mesh has rotated since last update. Translation doesn't
              // matter for infinite light source.
...
#else
  // When dealing with infinite light sources, we can assume that the shadow
  // doesn't change much based on object position.  Only the orientation to
  // light matters.
  Real cosAngle =
      fabs(Vector3::Dot_Product((Vector3 &)(prevXForm->operator[](0)),
                                (Vector3 &)(objectToWorld.operator[](0))));
  if (cosAngle >= cosAngleToCare) {
    cosAngle =
        fabs(Vector3::Dot_Product((Vector3 &)(prevXForm->operator[](1)),
                                  (Vector3 &)(objectToWorld.operator[](1))));
    if (cosAngle >= cosAngleToCare) {
      cosAngle =
          fabs(Vector3::Dot_Product((Vector3 &)(prevXForm->operator[](2)),
                                    (Vector3 &)(objectToWorld.operator[](2))));
      if (cosAngle < cosAngleToCare) isMeshRotating = true;
    } else
      isMeshRotating = true;
  } else
    isMeshRotating = true;
  ...
  if (m_shadowLengthScale) {  // Find light's distance from origin in xy plane
    ...
    if (newZ > lightPosWorld.Z) {  // clamped z component is higher than actual
                                   // light position allows so adjust it.
      lightPosWorld.Z = newZ;
    }
  }

  if (lightPosWorld !=
      m_lightPosHistory[lightIndex][meshIndex]) {  // Light position has moved,
                                                   // see if enough to matter
    ...
    ///@todo: Find a better way to deal with this - use maximum extrusion once!
    ///Also avoid hit for units climbing hills.
    if (fabs(objectCenter.Z - prevXForm->operator[](2).W) >
        SHADOW_EXTRUSION_BUFFER)
      isLightMoving = true;  // treat model rising just like rotation since
                             // volume needs update for longer extrusion.
  ...
  if (isLightMoving || isMeshRotating) {
    ...
    //
    Real det;
    D3DXMatrixInverse((D3DXMATRIX *)&worldToObject, &det,
                      (D3DXMATRIX *)&objectToWorld);

    // find out light position in object space
    Matrix4::Transform_Vector(worldToObject, lightPosWorld, &lightPosObject);

    // Updating shadow volumes is expensive, so verify that this volume is even
    // visible.

    // Generate bounding box around shadow volume by extruding AABB corners
    AABoxClass box(meshBox);  // copy current mesh bounding box (will be smaller
                              // than shadow box).
    SphereClass
        sphere;  // rough bounding sphere of shadow volume - based on box.
    Vector3 Corners[8];
    Vector3 lightRay;
    ...
    lightRay *= length;
    vectorScaleMax = vectorScale =
        (Real)fabs((Corners[0].Z - floorZ) /
                   lightRay.Z);  // length of vector from top corner to ground.
    ...
    if (vectorScaleTemp > vectorScaleMax)
      vectorScaleMax =
          vectorScaleTemp;  // keep track of maximum required extrusion length.

    lightRay = Corners[2] - lightPosWorld;  // vector light to corner
    length = 1.0f / lightRay.Length();
    lightRay *= length;
    vectorScale =
        (Real)fabs((Corners[2].Z - floorZ) /
                   lightRay.Z);  // length of vector from top corner to ground.
    ...
    if (vectorScale > vectorScaleMax)
      vectorScaleMax =
          vectorScale;  // keep track of maximum required extrusion length.

    lightRay = Corners[3] - lightPosWorld;  // vector light to corner
    length = 1.0f / lightRay.Length();
    lightRay *= length;
    vectorScaleTemp =
        (Real)fabs((Corners[3].Z - floorZ) /
                   lightRay.Z);  // length of vector from top corner to ground.
    ...
    sphere.Init(box.Center,
                box.Extent.Length());  // generate object space bounding sphere
                                       // containing box.
    ...
    if (result != CollisionMath::OUTSIDE) {
      ...
      if (m_numSilhouetteIndices[meshIndex] !=
          0) {  // this silhouette was built before and is being updated.
      ...
      if (m_shadowVolumeVB[lightIndex]
                          [meshIndex]) {  // Updating an existing vertex buffer
                                          // shadow volume.  This means we're
        // probably dealing with an animated mesh.  Update flags to reflect this
        // fact.
        if (isMeshRotating || isLightMoving) {
          if (isMeshRotating) {  // rotating meshes will most likely need
                                 // updates each frame, so stop using static
                                 // vertex buffers.
            m_shadowVolume[lightIndex][meshIndex]->SetFlags(
                m_shadowVolume[lightIndex][meshIndex]->GetFlags() |
                SHADOW_DYNAMIC);
          }
          // release memory used to store vertices/polygons
          resetShadowVolume(
              lightIndex,
              meshIndex);  // free vertex buffers since not used for dynamic.
          // Resize the shadow volume since we'll need room to store the
          // vertices in memory instead of VB.
          allocateShadowVolume(lightIndex, meshIndex);
        }
      }
      ...
      if (m_shadowVolume[lightIndex][meshIndex]->GetFlags() & SHADOW_DYNAMIC)
        ...
        constructVolumeVB(&lightPosObject, vectorScaleMax, lightIndex,
                          meshIndex);
      ...
      m_shadowVolume[lightIndex][meshIndex]->setVisibleState(
          Geometry::STATE_VISIBLE);  // this volume needs rendering.
    }  // end if inside view frustum
    else if (m_shadowVolume[lightIndex][meshIndex]) {  // outside view frustum,
                                                       // shadow wasn't updated.
      box.Translate(-objectCenter);  // translate box to object space.
      m_shadowVolume[lightIndex][meshIndex]->setBoundingBox(box);
      sphere.Center -= objectCenter;
      m_shadowVolume[lightIndex][meshIndex]->setBoundingSphere(sphere);
  ...
  else {  // not reconstructing volume, so don't know if visible or not.
    if (m_shadowVolume[lightIndex][meshIndex])
      m_shadowVolume[lightIndex][meshIndex]->setVisibleState(
          Geometry::STATE_UNKNOWN);
  }
}
...
// ============================================================================
void W3DVolumetricShadow::addSilhouetteIndices(Int meshIndex, Short edgeStart,
                                               Short edgeEnd) {
  //	DBGPRINTF(( "addSilhouetteIndices: Adding (%d,%d), the storage before
  //the add is = (%d/%d)\n", 							edgeStart, edgeEnd, m_numSilhouetteIndices,
...
void W3DVolumetricShadow::buildSilhouette(Int meshIndex,
  ...
  for (i = 0; i < numPolys; i++) {
    ...
    for (j = 0; j < MAX_POLYGON_NEIGHBORS; j++) {
      ...
      if (polyNeighbor->neighbor[j].neighborIndex != NO_NEIGHBOR) {
        ...
        otherNeighbor =
            geomMesh->GetPolyNeighbor(polyNeighbor->neighbor[j].neighborIndex);

        //
        // ignore neighbors that are marked as processed as those
        // onces have already detected edges if present
        //
        if (BitTest(otherNeighbor->status, POLY_PROCESSED)) continue;  // for j
  ...
  m_numIndicesPerMesh[meshIndex] =
      m_numSilhouetteIndices[meshIndex] - meshEdgeStart;
...
//
// ============================================================================
void W3DVolumetricShadow::constructVolume(Vector3 *lightPosObject,
                                          Real shadowExtrudeDistance,
                                          Int volumeIndex, Int meshIndex) {
  ...
  if (!indicesPerMesh) return;  // nothing to draw
  ...
  Int stripLength =
      1;  // keeps track of segments in strip (each being 2 triangles).
  ...
  for (i = 0; i < indicesPerMesh; i += 2) {
    ...
    for (k = i + 2; k < indicesPerMesh; k += 2)
      if (silhouetteIndices[k] == currentEdgeEnd) {  // swap the two edges
    ...
    if (k >= indicesPerMesh) {  // reached end of strip. Insert final edge.
      // Check if last edge wraps around to start.
      if (currentEdgeEnd ==
          stripStartIndex) {  // add end of strip that wraps around to start
                              // (forming closed cylinder/shape)
        //
      ...
      } else {  // add end of strip.  Finishes the last 2 polygons.
      ...
      if ((i + 2) >= indicesPerMesh) {  // finished with all silhouette edges
        polygonCount += 2;
#ifdef RECORD_SHADOW_STRIP_STATS
        numStrips++;
#endif
...
#endif
      continue;
    } else {  // continue existing strip by adding extra vertex and extrusion
...
void W3DVolumetricShadow::constructVolumeVB(Vector3 *lightPosObject,
  ...
  //*****************************************************************************************/
  // Do an initial pass through silhouette data to determine the actual
  // vertex/polygon counts. This number can't be determined any other way since
  // it depends on degree of vertex sharing in model.  We don't want to
  // overallocate because vertex buffer space is limited. This pass is also used
  // to sort the edges so they are all connected in strip order.
  {
#ifdef RECORD_SHADOW_STRIP_STATS
    Int numStrips = 0;  // keeps track of number of strips generated.
    Int stripLength =
    ...
    for (i = 0; i < indicesPerMesh; i += 2) {
      ...
      // look for edge connected to this one and move it next to this edge
      // in index list.  Rendering edges back-to-back improves vertex cache
      // usage.
      for (k = i + 2; k < indicesPerMesh; k += 2)
        if (silhouetteIndices[k] == currentEdgeEnd) {  // swap the two edges
          Int tempIndex = *(Int *)(&silhouetteIndices[i + 2]);
      ...
      if (k >= indicesPerMesh) {  // reached end of strip. Insert final edge.
        ...
        if (currentEdgeEnd ==
            stripStartIndex) {  // add end of strip that wraps around to start
                                // (forming closed cylinder/shape)
                                //
                                //  add the polygon consisting of the two edge
                                //  vertices and the first extruded point
                                //
        } else {  // add end of strip.  Finishes the last 2 polygons.
          lastEdgeVertex2Index = vertexCount;
          lastExtrude2Index = vertexCount + 1;
  ...
  DEBUG_ASSERTCRASH(m_shadowVolumeVB[volumeIndex][meshIndex] == NULL,
                    ("Updating Existing Static Vertex Buffer Shadow"));
  vbSlot = m_shadowVolumeVB[volumeIndex][meshIndex] =
      TheW3DBufferManager->getSlot(W3DBufferManager::VBM_FVF_XYZ, vertexCount);

  DEBUG_ASSERTCRASH(vbSlot != NULL,
                    ("Can't allocate vertex buffer slot for shadow volume"));

  DEBUG_ASSERTCRASH(vbSlot->m_size >= vertexCount,
                    ("Overflowing Shadow Vertex Buffer Slot"));

  DEBUG_ASSERTCRASH(
      m_shadowVolume[volumeIndex][meshIndex]->GetNumPolygon() == 0,
      ("Updating Existing Static Shadow Volume"));

  DEBUG_ASSERTCRASH(m_shadowVolumeIB[volumeIndex][meshIndex] == NULL,
                    ("Updating Existing Static Index Buffer Shadow"));
  ibSlot = m_shadowVolumeIB[volumeIndex][meshIndex] =
      TheW3DBufferManager->getSlot(polygonCount * 3);

  DEBUG_ASSERTCRASH(ibSlot != NULL,
                    ("Can't allocate index buffer slot for shadow volume"));

  DEBUG_ASSERTCRASH(ibSlot->m_size >= (polygonCount * 3),
                    ("Overflowing Shadow Index Buffer Slot"));

  if (!ibSlot || !vbSlot) {  // could not allocate storage to hold buffers
  ...
  DX8VertexBufferClass::AppendLockClass lockVtxBuffer(
      vbSlot->m_VB->m_DX8VertexBuffer, vbSlot->m_start, vertexCount);
  VertexFormatXYZ *vb = (VertexFormatXYZ *)lockVtxBuffer.Get_Vertex_Array();

  if (vb == NULL) return;

  DX8IndexBufferClass::AppendLockClass lockIdxBuffer(
      ibSlot->m_IB->m_DX8IndexBuffer, ibSlot->m_start, polygonCount * 3);
  ...
  *vb++ = *(VertexFormatXYZ *)&extrude2;
  ...
  for (i = 0; i < indicesPerMesh; i += 2) {
    ...
    // Check if another edge is connected to this one, edges were sorted in
    // initial pass so only need to check the next one.
    if (((i + 2) >= indicesPerMesh) ||
        silhouetteIndices[i + 2] !=
            currentEdgeEnd) {  // reached end of strip. Insert final edge.
      // Check if last edge wraps around to start.
      if (currentEdgeEnd ==
          stripStartIndex) {  // add end of strip that wraps around to start
                              // (forming closed cylinder/shape)
  ...
  //	DEBUG_ASSERTLOG(polygonCount == vertexCount, ("WARNING***Shadow volume
  //mesh not optimal: %s\n",m_geometry->Get_Name()));
}  // end constructVolume
...
Bool W3DVolumetricShadow::allocateShadowVolume(Int volumeIndex, Int meshIndex) {
  ...
  // Only allocate space here for dynamic shadows.  Shadows for
  // static/non-animated models will be stored in vertex buffers which are
  // allocated once exact size is known.
  if (shadowVolume->GetFlags() & SHADOW_DYNAMIC) {
    // for dynamic shadow casters, we need to allocate the maximum amount of
    // vertices that could ever be required.
    //		if (m_shadowVolumeVB[ volumeIndex ][meshIndex])
    //			TheW3DBufferManager->releaseSlot(m_shadowVolumeVB[
    //volumeIndex ][meshIndex]); 		m_shadowVolumeVB[ volumeIndex ][meshIndex] =
    //TheW3DVertexBufferManager->getSlot(W3DVertexBufferManager::VBM_FVF_XYZ,
    //numVertices);
...
void W3DVolumetricShadow::resetShadowVolume(Int volumeIndex, Int meshIndex) {
  ...
  if (geometry) {
    if (m_shadowVolumeVB[volumeIndex][meshIndex]) {
      TheW3DBufferManager->releaseSlot(
          m_shadowVolumeVB[volumeIndex][meshIndex]);
      m_shadowVolumeVB[volumeIndex][meshIndex] = NULL;
    }
    if (m_shadowVolumeIB[volumeIndex][meshIndex]) {
      TheW3DBufferManager->releaseSlot(
          m_shadowVolumeIB[volumeIndex][meshIndex]);
      m_shadowVolumeIB[volumeIndex][meshIndex] = NULL;
    }
    geometry->Release();
  }
...
Bool W3DVolumetricShadow::allocateSilhouette(Int meshIndex, Int numVertices) {
  ...
  assert(m_silhouetteIndex[meshIndex] == NULL &&
         m_numSilhouetteIndices[meshIndex] == 0 && numEntries > 0);
...
void W3DVolumetricShadow::deleteSilhouette(Int meshIndex) {
  if (m_silhouetteIndex[meshIndex]) delete[] m_silhouetteIndex[meshIndex];
...
void W3DVolumetricShadowManager::renderStencilShadows(void) {
  ...
  if (!m_pDev) return;  // need device to render anything.
  ...
  m_pDev->SetRenderState(
      D3DRS_STENCILFUNC,
      D3DCMP_LESSEQUAL);  // reference value is less or equal to stencil
  m_pDev->SetRenderState(D3DRS_STENCILPASS, D3DSTENCILOP_KEEP);
  // Upper bits of stencil could be used for storing occluded models which are
  // player colored.  So we mask out those pixels and only use the lower bits
  // for shadow calculations.
  m_pDev->SetRenderState(D3DRS_STENCILMASK,
...
void W3DVolumetricShadowManager::renderShadows(Bool forceStencilFill) {
  ...
  // Get a bounding box around our visible universe.  Bounded by terrain and the
  // sky so much tighter fitting volume than what's actually visible.  This will
  // cull particles falling under the ground.

  TheTerrainRenderObject->getMaximumVisibleBox(*shadowCameraFrustum, &bbox,
                                               TRUE);
  ...
  if (m_shadowList && TheGlobalData->m_useShadowVolumes) {
    ...
    // According to Nvidia there's a D3D bug that happens if you don't start
    // with a new dynamic VB each frame - so we force a DISCARD by overflowing
    // the counter.
    ...
    DX8Wrapper::Apply_Render_State_Changes();  // force update of view and
                                               // projection matrices
    ...
    if (DX8Caps::Get_Default_Caps().PrimitiveMiscCaps &
        D3DPMISCCAPS_COLORWRITEENABLE) {
      DX8Wrapper::_Get_D3D_Device8()->GetRenderState(D3DRS_COLORWRITEENABLE,
                                                     &oldColorWriteEnable);
      DX8Wrapper::Set_DX8_Render_State(D3DRS_COLORWRITEENABLE, 0);
    } else {  // device does not support disabling writes to color buffer so
              // fake it through alpha blending
    ...
    m_pDev->SetRenderState(D3DRS_STENCILENABLE, TRUE);
#endif
    // Any pixels with stencil already set to 128 contains a potential occluder.
    // If this pixels also has any of the player color stencil bits also set, it
    // means that it's an occluded player color and we need to NOT render
    // shadows here.  We do this determination by comparing the value in the
    // combined bits against a value containing only a potential occluder. If
    // the value of just the potential occluder bit is >= than the combined
    // bits, then we know none of the player color bits were set and it's okay
    // to render shadow.
    if (TheW3DShadowManager->getStencilShadowMask() == 0x80808080)
      m_pDev->SetRenderState(D3DRS_STENCILFUNC,
                             D3DCMP_NOTEQUAL);  // in this mode, MSB indicates
                                                // occluded player pixels.
    else
      m_pDev->SetRenderState(
          D3DRS_STENCILFUNC,
          D3DCMP_GREATEREQUAL);  // in this mode, multiple bits indicate
                                 // occluded player pixels.
    m_pDev->SetRenderState(
        D3DRS_STENCILREF,
        0x80808080);  // isolate MSB, it's used to indicate pixels containing
                      // potential occluders.
    m_pDev->SetRenderState(
        D3DRS_STENCILMASK,
        TheW3DShadowManager
            ->getStencilShadowMask());  // isolate upper bits containing
                                        // PotentialOccluderBit|PlayerColorBits
    ...
    //		m_pDev->SetRenderState(D3DRS_ZBIAS,1);	///@todo: See if this
    ...
    m_dynamicShadowVolumesToRender =
        NULL;  // clear list of pending dynamic shadows
    ...
    for (nextVb = TheW3DBufferManager->getNextVertexBuffer(
             NULL, W3DBufferManager::VBM_FVF_XYZ);
         nextVb != NULL; nextVb = TheW3DBufferManager->getNextVertexBuffer(
                             nextVb, W3DBufferManager::VBM_FVF_XYZ)) {
      nextTask = (W3DVolumetricShadowRenderTask *)nextVb->m_renderTaskList;
      while (nextTask) {
        nextTask->m_parentShadow->RenderVolume(nextTask->m_meshIndex,
                                               nextTask->m_lightIndex);
    ...
    shadowDynamicTask = m_dynamicShadowVolumesToRender;
    while (shadowDynamicTask) {  // dynamic shadow columes don't need to wait in
                                 // queue since they
      // all use the same vertex buffer.  Flush them ASAP.
      shadowDynamicTask->m_parentShadow->RenderVolume(
          shadowDynamicTask->m_meshIndex, shadowDynamicTask->m_lightIndex);
      shadowDynamicTask =
          (W3DVolumetricShadowRenderTask *)shadowDynamicTask->m_nextTask;
    }
    ...
    m_pDev->SetRenderState(D3DRS_CULLMODE, D3DCULL_CW);
    //		m_pDev->SetRenderState(D3DRS_ZBIAS,0);	///@todo: See if this
    //helps or makes things worse.
    ...
    // to the screen
    //
    ///@todo: Put this check back in after water is fixed so it doesn't require
    ///shadow rendering to fix alpha.
  ...
  } else if (forceStencilFill) {  // no shadows to render, but still need to
                                  // fill stencil buffer
    // for other effects.
...
/** This class will manage shadow geometry for each render object.  Shadow
geometry may be the same as render geometry but doesn't need to be.  This allows
lower LOD versions of the geometry to be used in shadow calculations.  Shadow
geometry also keeps extended vertex connectivity information that's not used
during rendering.
*/
...
/*
** An Iterator to get to all loaded W3DShadowGeometries in a
*W3DShadowGeometryManager
*/
class W3DShadowGeometryManagerIterator : public HashTableIteratorClass {
 public:
  W3DShadowGeometryManagerIterator(W3DShadowGeometryManager &manager)
      : HashTableIteratorClass(*manager.GeomPtrTable) {}
...
void W3DVolumetricShadowManager::invalidateCachedLightPositions(void) {
  if (!m_shadowList) return;  // there are no shadows to render.
  ...
  for (shadow = m_shadowList; shadow; shadow = shadow->m_next) {
    for (Int i = 0; i < MAX_SHADOW_LIGHTS; i++) {
      for (Int meshIndex = 0; meshIndex < MAX_SHADOW_CASTER_MESHES;
           meshIndex++) {
...
void W3DVolumetricShadowManager::ReleaseResources(void) {
  if (shadowIndexBufferD3D) shadowIndexBufferD3D->Release();
  if (shadowVertexBufferD3D) shadowVertexBufferD3D->Release();
...
Bool W3DVolumetricShadowManager::ReAcquireResources(void) {
  ...
  DEBUG_ASSERTCRASH(m_pDev, ("Trying to ReAquireResources on "
                             "W3DVolumetricShadowManager without device"));

  if (FAILED(m_pDev->CreateIndexBuffer(SHADOW_INDEX_SIZE * sizeof(WORD),
                                       D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC,
                                       D3DFMT_INDEX16, D3DPOOL_DEFAULT,
                                       &shadowIndexBufferD3D)))
    return FALSE;

  if (shadowVertexBufferD3D == NULL) {  // Create vertex buffer
  ...
  if (TheW3DBufferManager)
    if (!TheW3DBufferManager->ReAcquireResources()) return FALSE;
...
// ============================================================================
Bool W3DVolumetricShadowManager::init(void) { return TRUE; }  // end Init
...
// rendering.
// ============================================================================
W3DVolumetricShadow *W3DVolumetricShadowManager::addShadow(
  ...
  if (sg == NULL) {  // did not find a cached copy of the shadow geometry,
                     // create a new one
    m_W3DShadowGeometryManager->Load_Geom(robj, name);
    // try loading again
    sg = m_W3DShadowGeometryManager->Get_Geom(name);
    if (sg == NULL) return NULL;  // could not create the shadow geometry
  }
  ...
  if (shadowInfo->m_sizeX) {  // need to adjust sun elevation for this model in
                              // order to limit shadow length
...
void W3DVolumetricShadowManager::removeShadow(W3DVolumetricShadow *shadow) {
  ...
  for (next_shadow = m_shadowList; next_shadow;
       prev_shadow = next_shadow, next_shadow = next_shadow->m_next) {
...
/*
** An entry for a table of anims not found, so we can quickly determine their
*loss
*/
...
** Missing Geoms
**
** The idea here, allow the system to register which anims are determined to be
*missing
** so that if they are asked for again, we can quickly return NULL, without
*searching the
** disk again.
*/
...
int W3DShadowGeometryManager::Load_Geom(RenderObjClass *robj,
  ...
  if (res != TRUE) {  // load failed!
    newgeom->Release_Ref();
    // DEBUG_LOG(("****Shadow Volume Creation Failed on %s\n",name));
    goto Error;
  } else if (Peek_Geom(newgeom->Get_Name()) != NULL) {  // duplicate exists!
