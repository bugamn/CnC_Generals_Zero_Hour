...
#define SUN_DISTANCE_FROM_GROUND \
  10000.0f  // distance of sun (our only light source).
...
void DoShadows(RenderInfoClass &rinfo, Bool stencilPass) {
  ...
  if (TheW3DShadowManager &&
      stencilPass)  // reset so no more shadow processing this frame.
    TheW3DShadowManager->queueShadows(FALSE);
}

W3DShadowManager::W3DShadowManager(void) {
  DEBUG_ASSERTCRASH(TheW3DVolumetricShadowManager == NULL &&
                        TheW3DProjectedShadowManager == NULL,
...
Bool W3DShadowManager::init(void) {
  ...
  if (TheW3DVolumetricShadowManager && TheW3DVolumetricShadowManager->init()) {
    if (TheW3DVolumetricShadowManager->ReAcquireResources()) result = TRUE;
  }
  if (TheW3DProjectedShadowManager && TheW3DProjectedShadowManager->init()) {
    if (TheW3DProjectedShadowManager->ReAcquireResources()) result = TRUE;
  }
...
void W3DShadowManager::Reset(void) {
  if (TheW3DVolumetricShadowManager) TheW3DVolumetricShadowManager->reset();
  if (TheW3DProjectedShadowManager) TheW3DProjectedShadowManager->reset();
}
...
Shadow *W3DShadowManager::addShadow(RenderObjClass *robj,
  ...
  if (shadowInfo) type = shadowInfo->m_type;

  switch (type) {
    case SHADOW_VOLUME:
      if (TheW3DVolumetricShadowManager)
        return (Shadow *)TheW3DVolumetricShadowManager->addShadow(
            robj, shadowInfo, draw);
      break;
...
void W3DShadowManager::removeShadow(Shadow *shadow) { shadow->release(); }
...
/**Force update of all shadows even when light source and object have not
 * moved*/
...
void W3DShadowManager::setLightPosition(Int lightIndex, Real x, Real y,
                                        Real z) {
  if (lightIndex != 0) return;  ///@todo: Add support for multiple lights
