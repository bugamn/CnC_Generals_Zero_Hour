...
W3DBufferManager::W3DBufferManager(void) {
  ...
  for (Int i = 0; i < MAX_FVF; i++) m_W3DVertexBuffers[i] = NULL;
  for (i = 0; i < MAX_FVF; i++)
    for (Int j = 0; j < MAX_VB_SIZES; j++) m_W3DVertexBufferSlots[i][j] = NULL;

  m_W3DIndexBuffers = NULL;
  for (Int j = 0; j < MAX_IB_SIZES; j++) m_W3DIndexBufferSlots[j] = NULL;
}
...
void W3DBufferManager::freeAllSlots(void) {
  ...
  DEBUG_ASSERTCRASH(m_numEmptySlotsAllocated == 0,
                    ("Failed to free all empty vertex buffer slots"));
  DEBUG_ASSERTCRASH(m_numEmptyIndexSlotsAllocated == 0,
                    ("Failed to free all empty index buffer slots"));
}
...
void W3DBufferManager::freeAllBuffers(void) {
  ...
  for (i = 0; i < MAX_FVF; i++) {
    ...
    while (vb) {
      DEBUG_ASSERTCRASH(vb->m_usedSlots == NULL,
                        ("Freeing Non-Empty Vertex Buffer"));
  ...
  while (ib) {
    DEBUG_ASSERTCRASH(ib->m_usedSlots == NULL,
                      ("Freeing Non-Empty Index Buffer"));
  ...
  DEBUG_ASSERTCRASH(m_numEmptyVertexBuffersAllocated == 0,
                    ("Failed to free all empty vertex buffers"));
  DEBUG_ASSERTCRASH(m_numEmptyIndexBuffersAllocated == 0,
                    ("Failed to free all empty index buffers"));
}
...
Bool W3DBufferManager::ReAcquireResources(void) {
  for (Int i = 0; i < MAX_FVF; i++) {
    W3DVertexBuffer *vb = m_W3DVertexBuffers[i];
    while (vb) {
      DEBUG_ASSERTCRASH(vb->m_DX8VertexBuffer == NULL,
                        ("ReAcquire of existing vertex buffer"));
      vb->m_DX8VertexBuffer = NEW_REF(
          DX8VertexBufferClass, (FVFTypeIndexList[vb->m_format], vb->m_size,
                                 DX8VertexBufferClass::USAGE_DEFAULT));
      DEBUG_ASSERTCRASH(vb->m_DX8VertexBuffer,
                        ("Failed ReAcquire of vertex buffer"));
      if (!vb->m_DX8VertexBuffer) return FALSE;
      vb = vb->m_nextVB;  // get next vertex buffer of this type
    }
  }
  ...
  W3DIndexBuffer *ib = m_W3DIndexBuffers;
  while (ib) {
    DEBUG_ASSERTCRASH(ib->m_DX8IndexBuffer == NULL,
                      ("ReAcquire of existing index buffer"));
    ib->m_DX8IndexBuffer = NEW_REF(
        DX8IndexBufferClass, (ib->m_size, DX8IndexBufferClass::USAGE_DEFAULT));
    DEBUG_ASSERTCRASH(ib->m_DX8IndexBuffer,
                      ("Failed ReAcquire of index buffer"));
    if (!ib->m_DX8IndexBuffer) return FALSE;
    ib = ib->m_nextIB;  // get next vertex buffer of this type
  }
...
/**Searches through previously allocated vertex buffer slots and returns a
   matching type.  If none found, creates a new slot and adds it to the pool.
   Returns an integer slotId used to reference the VB. Returns -1 in case of
   failure.
*/
W3DBufferManager::W3DVertexBufferSlot *W3DBufferManager::getSlot(
  ...
  DEBUG_ASSERTCRASH(sizeIndex < MAX_VB_SIZES && size,
                    ("Allocating too large vertex buffer slot"));

  if ((vbSlot = m_W3DVertexBufferSlots[fvfType][sizeIndex]) !=
      0) {  // found a previously allocated slot matching required size
    m_W3DVertexBufferSlots[fvfType][sizeIndex] = vbSlot->m_nextSameSize;
    if (vbSlot->m_nextSameSize) vbSlot->m_nextSameSize->m_prevSameSize = NULL;
    return vbSlot;
  } else {  // need to allocate a new slot
...
void W3DBufferManager::releaseSlot(W3DVertexBufferSlot *vbSlot) {
  ...
  vbSlot->m_nextSameSize =
      m_W3DVertexBufferSlots[vbSlot->m_VB->m_format][sizeIndex];
  if (m_W3DVertexBufferSlots[vbSlot->m_VB->m_format][sizeIndex])
    m_W3DVertexBufferSlots[vbSlot->m_VB->m_format][sizeIndex]->m_prevSameSize =
        vbSlot;

  m_W3DVertexBufferSlots[vbSlot->m_VB->m_format][sizeIndex] = vbSlot;
}

/**Reserves space inside existing vertex buffer or allocates a new one to fit
 * the required size.
 */
W3DBufferManager::W3DVertexBufferSlot *W3DBufferManager::allocateSlotStorage(
  ...
  DEBUG_ASSERTCRASH(m_numEmptySlotsAllocated < MAX_NUMBER_SLOTS,
                    ("Nore more VB Slots"));

  pVB = m_W3DVertexBuffers[fvfType];
  while (pVB) {
    if ((pVB->m_size - pVB->m_startFreeIndex) >=
        size) {  // found enough free space in this vertex buffer

      if (m_numEmptySlotsAllocated <
          MAX_NUMBER_SLOTS) {  // we're allowing more slots to be allocated.
        ...
        if (pVB->m_usedSlots) pVB->m_usedSlots->m_prevSameVB = vbSlot;
  ...
  DEBUG_ASSERTCRASH(
      m_numEmptyVertexBuffersAllocated < MAX_VERTEX_BUFFERS_CREATED,
      ("Reached Max Static VB Shadow Geometry"));
  ...
  if (m_numEmptyVertexBuffersAllocated < MAX_VERTEX_BUFFERS_CREATED) {
    ...
    pVB->m_DX8VertexBuffer =
        NEW_REF(DX8VertexBufferClass, (FVFTypeIndexList[fvfType], vbSize,
                                       DX8VertexBufferClass::USAGE_DEFAULT));
...
//******************************** Index Buffer code
//******************************************************
/**Searches through previously allocated index buffer slots and returns a
   matching type.  If none found, creates a new slot and adds it to the pool.
   Returns an integer slotId used to reference the VB. Returns -1 in case of
   failure.
*/
W3DBufferManager::W3DIndexBufferSlot *W3DBufferManager::getSlot(Int size) {
  ...
  DEBUG_ASSERTCRASH(sizeIndex < MAX_IB_SIZES && size,
                    ("Allocating too large index buffer slot"));

  if ((ibSlot = m_W3DIndexBufferSlots[sizeIndex]) !=
      0) {  // found a previously allocated slot matching required size
    m_W3DIndexBufferSlots[sizeIndex] = ibSlot->m_nextSameSize;
    if (ibSlot->m_nextSameSize) ibSlot->m_nextSameSize->m_prevSameSize = NULL;
    return ibSlot;
  } else {  // need to allocate a new slot
...
/**Reserves space inside existing index buffer or allocates a new one to fit the
 * required size.
 */
W3DBufferManager::W3DIndexBufferSlot *W3DBufferManager::allocateSlotStorage(
    Int size) {
  W3DIndexBuffer *pIB;
  W3DIndexBufferSlot *ibSlot;
  //	Int sizeIndex = (size >> MIN_SLOT_SIZE_SHIFT)-1;

  DEBUG_ASSERTCRASH(m_numEmptyIndexSlotsAllocated < MAX_NUMBER_SLOTS,
                    ("Nore more IB Slots"));

  pIB = m_W3DIndexBuffers;
  while (pIB) {
    if ((pIB->m_size - pIB->m_startFreeIndex) >=
        size) {  // found enough free space in this index buffer
      ...
      if (m_numEmptyIndexSlotsAllocated <
        ...
        if (pIB->m_usedSlots) pIB->m_usedSlots->m_prevSameIB = ibSlot;
  ...
  DEBUG_ASSERTCRASH(m_numEmptyIndexBuffersAllocated < MAX_INDEX_BUFFERS_CREATED,
                    ("Reached Max Static IB Shadow Geometry"));

  if (m_numEmptyIndexBuffersAllocated < MAX_INDEX_BUFFERS_CREATED) {
    m_W3DIndexBuffers =
        &m_W3DEmptyIndexBuffers[m_numEmptyIndexBuffersAllocated];
    ...
    pIB->m_DX8IndexBuffer = NEW_REF(
        DX8IndexBufferClass, (ibSize, DX8IndexBufferClass::USAGE_DEFAULT));
    pIB->m_startFreeIndex = size;
    pIB->m_size = ibSize;
    ibSlot = &m_W3DIndexBufferEmptySlots[m_numEmptyIndexSlotsAllocated];
    m_numEmptyIndexSlotsAllocated++;
