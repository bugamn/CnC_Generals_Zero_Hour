...
/*
#define SC_ALPHA_MIRROR ( SHADE_CNST(ShaderClass::PASS_LEQUAL,
ShaderClass::DEPTH_WRITE_DISABLE, ShaderClass::COLOR_WRITE_ENABLE,
...
DETAILALPHA_DISABLE) ) ShaderClass
...
// W3DTreeBuffer::cull
//=============================================================================
/** Culls the trees, marking the visible flag.  If a tree becomes visible, it
sets it's sortKey */
//=============================================================================
void W3DTreeBuffer::cull(CameraClass *camera) {
  ...
  for (curTree = 0; curTree < m_numTrees; curTree++) {
    ...
    // Also calculate sort key if a tree is visible, and the view changed
    // setting m_updateAllKeys to true.
    if (doKey || (visible && m_updateAllKeys)) {
      // The sort key is essentially the distance of location in the direction
      // of the camera look at.
      m_trees[curTree].sortKey =
...
//=============================================================================
/** Culls the trees, marking the visible flag for the mirror view. Unlike
cull(), doesn't update anything except the visible flag. */
//=============================================================================
...
// W3DTreeBuffer::sort
//=============================================================================
/** Sorts the trees.  Does num_iterations of a bubble sort.  This is good
because it ends immediately if the trees are already sorted (which is most of
the time) and will perform a fixed amount of work each frame until it becomes
sorted. */
//=============================================================================
...
//=============================================================================
Int W3DTreeBuffer::doLighting(
    Vector3 *loc, Real r, Real g, Real b, SphereClass &bounds,
    RefRenderObjListIterator *pDynamicLightsIterator) {
  if (pDynamicLightsIterator == NULL) {
    return (REAL_TO_INT(b) | (REAL_TO_INT(g) << 8) | (REAL_TO_INT(r) << 16) |
            (255 << 24));
  }
  ...
  for (pDynamicLightsIterator->First(); !pDynamicLightsIterator->Is_Done();
    ...
    if (CollisionMath::Overlap_Test(bounds, pLight->Get_Bounding_Sphere()) ==
  ...
  shadeG *= 255.0f;
  shadeB *= 255.0f;
  return (REAL_TO_INT(shadeB) | (REAL_TO_INT(shadeG) << 8) |
          (REAL_TO_INT(shadeR) << 16) | (255 << 24));
}
...
//=============================================================================
void W3DTreeBuffer::loadTreesInVertexAndIndexBuffers(
    RefRenderObjListIterator *pDynamicLightsIterator) {
  ...
  for (curTree = 0; curTree < m_numTrees; curTree++) {
    ...
    Int diffuse = 0;
    if (!doVertexLighting) {
      diffuse = doLighting(&loc, shadeR, shadeG, shadeB,
                           m_trees[curTree].bounds, NULL);
    }
    ...
    const Vector2 *uvs =
        m_typeMesh[type]->Get_Model()->Get_UV_Array_By_Index(0);

    // If we are doing reduced resolution terrain, do reduced
    // poly trees.
    Bool doPanel =
        (TheGlobalData->m_useHalfHeightMap || TheGlobalData->m_stretchTerrain);
    ...
    if (doPanel) {
      ...
      for (j = 0; j < 6; j++) {
        ...
        if (m_curNumTreeVertices >= MAX_TREE_VERTEX) break;

        // Update the uv values.  The W3D models each have their own texture,
        // and we use one texture with all images in one, so we have to change
        // the uvs to match.
        Real U, V;
        if (type == SHRUB) {
        ...
        vLoc.Y += loc.Y;
        vLoc.Z =
            loc.Z + pVert[i].Y * scale;  // In W3D z is up, in 3dMax y is up.
        ...
        if (doVertexLighting) {
          curVb->diffuse =
              doLighting(&vLoc, shadeR, shadeG, shadeB, m_trees[curTree].bounds,
      ...
      for (i = 0; i < 6; i++) {
        if (m_curNumTreeIndices + 4 > MAX_TREE_INDEX) break;
        curIb--;
        *curIb = startVertex + i;
        m_curNumTreeIndices++;
      }
...
W3DTreeBuffer::W3DTreeBuffer(void) {
  ...
  for (i = 0; i < MAX_TYPES; i++) {
    switch (i) {
      case 0:
        m_typeMesh[i] =
            (MeshClass *)WW3DAssetManager::Get_Instance()->Create_Render_Obj(
                "ALPINETREE.TREE 1");
        break;
      case 1:
        m_typeMesh[i] =
  ...
  if (m_typeMesh[0] == NULL) {
    // DEBUG_LOG("!!!!!!!!!!!!*************** W3DTreeBuffer failed to
    // initialize.\n");
...
void W3DTreeBuffer::allocateTreeBuffers(void) {
  m_vertexTree =
      NEW_REF(DX8VertexBufferClass, (DX8_FVF_XYZDUV1, MAX_TREE_VERTEX + 4,
                                     DX8VertexBufferClass::USAGE_DYNAMIC));
  m_indexTree =
      NEW_REF(DX8IndexBufferClass,
              (2 * MAX_TREE_INDEX + 4, DX8IndexBufferClass::USAGE_DYNAMIC));
  m_curNumTreeVertices = 0;
  m_curNumTreeIndices = 0;
}
...
/** Removes all trees. */
//=============================================================================
void W3DTreeBuffer::clearAllTrees(void) { m_numTrees = 0; }
...
//=============================================================================
void W3DTreeBuffer::addTree(Coord3D loc, Real scale, Real angle,
                            AsciiString name, Bool mirrorVisible) {
  ...
  if (treeType == FENCE) {
    ...
    m_trees[m_numTrees].cos =
        WWMath::Sqrt(1.0 - m_trees[m_numTrees].sin * m_trees[m_numTrees].sin);
...
//=============================================================================
void W3DTreeBuffer::drawTrees(
    CameraClass *camera, RefRenderObjListIterator *pDynamicLightsIterator) {
  if (!m_isTerrainPass) {
    return;
  }
  ...
  // Draw all the trees.
  DX8Wrapper::Draw_Triangles(m_curTreeIndexOffset, m_curNumTreeIndices / 3, 0,
                             m_curNumTreeVertices);
}
