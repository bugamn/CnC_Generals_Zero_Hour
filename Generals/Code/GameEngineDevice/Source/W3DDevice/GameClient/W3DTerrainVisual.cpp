...
// ///////////////////////////////////////////////////////////////////// W3D
// implementation details for visual aspects of terrain Author: Colin Day, April
// 2001
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void W3DTerrainVisual::init(void) {
  ...
  m_waterRenderObject->init(
      TheGlobalData->m_waterPositionZ, TheGlobalData->m_waterExtentX,
      TheGlobalData->m_waterExtentY, W3DDisplay::m_3DScene,
      (WaterRenderObjClass::WaterType)TheGlobalData
          ->m_waterType);  // create a water plane that's 128x128 units
  m_waterRenderObject->Set_Position(
      Vector3(TheGlobalData->m_waterPositionX, TheGlobalData->m_waterPositionY,
              TheGlobalData->m_waterPositionZ));  // place water in world

#ifdef DO_UNIT_TIMINGS
#pragma MESSAGE("********************* WARNING- Doing UNIT TIMINGS. ")
#else
  if (TheGlobalData->m_waterType ==
      WaterRenderObjClass::
          WATER_TYPE_1_FB_REFLECTION) {  // add water render object to the
                                         // pre-pass scene (to be rendered
                                         // before main scene)
                                         // W3DDisplay::m_prePass3DScene->Add_Render_Object(
                                         // m_waterRenderObject);
  } else {  // add water render object to the post-pass scene (to be rendered
            // after main scene)
  ...
  if (m_waterRenderObject) {
    for (Int i = 0; i < 5; i++) {
      ...
      if (m_currentSkyboxTexNames[i] != m_initialSkyboxTexNames[i]) {
        m_waterRenderObject->replaceSkyboxTexture(m_currentSkyboxTexNames[i],
                                                  m_initialSkyboxTexNames[i]);
        m_currentSkyboxTexNames[i] =
            m_initialSkyboxTexNames[i];  // update current state to new texture
      }
    }
...
void W3DTerrainVisual::update(void) {
  ...
  if (m_waterRenderObject) m_waterRenderObject->update();
...
Bool W3DTerrainVisual::load(AsciiString filename) {
  ...
  if (TerrainVisual::load(filename) == FALSE) return FALSE;  // failed
  ...
  if (m_terrainRenderObject == NULL) return FALSE;
  ...
  while (pMapObj) {
    ...
    if (pMapObj->isLight()) {
      ...
      lightP->Set_Far_Attenuation_Range(d->getReal(TheKey_lightInnerRadius),
                                        d->getReal(TheKey_lightOuterRadius));
...
void W3DTerrainVisual::enableWaterGrid(Bool enable) {
  ...
  if (m_waterRenderObject) m_waterRenderObject->enableWaterGrid(enable);
...
//-------------------------------------------------------------------------------------------------
Bool W3DTerrainVisual::intersectTerrain(Coord3D *rayStart, Coord3D *rayEnd,
                                        Coord3D *result) {
  Bool hit = FALSE;

  // sanity
  if (rayStart == NULL || rayEnd == NULL) return hit;
...
void W3DTerrainVisual::getTerrainColorAt(Real x, Real y, RGBColor *pColor) {
  if (m_terrainHeightMap) m_terrainHeightMap->getTerrainColorAt(x, y, pColor);
...
void W3DTerrainVisual::setWaterGridHeightClamps(const WaterHandle *waterTable,
                                                Real minZ, Real maxZ) {
  if (m_waterRenderObject) m_waterRenderObject->setGridHeightClamps(minZ, maxZ);
...
void W3DTerrainVisual::setWaterTransform(const Matrix3D *transform) {
  if (m_waterRenderObject) m_waterRenderObject->setGridTransform(transform);
...
void W3DTerrainVisual::getWaterTransform(const WaterHandle *waterTable,
                                         Matrix3D *transform) {
  if (m_waterRenderObject) m_waterRenderObject->getGridTransform(transform);
...
void W3DTerrainVisual::changeWaterHeight(Real x, Real y, Real delta) {
  if (m_waterRenderObject) m_waterRenderObject->changeGridHeight(x, y, delta);

}  // end changeWaterHeight

// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void W3DTerrainVisual::addWaterVelocity(Real worldX, Real worldY, Real velocity,
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
Bool W3DTerrainVisual::getWaterGridHeight(Real worldX, Real worldY,
                                          Real *height) {
  Real gridX, gridY;

  if (m_isWaterGridRenderingEnabled && m_waterRenderObject &&
      m_waterRenderObject->worldToGridSpace(
          worldX, worldY, gridX,
          gridY)) {  // point falls within grid, return correct height
    m_waterRenderObject->getGridVertexHeight(REAL_TO_INT(gridX),
                                             REAL_TO_INT(gridY), height);
    return TRUE;
  }
  return FALSE;
}
...
void W3DTerrainVisual::setRawMapHeight(const ICoord2D *gridPos, Int height) {
  if (m_terrainHeightMap) {
    ...
    // if (m_terrainHeightMap->getHeight(x,y) != height) //ML changed to prevent
    // scissoring with roads
...
void W3DTerrainVisual::addFactionBibDrawable(Drawable *factionBuilding,
  ...
  if (m_terrainHeightMap) {
    ...
    Real extraWidth =
        factionBuilding->getTemplate()->getFactoryExtraBibWidth() + extra;
    const GeometryInfo info =
        factionBuilding->getTemplate()->getTemplateGeometryInfo();
    ...
    m_terrainRenderObject->addTerrainBibDrawable(
        corners, factionBuilding->getID(), highlight);
  }
}

// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void W3DTerrainVisual::addFactionBib(Object *factionBuilding, Bool highlight,
                                     Real extra) {
  if (m_terrainHeightMap) {
    const Matrix3D *mtx = factionBuilding->getTransformMatrix();
    Vector3 corners[4];
    ...
    mtx->Transform_Vector(*mtx, corners[2], &corners[2]);
    mtx->Transform_Vector(*mtx, corners[3], &corners[3]);
    m_terrainRenderObject->addTerrainBib(corners, factionBuilding->getID(),
                                         highlight);
  }
}
...
void W3DTerrainVisual::setShoreLineDetail(void) {
  if (m_terrainRenderObject) m_terrainRenderObject->setShoreLineDetail();
}

// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
/// Replace the skybox texture
void W3DTerrainVisual::replaceSkyboxTextures(const AsciiString *oldTexName[5],
                                             const AsciiString *newTexName[5]) {
  if (m_waterRenderObject) {
    for (Int i = 0; i < 5; i++) {
      // check if this texture was never changed before and is still using the
      // default art.
      ...
      if (m_currentSkyboxTexNames[i] != *newTexName[i]) {
        m_waterRenderObject->replaceSkyboxTexture(m_currentSkyboxTexNames[i],
                                                  *newTexName[i]);
        m_currentSkyboxTexNames[i] =
            *newTexName[i];  // update current state to new texture
      }
    }
  }
}
...
void W3DTerrainVisual::xfer(Xfer *xfer) {
  ...
  if (gridEnabled != m_isWaterGridRenderingEnabled) {
    DEBUG_CRASH(
        ("W3DTerrainVisual::xfer - m_isWaterGridRenderingEnabled mismatch\n"));
    throw SC_INVALID_DATA;

  }  // end if

  // xfer grid data if enabled
  if (gridEnabled) xfer->xferSnapshot(m_waterRenderObject);
  ...
  /*
          {
                  ...
                  if( width != getGridWidth() )
                  {
                          ...
                          DEBUG_CRASH(( "W3DTerainVisual::xfer - grid width
     mismatch '%d' should be '%d'\n", width, getGridWidth() )); throw
     SC_INVALID_DATA;
                  ...
                  if( height != getGridHeight() )
                  {
                          ...
                          DEBUG_CRASH(( "W3DTerainVisual::xfer - grid height
     mismatch '%d' should be '%d'\n", height, getGridHeight() )); throw
     SC_INVALID_DATA;
  ...
  */
  ...
  if (version >= 2) {
    ...
    Int len =
        m_terrainHeightMap->getXExtent() * m_terrainHeightMap->getYExtent();
    Int xferLen = len;
