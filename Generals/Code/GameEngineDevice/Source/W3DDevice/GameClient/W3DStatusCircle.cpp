...
#define SC_ALPHA_Z                                                             \
  (SHADE_CNST(                                                                 \
      ...
      ShaderClass::DSTBLEND_ONE_MINUS_SRC_ALPHA, ShaderClass::FOG_DISABLE,     \
      ShaderClass::GRADIENT_MODULATE, ShaderClass::SECONDARY_GRADIENT_DISABLE, \
      ShaderClass::TEXTURING_ENABLE, ShaderClass::DETAILCOLOR_DISABLE,         \
      ShaderClass::DETAILALPHA_DISABLE, ShaderClass::ALPHATEST_DISABLE,        \
      ShaderClass::CULL_MODE_ENABLE, ShaderClass::DETAILCOLOR_DISABLE,         \
      ShaderClass::DETAILALPHA_DISABLE))
...
#define VERTEX_BUFFER_TILE_LENGTH \
  32  // tiles of side length 32 (grid of 33x33 vertices).
...
W3DStatusCircle::~W3DStatusCircle(void) { freeMapResources(); }
...
bool W3DStatusCircle::Cast_Ray(RayCollisionTestClass &raytest) { return false; }

//@todo: MW Handle both of these properly!!
W3DStatusCircle::W3DStatusCircle(const W3DStatusCircle &src) { *this = src; }
...
Int W3DStatusCircle::initData(void) {
  ...
  m_vertexBufferScreen =
      NEW_REF(DX8VertexBufferClass,
              (DX8_FVF_XYZDUV1, 2 * 3, DX8VertexBufferClass::USAGE_DEFAULT));

  // go with a preset material for now.
  m_vertexMaterialClass =
      VertexMaterialClass::Get_Preset(VertexMaterialClass::PRELIT_DIFFUSE);

  m_shaderClass = ShaderClass::ShaderClass(
      SC_ALPHA);  // _PresetOpaque2DShader;//; //_PresetOpaqueShader;
...
void W3DStatusCircle::Render(RenderInfoClass &rinfo) {
  ...
  Int diffuse =
      (0xff << 24) | (clr << 16) | (clr << 8) | clr;  // b g<<8 r<<16 a<<24.
