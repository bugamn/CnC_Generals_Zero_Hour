...
// ////////////////////////////////////////////////////////////////////// W3D
// implementation details for logical terrain Author: Colin Day, April 2001
///////////////////////////////////////////////////////////////////////////////////////////////////
...
/** Update */
//-------------------------------------------------------------------------------------------------
void W3DTerrainLogic::update(void) { TerrainLogic::update(); }  // end update
...
Bool W3DTerrainLogic::loadMap(AsciiString filename, Bool query) {
  ...
  if (terrainHeightMap) {  // copy loaded data
    ...
    // now, get all the boudnaries, and set the current active boundary to
    // boundary 0.
  ...
  // Note - It is very important that this get called AFTER the map is read in.
  // jba. enhancing functionality
  if (TerrainLogic::loadMap(filename, query) == false) return FALSE;
...
void W3DTerrainLogic::getExtentIncludingBorder(Region3D *extent) const {
  ...
  Real border =
      TheTerrainRenderObject->getMap()->getBorderSize() * MAP_XY_FACTOR;
  extent->lo.x -= border;
  extent->lo.y -= border;
...
//-------------------------------------------------------------------------------------------------
Real W3DTerrainLogic::getLayerHeight(Real x, Real y, PathfindLayerEnum layer,
  ...
  if (layer != LAYER_GROUND) {
    ...
    if ((pBridge = findBridgeLayerAt(&loc, layer, clip)) != 0) {
      ...
      if (bridgeHeight > height) {
        return bridgeHeight;  // Don't return bridge height if it's in the
                              // ground.
      }
    }
  }
