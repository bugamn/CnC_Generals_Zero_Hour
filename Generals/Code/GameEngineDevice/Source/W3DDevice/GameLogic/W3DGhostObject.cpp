...
// /////////////////////////////////////////////////////
//
// Placeholder for objects that have been deleted but need to be maintained
// because a player can see them fogged.
//
...
class W3DRenderObjectSnapshot : public Snapshot {
  ...
  W3DRenderObjectSnapshot(RenderObjClass *m_parentRobj, DrawableInfo *drawInfo,
                          Bool cloneParentRobj = TRUE);
  ~W3DRenderObjectSnapshot() { REF_PTR_RELEASE(m_robj); }
  inline void update(
      RenderObjClass *robj, DrawableInfo *drawInfo,
      Bool cloneParentRobj =
          TRUE);  ///< refresh the current snapshot with latest state
 ...
 protected:
  ...
  const char
      *m_robjName;  ///< debug pointer so we know what this is a snapshot of.
#endif
  RenderObjClass
      *m_robj;  ///< render object representing state at time of snapshot
  W3DRenderObjectSnapshot
      *m_next;  ///< snapshot of next render object belonging to same drawable
};
...
void disableUVAnimations(RenderObjClass *robj) {
  if (robj && robj->Class_ID() == RenderObjClass::CLASSID_HLOD) {
    // Also disable any animations that may be playing using mappers (texture
    // scrolling)
    for (Int i = 0; i < robj->Get_Num_Sub_Objects(); i++) {
      RenderObjClass *subObj = robj->Get_Sub_Object(i);
      if (subObj &&
          subObj->Class_ID() ==
              RenderObjClass::CLASSID_MESH) {  // check if sub-object has the
                                               // correct material to do texture
                                               // scrolling.
          ...
          for (Int j = 0; j < mat->Vertex_Material_Count(); j++) {
            ...
            if (mapper && mapper->Mapper_ID() ==
              ...
              subObj->Set_User_Data(
                  &animationDisableOverride);  // tell W3D about custom material
                                               // settings
            }
          }
          REF_PTR_RELEASE(mat);
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void W3DRenderObjectSnapshot::update(RenderObjClass *robj,
  ...
  if (cloneParentRobj == TRUE) {
    ...
    if (robj->Class_ID() == RenderObjClass::CLASSID_HLOD) {
      ...
      HAnimClass *hanim =
          ((HLodClass *)robj)
              ->Peek_Animation_And_Info(frame, numFrames, mode, mult);
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
W3DRenderObjectSnapshot::W3DRenderObjectSnapshot(RenderObjClass *robj,
                                                 DrawableInfo *drawInfo,
                                                 Bool cloneParentRobj) {
  m_robj = NULL;
  m_next = NULL;
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void W3DRenderObjectSnapshot::crc(Xfer *xfer) {}  // end crc
...
void W3DRenderObjectSnapshot::xfer(Xfer *xfer) {
  ...
  // sanity
  DEBUG_ASSERTCRASH(m_robj,
                    ("W3DRenderObjectSnapshot::xfer - invalid m_robj\n"));

  // transform on the main render object
  Matrix3D transform;
  transform = m_robj->Get_Transform();
  xfer->xferUser(&transform, sizeof(Matrix3D));
  if (xfer->getXferMode() == XFER_LOAD) m_robj->Set_Transform(transform);
  ...
  for (Int i = 0; i < subObjectCount; ++i) {
    ...
    // it is present.  It is possible that in future patches we change the
    // artwork for some objects which could remove sub objects for which we have
    // data saved for in the save game file.  If we encounter data in the save
    // file for sub objects that are no longer in the artwork we just read the
    // data and throw it away
    //

    // visible/hidden status of this sub object
    if (subObject) visible = subObject->Is_Not_Hidden_At_All();
    xfer->xferBool(&visible);
    if (subObject && xfer->getXferMode() == XFER_LOAD)
      subObject->Set_Hidden(!visible);

    // transform of this sub object
    if (subObject) transform = subObject->Get_Transform();
    xfer->xferUser(&transform, sizeof(Matrix3D));
    if (subObject && xfer->getXferMode() == XFER_LOAD)
    ...
    if (subObject) REF_PTR_RELEASE(subObject);

  }  // end for, i

  // tell W3D that the transforms for our sub objects are all OK cause we've
  // done them ourselves
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void W3DRenderObjectSnapshot::loadPostProcess(void) {}  // end loadPostProcess

// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
W3DGhostObject::W3DGhostObject() {
  for (Int i = 0; i < MAX_PLAYER_COUNT; i++) m_parentSnapshots[i] = NULL;
...
W3DGhostObject::~W3DGhostObject() {
  ...
  for (Int i = 0; i < MAX_PLAYER_COUNT; i++) {
    DEBUG_ASSERTCRASH(m_parentSnapshots[i] == NULL,
                      ("Delete of non-empty GhostObject"));
  }
#else
  DEBUG_ASSERTCRASH(
...
void W3DGhostObject::snapShot(int playerIndex) {
  ...
  if (playerIndex != TheGhostObjectManager->getLocalPlayerIndex())
    return;  // we only snapshot things for the initial local player because
             // local player can't change in non-debug game.
#endif
  ...
  if (snap !=
      m_parentSnapshots[playerIndex]) {  // save off other info we may need in
                                         // case the parent object is destroyed.
    ...
    m_parentGeometryMajorRadius =
        m_parentObject->getGeometryInfo().getMajorRadius();
    m_parentGeometryminorRadius =
        m_parentObject->getGeometryInfo().getMinorRadius();
...
void W3DGhostObject::removeParentObject(void) {
  ...
  for (DrawModule **dm = draw->getDrawModules(); *dm; ++dm) {
    ...
    if (di) {
      ...
      robj = w3dDraw->getRenderObject();
      if (robj) {
        DEBUG_ASSERTCRASH(robj->Peek_Scene() != NULL,
                          ("Removing GhostObject parent not in scene "));
        robj->Remove();
      }
    }
  }
...
void W3DGhostObject::restoreParentObject(void) {
  ...
  for (DrawModule **dm = draw->getDrawModules(); *dm; ++dm) {
    ...
    if (di) {
      ...
      // robj may be null for modules which have no render objects such
      // as for build-ups that are currently disabled.
      if (robj) {  // if we have a render object that's not in the scene, it
                   // must have been
        // removed by the ghost object manager, so restore it.  If we have a
        // render object that is in the scene, then it was probably added
        // because the model changed while the object was ghosted (for damage
        // states, garrison, etc.).
...
#endif
    if (m_parentSnapshots[playerIndex]) {  // if we have a snapshot for this
                                           // object, remove it from
      // scene.
      removeFromScene(playerIndex);

      // Restore actual objects assuming they are still alive.
      if (m_parentObject) restoreParentObject();
...
// ------------------------------------------------------------------------------------------------
/**Add the dummy render objects to scene so player sees the correct version
 * within the fog*/
// ------------------------------------------------------------------------------------------------
void W3DGhostObject::addToScene(int playerIndex) {
  ...
  while (snap) {
    ((SimpleSceneClass *)W3DDisplay::m_3DScene)
        ->Add_Render_Object(snap->m_robj);
    snap = snap->m_next;
  }
}
...
void W3DGhostObject::xfer(Xfer *xfer) {
  ...
  DrawableID drawableID = m_drawableInfo.m_drawable
                              ? m_drawableInfo.m_drawable->getID()
                              : INVALID_DRAWABLE_ID;
  ...
  if (xfer->getXferMode() == XFER_LOAD) {
    ...
    if (drawableID != INVALID_DRAWABLE_ID && m_drawableInfo.m_drawable == NULL)
      DEBUG_CRASH((
          "W3DGhostObject::xfer - Unable to find drawable for ghost object\n"));

  }  // end if

  //
  // no need to mess with this "circular" back into itself pointer to the ghost
  // object because it is already valid and assigned upon creation of this ghost
  // object
  //
  ...
  for (Int i = 0; i < MAX_PLAYER_COUNT; ++i) {
    ...
    //
    // sanity, this catches when we read from the file a count of zero, but our
    // data structure already has something allocated in this snapshot index
    //
    if (snapshotCount == 0 && m_parentSnapshots[i] != NULL) {
      DEBUG_CRASH(
          ("W3DGhostObject::xfer - m_parentShapshots[ %d ] has data present "
           "but the count from the xfer stream is empty\n"));
    ...
    if (xfer->getXferMode() == XFER_SAVE) {
      ...
      for (UnsignedByte j = 0; j < snapshotCount; ++j) {
        ...
        renderObject = W3DDisplay::m_assetManager->Create_Render_Obj(
            name.str(), scale, color);
        disableUVAnimations(renderObject);

        // we're loading, allocate new snapshot
        objectSnapshot =
            NEW W3DRenderObjectSnapshot(renderObject, &m_drawableInfo, FALSE);
  ...
  //
  // since there is a snapshot for this object, there cannot be a regular
  // object/drawable in the world, we need to remove it
  //
  if (m_parentObject &&
      m_parentSnapshots[ThePlayerList->getLocalPlayer()->getPlayerIndex()] !=
          NULL &&
  ...
  for (playerIndex = 0; playerIndex < MAX_PLAYER_COUNT; ++playerIndex)
    if (m_parentSnapshots[playerIndex]) shroudednessCount++;
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (playerIndex = 0; playerIndex < MAX_PLAYER_COUNT; ++playerIndex) {
      ...
      if (m_parentSnapshots[playerIndex]) {
        ...
        // write shroudedness
        //				status =
        //m_partitionData->getShroudedStatus( playerIndex ); 				xfer->xferUser(
        //&status, sizeof( ObjectShroudStatus ) );
    ...
    for (i = 0; i < shroudednessCount; ++i) {
      ...
      //			xfer->xferUser( &status, sizeof(
      //ObjectShroudStatus ) ); 			m_partitionData->friend_setShroudedness(
      //playerIndex, status );
...
void W3DGhostObjectManager::reset(void) {
  ...
  // Remove any orphaned modules that were not deleted with their parent object
  // because player had fogged memory of them.
  while (mod) {
    nextmod = mod->m_nextSystem;
  ...
  DEBUG_ASSERTCRASH(m_usedModules == NULL,
                    ("Reset of Non-Empty GhostObjectManager"));
...
GhostObject *W3DGhostObjectManager::addGhostObject(Object *object,
                                                   PartitionData *pd) {
  // we disabled adding new ghost objects - used during map border resizing and
  // loading
  if (m_lockGhostObjects || m_saveLockGhostObjects) return NULL;
  ...
  mod->m_drawableInfo.m_drawable =
      NULL;  // these dummy render objects don't have drawables.
...
void W3DGhostObjectManager::setLocalPlayerIndex(int index) {
  ...
  // belonging to another player from the map.  We then insert the current local
  // player's ghost objects into the map.

  W3DGhostObject *mod = m_usedModules;

  while (mod) {
    mod->removeFromScene(m_localPlayer);
    if (mod->m_parentSnapshots[index]) {  // new player has his own snapshot
      if (!mod->m_parentSnapshots[m_localPlayer] &&
          mod->m_parentObject) {  // previous player didn't have a snapshot so
                                  // real object must
...
// ------------------------------------------------------------------------------------------------
/** When a game object/drawable dies, it is removed from the rest of the engine.
It leaves behind a GhostObject in case any players didn't see the death and have
a fogged view of the pre-death object. We need to manually determine if these
orphaned GhostObjects ever become visible and are no longer needed*/
// ------------------------------------------------------------------------------------------------
  ...
  while (mod) {
    ...
    if (!mod->m_parentObject) {
      ...
      for (int i = 0; i < numNonLocalPlayers; i++, playerIndexList++) {
        ...
        if (mod->m_parentSnapshots[*playerIndexList]) numStoredSnapshots++;
      }
#endif
      mod->getShroudStatus(m_localPlayer);
      if (mod->m_parentSnapshots[m_localPlayer]) numStoredSnapshots++;
...
// ------------------------------------------------------------------------------------------------
/*When a map border changes (via script) we reset the partition manager.  Since
ghost objects are stored inside the partition manager, we need to save and
restore them.  This function will save enough data to restore the state of the
partition manager.*/
// ------------------------------------------------------------------------------------------------
void W3DGhostObjectManager::releasePartitionData(void) {
  ...
  while (mod) {
    ...
    if (!mod->m_parentObject) {
      ThePartitionManager->unRegisterGhostObject(mod);
      mod->m_partitionData = NULL;
    } else {  // The parent object will handle unregistering so just tell to
              // break the
      // ghost object link.
...
void W3DGhostObjectManager::restorePartitionData(void) {
  ...
  while (mod) {
    ...
    if (mod->m_parentObject) {
      ...
      mod->m_parentObject->friend_getPartitionData()->friend_setGhostObject(
          mod);
    ...
    for (Int i = 0; i < MAX_PLAYER_COUNT; i++) {
      if (mod->m_parentSnapshots[i])
        mod->m_partitionData->friend_setShroudednessPrevious(
            i, OBJECTSHROUD_FOGGED);
    }
...
void W3DGhostObjectManager::xfer(Xfer *xfer) {
  ...
  W3DGhostObject *w3dGhostObject;
  for (w3dGhostObject = m_usedModules; w3dGhostObject;
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    DEBUG_ASSERTCRASH(m_usedModules == NULL,
                      ("W3DGhostObjectManager::xfer - The used module list is "
                       "not NULL upon load, but should be!\n"));

    // now it's time to unlock the ghost objects for loading
    DEBUG_ASSERTCRASH(m_saveLockGhostObjects == TRUE,
                      ("W3DGhostObjectManager::xfer - Ghost object manager is "
                       "not save locked, but should be\n"));
    ...
    for (UnsignedShort i = 0; i < count; ++i) {
      ...
      if (object) {
        ...
        DEBUG_ASSERTCRASH(ghostObject != NULL,
                          ("W3DGhostObjectManager::xfer - Could not create "
                           "ghost object for object '%s'\n",
                           object->getTemplate()->getName().str()));

        // link the ghost object and logical object togehter through
        // partition/ghostObject dat
        DEBUG_ASSERTCRASH(
            object->friend_getPartitionData()->getGhostObject() == NULL,
            ("W3DGhostObjectManager::xfer - Ghost object already on object "
             "'%s'\n",
             object->getTemplate()->getName().str()));
        ...
        // register ghost object object with partition system and fill out
        // partition data
