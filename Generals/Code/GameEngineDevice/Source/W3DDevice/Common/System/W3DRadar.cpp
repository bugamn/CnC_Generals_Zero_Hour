...
// Desc:   W3D radar implementation, this has the necessary device dependent
// drawing
...
//-------------------------------------------------------------------------------------------------
/** Find the texture format we're going to use for the radar.  The texture
 * format must be supported by the hardware.  The "more preferred" formats
 * appear at the top of the format tables in order from most preferred to least
 * preferred */
//-------------------------------------------------------------------------------------------------
void W3DRadar::initializeTextureFormats(void) {
  const WW3DFormat terrainFormats[] = {
      WW3D_FORMAT_R8G8B8, WW3D_FORMAT_X8R8G8B8, WW3D_FORMAT_R5G6B5,
      WW3D_FORMAT_X1R5G5B5,
      WW3D_FORMAT_UNKNOWN  // keep this one last
  };
  const WW3DFormat overlayFormats[] = {
      WW3D_FORMAT_A8R8G8B8, WW3D_FORMAT_A4R4G4B4,
      WW3D_FORMAT_UNKNOWN  // keep this one last
  };
...
void W3DRadar::deleteResources(void) {
  ...
  //
  if (m_terrainTexture) m_terrainTexture->Release_Ref();
  m_terrainTexture = NULL;
  if (m_terrainImage) m_terrainImage->deleteInstance();
  m_terrainImage = NULL;

  //
  // delete overlay resources used
  //
  if (m_overlayTexture) m_overlayTexture->Release_Ref();
  m_overlayTexture = NULL;
  if (m_overlayImage) m_overlayImage->deleteInstance();
  m_overlayImage = NULL;

  //
  // delete shroud resources used
  //
  if (m_shroudTexture) m_shroudTexture->Release_Ref();
  m_shroudTexture = NULL;
  if (m_shroudImage) m_shroudImage->deleteInstance();
  m_shroudImage = NULL;
...
void W3DRadar::reconstructViewBox(void) {
  ...
  // get the 4 points of the view corners in the 3D world at the average Z
  // height in the map
  ...
  //
  // save the camera settings for this view box, we will need to make it again
  // only if some of these change
  //
...
void W3DRadar::radarToPixel(const ICoord2D *radar, ICoord2D *pixel,
  ...
  // sanity
  if (radar == NULL || pixel == NULL) return;

  pixel->x = (radar->x * radarWidth / RADAR_CELL_WIDTH) + radarUpperLeftX;
  // note the "inverted" y here to orient the way our world looks with +x=right
  // and -y=down
  pixel->y =
      ((RADAR_CELL_HEIGHT - 1 - radar->y) * radarHeight / RADAR_CELL_HEIGHT) +
      radarUpperLeftY;

}  // end radarToPixel

//-------------------------------------------------------------------------------------------------
/** Draw a hero icon at a position, given radar box upper left location and
 * dimensions.  */
//-------------------------------------------------------------------------------------------------
void W3DRadar::drawHeroIcon(Int pixelX, Int pixelY, Int width, Int height,
                            const Coord3D *pos) {
  // get the hero icon image
  static const Image *image =
      (Image *)TheMappedImageCollection->findImageByName("HeroReticle");
  if (image != NULL) {
    ...
    // draw the icon
    TheDisplay->drawImage(image, offsetScreen.x, offsetScreen.y,
                          offsetScreen.x + iconWidth,
                          offsetScreen.y + iconHeight);
  }
}

//-------------------------------------------------------------------------------------------------
/** Draw a "box" into the texture passed in that represents the viewable area
 * for the tactical display into the game world */
//-------------------------------------------------------------------------------------------------
void W3DRadar::drawViewBox(Int pixelX, Int pixelY, Int width, Int height) {
  ...
  //
  // setup the clipping region ... note that this clipping region is not over
  // just the radar image area ... it's in the WHOLE window available for the
  // radar
  //
  IRegion2D clipRegion;
  ICoord2D radarWindowSize, radarWindowScreenPos;
  m_radarWindow->winGetSize(&radarWindowSize.x, &radarWindowSize.y);
  m_radarWindow->winGetScreenPosition(&radarWindowScreenPos.x,
                                      &radarWindowScreenPos.y);
  ...
  //
  // using our view box offset array, convert each of those radar cell offset
  // points into screen pixels and draw the box.  The view box array is setup
  // with the first index containing (0,0) (the point we just converted in
  // theory), with cell offsets to each of the other corners in the following
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void W3DRadar::drawSingleBeaconEvent(Int pixelX, Int pixelY, Int width,
  ...
  Real totalAnglesToSpin =
      2.0f * PI;  ///< spin around this many angles going from big to small
  ...
  // get the difference in frame from the current frame to the frame we were
  // created on
  frameDiff = currentFrame - event->createFrame;

  // compute the size of the event marker, it is largest when it starts and
  // smallest at the end
  eventSize = REAL_TO_INT(
      maxEventSize * (1.0f - frameDiff / TIME_FROM_FULL_SIZE_TO_SMALL_SIZE));
  ;

  // we never let the event size get too small
  if (eventSize < minEventSize) eventSize = minEventSize;

  // compute how much "angle" we will add to each point to make it rotate as
  // it's getting small
  addAngle =
      -totalAnglesToSpin * (frameDiff / TIME_FROM_FULL_SIZE_TO_SMALL_SIZE);

  // create a triangle around the event
  angle = 0.0f - addAngle;
  tri[0].x =
      REAL_TO_INT((DOUBLE_TO_REAL(Cos(angle)) * eventSize) + event->radarLoc.x);
  tri[0].y =
      REAL_TO_INT((DOUBLE_TO_REAL(Sin(angle)) * eventSize) + event->radarLoc.y);
  ...
  //
  // make the colors we're going to use, when we're at our smallest size we will
  // start to fade the alpha away to transparent so that at our lifetime frame
  ...
  if (currentFrame > event->fadeFrame) {
    a = REAL_TO_UNSIGNEDBYTE(
        (Real)a * (1.0f - (Real)(currentFrame - event->fadeFrame) /
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void W3DRadar::drawSingleGenericEvent(Int pixelX, Int pixelY, Int width,
                                      Int height, Int index) {
  RadarEvent *event = &(m_event[index]);
  ICoord2D tri[3];
  ICoord2D start, end;
...
void W3DRadar::drawEvents(Int pixelX, Int pixelY, Int width, Int height) {
  ...
  for (i = 0; i < MAX_RADAR_EVENTS; i++) {
    ...
    if (m_event[i].active == TRUE && m_event[i].type != RADAR_EVENT_FAKE) {
      // if we haven't played the sound for this event, do it now that we can
      // see it
      if (m_event[i].soundPlayed == FALSE &&
          m_event[i].type != RADAR_EVENT_BEACON_PULSE) {
...
void W3DRadar::renderObjectList(const RadarObject *listHead,
                                TextureClass *texture, Bool calcHero) {
  // sanity
  if (listHead == NULL || texture == NULL) return;
  ...
  Int playerIndex = 0;
  if (player) playerIndex = player->getPlayerIndex();
  ...
  for (const RadarObject *rObj = listHead; rObj;
    ...
    if (rObj->isTemporarilyHidden()) continue;
    ...
    //
    // objects with a local only unit priority will only appear on the radar if
    // they are controlled by the local player, or if the local player is an
    // observer (cause they are godlike and can see everything)
    //
    ...
    // adjust the alpha for stealth units so they "fade/blink" on the radar for
    // the controller if( obj->getRadarPriority() ==
    // RADAR_PRIORITY_LOCAL_UNIT_ONLY ) ML-- What the heck is this? local-only
    // and neutral-observier-viewed units are stealthy?? Since when? Now it
    // twinkles for any stealthed object, whether locally controlled or
    // neutral-observier-viewed
    if (obj->testStatus(OBJECT_STATUS_STEALTHED)) {
      ...
      if (ThePlayerList->getLocalPlayer()->getRelationship(obj->getTeam()) ==
          ENEMIES)
      ...
      Real alphaScale =
          INT_TO_REAL(TheGameLogic->getFrame() % framesForTransition) /
          (framesForTransition / 2.0f);
      if (alphaScale > 0.0f)
        a = REAL_TO_UNSIGNEDBYTE(((alphaScale - 1.0f) * (255.0f - minAlpha)) +
                                 minAlpha);
      else
...
//-------------------------------------------------------------------------------------------------
/** Shade the color passed in using the height parameter to lighten and darken
 * it.  Colors will be interpolated using the value "height" across the range
 * from loZ to hiZ.  The midZ is the "middle" point, height values above it will
 * be lightened, while lower ones are darkened. */
//-------------------------------------------------------------------------------------------------
void W3DRadar::interpolateColorForHeight(RGBColor *color, Real height, Real hiZ,
                                         Real midZ, Real loZ) {
...
void W3DRadar::init(void) {
  ...
  // poolify
  m_terrainTexture = MSGNEW("TextureClass")
      TextureClass(m_textureWidth, m_textureHeight, m_terrainTextureFormat,
  ...
  m_overlayTexture = MSGNEW("TextureClass")
      TextureClass(m_textureWidth, m_textureHeight, m_overlayTextureFormat,
                   TextureClass::MIP_LEVELS_1);
  DEBUG_ASSERTCRASH(m_overlayTexture,
                    ("W3DRadar: Unable to allocate overlay texture\n"));

  // set filter type for the overlay texture, try it and see if you like it, I
  // don't ;)
  ...
  m_shroudTexture = MSGNEW("TextureClass")
      TextureClass(m_textureWidth, m_textureHeight, m_shroudTextureFormat,
                   TextureClass::MIP_LEVELS_1);
  DEBUG_ASSERTCRASH(m_shroudTexture,
                    ("W3DRadar: Unable to allocate shroud texture\n"));
  ...
  //
  // the terrain image, note the UV coords change it from (0,0) in the upper
  // left to (0,0) in the lower left cause that's how we are initially oriented
  // in the world (positive X to the right and positive Y up)
  //
...
void W3DRadar::newMap(TerrainLogic *terrain) {
  //
  // extending functionality, call the base class ... this will cause a reset of
  // the system which will clear out our textures but not free them
  //
...
void W3DRadar::buildTerrainTexture(TerrainLogic *terrain) {
  ...
  surface = m_terrainTexture->Get_Surface_Level();
  DEBUG_ASSERTCRASH(surface,
                    ("W3DRadar: Can't get surface for terrain texture\n"));
    ...
    for (x = 0; x < m_textureWidth; x++) {
      ...
      if (bridge != NULL) {
        ...
        if (obj) {
          ...
          if (body->getDamageState() != BODY_RUBBLE) workingBridge = TRUE;
      ...
      Real waterZ;
      if (workingBridge == FALSE &&
          terrain->isUnderwater(worldPoint.x, worldPoint.y, &waterZ)) {
        const Int waterSamplesAway =
            1;  // how many "tiles" from the center tile we will sample away
          ...
          if (j >= 0 && j < m_textureHeight) {
            for (i = x - waterSamplesAway; i <= x + waterSamplesAway; i++) {
              if (i >= 0 && i < m_textureWidth) {
                ...
                Real underwaterZ =
                    terrain->getGroundHeight(worldPoint.x, worldPoint.y);
                ...
                if (terrain->isUnderwater(worldPoint.x, worldPoint.y)) {
                  ...
                  interpolateColorForHeight(&color, underwaterZ, waterZ, waterZ,
                                            m_mapExtent.lo.z);
              ...
              if (i >= 0 && i < m_textureWidth) {
                ...
                if (workingBridge) {
                  ...
                  TerrainRoadType *bridgeTemplate =
                      TheTerrainRoads->findBridge(bridgeTName);

                  // sanity
                  DEBUG_ASSERTCRASH(bridgeTemplate,
                                    ("W3DRadar::buildTerrainTexture - Can't "
                                     "find bridge template for '%s'\n",
                                     bridgeTName.str()));
                  ...
                  //
                  // we won't use the height of the terrain at this sample
                  // point, we will instead use the height for the entire bridge
                  //
                  Real bridgeHeight = (bridge->peekBridgeInfo()->fromLeft.z +
                                       bridge->peekBridgeInfo()->fromRight.z +
                                       bridge->peekBridgeInfo()->toLeft.z +
                                       bridge->peekBridgeInfo()->toRight.z) /
                                      4.0f;

                  // interpolate the color, but use the bridge height, not the
                  // terrain height
                  ...
                  TheTerrainVisual->getTerrainColorAt(worldPoint.x,
                                                      worldPoint.y, &color);
      ...
      //
      // draw the pixel for the terrain at this point, note that because of the
      // orientation of our world we draw it with positive y in the "up"
      // direction
      //
      // FYI: I tried making this faster by pulling out all the code inside
      // DrawPixel and locking only once ... but it made absolutely *no*
      // performance difference, the sampling and interpolation algorithm for
      // generating pretty looking terrain and water for the radar is just,
      // well, expensive.
      //
      surface->DrawPixel(x, y,
                         GameMakeColor(color.red * 255, color.green * 255,
                                       color.blue * 255, 255));
...
void W3DRadar::clearShroud() {
  ...
  if (!TheGlobalData->m_shroudOn) return;
#endif

  SurfaceClass *surface = m_shroudTexture->Get_Surface_Level();

  // fill to clear, shroud will make black.  Don't want to make something black
  // that logic can't clear
...
// ------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void W3DRadar::setShroudLevel(Int shroudX, Int shroudY,
  ...
  W3DShroud *shroud =
      TheTerrainRenderObject ? TheTerrainRenderObject->getShroud() : NULL;
  if (!shroud) return;

  SurfaceClass *surface = m_shroudTexture->Get_Surface_Level();
  DEBUG_ASSERTCRASH(surface,
                    ("W3DRadar: Can't get surface for Shroud texture\n"));
  ...
  */
  ...
  if (setting == CELLSHROUD_SHROUDED)
    ...
    alpha = 127;  ///< @todo placeholder to get feedback on logic work while
                  ///< graphic side being decided
  else
    alpha = 0;
 ...
 * NOTE about how drawing works: The radar images are computed at samples across
 * the map and are built into a "square" texture area.  At the time of drawing
 * and computing radar<->world coords we consider the "ratio" of width to height
 * of the map dimensions so that when we draw we preserve the aspect ratio of
 * the map and don't squish it in any direction that would cause the map to be
 * distorted.  Extra blank space is drawn around the radar images to keep the
 * whole radar area covered when the map displayed is "long" or "tall" */
//-------------------------------------------------------------------------------------------------
void W3DRadar::draw(Int pixelX, Int pixelY, Int width, Int height) {
  ...
  Player *player = ThePlayerList->getLocalPlayer();
  if (!player->hasRadar() && !TheRadar->isRadarForced()) return;

  //
  // given a upper left corner at pixelX|Y and a width and height to draw into,
  // figure out where we should start and end the image so that the final drawn
  // image has the same ratio as the map and isn't stretched or distorted
  //
  ICoord2D ul, lr;
  ...
  if (m_mapExtent.width() / width >= m_mapExtent.height() / height) {
    ...
    TheDisplay->drawFillRect(pixelX, pixelY, width, ul.y - pixelY - 1,
                             fillColor);
    TheDisplay->drawFillRect(pixelX, lr.y + 1, width,
                             pixelY + height - lr.y - 1, fillColor);
    TheDisplay->drawLine(pixelX, ul.y, pixelX + width, ul.y, 1, lineColor);
    TheDisplay->drawLine(pixelX, lr.y + 1, pixelX + width, lr.y + 1, 1,
                         lineColor);

  }  // end if
  else {
    // draw vertical bars to the left and right
    TheDisplay->drawFillRect(pixelX, pixelY, ul.x - pixelX - 1, height,
                             fillColor);
    TheDisplay->drawFillRect(lr.x + 1, pixelY, width - (lr.x - pixelX) - 1,
                             height, fillColor);
    TheDisplay->drawLine(ul.x, pixelY, ul.x, pixelY + height, 1, lineColor);
    TheDisplay->drawLine(lr.x + 1, pixelY, lr.x + 1, pixelY + height, 1,
                         lineColor);
  ...
  if (TheTacticalView->getZoom() != m_viewZoom) m_reconstructViewBox = TRUE;
  if (TheTacticalView->getAngle() != m_viewAngle) m_reconstructViewBox = TRUE;

  if (m_reconstructViewBox == TRUE) reconstructViewBox();
