...
/*						 interfaces with the Miles Sound
 * System.                       */
/* Revision History:                                                         */
...
static U32 AILCALLBACK streamingFileOpen(char const *fileName,
                                         U32 *file_handle);
...
static U32 AILCALLBACK streamingFileRead(U32 fileHandle, void *buffer,
                                         U32 bytes);
...
void MilesAudioManager::audioDebugDisplay(DebugDisplayInterface *dd, void *,
  ...
  if (dd) {
    ...
    dd->printf("Memory Usage : %d/%d\n", m_audioCache->getCurrentlyUsedSize(),
               m_audioCache->getMaxSize());
  ...
  if (fp) {
    ...
    fprintf(fp, "Music: %d\n", REAL_TO_INT(m_musicVolume * 100.0f));
    fprintf(fp, "Current 3D Provider: %s    ",
            TheAudio->getProviderName(m_selectedProvider).str());
    fprintf(
        fp, "Current Speaker Type: %s\n",
        TheAudio->translateUnsignedIntToSpeakerType(TheAudio->getSpeakerType())
            .str());
  ...
  if (dd) {
    ...
    for (Int i = 1; i <= maxChannels && i <= channelCount; ++i) {
      ...
      dd->printf("%2d: %-20s - (%s) Volume: %d (2D)\n", i,
                 ...
                 filenameNoSlashes.str(), REAL_TO_INT(volume));
      playingArray[i] = NULL;
    }
  }
  if (fp) {
  ...
  if (dd) {
    ...
    for (Int i = 1; i <= maxChannels && i <= channelCount; ++i) {
      ...
      dd->printf("%2d: %-20s - (%s) Volume: %d, Dist: %s, %s\n", i,
                 playing->m_audioEventRTS->getEventName().str(),
                 filenameNoSlashes.str(), REAL_TO_INT(volume), distStr, str);
      playingArray[i] = NULL;
    }
  }
  if (fp) {
...
void MilesAudioManager::init() {
...
#ifdef INTENSE_DEBUG
  DEBUG_LOG(
      ("Sound has temporarily been disabled in debug builds only. jkmcd\n"));
  // for now, _DEBUG builds only should have no sound. ask jkmcd or srj about
  // this.
  return;
#endif
  ...
  AIL_set_file_callbacks(streamingFileOpen, streamingFileClose,
                         streamingFileSeek, streamingFileRead);
}

//-------------------------------------------------------------------------------------------------
void MilesAudioManager::postProcessLoad() { AudioManager::postProcessLoad(); }
...
void MilesAudioManager::stopAudio(AudioAffect which) {
  ...
  // 2) Stop the sample, (so that when we later unload it, bad stuff doesn't
  // happen) 3) Set the status to stopped, so that when we next process the
  // playing list, we will
  ...
  if (BitTest(which, AudioAffect_Speech | AudioAffect_Music)) {
    for (it = m_playingStreams.begin(); it != m_playingStreams.end(); ++it) {
      playing = *it;
      if (playing) {
        if (playing->m_audioEventRTS->getAudioEventInfo()->m_soundType ==
            AT_Music) {
...
void MilesAudioManager::playAudioEvent(AudioEventRTS *event) {
  ...
  DEBUG_LOG(("MILES (%d) - Processing play request: %d (%s)",
             TheGameLogic->getFrame(), event->getPlayingHandle(),
  ...
  switch (info->m_soundType) {
    ...
    case AT_Streaming: {
      ...
      if (handleToKill) {
        for (it = m_playingStreams.begin(); it != m_playingStreams.end();
          ...
          if (playing->m_audioEventRTS &&
              playing->m_audioEventRTS->getPlayingHandle() == handleToKill) {
            // Release this streaming channel immediately because we are going
            // to play another sound in it's place.
      ...
      // Put this on here, so that the audio event RTS will be cleaned up
      // regardless.
    ...
    case AT_SoundEffect: {
      ...
      if (event->isPositionalAudio()) {
        // Sounds that are non-global are positional 3-D sounds. Deal with them
        // accordingly
#ifdef INTENSIVE_AUDIO_DEBUG
        ...
        if (handleToKill) {
          for (it = m_playing3DSounds.begin(); it != m_playing3DSounds.end();
            ...
            if (playing->m_audioEventRTS &&
                playing->m_audioEventRTS->getPlayingHandle() == handleToKill) {
              // Release this 3D sound channel immediately because we are going
              // to play another sound in it's place.
              releasePlayingAudio(playing);
        ...
        if (!handleToKill || foundSoundToReplace) {
          ...
          if (!sample3D) {
            // If we don't have an available sample, kill the lowest priority
            // assuming we have one that is lower than the sound we are trying
            // to add. One possibility for strangeness is when an interrupt
            // sound that wants to kill a handle to replace it, it's possible
            // that another request already killed it, in which case we need to
            // attempt to find another sound to kill.
        ...
        if (handleToKill) {
          for (it = m_playingSounds.begin(); it != m_playingSounds.end();
            ...
            if (playing->m_audioEventRTS &&
                playing->m_audioEventRTS->getPlayingHandle() == handleToKill) {
              // Release this 2D sound channel immediately because we are going
              // to play another sound in it's place.
              releasePlayingAudio(playing);
  ...
  // If we were able to successfully play audio, then we set it to NULL above.
  // (And it will be freed later. However, if audio is non-NULL at this point,
...
void MilesAudioManager::stopAudioEvent(AudioHandle handle) {
  ...
  if (handle == AHSV_StopTheMusic || handle == AHSV_StopTheMusicFade) {
    ...
    for (it = m_playingStreams.begin(); it != m_playingStreams.end(); ++it) {
      ...
      if (audio->m_audioEventRTS->getAudioEventInfo()->m_soundType ==
          AT_Music) {
...
void MilesAudioManager::releasePlayingAudio(PlayingAudio *release) {
  if (release->m_audioEventRTS->getAudioEventInfo()->m_soundType ==
      AT_SoundEffect) {
...
void MilesAudioManager::stopAllAudioImmediately(void) {
  ...
  for (hit = m_audioForcePlayed.begin(); hit != m_audioForcePlayed.end();
       ++hit) {
    if (*hit) {
...
void MilesAudioManager::freeAllMilesHandles(void) {
  // First, we need to ensure that we don't have any sample handles open. To
  // that end, we must stop all of our currently playing audio.
  stopAllAudioImmediately();
...
HSAMPLE MilesAudioManager::getFirst2DSample(AudioEventRTS *event) {
  ...
  // Find the first sample of lower priority than my augmented priority that is
  // interruptable and take its handle
...
void MilesAudioManager::adjustPlayingVolume(PlayingAudio *audio) {
  Real desiredVolume = audio->m_audioEventRTS->getVolume() *
                       audio->m_audioEventRTS->getVolumeShift();
  Real pan;
  if (audio->m_type == PAT_Sample) {
    AIL_sample_volume_pan(audio->m_sample, NULL, &pan);
    AIL_set_sample_volume_pan(audio->m_sample, m_soundVolume * desiredVolume,
                              pan);

  } else if (audio->m_type == PAT_3DSample) {
    AIL_set_3D_sample_volume(audio->m_3DSample,
                             m_sound3DVolume * desiredVolume);
    ...
    if (audio->m_audioEventRTS->getAudioEventInfo()->m_soundType == AT_Music) {
      AIL_set_stream_volume_pan(audio->m_stream, m_musicVolume * desiredVolume,
                                pan);

    } else {
      AIL_set_stream_volume_pan(audio->m_stream, m_speechVolume * desiredVolume,
                                pan);
    }
  }
}
...
void MilesAudioManager::initFilters(HSAMPLE sample,
  ...
  if (pitchShift == 0.0f) {
    DEBUG_CRASH(
        ("Invalid Pitch shift in sound: '%s'", event->getEventName().str()));
  } else {
    AIL_set_sample_playback_rate(
        sample, REAL_TO_INT(AIL_sample_playback_rate(sample) * pitchShift));
  }
...
void MilesAudioManager::initFilters3D(H3DSAMPLE sample,
  ...
  if (pitchShift == 0.0f) {
    ...
    AIL_set_3D_sample_playback_rate(
        sample, REAL_TO_INT(AIL_3D_sample_playback_rate(sample) * pitchShift));
  }

  // Low pass filter
  if (event->getAudioEventInfo()->m_lowPassFreq > 0 && !isOnScreen(pos)) {
    AIL_set_3D_sample_occlusion(
        sample, 1.0f - event->getAudioEventInfo()->m_lowPassFreq);
  }
}
...
void MilesAudioManager::nextMusicTrack(void) {
  ...
  for (it = m_playingStreams.begin(); it != m_playingStreams.end(); ++it) {
    playing = *it;
    if (playing && playing->m_audioEventRTS->getAudioEventInfo()->m_soundType ==
                       AT_Music) {
  ...
  for (it = m_playingStreams.begin(); it != m_playingStreams.end(); ++it) {
    ...
    if (playing && playing->m_audioEventRTS->getAudioEventInfo()->m_soundType ==
      ...
      if (playing->m_audioEventRTS->getEventName() == trackName) {
        if (INFINITE_LOOP_COUNT - AIL_stream_loop_count(playing->m_stream) >=
            numberOfTimes) {
          return TRUE;
        }
      }
...
AsciiString MilesAudioManager::getMusicTrackName(void) const {
  // First check the requests. If there's one there, then report that as the
...
void MilesAudioManager::openDevice(void) {
  ...
  // AIL_quick_startup should be replaced later with a call to actually pick
  // which device to use, etc
  const AudioSettings *audioSettings = getAudioSettings();
  m_selectedSpeakerType =
      TheAudio->translateSpeakerTypeToUnsignedInt(m_prefSpeaker);

  retval = AIL_quick_startup(
      audioSettings->m_useDigital, audioSettings->m_useMidi,
      audioSettings->m_outputRate, audioSettings->m_outputBits,
      audioSettings->m_outputChannels);

  // Quick handles tells us where to store the various devices. For now, we're
  // only interested in the digital handle.
  ...
  // Now that we're all done, update the cached variables so that everything is
  // in sync.
...
void MilesAudioManager::notifyOfAudioCompletion(UnsignedInt audioCompleted,
  ...
  if (!playing) {
    DEBUG_CRASH(
        ("Audio has completed playing, but we can't seem to find it. - jkmcd"));
    return;
  }

  if (getDisallowSpeech() &&
      playing->m_audioEventRTS->getAudioEventInfo()->m_soundType ==
          AT_Streaming) {
  ...
  if (playing->m_audioEventRTS->getNextPlayPortion() != PP_Done) {
    if (playing->m_type == PAT_Sample) {
      ...
      // If we don't have a file now, then we should drop to the stopped status
      // so that We correctly close this handle.
      if (playing->m_file) {
        return;
      }
      ...
      closeFile(playing->m_file);  // close it so as not to leak it.
      playing->m_file =
          playSample3D(playing->m_audioEventRTS, playing->m_3DSample);
  ...
  playing->m_status =
      PS_Stopped;  // it will be cleaned up on the next frame update
}

//-------------------------------------------------------------------------------------------------
PlayingAudio *MilesAudioManager::findPlayingAudioFrom(
    UnsignedInt audioCompleted, UnsignedInt flags) {
  std::list<PlayingAudio *>::iterator it;
  PlayingAudio *playing;
...
//-------------------------------------------------------------------------------------------------
UnsignedInt MilesAudioManager::getProviderIndex(
    AsciiString providerName) const {
...
Bool MilesAudioManager::doesViolateLimit(AudioEventRTS *event) const {
  ...
  // The exception is when we have requested multiple sounds in the same frame
  // and those requests violate the limit. Because we don't have any "old"
  // sounds to remove in the case of an interrupt, we need to catch it early and
  // prevent the sound from being added if we already reached the limit
...
Bool MilesAudioManager::isObjectPlayingVoice(UnsignedInt objID) const {
  ...
  // 2-D
  for (it = m_playingSounds.begin(); it != m_playingSounds.end(); ++it) {
    if ((*it)->m_audioEventRTS->getObjectID() == objID &&
        (*it)->m_audioEventRTS->getAudioEventInfo()->m_type & ST_VOICE) {
      return true;
    }
  }
...
//-------------------------------------------------------------------------------------------------
AudioEventRTS *MilesAudioManager::findLowestPrioritySound(
    AudioEventRTS *event) {
  AudioPriority priority = event->getAudioEventInfo()->m_priority;
  if (priority == AP_LOWEST) {
    // If the event we pass in is the lowest priority, don't bother checking
    // because there is nothing lower priority than lowest.
    return NULL;
  }
...
Bool MilesAudioManager::isPlayingLowerPriority(AudioEventRTS *event) const {
  // We don't actually want to do anything to this CONST function. Remember,
  // we're just checking to see if there is a lower priority sound.
  AudioPriority priority = event->getAudioEventInfo()->m_priority;
  if (priority == AP_LOWEST) {
    // If the event we pass in is the lowest priority, don't bother checking
    // because there is nothing lower priority than lowest.
...
Bool MilesAudioManager::killLowestPrioritySoundImmediately(
    AudioEventRTS *event) {
  // Actually, we want to kill the LOWEST PRIORITY SOUND, not the first "lower"
  // priority sound we find, because it could easily be
  ...
  if (lowestPriorityEvent) {
    ...
    if (event->isPositionalAudio()) {
      for (it = m_playing3DSounds.begin(); it != m_playing3DSounds.end();
        ...
        if (playing->m_audioEventRTS &&
            playing->m_audioEventRTS == lowestPriorityEvent) {
          // Release this 3D sound channel immediately because we are going to
          // play another sound in it's place.
          releasePlayingAudio(playing);
          m_playing3DSounds.erase(it);
          return TRUE;
        }
...
void MilesAudioManager::adjustVolumeOfPlayingAudio(AsciiString eventName,
  ...
  for (it = m_playingSounds.begin(); it != m_playingSounds.end(); ++it) {
    ...
    if (playing && playing->m_audioEventRTS->getEventName() == eventName) {
      ...
      Real desiredVolume = playing->m_audioEventRTS->getVolume() *
                           playing->m_audioEventRTS->getVolumeShift();
...
void MilesAudioManager::processPlayingList(void) {
  ...
  for (it = m_playing3DSounds.begin(); it != m_playing3DSounds.end();) {
    ...
    if (playing->m_status == PS_Stopped) {
      ...
      const Coord3D *pos =
          getCurrentPositionFromEvent(playing->m_audioEventRTS);
      if (pos) {
        if (playing->m_audioEventRTS->isDead()) {
          ...
          Real volForConsideration =
              getEffectiveVolume(playing->m_audioEventRTS);
          volForConsideration /=
              (m_sound3DVolume > 0.0f ? m_soundVolume : 1.0f);
          Bool playAnyways =
              BitTest(playing->m_audioEventRTS->getAudioEventInfo()->m_type,
                      ST_GLOBAL) ||
              playing->m_audioEventRTS->getAudioEventInfo()->m_priority ==
                  AP_CRITICAL;
          if (volForConsideration < m_audioSettings->m_minVolume &&
              !playAnyways) {
...
void MilesAudioManager::processFadingList(void) {
  ...
  for (it = m_fadingAudio.begin(); it != m_fadingAudio.end(); /* emtpy */) {
    ...
    volume *= (1.0f - 1.0f * playing->m_framesFaded /
                          getAudioSettings()->m_fadeAudioFrames);
...
void MilesAudioManager::setHardwareAccelerated(Bool accel) {
  ...
  if (m_hardwareAccel) {
    for (Int i = 0; i < MAX_HW_PROVIDERS; ++i) {
      UnsignedInt providerNdx = TheAudio->getProviderIndex(
          TheAudio->getAudioSettings()->m_preferred3DProvider[i]);
...
void MilesAudioManager::setDeviceListenerPosition(void) {
  if (m_listener) {
    AIL_set_3D_orientation(m_listener, m_listenerOrientation.x,
                           m_listenerOrientation.y, m_listenerOrientation.z, 0,
                           0, -1);
...
//-------------------------------------------------------------------------------------------------
const Coord3D *MilesAudioManager::getCurrentPositionFromEvent(
    AudioEventRTS *event) {
  if (!event->isPositionalAudio()) {
    return NULL;
  }
...
Bool MilesAudioManager::isOnScreen(const Coord3D *pos) const {
  ...
  // WorldToScreen will return True if the point is onscreen and false if it is
  // offscreen.
...
Bool MilesAudioManager::startNextLoop(PlayingAudio *looping) {
  ...
  if (looping->m_audioEventRTS->hasMoreLoops()) {
    ...
    if (looping->m_audioEventRTS->getDelay() > MSEC_PER_LOGICFRAME_REAL) {
      // fake it out so that this sound appears done, but also so that it will
      // not delete the sound on completion (which would suck)
    ...
    if (looping->m_type == PAT_3DSample) {
      looping->m_file =
          playSample3D(looping->m_audioEventRTS, looping->m_3DSample);
    } else {
    ...
    if (fileBuffer) {
      ...
      if (event->getAudioEventInfo()->m_type & ST_GLOBAL) {
        AIL_set_3D_sample_distances(
            sample3D, TheAudio->getAudioSettings()->m_globalMinRange,
            TheAudio->getAudioSettings()->m_globalMaxRange);
      } else {
        AIL_set_3D_sample_distances(sample3D,
                                    event->getAudioEventInfo()->m_minDistance,
...
void MilesAudioManager::buildProviderList(void) {
  ...
  char *name;
  UnsignedInt index = 0;
  while (index < MAXPROVIDERS &&
         AIL_enumerate_3D_providers(&next, &m_provider3D[index].id, &name)) {
...
void MilesAudioManager::initSamplePools(void) {
  ...
  for (i = 0; i < getAudioSettings()->m_sampleCount3D; ++i) {
    H3DSAMPLE sample =
        AIL_allocate_3D_sample_handle(m_provider3D[m_selectedProvider].id);
...
//-------------------------------------------------------------------------------------------------
void MilesAudioManager::friend_forcePlayAudioEventRTS(
  ...
  // Even though the event type is not Speech, this is used only for mission
  // briefings, so use the speech slider to adjust the volume. Get the volume
  // from the event, and pass 0.5 to play the audio in the middle. (0.0 is full
  // left, 1.0 is full right)
  AIL_quick_set_volume(haud, event.getVolume() * getVolume(AudioAffect_Speech),
                       0.5);
...
void AILCALLBACK set3DSampleCompleted(H3DSAMPLE sample3DCompleted) {
  TheAudio->notifyOfAudioCompletion((UnsignedInt)sample3DCompleted,
                                    PAT_3DSample);
}
...
U32 AILCALLBACK streamingFileOpen(char const *fileName, U32 *file_handle) {
  ...
  if (sizeof(U32) != sizeof(File *)) {
    RELEASE_CRASH(
        ("streamingFileOpen - This function requires work in order to compile "
         "on non 32-bit platforms.\n"));
  }
#endif

  (*file_handle) =
      (U32)TheFileSystem->openFile(fileName, File::READ | File::STREAMING);
...
AudioFileCache::AudioFileCache()
    : m_maxSize(0), m_currentlyUsedSize(0), m_mutexName("AudioFileCacheMutex") {
...
AudioFileCache::~AudioFileCache() {
  {
    ...
    for (it = m_openFiles.begin(); it != m_openFiles.end(); ++it) {
      if (it->second.m_openCount > 0) {
        DEBUG_CRASH(
            ("Sample '%s' is still playing, and we're trying to quit.\n",
             it->second.m_eventInfo->m_audioName.str()));
      }

      releaseOpenAudioFile(&it->second);
      // Don't erase it from the map, cause it makes this whole process way more
      // complicated, and we're about to go away anyways.
    }
  }
...
void *AudioFileCache::openFile(AudioEventRTS *eventToOpenFrom) {
  ...
  if (!file) {
    DEBUG_ASSERTLOG(strToFind.isEmpty(),
  ...
  if (eventToOpenFrom->isPositionalAudio()) {
    if (soundInfo.channels > 1) {
      DEBUG_CRASH(
          ("Requested Positional Play of audio '%s', but it is in stereo.",
           strToFind.str()));
      delete[] buffer;
      return NULL;
    }
  ...
  if (m_currentlyUsedSize > m_maxSize) {
    // We need to free some samples, or we're not going to be able to play this
    // sound.
...
void AudioFileCache::releaseOpenAudioFile(OpenAudioFile *fileToRelease) {
  ...
  if (fileToRelease->m_file) {
    if (fileToRelease->m_compressed) {
      // Files read in via AIL_decompress_ADPCM must be freed with
      // AIL_mem_free_lock.
...
//-------------------------------------------------------------------------------------------------
Bool AudioFileCache::freeEnoughSpaceForSample(
    const OpenAudioFile &sampleThatNeedsSpace) {
  Int spaceRequired = m_currentlyUsedSize - m_maxSize;
  Int runningTotal = 0;

  std::list<AsciiString> filesToClose;
  // First, search for any samples that have ref counts of 0. They are
  // low-hanging fruit, and should be considered immediately.
  OpenFilesHashIt it;
  for (it = m_openFiles.begin(); it != m_openFiles.end(); ++it) {
    if (it->second.m_openCount == 0) {
  ...
  // If we don't have enough space yet, then search through the events who have
  // a count of 1 or more and who are lower priority than this sound. Mical said
  // that at this point, sounds shouldn't care if other sounds are interruptable
  // or not. Kill any files of lower priority necessary to clear our the buffer.
  if (runningTotal < spaceRequired) {
    for (it = m_openFiles.begin(); it != m_openFiles.end(); ++it) {
      if (it->second.m_openCount > 0) {
        if (it->second.m_eventInfo->m_priority <
  ...
  // We weren't able to find enough sounds to truncate. Therefore, this sound is
  // not going to play.
  if (runningTotal < spaceRequired) {
    return FALSE;
  }
...
void MilesAudioManager::dumpAllAssetsUsed() {
  ...
  fprintf(logfile, "\nAudio Asset Report - BEGIN\n");
  {
    ...
    for (it = m_allEventsLoaded.begin(); it != m_allEventsLoaded.end(); ++it) {
      ...
      for (asIt = aei->m_attackSounds.begin();
           asIt != aei->m_attackSounds.end(); ++asIt) {
      ...
      for (asIt = aei->m_decaySounds.begin(); asIt != aei->m_decaySounds.end();
           ++asIt) {
        usedFiles.push_back(*asIt);
      }
