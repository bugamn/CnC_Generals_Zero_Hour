...
ArchiveFile *Win32BIGFileSystem::openArchiveFile(const Char *filename) {
  ...
  DEBUG_LOG(
      ("Win32BIGFileSystem::openArchiveFile - size of archive file is %d "
       "bytes\n",
       archiveFileSize));
  ...
  DEBUG_LOG(
      ("Win32BIGFileSystem::openArchiveFile - %d are contained in archive\n",
       numLittleFiles));
  //	for (Int i = 0; i < 2; ++i) {
  //		t = buffer[i];
  ...
  for (Int i = 0; i < numLittleFiles; ++i) {
    ...
    while ((buffer[filenameIndex] != '\\') && (buffer[filenameIndex] != '/') &&
           (filenameIndex >= 0)) {
      --filenameIndex;
    }
    ...
    //to archive file %s, file number %d\n", debugpath.str(),
...
Bool Win32BIGFileSystem::loadBigFilesFromDirectory(AsciiString dir,
  ...
  TheLocalFileSystem->getFileListInDirectory(dir, AsciiString(""), fileMask,
                                             filenameList, TRUE);
  ...
  while (it != filenameList.end()) {
    ...
    if (archiveFile != NULL) {
      DEBUG_LOG(
          ("Win32BIGFileSystem::loadBigFilesFromDirectory - loading %s into "
           "the directory tree.\n",
           (*it).str()));
      loadIntoDirectoryTree(archiveFile, *it, overwrite);
      m_archiveFileMap[(*it)] = archiveFile;
      DEBUG_LOG(
          ("Win32BIGFileSystem::loadBigFilesFromDirectory - %s inserted into "
           "the archive file map.\n",
           (*it).str()));
      actuallyAdded = TRUE;
    }
