...
void DirectInputMouse::openMouse(void) {
  ...
  if (FAILED(hr)) {
    DEBUG_LOG(
        ("ERROR - openMouse: Unabled to create direct input interface\n"));
    assert(0);
    closeMouse();
    return;
  ...
  hr = m_pMouseDevice->SetCooperativeLevel(
      ApplicationHWnd, DISCL_NONEXCLUSIVE | DISCL_FOREGROUND);
  if (FAILED(hr)) {
  ...
  if (FAILED(hr)) {
    DEBUG_LOG(
        ("WARNING - openMouse: Cann't get capabilities of mouse for button "
         "setup\n"));
    ...
    DEBUG_LOG((
        "OK - Mouse info: Buttons = '%d', Force Feedback = '%s', Axes = '%d'\n",
...
UnsignedByte DirectInputMouse::getMouseEvent(MouseIO *result, Bool flush) {
  ...
  if (m_pMouseDevice) {
    ...
    num = 1;
    m_pMouseDevice->Poll();
    hr = m_pMouseDevice->GetDeviceData(sizeof(DIDEVICEOBJECTDATA), &mdat, &num,
                                       0);
    switch (hr) {
      // ----------------------------------------------------------------------
      case DI_OK: {
      ...
      // ----------------------------------------------------------------------
      default:
        //				DBGPRINTF(("GetMouseEvent: GetDeviceData
        //Error: %X.\r\n", hr ));
        break;
