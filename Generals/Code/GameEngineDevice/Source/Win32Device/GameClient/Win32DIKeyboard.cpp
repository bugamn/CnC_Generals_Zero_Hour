...
void DirectInputKeyboard::openKeyboard(void) {
  ...
  hr = DirectInput8Create(ApplicationHInstance, DIRECTINPUT_VERSION,
  ...
  if (FAILED(hr)) {
    DEBUG_LOG(
        ("ERROR - openKeyboard: Unabled to set data format for keyboard\n"));
    assert(0);
    closeKeyboard();
    return;
  ...
  hr = m_pKeyboardDevice->SetCooperativeLevel(
      ApplicationHWnd, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE);
  if (FAILED(hr)) {
  ...
  if (FAILED(hr)) {
    DEBUG_LOG((
        "ERROR - openKeyboard: Unable to set keyboard buffer size property\n"));
    assert(0);
    closeKeyboard();
    return;
  ...
  if (FAILED(hr)) {
    ...
    // Note - This can happen in windowed mode, and we can re-acquire later.  So
    // don't close the keyboard. jba. closeKeyboard();
    return;
...
void DirectInputKeyboard::getKey(KeyboardIO *key) {
  ...
  if (m_pKeyboardDevice) {
    ...
    if (hr == DI_OK || hr == S_FALSE)
      hr = m_pKeyboardDevice->GetDeviceData(sizeof(DIDEVICEOBJECTDATA), &kbdat,
                                            &num, 0);
    switch (hr) {
      // ----------------------------------------------------------------------
      case DI_OK:
