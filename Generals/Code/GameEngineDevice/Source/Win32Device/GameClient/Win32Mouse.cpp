...
UnsignedByte Win32Mouse::getMouseEvent(MouseIO *result, Bool flush) {
  ...
  if (m_eventBuffer[m_nextGetIndex].msg == 0) return MOUSE_NONE;
  ...
  //
  m_nextGetIndex++;
  if (m_nextGetIndex >= Mouse::NUM_MOUSE_EVENTS) m_nextGetIndex = 0;
...
void Win32Mouse::translateEvent(UnsignedInt eventIndex, MouseIO *result) {
  ...
  switch (msg) {
    ...
    case 0x020A:  // WM_MOUSEWHEEL
    {
      ...
      // translate the screen mouse position to be relative to the application
      // window
      p.x = LOWORD(lParam);
      p.y = HIWORD(lParam);
...
//-------------------------------------------------------------------------------------------------
void Win32Mouse::addWin32Event(UINT msg, WPARAM wParam, LPARAM lParam,
                               DWORD time) {
  //
  ...
  // event will be lost
  //
  if (m_eventBuffer[m_nextFreeIndex].msg != 0) return;
  ...
  // wrap index at max
  m_nextFreeIndex++;
  if (m_nextFreeIndex >= Mouse::NUM_MOUSE_EVENTS) m_nextFreeIndex = 0;
...
/**Preload all the cursors we may need during the game.  This must be done
before the D3D device is created to avoid cursor corruption on buggy ATI Radeon
cards. */
void Win32Mouse::initCursorResources(void) {
  for (Int cursor = FIRST_CURSOR; cursor < NUM_MOUSE_CURSORS; cursor++) {
    for (Int direction = 0; direction < m_cursorInfo[cursor].numDirections;
         direction++) {
      if (!cursorResources[cursor][direction] &&
          !m_cursorInfo[cursor]
               .textureName
               .isEmpty()) {  // this cursor has never been loaded before.
        char resourcePath[256];
        // Check if this is a directional cursor
        if (m_cursorInfo[cursor].numDirections > 1)
          sprintf(resourcePath, "data\\cursors\\%s%d.ANI",
                  m_cursorInfo[cursor].textureName.str(), direction);
        else
          sprintf(resourcePath, "data\\cursors\\%s.ANI",
        ...
        if (TheGlobalData->m_modDir.isNotEmpty()) {
          AsciiString fname;
          if (m_cursorInfo[cursor].numDirections > 1)
            fname.format("%sdata\\cursors\\%s%d.ANI",
                         TheGlobalData->m_modDir.str(),
                         m_cursorInfo[cursor].textureName.str(), direction);
          else
            fname.format("%sdata\\cursors\\%s.ANI",
                         TheGlobalData->m_modDir.str(),
                         m_cursorInfo[cursor].textureName.str());
        ...
        DEBUG_ASSERTCRASH(cursorResources[cursor][direction],
                          ("MissingCursor %s\n", resourcePath));
      }
    }
