...
AsciiString EditParameter::m_selectedLocalizedString =
...
EditParameter::EditParameter(CWnd *pParent /*=NULL*/)
    : CDialog(EditParameter::IDD, pParent), m_int(0), m_real(0) {
...
Int EditParameter::edit(Parameter *pParm, AsciiString unitName) {
  if (pParm->getParameterType() == Parameter::COORD3D) {
    ...
    CColorDialog editColorDlg(
        colorref, CC_ANYCOLOR | CC_FULLOPEN | CC_RGBINIT | CC_SOLIDCOLOR);
    ...
    // Set the name of the unit, because some parameters build information from
    // the unit itself.
  ...
  switch (pParm->getParameterType()) {
    ...
    case Parameter::SCRIPT_SUBROUTINE:
      if (!loadScripts(NULL, true, uiString)) {
        if (!loadScripts(NULL, false, uiString)) {
          ...
          warningText.format("Script '%s' is not a subroutine.",
                             uiString.str());
        }
      }
      break;
    case Parameter::ATTACK_PRIORITY_SET:
      if (!loadAttackPrioritySets(NULL, uiString)) {
        warningText.format("Attack priority set '%s' does not exist.",
                           uiString.str());
      }
      break;
    ...
    case Parameter::COMMAND_BUTTON:
      if (!loadCommandButtons(NULL, uiString)) {
        warningText.format("Command button '%s' does not exist.",
                           uiString.str());
      }
      break;
    ...
    case Parameter::LOCALIZED_TEXT:
      if (loadLocalizedText(NULL, uiString) == AsciiString::TheEmptyString) {
        warningText.format("Localized string '%s' does not exist.",
                           uiString.str());
      }
      break;
    ...
    case Parameter::SPECIAL_POWER:
      if (!loadSpecialPowers(NULL, uiString)) {
        warningText.format("Special Power '%s' does not exist.",
                           uiString.str());
      }
      break;
    ...
    case Parameter::SCIENCE_AVAILABILITY:
      if (!loadScienceAvailabilities(NULL, uiString)) {
        warningText.format("Science availability '%s' does not exist.",
                           uiString.str());
      }
      break;
    ...
    case Parameter::BOUNDARY:
      if (TheTerrainRenderObject->getMap()->getAllBoundaries().size() <=
          pParm->getInt()) {
        warningText.format("Border %s does not exist.",
                           BORDER_COLORS[pParm->getInt() % BORDER_COLORS_SIZE]);
      }
      break;
    ...
    for (pCondition = pOr->getFirstAndCondition(); pCondition;
      ...
      for (i = 0; i < pCondition->getNumParameters(); i++) {
        if (type == pCondition->getParameter(i)->getParameterType()) {
          if (CB_ERR ==
              pCombo->FindStringExact(
                  -1, pCondition->getParameter(i)->getString().str())) {
...
void EditParameter::loadActionParameter(Script *pScr,
                                        Parameter::ParameterType type,
                                        CComboBox *pCombo) {
  ...
  for (pAction = pScr->getAction(); pAction; pAction = pAction->getNext()) {
    ...
    for (i = 0; i < pAction->getNumParameters(); i++) {
      if (type == pAction->getParameter(i)->getParameterType()) {
        if (CB_ERR == pCombo->FindStringExact(
                          -1, pAction->getParameter(i)->getString().str())) {
...
Bool EditParameter::loadAttackSetParameter(Script *pScr, CComboBox *pCombo,
                                           AsciiString match) {
  ScriptAction *pAction;
  Bool found = false;
  for (pAction = pScr->getAction(); pAction; pAction = pAction->getNext()) {
    // Attack priorities are created by SET_ATTACK_PRIORITY_* actions, but
    // referenced by *_APPLY_PRIORITY actions.  So just load from the SET_...
    // ones.
    ...
    Int i;
    for (i = 0; i < pAction->getNumParameters(); i++) {
      if (Parameter::ATTACK_PRIORITY_SET ==
          pAction->getParameter(i)->getParameterType()) {
        if (match == pAction->getParameter(i)->getString()) {
          found = true;
        }
        if (pCombo &&
            CB_ERR == pCombo->FindStringExact(
                          -1, pAction->getParameter(i)->getString().str())) {
          pCombo->AddString(pAction->getParameter(i)->getString().str());
        }
      }
...
Bool EditParameter::loadCreateUnitParameter(Script *pScr, CComboBox *pCombo,
                                            AsciiString match) {
  ScriptAction *pAction;
  Bool found = false;
  for (pAction = pScr->getAction(); pAction; pAction = pAction->getNext()) {
    if (pAction->getActionType() !=
            ScriptAction::CREATE_NAMED_ON_TEAM_AT_WAYPOINT &&
        pAction->getActionType() !=
            ScriptAction::UNIT_SPAWN_NAMED_LOCATION_ORIENTATION) {
      continue;
    }
    Int i;
    for (i = 0; i < pAction->getNumParameters(); i++) {
...
Bool EditParameter::loadCreateObjectListsParameter(
    Script *pScr, CComboBox *pCombo, std::vector<AsciiString> *strings,
    AsciiString match) {
  ScriptAction *pAction;
  Bool found = false;
  for (pAction = pScr->getAction(); pAction; pAction = pAction->getNext()) {
    ...
    if (Parameter::OBJECT_TYPE_LIST ==
        pAction->getParameter(0)->getParameterType()) {
      if (match == pAction->getParameter(0)->getString()) {
        found = true;
      }
      if (pCombo &&
          CB_ERR == pCombo->FindStringExact(
                        -1, pAction->getParameter(0)->getString().str())) {
        pCombo->AddString(pAction->getParameter(0)->getString().str());
      }
      if (strings && std::find(strings->begin(), strings->end(),
                               pAction->getParameter(0)->getString().str()) ==
                         strings->end()) {
...
AsciiString EditParameter::getCreatedUnitTemplateName(AsciiString unitName) {
  ...
  for (int i = 0; i < sidesListP->getNumSides(); i++) {
    ...
    for (pGroup = pSL->getScriptGroup(); pGroup; pGroup = pGroup->getNext()) {
      for (pScr = pSL->getScript(); pScr; pScr = pScr->getNext()) {
        for (pAction = pScr->getAction(); pAction;
...
Bool EditParameter::loadSpecialPowers(CComboBox *pCombo, AsciiString match) {
  ...
  for (int i = 0; i < numPowers; ++i) {
    const SpecialPowerTemplate *pPower =
        TheSpecialPowerStore->getSpecialPowerTemplateByIndex(i);
    if (!pPower) {
      continue;
    }
...
Bool EditParameter::loadAbilities(CComboBox *pCombo, AsciiString match) {
  ...
  MapObject *theUnit;
  for (theUnit = MapObject::getFirstMapObject(); theUnit;
       theUnit = theUnit->getNext()) {
    Bool exists;
    AsciiString objName =
        theUnit->getProperties()->getAsciiString(TheKey_objectName, &exists);
    if (!exists) {
      continue;
    }
    if (objName.isEmpty()) {
  ...
  if (theUnit) {
    ...
    // see if the object was created via script, and therefore wasn't in the
    // mapobject list.
  ...
  const CommandSet *commandSet =
      TheControlBar->findCommandSet(theTemplate->friend_getCommandSetString());
...
//-------------------------------------------------------------------------------------------------
Bool EditParameter::loadRevealNamesParameter(Script *pScr, CComboBox *pCombo,
                                             AsciiString match) {
  ScriptAction *pAction;
  Bool found = false;
  for (pAction = pScr->getAction(); pAction; pAction = pAction->getNext()) {
    if (pAction->getActionType() !=
        ScriptAction::MAP_REVEAL_PERMANENTLY_AT_WAYPOINT) {
      continue;
    }
    ...
    if (Parameter::REVEALNAME == pAction->getParameter(3)->getParameterType()) {
      ...
      if (pCombo &&
          CB_ERR == pCombo->FindStringExact(
                        -1, pAction->getParameter(3)->getString().str())) {
...
Bool EditParameter::loadAudioType(Parameter::ParameterType comboType,
                                  CComboBox *pCombo, AsciiString match) {
  Bool retVal = false;
...
Bool EditParameter::getMovieComment(AsciiString match,
  ...
  for (int i = 0; i < numVideos; ++i) {
    ...
    AsciiString videoName = pVideo->m_internalName;
    if (match == videoName &&
        pVideo->m_commentForWB != AsciiString::TheEmptyString) {
...
Bool EditParameter::loadCommandButtons(CComboBox *pCombo, AsciiString match) {
  ...
  File *fp = TheFileSystem->openFile("Data\\INI\\CommandButton.ini",
                                     File::READ | File::TEXT);
  // sanity
...
Bool EditParameter::loadFontNames(CComboBox *pCombo, AsciiString match) {
  ...
  for (font = TheFontLibrary->firstFont(); font;
    ...
    if (font->bold) string.concat(" [Bold]");
...
Bool EditParameter::loadWaypointPaths(CComboBox *pCombo, AsciiString match) {
  ...
  for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
    ...
    if (pMapObj->isWaypoint()) {
      AsciiString wayName;
      Bool exists;
      wayName = pMapObj->getProperties()->getAsciiString(
...
Bool EditParameter::loadObjectFlags(CComboBox *pCombo, AsciiString match) {
  ...
  for (i = 0; TheObjectFlagsNames[i]; ++i) {
    if (pCombo) pCombo->AddString(TheObjectFlagsNames[i]);
...
Bool EditParameter::loadScripts(CComboBox *pCombo, Bool subr,
                                AsciiString match) {
  if (pCombo) pCombo->ResetContent();
  Int i;
  SidesList *sidesListP = m_sidesListP;
...
Bool EditParameter::loadTeamOrUnit(CComboBox *pCombo, AsciiString match) {
  ...
  for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
    ...
    Bool exists;
    AsciiString objName =
        pMapObj->getProperties()->getAsciiString(TheKey_objectName, &exists);
    if (!exists) continue;
...
Bool EditParameter::loadBridges(CComboBox *pCombo, AsciiString match) {
  ...
  for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
    ...
    if (pMapObj->getFlag(FLAG_BRIDGE_POINT1)) {
      if (pMapObj->getNext() &&
          pMapObj->getNext()->getFlag(FLAG_BRIDGE_POINT2)) {
        if (pCombo) pCombo->AddString(objName.str());
        if (objName == match) didMatch = true;
      }
    }
  ...
  switch (m_parameter->getParameterType()) {
    ...
    case Parameter::KIND_OF_PARAM:
      ...
      for (i = KINDOF_FIRST; i < KINDOF_COUNT; i++) {
        pList->InsertString(-1,
                            KindOfMaskType::getBitNames()[i - KINDOF_FIRST]);
      }
      pList->SetCurSel(m_parameter->getInt());
      break;
    ...
    case Parameter::SOUND:
      ...
      showCombo = true;
      // enable the preview sound button only if we are dealing with a soundfx
      // or speech
    ...
    case Parameter::BOUNDARY: {
      ...
      showCombo = true;
      int numBoundaries =
          TheTerrainRenderObject->getMap()->getAllBoundaries().size();
      for (int i = 0; i < numBoundaries; ++i) {
        pCombo->InsertString(-1,
                             BORDER_COLORS[i % BORDER_COLORS_SIZE].m_colorName);
      }
      break;
    }
    ...
    case Parameter::SURFACES_ALLOWED: {
      ...
      // 0 is invalid for surfaces, so change this to a 3 (which means AIR and
      // GROUND)
    ...
    case Parameter::FACTION_NAME: {
      ...
      for (AsciiStringListIterator it = sideList.begin(); it != sideList.end();
           ++it) {
...
void EditParameter::OnCancel() { CDialog::OnCancel(); }
...
void EditParameter::OnPreviewSound() {
  ...
  if (m_parameter->getParameterType() == Parameter::SOUND ||
    ...
    if (!event.getFilename().isEmpty()) {
      PlaySound(event.getFilename().str(), NULL,
