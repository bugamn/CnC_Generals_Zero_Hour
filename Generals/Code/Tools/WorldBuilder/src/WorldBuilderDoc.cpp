...
static bool secondGreaterThan(const std::pair<AsciiString, Int> &__t1,
                              const std::pair<AsciiString, Int> &__t2) {
  return __t1.second > __t2.second;
}

static void FindIndexNearest(CWorldBuilderDoc *pDoc, const Coord3D *point,
                             CPoint *outNdx, DIRECTION pref);
static Bool IndexInRect(CWorldBuilderDoc *pDoc, const Coord3D *bl,
                        const Coord3D *tl, const Coord3D *br, const Coord3D *tr,
                        CPoint *index);
static Bool AddUniqueAndNeighbors(CWorldBuilderDoc *pDoc, const Coord3D *bl,
                                  const Coord3D *tl, const Coord3D *br,
                                  const Coord3D *tr, CPoint ndx,
...
class CompressedCachedMFCFileOutputStream : public OutputStream {
 ...
 public:
  CompressedCachedMFCFileOutputStream(CFile *pFile)
      : m_file(pFile), m_totalBytes(0) {};
  virtual Int write(const void *pData, Int numBytes) {
    UnsignedByte *tmp = new UnsignedByte[numBytes];
    memcpy(tmp, pData, numBytes);
    CachedChunk c;
  ...
  virtual void flush(void) {
    ...
    CompressionType compressionToUse =
        CompressionManager::getPreferredCompression();
    Dict *worldDict = MapObject::getWorldDict();
    if (worldDict) {
      Bool exists = FALSE;
      compressionToUse =
          (CompressionType)worldDict->getInt(TheKey_compression, &exists);
      if (!exists || compressionToUse > COMPRESSION_MAX ||
          compressionToUse < COMPRESSION_MIN)
        compressionToUse = CompressionManager::getPreferredCompression();
    }

    Int compressedLen = CompressionManager::getMaxCompressedSize(
        m_totalBytes, compressionToUse);
    UnsignedByte *destBuffer = NEW UnsignedByte[compressedLen];
    compressedLen = CompressionManager::compressData(
        compressionToUse, srcBuffer, m_totalBytes, destBuffer, compressedLen);
...
void CWorldBuilderDoc::Serialize(CArchive &ar) {
  ...
  if (ar.IsStoring() && m_heightMap) {
    try {
      ...
      char *msg =
          "WorldHeightMapEdit::WorldHeightMapEdit  height map file write "
          "failed: ";
      AfxMessageBox(msg);
      return;
    }
    ...
    if (theInputStream.open(AsciiString((const char *)pth))) try {
        ...
        try {
          ...
          if (file.isValidFileType()) {  // Backwards compatible files aren't
                                         // valid data chunk files.
            // Read the waypoints.
        ...
        // always assign unique IDs. The things will still live in the correct
        // layers, so this isn't an especially big deal.
        ...
        while (pMapObj) {
          ...
          layerName = pMapObj->getProperties()->getAsciiString(
              TheKey_objectLayer, &exists);
          if (exists) {
          ...
          MapObject *pTemplateObj =
              ObjectOptions::getObjectNamed(pMapObj->getName());
          if (pTemplateObj) {
        ...
        while (pos != NULL) {
          ...
          ASSERT_VALID(pWView);
          pWView->setCenterInView(center.x / MAP_XY_FACTOR,
                                  center.y / MAP_XY_FACTOR);
        }
...
AsciiString ConvertFaction(AsciiString name) {
  ...
  const PlayerTemplate *pt =
      ThePlayerTemplateStore->findPlayerTemplate(NAMEKEY(swapName));
  if (pt) {
    return swapName;
  }
  return AsciiString::TheEmptyString;
...
void CWorldBuilderDoc::validate(void) {
  ...
  for (int side = 0; side < TheSidesList->getNumSides(); side++) {
    ...
    if (!pt) {
      DEBUG_LOG(
          ("Faction %s could not be found in sides list!\n", tmplname.str()));
...
#define FIX_TEAM(key)                                               \
  ...
  for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
    ...
    if (pMapObj->isScorch() || pMapObj->isWaypoint() || pMapObj->isLight() ||
        pMapObj->getFlag(FLAG_ROAD_FLAGS) ||
        pMapObj->getFlag(FLAG_BRIDGE_FLAGS)) {
      continue;
    }

    // at this point, only objects with models and teams should be left to
    // process

    // start by verifying the ThingTemplate for the object.
    // swapDict contains a 'history' of missing model swaps done this load, so
    // all objects with a particular name are replaced with the exact same
    // model.
    ...
    // the following code verifies and fixes the team name, player name, and
    // faction linkages
    Bool exists;
    AsciiString teamName =
        pMapObj->getProperties()->getAsciiString(TheKey_originalOwner, &exists);
    if (exists) {
      ...
      if (teamInfo) {
          ...
          if (!pt) {
            ...
            if (tmplname.startsWith("FactionFundamentalist")) {
              ...
              if (swapName != AsciiString::TheEmptyString) {
                DEBUG_LOG(("Changing Faction from %s to %s\n", tmplname.str(),
                           swapName.str()));
                pSide->getDict()->setAsciiString(TheKey_playerFaction,
                                                 swapName);
              }
            }
          }
        ...
        DEBUG_LOG(
            ("Team %s could not be found in sides list!\n", teamName.str()));
      }
    } else {
...
void CWorldBuilderDoc::OnJumpToGame() {
  try {
    ...
    if (strstr(m_strPathName, TheGlobalData->getPath_UserData().str()) != NULL)
      filename.Format("%sMaps\\%s", TheGlobalData->getPath_UserData().str(),
                      m_strTitle);
    else
      filename.Format("Maps\\%s", m_strTitle);

    /*int retval =*/_spawnl(_P_NOWAIT, "\\projects\\rts\\run\\rtsi.exe",
                            "ignored", "-scriptDebug", "-win", "-file",
                            filename, NULL);
  } catch (...) {
  }
}
...
// if 'bReplace' is FALSE will not change path name (SaveCopyAs)
{
  ...
  if (newName.IsEmpty()) {
    ...
    if (info.browse) {
      if (!AfxGetApp()->DoPromptFileName(
              newName, bReplace ? AFX_IDS_SAVEFILE : AFX_IDS_SAVEFILECOPY,
      ...
      if (CFile::GetStatus(newName, status)) {
        if (!(status.m_attribute & CFile::directory)) {
          CString error =
              "Error: file '" + newName + "' exists, and is not a directory.";
          ::AfxMessageBox(error);
          return FALSE;
        }
      } else {
        ...
        if (status != 0) {
          CString error =
              "Error: could not create directory '" + newName + "'.";
          ::AfxMessageBox(error);
          return FALSE;
        }
      }
  ...
  if (bReplace) SetPathName(newName);
 ...
 *
 */
Bool CWorldBuilderDoc::ParseWaypointDataChunk(DataChunkInput &file,
                                              DataChunkInfo *info,
                                              void *userData) {
...
Bool CWorldBuilderDoc::ParseWaypointData(DataChunkInput &file,
  ...
  for (i = 0; i < m_numWaypointLinks; i++) {
    ...
    // DEBUG_LOG(("Waypoint link from %d to %d\n", m_waypointLinks[i].waypoint1,
    // m_waypointLinks[i].waypoint2));
  }
...
void CWorldBuilderDoc::autoSave(void) {
  ...
  if (m_heightMap) try {
      ...
      CFile theFile(autosave1.str(), CFile::modeCreate | CFile::modeWrite |
...
#ifdef _DEBUG
void CWorldBuilderDoc::AssertValid() const { CDocument::AssertValid(); }

void CWorldBuilderDoc::Dump(CDumpContext &dc) const { CDocument::Dump(dc); }
#endif  //_DEBUG
...
void CWorldBuilderDoc::OnFileResize() {
  ...
  if (htMapEditCopy == NULL) return;
  Coord3D objOffset;
  if (htMapEditCopy->resize(hi.xExtent, hi.yExtent, hi.initialHeight,
                            hi.borderWidth, hi.anchorTop, hi.anchorBottom,
                            hi.anchorLeft, hi.anchorRight,
                            &objOffset)) {  // does all the work.
...
WbView3d *CWorldBuilderDoc::Get3DView() {
  ...
  while (pos != NULL) {
    CView *pView = GetNextView(pos);
    if (pView->IsKindOf(RUNTIME_CLASS(WbView3d))) return (WbView3d *)pView;
  }
...
BOOL CWorldBuilderDoc::OnNewDocument() {
  ...
  TNewHeightInfo hi;
  hi.initialHeight =
      AfxGetApp()->GetProfileInt("GameOptions", "Default Map Height", 16);
  hi.xExtent =
      AfxGetApp()->GetProfileInt("GameOptions", "Default Map X-size", 100);
  hi.yExtent =
      AfxGetApp()->GetProfileInt("GameOptions", "Default Map Y-size", 100);
  hi.borderWidth =
      AfxGetApp()->GetProfileInt("GameOptions", "Default Map Border", 30);
  hi.forResize = false;
  if (!firstTime) {
    CString label;
    ...
    if (IDOK == htDialog.DoModal()) {
      htDialog.GetHeightInfo(&hi);
      AfxGetApp()->WriteProfileInt("GameOptions", "Default Map Height",
                                   hi.initialHeight);
      AfxGetApp()->WriteProfileInt("GameOptions", "Default Map X-size",
                                   hi.xExtent);
      AfxGetApp()->WriteProfileInt("GameOptions", "Default Map Y-size",
                                   hi.yExtent);
      AfxGetApp()->WriteProfileInt("GameOptions", "Default Map Border",
                                   hi.borderWidth);
    } else {
      return (false);
    }
  ...
  firstTime = false;
  m_heightMap = NEW_REF(WorldHeightMapEdit, (hi.xExtent, hi.yExtent,
                                             hi.initialHeight, hi.borderWidth));
  // note - mHeight map has ref count of 1.
...
void CWorldBuilderDoc::updateHeightMap(WorldHeightMap *htMap, Bool partial,
...
Bool CWorldBuilderDoc::getAllIndexesInRect(const Coord3D *bl, const Coord3D *br,
  ...
  Coord3D center = {(bl->x + tr->x) / 2, (bl->y + tr->y) / 2,
                    (bl->z + tr->z) / 2};
...
void CWorldBuilderDoc::OnEditLinkCenters() { m_linkCenters = !m_linkCenters; }
...
void CWorldBuilderDoc::compressWaypointIds(void) {
  ...
  for (i = 0; i < m_numWaypointLinks; i++) {
    ...
    MapObject *pWay2 = getWaypointByID(m_waypointLinks[i].waypoint2);
    DEBUG_ASSERTCRASH(
        pWay1 && pWay1->getWaypointID() == m_waypointLinks[i].waypoint1,
        ("Bad waypoint."));
    DEBUG_ASSERTCRASH(
        pWay2 && pWay2->getWaypointID() == m_waypointLinks[i].waypoint2,
        ("Bad waypoint."));
  }
  int count = 1;
  for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
...
//=============================================================================
/** If any waypoints have changed (m_waypointTableNeedsUpdate) updates the
waypoint table.  The waypoint table is used to locate waypoints by id, without
searching the objects list. (See getWaypointByID()) */
//=============================================================================
      ...
      if (pMapObj->isWaypoint()) {
        ...
        if (id > 0 && id < MAX_WAYPOINTS) {
          if (m_waypointTable[id] != NULL)
            DEBUG_LOG(("Duplicate waypoint id."));
          if (m_waypointTable[id] != NULL) {
...
void CWorldBuilderDoc::addWaypointLink(Int waypointID1, Int waypointID2) {
  ...
  DEBUG_ASSERTCRASH(m_numWaypointLinks < MAX_WAYPOINTS - 1,
                    ("Too many links."));
...
MapObject *CWorldBuilderDoc::getWaypointByID(Int waypointID) {
  updateWaypointTable();
  DEBUG_ASSERTCRASH(waypointID >= 0 && waypointID < MAX_WAYPOINTS,
                    ("Invalid id."));
  if (waypointID > 0 && waypointID < MAX_WAYPOINTS) {
    ...
    DEBUG_ASSERTCRASH(pObj == NULL,
                      ("Waypoint links to an obj that isn't a waypoint."));
  }
  return NULL;
}
...
void CWorldBuilderDoc::updateLWL(MapObject *pWay, MapObject *pSrcWay) {
  while (pWay) {
    Bool exists;
    AsciiString label;
    label = pSrcWay->getProperties()->getAsciiString(TheKey_waypointPathLabel1,
                                                     &exists);
    if (exists) {
      pWay->getProperties()->setAsciiString(TheKey_waypointPathLabel1, label);
    } else if (pWay->getProperties()->known(TheKey_waypointPathLabel1,
                                            Dict::DICT_ASCIISTRING)) {
      pWay->getProperties()->remove(TheKey_waypointPathLabel1);
    }
    label = pSrcWay->getProperties()->getAsciiString(TheKey_waypointPathLabel2,
                                                     &exists);
    if (exists) {
      pWay->getProperties()->setAsciiString(TheKey_waypointPathLabel2, label);
    } else if (pWay->getProperties()->known(TheKey_waypointPathLabel2,
                                            Dict::DICT_ASCIISTRING)) {
      pWay->getProperties()->remove(TheKey_waypointPathLabel2);
    }
    label = pSrcWay->getProperties()->getAsciiString(TheKey_waypointPathLabel3,
                                                     &exists);
    if (exists) {
      pWay->getProperties()->setAsciiString(TheKey_waypointPathLabel3, label);
    } else if (pWay->getProperties()->known(TheKey_waypointPathLabel3,
                                            Dict::DICT_ASCIISTRING)) {
      pWay->getProperties()->remove(TheKey_waypointPathLabel3);
    }

    Bool biDirectional;
    biDirectional = pSrcWay->getProperties()->getBool(
        TheKey_waypointPathBiDirectional, &exists);
    if (exists) {
      pWay->getProperties()->setBool(TheKey_waypointPathBiDirectional,
                                     biDirectional);
    }
    ...
    for (i = 0; i < m_numWaypointLinks; i++) {
      ...
      DEBUG_ASSERTCRASH(waypointID1 >= 0 && waypointID1 < MAX_WAYPOINTS,
                        ("Invalid id."));
      DEBUG_ASSERTCRASH(waypointID2 >= 0 && waypointID2 < MAX_WAYPOINTS,
                        ("Invalid id."));
      if (waypointID1 > 0 && waypointID1 < MAX_WAYPOINTS && waypointID2 > 0 &&
          waypointID2 < MAX_WAYPOINTS) {
...
//=============================================================================
void CWorldBuilderDoc::getWaypointLink(Int ndx, Int *waypointID1,
                                       Int *waypointID2) {
  *waypointID1 = 0;
  *waypointID2 = 0;
...
//=============================================================================
/** Returns true if the two waypoint ID's are linked.  Note that due to edits,
one or both waypoints may have been deleted. */
//=============================================================================
...
void CWorldBuilderDoc::OnViewHome() {
  ...
  // if waypoint "InitialCameraPosition" exists, replace pos with the
...
static void fprintUnit(FILE *theLogFile, Dict *teamDict, NameKeyType keyMinUnit,
                       NameKeyType keyMaxUnit, NameKeyType keyUnitType) {
  Bool exists;
...
void CWorldBuilderDoc::OnDumpDocToText(void) {
  ...
  char *aggroStrings[] = {"Passive", "Normal",    "Guard",
                          "Hunt",    "Agressive", "Sleep"};
  ...
  try {
    ...
    for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
      ...
      TeamsInfo *teamInfo =
          TheSidesList->findTeamInfo(d->getAsciiString(TheKey_originalOwner));
    ...
    for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
      ...
      if (tt) {
        if (tt->getEditorSorting() == ES_STRUCTURE) {
          Dict *d = pMapObj->getProperties();
          TeamsInfo *teamInfo = TheSidesList->findTeamInfo(
              d->getAsciiString(TheKey_originalOwner));
          Dict *teamDict = (teamInfo) ? teamInfo->getDict() : NULL;
          AsciiString objectOwnerName =
              (teamDict) ? teamDict->getAsciiString(TheKey_teamOwner) : noOwner;

          Bool showScript = false;
          AsciiString script =
              d->getAsciiString(TheKey_objectScriptAttachment, &showScript);
          ...
          fprintf(theLogFile, ", @ (%0.0f,%0.0f)", pMapObj->getLocation()->x,
    ...
    for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
      ...
      const ThingTemplate *tt = pMapObj->getThingTemplate();
      if (tt) {
        if (tt->getEditorSorting() == ES_VEHICLE ||
            tt->getEditorSorting() == ES_INFANTRY) {
          Bool exists;
          Dict *d = pMapObj->getProperties();
          TeamsInfo *teamInfo = TheSidesList->findTeamInfo(
              d->getAsciiString(TheKey_originalOwner));
          ...
          fprintf(theLogFile, "  %s", tt->getName().str());
          fprintf(theLogFile, ", %s", objectOwnerName.str());
          fprintf(theLogFile, ", @ %0.0f.%0.0f", pMapObj->getLocation()->x / 10,
                  pMapObj->getLocation()->y / 10);
          fprintf(theLogFile, ", Angle %0.0f", pMapObj->getAngle() * 180 / PI);
          fprintf(theLogFile, ", %d%%", d->getInt(TheKey_objectInitialHealth));
          if (showName) {
            fprintf(theLogFile, ", Name %s", name.str());
          ...
          fprintf(theLogFile, ", Team %s",
                  d->getAsciiString(TheKey_originalOwner).str());
          fprintf(theLogFile, ", %s",
                  d->getBool(TheKey_objectRecruitableAI, &exists)
                      ? "AIRecruitable"
                      : "Not AIRecruitable");
          fprintf(theLogFile, ", %s",
                  d->getBool(TheKey_objectSelectable, &exists)
                      ? "Selectable"
                      : "Not Selectable");
      ...
      for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
        ...
        if (pMapObj->getThingTemplate()) {
          ...
          if (it == mapOfTemplates.end()) {
            mapOfTemplates.insert(
                std::make_pair(pMapObj->getThingTemplate()->getName(), 1));
          } else {
            ++(it->second);
          }
        }
    ...
    for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
      ...
      if (pMapObj->isWaypoint()) {
        fprintf(theLogFile, "  %s, @ %0.0f.%0.0f\n",
                pMapObj->getWaypointName().str(),
                pMapObj->getLocation()->x / 10, pMapObj->getLocation()->y / 10);
      }
    }
    ...
    for (j = 0; j < TheSidesList->getNumSides(); j++) {
      ...
      for (Int i = 0; i < numTeams; i++) {
        ...
        if (ti->getDict()->getAsciiString(TheKey_teamOwner) == name) {
          ...
          AsciiString waypoint =
              ti->getDict()->getAsciiString(TheKey_teamHome, &exists);
          CString pri;
          pri.Format(TEXT("%d"), ti->getDict()->getInt(
                                     TheKey_teamProductionPriority, &exists));
          AsciiString trigger = ti->getDict()->getAsciiString(
              TheKey_teamProductionCondition, &exists);

          fprintf(theLogFile,
                  "TEAM %s home '%s', priority %s, condition '%s',\n",
                  teamName.str(), waypoint.str(), pri, trigger.str());
          fprintf(theLogFile, "  UNITS:");
          fprintUnit(theLogFile, ti->getDict(), TheKey_teamUnitMinCount1,
                     TheKey_teamUnitMaxCount1, TheKey_teamUnitType1);
          fprintUnit(theLogFile, ti->getDict(), TheKey_teamUnitMinCount2,
                     TheKey_teamUnitMaxCount2, TheKey_teamUnitType2);
          fprintUnit(theLogFile, ti->getDict(), TheKey_teamUnitMinCount3,
                     TheKey_teamUnitMaxCount3, TheKey_teamUnitType3);
          fprintUnit(theLogFile, ti->getDict(), TheKey_teamUnitMinCount4,
                     TheKey_teamUnitMaxCount4, TheKey_teamUnitType4);
          fprintUnit(theLogFile, ti->getDict(), TheKey_teamUnitMinCount5,
                     TheKey_teamUnitMaxCount5, TheKey_teamUnitType5);
          fprintUnit(theLogFile, ti->getDict(), TheKey_teamUnitMinCount6,
                     TheKey_teamUnitMaxCount6, TheKey_teamUnitType6);
          fprintUnit(theLogFile, ti->getDict(), TheKey_teamUnitMinCount7,
                     TheKey_teamUnitMaxCount7, TheKey_teamUnitType7);
          fprintf(theLogFile, "\n  SCRIPTS: ");
          AsciiString script =
              ti->getDict()->getAsciiString(TheKey_teamOnCreateScript, &exists);
          if (script.isEmpty()) script = "<none>";
          fprintf(theLogFile, "OnCreate='%s'", script.str());
          script =
              ti->getDict()->getAsciiString(TheKey_teamOnIdleScript, &exists);
          if (script.isEmpty()) script = "<none>";
          fprintf(theLogFile, " OnIdle='%s'", script.str());
          script = ti->getDict()->getAsciiString(TheKey_teamOnDestroyedScript,
                                                 &exists);
          if (script.isEmpty()) script = "<none>";
          fprintf(theLogFile, " OnDestroyed='%s'", script.str());
          script = ti->getDict()->getAsciiString(TheKey_teamEnemySightedScript,
                                                 &exists);
          if (script.isEmpty()) script = "<none>";
          fprintf(theLogFile, " OnEnemySighted='%s'", script.str());
          script =
              ti->getDict()->getAsciiString(TheKey_teamAllClearScript, &exists);
          if (script.isEmpty()) script = "<none>";
...
void FindIndexNearest(CWorldBuilderDoc *pDoc, const Coord3D *point,
                      CPoint *outNdx, DIRECTION pref) {
...
Bool IndexInRect(CWorldBuilderDoc *pDoc, const Coord3D *bl, const Coord3D *tl,
                 const Coord3D *br, const Coord3D *tr, CPoint *index) {
  Coord3D testPoint;
  pDoc->getCoordFromCellIndex(*index, &testPoint);
  return PointInsideRect3D(bl, tl, br, tr, &testPoint);
}

Bool AddUniqueAndNeighbors(CWorldBuilderDoc *pDoc, const Coord3D *bl,
                           const Coord3D *tl, const Coord3D *br,
                           const Coord3D *tr, CPoint ndx,
                           VecHeightMapIndexes *allIndices) {
  if (!allIndices) {
  ...
  if (std::find(allIndices->begin(), allIndices->end(), ndx) !=
      allIndices->end()) {
    return false;
  }

  // we have a winner. This index is both inside the rectangle and not already
  // in the index list.
...
void CWorldBuilderDoc::findBoundaryNear(Coord3D *pt, float okDistance,
