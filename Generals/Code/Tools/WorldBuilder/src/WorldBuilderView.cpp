...
#ifdef _DEBUG
void CWorldBuilderView::AssertValid() const { WbView::AssertValid(); }

void CWorldBuilderView::Dump(CDumpContext &dc) const { WbView::Dump(dc); }
...
void CWorldBuilderView::drawObjectInView(CDC *pDc, MapObject *pMapObj) {
  CPoint lpt;
  docToViewCoords(*pMapObj->getLocation(), &lpt);
  // un-adjuse by scroll offset since we are called from OnPaint, which adjusts
  // by this amt
  lpt.x += mXScrollOffset;
  lpt.y += mYScrollOffset;
...
//=============================================================================
void CWorldBuilderView::drawContours(CDC *pDc, CRgn *pRgn, Int minX, Int maxX,
                                     Int minY, Int maxY) {
  CWorldBuilderDoc *pDoc = WbDoc();
  ASSERT_VALID(pDoc);
  ...
  Int curHeight;
  Bool didWater =
      false;  // We do the water level first, then step through the contours.
  for (curHeight = ContourOptions::getContourOffset(); curHeight < 255;
    ...
    for (i = minX; i < maxX; i++) {
      for (j = minY; j < maxY; j++) {
        ...
        if (lowerLeft == upperRight && lowerLeft == upperLeft &&
            upperLeft == lowerRight) {
          continue;
        }
        // If it is all below or all above, we have no contours.
        if (lowerLeft < curHeight && upperLeft < curHeight &&
            upperRight < curHeight && lowerRight < curHeight) {
          continue;
        }
        if (lowerLeft > curHeight && upperLeft > curHeight &&
            upperRight > curHeight && lowerRight > curHeight) {
          ...
          if (lowerLeft != upperLeft || lowerLeft != upperRight) {
            if (lowerLeft == curHeight) {
              if (isBetween(curHeight, upperRight, upperLeft)) {
                pt1 = lowerLeftPt;
                interpolate(&pt2, curHeight, upperLeftPt, upperLeft,
            ...
            if (upperLeft == curHeight) {
              if (isBetween(curHeight, upperRight, lowerLeft)) {
                pt1 = upperLeftPt;
                interpolate(&pt2, curHeight, upperRightPt, upperRight,
            ...
            if (upperRight == curHeight) {
              if (isBetween(curHeight, lowerLeft, upperLeft)) {
                pt1 = upperRightPt;
                interpolate(&pt2, curHeight, lowerLeftPt, lowerLeft,
                            upperLeftPt, upperLeft);
                drawLine = true;
              }
              break;
            }
            if (isBetween(curHeight, lowerLeft, upperLeft)) {
              interpolate(&pt1, curHeight, lowerLeftPt, lowerLeft, upperLeftPt,
                          upperLeft);
              if (isBetween(curHeight, upperRight, upperLeft)) {
                interpolate(&pt2, curHeight, upperLeftPt, upperLeft,
                            upperRightPt, upperRight);
            ...
            if (isBetween(curHeight, upperRight, upperLeft)) {
              interpolate(&pt1, curHeight, upperLeftPt, upperLeft, upperRightPt,
                          upperRight);
              if (isBetween(curHeight, upperRight, lowerLeft)) {
                interpolate(&pt2, curHeight, upperRightPt, upperRight,
                            lowerLeftPt, lowerLeft);
          ...
          if (lowerLeft != lowerRight || lowerLeft != upperRight) {
            if (lowerLeft == curHeight) {
              if (isBetween(curHeight, upperRight, lowerRight)) {
                pt1 = lowerLeftPt;
                interpolate(&pt2, curHeight, lowerRightPt, lowerRight,
                            upperRightPt, upperRight);
                drawLine = true;
              }
              break;
            ...
            if (isBetween(curHeight, lowerLeft, lowerRight)) {
              interpolate(&pt1, curHeight, lowerLeftPt, lowerLeft, lowerRightPt,
                          lowerRight);
              if (isBetween(curHeight, upperRight, lowerLeft)) {
                interpolate(&pt2, curHeight, lowerLeftPt, lowerLeft,
                            upperRightPt, upperRight);
            ...
            if (isBetween(curHeight, upperRight, lowerRight)) {
              interpolate(&pt1, curHeight, lowerRightPt, lowerRight,
                          upperRightPt, upperRight);
              if (isBetween(curHeight, upperRight, lowerLeft)) {
                interpolate(&pt2, curHeight, upperRightPt, upperRight,
                            lowerLeftPt, lowerLeft);
    ...
    if (doingWater) {
      curHeight =
          ContourOptions::getContourOffset() - ContourOptions::getContourStep();
    }
  }
}
...
//=============================================================================
void CWorldBuilderView::interpolate(CPoint *pt, Int ht, CPoint pt1, Int ht1,
                                    CPoint pt2, Int ht2) {
...
//=============================================================================
void CWorldBuilderView::drawMyTexture(CDC *pDc, CRect *pRect, Int width,
  ...
  /*int val=*/::StretchDIBits(pDc->m_hDC, pRect->left, pRect->top,
                              pRect->Width(), pRect->Height(), 0, 0, width,
                              width, rgbData, &bi, DIB_RGB_COLORS, SRCCOPY);
}
...
void CWorldBuilderView::adjustDocSize() {
  CWorldBuilderDoc *pDoc = WbDoc();
  ASSERT_VALID(pDoc);
  // Initialize to a sensible default as we may get redrawn before we load a
  // map.
  ...
  // Calculate the scroll offset to place the cell m_centerX, m_centerY in the
  // middle.
  mXScrollOffset = (m_centerPt.X * m_cellSize) - clientRect.Width() / 2;
  mYScrollOffset = (flippedY * m_cellSize) - clientRect.Height() / 2;
  // Calculate the scroll bar limits so we can have up to 1/2 screen of white
  // space.
...
//=============================================================================
void CWorldBuilderView::OnVScroll(UINT nSBCode, UINT nPos,
 ...
 */
//=============================================================================
void CWorldBuilderView::OnHScroll(UINT nSBCode, UINT nPos,
                                  CScrollBar *pScrollBar) {
  int scrollPixels = 0;
...
void CWorldBuilderView::scrollInView(Real xScroll, Real yScroll, Bool end) {
  ...
  if (xScroll || yScroll) {
    ...
    CPoint pt((client.left + client.right) / 2 + mXScrollOffset,
              (client.bottom + client.top) / 2 + mYScrollOffset);
    CWorldBuilderDoc *pDoc = WbDoc();
    WorldHeightMapEdit *pMap = pDoc->GetHeightMap();
    if (pMap == NULL) return;
...
void CWorldBuilderView::OnShowGrid() {
  mShowGrid = !mShowGrid;
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowGrid",
                                 mShowGrid ? 1 : 0);
  Invalidate(false);
}
...
void CWorldBuilderView::OnViewShowcontours() {
  ...
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowContours",
                                 m_showContours ? 1 : 0);
  Invalidate(false);
}
...
void CWorldBuilderView::OnViewShowtexture() {
  ...
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowTexture",
                                 m_showTexture ? 1 : 0);
  Invalidate(false);
}
...
// ----------------------------------------------------------------------------
Bool CWorldBuilderView::viewToDocCoords(CPoint curPt, Coord3D *newPt,
                                        Bool constrain) {
  // Flip the y.
...
Bool CWorldBuilderView::docToViewCoords(Coord3D curPt, CPoint *newPt) {
  ...
  newPt->y = (pMap->getYExtent() - curPt.y / MAP_XY_FACTOR) * m_cellSize -
             mYScrollOffset;
#ifdef X_DEBUG
  Coord3D curPt2;
