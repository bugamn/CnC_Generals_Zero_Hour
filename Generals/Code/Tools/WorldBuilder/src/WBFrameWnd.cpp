...
BOOL CWBFrameWnd::LoadFrame(UINT nIDResource, DWORD dwDefaultStyle,
                            CWnd* pParentWnd, CCreateContext* pContext) {
  // dwDefaultStyle &= ~(WS_SIZEBOX|WS_MAXIMIZEBOX|WS_SYSMENU);

  BOOL ret = CFrameWnd::LoadFrame(nIDResource, dwDefaultStyle,
                                  CMainFrame::GetMainFrame(), pContext);
  if (ret) {
    Int top = ::AfxGetApp()->GetProfileInt(TWO_D_WINDOW_SECTION, "Top", 10);
    Int left = ::AfxGetApp()->GetProfileInt(TWO_D_WINDOW_SECTION, "Left", 10);
    this->SetWindowPos(NULL, left, top, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
    if (!m_cellSizeToolBar.Create(this, IDD_CELL_SLIDER, CBRS_LEFT,
                                  IDD_CELL_SLIDER)) {
...
void CWBFrameWnd::OnMove(int x, int y) {
  ...
  if (this->IsWindowVisible() && !this->IsIconic()) {
    ...
    ::AfxGetApp()->WriteProfileInt(TWO_D_WINDOW_SECTION, "Left",
                                   frameRect.left);
  }
}
...
BOOL CWB3dFrameWnd::LoadFrame(UINT nIDResource, DWORD dwDefaultStyle,
                              CWnd* pParentWnd, CCreateContext* pContext) {
  dwDefaultStyle &= ~(WS_SIZEBOX);

  BOOL ret = CMainFrame::LoadFrame(nIDResource, dwDefaultStyle,
                                   CMainFrame::GetMainFrame(), pContext);
  return (ret);
}
