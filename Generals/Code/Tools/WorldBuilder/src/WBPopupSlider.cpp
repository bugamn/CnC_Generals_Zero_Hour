...
void WBPopupSliderButton::SetupPopSliderButton(CWnd *pParentWnd, long controlID,
...
WBPopupSliderButton::WBPopupSliderButton() { m_owner = NULL; }
...
void WBPopupSliderButton::OnLButtonDown(UINT nFlags, CPoint point) {
  ...
  // just create the slider; it will delete itself when the user is done
  // scrolling
...
void PopupSlider::GetChannelRect(CRect *rect) {
  ...
  if (SB_HORZ == m_kind) {
    ...
    rect->bottom = THUMB_ICON_HEIGHT -
                   (CHANNEL_BELOW_THUMB_TOP + CHANNEL_ABOVE_THUMB_BOTTOM);
    rect->bottom += rect->top;
  } else {
    rect->InflateRect(0, -CHANNEL_X_INSET_AMOUNT);
    rect->left = PIXELS_ABOVE_SLIDER + CHANNEL_BELOW_THUMB_TOP + 2;
    rect->right = THUMB_ICON_HEIGHT -
                  (CHANNEL_BELOW_THUMB_TOP + CHANNEL_ABOVE_THUMB_BOTTOM);
...
void PopupSlider::New(CWnd *pParentWnd, long kind,
  ...
  try {
    ...
    pSliderOwner->GetPopSliderInfo(
        pPopupSlider->mSliderID, &(pPopupSlider->m_lo), &(pPopupSlider->m_hi),
        &(pPopupSlider->m_lineSize), &(pPopupSlider->m_curValue));

    DEBUG_ASSERTCRASH(pPopupSlider->m_hi != pPopupSlider->m_lo,
                      ("PopupSlider: endpoint values are the same!"));
    DEBUG_ASSERTCRASH(pPopupSlider->m_lineSize != 0,
                      ("PopupSlider: line size is zero!"));
...
BOOL PopupSlider::Create(const RECT &rect, CWnd *pParentWnd) {
  ...
  try {
    if (FALSE ==
        m_brush3dFaceColor.CreateSolidBrush(GetSysColor(COLOR_3DFACE))) {
      throw(-1);
    }

    mIcon = (HICON)LoadImage(AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_Thumb),
                             IMAGE_ICON, 0, 0, LR_LOADMAP3DCOLORS);

    DWORD dwExStyle = WS_EX_TOPMOST;
    DWORD dwStyle = WS_POPUP;
    UINT nClassStyle =
        CS_HREDRAW | CS_VREDRAW | CS_BYTEALIGNCLIENT | CS_SAVEBITS;
    HCURSOR hCursor = ::LoadCursor(NULL, IDC_ARROW);
    CString className =
        AfxRegisterWndClass(nClassStyle, hCursor, (HBRUSH)m_brush3dFaceColor);

    long winWidth, winHeight;

    winWidth = ((SB_HORZ == m_kind) ? POPUP_SLIDER_BIGDIMENSION
                                    : POPUP_SLIDER_SMALLDIMENSION);
    winHeight = ((SB_HORZ == m_kind) ? POPUP_SLIDER_SMALLDIMENSION
                                     : POPUP_SLIDER_BIGDIMENSION);

    CRect winRect(rect.right - winWidth, rect.bottom, rect.right,
                  rect.bottom + winHeight);

    // we'll just use "this" for the child ID
    if (FALSE == CWnd::CreateEx(dwExStyle, (LPCTSTR)className, "", dwStyle,
                                winRect.left, winRect.top, winRect.Width(),
                                winRect.Height(), pParentWnd->GetSafeHwnd(),
                                NULL, NULL))
      throw(-1);
    ...
    if (pParentWnd) {
      ...
      SetWindowPos(NULL, myWindowRect.left, myWindowRect.top,
                   myWindowRect.Width(), myWindowRect.Height(),
...
void PopupSlider::OnPaint() {
  ...
  dc.Draw3dRect(&channelRect, GetSysColor(COLOR_3DDKSHADOW),
                GetSysColor(COLOR_3DLIGHT));

  if (mIcon) {
    CRect iconRect;
    GetThumbIconRect(&iconRect);
    ::DrawIconEx(dc.GetSafeHdc(), iconRect.left, iconRect.top, mIcon, 0, 0, 0,
                 NULL, DI_NORMAL);
  }
