...
The	fence options panel has a subset of the objects available, and when one
is selected, makes the current object in the object options panel match this
object. Then the new object is created by the object options panel, so team
parenting and so forth is all handled in the object options panel.  jba. */
...
//-------------------------------------------------------------------------------------------------
void FenceOptions::addObject(MapObject *mapObject, const char *pPath,
                             const char *name, Int terrainNdx,
                             HTREEITEM parent) {
  char buffer[_MAX_PATH];
...
Bool FenceOptions::hasSelectedObject(void) {
  ...
  if (ObjectOptions::getCurGdfName() == AsciiString::TheEmptyString)
    return false;
