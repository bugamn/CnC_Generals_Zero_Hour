...
/// Constructor
ScorchTool::ScorchTool(void) : Tool(ID_SCORCH_TOOL, IDC_SCORCH) {}
...
void ScorchTool::mouseDown(TTrackingMode m, CPoint viewPt, WbView *pView,
  ...
  if (pObj) {
    ...
    MapObject *pNew =
        newInstance(MapObject)(docPt, AsciiString("Scorch"), 0, 0, NULL, NULL);
    pNew->getProperties()->setAsciiString(TheKey_originalOwner,
                                          NEUTRAL_TEAM_INTERNAL_STR);
    pNew->setSelected(true);
    pNew->setIsScorch();
    pNew->getProperties()->setReal(TheKey_objectRadius,
                                   ScorchOptions::getScorchSize());
    pNew->getProperties()->setInt(TheKey_scorchType,
                                  ScorchOptions::getScorchType());
...
void ScorchTool::mouseMoved(TTrackingMode m, CPoint viewPt, WbView *pView,
                            CWorldBuilderDoc *pDoc) {
  if (m != TRACK_L) return;
  Coord3D docPt;
  pView->viewToDocCoords(viewPt, &docPt);
...
void ScorchTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView *pView,
                         CWorldBuilderDoc *pDoc) {}
