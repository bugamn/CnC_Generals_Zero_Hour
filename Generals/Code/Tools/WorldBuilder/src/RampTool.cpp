...
Bool RampTool::followsTerrain(void) { return true; }

void RampTool::mouseMoved(TTrackingMode m, CPoint viewPt, WbView* pView,
  ...
  if (!((m == TRACK_L) && mIsMouseDown)) {
    ...
    docPt.z =
        TheTerrainRenderObject->getHeightMapHeight(docPt.x, docPt.y, NULL);
    mEndPoint = docPt;
  }
...
void RampTool::mouseDown(TTrackingMode m, CPoint viewPt, WbView* pView,
                         CWorldBuilderDoc* pDoc) {
  if (m != TRACK_L) {
    return;
  }
  ...
  pView->viewToDocCoords(viewPt, &docPt);
  mStartPoint = docPt;
  mStartPoint.z = TheTerrainRenderObject->getHeightMapHeight(
      mStartPoint.x, mStartPoint.y, NULL);

  mIsMouseDown = true;
}

void RampTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView* pView,
                       CWorldBuilderDoc* pDoc) {
  if (!((m == TRACK_L) && mIsMouseDown)) {
    return;
  }
  ...
  pView->viewToDocCoords(viewPt, &docPt);
  mEndPoint = docPt;
  mEndPoint.z = TheTerrainRenderObject->getHeightMapHeight(mEndPoint.x,
                                                           mEndPoint.y, NULL);
...
void RampTool::drawFeedback(Coord3D* endPoint) {
  ...
  DrawObject::setRampFeedbackParms(&mStartPoint, endPoint,
                                   TheRampOptions->getRampWidth());
}
...
void RampTool::applyRamp(CWorldBuilderDoc* pDoc) {
  ...
  BuildRectFromSegmentAndWidth(&mStartPoint, &mEndPoint, width, &bl, &tl, &br,
                               &tr);
  ...
  /*
          This part is pretty straightforward. Determine the U value for the
     shortest segment from the index's actual location to the segments from
     mStartPoint to mEndPoint, and then apply a linear gradient factor according
     to the height from the beginning to the end of mStartPoint and mEndPoint.
  */
  ...
  for (int i = 0; i < indiceCount; ++i) {
    ...
    worldHeightDup->setHeight(indices[i].x, indices[i].y,
  ...
  // Once we've applied the ramp, its no longer a mutable thing, so blow away
  // the feedback
  DrawObject::setDoRampFeedback(false);
}
