...
/// Constructor
MeshMoldTool::MeshMoldTool(void) : Tool(ID_MOLD_TOOL, IDC_MOLD_POINTER) {
  m_offsettingZ = false;
}

/// Destructor
MeshMoldTool::~MeshMoldTool(void) { REF_PTR_RELEASE(m_htMapEditCopy); }
...
void MeshMoldTool::mouseDown(TTrackingMode m, CPoint viewPt, WbView *pView,
  ...
  if (!m_tracking) {
    ...
    m_toolPos = cpt;
    Real height =
        TheTerrainRenderObject->getHeightMapHeight(cpt.x, cpt.y, NULL);
...
void MeshMoldTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView *pView,
                           CWorldBuilderDoc *pDoc) {
  if (m != TRACK_L) return;
  m_offsettingZ = false;
}
...
/// Track the mouse.
void MeshMoldTool::mouseMoved(TTrackingMode m, CPoint viewPt, WbView *pView,
                              CWorldBuilderDoc *pDoc) {
  if (m != TRACK_L) {
    return;
  }
  ...
  if (p3View) {
    ...
    if (pDraw) {
      ...
      if (pMesh) {
        ...
        Int minX = ceil((m_toolPos.x + bounds.Center.X - bounds.Radius) /
                        MAP_XY_FACTOR);
        Int minY = ceil((m_toolPos.y + bounds.Center.Y - bounds.Radius) /
                        MAP_XY_FACTOR);
        Int maxX = floor((m_toolPos.x + bounds.Center.X + bounds.Radius) /
                         MAP_XY_FACTOR);
        Int maxY = floor((m_toolPos.y + bounds.Center.Y + bounds.Radius) /
                         MAP_XY_FACTOR);
        maxX++;
        maxY++;
        if (minX < 0) minX = 0;
        ...
        for (j = minY; j < maxY; j++) {
          for (i = minX; i < maxX; i++) {
            ...
            if (pMesh->Cast_Ray(rayCollide)) {
              ...
              Int newHeight =
                  floor(((intersection.Z + MeshMoldOptions::getHeight()) /
                         MAP_HEIGHT_SCALE) +
                        0.5);
              // check boundary values
              if (newHeight < m_htMapEditCopy->getMinHeightValue())
                newHeight = m_htMapEditCopy->getMinHeightValue();
              if (newHeight > m_htMapEditCopy->getMaxHeightValue())
                newHeight = m_htMapEditCopy->getMaxHeightValue();
              Bool apply = true;
              if (MeshMoldOptions::isRaisingOnly()) {
                apply = newHeight >
                        m_htMapEditCopy->getHeight(i + border, j + border);
        ...
        // Offset by 2, because the normals change out two past the actual
        // height change.
