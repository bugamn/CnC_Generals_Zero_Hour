...
// This function spiders out and un/picks all Waypoints that have some form of
// indirect contact with this point Has a recursive helper function as well.
//
static void helper_pickAllWaypointsInPath(Int sourceID, CWorldBuilderDoc *pDoc,
                                          const Int numWaypointLinks,
...
static void pickAllWaypointsInPath(Int sourceID, Bool select) {
  ...
  helper_pickAllWaypointsInPath(sourceID, pDoc, pDoc->getNumWaypointLinks(),
                                alreadyTouched);

  // already touched should now be filled with waypointIDs that want to be
  // un/selected
  MapObject *pMapObj = MapObject::getFirstMapObject();
  while (pMapObj) {
    if (pMapObj->isWaypoint()) {
      if (std::find(alreadyTouched.begin(), alreadyTouched.end(),
                    pMapObj->getWaypointID()) != alreadyTouched.end()) {
...
static void helper_pickAllWaypointsInPath(Int sourceID, CWorldBuilderDoc *pDoc,
                                          const Int numWaypointLinks,
                                          std::vector<Int> &alreadyTouched) {
  if (std::find(alreadyTouched.begin(), alreadyTouched.end(), sourceID) !=
      alreadyTouched.end()) {
    return;
  }
  ...
  for (int i = 0; i < numWaypointLinks; ++i) {
    ...
    if (way1 == sourceID) {
      helper_pickAllWaypointsInPath(way2, pDoc, numWaypointLinks,
                                    alreadyTouched);
    }

    if (way2 == sourceID) {
      helper_pickAllWaypointsInPath(way1, pDoc, numWaypointLinks,
                                    alreadyTouched);
    }
  }
}
...
void PointerTool::clearSelection(
    void)  ///< Clears the selected objects selected flags.
{
  // Clear selection.
...
Bool PointerTool::allowPick(MapObject *pMapObj, WbView *pView) {
  ...
  if (tt && tt->getEditorSorting() == ES_AUDIO) {
    if (pView->GetPickConstraint() == ES_NONE ||
        pView->GetPickConstraint() == ES_AUDIO) {
      return true;
    }
  }
  if ((tt && !pView->getShowModels()) ||
      (pMapObj->getFlags() & FLAG_DONT_RENDER)) {
    return false;
  }
  if (pView->GetPickConstraint() != ES_NONE) {
    if (tt) {
...
void PointerTool::mouseDown(TTrackingMode m, CPoint viewPt, WbView *pView,
  ...
  m_doPolyTool = false;
  if (pView->GetPickConstraint() == ES_NONE ||
      pView->GetPickConstraint() == ES_WAYPOINT) {
  ...
  if (pView->GetPickConstraint() == ES_NONE ||
      pView->GetPickConstraint() == ES_ROAD) {
    if (!shiftKey && pClosestPicked &&
        (pClosestPicked->getFlags() & FLAG_ROAD_FLAGS)) {
  ...
  if (anySelected) {
    ...
    if (m_curObject) {
      // adjust the starting point so if we are snapping, the object snaps as
      // well.
...
void PointerTool::mouseMoved(TTrackingMode m, CPoint viewPt, WbView *pView,
                             CWorldBuilderDoc *pDoc) {
  Coord3D cpt;
  ...
  while (curMapObj) {
    if (curMapObj->isSelected()) {
      // pDoc->invalObject(curMapObj);			// invaling in all views
      // can be too slow.
...
void PointerTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView *pView,
                          CWorldBuilderDoc *pDoc) {
  if (m != TRACK_L) return;
  ...
  if (m_curObject && m_moving) {
    ...
    for (pObj = MapObject::getFirstMapObject(); pObj; pObj = pObj->getNext()) {
      ...
      if (pView->docToViewCoords(loc, &viewPt)) {
        picked = (viewPt.x >= box.left && viewPt.x <= box.right &&
                  viewPt.y >= box.top && viewPt.y <= box.bottom);
        if (picked) {
