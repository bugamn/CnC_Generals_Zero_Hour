...
TeamReinforcement::TeamReinforcement()
    : CPropertyPage(TeamReinforcement::IDD), m_teamDict(NULL) {
...
BOOL TeamReinforcement::OnInitDialog() {
  ...
  AsciiString transport =
      m_teamDict->getAsciiString(TheKey_teamTransport, &exists);
  ...
  AsciiString homeWaypoint =
      m_teamDict->getAsciiString(TheKey_teamReinforcementOrigin, &exists);
...
void TeamReinforcement::OnDeployBy() {
  ...
  if (pCheck->GetCheck()) {
    ...
    m_teamDict->setAsciiString(TheKey_teamTransport,
