...
BOOL ReplaceUnitDialog::OnInitDialog() {
  ...
  if (pWnd) pWnd->SetWindowText(m_missingName.str());
...
void PickUnitDialog::OnMove(int x, int y) {
  ...
  if (this->IsWindowVisible() && !this->IsIconic()) {
    ...
    ::AfxGetApp()->WriteProfileInt(BUILD_PICK_PANEL_SECTION, "Top",
                                   frameRect.top);
    ::AfxGetApp()->WriteProfileInt(BUILD_PICK_PANEL_SECTION, "Left",
                                   frameRect.left);
  }
}
...
//-------------------------------------------------------------------------------------------------
void PickUnitDialog::addObject(MapObject *mapObject, const char *pPath,
                               Int index, HTREEITEM parent) {
  char buffer[_MAX_PATH];
...
const ThingTemplate *PickUnitDialog::getPickedThing(void) {
  if (m_currentObjectIndex >= 0) {
    ...
    for (tTemplate = TheThingFactory->firstTemplate(); tTemplate;
         tTemplate = tTemplate->friend_getNextTemplate()) {
      if (m_currentObjectName == tTemplate->getName()) return tTemplate;
    }
  }
  return NULL;
}
