...
TeamIdentity::TeamIdentity() : CPropertyPage(TeamIdentity::IDD) {
...
BOOL TeamIdentity::OnInitDialog() {
  ...
  loadUnitsInfo(IDC_MIN_UNIT1, TheKey_teamUnitMinCount1, IDC_MAX_UNIT1,
                TheKey_teamUnitMaxCount1, IDC_UNIT_TYPE1, TheKey_teamUnitType1);

  loadUnitsInfo(IDC_MIN_UNIT2, TheKey_teamUnitMinCount2, IDC_MAX_UNIT2,
                TheKey_teamUnitMaxCount2, IDC_UNIT_TYPE2, TheKey_teamUnitType2);

  loadUnitsInfo(IDC_MIN_UNIT3, TheKey_teamUnitMinCount3, IDC_MAX_UNIT3,
                TheKey_teamUnitMaxCount3, IDC_UNIT_TYPE3, TheKey_teamUnitType3);

  loadUnitsInfo(IDC_MIN_UNIT4, TheKey_teamUnitMinCount4, IDC_MAX_UNIT4,
                TheKey_teamUnitMaxCount4, IDC_UNIT_TYPE4, TheKey_teamUnitType4);

  loadUnitsInfo(IDC_MIN_UNIT5, TheKey_teamUnitMinCount5, IDC_MAX_UNIT5,
                TheKey_teamUnitMaxCount5, IDC_UNIT_TYPE5, TheKey_teamUnitType5);

  loadUnitsInfo(IDC_MIN_UNIT6, TheKey_teamUnitMinCount6, IDC_MAX_UNIT6,
                TheKey_teamUnitMaxCount6, IDC_UNIT_TYPE6, TheKey_teamUnitType6);

  loadUnitsInfo(IDC_MIN_UNIT7, TheKey_teamUnitMinCount7, IDC_MAX_UNIT7,
                TheKey_teamUnitMaxCount7, IDC_UNIT_TYPE7, TheKey_teamUnitType7);

  CComboBox *pCombo = (CComboBox *)GetDlgItem(IDC_HOME_WAYPOINT);
  Bool exists;
  EditParameter::loadWaypoints(pCombo);
  AsciiString homeWaypoint =
      m_teamDict->getAsciiString(TheKey_teamHome, &exists);
  ...
  for (Int i = 0; i < TheSidesList->getNumSides(); i++) {
    AsciiString name = TheSidesList->getSideInfo(i)->getDict()->getAsciiString(
        TheKey_playerName);
    if (name.isEmpty()) name = NEUTRAL_NAME_STR;
  ...
  AsciiString oname_ui = TheSidesList->getSideInfo(myPlayerIndex)
                             ->getDict()
  ...
  AsciiString description =
      m_teamDict->getAsciiString(TheKey_teamDescription, &exists);
  ...
  Int increase =
      m_teamDict->getInt(TheKey_teamProductionPrioritySuccessIncrease, &exists);
  description.format("%d", increase);
  pWnd->SetWindowText(description.str());

  pWnd = GetDlgItem(IDC_PRIORITY_DECREASE);
  Int decrease =
      m_teamDict->getInt(TheKey_teamProductionPriorityFailureDecrease, &exists);
  ...
  pCombo->SetFocus();
  AsciiString script =
      m_teamDict->getAsciiString(TheKey_teamProductionCondition, &exists);
  if (exists) {
  ...
  if (pButton) {
    Bool executeActions =
        m_teamDict->getBool(TheKey_teamExecutesActionsOnCreate, &exists);
    if (!exists) {
  ...
  if (wNotifyCode == EN_CHANGE) {
    ...
    switch (editCtrl) {
      default:
        editCtrl = 0;
      case IDC_MIN_UNIT1:
        key = TheKey_teamUnitMinCount1;
        break;
      case IDC_MAX_UNIT1:
        key = TheKey_teamUnitMaxCount1;
        break;

      case IDC_MIN_UNIT2:
        key = TheKey_teamUnitMinCount2;
        break;
      case IDC_MAX_UNIT2:
        key = TheKey_teamUnitMaxCount2;
        break;

      case IDC_MIN_UNIT3:
        key = TheKey_teamUnitMinCount3;
        break;
      case IDC_MAX_UNIT3:
        key = TheKey_teamUnitMaxCount3;
        break;

      case IDC_MIN_UNIT4:
        key = TheKey_teamUnitMinCount4;
        break;
      case IDC_MAX_UNIT4:
        key = TheKey_teamUnitMaxCount4;
        break;

      case IDC_MIN_UNIT5:
        key = TheKey_teamUnitMinCount5;
        break;
      case IDC_MAX_UNIT5:
        key = TheKey_teamUnitMaxCount5;
        break;

      case IDC_MIN_UNIT6:
        key = TheKey_teamUnitMinCount6;
        break;
      case IDC_MAX_UNIT6:
        key = TheKey_teamUnitMaxCount6;
        break;

      case IDC_MIN_UNIT7:
        key = TheKey_teamUnitMinCount7;
        break;
      case IDC_MAX_UNIT7:
        key = TheKey_teamUnitMaxCount7;
        break;
    }
    if (editCtrl != 0) {
    ...
    switch (cmboCtrl) {
      case IDC_UNIT_TYPE1:
        key = TheKey_teamUnitType1;
        break;
      case IDC_UNIT_TYPE2:
        key = TheKey_teamUnitType2;
        break;
      case IDC_UNIT_TYPE3:
        key = TheKey_teamUnitType3;
        break;
      case IDC_UNIT_TYPE4:
        key = TheKey_teamUnitType4;
        break;
      case IDC_UNIT_TYPE5:
        key = TheKey_teamUnitType5;
        break;
      case IDC_UNIT_TYPE6:
        key = TheKey_teamUnitType6;
        break;
      case IDC_UNIT_TYPE7:
        key = TheKey_teamUnitType7;
        break;
      default:
        cmboCtrl = 0;
    }
...
void TeamIdentity::OnUnitType1Button() { OnUnitTypeButton(IDC_UNIT_TYPE1); }

void TeamIdentity::OnUnitType2Button() { OnUnitTypeButton(IDC_UNIT_TYPE2); }

void TeamIdentity::OnUnitType3Button() { OnUnitTypeButton(IDC_UNIT_TYPE3); }

void TeamIdentity::OnUnitType4Button() { OnUnitTypeButton(IDC_UNIT_TYPE4); }

void TeamIdentity::OnUnitType5Button() { OnUnitTypeButton(IDC_UNIT_TYPE5); }

void TeamIdentity::OnUnitType6Button() { OnUnitTypeButton(IDC_UNIT_TYPE6); }

void TeamIdentity::OnUnitType7Button() { OnUnitTypeButton(IDC_UNIT_TYPE7); }
...
void TeamIdentity::OnExecuteActions() {
  ...
  m_teamDict->setBool(TheKey_teamExecutesActionsOnCreate,
                      pButton->GetCheck() ? true : false);
}
    ...
    if (tnamecur != tnamenew) {
      if (m_sides->findTeamInfo(tnamenew) || m_sides->findSideInfo(tnamenew)) {
        ...
        if (count > 0) {
          ...
          if (::AfxMessageBox(msg, MB_YESNO) == IDYES) set = true;
        }
      }
    }
    if (set) {
      ...
      Bool exists;
      AsciiString description =
          m_teamDict->getAsciiString(TheKey_teamName, &exists);
      pWnd->SetWindowText(description.str());
    }
  }
}
