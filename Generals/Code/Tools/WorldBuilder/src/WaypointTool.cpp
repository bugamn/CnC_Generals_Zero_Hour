...
/// Constructor
WaypointTool::WaypointTool(void) : Tool(ID_WAYPOINT_TOOL, IDC_WAYPOINT) {}

/// Destructor
WaypointTool::~WaypointTool(void) {}

/// Clears it's is active flag.
void WaypointTool::deactivate() { m_isActive = false; }
...
void WaypointTool::mouseDown(TTrackingMode m, CPoint viewPt, WbView *pView,
  ...
  if (pObj) {
    ...
    MapObject *pNew = newInstance(MapObject)(
        docPt, AsciiString("*Waypoints/Waypoint"), 0, 0, NULL, NULL);
...
void WaypointTool::mouseMoved(TTrackingMode m, CPoint viewPt, WbView *pView,
                              CWorldBuilderDoc *pDoc) {
  if (m != TRACK_L) return;
  Coord3D docPt;
  pView->viewToDocCoords(viewPt, &docPt);
...
void WaypointTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView *pView,
                           CWorldBuilderDoc *pDoc) {
