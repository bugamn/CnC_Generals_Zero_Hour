...
void WaypointOptions::updateTheUI(void) {
  ...
  CComboBox *pListWayptNames =
      (CComboBox *)GetDlgItem(IDC_LIST_OF_WAYPOINT_NAMES);
  ...
  if (pCombo && !theTrigger) {
    pCombo->ResetContent();
    pCombo->AddString(
        (TheNameKeyGenerator->keyToName(TheKey_InitialCameraPosition)).str());
    pCombo->AddString(
        (TheNameKeyGenerator->keyToName(TheKey_Player_1_Start)).str());
    pCombo->AddString(
        (TheNameKeyGenerator->keyToName(TheKey_Player_2_Start)).str());
    pCombo->AddString(
        (TheNameKeyGenerator->keyToName(TheKey_Player_3_Start)).str());
    pCombo->AddString(
        (TheNameKeyGenerator->keyToName(TheKey_Player_4_Start)).str());
    pCombo->AddString(
        (TheNameKeyGenerator->keyToName(TheKey_Player_5_Start)).str());
    pCombo->AddString(
        (TheNameKeyGenerator->keyToName(TheKey_Player_6_Start)).str());
    pCombo->AddString(
        (TheNameKeyGenerator->keyToName(TheKey_Player_7_Start)).str());
    pCombo->AddString(
        (TheNameKeyGenerator->keyToName(TheKey_Player_8_Start)).str());
  ...
  if ((theMapObj || isWaypointTool) && pWnd) {
    if (theMapObj) {
      Bool exists;
      pWnd->EnableWindow();
      pWnd->SetWindowText(theMapObj->getProperties()
                              ->getAsciiString(TheKey_waypointName)
                              .str());
      ...
      const Coord3D *waypointLocation =
          getSingleSelectedWaypoint()->getLocation();
      ...
      if (pDoc->isWaypointLinked(theMapObj)) {
        ...
        AsciiString name;
        name = theMapObj->getProperties()->getAsciiString(
      ...
      if (pBiDirCheck) {
        ...
        Bool checked = theMapObj->getProperties()->getBool(
...
void WaypointOptions::OnChangeSelectedWaypoint() {
  ...
  // retrieve information from dialog box, if user-typed -- sel will be -1,
  ...
  // find and store the waypoint that corresponds to the information in the
  // dialog box
  ...
  // if no waypoint corresponds to whatever is in the dialog box, then don't do
  // anything
...
void WaypointOptions::OnEditWaypointLocationX() {
  ...
  // make sure that there is a value in the field, otherwise funky stuff would
  // happen
...
Bool WaypointOptions::isUnique(AsciiString name, MapObject *theMapObj) {
  MapObject *pMapObj;
  Bool didMatch = false;
  for (pMapObj = MapObject::getFirstMapObject(); !didMatch && pMapObj;
       pMapObj = pMapObj->getNext()) {
    if (pMapObj->isWaypoint()) {
...
void WaypointOptions::OnChangeWaypointnameEdit() {
  ...
  if (pCombo) {
    ...
    for (pTrig = PolygonTrigger::getFirstPolygonTrigger(); !didMatch && pTrig;
         pTrig = pTrig->getNext()) {
    ...
    if (didMatch) {
      ...
      if (theMapObj) {
        AsciiString layerName =
            TheLayersList->removeMapObjectFromLayersList(theMapObj);
...
void WaypointOptions::OnWaypointBidirectional() {
  ...
  if (pWnd) {
    ...
    pWnd->GetWindowText(theText);
    AsciiString name((LPCTSTR)theText);
    theMapObj->getProperties()->setBool(TheKey_waypointPathBiDirectional,
                                        checked);
    pDoc->updateLinkedWaypointLabels(theMapObj);
  }
}
