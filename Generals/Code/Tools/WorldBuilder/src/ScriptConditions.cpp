...
ScriptConditionsDlg::ScriptConditionsDlg()
    : CPropertyPage(ScriptConditionsDlg::IDD),
      m_condition(NULL),
      m_orCondition(NULL),
      m_index(0) {
...
BOOL ScriptConditionsDlg::OnInitDialog() {
  ...
  pList->SetHorizontalExtent(
      10000);   // band-aid fix until more precise length can be determined
...
void ScriptConditionsDlg::OnDblclkConditionList() { OnEditCondition(); }
...
Int ScriptConditionsDlg::doMoveDown(OrCondition **outWhichNow) {
  ...
  if (m_condition && m_orCondition) {
    ...
    if (pPrev) {
      ...
      DEBUG_ASSERTCRASH(m_condition == m_orCondition->getFirstAndCondition(),
                        ("Logic error."));
      pCur->setNextCondition(pNext->getNext());
      pNext->setNextCondition(pCur);
    ...
    if (pPrev) {
      ...
      DEBUG_ASSERTCRASH(m_orCondition == m_script->getOrCondition(),
                        ("Logic error."));
      pCur->setNextOrCondition(pNext->getNextOrCondition());
      pNext->setNextOrCondition(pCur);
