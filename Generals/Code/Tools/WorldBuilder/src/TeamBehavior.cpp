...
TeamBehavior::TeamBehavior() : CPropertyPage(TeamBehavior::IDD) {
...
BOOL TeamBehavior::OnInitDialog() {
  ...
  Bool transportsReturn =
      m_teamDict->getBool(TheKey_teamTransportsReturn, &exists);
  ...
  if (pCheck) {
    Bool perimeter =
        m_teamDict->getBool(TheKey_teamIsPerimeterDefense, &exists);
  ...
  pCombo->SetCurSel(m_teamDict->getInt(TheKey_teamAggressiveness, &exists) -
                    AI_SLEEP);
