...
CFixTeamOwnerDialog::CFixTeamOwnerDialog(TeamsInfo *ti, SidesList *sl,
...
AsciiString CFixTeamOwnerDialog::getSelectedOwner() { return m_selectedOwner; }
...
BOOL CFixTeamOwnerDialog::OnInitDialog() {
  ...
  for (Int i = 0; i < numSides; ++i) {
    ...
    AsciiString displayName =
        si->getDict()->getAsciiString(TheKey_playerDisplayName, &displayExists);
    if (displayExists) {
      ...
      AsciiString internalName =
          si->getDict()->getAsciiString(TheKey_playerName, &displayExists);
