...
TerrainModal::TerrainModal(AsciiString path, WorldHeightMapEdit *pMap,
                           CWnd *pParent /*=NULL*/)
...
void TerrainModal::updateLabel(void) {
  ...
  const char *tName =
      pDoc->GetHeightMap()->getTexClassUiName(m_currentFgTexture).str();
...
void TerrainModal::addTerrain(char *pPath, Int terrainNdx, HTREEITEM parent) {
  ...
  // defined in an INI file, otherwise it was from legacy GDF stuff.  We will
  // sort all of the legacy GDF terrain entries in a tree leaf all their own
  // while the others are sorted according to a field specified in INI
  //
  if (terrain) {
