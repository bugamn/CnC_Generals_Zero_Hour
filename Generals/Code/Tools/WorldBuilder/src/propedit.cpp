...
PropEdit::PropEdit(AsciiString *key, Dict::DataType *type, AsciiString *value,
                   Bool valueOnly, CWnd *parent)
    : CDialog(PropEdit::IDD, parent),
      m_key(key),
      m_type(type),
      m_value(value),
      m_updating(0),
...
#ifdef RESTRICT_KEYS
  tmp.MakeLower();  // we force user-entered keys to all-lowercase, to prevent
                    // case issues
#endif
  *m_key = (LPCTSTR)tmp;
  ...
  if (!m_valueOnly) {
    ...
    if (len <= 0) enabOK = false;

#ifdef RESTRICT_KEYS
    // only letters, numbers, and underline allowed
    for (int i = 0; i < len; i++)
      if (!isalnum(tmp[i]) && tmp[i] != '_') enabOK = false;
#endif
  }
...
void PropEdit::OnChangeKeyname() { validate(); }

void PropEdit::OnEditchangeKeytype() { validate(); }

void PropEdit::OnCloseupKeytype() { validate(); }

void PropEdit::OnSelchangeKeytype() { validate(); }

void PropEdit::OnChangeValue() { validate(); }
...
void PropEdit::OnPropbool() { validate(); }
