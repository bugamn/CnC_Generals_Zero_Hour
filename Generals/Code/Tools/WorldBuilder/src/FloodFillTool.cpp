...
FloodFillTool::FloodFillTool(void)
    : Tool(ID_TILE_FLOOD_FILL, IDC_FLOOD_FILL), m_cliffCursor(NULL) {}
...
/** Creates a copy of the height map, flood fills it at pt with
m_textureClassToDraw which has been set by the calling routine.  Then builds the
command, and passes it to the doc. */
void FloodFillTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView *pView,
  ...
  if (m_adjustCliffTextures) {
    ...
    didIt =
        htMapEditCopy->floodFill(ndx.x, ndx.y, m_textureClassToDraw, shiftKey);
  }
  if (didIt) {
