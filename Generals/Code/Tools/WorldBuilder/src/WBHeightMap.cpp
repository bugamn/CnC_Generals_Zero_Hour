...
//=============================================================================
WBHeightMap::WBHeightMap() : m_drawEntireMap(false), m_flattenHeights(false) {}
...
Bool WBHeightMap::Cast_Ray(RayCollisionTestClass& raytest) {
  ...
  Int EndCellY;
  const Int overhang =
      2 * VERTEX_BUFFER_TILE_LENGTH;  // Allow picking past the edge for
                                      // scrolling & objects.
  ...
  for (p = 0; p < 3; p++) {
    ...
    Vector3 minPt(MAP_XY_FACTOR * (StartCellX - 1),
                  MAP_XY_FACTOR * (StartCellY - 1), theZ - 1);
    Vector3 maxPt(MAP_XY_FACTOR * (EndCellX + 1),
                  MAP_XY_FACTOR * (EndCellY + 1), theZ + 1);
    MinMaxAABoxClass mmbox(minPt, maxPt);
    hbox.Init(mmbox);
  }
