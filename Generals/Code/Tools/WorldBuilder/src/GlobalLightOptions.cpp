...
void GlobalLightOptions::showLightFeedback(Int lightIndex) {
  ...
  light.X = sin(PI / 2.0f + m_angleElevation[lightIndex] / 180.0f * PI) *
            cos(m_angleAzimuth[lightIndex] / 180.0f *
                PI);  // -WWMath::Sin(PI*(m_angleLR[lightIndex]-90)/180);
  light.Y = sin(PI / 2.0f + m_angleElevation[lightIndex] / 180.0f * PI) *
            sin(m_angleAzimuth[lightIndex] / 180.0f *
                PI);  //-WWMath::Sin(PI*(m_angleFB[lightIndex]-90)/180);
...
void GlobalLightOptions::applyAngle(Int lightIndex) {
  ...
  if (m_lighting == K_OBJECTS || m_lighting == K_BOTH) {
    tl = TheGlobalData
             ->m_terrainObjectsLighting[TheGlobalData->m_timeOfDay][lightIndex];
  } else {
    tl = TheGlobalData
             ->m_terrainLighting[TheGlobalData->m_timeOfDay][lightIndex];
  }
...
static void SpitLights() {
  ...
  for (Int time = 0; time < 4; time++) {
    for (Int light = 0; light < 3; light++) {
      redA = TheGlobalData->m_terrainLighting[time + TIME_OF_DAY_FIRST][light]
                 .ambient.red *
             255;
      greenA = TheGlobalData->m_terrainLighting[time + TIME_OF_DAY_FIRST][light]
                   .ambient.green *
               255;
      blueA = TheGlobalData->m_terrainLighting[time + TIME_OF_DAY_FIRST][light]
                  .ambient.blue *
              255;

      redD = TheGlobalData->m_terrainLighting[time + TIME_OF_DAY_FIRST][light]
                 .diffuse.red *
             255;
      greenD = TheGlobalData->m_terrainLighting[time + TIME_OF_DAY_FIRST][light]
                   .diffuse.green *
               255;
      blueD = TheGlobalData->m_terrainLighting[time + TIME_OF_DAY_FIRST][light]
                  .diffuse.blue *
              255;

      x = TheGlobalData->m_terrainLighting[time + TIME_OF_DAY_FIRST][light]
              .lightPos.x;
      y = TheGlobalData->m_terrainLighting[time + TIME_OF_DAY_FIRST][light]
              .lightPos.y;
      z = TheGlobalData->m_terrainLighting[time + TIME_OF_DAY_FIRST][light]
              .lightPos.z;

      DEBUG_LOG(("TerrainLighting%sAmbient%s = R:%d G:%d B:%d\n", times[time],
                 lights[light], redA, greenA, blueA));
      DEBUG_LOG(("TerrainLighting%sDiffuse%s = R:%d G:%d B:%d\n", times[time],
                 lights[light], redD, greenD, blueD));
      DEBUG_LOG(("TerrainLighting%sLightPos%s = X:%0.2f Y:%0.2f Z:%0.2f\n",
                 times[time], lights[light], x, y, z));

      redA = TheGlobalData
                 ->m_terrainObjectsLighting[time + TIME_OF_DAY_FIRST][light]
                 .ambient.red *
             255;
      greenA = TheGlobalData
                   ->m_terrainObjectsLighting[time + TIME_OF_DAY_FIRST][light]
                   .ambient.green *
               255;
      blueA = TheGlobalData
      ...
      DEBUG_LOG(
          ("TerrainObjectsLighting%sLightPos%s = X:%0.2f Y:%0.2f Z:%0.2f\n",
           times[time], lights[light], x, y, z));
  ...
  for (time = 0; time < 4; time++) {
    for (Int light = 0; light < 3; light++) {
      ...
      GlobalData::TerrainLighting tl =
          TheGlobalData->m_terrainLighting[theTime][light];

      DEBUG_LOG(
          ("TheGlobalData->m_terrainLighting[%d][%d].ambient.red = %0.4ff;\n",
           theTime, light, tl.ambient.red));
      DEBUG_LOG(
          ("TheGlobalData->m_terrainLighting[%d][%d].ambient.green = %0.4ff;\n",
           theTime, light, tl.ambient.green));
      DEBUG_LOG(
          ("TheGlobalData->m_terrainLighting[%d][%d].ambient.blue = %0.4ff;\n",
           theTime, light, tl.ambient.blue));

      DEBUG_LOG(
          ("TheGlobalData->m_terrainLighting[%d][%d].diffuse.red = %0.4ff;\n",
           theTime, light, tl.diffuse.red));
      DEBUG_LOG(
          ("TheGlobalData->m_terrainLighting[%d][%d].diffuse.green = %0.4ff;\n",
           theTime, light, tl.diffuse.green));
      DEBUG_LOG(
          ("TheGlobalData->m_terrainLighting[%d][%d].diffuse.blue = %0.4ff;\n",
           theTime, light, tl.diffuse.blue));

      DEBUG_LOG(
          ("TheGlobalData->m_terrainLighting[%d][%d].lightPos.x = %0.4ff;\n",
           theTime, light, tl.lightPos.x));
      DEBUG_LOG(
          ("TheGlobalData->m_terrainLighting[%d][%d].lightPos.y = %0.4ff;\n",
           theTime, light, tl.lightPos.y));
      DEBUG_LOG(
          ("TheGlobalData->m_terrainLighting[%d][%d].lightPos.z = %0.4ff;\n",
           theTime, light, tl.lightPos.z));

      tl = TheGlobalData->m_terrainObjectsLighting[theTime][light];

      DEBUG_LOG(
          ("TheGlobalData->m_terrainObjectsLighting[%d][%d].ambient.red = "
           "%0.4ff;\n",
           theTime, light, tl.ambient.red));
      DEBUG_LOG(
          ("TheGlobalData->m_terrainObjectsLighting[%d][%d].ambient.green = "
           "%0.4ff;\n",
           theTime, light, tl.ambient.green));
      DEBUG_LOG(
          ("TheGlobalData->m_terrainObjectsLighting[%d][%d].ambient.blue = "
           "%0.4ff;\n",
           theTime, light, tl.ambient.blue));

      DEBUG_LOG(
          ("TheGlobalData->m_terrainObjectsLighting[%d][%d].diffuse.red = "
           "%0.4ff;\n",
           theTime, light, tl.diffuse.red));
      DEBUG_LOG(
          ("TheGlobalData->m_terrainObjectsLighting[%d][%d].diffuse.green = "
           "%0.4ff;\n",
           theTime, light, tl.diffuse.green));
      DEBUG_LOG(
          ("TheGlobalData->m_terrainObjectsLighting[%d][%d].diffuse.blue = "
           "%0.4ff;\n",
           theTime, light, tl.diffuse.blue));

      DEBUG_LOG(
          ("TheGlobalData->m_terrainObjectsLighting[%d][%d].lightPos.x = "
           "%0.4ff;\n",
           theTime, light, tl.lightPos.x));
      DEBUG_LOG(
          ("TheGlobalData->m_terrainObjectsLighting[%d][%d].lightPos.y = "
           "%0.4ff;\n",
           theTime, light, tl.lightPos.y));
      DEBUG_LOG(
          ("TheGlobalData->m_terrainObjectsLighting[%d][%d].lightPos.z = "
           "%0.4ff;\n",
           theTime, light, tl.lightPos.z));
...
void GlobalLightOptions::OnResetLights() {
  ...
  if (pView) {
    pView->setLighting(
        &TheGlobalData->m_terrainLighting[TheGlobalData->m_timeOfDay][K_SUN],
        K_TERRAIN, K_SUN);
    pView->setLighting(
        &TheGlobalData
             ->m_terrainLighting[TheGlobalData->m_timeOfDay][K_ACCENT1],
        K_TERRAIN, K_ACCENT1);
    pView->setLighting(
        &TheGlobalData
             ->m_terrainLighting[TheGlobalData->m_timeOfDay][K_ACCENT2],
        K_TERRAIN, K_ACCENT2);

    pView->setLighting(
        &TheGlobalData
             ->m_terrainObjectsLighting[TheGlobalData->m_timeOfDay][K_SUN],
        K_OBJECTS, K_SUN);
    pView->setLighting(
        &TheGlobalData
             ->m_terrainObjectsLighting[TheGlobalData->m_timeOfDay][K_ACCENT1],
        K_OBJECTS, K_ACCENT1);
    pView->setLighting(
        &TheGlobalData
             ->m_terrainObjectsLighting[TheGlobalData->m_timeOfDay][K_ACCENT2],
        K_OBJECTS, K_ACCENT2);
  }
...
void GlobalLightOptions::stuffValuesIntoFields(Int lightIndex) {
  ...
  elevation =
      (angleElevation - PI / 2.0f) * 180.0f / PI;  // 90-(angleLR/PI)*180;
...
void GlobalLightOptions::applyColor(Int lightIndex) {
  ...
  if (lightIndex == K_SUN) {
    ...
    if (GetInt(IDC_RA_EDIT, &clr)) tl.ambient.red = ComponentToPercent(clr);
    if (GetInt(IDC_GA_EDIT, &clr)) tl.ambient.green = ComponentToPercent(clr);
    if (GetInt(IDC_BA_EDIT, &clr)) tl.ambient.blue = ComponentToPercent(clr);
  } else {
    tl.ambient.red = 0;
    tl.ambient.green = 0;
    tl.ambient.blue = 0;
...
void GlobalLightOptions::GetPopSliderInfo(const long sliderID, long *pMin,
                                          long *pMax, long *pLineSize,
                                          long *pInitial) {
  switch (sliderID) {
    case IDC_FB_POPUP:
      *pMin = 0;
...
void GlobalLightOptions::OnClose() {
  ...
  if (pView) {
    ...
    pView->doLightFeedback(false, lightRay,
                           0);  // turn off the light direction indicator
  }
};
  ...
  if (!IsWindowVisible()) {
    ...
    if (pView) {
      ...
      pView->doLightFeedback(false, lightRay,
                             0);  // turn off the light direction indicator
      pView->doLightFeedback(false, lightRay,
                             1);  // turn off the light direction indicator
      pView->doLightFeedback(false, lightRay,
                             2);  // turn off the light direction indicator
    }
  }
#if 0
	SpitLights();
...
void GlobalLightOptions::OnMove(int x, int y) {
  ...
  if (this->IsWindowVisible() && !this->IsIconic()) {
    ...
    ::AfxGetApp()->WriteProfileInt(GLOBALLIGHT_OPTIONS_PANEL_SECTION, "Top",
                                   frameRect.top);
    ::AfxGetApp()->WriteProfileInt(GLOBALLIGHT_OPTIONS_PANEL_SECTION, "Left",
                                   frameRect.left);
  }
}
