...
/// Undoable - destructor.
//
Undoable::~Undoable(void) { REF_PTR_RELEASE(mNext); }

//
/// Create a new undoable.
//
Undoable::Undoable(void) : mNext(NULL) {}

//
/// Link another undoable to this.
//
void Undoable::LinkNext(Undoable *pNext) { REF_PTR_SET(mNext, pNext); }

//
/// Redo defaults to Do().
//
void Undoable::Redo(void) { Do(); }
...
WBDocUndoable::WBDocUndoable(CWorldBuilderDoc *pDoc,
                             WorldHeightMapEdit *pNewHtMap, Coord3D *pObjOffset)
    : mPNewHeightMapData(NULL), mPOldHeightMapData(NULL), mPDoc(NULL) {
  if (pObjOffset) {
...
void WBDocUndoable::Do(void) {
  mPDoc->SetHeightMap(mPNewHeightMapData,
                      false);  // SetHeightMap but don't inval.
  if (m_offsetObjects) {
...
void WBDocUndoable::Redo(void) {
  ...
  mPDoc->SetHeightMap(mPNewHeightMapData,
                      true);  // SetHeightMap and inval the world.
  if (m_offsetObjects) {
    MapObject *pCur = MapObject::getFirstMapObject();
    while (pCur) {
      Coord3D loc = *pCur->getLocation();
...
void WBDocUndoable::Undo(void) {
  ...
  mPDoc->SetHeightMap(mPOldHeightMapData,
                      true);  // SetHeightMap and inval the world.
  if (m_offsetObjects) {
    MapObject *pCur = MapObject::getFirstMapObject();
    while (pCur) {
...
AddObjectUndoable::AddObjectUndoable(CWorldBuilderDoc *pDoc,
    ...
    : m_pDoc(NULL), m_numObjects(0), m_addedToList(false), m_objectToAdd(NULL) {
...
void AddObjectUndoable::Do(void) {
  ...
  m_numObjects = 0;
  while (pCur) {
    if (!pCur->getFlag(FLAG_ROAD_FLAGS) && !pCur->getFlag(FLAG_BRIDGE_FLAGS) &&
        !pCur->isWaypoint()) {
  ...
  for (int i = 0; i < m_numObjects; i++) {
    ...
    Bool dontCare;
    TheLayersList->addMapObjectToLayersList(
        pCur, props->getAsciiString(TheKey_objectLayer, &dontCare));
  ...
  while (pCur) {
    ...
    if (!(pCur->isWaypoint() || (flags & flagsWeDontWant) != 0)) ++numObjects;
...
void AddObjectUndoable::Undo(void) {
  ...
  while (pCur) {
    m_pDoc->invalObject(pCur);
    // Note : This undoable can add multiple objects, and removes multiple
    // objects.
...
MoveInfo::MoveInfo(MapObject *pObjToMove) : m_next(NULL) {
...
void MoveInfo::SetOffset(CWorldBuilderDoc *pDoc, Real x, Real y) {
  ...
  m_newLocation.x += x;
  m_newLocation.y += y;
  if (m_objectToModify->getFlag(FLAG_ROAD_POINT1) ||
      m_objectToModify->getFlag(FLAG_ROAD_POINT2)) {
...
ModifyObjectUndoable::ModifyObjectUndoable(CWorldBuilderDoc *pDoc)
    : m_pDoc(NULL), m_moveList(NULL), m_inval(false) {
  m_pDoc = pDoc;  // not ref counted.
...
//
ModifyFlagsUndoable::ModifyFlagsUndoable(CWorldBuilderDoc *pDoc, Int flagMask,
                                         Int flagValue)
    : m_pDoc(NULL), m_flagsList(NULL) {
  m_pDoc = pDoc;  // not ref counted.
  MapObject *curMapObj = MapObject::getFirstMapObject();
...
/// Set the new values, and force an inval of the objects.
//
void ModifyFlagsUndoable::Redo(void) { Do(); }
...
SidesListUndoable::SidesListUndoable(const SidesList &newSL,
  ...
  DEBUG_ASSERTLOG(!modified, ("*** had to clean up sides in SidesListUndoable! "
...
DictItemUndoable::DictItemUndoable(Dict **d, Dict data, NameKeyType key,
                                   Int dictsToModify, CWorldBuilderDoc *pDoc,
                                   Bool inval)
    : m_newDictData(data),
      m_key(key),
...
/*static*/ Dict DictItemUndoable::buildSingleItemDict(AsciiString k,
                                                      Dict::DataType t,
                                                      AsciiString v) {
  Dict d;
...
void DeleteInfo::DoDelete(WorldHeightMapEdit *pMap) {
  ...
  if (m_priorObject) {
    ...
    DEBUG_ASSERTCRASH(MapObject::TheMapObjectListPtr == m_objectToDelete,
                      ("oops"));
...
//
// DeleteObjectUndoable - create a new undoable.	Deletes all selected
// objects.
//
DeleteObjectUndoable::DeleteObjectUndoable(CWorldBuilderDoc *pDoc)
    : m_pDoc(NULL), m_deleteList(NULL) {
...
void DeleteObjectUndoable::Undo(void) {
  ...
  while (pCur) {
    ...
    Bool exists;
    TheLayersList->addMapObjectToLayersList(
        pCur->m_objectToDelete,
        objDict->getAsciiString(TheKey_objectLayer, &exists));
...
AddPolygonUndoable::AddPolygonUndoable(PolygonTrigger *pTrig)
    : m_trigger(pTrig), m_isTriggerInList(false) {}
...
// AddPolygonUndoable - destructor.
//
AddPolygonPointUndoable::~AddPolygonPointUndoable(void) { m_trigger = NULL; }
...
AddPolygonPointUndoable::AddPolygonPointUndoable(PolygonTrigger *pTrig,
                                                 ICoord3D pt)
    : m_trigger(pTrig), m_point(pt) {}

//
/// Add the trigger.
//
void AddPolygonPointUndoable::Do(void) { m_trigger->addPoint(m_point); }
...
ModifyPolygonPointUndoable::ModifyPolygonPointUndoable(PolygonTrigger *pTrig,
                                                       Int ndx)
    : m_trigger(pTrig), m_pointIndex(ndx) {
...
//
MovePolygonUndoable::~MovePolygonUndoable(void) { m_trigger = NULL; }
...
//
InsertPolygonPointUndoable::InsertPolygonPointUndoable(PolygonTrigger *pTrig,
                                                       ICoord3D pt, Int ndx)
    : m_trigger(pTrig), m_point(pt), m_pointIndex(ndx) {}
...
//
DeletePolygonPointUndoable::DeletePolygonPointUndoable(PolygonTrigger *pTrig,
                                                       Int ndx)
    : m_trigger(pTrig), m_pointIndex(ndx) {
  m_point = *pTrig->getPoint(ndx);
}
...
DeletePolygonUndoable::DeletePolygonUndoable(PolygonTrigger *pTrig)
    : m_trigger(pTrig), m_isTriggerInList(true) {}
