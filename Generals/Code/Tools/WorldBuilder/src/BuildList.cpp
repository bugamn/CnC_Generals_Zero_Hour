...
void BuildList::updateCurSide(void) {
  ...
  if (!pList) {
    DEBUG_LOG(
        ("*** BuildList::updateCurSide Missing resource!!! IDC_BUILD_LIST\n"));
    return;
  }
  pList->ResetContent();
...
void BuildList::OnAddBuilding() { BuildListTool::addBuilding(); }
  ...
  for (i = 0; i < TheSidesList->getNumSides(); i++) {
    ...
    for (BuildListInfo *pBuild = pSide->getBuildList(); pBuild;
      ...
      if (pInfo == pBuild) {
        ...
        //				CComboBox *pCombo =
        //(CComboBox*)m_staticThis->GetDlgItem(IDC_SIDES_COMBO);
...
void BuildList::OnSelchangeBuildList() {
  ...
  for (MapObject *pMapObj = MapObject::getFirstMapObject(); pMapObj;
    ...
    if (!thingTemplate) continue;
    Dict *d = pMapObj->getProperties();
    Bool exists;
    AsciiString objectTeamName =
        d->getAsciiString(TheKey_originalOwner, &exists);
    ...
    AsciiString objectOwnerName =
        (teamDict) ? teamDict->getAsciiString(TheKey_teamOwner)
...
void BuildList::OnEditchangeRebuilds() {
  ...
  Int sel = pCombo->GetCurSel();
  if (sel >= 0)
    return;  // An entry is selected, and handled by OnSelendokRebuilds..
...
void BuildList::OnDblclkBuildList() {
  ...
  BaseBuildProps dlg;
  dlg.setProps(pBI->getBuildingName(), pBI->getScript(), pBI->getHealth(),
               pBI->getUnsellable());
  if (dlg.DoModal() == IDOK) {
...
void BuildList::GetPopSliderInfo(const long sliderID, long *pMin, long *pMax,
                                 long *pLineSize, long *pInitial) {
  switch (sliderID) {
