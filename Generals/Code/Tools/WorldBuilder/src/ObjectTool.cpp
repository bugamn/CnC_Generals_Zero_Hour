...
void ObjectTool::mouseDown(TTrackingMode m, CPoint viewPt, WbView *pView,
...
void ObjectTool::mouseMoved(TTrackingMode m, CPoint viewPt, WbView *pView,
                            CWorldBuilderDoc *pDoc) {
  ...
  if (m == TRACK_L) {  // Mouse is down, so preview the angle if > hysteresis.
    ...
    justAClick = (abs(viewPt.x - m_downPt2d.x) < HYSTERESIS ||
...
void ObjectTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView *pView,
                         CWorldBuilderDoc *pDoc) {
  if (m != TRACK_L) return;
  ...
  Real angle = justAClick ? 0 : calcAngle(loc, cpt, pView);
  MapObject *pNew =
      ObjectOptions::duplicateCurMapObjectForPlace(&loc, angle, true);
  if (pNew) {
