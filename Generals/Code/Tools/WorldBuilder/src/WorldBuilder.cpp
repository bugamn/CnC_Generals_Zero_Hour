...
void initSubsystem(SUBSYSTEM *&sysref, SUBSYSTEM *sys, const char *path1 = NULL,
...
ON_UPDATE_COMMAND_UI(ID_TEXTURESIZING_MAPCLIFFTEXTURES,
...
BOOL CWorldBuilderApp::InitInstance() {
  ...
  initSubsystem(TheWritableGlobalData, new GlobalData(),
                "Data\\INI\\Default\\GameData.ini", "Data\\INI\\GameData.ini");

#if defined(_DEBUG) || defined(_INTERNAL)
  ini.load(AsciiString("Data\\INI\\GameDataDebug.ini"), INI_LOAD_MULTIFILE,
           NULL);
#endif
  ...
  initSubsystem(TheRankInfoStore, new RankInfoStore(), NULL,
                "Data\\INI\\Rank.ini");
  ...
  initSubsystem(TheWeaponStore, new WeaponStore(), NULL,
                "Data\\INI\\Weapon.ini");
  ...
  initSubsystem(TheLocomotorStore, new LocomotorStore(), NULL,
                "Data\\INI\\Locomotor.ini");
  initSubsystem(TheDamageFXStore, new DamageFXStore(), NULL,
  ...
  DEBUG_ASSERTCRASH(
      !TheGlobalData->m_useHalfHeightMap,
      ("TheGlobalData->m_useHalfHeightMap : Don't use this setting in WB."));
  ...
  m_3dtemplate = new CSingleDocTemplate(
      IDR_MAPDOC, RUNTIME_CLASS(CWorldBuilderDoc), RUNTIME_CLASS(CWB3dFrameWnd),
  ...
  if (!pMainFrame->LoadFrame(IDR_MAPDOC)) return FALSE;
...
int CWorldBuilderApp::ExitInstance() {
  ...
  TheMemoryPoolFactory->debugMemoryReport(
      REPORT_FACTORYINFO | REPORT_CP_LEAKS | REPORT_CP_STACKTRACE, gFirstCP,
      lastCP);
#endif
