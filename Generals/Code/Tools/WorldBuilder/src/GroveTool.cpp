...
void GroveTool::plantShrub(Coord3D *pos) {
  // TODO: Determine when we can tell something is a shurubbery, and plant it
  // here - jkmcd
...
void GroveTool::_plantGroveInBox(CPoint tl, CPoint br, WbView *pView) {
  ...
  for (int i = 0; i < numTotalTrees; ++i) {
    ...
    if (!TheGroveOptions->getCanPlaceInWater() &&
        localIsUnderwater(position.x, position.y)) {
      continue;
    }

    if (!TheGroveOptions->getCanPlaceOnCliffs() &&
        _positionIsTooCliffyForTrees(position)) {
      continue;
    }
 ...
 */
void GroveTool::plantGrove(Coord3D pos, Coord3D prevDir, Real baseHeight,
                           Int level, CPoint bounds) {
  ...
  for (Int i = 0; i < numChildren; i++) {
    if (GameLogicRandomValueReal(0.0f, 1.0f) < childChance) {
      for (retry = 0; retry < maxTries; retry++) {
        ...
        spread = GameLogicRandomValueReal(3.0f * MAP_XY_FACTOR,
                                          6.0f * MAP_XY_FACTOR);
        childPos.x = pos.x + spread * (Real)cos(angle);
        childPos.y = pos.y + spread * (Real)sin(angle);
        childPos.z = 0;
        if (TheTerrainRenderObject) {
          TheTerrainRenderObject->getHeightMapHeight(childPos.x, childPos.y,
                                                     &normal);
        }
        ...
        //	(note that if prevDir the zero vector, it passes this test (for
        //the 1st tree))
        ...
        if (dir.x * prevDir.x + dir.y * prevDir.y >= 0.0f &&
            ...
            fabs(childPos.z - baseHeight) < heightTolerance && childPos.x > 0 &&
            childPos.y > 0 && childPos.x < bounds.x && childPos.y < bounds.y &&
...
//
/// Constructor
GroveTool::GroveTool(void) : Tool(ID_GROVE_TOOL, IDC_GROVE) {
  m_headMapObj = NULL;
}
...
void GroveTool::mouseDown(TTrackingMode m, CPoint viewPt, WbView *pView,
...
void GroveTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView *pView,
                        CWorldBuilderDoc *pDoc) {
  if (m != TRACK_L) return;
...
void GroveTool::mouseMoved(TTrackingMode m, CPoint viewPt, WbView *pView,
                           CWorldBuilderDoc *pDoc) {
  if (m != TRACK_L) {
    return;
  }

  if (abs(viewPt.x - m_downPt.x) > DRAG_THRESHOLD ||
      abs(viewPt.y - m_downPt.y) > DRAG_THRESHOLD) {
    m_dragging = true;
  }
...
void GroveTool::addObj(Coord3D *pos, AsciiString name) {
  ...
  MapObject *pNew = newInstance(MapObject)(theLoc, pCur->getName(), angle, 0,
                                           NULL, pCur->getThingTemplate());
...
static Bool _positionIsTooCliffyForTrees(Coord3D pos) {
  ...
  otherPos.z =
      TheTerrainRenderObject->getHeightMapHeight(otherPos.x, otherPos.y, NULL);
