...
SaveMap::SaveMap(TSaveMapInfo *pInfo, CWnd *pParent /*=NULL*/)
    : CDialog(SaveMap::IDD, pParent), m_pInfo(pInfo) {
#if defined(_DEBUG) || defined(_INTERNAL)
  m_pInfo->usingSystemDir = m_usingSystemDir = ::AfxGetApp()->GetProfileInt(
      MAP_OPENSAVE_PANEL_SECTION, "UseSystemDir", TRUE);
#else
...
void SaveMap::OnSystemMaps() { populateMapListbox(TRUE); }

void SaveMap::OnUserMaps() { populateMapListbox(FALSE); }
...
void SaveMap::OnCancel() { CDialog::OnCancel(); }
...
#if defined(_DEBUG) || defined(_INTERNAL)
  ::AfxGetApp()->WriteProfileInt(MAP_OPENSAVE_PANEL_SECTION, "UseSystemDir",
                                 m_usingSystemDir);
#endif
...
BOOL SaveMap::OnInitDialog() {
  ...
  if (pSystemMaps != NULL) pSystemMaps->SetCheck(m_usingSystemDir);

  CButton *pUserMaps = (CButton *)this->GetDlgItem(IDC_USERMAPS);
  if (pUserMaps != NULL) pUserMaps->SetCheck(!m_usingSystemDir);

#if !defined(_DEBUG) && !defined(_INTERNAL)
  if (pSystemMaps) pSystemMaps->ShowWindow(FALSE);
  if (pUserMaps) pUserMaps->ShowWindow(FALSE);
#endif
