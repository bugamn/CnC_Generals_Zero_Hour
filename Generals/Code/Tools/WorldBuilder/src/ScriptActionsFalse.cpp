...
ScriptActionsFalse::ScriptActionsFalse()
    : CPropertyPage(ScriptActionsFalse::IDD), m_falseAction(NULL), m_index(0) {
...
void ScriptActionsFalse::OnDblclkActionList() { OnEditAction(); }

void ScriptActionsFalse::OnNew() {
  ScriptAction *pAct =
      newInstance(ScriptAction)(ScriptAction::DEBUG_MESSAGE_BOX);
  EditAction aDlg;
  aDlg.setAction(pAct);
...
Bool ScriptActionsFalse::doMoveDown() {
  if (m_falseAction && m_falseAction->getNext()) {
    ...
    if (pPrev) {
      ...
      DEBUG_ASSERTCRASH(m_falseAction == m_script->getFalseAction(),
                        ("Logic error."));
      pCur->setNextAction(pNext->getNext());
      pNext->setNextAction(pCur);
