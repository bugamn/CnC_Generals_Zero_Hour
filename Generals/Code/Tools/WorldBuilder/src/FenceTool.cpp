...
FenceTool::FenceTool(void)
    : Tool(ID_FENCE_TOOL, IDC_FENCE), m_mapObjectList(NULL), m_objectCount(1) {
...
void FenceTool::updateMapObjectList(Coord3D downPt, Coord3D curPt,
                                    WbView *pView, CWorldBuilderDoc *pDoc,
                                    Bool checkPlayers) {
  ...
  for (i = 1; i < m_objectCount; i++) {
    if (pCurObj->getNext() == NULL) {
      pCurObj->setNextMap(ObjectOptions::duplicateCurMapObjectForPlace(
          &downPt, angle, checkPlayers));
    }
...
void FenceTool::mouseDown(TTrackingMode m, CPoint viewPt, WbView *pView,
  ...
  if (FenceOptions::hasSelectedObject()) {
    ...
    m_mapObjectList =
        ObjectOptions::duplicateCurMapObjectForPlace(&m_downPt3d, 0, false);
    m_objectCount = 1;
...
void FenceTool::mouseMoved(TTrackingMode m, CPoint viewPt, WbView *pView,
                           CWorldBuilderDoc *pDoc) {
  ...
  MapObject *pCur = ObjectOptions::getObjectNamed(
      AsciiString(ObjectOptions::getCurObjectName()));
  p3View->setObjTracking(NULL, m_downPt3d, 0, false);
...
void FenceTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView *pView,
                        CWorldBuilderDoc *pDoc) {
  if (m != TRACK_L) return;
  Coord3D cpt;
  pView->viewToDocCoords(viewPt, &cpt);  // Do constrain.
