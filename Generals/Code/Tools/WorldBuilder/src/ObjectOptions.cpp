...
static Int findSideListEntryWithPlayerOfSide(AsciiString side) {
  for (int i = 0; i < TheSidesList->getNumSides(); i++) {
    AsciiString ptname =
        TheSidesList->getSideInfo(i)->getDict()->getAsciiString(
            TheKey_playerFaction);
    const PlayerTemplate *pt =
        ThePlayerTemplateStore->findPlayerTemplate(NAMEKEY(ptname));
...
/*static*/ void ObjectOptions::update() {
  if (m_staticThis) m_staticThis->updateLabel();
}
...
void ObjectOptions::updateLabel() {
  ...
  DEBUG_ASSERTCRASH(TheSidesList->getNumTeams() == 0 || neutral != -1,
                    ("must have a neutral"));
  if (sel == -1) {
    DEBUG_ASSERTCRASH(defTeamName.isEmpty(),
                      ("owning team not found, using neutral"));
    sel = neutral;
  }
  if (sel == -1) {
    DEBUG_ASSERTCRASH(!pCur || TheSidesList->getNumTeams() == 0,
                      ("hmm, should not happen"));
    m_curOwnerName.clear();
  } else {
    DEBUG_ASSERTCRASH(pCur, ("hmm, should not happen"));
    m_curOwnerName = TheSidesList->getTeamInfo(sel)->getDict()->getAsciiString(
        TheKey_teamName);
  }
  list->SetCurSel(sel);
}
...
BOOL ObjectOptions::OnInitDialog() {
  ...
  for (tTemplate = TheThingFactory->firstTemplate(); tTemplate;
    ...
    pMap = newInstance(MapObject)(loc, tTemplate->getName(), 0.0f, 0, NULL,
                                  tTemplate);
...
#ifdef LOAD_TEST_ASSETS
  {
    ...
    TheFileSystem->getFileListInDirectory(
        AsciiString(dirBuf), AsciiString("*.w3d"), filenameList, FALSE);
    ...
    if (filenameList.size() > 0) {
      ...
      do {
        ...
        Coord3D pt = {0, 0, 0};
        MapObject *pMap = newInstance(MapObject)(pt, AsciiString(fileBuf), 0.0f,
                                                 0, NULL, NULL);
        pMap->setNextMap(m_objectsList);
        m_objectsList = pMap;
...
HTREEITEM ObjectOptions::_FindOrDont(const char *pLabel, HTREEITEM startPoint) {
  ...
  while (itemsToEx.front()) {
    ...
    if (!m_objectTreeView.ItemHasChildren(hItem)) {
      ...
      // add the first child, the others will be caught by the adding of the
      // siblings
...
void ObjectOptions::addObject(MapObject *mapObject, const char *pPath,
  ...
  //
  // if we have an thing template in mapObject, we've read it from the new INI
  // database, we will sort those items into the tree based on properties of the
  // template that make it easier for us to browse when building levels
  //
  ...
  if (thingTemplate) {
    ...
    for (EditorSortingType i = ES_FIRST; i < ES_NUM_SORTING_TYPES;
    ...
    if (i == ES_NUM_SORTING_TYPES) parent = findOrAdd(parent, "UNSORTED");
...
MapObject *ObjectOptions::duplicateCurMapObjectForPlace(const Coord3D *loc,
  ...
  if (pCur) {
      ...
      if (!found && tt) {
        ...
        if (!found) {
          AddPlayerDialog addPlyr(
              pCur->getThingTemplate()->getDefaultOwningSide());
          if (addPlyr.DoModal() == IDOK) {
            for (int i = 0; i < TheSidesList->getNumSides(); i++) {
              AsciiString playerTmplName =
                  TheSidesList->getSideInfo(i)->getDict()->getAsciiString(
                      TheKey_playerFaction);
              if (playerTmplName == addPlyr.getAddedSide()) {
                m_curOwnerName.set("team");
                m_curOwnerName.concat(
                    TheSidesList->getSideInfo(i)->getDict()->getAsciiString(
                        TheKey_playerName));
                found = true;
                break;
    ...
    if (found) {
      MapObject *pNew = newInstance(MapObject)(
          *loc, pCur->getName(), angle, pCur->getFlags(), pCur->getProperties(),
...
void ObjectOptions::OnCloseupOwningteam() { OnEditchangeOwningteam(); }

void ObjectOptions::OnSelchangeOwningteam() { OnEditchangeOwningteam(); }
