...
void WaterTool::mouseDown(TTrackingMode m, CPoint viewPt, WbView *pView,
  ...
  if (m_poly_curSelectedPolygon) {
    DEBUG_ASSERTCRASH(m_poly_curSelectedPolygon->isWaterArea(),
                      ("Should be water."));
...
void WaterTool::setCursor(void) {
  if (m_poly_mouseUpPlus || (m_poly_isAdding && m_poly_curSelectedPolygon)) {
    if (m_poly_plusCursor == NULL) {
      m_poly_plusCursor =
          AfxGetApp()->LoadCursor(MAKEINTRESOURCE(IDC_WATER_PLUS));
    }
    ::SetCursor(m_poly_plusCursor);
  } else if (m_poly_mouseUpMove) {
    if (m_poly_moveCursor == NULL) {
      m_poly_moveCursor =
          AfxGetApp()->LoadCursor(MAKEINTRESOURCE(IDC_WATER_MOVE));
    }
...
void WaterTool::mouseMoved(TTrackingMode m, CPoint viewPt, WbView *pView,
                           CWorldBuilderDoc *pDoc) {
  Coord3D docPt;
  pView->viewToDocCoords(viewPt, &docPt);
  if (m == TRACK_NONE) {
...
/** Mouse up - not much. */
void WaterTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView *pView,
                        CWorldBuilderDoc *pDoc) {
  if (m != TRACK_L) return;
...
void WaterTool::fillTheArea(TTrackingMode m, CPoint viewPt, WbView *pView,
                            CWorldBuilderDoc *pDoc) {
  ...
  while (pointLimit > 0) {
    ...
    if (bottom) {
      bottom = false;
      if (waterHeight < mapZtoHeight(intMapHeight)) {
        DEBUG_ASSERTCRASH(waterHeight > mapZtoHeight(pMap->getHeight(i + 1, j)),
                          ("Logic error. jba."));
        Real dx =
            (mapZtoHeight(pMap->getHeight(i, j)) - waterHeight) /
            (mapZtoHeight(pMap->getHeight(i, j) - pMap->getHeight(i + 1, j)));
        pt.x += dx * MAP_XY_FACTOR;
        Bool topLeftDry =
            (waterHeight < mapZtoHeight(pMap->getHeight(i, j + 1)));
        Bool topRightDry =
            (waterHeight < mapZtoHeight(pMap->getHeight(i + 1, j + 1)));
        if (topRightDry) {
          i++;
          left = true;
        } else if (topLeftDry) {
      ...
      left = false;
      if (waterHeight < mapZtoHeight(pMap->getHeight(i, j + 1))) {
        DEBUG_ASSERTCRASH(waterHeight > mapZtoHeight(intMapHeight),
                          ("Logic error. jba."));
        Real dy =
            (waterHeight - mapZtoHeight(pMap->getHeight(i, j))) /
            (mapZtoHeight(pMap->getHeight(i, j + 1) - pMap->getHeight(i, j)));
        pt.y += dy * MAP_XY_FACTOR;
        Bool bottomRightDry =
            (waterHeight < mapZtoHeight(pMap->getHeight(i + 1, j)));
        Bool topRightDry =
            (waterHeight < mapZtoHeight(pMap->getHeight(i + 1, j + 1)));
        if (bottomRightDry) {
          j--;
      ...
      if (waterHeight < mapZtoHeight(pMap->getHeight(i + 1, j))) {
        DEBUG_ASSERTCRASH(
            waterHeight > mapZtoHeight(pMap->getHeight(i + 1, j + 1)),
            ("Logic error. jba."));
        Real dy = (mapZtoHeight(pMap->getHeight(i + 1, j)) - waterHeight) /
                  (mapZtoHeight(pMap->getHeight(i + 1, j) -
                                pMap->getHeight(i + 1, j + 1)));
        pt.y += dy * MAP_XY_FACTOR;
        Bool bottomLeftDry =
            (waterHeight < mapZtoHeight(pMap->getHeight(i, j)));
        Bool topLeftDry =
      ...
      pt.y = (j + 1) * MAP_XY_FACTOR;
      if (waterHeight < mapZtoHeight(pMap->getHeight(i + 1, j + 1))) {
        DEBUG_ASSERTCRASH(waterHeight > mapZtoHeight(pMap->getHeight(i, j + 1)),
                          ("Logic error. jba."));
        Real dx = (waterHeight - mapZtoHeight(pMap->getHeight(i, j + 1))) /
                  (mapZtoHeight(pMap->getHeight(i + 1, j + 1) -
                                pMap->getHeight(i, j + 1)));
        pt.x += dx * MAP_XY_FACTOR;
        Bool bottomLeftDry =
            (waterHeight < mapZtoHeight(pMap->getHeight(i, j)));
        Bool bottomRightDry =
