...
//
/// Constructor
BlendEdgeTool::BlendEdgeTool(void) : Tool(ID_BLEND_EDGE_TOOL, IDC_BLEND_EDGE) {}

/// Destructor
BlendEdgeTool::~BlendEdgeTool(void) {}

/** Execute the tool on mouse down - Place an object. */
void BlendEdgeTool::mouseDown(TTrackingMode m, CPoint viewPt, WbView *pView,
...
custom blends. */
void BlendEdgeTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView *pView,
                            CWorldBuilderDoc *pDoc) {
  CPoint from, to;
  ...
  if (!pDoc->getCellIndexFromCoord(m_downPt, &from) ||
      !pDoc->getCellIndexFromCoord(cpt, &to)) {
    return;
  }
  ...
  if (m == TRACK_L)
    htMapEditCopy->blendTile(to.x, to.y, from.x, from.y, -1,
                             -1);  // does all the work.
  else
    htMapEditCopy->blendTile(to.x, to.y, from.x, from.y,
                             TerrainMaterial::getFgTexClass(),
                             -1);  // does all the work.
  IRegion2D partialRange = {to.x, to.y, to.x + 1, to.y + 1};
  pDoc->updateHeightMap(htMapEditCopy, true,
                        partialRange);  // update the render object with new
                                        // tile and/or height data.
