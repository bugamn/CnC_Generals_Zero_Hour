...
BOOL EditAction::OnInitDialog() {
  ...
  m_myEditCtrl.Create(WS_CHILD | ES_MULTILINE, rect, this,
                      IDC_RICH_EDIT_HERE + 1);
  m_myEditCtrl.ShowWindow(SW_SHOW);
  m_myEditCtrl.SetEventMask(m_myEditCtrl.GetEventMask() | ENM_LINK |
                            ENM_SELCHANGE);
...
void EditAction::formatScriptActionText(Int parameterNdx) {
  ...
  for (i = 0; i < MAX_PARMS; i++) {
    ...
    if (i < m_action->getNumParameters()) {
      warningText.concat(
          EditParameter::getWarningText(m_action->getParameter(i)));
      informationText.concat(
          EditParameter::getInfoText(m_action->getParameter(i)));
...
BOOL EditAction::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT *pResult) {
  if (LOWORD(wParam) == IDC_RICH_EDIT_HERE + 1) {
    ...
    if (pHdr->hwndFrom == m_myEditCtrl.m_hWnd && pHdr->code == EN_LINK) {
        ...
        for (i = 0; i < MAX_PARMS; i++) {
          ...
          if (i < m_action->getNumParameters()) {
            ...
            if (curChar == chrg.cpMin && curChar + numChars == chrg.cpMax) {
              // Kris:
              // Before we edit the parameter, there is a new prerequisite for
              // parameters but only a few will ever care. We will store what we
              // perceive as the unit, and for simplicity, we'll store the first
              // occurrence of the unit, although this can change in the future
              // should the need arise.
              AsciiString unitName;
              for (int j = 0; j < MAX_PARMS; j++) {
                Parameter *parameter = m_action->getParameter(j);
                if (parameter &&
                    parameter->getParameterType() == Parameter::UNIT) {
                  unitName = parameter->getString();
                  break;
                }
              }

              if (EditParameter::edit(m_action->getParameter(i), unitName) ==
                  IDOK) {
                m_myEditCtrl.SetWindowText(m_action->getUiText().str());
      ...
      return true;
    } else if (pHdr->hwndFrom == m_myEditCtrl.m_hWnd &&
               pHdr->code == EN_SELCHANGE) {
      if (m_updating) {
        return true;
      }
      CHARRANGE curChrg;
      m_myEditCtrl.GetSel(curChrg);
      if (curChrg.cpMin == m_curLinkChrg.cpMin &&
          curChrg.cpMax == m_curLinkChrg.cpMax) {
        return true;
      }
      if (m_modifiedTextColor) {
        formatScriptActionText(-1);
...
/** Not actually a timer - just used to send a delayed message to self because
rich edit control is stupid.  jba. */
