...
BOOL ContourOptions::OnInitDialog() {
  ...
  m_contourStepSlider.Create(TBS_HORZ | TBS_BOTTOM | TBS_AUTOTICKS, rect, this,
                             IDC_SLIDER1);
  m_contourStepSlider.SetRange(MIN_CONTOUR_STEP, MAX_CONTOUR_STEP);
  m_contourStepSlider.SetTicFreq(1);
  m_contourStepSlider.SetPos(MIN_CONTOUR_STEP + MAX_CONTOUR_STEP -
                             m_contourStep);
  ...
  m_contourOffsetSlider.Create(TBS_HORZ | TBS_BOTTOM | TBS_AUTOTICKS, rect,
                               this, IDC_SLIDER2);
  ...
  m_contourWidthSlider.Create(TBS_HORZ | TBS_BOTTOM | TBS_AUTOTICKS, rect, this,
                              IDC_SLIDER3);
...
/** Gets the info, determines if it is the feather or width slider,
                ...
                and the brush tool. */
void ContourOptions::OnHScroll(UINT nSBCode, UINT nPos,
                               CScrollBar *pScrollBar) {
  CString str;
  m_updating = true;
  Bool step = (pScrollBar && pScrollBar->m_hWnd == m_contourStepSlider.m_hWnd);
  Bool offset =
      (pScrollBar && pScrollBar->m_hWnd == m_contourOffsetSlider.m_hWnd);
  if (step) {
  ...
  if (nSBCode != TB_THUMBTRACK) {
    CWorldBuilderView *pView = CWorldBuilderDoc::GetActive2DView();
    if (pView) pView->setCellSize(pView->getCellSize());
  }
  m_updating = false;
}
