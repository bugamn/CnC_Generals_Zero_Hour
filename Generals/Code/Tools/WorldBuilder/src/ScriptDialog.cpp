...
void CSDTreeCtrl::OnRButtonDown(UINT nFlags, CPoint point) {
  ...
  if (item) {
    ...
    if (sd->friend_getCurScript() != NULL) {
      ...
      pPopup->CheckMenuItem(
          ID_SCRIPTACTIVATE,
          MF_BYCOMMAND | (active ? MF_CHECKED : MF_UNCHECKED));
...
Script *ScriptDialog::friend_getCurScript(void) { return getCurScript(); }

ScriptGroup *ScriptDialog::friend_getCurGroup(void) { return getCurGroup(); }

Script *ScriptDialog::getCurScript(void) {
  if (m_curSelection.m_objType == ListType::SCRIPT_IN_PLAYER_TYPE ||
      m_curSelection.m_objType == ListType::SCRIPT_IN_GROUP_TYPE) {
    ScriptList *pSL =
        m_sides.getSideInfo(m_curSelection.m_playerIndex)->getScriptList();
    if (pSL) {
      Script *pScr = NULL;
      if (m_curSelection.m_objType == ListType::SCRIPT_IN_PLAYER_TYPE) {
        pScr = pSL->getScript();
...
/** Updates the warning flags in the scripts, script groups & script conditions
 * & actions. */
...
BOOL ScriptDialog::OnInitDialog() {
  ...
  top.left =
      ::AfxGetApp()->GetProfileInt(SCRIPT_DIALOG_SECTION, "Left", top.left);
  ...
  while (child != NULL) {
    ...
    if (lt.m_objType == ListType::PLAYER_TYPE) {
      if (updateIcons(child)) {
        pTree->SetItemState(child, INDEXTOSTATEIMAGEMASK(3),
                            TVIS_STATEIMAGEMASK);
      } else {
        pTree->SetItemState(child, INDEXTOSTATEIMAGEMASK(1),
                            TVIS_STATEIMAGEMASK);
      }
    }
    ...
    /// script
      ...
      if (pScr) {
        if (pScr->hasWarnings()) {
          pTree->SetItemState(child, INDEXTOSTATEIMAGEMASK(4),
                              TVIS_STATEIMAGEMASK);
          warnings = true;
        } else {
          setIconScript(child);
...
void ScriptDialog::addScriptList(HTREEITEM hPlayer, Int playerIndex,
                                 ScriptList *pSL) {
  CTreeCtrl *pTree = (CTreeCtrl *)GetDlgItem(IDC_SCRIPT_TREE);
  TVINSERTSTRUCT ins;
  Int groupNdx;
...
HTREEITEM ScriptDialog::findItem(ListType sel, Bool failSafe) {
  ...
  HTREEITEM script;
  for (script = pTree->GetChildItem(group); script != NULL;
       script = pTree->GetNextSiblingItem(script)) {
    ::memset(&item, 0, sizeof(item));
    item.mask = TVIF_HANDLE | TVIF_PARAM;
    item.hItem = script;
    pTree->GetItem(&item);
    ...
    DEBUG_ASSERTCRASH(lt.m_objType == ListType::SCRIPT_IN_PLAYER_TYPE ||
                          lt.m_objType == ListType::SCRIPT_IN_GROUP_TYPE,
                      ("Not script"));
...
void ScriptDialog::markWaypoint(MapObject *pObj) {
  ...
  if (!pObj) return;
  if (pObj->isWaypoint() &&
      !pObj->getProperties()->getBool(TheKey_exportWithScript, &exists)) {
...
void ScriptDialog::scanParmForWaypointsAndTeams(Parameter *pParm, Bool doUnits,
                                                Bool doWaypoints,
                                                Bool doTriggers) {
  ...
  if (pParm->getParameterType() == Parameter::WAYPOINT_PATH && doWaypoints) {
    ...
    for (pObj = MapObject::getFirstMapObject(); pObj; pObj = pObj->getNext()) {
      if (pObj->isWaypoint()) {
        Bool exists;
        if (waypointPathLabel == pObj->getProperties()->getAsciiString(
  ...
  if (pParm->getParameterType() == Parameter::TEAM) {
    ...
    if (doUnits) {
      ...
      for (pObj = MapObject::getFirstMapObject(); pObj;
           pObj = pObj->getNext()) {
        Bool exists;
        AsciiString objsTeamName = pObj->getProperties()->getAsciiString(
  ...
  if (pParm->getParameterType() == Parameter::UNIT) {
    ...
    if (doUnits) {
      ...
      for (pObj = MapObject::getFirstMapObject(); pObj;
        ...
        AsciiString objsTeamName = pObj->getProperties()->getAsciiString(
            TheKey_originalOwner, &exists);
        AsciiString objsUnitName =
            pObj->getProperties()->getAsciiString(TheKey_objectName, &exists);
  ...
  if (pParm->getParameterType() == Parameter::TRIGGER_AREA && doTriggers) {
    ...
    for (pTrig = PolygonTrigger::getFirstPolygonTrigger(); pTrig;
         pTrig = pTrig->getNext()) {
...
/** Looks for referenced waypoints & teams. */
void ScriptDialog::scanForWaypointsAndTeams(Script *pScript, Bool doUnits,
                                            Bool doWaypoints, Bool doTriggers) {
  pScript->setWarnings(false);
  OrCondition *pOr;
  for (pOr = pScript->getOrCondition(); pOr; pOr = pOr->getNextOrCondition()) {
    Condition *pCondition;
    for (pCondition = pOr->getFirstAndCondition(); pCondition;
      ...
      Int i;
      for (i = 0; i < pCondition->getNumParameters(); i++) {
        scanParmForWaypointsAndTeams(pCondition->getParameter(i), doUnits,
                                     doWaypoints, doTriggers);
      }
    }
  }
  ScriptAction *pAction;
  for (pAction = pScript->getAction(); pAction; pAction = pAction->getNext()) {
    ...
    Int i;
    for (i = 0; i < pAction->getNumParameters(); i++) {
      scanParmForWaypointsAndTeams(pAction->getParameter(i), doUnits,
                                   doWaypoints, doTriggers);
    }
  }
}
...
void ScriptDialog::OnSave() {
  ...
  CFileDialog fileDlg(false, ".scb", NULL,
  ...
  for (i = 0; i < m_sides.getNumTeams(); i++) {
    m_sides.getTeamInfo(i)->getDict()->setBool(TheKey_exportWithScript,
                                               doAllScripts);
  }
  ...
  CFile theFile(path, CFile::modeCreate | CFile::modeWrite |
  ...
  try {
    ...
    /***************Players DATA ***************/
    chunkWriter.openDataChunk("ScriptsPlayers",
                              K_PLAYERS_NAMES_FOR_SCRIPTS_VERSION_1);
    if (doAllScripts) {
      chunkWriter.writeInt(m_sides.getNumSides());
      for (i = 0; i < m_sides.getNumSides(); i++) {
        AsciiString name = m_sides.getSideInfo(i)->getDict()->getAsciiString(
            TheKey_playerName);
...
void ScriptDialog::OnLoad() {
  CFileDialog fileDlg(true, ".scb", NULL, 0, "Script files (.scb)|*.scb||",
                      this);
  ...
  if (theInputStream.open(AsciiString(path))) try {
      ...
      file.registerParser(AsciiString("WaypointsList"),
                          AsciiString::TheEmptyString, ParseWaypointDataChunk);
      file.registerParser(AsciiString("ScriptTeams"),
                          AsciiString::TheEmptyString, ParseTeamsDataChunk);
      file.registerParser(AsciiString("ScriptsPlayers"),
                          AsciiString::TheEmptyString, ParsePlayersDataChunk);
      if (!file.parse(this)) {
      ...
      if (m_firstReadObject) {
        AddObjectUndoable *pUndo =
            new AddObjectUndoable(pDoc, m_firstReadObject);
        pDoc->AddAndDoUndoable(pUndo);
        REF_PTR_RELEASE(pUndo);    // belongs to pDoc now.
      ...
      Int i;
      for (i = 0; i < count; i++) {
        if (scripts[i]->getScript() == NULL &&
            scripts[i]->getScriptGroup() == NULL)
          continue;
        Int curSide = -1;
        if (count == 1) {
          ...
          for (j = 0; j < m_sides.getNumSides(); j++) {
            AsciiString name =
                m_sides.getSideInfo(i)->getDict()->getAsciiString(
                    TheKey_playerName);
        ...
        if (curSide >= m_sides.getNumSides()) {
          curSide = 0;
          ::AfxMessageBox(
              "Imported scripts came from more players than exist in this map. "
              " Additional scripts moved to Neutral player.");
        }
        ...
        for (pGroup = scripts[i]->getScriptGroup(); pGroup;
             pGroup = pNextGroup) {
        ...
        scripts[i]
            ->discard(); /* Frees the script list, but none of it's children, as
                            they have been copied into the current scripts. */
        scripts[i] = NULL;
        if (pSL) {
...
Bool ScriptDialog::ParseObjectDataChunk(DataChunkInput &file,
  ...
  if (pThisOne->getProperties()->getType(TheKey_waypointID) == Dict::DICT_INT) {
    ...
    if (pThis->m_maxWaypoint < pThisOne->getWaypointID())
      pThis->m_maxWaypoint = pThisOne->getWaypointID();
  }

  DEBUG_LOG(
      ("Adding object %s (%s)\n", name.str(),
       pThisOne->getProperties()->getAsciiString(TheKey_originalOwner).str()));
  ...
  for (pObj = MapObject::getFirstMapObject(); pObj; pObj = pObj->getNext()) {
    ...
    // If the locations match, and they are both waypoints or both not
    // waypoints, and the names match, They're duplicate.
    if (locsMatch && (pObj->isWaypoint() == pThisOne->isWaypoint()) &&
        (pObj->getName() == pThisOne->getName())) {
      duplicate = true;
    }
    if (pThisOne->isWaypoint() && pObj->isWaypoint()) {
      // If both waypoints, and the names match, are dupes.
      if (!duplicate &&
          (pThisOne->getWaypointName() == pObj->getWaypointName())) {
        AsciiString warning;
        warning.format(
            "Duplicate named waypoints '%s', renaming imported waypoint.",
            pThisOne->getWaypointName().str());
        ::AfxMessageBox(warning.str(), MB_OK);
        AsciiString name =
            WaypointOptions::GenerateUniqueName(pThisOne->getWaypointID());
        name.concat("-imp");
      }
    }
    if (duplicate) break;
  ...
  if (pPrevious) {
    DEBUG_ASSERTCRASH(
        pThis->m_firstReadObject != NULL && pPrevious->getNext() == NULL,
        ("Bad linkage."));
...
Bool ScriptDialog::ParseWaypointDataChunk(DataChunkInput &file,
  ...
  for (i = 0; i < count; i++) {
    ...
    pDoc->addWaypointLink(waypoint1 + pThis->m_waypointBase,
                          waypoint2 + pThis->m_waypointBase);
    if (pThis->m_maxWaypoint < waypoint1 + pThis->m_waypointBase)
      pThis->m_maxWaypoint = waypoint1 + pThis->m_waypointBase;
    if (pThis->m_maxWaypoint < waypoint2 + pThis->m_waypointBase)
      pThis->m_maxWaypoint = waypoint1 + pThis->m_waypointBase;
  }
...
Bool ScriptDialog::ParseTeamsDataChunk(DataChunkInput &file,
  ...
  while (!file.atEndOfChunk()) {
    ...
    if (pThis->m_sides.findSideInfo(player)) {
      ...
      AsciiString warning;
      warning.format(
          "Importing team %s of player %s.  Player %s doesn't exist, Select "
          "player..",
...
Bool ScriptDialog::ParsePolygonTriggersDataChunk(DataChunkInput &file,
  ...
  while (count > 0) {
    ...
    for (pCurrentTrigger = PolygonTrigger::getFirstPolygonTrigger();
         pCurrentTrigger; pCurrentTrigger = pCurrentTrigger->getNext()) {
      if (triggerName == pCurrentTrigger->getTriggerName()) {
        duplicate = true;
        AsciiString warning;
        warning.format("Duplicated trigger named '%s' discarded.",
                       triggerName.str());
        ::AfxMessageBox(warning.str(), MB_OK);
        break;
      }
...
void ScriptDialog::OnCancel() { CDialog::OnCancel(); }
...
void ScriptDialog::doDropOn(HTREEITEM hDrag, HTREEITEM hTarget) {
  ...
  // If we are dragging a group onto a script, adjust the group index so we add
  // after.
  ...
  if (target.m_playerIndex != drag.m_playerIndex) {
    reloadPlayer(drag.m_playerIndex,
                 m_sides.getSideInfo(drag.m_playerIndex)->getScriptList());
  }
...
void ScriptDialog::OnMove(int x, int y) {
  ...
  if (this->IsWindowVisible() && !this->IsIconic()) {
    ...
    ::AfxGetApp()->WriteProfileInt(SCRIPT_DIALOG_SECTION, "Left",
                                   frameRect.left);
  }
}
...
void ScriptDialog::OnScriptActivate() {
  ...
  if (getCurScript() != NULL) {
    ...
    if (getCurScript()->hasWarnings()) {
      ...
      if (getCurScript()->isActive())
        pTree->SetItemState(item, INDEXTOSTATEIMAGEMASK(2),
                            TVIS_STATEIMAGEMASK);
      else
        pTree->SetItemState(item, INDEXTOSTATEIMAGEMASK(6),
                            TVIS_STATEIMAGEMASK);
    }
    ...
    if (getCurGroup()->hasWarnings()) {
      ...
      if (getCurGroup()->isActive())
        pTree->SetItemState(item, INDEXTOSTATEIMAGEMASK(1),
                            TVIS_STATEIMAGEMASK);
      else
        pTree->SetItemState(item, INDEXTOSTATEIMAGEMASK(5),
                            TVIS_STATEIMAGEMASK);
    }
  }
}
