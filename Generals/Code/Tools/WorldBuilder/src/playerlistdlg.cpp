...
static void fixDefaultTeamName(SidesList &sides, AsciiString oldpname,
  ...
  if (ti) {
    ...
    DEBUG_LOG(("rename team %s -> %s\n",
               ti->getDict()->getAsciiString(TheKey_teamName).str(),
               tname.str()));
...
static void updateAllTeams(SidesList &sides, AsciiString oldpname,
                           AsciiString newpname) {
...
static void ensureValidPlayerName(Dict *d) {
  ...
  for (char *p = buf; *p; ++p)
    if (!islegalplayernamechar(*p)) *p = '_';
  d->setAsciiString(TheKey_playerName, AsciiString(buf));
}

static AsciiString playerNameForUI(SidesList &sides, int i) {
  AsciiString b =
      sides.getSideInfo(i)->getDict()->getAsciiString(TheKey_playerName);
  if (b.isEmpty()) b = NEUTRAL_NAME_STR;
  return b;
}
...
static Bool containsToken(const AsciiString &cur_allies,
                          const AsciiString &tokenIn) {
  AsciiString name, token;

  name = cur_allies;
  while (name.nextToken(&token)) {
    if (token == tokenIn) return true;
  }
  return false;
}

static AsciiString removeDupsFromEnemies(const AsciiString &cur_allies,
                                         const AsciiString &cur_enemies) {
  AsciiString new_enemies, tmp, token;

  tmp = cur_enemies;
  while (tmp.nextToken(&token)) {
    if (containsToken(cur_allies, token)) continue;
    if (!new_enemies.isEmpty()) new_enemies.concat(" ");
    new_enemies.concat(token);
  }
  return new_enemies;
}

static AsciiString extractFromAlliesList(CListBox *alliesList,
                                         SidesList &sides) {
  char buffer[1024];
  AsciiString allies;
...
static void buildAlliesList(CListBox *alliesList, SidesList &sides,
  ...
  for (i = 0; i < sides.getNumSides(); i++) {
    ...
    if (name == omitPlayer || name.isEmpty()) continue;
    name = playerNameForUI(sides, i);
    alliesList->AddString(name.str());
  }
}

static void selectAlliesList(CListBox *alliesList, SidesList &sides,
                             const AsciiString &cur_allies) {
  Int oindex_in_list;
  AsciiString name, token, oname;

  if (extractFromAlliesList(alliesList, sides) == cur_allies) return;
  ...
  while (name.nextToken(&token)) {
    ...
    oindex_in_list =
        alliesList->FindStringExact(-1, playerNameForUI(sides, i).str());
    if (oindex_in_list == -1) {
...
// returns a str describing how t1 considers t2. (implies nothing about how t2
// considers t1)
static const char *calcRelationStr(SidesList &sides, int t1, int t2) {
  ...
  if (containsToken(ti1->getDict()->getAsciiString(TheKey_playerAllies),
                    t2name))
    return allied;
  else if (containsToken(ti1->getDict()->getAsciiString(TheKey_playerEnemies),
                         t2name))
    return enemies;
...
void PlayerListDlg::OnNewplayer() {
  if (m_sides.getNumSides() >=
      MAX_PLAYER_COUNT - 1)  /// Added -1 so we can always have an observer even
                             /// for Single player games.
    return;

  AddPlayerDialog addPlyr("");
  if (addPlyr.DoModal() != IDOK) return;
...
void PlayerListDlg::OnEditplayer() {
  // TODO: the dialog referenced here has no ok or cancel buttons, so locks the
  // editor re-enable this routine once it is not a guaranteed hang
  ...
  if (editor.DoModal() == IDOK) {
    ...
    if (playerDict->getAsciiString(TheKey_playerName) !=
        playerDictCopy.getAsciiString(TheKey_playerName)) {
      AsciiString tname;
      tname.set("team");
...
void PlayerListDlg::OnRemoveplayer() {
  ...
  for (i = 0; i < m_sides.getNumTeams(); i++) {
    ...
    if (tdict->getAsciiString(TheKey_teamOwner) == pname) {
      count += MapObject::countMapObjectsWithOwner(
          tdict->getAsciiString(TheKey_teamName));
    }
  }
  ...
  if (m_sides.getNumSides() <= 1) return;
...
void PlayerListDlg::updateTheUI(void) {
  ...
  Bool modified = m_sides.validateSides();
  DEBUG_ASSERTLOG(!modified,
                  ("had to clean up sides in PlayerListDlg::updateTheUI! "
  ...
  // update factions popup
  {
    ...
    if (ThePlayerTemplateStore) {
      for (i = 0; i < ThePlayerTemplateStore->getPlayerTemplateCount(); i++) {
        AsciiString nm =
            ThePlayerTemplateStore->getNthPlayerTemplate(i)->getName();
        factions->AddString(nm.str());
      }
    }
    i = factions->FindStringExact(
        -1, pdict->getAsciiString(TheKey_playerFaction).str());
  ...
  AsciiString cur_allies = m_sides.getSideInfo(m_curPlayerIdx)
                               ->getDict()
                               ->getAsciiString(TheKey_playerAllies);
  ...
  AsciiString cur_enemies = m_sides.getSideInfo(m_curPlayerIdx)
                                ->getDict()
  ...
  if (newbtn) newbtn->EnableWindow(m_sides.getNumSides() < MAX_PLAYER_COUNT);
  if (editbtn) editbtn->EnableWindow(!isneutral);
  if (rmvbtn) rmvbtn->EnableWindow(m_sides.getNumSides() > 1 && !isneutral);
...
void PlayerListDlg::OnDblclkPlayers() { OnEditplayer(); }
...
void PlayerListDlg::OnSelchangeAllieslist() {
  ...
  m_sides.getSideInfo(m_curPlayerIdx)
      ->getDict()
      ->setAsciiString(TheKey_playerAllies, allies);
  m_sides.getSideInfo(m_curPlayerIdx)
      ->getDict()
      ->setAsciiString(TheKey_playerEnemies, enemies);

  updateTheUI();
}

void PlayerListDlg::OnSelchangeEnemieslist() { OnSelchangeAllieslist(); }
...
void PlayerListDlg::OnEditchangePlayerfaction() {
  ...
  if (faction) {
    // get the text out of the combo. If it is user-typed, sel will be -1,
...
void PlayerListDlg::OnChangePlayername() {
  ...
  if (pnameold == pnamenew) return;  // hmm, no change, so just punt.
