...
void MoundTool::mouseDown(TTrackingMode m, CPoint viewPt, WbView *pView,
...
void MoundTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView *pView,
                        CWorldBuilderDoc *pDoc) {
  if (m != TRACK_L) return;
...
void MoundTool::mouseMoved(TTrackingMode m, CPoint viewPt, WbView *pView,
                           CWorldBuilderDoc *pDoc) {
  Coord3D cpt;
  ...
  for (i = ndx.x - sub; i < ndx.x + add; i++) {
    ...
    for (j = ndx.y - sub; j < ndx.y + add; j++) {
      ...
      float fNewHeight = (blendFactor * (htDelta + curHeight)) +
                         ((1.0f - blendFactor) * curHeight);
      ...
      if (delta < m_brushWidth) {
        ...
        float fNewHeight = ((factor - feather) * (htDelta + curHeight) +
                            (feather * curHeight)) /
                           factor;
