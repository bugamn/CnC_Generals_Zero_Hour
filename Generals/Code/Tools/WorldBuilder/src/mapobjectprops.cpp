...
void MapObjectProps::makeMain() {
  ...
  if (TheMapObjectProps == NULL) TheMapObjectProps = this;
}

MapObjectProps::MapObjectProps(Dict* dictToEdit, const char* title,
                               CWnd* pParent /*=NULL*/)
...
MapObjectProps::~MapObjectProps() {
  if (TheMapObjectProps == this) TheMapObjectProps = NULL;
}
...
ON_EN_KILLFOCUS(IDC_MAPOBJECT_ShroudClearingDistance,
...
int MapObjectProps::getSel() {
  ...
  //	if (!m_dictToEdit || sel == LB_ERR || sel < 0 || sel >=
  //m_dictToEdit->getPairCount())
  return -1;
  //	return sel;
}
  ...
  if (propDlg.DoModal() == IDOK) {
    ...
    if (this == TheMapObjectProps) {
      ...
      DictItemUndoable* pUndo =
          new DictItemUndoable(&m_dictToEdit, newDict, newDict.getNthKey(0));
...
void MapObjectProps::OnNewprop() {
  ...
  if (propDlg.DoModal() == IDOK) {
    Dict newDict =
        DictItemUndoable::buildSingleItemDict(key, lastNewType, value);
    if (this == TheMapObjectProps) {
      // it's the floating panel; use an undoable
      CWorldBuilderDoc* pDoc = CWorldBuilderDoc::GetActiveDoc();
      DictItemUndoable* pUndo =
...
void MapObjectProps::OnDblclkProperties() { OnEditprop(); }
...
void MapObjectProps::_DictToTeam(void) {
  ...
  if (m_dictToEdit) {
    ...
    DEBUG_ASSERTLOG(i >= 0,
                    ("missing team '%s'. Non-fatal (jkmcd)\n", name.str()));
  }
  owner->SetCurSel(i);
}
...
void MapObjectProps::_DictToScript(void) {
  ...
  AsciiString script =
      m_dictToEdit->getAsciiString(TheKey_objectScriptAttachment, &exists);
...
void MapObjectProps::_DictToPrebuiltUpgrades(void) {
  ...
  // Then, if there's multiple units selected, add the Single Selection Only
  // string
  ...
  if (mi.getCount() == 0) {
    ...
    for (int i = 0; i < numBehaviorModules; ++i) {
      if (mi.getNthName(i).compareNoCase("GenerateMinefieldBehavior") == 0) {
        const GenerateMinefieldBehaviorModuleData* gmbmd =
            (const GenerateMinefieldBehaviorModuleData*)mi.getNthData(i);
        if (!gmbmd) {
          continue;
        }
  ...
  // Finally, walk through the upgrades that he already has, and select the
  // appropriate members from the list
  ...
  do {
    ...
    if (exists) {
      ...
      if (selNdx == LB_ERR) {
        DEBUG_CRASH(
            ("Object claims '%s', but it wasn't found in the list of possible "
             "upgrades.",
             upgradeString.str()));
        ++upgradeNum;
        continue;
...
void MapObjectProps::_TeamToDict(void) {
  ...
  DictItemUndoable* pUndo = new DictItemUndoable(
      m_allSelectedDicts.begin(), newDict, newDict.getNthKey(0),
...
void MapObjectProps::_NameToDict(void) {
  ...
  DictItemUndoable* pUndo =
      new DictItemUndoable(m_allSelectedDicts.begin(), newDict,
                           newDict.getNthKey(0), m_allSelectedDicts.size());
  pDoc->AddAndDoUndoable(pUndo);
  REF_PTR_RELEASE(pUndo);  // belongs to pDoc now.
                           // Update is called by Do
}
...
void MapObjectProps::_PrebuiltUpgradesToDict(void) {
  ...
  for (Int i = 0; i < countOfItems; ++i) {
    if (pBox->GetSel(i) > 0) {
      ...
      keyName.format(
          "%s%d",
          TheNameKeyGenerator->keyToName(TheKey_objectGrantUpgrade).str(),
          upgradeNum);
      newDict.setAsciiString(NAMEKEY(keyName),
                             AsciiString(selTxt.GetBuffer(0)));
      ++upgradeNum;
    }
  }
  ...
  CWorldBuilderDoc* pDoc = CWorldBuilderDoc::GetActiveDoc();
  DictItemUndoable* pUndo =
      new DictItemUndoable(m_allSelectedDicts.begin(), newDict, NAMEKEY_INVALID,
                           m_allSelectedDicts.size(), pDoc, true);
  pDoc->AddAndDoUndoable(pUndo);
  REF_PTR_RELEASE(pUndo);  // belongs to pDoc now.
}
...
void MapObjectProps::_VisibilityToDict(void) {
  ...
  DictItemUndoable* pUndo =
      new DictItemUndoable(m_allSelectedDicts.begin(), newDict,
                           TheKey_objectVisualRange, m_allSelectedDicts.size());
  pDoc->AddAndDoUndoable(pUndo);
  REF_PTR_RELEASE(pUndo);  // belongs to pDoc now.
                           // Update is called by Do
}
...
void MapObjectProps::_ShroudClearingDistanceToDict(void) {
  ...
  DictItemUndoable* pUndo = new DictItemUndoable(
      m_allSelectedDicts.begin(), newDict, TheKey_objectShroudClearingDistance,
...
void MapObjectProps::GetPopSliderInfo(const long sliderID, long* pMin,
