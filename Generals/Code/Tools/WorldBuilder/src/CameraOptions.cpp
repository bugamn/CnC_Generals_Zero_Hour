...
void CameraOptions::OnMove(int x, int y) {
  ...
  if (this->IsWindowVisible() && !this->IsIconic()) {
    ...
    ::AfxGetApp()->WriteProfileInt(CAMERA_OPTIONS_PANEL_SECTION, "Top",
                                   frameRect.top);
    ::AfxGetApp()->WriteProfileInt(CAMERA_OPTIONS_PANEL_SECTION, "Left",
                                   frameRect.left);
  }
}
...
void CameraOptions::stuffValuesIntoFields(void) {
  ...
  if (p3View) {
    ...
    s.format("(%g, %g)", target.X - 1.0f * (source.X - target.X),
             target.Y - 1.0f * (source.Y - target.Y));
    putAsciiString(IDC_TARGETTEXT, s);
  }
}

void CameraOptions::update(void) { stuffValuesIntoFields(); }
...
void CameraOptions::GetPopSliderInfo(const long sliderID, long *pMin,
