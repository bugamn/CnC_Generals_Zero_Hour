...
Bool RoadOptions::m_tightCurve =
    false;                           ///< Broad curve false, tight curve true.
...
RoadOptions::~RoadOptions(void) { m_currentRoadName.clear(); }
...
BOOL RoadOptions::OnInitDialog() {
...
#ifdef LOAD_TEST_ASSETS
  {
    ...
    TheFileSystem->getFileListInDirectory(
        AsciiString(findBuf), AsciiString("*.tga"), filenameList, FALSE);
  ...
  for (bridge = TheTerrainRoads->firstBridge(); bridge;
...
void RoadOptions::SelectConnected(void) {
  ...
  for (MapObject *pMapObj = MapObject::getFirstMapObject(); pMapObj;
    ...
    if (pMapObj->getFlag(FLAG_ROAD_POINT1)) {
      if (pMapObj->isSelected() ||
          pMapObj->getNext() && pMapObj->getNext()->isSelected()) {
  ...
  while (changed) {
    changed = false;
    for (std::list<MapObject *>::iterator it = roadSegs.begin();
         it != roadSegs.end(); ++it) {
      MapObject *o = *it;
      const Coord3D *oLoc = o->getLocation();
      const Coord3D *onLoc = o->getNext()->getLocation();
      for (std::list<MapObject *>::iterator connected = connectedSegs.begin();
           connected != connectedSegs.end(); ++connected) {
        ...
        if (qd1 < MAP_XY_FACTOR / 100 || qd2 < MAP_XY_FACTOR / 100 ||
            qd3 < MAP_XY_FACTOR / 100 || qd4 < MAP_XY_FACTOR / 100) {
