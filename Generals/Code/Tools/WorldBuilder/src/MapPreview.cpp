...
//
//	purpose:	Contains the code used to generate and save the map
//preview to 						the map file.  (Original code was taken from the Radar code in 						game
//engine).
//
//-----------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////
...
void MapPreview::interpolateColorForHeight(RGBColor *color, Real height,
...
Bool MapPreview::mapPreviewToWorld(const ICoord2D *radar, Coord3D *world) {
  ...
  // more sanity
  if (x < 0) x = 0;
  if (x >= MAP_PREVIEW_WIDTH) x = MAP_PREVIEW_WIDTH - 1;
  if (y < 0) y = 0;
  if (y >= MAP_PREVIEW_HEIGHT) y = MAP_PREVIEW_HEIGHT - 1;
  CWorldBuilderDoc *pDoc = CWorldBuilderDoc::GetActiveDoc();
  WorldHeightMapEdit *pMap = pDoc->GetHeightMap();
  Real xSample, ySample;

  xSample = INT_TO_REAL(pMap->getXExtent() - (2 * pMap->getBorderSize())) /
            MAP_PREVIEW_WIDTH;
  ySample = INT_TO_REAL(pMap->getYExtent() - (2 * pMap->getBorderSize())) /
            MAP_PREVIEW_HEIGHT;
  // translate to world
...
void MapPreview::buildMapPreviewTexture(CString tgaName) {
  ...
  for (i = 0; i < MAP_PREVIEW_HEIGHT; ++i) {
    for (j = 0; j < MAP_PREVIEW_WIDTH; ++j) {
      ...
      if (tempHeight < minHeight) minHeight = tempHeight;
      ++terrainCount;
    }
  }
  ...
  for (y = 0; y < MAP_PREVIEW_HEIGHT; y++) {
    for (x = 0; x < MAP_PREVIEW_WIDTH; x++) {
      ...
      if (localIsUnderwater(
              MAP_XY_FACTOR * (worldPoint.x - pMap->getBorderSize()),
            ...
            for (i = x - waterSamplesAway; i <= x + waterSamplesAway; i++) {
              if (i >= 0 && i < MAP_PREVIEW_WIDTH) {
                ...
                if (localIsUnderwater(
                        MAP_XY_FACTOR * (worldPoint.x - pMap->getBorderSize()),
                        MAP_XY_FACTOR *
                            (worldPoint.y - pMap->getBorderSize()))) {
                  // this is our "color" for water
                  color = waterColor;

                  // interpolate the water color for height in the water table
                  interpolateColorForHeight(
                      &color, underwaterZ, pMap->getMaxHeightValue(),
                      getTerrainAverageZ, pMap->getMinHeightValue());
          ...
          if (j >= 0 && j < MAP_PREVIEW_HEIGHT) {
            for (i = x - samplesAway; i <= x + samplesAway; i++) {
              if (i >= 0 && i < MAP_PREVIEW_WIDTH) {
                ...
                pMap->getTerrainColorAt(
                    MAP_XY_FACTOR * (worldPoint.x - pMap->getBorderSize()),
                    MAP_XY_FACTOR * (worldPoint.y - pMap->getBorderSize()),
                    &color);

                // interpolate the color for height
                interpolateColorForHeight(&color, z, maxHeight,
                                          getTerrainAverageZ, minHeight);
                // pMap->getMaxHeightValue(),getTerrainAverageZ,
                // pMap->getMinHeightValue() );
      ...
      // well, expensive.
      //
      m_pixelBuffer[y][x] = 255 | (REAL_TO_INT(color.red * 255) << 8) |
