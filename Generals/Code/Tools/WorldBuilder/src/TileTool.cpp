...
/// Constructor
TileTool::TileTool(void) : Tool(ID_TILE_TOOL, IDC_TILE_CURSOR) {
  m_htMapEditCopy = NULL;
}

/// Destructor
TileTool::~TileTool(void) { REF_PTR_RELEASE(m_htMapEditCopy); }
...
void TileTool::mouseDown(TTrackingMode m, CPoint viewPt, WbView *pView,
...
void TileTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView *pView,
                       CWorldBuilderDoc *pDoc) {
...
void TileTool::mouseMoved(TTrackingMode m, CPoint viewPt, WbView *pView,
                          CWorldBuilderDoc *pDoc) {
    ...
    for (i = minX; i < minX + width; i++) {
      ...
      for (j = minY; j < minY + width; j++) {
        ...
        if (TerrainMaterial::isPaintingPathingInfo()) {
          m_htMapEditCopy->setCliff(i, j,
                                    !TerrainMaterial::isPaintingPassable());
        } else {
          if (m_htMapEditCopy->setTileNdx(i, j, m_textureClassToDraw,
                                          width == 1)) {
            fullUpdate = true;
          }
        }
...
/// Constructor
BigTileTool::BigTileTool(void) { m_toolID = ID_BIG_TILE_TOOL; }
