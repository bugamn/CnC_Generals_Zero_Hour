...
void CLLTreeCtrl::OnRButtonDown(UINT nFlags, CPoint point) {
  ...
  buildMoveMenu(moveViewSelectionMenu,
                ID_LAYERSLIST_MERGEVIEWSELECTIONINTO_BEGIN);
  ...
  if (item) {
    if (contextIsLayer) {
      ...
      pPopup->CheckMenuItem(
          ID_HIDECURRENTLAYER,
          MF_BYCOMMAND | (hidden ? MF_CHECKED : MF_UNCHECKED));
...
LayersList::LayersList(UINT nIDTemplate, CWnd *parentWnd)
    : CDialog(nIDTemplate, parentWnd) {
  mTree = NULL;
  resetLayers();
}
...
void LayersList::addMapObjectToLayersList(MapObject *objToAdd,
                                          AsciiString layerToAddTo) {
  if (!objToAdd || findMapObjectAndList(objToAdd)) {
    DEBUG_CRASH(
        ("MapObject added was NULL or object already in Layers List. jkmcd"));
    return;
  }
  ListLayerIt layerIt;
  ...
  if (!findLayerNamed(layerToAddTo, &layerIt)) {
    ...
    if (!findLayerNamed(layerToAddTo, &layerIt)) {
      // if we still can't find that layer name, then try adding it to the
      // default layer.
...
void LayersList::changeMapObjectLayer(MapObject *objToChange,
...
void LayersList::removeLayerNamed(IN AsciiString layerToRemove) {
  ...
  for (ListMapObjectPtrIt obj = layerIt->objectsInLayer.begin();
       obj != layerIt->objectsInLayer.end();
       obj = layerIt->objectsInLayer.begin()) {
...
void LayersList::mergeLayerInto(IN ListLayerIt src, IN ListLayerIt dst) {
  ...
  for (mopIt = src->objectsInLayer.begin(); mopIt != src->objectsInLayer.end();
       mopIt = src->objectsInLayer.begin()) {
...
void LayersList::updateUIFromList(void) {
  ...
  for (ListLayerIt layersIt = mLayers.begin(); layersIt != mLayers.end();
       ++layersIt) {
    // Add a branch for each Layer.
    int iconToShow = (layersIt->show ? 0 : 1);
    HTREEITEM thisBranch =
        pTree->InsertItem(layersIt->layerName.str(), iconToShow, iconToShow);

    for (ListMapObjectPtrIt objIt = layersIt->objectsInLayer.begin();
         objIt != layersIt->objectsInLayer.end(); ++objIt) {
      Bool exists;
      AsciiString uniqueID =
          (*objIt)->getProperties()->getAsciiString(TheKey_uniqueID, &exists);
      if (exists) {
        pTree->InsertItem(uniqueID.str(), iconToShow, iconToShow, thisBranch);
      } else {
        pTree->InsertItem((*objIt)->getName().str(), iconToShow, iconToShow,
                          thisBranch);
      }
    }
  }
...
Bool LayersList::findMapObjectAndList(IN MapObject *objectToFind,
                                      OUT ListLayerIt *layerIt,
                                      OUT ListMapObjectPtrIt *objectIt) {
  if (!objectToFind) {
    return false;
  }
  ...
  for (ListLayerIt layersIt = mLayers.begin(); layersIt != mLayers.end();
       ++layersIt) {
    for (ListMapObjectPtrIt objPtrIt = layersIt->objectsInLayer.begin();
         objPtrIt != layersIt->objectsInLayer.end(); ++objPtrIt) {
  ...
  // if we get here, it means we didn't find the object anywhere in any of the
  // layers
  return false;
}
...
void LayersList::addMapObjectToLayer(IN MapObject *objectToAdd,
                                     IN ListLayerIt *layerIt) {
  if (!objectToAdd) {
    DEBUG_CRASH(
        ("No object to add in addMapObjectToLayer. This should not happen. "
         "jkmcd"));
    return;
  }
  ...
  if (layerIt) {
    ...
    if (!findLayerNamed(AsciiString(TheDefaultLayerName.c_str()),
                        &layerToAddTo)) {
  ...
  if (layerToAddTo->layerName.compareNoCase(
          AsciiString(TheDefaultLayerName.c_str())) == 0) {
...
void LayersList::removeMapObjectFromLayer(IN MapObject *objectToRemove,
                                          IN ListLayerIt *layerIt,
                                          IN ListMapObjectPtrIt *objectIt) {
  if (!objectToRemove) {
  ...
  if (layerIt && objectIt) {
    ...
    if (!findMapObjectAndList(objectToRemove, &layerToRemoveFrom,
                              &objectBeingRemove)) {
      DEBUG_CRASH(
          ("Couldn't find the object anywhere. Why did we try to remove it? "
           "jkmcd"));
      return;
    }
  }
  ...
  if (layer) {
    Bool dontcare;
    AsciiString objectUID = (*objectBeingRemove)
                                ->getProperties()
                                ->getAsciiString(TheKey_uniqueID, &dontcare);
  ...
  // don't need to update the layer in the object because only 1 of 2 things can
  // be happening:
  // 1. We are about to delete the object
  // 2. The object is about to be moved to the default layer (and will be
  // updated anyways.)
...
void LayersList::OnEndEditLabel(NMHDR *pNotifyStruct, LRESULT *pResult) {
  ...
  if (mCurrentlyEditingLabel.compareNoCase(
          AsciiString(TheDefaultLayerName.c_str())) == 0) {
...
void LayersList::OnDeleteLayer() {
  ...
  if (asciiCatToDelete.compareNoCase(
          AsciiString(TheDefaultLayerName.c_str())) == 0) {
    return;
  }
...
void LayersList::OnHideShowLayer() {
  ...
  for (it = layerIt->objectsInLayer.begin();
       it != layerIt->objectsInLayer.end(); ++it) {
    if (layerIt->show) {
...
void LayersList::OnMergeLayer(UINT commandID) {
  ...
  while (srclayerIt != mLayers.end() &&
         srclayerIt->layerName.compareNoCase(lastClickedLayer) != 0) {
    ++srclayerIt;
  }
...
ON_COMMAND_RANGE(ID_LAYERSLIST_MERGELAYERINTO_BEGIN,
                 ID_LAYERSLIST_MERGELAYERINTO_END, OnMergeLayer)
ON_COMMAND_RANGE(ID_LAYERSLIST_MERGEOBJECTINTO_BEGIN,
                 ID_LAYERSLIST_MERGEOBJECTINTO_END, OnMergeObject)
ON_COMMAND_RANGE(ID_LAYERSLIST_MERGEVIEWSELECTIONINTO_BEGIN,
                 ID_LAYERSLIST_MERGEVIEWSELECTIONINTO_END, OnMergeViewSelection)
END_MESSAGE_MAP()
