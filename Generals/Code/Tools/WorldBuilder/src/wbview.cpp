...
WbView::WbView()
  ...
  Int showWay =
      ::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION, "ShowWaypoints", 1);
  m_showWaypoints = (showWay != 0);
  Int showPoly = ::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION,
                                              "ShowPolygonTriggers", 1);
  m_showPolygonTriggers = (showPoly != 0);
  Int showObj =
      ::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION, "ShowObjectIcons", 1);
  m_showObjects = (showObj != 0);
  Int showNames =
      ::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION, "ShowNames", 1);
  m_showNames = (showNames != 0);
  Int snapToGrid =
      ::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION, "SnapToGrid", 0);
  m_snapToGrid = (snapToGrid != 0);
  Int showTerrain =
      ::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION, "ShowTerrain", 1);
...
ON_COMMAND(ID_OBJECTPROPERTIES_REFLECTSINMIRROR,
...
ON_UPDATE_COMMAND_UI(ID_VIEW_SHOWPOLYGONTRIGGERS,
...
ON_UPDATE_COMMAND_UI(ID_OBJECTPROPERTIES_REFLECTSINMIRROR,
...
void WbView::OnDraw(CDC *pDC) { DEBUG_ASSERTCRASH((0), ("oops")); }

/////////////////////////////////////////////////////////////////////////////
// WbView diagnostics

#ifdef _DEBUG
void WbView::AssertValid() const { CView::AssertValid(); }

void WbView::Dump(CDumpContext &dc) const { CView::Dump(dc); }
#endif  //_DEBUG
...
void WbView::mouseDown(TTrackingMode m, CPoint viewPt) {
  ...
  if (m_trackingMode != TRACK_NONE) return;
...
void WbView::mouseMove(TTrackingMode m, CPoint viewPt) {
  ...
  while (pObj) {
    ...
    const Int flagsWeDontWant = FLAG_ROAD_FLAGS | FLAG_ROAD_CORNER_ANGLED |
                                FLAG_BRIDGE_FLAGS | FLAG_ROAD_CORNER_TIGHT |
                                FLAG_ROAD_JOIN;
  ...
  if (pObj) {
    ...
    Bool exists;
    AsciiString objectID =
        pObj->getProperties()->getAsciiString(TheKey_uniqueID, &exists);
    CString obj;
    if (exists) {
...
void WbView::OnRButtonUp(UINT nFlags, CPoint point) { mouseUp(TRACK_R, point); }

void WbView::OnRButtonDown(UINT nFlags, CPoint point) {
  mouseDown(TRACK_R, point);
}

void WbView::OnLButtonUp(UINT nFlags, CPoint point) { mouseUp(TRACK_L, point); }

void WbView::OnLButtonDown(UINT nFlags, CPoint point) {
  mouseDown(TRACK_L, point);
}

void WbView::OnMButtonUp(UINT nFlags, CPoint point) { mouseUp(TRACK_M, point); }
...
TPickedStatus WbView::picked(MapObject *pObj, Coord3D docPt) {
  ...
  if (pObj->getFlag(FLAG_ROAD_FLAGS) || pObj->getFlag(FLAG_BRIDGE_FLAGS) ||
      pObj->isWaypoint()) {
    doArrow = false;
  }
...
WorldHeightMapEdit *WbView::getTrackingHeightMap() {
  ...
  // If we aren't editing, or the tool doesn't provide a map, use the current
  // one.
  if (pMap == NULL) {
...
void WbView::OnEditCopy() {
  ...
  for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
    ...
    if (pMapObj->getFlag(FLAG_ROAD_POINT1)) {
      ...
      DEBUG_ASSERTCRASH(pMapObj2 && pMapObj2->getFlag(FLAG_ROAD_POINT2),
                        ("oops"));
...
void WbView::OnUpdateEditCopy(CCmdUI *pCmdUI) { pCmdUI->Enable(); }
...
void WbView::OnUpdateEditCut(CCmdUI *pCmdUI) { pCmdUI->Enable(); }
...
void WbView::OnViewShowObjects() {
  ...
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowObjectIcons",
                                 m_showObjects ? 1 : 0);
}
...
void WbView::OnViewSnaptogrid() {
  ...
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "SnapToGrid",
                                 m_snapToGrid ? 1 : 0);
}
...
void WbView::OnEditReplace() {
  ...
  for (MapObject *pObj = MapObject::getFirstMapObject(); pObj;
       pObj = pObj->getNext()) {
    if (pObj->isSelected()) {
...
void WbView::OnObjectpropertiesReflectsinmirror() {
  ...
  CWorldBuilderDoc *pDoc = WbDoc();
  ModifyFlagsUndoable *pUndo =
      new ModifyFlagsUndoable(pDoc, FLAG_DRAWS_IN_MIRROR, !reflects);
  pDoc->AddAndDoUndoable(pUndo);
  REF_PTR_RELEASE(pUndo);  // belongs to pDoc now.
}

// This is actually lock angle - used to be horizontal & vertical, now just 1.
void WbView::OnLockHorizontal() { m_lockAngle = !m_lockAngle; }
...
void WbView::OnEditGloballightoptions() {
  ...
  //	Coord3D lightRay;
  //	lightRay.x=0.0f;lightRay.y=0.0f;lightRay.z=-1.0f;	//default light
  //above terrain. 	doLightFeedback(false,lightRay,0);	//turn off the light
  //direction indicator
}

void WbView::OnViewShowwaypoints() {
  m_showWaypoints = !m_showWaypoints;
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowWaypoints",
                                 m_showWaypoints ? 1 : 0);
  PointerTool::clearSelection();
}
...
void WbView::OnViewShowpolygontriggers() {
  ...
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowPolygonTriggers",
                                 m_showPolygonTriggers ? 1 : 0);
}
...
void WbView::OnShowNames() {
  ...
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowNames",
                                 m_showNames ? 1 : 0);
}
...
void WbView::OnValidationFixTeams() {
  ...
  DictItemUndoable *pUndo =
      new DictItemUndoable(allTeamDicts.begin(), newDict, newDict.getNthKey(0),
                           allTeamDicts.size(), pDoc, true);
  pDoc->AddAndDoUndoable(pUndo);
  REF_PTR_RELEASE(pUndo);  // belongs to pDoc now.
...
void WbView::OnShowTerrain() {
  ...
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowTerrain",
                                 m_showTerrain ? 1 : 0);
}
