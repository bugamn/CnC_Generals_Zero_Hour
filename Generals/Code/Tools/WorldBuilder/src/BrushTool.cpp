...
/// Constructor
BrushTool::BrushTool(void) : Tool(ID_BRUSH_TOOL, IDC_BRUSH_CROSS) {
...
void BrushTool::setWidth(Int width) {
  if (m_brushWidth != width) {
    ...
    DrawObject::setBrushFeedbackParms(m_brushSquare, m_brushWidth,
                                      m_brushFeather);
  }
};
...
void BrushTool::mouseDown(TTrackingMode m, CPoint viewPt, WbView *pView,
...
void BrushTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView *pView,
                        CWorldBuilderDoc *pDoc) {
  if (m != TRACK_L) return;
...
void BrushTool::mouseMoved(TTrackingMode m, CPoint viewPt, WbView *pView,
                           CWorldBuilderDoc *pDoc) {
  Coord3D cpt;
  ...
  for (i = ndx.x - sub; i < ndx.x + add; i++) {
    ...
    for (j = ndx.y - sub; j < ndx.y + add; j++) {
      ...
      if (m_brushSquare) {
        blendFactor =
            calcSquareBlendFactor(ndx, i, j, m_brushWidth, m_brushFeather);
      } else {
        blendFactor =
            calcRoundBlendFactor(ndx, i, j, m_brushWidth, m_brushFeather);
      }
      Int curHeight = m_htMapFeatherCopy->getHeight(i, j);
      float fNewHeight =
          blendFactor * m_brushHeight + ((1.0f - blendFactor) * curHeight);
