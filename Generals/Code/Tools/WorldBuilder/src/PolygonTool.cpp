...
// Pick a polygon.
Bool PolygonTool::poly_pickPoly(PolygonTrigger *pTrig, Coord3D loc,
                                Int tolerance) {
  ICoord3D iDocPt;
...
// Pick a polygon.
PolygonTrigger *PolygonTool::pickPolygon(Coord3D loc, CPoint viewPt,
                                         WbView *pView) {
...
// Pick a point.
Int PolygonTool::poly_pickPoint(PolygonTrigger *pTrig, CPoint viewPt,
                                WbView *pView) {
...
void PolygonTool::poly_pickOnMouseDown(CPoint viewPt, WbView *pView) {
  ...
  if (m_poly_curSelectedPolygon == NULL || !m_poly_isAdding) {
    ...
    if (m_poly_curSelectedPolygon &&
        poly_pickPoly(m_poly_curSelectedPolygon, docPt, SNAP_DISTANCE)) {
...
void PolygonTool::mouseDown(TTrackingMode m, CPoint viewPt, WbView *pView,
...
/// Perform the tool behavior on mouse down.
void PolygonTool::startMouseDown(TTrackingMode m, CPoint viewPt, WbView *pView,
                                 CWorldBuilderDoc *pDoc) {
  ...
  if (m_poly_curSelectedPolygon == NULL) {
    ...
    AddPolygonUndoable *pUndo =
        new AddPolygonUndoable(m_poly_curSelectedPolygon);
    pDoc->AddAndDoUndoable(pUndo);
    REF_PTR_RELEASE(pUndo);  // belongs to pDoc now.
    m_poly_isAdding = true;
  } else {
    m_poly_dragPointNdx =
        poly_pickPoint(m_poly_curSelectedPolygon, viewPt, pView);
    if (m_poly_isAdding) {
    ...
    if (m_poly_isAdding && m_poly_dragPointNdx == -1) {
      ...
      AddPolygonPointUndoable *pUndo =
          new AddPolygonPointUndoable(m_poly_curSelectedPolygon, iDocPt);
      pDoc->AddAndDoUndoable(pUndo);
      REF_PTR_RELEASE(pUndo);  // belongs to pDoc now.
    } else if (m_poly_dragPointNdx != -1) {
      // Moving a point on the polygon.
      ModifyPolygonPointUndoable *pUndo = new ModifyPolygonPointUndoable(
          m_poly_curSelectedPolygon, m_poly_dragPointNdx);
      pDoc->AddAndDoUndoable(pUndo);
      REF_PTR_RELEASE(pUndo);  // belongs to pDoc now.
      ...
      Coord3D docPt = m_poly_unsnappedMouseDownPt;
      m_poly_dragPointNdx =
          poly_getInsertIndex(m_poly_curSelectedPolygon, docPt);
      InsertPolygonPointUndoable *pUndo = new InsertPolygonPointUndoable(
          m_poly_curSelectedPolygon, iDocPt, m_poly_dragPointNdx);
      pDoc->AddAndDoUndoable(pUndo);
      REF_PTR_RELEASE(pUndo);  // belongs to pDoc now.
    }
  }
...
Bool PolygonTool::deleteSelectedPolygon(void) {
  ...
  if (m_poly_dragPointNdx != -1) {
    // Moving a point on the polygon.
    DeletePolygonPointUndoable *pUndo = new DeletePolygonPointUndoable(
        m_poly_curSelectedPolygon, m_poly_dragPointNdx);
    pDoc->AddAndDoUndoable(pUndo);
    REF_PTR_RELEASE(pUndo);  // belongs to pDoc now.
    m_poly_dragPointNdx = -1;
    ...
    // delete the polygon.
    DeletePolygonUndoable *pUndo =
        new DeletePolygonUndoable(m_poly_curSelectedPolygon);
    pDoc->AddAndDoUndoable(pUndo);
    REF_PTR_RELEASE(pUndo);  // belongs to pDoc now.
    m_poly_curSelectedPolygon = NULL;
    m_poly_dragPointNdx = -1;
...
void PolygonTool::setCursor(void) {
  if (m_poly_mouseUpPlus || (m_poly_isAdding && m_poly_curSelectedPolygon)) {
    if (m_poly_plusCursor == NULL) {
      m_poly_plusCursor =
          AfxGetApp()->LoadCursor(MAKEINTRESOURCE(IDC_POLYGON_PLUS));
    }
    ::SetCursor(m_poly_plusCursor);
  } else if (m_poly_mouseUpMove) {
    if (m_poly_moveCursor == NULL) {
      m_poly_moveCursor =
          AfxGetApp()->LoadCursor(MAKEINTRESOURCE(IDC_POLYGON_MOVE));
    }
...
void PolygonTool::mouseMoved(TTrackingMode m, CPoint viewPt, WbView *pView,
                             CWorldBuilderDoc *pDoc) {
  Coord3D docPt;
  pView->viewToDocCoords(viewPt, &docPt);
  if (m == TRACK_NONE) {
...
/** Mouse up - not much. */
void PolygonTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView *pView,
                          CWorldBuilderDoc *pDoc) {
  if (m != TRACK_L) return;
