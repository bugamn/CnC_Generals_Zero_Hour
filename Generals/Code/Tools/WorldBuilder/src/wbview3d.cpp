...
class PlaceholderView : public View {
 ...
 public:
  ...
  virtual Bool worldToScreen(const Coord3D *w, ICoord2D *s) {
    return FALSE;
  ...
  virtual void getScreenCornerWorldPointsAtZ(Coord3D *topLeft,
                                             Coord3D *topRight,
                                             Coord3D *bottomLeft,
                                             Coord3D *bottomRight, Real z) {};
  ...
  virtual void moveCameraAlongWaypointPath(Waypoint *way, Int frames,
                                           Int shutter, Bool orient) {};
  virtual Bool isCameraMovementFinished(void) { return true; };
  virtual void resetCamera(const Coord3D *location, Int frames) {
  };  ///< Move camera to location, and reset to default angle & zoom.
  virtual void rotateCamera(Real rotations, Int frames) {
  };  ///< Rotate camera about current viewpoint.
  virtual void rotateCameraTowardObject(ObjectID id, Int milliseconds,
                                        Int holdMilliseconds) {
  };  ///< Rotate camera to face an object, and hold on it
  virtual void cameraModFinalZoom(Real finalZoom) {
  ...
  virtual void cameraModFinalPitch(Real finalPitch) {
  ...
  virtual void rotateCameraTowardPosition(const Coord3D *pLoc,
                                          Int milliseconds) {
  };  ///< Rotate camera to face an object, and hold on it

  virtual const Coord3D &get3DCameraPosition() const {
    static Coord3D dummy;
    return dummy;
...
void SkeletonSceneClass::Remove_Render_Object(RenderObjClass *obj) {
  if (RenderList.Contains(obj)) {
    ...
    REF_PTR_SET(refPtr, obj);  // ref it, as when it gets removed from the
                               // scene, may get deleted otherwise.
    RTS3DScene::Remove_Render_Object(obj);
    REF_PTR_RELEASE(refPtr);
  }
}

void WbView3d::setObjTracking(MapObject *pMapObj, Coord3D pos, Real angle,
                              Bool show) {
  ...
  if (modelName != m_objectToolTrackingModelName) {
    ...
    m_objectToolTrackingObj =
        m_assetManager->Create_Render_Obj(modelName.str(), scale, 0);
  }
...
WbView3d::WbView3d()
  ...
  m_actualWinSize.x = ::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION, "Width",
                                                   THREE_D_VIEW_WIDTH);
  m_actualWinSize.y = ::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION, "Height",
                                                   THREE_D_VIEW_HEIGHT);
  ...
  m_showWireframe = (::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION,
                                                  "ShowWireframe", 0) != 0);
  m_showEntireMap = (::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION,
                                                  "ShowEntireMap", 1) != 0);
  m_projection = (::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION,
                                               "ShowTopDownView", 0) != 0);
  m_showShadows =
      (::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION, "ShowShadows", 1) != 0);
  TheWritableGlobalData->m_showSoftWaterEdge =
      (::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION, "ShowSoftWater", 1) !=
       0);
  TheWritableGlobalData->m_use3WayTerrainBlends =
      (::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION, "ShowExtraBlends", 1) >
               1
           ? 2
           : 1);
  setShowModels(
      ::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION, "ShowModels", 1) != 0);
  setShowGarrisoned(::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION,
                                                 "ShowGarrisoned", 0) != 0);
}
...
void WbView3d::shutdownWW3D(void) {
  if (m_ww3dInited) {
    ...
    if (m_assetManager) {
      PredictiveLODOptimizerClass::Free();  /// @todo: where does this need to
                                            /// be done?
...
void WbView3d::ReAcquireResources(void) {
  if (TheTerrainRenderObject) {
    ...
    TheTerrainRenderObject->worldBuilderUpdateBridgeTowers(m_assetManager,
                                                           m_scene);
  }
...
void WbView3d::reset3dEngineDisplaySize(Int width, Int height) {
  if (m_actualWinSize.x == width && m_actualWinSize.y == height) {
    return;
  }
...
static Real getHeightAroundPos(WBHeightMap *heightMap, Real x, Real y) {
  ...
  terrainHeightMax = max(terrainHeightMax, heightMap->getHeightMapHeight(
                                               x + TERRAIN_SAMPLE_SIZE,
                                               y - TERRAIN_SAMPLE_SIZE, NULL));
  terrainHeightMax = max(terrainHeightMax, heightMap->getHeightMapHeight(
                                               x - TERRAIN_SAMPLE_SIZE,
                                               y - TERRAIN_SAMPLE_SIZE, NULL));
  terrainHeightMax = max(terrainHeightMax, heightMap->getHeightMapHeight(
...
void WbView3d::setupCamera() {
  ...
  if (zOffset != 0) {
    Real zPos =
        (m_cameraOffset.length() - m_groundLevel) / m_cameraOffset.length();
  ...
  */
  ...
  /*
  DEBUG_LOG(("Camera: pos=(%g,%g) height=%g pitch=%g FXPitch=%g yaw=%g
  groundLevel=%g\n", targetPos.X, targetPos.Y, m_actualHeightAboveGround, pitch,
          m_FXPitch,
          angle, m_groundLevel));
          */
  ...
  if (m_projection) {
    camtransform.Make_Identity();
    camtransform.Set_Translation(
        Vector3(targetPos.X, targetPos.Y, lookDistance));
...
void WbView3d::resetRenderObjects() {
  ...
  if (TheW3DShadowManager) TheW3DShadowManager->Reset();

  updateLights();
  if (m_heightMapRenderObj) m_scene->Add_Render_Object(m_heightMapRenderObj);
}

// ----------------------------------------------------------------------------
void WbView3d::stepTimeOfDay() {
  TheWritableGlobalData->m_timeOfDay =
      (TimeOfDay)(TheGlobalData->m_timeOfDay + 1);
...
void WbView3d::setLighting(const GlobalData::TerrainLighting *tl,
                           Int whichLighting, Int whichLight) {
  if (whichLighting == GlobalLightOptions::K_TERRAIN) {
    TheWritableGlobalData
        ->m_terrainLighting[TheGlobalData->m_timeOfDay][whichLight] = *tl;
  } else if (whichLighting == GlobalLightOptions::K_OBJECTS) {
    TheWritableGlobalData
        ->m_terrainObjectsLighting[TheGlobalData->m_timeOfDay][whichLight] =
        *tl;
  ...
  if (m_globalLight) {
    ...
    m_globalLight[whichLight]->Set_Diffuse(
    ...
    if (m_scene &&
        whichLight == 0) {  // only let the first light contribute to ambient
      m_scene->Set_Ambient_Light(
          Vector3(ol->ambient.red, ol->ambient.green, ol->ambient.blue));
      m_baseBuildScene->Set_Ambient_Light(
          Vector3(ol->ambient.red, ol->ambient.green, ol->ambient.blue));
    }
  }
  if (TheTerrainRenderObject) {
...
void WbView3d::updateLights() {
...

  {
    TheWritableGlobalData->setTimeOfDay(TheGlobalData->m_timeOfDay);
    const GlobalData::TerrainLighting *ol =
        &TheGlobalData->m_terrainObjectsLighting[TheGlobalData->m_timeOfDay][0];
    ...
    for (Int i = 0; i < MAX_GLOBAL_LIGHTS; i++) {
      if (m_globalLight[i]) {
        ol = &TheGlobalData
                  ->m_terrainObjectsLighting[TheGlobalData->m_timeOfDay][i];
        m_globalLight[i]->Set_Ambient(Vector3(0.0f, 0.0f, 0.0f));
        m_globalLight[i]->Set_Diffuse(
            Vector3(ol->diffuse.red, ol->diffuse.green, ol->diffuse.blue));
  ...
  while (pMapObj && m_heightMapRenderObj) {
    if (pMapObj->isLight()) {
      ...
      lightP->Set_Ambient(Vector3(lightAmbientColor.red,
                                  lightAmbientColor.green,
                                  lightAmbientColor.blue));
      lightP->Set_Diffuse(Vector3(lightDiffuseColor.red,
                                  lightDiffuseColor.green,
                                  lightDiffuseColor.blue));

      lightP->Set_Position(
          Vector3(loc.x, loc.y, loc.z + lightHeightAboveTerrain));
...
void WbView3d::updateScorches(void) {
  ...
  for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
    ...
    if (pMapObj->isScorch()) {
      ...
      Scorches type =
          (Scorches)pMapObj->getProperties()->getInt(TheKey_scorchType);
  ...
  for (pMapObj = pObject; pMapObj; pMapObj = pMapObj->getNext()) {
    ...
    if (!renderObj) {
      ...
      AsciiString modelName =
          getModelNameAndScale(pMapObj, &scale, BODY_PRISTINE);
...
// ----------------------------------------------------------------------------
/// @todo srj -- this is a terrible hack, since things can have multiple models,
/// and it's private info. fix later.
AsciiString WbView3d::getBestModelName(const ThingTemplate *tt,
  ...
  if (tt) {
    ...
    if (mi.getCount() > 0) {
      //		const W3DModelDrawModuleData* md = dynamic_cast<const
      //W3DModelDrawModuleData*>(mi->getNthData(0));
...
void WbView3d::invalBuildListItemInView(BuildListInfo *pBuildToInval) {
  ...
  for (i = 0; i < TheSidesList->getNumSides(); i++) {
    ...
    for (BuildListInfo *pBuild = pSide->getBuildList(); pBuild;
         pBuild = pBuild->getNext()) {
      ...
      // Build list render obj is not refcounted, so check & make sure it exists
      // in the scene.
      ...
      if (!renderObj) {
        ...
        const ThingTemplate *tTemplate =
            TheThingFactory->findTemplate(thingName);
        ...
        if (renderObj == NULL && modelName.isEmpty() == FALSE &&
            strncmp(modelName.str(), "No ", 3)) {
          renderObj = m_assetManager->Create_Render_Obj(modelName.str(), scale,
                                                        playerColor);
          if (m_showShadows && tTemplate->getShadowType() != SHADOW_NONE) {
            ...
            strcpy(shadowInfo.m_ShadowName,
                   tTemplate->getShadowTextureName().str());
            DEBUG_ASSERTCRASH(
  ...
  // Build list render obj is not refcounted, so check & make sure it exists in
  // the scene.
...
AsciiString WbView3d::getModelNameAndScale(MapObject *pMapObj, Real *scale,
  ...
  if (pMapObj && pMapObj->getProperties()) {
    objWeather =
        pMapObj->getProperties()->getInt(TheKey_objectWeather, &exists);
  }
  switch (objWeather) {
    default:
    case 0:
  ...
  *scale = 1.0f;
  ...
  if (strncmp(TEST_STRING, pMapObj->getName().str(), strlen(TEST_STRING)) ==
      0) {
    ...
    modelName =
        "No Model Name";  // must be this while GDF exists (it's the default)
    const ThingTemplate *tTemplate;
...
void WbView3d::invalObjectInView(MapObject *pMapObjIn) {
  ...
  Bool isLight = false;
  Bool isScorch = false;
  if (pMapObjIn == NULL) isScorch = true;
  MapObject *pMapObj;
  for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
       pMapObj = pMapObj->getNext()) {
    if (pMapObjIn == pMapObj) found = true;
    ...
    if (pMapObj) {
      ...
      if (tt && !(pMapObj->getFlags() & FLAG_DONT_RENDER)) {
        ...
        AsciiString objectTeamName = pMapObj->getProperties()->getAsciiString(
            TheKey_originalOwner, &exists);
        if (exists) {
          TeamsInfo *teamInfo = TheSidesList->findTeamInfo(objectTeamName);
          if (teamInfo) {
            AsciiString teamOwner =
                teamInfo->getDict()->getAsciiString(TheKey_teamOwner);
            SidesInfo *pSide = TheSidesList->findSideInfo(teamOwner);
            if (pSide) {
              Bool hasColor = false;
              Int color =
                  pSide->getDict()->getInt(TheKey_playerColor, &hasColor);
              if (hasColor) {
                playerColor = color;
              } else {
                AsciiString tmplname =
                    pSide->getDict()->getAsciiString(TheKey_playerFaction);
      ...
      Int health = 100;
      health =
          pMapObj->getProperties()->getInt(TheKey_objectInitialHealth, &exists);
    ...
    if (!renderObj) {
      Real scale = 1.0;
      AsciiString modelName =
          getModelNameAndScale(pMapObj, &scale, curDamageState);
      // set render object, or create if we need to
      if (renderObj == NULL && modelName.isEmpty() == FALSE &&
          strncmp(modelName.str(), "No ", 3)) {
        if (!getShowModels()) {
        ...
        if (m_showShadows) {
          ...
          if (tTemplate && tTemplate->getShadowType() != SHADOW_NONE &&
              !(pMapObj->getFlags() &
                FLAG_DONT_RENDER)) {  // add correct type of shadow
            strcpy(shadowInfo.m_ShadowName,
                   tTemplate->getShadowTextureName().str());
            DEBUG_ASSERTCRASH(
                shadowInfo.m_ShadowName[0] != '\0',
    ...
    if (renderObj && !(pMapObj->getFlags() & FLAG_DONT_RENDER)) {
      ...
      if (isVehicle) {
        // note that this affects our orientation, but NOT our position...
        // specifically, it does NOT force us to "stick" to the ground!
        Matrix3D mtx;
...
// ----------------------------------------------------------------------------
void WbView3d::updateHeightMapInView(WorldHeightMap *htMap, Bool partial,
  ...
  if (m_heightMapRenderObj) {
    ...
    if (partial) {
      ...
      if (m_showEntireMap) {
        ...
        m_heightMapRenderObj->initHeightData(
            htMap->getXExtent(), htMap->getYExtent(), htMap, &lightListIt);
      } else {
        htMap->setDrawWidth(m_partialMapSize);
        htMap->setDrawHeight(m_partialMapSize);
        m_heightMapRenderObj->initHeightData(
            htMap->getDrawWidth(), htMap->getDrawHeight(), htMap, &lightListIt);
      }
  ...
  invalObjectInView(
      NULL);  // update all the map objects, to account for ground changes
...
MapObject *WbView3d::picked3dObjectInView(CPoint viewPt) {
  ...
  if (m_intersector && m_layer) {
    ...
    Bool hit =
        m_intersector->Intersect_Screen_Point_Layer(logX, logY, *m_layer);
    if (hit) {
      MapObject *pObj;
      for (pObj = MapObject::getFirstMapObject(); pObj;
           pObj = pObj->getNext()) {
        if (pObj->getRenderObj() ==
            m_intersector->Result.IntersectedRenderObject) {
          return pObj;
        }
      }
    }
...
BuildListInfo *WbView3d::pickedBuildObjectInView(CPoint viewPt) {
  ...
  if (m_intersector && m_buildLayer) {
    ...
    // do the intersection using W3D intersector class
    Bool hit =
        m_intersector->Intersect_Screen_Point_Layer(logX, logY, *m_buildLayer);
    if (hit) {
      for (i = 0; i < TheSidesList->getNumSides(); i++) {
        SidesInfo *pSide = TheSidesList->getSideInfo(i);
        for (BuildListInfo *pBuild = pSide->getBuildList(); pBuild;
...
Bool WbView3d::viewToDocCoords(CPoint curPt, Coord3D *newPt, Bool constrain) {
  ...
  //	const Int VIEW_BORDER = 3000;  // keeps you from falling off the edge of
  //the world.
  Bool result = false;
  CRect client;
  this->GetClientRect(&client);
  ...
  // follows the cursor.  Most tools, however, don't want to jump up & down
  // clifs and such.  So they use a fixed z plane when tracking, so things don't
  // move depending what you move over.
  ...
  if (!result) {
    intersection.X =
        Vector3::Find_X_At_Z(m_curTrackingZ, rayLocation, rayDirectionPt);
    intersection.Y =
        Vector3::Find_Y_At_Z(m_curTrackingZ, rayLocation, rayDirectionPt);
    result = true;
  }
...
void WbView3d::updateHysteresis(void) {
  ...
  // transform the screen coordinates by the camera's matrix into world
  // coordinates.
  x = zmod * camera_matrix[0][2] + xmod * camera_matrix[0][0] +
      ymod * camera_matrix[0][1];
  y = zmod * camera_matrix[1][2] + xmod * camera_matrix[1][0] +
      ymod * camera_matrix[1][1];
  z = zmod * camera_matrix[2][2] + xmod * camera_matrix[2][0] +
      ymod * camera_matrix[2][1];
  rayDirection.Set(x, y, z);
  rayDirectionPt = rayLocation + rayDirection;
...
Bool WbView3d::docToViewCoords(Coord3D curPt, CPoint *newPt) {
  ...
  //
  Int sx, sy;
  W3DLogicalScreenToPixelScreen(screen.X, screen.Y, &sx, &sy,
...
void WbView3d::redraw(void) {
  ...
  if (m_heightMapRenderObj) {
    if (m_needToLoadRoads) {
      ...
      m_heightMapRenderObj->worldBuilderUpdateBridgeTowers(m_assetManager,
                                                           m_scene);
  ...
  if (m_drawObject) {
    m_drawObject->setDrawObjects(
        m_showObjects, m_showWaypoints || WaypointTool::isActive(),
...
void WbView3d::render() {
  ...
  if (WW3D::Begin_Render(true, true, Vector3(0.5f, 0.5f, 0.5f),
    ...
    if (m_showObjToolTrackingObj && m_objectToolTrackingObj) {
      ...
      m_transparentObjectsScene->Set_Ambient_Light(
...
ON_UPDATE_COMMAND_UI(ID_VIEW_SHOWIMPASSABLEAREAS,
...
#ifdef _DEBUG
// ----------------------------------------------------------------------------
void WbView3d::AssertValid() const { WbView::AssertValid(); }

// ----------------------------------------------------------------------------
void WbView3d::Dump(CDumpContext &dc) const { WbView::Dump(dc); }
#endif  //_DEBUG
...
void WbView3d::initWW3D() {
  ...
  if (!m_ww3dInited) {
    ...
    bogusTacticalView.setOrigin(0, 0);
    if (WW3D::Set_Render_Device(0, m_actualWinSize.x, m_actualWinSize.y, 32,
                                true, true) != WW3D_ERROR_OK) {
      ...
      if (WW3D::Set_Render_Device(0, m_actualWinSize.x, m_actualWinSize.y, 16,
                                  true, true) != WW3D_ERROR_OK) {
    ...
    TheWritableGlobalData->m_enableBehindBuildingMarkers =
        false;  // this is only for the game.
...
int WbView3d::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  if (WbView::OnCreate(lpCreateStruct) == -1) return -1;
  ...
  TheWritableGlobalData->m_useCloudMap =
      AfxGetApp()->GetProfileInt("GameOptions", "cloudMap", 0);
  AfxGetApp()->WriteProfileInt(
      "GameOptions", "cloudMap",
      TheGlobalData->m_useCloudMap);  // Just in case it wasn't already there
  m_partialMapSize =
      AfxGetApp()->GetProfileInt("GameOptions", "partialMapSize", 97);

  m_showLayersList =
      AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION, "ShowLayersList", 0);
  m_showMapBoundaries =
      AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION, "ShowMapBoundaries", 0);
  m_showAmbientSounds =
      AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION, "ShowAmbientSounds", 0);
...
void WbView3d::drawLabels(HDC hdc) {
  ...
  Real selectedRadius =
      120.0f;  // default distance of lightfeeback model from object
  selectedPos.x = 0;
  selectedPos.y = 0;
  selectedPos.z = 0;
  ...
  if (isNamesVisible()) {
    for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
      ...
      if (m_doLightFeedback &&
          pMapObj
              ->isSelected()) {  // find out position of selected object in
                                 // order to use it for light feedback tracking.
        selectedPos = *pMapObj->getLocation();
        selectedPos.z = m_heightMapRenderObj->getHeightMapHeight(
            selectedPos.x, selectedPos.y, NULL);
      ...
      if (pMapObj->isWaypoint() && m_showWaypoints) {
        ...
        pos.z = m_heightMapRenderObj->getHeightMapHeight(pos.x, pos.y, NULL);
      } else if (pMapObj->getThingTemplate() &&
                 !(pMapObj->getFlags() &
                   (FLAG_ROAD_FLAGS | FLAG_BRIDGE_FLAGS)) &&
      ...
      for (i = 0; i < 4; i++) {
        ...
        switch (i) {
          case 0:
            break;
          case 1:
            name = pMapObj->getProperties()->getAsciiString(
                TheKey_waypointPathLabel1, &exists);
            break;
          case 2:
            name = pMapObj->getProperties()->getAsciiString(
                TheKey_waypointPathLabel2, &exists);
            ;
            break;
          case 3:
            name = pMapObj->getProperties()->getAsciiString(
                TheKey_waypointPathLabel3, &exists);
            ;
            break;
          default:
            name.clear();
        ...
        if (!name.isEmpty()) {
          ...
          if (!name.isEmpty()) {
            ...
            Vector3 world, screen;
            world.Set(pos.x + MAP_XY_FACTOR / 2, pos.y, pos.z);
            if (CameraClass::INSIDE_FRUSTUM !=
                m_camera->Project(screen, world)) {
              continue;
            }
  ...
  if (hdc && m_doLightFeedback) {  // Draw Lines to indicate the direction of
                                   // each light source
    //		Int
    //LightColors[MAX_GLOBAL_LIGHTS]={RGB(255,0,0),RGB(0,255,0),RGB(0,0,255)};
    ...
    for (Int lIndex = 0; lIndex < MAX_GLOBAL_LIGHTS; lIndex++) {
      ...
      if (m_lightFeedbackMesh[lIndex] == NULL) {
        ...
        m_lightFeedbackMesh[lIndex] =
            WW3DAssetManager::Get_Instance()->Create_Render_Obj(nameBuf);
      }
      if (m_lightFeedbackMesh[lIndex] == NULL) {
        break;
      }
      ...
      if (CameraClass::INSIDE_FRUSTUM ==
              m_camera->Project(screenStart, worldStart) &&
          CameraClass::INSIDE_FRUSTUM ==
              m_camera->Project(screenEnd, worldEnd)) {
        CRect rClient;
        GetClientRect(&rClient);
        ...
        W3DLogicalScreenToPixelScreen(screenStart.X, screenStart.Y, &sxStart,
                                      &syStart, rClient.right - rClient.left,
                                      rClient.bottom - rClient.top);
        W3DLogicalScreenToPixelScreen(screenEnd.X, screenEnd.Y, &sxEnd, &syEnd,
                                      rClient.right - rClient.left,
                                      rClient.bottom - rClient.top);
    ...
    if (!m_doLightFeedback) {  // not in light feedback mode.  Make sure the
                               // temporary feeback models are gone
...
BOOL WbView3d::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt) {
  if (m_trackingMode == TRACK_NONE) {
    ...
    MSG msg;
    while (
        ::PeekMessage(&msg, m_hWnd, WM_MOUSEWHEEL, WM_MOUSEWHEEL, PM_REMOVE)) {
...
void WbView3d::setDefaultCamera() {
  ...
  if (m_heightMapRenderObj) {
    m_groundLevel =
        m_heightMapRenderObj->getHeightMapHeight(pos.x, pos.y, NULL);
  }
...
// ----------------------------------------------------------------------------
Real WbView3d::getCameraPitch(void) { return m_FXPitch; }
...
void WbView3d::scrollInView(Real xScroll, Real yScroll, Bool end) {
  ...
  redraw();
  drawLabels();
  if (end) WbDoc()->syncViewCenters(m_centerPt.X, m_centerPt.Y);
  CMainFrame::GetMainFrame()->handleCameraChange();
}

void WbView3d::OnViewShowwireframe() {
  m_showWireframe = !m_showWireframe;
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowWireframe",
                                 m_showWireframe ? 1 : 0);
}
...
void WbView3d::OnViewShowentire3dmap() {
  ...
  Invalidate(false);
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowEntireMap",
                                 m_showEntireMap ? 1 : 0);
}
...
void WbView3d::OnViewShowtopdownview() {
  ...
  invalObjectInView(NULL);
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowTopDownView",
                                 m_projection ? 1 : 0);
}
...
void WbView3d::OnViewShowclouds() {
  ...
  AfxGetApp()->WriteProfileInt("GameOptions", "cloudMap",
...
void WbView3d::OnViewShowshadows() {
  ...
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowShadows",
                                 m_showShadows ? 1 : 0);
}
...
void WbView3d::OnViewShowSoftWater() {
  ...
  if (TheGlobalData
          ->m_showSoftWaterEdge) {  // we just turned it on, so recompute
                                    // shoreline tiles since they may not exist.
    TheTerrainRenderObject->updateShorelineTiles(
        0, 0, WbDoc()->GetHeightMap()->getXExtent() - 1,
        WbDoc()->GetHeightMap()->getYExtent() - 1, WbDoc()->GetHeightMap());
  }

  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowSoftWater",
...
void WbView3d::OnViewExtraBlends() {
  if (TheGlobalData->m_use3WayTerrainBlends == 1)
    TheWritableGlobalData->m_use3WayTerrainBlends =
        2;  // debug mode which draws the tiles in black
  else
    TheWritableGlobalData->m_use3WayTerrainBlends = 1;

  ::AfxGetApp()->WriteProfileInt(
      MAIN_FRAME_SECTION, "ShowExtraBlends",
...
void WbView3d::OnViewShowModels() {
  ...
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowModels",
                                 getShowModels() ? 1 : 0);
  resetRenderObjects();
  invalObjectInView(NULL);
}
...
void WbView3d::OnViewGarrisoned() {
  ...
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowGarrisoned",
                                 getShowGarrisoned() ? 1 : 0);
  resetRenderObjects();
  invalObjectInView(NULL);
}
  ...
  if (TheTerrainRenderObject) {
    {
      ...
      opts.SetDefaultSlopeToShow(
          TheTerrainRenderObject->getViewImpassableAreaSlope());
      if (opts.DoModal() == IDOK) {
        TheTerrainRenderObject->setViewImpassableAreaSlope(
            opts.GetSlopeToShow());
      } else {
        TheTerrainRenderObject->setViewImpassableAreaSlope(
            opts.GetDefaultSlope());
      }
    }
...
void WbView3d::OnViewPartialmapsize96x96() {
  m_partialMapSize = 97;
  AfxGetApp()->WriteProfileInt("GameOptions", "partialMapSize",
                               m_partialMapSize);
  m_showEntireMap = false;
  IRegion2D range = {0, 0, 0, 0};
...
void WbView3d::OnViewLayersList() {
  ...
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowLayersList",
                                 m_showLayersList ? 1 : 0);
...
void WbView3d::OnViewShowMapBoundaries() {
  ...
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowMapBoundaries",
                                 m_showMapBoundaries ? 1 : 0);
  DrawObject::setDoBoundaryFeedback(m_showMapBoundaries);
}
...
void WbView3d::OnViewShowAmbientSounds() {
  ...
  ::AfxGetApp()->WriteProfileInt(MAIN_FRAME_SECTION, "ShowAmbientSounds",
                                 m_showAmbientSounds ? 1 : 0);
  DrawObject::setDoAmbientSoundFeedback(m_showAmbientSounds);
}
