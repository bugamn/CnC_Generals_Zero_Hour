...
void BuildListTool::createWindow(void) {
  CRect frameRect;
  frameRect.top =
      ::AfxGetApp()->GetProfileInt(BUILD_PICK_PANEL_SECTION, "Top", 0);
  frameRect.left =
      ::AfxGetApp()->GetProfileInt(BUILD_PICK_PANEL_SECTION, "Left", 0);
  ...
  m_pickBuildingDlg.SetWindowPos(NULL, frameRect.left, frameRect.top, 0, 0,
...
void BuildListTool::deactivate() {
  ...
  Coord3D loc;
  loc.x = loc.y = loc.z = 0;
  p3View->setObjTracking(NULL, loc, 0,
                         false);  // Turn off object cursor tracking.
  p3View->resetRenderObjects();
  p3View->invalObjectInView(NULL);
...
void BuildListTool::setCursor(void) {
  if (isDoingAdd()) {
    ...
    if (m_rotateCursor == NULL) {
      m_rotateCursor =
          AfxGetApp()->LoadCursor(MAKEINTRESOURCE(IDC_BUILD_ROTATE));
    }
    ::SetCursor(m_rotateCursor);
  } else {
    if (m_pointerCursor == NULL) {
      m_pointerCursor =
          AfxGetApp()->LoadCursor(MAKEINTRESOURCE(IDC_BUILD_POINTER));
    }
...
void BuildListTool::mouseDown(TTrackingMode m, CPoint viewPt, WbView *pView,
  ...
  if (pInfo) {
    ...
    if (pInfo->isSelected() && len > 0.5f * MAP_XY_FACTOR &&
        len < 1.5f * MAP_XY_FACTOR) {
      m_rotating = true;
    }
    m_moving = true;
...
void BuildListTool::mouseMoved(TTrackingMode m, CPoint viewPt, WbView *pView,
                               CWorldBuilderDoc *pDoc) {
  Coord3D cpt;
  ...
  if (isDoingAdd()) {
    Coord3D loc = cpt;
    MapObject *pCur =
        ObjectOptions::getObjectNamed(m_pickBuildingDlg.getPickedUnit());
    loc.z = 0;
    if (pCur) {
...
void BuildListTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView *pView,
                            CWorldBuilderDoc *pDoc) {
  if (m != TRACK_L) return;
  if (!isDoingAdd()) {
    // Update the cursor.
  ...
  enum { HYSTERESIS = 3 };
  Bool justAClick = (abs(viewPt.x - m_downPt2d.x) < HYSTERESIS ||
                     abs(viewPt.x - m_downPt2d.x) < HYSTERESIS);
