...
TGlobalTextureClass
    WorldHeightMapEdit::m_globalTextureClasses[NUM_TEXTURE_CLASSES];
/** Destructor -.
 */
...
//
WorldHeightMapEdit::WorldHeightMapEdit(Int width, Int height,
                                       UnsignedByte initialHeight, Int border)
    : WorldHeightMap(), m_warnTooManyTex(false), m_warnTooManyBlend(false) {
  int i;
  ...
  m_numBlendedTiles = 1;
  m_numCliffInfo = 1;
  // Note - we have one less cell than the width & height. But for paranoia,
  // allocate extra row. jba.
  //
  ...
  for (i = 0; i < m_numGlobalTextureClasses; i++) {
    for (j = 0; j < m_numTextureClasses; j++) {
      if (m_globalTextureClasses[i].name == m_textureClasses[j].name) {
        DEBUG_ASSERTCRASH(m_textureClasses[j].globalTextureClass == -1,
                          ("oops"));  // should be unintialized at this point.
        if (m_globalTextureClasses[i].width != m_textureClasses[i].width) {
          didMajorRemap = true;  // This will handle the differing tile widths
                                 // in setBlendUsingCanonicalTile
        }
...
#define BLEND_TILE_PREFIX "TE"
  ...
  Bool isBlend =
      (0 == strncmp(BLEND_TILE_PREFIX, uiName, strlen(BLEND_TILE_PREFIX)));
...
void WorldHeightMapEdit::loadBaseImages(void) {
  ...
  for (terrain = TheTerrainTypes->firstTerrain(); terrain;
...
void WorldHeightMapEdit::loadDirectoryOfImages(char *pFilePath) {
  ...
  TheFileSystem->getFileListInDirectory(AsciiString(findBuf),
                                        AsciiString("*.*"), filenameList, TRUE);
...
void WorldHeightMapEdit::loadImagesFromTerrainType(TerrainType *terrain) {
  ...
  // open file
  CachedFileInputStream stream;
  if (!stream.open(texturePath)) return;
...
void WorldHeightMapEdit::getTexClassNeighbors(Int xIndex, Int yIndex,
                                              Int textureClass, Int *pSideCount,
                                              Int *pTotalCount) {
  Int i, j;
  int sideCount = 0;
  int totalCount = 0;
...
void WorldHeightMapEdit::saveToFile(DataChunkOutput &chunkWriter) {
  ...
  */
  ...
  chunkWriter.writeInt(m_dataSize);
  chunkWriter.writeArrayOfBytes((char *)m_tileNdxes,
                                m_dataSize * sizeof(Short));
  chunkWriter.writeArrayOfBytes((char *)m_blendTileNdxes,
                                m_dataSize * sizeof(Short));
  chunkWriter.writeArrayOfBytes((char *)m_extraBlendTileNdxes,
                                m_dataSize * sizeof(Short));
  chunkWriter.writeArrayOfBytes((char *)m_cliffInfoNdxes,
                                m_dataSize * sizeof(Short));
  chunkWriter.writeArrayOfBytes((char *)m_cellCliffState,
  ...
  MapObject::getWorldDict()->setInt(TheKey_weather,
                                    (Int)TheGlobalData->m_weather);
  ...
  SidesList::WriteSidesDataChunk(
      chunkWriter);  // Player data dicts are in the sides chunk.
  ...
  for (i = 0; i < 4; i++) {
    chunkWriter.writeReal(
        TheGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0].ambient.red);
    chunkWriter.writeReal(
        TheGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0]
            .ambient.green);
    chunkWriter.writeReal(
        TheGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0]
            .ambient.blue);
    chunkWriter.writeReal(
        TheGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0].diffuse.red);
    chunkWriter.writeReal(
        TheGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0]
            .diffuse.green);
    chunkWriter.writeReal(
        TheGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0]
            .diffuse.blue);
    chunkWriter.writeReal(
        TheGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0].lightPos.x);
    chunkWriter.writeReal(
        TheGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0].lightPos.y);
    chunkWriter.writeReal(
        TheGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][0].lightPos.z);

    chunkWriter.writeReal(
        TheGlobalData->m_terrainObjectsLighting[i + TIME_OF_DAY_FIRST][0]
            .ambient.red);
    chunkWriter.writeReal(
        TheGlobalData->m_terrainObjectsLighting[i + TIME_OF_DAY_FIRST][0]
            .ambient.green);
    ...
    for (Int j = 1; j < MAX_GLOBAL_LIGHTS;
         j++) {  // save state of new lights added in version 3.
      chunkWriter.writeReal(
          TheGlobalData->m_terrainObjectsLighting[i + TIME_OF_DAY_FIRST][j]
              .ambient.red);
      chunkWriter.writeReal(
          TheGlobalData->m_terrainObjectsLighting[i + TIME_OF_DAY_FIRST][j]
              .ambient.green);
    ...
    for (j = 1; j < MAX_GLOBAL_LIGHTS; j++) {
      chunkWriter.writeReal(
          TheGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][j]
              .ambient.red);
      chunkWriter.writeReal(
          TheGlobalData->m_terrainLighting[i + TIME_OF_DAY_FIRST][j]
              .ambient.green);
...
Bool WorldHeightMapEdit::setTileNdx(Int xIndex, Int yIndex, Int textureClass,
                                    Bool singleTile) {
...
/* return index of globalTextureClass corresponding to texture holding this
 * tile*/
Int WorldHeightMapEdit::getTextureClassFromNdx(Int tileNdx) {
  ...
  for (i = 0; i < m_numTextureClasses; i++) {
    ...
    // see if the blend tile is in a texture class, and get the right tile for
    // xIndex, yIndex.
    if (tileNdx >= m_textureClasses[i].firstTile &&
        tileNdx <
            m_textureClasses[i].firstTile + m_textureClasses[i].numTiles) {
...
Int WorldHeightMapEdit::getTextureClass(Int xIndex, Int yIndex,
                                        Bool baseClass) {
  Int ndx = (yIndex * m_width) + xIndex;
  DEBUG_ASSERTCRASH((ndx >= 0 && ndx < this->m_dataSize), ("oops"));
  ...
  if (!baseClass &&
      (m_blendTileNdxes[ndx] != 0 || m_extraBlendTileNdxes[ndx] != 0)) {
    return (-1);  // blended, so not of the original class.
  }
  return getTextureClassFromNdx(textureNdx);  // get globalTextureClass index
}
/*Get index of sub-tile that would show up here if it wasn't blended but tiled
 * across*/
Int WorldHeightMapEdit::getBlendTileNdxForClass(Int xIndex, Int yIndex,
                                                Int textureClass) {
...
Int WorldHeightMapEdit::getTileIndexFromTerrainType(TerrainType *terrain) {
  ...
  for (Int i = 0; i < NUM_TEXTURE_CLASSES; i++)
    if (m_globalTextureClasses[i].terrainType == terrain) return i;
...
Int WorldHeightMapEdit::allocateTiles(Int textureClass) {
  ...
  if (textureClass >= 0 && textureClass < m_numGlobalTextureClasses) {
    ...
    if (this->m_numBitmapTiles + m_globalTextureClasses[textureClass].numTiles >
        NUM_SOURCE_TILES) {
    ...
    m_textureClasses[m_numTextureClasses].numTiles =
        m_globalTextureClasses[textureClass].numTiles;
    m_textureClasses[m_numTextureClasses].width =
        m_globalTextureClasses[textureClass].width;
    m_textureClasses[m_numTextureClasses].name =
        m_globalTextureClasses[textureClass].name;
    m_textureClasses[m_numTextureClasses].isBlendEdgeTile =
        m_globalTextureClasses[textureClass].isBlendEdgeTile;
    m_numTextureClasses++;
    ...
    for (i = 0; i < numTiles; i++) {
      REF_PTR_SET(m_sourceTiles[m_numBitmapTiles + i],
                  m_globalTextureClasses[textureClass].tiles[i]);
    }
...
Int WorldHeightMapEdit::allocateEdgeTiles(Int globalTextureClass) {
  if (globalTextureClass >= 0 &&
      globalTextureClass < m_numGlobalTextureClasses) {
    Int localClass;
    Int firstTile = -1;
    ...
    DEBUG_ASSERTCRASH(
        m_globalTextureClasses[globalTextureClass].isBlendEdgeTile,
        ("Shouldn't use this for edge tiles."));
    // hasn't been copied into m_sourceTiles yet.
    if (m_numEdgeTiles + m_globalTextureClasses[globalTextureClass].numTiles >
        NUM_SOURCE_TILES) {
      m_warnTooManyTex = true;
      return (-1);
    }
    ...
    m_edgeTextureClasses[m_numEdgeTextureClasses].globalTextureClass =
        globalTextureClass;
    m_edgeTextureClasses[m_numEdgeTextureClasses].firstTile = m_numEdgeTiles;
    firstTile = m_numBitmapTiles;
    m_edgeTextureClasses[m_numEdgeTextureClasses].numTiles =
        m_globalTextureClasses[globalTextureClass].numTiles;
    m_edgeTextureClasses[m_numEdgeTextureClasses].width =
        m_globalTextureClasses[globalTextureClass].width;
    m_edgeTextureClasses[m_numEdgeTextureClasses].name =
        m_globalTextureClasses[globalTextureClass].name;
    m_edgeTextureClasses[m_numEdgeTextureClasses].isBlendEdgeTile =
        m_globalTextureClasses[globalTextureClass].isBlendEdgeTile;
    ...
    for (i = 0; i < numTiles; i++) {
      REF_PTR_SET(m_edgeTiles[m_numEdgeTiles + i],
                  m_globalTextureClasses[globalTextureClass].tiles[i]);
    }
...
Int WorldHeightMapEdit::getTileNdxForClass(Int xIndex, Int yIndex,
                                           Int textureClass) {
  int tileNdx = 0;
  if (textureClass >= 0 && textureClass < m_numGlobalTextureClasses) {
    Int firstTile =
        getFirstTile(textureClass);  // search used TextureClasses for one with
                                     // the same globalTextureIndex
    DEBUG_ASSERTCRASH(!m_globalTextureClasses[textureClass].isBlendEdgeTile,
                      ("Shouldn't use blend edge tiles for tiling."));
    if (firstTile == -1) {
    ...
    tileNdx = firstTile + ((xIndex / 2) % width) +
              width * ((yIndex / 2) %
                       width);  // 2 terrain cells across tile so divide by 2
...
/** Returns true if this texture class is already present, or can be added to
 * the map.
 */
...
void WorldHeightMapEdit::blendTile(Int xIndex, Int yIndex, Int srcXIndex,
                                   Int srcYIndex, Int textureClass,
                                   Int edgeClass) {
  ...
  if (textureClass < 0) {
    textureClass =
        getTextureClass(srcXIndex, srcYIndex);  // index into globalTextureClass
  }
  if (textureClass >= 0) {  // get index of sub-tile that would show up here if
                            // we continued tiling src.
    blendTileNdx = getBlendTileNdxForClass(xIndex, yIndex, textureClass);
    DEBUG_ASSERTCRASH((blendTileNdx / 4 < m_numBitmapTiles),
                      ("oops"));  // check it falls into one of the 64x64 tiles
                                  // of textures used on map.
  }

  if (curTileNdx ==
      blendTileNdx) {  // destination already contains continuation of source
                       // tile so no blend needed.
    m_tileNdxes[ndx] = blendTileNdx;
    m_blendTileNdxes[ndx] = 0;
    m_extraBlendTileNdxes[ndx] = 0;
    return;
  }
  // Updates m_blendTileNdxes[] for this coordinate with index into
  // m_blendedTiles[]
  blendSpecificTiles(xIndex, yIndex, srcXIndex, srcYIndex, curTileNdx,
                     blendTileNdx, false, edgeClass);
}

/*Allocate a new blend description or return existing one out of
 * m_blendedTiles[] and increment m_numBlendedTiles*/
Int WorldHeightMapEdit::findOrCreateBlendTile(TBlendTileInfo *pBlendInfo) {
  ...
  // Make a quick scan through the blended tiles, and see if we already got this
  // one.
  int k;
  for (k = 1; k < m_numBlendedTiles; k++) {
    TBlendTileInfo *curInfo = &m_blendedTiles[k];
#define MATCH(x) (curInfo->x == pBlendInfo->x)
    if (MATCH(blendNdx) && MATCH(horiz) && MATCH(vert) &&
        MATCH(rightDiagonal) && MATCH(leftDiagonal) && MATCH(longDiagonal) &&
        MATCH(inverted) && MATCH(customBlendEdgeClass)) {
      // got a match.
      return k;
    }
#undef MATCH
...
Updates m_blendTileNdxes[] for this coordinate with index into
m_blendedTiles[]*/
void WorldHeightMapEdit::blendSpecificTiles(Int xIndex, Int yIndex,
                                            Int srcXIndex, Int srcYIndex,
                                            Int curTileNdx, Int blendTileNdx,
                                            Bool longDiagonal, Int edgeClass) {
  TBlendTileInfo blendInfo;
  blendInfo.blendNdx =
      blendTileNdx;  // store the tile that will blend over existing tile
  ...
  // Check if there is already a blend tile at the destination and record its
  // flip state. We need to know this so that we don't accidently apply a third
  // blend layer with with a different flip and introduce z-fighting over this
  // tile.
  ...
  if (TheGlobalData->m_use3WayTerrainBlends && m_blendTileNdxes[ndx] != 0) {
    ...
    baseIsDiagonal =
        baseBlendInfo->rightDiagonal || baseBlendInfo->leftDiagonal;
    if (baseBlendInfo->rightDiagonal && !baseBlendInfo->inverted)
      baseNeedsFlip = true;
    else if (baseBlendInfo->leftDiagonal && baseBlendInfo->inverted)
      baseNeedsFlip = true;
  }
  ...
  if (srcYIndex == yIndex) {  // same vertical coordinates so horizontal blend
    ...
    blendInfo.inverted =
        (srcXIndex <
         xIndex);  // blend from opaque at left to transparent at right.
    // check if this blend tile will end up in a third blend layer and if so,
    // copy flip state from base layer.
    if (baseBlendInfo && baseNeedsFlip) blendInfo.inverted |= FLIPPED_MASK;
  } else if (srcXIndex == xIndex) {
    blendInfo.vert = true;  // same horizontal coordinates so vertical blend
    blendInfo.inverted =
        (srcYIndex < yIndex);  // invert if blend from bottom to top
    if (baseBlendInfo && baseNeedsFlip) blendInfo.inverted |= FLIPPED_MASK;
    ...
    if (blendInfo.rightDiagonal && !blendInfo.inverted)
      flipped = true;
    else if (blendInfo.leftDiagonal && blendInfo.inverted)
      flipped = true;
    // Check if this tile will end up in 3rd layer and if so, make sure it can
    // match flip of base layer.
    if (baseBlendInfo && baseIsDiagonal && baseNeedsFlip != flipped)
      return;  // the base requires a certain flip state which we can't alter so
               // can't apply extra blend here.
  }
  updateFlatCellForAdjacentCliffs(xIndex, yIndex,
                                  getTextureClassFromNdx(blendTileNdx));

  // Check if this tile is really necessary or if the base blend already does
  // the same thing as the 3way blend.
  if (baseBlendInfo && baseBlendInfo->blendNdx == blendTileNdx) return;
  ...
  if (newNdx >= 0) {
    ...
    if (TheGlobalData->m_use3WayTerrainBlends &&
        m_blendTileNdxes[ndx] !=
            0) {  // this tile already has a blend applied to it.  So we put the
                  // new blend into the
      // secondary layer.
      m_extraBlendTileNdxes[ndx] = newNdx;
      // force the primary layer to flip if the extra blend layer needs flip.
      // we only do this on vertical/horizontal base blends because they work in
      // either flip cases.
...
/******************************************************************
        autoBlendOut
                ...
                If edgeClass == -1, use an alpha blend.  Otherwise, use tile
   class edgeClass.
*/
void WorldHeightMapEdit::autoBlendOut(Int xIndex, Int yIndex,
                                      Int globalEdgeClass) {
  Int ndx = (yIndex * m_width) + xIndex;
  ...
  if (foo) {
    if (globalEdgeClass < 0) {
      for (globalEdgeClass = m_numGlobalTextureClasses - 1;
           globalEdgeClass >= 0; globalEdgeClass--) {
  ...
  while (pNodesToProcess) {
    ...
    getTexClassNeighbors(pCurNode->m_x, pCurNode->m_y, curTileClass, &sameSides,
                         &sameTotal);
    if (sameTotal == 8) {
        ...
        if (m_blendTileNdxes[curNdx] > 0) {
          if (curTileClass ==
              getTextureClassFromNdx(
                  m_blendedTiles[m_blendTileNdxes[curNdx]].blendNdx)) {
            continue;
          }
        }
        if (curTileClass != getTextureClass(i, j, true)) {
...
void WorldHeightMapEdit::blendToThisClass(Int xIndex, Int yIndex,
  ...
  DEBUG_ASSERTCRASH((total > 0),
                    ("oops"));  // if no neighbors, should not happen.
  DEBUG_ASSERTCRASH((sides < 3),
                    ("oops"));  // Should have been squished out earlier.
  if (total < 1) return;
  Int i, j;
  ...
  if (sides == 0) {
    ...
    blendSpecificTiles(xIndex, yIndex, srcXIndex, srcYIndex, curTileNdx,
                       blendTileNdx, true, edgeClass);
  }
}

/******************************************************************
        floodFill
                Fills the region at xIndex, yIndex with the specified texture.
   DoReplace forces texture to be replaced on entire map.
*/
Bool WorldHeightMapEdit::floodFill(Int xIndex, Int yIndex, Int textureClass,
                                   Bool doReplace) {
  Int ndx = (yIndex * m_width) + xIndex;
  ...
  if (!canFitTexture(textureClass) || doReplace) {
    CString confirm;
    confirm.Format(IDS_CONFIRM_REPLACE_TEXTURE,
                   m_globalTextureClasses[curTileClass].name.str());
  ...
  if (!textureExistsInMap && doReplace) {
    if (m_globalTextureClasses[textureClass].numTiles <=
        m_globalTextureClasses[curTileClass].numTiles) {
      Int i;
      Int localClass = -1;
      for (i = 0; i < m_numTextureClasses; i++) {
      ...
      m_textureClasses[localClass].width =
          m_globalTextureClasses[textureClass].width;
      m_textureClasses[localClass].name =
          m_globalTextureClasses[textureClass].name;
      m_textureClasses[localClass].isBlendEdgeTile =
          m_globalTextureClasses[textureClass].isBlendEdgeTile;
      REF_PTR_RELEASE(m_terrainTex);  // need to update the texture.
      Int numTiles = m_globalTextureClasses[textureClass].numTiles;
      for (i = 0; i < numTiles; i++) {
        REF_PTR_SET(m_sourceTiles[m_textureClasses[localClass].firstTile + i],
                    m_globalTextureClasses[textureClass].tiles[i]);
      }
      optimizeTiles();
      return true;
    ...
    for (i = 0; i < m_width; i++) {
      for (j = 0; j < m_height; j++) {
        ...
        if (curTileClass == getTextureClass(i, j, true)) {
          ...
          m_cliffInfoNdxes[ndx] =
              0;  // remove any cliff adjustment as we are doing a new texture.
        } else {  // adjust blended tiles so the blend texture matches the new
                  // filled texture
          /* Check blend */
          if (blendNdx == 0) continue;  // no blend.
          TBlendTileInfo blendInfo = m_blendedTiles[blendNdx];
          if (curTileClass ==
              this->getTextureClassFromNdx(blendInfo.blendNdx)) {
...
void WorldHeightMapEdit::reloadTextures(void) {
  ...
  for (i = 0; i < m_numGlobalTextureClasses; i++) {
    ...
    stream.rewind();
    /*Bool ok =*/readTiles(&stream, m_globalTextureClasses[i].tiles,
...
void WorldHeightMapEdit::showTileStatusInfo(void) {
  ...
  tmp.Format("Base tiles used %d of %d (%d%%)\n", m_numBitmapTiles,
             availableTiles, (m_numBitmapTiles * 100 + 50) / availableTiles);
  message += tmp;
  tmp.Format("Blend tiles used %d of %d (%d%%)\n", m_numBlendedTiles,
             availableBlends, (m_numBlendedTiles * 100 + 50) / availableBlends);
  message += tmp;
...
/******************************************************************
        setHeight
                This sets the height, and adjusts the cliff flag for the cells
   affected.
*/
void WorldHeightMapEdit::setHeight(Int xIndex, Int yIndex,
                                   UnsignedByte height) {
  Int ndx = (yIndex * m_width) + xIndex;
  ...
  for (x = 0; x < m_width; x++) {
    for (y = 0; y < m_height; y++) {
      ...
      if (blendNdx == 0) {
        ...
        // remove any forced flips due to 3-way blending if there is no 3-way
        // blend.
        if (!extraBlendNdx) curBlendInfo.inverted &= ~FLIPPED_MASK;

        curBlendInfo.blendNdx =
            getBlendTileNdxForClass(x, y, curBlendInfo.blendNdx);

        if (curBlendInfo.blendNdx ==
            m_tileNdxes[i]) {  // Tile index same as blend index would mean same
                               // texture
...
*/
Bool WorldHeightMapEdit::resize(Int newXSize, Int newYSize, Int newHeight,
                                Int newBorder, Bool anchorTop,
                                Bool anchorBottom, Bool anchorLeft,
                                Bool anchorRight, Coord3D *pObjOffset) {
  ...
  m_cellFlipState =
      MSGNEW("WorldHeightMapEdit::resize") UnsignedByte[numBytesX * m_height];
  m_cellCliffState =
      MSGNEW("WorldHeightMapEdit::resize") UnsignedByte[numBytesX * m_height];
  ...
  for (i = 0; i < m_dataSize; i++) {
    ...
    if (m_extraBlendTileNdxes[i] < 0 ||
        m_extraBlendTileNdxes[i] >= m_numBlendedTiles) {
      m_extraBlendTileNdxes[i] = 0;
    }
  }
...
void WorldHeightMapEdit::dbgVerifyAfterUndo(void) {
  ...
  for (j = 0; j < m_numTextureClasses; j++) {
    ...
    if (m_globalTextureClasses[globalClass].forDebugOnly_fileTextureClass !=
        j) {
      DEBUG_ASSERTCRASH(
          (m_globalTextureClasses[globalClass].forDebugOnly_fileTextureClass ==
           -1),
          ("oops"));
      m_globalTextureClasses[globalClass].forDebugOnly_fileTextureClass = j;
    }
    DEBUG_ASSERTCRASH((m_textureClasses[j].width ==
                       m_globalTextureClasses[globalClass].width),
                      ("oops"));
    DEBUG_ASSERTCRASH((m_textureClasses[j].numTiles ==
                       m_globalTextureClasses[globalClass].numTiles),
                      ("oops"));
    TileData *pTile = m_sourceTiles[m_textureClasses[j].firstTile];
    DEBUG_ASSERTCRASH((pTile == m_globalTextureClasses[globalClass].tiles[0]),
                      ("oops"));
    DEBUG_ASSERTCRASH(
        (j ==
         m_globalTextureClasses[globalClass].forDebugOnly_fileTextureClass),
  ...
  for (i = 0; i < m_numGlobalTextureClasses; i++) {
    ...
    if (localClass >= 0) {
      ...
      DEBUG_ASSERTCRASH((m_textureClasses[localClass].globalTextureClass == i),
                        ("oops"));
      DEBUG_ASSERTCRASH((m_textureClasses[localClass].width ==
                         m_globalTextureClasses[i].width),
                        ("oops"));
      DEBUG_ASSERTCRASH((m_textureClasses[localClass].numTiles ==
                         m_globalTextureClasses[i].numTiles),
                        ("oops"));
      TileData *pTile = m_sourceTiles[m_textureClasses[localClass].firstTile];
      DEBUG_ASSERTCRASH((pTile == m_globalTextureClasses[i].tiles[0]),
                        ("oops"));
      DEBUG_ASSERTCRASH((m_textureClasses[localClass].numTiles ==
                         m_globalTextureClasses[i].numTiles),
                        ("oops"));
  ...
  for (i = 0; i < m_numTextureClasses; i++) {
    ...
    if (m_textureClasses[i].globalTextureClass >= 0) {
      AsciiString path1 =
          m_globalTextureClasses[m_textureClasses[i].globalTextureClass].name;
      AsciiString path2 = m_textureClasses[i].name;
      DEBUG_ASSERTCRASH(path1 == path2, ("oops"));
      DEBUG_ASSERTCRASH(
          (m_globalTextureClasses[m_textureClasses[i].globalTextureClass]
               .forDebugOnly_fileTextureClass == i),
          ("oops"));
    }
  }
...
Bool WorldHeightMapEdit::selectSimilar(void) {
  ...
  MapObject *otherObj;
  Bool anySelected = false;
  for (selectedObj = firstObj; selectedObj;
       selectedObj = selectedObj->getNext()) {
  ...
  for (otherObj = firstObj; otherObj != NULL; otherObj = otherObj->getNext()) {
    ...
    AsciiString layerName =
        otherObj->getProperties()->getAsciiString(TheKey_objectLayer, &exists);
...
Bool WorldHeightMapEdit::selectInvalidTeam(void) {
  ...
  for (i = 0; i < TheSidesList->getNumTeams(); i++) {
    name = TheSidesList->getTeamInfo(i)->getDict()->getAsciiString(
        TheKey_teamName);
...
static const Real STRETCH_LIMIT =
    1.5f;  // If it is stretching less than this, don't adjust.
static inline Int IABS(Int x) {
  if (x >= 0) return x;
  return -x;
};

static const Real TEX_PER_CELL =
    32.0f / TEXTURE_WIDTH;  // we use 32 texels per cell.
...
Bool WorldHeightMapEdit::doCliffAdjustment(Int xIndex, Int yIndex) {
  ...
  Real textureClassExtent =
      m_globalTextureClasses[curTileClass].width * TILE_PIXEL_EXTENT;
  ...
  while (pNodesToProcess) {
    ...
    if (blend && curTileClass ==
                     getTextureClassFromNdx(m_blendedTiles[blend].blendNdx)) {
      classMatch = true;
    }
    ...
    if (isCliff) {
      ...
      updateForAdjacentCliffs(pCurNode->m_x, pCurNode->m_y, pProcessed,
                              cliffInfo);
  ...
  while (pUnCliffyNodes) {
    ...
    if (!pProcessed[ndx]) {
      m_cliffInfoNdxes[ndx] = 0;
      updateFlatCellForAdjacentCliffs(pCurNode->m_x, pCurNode->m_y,
                                      curTileClass, pProcessed);
      pProcessed[ndx] = true;
    }
    delete pCurNode;
  }
...
void WorldHeightMapEdit::updateFlatCellForAdjacentCliffs(
    Int xIndex, Int yIndex, Int curTileClass, UnsignedByte *pProcessed) {
  TCliffInfo tmpCliff;
  Bool lock0 = false;
  Bool lock1 = false;
  Bool lock2 = false;
  ...
  if (!pProcessed && m_cliffInfoNdxes[ndx]) {
    if (curTileClass !=
        getTextureClassFromNdx(m_cliffInfo[m_cliffInfoNdxes[ndx]].tileIndex)) {
      okToProcess = false;
    }
  }
...
void WorldHeightMapEdit::findBoundaryNear(Coord3D *pt, float okDistance,
                                          Int *outNdx, Int *outHandle) {
  if (!outNdx) {
    return;
  }
