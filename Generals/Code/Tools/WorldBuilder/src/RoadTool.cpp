...
//
/// Constructor
RoadTool::RoadTool(void) : Tool(ID_ROAD_TOOL, IDC_ROAD) { m_mapObj = NULL; }

/// Destructor
RoadTool::~RoadTool(void) { m_mapObj = NULL; }
//-----------------------------------------------------------------------------
//         Public Functions
//-----------------------------------------------------------------------------
  ...
  for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
    ...
    if (pMapObj->getFlag(FLAG_ROAD_POINT1)) {
      ...
      if (!pMapObj->isSelected()) {
        dist.Set(pMapObj->getLocation()->x - pLoc->x,
                 pMapObj->getLocation()->y - pLoc->y);
      ...
      if (!pMapObj2->isSelected()) {
        dist.Set(pMapObj2->getLocation()->x - pLoc->x,
                 pMapObj2->getLocation()->y - pLoc->y);
        if (dist.Length() < snapDist) {
...
void RoadTool::mouseDown(TTrackingMode m, CPoint viewPt, WbView *pView,
  ...
  loc1 = cpt;
  if (!pDoc->getCellPositionFromCoord(cpt, &loc1)) return;
  ...
  if (isBridge) {
    ...
    const ThingTemplate *tt =
        TheThingFactory->findTemplate(RoadOptions::getCurRoadName());
    if (tt) {
      isLandmark = tt->isBridge();
    }
    if (isLandmark) {
      MapObject *pNew1 = newInstance(MapObject)(
          loc1, RoadOptions::getCurRoadName(), 0.0f, 0, NULL, tt);
      pNew1->getProperties()->setAsciiString(TheKey_originalOwner,
  ...
  MapObject *pNew1 =
      newInstance(MapObject)(loc1, roadName, 0.0f, 0, NULL, NULL);
  MapObject *pNew2 =
      newInstance(MapObject)(loc2, roadName, 0.0f, 0, NULL, NULL);
  ...
  pNew1->getProperties()->setAsciiString(TheKey_originalOwner,
                                         NEUTRAL_TEAM_INTERNAL_STR);
  pNew2->getProperties()->setAsciiString(TheKey_originalOwner,
                                         NEUTRAL_TEAM_INTERNAL_STR);
  ...
  if (divideSegment) {
    // Roads belong to the neutral player. :)
    pNew3->getProperties()->setAsciiString(TheKey_originalOwner,
                                           NEUTRAL_TEAM_INTERNAL_STR);
    pNew4->getProperties()->setAsciiString(TheKey_originalOwner,
                                           NEUTRAL_TEAM_INTERNAL_STR);
...
void RoadTool::mouseMoved(TTrackingMode m, CPoint viewPt, WbView *pView,
                          CWorldBuilderDoc *pDoc) {
  if (m != TRACK_L) return;
...
void RoadTool::mouseUp(TTrackingMode m, CPoint viewPt, WbView *pView,
                       CWorldBuilderDoc *pDoc) {
  if (m != TRACK_L) return;
  m_mapObj = NULL;
}
