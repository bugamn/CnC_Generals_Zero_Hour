...
ObjectPreview::ObjectPreview() { m_tTempl = NULL; }
...
static UnsignedByte *saveSurface(IDirect3DSurface8 *surface) {
  ...
  HRESULT hr = m_pDev->CreateImageSurface(desc.Width, desc.Height, desc.Format,
                                          &tempSurface);
...
static UnsignedByte *generatePreview(const ThingTemplate *tt) {
  ...
  if (modelName.isEmpty() == FALSE && strncmp(modelName.str(), "No ", 3)) {
    ...
    if (model) {
      ...
      model->Set_Position(
          Vector3(-sphere.Center.X, -sphere.Center.Y, -sphere.Center.Z));

      // Create reflection texture
      TextureClass *objectTexture =
      ...
      Matrix3D camTran;
      camTran.Look_At(Vector3(dist * 2, dist * 2, dist),
      ...
      if (orthoCamera) camera->Set_Projection_Type(CameraClass::ORTHO);
...
void ObjectPreview::OnPaint() {
  ...
  if (pData) {
    DrawMyTexture(&dc, previewRect.top, previewRect.left, previewRect.Width(),
                  previewRect.Height(), pData);
  } else {
...
void ObjectPreview::DrawMyTexture(CDC *pDc, int top, int left, Int width,
                                  Int height, UnsignedByte *rgbData) {
  ...
  pBI->bmiHeader.biSizeImage =
      (PREVIEW_WIDTH * PREVIEW_HEIGHT) * (pBI->bmiHeader.biBitCount / 8);
  ...
  //::Sleep(10);
  // int val=::StretchDIBits(pDc->m_hDC, left, top, width, height, 0, 0,
  // PREVIEW_WIDTH, PREVIEW_HEIGHT, rgbData, pBI, 	DIB_RGB_COLORS, SRCCOPY);
  /*int val=*/::StretchDIBits(pDc->m_hDC, left, top, width, height,
                              PREVIEW_WIDTH / 4, PREVIEW_HEIGHT / 4,
                              PREVIEW_WIDTH / 2, PREVIEW_HEIGHT / 2, rgbData,
