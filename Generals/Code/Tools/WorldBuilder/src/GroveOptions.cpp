...
void GroveOptions::makeMain(void) { TheGroveOptions = this; }
...
GroveOptions::~GroveOptions() { TheGroveOptions = NULL; }

void GroveOptions::_setTreesToLists(void) {
  CString str;
  for (VecPairNameDisplayNameIt it = mVecDisplayNames.begin();
       it != mVecDisplayNames.end(); it++) {
...
void GroveOptions::_setDefaultRatios(void) {
  ...
  if (pWnd) {
    defaultRatio =
        AfxGetApp()->GetProfileInt("GroveOptions", "DefaultRatio1", 0);
  ...
  if (pWnd) {
    defaultRatio =
        AfxGetApp()->GetProfileInt("GroveOptions", "DefaultRatio2", 0);
    sprintf(buff, "%d", defaultRatio);
    pWnd->SetWindowText(buff);
  }
  ...
  if (pWnd) {
    defaultRatio =
        AfxGetApp()->GetProfileInt("GroveOptions", "DefaultRatio3", 0);
    sprintf(buff, "%d", defaultRatio);
    pWnd->SetWindowText(buff);
  }
  ...
  if (pWnd) {
    defaultRatio =
        AfxGetApp()->GetProfileInt("GroveOptions", "DefaultRatio4", 0);
    sprintf(buff, "%d", defaultRatio);
    pWnd->SetWindowText(buff);
  }
  ...
  if (pWnd) {
    defaultRatio =
        AfxGetApp()->GetProfileInt("GroveOptions", "DefaultRatio5", 0);
    sprintf(buff, "%d", defaultRatio);
    pWnd->SetWindowText(buff);
  }
}
...
void GroveOptions::_setDefaultNumTrees(void) {
  ...
  int defaultNumTrees =
      AfxGetApp()->GetProfileInt("GroveOptions", "NumberofTrees", 10);
  static char buff[ARBITRARY_BUFF_SIZE];
...
void GroveOptions::_updatePlacementAllowed(void) {
  ...
  pButt = (CButton*)GetDlgItem(IDC_Grove_AllowCliffPlacement);
  if (pButt) {
    AfxGetApp()->WriteProfileInt("GroveOptions", "AllowCliffPlace",
                                 pButt->GetCheck());
  }

  pButt = (CButton*)GetDlgItem(IDC_Grove_AllowWaterPlacement);
  if (pButt) {
    AfxGetApp()->WriteProfileInt("GroveOptions", "AllowWaterPlace",
                                 pButt->GetCheck());
  }
}
...
UnicodeString GetDisplayNameFromPair(const PairNameDisplayName* pNamePair) {
  ...
  // The unicode portion of the pair was empty. We need to use the Ascii version
  // instead.
