...
WaterOptions::WaterOptions(CWnd *pParent /*=NULL*/) : m_moveUndoable(NULL) {
  ...
  if (theTrigger) {
    ...
    if (river) {
      ...
      if (curPoint >= 0) {
        ...
        Int i;
        for (i = curPoint;
             i < theTrigger->getNumPoints() - 1 && i < curPoint + 4; i++) {
        ...
        if (delta > 1) {
          ...
          if (pointsOut < pointsIn) {
            pNew =
                adjustCount(theTrigger, newPoint + 1, endPoint, pointsIn - 1);
            theTrigger->setRiverStart(pointsIn);
          } else {
            pNew =
                adjustCount(theTrigger, endPoint + 1, newPoint, pointsOut - 1);
...
/// Adjust the spacing.
PolygonTrigger *WaterOptions::adjustCount(PolygonTrigger *trigger, Int firstPt,
                                          Int lastPt, Int desiredPointCount) {
...
void WaterOptions::GetPopSliderInfo(const long sliderID, long *pMin, long *pMax,
