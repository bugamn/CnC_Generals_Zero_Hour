...
static AsciiString teamNameForUI(SidesList &sides, int i) {
  ...
  if (sides.isPlayerDefaultTeam(ti)) {
    ...
    if (ostr.isEmpty()) ostr = NEUTRAL_NAME_STR;
    AsciiString n;
...
static Int findTeamParentIndex(SidesList &sides, Int i) {
  AsciiString oname =
      sides.getTeamInfo(i)->getDict()->getAsciiString(TheKey_teamOwner);

  for (int j = 0; j < sides.getNumSides(); j++) {
    if (oname ==
        sides.getSideInfo(j)->getDict()->getAsciiString(TheKey_playerName)) {
      return -(j + 1);
    }
  }
  DEBUG_CRASH(("hmm"));
...
void CTeamsDialog::updateUI(Int whatToRebuild) {
  ...
  DEBUG_ASSERTLOG(!modified,
                  ("had to clean up sides in CTeamsDialog::updateUI! (caller "
                   "should do this)"));
  if (modified) {
  ...
  if (m_curTeam >= m_sides.getNumTeams()) m_curTeam = m_sides.getNumTeams() - 1;
  ...
  del->EnableWindow(!isDefault);  // toplevel team names are delete-able, but
                                  // "default" teams are not
  CButton *copyteam = (CButton *)GetDlgItem(IDC_COPYTEAM);
  copyteam->EnableWindow(!isDefault);  // toplevel team names are delete-able,
                                       // but "default" teams are not
  ...
  newteam->EnableWindow(
      whichPlayer >
      0);  // toplevel team names are delete-able, but "default" teams are not
...
void CTeamsDialog::OnCancel() { CDialog::OnCancel(); }
...
void CTeamsDialog::OnNewteam() {
  ...
  AsciiString oname = m_sides.getTeamInfo(m_curTeam)->getDict()->getAsciiString(
      TheKey_teamOwner);

  Dict d;
  d.setAsciiString(TheKey_teamName, tname);
  d.setAsciiString(TheKey_teamOwner,
                   oname);  // owned by the parent of whatever is selected.
...
void CTeamsDialog::OnDeleteteam() {
  ...
  AsciiString tname = m_sides.getTeamInfo(m_curTeam)->getDict()->getAsciiString(
      TheKey_teamName);
...
void CTeamsDialog::UpdateTeamsList() {
  ...
  for (Int i = 0, inserted = 0; i < numTeams; i++) {
    ...
    if (ti->getDict()->getAsciiString(TheKey_teamOwner) ==
        playerNameForUI(m_sides, which).str()) {
      Bool exists;
      ...
      CString pri;
      pri.Format(TEXT("%d"),
                 ti->getDict()->getInt(TheKey_teamProductionPriority, &exists));
...
void CTeamsDialog::OnSelchangePlayerList() { updateUI(REBUILD_ALL); }
...
void CTeamsDialog::OnSelectTeamMembers() {
  ...
  for (pObj = MapObject::getFirstMapObject(); pObj; pObj = pObj->getNext()) {
    ...
    AsciiString objectsTeam =
        pObj->getProperties()->getAsciiString(TheKey_originalOwner);
...
void CTeamsDialog::OnMoveDownTeam() {
  ...
  if (m_curTeam >= m_sides.getNumTeams() - 1) return;
...
void CTeamsDialog::validateTeamOwners(void) {
  ...
  for (Int i = 0; i < numTeams; ++i) {
    ...
    Bool exists;
    AsciiString owner =
        ti->getDict()->getAsciiString(TheKey_teamOwner, &exists);
...
Bool CTeamsDialog::isValidTeamOwner(AsciiString ownerName) {
  ...
  for (Int i = 0; i < numOwners; ++i) {
    ...
    Bool exists;
    AsciiString sideOwnerName =
        side->getDict()->getAsciiString(TheKey_playerName, &exists);
