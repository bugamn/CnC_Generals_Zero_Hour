...
// Texturing, no zbuffer, disabled zbuffer write, primary gradient, alpha
// blending
#define SC_OPAQUE                                                             \
  (SHADE_CNST(                                                                \
      ShaderClass::PASS_ALWAYS, ShaderClass::DEPTH_WRITE_DISABLE,             \
...
void DrawObject::stopWaypointDragFeedback() { m_dragWaypointFeedback = false; }

DrawObject::~DrawObject(void) { freeMapResources(); }
...

{
  ...
  m_waterDrawObject->init(0, 0, 0, NULL,
                          WaterRenderObjClass::WATER_TYPE_0_TRANSLUCENT);
  TheWaterRenderObj = m_waterDrawObject;
}

Bool DrawObject::Cast_Ray(RayCollisionTestClass &raytest) { return false; }

//@todo: MW Handle both of these properly!!
DrawObject::DrawObject(const DrawObject &src) { *this = src; }
...
void DrawObject::Get_Obj_Space_Bounding_Sphere(SphereClass &sphere) const {
  Vector3 ObjSpaceCenter(TheGlobalData->m_waterExtentX,
                         TheGlobalData->m_waterExtentY, 50 * MAP_XY_FACTOR);
...
void DrawObject::Get_Obj_Space_Bounding_Box(AABoxClass &box) const {
  Vector3 minPt(-2 * TheGlobalData->m_waterExtentX,
                -2 * TheGlobalData->m_waterExtentY, 0);
  Vector3 maxPt(2 * TheGlobalData->m_waterExtentX,
                2 * TheGlobalData->m_waterExtentY, 100 * MAP_XY_FACTOR);
  box.Init(minPt, maxPt);
}

Int DrawObject::Class_ID(void) const { return RenderObjClass::CLASSID_UNKNOWN; }

RenderObjClass *DrawObject::Clone(void) const { return new DrawObject(*this); }
...
Int DrawObject::initData(void) {
  ...
  m_vertexBufferTile1 =
      NEW_REF(DX8VertexBufferClass, (DX8_FVF_XYZDUV1, m_numTriangles * 3,
  ...
  m_vertexFeedback =
      NEW_REF(DX8VertexBufferClass, (DX8_FVF_XYZDUV1, NUM_FEEDBACK_VERTEX,
                                     DX8VertexBufferClass::USAGE_DEFAULT));
  ...
  m_shaderClass = ShaderClass::ShaderClass(SC_OPAQUE);  //_PresetOpaque2DShader;//ShaderClass(SC_OPAQUE);
                                                        ////_PresetOpaqueShader;
...
void DrawObject::updateMeshVB(void) {
  ...
  if (m_moldMesh == NULL) {
    ...
    pMgr->Set_WW3D_Load_On_Demand(false);  // We don't want it fishing for these
                                           // assets in the game assets.
    ...
    if (m_moldMesh == NULL) {
      ...
      m_moldMesh =
          (MeshClass *)pMgr->Create_Render_Obj(m_curMeshModelName.str());
    }
    if (m_moldMesh) {
  ...
  //m_moldMesh->Peek_Model()->Get_Vertex_Normal_Array();
...
#else
    TheTerrainRenderObject->doTheLight(&vb, &lightRay, (Vector3 *)(&pNormal[i]),
                                       NULL, 1.0f);
...
void DrawObject::updateRampVB(void) {
  ...
  /*
          Generate the rectangle via the function
     BuildRectFromSegmentAndWidth(...). Note that for the rectangular case, this
     is easy, as we simply step along the line at pre-determined step sizes,
     with no additional calculation. (IE, we can simply perform linear
     interpolation.) However, with the curved case, we will need to recalculate
     the value every step along the way.


          Ultimately, what I'd like to do is to precompute what the terrain is
     actually going to do, and then use the faux-adjusted vertices, but this is
     much easier to start from. jkmcd
  */
  ...
  for (i = 0; i < numVertex; i++) {
    ...
    curVb->diffuse = curVb->diffuse =
        0x0000ffff | (theAlpha << 24);  // bright cyan.

    Vector3 vLoc;
    vLoc.X = (br.X - bl.X) * INT_TO_REAL(i % widthVerts) / (widthVerts - 1) +
             (tl.X - bl.X) * INT_TO_REAL(i / lengthVerts) / (lengthVerts - 1) +
             bl.X;

    vLoc.Y = (br.Y - bl.Y) * INT_TO_REAL(i % widthVerts) / (widthVerts - 1) +
             (tl.Y - bl.Y) * INT_TO_REAL(i / lengthVerts) / (lengthVerts - 1) +
             bl.Y;

    vLoc.Z = (br.Z - bl.Z) * INT_TO_REAL(i % widthVerts) / (widthVerts - 1) +
             (tl.Z - bl.Z) * INT_TO_REAL(i / lengthVerts) / (lengthVerts - 1) +
             bl.Z;
  ...
  for (i = 0; i < numBoundaries; ++i) {
    ...
    for (j = 0; j < 4; ++j) {
      ...
      if (j == 0) {
        ...
        startPt.z = TheTerrainRenderObject->getHeightMapHeight(startPt.x,
                                                               startPt.y, NULL);
        endPt.x = 0;
        ...
        endPt.z =
            TheTerrainRenderObject->getHeightMapHeight(endPt.x, endPt.y, NULL);
      } else if (j == 1) {
        startPt = endPt;
      ...
      Vector3 normal(endPt.x - startPt.x, endPt.y - startPt.y,
                     endPt.z - startPt.z);
      normal.Normalize();
...
void DrawObject::updateAmbientSoundVB(void) {
  ...
  while (mo) {
    if (!mo->getThingTemplate() ||
        (mo->getThingTemplate()->getEditorSorting() != ES_AUDIO)) {
      mo = mo->getNext();
      continue;
    }
    ...
    startPt.z =
        TheTerrainRenderObject->getHeightMapHeight(startPt.x, startPt.y, NULL);
  ...
  for (i = 0; i <= pDoc->getNumWaypointLinks(); i++) {
    ...
    for (k = 0; k < 2; k++) {
      ...
      if (k == 0 || i == pDoc->getNumWaypointLinks()) {
        ...
        if (pWay) {
          Bool biDirectional = pWay->getProperties()->getBool(
      ...
      if (i == pDoc->getNumWaypointLinks()) {
        ...
        if (pWay1 && pWay2) {
          ...
          wayLayer = pWay1->getProperties()->getAsciiString(TheKey_objectLayer,
                                                            &exists);
          if (exists && TheLayersList->isLayerHidden(wayLayer)) {
            gotLocation = false;
          }

          wayLayer = pWay2->getProperties()->getAsciiString(TheKey_objectLayer,
                                                            &exists);
          if (exists && TheLayersList->isLayerHidden(wayLayer)) {
            gotLocation = false;
          }
      ...
      if (gotLocation) {
        ...
        normal.Rotate_Z(PI / 2);
        loc1.z =
            TheTerrainRenderObject->getHeightMapHeight(loc1.x, loc1.y, NULL);
        loc2.z =
            TheTerrainRenderObject->getHeightMapHeight(loc2.x, loc2.y, NULL);
...
void DrawObject::updatePolygonVB(PolygonTrigger *pTrig, Bool selected,
                                 Bool isOpen) {
  //	const Int theAlpha = 64;
...
#define ADJUST_FROM_INDEX_TO_REAL(k) \
  ((k - pMap->getBorderSize()) * MAP_XY_FACTOR)
  ...
  for (j = minY; j < maxY; j++) {
    for (i = minX; i < maxX; i++) {
      ...
      if (m_squareFeedback) {
        ...
        Real blendFactor = Tool::calcRoundBlendFactor(m_cellCenter, i, j,
                                                      brushWidth, featherWidth);
        if (blendFactor > 0.99) {
          curVb->diffuse = diffuse;
      ...
      Real X, Y, theZ;
      if (doubleResolution) {
        X = ADJUST_FROM_INDEX_TO_REAL(i) / 2.0f +
            ADJUST_FROM_INDEX_TO_REAL(2 * offset + m_cellCenter.x) / 2.0;
        Y = ADJUST_FROM_INDEX_TO_REAL(j) / 2.0f +
            ADJUST_FROM_INDEX_TO_REAL(2 * offset + m_cellCenter.y) / 2.0;
...
/* This is a code snippet that starts to attempt to solve the concave area
problem, but doesn't, really. const Int maxPoints = 256; Bool pointFlags[256];
                                ...
                                for (k=1; k<numPoints; k++) {
                                        ...
                                        if (pt.y < pLL2.y || (pt.y==pLL2.y &&
pt.x<pLL2.x) ) { pLL2 = pt; pLL1 = *pTrig->getPoint(k-1); if (k<numPoints-1) {
                                                        pLL3 =
*pTrig->getPoint(k+1); } else { pLL3 = *pTrig->getPoint(0);
                                                }
                                        }
                                }
                                ICoord3D v1, v2;
                                ...
                                while (didSomething) {
                                        ...
                                        for (k=0; k<pTrig->getNumPoints()-1;
k++) { if (!pointFlags[k]) continue; Int kPlus1; for (kPlus1 = k+1; kPlus1 <
pTrig->getNumPoints()-1; kPlus1++) { if (pointFlags[kPlus1]) break;
                                                }
                                                if (kPlus1 >=
pTrig->getNumPoints()-1) continue; Int kPlus2 = kPlus1+1; for (kPlus2 =
kPlus1+1; kPlus2 < pTrig->getNumPoints(); kPlus2++) { if (pointFlags[kPlus2])
break;
                                                }

                                                ICoord3D pt1 =
*pTrig->getPoint(k); ICoord3D pt2 = *pTrig->getPoint(kPlus1); ICoord3D pt3 =
*pTrig->getPoint(kPlus2);
...
Int DrawObject::updateVB(DX8VertexBufferClass *pVB, Int color, Bool doArrow,
                         Bool doDiamond) {
  Int i, k;
  ...
  static const Int highlightColors[NUM_HIGHLIGHT] = {((255 << 8) + (255 << 16)),
                                                     ((255 << 16)), (255 << 8)};
  Int diffuse =
      b + (g << 8) + (r << 16) + (theAlpha << 24);  // b g<<8 r<<16 a<<24.
  if (pVB) {
    ...
    for (k = 0; k < 3; k++) {
      if (k == 0) {
        ...
        vb->x = 2 * theRadius + 2 * halfLineWidth;
        vb->y = 0;
      } else {
        vb->x = theRadius;
        vb->y = 2 * halfLineWidth;
      }
      vb->z = theZ;
      vb->diffuse = highlightColors[curHighlight] + (theAlpha << 24);
      vb->u1 = 0;
    ...
    for (i = 0; i < limit; i++) {
      for (k = 0; k < 3; k++) {
        ...
        if (k == 0) {
          vb[3 * NUM_TRI].z += 3.0;
          vb[3 * NUM_TRI].diffuse = highlightColors[curHighlight] +
                                    (theAlpha << 24);  // b g<<8 r<<16 a<<24.
        }
        vb++;
      }
      curAngle += deltaAngle;
...
void DrawObject::setRampFeedbackParms(const Coord3D *start, const Coord3D *end,
                                      Real rampWidth) {
  DEBUG_ASSERTCRASH(
      start && end,
      ("Parameter passed into setRampFeedbackParms was NULL. Not allowed"));
  if (!(start && end)) {
    return;
  }
...
// So just shut it off for now.  The failure case was new doc, add a poly
// trigger. Adding any other object fixed the problem.	jba
...
void DrawObject::Render(RenderInfoClass &rinfo) {
  ...
  if (m_drawObjects || m_drawWaypoints) {
    ...
    for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
      ...
      if (pMapObj->getFlag(FLAG_ROAD_FLAGS) ||
          pMapObj->getFlag(FLAG_BRIDGE_FLAGS) || pMapObj->isWaypoint()) {
        doArrow = false;
      }
      ...
      count++;
      ///@todo - remove the istree stuff, or get the info from the thing
      ///template.  jba.
      Bool isTree = false;
      ...
      if (isTree) {
        DX8Wrapper::Draw_Triangles(NUM_TRI * 3, polyCount, 0,
                                   (m_numTriangles * 3));
      } else {
  ...
  if (m_drawPolygonAreas) {
    ...
    for (selected = 0; selected < 2; selected++) {
      for (PolygonTrigger *pTrig = PolygonTrigger::getFirstPolygonTrigger();
        ...
        for (i = 0; i < pTrig->getNumPoints(); i++) {
          Bool pointSelected =
              (polySelected && PolygonTool::getSelectedPointNdx() == i);
          ICoord3D iLoc = *pTrig->getPoint(i);
          Coord3D loc;
          loc.x = iLoc.x;
          loc.y = iLoc.y;
          loc.z =
              TheTerrainRenderObject->getHeightMapHeight(loc.x, loc.y, NULL);
          SphereClass bounds(Vector3(loc.x, loc.y, loc.z), THE_RADIUS);
          if (rinfo.Camera.Cull_Sphere(bounds)) {
            continue;
          }
        ...
        DX8Wrapper::Set_Vertex_Buffer(m_vertexBufferTile1);
        updatePolygonVB(pTrig, polySelected,
                        polySelected && PolygonTool::isSelectedOpen());
        DX8Wrapper::Set_Vertex_Buffer(m_vertexFeedback);
        if (m_feedbackIndexCount > 0) {
          DX8Wrapper::Set_Index_Buffer(m_indexFeedback, 0);
          DX8Wrapper::Draw_Triangles(0, m_feedbackIndexCount / 3, 0,
                                     m_feedbackVertexCount);
        }
      }
      DX8Wrapper::Set_Index_Buffer(m_indexBuffer, 0);
  ...
  if (m_rampFeedback) {
    ...
    if (m_feedbackIndexCount > 0) {
      ...
      DX8Wrapper::Set_DX8_Render_State(
          D3DRS_FILLMODE, D3DFILL_WIREFRAME);  // we want a solid ramp
      DX8Wrapper::Set_DX8_Render_State(D3DRS_LIGHTING,
  ...
  if (m_boundaryFeedback) {
    ...
    if (m_feedbackIndexCount > 0) {
      ...
      DX8Wrapper::Set_DX8_Render_State(D3DRS_FILLMODE,
                                       D3DFILL_SOLID);  // we want a solid ramp
      DX8Wrapper::Set_DX8_Render_State(D3DRS_LIGHTING,
                                       FALSE);  // disable lighting
      DX8Wrapper::Draw_Triangles(0, m_feedbackIndexCount / 3, 0,
                                 m_feedbackVertexCount);
...
void BuildRectFromSegmentAndWidth(const Coord3D *start, const Coord3D *end,
                                  Real width, Coord3D *outBL, Coord3D *outTL,
                                  Coord3D *outBR, Coord3D *outTR) {
  /*
          Here's how we're generating the surface to render:
                  1) Assign longSeg to be the segment from rampStartPoint to
     rampStopPoint 2) Cross product with the segment (0, 0, 1) 3) Normalize to
     get the unit vector (which is in the XY plane.) 4) Multiply the unit vector
     by the ramp width / 2 5) Store the four corners of the ramp as startPoint +
     unit, startPoint - unit, endPoint + unit and endPoint - unit. 6) This gives
     us a surface that has endpoints which always lie flat along the ground.
  */
  ...
  Coord3D bl = {start->x + unitVec.X, start->y + unitVec.Y,
                start->z + unitVec.Z};
  Coord3D tl = {end->x + unitVec.X, end->y + unitVec.Y, end->z + unitVec.Z};
  Coord3D br = {start->x - unitVec.X, start->y - unitVec.Y,
                start->z - unitVec.Z};
