...
TerrainMaterial::TerrainMaterial(CWnd *pParent /*=NULL*/)
    : m_updating(false), m_currentWidth(3) {
...
void TerrainMaterial::updateLabel(void) {
  ...
  AsciiString name =
      pDoc->GetHeightMap()->getTexClassUiName(m_currentFgTexture);
...
void TerrainMaterial::updateTextureSelection(void) {
  if (m_staticThis) {
    m_staticThis->setTerrainTreeViewSelection(TVI_ROOT,
                                              m_staticThis->m_currentFgTexture);
...
BOOL TerrainMaterial::OnInitDialog() {
  ...
  ScreenToClient(&rect);
  rect.DeflateRect(2, 2, 2, 2);
  m_terrainSwatches.Create(NULL, "", WS_CHILD, rect, this,
                           IDC_TERRAIN_SWATCHES);
...
void TerrainMaterial::addTerrain(char *pPath, Int terrainNdx,
                                 HTREEITEM parent) {
  TerrainType *terrain = TheTerrainTypes->findTerrain(
      WorldHeightMapEdit::getTexClassName(terrainNdx));
  Bool doAdd = FALSE;
  char buffer[_MAX_PATH];
  //
  // if we have a 'terrain' entry, it means that our terrain index was properly
  // defined in an INI file, otherwise it was from eval textures.  We will sort
  // all of the eval texture entries in a tree leaf all their own while the
  // others are sorted according to a field specified in INI
  //
  if (terrain) {
    if (terrain->isBlendEdge()) {
      return;  // Don't add blend edges to the materials list.
    }
    for (TerrainClass i = TERRAIN_NONE; i < TERRAIN_NUM_CLASSES;
         i = (TerrainClass)(i + 1)) {
  ...
  Int percent = (WorldHeightMapEdit::getTexClassNumTiles(terrainNdx) * 100 +
                 availableTiles / 2) /
                availableTiles;
...
void TerrainMaterial::GetPopSliderInfo(const long sliderID, long *pMin,
