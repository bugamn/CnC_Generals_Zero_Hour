...
static void DisconnectedCallback(PEER peer, const char *reason, void *param) {
  ...
  if (matcher) matcher->handleDisconnect(reason);
}
...
static void RoomMessageCallback(PEER peer, RoomType roomType, const char *nick,
  ...
  MatcherClass *matcher = (MatcherClass *)param;
  if (matcher) matcher->handleRoomMessage(nick, message, messageType);
}
...
static void PlayerMessageCallback(PEER peer, const char *nick,
  ...
  MatcherClass *matcher = (MatcherClass *)param;
  if (matcher) matcher->handlePlayerMessage(nick, message, messageType);
}
...
static void PlayerJoinedCallback(PEER peer, RoomType roomType, const char *nick,
  ...
  MatcherClass *matcher = (MatcherClass *)param;
  if (matcher) matcher->handlePlayerJoined(nick);
}
...
static void PlayerLeftCallback(PEER peer, RoomType roomType, const char *nick,
  ...
  MatcherClass *matcher = (MatcherClass *)param;
  if (matcher) matcher->handlePlayerLeft(nick);
}
...
static void PlayerChangedNickCallback(PEER peer, RoomType roomType,
  ...
  MatcherClass *matcher = (MatcherClass *)param;
  if (matcher) matcher->handlePlayerChangedNick(oldNick, newNick);
}

static void EnumPlayersCallback(PEER peer, PEERBool success, RoomType roomType,
                                int index, const char *nick, int flags,
                                void *param) {
  MatcherClass *matcher = (MatcherClass *)param;
  if (matcher)
...
static void ConnectCallback(PEER peer, PEERBool success, void *param) {
  MatcherClass *matcher = (MatcherClass *)param;
  if (matcher) matcher->handleConnect(success == PEERTrue);
}

static void JoinCallback(PEER peer, PEERBool success, PEERJoinResult result,
                         RoomType roomType, void *param) {
  MatcherClass *matcher = (MatcherClass *)param;
  if (matcher) matcher->handleJoin(success == PEERTrue);
}

static void NickErrorCallback(PEER peer, int type, const char *badNick,
                              void *param) {
  ...
  if (type == PEER_IN_USE) {
    ...
    int newVal = 0;
    if (badNick[len - 1] == '}' && badNick[len - 3] == '{' &&
    ...
    nickStr.append(tmp);
    nickStr.append("}");
    DBGMSG("Nickname taken: was " << badNick << ", new val = " << newVal
    ...
    if (newVal < 10) {
      ...
      MatcherClass *matcher = (MatcherClass *)param;
      if (matcher) matcher->handleNickError(badNick);
    }
  } else {
    // Cancel the connect.
    peerRetryWithNick(peer, NULL);
...
void callbackAll(CHAT chat, CHATBool success, int numChannels,
...
void MatcherClass::handleConnect(bool success) {
  ...
  // chatEnumChannels( peerGetChat(m_peer), "", callbackEach, callbackAll, NULL,
  // CHATTrue ); DEBUG_LOG(("Done enumerating chat channels\n"));
}

void MatcherClass::handleGroupRoomList(bool success, int groupID,
                                       const char *name) {}
...
void MatcherClass::handleNickError(const char *badNick) { exit(1); }
...
void MatcherClass::connectAndLoop(void) {
  ...
  // Set the title.
  /////////////////
  if (!peerSetTitle(m_peer, title.get(), secretKey.get(), title.get(),
                    secretKey.get(), 0, 30, PEERTrue, pingRooms,
                    crossPingRooms)) {
    peerShutdown(m_peer);
  ...
  peerConnect(m_peer, m_baseNick.get(), m_profileID, NickErrorCallback,
              ConnectCallback, this, PEERTrue);
  if (!m_connectSuccess) {
    peerShutdown(m_peer);
  ...
  bool cdOk = false;
  peerAuthenticateCDKey(m_peer, serialNo.get(), AuthenticateCDKeyCallback,
                        &cdOk, PEERTrue);
  if (!cdOk) {
    peerShutdown(m_peer);
  ...
  m_groupID = 0;
  peerListGroupRooms(m_peer, NULL, ListGroupRoomsCallback, &m_groupID,
                     PEERTrue);
  m_groupID = s_groupID;
