...
int calcPingDelta(const GeneralsUser *a, const GeneralsUser *b) {
  ...
  for (int i = 0; i < (int)a->pseudoPing.size(); ++i) {
    ...
    if (p1 * p1 + p2 * p2 < bestPing) bestPing = p1 * p1 + p2 * p2;
  }
...
GeneralsMatcher::GeneralsMatcher() {
  ...
  Global.config.getInt("SecondsBetweenPoolSizeAnnouncements",
                       m_secondsBetweenPoolSizeAnnouncements, NULL);
...
void GeneralsMatcher::sendMatchInfo(
    ...
    GeneralsUser *user1, GeneralsUser *user2, GeneralsUser *user3,
    GeneralsUser *user4, GeneralsUser *user5, GeneralsUser *user6,
    GeneralsUser *user7, GeneralsUser *user8, int numPlayers, int ladderID) {
...
void GeneralsMatcher::checkMatches(void) {
  ...
  for (LadderMap::iterator it = m_ladders.begin(); it != m_ladders.end();
       ++it) {
    checkMatchesInUserMap(it->second, it->first, 2, showPoolSize);
  }
}

double GeneralsMatcher::computeMatchFitness(const std::string &i1,
                                            const GeneralsUser *u1,
                                            const std::string &i2,
                                            const GeneralsUser *u2) {
  // DBGMSG("matching "<<i1<< " vs "<<i2);
  if (u1->status != STATUS_WORKING || u2->status != STATUS_WORKING) return 0.0;

  // see if they pinged the same # of servers (sanity).
  if (u1->pseudoPing.size() != u2->pseudoPing.size()) return 0.0;
  ...
  if (!u1->widened && pingDelta > u1->maxPing) return 0.0;
  if (!u2->widened && pingDelta > u2->maxPing) return 0.0;
  // DBGMSG("pingDelta="<<pingDelta);

  // DBGMSG(u1->discons << "," << u1->maxDiscons << "  " << u2->discons << ","
  // << u2->maxDiscons);
  //  check discons
...

  {
    ...
    if (!MapSetCount(tmp)) return 0.0;
  }

  // they have something in common.  calculate match fitness.
  double matchFitness =
      (weightAvgPoints * (1 - pointPercent) +
       weightLowPing * (MaxPingValue - pingDelta) / MaxPingValue) /
      (double)totalWeight;
...
void GeneralsMatcher::checkMatchesInUserMap(UserMap &userMap, int ladderID,
                                            int numPlayers, bool showPoolSize) {
  ...
  for (i1 = userMap.begin(); i1 != userMap.end(); ++i1) {
    ...
    if (u1->timeToWiden && u1->timeToWiden < now) {
      u1->timeToWiden = 0;
      u1->widened = true;
      for (int m = 0; m < (int)u1->maps.size(); ++m) u1->maps[m] = 1;
      DBGMSG("Widening search for " << i1->first);
      peerMessagePlayer(m_peer, i1->first.c_str(), "MBOT:WIDENINGSEARCH",
                        NormalMessage);
    }
  }
  ...
  for (i1 = userMap.begin(); i1 != userMap.end(); ++i1) {
    u1 = i1->second;
    if (u1->status != STATUS_WORKING) continue;
    ...
    for (++i2; i2 != userMap.end(); ++i2) {
      u2 = i2->second;
      if (u2->status != STATUS_WORKING) continue;
      ...
      if (matchFitness > fitnessThreshold) {
        if (numPlayers == 2) {
          ...
          for (++i3; i3 != userMap.end(); ++i3) {
            u3 = i3->second;
            if (u3->status != STATUS_WORKING) continue;

            double matchFitness1 =
                computeMatchFitness(i1->first, u1, i3->first, u3);
            double matchFitness2 =
                computeMatchFitness(i2->first, u2, i3->first, u3);
            MapBitSet tmp = MapSetUnion(u1->maps, u2->maps);
            tmp = MapSetUnion(tmp, u3->maps);
            if (MapSetCount(tmp) && matchFitness1 > fitnessThreshold &&
                matchFitness2 > fitnessThreshold) {
              i4 = i3;
              for (++i4; i4 != userMap.end(); ++i4) {
                u4 = i4->second;
                if (u4->status != STATUS_WORKING) continue;

                double matchFitness1 =
                    computeMatchFitness(i1->first, u1, i4->first, u4);
                double matchFitness2 =
                    computeMatchFitness(i2->first, u2, i4->first, u4);
                double matchFitness3 =
                    computeMatchFitness(i3->first, u3, i4->first, u4);
                MapBitSet tmp = MapSetUnion(u1->maps, u2->maps);
                tmp = MapSetUnion(tmp, u3->maps);
                ...
                if (MapSetCount(tmp) && matchFitness1 > fitnessThreshold &&
                  ...
                  if (numPlayers == 4) {
                    ...
                    sendMatchInfo(i1->first, i2->first, i3->first, i4->first,
                                  "", "", "", "", u1, u2, u3, u4, NULL, NULL,
                                  NULL, NULL, 4, ladderID);
                    break;
                  } else {
                    i5 = i4;
                    for (++i5; i5 != userMap.end(); ++i3) {
                      u5 = i5->second;
                      if (u5->status != STATUS_WORKING) continue;

                      double matchFitness1 =
                          computeMatchFitness(i1->first, u1, i5->first, u5);
                      double matchFitness2 =
                          computeMatchFitness(i2->first, u2, i5->first, u5);
                      double matchFitness3 =
                          computeMatchFitness(i3->first, u3, i5->first, u5);
                      double matchFitness4 =
                          computeMatchFitness(i4->first, u4, i5->first, u5);
                      MapBitSet tmp = MapSetUnion(u1->maps, u2->maps);
                      tmp = MapSetUnion(tmp, u3->maps);
                      tmp = MapSetUnion(tmp, u4->maps);
                      tmp = MapSetUnion(tmp, u5->maps);
                      if (MapSetCount(tmp) &&
                          matchFitness1 > fitnessThreshold &&
                          matchFitness2 > fitnessThreshold &&
                          matchFitness3 > fitnessThreshold &&
                          matchFitness4 > fitnessThreshold) {
                        i6 = i5;
                        for (++i6; i6 != userMap.end(); ++i6) {
                          u6 = i6->second;
                          if (u6->status != STATUS_WORKING) continue;

                          double matchFitness1 =
                              computeMatchFitness(i1->first, u1, i6->first, u6);
                          double matchFitness2 =
                              computeMatchFitness(i2->first, u2, i6->first, u6);
                          double matchFitness3 =
                              computeMatchFitness(i3->first, u3, i6->first, u6);
                          double matchFitness4 =
                              computeMatchFitness(i4->first, u4, i6->first, u6);
                          double matchFitness5 =
                              computeMatchFitness(i5->first, u5, i6->first, u6);
                          MapBitSet tmp = MapSetUnion(u1->maps, u2->maps);
                          tmp = MapSetUnion(tmp, u3->maps);
                          tmp = MapSetUnion(tmp, u4->maps);
                          tmp = MapSetUnion(tmp, u5->maps);
                          ...
                          if (MapSetCount(tmp) &&
                            ...
                            if (numPlayers == 6) {
                              // match 6 players
                              sendMatchInfo(i1->first, i2->first, i3->first,
                                            i4->first, i5->first, i6->first, "",
                              ...
                              for (++i7; i7 != userMap.end(); ++i7) {
                                u7 = i7->second;
                                if (u7->status != STATUS_WORKING) continue;

                                double matchFitness1 = computeMatchFitness(
                                    i1->first, u1, i7->first, u7);
                                double matchFitness2 = computeMatchFitness(
                                    i2->first, u2, i7->first, u7);
                                double matchFitness3 = computeMatchFitness(
                                    i3->first, u3, i7->first, u7);
                                double matchFitness4 = computeMatchFitness(
                                    i4->first, u4, i7->first, u7);
                                double matchFitness5 = computeMatchFitness(
                                    i5->first, u5, i7->first, u7);
                                double matchFitness6 = computeMatchFitness(
                                    i6->first, u6, i7->first, u7);
                                MapBitSet tmp = MapSetUnion(u1->maps, u2->maps);
                                tmp = MapSetUnion(tmp, u3->maps);
                                tmp = MapSetUnion(tmp, u4->maps);
                                tmp = MapSetUnion(tmp, u5->maps);
                                ...
                                if (MapSetCount(tmp) &&
                                  ...
                                  for (++i8; i8 != userMap.end(); ++i8) {
                                    u8 = i8->second;
                                    if (u8->status != STATUS_WORKING) continue;

                                    double matchFitness1 = computeMatchFitness(
                                        i1->first, u1, i8->first, u8);
                                    double matchFitness2 = computeMatchFitness(
                                        i2->first, u2, i8->first, u8);
                                    double matchFitness3 = computeMatchFitness(
                                        i3->first, u3, i8->first, u8);
                                    double matchFitness4 = computeMatchFitness(
                                        i4->first, u4, i8->first, u8);
                                    double matchFitness5 = computeMatchFitness(
                                        i5->first, u5, i8->first, u8);
                                    double matchFitness6 = computeMatchFitness(
                                        i6->first, u6, i8->first, u8);
                                    double matchFitness7 = computeMatchFitness(
                                        i7->first, u7, i8->first, u8);
                                    MapBitSet tmp =
                                        MapSetUnion(u1->maps, u2->maps);
                                    tmp = MapSetUnion(tmp, u3->maps);
                                    tmp = MapSetUnion(tmp, u4->maps);
                                    tmp = MapSetUnion(tmp, u5->maps);
                                    tmp = MapSetUnion(tmp, u6->maps);
                                    ...
                                    if (MapSetCount(tmp) &&
                                        ...
                                        matchFitness5 > fitnessThreshold &&
                                        matchFitness6 > fitnessThreshold &&
                                        matchFitness7 > fitnessThreshold) {
                                      // match 8 players
                                      sendMatchInfo(
                                          i1->first, i2->first, i3->first,
                                          i4->first, i5->first, i6->first,
                                          i7->first, i8->first, u1, u2, u3, u4,
                                          u5, u6, u7, u8, 8, ladderID);
                                      break;
                                    }
                                  }
                                }
    ...
    if (bestUser && numPlayers == 2) {
      ...
      DBGMSG("Matching " << i1->first << " with " << bestName
                         ...
                         << "\n"
                            "\tping in ms: "
                         << sqrt(1000000 * calcPingDelta(u1, bestUser) /
                                 (255 * 255 * 2))
                         << "\n"
                            "\tprevious attempts: "
                         << u1->widened << ", " << bestUser->widened);
      sendMatchInfo(i1->first, bestName, "", "", "", "", "", "", u1, bestUser,
                    NULL, NULL, NULL, NULL, NULL, NULL, 2, ladderID);
      break;
    }
  }  // for i1
...
GeneralsUser *GeneralsMatcher::findUserInAnyLadder(const std::string &who) {
  for (LadderMap::iterator lIt = m_ladders.begin(); lIt != m_ladders.end();
       ++lIt) {
    UserMap::iterator uIt = lIt->second.find(who);
    if (uIt != lIt->second.end()) return uIt->second;
  }
  return NULL;
}

GeneralsUser *GeneralsMatcher::findUserInLadder(const std::string &who,
                                                int ladderID) {
  LadderMap::iterator lIt = m_ladders.find(ladderID);
  if (lIt == m_ladders.end()) return NULL;

  UserMap::iterator uIt = lIt->second.find(who);
  if (uIt == lIt->second.end()) return NULL;
...
GeneralsUser *GeneralsMatcher::findNonLadderUser(const std::string &who) {
  ...
  if (it != m_nonLadderUsers1v1.end()) return it->second;

  it = m_nonLadderUsers2v2.find(who);
  if (it != m_nonLadderUsers2v2.end()) return it->second;

  it = m_nonLadderUsers3v3.find(who);
  if (it != m_nonLadderUsers3v3.end()) return it->second;

  it = m_nonLadderUsers4v4.find(who);
  if (it != m_nonLadderUsers4v4.end()) return it->second;
...
GeneralsUser *GeneralsMatcher::findNonMatchingUser(const std::string &who) {
  ...
  if (it == m_nonMatchingUsers.end()) return NULL;
...
void GeneralsMatcher::addUserInLadder(const std::string &who, int ladderID,
                                      GeneralsUser *user) {
  m_ladders[ladderID][who] = user;
}

void GeneralsMatcher::addNonLadderUser(const std::string &who,
                                       GeneralsUser *user) {
...
void GeneralsMatcher::addNonMatchingUser(const std::string &who,
                                         GeneralsUser *user) {
...
GeneralsUser *GeneralsMatcher::removeUserInLadder(const std::string &who,
                                                  int ladderID) {
  LadderMap::iterator lIt = m_ladders.find(ladderID);
  if (lIt == m_ladders.end()) return NULL;
...
void GeneralsMatcher::handleRoomMessage(const char *nick, const char *message,
...
void GeneralsMatcher::handlePlayerMessage(const char *nick, const char *message,
                                          MessageType messageType) {
  if (messageType == ActionMessage) {
    DBGMSG(nick << " " << message);
  } else {
  ...
  if (messageType != NormalMessage) return;
...
void GeneralsMatcher::handlePlayerLeft(const char *nick) {
  ...
  if (m_nick != nick) removeUser(nick);
}

void GeneralsMatcher::handlePlayerChangedNick(const char *oldNick,
                                              const char *newNick) {
  DBGMSG("Player " << oldNick << " changed nick to " << newNick
                   << " - resetting to non-matching state");
  removeUser(oldNick);
  addUser(newNick);
}

void GeneralsMatcher::handlePlayerEnum(bool success, int gameSpyIndex,
                                       const char *nick, int flags) {
  if (!nick) nick = "";
  DBGMSG("PlayerEnum: success=" << success << " index=" << gameSpyIndex
...
void GeneralsClientMatcher::handleRoomMessage(const char *nick,
                                              const char *message,
                                              MessageType messageType) {}
void GeneralsClientMatcher::handlePlayerMessage(const char *nick,
                                                const char *message,
                                                MessageType messageType) {}
void GeneralsClientMatcher::handlePlayerJoined(const char *nick) {}
void GeneralsClientMatcher::handlePlayerLeft(const char *nick) {}
void GeneralsClientMatcher::handlePlayerChangedNick(const char *oldNick,
                                                    const char *newNick) {}
void GeneralsClientMatcher::handlePlayerEnum(bool success, int gameSpyIndex,
                                             const char *nick, int flags) {}
