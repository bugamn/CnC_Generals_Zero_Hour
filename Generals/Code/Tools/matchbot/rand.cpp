...
static const double theMultFactor =
    1.0 / (pow(2, 8 * sizeof(unsigned int)) - 1.0);
...
int RandClass::Int(void) { return (int)randomValue(); }
...
double RandClass::Double(void) { return Double(0.0, 1.0); }

double RandClass::Double(double low, double high) {
  double delta = high - low;
  double rval;

  if (delta <= 0.0) return high;
