...
bit8 Wtime::ParseDate(char *in) {
  ...
  for (i = 0; i < 7; i++)  // parse day of week
    if (strncmp(ptr, DAYS[i], strlen(DAYS[i])) == 0) t.tm_wday = i;
...
void Wtime::PrintDate(char *out) const {
  sprintf(out, "%s, %lu %s %lu", DAYS[GetWDay() - 1], GetMDay(),
          MONTHS[GetMonth() - 1], GetYear());
}

uint32 Wtime::GetSec(void) const { return (sec); }

uint32 Wtime::GetUsec(void) const { return (usec); }

void Wtime::SetSec(uint32 newsec) { sec = newsec; }

void Wtime::SetUsec(uint32 newusec) { usec = newusec; }
...
bit8 Wtime::GetSign(void) const { return (sign); }
