...
static char *Program_Usage =
    "A config filename can be given on the command line "
    "(default=matchbot.cfg)\n";
...
int main(int argc, char **argv) {
  ...
  if (paranoid_output_file != "STDOUT") {
    ...
    if (!paranoid_output_device) {
      cerr << "Could not open " << paranoid_output_file.get() << " for writing!"
           << endl;
      exit(-1);
    }
  } else {
  ...
  if (s_generalsMatcher) delete s_generalsMatcher;
  if (s_generalsClientMatcher) delete s_generalsClientMatcher;
  ...
  while (1) {
    ...
    if ((timeofday > 0) && (timeofday <= 300)) {
      ...
      sprintf(filenamebuf, "%s/%02d%02d%04d_%02d%02d%02d_log", logpath.get(),
              xtime.getMonth(), xtime.getMDay(), xtime.getYear(),
...
void paranoidLogMonitor(void *) {
  ...
  while (1) {
    ...
    if ((timeofday > 0) && (timeofday <= 300)) {
      ...
      MyMsgManager::ReplaceAllStreams((FileD *)paranoid_output_device,
                                      logfilename.get(), newfilename.get());
      Wstring logpath = "logs";
