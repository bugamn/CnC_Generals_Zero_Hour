...
bit8 ConfigFile::readFile(FILE *in) {
  ...
  while (fgets(string, 256, in)) {
    ...
    if (strlen(sectionname)) key.cat(sectionname);
...
//
bit8 ConfigFile::enumerate(int &index, int &offset, Wstring &key,
                           Wstring &value, IN char *section) const {
  int seclen = strlen(section);
  while (1) {
    Critsec_.lock();
    if (Dictionary_.iterate(index, offset, key, value) ==
        FALSE)  // out of keys?
    {
      Critsec_.unlock();
      return (FALSE);
    ...
    if (strlen(section) + 2 >=
        strlen(key.get()))  // key should have form: X[section]
      continue;

    // Is this key part of our section?
    const char *keystr = key.get() + strlen(key.get()) - seclen - 1;
    if (strncmp(keystr, section, strlen(section)) == 0) break;
  }
...
bit8 ConfigFile::getString(IN Wstring &_key, Wstring &value,
...
// Get a config entry as a string
bit8 ConfigFile::getString(IN char *key, Wstring &value,
                           IN char *section) const {
  Wstring sKey;
  sKey.set(key);
...
bit8 ConfigFile::getInt(IN Wstring &_key, sint32 &value,
                        IN char *section) const {
  Wstring key(_key);
  key.toUpper();
...
// Get a config entry as an integer
bit8 ConfigFile::getInt(IN Wstring &_key, sint16 &value,
                        IN char *section) const {
  Wstring key(_key);
  key.toUpper();
...
bit8 ConfigFile::setString(IN Wstring &_key, IN Wstring &value,
                           IN char *section) {
  Wstring key(_key);
  key.toUpper();
  ...
  for (i = 0; i < sectionList.length(); i++) {
    ...
    if (!strcmp(test.get(), section)) break;
  }
  if (i == sectionList.length()) {
    // New section!
    // DBGMSG("New section " << section << ", " << sectionList.length() << "
    // entries");
    test.set(section);
