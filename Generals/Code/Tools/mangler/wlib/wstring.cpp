...
Wstring::Wstring(IN char *string) : str(NULL), strsize(0) { set(string); }
...
bit8 Wstring::operator==(IN char *other) RO {
  if ((str == NULL) && (other == NULL)) return (TRUE);
...
bit8 Wstring::operator==(IN Wstring &other) RO {
  if ((str == NULL) && (other.str == NULL)) return (TRUE);
...
char Wstring::remove(sint32 pos, sint32 count) {
  ...
  if (pos + count > len) pos = len - count;
  if (pos < 0) {
...
void Wstring::cellCopy(char *dest, uint32 len) {
  ...
  for (i = (uint32)strlen(str); i < len; i++) dest[i] = ' ';
  dest[len] = 0;
}
...
bit8 Wstring::insert(char *instring, uint32 pos) {
  if (str == NULL) return (set(instring));
...
bit8 Wstring::replace(IN char *replaceThis, IN char *withThis) {
  ...
  while (src && src[0]) {
    ...
    if (foundStr) {
      ...
      if (len) {
        if (!dest.cat(len, src)) return (FALSE);
      }
      if (!dest.cat(withThis)) return (FALSE);
      src = foundStr + strlen(replaceThis);
    } else {
      if (!dest.cat(src)) return (FALSE);
...
bit8 Wstring::set(char c, uint32 index) {
  if (index >= (uint32)strlen(str)) return FALSE;
...
void Wstring::toLower(void) {
  ...
  for (i = 0; i < (uint32)strlength; i++) {
    if ((str[i] >= 'A') && (str[i] <= 'Z')) str[i] = (sint8)tolower(str[i]);
  }
}
...
void Wstring::toUpper(void) {
  ...
  for (i = 0; i < (uint32)strlength; i++) {
    if ((str[i] >= 'a') && (str[i] <= 'z')) str[i] = (sint8)toupper(str[i]);
  }
}
...
sint32 Wstring::getToken(int offset, char *delim, Wstring &out) RO {
  ...
  for (i = offset; i < (int)length(); i++) {
    if (strchr(delim, str[i]) == NULL) break;
  }
  if (i >= (int)length()) return (-1);
  start = i;
...
sint32 Wstring::getLine(int offset, Wstring &out) {
  ...
  start = i = offset;
  if (start >= (sint32)length()) return (-1);
