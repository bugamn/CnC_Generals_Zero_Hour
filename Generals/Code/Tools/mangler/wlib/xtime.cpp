...
static char *FULLDAYS[] = {"Sunday",   "Monday", "Tuesday", "Wednesday",
                           "Thursday", "Friday", "Saturday"};

static char *MONTHS[] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
                         "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};

static char *FULLMONTHS[] = {"January",   "February", "March",    "April",
...
static sint32 Get_Day(int month, int day, int year) {
  ...
  static int DaysAtMonth[] = {0,   31,  59,  90,  120, 151,
...
static bit8 Get_Date_From_Day(sint32 days, OUT sint32 &year, OUT sint32 &yday) {
  ...
  while (days < 0 || days >= (IS_LEAP(y) ? 366 : 365)) {
    ...
    days -=
        ((yg - y) * 365 + LEAPS_THRU_END_OF(yg - 1) - LEAPS_THRU_END_OF(y - 1));
    y = yg;
  }
  year = y;
  yday = days + 1;  // 1 based
...
bit8 Xtime::FormatTime(char *out, char *format) {
  ...
  while (*ptr != 0) {
    ...
    /*else if (strncmp(ptr,"w",1)==0)
    {
      ...
      sprintf(out + strlen(out), "%ld",
              ((getMonth() - 1) / 4) + 1);  // GetQuarter
      ptr += 1;
...
sint32 Xtime::getDay(void) const { return (day_); }

// Get msecs since start of day
sint32 Xtime::getMsec(void) const { return (msec_); }

// Set days since year 0
void Xtime::setDay(sint32 newday) { day_ = newday; }

// Set msec since start of this day
void Xtime::setMsec(sint32 newmsec) { msec_ = newmsec; }
...
bit8 Xtime::getTimeval(struct timeval &tv) {
  ...
  if ((day_ < 719528) || (day_ >= 719528 + 24855)) return (FALSE);
...
// Set the time
//
bit8 Xtime::setTime(int month, int mday, int year, int hour, int minute,
                    int second) {
...
int Xtime::getYDay(void) const  // Day of Year  (1-366)  (366 = leap yr)
{
  int year;
  sint32 dayofyear;
  if (Get_Date_From_Day(day_, year, dayofyear) == FALSE) return (-1);
  return dayofyear;
}
...
//
bit8 Xtime::getTime(int &month, int &mday, int &year, int &hour, int &minute,
                    int &second) const {
  int i;
  sint32 dayofyear;
  if (Get_Date_From_Day(day_, year, dayofyear) == FALSE) return (FALSE);
  ...
  bit8 isleap = IS_LEAP(year);
  for (i = 0; i < 12; i++) {
    if (DaysAtMonth[isleap][i] >= dayofyear) break;
    month = i;
  }
  month++;  // 1 based
