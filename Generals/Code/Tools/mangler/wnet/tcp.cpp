...
TCP::TCP(int new_mode) {
  ...
  fd = -1;
  clientCount = 0;
  if ((new_mode == CLIENT) || (new_mode == SERVER)) mode = new_mode;
  FD_ZERO(&clientList);
...
char *TCP::Gets(char *string, int n, int whichFD) {
  ...
  if (whichFD == 0) whichFD = GetFD();

  if (whichFD <= 0) return (NULL);
...
sint32 TCP::EncapsulatedRead(uint8 *msg, uint32 len, sint32 whichFD) {
  ...
  if (mode == CLIENT)
    ...
    if ((whichFD > maxFD) || (!FD_ISSET(whichFD, &clientList))) return (0);
  } else
    return (-1);
...
sint32 TCP::CloseAll(void) {
  ...
  if (mode == CLIENT) return (Close());

  for (i = 0; i <= maxFD; i++) {
    if ((i != fd) && (FD_ISSET(i, &clientList))) Close(i);
  }
...
int TCP::Wait(sint32 sec, sint32 usec, fd_set &returnSet, sint32 whichFD) {
  ...
  if (mode == SERVER) {
    if (whichFD == 0) {
      inputSet = clientList;
      if (fd > 0) FD_SET(fd, &inputSet);
    } else if (whichFD > 0)
      FD_SET(whichFD, &inputSet);
  } else if (mode == CLIENT) {
    if (whichFD == 0) whichFD = fd;
    if (whichFD > 0) FD_SET(whichFD, &inputSet);
  }
...
int TCP::Wait(sint32 sec, sint32 usec, fd_set &givenSet, fd_set &returnSet) {
  ...
  if ((sec == -1) && (usec == -1)) noTimeout = TRUE;
  ...
  {
    if (FD_ISSET(i, &givenSet)) givenMax = i;
  }
  ...
  while (!done) {
    ...
    else if ((retval == -1) && (errno == EINTR))  // in case of signal
    {
      ...
      if ((noTimeout == FALSE) && (timenow.GetSec() == 0) &&
          (timenow.GetUsec() == 0))
        done = 1;
      else
        returnSet = backupSet;
...
bit8 TCP::Bind(char *Host, uint16 port, bit8 reuseAddr) {
  ...
  if (isdigit(Host[0])) return (Bind(ntohl(inet_addr(Host)), port, reuseAddr));
...
bit8 TCP::Bind(uint32 IP, uint16 Port, bit8 reuseAddr) {
  ...
  if (retval == -1) ERRMSG("Couldn't set nonblocking mode!");
  ...
  if (reuseAddr == TRUE) {
    ...
    /******************  this may make the socket get garbage data??
        opval=1;
        retval=setsockopt(fd,SOL_SOCKET,SO_REUSEPORT,(char
    *)&opval,sizeof(opval)); if (retval!=0) fprintf(stderr,"Could not set socket
    to SO_REUSEPORT\n");
    **********************/
#endif
#ifdef SO_REUSEADDR
    opval = 1;
    retval =
        setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (char *)&opval, sizeof(opval));
    if (retval != 0) fprintf(stderr, "Could not set socket to SO_REUSEADDR\n");
#endif
  }
  ...
  if (retval == -1) {
    status = GetStatus();
    DBGMSG("Bind failure (" << status << ") IP " << IP << " PORT "
                            << ntohs(Port));
    return (FALSE);
  }
  myIP = IP;
...
bit8 TCP::Connect(char *Host, uint16 port) {
  ...
  if (isdigit(Host[0])) return (Connect(ntohl(inet_addr(Host)), port));
...
bit8 TCP::Connect(uint32 IP, uint16 Port) {
  ...
  if (mode != CLIENT) {
    ERRMSG("Can't connect in server mode");
    return (FALSE);
  }
...
bit8 TCP::ConnectAsync(char *Host, uint16 port) {
  ...
  if (isdigit(Host[0])) return (ConnectAsync(ntohl(inet_addr(Host)), port));
...
bit8 TCP::ConnectAsync(uint32 IP, uint16 Port) {
  ...
  if ((result == -1) &&
      ((status == BADF) || (status == NOTSOCK) || (status == INVAL))) {
    Close();
