...
int main(int argc, char **argv) {
  ...
  if (hostIPStr.length()) {
    INFMSG("Binding to " << hostIPStr.get() << ":" << port << "-"
                         << (port + 3));
  ...
  while (1) {
    ...
    retval = udp.Read(buf, packet_size,
                      &addr);  // Wait until there is something on the socket
    if (retval > 0) {
      ManglerData *packet = (ManglerData *)buf;
      theAddr = (unsigned char *)&(addr.sin_addr.s_addr);
      if (retval != packet_size) {
        WRNMSG("Recieved mis-sized packet ("
               << retval << " bytes) from " << theAddr[0] << "." << theAddr[1]
               << "." << theAddr[2] << "." << theAddr[3] << ":"
               << addr.sin_port);
      } else {
        ...
        packet->MyMangledPortNumber =
            addr.sin_port;  // not changing to host order, cause its in network
                            // byte order now, and the game will expect it to
                            // stay that way.
        ...
        udp.Write(buf, packet_size, ntohl(addr.sin_addr.s_addr),
                  ntohs(addr.sin_port));
        INFMSG("Saw " << (int)theAddr[0] << "." << (int)theAddr[1] << "."
                      << (int)theAddr[2] << "." << (int)theAddr[3] << ":"
                      << ntohs(addr.sin_port) << ((blitz) ? " Blitzed" : ""));

        if (blitz) {
          udp2.Write(buf, packet_size, ntohl(addr.sin_addr.s_addr),
                     ntohs(addr.sin_port) + 1);
          udp3.Write(buf, packet_size, ntohl(addr.sin_addr.s_addr),
                     ntohs(addr.sin_port) + 2);
          udp4.Write(buf, packet_size, ntohl(addr.sin_addr.s_addr),
                     ntohs(addr.sin_port) + 3);
        }
      }
    }
  }
