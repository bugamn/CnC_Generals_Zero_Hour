...
LRESULT CALLBACK GeneralsSetupDialogProc(HWND hDlg, UINT message, WPARAM wParam,
                                         LPARAM lParam) {
  switch (message) {
    case WM_INITDIALOG: {
...
void updateDisplay(HWND hDlg) {
  ...
  if (g_wolapiInstalled) {
    char buf[200];
    sprintf(buf, "%d.%d (%s)", MAJOR(g_wolapiRealVersion),
            MINOR(g_wolapiRealVersion), g_wolapiRealFilename);
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam)) {
        ...
        case IDC_SETUP_GENERALS: {
          DialogBox(g_hInst, (LPCTSTR)IDD_GENERALSSETUPBOX, hDlg,
                    (DLGPROC)GeneralsSetupDialogProc);
          updateDisplay(hDlg);
          break;
        }
        case IDC_UNINSTALL_GENERALS: {
          if (MessageBox(
            ...
            MessageBox(hDlg, "Oops! Can't do that yet!", "Unimplemented",
                       MB_OK);
            updateDisplay(hDlg);
          }
          break;
