...
#include <snmp.h>
#include <winreg.h>
extern CComModule _Module;  // Required for COM - must be between atlbase.h and
                            // atlcom.h.  Funky, no?
#include <atlcom.h>
#include <stdarg.h>
#include <stdio.h>
...
#define GENERALS_REG_KEY_PATH \
  "SOFTWARE\\Westwood\\Generals"  ///< Generals registry key
#define GENERALS_REG_KEY_BOTTOM \
  GENERALS_REG_KEY_PATH         \
...
#define GENERALS_REG_KEY_INSTALLPATH \
  "InstallPath"                           ///< Install path registry key
#define GENERALS_REG_KEY_SERIAL "Serial"  ///< Serial # registry key
#define GENERALS_REG_KEY \
  GENERALS_REG_KEY_TOP   \
      "\\" GENERALS_REG_KEY_BOTTOM  ///< Full Generals registry path

#define WOLAPI_REG_KEY_TOP "HKEY_LOCAL_MACHINE"  ///< Registry base
#define WOLAPI_REG_KEY_PATH \
  "SOFTWARE\\Westwood\\WOLAPI"  ///< WOLAPI registry key
#define WOLAPI_REG_KEY_BOTTOM \
  WOLAPI_REG_KEY_PATH "\\"  ///< WOLAPI registry key with trailing backslashes
...
#define WOLAPI_REG_KEY \
  WOLAPI_REG_KEY_TOP "\\" WOLAPI_REG_KEY_BOTTOM  ///< Full WOLAPI registry path

#define DLL_REG_KEY_TOP "HKEY_CLASSES_ROOT"  ///< Registry base
#define DLL_REG_KEY_PATH \
  "CLSID\\{18FD6763-F5EA-4fa5-B2A9-668554152FAE}\\InprocServer32"  ///< WOLAPI
                                                                   ///< registry
                                                                   ///< key
#define DLL_REG_KEY_BOTTOM \
  DLL_REG_KEY_PATH "\\"  ///< WOLAPI registry key with trailing backslashes
...
void getPathsFromRegistry(void) {
  ...
  if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, GENERALS_REG_KEY_PATH, 0, KEY_ALL_ACCESS,
                   &handle) == ERROR_SUCCESS) {
    returnValue =
        RegQueryValueEx(handle, GENERALS_REG_KEY_INSTALLPATH, NULL, &type,
                        (unsigned char *)&g_generalsFilename, &size);
  ...
  if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, GENERALS_REG_KEY_PATH, 0, KEY_ALL_ACCESS,
                   &handle) == ERROR_SUCCESS) {
    returnValue = RegQueryValueEx(handle, GENERALS_REG_KEY_SERIAL, NULL, &type,
                                  (unsigned char *)&g_generalsSerial, &size);
  ...
  if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, WOLAPI_REG_KEY_PATH, 0, KEY_ALL_ACCESS,
                   &handle) == ERROR_SUCCESS) {
    returnValue =
        RegQueryValueEx(handle, WOLAPI_REG_KEY_INSTALLPATH, NULL, &type,
                        (unsigned char *)&g_wolapiRegFilename, &size);
  ...
  if (RegOpenKeyEx(HKEY_CLASSES_ROOT, DLL_REG_KEY_PATH, 0, KEY_ALL_ACCESS,
                   &handle) == ERROR_SUCCESS) {
    returnValue =
        RegQueryValueEx(handle, DLL_REG_KEY_LOCATION, NULL, &type,
                        (unsigned char *)&g_wolapiRealFilename, &size);
...
void setupGenerals(const char *genPath, const char *genSerial) {
  ...
  if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, GENERALS_REG_KEY_PATH, 0, "REG_NONE",
                     REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &handle,
                     NULL) == ERROR_SUCCESS) {
    type = REG_SZ;
    size = strlen(genPath) + 1;
    returnValue = RegSetValueEx(handle, GENERALS_REG_KEY_INSTALLPATH, 0, type,
                                (unsigned char *)genPath, size);

    size = strlen(genSerial) + 1;
    returnValue = RegSetValueEx(handle, GENERALS_REG_KEY_SERIAL, 0, type,
                                (unsigned char *)genSerial, size);

    size = strlen("Generals") + 1;
    returnValue = RegSetValueEx(handle, GENERALS_REG_KEY_NAME, 0, type,
                                (unsigned char *)"Generals", size);

    type = REG_DWORD;
    size = sizeof(DWORD);
    unsigned long value = 65536;
    returnValue = RegSetValueEx(handle, GENERALS_REG_KEY_VERSION, 0, type,
                                (unsigned char *)&value, size);
    value = 12544;
    returnValue = RegSetValueEx(handle, GENERALS_REG_KEY_SKU, 0, type,
                                (unsigned char *)&value, size);
...
void checkInstalledWolapiVersion(void) {
  ...
  CoCreateInstance(CLSID_Chat, NULL, CLSCTX_INPROC_SERVER, IID_IChat,
                   (void **)&g_pChat);
