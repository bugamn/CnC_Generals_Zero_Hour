...
wchar_t *localeStringsMissing[MISSING_STRING_HINTS_MAX] = {
    {L"0 MissingInstall"},        {L"1 MissingExplore"},
    {L"2 MissingPreviews"},       {L"3 MissingCancel"},
    {L"4 MissingAutoRunTitle"},   {L"5 MissingNoExplorer"},
    {L"6 MissingWinVersionText"}, {L"7 MissingWinVersionTitle"},
    {L"8 MissingCantFind"},       {L"9 MissingUninstall"},
    {L"10 MissingWebsite"},       {L"11 MissingCheckForUpdates"},
    {L"12 MissingWorldBuilder"},  {L"13 MissingPlay"},
    {L"14 MissingGameTitle"},     {L"15 MissingFullGameTitle"},
    {L"16 MissingRegistryKey"},   {L"17 MissingMainWindow"},
    {L"18 MissingThing"},         {L"19 UNKNOWN MESSAGE"}};
...
#define LANGUAGE_IS_DBCS(l)                        \
  (((l) == IDL_JAPANESE) || ((l) == IDL_KOREAN) || \
   ((l) == IDL_CHINESE))                                              // [OYO]
...
// make using the single vs. multi language files more transparent to the
// program.
//=============================================================================
...
int Locale_Init(int language, char *file) {
  ...
  /*
          ...
          LOCALE_init();
                  ...
                  //---------------------------------------------------------------------
                  // Create a file buffer that holds all the strings in the
  file.
                  //---------------------------------------------------------------------
                  long	filesize;
                  HRSRC 	hRsrc;
                          ...
                          case 1252:
                                  ...
                                  switch( LanguageID ) {
                                          ...
                                          case LANG_GERMAN:
                                                  ...
                                                  SubLanguage		=
  SUBLANG_GERMAN; break;

                                          case LANG_FRENCH:
                                                  PrimaryLanguage = LANG_FRENCH;
                                                  SubLanguage		=
  SUBLANG_FRENCH; break;

                                          case LANG_ENGLISH:
                                                  PrimaryLanguage =
  LANG_ENGLISH; SubLanguage		= SUBLANG_ENGLISH_US; break;
                                  }
                                  break;
                  }

                  hRsrc = FindResourceEx( module, RT_RCDATA, "STRINGS",
  MAKELANGID( PrimaryLanguage, SubLanguage )); if ( hRsrc == NULL ) { hRsrc =
  FindResourceEx( module, RT_RCDATA, "STRINGS", MAKELANGID( LANG_ENGLISH,
  SUBLANG_ENGLISH_US ));
                  }
                  if ( hRsrc ) {
...
const char *Locale_GetString(int StringID, char *String) {
  ...
  WideCharToMultiByte(CodePage, 0, wide_buffer, _MAX_PATH, buffer, _MAX_PATH,
                      NULL, NULL);
...
const wchar_t* Locale_GetString( int StringID, wchar_t *String )
{
        ...
        memset(	wide_buffer, '\0', _MAX_PATH );
                ...
                if (TheGameText != NULL)
                        localeStr = (wchar_t *)TheGameText->fetch(
s_stringLabels[StringID] );

                if (localeStr == NULL)
                {
                        return localeStringsMissing[ min(
MISSING_STRING_HINTS_MAX - 1, StringID ) ];
                }
                else
                {
