 ...
 **
 *																			*
 *	Clip_Line_To_Rect			--	Clips a line (two
 *points) against a		* rectangle, using CS algorithm. * Compute_Code
 *--	computes line clipping bit code for 	* point & rectangle.
 ** Copy_File 					--	Copies a file from one
 *dir to another.	* Convert_Hex_To_Version		--	Converts
 *a hex num obtained from the	* Registry, into a string
 ** representation of a version 		* number ( XX.XX ).
 ** Convert_Version_To_Hex		--	Converts a string to an unsigned
 *long.	* Convert_To_Version_Format	--	Converts version string's "," to
 *"."s   * Dialog_Box 					--	draws a dialog
 *background box           * Draw_Box 					--
 *Displays a highlighted box.             * Fatal
 *--	General purpose fatal error handler.	* Get_Version
 *--	Retrieves a version string from a file. * Get_String
 *--	Returns a pointer to the undipped text. * Is_File_Available
 *--	Use both FindFirst to check that CD is	* in drive & if File_Exists() to
 ** determine if file is really there.	* Pad_With_Zeros            	--
 *Adds zeros to the beginning of string.	* String_Width
 *--	Calculate with of the string.			* Strip_Newlines
 *--	Remove '\r' from string passed in.		* TextPtr
 *--	Returns a pointer to the undipped text. * Path_Name_Valid
 *--	Validate that the path has the correct  * number of chars between '\' in
 *the	* path.								*
 *	Path_Get_Next_Directory		--	Return the next dir path from
 *string.	* Path_Add_Back_Slash			--	Add a "\\" to
 *the end of the string.	* Path_Remove_Back_Slash		--
 *Remove a '\\' at the end of the string.	* Path_Trim_Blanks
 *--	Trim lead/trail white spaces off string	* Pad_With_Zeros
 *--	Adds zeros to the beginning of string.  * Remove_Ending_Spaces
 *--	Remove any blank spaces at end of string* Remove_Spaces
 *--	Remove spaces from string passed in.    *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
//			 Some dialogs mistake a single "&" for an accelerator
//key.
//
...
////////////////UnicodeString Fix_Single_Ampersands( UnicodeString string, bool
///upper_case)
////////////////{
 ...
 **
 *																			*
 * HISTORY: * 04/13/1998  ML/MG : Created. *
 *==========================================================================*/
 ...
 **
 *																			*
 * HISTORY: * 10/08/2001  MML : Created. *
 *==========================================================================*/
...
void PlugInProductName(char *szString, int nName) {
  ...
  //	LoadString( Main::hInstance, nName, szProduct, MAX_PATH );
          ...
          switch( nCount ) {
                  ...
                  case 3:
                          sprintf( szOut, szTextBuf, szProduct, szProduct,
  szProduct ); break; case 4: sprintf( szOut, szTextBuf, szProduct, szProduct,
  szProduct, szProduct ); break; case 5: sprintf( szOut, szTextBuf, szProduct,
  szProduct, szProduct, szProduct, szProduct, szProduct ); break;
          }
