 ...
 ** TTFontClass::Char_Pixel_Width -- Fetch the pixel width of the character
 *specified.			* TTFontClass::Find_Text_VLength -- Finds length
 *of text in pixels							*
 *   TTFontClass::Get_Height -- Fetch the normalized height of the nominal font
 *character.		* TTFontClass::Get_Width -- Get normalized width of the
 *nominal font character.				* TTFontClass::Print --
 *Print text to the surface specified.
 ...
 ** TTFontClass::String_Pixel_Width -- Determines the width of the string in
 *pixels.			*
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - -*/
...
// unsigned long TEXT_COLOR			  		= RGB( 247, 171,
// 11 ); unsigned long SHADOW_COLOR		  		= RGB(  40,   8,
// 8 );
 ...
 **
 *																					*
 *	This constructs a font object as it is based upon the true type windows
 *fonts.	*
 *																					*
 ...
 **
 *                                                                                  *
 * HISTORY: * 08/24/1999 MML : Created. *
 *==================================================================================*/
TTFontClass::TTFontClass(HDC hdc, char *filename, char *facename, int height,
                         int weight, BYTE charset, int width, int escapement,
                         int orientation, BYTE italic, BYTE underline,
                         BYTE strikeout, BYTE outputPrecision,
                         BYTE clipPrecision, BYTE quality,
                         BYTE pitchAndFamily) {
  HGDIOBJ old_object;
  TEXTMETRIC tm;
  ...
  Msg(__LINE__, __FILE__, "TTFontClass -- filename=%s, facename=%s, height=%d.",
      filename, facename, height);

  //--------------------------------------------------------------------------
  // Make sure the font file is "Registered".  Then create the font.
  //--------------------------------------------------------------------------
  AddFontResource(szFilename);
  Font = CreateFont(height, width, escapement, orientation, weight, italic,
                    underline, strikeout, charset, outputPrecision,
                    clipPrecision, quality, pitchAndFamily, szFacename);
  ...
  if (hdc) {
    ...
    CharSet =
        tm.tmCharSet;  // [OYO] It is important to support Double Byte Chars
    SelectObject(hdc, old_object);
 ...
 *                                                                                             *
 * WARNINGS:   The return width is the screen real estate width which may differ
 *from the      * actual pixels of the character. This difference is controlled
 *by the font       * X spacing. *
 *                                                                                             *
 * HISTORY: * 05/26/1997 JLB : Created. *
 *=============================================================================================*/
 ...
 *    This will return with the pixel width of the character specified.
 *
 * INPUT:  		HDC hdc		  		-- must be passed in
 *from calling function. char const * string	-- the pointer to the character
 *in the string. int *num_bytes		-- return number of bytes this character
 *has: 1 or 2.
 *
 * OUTPUT:		Returns with the pixel width of the character.
 *
 * WARNINGS:	The return width is the screen real estate width which may
 *differ from the actual pixels of the character. This difference is controlled
 *by the font X spacing.
 *
 * NOTE:		This is function determines if the character is a
 *double-byte or single-byte character, then calls the standard
 *Char_Pixel_Width.
 *
 * HISTORY:
...
//
int TTFontClass::Char_Pixel_Width(HDC hdc, char const *string,
                                  int *num_bytes) const {
  ...
  if (IsFontDBCS() && IsDBCSLeadByte(*letter)) {
    ...
    length +=
        Char_Pixel_Width(hdc, c);  // pixel length of double-byte character.
  } else {
    length += Char_Pixel_Width(
        hdc, *letter);  // pixel length of single-byte character.
  }
  return (length);
}
 ...
 *pixels.				*
 *																								*
 *    This routine is used to determine how many pixels wide the string will be
 *if it were		* printed.
 **
 *																								*
 ...
 **
 *																								*
 * OUTPUT:		Returns with the number of pixels the string would span
 *if it were printed.		*
 *																								*
...
// [OYO]	Supports DBCS ( multi-byte characters ).
//
void TTFontClass::String_Pixel_Bounds(HDC hdc, const char *string,
                                      Rect &bounds) const {
  int width;
  int height;
...
UINT TTFontClass::Get_Double_Byte_Char(const char *string,
  ...
  //		To specify a different code page, use the IsDBCSLeadByteEx
  //function.
  //
 ...
 **
 *                                                                                             *
 *    This routine is used to fetch the width of the widest character in the
 *font but the      * width has been biased according to any X spacing override
 *present.                       *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the normalized width of the widest character in the
 *font.             *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 05/26/1997 JLB : Created. *
 *=============================================================================================*/
int TTFontClass::Get_Width(void) const { return (MaxCharWidth - Overhang); }

/***********************************************************************************************
 * TTFontClass::Get_Height -- Fetch the normalized height of the nominal font
 *character.       *
 *                                                                                             *
 *    This will return the height of the font but the returned height will be
 *adjusted by any  * Y spacing override present. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with the height of the font normalized by any spacing
 *overrides.           *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 05/26/1997 JLB : Created. *
 *=============================================================================================*/
int TTFontClass::Get_Height(void) const { return (Height); }

/***********************************************************************************************
 * TTFontClass::Set_XSpacing -- Set the X spacing override value. *
 *                                                                                             *
 *    Use this routine to control the horizontal spacing override for this font.
 *If the value  * is negative, the font becomes compressed. If the value is
 *positive, then the font        * becomes expanded. *
 *                                                                                             *
 ...
 * TTFontClass::Set_YSpacing -- Set the vertical (Y) spacing override value. *
 *                                                                                             *
 *    Use this routine to control the "line spacing" of a font. If the Y spacing
 *is negative   * then the font becomes closer to the line above it. If value is
 *positive, then more       * space occurs between lines. *
 *                                                                                             *
 ...
 **
 *                                                                                             *
 *    This displays text to the surface specified and with the attributes
 *specified.           *
 *                                                                                             *
 * INPUT:   string   -- The string to display to the surface. * surface  -- The
 *surface to display the text upon.                                  * cliprect
 *-- The clipping rectangle that both clips the text and biases the print   *
 *                      location to. * point    -- The draw position for the
 *upper left corner of the first character.    * convert  -- The pixel convert
 *object that is used to draw the correct colors to    * the destination
 *surface.                                               * remap    -- Auxiliary
 *remap table for font colors.                                 *
 *                                                                                             *
 * OUTPUT:  Returns with the point where the next print should begin if it is to
 *smoothly      * continue where this print operation left off. *
 *                                                                                             *
 * WARNINGS: There are two separate drawing routines; one for old fonts and one
 *for new fonts. *
 *                                                                                             *
 * HISTORY: * 05/26/1997 JLB : Created. * 06/20/1887 BNA : Modified to handle
 *new fonts.
 **
 *=============================================================================================*/
...
Point2D TTFontClass::Print(
    HDC hdc, wchar_t const *string, Rect const &cliprect,
    ...
    TextShadowType shadow) /* = TPF_FULLSHADOW		*/
{
  ...
  WideCharToMultiByte(CodePage, 0, string, length, buffer, _MAX_PATH, NULL,
                      NULL);
    ...
    TextShadowType shadow) /* = TPF_FULLSHADOW		*/
{
  ...
  Msg(__LINE__, __FILE__, "TTFontClass::Print -- rect = [%d, %d, %d, %d].",
      rect.left, rect.top, rect.right, rect.bottom);
  //	Msg( __LINE__, __FILE__, "TTFontClass::Print -- rect2 = [%d, %d, %d,
  //%d].", rect.left, rect.top, rect.right, rect.bottom );

  //--------------------------------------------------------------------------
  // Print the "Shadow" depending on style desired, then the text in the
  // requested color.
  //--------------------------------------------------------------------------
  if (hdc) {
    assert(Font != NULL);
    ...
    //		result = ExtTextOutW(
    //			hdc,				// handle to DC
    //			rect.left,			// x-coordinate of
    //reference point 			rect.top,			// y-coordinate of reference
    //point 			ETO_CLIPPED,		// text-output options 			&rect,
    //// optional dimensions 			string,				// string
    //			length,				// number of characters
    //in string 			NULL				// array of spacing values
    //		);
    ...
    // starting position 			string,				// character
    // string 			length				// number of characters
    // 		);

    result = DrawTextW(hdc, string, length, &rect, flag);
...
Point2D TTFontClass::Print(
    HDC hdc, char const *string, Rect const &cliprect,
    COLORREF forecolor,    /* = TEXT_COLOR,		*/
    COLORREF backcolor,    /* = TEXT_SHADOW_COLOR,	*/
    TextFormatType flag,   /* = TPF_DEFAULT,		*/
    TextShadowType shadow) /* = TPF_FULLSHADOW		*/
{
  ...
  //	Msg( __LINE__, __FILE__, "TTFontClass::Print -- strlen = %d.", length );
  //	Msg( __LINE__, __FILE__, "TTFontClass::Print -- rect = [%d, %d, %d,
  //%d].", rect.left, rect.top, rect.right, rect.bottom ); 	Msg( __LINE__,
  //__FILE__, "TTFontClass::Print -- rect2 = [%d, %d, %d, %d].", rect.left,
 ...
 *                                                                         *
 * INPUT:	BYTE *string - the strength to find the vertical length of * int
 *width    - the width of the region to fit it in        	   *
 *                                                                     	   *
 ...
 * WARNINGS:	none *
 *                                                                                             *
 * HISTORY: * 03/26/1998 MML : Created. *
 *=============================================================================================*/
FontManagerClass::FontManagerClass(HDC hdc) {
  ...
  if (hdc) {
    ...
    GetPrivateProfileString("Fonts", "Font", "Arial.tff", szFile, MAX_PATH,
                            szPath);
    GetPrivateProfileString("Fonts", "Fontname", "Arial", szFacename, MAX_PATH,
                            szPath);
    ...
    Msg(__LINE__, __FILE__, "FontManagerClass -- szFile = %s, szFilename = %s.",
        szFile, szFacename);
    ...
    switch (LanguageID) {  // [OYO] Add this line if you wish to support another
                           // languages
      ...
      case LANG_JAP:  // [OYO] Use MS PGothic for Japanese Win9x
        ...
        if (codepage == 932) {
          ...
          TTButtonFontPtr = new TTFontClass(hdc, szFile, szFacename, 20,
                                            FW_NORMAL, SHIFTJIS_CHARSET);
          TTButtonFontPtrSmall = new TTFontClass(hdc, szFile, szFacename, 12,
                                                 FW_NORMAL, SHIFTJIS_CHARSET);

          TTTextFontPtr = new TTFontClass(hdc, szFile, szFacename, 16,
                                          FW_MEDIUM, SHIFTJIS_CHARSET);
          TTTextFontPtr640 = new TTFontClass(hdc, szFile, szFacename, 14,
                                             FW_MEDIUM, SHIFTJIS_CHARSET);
          TTTextFontPtr800 = new TTFontClass(hdc, szFile, szFacename, 14,
                                             FW_MEDIUM, SHIFTJIS_CHARSET);
          TTLicenseFontPtr = new TTFontClass(hdc, szFile, szFacename, 12,
                                             FW_NORMAL, SHIFTJIS_CHARSET);
        }
        break;
      ...
      case LANG_KOR:  // [OYO] Use GulimChe for Korean Win9x
        ...
        if (codepage == 949) {
          ...
          TTButtonFontPtr = new TTFontClass(hdc, szFile, szFacename, 20,
                                            FW_NORMAL, HANGEUL_CHARSET);
          TTButtonFontPtrSmall = new TTFontClass(hdc, szFile, szFacename, 20,
                                                 FW_NORMAL, HANGEUL_CHARSET);

          TTTextFontPtr = new TTFontClass(hdc, szFile, szFacename, 16,
                                          FW_MEDIUM, HANGEUL_CHARSET);
          TTTextFontPtr640 = new TTFontClass(hdc, szFile, szFacename, 14,
                                             FW_MEDIUM, HANGEUL_CHARSET);
          TTTextFontPtr800 = new TTFontClass(hdc, szFile, szFacename, 14,
                                             FW_MEDIUM, HANGEUL_CHARSET);
      ...
      case LANG_CHI:
        ...
        if (codepage == 950) {
          ...
          TTButtonFontPtr = new TTFontClass(hdc, szFile, szFacename, 20,
                                            FW_NORMAL, CHINESEBIG5_CHARSET);
          TTButtonFontPtrSmall = new TTFontClass(
              hdc, szFile, szFacename, 14, FW_NORMAL, CHINESEBIG5_CHARSET);

          TTTextFontPtr = new TTFontClass(hdc, szFile, szFacename, 16,
                                          FW_MEDIUM, CHINESEBIG5_CHARSET);
          TTTextFontPtr640 = new TTFontClass(hdc, szFile, szFacename, 14,
                                             FW_MEDIUM, CHINESEBIG5_CHARSET);
          TTTextFontPtr800 = new TTFontClass(hdc, szFile, szFacename, 14,
                                             FW_MEDIUM, CHINESEBIG5_CHARSET);
      ...
      default:
        ...
        TTButtonFontPtr =
            new TTFontClass(hdc, szFile, szFacename, 22, FW_SEMIBOLD,
                            ANSI_CHARSET, 0, 0, 0, FALSE);

        if (LANG_FRE == LanguageID) {
          TTButtonFontPtrSmall =
              new TTFontClass(hdc, szFile, szFacename, 20, FW_SEMIBOLD,
                              ANSI_CHARSET, 0, 0, 0, FALSE);
        } else {
          TTButtonFontPtrSmall =
              new TTFontClass(hdc, szFile, szFacename, 22, FW_SEMIBOLD,
        ...
        TTTextFontPtr =
            new TTFontClass(hdc, szFile, szFacename, 16, FW_SEMIBOLD,
                            ANSI_CHARSET, 0, 0, 0, FALSE);
        TTTextFontPtr640 =
            new TTFontClass(hdc, szFile, szFacename, 14, FW_SEMIBOLD,
                            ANSI_CHARSET, 0, 0, 0, FALSE);
        TTTextFontPtr800 =
            new TTFontClass(hdc, szFile, szFacename, 14, FW_SEMIBOLD,
                            ANSI_CHARSET, 0, 0, 0, FALSE);
        TTLicenseFontPtr =
            new TTFontClass(hdc, szFile, szFacename, 12, FW_MEDIUM,
                            ANSI_CHARSET, 0, 0, 0, FALSE);
        break;
    }
 ...
 *                                                                                             *
 *    This routine will examine the specified flags and return with a pointer to
 *the font      * that the flags represent. *
 *                                                                                             *
