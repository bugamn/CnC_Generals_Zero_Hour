...
// #define	PRETEND_ON_CD_TEST				TRUE
// // should be FALSE
...
#define GAMESPY_WEBSITE                    \
  "http://www.gamespyarcade.com/features/" \
  "launch.asp?svcname=ccrenegade&distID=391"
...
// "C6D925A3-7A9B-4ca3-866D-8B4D506C3665"
...
void Error_Message(HINSTANCE hInstance, const char *title, const char *string,
                   char *path);

bool Is_On_CD(char *);
HBITMAP LoadResourceBitmap(HMODULE hInstance, char *lpString,
                           HPALETTE FAR *lphPalette,
                           bool loading_a_button = FALSE);
HBITMAP LoadResourceButton(HMODULE hInstance, char *lpString,
                           HPALETTE FAR lphPalette);
BOOL Options(Command_Line_Arguments *Orgs);
void Prog_End(void);
bool Prompt_For_CD(HWND window_handle, char *volume_name, const char *message1,
                   const char *message2, int *cd_drive);
void Reformat_Volume_Name(char *volume_name, char *new_volume_name);
int Show_Message(HWND window_handle, const char *message_num1,
...
BOOL CALLBACK Dialog_Box_Proc(HWND window_handle, UINT message, WPARAM w_param,
                              LPARAM l_param);
LRESULT CALLBACK Wnd_Proc(HWND hWnd, UINT iMessage, WPARAM wParam,
                          LPARAM lParam);
...
int PASCAL WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  ); MultiByteToWideChar( CP_ACP, MB_PRECOMPOSED, szPath, _MAX_PATH,
  szWideBuffer0, _MAX_PATH ); sprintf( szBuffer2, szBuffer1, szWideBuffer0 );
  ...
  */
  ...
  WideCharToMultiByte(CodePage, 0, productName.str(),
                      productName.getLength() + 1, szProduct_Name, _MAX_PATH,
                      NULL, NULL);
  ...
  //object. 	If the named mutex object existed before the function call, the
  //function returns 	a handle to the existing object and GetLastError returns
  //ERROR_ALREADY_EXISTS. 	Otherwise, the caller created the mutex. 	If the
  ...
  if (AppMutex != NULL && (GetLastError() == ERROR_ALREADY_EXISTS)) {
    Msg(__LINE__, __FILE__, "AppMutex of %s already exists. Exit here.",
        szBuffer);
  ...
  //	Return Values
  //	If the function succeeds, the return value is a handle to the mutex
  //object. 	If the function fails, the return value is NULL. To get extended
  //error 	information, call GetLastError.
  //
  // WARNING: DO NOT use this number for any other application except
  // Game/Setup.
  //=========================================================================
  if (WinVersion.Is_Win_XP() || WinVersion.Version() > 500) {
  ...
  if (GameAppMutex != NULL) {
    ...
    if (ccwindow) {
      ...
      if (ccwindow) {
        ...
        Msg(__LINE__, TEXT(__FILE__), TEXT("SetForegroundWindow = %d."),
            result);
      }
    }
  ...
  if (prev) {
    ...
    Msg(__LINE__, __FILE__,
        "Either same app, game or setup was found. Exit and set window to "
        "foreground.");
    return 0;
  }
  ...
  Msg(__LINE__, TEXT(__FILE__), TEXT("About to lock on CD: %c:\\ "),
      toupper(driveLetter));
...
void MainWindow::Register(void) {
  ...
  //	wndclass.hIcon	= (HICON)LoadImage( NULL, szBuffer, IMAGE_ICON, 0, 0,
  ...
  if (!RegisterClassEx((const WNDCLASSEX *)&wndclass)) {
    ...
    FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM,
                  NULL, GetLastError(),
                  MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR)&szMessage,
                  0, NULL);
...
MainWindow::MainWindow(void) {
  ...
  WideCharToMultiByte(CodePage, 0, szFullProductName, _MAX_PATH, szBuffer,
                      _MAX_PATH, NULL, NULL);

#else

  WideCharToMultiByte(CodePage, 0, fullProductName.str(),
                      fullProductName.getLength() + 1, szBuffer, _MAX_PATH,
                      NULL, NULL);
  ...
  hWnd = CreateWindowEx(0, szClassName, szClassName,
                        WS_POPUPWINDOW | WS_MINIMIZE | !WS_VISIBLE, 0, 0, 640,
                        480, NULL, NULL, Main::hInstance, (LPTSTR)this);
...
LRESULT MainWindow::Window_Proc(HWND hWnd, UINT iMessage, WPARAM wParam,
                                LPARAM lParam) {
  static RECT rect;
  ...
  switch (iMessage) {
    ...
    case WM_GO:
      ...
      decision = DialogBox(Main::hInstance, _TEXT("BitmapDialog"), hWnd,
                           Dialog_Box_Proc);
      ...
      // HIWORD( lParam )		Type of Window: 	CTLCOLOR_MSGBOX,
      // _EDIT, _LISTBOX, _BTN, _DLG, _SCROLLBAR, _STATIC
      //-----------------------------------------------------------------------
#if (WINDOW_BRUSH)
    case WM_CTLCOLOR:
      if (HIWORD(lParam) == CTLCOLOR_BTN) {
        ...
        UnrealizeObject(
            hBrush);            // reset the origin of the brush next time used.
        point.x = point.y = 0;  // create a point.
        ClientToScreen(hWnd, &point);  // translate into screen coordinates.
        SetBrushOrg((HDC)wParam, point.x,
                    point.y);  // New Origin to use when next selected.
        return ((DWORD)hBrush);
      }
      break;
...
//*****************************************************************************
// MainWindow::Is_Product_Registered -- Check the Registration Table
// information.
//
// INPUT:  		none.
//
//	OUTPUT: 		BOOL - TRUE if product is already installed,
//FALSE if not.
//
// WARNINGS:	none.
//
...
BOOL MainWindow::Is_Product_Registered(void) {
  ...
  memset(FindFileData.cAlternateFileName, '\0',
         sizeof(FindFileData.cAlternateFileName));
  ...
  // Try to open the key.
  //--------------------------------------------------------------------------
  if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, key, 0, KEY_QUERY_VALUE, &phKey) ==
      ERROR_SUCCESS) {
    //-----------------------------------------------------------------------
    // Get Full path\filename of product to execute ("Play").
    //-----------------------------------------------------------------------
    Size = _MAX_PATH;
    if (RegQueryValueEx(phKey, INSTALL_PATH_KEY, NULL, &Type,
                        (unsigned char *)szGamePath, &Size) == ERROR_SUCCESS) {
  ...
  //--------------------------------------------------------------------------
  // Query the Uninstall key
  // "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall"
  //--------------------------------------------------------------------------
  if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, key, 0, KEY_ALL_ACCESS, &phKey) ==
      ERROR_SUCCESS) {
    Size = _MAX_PATH;
    if (RegQueryValueEx(phKey, UNINSTALL_STRING_SUBKEY, NULL, &Type,
                        (unsigned char *)szUninstallPath,
                        &Size) == ERROR_SUCCESS) {
      //--------------------------------------------------------------------
  ...
  /*
     ...
     KEY_QUERY_VALUE, &phKey ) == ERROR_SUCCESS ) {
                          ...
                          //-----------------------------------------------------------------------
                          // Get Full path\filename of product to execute
     ("Register.exe").
                          //-----------------------------------------------------------------------
                          Size = _MAX_PATH;
                          if ( RegQueryValueEx( phKey, INSTALLPATH_SUBKEY, NULL,
     &Type, (unsigned char *)szRegisterPath, &Size ) == ERROR_SUCCESS ) {

                                  //--------------------------------------------------------------------
                                  // Check if this executable exists.
                                  //--------------------------------------------------------------------
                                  handle = FindFirstFile( szRegisterPath,
     &FindFileData ); if ( handle != INVALID_HANDLE_VALUE ) {
  ...
  */
  ...
  /*
     ...
     ("Register.exe").
                  //-----------------------------------------------------------------------
                  Size = _MAX_PATH;
                  if ( RegQueryValueEx( phKey, INSTALLPATH_SUBKEY, NULL, &Type,
     (unsigned char *)szBuffer, &Size ) == ERROR_SUCCESS ) {
  ...
  */
  ...
  Msg(__LINE__, TEXT(__FILE__),
      TEXT("---------------- Is_Product_Registered---------------"));
  Msg(__LINE__, TEXT(__FILE__), TEXT(" InstallProduct		= %d."),
      InstallProduct);
  Msg(__LINE__, TEXT(__FILE__), TEXT(" UninstallAvailable	= %d."),
      UninstallAvailable);
  Msg(__LINE__, TEXT(__FILE__), TEXT(" IsUserRegistered		= %d."),
      IsUserRegistered);
  Msg(__LINE__, TEXT(__FILE__), TEXT(" DisplayRegisterButton	= %d."),
      DisplayRegisterButton);
  Msg(__LINE__, TEXT(__FILE__),
      TEXT(" szGamePath			= %s."), szGamePath);
  Msg(__LINE__, TEXT(__FILE__),
      TEXT(" szSetupPath			= %s."), szSetupPath);
  Msg(__LINE__, TEXT(__FILE__), TEXT(" szRegisterPath		= %s."),
      szRegisterPath);
  Msg(__LINE__, TEXT(__FILE__), TEXT(" szInternetPath		= %s."),
      szInternetPath);
  Msg(__LINE__, TEXT(__FILE__), TEXT(" szUninstallPath		= %s."),
      szUninstallPath);
...
//		this dir.  The Explorer will open and display everything in this
//dir.
//
//	INPUT:  	none.
//
...
BOOL MainWindow::Run_Explorer(char *szString, HWND hWnd, RECT *rect) {
  ...
  //--------------------------------------------------------------------------
  result = CreateProcess(szWindowsPath,  // address of module name
...
//*****************************************************************************
// MainWindow::Run_WorldBuilder -- Launch the map editor based on registry
// information.
//
// INPUT:  		none.
//
...
//*****************************************************************************
// MainWindow::Run_PatchGet -- Launch the patch checker based on registry
// information.
//
// INPUT:  		none.
//
...
unsigned int MainWindow::Run_OpenFile(int cd_drive, const char *filename,
                                      bool wait /* = false */) {
...
//
//		Either run Register.exe or Game Update Program from the user's
//harddrive.
//
// INPUT:         none.
//
...
unsigned int MainWindow::Run_Uninstall(HWND hWnd, RECT *rect) {
  ...
  result = CreateProcess(szUninstallPath,  // address of module name
  ...
  //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  // MML 5/27/99:  This function is dropping through because we launch
  // Uninstll.exe which in turn launches Uninst.exe thus ::Run_Install ends
  // before Uninst.exe is done.
  //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
void MainWindow::Create_Buttons(HWND hWnd, RECT *dlg_rect) {
  ...
  for (i = 0; i < NUM_BUTTONS; i++) {
    ...
    Msg(__LINE__, TEXT(__FILE__), TEXT("ButtonSizes[%d] = ( %d, %d, %d, %d )"),
        i, ButtonSizes[i].left, ButtonSizes[i].top, ButtonSizes[i].right,
        ButtonSizes[i].bottom);
  }
  ...
  Msg(__LINE__, TEXT(__FILE__), TEXT("next_button_name = %s."),
      next_button_name);
  Msg(__LINE__, TEXT(__FILE__), TEXT("focused_button_name = %s."),
      focused_button_name);
  ...
  if (InstallProduct) {
    ...
    //---------------------------------------------------------------------
    Msg(__LINE__, TEXT(__FILE__),
        TEXT("ButtonList[%d]: Id=%d, ButtonSizes=%d, String=%s."), i, IDD_OK, j,
        "Install");
    ButtonList[i++] = new DrawButton(IDD_OK, button_size[j++], BUTTON_REG,
    ...
    //---------------------------------------------------------------------
    Msg(__LINE__, TEXT(__FILE__),
        TEXT("ButtonList[%d]: Id=%d, ButtonSizes=%d, String=%s."), i, IDD_OK2,
        j, "Play");
    ButtonList[i++] = new DrawButton(IDD_OK2, button_size[j++], BUTTON_REG,
                                     BUTTON_SEL, BUTTON_SEL,
#ifdef LEAN_AND_MEAN
    ...
    /*
                    ...
                    //---------------------------------------------------------------------
                    Msg( __LINE__, TEXT(__FILE__), TEXT("ButtonList[%d]: Id=%d,
    ButtonSizes=%d, String=%s."), i, IDD_OK2, j, "WorldBuilder" );
  ...
  //-------------------------------------------------------------------------
  Msg(__LINE__, TEXT(__FILE__),
      TEXT("ButtonList[%d]: Id=%d, ButtonSizes=%d, String=%s."), i, IDD_EXPLORE,
      j, "Explore");
  //	ButtonList[i--] = new DrawButton (
  ButtonList[i++] =
      new DrawButton(IDD_EXPLORE,
                     //		button_size[j--],
                     button_size[j++], BUTTON_REG, BUTTON_SEL, BUTTON_SEL,
#ifdef LEAN_AND_MEAN
  ...
  if (UninstallAvailable && !InstallProduct) {
    Msg(__LINE__, TEXT(__FILE__),
        TEXT("ButtonList[%d]: Id=%d, ButtonSizes=%d, String=%s."), i,
        IDD_UNINSTALL, j, "Uninstall");
    //		ButtonList[i--] = new DrawButton(
    ButtonList[i++] =
  ...
  Msg(__LINE__, TEXT(__FILE__),
      TEXT("ButtonList[%d]: Id=%d, ButtonSizes=%d, String=%s."), i,
      IDD_PREVIEWS, j, "Preview Movies");
  ButtonList[i++] = new DrawButton(IDD_PREVIEWS, button_size[j++], BUTTON_REG,
                                   BUTTON_SEL, BUTTON_SEL,
#ifdef LEAN_AND_MEAN
  ...
  //--------------------------------------------------------------------------
  Msg(__LINE__, TEXT(__FILE__),
      TEXT("ButtonList[%d]: Id=%d, ButtonSizes=%d, String=%s."), i, IDD_HELP, j,
      "Help file");
  ButtonList[i++] = new DrawButton(IDD_HELP, button_size[j++], BUTTON_REG,
                                   BUTTON_SEL, BUTTON_SEL,
#ifdef LEAN_AND_MEAN
  ...
  //--------------------------------------------------------------------------
  Msg(__LINE__, TEXT(__FILE__),
      TEXT("ButtonList[%d]: Id=%d, ButtonSizes=%d, String=%s."), i, IDD_CANCEL,
      j, "Cancel");
  //	ButtonList[i--] = new DrawButton(
  ButtonList[i++] =
      new DrawButton(IDD_CANCEL,
                     //		button_size[j--],
                     button_size[j++], BUTTON_REG, BUTTON_SEL, BUTTON_SEL,
#ifdef LEAN_AND_MEAN
  ...
  for (i = 0; i < NUM_BUTTONS; i++) {
    if (ButtonList[i]) {
      Msg(__LINE__, TEXT(__FILE__),
          TEXT("ButtonList[%d]: Id=%d, ButtonSizes=[%d,%d,%d,%d]."), i,
          ButtonList[i]->Return_Id(), ButtonList[i]->Return_X_Pos(),
          ButtonList[i]->Return_Y_Pos(), ButtonList[i]->Return_Width(),
...
BOOL CALLBACK Dialog_Box_Proc(HWND window_handle, UINT message, WPARAM w_param,
  ...
  static RECT BackgroundRect[(NUM_BUTTONS * 3) +
                             3];  // Background areas outside button area.
  ...
  switch (message) {
    case WM_INITDIALOG: {
      ...
      WideCharToMultiByte(CodePage, 0, wideBuffer2.str(),
                          wideBuffer2.getLength() + 1, szBuffer, _MAX_PATH,
                          NULL, NULL);
      ...
      // Set Icon.
      //-----------------------------------------------------------------------
      SendMessage(window_handle, WM_SETICON, ICON_SMALL,
                  (long)LoadIcon(Main::hInstance, MAKEINTRESOURCE(1)));
      ...
      //=======================================================================
      //				memset( szString1,		'\0',
      //MAX_PATH ); 				memset( szString2,		'\0', MAX_PATH ); 				memset(
      //szWholeString,	'\0', 1000 );

      //				Locale_GetString( EA_LICENSE_MSG1,
      //szString1 ); 				Locale_GetString( EA_LICENSE_MSG2, szString2 ); 				swprintf(
      //szWholeString, L"%s %s", szString1, szString2 );
      ...
      //				Locale_GetString( WESTWOOD_COM_MSG,
      //szWSMsg1 ); 				wsMsg1 = TheGameText->fetch("Autorun:WestwoodComMsg");

      //				PlugInProductName( szWSMsg1, (wchar_t
      //*)(fullProductName.str()) ); 				PlugInProductName( (wchar_t
      //*)(wsMsg1.str()), (wchar_t *)(fullProductName.str()) ); 				wsMsg1 =
      //Fix_Single_Ampersands ( wsMsg1, false ); 				swprintf( szWholeWSMsg, L"%s",
      //wsMsg1.str() );

      //				memset( szInstallWarningMsg, '\0',
      //_MAX_PATH );

      //				Locale_GetString(
      //IDS_INSTALL_WARNING_MSG, szWideBuffer ); 				wideBuffer =
      //TheGameText->fetch("Autorun:InstallWarningMsg");

      //				swprintf( szInstallWarningMsg,
      //wideBuffer.str(), fullProductName.str() );
      ...
      for (i = 0; i < NUM_FLICKER_FRAMES; i++) {
        hFlicker[i] = 0;
        //					sprintf( buffer2, "%s%02d",
        //buffer1, i ); 					hFlicker[i] = LoadResourceBitmap( Main::hModule,
        //buffer2, &hpal );
      ...
      if (b640X480) {
        ...
        if (true) {
          ...
          dlg_rect.bottom = bm.bmHeight + GetSystemMetrics(SM_CYCAPTION) +
                            6;  // desktop larger than image
        } else {
          dlg_rect.right = 640;  // bm.bmWidth + 6;
          dlg_rect.bottom =
              480;  // bm.bmHeight + GetSystemMetrics( SM_CYCAPTION ) + 6;
                    // // desktop larger than image
        }
      } else {
        dlg_rect.right = bm.bmWidth + 6;
        dlg_rect.bottom = bm.bmHeight + GetSystemMetrics(SM_CYCAPTION) +
      ...
      //				Msg( __LINE__, TEXT(__FILE__),
      //TEXT("buttons_rect = [%d,%d,%d,%d]"), buttons_rect.left,
      //buttons_rect.top, buttons_rect.right, buttons_rect.bottom );
      ...
      for (j = 0; j < NUM_BUTTONS; j++) {
        if (ButtonList[j] != NULL) {
          ...
          buttons_rect.right = __max(ButtonSizes[j].left + ButtonSizes[j].right,
                                     buttons_rect.right);
          buttons_rect.bottom = __max(
              ButtonSizes[j].top + ButtonSizes[j].bottom, buttons_rect.bottom);
        }
      }

      Msg(__LINE__, TEXT(__FILE__), TEXT("buttons_rect = [%d,%d,%d,%d]"),
          buttons_rect.left, buttons_rect.top, buttons_rect.right,
          buttons_rect.bottom);
      Msg(__LINE__, TEXT(__FILE__),
          TEXT("----------------------------- determining button area "
               "---------------------------"));
      ...
      //=======================================================================
      if (!b640X480) {
        MoveWindow(window_handle, (rect.right - dlg_rect.right) / 2,
                   (rect.bottom - dlg_rect.bottom) / 2, dlg_rect.right,
                   dlg_rect.bottom, TRUE);
      } else {
        MoveWindow(window_handle, 0, 0, dlg_rect.right, dlg_rect.bottom, TRUE);
      }
      ...
      for (int index = i; index < NUM_BUTTONS; index++) {
        ...
        if (ButtonList[index] != NULL) {
          ...
          BackgroundRect[j].top = ButtonList[index]->Return_Y_Pos();
          BackgroundRect[j].bottom = ButtonList[index]->Return_Height();
          BackgroundRect[j].left = ButtonList[index]->Return_X_Pos() +
                                   ButtonList[index]->Return_Width();
          BackgroundRect[j].right =
              BackgroundRect[3].left - (ButtonList[index]->Return_X_Pos() +
                                        ButtonList[index]->Return_Width());
          j++;
        }
      }
      ...
      Msg(__LINE__, TEXT(__FILE__),
          TEXT("----------------------------- WM_INITDIALOG "
               "---------------------------"));
      Msg(__LINE__, TEXT(__FILE__),
          TEXT("DeskTopWindowRect(w/o tray) = [%d,%d,%d,%d]"), tray_rect.left,
          tray_rect.top, tray_rect.right, tray_rect.bottom);
      Msg(__LINE__, TEXT(__FILE__), TEXT("DeskTopWindowRect	= [%d,%d,%d,%d]"),
          rect.left, rect.top, rect.right, rect.bottom);
      Msg(__LINE__, TEXT(__FILE__), TEXT("b640X480		= [%d]"),
          b640X480);
      Msg(__LINE__, TEXT(__FILE__), TEXT("b800X600		= [%d]"),
          b800X600);
      Msg(__LINE__, TEXT(__FILE__), TEXT("ClientRect		= [%d,%d,%d,%d]"),
          dlg_rect.left, dlg_rect.top, dlg_rect.right, dlg_rect.bottom);

      for (index = 0; index < (NUM_BUTTONS * 3) + 3; index++) {
        Msg(__LINE__, TEXT(__FILE__),
            TEXT("BackgroundRect[%d]	= [%d,%d,%d,%d]"), index,
            BackgroundRect[index].top, BackgroundRect[index].bottom,
            BackgroundRect[index].left, BackgroundRect[index].right);
      }

      Msg(__LINE__, TEXT(__FILE__), TEXT("BitmapRect	   	= [%d,%d,%d,%d]"),
          bitmap_rect.left, bitmap_rect.top, bitmap_rect.right,
          bitmap_rect.bottom);
      Msg(__LINE__, TEXT(__FILE__),
          TEXT("FlickerRect		= [%d,%d,%d,%d]"), flicker_rect.left,
          flicker_rect.top, flicker_rect.right, flicker_rect.bottom);
      Msg(__LINE__, TEXT(__FILE__),
          TEXT("ButtonsRect		= [%d,%d,%d,%d]"), buttons_rect.left,
          buttons_rect.top, buttons_rect.right, buttons_rect.bottom);
      Msg(__LINE__, TEXT(__FILE__),
          TEXT("---------------------------------------------------------------"
               "--------"));
      ...
      //=======================================================================
      //				timer_id = SetTimer( window_handle,
      //1000, 250L, NULL );
      ...
      //-------------------------------------------------------------------------------
      //		case WM_SETCURSOR:
      //			SetCursor(LoadCursor( Main::hInstance,
      //MAKEINTRESOURCE(2))); 			break;
    ...
    case WM_TIMER: {
      if (w_param == 1000) {
        ...
        if (Flicker && hFlicker[0]) {
          ...
          Msg(__LINE__, TEXT(__FILE__),
              TEXT("WM_TIMER: FlickerRect = [%d,%d,%d,%d]"), flicker_rect.left,
              flicker_rect.right, flicker_rect.top, flicker_rect.bottom);
        }
      }
      ...
      if (GameAppMutex != NULL) {
        ...
        PostMessage(window_handle, WM_COMMAND,
                    (WPARAM)MAKELONG(IDD_CANCEL, BN_CLICKED),
                    (LPARAM)GetDlgItem(window_handle, IDD_CANCEL));
      }
    }
      return (0);
    ...
    case WM_PAINT:
      ...
      if (GetUpdateRect(window_handle, &paint_rect, FALSE)) {
        ...
        //TEXT("---------------------- WM_PAINT ---------------------"), i );
        //				Msg( __LINE__, TEXT(__FILE__),
        //TEXT("Rectangle to update  = [%d,%d,%d,%d]"), paint_rect.left,
        //paint_rect.top, paint_rect.right, paint_rect.bottom );
        ...
        if (hBitmap) {
          ...
          prevButtonDCStretchMode =
              SetStretchBltMode(buttonDC, STRETCH_DELETESCANS);
          prevLicenseDCStretchMode =
              SetStretchBltMode(licenseDC, STRETCH_DELETESCANS);
          ...
          for (i = 0; i < NUM_BUTTONS; i++) {
            if (ButtonList[i]) {
              ...
              if (paint_rect.left == rect.left && paint_rect.top == rect.top &&
                  paint_rect.right == rect.right &&
                  paint_rect.bottom == rect.bottom) {
                PaintBackground = FALSE;
                //									Msg(
                //__LINE__, TEXT(__FILE__), TEXT("Rectangle matches a button to
                //update = [%d,%d,%d,%d]"), rect.left, rect.top, rect.right,
                //rect.bottom );
                break;
              }
            }
          ...
          if (PaintBackground) {
            ...
            for (int i = 0; i < (NUM_BUTTONS * 3) + 3; i++) {
              if (BackgroundRect[i].left != 0 || BackgroundRect[i].top != 0 ||
                  BackgroundRect[i].right != 0 ||
                  BackgroundRect[i].bottom != 0) {
                result = StretchBlt(
                    hDC, BackgroundRect[i].left, BackgroundRect[i].top,
                    BackgroundRect[i].right, BackgroundRect[i].bottom, memDC,
                    BackgroundRect[i].left, BackgroundRect[i].top,
                    BackgroundRect[i].right, BackgroundRect[i].bottom, SRCCOPY);

                if (result != 0) {
                  //										Msg(
                  //__LINE__, TEXT(__FILE__), TEXT("Paint this rect =
                  //[%d,%d,%d,%d]"), BackgroundRect[i].left,
                  //BackgroundRect[i].top, BackgroundRect[i].right,
                ...
                MoveToEx(hDC, BackgroundRect[i].left + 1,
                         BackgroundRect[i].top + 1, NULL);
                LineTo(hDC, BackgroundRect[i].right - 1,
                       BackgroundRect[i].top + 1);
                LineTo(hDC, BackgroundRect[i].right - 1,
                       BackgroundRect[i].bottom - 1);
                LineTo(hDC, BackgroundRect[i].left + 1,
                       BackgroundRect[i].bottom - 1);
                LineTo(hDC, BackgroundRect[i].left + 1,
                       BackgroundRect[i].top + 1);
            ...
            /*
               ...
               Draw the background bitmap.
                                                                    //-----------------------------------------------------------
                                                                    oldLicenseBitmap
               = ( HBITMAP )SelectObject( licenseDC, hLicenseBitmap );

                                                                    //-----------------------------------------------------------
                                                                    // Updates
               area around the button area.
                                                                    //-----------------------------------------------------------
                                                                    result =
               StretchBlt( hDC, license_rect.left, license_rect.top,
                                                                                            license_rect.right,
                                                                                            license_rect.bottom,
                                                                                            licenseDC,
                                                                                            0,
            ...
            */
            ...
            //-----------------------------------------------------------
            StretchBlt(hDC, dlg_rect.left, dlg_rect.top, dlg_rect.right,
                       dlg_rect.bottom, memDC, bitmap_rect.left,
                       bitmap_rect.top, bitmap_rect.right, bitmap_rect.bottom,
                       SRCCOPY);
#endif
          ...
          if (Flicker && hFlicker[0]) {
            //							oldBitmap = (
            //HBITMAP )SelectObject( memDC, hFlicker[FlickerIndex] );
            oldBitmap = (HBITMAP)SelectObject(memDC, hFlicker[0]);

            StretchBlt(hDC, flicker_rect.left, flicker_rect.top,
                       flicker_rect.right, flicker_rect.bottom, memDC, 0, 0,
                       flicker_rect.right, flicker_rect.bottom, SRCCOPY);
            ...
            //							Msg( __LINE__,
            //TEXT(__FILE__), TEXT("		Drawing Flicker
            //[%d,%d,%d,%d]."), flicker_rect.left, flicker_rect.top,
            //flicker_rect.right, flicker_rect.bottom );
          }
          ...
          for (i = 0; i < NUM_BUTTONS; i++) {
            if (ButtonList[i]) {
              ...
              if (ButtonList[i]->Draw_Bitmaps()) {
                ...
                hButtonBitmap = LoadResourceBitmap(Main::hInstance,
                                                   szButtonBitmap, &hpal, TRUE);
                if (hButtonBitmap) {
                  ...
                  StretchBlt(hDC, dst_rect.left, dst_rect.top, dst_rect.right,
                             dst_rect.bottom, buttonDC, src_rect.left,
              ...
              if (fontptr) {
                ...
                /*
                ** This function was combining the pixel color with the
                *background,
                ** so it never looked correct.
                */
                //									SetTextColor(
                //hDC, RGB( 0, 240, 0 )); 									DrawFocusRect(	hDC, &dst_rect
                //);

                if (ButtonList[i]->Get_State() == DrawButton::PRESSED_STATE) {
                  fontptr->Print(hDC, ButtonList[i]->Return_Text(), rect,
                                 TEXT_PRESSED_COLOR, TEXT_PRESSED_SHADOW_COLOR,
                                 TPF_BUTTON, TPF_SHADOW);

                } else if (ButtonList[i]->Get_State() ==
                           DrawButton::FOCUS_STATE) {
                  fontptr->Print(hDC, ButtonList[i]->Return_Text(), rect,
                                 TEXT_FOCUSED_COLOR, TEXT_FOCUSED_SHADOW_COLOR,
                                 TPF_BUTTON, TPF_SHADOW);

                } else {
                  fontptr->Print(hDC, ButtonList[i]->Return_Text(), rect,
                                 TEXT_NORMAL_COLOR, TEXT_NORMAL_SHADOW_COLOR,
                                 TPF_BUTTON, TPF_SHADOW);
                }
                ...
                LineTo(
                    ...
                    outline_rect.right,  // x-coordinate of line's ending point
                    outline_rect
                        .bottom);  // y-coordinate of line's ending point
...
//---------------------------------------------------------------
            ...
            if (hPen1) {
              for (int i = 0; i < NUM_BUTTONS; i++) {
                if (ButtonList[i]) {
                  ...
                  MoveToEx(hDC, ButtonList[i]->Return_X_Pos() - 1,
                           ButtonList[i]->Return_Y_Pos() - 1, NULL);
                  LineTo(hDC,
                         ButtonList[i]->Return_X_Pos() +
                             ButtonList[i]->Return_Width() + 1,
                         ButtonList[i]->Return_Y_Pos() - 1);
                  LineTo(hDC,
                         ButtonList[i]->Return_X_Pos() +
                             ButtonList[i]->Return_Width() + 1,
                         ButtonList[i]->Return_Y_Pos() +
                             ButtonList[i]->Return_Height() + 1);
                  LineTo(hDC, ButtonList[i]->Return_X_Pos() - 1,
                         ButtonList[i]->Return_Y_Pos() +
                             ButtonList[i]->Return_Height() + 1);
                  LineTo(hDC, ButtonList[i]->Return_X_Pos() - 1,
                         ButtonList[i]->Return_Y_Pos() - 1);
        ...
        //(LPRECT) &dlg_rect ); 					FillRect( hDC, &dlg_rect, (HBRUSH)( COLOR_WINDOW
        //+ 1 ));
...
//---------------------------------------------------------------
        ...
        if (fontptr) {
          ...
          fontptr->Print(hDC, szWholeString, text_rect, TEXT_COLOR,
                         SHADOW_COLOR, TPF_CENTER_TEXT, TPF_SHADOW);
        }
#endif
    ...
    case WM_COMMAND: {
      ...
      switch (idCtl) {
        ...
        case IDD_PREVIEWS: {
          ...
          // show the previews in succession.  each will wait for the previous
          // to finish before playing.
          ...
          //						if (success != 0) {
          //							success =
          //GlobalMainWindow->Run_OpenFile(cd_drive, BFAVI_FILENAME, true);
          //						}
          /*
                                                          if (success == 0) {
                                                                  std::wstring
             wideBuffer = TheGameText->fetch("Autorun:CantRunAVIs");
                                                                  std::wstring
             wideBuffer2 = TheGameText->fetch("Autorun:Error"); int length =
             wideBuffer.length(); WideCharToMultiByte( CodePage, 0,
             wideBuffer.c_str(), length+1, szBuffer, _MAX_PATH, NULL, NULL );
                                                                  length =
             wideBuffer2.length(); WideCharToMultiByte( CodePage, 0,
             wideBuffer2.c_str(), length+1, szBuffer2, _MAX_PATH, NULL, NULL );
                                                                  MessageBox(
             NULL, szBuffer, szBuffer2, MB_APPLMODAL | MB_OK );
                                                          }
          */
        } break;
        ...
        case IDD_HELP: {
          ...
          /*
                                                          if (success == 0) {
                                                                  std::wstring
             wideBuffer = TheGameText->fetch("Autorun:CantRunHelp");
                                                                  std::wstring
             wideBuffer2 = TheGameText->fetch("Autorun:Error"); int length =
             wideBuffer.length(); WideCharToMultiByte( CodePage, 0,
             wideBuffer.c_str(), length+1, szBuffer, _MAX_PATH, NULL, NULL );
        ...
        case IDD_OK4:
          ...
          if (!Is_On_CD(PRODUCT_VOLUME_CD1)) {
            ...
            //IDS_INSERT_CDROM_WITH_VOLUME1, IDS_EXIT_MESSAGE2, &cd_drive );
            result = Prompt_For_CD(window_handle, volume_to_match,
          ...
          if (result) {
            if (idCtl == IDD_OK) {
              ...
              result = GlobalMainWindow->Run_Setup(window_handle, &dlg_rect,
                                                   cd_drive);
              ...
              Msg(__LINE__, TEXT(__FILE__),
                  TEXT("IDD_OK3 Selected, running WorldBuilder."));
              result =
                  GlobalMainWindow->Run_WorldBuilder(window_handle, &dlg_rect);
            } else if (idCtl == IDD_OK4) {
              Msg(__LINE__, TEXT(__FILE__),
                  TEXT("IDD_OK4 Selected, running PatchGet."));
        ...
        case IDD_VIEW_DEMO:
          ...
          if (!Is_On_CD(PRODUCT_VOLUME_CD2)) {
            ...
            //							result =
            //Prompt_For_CD( window_handle, volume_to_match,
            //IDS_INSERT_CDROM_WITH_VOLUME2, IDS_EXIT_MESSAGE2, &cd_drive );
            result =
                Prompt_For_CD(window_handle, volume_to_match,
                              AsciiString("Autorun:InsertCDROMWithVolume2"),
                              AsciiString("Autorun:ExitMessage2"), &cd_drive);
          }

          if (result) {
            Msg(__LINE__, TEXT(__FILE__), TEXT("IDD_VIEW_DEMO Selected."));
            result =
                GlobalMainWindow->Run_Demo(window_handle, &dlg_rect, cd_drive);
          }
        ...
        case IDD_GAMESPY:
          ...
          if (ViewHTML(GAMESPY_WEBSITE)) {
            ...
            Error_Message(Main::hInstance, AsciiString("Autorun:Generals"),
                          AsciiString("Autorun:CantFindExplorer"),
                          GAME_WEBSITE);
          }
          break;
#endif
        ...
        case IDD_INTERNET:
          ...
          if (ViewHTML(GAME_WEBSITE)) {
            ...
            Error_Message(Main::hInstance, "Autorun:Generals",
                          "Autorun:CantFindExplorer", GAME_WEBSITE);
          }
          break;
...
//-------------------------------------------------------------------------------
//	WM_CTLCOLOR Message.
//	wParam					Handle to Child Window's device
//context 	LOWORD( lParam )		Child Window handle 	HIWORD( lParam )
//Type of Window: 	CTLCOLOR_MSGBOX, _EDIT, _LISTBOX, _BTN, _DLG,
//_SCROLLBAR, _STATIC
//
//	WM_CTLCOLORMSGBOX
//	WM_CTLCOLOREDIT
...
//	WM_CTLCOLORSTATIC
//	#define WM_CTLCOLOR
//0x0019 	#define GET_WM_CTLCOLOR_HDC (wp, lp, msg)		(HDC)(wp)
//	#define GET_WM_CTLCOLOR_HWND(wp, lp, msg)		(HWND)(lp)
//	#define GET_WM_CTLCOLOR_TYPE(wp, lp, msg)		(WORD)(msg -
//WM_CTLCOLORMSGBOX) 	#define GET_WM_CTLCOLOR_MSG (type)
...
//-------------------------------------------------------------------------------
    ...
    case WM_CTLCOLOR:
      if (HIWORD(l_param) == CTLCOLOR_STATIC) {
        ...
        SetBkColor((HDC)wParam, GetSysColor(COLOR_WINDOW));
        //				SetBkColor(( HDC )w_param, RGB( 192,
        //192, 192 ));

        UnrealizeObject(
            hStaticBrush);      // reset the origin of the brush next time used.
        point.x = point.y = 0;  // create a point.
        ClientToScreen(window_handle,
                       &point);  // translate into screen coordinates.
        SetBrushOrgEx((HDC)w_param, point.x, point.y,
                      NULL);  // New Origin to use when next selected.
...
//===============================================================================
    ...
    case WM_LBUTTONDOWN: {
      ...
      for (i = 0; i < NUM_BUTTONS; i++) {
        ...
        //-------------------------------------------------------------------
        if (ButtonList[i] &&
            ButtonList[i]->Is_Mouse_In_Region(mouse_x, mouse_y)) {
          if (ButtonList[i]->Get_State() != DrawButton::PRESSED_STATE) {
            ButtonList[i]->Return_Area(&rect);
            ...
            Msg(__LINE__, TEXT(__FILE__),
                TEXT("WM_LBUTTONDOWN -- %s. rect = [%d,%d,%d,%d]."),
                ButtonList[i]->Return_Normal_Bitmap(), rect.left, rect.top,
...
//===============================================================================
    ...
    case WM_LBUTTONUP: {
      ...
      if (found_focus == -1) {
        ...
        if (ButtonList[focus_index] &&
            (ButtonList[focus_index]->Get_State() != DrawButton::FOCUS_STATE)) {
          ButtonList[focus_index]->Set_State(DrawButton::FOCUS_STATE);
          ButtonList[focus_index]->Return_Area(&rect);
          InvalidateRect(window_handle, &rect, FALSE);

          Msg(__LINE__, TEXT(__FILE__),
              TEXT("WM_LBUTTONUP -- %s[FOCUS_STATE] = [x=%d, y=%d, w=%d, "
                   "h=%d]."),
              ButtonList[focus_index]->Return_Normal_Bitmap(), rect.left,
        ...
        if (focus_index == found_focus) {
          ...
          Msg(__LINE__, TEXT(__FILE__),
              TEXT("WM_LBUTTONUP -- %s[FOCUS_STATE] = [x=%d, y=%d, w=%d, "
                   "h=%d]."),
              ButtonList[found_focus]->Return_Normal_Bitmap(), rect.left,
              rect.top, rect.right, rect.bottom);
          ...
          if (ButtonList[focus_index]) {
            ...
            Msg(__LINE__, TEXT(__FILE__),
                TEXT("WM_LBUTTONUP -- %s[NORMAL_STATE] = [x=%d, y=%d, w=%d, "
                     "h=%d]."),
                ButtonList[focus_index]->Return_Normal_Bitmap(), rect.left,
                rect.top, rect.right, rect.bottom);
      ...
      if (found_focus >= 0) {
        if ((ButtonList[found_focus]) &&
          ...
          SendMessage(window_handle, WM_COMMAND,
          ...
          if (CurrentButton != LastButton) {
            ...
            for (j = 0; j < NUM_BUTTONS; j++) {
              if (ButtonList[j]) {
                ...
                //									Msg(
                //__LINE__, TEXT(__FILE__), TEXT("WM_MOUSEMOVE --
                //%s[NORMAL_STATE]]."), ButtonList[j]->Return_Normal_Bitmap());
              }
            }
            ...
            if (w_param & MK_LBUTTON) {
              ...
              if (ButtonList[i] &&
                ...
                //									Msg(
                //__LINE__, TEXT(__FILE__), TEXT("WM_MOUSEMOVE --
                //%s[PRESSED_STATE]."), ButtonList[i]->Return_Normal_Bitmap());
              }
              ...
              //--------------------------------------------------------
              if (ButtonList[i] &&
                  ButtonList[i]->Get_State() != DrawButton::FOCUS_STATE) {
                ButtonList[i]->Set_State(DrawButton::FOCUS_STATE);
                //									Msg(
                //__LINE__, TEXT(__FILE__), TEXT("WM_MOUSEMOVE --
                //%s[FOCUS_STATE]."), ButtonList[i]->Return_Normal_Bitmap());
              }
            }  // end of if
    ...
    case WM_SETFOCUS:
      ...
      //			nResult = UpdateWindow( window_handle );
      //			Msg( __LINE__, TEXT(__FILE__),
      //TEXT("WM_LBUTTONDBLCLK -- dlg_rect = [x=%d, y=%d, w=%d, h=%d]."),
      //				dlg_rect.left, dlg_rect.top,
      //dlg_rect.right, dlg_rect.bottom );
      break;
      ...
      switch (w_param) {
        ...
        case VK_RETURN:
          ...
          for (i = 0; i < NUM_BUTTONS; i++) {
            if (ButtonList[i] &&
                ButtonList[i]->Get_State() == DrawButton::FOCUS_STATE) {
              SendMessage(window_handle, WM_COMMAND, ButtonList[i]->Return_Id(),
                          0L);
              break;
            }
          }
        ...
        case VK_DOWN: {
          ...
          //-----------------------------------------------------------
          if (ButtonList[focused_button] &&
              (ButtonList[focused_button]->Get_State() !=
               DrawButton::NORMAL_STATE)) {
            ButtonList[focused_button]->Set_State(DrawButton::NORMAL_STATE);
            ButtonList[focused_button]->Return_Area(&rect);
            InvalidateRect(window_handle, &rect, FALSE);

            Msg(__LINE__, TEXT(__FILE__), TEXT("VK_DOWN/VK_TAB -- %s = [%s]."),
          ...
          if (ButtonList[next_button] &&
              (ButtonList[next_button]->Get_State() !=
               DrawButton::FOCUS_STATE)) {
            ...
            Msg(__LINE__, TEXT(__FILE__), TEXT("VK_DOWN/VK_TAB -- %s = [%s]."),
                ButtonList[next_button]->Return_Normal_Bitmap(), "FOCUS_STATE");
          }
        } break;
...
BOOL Options(Command_Line_Arguments *Orgs) {
  ...
  for (i = 0; i < Orgs->Get_argc(); i++) {
    ...
    Msg(__LINE__, TEXT(__FILE__), TEXT("Options -- Argument[%d] = %s."), i,
        buffer);
    ...
    if ((buffer[0] == '/') || (buffer[0] == '-')) {
      switch (tolower(buffer[1])) {
        ...
        case 'v': {
          ...
          GetPrivateProfileString("Setup", "Version", "1.0", szVersion,
                                  sizeof(szVersion), szPath);
  ...
  Msg(__LINE__, TEXT(__FILE__), TEXT("Options -- GetLocalInfo = %s"),
      szBuffer1);

  sprintf(szDefaultLangID, "%04X", GetUserDefaultLangID());
  Msg(__LINE__, __FILE__, "Options -- GetUserDefaultLangID() = %s",
      szDefaultLangID);

  sprintf(szDefaultLangID, "%04X", GetSystemDefaultLangID());
  Msg(__LINE__, __FILE__, "Options -- GetSystemDefaultLangID() = %s",
      szDefaultLangID);
  Msg(__LINE__, __FILE__,
      "-------------------------------------------------------------");
#endif
...
BOOL Valid_Environment(void) {
  ...
  if (!result) {
    ...
    WideCharToMultiByte(CodePage, 0, wideBuffer.c_str(), length + 1, szBuffer,
                        _MAX_PATH, NULL, NULL);
    length = wideBuffer2.length();
    WideCharToMultiByte(CodePage, 0, wideBuffer2.c_str(), length + 1, szBuffer2,
                        _MAX_PATH, NULL, NULL);
...
HBITMAP LoadResourceBitmap(HINSTANCE hInstance, LPTSTR lpString,
                           HPALETTE FAR *lphPalette, bool loading_a_button) {
  //	HDC 		hdc;
  int iNumColors;
  HRSRC hRsrc;
  HGLOBAL hGlobal;
...
HPALETTE CreateDIBPalette(LPBITMAPINFO lpbmi, LPINT lpiNumColors) {
  ...
  if (*lpiNumColors) {
    hLogPal = GlobalAlloc(
        GHND, sizeof(LOGPALETTE) + sizeof(PALETTEENTRY) * (*lpiNumColors));
...
// 	09/26/1996  MML : Created.
//=============================================================================
HBITMAP LoadResourceButton(HINSTANCE hInstance, LPTSTR lpString,
                           HPALETTE FAR lphPalette) {
  HDC hdc;
  int iNumColors;
  HRSRC hRsrc;
  ...
  if (hRsrc) {
    ...
    hBitmapFinal = CreateDIBitmap(
        hdc, (LPBITMAPINFOHEADER)lpbi, (LONG)CBM_INIT,
...
void Cant_Find_MessageBox(HINSTANCE hInstance, char *szPath) {
  ...
  WideCharToMultiByte(CodePage, 0, wideBuffer3.str(),
                      wideBuffer3.getLength() + 1, szBuffer3, _MAX_PATH, NULL,
                      NULL);
  WideCharToMultiByte(CodePage, 0, wideBuffer2.str(),
                      wideBuffer2.getLength() + 1, szBuffer2, _MAX_PATH, NULL,
                      NULL);
  ...
  if (strlen(szPath) < 3) {
    MessageBox(NULL, "The path specified in Cant_Find_MessageBox was blank",
               "Autorun", MB_APPLMODAL | MB_OK);
    return;
  }
  if (strlen(szBuffer1) < 3 && strlen(szBuffer3) < 3) {
    MessageBox(NULL,
               "***MISSING MESSAGES***... IDS_AUTORUN_TITLE and IDS_CANT_FIND",
               "Autorun", MB_APPLMODAL | MB_OK);
    return;
  }
  if (strlen(szBuffer1) < 3) {
    MessageBox(NULL, "***MISSING MESSAGE***... IDS_AUTORUN_TITLE", "Autorun",
               MB_APPLMODAL | MB_OK);
    return;
  }
  if (strlen(szBuffer3) < 3) {
    MessageBox(NULL, "***MISSING MESSAGE***... IDS_CANT_FIND", "Autorun",
               MB_APPLMODAL | MB_OK);
    return;
  }
...
unsigned int LaunchObjectClass::Launch(void) {
  ...
  Msg(__LINE__, TEXT(__FILE__), TEXT("Current Working Dir = %d\\%s."), cde,
      szBuffer);
#endif
  ...
  result = CreateProcess(szPath,       // address of module name
                         lpszComLine,  // address of command line
                         NULL,         // address of process security attributes
                         NULL,         // address of thread security attributes
                         FALSE,        // new process inherits handles
...
void Debug_Date_And_Time_Stamp(void) {
  ...
  Msg(__LINE__, __FILE__, "%s, %s %d, %d		%d:%d:%d %s",
      Week_Day_Strings[today->tm_wday], Month_Strings[today->tm_mon],
      today->tm_mday, today->tm_year + 1900, today->tm_hour, today->tm_min,
      today->tm_sec, ampm);
  ...
  while (result == IDRETRY) {
    if (CD_Volume_Verification(drive, szBuffer, volume_name)) {
      ...
      *cd_drive = drive;
      ...
      while ((result == IDRETRY) &&
             (CDList.Get_Index() < CDList.Get_Number_Of_Drives())) {
...
int Show_Message(HWND window_handle, const char *message1,
                 const char *message2) {
#ifndef LEAN_AND_MEAN
  ...
  WideCharToMultiByte(CodePage, 0, szString3, _MAX_PATH, szBuffer, _MAX_PATH,
                      NULL, NULL);
  result = MessageBox(window_handle, szBuffer, "Autorun",
