 ...
 ** char *filename -- file where message originated.			* char
 ...
 **
 *																			*
 * INPUT:		int		line		--	line where
 *message originated.	   	* wchar_t *filename	-- file where message
 *originated.	   	* wchar_t *fmt		-- variable argument list.
 **
 *																			*
 * OUTPUT:		none.
 **
...
void __cdecl Msg(int line, char *filename, wchar_t *fmt, UINT codepage, ...) {
  ...
  if (file.Query_Open()) {
    ...
    //---------------------------------------------------------------------
    WideCharToMultiByte(codepage, 0, szBuffer1, -1, szBuffer3, MAX_PATH * 3,
                        NULL, NULL);
...
bool StandardFileClass::Open(const char *no_path_file_name, int open_mode) {
  ...
  ASSERT(open_mode == MODE_READ_ONLY || open_mode == MODE_WRITE_ONLY ||
         open_mode == MODE_READ_AND_WRITE || open_mode == MODE_WRITE_TRUNCATE ||
...
//
  ...
  // "r+" - open existing file for read and write, starting at beginning of
  // file. File must exist. "w+" - create new file, or truncate if existing, for
  // read and write. "a+" - create new file, or append to existing, for read and
  // write.
  //
...
int StandardFileClass::Read(void *buffer, unsigned long int bytes_to_read) {
  ...
  //	Msg( __LINE__, __FILE__, "Read --- bytes_to_read = %d, items_read =
  //%d.", bytes_to_read, items_read );
...
bool StandardFileClass::Seek(int distance, int seek_file_position) {
  ...
  //
  ASSERT(Currently_Open == TRUE);
  ASSERT(seek_file_position == SEEK_SET || seek_file_position == SEEK_CUR ||
...
bool StandardFileClass::Query_Open(void) { return (Currently_Open); }

//------------------------------------------------------------------------------
// char *StandardFileClass::Query_Name_String
//------------------------------------------------------------------------------

char *StandardFileClass::Query_Name_String(void) { return (File_Name); }
...
HANDLE Open_File(char const *file_name, int mode) {
  ...
  ASSERT(mode == MODE_READ_ONLY || mode == MODE_WRITE_ONLY ||
  ...
  //
  windows_file_handle =
      CreateFile(file_name, access, share, NULL, creation,
...
int Read_File(HANDLE handle, void *buffer, unsigned long int bytes_to_read) {
  ...
  // &&& use real HANDLE
  //
  success = ReadFile((HANDLE)handle, (void *)buffer, (DWORD)bytes_to_read,
                     (DWORD *)&bytes_actually_read, NULL);
...
int Write_File(HANDLE handle, void const *buffer,
  ...
  //
  success = WriteFile((HANDLE)handle, buffer, (DWORD)bytes_to_write,
                      (DWORD *)&bytes_actually_written, NULL);
...
bool Seek_File(HANDLE handle, int distance, int seek_file_location) {
  ...
  ASSERT(handle != INVALID_FILE_HANDLE);
  // ASSERT( distance >= 0 );
  ASSERT(seek_file_location == SEEK_SET || seek_file_location == SEEK_CUR ||
  ...
  // make this a real handle
  //
  success = SetFilePointer((HANDLE)handle, distance, NULL, move_method);
...
// bool Find_File
//------------------------------------------------------------------------------
bool Find_File(char const *file_name) { return (File_Exists(file_name)); }
#endif
