...
typedef struct {
  ...
  /*  unsigned int   LanguageOffset[LanguageCount]; \\ offsets in bytes from
   * start of file to language chunk */
} LOCALEFILE_HEADERCHUNK;

/* offset LOCALEFILE_HEADERCHUNK_LANGUAGE_OFFSET bytes from the start of the
 * chunk to the language offset table */
...
typedef struct {
  ...
  unsigned int ChunkSize;   /* size of chunk in bytes */
  unsigned int StringCount; /* number of string ids in this chunk (same value in
                               all language chunks) */
  unsigned int pad;         /* must be zero */
  /*  STRINGID StringID[StringCount];  */
  /*  { */
  /*      unsigned short ID;          \\ id that user gives to look up value */
  /*      unsigned short Index;       \\ index to look up value in language
   * chunks */
  /*  } */
} LOCALEFILE_INDEXCHUNK;

/* offset LOCALEFILE_INDEXCHUNK_STRINGID_OFFSET bytes from the start of the
 * chunk to the string id table */
...
typedef struct {
  ...
  unsigned int ChunkSize;   /* size of chunk in bytes including this header and
                               all string data */
  unsigned int LanguageID;  /* language strings are in for this bank */
  unsigned int StringCount; /* number of strings in this chunk */
  /*  unsigned int   StringOffset[StringCount];   \\ offsets in bytes from start
   * of chunk to string */
  /*  const char*    Data[StringCount];           \\ StringCount null terminated
   * strings */
} LOCALEFILE_LANGUAGECHUNK;

/* offset LOCALEFILE_LANGUAGECHUNK_STRING_OFFSETbytes from the start of the
 * chunk to the string offset table */
...
int LOCALE_isinitialized(void) {
  if (lx == NULL) {
    //        TRACE("LOCALE API is not initialized - call LOCALE_init before
    //        calling LOCALE functions\n");
  }
  return (lx != NULL);
}
...
int LOCALE_init(void) {
  ...
  ASSERT(lx == NULL); /* can only call LOCALE_init after a restore or once,
                         cannot double init locale API */
...
void LOCALE_restore(void) {
  ...
  if (lx != NULL) {
    ASSERT(LOCALE_isinitialized()); /* must call LOCALE_init before calling this
                                       function */
    ASSERT(lx != NULL);
...
int LOCALE_getbanklanguageid(void) {
  ASSERT(LOCALE_isinitialized());   /* must call LOCALE_init before calling this
                                       function */
  ASSERT(lx->pBank[lx->BankIndex]); /* must load a table into bank before
...
static int readheader(GSTREAM* g) {
  ...
  Msg(__LINE__, __FILE__, "readheader - HeaderChunkSize = %d.",
      HeaderChunkSize);
  Msg(__LINE__, __FILE__, "readheader - header.LanguageCount = %d.",
      header.LanguageCount);
  Msg(__LINE__, __FILE__, "readheader - header.Flags = %d.", header.Flags);

  ASSERT(header.ChunkID ==
         LOCALEFILE_HEADERCHUNKID); /* ensure that this is a valid .loc file */
  ...
  if (header.Flags == 1) {
    ...
    Msg(__LINE__, __FILE__, "readheader - IndexChunkSize = %d.",
        IndexChunkSize);

    /* alloc and read index chunk */
    lx->pIndex[lx->BankIndex] =
        (LOCALEFILE_INDEXCHUNK*)galloc((long)IndexChunkSize);
    if (lx->pIndex[lx->BankIndex]) {
      ...
      //			VERIFY(gread(g, lx->pIndex[lx->BankIndex],
      //IndexChunkSize) == IndexChunkSize);
      if (gread(g, lx->pIndex[lx->BankIndex], IndexChunkSize) !=
          IndexChunkSize) {
        return ok;
      }
      Msg(__LINE__, __FILE__, "readheader - IndexChunkSize = %d.",
...
static int readstrings(GSTREAM* g, int LanguageID) {
  ...
  Msg(__LINE__, __FILE__, "readstrings::LanguageChunkOffsetPos = %d.",
      LanguageChunkOffsetPos);
  Msg(__LINE__, __FILE__, "readstrings::LanguageChunkPos = %d.",
      LanguageChunkPos);
  Msg(__LINE__, __FILE__, "readstrings::LanguageChunkSize = %d.",
      LanguageChunkSize);

  /* alloc and read language chunk */
  lx->pBank[lx->BankIndex] =
      (LOCALEFILE_LANGUAGECHUNK*)galloc((long)LanguageChunkSize);
  if (lx->pBank[lx->BankIndex]) {
    Msg(__LINE__, __FILE__, "readstrings:: A.");

    //		VERIFY(gseek(g, LanguageChunkPos));
    //		VERIFY(gread(g, lx->pBank[lx->BankIndex], LanguageChunkSize) ==
    //LanguageChunkSize);
    if (!gseek(g, LanguageChunkPos)) {
      return ok;
    }
    if (gread(g, lx->pBank[lx->BankIndex], LanguageChunkSize) !=
        LanguageChunkSize) {
      return ok;
    }
...
int LOCALE_loadtable(const char* PathName, int LanguageID) {
  ...
  ASSERT(LOCALE_isinitialized()); /* must call LOCALE_init before calling this
                                     function */
  ASSERT(lx->pBank[lx->BankIndex] ==
         NULL); /* bank must be empty before loading a new table */
  ASSERT(lx->pIndex[lx->BankIndex] ==
         NULL); /* bank must be empty before loading a new table */
...
void LOCALE_freetable(void) {
  if (lx != NULL) {
    ASSERT(LOCALE_isinitialized()); /* must call LOCALE_init before calling this
                                       function */
    ASSERT(lx->pBank[lx->BankIndex]); /* table must be loaded before calling
                                         this function  */
...
static int getstringbyindex(unsigned short key, const INDEX* pIndex) {
  ...
  ASSERT(LOCALE_isinitialized()); /* must call LOCALE_init before calling this
                                     function */
  ASSERT(pIndex != NULL); /* index not loaded - .loc file must have index
                             created (use -i option) */

  base = ((unsigned char*)pIndex) + LOCALEFILE_INDEXCHUNK_STRINGID_OFFSET;
  result = (unsigned short*)bsearch((unsigned char*)&key, base,
                                    pIndex->StringCount, 4, compare);
...
const char* LOCALE_getstring(int StringID) {
  ...
  if (lx->pIndex[lx->BankIndex] != NULL) {
    StringID =
        getstringbyindex((unsigned short)StringID, lx->pIndex[lx->BankIndex]);
  }

  Msg(__LINE__, __FILE__, "Locale_getstring::( %d ).", StringID);
  Msg(__LINE__, __FILE__, "Locale_getstring::( lx->BankIndex = %d ).",
      lx->BankIndex);
  Msg(__LINE__, __FILE__,
      "Locale_getstring::( lx->pBank[lx->BankIndex]->StringCount = %d ).",
      lx->pBank[lx->BankIndex]->StringCount);

  if ((StringID >= 0) &&
      (StringID < (int)(lx->pBank[lx->BankIndex]->StringCount))) {
    ...
    offset = *(unsigned int*)(p + LOCALEFILE_LANGUAGECHUNK_STRING_OFFSET +
                              StringID * 4);
