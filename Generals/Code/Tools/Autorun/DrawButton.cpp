...
DrawButton::DrawButton(int id, RECT button_rect, char *normal, char *focus,
                       char *pressed, const char *string,
                       TTFontClass *fontptr) {
  Id = id;
  ...
  Msg(__LINE__, TEXT(__FILE__), TEXT("	rect	= [%d,%d,%d,%d]"), rect.left,
      rect.top, rect.right, rect.bottom);
  Msg(__LINE__, TEXT(__FILE__), TEXT("	MyRect	= [%d,%d,%d,%d]"), MyRect.X,
      MyRect.Y, MyRect.Width, MyRect.Height);
}

DrawButton::DrawButton(int id, RECT button_rect, char *normal, char *focus,
                       char *pressed, const wchar_t *string,
                       TTFontClass *fontptr) {
  Id = id;
...
void DrawButton::Draw_Text(HDC hDC) {
  ...
  */
  ...
  if (Get_State() == DrawButton::PRESSED_STATE) {
    MyFontPtr->Print(hDC, String, rect, TEXT_PRESSED_COLOR,
                     TEXT_PRESSED_SHADOW_COLOR, TPF_BUTTON, TPF_SHADOW);

  } else if (Get_State() == DrawButton::FOCUS_STATE) {
    MyFontPtr->Print(hDC, String, rect, TEXT_FOCUSED_COLOR,
                     TEXT_FOCUSED_SHADOW_COLOR, TPF_BUTTON, TPF_SHADOW);

  } else {
    MyFontPtr->Print(hDC, String, rect, TEXT_NORMAL_COLOR,
                     TEXT_NORMAL_SHADOW_COLOR, TPF_BUTTON, TPF_SHADOW);
  }
}
...
//
// WARNINGS:	No keyboard/mouse/paint handling built in.  Do manually.
//				Note: width is shortened below to accomodate
//actual bitmap area.
//
// HISTORY:
//   07/15/1996  MML : Created.
...
bool DrawButton::Is_Mouse_In_Region(int mouse_x, int mouse_y) {
  ...
  if ((mouse_x >= rect.left) && (mouse_y >= rect.top) &&
