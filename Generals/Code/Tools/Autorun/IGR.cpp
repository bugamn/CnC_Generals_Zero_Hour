 ...
 * WARNINGS:   none *
 *                                                                                           *
 * HISTORY: * 07/05/00 JeffB: Initial coding
 **
 *===========================================================================================*/
bool IGROptionsClass::Init(void) {
  ...
  if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, key, 0, KEY_ALL_ACCESS, &handle) ==
      ERROR_SUCCESS) {
    // If successful, get the options
    IGROptionsType ReadOptions = 0;

    returnValue = RegQueryValueEx(
        handle, WOLAPI_REG_KEY_OPTIONS, NULL, (unsigned long *)&type,
        (unsigned char *)&ReadOptions, (unsigned long *)&size);

    if (returnValue == ERROR_SUCCESS) {
      // If successful, and we got a DWORD, store options and set the valid
      // flage
      if (type == REG_DWORD) {
        options = ReadOptions;
        valid = true;
...
/***********************************************************************************************
 * IGROptions::Is_Storing_Nicks_Allowed -- Set the passed options in the
 *registry			   *
 *                                                                                             *
 * INPUT:   None
 **
...
/***********************************************************************************************
 * IGROptions::Is_Running_Reg_App_Allowed -- Set the passed options in the
 *registry			   *
 *                                                                                             *
 * INPUT:   None
 **
...
bool IGROptionsClass::Set_Options(IGROptionsType options) {
  ...
  if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, key, 0, KEY_ALL_ACCESS, &handle) !=
    ...
    if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, key, 0, NULL,
                       ...
                       (unsigned long *)&disp) != ERROR_SUCCESS)
      return false;
  }

  if (RegSetValueEx(handle, WOLAPI_REG_KEY_OPTIONS, 0, REG_DWORD,
                    (unsigned char *)&options, sizeof(int)) == ERROR_SUCCESS) {
    ReturnValue = true;
  }
  RegCloseKey(handle);
