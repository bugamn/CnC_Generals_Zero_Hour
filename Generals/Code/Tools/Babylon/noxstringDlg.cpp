...
HCURSOR CNoxstringDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }
...
// DEL 	CFileDialog *dlg = new CFileDialog ( TRUE, "xls", "*.xls",
// OFN_FILEMUSTEXIST, szFilter, this ); DEL 	if ( dlg ) DEL 	{ DEL
// dlg->DoModal (); DEL 		if ( dlg->GetPathName() != "*.xls" ) DEL
...
// DEL 	// send this notification unless you override the
// CDialog::OnInitDialog() DEL 	// function and call
// CRichEditCtrl().SetEventMask() DEL 	// with the ENM_CHANGE flag ORed into
// the mask. DEL DEL 	// TODO: Add your control notification handler code here
// DEL
// DEL }
...
void CNoxstringDlg::OnExport() {
  if (CanOperate()) {
    ...
    if (dlg.DoModal() == IDOK) {
      ExportTranslations(MainDB, dlg.Filename(), dlg.Language(), dlg.Options(),
                         this);
    }
  }
}
...
// DEL 		SetDlgItemText ( IDC_STATUS, "File selected: Click 'convert' to
// start process"); DEL 		wnd->EnableWindow  ( TRUE ); DEL
// return TRUE; DEL 	} DEL 	if ( buffer ) DEL 	{ DEL
// AfxMessageBox ("Must be an Excel file"); DEL 	} DEL wnd->EnableWindow
// ( FALSE ); DEL 	SetDlgItemText ( IDC_STATUS, "Select excel file...");
// DEL 	SetDlgItemText ( IDC_XLFILE, "Browse or drop excel file to be
// convertered" ); DEL DEL 	return FALSE; DEL }
...
// DEL 			{
// DEL 				SetDlgItemText ( IDC_STATUS, "Created .xls
// file"); DEL 			} DEL 			else DEL
// { DEL 				SetDlgItemText ( IDC_STATUS, "Falied to
// create .xls file"); DEL 			} DEL DEL 		} DEL
// else DEL 		{ DEL 		SetDlgItemText ( IDC_STATUS, "Failed to
// parse .str file"); DEL DEL 		} DEL DEL 		EnableWindow  (
// TRUE ); DEL 		return TRUE; DEL 	} DEL 	return FALSE; DEL }
...
static DWORD CALLBACK streamin_cb(DWORD dwCookie, LPBYTE pbBuff, LONG bytes,
                                  LONG *transfered) {
...
static DWORD CALLBACK streamout_cb(DWORD dwCookie, LPBYTE pbBuff, LONG bytes,
                                   LONG *transfered) {
...
static int readToEndOfQuote(FILE *file, char *in, char *out, char *wavefile,
                            int maxBufLen, int in_comment) {
  int slash = FALSE;
  int state = 0;
...
int CNoxstringDlg::LoadStrFile(TransDB *db, const char *filename,
                               void (*cb)(void)) {
  FILE *file = NULL;
  NoxLabel *label = NULL;
  int status = FALSE;
  ...
  while (fgets(buffer, sizeof(buffer) - 1, file)) {
    ...
    while (TRUE) {
      ...
      if (buffer[0] == '"') {
        ...
        strcat(buffer, "\n");
        line_number += readToEndOfQuote(file, &buffer[1], buffer2, buffer3,
                                        sizeof(buffer2) - 1, FALSE);
...
int CNoxstringDlg::CanProceed(void) {
  ...
  retry:
    int result = AfxMessageBox(
        "Main database has changed!\n\n Do you wish to save it before "
        "proceeding?",
        MB_YESNOCANCEL);
...
int CNoxstringDlg::CanOperate(void) {
  ...
  if (NoxstrDB->IsChanged() || NoxstrDB->HasErrors()) {
    ...
    if (NoxstrDB->HasErrors()) {
      string =
          "Generals.str has errors! As a result the translation database is "
          "not up to date!\n\nRecommend you fix problems in Nox.str before "
          "proceeding.\n\n\n\nDo you wish to continue anyway?";
    }

    if (NoxstrDB->IsChanged()) {
      string =
          "The translation database is not up to date! Generals.str has "
          "changed since the last time the database was updated.\n\nRecommend "
          "you update the database before proceeding.\n\n\n\nDo you wish to "
          "continue anyway?";
    }
...
void CNoxstringDlg::OnReload() {
  ...
  if (FileExists(NoxstrFilename)) {
    if ((errors = ValidateStrFile(NoxstrFilename))) {
      if (errors == -1) {
        if (AfxMessageBox(
                "Unable to verify string file!\n\nMake sure \"strcheck.exe\" "
                "is in your path and \"strcheck.rst\" is writeable.\n\nDo you "
                "wish to continue loading? \n\nWarning: Any errors in the "
                "string file could cause inappropiate updates to the database.",
                MB_YESNO) == IDYES) {
          errors = 0;
        }
      } else {
        sprintf(buffer,
                "\"%s\" has %d formating error%s!\n\nFile will not be loaded.",
                NoxstrFilename, errors, errors == 1 ? "" : "s");
        AfxMessageBox(buffer);
      }
    }
    if (!errors) {
  ...
  if (str_loaded) {
    ...
    if ((num_errors = NoxstrDB->Errors())) {
      sprintf(buffer,
              "Generals.str has %d error(s):\n\nClick \"Errors\" for a "
              "detailed list.\n\nAll errors must be fixed before \"Update\" "
              "will be enabled.",
              num_errors);
      AfxMessageBox(buffer);
      win = GetDlgItem(IDC_UPDATE);
  ...
  if (str_loaded && !db_error && !num_errors) {
    if (UpdateDB(NoxstrDB, MainDB, FALSE)) {
      ...
      if (db_loaded) {
        if (db_readonly) {
          sprintf(buffer,
                  "\"%s\" has changed!\n\nHowever, as the database is READ "
                  "ONLY you cannot update the changes.",
                  NoxstrFilename);
        } else {
          sprintf(
              buffer,
              "\"%s\" has changed!\n\nRecomended that you update the database "
              "with these new changes.\n\nDo you wish to update now?",
              NoxstrFilename);
        }
      } else {
        sprintf(buffer,
                "New Database!\n\nRecomended that you update the new "
                "database.\n\nDo you wish to update now?",
                NoxstrFilename);
      }
...
#define MACRO_UPDATE(field, count)                            \
  {                                                           \
    if (wcsicmp(source->##field(), destination->##field())) { \
      if (update) {                                           \
...
int CNoxstringDlg::UpdateLabel(NoxLabel *source, NoxLabel *destination,
                               UPDATEINFO &info, int update, int skip) {
  NoxText *stext, *dtext;
  ListSearch sh;
...
int CNoxstringDlg::UpdateDB(TransDB *source, TransDB *destination, int update) {
  ...
  if (update) {
    sprintf(buffer, "Updating \"%s\" from \"%s\"...", destination->Name(),
            source->Name());
    Log("");
    Status(buffer);
  } else {
  ...
  // go through all unprocessed labels in the destination database and remove
  // them.
  ...
  if (update) {
    if (info.new_labels) {
      sprintf(buffer, "Added %d new label%c", info.new_labels,
              info.new_labels == 1 ? ' ' : 's');
      Log(buffer);
      changes = TRUE;
    }
    ...
    if (info.deleted_labels) {
      sprintf(buffer, "Deleted %d label%c", info.deleted_labels,
              info.deleted_labels == 1 ? ' ' : 's');
      Log(buffer);
      changes = TRUE;
    }

    if (info.modified_labels) {
      sprintf(buffer, "Modified %d label%c", info.modified_labels,
              info.modified_labels == 1 ? ' ' : 's');
      Log(buffer);
      changes = TRUE;
    }

    if (info.new_strings) {
      sprintf(buffer, "Added %d new string%c", info.new_strings,
              info.new_strings == 1 ? ' ' : 's');
      Log(buffer);
      changes = TRUE;
    }

    if (info.deleted_strings) {
      sprintf(buffer, "Deleted %d string%c", info.deleted_strings,
              info.deleted_strings == 1 ? ' ' : 's');
      Log(buffer);
      changes = TRUE;
    }

    if (info.modified_strings) {
      sprintf(buffer, "Modified %d string%c", info.modified_strings,
              info.modified_strings == 1 ? ' ' : 's');
      Log(buffer);
      changes = TRUE;
    }

    if (info.skipped_labels) {
      sprintf(buffer, "Skipped %d label%c", info.skipped_labels,
              info.skipped_labels == 1 ? ' ' : 's');
      Log(buffer);
      changes = TRUE;
    }

    if (info.updated_comments) {
      sprintf(buffer, "Updated %d comment%c", info.updated_comments,
              info.updated_comments == 1 ? ' ' : 's');
      Log(buffer);
      changes = TRUE;
    }

    if (info.updated_contexts) {
      sprintf(buffer, "Updated %d context%c", info.updated_contexts,
              info.updated_contexts == 1 ? ' ' : 's');
      Log(buffer);
      changes = TRUE;
    }

    if (info.updated_speakers) {
      sprintf(buffer, "Updated %d speaker%c", info.updated_speakers,
              info.updated_speakers == 1 ? ' ' : 's');
      Log(buffer);
      changes = TRUE;
    }

    if (info.updated_listeners) {
      sprintf(buffer, "Updated %d listener%c", info.updated_listeners,
              info.updated_listeners == 1 ? ' ' : 's');
      Log(buffer);
      changes = TRUE;
    }

    if (info.updated_maxlen) {
      sprintf(buffer, "Updated %d max length%c", info.updated_maxlen,
              info.updated_maxlen == 1 ? ' ' : 's');
      Log(buffer);
      changes = TRUE;
    }

    if (info.updated_waves) {
      sprintf(buffer, "Updated %d speech file%c", info.updated_waves,
              info.updated_waves == 1 ? ' ' : 's');
      Log(buffer);
      changes = TRUE;
    }
  ...
  if (CanOperate()) {
    ...
    if (fd.DoModal() == IDOK) {
      if (ImportTranslations(MainDB, (LPCSTR)fd.GetPathName(), this) == -1) {
        //				ProcessWaves ( NoxstrDB, fd.GetPathName
        //(), this );
      }
    }
  }
...
void CNoxstringDlg::OnGenerate() {
  if (CanOperate()) {
    ...
    if (dlg.DoModal() == IDOK) {
      GenerateGameFiles(MainDB, dlg.FilePrefix(), dlg.Options(),
                        dlg.Langauges(), this);
    }
  }
}
...
int CNoxstringDlg::ValidateStrFile(const char *filename) {
  ...
  if (!CreateProcess(NULL, buffer, NULL, NULL, FALSE, 0, NULL, NULL,
...
void CNoxstringDlg::VerifyTranslations(TransDB *db, LangID langid) {
  ...
  if (info->numstrings) {
    if (info->too_big || info->missing || info->retranslate ||
        info->bad_format) {
      if (info->missing) {
      ...
      if (langid == LANGID_US) {
        sprintf(buffer, "Recommemd editing \"%s\" to fix problems and reload",
                NoxstrFilename);
      } else {
        sprintf(buffer,
                "Recommemd exporting translations for update and re-import");
      }
      Log(buffer);
    } else {
      Log("OK", SAME_LINE);
...
void CNoxstringDlg::OnReports() {
  ...
  if (CanOperate()) {
    ...
    if (dlg.DoModal() == IDOK) {
      GenerateReport(MainDB, dlg.Filename(), dlg.Options(), dlg.Langauges(),
                     this);
    }
  }
}
...
void CNoxstringDlg::OnReset() {
  ...
  if (CurrentLanguage != LANGID_UNKNOWN) {
    sprintf(buffer, "Are you sure you want to invalidate all %s dialog?",
