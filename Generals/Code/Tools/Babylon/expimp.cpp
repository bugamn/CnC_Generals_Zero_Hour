...
static void writeText(NoxText *text, int row) {
  ...
  swprintf(buffer, L"=IF(%c%d>%c%d,\"Too long!\",\" \")", 'A' + CELL_STRLEN - 1,
           row, 'A' + CELL_MAXLEN - 1, row);
...
static int export_trans(TransDB *db, LangID langid, TROPTIONS *options,
                        void (*cb)(void), int write) {
  NoxLabel *label;
  NoxText *text;
  Translation *trans;
  ...
  while (label) {
    ...
    while (text) {
      ...
      if (options->filter == TR_UNSENT) {
        ...
        export = text->IsDialog() &&
                 text->DialogIsPresent(DialogPath, langid) &&
        ...
        if (write) {
...

          {
            ...
            if (langid == LANGID_JABBER ||
                (trans &&
                 (options->include_translations || too_long || bad_format))) {
              if (langid == LANGID_JABBER) {
...
int ExportTranslations(TransDB *db, const char *filename, LangID langid,
                       TROPTIONS *options, CNoxstringDlg *dlg) {
  int exports;
  ...
  if (!FileExists(buffer)) {
    if (dlg) {
      ...
      sprintf(buffer2, "Template file \"%s\" is missing. Cannot export.",
              buffer);
  ...
  if (NewWorkBook(buffer)) {
    if ((exports = export_trans(db, langid, options, progress_cb, TRUE)) !=
        -1) {
    ...
    if (dlg) {
      dlg->Log("FAILED", SAME_LINE);
      sprintf(buffer2,
              "Failed to create new work book. File \"%s\" may be corrupt",
              buffer);
      AfxMessageBox(buffer2);
      dlg->Log(buffer2);
    }
...
static int import_trans(TransDB *db, LangID langid, void (*cb)(void),
                        CNoxstringDlg *dlg) {
  int row = 3;
  int id;
  int count = 0;
  ...
  while ((id = GetInt(row, CELL_STRINGID)) != -1) {
    ...
    if (text->Revision() < revision) {
      if (dlg) {
        sprintf(buffer,
                "ERROR: expecting revision %d for string ID %d but found "
                "revision %d. Possible bad ID!",
                text->Revision(), id, revision);
        dlg->Log(buffer);
      }
      error_count++;
      goto next;
    ...
    if (wcscmp(text->Get(), olebuf)) {
      ...
      if (!olebuf2[0] || wcscmp(text->Label()->Name(), olebuf2)) {
        ...
        // It is possible that the xl was resorted so ask user to do a visual
        // confirmation

        bad_id = dlg.DoModal() == IDNO;
      } else if (text->Label()->FindText(olebuf)) {
        // we did find the label but text other than the current ID sourced text
        // matches with the import text this means the ID is definitely wrong
        bad_id = TRUE;
      } else {
        bad_id = FALSE;
      }
  ...
  if (bad_id) {
    ...
    AfxMessageBox(
        "The imported translation file has bad string IDs! Fix the string IDs "
        "and re-import");
  }
  return count;
}

static int update_sent_trans(TransDB *db, LangID langid, void (*cb)(void),
                             CNoxstringDlg *dlg) {
  int row = 3;
  int id;
  int count = 0;
...
int ImportTranslations(TransDB *db, const char *filename, CNoxstringDlg *dlg) {
  ...
  if (OpenWorkBook(filename)) {
    ...
    if (!info) {
      if (dlg) {
        AfxMessageBox(
            "Import file is of an unknown language or is not a translation "
            "file");
        dlg->Log("FAILED", SAME_LINE);
        dlg->Ready();
      }
...
static int generate_noxstr(TransDB *db, const char *filename, LangID langid,
                           GNOPTIONS *options) {
  int ok = FALSE;
  FILE *file;
...
static int writeCSFString(FILE *file, OLECHAR *string, char *wave,
                          LANGINFO *linfo) {
  int id = CSF_STRING;
  int len;
...
static int generate_csf(TransDB *db, const char *filename, LangID langid,
                        GNOPTIONS *options) {
  CSF_HEADER header;
  int header_size;
  int ok = FALSE;
...
int GenerateGameFiles(TransDB *db, const char *filepattern, GNOPTIONS *options,
                      LangID *languages, CNoxstringDlg *dlg) {
  ...
  while ((langid = *languages++) != LANGID_UNKNOWN) {
    ...
    sprintf(filename, "%s_%s.%s", filepattern, info->initials,
            options->format == GN_NOXSTR ? "str" : "csf");
    strlwr(filename);
        ...
        if (trnwarning || dlgwarning) {
          ...
          if (trnwarning) {
            ...
            if ((missing = trnreport.missing + trnreport.retranslate)) {
              sprintf(buffer, "%d translation%s missing", missing,
                      missing > 1 ? "s are" : " is");
              dlg->Log(buffer);
            }

            if (trnreport.too_big) {
              sprintf(buffer, "%d string%s too big", trnreport.too_big,
                      trnreport.too_big > 1 ? "s are" : " is");
              dlg->Log(buffer);
            }

            if (trnreport.bad_format) {
              sprintf(buffer, "%d translation%s bad format",
                      trnreport.bad_format,
                      trnreport.bad_format > 1 ? "s have a" : " has a");
              dlg->Log(buffer);
            }
          }
          ...
          if (dlgwarning) {
            if (dlgreport.missing) {
              sprintf(buffer, "%d dialog%s missing", dlgreport.missing,
                      dlgreport.missing > 1 ? "s are" : " is");
              dlg->Log(buffer);
            }

            if (dlgreport.unresolved) {
              sprintf(buffer, "%d dialog%s not verified", dlgreport.unresolved,
                      dlgreport.unresolved > 1 ? "s are" : " is");
              dlg->Log(buffer);
            }
          }
        } else {
...
int GenerateReport(TransDB *db, const char *filename, RPOPTIONS *options,
                   LangID *languages, CNoxstringDlg *dlg) {
  LangID langid;
  int count = 0;
  int num;
...
int UpdateSentTranslations(TransDB *db, const char *filename,
                           CNoxstringDlg *dlg) {
  int imports = -1;
