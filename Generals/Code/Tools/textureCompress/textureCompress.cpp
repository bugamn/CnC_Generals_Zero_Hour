...
static void debugLog(const char* fmt, ...) {
  ...
  puts(buffer);
  if (theDebugMunkee) fputs(buffer, theDebugMunkee->m_fp);
}
...
static void usage(const char* progname) {
  if (!progname) progname = "textureCompress";
...
static time_t FileTimeToTimet(const FILETIME& ft) {
  LONGLONG ll =
      (ft.dwHighDateTime << 32) + ft.dwLowDateTime - 116444736000000000;
  ll /= 10000000;
  return (time_t)ll;
}
...
void FileInfo::set(const WIN32_FIND_DATA& info) {
  ...
  modTime =
      origStat
          .st_mtime;  // use stat(), since the LONGLONG code is unpredictable
...
FileInfoSet* Directory::getFiles(void) { return &m_files; }

FileInfoSet* Directory::getSubdirs(void) { return &m_subdirs; }
...
void eraseCachedFiles(const std::string& sourceDirName,
  ...
  for (sit = cachedFilesToErase.begin(); sit != cachedFilesToErase.end();
       ++sit) {
...
void compressOrigFiles(const std::string& sourceDirName,
  ...
  HANDLE h = CreateFile(tmpFname, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
  ...
  StringSet::const_iterator sit;
  for (sit = origFilesToCompress.begin(); sit != origFilesToCompress.end();
       ++sit) {
...
//-------------------------------------------------------------------------------------------------
static void scanDir(const std::string& sourceDirName,
                    const std::string& targetDirName,
                    const std::string& cacheDirName,
                    const std::string& dxtOutFname) {
  ...
  for (FileInfoSet::iterator targetIt = targetFiles->begin();
       targetIt != targetFiles->end(); ++targetIt) {
    FileInfo f = *targetIt;
  ...
  for (FileInfoSet::iterator cacheIt = cacheFiles->begin();
       cacheIt != cacheFiles->end(); ++cacheIt) {
    FileInfo f = *cacheIt;
    ...
    if (fit != sourceFiles->end()) {
      ...
      if (f.modTime < sf.modTime) {
        ...
        if (it == targetFiles->end()) cachedFilesToCopy.insert(fname);
      }
    } else {
      cachedFilesToErase.insert(fname);
    }
  ...
  for (FileInfoSet::iterator sourceIt = sourceFiles->begin();
       sourceIt != sourceFiles->end(); ++sourceIt) {
    FileInfo f = *sourceIt;
  ...
  // now dump our files
  eraseCachedFiles(sourceDirName, targetDirName, cacheDirName,
                   cachedFilesToErase);
  copyCachedFiles(sourceDirName, targetDirName, cacheDirName,
                  cachedFilesToCopy);
  copyOrigFiles(sourceDirName, targetDirName, cacheDirName, origFilesToCopy);
  compressOrigFiles(sourceDirName, targetDirName, cacheDirName,
                    origFilesToCompress, dxtOutFname);
}
