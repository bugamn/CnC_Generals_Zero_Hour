...
void dumpHelp(const char *exe) {
  DEBUG_LOG(
      ("Usage:\n  To print the compression type of an existing file: %s -in "
       "infile\n",
       exe));
  DEBUG_LOG(
      ("  To compress a file: %s -in infile -out outfile <-type "
       "compressionmode>\n\n",
       exe));
...
void main(int argc, char **argv) {
    ...
    if (!strcmp(argv[i], "-type")) {
      ...
      if (i < argc) {
        for (int j = COMPRESSION_MIN; j <= COMPRESSION_MAX; ++j) {
          if (!stricmp(CompressionManager::getCompressionNameByType(
                           (CompressionType)j),
                       argv[i])) {
  ...
  if (outFile.empty()) {
    ...
    DEBUG_LOG(
        ("'%s' is compressed using %s, from %d to %d bytes, %g%% of its "
         "original size\n",
