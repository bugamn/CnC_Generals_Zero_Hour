...
-------------------------------------------------------------------------------
 ...
 This does not download patches or do version checks, the game/app is
responsible for that.  This just applies patches that are in the correct
location for the game.  All patches should be in the "Patches" folder of the
app.
  ...
  while (true) {
    ...
    if (launchgame) {
      ...
      if (exit_code == UPDATE_RETVAL) {
        ...
        strcpy(patchgrab.command,
               "patchget.dat");  // the program that grabs game patches
...
int main(int argc, char *argv[]) {
  ...
  if (hasSecondEXE) {
    ...
    if (cutoffTime == 0) {
      // We didn't have the FLAG parameter; somebody's been hacking.  No game
      // for you!  Bad hacker!
    ...
    if (time(NULL) > cutoffTime) {
      ...
      // Its still early in the product's lifetime, so run the 2nd (SafeDisk'd)
      // launcher. We don't have to wait around since it'll do the entire talk
      // to game, look for patches, etc. deal.
...
void CreatePrimaryWin(char *prefix) {
  ...
  */
  HWND hwnd = CreateWindowEx(
      WS_EX_TOPMOST, name, name, WS_POPUP, 0, 0, GetSystemMetrics(SM_CXSCREEN),
