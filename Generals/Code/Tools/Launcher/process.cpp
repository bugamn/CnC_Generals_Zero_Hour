...
bit8 Create_Process(Process &process) {
...
#ifndef COPY_PROTECT
  retval = CreateProcess(NULL, cmdargs, NULL, NULL, FALSE, 0, NULL,
                         NULL /*process.directory*/, &si, &piProcess);
#else
#ifdef USE_GAMEDIR_FROM_LCF
  retval = CreateProcess(NULL, cmdargs, NULL, NULL, TRUE, 0, NULL,
                         process.directory, &si, &piProcess);
#else
  retval = CreateProcess(NULL, cmdargs, NULL, NULL, TRUE, 0, NULL,
                         NULL /*process.directory*/, &si, &piProcess);
#endif
  ...
  DBGMSG("(" << retval << ") New process:  HANDLE "
             << (void *)piProcess.hProcess << "   ID "
             << (DWORD)piProcess.dwProcessId);
  DBGMSG("(" << retval << ") New thread:  HANDLE " << (void *)piProcess.hThread
             << "   ID " << (DWORD)piProcess.dwThreadId);
  if (retval == 0) {
    char message_buffer[256];
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(),
                  MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), &message_buffer[0],
                  256, NULL);
