...
//
BOOL CALLBACK Update_Info_Proc(HWND hwnd, UINT iMsg, WPARAM wParam,
                               LPARAM lParam) {
  ...
  switch (iMsg) {
    case WM_INITDIALOG: {
      ...
      while (fgets(line, 255, in)) {
        ...
        SendDlgItemMessage(hwnd, IDC_TEXT, EM_SETSEL, (WPARAM)lastsel,
                           (LPARAM)lastsel);
        SendDlgItemMessage(hwnd, IDC_TEXT, EM_REPLACESEL, 0, (LPARAM)(line));
        SendDlgItemMessage(hwnd, IDC_TEXT, EM_GETSEL, (WPARAM)NULL,
                           (LPARAM)&lastsel);
      }
      unselectText = 1;
      fclose(in);
    ...
    case WM_PAINT:
      if (unselectText) SendDlgItemMessage(hwnd, IDC_TEXT, EM_SETSEL, -1, 0);
      unselectText = 0;
      return (0);
      break;
...
void Apply_Patch(char *patchfile, ConfigFile &config, int skuIndex) {
  ...
  // RTPatch type patch
  //
  else if (strcasecmp(patchfile + strlen(patchfile) - strlen(".rtp"), ".rtp") ==
           0) {
    MSG msg;
    ...
    DialogBox(Global_instance, MAKEINTRESOURCE(IDD_CHANGELOG), NULL,
              (DLGPROC)Update_Info_Proc);
#endif
  }
  //
  // Execute now (without rebooting) type patch
  //
  else if (strcasecmp(patchfile + strlen(patchfile) - strlen(".exn"), ".exn") ==
           0) {
    Process proc;
  ...
  //
  else if (strcasecmp(patchfile + strlen(patchfile) - strlen(".web"), ".web") ==
           0) {
    char message[256];
