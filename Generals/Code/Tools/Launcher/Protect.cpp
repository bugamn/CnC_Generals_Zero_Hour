...
const char* const LAUNCHER_GUID =
    "150C6462-4E49-4ccf-B073-57579569D994";  // Generals Multiplayer Test
                                             // Launcher GUID
const char* const protectGUID =
    "6096561D-8A70-48ed-9FF8-18552419E50D";  // Generals Multiplayer Test
                                             // Protect GUID
...
void InitializeProtect(void) {
  ...
  if ((mLauncherMutex == NULL) ||
      (mLauncherMutex && (GetLastError() == ERROR_ALREADY_EXISTS))) {
  ...
  if ((mMappedFile == NULL) ||
      (mMappedFile && (GetLastError() == ERROR_ALREADY_EXISTS))) {
...
CDAPFN_DECLARE_GLOBAL(SendProtectMessage, CDAPFN_OVERHEAD_L5,
                      CDAPFN_CONSTRAINT_NONE);
...
void SendProtectMessage(HANDLE process, DWORD threadID) {
  ...
  LPVOID mapAddress =
      MapViewOfFileEx(mMappedFile, FILE_MAP_ALL_ACCESS, 0, 0, 0, NULL);
  ...
  if (buffer && (bufferSize > 0)) {
    ...
    if (result != ERROR_SUCCESS) {
      result = RegOpenKeyEx(HKEY_CURRENT_USER,
                            "Software\\Electronic Arts\\EA Games\\Generals", 0,
                            KEY_READ, &hKey);
    ...
    if (result == ERROR_SUCCESS) {
      ...
      result = RegQueryValueEx(hKey, "InstallPath", NULL, &type, installPath,
                               &sizeOfBuffer);

      assert((result == ERROR_SUCCESS) &&
             "Failed to obtain game install path!");
      assert((strlen((const char*)installPath) > 0) &&
             "Game install path invalid!");
      ...
      assert((result == ERROR_SUCCESS) &&
             "Failed to open game serial registry key");

      if (result == ERROR_SUCCESS) {
        result = RegQueryValueEx(hKey, "", NULL, &type, gameSerialNumber,
                                 &sizeOfBuffer);
        assert((result == ERROR_SUCCESS) &&
               "Failed to obtain game serial number!");
        assert((strlen((const char*)gameSerialNumber) > 0) &&
               "Game serial number invalid!");
      }
    ...
    if (result == ERROR_SUCCESS) {
      ...
      result = RegQueryValueEx(hKey, "ProductID", NULL, &type, winProductID,
                               &sizeOfBuffer);

      assert((result == ERROR_SUCCESS) &&
             "Failed to obtain windows product ID!");
      assert((strlen((const char*)winProductID) > 0) &&
  ...
  handles[1] = process;
  DWORD waitResult =
      WaitForMultipleObjects(2, &handles[0], FALSE, ((5 * 60) * 1000));
