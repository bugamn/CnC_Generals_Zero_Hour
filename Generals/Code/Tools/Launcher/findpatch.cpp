...
//   - *.exe = Executable that should be put in the RunOnce registry entry &
//   reboot
...
bit8 Get_App_Dir(OUT char *filename, int maxlen, ConfigFile &config,
  ...
  if (config.getString(string, key) == FALSE) return (FALSE);
  ...
  /////////////DWORD  regPrevious;
  regRetval =
      RegOpenKeyEx(HKEY_LOCAL_MACHINE, path.get(), 0, KEY_READ, &regKey);
