...
bit8 LoadBmp::init(char *filename, HWND hwnd) {
  ...
  hBitmapFile = CreateFile(filename, GENERIC_READ, FILE_SHARE_READ,
                           (LPSECURITY_ATTRIBUTES)NULL, OPEN_EXISTING,
                           FILE_ATTRIBUTE_READONLY, (HANDLE)NULL);

  if (hBitmapFile == NULL) return (FALSE);
  ...
  // Retrieve the BITMAPFILEHEADER structure.
  ReadFile(hBitmapFile, &bitmapInfoHeader, sizeof(BITMAPINFOHEADER), &dwRead,
           (LPOVERLAPPED)NULL);
  ...
  BitmapHandle_ = CreateDIBitmap(hdc, &bitmapInfoHeader, CBM_INIT, lpvBits,
                                 lpHeaderMem, DIB_RGB_COLORS);
  ReleaseDC(hwnd, hdc);

  if (BitmapHandle_ == NULL) return (FALSE);
...
bit8 LoadBmp::drawBmp(void) {
  ...
  StretchBlt(ps.hdc, 0, 0, clientRect.right, clientRect.bottom, hdcMem, 0, 0,
