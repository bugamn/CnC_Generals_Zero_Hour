 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * GameMtl::GameMtl -- constructor * GameMtl::~GameMtl --
 *destructor                                                           *
 *   GameMtl::ClassID -- returns the max ClassID of the material plugin *
 *   GameMtl::SuperClassID -- returns the super class ID * GameMtl::GetClassName
 *-- returns the name of this plugin clas                             *
 *   GameMtl::NumSubs -- returns the number of sub animations *
 *   GameMtl::SubAnimName -- returns the name of the i'th sub animation *
 *   GameMtl::SubAnim -- returns the i'th sub-anim * GameMtl::Clone -- clones
 *this material                                                    *
 *   GameMtl::NotifyRefChanged -- NotifyRefChanged handler *
 *   GameMtl::SetReference -- set the i'th reference * GameMtl::GetReference --
 *returnst the i'th reference                                      *
 *   GameMtl::NumSubTexmaps -- returns the number of texture maps in this
 *material             * GameMtl::SetSubTexmap -- set the i'th texture map *
 ...
 *   GameMtl::Reset -- reset this material to default settings * GameMtl::Update
 *-- time has changed                                                       *
 ...
 *   GameMtl::Load -- loading from a MAX file * GameMtl::Save -- Saving into a
 *MAX file                                                   * GameMtl::Shade --
 *evaluate the material for the renderer.                                 *
 *   GameMtl::ps2_shade -- Emulate the PS2 shader. *
 *   GameMtl::Compute_PC_Shader_From_PS2_Shader -- Determine if a PC shader can
 *be created.    * GameMtl::Compute_PS2_Shader_From_PC_Shader -- Change a W3D
 *material to a PS2 W3D material.*
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
enum {
  ...
  PB_STAGE1_MAPPING_TYPE = 53,  // (gth) this can't be 55, it is used as an
                                // array index	// = 55,	// yes, 55
...
static ParamBlockDescID PassParameterBlockDescVer1[] = {
    ...
    {TYPE_FLOAT, NULL, TRUE, 5},   // Opacity
    {TYPE_FLOAT, NULL, TRUE, 6},   // Translucency
    {TYPE_BOOL, NULL, FALSE,
     34},                         // Copy specular to diffuse (new to version 1)
    {TYPE_INT, NULL, FALSE, 7},   // Mapping Type
    {TYPE_INT, NULL, FALSE, 8},   // PSX Translucency Type
    {TYPE_BOOL, NULL, FALSE, 9},  // PSX Lighting Flag
    ...
    {TYPE_BOOL, NULL, FALSE,
     36},  // Stage0 Texture No Mipmap (new to version 1)
    ...
    {TYPE_BOOL, NULL, FALSE,
     41},  // Stage1 Texture No Mipmap (new to version 1)
    ...
    {TYPE_BOOL, NULL, FALSE,
     45},  // Stage0 Texture Alpha Bitmap (new to version 1)
    {TYPE_BOOL, NULL, FALSE,
     46},  // Stage1 Texture Alpha Bitmap (new to version 1)

    {TYPE_BOOL, NULL, FALSE, 47},  // Alpha Test (new to version 1)
    {TYPE_INT, NULL, FALSE,
     48},  // Shader preset (new to version 1) (now obsolete and ignored)
};
...
static ParamBlockDescID PassParameterBlockDescVer5[] = {
    ...
    {TYPE_BOOL, NULL, FALSE, 23},  // Stage0 Texture Publish
    {TYPE_BOOL, NULL, FALSE,
     35},  // Stage0 Texture Resize (new to version 1) OBSOLETE!
    {TYPE_BOOL, NULL, FALSE,
     36},  // Stage0 Texture No Mipmap (new to version 1) OBSOLETE!
    {TYPE_BOOL, NULL, FALSE, 37},   // Stage0 Texture Clamp U (new to version 1)
    {TYPE_BOOL, NULL, FALSE, 38},   // Stage0 Texture Clamp V (new to version 1)
    {TYPE_INT, NULL, FALSE, 39},    // Stage0 Texture Hint (new to version 1)
    {TYPE_BOOL, NULL, FALSE, 24},   // Stage0 Texture Display (in viewport...)
    ...
    {TYPE_BOOL, NULL, FALSE, 29},  // Stage1 Texture Publish
    {TYPE_BOOL, NULL, FALSE,
     40},  // Stage1 Texture Resize (new to version 1)	 OBSOLETE!
    {TYPE_BOOL, NULL, FALSE,
     41},  // Stage1 Texture No Mipmap (new to version 1) OBSOLETE!
    {TYPE_BOOL, NULL, FALSE, 42},   // Stage1 Texture Clamp U (new to version 1)
    {TYPE_BOOL, NULL, FALSE, 43},   // Stage1 Texture Clamp V (new to version 1)
    {TYPE_INT, NULL, FALSE, 44},    // Stage1 Texture Hint (new to version 1)
    {TYPE_BOOL, NULL, FALSE, 30},   // Stage1 Texture Display (in viewport...)
    ...
    {TYPE_BOOL, NULL, FALSE,
     56},  // Stage0 no texture reduction (new to version 5)
    {TYPE_BOOL, NULL, FALSE,
     57},  // Stage0 no texture reduction (new to version 5)
};
...
const int DISPLACEMENT_INDEX =
    (W3dMaterialClass::MAX_PASSES * W3dMaterialClass::MAX_STAGES);
 ...
 * HISTORY: * 11/23/98   GTH : Created. *
 *=============================================================================================*/
SClass_ID GameMtl::SuperClassID() { return MATERIAL_CLASS_ID; }
 ...
 * HISTORY: * 11/23/98   GTH : Created. *
 *=============================================================================================*/
TSTR GameMtl::SubAnimName(int i) { return _T(""); }
 ...
 * HISTORY: * 11/23/98   GTH : Created. *
 *=============================================================================================*/
Animatable *GameMtl::SubAnim(int i) { return NULL; }
  ...
  for (int pass = 0; pass < W3dMaterialClass::MAX_PASSES; pass++) {
    ...
    for (int stage = 0; stage < W3dMaterialClass::MAX_STAGES; stage++) {
      if (Texture[pass][stage]) {
        mnew->ReplaceReference(texture_ref_index(pass, stage),
                               Texture[pass][stage]->Clone());
      } else {
 ...
 * HISTORY: * 11/23/98   GTH : Created. *
 *=============================================================================================*/
RefResult GameMtl::NotifyRefChanged(Interval changeInt, RefTargetHandle hTarget,
  ...
  switch (message) {
    ...
    case REFMSG_GET_PARAM_NAME: {
      ...
      for (int pass = 0; pass < Get_Pass_Count(); pass++) {
        if (hTarget == (RefTargetHandle)PassParameterBlock[pass])
          pass_parameter = true;
      }
      ...
      if (pass_parameter) {
        switch (gpn->index) {
          ...
          case PB_DIFFUSE:
            ...
            break;
          case PB_SPECULAR:
            gpn->name = _T("Specular");
            break;
          case PB_EMISSIVE:
            gpn->name = _T("Emissive");
            break;
          case PB_SHININESS:
            gpn->name = _T("Shininess");
            break;
          case PB_OPACITY:
            gpn->name = _T("Opacity");
            break;
          case PB_TRANSLUCENCY:
            gpn->name = _T("Translucency");
            break;
          default:
            gpn->name = _T("");
            break;
...
void GameMtl::SetReference(int i, RefTargetHandle rtarg) {
  ...
  if ((i >= REF_PASS_PARAMETERS) &&
      (i < REF_PASS_PARAMETERS + W3dMaterialClass::MAX_PASSES)) {
...
/***********************************************************************************************
 * GameMtl::Get_Displacement_Map_Index -- returns the Sub-texmap index for the
 *displacement map.
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void GameMtl::Reset() {
  ...
  ReplaceReference(REF_MAIN, CreateParameterBlock(MainParameterBlockDesc, 1,
                                                  CURRENT_VERSION));

  for (int pass = 0; pass < W3dMaterialClass::MAX_PASSES; pass++) {
    // Install a parameter block for each pass
    IParamBlock *pblock = CreateParameterBlock(
        PassParameterBlockDescVer5,
        sizeof(PassParameterBlockDescVer5) / sizeof(ParamBlockDescID),
        CURRENT_VERSION);
 ...
 * HISTORY: * 11/23/98   GTH : Created. *
 *=============================================================================================*/
Interval GameMtl::Validity(TimeValue t) { return FOREVER; }
...
IOResult GameMtl::Load(ILoad *iload) {
  ...
  for (int i = 0; i < W3dMaterialClass::MAX_PASSES; i++) {
    iload->RegisterPostLoadCallback(new ParamBlockPLCB(
        PassParameterBlockVersions, NUM_OLDVERSIONS,
 ...
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * This function could use a little work.  It doesn't always produce
 *correct results.          *
 *                                                                                             *
 * HISTORY: * 11/23/98   GTH : Created. *
 *=============================================================================================*/
  ...
  for (int pass = 0; pass < Get_Pass_Count(); pass++) {
    ...
    */
    ...
    for (int light_index = 0; light_index < sc.nLights; light_index++) {
      ...
      if (light->Illuminate(sc, sc.Normal(), light_color, light_dir, dot_nl,
                            diffuse_coef)) {
        // ambient += light_color;
        ...
        if (c > 0.f) {
          specular +=
              (float)pow(c, Get_Shininess(pass, sc.CurTime())) * light_color;
        }
      }
    }
    ...
    if (Get_Texture_Enable(pass, 1) && Texture[pass][1]) {
      ...
      switch (Get_Detail_Color_Func(pass)) {
        ...
        case W3DSHADER_DETAILCOLORFUNC_DETAILBLEND:
          texel.r = (detail_texel.a * texel.r) +
                    ((1.0f - detail_texel.a) * detail_texel.r);
          texel.g = (detail_texel.a * texel.g) +
                    ((1.0f - detail_texel.a) * detail_texel.g);
          texel.b = (detail_texel.a * texel.b) +
                    ((1.0f - detail_texel.a) * detail_texel.b);
          break;
      }
    ...
    */
    ...
    switch (Get_Pri_Gradient(pass)) {
      case W3DSHADER_PRIGRADIENT_DISABLE:
        break;
    ...
    switch (Get_Sec_Gradient(pass)) {
      case W3DSHADER_SECGRADIENT_DISABLE:
        break;
      case W3DSHADER_SECGRADIENT_ENABLE:
        src = src + sec_gradient;
        break;
    }
    ...
    switch (Get_Src_Blend(pass)) {
      case W3DSHADER_SRCBLENDFUNC_ZERO:
        src_blend = Color(0, 0, 0);
        break;
      case W3DSHADER_SRCBLENDFUNC_ONE:
        src_blend = Color(1, 1, 1);
        break;
      case W3DSHADER_SRCBLENDFUNC_SRC_ALPHA:
        src_blend = Color(alpha, alpha, alpha);
        break;
      case W3DSHADER_SRCBLENDFUNC_ONE_MINUS_SRC_ALPHA:
        src_blend = Color(1.0f - alpha, 1.0f - alpha, 1.0f - alpha);
        break;
    }
...
/***********************************************************************************************
 * GameMtl::Compute_PC_Shader_From_PS2_Shader -- Determine if a PC shader can be
 *created.      *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 * HISTORY: * 10/13/1999MLL: Created. *
 *=============================================================================================*/
int GameMtl::Compute_PC_Shader_From_PS2_Shader(int pass) {
  int param_value[4];

  // These match the PC equation paramaters.
  static const int src_blend[4] = {ST_ZERO, ST_ONE, ST_SRC_ALPHA,
                                   1 - ST_SRC_ALPHA};
  static const int dest_blend[6] = {ST_ZERO,    ST_ONE,       ST_SRC,
                                    1 - ST_SRC, ST_SRC_ALPHA, 1 - ST_SRC_ALPHA};
  ...
  equation_value =
      ((param_value[0] - param_value[1]) * param_value[3]) + param_value[2];

  for (i = 0; i < 4; i++) {
    for (j = 0; j < 6; j++) {
      // Calculate the PC shader.  If equal, we have found a conversion.
      if (((src_blend[i] * ST_SRC) + (dest_blend[j] * ST_DEST)) ==
          equation_value) {
        break;
      }
    }
...
/***********************************************************************************************
 * GameMtl::Compute_PS2_Shader_From_PC_Shader -- Change a W3D material to a PS2
W3D material.  * W3DSHADER_PRIGRADIENT_ * *
 * *
 * *
 * *
 * HISTORY: *
    ...
    for (j = 0; j < 3; j++) {
      for (k = 0; k < 3; k++) {
        for (l = 0; l < 3; l++) {
          ...
          if (equation_value ==
              ((a_blend[i] - b_blend[j]) * c_blend[k]) + d_blend[l]) {
            goto finished;
          }
        }
      }
...
int GameMtl::Get_Copy_Specular_To_Diffuse(int pass) {
  int val;
  PassParameterBlock[pass]->GetValue(PB_COPY_SPECULAR_TO_DIFFUSE, 0, val,
                                     FOREVER);
  return val;
}
...
int GameMtl::Get_Texture_Enable(int pass, int stage) {
  int val;
  if (stage == 0) {
    PassParameterBlock[pass]->GetValue(PB_STAGE0_TEXTURE_ENABLE, 0, val,
                                       FOREVER);
  } else {
    PassParameterBlock[pass]->GetValue(PB_STAGE1_TEXTURE_ENABLE, 0, val,
                                       FOREVER);
  }
  return val;
}
int GameMtl::Get_Texture_Publish(int pass, int stage) {
  int val;
  if (stage == 0) {
    PassParameterBlock[pass]->GetValue(PB_STAGE0_TEXTURE_PUBLISH, 0, val,
                                       FOREVER);
  } else {
    PassParameterBlock[pass]->GetValue(PB_STAGE1_TEXTURE_PUBLISH, 0, val,
                                       FOREVER);
  }
  return val;
}
int GameMtl::Get_Texture_Resize(int pass, int stage) {
  int val;
  if (stage == 0) {
    PassParameterBlock[pass]->GetValue(PB_STAGE0_TEXTURE_RESIZE, 0, val,
                                       FOREVER);
  } else {
    PassParameterBlock[pass]->GetValue(PB_STAGE1_TEXTURE_RESIZE, 0, val,
                                       FOREVER);
  }
  return val;
}
int GameMtl::Get_Texture_No_Mipmap(int pass, int stage) {
  int val;
  if (stage == 0) {
    PassParameterBlock[pass]->GetValue(PB_STAGE0_TEXTURE_NO_MIPMAP, 0, val,
                                       FOREVER);
  } else {
    PassParameterBlock[pass]->GetValue(PB_STAGE1_TEXTURE_NO_MIPMAP, 0, val,
                                       FOREVER);
  }
  return val;
}
int GameMtl::Get_Texture_Clamp_U(int pass, int stage) {
  int val;
  if (stage == 0) {
    PassParameterBlock[pass]->GetValue(PB_STAGE0_TEXTURE_CLAMP_U, 0, val,
                                       FOREVER);
  } else {
    PassParameterBlock[pass]->GetValue(PB_STAGE1_TEXTURE_CLAMP_U, 0, val,
                                       FOREVER);
  }
  return val;
}
int GameMtl::Get_Texture_Clamp_V(int pass, int stage) {
  int val;
  if (stage == 0) {
    PassParameterBlock[pass]->GetValue(PB_STAGE0_TEXTURE_CLAMP_V, 0, val,
                                       FOREVER);
  } else {
    PassParameterBlock[pass]->GetValue(PB_STAGE1_TEXTURE_CLAMP_V, 0, val,
                                       FOREVER);
  }
  return val;
}
int GameMtl::Get_Texture_No_LOD(int pass, int stage) {
  int val;
  if (stage == 0) {
    PassParameterBlock[pass]->GetValue(PB_STAGE0_TEXTURE_NO_LOD, 0, val,
                                       FOREVER);
  } else {
    PassParameterBlock[pass]->GetValue(PB_STAGE1_TEXTURE_NO_LOD, 0, val,
                                       FOREVER);
  }
  return val;
}
int GameMtl::Get_Texture_Alpha_Bitmap(int pass, int stage) {
  int val;
  if (stage == 0) {
    PassParameterBlock[pass]->GetValue(PB_STAGE0_TEXTURE_ALPHA_BITMAP, 0, val,
                                       FOREVER);
  } else {
    PassParameterBlock[pass]->GetValue(PB_STAGE1_TEXTURE_ALPHA_BITMAP, 0, val,
                                       FOREVER);
  }
  return val;
}
...
int GameMtl::Get_Texture_Display(int pass, int stage) {
  int val;
  if (stage == 0) {
    PassParameterBlock[pass]->GetValue(PB_STAGE0_TEXTURE_DISPLAY, 0, val,
                                       FOREVER);
  } else {
    PassParameterBlock[pass]->GetValue(PB_STAGE1_TEXTURE_DISPLAY, 0, val,
                                       FOREVER);
  }
  return val;
}
float GameMtl::Get_Texture_Frame_Rate(int pass, int stage) {
  float val;
  if (stage == 0) {
    PassParameterBlock[pass]->GetValue(PB_STAGE0_TEXTURE_FRAME_RATE, 0, val,
                                       FOREVER);
  } else {
    PassParameterBlock[pass]->GetValue(PB_STAGE1_TEXTURE_FRAME_RATE, 0, val,
                                       FOREVER);
  }
  return val;
}
int GameMtl::Get_Texture_Frame_Count(int pass, int stage) {
  int val;
  if (stage == 0) {
    PassParameterBlock[pass]->GetValue(PB_STAGE0_TEXTURE_FRAME_COUNT, 0, val,
                                       FOREVER);
  } else {
    PassParameterBlock[pass]->GetValue(PB_STAGE1_TEXTURE_FRAME_COUNT, 0, val,
                                       FOREVER);
  }
  return val;
}
int GameMtl::Get_Texture_Anim_Type(int pass, int stage) {
  int val;
  if (stage == 0) {
    PassParameterBlock[pass]->GetValue(PB_STAGE0_TEXTURE_ANIM_TYPE, 0, val,
                                       FOREVER);
  } else {
    PassParameterBlock[pass]->GetValue(PB_STAGE1_TEXTURE_ANIM_TYPE, 0, val,
                                       FOREVER);
  }
  return val;
}
  ...
  if (IsMultiMtl()) {
    ...
    for (unsigned mi = 0; mi < NumSubMtls(); mi++) {
      ...
      if (GetSubMtl(mi)->ClassID() == GameMaterialClassID) {
        ...
        for (pass = 0; pass < ((GameMtl *)(GetSubMtl(mi)))->Get_Pass_Count();
             pass++) {
...
void GameMtl::texmap_index_to_pass_stage(int index, int *set_pass,
                                         int *set_stage) {
...
Interval GameMtl::DisplacementValidity(TimeValue t) { return FOREVER; }
...
void GameMtlPostLoad::proc(ILoad *iload) {
  ...
  // older material formats did not save the map channel and will default to
  // zero, we need to change the map channel to one in this case.
  for (int pass = 0; pass < W3dMaterialClass::MAX_PASSES; pass++) {
    for (int stage = 0; stage < W3dMaterialClass::MAX_STAGES; stage++) {
      if ((m->Get_Map_Channel(pass, stage) < 1) ||
          (m->Get_Map_Channel(pass, stage) > 99)) {
        m->Set_Map_Channel(pass, stage, 1);
      }
    }
  }

  // Now that I've removed the UI for the RESIZE and NO_MIPMAP options in the
  // texture pane, we initialize the No_LOD setting to the existing NO_MIPMAP
  // setting NOTE: I created a new flag for gamemtl which gets set when this
  // conversion takes place for the first time.
