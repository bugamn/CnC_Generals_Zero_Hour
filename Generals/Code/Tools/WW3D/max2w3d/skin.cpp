...
** Static functions
*/
static BOOL CALLBACK _sot_dialog_proc(HWND hWnd, UINT message, WPARAM wParam,
                                      LPARAM lParam);
static BOOL CALLBACK _skeleton_dialog_thunk(HWND hWnd, UINT message,
                                            WPARAM wParam, LPARAM lParam);
static BOOL CALLBACK _bone_influence_dialog_thunk(HWND hWnd, UINT message,
                                                  WPARAM wParam, LPARAM lParam);
static TriObject *Get_Tri_Object(TimeValue t, ObjectState &os, Interval &valid,
                                 BOOL &needsdel);
...
void SkinWSMObjectClass::BeginEditParams(IObjParam *ip, ULONG flags,
                                         Animatable *prev) {
  ...
  */
  if (SotHWND == NULL) {
    SotHWND = ip->AddRollupPage(AppInstance, MAKEINTRESOURCE(IDD_SKIN_SOT),
                                _sot_dialog_proc, Get_String(IDS_SOT),
                                (LPARAM)InterfacePtr, APPENDROLL_CLOSED);
  } else {
  ...
  */
  if (SkeletonHWND == NULL) {
    SkeletonHWND = InterfacePtr->AddRollupPage(
        AppInstance, MAKEINTRESOURCE(IDD_SKELETON_PARAMETERS),
        _skeleton_dialog_thunk, Get_String(IDS_SKELETON_PARAMETERS),
        (LPARAM)this, 0);
  } else {
...
void SkinWSMObjectClass::EndEditParams(IObjParam *ip, ULONG flags,
                                       Animatable *next) {
...
void SkinWSMObjectClass::BuildMesh(TimeValue t) {
  ...
  */
  ...
  for (i = 0; i < NumBoneIconVerts; i++) {
    mesh.setVert(
        i, Point3(BoneIconVerts[i].X, BoneIconVerts[i].Y, BoneIconVerts[i].Z));
  }

  for (i = 0; i < NumBoneIconFaces; i++) {
    Build_Tri(&(mesh.faces[i]), BoneIconFaces[i].V0, BoneIconFaces[i].V1,
              BoneIconFaces[i].V2);
  }
...
SkinModifierClass::SkinModifierClass(void) { Default_Init(); }
...
RefTargetHandle SkinModifierClass::Clone(RemapDir &remap) {
  SkinModifierClass *newmod =
      new SkinModifierClass(WSMNodeRef, (SkinWSMObjectClass *)WSMObjectRef);
  return newmod;
}

void SkinModifierClass::BeginEditParams(IObjParam *ip, ULONG flags,
                                        Animatable *prev) {
  static int i = 0;
...
void SkinModifierClass::EndEditParams(IObjParam *ip, ULONG flags,
                                      Animatable *next) {
  /*
  ** just checking...
  */
...
void SkinModifierClass::SetReference(int i, RefTargetHandle rtarg) {
  switch (i) {
    case OBJ_REF:
      WSMObjectRef = (SkinWSMObjectClass *)rtarg;
      break;
    case NODE_REF:
      WSMNodeRef = (INode *)rtarg;
      break;
  }
}
...
void SkinModifierClass::ModifyObject(TimeValue t, ModContext &mc,
...
void SkinModifierClass::ActivateSubobjSel(int level, XFormModes &modes) {
  ...
  */
  NotifyDependents(FOREVER,
                   SELECT_CHANNEL | DISP_ATTRIB_CHANNEL | SUBSEL_TYPE_CHANNEL,
                   REFMSG_CHANGE);
}

int SkinModifierClass::HitTest(TimeValue t, INode *inode, int type,
                               int crossing, int flags, IPoint2 *p,
                               ViewExp *vpt, ModContext *mc) {
  Interval valid = FOREVER;
  int needsdel;
  int savedLimits;
  int res = 0;
  ...
  */
  ...
  TriObject *tobj =
      Get_Tri_Object(InterfacePtr->GetTime(), os, valid, needsdel);
    ...
    switch (SubObjSelLevel) {
      ...
      case VERTEX_SEL_LEVEL:
        ...
        for (int j = 0; j < tobj->mesh.vertSel.GetSize(); j++) {
          if (tobj->mesh.vertSel[j])
            tobj->mesh.vertSel.Clear(j);
          else
            tobj->mesh.vertSel.Set(j);
        }
  ...
  for (int boneidx = 0; boneidx < skinobj->Num_Bones(); boneidx++) {
    if (skinobj->Get_Bone(boneidx) != NULL) {
      ...
      for (int vertidx = 0; vertidx < skindata->VertData.Count(); vertidx++) {
        if (skindata->VertData[vertidx].BoneIdx[0] == boneidx)
          boneverts.Set(vertidx);
        else
          boneverts.Clear(vertidx);
      }
...
void SkinModifierClass::Install_Bone_Influence_Dialog(void) {
  ...
  */
  ...
  if (!loaded) {
    LoadString(AppInstance, IDS_BONE_INFLUENCE_PARAMS, string,
               MAX_STRING_LENGTH);
    loaded = 1;
  }
  ...
  */
  BoneInfluenceHWND = InterfacePtr->AddRollupPage(
      AppInstance, MAKEINTRESOURCE(IDD_BONE_INFLUENCE_PARAMS),
      _bone_influence_dialog_thunk, string, (LPARAM)this, 0);
}
...
static BOOL CALLBACK _skeleton_dialog_thunk(HWND hWnd, UINT message,
                                            WPARAM wParam, LPARAM lParam) {
  SkinWSMObjectClass *skinobj =
      (SkinWSMObjectClass *)GetWindowLong(hWnd, GWL_USERDATA);
...
BOOL SkinWSMObjectClass::Skeleton_Dialog_Proc(HWND hWnd, UINT message,
                                              WPARAM wParam, LPARAM lParam) {
  switch (message) {
    case WM_INITDIALOG:
      ...
      */
      ...
      RemoveBonesButton =
          GetICustButton(GetDlgItem(hWnd, IDC_REMOVE_BONES_BUTTON));
      ...
      */
      ...
      BasePoseSpin->LinkToEdit(GetDlgItem(hWnd, IDC_BASE_POSE_EDIT),
                               EDITTYPE_INT);
      return TRUE;
...
static BOOL CALLBACK _bone_influence_dialog_thunk(HWND hWnd, UINT message,
                                                  WPARAM wParam,
                                                  LPARAM lParam) {
  SkinModifierClass *skinmod =
      (SkinModifierClass *)GetWindowLong(hWnd, GWL_USERDATA);
...
BOOL SkinModifierClass::Bone_Influence_Dialog_Proc(HWND hWnd, UINT message,
                                                   WPARAM wParam,
                                                   LPARAM lParam) {
  switch (message) {
    case WM_INITDIALOG:
      ...
      */
      ...
      LinkByNameButton =
          GetICustButton(GetDlgItem(hWnd, IDC_LINK_BY_NAME_BUTTON));
      ...
      LinkButton->SetTooltip(
          TRUE, _T("Link Vertices to a bone by selecting the bone"));
