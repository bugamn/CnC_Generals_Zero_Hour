...
** Static functions
*/
static BOOL CALLBACK _options_dialog_proc(HWND Hwnd, UINT message,
...
/***********************************************************************************************
 * W3dOptionsDialogClass::W3dOptionsDialogClass -- constructor for the options
 *dialog object   *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
bool W3dOptionsDialogClass::Get_Export_Options(
  ...
  /*BOOL result = DialogBoxParam
                                          (
                                                  AppInstance,
                                                  MAKEINTRESOURCE
     (IDD_W3D_EXPORT_OPTIONS), MaxInterface->GetMAXHWnd(), (DLGPROC)
     _options_dialog_proc, (LPARAM) this
                                          );*/

  PresetExportOptionsDialogClass dialog(MaxInterface,
...
/***********************************************************************************************
 * W3dOptionsDialogClass::Dialog_Proc -- Handles the windows message for the
 *options dialog    *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 07/24/1997 GH  : Created. *
 *=============================================================================================*/
bool W3dOptionsDialogClass::Dialog_Proc(HWND hwnd, UINT message, WPARAM wParam,
                                        LPARAM) {
...
void W3dOptionsDialogClass::Dialog_Init() {
  ...
  if ((Options->ReduceAnimationPercent < 1) ||
      (Options->ReduceAnimationPercent > 99)) {
    Options->ReduceAnimationPercent = 50;
  }

  if ((Options->CompressAnimationFlavor < 0) ||
      (Options->CompressAnimationFlavor >= ANIM_FLAVOR_VALID)) {
  ...
  RangeLowSpin = SetupIntSpinner(Hwnd, IDC_RANGE_LOW_SPIN, IDC_RANGE_LOW_EDIT,
                                 startframe, endframe, Options->StartFrame);

  RangeHighSpin =
      SetupIntSpinner(Hwnd, IDC_RANGE_HIGH_SPIN, IDC_RANGE_HIGH_EDIT,
                      startframe, endframe, Options->EndFrame);

  // initialize the 'DisableExportAABTrees' option
  CheckDlgButton(Hwnd, IDC_EXPORT_MESH_AABTREES,
                 !Options->DisableExportAABTrees);

  CheckDlgButton(Hwnd, IDC_EXPORT_MESH_MAT_TO_TEXTURE,
                 Options->EnableMaterialColorToTextureConversion);

#if ENABLE_MESH_OPTIMIZING
  CheckDlgButton(Hwnd, IDC_EXPORT_MESH_OPTIMIZE,
...
BOOL W3dOptionsDialogClass::Dialog_Ok() {
  ...
  // export options:
  bool export_h =
      (IsDlgButtonChecked(Hwnd, IDC_WHT_EXPORT_RADIO) == BST_CHECKED);
  ...
  bool export_a =
      (IsDlgButtonChecked(Hwnd, IDC_WHA_EXPORT_RADIO) == BST_CHECKED);
  changed = changed || (Options->ExportAnimation != export_a);
  Options->ExportAnimation = export_a;

  bool export_g =
      (IsDlgButtonChecked(Hwnd, IDC_WTM_EXPORT_RADIO) == BST_CHECKED);
  changed = changed || (Options->ExportGeometry != export_g);
  Options->ExportGeometry = export_g;

  if (export_g) {
    bool smooth_meshes =
        (IsDlgButtonChecked(Hwnd, IDC_EXPORT_MESH_SMOOTH_CHECK) == BST_CHECKED);
    changed = changed || (Options->SmoothBetweenMeshes != smooth_meshes);
    Options->SmoothBetweenMeshes = smooth_meshes;

    bool disable_export_aabs =
        (IsDlgButtonChecked(Hwnd, IDC_EXPORT_MESH_AABTREES) != BST_CHECKED);
    changed =
        changed || (Options->DisableExportAABTrees != disable_export_aabs);
    Options->DisableExportAABTrees = disable_export_aabs;

    bool convert_materials =
        (IsDlgButtonChecked(Hwnd, IDC_EXPORT_MESH_MAT_TO_TEXTURE) ==
         BST_CHECKED);
    changed = changed || (Options->EnableMaterialColorToTextureConversion !=
                          convert_materials);
    Options->EnableMaterialColorToTextureConversion = convert_materials;

#if ENABLE_MESH_OPTIMIZING
    bool optimize_mesh_data =
        (IsDlgButtonChecked(Hwnd, IDC_EXPORT_MESH_OPTIMIZE) == BST_CHECKED);
    changed =
        changed || (Options->EnableOptimizeMeshData != optimize_mesh_data);
  ...
  // Hierarchy Options:
  bool xlation_only =
      (IsDlgButtonChecked(Hwnd, IDC_TRANSLATION_ONLY_CHECK) == BST_CHECKED);
  changed = changed || (Options->TranslationOnly != xlation_only);
  Options->TranslationOnly = xlation_only;

  bool terrain_mode =
      (IsDlgButtonChecked(Hwnd, IDC_TERRAIN_MODE_CHECK) == BST_CHECKED);
  changed = changed || (Options->EnableTerrainMode != terrain_mode);
  Options->EnableTerrainMode = terrain_mode;

  if (Options->LoadHierarchy &&
      (Options->ExportAnimation || Options->ExportGeometry)) {
    if (!GotHierarchyFilename) {
      MessageBox(Hwnd, "You have not supplied a Base Pose hierarchy file!",
                 "Error", MB_OK);
  ...
  bool compress_animation =
      (IsDlgButtonChecked(Hwnd, IDC_COMPRESS_ANIMATION_CHECK) == BST_CHECKED);
  changed = changed || (Options->CompressAnimation != compress_animation);
  Options->CompressAnimation = compress_animation;

  bool reduce_animation =
      (IsDlgButtonChecked(Hwnd, IDC_REDUCE_ANIMATION_CHECK) == BST_CHECKED);
  ...
  changed =
      changed || (Options->CompressAnimationTranslationError != trans_error);
  ...
  bool review_log =
      (IsDlgButtonChecked(Hwnd, IDC_VIEWLOG_CHECK) == BST_CHECKED);
  changed = changed || (Options->ReviewLog != review_log);
  Options->ReviewLog = review_log;

  // Geometry options:
  // Options->UseVoxelizer = (IsDlgButtonChecked(Hwnd,IDC_VOXELIZER_CHECK) ==
  // BST_CHECKED);
...
/***********************************************************************************************
 * _options_dialog_proc -- thunks into the Options dialog class's windows
 *message handler      *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 07/24/1997 GH  : Created. *
 *=============================================================================================*/
BOOL CALLBACK _options_dialog_proc(HWND hwnd, UINT message, WPARAM wParam,
                                   LPARAM lParam) {
 ...
 *=============================================================================================*/
void _init_ofn(void) {
  static char _szhierarchyfilter[] =
