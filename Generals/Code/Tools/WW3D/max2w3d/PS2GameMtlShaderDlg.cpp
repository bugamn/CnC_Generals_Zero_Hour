 ...
 **
 *---------------------------------------------------------------------------------------------*
 * Functions: * PS2GameMtlShaderDlg -- Constructor. * PGMSD::Dialog_Proc --
 *Respond to user selections.                                         *
 *   PGMSD::ReloadDialog -- Setup the dialog box. * PGMSD::Apply_Preset --
 *Notify the material of the values for the selected setting.        *
 *   PGMSD::Set_Preset -- Sets the dialog to one of the presets or custom. *
 *   PGMSD::CompareShaderToBlendPreset -- Determine if the settings conform to
 *one of the prese*
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
static char *_PS2ShaderBlendSettingPresetNames[NUM_PS2_SHADER_BLEND_PRESETS +
                                               1] = {"Opaque",
                                                     "Additive",
                                                     "Source Subtracted",
...
//	(A - B) * C + D
static const PS2ShaderBlendSettingPreset
    PS2ShaderBlendSettingPresets[NUM_PS2_SHADER_BLEND_PRESETS] = {
        ...
        {PSS_SRC, PSS_DEST, PSS_ONE, PSS_ZERO, false,
         false},  // Dest subtracted
        {PSS_SRC, PSS_DEST, PSS_SRC_ALPHA, PSS_DEST, false,
         false},                                             // Alpha blend
        {PSS_SRC, PSS_ZERO, PSS_ONE, PSS_ZERO, true, true},  // Alpha test
        {PSS_SRC, PSS_DEST, PSS_SRC_ALPHA, PSS_DEST, true,
         true},  // Alpha test & blend
};
 ...
 *=============================================================================================*/
PS2GameMtlShaderDlg::PS2GameMtlShaderDlg(HWND parent, IMtlParams *imp,
                                         GameMtl *mtl, int pass)
 ...
 * HISTORY: * 10/12/1999MLL: Created. *
 *=============================================================================================*/
BOOL PS2GameMtlShaderDlg::Dialog_Proc(HWND dlg_wnd, UINT message, WPARAM wparam,
                                      LPARAM lparam) {
  int cursel;
  int i;
  ...
  switch (message) {
    case WM_INITDIALOG:
      for (i = 0; i <= NUM_PS2_SHADER_BLEND_PRESETS; i++) {
        SendDlgItemMessage(dlg_wnd, IDC_PS2_PRESET_COMBO, CB_ADDSTRING, 0,
                           (LONG)_PS2ShaderBlendSettingPresetNames[i]);
      }
      ...
      if (code == CBN_SELCHANGE) {
        switch (id) {
          ...
          case IDC_PS2_PRESET_COMBO:
            cursel = SendDlgItemMessage(dlg_wnd, IDC_PS2_PRESET_COMBO,
                                        CB_GETCURSEL, 0, 0);
            Apply_Preset(cursel);
            break;
          case IDC_A_COMBO:
            cursel =
                SendDlgItemMessage(dlg_wnd, IDC_A_COMBO, CB_GETCURSEL, 0, 0);
          ...
          case IDC_B_COMBO:
            cursel =
                SendDlgItemMessage(dlg_wnd, IDC_B_COMBO, CB_GETCURSEL, 0, 0);
          ...
          case IDC_C_COMBO:
            cursel =
                SendDlgItemMessage(dlg_wnd, IDC_C_COMBO, CB_GETCURSEL, 0, 0);
          ...
          case IDC_D_COMBO:
            cursel =
                SendDlgItemMessage(dlg_wnd, IDC_D_COMBO, CB_GETCURSEL, 0, 0);
...
void PS2GameMtlShaderDlg::ReloadDialog(void) {
  ...
  SendDlgItemMessage(m_hWnd, IDC_A_COMBO, CB_SETCURSEL,
                     TheMtl->Get_PS2_Shader_Param_A(PassIndex), 0);
  SendDlgItemMessage(m_hWnd, IDC_B_COMBO, CB_SETCURSEL,
                     TheMtl->Get_PS2_Shader_Param_B(PassIndex), 0);
  SendDlgItemMessage(m_hWnd, IDC_C_COMBO, CB_SETCURSEL,
                     TheMtl->Get_PS2_Shader_Param_C(PassIndex), 0);
  SendDlgItemMessage(m_hWnd, IDC_D_COMBO, CB_SETCURSEL,
                     TheMtl->Get_PS2_Shader_Param_D(PassIndex), 0);
...
/***********************************************************************************************
 * PGMSD::Apply_Preset -- Notify the material of the values for the selected
 *setting.          *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
...
void PS2GameMtlShaderDlg::Apply_Preset(int preset_index) {
  ...
  const PS2ShaderBlendSettingPreset &preset =
      PS2ShaderBlendSettingPresets[preset_index];
...
/***********************************************************************************************
 * PGMSD::CompareShaderToBlendPreset -- Determine if the settings conform to one
 *of the presets*
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 ...
 * HISTORY: * 10/12/1999MLL: Created. *
 *=============================================================================================*/
bool PS2GameMtlShaderDlg::CompareShaderToBlendPreset(
    const PS2ShaderBlendSettingPreset &blend_preset) {
  bool depthmask =
