...
static BOOL CALLBACK _settings_form_dlg_proc(HWND hWnd, UINT msg, WPARAM wParam,
                                             LPARAM lParam);
static BOOL CALLBACK _w3d_utility_tools_dlg_proc(HWND hWnd, UINT msg,
...
/**********************************************************************************************
**
** MaterialReferenceMaker - Class to support Export Utilities in the W3D Tools
*panel.
**
**********************************************************************************************/
...
int MaterialReferenceMaker::NumRefs() { return (ReferenceCount); }
...
RefResult MaterialReferenceMaker::NotifyRefChanged(Interval changeInt,
...
/**********************************************************************************************
**
** SettingsFormClass - code for the W3DUTILITY_SETTINGS_DIALOG.  Used in the
*command panel
...
/**********************************************************************************************
**
** W3DUtilityClass - Utility plugin which presents windows controls for setting
*all of
...
class W3DUtilityClass : public UtilityObj {
  ...
  */
  ...
  static void enable_two_sided_in_all_selected(INodeListClass *list,
                                               bool onoff);
  static void enable_znormals_in_all_selected(INodeListClass *list, bool onoff);
  static void enable_vertex_alpha_in_all_selected(INodeListClass *list,
                                                  bool onoff);
  static void enable_shadow_in_all_selected(INodeListClass *list, bool onoff);
  static void enable_shatterable_in_all_selected(INodeListClass *list,
                                                 bool onoff);
  static void enable_npatches_in_all_selected(INodeListClass *list, bool onoff);
  static void enable_physical_collision_in_all_selected(INodeListClass *list,
                                                        bool onoff);
  static void enable_projectile_collision_in_all_selected(INodeListClass *list,
                                                          bool onoff);
  static void enable_vis_collision_in_all_selected(INodeListClass *list,
                                                   bool onoff);
  static void enable_camera_collision_in_all_selected(INodeListClass *list,
                                                      bool onoff);
  static void enable_vehicle_collision_in_all_selected(INodeListClass *list,
                                                       bool onoff);

  static void set_geometry_type_in_all_selected(INodeListClass *list,
                                                int geotype);
  static void set_dazzle_type_in_all_selected(INodeListClass *list,
                                              char *dazzletype);
  ...
  */
  ...
  int convert_materials(MaterialConversionEnum conversion,
...
ClassDesc *Get_W3D_Utility_Desc(void) { return &W3DUtilityDesc; }
...
void W3DUtilityClass::BeginEditParams(Interface *ip, IUtil *iu) {
  ...
  SettingsPanelHWND = InterfacePtr->AddRollupPage(
      AppInstance, MAKEINTRESOURCE(IDD_W3DUTILITY_SETTINGS_DIALOG),
      _settings_form_dlg_proc, Get_String(IDS_W3DUTILITY_SETTINGS), 0);

  ToolsPanelHWND = InterfacePtr->AddRollupPage(
      AppInstance, MAKEINTRESOURCE(IDD_W3DUTILITY_TOOLS_DIALOG),
      _w3d_utility_tools_dlg_proc, Get_String(IDS_W3DUTILITY_TOOLS), 0);
...
void W3DUtilityClass::SelectionSetChanged(Interface *ip, IUtil *iu) {
  // (gth) the settings panels which need to respond to the selection set
  // changing are now registered directly with MAX and don't need to be updated
  // here
  //	update_dialog();
}
...
void W3DUtilityClass::eval_node_states(INodeListClass *list,
                                       NodeStatesStruct *ns) {
  ...
  */
  for (unsigned int ni = 0; ni < list->Num_Nodes(); ni++) {
    ...
    ns->CollisionProjectile +=
        (wdata->Is_Projectile_Collision_Enabled() ? 1 : 0);
    ...
    switch (wdata->Get_Geometry_Type()) {
      case W3DAppData2Struct::GEO_TYPE_CAMERA_ALIGNED:
        ns->GeometryCameraAligned = true;
        break;
      case W3DAppData2Struct::GEO_TYPE_CAMERA_ORIENTED:
        ns->GeometryCameraOriented = true;
        break;
      case W3DAppData2Struct::GEO_TYPE_NORMAL_MESH:
        ns->GeometryNormal = true;
        break;
      case W3DAppData2Struct::GEO_TYPE_AABOX:
        ns->GeometryAABox = true;
        break;
      case W3DAppData2Struct::GEO_TYPE_OBBOX:
        ns->GeometryOBBox = true;
        break;
      case W3DAppData2Struct::GEO_TYPE_NULL:
        ns->GeometryNull = true;
        break;
      case W3DAppData2Struct::GEO_TYPE_DAZZLE:
        ns->GeometryDazzle = true;
        ns->DazzleCount++;
        break;
      case W3DAppData2Struct::GEO_TYPE_AGGREGATE:
        ns->GeometryAggregate = true;
        break;
    }
...
void W3DUtilityClass::set_hierarchy_in_all_selected(INodeListClass *node_list,
                                                    bool onoff) {
...
void W3DUtilityClass::set_geometry_in_all_selected(INodeListClass *node_list,
                                                   bool onoff) {
  for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
    W3DAppData2Struct *wdata = get_app_data_2((*node_list)[ni]);
...
void W3DUtilityClass::enable_hidden_in_all_selected(INodeListClass *node_list,
                                                    bool onoff) {
  for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
    W3DAppData2Struct *wdata = get_app_data_2((*node_list)[ni]);
...
void W3DUtilityClass::enable_two_sided_in_all_selected(
    INodeListClass *node_list, bool onoff) {
  for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
    W3DAppData2Struct *wdata = get_app_data_2((*node_list)[ni]);
...
void W3DUtilityClass::enable_znormals_in_all_selected(INodeListClass *node_list,
                                                      bool onoff) {
  for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
    W3DAppData2Struct *wdata = get_app_data_2((*node_list)[ni]);
...
void W3DUtilityClass::enable_vertex_alpha_in_all_selected(
    INodeListClass *node_list, bool onoff) {
  for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
    W3DAppData2Struct *wdata = get_app_data_2((*node_list)[ni]);
...
void W3DUtilityClass::enable_shadow_in_all_selected(INodeListClass *node_list,
                                                    bool onoff) {
  for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
    W3DAppData2Struct *wdata = get_app_data_2((*node_list)[ni]);
...
void W3DUtilityClass::enable_shatterable_in_all_selected(
    INodeListClass *node_list, bool onoff) {
  for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
    W3DAppData2Struct *wdata = get_app_data_2((*node_list)[ni]);
...
void W3DUtilityClass::enable_npatches_in_all_selected(INodeListClass *node_list,
                                                      bool onoff) {
  for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
    W3DAppData2Struct *wdata = get_app_data_2((*node_list)[ni]);
...
void W3DUtilityClass::enable_physical_collision_in_all_selected(
    INodeListClass *node_list, bool onoff) {
  for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
    W3DAppData2Struct *wdata = get_app_data_2((*node_list)[ni]);
...
void W3DUtilityClass::enable_projectile_collision_in_all_selected(
    INodeListClass *node_list, bool onoff) {
  for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
    W3DAppData2Struct *wdata = get_app_data_2((*node_list)[ni]);
...
void W3DUtilityClass::enable_vis_collision_in_all_selected(
    INodeListClass *node_list, bool onoff) {
  for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
    W3DAppData2Struct *wdata = get_app_data_2((*node_list)[ni]);
...
void W3DUtilityClass::enable_camera_collision_in_all_selected(
    INodeListClass *node_list, bool onoff) {
  for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
    W3DAppData2Struct *wdata = get_app_data_2((*node_list)[ni]);
...
void W3DUtilityClass::enable_vehicle_collision_in_all_selected(
    INodeListClass *node_list, bool onoff) {
  for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
    W3DAppData2Struct *wdata = get_app_data_2((*node_list)[ni]);
...
void W3DUtilityClass::set_geometry_type_in_all_selected(
    INodeListClass *node_list, int geotype) {
  for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
    W3DAppData2Struct *wdata = get_app_data_2((*node_list)[ni]);
...
void W3DUtilityClass::set_dazzle_type_in_all_selected(INodeListClass *node_list,
                                                      char *dazzle_type) {
  for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
    W3DDazzleAppDataStruct *dazzledata = get_dazzle_app_data((*node_list)[ni]);
    strncpy(dazzledata->DazzleType, dazzle_type,
            sizeof(dazzledata->DazzleType) - 1);
  }
  update_settings_controls(node_list);
}
...
void W3DUtilityClass::generate_lod_ext(INode *node) {
  ...
  */
  ...
  if ((ext != NULL) && (sscanf(ext, ".%d", &old_lod) == 1)) {
    ...
    ** one, replace it.
    */
    if (old_lod == LodExtensionOptions.LodIndex) return;  // same lod index

    /*
    ** Room for the new extension? (2 because when we export, the extension
    *will,
...
void W3DUtilityClass::create_floater(void) {
  SettingsFloater.Create(InterfacePtr, IDD_W3DUTILITY_SETTINGS_DIALOG,
...
void W3DUtilityClass::export_with_standard_materials() {
  ...
  MaterialReferenceMaker::ReferenceCount =
      convert_materials(GAME_REFERENCE_COUNT, NULL);

  MaterialReferenceMaker *gamenodematerials = NULL;

  if (MaterialReferenceMaker::ReferenceCount > 0) {
    gamenodematerials =
        new MaterialReferenceMaker[MaterialReferenceMaker::ReferenceCount];
...
int W3DUtilityClass::convert_materials(
    MaterialConversionEnum conversion,
    MaterialReferenceMaker *gamenodematerials) {
  ...
  if (rootnode != NULL) {
    ...
    if (meshlist != NULL) {
      for (unsigned nodeindex = 0; nodeindex < meshlist->Num_Nodes();
           nodeindex++) {
          ...
          if (!nodemtl->IsMultiMtl()) {
            switch (conversion) {
              ...
              case GAME_TO_STANDARD:
                ...
                if (nodemtl->ClassID() == GameMaterialClassID) {
                  // Make a reference to the game material to ensure that it is
                  // not deleted by the system.
                  gamenodematerials[gamenodematerialindex].MakeRefByID(
                      FOREVER, gamenodematerialindex, nodemtl);

                  // Does this material already have an equivalent standard
                  // material?
                  if (((GameMtl *)nodemtl)->Substitute_Material() == NULL) {
                    ((GameMtl *)nodemtl)
                        ->Set_Substitute_Material(
                            new_standard_material((GameMtl *)nodemtl));
                  }
                  ((*meshlist)[nodeindex])
                      ->SetMtl(((GameMtl *)nodemtl)->Substitute_Material());
              ...
              case STANDARD_TO_GAME:
                ...
                // Change materials to game materials if they were previously
                // game materials before being converted to standard materials.
                if (gamenodematerials[gamenodematerialindex].MaterialPtr !=
                    NULL) {
                  ((*meshlist)[nodeindex])
                      ->SetMtl(
                          gamenodematerials[gamenodematerialindex].MaterialPtr);
                  ((GameMtl *)gamenodematerials[gamenodematerialindex]
                       .MaterialPtr)
                      ->Set_Substitute_Material(NULL);
                }
                break;
            }
            ...
            for (int materialindex = 0; materialindex < nodemtl->NumSubMtls();
                 materialindex++) {
                ...
                switch (conversion) {
                  case GAME_REFERENCE_COUNT:
                    if (submaterial->ClassID() == GameMaterialClassID) {
                      assert(((GameMtl *)submaterial)->Substitute_Material() ==
                             NULL);
                    }
                    break;
                  ...
                  case GAME_TO_STANDARD:
                    ...
                    if (submaterial->ClassID() == GameMaterialClassID) {
                      // Make a reference to the game material to ensure that it
                      // is not deleted by the system.
                      gamenodematerials[gamenodematerialindex].MakeRefByID(
                          FOREVER, gamenodematerialindex, submaterial);

                      // Does this material already have an equivalent standard
                      // material?
                      if (((GameMtl *)submaterial)->Substitute_Material() ==
                          NULL) {
                        ((GameMtl *)submaterial)
                            ->Set_Substitute_Material(
                                new_standard_material((GameMtl *)submaterial));
                      }
                      nodemtl->SetSubMtl(
                          materialindex,
                  ...
                  case STANDARD_TO_GAME:
                    ...
                    // Change materials to game materials if they were
                    // previously game materials before being converted to
                    // standard materials.
                    if (gamenodematerials[gamenodematerialindex].MaterialPtr !=
                        NULL) {
                      nodemtl->SetSubMtl(
...
StdMat *W3DUtilityClass::new_standard_material(GameMtl *gamemtl) {
  ...
  // NOTE 0: Only consider pass 0 in the game material - ignore all other
  // passes. NOTE 1: Use defaults for all standard material attributes that
  // cannot be
...
bool W3DUtilityClass::is_alpha_mesh(INode *node, Mtl *nodemtl) {
  ...
  if (tri != NULL) {
    ...
    if (nodemtl == NULL) {
      ...
      // Initialize each sub-material flag to false (indicates that the material
      // is un-used)
...
W3DAppData0Struct *W3DUtilityClass::get_app_data_0(INode *node) {
  ...
  */
  ...
  AppDataChunk *appdata =
      node->GetAppDataChunk(W3DUtilityClassID, UTILITY_CLASS_ID, 0);
...
W3DAppData1Struct *W3DUtilityClass::get_app_data_1(INode *node) {
  ...
  AppDataChunk *appdata =
      node->GetAppDataChunk(W3DUtilityClassID, UTILITY_CLASS_ID, 1);
  ...
  if (appdata) {
    ...
    node->AddAppDataChunk(W3DUtilityClassID, UTILITY_CLASS_ID, 1,
                          sizeof(W3DAppData1Struct), wdata);
...
** NOTE: When you use the _settings_form_dlg_proc, a SettingsFormClass will
*automatically
** be allocated and attached to the dialog.  You can cause all of the active
*forms to
** refresh their status by calling Update_All_Instances.  The forms will be
*destroyed when
...
BOOL CALLBACK _settings_form_dlg_proc(HWND hWnd, UINT msg, WPARAM wParam,
                                      LPARAM lParam) {
  ...
  SettingsFormClass *form =
      (SettingsFormClass *)::GetProp(hWnd, "SettingsFormClass");
  if (form) {
...
static void _settings_form_selection_changed_callback(void *param,
                                                      NotifyInfo *info) {
  ((SettingsFormClass *)param)->Selection_Changed();
}

SettingsFormClass::SettingsFormClass(HWND hwnd) : Hwnd(hwnd), RegionSpin(NULL) {
  /*
  ...
  ** Register with MAX
  */
  ::RegisterNotification(_settings_form_selection_changed_callback, this,
...
SettingsFormClass::~SettingsFormClass(void) {
  ...
  ** Unregister from MAX
  */
  ::UnRegisterNotification(_settings_form_selection_changed_callback, this,
                           NOTIFY_SELECTIONSET_CHANGED);
...
void SettingsFormClass::Update_All_Instances(void) {
  ...
  */
  INodeListClass node_list(::GetCOREInterface()->GetRootNode(),
                           ::GetCOREInterface()->GetTime(), &_INodeFilter);
...
void SettingsFormClass::Init(void) {
  ...
  // Load the section of Dazzle.INI that defines all of the types.  The windows
  // function that I'm using here, reads in a NULL-terminated string for each
  // entry in the section.  Each string is of the form 'key=value'.  Based on my
  // testing, it appears that windows removes any white space before or after
  ...
  char *dazzle_types_buffer =
      new char[DAZZLE_SECTION_BUFFERSIZE];  // max size of a section for Win95

  ::GetPrivateProfileSection(DAZZLE_TYPES_SECTION, dazzle_types_buffer,
                             DAZZLE_SECTION_BUFFERSIZE, dllpath);

  // Now we need to handle each string in the section buffer; skipping the
  // 'key=' and adding the dazzle type name into the combo box.
  ...
  */
  RegionSpin = SetupIntSpinner(Hwnd, IDC_DAMREG_INDEX_SPIN,
                               IDC_DAMREG_INDEX_EDIT, NO_DAMAGE_REGION,
                               MAX_DAMAGE_REGIONS - 1, NO_DAMAGE_REGION);
}
...
bool SettingsFormClass::Dialog_Proc(HWND hWnd, UINT message, WPARAM wParam,
                                    LPARAM lParam) {
  int check;
  ...
  switch (message) {
    ...
    case WM_COMMAND: {
      ...
      */
      ...
      switch (LOWORD(wParam)) {
        /*
        ** Tri-State Checkboxes, make sure that the ID of all checkboxes is
        *present
        ** in the 'if' statement above!
        */
        case IDC_HIERARCHY_CHECK:
          W3DUtilityClass::set_hierarchy_in_all_selected(&node_list,
                                                         check == BST_CHECKED);
          break;

        case IDC_GEOMETRY_CHECK:
          W3DUtilityClass::set_geometry_in_all_selected(&node_list,
                                                        check == BST_CHECKED);
          break;

        case IDC_GEOMETRY_HIDE:
          W3DUtilityClass::enable_hidden_in_all_selected(&node_list,
                                                         check == BST_CHECKED);
          break;

        case IDC_GEOMETRY_TWO_SIDED:
          W3DUtilityClass::enable_two_sided_in_all_selected(
              &node_list, check == BST_CHECKED);
          break;

        case IDC_GEOMETRY_ZNORMALS:
          W3DUtilityClass::enable_znormals_in_all_selected(
              &node_list, check == BST_CHECKED);
          break;

        case IDC_GEOMETRY_VERTEX_ALPHA:
          W3DUtilityClass::enable_vertex_alpha_in_all_selected(
              &node_list, check == BST_CHECKED);
          break;

        case IDC_GEOMETRY_CAST_SHADOW:
          W3DUtilityClass::enable_shadow_in_all_selected(&node_list,
                                                         check == BST_CHECKED);
          break;

        case IDC_GEOMETRY_SHATTERABLE:
          W3DUtilityClass::enable_shatterable_in_all_selected(
              &node_list, check == BST_CHECKED);
          break;

        case IDC_GEOMETRY_NPATCH:
          W3DUtilityClass::enable_npatches_in_all_selected(
              &node_list, check == BST_CHECKED);
          break;

        case IDC_COLLISION_PHYSICAL:
          W3DUtilityClass::enable_physical_collision_in_all_selected(
              &node_list, check == BST_CHECKED);
          break;

        case IDC_COLLISION_PROJECTILE:
          W3DUtilityClass::enable_projectile_collision_in_all_selected(
              &node_list, check == BST_CHECKED);
          break;

        case IDC_COLLISION_VIS:
          W3DUtilityClass::enable_vis_collision_in_all_selected(
              &node_list, check == BST_CHECKED);
          break;

        case IDC_COLLISION_CAMERA:
          W3DUtilityClass::enable_camera_collision_in_all_selected(
              &node_list, check == BST_CHECKED);
          break;

        case IDC_COLLISION_VEHICLE:
          W3DUtilityClass::enable_vehicle_collision_in_all_selected(
              &node_list, check == BST_CHECKED);
          break;
        ...
        */
        case IDC_GEOMETRY_NORMAL:
          W3DUtilityClass::set_geometry_type_in_all_selected(
              &node_list, W3DAppData2Struct::GEO_TYPE_NORMAL_MESH);
          break;

        case IDC_GEOMETRY_CAMERA_ALIGNED:
          W3DUtilityClass::set_geometry_type_in_all_selected(
              &node_list, W3DAppData2Struct::GEO_TYPE_CAMERA_ALIGNED);
          break;

        case IDC_GEOMETRY_CAMERA_ORIENTED:
          W3DUtilityClass::set_geometry_type_in_all_selected(
              &node_list, W3DAppData2Struct::GEO_TYPE_CAMERA_ORIENTED);
          break;

        case IDC_GEOMETRY_NULL:
          W3DUtilityClass::set_geometry_type_in_all_selected(
              &node_list, W3DAppData2Struct::GEO_TYPE_NULL);
          break;

        case IDC_GEOMETRY_AABOX:
          W3DUtilityClass::set_geometry_type_in_all_selected(
              &node_list, W3DAppData2Struct::GEO_TYPE_AABOX);
          break;

        case IDC_GEOMETRY_OBBOX:
          W3DUtilityClass::set_geometry_type_in_all_selected(
              &node_list, W3DAppData2Struct::GEO_TYPE_OBBOX);
          break;

        case IDC_GEOMETRY_AGGREGATE:
          W3DUtilityClass::set_geometry_type_in_all_selected(
              &node_list, W3DAppData2Struct::GEO_TYPE_AGGREGATE);
          break;

        case IDC_GEOMETRY_DAZZLE:
          W3DUtilityClass::set_geometry_type_in_all_selected(
              &node_list, W3DAppData2Struct::GEO_TYPE_DAZZLE);
          break;

        /*
        ** Dazzle type setting.  Whenever the user changes the selected dazzle
        *type, apply
        ...
        case IDC_DAZZLE_COMBO:
          if (HIWORD(wParam) == CBN_SELCHANGE) {
            ...
            if (dazzle_combo != NULL) {
              ...
              if (len < 128) {
                ::SendMessage(dazzle_combo, CB_GETLBTEXT, (WPARAM)cursel,
                              (LPARAM)dazzle_type);
                W3DUtilityClass::set_dazzle_type_in_all_selected(&node_list,
                                                                 dazzle_type);
              }
            }
          }
    ...
    */
    case CC_SPINNER_CHANGE: {
      ...
      W3DUtilityClass::set_region_in_all_selected(&node_list,
                                                  RegionSpin->GetIVal());
      break;
    }
...
void SettingsFormClass::Update_Controls(INodeListClass *node_list) {
  ...
  */
  SendDlgItemMessage(Hwnd, IDC_HIERARCHY_CHECK, BM_SETCHECK, ns.ExportHierarchy,
                     0L);
  SendDlgItemMessage(Hwnd, IDC_GEOMETRY_CHECK, BM_SETCHECK, ns.ExportGeometry,
                     0L);
  SendDlgItemMessage(Hwnd, IDC_GEOMETRY_HIDE, BM_SETCHECK, ns.GeometryHidden,
                     0L);
  SendDlgItemMessage(Hwnd, IDC_GEOMETRY_TWO_SIDED, BM_SETCHECK,
                     ns.GeometryTwoSided, 0L);
  SendDlgItemMessage(Hwnd, IDC_GEOMETRY_ZNORMALS, BM_SETCHECK,
                     ns.GeometryZNormals, 0L);
  SendDlgItemMessage(Hwnd, IDC_GEOMETRY_VERTEX_ALPHA, BM_SETCHECK,
                     ns.GeometryVertexAlpha, 0L);
  SendDlgItemMessage(Hwnd, IDC_GEOMETRY_CAST_SHADOW, BM_SETCHECK,
                     ns.GeometryCastShadow, 0L);
  SendDlgItemMessage(Hwnd, IDC_GEOMETRY_SHATTERABLE, BM_SETCHECK,
                     ns.GeometryShatterable, 0L);
  SendDlgItemMessage(Hwnd, IDC_GEOMETRY_NPATCH, BM_SETCHECK, ns.GeometryNPatch,
                     0L);
  SendDlgItemMessage(Hwnd, IDC_COLLISION_PHYSICAL, BM_SETCHECK,
                     ns.CollisionPhysical, 0L);
  SendDlgItemMessage(Hwnd, IDC_COLLISION_PROJECTILE, BM_SETCHECK,
                     ns.CollisionProjectile, 0L);
  SendDlgItemMessage(Hwnd, IDC_COLLISION_VIS, BM_SETCHECK, ns.CollisionVis, 0L);
  SendDlgItemMessage(Hwnd, IDC_COLLISION_CAMERA, BM_SETCHECK,
                     ns.CollisionCamera, 0L);
  SendDlgItemMessage(Hwnd, IDC_COLLISION_VEHICLE, BM_SETCHECK,
                     ns.CollisionVehicle, 0L);
  ...
  */
  ...
  int selindex = ::SendMessage(dazzle_combo, CB_FINDSTRING, (WPARAM)0,
                               (LPARAM)ns.DazzleType);
  if (selindex != CB_ERR) {
  ...
  */
  CheckDlgButton(Hwnd, IDC_GEOMETRY_CAMERA_ALIGNED,
                 (ns.GeometryCameraAligned ? BST_CHECKED : BST_UNCHECKED));
  CheckDlgButton(Hwnd, IDC_GEOMETRY_CAMERA_ORIENTED,
                 (ns.GeometryCameraOriented ? BST_CHECKED : BST_UNCHECKED));
  CheckDlgButton(Hwnd, IDC_GEOMETRY_NORMAL,
                 (ns.GeometryNormal ? BST_CHECKED : BST_UNCHECKED));
  CheckDlgButton(Hwnd, IDC_GEOMETRY_NULL,
                 (ns.GeometryNull ? BST_CHECKED : BST_UNCHECKED));
  CheckDlgButton(Hwnd, IDC_GEOMETRY_AABOX,
                 (ns.GeometryAABox ? BST_CHECKED : BST_UNCHECKED));
  CheckDlgButton(Hwnd, IDC_GEOMETRY_OBBOX,
                 (ns.GeometryOBBox ? BST_CHECKED : BST_UNCHECKED));
  CheckDlgButton(Hwnd, IDC_GEOMETRY_AGGREGATE,
                 (ns.GeometryAggregate ? BST_CHECKED : BST_UNCHECKED));
  CheckDlgButton(Hwnd, IDC_GEOMETRY_DAZZLE,
                 (ns.GeometryDazzle ? BST_CHECKED : BST_UNCHECKED));
}
