...
/* $Header: /Commando/Code/Tools/max2w3d/vxllayer.cpp 4     10/28/97 6:08p
 * Greg_h $ */
/***********************************************************************************************
 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * VoxelLayerClass::VoxelLayerClass -- Constructor for
 *VoxelLayerClass                       * VoxelLayerClass::Intersect_Triangle --
 *Intersect a triangle with the slab                 *
 *   VoxelLayerClass::Draw_Line -- Draw a line of voxels into the slab *
 *   VoxelLayerClass::Scan_Triangle -- Clip and scan-convert a triangle into the
 *slab          * clip_tri_to_slab -- Clips a triangle against a voxel slab *
 *   clip_poly -- clip a polygon against a single 3D plane * output -- Emit a
 *vertex into a polygons vertex list                                       *
 ...
 *   clear_scan_table -- clears the static scanline table * fixup_scan_table --
 *ensure all spans are left->right in order                             *
...
** local functions
***************************************************************************************/
static void clip_tri_to_slab(Point3 p0, Point3 p1, Point3 p2, float z0,
                             float z1, vertexstruct *outverts, int *setnum);

static void clip_poly(vertexstruct *inverts, int innum, vertexstruct *outverts,
                      int *outnum, const PlaneClass &clipplane);

static void output(const vertexstruct &outvert, vertexstruct *poly,
                   int *numverts);

static int inside(const vertexstruct &p, const PlaneClass &plane);

static vertexstruct intersect(const vertexstruct &p0, const vertexstruct &p1,
...
static void scan_edge(const vertexstruct &p0, const vertexstruct &p1);
 ...
 *=============================================================================================*/
VoxelLayerClass::VoxelLayerClass(INodeListClass &object_list, TimeValue time,
                                 Matrix3 parenttm, Point3 offset, Point3 scale,
                                 float slicez, float sliceh, int bmwidth,
                                 int bmheight) {
  unsigned i;
 ...
 * HISTORY: * 06/10/1997 GH  : Created. *
 *=============================================================================================*/
void VoxelLayerClass::Draw_Line(double x0, double y0, double x1, double y1) {
 ...
 * HISTORY: * 06/10/1997 GH  : Created. *
 *=============================================================================================*/
void VoxelLayerClass::Scan_Triangle(Point3 p0, Point3 p1, Point3 p2) {
  int i;
  ...
  for (i = (int)floor(miny); i <= (int)floor(maxy); i++) {
    ...
    Draw_Line(_scantab[i].P[LEFT].Pos.x, _scantab[i].P[LEFT].Pos.y,
              _scantab[i].P[RIGHT].Pos.x, _scantab[i].P[RIGHT].Pos.y);
    //		}
  }
}
...
static void clip_tri_to_slab(Point3 p0, Point3 p1, Point3 p2, float z0,
  ...
  clip_poly(outverts, 3, tmpverts, setnum,
            PlaneClass(Vector3(0.0f, 0.0f, 1.0f), -z0));

  // clip from the tmp buffer to the out buffer against top of slab:
  clip_poly(tmpverts, *setnum, outverts, setnum,
            PlaneClass(Vector3(0.0f, 0.0f, -1.0f), z1));
}
 ...
 *=============================================================================================*/
static int inside(const vertexstruct &p, const PlaneClass &plane) {
  float dist = p.Pos.x * plane.N[0] + p.Pos.y * plane.N[1] +
               p.Pos.z * plane.N[2] + plane.D;
  if (dist >= 0.0f) {
    return 1;
  } else {
    return 0;
...
static vertexstruct intersect(const vertexstruct &p0, const vertexstruct &p1,
  ...
  float num = -(plane.N[0] * p0.Pos.x + plane.N[1] * p0.Pos.y +
                plane.N[2] * p0.Pos.z + plane.D);

  float den =
      plane.N[0] * delta.x + plane.N[1] * delta.y + plane.N[2] * delta.z;
...
static void scan_edge(const vertexstruct &p0, const vertexstruct &p1) {
  ...
  for (double y = floor(top->Pos.y); y <= floor(bot->Pos.y); y += 1.0f) {
    ...
    double t =
        (y - floor(top->Pos.y)) / (floor(bot->Pos.y) - floor(top->Pos.y));

    // position:
    _scantab[(int)y].P[side].Pos =
        (1.0f - (float)t) * top->Pos + (float)t * bot->Pos;

    // barycentric coords:
    _scantab[(int)y].P[side].Bary =
        (1.0f - (float)t) * top->Bary + (float)t * bot->Bary;
  }
}
