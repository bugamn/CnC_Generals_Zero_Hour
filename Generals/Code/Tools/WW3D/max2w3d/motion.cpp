 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * MotionClass::MotionClass -- constructor *
 *   MotionClass::MotionClass -- constructor * MotionClass::init -- initialize *
 *   MotionClass::~MotionClass -- destructor * MotionClass::compute_frame_motion
 *-- compute the motion for a specified frame             *
 *   MotionClass::set_motion_matrix -- save a motin matrix *
 *   MotionClass::get_motion_matrix -- retrieve a motion matrix *
 *   MotionClass::set_eulers -- store euler angles * MotionClass::get_eulers --
 *retrieve euler angles                                          *
 *   MotionClass::Save -- save the motion to a W3D file *
 *   MotionClass::save_header -- save the header * MotionClass::save_channels --
 *save the motion channels                                    *
 *   MotionClass::set_visibility -- store a visibility bit *
 *   MotionClass::get_visibility -- retrieve the visibility bit for this
 *node:frame            *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 *=============================================================================================*/
MotionClass::MotionClass(IScene *scene, INode *rootnode,
                         HierarchySaveClass *basepose,
                         W3dExportOptionsStruct &options, int framerate,
                         Progress_Meter_Class *meter, HWND MaxHwnd, char *name,
                         Matrix3 &offset)
    : BasePose(basepose),
      Scene(scene),
      RootNode(rootnode),
 ...
 * HISTORY: * 10/26/1997 GH  : Created. *
 *=============================================================================================*/
MotionClass::MotionClass(IScene *scene, INodeListClass *rootlist,
                         HierarchySaveClass *basepose,
                         W3dExportOptionsStruct &options, int framerate,
                         Progress_Meter_Class *meter, HWND MaxHwnd, char *name,
                         Matrix3 &offset)
...
void MotionClass::compute_frame_motion(int frame) {
  ...
  */
  ...
  if (RootNode != NULL) {
    tree = new HierarchySaveClass(RootNode, frametime, *Meter, "NoName", false,
                                  BasePose);
  } else {
    tree = new HierarchySaveClass(RootList, frametime, *Meter, "NoName", false,
                                  BasePose, Offset);
  }
      ...
      */
      ...
      if ((node) && (vis)) {
        if (frame != 0) {
          ...
          // if data at frametime_prev == data at frametime_mid and != data at
          // frametime then we have a binary movement!
 ...
 * HISTORY: * 10/26/1997 GH  : Created. *
 *=============================================================================================*/
void MotionClass::set_motion_matrix(int node, int frame,
  ...
  */
  if (frame > 0) {
    ...
    */
    ...
    double mag1 =
        (x2 - px) * (x2 - px) + (y2 - py) * (y2 - py) + (z2 - pz) * (z2 - pz);
...
Point3 MotionClass::get_eulers(int node, int frame) {
  return Point3(EulerDelta[node][frame].x, EulerDelta[node][frame].y,
  ...
  for (int nodeidx = 0; nodeidx < BasePose->Num_Nodes(); nodeidx++) {
    ...
    */
    if (NodeValidFlags[nodeidx]) {
      ...
      VectorChannelClass xrchan(nodeidx, NumFrames, ANIM_CHANNEL_XR, 1,
                                identity);
      VectorChannelClass yrchan(nodeidx, NumFrames, ANIM_CHANNEL_YR, 1,
                                identity);
      VectorChannelClass zrchan(nodeidx, NumFrames, ANIM_CHANNEL_ZR, 1,
                                identity);
      VectorChannelClass qchan(nodeidx, NumFrames, ANIM_CHANNEL_Q, 4, identity);

      xchan.SetSaveOptions(CompressAnimation, CompressAnimationFlavor,
                           CompressAnimationTranslationError,
                           CompressAnimationRotationError, ReduceAnimation,
                           ReduceAnimationPercent);
      ychan.SetSaveOptions(CompressAnimation, CompressAnimationFlavor,
                           CompressAnimationTranslationError,
                           CompressAnimationRotationError, ReduceAnimation,
                           ReduceAnimationPercent);
      zchan.SetSaveOptions(CompressAnimation, CompressAnimationFlavor,
                           CompressAnimationTranslationError,
                           CompressAnimationRotationError, ReduceAnimation,
                           ReduceAnimationPercent);
      xrchan.SetSaveOptions(CompressAnimation, CompressAnimationFlavor,
                            CompressAnimationTranslationError,
                            CompressAnimationRotationError, ReduceAnimation,
                            ReduceAnimationPercent);
      yrchan.SetSaveOptions(CompressAnimation, CompressAnimationFlavor,
                            CompressAnimationTranslationError,
                            CompressAnimationRotationError, ReduceAnimation,
                            ReduceAnimationPercent);
      zrchan.SetSaveOptions(CompressAnimation, CompressAnimationFlavor,
                            CompressAnimationTranslationError,
                            CompressAnimationRotationError, ReduceAnimation,
                            ReduceAnimationPercent);
      qchan.SetSaveOptions(CompressAnimation, CompressAnimationFlavor,
                           CompressAnimationTranslationError,
                           CompressAnimationRotationError, ReduceAnimation,
                           ReduceAnimationPercent);
