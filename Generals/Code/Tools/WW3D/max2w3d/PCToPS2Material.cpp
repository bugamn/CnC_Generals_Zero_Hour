 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * PTPMC::BeginEditParams -- Change all W3D materials in the mesh
 *to be PS2 compatible.      *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
/***********************************************************************************************
 * PTPMC::BeginEditParams -- Change all W3D materials in the mesh to be PS2
 *compatible.        *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 * HISTORY: * 10/27/1999MLL: Created. *
 *=============================================================================================*/
void PCToPS2MaterialClass::BeginEditParams(Interface *ip, IUtil *iu) {
  // This function is called when the utility is chosen.
  // Since we don't need any window gadgets, we'll just go through all the
      ...
      for (i = 0; i < meshlist->Num_Nodes(); i++) {
        ...
        if (!nodemtl->IsMultiMtl()) {
          ...
          if (nodemtl->ClassID() == GameMaterialClassID) {
            ...
            for (pass = 0; pass < ((GameMtl *)nodemtl)->Get_Pass_Count();
                 pass++) {
          ...
          for (unsigned mi = 0; mi < nodemtl->NumSubMtls(); mi++) {
            ...
            if (nodemtl->GetSubMtl(mi)->ClassID() == GameMaterialClassID) {
              ...
              for (pass = 0;
                   pass < ((GameMtl *)nodemtl->GetSubMtl(mi))->Get_Pass_Count();
                   pass++) {
                // Change the material for each pass.
                ((GameMtl *)nodemtl->GetSubMtl(mi))
                    ->Compute_PS2_Shader_From_PC_Shader(pass);
              }
            } else if (nodemtl->GetSubMtl(mi)->ClassID() ==
...
class PCToPS2MaterialClassDesc : public ClassDesc {
 public:
  ...
  void *Create(BOOL loading) { return ((void *)new PCToPS2MaterialClass); }
