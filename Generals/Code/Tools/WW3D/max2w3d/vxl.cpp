...
/*
        ...
        This module will voxelize one or more meshes.  It is only used to
   compute some things like Moment of Inertia and Center of Mass for the object.
        Much of the code which was doing the shading and lighting computations
        has been stripped out.

*/

static void compute_dimensions(INodeListClass &meshlist,
                               const Matrix3 &parenttm, TimeValue curtime,
 ...
 *
 ************************************************************************/
VoxelClass::VoxelClass(INodeListClass &meshlist, int resolution,
                       Matrix3 parenttm, TimeValue time,
  ...
  ** Voxelize the meshes!
  */
  Quantize_Meshes(meshlist, meter);
}
 ...
 *
 ************************************************************************/
void VoxelClass::Quantize_Meshes(INodeListClass &meshlist,
                                 Progress_Meter_Class &meter) {
  /*
  ...
  */
  ...
  for (int slicecount = 0; slicecount < ZDim; slicecount++) {
    float slicez =
        min_z + (max_z - min_z) * ((float)slicecount / (float)(ZDim - 1));

    VoxelLayerClass *vlayer = new VoxelLayerClass(
        meshlist, CurTime, ParentTM, Offset, Scale, slicez, sliceh, XDim, YDim);
 ...
 *
 ************************************************************************/
void VoxelClass::Set_Layer(VoxelLayerClass &vlayer, uint32 z) {
 ...
 *
 ************************************************************************/
void VoxelClass::Compute_Bounding_Box(Point3 size, Point3 offset) {
  int i;
 ...
 *
 ************************************************************************/
void VoxelClass::Compute_Visiblity(Progress_Meter_Class &meter) {
  int x, y, z;
 ...
 *
 ************************************************************************/
uint8 VoxelClass::raw_read_vis(int i, int j, int k) {
  if (i < 0) return 0;
  if (j < 0) return 0;
  if (k < 0) return 0;
 ...
 *
 ************************************************************************/
void VoxelClass::raw_set_vis(int i, int j, int k, uint8 val) {
  if (i < 0) return;
  if (j < 0) return;
  if (k < 0) return;
...
void compute_dimensions(INodeListClass &meshlist, const Matrix3 &parenttm,
                        TimeValue curtime, Point3 *set_min, Point3 *set_max) {
...
void VoxelClass::Compute_Physical_Properties(double Volume[1], double CM[3],
                                             double I[9]) {
  int i, j, k;
...
Point3 VoxelClass::Voxel_Position(int i, int j, int k) {
  ...
  return Point3(Offset.x + i * BlockXDim + BlockXDim / 2.0,
