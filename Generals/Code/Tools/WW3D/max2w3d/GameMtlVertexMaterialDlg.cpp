 ...
 *                                                                                             *
 *                     $Archive::
 * /Commando/Code/Tools/max2w3d/GameMtlVertexMaterialDlg.cpp   $*
 *                                                                                             *
 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * GameMtlVertexMaterialDlg::GameMtlVertexMaterialDlg --
 *constructor                         *
 *   GameMtlVertexMaterialDlg::~GameMtlVertexMaterialDlg -- destructor *
 *   GameMtlVertexMaterialDlg::Dialog_Proc -- windows message handler *
 *   GameMtlVertexMaterialDlg::ReloadDialog -- reload the contents of the
 *controls             * GameMtlVertexMaterialDlg::ActivateDlg -- activate /
 ...
 *=============================================================================================*/
GameMtlVertexMaterialDlg::GameMtlVertexMaterialDlg(HWND parent, IMtlParams *imp,
 ...
 * HISTORY: * 11/23/98   GTH : Created. *
 *=============================================================================================*/
BOOL GameMtlVertexMaterialDlg::Dialog_Proc(HWND dlg_wnd, UINT message,
  ...
  switch (message) {
    case WM_INITDIALOG: {
      AmbientSwatch =
          GetIColorSwatch(GetDlgItem(dlg_wnd, IDC_AMBIENT_COLOR),
                          TheMtl->Get_Ambient(PassIndex, IParams->GetTime()),
                          Get_String(IDS_AMBIENT_COLOR));
      DiffuseSwatch =
          GetIColorSwatch(GetDlgItem(dlg_wnd, IDC_DIFFUSE_COLOR),
                          TheMtl->Get_Diffuse(PassIndex, IParams->GetTime()),
                          Get_String(IDS_DIFFUSE_COLOR));
      SpecularSwatch =
          GetIColorSwatch(GetDlgItem(dlg_wnd, IDC_SPECULAR_COLOR),
                          TheMtl->Get_Specular(PassIndex, IParams->GetTime()),
                          Get_String(IDS_SPECULAR_COLOR));
      EmissiveSwatch =
          GetIColorSwatch(GetDlgItem(dlg_wnd, IDC_EMISSIVE_COLOR),
                          TheMtl->Get_Emissive(PassIndex, IParams->GetTime()),
                          Get_String(IDS_EMISSIVE_COLOR));

      OpacitySpin = SetupFloatSpinner(
          dlg_wnd, IDC_OPACITY_SPIN, IDC_OPACITY_EDIT, 0.0f, 1.0f,
          TheMtl->Get_Opacity(PassIndex, IParams->GetTime()), 0.01f);
      TranslucencySpin = SetupFloatSpinner(
          dlg_wnd, IDC_TRANSLUCENCY_SPIN, IDC_TRANSULCENCY_EDIT, 0.0f, 1.0f,
          TheMtl->Get_Translucency(PassIndex, IParams->GetTime()), 0.01f);
      ShininessSpin = SetupFloatSpinner(
          dlg_wnd, IDC_SHININESS_SPIN, IDC_SHININESS_EDIT, 1.0f, 1000.0f,
          TheMtl->Get_Shininess(PassIndex, IParams->GetTime()), 1.0f);
      UVChannelSpin[0] = SetupIntSpinner(dlg_wnd, IDC_STAGE0UVCHAN_SPIN,
                                         IDC_STAGE0UVCHAN_EDIT, 1, 99, 1);
      UVChannelSpin[1] = SetupIntSpinner(dlg_wnd, IDC_STAGE1UVCHAN_SPIN,
                                         IDC_STAGE1UVCHAN_EDIT, 1, 99, 1);
      break;
    }
    ...
    case WM_COMMAND: {
      switch (id) {
        case IDC_COPY_SPECULAR_DIFFUSE:
          TheMtl->Set_Copy_Specular_To_Diffuse(
              PassIndex,
              GetCheckBox(dlg_wnd, IDC_COPY_SPECULAR_DIFFUSE) == TRUE);
          break;

        case IDC_MAPPING0_COMBO:
          if (code == CBN_SELCHANGE) {
            val = SendDlgItemMessage(dlg_wnd, IDC_MAPPING0_COMBO, CB_GETCURSEL,
                                     0, 0);
        ...
        case IDC_MAPPING1_COMBO:
          if (code == CBN_SELCHANGE) {
            val = SendDlgItemMessage(dlg_wnd, IDC_MAPPING1_COMBO, CB_GETCURSEL,
                                     0, 0);
          ...
          switch (code) {
            ...
            case EN_CHANGE:
              int len = GetWindowTextLength(
                  GetDlgItem(dlg_wnd, IDC_MAPPING0_ARGS_EDIT));
              char *buffer = TheMtl->Get_Mapping_Arg_Buffer(PassIndex, 0, len);
              GetWindowText(GetDlgItem(dlg_wnd, IDC_MAPPING0_ARGS_EDIT), buffer,
                            len + 1);
              break;
          }
          break;
        ...
        case IDC_MAPPING1_ARGS_EDIT:
          switch (code) {
            ...
            case EN_CHANGE:
              int len = GetWindowTextLength(
                  GetDlgItem(dlg_wnd, IDC_MAPPING1_ARGS_EDIT));
              char *buffer = TheMtl->Get_Mapping_Arg_Buffer(PassIndex, 1, len);
              GetWindowText(GetDlgItem(dlg_wnd, IDC_MAPPING1_ARGS_EDIT), buffer,
                            len + 1);
              break;
          }
          break;
        ...
        case IDC_NO_RT_LIGHTING:
          TheMtl->Set_PSX_Lighting(PassIndex,
                                   !GetCheckBox(dlg_wnd, IDC_NO_RT_LIGHTING));
          break;
      }
      break;
    }
    ...
    case CC_COLOR_CHANGE: {
      ...
      TheMtl->Set_Ambient(PassIndex, IParams->GetTime(),
                          AmbientSwatch->GetColor());
      TheMtl->Set_Diffuse(PassIndex, IParams->GetTime(),
                          DiffuseSwatch->GetColor());
      TheMtl->Set_Specular(PassIndex, IParams->GetTime(),
                           SpecularSwatch->GetColor());
      TheMtl->Set_Emissive(PassIndex, IParams->GetTime(),
                           EmissiveSwatch->GetColor());
      TheMtl->Notify_Changed();
      break;
    }

    case CC_SPINNER_CHANGE: {
      TheMtl->Set_Shininess(PassIndex, IParams->GetTime(),
                            ShininessSpin->GetFVal());
      TheMtl->Set_Opacity(PassIndex, IParams->GetTime(),
                          OpacitySpin->GetFVal());
      TheMtl->Set_Translucency(PassIndex, IParams->GetTime(),
                               TranslucencySpin->GetFVal());
      for (int i = 0; i < MAX_STAGES; i++) {
...
void GameMtlVertexMaterialDlg::ReloadDialog(void) {
  ...
  SpecularSwatch->InitColor(
      TheMtl->Get_Specular(PassIndex, IParams->GetTime()));
  EmissiveSwatch->InitColor(
      TheMtl->Get_Emissive(PassIndex, IParams->GetTime()));

  ShininessSpin->SetValue(TheMtl->Get_Shininess(PassIndex, IParams->GetTime()),
                          FALSE);
  OpacitySpin->SetValue(TheMtl->Get_Opacity(PassIndex, IParams->GetTime()),
                        FALSE);
  TranslucencySpin->SetValue(
      TheMtl->Get_Translucency(PassIndex, IParams->GetTime()), FALSE);
  for (int i = 0; i < MAX_STAGES; i++) {
    UVChannelSpin[i]->SetValue(TheMtl->Get_Map_Channel(PassIndex, i), FALSE);
  }

  SetCheckBox(m_hWnd, IDC_COPY_SPECULAR_DIFFUSE,
              TheMtl->Get_Copy_Specular_To_Diffuse(PassIndex));

  SendDlgItemMessage(m_hWnd, IDC_MAPPING0_COMBO, CB_SETCURSEL,
                     TheMtl->Get_Mapping_Type(PassIndex, 0), 0);

  SendDlgItemMessage(m_hWnd, IDC_MAPPING1_COMBO, CB_SETCURSEL,
                     TheMtl->Get_Mapping_Type(PassIndex, 1), 0);
