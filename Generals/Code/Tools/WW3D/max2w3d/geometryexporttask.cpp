 ...
 *                                                                                             *
 *                     $Archive::
 * /Commando/Code/Tools/max2w3d/geometryexporttask.cpp         $*
 *                                                                                             *
 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * GeometryExportTaskClass::GeometryExportTaskClass -- Constructor
 ...
 *   GeometryExportTaskClass::Get_Full_Name -- Composes the full name of this
 *robj             * GeometryExportTaskClass::Create_Task -- virtual constructor
 *for export tasks              * GeometryExportTaskClass::Optimize_Geometry --
 *Optimizes the export tasks                  *
 *   GeometryExportTaskClass::Generate_Unique_Name -- create a unique name for
 *this object     * MeshGeometryExportTaskClass::Is_Single_Material -- Tests if
 *this mesh uses a single mater *
 *   MeshGeometryExportTaskClass::Get_Single_Material -- returns pointer to the
 *material       * MeshGeometryExportTaskClass::Cache_Single_Material -- updates
 *the cached material pointer * MeshGeometryExportTaskClass::Split -- Splits
 *into single material meshes                  *
 *   MeshGeometryExportTaskClass::Reduce_To_Single_Material -- deletes polys *
 *   MeshGeometryExportTaskClass::Can_Combine -- can this mesh combine with
 *anything           * MeshGeometryExportTaskClass::Can_Combine_With -- can this
 *mesh be combined with the given * MeshGeometryExportTaskClass::Combine_Mesh --
 *Add the given mesh into this mesh            *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
const int OPTIMIZATION_FACECOUNT_LIMIT =
    256;  // TODO: what should this number be...
const float OPTIMIZATION_COMBINING_DISTANCE =
    20.0f;  // TODO: need a smarter method for combining...
...
class MeshGeometryExportTaskClass : public GeometryExportTaskClass {
 public:
  MeshGeometryExportTaskClass(INode *node, GeometryExportContextClass &context)
    ...
    */
    ...
    /*
    ** Store a pointer to the material if this mesh uses only one material (even
    *inside a Multi-Sub)
    */
    Update_Cached_Data();
  }
  ...
  virtual void Export_Geometry(GeometryExportContextClass &context) {
    ...
    */
    ...
    MeshSaveClass *mesh =
        new MeshSaveClass(Name, ContainerName, Node, &MeshData, ExportSpace,
                          ExportOptions, context.HTree, context.CurTime,
                          *context.ProgressMeter, context.materialColors,
                          context.numMaterialColors, context.numHouseColors,
                          context.materialColorTexture, &context.WorldInfo);
  ...
  /*
  ** Naming.  During the optimization phase, sometimes new meshes are created
  *and require
  ** new unique names.  These meshes are flagged and then new names are
  *generated prior
  ** to exporting.
  */
 ...
 protected:
  ...
  W3DAppData2Struct ExportOptions;  // Copy of the export options in case we
                                    // want to change them during optimization
  bool NameDirty;

  // Cached Data about the Node/Mesh.  Updated by calling Update_Cached_Data
  // whenever the mesh changes.
  Mtl *SingleMtl;  // Pointer to the single material (if the mesh uses only one,
                   // even in a multi-mtl)
...
class CollisionBoxGeometryExportTaskClass : public GeometryExportTaskClass {
 public:
  CollisionBoxGeometryExportTaskClass(INode *node,
  ...
  virtual void Export_Geometry(GeometryExportContextClass &context) {
    ...
    */
    CollisionBoxSaveClass *colbox =
        new CollisionBoxSaveClass(Name, ContainerName, Node, ExportSpace,
                                  context.CurTime, *context.ProgressMeter);
...
class DazzleGeometryExportTaskClass : public GeometryExportTaskClass {
 public:
  DazzleGeometryExportTaskClass(INode *node,
                                GeometryExportContextClass &context)
      : GeometryExportTaskClass(node, context) {}
  ...
  virtual void Export_Geometry(GeometryExportContextClass &context) {
    ...
    */
    DazzleSaveClass *dazzle =
        new DazzleSaveClass(Name, ContainerName, Node, ExportSpace,
                            context.CurTime, *context.ProgressMeter);
    /*
    ** Export it.
    */
...
** Export task for INodes which are to generate W3D NULL objects.  Note that
*this
** does not do anything in the Export_Geometry call, these only create entries
*in
...
class AggregateGeometryExportTaskClass : public GeometryExportTaskClass {
 public:
  AggregateGeometryExportTaskClass(INode *node,
                                   GeometryExportContextClass &context)
      : GeometryExportTaskClass(node, context) {
    memset(ContainerName, 0, sizeof(ContainerName));
  }
  ...
  virtual bool Is_Aggregate(void) { return true; }
...
GeometryExportTaskClass::GeometryExportTaskClass(
    INode *node, GeometryExportContextClass &context)
    : BoneIndex(0), ExportSpace(1), CurTime(context.CurTime), Node(node) {
  /*
  ** Set up the names
  */
  ...
  */
  if (context.HTree != NULL) {
    if (!Is_Skin(node)) {
      context.HTree->Get_Export_Coordinate_System(Node, &BoneIndex, NULL,
                                                  &ExportSpace);
    } else {
      BoneIndex = 0;
 ...
 *=============================================================================================*/
GeometryExportTaskClass::GeometryExportTaskClass(
    const GeometryExportTaskClass &that)
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/24/2000 gth : Created. *
 *=============================================================================================*/
 ...
 * HISTORY: * 10/20/2000 gth : Created. *
 *=============================================================================================*/
GeometryExportTaskClass *GeometryExportTaskClass::Create_Task(
    INode *node, GeometryExportContextClass &context) {
  if (!::Is_Geometry(node)) {
    return NULL;
  }

  // NOTE: we *have* to check Is_Proxy first because it is tied to a naming
  // convention rather than an explicit UI setting like the rest of the types.
  if (::Is_Proxy(*node)) {
    return new ProxyExportTaskClass(node, context);
  }

  if (::Is_Normal_Mesh(node) || Is_Camera_Aligned_Mesh(node) ||
      Is_Camera_Oriented_Mesh(node) || Is_Skin(node)) {
 ...
 *                                                                                             *
 *    This function will attempt to split meshes so that they use only a single
 *material and   * then try to combine small meshes that use the same material.
 *Export tasks may be        * removed and new ones added. *
 *                                                                                             *
 * INPUT: * tasks - dynamic vector of export task pointers.  Some tasks may be
 *deleted, some added      *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
...
void GeometryExportTaskClass::Optimize_Geometry(
  ...
  */
  ...
  while (meshes.Count() > 0) {
    ...
    /*
    ** If this mesh already uses only one material, just transfer it to the
    *simple_meshes array.
    ** Otherwise, have it split into new tasks, add them to the simple_meshes
    *array, and delete this task.
    */
 ...
 *                                                                                             *
 * INPUT: * index - will be used to create the name, don't re-use the same index
 *within an LOD          * context - geometry export context *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/23/2000 gth : Created. *
 *=============================================================================================*/
void GeometryExportTaskClass::Generate_Name(
    char *root, int index, GeometryExportContextClass &context) {
  /*
  ** Check if the original had a Renegade "building prefix" in its name.
  *Building meshes
  ** are named with a 2-3 letter user specified prefix followed by either '^' or
  *'#'.  We
  ...
  */
  ...
  /*
  ** Append the LOD index so the generated name doesn't collide with meshes in
  *the same
...
/***********************************************************************************************
 * MeshGeometryExportTaskClass::Update_Cached_Data -- updates the cached
 *material pointer, etc *
 *                                                                                             *
...
void MeshGeometryExportTaskClass::Update_Cached_Data(void) {
  ...
  /*
  ** Set the SingleMtl pointer if this mesh uses only one material (again, even
  *if its in a Multi-Sub)
  */
  if (nodemtl == NULL) {
    SingleMtl = NULL;
...
/***********************************************************************************************
 * MeshGeometryExportTaskClass::Is_Single_Material -- Tests if this mesh uses a
 *single materia *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 *                                                                                             *
 * This function will create new export tasks and add them to the supplied
 *array.  Each of     * these will be single-material meshes. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 ...
 * HISTORY: * 10/20/2000 gth : Created. *
 *=============================================================================================*/
void MeshGeometryExportTaskClass::Split(
    DynamicVectorClass<MeshGeometryExportTaskClass *> &simple_meshes) {
  ...
  /*
  ** Initialize each sub-material flag to false (indicates that the material is
  *un-used)
  */
  for (mat_index = 0; mat_index < sub_mtl_count; mat_index++) {
    sub_mtl_flags[mat_index] = false;
  ...
  */
  ...
  /*
  ** Loop over the used materials, creating a new MeshGeometryExportTaskClass
  *for each one
  */
  for (mat_index = 0; mat_index < sub_mtl_count; mat_index++) {
    if (sub_mtl_flags[mat_index]) {
      MeshGeometryExportTaskClass *new_task =
          new MeshGeometryExportTaskClass(*this);
 ...
 * MeshGeometryExportTaskClass::Reduce_To_Single_Material -- deletes polys *
 *                                                                                             *
 *    This function deletes all polys (and subsequent un-used vertices) except
 *for the ones    * that use the specified material. *
 *                                                                                             *
 * INPUT: * mat_id - only faces using this material id will remain in the mesh.
 **
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 ...
 *anything             *
 *                                                                                             *
 *    This will return false if the mesh has multiple materials, too many polys,
 *etc.          *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
bool MeshGeometryExportTaskClass::Can_Combine(void) {
  ...
  ** - (Renegade) VIS is enabled (don't want to create really wierd shaped
  *vis-sectors...)
  */
...
/***********************************************************************************************
 * MeshGeometryExportTaskClass::Can_Combine_With -- can this mesh be combined
 *with the given m *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 10/24/2000 gth : Created. *
 *=============================================================================================*/
bool MeshGeometryExportTaskClass::Can_Combine_With(
    MeshGeometryExportTaskClass *other_mesh) {
  /*
  ...
  */
  if (MeshData.numFaces + other_mesh->MeshData.numFaces >
...
/***********************************************************************************************
 * MeshGeometryExportTaskClass::Combine_Mesh -- Add the given mesh into this
 *mesh              *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 10/24/2000 gth : Created. *
 *=============================================================================================*/
void MeshGeometryExportTaskClass::Combine_Mesh(
    MeshGeometryExportTaskClass *other_mesh) {
  /*
  ...
  */
  Mesh new_mesh;
  ::CombineMeshes(new_mesh, MeshData, other_mesh->MeshData, &our_tm, &his_tm,
                  0);
  MeshData = new_mesh;
...
Point3 MeshGeometryExportTaskClass::Get_Shared_Vertex_Normal(
    const Point3 &world_pos, int smgroup) {
