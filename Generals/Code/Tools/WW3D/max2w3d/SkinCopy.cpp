 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * find_skin_binding -- Find the "WWSkin Binding" modifier on this
 *object.                   * find_skin_wsm -- Finds the node for the WWSkin WSM
 *used by this object.                   * get_skin_wsm_obj -- Gets the
 *SkinWSMObjectClass from a WWSkin WSM node.                   * duplicate_wsm
 *-- Duplicates a WWSkin WSM                                                  *
 *   find_equivalent_node -- Searches a hierarchy for an object equivalent to
 *the given one.   *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
INode *find_equivalent_node(INode *source, INode *tree,
...
**
** MAXScript Function:
** wwCopySkinInfo - Usage: wwCopySkinInfo from_node to_node wsm_node
*to_tree_root
**
...
Value *copy_skin_info_cf(Value **arg_list, int count) {
  ...
  if (arg_list[2] == &undefined) {
    ...
    if (wsm_node == NULL) return &undefined;
  } else
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/19/1999 AJA : Created. *
 *=============================================================================================*/
SkinModifierClass *find_skin_binding(INode *skinned_obj) {
  ...
  if (dobj == NULL) return NULL;  // not bound to a space warp
  ...
  for (int i = 0; i < dobj->NumModifiers(); i++) {
    ...
    if (mod->ClassID() != SKIN_MOD_CLASS_ID) continue;
...
SkinWSMObjectClass *get_skin_wsm_obj(INode *wsm_node) {
  ...
  if (obj->ClassID() != SKIN_OBJ_CLASS_ID) return NULL;
 ...
 *                                                                                             *
 * INPUT: wsm_node - INode of the WWSkin WSM object. * tree - The root of a tree
 *containing equivalents of the bones in the WWSkin used     * by skinned_obj.
 *The bone names must be the same (after being processed by     * Set_W3D_Name()
 **
 *                                                                                             *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/18/1999 AJA : Created. * 11/3/1999  AJA : Changed first
 *argument from skinned_obj to wsm_node.                     *
 *=============================================================================================*/
INode *duplicate_wsm(INode *wsm_node, INode *tree) {
  SkinWSMObjectClass *wsm_obj = get_skin_wsm_obj(wsm_node);

  if (!wsm_node || !wsm_obj) return NULL;

  /*
  ** Duplicate the WSM.
  */

  SkinWSMObjectClass *new_wsm_obj = (SkinWSMObjectClass *)CreateInstance(
      WSM_OBJECT_CLASS_ID, SKIN_OBJ_CLASS_ID);
  if (!new_wsm_obj) return NULL;
  ...
  for (int i = 0; i < wsm_obj->Num_Bones(); i++) {
    ...
    if (!src_bone || !dst_bone) return NULL;
...
/***********************************************************************************************
 * find_equivalent_node -- Searches a hierarchy for an object equivalent to the
 *given one.     *
 *                                                                                             *
 * INPUT: source - The node to search for an equivalent of. * tree - The
 *hierarchy to search in.                                                   *
 *                                                                                             *
...
INode *find_equivalent_node(INode *source, INode *tree, bool name_is_valid) {
  ...
  // The name of the source object. We'll only evaluate this once as an easy
  // optimization.
  static char src_name[W3D_NAME_LEN];
  if (!name_is_valid) Set_W3D_Name(src_name, source->GetName());
  ...
  // Is this the node we're looking for?
  if (strcmp(src_name, chk_name) == 0) return tree;  // Yup, sure is.
  ...
  for (int i = 0; i < tree->NumberOfChildren(); i++) {
    ...
    if (retval != NULL) return retval;  // we found the node in our children
  }
...
Value *copy_skin_info(INode *source, INode *target, INode *wsm) {
  ...
  if (source_modifier == NULL) return &undefined;
  ...
  SkinModifierClass *new_modifier =
      new SkinModifierClass(wsm, get_skin_wsm_obj(wsm));
  if (new_modifier == NULL)
    throw RuntimeError(
        "Out of memory - Unable to allocate a new SkinModifierClass object!");
  ...
  ModContext *new_context = new ModContext(
      source_context->tm, source_context->box, source_context->localData);
  if (new_context == NULL)
    throw RuntimeError(
        "Out of memory - Unable to allocate a new ModContext object!");
...
IDerivedObject *setup_wsm_derived_obj(INode *node) {
  ...
  if (dobj != NULL) {
    ...
    if (dobj == NULL) {
      char msg[128];
      sprintf(msg, "Error setting up the WSMDerivedObject for %s",
              node->GetName());
