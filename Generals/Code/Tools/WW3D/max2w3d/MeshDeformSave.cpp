...
/* $Header: /Commando/Code/Tools/max2w3d/MeshDeformSave.cpp 6     11/12/99
 * 11:12a Greg_h $ */
/***********************************************************************************************
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformSaveClass::Initialize(MeshBuilderClass &builder, Object *object,
                                     Mesh &mesh, Matrix3 *transform) {
  // Start fresh
  Reset();
  ...
  if ((object != NULL) && (object->SuperClassID() == GEN_DERIVOB_CLASS_ID)) {
    //
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformSaveClass::Initialize(MeshBuilderClass &builder, Mesh &mesh,
                                     MeshDeformModData &mod_data,
                                     Matrix3 *transform) {
  //
...
bool MeshDeformSaveClass::Export_Sets(ChunkSaveClass &chunk_save) {
  ...
  //
  for (int set_index = 0; (set_index < m_DeformSets.Count()) && retval;
       set_index++) {
    ...
    if (retval) {
      ...
      retval &=
          (chunk_save.Write(&set_info, sizeof(set_info)) == sizeof(set_info));
      if (retval) {
        //
...
bool MeshDeformSaveClass::Export_Keyframes(ChunkSaveClass &chunk_save,
  ...
  for (int keyframe_index = 0; (keyframe_index < count) && retval;
       keyframe_index++) {
    //
    ...
    if (retval) {
      ...
      retval &= (chunk_save.Write(&keyframe_info, sizeof(keyframe_info)) ==
      ...
      if (retval) {
        ...
        for (int index = 0; (index < data_count) && retval; index++) {
          MeshDeformSaveSetClass::DEFORM_DATA &data =
              set_save.Get_Deform_Data(keyframe_index, index);
          ...
          if (retval) {
            ...
            // If we are using vertex alpha instead of vertex color, then
            // convert the v-color into an alpha setting
            data_struct.Color.A = 255;
            if (m_AlphaPasses != 0) {
              data_struct.Color.A = (data_struct.Color.R + data_struct.Color.G +
                                     data_struct.Color.B) /
                                    3.0F;
            }

            retval &= (chunk_save.Write(&data_struct, sizeof(data_struct)) ==
                       sizeof(data_struct));
            retval &= chunk_save.End_Chunk();
          }
        }
...
{ MeshDeformSaveSetClass *set_save = m_DeformSets[set_index];
                ...
                //
                for (int keyframe_index = 0; keyframe_index <
set_save->Get_Keyframe_Count (); keyframe_index ++) { temp_list.Delete_All ();
                        for (int index = 0; index <
set_save->Get_Deform_Data_Count (keyframe_index); index ++) {
                                MeshDeformSaveSetClass::DEFORM_DATA &data =
set_save->Get_Deform_Data (keyframe_index, index);

                                //
                                //	Now try to find the 'W3D' index of this
vertex (its different than the max version).
                                //
                                //bool found = false;
                                for (int vert_index = 0; vert_index <
builder.Get_Vertex_Count (); vert_index++) { MeshBuilderClass::VertClass &vert =
builder.Get_Vertex (vert_index);

                                        //
                                        //	Reindex this vertex if its the
one we are looking for.
                                        //
                                        if (vert.Id == (int)data.vert_index) {
                                                MeshDeformSaveSetClass::DEFORM_DATA
new_data = data; new_data.vert_index = vert_index; temp_list.Add (new_data);
                        ...
                        set_save->Replace_Deform_Data (keyframe_index,
temp_list);
                }
        }
    ...
    if (set_save) {
      ...
      for (int keyframe_index = 0; (keyframe_index < count) && !retval;
        ...
        for (int index = 0; (index < data_count) && !retval; index++) {
          ...
          //
          if ((data.color.x != 1) || (data.color.y != 1) ||
              (data.color.z != 1)) {
            retval = true;
          }
        }
