 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * GameMtlDlg::GameMtlDlg -- constructor * GameMtlDlg::~GameMtlDlg
 *-- destructor!                                                    *
 *   GameMtlDlg::ClassID -- Returns the ClassID of GameMtl *
 *   GameMtlDlg::Invalidate -- causes the dialog to be redrawn *
 *   GameMtlDlg::ReloadDialog -- Updates the values in all of the dialog's
 *controls            * GameMtlDlg::SetTime -- Sets the time value, updates the
 *material and the dialog           * GameMtlDlg::PanelProc -- Windows Message
 *handler                                          * PanelDlgProc -- Windows
 *Proc which thunks into GameMtlDlg::PanelProc                      *
 *   GameMtlDlg::LoadDialog -- Sets the state of all of the dialog's controls *
 *   GameMtlDlg::UpdateMtlDisplay -- Informs MAX that the material parameters
 *have changed     * GameMtlDlg::ActivateDlg -- Activates and deactivates the
 *dialog                           * GameMtlDlg::SetThing -- Sets the material
 *to be edited                                    * GameMtlDlg::BuildDialog --
 *Adds the dialog to the material editor                         *
 ...
 *   GameMtlDlg::NotesProc -- Dialog Proc for the Notes panel * HintsDlgProc --
 *Dialog proc which thunks to GameMtlDlg::HintsProc                         *
 *   GameMtlDlg::HintsProc -- Dialog Proc for the hints panel *
 *   GameMtlDlg::PsxProc -- Dialog proc for the PSX options panel * PsxDlgProc
 *-- Dialog proc which thunks into GameMtlDlg::PsxProc *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
static inline float PcToFrac(int pc) { return (float)pc / 100.0f; }
 ...
 *                                                                                             *
 * INPUT: * hwMtlEdit - windows handle of the MAX material editor
 ...
 * HISTORY: * 06/26/1997 GH  : Created. *
 *=============================================================================================*/
Class_ID GameMtlDlg::ClassID() { return GameMaterialClassID; }
...
/***********************************************************************************************
 * GameMtlDlg::SetTime -- Sets the time value, updates the material and the
 *dialog             *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 06/26/1997 GH  : Created. *
 *=============================================================================================*/
BOOL GameMtlDlg::PanelProc(HWND hwndDlg, UINT msg, WPARAM wParam,
                           LPARAM lParam) {
  ...
  switch (msg) {
    case WM_INITDIALOG: {
      ...
      DiffuseSwatch =
          GetIColorSwatch(GetDlgItem(hwndDlg, IDC_DIFFUSE_COLOR),
                          TheMtl->GetDiffuse(), Get_String(IDS_DIFFUSE_COLOR));
      SpecularSwatch = GetIColorSwatch(GetDlgItem(hwndDlg, IDC_SPECULAR_COLOR),
                                       TheMtl->GetSpecular(),
                                       Get_String(IDS_SPECULAR_COLOR));

      AmbientCoeffSwatch = GetIColorSwatch(
          GetDlgItem(hwndDlg, IDC_AMBIENT_COEFF), TheMtl->GetAmbientCoeff(),
          Get_String(IDS_AMBIENT_COEFF));
      DiffuseCoeffSwatch = GetIColorSwatch(
          GetDlgItem(hwndDlg, IDC_DIFFUSE_COEFF), TheMtl->GetDiffuseCoeff(),
          Get_String(IDS_DIFFUSE_COEFF));
      SpecularCoeffSwatch = GetIColorSwatch(
          GetDlgItem(hwndDlg, IDC_SPECULAR_COEFF), TheMtl->GetSpecularCoeff(),
          Get_String(IDS_SPECULAR_COEFF));
      EmissiveCoeffSwatch = GetIColorSwatch(
          GetDlgItem(hwndDlg, IDC_EMISSIVE_COEFF), TheMtl->GetEmissiveCoeff(),
          Get_String(IDS_EMISSIVE_COEFF));

      DCTFramesSpin =
          SetupIntSpinner(hwndDlg, IDC_DCT_FRAMES_SPIN, IDC_DCT_FRAMES_EDIT, 1,
                          999, TheMtl->DCTFrames);
      DITFramesSpin =
          SetupIntSpinner(hwndDlg, IDC_DIT_FRAMES_SPIN, IDC_DIT_FRAMES_EDIT, 1,
                          999, TheMtl->DITFrames);
      SCTFramesSpin =
          SetupIntSpinner(hwndDlg, IDC_SCT_FRAMES_SPIN, IDC_SCT_FRAMES_EDIT, 1,
                          999, TheMtl->SCTFrames);
      SITFramesSpin =
          SetupIntSpinner(hwndDlg, IDC_SIT_FRAMES_SPIN, IDC_SIT_FRAMES_EDIT, 1,
                          999, TheMtl->SITFrames);

      DCTRateSpin =
          SetupFloatSpinner(hwndDlg, IDC_DCT_RATE_SPIN, IDC_DCT_RATE_EDIT, 0.0f,
                            60.0f, TheMtl->DCTFrameRate, 5.0f);
      DITRateSpin =
          SetupFloatSpinner(hwndDlg, IDC_DIT_RATE_SPIN, IDC_DIT_RATE_EDIT, 0.0f,
                            60.0f, TheMtl->DITFrameRate, 5.0f);
      SCTRateSpin =
          SetupFloatSpinner(hwndDlg, IDC_SCT_RATE_SPIN, IDC_SCT_RATE_EDIT, 0.0f,
                            60.0f, TheMtl->SCTFrameRate, 5.0f);
      SITRateSpin =
          SetupFloatSpinner(hwndDlg, IDC_SIT_RATE_SPIN, IDC_SIT_RATE_EDIT, 0.0f,
                            60.0f, TheMtl->SITFrameRate, 5.0f);

      OpacitySpin =
          SetupFloatSpinner(hwndDlg, IDC_OPACITY_SPIN, IDC_OPACITY_EDIT, 0.0f,
                            1.0f, TheMtl->GetOpacity(), 0.01f);
      TranslucencySpin = SetupFloatSpinner(hwndDlg, IDC_TRANSLUCENCY_SPIN,
                                           IDC_TRANSULCENCY_EDIT, 0.0f, 1.0f,
                                           TheMtl->GetTranslucency(), 0.01f);
      ShininessSpin =
          SetupFloatSpinner(hwndDlg, IDC_SHININESS_SPIN, IDC_SHININESS_EDIT,
                            1.0f, 1000.0f, TheMtl->GetShininess(), 1.0f);
      FogSpin = SetupFloatSpinner(hwndDlg, IDC_FOG_SPIN, IDC_FOG_EDIT, 0.0f,
                                  1.0f, TheMtl->FogCoeff, 0.01f);

      SendDlgItemMessage(hwndDlg, IDC_DCT_MAPPING_COMBO, CB_ADDSTRING, 0,
                         (LPARAM)(LPCTSTR)Get_String(IDS_UV_MAPPING));
      SendDlgItemMessage(hwndDlg, IDC_DCT_MAPPING_COMBO, CB_ADDSTRING, 0,
                         (LPARAM)(LPCTSTR)Get_String(IDS_ENVIRONMENT_MAPPING));

      SendDlgItemMessage(hwndDlg, IDC_DIT_MAPPING_COMBO, CB_ADDSTRING, 0,
                         (LPARAM)(LPCTSTR)Get_String(IDS_UV_MAPPING));
      SendDlgItemMessage(hwndDlg, IDC_DIT_MAPPING_COMBO, CB_ADDSTRING, 0,
                         (LPARAM)(LPCTSTR)Get_String(IDS_ENVIRONMENT_MAPPING));

      SendDlgItemMessage(hwndDlg, IDC_SCT_MAPPING_COMBO, CB_ADDSTRING, 0,
                         (LPARAM)(LPCTSTR)Get_String(IDS_UV_MAPPING));
      SendDlgItemMessage(hwndDlg, IDC_SCT_MAPPING_COMBO, CB_ADDSTRING, 0,
                         (LPARAM)(LPCTSTR)Get_String(IDS_ENVIRONMENT_MAPPING));

      SendDlgItemMessage(hwndDlg, IDC_SIT_MAPPING_COMBO, CB_ADDSTRING, 0,
                         (LPARAM)(LPCTSTR)Get_String(IDS_UV_MAPPING));
      SendDlgItemMessage(hwndDlg, IDC_SIT_MAPPING_COMBO, CB_ADDSTRING, 0,
                         (LPARAM)(LPCTSTR)Get_String(IDS_ENVIRONMENT_MAPPING));
    ...
    case WM_COMMAND: {
      switch (id) {
        ...
        case IDC_USE_ALPHA_CHECK:
          TheMtl->SetAttribute(GAMEMTL_USE_ALPHA,
                               GetCheckBox(hwndDlg, IDC_USE_ALPHA_CHECK));
          UpdateMtlDisplay();
          TheMtl->NotifyChanged();
          break;
        case IDC_USE_SORTING_CHECK:
          TheMtl->SetAttribute(GAMEMTL_USE_SORTING,
                               GetCheckBox(hwndDlg, IDC_USE_SORTING_CHECK));
          break;
        case IDC_DCT_MAPPING_COMBO:
          mtype = SendDlgItemMessage(hwndDlg, IDC_DCT_MAPPING_COMBO,
                                     CB_GETCURSEL, 0, 0);
          TheMtl->DCTMappingType = mtype;
          break;
        case IDC_DIT_MAPPING_COMBO:
          mtype = SendDlgItemMessage(hwndDlg, IDC_DIT_MAPPING_COMBO,
                                     CB_GETCURSEL, 0, 0);
          TheMtl->DITMappingType = mtype;
          break;
        case IDC_SCT_MAPPING_COMBO:
          mtype = SendDlgItemMessage(hwndDlg, IDC_SCT_MAPPING_COMBO,
                                     CB_GETCURSEL, 0, 0);
        ...
        case IDC_VIEWPORT_DISPLAY_CHECK:
          TheMtl->Set_Viewport_Display_Status(
              GetCheckBox(hwndDlg, IDC_VIEWPORT_DISPLAY_CHECK));
          TheMtl->NotifyChanged();
          UpdateMtlDisplay();
          break;
      }
 ...
 * HISTORY: * 06/26/1997 GH  : Created. *
 *=============================================================================================*/
BOOL GameMtlDlg::NotesProc(HWND hwndDlg, UINT msg, WPARAM wParam,
                           LPARAM lParam) {
  int id = LOWORD(wParam);
  int code = HIWORD(wParam);
 ...
 * HISTORY: * 06/26/1997 GH  : Created. *
 *=============================================================================================*/
static BOOL CALLBACK NotesDlgProc(HWND hwndDlg, UINT msg, WPARAM wParam,
                                  LPARAM lParam) {
  GameMtlDlg *theDlg;
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 3/30/98    GTH : Created. *
 *=============================================================================================*/
BOOL GameMtlDlg::HintsProc(HWND hwndDlg, UINT msg, WPARAM wParam,
                           LPARAM lParam) {
  int id = LOWORD(wParam);
  int code = HIWORD(wParam);
    ...
    case WM_COMMAND: {
      switch (id) {
        case IDC_DIT_OVER_DCT_CHECK:
          TheMtl->SetAttribute(GAMEMTL_DIT_OVER_DCT,
                               GetCheckBox(hwndDlg, IDC_DIT_OVER_DCT_CHECK));
          break;

        case IDC_SIT_OVER_SCT_CHECK:
          TheMtl->SetAttribute(GAMEMTL_SIT_OVER_SCT,
                               GetCheckBox(hwndDlg, IDC_SIT_OVER_SCT_CHECK));
          break;

        case IDC_DIT_OVER_DIG_CHECK:
          TheMtl->SetAttribute(GAMEMTL_DIT_OVER_DIG,
                               GetCheckBox(hwndDlg, IDC_DIT_OVER_DIG_CHECK));
          break;

        case IDC_SIT_OVER_SIG_CHECK:
          TheMtl->SetAttribute(GAMEMTL_SIT_OVER_SIG,
                               GetCheckBox(hwndDlg, IDC_SIT_OVER_SIG_CHECK));
          break;

        case IDC_FAST_SPECULAR_AFTER_ALPHA_CHECK:
          TheMtl->SetAttribute(
              GAMEMTL_FAST_SPECULAR_AFTER_ALPHA,
              GetCheckBox(hwndDlg, IDC_FAST_SPECULAR_AFTER_ALPHA_CHECK));
          break;
      }
    } break;
  }
 ...
 * HISTORY: * 3/30/98    GTH : Created. *
 *=============================================================================================*/
static BOOL CALLBACK HintsDlgProc(HWND hwndDlg, UINT msg, WPARAM wParam,
                                  LPARAM lParam) {
  GameMtlDlg *theDlg;
  ...
  switch (msg) {
    ...
    case WM_COMMAND: {
      switch (id) {
        ...
        case IDC_100_TRANS:
          TheMtl->SetMaskedAttribute(GAMEMTL_PSX_TRANS_MASK,
                                     GAMEMTL_PSX_100_TRANS);
          break;

        case IDC_50_TRANS:
          TheMtl->SetMaskedAttribute(GAMEMTL_PSX_TRANS_MASK,
                                     GAMEMTL_PSX_50_TRANS);
          break;

        case IDC_25_TRANS:
          TheMtl->SetMaskedAttribute(GAMEMTL_PSX_TRANS_MASK,
                                     GAMEMTL_PSX_25_TRANS);
          break;

        case IDC_MINUS_100_TRANS:
          TheMtl->SetMaskedAttribute(GAMEMTL_PSX_TRANS_MASK,
                                     GAMEMTL_PSX_MINUS_100_TRANS);
          break;

        case IDC_NO_RT_LIGHTING:
          TheMtl->SetAttribute(GAMEMTL_PSX_NO_RT_LIGHTING,
                               GetCheckBox(hwndDlg, IDC_NO_RT_LIGHTING));
          break;
      }
    } break;
  }
 ...
 * HISTORY: * 3/31/98    GTH : Created. *
 *=============================================================================================*/
static BOOL CALLBACK PsxDlgProc(HWND hwndDlg, UINT msg, WPARAM wParam,
                                LPARAM lParam) {
  GameMtlDlg *theDlg;
...
void GameMtlDlg::LoadDialog(BOOL draw) {
  ...
  */
  if (TheMtl && HwndPanel) {
    ...
    ** Checkboxes
    */
    SetCheckBox(HwndPanel, IDC_USE_ALPHA_CHECK,
                TheMtl->GetAttribute(GAMEMTL_USE_ALPHA));
    SetCheckBox(HwndPanel, IDC_USE_SORTING_CHECK,
                TheMtl->GetAttribute(GAMEMTL_USE_SORTING));
    SetCheckBox(HwndPanel, IDC_VIEWPORT_DISPLAY_CHECK,
                TheMtl->Get_Viewport_Display_Status());
    ...
    ** Mapping types
    */
    SendDlgItemMessage(HwndPanel, IDC_DCT_MAPPING_COMBO, CB_SETCURSEL,
                       TheMtl->DCTMappingType, 0);
    SendDlgItemMessage(HwndPanel, IDC_DIT_MAPPING_COMBO, CB_SETCURSEL,
                       TheMtl->DITMappingType, 0);
    SendDlgItemMessage(HwndPanel, IDC_SCT_MAPPING_COMBO, CB_SETCURSEL,
                       TheMtl->SCTMappingType, 0);
    SendDlgItemMessage(HwndPanel, IDC_SIT_MAPPING_COMBO, CB_SETCURSEL,
                       TheMtl->SITMappingType, 0);
    ...
    */
    SetCheckBox(HwndPsx, IDC_NO_RT_LIGHTING,
                TheMtl->GetAttribute(GAMEMTL_PSX_NO_RT_LIGHTING));
    ...
    */
    SetCheckBox(HwndHints, IDC_DIT_OVER_DCT_CHECK,
                TheMtl->GetAttribute(GAMEMTL_DIT_OVER_DCT));
    SetCheckBox(HwndHints, IDC_SIT_OVER_SCT_CHECK,
                TheMtl->GetAttribute(GAMEMTL_SIT_OVER_SCT));
    SetCheckBox(HwndHints, IDC_DIT_OVER_DIG_CHECK,
                TheMtl->GetAttribute(GAMEMTL_DIT_OVER_DIG));
    SetCheckBox(HwndHints, IDC_SIT_OVER_SIG_CHECK,
                TheMtl->GetAttribute(GAMEMTL_SIT_OVER_SIG));
    SetCheckBox(HwndHints, IDC_FAST_SPECULAR_AFTER_ALPHA_CHECK,
                TheMtl->GetAttribute(GAMEMTL_FAST_SPECULAR_AFTER_ALPHA));
...
/***********************************************************************************************
 * GameMtlDlg::UpdateMtlDisplay -- Informs MAX that the material parameters have
 *changed       *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 06/26/1997 GH  : Created. *
 *=============================================================================================*/
void GameMtlDlg::UpdateMtlDisplay() { IParams->MtlChanged(); }
...
void GameMtlDlg::BuildDialog() {
  ...
  HwndPanel = IParams->AddRollupPage(
      AppInstance, MAKEINTRESOURCE(IDD_GAMEMTL_PANEL), PanelDlgProc,
      Get_String(IDS_PARAMETERS), (LPARAM)this,
      (TheMtl->GetFlag(GAMEMTL_ROLLUP1_OPEN) ? 0 : APPENDROLL_CLOSED));

  HwndPsx = IParams->AddRollupPage(
      AppInstance, MAKEINTRESOURCE(IDD_GAMEMTL_PSX_PANEL), PsxDlgProc,
      Get_String(IDS_PSX_OPTIONS), (LPARAM)this,
      (TheMtl->GetFlag(GAMEMTL_ROLLUP2_OPEN) ? 0 : APPENDROLL_CLOSED));

  HwndHints = IParams->AddRollupPage(
      AppInstance, MAKEINTRESOURCE(IDD_GAMEMTL_HINTS_PANEL), HintsDlgProc,
      Get_String(IDS_MATERIAL_HINTS), (LPARAM)this,
      (TheMtl->GetFlag(GAMEMTL_ROLLUP3_OPEN) ? 0 : APPENDROLL_CLOSED));

  HwndNotes = IParams->AddRollupPage(
      AppInstance, MAKEINTRESOURCE(IDD_MATERIAL_NOTES_PANEL), NotesDlgProc,
      Get_String(IDS_NOTES), (LPARAM)this,
