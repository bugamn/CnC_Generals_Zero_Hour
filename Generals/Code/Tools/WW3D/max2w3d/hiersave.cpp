...
/* $Header: /Commando/Code/Tools/max2w3d/hiersave.cpp 56    10/30/00 6:58p
 * Greg_h $ */
/***********************************************************************************************
 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * HierarchySaveClass::HierarchySaveClass -- constructor *
 ...
 *   HierarchySaveClass::Get_Node_Transform -- returns the transformation matrix
 *of specified n* HierarchySaveClass::get_relative_transform -- retruns tm
 *between this node and its parent * HierarchySaveClass::Get_Name -- returns the
 *name of this hierarchy                        * HierarchySaveClass::Get_Node
 *-- Get the Max INode                                         *
 ...
 *   HierarchySaveClass::Get_Export_Coordinate_System - find the bone and
 *coordinate system    * HierarchySaveClass::Save -- write the hierarchy into a
 *W3D file                           * HierarchySaveClass::Load -- read the
 *hierarchy from a W3D file                            *
 ...
 *   HierarchySaveClass::Get_Fixup_Transform -- gets the "fixup" transform for a
 *node          * HierarchySaveClass::fixup_matrix -- conditions a matrix *
 ...
 *   HierarchySaveClass::save_fixups -- writes the fixup transforms into a W3D
 *file            * HierarchySaveClass::load_header -- reads the header from a
 *W3D file                       * HierarchySaveClass::load_pivots -- reads the
 *pivots from a W3D file                       * HierarchySaveClass::load_fixups
 *-- reads the fixup transforms from a W3D file             *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 * HierarchySaveClass::HierarchySaveClass -- constructor *
 *                                                                                             *
 * INPUT: * root - root INode to construct the HTree from * time - current time
 *in Max, transforms at this time will be used                            *
 * treemeter - progress meter * hname - name for the hierarchy tree * fixup_type
 *- can be used to force all transforms to be translation only * fixuptree -
 *htree loaded from a previous export *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/26/1997 GH  : Created. *
 *=============================================================================================*/
HierarchySaveClass::HierarchySaveClass(INode* root, TimeValue time,
                                       Progress_Meter_Class& treemeter,
                                       char* hname, int fixuptype,
  ...
  /*
  ** This code-path is activated when the user has created a custom origin.  In
  *this case, we
  ** need to compute the transform which will make all bones relative to this
  *origin.
  */
 ...
 * INPUT: *
 *                                                                                             *
 * rootlist - list of root nodes to add to the htree * time - current time in
 *Max, transforms at this time will be used                            *
 * treemeter - progress meter * hname - name for the hierarchy tree * fixup_type
 *- can be used to force all transforms to be translation only * fixuptree -
 *htree loaded from a previous export * origin_offset - origin offset transform
 **
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 10/26/1997 GH  : Created. *
 *=============================================================================================*/
HierarchySaveClass::HierarchySaveClass(INodeListClass* rootlist, TimeValue time,
                                       Progress_Meter_Class& treemeter,
                                       char* hname, int fixuptype,
                                       HierarchySaveClass* fixuptree,
 ...
 * HISTORY: * 10/26/1997 GH  : Created. *
 *=============================================================================================*/
HierarchySaveClass::HierarchySaveClass() : Node(NULL), CurNode(0), CurTime(0) {}
 ...
 * HISTORY: * 10/26/1997 GH  : Created. *
 *=============================================================================================*/
HierarchySaveClass::~HierarchySaveClass(void) { Free(); }
 ...
 * HISTORY: * 10/26/1997 GH  : Created. *
 *=============================================================================================*/
void HierarchySaveClass::Free(void) { Node.Clear(); }
...
/***********************************************************************************************
 * HierarchySaveClass::get_relative_transform -- retruns tm between this node
 *and its parent   *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * HierarchySaveClass::Get_Export_Coordinate_System - find the bone and
 *coordinate system      * for the given object *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
...
void HierarchySaveClass::add_tree(INode* node, int pidx) {
  ...
  if (node->IsHidden()) {
    // if the node is hidden, do not add it but add its children to the current
    // parent.
    nextparent = pidx;

  } else if (TerrainModeEnabled &&
             (Is_Normal_Mesh(node) || Is_Null_Object(node))) {
...
Matrix3 HierarchySaveClass::Get_Fixup_Transform(int node) const {
  ...
  for (int j = 0; j < 4; j++) {
    m.SetRow(j, Point3(Node[node].Fixup.TM[j][0], Node[node].Fixup.TM[j][1],
                       Node[node].Fixup.TM[j][2]));
  }
...
bool HierarchySaveClass::save_header(ChunkSaveClass& csave) {
  ...
  if (csave.Write(&HierarchyHeader, sizeof(HierarchyHeader)) !=
...
bool HierarchySaveClass::save_pivots(ChunkSaveClass& csave) {
  ...
  for (uint32 i = 0; i < HierarchyHeader.NumPivots; i++) {
    if (csave.Write(&Node[i].Pivot, sizeof(W3dPivotStruct)) !=
...
bool HierarchySaveClass::save_fixups(ChunkSaveClass& csave) {
  ...
  for (uint32 i = 0; i < HierarchyHeader.NumPivots; i++) {
    if (csave.Write(&Node[i].Fixup, sizeof(W3dPivotFixupStruct)) !=
...
bool HierarchySaveClass::load_header(ChunkLoadClass& cload) {
  ...
  ** Load the header
  */
  if (cload.Read(&HierarchyHeader, sizeof(HierarchyHeader)) !=
      sizeof(HierarchyHeader)) {
    return false;
  }
...
bool HierarchySaveClass::load_pivots(ChunkLoadClass& cload) {
  for (uint32 i = 0; i < HierarchyHeader.NumPivots; i++) {
    Node[i].MaxNode = NULL;
    if (cload.Read(&Node[i].Pivot, sizeof(W3dPivotStruct)) !=
        sizeof(W3dPivotStruct)) {
      return false;
    }
  }
...
bool HierarchySaveClass::load_fixups(ChunkLoadClass& cload) {
  for (uint32 i = 0; i < HierarchyHeader.NumPivots; i++) {
    if (cload.Read(&Node[i].Fixup, sizeof(W3dPivotFixupStruct)) !=
        sizeof(W3dPivotFixupStruct)) {
      return false;
    }
  }
