...
/* $Header: /Commando/Code/Tools/max2w3d/vchannel.cpp 10    10/30/00 6:56p
 * Greg_h $ */
/***********************************************************************************************
...
static float filtertable[FILTER_TABLE_SIZE] = {
    0.00000001f, 0.0000001f, 0.000001f,  0.00001f,    0.0001f, 0.001f,
    0.01f,       0.1f,       1.0f,       10.0f,       100.0f,  1000.0f,
    10000.0f,    100000.0f,  1000000.0f, 10000000.0f,

};
static bool table_valid = false;

VectorChannelClass::VectorChannelClass(uint32 id, int maxframes, uint32 flags,
                                       int vectorlength, float32 *identvect)
    : ID(id),
      Flags(flags),
      MaxFrames(maxframes),
  ...
  if (false == table_valid) {
    ...
    for (int i = 0; i < FILTER_TABLE_GEN_SIZE; i++) {
      ...
      filtertable[i + FILTER_TABLE_GEN_START] =
          1.0f - sin(DEG_TO_RAD(90.0f * ratio));
    }
...
bool VectorChannelClass::SaveTimeCoded(ChunkSaveClass &csave,
  ...
  W3dTimeCodedAnimChannelStruct *chn =
      (W3dTimeCodedAnimChannelStruct *)malloc(channelsize);
...
{
        uint32		NumFrames;			// number of frames of
animation uint16		Pivot;				// pivot
effected by this channel uint8			VectorLen;
// num Channels uint8			Flags;				//
channel type float			Scale;				//
Filter Table Scale

        uint32		Data[1];				// OpCode Data
Stream
...
//				value1 (continue compression from this initial
//value) 				float *indata  // 16 values to compress 				float *outdata	// 16
//decompressed values
//
// output: float error;		// aggregate error for packet
//
float VectorChannelClass::test_compress(int filter_index, float scale,
                                        float value1, float *indata,
                                        float *outdata) {
  float error = 0.0f;
  ...
  for (int data_idx = 0; data_idx < 16; data_idx++) {
    ...
    for (float try_factor = -8.0f; try_factor < 8.0f; try_factor += 1.0f) {
      float temp =
          (try_factor * filter) + last_value;  // decompress using this filter
...
//				scale (filter table scale)
//				value1 (continue compression from this initial
//value) 				float *indata  // 16 values to compress 				unsigned char *pPacket
//
// output: float error;		// aggregate error for packet
//
float VectorChannelClass::compress(int filter_index, float scale, float value1,
                                   float *indata, unsigned char *pPacket,
                                   float *outdata) {
  float error = 0.0f;
  ...
  *pPacket = filter_index;
  ...
  for (int data_idx = 0; data_idx < 16; data_idx++) {
    // NOTE: PLAN TO ADD A LOOP IN HERE, to DETERMINE best factor via Brute
    // Force This could help under/over-flow problems
    {
      // brute
      int best_factor = 100;
      float least_error = 999999999.9f;
      for (float try_factor = -8.0f; try_factor < 8.0f; try_factor += 1.0f) {
        float temp =
            (try_factor * filter) + last_value;  // decompress using this filter
        temp -=
            indata[data_idx];  // delta decompressed value, vs original value
        temp = fabs(temp);
        if (temp < least_error) {
          least_error = temp;
          best_factor = try_factor;
...
bool VectorChannelClass::SaveAdaptiveDelta(ChunkSaveClass &csave,
                                           BitChannelClass *binmov) {
  ...
  memset(
      &chn->Data[0],
      channelsize - (sizeof(W3dAdaptiveDeltaAnimChannelStruct) - sizeof(char)),
      0x00);
  ...
  scale /= 7.0f;  // range of the delta encode is +- 7 units, in which ever
                  // filter range the smaller the scale, the better our
                  // precision is going to be
  ...
  for (i = 0; i < numpackets; i++) {
    for (int vi = 0; vi < VectorLen; vi++) {
      ...
      for (int try_filter = 0; try_filter < FILTER_TABLE_SIZE; try_filter++) {
        float temp_error = test_compress(try_filter, scale, last_value,
                                         original_packet, decompressed_packet);
      ...
      // log->printf("\nvi= %d frames %d to %d : error = %f ",vi, frame,
      // frame+15, least_error);
      ...
      pPacket += (sizeof(AdaptiveDeltaPacketStruct) * VectorLen *
                  ((frame - 1) >> 4));  // skip up to the appropriate packet
      pPacket += sizeof(AdaptiveDeltaPacketStruct) *
                 vi;  // skip up the appropriate vector index

      compress(best_filter, scale, last_value, original_packet, pPacket,
               decompressed_packet);
...
void VectorChannelClass::SetSaveOptions(bool compress, int flavor, float Terr,
                                        float Rerr, bool reduce,
                                        int reduce_percent) {
...
//  R2 - set invisible data to repeat last known position, the previous algorthm
//  caused problems with the current movie assets
//
...
//
void VectorChannelClass::remove_packet(W3dTimeCodedAnimChannelStruct *c,
                                       uint32 packet_idx) {
  assert(c);
...
void VectorChannelClass::compress(W3dTimeCodedAnimChannelStruct *c) {
  ...
  if (ReduceAnimation) {
    if (ReduceAnimationPercent) {
      ...
      if (maxFrames >= c->NumTimeCodes)
        return;  // desired minimum already attained
...
//
uint32 VectorChannelClass::find_useless_packet(W3dTimeCodedAnimChannelStruct *c,
                                               double tolerance) {
  ...
  if (c->NumTimeCodes > 1) {
    if (c->NumTimeCodes > 2) {
      ...
      for (uint32 try_idx = 0; try_idx < (c->NumTimeCodes - 2); try_idx++) {
        ...
        float32 tStart =
            ((*pTcSrc) &
             ~W3D_TIMECODED_BINARY_MOVEMENT_FLAG);  // upgrade to floats
...
// #define W3D_TIMECODED_BINARY_MOVEMENT_FLAG  0x80000000
//
uint32 VectorChannelClass::find_useless_packetQ(
    W3dTimeCodedAnimChannelStruct *c, double tolerance) {
  assert(c);                // make sure pointer exists
  assert(c->NumTimeCodes);  // make sure some packets exist
...
// #define W3D_TIMECODED_BINARY_MOVEMENT_FLAG  0x80000000
//
uint32 VectorChannelClass::find_least_useful_packet(
    W3dTimeCodedAnimChannelStruct *c) {
  static float32 tempvec[MAX_VECTOR_SIZE];
...
// #define W3D_TIMECODED_BINARY_MOVEMENT_FLAG  0x80000000
//
uint32 VectorChannelClass::find_least_useful_packetQ(
    W3dTimeCodedAnimChannelStruct *c) {
  assert(c);                // make sure pointer exists
  assert(c->NumTimeCodes);  // make sure some packets exist
  assert(c->VectorLen == 4);
