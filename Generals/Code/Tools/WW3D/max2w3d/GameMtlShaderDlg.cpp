 ...
 *                                                                                             *
 *                     $Archive::
 * /Commando/Code/Tools/max2w3d/GameMtlShaderDlg.cpp           $*
 *                                                                                             *
 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * GameMtlShaderDlg::GameMtlShaderDlg -- constructor *
 *   GameMtlShaderDlg::~GameMtlShaderDlg -- destructor *
 *   GameMtlShaderDlg::Dialog_Proc -- windows message handler *
 *   GameMtlShaderDlg::ReloadDialog -- reload the contents of all of the
 *controls              * GameMtlShaderDlg::ActivateDlg -- activate/deactivate
 *the dialog                           * GameMtlShaderDlg::Apply_Preset -- apply
 *a preset shader setting                           *
 *   GameMtlShaderDlg::Set_Preset -- determine preset shader setting from game
 *material        * GameMtlShaderDlg::CompareShaderToBlendPreset -- compare
 *preset to game material shader    * GameMtlShaderDlg::Set_Advanced_Defaults --
 *set advanced settings to defaults              *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
// Note: the array has NUM_SHADER_BLEND_PRESETS + 1 entries (due to the 'Custom'
// entry).
...
static const ShaderBlendSettingPreset
    ShaderBlendSettingPresets[NUM_SHADER_BLEND_PRESETS] = {
        {W3DSHADER_SRCBLENDFUNC_ONE, W3DSHADER_DESTBLENDFUNC_ZERO, true,
         false},  // Opaque
        {W3DSHADER_SRCBLENDFUNC_ONE, W3DSHADER_DESTBLENDFUNC_ONE, false,
         false},  // Add
        {W3DSHADER_SRCBLENDFUNC_ZERO, W3DSHADER_DESTBLENDFUNC_SRC_COLOR, false,
         false},  // Multiply
        {W3DSHADER_SRCBLENDFUNC_ONE, W3DSHADER_DESTBLENDFUNC_SRC_COLOR, false,
         false},  // Multiply and Add
        {W3DSHADER_SRCBLENDFUNC_ONE,
         W3DSHADER_DESTBLENDFUNC_ONE_MINUS_SRC_COLOR, false, false},  // Screen
        {W3DSHADER_SRCBLENDFUNC_SRC_ALPHA,
         W3DSHADER_DESTBLENDFUNC_ONE_MINUS_SRC_ALPHA, false,
         false},  // Alpha Blend
        {W3DSHADER_SRCBLENDFUNC_ONE, W3DSHADER_DESTBLENDFUNC_ZERO, true,
         true},  // Alpha Test
        {W3DSHADER_SRCBLENDFUNC_SRC_ALPHA,
         W3DSHADER_DESTBLENDFUNC_ONE_MINUS_SRC_ALPHA, true,
         true}  // Alpha Test and Blend
};
 ...
 *=============================================================================================*/
GameMtlShaderDlg::GameMtlShaderDlg(HWND parent, IMtlParams *imp, GameMtl *mtl,
                                   int pass)
 ...
 * HISTORY: * 11/23/98   GTH : Created. *
 *=============================================================================================*/
BOOL GameMtlShaderDlg::Dialog_Proc(HWND dlg_wnd, UINT message, WPARAM wparam,
                                   LPARAM lparam) {
  int cursel;
  int i;
  ...
  switch (message) {
    case WM_INITDIALOG:
      for (i = 0; i <= NUM_SHADER_BLEND_PRESETS; i++) {
        SendDlgItemMessage(dlg_wnd, IDC_PRESET_COMBO, CB_ADDSTRING, 0,
                           (LONG)_ShaderBlendSettingPresetNames[i]);
      }
      ...
      if (code == CBN_SELCHANGE) {
        switch (id) {
          case IDC_DEPTHCOMPARE_COMBO:
            cursel = SendDlgItemMessage(dlg_wnd, IDC_DEPTHCOMPARE_COMBO,
                                        CB_GETCURSEL, 0, 0);
          ...
          case IDC_PRIGRADIENT_COMBO:
            cursel = SendDlgItemMessage(dlg_wnd, IDC_PRIGRADIENT_COMBO,
                                        CB_GETCURSEL, 0, 0);
            TheMtl->Set_Pri_Gradient(PassIndex, cursel);
            TheMtl->Notify_Changed();
            break;
          case IDC_SECGRADIENT_COMBO:
            cursel = SendDlgItemMessage(dlg_wnd, IDC_SECGRADIENT_COMBO,
                                        CB_GETCURSEL, 0, 0);
            TheMtl->Set_Sec_Gradient(PassIndex, cursel);
            TheMtl->Notify_Changed();
            break;
          case IDC_SRCBLEND_COMBO:
            cursel = SendDlgItemMessage(dlg_wnd, IDC_SRCBLEND_COMBO,
                                        CB_GETCURSEL, 0, 0);
          ...
          case IDC_DETAILCOLOR_COMBO:
            cursel = SendDlgItemMessage(dlg_wnd, IDC_DETAILCOLOR_COMBO,
                                        CB_GETCURSEL, 0, 0);
            TheMtl->Set_Detail_Color_Func(PassIndex, cursel);
            TheMtl->Notify_Changed();
            break;
          case IDC_DETAILALPHA_COMBO:
            cursel = SendDlgItemMessage(dlg_wnd, IDC_DETAILALPHA_COMBO,
                                        CB_GETCURSEL, 0, 0);
            TheMtl->Set_Detail_Alpha_Func(PassIndex, cursel);
            TheMtl->Notify_Changed();
            break;
          case IDC_PRESET_COMBO:
            cursel = SendDlgItemMessage(dlg_wnd, IDC_PRESET_COMBO, CB_GETCURSEL,
                                        0, 0);
            Apply_Preset(cursel);
            break;
        }
        ...
        switch (id) {
          case IDC_DEPTHMASK_CHECK:
            if (SendDlgItemMessage(dlg_wnd, IDC_DEPTHMASK_CHECK, BM_GETCHECK, 0,
                                   0)) {
              TheMtl->Set_Depth_Mask(PassIndex,
          ...
          case IDC_ALPHATEST_CHECK:
            if (SendDlgItemMessage(dlg_wnd, IDC_ALPHATEST_CHECK, BM_GETCHECK, 0,
                                   0)) {
...
void GameMtlShaderDlg::ReloadDialog(void) {
  ...
  SendDlgItemMessage(m_hWnd, IDC_DESTBLEND_COMBO, CB_SETCURSEL,
                     TheMtl->Get_Dest_Blend(PassIndex), 0);
  SendDlgItemMessage(m_hWnd, IDC_SRCBLEND_COMBO, CB_SETCURSEL,
                     TheMtl->Get_Src_Blend(PassIndex), 0);
  SendDlgItemMessage(m_hWnd, IDC_PRIGRADIENT_COMBO, CB_SETCURSEL,
                     TheMtl->Get_Pri_Gradient(PassIndex), 0);
  SendDlgItemMessage(m_hWnd, IDC_SECGRADIENT_COMBO, CB_SETCURSEL,
                     TheMtl->Get_Sec_Gradient(PassIndex), 0);
  SendDlgItemMessage(m_hWnd, IDC_DEPTHCOMPARE_COMBO, CB_SETCURSEL,
                     TheMtl->Get_Depth_Compare(PassIndex), 0);
  SendDlgItemMessage(m_hWnd, IDC_DETAILCOLOR_COMBO, CB_SETCURSEL,
                     TheMtl->Get_Detail_Color_Func(PassIndex), 0);
  SendDlgItemMessage(m_hWnd, IDC_DETAILALPHA_COMBO, CB_SETCURSEL,
                     TheMtl->Get_Detail_Alpha_Func(PassIndex), 0);
  Set_Preset();
...
void GameMtlShaderDlg::Apply_Preset(int preset_index) {
  ...
  const ShaderBlendSettingPreset &preset =
      ShaderBlendSettingPresets[preset_index];

  TheMtl->Set_Src_Blend(PassIndex, preset.SrcBlend);

  TheMtl->Set_Dest_Blend(PassIndex, preset.DestBlend);

  int depth_mask = preset.DepthMask ? W3DSHADER_DEPTHMASK_WRITE_ENABLE
                                    : W3DSHADER_DEPTHMASK_WRITE_DISABLE;
  TheMtl->Set_Depth_Mask(PassIndex, depth_mask);

  int alpha_test = preset.AlphaTest ? W3DSHADER_ALPHATEST_ENABLE
                                    : W3DSHADER_ALPHATEST_DISABLE;
...
/***********************************************************************************************
 * GameMtlShaderDlg::CompareShaderToBlendPreset -- compare preset to game
 *material shader      *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 02/26/99   NH : Created. *
 *=============================================================================================*/
bool GameMtlShaderDlg::CompareShaderToBlendPreset(
    const ShaderBlendSettingPreset &blend_preset) {
  ...
  bool depthmask =
      TheMtl->Get_Depth_Mask(PassIndex) != W3DSHADER_DEPTHMASK_WRITE_DISABLE;
  if (depthmask != blend_preset.DepthMask) return false;
  bool alphatest =
      TheMtl->Get_Alpha_Test(PassIndex) != W3DSHADER_ALPHATEST_DISABLE;
