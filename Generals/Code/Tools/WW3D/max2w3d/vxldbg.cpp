...
** Static functions
*/
static BOOL CALLBACK _dialog_proc(HWND hWnd, UINT message, WPARAM wParam,
                                  LPARAM lParam);
...
bool VoxelDebugWindowClass::Dialog_Proc(HWND hWnd, UINT message, WPARAM wParam,
                                        LPARAM) {
  RECT crect;
  ...
  switch (message) {
    ...
    case WM_INITDIALOG:
      ...
      Bitmap = new SimpleDIBClass(ViewportHWND, Voxel->Get_Width(),
                                  Voxel->Get_Height(), _VoxelPalette);

      // initialize the layer spinner
      LayerSpin = SetupIntSpinner(hWnd, IDC_LAYER_SPIN, IDC_LAYER_EDIT, 0,
                                  Voxel->Num_Layers(), 0);
...
void VoxelDebugWindowClass::update_display(void) {
  ...
  */
  ...
  StretchBlt(hdcwindow, x0, y0, x1, y1, hdcdib, 0, 0, Bitmap->Get_Width(),
             Bitmap->Get_Height(), SRCCOPY);
...
BOOL CALLBACK _dialog_proc(HWND hWnd, UINT message, WPARAM wParam,
                           LPARAM lParam) {
