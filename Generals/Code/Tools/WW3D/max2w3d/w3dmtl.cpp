...
void W3dMapClass::Set_Filename(const char *fullpath) {
  ...
  if (fullpath) {
    ...
    // strupr(fname);						(gth) need to
    // preserve case since unix/PS2 is case sensitive...
...
W3dMaterialClass::~W3dMaterialClass(void) { Free(); }
...
void W3dMaterialClass::Set_Mapper_Args(const char *args_buffer, int pass,
                                       int stage) {
  assert(pass >= 0);
  assert(pass < PassCount);
  assert(stage >= 0);
...
void W3dMaterialClass::Set_Texture(const W3dMapClass &map, int pass,
                                   int stage) {
  assert(pass >= 0);
  assert(pass < PassCount);
...
int W3dMaterialClass::Get_Sort_Level(void) const { return SortLevel; }

int W3dMaterialClass::Get_Pass_Count(void) const { return PassCount; }
...
void W3dMaterialClass::Init(Mtl *mtl, char *materialColorTexture) {
  ...
  if ((mtl->ClassID() == GameMaterialClassID) || ps2_mat) {
    Init((GameMtl *)mtl,
         materialColorTexture);  ///@todo: Fix this for substituted textures.
    return;
  }
  ...
  */
  ...
  mat.Emissive.R = mat.Emissive.G = mat.Emissive.B =
      0;  //(uint8)(255 .0f * mtl->GetSelfIllum());
  ...
  if (materialColorTexture && !mtl->GetSubTexmap(ID_DI) &&
      !mtl->GetSubTexmap(ID_RL)) {  // no textures on material, substitute
                                    // textures to improve rendering speed.
    tex.Set_Filename(
        materialColorTexture);  ///@todo: Fix this to procedural name/path
    W3d_Shader_Set_Texturing(&shader, W3DSHADER_TEXTURING_ENABLE);
    // This texture will hold solid pixels of material color, don't need any
    // filtering.
    //		W3dTextureInfoStruct texinfo;
    //		memset(&texinfo,0,sizeof(texinfo));
    //		texinfo.Attributes = texinfo.Attributes |
    ///*W3DTEXTURE_NO_LOD|*/W3DTEXTURE_CLAMP_U | W3DTEXTURE_CLAMP_V;
...
void W3dMaterialClass::Init(GameMtl *gamemtl, char *materialColorTexture) {
  ...
  for (int pass = 0; pass < gamemtl->Get_Pass_Count(); pass++) {
    ...
    */
    ...
    shader.PostDetailColorFunc = gamemtl->Get_Detail_Color_Func(
        pass);  // (gth) set up the post-detail options
    ...
    */
    ...
    switch (gamemtl->Get_Mapping_Type(pass, 1)) {
      case GAMEMTL_MAPPING_UV:
        mat.Attributes |= W3DVERTMAT_STAGE1_MAPPING_UV;
        break;
      case GAMEMTL_MAPPING_ENV:
    ...
    switch (gamemtl->Get_PSX_Translucency(pass)) {
      case GAMEMTL_PSX_TRANS_NONE:
        mat.Attributes |= W3DVERTMAT_PSX_TRANS_NONE;
        break;
      case GAMEMTL_PSX_TRANS_100:
        mat.Attributes |= W3DVERTMAT_PSX_TRANS_100;
        break;
      case GAMEMTL_PSX_TRANS_50:
        mat.Attributes |= W3DVERTMAT_PSX_TRANS_50;
        break;
      case GAMEMTL_PSX_TRANS_25:
        mat.Attributes |= W3DVERTMAT_PSX_TRANS_25;
        break;
      case GAMEMTL_PSX_TRANS_MINUS_100:
        mat.Attributes |= W3DVERTMAT_PSX_TRANS_MINUS_100;
        break;
    };
    ...
    */
    ...
    for (int stage = 0; stage < MAX_STAGES; stage++) {
      if (gamemtl->Get_Texture_Enable(pass, stage) &&
        ...
        // If the shader indicates bump-environment mapping mark this texture as
        // a bumpmap.
        if ((stage == 0) && (gamemtl->Get_Pri_Gradient(pass) ==
        ...
        Set_Mapper_Args(gamemtl->Get_Mapping_Arg_Buffer(pass, stage), pass,
                        stage);

      } else {
        if (materialColorTexture) {  // no textures on material, substitute
                                     // textures to improve rendering speed.
          w3dmap.Reset();
        ...
        break;  // break out of the loop (and dont put in stage 1 if stage 0 is
                // missing...)
      }
    }
...
bool W3dMaterialClass::Is_Multi_Pass_Transparent(void) const {
  return ((PassCount >= 2) &&
          (Get_Shader(0).DestBlend != W3DSHADER_DESTBLENDFUNC_ZERO));
}
...
void W3dMaterialDescClass::VertMatClass::Set_Mapper_Args(const char *args,
                                                         int stage) {
  if (MapperArgs[stage]) {
    delete[] (MapperArgs[stage]);
    MapperArgs[stage] = NULL;
...
bool W3dMaterialDescClass::MaterialRemapClass::operator!=(
...
W3dMaterialDescClass::W3dMaterialDescClass(void) { Reset(); }
...
W3dMaterialDescClass::ErrorType W3dMaterialDescClass::Add_Material(
    const W3dMaterialClass &mat, const char *name) {
  /*
  ...
  */
  ...
  if (mat.Is_Multi_Pass_Transparent()) {
    ...
    if (IDYES != MessageBox(window, msg, "Confirmation",
  ...
  */
  ...
  for (int pass = 0; pass < PassCount; pass++) {
    remap.VertexMaterialIdx[pass] = Add_Vertex_Material(
        mat.Get_Vertex_Material(pass), mat.Get_Mapper_Args(pass, 0),
        mat.Get_Mapper_Args(pass, 1), pass, name);

    remap.ShaderIdx[pass] = Add_Shader(mat.Get_Shader(pass), pass);

    for (int stage = 0; stage < W3dMaterialClass::MAX_STAGES; stage++) {
      remap.TextureIdx[pass][stage] =
          Add_Texture(mat.Get_Texture(pass, stage), pass, stage);
...
int W3dMaterialDescClass::Pass_Count(void) { return PassCount; }

int W3dMaterialDescClass::Vertex_Material_Count(void) {
  return VertexMaterials.Count();
}

int W3dMaterialDescClass::Shader_Count(void) { return Shaders.Count(); }

int W3dMaterialDescClass::Texture_Count(void) { return Textures.Count(); }

int W3dMaterialDescClass::Get_Sort_Level(void) { return SortLevel; }

W3dVertexMaterialStruct *W3dMaterialDescClass::Get_Vertex_Material(
    int vmat_index) {
  assert(vmat_index >= 0);
...
int W3dMaterialDescClass::Get_Texture_Index(int mat_index, int pass,
                                            int stage) {
  assert(mat_index >= 0);
  assert(mat_index < MaterialRemaps.Count());
  assert(pass >= 0);
...
const char *W3dMaterialDescClass::Get_Mapper_Args(int mat_index, int pass,
                                                  int stage) {
  int index = Get_Vertex_Material_Index(mat_index, pass);
  if (index == -1) {
    return NULL;
...
W3dMapClass *W3dMaterialDescClass::Get_Texture(int mat_index, int pass,
                                               int stage) {
...
bool W3dMaterialDescClass::Stage_Needs_Texture_Coordinates(int pass,
  ...
  for (int mi = 0; mi < MaterialRemaps.Count(); mi++) {
    ...
    if (shader) {
      if (stage == 0) {
        if (W3d_Shader_Get_Texturing(shader) == W3DSHADER_TEXTURING_ENABLE)
          return true;
      }

      if (stage == 1) {
        if ((W3d_Shader_Get_Detail_Color_Func(shader) !=
             W3DSHADER_DETAILCOLORFUNC_DISABLE) ||
            (W3d_Shader_Get_Detail_Alpha_Func(shader) !=
...
int W3dMaterialDescClass::Add_Vertex_Material(W3dVertexMaterialStruct *vmat,
  ...
  for (int vi = 0; vi < VertexMaterials.Count(); vi++) {
    if ((crc == VertexMaterials[vi].Crc) &&
        (pass == VertexMaterials[vi].PassIndex)) {
      break;
    }
  }
...
unsigned long W3dMaterialDescClass::Compute_Crc(
    const W3dVertexMaterialStruct &vmat, const char *mapper_args0,
    const char *mapper_args1) {
  unsigned long crc = 0;
  crc = CRC_Memory((const unsigned char *)&vmat.Attributes,
                   sizeof(vmat.Attributes), crc);
  crc = CRC_Memory((const unsigned char *)&vmat.Ambient, sizeof(vmat.Ambient),
                   crc);
  crc = CRC_Memory((const unsigned char *)&vmat.Diffuse, sizeof(vmat.Diffuse),
                   crc);
  crc = CRC_Memory((const unsigned char *)&vmat.Specular, sizeof(vmat.Specular),
                   crc);
  crc = CRC_Memory((const unsigned char *)&vmat.Emissive, sizeof(vmat.Emissive),
                   crc);
  crc = CRC_Memory((const unsigned char *)&vmat.Shininess,
                   sizeof(vmat.Shininess), crc);
  crc = CRC_Memory((const unsigned char *)&vmat.Opacity, sizeof(vmat.Opacity),
                   crc);
  crc = CRC_Memory((const unsigned char *)&vmat.Translucency,
                   sizeof(vmat.Translucency), crc);
...
unsigned long W3dMaterialDescClass::Compute_Crc(const W3dMapClass &map) {
  ...
  if (map.AnimInfo != NULL) {
    crc = CRC_Memory((const unsigned char *)&map.AnimInfo->Attributes,
                     sizeof(map.AnimInfo->Attributes), crc);
    crc = CRC_Memory((const unsigned char *)&map.AnimInfo->AnimType,
                     sizeof(map.AnimInfo->AnimType), crc);
    crc = CRC_Memory((const unsigned char *)&map.AnimInfo->FrameCount,
                     sizeof(map.AnimInfo->FrameCount), crc);
    crc = CRC_Memory((const unsigned char *)&map.AnimInfo->FrameRate,
                     sizeof(map.AnimInfo->FrameRate), crc);
  }
  crc = CRC_Stringi(map.Filename, crc);
  return crc;
}

unsigned long W3dMaterialDescClass::Add_String_To_Crc(const char *str,
