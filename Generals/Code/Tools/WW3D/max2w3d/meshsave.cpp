...
/* $Header: /Commando/Code/Tools/max2w3d/meshsave.cpp 107   8/21/01 10:28a
 * Greg_h $ */
/***********************************************************************************************
 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * MeshSaveClass::MeshSaveClass -- constructor, processes a Max
 *mesh                         * MeshSaveClass::~MeshSaveClass -- destructor,
 ...
 *   MeshSaveClass::write_normals -- writes the vertex normals chunk into a wtm
 *file           * MeshSaveClass::write_vert_normals -- Writes the surrender
 *normal chunk into a wtm file    * MeshSaveClass::write_triangles -- Write the
 *triangles chunk into a wtm file.              *
 *   MeshSaveClass::write_sr_triangles -- writes the triangles in surrender
 *friendly format    * MeshSaveClass::write_triangles -- write the triangles
 *chunk                               * MeshSaveClass::compute_surrender_vertex
 *-- Compute the surrender vertex normals           *
 *   MeshSaveClass::setup_material -- Gets the texture names and base colors for
 *a material    * MeshSaveClass::compute_bounding_volumes -- computes a bounding
 *box and bounding sphere for* MeshSaveClass::set_transform -- set the default
 *transformation matrix for the mesh        *
 *   MeshSaveClass::compute_physical_properties -- computes the volume and
 *moment of inertia   * MeshSaveClass::prep_mesh -- pre-transform the MAX mesh
 *by a specified matrix              * MeshSaveClass::write_user_text -- write
 *the user text chunk                               *
 *   MeshSaveClass::get_htree_bone_index_for_inode -- searches the htree for the
 *given INode   * MeshSaveClass::get_skin_modifier_objects -- Searches for the
 *WWSkin modifier for this mes * MeshSaveClass::inv_deform_mesh -- preprocess
 *the mesh for skinning                        * MeshSaveClass::create_materials
 *-- create the materials for this mesh                     *
 *   MeshSaveClass::write_ps2_shaders -- Write shaders specific to the PS2 in
 *their own chunk. *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
const int VOXEL_RESOLUTION =
    64;  // resolution to use when computing I, V and CM

#define DEBUG_VOXELS 0
#define MIN_AABTREE_POLYGONS 8
#define DIFFUSE_HOUSECOLOR_TEXTURE_PREFIX \
  0x4443485A  //'ZHCD' prefix put on all code generated textures
#define DIFFUSE_COLOR_TEXTURE_PREFIX \
  0x44434d5A  //'ZMCD' prefix put on all code generated textures
...
** compute vertex normals or store u-v's in that case (prevents vertex
*splitting)
**
************************************************************************************/
 ...
 *                                                                                             *
 * This class takes a MAX mesh and computes the information for a W3D mesh or
 *skin.            *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * inode - the max INode containing the mesh/skin to export * exportspace -
 *matrix defining the desired coordinate system for the mesh * htree - hierarchy
 *tree that this mesh is being connected to                                 *
 * curtime - current time in Max. * meter - progress meter * mesh_name - name to
 *use for the mesh                                                        *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 06/10/1997 GH  : Created. *
 *=============================================================================================*/
MeshSaveClass::MeshSaveClass(
    char *mesh_name, char *container_name, INode *inode, const Mesh *input_mesh,
    Matrix3 &exportspace, W3DAppData2Struct &exportoptions,
    HierarchySaveClass *htree, TimeValue curtime, Progress_Meter_Class &meter,
    unsigned int *materialColors, int &numMaterialColors, int &numHouseColors,
    char *materialColorTexture, WorldInfoClass *world_info)
    : MaxINode(inode),
  ...
  meter.Finish_In_Steps(3 * Header.NumTris +  // normals
  ...
  if (((Header.Attributes & W3D_MESH_FLAG_GEOMETRY_TYPE_MASK) ==
  ...
  if (materialColorTexture) {  // diffuse color materials are replaced by
                               // textures
...
// search through previously found material colors and return index.  house
// colors are always placed in top row.
void getMaterialUV(UVVert &tvert, unsigned int diffuse,
                   unsigned int *materialColors, int &numMaterialColors,
                   int &numHouseColors, bool house) {
  int i;

  if (house) {  // this material is a house color, place it in first row.
    for (i = 0; i < 16; i++) {
      if (materialColors[i] == diffuse) {
        tvert.x = ((double)(i % 16) + 0.5) /
                  16.0;  ///@todo: MW: Remove hard-coded texture size
    ...
    ExportLog::printf("\nUndefined House Color %d,%d,%d",
                      (diffuse >> 16) & 0xff, (diffuse >> 8) & 0xff,
                      diffuse & 0xff);
...
void MeshSaveClass::Build_Mesh(Mesh &mesh, Mtl *node_mtl,
                               ...
                               int &numMaterialColors, int &numHouseColors) {
  int vert_counter;
  int face_index;
  int pass;
  int stage;
  ...
  */
  ...
  for (face_index = 0; face_index < mesh.getNumFaces(); face_index++) {
    ...
    if (Header.NumMaterials > 0) {
      mat_index =
          MaterialRemapTable[(maxface.getMatID() % Header.NumMaterials)];
    }
    assert(mat_index != -1);
    ...
    for (pass = 0; pass < MaterialDesc.Pass_Count(); pass++) {
      ...
      for (stage = 0; stage < W3dMaterialClass::MAX_STAGES; stage++) {
        face.TextureIndex[pass][stage] =
            MaterialDesc.Get_Texture_Index(mat_index, pass, stage);
      }
    }
    ...
    */
    ...
    if ((node_mtl != NULL) && (node_mtl->NumSubMtls() > 1)) {
      mtl_to_use =
          node_mtl->GetSubMtl(maxface.getMatID() % node_mtl->NumSubMtls());
    }

    if ((mtl_to_use != NULL) &&
        ((mtl_to_use->ClassID() == GameMaterialClassID) ||
    ...
    for (vert_counter = 0; vert_counter < 3; vert_counter++) {
      ...
      */
      ...
      /*
      ** Texture coordinate.  Apply uv coords if the mesh has them and is not
      *being
      ** instructed to ignore them.
      ** - check if the mesh needs them (uses_simple_rendering() == false)
      ** - for each pass and stage, look up what map channel this face's
      *material is using
        ...
        for (stage = 0; stage < W3dMaterialClass::MAX_STAGES; stage++) {
          ...
          */
          ...
          W3dVertexMaterialStruct *vmat =
              MaterialDesc.Get_Vertex_Material(mat_index, pass);
          ...
          if (map3d && map3d->Filename &&
              *((unsigned int *)map3d->Filename) ==
                  DIFFUSE_COLOR_TEXTURE_PREFIX)  // check for prefix
          {
            double Diffuse =
                vmat->Diffuse.Get_Color() >> 8;  // get material color
            ...
            if (strnicmp(MaterialDesc.Get_Vertex_Material_Name(mat_index, pass),
                         "HouseColor", 10) == 0)
              getMaterialUV(tvert, vmat->Diffuse.Get_Color() >> 8,
                            materialColors, numMaterialColors, numHouseColors,
                            true);
            else
              getMaterialUV(tvert, vmat->Diffuse.Get_Color() >> 8,
                            materialColors, numMaterialColors, numHouseColors,
            ...
            // Keep track of first vertex material converted, so we can remap
            // all other non-textured materials to use the same material.
            if (firstSolidColoredMaterial == -1)
              firstSolidColoredMaterial =
                  MaterialDesc.Get_Vertex_Material_Index(mat_index, pass);
          }

          /*
          ** If the mesh needs uv coords and they are present, copy the uv into
          *tvert
          */
          else if (!use_simple_rendering(Header.Attributes)) {
            ...
            ///@todo: MW: Forced ingoring of uv coordinates if no texture!  Is
            ///this ok?
            W3dMapClass *map3d =
                MaterialDesc.Get_Texture(mat_index, pass, stage);
      ...
      */
      for (pass = 0; pass < MaterialDesc.Pass_Count(); pass++) {
        // if solid-color texture substitution, replace the vertex material with
        // first solid one found.
        W3dMapClass *map3d = MaterialDesc.Get_Texture(mat_index, pass, 0);
        if (map3d && map3d->Filename &&
            *((unsigned int *)map3d->Filename) ==
                DIFFUSE_COLOR_TEXTURE_PREFIX)  // check for prefix
          face.Verts[vert_counter].VertexMaterialIndex[pass] =
              firstSolidColoredMaterial;
        else
          face.Verts[vert_counter].VertexMaterialIndex[pass] =
              MaterialDesc.Get_Vertex_Material_Index(mat_index, pass);
      ...
      */
      ...
      if (is_skin) {
        ...
        // If this is a valid bone, try to find the corresponding bone index in
        // the HTree
        if ((skin_bone_index != -1) &&
            (skin_bone_index < skinobj->Num_Bones()) &&
            (skinobj->BoneTab[skin_bone_index] != NULL)) {
          face.Verts[vert_counter].BoneIndex =
              get_htree_bone_index_for_inode(skinobj->BoneTab[skin_bone_index]);
        }
      }
    }
...
/***********************************************************************************************
 * MeshSaveClass::get_skin_modifier_objects -- Searches for the WWSkin modifier
 *for this mesh  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: *
 *=============================================================================================*/
void MeshSaveClass::get_skin_modifier_objects(
    SkinDataClass **skin_data_ptr, SkinWSMObjectClass **skin_obj_ptr) {
  *skin_data_ptr = NULL;
  *skin_obj_ptr = NULL;
  ...
  for (int i = 0; i < MaxINode->NumRefs(); i++) {
    ...
    if (refTarg != NULL &&
        refTarg->ClassID() == Class_ID(WSM_DERIVOB_CLASS_ID, 0)) {
      ...
      for (int j = 0; j < wsm_der_obj->NumModifiers(); j++) {
        ...
        if (mod->ClassID() == SKIN_MOD_CLASS_ID) {
          ...
          *skin_obj_ptr = (SkinWSMObjectClass *)skinmod->GetReference(
              SkinModifierClass::OBJ_REF);
        }
      }
    }
  }
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/26/1997 GH  : Created. * 5/1/2000   gth : Created. *
 *=============================================================================================*/
void MeshSaveClass::inv_deform_mesh() {
  ...
  memset(VertInfluences, 0,
         sizeof(W3dVertInfStruct) * Builder.Get_Vertex_Count());
  Header.VertexChannels |= W3D_VERTEX_CHANNEL_BONEID;

  for (int vert_index = 0; vert_index < Builder.Get_Vertex_Count();
       vert_index++) {
    MeshBuilderClass::VertClass &vert = Builder.Get_Vertex(vert_index);

    if (vert.BoneIndex == 0) {
      // set the influence to 0 (root node) and leave the vert and normal
      // unchanged (gth) 07/11/2000: Note that in this case, the mesh
      // coordinates have already been transformed relative to the user or scene
      // origin and do not need to be further modified.
 ...
 * MeshSaveClass::Write_To_File -- Append the mesh to an open wtm file *
 *                                                                                             *
 * INPUT: * csave - ChunkSaveClass object to handle writing the chunk-based file
...
int MeshSaveClass::write_header(ChunkSaveClass &csave) {
  ...
  if (csave.Write(&Header, sizeof(W3dMeshHeader3Struct)) !=
 ...
 **
 *                                                                                             *
 * OUTPUT: * 0 if nothing went wrong, Non-Zero otherwise
 **
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
...
int MeshSaveClass::write_verts(ChunkSaveClass &csave) {
  ...
  for (int i = 0; i < Builder.Get_Vertex_Count(); i++) {
    ...
    if (csave.Write(&(w3dvert), sizeof(W3dVectorStruct)) !=
        sizeof(W3dVectorStruct)) {
      return 1;
    }
  }
...
/***********************************************************************************************
 * MeshSaveClass::write_vert_normals -- Writes the surrender normal chunk into a
 *wtm file      *
 *                                                                                             *
 * INPUT: * csave - chunk save object
 **
...
int MeshSaveClass::write_vert_normals(ChunkSaveClass &csave) {
  ...
  for (int i = 0; i < Builder.Get_Vertex_Count(); i++) {
    ...
    if (csave.Write(&(norm), sizeof(W3dVectorStruct)) !=
        sizeof(W3dVectorStruct)) {
      return 1;
    }
  }
...
/***********************************************************************************************
 * MeshSaveClass::write_vert_influences -- skins will have this chunk that binds
 *verts to bones*
 *                                                                                             *
 * INPUT: * csave - chunk save object
 **
...
int MeshSaveClass::write_vert_influences(ChunkSaveClass &csave) {
  ...
  if (csave.Write(VertInfluences, count * sizeof(W3dVertInfStruct)) !=
      count * sizeof(W3dVertInfStruct)) {
    return 1;
  }
...
int MeshSaveClass::write_vert_shade_indices(ChunkSaveClass &csave) {
  ...
  for (int i = 0; i < Builder.Get_Vertex_Count(); i++) {
    ...
    if (csave.Write(&(shade_index), sizeof(shade_index)) !=
        sizeof(shade_index)) {
      return 1;
    }
  }
...
int MeshSaveClass::write_vertex_materials(ChunkSaveClass &csave) {
  ...
  for (int i = 0; i < MaterialDesc.Vertex_Material_Count(); i++) {
    ...
    if (csave.Write(vmat, sizeof(W3dVertexMaterialStruct)) !=
...
int MeshSaveClass::write_shaders(ChunkSaveClass &csave) {
  ...
  for (int i = 0; i < MaterialDesc.Shader_Count(); i++) {
    ...
    if (csave.Write(shader, sizeof(W3dShaderStruct)) !=
...
int MeshSaveClass::write_ps2_shaders(ChunkSaveClass &csave) {
  ...
  for (int i = 0; i < MaterialDesc.Shader_Count(); i++) {
    ...
    if (csave.Write(&ps2shader, sizeof(W3dPS2ShaderStruct)) !=
  ...
  for (int i = 0; i < MaterialDesc.Shader_Count(); i++) {
    ...
    switch (W3d_Shader_Get_Depth_Compare(shader)) {
      ...
      case PSS_DEPTHCOMPARE_PASS_LESS:
        ...
        break;
      case PSS_DEPTHCOMPARE_PASS_LEQUAL:
        W3d_Shader_Set_Depth_Compare(shader,
...
int MeshSaveClass::write_textures(ChunkSaveClass &csave) {
  ...
  for (int i = 0; i < MaterialDesc.Texture_Count(); i++) {
    ...
    if (csave.Write(map->Filename, strlen(map->Filename) + 1) !=
        strlen(map->Filename) + 1)
      return 1;
    csave.End_Chunk();
    ...
    if (map->AnimInfo != NULL) {
      ...
      if (csave.Write(map->AnimInfo, sizeof(W3dTextureInfoStruct)) !=
...
int MeshSaveClass::write_texture_stage(ChunkSaveClass &csave, int pass,
                                       int stage) {
  const MeshBuilderClass::MeshStatsStruct &stats = Builder.Get_Mesh_Stats();
...
int MeshSaveClass::write_texture_ids(ChunkSaveClass &csave, int pass,
                                     int stage) {
  const MeshBuilderClass::MeshStatsStruct &stats = Builder.Get_Mesh_Stats();
...
int MeshSaveClass::write_texture_coords(ChunkSaveClass &csave, int pass,
                                        int stage) {
  const MeshBuilderClass::MeshStatsStruct &stats = Builder.Get_Mesh_Stats();
  ...
  for (int i = 0; i < Builder.Get_Vertex_Count(); i++) {
    ...
    if (csave.Write(&(tex), sizeof(W3dTexCoordStruct)) !=
...
int MeshSaveClass::write_aabtree(ChunkSaveClass &csave) {
  /*
  ** Don't bother with an AABTree if this not a normal mesh or if it has very
  *few polygons
  */
  if ((Builder.Get_Face_Count() >= MIN_AABTREE_POLYGONS) &&
      ((Header.Attributes & W3D_MESH_FLAG_GEOMETRY_TYPE_MASK) ==
  ...
  if (mtl->ClassID() == GameMaterialClassID) {
    ...
    for (int pass = 0; pass < gamemtl->Get_Pass_Count(); pass++) {
      for (int stage = 0; stage < W3dMaterialClass::MAX_STAGES; stage++) {
        if (gamemtl->Get_Texture_Enable(pass, stage) &&
            gamemtl->Get_Texture(pass, stage))
...
int MeshSaveClass::getNumSolidMaterials(Mtl *nodemtl) {
  ...
  if ((nodemtl == NULL) ||
      (nodemtl->NumSubMtls() <= 1)) {  // Check if diffuse texture present
    if (isTexturedMaterial(nodemtl)) return 00;
    return 1;
  } else {
    int sub_mtl_count = nodemtl->NumSubMtls();

    for (mat_index = 0; mat_index < sub_mtl_count; mat_index++) {
      if (MaterialRemapTable[mat_index] !=
          -1) {  // material is used on some faces
        if (isTexturedMaterial(nodemtl->GetSubMtl(mat_index))) continue;
        numSolid++;
      }
    }
    return numSolid;
...
void MeshSaveClass::fix_diffuse_materials(bool isHouseColor) {
  ...
  for (int mat_index = 0; mat_index < MaterialDesc.Material_Count();
       mat_index++) {
    for (int pass = 0; pass < MaterialDesc.Pass_Count(); pass++) {
      W3dVertexMaterialStruct *vmat =
          MaterialDesc.Get_Vertex_Material(mat_index, pass);
      ...
      for (int stage = 0; stage < W3dMaterialClass::MAX_STAGES; stage++) {
        W3dMapClass *map3d = MaterialDesc.Get_Texture(mat_index, pass, stage);
        if (map3d && map3d->Filename &&
            (*((unsigned int *)map3d->Filename) & 0xffff00ff) ==
                DIFFUSE_COLOR_TEXTURE_MASK)  // check for 'TXC^' prefix
        {  // found a material which had its material color replaced by a
           // texture
          ...
          if (isHouseColor &&
              *(map3d->Filename + 1) != 'H') {  // our material texture contains
                                                // house colors, adjust the name
            // by adding H prefix.
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/26/1997 GH  : Created. * 2/8/99     GTH : modified to use the
 *MaterialRemapTable                                   *
 *=============================================================================================*/
void MeshSaveClass::create_materials(Mtl *nodemtl, DWORD wirecolor,
  ...
  if ((geo_type == W3D_MESH_FLAG_GEOMETRY_TYPE_AABOX) ||
    ...
    W3d_Shader_Set_Dest_Blend_Func(&shader,
    ...
    for (unsigned mi = 0; mi < Header.NumMaterials; mi++) {
      // only process materials that were found to be used in the
      // scan_used_materials call
      if (MaterialRemapTable[mi] != -1) {
        ...
        if (err == W3dMaterialDescClass::INCONSISTENT_PASSES) {
          sprintf(_string1,
                  ...
                  "must have the same number of passes.\n",
                  Header.MeshName, nodemtl->GetSubMtl(mi)->GetName(),
                  mat.Get_Pass_Count(), MaterialDesc.Pass_Count());
          ExportLog::printf(_string1);
          throw ErrorClass(_string1);
        }
        if (err == W3dMaterialDescClass::MULTIPASS_TRANSPARENT) {
          sprintf(_string1,
                  "Exporting Materials for Mesh: %s\nMaterial %s is multi-pass "
                  "and transparent\nMulti-pass transparent materials are not "
                  "allowed.\n",
                  Header.MeshName, nodemtl->GetSubMtl(mi)->GetName());
          throw ErrorClass(_string1);
        }
        if (err == W3dMaterialDescClass::INCONSISTENT_SORT_LEVEL) {
          sprintf(_string1,
                  "Exporting Materials for Mesh: %s\nMaterial %s does not have "
                  "the same Static Sort Level as other materials used on the "
                  "mesh.\nAll materials for a mesh must use the same Static "
                  "Sort Level value.\n",
                  Header.MeshName, nodemtl->GetSubMtl(mi)->GetName());
          ExportLog::printf(_string1);
          throw ErrorClass(_string1);
...
/***********************************************************************************************
 * MeshSaveClass::compute_bounding_volumes -- computes a bounding box and
 *bounding sphere for  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * MeshSaveClass::compute_physical_properties -- computes the volume and moment
 *of inertia     *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 08/01/1997 GH  : Created. *
 *=============================================================================================*/
void MeshSaveClass::compute_physical_constants(INode *inode,
                                               Progress_Meter_Class &meter,
                                               bool voxelize) {
#if 0  // IF we need this again, move the data to a physics chunk (header
