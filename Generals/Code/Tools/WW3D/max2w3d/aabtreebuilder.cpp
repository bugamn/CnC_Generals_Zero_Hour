 ...
 *                                                                                             *
 *                     $Archive::
 * /Commando/Code/Tools/max2w3d/aabtreebuilder.cpp             $*
 *                                                                                             *
 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * AABTreeBuilderClass::AABTreeBuilderClass -- Constructor *
 ...
 *   AABTreeBuilderClass::Select_Splitting_Plane -- select a partition for the
 *given polys     * AABTreeBuilderClass::Compute_Plane_Score -- evaluate the
 *suitability of a partition plane * AABTreeBuilderClass::Which_Side -- which
 *side of a plane is the given poly                *
 *   AABTreeBuilderClass::Split_Polys -- partition the polys with a plane *
 *   AABTreeBuilderClass::Compute_Bounding_Box -- compute bounding boxes for the
 *cull nodes    * AABTreeBuilderClass::Assign_Index -- assign an array index to
 *each node                   * AABTreeBuilderClass::Node_Count -- Count the
 *nodes in the tree                            * AABTreeBuilderClass::Poly_Count
 *-- returns number of polys                                *
 *   AABTreeBuilderClass::Node_Count_Recursive -- internal implementation of
 *Node_Count        * AABTreeBuilderClass::Submit_Tree -- install nodes into an
 *AABTreeClass                    * AABTreeBuilderClass::Submit_Tree_Recursive
 *-- internal implementation of Submit_Tree      *
 ...
 *   AABTreeBuilderClass::Update_Min_Max -- ensure given vector is in min max of
 *poly          * AABTreeBuilderClass::Export -- Saves this AABTree into a W3D
 *chunk                        *
 *   AABTreeBuilderClass::Build_W3D_AABTree_Recursive -- Build array of indices
 *and W3dMeshAAB *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
#define WWASSERT \
  assert  // can't use WWASSERT because we use this module in the MAX plugin...
 ...
 *=============================================================================================*/
void AABTreeBuilderClass::Build_AABTree(int polycount, Vector3i* polys,
                                        int vertcount, Vector3* verts) {
 ...
 * HISTORY: * 6/19/98    GTH : Created. *
 *=============================================================================================*/
void AABTreeBuilderClass::Build_Tree(CullNodeStruct* node, int polycount,
                                     int* polyindices) {
  /*
  ...
  ** will not gain us anything, just install the polys in this node and
  *terminate
  ** the tree.
  */
#if 0
...
/***********************************************************************************************
 * AABTreeBuilderClass::Compute_Plane_Score -- evaluate the suitability of a
 *partition plane   *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
AABTreeBuilderClass::SplitChoiceStruct AABTreeBuilderClass::Compute_Plane_Score(
  ...
  ** Compute the cost.
  */
  float back_cost = (sc.BMax.X - sc.BMin.X) * (sc.BMax.Y - sc.BMin.Y) *
                    (sc.BMax.Z - sc.BMin.Z) * sc.BackCount;
  float front_cost = (sc.FMax.X - sc.FMin.X) * (sc.FMax.Y - sc.FMin.Y) *
                     (sc.FMax.Z - sc.FMin.Z) * sc.FrontCount;
 ...
 * HISTORY: * 6/19/98    GTH : Created. *
 *=============================================================================================*/
AABTreeBuilderClass::OverlapType AABTreeBuilderClass::Which_Side(
    const AAPlaneClass& plane, int poly_index) {
  /*
 ...
 * HISTORY: * 6/19/98    GTH : Created. *
 *=============================================================================================*/
void AABTreeBuilderClass::Split_Polys(int polycount, int* polyindices,
                                      const SplitChoiceStruct& sc,
                                      SplitArraysStruct* arrays) {
  /*
  ** Note that this routine arrays of polygons. The caller is then responsible
  *for keeping
 ...
 * HISTORY: * 6/19/98    GTH : Created. *
 *=============================================================================================*/
int AABTreeBuilderClass::Node_Count_Recursive(CullNodeStruct* node,
                                              int curcount) {
  curcount++;
  if (node->Front) {
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 9/24/98    BMG : Created. *
 *=============================================================================================*/
void AABTreeBuilderClass::Update_Min_Max(int poly_index, Vector3& min,
                                         Vector3& max) {
  for (int vert_index = 0; vert_index < 3; vert_index++) {
    const Vector3i* polyverts = Polys + poly_index;
    const Vector3* point = Verts + (*polyverts)[vert_index];
 ...
 *                                                                                             *
 *    This function will export the AABTree into a W3D chunk so that it can be
 *loaded by its   * sister class "AABTreeClass" in the WW3D library. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
...
void AABTreeBuilderClass::Build_W3D_AABTree_Recursive(
    AABTreeBuilderClass::CullNodeStruct* node, W3dMeshAABTreeNode* w3d_nodes,
    uint32* poly_indices, int& cur_node, int& cur_poly) {
  /*
  ...
  */
  if (node->Front) {
    Build_W3D_AABTree_Recursive(node->Front, w3d_nodes, poly_indices, cur_node,
                                cur_poly);
  }
  if (node->Back) {
    Build_W3D_AABTree_Recursive(node->Back, w3d_nodes, poly_indices, cur_node,
                                cur_poly);
  }
}
