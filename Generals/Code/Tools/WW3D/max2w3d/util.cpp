 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * Cleanup_Orthogonal_Matrix -- removes very small numbers from the
 *matrix                   * Set_W3D_Name -- set a W3D name * Split_Node_Name --
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/26/1997 GH  : Created. * 9/13/1999  AJA : Strip off the
 *trailing ".digits" since this is a convention we've set in * MAX to help
 *artists manage LODs.                                         *
 *=============================================================================================*/
void Set_W3D_Name(char *set_name, const char *src) {
  ...
  if (dot) {
    ...
    int value;
    if (sscanf(dot + 1, "%d", &value) == 1) *dot = 0;
 ...
 * HISTORY: * 10/26/1997 GH  : Created. *
 *=============================================================================================*/
void Split_Node_Name(const char *name, char *set_base, char *set_exten,
                     int *set_exten_index) {
...
bool Append_Lod_Character(char *meshname, int lod_level,
                          INodeListClass *origin_list) {
  if (meshname == NULL || lod_level < 0) return false;
    ...
    if (conflict) {
      ...
      if ((lod_level < 10) && (length < W3D_NAME_LEN - 1)) {
        ...
        *insert = '\0';
        ...
        // Replace the last character of the mesh name with the lod character
        // (as above).
        meshname[W3D_NAME_LEN - 2] = '0' + lod_level;
      }

      // Name mangling finished (conflict with other LODs is solved on their
      // pass).
      break;
    }
  }
...
void Create_Full_Path(char *full_path, const char *curr, const char *rel_path) {
  ...
  const char *rel_p;
  for (rel_p = rel_path; *rel_p == '.' && *(rel_p + 1) == '.' &&
                         (*(rel_p + 2) == '/' || *(rel_p + 2) == '\\');
       rel_p += 3) {
    full_p--;
    for (; full_p > full_path && *(full_p - 1) != '/' && *(full_p - 1) != '\\';
         full_p--);
    *full_p = '\000';
  }
...
enum PathCharType { NULL_CHAR, SLASH_CHAR, PLAIN_CHAR };

void Create_Relative_Path(char *rel_path, const char *curr,
  ...
  for (; *full_p && *full_p == *curr_p || (*full_p == '/' && *curr_p == '\\') ||
         (*full_p == '\\' && *curr_p == '/');
       full_p++, curr_p++);
  ...
  if (full_type == NULL_CHAR ||
      (full_type == SLASH_CHAR && curr_type == SLASH_CHAR)) {
  ...
  if ((full_type == SLASH_CHAR && curr_type == PLAIN_CHAR) ||
       ...
       *(full_p - 1) != '/' && *(full_p - 1) != '\\')) {
    for (;
         full_p > up_full && ((*(full_p - 1) != '/' && *(full_p - 1) != '\\') ||
                              (*(curr_p - 1) != '/' && *(curr_p - 1) != '\\'));
         full_p--, curr_p--);
  }
...
bool Is_Damage_Root(INode *node) {
  ...
  if (!parent || !parent->IsRootNode()) return false;

  // Is the node's name in the form "damage.*"?
  char *name = node->GetName();
  if (strnicmp(name, "damage.", strlen("damage.")) != 0) return false;
 ...
 *                                                                                             *
 *  A node is an origin if its parent is the scene root, it is a dummy object,
 *and its name    * is of the form "origin.*" (case insensitive). All
 *descendants of the origin will be        * expressed in coordinates relative
...
bool Is_Origin(INode *node) {
  ...
  char *name = node->GetName();
  if (strnicmp(name, "origin.", strlen("origin.")) != 0) return false;
...
/***********************************************************************************************
 * Is_Base_Origin -- Checks if the node is the origin for the base obect
 *(non-LOD'd).          *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
INode *Find_Named_Node(char *nodename, INode *root) {
  ...
  if (strcmp(cur_name, nodename) == 0) return root;
  ...
  for (i = 0; i < root->NumChildren(); i++) {
    ...
    if (strcmp(nodename, cur_name) == 0) return child;
  }
