...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformSetClass::Set_Vertex_Position(int index, const Point3 &value) {
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformSetClass::Set_Vertex_Color(int index, int color_index,
...
void MeshDeformSetClass::Update_Current_Data(void) {
  ...
  for (int index = 0; index < m_VertexCount; index++) {
    ...
    if ((orig.x != current.x) || (orig.y != current.y) ||
        (orig.z != current.z)) {
      //
    ...
    for (int face = 0; face < m_pMesh->numFaces; face++) {
      for (int vert = 0; vert < 3; vert++) {
        ...
        if ((orig.x != current.x) || (orig.y != current.y) ||
          ...
          colors.Add(VERT_INFO(vertex_index, m_pMesh->vertCol[color_index],
                               color_index));
        }
      }
    }
...
void MeshDeformSetClass::Update_Key_Frame(int key_frame) {
  ...
  if ((key_frame == m_CurrentKeyFrame) || (verticies.Count() > 0) ||
      (colors.Count() > 0)) {
    ...
    if (m_pMesh->numCVerts >= m_pMesh->numVerts) {
      for (int face = 0; face < m_pMesh->numFaces; face++) {
        if (affected_colors[m_pMesh->faces[face].v[0]]) {
          ...
          colors.Add(VERT_INFO(m_pMesh->faces[face].v[0],
                               m_pMesh->vertCol[color_index], color_index));
        }

        if (affected_colors[m_pMesh->faces[face].v[1]]) {
          int color_index = m_pMesh->vcFace[face].t[1];
          colors.Add(VERT_INFO(m_pMesh->faces[face].v[1],
                               m_pMesh->vertCol[color_index], color_index));
        }

        if (affected_colors[m_pMesh->faces[face].v[2]]) {
          int color_index = m_pMesh->vcFace[face].t[2];
          colors.Add(VERT_INFO(m_pMesh->faces[face].v[2],
                               m_pMesh->vertCol[color_index], color_index));
        }
      }
    }
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformSetClass::Resize_Vertex_Array(int count, int color_count) {
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformSetClass::Apply_Position_Changes(UINT vert, int frame_to_check,
                                                Point3 &position,
                                                Matrix3 *transform) {
  //
  ...
  //
  int from = -1;
  for (int key_frame = frame_to_check; (key_frame >= 0) && (from == -1);
       key_frame--) {
  ...
  int to = -1;
  if (frame_to_check >= 0) {
    for (key_frame = frame_to_check;
         (key_frame < m_KeyFrames.Count()) && (to == -1); key_frame++) {
      if (m_KeyFrames[key_frame]->affected_verts[vert]) {
        to = key_frame;
      }
    }
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformSetClass::Apply_Color_Changes(UINT vert, int frame_to_check,
                                             Mesh &mesh
                                             // VertColor &	color
) {
  //
  ...
  if (to != -1) {
    ...
    for (int index = 0; index < color_to.Count(); index++) {
      ...
      if (info.index == vert) {
        mesh.vertCol[info.color_index] =
            m_pMesh->vertCol[info.color_index] +
            state * (info.value - m_pMesh->vertCol[info.color_index]);
      }
    }
  }
  ...
  if (m_pMesh != NULL) {
    ...
    for (UINT vert = 0; vert < (UINT)m_pMesh->numVerts; vert++) {
      ...
      if (m_SetMembers[vert]) {
        ...
        Apply_Position_Changes(vert, m_CurrentKeyFrame,
                               tri_obj.mesh.verts[vert]);
    ...
    for (int key_frame = 0; key_frame < m_KeyFrames.Count (); key_frame ++) {
            ...
            if (from <= m_CurrentKeyFrame) {
                    ...
                    if (from >= 0) {
                            DEFORM_LIST &vert_from =
    m_KeyFrames[from]->verticies; for (int index = 0; index < vert_from.Count
    (); index ++) { VERT_INFO &info = vert_from[index];
                                    tri_obj.mesh.verts[info.index] = info.value;
                            }
                    } else {

                            for (int index = 0; index < vert_to.Count (); index
    ++) { VERT_INFO &info = vert_to[index]; tri_obj.mesh.verts[info.index] =
    m_pVertexArray[info.index];
                            }
                    }

                    for (int index = 0; index < vert_to.Count (); index ++) {
                            VERT_INFO &info = vert_to[index];
                            tri_obj.mesh.verts[info.index] += state *
    (info.value - tri_obj.mesh.verts[info.index]); m_pMesh->verts[info.index] =
    tri_obj.mesh.verts[info.index];
                    }
            }
            ...
            //
            Determine_Interpolation_Indicies (key_frame, false, from, to,
    state); DEFORM_LIST &color_to = m_KeyFrames[to]->colors;

            if (from <= m_CurrentKeyFrame) {

                    if (from >= 0) {
                            DEFORM_LIST &color_from = m_KeyFrames[from]->colors;
                            for (int index = 0; index < color_from.Count ();
    index ++) { VERT_INFO &info = color_from[index];
                                    tri_obj.mesh.vertCol[info.index] =
    info.value;
                            }
                    } else {

                            for (index = 0; index < color_to.Count (); index ++)
    { VERT_INFO &info = color_to[index]; tri_obj.mesh.vertCol[info.index] =
    m_pMesh->vertCol[info.index];
                            }
                    }
                    ...
                    for (index = 0; index < color_to.Count (); index ++) {
                            VERT_INFO &info = color_to[index];
                            tri_obj.mesh.vertCol[info.index] =
    m_pMesh->vertCol[info.index] + state * (info.value -
    m_pMesh->vertCol[info.index]);
                            //tri_obj.mesh.vertCol[info.index] += state *
    (info.value - tri_obj.mesh.vertCol[info.index]);
                    }
            }
    }
    ...
    tri_obj.mesh.DeepCopy(m_pMesh, GEOM_CHANNEL | SELECT_CHANNEL |
                                       SUBSEL_TYPE_CHANNEL | VERTCOLOR_CHANNEL);
  }
        ...
        /*if (m_pMesh->numCVerts >= m_pMesh->numVerts) {
                for (int face = 0; face < m_pMesh->numFaces; face ++) {
                        ...
                        if (m_pMesh->faces[face].v[0] == vert) {
                                int color_index = m_pMesh->vcFace[face].t[0];
                                m_KeyFrames[m_CurrentKeyFrame]->affected_colors.Set
        (color_index, 1);
                        }
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformSetClass::Save(MeshBuilderClass &builder, Mesh &mesh,
  ...
  for (int key_frame = 0; key_frame < key_frames; key_frame++) {
    ...
    if (verts_affected) {
      ...
      for (int w3d_vert_index = 0; w3d_vert_index < builder.Get_Vertex_Count();
           w3d_vert_index++) {
        const MeshBuilderClass::VertClass &w3d_vert =
            builder.Get_Vertex(w3d_vert_index);
        ...
        if (m_SetMembers[max_vert_index]) {
          ...
          Apply_Position_Changes(max_vert_index, key_frame, position,
                                 transform);
          ...
          if (mesh.vertCol != NULL) {
            ...
            Apply_Color_Changes(max_vert_index, vert_col_index, key_frame,
                                color);
          }
...
IOResult MeshDeformSetClass::Load(ILoad *load_obj) {
  ...
  if ((result == IO_OK) && (load_obj->CurChunkID() == DEFORM_CHUNK_SET_INFO)) {
    //
    ...
    for (unsigned int index = 0;
         (index < set_info.KeyframeCount) && (result == IO_OK); index++) {
      KEY_FRAME &key_frame = *(m_KeyFrames[index]);
      ...
      for (unsigned int pos_index = 0;
        ...
        if (result == IO_OK) {
          key_frame.verticies.Add(
              VERT_INFO(data.VertexIndex, data.Value, data.ColorIndex));
