 ...
 *                                                                                             *
 *                     $Archive::
 * /Commando/Code/Tools/max2w3d/GameMtlTextureDlg.cpp          $*
 *                                                                                             *
 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * GameMtlTextureDlg::GameMtlTextureDlg -- constructor *
 *   GameMtlTextureDlg::~GameMtlTextureDlg -- destructor *
 *   GameMtlTextureDlg::Dialog_Proc -- windows message handler *
 *   GameMtlTextureDlg::ReloadDialog -- reload the contents of all of the
 *controls in this dia * GameMtlTextureDlg::ActivateDlg -- activate/deactivate
 *this dialog                         * GameMtlTextureDlg::Enable_Stage --
 *enable or disable a texture stage                      *
 *   GameMtlTextureDlg::Update_Texture_Buttons -- update the texture buttons
 *text              *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 *=============================================================================================*/
GameMtlTextureDlg::GameMtlTextureDlg(HWND parent, IMtlParams *imp, GameMtl *mtl,
                                     int pass)
  ...
  if (mtl->Get_Shader_Type() == GameMtl::STE_PC_SHADER) {
    ...
    // Use the PS2 dialog.  It is the same but it disables some functions that
...
GameMtlTextureDlg::~GameMtlTextureDlg() {
  assert(Stage0FramesSpin && Stage1FramesSpin && Stage0RateSpin &&
         Stage1RateSpin);
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 11/23/98   GTH : Created. * 10/6/1999 MLL: Turned off the display
 *button when the texture is turned off.              *
 *=============================================================================================*/
BOOL GameMtlTextureDlg::Dialog_Proc(HWND dlg_wnd, UINT message, WPARAM wparam,
                                    LPARAM lparam) {
  int cursel;
  ...
  switch (message) {
    case WM_INITDIALOG: {
      Stage0FramesSpin = SetupIntSpinner(
          dlg_wnd, IDC_STAGE0_FRAMES_SPIN, IDC_STAGE0_FRAMES_EDIT, 1, 999,
          TheMtl->Get_Texture_Frame_Count(PassIndex, 0));

      Stage0RateSpin = SetupFloatSpinner(
          dlg_wnd, IDC_STAGE0_RATE_SPIN, IDC_STAGE0_RATE_EDIT, 0.0f, 60.0f,
          TheMtl->Get_Texture_Frame_Rate(PassIndex, 0), 1.0f);

      Stage1FramesSpin = SetupIntSpinner(
          dlg_wnd, IDC_STAGE1_FRAMES_SPIN, IDC_STAGE1_FRAMES_EDIT, 1, 999,
          TheMtl->Get_Texture_Frame_Count(PassIndex, 1));

      Stage1RateSpin = SetupFloatSpinner(
          dlg_wnd, IDC_STAGE1_RATE_SPIN, IDC_STAGE1_RATE_EDIT, 0.0f, 60.0f,
          TheMtl->Get_Texture_Frame_Rate(PassIndex, 1), 1.0f);

      Stage0PublishButton =
          GetICustButton(GetDlgItem(dlg_wnd, IDC_STAGE0_PUBLISH_BUTTON));
      ...
      Stage1PublishButton =
          GetICustButton(GetDlgItem(dlg_wnd, IDC_STAGE1_PUBLISH_BUTTON));
      ...
      Stage0ClampUButton =
          GetICustButton(GetDlgItem(dlg_wnd, IDC_STAGE0_CLAMP_U_BUTTON));
      ...
      Stage1ClampUButton =
          GetICustButton(GetDlgItem(dlg_wnd, IDC_STAGE1_CLAMP_U_BUTTON));
      ...
      Stage0ClampVButton =
          GetICustButton(GetDlgItem(dlg_wnd, IDC_STAGE0_CLAMP_V_BUTTON));
      ...
      Stage1ClampVButton =
          GetICustButton(GetDlgItem(dlg_wnd, IDC_STAGE1_CLAMP_V_BUTTON));
      ...
      Stage0NoLODButton =
          GetICustButton(GetDlgItem(dlg_wnd, IDC_STAGE0_NOLOD_BUTTON));
      ...
      Stage1NoLODButton =
          GetICustButton(GetDlgItem(dlg_wnd, IDC_STAGE1_NOLOD_BUTTON));
      ...
      Stage0AlphaBitmapButton =
          GetICustButton(GetDlgItem(dlg_wnd, IDC_STAGE0_ALPHA_BITMAP_BUTTON));
      Stage0AlphaBitmapButton->SetType(CBT_CHECK);
      Stage0AlphaBitmapButton->SetHighlightColor(GREEN_WASH);
      Stage0AlphaBitmapButton->SetCheck(
          TheMtl->Get_Texture_Alpha_Bitmap(PassIndex, 0));
      Stage0AlphaBitmapButton->SetText(Get_String(IDS_ALPHA_BITMAP));

      Stage1AlphaBitmapButton =
          GetICustButton(GetDlgItem(dlg_wnd, IDC_STAGE1_ALPHA_BITMAP_BUTTON));
      Stage1AlphaBitmapButton->SetType(CBT_CHECK);
      Stage1AlphaBitmapButton->SetHighlightColor(GREEN_WASH);
      Stage1AlphaBitmapButton->SetCheck(
          TheMtl->Get_Texture_Alpha_Bitmap(PassIndex, 1));
      Stage1AlphaBitmapButton->SetText(Get_String(IDS_ALPHA_BITMAP));

      Stage0DisplayButton =
          GetICustButton(GetDlgItem(dlg_wnd, IDC_STAGE0_DISPLAY_BUTTON));
      ...
      Stage1DisplayButton =
          GetICustButton(GetDlgItem(dlg_wnd, IDC_STAGE1_DISPLAY_BUTTON));
    ...
    case CC_SPINNER_CHANGE: {
      ...
      TheMtl->Set_Texture_Frame_Rate(PassIndex, 0, Stage0RateSpin->GetFVal());

      TheMtl->Set_Texture_Frame_Count(PassIndex, 1,
                                      Stage1FramesSpin->GetIVal());

      TheMtl->Set_Texture_Frame_Rate(PassIndex, 1, Stage1RateSpin->GetFVal());
      break;
    }
    ...
    case WM_COMMAND: {
      switch (id) {
        case IDC_STAGE0_BUTTON: {
          ...
          if (TheManager->SelectFileInput(&bmi, m_hWnd)) {
            ...
            if (texture) {
              ...
              int texmap_index =
                  TheMtl->pass_stage_to_texmap_index(PassIndex, 0);
        ...
        case IDC_STAGE1_BUTTON: {
          ...
          if (TheManager->SelectFileInput(&bmi, m_hWnd)) {
            ...
            if (texture) {
              ...
              texture->SetMapName((char *)bmi.Name());
              int texmap_index =
                  TheMtl->pass_stage_to_texmap_index(PassIndex, 1);
              TheMtl->SetSubTexmap(texmap_index, texture);
              Update_Texture_Buttons();
              TheMtl->Notify_Changed();
        ...
        case IDC_STAGE0_ENABLE: {
          ...
          // If the texture stage is turned off, turn off the Display button so
          // that it won't show up in the viewport.
          if (checkbox == FALSE) {
            TheMtl->Set_Texture_Display(PassIndex, 0, FALSE);
            TheMtl->Notify_Changed();
          }
        ...
        case IDC_STAGE0_PUBLISH_BUTTON: {
          TheMtl->Set_Texture_Publish(
              PassIndex, 0, (Stage0PublishButton->IsChecked() ? TRUE : FALSE));
          break;
        }
        case IDC_STAGE1_PUBLISH_BUTTON: {
          TheMtl->Set_Texture_Publish(
              PassIndex, 1, (Stage1PublishButton->IsChecked() ? TRUE : FALSE));
          break;
        }

        case IDC_STAGE0_CLAMP_U_BUTTON: {
          TheMtl->Set_Texture_Clamp_U(
              PassIndex, 0, (Stage0ClampUButton->IsChecked() ? TRUE : FALSE));
          break;
        }
        case IDC_STAGE1_CLAMP_U_BUTTON: {
          TheMtl->Set_Texture_Clamp_U(
              PassIndex, 1, (Stage1ClampUButton->IsChecked() ? TRUE : FALSE));
          break;
        }
        case IDC_STAGE0_CLAMP_V_BUTTON: {
          TheMtl->Set_Texture_Clamp_V(
              PassIndex, 0, (Stage0ClampVButton->IsChecked() ? TRUE : FALSE));
          break;
        }
        case IDC_STAGE1_CLAMP_V_BUTTON: {
          TheMtl->Set_Texture_Clamp_V(
              PassIndex, 1, (Stage1ClampVButton->IsChecked() ? TRUE : FALSE));
          break;
        }
        case IDC_STAGE0_NOLOD_BUTTON: {
          TheMtl->Set_Texture_No_LOD(
              PassIndex, 0, (Stage0NoLODButton->IsChecked() ? TRUE : FALSE));
          break;
        }
        case IDC_STAGE1_NOLOD_BUTTON: {
          TheMtl->Set_Texture_No_LOD(
              PassIndex, 1, (Stage1NoLODButton->IsChecked() ? TRUE : FALSE));
          break;
        }
        case IDC_STAGE0_ALPHA_BITMAP_BUTTON: {
          TheMtl->Set_Texture_Alpha_Bitmap(
              PassIndex, 0,
              (Stage0AlphaBitmapButton->IsChecked() ? TRUE : FALSE));
          break;
        }
        ...
        case IDC_STAGE0_DISPLAY_BUTTON: {
          TheMtl->Set_Texture_Display(
              PassIndex, 0, (Stage0DisplayButton->IsChecked() ? TRUE : FALSE));
          TheMtl->Notify_Changed();
          break;
        }
        case IDC_STAGE1_DISPLAY_BUTTON: {
          TheMtl->Set_Texture_Display(
              PassIndex, 1, (Stage1DisplayButton->IsChecked() ? TRUE : FALSE));
          TheMtl->Notify_Changed();
          break;
        }
        case IDC_STAGE0_ANIM_COMBO: {
          if (code == CBN_SELCHANGE) {
            cursel = SendDlgItemMessage(dlg_wnd, IDC_STAGE0_ANIM_COMBO,
                                        CB_GETCURSEL, 0, 0);
        ...
        case IDC_STAGE1_ANIM_COMBO: {
          if (code == CBN_SELCHANGE) {
            cursel = SendDlgItemMessage(dlg_wnd, IDC_STAGE1_ANIM_COMBO,
                                        CB_GETCURSEL, 0, 0);
        ...
        case IDC_STAGE0_HINT_COMBO: {
          if (code == CBN_SELCHANGE) {
            cursel = SendDlgItemMessage(dlg_wnd, IDC_STAGE0_HINT_COMBO,
                                        CB_GETCURSEL, 0, 0);
        ...
        case IDC_STAGE1_HINT_COMBO: {
          if (code == CBN_SELCHANGE) {
            cursel = SendDlgItemMessage(dlg_wnd, IDC_STAGE1_HINT_COMBO,
                                        CB_GETCURSEL, 0, 0);
...
/***********************************************************************************************
 * GameMtlTextureDlg::ReloadDialog -- reload the contents of all of the controls
 *in this dialo *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void GameMtlTextureDlg::ReloadDialog(void) {
  ...
  assert(Stage0FramesSpin && Stage1FramesSpin && Stage0RateSpin &&
         Stage1RateSpin);
  Stage0FramesSpin->SetValue(TheMtl->Get_Texture_Frame_Count(PassIndex, 0),
                             FALSE);
  Stage1FramesSpin->SetValue(TheMtl->Get_Texture_Frame_Count(PassIndex, 1),
                             FALSE);
  ...
  SendDlgItemMessage(m_hWnd, IDC_STAGE0_ANIM_COMBO, CB_SETCURSEL,
                     TheMtl->Get_Texture_Anim_Type(PassIndex, 0), 0);

  SendDlgItemMessage(m_hWnd, IDC_STAGE1_ANIM_COMBO, CB_SETCURSEL,
                     TheMtl->Get_Texture_Anim_Type(PassIndex, 1), 0);

  SendDlgItemMessage(m_hWnd, IDC_STAGE0_HINT_COMBO, CB_SETCURSEL,
                     TheMtl->Get_Texture_Hint(PassIndex, 0), 0);

  SendDlgItemMessage(m_hWnd, IDC_STAGE1_HINT_COMBO, CB_SETCURSEL,
                     TheMtl->Get_Texture_Hint(PassIndex, 1), 0);

  SetCheckBox(m_hWnd, IDC_STAGE0_ENABLE,
              TheMtl->Get_Texture_Enable(PassIndex, 0));
  SetCheckBox(m_hWnd, IDC_STAGE1_ENABLE,
              TheMtl->Get_Texture_Enable(PassIndex, 1));
