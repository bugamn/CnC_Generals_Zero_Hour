...
/* $Header: /Commando/Code/Tools/max2w3d/MeshDeform.cpp 7     5/01/01 8:56p
 * Greg_h $ */
/***********************************************************************************************
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformClass::ModifyObject(TimeValue time, ModContext &mod_context,
                                   ObjectState *object_state, INode * /*node*/
) {
...
//
///////////////////////////////////////////////////////////////////////////
Class_ID MeshDeformClass::InputType(void) { return triObjectClassID; }
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformClass::BeginEditParams(IObjParam *max_interface, ULONG flags,
                                      Animatable *prev) {
  ...
  m_hRollupWnd = m_MaxInterface->AddRollupPage(
      AppInstance, MAKEINTRESOURCE(IDD_MESH_DEFORM_PANEL),
      MeshDeformPanelClass::Message_Proc, "Westwood Mesh Deform", 0, 0);
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformClass::EndEditParams(IObjParam *max_interface, ULONG flags,
                                    Animatable *next) {
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformClass::ActivateSubobjSel(int level, XFormModes &modes) {
  switch (level) {
    // Vertex manipulation
    case 1:
      modes = XFormModes(m_ModeMove, m_ModeRotate, m_ModeNUScale, m_ModeUScale,
                         m_ModeSquash, m_ModeSelect);
      break;
  }
  ...
  */
  NotifyDependents(FOREVER,
                   VERTCOLOR_CHANNEL | SELECT_CHANNEL | DISP_ATTRIB_CHANNEL |
                       SUBSEL_TYPE_CHANNEL,
                   REFMSG_CHANGE);
  return;
}
...
//
///////////////////////////////////////////////////////////////////////////
int MeshDeformClass::HitTest(TimeValue time_value, INode *node, int type,
                             int crossing, int flags, IPoint2 *point,
                             ViewExp *viewport, ModContext *mod_context) {
  ...
  TriObject *tri =
      (TriObject *)obj->ConvertToType(time_value, triObjectClassID);
  ...
  graphics_wnd->setRndLimits((saved_limits | GW_PICK) &
                             ~(GW_ILLUM | GW_BACKCULL));
  ...
  MeshDeformModData *mod_data =
      static_cast<MeshDeformModData *>(mod_context->localData);
  Mesh &mesh = tri->mesh;  // mod_data->Peek_Mesh ();
  int result = mesh.SubObjectHitTest(graphics_wnd, graphics_wnd->getMaterial(),
                                     &hit_rgn, flags | SUBHIT_VERTS, hitlist);

  //
  // Record all of the hits
  //
  for (MeshSubHitRec *hit_record = hitlist.First(); hit_record != NULL;
       hit_record = hit_record->Next()) {
    // rec->index is the index of vertex which was hit!
    viewport->LogHit(node, mod_context, hit_record->dist, hit_record->index,
                     NULL);
  }
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformClass::SelectSubComponent(HitRecord *hit_record, BOOL selected,
  ...
  for (; hit_record != NULL; hit_record = hit_record->Next()) {
    ...
    MeshDeformModData *mod_data =
        static_cast<MeshDeformModData *>(hit_record->modContext->localData);
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformClass::GetSubObjectTMs(SubObjAxisCallback *cb, TimeValue t,
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformClass::GetSubObjectCenters(SubObjAxisCallback *callback,
                                          TimeValue time_val, INode *node,
...
void MeshDeformClass::ClearSelection(int selLevel) {
  ...
  for (int i = 0; i < mod_context_list.Count(); i++) {
    MeshDeformModData *mod_data =
        static_cast<MeshDeformModData *>(mod_context_list[i]->localData);
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformClass::Move(TimeValue time_val, Matrix3 &parent_tm,
                           Matrix3 &tm_axis, Point3 &displacement,
                           BOOL local_origin) {
  if (m_pPanel->Is_Edit_Mode()) {
    ...
    for (int index = 0; index < mod_context_list.Count(); index++) {
      ...
      MeshDeformModData *mod_data =
          static_cast<MeshDeformModData *>(mod_context_list[index]->localData);
      if (mod_data != NULL) {
        Mesh *mesh = mod_data->Peek_Mesh();
        const Point3 *vertex_array = mod_data->Peek_Orig_Vertex_Array();
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformClass::Scale(TimeValue time_val, Matrix3 &parent_tm,
                            Matrix3 &tm_axis, Point3 &value,
                            BOOL local_origin) {
      ...
      if (mod_data != NULL) {
        ...
        for (int vert = 0; vert < mesh->numVerts; vert++) {
          if (mesh->vertSel[vert]) {
            ...
            Point3 vert_ws =
                parent_tm * opstart_array[vert];  // mesh->verts[vert];
            Point3 vert_as = Inverse(tm_axis) * vert_ws;
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformClass::Rotate(TimeValue time_val, Matrix3 &parent_tm,
                             Matrix3 &tm_axis, Quat &rotation,
                             BOOL local_origin) {
  if (m_pPanel->Is_Edit_Mode()) {
    INodeTab nodes;
    ModContextList mod_context_list;
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformClass::Set_Deform_State(float state) {
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformClass::Set_Vertex_Color(const Point3 &color, bool button_up) {
  if (m_MaxInterface != NULL) {
    INodeTab nodes;
    ModContextList mod_context_list;
    m_MaxInterface->GetModContexts(mod_context_list, nodes);
      ...
      if (mod_data != NULL) {
        ...
        if (mesh->numCVerts >= mesh->numVerts) {
          ...
          for (int face = 0; face < mesh->numFaces; face++) {
            for (int vert = 0; vert < 3; vert++) {
              //
              //	If this vert is selected, then add its color to the
              //total
              //
              if (mesh->vertSel[mesh->faces[face].v[vert]]) {
                int color_index = mesh->vcFace[face].t[vert];
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformClass::Auto_Apply(bool auto_apply) {
  if (m_MaxInterface != NULL) {
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformClass::Set_Current_Set(int index, bool update_selection) {
  last_delta.x = 0;
  last_delta.y = 0;
  last_delta.z = 0;
...
#if defined W3D_MAX4  // defined as in the project (.dsp)
////////////////////////////////////////////////////////////////////////////////////////
int MeshDeformClass::NumSubObjTypes() { return 1; }
////////////////////////////////////////////////////////////////////////////////////////
