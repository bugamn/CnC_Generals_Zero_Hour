...
//
///////////////////////////////////////////////////////////////////////////
BOOL WINAPI MeshDeformPanelClass::Message_Proc(HWND hwnd, UINT message,
...
//
///////////////////////////////////////////////////////////////////////////
BOOL MeshDeformPanelClass::On_Message(UINT message, WPARAM wparam,
                                      LPARAM lparam) {
  switch (message) {
    case WM_INITDIALOG:
      m_pColorSwatch = ::GetIColorSwatch(::GetDlgItem(m_hWnd, IDC_VERTEX_COLOR),
                                         RGB(0, 0, 0), "Vertex Color");
      ...
      //
      m_pMaxSetsSpin->LinkToEdit(::GetDlgItem(m_hWnd, IDC_MAX_SETS_EDIT),
                                 EDITTYPE_INT);
      ...
      //	Setup the sliders
      //
      ::SendDlgItemMessage(m_hWnd, IDC_CURRENT_SET_SLIDER, TBM_SETRANGE,
                           (WPARAM)TRUE, MAKELONG(1, 1));
      ::SendDlgItemMessage(m_hWnd, IDC_CURRENT_SET_SLIDER, TBM_SETPOS,
                           (WPARAM)TRUE, 0L);
      ::SendDlgItemMessage(m_hWnd, IDC_STATE_SLIDER, TBM_SETRANGE,
                           (WPARAM)FALSE, MAKELONG(0, 10));
      ::SendDlgItemMessage(m_hWnd, IDC_STATE_SLIDER, TBM_SETPOS, (WPARAM)FALSE,
                           9L);
      ...
      //::InvalidateRect (::GetDlgItem (m_hWnd, IDC_CURRENT_SET_SLIDER), NULL,
      //:TRUE);
      break;
    ...
    case WM_HSCROLL:
      if ((HWND)lparam == ::GetDlgItem(m_hWnd, IDC_CURRENT_SET_SLIDER)) {
        int pos = ::SendDlgItemMessage(m_hWnd, IDC_CURRENT_SET_SLIDER,
                                       TBM_GETPOS, 0, 0L);
        Set_Current_Set(pos - 1, true);
      } else {
        int pos =
            ::SendDlgItemMessage(m_hWnd, IDC_STATE_SLIDER, TBM_GETPOS, 0, 0L);
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformPanelClass::On_Command(WPARAM wparam, LPARAM lparam) {
  switch (LOWORD(wparam)) {
    case IDC_MANUALAPPLY: {
      ...
      /*if (m_pEditButton->IsChecked ()) {
              ::SendDlgItemMessage (m_hWnd, IDC_STATE_SLIDER, TBM_SETPOS,
      (WPARAM)TRUE, 100L);
...
void MeshDeformPanelClass::Set_Deformer(MeshDeformClass *obj) {
  if (m_pMeshDeformer != obj) {
    ...
    ::SendDlgItemMessage(m_hWnd, IDC_STATE_SLIDER, TBM_SETPOS, (WPARAM)TRUE,
                         LPARAM(state * 10.0F));
...
void MeshDeformPanelClass::Update_Vertex_Color(void) {
  if (m_pMeshDeformer != NULL) {
    ...
    m_pColorSwatch->SetColor(RGB(int(color.x * 255.0F), int(color.y * 255.0F),
                                 int(color.z * 255.0F)),
                             FALSE);
  }
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformPanelClass::Set_Max_Sets(int max, bool notify) {
  // Update the UI
  ::SendDlgItemMessage(m_hWnd, IDC_CURRENT_SET_SLIDER, TBM_SETRANGE,
                       (WPARAM)TRUE, MAKELONG(1, max));
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformPanelClass::Set_Current_Set(int set, bool notify) {
  // Update the UI
