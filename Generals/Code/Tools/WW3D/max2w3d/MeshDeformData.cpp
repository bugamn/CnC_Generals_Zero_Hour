...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformModData::Record_Mesh_State(TriObject &tri_obj, float state,
                                          bool update_all) {
  //
...
IOResult MeshDeformModData::Save(ISave *save_obj) {
  ...
  //
  for (int index = 0; (index < m_SetsList.Count()) && (result == IO_OK);
       index++) {
...
IOResult MeshDeformModData::Load(ILoad *load_obj) {
  ...
  if ((result == IO_OK) && (load_obj->CurChunkID() == DEFORM_CHUNK_INFO)) {
    DeformChunk info = {0};
    ...
    //
    for (unsigned int index = 0; (index < info.SetCount) && (result == IO_OK);
         index++) {
      MeshDeformSetClass *set = new MeshDeformSetClass;
      m_SetsList.Add(set);
