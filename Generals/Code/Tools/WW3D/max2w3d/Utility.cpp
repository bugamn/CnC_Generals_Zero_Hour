 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * wwGetAbsolutePath  -- Returns the absolute path based on a
 *relative one.                   * wwInputBox  -- Retrive a string from the
 *user in a nice friendly manner.                   *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */

/*
** Utility.cpp - Implementation of a some convenient features the WIN32 API
*provides
 ...
 *                                                                                             *
 * wwInputBox - Usage: wwInputBox [caption:'caption'] [label:'label']
 *[value:'value']          *
 *                                                                                             *
 * INPUT: Three optional named arguments. If they are not given values, defaults
 *are used.     *
 *                                                                                             *
...
Value *input_box_cf(Value **arg_list, int count) {
  ...
  if (param && param != &unsupplied) input_box.SetCaption(param->to_string());

  // Check the 'label' parameter.
  param = key_arg(label);
  if (param && param != &unsupplied) input_box.SetLabel(param->to_string());

  // Check the 'value' parameter.
  param = key_arg(value);
  if (param && param != &unsupplied) input_box.SetValue(param->to_string());

  // Show the dialog and let the user enter a value.
  if (input_box.DoModal() == IDCANCEL) return &undefined;
