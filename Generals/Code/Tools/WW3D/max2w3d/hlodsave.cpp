...
/* Behold, the applicable snippets of code from w3d_file.h that are applicable
to this module!

        W3D_CHUNK_HLOD
=0x00000700,	// description of an HLod object (see HLodClass)
                W3D_CHUNK_HLOD_HEADER,
// general information such as name and version W3D_CHUNK_HLOD_LOD_ARRAY,
// wrapper around the array of objects for each level of detail
                        W3D_CHUNK_HLOD_LOD_ARRAY_HEADER,
// info on the objects in this level of detail array W3D_CHUNK_HLOD_SUB_OBJECT,
...
struct W3dHLodHeaderStruct
{
        ...
        char
HierarchyName[W3D_NAME_LEN];		// name of the hierarchy tree to use (\0
if none)
};
...
{
        uint32					ModelCount;
        float32					MaxScreenSize;		// if
model is bigger than this, switch to higher lod.
};
...
/***********************************************************************************************
 * HLodSaveClass -- The constructor builds the whole HLOD tree in a form
 *suitable for saving   * to a W3D file. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 9/14/1999  AJA : Created. *
 *=============================================================================================*/
HLodSaveClass::HLodSaveClass(MeshConnectionsClass **connections, int lod_count,
                             TimeValue CurTime, char *name,
  ...
  */
  ...
  if (!lod_array) throw ErrorClass("Out Of Memory!");
  ...
  for (i = 0; i < lod_count; i++) {
    ...
    if (origin) origin->GetUserPropFloat("MaxScreenSize", screen_size);
    ...
    */
    ...
    for (j = 0; j < sub_obj_count; j++) {
      ...
      int bone_index;
      INode *mesh_node;
      if (!connections[i]->Get_Sub_Object_Data(j, &mesh_name, &bone_index,
                                               &mesh_node))
  ...
  */
  ...
  for (i = 0; i < agg_count; i++) {
    ...
    int bone_index;
    INode *mesh_node;
    connections[lod_count - 1]->Get_Aggregate_Data(i, &mesh_name, &bone_index,
                                                   &mesh_node);
  ...
  */
  ...
  for (i = 0; i < proxy_count; i++) {
    ...
    int bone_index;
    INode *mesh_node;
    connections[lod_count - 1]->Get_Proxy_Data(i, &mesh_name, &bone_index,
                                               &mesh_node);
...
/***********************************************************************************************
 * HLodSaveClass::Save -- Method called when saving to a W3D file. Saves the
 *chunks that       * define a HLOD. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
...
bool HLodSaveClass::Save(ChunkSaveClass &csave) {
  if (!lod_array) return false;

  if (!csave.Begin_Chunk(W3D_CHUNK_HLOD)) return false;

  if (!save_header(csave)) return false;

  if (!save_lod_arrays(csave)) return false;

  if (!save_aggregate_array(csave)) return false;

  if (!save_proxy_array(csave)) return false;

  if (!csave.End_Chunk()) return false;
...
bool HLodSaveClass::save_header(ChunkSaveClass &csave) {
  if (!csave.Begin_Chunk(W3D_CHUNK_HLOD_HEADER)) return false;

  if (csave.Write(&header, sizeof(header)) != sizeof(header)) return false;
...
bool HLodSaveClass::save_lod_arrays(ChunkSaveClass &csave) {
  for (int i = 0; i < header.LodCount; i++) {
    if (!csave.Begin_Chunk(W3D_CHUNK_HLOD_LOD_ARRAY)) return false;

    if (!save_sub_object_array(csave, lod_array[i])) return false;
...
bool HLodSaveClass::save_aggregate_array(ChunkSaveClass &csave) {
  if (aggregate_array.num_sub_objects > 0) {
    if (!csave.Begin_Chunk(W3D_CHUNK_HLOD_AGGREGATE_ARRAY)) return false;

    if (!save_sub_object_array(csave, aggregate_array)) return false;
...
bool HLodSaveClass::save_proxy_array(ChunkSaveClass &csave) {
  if (proxy_array.num_sub_objects > 0) {
    if (!csave.Begin_Chunk(W3D_CHUNK_HLOD_PROXY_ARRAY)) return false;

    if (!save_sub_object_array(csave, proxy_array)) return false;
...
/***********************************************************************************************
 * HLodSaveClass::save_sub_object_array -- Writes the mesh to bone connectivity
 *info for each  * mesh in an LOD. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
...
bool HLodSaveClass::save_sub_object_array(ChunkSaveClass &csave,
  ...
  if (!csave.Begin_Chunk(W3D_CHUNK_HLOD_SUB_OBJECT_ARRAY_HEADER)) return false;

  if (csave.Write(&(array.header), sizeof(array.header)) !=
      sizeof(array.header))
    return false;
  ...
  for (int j = 0; j < array.num_sub_objects; j++) {
    if (!csave.Begin_Chunk(W3D_CHUNK_HLOD_SUB_OBJECT)) return false;

    if (csave.Write(&(array.sub_obj[j]), sizeof(array.sub_obj[j])) !=
        sizeof(array.sub_obj[j]))
      return false;
