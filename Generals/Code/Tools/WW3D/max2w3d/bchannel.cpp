...
BitChannelClass::BitChannelClass(uint32 id, int maxframes, uint32 chntype,
                                 bool default_val)
    : ID(id),
      ChannelType(chntype),
      MaxFrames(maxframes),
...
bool BitChannelClass::Save(ChunkSaveClass &csave, bool compress) {
  ...
  if (compress) {
    ...
    W3dTimeCodedBitChannelStruct *chn =
        (W3dTimeCodedBitChannelStruct *)malloc(channelsize);
...
bool BitChannelClass::is_default(bool bit) { return (bit == DefaultVal); }
...
//
void BitChannelClass::remove_packet(W3dTimeCodedBitChannelStruct *c,
                                    uint32 packet_idx) {
  assert(c);
