 ...
 *                                                                                             *
 *                     $Archive::
 * /Commando/Code/Tools/max2w3d/gridsnapmodifier.cpp           $*
 *                                                                                             *
...
/*
  ...
  WARNING WARNING WARNING PLEASE READ - This modifier was an experiment to see
  if we could solve cracks in adjacent meshes by snapping to a world-space grid.
  It didn't work for a few reasons:
  - I couldn't implement the world space snapping; the SimpleMod code seems to
  always force you to work relative to each object.
  - Snapping to a grid won't always snap vertices to the same grid.  The
  probability that it will work is a function of the distance between the points
...
** This modifier will snap all vertices in the geometry being modified to a
*grid.  Its motivation is to
** try to help solve the problem of cracks between adjacent meshes in Renegade
*levels.  This will work
** a lot better if the objects have reset-transforms prior to being processed by
*this modifier.
*/
...
** This is the callback object used by GridSnapModifierClass to implement the
*actual geometry changes.  This
** architecture is required by the SimpleMod base-class.  This Deformer simply
*snaps vertex positions
...
** This object "links" the plugin into Max's plugin system.  It links the
*Class-ID to a virtual construction
** method.  The function Get_Grid_Snap_Modifier_Desc is the only hook to
*external code.
*/
...
ClassDesc* Get_Grid_Snap_Modifier_Desc(void) { return &_GridSnapModifierDesc; }
...
static ParamBlockDesc2 _GridSnapParamBlockDesc(
    ...
    GSM_PARAMS, _T("GridSnap Parameters"), 0, &_GridSnapModifierDesc,
    P_AUTO_CONSTRUCT + P_AUTO_UI, SIMPMOD_PBLOCKREF,

    // dialog box
    IDD_GRIDSNAP_PARAMS, IDS_GRIDSNAP_TITLE, 0, 0, NULL,

    // parameters
    GSM_PARAM_GRIDDIMENSION, _T("Grid Dimension"), TYPE_FLOAT, P_RESET_DEFAULT,
    IDS_GRID_DIMENSION, p_default, 0.001f, p_range, 0.0001f, 10.0f, p_ui,
    TYPE_SPINNER, EDITTYPE_FLOAT, IDC_GRIDDIM_EDIT, IDC_GRIDDIM_SPIN, 0.0001f,
    end,
...
void GridSnapModifierClass::BeginEditParams(IObjParam* ip, ULONG flags,
                                            Animatable* prev) {
  this->ip = ip;
...
void GridSnapModifierClass::EndEditParams(IObjParam* ip, ULONG flags,
                                          Animatable* next) {
...
Deformer& GridSnapModifierClass::GetDeformer(TimeValue t, ModContext& mc,
