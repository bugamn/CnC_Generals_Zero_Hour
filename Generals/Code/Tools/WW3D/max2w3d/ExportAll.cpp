 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * wwExportTreeSettings  -- Returns the directory to export, and
 *recursive flag.              *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */

/*
** ExportAll.cpp - Implements wwExportTreeSettings, which presents the user with
*a dialog
** to allow them to choose which directory they want to export, and whether they
*want to
** export all subdirectories as well. These settings are then passed back so
*that a script
** can go through the directory (and maybe the subdirectories) and export all
*.max files
** it finds.
*/
...
/***********************************************************************************************
 * export_tree_settings_cf - Returns the directory to export, and recursive
 *flag.              *
 *                                                                                             *
 * wwExportTreeSettings - Usage: wwExportTreeSettings #(<initial path>,
 *<recursive>)           *
 *                                                                                             *
 * INPUT: A MaxScript array containing two values: * initial_path (string)
 *- the initial export directory                                   * recursive
 *(bool)			- the initial recursive setting *
 *                                                                                             *
 * OUTPUT: An array of the same format containing the values the user chose. *
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/4/1999  AJA : Created. *
 *=============================================================================================*/
Value *export_tree_settings_cf(Value **arg_list, int count) {
  ...
  if (dlg.DoModal() == IDCANCEL) return &undefined;
