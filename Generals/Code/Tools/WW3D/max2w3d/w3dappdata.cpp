 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * Is_Bone -- check if a node is a "bone" * Is_Geometry -- Check if
 *an INode is marked for "geometry" export                          *
 *   Is_Normal_Mesh -- check if a node is a mesh * Is_Camera_Aligned_Mesh --
 *check if a node is to be a camera aligned mesh                  *
 ...
 *   Is_Collision_OBBox -- check if a node is a collision box mesh * Is_Skin --
 *check if a node is a skin * Is_Shadow -- check if a node marked to cast
 *shadows                                       * Is_Damage -- check if a node
 *is a damage version of a mesh                                *
 *   Is_Damage_For_Mesh -- check if a node is a damage version of a particular
 *mesh            * Is_Physical_Collision -- Is node categorized as "physical
 *collision geometry"             * Is_Projectile_Collision -- Is node
 *categorized as "projectile collision geometry"?        * Is_Vis_Collision --
 *Is node categorized as "vis collision geometry"?                      *
 *   Is_Camera_Collision -- Is node categorized as "camera collision geometry"?
 ** Is_Hidden -- checks whether the node should be hidden * Is_Two_Sided --
 *Checks whether the node should be two sided                               *
 *   Is_ZNormals -- checks if the node should be exported with 0,0,1 vert
 *normals              * Is_Dazzle -- check if a node is a dazzle object *
...
** version 1 - added camera collision option, initialize to match physical
*collision settting
...
W3DAppData2Struct::W3DAppData2Struct(void)
    : ExportFlags(0), GeometryType(0), GeometryFlags(0), CollisionFlags(0) {
...
void W3DAppData2Struct::Init_From_AppData0(W3DAppData0Struct &olddata) {
  ...
  Enable_Camera_Collision(
      olddata.Is_Physical_Collision());  // make camera setting match physical
...
void W3DAppData2Struct::Update_Version(void) {
  ...
  ** setting to be equal to the physical collision setting and convert to
  *version 1
  */
  if (Get_Version() == 0) {
...
W3DAppData2Struct *W3DAppData2Struct::Get_App_Data(INode *node,
  ...
  */
  if (appdata) {
    ...
    */
    AppDataChunk *oldappdata = node->GetAppDataChunk(
        W3DUtilityClassID, UTILITY_CLASS_ID, W3D_APPDATA_0);
    if (oldappdata) {
      /*
      ...
      */
      node->RemoveAppDataChunk(W3DUtilityClassID, UTILITY_CLASS_ID,
                               W3D_APPDATA_0);
    ...
    node->AddAppDataChunk(W3DUtilityClassID, UTILITY_CLASS_ID, 2,
                          sizeof(W3DAppData2Struct), wdata);
...
W3DDazzleAppDataStruct *W3DDazzleAppDataStruct::Get_App_Data(
    INode *node, bool create_if_missing) {
  /*
  ** Try to get the existing AppData chunk
  */
  W3DDazzleAppDataStruct *dazzledata = NULL;
  AppDataChunk *appdata = node->GetAppDataChunk(
      W3DUtilityClassID, UTILITY_CLASS_ID, W3D_DAZZLE_APPDATA);
  ...
  if (appdata) {
    ...
    */
    ...
    node->AddAppDataChunk(W3DUtilityClassID, UTILITY_CLASS_ID,
                          W3D_DAZZLE_APPDATA, sizeof(W3DDazzleAppDataStruct),
                          dazzledata);

    appdata = node->GetAppDataChunk(W3DUtilityClassID, UTILITY_CLASS_ID,
                                    W3D_DAZZLE_APPDATA);
    assert(appdata);
  }
  return dazzledata;
...
/***********************************************************************************************
**
**	Utility functions for reading the W3D options that are set for a given
*INode
**
***********************************************************************************************/
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 5/5/98     GTH : Created. *
 *=============================================================================================*/
bool Is_Camera_Aligned_Mesh(INode *node) {
  if (!Is_Geometry(node)) return false;
  return (get_geometry_type(node) ==
          W3DAppData2Struct::GEO_TYPE_CAMERA_ALIGNED);
}
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 3/2/99     NH : Created. *
 *=============================================================================================*/
bool Is_Camera_Oriented_Mesh(INode *node) {
  if (!Is_Geometry(node)) return false;
  return (get_geometry_type(node) ==
          W3DAppData2Struct::GEO_TYPE_CAMERA_ORIENTED);
}
...
/***********************************************************************************************
 * Is_Projectile_Collision -- Is node categorized as "projectile collision
 *geometry"?          *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * This has nothing to do with its hidden status inside of max.
 *Things hidden in max are      * ignored by the exporter.  (artist request way
 *back...wierd huh?)                            *
 *                                                                                             *
 * HISTORY: * 11/18/98   GTH : Created. *
 *=============================================================================================*/
