 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * W3dExportClass::W3dExportClass -- constructor *
 ...
 *   W3dExportClass::Export_Damage_Animations -- Exports damage animations for
 *the model       * W3dExportClass::Export_Geometry -- Export the geometry data
...
/************************************************************************************************
**
**	GeometryFilterClass - filters out nodes which are not marked for W3D
*geometry export
**
************************************************************************************************/
class GeometryFilterClass : public INodeFilterClass {
 public:
  virtual BOOL Accept_Node(INode *node, TimeValue time) {
    ...
    if (obj
        //			&& !Is_Proxy (*node)
        && !Is_Origin(node) && !node->IsHidden() && Is_Geometry(node)) {
      return TRUE;
    } else {
      return FALSE;
    }
...
/************************************************************************************************
**
** OriginFilterClass - Filters out nodes which are not "origin" objects. Origins
*are MAX dummy
** objects whose parents are the scene root, and are named "origin.*" (ie. first
*7 characters
** in the name are "origin.". These origin objects will be (0,0,0) for all of
*its descendants.
** This allows an artist to create multiple models within one MAX scene but
*still have each
** mesh's coordinates equal without needing to stack all the models on the world
*origin.
** We iterate through the origin objects when exporting a scene containing
*multiple LODs of
** one model.
**
************************************************************************************************/
...
/************************************************************************************************
**
** DamageRootFilterClass - Filters out all nodes which are not "damage root"
*objects. These nodes
** are MAX dummy objects whose parents are the scene root, and are named
*"damage.*" (ie. first 7
** characters in the name are "damage.". These damage roots mean that all of its
*children
...
class DamageRootFilterClass : public INodeFilterClass {
 public:
  virtual BOOL Accept_Node(INode *node, TimeValue time) {
    return Is_Damage_Root(node);
  }
};

/************************************************************************************************
**
** DamageRegionFilterClass - Filters out all node that are not a bone which is
*part of a certain
...
/***********************************************************************************************
 * W3dExportClass::DoExport -- This method is called for the plug-in to perform
 *it's file expo *
 *                                                                                             *
 * INPUT: * name - filename to use
 ** export - A pointer the plug-in may use to call methods to enumerate the
 *scene 			  * max - An interface pointer the plug-in may
 *use to call methods of MAX.						  *
 * 																														  *
 * OUTPUT: * Nonzero on successful export; otherwise 0.
 **
 * 																														  *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 06/09/1997 GH  : Created. * 10/17/2000 gth : Removed the old
 *export code-path, everything goes through an origin now  *
 *=============================================================================================*/
int W3dExportClass::DoExport(const TCHAR *filename, ExpInterface *export,
  ...
  try {
    ...
    /*
    ** The Animation and the Hierarchy will be named with the root portion of
    *the W3D filename
    ...
    */
    ...
    /*
    ** The batch export process (suppressPrompt == TRUE) needs to know the
    *directory of the
    ** MAX file being exported. This is so that it can use the old relative
    *pathname of the
    ...
    */
    if ((!ExportOptions.ExportHierarchy) && (!ExportOptions.ExportAnimation) &&
        (!ExportOptions.ExportGeometry)) {
      return 1;
    }
    ...
    */
    ...
    if (!stream.Open(FileClass::WRITE)) {
      MessageBox(NULL, "Unable to open file.", "Error",
    ...
    **
    ** Are we doing an old export (one model/LOD per scene) or a new export
    *(multiple LODs
...
/***********************************************************************************************
 * W3dExportClass::DoOriginBasedExport -- New export codepath. Exports any
 *objects linked to   * an origin object. Assumes origins named "origin.01" and
 *greater represent LODs of the       * original object ("origin.00"). Also
 *assumes "damage.01" and greater represent damaged       * versions of the
 *original object.                                                            *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 9/13/1999  AJA : Created. * 9/21/1999  AJA : Added support for the
 *new animation exporting process (damage-related).  *
 *=============================================================================================*/
void W3dExportClass::DoOriginBasedExport(char *rootname,
  ...
  */
  ...
  if (!Export_Hierarchy(rootname, csave, treemeter, base_origin)) {
    MessageBox(NULL, "Hierarchy Export Failure!", "Error",
               MB_OK | MB_SETFOREGROUND);
    End_Progress_Bar();
    return;
  }
  ...
  if (damage_list->Num_Nodes() <= 0) {
    ...
    */
    ...
    if (!Export_Animation(rootname, csave, animmeter, base_origin)) {
      MessageBox(NULL, "Animation Export Failure!", "Error",
                 MB_OK | MB_SETFOREGROUND);
      End_Progress_Bar();
      return;
    }
    ...
    */
    ...
    for (i = 0; i < damage_list->Num_Nodes(); i++) {
      if (!Export_Damage_Animations(rootname, csave, damagemeter,
                                    (*damage_list)[i])) {
        MessageBox(NULL, "Damage Animation Export Failure!", "Error",
                   MB_OK | MB_SETFOREGROUND);
        End_Progress_Bar();
        return;
      }
  ...
  */
  ...
  if (!connections) {
    MessageBox(NULL, "Memory allocation failure!", "Error",
               MB_OK | MB_SETFOREGROUND);
    End_Progress_Bar();
    return;
  }
  ...
  for (i = 0; i < count; i++) {
    ...
    */
    ...
    if (!Export_Geometry(rootname, csave, meshmeter, origin, &meshcon)) {
      MessageBox(NULL, "Geometry Export Failure!", "Error",
                 MB_OK | MB_SETFOREGROUND);
      End_Progress_Bar();
      return;
    }
  ...
  */
  if (ExportOptions.LoadHierarchy || ExportOptions.ExportHierarchy) {
    rootname[idx] =
        '\0';  // remove the trailing character (signifies which lod level)
    HierarchySaveClass *htree = get_hierarchy_tree();
    if (htree) {
      Progress_Meter_Class hlod_meter(meter, meter.Increment);
      if (!Export_HLod(rootname, htree->Get_Name(), csave, hlod_meter,
                       connections, count)) {
        MessageBox(NULL, "HLOD Generation Failure!", "Error",
                   MB_OK | MB_SETFOREGROUND);
        End_Progress_Bar();
        return;
      }
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/16/1997 GH  : Created. * 13/9/1999  AJA : Split into two calls,
 *one that takes a node list and one that takes a    * single root node. *
 *   10/17/2000 gth : Removed the old code-path, we always use an origin now *
 *=============================================================================================*/
bool W3dExportClass::Export_Hierarchy(char *name, ChunkSaveClass &csave,
                                      Progress_Meter_Class &meter,
                                      INode *root) {
  if (!ExportOptions.ExportHierarchy) return true;
  HierarchySaveClass::Enable_Terrain_Optimization(
      ExportOptions.EnableTerrainMode);

  if (root == NULL) return false;

  try {
    HierarchyTree =
        new HierarchySaveClass(root, CurTime, meter, name, FixupType);
 ...
 *   10/17/2000 gth : Removed the old code-path, we always use an origin now *
 *=============================================================================================*/
bool W3dExportClass::Export_Animation(char *name, ChunkSaveClass &csave,
                                      Progress_Meter_Class &meter,
                                      INode *root) {
  ...
  try {
    motion =
        new MotionClass(ExportInterface->theScene, root, htree, ExportOptions,
                        FrameRate, &meter, MaxInterface->GetMAXHWnd(), name);
  } catch (ErrorClass err) {
    MessageBox(NULL, err.error_message, "Error!", MB_OK | MB_SETFOREGROUND);
    return false;
  }
...
/***********************************************************************************************
 * W3dExportClass::Export_Damage_Animations -- Exports damage animations for the
 *model         *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
bool W3dExportClass::Export_Damage_Animations(char *name, ChunkSaveClass &csave,
  ...
  if ((damage_root == NULL) || (htree == NULL)) return false;

  int damage_state = Get_Damage_State(damage_root);

  /*
  ** While exporting damage animations, we need the offset from our origin to
  *the real
  ** scene origin.
  */
  ...
  for (current_region = 0; current_region < MAX_DAMAGE_REGIONS;
       current_region++) {
    ...
    if (bone_list.Num_Nodes() <= 0) continue;
    ...
    try {
      motion =
          new MotionClass(ExportInterface->theScene, &bone_list, htree,
                          ExportOptions, FrameRate, &meter,
                          MaxInterface->GetMAXHWnd(), anim_name, originoffset);
    } catch (ErrorClass err) {
      MessageBox(NULL, err.error_message, "Error!", MB_OK | MB_SETFOREGROUND);
      return false;
    }
  ...
  if (num_damage_bones <= 0) {
    MessageBox(
        ...
        "You can do this in the W3D Tools panel.",
        name, MB_OK | MB_ICONINFORMATION | MB_SETFOREGROUND);
  }
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/16/1997 GH  : Created. * 13/9/1999  AJA : Added an optional
 *"root" parameter to export geometry of the node's      * descendants. *
 *   10/17/2000 gth : Made the "root" parameter a requirement, just pass in the
 *scene root     * if you want to export all geometry in the scene. * 10/30/2000
 *gth : If exporting only geometry, only export the first mesh *
 *=============================================================================================*/
bool W3dExportClass::Export_Geometry(char *name, ChunkSaveClass &csave,
                                     Progress_Meter_Class &meter, INode *root,
                                     MeshConnectionsClass **out_connection) {
  unsigned int i;
  ...
  */
  ...
  unsigned int
      materialColors[16 * 16];  ///@todo: MW: Fix this to remove solid colors.
  char materialColorFilename[_MAX_FNAME + 1];
  memset(materialColors, 0, sizeof(materialColors));
  for (i = 0; i < TEAM_COLOR_PALETTE_SIZE;
       i++) {  // preset the first 16 colors to predefined set of house colors
  ...
  */
  GeometryExportContextClass context(name, csave, world_info, ExportOptions,
                                     htree, root, get_origin_list(), CurTime,
                                     materialColors);
  ...
  /*
  ** Initialize a list of geometry export tasks containing all nodes marked for
  *geometry export.
  ** If we're only exporting geometry, only export the first mesh. (no more
  *collections)
  */
  ...
  if ((htree == NULL) && (geometry_list->Num_Nodes() > 1)) {
    ...
    ExportLog::printf(
        "\nDiscarding extra meshes since we are not exporting a hierarchical "
        "model.\n");
  }

  for (i = 0; i < geometry_count; i++) {
    GeometryExportTaskClass *export_task =
        GeometryExportTaskClass::Create_Task((*geometry_list)[i], context);
    if (export_task != NULL) {
  ...
  /*
  ** Optimize the mesh data if the user desired, modifying the list of geometry
  *export tasks.
  */
  ...
  /*
  ** If there is only one piece of geometry to export and no place-holders, and
  *we're not
  ** exporting a hierarchical model, then we force the name to match the
  *filename
  */
  ...
  */
  for (i = 0; i < export_tasks.Count(); i++) {
    ...
    try {
      ...
      MessageBox(MaxInterface->GetMAXHWnd(), err.error_message, "Error!",
                 MB_OK | MB_SETFOREGROUND);
      continue;
    }
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/17/2000 gth : Created. *
 *=============================================================================================*/
bool W3dExportClass::Export_HLod(char *name, const char *htree_name,
                                 ChunkSaveClass &csave,
                                 Progress_Meter_Class &meter,
  ...
  HLodSaveClass hlod_save(connections, lod_count, CurTime, name, htree_name,
                          meter, get_origin_list());
  if (!hlod_save.Save(csave)) return false;
  return true;
}
  ...
  */
  if (!ExportOptions.ExportHierarchy) {
    ...
    if (HierarchyTree) {
      ...
      char buf[256];
      sprintf(buf,
              "Unable to load hierarchy file: %s\nIf this Max file has been "
              "moved, please re-select the hierarchy file.",
              HierarchyFilename);
      MessageBox(MaxInterface->GetMAXHWnd(), buf, "Error",
                 MB_OK | MB_SETFOREGROUND);
      return NULL;
    }
  }
...
INodeListClass *W3dExportClass::get_origin_list(void) {
  ...
  ** NOTE: it would also be a problem if the origin list contained both the
  *scene root
  ** and the user placed origins.  Thats not happening now because the
  *OriginList
  ** does not collect the scene root... were that to change we'd have to update
  *this
  ** code as well.
  */
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/16/1997 GH  : Created. * 9/30/1999  AJA : Added support for the
 *MAX suppress_prompts flag.                         *
 *=============================================================================================*/
bool W3dExportClass::get_export_options(BOOL suppress_prompts) {
  ...
  AppDataChunk *appdata = MaxInterface->GetScenePointer()->GetAppDataChunk(
      W3D_EXPORTER_CLASS_ID, SCENE_EXPORT_CLASS_ID, 0);

  if (appdata) {
    options =
        &(((ExportInfoAppDataChunkStruct *)(appdata->data))->ExportOptions);
  } else {
    ExportInfoAppDataChunkStruct *appdata_struct =
        (ExportInfoAppDataChunkStruct *)malloc(
    ...
    options->CompressAnimationTranslationError =
        0.001f;  // DEFAULT_LOSSY_ERROR_TOLERANCE;
    options->CompressAnimationRotationError =
        0.050f;  // DEFAULT_LOSSY_ERROR_TOLERANCE;
    ...
    MaxInterface->GetScenePointer()->AddAppDataChunk(
        W3D_EXPORTER_CLASS_ID, SCENE_EXPORT_CLASS_ID, 0,
        sizeof(ExportInfoAppDataChunkStruct), appdata_struct);
  }

  // (gth) disabling the 'optimize mesh data' feature due to problems with
  // external tools
  options->EnableOptimizeMeshData = false;
...
void W3dExportClass::Start_Progress_Bar(void) {
  MaxInterface->ProgressStart("Processing Triangle Mesh", TRUE,
...
static bool check_lod_extensions(INodeListClass &list, INode *origin) {
  /*
  ** Assumptions:
  ** - If origin == NULL, then we're just exporting a single model and don't
  *need to
  ** worry about lod extensions at all.
  ** - If origin is the root of the scene, then we're just exporting a single
  *model as well.
  ...
  */
  ...
  for (unsigned i = 0; i < list.Num_Nodes(); i++) {
    ...
    if (this_ext == NULL) return false;

    // Check that the extensions are the same.
    if (strcmp(this_ext, extension) != 0) return false;
  }
...
bool W3dExportClass::get_base_object_tm(Matrix3 &tm) {
  ...
  if (!base_origin) return false;

  tm = base_origin->GetNodeTM(CurTime);
  return true;
}

static DWORD WINAPI progress_callback(LPVOID arg) { return 0; }
