...
static BOOL CALLBACK DisplacementMapDlgProc(HWND hwndDlg, UINT msg,
                                            WPARAM wPara, LPARAM lParam);
static BOOL CALLBACK SurfaceTypePanelDlgProc(HWND hwndDlg, UINT msg,
                                             WPARAM wPara, LPARAM lParam);
static BOOL CALLBACK PassCountPanelDlgProc(HWND hwndDlg, UINT msg, WPARAM wPara,
                                           LPARAM lParam);
static BOOL CALLBACK PassCountDialogDlgProc(HWND hwndDlg, UINT msg,
...
GameMtlDlg::~GameMtlDlg() {
  ...
  TheMtl->Set_Flag(GAMEMTL_DISPLACEMENT_ROLLUP_OPEN,
                   IParams->IsRollupPanelOpen(HwndDisplacementMap));
#endif  // WANT_DISPLACEMENT_MAPS

  TheMtl->Set_Flag(GAMEMTL_SURFACE_ROLLUP_OPEN,
                   IParams->IsRollupPanelOpen(HwndSurfaceType));
  TheMtl->Set_Flag(GAMEMTL_PASSCOUNT_ROLLUP_OPEN,
                   IParams->IsRollupPanelOpen(HwndPassCount));
...
void GameMtlDlg::SetThing(ReferenceTarget *m) {
  ...
  assert((m->ClassID() == GameMaterialClassID) ||
         (m->ClassID() == PS2GameMaterialClassID));
  assert(TheMtl);
...
BOOL GameMtlDlg::DisplacementMapProc(HWND hDlg, UINT message, WPARAM wParam,
                                     LPARAM lParam) {
  switch (message) {
    case WM_INITDIALOG:
    ...
    case WM_USER + 101: {
      SetDlgItemInt(hDlg, IDC_AMOUNT_EDIT,
                    TheMtl->Get_Displacement_Amount() * 100, TRUE);
      ...
      switch (LOWORD(wParam)) {
        case IDC_TEXTURE_BUTTON:
          if (HIWORD(wParam) == BN_CLICKED) {
            PostMessage(HwndEdit, WM_TEXMAP_BUTTON,
                        TheMtl->Get_Displacement_Map_Index(), (LPARAM)TheMtl);
          }
      }
      break;
  }
...
BOOL GameMtlDlg::SurfaceTypeProc(HWND hDlg, UINT message, WPARAM wParam,
                                 LPARAM lParam) {
  switch (message) {
    case WM_INITDIALOG: {
      //
      ...
      for (int index = 0; index < SURFACE_TYPE_MAX; index++) {
        ::SendDlgItemMessage(hDlg, IDC_SURFACE_TYPE_COMBO, CB_ADDSTRING, 0,
      ...
      SetupIntSpinner(hDlg, IDC_SORT_LEVEL_SPIN, IDC_SORT_LEVEL, 0,
                      MAX_SORT_LEVEL, sort_level);

      // Check the checkbox if sort_level is not SORT_LEVEL_NONE.
      ::SendDlgItemMessage(
          hDlg, IDC_ENABLE_SORT_LEVEL, BM_SETCHECK,
    ...
    case WM_USER + 101: {
      ...
      //
      ::SendDlgItemMessage(hDlg, IDC_SURFACE_TYPE_COMBO, CB_SETCURSEL,
      ...
      ISpinnerControl *spinner =
          GetISpinner(::GetDlgItem(hDlg, IDC_SORT_LEVEL_SPIN));
      assert(spinner);
      ...
      switch (LOWORD(wParam)) {
        case IDC_SURFACE_TYPE_COMBO:
          if (HIWORD(wParam) == CBN_SELCHANGE) {
            unsigned int type = ::SendDlgItemMessage(
                hDlg, IDC_SURFACE_TYPE_COMBO, CB_GETCURSEL, 0, 0L);
        ...
        case IDC_ENABLE_SORT_LEVEL:
          if (HIWORD(wParam) == BN_CLICKED) {
            // If the 'enable' checkbox was unchecked, set the sort level to
            // NONE.
            ...
            if (state == BST_UNCHECKED) {
              ...
              // Sort level was enabled, so set it's level to 1 if it was NONE
              // before.
    ...
    case CC_SPINNER_CHANGE: {
      ...
      switch (LOWORD(wParam)) {
        case IDC_SORT_LEVEL_SPIN:
          // Check the 'enabled' checkbox if sort level != SORT_LEVEL_NONE,
          // uncheck it otherwise.
          ::SendDlgItemMessage(hDlg, IDC_ENABLE_SORT_LEVEL, BM_SETCHECK,
                               spinner->GetIVal() == SORT_LEVEL_NONE
                                   ? BST_UNCHECKED
                                   : BST_CHECKED,
                               0);
...
BOOL GameMtlDlg::PassCountProc(HWND hDlg, UINT message, WPARAM wParam,
                               LPARAM lParam) {
  switch (message) {
    case WM_INITDIALOG:
      break;
...
static BOOL CALLBACK PassCountPanelDlgProc(HWND hwndDlg, UINT msg,
                                           WPARAM wParam, LPARAM lParam) {
  GameMtlDlg *theDlg;
  if (msg == WM_INITDIALOG) {
    theDlg = (GameMtlDlg *)lParam;
...
void GameMtlDlg::Set_Pass_Count_Dialog(void) {
  int res = DialogBoxParam(
      AppInstance, MAKEINTRESOURCE(IDD_GAMEMTL_PASS_COUNT_DIALOG),
      HwndPassCount, PassCountDialogDlgProc, (LPARAM)TheMtl->Get_Pass_Count());
  ...
  switch (msg) {
    case WM_INITDIALOG: {
      ISpinnerControl *spin = SetupIntSpinner(
          hwndDlg, IDC_PASSCOUNT_SPIN, IDC_PASSCOUNT_EDIT, 1, 4, (int)lParam);
      ...
      switch (LOWORD(wParam)) {
        case IDOK: {
          ISpinnerControl *spin =
              GetISpinner(GetDlgItem(hwndDlg, IDC_PASSCOUNT_SPIN));
...
void GameMtlDlg::Build_Dialog() {
  ...
  HwndSurfaceType = IParams->AddRollupPage(
      AppInstance, MAKEINTRESOURCE(IDD_GAMEMTL_SURFACE_TYPE),
      SurfaceTypePanelDlgProc, Get_String(IDS_SURFACE_TYPE), (LPARAM)this,
      TheMtl->Get_Flag(GAMEMTL_SURFACE_ROLLUP_OPEN) ? 0 : APPENDROLL_CLOSED);

#ifdef WANT_DISPLACEMENT_MAPS
  HwndDisplacementMap = IParams->AddRollupPage(
      AppInstance, MAKEINTRESOURCE(IDD_GAMEMTL_DISPLACEMENT_MAP),
      DisplacementMapDlgProc, Get_String(IDS_DISPLACEMENT_MAP), (LPARAM)this,
      TheMtl->Get_Flag(GAMEMTL_DISPLACEMENT_ROLLUP_OPEN) ? 0
                                                         : APPENDROLL_CLOSED);
#endif  // WANT_DISPLACEMENT_MAPS

  HwndPassCount = IParams->AddRollupPage(
      AppInstance, MAKEINTRESOURCE(IDD_GAMEMTL_PASS_COUNT),
      PassCountPanelDlgProc, Get_String(IDS_PASS_COUNT), (LPARAM)this,
