...
void AlphaModifierClass::ModifyObject(TimeValue t, ModContext &mc,
  ...
  // This must be done because the mesh changes each time ModfiyObject is
  // called.
  for (i = 0; i < numVert; i++) {
    if (SelectedVertices[i]) {
      mesh->VertSel().Set(i);
  ...
  object->UpdateValidity(SELECT_CHAN_NUM,
...
/*===========================================================================*\
 |	NotifyInputChanged is called each time the input object is changed in
some way |	We can find out how it was changed by checking partID and
message
\*===========================================================================*/

void AlphaModifierClass::NotifyInputChanged(Interval changeInt, PartID partID,
                                            RefMessage message,
                                            ModContext *mc) {
...
static ParamBlockDesc2 alpha_param_blk(
    ...
    DL_EDIT_VALUE, _T("Custom Data Value"), TYPE_FLOAT, P_ANIMATABLE,
    IDS_ALPHA_MODIFIER_CLASS, p_default, 0.0f, p_range, 0.0f, 100.0f, p_ui,
    TYPE_SPINNER, EDITTYPE_FLOAT, IDC_ALPHA_EDIT, IDC_ALPHA_SPIN, 1.0f, end,

    DL_FIND_CHECK_BOX, _T("1 Custom Data Value"), TYPE_BOOL, 0,
    IDS_ALPHA_MODIFIER_CLASS, p_default, FALSE, p_ui, TYPE_SINGLECHEKBOX,
    IDC_ALPHA_CHECKBOX, p_enabled, TRUE, end,

    /*
                    DL_EDIT_PASS,	_T("2 Custom Data Value"),
       TYPE_INT,	P_ANIMATABLE,	IDS_ALPHA_MODIFIER_CLASS, p_default,
       1, p_range, 		1, 4, p_ui,			TYPE_SPINNER,
       EDITTYPE_INT, IDC_ALPHA_EDIT2, IDC_ALPHA_SPIN2, 1.0, end,
    */
...
BOOL AlphaModDlgProc::DlgProc(TimeValue t, IParamMap2 *map, HWND hWnd, UINT msg,
...
void AlphaModifierClass::BeginEditParams(IObjParam *ip, ULONG flags,
                                         Animatable *prev) {
...
void AlphaModifierClass::EndEditParams(IObjParam *ip, ULONG flags,
                                       Animatable *next) {
