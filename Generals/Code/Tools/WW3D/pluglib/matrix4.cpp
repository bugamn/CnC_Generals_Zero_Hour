 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * Matrix4::Multiply -- Multiply two Matrix4's together *
 *   Matrix4::Multiply -- Multiply a Matrix3D * Matrix4 * Matrix4::Multiply --
 *Multiply a Matrix4 * Matrix3D                                        *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 *                                                                                             *
 * INPUT: * a - first operand * b - second operand * res - pointer to matrix to
 *store the result in (must not point to a or b)                  *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
...
void Matrix4::Multiply(const Matrix4 &a, const Matrix3D &b, Matrix4 *res) {
  ...
  // ROWCOL multiplies a row of 'a' by one of the first three columns of 'b'
  // (4th entry in b is zero) ROWCOL4 multiplies a row of 'a' by the fourth
  // column of 'b' (4th entry in b is one)

#define ROWCOL(i, j) a[i][0] * b[0][j] + a[i][1] * b[1][j] + a[i][2] * b[2][j]
#define ROWCOL4(i, j) \
  a[i][0] * b[0][j] + a[i][1] * b[1][j] + a[i][2] * b[2][j] + a[i][3]
