 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * ChunkSaveClass::ChunkSaveClass -- Constructor *
 *   ChunkSaveClass::Begin_Chunk -- Begin a new chunk in the file *
 *   ChunkSaveClass::End_Chunk -- Close a chunk, computes the size and adds to
 *the header      * ChunkSaveClass::Begin_Micro_Chunk -- begins a new
 *"micro-chunk"                           * ChunkSaveClass::End_Micro_Chunk --
 *close a micro-chunk                                    * ChunkSaveClass::Write
 *-- Write data into the current chunk                                *
 *   ChunkSaveClass::Write -- write an IOVector2Struct * ChunkSaveClass::Write
 *-- write an IOVector3Struct                                         *
 *   ChunkSaveClass::Write -- write an IOVector4Struct * ChunkSaveClass::Write
 *-- write an IOQuaternionStruct                                      *
 *   ChunkSaveClass::Cur_Chunk_Depth -- returns the current chunk recursion
 *depth (debugging)  * ChunkLoadClass::ChunkLoadClass -- Constructor *
 *   ChunkLoadClass::Open_Chunk -- Open a chunk in the file, reads in the chunk
 *header         * ChunkLoadClass::Close_Chunk -- Close a chunk, seeks to the
 *end if needed                  * ChunkLoadClass::Cur_Chunk_ID -- Returns the
 *ID of the current chunk                       *
 *   ChunkLoadClass::Cur_Chunk_Length -- Returns the current length of the
 *current chunk       * ChunkLoadClass::Cur_Chunk_Depth -- returns the current
 *chunk recursion depth              * ChunkLoadClass::Contains_Chunks -- Test
 *whether the current chunk contains chunks (or dat *
 *   ChunkLoadClass::Open_Micro_Chunk -- reads in a micro-chunk header *
 *   ChunkLoadClass::Close_Micro_Chunk -- closes a micro-chunk *
 *   ChunkLoadClass::Cur_Micro_Chunk_ID -- returns the ID of the current
 *micro-chunk (asserts  * ChunkLoadClass::Cur_Micro_Chunk_Length -- returns the
 *size of the current micro chunk     * ChunkLoadClass::Read -- Read data from
 *the file                                           * ChunkLoadClass::Read --
 *read an IOVector2Struct                                           *
 *   ChunkLoadClass::Read -- read an IOVector3Struct * ChunkLoadClass::Read --
 *read an IOVector4Struct                                           *
 ...
 *                                                                                             *
 * INPUT: * file - pointer to a FileClass object to write to
 **
 * 																														  *
 * OUTPUT:
 **
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 07/17/1997 GH  : Created. *
 *=============================================================================================*/
ChunkSaveClass::ChunkSaveClass(FileClass* file)
    : File(file), StackIndex(0), InMicroChunk(false), MicroChunkPosition(0) {
...
/***********************************************************************************************
 * ChunkSaveClass::End_Chunk -- Close a chunk, computes the size and adds to the
 *header        *
 *  																														  *
 * INPUT:
 **
 ...
 *                                                                                             *
 * Micro chunks are used to wrap individual variables.  They aren't hierarchical
 *so if you     * attempt to open a micro chunk while already in one, an assert
 *will occur.                   *
 *                                                                                             *
 * INPUT: * id - 8bit id *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * id is asserted to be between 0 and 255 * cannot nest micro chunks
 *so it asserts that you are currently not in another micro-chunk    *
 *                                                                                             *
 * HISTORY: * 9/3/99     GTH : Created. *
 *=============================================================================================*/
bool ChunkSaveClass::Begin_Micro_Chunk(uint32 id) {
  ...
  // this header are tracked in the wrapping chunk.  This is because
  // micro-chunks are simply data inside the normal chunks...
...
uint32 ChunkSaveClass::Write(const void* buf, uint32 nbytes) {
  ...
  if (InMicroChunk) {
    assert(MCHeader.Get_Size() <
           255 - nbytes);  // micro chunks can only be 255 bytes
...
/***********************************************************************************************
 * ChunkSaveClass::Cur_Chunk_Depth -- returns the current chunk recursion depth
 *(debugging)    *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
bool ChunkLoadClass::Open_Chunk() {
  // if user didn't close any micro chunks that he opened, bad things could
  // happen
  ...
  if ((StackIndex > 0) && (PositionStack[StackIndex - 1] ==
                           HeaderStack[StackIndex - 1].Get_Size())) {
    return false;
  }

  // read the chunk header
  if (File->Read(&HeaderStack[StackIndex], sizeof(ChunkHeader)) !=
      sizeof(ChunkHeader)) {
    return false;
  }
...
/***********************************************************************************************
 * ChunkLoadClass::Cur_Chunk_Length -- Returns the current length of the current
 *chunk         *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 07/17/1997 GH  : Created. *
 *=============================================================================================*/
int ChunkLoadClass::Cur_Chunk_Depth() { return StackIndex; }

/***********************************************************************************************
 * ChunkLoadClass::Contains_Chunks -- Test whether the current chunk contains
 *chunks (or data) *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
bool ChunkLoadClass::Open_Micro_Chunk() {
  ...
  // read the chunk header
  // calling the ChunkLoadClass::Read fn so that if we exhaust the chunk, the
  // read will fail
...
/***********************************************************************************************
 * ChunkLoadClass::Cur_Micro_Chunk_ID -- returns the ID of the current
 *micro-chunk (asserts if *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * Asserts if you are not currently inside a micro-chunk * Micro
 *chunks have an id between 0 and 255 *
 *                                                                                             *
 * HISTORY: * 9/3/99     GTH : Created. *
 *=============================================================================================*/
...
/***********************************************************************************************
 * ChunkLoadClass::Cur_Micro_Chunk_Length -- returns the size of the current
 *micro chunk       *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 *                                                                                             *
 * WARNINGS: * Asserts if you are not currently inside a micro-chunk * Micro
 *chunks have a maximum size of 255 bytes *
 *                                                                                             *
 * HISTORY: * 9/3/99     GTH : Created. *
 *=============================================================================================*/
...
uint32 ChunkLoadClass::Seek(uint32 nbytes) {
  ...
  if (PositionStack[StackIndex - 1] + nbytes >
      (int)HeaderStack[StackIndex - 1].Get_Size()) {
    return 0;
  }
