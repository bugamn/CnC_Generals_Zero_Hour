...
//=============================================================================
LRESULT CALLBACK EditWindow::editProc(HWND hWnd, UINT message, WPARAM wParam,
                                      LPARAM lParam) {
  switch (message) {
    // ------------------------------------------------------------------------
    case WM_TIMER: {
      ...
      if (TheEditWindow) {
        if (timerID == TIMER_EDIT_WINDOW_PULSE) TheEditWindow->updatePulse();
      ...
      switch (controlID) {
        ...
        case POPUP_MENU_BRING_TO_TOP: {
          ...
          switch (controlID) {
            // ----------------------------------------------------------------
            case POPUP_MENU_NEW_WINDOW:
              TheEditor->newWindow(GWS_USER_WINDOW, window, pos.x, pos.y,
            ...
            // ----------------------------------------------------------------
            case POPUP_MENU_NEW_PUSH_BUTTON:
              TheEditor->newWindow(GWS_PUSH_BUTTON, window, pos.x, pos.y,
                                   15 * TheEditor->getGridResolution(),
            ...
            // ----------------------------------------------------------------
            case POPUP_MENU_NEW_CHECK_BOX:
              TheEditor->newWindow(GWS_CHECK_BOX, window, pos.x, pos.y,
                                   15 * TheEditor->getGridResolution(),
                                   3 * TheEditor->getGridResolution());
              break;
            ...
            // ----------------------------------------------------------------
            case POPUP_MENU_NEW_RADIO_BUTTON:
              TheEditor->newWindow(GWS_RADIO_BUTTON, window, pos.x, pos.y,
                                   15 * TheEditor->getGridResolution(),
                                   3 * TheEditor->getGridResolution());
              break;
            ...
            // ----------------------------------------------------------------
            case POPUP_MENU_NEW_TAB_CONTROL:
              TheEditor->newWindow(GWS_TAB_CONTROL, window, pos.x, pos.y,
            ...
            // ----------------------------------------------------------------
            case POPUP_MENU_NEW_LISTBOX:
              TheEditor->newWindow(GWS_SCROLL_LISTBOX, window, pos.x, pos.y,
            ...
            // ----------------------------------------------------------------
            case POPUP_MENU_NEW_COMBO_BOX:
              TheEditor->newWindow(GWS_COMBO_BOX, window, pos.x, pos.y,
                                   15 * TheEditor->getGridResolution(),
                                   3 * TheEditor->getGridResolution());
              break;
            ...
            // ----------------------------------------------------------------
            case POPUP_MENU_NEW_HORIZONTAL_SLIDER:
              TheEditor->newWindow(GWS_HORZ_SLIDER, window, pos.x, pos.y,
                                   20 * TheEditor->getGridResolution(),
                                   GADGET_SIZE);
              break;
            ...
            // ----------------------------------------------------------------
            case POPUP_MENU_NEW_VERTICAL_SLIDER:
              TheEditor->newWindow(GWS_VERT_SLIDER, window, pos.x, pos.y,
                                   GADGET_SIZE,
                                   20 * TheEditor->getGridResolution());
              break;
            ...
            // ----------------------------------------------------------------
            case POPUP_MENU_NEW_PROGRESS_BAR:
              TheEditor->newWindow(GWS_PROGRESS_BAR, window, pos.x, pos.y,
            ...
            // ----------------------------------------------------------------
            case POPUP_MENU_NEW_TEXT_ENTRY:
              TheEditor->newWindow(GWS_ENTRY_FIELD, window, pos.x, pos.y,
                                   20 * TheEditor->getGridResolution(), 25);
              break;

            // ----------------------------------------------------------------
            case POPUP_MENU_NEW_STATIC_TEXT:
              TheEditor->newWindow(GWS_STATIC_TEXT, window, pos.x, pos.y,
                                   15 * TheEditor->getGridResolution(),
                                   15 * TheEditor->getGridResolution());
              break;
            ...
            // ----------------------------------------------------------------
            case POPUP_MENU_PROPERTIES:
              if (window) InitPropertiesDialog(window, pos.x, pos.y);
              break;
...
void EditWindow::registerEditWindowClass(void) {
  ...
  if (atom != 0) m_classRegistered = TRUE;
...
void EditWindow::init(UnsignedInt clientWidth, UnsignedInt clientHeight) {
  UnsignedInt windowStyle =
      WS_CHILD | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_BORDER | WS_CAPTION;
  UnsignedInt extendedWindowStyle = 0;
  ICoord2D size;

  // register an edit window class with windows if not already done
  if (m_classRegistered == FALSE) registerEditWindowClass();
  ...
  // create the window
  m_editWindowHWnd =
      CreateWindowEx(extendedWindowStyle,  // extended window style
  ...
  TheW3DFileSystem =
      new W3DFileSystem;  // our own file system for asset locations

  // initialize W3D
  WWMath::Init();
  WW3D::Init(m_editWindowHWnd);
  WW3D::Set_Screen_UV_Bias(TRUE);  ///< this makes text look good :)
  if (WW3D::Set_Render_Device(0, m_size.x, m_size.y, m_bitDepth, TRUE) !=
      WW3D_ERROR_OK) {
    assert(0);
    shutdown();
    return;
...
void EditWindow::shutdown(void) {
  ...
  if (m_editWindowHWnd) DestroyWindow(m_editWindowHWnd);
  m_editWindowHWnd = NULL;
...
void EditWindow::updatePulse(void) {
  ...
  static Int pulseMax = 175, pulseMin = 75;

  // this is used for drawing pusling lines for moving stuff
  if (dir == 1) {
    m_pulse += stepSize;
    if (m_pulse >= pulseMax) dir = 0;
  }  // end if
  else {
    m_pulse -= stepSize;
    if (m_pulse <= pulseMin) dir = 1;
  }  // end else
...
//=============================================================================
void EditWindow::mouseEvent(UnsignedInt windowsMessage, WPARAM wParam,
                            LPARAM lParam) {
  Int x = LOWORD(lParam);
  Int y = HIWORD(lParam);
  ...
  if (windowsMessage == WM_MOUSEMOVE) {
    ...
    mousePrint.x = x;
    mousePrint.y = y;
    if (mousePrint.x < 0) mousePrint.x = 0;
    if (mousePrint.x > m_size.x) mousePrint.x = m_size.x;
    if (mousePrint.y < 0) mousePrint.y = 0;
    if (mousePrint.y > m_size.y) mousePrint.y = m_size.y;

    sprintf(buffer, "Mouse Location (X = %d, Y = %d)", mousePrint.x,
            mousePrint.y);
  ...
  if (TheEditor->getMode() == MODE_TEST_RUN) {
    if (TheWin32Mouse)
      TheWin32Mouse->addWin32Event(windowsMessage, wParam, lParam,
                                   NO_TIME_FROM_WINDOWS);
  ...
  //
  if (TheEditor->getMode() == MODE_KEYBOARD_MOVE) return;
  ...
  switch (windowsMessage) {
    ...
    case WM_LBUTTONDOWN: {
      ...
      if (TheEditor->getMode() == MODE_RESIZE_TOP_LEFT ||
        ...
        if (m_windowToResize) m_resizingWindow = TRUE;
        ...
        if (window) {
          ...
          if (TheEditor->isWindowSelected(window) == FALSE) {
            ...
            //
            if (controlDown == FALSE) TheEditor->clearSelections();
            ...
            //
            if (controlDown == TRUE) TheEditor->unSelectWindow(window);
    ...
    case WM_LBUTTONUP: {
      ...
      if (TheEditor->getMode() == MODE_DRAG_MOVE) {
        ...
        if (window) {
          ...
          TheEditor->computeResizeLocation(TheEditor->getMode(), window,
                                           &m_resizeOrigin, &dest, &resultLoc,
                                           &resultSize);
...
Bool EditWindow::inCornerTolerance(ICoord2D *dest, ICoord2D *source,
  ...
  // check if in region
  if (dest->x >= region.lo.x && dest->x <= region.hi.x &&
      dest->y >= region.lo.y && dest->y <= region.hi.y)
    return TRUE;
...
//=============================================================================
Bool EditWindow::inLineTolerance(ICoord2D *dest, ICoord2D *lineStart,
                                 ICoord2D *lineEnd, Int tolerance) {
  IRegion2D region;

  // sanity
  if (dest == NULL || lineStart == NULL || lineEnd == NULL) return FALSE;
...
void EditWindow::handleResizeAvailable(Int mouseX, Int mouseY) {
  ...
  if (TheEditor->selectionCount() != 1) return;
...
void EditWindow::drawHiddenOutlines(GameWindow *windowList, Color c) {
  ...
  while (parent) {
    if (BitTest(parent->winGetStatus(), WIN_STATUS_HIDDEN)) hidden = TRUE;
    parent = parent->winGetParent();

  }  // end while
  if (BitTest(windowList->winGetStatus(), WIN_STATUS_HIDDEN)) hidden = TRUE;
  if (hidden) {
    ICoord2D pos;
    ICoord2D size;
...
void EditWindow::drawUIFeedback(void) {
  ...
  if (TheEditor->isGridVisible() == TRUE) drawGrid();
  ...
  if (m_dragSelecting) {
    ...
    drawOpenRect(m_selectRegion.lo.x, m_selectRegion.lo.y, width, height,
                 selectBoxWidth, selectBoxColor);
  ...
  while (select) {
    ...
    // draw a box on the window
    drawOpenRect(origin.x, origin.y, size.x, size.y, windowSelectWidth,
                 windowSelectColor);
  ...
  //
  if (TheEditor->getMode() == MODE_DRAG_MOVE ||
      TheEditor->getMode() == MODE_KEYBOARD_MOVE) {
    ICoord2D origin, size;
    ...
    while (select) {
      ...
      // keep location legal
      TheEditor->computeSafeLocation(window, moveLoc.x, moveLoc.y, &safeLoc.x,
                                     &safeLoc.y);
      ...
      drawOpenRect(safeLoc.x, safeLoc.y, size.x, size.y, outlineWidth,
                   outlineColor);
  ...
  if (m_resizingWindow) {
    ...
    // adjust resize dest by the grid if it's on
    if (TheEditor->isGridSnapOn()) TheEditor->gridSnapLocation(&dest, &dest);
...
void EditWindow::drawGrid(void) {
  ...
  Color color = GameMakeColor(gridColor->red, gridColor->green, gridColor->blue,
                              gridColor->alpha);
  ...
  //				TheDisplay->drawLine( x, y, x + 1, y + 1, 1,
  0xFFFFFFFF );
...
void EditWindow::draw(void) {
  ...
  // for now, use constant time steps to avoid animations running independent of
  // framerate
  syncTime += 50;

  // start render block
  WW3D::Begin_Render(true, true,
                     Vector3(m_backgroundColor.red, m_backgroundColor.green,
  ...
  if (TheEditor->getMode() != MODE_TEST_RUN) drawUIFeedback();
...
void EditWindow::setSize(ICoord2D *size) {
  ...
  if (m_2DRender)
    m_2DRender->Set_Coordinate_Range(RectClass(0, 0, m_size.x, m_size.y));
...
//=============================================================================
void EditWindow::drawLine(Int startX, Int startY, Int endX, Int endY,
 ...
 * specified color */
//=============================================================================
void EditWindow::drawOpenRect(Int startX, Int startY, Int width, Int height,
                              Real lineWidth, UnsignedInt lineColor) {
  m_2DRender->Reset();
  m_2DRender->Enable_Texturing(FALSE);
 ...
 * specified color */
//=============================================================================
void EditWindow::drawFillRect(Int startX, Int startY, Int width, Int height,
                              UnsignedInt color) {
  m_2DRender->Reset();
  m_2DRender->Enable_Texturing(FALSE);
  m_2DRender->Add_Rect(
...
//=============================================================================
void EditWindow::drawImage(const Image *image, Int startX, Int startY, Int endX,
...
void EditWindow::notifyWindowDeleted(GameWindow *window) {
  ...
  if (m_pickedWindow == window) m_pickedWindow = NULL;
