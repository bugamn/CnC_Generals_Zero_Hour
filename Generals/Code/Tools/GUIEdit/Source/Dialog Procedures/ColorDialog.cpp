...
HSVColorReal rgbToHSV(RGBColorReal rgbColor) {
  ...
  if (saturation == 0) {
    ...
    if (hue < 0) hue += 360;  // make sure hue is non negative
  ...
  if (hsvColor.hue == 0.0f) hsvColor.hue = 1.0f;
  if (hsvColor.saturation == 0.0f) hsvColor.saturation += 0.01f;
  if (hsvColor.value == 0.0f) hsvColor.value += 0.01f;
...
RGBColorReal hsvToRGB(HSVColorReal hsvColor) {
  ...
  if (saturation == 0.0f) {
    ...
    if (hue == 0.0f) {  // achromatic color ... there is no hue
...
BOOL CALLBACK SelectColorDlgProc(HWND hWnd, UINT uMsg, WPARAM wParam,
                                 LPARAM lParam);
 ...
 */
// ============================================================================
RGBColorInt *SelectColor(Int red, Int green, Int blue, Int alpha, Int mouseX,
                         Int mouseY) {
...
// ============================================================================
BOOL CALLBACK SelectColorDlgProc(HWND hWndDlg, UINT uMsg, WPARAM wParam,
                                 LPARAM lParam) {
  ...
  switch (uMsg) {
    ...
    case WM_INITDIALOG: {
      ...
      if (mode == MODE_HSV) {
        ...
        SetDlgItemInt(hWndDlg, LABEL_ALPHA, (Int)hsvColor.alpha, FALSE);
        ...
        SetDlgItemInt(hWndDlg, LABEL_COLOR1, selectedColor.red, FALSE);
        SetScrollPos(hWndScrollColor2, SB_CTL, selectedColor.green, TRUE);
        SetDlgItemInt(hWndDlg, LABEL_COLOR2, selectedColor.green, FALSE);
        SetScrollPos(hWndScrollColor3, SB_CTL, selectedColor.blue, TRUE);
        SetDlgItemInt(hWndDlg, LABEL_COLOR3, selectedColor.blue, FALSE);
        SetScrollPos(hWndScrollAlpha, SB_CTL, selectedColor.alpha, TRUE);
        SetDlgItemInt(hWndDlg, LABEL_ALPHA, selectedColor.alpha, FALSE);
    ...
    case WM_DRAWITEM: {
      ...
      switch (idCtl) {
        case BUTTON_PREVIEW: {
          ...
          if (mode == MODE_RGB) {
            rgbColor.red =
                (Real)GetDlgItemInt(hWndDlg, LABEL_COLOR1, NULL, FALSE);
            rgbColor.green =
                (Real)GetDlgItemInt(hWndDlg, LABEL_COLOR2, NULL, FALSE);
            rgbColor.blue =
                (Real)GetDlgItemInt(hWndDlg, LABEL_COLOR3, NULL, FALSE);
          }  // end if
          else {
            hsvColor.hue =
                (Real)GetDlgItemInt(hWndDlg, LABEL_COLOR1, NULL, FALSE);
          ...
          hBrushNew = CreateSolidBrush(RGB(
              (BYTE)rgbColor.red, (BYTE)rgbColor.green, (BYTE)rgbColor.blue));
        ...
        case BUTTON_COLORBAR1: {
          ...
          for (x = 0; x < (rect.right - rect.left) - 1; x++) {
            ...
            for (y = 0; y < rect.bottom; y++)
              SetPixel(lpdis->hDC, x, y,
                       RGB((BYTE)rgbColor.red, (BYTE)rgbColor.green,
                           (BYTE)rgbColor.blue));
    ...
    case WM_HSCROLL: {
      ...
      if (hWndScroll == hWndScrollColor1 || hWndScroll == hWndScrollColor2 ||
          hWndScroll == hWndScrollColor3 || hWndScroll == hWndScrollAlpha) {
        RGBColorReal rgbColor;
        HSVColorReal hsvColor;
        ...
        if (mode == MODE_RGB) {
          ...
          rgbColor.blue =
              (Real)GetDlgItemInt(hWndDlg, LABEL_COLOR3, NULL, FALSE);
          rgbColor.alpha =
              (Real)GetDlgItemInt(hWndDlg, LABEL_ALPHA, NULL, FALSE);
      ...
      switch (wID) {
        ...
        case BUTTON_RGB_HSV: {
          ...
          if (mode == MODE_RGB) {  // switch to HSV
            ...
            SetDlgItemInt(hWndDlg, LABEL_COLOR2, (Int)hsvColor.saturation,
                          FALSE);
            ...
            SetWindowText(GetDlgItem(hWndDlg, BUTTON_RGB_HSV), "Switch to RGB");
            ...
            // change the text for the button
            SetWindowText(GetDlgItem(hWndDlg, BUTTON_RGB_HSV), "Switch to HSV");
