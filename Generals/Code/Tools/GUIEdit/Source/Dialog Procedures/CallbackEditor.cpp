...
//
// Desc:			 A handy dandy little dialog to just edit the
// callbacks for
//						 user windows ... a super
//convient luxury at a bargain price!
//
//-----------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////
...
void SaveCallbacks(GameWindow *window, HWND dialog) {
  ...
  SendDlgItemMessage(dialog, COMBO_TOOLTIP, CB_GETLBTEXT, index,
                     (LPARAM)buffer);
  ...
  if (window) TheEditor->setUnsaved(TRUE);
...
static void setCurrentWindow(GameWindow *window, HWND dialog) {
  ...
  SendDlgItemMessage(dialog, COMBO_SYSTEM, CB_SELECTSTRING, -1,
                     (LPARAM)name.str());
  ...
  if (name.isEmpty()) name = GUIEDIT_NONE_STRING;
  SendDlgItemMessage(dialog, COMBO_INPUT, CB_SELECTSTRING, -1,
                     (LPARAM)name.str());
  ...
  if (name.isEmpty()) name = GUIEDIT_NONE_STRING;
  SendDlgItemMessage(dialog, COMBO_TOOLTIP, CB_SELECTSTRING, -1,
                     (LPARAM)name.str());
  ...
  if (name.isEmpty()) name = GUIEDIT_NONE_STRING;
  SendDlgItemMessage(dialog, COMBO_DRAW, CB_SELECTSTRING, -1,
                     (LPARAM)name.str());
