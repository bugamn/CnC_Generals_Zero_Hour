...
//=============================================================================
static LRESULT CALLBACK genericPropertiesCallback(HWND hWndDialog, UINT message,
                                                  WPARAM wParam,
                                                  LPARAM lParam) {
  Int returnCode;
      ...
      switch (controlID) {
        ...
        case BUTTON_HILITE_BORDER_COLOR: {
          ...
          if (currColor) {
            ...
            GetCursorPos(&mouse);
            newColor =
                SelectColor(currColor->red, currColor->green, currColor->blue,
                            currColor->alpha, mouse.x, mouse.y);

            if (newColor) {
              Color newGameColor =
                  GameMakeColor(newColor->red, newColor->green, newColor->blue,
                                newColor->alpha);
        ...
        case IDOK: {
          ...
          if (window) {
            ...
            image = ComboBoxSelectionToImage(
                GetDlgItem(hWndDialog, COMBO_ENABLED_IMAGE));
            window->winSetEnabledImage(0, image);
            rgbColor = GetControlColor(BUTTON_ENABLED_COLOR);
            color = GameMakeColor(rgbColor->red, rgbColor->green,
                                  rgbColor->blue, rgbColor->alpha);
            ...
            image = ComboBoxSelectionToImage(
                GetDlgItem(hWndDialog, COMBO_DISABLED_IMAGE));
            ...
            image = ComboBoxSelectionToImage(
                GetDlgItem(hWndDialog, COMBO_HILITE_IMAGE));
...
void InitCallbackCombos(HWND dialog, GameWindow *window) {
  ...
  if (window) editData = window->winGetEditData();
  ...
  name.clear();
  if (editData) name = editData->systemCallbackString;
  ...
  // select the current function of the window in the combo if present
  name.clear();
  if (editData) name = editData->inputCallbackString;
  if (name.isEmpty()) name = GUIEDIT_NONE_STRING;
  SendMessage(combo, CB_SELECTSTRING, -1, (LPARAM)name.str());
  ...
  // select the current function of the window in the combo if present
  name.clear();
  if (editData) name = editData->tooltipCallbackString;
  if (name.isEmpty()) name = GUIEDIT_NONE_STRING;
  SendMessage(combo, CB_SELECTSTRING, -1, (LPARAM)name.str());
  ...
  entry =
      TheFunctionLexicon->getTable(FunctionLexicon::TABLE_GAME_WIN_DEVICEDRAW);
  while (entry && entry->key != NAMEKEY_INVALID) {
    SendMessage(combo, CB_ADDSTRING, 0, (LPARAM)entry->name);
    entry++;
  }  // end while
  ...
  // select the current function of the window in the combo if present
  name.clear();
  if (editData) name = editData->drawCallbackString;
  if (name.isEmpty()) name = GUIEDIT_NONE_STRING;
  SendMessage(combo, CB_SELECTSTRING, -1, (LPARAM)name.str());
  ...
  entry = TheFunctionLexicon->getTable(
  ...
  entry =
      TheFunctionLexicon->getTable(FunctionLexicon::TABLE_WIN_LAYOUT_UPDATE);
  while (entry && entry->key != NAMEKEY_INVALID) {
    SendMessage(combo, CB_ADDSTRING, 0, (LPARAM)entry->name);
    entry++;
  }
  ...
  entry =
      TheFunctionLexicon->getTable(FunctionLexicon::TABLE_WIN_LAYOUT_SHUTDOWN);
  while (entry && entry->key != NAMEKEY_INVALID) {
    SendMessage(combo, CB_ADDSTRING, 0, (LPARAM)entry->name);
    entry++;
  }
...
HWND InitUserWinPropertiesDialog(GameWindow *window) {
  ...
  dialog = CreateDialog(
      TheEditor->getInstance(), (LPCTSTR)GENERIC_PROPERTIES_DIALOG,
      TheEditor->getWindowHandle(), (DLGPROC)genericPropertiesCallback);
  ...
  if (image)
    SendDlgItemMessage(dialog, COMBO_ENABLED_IMAGE, CB_SELECTSTRING, -1,
                       (LPARAM)image->getName().str());
  image = window->winGetDisabledImage(0);
  if (image)
    SendDlgItemMessage(dialog, COMBO_DISABLED_IMAGE, CB_SELECTSTRING, -1,
                       (LPARAM)image->getName().str());
  image = window->winGetHiliteImage(0);
  if (image)
    SendDlgItemMessage(dialog, COMBO_HILITE_IMAGE, CB_SELECTSTRING, -1,
                       (LPARAM)image->getName().str());

  // initialize the color buttons
  SetControlColor(BUTTON_ENABLED_COLOR, window->winGetEnabledColor(0));
  SetControlColor(BUTTON_ENABLED_BORDER_COLOR,
                  window->winGetEnabledBorderColor(0));
  SetControlColor(BUTTON_DISABLED_COLOR, window->winGetDisabledColor(0));
  SetControlColor(BUTTON_DISABLED_BORDER_COLOR,
                  window->winGetDisabledBorderColor(0));
  SetControlColor(BUTTON_HILITE_COLOR, window->winGetHiliteColor(0));
  SetControlColor(BUTTON_HILITE_BORDER_COLOR,
                  window->winGetHiliteBorderColor(0));
