        ...
        case IDOK: {
          ...
          if (window) {
            ...
            entryData->maxTextLen =
                GetDlgItemInt(hWndDialog, EDIT_MAX_CHARS, NULL, TRUE);
            entryData->secretText =
                IsDlgButtonChecked(hWndDialog, CHECK_SECRET_TEXT);
            entryData->aSCIIOnly =
                IsDlgButtonChecked(hWndDialog, CHECK_ASCII_TEXT);
...
HWND InitTextEntryPropertiesDialog(GameWindow *window) {
  ...
  dialog = CreateDialog(
      TheEditor->getInstance(), (LPCTSTR)TEXT_ENTRY_PROPERTIES_DIALOG,
