...
//=============================================================================
static LRESULT CALLBACK sliderPropertiesCallback(HWND hWndDialog, UINT message,
    ...
    case WM_COMMAND: {
      ...
      switch (controlID) {
        ...
        case BUTTON_SUBCONTROL_COLOR: {
          ...
          if (window) vert = BitTest(window->winGetStyle(), GWS_VERT_SLIDER);
        ...
        case IDOK: {
          ...
          if (window) {
            ...
            info = GetStateInfo(vert ? VSLIDER_ENABLED_CENTER
                                     : HSLIDER_ENABLED_CENTER);
            GadgetSliderSetEnabledImageCenter(window, info->image);

            info = GetStateInfo(vert ? VSLIDER_ENABLED_SMALL_CENTER
            ...
            info = GetStateInfo(vert ? VSLIDER_DISABLED_CENTER
                                     : HSLIDER_DISABLED_CENTER);
            GadgetSliderSetDisabledImageCenter(window, info->image);

            info = GetStateInfo(vert ? VSLIDER_DISABLED_SMALL_CENTER
            ...
            info = GetStateInfo(vert ? VSLIDER_THUMB_ENABLED_PUSHED
            ...
            GadgetSliderSetEnabledSelectedThumbBorderColor(window,
                                                           info->borderColor);

            // ----------------------------------------------------------------
            info = GetStateInfo(vert ? VSLIDER_THUMB_DISABLED
            ...
            info = GetStateInfo(vert ? VSLIDER_THUMB_DISABLED_PUSHED
            ...
            GadgetSliderSetDisabledSelectedThumbBorderColor(window,
                                                            info->borderColor);

            // ----------------------------------------------------------------
            info = GetStateInfo(vert ? VSLIDER_THUMB_HILITE
            ...
            info = GetStateInfo(vert ? VSLIDER_THUMB_HILITE_PUSHED
            ...
            GadgetSliderSetHiliteSelectedThumbBorderColor(window,
                                                          info->borderColor);
            ...
            sliderData->minVal =
                GetDlgItemInt(hWndDialog, EDIT_SLIDER_MIN, NULL, FALSE);
            sliderData->maxVal =
                GetDlgItemInt(hWndDialog, EDIT_SLIDER_MAX, NULL, FALSE);
            ...
            if (sliderData->minVal > sliderData->maxVal) {
              ...
              MessageBox(NULL,
                         "Slider min greated than max, the values were swapped",
...
HWND InitSliderPropertiesDialog(GameWindow *window) {
  ...
  dialog = CreateDialog(
      TheEditor->getInstance(), (LPCTSTR)SLIDER_PROPERTIES_DIALOG,
      TheEditor->getWindowHandle(), (DLGPROC)sliderPropertiesCallback);
  ...
  StoreImageAndColor(vert ? VSLIDER_ENABLED_TOP : HSLIDER_ENABLED_LEFT, image,
                     color, borderColor);
  ...
  StoreImageAndColor(vert ? VSLIDER_ENABLED_BOTTOM : HSLIDER_ENABLED_RIGHT,
                     image, color, borderColor);

  image = GadgetSliderGetEnabledImageCenter(window);
  StoreImageAndColor(vert ? VSLIDER_ENABLED_CENTER : HSLIDER_ENABLED_CENTER,
                     image, color, borderColor);

  image = GadgetSliderGetEnabledImageSmallCenter(window);
  StoreImageAndColor(
      vert ? VSLIDER_ENABLED_SMALL_CENTER : HSLIDER_ENABLED_SMALL_CENTER, image,
      color, borderColor);
  ...
  StoreImageAndColor(vert ? VSLIDER_DISABLED_TOP : HSLIDER_DISABLED_LEFT, image,
                     color, borderColor);
  ...
  StoreImageAndColor(vert ? VSLIDER_DISABLED_BOTTOM : HSLIDER_DISABLED_RIGHT,
                     image, color, borderColor);

  image = GadgetSliderGetDisabledImageCenter(window);
  StoreImageAndColor(vert ? VSLIDER_DISABLED_CENTER : HSLIDER_DISABLED_CENTER,
                     image, color, borderColor);

  image = GadgetSliderGetDisabledImageSmallCenter(window);
  StoreImageAndColor(
      vert ? VSLIDER_DISABLED_SMALL_CENTER : HSLIDER_DISABLED_SMALL_CENTER,
      image, color, borderColor);
  ...
  StoreImageAndColor(vert ? VSLIDER_HILITE_TOP : HSLIDER_HILITE_LEFT, image,
                     color, borderColor);
  ...
  StoreImageAndColor(vert ? VSLIDER_HILITE_BOTTOM : HSLIDER_HILITE_RIGHT, image,
                     color, borderColor);

  image = GadgetSliderGetHiliteImageCenter(window);
  StoreImageAndColor(vert ? VSLIDER_HILITE_CENTER : HSLIDER_HILITE_CENTER,
                     image, color, borderColor);

  image = GadgetSliderGetHiliteImageSmallCenter(window);
  StoreImageAndColor(
      vert ? VSLIDER_HILITE_SMALL_CENTER : HSLIDER_HILITE_SMALL_CENTER, image,
      color, borderColor);
  ...
  StoreImageAndColor(vert ? VSLIDER_THUMB_ENABLED : HSLIDER_THUMB_ENABLED,
                     image, color, borderColor);
  ...
  StoreImageAndColor(
      vert ? VSLIDER_THUMB_ENABLED_PUSHED : HSLIDER_THUMB_ENABLED_PUSHED, image,
      color, borderColor);
  ...
  StoreImageAndColor(vert ? VSLIDER_THUMB_DISABLED : HSLIDER_THUMB_DISABLED,
                     image, color, borderColor);
  ...
  StoreImageAndColor(
      vert ? VSLIDER_THUMB_DISABLED_PUSHED : HSLIDER_THUMB_DISABLED_PUSHED,
      image, color, borderColor);
  ...
  StoreImageAndColor(vert ? VSLIDER_THUMB_HILITE : HSLIDER_THUMB_HILITE, image,
                     color, borderColor);
  ...
  StoreImageAndColor(
      vert ? VSLIDER_THUMB_HILITE_PUSHED : HSLIDER_THUMB_HILITE_PUSHED, image,
      color, borderColor);
