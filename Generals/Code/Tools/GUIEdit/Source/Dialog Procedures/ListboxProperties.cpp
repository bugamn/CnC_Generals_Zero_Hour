...
static void addScrollbar(GameWindow *listbox) {
  ...
  Color enabledSelectedItemBorder =
      GadgetListBoxGetEnabledSelectedItemBorderColor(listbox);

  Color disabled = GadgetListBoxGetDisabledColor(listbox);
  Color disabledBorder = GadgetListBoxGetDisabledBorderColor(listbox);
  Color disabledSelectedItem =
      GadgetListBoxGetDisabledSelectedItemColor(listbox);
  Color disabledSelectedItemBorder =
      GadgetListBoxGetDisabledSelectedItemBorderColor(listbox);
  ...
  Color hiliteSelectedItemBorder =
      GadgetListBoxGetHiliteSelectedItemBorderColor(listbox);

  GadgetListBoxSetColors(listbox, enabled, enabledBorder, enabledSelectedItem,
                         enabledSelectedItemBorder, disabled, disabledBorder,
                         disabledSelectedItem, disabledSelectedItemBorder,
                         hilite, hiliteBorder, hiliteSelectedItem,
  ...
  if (downButton) {
    ...
    info =
        TheDefaultScheme->getImageAndColor(LISTBOX_DOWN_BUTTON_ENABLED_PUSHED);
    ...
    info =
        TheDefaultScheme->getImageAndColor(LISTBOX_DOWN_BUTTON_DISABLED_PUSHED);
  ...
  if (slider) {
    ...
    info =
        TheDefaultScheme->getImageAndColor(LISTBOX_SLIDER_ENABLED_SMALL_CENTER);
    ...
    info =
        TheDefaultScheme->getImageAndColor(LISTBOX_SLIDER_HILITE_SMALL_CENTER);
    ...
    info =
        TheDefaultScheme->getImageAndColor(LISTBOX_SLIDER_THUMB_ENABLED_PUSHED);
    ...
    info =
        TheDefaultScheme->getImageAndColor(LISTBOX_SLIDER_THUMB_HILITE_PUSHED);
...
//=============================================================================
static LRESULT CALLBACK listboxPropertiesCallback(HWND hWndDialog, UINT message,
                                                  WPARAM wParam,
                                                  LPARAM lParam) {
  Int returnCode;
    ...
    case WM_COMMAND: {
      ...
      switch (controlID) {
        ...
        case BUTTON_SUBCONTROL_COLOR: {
          ...
          StoreColor(LISTBOX_DOWN_BUTTON_ENABLED, info->color,
                     info->borderColor);
          StoreColor(LISTBOX_SLIDER_ENABLED_TOP, info->color,
                     info->borderColor);
          StoreColor(LISTBOX_SLIDER_THUMB_ENABLED, info->color,
                     info->borderColor);

          info = GetStateInfo(LISTBOX_ENABLED_SELECTED_ITEM_LEFT);
          StoreColor(LISTBOX_UP_BUTTON_ENABLED_PUSHED, info->color,
                     info->borderColor);
          StoreColor(LISTBOX_DOWN_BUTTON_ENABLED_PUSHED, info->color,
                     info->borderColor);
          StoreColor(LISTBOX_SLIDER_THUMB_ENABLED_PUSHED, info->color,
                     info->borderColor);

          info = GetStateInfo(LISTBOX_DISABLED);
          StoreColor(LISTBOX_UP_BUTTON_DISABLED, info->color,
                     info->borderColor);
          StoreColor(LISTBOX_DOWN_BUTTON_DISABLED, info->color,
                     info->borderColor);
          StoreColor(LISTBOX_SLIDER_DISABLED_TOP, info->color,
                     info->borderColor);
          StoreColor(LISTBOX_SLIDER_THUMB_DISABLED, info->color,
                     info->borderColor);

          info = GetStateInfo(LISTBOX_DISABLED_SELECTED_ITEM_LEFT);
          StoreColor(LISTBOX_UP_BUTTON_DISABLED_PUSHED, info->color,
                     info->borderColor);
          StoreColor(LISTBOX_DOWN_BUTTON_DISABLED_PUSHED, info->color,
                     info->borderColor);
          StoreColor(LISTBOX_SLIDER_THUMB_DISABLED_PUSHED, info->color,
                     info->borderColor);

          info = GetStateInfo(LISTBOX_HILITE);
          StoreColor(LISTBOX_UP_BUTTON_HILITE, info->color, info->borderColor);
          StoreColor(LISTBOX_DOWN_BUTTON_HILITE, info->color,
                     info->borderColor);
          StoreColor(LISTBOX_SLIDER_HILITE_TOP, info->color, info->borderColor);
          StoreColor(LISTBOX_SLIDER_THUMB_HILITE, info->color,
                     info->borderColor);

          info = GetStateInfo(LISTBOX_HILITE_SELECTED_ITEM_LEFT);
          StoreColor(LISTBOX_UP_BUTTON_HILITE_PUSHED, info->color,
                     info->borderColor);
          StoreColor(LISTBOX_DOWN_BUTTON_HILITE_PUSHED, info->color,
                     info->borderColor);
          StoreColor(LISTBOX_SLIDER_THUMB_HILITE_PUSHED, info->color,
                     info->borderColor);
        ...
        case IDOK: {
          ...
          if (window) {
            ...
            GadgetListBoxSetEnabledSelectedItemBorderColor(window,
                                                           info->borderColor);
            ...
            GadgetListBoxSetEnabledSelectedItemImageSmallCenter(window,
                                                                info->image);
            ...
            GadgetListBoxSetDisabledSelectedItemBorderColor(window,
                                                            info->borderColor);
            ...
            GadgetListBoxSetDisabledSelectedItemImageCenter(window,
                                                            info->image);
            info = GetStateInfo(LISTBOX_DISABLED_SELECTED_ITEM_SMALL_CENTER);
            GadgetListBoxSetDisabledSelectedItemImageSmallCenter(window,
                                                                 info->image);
            ...
            GadgetListBoxSetHiliteSelectedItemBorderColor(window,
                                                          info->borderColor);
            ...
            GadgetListBoxSetHiliteSelectedItemImageSmallCenter(window,
                                                               info->image);
            ...
            listData->forceSelect =
                IsDlgButtonChecked(hWndDialog, CHECK_FORCE_SELECT);
            listData->autoScroll =
                IsDlgButtonChecked(hWndDialog, CHECK_AUTO_SCROLL);
            listData->scrollIfAtEnd =
                IsDlgButtonChecked(hWndDialog, CHECK_SCROLL_IF_AT_END);
            listData->autoPurge =
                IsDlgButtonChecked(hWndDialog, CHECK_AUTO_PURGE);

            // addition or subtraction of a scroll bar
            Bool wantScrollBar =
                IsDlgButtonChecked(hWndDialog, CHECK_HAS_SCROLLBAR);
            ...
            Int newMaxItems =
                GetDlgItemInt(hWndDialog, EDIT_MAX_ITEMS, NULL, FALSE);
            if (newMaxItems != listData->listLength)
              GadgetListBoxSetListLength(window, newMaxItems);

            // multi-select
            Bool wantMultiSelect =
                IsDlgButtonChecked(hWndDialog, CHECK_MULTI_SELECT);
            ...
            // Multi-column
            Int newColumns =
                GetDlgItemInt(hWndDialog, EDIT_NUM_COLUMNS, NULL, FALSE);
            ...
            if (newColumns > 1) {
              ...
              if (strlen(percentages) == 0) {
                MessageBox(NULL,
                           "You have specified a column amount greater then 1, "
                           "please enter the same about of percentages",
                           "whoops", MB_OK | MB_ICONSTOP | MB_APPLMODAL);
                break;
              }
              ...
              while (token != NULL) {
                ...
                if (i > newColumns && token) {
                  ...
                  sprintf(whoopsMsg,
                          "You have Specified %d columns but I have read in "
                          "more then that for the percentages, please double "
                          "check your data",
                          newColumns);
                  MessageBox(NULL, whoopsMsg, "Whoops",
                  ...
                  sprintf(
                      whoopsMsg,
                      "You have Specified %d columns but I have read in only "
                      "%d for the percentages, please double check your data",
                      newColumns, i);
                  MessageBox(NULL, whoopsMsg, "Whoops",
                             MB_OK | MB_ICONSTOP | MB_APPLMODAL);
                  return 0;
                  ...
                  Char *whoopsMsg = new char[250];
                  sprintf(whoopsMsg,
                          "Please Double check to make sure your percentages "
                          "add up to 100.",
                          newColumns, i - 1);
                  MessageBox(NULL, whoopsMsg, "Whoops",
                             MB_OK | MB_ICONSTOP | MB_APPLMODAL);
...
HWND InitListboxPropertiesDialog(GameWindow *window) {
  ...
  dialog = CreateDialog(
      TheEditor->getInstance(), (LPCTSTR)LISTBOX_PROPERTIES_DIALOG,
      TheEditor->getWindowHandle(), (DLGPROC)listboxPropertiesCallback);
  ...
  StoreImageAndColor(LISTBOX_ENABLED_SELECTED_ITEM_LEFT, image, color,
                     borderColor);
  ...
  StoreImageAndColor(LISTBOX_DISABLED_SELECTED_ITEM_LEFT, image, color,
                     borderColor);
  ...
  StoreImageAndColor(LISTBOX_HILITE_SELECTED_ITEM_LEFT, image, color,
                     borderColor);
  ...
  if (upButton) {
    ...
    StoreImageAndColor(LISTBOX_UP_BUTTON_ENABLED_PUSHED, image, color,
                       borderColor);
    ...
    StoreImageAndColor(LISTBOX_UP_BUTTON_DISABLED_PUSHED, image, color,
                       borderColor);
    ...
    StoreImageAndColor(LISTBOX_UP_BUTTON_HILITE_PUSHED, image, color,
                       borderColor);
  ...
  if (downButton) {
    ...
    StoreImageAndColor(LISTBOX_DOWN_BUTTON_ENABLED_PUSHED, image, color,
                       borderColor);
    ...
    StoreImageAndColor(LISTBOX_DOWN_BUTTON_DISABLED_PUSHED, image, color,
                       borderColor);
    ...
    StoreImageAndColor(LISTBOX_DOWN_BUTTON_HILITE_PUSHED, image, color,
                       borderColor);
  ...
  if (slider) {
    ...
    StoreImageAndColor(LISTBOX_SLIDER_ENABLED_BOTTOM, image, color,
                       borderColor);

    image = GadgetSliderGetEnabledImageCenter(slider);
    StoreImageAndColor(LISTBOX_SLIDER_ENABLED_CENTER, image, color,
                       borderColor);

    image = GadgetSliderGetEnabledImageSmallCenter(slider);
    StoreImageAndColor(LISTBOX_SLIDER_ENABLED_SMALL_CENTER, image, color,
                       borderColor);
    ...
    StoreImageAndColor(LISTBOX_SLIDER_DISABLED_BOTTOM, image, color,
                       borderColor);

    image = GadgetSliderGetDisabledImageCenter(slider);
    StoreImageAndColor(LISTBOX_SLIDER_DISABLED_CENTER, image, color,
                       borderColor);

    image = GadgetSliderGetDisabledImageSmallCenter(slider);
    StoreImageAndColor(LISTBOX_SLIDER_DISABLED_SMALL_CENTER, image, color,
                       borderColor);
    ...
    StoreImageAndColor(LISTBOX_SLIDER_HILITE_SMALL_CENTER, image, color,
                       borderColor);
    ...
    StoreImageAndColor(LISTBOX_SLIDER_THUMB_ENABLED_PUSHED, image, color,
                       borderColor);
    ...
    StoreImageAndColor(LISTBOX_SLIDER_THUMB_DISABLED, image, color,
                       borderColor);
    ...
    StoreImageAndColor(LISTBOX_SLIDER_THUMB_DISABLED_PUSHED, image, color,
                       borderColor);
    ...
    StoreImageAndColor(LISTBOX_SLIDER_THUMB_HILITE_PUSHED, image, color,
                       borderColor);
  ...
  if (listData->columns > 1) {
    ...
    for (Int i = 1; i < listData->columns; i++) {
      strcat(percentages, ",");
      strcat(percentages,
             itoa(listData->columnWidthPercentage[i], tempStr, 10));
    }
