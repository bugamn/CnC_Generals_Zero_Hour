...
///////////////////////////////////////////////////////////////////////////////
static char
    buffer[BUFFER_SIZE];  ///< buffer for writing data to before file output
...
static void clearBufferToSpaces(void) {
  ...
  for (i = 0; i < BUFFER_SIZE; i++) buffer[i] = ' ';
...
static Bool saveStatus(GameWindow *window, FILE *fp, Int dataIndent) {
  ...
  while (WindowStatusNames[i]) {
    ...
    if (BitTest(window->winGetStatus(), bit)) {
      ...
      if (bitWritten == TRUE) strcat(buffer, "+");
  ...
  if (bitWritten == FALSE) strcat(buffer, "NONE");
...
static Bool saveCallbacks(GameWindow *window, FILE *fp, Int dataIndent) {
  ...
  if (name.isEmpty()) name = GUIEDIT_NONE_STRING;
...
static Bool saveHeaderTemplate(GameWindow *window, FILE *fp, Int dataIndent) {
  ...
  if (headerName.isEmpty()) headerName = GUIEDIT_NONE_STRING;
...
static Bool saveTextColor(GameWindow *window, FILE *fp, Int dataIndent) {
  ...
  for (i = 0; i < max; i++) {
    switch (i) {
      case 0:
        color = window->winGetEnabledTextColor();
        border = window->winGetEnabledTextBorderColor();
        break;
      case 1:
        color = window->winGetDisabledTextColor();
        border = window->winGetDisabledTextBorderColor();
        break;
      case 2:
        color = window->winGetHiliteTextColor();
    ...
    if (i == 0)
      sprintf(
          &buffer[dataIndent],
          "TEXTCOLOR = ENABLED:  %d %d %d %d, ENABLEDBORDER:  %d %d %d %d,\n",
          r, g, b, a, br, bg, bb, ba);
    else if (i == max - 1)
      sprintf(
          &buffer[dataIndent],
          "            HILITE:   %d %d %d %d, HILITEBORDER:   %d %d %d %d;\n",
          r, g, b, a, br, bg, bb, ba);
    else
      sprintf(
          &buffer[dataIndent],
          "            DISABLED: %d %d %d %d, DISABLEDBORDER: %d %d %d %d,\n",
          r, g, b, a, br, bg, bb, ba);
    writeBufferToFile(fp, buffer);
...
//=============================================================================
static Bool saveDrawData(char *token, GameWindow *window, FILE *fp,
                         Int dataIndent) {
  Int i;
  WinInstanceData *instData = window->winGetInstanceData();
  char spaces[128];
  ...
  for (i = 0; i < MAX_DRAW_DATA; i++) {
    ...
    if (i == 0)
      sprintf(&buffer[dataIndent],
              "%s = IMAGE: %s, COLOR: %d %d %d %d, BORDERCOLOR: %d %d %d %d,\n",
              token, image ? image->getName().str() : "NoImage", r, g, b, a, br,
              bg, bb, ba);
    else if (i == MAX_DRAW_DATA - 1)
      sprintf(&buffer[dataIndent],
              "%s   IMAGE: %s, COLOR: %d %d %d %d, BORDERCOLOR: %d %d %d %d;\n",
              spaces, image ? image->getName().str() : "NoImage", r, g, b, a,
              br, bg, bb, ba);
    else
      sprintf(&buffer[dataIndent],
              "%s   IMAGE: %s, COLOR: %d %d %d %d, BORDERCOLOR: %d %d %d %d,\n",
...
static Bool saveListboxData(GameWindow *window, FILE *fp, Int dataIndent) {
  ...
  if (listData->upButton) {
    saveDrawData("LISTBOXENABLEDUPBUTTONDRAWDATA", listData->upButton, fp,
                 dataIndent);
    saveDrawData("LISTBOXDISABLEDUPBUTTONDRAWDATA", listData->upButton, fp,
                 dataIndent);
    saveDrawData("LISTBOXHILITEUPBUTTONDRAWDATA", listData->upButton, fp,
                 dataIndent);
  ...
  if (listData->downButton) {
    saveDrawData("LISTBOXENABLEDDOWNBUTTONDRAWDATA", listData->downButton, fp,
                 dataIndent);
    saveDrawData("LISTBOXDISABLEDDOWNBUTTONDRAWDATA", listData->downButton, fp,
                 dataIndent);
    saveDrawData("LISTBOXHILITEDOWNBUTTONDRAWDATA", listData->downButton, fp,
                 dataIndent);
  ...
  if (listData->slider) {
    ...
    saveDrawData("LISTBOXENABLEDSLIDERDRAWDATA", listData->slider, fp,
                 dataIndent);
    saveDrawData("LISTBOXDISABLEDSLIDERDRAWDATA", listData->slider, fp,
                 dataIndent);
    saveDrawData("LISTBOXHILITESLIDERDRAWDATA", listData->slider, fp,
                 dataIndent);
    if (thumb) {
...
static Bool saveComboBoxData(GameWindow *window, FILE *fp, Int dataIndent) {
  ...
  if (comboData->dropDownButton) {
    saveDrawData("COMBOBOXDROPDOWNBUTTONENABLEDDRAWDATA",
                 comboData->dropDownButton, fp, dataIndent);
    saveDrawData("COMBOBOXDROPDOWNBUTTONDISABLEDDRAWDATA",
                 comboData->dropDownButton, fp, dataIndent);
    saveDrawData("COMBOBOXDROPDOWNBUTTONHILITEDRAWDATA",
                 comboData->dropDownButton, fp, dataIndent);

  }  // end if

  if (comboData->editBox) {
    saveDrawData("COMBOBOXEDITBOXENABLEDDRAWDATA", comboData->editBox, fp,
                 dataIndent);
    saveDrawData("COMBOBOXEDITBOXDISABLEDDRAWDATA", comboData->editBox, fp,
                 dataIndent);
    saveDrawData("COMBOBOXEDITBOXHILITEDRAWDATA", comboData->editBox, fp,
                 dataIndent);
  ...
  if (comboData->listBox) {
    ...
    saveDrawData("COMBOBOXLISTBOXENABLEDDRAWDATA", comboData->listBox, fp,
                 dataIndent);
    saveDrawData("COMBOBOXLISTBOXDISABLEDDRAWDATA", comboData->listBox, fp,
                 dataIndent);
    saveDrawData("COMBOBOXLISTBOXHILITEDRAWDATA", comboData->listBox, fp,
                 dataIndent);
    ...
    if (listData->downButton) {
      saveDrawData("LISTBOXENABLEDDOWNBUTTONDRAWDATA", listData->downButton, fp,
                   dataIndent);
      saveDrawData("LISTBOXDISABLEDDOWNBUTTONDRAWDATA", listData->downButton,
                   fp, dataIndent);
      saveDrawData("LISTBOXHILITEDOWNBUTTONDRAWDATA", listData->downButton, fp,
                   dataIndent);
...
static Bool saveTabControlData(GameWindow *window, FILE *fp, Int dataIndent) {
  ...
  writeBufferToFile(fp, buffer);
  sprintf(&buffer[dataIndent], "                 PANEDISABLED: %d,",
          NUM_TAB_PANES);  // number of entries to follow
  writeBufferToFile(fp, buffer);
  for (Int paneIndex = 0; paneIndex < NUM_TAB_PANES; paneIndex++) {
    if (paneIndex == NUM_TAB_PANES - 1)
      sprintf(&buffer[dataIndent], "%d;",
              tabControlData->subPaneDisabled[paneIndex]);
    else
      sprintf(&buffer[dataIndent], "%d,",
              tabControlData->subPaneDisabled[paneIndex]);
...
static Bool saveGadgetData(GameWindow *window, FILE *fp, Int dataIndent) {
  ...
  else if (BitTest(window->winGetStyle(), GWS_VERT_SLIDER | GWS_HORZ_SLIDER))
...
static Bool saveWindow(FILE *fp, GameWindow *window, Int indent) {
  ...
  if (BitTest(window->winGetStyle(),
              GWS_TAB_CONTROL)) {  // Seems cleaner to put this before the
                                   // children list since this Gadget needs both
  ...
  if ((TheEditor->windowIsGadget(window) == FALSE) ||
      BitTest(window->winGetStyle(), GWS_TAB_CONTROL)) {
    child = window->winGetChild();
    if (child) {
      // traverse to end of child list
      while (child->winGetNext() != NULL) child = child->winGetNext();
...
void GUIEdit::validateNames(GameWindow *root, char *filename, Bool *valid) {
  ...
  if (strlen(filename) >= MAX_WINDOW_NAME_LEN) {
    sprintf(offendingNames,
            "Filename '%s' is too long.  The max is '%d' for filename AND "
            "window name!\n",
  ...
  if (strlen(filename) + instData->m_decoratedNameString.getLength() >=
      MAX_WINDOW_NAME_LEN) {
  ...
  if (TheEditor->isNameDuplicate(TheWindowManager->winGetWindowList(), root,
...
Bool GUIEdit::saveData(char *filePathAndFilename, char *filename) {
  ...
  // sanity
  if (filePathAndFilename == NULL) return FALSE;
  ...
  Bool valid = TRUE;
  sprintf(offendingNames,
          "\nOne or more window names are illegal.  A window name PLUS the "
          "filename must be under '%d' characters and names cannot be "
          "duplicates.\n\nList of illegal window names:\n\n",
          MAX_WINDOW_NAME_LEN);
  validateNames(window, filename, &valid);
  if (valid == FALSE) {
    MessageBox(TheEditor->getWindowHandle(), offendingNames,
               "Window Name Error", MB_OK);
    return FALSE;

  }  // end if

  // update all radio button screen identifiers with the filename
  updateRadioScreenIdentifiers(
  ...
  window = TheWindowManager->winGetWindowList();
  while (window->winGetNext() != NULL) window = window->winGetNext();
