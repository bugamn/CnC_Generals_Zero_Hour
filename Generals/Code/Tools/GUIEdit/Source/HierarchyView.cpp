...
//
// Desc:			 Manipulation the widows heirarchy through the
// tree
//
//-----------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////
  ...
  switch (message) {
    ...
    case WM_SIZE: {
      ...
      if (TheHierarchyView) {
        ...
        if (tree) {
          ...
          MoveWindow(tree, border, p.y, width - (border * 2),
                     (height - border) - p.y, TRUE);
    ...
    case WM_MOUSEMOVE: {
      ...
      if (dragWindow) {
        ...
        HTREEITEM overItem =
            TheHierarchyView->treePointToItem(treePoint.x, treePoint.y);
        if (overItem) {
        ...
        if (overItem) {
          ...
          if (TheHierarchyView->validateDragDropOperation(dragWindow,
            ...
            if (TheEditor->windowIsGadget(overWindow) == FALSE) {
              ...
              menu = LoadMenu(TheEditor->getInstance(),
                              (LPCTSTR)HIERARCHY_DRAG_DROP_MENU);
              subMenu = GetSubMenu(menu, 0);
              screen.x = x;
              screen.y = y;
              ClientToScreen(hWndDialog, &screen);
              TrackPopupMenuEx(subMenu, 0, screen.x, screen.y, hWndDialog,
                               NULL);
    ...
    case WM_RBUTTONUP: {
      ...
      if (overItem) {
        ...
        menu =
            LoadMenu(TheEditor->getInstance(), (LPCTSTR)HIERARCHY_POPUP_MENU);
        subMenu = GetSubMenu(menu, 0);
        screen.x = x;
        screen.y = y;
        ClientToScreen(hWndDialog, &screen);
          ...
          switch (notifyMessageHandler->code) {
            ...
            case TVN_SELCHANGED: {
              ...
              if (treeNotify->action != TVC_UNKNOWN) {
                ...
                TheEditor->clearSelections();
                if (window) TheEditor->selectWindow(window);
            ...
            case NM_DBLCLK: {
              ...
              HTREEITEM selected =
                  TreeView_GetSelection(TheHierarchyView->getTreeHandle());
              if (selected) {
                GameWindow *overWindow;
    ...
    case WM_COMMAND: {
      ...
      switch (controlID) {
        ...
        case HIERARCHY_POPUP_DELETE: {
          ...
          if (target) TheEditor->deleteWindow(target);
...
HTREEITEM HierarchyView::findItemEntry(HTREEITEM node, GameWindow *window) {
  // end of recursion
  if (node == NULL || window == NULL) return NULL;
  ...
  if ((GameWindow *)item.lParam == window) return node;

  // not there, check our children
  HTREEITEM child;
  HTREEITEM found = NULL;
  for (child = TreeView_GetNextItem(m_tree, node, TVGN_CHILD); child;
...
//=============================================================================
void HierarchyView::addWindowToTree(GameWindow *window, HTREEITEM treeParent,
  ...
  if (newItem == NULL) {
    ...
    insert.itemex.lParam =
        (LPARAM)window;  // attach window to this item in lParam
  ...
  //
  if (addChildren && TheEditor->windowIsGadget(window) == FALSE ||
  ...
  if (addSiblings)
    addWindowToTree(window->winGetNext(), treeParent, option, addChildren,
                    addSiblings);
...
void HierarchyView::init(void) {
  ...
  m_dialog = CreateDialog(TheEditor->getInstance(), (LPCSTR)HIERARCHY_DIALOG,
                          TheEditor->getWindowHandle(), (DLGPROC)dialogProc);
  ...
  MoveWindow(m_dialog, appRect.right - dialogSize.x, 0, dialogSize.x,
             dialogSize.y, TRUE);
...
void HierarchyView::addWindow(GameWindow *window, HierarchyOption option) {
  // sanity
  if (window == NULL || m_dialog == NULL) return;

  // do not add again if already in the tree
  if (findTreeEntry(window) != NULL) return;
...
void HierarchyView::removeWindow(GameWindow *window) {
  ...
  if (window == m_dragWindow) m_dragWindow = NULL;

  // clean up drag target
  if (window == m_dragTarget) m_dragTarget = NULL;

  // if this window is the popup target remove it
  if (window == m_popupTarget) m_popupTarget = NULL;
...
void HierarchyView::setDialogPos(ICoord2D *pos) {
  // sanity
  if (pos) dialogPos = *pos;

  MoveWindow(m_dialog, dialogPos.x, dialogPos.y, dialogSize.x, dialogSize.y,
             TRUE);
...
void HierarchyView::selectWindow(GameWindow *window) {
  ...
  if (window) item = findTreeEntry(window);
