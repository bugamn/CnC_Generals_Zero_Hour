...
static void loadSchemeDataToDialog(HWND hWndDialog) {
  ...
  LoadStateCombo(GWS_PUSH_BUTTON | GWS_RADIO_BUTTON | GWS_CHECK_BOX |
                     GWS_PROGRESS_BAR | GWS_SCROLL_LISTBOX | GWS_HORZ_SLIDER |
                     GWS_VERT_SLIDER | GWS_STATIC_TEXT | GWS_ENTRY_FIELD |
                     GWS_USER_WINDOW | GWS_COMBO_BOX,
  ...
  LoadTextStateCombo(
      GetDlgItem(hWndDialog, COMBO_TEXT_STATE),
      theScheme->getEnabledTextColor(), theScheme->getEnabledTextBorderColor(),
      theScheme->getDisabledTextColor(),
      theScheme->getDisabledTextBorderColor(), theScheme->getHiliteTextColor(),
...
static void saveData(HWND hWndDialog) {
  ...
  for (i = FIRST_VALID_IDENTIFIER; i < NUM_STATE_IDENTIFIERS; i++) {
    ...
    assert(info);
    theScheme->storeImageAndColor(info->stateID, info->image, info->color,
                                  info->borderColor);
  ...
  // save the font
  theScheme->setFont(
      GetSelectedFontFromCombo(GetDlgItem(hWndDialog, COMBO_FONT)));
...
char *openDialog(void) {
  ...
  ofn.lpstrInitialDir = NULL;
  ofn.lpstrTitle = NULL;
  ofn.Flags = OFN_NOREADONLYRETURN | OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST |
              OFN_NOCHANGEDIR;
  ofn.nFileOffset = 0;
  ofn.nFileExtension = 0;
  ofn.lpstrDefExt = "ls";
...
//=============================================================================
static LRESULT CALLBACK layoutSchemeCallback(HWND hWndDialog, UINT message,
  ...
  //
  if (HandleCommonDialogMessages(hWndDialog, message, wParam, lParam,
                                 &returnCode) == TRUE)
    return returnCode;
  ...
  switch (message) {
    ...
    case WM_COMMAND: {
      ...
      //			HWND hWndControl = (HWND)lParam;  // control
      //window handle
      ...
      switch (controlID) {
        ...
        case BUTTON_APPLY_SCHEME: {
          ...
          result = MessageBox(
              NULL,
              "This will apply these scheme color and image settings to ALL "
              "windows and gadgets currently loaded in the edit window.  Are "
          ...
          //
          theScheme->applyPropertyTablesToWindow(
          ...
          //
          theScheme->applyPropertyTablesToWindow(
              TheGUIEditWindowManager->getClipboardList());
          theScheme->applyPropertyTablesToWindow(
              TheGUIEditWindowManager->getClipboardDupeList());
        ...
        case BUTTON_LOAD: {
          ...
          if (theScheme && filename) {
            ...
            if (theScheme->loadScheme(filename)) {
              // load the dialog engine and items with the current data of the
              // scheme
              loadSchemeDataToDialog(hWndDialog);
...
void LayoutScheme::applyPropertyTablesToWindow(GameWindow *root) {
  ...
  if (BitTest(root->winGetStyle(), GWS_PUSH_BUTTON)) {
    ...
    if (slider) {
      ...
      GadgetSliderSetDisabledSelectedThumbBorderColor(slider,
                                                      info->borderColor);
    ...
    if (dropDownButton) {
      ...
      GadgetButtonSetEnabledSelectedBorderColor(dropDownButton,
                                                info->borderColor);
      ...
      GadgetButtonSetDisabledSelectedBorderColor(dropDownButton,
                                                 info->borderColor);
      ...
      GadgetButtonSetHiliteSelectedBorderColor(dropDownButton,
                                               info->borderColor);
    }  // end if
    ...
    if (listBox) {
      ...
      GadgetListBoxSetEnabledSelectedItemBorderColor(listBox,
                                                     info->borderColor);
      ...
      GadgetListBoxSetDisabledSelectedItemBorderColor(listBox,
                                                      info->borderColor);
      ...
      GadgetListBoxSetDisabledSelectedItemImageSmallCenter(listBox,
                                                           info->image);
      ...
      if (downButton) {
        ...
        GadgetButtonSetEnabledSelectedImage(downButton, info->image);
        GadgetButtonSetEnabledSelectedColor(downButton, info->color);
        GadgetButtonSetEnabledSelectedBorderColor(downButton,
                                                  info->borderColor);
        ...
        GadgetButtonSetDisabledSelectedImage(downButton, info->image);
        GadgetButtonSetDisabledSelectedColor(downButton, info->color);
        GadgetButtonSetDisabledSelectedBorderColor(downButton,
                                                   info->borderColor);
      ...
      if (slider) {
        ...
        GadgetSliderSetEnabledSelectedThumbImage(slider, info->image);
        GadgetSliderSetEnabledSelectedThumbColor(slider, info->color);
        GadgetSliderSetEnabledSelectedThumbBorderColor(slider,
                                                       info->borderColor);
        ...
        GadgetSliderSetHiliteSelectedThumbImage(slider, info->image);
        GadgetSliderSetHiliteSelectedThumbColor(slider, info->color);
        GadgetSliderSetHiliteSelectedThumbBorderColor(slider,
                                                      info->borderColor);
...
LayoutScheme::LayoutScheme(void) {
  ...
  memset(&m_imageAndColorTable, 0,
         sizeof(ImageAndColorInfo) * NUM_STATE_IDENTIFIERS);
...
void LayoutScheme::init(void) {
  ...
  //	Color cyan				= GameMakeColor(   0, 255, 255,
  //alpha ); 	Color darkCyan		= GameMakeColor(  64, 128, 128, alpha );
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("RadioButtonHiliteBoxSelected"));
  storeImageAndColor(RADIO_HILITE_CHECKED_BOX, image, yellow, white);

  // check box
  image =
      TheMappedImageCollection->findImageByName(AsciiString("CheckBoxEnabled"));
  storeImageAndColor(CHECK_BOX_ENABLED, image, red, lightRed);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("CheckBoxEnabledBoxUnselected"));
  storeImageAndColor(CHECK_BOX_ENABLED_UNCHECKED_BOX, image,
                     WIN_COLOR_UNDEFINED, lightBlue);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("CheckBoxEnabledBoxSelected"));
  storeImageAndColor(CHECK_BOX_ENABLED_CHECKED_BOX, image, blue, lightBlue);

  image = TheMappedImageCollection->findImageByName(
      AsciiString("CheckBoxDisabled"));
  storeImageAndColor(CHECK_BOX_DISABLED, image, gray, lightGray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("CheckBoxDisabledBoxUnselected"));
  storeImageAndColor(CHECK_BOX_DISABLED_UNCHECKED_BOX, image,
                     WIN_COLOR_UNDEFINED, lightGray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("CheckBoxDisabledBoxSelected"));
  storeImageAndColor(CHECK_BOX_DISABLED_CHECKED_BOX, image, darkGray, white);

  image =
      TheMappedImageCollection->findImageByName(AsciiString("CheckBoxHilite"));
  storeImageAndColor(CHECK_BOX_HILITE, image, green, lightGreen);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("CheckBoxHiliteBoxUnselected"));
  storeImageAndColor(CHECK_BOX_HILITE_UNCHECKED_BOX, image, WIN_COLOR_UNDEFINED,
                     lightBlue);
  image = TheMappedImageCollection->findImageByName(
  ...
  // horz slider
  image = TheMappedImageCollection->findImageByName(
      AsciiString("HSliderEnabledLeftEnd"));
  storeImageAndColor(HSLIDER_ENABLED_LEFT, image, red, lightRed);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("HSliderEnabledRightEnd"));
  storeImageAndColor(HSLIDER_ENABLED_RIGHT, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("HSliderEnabledRepeatingCenter"));
  storeImageAndColor(HSLIDER_ENABLED_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("HSliderEnabledSmallRepeatingCenter"));
  storeImageAndColor(HSLIDER_ENABLED_SMALL_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("HSliderDisabledRightEnd"));
  storeImageAndColor(HSLIDER_DISABLED_RIGHT, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("HSliderDisabledRepeatingCenter"));
  storeImageAndColor(HSLIDER_DISABLED_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("HSliderDisabledSmallRepeatingCenter"));
  storeImageAndColor(HSLIDER_DISABLED_SMALL_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("HSliderHiliteRightEnd"));
  storeImageAndColor(HSLIDER_HILITE_RIGHT, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("HSliderHiliteRepeatingCenter"));
  storeImageAndColor(HSLIDER_HILITE_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("HSliderHiliteSmallRepeatingCenter"));
  storeImageAndColor(HSLIDER_HILITE_SMALL_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);

  image = TheMappedImageCollection->findImageByName(
      AsciiString("HSliderThumbEnabled"));
  storeImageAndColor(HSLIDER_THUMB_ENABLED, image, lightRed, red);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("HSliderThumbEnabledSelected"));
  storeImageAndColor(HSLIDER_THUMB_ENABLED_PUSHED, image, gray, lightGray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("HSliderThumbDisabled"));
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("HSliderThumbHilite"));
  storeImageAndColor(HSLIDER_THUMB_HILITE, image, green, lightGreen);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("HSliderThumbHiliteSelected"));
  storeImageAndColor(HSLIDER_THUMB_HILITE_PUSHED, image, blue, lightBlue);

  // vert slider
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderEnabledTopEnd"));
  storeImageAndColor(VSLIDER_ENABLED_TOP, image, red, lightRed);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderEnabledBottomEnd"));
  storeImageAndColor(VSLIDER_ENABLED_BOTTOM, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderEnabledRepeatingCenter"));
  storeImageAndColor(VSLIDER_ENABLED_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderEnabledSmallRepeatingCenter"));
  storeImageAndColor(VSLIDER_ENABLED_SMALL_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);

  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderDisabledTopEnd"));
  storeImageAndColor(VSLIDER_DISABLED_TOP, image, gray, darkGray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderDisabledBottomEnd"));
  storeImageAndColor(VSLIDER_DISABLED_BOTTOM, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderDisabledRepeatingCenter"));
  storeImageAndColor(VSLIDER_DISABLED_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderDisabledSmallRepeatingCenter"));
  storeImageAndColor(VSLIDER_DISABLED_SMALL_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);

  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderHiliteTopEnd"));
  storeImageAndColor(VSLIDER_HILITE_TOP, image, green, darkGreen);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderHiliteBottomEnd"));
  storeImageAndColor(VSLIDER_HILITE_BOTTOM, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderHiliteRepeatingCenter"));
  storeImageAndColor(VSLIDER_HILITE_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderHiliteSmallRepeatingCenter"));
  storeImageAndColor(VSLIDER_HILITE_SMALL_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);

  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderThumbEnabled"));
  storeImageAndColor(VSLIDER_THUMB_ENABLED, image, red, lightRed);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderThumbEnabledSelected"));
  storeImageAndColor(VSLIDER_THUMB_ENABLED_PUSHED, image, gray, lightGray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderThumbDisabled"));
  storeImageAndColor(VSLIDER_THUMB_DISABLED, image, darkGray, gray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderThumbDisabledSelected"));
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderThumbHiliteSelected"));
  storeImageAndColor(VSLIDER_THUMB_HILITE_PUSHED, image, blue, lightBlue);

  // listbox
  image =
      TheMappedImageCollection->findImageByName(AsciiString("ListBoxEnabled"));
  storeImageAndColor(LISTBOX_ENABLED, image, red, lightRed);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxEnabledSelectedItemLeftEnd"));
  storeImageAndColor(LISTBOX_ENABLED_SELECTED_ITEM_LEFT, image, yellow, white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxEnabledSelectedItemRightEnd"));
  storeImageAndColor(LISTBOX_ENABLED_SELECTED_ITEM_RIGHT, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxEnabledSelectedItemRepeatingCenter"));
  storeImageAndColor(LISTBOX_ENABLED_SELECTED_ITEM_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxEnabledSelectedItemSmallRepeatingCenter"));
  storeImageAndColor(LISTBOX_ENABLED_SELECTED_ITEM_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image =
      TheMappedImageCollection->findImageByName(AsciiString("ListBoxDisabled"));
  storeImageAndColor(LISTBOX_DISABLED, image, gray, lightGray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxDisabledSelectedItemLeftEnd"));
  storeImageAndColor(LISTBOX_DISABLED_SELECTED_ITEM_LEFT, image, lightGray,
                     white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxDisabledSelectedItemRightEnd"));
  storeImageAndColor(LISTBOX_DISABLED_SELECTED_ITEM_RIGHT, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxDisabledSelectedItemRepeatingCenter"));
  storeImageAndColor(LISTBOX_DISABLED_SELECTED_ITEM_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxDisabledSelectedItemSmallRepeatingCenter"));
  storeImageAndColor(LISTBOX_DISABLED_SELECTED_ITEM_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image =
      TheMappedImageCollection->findImageByName(AsciiString("ListBoxHilite"));
  storeImageAndColor(LISTBOX_HILITE, image, green, darkGreen);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxHiliteSelectedItemLeftEnd"));
  storeImageAndColor(LISTBOX_HILITE_SELECTED_ITEM_LEFT, image, white,
                     darkGreen);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxHiliteSelectedItemRightEnd"));
  storeImageAndColor(LISTBOX_HILITE_SELECTED_ITEM_RIGHT, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxHiliteSelectedItemRepeatingCenter"));
  storeImageAndColor(LISTBOX_HILITE_SELECTED_ITEM_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxHiliteSelectedItemSmallRepeatingCenter"));
  storeImageAndColor(LISTBOX_HILITE_SELECTED_ITEM_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeUpButtonDisabled"));
  storeImageAndColor(LISTBOX_UP_BUTTON_DISABLED, image, gray, lightGray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeUpButtonDisabled"));
  storeImageAndColor(LISTBOX_UP_BUTTON_DISABLED_PUSHED, image, lightGray,
                     white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeUpButtonHilite"));
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDownButtonDisabled"));
  storeImageAndColor(LISTBOX_DOWN_BUTTON_DISABLED_PUSHED, image, lightGray,
                     white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDownButtonHilite"));
  storeImageAndColor(LISTBOX_DOWN_BUTTON_HILITE, image, green, darkGreen);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDownButtonHiliteSelected"));
  storeImageAndColor(LISTBOX_DOWN_BUTTON_HILITE_PUSHED, image, white,
                     darkGreen);
  ...
  storeImageAndColor(LISTBOX_SLIDER_ENABLED_BOTTOM, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeEnabledRepeatingCenter"));
  storeImageAndColor(LISTBOX_SLIDER_ENABLED_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeEnabledSmallRepeatingCenter"));
  storeImageAndColor(LISTBOX_SLIDER_ENABLED_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);

  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDisabledTopEnd"));
  storeImageAndColor(LISTBOX_SLIDER_DISABLED_TOP, image, gray, darkGray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDisabledBottomEnd"));
  storeImageAndColor(LISTBOX_SLIDER_DISABLED_BOTTOM, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDisabledRepeatingCenter"));
  storeImageAndColor(LISTBOX_SLIDER_DISABLED_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDisabledSmallRepeatingCenter"));
  storeImageAndColor(LISTBOX_SLIDER_DISABLED_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);

  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeHiliteTopEnd"));
  storeImageAndColor(LISTBOX_SLIDER_HILITE_TOP, image, green, darkGreen);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeHiliteBottomEnd"));
  storeImageAndColor(LISTBOX_SLIDER_HILITE_BOTTOM, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeHiliteRepeatingCenter"));
  storeImageAndColor(LISTBOX_SLIDER_HILITE_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeHiliteSmallRepeatingCenter"));
  storeImageAndColor(LISTBOX_SLIDER_HILITE_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);

  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeThumbEnabled"));
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeThumbDisabled"));
  storeImageAndColor(LISTBOX_SLIDER_THUMB_DISABLED, image, gray, lightGray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeThumbDisabled"));
  storeImageAndColor(LISTBOX_SLIDER_THUMB_DISABLED_PUSHED, image, lightGray,
                     white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeThumbHilite"));
  storeImageAndColor(LISTBOX_SLIDER_THUMB_HILITE, image, green, darkGreen);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeThumbHiliteSelected"));
  storeImageAndColor(LISTBOX_SLIDER_THUMB_HILITE_PUSHED, image, white,
                     darkGreen);
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxEnabledSelectedItemRightEnd"));
  storeImageAndColor(COMBOBOX_ENABLED_SELECTED_ITEM_RIGHT, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxEnabledSelectedItemRepeatingCenter"));
  storeImageAndColor(COMBOBOX_ENABLED_SELECTED_ITEM_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxEnabledSelectedItemSmallRepeatingCenter"));
  storeImageAndColor(COMBOBOX_ENABLED_SELECTED_ITEM_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image =
      TheMappedImageCollection->findImageByName(AsciiString("ListBoxDisabled"));
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxDisabledSelectedItemLeftEnd"));
  storeImageAndColor(COMBOBOX_DISABLED_SELECTED_ITEM_LEFT, image, lightGray,
                     white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxDisabledSelectedItemRightEnd"));
  storeImageAndColor(COMBOBOX_DISABLED_SELECTED_ITEM_RIGHT, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxDisabledSelectedItemRepeatingCenter"));
  storeImageAndColor(COMBOBOX_DISABLED_SELECTED_ITEM_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxDisabledSelectedItemSmallRepeatingCenter"));
  storeImageAndColor(COMBOBOX_DISABLED_SELECTED_ITEM_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image =
      TheMappedImageCollection->findImageByName(AsciiString("ListBoxHilite"));
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxHiliteSelectedItemLeftEnd"));
  storeImageAndColor(COMBOBOX_HILITE_SELECTED_ITEM_LEFT, image, white,
                     darkGreen);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxHiliteSelectedItemRightEnd"));
  storeImageAndColor(COMBOBOX_HILITE_SELECTED_ITEM_RIGHT, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxHiliteSelectedItemRepeatingCenter"));
  storeImageAndColor(COMBOBOX_HILITE_SELECTED_ITEM_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxHiliteSelectedItemSmallRepeatingCenter"));
  storeImageAndColor(COMBOBOX_HILITE_SELECTED_ITEM_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDownButtonEnabled"));
  storeImageAndColor(COMBOBOX_DROP_DOWN_BUTTON_ENABLED_PUSHED, image, yellow,
                     white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDownButtonDisabled"));
  storeImageAndColor(COMBOBOX_DROP_DOWN_BUTTON_DISABLED, image, gray,
                     lightGray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDownButtonDisabled"));
  storeImageAndColor(COMBOBOX_DROP_DOWN_BUTTON_DISABLED_PUSHED, image,
                     lightGray, gray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDownButtonHilite"));
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDownButtonHilite"));
  storeImageAndColor(COMBOBOX_DROP_DOWN_BUTTON_HILITE_PUSHED, image, yellow,
                     white);

  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryEnabledLeftEnd"));
  storeImageAndColor(COMBOBOX_EDIT_BOX_ENABLED_LEFT, image, red, lightRed);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryEnabledRightEnd"));
  storeImageAndColor(COMBOBOX_EDIT_BOX_ENABLED_RIGHT, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryEnabledRepeatingCenter"));
  storeImageAndColor(COMBOBOX_EDIT_BOX_ENABLED_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryEnabledSmallRepeatingCenter"));
  storeImageAndColor(COMBOBOX_EDIT_BOX_ENABLED_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryDisabledLeftEnd"));
  storeImageAndColor(COMBOBOX_EDIT_BOX_DISABLED_LEFT, image, gray, black);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryDisabledRightEnd"));
  storeImageAndColor(COMBOBOX_EDIT_BOX_DISABLED_RIGHT, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryDisabledRepeatingCenter"));
  storeImageAndColor(COMBOBOX_EDIT_BOX_DISABLED_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryDisabledSmallRepeatingCenter"));
  storeImageAndColor(COMBOBOX_EDIT_BOX_DISABLED_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryHiliteLeftEnd"));
  storeImageAndColor(COMBOBOX_EDIT_BOX_HILITE_LEFT, image, green, darkGreen);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryHiliteRightEnd"));
  storeImageAndColor(COMBOBOX_EDIT_BOX_HILITE_RIGHT, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryHiliteRepeatingCenter"));
  storeImageAndColor(COMBOBOX_EDIT_BOX_HILITE_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryHiliteSmallRepeatingCenter"));
  storeImageAndColor(COMBOBOX_EDIT_BOX_HILITE_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxEnabledSelectedItemLeftEnd"));
  storeImageAndColor(COMBOBOX_LISTBOX_ENABLED_SELECTED_ITEM_LEFT, image, yellow,
                     white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxEnabledSelectedItemRightEnd"));
  storeImageAndColor(COMBOBOX_LISTBOX_ENABLED_SELECTED_ITEM_RIGHT, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxEnabledSelectedItemRepeatingCenter"));
  storeImageAndColor(COMBOBOX_LISTBOX_ENABLED_SELECTED_ITEM_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxEnabledSelectedItemSmallRepeatingCenter"));
  storeImageAndColor(COMBOBOX_LISTBOX_ENABLED_SELECTED_ITEM_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image =
      TheMappedImageCollection->findImageByName(AsciiString("ListBoxDisabled"));
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxDisabledSelectedItemLeftEnd"));
  storeImageAndColor(COMBOBOX_LISTBOX_DISABLED_SELECTED_ITEM_LEFT, image,
                     lightGray, white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxDisabledSelectedItemRightEnd"));
  storeImageAndColor(COMBOBOX_LISTBOX_DISABLED_SELECTED_ITEM_RIGHT, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxDisabledSelectedItemRepeatingCenter"));
  storeImageAndColor(COMBOBOX_LISTBOX_DISABLED_SELECTED_ITEM_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxDisabledSelectedItemSmallRepeatingCenter"));
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxHiliteSelectedItemLeftEnd"));
  storeImageAndColor(COMBOBOX_LISTBOX_HILITE_SELECTED_ITEM_LEFT, image, white,
                     darkGreen);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxHiliteSelectedItemRightEnd"));
  storeImageAndColor(COMBOBOX_LISTBOX_HILITE_SELECTED_ITEM_RIGHT, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxHiliteSelectedItemRepeatingCenter"));
  storeImageAndColor(COMBOBOX_LISTBOX_HILITE_SELECTED_ITEM_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ListBoxHiliteSelectedItemSmallRepeatingCenter"));
  storeImageAndColor(COMBOBOX_LISTBOX_HILITE_SELECTED_ITEM_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeUpButtonEnabled"));
  storeImageAndColor(COMBOBOX_LISTBOX_UP_BUTTON_ENABLED_PUSHED, image, yellow,
                     white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeUpButtonDisabled"));
  storeImageAndColor(COMBOBOX_LISTBOX_UP_BUTTON_DISABLED, image, gray,
                     lightGray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeUpButtonDisabled"));
  storeImageAndColor(COMBOBOX_LISTBOX_UP_BUTTON_DISABLED_PUSHED, image,
                     lightGray, white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeUpButtonHilite"));
  storeImageAndColor(COMBOBOX_LISTBOX_UP_BUTTON_HILITE, image, green,
                     darkGreen);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeUpButtonHiliteSelected"));
  storeImageAndColor(COMBOBOX_LISTBOX_UP_BUTTON_HILITE_PUSHED, image, white,
                     darkGreen);

  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDownButtonEnabled"));
  storeImageAndColor(COMBOBOX_LISTBOX_DOWN_BUTTON_ENABLED, image, red,
                     lightRed);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDownButtonEnabled"));
  storeImageAndColor(COMBOBOX_LISTBOX_DOWN_BUTTON_ENABLED_PUSHED, image, yellow,
                     white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDownButtonDisabled"));
  storeImageAndColor(COMBOBOX_LISTBOX_DOWN_BUTTON_DISABLED, image, gray,
                     lightGray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDownButtonDisabled"));
  storeImageAndColor(COMBOBOX_LISTBOX_DOWN_BUTTON_DISABLED_PUSHED, image,
                     lightGray, white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDownButtonHilite"));
  storeImageAndColor(COMBOBOX_LISTBOX_DOWN_BUTTON_HILITE, image, green,
                     darkGreen);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDownButtonHiliteSelected"));
  storeImageAndColor(COMBOBOX_LISTBOX_DOWN_BUTTON_HILITE_PUSHED, image, white,
                     darkGreen);
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeEnabledBottomEnd"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_ENABLED_BOTTOM, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeEnabledRepeatingCenter"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_ENABLED_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeEnabledSmallRepeatingCenter"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_ENABLED_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);

  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDisabledTopEnd"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_DISABLED_TOP, image, gray,
                     darkGray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDisabledBottomEnd"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_DISABLED_BOTTOM, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDisabledRepeatingCenter"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_DISABLED_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeDisabledSmallRepeatingCenter"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_DISABLED_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);

  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeHiliteTopEnd"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_HILITE_TOP, image, green,
                     darkGreen);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeHiliteBottomEnd"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_HILITE_BOTTOM, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeHiliteRepeatingCenter"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_HILITE_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeHiliteSmallRepeatingCenter"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_HILITE_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);

  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeThumbEnabled"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_THUMB_ENABLED, image, red,
                     lightRed);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeThumbEnabled"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_THUMB_ENABLED_PUSHED, image,
                     yellow, white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeThumbDisabled"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_THUMB_DISABLED, image, gray,
                     lightGray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeThumbDisabled"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_THUMB_DISABLED_PUSHED, image,
                     lightGray, white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeThumbHilite"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_THUMB_HILITE, image, green,
                     darkGreen);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("VSliderLargeThumbHiliteSelected"));
  storeImageAndColor(COMBOBOX_LISTBOX_SLIDER_THUMB_HILITE_PUSHED, image, white,
                     darkGreen);

  // progress bar
  //---------------------------------------------------------------------------
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarEnabledLeftEnd"));
  storeImageAndColor(PROGRESS_BAR_ENABLED_LEFT, image, red, lightRed);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarEnabledRightEnd"));
  storeImageAndColor(PROGRESS_BAR_ENABLED_RIGHT, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarEnabledRepeatingCenter"));
  storeImageAndColor(PROGRESS_BAR_ENABLED_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarEnabledSmallRepeatingCenter"));
  storeImageAndColor(PROGRESS_BAR_ENABLED_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);

  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarEnabledBarLeftEnd"));
  storeImageAndColor(PROGRESS_BAR_ENABLED_BAR_LEFT, image, yellow, white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarEnabledBarRightEnd"));
  storeImageAndColor(PROGRESS_BAR_ENABLED_BAR_RIGHT, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarEnabledBarRepeatingCenter"));
  storeImageAndColor(PROGRESS_BAR_ENABLED_BAR_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarEnabledBarSmallRepeatingCenter"));
  storeImageAndColor(PROGRESS_BAR_ENABLED_BAR_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);

  //---------------------------------------------------------------------------
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarDisabledLeftEnd"));
  storeImageAndColor(PROGRESS_BAR_DISABLED_LEFT, image, darkGray, lightGray);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarDisabledRightEnd"));
  storeImageAndColor(PROGRESS_BAR_DISABLED_RIGHT, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarDisabledRepeatingCenter"));
  storeImageAndColor(PROGRESS_BAR_DISABLED_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarDisabledSmallRepeatingCenter"));
  storeImageAndColor(PROGRESS_BAR_DISABLED_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);

  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarDisabledBarLeftEnd"));
  storeImageAndColor(PROGRESS_BAR_DISABLED_BAR_LEFT, image, lightGray, white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarDisabledBarRightEnd"));
  storeImageAndColor(PROGRESS_BAR_DISABLED_BAR_RIGHT, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarDisabledBarRepeatingCenter"));
  storeImageAndColor(PROGRESS_BAR_DISABLED_BAR_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarDisabledBarSmallRepeatingCenter"));
  storeImageAndColor(PROGRESS_BAR_DISABLED_BAR_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);

  //---------------------------------------------------------------------------
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarHiliteLeftEnd"));
  storeImageAndColor(PROGRESS_BAR_HILITE_LEFT, image, green, darkGreen);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarHiliteRightEnd"));
  storeImageAndColor(PROGRESS_BAR_HILITE_RIGHT, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarHiliteRepeatingCenter"));
  storeImageAndColor(PROGRESS_BAR_HILITE_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarHiliteSmallRepeatingCenter"));
  storeImageAndColor(PROGRESS_BAR_HILITE_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);

  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarHiliteBarLeftEnd"));
  storeImageAndColor(PROGRESS_BAR_HILITE_BAR_LEFT, image, yellow, white);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarHiliteBarRightEnd"));
  storeImageAndColor(PROGRESS_BAR_HILITE_BAR_RIGHT, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarHiliteBarRepeatingCenter"));
  storeImageAndColor(PROGRESS_BAR_HILITE_BAR_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("ProgressBarHiliteBarSmallRepeatingCenter"));
  storeImageAndColor(PROGRESS_BAR_HILITE_BAR_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);

  // static text
  //---------------------------------------------------------------------------
  image = TheMappedImageCollection->findImageByName(
      AsciiString("StaticTextEnabled"));
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryEnabledRightEnd"));
  storeImageAndColor(TEXT_ENTRY_ENABLED_RIGHT, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryEnabledRepeatingCenter"));
  storeImageAndColor(TEXT_ENTRY_ENABLED_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryEnabledSmallRepeatingCenter"));
  storeImageAndColor(TEXT_ENTRY_ENABLED_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  ...
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryDisabledRightEnd"));
  storeImageAndColor(TEXT_ENTRY_DISABLED_RIGHT, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryDisabledRepeatingCenter"));
  storeImageAndColor(TEXT_ENTRY_DISABLED_CENTER, image, WIN_COLOR_UNDEFINED,
                     WIN_COLOR_UNDEFINED);
  image = TheMappedImageCollection->findImageByName(
      AsciiString("TextEntryDisabledSmallRepeatingCenter"));
  storeImageAndColor(TEXT_ENTRY_DISABLED_SMALL_CENTER, image,
                     WIN_COLOR_UNDEFINED, WIN_COLOR_UNDEFINED);
  ...
  // default font
  m_font =
      TheWindowManager->winFindFont(AsciiString("Times New Roman"), 14, FALSE);
}  // end init
...
ImageAndColorInfo *LayoutScheme::findEntry(StateIdentifier id) {
  ...
  for (Int i = 0; i < NUM_STATE_IDENTIFIERS; i++) {
    if (m_imageAndColorTable[i].stateID == id) return &m_imageAndColorTable[i];
...
void LayoutScheme::storeImageAndColor(StateIdentifier id, const Image *image,
  ...
  if (id < 0 || id >= NUM_STATE_IDENTIFIERS) {
    DEBUG_LOG(
        ("Illegal state identifier in layout scheme store image and color "
         "'%d'\n",
         id));
    assert(0);
    return;
...
Bool LayoutScheme::saveScheme(char *filename) {
  FILE *fp;
  UnsignedByte colorR, colorG, colorB, colorA, bColorR, bColorG, bColorB,
      bColorA;
  ...
  if (fp == NULL) {
    ...
    MessageBox(TheEditor->getWindowHandle(),
               "Unable to open scheme for for saving.  Read only?",
               "Save Error", MB_OK);
    return FALSE;
  ...
  GameGetColorComponents(m_enabledText.color, &colorR, &colorG, &colorB,
                         &colorA);
  fprintf(fp, "Enabled Text: (%d,%d,%d,%d)\n", colorR, colorG, colorB, colorA);
  GameGetColorComponents(m_enabledText.borderColor, &colorR, &colorG, &colorB,
                         &colorA);
  fprintf(fp, "Enabled Text Border: (%d,%d,%d,%d)\n", colorR, colorG, colorB,
          colorA);

  GameGetColorComponents(m_disabledText.color, &colorR, &colorG, &colorB,
                         &colorA);
  fprintf(fp, "Disabled Text: (%d,%d,%d,%d)\n", colorR, colorG, colorB, colorA);
  GameGetColorComponents(m_disabledText.borderColor, &colorR, &colorG, &colorB,
                         &colorA);
  fprintf(fp, "Disabled Text Border: (%d,%d,%d,%d)\n", colorR, colorG, colorB,
          colorA);

  GameGetColorComponents(m_hiliteText.color, &colorR, &colorG, &colorB,
                         &colorA);
  fprintf(fp, "Hilite Text: (%d,%d,%d,%d)\n", colorR, colorG, colorB, colorA);
  GameGetColorComponents(m_hiliteText.borderColor, &colorR, &colorG, &colorB,
                         &colorA);
  ...
  char fontBuffer[256];
  if (m_font)
    sprintf(fontBuffer, "\"%s\" Size: %d Bold: %d", m_font->nameString.str(),
            m_font->pointSize, m_font->bold);
  else
  ...
  for (Int i = 0; i < NUM_STATE_IDENTIFIERS; i++) {
    ...
    GameGetColorComponents(info->color, &colorR, &colorG, &colorB, &colorA);
    GameGetColorComponents(info->borderColor, &bColorR, &bColorG, &bColorB,
                           &bColorA);

    // output it
    fprintf(fp, "%d: Image: %s Color: (%d,%d,%d,%d) Border: (%d,%d,%d,%d)\n", i,
            info->image ? info->image->getName().str() : "NONE", colorR, colorG,
            colorB, colorA, bColorR, bColorG, bColorB, bColorA);
...
Bool LayoutScheme::loadScheme(char *filename) {
  ...
  if (version != SCHEME_VERSION) {
    ...
    MessageBox(TheEditor->getWindowHandle(), "Old layout version, cannot open.",
               "Old File", MB_OK);
    return FALSE;
  ...
  for (Int i = 0; i < numStates; i++) {
    ...
    fscanf(fp, "%d: Image: %s Color: (%d,%d,%d,%d) Border: (%d,%d,%d,%d)\n",
           &state, imageBuffer, &colorR, &colorG, &colorB, &colorA, &bColorR,
           &bColorG, &bColorB, &bColorA);
