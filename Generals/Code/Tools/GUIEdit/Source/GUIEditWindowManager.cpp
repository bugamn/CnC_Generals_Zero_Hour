...
// ///////////////////////////////////////////////////////////////// Created:
// Colin Day, July 2001 Desc:       Window manager for the GUI edit tool, we
// want this up
//						 fast and to look like what we
//use in the game so we're going 						 to use the WW3D window manager, and just
//override the 						 drawing functions to draw lines and images to the 						 display.  We
//will also be adding our own functionality 						 here for editing and interacting
//with the GUI windows.
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void GUIEditWindowManager::linkToClipboard(GameWindow *window,
                                           GameWindow **list) {
  // sanity
  if (window == NULL || list == NULL) return;

  // debug sanity checking, can't add if already in it
  if (isWindowInClipboard(window, list)) return;

  window->winSetPrev(NULL);
  window->winSetNext(*list);
  if (*list) (*list)->winSetPrev(window);
  *list = window;
...
void GUIEditWindowManager::unlinkFromClipboard(GameWindow *window,
  ...
  if (isWindowInClipboard(window, list) == FALSE) return;
...
void GUIEditWindowManager::removeSupervisedChildSelections(void) {
  ...
  for (select = TheEditor->getSelectList(); select; select = next) {
    ...
    while (parent) {
      if (TheEditor->isWindowSelected(parent)) parentSelected = TRUE;
    ...
    // selection
    //
    if (parentSelected == TRUE) TheEditor->unSelectWindow(window);
...
void GUIEditWindowManager::orphanSelectedChildren( void )
{
...
)
        {
                ...
                if( parent )
                {
                        ...
                        // get the screen position of the window BEFORE we
remove the parent window->winGetScreenPosition( &pos.x, &pos.y );

                        // remove the child from the parent and add to top level
of window system window->winSetParent( NULL );

                        //
                        // adjust the position, which previously was relative to
the parent
...
GameWindow *GUIEditWindowManager::winCreate(GameWindow *parent,
                                            UnsignedInt status, Int x, Int y,
  ...
  window = W3DGameWindowManager::winCreate(parent, status, x, y, width, height,
                                           system, instData);
...
void GUIEditWindowManager::cutSelectedToClipboard(void) {
  ...
  //
  if (m_clipboard) TheEditor->deleteSelected();
...
void GUIEditWindowManager::incrementName(GameWindow *window) {
  ...
  //
  for (i = 0; i < MAX_WINDOW_NAME_LEN; i++) numberBuffer[i] = ' ';
...
void GUIEditWindowManager::validateClipboardNames(GameWindow *root) {
  ...
  while (nameOK == FALSE) {
    ...
    if (!TheEditor->isNameDuplicate(m_windowList, root,
                                    instData->m_decoratedNameString) &&
        !TheEditor->isNameDuplicate(m_clipboardDup, root,
                                    instData->m_decoratedNameString))
      nameOK = TRUE;
    else
    ...
    if (++sanityLoop > 99999999) {
      ...
      sprintf(buffer,
              "Unable to make unique name for '%s', please manually edit the "
              "name before saving.",
              instData->m_decoratedNameString.str());
      MessageBox(TheEditor->getWindowHandle(), buffer, "Error Mapping Names",
                 MB_OK);
      nameOK = TRUE;
...
void GUIEditWindowManager::pasteClipboard(void) {
  ...
  for (window = m_clipboardDup; window; window = next) {
    ...
    TheEditor->computeSafeLocation(window, pos.x, pos.y, &safePos.x,
                                   &safePos.y);
  ...
  if (firstWindow) TheEditor->setUnsaved(TRUE);
...
void InstDrawCopy(WinInstanceData *instData, WinInstanceData *sourceInstData) {
  ...
  memcpy(&instData->m_enabledDrawData, &sourceInstData->m_enabledDrawData,
         sizeof(WinDrawData) * MAX_DRAW_DATA);
  memcpy(&instData->m_disabledDrawData, &sourceInstData->m_disabledDrawData,
         sizeof(WinDrawData) * MAX_DRAW_DATA);
  memcpy(&instData->m_hiliteDrawData, &sourceInstData->m_hiliteDrawData,
         sizeof(WinDrawData) * MAX_DRAW_DATA);
}
...
GameWindow *GUIEditWindowManager::duplicateWindow(GameWindow *source,
  ...
  if (BitTest(style, GWS_PUSH_BUTTON)) {
    duplicate = TheWindowManager->gogoGadgetPushButton(
        parent, status, pos.x, pos.y, size.x, size.y, &instDataCopy,
        source->winGetFont(), FALSE);
    ...
    duplicate = TheWindowManager->gogoGadgetRadioButton(
        parent, status, pos.x, pos.y, size.x, size.y, &instDataCopy,
        &radioDataCopy, source->winGetFont(), FALSE);

  }  // end else if
  else if (BitTest(style, GWS_CHECK_BOX)) {
    duplicate = TheWindowManager->gogoGadgetCheckbox(
        parent, status, pos.x, pos.y, size.x, size.y, &instDataCopy,
        source->winGetFont(), FALSE);
    ...
    duplicate = TheWindowManager->gogoGadgetSlider(
        parent, status, pos.x, pos.y, size.x, size.y, &instDataCopy,
        &sliderDataCopy, source->winGetFont(), FALSE);
    ...
    comboDataCopy.listboxData->scrollIfAtEnd =
        comboData->listboxData->scrollIfAtEnd;
    comboDataCopy.listboxData->forceSelect =
        comboData->listboxData->forceSelect;
    comboDataCopy.listboxData->listLength = comboData->listboxData->listLength;
    comboDataCopy.listboxData->multiSelect =
        comboData->listboxData->multiSelect;
    ...
    comboDataCopy.listboxData->columnWidth =
        comboData->listboxData->columnWidth;
    comboDataCopy.listboxData->columnWidthPercentage =
        comboData->listboxData->columnWidthPercentage;

    comboDataCopy.entryData->alphaNumericalOnly =
        comboData->entryData->alphaNumericalOnly;
    ...
    comboDataCopy.entryData->numericalOnly =
        comboData->entryData->numericalOnly;

    duplicate = TheWindowManager->gogoGadgetComboBox(
        parent, status, pos.x, pos.y, size.x, size.y, &instDataCopy,
        &comboDataCopy, source->winGetFont(), FALSE);
    ...
    if (listData->columnWidthPercentage) {
      ...
      for (Int i = 0; i < listData->columns; i++) {
        listDataCopy.columnWidthPercentage[i] =
            listData->columnWidthPercentage[i];
      }
    } else
      listDataCopy.columnWidthPercentage = NULL;

    duplicate = TheWindowManager->gogoGadgetListBox(
        parent, status, pos.x, pos.y, size.x, size.y, &instDataCopy,
        &listDataCopy, source->winGetFont(), FALSE);
    ...
    duplicate = TheWindowManager->gogoGadgetTextEntry(
        parent, status, pos.x, pos.y, size.x, size.y, &instDataCopy,
        &entryDataCopy, source->winGetFont(), FALSE);
    ...
    duplicate = TheWindowManager->gogoGadgetStaticText(
        parent, status, pos.x, pos.y, size.x, size.y, &instDataCopy,
        &textDataCopy, source->winGetFont(), FALSE);

  }  // end else if
  else if (BitTest(style, GWS_PROGRESS_BAR)) {
    duplicate = TheWindowManager->gogoGadgetProgressBar(
        parent, status, pos.x, pos.y, size.x, size.y, &instDataCopy,
        source->winGetFont(), FALSE);
    ...
    duplicate = TheWindowManager->winCreate(
        parent, status, pos.x, pos.y, size.x, size.y, NULL, &instDataCopy);

  }  // end else if
  else {
    MessageBox(TheEditor->getWindowHandle(),
               "Cannot duplicate window, undefined style.", "Internal Error",
               MB_OK);
    assert(0);
  ...
  // necessary.
  //
  if (parent == NULL) unlinkWindow(duplicate);
  ...
  if (duplicate) {
    ...
    if (sourceEditData && editData) *editData = *sourceEditData;
    //			memcpy( editData, sourceEditData, sizeof(
    //GameWindowEditData ) );
...
void GUIEditWindowManager::createClipboardDuplicate(void) {
  ...
  for (window = lastWindow; window; window = window->winGetPrev()) {
    ...
    if (duplicate) linkToClipboard(duplicate, &m_clipboardDup);
...
//-------------------------------------------------------------------------------------------------
void GUIEditWindowManager::makeChildOf(GameWindow *target, GameWindow *parent) {
  // sanity
  ...
  if (TheEditor->windowIsGadget(parent)) {
    MessageBox(TheEditor->getWindowHandle(),
               "You cannot give a gadget control a child.", "Child Not Allowed",
               MB_OK);
    return;
  ...
  target->winGetScreenPosition(&screenPosBeforeMove.x, &screenPosBeforeMove.y);
  ...
  // kee the location legal
  TheEditor->computeSafeLocation(target, origin.x, origin.y, &safeLoc.x,
                                 &safeLoc.y);
...
void GUIEditWindowManager::moveAheadOf(GameWindow *windowToMove,
  ...
  if (newParent) newParent->winGetScreenPosition(&parentPos.x, &parentPos.y);
  ...
  // kee the location legal
  TheEditor->computeSafeLocation(windowToMove, origin.x, origin.y, &safeLoc.x,
                                 &safeLoc.y);
