...
void GUIEdit::selectWindowsInRegion(IRegion2D *region) {
  ...
  while (window) {
    ...
    if (origin.x >= region->lo.x && origin.y >= region->lo.y &&
...
void GUIEdit::init(void) {
  ...
  //
  TheGUIEditWindowManager =
      static_cast<GUIEditWindowManager *>(TheWindowManager);
  ...
  // create the default scheme, this must be done after image collections are
  // loaded
  ...
  if (TheHeaderTemplateManager) TheHeaderTemplateManager->init();
...
Bool GUIEdit::readConfigFile(char *filename) {
  ...
  fscanf(fp, "BACKGROUNDCOLOR = %f %f %f %f\n", &backColor.red,
         &backColor.green, &backColor.blue, &backColor.alpha);
...
void GUIEdit::readFontFile(char *filename) {
  ...
  for (Int i = 0; i < fontCount; i++) {
    ...
    // set the font
    GameFont *font =
        TheFontLibrary->getFont(AsciiString(fontBuffer), size, bold);
    if (font == NULL) {
      char buffer[1024];

      sprintf(buffer,
              "Warning: The font '%s' Size: '%d' Bold: '%d', specified in the "
              "config file could not be loaded.  Does that font exist?",
...
/** Given a mouse position, get the topmost window at that location. We needed
                to pull this out into it's own special case function since the
   standard winPointInAnyChild is used by the game and we needed a special case
   to select a disabled control in GUIEdit
 */
//=============================================================================
static GameWindow *pointInChild(Int x, Int y, GameWindow *win) {
  ...
  for (child = win->winGetChild(); child; child = child->winGetNext()) {
    ...
    if (x >= origin.x && x <= origin.x + tempX && y >= origin.y &&
        y <= origin.y + tempY &&
 ...
 */
//=============================================================================
static GameWindow *pointInAnyChild(Int x, Int y, Bool ignoreHidden,
                                   GameWindow *win) {
  GameWindow *parent;
  GameWindow *child;
  ICoord2D origin;
  ...
  for (child = win->winGetChild(); child; child = child->winGetNext()) {
    ...
    if (x >= origin.x && x <= origin.x + tempX && y >= origin.y &&
      ...
      if (!(ignoreHidden == TRUE &&
            BitTest(child->winGetStatus(), WIN_STATUS_HIDDEN)))
...
GameWindow *GUIEdit::getWindowAtPos(Int x, Int y) {
  ...
  for (window = TheWindowManager->winGetWindowList(); window;
    ...
    if (x >= region.lo.x && x <= region.hi.x && y >= region.lo.y &&
        y <= region.hi.y) {
  ...
  if (pick) {
    ...
    //
    if (parent) {
      if (BitTest(parent->winGetStyle(), GWS_VERT_SLIDER | GWS_HORZ_SLIDER |
                                             GWS_SCROLL_LISTBOX |
                                             GWS_COMBO_BOX | GWS_TAB_CONTROL)) {
        ...
        //
        if (BitTest(parent->winGetStyle(), GWS_HORZ_SLIDER | GWS_VERT_SLIDER)) {
          GameWindow *grandParent = parent->winGetParent();

          if (grandParent &&
              BitTest(grandParent->winGetStyle(), GWS_SCROLL_LISTBOX))
            pick = grandParent;
 ...
 * locations */
//=============================================================================
void GUIEdit::clipCreationParamsToParent(GameWindow *parent, Int *x, Int *y,
  ...
  // sanity
  if (parent == NULL || x == NULL || y == NULL || width == NULL ||
      height == NULL)
    return;
  ...
  newWidth = *width;
  if (*x + *width > parentScreenRegion.hi.x) newWidth = parentSize.x - newX;

  newHeight = *height;
  if (*y + *height > parentScreenRegion.hi.y) newHeight = parentSize.y - newY;
...
void GUIEdit::removeWindowCleanup(GameWindow *window) {
  ...
  if (m_propertyTarget == window) m_propertyTarget = NULL;
...
//=============================================================================
GameWindow *GUIEdit::newWindow(UnsignedInt windowStyle, GameWindow *parent,
...
//=============================================================================
GameWindow *GUIEdit::newUserWindow(GameWindow *parent, Int x, Int y, Int width,
                                   Int height) {
  UnsignedInt status = WIN_STATUS_ENABLED;
  GameWindow *window;

  // validate the parent to disallow illegal relationships
  if (validateParentForCreate(parent) == FALSE) return NULL;
  ...
  //
  if (parent) clipCreationParamsToParent(parent, &x, &y, &width, &height);
...
//=============================================================================
GameWindow *GUIEdit::newPushButton(GameWindow *parent, Int x, Int y, Int width,
                                   Int height) {
  GameWindow *window;
  ...
  window = TheWindowManager->gogoGadgetPushButton(
      parent, status, x, y, width, height, &instData, NULL, TRUE);
...
//=============================================================================
GameWindow *GUIEdit::newCheckBox(GameWindow *parent, Int x, Int y, Int width,
                                 Int height) {
  GameWindow *window;
  WinInstanceData instData;
  UnsignedInt status = WIN_STATUS_ENABLED | WIN_STATUS_IMAGE;
  ...
  // create gadget
  window = TheWindowManager->gogoGadgetCheckbox(parent, status, x, y, width,
                                                height, &instData, NULL, TRUE);
...
//=============================================================================
GameWindow *GUIEdit::newRadioButton(GameWindow *parent, Int x, Int y, Int width,
                                    Int height) {
  ...
  window = TheWindowManager->gogoGadgetRadioButton(
      parent, status, x, y, width, height, &instData, &radioData, NULL, TRUE);
...
//=============================================================================
GameWindow *GUIEdit::newTabControl(GameWindow *parent, Int x, Int y, Int width,
                                   Int height) {
...
//=============================================================================
GameWindow *GUIEdit::newHorizontalSlider(GameWindow *parent, Int x, Int y,
                                         Int width, Int height) {
  GameWindow *window;
  WinInstanceData instData;
  UnsignedInt status = WIN_STATUS_ENABLED | WIN_STATUS_IMAGE;
  ...
  window = TheWindowManager->gogoGadgetSlider(
      parent, status, x, y, width, height, &instData, &sliderData, NULL, TRUE);
...
//=============================================================================
GameWindow *GUIEdit::newVerticalSlider(GameWindow *parent, Int x, Int y,
                                       Int width, Int height) {
  GameWindow *window;
  WinInstanceData instData;
  UnsignedInt status = WIN_STATUS_ENABLED | WIN_STATUS_IMAGE;
...
//=============================================================================
GameWindow *GUIEdit::newProgressBar(GameWindow *parent, Int x, Int y, Int width,
                                    Int height) {
  GameWindow *window;
  WinInstanceData instData;
  UnsignedInt status = WIN_STATUS_ENABLED | WIN_STATUS_IMAGE;
  ...
  if (window) {
    ...
    info =
        TheDefaultScheme->getImageAndColor(PROGRESS_BAR_ENABLED_SMALL_CENTER);
    ...
    info = TheDefaultScheme->getImageAndColor(
    ...
    info =
        TheDefaultScheme->getImageAndColor(PROGRESS_BAR_DISABLED_SMALL_CENTER);
...
//=============================================================================
GameWindow *GUIEdit::newComboBox(GameWindow *parent, Int x, Int y, Int width,
                                 Int height) {
  GameWindow *window;
  WinInstanceData instData;
  UnsignedInt status = WIN_STATUS_ENABLED | WIN_STATUS_IMAGE;
  ...
  // create the control
  window = TheWindowManager->gogoGadgetComboBox(
      parent, status, x, y, width, height, &instData, comboData, NULL, TRUE);
  ...
  if (window) {
    ...
    info =
        TheDefaultScheme->getImageAndColor(COMBOBOX_ENABLED_SELECTED_ITEM_LEFT);
    ...
    info =
        TheDefaultScheme->getImageAndColor(COMBOBOX_HILITE_SELECTED_ITEM_LEFT);
    ...
    info =
        TheDefaultScheme->getImageAndColor(COMBOBOX_HILITE_SELECTED_ITEM_RIGHT);
    ...
    if (editBox) {
      ...
      info =
          TheDefaultScheme->getImageAndColor(COMBOBOX_EDIT_BOX_ENABLED_RIGHT);
      GadgetTextEntrySetEnabledImageRight(editBox, info->image);
      info =
          TheDefaultScheme->getImageAndColor(COMBOBOX_EDIT_BOX_ENABLED_CENTER);
      ...
      info =
          TheDefaultScheme->getImageAndColor(COMBOBOX_EDIT_BOX_DISABLED_LEFT);
      ...
      info =
          TheDefaultScheme->getImageAndColor(COMBOBOX_EDIT_BOX_DISABLED_RIGHT);
      GadgetTextEntrySetDisabledImageRight(editBox, info->image);
      info =
          TheDefaultScheme->getImageAndColor(COMBOBOX_EDIT_BOX_DISABLED_CENTER);
      ...
      info =
          TheDefaultScheme->getImageAndColor(COMBOBOX_EDIT_BOX_HILITE_CENTER);
    ...
    if (dropDownButton) {
      info =
          TheDefaultScheme->getImageAndColor(COMBOBOX_DROP_DOWN_BUTTON_ENABLED);
      ...
      info =
          TheDefaultScheme->getImageAndColor(COMBOBOX_DROP_DOWN_BUTTON_HILITE);
    ...
    /// @TODO: We still need to set the IME Composite Text colors in
    /// TheDefaultScheme
...
/** Create a new list box */
//=============================================================================
GameWindow *GUIEdit::newListbox(GameWindow *parent, Int x, Int y, Int width,
                                Int height) {
  GameWindow *window;
  WinInstanceData instData;
  UnsignedInt status = WIN_STATUS_ENABLED | WIN_STATUS_IMAGE;
  ...
  window = TheWindowManager->gogoGadgetListBox(
      parent, status, x, y, width, height, &instData, &listData, NULL, TRUE);
  ...
  if (window) {
    ...
    info =
        TheDefaultScheme->getImageAndColor(LISTBOX_ENABLED_SELECTED_ITEM_LEFT);
    ...
    info =
        TheDefaultScheme->getImageAndColor(LISTBOX_ENABLED_SELECTED_ITEM_RIGHT);
    ...
    info =
        TheDefaultScheme->getImageAndColor(LISTBOX_DISABLED_SELECTED_ITEM_LEFT);
    ...
    info =
        TheDefaultScheme->getImageAndColor(LISTBOX_HILITE_SELECTED_ITEM_LEFT);
    ...
    info =
        TheDefaultScheme->getImageAndColor(LISTBOX_HILITE_SELECTED_ITEM_RIGHT);
    GadgetListBoxSetHiliteSelectedItemImageRight(window, info->image);
    info =
        TheDefaultScheme->getImageAndColor(LISTBOX_HILITE_SELECTED_ITEM_CENTER);
    ...
    if (upButton) {
      ...
      GadgetButtonSetEnabledBorderColor(upButton, info->borderColor);
      info =
          TheDefaultScheme->getImageAndColor(LISTBOX_UP_BUTTON_ENABLED_PUSHED);
      GadgetButtonSetEnabledSelectedImage(upButton, info->image);
      GadgetButtonSetEnabledSelectedColor(upButton, info->color);
      GadgetButtonSetEnabledSelectedBorderColor(upButton, info->borderColor);
      ...
      GadgetButtonSetDisabledBorderColor(upButton, info->borderColor);
      info =
          TheDefaultScheme->getImageAndColor(LISTBOX_UP_BUTTON_DISABLED_PUSHED);
      GadgetButtonSetDisabledSelectedImage(upButton, info->image);
      GadgetButtonSetDisabledSelectedColor(upButton, info->color);
      GadgetButtonSetDisabledSelectedBorderColor(upButton, info->borderColor);
      ...
      GadgetButtonSetHiliteBorderColor(upButton, info->borderColor);
      info =
          TheDefaultScheme->getImageAndColor(LISTBOX_UP_BUTTON_HILITE_PUSHED);
      GadgetButtonSetHiliteSelectedImage(upButton, info->image);
      GadgetButtonSetHiliteSelectedColor(upButton, info->color);
      GadgetButtonSetHiliteSelectedBorderColor(upButton, info->borderColor);
    ...
    if (downButton) {
      ...
      GadgetButtonSetHiliteBorderColor(downButton, info->borderColor);
      info =
          TheDefaultScheme->getImageAndColor(LISTBOX_DOWN_BUTTON_HILITE_PUSHED);
      GadgetButtonSetHiliteSelectedImage(downButton, info->image);
      GadgetButtonSetHiliteSelectedColor(downButton, info->color);
      GadgetButtonSetHiliteSelectedBorderColor(downButton, info->borderColor);
...
//=============================================================================
GameWindow *GUIEdit::newTextEntry(GameWindow *parent, Int x, Int y, Int width,
                                  Int height) {
  GameWindow *window;
  WinInstanceData instData;
  UnsignedInt status = WIN_STATUS_ENABLED | WIN_STATUS_IMAGE;
  ...
  window = TheWindowManager->gogoGadgetTextEntry(
      parent, status, x, y, width, height, &instData, &entryData, NULL, TRUE);
...
//=============================================================================
GameWindow *GUIEdit::newStaticText(GameWindow *parent, Int x, Int y, Int width,
                                   Int height) {
  GameWindow *window;
  WinInstanceData instData;
  UnsignedInt status = WIN_STATUS_ENABLED | WIN_STATUS_IMAGE;
  ...
  window = TheWindowManager->gogoGadgetStaticText(
      parent, status, x, y, width, height, &instData, &textData, NULL, TRUE);
...
void GUIEdit::createStatusBar(void) {
  ...
  // create the bar
  m_statusBarHWnd = CreateStatusWindow(
      WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | CCS_BOTTOM | SBARS_SIZEGRIP,
      "Ready", m_appHWnd, STATUS_BAR_ID);
...
//=============================================================================
void GUIEdit::createToolbar(void) {}  // end createToolbar
...
Bool GUIEdit::menuExit(void) {
  ...
  //
  if (m_unsaved) {
    result = MessageBox(m_appHWnd, "Save file before quitting?", "Save?",
                        MB_YESNOCANCEL);
    if (result == IDCANCEL)
  ...
  if (result == IDYES) DestroyWindow(m_appHWnd);
...
Bool GUIEdit::menuNew(void) {
  ...
  if (m_unsaved == TRUE) {
    ...
    if (result == IDCANCEL)
      ...
      if (success == FALSE) {
        result = MessageBox(m_appHWnd,
                            "File not saved.  Proceed with new layout anyway?  "
                            "Current layout will be LOST!",
                            "File Not Saved", MB_YESNO);
        if (result == IDNO)
...
Bool GUIEdit::menuOpen(void) {
  ...
  if (m_unsaved == TRUE) {
    ...
    if (result == IDCANCEL)
      ...
      if (success == FALSE) {
        result = MessageBox(m_appHWnd,
                            "File not saved.  Proceed with open layout anyway? "
                            " Current layout will be LOST!",
                            "File Not Saved", MB_YESNO);
        if (result == IDNO)
          return TRUE;  // they chose to proceed anyway, no error
  ...
  if (filePath == NULL) return FALSE;  // file not opened
...
Bool GUIEdit::menuSave(void) {
  ...
  if (strlen(m_savePathAndFilename) == 0) return menuSaveAs();
...
Bool GUIEdit::menuSaveAs(void) {
  ...
  // if no filename came back they cancelled this operation
  if (filePath == NULL) return FALSE;  // save not done
...
Bool GUIEdit::menuCopy(void) {
  ...
  if (select == NULL) {
    MessageBox(m_appHWnd,
               "You must have windows selected before you can copy them.",
...
void GUIEdit::selectWindow(GameWindow *window) {
  ...
  if (isWindowSelected(window) == TRUE) return;
  ...
  if (m_selectList) m_selectList->prev = entry;
  m_selectList = entry;

  // select this window in the hierarchy if there is only one selected
  if (selectionCount() == 1) TheHierarchyView->selectWindow(window);
...
void GUIEdit::clearSelections(void) {
  while (m_selectList) unSelectWindow(m_selectList->window);
...
void GUIEdit::notifyNewWindow(GameWindow *window) {
  ...
  //
  if ((windowIsGadget(window) == FALSE) ||
      (window->winGetStyle() & GWS_TAB_CONTROL)) {
    GameWindow *child;
...
void GUIEdit::deleteSelected(void) {
  ...
  // delete them all
  for (i = 0; i < count; i++) deleteWindow(deleteList[i]);
...
void GUIEdit::dragMoveSelectedWindows(ICoord2D *dragOrigin,
  ...
  // sanity
  if (dragOrigin == NULL || dragDest == NULL) return;
...
GameWindow *GUIEdit::getFirstSelected(void) {
  if (m_selectList) return m_selectList->window;
 ...
 * and on the screen */
//=============================================================================
void GUIEdit::computeSafeLocation(GameWindow *window, Int x, Int y, Int *safeX,
                                  Int *safeY) {
  Int dx, dy;
  ICoord2D origin, size;
  GameWindow *parent;
 ...
 */
//=============================================================================
void GUIEdit::computeSafeSizeLocation(GameWindow *window, Int newX, Int newY,
                                      Int newWidth, Int newHeight, Int *safeX,
                                      Int *safeY, Int *safeWidth,
                                      Int *safeHeight) {
  GameWindow *parent;
  ICoord2D parentLoc;
  ICoord2D parentSize;
  ...
  if (parent) topLeftLimit = parentLoc;
 ...
 * window should be */
//=============================================================================
void GUIEdit::computeResizeLocation(EditMode resizeMode, GameWindow *window,
                                    ICoord2D *resizeOrigin,
                                    ICoord2D *resizeDest, ICoord2D *resultLoc,
                                    ICoord2D *resultSize) {
  ...
  switch (resizeMode) {
    ...
    case MODE_RESIZE_TOP_LEFT:
      ...
      if (newX >= bottomRight.x - sizeLimit) newX = bottomRight.x - sizeLimit;
      if (newY >= bottomRight.y - sizeLimit) newY = bottomRight.y - sizeLimit;
    ...
    case MODE_RESIZE_TOP_RIGHT:
      ...
      // don't let it invert
      if (newSizeX < sizeLimit) newSizeX = sizeLimit;
      if (newSizeY < sizeLimit) newSizeY = sizeLimit;
  ...
  computeSafeSizeLocation(window, newX, newY, newSizeX, newSizeY, &resultLoc->x,
                          &resultLoc->y, &resultSize->x, &resultSize->y);
...
void GUIEdit::gridSnapLocation(ICoord2D *source, ICoord2D *snapped) {
  // sanity
  if (source == NULL || snapped == NULL) return;
 ...
 * if present */
//=============================================================================
Bool GUIEdit::isNameDuplicate(GameWindow *root, GameWindow *ignore,
                              AsciiString name) {
  WinInstanceData *instData;

  // end of recursion, sanity for name, and empty name ("") is always OK
  if (root == NULL || name.isEmpty()) return FALSE;  // name is a-ok! :)
  ...
  // You only call this on the first child since the call right after it will
  // handle siblings (depth first)
...
void GUIEdit::setShowSeeThruOutlines(Bool show) {
  ...
  if (m_showSeeThruOutlines)
    ...
    CheckMenuItem(GetMenu(m_appHWnd), MENU_SHOW_SEE_THRU_OUTLINES,
                  MF_UNCHECKED);
