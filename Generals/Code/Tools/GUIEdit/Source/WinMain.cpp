...
char *gAppPrefix = "ge_";  /// So GuiEdit can have a different debug log file
                           /// name if we need it
...
static BOOL initInstance(HINSTANCE hInstance, int nCmdShow) {
  ...
  if (ApplicationHWnd == NULL) return FALSE;
    ...
    case WM_COMMAND: {
      ...
      switch (controlID) {
        ...
        case MENU_SHOW_SEE_THRU_OUTLINES:
          TheEditor->setShowSeeThruOutlines(
              !TheEditor->getShowSeeThruOutlines());
          break;
        ...
        case MENU_BACKGROUND_COLOR:
          ...
          if (TheEditWindow) {
            ...
            newColor = SelectColor(
                (Int)(255.0f * color.red), (Int)(255.0f * color.green),
                (Int)(255.0f * color.blue), (Int)(255.0f * color.alpha),
                mouse.x, mouse.y);
            if (newColor) {
        ...
        // --------------------------------------------------------------------
        case MENU_ABOUT: {
          DialogBox(ApplicationHInstance, (LPCTSTR)ABOUT_DIALOG, hWnd,
    ...
    case WM_KEYDOWN: {
      ...
      switch (virtualKey) {
        ...
        case VK_DELETE: {
          ...
          if (TheEditor) TheEditor->deleteSelected();
          break;
        ...
        case VK_LEFT: {
          ...
          if (TheEditor && TheEditWindow) {
            if (TheEditor->getMode() == MODE_EDIT) {
              if (TheEditor->selectionCount() <= 0) break;
              ICoord2D zero;
              zero.x = zero.y = 0;
...
//=============================================================================
LRESULT CALLBACK AboutCallback(HWND hDlg, UINT message, WPARAM wParam,
                               LPARAM lParam) {
  switch (message) {
    case WM_INITDIALOG:
      return TRUE;
