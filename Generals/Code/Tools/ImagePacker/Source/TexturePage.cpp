...
//
// Desc:			 This class represents a texture that contains
// packed
//						 images.
//
//-----------------------------------------------------------------------------
 ...
 * same for both */
//=============================================================================
void TexturePage::extendToRowIfOpen(char *src, Int buffWidth, Int buffBPP,
                                    Bool extendAlpha, Int imageHeight,
                                    UnsignedInt fitBits, Int srcX, Int srcY) {
  char otherAlpha;
  char otherColor[3];
  char *row = NULL;
  ...
  if (row) {
    ...
    if (buffBPP == 4) {
      if (otherAlpha == 0) otherOpen = TRUE;

    }  // end if
    else {
      if (otherColor[0] == 0 && otherColor[1] == 0 && otherColor[2] == 0)
        otherOpen = TRUE;
 ...
 */
//=============================================================================
void TexturePage::extendImageEdges(Byte *destBuffer, Int destWidth,
                                   Int destHeight, Int destBPP,
                                   ImageInfo *image, Bool extendAlpha) {
  // sanity
  if (destBuffer == NULL || image == NULL) return;
  ...
  for (y = 0; y < imageHeight; y++) {
    ...
    for (x = 0; x < imageWidth; x++) {
      ...
      currPixel = FALSE;
      if (destBPP == 4) {
        if (alpha != 0) currPixel = TRUE;

      }  // end if
      else {
        if (color[0] != 0 && color[1] != 0 && color[2] != 0) currPixel = TRUE;
      ...
      //
      if (currPixel == TRUE)
        extendToRowIfOpen(ptr, destWidth, destBPP, extendAlpha, imageHeight,
                          image->m_fitBits, x, y);
      ...
      if (prevPixel == FALSE && currPixel == TRUE) {
        ...
        if (x != 0 || BitTest(image->m_fitBits, ImageInfo::FIT_XBORDER_LEFT)) {
          ...
          if (destBPP == 4 && extendAlpha) *(ptr - 4) = alpha;
          *(ptr - 3) = color[0];
          *(ptr - 2) = color[1];
          *(ptr - 1) = color[2];
        ...
        //
        DEBUG_ASSERTCRASH(x != 0,
                          ("Coming from off image and detecting right edge!"));

        // extend the previous pixel to this location
        if (destBPP == 4) {
          if (extendAlpha) ptr[0] = *(ptr - 4);
          ptr[1] = *(ptr - 3);
          ptr[2] = *(ptr - 2);
          ptr[3] = *(ptr - 1);
 ...
 */
//=============================================================================
Bool TexturePage::addImageData(Byte *destBuffer, Int destWidth, Int destHeight,
                               Int destBPP, ImageInfo *image) {
  // sanity
  if (destBuffer == NULL || image == NULL) return FALSE;
  ...
  if (BitTest(image->m_status, ImageInfo::ROTATED90C) == FALSE) {
    ...
    for (y = 0; y < image->m_size.y; y++) {
      ...
      src = sourceBuffer +
            ((image->m_size.y - 1 - y) * image->m_size.x * sourceBPP);

      // compute destination location
      dest = destBuffer +
             ((destHeight - 1 - (image->m_pagePos.lo.y + y)) * destWidth *
              destBPP) +
             (image->m_pagePos.lo.x * destBPP);
    ...
    for (y = 0; y < image->m_size.y; y++) {
      ...
      for (x = 0; x < image->m_size.x; x++) {
        // compute destination location
        dest = destBuffer +
               (((destHeight - 1) - (image->m_pagePos.lo.y + x)) * destWidth *
                destBPP) +
  ...
  // the image we just copied into the texture page and "bleed" the edges
  // outward and if a border is present, into the border
  //
  if (BitTest(TheImagePacker->getGapMethod(),
              ImagePacker::GAP_METHOD_EXTEND_RGB))
    extendImageEdges(destBuffer, destWidth, destHeight, destBPP, image, FALSE);
 ...
 */
//=============================================================================
UnsignedInt TexturePage::buildFitRegion(IRegion2D *region, Int startX,
                                        Int startY, Int imageWidth,
                                        Int imageHeight, Int *xGutter,
                                        Int *yGutter, Bool allSidesBorder) {
  // sanity
  if (region == NULL || xGutter == NULL || yGutter == NULL) return 0;
  ...
  // returned below)
  //
  if (imageWidth == m_size.x - 1) xBorder = 1;
  if (imageHeight == m_size.y - 1) yBorder = 1;

  // build the region
  region->lo.x = startX;
  region->lo.y = startY;
  region->hi.x =
      startX + imageWidth - 1 + *xGutter + xBorder;  // -1 for zero based
  region->hi.y =
      startY + imageHeight - 1 + *yGutter + yBorder;  // -1 for zero based
  ...
  if (*xGutter != 0) BitSet(fitBits, ImageInfo::FIT_XGUTTER);
  if (*yGutter != 0) BitSet(fitBits, ImageInfo::FIT_YGUTTER);
  if (xBorder >= 1) BitSet(fitBits, ImageInfo::FIT_XBORDER_RIGHT);
  if (xBorder == 2) BitSet(fitBits, ImageInfo::FIT_XBORDER_LEFT);
  if (yBorder >= 1) BitSet(fitBits, ImageInfo::FIT_YBORDER_BOTTOM);
  if (yBorder == 2) BitSet(fitBits, ImageInfo::FIT_YBORDER_TOP);
...
TexturePage::~TexturePage(void) {
  ...
  if (m_targa) delete m_targa;

  // delete the final image buffer if present
  if (m_packedImage) delete[] m_packedImage;
...
Bool TexturePage::addImage(ImageInfo *image) {
  ...
  useGutter =
      BitTest(TheImagePacker->getGapMethod(), ImagePacker::GAP_METHOD_GUTTER);
  ...
  while (triesLeft) {
    ...
    for (y = 0; y < m_size.y; y++) {
      for (x = 0; x < m_size.x; x++) {
        ...
        //
        // compute the region of the image at this location, the region that
        // will be used will take up the image region AND the gutter.  UNLESS
        // the image is as big as the texture page, in that case there is no
        // reason to use a gutter size.  Also note that if we're trying to fit
        ...
        // build the region
        fitBits = buildFitRegion(&region, x, y, imageWidth, imageHeight,
                                 &xGutter, &yGutter, useRGBExtend);
        ...
        if (region.hi.x >= m_size.x) {
          ...
          //
          if (region.hi.x - xGutter < m_size.x) xGutter = m_size.x - imageWidth;
        ...
        // the 4 corners
        //
        if (lineUsed(region.lo.x, region.lo.y, region.hi.x,
                     region.lo.y) ||  // top side
            lineUsed(region.hi.x, region.lo.y, region.hi.x,
                     region.hi.y) ||  // right side
            lineUsed(region.lo.x, region.hi.y, region.hi.x,
                     region.hi.y) ||  // bottom side
            lineUsed(region.lo.x, region.lo.y, region.lo.x,
                     region.hi.y))  // left side
          continue;
        ...
        //
        if (tryRotate == TRUE) BitSet(image->m_status, ImageInfo::ROTATED90C);
        ...
        if (m_imageList) m_imageList->m_prevPageImage = image;
        m_imageList = image;
...
Bool TexturePage::generateTexture(void) {
  ...
  DEBUG_ASSERTCRASH(
      m_packedImage == NULL,
      ("The packed image list must be NULL before generating texture\n"));
  DEBUG_ASSERTCRASH(
      m_targa == NULL,
      ("The targa must be NULL before generating a new texture\n"));
  ...
  for (image = m_imageList; image; image = image->m_nextPageImage) {
    if (addImageData(m_packedImage, m_size.x, m_size.y, bpp, image) == FALSE) {
      BitSet(m_status, PAGE_ERROR);
...
void TexturePage::getPixel(Int x, Int y, Byte *r, Byte *g, Byte *b, Byte *a) {
  ...
  if (bpp == 4) {
    ...
    *b = buf[3];
    ...
    if (a) *a = (char)0xff;  // no data, just return solid alpha
