...
//						 separate image files and
//combines them into a single 						 image as close as possible so that we can conserve
//texture 						 memory
//
//-----------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////
...
// DEFINES ////////////////////////////////////////////////////////////////////
char *gAppPrefix =
    "ip_";  // So IP can have a different debug log file name if we need it.
...
TexturePage *ImagePacker::createNewTexturePage(void) {
  ...
  if (m_pageList) m_pageList->m_prev = page;
  m_pageList = page;

  // add the tail pointer if this is the first page
  if (m_pageTail == NULL) m_pageTail = page;
...
Bool ImagePacker::validateImages(void) {
  ...
  if (errors == TRUE) {
    proceed =
        DialogBox(ApplicationHInstance, (LPCTSTR)IMAGE_ERRORS,
                  TheImagePacker->getWindowHandle(), (DLGPROC)ImageErrorProc);
...
Bool ImagePacker::packImages(void) {
  ...
  for (i = 0; i < m_imageCount; i++) {
    ...
    if (BitTest(image->m_status, ImageInfo::CANTPROCESS)) continue;
...
void ImagePacker::writeFinalTextures(void) {
  ...
  for (page = m_pageTail; page; page = page->m_prev) {
    ...
    sprintf(buffer, "Generating texture #%d of %d.", page->getID(),
            m_pageCount);
  ...
  if (errors == TRUE) {
    DialogBox(ApplicationHInstance, (LPCTSTR)PAGE_ERRORS,
              TheImagePacker->getWindowHandle(), (DLGPROC)PageErrorProc);
...
void ImagePacker::sortImageList(void) {
  ...
  qsort((void *)m_imageList, m_imageCount, sizeof(ImageInfo *),
        sortImageCompare);
...
void ImagePacker::addImagesInDirectory(char *dir) {
  ...
  if (hFile != INVALID_HANDLE_VALUE) {
    ...
    if (!(item.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) &&
        strcmp(item.cFileName, ".") && strcmp(item.cFileName, "..")) {
      len = strlen(item.cFileName);
      if (len > 4 && item.cFileName[len - 4] == '.' &&
    ...
    while (FindNextFile(hFile, &item) != 0) {
      ...
      if (!(item.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) &&
        ...
        len = strlen(item.cFileName);
        if (len > 4 && item.cFileName[len - 4] == '.' &&
            (item.cFileName[len - 3] == 't' ||
             item.cFileName[len - 3] == 'T') &&
            (item.cFileName[len - 2] == 'g' ||
             item.cFileName[len - 2] == 'G') &&
            (item.cFileName[len - 1] == 'a' ||
             item.cFileName[len - 1] == 'A')) {
          sprintf(filePath, "%s%s", dir, item.cFileName);
          addImage(filePath);
...
Bool ImagePacker::checkOutputDirectory(void) {
  ...
  if (fileCount != 0) {
    ...
    sprintf(buffer,
            "The output directory (%s) must be empty before proceeding.  "
            "Delete '%d' files and continue with build process?",
            m_outputDirectory, fileCount);
    response = MessageBox(NULL, buffer, "Delete files to continue?",
...
void ImagePacker::addDirectory(char *path, Bool subDirs) {
  ...
  for (dir = m_dirList; dir; dir = dir->m_next)
    if (stricmp(dir->m_path, path) == 0) return;  // already in list

  // save our current directory
  GetCurrentDirectory(_MAX_PATH, currDir);

  // set our directory to this one
  if (SetCurrentDirectory(path) == 0) return;  // directory does not exist
  ...
  if (m_dirList) m_dirList->m_prev = dir;
  m_dirList = dir;
...
void ImagePacker::addImage(char *path) {
  ...
  // update status
  sprintf(m_statusBuffer, "Loading Image %d of %d.", m_imageCount,
          m_imagesInDirs);
  statusMessage(m_statusBuffer);
...
Bool ImagePacker::generateINIFile(void) {
  ...
  for (page = m_pageTail; page; page = page->m_prev) {
    ...
    if (BitTest(page->m_status, TexturePage::PAGE_ERROR)) continue;

    // go through each image on this page
    for (image = page->getFirstImage(); image; image = image->m_nextPageImage) {
      //
...
Bool ImagePacker::getSettingsFromDialog(HWND dialog) {
  ...
  if (IsDlgButtonChecked(dialog, RADIO_TARGET_OTHER)) {
    ...
    MessageBox(NULL, "Internal Error. Target Size Unknown.", "Error",
               MB_OK | MB_ICONERROR);
    return FALSE;
  ...
  if (IsDlgButtonChecked(dialog, CHECK_INI) == BST_CHECKED) createINI = TRUE;
  ...
  for (i = 0; i < len; i++) {
    ...
    for (Int j = 0; j < illegalLen; j++) {
      if (m_outputFile[i] == illegal[j]) {
        ...
        sprintf(buffer,
                "Output filename '%s' contains one or more of the following "
                "illegal characters:\n\n%s",
  ...
  for (i = 0; i < count; i++) {
    ...
    SendDlgItemMessage(dialog, LIST_FOLDERS, LB_GETTEXT, i, (LPARAM)buffer);
...
Bool ImagePacker::init(void) {
  ...
  m_targa = new Targa;
  if (m_targa == NULL) {
    DEBUG_ASSERTCRASH(m_targa,
                      ("Unable to allocate targa header during init\n"));
...
Bool ImagePacker::process(void) {
  ...
  if (packImages()) {
    ...
    if (createINIFile() == TRUE) generateINIFile();

    // update preview window
    UpdatePreviewWindow();

    // all done
    sprintf(m_statusBuffer,
            "Image Packing Complete: '%d' Texture Pages Generated from '%d' "
            "Images in '%d' Folder(s)",
