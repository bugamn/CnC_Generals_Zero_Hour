...
//=============================================================================
BOOL CALLBACK ImageErrorProc(HWND hWndDialog, UINT message, WPARAM wParam,
                             LPARAM lParam) {
  switch (message) {
    // ------------------------------------------------------------------------
    case WM_INITDIALOG: {
      ...
      for (i = 0; i < count; i++) {
        ...
        if (BitTest(image->m_status, ImageInfo::CANTPROCESS)) {
          ...
          sprintf(buffer, "%s: (%dx%dx%d) %s", reason, image->m_size.x,
                  image->m_size.y, image->m_colorDepth, image->m_path);
