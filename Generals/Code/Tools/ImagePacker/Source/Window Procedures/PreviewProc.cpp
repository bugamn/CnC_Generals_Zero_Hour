...
LRESULT CALLBACK PreviewProc(HWND hWnd, UINT message, WPARAM wParam,
  ...
  switch (message) {
    case WM_PAINT: {
      ...
      TexturePage *page;
      for (page = TheImagePacker->getFirstTexturePage(); page;
           page = page->m_next) {
        // if this is not the target page, ignore it
        if (page->getID() != TheImagePacker->getTargetPreviewPage()) continue;
        ...
        if (TheImagePacker->getUseTexturePreview()) {
          ...
          ImageInfo *image;
          for (image = page->getFirstImage(); image;
...
void UpdatePreviewWindow(void) {
  ...
  // construct title
  sprintf(title, "Page #%d of %d", TheImagePacker->getTargetPreviewPage(),
  ...
  MoveWindow(preview, clientRect.left, clientRect.top,
