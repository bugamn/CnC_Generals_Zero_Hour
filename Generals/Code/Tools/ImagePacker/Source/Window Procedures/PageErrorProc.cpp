...
//=============================================================================
BOOL CALLBACK PageErrorProc(HWND hWndDialog, UINT message, WPARAM wParam,
                            LPARAM lParam) {
  switch (message) {
    // ------------------------------------------------------------------------
    case WM_INITDIALOG: {
      ...
      // sanity
      if (TheImagePacker == NULL) return TRUE;
      ...
      for (page = TheImagePacker->getFirstTexturePage(); page;
        ...
        if (BitTest(page->m_status, TexturePage::PAGE_ERROR)) {
          ...
          sprintf(buffer, "%s: (%dx%d) %s%d", reason, page->getWidth(),
                  page->getHeight(), TheImagePacker->getOutputFile(),
                  page->getID());
