...
static void selectDrive(HWND dialog) {
  ...
  Int index;
  index = SendDlgItemMessage(dialog, COMBO_DRIVE, CB_FINDSTRINGEXACT, -1,
                             (LPARAM)string);
  if (index != CB_ERR)
...
//=============================================================================
BOOL CALLBACK DirectorySelectProc(HWND hWndDialog, UINT message, WPARAM wParam,
                                  LPARAM lParam) {
  ...
  switch (message) {
    ...
    case WM_INITDIALOG: {
      ...
      SendDlgItemMessage(hWndDialog, LIST_DIR, LB_SETHORIZONTALEXTENT, 1280, 0);

      // load the listbox with directory items
      SendDlgItemMessage(hWndDialog, LIST_DIR, LB_DIR,
                         DDL_DIRECTORY | DDL_EXCLUSIVE, (LPARAM) "*.*");
      ...
      // load the drive box
      SendDlgItemMessage(hWndDialog, COMBO_DRIVE, CB_DIR,
                         DDL_DRIVES | DDL_EXCLUSIVE, (LPARAM) "*.*");
      ...
      switch (controlID) {
        ...
        case BUTTON_ADD: {
          ...
          for (Int i = 0; i < count; i++) {
            ...
            if (SendDlgItemMessage(hWndDialog, LIST_DIR, LB_GETSEL, i, 0) > 0) {
              // get the text
              SendDlgItemMessage(hWndDialog, LIST_DIR, LB_GETTEXT, i,
                                 (LPARAM)text);

              // remove the brackets on the text
              Int j, len = strlen(text);
              for (j = 0; j < len - 1; j++) text[j] = text[j + 1];
              text[len - 2] = '\0';

              // ignore the ".." directory
              if (strcmp(text, "..") == 0) continue;
              ...
              if (SendDlgItemMessage(TheImagePacker->getWindowHandle(),
                                     LIST_FOLDERS, LB_FINDSTRINGEXACT, -1,
                                     (LPARAM)toAdd) != LB_ERR) {
                char message[_MAX_PATH + 32];

                sprintf(message, "Ignoring folder '%s', already in list.",
                        toAdd);
              ...
              SendDlgItemMessage(TheImagePacker->getWindowHandle(),
                                 LIST_FOLDERS, LB_INSERTSTRING, -1,
                                 (LPARAM)toAdd);
        ...
        case COMBO_DRIVE: {
          if (notifyCode == CBN_SELCHANGE) {
            ...
            // get selected index
            selected =
                SendDlgItemMessage(hWndDialog, COMBO_DRIVE, CB_GETCURSEL, 0, 0);
            if (selected != CB_ERR) {
              char string[32];

              SendDlgItemMessage(hWndDialog, COMBO_DRIVE, CB_GETLBTEXT,
                                 selected, (LPARAM)string);
              ...
              SendDlgItemMessage(hWndDialog, LIST_DIR, LB_RESETCONTENT, 0, 0);
              SendDlgItemMessage(hWndDialog, LIST_DIR, LB_DIR,
                                 DDL_DIRECTORY | DDL_EXCLUSIVE, (LPARAM) "*.*");
        ...
        case LIST_DIR: {
          if (notifyCode == LBN_SELCHANGE) {
            ...
            // get selected count
            selCount =
                SendDlgItemMessage(hWndDialog, LIST_DIR, LB_GETSELCOUNT, 0, 0);
            ...
            if (selCount > 0) {
              ...
              Int index = 0;
              if (selCount == 1 &&
                  SendDlgItemMessage(hWndDialog, LIST_DIR, LB_GETSEL, index,
                                     0) > 0) {
                char text[_MAX_PATH];

                SendDlgItemMessage(hWndDialog, LIST_DIR, LB_GETTEXT, index,
                                   (LPARAM)text);
                if (strcmp(text, "[..]") == 0) enable = FALSE;
          ...
          if (notifyCode == LBN_DBLCLK) {
            ...
            selected =
                SendDlgItemMessage(hWndDialog, LIST_DIR, LB_GETCURSEL, 0, 0);

            // get text of selected item
            SendDlgItemMessage(hWndDialog, LIST_DIR, LB_GETTEXT, selected,
                               (LPARAM)text);
