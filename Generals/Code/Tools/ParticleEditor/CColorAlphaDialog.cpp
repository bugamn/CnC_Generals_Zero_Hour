...
static const UINT colorControls[][2] = {{IDC_PSEd_Color1, IDC_PSEd_CF1_Frame},
...
void CColorAlphaDialog::performUpdate(IN Bool toUI) {
  ...
  {  // update the colors
    for (int i = 0; i < MAX_KEYFRAMES; ++i) {
      ...
      CButtonShowColor *pSwatch =
          (CButtonShowColor *)GetDlgItem(colorControls[i][0]);
...
void CColorAlphaDialog::onColorPress(Int colorPressed) {
  CColorDialog dlg(CButtonShowColor::RGBtoBGR(
      m_colorButton[colorPressed - 1].getColor().getAsInt()));
  dlg.m_cc.Flags |= (CC_FULLOPEN | CC_ANYCOLOR);
  dlg.m_cc.lpCustColors = m_customColors;
  if (dlg.DoModal() == IDOK) {
    m_colorButton[colorPressed - 1].setColor(
        CButtonShowColor::BGRtoRGB(dlg.GetColor()));
    static char buff[ARBITRARY_BUFF_SIZE];
