...
CButtonShowColor::~CButtonShowColor() { DestroyWindow(); }

// Convert from 0x00RRGGBB to 0x00BBGGRR
COLORREF CButtonShowColor::RGBtoBGR(Int color) {
  return ((color & 0x00FF0000) >> 16 | (color & 0x0000FF00) << 0 |
          (color & 0x000000FF) << 16);
}

// Convert from 0x00BBGGRR to 0x00RRGGBB
Int CButtonShowColor::BGRtoRGB(COLORREF color) { return RGBtoBGR(color); }
