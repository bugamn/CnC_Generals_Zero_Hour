...
MoreParmsDialog::MoreParmsDialog(UINT nIDTemplate, CWnd *pParentWnd)
  ...
  {  // Update all fields on this panel.
    ...
    {  // slave system
      ...
      if (pCombo->GetCount() == 0) {
        // This is done here because InitPanel is called before Particle Systems
        // have been sent over.
    ...
    {  // slave system
      ...
      if (pCombo) {
        if (toUI) {
          pParent->getPerParticleSystemFromSystem(buff,
                                                  ARBITRARY_BUFF_SIZE - 1);
          if (buff[0] == 0) {
            pCombo->SelectString(-1, NONE_STRING);
          } else {
            pCombo->SelectString(-1, buff);
    ...
    {  // wind motion
      ...
      if (pCombo) {
        ...
        if (toUI) {
          ...
          if (selndx >= 0) {
            pParent->updateWindMotionToSystem(
                (ParticleSystemInfo::WindMotion)(selndx + 1));
          }
        }
      }
    }
