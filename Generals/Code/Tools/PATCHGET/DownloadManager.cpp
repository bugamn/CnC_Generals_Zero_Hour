...
// ////////////////////////////////////////////////////// Generals download
// manager code Author: Matthew D. Campbell, July 2002
...
HRESULT DownloadManager::downloadFile(std::string server, std::string username,
                                      std::string password, std::string file,
                                      std::string localfile, std::string regkey,
                                      bool tryResume) {
  return m_download->DownloadFile(server.c_str(), username.c_str(),
                                  password.c_str(), file.c_str(),
                                  localfile.c_str(), regkey.c_str(), tryResume);
}
...
void DownloadManager::queueFileForDownload(std::string server,
                                           ...
                                           std::string regkey, bool tryResume) {
  QueuedDownload q;
  q.file = file;
...
HRESULT DownloadManager::downloadNextQueuedFile(void) {
  ...
  if (it != m_queuedDownloads.end()) {
    ...
    m_wasError = m_sawEnd = false;
    return downloadFile(q.server, q.userName, q.password, q.file, q.localFile,
                        q.regKey, q.tryResume);
  } else {
...
HRESULT DownloadManager::OnProgressUpdate(int bytesread, int totalsize,
                                          int timetaken, int timeleft) {
  DEBUG_LOG(("DownloadManager::OnProgressUpdate(): %d/%d %d/%d\n", bytesread,
             totalsize, timetaken, timeleft));
  return S_OK;
}
