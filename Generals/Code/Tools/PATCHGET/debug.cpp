...
static int doCrashBox(const char *buffer, bool logResult) {
  ...
  result = ::MessageBox(NULL, buffer, "Assertion Failure",
                        MB_ABORTRETRYIGNORE | MB_APPLMODAL | MB_ICONWARNING);
...
void DebugCrash(const char *format, ...) {
  ...
  if (strlen(theBuffer) >= sizeof(theBuffer))
    ::MessageBox(NULL, "String too long for debug buffers", "",
                 MB_OK | MB_APPLMODAL);
  ...
  if (result == IDIGNORE && TheCurrentIgnoreCrashPtr != NULL) {
    int yn;
    yn = ::MessageBox(NULL, "Ignore this crash from now on?", "",
