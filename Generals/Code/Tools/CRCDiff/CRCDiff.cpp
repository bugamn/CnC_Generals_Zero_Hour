...
static void exitWait(void) { system("PAUSE"); }
...
static void outputLine(int frame, int index, int linkNum, const char *class1,
                       const char *line1, const char *class2, const char *line2,
                       bool same = false) {
  dumpQueued();
  if (!line1) line1 = "&nbsp;";
  if (!line2) line2 = "&nbsp;";
...
static void queueLine(int frame, int index, const char *line1) {
  ...
  if (queuedLines.size() > 150) queuedLines.pop_front();
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 7) {
    cout << "Usage: munkeeDiff top.html row.html bottom.html in1.txt in2.txt "
            "out.txt"
         << endl;
  ...
  while (fileOk[0] || fileOk[1]) {
    for (int i = 0; i < 2; ++i) {
      if (fileOk[i] == true && lastFrame[i] < 0) {
        fileOk[i] =
            getNextLine(ifp[i], lastLine[i], lastFrame[i], lastIndex[i]);
      }
    }
    ...
    if (fileOk[0] && fileOk[1]) {
      if (lastFrame[0] < lastFrame[1] ||
        ...
        // if (!seenLeft)
        // cout << "Seen left on " << lastFrame[0] << ":" << lastIndex[0] <<
        // endl;
        // seenLeft = true;
        if (seenRight && seenLeft) {
          outputLine(lastFrame[0], lastIndex[0], linkNum++, "leftOnly",
                     lastLine[0], NULL, NULL);
          ++numDiffs;
        }
        lastFrame[0] = -1;
      } else if (lastFrame[1] < lastFrame[0] || (lastFrame[1] == lastFrame[0] &&
                                                 lastIndex[1] < lastIndex[0])) {
        // if (!seenRight)
        // cout << "Seen right on " << lastFrame[1] << ":" << lastIndex[1] <<
        // endl;
        // seenRight = true;
        if (seenRight && seenLeft) {
          outputLine(lastFrame[1], lastIndex[1], linkNum++, NULL, NULL,
                     "rightOnly", lastLine[1]);
          ++numDiffs;
        }
        lastFrame[1] = -1;
      } else {
        ...
        if (res != 0) {
          if (!seenLeft || !seenRight)
            cout << "Seen both on " << lastFrame[0] << ":" << lastIndex[0]
                 << endl;
          seenLeft = seenRight = true;
          outputLine(lastFrame[0], lastIndex[0], linkNum++, "leftDiff",
                     lastLine[0], "rightDiff", lastLine[1]);
          ++numDiffs;
        } else {
          // if (!seenLeft || !seenRight)
          //	cout << "Seen both on " << lastFrame[0] << ":" << lastIndex[0]
          //<< endl; seenLeft = seenRight = true;
          static bool printedFirst = false;
          if (!printedFirst) {
            outputLine(lastFrame[0], lastIndex[0], linkNum, "leftSame",
                       lastLine[0], "rightSame", lastLine[1], true);
            printedFirst = true;
