...
void Expander::clear(void) { m_expansions.clear(); }

void Expander::expand(const std::string& input, std::string& output,
                      bool stripUnknown) {
  output = "";
  ...
  while (pos != input.npos) {
    ...
    // input.substr(pos+m_left.length()).c_str()));
    ...
    if (endpos != input.npos) {
      ...
      std::string sub =
          input.substr(pos + m_left.length(), endpos - pos - m_left.length());
