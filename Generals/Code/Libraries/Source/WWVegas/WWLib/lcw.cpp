 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * LCW_Comp -- Performes LCW compression on a block of data. *
 ...
 *                                                                                             *
 *    This routine will compress a block of data using the LCW compression
 *method. LCW has     * the primary characteristic of very fast uncompression at
 *the expense of very slow        * compression times. *
 *                                                                                             *
 * INPUT:   source   -- Pointer to the source data to compress. *
 *                                                                                             *
 *          dest     -- Pointer to the destination location to store the
 *compressed data       * to. *
 *                                                                                             *
 *          datasize -- The size (in bytes) of the source data to compress. *
 *                                                                                             *
 * OUTPUT:  Returns with the number of bytes of output data stored into the
 *destination        * buffer. *
 *                                                                                             *
 * WARNINGS:   Be sure that the destination buffer is big enough. The maximum
 *size required    * for the destination buffer is (datasize + datasize/128). *
 *                                                                                             *
...
int LCW_Comp(void const* source, void* dest, int datasize) {
  ...
  __asm {
      ...
      // n=11xxxxxx,w1			med run		run x+3 bytes from
      // offset w1 n=11111111,w1,w2		long run	run w1 bytes
      // from offset w2 n=10000000			end		end of
      // data reached
...
searchdone:
	__asm {
    mov ecx,
        [count]  //; get the count of the longest run
        mov edi,
        [ndest]  //; get the offset of our compressed data
        cmp ecx,
        2                //; see if its not enough run to matter
        jbe short lenin  //; if its 0,1, or 2 its too small

            cmp ecx,
        10               //; if not, see if it would fit in a short
        ja short medrun  //; if not, see if its a medium run

            mov eax,
        esi  //; if its short get the current address
            sub eax,
        [matchoff]  //; sub the offset of the match
        cmp eax,
        0FFFh            //; if its less than 12 bits its a short
...
len:
	__asm {
    mov ebx,
        [lenoff]  //; get the offset of the length code
        cmp[ebx],
        0BFh       //; see if its maxed out
  ...
  __asm {
      ...
      //		inc	[BYTE PTR ebx] //; inc the count code
		...
		mov	[inlen],1            //; we are now in a length so save it
                   //		mov	[DWORD PTR inlen],1	//; we are now in a
      //length so save it
...
outofhere:
	__asm {
    mov ax,
        080h   //; remember to send an end of data code
        stosb  //; store it
            mov eax,
        edi  //; get the last compressed address
            sub eax,
        [a1stdest]  //; sub the first for the compressed size
        mov[retval],
        eax
  }
#endif
