...
MixFileFactoryClass::MixFileFactoryClass(const char *mix_filename,
  ...
  if (file && file->Is_Available()) {
    ...
    if (IsValid) {
      IsValid =
          (::memcmp(header.signature, "MIX1", sizeof(header.signature)) == 0);
    }
    ...
    if (IsValid) {
      ...
      IsValid =
          (file->Read(&FileCount, sizeof(FileCount)) == sizeof(FileCount));
    }
...
MixFileFactoryClass::~MixFileFactoryClass(void) { FileInfo.Resize(0); }

bool MixFileFactoryClass::Build_Filename_List(
...
FileClass *MixFileFactoryClass::Get_File(char const *filename) {
  ...
  //	Binary search for the file in this mixfile. If it is found, then create
  //the file
  ...
  if (Get_Temp_Filename(path, full_path)) {
    ...
    for (int index = 0; index < FilenameList.Count(); index++) {
      ...
      if (file_data != NULL) {
        ...
        //
        for (int temp_index = 0; temp_index < PendingAddFileList.Count();
             temp_index++) {
          if (filename.Compare_No_Case(
                  PendingAddFileList[temp_index].Filename) == 0) {
    ...
    for (index = 0; index < PendingAddFileList.Count(); index++) {
      new_mix_file.Add_File(PendingAddFileList[index].FullPath,
                            PendingAddFileList[index].Filename);
    }
  }
...
**
*/
bool MixFileFactoryClass::Get_Temp_Filename(const char *path,
                                            StringClass &full_path) {
  bool retval = false;
...
void MixFileCreator::Add_File(const char *source_filename,
    ...
    if (file && file->Is_Available()) {
      ...
      WWDEBUG_SAY(
          ("Saving File %s CRC %08X Offset %d (0x%08X) Size %d (0x%08X)\n",
           saved_filename, info.CRC, info.Offset, info.Offset, info.Size,
           info.Size));
