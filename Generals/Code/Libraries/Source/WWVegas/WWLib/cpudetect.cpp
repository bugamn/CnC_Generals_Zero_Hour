...
static void Get_OS_Info(OSInfoStruct& os_info, unsigned OSVersionPlatformId,
...
CPUDetectClass::ProcessorManufacturerType
    CPUDetectClass::ProcessorManufacturer =
        CPUDetectClass::MANUFACTURER_UNKNOWN;
...
const char* CPUDetectClass::Get_Processor_Manufacturer_Name() {
  static const char* ManufacturerNames[] = {"<Unknown>", "Intel", "UMC",
                                            "AMD",       "Cyrix", "NextGen",
                                            "VIA",       "Rise",  "Transmeta"};
...
static unsigned Calculate_Processor_Speed(__int64& ticks_per_second) {
  ...
  ticks_per_second =
      (__int64)((1000.0 / (double)elapsed) * (double)t);  // Ticks per second
...
void CPUDetectClass::Init_Processor_Speed() {
  ...
  // Loop until two subsequent samples are within 5% of each other (max 5
  // iterations).
...
void CPUDetectClass::Init_Processor_Manufacturer() {
  VendorID[0] = 0;
  unsigned max_cpuid;
  CPUID(max_cpuid, (unsigned&)VendorID[0], (unsigned&)VendorID[8],
        (unsigned&)VendorID[4], 0);
...
void CPUDetectClass::Process_Cache_Info(unsigned value) {
  switch (value) {
    ...
    case 0x04:  // Data TLB, 4M pages, 4-way set associative, 8 entries
      break;
    case 0x06:  // Instruction cache, 8K, 4-way set associative, 32 byte line
                // size
      L1InstructionCacheSize = 8 * 1024;
      L1InstructionCacheLineSize = 32;
      L1InstructionCacheSetAssociative = 4;
      break;
    case 0x08:  // Instruction cache 16K, 4-way set associative, 32 byte line
                // size
    ...
    case 0x40:  // No L2 cache (P6 family), or No L3 cache (Pentium 4 processor)
      // Nice of Intel, Pentium4 has an exception and this field is defined as
      // "no L3 cache"
    ...
    case 0x41:  // Unified cache, 32 byte cache line,4-way set associative, 128K
      ...
      break;
    case 0x42:  // Unified cache, 32 byte cache line, 4-way set associative,
                // 256K
      L2CacheSize = 256 * 1024;
      L2CacheLineSize = 32;
      L2CacheSetAssociative = 4;
      break;
    case 0x43:  // Unified cache, 32 byte cache line, 4-way set associative,
                // 512K
    ...
    case 0x45:  // Unified cache, 32 byte cache line, 4-way set associative, 2M
      ...
      L2CacheSetAssociative = 4;
      break;
    case 0x50:  // Instruction TLB, 4K, 2M or 4M pages, fully associative, 64
                // entries
      break;
    case 0x51:  // Instruction TLB, 4K, 2M or 4M pages, fully associative, 128
                // entries
      break;
    case 0x52:  // Instruction TLB, 4K, 2M or 4M pages, fully associative, 256
                // entries
      break;
    ...
    case 0x5D:  // Data TLB, 4K or 4M pages, fully associative, 256 entries
      break;
    case 0x66:  // Data cache, sectored, 64 byte cache line, 4 way set
                // associative, 8K
      L1DataCacheSize = 8 * 1024;
      L1DataCacheLineSize = 64;
      L1DataCacheSetAssociative = 4;
      break;
    case 0x67:  // Data cache, sectored, 64 byte cache line, 4 way set
                // associative, 16K
      L1DataCacheSize = 16 * 1024;
      L1DataCacheLineSize = 64;
      L1DataCacheSetAssociative = 4;
      break;
    case 0x68:  // Data cache, sectored, 64 byte cache line, 4 way set
                // associative, 32K
    ...
    case 0x72:  // Instruction Trace cache, 8 way set associative, 32K uOps
      ...
      L1InstructionTraceCacheSetAssociative = 8;
      break;
    case 0x79:  // Unified cache, sectored, 64 byte cache line, 8 way set
                // associative, 128K
      L2CacheSize = 128 * 1024;
      L2CacheLineSize = 64;
      L2CacheSetAssociative = 8;
      break;
    case 0x7A:  // Unified cache, sectored, 64 byte cache line, 8 way set
                // associative, 256K
      L2CacheSize = 256 * 1024;
      L2CacheLineSize = 64;
      L2CacheSetAssociative = 8;
      break;
    case 0x7B:  // Unified cache, sectored, 64 byte cache line, 8 way set
                // associative, 512K
      L2CacheSize = 512 * 1024;
      L2CacheLineSize = 64;
      L2CacheSetAssociative = 8;
      break;
    case 0x7C:  // Unified cache, sectored, 64 byte cache line, 8 way set
                // associative, 1M
      L2CacheSize = 1024 * 1024;
      L2CacheLineSize = 64;
      L2CacheSetAssociative = 8;
      break;
    case 0x82:  // Unified cache, 32 byte cache line, 8 way set associative,
                // 256K
      L2CacheSize = 256 * 1024;
      L2CacheLineSize = 32;
      L2CacheSetAssociative = 8;
      break;
    case 0x83:  // Unified cache, 32 byte cache line, 8 way set associative,
                // 512K
      L2CacheSize = 512 * 1024;
      L2CacheLineSize = 32;
      L2CacheSetAssociative = 8;
...
void CPUDetectClass::Init_Cache() {
  ...
  // This code can only determine cache sizes for cpuid count 1 (see Intel
  // manuals for more info on the subject)
    ...
    case 6:
      switch (ProcessorModel) {
          ...
          // This one is either PII, PIIXeon or Celeron with no L2 cache!
          {
            ...
            if (L2CacheSize == 0) {
              IntelProcessor = INTEL_PROCESSOR_CELERON_MODEL_5;
                       ...
                       512 * 1024) {  // If Xeon has 512k L2 cache we will
                                      // assume a PII - tough luck!
        ...
        case 6:
          ...
          break;
        case 7:
          if (L2CacheSize <= 512 * 1024) {  // If Xeon has 512k L2 cache we will
                                            // assume a PIII - tough luck!
...
void CPUDetectClass::Init_Processor_String() {
  ...
  //	if (ProcessorManufacturer==MANUFACTURER_INTEL &&
  //IntelProcessor<INTEL_PROCESSOR_PENTIUM4) { 		ext_id.Eax=0;	// No support
  //for extended cpuid in Intel cpus prior to Pentium4
  //	}
  // If extended cpuid level's highest bit is set, we'll get the cpu string from
  // 2/3/4
  if (ext_id.Eax & 0x80000000) {
    CPUDetectClass::CPUID((unsigned&)ProcessorString[0],
                          (unsigned&)ProcessorString[4],
                          (unsigned&)ProcessorString[8],
                          (unsigned&)ProcessorString[12], 0x80000002);
    CPUDetectClass::CPUID((unsigned&)ProcessorString[16],
                          (unsigned&)ProcessorString[20],
                          (unsigned&)ProcessorString[24],
                          (unsigned&)ProcessorString[28], 0x80000003);
    CPUDetectClass::CPUID((unsigned&)ProcessorString[32],
                          (unsigned&)ProcessorString[36],
                          (unsigned&)ProcessorString[40],
                          (unsigned&)ProcessorString[44], 0x80000004);
  }
  // If no extended cpuid available (Intel processors prior to P4), compose the
  // string
  else {
...
void CPUDetectClass::Init_Processor_Features() {
  ...
  if (ProcessorManufacturer == MANUFACTURER_AMD) {
    if (Has_CPUID_Instruction()) {
      ...
      if (max_ext_id.Eax >=
          0x80000001) {  // Signature & features field available?
...
bool CPUDetectClass::CPUID(unsigned& u_eax_, unsigned& u_ebx_, unsigned& u_ecx_,
                           unsigned& u_edx_, unsigned cpuid_type) {
  if (!Has_CPUID_Instruction())
...
void CPUDetectClass::Init_Processor_Log() {
  ...
  if (CPUDetectClass::Get_L1_Instruction_Cache_Size()) {
    SYSLOG(
        ("L1 Instruction Cache: %d byte cache lines, %d way set associative, "
         "%dk\r\n",
...
void CPUDetectClass::Init_Compact_Log() {
  ...
  Get_OS_Info(os_info, OSVersionPlatformId, OSVersionNumberMajor,
              OSVersionNumberMinor, OSVersionBuildNumber);
  ...
  COMPACTLOG(
      ("%s\t%d\t", Get_Processor_Manufacturer_Name(), Get_Processor_Speed()));
...
static class CPUDetectInitClass {
 public:
  CPUDetectInitClass::CPUDetectInitClass() {
    ...
    // Every processor our games run should have CPUID so it would be extremely
    // unlikely for it not to be present. One can never be sure about the clones
    // though...
...
OSInfoStruct Windows9xVersionTable[] = {
    ...
    {"WIN95", "B20UPD", "Windows 95B OSR 2.1 final Update", 4, 0, 950, 4, 3,
     1212},
    ...
    {"WIN98", "BETA3", "Windows 98 Beta 3", 4, 10, 1650, /*,3*/ 4, 10,
     1650 /*,3*/},
    {"WIN98", "BETA3", "Windows 98 Beta 3", 4, 10, 1650, /*,8*/ 4, 10,
     1650 /*,8*/},
    ...
    {"WIN98", "SP1B2", "Windows 98 SP1 Beta 2", 4, 10, 2150, /*,0*/ 4, 10,
     2150 /*,0*/},
    {"WIN98", "SP1B2", "Windows 98 SP1 Beta 2", 4, 10, 2150, /*,4*/ 4, 10,
     2150 /*,4*/},
    ...
    {"WINME", "MEB1", "Windows ME Beta 1", 4, 90, 2419, /*,4*/ 4, 90,
     2419 /*,4*/},
    ...
    {"WINME", "MEB3", "Windows ME Beta 3", 4, 90, 2499, /*,3*/ 4, 90,
     2499 /*,3*/},
...
void Get_OS_Info(OSInfoStruct& os_info, unsigned OSVersionPlatformId,
                 unsigned OSVersionNumberMajor, unsigned OSVersionNumberMinor,
                 unsigned OSVersionBuildNumber) {
  ...
  switch (OSVersionPlatformId) {
    default:
      ...
      break;
    case VER_PLATFORM_WIN32_WINDOWS: {
      for (int i = 0; i < sizeof(Windows9xVersionTable) / sizeof(os_info);
           ++i) {
