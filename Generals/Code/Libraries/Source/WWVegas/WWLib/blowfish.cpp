 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * BlowfishEngine::Decrypt -- Decrypts data using blowfish
 *algorithm.                        * BlowfishEngine::Encrypt -- Encrypt an
 *arbitrary block of data.                            *
 *   BlowfishEngine::Process_Block -- Process a block of data using Blowfish
 *algorithm.        * BlowfishEngine::Sub_Key_Encrypt -- Encrypts a block for
 *use in S-Box processing.          * BlowfishEngine::Submit_Key -- Submit a key
 *that will allow data processing.               *
 ...
 * BlowfishEngine::~BlowfishEngine -- Destructor for the Blowfish engine. *
 *                                                                                             *
 *    This destructor will clear out the s-box tables so that even if the memory
 *for the       * class remains, it will contain no compromising data. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *                                                                                             *
 *    This routine must be called before any data can be encrypted or decrypted.
 *This routine  * need only be called when the key is to be changed or set for
 *the first time. Once the    * key has been set, the engine may be used to
 *encrypt, decrypt, or both operations         * indefinitely. The key must be
...
void BlowfishEngine::Submit_Key(void const *key, int length) {
  ...
  */
  ...
  /*
  **	Perform a similar transmutation to the S-Box tables. Also notice that
  *the *	working 64 bit number is carried into this process from the
  *previous *	operation.
  */
 ...
 *                                                                                             *
 *    Use this routine to encrypt an arbitrary block of data. The block must be
 *an even        * multiple of 8 bytes. Any bytes left over will not be
 *encrypted. The 8 byte requirement   * is necessary because the underlying
 *algorithm processes blocks in 8 byte chunks.         * Partial blocks are
 *unrecoverable and useless.                                            *
 *                                                                                             *
 * INPUT:   plaintext-- Pointer to the data block to be encrypted. *
 *                                                                                             *
 *          length   -- The length of the data block. *
 *                                                                                             *
 *          cyphertext- Pointer to the output buffer that will hold the
 *encrypted data.        *
 *                                                                                             *
 * OUTPUT:  Returns with the actual number of bytes encrypted. *
 *                                                                                             *
 * WARNINGS:   You must submit the key before calling this routine. This will
 *only encrypt     * the plaintext in 8 byte increments. Modulo bytes left over
 *are not processed.   *
 *                                                                                             *
 * HISTORY: * 04/14/1996 JLB : Created. *
 *=============================================================================================*/
 ...
 *                                                                                             *
 *    Use this routine to decrypt an arbitrary block of data. The block must be
 *an even        * multiple of 8 bytes. Any bytes left over will not be
 *decrypted. The 8 byte requirement   * is necessary because the underlying
 *algorithm processes blocks in 8 byte chunks.         * Partial blocks are
 *unrecoverable and useless.                                            *
 *                                                                                             *
 ...
 *          length   -- The length of the data block. *
 *                                                                                             *
 *          plaintext-- Pointer to the output buffer that will hold the
 *decrypted data.        *
 *                                                                                             *
 * OUTPUT:  Returns with the actual number of bytes decrypted. *
 *                                                                                             *
 * WARNINGS:   You must submit the key before calling this routine. This will
 *only decrypt     * the cyphertext in 8 byte increments. Modulo bytes left over
 *are not processed.  *
 *                                                                                             *
 * HISTORY: * 04/14/1996 JLB : Created. *
 *=============================================================================================*/
 ...
 *algorithm.          *
 *                                                                                             *
 *    This is the main processing routine for encryption and decryption. The
 *algorithm         * consists of a 16 round Feistal network and uses
 *mathematics from different algebraic     * groups (strengthens against
 *differential cryptanalysis). The large S-Boxes and the       * rounds
 *strengthen it against linear cryptanalysis. *
 *                                                                                             *
 * INPUT:   plaintext   -- Pointer to the source text (it actually might be a
 *pointer to       * the cyphertext if this is called as a decryption process).
 **
 *                                                                                             *
 *          cyphertext  -- Pointer to the output buffer that will hold the
 *processed block.    *
 *                                                                                             *
 *          ptable      -- Pointer to the permutation table. This algorithm will
 *encrypt       * and decrypt using the same S-Box tables. The encryption
 *control     * is handled by the permutation table. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
...
void BlowfishEngine::Process_Block(void const *plaintext, void *cyphertext,
  ...
  *encryption/decryption *	process. Since there is an exchange that occurs
  ...
  */
  ...
  /*
  **	The final two longs in the permutation table are processed into the
...
/***********************************************************************************************
 * BlowfishEngine::Sub_Key_Encrypt -- Encrypts a block for use in S-Box
 *processing.            *
 *                                                                                             *
 *    This is the same as the normal process block function but it doesn't have
 *the endian     * fixup logic. Since this routine is only called for S-Box
 *table generation and it is      * known that the S-Box initial data is already
 *in local machine endian format, the         * byte order fixups are not
 *needed. This also has a tendency to speed up S-Box generation  * as well. *
 *                                                                                             *
 * INPUT:   left  -- The left half of the data block. *
 *                                                                                             *
 *          right -- The right half of the data block. *
 *                                                                                             *
 * OUTPUT:  none, but the processed block is stored back into the left and right
 *half          * integers. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
/*
**	These tables have the bytes stored in machine endian format. Because of
*this, *	a special block cypher routine is needed when the sub-keys are
*generated. *	This is kludgier than it otherwise should be. However, storing
*these *	integers in machine independent format would be even more
*painful.
*/
