 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * DSurface::Blit_From -- Blit from one surface to this one. *
 *   DSurface::Blit_From -- Blit graphic memory from one rectangle to another. *
 *   DSurface::Build_Hicolor_Pixel -- Construct a hicolor pixel according to the
 *surface pixel * DSurface::Build_Remap_Table -- Build a highcolor remap table.
 ...
 *   DSurface::DSurface -- Create a surface attached to specified DDraw Surface
 *Object.        * DSurface::DSurface -- Default constructor for surface object.
 ...
 *   DSurface::Restore_Check -- Checks for and restores surface memory if
 *necessary.           * DSurface::Stride -- Fetches the bytes between rows. *
 ...
 * DSurface::DSurface -- Off screen direct draw surface constructor. *
 *                                                                                             *
 *    This constructor will create a Direct Draw enabled surface in video memory
 *if possible.  * Such a surface will be able to use hardware assist if
 *possible. The surface created      * is NOT visible. It only exists as a work
 *surface and cannot be flipped to the visible    * surface. It can only be
 *blitted to the visible surface.                                  *
 *                                                                                             *
 ...
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   The surface pixel format is the same as that of the visible
 *display mode. It    * is important to construct surfaces using this routine,
 *only AFTER the display   * mode has been set. *
 *                                                                                             *
 * HISTORY: * 02/07/1997 JLB : Created. *
 *=============================================================================================*/
DSurface::DSurface(int width, int height, bool system_memory,
 ...
 *                                                                                             *
 *    This default constructor for a surface object should not be used. Although
 *it properly   * creates a non-functional surface, there is no use for such a
 *surface. This default       * constructor is provided for those rare cases
 *where symatics require a default            * constructor. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 *                                                                                             *
 * HISTORY: * 06/21/2000 NAK : Created. *
 *=============================================================================================*/
HDC DSurface::GetDC(void) {
  HDC hdc = NULL;
  HRESULT hr;

  // We have to remove all current locks to get the device context
  // unfortunately...
  while (LockCount) {
    Unlock();
    DCUnlockCount++;
...
int DSurface::ReleaseDC(HDC hdc) {
  ...
  // ReleaseDC() unlocks the surface internally, so we need to reflect that
  // here.
 ...
 * DSurface::Create_Primary -- Creates a primary (visible) surface. *
 *                                                                                             *
 *    This routine is used to create the surface object that represents the
 *currently          * visible display. The surface is not allocated, it is
 *merely linked to the preexisting    * surface that the Windows GDI is also
 *currently using.                                    *
 *                                                                                             *
 * INPUT:   backsurface -- Optional pointer to specify where the backpage (flip
 *enabled)       * pointer will be placed. If this parameter is NULL, then no *
 *                         back surface will be created. *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the primary surface. *
 *                                                                                             *
 * WARNINGS:   There can be only one primary surface. If an additional call to
 *this routine    * is made, another surface pointer will be returned, but it
 *will point to the     * same surface as before. *
 *                                                                                             *
 * HISTORY: * 02/07/1997 JLB : Created. *
 *=============================================================================================*/
DSurface *DSurface::Create_Primary(DSurface **backsurface1) {
  ...
  */
  ...
  HRESULT result = DirectDrawObject->CreateSurface(surface->Description,
  ...
  if (result == DD_OK) {
    ...
    */
    ...
    surface->BytesPerPixel =
        (surface->Description->ddpfPixelFormat.dwRGBBitCount + 7) / 8;
    surface->IsPrimary = true;

    //		surface->Window.Set(Rect(0, 0, surface->Description->dwWidth,
    //surface->Description->dwHeight));
    ...
    */
    memcpy(&PixelFormat, &surface->Description->ddpfPixelFormat,
 ...
 *Object.          *
 *                                                                                             *
 *    If an existing Direct Draw Surface Object is available, use this
 *constructor to create   * a DSurface object that is attached to the surface
 *specified.                             *
 *                                                                                             *
...
DSurface::DSurface(LPDIRECTDRAWSURFACE surfaceptr)
  ...
  if (SurfacePtr != NULL) {
    ...
    if (result == DD_OK) {
      BytesPerPixel = (Description->ddpfPixelFormat.dwRGBBitCount + 7) / 8;
      //			Window.Set(Rect(0, 0, Description->dwWidth,
 ...
 * DSurface::Bytes_Per_Pixel -- Fetches the bytes per pixel of the surface. *
 *                                                                                             *
 *    This routine will return with the number of bytes that each pixel
 *consumes. The value    * is dependant upon the graphic mode of the display. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine will return the number of bytes to add so that the pointer
 *will be          * positioned at the same column, but one row down the screen.
 *This value may very well     * NOT be equal to the width multiplied by the
 *bytes per pixel.                             *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 * DSurface::Lock -- Fetches a working pointer into surface memory. *
 *                                                                                             *
 *    This routine will return with a pointer to the pixel at the location
 *specified. In order * to directly manipulate surface memory, the surface
 *memory must be mapped into the        * program's logical address space. In
 *addition, all blitter activity on the surface will   * be suspended. Every
 *call to Lock must be have a corresponding call to Unlock if the      * pointer
 *returned is not equal to NULL. *
 *                                                                                             *
 * INPUT:   point -- Pixel coordinate to return a pointer to. *
 *                                                                                             *
 * OUTPUT:  Returns with a pointer to the pixel specified. If the return value
 *is NULL, then   * the surface could not be locked and no call to Unlock should
 *be performed.         *
 *                                                                                             *
 * WARNINGS:   It is important not to keep a surface locked indefinately since
 *the blitter     * will not be able to function. Due to the time that locking
 *consumes, it is      * also important to not perform unnecessarily frequent
 *Lock calls.                *
 *                                                                                             *
 * HISTORY: * 02/07/1997 JLB : Created. *
 *=============================================================================================*/
void *DSurface::Lock(Point2D point) const {
  ...
  if (LockCount == 0) {
    ...
    HRESULT result = SurfacePtr->Lock(
        NULL, &desc, DDLOCK_SURFACEMEMORYPTR | DDLOCK_WAIT, NULL);
 ...
 * DSurface::Unlock -- Unlock a previously locked surface. *
 *                                                                                             *
 *    After a surface has been successfully locked, a call to the Unlock()
 ...
 *                                                                                             *
 * WARNINGS:   Only pair a call to Unlock if the prior Lock actually returned a
 *non-NULL       * value. *
 *                                                                                             *
 * HISTORY: * 02/07/1997 JLB : Created. *
 *=============================================================================================*/
 ...
 *necessary.             *
 *                                                                                             *
 *    This routine will check to see if surface memory has been lost to the
 *surface. If it     * has, then the surface memory will be restored. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  none *
 ...
 * DSurface::Blit_From -- Blit graphic memory from one rectangle to another. *
 *                                                                                             *
 *    This routine will use the blitter (if possible) to blit a block of graphic
 *memory from   * one screen rectangle to another. If the rectangles do no match
 *in size, scaling may      * be performed. *
 *                                                                                             *
 ...
 * HISTORY: * 02/07/1997 JLB : Created. *
 *=============================================================================================*/
bool DSurface::Blit_From(Rect const &destrect, Surface const &ssource,
                         Rect const &sourcerect, bool trans) {
  return (Blit_From(Get_Rect(), destrect, ssource, ssource.Get_Rect(),
                    sourcerect, trans));
}
 ...
 *                                                                                             *
 *    Use this routine to blit a rectangle from the specified surface to this
 *surface while    * performing clipping upon the blit rectangles specified. *
 *                                                                                             *
 * INPUT:   dcliprect   -- The clipping rectangle to use for this surface. *
 *                                                                                             *
 *          destrect    -- The destination rectangle of the blit. The is
 *relative to the       * dcliprect parameter. *
 *                                                                                             *
 *          ssource     -- The source surface of the blit. *
 *                                                                                             *
 *          scliprect   -- The source clipping rectangle. *
 *                                                                                             *
 *          sourcrect   -- The source rectangle of the blit. This rectangle is
 *relative to     * the source clipping rectangle. *
 *                                                                                             *
 *          trans       -- Is this a transparent blit request? *
 *                                                                                             *
 * OUTPUT:  bool; Was there a blit performed? A 'false' return value would
 *indicate that the   * blit was clipped into nothing. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 05/27/1997 JLB : Created. *
 *=============================================================================================*/
bool DSurface::Blit_From(Rect const &dcliprect, Rect const &destrect,
                         Surface const &ssource, Rect const &scliprect,
                         Rect const &sourcerect, bool trans) {
  if (!dcliprect.Is_Valid() || !scliprect.Is_Valid() || !destrect.Is_Valid() ||
      !sourcerect.Is_Valid())
    return (false);

  /*
  **	For non-direct draw surfaces, perform a manual blit operation. This is
  *also *	necessary if any of the surfaces are currently locked. It is
  *also necessary if the *	blit regions overlap and the blitter cannot
  ...
  */
  ...
  if (Blit_Clip(drect, dwindow, srect, swindow)) {
    ...
    HRESULT result = SurfacePtr->Blt(&xdestrect, source.SurfacePtr, &xsrcrect,
                                     DDBLT_WAIT, NULL);
    return (result == DD_OK);
  }
  return (false);
 ...
 * DSurface::Fill_Rect -- Fills a rectangle with clipping control. *
 *                                                                                             *
 *    This routine will fill a rectangle on this surface, but will clip the
 *request against    * a clipping rectangle first. *
 *                                                                                             *
 * INPUT:   cliprect -- The clipping rectangle to use for this surface. *
 *                                                                                             *
 *          fillrect -- The rectangle to fill with the specified color. The
 *rectangle is       * relative to the clipping rectangle. *
 *                                                                                             *
 *          color    -- The color (surface dependant format) to use when filling
 *the rectangle * pixels. *
 *                                                                                             *
 * OUTPUT:  bool; Was a fill operation performed? A 'false' return value would
 *mean that the   * fill request was clipped into nothing. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 05/27/1997 JLB : Created. *
 *=============================================================================================*/
bool DSurface::Fill_Rect(Rect const &cliprect, Rect const &fillrect,
                         int color) {
  if (!fillrect.Is_Valid()) return (false);

  /*
  **	If the buffer is locked, then using the blitter to perform the fill is
  *not possible. *	In such a case, perform a manual fill of the region.
  */
  if (Is_Locked()) {
  ...
  *performed *	if the desired fill rectangle has been completely clipped away.
  */
  ...
  HRESULT result =
      SurfacePtr->Blt(&rect, NULL, NULL, DDBLT_WAIT | DDBLT_COLORFILL, &fx);
  return (result == DD_OK);
}
 ...
 *surface pixel f *
 *                                                                                             *
 *    This routine will construct a pixel according to the highcolor pixel
 ...
 *                                                                                             *
 * OUTPUT:  Returns with a screen format pixel number that most closesly matches
 *the color     * specified. *
 *                                                                                             *
 * WARNINGS:   The return value is card dependant and only applies to hicolor
 *displays.        *
 *                                                                                             *
 * HISTORY: * 05/27/1997 JLB : Created. *
 *=============================================================================================*/
int DSurface::Build_Hicolor_Pixel(int red, int green, int blue) {
  return (((red >> RedLeft) << RedRight) |
          ((green >> GreenLeft) << GreenRight) |
          ((blue >> BlueLeft) << BlueRight));
}

/***********************************************************************************************
 * DSurface::Build_Remap_Table -- Build a highcolor remap table. *
 *                                                                                             *
 *    This will build a complete hicolor remap table for the palette specified.
 *This table     * can then be used to quickly fetch a pixel that matches the
 *color index of the palette.   *
 *                                                                                             *
 * INPUT:   table -- The location to store the hicolor table. The buffer must be
 *256*2 bytes   * long. *
 *                                                                                             *
...
void DSurface::Build_Remap_Table(unsigned short *table,
  ...
  */
  for (int index = 0; index < 256; index++) {
    table[index] = (unsigned short)Build_Hicolor_Pixel(
        palette[index].Get_Red(), palette[index].Get_Green(),
