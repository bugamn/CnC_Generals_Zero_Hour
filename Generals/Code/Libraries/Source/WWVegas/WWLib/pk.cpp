 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * PKey::Decode_Exponent -- Decodes the exponent back into the key.
 ...
 *   PKey::Encode_Exponent -- Encode the exponent portion of the key into a
 *buffer.            * PKey::Encode_Modulus -- Encode the modulus portion of the
 *key.                            * PKey::Encrypt -- Encrypt blocks of
 *plaintext.                                             * PKey::Generate --
 ...
 * PKey::PKey -- Construct a key using encoded strings. *
 *                                                                                             *
 *    This constructor will construct a key based on the encoded strings
 *supplied.             *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This will store the modulus portion of the key into a buffer. The number
 *of bytes        * stored into the buffer depends on the value of the key. *
 *                                                                                             *
 * INPUT:   buffer   -- Pointer to the buffer that will hold the encoded modulus
 *value.        *
 *                                                                                             *
 * OUTPUT:  Returns with the number of bytes stored to the buffer. *
 *                                                                                             *
 * WARNINGS:   Be sure that the buffer can hold the encoded bytes. This is
 *normally around the * same size as the Crypt_Block_Size() (plus a byte or
 *two).                       *
 *                                                                                             *
 * HISTORY: * 07/08/1996 JLB : Created. *
 *=============================================================================================*/
 ...
 *buffer.              *
 *                                                                                             *
 *    This routine will encode the exponent portion of the key. This is only
 *necessary for the * slow key since the fast key always has an exponent of
 *65537.                             *
 *                                                                                             *
 * INPUT:   buffer   -- Pointer to the buffer that will be filled with the
 *encoded exponent.   *
 *                                                                                             *
 * OUTPUT:  Returns with the nuber of bytes stored into the buffer. *
 *                                                                                             *
 * WARNINGS:   Be sure the buffer is big enough to hold the encoded exponent.
 *Usually this is  * about the same size as the Crypt_Block_Size (plus a byte or
 *two).               *
 *                                                                                             *
 * HISTORY: * 07/08/1996 JLB : Created. *
 *=============================================================================================*/
 ...
 * PKey::Decode_Modulus -- Decodes the modulus value back into the key. *
 *                                                                                             *
 *    This is the counterpart to the Encode_Modulus() function. It will
 *initialize the         * modulus portion of the key with the encoded data
 *supplied.                               *
 *                                                                                             *
 * INPUT:   buffer   -- Pointer to the buffer that holds the previously encoded
 *modulus value. *
 *                                                                                             *
 * OUTPUT:  void *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This is the counterpart to the Encode_Exponent function. It will decode a
 *previously     * encoded exponent portion back into the key. *
 *                                                                                             *
 * INPUT:   buffer   -- Pointer to the buffer that holds the encoded exponent
 *value.           *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 ...
 *                                                                                             *
 *    Public key cryptography relies on having two paired keys. The key used to
 *encrypt        * data must be decrypted by using the other key. Which key
 *designated as the public or     * private key is arbitrary. However, one is
 *faster than the other. Use the faster key for  * the more common operation. *
 *                                                                                             *
 * INPUT:   random   -- Reference to a source of random data. *
 *                                                                                             *
 *          bits     -- The number of bits to use for key generation. Use a
 *number greater     * than 16 but less than 2048. The ideal bit size is one
 *that is evenly   * divisible by 8 and then add one. Practical numbers range
 *from 65 to    * 1025 bits. *
 *                                                                                             *
 *          fastkey  -- Reference to the key that has fast encryption/decryption
 *properties.   *
 *                                                                                             *
 ...
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   This routine can take a very long time. It can take MINUTES to
 *generate a       * 1024 bit key (even on a Pentium Pro 200Mghz machine). *
 *                                                                                             *
...
void PKey::Generate(Straw &random, int bits, PKey &fastkey, PKey &slowkey) {
  ...
  */
  for (;;) {
    ...
    **	the fast key is ALWAYS 65537. Given this, it is possible to economize
    *the *	fast key into being just the modulus and the slow key to being
    *just the *	exponent (presuming the slow key also has access to the fast key
    *so that *	it can get the modulus).
    */
    fastkey.Exponent = e;
    fastkey.Modulus = n;
 ...
 *                                                                                             *
 *    This routine will encrypt the supplied plaintext into cyphertext by
 *processing the input * in block. The source is processed in whole blocks.
 *Partial blocks are not supported by   * public key cryptography. *
 *                                                                                             *
 ...
 **
 *                                                                                             *
 * OUTPUT:  Returns with the number of cypher text bytes placed into the
 *destination buffer.   *
 *                                                                                             *
 * WARNINGS:   Be sure that the destination buffer is big enough to hold the
 *output.           *
 *                                                                                             *
 ...
 * PKey::Decrypt -- Decrypt supplied cyphertext into its original plaintext. *
 *                                                                                             *
 *    This routine will process the supplied cyphertext by breaking it up into
 *blocks and      * then decrypting each block in turn. The block size is
 *dependant upon the key. By NOT     * embedding this information into the
 *cypher data, it makes the encryption more secure.    *
 *                                                                                             *
 * INPUT:   source   -- Pointer to the cypher text to be decrypted. *
 *                                                                                             *
 *          length   -- The number of cypher text bytes supplied to this
 *routine.              *
 *                                                                                             *
 *          dest     -- Pointer to the buffer to hold the plaintext. *
 *                                                                                             *
 * OUTPUT:  Returns with the number of plaintext bytes output to the destination
 *buffer.       *
 *                                                                                             *
 * WARNINGS:   Only whole blocks are processed. If the source has any partial
 *block sized      * data, then it will be left unprocessed. *
 *                                                                                             *
 * HISTORY: * 07/05/1996 JLB : Created. *
 *=============================================================================================*/
