 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * SHAEngine::Result -- Fetch the current digest. * SHAEngine::Hash
...
inline unsigned long _rotl(unsigned long X, int n) {
  return (unsigned long)((X << n) |
                         ((unsigned)X >> ((sizeof(unsigned long) * 8) - n)));
}
...
void SHAEngine::Process_Block(void const *source, SHADigest &acc) const {
  ...
  */
  ...
  for (index = SRC_BLOCK_SIZE / sizeof(long);
       index < PROC_BLOCK_SIZE / sizeof(long); index++) {
