...
unsigned int SecureRandomClass::RandomCache[SecureRandomClass::SHADigestBytes /
                                            sizeof(unsigned int)];
...
void SecureRandomClass::Add_Seeds(unsigned char *values, int length) {
  for (int i = 0; i < length; i++) {
    ...
    for (int j = SeedLength - 1; j >= 1; j--) Seeds[j] = Seeds[j - 1];
    Seeds[0] = uctemp;
  }
...
unsigned long SecureRandomClass::Randval(void) {
  if (RandomCacheEntries == 0) {
    ...
    int_seeds[(SeedLength / sizeof(int)) - 1] ^=
        Counter;  // remove the last counter (double xor)
    int_seeds[(SeedLength / sizeof(int)) - 1] ^=
        (Counter + 1);  // put the new counter in place
...
//
//	Note the use of XORs everywhere.  The XOR of a good random number and a
//bad random 		number is still a good random number.
//
...
void SecureRandomClass::Generate_Seed(void) {
  ...
  if (in) {
    for (i = 0; i < SeedLength; i++) Seeds[i] ^= fgetc(in);
    fclose(in);
  } else
    assert(0);
#else
  ...
  //
  DWORD spc, bps, nfc,
      tnc;  // various drive attributes (we don't care what they mean)
