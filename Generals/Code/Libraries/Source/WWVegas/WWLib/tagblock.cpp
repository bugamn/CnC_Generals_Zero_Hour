 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * TagBlockFile::TagBlockFile -- Create/open tag file
 ** TagBlockFile::~TagBlockFile -- Close down the tag file. *
 *   TagBlockFile::Create_Index -- Create a index into the IndexList sorted by
 *CRC.            * TagBlockFile::Find_Block -- Find block assocated with name.
 ...
 *   TagBlockFile::End_Write_Access -- Stop write access for handle - flushes
 *data bug keeps ha* TagBlockFile::Reset_File -- Clear file so no blocks exist.
 ** TagBlockFile::Empty_Index_List -- Clear out tag block list in memory *
 *---------------------------------------------------------------------------------------------*
 *   TagBlockHandle::Write -- Write data to the block. * TagBlockHandle::Read --
 *Read from a tag block.                                            *
 ...
 *                                                                                             *
 * INPUT: * const char *fname - name of file that is or wants to be a
 *TagBlockFile.              *
 *                                                                                             *
 * OUTPUT: * Will assume a file that has invalid data to be corrupt and will
 *write over it.       * So don't pass in a file that is not a tag file. *
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 05/11/1999 SKB : Created. *
 *=============================================================================================*/
  ...
  if (Header.Version == FILE_VERSION) {
    ...
    for (block = 0; block < Header.NumBlocks; block++) {
      ...
      if (blockheader.Index == block) {
        ...
        curpos = Calc_Tag_Offset(curpos) + blockheader.TagSize +
 ...
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * Any TagBlockHandles that have not been deleted are now invalide
 *but cannot be deleted.* You must delete any handles associated with this
 *before closing the TagFile.          *
 *                                                                                             *
 * HISTORY: * 05/11/1999 SKB : Created. *
 *=============================================================================================*/
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 11/29/1999 SKB : Created. *
 *=============================================================================================*/
...
TagBlockHandle *TagBlockFile::Open_Tag(const char *tagname) {
  ...
  // Keep track of how many handles there are so we can assert if they are not
  // all shut down.
  NumOpenHandles++;
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 05/12/1999 SKB : Created. *
 *=============================================================================================*/
...
/***********************************************************************************************
 * TagBlockFile::End_Write_Access -- Stop write access for handle - flushes data
 *bug keeps han *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 05/12/1999 SKB : Created. *
 *=============================================================================================*/
TagBlockHandle::TagBlockHandle(TagBlockFile *tagfile, TagBlockIndex *tagindex,
