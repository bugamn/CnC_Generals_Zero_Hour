 ...
 *modified.         * RawFileClass::Is_Available -- Checks to see if the
 ...
 *date and time the file was last modified.         * RawFileClass::Set_Name --
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 06/10/1999 PDS : Created. *
 *=============================================================================================*/
    ...
    switch (rights) {
      ...
      */
      ...
      case READ | WRITE:
        ...
        Handle = CreateFileA(Filename, GENERIC_READ | GENERIC_WRITE, 0, NULL,
...
int RawFileClass::Read(void *buffer, int size) {
  ...
  */
  ...
  while (size > 0) {
    ...
    if ((bytesread == 0) && (!feof(Handle))) readok = ferror(Handle);
#else
...
int RawFileClass::Write(void const *buffer, int size) {
  ...
  */
  ...
  if (byteswritten != size) writeok = FALSE;
#else
  writeok =
      WriteFile(Handle, buffer, size, &(unsigned long &)byteswritten, NULL);
#endif
  ...
  if (RawFileClass::Is_Open()) {
    ...
    if (GetFileInformationByHandle(Handle, &info)) {
      ...
      if (DosDateTimeToFileTime((WORD)(datetime >> 16),
        ...
        return (SetFileTime(Handle, &info.ftCreationTime, &filetime,
                            &filetime) != 0);
      }
    }
  }
 ...
 *                                                                                             *
 * INPUT:   pos   -- The position to seek the file relative to the "dir"
 *parameter.            *
 *                                                                                             *
...
/***********************************************************************************************
 * RawFileClass::Detach -- Removes the file handle from the object without
 *closing the handle.
 *                                                                                             *
 * INPUT
 *                                                                                             *
