...
/*
** Statics
** NOTE: If _TheFileFactory is ever changed to point to an object of a different
*class which does
** not derive from SimpleFileFactoryClass, _TheSimpleFileFactory should be set
*to NULL.
*/
...
FileClass *SimpleFileFactoryClass::Get_File(char const *filename) {
  // strip off the path (if needed). Note that if path stripping is off, and the
  // requested file has a path in its name, and the current subdirectory is not
  // empty, the paths will just be concatenated which may not produce reasonable
  // results.
  ...
  RawFileClass *file =
      W3DNEW BufferedFileClass();  // new RawWritingFileClass();
  assert(file);
