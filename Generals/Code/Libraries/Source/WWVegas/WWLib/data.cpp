 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * Load_Alloc_Data -- Allocates a buffer and loads the file into
 *it.                         * Load_Uncompress -- Loads and uncompresses data
 *to a buffer.                               * Hires_Load -- Allocates memory
 *for, and loads, a resolution dependant file.               * Fetch_String --
 ...
 * HISTORY: * 10/17/1994 JLB : Created. *
 *=============================================================================================*/
long Load_Uncompress(FileClass &file, Buffer &uncomp_buff, Buffer &dest_buff,
                     void *reserved_data) {
  unsigned short size;
 ...
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 12/25/1996 JLB : Created. *
 *=============================================================================================*/
char const *Fetch_String(int id) {
  ...
  /*
  **	Determine if the string ID requested is valid. If not then return an
  *empty string pointer.
  */
  if (id == -1 || id == TXT_NONE) return ("");

  /*
  **	Adjust the 'time stamp' tracking value. This is an artificial value used
  *merely to track *	the relative age of the strings requested.
  */
  _time = _time + 1;

  /*
  **	Check to see if the requested string has already been fetched into a
  *buffer. If so, then *	return a pointer to that string (update the time
  *stamp as well).
  */
  ...
  /*
  **	Find a suitable buffer to hold the string to be fetched. The buffer
  *should either be *	empty or have the oldest fetched string.
  */
  int oldest = -1;
  int oldtime = -1;
  ...
  /*
  **	A suitable buffer has been found so fetch the string resource and then
  *return a pointer *	to the string.
  */
  char *stringptr = _buffers[oldest].String;
  _buffers[oldest].ID = id;
  _buffers[oldest].TimeStamp = _time;
  if (LoadString(ProgramInstance, id, stringptr,
...
int Load_Picture(FileClass &file, Buffer &scratchbuf, Buffer &destbuf,
                 unsigned char *palette, PicturePlaneType) {
