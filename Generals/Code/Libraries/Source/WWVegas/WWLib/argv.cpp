 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * CurrentPos -- Create an instance to parse argv with. *
 *   ArgvClass::Free -- Release data allocated. * ArgvClass::Load_File -- Load
 *args from a file.                                            *
 ...
 *                                                                                             *
 * INPUT: * bool case_sensitive - Do you want to perform a case sensitive search
 *(stricmp)?		  * bool exact_size     - Do you want string of same
 *lenght (strncmp) ?						  *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 06/18/1999 SKB : Created. *
 *=============================================================================================*/
ArgvClass::ArgvClass(bool case_sensitive, bool exact_size)
    : Flags(0), LastArg(0), CurrentPos(-1) {
 ...
 *                                                                                             *
 * INPUT: * const char *arg - String to search for. If NULL, LastArg will be
 *used.                 *
 *                                                                                             *
 * OUTPUT: * const char *string found (null if not found)
 **
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 06/17/1999 SKB : Created. *
 *=============================================================================================*/
 ...
 *                                                                                             *
 * INPUT: * LPSTR lpCmdLine - A string of white space seperated strings.  Quotes
 *force spaces to  * be ignored. * char *fileprefix - A prefix on an arguement
 *telling system to load postfix file name  * as command line params. *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * This may be called multible times with different strings. * Once
 *Argc reaches MAX_ARGC, no more will be added. *
 *                                                                                             *
 * HISTORY: * 06/17/1999 SKB : Created. * 07/15/2001 SKB : Put file arguements
 *in the correct order they were included.             *
 *=============================================================================================*/
 ...
 *                                                                                             *
 * INPUT: * const char *fname - file to load. *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
  ...
  if (fp) {
    while (Argc < MAX_ARGC) {
      ...
      if ((*string != '#') && (*string != ';')) {
        ...
        // If there is anyting in the string. (NAK: old code used to fail for 1
        // char options)
        if (strlen(string)) {
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 08/23/1999 SKB : Created. *
 *=============================================================================================*/
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 08/23/1999 SKB : Created. * 06/25/2001 SKB : add flag user can
 *check to see if value was extracted from next location.*
 *=============================================================================================*/
 ...
 *                                                                                             *
 * INPUT: * attrib = cmd line attrib to add/replace * value = new value for
 *attrib                                                          *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
...
void ArgvClass::Update_Value(const char *attrib, const char *value) {
  if ((Find_Value(attrib)) != NULL) {
    ...
    } else  // add new value
    {
      ...
      memmove(&(Argv[CurrentPos + 2]), &(Argv[CurrentPos + 1]),
              sizeof(char *) * (MAX_ARGC - CurrentPos - 2));
      Argv[CurrentPos + 1] = strdup(value);
      Argc++;
    }
 ...
 *                                                                                             *
 * INPUT: * attrib = thing to add * value = new optional value *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
 ...
 *                                                                                             *
 * INPUT: * attrib = thing to remove *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * THIS CONTAINS A POTENTIAL BUG - I don't want to fix it because it
 *might be a desired  * behavior.  Given:
 ** Argv[0] = "-i test" "*.txt" as values in Argv,
 ** calling Remove_Value("-i") will remove *.txt	as well.
 **
 *																															  *
 * HISTORY: * 12/13/1999 NAK : Created. * 06/25/2001 SKB : WARNINGS message *
 *=============================================================================================*/
bool ArgvClass::Remove_Value(const char *attrib) {
  ...
  if ((Find_Value(attrib)) != NULL) {
    ...
    if (((CurrentPos + 1) < Argc) &&
        (Argv[CurrentPos + 1][0] != '-'))  // value for this arg
    {
      free(Argv[CurrentPos + 1]);
      removeCount = 2;
    }
