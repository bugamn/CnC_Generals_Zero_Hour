...
/***********************************************************************************************
  ...
  This class has all of the real implementation.  Templates are provided in the
header file which create typed lists by simply type-casting the void pointers
that GenericMultiList uses.
...
bool GenericMultiListClass::Internal_Add_Tail(MultiListObjectClass *obj,
  ...
  // link the node to the tail of this list (next should be head, prev should be
  // head.Prev)
  node->Prev = Head.Prev;
  node->Next = &(Head);
  node->Next->Prev = node;
...
bool GenericMultiListClass::Internal_Add_After(
    MultiListObjectClass *obj, const MultiListObjectClass *existing_list_member,
    bool onlyonce) {
  WWMEMLOG(MEM_GAMEDATA);
  assert(obj);
  ...
  // find the node hanging off 'existing_list_member' that corresponds to this
  // list (O(numlists))
