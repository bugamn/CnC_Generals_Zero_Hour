 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * Get_CPU_Rate -- Fetch the rate of CPU ticks per second. *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 *                                                                                             *
 *    This routine will query the CPU to determine how many clock per second it
 *is.            *
 *                                                                                             *
 * INPUT:   high  -- Reference to the location that will be filled with the
...
int Get_RDTSC_CPU_Speed(void) {
  ...
  // DWORD	current = 0;				// Elapsed time during
  // loop
  ...
  do {
    ...
    */
    ...
    total = (freq + freq2 + freq3);  // Total last three frequency calcs
           ...
           (tries < 20) && ((abs(3 * freq - total) > 3 * TOLERANCE) ||
