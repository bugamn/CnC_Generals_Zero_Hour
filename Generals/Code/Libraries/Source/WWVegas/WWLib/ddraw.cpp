 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * Set_Video_Mode -- Initializes Direct Draw and sets the required
 *Video Mode                * Process_DD_Result -- Does a message box based on
 *the result of a DD command               * Reset_Video_Mode -- Resets video
 *mode and deletes Direct Draw Object                      *
 *   Get_Free_Video_Memory -- returns amount of free video memory *
 *   Get_Video_Hardware_Caps -- returns bitmask of direct draw video hardware
 *support          * Wait_Vert_Blank -- Waits for the start (leading edge) of a
 *vertical blank                 * Set_Palette -- set a direct draw palette *
 *   Check_Overlapped_Blit_Capability -- See if video driver supports blitting
 *overlapped regions* Wait_Blit -- waits for the DirectDraw blitter to become
 *idle                              *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
LPDIRECTDRAW2 DirectDraw2Interface =
    NULL;  // Pointer to direct draw 2 interface
...
static bool FirstPaletteSet =
    false;  // Is this the first time 'Set_Palette' has been called?
...
void Set_Palette(PaletteClass const &pal, int time, void (*callback)()) {
  ...
  while (timer) {
    /*
    **	Build an intermediate palette that is as close to the destination
    *palette *	as the current time is proportional to the ending time.
    */
    ...
    /*
    **	Remember the current time so that multiple palette sets within the same
    *game *	time tick won't occur. This is probably unnecessary since the
    *palette setting *	code, at the time of this writing, delays at least one
    *game tick in the process *	of setting the palette.
    */
    int holdtime = timer;
    ...
    */
    ...
    /*
    **	This loop ensures that the palette won't be set more than once per game
    *tick. Setting *	the palette more than once per game tick will have no
    *effect since the calculation will *	result in the same intermediate
    *palette that was previously calculated.
    */
 ...
 *                                                                                             *
 * INPUT:		HRESULT result				- the result
 *returned from the direct draw command		  * int     display_ok_msg
 *- should a message be displayed if command ok			  * *
 *                                                                                             *
 * OUTPUT:		none
 **
 *                                                                                             *
 * HISTORY: * 09/27/1995 PWG : Created. *
 *=============================================================================================*/
  ...
  static struct {
    ...
    char const *Message;
      ...
      {DDERR_BLTFASTCANTCLIP,
       "Return if a clipper object is attached to the source surface passed "
       "into a BltFast call."},
      ...
      {DDERR_CANTLOCKSURFACE,
       "Unable to lock surface because no driver exists which can supply a "
       "pointer to the surface."},
      {DDERR_CLIPPERISUSINGHWND,
       "An attempt was made to set a cliplist for a clipper object that is "
      ...
      {DDERR_INCOMPATIBLEPRIMARY,
       "Unable to match primary surface creation request with existing primary "
       "surface."},
      {DDERR_INVALIDCAPS,
      ...
      {DDERR_INVALIDPOSITION,
       "Returned when the position of the overlay on the destination is no "
       "longer legal for that destination."},
      ...
      //		{DDERR_NOANTITEARHW, "Operation could not be carried out
      //because there is no hardware support for synchronizing blts to avoid
      //tearing.	"},
      {DDERR_NOBLTHW, "No blter hardware present."},
      //		{DDERR_NOBLTQUEUEHW, "Operation could not be carried out
      //because there is no hardware support for asynchronous blting."},
      ...
      {DDERR_NOCOLORKEYHW,
       ...
       "support of the destination color key."},
      ...
      {DDERR_NOHWND,
       "Clipper notification requires an HWND or no HWND has previously been "
       "set as the CooperativeLevel HWND."},
      {DDERR_NOMIRRORHW,
       "Operation could not be carried out because there is no hardware "
       "present or available."},
      {DDERR_NOOVERLAYDEST,
       "Returned when GetOverlayPosition is called on an overlay that "
      ...
      {DDERR_NOT8BITCOLOR,
       "DirectDrawSurface is not in 8 bit color mode and the requested "
      ...
      {DDERR_SURFACEALREADYATTACHED,
       "This surface is already attached to the surface it is being attached "
       "to."},
      ...
      {DDERR_SURFACELOST,
       "Access to this surface is being refused because the surface memory is "
      ...
      {DDERR_WASSTILLDRAWING,
       "Informs DirectDraw that the previous Blt which is transfering "
       "information to or from this Surface is incomplete."},
      {DDERR_WRONGMODE,
       "This surface can not be restored because it was created in a different "
       "mode."},
      {DDERR_XALIGN,
       "Rectangle provided was not horizontally aligned on required "
       "boundary."}};
#endif
  /*
  **	If there iwas no error detected, then either bail out or display a
  *message to *	this effect as indicated by the "display_ok_msg" parameter.
  */
  if (result == DD_OK) {
    if (display_ok_msg) {
      MessageBox(MainWindow, "Direct Draw operation processed without error",
                 "Note", MB_OK);
    }
    return;
  }
  ...
  */
  for (int index = 0; index < ARRAY_SIZE(_errors); index++) {
    if (_errors[index].Error == result) {
      MessageBox(MainWindow, _errors[index].Message,
                 "Westwood Library Direct Draw Error",
  ...
  /*
  **	Since it fell out of the above loop, this must be an unrecognized error
  *code.
  */
  char str[80];
 ...
 *overlapped regions*
 *                                                                                             *
 *  We will check for this by drawing something to a video page and blitting it
 *over itself.   * If we end up with the top line repeating then overlapped
 *region blits dont work.            *
 *                                                                                             *
 * INPUT:    Nothing *
 *                                                                                             *
 * OUTPUT:   Nothing *
...
void Check_Overlapped_Blit_Capability(void) {
  ...
  */
  ...
  /*
  ** Blit the buffer down by one line. If we end up with a vertical strip of
  *pixel 255's then
  ** overlapped blits dont work
  */

  test_buffer.Blit(test_buffer, 0, 0, 0, 1, test_buffer.Get_Width(),
                   test_buffer.Get_Height() - 1);
  ...
  if (DirectDrawObject == NULL) {
    ...
    if (result == DD_OK) {
      if (Debug_Windowed) {
        ...
        result = DirectDrawObject->SetCooperativeLevel(
            MainWindow, DDSCL_EXCLUSIVE | DDSCL_FULLSCREEN);
      }
      Process_DD_Result(result, false);
    }
  }
 ...
 *                                                                                             *
 * INPUT:  		int width   			- the width of the video
 *mode in pixels						  * int height
 *- the height of the video mode in pixels                   * int
 *bits_per_pixel	- the number of bits per pixel the video mode supports *
 *                                                                                             *
 * OUTPUT:     none *
 *                                                                                             *
...
bool Set_Video_Mode(HWND, int w, int h, int bits_per_pixel) {
  ...
  //
  // MessageBox(MainWindow, "In Set_Video_Mode. About to call call
  // SetDisplayMode.","Note", MB_ICONEXCLAMATION|MB_OK);
  ...
  //
  // MessageBox(MainWindow, "In Set_Video_Mode. About to call
  // CreatePalette.","Note", MB_ICONEXCLAMATION|MB_OK);
  result = DirectDrawObject->CreatePalette(
      DDPCAPS_8BIT | DDPCAPS_ALLOW256, &PaletteEntries[0], &PalettePtr, NULL);
  Process_DD_Result(result, false);
  if (result != DD_OK) {
    return (false);
  }
  ...
  // MessageBox(MainWindow, "In Set_Video_Mode. About to return
  // success.","Note", MB_ICONEXCLAMATION|MB_OK);
#if (0)
  /*
  ...
  */
  ...
  if (result != DD_OK) {
    ...
    if (capabilities.dwCaps & DDCAPS_CANBLTSYSMEM) {
      ...
      SystemToSystemBlits =
          (capabilities.dwSSBCaps & DDCAPS_BLT) ? true : false;
    }
  }
#endif  //(0)
...
unsigned int Get_Free_Video_Memory(void) {
  ...
  if (DirectDrawObject) {
    ...
    if (DD_OK == DirectDrawObject->GetCaps(&video_capabilities, NULL)) {
      char string[256];
      wsprintf(string, "In Get_Free_Video_Memory. About to return %d bytes",
               video_capabilities.dwVidMemFree);
...
unsigned Get_Video_Hardware_Capabilities(void) {
  ...
  */
  video_capabilities.dwSize = sizeof(video_capabilities);
  // MessageBox(MainWindow, "In Get_Video_Hardware_Capabilities. About to call
  // GetCaps","Note", MB_ICONEXCLAMATION|MB_OK);
  ...
  */
  ...
  // MessageBox(MainWindow, "In Get_Video_Hardware_Capabilities. About to return
  // success.","Note", MB_ICONEXCLAMATION|MB_OK);
  return (video);
}
