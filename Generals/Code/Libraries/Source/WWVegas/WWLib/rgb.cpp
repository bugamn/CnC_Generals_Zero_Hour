 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * RGBClass::Adjust -- Adjust one RGB value toward another. *
 ...
 *                                                                                             *
 *    This routine is used to modify an RGB value to proportionately match
 *another RGB value   * according to the ratio parameter specified. Typical use
 *of this routine is in palette    * fading from one palette to another or to
 *black.                                          *
 *                                                                                             *
 * INPUT:   ratio    -- The ration of transformation. This value is in the form
 *of 0 to 255,   * with 0 being no change, and 255 being 100% transformed into
 *the        * destination color. *
 *                                                                                             *
 *          rgb      -- Reference to the destination RGB color to transform this
 *color into.   *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 ...
 * RGBClass::Difference -- Determines the "distance" between two colors. *
 *                                                                                             *
 *    This routine is used to calculate a relative distance between two colors.
 *The value is   * relative only to itself and thus is useful only for
 *determining the magnitude of         * color difference rather than the nature
 *of the color difference. Palette remapping       * code uses this routine to
 *find closest matches for colors.                               *
 *                                                                                             *
 * INPUT:   rgb   -- Reference to the color to be compared to this color. *
 *                                                                                             *
 * OUTPUT:  Returns the difference between the two colors. The value returned is
 *zero if the   * colors exactly match. The greater the positive value the
 *greater the difference    * between the colors. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
 * HISTORY: * 12/02/1995 JLB : Created. *
...
int RGBClass::Difference(RGBClass const& rgb) const {
  ...
  /*
  **	At first crack, the difference algorithm might be coded as the sum of
  *the color differences *	(or sum of the square of the color distances).
  *However, this would not take advantage of the *	fact that the human eye
  *is most sensative to green, followed by the color blue. With this *	thought
  *in mind, the following difference algorithm is used.
  */
  return (4 * g + 3 * b + 2 * r);
}
 ...
 * RGBClass::operator HSVClass -- Conversion operator for RGB to HSV object. *
 *                                                                                             *
 *    This conversion operator will convert an RGBClass object into an HSVClass
 *object.        *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 * OUTPUT:  Returns with a reference (implicit) to the HSVClass object that most
 *closely       * represents the RGBClass object. *
 *                                                                                             *
 * WARNINGS:   none *
 *                                                                                             *
...
RGBClass::operator HSVClass(void) const {
  ...
  */
  ...
  HSVClass hsv((unsigned char)hue, (unsigned char)saturation,
               (unsigned char)value);
  return (hsv);
}
