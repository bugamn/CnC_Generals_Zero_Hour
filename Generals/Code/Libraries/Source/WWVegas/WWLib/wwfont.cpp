 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * WWFontClass::Print -- Print text to the surface specified. *
 *   WWFontClass::WWFontClass -- Constructor for a font class object. *
 *   WWFontClass::Char_Pixel_Width -- Fetch the pixel width of the character
 *specified.        * WWFontClass::String_Pixel_Width -- Determines the width of
 *the string in pixels.          * WWFontClass::Raw_Width -- Fetch the raw width
 *of a character.                             * WWFontClass::Raw_Height -- Fetch
 *the height of the font.                                  *
 *   WWFontClass::Get_Width -- Get normalized width of the nominal font
 *character.             * WWFontClass::Get_Height -- Fetch the normalized
 *height of the nominal font character.     * WWFontClass::Set_XSpacing -- Set
 *the X spacing override value.                            *
 ...
 * WWFontClass::WWFontClass -- Constructor for a font class object. *
 *                                                                                             *
 *    This constructs a font object as it is based upon the font data output by
 *the legacy     * FONTMAKE.EXE utility. *
 *                                                                                             *
 * INPUT:   fontdata -- Pointer to the data as output by the fontmake utility. *
 *                                                                                             *
 *          isoutlined  -- Is this font data available with outline pixels
 *embedded?           *
 *                                                                                             *
...
WWFontClass::WWFontClass(void const *fontdata, bool isoutlined, int shadow,
                         ConvertClass *convert, unsigned char *remap)
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 01/24/2000 SKB : Created. *
 *=============================================================================================*/
...
int WWFontClass::Char_Pixel_Width(char c) const {
  int raw = (*(((unsigned char *)FontData) + FontData->WidthBlockOffset +
               (unsigned char)c));
  raw += FontXSpacing;
  return (raw);
}
...
/***********************************************************************************************
 * WWFontClass::String_Pixel_Width -- Determines the width of the string in
 *pixels.            *
 *                                                                                             *
 ...
 *                                                                                             *
 * OUTPUT:  Returns with the number of pixels the string would span if it were
 *printed.        *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine returns the nominal width of the font. This width is
 *basically the width of * the widest character in the font. *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
...
int WWFontClass::Raw_Width(void) const {
  return (*(((unsigned char *)FontData) + FontData->InfoBlockOffset +
            FONTINFOMAXWIDTH));
}
 ...
 *                                                                                             *
 *    This returns the height of the font without regard for any Y spacing
 *offsets.            *
 *                                                                                             *
 * INPUT:   none *
 *                                                                                             *
 ...
 *                                                                                             *
 * WARNINGS:   All characters in the font have the same logical height even if
 *they actually   * consume less pixels. *
 *                                                                                             *
 * HISTORY: * 05/26/1997 JLB : Created. *
 *=============================================================================================*/
...
/***********************************************************************************************
 * WWFontClass::Get_Height -- Fetch the normalized height of the nominal font
 *character.       *
 *                                                                                             *
 ...
 *                                                                                             *
 *          cliprect -- The clipping rectangle that both clips the text and
 *biases the print   * location to. *
 *                                                                                             *
 *          point    -- The draw position for the upper left corner of the first
 *character.    *
 *                                                                                             *
 *          convert  -- The pixel convert object that is used to draw the
 *correct colors to    * the destination surface. *
 *                                                                                             *
 ...
 *                                                                                             *
 * WARNINGS:   There are two separate drawing routines; one for old fonts and
 *one for new      * fonts. *
 *                                                                                             *
 * HISTORY: * 05/26/1997 JLB : Created. * 06/20/1887 BNA : Modified to handle
 *new fonts.                                            * 01/24/2000 SKB :
 *Updated from C&C to include BNA code.                                    *
 *   01/24/2000 SKB : put in call for Get_Remap_Palette * 01/24/2000 SKB : Put
 *in call for get converer.                                            *
 *=============================================================================================*/
Point2D WWFontClass::Print(char const *string, Surface &surface,
                           Rect const &cliprect, Point2D const &drawpoint,
                           ConvertClass const &convertref,
  ...
  */
  ...
  // This font palette assumes that the font will be used as defined by
  // converter.
  static unsigned char const fontpalette[16] = {0, 1, 2,  3,  4,  5,  6,  7,
  ...
  /*
  **	Verify that the clipping rectangle (if present) falls within the bounds
  *of the surface.
  */
  ...
  /*
  **	Trivially check to see if the print start position is outside the bounds
  *of the clipping *	rectangle.
  */
  if (xpos >= cliprect.X + cliprect.Width ||
      ypos >= cliprect.Y + cliprect.Height) {
    return (drawpoint);
  }
  ...
  */
  ...
  if (buffer != NULL) {
    ...
    unsigned short *fontheight =
        (unsigned short *)(((unsigned char *)FontData) +
                           FontData->HeightOffset);
    unsigned short *fontoffset =
        (unsigned short *)(((unsigned char *)FontData) +
                           FontData->OffsetBlockOffset);
    ...
    */
    while (*string != '\0') {
      ...
      /*
      **	Certain control characters serve a formatting purpose. They
      *merely *	adjust the next draw position.
      */
      if (c == '\r') {
        xpos = startx;
      ...
      /*
      **	Fetch working values for the character to display. These are
      *used to *	control the size of the character rectangle.
      */
      ...
      */
      Rect crect(xpos, ypos, width + ((xspacing > 0) ? xspacing : 0),
                 *(((unsigned char *)FontData) + FontData->InfoBlockOffset +
                   FONTINFOMAXHEIGHT) +
                     ((yspacing > 0) ? yspacing : 0));

      /*
      **	Check to see if any part of this character would appear within
      *the clipping *	rectangle. If not, then don't process this character.
      */
      ...
      if (crect.Is_Valid()) {
        ...
        **	data with clipping.
        */
        if (FontData->FontCompress !=
            (char)2) {  // if the font is the old style
          unsigned char *dataptr = ((unsigned char *)FontData) + fontoffset[c];
          void *drawbuff =
              (void *)(((char *)buffer) +
                       ((ypos + firstrow) * surface.Stride()) + xpos * bbp);

          for (int h = 0; h < dheight; h++) {
            /*
            **	If the current row to be drawn falls below the clipping
            *rectangle, then *	no further drawing for this character is needed
            *-- bail out.
            */
            if (ypos + firstrow + h >= crect.Y + crect.Height) break;

            /*
            **	If the current row to be drawn falls above the clipping
            *rectangle, *	then skip the row and advance the source data
            *pointer as well.
            */
            if (ypos + firstrow + h < crect.Y) {
              ...
              /*
              **	Process one row of character data. For simplicity
              *reasons, just *	loop through all pixels of this row and perform
              ...
              */
              ...
              while (workwidth > 0) {
                ...
                **	be fetched at the same time since they are nibble
                *packed.
                */
              ...
              /*
              **	Move the output pixel pointer to the next line down but
              *at the *	left margin of the character block.
              */
              drawbuff = (void *)((((char *)drawbuff) - (width * bbp)) +
                                  surface.Stride());
            }
          }
        } else {
