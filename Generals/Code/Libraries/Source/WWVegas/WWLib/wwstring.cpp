...
//
// A trick to optimize strings that are allocated from the stack and used only
// temporarily
//
// For alignment reasons we need twice as large block...
char StringClass::m_TempStrings[(StringClass::MAX_TEMP_STRING * 2) *
                                StringClass::MAX_TEMP_BYTES];
...
void StringClass::Get_String(int length, bool is_temp) {
  ...
  if (is_temp && length <= MAX_TEMP_LEN &&
      ReservedMask != ALL_TEMP_STRINGS_USED_MASK) {
    //
    ...
    for (int index = 0; index < MAX_TEMP_STRING; index++, mask <<= 1) {
      ...
      if (!(ReservedMask & mask)) {
        ...
        temp_string += sizeof(_HEADER);  // The buffer contains header as well,
                                         // and it needs to be at the start
...
void StringClass::Free_String(void) {
  if (m_Buffer != m_EmptyString) {
    unsigned buffer_base =
        reinterpret_cast<unsigned>(m_Buffer - sizeof(StringClass::_HEADER));
    unsigned temp_base = reinterpret_cast<unsigned>(
        m_TempStrings + MAX_TEMP_BYTES * MAX_TEMP_STRING);
