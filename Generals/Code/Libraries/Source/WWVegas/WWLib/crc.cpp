 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * CRCEngine::operator() -- Submits one byte of data to the CRC
 *engine.                      * CRCEngine::operator() -- Submits an arbitrary
 *data block to the CRC engine.               *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 *                                                                                             *
 *    This routine will take the specified byte of data and submit it to the CRC
 *engine        * for processing. This routine is designed to be as fast as
 *possible since the typical     * use of this routine is to feed one of
 *presumably many byte sized chunks of data to the   * CRC engine. *
 *                                                                                             *
 * INPUT:   datum -- One byte of data to submit to the CRC engine. *
 *                                                                                             *
 * OUTPUT:  none *
 *                                                                                             *
 * WARNINGS:   If possible, use the buffer/size operator to submit data rather
 *than repeated   * calls to this routine. *
 *                                                                                             *
 ...
 *                                                                                             *
 *    This routine will submit the specified block to the CRC engine. The block
 *can be of      * arbitrary length. *
 *                                                                                             *
 * INPUT:   buffer   -- Pointer to the buffer that contains the data. The buffer
...
long CRCEngine::operator()(void const *buffer, int length) {
  if (buffer != NULL && length > 0) {
    ...
    */
    ...
    int longcount =
        bytes_left / sizeof(long);  // Whole 'long' elements remaining.
    while (longcount--) {
...
unsigned long CRC::Memory(unsigned char *data, unsigned long length,
                          unsigned long crc) {
