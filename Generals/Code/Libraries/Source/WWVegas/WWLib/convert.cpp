...
ConvertClass::ConvertClass(PaletteClass const &artpalette,
  ...
  **	of the display surface. Check the pixel format and set the values
  *accordingly.
  */
  if (BBP == 1) {
    /*
    ...
    */
    ...
    for (int index = 1; index < 256; index++) {
      trans[index] =
          (unsigned char)screenpalette.Closest_Color(artpalette[index]);
    }
    ...
    */
    PlainBlitter =
        W3DNEW BlitPlainXlat<unsigned char>((unsigned char const *)Translator);
    TransBlitter =
        W3DNEW BlitTransXlat<unsigned char>((unsigned char const *)Translator);
    RemapBlitter = W3DNEW BlitTransZRemapXlat<unsigned char>(
        &RemapTable, (unsigned char const *)Translator);
    ShadowBlitter = W3DNEW BlitTransRemapDest<unsigned char>(ShadowTable);
    Translucent1Blitter = W3DNEW BlitTransRemapXlat<unsigned char>(
        ShadowTable, (unsigned char const *)Translator);
    Translucent2Blitter = W3DNEW BlitTransRemapXlat<unsigned char>(
        ShadowTable, (unsigned char const *)Translator);
    Translucent3Blitter = W3DNEW BlitTransRemapXlat<unsigned char>(
        ShadowTable, (unsigned char const *)Translator);

    /*
    **	Create the RLE aware blitter objects.
    */
    RLETransBlitter = W3DNEW RLEBlitTransXlat<unsigned char>(
        (unsigned char const *)Translator);
    RLERemapBlitter = W3DNEW RLEBlitTransZRemapXlat<unsigned char>(
        &RemapTable, (unsigned char const *)Translator);
    RLEShadowBlitter = W3DNEW RLEBlitTransRemapDest<unsigned char>(ShadowTable);
    RLETranslucent1Blitter = W3DNEW RLEBlitTransRemapXlat<unsigned char>(
        ShadowTable, (unsigned char const *)Translator);
    RLETranslucent2Blitter = W3DNEW RLEBlitTransRemapXlat<unsigned char>(
        ShadowTable, (unsigned char const *)Translator);
    RLETranslucent3Blitter = W3DNEW RLEBlitTransRemapXlat<unsigned char>(
        ShadowTable, (unsigned char const *)Translator);
    ...
    */
    ...
    ((DSurface &)surface)
        .Build_Remap_Table((unsigned short *)Translator, artpalette);
    ...
    **	required for the draw permutations.
    */
    PlainBlitter = W3DNEW BlitPlainXlat<unsigned short>(
        (unsigned short const *)Translator);
    TransBlitter = W3DNEW BlitTransXlat<unsigned short>(
        (unsigned short const *)Translator);
    RemapBlitter = W3DNEW BlitTransZRemapXlat<unsigned short>(
        &RemapTable, (unsigned short const *)Translator);
    ShadowBlitter =
        W3DNEW BlitTransDarken<unsigned short>((unsigned short)maskhalf);
    Translucent1Blitter = W3DNEW BlitTransLucent75<unsigned short>(
        (unsigned short const *)Translator, (unsigned short)maskquarter);
    Translucent2Blitter = W3DNEW BlitTransLucent50<unsigned short>(
        (unsigned short const *)Translator, (unsigned short)maskhalf);
    Translucent3Blitter = W3DNEW BlitTransLucent25<unsigned short>(
        (unsigned short const *)Translator, (unsigned short)maskquarter);

    /*
    **	Create the RLE aware blitter objects.
    */
    RLETransBlitter = W3DNEW RLEBlitTransXlat<unsigned short>(
        (unsigned short const *)Translator);
    RLERemapBlitter = W3DNEW RLEBlitTransZRemapXlat<unsigned short>(
        &RemapTable, (unsigned short const *)Translator);
    RLEShadowBlitter =
        W3DNEW RLEBlitTransDarken<unsigned short>((unsigned short)maskhalf);
    RLETranslucent1Blitter = W3DNEW RLEBlitTransLucent75<unsigned short>(
        (unsigned short const *)Translator, (unsigned short)maskquarter);
    RLETranslucent2Blitter = W3DNEW RLEBlitTransLucent50<unsigned short>(
        (unsigned short const *)Translator, (unsigned short)maskhalf);
    RLETranslucent3Blitter = W3DNEW RLEBlitTransLucent25<unsigned short>(
        (unsigned short const *)Translator, (unsigned short)maskquarter);
  }
}
...
Blitter const *ConvertClass::Blitter_From_Flags(ShapeFlags_Type flags) const {
  ...
  */
  switch (flags &
          (SHAPE_TRANSLUCENT25 | SHAPE_TRANSLUCENT50 | SHAPE_TRANSLUCENT75)) {
...
RLEBlitter const *ConvertClass::RLEBlitter_From_Flags(
    ShapeFlags_Type flags) const {
  ...
  */
  ...
  // shape (since it only compresses transparent pixels and the reason we
  // compress them is so we can skip them easily.)
