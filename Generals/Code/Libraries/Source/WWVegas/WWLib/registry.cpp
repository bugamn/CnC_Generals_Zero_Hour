...
RegistryClass::RegistryClass(const char *sub_key) : IsValid(false) {
  ...
  if (::RegCreateKeyEx(HKEY_LOCAL_MACHINE, sub_key, 0, NULL, 0, KEY_ALL_ACCESS,
...
int RegistryClass::Get_Int(const char *name, int def_value) {
  ...
  if ((::RegQueryValueEx((HKEY)Key, name, NULL, &type, (LPBYTE)&data,
...
void RegistryClass::Set_Int(const char *name, int value) {
  assert(IsValid);
  if (::RegSetValueEx((HKEY)Key, name, 0, REG_DWORD, (LPBYTE)&value,
...
void RegistryClass::Set_Bin(const char *name, const void *buffer,
                            int buffer_size) {
  assert(IsValid);
  assert(buffer != NULL);
  assert(buffer_size > 0);
...
void RegistryClass::Get_String(const char *name, StringClass &string,
  ...
  DWORD type = 0;
  LONG result =
      ::RegQueryValueEx((HKEY)Key, name, NULL, &type, NULL, &data_size);
...
char *RegistryClass::Get_String(const char *name, char *value, int value_size,
  ...
  assert(IsValid);
  DWORD type = 0;
  if ((::RegQueryValueEx((HKEY)Key, name, NULL, &type, (LPBYTE)value,
                         (DWORD *)&value_size) == ERROR_SUCCESS) &&
      (type == REG_SZ)) {
  } else {
    //*value = 0;
...
void RegistryClass::Get_Value_List(DynamicVectorClass<StringClass> &list) {
  ...
  while (::RegEnumValue((HKEY)Key, index++, value_name, &sizeof_name, 0, NULL,
...
void RegistryClass::Get_String(const WCHAR *name, WideStringClass &string,
  ...
  DWORD type = 0;
  LONG result =
      ::RegQueryValueExW((HKEY)Key, name, NULL, &type, NULL, &data_size);
  if (result == ERROR_SUCCESS && type == REG_SZ) {
    //
    //	Read the entry from the registry
    //
    ::RegQueryValueExW((HKEY)Key, name, NULL, &type,
                       (LPBYTE)string.Get_Buffer((data_size / 2) + 1),
                       &data_size);
  }
