...
//	Local inlines
////////////////////////////////////////////////////////////////////////////////////////////////
__inline bool WWAudioClass::Is_OK_To_Give_Handle(
    const AudibleSoundClass &sound_obj) {
  bool is_ok = false;
  AudibleSoundClass::SOUND_TYPE type = sound_obj.Get_Type();
  if (((type == AudibleSoundClass::TYPE_SOUND_EFFECT) &&
       m_AreSoundEffectsEnabled) ||
...
WWAudioClass::DRIVER_TYPE_2D WWAudioClass::Open_2D_Device(LPWAVEFORMAT format) {
  ...
  m_PlaybackBits = (format->nAvgBytesPerSec << 3) /
  ...
  if ((success == AIL_NO_ERROR) && (m_Driver2D != NULL) &&
      (m_Driver2D->emulated_ds == TRUE)) {
    ::AIL_waveOutClose(m_Driver2D);
    success = 2;
    WWDEBUG_SAY(
        ("WWAudio: Detected 2D DirectSound emulation, switching to "
         "WaveOut.\r\n"));
  }
...
//
////////////////////////////////////////////////////////////////////////////////////////////////
WWAudioClass::DRIVER_TYPE_2D WWAudioClass::Open_2D_Device(bool stereo, int bits,
                                                          int hertz) {
  ...
  wave_format.wf.nAvgBytesPerSec =
      (wave_format.wf.nChannels * wave_format.wf.nSamplesPerSec * bits) >> 3;
  ...
  while (
      ((type = Open_2D_Device((LPWAVEFORMAT)&wave_format)) == DRIVER2D_ERROR) &&
...
bool WWAudioClass::Free_Cache_Space(int bytes) {
  ...
  for (int hash_index = 0;
       (hash_index < MAX_CACHE_HASH) && (bytes_freed < bytes); hash_index++) {
...
bool WWAudioClass::Cache_Buffer(SoundBufferClass *buffer,
  ...
  if ((buffer != NULL) && (string_id != NULL) &&
    ...
    int space_needed =
        (m_CurrentCacheSize + buffer->Get_Raw_Length()) - (int)m_MaxCacheSize;
    if (space_needed > 0) {
...
//
////////////////////////////////////////////////////////////////////////////////////////////////
AudibleSoundClass *WWAudioClass::Create_Sound_Effect(
    const char *string_id, unsigned char *raw_wave_data, unsigned long bytes) {
  // Create a new sound object
  AudibleSoundClass *sound_obj = W3DNEW AudibleSoundClass;
  if (Is_Disabled() == false) {
    // Try to find the buffer in our cache, otherwise create a new buffer.
...
AudibleSoundClass *WWAudioClass::Create_Sound(int definition_id,
  ...
  DefinitionClass *definition =
      DefinitionMgrClass::Find_Definition(definition_id);
  if (definition != NULL) {
    //
    ...
    if (definition->Get_Class_ID() == CLASSID_SOUND) {
      AudibleSoundDefinitionClass *sound_def =
          reinterpret_cast<AudibleSoundDefinitionClass *>(definition);
...
AudibleSoundClass *WWAudioClass::Create_Sound(const char *def_name,
  ...
  //
  DefinitionClass *definition =
      DefinitionMgrClass::Find_Typed_Definition(def_name, CLASSID_SOUND, true);
  if (definition != NULL) {
    //
    //	Make sure this is really a sound definition
    //
...
//
////////////////////////////////////////////////////////////////////////////////////////////////
AudibleSoundClass *WWAudioClass::Create_Continuous_Sound(
    int definition_id, RefCountClass *user_obj, uint32 user_data,
    int classid_hint) {
  //
  //	Create an instance of the sound and play it
  //
  AudibleSoundClass *sound =
      Create_Sound(definition_id, user_obj, user_data, classid_hint);
  if (sound != NULL) {
    if (sound->Get_Loop_Count() != INFINITE_LOOPS) {
      WWDEBUG_SAY(
          ("Audio Error:  Creating a continuous sound with a finite loop "
           "count!\r\n"));
    }
  }
...
//
////////////////////////////////////////////////////////////////////////////////////////////////
int WWAudioClass::Create_Instant_Sound(int definition_id, const Matrix3D &tm,
                                       RefCountClass *user_obj,
  ...
  if (sound != NULL) {
    if (sound->Get_Loop_Count() == INFINITE_LOOPS) {
      WWDEBUG_SAY(
          ("Audio Error:  Creating an instant sound %s with an infinite loop "
           "count!\r\n",
...
////////////////////////////////////////////////////////////////////////////////////////////////
AudibleSoundClass *WWAudioClass::Create_Continuous_Sound(
    const char *def_name, RefCountClass *user_obj, uint32 user_data,
    int classid_hint) {
  //
  //	Create an instance of the sound and play it
  //
  AudibleSoundClass *sound =
      Create_Sound(def_name, user_obj, user_data, classid_hint);
  if (sound != NULL) {
    if (sound->Get_Loop_Count() != INFINITE_LOOPS) {
      WWDEBUG_SAY(
          ("Audio Error:  Creating a continuous sound with a finite loop "
...
//
////////////////////////////////////////////////////////////////////////////////////////////////
int WWAudioClass::Create_Instant_Sound(const char *def_name, const Matrix3D &tm,
                                       RefCountClass *user_obj,
                                       uint32 user_data, int classid_hint) {
  int sound_id = 0;
...
void WWAudioClass::Free_Completed_Sounds(void) {
  if (m_CompletedSounds.Count() > 0) {
    ...
    for (int index = 0; index < m_CompletedSounds.Count(); index++) {
      ...
      bool found = false;
      for (int play_index = 0; (play_index < m_Playlist.Count()) && !found;
           play_index++) {
...
bool WWAudioClass::Add_To_Playlist(AudibleSoundClass *sound) {
  ...
  if (sound != NULL) {
    ...
    for (int index = 0;
         (index < m_Playlist.Count()) && (already_added == false); index++) {
...
bool WWAudioClass::Remove_From_Playlist(AudibleSoundClass *sound_obj) {
  ...
  if (sound_obj != NULL) {
    // Loop through all the entries in the playlist
    for (int index = 0; (index < m_Playlist.Count()) && (retval == false);
         index++) {
    ...
    if (sound_obj->Get_Loop_Count() != INFINITE_LOOPS) {
      for (index = 0; index < m_EOSCallbackList.Count(); index++) {
        ...
        LPFNEOSCALLBACK callback =
            m_EOSCallbackList.Get_Callback(index, &user_data);
        if (callback != NULL) {
  ...
  for (int index = 0; (index < m_2DSampleHandles.Count()) && !found; index++) {
    ...
    if (sample != NULL) {
      ...
      if (sound_obj == NULL) {
        ...
        //
        //	Determine if this sound's priority is lesser then the sound we
        //want to play.
        // This is done by comparing both the designer-specified priority and
        // the current runtime priority (which is calculated by distance to the
        // listener).
        //
        ...
        if ((priority < lowest_priority) ||
            (priority == lowest_priority &&
             runtime_priority <= lowest_runtime_priority)) {
...
WWAudioClass::Get_3D_Sample(const Sound3DClass &sound_obj) {
  ...
  for (int index = 0; (index < m_3DSampleHandles.Count()) && !found; index++) {
    ...
    if (sample != NULL) {
      // Get a pointer to the object that is currently using this sample
      AudibleSoundClass *sound_obj =
          (AudibleSoundClass *)::AIL_3D_object_user_data(sample,
                                                         INFO_OBJECT_PTR);
      if (sound_obj == NULL) {
        // Return this sample handle to the caller
        free_sample = sample;
...
bool WWAudioClass::Select_3D_Device(int index) {
  ...
  if ((index >= 0) && (index < m_Driver3DList.Count())) {
    Select_3D_Device(m_Driver3DList[index]->name,
                     m_Driver3DList[index]->driver);
...
int WWAudioClass::Find_3D_Device(DRIVER_TYPE_3D type) {
  ...
  // Loop through all the driver entries and free them all
  int driver_index = -1;
  for (int index = 0; (index < m_Driver3DList.Count()) && (driver_index == -1);
       index++) {
    DRIVER_INFO_STRUCT *info = m_Driver3DList[index];
...
bool WWAudioClass::Validate_3D_Sound_Buffer(SoundBufferClass *buffer) {
  ...
  //
  if ((buffer != NULL) && (buffer->Get_Channels() == 1) &&
...
void WWAudioClass::Initialize(const char *registry_subkey_name) {
  ...
  //
  //	Register the file callbacks so we can support streaming from MIX
  //files...
  //
...
void WWAudioClass::Fire_Text_Callback(AudibleSoundClass *sound_obj,
  ...
  if (text.Get_Length() > 0) {
    ...
    for (int index = 0; index < m_TextCallbackList.Count(); index++) {
      uint32 user_data = 0L;
      LPFNTEXTCALLBACK callback =
          m_TextCallbackList.Get_Callback(index, &user_data);
      if (callback != NULL) {
        //
...
//
////////////////////////////////////////////////////////////////////////////////////////////////
bool WWAudioClass::Simple_Play_2D_Sound_Effect(FileClass &file, float priority,
                                               float volume) {
  bool retval = false;
...
bool WWAudioClass::Load_From_Registry(const char *subkey_name) {
  ...
  if (Load_From_Registry(subkey_name, device_name, is_stereo, bits, hertz,
                         m_AreSoundEffectsEnabled, m_IsMusicEnabled,
                         m_SoundVolume, m_MusicVolume)) {
    //
...
////////////////////////////////////////////////////////////////////////////////////////////
bool WWAudioClass::Load_From_Registry(const char *subkey_name,
                                      StringClass &device_name, bool &is_stereo,
                                      int &bits, int &hertz,
                                      bool &sound_enabled, bool &music_enabled,
                                      float &sound_volume,
                                      float &music_volume) {
  bool retval = false;
  ...
  if (registry.Is_Valid()) {
    ...
    registry.Get_String(VALUE_NAME_DEVICE_NAME, temp_buffer,
                        sizeof(temp_buffer));
...
bool WWAudioClass::Save_To_Registry(const char *subkey_name) {
  ...
  //
  return Save_To_Registry(subkey_name, device_name, m_PlaybackStereo,
                          m_PlaybackBits, m_PlaybackRate,
                          m_AreSoundEffectsEnabled, m_IsMusicEnabled,
                          m_SoundVolume, m_MusicVolume);
}
...
bool WWAudioClass::Save_To_Registry(const char *subkey_name,
                                    ...
                                    bool is_stereo, int bits, int hertz,
                                    bool sound_enabled, bool music_enabled,
                                    float sound_volume, float music_volume
...
//
////////////////////////////////////////////////////////////////////////////////////////////
S32 AILCALLBACK WWAudioClass::File_Seek_Callback(U32 file_handle, S32 offset,
                                                 U32 type) {
  S32 retval = 0;
...
//
////////////////////////////////////////////////////////////////////////////////////////////
U32 AILCALLBACK WWAudioClass::File_Read_Callback(U32 file_handle, void *buffer,
                                                 U32 bytes) {
  U32 retval = 0;
