...
//
////////////////////////////////////////////////////////////////////////////////////////////////
void SoundSceneClass::Collect_Logical_Sounds(int listener_count) {
  ...
  PriorityMultiListIterator<LogicalListenerClass> priority_queue(
      &m_LogicalListeners);
  ...
  // frame.
  //
  for (int total = 0; total < count && priority_queue.Process_Head(&listener);
       total++) {
    ...
    for (cull_obj = m_LogicalCullingSystem.Get_First_Collected_Object();
         cull_obj != NULL;
         cull_obj =
             m_LogicalCullingSystem.Get_Next_Collected_Object(cull_obj)) {
      //
      // Get a pointer to the current 'cull-sound' object.
      //
      LogicalSoundClass *sound_obj =
          (LogicalSoundClass *)cull_obj->Peek_Sound_Obj();
      ...
      if ((listener_pos - sound_pos).Length2() <= test_radius2) {
        ...
        if (sound_obj->Allow_Notify(timestamp)) {
          listener->On_Event(AudioCallbackClass::EVENT_LOGICAL_HEARD,
                             (uint32)listener, (uint32)sound_obj);
        }
      }
    }
  }
  ...
  //
  MultiListIterator<LogicalSoundClass> single_shot_it(
      &m_SingleShotLogicalSounds);
  for (single_shot_it.First(); !single_shot_it.Is_Done();
    ...
    //
    if (sound_obj->Get_Listener_Timestamp() <=
        LogicalListenerClass::Get_Oldest_Timestamp()) {
...
void SoundSceneClass::Collect_Audible_Sounds(Listener3DClass *listener,
  ...
  //
  // Loop through all the dynamic sounds that are currently audible and make
  // sure they are 'really' audible.  The culling systems just check bounding
  // boxes but we need to be able to check attenuation spheres.
  //
  ...
  for (cull_obj = m_DynamicCullingSystem.Get_First_Collected_Object();
    ...
    if (length2 <= radius2) {
      AudibleInfoClass *audible_info =
          W3DNEW AudibleInfoClass(sound_obj, length2);
...
//	On_Frame_Update
//
//	Note:  This method could be made more efficient by using another data
//structure besides
// linked lists.  However the differece may be negligable due to the low density
// of 3D sounds that are audible at once.
//
////////////////////////////////////////////////////////////////////////////////////////////////
void SoundSceneClass::On_Frame_Update(unsigned int milliseconds) {
  ...
  for (aux_iterator.First(); !aux_iterator.Is_Done(); aux_iterator.Next()) {
    ...
    //
    bool found = false;
    for (pri_iterator.First(); !pri_iterator.Is_Done() && !found;
         pri_iterator.Next()) {
  ...
  for (audible_iterator.First(); !audible_iterator.Is_Done();
       audible_iterator.Next()) {
  ...
  for (newly_audible_it.First(); !newly_audible_it.Is_Done();
       newly_audible_it.Next()) {
...
//	Remove_Sound
//
//	Note:  This method should really be rewritten to use a different list
//type.  Linked lists
...
void SoundSceneClass::Add_Logical_Sound(LogicalSoundClass *sound_obj,
  ...
  if (sound_obj != NULL) {
    ...
    if (Is_Logical_Sound_In_Scene(sound_obj, single_shot) == false) {
      sound_obj->Set_Listener_Timestamp(
          LogicalListenerClass::Get_Newest_Timestamp());
...
//
////////////////////////////////////////////////////////////////////////////////////////////////
void SoundSceneClass::Remove_Logical_Listener(
    LogicalListenerClass *listener_obj) {
...
bool SoundSceneClass::Is_Sound_In_Scene(AudibleSoundClass *sound_obj,
  ...
  SoundCullObjClass *cull_obj = sound_obj->Peek_Cullable_Wrapper();
  if (cull_obj != NULL) {
    retval = (m_DynamicSounds.Is_In_List(cull_obj) ||
              m_StaticSounds.Is_In_List(cull_obj));
  }
...
bool SoundSceneClass::Is_Logical_Sound_In_Scene(LogicalSoundClass *sound_obj,
  ...
  //
  //	Check to see if this sound is in either the continuous list or the
  //single shot list.
  //
...
void SoundSceneClass::Save_Static_Sounds(ChunkSaveClass &csave) {
  ...
  for (static_iterator.First(); !static_iterator.Is_Done();
       static_iterator.Next()) {
...
//
////////////////////////////////////////////////////////////////////////////////////////////////
bool SoundSceneClass::Save_Dynamic(ChunkSaveClass &csave) { return true; }
...
//
////////////////////////////////////////////////////////////////////////////////////////////////
bool SoundSceneClass::Load_Dynamic(ChunkLoadClass &cload) { return true; }
...
void SoundSceneClass::Flush_Scene(void) {
  ...
  for (dynamic_iterator.First(); !dynamic_iterator.Is_Done();
       dynamic_iterator.Next()) {
  ...
  for (temp_static_it.First(); !temp_static_it.Is_Done();
       temp_static_it.Next()) {
  ...
  for (temp_dynamic_it.First(); !temp_dynamic_it.Is_Done();
       temp_dynamic_it.Next()) {
