...
bool Sound3DClass::On_Frame_Update(unsigned int milliseconds) {
  ...
  if (m_bAutoCalcVel && Get_Class_ID() != CLASSID_LISTENER) {
    ...
    //
    //	Don't update the velocity if we haven't moved (optimization -- Miles
    //calls
    // can be really slow)
    //
    if (last_pos != curr_pos) {
      ...
      //
      //	Extrapolate our current velocity given the last time slice and
      //the distance
      // we moved.
      //
...
void Sound3DClass::Update_Miles_Transform(void) {
  ...
  if (m_SoundHandle != NULL) {
    //
    //	Build a matrix to transform coordinates from world-space to
    //listener-space
    //
    ...
    ::AIL_set_3D_position(m_SoundHandle->Get_H3DSAMPLE(), -position.Y,
    ...
    ::AIL_set_3D_orientation(m_SoundHandle->Get_H3DSAMPLE(), -facing.Y,
                             facing.Z, facing.X, -up.Y, up.Z, up.X);
  }
...
void Sound3DClass::Set_Position(const Vector3 &position) {
  ...
  if (m_Transform.Get_Translation() != position) {
    ...
    if (m_SoundHandle != NULL) {
      ...
      Matrix3D::Inverse_Transform_Vector(m_ListenerTransform, sound_pos,
                                         &listener_space_pos);

      //
      //	Update the object's position inside of Miles
      //
      ::AIL_set_3D_position(m_SoundHandle->Get_H3DSAMPLE(),
                            -listener_space_pos.Y, listener_space_pos.Z,
...
void Sound3DClass::Set_Velocity(const Vector3 &velocity) {
  ...
  if (m_SoundHandle != NULL) {
    ...
    // m_CurrentVelocity.Y, m_CurrentVelocity.Z));
...
void Sound3DClass::Set_DropOff_Radius(float radius) {
  ...
  if (m_SoundHandle != NULL) {
    ::AIL_set_3D_sample_distances(
        m_SoundHandle->Get_H3DSAMPLE(), m_DropOffRadius,
...
void Sound3DClass::Allocate_Miles_Handle(void) {
  ...
  if (m_SoundHandle == NULL) {
    Set_Miles_Handle(
        (MILES_HANDLE)WWAudioClass::Get_Instance()->Get_3D_Sample(*this));
  }
...
//
/////////////////////////////////////////////////////////////////////////////////
const PersistFactoryClass &Sound3DClass::Get_Factory(void) const {
