...
//	Static factories
//////////////////////////////////////////////////////////////////////////////////
DECLARE_DEFINITION_FACTORY(AudibleSoundDefinitionClass, CLASSID_SOUND_DEF,
                           "Sound")
_SoundDefFactory;
SimplePersistFactoryClass<AudibleSoundDefinitionClass, CHUNKID_SOUND_DEF>
    _AudibleSoundDefPersistFactory;
SimplePersistFactoryClass<AudibleSoundClass, CHUNKID_AUDIBLE_SOUND>
    _AudibleSoundPersistFactory;
...
//
////////////////////////////////////////////////////////////////////////////////////////////////
const AudibleSoundClass &AudibleSoundClass::operator=(
...
void AudibleSoundClass::Set_Volume(float volume) {
  ...
  // Do we have a valid sample handle from miles?
  if (m_SoundHandle != NULL) {
    // Calculate the 'real' volume to set based on the global volume and the
    // sound effect volume.
    float real_volume = Determine_Real_Volume();
    m_SoundHandle->Set_Sample_Volume(int(real_volume * 127.0F));
  }
...
//
////////////////////////////////////////////////////////////////////////////////////////////////
int AudibleSoundClass::Get_Loops_Left(void) const { return m_LoopsLeft; }
...
bool AudibleSoundClass::On_Frame_Update(unsigned int milliseconds) {
  ...
  //
  if ((m_LoopCount != INFINITE_LOOPS) && (m_State == STATE_PLAYING) &&
      (m_Length > 0)) {
    Update_Play_Position();
  }
...
AudibleSoundDefinitionClass::AudibleSoundDefinitionClass(void)
  ...
  //
  NAMED_EDITABLE_PARAM(AudibleSoundDefinitionClass,
                       ParameterClass::TYPE_SOUND_FILENAME, m_Filename,
                       "Filename");
  NAMED_EDITABLE_PARAM(AudibleSoundDefinitionClass, ParameterClass::TYPE_FLOAT,
                       m_DropOffRadius, "Drop-off Radius");
  NAMED_EDITABLE_PARAM(AudibleSoundDefinitionClass, ParameterClass::TYPE_FLOAT,
                       m_MaxVolRadius, "Max-Vol Radius");
  NAMED_EDITABLE_PARAM(AudibleSoundDefinitionClass, ParameterClass::TYPE_BOOL,
                       m_Is3D, "Is 3D Sound");
  ...
  ENUM_PARAM(AudibleSoundDefinitionClass, m_Type,
             ("Sound Effect", AudibleSoundClass::TYPE_SOUND_EFFECT, "Music",
              AudibleSoundClass::TYPE_MUSIC, 0));
  ...
  NAMED_EDITABLE_PARAM(AudibleSoundDefinitionClass, ParameterClass::TYPE_STRING,
                       m_DisplayText, "Display Text");

  //
  //	Logical sound params
  //
  NAMED_EDITABLE_PARAM(AudibleSoundDefinitionClass, ParameterClass::TYPE_BOOL,
                       m_CreateLogical, "Create Logical Sound");
  NAMED_EDITABLE_PARAM(AudibleSoundDefinitionClass, ParameterClass::TYPE_FLOAT,
                       m_LogicalDropOffRadius, "Logical Drop-off Radius");
  NAMED_EDITABLE_PARAM(AudibleSoundDefinitionClass, ParameterClass::TYPE_FLOAT,
                       m_LogicalNotifyDelay, "Logical Notif Delay");
  ...
  for (int index = 0; index < count; index++) {
    ...
    int id =
        WWAudioClass::Get_Instance()->Get_Logical_Type(index, display_name);
    param->Add_Value(display_name, id);
  }
  GENERIC_EDITABLE_PARAM(AudibleSoundDefinitionClass, param);
#endif

  NAMED_EDITABLE_PARAM(AudibleSoundDefinitionClass, ParameterClass::TYPE_COLOR,
                       m_AttenuationSphereColor, "Sphere Color");
...
//
////////////////////////////////////////////////////////////////////////////////////////////////
void AudibleSoundDefinitionClass::Initialize_From_Sound(
...
//
////////////////////////////////////////////////////////////////////////////////////////////////
const PersistFactoryClass &AudibleSoundDefinitionClass::Get_Factory(
    void) const {
...
//
//////////////////////////////////////////////////////////////////////////////////
AudibleSoundClass *AudibleSoundDefinitionClass::Create_Sound(
    int classid_hint) const {
  ...
  if (dir_delimiter != NULL && m_Filename.Get_Length() > 2 &&
      m_Filename[1] != ':') {
  ...
  //
  if (m_Is3D) {
    new_sound = WWAudioClass::Get_Instance()->Create_3D_Sound(real_filename,
                                                              classid_hint);
  } else {
    new_sound =
        WWAudioClass::Get_Instance()->Create_Sound_Effect(real_filename);
  }
...
bool AudibleSoundClass::Load(ChunkLoadClass &cload) {
      ...
      case CHUNKID_VARIABLES: {
        ...
        while (cload.Open_Micro_Chunk()) {
          switch (cload.Cur_Micro_Chunk_ID()) {
            ...
            READ_MICRO_CHUNK(cload, VARID_START_OFFSET, m_StartOffset);
            READ_MICRO_CHUNK(cload, VARID_PITCH_FACTOR, m_PitchFactor);
            READ_MICRO_CHUNK(cload, VARID_LISTENER_TRANSFORM,
                             m_ListenerTransform);
  ...
  if (filename.Get_Length() > 0) {
    ...
    SoundBufferClass *buffer =
        WWAudioClass::Get_Instance()->Get_Sound_Buffer(filename, is_3d);
    Set_Buffer(buffer);
