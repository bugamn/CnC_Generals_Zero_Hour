...
void StreakLineClass::Set_Point_Location(unsigned int point_idx,
...
float StreakLineClass::Get_Opacity(void) { return LineRenderer.Get_Opacity(); }
...
float StreakLineClass::Get_Width(void) { return LineRenderer.Get_Width(); }
...
void StreakLineClass::Set_Texture_Mapping_Mode(
...
float StreakLineClass::Get_Cost(void) const { return Get_Num_Polys(); }
...
void StreakLineClass::Render_Streak_Line(RenderInfoClass &rinfo) {
  ...
  StreakRenderer.RenderStreak(
      rinfo, Transform, PointLocations.Count(), &(PointLocations[0]),
      &(PointColors[0]), &(PointWidths[0]), bounding_sphere, Personalities);
}
