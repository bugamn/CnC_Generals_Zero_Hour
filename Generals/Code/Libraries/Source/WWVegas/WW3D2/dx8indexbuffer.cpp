...
IndexBufferClass::IndexBufferClass(unsigned type_, unsigned short index_count_)
    : index_count(index_count_), type(type_), engine_refs(0) {
  ...
  WWDEBUG_SAY(("Total IB count: %d, total %d indices, total size %d bytes\n",
               _IndexBufferCount, _IndexBufferTotalIndices,
...
IndexBufferClass::WriteLockClass::WriteLockClass(
    IndexBufferClass* index_buffer_)
  ...
  switch (index_buffer->Type()) {
    case BUFFER_TYPE_DX8:
      DX8_Assert();
      DX8_ErrorCode(static_cast<DX8IndexBufferClass*>(index_buffer)
    ...
    case BUFFER_TYPE_SORTING:
      indices =
          static_cast<SortingIndexBufferClass*>(index_buffer)->index_buffer;
      break;
    default:
      WWASSERT(0);
      break;
...
IndexBufferClass::AppendLockClass::AppendLockClass(
    IndexBufferClass* index_buffer_, unsigned start_index, unsigned index_range)
    : index_buffer(index_buffer_) {
  DX8_THREAD_ASSERT();
...
DX8IndexBufferClass::DX8IndexBufferClass(unsigned short index_count_,
  ...
  DX8CALL(CreateIndexBuffer(
      sizeof(WORD) * index_count, usage_flags, D3DFMT_INDEX16,
...
DX8IndexBufferClass::~DX8IndexBufferClass() { index_buffer->Release(); }
...
DynamicIBAccessClass::DynamicIBAccessClass(unsigned short type_,
    ...
    : IndexCount(index_count_), IndexBuffer(0), Type(type_) {
...
DynamicIBAccessClass::WriteLockClass::WriteLockClass(
  ...
  switch (DynamicIBAccess->Get_Type()) {
    case BUFFER_TYPE_DYNAMIC_DX8:
      ...
      DX8_ErrorCode(
          static_cast<DX8IndexBufferClass*>(DynamicIBAccess->IndexBuffer)
              ->Get_DX8_Index_Buffer()
              ->Lock(DynamicIBAccess->IndexBufferOffset * sizeof(WORD),
    ...
    case BUFFER_TYPE_DYNAMIC_SORTING:
      Indices =
          static_cast<SortingIndexBufferClass*>(DynamicIBAccess->IndexBuffer)
              ->index_buffer;
...
void DynamicIBAccessClass::Allocate_DX8_Dynamic_Buffer() {
  ...
  if (IndexCount > _DynamicDX8IndexBufferSize) {
    ...
    if (_DynamicDX8IndexBufferSize < DEFAULT_IB_SIZE)
      _DynamicDX8IndexBufferSize = DEFAULT_IB_SIZE;
  }
  ...
  if (!_DynamicDX8IndexBuffer) {
    ...
    _DynamicDX8IndexBuffer = NEW_REF(
        DX8IndexBufferClass,
        (_DynamicDX8IndexBufferSize, (DX8IndexBufferClass::UsageType)usage));
    _DynamicDX8IndexBufferOffset = 0;
  }

  // Any room at the end of the buffer?
  if (((unsigned)IndexCount + _DynamicDX8IndexBufferOffset) >
