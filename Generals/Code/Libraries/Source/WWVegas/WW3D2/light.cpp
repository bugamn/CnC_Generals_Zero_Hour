 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * LightClass::LightClass -- Constructor * LightClass::LightClass
 *-- copy constructor                                                *
 *   LightClass::operator == -- assignment operator * LightClass::~LightClass --
 *destructor                                                     *
 *   LightClass::Clone -- virtual copy constructor *
 *   LightClass::Get_Attenuation_Range -- returns a dist beyond which the light
 *is attenuated  * LightClass::Get_Obj_Space_Bounding_Sphere -- returns the
 *object space bounding sphere     * LightClass::Get_Obj_Space_Bounding_Box --
 ...
 *   LightClass::Set_Transform -- sets transform and marks srLight transform as
 *dirty.         * LightClass::Set_Position -- sets position and marks srLight
 *transform as dirty.           * LightClass::Notify_Added -- lights add
 *themselves to the VP list when added               *
 *   LightClass::Notify_Removed -- lights remove themselves from the VP list
 *when removed      * LightClass::Load_W3D -- Initialize this light from a W3D
 *file                             * LightClass::Save_W3D -- Save this light's
 *settings into a W3D file                        * LightClass::Get_Factory --
 *get the PersistFactory for LightClass                          *
 *   LightClass::Save -- persistant object support * LightClass::Load --
 *persistant object support                                             *
 *   LightImpClass::LightImpClass -- constructor * LightImpClass::Process_Push
 *-- exposes the "push" process for an srLight                  *
...
/*
** Chunk ID's used by LightClass's save-load support (different from the W3D
*file format...)
*/
enum {
 ...
 *                                                                                             *
 * INPUT: * type - type of light to create. *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
...
/***********************************************************************************************
 * LightClass::Notify_Removed -- lights remove themselves from the VP list when
 *removed        *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * LightClass::Get_Obj_Space_Bounding_Sphere -- returns the object space
 *bounding sphere       *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * LightClass::Get_Obj_Space_Bounding_Box -- returns the object space bounding
 *box             *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
WW3DErrorType LightClass::Load_W3D(ChunkLoadClass &cload) {
  ...
  Enable_Shadows((lightinfo.Attributes & W3D_LIGHT_ATTRIBUTE_CAST_SHADOWS) ==
  ...
  while (cload.Open_Chunk()) {
    switch (cload.Cur_Chunk_ID()) {
      case LIGHT_CHUNK_W3DFILE:
        // The W3D code is non-symmetrical, the Save function writes a W3D_LIGHT
        // chunk but the load function expects the external user to have opened
        // it.  So open it here...
        cload.Open_Chunk();
        Load_W3D(cload);
        cload.Close_Chunk();
