 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * MMMC::MetalMapManagerClass -- Create metal map manager according
 *to given metal parameters* MMMC::MetalMapManagerClass -- Create metal map
 *manager from INI                           * MMMC::~MetalMapManagerClass --
 *MetalMapManagerClass destructor                            *
 ...
 *   MMMC::Update_Lighting -- Update the lighting parameters used for generating
 *the maps      * MMMC::Update_Textures -- Update  metal map textures (call
 *once/frame before rendering)    * MMMC::initialize_normal_table -- Utility
 *function to initialize the normal table          *
...
MetalMapManagerClass::MetalMapManagerClass(INIClass &ini)
  ...
  for (int lp = 0;; lp++) {
    ...
    if (!ini.Find_Section(section)) {
      break;  // NAK - Mar 8, 2000: changed to a break to fix off by one error
              // in lp
    }
  }
  ...
  if (lp > 0) {
    ...
    for (int i = 0; i < lp; i++) {
      ...
      metal_params[i].AmbientColor.Set(color.X * cf, color.Y * cf,
                                       color.Z * cf);
      ...
      metal_params[i].DiffuseColor.Set(color.X * cf, color.Y * cf,
                                       color.Z * cf);
      metal_params[i].AmbientColor.Update_Min(white);
      metal_params[i].AmbientColor.Update_Max(black);
      color = ini.Get_Point(section, "SpecularColor", white_tpoint);
      metal_params[i].SpecularColor.Set(color.X * cf, color.Y * cf,
                                        color.Z * cf);
      metal_params[i].AmbientColor.Update_Min(white);
      metal_params[i].AmbientColor.Update_Max(black);
  ...
  for (int i = 0; i < lp; i++) {
    // Create texture. NOTE: we should add code here to ensure we use a native
    // texture format
    Textures[i] = NEW_REF(TextureClass,
                          (METALMAP_SIZE, METALMAP_SIZE, WW3D_FORMAT_A8R8G8B8,
 ...
 * HISTORY: * 11/19/1999 NH : Created. *
 *=============================================================================================*/
void MetalMapManagerClass::Update_Lighting(const Vector3 &ambient,
                                           const Vector3 &main_light_color,
                                           const Vector3 &main_light_dir,
...
/***********************************************************************************************
 * MMMC::Update_Textures -- Update  metal map textures (call once/frame before
 *rendering)      *
 *                                                                                             *
 *                                                                                             *
 * INPUT: *
...
void MetalMapManagerClass::Update_Textures(void) {
  ...
  // NOTE: when our lighting equation gets more complicated we might want to do
  // some testing to detect zero components...
  ...
  for (int i = 0; i < MapCount; i++) {
    ...
    if (shinyness > 1.0f) {
      VectorProcessorClass::Power(temp_specular, n_dot_h, shinyness,
                                  METALMAP_SIZE_2);
    ...
    Vector3 specular_color(
        cur_params.SpecularColor.X * CurrentMainLightColor.X,
    ...
    for (int y = 0; y < METALMAP_SIZE; y++) {
      for (int x = 0; x < METALMAP_SIZE; x++) {
        Vector3 result = ambient_color + (diffuse_color * n_dot_l[idx]) +
                         (specular_color * specular[idx]);
...
/***********************************************************************************************
 * MMMC::initialize_normal_table -- Utility function to initialize the normal
 *table            *
 *                                                                                             *
 *                                                                                             *
 * INPUT: *
...
void MetalMapManagerClass::initialize_normal_table(void) {
  // NOTE: changing the actual static _NormalTable member must be the last thing
  // this function does to avoid synchronization errors.

  static Vector3 _normal_table[METALMAP_SIZE_2];

  // Calculate vectors (area outside sphere should be filled with a radial fill
  // of the vectors at the sphere's edge to avoid aliasing artifacts)
  ...
  for (int y = 0; y < METALMAP_SIZE; y++) {
    for (int x = 0; x < METALMAP_SIZE; x++) {
      ...
      normal.Z = sqrt(z2);
      normal.Normalize();  // Needed for "outside sphere" case and for safety's
                           // sake
 ...
 * HISTORY: * 11/23/1999 NH : Created. *
 *=============================================================================================*/
void MetalMapManagerClass::initialize_metal_params(int map_count,
  ...
  if (MapCount > 0) {
    ...
    for (int i = 0; i < MapCount; i++) {
      ...
      assert(MetalParameters[i].AmbientColor.X >= 0.0f &&
             MetalParameters[i].AmbientColor.X <= 1.0f);
      assert(MetalParameters[i].AmbientColor.Y >= 0.0f &&
             MetalParameters[i].AmbientColor.Y <= 1.0f);
      assert(MetalParameters[i].AmbientColor.Z >= 0.0f &&
             MetalParameters[i].AmbientColor.Z <= 1.0f);
      assert(MetalParameters[i].DiffuseColor.X >= 0.0f &&
             MetalParameters[i].DiffuseColor.X <= 1.0f);
      assert(MetalParameters[i].DiffuseColor.Y >= 0.0f &&
             MetalParameters[i].DiffuseColor.Y <= 1.0f);
      assert(MetalParameters[i].DiffuseColor.Z >= 0.0f &&
             MetalParameters[i].DiffuseColor.Z <= 1.0f);
      assert(MetalParameters[i].SpecularColor.X >= 0.0f &&
             MetalParameters[i].SpecularColor.X <= 1.0f);
      assert(MetalParameters[i].SpecularColor.Y >= 0.0f &&
             MetalParameters[i].SpecularColor.Y <= 1.0f);
      assert(MetalParameters[i].SpecularColor.Z >= 0.0f &&
             MetalParameters[i].SpecularColor.Z <= 1.0f);
