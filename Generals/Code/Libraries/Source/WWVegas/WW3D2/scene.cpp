 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * SceneClass::SceneClass -- constructor * SceneClass::~SceneClass
 *-- destructor                                                     *
 *   SceneClass::Render -- preps the scene for rendering, derived classes should
 *override      * SceneClass::Add_Render_Object -- base add function *
 *   SceneClass::Remove_Render_Object -- base remove function *
 *   SceneClass::Set_Depth_Cue -- set the depth cue values *
 *   SceneClass::Set_Fog_Range -- set the fog range * SceneClass::Get_Fog_Range
 *-- get the fog range                                            *
 *   SceneClass::Render -- preps the scene for rendering, derived classes should
 *add functional* SimpleSceneClass -- Constructor *
 *   SimpleSceneClass::~SimpleSceneClass -- destructor *
 *   SimpleSceneClass::Add_Render_Object -- add a render object to the scene *
 *   SimpleSceneClass::Remove_Render_Object -- remove a render object from this
 *scene          * SimpleSceneClass::Visiblity_Check -- set the visiblity status
 *of the render objects       * SimpleSceneClass::Render -- internal scene
 *rendering function                             * SimpleSceneClass::Render --
 *Render this scene                                             *
 ...
 *   SceneClass::Save -- saves scene settings into a chunk * SceneClass::Load --
 *loads scene settings from a chunk                                     *
 *   SimpleSceneClass::Compute_Point_Visibility -- returns visibility of a point
 ** SimpleSceneClass::Remove_All_Render_Objects -- Removes all render objects
 *from the scene  *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 *                                                                                             *
 * This function only sets the render object's scene pointer and calls the
 *notify method.      * Derived classes must add the functionality of actually
 *storing a pointer to the object      * and doing something with it :) *
 *                                                                                             *
 * INPUT: * obj - pointer to the object to add *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
 ...
 * SceneClass::Remove_Render_Object -- base remove function *
 *                                                                                             *
 * Similar to the add function; concrete derived classes must override and
 *actually remove     * the object... *
 *                                                                                             *
 * INPUT: * obj - pointer to the object to remove *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
  ...
  if (Get_Extra_Pass_Polygon_Mode() == EXTRA_PASS_DISABLE) {
    ...
    switch (Get_Extra_Pass_Polygon_Mode()) {
      ...
      case EXTRA_PASS_CLEAR_LINE:
        DX8Wrapper::Clear(true, false,
                          Vector3(0.0f, 0.0f, 0.0f));  // Clear color but not z
        WW3D::Enable_Texturing(false);
        DX8Wrapper::Set_DX8_Render_State(D3DRS_FILLMODE, D3DFILL_WIREFRAME);
        DX8Wrapper::Set_DX8_Render_State(D3DRS_ZBIAS, 7);
        Customized_Render(rinfo);
  ...
  if (cload.Cur_Chunk_ID() == SCENECLASS_CHUNK_VARIABLES) {
    while (cload.Open_Micro_Chunk()) {
      switch (cload.Cur_Micro_Chunk_ID()) {
        ...
        READ_MICRO_CHUNK(cload, SCENECLASS_VARIABLE_POLYRENDERMODE,
                         PolyRenderMode);
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 3/24/98    GTH : Created. * 9/10/99    GTH : Created. *
 *=============================================================================================*/
 ...
 *=============================================================================================*/
SimpleSceneClass::~SimpleSceneClass(void) { Remove_All_Render_Objects(); }
...
/***********************************************************************************************
 * SimpleSceneClass::Visiblity_Check -- set the visiblity status of the render
 *objects         *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 3/24/98    GTH : Created. * 4/13/98    NH : Added non-trivial
 *checking (sphere vs. frustum).                          *
 *=============================================================================================*/
void SimpleSceneClass::Visibility_Check(CameraClass* camera) {
  RefRenderObjListIterator it(&RenderList);

  // Loop over all top-level RenderObjects in this scene. If the bounding sphere
  // is not in front of all the frustum planes, it is invisible.
 ...
 * SimpleSceneClass::Compute_Point_Visibility -- returns visibility of a point *
 *                                                                                             *
 *    This function is used by the default dazzle behavior.  SimpleSceneClass's
 *implementation * casts a ray against every object in the scene to see if the
 *dazzle is occluded.          *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 6/13/2001  gth : Created. *
 *=============================================================================================*/
float SimpleSceneClass::Compute_Point_Visibility(RenderInfoClass& rinfo,
                                                 const Vector3& point) {
...
void SimpleSceneClass::Post_Render_Processing(RenderInfoClass& rinfo) {
  // process the 'Release' list.  These are objects that have notified us that
  // they want to be released.  We have to walk this list twice, first
  // un-linking the object from the scene or its container.  And then removing
  // them all from the list.  (this last removal will destroy any auto-created
  // objects)
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 3/27/98    GTH : Created. *
 *=============================================================================================*/
SceneIterator* SimpleSceneClass::Create_Iterator(bool onlyvisible) {
  SimpleSceneIterator* it =
      W3DNEW SimpleSceneIterator(&RenderList, onlyvisible);
  return it;
}
 ...
 * HISTORY: * 3/27/98    GTH : Created. *
 *=============================================================================================*/
void SimpleSceneClass::Destroy_Iterator(SceneIterator* it) { delete it; }

SimpleSceneIterator::SimpleSceneIterator(RefRenderObjListClass* list,
                                         bool onlyvis)
    : RobjIterator(list) {
  // TODO: make SimpleSceneIterator able to iterate through only the visible
  // nodes.
  OnlyVis = onlyvis;
}

void SimpleSceneIterator::First(void) { RobjIterator.First(); }

void SimpleSceneIterator::Next(void) { RobjIterator.Next(); }

bool SimpleSceneIterator::Is_Done(void) { return RobjIterator.Is_Done(); }
