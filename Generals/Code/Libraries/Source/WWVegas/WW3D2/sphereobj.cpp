 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * SphereRenderObjClass::SphereRenderObjClass -- Constructor *
 *   SphereRenderObjClass::SphereRenderObjClass -- Constructor - init from a
 *definition        * SphereRenderObjClass::SphereRenderObjClass -- Copy
 *constructor                            * SphereRenderObjClass::operator --
 *assignment operator                                     *
 ...
 *   SphereRenderObjClass::vis_render_Sphere -- submits Sphere to the GERD for
 *VIS             * SphereRenderObjClass::SphereRenderObjClass -- constructor *
 *   SphereRenderObjClass::SphereRenderObjClass -- Constructor - init from a
 *definition        * SphereRenderObjClass::SphereRenderObjClass -- copy
 *constructor                            *
 *   SphereRenderObjClass::SphereRenderObjClass -- Constructor from a wwmath
 *aaSphere          * SphereRenderObjClass::operator -- assignment operator *
 ...
 *   SphereRenderObjClass::update_cached_Sphere -- update the world-space
 *version of this Spher* SphereRenderObjClass::Cast_Ray -- cast a ray against
 *this Sphere                          * SphereRenderObjClass::Cast_AASphere --
 *cast an AASphere against this Sphere               *
 *   SphereRenderObjClass::Cast_OBSphere -- cast an OBSphere against this Sphere
 ** SphereRenderObjClass::Intersect_AASphere -- intersect this Sphere with an
 *AASphere        * SphereRenderObjClass::Intersect_OBSphere -- Intersect this
 *Sphere with an OBSphere        *
 *   SphereRenderObjClass::Get_Obj_Space_Bounding_Sphere -- return the
 *object-space bounding sp* SphereRenderObjClass::Get_Obj_Space_Bounding_Sphere
 *-- returns the obj-space bounding Sphe*
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
SphereRenderObjClass::SphereRenderObjClass(void)
  ...
  //	(gth) testing whether we can get away without poly-sorting rings and
  //spheres
...
SphereRenderObjClass::SphereRenderObjClass(const W3dSphereStruct &def)
  ...
  if (def.TextureName[0] != 0) {
    SphereTexture =
        WW3DAssetManager::Get_Instance()->Get_Texture(def.TextureName);
  }
 ...
 * HISTORY: * 1/19/00    gth : Created. *
 *=============================================================================================*/
SphereRenderObjClass &SphereRenderObjClass::operator=(
    const SphereRenderObjClass &that) {
  if (this != &that) {
...
/***********************************************************************************************
 * SphereRenderObjClass::Generate_Shared_Mesh_Arrays  -- Generates mesh LOD
 *arrays.				  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 03/08/00    pds : Created. *
 *=============================================================================================*/
void SphereRenderObjClass::Generate_Shared_Mesh_Arrays(
    const AlphaVectorStruct &alphavector) {
...
/***********************************************************************************************
 * SphereRenderObjClass::Init_Material -- Sets up the material and default
 ...
 * HISTORY: * 1/19/00    gth : Created. *
 *=============================================================================================*/
const char *SphereRenderObjClass::Get_Name(void) const { return Name; }
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 3/01/00    jga : Created. * 2/19/01    HY  : upgraded to DX8 *
 *=============================================================================================*/
void SphereRenderObjClass::render_sphere() {
  ...
  DynamicVBAccessClass vb(BUFFER_TYPE_DYNAMIC_SORTING, dynamic_fvf_type,
                          mesh.Vertex_ct);
  {
    ...
    for (int i = 0; i < mesh.Vertex_ct; i++) {
      ...
      if (Flags & USE_ALPHA_VECTOR) {
        ...
        vb->diffuse = 0xFFFFFFFF;  // TODO could combine the material color with
                                   // this and turn off lighting
      }
...
#else
  SortingRendererClass::Insert_Triangles(Get_Bounding_Sphere(), 0, mesh.face_ct,
                                         0, mesh.Vertex_ct);
#endif
...
void SphereRenderObjClass::Render(RenderInfoClass &rinfo) {
  ...
  if (WW3D::Are_Static_Sort_Lists_Enabled() && sort_level != SORT_LEVEL_NONE) {
    ...
    // This code assumes that screen_size returns back the percentage of the
    // screen, 0.0 = 0%, 1.0f = 100%

    // Currently it will use Linear interpolation over 0.0 to 1.0, with the
    // SPHERE_NUM_LOD to set the CurrentLOD
    ...
    //
    bool is_additive =
        (SphereShader.Get_Dst_Blend_Func() == ShaderClass::DSTBLEND_ONE);
    if (is_additive) {
    ...
    if (Flags & USE_ALPHA_VECTOR) {
      ...
      SphereMeshArray[CurrentLOD].Set_Alpha_Vector(Vector, use_inverse,
                                                   is_additive);
    }
    ...
    if (Flags & USE_CAMERA_ALIGN) {
      ...
      Matrix3D tm(0.0f, 1.0f, 0.0f, cpos.X, 0.0f, 0.0f, 1.0f, cpos.Y, 1.0f,
                  0.0f, 0.0f, cpos.Z);
...
/***********************************************************************************************
 * SphereRenderObjClass::update_cached_box -- update the world-space version of
 *this box        *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 1/19/00    gth : Created. *
 *=============================================================================================*/
void SphereRenderObjClass::Get_Obj_Space_Bounding_Sphere(
    SphereClass &sphere) const {
  sphere.Init(ObjSpaceCenter, ObjSpaceExtent.Length());
}

/***********************************************************************************************
 * SphereRenderObjClass::Get_Obj_Space_Bounding_Box -- returns the obj-space
 *bounding box       *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * SphereRenderObjClass::Update_Cached_Bounding_Volumes -- Updates world-space
 *bounding volumes *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void SphereRenderObjClass::Update_Cached_Bounding_Volumes(void) const {
  ...
  CachedBoundingSphere.Center = CachedBoundingBox.Center =
      Get_Position() + ObjSpaceCenter;
...
/***********************************************************************************************
 * SphereRenderObjClass::Get_Default_Color - get the default (or first frame)
 *value				  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * SphereRenderObjClass::Get_Default_Alpha - get the default (or first frame)
 *value				  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * SphereRenderObjClass::Get_Default_Scale - get the default (or first frame)
 *value				  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * SphereRenderObjClass::Get_Default_Vector - get the default (or first frame)
 *value			  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * SphereRenderObjClass::Update_On_Visibilty	-- Either starts or stops the
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 3/07/00    jga : Created. *
 *=============================================================================================*/
void SphereRenderObjClass::animate(void) {
  if (Is_Animating()) {
    if (ColorChannel.Get_Key_Count() > 0 || AlphaChannel.Get_Key_Count() > 0 ||
        ScaleChannel.Get_Key_Count() > 0 || VectorChannel.Get_Key_Count() > 0) {
      //
...
SpherePrototypeClass::~SpherePrototypeClass(void) { return; }
...
SphereMeshClass::SphereMeshClass(void)
    : Radius(0.0f),
      ...
      face_ct(0),
      tri_poly(NULL),
      inverse_alpha(false) {}  // Empty SphereMesh Constructor

/***********************************************************************************************
 * SphereMeshClass::Set_Alpha_Vector -- Unit Direction Vector, for Alpha hole
 *effects			  *
 *                                                                                             *
 ...
 * HISTORY: * 3/07/00    jga : Created. *
 *=============================================================================================*/
void SphereMeshClass::Set_Alpha_Vector(const AlphaVectorStruct &v, bool inverse,
                                       bool is_additive, bool force) {
  //
  //	Exit if there is nothing to do...
  //
  if (alpha_vector == v && inverse == inverse_alpha &&
      is_additive == IsAdditive && !force) {
    return;
  }
...
/***********************************************************************************************
 * SphereMeshClass::Generate -- Alloc Memory, and Generate Geometry, for the
 *SphereMesh		  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void SphereMeshClass::Generate(float radius, int slices, int stacks) {
  ...
  *veclist = vec;  // assign north pole
  ...
  uv->U = 0.5f;
  uv->V = 0.0f;  // assign uv for north pole
  uv++;
 ...
 * HISTORY: * 3/07/00    jga : Created. *
 *=============================================================================================*/
SphereMeshClass::~SphereMeshClass(void) { Free(); }  // Destructor
