...
Vector2 *PointGroupClass::_TriVertexUVFrameTable[5] = {NULL, NULL, NULL, NULL,
                                                       NULL};
Vector2 *PointGroupClass::_QuadVertexUVFrameTable[5] = {NULL, NULL, NULL, NULL,
                                                        NULL};
VertexMaterialClass *PointGroupClass::PointMaterial = NULL;

// Static arrays for intermediate calcs (never resized down, just up):
VectorClass<Vector3>
    PointGroupClass::compressed_loc;  // point locations 'compressed' by APT
VectorClass<Vector4>
    PointGroupClass::compressed_diffuse;  // point colors 'compressed' by APT
VectorClass<float>
    PointGroupClass::compressed_size;  // point sizes 'compressed' by APT
VectorClass<unsigned char>
    PointGroupClass::compressed_orient;  // point orientations 'compressed' by
                                         // APT
VectorClass<unsigned char>
    PointGroupClass::compressed_frame;  // point frames 'compressed' by APT
VectorClass<Vector3>
    PointGroupClass::transformed_loc;  // transformed point locations

// This array has vertex locations for screenspace mode - calculated to cover
// exactly 1x1 and 2x2 pixels.
Vector3 PointGroupClass::_ScreenspaceVertexLocationSizeTable[2][3] = {
    Vector3(0.5f, 0.0f, -1.0f), Vector3(1.0f, 1.0f, -1.0f),
    Vector3(0.0f, 1.0f, -1.0f), Vector3(1.0f, -0.5f, -1.0f),
    Vector3(2.7f, 2.0f, -1.0f), Vector3(-0.7f, 2.0f, -1.0f)};
...
void PointGroupClass::Set_Arrays(ShareBufferClass<Vector3> *locs,
                                 ...
                                 unsigned int active_point_count, float vpxmin,
                                 float vpymin, float vpxmax, float vpymax) {
  ...
  if (APT) {
    ...
    PointCount =
        (active_point_count >= 0) ? active_point_count : PointLoc->Get_Count();
  }
 ...
 * 				the shader depending on whether a color or alpha
 *array was * passed in Set_Point_Arrays. also, texturing will be        *
 * 				enabled or disabled dependent on whether a
 *non-NULL        * texture was set.                                           *
 *					these will override the primary
 *gradient/texturing         * settings in the given shader. *
 *                                                                        *
 * HISTORY:                                                               *
 *   11/17/1998 NH  : Created.                                            *
 *   02/08/2001 HY  : Upgraded to DX8                                     *
 ...
 *   04/25/2002 JM  : Created.                                            *
 *========================================================================*/
bool PointGroupClass::Get_Billboard(void) { return Billboard; }
...
void PointGroupClass::Render(RenderInfoClass &rinfo) {
  /// @todo lorenzen asks: is particle culling in the shader perhaps faster than
  /// in DoParticles? Fix winding and find out...
  ...
  // If there is a color or alpha array enable gradient in shader - otherwise
  // disable.
  float value_255 = 0.9961f;  // 254 / 255
  bool default_white_opaque =
      (DefaultPointColor.X > value_255 && DefaultPointColor.Y > value_255 &&
       DefaultPointColor.Z > value_255 && DefaultPointAlpha > value_255);

  // The reason we check for lack of texture here is that SR seems to render
  // black triangles rather than white triangles as would be expected) when
  // there is no texture AND no gradient.
  ...
  if (APT) {
    ...
    VectorProcessorClass::CopyIndexed(&compressed_loc[0], PointLoc->Get_Array(),
                                      APT->Get_Array(), PointCount);
  ...
  // need to interrupt this processing. If we are not billboarding, then we need
  // the actual position of the vertice to lay it down flat.
  ...
  Update_Arrays(current_loc, current_diffuse, current_size, current_orient,
                current_frame, PointCount, PointLoc->Get_Count(), vnum, pnum);
  ...
  // Enable sorting if the primitives are translucent and alpha testing is not
  // enabled.
  const bool sort =
  ...
  /// @todo lorenzen sez: if tri-based particles are not supported, elim this
  /// test
  if (PointMode == QUADS) {
  ...
  while (current < vnum) {
        ...
        *(Vector3 *)(vb + fvfinfo.Get_Location_Offset()) = VertexLoc[i];
        if (current_diffuse) {
          ...
          *(unsigned int *)(vb + fvfinfo.Get_Diffuse_Offset()) =
              DX8Wrapper::Convert_Color_Clamp(
                  Vector4(DefaultPointColor[0], DefaultPointColor[1],
                          DefaultPointColor[2], DefaultPointAlpha));
    ...
    if (sort) {
      SortingRendererClass::Insert_Triangles(0, delta / verticesperprimitive, 0,
                                             delta);
    } else {
 ...
 *   11/17/1998 NH  : Created.                                            *
 *========================================================================*/
void PointGroupClass::Update_Arrays(Vector3 *point_loc, Vector4 *point_diffuse,
                                    float *point_size,
                                    unsigned char *point_orientation,
                                    unsigned char *point_frame,
                                    int active_points, int total_points,
                                    int &vnum, int &pnum) {
  ...
  // Resize the arrays if they are too small. We only need to check the length
  // of one array since they always all have the same length.
  ...
  /*
  ** Generate the vertex locations from the point locations (note that both are
  *in camera space).
  ** Vertex locations depend on the point mode and the points' orientation and
  *size
  */

  // This defines the loop we run: the LSB indicates whether there is a size
  // override array, the next bit indicates whether there is an orientation
  // override array, and the higher bits indicate the point mode.
  ...
  /// @todo lorenzen sez: this switch statement may be done more compactly
  /// another way... look into it

  switch (loop_sel) {
    case TRIS_NOSIZE_NOORIENT: {
      // Setup constant vertex offsets (since size and orientation are
      // invariants)
    ...
    case TRIS_SIZE_NOORIENT: {
      // Scale vertex offsets and add them to point locations to get vertex
      // locations
      for (i = 0; i < active_points; i++) {
        vertex_loc[vert + 0] =
            point_loc[i] +
    ...
    case TRIS_NOSIZE_ORIENT: {
      ...
      for (i = 0; i < active_points; i++) {
        vertex_loc[vert + 0] =
            point_loc[i] +
            _TriVertexLocationOrientationTable[point_orientation[i]][0] *
                DefaultPointSize;
        vertex_loc[vert + 1] =
            point_loc[i] +
            _TriVertexLocationOrientationTable[point_orientation[i]][1] *
                DefaultPointSize;
        vertex_loc[vert + 2] =
            point_loc[i] +
            _TriVertexLocationOrientationTable[point_orientation[i]][2] *
                DefaultPointSize;
        vert += 3;
      }
    } break;
    ...
    case QUADS_NOSIZE_ORIENT: {
      ...
      for (i = 0; i < active_points; i++) {
        vertex_loc[vert + 0] =
            point_loc[i] +
            _QuadVertexLocationOrientationTable[point_orientation[i]][0] *
                DefaultPointSize;
        vertex_loc[vert + 1] =
            point_loc[i] +
            _QuadVertexLocationOrientationTable[point_orientation[i]][1] *
                DefaultPointSize;
        vertex_loc[vert + 2] =
            point_loc[i] +
            _QuadVertexLocationOrientationTable[point_orientation[i]][2] *
                DefaultPointSize;
        vertex_loc[vert + 3] =
            point_loc[i] +
            _QuadVertexLocationOrientationTable[point_orientation[i]][3] *
                DefaultPointSize;
        vert += 4;
      }
    } break;
    ...
    case QUADS_SIZE_ORIENT: {
      ...
      for (i = 0; i < active_points; i++) {
        if (!Billboard) {
          // If we're not billboarding, then the coordinate we have is in screen
          // space.
          Matrix4 rotMat;
          D3DXMatrixRotationZ(
              &(D3DXMATRIX &)rotMat,
          ...
          vertex_loc[vert + 0].X =
              point_loc[i].X +
              (orientedVecX.X + orientedVecY.X) * point_size[i];
          vertex_loc[vert + 0].Y =
              point_loc[i].Y +
              (orientedVecX.Y + orientedVecY.Y) * point_size[i];
          ...
          // now apply the view transform so that this data is in the format
          // expected upon the functions return.
    ...
    case SCREEN_NOSIZE_ORIENT: {
      ...
      for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 3; j++) {
          scaled_locs[i][j].X =
              _ScreenspaceVertexLocationSizeTable[i][j].X * x_scale;
          scaled_locs[i][j].Y =
              _ScreenspaceVertexLocationSizeTable[i][j].Y * y_scale;
  ...
  */
  ...
  unsigned int frame_mask =
      ~(0xFFFFFFFF << (FrameRowColumnCountLog2 +
                       FrameRowColumnCountLog2));  // To ensure frames in range
  if (point_frame) {
...
void PointGroupClass::_Init(void) {
  ...
  */
  ...
  // Unrotated locations
  Vector3 tri_locs[3] = {Vector3(0.0f, -2.0f, 0.0f),
  ...
  /// @todo lorenzen sez: unwrap loop and use pointer arithmetic (if this gets
  /// called a lot)
  ...
  for (i = 0; i < 256; i++) {
    ...
    for (j = 0; j < 3; j++) {
      _TriVertexLocationOrientationTable[i][j].X =
          tri_locs[j].X * c - tri_locs[j].Y * s;
      _TriVertexLocationOrientationTable[i][j].Y =
          tri_locs[j].X * s + tri_locs[j].Y * c;
      _TriVertexLocationOrientationTable[i][j].Z = tri_locs[j].Z;
    }
    for (j = 0; j < 4; j++) {
      _QuadVertexLocationOrientationTable[i][j].X =
          quad_locs[j].X * c - quad_locs[j].Y * s;
      _QuadVertexLocationOrientationTable[i][j].Y =
          quad_locs[j].X * s + quad_locs[j].Y * c;
  ...
  */
  ...
  Vector2 tri_uvs[3] = {Vector2(0.5f, 0.0f), Vector2(0.0f, 0.866f),
                        Vector2(1.0f, 0.866f)};
  Vector2 quad_uvs[4] = {Vector2(0.0f, 0.0f), Vector2(0.0f, 1.0f),
  ...
  for (i = 0; i < 5; i++) {
    ...
    Vector2 *tri_table = _TriVertexUVFrameTable[i] =
        W3DNEWARRAY Vector2[count * 3];
    Vector2 *quad_table = _QuadVertexUVFrameTable[i] =
        W3DNEWARRAY Vector2[count * 4];
...
void PointGroupClass::RenderVolumeParticle(RenderInfoClass &rinfo,
  ...
  for (int t = 0; t < depth; ++t) {
    ...
    if (APT) {
      ...
      /// @todo lorenzen sez: precompute pointers to indexed array elements,
      /// below
      ...
      VectorProcessorClass::CopyIndexed(&compressed_loc[0],
                                        PointLoc->Get_Array(), APT->Get_Array(),
                                        PointCount);
      current_loc = &compressed_loc[0];
      if (PointDiffuse) {
        if (compressed_diffuse.Length() < PointCount) {
    ...
    // need to interrupt this processing. If we are not billboarding, then we
    // need the actual position of the vertice to lay it down flat.
    if (Get_Flag(TRANSFORM) && Billboard) {
      // Resize transformed location array if needed (2x guardband to prevent
      // frequent reallocations):
      if (transformed_loc.Length() < PointCount) {
      ...
      for (int i = 0; i < PointCount; i++) {
        /// @todo lorenzen sez: use pointer arithmetic here and a fast while
        /// loop
    ...
    while (current < vnum) {
      ...
      if (sort)
        SortingRendererClass::Insert_Triangles(0, delta / verticesperprimitive,
                                               0, delta);
      else
        DX8Wrapper::Draw_Triangles(0, delta / verticesperprimitive, 0, delta);
