 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * DecalSystemClass::DecalSystemClass -- Constructor *
 ...
 *   DecalSystemClass::Generate_Unique_Global_Decal_Id -- Generates a unique id
 *for a decal    * DecalGeneratorClass::DecalGeneratorClass -- Constructor *
 *   DecalGeneratorClass::~DecalGeneratorClass -- Destructor *
 *   DecalGeneratorClass::Add_Mesh -- Meshes that generate a decal should add
 *themselves       * DecalGeneratorClass::Get_Mesh_List -- returns the list of
 *meshes                          * DecalGeneratorClass::Set_Mesh_Transform --
 *sets the current mesh coordinate system        *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
DecalGeneratorClass *DecalSystemClass::Lock_Decal_Generator(void) {
  DecalGeneratorClass *gen =
      W3DNEW DecalGeneratorClass(Generate_Decal_Id(), this);
  return gen;
}
 ...
 * DecalSystemClass::Unlock_Decal_Generator -- Destroys the decal generator *
 *                                                                                             *
 * Derived classes may take a record of the results of the generator here.  Then
 *they can      * put a cap on the total number of decals in the system or
 *whatever...                        *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * Hopefully you won't use more than 4 billion decals at one time...
 **
 *                                                                                             *
 * HISTORY: * 1/26/00    gth : Created. *
 *=============================================================================================*/
 ...
 *themselves         *
 *                                                                                             *
 * This adds a mesh to the internal list.  All meshes that actually add a decal
 *should add     * themselves to this list so that the DecalSystem can keep
 *track of them.                     *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * DecalGeneratorClass::Set_Mesh_Transform -- sets the current mesh coordinate
 *system          *
 *                                                                                             *
 * The coordinate system of the mesh is needed in order to compute texture
 *coordinates for     * the decal polygons. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
...
MultiFixedPoolDecalSystemClass::MultiFixedPoolDecalSystemClass(
    ...
    : Pools(0), PoolCount(num_pools), Generator_PoolID(0), Generator_SlotID(0) {
  if (PoolCount) {
    WWASSERT(pool_sizes);
...
MultiFixedPoolDecalSystemClass::MultiFixedPoolDecalSystemClass(
    const MultiFixedPoolDecalSystemClass &that)
    : Pools(0),
  ...
  // Allocate arrays (we dont' copy array contents because those are
  // mesh-specific and will be filled when the state is set anyway)
...
// This clears the slot in addition to locking the generator, thus preventing
// any decal id collisions (since any decal previously in that slot will have
// the same id as the new one).
...
// This will register the decal in the system in the appropriate pool and slot
// (determined by the generator's pool and slot ids), removing any decal which
// may have been there before.
void MultiFixedPoolDecalSystemClass::Unlock_Decal_Generator(
...
// This notifies the system that a mesh which has decals on it was destroyed -
// therefore we need to remove the mesh from our list to avoid dangling
// pointers.
void MultiFixedPoolDecalSystemClass::Decal_Mesh_Destroyed(
    uint32 decal_id, DecalMeshClass *mesh) {
  // We must remove this mesh from all lists where it is present. The method is:
  // for each decal id present in the decal mesh, find the logical decal and
  // remove the decal mesh from its list.
  assert(mesh);
  MeshClass *parent = mesh->Peek_Parent();
  find_logical_decal(decal_id).MeshList.Remove(parent);
}

// Not part of the DecalSystemClass interface - this function removes any decal
// currently in the given slot in the given pool.
...
// Get a reference to the logical decal at the given pool and slot id (performs
// range checking)
MultiFixedPoolDecalSystemClass::LogicalDecalClass &
MultiFixedPoolDecalSystemClass::find_logical_decal(uint32 pool_id,
                                                   uint32 slot_id) {
...
MultiFixedPoolDecalSystemClass::LogicalDecalClass &
MultiFixedPoolDecalSystemClass::find_logical_decal(uint32 decal_id) {
...
MultiFixedPoolDecalSystemClass::LogicalDecalClass::~LogicalDecalClass(void) {
  // We don't need to do anything here since the mesh list will get removed
  // automatically and the decals themselves don't neccessarily need to be
  // removed because the logical decal is. (we don't hold references to the
  // meshes in the mesh list)
}

// Sets the logical decal to the one specified by the given generator (clearing
// any existing decal information)
void MultiFixedPoolDecalSystemClass::LogicalDecalClass::Set(
    DecalGeneratorClass *generator) {
...
void MultiFixedPoolDecalSystemClass::LogicalDecalPoolClass::Initialize(
    uint32 size) {
  if (Array) {
    delete[] Array;
    Array = 0;
