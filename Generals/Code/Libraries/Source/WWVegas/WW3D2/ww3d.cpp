 ...
 *-- Time sychronization * WW3D::Set_Ext_Swap_Interval -- Sets the swap interval
 *the device should aim sync for.     * WW3D::Get_Ext_Swap_Interval -- Queries
 *the swap interval the device is aiming sync for.   * WW3D::Set_Polygon_Mode --
...
WW3D::TextureThumbnailModeEnum WW3D::TextureThumbnailMode =
    TEXTURE_THUMBNAIL_MODE_ON;
WW3D::TextureCompressionModeEnum WW3D::TextureCompressionMode =
...
WW3DErrorType WW3D::Init(void *hwnd, char *defaultpal) {
  ...
  */
  ...
  if (!DX8Wrapper::Init(_Hwnd)) return WW3D_ERROR_INITIALIZATION_FAILED;
  ...
  */
  DefaultStaticSortLists =
      W3DNEWARRAY RefRenderObjListClass[MAX_SORT_LEVEL + 1];
 ...
 *=============================================================================================*/
void *WW3D::Get_Window(void) { return _Hwnd; }
 ...
 *=============================================================================================*/
bool WW3D::Is_Windowed(void) { return DX8Wrapper::Is_Windowed(); }
 ...
 *=============================================================================================*/
int WW3D::Get_Render_Device(void) { return DX8Wrapper::Get_Render_Device(); }
...
WW3DErrorType WW3D::Render(SceneClass *scene, CameraClass *cam, bool clear,
  ...
  // LightEnvironment system this setting will get overriden.
  Vector3 ambient = scene->Get_Ambient_Light();
  DX8Wrapper::Set_DX8_Render_State(D3DRS_AMBIENT,
                                   DX8Wrapper::Convert_Color(ambient, 0.0f));
 ...
 *=============================================================================================*/
void WW3D::Flip_To_Primary(void) { DX8Wrapper::Flip_To_Primary(); }
...
void WW3D::Make_Screen_Shot(const char *filename_base) {
  ...
  RawFileClass *file =
      (RawFileClass *)_TheWritingFileFactory->Get_File(filename);
  if (file) {
    file->Create();
    file->Close();
 ...
 * HISTORY: * 5/19/99    GTH : Created. *
 *=============================================================================================*/
bool WW3D::Is_Movie_Ready() { return Movie != 0; }
...
void WW3D::Enable_Coloring(unsigned int color) { IsColoringEnabled = color; }
 ...
 *=============================================================================================*/
int WW3D::Get_Texture_Reduction(void) { return _TextureReduction; }
 ...
 * HISTORY: * 11/25/99    TSS : Created. *
 *=============================================================================================*/
int WW3D::Get_Texture_Min_Mip_Levels(void) { return _TextureMinMipLevels; }
...
WW3DErrorType WW3D::On_Deactivate_App(void) {
  ...
  if (Gerd == NULL) return WW3D_ERROR_OK;

  if (IsWindowed) return WW3D_ERROR_OK;

  if (!Gerd->isWindowOpen()) return WW3D_ERROR_OK;
...
void WW3D::Render_And_Clear_Static_Sort_Lists(RenderInfoClass &rinfo) {
  ...
  // We go from higher sort level to lower, since lower sort level means higher
  // priority (in front), so lower sort level meshes need to be rendered later.
  for (unsigned int sort_level = MaxStaticSortLevel;
       sort_level >= MinStaticSortLevel; sort_level--) {
    bool render = false;
    for (RenderObjClass *robj =
             CurrentStaticSortLists[sort_level].Remove_Head();
         robj; robj->Release_Ref(),
                        robj =
                            CurrentStaticSortLists[sort_level].Remove_Head()) {
      robj->Render(rinfo);
      render = true;
...
void WW3D::Override_Current_Static_Sort_Lists(RefRenderObjListClass *sort_list,
