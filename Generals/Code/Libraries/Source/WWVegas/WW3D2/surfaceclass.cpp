 ...
 *zero pixels in the region (x0, * PixelSize -- Helper Function to find the size
 *in bytes of a pixel                         *
 *   SurfaceClass::Is_Transparent_Column -- Tests to see if the column is
 *transparent or not   * SurfaceClass::Copy -- Copies from a byte array to the
 *surface                             * SurfaceClass::CreateCopy -- Creates a
 *byte array copy of the surface                      * SurfaceClass::DrawHLine
...
SurfaceClass::SurfaceClass(const char *filename) : D3DSurface(NULL) {
...
SurfaceClass::SurfaceClass(IDirect3DSurface8 *d3d_surface) : D3DSurface(NULL) {
  Attach(d3d_surface);
  SurfaceDescription desc;
  Get_Description(desc);
  SurfaceFormat = desc.Format;
...
void SurfaceClass::Unlock(void) { DX8_ErrorCode(D3DSurface->UnlockRect()); }
 ...
 *=============================================================================================*/
void SurfaceClass::Copy(Vector2i &min, Vector2i &max,
...
void SurfaceClass::Copy(unsigned int dstx, unsigned int dsty, unsigned int srcx,
  ...
  if (sd.Format == osd.Format && sd.Width == osd.Width &&
    ...
    DX8_ErrorCode(D3DXLoadSurfaceFromSurface(D3DSurface, NULL, &dest,
                                             other->D3DSurface, NULL, &src,
                                             D3DX_FILTER_NONE, 0));
  }
}
 ...
 * HISTORY: *
 *=============================================================================================*/
void SurfaceClass::DrawPixel(const unsigned int x, const unsigned int y,
                             unsigned int color) {
  SurfaceDescription sd;
  Get_Description(sd);
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 4/9/2001   hy : Created. * 4/9/2001   hy : Created. *
 *=============================================================================================*/
void SurfaceClass::DrawHLine(const unsigned int y, const unsigned int x1,
