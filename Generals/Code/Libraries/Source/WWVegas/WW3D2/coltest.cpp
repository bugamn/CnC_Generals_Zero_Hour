...
bool AABoxCollisionTestClass::Cull(const AABoxClass& box) {
  ...
  //	if (WWMath::Fabs(Move.X) + WWMath::Fabs(Move.Y) + WWMath::Fabs(Move.Z) >
  //MOVE_THRESHOLD) { 		CastResultStruct res; 		return
  //!Box.Cast_To_Box(Move,box,&res); 	} else {
  Vector3 min_corner;
  Vector3 max_corner;
...
void AABoxCollisionTestClass::Transform(const Matrix3D& tm) {
  ...
  tm.Transform_Center_Extent_AABox(tmpcenter, tmpextent, &Box.Center,
                                   &Box.Extent);
...
OBBoxCollisionTestClass::OBBoxCollisionTestClass(
    const OBBoxCollisionTestClass& that, const Matrix3D& tm)
    : CollisionTestClass(that) {
  tm.Transform_Min_Max_AABox(that.SweepMin, that.SweepMax, &SweepMin,
                             &SweepMax);
...
OBBoxCollisionTestClass::OBBoxCollisionTestClass(
    const AABoxCollisionTestClass& that, const Matrix3D& tm)
    : CollisionTestClass(that) {
  tm.Transform_Min_Max_AABox(that.SweepMin, that.SweepMax, &SweepMin,
                             &SweepMax);
  Matrix3D::Rotate_Vector(tm, that.Move, &Move);
