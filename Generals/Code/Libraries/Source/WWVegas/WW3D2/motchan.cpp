 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * MotionChannelClass::MotionChannelClass -- constructor *
 ...
 * HISTORY: * 08/11/1997 GH  : Created. *
 *=============================================================================================*/
MotionChannelClass::~MotionChannelClass(void) { Free(); }
...
bool MotionChannelClass::Load_W3D(ChunkLoadClass &cload) {
  ...
  if (cload.Read(&chan, sizeof(W3dAnimChannelStruct)) !=
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 1/21/98    GTH : Created. *
 *=============================================================================================*/
BitChannelClass::~BitChannelClass(void) { Free(); }
...
bool BitChannelClass::Load_W3D(ChunkLoadClass &cload) {
  ...
  if (cload.Read(&chan, sizeof(W3dBitChannelStruct)) !=
      sizeof(W3dBitChannelStruct)) {
    return false;
  }
 ...
 * HISTORY: * 08/11/1997 GH  : Created. *
 *=============================================================================================*/
TimeCodedMotionChannelClass::~TimeCodedMotionChannelClass(void) { Free(); }
...
bool TimeCodedMotionChannelClass::Load_W3D(ChunkLoadClass &cload) {
  ...
  if (cload.Read(&chan, sizeof(W3dTimeCodedAnimChannelStruct)) !=
...
/***********************************************************************************************
 * TimeCodedMotionChannelClass::Get_Vector -- returns the vector for the
 *specified frame #              *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 01/27/2000 JGA  : Created. *
 *=============================================================================================*/
...
uint32 TimeCodedMotionChannelClass::binary_search_index(uint32 timecode) {
  ...
  // int32		LastTimeCodeIdx;	// absolute index to last time
  // code int32		CachedIdx;			// Last Index Used
...
/***********************************************************************************************
 * TimeCodedMotionChannelClass::set_identity -- returns an "identity" vector
 *(not really...hmm...)      *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * TimeCodedBitChannelClass::TimeCodedBitChannelClass -- Constructor for
 *BitChannelClass                         *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
TimeCodedBitChannelClass::TimeCodedBitChannelClass(void)
    : PivotIdx(0), Type(0), DefaultVal(0), Bits(NULL), CachedIdx(0) {}

/***********************************************************************************************
 * TimeCodedBitChannelClass::~TimeCodedBitChannelClass -- Destructor for
 *BitChannelClass                         *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 1/21/98    GTH : Created. *
 *=============================================================================================*/
TimeCodedBitChannelClass::~TimeCodedBitChannelClass(void) { Free(); }
...
bool TimeCodedBitChannelClass::Load_W3D(ChunkLoadClass &cload) {
  ...
  if (cload.Read(&chan, sizeof(W3dTimeCodedBitChannelStruct)) !=
      sizeof(W3dTimeCodedBitChannelStruct)) {
    return false;
  }
  ...
  assert((sizeof(W3dTimeCodedBitChannelStruct) + bytesleft) ==
         (unsigned)chunk_size);
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 02/18/2000 JGA : Created. *
 *=============================================================================================*/
  ...
  if (false == table_valid) {
    ...
    for (int i = 0; i < FILTER_TABLE_GEN_SIZE; i++) {
      ...
      filtertable[i + FILTER_TABLE_GEN_START] =
          1.0f - WWMath::Sin(DEG_TO_RAD(90.0f * ratio));
    }
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 02/18/1000 JGA : Created. *
 *=============================================================================================*/
AdaptiveDeltaMotionChannelClass::~AdaptiveDeltaMotionChannelClass(void) {
  Free();
}
...
bool AdaptiveDeltaMotionChannelClass::Load_W3D(ChunkLoadClass &cload) {
  ...
  if (cload.Read(&chan, sizeof(W3dAdaptiveDeltaAnimChannelStruct)) !=
  ...
  CacheData = MSGW3DNEWARRAY("AdaptiveDeltaMotionChannelClass::CacheData") float
      [VectorLen * 2];  // cacheframe & cachedframe+1 by VectorLen

  Data =
      MSGW3DNEWARRAY("AdaptiveDeltaMotionChannelClass::Data") uint32[numInts];
  Data[0] = chan.Data[0];
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 02/23/2000 JGA  : Created. *
 *=============================================================================================*/
#define PACKET_SIZE (9)
void AdaptiveDeltaMotionChannelClass::decompress(uint32 frame_idx,
                                                 float *outdata) {
  ...
  for (int vi = 0; vi < VectorLen; vi++) {
    ...
    unsigned char *pPacket =
        (unsigned char *)Data;  // pointer to current packet
    pPacket +=
        (sizeof(float) * VectorLen);  // skip non-compressed header information
...
void AdaptiveDeltaMotionChannelClass::decompress(uint32 src_idx, float *srcdata,
                                                 uint32 frame_idx,
                                                 float *outdata) {
  ...
  for (int vi = 0; vi < VectorLen; vi++) {
    // Decompress all the vector indices, since they will probably all be needed
    unsigned char *pPacket =
        (unsigned char *)base;    // pointer to current packet
    pPacket += PACKET_SIZE * vi;  // skip to the appropriate packet start
    pPacket += (PACKET_SIZE * VectorLen) *
               ((src_idx - 1) >> 4);  // skip out to current packet
...
/***********************************************************************************************
 * AdaptiveDeltaMotionChannelClass::getframe returns decompressed data for
 *frame/vectorindex   *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 02/18/2000 JGA  : Created. *
 *=============================================================================================*/
float AdaptiveDeltaMotionChannelClass::getframe(uint32 frame_idx,
                                                uint32 vector_idx) {
  ...
  if (frame_idx < CacheFrame) {
    // Requested Frame isn't cached, so cache it, and frame_idx+1, and return
    ...
    if (frame_idx != (NumFrames - 1)) {
      decompress(frame_idx, &CacheData[0], frame_idx + 1,
                 &CacheData[VectorLen]);
    }
...
/***********************************************************************************************
 * AdaptiveDeltaMotionChannelClass::Get_Vector -- returns the vector for the
 *specified frame # *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
Quaternion AdaptiveDeltaMotionChannelClass::Get_QuatVector(float32 frame) {
  ...
  Quaternion q1(1);
  q1.Set(getframe(frame1, 0), getframe(frame1, 1), getframe(frame1, 2),
         getframe(frame1, 3));

  Quaternion q2(1);

  q2.Set(getframe(frame2, 0), getframe(frame2, 1), getframe(frame2, 2),
         getframe(frame2, 3));
