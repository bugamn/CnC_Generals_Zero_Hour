 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * MeshModelClass::Load_W3D -- Load a mesh from a W3D file *
 ...
 *   MeshModelClass::read_vertex_normals -- reads a vertex normal chunk from the
 *w3d file      * MeshModelClass::read_v3_materials -- Reads in version 3
 *materials.                        * MeshModelClass::read_triangles -- read the
 *triangles chunk                                *
 *   MeshModelClass::read_per_tri_materials -- read the material indices for
 *each triangle     * MeshModelClass::read_user_text -- read in the user text
 *chunk                             * MeshModelClass::read_vertex_colors -- read
 *in the vertex colors chunk                     *
 *   MeshModelClass::read_vertex_influences -- read in the vertex influences
 *chunk             * MeshModelClass::read_vertex_shade_indices -- read the
 *shade index chunk                   * MeshModelClass::read_material_info --
 ...
 *   MeshModelClass::read_dig -- read the per-vertex diffuse illumination for a
 *pass           * MeshModelClass::read_scg -- read the specular color for a
 *pass                            * MeshModelClass::read_texture_stage -- read
 *texture stage chunks                           *
 ...
 ** MeshModelClass::read_per_face_texcoord_ids -- read uv indices for given
 *(pass,stage).	  * MeshModelClass::read_prelit_material -- read prelit material
 *chunks.							  *
 *   MeshModelClass::read_aabtree -- loads the aabtree chunk *
 *   MeshModelClass::Save -- Save this mesh model! *
 *   MeshLoadContextClass::MeshLoadContextClass -- constructor for
 *MeshLoadContextClass        * MeshLoadContextClass::~MeshLoadContextClass --
 *destructor                                 *
 *   MeshLoadContextClass::Get_Texcoord_Array -- returns the texture coordinates
 *array         * MeshLoadContextClass::Add_Shader -- adds a shader to the array
 ...
 *   MeshLoadContextClass::Peek_Legacy_Vertex_Material -- returns a pointer to a
 *legacy vmat   * MeshLoadContextClass::Peek_Legacy_Texture -- returns a pointer
 *to a texture               * MeshSaveContextClass::MeshSaveContextClass --
 *constructor                                 *
...
#define MESH_SINGLE_MATERIAL_HACK \
  0  // (gth) forces all multi-material meshes to use their first material only.
     // (NOT RECOMMENDED, TESTING ONLY!)
#define MESH_FORCE_STATIC_SORT_HACK \
  0  // (gth) forces all sorting meshes to use static sort level 1 instead.
...
class MeshLoadContextClass : public W3DMPO {
  ...
  */
  void Add_Legacy_Material(ShaderClass shader, VertexMaterialClass *vmat,
                           TextureClass *tex);
  ...
  */
  ...
  /*
  ** Currently, the only tool that creates DIG chunks is the lightmap tool.
  *Since DX8 support
  ** for linking the emissive material color to an array seems poor, We're just
  *going to multiply
  ** the DIG array into the DCG array (or make it the DCG array).  Now, to
  *properly support the
  ** "alternate material set", we have to know whether we've already encountered
  *a DIG chunk so
 ...
 private:
  struct LegacyMaterialClass {
    LegacyMaterialClass(void)
        : Name(NULL), VertexMaterialIdx(0), ShaderIdx(0), TextureIdx(0) {}
    ~LegacyMaterialClass(void) {
      if (Name) free(Name);
    }
  ...
  ** this MeshMatDescClass object.  When loading is finished, an alternate
  *MeshMatDescClass
  ** will be allocated in the mesh model.  This MeshMatDescClass will be
  *initialized to be
  ** identical to the default MeshMatDescClass and then any data contained in
  *this
...
WW3DErrorType MeshModelClass::Load_W3D(ChunkLoadClass &cload) {
  ...
  */
  ...
  if (cload.Read(&(context->Header), sizeof(W3dMeshHeader3Struct)) !=
  ...
  ** Set Bounding Info
  */
  BoundBoxMin.Set(context->Header.Min.X, context->Header.Min.Y,
                  context->Header.Min.Z);
  BoundBoxMax.Set(context->Header.Max.X, context->Header.Max.Y,
                  context->Header.Max.Z);

  BoundSphereCenter.Set(context->Header.SphCenter.X,
                        context->Header.SphCenter.Y,
                        context->Header.SphCenter.Z);
  ...
  */
  if (context->Header.Version >= W3D_MAKE_VERSION(4, 1)) {
    int geometry_type =
        context->Header.Attributes & W3D_MESH_FLAG_GEOMETRY_TYPE_MASK;
  ...
  if (context->Header.Attributes & W3D_MESH_FLAG_PRELIT_MASK) {
    // Select from the available prelit materials based on current prelit
    // lighting mode. If the model does not have the current prelit mode, select
    // the next highest quality prelit material that is available.
    switch (WW3D::Get_Prelit_Mode()) {
      case WW3D::PRELIT_MODE_LIGHTMAP_MULTI_TEXTURE:
        if (context->Header.Attributes &
            W3D_MESH_FLAG_PRELIT_LIGHTMAP_MULTI_TEXTURE) {
      ...
      case WW3D::PRELIT_MODE_LIGHTMAP_MULTI_PASS:
        if (context->Header.Attributes &
            W3D_MESH_FLAG_PRELIT_LIGHTMAP_MULTI_PASS) {
      ...
      default:
        ...
        // This prelighting option MUST be available if none of the others are
        // available.
  ...
  */
  ...
  /*
  ** If this mesh is collideable and no AABTree was in the file, generate one
  *now
  */
  if ((((W3dAttributes & W3D_MESH_FLAG_COLLISION_TYPE_MASK) >>
        W3D_MESH_FLAG_COLLISION_TYPE_SHIFT) != 0) &&
      (CullTree == NULL)) {
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 3/6/98     GTH : Created. * 2/16/99    GTH : Moved into MeshModel
 **
 *=============================================================================================*/
  ...
  if (uvarray != NULL) {
    ...
    */
    for (int i = 0; i < VertexCount; i++) {
      if (cload.Read(&(texcoord), sizeof(W3dTexCoordStruct)) !=
    ...
    DefMatDesc->Install_UV_Array(context->CurPass, context->CurTexStage,
                                 uvarray, elementcount);
  }
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 4/2/98     GTH : Created. * 2/16/99    GTH : Moved into
 *MeshModelClass                                                *
 *=============================================================================================*/
WW3DErrorType MeshModelClass::read_v3_materials(ChunkLoadClass &cload,
  ...
  for (unsigned int mi = 0; mi < context->Header.NumMaterials; mi++) {
    ...
    */
    ...
    if (cload.Read(&mat, sizeof(W3dMaterial3Struct)) !=
        sizeof(W3dMaterial3Struct))
      goto Error;
    ...
    */
    while (cload.Open_Chunk()) {
      if (cload.Cur_Chunk_ID() == W3D_CHUNK_MATERIAL3_DC_MAP) {
        ...
        */
        ...
        if (cload.Read(&mapinfo, sizeof(W3dMap3Struct)) !=
            sizeof(W3dMap3Struct))
          goto Error;
        if (!cload.Close_Chunk()) goto Error;
    ...
    // If not texturing, move the diffuse color to ambient (simulating old
    // behavior)
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 4/7/98     GTH : Created. *
 *=============================================================================================*/
WW3DErrorType MeshModelClass::read_per_tri_materials(
    ChunkLoadClass &cload, MeshLoadContextClass *context) {
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/28/1997 GH  : Created. * 2/16/99    GTH : Moved into
 *MeshModelClass                                                *
 *=============================================================================================*/
WW3DErrorType MeshModelClass::read_vertex_colors(
    ChunkLoadClass &cload, MeshLoadContextClass *context) {
  /*
  ** The W3D file format supports arbitrary vertex color arrays for each pass;
  *however since
  ** our conversion to hardware T&L, we only support two unique color arrays. So
  *here is
  ** what is happening in this function:
  **
  ** 1 - If this is the first diffuse color array we've encountered, load the
  *values
  ** 2 - Always set the DCG source for this pass to the array (vertex materials
  *will be fixed later)
  **
  ** A side effect is that if two DCG chunks are encountered, only the first is
  *used...
  */
 ...
 * HISTORY: * 2/16/99    GTH : Created. *
 *=============================================================================================*/
WW3DErrorType MeshModelClass::read_material_info(
    ChunkLoadClass &cload, MeshLoadContextClass *context) {
  if (cload.Read(&(context->MatInfo), sizeof(W3dMaterialInfoStruct)) !=
      sizeof(W3dMaterialInfoStruct)) {
    return WW3D_ERROR_LOAD_FAILED;
  }
 ...
 * HISTORY: * 2/16/99    GTH : Created. *
 *=============================================================================================*/
WW3DErrorType MeshModelClass::read_vertex_materials(
    ChunkLoadClass &cload, MeshLoadContextClass *context) {
  while (cload.Open_Chunk()) {
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 2/16/99    GTH : Created. * 3/05/99	 PDS : Broke the guts of
 *this function into a util function in texture.cpp		  *
 *=============================================================================================*/
WW3DErrorType MeshModelClass::read_textures(ChunkLoadClass &cload,
                                            MeshLoadContextClass *context) {
  // Keep reading textures until there are no more...
  for (TextureClass *newtex = ::Load_Texture(cload); newtex != NULL;
 ...
 * HISTORY: * 2/16/99    GTH : Created. *
 *=============================================================================================*/
WW3DErrorType MeshModelClass::read_material_pass(
    ChunkLoadClass &cload, MeshLoadContextClass *context) {
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 2/16/99    GTH : Created. * 9/1/2000   gth : Added alternate
 *material desc support                                    *
 *=============================================================================================*/
WW3DErrorType MeshModelClass::read_vertex_material_ids(
    ChunkLoadClass &cload, MeshLoadContextClass *context) {
  /*
  ** Determine whether this chunk should be read into the default or alternate
...
#pragma message( \
    "(gth) Hacking to make Generals behave as if all meshes have 1 material")
  cload.Read(&vmat, sizeof(uint32));
  matdesc->Set_Single_Material(context->Peek_Vertex_Material(vmat),
                               context->CurPass);
...
WW3DErrorType MeshModelClass::read_shader_ids(ChunkLoadClass &cload,
  ...
  */
  ...
  if (cload.Cur_Chunk_Length() == 1 * sizeof(uint32)) {
    ...
    // turn on sorting of pass 0 has non-zero dest blend (unless alpha testing
    // on)
 ...
 *                                                                                             *
 * HISTORY: * 2/16/99    GTH : Created. * 9/1/2000   gth : Added alternate
 *material desc support                                    * 2/9/2001   gth :
 *converted to handle dx8 limitations                                      *
 *=============================================================================================*/
WW3DErrorType MeshModelClass::read_dcg(ChunkLoadClass &cload,
  ...
  */
  MeshMatDescClass *matdesc = DefMatDesc;
  if (DefMatDesc->Get_DCG_Source(context->CurPass) !=
  ...
  ** 1 - If this is the first diffuse color array we've encountered, load the
  *values.
  ** 2 - Otherwise, if we are in PRELIT_VERTEX mode, put the alpha from this
  *array into the color array.
  ** 3 - Always set the DCG source for this pass to the array.
  **
  ** Our tools *currently* will only generate two color arrays in the case where
  *one of them
  ** is being used for alpha and the other is used for precomputed vertex
  *lighting...  This will
  ** break if our tools change.  The file format isn't restricting you from
  *defining something
...
WW3DErrorType MeshModelClass::read_dig(ChunkLoadClass &cload,
  ...
  */
  ...
  /*
  ** It appears that there isn't wide support for having the emissive material
  *color source
  ** be a vertex color array so when there is a pre-existing color array, I'm
  *just multiplying
 ...
 * HISTORY: * 2/16/99    GTH : Created. * 9/1/2000   gth : Added alternate
 *material desc support                                    * 2/9/2001   gth :
 *new dx8 code no longer supports this chunk                               *
 *=============================================================================================*/
 ...
 * HISTORY: * 2/16/99    GTH : Created. *
 *=============================================================================================*/
WW3DErrorType MeshModelClass::read_texture_stage(
    ChunkLoadClass &cload, MeshLoadContextClass *context) {
  while (cload.Open_Chunk()) {
    WW3DErrorType error = WW3D_ERROR_OK;
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 2/16/99    GTH : Created. * 7/14/99    IML : Lightmap support:
 *calculate vertex count directly from chunk size.		  * 9/1/2000 gth
 *: Added alternate material desc support                                    *
 *=============================================================================================*/
WW3DErrorType MeshModelClass::read_stage_texcoords(
    ChunkLoadClass &cload, MeshLoadContextClass *context) {
  ...
  */
  ...
  matdesc->Install_UV_Array(context->CurPass, context->CurTexStage, uvs,
                            elementcount);
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 02/02/99    IML : Created. * 9/1/2000   gth : Added alternate
 *material desc support                                    *
 *=============================================================================================*/
WW3DErrorType MeshModelClass::read_per_face_texcoord_ids(
    ChunkLoadClass &cload, MeshLoadContextClass *context) {
  unsigned size;
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 02/02/99    IML : Created. *
 *=============================================================================================*/
WW3DErrorType MeshModelClass::read_prelit_material(
    ChunkLoadClass &cload, MeshLoadContextClass *context) {
  // If this chunk ID matches the selected prelit chunk ID then load it,
  // otherwise skip it.
...
/***********************************************************************************************
 * MeshModelClass::post_process -- post loading, perform any processing on this
 *model.			  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 05/02/00   IML : Created. * 7/13/2001  hy : Added static sort
 *postprocessing                                          *
 *=============================================================================================*/
void MeshModelClass::post_process() {
  ...
  if (Get_Flag(SORT) && SortLevel == SORT_LEVEL_NONE &&
      WW3D::Is_Munge_Sort_On_Load_Enabled()) {
...
void MeshModelClass::post_process_fog(void) {
  ...
  if (DefMatDesc->Get_Pass_Count() == 2) {
    ...
    if (!DefMatDesc->ShaderArray[0] && !DefMatDesc->ShaderArray[1]) {
      ...
      // Analyze the mesh to determine if it is the emissive map effect and if
      // it is, fix it up appropriately.
      bool emissive_map_effect =
          ...
          shader0.Get_Secondary_Gradient() ==
      ...
      if (emissive_map_effect) {
        // Change the shader/texture setting into an equivalent one which will
        // enable setting fog correctly: Note that we are setting up pass 0 to
        // have a texture now.
        ...
        if (DefMatDesc->TextureArray[1][0]) {
          if (!DefMatDesc->TextureArray[0][0]) {
            DefMatDesc->TextureArray[0][0] = NEW_REF(
                TexBufferClass,
                (PolyCount, "MeshModelClass::DefMatDesc::TextureArray"));
            for (int i = 0; i < PolyCount; i++) {
              DefMatDesc->TextureArray[0][0]->Set_Element(
                  i, DefMatDesc->TextureArray[1][0]->Peek_Element(i));
            }
          }
        }

        // Make pass 0 point to the same UV array as pass 1. If pass 1 has a
        // vertex material array, we only take the first one for determining UV
        // source. The UV source is used to set the UV source of all the vertex
        // materials in pass 0.
        int uv_source = 0;
        ...
        if (DefMatDesc->MaterialArray[0]) {
          for (int i = 0; i < VertexCount; i++) {
            DefMatDesc->MaterialArray[0]->Peek_Element(i)->Set_UV_Source(
                0, uv_source);
          }
        } else {
      ...
      // Analyze the mesh to determine if it is the shiny mask effect and if it
      // is, fix it up appropriately.
      bool shiny_mask_effect =
          shader0.Get_Src_Blend_Func() == ShaderClass::SRCBLEND_ONE &&
          shader0.Get_Dst_Blend_Func() == ShaderClass::DSTBLEND_ZERO &&
          shader1.Get_Src_Blend_Func() == ShaderClass::SRCBLEND_ONE &&
          (shader1.Get_Dst_Blend_Func() == ShaderClass::DSTBLEND_SRC_ALPHA ||
           shader1.Get_Dst_Blend_Func() ==
  ...
  // Mesh is not one of the special two-pass combinations. Apply a per-pass
  // generic fix-up.
  for (int pass = 0; pass < DefMatDesc->PassCount; pass++) {
    DefMatDesc->Shader[pass].Enable_Fog(Get_Name());
    if (DefMatDesc->ShaderArray[pass]) {
      for (int tri = 0; tri < DefMatDesc->ShaderArray[pass]->Get_Count();
           tri++) {
...
void MeshModelClass::install_alternate_material_desc(
    MeshLoadContextClass *context) {
 ...
 * MeshLoadContextClass::Add_Legacy_Material -- adds a legacy material *
 *                                                                                             *
 * This function will check to see if the parameters of any of the parts of the
 *material are   * duplicated.  Only unique shaders/vertexmaterials will be
 *actually added.                    *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 12/10/98   GTH : Created. *
 *=============================================================================================*/
void MeshLoadContextClass::Add_Legacy_Material(ShaderClass shader,
  ...
  if (tex == NULL) {
    ...
    for (int ti = 0; ti < Textures.Count(); ti++) {
      ...
      if (stricmp(Textures[ti]->Get_Texture_Name(), tex->Get_Texture_Name()) ==
          0)
        break;
    }
 ...
 **
 *                                                                                             *
 * This function does the re-indexing to go from the legacy shader index to the
 *actual shader  * index and then returns that shader *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 ...
 * HISTORY: * 12/10/98   GTH : Created. *
 *=============================================================================================*/
ShaderClass MeshLoadContextClass::Peek_Legacy_Shader(
 ...
 * HISTORY: * 12/10/98   GTH : Created. *
 *=============================================================================================*/
VertexMaterialClass *MeshLoadContextClass::Peek_Legacy_Vertex_Material(
    int legacy_material_index) {
  WWASSERT(legacy_material_index >= 0);
  WWASSERT(legacy_material_index < LegacyMaterials.Count());
 ...
 * HISTORY: * 12/10/98   GTH : Created. *
 *=============================================================================================*/
TextureClass *MeshLoadContextClass::Peek_Legacy_Texture(
    int legacy_material_index) {
  WWASSERT(legacy_material_index >= 0);
  WWASSERT(legacy_material_index < LegacyMaterials.Count());
 ...
 * HISTORY: *
 *=============================================================================================*/
MeshSaveContextClass::MeshSaveContextClass(void) : CurPass(0), CurStage(0) {}
...
#if 0  // MESH SAVING CODE HAS NOT BEEN MAINTAINED... Leaving here for future
