 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * MatrixMapperClass::MatrixMapperClass -- Constructor *
 *   MatrixMapperClass::Set_Texture_Transform -- Sets the
 *viewspace-to-texturespace transform  *
 *   MatrixMapperClass::Update_View_To_Pixel_Transform -- recomputes ViewToPixel
 ** MatrixMapperClass::Compute_Texture_Coordinate -- compute a single texture
 *coord           * MatrixMapperClass::isActive -- check if this mapper should
 *be active                      * MatrixMapperClass::process -- process the
 *given VertexPipe                                *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 * HISTORY: * 11/13/99   gth : Created. *
 *=============================================================================================*/
void MatrixMapperClass::Set_Texture_Transform(const Matrix4& view_to_texture,
                                              float texsize) {
...
void MatrixMapperClass::Update_View_To_Pixel_Transform(float tex_size) {
  /*
  ** Create a ViewToPixel matrix which also does all of the offseting and
  *flipping that has
  ** to take place to get the actual texture coordinates.
  **
  ** Here is a description of the math:
  ** If you transform eye-space points with the ViewToTexture matrix, you then
  *need to compute:
  ...
  ** | 0  0 0 1 |
  **
  ** The code below manually "optimally" pre-multiplies this matrix with the
  *ViewToTexture matrix
  **
  ** In addition, the z-coordinate is modified so that it goes from 0 to 1
  *rather than -1 to +1.
  ...
  */
  ...
  if (Get_Flag(INVERT_DEPTH_GRADIENT)) {
    ViewToPixel[2][0] =
        -0.5f * ViewToTexture[2][0] + 0.5f * ViewToTexture[3][0];
    ViewToPixel[2][1] =
        -0.5f * ViewToTexture[2][1] + 0.5f * ViewToTexture[3][1];
    ViewToPixel[2][2] =
        -0.5f * ViewToTexture[2][2] + 0.5f * ViewToTexture[3][2];
    ViewToPixel[2][3] =
        -0.5f * ViewToTexture[2][3] + 0.5f * ViewToTexture[3][3];
  } else {
 ...
 *coord             *
 *                                                                                             *
 * INPUT: * point - 3D point to calculate the texture coordinate for * set_stq -
 *pointer to texture coordinate, s,t will be stored in X,Y.  q will be stored in
 *Z  *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
...
void MatrixMapperClass::Compute_Texture_Coordinate(const Vector3& point,
                                                   Vector3* set_stq) {
  set_stq->X = ViewToPixel[0][0] * point.X + ViewToPixel[0][1] * point.Y +
               ViewToPixel[0][2] * point.Z + ViewToPixel[0][3];
  set_stq->Y = ViewToPixel[1][0] * point.X + ViewToPixel[1][1] * point.Y +
               ViewToPixel[1][2] * point.Z + ViewToPixel[1][3];
  set_stq->Z = ViewToPixel[3][0] * point.X + ViewToPixel[3][1] * point.Y +
               ViewToPixel[3][2] * point.Z + ViewToPixel[3][3];
}
...
void MatrixMapperClass::Apply(int uv_array_index) {
  ...
  switch (Type) {
    case ORTHO_PROJECTION:
      ...
      */
      ...
      DX8Wrapper::Set_DX8_Texture_Stage_State(
          uv_array_index, D3DTSS_TEXCOORDINDEX, D3DTSS_TCI_CAMERASPACEPOSITION);
      DX8Wrapper::Set_DX8_Texture_Stage_State(
          uv_array_index, D3DTSS_TEXTURETRANSFORMFLAGS, D3DTTFF_COUNT2);
      break;
    case PERSPECTIVE_PROJECTION:
      ...
      */
      ...
      DX8Wrapper::Set_DX8_Texture_Stage_State(
          uv_array_index, D3DTSS_TEXCOORDINDEX, D3DTSS_TCI_CAMERASPACEPOSITION);
      DX8Wrapper::Set_DX8_Texture_Stage_State(
          uv_array_index, D3DTSS_TEXTURETRANSFORMFLAGS,
    ...
    case NORMAL_GRADIENT:
      ...
      */
      m[0].Set(0, 0, 0, GradientUCoord);
      m[1].Set(ViewSpaceProjectionNormal.X, ViewSpaceProjectionNormal.Y,
               ViewSpaceProjectionNormal.Z, 0);
      DX8Wrapper::Set_Transform((D3DTRANSFORMSTATETYPE)(D3DTS_TEXTURE0 + Stage),
                                m);
      DX8Wrapper::Set_DX8_Texture_Stage_State(
          uv_array_index, D3DTSS_TEXCOORDINDEX, D3DTSS_TCI_CAMERASPACENORMAL);
      DX8Wrapper::Set_DX8_Texture_Stage_State(
          uv_array_index, D3DTSS_TEXTURETRANSFORMFLAGS, D3DTTFF_COUNT2);
      break;
  }
