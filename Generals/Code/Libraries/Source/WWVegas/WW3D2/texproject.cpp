 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * TexProjectClass::TexProjectClass -- Constructor *
 ...
 ** TexProjectClass::Is_Attenuation_Enabled -- Get the state of the ATTENUATE
 *flag            * TexProjectClass::Is_Depth_Gradient_Enabled -- returns
 *whether the depth gradient is enabl * TexProjectClass::Enable_Depth_Gradient
 *-- enable/disable depth gradient                   *
 *   TexProjectClass::Init_Multiplicative -- Initialize this to be a
 *multiplicative texture pr * TexProjectClass::Is_Intensity_Zero -- check if we
 *can eliminate this projector            * TexProjectClass::Init_Additive --
 *Set up the projector to be additive                     *
 *   TexProjectClass::Set_Perspective_Projection -- Set up a perspective
 *projection            * TexProjectClass::Set_Ortho_Projection -- Set up an
 *orthographic projection                * TexProjectClass::Set_Texture -- Set
 ...
 *   TexProjectClass::Compute_Perspective_Projection -- Set up a perspective
 *projection of an  * TexProjectClass::Compute_Perspective_Projection -- Set up
 *a perspective projection of an  * TexProjectClass::Compute_Ortho_Projection --
 *Automatic Orthographic projection            *
 *   TexProjectClass::Compute_Ortho_Projection -- Automatic Orthographic
 *projection            * TexProjectClass::Compute_Texture -- Generates texture
 *by rendering an object              * TexProjectClass::Configure_Camera --
 *setup a camera to match this projector               *
 *   TexProjectClass::Pre_Render_Update -- Prepare the projector for rendering *
 *   TexProjectClass::Update_WS_Bounding_Volume -- Recalculate the world-space
 *bounding box    * TexProjectClass::Get_Surface -- Returns pointer to the
 *texture surface                    * TexProjectClass::Set_Render_Target --
 *Install a render target for this projector to use   *
 *   TexProjectClass::Needs_Render_Target -- returns wheter this projector needs
 *a render targ *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
** ---------------------------------------------------------------------------------
**
** Shadow mapping: from pre-projected view space to world space, to shadow
*space,
** then projecting.
**
...
**                                                            -1             -1
** (4) Vshadow = PShadow * Mwrld-shadow * Mobj-wrld * Mobj-wrld * Mwrld-camera *
*Vview
**                                                   -1
...
** - Should be able to handle lots of pre-calculated static texture projectors.
*They
**   should cull well and we can pre-generate the textures.
**
** Ideas maybe used in conjunction with texture projections:
** - Light volumes: the problem is when the volume is cliped it looks funny, we
*can
**   fill in the clipped face of a non-translucent object by rendering the
*backfaces
** - Use the backface-fill to make the camera slicing through the commando look
*like
 ...
 * TexProjectClass::Set_Texture_Size -- Set the size of texture to use *
 *                                                                                             *
 *    This function stores the desired texture size in this TexProjectClass.
 *Note that        * this size is only used if you have the TexProjectClass
 *generate a texture.               *
 *                                                                                             *
 * INPUT: * size - dimension of the texture in pixels *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
 ...
 * TexProjectClass::Get_Texture_Size -- Returns the stored texture size *
 *                                                                                             *
 *    Returns the size stored in the flags variable.  This can be different than
 *the actual    * texture's size if you manually installed a texture.  This size
 *is only used when         * automatically generating a texture as is the case
 ...
 * TexProjectClass::Set_Intensity -- Set the intensity of this projector *
 *                                                                                             *
 *    The "intensity" is a value between 0 and 1.  At 0, the projector will have
 *no effect     * At 1, the projector will be at its highest strength.  The
 *intensity will automatically   * smoothly interpolate towards the value
 *specified unless you use the immediate option.    *
 *                                                                                             *
 * INPUT: * intensity - desired intensity, 0 <= intensity <= 1 * immediate -
 *should the intensity be immediately set to the desired value? (default =
 *false) *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 ...
 * TexProjectClass::Get_Intensity -- returns the current "desired" intensity *
 *                                                                                             *
 *    This will return the last value that was sent into this object through
 *Set_Intensity.    * Note however that the actual intensity used in rendering
 *may not have arrived at         * that value yet. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
...
/***********************************************************************************************
 * TexProjectClass::Is_Intensity_Zero -- check if we can eliminate this
 *projector              *
 *                                                                                             *
 *    Only returns true if the current intensity is zero AND the target
 *intensity is zero      *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * TexProjectClass::Set_Attenuation -- Set the attenuation factor *
 *                                                                                             *
 *    Attenuation scales the intensity.  I use attenuation to make shadows fade
 *away as they   * get farther away from the light source or from the viewer. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
...
/***********************************************************************************************
 * TexProjectClass::Is_Depth_Gradient_Enabled -- returns whether the depth
 *gradient is enabled *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * TexProjectClass::Init_Multiplicative -- Initialize this to be a
 *multiplicative texture proj *
 *                                                                                             *
 *    Set up the internal materials so that the texture is multiplied with
 *whatever it is      * projected onto. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
...
void TexProjectClass::Init_Multiplicative(void) {
  ...
  ** Set up the shader
  */
  static ShaderClass mult_shader(
      SHADE_CNST(ShaderClass::PASS_LEQUAL,                 // depth_compare,
  ...
  if (Get_Flag(USE_DEPTH_GRADIENT)) {
    ...
    */
    TextureClass *grad_tex = WW3DAssetManager::Get_Instance()->Get_Texture(
  ...
  */
  ...
  vmtl->Set_Lighting(
      true);  // I need the emissive value to scale the intensity of the shadow
...
void TexProjectClass::Init_Additive(void) {
  ...
  */
  ...
  /*
  ** Additive projectors always use the normal gradient so they need
  *multi-texturing
  */
  ...
  */
  TextureClass *grad_tex =
      WW3DAssetManager::Get_Instance()->Get_Texture("AddProjectorGradient.tga");
  if (grad_tex) {
    grad_tex->Set_U_Addr_Mode(TextureClass::TEXTURE_ADDRESS_CLAMP);
    grad_tex->Set_V_Addr_Mode(TextureClass::TEXTURE_ADDRESS_CLAMP);
    MaterialPass->Set_Texture(grad_tex, 1);
  ...
  */
  ...
  vmtl->Set_Opacity(1.0f);
  vmtl->Set_Lighting(
      true);  // need emissive to scale the intensity of the projector
 ...
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * the pointer is ref-counted!  make sure to keep track of your
 *references                     *
 *                                                                                             *
 * HISTORY: * 1/11/00    gth : Created. *
 *=============================================================================================*/
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 1/27/00    gth : Created. *
 *=============================================================================================*/
void TexProjectClass::Set_Perspective_Projection(float hfov, float vfov,
 ...
 * HISTORY: * 1/27/00    gth : Created. *
 *=============================================================================================*/
void TexProjectClass::Set_Ortho_Projection(float xmin, float xmax, float ymin,
                                           float ymax, float znear,
                                           float zfar) {
  XMin = xmin;
  XMax = xmax;
  YMin = ymin;
 ...
 *projection of an ob *
 *                                                                                             *
 * This function automates the process of generating the perspective parameters
 *needed to      * tightly bound the projection of an object. *
 *                                                                                             *
 * INPUT: * model - object which we are created a projection of * lightpos -
 *positional light source * znear - distance to near clipping plane for the
 *projection (if -1.0, will be generated)     * zfar - distance to far clipping
 *plane for the projection (if -1.0, will be generated)       *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
 ...
 * HISTORY: * 1/11/00    gth : Created. *
 *=============================================================================================*/
bool TexProjectClass::Compute_Perspective_Projection(RenderObjClass *model,
 ...
 *needed to      * tightly bound the projection of an object. *
 *                                                                                             *
 * INPUT: * obj_box - object space bounding box of the object we are projecting
 ** tm - transform of the object we are projecting * lightpos - positional light
 *source                                                          * user_znear -
 *distance to near clipping plane for the projection (if -1.0, will be
 *generated)* user_zfar - distance to far clipping plane for the projection (if
 *-1.0, will be generated)  *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
...
bool TexProjectClass::Compute_Perspective_Projection(const AABoxClass &obj_box,
  ...
  */
  ...
  /*
  ** Calculate the axis-aligned bounding box of the model in the camera's
  *coordinate system.
  */
  ...
  */
  ...
  /*
  ** Compute the frustum parameters. Remember that our z coordinates are
  *negative but the
  ** projection code needs positive z distances.
  */
  float znear =
      -box.Center.Z;  //-(box.Center.Z + obj_box.Extent.Quick_Length());
 ...
 *                                                                                             *
 * INPUT: * model - object which we are created a projection of * lightdir -
 *directional light source * znear - distance to near clipping plane for the
 *projection (if -1.0, will be generated)     * zfar - distance to far clipping
 *plane for the projection (if -1.0, will be generated)       *
 *                                                                                             *
 * OUTPUT: *
 ...
 * HISTORY: * 1/11/00    gth : Created. *
 *=============================================================================================*/
bool TexProjectClass::Compute_Ortho_Projection(RenderObjClass *model,
 ...
 *                                                                                             *
 * INPUT: * obj_box - object space bounding box of the object we are projecting
 ** tm - transform of the object we are projecting * lightdir - directional
 *light                                                                *
 * user_znear - distance to near clipping plane for the projection (if -1.0,
 *will be generated)* user_zfar - distance to far clipping plane for the
 *projection (if -1.0, will be generated)  *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
...
bool TexProjectClass::Compute_Ortho_Projection(const AABoxClass &obj_box,
  ...
  */
  ...
  Vector3 camera_position =
      camera_target - 2.0f * wrldbox.Extent.Length() * lightdir;
  ...
  ** All done!
  */
  Set_Ortho_Projection(box.Center.X - box.Extent.X, box.Center.X + box.Extent.X,
                       box.Center.Y - box.Extent.Y, box.Center.Y + box.Extent.Y,
                       znear, zfar);
  Set_Transform(texture_tm);
  return true;
}
 ...
 **
 *                                                                                             *
 * INPUT: * model - pointer to the render object to generate a shadow texture
 *for                       * context - shadow render context which has been
 *initialized to <TextureSize> resolution      *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
...
/***********************************************************************************************
 * TexProjectClass::Needs_Render_Target -- returns wheter this projector needs a
 *render target *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * TexProjectClass::Set_Render_Target -- Install a render target for this
 *projector to use     *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * TexProjectClass::Peek_Render_Target -- Returns pointer to the render target
 *if we have one  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void TexProjectClass::Pre_Render_Update(const Matrix3D &camera) {
  ...
  */
  ...
  if (Get_Flag(ADDITIVE)) {
    ...
    vmat->Set_Emissive(1.0f - actual_intensity, 1.0f - actual_intensity,
                       1.0f - actual_intensity);
  }
