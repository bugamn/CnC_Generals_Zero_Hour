...
void Bitmap_Assert(bool condition) { WWASSERT(condition); }
...
void BitmapHandlerClass::Copy_Image_Generate_Mipmap(
    unsigned width, unsigned height, unsigned char* dest_surface,
    unsigned dest_pitch, WW3DFormat dest_format, unsigned char* src_surface,
    unsigned src_pitch, WW3DFormat src_format, unsigned char* mip_surface,
    unsigned mip_pitch) {
  ...
  if (src_format == dest_format && src_format == WW3D_FORMAT_A8R8G8B8) {
    ...
    for (unsigned y = 0; y < height / 2; ++y) {
      ...
      for (unsigned x = 0; x < width / 2; x++) {
        ...
        *mip_ptr++ = Combine_A8R8G8B8(b8g8r8a8_00, b8g8r8a8_01, b8g8r8a8_10,
                                      b8g8r8a8_11);
      }
    }
    return;
  ...
  WWASSERT(src_format !=
           WW3D_FORMAT_P8);  // This function doesn't support paletted formats
  ...
  for (unsigned y = 0; y < height / 2; ++y) {
    ...
    unsigned b8g8r8a8_10;
    unsigned b8g8r8a8_11;
    for (unsigned x = 0; x < width / 2; x++, dest_ptr += dest_bpp * 2,
                  src_ptr += src_bpp * 2, mip_ptr += dest_bpp) {
      ...
      Read_B8G8R8A8(b8g8r8a8_11, src_ptr + src_bpp + src_pitch, src_format,
                    NULL, 0);
      Write_B8G8R8A8(dest_ptr + dest_bpp + dest_pitch, dest_format,
                     b8g8r8a8_11);
...
void BitmapHandlerClass::Copy_Image(
    unsigned char* dest_surface, unsigned dest_surface_width,
    unsigned dest_surface_height, unsigned dest_surface_pitch,
    WW3DFormat dest_surface_format, unsigned char* src_surface,
    unsigned src_surface_width, unsigned src_surface_height,
    unsigned src_surface_pitch, WW3DFormat src_surface_format,
    const unsigned char* src_palette, unsigned src_palette_bpp,
  ...
  if (dest_surface_format == WW3D_FORMAT_U8V8 ||
    ...
    for (unsigned y = 0; y < dest_surface_height; y++) {
      ...
      for (unsigned x = 0; x < dest_surface_width; x++) {
        ...
        Read_B8G8R8A8(pixel01, src_ptr_mid + src_bpp, src_surface_format, NULL,
                      0);
        Read_B8G8R8A8(pixelM1, src_ptr_mid - src_bpp, src_surface_format, NULL,
                      0);
  ...
  if (src_surface_format == dest_surface_format &&
      (src_surface_format == WW3D_FORMAT_A8R8G8B8 ||
       src_surface_format == WW3D_FORMAT_X8R8G8B8)) {
    // One-to-one copy or scaling?
    dest_surface_pitch /= 4;
    src_surface_pitch /= 4;
    if (dest_surface_width == src_surface_width &&
          ...
          *(unsigned*)dest_surface = *(unsigned*)src_surface;
          ...
          for (unsigned y = 0; y < dest_surface_height / 2; ++y) {
            ...
            for (unsigned x = 0; x < dest_surface_width / 2; x++) {
              ...
              *mip_ptr++ = Combine_A8R8G8B8(b8g8r8a8_00, b8g8r8a8_01,
                                            b8g8r8a8_10, b8g8r8a8_11);
            }
          }
        }
      } else {
  ...
  if (dest_surface_width == src_surface_width &&
    ...
    // Generate the next mip level while copying the current surface?
    if (generate_mip_level) {
      WWASSERT(src_surface_format !=
               WW3D_FORMAT_P8);  // Paletted textures can't be mipmapped
      if (dest_surface_width == 1) {
        unsigned char* dest_ptr = dest_surface;
        unsigned char* src_ptr = src_surface;
        unsigned b8g8r8a8;
        Read_B8G8R8A8(b8g8r8a8, src_ptr, src_surface_format, src_palette,
                      src_palette_bpp);
        ...
        for (unsigned y = 0; y < dest_surface_height / 2; ++y) {
          ...
          unsigned b8g8r8a8_11;
          for (unsigned x = 0; x < dest_surface_width / 2; x++,
                        dest_ptr += dest_bpp * 2, src_ptr += src_bpp * 2,
                        mip_ptr += src_bpp) {
            Read_B8G8R8A8(b8g8r8a8_00, src_ptr, src_surface_format, src_palette,
                          src_palette_bpp);
            Write_B8G8R8A8(dest_ptr, dest_surface_format, b8g8r8a8_00);

            Read_B8G8R8A8(b8g8r8a8_01, src_ptr + src_bpp, src_surface_format,
                          src_palette, src_palette_bpp);
            Write_B8G8R8A8(dest_ptr + dest_bpp, dest_surface_format,
                           b8g8r8a8_01);

            Read_B8G8R8A8(b8g8r8a8_10, src_ptr + src_surface_pitch,
                          src_surface_format, src_palette, src_palette_bpp);
            Write_B8G8R8A8(dest_ptr + dest_surface_pitch, dest_surface_format,
                           b8g8r8a8_10);

            Read_B8G8R8A8(b8g8r8a8_11, src_ptr + src_bpp + src_surface_pitch,
                          src_surface_format, src_palette, src_palette_bpp);
            Write_B8G8R8A8(dest_ptr + dest_bpp + dest_surface_pitch,
                           dest_surface_format, b8g8r8a8_11);

            unsigned b8g8r8a8 = Combine_A8R8G8B8(b8g8r8a8_00, b8g8r8a8_01,
                                                 b8g8r8a8_10, b8g8r8a8_11);
      ...
      for (unsigned y = 0; y < dest_surface_height; ++y) {
        ...
        for (unsigned x = 0; x < dest_surface_width;
             ++x, dest_ptr += dest_bpp, src_ptr += src_bpp) {
          Copy_Pixel(dest_ptr, dest_surface_format, src_ptr, src_surface_format,
                     src_palette, src_palette_bpp);
        }
      }
    }
    ...
    for (unsigned y = 0; y < dest_surface_height; ++y) {
      ...
      for (unsigned x = 0; x < dest_surface_width; ++x, dest_ptr += dest_bpp) {
        unsigned src_x = x * src_surface_width / dest_surface_width;
        src_x *= src_bpp;
        Copy_Pixel(dest_ptr, dest_surface_format, src_ptr + src_x,
                   src_surface_format, src_palette, src_palette_bpp);
      }
    }
  }
