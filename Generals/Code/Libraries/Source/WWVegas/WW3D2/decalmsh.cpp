 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * DecalMeshClass::DecalMeshClass -- Constructor *
 ...
 *   RigidDecalMeshClass::Process_Material_Run -- scans the mesh for material
 *runs             * RigidDecalMeshClass::Create_Decal -- Generate a new decal *
 ...
 *   SkinDecalMeshClass::Process_Material_Run -- scans the mesh for material
 *runs              *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
DecalMeshClass::DecalMeshClass(MeshClass* parent, DecalSystemClass* system)
    : Parent(parent), DecalSystem(system) {
...
RigidDecalMeshClass::~RigidDecalMeshClass(void) {
  ...
  // Release all of our references.  The memory in the arrays will automatically
  // be released by the SimpleDynVecClass...
...
void RigidDecalMeshClass::Render(void) {
  ...
  /*
  ** Install the mesh'es transform.  NOTE, this could go wrong if someone
  *changes the
  ** transform between the time that the mesh is rendered and the time that the
  *decal
  ...
  */
  DynamicVBAccessClass dynamic_vb(BUFFER_TYPE_DYNAMIC_DX8, dynamic_fvf_type,
                                  Verts.Count());
  {
 ...
 **
 *                                                                                             *
 *    This function will install the materials for poly[start_index] and scan
 *forward for      * the next material change.  It will return the start index
 *for the next material change   *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 ...
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * All Decals on a mesh must be generated from the same
 *DecalSystemClass!                      *
 *                                                                                             *
 * HISTORY: * 1/26/00    gth : Created. *
 *=============================================================================================*/
bool RigidDecalMeshClass::Create_Decal(
    DecalGeneratorClass* generator, const OBBoxClass& localbox,
    SimpleDynVecClass<uint32>& apt,
    const DynamicVectorClass<Vector3>* world_vertex_locs) {
  // Since we can't rely on the hardware polygon offset function, I'm physically
  // offsetting the decal polygons along the normal of the decal generator.  If
  // we could instead rely on hardware "polygon offset" we could remove this
  // code and we could make decals non-sorting
#if 0
  ...
  */
  ...
  newdecal.FaceStartIndex =
      Polys.Count();       // start faces at the end of the current array
  newdecal.FaceCount = 0;  // init facecount to zero
  newdecal.VertexStartIndex =
      Verts.Count();         // start vertices at the end of the current array
  ...
  */
  ...
  Matrix3::Rotate_Vector(localbox.Basis, Vector3(localbox.Extent.X, 0, 0),
                         &extent);
  ...
  Matrix3::Rotate_Vector(localbox.Basis, Vector3(0, localbox.Extent.Y, 0),
                         &extent);
    ...
    */
    ...
    if (dot > generator->Get_Backface_Threshhold()) {
      ...
      */
      ...
      for (j = 0; j < 3; j++) {
        _DecalPoly0.Add_Vertex(src_verts[poly[j]] /*+ zbias_offset*/,
                               src_vnorms[poly[j]]);
      }
      ...
      if (_DecalPoly0.Verts.Count() >= 3) {
        ...
        */
        ...
        for (j = 1; j < _DecalPoly0.Verts.Count() - 1; j++) {
          /*
          ** Check if this triangle is degenerate (Sutherland-Hodgeman can
          *sometimes create degenerate tris)
          */
          // TODO
          ...
          */
          ...
          Textures.Add(
              material->Get_Texture());  // Get_Texture gives us a reference...
        }
        ...
        */
        for (j = 0; j < _DecalPoly0.Verts.Count(); j++) {
          ...
          VertexMaterials.Add(
              material->Get_Material());  // Get_Material gives us a ref.
...
bool RigidDecalMeshClass::Delete_Decal(uint32 id) {
  ...
  */
  for (int poly_index = 0; poly_index < Polys.Count(); poly_index++) {
    if (Polys[poly_index].I > decal->VertexStartIndex)
      Polys[poly_index].I -= decal->VertexCount;
    if (Polys[poly_index].J > decal->VertexStartIndex)
      Polys[poly_index].J -= decal->VertexCount;
    if (Polys[poly_index].K > decal->VertexStartIndex)
      Polys[poly_index].K -= decal->VertexCount;
  }
...
void SkinDecalMeshClass::Render(void) {
  ...
  */
  ...
  /*
  ** Skin decals have to get the deformed vertices of their parent meshes.  For
  *this
  ...
  */
  ...
  Parent->Get_Deformed_Vertices(&(_TempVertexBuffer[0]),
                                &(_TempNormalBuffer[0]));
 ...
 * HISTORY: * 1/31/00    NH : Created. *
 *=============================================================================================*/
bool SkinDecalMeshClass::Create_Decal(
    DecalGeneratorClass* generator, const OBBoxClass& localbox,
    SimpleDynVecClass<uint32>& apt,
    const DynamicVectorClass<Vector3>* world_vertex_locs) {
  int i;
  ...
  // The dynamically updated vertex locations are needed - we have no static
  // geometry
  ...
  */
  ...
  newdecal.VertexStartIndex =
      ParentVertexIndices
          .Count();          // start vertices at the end of the current array
  newdecal.VertexCount = 0;  // init vertcount to zero
  ...
  */
  ...
  /*
  ** Set up the generator for the world coordinate system (the deformed vertices
  *are in worldspace)
  */
  ...
  */
  ...
  for (i = 0; i < apt.Count(); i++) {
    ...
    Textures.Add(material->Get_Texture(),
                 face_size_hint);  // Get_Texture gives us a reference...
  }
  ...
  */
  ...
  for (i = 0; i < apt.Count(); i++) {
    ...
    for (int vi = 0; vi < 3; vi++) {
      ...
      */
      ...
      VertexMaterials.Add(material->Get_Material(),
                          vertex_size_hint);  // Get_Material gives us a ref.

      /*
      ** Compute the uv coordinates for this vertex
      */
      Vector3 stq;
      generator->Compute_Texture_Coordinate(
          (*world_vertex_locs)[ParentVertexIndices[ParentVertexIndices.Count() -
                                                   1]],
          &stq);
      TexCoords.Add(Vector2(stq.X, stq.Y));
    }
  ...
  newdecal.FaceCount = Polys.Count() - newdecal.FaceStartIndex;
  newdecal.VertexCount =
      ParentVertexIndices.Count() - newdecal.VertexStartIndex;
  Decals.Add(newdecal);
