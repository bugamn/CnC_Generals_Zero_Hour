...
TimeCodedMorphKeysClass::TimeCodedMorphKeysClass(void) : CachedIdx(0) {}

TimeCodedMorphKeysClass::~TimeCodedMorphKeysClass(void) { Free(); }
...
void TimeCodedMorphKeysClass::Get_Morph_Info(float morph_frame,
                                             int *pose_frame0, int *pose_frame1,
                                             float *fraction) {
  if (morph_frame < 0.0f) {
  ...
  *fraction = (morph_frame - Keys[key_index].MorphFrame) /
              (Keys[key_index + 1].MorphFrame - Keys[key_index].MorphFrame);
}
...
HMorphAnimClass::~HMorphAnimClass(void) { Free(); }
...
int Build_List_From_String(const char *buffer, const char *delimiter,
  ...
  if (success) {
    ...
    if (ChannelCount > 0) {
      ...
      for (int index = 0; index < ChannelCount; index++) {
        ...
        channel_anim_name.Format("%s.%s", HierarchyName,
                                 (const char *)column_list[index + 1]);
        PoseData[index] =
            WW3DAssetManager::Get_Instance()->Get_HAnim(channel_anim_name);
      }
...
void HMorphAnimClass::Free_Morph(void) { Free(); }
...
int HMorphAnimClass::Create_New_Morph(const int channels, HAnimClass *anim[]) {
  ...
  PoseData = W3DNEWARRAY HAnimClass *[ChannelCount];
  for (int i = 0; i < ChannelCount; i++) PoseData[i] = anim[i];

  // Create a timecodekey array for each channel and initialize the pivot
  // channels
  MorphKeyData = W3DNEWARRAY TimeCodedMorphKeysClass[ChannelCount];
  PivotChannel = W3DNEWARRAY uint32[NumNodes];
...
void HMorphAnimClass::read_channel(ChunkLoadClass &cload, int channel) {
  ...
  cload.Read(anim_name.Get_Buffer(cload.Cur_Chunk_Length()),
...
void HMorphAnimClass::Get_Translation(Vector3 &trans, int pividx,
                                      float frame) const {
  int channel = PivotChannel[pividx];
  int pose_frame0, pose_frame1;
  float fraction;
  MorphKeyData[channel].Get_Morph_Info(frame, &pose_frame0, &pose_frame1,
                                       &fraction);
...
void HMorphAnimClass::Get_Orientation(Quaternion &q, int pividx,
                                      float frame) const {
  int channel = PivotChannel[pividx];
  int pose_frame0, pose_frame1;
  float fraction;
...
void HMorphAnimClass::Get_Transform(Matrix3D &mtx, int pividx,
                                    float frame) const {
  int channel = PivotChannel[pividx];
  int pose_frame0, pose_frame1;
  float fraction;
...
void HMorphAnimClass::Insert_Morph_Key(const int channel, uint32 morph_frame,
                                       uint32 pose_frame) {
...
void HMorphAnimClass::Release_Keys(void) {
  for (int i = 0; i < ChannelCount; i++) MorphKeyData[i].Free();
