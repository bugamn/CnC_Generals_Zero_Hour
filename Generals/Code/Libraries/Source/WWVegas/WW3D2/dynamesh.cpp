...
void DynamicMeshModel::Compute_Plane_Equations(void) {
  ...
  // Set the poly and vertex counts to the dynamic counts, call the base class
  // function, then set them back.
...
void DynamicMeshModel::Render(RenderInfoClass &rinfo) {
  ...
  /*
  ** Write the vertex data to the vertex buffer. We assume the FVF contains
  *positions, normals,
  ** one texture channel, and the diffuse color channel (color0). If it does not
  *contain all
  ** these components, the code will fail.
  */
  DynamicVBAccessClass dynamic_vb(buffer_type, dynamic_fvf_type,
                                  DynamicMeshVNum);
  ...
  {  // scope for lock
    ...
    unsigned char *vertices =
        (unsigned char *)lock.Get_Formatted_Vertex_Array();
    ...
    for (int i = 0; i < DynamicMeshVNum; i++) {
      ...
      *(Vector3 *)(vertices + fvf_info.Get_Normal_Offset()) = normals[i];
      ...
      if (colors) {
        ...
        *(unsigned int *)(vertices + fvf_info.Get_Diffuse_Offset()) =
            default_color;
      }
  ...
  */
  ...
  for (unsigned int pass = 0; pass < pass_count; pass++) {
    /*
    ** Set current render states (texture, vertex material, shader). Scan
    *triangles until one
    ...
    */
    ...
    // If no texture, shader or material arrays for this pass just draw and go
    // to next pass
    if (!texture_array && !material_array && !shader_array) {
      if (buffer_type == BUFFER_TYPE_DYNAMIC_SORTING) {
        SortingRendererClass::Insert_Triangles(sphere, 0, DynamicMeshPNum, 0,
                                               DynamicMeshVNum);
      } else {
    ...
    while (!done) {
      ...
      if (done) {
        ...
        texture_changed = texture_array && texture_array[cur_tri_idx] !=
                                               texture_array[next_tri_idx];
        material_changed =
            material_array && material_array[tris[cur_tri_idx].I] !=
                                  material_array[tris[next_tri_idx].I];
        shader_changed = shader_array && shader_array[cur_tri_idx] !=
                                             shader_array[next_tri_idx];
      }

      // If run ends (mesh ends or state changes) draw, reset indices, set state
      // for next run.
      if (done || texture_changed || material_changed || shader_changed) {
        ...
        max_vert_idx = 0;
        if (texture_changed)
          DX8Wrapper::Set_Texture(0, texture_array[next_tri_idx]);
        if (material_changed)
...
void DynamicMeshModel::Initialize_Texture_Array(int pass, int stage,
...
void DynamicMeshClass::Render(RenderInfoClass &rinfo) {
  ...
  if (WW3D::Are_Static_Sort_Lists_Enabled() && SortLevel != SORT_LEVEL_NONE) {
    ...
    if (CollisionMath::Overlap_Test(frustum, Get_Bounding_Box()) !=
...
bool DynamicMeshClass::End_Vertex() {
  ...
  while (pass--) {
    if (MultiVertexMaterial[pass]) {
      VertexMaterialClass *mat =
          Peek_Material_Info()->Get_Vertex_Material(VertexMaterialIdx[pass]);
  ...
  for (int color_array_index = 0; color_array_index < MAX_COLOR_ARRAYS;
       color_array_index++) {
    if (MultiVertexColor[color_array_index]) {
      //			Vector4 * color =
      //&((Model->Get_Color_Array(color_array_index))[VertCount]); 			color->X =
      //CurVertexColor[color_array_index].X; 			color->Y =
      //CurVertexColor[color_array_index].Y; 			color->Z =
      //CurVertexColor[color_array_index].Z; 			color->W =
      //CurVertexColor[color_array_index].W;
      unsigned *color =
          &((Model->Get_Color_Array(color_array_index))[VertCount]);
      *color =
          DX8Wrapper::Convert_Color_Clamp(CurVertexColor[color_array_index]);
    }
  }
...
DynamicMeshClass::DynamicMeshClass(const DynamicMeshClass &src)
  ...
  for (int color_array_index = 0; color_array_index < MAX_COLOR_ARRAYS;
       color_array_index++) {
    MultiVertexColor[color_array_index] =
...
DynamicMeshClass::~DynamicMeshClass() { REF_PTR_RELEASE(Model); }
...
int DynamicMeshClass::Set_Vertex_Material(VertexMaterialClass *material,
                                          bool dont_search, int pass) {
  // Check if same as the last vertex material
  if (Peek_Material_Info()->Vertex_Material_Count() &&
      (VertexMaterialIdx[pass] != -1) &&
      Peek_Material_Info()->Peek_Vertex_Material(VertexMaterialIdx[pass]) ==
          material) {
    return VertexMaterialIdx[pass];
  }
  ...
  if ((!MultiVertexMaterial[pass]) &&
      Peek_Material_Info()->Vertex_Material_Count() &&
      (VertexMaterialIdx[pass] != -1) &&
      Peek_Material_Info()->Peek_Vertex_Material(VertexMaterialIdx[pass]) !=
          material) {
  ...
  // it.
  if (!dont_search) {
    for (int lp = 0, found = 0;
         lp < Peek_Material_Info()->Vertex_Material_Count(); lp++) {
    ...
    if (!found) {
      ...
      VertexMaterialIdx[pass] =
          Peek_Material_Info()->Vertex_Material_Count() - 1;
    }
  } else {
    Peek_Material_Info()->Add_Vertex_Material(material);
...
int DynamicMeshClass::Set_Texture(TextureClass *texture, bool dont_search,
                                  int pass) {
  // Check if same as the last texture
  if (Peek_Material_Info()->Texture_Count() && (TextureIdx[pass] != -1) &&
      Peek_Material_Info()->Peek_Texture(TextureIdx[pass]) == texture) {
    return TextureIdx[pass];
  }

  // if there are textures in the list then we may have just jumped
  // to becoming a multi-texture object.  Take care of that here.
  if ((!MultiTexture[pass]) && Peek_Material_Info()->Texture_Count() &&
      (TextureIdx[pass] != -1) &&
      Peek_Material_Info()->Peek_Texture(TextureIdx[pass]) != texture) {
  ...
  // it.
  if (!dont_search) {
    for (int lp = 0, found = 0; lp < Peek_Material_Info()->Texture_Count();
         lp++) {
...
void DynamicScreenMeshClass::Move_Vertex(int index, float x, float y, float z) {
  DynamicMeshClass::Move_Vertex(index, (x * 2) - 1, Aspect - (y * 2 * Aspect),
                                0);
}
