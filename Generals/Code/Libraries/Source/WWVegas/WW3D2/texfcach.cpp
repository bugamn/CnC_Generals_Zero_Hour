 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * TextureFileCache::TextureFileCache -- Open cache.
 ...
 ** TextureFileCache::Close_Texture_Handle -- Close the current texture so we
 *can open anoth* TextureFileCache::Read_Texture -- Read in the texture into
 *surface buffer.              *
 *   *TextureFileCache::Create_First_Texture_As_Surface -- Load first texture
 *into a surface.  * *TextureFileCache::Find_Cached_Surface -- Search for a
 *texture already cached.            * TextureFileCache::Add_Cached_Surface --
 *Add a new cached texture.                         * *TFC::Get_Surface -- Load
 *a texture reduced N times.                                      *
 *   TextureFileCache::Reset_File -- virtual function to reset file and write
 *out file.        *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
#ifdef WW3D_DX8
...
static void Verify_Compression_Buffer() {
  WWASSERT(compression_buffer[compression_buffer.Length() - 1] ==
           BUFFER_OVERRUN_TEST_VALUE);
}

class Compressor {
 public:
  static int Compress(const unsigned char *in, unsigned int in_len,
                      unsigned char *out, unsigned int *out_len);

  static int Decompress(const unsigned char *in, unsigned int in_len,
                        unsigned char *out, unsigned int *out_len);
};
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 05/07/1999 SKB : Created. * 06/27/2000 SKB : added CachedSurfaces
...
/***********************************************************************************************
 * *TextureFileCache::_Create_File_Name -- Create a file name from prefix passed
 *in.           *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * Caller of this function must free _FileNamePtr when done with it.
 **
 *                                                                                             *
 * HISTORY: * 05/13/1999 SKB : Created. *
 *=============================================================================================*/
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 05/07/1999 SKB : Created. *
 *=============================================================================================*/
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 05/12/1999 SKB : Created. * 06/27/2000 SKB : added
 *Header.PixelFormat                                                 *
 *   06/27/2000 SKB : changed cached textures to surfaces * 08/14/2000     :
 *save file's datatime stamp.                                              *
 *=============================================================================================*/
bool TextureFileCache::Save_Texture(const char *texturename,
                                    srTextureIFace::MultiRequest &mreq,
  ...
  for (idx = 0, lod = mreq.largeLOD; lod <= mreq.smallLOD; idx++, lod++) {
    ...
    retcode =
        Compressor::Compress((const unsigned char *)surface->getDataPtr(),
    ...
    int readin =
        TextureHandle->Write(Get_Compression_Buffer(compsize), compsize);
 ...
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * Sureface is only needed to get default values.  The texel data
 *does not get filled in.  *
 *                                                                                             *
 * HISTORY: * 05/13/1999 SKB : Created. *
 *=============================================================================================*/
srColorSurfaceIFace *TextureFileCache::Load_Original_Texture_Surface(
    const char *texturename) {
  CriticalSectionClass::LockClass m(mutex);
  if (Open_Texture_Handle(texturename)) {
    // Create surface that we wish to return.
    srColorSurfaceIFace *surface = W3DNEW srColorSurface(
        Header.SourcePixelFormat, Header.SourceWidth, Header.SourceHeight);
    return (surface);
  }
  return (NULL);
}
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 06/26/2000 SKB : Created. *
 *=============================================================================================*/
srColorSurfaceIFace *TextureFileCache::Get_Surface(const char *texturename,
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 05/12/1999 SKB : Created. * 06/02/1999 SKB : deal with texture
 *requests larger then what is in cache.                 * 06/27/2000 SKB :
 *changed cached textures to surfaces                                      *
 *=============================================================================================*/
bool TextureFileCache::Load_Texture(const char *texturename,
                                    srTextureIFace::MultiRequest &mreq) {
  CriticalSectionClass::LockClass m(mutex);
  // If we can't get a handle, then we can't load the texture.
  if (!Open_Texture_Handle(texturename)) {
  ...
  // NOTE: Mip maps that are saved in the cache are the first mip maps requested
  // and created
  //			for a given texture name.  If a user susequently
  //requests mip maps of different
  // 		sizes that are not in the cache, they will be created by use of
  // surface copy functions.
  //			When this happens it is unfortunate because it indicates
  //some sort of inefficiency. 			In code below I have put in comments 'A WASTE' to
  //indicate what part of code deels with it.
  int idx;
  unsigned lod;
  ...
  // Now if we have in the file something that the texture needs, load it. (This
  // is most desired.)
  if (idxsize == lodsize) {
    ...
    while ((lod <= mreq.smallLOD) && (idx < Header.NumMipMaps)) {
      ...
      if (cached) {
        ...
        // Copy data over using size of dest surface incase asserts disabled and
        // sizes are not right.
        memcpy(surface->getDataPtr(), cached->getDataPtr(), size);
      } else {
        // Cache pointer so if we need texture again, we will have a pointer to
        ...
        // Read texture in - note that Textures[lod] has to be set prior to
        // this.
  ...
  if (mreq.largeLOD < firstlod) {
    ...
    // Now scale the largest mip map we had up to what user wants (ONCE AGAIN: A
    // WASTE).
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 06/01/1999 SKB : Created. * 06/27/2000 SKB : changed cached
...
void TextureFileCache::Read_Texture(int offsetidx, srColorSurface *surface) {
  ...
  retcode = Compressor::Decompress(
      (const unsigned char *)Get_Compression_Buffer(compsize),
      (unsigned int)compsize, (unsigned char *)surface->getDataPtr(),
      (unsigned *)&decompsize);
...
/***********************************************************************************************
 * *TextureFileCache::Create_First_Texture_As_Surface -- Load first texture into
 *a surface.    *
 *                                                                                             *
 * INPUT: * srColorSurfaceIFace *surfacetype - only used to create surface type
 *we want.				  *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 06/02/1999 SKB : Created. *
 *=============================================================================================*/
srColorSurfaceIFace *TextureFileCache::Create_First_Texture_As_Surface(
    srColorSurfaceIFace *surfacetype) {
...
/***********************************************************************************************
 * TextureFileCache::Close_Texture_Handle -- Close the current texture so we can
 *open another. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 06/01/1999 SKB : Created. *
 *=============================================================================================*/
...
/***********************************************************************************************
 * *TextureFileCache::Find_Cached_Surface -- Search for a texture already
 *cached.              *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
bool TextureFileCache::Validate_Texture(const char *FileName) {
  // The functions used in here are all thread safe so this function doesn't
  // have to be mutex guarded

  if (!Texture_Exists(FileName)) {
    if (!TextureLoader::Texture_File_Exists(FileName)) {
      return false;
    }
    // We need to load the surface from the file first to determine the real
    // size
