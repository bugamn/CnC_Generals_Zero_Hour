 ...
 **
 *                                                                                             *
 * Creates a TextDrawClass object by creating and initializing a Dynamic Mesh,
 *inserting it	  * into the given scene, and allocating space for the given
 *number of maximum chars.			  *
 *                                                                                             *
 ***********************************************************************************************/
TextDrawClass::TextDrawClass(int max_chars)
  ...
  Set_Coordinate_Ranges(Vector2(0, 0), Vector2(1, 1), Vector2(-1, 0.75f),
                        Vector2(1, -0.75));
  //	Set_Coordinate_Ranges( Vector2( -320,240 ), Vector2( 320,-240 ),
  //Vector2( -320,240 ), Vector2( 320,-240 ) );
}
...
**
*/
void TextDrawClass::Set_Coordinate_Ranges(const Vector2 &src_ul,
                                          const Vector2 &src_lr,
                                          const Vector2 &dest_ul,
...
**
*/
void TextDrawClass::Quad(float x0, float y0, float x1, float y1, float u0,
...
void TextDrawClass::Quad(const RectClass &rect, const RectClass &uv) {
  TextDrawClass::Quad(rect.Left, rect.Top, rect.Right, rect.Bottom, uv.Left,
                      uv.Top, uv.Right, uv.Bottom);
}
...
void TextDrawClass::Line_Ends(const Vector2 &a, const Vector2 &b, float width,
                              float end_percent) {
  Vector2 a_ = b - a;
  a_ *= end_percent;
  a_ += a;
 ...
 **
 *                                                                                             *
 * WARNING:  Should not be used to draw characters which need to wrap or have
 *embedded line    * feeds. *
 *                                                                                             *
...
float TextDrawClass::Get_Height(Font3DInstanceClass *font,
                                const char *message) {
...
/***********************************************************************************************
 *                                                                                             *
 * float	TextDrawClass::Print( Font3DInstanceClass *, char, float, float,
 *float )							  *
 *                                                                                             *
 * Draws (actually creates two trianlges to display) a character on the screen
 *at the given    * normalized screen unit coordinates at the current font
 *scale.                               *
 *                                                                                             *
 * Returns the scaled character width in normalized screen unit *
 *                                                                                             *
 ***********************************************************************************************/
float TextDrawClass::Print(Font3DInstanceClass *font, char ch, float screen_x,
                           float screen_y) {
  /*
  ...
  */
  ...
  /*
  ** If asked to draw the space char (' '), don't add any triangles, just return
  *the scaled spacing
  */
  ...
  */
  ...
  float screen_y1 =
      screen_y0 + (font->Char_Height() * WWMath::Sign(-TranslateScale.Y));
  ...
  /*
  ** Get the font texture uv coordinate for teh upper right and lower left
  *corners of the rect
  */
  ...
  ** Draw the quad
  */
  Quad(screen_x0, screen_y0, screen_x1, screen_y1, font_uv.Left, font_uv.Top,
       font_uv.Right, font_uv.Bottom);
...
/***********************************************************************************************
 *                                                                                             *
 * float	TextDrawClass::Print( Font3DInstanceClass *, char *, float,
 *float, float )						  *
 *                                                                                             *
 * Draws the given string at the given pixel coordinates.  Uses the given font
 *and its current * scale.  Passes each character to the above routine and moves
 *the x-coordinate forward after * each char. *          *
 *                                                                                             *
 * WARNING:  Should not be used to draw characters which need to wrap or have
 *embedded line    * feeds. *                *
 *                                                                                             *
 ...
 *                                                                                             *
 ***********************************************************************************************/
float TextDrawClass::Print(Font3DInstanceClass *font, const char *message,
 ...
 *                                                                                             *
 ***********************************************************************************************/
void TextDrawClass::Show_Font(Font3DInstanceClass *font, float screen_x,
                              float screen_y) {
  ...
  Quad(screen_x, screen_y, screen_x + size_x,
       screen_y + size_y * WWMath::Sign(-TranslateScale.Y));
}
