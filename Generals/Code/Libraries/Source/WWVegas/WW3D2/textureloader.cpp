...
static bool Is_Format_Compressed(WW3DFormat texture_format,
  ...
  compressed |=
      (texture_format == WW3D_FORMAT_UNKNOWN && DX8Caps::Support_DXTC() &&
       WW3D::Get_Texture_Compression_Mode() ==
           WW3D::TEXTURE_COMPRESSION_ENABLE &&
       allow_compression);
...
IDirect3DTexture8* TextureLoader::Load_Thumbnail(const StringClass& filename,
  ...
  if (texture_format == WW3D_FORMAT_UNKNOWN) {
    dest_format = Get_Valid_Texture_Format(
        WW3D_FORMAT_A8R8G8B8, false);  // no compressed formats please
  } else {
    dest_format = Get_Valid_Texture_Format(
  ...
  for (level = 0; level < d3d_texture->GetLevelCount(); ++level) {
    DX8_ErrorCode(d3d_texture->LockRect(level, &locked_rects[level], NULL, 0));
  }
...
IDirect3DTexture8* Load_Compressed_Texture(
    const StringClass& filename, unsigned reduction_factor,
  ...
  IDirect3DTexture8* d3d_texture = DX8Wrapper::_Create_DX8_Texture(
      width, height, dest_format, (TextureClass::MipCountType)mips);
...
IDirect3DSurface8* TextureLoader::Load_Surface_Immediate(
    const StringClass& filename, WW3DFormat texture_format,
    bool allow_compression) {
  bool compressed = Is_Format_Compressed(texture_format, allow_compression);

  if (compressed) {
    IDirect3DTexture8* comp_tex = Load_Compressed_Texture(
        filename, 0, TextureClass::MIP_LEVELS_1, WW3D_FORMAT_UNKNOWN);
    if (comp_tex) {
      IDirect3DSurface8* d3d_surface = NULL;
...
void TextureLoader::Load_Mipmap_Levels(TextureLoadTaskClass* task) {
  ...
  if (task->Peek_Texture()->Is_Compression_Allowed()) {
    DDSFileClass dds_file(task->Peek_Texture()->Get_Full_Path(),
                          task->Get_Reduction());
    if (dds_file.Is_Available() && dds_file.Load()) {
      ...
      for (unsigned level = 0; level < task->Get_Mip_Level_Count(); ++level) {
        ...
        dds_file.Copy_Level_To_Surface(level, task->Get_Format(), width, height,
...
bool TextureLoader::Load_Uncompressed_Mipmap_Levels_From_TGA(
  ...
  dest_format =
      task->Get_Format();  // Texture can be requested in different format than
                           // the most obvious from the TGA
  ...
  for (unsigned level = 0; level < task->Get_Mip_Level_Count(); ++level) {
    ...
    BitmapHandlerClass::Copy_Image(
        task->Get_Locked_Surface_Ptr(level), width, height,
        task->Get_Locked_Surface_Pitch(level), task->Get_Format(), src_surface,
        src_width, src_height, src_pitch, src_format, NULL, 0, true);
...
bool Is_Load_List_Empty() { return !LoadListHead; }
// ----------------------------------------------------------------------------
//
...
void TextureLoader::Update() {
  WWASSERT_PRINT(DX8Wrapper::_Get_Main_Thread_ID() ==
...
IDirect3DTexture8* TextureLoader::Generate_Bumpmap(TextureClass* texture) {
  ...
  IDirect3DTexture8* d3d_texture = DX8Wrapper::_Create_DX8_Texture(
      width, height, bump_format, TextureClass::MIP_LEVELS_1);

  D3DLOCKED_RECT src_locked_rect;
  DX8_ErrorCode(texture->Peek_DX8_Texture()->LockRect(0, &src_locked_rect, NULL,
                                                      D3DLOCK_READONLY));

  D3DLOCKED_RECT dest_locked_rect;
  DX8_ErrorCode(d3d_texture->LockRect(0, &dest_locked_rect, NULL, 0));
  ...
  for (unsigned y = 0; y < desc.Height; y++) {
    ...
    for (unsigned x = 0; x < desc.Width; x++) {
      ...
      BitmapHandlerClass::Read_B8G8R8A8(pixel01, src_ptr_mid + bpp, format,
                                        NULL, 0);
      BitmapHandlerClass::Read_B8G8R8A8(pixelM1, src_ptr_mid - bpp, format,
                                        NULL, 0);
      BitmapHandlerClass::Read_B8G8R8A8(pixel10, src_ptr_prev_line, format,
                                        NULL, 0);
      BitmapHandlerClass::Read_B8G8R8A8(pixel1M, src_ptr_next_line, format,
                                        NULL, 0);
...
void TextureLoader::Request_High_Priority_Loading(
    TextureClass* tc, TextureClass::MipCountType mip_level_count) {
...
void TextureLoadTaskClass::Begin_Texture_Load() {
  ...
  if (ThreadClass::_Get_Current_Thread_ID() ==
      DX8Wrapper::_Get_Main_Thread_ID()) {
    bool loaded = false;
    if (Texture->Is_Compression_Allowed()) {
      ...
      if (dds_file.Is_Available()) {
        ...
        // help... (mainly for cases where loaded texture is larger than
        // hardware limit)
        if (width != dds_file.Get_Width(0) ||
            height != dds_file.Get_Height(0)) {
        ...
        // (not necessarily all) Otherwise take as many mip levels as the
        // texture wants, not to exceed the count in file...
        if (!mip_level_count)
        ...
        // changed here it might not match the size...well actually it doesn't
        // have to match but it can't be bigger than the size)
        ...
        D3DTexture = DX8Wrapper::_Create_DX8_Texture(
            Width, Height, Format, (TextureClass::MipCountType)mip_level_count);
    ...
    if (!loaded) {
      ...
      if (src_format != WW3D_FORMAT_A8R8G8B8 &&
        ...
        WWDEBUG_SAY(
            ("Invalid TGA format used in %s - only 24 and 32 bit formats "
             "should be used!\n",
             Texture->Get_Full_Path()));
      }
      ...
      if (width != ow || height != oh) {
        WWDEBUG_SAY(
            ("Invalid texture size, scaling required. Texture: %s, size: %d x "
             "%d -> %d x %d\n",
...
void TextureLoadTaskClass::Begin_Thumbnail_Load() {
  ...
  if (thread_id == DX8Wrapper::_Get_Main_Thread_ID()) {
    ...
    D3DTexture =
        TextureLoader::Load_Thumbnail(Texture->Get_Full_Path(), format);
...
void TextureLoadTaskClass::Set_Succ(TextureLoadTaskClass* succ) {
  WWASSERT((succ && !Succ) ||
           (!succ));  // Can't set successor pointer if it has been set already
  Succ = succ;
}
...
// ----------------------------------------------------------------------------
//
// Apply the D3D texture surface to the client texture. The parameter
// 'initialize' determines whether the client texture will be set to initialized
// state or not. Generally thumbnail tasks will not set texture to initialised
// state but all other loads do.
//
// ----------------------------------------------------------------------------
...
TextureLoadTaskClass* TextureLoadTaskClass::Get_Instance(TextureClass* tc,
                                                         bool high_priority) {
  CriticalSectionClass::LockClass m(mutex);
