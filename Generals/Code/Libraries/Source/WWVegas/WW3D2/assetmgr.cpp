 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * WW3DAssetManager::WW3DAssetManager -- Constructor *
 *   WW3DAssetManager::~WW3DAssetManager -- Destructor * WW3DAssetManager::Free
 *-- free all memory (un-needed?)                                    *
 ...
 ** WW3DAssetManager::Render_Obj_Exists -- Check whether a render object with
 *the given name  * WW3DAssetManager::Create_Render_Obj_Iterator -- Create an
 *iterator which can enumerate al *
 *   WW3DAssetManager::Release_Render_Obj_Iterator -- release a render object
 *iterator         * WW3DAssetManager::Create_HAnim_Iterator -- Creates an HAnim
 *Iterator                      * WW3DAssetManager::Create_HTree_Iterator --
 *creates an htree iterator                      *
 *   WW3DAssetManager::Create_Material_Iterator -- Create a material iterator *
 *   WW3DAssetManager::Create_Font3DData_Iterator -- Create a Font3DData
 *iterator              * WW3DAssetManager::Get_HAnim -- Returns a pointer to a
 *names HAnim                         * WW3DAssetManager::Get_HTree -- Returns a
 *pointer to the named HTree                       *
 *   WW3DAssetManager::Get_Material -- Gets a pointer to a loaded material or
 *creates it       * WW3DAssetManager::Get_Material -- Gets a pointer to a
 *loaded material or creates the mate * WW3DAssetManager::Get_Material -- Gets a
 *pointer to a loaded material or creates it       *
 *   WW3DAssetManager::Get_Font3DInstance -- Creates a pointer to a
 *Font3DInstance             * WW3DAssetManager::Get_Font3DData -- Gets a
 *pointer to a loaded Font3DData or creates it   *
 ...
 ** WW3DAssetManager::Register_Prototype_Loader -- add a new loader to the
 *system             * WW3DAssetManager::Find_Prototype_Loader -- find the
 *loader that handles this chunk type   * WW3DAssetManager::Add_Prototype --
 *adds the prototype to the hash table                   *
 *   WW3DAssetManager::Find_Prototype -- searches the hash table for the
 *prototype             * WW3DAssetManager::Open_Texture_File_Cache -- Turn on
 *the texture cache system.            *
 *   WW3DAssetManager::Close_Texture_File_Cache -- Turn off the texture cache
 *system.          * CachedTextureFileClass::getMipmapData -- get data for
 *texture - check to see if in cache. *
 *   CachedTextureFileClass::getMipmapLevelPartial -- not yet implemented *
 *   CachedTextureFileClass::setupDefaultValues -- loads texture in to get
 *default data.       * WW3DAssetManager::Get_Streaming_Texture -- Gets a
 *streaming texture.                      *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
/*
** Static member variable which keeps track of the single instanced asset
*manager
*/
...
class HAnimIterator : public AssetIterator {
 public:
  HAnimIterator(void)
      : Iterator(WW3DAssetManager::Get_Instance()->HAnimManager) {};
...
class Font3DDataIterator : public AssetIterator {
 public:
  virtual void First(void) {
    Node = WW3DAssetManager::Get_Instance()->Font3DDatas.Head();
  }
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 12/21/97   GTH : Created. * 05/10/1999 SKB : Add TextureCache *
 *=============================================================================================*/
WW3DAssetManager::WW3DAssetManager(void)
  ...
  memset(PrototypeHashTable, 0,
         sizeof(PrototypeClass *) * PROTOTYPE_HASH_TABLE_SIZE);
}
...
static void Log_Textures(bool inited, unsigned &total_count,
                         unsigned &total_mem) {
  ...
  for (ite.First(); !ite.Is_Done(); ite.Next()) {
    ...
    WWDEBUG_SAY(
        ("%32s	%4d * %4d (%15s), init %d, size: %14s bytes, refs: %d\n",
         tex->Get_Texture_Name(), desc.Width, desc.Height, tex_format,
         tex->Is_Initialized(), number, tex->Num_Refs()));
  }
}
...
void WW3DAssetManager::Log_Texture_Statistics() {
  ...
  /*
          RenderObjIterator *
  rite=WW3DAssetManager::Get_Instance()->Create_Render_Obj_Iterator(); if (rite)
  { for (rite->First(); !rite->Is_Done(); rite->Next()) {
  //			RenderObjClass *
  robj=Create_Render_Obj(rite->Current_Item_Name());
  //			if (robj) {
  //
  //				robj->Release_Ref();
  //			}
                          if
  (rite->Current_Item_Class_ID()==RenderObjClass::CLASSID_HMODEL) {
                                  WWDEBUG_SAY(("robj:
 ...
 * HISTORY: * 12/21/97   GTH : Created. *
 *=============================================================================================*/
void WW3DAssetManager::Free(void) { Free_Assets(); }
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 12/21/97   GTH : Created. * 05/10/1999 SKB : Close down texture
 *cache file.                                           *
 *=============================================================================================*/
void WW3DAssetManager::Free_Assets(void) {
  ...
  // NONONONOO.... Don't close it as we might want to free the assets and still
  // be able to load textures.
  //	Close_Texture_File_Cache();
}

/***********************************************************************************************
 * WW3DAssetManager::Free_Unused_Assets -- Release all assets that are
 *referenced only by      * the asset manager. * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 02/18/99   EHC : Created. *
 *=============================================================================================*/
...
/***********************************************************************************************
 * WW3DAssetManager::Free_Assets_With_Exclusion_List -- Release all assets that
 *are not named  * in the exclusion list.                 *
 *                                                                                             *
 * This function checks if each prototype is named or is a child of something
 *named in the     * given exclusion list. *
 *                                                                                             *
 * INPUT: * exclusion_list - list of names of render object prototypes to not
 *release.                  *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 12/12/2002 GH  : Created. *
 *=============================================================================================*/
void WW3DAssetManager::Free_Assets_With_Exclusion_List(
    const DynamicVectorClass<StringClass> &exclusion_names) {
  ...
  // temporary vector to hold the prototypes that get excluded from being
  // deleted. grow by the initial size so we don't waste lots of time
  // re-allocating!
  const int DEFAULT_EXCLUDE_ARRAY_SIZE = 8000;
  DynamicVectorClass<PrototypeClass *> exclude_array(
      DEFAULT_EXCLUDE_ARRAY_SIZE);
  exclude_array.Set_Growth_Step(DEFAULT_EXCLUDE_ARRAY_SIZE);

  // iterate the array of prototypes saving each one that should be excluded
  // from deletion
...
/***********************************************************************************************
 * WW3DAssetManager::Create_Asset_List -- Create a list of the W3D files that
 *are loaded       *
 *                                                                                             *
 * This function checks if each prototype is named or is a child of something
 *named in the     * given exclusion list. *
 *                                                                                             *
 * INPUT: * model_list - dynamic vector to populate with names of w3d files
 *loaded                      *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * As in other places in the code, we are assuming that w3d
 *filenames match the "top-level"    * render object contained within them! *
 *                                                                                             *
 * HISTORY: * 12/12/2002 GH  : Created. *
 *=============================================================================================*/
void WW3DAssetManager::Create_Asset_List(
    DynamicVectorClass<StringClass> &model_list) {
  for (int i = 0; i < Prototypes.Count(); i++) {
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 7/29/98    GTH : Created. * 2/19/99    EHC : Now has the
 *Add_Prototype call responsible for adding the prototype to   * the Prototypes
 *list object.                                              *
 *=============================================================================================*/
 ...
 * WW3DAssetManager::Create_Render_Obj -- Create a render object for the user *
 *                                                                                             *
 *    This function will create any type of render object.  I.e. if you pass in
 *the name       * of an HModel, it will create an hmodel for you. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
...
RenderObjClass *WW3DAssetManager::Create_Render_Obj(const char *name) {
  ...
  if (WW3D_Load_On_Demand &&
      proto == NULL) {  // If we didn't find one, try to load on demand
...
/***********************************************************************************************
 * WW3DAssetManager::Render_Obj_Exists -- Check whether a render object with the
 *given name ex *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * WW3DAssetManager::Create_Render_Obj_Iterator -- Create an iterator which can
 *enumerate all  *
 *                                                                                             *
 *    The iterator returned can enumerate all of the loaded render objects for
 *you.            * The user is responsible for releasing the iterator! *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * User must release the iterator back to the asset manager or there
 ...
 *                                                                                             *
 *    Creates an iterator which can enumerate each of the named hierarchical
 *animations        * which are currently loaded. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * User must delete the iterator! *
 *                                                                                             *
 * HISTORY: * 12/21/97   GTH : Created. *
 *=============================================================================================*/
 ...
 **
 *                                                                                             *
 *   Creates an iterator which can enumerate each of the Font3DDatas currently
 *loaded          *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * The implementation has changed since its inception, the asset
 *manager no longer owns the * hanim's so they need to be released by the
 *caller.
 **
 *                                                                                             *
 * HISTORY: * 12/21/97   GTH : Created. *
 ...
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * DO NOT DELETE the HTree you get from this function, it is "owned"
...
HTreeClass *WW3DAssetManager::Get_HTree(const char *name) {
  ...
  if (WW3D_Load_On_Demand &&
      htree == NULL) {  // If we didn't find it, try to load on demand
...
/***********************************************************************************************
 * WW3DAssetManager::Get_Bumpmap_Based_On_Texture -- Generate a bumpmap from
 *texture. The      * resulting texture is stored to the hash table so that any
 *further requests will share it.   *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 1/31/2001  NH : Created. *
 *=============================================================================================*/

TextureClass *WW3DAssetManager::Get_Bumpmap_Based_On_Texture(
  ...
  */
  ...
  if (tex && texture_format != WW3D_FORMAT_UNKNOWN) {
    WWASSERT_PRINT(
        ...
        ("Texture %s has already been loaded witt different format", filename));
  }
...
void WW3DAssetManager::Release_All_Textures(void) {
  ...
  */
  ...
  for (ite.First(); !ite.Is_Done(); ite.Next()) {
    TextureClass *tex = ite.Peek_Value();
    //		WWASSERT(tex->Num_Refs()==1);	// If asset manager is releasing
    //the texture,
...
/***********************************************************************************************
 * WW3DAssetManager::Release_Unused_Textures -- release all textures with
 *refcount == 1        *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
  ...
  for (ite.First(); !ite.Is_Done(); ite.Next()) {
    ...
    if (tex->Num_Refs() == 1) {
      ...
      if (count == 256) {
        ...
        count = 0;
        ite.First();  // iterator doesn't support modifying the hash table while
                      // iterating, so start from the beginning.
      }
    }
  }
...
/***********************************************************************************************
 * WW3DAssetManager::Release_Texture -- release a specific texture from the
 *asset manager      *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void WW3DAssetManager::Release_Texture(TextureClass *tex) {
  /*
  ** Try to find the texture in the list, if found release it and remove it from
  *the list.
  */
  ...
  //	for (	SLNode<TextureClass> *node = Textures.Head(); node; node =
  //node->Next()) { 		if (node->Data() == tex) { 			Textures.Remove(tex);
...
/***********************************************************************************************
 * WW3DAssetManager::Get_Font3DData -- Gets a pointer to a loaded Font3DData or
 *creates it     *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
Font3DDataClass *WW3DAssetManager::Get_Font3DData(const char *name) {
  ...
  for (SLNode<Font3DDataClass> *node = Font3DDatas.Head(); node;
       node = node->Next()) {
...
/***********************************************************************************************
 * WW3DAssetManager::Release_All_Font3DDatas -- Release all Font3DDatas from the
 *asset manager *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * WW3DAssetManager::Release_Unused_Font3DDatas -- Release all Font3DDatas with
 *refcount == 1  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * WW3DAssetManager::Get_FontChars -- Gets a pointer to a loaded FontChars or
 *creates it       *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 6/1/01     BMG : Created. *
 *=============================================================================================*/
FontCharsClass *WW3DAssetManager::Get_FontChars(const char *name,
                                                int point_size, bool is_bold) {
  WWPROFILE("WW3DAssetManager::Get_FontChars");

  // loop through and see if we already have the font chars and we can just
  // return it.
...
/***********************************************************************************************
 * WW3DAssetManager::Release_All_FontChars -- Release all FontChars from the
 *asset manager     *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 **
 *                                                                                             *
 *    The library will automatically install loaders for the "built-in" render
 *object          * types.  This function exists so that the user can design
 *App-specific render objects,    * define a chunk format for them, and have the
 *asset manager load them in like everything  * else. *
 *                                                                                             *
 * INPUT: * loader - pointer to a global or static instance of your loader type.
 **
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * WW3DAssetManager::Find_Prototype_Loader -- find the loader that handles this
 *chunk type     *
 *                                                                                             *
 * INPUT: * chunk_id - chunk type that the loader needs to handle *
 *                                                                                             *
 * OUTPUT: * pointer to the appropriate loader or NULL if one wasn't found *
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 7/28/98    GTH : Created. *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 7/29/98    GTH : Created. * 12/8/98    GTH : Renamed to simply
 *Add_Prototype                                          * 2/19/99    EHC : Now
 *adds the prototype to the prototype list                             *
 *=============================================================================================*/
