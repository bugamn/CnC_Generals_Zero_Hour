...
const char *EMITTER_TYPE_NAMES[EMITTER_TYPEID_COUNT] = {"Default"};
...
ParticleEmitterDefClass::ParticleEmitterDefClass(
    const ParticleEmitterDefClass &src)
    : m_pName(NULL),
      m_Version(0L),
      m_pUserString(NULL),
      m_iUserType(EMITTER_TYPEID_DEFAULT),
...
ParticleEmitterDefClass::~ParticleEmitterDefClass(void) {
  ...
  if (m_pUserString != NULL) {
    // free() is used because the buffer was allocated with ::malloc() or
    // ::_strdup().
    ::free(m_pUserString);
    m_pUserString = NULL;
  }
...
//	ParticleEmitterDefClass
//
const ParticleEmitterDefClass &ParticleEmitterDefClass::operator=(
    const ParticleEmitterDefClass &src) {
  //
...
//	Set_Velocity_Random
//
void ParticleEmitterDefClass::Set_Velocity_Random(
    Vector3Randomizer *randomizer) {
  SAFE_DELETE(m_pVelocityRandomizer);
...
//	Set_Creation_Volume
//
void ParticleEmitterDefClass::Set_Creation_Volume(
    Vector3Randomizer *randomizer) {
  SAFE_DELETE(m_pCreationVolume);
...
void ParticleEmitterDefClass::Initialize_To_Ver2(void) {
  ...
  W3dUtilityClass::Convert_Shader(ShaderClass::_PresetAdditiveSpriteShader,
                                  &m_InfoV2.Shader);
...
void ParticleEmitterDefClass::Convert_To_Ver2(void) {
  if (m_Version < 0x00020000) {
    ...
    TextureClass *ptexture =
        WW3DAssetManager::Get_Instance()->Get_Texture(m_Info.TextureFilename);
    if (ptexture != NULL) {
    ...
    m_OpacityKeyframes.Values =
        W3DNEW float(((float)(m_Info.EndColor.A)) / 255);
...
//	Read_User_Data
//
WW3DErrorType ParticleEmitterDefClass::Read_User_Data(
  ...
  if (chunk_load.Open_Chunk() &&
    ...
    if (chunk_load.Read(&user_info, sizeof(user_info)) == sizeof(user_info)) {
      ...
      if (user_info.SizeofStringParam > 0) {
        m_pUserString =
            (char *)::malloc(sizeof(char) * (user_info.SizeofStringParam + 1));
        m_pUserString[0] = 0;

        // Attempt to read the user-string from the chunk
        if (chunk_load.Read(m_pUserString, user_info.SizeofStringParam) !=
            user_info.SizeofStringParam) {
...
//	Create_Randomizer
//
Vector3Randomizer *ParticleEmitterDefClass::Create_Randomizer(
    W3dVolumeRandomizerStruct &info) {
  Vector3Randomizer *randomizer = NULL;
  switch (info.ClassID) {
    case Vector3Randomizer::CLASSID_SOLIDBOX:
      randomizer = W3DNEW Vector3SolidBoxRandomizer(
          Vector3(info.Value1, info.Value2, info.Value3));
      break;
    ...
    case Vector3Randomizer::CLASSID_SOLIDCYLINDER:
      randomizer =
          W3DNEW Vector3SolidCylinderRandomizer(info.Value1, info.Value2);
      break;
  }
  ...
  if (chunk_load.Open_Chunk() &&
    ...
    if (chunk_load.Read(&info, sizeof(info)) == sizeof(info)) {
      ...
      if (m_ColorKeyframes.NumKeyFrames > 0) {
        m_ColorKeyframes.KeyTimes =
            W3DNEWARRAY float[m_ColorKeyframes.NumKeyFrames];
        m_ColorKeyframes.Values =
            W3DNEWARRAY Vector3[m_ColorKeyframes.NumKeyFrames];
      }
      ...
      if (m_SizeKeyframes.NumKeyFrames > 0) {
        m_SizeKeyframes.KeyTimes =
            W3DNEWARRAY float[m_SizeKeyframes.NumKeyFrames];
        m_SizeKeyframes.Values =
            W3DNEWARRAY float[m_SizeKeyframes.NumKeyFrames];
      }
      ...
      for (index = 0; index < m_ColorKeyframes.NumKeyFrames; index++) {
        Read_Color_Keyframe(chunk_load, &m_ColorKeyframes.KeyTimes[index],
                            &m_ColorKeyframes.Values[index]);
      }

      //
      //	If the last keyframe is 'black' and we are using a color
      //randomizer,
      ...
      if (last_keyframe > 0 && m_ColorKeyframes.Values[last_keyframe].X == 0 &&
          ...
          (m_ColorKeyframes.Rand.X > 0 || m_ColorKeyframes.Rand.Y > 0 ||
           m_ColorKeyframes.Rand.Z > 0)) {
      ...
      for (index = 0; index < m_OpacityKeyframes.NumKeyFrames; index++) {
        Read_Opacity_Keyframe(chunk_load, &m_OpacityKeyframes.KeyTimes[index],
      ...
      for (index = 0; index < m_SizeKeyframes.NumKeyFrames; index++) {
        Read_Size_Keyframe(chunk_load, &m_SizeKeyframes.KeyTimes[index],
...
//
WW3DErrorType ParticleEmitterDefClass::Read_Line_Properties(
    ChunkLoadClass &chunk_load) {
  // Assume error
  WW3DErrorType ret_val = WW3D_ERROR_LOAD_FAILED;
  ...
  if (chunk_load.Cur_Chunk_ID() == W3D_CHUNK_EMITTER_INFO) {
    // Read the chunk straight into our member structure
    if (chunk_load.Read(&m_LineProperties, sizeof(m_LineProperties)) ==
...
//
WW3DErrorType ParticleEmitterDefClass::Read_Rotation_Keyframes(
    ChunkLoadClass &chunk_load) {
  // Assume success
  ...
  if (m_RotationKeyframes.NumKeyFrames > 0) {
    m_RotationKeyframes.KeyTimes =
        W3DNEWARRAY float[m_RotationKeyframes.NumKeyFrames];
    m_RotationKeyframes.Values =
        W3DNEWARRAY float[m_RotationKeyframes.NumKeyFrames];
  }

  // Read in the keys
  for (unsigned int i = 0;
       (i < header.KeyframeCount) && (ret_val == WW3D_ERROR_OK); i++) {
    W3dEmitterRotationKeyframeStruct key;
    if (chunk_load.Read(&key, sizeof(key)) == sizeof(key)) {
...
//
WW3DErrorType ParticleEmitterDefClass::Read_Frame_Keyframes(
    ChunkLoadClass &chunk_load) {
  // Assume success
  WW3DErrorType ret_val = WW3D_ERROR_OK;
...
//	Save_User_Data
//
WW3DErrorType ParticleEmitterDefClass::Save_User_Data(
...
//
WW3DErrorType ParticleEmitterDefClass::Save_Color_Keyframes(
    ChunkSaveClass &chunk_save) {
  // Assume error
  WW3DErrorType ret_val = WW3D_ERROR_SAVE_FAILED;
...
//
WW3DErrorType ParticleEmitterDefClass::Save_Opacity_Keyframes(
    ChunkSaveClass &chunk_save) {
  // Assume error
  WW3DErrorType ret_val = WW3D_ERROR_SAVE_FAILED;
...
//	Save_Size_Keyframes
//
WW3DErrorType ParticleEmitterDefClass::Save_Size_Keyframes(
    ChunkSaveClass &chunk_save) {
  // Assume error
  WW3DErrorType ret_val = WW3D_ERROR_SAVE_FAILED;
...
WW3DErrorType ParticleEmitterDefClass::Save_Line_Properties(
    ChunkSaveClass &chunk_save) {
  // Assume error
  WW3DErrorType ret_val = WW3D_ERROR_SAVE_FAILED;
  ...
  if (chunk_save.Begin_Chunk(W3D_CHUNK_EMITTER_LINE_PROPERTIES) == TRUE) {
    ...
    if (chunk_save.Write(&m_LineProperties, sizeof(m_LineProperties)) ==
        sizeof(m_LineProperties)) {
      // Success!
      ret_val = WW3D_ERROR_OK;
    }
...
//
WW3DErrorType ParticleEmitterDefClass::Save_Rotation_Keyframes(
    ChunkSaveClass &chunk_save) {
  // Assume error
  WW3DErrorType ret_val = WW3D_ERROR_SAVE_FAILED;
  ...
  if (chunk_save.Begin_Chunk(W3D_CHUNK_EMITTER_ROTATION_KEYFRAMES) == TRUE) {
    ...
    for (unsigned int index = 0; (index < header.KeyframeCount) && success;
         index++) {
...
// opacity which are embedded inside the PROPS chunk.
//
WW3DErrorType ParticleEmitterDefClass::Save_Frame_Keyframes(
    ChunkSaveClass &chunk_save) {
  // Assume error
  WW3DErrorType ret_val = WW3D_ERROR_SAVE_FAILED;
...
//	Set_Color_Keyframes
//
void ParticleEmitterDefClass::Set_Color_Keyframes(
    ParticlePropertyStruct<Vector3> &keyframes) {
  SAFE_DELETE_ARRAY(m_ColorKeyframes.KeyTimes);
  SAFE_DELETE_ARRAY(m_ColorKeyframes.Values);
...
//
void ParticleEmitterDefClass::Set_Opacity_Keyframes(
    ParticlePropertyStruct<float> &keyframes) {
  SAFE_DELETE_ARRAY(m_OpacityKeyframes.KeyTimes);
  SAFE_DELETE_ARRAY(m_OpacityKeyframes.Values);
...
//	Set_Size_Keyframes
//
void ParticleEmitterDefClass::Set_Size_Keyframes(
    ParticlePropertyStruct<float> &keyframes) {
  SAFE_DELETE_ARRAY(m_SizeKeyframes.KeyTimes);
  SAFE_DELETE_ARRAY(m_SizeKeyframes.Values);
...
//
void ParticleEmitterDefClass::Set_Rotation_Keyframes(
    ParticlePropertyStruct<float> &keyframes, float orient_rnd) {
  SAFE_DELETE_ARRAY(m_RotationKeyframes.KeyTimes);
  SAFE_DELETE_ARRAY(m_RotationKeyframes.Values);
...
//	Set_Frame_Keyframes
//
void ParticleEmitterDefClass::Set_Frame_Keyframes(
    ParticlePropertyStruct<float> &keyframes) {
  SAFE_DELETE_ARRAY(m_FrameKeyframes.KeyTimes);
  SAFE_DELETE_ARRAY(m_FrameKeyframes.Values);
...
//	Get_Color_Keyframes
//
void ParticleEmitterDefClass::Get_Color_Keyframes(
    ParticlePropertyStruct<Vector3> &keyframes) const {
...
//
void ParticleEmitterDefClass::Get_Opacity_Keyframes(
    ParticlePropertyStruct<float> &keyframes) const {
...
//	Get_Size_Keyframes
//
void ParticleEmitterDefClass::Get_Size_Keyframes(
    ParticlePropertyStruct<float> &keyframes) const {
...
//
void ParticleEmitterDefClass::Get_Rotation_Keyframes(
    ParticlePropertyStruct<float> &keyframes) const {
...
//	Get_Frame_Keyframes
//
void ParticleEmitterDefClass::Get_Frame_Keyframes(
    ParticlePropertyStruct<float> &keyframes) const {
...
//	Create
//
RenderObjClass *ParticleEmitterPrototypeClass::Create(void) {
...
//	Load
//
PrototypeClass *ParticleEmitterLoaderClass::Load_W3D(
    ChunkLoadClass &chunk_load) {
  // Assume failure
