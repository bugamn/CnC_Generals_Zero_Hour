...
*2^MAX_SEGLINE_SUBDIV_LEVELS, so that the
...
SegLineRendererClass &SegLineRendererClass::operator=(
    const SegLineRendererClass &that) {
  if (this != &that) {
...
void SegLineRendererClass::Set_Texture_Tile_Factor(float factor) {
  if (factor > 50.0f) {  ///@todo: I raised this number and didn't see much
...
void SegLineRendererClass::Render(RenderInfoClass &rinfo,
  ...
  */
  ...
  // This is also the reason we stop when chidx = NumPoints - 1: the last point
  // has already been processed in the previous iteration so we don't need
  // another one).
  for (unsigned int chidx = 0; chidx < num_points - 1;
       chidx += (chunk_size - 1)) {
    ...
    */
    ...
    VectorProcessorClass::Transform(&xformed_pts[0], &points[chidx], modelview,
                                    point_cnt);
    ...
    */
    ...
    LineSegmentIntersection intersection[MAX_SEGLINE_POINT_BUFFER_SIZE + 1]
                                        [NUM_EDGES];

    for (sidx = 1; sidx < point_cnt;
         sidx++) {  // #segments = #points - 1 (+ 2 dummy segments)
    ...
    */
    ...
    for (iidx = 2; iidx < num_intersections[TOP_EDGE]; iidx++) {
      ...
      // Top:
      vdp = Vector3::Dot_Product(segment[iidx - 1].EdgePlane[TOP_EDGE],
                                 segment[iidx].EdgePlane[TOP_EDGE]);
      if (fabs(vdp) < parallel_factor) {
        ...
        Vector3::Cross_Product(segment[iidx - 1].EdgePlane[TOP_EDGE],
                               segment[iidx].EdgePlane[TOP_EDGE],
                               &(intersection[iidx][TOP_EDGE].Direction));
        intersection[iidx][TOP_EDGE].Direction.Normalize();
        if (Vector3::Dot_Product(intersection[iidx][TOP_EDGE].Direction,
                                 midpoint) < 0.0f) {
          intersection[iidx][TOP_EDGE].Direction =
              -intersection[iidx][TOP_EDGE].Direction;
        }
        ...
        Vector3 pl;
        if (vdp > 0.0f) {
          pl = segment[iidx - 1].EdgePlane[TOP_EDGE] +
               segment[iidx].EdgePlane[TOP_EDGE];
        } else {
          pl = segment[iidx - 1].EdgePlane[TOP_EDGE] -
               segment[iidx].EdgePlane[TOP_EDGE];
      ...
      // Bottom:
      vdp = Vector3::Dot_Product(segment[iidx - 1].EdgePlane[BOTTOM_EDGE],
                                 segment[iidx].EdgePlane[BOTTOM_EDGE]);
      if (fabs(vdp) < parallel_factor) {
        // Not parallel - intersect planes to get line (get vector, normalize
        // it, ensure it is pointing towards the midpoint)
        Vector3::Cross_Product(segment[iidx - 1].EdgePlane[BOTTOM_EDGE],
                               segment[iidx].EdgePlane[BOTTOM_EDGE],
                               &(intersection[iidx][BOTTOM_EDGE].Direction));
        intersection[iidx][BOTTOM_EDGE].Direction.Normalize();
        if (Vector3::Dot_Product(intersection[iidx][BOTTOM_EDGE].Direction,
                                 midpoint) < 0.0f) {
          intersection[iidx][BOTTOM_EDGE].Direction =
              -intersection[iidx][BOTTOM_EDGE].Direction;
        }
        ...
        Vector3 pl;
        if (vdp > 0.0f) {
          pl = segment[iidx - 1].EdgePlane[BOTTOM_EDGE] +
               segment[iidx].EdgePlane[BOTTOM_EDGE];
        } else {
          pl = segment[iidx - 1].EdgePlane[BOTTOM_EDGE] -
               segment[iidx].EdgePlane[BOTTOM_EDGE];
      ...
      // Find StartPlane:
      Vector3::Cross_Product(intersection[iidx][TOP_EDGE].Direction,
                             intersection[iidx][BOTTOM_EDGE].Direction,
                             &start_pl);
      start_pl.Normalize();
        ...
        for (edge = FIRST_EDGE; edge <= MAX_EDGE;
          ...
          unsigned int num_isects =
              num_intersections[edge];  // Capture here because will change
                                        // inside loop
          for (iidx_r = 1, iidx_w = 1; iidx_r < num_isects;
               iidx_r++, iidx_w++) {
            ...
            // Note: iidx_r is used for anything at or after the current
            // position, iidx_w is used for anything before the current position
            // (previous positions have potentially already been merged). Note:
            // iidx_r is used for anything at or after the current position,
            // iidx_w is used for anything before the current position (previous
            // positions have potentially already been merged).
            LineSegmentIntersection *curr_int = &(intersection[iidx_r][edge]);
            LineSegmentIntersection *next_int =
                &(intersection[iidx_r + 1][edge]);
            LineSegmentIntersection *write_int = &(intersection[iidx_w][edge]);
            LineSegmentIntersection *prev_int =
                &(intersection[iidx_w - 1][edge]);
          ...
          // If iidx_r is exactly equal to num_isects (rather than being larger
          // by one) at this point, this means that the last intersection was
          // not merged with the previous one. In this case, we need to do one
          // last copy:
          if (iidx_r == num_isects) {
            LineSegmentIntersection *write_int = &(intersection[iidx_w][edge]);
            LineSegmentIntersection *curr_int = &(intersection[iidx_r][edge]);
    ...
    */
    ...
    for (;;) {
      if (residual_top_points == 1 && residual_bottom_points == 1) {
        ...
        vArray[vidx].u1 = u_values[0] + uv_offset.X;
        vArray[vidx].v1 =
            intersection[top_int_idx][TOP_EDGE].TexV + uv_offset.Y;
        vidx++;
        vArray[vidx].x = bottom.X;
        vArray[vidx].y = bottom.Y;
        vArray[vidx].z = bottom.Z;
        vArray[vidx].u1 = u_values[1] + uv_offset.X;
        vArray[vidx].v1 =
            intersection[bottom_int_idx][BOTTOM_EDGE].TexV + uv_offset.Y;
        vidx++;
      } else {
    ...
    */
    ...
    if (!rgba_all) {
      ...
      mat = VertexMaterialClass::Get_Preset(
          VertexMaterialClass::PRELIT_NODIFFUSE);
    }
...
void SegLineRendererClass::subdivision_util(unsigned int point_cnt,
  ...
  SegLineSubdivision
      stack[2 * MAX_SEGLINE_SUBDIV_LEVELS];  // Maximum number needed
  int tos = 0;

  for (unsigned int pidx = 0; pidx < point_cnt - 1; pidx++) {
    // Subdivide the (pidx, pidx + 1) segment. Produce pidx and all subdivided
    // points up to (not including) pidx + 1.
    tos = 0;
