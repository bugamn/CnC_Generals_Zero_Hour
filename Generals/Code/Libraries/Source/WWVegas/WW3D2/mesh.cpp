 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * MeshClass::MeshClass -- Constructor for MeshClass *
 *   MeshClass::MeshClass -- Copy Constructor for MeshClass *
 *	  MeshClass::operator == -- assignment operator for MeshClass *
 *   MeshClass::~MeshClass -- destructor * MeshClass::Contains -- Determines
 *whether mesh contains a (worldspace) point.             * MeshClass::Free --
 *Releases all memory/assets in use by this mesh                         *
 *   MeshClass::Clone -- Creates a clone of this mesh * MeshClass::Get_Name --
 *returns the name of the mesh                                       *
 *   MeshClass::Set_Name -- sets the name of this mesh *
 *   MeshClass::Get_W3D_Flags -- access to the W3D flags *
 *   MeshClass::Get_User_Text -- access to the text buffer * MeshClass::Scale --
 *Scales the mesh                                                       *
 *   MeshClass::Scale -- Scales the mesh * MeshClass::Init -- Init the mesh from
 *a MeshBuilder object                                * MeshClass::Load --
 ...
 *   MeshClass::Generate_Culling_Tree -- Generates a hierarchical culling tree
 *for the mesh    * MeshClass::Direct_Load -- read the w3d file directly into
 *this mesh object                * MeshClass::read_chunks -- read all of the
 *chunks from the .wtm file                       * MeshClass::read_vertices --
 *reads the vertex chunk                                        *
 *   MeshClass::read_texcoords -- read in the texture coordinates chunk *
 *   MeshClass::install_texture_coordinates -- installs the given u-v's in each
 *channel that is* MeshClass::read_vertex_normals -- reads a surrender normal
 *chunk from the wtm file        * MeshClass::read_v3_materials -- Reads in
 *version 3 materials.                             * MeshClass::read_map --
 *Reads definition of a texture map from the file                    *
 *   MeshClass::read_triangles -- read the triangles chunk *
 *   MeshClass::read_per_tri_materials -- read the material indices for each
 *triangle          * MeshClass::read_user_text -- read in the user text chunk *
 ...
 ** MeshClass::Render -- renders this mesh * MeshClass::Special_Render --
 *special render function for meshes                           *
 *   MeshClass::update_skin -- deforms the mesh * MeshClass::clone_materials --
 *clone the materials for this mesh                           *
 *   MeshClass::install_materials -- transfers the materials into the mesh *
 *   MeshClass::Get_Model -- user access to the mesh model *
 *   MeshClass::Get_Obj_Space_Bounding_Sphere -- returns obj-space bounding
 *sphere             * MeshClass::Get_Obj_Space_Bounding_Box -- returns the
 *obj-space bounding box               * MeshClass::Get_Deformed_Vertices --
 *Gets the deformed vertices for a skin                 *
 *   MeshClass::Get_Deformed_Vertices -- Gets the deformed vertices for a skin *
 *   MeshClass::Render_Material_Pass -- Render a procedural material pass for
 *this mesh        * MeshClass::Replace_VertexMaterial -- Replaces existing
 *vertex material with a new one. Wi * MeshClass::Make_Unique -- Makes mesh
 *unique in the renderer, but still shares system ram  *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
/*
** This #define causes the collision code to always recompute the triangle
*normals rather
** than using the ones in the model.
** TODO: ensure that the models have unit normals and start re-using them again
*or write
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 1/6/98     GTH : Created. * 12/8/98    GTH : Modified the box
 *scaling to use the non-uniform parameters               *
 *=============================================================================================*/
...
void MeshClass::Compose_Deformed_Vertex_Buffer(VertexFormatXYZNDUV2 *verts,
                                               const Vector2 *uv0,
                                               const Vector2 *uv1,
                                               const unsigned *diffuse) {
  WWASSERT(Model->Get_Flag(MeshGeometryClass::SKIN));
  Model->compose_deformed_vertex_buffer(verts, uv0, uv1, diffuse,
                                        Container->Get_HTree());
}
...
void MeshClass::Create_Decal(DecalGeneratorClass *generator) {
  ...
  if (Is_Translucent() &&
      (generator->Is_Applied_To_Translucent_Meshes() == false)) {
    return;
  }
  ...
  if (!Model->Get_Flag(MeshGeometryClass::SKIN)) {
    ...
    OBBoxClass::Transform(modeltm_inv, generator->Get_Bounding_Volume(),
                          &localbox);
    ...
    if (apt.Count() > 0) {
      if (DecalMesh == NULL) {
        DecalMesh = NEW_REF(RigidDecalMeshClass,
                            (this, generator->Peek_Decal_System()));
      }
    ...
    // Skin
    // The deformed worldspace vertices are used both for the APT and in
    // Create_Decal() to generate the texture coordinates.
    ...
    if (apt.Count() > 0) {
      if (DecalMesh == NULL) {
        DecalMesh =
            NEW_REF(SkinDecalMeshClass, (this, generator->Peek_Decal_System()));
      }
...
void MeshClass::Render(RenderInfoClass &rinfo) {
  ...
  if (WW3D::Are_Static_Sort_Lists_Enabled() && sort_level != SORT_LEVEL_NONE) {
    ...
    if (Model->Get_Flag(MeshGeometryClass::SKIN) ||
        CollisionMath::Overlap_Test(frustum, Get_Bounding_Box()) !=
            CollisionMath::OUTSIDE) {
      bool rendered_something = false;

      /*
      ** If this mesh model has never been rendered, we need to generate the DX8
      *datastructures
      */
      ...
      */
      if (sort_level ==
          SORT_LEVEL_NONE)  // sorting ones get their environment set above.
      {
        Set_Lighting_Environment(rinfo.light_environment);
        // Add custom alpha
        m_alphaOverride = rinfo.alphaOverride;
      ...
      */
      DX8FVFCategoryContainer *fvf_container =
          Model->PolygonRendererList.Peek_Head()
              ->Get_Texture_Category()
              ->Get_Container();
      if ((rinfo.Current_Override_Flags() &
        ...
        */
        ...
        while (!it.Is_Done()) {
          ...
          polygon_renderer->Get_Texture_Category()->Add_Render_Task(
              polygon_renderer, this);
          it.Next();
        }
      ...
      /*
      ** If the rendering context specifies procedural material passes, register
      *them
      ** for rendering
      */
      for (int i = 0; i < rinfo.Additional_Pass_Count(); i++) {
        ...
        if ((!Is_Translucent()) ||
            (matpass->Is_Enabled_On_Translucent_Meshes())) {
      ...
      /*
      ** If we rendered any base or procedural passes and this is a skin, we
      *need
      ** to tell the mesh rendering system to process this skin
      */
      if (rendered_something && Model->Get_Flag(MeshGeometryClass::SKIN)) {
        // WWASSERT(dynamic_cast<DX8SkinFVFCategoryContainer *>(fvf_container)
        // != NULL);
        static_cast<DX8SkinFVFCategoryContainer *>(fvf_container)
            ->Add_Visible_Skin(this);
      }
      ...
      */
      if (DecalMesh != NULL) {
        ...
        rinfo.Camera.Transform_To_View_Space(cam_space_sphere_center,
                                             ws_sphere.Center);
        if (-cam_space_sphere_center.Z - ws_sphere.Radius <
            WW3D::Get_Decal_Rejection_Distance()) {
...
/***********************************************************************************************
 * MeshClass::Render_Material_Pass -- Render a procedural material pass for this
 *mesh          *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void MeshClass::Render_Material_Pass(MaterialPassClass *pass,
  ...
  if (Model->Get_Flag(MeshModelClass::SKIN)) {
    /*
    ** In the case of skin meshes, we need to render our polys with the identity
    *transform
    */
    ...
    if (oldOpacity >=
        0) {  // opacity was modified for this mesh instance, so need to restore
              // the material setting which may be shared
      // among other instances.
      pass->Peek_Material()->Set_Opacity(oldOpacity);
    }
    if (oldEmissive.X >=
        0) {  // emissive was modified for this mesh instance, so need to
              // restore the material setting which may be shared
      // among other instances.
      pass->Peek_Material()->Set_Emissive(oldEmissive);
    }
    // MW: Need uninstall custom materials in case they leave D3D in unknown
    // state
    pass->UnInstall_Materials();

  } else if ((pass->Get_Cull_Volume() != NULL) &&
    ...
    */
    ...
    if (_apt.Count() > 0) {
      ...
      */
      ...
      DX8Wrapper::Draw_Triangles(0, _apt.Count(), min_v, max_v - min_v + 1);
      // MW: Need uninstall custom materials in case they leave D3D in unknown
      // state
      pass->UnInstall_Materials();
    }
...
void MeshClass::Special_Render(SpecialRenderInfoClass &rinfo) {
  if ((Is_Not_Hidden_At_All() == false) &&
      (rinfo.RenderType != SpecialRenderInfoClass::RENDER_SHADOW)) {
    return;
  }
  ...
  if (rinfo.RenderType == SpecialRenderInfoClass::RENDER_VIS) {
    ...
    if (Model->Get_Flag(MeshModelClass::SKIN) == 0) {
      ...
      rinfo.VisRasterizer->Render_Triangles(
          Model->Get_Vertex_Array(), Model->Get_Vertex_Count(),
          Model->Get_Polygon_Array(), Model->Get_Polygon_Count(),
          Get_Bounding_Box());
    } else {
      int vertex_count = Model->Get_Vertex_Count();
      if (_TempVertexBuffer.Count() < vertex_count)
      ...
      rinfo.VisRasterizer->Render_Triangles(
          dst_vert, Model->Get_Vertex_Count(), Model->Get_Polygon_Array(),
...
/***********************************************************************************************
 * MeshClass::Replace_VertexMaterial -- Replaces existing vertex material with a
 *new one. Will *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
...
/***********************************************************************************************
 * MeshClass::Make_Unique -- Makes mesh unique in the renderer, but still shares
 *system ram ge *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
...
WW3DErrorType MeshClass::Load_W3D(ChunkLoadClass &cload) {
  ...
  */
  int col_bits = (Model->W3dAttributes & W3D_MESH_FLAG_COLLISION_TYPE_MASK) >>
                 W3D_MESH_FLAG_COLLISION_TYPE_SHIFT;
  Set_Collision_Type(col_bits << 1);
  Set_Hidden(Model->W3dAttributes & W3D_MESH_FLAG_HIDDEN);

  /*
  ** Indicate whether this mesh is translucent.  The mesh is considered
  *translucent
  ** if sorting has been enabled (alpha blending on pass 0) or if pass0 contains
  *alpha-test.
  ** This flag is mainly being used by visibility preprocessing code in
  *Renegade.
  */
  int is_translucent = Model->Get_Flag(MeshModelClass::SORT);
  int is_alpha = 0;  // keep track of alpha pieces that require sorting
                     // (including static sort lists).
  int is_additive = 0;

  if (Model->Has_Shader_Array(0)) {
    for (int i = 0; i < Model->Get_Polygon_Count(); i++) {
      is_translucent |= (Model->Get_Shader(i, 0).Get_Alpha_Test() ==
                         ShaderClass::ALPHATEST_ENABLE);
      is_alpha |= (Model->Get_Shader(i, 0).Get_Dst_Blend_Func() !=
                       ShaderClass::DSTBLEND_ZERO ||
                   Model->Get_Shader(i, 0).Get_Src_Blend_Func() !=
                       ShaderClass::SRCBLEND_ONE) &&
                  (Model->Get_Shader(i, 0).Get_Alpha_Test() !=
                   ShaderClass::ALPHATEST_ENABLE);
      is_additive |= (Model->Get_Shader(i, 0).Get_Dst_Blend_Func() ==
                          ShaderClass::DSTBLEND_ONE &&
                      Model->Get_Shader(i, 0).Get_Src_Blend_Func() ==
                          ShaderClass::SRCBLEND_ONE);
    }
  } else {
    is_translucent |= (Model->Get_Single_Shader(0).Get_Alpha_Test() ==
                       ShaderClass::ALPHATEST_ENABLE);
    is_alpha |= (Model->Get_Single_Shader(0).Get_Dst_Blend_Func() !=
                     ShaderClass::DSTBLEND_ZERO ||
                 Model->Get_Single_Shader(0).Get_Src_Blend_Func() !=
                     ShaderClass::SRCBLEND_ONE) &&
                (Model->Get_Single_Shader(0).Get_Alpha_Test() !=
                 ShaderClass::ALPHATEST_ENABLE);
    is_additive |= (Model->Get_Single_Shader(0).Get_Dst_Blend_Func() ==
                        ShaderClass::DSTBLEND_ONE &&
                    Model->Get_Single_Shader(0).Get_Src_Blend_Func() ==
                        ShaderClass::SRCBLEND_ONE);
  }
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 6/17/98    GTH : Created. *
 *=============================================================================================*/
bool MeshClass::Cast_Ray(RayCollisionTestClass &raytest) {
  if ((Get_Collision_Type() & raytest.CollisionType) == 0) return false;
  // Modified for 'Generals' so we could select trees but filter out headlight
  // beams, etc. -MW
  if (raytest.CheckTranslucent && Is_Alpha() != 0) return false;
  ...
  if (Model->Get_Flag(MeshModelClass::ALIGNED)) {
    ...
    world.Obj_Look_At(mesh_position, mesh_position - raytest.Ray.Get_Dir(),
                      0.0f);
  ...
  if (hit) {
    ...
    Matrix3D::Rotate_Vector(world, raytest.Result->Normal,
                            &(raytest.Result->Normal));
    if (raytest.Result->ComputeContactPoint) {
      Matrix3D::Transform_Vector(world, raytest.Result->ContactPoint,
                                 &(raytest.Result->ContactPoint));
    }
  }
...
bool MeshClass::Cast_AABox(AABoxCollisionTestClass &boxtest) {
  ...
  // This function analyses the tranform to call optimized functions in certain
  // cases
...
bool MeshClass::Cast_OBBox(OBBoxCollisionTestClass &boxtest) {
  ...
  */
  ...
  /*
  ** If we hit, transform the result of the test back to the original coordinate
  *system.
  */
  if (hit) {
    boxtest.CollidedRenderObj = this;
    Matrix3D::Rotate_Vector(tm, boxtest.Result->Normal,
                            &(boxtest.Result->Normal));
    if (boxtest.Result->ComputeContactPoint) {
      Matrix3D::Transform_Vector(tm, boxtest.Result->ContactPoint,
                                 &(boxtest.Result->ContactPoint));
    }
  }
 ...
 * MeshClass::Intersect_AABox -- test for intersection with given AABox *
 *                                                                                             *
 * The AAbox given is assumed to be in world space.  Since meshes aren't
 *generally in world    * space, the test must be transformed into our local
 *coordinate system (which turns it into   * an OBBox...) *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 ...
 * MeshClass::Intersect_OBBox -- test for intersection with the given OBBox *
 *                                                                                             *
 * The given OBBox is assumed to be in world space so we need to transform it
 *into the mesh's  * local coordinate system. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
...
/***********************************************************************************************
 * MeshClass::Generate_Culling_Tree -- Generates a hierarchical culling tree for
 *the mesh      *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void MeshClass::Update_Cached_Bounding_Volumes(void) const {
  ...
  // If we are camera-aligned or -oriented, we don't know which way we are
  // facing at this point, so the box we return needs to contain the sphere.
  // Otherewise do the normal computation.
  if (Model->Get_Flag(MeshModelClass::ALIGNED) ||
      Model->Get_Flag(MeshModelClass::ORIENTED)) {
    CachedBoundingBox.Center = CachedBoundingSphere.Center;
    CachedBoundingBox.Extent.Set(CachedBoundingSphere.Radius,
...
int MeshClass::Get_Draw_Call_Count(void) const {
  if (Model != NULL) {
    ...
    // Otherwise if we have textures, return the number of textures (e.g. dont
    // have prs when sorting)
