 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * CollectionClass::CollectionClass -- default constructor for
 *collection render object      * CollectionClass::CollectionClass --
 *constructor for collection render object              *
 *   CollectionClass::CollectionClass -- copy constructor *
 *   CollectionClass::CollectionClass -- assignment operator *
 *   CollectionClass::~CollectionClass -- destructor * CollectionClass::Clone --
 ...
 ** CollectionClass::Get_Num_Polys -- returns the number of polygons in this
 *collection       * CollectionClass::Render -- render this collection *
 *   CollectionClass::Special_Render -- passes the special render call to all
 *sub-objects      * CollectionClass::Set_Transform -- set the transform for
 *this collection                   * CollectionClass::Set_Position -- set the
 *position for this collection                     *
 *   CollectionClass::Get_Num_Sub_Objects -- returns the number of sub objects *
 *   CollectionClass::Get_Sub_Object -- returns a pointer to the desired sub
 *object            * CollectionClass::Add_Sub_Object -- adds another object
 *into this collection               * CollectionClass::Remove_Sub_Object --
 *removes a sub object from this collection           *
 *   CollectionClass::Cast_Ray -- passes the ray test to each sub object *
 *   CollectionClass::Cast_AABox -- passes the axis-aligned box test to each sub
 *object        * CollectionClass::Cast_OBBox -- passes the oriented box test to
 *each sub object            * CollectionClass::Intersect_AABox -- test for
 *intersection with an AABox                   *
 *   CollectionClass::Intersect_OBBox -- test for intersection with an OBBox *
 *   CollectionClass::Get_Obj_Space_Bounding_Sphere -- returns the object space
 *bounding spher * CollectionClass::Get_Obj_Space_Bounding_Box -- returns the
 *object-space bounding box      * CollectionClass::Snap_Point_Count -- returns
 *the number of snap points in this collecion  * CollectionClass::Get_Snap_Point
 *-- return the desired snap point                          *
 *   CollectionClass::Scale -- scale the objects in this collection *
 *   CollectionClass::Scale -- scale the objects in this collection *
 *   CollectionClass::Update_Obj_Space_Bounding_Volumes -- recomputes the object
 *space boundin * CollectionClass::Update_Sub_Object_Transforms -- recomputes
 *all sub object transforms     * CollectionLoaderClass::Load -- reads a
 *collection from a w3d file                         *
 *   CollectionDefClass::CollectionDefClass -- constructor *
 *   CollectionDefClass::~CollectionDefClass -- destructor for collection
 *definition           * CollectionDefClass::Free -- releases assets in use by a
 *collection definition             * CollectionDefClass::Get_Name -- returns
 *name of the collection                            * CollectionDefClass::Load
 *-- loads a collection definition from a w3d file                 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
class CollectionPrototypeClass : public W3DMPO, public PrototypeClass {
 ...
 public:
  ...
  virtual RenderObjClass *Create(void) {
    return NEW_REF(CollectionClass, (*ColDef));
  }
...
/***********************************************************************************************
 * CollectionClass::CollectionClass -- default constructor for collection render
 *object        *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 23/8/00    GTH : Created. *
 *=============================================================================================*/
CollectionClass::CollectionClass(void) : SnapPoints(NULL) {
...
CollectionClass::CollectionClass(const CollectionDefClass &def)
    : SubObjects(def.ObjectNames.Count()), SnapPoints(NULL) {
  // Set our name
  ...
  for (int i = 0; i < def.ObjectNames.Count(); i++) {
    ...
    SubObjects.Add(WW3DAssetManager::Get_Instance()->Create_Render_Obj(
        def.ObjectNames[i]));
 ...
 * HISTORY: * 12/8/98    GTH : Created. *
 *=============================================================================================*/
CollectionClass::~CollectionClass(void) { Free(); }
...
/***********************************************************************************************
 * CollectionClass::Remove_Sub_Object -- removes a sub object from this
 *collection             *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * CollectionClass::Cast_OBBox -- passes the oriented box test to each sub
 *object              *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * CollectionClass::Get_Obj_Space_Bounding_Box -- returns the object-space
 *bounding box        *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * CollectionClass::Snap_Point_Count -- returns the number of snap points in
 *this collecion    *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 *                                                                                             *
 * This function will set the passed vector to be equal to the object space
 *coordinates of     * the desired snap point. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
...
void CollectionClass::Update_Obj_Space_Bounding_Volumes(void) {
  ...
  MinMaxAABoxClass box(Vector3(FLT_MAX, FLT_MAX, FLT_MAX),
...
/***********************************************************************************************
 * CollectionClass::Update_Sub_Object_Transforms -- recomputes all sub object
 *transforms       *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * CollectionClass::Get_Placeholder -- Returns information about a placeholder
 *object.
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * CollectionClass::Get_Proxy_Count -- Returns the count of proxy objects in the
 *collection.
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 4/28/99    PDS : Created. *
 *=============================================================================================*/
int CollectionClass::Get_Proxy_Count(void) const { return ProxyList.Count(); }
 ...
 * HISTORY: * 12/8/98    GTH : Created. *
 *=============================================================================================*/
CollectionDefClass::CollectionDefClass(void) { SnapPoints = NULL; }
 ...
 * HISTORY: * 12/8/98    GTH : Created. *
 *=============================================================================================*/
CollectionDefClass::~CollectionDefClass(void) { Free(); }
 ...
 * HISTORY: * 12/8/98    GTH : Created. *
 *=============================================================================================*/
const char *CollectionDefClass::Get_Name(void) const { return Name; }
  ...
  while (cload.Open_Chunk()) {
    switch (cload.Cur_Chunk_ID()) {
      ...
      case W3D_CHUNK_PLACEHOLDER: {
        ...
        Matrix3D transform(
            info.transform[0][0], info.transform[1][0], info.transform[2][0],
            info.transform[3][0], info.transform[0][1], info.transform[1][1],
            info.transform[2][1], info.transform[3][1], info.transform[0][2],
            info.transform[1][2], info.transform[2][2], info.transform[3][2]);
