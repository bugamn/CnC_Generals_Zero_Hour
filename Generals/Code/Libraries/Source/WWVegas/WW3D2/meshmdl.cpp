...
/*
** Temporary Buffers
** These buffers are used by the skin code for temporary storage of the deformed
*vertices and
** vertex normals.
*/
...
MeshModelClass& MeshModelClass::operator=(const MeshModelClass& that) {
  if (this != &that) {
    // Remove all polygon renderers, this will remove the mesh from the
    // rendering system. The mesh will be initialized to rendering system the
...
void MeshModelClass::Register_For_Rendering() {
  ...
  if (WW3D::Get_NPatches_Level() > 1) {
    if (WW3D::Get_NPatches_Gap_Filling_Mode() !=
    ...
    if (WW3D::Get_NPatches_Gap_Filling_Mode() ==
        WW3D::NPATCHES_GAP_FILLING_FORCE) {
      Init_For_NPatch_Rendering();
    } else if (GapFiller) {
      delete GapFiller;
      GapFiller = NULL;
...
void MeshModelClass::Replace_Texture(TextureClass* texture,
  ...
  for (int stage = 0; stage < MeshMatDescClass::MAX_TEX_STAGES; ++stage) {
    for (int pass = 0; pass < Get_Pass_Count(); ++pass) {
      ...
      // If this mesh model has been initialized for rendering we need to tell
      // the rendering system to change texturing as well.
      DX8FVFCategoryContainer* fvf_category = Peek_FVF_Category_Container();
      if (fvf_category) {
        fvf_category->Change_Polygon_Renderer_Texture(
            PolygonRendererList, texture, new_texture, pass, stage);
      }
    }
  }
}
...
void MeshModelClass::Replace_VertexMaterial(VertexMaterialClass* vmat,
  ...
  for (int pass = 0; pass < Get_Pass_Count(); ++pass) {
    ...
    // If this mesh model has been initialized for rendering we need to tell the
    // rendering system to change texturing as well.
    DX8FVFCategoryContainer* fvf_category = Peek_FVF_Category_Container();
    if (fvf_category) {
      fvf_category->Change_Polygon_Renderer_Material(PolygonRendererList, vmat,
                                                     new_vmat, pass);
    }
  }
}
...
DX8FVFCategoryContainer* MeshModelClass::Peek_FVF_Category_Container() {
  ...
  DX8TextureCategoryClass* texture_category =
      polygon_renderer->Get_Texture_Category();
...
void MeshModelClass::Shadow_Render(SpecialRenderInfoClass& rinfo,
  ...
  if (rinfo.BWRenderer != NULL) {
    ...
    rinfo.BWRenderer->Set_Vertex_Locations(
        reinterpret_cast<Vector2*>(transf_ptr), VertexCount);
    rinfo.BWRenderer->Render_Triangles(
        reinterpret_cast<const unsigned long*>(Poly->Get_Array()),
        PolyCount * 3);
    return;
  }
}
...
void MeshModelClass::get_deformed_vertices(Vector3* dst_vert,
                                           const HTreeClass* htree) {
...
// Destination pointers MUST point to arrays large enough to hold all vertices
void MeshModelClass::get_deformed_vertices(Vector3* dst_vert, Vector3* dst_norm,
                                           const HTreeClass* htree) {
  int vi;
  ...
  for (vi = 0; vi < vertex_count;) {
    ...
    VectorProcessorClass::Transform(dst_vert + vi, src_vert + vi, mytm,
                                    cnt - vi);
    mytm.Set_Translation(Vector3(0.0f, 0.0f, 0.0f));
    VectorProcessorClass::Transform(dst_norm + vi, src_norm + vi, mytm,
                                    cnt - vi);
    vi = cnt;
  }
}
...
void MeshModelClass::compose_deformed_vertex_buffer(VertexFormatXYZNDUV2* verts,
                                                    const Vector2* uv0,
                                                    const Vector2* uv1,
...
// Destination pointers MUST point to arrays large enough to hold all vertices
void MeshModelClass::get_deformed_screenspace_vertices(
    Vector4* dst_vert, const RenderInfoClass& rinfo,
    const Matrix3D& mesh_transform, const HTreeClass* htree) {
  Matrix4 prj = rinfo.Camera.Get_Projection_Matrix() *
                rinfo.Camera.Get_View_Matrix() * mesh_transform;
  ...
  if (Get_Flag(SKIN) && VertexBoneLink && htree) {
    ...
    for (int vi = 0; vi < vertex_count;) {
      ...
      // Count equal matrices (the vertices should be pre-sorted by matrices
      // they use)
      for (int cnt = vi; cnt < vertex_count; cnt++)
        if (idx != bonelink[cnt]) break;

      // Transform to screenspace (x,y,z,w)
      VectorProcessorClass::Transform(dst_vert + vi, src_vert + vi, tm,
                                      cnt - vi);
...
void MeshModelClass::Make_Geometry_Unique() {
  ...
  ShareBufferClass<Vector3>* unique_verts =
      NEW_REF(ShareBufferClass<Vector3>, (*Vertex));
  REF_PTR_SET(Vertex, unique_verts);
  REF_PTR_RELEASE(unique_verts);

  ShareBufferClass<Vector3>* norms =
      NEW_REF(ShareBufferClass<Vector3>, (*VertexNorm));
  REF_PTR_SET(VertexNorm, norms);
  REF_PTR_RELEASE(norms);

#if (!OPTIMIZE_PLANEEQ_RAM)
  ShareBufferClass<Vector4>* peq =
      NEW_REF(ShareBufferClass<Vector4>, (*PlaneEq, "MeshModelClass::PlaneEq"));
...
template <>
inline unsigned int HashTemplateKeyClass<Vector3>::Get_Hash_Value(
...
template <>
inline unsigned int HashTemplateKeyClass<TriangleSide>::Get_Hash_Value(
    const TriangleSide& side) {
...
GapFillerClass::GapFillerClass(MeshModelClass* mmc_)
    : mmc(NULL), PolygonCount(0) {
  REF_PTR_SET(mmc, mmc_);

  ArraySize =
      mmc->Get_Polygon_Count() * 6;  // Each side of each triangle can have 2
                                     // polygons added, in the worst case
...
GapFillerClass::GapFillerClass(const GapFillerClass& that)
    : mmc(NULL), PolygonCount(that.PolygonCount) {
...
void GapFillerClass::Add_Polygon(unsigned polygon_index, unsigned vidx1,
                                 unsigned vidx2, unsigned vidx3) {
  ...
  for (int pass = 0; pass < mmc->Get_Pass_Count(); ++pass) {
    ...
    if (mmc->Has_Material_Array(pass)) {
      ...
      MaterialArray[pass][PolygonCount] =
          mmc->Get_Material(mmc->Get_Polygon_Array()[polygon_index][0], pass);
    }
    for (int stage = 0; stage < MeshMatDescClass::MAX_TEX_STAGES; ++stage) {
      if (mmc->Has_Texture_Array(pass, stage)) {
        TextureArray[pass][stage][PolygonCount] =
            mmc->Get_Texture(polygon_index, pass, stage);
      }
    }
  }
  PolygonCount++;
  ...
  for (int pass = 0; pass < mmc->Get_Pass_Count(); ++pass) {
    for (int stage = 0; stage < MeshMatDescClass::MAX_TEX_STAGES; ++stage) {
      if (TextureArray[pass][stage]) {
        ...
        TextureClass** new_texture_array =
            W3DNEWARRAY TextureClass* [PolygonCount];
        memcpy(new_texture_array, TextureArray[pass][stage],
               PolygonCount * sizeof(TextureClass*));
        delete[] TextureArray[pass][stage];
    ...
    if (MaterialArray[pass]) {
      ...
      VertexMaterialClass** new_material_array =
          W3DNEWARRAY VertexMaterialClass* [PolygonCount];
      memcpy(new_material_array, MaterialArray[pass],
             PolygonCount * sizeof(VertexMaterialClass*));
      delete[] MaterialArray[pass];
    ...
    if (ShaderArray[pass]) {
      ...
      memcpy(new_shader_array, ShaderArray[pass],
             PolygonCount * sizeof(ShaderClass));
      delete[] ShaderArray[pass];
...
void MeshModelClass::Init_For_NPatch_Rendering() {
  ...
  for (i = 0; i < polygon_count; ++i) {
    bool duplicates[3];
    duplicates[0] =
        DuplicateLocationHash.Exists(locations[polygon_indices[i][0]]);
    duplicates[1] =
        DuplicateLocationHash.Exists(locations[polygon_indices[i][1]]);
    duplicates[2] =
        DuplicateLocationHash.Exists(locations[polygon_indices[i][2]]);
    if (duplicates[0] && duplicates[1]) {
      TriangleSide tri(locations[polygon_indices[i][0]],
                       locations[polygon_indices[i][1]]);
      if (SideHash.Exists(tri)) {
        ...
        bool diff =
            !(idx1 ^ idx3) | !(idx1 ^ idx4) | !(idx2 ^ idx3) | !(idx2 ^ idx4);
        if (!diff) {
    ...
    if (duplicates[1] && duplicates[2]) {
      TriangleSide tri(locations[polygon_indices[i][1]],
                       locations[polygon_indices[i][2]]);
      if (SideHash.Exists(tri)) {
        SideIndexInfo side_index = SideHash.Get(tri);
        unsigned idx1 = side_index.vidx1;
        unsigned idx2 = side_index.vidx2;
    ...
    if (duplicates[2] && duplicates[0]) {
      TriangleSide tri(locations[polygon_indices[i][2]],
                       locations[polygon_indices[i][0]]);
      if (SideHash.Exists(tri)) {
        SideIndexInfo side_index = SideHash.Get(tri);
        unsigned idx1 = side_index.vidx1;
        unsigned idx2 = side_index.vidx2;
