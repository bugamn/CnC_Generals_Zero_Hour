...
/*
       ...
       WW3D_FORMAT_A8L8,
       WW3D_FORMAT_A4L4,
       WW3D_FORMAT_COUNT	// Used only to determine number of surface
   formats
*/
...
void Vector4_to_Color(unsigned int* outc, const Vector4& inc,
...
void Color_to_Vector4(Vector4* outc, const unsigned int inc,
                      const WW3DFormat format) {
  WWASSERT(outc);
...
void Get_WW3D_Format(WW3DFormat& dest_format, WW3DFormat& src_format,
                     unsigned& src_bpp, const Targa& targa) {
  ...
  switch (targa.Header.PixelDepth) {
    ...
    case 8:
      ...
      else if (targa.Header.ImageType == TGA_MONO)
        src_format = WW3D_FORMAT_L8;
      else
        src_format = WW3D_FORMAT_A8;
      break;
    default:
...
WW3DFormat Get_Valid_Texture_Format(WW3DFormat format,
  ...
  if (!DX8Caps::Support_DXTC() || !is_compression_allowed ||
      WW3D::Get_Texture_Compression_Mode() ==
          WW3D::TEXTURE_COMPRESSION_DISABLE) {
    switch (format) {
    ...
    switch (format) {
      case WW3D_FORMAT_DXT1:
        // NVidia hack - switch to DXT2 is there is no DXT1 support (which is
        // disabled on NVidia cards)
        if (!DX8Caps::Support_Texture_Format(WW3D_FORMAT_DXT1) &&
            DX8Caps::Support_Texture_Format(WW3D_FORMAT_DXT2)) {
  ...
  if (bits <= 16) {
    switch (format) {
      ...
      case WW3D_FORMAT_P8:
      default:
        // Basically, anything goes here (just make sure the most common 32 bit
        // formats are converted to 16 bit
        break;
    }
  }
