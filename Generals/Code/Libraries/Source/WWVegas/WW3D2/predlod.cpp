...
/*
**	NOTE: LODHeapNodes contain pointers to RenderObjClass, but these
*pointers
...
class LODHeap {
 public:
  ...
  LODHeapNode *Top(void) { return &(Nodes[1]); }
...
void PredictiveLODOptimizerClass::Optimize_LODs(float max_cost) {
  ...
  // Allocate and fill arrays. (one extra entry since the zeroth entry is not
  // used).
  //	LODHeapNode *visible_obj_array1 = W3DNEWARRAY LODHeapNode[NumObjects +
  //1]; 	LODHeapNode *visible_obj_array2 = W3DNEWARRAY LODHeapNode[NumObjects +
  //1];

  // Insert objects into arrays: (0th entry reserved for sentinel values)
  for (int i = 0; i < NumObjects; i++) {
    RenderObjClass *robj = ObjectArray[i];
    // We use minus Value for the first queue to make it ordered by minimum
    // Value.
    VisibleObjArray1[i + 1] = LODHeapNode(robj, -(robj->Get_Value()));
    VisibleObjArray2[i + 1] =
        LODHeapNode(robj, robj->Get_Post_Increment_Value());
  }
  ...
  while (!done) {
    // Initialize max_data and min_data so comparison at end of loop uses
    // correct values.
    max_data = NULL;
    min_data = NULL;
    ...
    if (TotalCost <= max_cost) {
      ...
      if (max_post_increment_value_queue.Top()->Get_Key() ==
          RenderObjClass::AT_MAX_LOD) {
        done = true;
        break;
      }
      ...
      max_post_increment_value_queue.Change_Key_Top(
          max_data->Get_Post_Increment_Value());
    ...
    while (TotalCost > max_cost) {
      ...
      // so stop.
      if (min_current_value_queue.Top()->Get_Key() ==
          -RenderObjClass::AT_MIN_LOD) {
        done = true;
        break;
      }
      ...
      max_post_increment_value_queue.Change_Key(
          min_data, min_data->Get_Post_Increment_Value());
