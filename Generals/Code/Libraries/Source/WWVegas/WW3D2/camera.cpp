 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * CameraClass::CameraClass -- constructor *
 *   CameraClass::CameraClass -- copy constructor * CameraClass::operator == --
 *assignment operator                                           *
 *   CameraClass::~CameraClass -- destructor * CameraClass::Clone -- virtual
 *copy constructor                                            * CameraClass::Add
 *-- add the camera to the world?                                          *
 *   CameraClass::Remove -- Remove the camera from the world? *
 *   CameraClass::Get_Obj_Space_Bounding_Sphere -- returns the object space
 *bounding sphere    * CameraClass::Get_Object_Space_Bounding_Box -- returns the
 *object space bounding box       * CameraClass::Set_Transform -- set the
 ...
 *   CameraClass::Set_Focal_Length -- set the view plane using focal length and
 *aspect ratio   * CameraClass::Get_View_Plane -- get the corners of the current
 *view plane                  * CameraClass::Project -- project a point from ws
 *to the view plane                         *
 *   CameraClass::Project_Camera_Space_Point -- Project a point that is already
 *in camera spac * CameraClass::Un_Project -- "unproject" a point from the view
 *plane to world space         * CameraClass::Transform_To_View_Space --
 *transforms the given world space point to camera  *
 *   CameraClass::Rotate_To_View_Space -- rotates the given world space vector
 *to camera space * CameraClass::Get_Near_Clip_Bounding_Box -- returns an obb
 *that contains near clip plane   * CameraCLass::Update_Frustum -- updates the
 *frustum parameters                             * CameraClass::Cull_Box --
 *tests whether the given box can be culled                        *
 *   CameraClass::Device_To_View_Space -- converts the given device coordinate
 *to view space   * CameraClass::Device_To_World_Space -- converts given device
 *coord to world space          * CameraClass::Camera_Push -- pushes the
 *camera's parameters into the given GERD            * CameraClass::Camera_Pop
 *-- pops the camera's parameters from the given GERD               *
 *   CameraClass::Set_Aspect_Ratio -- sets the aspect ratio of the camera *
 *   CameraClass::Apply_D3D_State -- sets the D3D states controlled by the
 *camera              *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 3/21/98    GTH : Created. * 4/13/2001  hy : added in copy code for
...
CameraClass::CameraClass(const CameraClass &src)
  ...
  // just being paraniod in case any parent class doesn't completely copy the
  // entire state...
  FrustumValid = false;
}
...
/***********************************************************************************************
 * CameraClass::Get_Object_Space_Bounding_Box -- returns the object space
 *bounding box         *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void CameraClass::Get_Obj_Space_Bounding_Box(AABoxClass &box) const {
  ...
  box.Extent.Set(ZFar, ZFar,
                 ZFar);  // could optimize this but its not really used.
}
 ...
 * CameraClass::Set_View_Plane -- control over the view plane *
 *                                                                                             *
 * INPUT: * min - x,y of the upper left corner of the view rectangle (dist is
 *assumed to be 1.0)        * max - x,y of the lower right corner of the view
 *rectangle                                   *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 ...
 *                                                                                             *
 * INPUT: * dest - will be set to a point on the normalized view plane.  x,y,
 *and z range from -1 to 1  * ws_point - input point in world space *
 *                                                                                             *
 * OUTPUT: * ProjectionResType indicating whether the point was in the frustum *
 *                                                                                             *
 * WARNINGS: * When the input is behind the near clip plane, 0,0,0 is returned.
 **
 *                                                                                             *
 * HISTORY: * 3/21/98    GTH : Created. *
 *=============================================================================================*/
CameraClass::ProjectionResType CameraClass::Project(
    Vector3 &dest, const Vector3 &ws_point) const {
  Update_Frustum();
  ...
  if ((dest.X < -1.0f) || (dest.X > 1.0f) || (dest.Y < -1.0f) ||
      (dest.Y > 1.0f)) {
...
/***********************************************************************************************
 * CameraClass::Project_Camera_Space_Point -- Project a point that is already in
 *camera space  *
 *                                                                                             *
 * INPUT: * dest - will be set to a point on the normalized view plane.  x,y,
 *and z range from -1 to 1  * cam_point - input point in camera space *
 *                                                                                             *
 * OUTPUT: * ProjectionResType indicating whether the point was in the frustum *
 *                                                                                             *
 * WARNINGS: * When the input is behind the near clip plane, 0,0,0 is returned.
 **
 *                                                                                             *
 * HISTORY: * 11/17/2000 gth : Created. *
 *=============================================================================================*/
CameraClass::ProjectionResType CameraClass::Project_Camera_Space_Point(
    Vector3 &dest, const Vector3 &cam_point) const {
  Update_Frustum();
...
/***********************************************************************************************
 * CameraClass::Un_Project -- "unproject" a point from the view plane to world
 *space           *
 *                                                                                             *
 * The given point is assumed to be on the view_plane at z=-1.  The 3D world
 *space point that  * this represents will be returned in dest. *
 *                                                                                             *
 * INPUT: * dest - will be filled in with the 3D world space point that the
 *given point represents      * view_point - point on the view_plane to be
 *un-projected                                     *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * CameraClass::Transform_To_View_Space -- transforms the given world space
 *point to camera sp *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * CameraClass::Rotate_To_View_Space -- rotates the given world space vector to
 *camera space   *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * CameraClass::Get_Near_Clip_Bounding_Box -- returns an obb that contains near
 *clip plane     *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void CameraClass::Update_Frustum(void) const {
...
#endif
  NearClipBBox.Extent.X =
      (vpmax.X - vpmin.X) * (-znear) *
      0.5f;  // (near_clip_x / |znear|) == (vpmin.X / 1.0f)...
  ...
  if (Projection == PERSPECTIVE) {
    ProjectionTransform.Init_Perspective(
        vpmin.X * znear_dist, vpmax.X * znear_dist, vpmin.Y * znear_dist,
        vpmax.Y * znear_dist, znear_dist, zfar_dist);

  } else {
    ProjectionTransform.Init_Ortho(vpmin.X, vpmax.X, vpmin.Y, vpmax.Y,
                                   znear_dist, zfar_dist);
  }
}

/***********************************************************************************************
 * CameraClass::Device_To_View_Space -- converts the given device coordinate to
 *view space     *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void CameraClass::Device_To_View_Space(const Vector2 &device_coord,
  ...
  set_view->X = vs_min.X + (ndev.X - Viewport.Min.X) * (vs_max.X - vs_min.X) /
                               (Viewport.Width());
  set_view->Y = vs_max.Y - (ndev.Y - Viewport.Min.Y) * (vs_max.Y - vs_min.Y) /
                               (Viewport.Height());
  set_view->Z = -1.0f;
}

/***********************************************************************************************
 * CameraClass::Device_To_World_Space -- converts given device coord to world
 *space            *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
float CameraClass::Get_Aspect_Ratio(void) const { return AspectRatio; }
