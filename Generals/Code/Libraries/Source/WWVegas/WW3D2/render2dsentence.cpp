...
void Render2DSentenceClass::Build_Textures(void) {
  ...
  for (int index = 0; index < PendingSurfaces.Count(); index++) {
    ...
    TextureClass *new_texture =
        W3DNEW TextureClass(desc.Width, desc.Width, WW3D_FORMAT_A4R4G4B4,
  ...
  while (!end) {
    ...
    while (1) {
      ...
      while ((*word != 0) && (*word > L' ') && (*word != L'\n')) {
        ...
        if (WrapWidth > 0 && word_width >= WrapWidth && useHardWordWrap) break;
      }
      //
...
void FontCharsClass::Create_GDI_Font(const char *font_name) {
  ...
  GDIBitmap =
      ::CreateDIBSection(screen_dc, (const BITMAPINFO *)&bitmap_info,
                         DIB_RGB_COLORS, (void **)&GDIBitmapBits, NULL, 0L);
...
//
////////////////////////////////////////////////////////////////////////////////////
void FontCharsClass::Initialize_GDI_Font(const char *font_name, int point_size,
                                         bool is_bold) {
  //
