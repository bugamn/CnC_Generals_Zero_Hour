...
ParticleEmitterClass::ParticleEmitterClass(
    float emit_rate, unsigned int burst_size, Vector3Randomizer *pos_rnd,
    ...
    ParticlePropertyStruct<float> &frames, Vector3 accel, float max_age,
  ...
  // The maximum number of particles is determined by the emission rate, burst
  // size and lifetime. However, it is capped both by the particle cap and by
  // the maximum buffer size, if these are active.
  ...
  Buffer = W3DNEW ParticleBufferClass(
      this, max_num, color, opacity, size, rotation, orient_rnd, frames,
      accel / 1000000.0f, max_age, tex, shader, pingpong, render_mode,
      frame_mode, line_props);
...
ParticleEmitterClass::ParticleEmitterClass(const ParticleEmitterClass &src)
    : IsInScene(false), RenderObjClass(src) {
...
ParticleEmitterClass &ParticleEmitterClass::operator=(
    const ParticleEmitterClass &that) {
...
ParticleEmitterClass *ParticleEmitterClass::Create_From_Definition(
    const ParticleEmitterDefClass &definition) {
  // Assume failure
  ...
  /*if (ptexture) {
          // If texture has an alpha channel do alpha blending instead of
  additive
  ...
  pemitter = NEW_REF(
      ...
      (definition.Get_Emission_Rate(), definition.Get_Burst_Size(),
       definition.Get_Creation_Volume(), definition.Get_Velocity(),
       definition.Get_Velocity_Random(), definition.Get_Outward_Vel(),
       definition.Get_Vel_Inherit(), color_keys, opacity_keys, size_keys,
       rotation_keys, definition.Get_Initial_Orientation_Random(), frame_keys,
       definition.Get_Acceleration(), definition.Get_Lifetime(), ptexture,
       shader, definition.Get_Max_Emissions(), 0, false,
       definition.Get_Render_Mode(), definition.Get_Frame_Mode(),
...
void ParticleEmitterClass::Create_New_Particles(const Quaternion &curr_quat,
  ...
  // Since the particles are written into a wraparound buffer, we can take the
  // time modulo a time constant which represents the time it takes to fill up
  // the entire buffer with new particles. We will do this so we don't run into
  // performance problems with very large frame times.
  ...
  if (VelInheritFactor) {
    InheritedWorldSpaceEmitterVel =
        (curr_orig - PrevOrig) * (VelInheritFactor / fl_frametime);
  } else {
  ...
  for (; EmitRemain > EmitRate;) {
    ...
    for (unsigned int i = 0; i < burst_size; i++) {
      Initialize_Particle(Buffer->Add_Uninitialized_New_Particle(), age, quat,
                          orig);
    }
...
void ParticleEmitterClass::Initialize_Particle(NewParticleStruct *newpart,
  ...
  newpart->Velocity =
      InheritedWorldSpaceEmitterVel + quat.Rotate_Vector(rand_vel);
}
...
ParticleEmitterDefClass *ParticleEmitterClass::Build_Definition(void) const {
  ...
  if (pdefinition != NULL) {
    ...
    pdefinition->Set_Rotation_Keyframes(rotations,
