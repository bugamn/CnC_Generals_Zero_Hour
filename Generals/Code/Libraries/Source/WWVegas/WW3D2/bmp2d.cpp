...
Bitmap2DObjClass::Bitmap2DObjClass(const char *filename, float screen_x,
                                   float screen_y, bool center, bool additive,
                                   bool colorizable, int usable_width,
                                   int usable_height, bool ignore_alpha)
  ...
  if (usable_width == -1) usable_width = sd.Width;
  if (usable_height == -1) usable_height = sd.Height;
  ...
  for (int lpy = 0, tlpy = 0; lpy < mh; lpy++, tlpy += piece) {
    for (int lpx = 0, tlpx = 0; lpx < mw; lpx++, tlpx += piece) {
      ...
      SurfaceClass *piece_surface =
          NEW_REF(SurfaceClass, (pot, pot, sd.Format));
      piece_surface->Copy(0, 0, tlpx, tlpy, pot, pot, surface);
      TextureClass *piece_texture =
          NEW_REF(TextureClass, (piece_surface, TextureClass::MIP_LEVELS_1));
      ...
      // calculate our actual texture coordinates based on the difference
      // between the width and height of the texture and the width and height
      // the font occupys.
      ...
      // figure out the screen space x and y positions of the object in
      // question.
...
Bitmap2DObjClass::Bitmap2DObjClass(TextureClass *texture, float screen_x,
                                   float screen_y, bool center, bool additive,
                                   bool colorizable, bool ignore_alpha)
    : DynamicScreenMeshClass(2, 4) {
  int resw, resh, resbits;
  bool windowed;
  ...
  // Set shader to additive if requested, else alpha or opaque depending on
  // whether the texture has an alpha channel. Sorting is never set - if you
  // wish to sort these types of objects you should use static sort levels (note
  // that static sort levels are not implemented for these objects yet, but it
