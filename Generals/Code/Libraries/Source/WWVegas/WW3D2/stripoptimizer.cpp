...
int StripOptimizerClass::Get_Strip_Index_Count(const int* strips,
                                               int strip_count) {
  int cnt = 0;
...
void StripOptimizerClass::Optimize_Strip_Order(int* strips, int strip_count) {
  ...
  int outSize = Get_Strip_Index_Count(strips, strip_count) +
                strip_count;  // output memory alloc size
...
void StripOptimizerClass::Optimize_Triangle_Order(int* tris,
                                                  int triangle_count) {
...
int* StripOptimizerClass::Combine_Strips(const int* strips, int strip_count) {
  int check =
      Get_Strip_Index_Count(strips, strip_count) + (strip_count - 1) * 3;
  ...
  for (int i = 0; i < strip_count; i++) {
    ...
    if (i != 0) {
      *o++ = *strips;  // duplicate first
      ...
      if (!prevEven) *o++ = *strips;
    }

    for (int j = 0; j < len; j++) *o++ = *strips++;  // copy the strip

    if (i != (strip_count - 1)) *o++ = o[-1];
...
struct Triangle {
  ...
  int getConnectivity(void) const {
    int cnt = 0;
    if (m_neighbors[0]) cnt++;
    if (m_neighbors[1]) cnt++;
    if (m_neighbors[2]) cnt++;
    return cnt;
  }
...
class Stripify {
 ...
 private:
  ...
  static Vector3i getTriangleNodeConnectivityWeights(const TriangleQueue& queue,
                                                     const Triangle& tri);
...
template <>
inline unsigned int HashTemplateKeyClass<Strip::Edge>::Get_Hash_Value(
    const Strip::Edge& s) {
...
inline Triangle* TriangleQueue::getTop(void) const {
  for (int i = 0; i < 4; i++)
    if (m_bin[i]) return m_bin[i];  // return head
  return 0;                         // end
}
...
inline void TriangleQueue::reinsert(Triangle* t) {
  ...
  if (t->m_next) t->m_next->m_prev = t->m_prev;

  t->m_prev = 0;
  t->m_next = m_bin[w];
  if (t->m_next) t->m_next->m_prev = t;
...
inline TriangleQueue::TriangleQueue(Triangle* tris, int N) {
  int i;
  for (i = 0; i < 4; i++) m_bin[i] = 0;  // initialize to zero
  ...
  for (i = 0; i < m_vertexCount; i++) m_nodeConnectivity[i] = 0;

  for (i = 0; i < N; i++) {
    Triangle* t = tris + i;
    int w = t->getConnectivity();
    WWASSERT(w >= 0 && w <= 3);
    WWASSERT(!t->m_prev && !t->m_next &&
             t->m_bin == -1);  // must not be in a bin
    t->m_prev = 0;
    t->m_next = m_bin[w];
    if (t->m_next) t->m_next->m_prev = t;
    m_bin[w] = t;  // head of bin
    t->m_bin = w;  // set bin
    for (int j = 0; j < 3; j++) m_nodeConnectivity[t->m_vertices[j]]++;
  }
}
 ...
 *
 * Description:		Returns "node connectivity" weights for each triangle
 *vertex
 *
...
inline Vector3i Stripify::getTriangleNodeConnectivityWeights(
    const TriangleQueue& queue, const Triangle& tri) {
  int weight[3];
  for (int i = 0; i < 3; i++) {
 ...
 *
 * Description:		Converts input triangles into internal Triangle
 *structure
 *
 ...
 *
 * Notes:			The function sets up the initial connectivity
 *information
 *
 *****************************************************************************/
        ...
        if (n)  // if edge is already in the hash...
        {
          ...
          for (k = 0; k < 3; k++)    // find matching edge
            if (!n->m_neighbors[k])  // this neighbor not located yet
            {
              ...
              if (ek.v[0] == edge.v[1] &&
                  ek.v[1] == edge.v[0])  // if matching edge (note that order
                                         // must be different)
          ...
          if (k < 3)  // (k==3) -> no match
          {
            ...
            hash.Remove(e);         // remove from hash (this speeds up the hash
 ...
 *					[int]		length of first strip
 *(in vertices) [int,..]	vertex indices of the first strip [int]
 *length of second strip
 *					...
 *
 *					The routine assumes that degenerate
 *triangles have been remove and input vertices have been welded
 *
 *****************************************************************************/
...
int* Stripify::stripify(const Vector3i* inTris, int N) {
  ...
  for (;;) {
    ...
    for (int i = 0; i < 3; i++)
      ...
      {
        int weight = t->m_neighbors[i]
                         ->getConnectivity();  // get connectivity of neighbor
    ...
    for (;;) {
      ...
      for (i = 0; i < 3; i++)
        if (t->m_neighbors[i]) {
          ...
          for (int j = 0; j < 3; j++) {
            Edge e = n->getEdge(j);
            if (!(len & 1)) swap(e.v[0], e.v[1]);  // swap
      ...
      for (i = 0; i < 3; i++)
        ...
        {
          Edge e =
              next->getEdge(i);  // a swap happens if it contains the prevprev
