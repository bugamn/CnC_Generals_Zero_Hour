...
static void Record_Texture_End() {
  ...
  if (record_texture_mode == Debug_Statistics::RECORD_TEXTURE_DETAILS) {
    ...
    for (int a = 0; a < texture_count; ++a) {
      ...
      StringClass flash = "  ";
      //			if (t && t->getClassID() ==
      //ID_TEXTURE_FILE_CLASS) { 				TextureFileClass*
      //tfc=static_cast<TextureFileClass*>(t); 				id=tfc->ID(); 				if
      //(tfc->Get_Texture_Flash()) flash="F";
      //			}

      working_string.Format(
          "%4.4d  %3.3d   %3.3d     %s ", id, texture_statistics[a].usage_count,
      ...
      if (t) {
        //				int
        //red_factor=t->Get_Current_Reduction_Factor();
        unsigned bytes = t->Get_Texture_Memory_Usage();
        //				unsigned
        //non_red_bytes=t->Get_Non_Reduced_Texture_Memory_Usage();
        if (!t->Is_Initialized()) {
          //					non_red_bytes=bytes;
          texture_statistics_string += "*";
        } else {
        ...
        //				int
        //percents=100-100*bytes/non_red_bytes;
    ...
    texture_statistics_string +=
        "\nid              = id of texture. Use with command 'flash_texture "
        "[id]'\n";
    texture_statistics_string +=
...
Debug_Statistics::RecordTextureMode
Debug_Statistics::Get_Record_Texture_Mode() {
...
int Debug_Statistics::Get_Record_Texture_Size()  // Return total textures used
                                                 // during latest frame, in
                                                 // bytes
{
  return last_frame_texture_memory;
}

int Debug_Statistics::Get_Record_Texture_Count()  // Return total textures used
                                                  // during latest frame, in
                                                  // bytes
{
...
int Debug_Statistics::
    Get_Record_Lightmap_Texture_Size()  // Return total lightmap textures used
                                        // during latest frame, in bytes
{
  return last_frame_lightmap_texture_memory;
}

int Debug_Statistics::
    Get_Record_Lightmap_Texture_Count()  // Return total lightmap textures used
                                         // during latest frame, in bytes
{
...
int Debug_Statistics::
    Get_Record_Procedural_Texture_Size()  // Return total procedural textures
                                          // used during latest frame, in bytes
{
  return last_frame_procedural_texture_memory;
}

int Debug_Statistics::
    Get_Record_Procedural_Texture_Count()  // Return total procedural textures
                                           // used during latest frame, in bytes
{
...
void Debug_Statistics::Record_DX8_Polys_And_Vertices(
    int pcount, int vcount, const ShaderClass& shader) {
  if (shader.Get_NPatch_Enable() == ShaderClass::NPATCH_ENABLE &&
      DX8Caps::Support_NPatches()) {
...
int Debug_Statistics::Get_Draw_Calls() { return last_frame_draw_calls; }
