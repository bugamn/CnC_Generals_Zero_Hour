...
// After the dot product between the camera direction and the camera space
// location of the light source, we do a power to Halopow for the halo size and
// power to DazzlePow for the dazzle size. Halo/DazzleArea defines the angle
// where the values are valid, so any angle beyond HaloArea/DazzleArea results
// the halo or dazzle being scaled to zero. The angles are re-scaled from
// normalized angle scale of (-1.0...1.0) only HaloArea/DazzleArea defined part
// is used.
const char* HALO_INTENSITY_POW_STRING =
    "HaloIntensityPow";  // 1.0 would be linear fadeout, smaller than that will
                         // steepen the curve (smaller hotspot)
const char* HALO_SIZE_POW_STRING =
    "HaloSizePow";  // 1.0 would be linear fadeout, smaller than that will
                    // steepen the curve (smaller hotspot)
...
const char* DAZZLE_AREA_STRING =
    "DazzleArea";  // Something like 0.05 is a good starting point for a
                   // dazzle...
...
// Direction area defines the maximum difference between the light direction and
// the eyespace location, so the dazzle can only be seen if the camera is inside
// the light cone. Value 0.0 means dazzle is not directional, it can be seen
// from any direction. Halo is not affected. Dazzle direction defines the light
// direction vector.
...
// Dazzle and halo start to afde out after FadeoutStart meters and are
// completely gone by FadeoutEnd meters.
...
// The weight of history for the intensities. The history weight is per
// millisecond, so if you want to have any real history, values higher than 0.95
// are recommended (don't use value of 1.0 or anything higher!)
...
// If false, camera matrix's translation term isn't used. If translation isn't
// used the dazzle is treated as always visible, scene graph visibility is not
// used.
const char* USE_CAMERA_TRANSLATION = "UseCameraTranslation";

// Halo and dazzle colors, RGB, (1.0, 1.0, 1.0) is white and (0.0, 0.0, 0.0) is
// black
const char* HALO_COLOR_STRING = "HaloColor";
const char* DAZZLE_COLOR_STRING = "DazzleColor";

// Dazzle test color could be white in many cases but if the level has a lot of
// white then another color can be defined.
const char* DAZZLE_TEST_COLOR_STRING = "DazzleTestColor";

// Dazzles can blink on and off.  The blink period defines the time for one
// cycle, the blink "on time" defines the amount of time within that period that
// the dazzle is "on"
...
// The number of flare sprites in lensflare. The FlareLocation, FlareSize and
// FlareColor paremeters are procedural, the names are produced based on this
// parameter. If FlareCount is 3, there exists FlareLocation1, FlareLocation2
// and FlareLocation3... And so on.
const char* FLARE_COUNT_STRING = "FlareCount";

// 1D-locations of the flares. 0.0 means the location of the center of the
// screen and 1.0 means the location of the lightsource. The values can be
// smaller than zero and larger than 1.0.
...
// Uv coordinates (as there can be only one texture used, but one may want
// multiple images). The values is a 4-float vector: start_u, start_v, end_u,
// end_v.
...
static const DazzleVisibilityClass* _VisibilityHandler =
    &_DefaultVisibilityHandler;
...
class DazzleINIClass : public INIClass {
 public:
  ...
  const Vector2 Get_Vector2(char const* section, char const* entry,
                            const Vector2& defvalue = Vector2(0, 0));
  const Vector3 Get_Vector3(char const* section, char const* entry,
                            const Vector3& defvalue = Vector3(0, 0, 0));
  const Vector4 Get_Vector4(char const* section, char const* entry,
                            const Vector4& defvalue = Vector4(0, 0, 0,
                                                              0)) const;
};
...
const Vector4 DazzleINIClass::Get_Vector4(char const* section,
                                          char const* entry,
                                          const Vector4& defvalue) const {
  if (section != NULL && entry != NULL) {
    INIEntry* entryptr = Find_Entry(section, entry);
    if (entryptr && entryptr->Value != NULL) {
      Vector4 ret;
      if (sscanf(entryptr->Value, "%f,%f,%f,%f", &ret[0], &ret[1], &ret[2],
                 &ret[3]) == 4) {
        return ret;
      }
    }
...
LensflareTypeClass::~LensflareTypeClass() { REF_PTR_RELEASE(texture); }
...
void LensflareTypeClass::Generate_Vertex_Buffers(
    VertexFormatXYZNDUV2* vertex, int& vertex_count, float screen_x_scale,
    float screen_y_scale, float dazzle_intensity,
    const Vector4& transformed_location) {
  // Lensflares are placed on a line 2D that goes through the lightsource and
  // the center of the screen
  //	float
  //scale=sqrt(transformed_location[0]*transformed_location[0]+transformed_location[1]*transformed_location[1]);

  float z = transformed_location[2];

  float distance_multiplier =
      sqrt(transformed_location[0] * transformed_location[0] +
           transformed_location[1] * transformed_location[1]) +
      1.0f;
...
DazzleTypeClass::DazzleTypeClass(const DazzleInitClass& is)
  ...
  dazzle_test_color_integer =
      0xff000000 | (unsigned(255.0f * ic.dazzle_test_color[2]) << 16) |
...
void DazzleTypeClass::Set_Dazzle_Shader(
    const ShaderClass& s)  // Set shader for the dazzle type
{
  dazzle_shader = s;
}
...
void DazzleTypeClass::Set_Halo_Shader(
    const ShaderClass& s)  // Set shader for the dazzle type
{
  halo_shader = s;
}
...
TextureClass* DazzleTypeClass::Get_Dazzle_Texture() {
  if (!primary_texture) {
    primary_texture =
        WW3DAssetManager::Get_Instance()->Get_Texture(ic.primary_texture_name);
...
TextureClass* DazzleTypeClass::Get_Halo_Texture() {
  if (!secondary_texture) {
    secondary_texture = WW3DAssetManager::Get_Instance()->Get_Texture(
        ic.secondary_texture_name);
...
void DazzleRenderObjClass::Init_From_INI(const INIClass* ini) {
  ...
  for (entry = 0; entry < count; entry++) {
    ...
    ini->Get_String(LENSFLARE_LIST_STRING,
    ...
    if (lic.flare_count) {
      lic.flare_locations =
          W3DNEWARRAY float[lic.flare_count];  // Flare location is 1D
    ...
    for (int flare = 0; flare < lic.flare_count; ++flare) {
      ...
      TPoint3D<float> tp = ini->Get_Point(section_name, tmp_name,
                                          TPoint3D<float>(1.0f, 1.0f, 1.0f));
      ...
      lic.flare_uv[flare] = dini->Get_Vector4(section_name, tmp_name,
  ...
  for (entry = 0; entry < count; entry++) {
    char section_name[80];
    ini->Get_String(DAZZLE_LIST_STRING,
                    ini->Get_Entry(DAZZLE_LIST_STRING, entry), "", section_name,
                    sizeof(section_name));

    DazzleInitClass dic;

    dic.primary_texture_name =
        ini->Get_String(section_name, DAZZLE_TEXTURE_STRING);
    dic.secondary_texture_name =
        ini->Get_String(section_name, HALO_TEXTURE_STRING);
    dic.halo_intensity =
        ini->Get_Float(section_name, HALO_INTENSITY_STRING, 0.95f);
    dic.halo_intensity_pow =
        ini->Get_Float(section_name, HALO_INTENSITY_POW_STRING, 0.95f);
    dic.halo_size_pow =
        ini->Get_Float(section_name, HALO_SIZE_POW_STRING, 0.95f);
    ...
    dic.dazzle_direction_area =
        ini->Get_Float(section_name, DAZZLE_DIRECTION_AREA_STRING, 0.50f);
    dic.dazzle_intensity =
        ini->Get_Float(section_name, DAZZLE_INTENSITY_STRING, 0.90f);
    dic.dazzle_intensity_pow =
        ini->Get_Float(section_name, DAZZLE_INTENSITY_POW_STRING, 0.90f);
    dic.dazzle_size_pow =
        ini->Get_Float(section_name, DAZZLE_SIZE_POW_STRING, 0.90f);
    dic.dazzle_scale_x =
        ini->Get_Float(section_name, DAZZLE_SCALE_X_STRING, 100.0f);
    dic.dazzle_scale_y =
        ini->Get_Float(section_name, DAZZLE_SCALE_Y_STRING, 25.0f);
    dic.fadeout_start =
        ini->Get_Float(section_name, FADEOUT_START_STRING, 25.0f);
    dic.fadeout_end = ini->Get_Float(section_name, FADEOUT_END_STRING, 50.0f);
    dic.size_optimization_limit =
        ini->Get_Float(section_name, SIZE_OPTIMIZATION_LIMIT_STRING, 0.05f);
    dic.history_weight =
        ini->Get_Float(section_name, HISTORY_WEIGHT_STRING, 0.5f);
    dic.use_camera_translation =
        !!ini->Get_Int(section_name, USE_CAMERA_TRANSLATION, 1);
    dic.lensflare_name = ini->Get_String(section_name, DAZZLE_LENSFLARE_STRING);
    dic.type = entry;

    TPoint3D<float> tp = ini->Get_Point(section_name, DAZZLE_DIRECTION_STRING,
                                        TPoint3D<float>(0.0f, 0.0f, 0.0f));
    tp.Normalize();
    dic.dazzle_direction = reinterpret_cast<const Vector3&>(tp);

    tp = ini->Get_Point(section_name, DAZZLE_TEST_COLOR_STRING,
                        TPoint3D<float>(1.0f, 1.0f, 1.0f));
    dic.dazzle_test_color = reinterpret_cast<const Vector3&>(tp);

    tp = ini->Get_Point(section_name, DAZZLE_COLOR_STRING,
                        TPoint3D<float>(1.0f, 1.0f, 1.0f));
    dic.dazzle_color = reinterpret_cast<const Vector3&>(tp);

    tp = ini->Get_Point(section_name, HALO_COLOR_STRING,
                        TPoint3D<float>(1.0f, 1.0f, 1.0f));
    ...
    dic.blink_on_time =
        ini->Get_Float(section_name, BLINK_ON_TIME_STRING, 0.0f);
...
void DazzleRenderObjClass::Init_Lensflare(const LensflareInitClass& i) {
  ...
  if (i.type >= lensflare_count) {
    unsigned new_count = i.type + 1;
    LensflareTypeClass** new_lensflares =
        W3DNEWARRAY LensflareTypeClass* [new_count];
...
void DazzleRenderObjClass::Install_Dazzle_Visibility_Handler(
    const DazzleVisibilityClass* visibility_handler) {
...
// ----------------------------------------------------------------------------
//
// Copy constructor creates an exact copy of the object, except for the
// visibility information, which is initialised with default values.
//
// ----------------------------------------------------------------------------
...
DazzleRenderObjClass& DazzleRenderObjClass::operator=(
...
void DazzleRenderObjClass::Get_Obj_Space_Bounding_Sphere(
...
void DazzleRenderObjClass::Set_Layer(DazzleLayerClass* layer) {
  // Never insert a dazzle into a list if it already is on one - this caould
  // create infinite loops in the dazzle list.
  if (on_list) return;
...
void DazzleRenderObjClass::Render(RenderInfoClass& rinfo) {
  ...
  if (Is_Not_Hidden_At_All()) {
    ...
    if (ic.blink_period > 0.0f) {
      float elapsed_time =
          ((float)(WW3D::Get_Sync_Time() - creation_time)) / 1000.0f;
    ...
    if (is_on == false) {
      ...
      //			visibility =
      //_VisibilityHandler->Compute_Dazzle_Visibility(rinfo,this,position);
      ...
      Vector3 camera_dir(-view_transform[2][0], -view_transform[2][1],
                         -view_transform[2][2]);
      ...
      params->Calculate_Intensities(dazzle_intensity, dazzle_size,
                                    current_halo_intensity, camera_dir,
                                    current_dir, current_distance);
      ...
      if (dazzle_intensity > 0.0f) {
        visibility =
            _VisibilityHandler->Compute_Dazzle_Visibility(rinfo, this, loc);
        dazzle_intensity *= visibility;
      } else {
        visibility = 0.0f;
      }
      ...
      if (visibility == 0.0f) {
        float i = dazzle_intensity * (1.0f - weight) +
                  current_dazzle_intensity * weight;
    ...
    // If this dazzle is visible or it is currently fading, submit it for
    // rendering
...
void DazzleRenderObjClass::Render_Dazzle(CameraClass* camera) {
  ...
  bool windowed;
  WW3D::Get_Device_Resolution(display_width, display_height, display_bits,
                              windowed);
  ...
  LensflareTypeClass* lensflare =
      DazzleRenderObjClass::Get_Lensflare_Class(types[type]->lensflare_id);
  if (lensflare) {
    lens_max_verts = 4 * lensflare->lic.flare_count;
  }

  DynamicVBAccessClass vb_access(BUFFER_TYPE_DYNAMIC_DX8, dynamic_fvf_type,
                                 vertex_count * 2 + lens_max_verts);
  {
    ...
    Vector3 halo_dxt(
        camera->Compute_Projected_Sphere_Radius(current_distance, halo_scale_x),
        0.0f, 0.0f);

    Vector3 dazzle_dyt(0.0f, screen_y_scale * dazzle_scale_y, 0.0f);
    Vector3 halo_dyt(
        0.0f,
        camera->Compute_Projected_Sphere_Radius(current_distance, halo_scale_y),
        0.0f);
    ...
    if (current_dazzle_intensity > 0.0f) {
      ...
      Vector3 col(dazzle_color[0] * params->ic.dazzle_color[0],
    ...
    if (current_halo_intensity) {
      ...
      Vector3 col(halo_color[0] * params->ic.halo_color[0],
    ...
    if (lensflare && current_dazzle_intensity > 0.0f) {
      VertexFormatXYZNDUV2* vertex =
          verts + halo_vertex_count + dazzle_vertex_count;

      lensflare->Generate_Vertex_Buffers(
          vertex, lensflare_vertex_count, screen_x_scale, screen_y_scale,
          current_dazzle_intensity * lensflare_intensity, transformed_loc);
  ...
  int poly_count =
      halo_poly_count > dazzle_poly_count ? halo_poly_count : dazzle_poly_count;
  ...
  if (lensflare_poly_count) {
    DX8Wrapper::Set_Index_Buffer(ib_access,
                                 dazzle_vertex_count + halo_vertex_count);
    DX8Wrapper::Set_Shader(default_dazzle_shader);
...
DazzleTypeClass* DazzleRenderObjClass::Get_Type_Class(
    unsigned id)  // Return dazzle type class pointer, or NULL if not found
{
...
LensflareTypeClass* DazzleRenderObjClass::Get_Lensflare_Class(
    unsigned id)  // Return lensflare type class pointer, or NULL if not found
{
      ...
      case DAZZLEFACTORY_CHUNKID_VARIABLES:
        ...
        while (cload.Open_Micro_Chunk()) {
          switch (cload.Cur_Micro_Chunk_ID()) {
            ...
            READ_MICRO_CHUNK_STRING(cload, DAZZLEFACTORY_VARIABLE_TYPENAME,
                                    dazzle_type, sizeof(dazzle_type));
          }
...
void DazzlePersistFactoryClass::Save(ChunkSaveClass& csave,
                                     PersistClass* obj) const {
  DazzleRenderObjClass* robj = (DazzleRenderObjClass*)obj;
  unsigned int dazzle_type = robj->Get_Dazzle_Type();
  const char* dazzle_type_name =
      DazzleRenderObjClass::Get_Type_Name(dazzle_type);
  ...
  WRITE_MICRO_CHUNK_STRING(csave, DAZZLEFACTORY_VARIABLE_TYPENAME,
                           dazzle_type_name);
...
** The default dazzle visibility handler asks the scene to determine whether the
*dazzle
** is occluded.
**
**********************************************************************************************/
...
float DazzleVisibilityClass::Compute_Dazzle_Visibility(
    RenderInfoClass& rinfo, DazzleRenderObjClass* dazzle,
