 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * MeshGeometryClass::MeshGeometryClass -- Constructor *
 *   MeshGeometryClass::MeshGeometryClass -- Copy Constructor *
 *    -- assignment operator * MeshGeometryClass::~MeshGeometryClass --
 *destructor                                       *
 *   MeshGeometryClass::Reset_Geometry -- releases current resources and
 *allocates space if ne * MeshGeometryClass::Get_Name -- returns the name *
 ...
 *   MeshGeometryClass::Intersect_OBBox -- test for intersection with the given
 *OBBox          * MeshGeometryClass::Cast_World_Space_AABox -- test for
 *intersection with a worldspace AABox*
 *   MeshGeometryClass::cast_semi_infinite_axis_aligned_ray -- casts an axis
 *aligned ray       * MeshGeometryClass::cast_aabox_identity -- aligned aabox
 *test                              * MeshGeometryClass::cast_aabox_z90 -- aabox
 *test which is rotated about z by 90            *
 *   MeshGeometryClass::cast_aabox_z180 -- aabox test which is rotated about z
 *by 180          * MeshGeometryClass::cast_aabox_z270 -- aabox test which is
 *rotated about z by 270          * MeshGeometryClass::cast_ray_brute_force --
 ...
 *   MeshGeometryClass::intersect_obbox_brute_force -- brute force intersection
 *check          * MeshGeometryClass::Compute_Plane_Equations -- Recalculates
 *the plane equations            * MeshGeometryClass::Compute_Vertex_Normals --
 ...
 *   MeshGeometryClass::Get_Vertex_Normal_Array -- validates and returns the
 *vertex normal arr * MeshGeometryClass::get_planes -- get the plane array
 *memory (internal)                    * MeshGeometryClass::Get_Plane_Array --
 *validates and returns the array of plane equations  *
 *   MeshGeometryClass::Compute_Plane -- compute the plane equation of a single
 *poly           * MeshGeometryClass::Generate_Culling_Tree -- Generate an
 *AABTree for this mesh             * MeshGeometryClass::Load_W3D -- Load a mesh
 *from a w3d                                     *
 *   MeshGeometryClass::read_chunks -- read w3d chunks *
 *   MeshGeometryClass::read_vertices -- read the vertex chunk from a W3D file *
 *   MeshGeometryClass::read_vertex_normals -- read the vertex normals chunk
 *from a w3d file   * MeshGeometryClass::read_triangles -- read the triangles
 *chunk from a w3d file             * MeshGeometryClass::read_user_text -- read
 *the user text chunk from a w3d file             *
 *   MeshGeometryClass::read_vertex_influences -- read the vertex influences
 *chunk from a w3d  * MeshGeometryClass::read_vertex_shade_indices -- read the
 *vertex shade indices chunk       * MeshGeometryClass::read_aabtree -- read the
 *AABTree chunk from a w3d file                 *
 *   MeshGeometryClass::Generate_APT -- generate an apt for a box and view
 *direction           * MeshGeometryClass::Generate_Rigid_APT -- generate an apt
 ...
 * HISTORY: * 11/9/2000  gth : Created. *
 *=============================================================================================*/
MeshGeometryClass::~MeshGeometryClass(void) { Reset_Geometry(0, 0); }

/***********************************************************************************************
 * MeshGeometryClass::Reset_Geometry -- releases current resources and allocates
 *space if need *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void MeshGeometryClass::Reset_Geometry(int polycount, int vertcount) {
  ...
  if ((polycount != 0) && (vertcount != 0)) {
    Poly = NEW_REF(ShareBufferClass<Vector3i>,
                   (PolyCount, "MeshGeometryClass::Poly"));
    PolySurfaceType =
        NEW_REF(ShareBufferClass<uint8>,
...
void MeshGeometryClass::Set_Name(const char *newname) {
  ...
  if (newname) {
    MeshName = NEW_REF(ShareBufferClass<char>,
                       (strlen(newname) + 1, "MeshGeometryClass::MeshName"));
...
void MeshGeometryClass::Set_User_Text(char *usertext) {
  ...
  if (usertext) {
    UserText = NEW_REF(ShareBufferClass<char>,
                       (strlen(usertext) + 1, "MeshGeometryClass::UserText"));
...
/***********************************************************************************************
 * MeshGeometryClass::Generate_Rigid_APT -- generate an apt using backface
 *culling             *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void MeshGeometryClass::Generate_Skin_APT(const OBBoxClass &world_box,
  ...
  for (int poly_counter = 0; poly_counter < PolyCount; poly_counter++) {
    ...
    // We would have to do a non-trivial amount of computation to get the
    // triangle normal (since this is a skin we have no valid plane equations)
    // and we know that N is not used in the Intersection_Test, so we set it to
    // a dummy value.
...
bool MeshGeometryClass::Contains(const Vector3 &point) {
  // To determine whether the mesh contains a point, we will use the parity
  // method (cast a semi-infinite ray from the point and check the number of
  // intersections with the mesh. An even number of intersections means the
  // point is outside the mesh, and an odd number means the point is inside the
  // mesh. Since we can cast the ray in any direction we will use axis-aligned
  // rays for speed.

  // The method fails if the ray goes through any triangle edge or corner. For
  // robustness we will cast rays in all six axis-aligned directions, and take a
  // majority vote. We will significantly reduce the weighting of rays which go
  // through corners/edges. Also, if any raycast reports that the point is
  // embedded in a triangle, we will stop and report that the point is contained
  // in the mesh (using the normal algorithm in this case could lead to errors).
  float yes = 0.0f;  // weighted sum of rays indicating the point is contained
                     // in the mesh
  float no = 0.0f;   // weighted sum of rays indicating the point is not
                     // contained in the mesh
  for (int axis_dir = 0; axis_dir < 6; axis_dir++) {
    unsigned char flags = TRI_RAYCAST_FLAG_NONE;
    int intersections =
        cast_semi_infinite_axis_aligned_ray(point, axis_dir, flags);
...
/***********************************************************************************************
 *   MeshGeometryClass::Cast_World_Space_AABox -- test for intersection with a
 *worldspace AABox*
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
bool MeshGeometryClass::Cast_World_Space_AABox(AABoxCollisionTestClass &boxtest,
  ...
  ** NOTE: This code assumes that the matrix is orthogonal!  Much code in W3D
  *assumes
  ** orthogonal matrices, if that convention is broken this code is also broken.
  ** Basically what I'm doing here is doing the minimum number of compares
  *needed
  ...
  */
  ...
  if ((transform[0][0] == 1.0f) && (transform[1][1] == 1.0f)) {
    ...
    */
    if (hit) {
      Matrix3D::Rotate_Vector(transform, obbox.Result->Normal,
                              &(obbox.Result->Normal));
      if (boxtest.Result->ComputeContactPoint) {
        Matrix3D::Transform_Vector(transform, obbox.Result->ContactPoint,
                                   &(obbox.Result->ContactPoint));
      }
    }
  }
...
int MeshGeometryClass::cast_semi_infinite_axis_aligned_ray(
    const Vector3 &start_point, int axis_dir, unsigned char &flags) {
  int count = 0;
  if (CullTree) {
    count = CullTree->Cast_Semi_Infinite_Axis_Aligned_Ray(start_point, axis_dir,
                                                          flags);
  } else {
    const Vector3 *loc = Get_Vertex_Array();
    ...
    // These tables translate between the axis_dir representation (which is an
    // integer in which 0 indicates a ray along the positive x axis, 1 along the
    // negative x axis, 2 the positive y axis, 3 negative y axis, 4 positive z
    // axis, 5 negative z axis) and a four-integer representation (axis_r is the
    // axis number - 0, 1 or 2 - of the axis along which the ray is cast; axis_1
    // and axis_2 are the axis numbers of the other two axes; direction is 0 for
    ...
    // The functions called after this point will 'or' bits into this variable,
    // so it needs to be initialized here to TRI_RAYCAST_FLAG_NONE.
    ...
    */
    ...
    for (int poly_counter = 0; poly_counter < poly_count; poly_counter++) {
      ...
      count += (unsigned int)Cast_Semi_Infinite_Axis_Aligned_Ray_To_Triangle(
          v0, v1, v2, tri_plane, start_point, axis_r[axis_dir],
          axis_1[axis_dir], axis_2[axis_dir], direction[axis_dir], flags);
    }
  }
...
/***********************************************************************************************
 * MeshGeometryClass::cast_aabox_z90 -- aabox test which is rotated about z by
 *90              *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
bool MeshGeometryClass::cast_aabox_z90(AABoxCollisionTestClass &boxtest,
  ...
  // if we hit something, we need to rotate the normal back out of the mesh
  // coordinate system
  if (hit) {
...
/***********************************************************************************************
 * MeshGeometryClass::cast_aabox_z180 -- aabox test which is rotated about z by
 *180            *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * MeshGeometryClass::cast_aabox_z270 -- aabox test which is rotated about z by
 *270            *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 3/1/2001  NH : Created. *
 *=============================================================================================*/
bool MeshGeometryClass::intersect_obbox_brute_force(
    OBBoxIntersectionTestClass &localtest) {
  TriClass tri;
  const Vector3 *loc = Get_Vertex_Array();
  const Vector3i *polyverts = Get_Polygon_Array();
 ...
 * HISTORY: * 3/1/2001  NH : Created. *
 *=============================================================================================*/
bool MeshGeometryClass::cast_aabox_brute_force(
  ...
  */
  ...
  for (int srtri = 0; srtri < Get_Polygon_Count(); srtri++) {
    ...
    if (CollisionMath::Collide(boxtest.Box, boxtest.Move, tri,
                               boxtest.Result)) {
      polyhit = srtri;
    }
 ...
 * HISTORY: * 3/1/2001  NH : Created. *
 *=============================================================================================*/
bool MeshGeometryClass::cast_obbox_brute_force(
  ...
  */
  ...
  for (int srtri = 0; srtri < Get_Polygon_Count(); srtri++) {
    ...
    if (CollisionMath::Collide(boxtest.Box, boxtest.Move, tri, Vector3(0, 0, 0),
                               boxtest.Result)) {
      polyhit = srtri;
    }
...
/***********************************************************************************************
 * MeshGeometryClass::Compute_Plane_Equations -- Recalculates the plane
 *equations              *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * This function should not normally be needed during run-time. *
 *    The array pointer must point to enough memory to hold a plane equation per
 *polygon       *
 *                                                                                             *
 * HISTORY: * 11/9/2000  gth : Created. *
 *=============================================================================================*/
void MeshGeometryClass::Compute_Plane_Equations(Vector4 *peq) {
  ...
  for (int pidx = 0; pidx < PolyCount; pidx++) {
    ...
    peq[pidx].Set(normal.X, normal.Y, normal.Z,
 ...
 *                                                                                             *
 * WARNINGS: * This function should not normally be needed during run-time. *
 *		The array pointer must point to an array of Vector3's of size
 *NumVerts                   *
 *                                                                                             *
 * HISTORY: * 11/9/2000  gth : Created. *
 *=============================================================================================*/
...
Vector4 *MeshGeometryClass::get_planes(bool create) {
  ...
  if (create && !PlaneEq) {
    PlaneEq = NEW_REF(ShareBufferClass<Vector4>,
                      (PolyCount, "MeshGeometryClass::PlaneEq"));
  }
  if (PlaneEq) {
...
/***********************************************************************************************
 * MeshGeometryClass::Get_Plane_Array -- validates and returns the array of
 *plane equations    *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void MeshGeometryClass::Generate_Culling_Tree(void) {
  WWMEMLOG(MEM_CULLINGDATA);
  {
    ...
    builder.Build_AABTree(PolyCount, Poly->Get_Array(), VertexCount,
                          Vertex->Get_Array());
 ...
 *                                                                                             *
 *    This function will extract just the geometry information from a W3D mesh.
 *It must       * be completely replaced by any derived classes that want to
 *handle all of the chunks.     *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
...
WW3DErrorType MeshGeometryClass::Load_W3D(ChunkLoadClass &cload) {
  /*
  ** This function will initialize this MeshGeometryClass from the contents of a
  *W3D file.
  ** Note that derived classes need to completely replace this function; only
  *re-using the individual
  ...
  */
  ...
  if (cload.Read(&header, sizeof(W3dMeshHeader3Struct)) !=
  ...
  */
  ...
  BoundSphereCenter.Set(header.SphCenter.X, header.SphCenter.Y,
                        header.SphCenter.Z);
 ...
 * MeshGeometryClass::read_chunks -- read w3d chunks *
 *                                                                                             *
 *    Again, derived classes must replace this function with one that handles
 *all of their     * chunks in addition to calling to this class for the
 *individual chunks that it handles.   *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
...
/***********************************************************************************************
 * MeshGeometryClass::read_vertex_normals -- read the vertex normals chunk from
 *a w3d file     *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
WW3DErrorType MeshGeometryClass::read_user_text(ChunkLoadClass &cload) {
  ...
  */
  UserText =
      NEW_REF(ShareBufferClass<char>, (textlen, "MeshGeometryClass::UserText"));
...
/***********************************************************************************************
 * MeshGeometryClass::read_vertex_influences -- read the vertex influences chunk
 *from a w3d fi *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
WW3DErrorType MeshGeometryClass::read_vertex_influences(ChunkLoadClass &cload) {
  ...
  for (int i = 0; i < Get_Vertex_Count(); i++) {
    if (cload.Read(&vinf, sizeof(W3dVertInfStruct)) !=
...
/***********************************************************************************************
 * MeshGeometryClass::read_vertex_shade_indices -- read the vertex shade indices
 *chunk         *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 11/9/2000  gth : Created. *
 *=============================================================================================*/
WW3DErrorType MeshGeometryClass::read_vertex_shade_indices(
