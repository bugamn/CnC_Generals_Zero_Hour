 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * RingRenderObjClass::RingRenderObjClass -- Constructor *
 *   RingRenderObjClass::RingRenderObjClass -- Constructor - init from a
 *definition            * RingRenderObjClass::RingRenderObjClass -- Copy
 *constructor                                *
 ...
 *   RingRenderObjClass::Init_Ring_Render_System -- global initialization needed
 *for Ring		  * RingRenderObjClass::Shutdown_Ring_Render_System --
 *cleanup Ring render system				  *
 *   RingRenderObjClass::Set_Ring_Display_Mask -- Sets global display mask for
 *all Ringes		  * RingRenderObjClass::Get_Ring_Display_Mask -- returns
 *the display mask							  *
 ...
 ** RingRenderObjClass::update_cached_Ring -- update the world-space version of
 *this Ring	  * RingRenderObjClass::Cast_Ray -- cast a ray against this Ring
 ...
 ** RingRenderObjClass::Get_Obj_Space_Bounding_Ring -- return the object-space
 *bounding sp	  * RingRenderObjClass::Get_Obj_Space_Bounding_Ring -- returns
 *the obj-space bounding Sphe	  *
 *   RingRenderObjClass::Update_Cached_Bounding_Volumes -- Updates world-space
 *bounding volum  *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
#define STATIC_SORT_RINGS \
  1  // makes all ring use a static sort level rather than per-poly sorting
...
class RingMeshClass {
 ...
 private:
  ...
  Vector3i *tri_poly;  // array of triangle polys, vertex indices (can be
                       // discard if switch to fan renderer)
};
 ...
 * HISTORY: * 1/19/00    gth : Created. *
 *=============================================================================================*/
RingRenderObjClass &RingRenderObjClass::operator=(
    const RingRenderObjClass &that) {
  if (this != &that) {
...
/***********************************************************************************************
 * RingRenderObjClass::Generate_Shared_Mesh_Arrays  -- Generates mesh LOD
 *arrays.				  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * RingRenderObjClass::Init_Material -- Sets up the material and default shader
 *for the Ring.*
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
#else
  SortingRendererClass::Insert_Triangles(Get_Bounding_Sphere(), 0, ring.face_ct,
                                         0, ring.Vertex_ct);
#endif
 ...
 * HISTORY: * 1/19/00    gth : Created. *
 *=============================================================================================*/
void RingRenderObjClass::vis_render_ring(SpecialRenderInfoClass &rinfo,
                                         const Vector3 &center,
                                         const Vector3 &extent) {
  WWASSERT(0);
}  // vis_render_ring
...
void RingRenderObjClass::Render(RenderInfoClass &rinfo) {
  ...
  if (WW3D::Are_Static_Sort_Lists_Enabled() && sort_level != SORT_LEVEL_NONE) {
    ...
    // RING_NUM_LOD to set the CurrentLOD
    ...
    //
    Vector2 inner_scale(InnerExtent.X * InnerScale.X,
                        InnerExtent.Y * InnerScale.Y);
    Vector2 outer_scale(OuterExtent.X * OuterScale.X,
                        OuterExtent.Y * OuterScale.Y);
    ...
    if (Flags & USE_CAMERA_ALIGN) {
      ...
      Matrix3D tm(1.0f, 0.0f, 0.0f, cpos.x, 0.0f, 1.0f, 0.0f, cpos.y, 0.0f,
                  0.0f, 1.0f, cpos.z);
...
/***********************************************************************************************
 * RingRenderObjClass::Get_Default_Color - get the default (or first frame)
 *value				  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * RingRenderObjClass::Get_Default_Alpha - get the default (or first frame)
 *value				  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * RingRenderObjClass::Get_Default_Inner_Scale - get the default (or first
 *frame) value		  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * RingRenderObjClass::Get_Default_Outer_Scale - get the default (or first
 *frame) value		  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * RingRenderObjClass::update_cached_box -- update the world-space version of
 *this box        *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 1/19/00    gth : Created. *
 *=============================================================================================*/
void RingRenderObjClass::Get_Obj_Space_Bounding_Sphere(
...
/***********************************************************************************************
 * RingRenderObjClass::Get_Obj_Space_Bounding_Box -- returns the obj-space
 *bounding box       *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
RingMeshClass::RingMeshClass(void)
    : Radius(0.0f),
      ...
      TileCount(5),
      InnerScale(1.0F, 1.0F),
      OuterScale(1.0F, 1.0F) {}  // empty ringmesh Constructor
...
void RingMeshClass::Scale(const Vector2 &inner_scale,
                          const Vector2 &outer_scale) {
