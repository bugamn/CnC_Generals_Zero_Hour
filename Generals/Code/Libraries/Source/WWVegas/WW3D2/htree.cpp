 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * HTreeClass::HTreeClass -- constructor * HTreeClass::~HTreeClass
 *-- destructor                                                     *
 *   HTreeClass::Load -- loads a hierarchy tree from a file *
 *   HTreeClass::read_pivots -- reads the pivots out of a file *
 *   HTreeClass::Free -- de-allocate all memory in use * HTreeClass::Base_Update
 *-- Computes the base pose transform for each pivot                *
 *   HTreeClass::Anim_Update -- Computes the transform for each pivot with
 *motion              * HTreeClass::Blend_Update -- computes each pivot as a
 *blend of two anims                   * HTreeClass::Combo_Update -- compute
 *each pivot's transform using an anim combo            *
 *   HTreeClass::Get_Transform -- returns the transformation for the desired
 *pivot             * HTreeClass::Find_Bone -- Find a bone by name *
 *   HTreeClass::Get_Bone_Name -- get the name of a bone from its index *
 *   HTreeClass::Update_Parent_Need_Bits -- all "needed" children force their
 *parents to be nee* HTreeClass::HTreeClass -- copy constructor *
 *   HTreeClass::Get_Parent_Index -- returns index of the parent of the given
 *bone             *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 08/11/1997 GH  : Created. *
 *=============================================================================================*/
HTreeClass::HTreeClass(void) : NumPivots(0), Pivot(NULL), ScaleFactor(1.0f) {}
 ...
 * HISTORY: * 08/11/1997 GH  : Created. *
 *=============================================================================================*/
HTreeClass::~HTreeClass(void) { Free(); }
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 3/4/98     GTH : Created. *
 *=============================================================================================*/
HTreeClass::HTreeClass(const HTreeClass &src)
    : NumPivots(0), Pivot(NULL), ScaleFactor(1.0f) {
...
int HTreeClass::Load_W3D(ChunkLoadClass &cload) {
  ...
  */
  ...
  if (cload.Read(&header, sizeof(W3dHierarchyStruct)) !=
      sizeof(W3dHierarchyStruct)) {
    return LOAD_ERROR;
  }
...
bool HTreeClass::read_pivots(ChunkLoadClass &cload, bool pre30) {
  ...
  /*
  ** At (w3d file format) version 3.0, I added a node for the root.  Pre-3.0
  *htrees didn't have
  ...
  */
  ...
  for (int pidx = first_piv; pidx < NumPivots; pidx++) {
    ...
    Pivot[pidx].BaseTransform.Translate(
        Vector3(piv.Translation.X, piv.Translation.Y, piv.Translation.Z));

#ifdef ALLOW_TEMPORARIES
    Pivot[pidx].BaseTransform =
        Pivot[pidx].BaseTransform *
        Build_Matrix3D(Quaternion(piv.Rotation.Q[0], piv.Rotation.Q[1],
                                  piv.Rotation.Q[2], piv.Rotation.Q[3]),
                       mtx);
#else
...
/***********************************************************************************************
 * HTreeClass::Simple_Evaluate_Pivot -- Returns the transform of a pivot at the
 *given frame.	  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
bool HTreeClass::Simple_Evaluate_Pivot(HAnimClass *motion, int pivot_index,
  ...
  if (motion != NULL && end_tm != NULL && pivot_index >= 0 &&
    ...
    for (PivotClass *pivot = &Pivot[pivot_index];
         pivot != NULL && pivot->Parent != NULL; pivot = pivot->Parent) {
      //
      ...
      //			Quaternion q;
      //			motion->Get_Orientation (q, pivot->Index,
      //frame); 			Matrix3D anim_tm = ::Build_Matrix3D(q);
      ...
      //
      //	Transform the animation transform by the 'relative-to-parent'
      //transform.
      //
      Matrix3D curr_tm;
...
void HTreeClass::Base_Update(const Matrix3D &root) {
  ...
  for (int piv_idx = 1; piv_idx < NumPivots; piv_idx++) {
    ...
    Matrix3D::Multiply(pivot->Parent->Transform, pivot->BaseTransform,
                       &(pivot->Transform));
    pivot->IsVisible = 1;
 ...
 * HISTORY: * 08/11/1997 GH  : Created. *
 *=============================================================================================*/
void HTreeClass::Anim_Update(const Matrix3D &root, HAnimClass *motion,
                             float frame) {
  PivotClass *pivot;
  Matrix3D mtx;
  ...
  for (int piv_idx = 1; piv_idx < NumPivots; piv_idx++) {
    ...
    // Don't update this pivot if the HTree doesn't have animation data for
    // it...
...
/*Customized version of the above which excludes interpolation and assumes
HRawAnimClass For use by 'Generals' -MW*/
void HTreeClass::Anim_Update(const Matrix3D &root, HRawAnimClass *motion,
                             float frame) {
  ...
  if (iframe >= motion->Get_Num_Frames()) iframe = 0;

  Vector3 trans;
  Quaternion q;
  Matrix3D mtx;

  struct NodeMotionStruct *nodeMotion =
      ((HRawAnimClass *)motion)->Get_Node_Motion_Array();
  ...
  for (int piv_idx = 1; pivot < endpivot; pivot++, nodeMotion++) {
    ...
    if (pivot < lastAnimPivot) {
      ...
      if (nodeMotion->X != NULL) nodeMotion->X->Get_Vector(iframe, &(trans[0]));
      if (nodeMotion->Y != NULL) nodeMotion->Y->Get_Vector(iframe, &(trans[1]));
      if (nodeMotion->Z != NULL) nodeMotion->Z->Get_Vector(iframe, &(trans[2]));
 ...
 * HISTORY: * 3/4/98     GTH : Created. *
 *=============================================================================================*/
void HTreeClass::Blend_Update(const Matrix3D &root, HAnimClass *motion0,
                              float frame0, HAnimClass *motion1, float frame1,
  ...
  int num_anim_pivots =
      MIN(motion0->Get_Num_Pivots(), motion1->Get_Num_Pivots());
  ...
  for (int piv_idx = 1; piv_idx < NumPivots; piv_idx++) {
    ...
    if (piv_idx < num_anim_pivots) {
      ...
      pivot->IsVisible = (motion0->Get_Visibility(piv_idx, frame0) ||
                          motion1->Get_Visibility(piv_idx, frame1));
    }
...
/***********************************************************************************************
 * HTreeClass::Combo_Update -- compute each pivot's transform using an anim
 *combo              *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void HTreeClass::Combo_Update(const Matrix3D &root, HAnimComboClass *anim) {
  ...
  for (int anim_num = 0; anim_num < anim->Get_Num_Anims(); anim_num++) {
    num_anim_pivots =
        MIN(num_anim_pivots, anim->Peek_Motion(anim_num)->Get_Num_Pivots());
  }
...
#define ASSUME_NORMALIZED_ANIM_COMBO_WEIGHTS
      ...
      if (weight_total != 0.0f) {
        // SKB: Removed assert because I have a case where I don't want
        // normalization. 	  One anim moves X, the other moves Y.  Assert was just
        // in to warn programmers.
        //				WWASSERT(WWMath::Fabs( weight_total
        //- 1.0 ) < WWMATH_EPSILON);
      ...
      for (anim_num = 0;
           (anim_num < anim->Get_Num_Anims()) && (!pivot->IsVisible);
           anim_num++) {
        HAnimClass *motion = anim->Get_Motion(anim_num);
        if (motion != NULL) {
          float frame_num = anim->Get_Frame(anim_num);
 ...
 **
 *                                                                                             *
 * INPUT: * boneidx - the bone you are interested in *
 *                                                                                             *
 * OUTPUT: * the index of that bone's parent *
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 4/12/2000  gth : Created. *
 *=============================================================================================*/
...
void HTreeClass::Capture_Bone(int boneindex) {
  ...
  if (Pivot[boneindex].CapTransformPtr == NULL) {
    Pivot[boneindex].CapTransformPtr =
        MSGW3DNEW("PivotClassCaptureBoneMtx") DynamicMatrix3D;
...
void HTreeClass::Control_Bone(int boneindex, const Matrix3D &relative_tm,
...
#ifdef LAZY_CAP_MTX_ALLOC
  if (Pivot[boneindex].CapTransformPtr == NULL) return;
...
HTreeClass *HTreeClass::Create_Morphed(int num_morph_sources,
  ...
  for (int pi = 0; pi < new_tree->NumPivots; pi++) {
    ...
    for (int nm = 0; nm < num_morph_sources; nm++) {
      pos.X += tree_array[nm]->Pivot[pi].BaseTransform.Get_Translation().X *
               morph_weights[nm];
      pos.Y += tree_array[nm]->Pivot[pi].BaseTransform.Get_Translation().Y *
               morph_weights[nm];
      pos.Z += tree_array[nm]->Pivot[pi].BaseTransform.Get_Translation().Z *
               morph_weights[nm];
    }
...
HTreeClass *HTreeClass::Create_Interpolated(const HTreeClass *tree_a0_b0,
  ...
  for (int pi = 0; pi < new_tree->NumPivots; pi++) {
    Vector3::Lerp(tree_a0_b0->Pivot[pi].BaseTransform.Get_Translation(),
                  tree_a0_b1->Pivot[pi].BaseTransform.Get_Translation(), lerp_b,
                  &pos_a0);
    Vector3::Lerp(tree_a1_b0->Pivot[pi].BaseTransform.Get_Translation(),
                  tree_a1_b1->Pivot[pi].BaseTransform.Get_Translation(), lerp_b,
                  &pos_a1);
  ...
  if (a_scale_abs + b_scale_abs > 0) {
    ...
    for (int pi = 0; pi < new_tree->NumPivots; pi++) {
      Vector3::Lerp(tree_base->Pivot[pi].BaseTransform.Get_Translation(),
                    tree_a->Pivot[pi].BaseTransform.Get_Translation(), a_scale,
                    &pos_a);
      Vector3::Lerp(tree_base->Pivot[pi].BaseTransform.Get_Translation(),
                    tree_b->Pivot[pi].BaseTransform.Get_Translation(), b_scale,
                    &pos_b);

      pos = (pos_a * a_scale_abs + pos_b * b_scale_abs) /
            (a_scale_abs + b_scale_abs);
