...
DDSFileClass::DDSFileClass(const char* name, unsigned reduction_factor)
  ...
  // The name could be given in .tga or .dds format, so ensure we're opening
  // .dds...
  int len = strlen(Name);
  Name[len - 3] = 'd';
  Name[len - 2] = 'd';
  ...
  WWASSERT(Format == WW3D_FORMAT_DXT1 || Format == WW3D_FORMAT_DXT2 ||
  ...
  if (MipLevels <= WW3D::Get_Texture_Min_Mip_Levels())
    ...
    if (ReductionFactor >= mipToDrop) ReductionFactor = mipToDrop;
  }
  ...
  unsigned level_size = Calculate_DXTC_Surface_Size(SurfaceDesc.Width,
                                                    SurfaceDesc.Height, Format);
  ...
  for (unsigned level = 0; level < ReductionFactor; ++level) {
    ...
    if (level_size >
        16) {  // If surface is bigger than one block (8 or 16 bytes)...
      level_size /= 4;
    }
  }
...
// For some reason DX-Tex tool doesn't fill the surface size field, so we need
// to calculate it...
unsigned DDSFileClass::Calculate_DXTC_Surface_Size(unsigned width,
                                                   unsigned height,
                                                   WW3DFormat format) {
...
void DDSFileClass::Copy_Level_To_Surface(unsigned level,
  ...
  Copy_Level_To_Surface(level, D3DFormat_To_WW3DFormat(surface_desc.Format),
...
void DDSFileClass::Copy_Level_To_Surface(unsigned level, WW3DFormat dest_format,
                                         unsigned dest_width,
  ...
  if (dest_format == Format && dest_width == Get_Width(level) &&
      dest_height == Get_Height(level)) {
    ...
    if (dest_width == Get_Width(level) && dest_height == Get_Height(level)) {
      ...
      // This is needed on NVidia cards that have problems with DXT1
      // compression.
      if (Format == WW3D_FORMAT_DXT1 && dest_format == WW3D_FORMAT_DXT2) {
        const unsigned* src_ptr =
            reinterpret_cast<const unsigned*>(Get_Memory_Pointer(level));
        ...
        for (unsigned y = 0; y < dest_height; y += 4) {
          ...
          for (unsigned x = 0; x < dest_width;
               x += 4, dest_ptr += dest_bpp * 4) {
            contains_alpha |=
                Get_4x4_Block(dest_ptr, dest_pitch, dest_format, level, x, y);
          }
        }
        if (Format == WW3D_FORMAT_DXT1 && contains_alpha) {
          WWDEBUG_SAY(
              ("Warning: DXT1 format should not contain alpha information - "
               "file %s\n",
               Name));
        }
      }
...
WWINLINE static unsigned Combine_Colors(unsigned col1, unsigned col2,
                                        unsigned rel) {
...
// ----------------------------------------------------------------------------
//
// Note that this is NOT an efficient way of extracting pixels from compressed
// image - we should implement faster block-copy method for non-scaled copying.
//
// ----------------------------------------------------------------------------
...
unsigned DDSFileClass::Get_Pixel(unsigned level, unsigned x, unsigned y) const {
  ...
  switch (Format) {
    // Note that we don't currently really support alpha on DXT1 - all alpha
    // textures should use DXT5. The reason for this is that when converting
    // from DXT1 to 16 bit uncompressed texture we want to be able to use RGB565
    // format instead of ARGB4444. As the alpha is encoded in DXT1 per-block
    // basis there isn't really a way to tell if the surface has an alpha or not
    // so either we use alpha or we don't.
    case WW3D_FORMAT_DXT1: {
      const unsigned char* block_memory =
          Get_Memory_Pointer(level) + (x / 4) * 8 +
          ((y / 4) * (Get_Width(level) / 4)) * 8;
    ...
    case WW3D_FORMAT_DXT4:
      return 0xffffffff;
    case WW3D_FORMAT_DXT5: {
      const unsigned char* alpha_block =
          Get_Memory_Pointer(level) + (x / 4) * 16 +
          ((y / 4) * (Get_Width(level) / 4)) * 16;
      ...
      if (alpha0 > alpha1) {
        ...
        switch (alpha_index) {
          ...
          case 1:
            ...
            break;
          case 2:
            alpha_value = (6 * alpha0 + 1 * alpha1 + 3) / 7;
            break;  // bit code 010
          case 3:
            alpha_value = (5 * alpha0 + 2 * alpha1 + 3) / 7;
            break;  // bit code 011
          case 4:
            alpha_value = (4 * alpha0 + 3 * alpha1 + 3) / 7;
            break;  // bit code 100
          case 5:
            alpha_value = (3 * alpha0 + 4 * alpha1 + 3) / 7;
            break;  // bit code 101
          case 6:
            alpha_value = (2 * alpha0 + 5 * alpha1 + 3) / 7;
            break;  // bit code 110
          case 7:
            alpha_value = (1 * alpha0 + 6 * alpha1 + 3) / 7;
            break;  // bit code 111
        }
      } else {
        // 6-alpha block.
        ...
        switch (alpha_index) {
          ...
          case 1:
            ...
            break;
          case 2:
            alpha_value = (4 * alpha0 + 1 * alpha1 + 2) / 5;
            break;  // Bit code 010
          case 3:
            alpha_value = (3 * alpha0 + 2 * alpha1 + 2) / 5;
            break;  // Bit code 011
          case 4:
            alpha_value = (2 * alpha0 + 3 * alpha1 + 2) / 5;
            break;  // Bit code 100
          case 5:
            alpha_value = (1 * alpha0 + 4 * alpha1 + 2) / 5;
            break;  // Bit code 101
          case 6:
            alpha_value = 0;
            break;  // Bit code 110
...
bool DDSFileClass::Get_4x4_Block(
    ...
    unsigned source_y)
    const  // DDS y offset to copy from, must be aligned by 4!
{
  ...
  switch (Format) {
    ...
    case WW3D_FORMAT_DXT1: {
      const unsigned char* block_memory =
          Get_Memory_Pointer(level) + (source_x / 4) * 8 +
          ((source_y / 4) * (Get_Width(level) / 4)) * 8;

      unsigned col0 = RGB565_To_ARGB8888(*(unsigned short*)&block_memory[0]);
      unsigned col1 = RGB565_To_ARGB8888(*(unsigned short*)&block_memory[2]);
      // Even if we don't support alpha, decompression is different if source
      // has alpha
      ...
      if (col0 > col1) {
        for (int y = 0; y < 4; ++y) {
          ...
          for (int x = 0; x < 4; ++x) {
            switch (line & 3) {
              case 0:
                dest_pixel = col0 | 0xff000000;
                break;
              case 1:
                dest_pixel = col1 | 0xff000000;
                break;
              case 2:
                dest_pixel = Combine_Colors(col1, col0, 85) | 0xff000000;
                break;
              case 3:
                dest_pixel = Combine_Colors(col0, col1, 85) | 0xff000000;
                break;
            }
            line >>= 2;

            BitmapHandlerClass::Write_B8G8R8A8(tmp_dest_ptr, dest_format,
                                               dest_pixel);
          ...
          for (int x = 0; x < 4; ++x) {
            switch (line & 3) {
              ...
              case 1:
                ...
                break;
              case 2:
                dest_pixel = Combine_Colors(col1, col0, 128) | 0xff000000;
                break;
              case 3:
                dest_pixel = 0x00000000;
                break;
            }
            line >>= 2;
    ...
    case WW3D_FORMAT_DXT5: {
      // Init alphas
      const unsigned char* alpha_block =
          Get_Memory_Pointer(level) + (source_x / 4) * 16 +
          ((source_y / 4) * (Get_Width(level) / 4)) * 16;
      ...
      for (int y = 0; y < 4; ++y) {
        ...
        for (int x = 0; x < 4; ++x, bit_idx += 3) {
          ...
          switch (line & 3) {
            case 0:
              dest_pixel = col0 | alpha_value;
              break;
            case 1:
              dest_pixel = col1 | alpha_value;
              break;
            case 2:
              dest_pixel = Combine_Colors(col1, col0, 85) | alpha_value;
              break;
            case 3:
              dest_pixel = Combine_Colors(col0, col1, 85) | alpha_value;
              break;
          }
          line >>= 2;
                              ...
                              for (int y=0;y<4;++y) {
                                      ...
                                      for (int x=0;x<4;++x,bit_idx+=3) {
                                              ...
                                              for (int i=0;i<3;++i) {
                                                      WWASSERT(byte_idx<6);
                                                      unsigned
         alpha_bit=(alpha_block[2+byte_idx]>>(tmp_bit_idx))&1;
                                                      alpha_index|=alpha_bit<<(i);
                                                      tmp_bit_idx++;
                                                      if (tmp_bit_idx>=8) {
                                                              tmp_bit_idx=0;
                                              ...
                                              WWASSERT(alpha_index<8);
                                              unsigned
         alpha_value=alphas[alpha_index]; contains_alpha&=alpha_value;
                                              alpha_value<<=24;
                                              ...
                                              switch (line&3) {
                                              case 0:
         dest_pixel=col0|alpha_value; break; case 1:
         dest_pixel=col1|alpha_value; break; case 2:
         dest_pixel=Combine_Colors(col1,col0,85)|alpha_value; break; case 3:
         dest_pixel=Combine_Colors(col0,col1,85)|alpha_value; break;
                                              }
                                              line>>=2;
      ...
      */
      return contains_alpha !=
             0xff;  // Alpha block... DXT5 should only be used when the image
                    // needs alpha but for now check anyway...
    }
  }
  return false;
}
