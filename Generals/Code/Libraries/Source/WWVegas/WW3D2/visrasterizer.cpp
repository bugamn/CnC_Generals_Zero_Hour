...
void VisPolyClass::Reset(void) { Verts.Delete_All(false); }

void VisPolyClass::Add_Vertex(const Vector3 &point) { Verts.Add(point); }
...
void VisPolyClass::Clip(const PlaneClass &plane, VisPolyClass &dest) const {
  ...
  // perform clipping
  prev_point_in_front = !plane.In_Front(
      Verts[iprev]);  // Note, plane normal is outward so we invert this test
  for (int j = 0; j < vcount; j++) {
    cur_point_in_front = !plane.In_Front(
        Verts[i]);  // Note, plane nomral is out so we invert this test
...
VisRasterizerClass::VisRasterizerClass(void)
    : ModelTransform(1), Camera(NULL), MVTransform(1) {}

VisRasterizerClass::~VisRasterizerClass(void) { REF_PTR_RELEASE(Camera); }
...
CameraClass *VisRasterizerClass::Peek_Camera(void) { return Camera; }
...
const Matrix3D &VisRasterizerClass::Get_MV_Transform(void) {
  // TODO: optimize this
  Update_MV_Transform();  // the user can and does mess with the camera
                          // directly!
  return MVTransform;
}
...
bool VisRasterizerClass::Render_Triangles(const Vector3 *verts, int vcount,
                                          const Vector3i *tris, int tcount,
  ...
  */
  if (CollisionMath::Overlap_Test(Camera->Get_Frustum(), bounds) ==
...
bool VisRasterizerClass::Render_Triangles_No_Clip(const Vector3 *verts,
  ...
  */
  for (int tri_index = 0; tri_index < tcount; tri_index++) {
    ...
    pixel_passed |=
        IDBuffer.Render_Triangle(tverts[tri.I], tverts[tri.J], tverts[tri.K]);
    if (pixel_passed &&
        (IDBuffer.Get_Render_Mode() == IDBufferClass::NON_OCCLUDER_MODE)) {
      return true;
    }
  }
  return pixel_passed;
...
bool VisRasterizerClass::Render_Triangles_Clip(const Vector3 *verts, int vcount,
                                               const Vector3i *tris,
                                               int tcount) {
  bool pixel_passed = false;
  ...
  */
  ...
  /*
  ** 3. Clip triangles to the view volume and pass on to the ID buffer for scan
  *conversion
  */
  for (int tri_index = 0; tri_index < tcount; tri_index++) {
    /*
    ...
    if (final_vcount >= 3) {
      ...
      */
      for (i = 1; i < final_vcount - 1; i++) {
        pixel_passed |= IDBuffer.Render_Triangle(final_verts[0], final_verts[i],
                                                 final_verts[i + 1]);
        if (pixel_passed &&
            (IDBuffer.Get_Render_Mode() == IDBufferClass::NON_OCCLUDER_MODE)) {
          return true;
...
IDBufferClass::~IDBufferClass(void) { Reset(); }
...
*/
struct EdgeStruct {
  EdgeStruct(const GradientsStruct &grad, const Vector3 *verts, int top,
             int bottom) {
...
bool IDBufferClass::Render_Triangle(const Vector3 &p0, const Vector3 &p1,
                                    const Vector3 &p2) {
  ...
  */
  ...
  while (height--) {
    if (RenderMode == OCCLUDER_MODE) {
      ...
      pixels_passed +=
          Render_Non_Occluder_Scanline(grads, left_edge, right_edge);
    }
    left_edge->Step();
    right_edge->Step();
  }
