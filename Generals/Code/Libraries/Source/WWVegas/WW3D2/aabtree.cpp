 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * AABTreeClass::AABTreeClass -- Constructor *
 *   AABTreeClass::AABTreeClass -- Constructor * AABTreeClass::AABTreeClass --
 *copy constructor                                            *
 *   AABTreeClass::~AABTreeClass -- Destructor * AABTreeClass::operator =  --
 *assignment operator                                          *
 *   AABTreeClass::Reset -- reset this tree, releases all allocated resources *
 *   AABTreeClass::Build_Tree_Recursive -- Initializes this tree from the given
 *builder        * AABTreeClass::Set_Mesh -- set the mesh pointer *
 ...
 *   AABTreeClass::Cast_Semi_Infinite_Axis_Aligned_Ray_Recursive -- Internal
 *implementation    * AABTreeClass::Cast_AABox_Recursive -- internal
 *implementation of Cast_AABox               *
 *   AABTreeClass::Cast_OBBox_Recursive -- Internal implementation of Cast_OBBox
 ** AABTreeClass::Intersect_OBBox_Recursive -- internal implementation of
 *Intersect_OBBox     * AABTreeClass::Cast_Ray_To_Polys -- cast the ray to polys
 *in the given node                *
 *   AABTreeClass::Cast_Semi_Infinite_Axis_Aligned_Ray_To_Polys -- cast ray to
 *polys in the nod* AABTreeClass::Cast_AABox_To_Polys -- cast aabox to polys in
 *the given node                * AABTreeClass::Cast_OBBox_To_Polys -- cast
 *obbox to polys in the given node                *
 *   AABTreeClass::Intersect_OBBox_With_Polys -- Test polys for intersection *
 *   AABTreeClass::Update_Bounding_Boxes_Recursive -- recompute the bounding
 *boxes             * AABTreeClass::Update_Min_Max -- updates min and max given
 *a polygon index                 * AABTreeClass::Load_W3D -- Load a W3D
 *description of an AABTree                            *
 ...
 *   AABTreeClass::Generate_OBBox_APT_Recursive -- recurse, generate the apt for
 *a box and vie *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 6/19/98    GTH : Created. * 5/23/2000  gth : Created. *
 *=============================================================================================*/
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 5/19/2000  gth : Created. *
 *=============================================================================================*/
void AABTreeClass::Build_Tree_Recursive(
    AABTreeBuilderClass::CullNodeStruct *node, int &curpolyindex) {
  /*
  ...
  */
  ...
  /*
  ** If this is a non-leaf node, set up the child indices, otherwise set up the
  *polygon indices
  */
  if (node->Front != NULL) {
 ...
 * AABTreeClass::Generate_APT -- Generate an active poly table for the mesh *
 *                                                                                             *
 * INPUT: * box - oriented box to cull the mesh against (in the mesh's
 *coordinate system!!!)            * apt - vector of ints to fill the apt into *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
 ...
 * AABTreeClass::Generate_OBBox_APT_Recursive -- recursively generate the apt *
 *                                                                                             *
 * INPUT: * node - current node in the recursion * context - structure
 *containing the results/current state                                    *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 1/25/00    gth : Created. *
 *=============================================================================================*/
void AABTreeClass::Generate_OBBox_APT_Recursive(
    CullNodeStruct *node, OBBoxAPTContextStruct &context) {
  /*
  ** Test the volume against the bounding volume of this node
  ** If it is outside, stop descending the tree.
  */
  AABoxClass nodebox;
  nodebox.Init_Min_Max(
      node->Min, node->Max);  // NOTE: too bad we have to compute the nodebox!!!
 ...
 * AABTreeClass::Generate_APT -- generate an apt from a box and viewdir *
 *                                                                                             *
 * Indices of the polys which intersect the box and are not backfacing to the
 *given viewdir    * will be added to the passed in apt. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 ...
 *=============================================================================================*/
void AABTreeClass::Generate_APT(const OBBoxClass &box, const Vector3 &viewdir,
                                SimpleDynVecClass<uint32> &apt) {
...
/***********************************************************************************************
 * AABTreeClass::Generate_OBBox_APT_Recursive -- recurse, generate the apt for a
 *box and viewd *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 *=============================================================================================*/
void AABTreeClass::Generate_OBBox_APT_Recursive(
    CullNodeStruct *node, OBBoxRayAPTContextStruct &context) {
  /*
  ** Test the volume against the bounding volume of this node
  ** If it is outside, stop descending the tree.
  */
 ...
 * AABTreeClass::Cast_Ray_Recursive -- Internal implementation of Cast_Ray *
 *                                                                                             *
 * INPUT: * raytest - contains all of the ray test information * node - current
 *cull node being processed                                                    *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
 ...
 * HISTORY: * 6/19/98    GTH : Created. *
 *=============================================================================================*/
bool AABTreeClass::Cast_Ray_Recursive(CullNodeStruct *node,
 ...
 *implementation      *
 *                                                                                             *
 * INPUT: * node - current cull node being processed * start_point - starting
 *point                                                                * axis_r
 *- the axis along which the ray is projected * axis_1, axis_2 - the other two
 *axes                                                         * direction - 0
 *means the ray goes toward the negative axis, 1 means positive. * flags -
 *reference bitfield for result flags (ray hit edge, start point embedded in
 *tri, etc)*
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 ...
 *=============================================================================================*/
int AABTreeClass::Cast_Semi_Infinite_Axis_Aligned_Ray_Recursive(
    CullNodeStruct *node, const Vector3 &start_point, int axis_r, int axis_1,
    int axis_2, int direction, unsigned char &flags) {
  /*
  ** Cull the ray against the bounding volume of this node
  ** If it is culled, stop descending the tree.
  ** We do two main tests: a 2D test against axis1 and axis2 to see if the
  *starting point (and
  ** hence the ray) falls within the 2D projection of the bbox, and a 1D test to
  *see if the ray
  ** is completely above or below the bbox. The second test checks (start > max)
  *or (start < min)
  ** depending on the direction of the ray - we do this in a branchless fashion
  *by turning
  ** (start < min) into (-start > -min). Then we can use tables to perform the
  *correct check.
  */
  ...
  if (start_point[axis_1] < node->Min[axis_1] ||
      start_point[axis_2] < node->Min[axis_2] ||
      start_point[axis_1] > node->Max[axis_1] ||
      start_point[axis_2] > node->Max[axis_2] ||
  ...
  int count = 0;
  if (node->Is_Leaf()) {
    return Cast_Semi_Infinite_Axis_Aligned_Ray_To_Polys(
        node, start_point, axis_r, axis_1, axis_2, direction, flags);

  } else {
    count += Cast_Semi_Infinite_Axis_Aligned_Ray_Recursive(
        &(Nodes[node->Get_Front_Child()]), start_point, axis_r, axis_1, axis_2,
        direction, flags);
    count += Cast_Semi_Infinite_Axis_Aligned_Ray_Recursive(
        &(Nodes[node->Get_Back_Child()]), start_point, axis_r, axis_1, axis_2,
        direction, flags);
  }
 ...
 *                                                                                             *
 * INPUT: * boxtest - contains description of the collision operation to be
 *performed                   * node - current cull node being processed *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
 ...
 * HISTORY: * 6/19/98    GTH : Created. *
 *=============================================================================================*/
bool AABTreeClass::Cast_AABox_Recursive(CullNodeStruct *node,
  ...
  */
  ...
  if (node->Is_Leaf()) {
    ...
    res =
        res | Cast_AABox_Recursive(&(Nodes[node->Get_Front_Child()]), boxtest);
 ...
 * AABTreeClass::Cast_OBBox_Recursive -- Internal implementation of Cast_OBBox *
 *                                                                                             *
 * INPUT: * boxtest - contains description of the collision test to be performed
 ...
 * HISTORY: * 6/19/98    GTH : Created. *
 *=============================================================================================*/
bool AABTreeClass::Cast_OBBox_Recursive(CullNodeStruct *node,
  ...
  */
  ...
  if (node->Is_Leaf()) {
    ...
    res =
        res | Cast_OBBox_Recursive(&(Nodes[node->Get_Front_Child()]), boxtest);
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 1/20/00    gth : Created. *
 *=============================================================================================*/
bool AABTreeClass::Intersect_OBBox_Recursive(AABTreeClass::CullNodeStruct *node,
  ...
  */
  ...
  if (node->Is_Leaf()) {
    ...
    res = res |
          Intersect_OBBox_Recursive(&(Nodes[node->Get_Front_Child()]), test);
    res =
        res | Intersect_OBBox_Recursive(&(Nodes[node->Get_Back_Child()]), test);
  }
...
#ifdef _DEBUG
#pragma optimize( \
    "", off)  // We get an odd error when using optimized in the debug.
 ...
 * HISTORY: * 6/19/98    GTH : Created. *
 *=============================================================================================*/
bool AABTreeClass::Cast_Ray_To_Polys(CullNodeStruct *node,
                                     RayCollisionTestClass &raytest) {
 ...
 * HISTORY: * 8/30/00    NH : Created. *
 *=============================================================================================*/
int AABTreeClass::Cast_Semi_Infinite_Axis_Aligned_Ray_To_Polys(
    CullNodeStruct *node, const Vector3 &start_point, int axis_r, int axis_1,
    int axis_2, int direction, unsigned char &flags) {
  int count = 0;
    ...
    */
    ...
    for (int poly_counter = 0; poly_counter < polycount; poly_counter++) {
      ...
      count += (unsigned int)Cast_Semi_Infinite_Axis_Aligned_Ray_To_Triangle(
          v0, v1, v2, tri_plane, start_point, axis_r, axis_1, axis_2, direction,
          flags);
    }
  }
 ...
 * HISTORY: * 6/19/98    GTH : Created. *
 *=============================================================================================*/
bool AABTreeClass::Cast_AABox_To_Polys(CullNodeStruct *node,
                                       AABoxCollisionTestClass &boxtest) {
  if (node->Get_Poly_Count() > 0) {
    /*
    ** Simply loop through the polys in this node, checking each for collision
 ...
 * HISTORY: * 6/19/98    GTH : Created. *
 *=============================================================================================*/
bool AABTreeClass::Cast_OBBox_To_Polys(CullNodeStruct *node,
                                       OBBoxCollisionTestClass &boxtest) {
  int polycount = node->Get_Poly_Count();
  int poly0 = node->Get_Poly0();
...
bool AABTreeClass::Intersect_OBBox_With_Polys(
    CullNodeStruct *node, OBBoxIntersectionTestClass &test) {
  int poly0 = node->Get_Poly0();
  int polycount = node->Get_Poly_Count();
 ...
 **
 *                                                                                             *
 * Typically this function will be used when you have modified the mesh slightly
 *and you need  * to update the bounding boxes but you want to keep the
