 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * HLodLoaderClass::Load_W3D -- Loads an HlodDef from a W3D file *
 *   HLodPrototypeClass::Create -- Creates an HLod from an HLodDef *
 *   HLodDefClass::HLodDefClass -- Constructor * HLodDefClass::HLodDefClass --
 *Copy Constructor                                            *
 *   HLodDefClass::~HLodDefClass -- Destructor * HLodDefClass::Free -- releases
 *all resources being used                                   *
 *   HLodDefClass::Initialize -- init this def from an HLod * HLodDefClass::Save
 *-- save this HLodDef                                                   *
 ...
 *   HLodDefClass::SubObjectArrayClass::SubObjectArrayClass -- LodArray
 *constructor            *
 *   HLodDefClass::SubObjectArrayClass::~SubObjectArrayClass -- LodArray
 *destructor            * HLodDefClass::SubObjectArrayClass::Save_W3D -- saves a
 *w3d file for this HLodDef          *
 *   HLodDefClass::SubObjectArrayClass::Load_W3D -- LodArray load function *
 *   HLodDefClass::SubObjectArrayClass::Reset -- release the contents of this
 *array            * HLodClass::HLodClass -- Constructor * HLodClass::HLodClass
 *-- copy constructor                                                  *
 *   HLodClass::HLodClass -- Constructor * HLodClass::HLodClass -- Constructs an
 *HLod from an HLodDef                                * HLodClass::HLodClass --
 *Constructs an HLod from an HModelDef                              *
 *   HLodClass::operator -- assignment operator * HLodClass::~HLodClass --
 *Destructor                                                       *
 *   HLodClass::Free -- releases all resources * HLodClass::Clone -- virtual
 *copy constructor                                              *
 ...
 *   HLodClass::Get_Lod_Model_Count -- number of sub-objs in a given level of
 *detail           * HLodClass::Peek_Lod_Model -- returns pointer to a model in
 *one of the LODs                * HLodClass::Get_Lod_Model -- returns a pointer
 *to a model in one of the LODs               * HLodClass::Get_Lod_Model_Bone --
 *returns the bone index of a model                        *
 ...
 *   HLodClass::Get_Additional_Model_Bone -- returns the bone index of an
 *additional model     * HLodClass::Is_NULL_Lod_Included -- does this HLod have
 *NULL as its lowest LOD             * HLodClass::Include_NULL_Lod -- Add NULL
 *as the lowest LOD                                 * HLodClass::Get_Num_Polys
 *-- returns polycount of the current LOD                          *
 *   HLodClass::Render -- render this HLod * HLodClass::Special_Render --
 *Special_Render for HLod                                      *
 *   HLodClass::Set_Transform -- Sets the transform * HLodClass::Set_Position --
 *Sets the position                                              *
 ...
 *   HLodClass::Get_Num_Sub_Objects_On_Bone -- returns the number of objects on
 *the given bone * HLodClass::Get_Sub_Object_On_Bone -- returns obj on the given
 *bone                        * HLodClass::Get_Sub_Object_Bone_Index -- returns
 *bone index of given object                * HLodClass::Add_Sub_Object_To_Bone
 ...
 *   HLodClass::Set_Animation -- set animation state to a blend of two
 *animations              * HLodClass::Set_Animation -- set animation state to a
 *combination of anims                 * HLodClass::Cast_Ray -- cast a ray
 *against this HLod                                       *
 ...
 *   HLodClass::Increment_LOD -- move to next lod * HLodClass::Decrement_LOD --
 *move to previous lod                                          *
 *   HLodClass::Get_Cost -- returns the cost of this LOD * HLodClass::Get_Value
 ...
 *   HLodClass::Get_Current_LOD -- returns a render object which represents the
 *current LOD    * HLodClass::Set_Texture_Reduction_Factor -- resizeable texture
 *support                     * HLodClass::Scale -- scale this HLod model *
 *   HLodClass::Get_Num_Snap_Points -- returns the number of snap points in this
 *model         * HLodClass::Get_Snap_Point -- returns specified snap-point *
 *   HLodClass::Update_Sub_Object_Transforms -- updates transforms of all
 *sub-objects          * HLodClass::Update_Obj_Space_Bounding_Volumes -- update
 *object-space bounding volumes      * HLodClass::add_lod_model -- adds a model
 *to one of the lods                               * HLodClass::Create_Decal --
 *create a decal on this HLod                                    *
...
/**
** ProxyRecordClass
** This is a structure that contains the data describing a single "proxy"
*object.  These
** are used for application purposes and simply provide a way for the assets to
*associate
** a string with a bone index.
*/
class ProxyRecordClass {
 public:
  ProxyRecordClass(void) : BoneIndex(0) { memset(Name, 0, sizeof(Name)); }
...
/**
** ProxyArrayClass
** This is a ref-counted list of proxy objects.  It is generated whenever an
*HLODdef contains
** proxies.  Each instantiated HLOD simply add-refs a pointer to the single
*list.
*/
class ProxyArrayClass : public W3DMPO,
                        public VectorClass<ProxyRecordClass>,
...
void HLodDefClass::Initialize(HLodClass &src_lod) {
  ...
  if (LodCount > 0) {
    ...
    for (int index = 0; index < LodCount; index++) {
      ...
      for (int model_index = 0; model_index < Lod[index].ModelCount;
           model_index++) {
        // Record information about this model (if possible)
        RenderObjClass *prender_obj =
            src_lod.Peek_Lod_Model(index, model_index);
        if (prender_obj != NULL) {
          model_names[model_index] = ::strdup(prender_obj->Get_Name());
          bone_indicies[model_index] =
              src_lod.Get_Lod_Model_Bone(index, model_index);
        } else {
...
WW3DErrorType HLodDefClass::Save_Header(ChunkSaveClass &csave) {
  ...
  if (csave.Begin_Chunk(W3D_CHUNK_HLOD_HEADER) == TRUE) {
    ...
    ::lstrcpyn(header.HierarchyName, HierarchyTreeName,
               sizeof(header.HierarchyName));
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/25/2000 gth : Created. *
 *=============================================================================================*/
 ...
 * HLodDefClass::read_proxy_array -- load the proxy names *
 *                                                                                             *
 *    This function is coded separately from SubObjectArrayClass::Load because
 *we are going    * to store the proxies in a shared data structure.  Because
 *all of the proxy data is       * constant, each instanced HLOD can just
 *add-ref a pointer to its proxy array.             *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/27/2000 gth : Created. *
 *=============================================================================================*/
 ...
 * HISTORY: * 1/26/00    gth : Created. *
 *=============================================================================================*/
HLodDefClass::SubObjectArrayClass::~SubObjectArrayClass(void) { Reset(); }
...
/***********************************************************************************************
 * HLodDefClass::SubObjectArrayClass::Save_W3D -- saves a w3d file for this
 *HLodDef            *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
bool HLodDefClass::SubObjectArrayClass::Save_W3D(ChunkSaveClass &csave) {
  ...
  if (csave.Begin_Chunk(W3D_CHUNK_HLOD_LOD_ARRAY) == TRUE) {
    ...
    if (ret_val) {
      ...
      for (int index = 0; (index < ModelCount) && ret_val; index++) {
 ...
 * HLodClass::HLodClass -- Constructor *
 *                                                                                             *
 * Creates an HLodClass from an array of render objects.  Each render object is
 *assumed to     * be an alternate level of detail. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
...
HLodClass::HLodClass(const char *name, RenderObjClass **lods, int count)
  ...
  // Ok, now suck the sub-objects out of each LOD model and place them into this
  // HLOD.
  for (int lod_index = 0; lod_index < LodCount; lod_index++) {
    ...
    if ((lod_obj->Class_ID() == RenderObjClass::CLASSID_HMODEL) ||
      ...
      // here we insert all sub-objects of this render object into the current
      // LOD array
      ...
      // just insert the render object as the sole member of the current LOD
      // array.  This case happens if this level of detail is a simple object
      // such as a mesh or NullRenderObj
...
HLodClass::HLodClass(const HLodDefClass &def)
  ...
  for (int iagg = 0; iagg < def.Aggregates.ModelCount; iagg++) {
    RenderObjClass *robj = WW3DAssetManager::Get_Instance()->Create_Render_Obj(
        def.Aggregates.ModelName[iagg]);
...
HLodClass::HLodClass(const HModelDefClass &def)
  ...
  for (imodel = 0; imodel < def.SubObjectCount; ++imodel) {
    RenderObjClass *robj = WW3DAssetManager::Get_Instance()->Create_Render_Obj(
        def.SubObjects[imodel].RenderObjName);
    if (robj) {
 ...
 * HISTORY: * 1/26/00    gth : Created. *
 *=============================================================================================*/
RenderObjClass *HLodClass::Clone(void) const { return W3DNEW HLodClass(*this); }

/***********************************************************************************************
 * HLodClass::Get_Obj_Space_Bounding_Box -- Return the bounding box mesh if we
 *have one.		  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void HLodClass::Get_Obj_Space_Bounding_Box(AABoxClass &box) const {
  ...
  if (BoundingBoxIndex >= 0 && BoundingBoxIndex < count) {
    ...
    if (mesh != NULL && mesh->Class_ID() == RenderObjClass::CLASSID_OBBOX) {
      ...
      //
      Matrix3D box_tm;
      Simple_Evaluate_Bone(Lod[LodCount - 1][BoundingBoxIndex].BoneIndex,
                           &box_tm);
      ...
      box_to_hlod_tm.Transform_Center_Extent_AABox(
          obbox_mesh->Get_Local_Center(), obbox_mesh->Get_Local_Extent(),
...
/***********************************************************************************************
 * HLodClass::Get_Obj_Space_Bounding_Sphere -- Use the bounding box mesh to
 *calculate a sphere.*
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
const AABoxClass &HLodClass::Get_Bounding_Box(void) const {
  if (BoundingBoxIndex >= 0) {
    ...
    //
    Get_Transform().Transform_Center_Extent_AABox(box.Center, box.Extent,
...
RenderObjClass *HLodClass::Peek_Additional_Model(int model_index) const {
  ...
  if ((model_index >= 0) && (model_index < AdditionalModels.Count())) {
    // Get a pointer to the requested model
...
void HLodClass::Include_NULL_Lod(bool include) {
  if ((include == false) && Is_NULL_Lod_Included()) {
    ...
    RenderObjClass *null_object =
        WW3DAssetManager::Get_Instance()->Create_Render_Obj("NULL");
...
bool HLodClass::Get_Proxy(int index, ProxyClass &proxy) const {
  ...
  if (ProxyArray != NULL) {
    ...
    Matrix3D transform =
        HTree->Get_Transform((*ProxyArray)[index].Get_Bone_Index());
...
void HLodClass::Render(RenderInfoClass &rinfo) {
  ...
  for (i = 0; i < Lod[CurLod].Count(); i++) {
    if (Lod[CurLod][i].Model->Class_ID() !=
        CLASSID_OBBOX)  /// We have no use for these - MW
...
void HLodClass::Special_Render(SpecialRenderInfoClass &rinfo) {
  ...
  if (rinfo.RenderType ==
      SpecialRenderInfoClass::RENDER_SHADOW) {  // (gth) HACK HACK! yikes
...
int HLodClass::Remove_Sub_Object(RenderObjClass *removeme) {
  ...
  if (found) {
    ...
    // this is the combination of this HLod being in the scene and and this
    // model either being in the current LOD or being in the additional model
    // list...
...
// Custom version of above function for cases where we know the lod/model index.
// -MW
...
int HLodClass::Add_Sub_Object_To_Bone(RenderObjClass *subobj, int boneindex) {
  ...
  newnode.Model->Set_Container(this);
  newnode.Model->Set_Animation_Hidden(HTree->Get_Visibility(boneindex) ==
                                      false);
 ...
 * HISTORY: * 1/26/00    gth : Created. *
 *=============================================================================================*/
void HLodClass::Set_Animation(HAnimClass *motion0, float frame0,
                              HAnimClass *motion1, float frame1,
                              float percentage) {
  Animatable3DObjClass::Set_Animation(motion0, frame0, motion1, frame1,
                                      percentage);
  Set_Sub_Object_Transforms_Dirty(true);
}
...
void HLodClass::Prepare_LOD(CameraClass &camera) {
  ...
  */
  ...
  if (LodCount > 1) {
    /*
    ** Prepare cost and value arrays (and ensure current LOD doesn't violate
    *clamping):
    */
...
void HLodClass::Recalculate_Static_LOD_Factors(void) {
  ...
  */
  ...
  for (int i = 0; i < LodCount; i++) {
    ...
    // If polycount is zero set Cost to a small nonzero amount to avoid
    // divisions by zero.
    Lod[i].NonPixelCost = (polycount != 0) ? polycount : 0.000001f;

    // A polycount of zero yields a benefit factor of zero: otherwise apply
    // formula.
    Lod[i].BenefitFactor =
        (polycount != 0) ? (1 - (0.5f / (polycount * polycount))) : 0.0f;
  }
}
 ...
 * HISTORY: * 1/26/00    gth : Created. *
 *=============================================================================================*/
float HLodClass::Get_Cost(void) const { return (Cost[CurLod]); }
 ...
 * HISTORY: * 1/26/00    gth : Created. *
 *=============================================================================================*/
float HLodClass::Get_Value(void) const { return (Value[CurLod]); }
 ...
 * HISTORY: * 1/26/00    gth : Created. *
 *=============================================================================================*/
int HLodClass::Get_LOD_Level(void) const { return CurLod; }
 ...
 * HISTORY: * 1/26/00    gth : Created. *
 *=============================================================================================*/
int HLodClass::Calculate_Cost_Value_Arrays(float screen_area, float *values,
                                           float *costs) const {
  int lod = 0;
  ...
  for (lod = 0; lod < LodCount; lod++) {
    costs[lod] =
        Lod[lod].NonPixelCost + Lod[lod].PixelCostPerArea * screen_area;
  }
  ...
  // Calculate Value heuristic for any remaining LODs based on normalized screen
  // area:
  lod++;
...
/***********************************************************************************************
 * HLodClass::Update_Obj_Space_Bounding_Volumes -- update object-space bounding
 *volumes        *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void HLodClass::Update_Obj_Space_Bounding_Volumes(void) {
  ...
  if (BoundingBoxIndex < 0 || BoundingBoxIndex >= count ||
      high_lod[BoundingBoxIndex].Model->Class_ID() !=
          RenderObjClass::CLASSID_OBBOX) {
    BoundingBoxIndex = -1;
  }
  ...
  // loop through all sub-objects, combining their object-space bounding spheres
  // and boxes. Put our HTree in its base pose at the origin.
  SphereClass sphere;
  ...
  const Matrix3D &bonetm =
      HTree->Get_Transform(Get_Sub_Object_Bone_Index(robj));
 ...
 * HLodClass::Delete_Decal -- remove a decal from this HLod *
 *                                                                                             *
 *    The decal_id is the ID which was assigned to the DecalGeneratorClass when
 *you created    * the decal. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
