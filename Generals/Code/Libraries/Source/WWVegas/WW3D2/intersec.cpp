...
bool IntersectionClass::Intersect_Screen_Point_RenderObject(
    float screen_x, float screen_y, const LayerClass &Layer,
    RenderObjClass *RObj, IntersectionResultClass *FinalResult) {
  Get_Screen_Ray(screen_x, screen_y, Layer);
  return Intersect_RenderObject(RObj, FinalResult);
}

bool IntersectionClass::Intersect_RenderObject(
    RenderObjClass *RObj, IntersectionResultClass *FinalResult) {
...
// iterate through the layers of a world, front to back, returning true if/when
// an intersection with an object occurs.
bool IntersectionClass::Intersect_Screen_Point_Layer_Range(
    float screen_x, float screen_y, const LayerClass &TopLayer,
    const LayerClass &BackLayer) {
  // intersect from front layer to back layers. An intersection with an object
  // in any layer is assumed to be in front of any potential intersections in
  // layers below it.
  ...
  while (Layer->Is_Valid()) {
    if (Intersect_Screen_Point_Layer(screen_x, screen_y, *Layer)) return true;

    // if this is the back layer then that is all we need to test
    if (Layer == &BackLayer) return false;
...
bool IntersectionClass::Intersect_Screen_Point_Layer(float screen_x,
                                                     float screen_y,
                                                     const LayerClass &Layer) {
  // mark this object as not intersecting yet
  Result.Intersects = false;

  // first, do a test to make sure the screen coords are within the rendering
  // area for this layer.
  const ViewportClass &v = Layer.Camera->Get_Viewport();

  if ((screen_x < v.Min.X) || (screen_x > v.Max.X) || (screen_y < v.Min.Y) ||
      (screen_y > v.Max.Y))
    return false;
...
bool IntersectionClass::Intersect_Layer(const LayerClass &Layer,
  ...
  while (!it->Is_Done()) {
    ...
    // Added 'Generals' code to only detect intersection on matching
    // Collision_Type. MW
    if (robj->Get_Collision_Type() & Result.CollisionType &&
        (Test_All || robj->Is_Really_Visible()) &&
        robj->Intersect(this, &FinalResult)) {
...
void IntersectionClass::Append_Object_Array(int MaxCount, int &CurrentCount,
...
// determines if specified plane-intersection point (co-planar with polygon) is
// within the the passed polygon. If Interpolated_Normal is specified, it will
// interpolate the normal for the intersection point note: Polygon normal MUST
// BE CORRECT
...
bool IntersectionClass::Intersect_Box(Vector3 &Box_Min, Vector3 &Box_Max,
                                      IntersectionResultClass *FinalResult) {
  // Fast Ray-Box Intersection, modified from code written by Andrew Woo from
  // "Graphics Gems", Academic Press, 1990

  enum { RIGHT = 0, LEFT, MIDDLE, PLANE_COUNT };
  ...
  for (counter = 0; counter < PLANE_COUNT; counter++) {
    if ((quadrant[counter] != MIDDLE) && ((*RayDirection)[counter] != 0.0f))
      distance[counter] = (candidate_plane[counter] - (*RayLocation)[counter]) /
                          (*RayDirection)[counter];
    else
  ...
  for (counter = 1; counter < PLANE_COUNT; counter++) {
    if (distance[nearest_plane] < distance[counter]) nearest_plane = counter;
  }

  // Check to make sure the nearest plane is not behind the ray (inside box
  // tested above)
  if (distance[nearest_plane] < 0.0f) return FinalResult->Intersects = false;

  for (counter = 0; counter < PLANE_COUNT; counter++) {
    if (nearest_plane != counter) {
      (*intersection)[counter] =
          (*RayLocation)[counter] +
          distance[nearest_plane] * (*RayDirection)[counter];
      if ((*intersection)[counter] < Box_Min[counter] ||
          (*intersection)[counter] > Box_Max[counter])
...
// simply returns true if a ray hits the bounding sphere of any node in a
// hierarchy note: Result will only contain range, not the intersection
// point/normal.
bool IntersectionClass::Intersect_Hierarchy_Sphere_Quick(
  ...
  while (counter--) {
    ...
    obj->Release_Ref();  // you already own a reference to this object
                         // indirectly..
    if (obj->Intersect_Sphere_Quick(this, FinalResult)) return true;
  }
  return false;
}
...
bool IntersectionClass::Intersect_Hierarchy_Sphere(
    RenderObjClass *Hierarchy, IntersectionResultClass *FinalResult) {
  int counter = Hierarchy->Get_Num_Sub_Objects();
  while (counter--) {
    RenderObjClass *obj = Hierarchy->Get_Sub_Object(counter);
    obj->Release_Ref();  // you already own a reference to this object
                         // indirectly..
    if (obj->Intersect_Sphere(this, FinalResult)) return true;
  }
  return false;
}
...
void IntersectionClass::Append_Hierarchy_Objects(
    ...
    RenderObjClass *Hierarchy, bool Test_Bounding_Sphere, bool Convex) {
  ...
  if (Test_Bounding_Sphere) {
    while (counter--) {
      RenderObjClass *obj = Hierarchy->Get_Sub_Object(counter);
      obj->Release_Ref();  // you already own a reference to the object
                           // indirectly
      if (obj->Intersect_Sphere_Quick(this, &result)) {
        Append_Object_Array(MaxCount, CurrentCount, ObjectArray, obj);
        //				OutputDebugString("o"); // this shows
        //one o per sphere intersection
      } else {
        //				OutputDebugString("."); // this shows
        //one . per sphere miss
      }
    }
...
bool IntersectionClass::Intersect_Hierarchy(
    RenderObjClass *Hierarchy, IntersectionResultClass *FinalResult,
    bool Test_Bounding_Sphere, bool Convex) {
  //	OutputDebugString("\n");
  //	return FinalResult->Intersects = false;
  ...
  Append_Hierarchy_Objects(MAX_HIERARCHY_NODE_COUNT, candidate_count,
                           candidate_objects, Hierarchy, Test_Bounding_Sphere,
                           Convex);

  // make sure there's at least one sphere hit before continuing to more
  ...
  // note: Test_Bounding_Sphere argument is false because the
  // Append_Hierarchy_Objects will have already performed that test if
  // indicated.
  if (Intersect_Object_Array(candidate_count, candidate_objects, FinalResult,
                             false, Convex)) {
    return true;
  }
  return false;
...
RenderObjClass *IntersectionClass::Intersect_Sub_Object(
    float screenx, float screeny, LayerClass &layer, RenderObjClass *robj,
    IntersectionResultClass *result) {
  if (robj->Get_Num_Sub_Objects()) {
    for (int lp = 0; lp < robj->Get_Num_Sub_Objects(); lp++) {
      RenderObjClass *sub = robj->Get_Sub_Object(lp);
      RenderObjClass *retval =
          Intersect_Sub_Object(screenx, screeny, layer, sub, result);
      sub->Release_Ref();
      if (retval) return retval;
    }
  }
  if (Intersect_Screen_Point_RenderObject(screenx, screeny, layer, robj,
                                          result)) {
    return robj;
  }
  return NULL;
...
bool IntersectionClass::Intersect_Object_Array(
    int Object_Count, RenderObjClass **ObjectArray,
    IntersectionResultClass *FinalResult, bool Test_Bounding_Sphere,
    bool Convex) {
  IntersectionResultClass TemporaryResults[MAX_HIERARCHY_NODE_COUNT];
  assert(Object_Count <= MAX_HIERARCHY_NODE_COUNT);
  return Intersect_Object_Array(Object_Count, ObjectArray, FinalResult,
                                TemporaryResults, Test_Bounding_Sphere, Convex);
}

bool IntersectionClass::Intersect_Object_Array(
    int Object_Count, RenderObjClass **ObjectArray,
    IntersectionResultClass *FinalResult,
    IntersectionResultClass *TemporaryResults, bool Test_Bounding_Sphere,
    bool Convex) {
  ...
  // if it's a convex hierarchy (ie a control panel) then find the first hit
  // otherwise use the more expensive exact intersection routine for use with
  // potentially concave hierarchies.
  int nearest_index = -1;
  if (ConvexTest || Convex) {
    if (Test_Bounding_Sphere) {
      while (counter--) {
        if (ObjectArray[counter]->Intersect_Sphere_Quick(
                this, &TemporaryResults[counter])) {
    ...
    if (Test_Bounding_Sphere) {
      while (counter--) {
        if (ObjectArray[counter]->Intersect_Sphere_Quick(
                this, &TemporaryResults[counter])) {
          hit |=
              ObjectArray[counter]->Intersect(this, &TemporaryResults[counter]);
        }
      }
    } else {
      while (counter--) {
  ...
  if (!(Convex || ConvexTest)) {
    ...
    while (counter--) {
      if (TemporaryResults[counter].Intersects &&
