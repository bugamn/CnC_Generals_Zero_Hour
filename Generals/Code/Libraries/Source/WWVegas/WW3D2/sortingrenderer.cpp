...
unsigned DEFAULT_SORTING_POLY_COUNT =
    16384;  // (count * 3) must be less than 65536
...
void SortingRendererClass::SetMinVertexBufferSize(unsigned val) {
  ...
  DEFAULT_SORTING_POLY_COUNT =
      val / 2;  // typically have 2:1 vertex:triangle ratio.
}
...
struct ShortVectorIStruct {
  ...
  ShortVectorIStruct(unsigned short i_, unsigned short j_, unsigned short k_)
      : i(i_), j(j_), k(k_) {}
  ShortVectorIStruct() {}
};
...
// Performs insertion sort on array 'array' elements [l-r]. Uses values from
// array 'keys' as sort keys.
//
// ----------------------------------------------------------------------------
...
//
//	Performs quicksort on array 'array'. Uses values from array 'keys' as
//sort keys.
//
...
class SortingNodeStruct : public DLNodeClass<SortingNodeStruct> {
 ...
 public:
  ...
  unsigned short
      polygon_count;  // Polygon count to process (3 indices = one polygon)
...
static TempIndexStruct* Get_Temp_Index_Array(unsigned count) {
  if (count < DEFAULT_SORTING_POLY_COUNT) count = DEFAULT_SORTING_POLY_COUNT;
...
void SortingRendererClass::Insert_Triangles(const SphereClass& bounding_sphere,
  ...
  WWASSERT((
      ...
      (state->sorting_state.vertex_buffer_type == BUFFER_TYPE_SORTING ||
       state->sorting_state.vertex_buffer_type ==
  ...
  SortingVertexBufferClass* vertex_buffer =
      static_cast<SortingVertexBufferClass*>(
          state->sorting_state.vertex_buffer);
  WWASSERT(vertex_buffer);
  WWASSERT(state->vertex_count <= vertex_buffer->Get_Vertex_Count());

  D3DXMATRIX mtx = (D3DXMATRIX&)state->sorting_state.world *
                   (D3DXMATRIX&)state->sorting_state.view;
  ...
  state->transformed_center =
      Vector3(transformed_vec[0], transformed_vec[1], transformed_vec[2]);

  /// @todo lorenzen sez use a bucket sort here... and stop copying so much data
  /// so many times
...
void SortingRendererClass::Insert_Triangles(unsigned short start_index,
  ...
  Insert_Triangles(sphere, start_index, polygon_count, min_vertex_index,
                   vertex_count);
}
...
void SortingRendererClass::Flush_Sorting_Pool() {
  ...
  ShortVectorIStruct* polygon_idx_array =
      (ShortVectorIStruct*)Get_Polygon_Index_Array(overlapping_polygon_count);

  unsigned vertexAllocCount = overlapping_vertex_count;
  if (DynamicVBAccessClass::Get_Default_Vertex_Count() <
      DEFAULT_SORTING_VERTEX_COUNT)
    vertexAllocCount =
        DEFAULT_SORTING_VERTEX_COUNT;  // make sure that we force the DX8
                                       // dynamic vertex buffer to maximum size
  if (overlapping_vertex_count > vertexAllocCount)
    vertexAllocCount = overlapping_vertex_count;
  WWASSERT(DEFAULT_SORTING_VERTEX_COUNT == 1 ||
           vertexAllocCount <= DEFAULT_SORTING_VERTEX_COUNT);
  DynamicVBAccessClass dyn_vb_access(
      BUFFER_TYPE_DYNAMIC_DX8, dynamic_fvf_type,
      vertexAllocCount /*overlapping_vertex_count*/);
  {
    ...
    for (node_id = 0; node_id < overlapping_node_count; ++node_id) {
      ...
      D3DXMATRIX d3d_mtx = (D3DXMATRIX&)state->sorting_state.world *
                           (D3DXMATRIX&)state->sorting_state.view;
      ...
      for (unsigned i = 0; i < state->vertex_count; ++i, ++src_verts) {
        vertex_z_array[i] =
            (mtx[2][0] * src_verts->x + mtx[2][1] * src_verts->y +
             mtx[2][2] * src_verts->z + mtx[2][3]);
      ...
      SortingIndexBufferClass* index_buffer =
          static_cast<SortingIndexBufferClass*>(
              state->sorting_state.index_buffer);
      WWASSERT(index_buffer);
      indices = index_buffer->index_buffer;
      WWASSERT(indices);
      indices += state->start_index;
      ...
      for (i = 0; i < state->polygon_count; ++i) {
        ...
        polygon_idx_array[array_index] = ShortVectorIStruct(
            idx1 + vertex_array_offset, idx2 + vertex_array_offset,
  ...
  /*	///@todo: Add code to break up rendering into multiple index buffer
     fills to allow more than 65536/3 triangles.  -MW int
     total_overlapping_polygon_count = overlapping_polygon_count; while (  > 0)
          {
                  if ((total_overlapping_polygon_count*3) > 65535)
                  {	//overflowed the index buffer, must break into multiple
     batches overlapping_polygon_count = 65535/3;
                  }
                  else
                          overlapping_polygon_count =
     total_overlapping_polygon_count;
  ...
  */
  ...
  if ((unsigned)(DynamicIBAccessClass::Get_Default_Index_Count() / 3) <
      DEFAULT_SORTING_POLY_COUNT)
    polygonAllocCount =
        DEFAULT_SORTING_POLY_COUNT;  // make sure that we force the DX8 index
                                     // buffer to maximum size
  if (overlapping_polygon_count > polygonAllocCount)
    polygonAllocCount = overlapping_polygon_count;
  WWASSERT(DEFAULT_SORTING_POLY_COUNT <= 1 ||
           polygonAllocCount <= DEFAULT_SORTING_POLY_COUNT);

  DynamicIBAccessClass dyn_ib_access(BUFFER_TYPE_DYNAMIC_DX8,
                                     polygonAllocCount * 3);
  {
    DynamicIBAccessClass::WriteLockClass lock(&dyn_ib_access);
    ShortVectorIStruct* sorted_polygon_index_array =
        (ShortVectorIStruct*)lock.Get_Index_Array();
  ...
  DX8Wrapper::Set_Index_Buffer(
      dyn_ib_access,
      0);  // Override with this buffer (do something to prevent need for this!)
  DX8Wrapper::Set_Vertex_Buffer(
      dyn_vb_access);  // Override with this buffer (do something to prevent
                       // need for this!)
  ...
  for (unsigned i = 1; i < overlapping_polygon_count; ++i) {
    if (node_id != tis[i].idx) {
      ...
      DX8Wrapper::Draw_Triangles(start_index * 3, count_to_render,
                                 state->min_vertex_index, state->vertex_count);
  ...
  // Release all references and return nodes back to the clean list for the
  // frame...
  for (node_id = 0; node_id < overlapping_node_count; ++node_id) {
    SortingNodeStruct* state = overlapping_nodes[node_id];
    Release_Refs(state);
...
void SortingRendererClass::Flush() {
  ...
  while (SortingNodeStruct* state = sorted_list.Head()) {
    ...
    if ((state->sorting_state.index_buffer_type == BUFFER_TYPE_SORTING ||
         state->sorting_state.index_buffer_type ==
             BUFFER_TYPE_DYNAMIC_SORTING) &&
        (state->sorting_state.vertex_buffer_type == BUFFER_TYPE_SORTING ||
         state->sorting_state.vertex_buffer_type ==
             BUFFER_TYPE_DYNAMIC_SORTING)) {
      ...
      DX8Wrapper::Draw_Triangles(state->start_index, state->polygon_count,
                                 state->min_vertex_index, state->vertex_count);
...
void SortingRendererClass::Insert_VolumeParticle(
    const SphereClass& bounding_sphere, unsigned short start_index,
    unsigned short polygon_count, unsigned short min_vertex_index,
    unsigned short vertex_count, unsigned short layerCount) {
  if (!WW3D::Is_Sorting_Enabled()) {
    DX8Wrapper::Draw_Triangles(start_index, polygon_count, min_vertex_index,
                               vertex_count);
    return;
  ...
  DX8_RECORD_SORTING_RENDER(
      polygon_count * layerCount,
      vertex_count * layerCount);  // THIS IS VOLUME_PARTICLE SPECIFIC
  ...
  state->min_vertex_index = min_vertex_index;
  state->polygon_count =
      polygon_count * layerCount;  // THIS IS VOLUME_PARTICLE SPECIFIC
  state->vertex_count =
      vertex_count * layerCount;  // THIS IS VOLUME_PARTICLE SPECIFIC
  ...
  // BUT WHAT IS THE DEAL WITH THE VERTCOUNT AND POLYCOUNT BEING N BUT
  // TRANSFORMED CENTER COUNT == 1
  ...
  //index_buffer=static_cast<SortingIndexBufferClass*>(state->sorting_state.index_buffer);
  //	WWASSERT(index_buffer);
  //	indices=index_buffer->index_buffer;
  //	WWASSERT(indices);
  //	indices+=state->start_index;
