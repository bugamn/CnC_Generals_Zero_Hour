...
SegmentedLineClass &SegmentedLineClass::operator=(
    const SegmentedLineClass &that) {
...
void SegmentedLineClass::Reset_Line(void) { LineRenderer.Reset_Line(); }
...
// These are segment points, and include the start and end point of the
// entire line. Therefore there must be at least two.
int SegmentedLineClass::Get_Num_Points(void) { return PointLocations.Count(); }

// Set object-space location for a given point.
// NOTE: If given position beyond end of point list, do nothing.
void SegmentedLineClass::Set_Point_Location(unsigned int point_idx,
...
void SegmentedLineClass::Get_Point_Location(unsigned int point_idx,
                                            Vector3 &loc) {
  if (point_idx < (unsigned int)PointLocations.Count()) {
...
SegLineRendererClass::TextureMapMode
SegmentedLineClass::Get_Texture_Mapping_Mode(void) {
...
float SegmentedLineClass::Get_Width(void) { return LineRenderer.Get_Width(); }

void SegmentedLineClass::Set_Width(float width) {
  // Widths need to be clamped because they are not automatically clamped later
  // (like colors and alphas are).
...
void SegmentedLineClass::Set_Texture_Mapping_Mode(
    SegLineRendererClass::TextureMapMode mode) {
...
void SegmentedLineClass::Render(RenderInfoClass &rinfo) {
  ...
  //	if (LineRenderer.Peek_Texture())
  //LineRenderer.Peek_Texture()->Process_Reduction();

  unsigned int sort_level = SORT_LEVEL_NONE;

  if (!WW3D::Is_Sorting_Enabled()) sort_level = Get_Shader().Guess_Sort_Level();
...
void SegmentedLineClass::Get_Obj_Space_Bounding_Sphere(
...
void SegmentedLineClass::Get_Obj_Space_Bounding_Box(AABoxClass &box) const {
  ...
  if (num_points >= 2) {
    ...
    // We create two bounding boxes; one from the points, and if we have random
    // noise subdivision we create another one from the midpoints and factor the
    // noise amplitude into the second box, and then combine the two.
    ...
    if (MaxSubdivisionLevels > 0) {
      ...
      // We ignore the actual number of subdivision levels: we multiply the
      // random noise amplitude by 2, which is the limit as the number of
      // subdivision levels goes to infinity.
...
float SegmentedLineClass::Get_Cost(void) const { return Get_Num_Polys(); }
...
{
        if (LineRenderer.Peek_Texture())
LineRenderer.Peek_Texture()->Set_Reduction_Factor(trf);
}*/
...
void SegmentedLineClass::Render_Seg_Line(RenderInfoClass &rinfo) {
  ...
  LineRenderer.Render(rinfo, Transform, PointLocations.Count(),
                      &(PointLocations[0]), bounding_sphere);
}
...
bool SegmentedLineClass::Cast_Ray(RayCollisionTestClass &raytest) {
  ...
  //
  float fraction = 1.0F;
  for (uint32 index = 1; index < (unsigned int)PointLocations.Count();
       index++) {
    ...
    if (raytest.Ray.Find_Intersection(line_seg, &p0, &fraction, &p1, NULL)) {
      ...
      if (dist <= LineRenderer.Get_Width() && fraction >= 0 &&
