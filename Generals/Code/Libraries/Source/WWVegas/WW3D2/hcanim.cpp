 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * NodeMotionStruct::NodeMotionStruct -- constructor *
 ...
 ** HCompressedAnimClass::Get_Translation -- returns the translation vector for
 *the given fram* HCompressedAnimClass::Get_Orientation -- returns a quaternion
 *for the orientation of the p* HCompressedAnimClass::read_bit_channel -- read a
 *bit channel from the file                *
 *   HCompressedAnimClass::add_bit_channel -- install a bit channel into the
 *animation         * HCompressedAnimClass::Get_Visibility -- return visibility
 *state for given pivot/frame     *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 10/23/98   GTH : Created. * 02/02/00   JGA : Compressed *
 *=============================================================================================*/
...
HCompressedAnimClass::HCompressedAnimClass(void)
    : NumFrames(0), NumNodes(0), Flavor(0), FrameRate(0), NodeMotion(NULL) {
 ...
 * HISTORY: * 08/11/1997 GH  : Created. *
 *=============================================================================================*/
HCompressedAnimClass::~HCompressedAnimClass(void) { Free(); }
...
int HCompressedAnimClass::Load_W3D(ChunkLoadClass& cload) {
  ...
  */
  ...
  if (cload.Read(&aheader, sizeof(W3dAnimHeaderStruct)) !=
      sizeof(W3dAnimHeaderStruct)) {
    return LOAD_ERROR;
  }
  ...
  HTreeClass* base_pose =
      WW3DAssetManager::Get_Instance()->Get_HTree(HierarchyName);
  if (base_pose == NULL) {
    goto Error;
  }
  ...
  // Just for now
  WWASSERT((Flavor == ANIM_FLAVOR_TIMECODED) ||
           (Flavor == ANIM_FLAVOR_ADAPTIVE_DELTA));
    ...
    switch (cload.Cur_Chunk_ID()) {
        ...
        switch (Flavor) {
          case ANIM_FLAVOR_TIMECODED:
            ...
            if (tc_chan->Get_Pivot() < NumNodes) {
              ...
              delete tc_chan;
              WWDEBUG_SAY(
                  ("ERROR! animation %s indexes a bone not present in the "
                   "model. Please re-export!\r\n",
                   Name));
            }
      ...
      case W3D_CHUNK_COMPRESSED_BIT_CHANNEL:
        ...
        if (newbitchan->Get_Pivot() < NumNodes) {
          ...
          delete newbitchan;
          WWDEBUG_SAY(
              ("ERROR! animation %s indexes a bone not present in the model. "
               "Please re-export!\r\n",
               Name));
        }
 ...
 * HISTORY: * 08/11/1997 GH  : Created. *
 *=============================================================================================*/
bool HCompressedAnimClass::read_channel(ChunkLoadClass& cload,
                                        TimeCodedMotionChannelClass** newchan) {
...
bool HCompressedAnimClass::read_channel(
    ChunkLoadClass& cload, AdaptiveDeltaMotionChannelClass** newchan) {
...
void HCompressedAnimClass::add_channel(
    AdaptiveDeltaMotionChannelClass* newchan) {
  int idx = newchan->Get_Pivot();
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 1/19/98    GTH : Created. *
 *=============================================================================================*/
bool HCompressedAnimClass::read_bit_channel(
    ChunkLoadClass& cload, TimeCodedBitChannelClass** newchan) {
 ...
 * HISTORY: * 08/11/1997 GH  : Created. *
 *=============================================================================================*/
void HCompressedAnimClass::Get_Translation(Vector3& trans, int pividx,
                                           float frame) const {
...
/***********************************************************************************************
 * HCompressedAnimClass::Get_Orientation -- returns a quaternion for the
 *orientation of the    *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 08/11/1997 GH  : Created. *
 *=============================================================================================*/
void HCompressedAnimClass::Get_Orientation(Quaternion& q, int pividx,
                                           float frame) const {
  switch (Flavor) {
    case ANIM_FLAVOR_TIMECODED:
...
/***********************************************************************************************
 * HCompressedAnimClass::Get_Transform -- returns the transform matrix for the
 *given frame	  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 08/11/1997 GH  : Created. *
 *=============================================================================================*/
void HCompressedAnimClass::Get_Transform(Matrix3D& mtx, int pividx,
                                         float frame) const {
  struct NodeCompressedMotionStruct* motion = &NodeMotion[pividx];
...
/***********************************************************************************************
 * HCompressedAnimClass::Get_Visibility -- return visibility state for given
 *pivot/frame       *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * HAnimClass::Is_Node_Motion_Present -- return true if there is motion defined
 *for this frame *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
