...
NamedPivotMapClass::WeightInfoStruct::operator=(WeightInfoStruct const &that) {
  if (Name) delete[] Name;
  assert(that.Name != 0);
...
void NamedPivotMapClass::Update_Pivot_Map(const HTreeClass *Tree) {
  ...
  // for each named pivot, find the correct index and set the weight if the
  // indicated bone is present note: there is no check for redundant names
...
HAnimComboDataClass::HAnimComboDataClass(const HAnimComboDataClass &src)
    : PivotMap(src.Get_Pivot_Map()), HAnim(src.Get_HAnim()) {
...
HAnimComboDataClass::~HAnimComboDataClass(void) {
  if (HAnim) HAnim->Release_Ref();
  if (PivotMap) PivotMap->Release_Ref();
}
...
void HAnimComboDataClass::Clear(void) {
  ...
  // not sure if the pivot map should be deleted or just have everything set to
  // one. removing it effectively sets it to one, so that's what I'm doing for
  // now.
  if (PivotMap) {
...
/*
**	This function will replace the current pivot map (if any) with another
*pivot map that is
...
HAnimComboClass::~HAnimComboClass(void) { Reset(); }
...
void HAnimComboClass::Clear(void) {
  ...
  while (numAnimations--) {
    ...
    if (data && (!data->Is_Shared())) data->Clear();
  }
}
...
bool HAnimComboClass::Normalize_Weights(void) {
  // NOTE: This can only work if either no anims have pivot weight maps (in
  // which case we will adjust the anim weights to ensure normalization), or
  // else if all do (in which case we will adjust the pivot maps). Otherwise we
  // do nothing and return false.
  int anim_count = Get_Num_Anims();
  if (!anim_count) return true;  // Trivially succeeded

  // Loop over all anims. Check if all or none have pivot maps, and also
  // calculate the minimum number of pivots.
  int anim_idx = 0;
  ...
  for (anim_idx = 0; anim_idx < anim_count; anim_idx++) {
    num_anim_pivots =
        MIN(num_anim_pivots, Peek_Motion(anim_idx)->Get_Num_Pivots());
  ...
  if (none_pivot_maps) {
    ...
    // weight_total should be very close to 1. If not, normalize this pivot's
    // weights
    if (weight_total != 0.0 &&
        WWMath::Fabs(weight_total - 1.0) > WWMATH_EPSILON) {
    ...
    if (all_pivot_maps) {
      // For each pivot, calculate total weight of all active anims, ensure
      // close to 1.
      for (int piv_idx = 1; piv_idx < num_anim_pivots; piv_idx++) {
        ...
        for (anim_idx = 0; anim_idx < anim_count; anim_idx++) {
          if (Peek_Motion(anim_idx) != NULL) {
            float weight = Get_Weight(anim_idx) *
                           (*Peek_Pivot_Weight_Map(anim_idx))[piv_idx];
            weight_total += weight;
          }
        }

        // weight_total should be very close to 1. If not, normalize this
        // pivot's weights
        if (weight_total != 0.0 &&
            WWMath::Fabs(weight_total - 1.0) > WWMATH_EPSILON) {
          float oo_total = 1.0f / weight_total;
