 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * FileListTextureClass::Load_Frame_Surface -- Load source texture
 **
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
static int Calculate_Texture_Memory_Usage(const TextureClass* texture,
                                          int red_factor = 0) {
  // Set performance statistics

  int size = 0;
  IDirect3DTexture8* d3d_texture =
      const_cast<TextureClass*>(texture)->Peek_DX8_Texture();
...
TextureClass::TextureClass(unsigned width, unsigned height, WW3DFormat format,
                           MipCountType mip_level_count, PoolType pool,
                           bool rendertarget)
    : D3DTexture(NULL),
      ...
      MipMapFilter((mip_level_count != MIP_LEVELS_1) ? FILTER_TYPE_DEFAULT
                                                     : FILTER_TYPE_NONE),
  ...
  D3DTexture = DX8Wrapper::_Create_DX8_Texture(
      width, height, format, mip_level_count, d3dpool, rendertarget);
  if (pool == POOL_DEFAULT) {
    Dirty = true;
    DX8TextureTrackerClass* track = W3DNEW DX8TextureTrackerClass(
        width, height, format, mip_level_count, rendertarget, this);
...
TextureClass::TextureClass(const char* name, const char* full_path,
  ...
  switch (TextureFormat) {
    ...
    case WW3D_FORMAT_X8L8V8U8:  // Bumpmap
      // If requesting bumpmap format that isn't available we'll just return the
      // surface in whatever color format the texture file is in. (This is
      // illegal case, the format support should always be queried before
      // creating a bump texture!)
      if (!DX8Caps::Support_Texture_Format(TextureFormat)) {
        TextureFormat = WW3D_FORMAT_UNKNOWN;
      }
      // If bump format is valid, make sure compression is not allowed so that
      // we don't even attempt to load from a compressed file (quality isn't
      // good enough for bump map). Also disable mipmapping.
      else {
        IsCompressionAllowed = false;
  ...
  if (!WW3D::Is_Texturing_Enabled()) {
    Initialized = true;
    D3DTexture = 0;
  } else if (WW3D::Get_Texture_Thumbnail_Mode() ==
                 WW3D::TEXTURE_THUMBNAIL_MODE_OFF ||
...
TextureClass::TextureClass(SurfaceClass* surface, MipCountType mip_level_count)
  ...
  D3DTexture = DX8Wrapper::_Create_DX8_Texture(surface->Peek_D3D_Surface(),
                                               mip_level_count);
  LastAccessed = WW3D::Get_Sync_Time();
}
...
TextureClass::TextureClass(IDirect3DTexture8* d3d_texture)
    : D3DTexture(d3d_texture),
      ...
      TextureMinFilter(FILTER_TYPE_DEFAULT),
      TextureMagFilter(FILTER_TYPE_DEFAULT),
      MipMapFilter((d3d_texture->GetLevelCount() != 1) ? FILTER_TYPE_DEFAULT
                                                       : FILTER_TYPE_NONE),
      UAddressMode(TEXTURE_ADDRESS_REPEAT),
      VAddressMode(TEXTURE_ADDRESS_REPEAT),
...
bool TextureClass::Is_Missing_Texture() {
  ...
  if (D3DTexture == missing_texture) flag = true;

  if (missing_texture) missing_texture->Release();
...
void TextureClass::Set_Texture_Name(const char* name) { Name = name; }
...
void TextureClass::Get_Level_Description(
    SurfaceClass::SurfaceDescription& surface_desc, unsigned int level) {
  if (!D3DTexture) {
...
void TextureClass::Set_Mip_Mapping(FilterType mipmap) {
  if (mipmap != FILTER_TYPE_NONE && Get_Mip_Level_Count() <= 1) {
    WWASSERT_PRINT(0,
                   "Trying to enable MipMapping on texture w/o Mip levels!\n");
    return;
  }
  MipMapFilter = mipmap;
}
...
void TextureClass::Apply(unsigned int stage) {
  ...
  DX8Wrapper::Set_DX8_Texture_Stage_State(stage, D3DTSS_MINFILTER,
                                          _MinTextureFilters[TextureMinFilter]);
...
unsigned TextureClass::Get_Texture_Memory_Usage() const {
  if (/*!ReductionEnabled || */ !Initialized)
...
void TextureClass::_Init_Filters() {
  ...
  // Jani: Disabling anisotropic filtering as it doesn't seem to work with the
  // latest nVidia drivers.
  if (dx8caps.TextureFilterCaps & D3DPTFILTERCAPS_MAGFAFLATCUBIC)
    _MagTextureFilters[FILTER_TYPE_BEST] = D3DTEXF_FLATCUBIC;
  else if (dx8caps.TextureFilterCaps & D3DPTFILTERCAPS_MAGFANISOTROPIC)
    _MagTextureFilters[FILTER_TYPE_BEST] = D3DTEXF_ANISOTROPIC;
  else if (dx8caps.TextureFilterCaps & D3DPTFILTERCAPS_MAGFGAUSSIANCUBIC)
    _MagTextureFilters[FILTER_TYPE_BEST] = D3DTEXF_GAUSSIANCUBIC;
  else if (dx8caps.TextureFilterCaps & D3DPTFILTERCAPS_MAGFLINEAR)
    _MagTextureFilters[FILTER_TYPE_BEST] = D3DTEXF_LINEAR;
  else if (dx8caps.TextureFilterCaps & D3DPTFILTERCAPS_MAGFPOINT)
    _MagTextureFilters[FILTER_TYPE_BEST] = D3DTEXF_POINT;
  else {
  ...
  else if (dx8caps.TextureFilterCaps & D3DPTFILTERCAPS_MINFPOINT)
    _MinTextureFilters[FILTER_TYPE_BEST] = D3DTEXF_POINT;
  else {
  ...
  else if (dx8caps.TextureFilterCaps & D3DPTFILTERCAPS_MIPFPOINT)
  ...
  _MinTextureFilters[FILTER_TYPE_DEFAULT] =
      _MinTextureFilters[FILTER_TYPE_BEST];
  _MagTextureFilters[FILTER_TYPE_DEFAULT] =
      _MagTextureFilters[FILTER_TYPE_BEST];
  ...
  if (cload.Open_Chunk() && (cload.Cur_Chunk_ID() == W3D_CHUNK_TEXTURE)) {
    ...
    */
    if (hastexinfo) {
      ...
      bool no_lod =
          ((texinfo.Attributes & W3DTEXTURE_NO_LOD) == W3DTEXTURE_NO_LOD);
      ...
      newtex->Set_U_Addr_Mode(u_clamp ? TextureClass::TEXTURE_ADDRESS_CLAMP
                                      : TextureClass::TEXTURE_ADDRESS_REPEAT);
      bool v_clamp = ((texinfo.Attributes & W3DTEXTURE_CLAMP_V) != 0);
      newtex->Set_V_Addr_Mode(v_clamp ? TextureClass::TEXTURE_ADDRESS_CLAMP
                                      : TextureClass::TEXTURE_ADDRESS_REPEAT);
      ...
      switch (texinfo.Attributes & W3DTEXTURE_TYPE_MASK) {
        ...
        case W3DTEXTURE_TYPE_BUMPMAP: {
          ...
          // Format is assumed to be a grayscale heightmap. Convert it to a bump
          // map.
          newtex =
              WW3DAssetManager::Get_Instance()->Get_Bumpmap_Based_On_Texture(
                  newtex);
...
void setup_texture_attributes(TextureClass* tex,
  ...
  if (tex->Get_Mip_Mapping() == TextureClass::FILTER_TYPE_NONE)
    texinfo->Attributes |= W3DTEXTURE_NO_LOD;
  if (tex->Get_U_Addr_Mode() == TextureClass::TEXTURE_ADDRESS_CLAMP)
    texinfo->Attributes |= W3DTEXTURE_CLAMP_U;
  if (tex->Get_V_Addr_Mode() == TextureClass::TEXTURE_ADDRESS_CLAMP)
...
void Save_Texture(TextureClass* texture, ChunkSaveClass& csave) {
  ...
  if ((texinfo.Attributes != 0) || (texinfo.AnimType != 0) ||
