...
//	AggregateDefClass
//
AggregateDefClass::AggregateDefClass(void) : m_pName(NULL) {
...
//	operator=
//
const AggregateDefClass &AggregateDefClass::operator=(
    const AggregateDefClass &src) {
  int index;
  ...
  for (index = 0; index < src.m_SubobjectList.Count(); index++) {
    ...
    if (pinfo != NULL) {
      ...
      W3dAggregateSubobjectStruct *new_info =
          W3DNEW W3dAggregateSubobjectStruct;
...
RenderObjClass *AggregateDefClass::Create(void) {
  ...
  if (pmodel != NULL) {
    ...
    pmodel->Set_Sub_Objects_Match_LOD(
        (m_MiscInfo.Flags & W3D_AGGREGATE_FORCE_SUB_OBJ_LOD) ==
...
//	Find_Subobject
//
RenderObjClass *AggregateDefClass::Find_Subobject(
  ...
  for (int index = 1; (mesh_path[index][0] != 0) && (parent_model != NULL);
       index++) {
    ...
    if (bone_path[index][0] == 0) {
      ...
      for (int subobj_index = 0;
           (subobj_index < subobj_count) && (sub_obj == NULL); subobj_index++) {
        // Is this the subobject we were looking for?
        RenderObjClass *ptemp_obj =
            parent_model->Get_Sub_Object_On_Bone(subobj_index, bone_index);
  ...
  for (int index = 0; index < m_SubobjectList.Count(); index++) {
    ...
    if (psubobj_info != NULL) {
      ...
      RenderObjClass *prender_obj =
          Create_Render_Object(psubobj_info->SubobjectName);
      if (prender_obj != NULL) {
        // Attach this object to the requested bone
        if (base_model.Add_Sub_Object_To_Bone(
                prender_obj, psubobj_info->BoneName) == false) {
...
//
RenderObjClass *AggregateDefClass::Create_Render_Object(
  ...
  prender_obj =
      WW3DAssetManager::Get_Instance()->Create_Render_Obj(passet_name);

  // If we couldn't find the render object in the asset manager, then attempt to
  // load it from file
  if ((prender_obj == NULL) && Load_Assets(passet_name)) {
...
void AggregateDefClass::Initialize(RenderObjClass &base_model) {
  ...
  orig_model_name =
      (orig_model_name == NULL) ? base_model.Get_Name() : orig_model_name;
  ...
  m_MiscInfo.Flags = 0;
  m_MiscInfo.Flags |= base_model.Is_Sub_Objects_Match_LOD_Enabled()
                          ? W3D_AGGREGATE_FORCE_SUB_OBJ_LOD
                          : 0;
  ...
  RenderObjClass *pvanilla_model =
      (RenderObjClass *)Create_Render_Object(orig_model_name);

  // Build lists of changes from the delta between the original model and the
  // provided one
...
void AggregateDefClass::Build_Subobject_List(RenderObjClass &original_model,
  ...
  for (int bone_index = 0; bone_index < bone_count; bone_index++) {
    ...
    for (index = 0;
         ...
         index++) {
      RenderObjClass *psubobj =
          original_model.Get_Sub_Object_On_Bone(index, bone_index);
      if (psubobj != NULL) {
    ...
    for (index = 0; index < model.Get_Num_Sub_Objects_On_Bone(bone_index);
         index++) {
      ...
      for (int node_index = 0; node_index < node_count; node_index++) {
        ...
        if (psubobject != NULL &&
          ...
          ((RenderObjClass &)original_model)
              .Add_Sub_Object_To_Bone(prender_obj, pbone_name);
...
//	Is_Object_In_List
//
bool AggregateDefClass::Is_Object_In_List(
  ...
  for (int node_index = 0;
       (node_index < node_list.Count()) && (retval == false); node_index++) {
      ...
      case W3D_CHUNK_TEXTURE_REPLACER_INFO:
        if (chunk_load.Read(&header, sizeof(header)) == sizeof(header)) {
          if (header.ReplacedTexturesCount > 0) {
            WWDEBUG_SAY(
                ("Obsolete texture replacement chunk encountered in aggregate: "
                 "%s\r\n",
                 m_pName));
          }
        }
...
WW3DErrorType AggregateDefClass::Read_Subobject(ChunkLoadClass &chunk_load) {
  ...
  W3dAggregateSubobjectStruct subobj_info = {0};
  if (chunk_load.Read(&subobj_info, sizeof(subobj_info)) ==
      sizeof(subobj_info)) {
    // Add this subobject to our list
    Add_Subobject(subobj_info);
...
//	Add_Subobject
//
void AggregateDefClass::Add_Subobject(
    const W3dAggregateSubobjectStruct &subobj_info) {
...
//	Save_Subobject
//
WW3DErrorType AggregateDefClass::Save_Subobject(
    ChunkSaveClass &chunk_save, W3dAggregateSubobjectStruct *psubobject) {
  // Assume error
  WW3DErrorType ret_val = WW3D_ERROR_SAVE_FAILED;

  // Write the subobj structure out to the chunk
  if (chunk_save.Write(psubobject, sizeof(W3dAggregateSubobjectStruct)) ==
      sizeof(W3dAggregateSubobjectStruct)) {
    // Success!
    ret_val = WW3D_ERROR_OK;
  }
...
WW3DErrorType AggregateDefClass::Save_Class_Info(ChunkSaveClass &chunk_save) {
  ...
  if (chunk_save.Begin_Chunk(W3D_CHUNK_AGGREGATE_CLASS_INFO) == TRUE) {
    ...
    if (chunk_save.Write(&m_MiscInfo, sizeof(m_MiscInfo)) ==
        sizeof(m_MiscInfo)) {
      // Success!
      ret_val = WW3D_ERROR_OK;
    }
