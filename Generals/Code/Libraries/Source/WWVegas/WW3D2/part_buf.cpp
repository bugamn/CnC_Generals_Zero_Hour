...
// A random permutation of the numbers 0 to 15 - used for LOD particle
// decimation. It was generated by the amazingly high-tech method of pulling
...
// Static array of screen-size clamps for the 17 possible LOD levels a particle
// buffer can have. We can change these from being global to being per-buffer
// later if we wish. Default is NO_MAX_SCREEN_SIZE.
float ParticleBufferClass::LODMaxScreenSizes[17] = {
    NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE,
    NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE,
    NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE,
    NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE,
    NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE,
    NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE};

static Random4Class rand_gen;
const float oo_intmax = 1.0f / (float)INT_MAX;

ParticleBufferClass::ParticleBufferClass(
    ParticleEmitterClass *emitter, unsigned int buffer_size,
    ...
    ParticlePropertyStruct<float> &frame, Vector3 accel, float max_age,
    TextureClass *tex, ShaderClass shader, bool pingpong, int render_mode,
  ...
  Position[0] = NEW_REF(ShareBufferClass<Vector3>,
                        (MaxNum, "ParticleBufferClass::Position"));
  if (PingPongPosition) {
    Position[1] = NEW_REF(ShareBufferClass<Vector3>,
                          (MaxNum, "ParticleBufferClass::Position"));
  }
  APT = NEW_REF(ShareBufferClass<unsigned int>,
                (MaxNum, "ParticleBufferClass::APT"));
  ...
  // lorenzen
  //  If the render mode is W3D_EMITTER_RENDER_MODE_LINE and we are supplied
  //  with a line properties structure, set up a line renderer
  if (RenderMode == W3D_EMITTER_RENDER_MODE_LINE) {
    if (line_props != NULL) {
      ...
      // We were in line mode but didn't get any line properties, drop back to
      // triangles
...
ParticleBufferClass::ParticleBufferClass(const ParticleBufferClass &src)
  ...
  */
  ...
  if (src.Color) {
    // Create color array
    Color = NEW_REF(ShareBufferClass<Vector3>,
                    (MaxNum, "ParticleBufferClass::Color"));
  ...
  if (src.Alpha) {
    // Create alpha array
    Alpha = NEW_REF(ShareBufferClass<float>,
                    (MaxNum, "ParticleBufferClass::Alpha"));
  ...
  if (src.Size) {
    // Create size array
    Size =
        NEW_REF(ShareBufferClass<float>, (MaxNum, "ParticleBufferClass::Size"));
  ...
  if (src.Orientation) {
    ...
    Orientation = NEW_REF(ShareBufferClass<uint8>,
                          (MaxNum, "ParticleBufferClass::Orientation"));
    ...
    if (src.RandomRotationEntries) {
      RandomRotationEntries =
          W3DNEWARRAY float[NumRandomRotationEntriesMinus1 + 1];
    ...
    if (src.RandomOrientationEntries) {
      RandomOrientationEntries =
          W3DNEWARRAY float[NumRandomOrientationEntriesMinus1 + 1];
    ...
    // Unlike other properties, if there is no Orientation array then all the
    // arrays are NULL (including the Values array) - there is an implicit
    // starting value of 0.
  }
  ...
  if (src.Frame) {
    // Create frame array
    Frame = NEW_REF(ShareBufferClass<uint8>,
                    (MaxNum, "ParticleBufferClass::Frame"));
...
ParticleBufferClass &ParticleBufferClass::operator=(
    const ParticleBufferClass &that) {
...
int ParticleBufferClass::Get_Num_Polys(void) const {
  // Currently in particle buffers, the cost happens to be equal to thwe polygon
  // count.
  return (int)Get_Cost();
}
...
void ParticleBufferClass::Render(RenderInfoClass &rinfo) {
  ...
  if (WW3D::Are_Static_Sort_Lists_Enabled() && sort_level != SORT_LEVEL_NONE) {
    ...
    // Since we are rendering the particles, visual state needs to be updated
    // (but not if the entire particle buffer is decimated away)
...
void ParticleBufferClass::Render_Particles(RenderInfoClass &rinfo) {
  // If the number of active points is less than the maximum or we need to
  // decimate particles (for LOD purposes), build the active point table:
  ...
  if (!Orientation) {
    // The rotation keyframes are used to derive the orientation indirectly, as
    // well as the starting orientation randomizer. If there is no Orientation
    // array that means both are absent so the orientation should just be set to
    // 0.
  ...
  if (Color || Alpha) {
    unsigned cnt = MaxNum;
    if (!Diffuse) {
      Diffuse = NEW_REF(ShareBufferClass<Vector4>,
                        (MaxNum, "ParticleBufferClass::Diffuse"));
    }
    if (Color && Alpha) {
      VectorProcessorClass::Copy(Diffuse->Get_Array(), Color->Get_Array(),
                                 Alpha->Get_Array(), cnt);
    } else if (Color) {
      VectorProcessorClass::Copy(Diffuse->Get_Array(), Color->Get_Array(), 1.0f,
                                 cnt);
    } else {
      VectorProcessorClass::Copy(Diffuse->Get_Array(),
                                 Vector3(1.0f, 1.0f, 1.0f), Alpha->Get_Array(),
                                 cnt);
    }
    VectorProcessorClass::Clamp(Diffuse->Get_Array(), Diffuse->Get_Array(),
                                0.0f, 1.0f, cnt);
  } else if (Diffuse) {
    Diffuse->Release_Ref();
    Diffuse = NULL;
...
void ParticleBufferClass::Render_Line(RenderInfoClass &rinfo) {
  ...
  if (tmp_points.Count() > 0) {
    ...
    LineRenderer->Render(rinfo, Transform, tmp_points.Count(), &(tmp_points[0]),
                         bounding_sphere);
  }
}
...
void ParticleBufferClass::Get_Obj_Space_Bounding_Sphere(
...
void ParticleBufferClass::Prepare_LOD(CameraClass &camera) {
  ...
  // Estimate the screen area of the particle buffer. We shall take the lesser
  // of two metrics: the standard bounding-sphere projection (which for many
  // particle systems may grossly overestimate the actual screen area), and a
  // measurement based on the screen area of individual particles times the
  // maximum number of particles (in the case of densely overlapping particles
  // this metric can also give numbers which are too high, which is why we use
  // the bounding sphere as backup). Note - to find the area of individual
  // particles we treat them as all being the maximum size and being in the
  ...
  float bs_rad_sq =
      bounding_sphere_projected_radius * bounding_sphere_projected_radius;
  float p_rad_sq =
      particle_projected_radius * particle_projected_radius * MaxNum;
  float proj_area =
      WWMATH_PI * MIN(bs_rad_sq, p_rad_sq) * width_factor * height_factor;
...
int ParticleBufferClass::Get_LOD_Count(void) const { return LodCount; }

int ParticleBufferClass::Calculate_Cost_Value_Arrays(float screen_area,
                                                     float *values,
                                                     float *costs) const {
  unsigned int lod = 0;

  // Calculate Cost heuristic for each LOD (we currently ignore pixel costs for
  // particle systems)
  float tris_per_particle =
      PointGroup->Get_Point_Mode() == PointGroupClass::QUADS ? 2.0f : 1.0f;
  float cost_factor = (float)MaxNum * tris_per_particle * 0.0625f;  // 1/16
  for (lod = 0; lod < LodCount; lod++) {
    costs[lod] = cost_factor * (float)lod;
    // If cost is zero set it to a small nonzero amount to avoid divisions by
    // zero.
  ...
  for (; lod < LodCount; lod++) {
    // Currently the cost happens to be equal to the poly count. We use a
    // floating- point poly count since costs[] contains an approximation to the
    // true polycount which may be less than one in some cases (we want to avoid
    // 0 polycounts except for true null LODs)
    float polycount = costs[lod];
    float benefit_factor = (polycount > WWMATH_EPSILON)
                               ? (1 - (0.5f / (polycount * polycount)))
                               : 0.0f;
...
void ParticleBufferClass::Reset_Colors(
    ParticlePropertyStruct<Vector3> &new_props) {
  ...
  // If the randomizer is effectively zero and there are no keyframes, then we
  // just create a values array with one entry and store the starting value in
  // it (the keyframes and random table will not be used in this case).
  static const float eps_byte = 0.0038f;  // Epsilon value - less than 1/255
  bool color_rand_zero =
      (fabs(new_props.Rand.X) < eps_byte && fabs(new_props.Rand.Y) < eps_byte &&
       fabs(new_props.Rand.Z) < eps_byte);
  if (color_rand_zero && new_props.NumKeyFrames == 0) {
    // Release Color, ColorKeyFrameTimes and ColorKeyFrameDeltas if present.
    // Reuse ColorKeyFrameValues if the right size, otherwise release and
    // reallocate.
    if (Color) {
      Color->Release_Ref();
      Color = NULL;
    ...
    // Reuse ColorKeyFrameValues, ColorKeyFrameTimes and ColorKeyFrameDeltas if
    // the right size, otherwise release and reallocate.
    unsigned int new_num_color_key_frames =
        ckey + 1;  // Includes start keyframe (keytime == 0).
    ...
    for (i = 0; i < NumColorKeyFrames - 1; i++) {
      ColorKeyFrameDeltas[i] =
          (ColorKeyFrameValues[i + 1] - ColorKeyFrameValues[i]) /
    ...
    if (color_constant_at_end) {
      ...
      // This is OK because if color_constant_at_end is false, NumColorKeyFrames
      // is equal or smaller than color.NumKeyFrames so
      // color.Values[NumColorKeyFrames - 1] and
      // color.KeyTimes[NumColorKeyFrames - 1] exist.
      ColorKeyFrameDeltas[i] =
          (new_props.Values[i] - ColorKeyFrameValues[i]) /
    ...
    if (color_rand_zero) {
      if (RandomColorEntries) {
        // Reuse RandomColorEntries if the right size, otherwise release and
        // reallocate.
      ...
      // Default size of randomizer tables (tables for non-zero randomizers will
      // be this size)
      unsigned int pot_num = Find_POT(MaxNum);
      unsigned int default_randomizer_entries =
      ...
      for (unsigned int j = 0; j <= NumRandomColorEntriesMinus1; j++) {
        RandomColorEntries[j] =
            Vector3(rand_gen * rscale, rand_gen * gscale, rand_gen * bscale);
      }
    }
  }
}

void ParticleBufferClass::Reset_Opacity(
    ParticlePropertyStruct<float> &new_props) {
  unsigned int i;  // Used in loops
  unsigned int ui_previous_key_time = 0;
  unsigned int ui_current_key_time = 0;
  ...
  if (alpha_rand_zero && new_props.NumKeyFrames == 0) {
    // Release Alpha, AlphaKeyFrameTimes and AlphaKeyFrameDeltas if present.
    // Reuse AlphaKeyFrameValues if the right size, otherwise release and
    // reallocate.
    if (Alpha) {
      Alpha->Release_Ref();
      Alpha = NULL;
    ...
    // Reuse AlphaKeyFrameValues, AlphaKeyFrameTimes and AlphaKeyFrameDeltas if
    // the right size, otherwise release and reallocate.
    unsigned int new_num_alpha_key_frames =
        akey + 1;  // Includes start keyframe (keytime == 0).
    ...
    for (i = 0; i < NumAlphaKeyFrames - 1; i++) {
      AlphaKeyFrameDeltas[i] =
          (AlphaKeyFrameValues[i + 1] - AlphaKeyFrameValues[i]) /
    ...
    if (alpha_constant_at_end) {
      ...
      // This is OK because if alpha_constant_at_end is false, NumAlphaKeyFrames
      // is equal or smaller than opacity.NumKeyFrames so
      // opacity.Values[NumAlphaKeyFrames - 1] and
      // opacity.KeyTimes[NumAlphaKeyFrames - 1] exist.
      AlphaKeyFrameDeltas[i] =
          (new_props.Values[i] - AlphaKeyFrameValues[i]) /
    ...
    if (alpha_rand_zero) {
      if (RandomAlphaEntries) {
        // Reuse RandomAlphaEntries if the right size, otherwise release and
        // reallocate.
...
void ParticleBufferClass::Reset_Size(ParticlePropertyStruct<float> &new_props) {
  ...
  // just create a values array with one entry and store the starting value in
  // it (the keyframes and random table will not be used in this case).
  static const float eps_size =
      1.0e-12f;  // Size scale unknown so must use very small epsilon
  ...
  if (size_rand_zero && new_props.NumKeyFrames == 0) {
    ...
    if (!Size) {
      Size = NEW_REF(ShareBufferClass<float>,
                     (MaxNum, "ParticleBufferClass::Size"));
    }
    ...
    // Reuse SizeKeyFrameValues, SizeKeyFrameTimes and SizeKeyFrameDeltas if the
    // right size, otherwise release and reallocate.
    unsigned int new_num_size_key_frames =
        skey + 1;  // Includes start keyframe (keytime == 0).
    ...
    for (i = 0; i < NumSizeKeyFrames - 1; i++) {
      SizeKeyFrameDeltas[i] =
          (SizeKeyFrameValues[i + 1] - SizeKeyFrameValues[i]) /
    ...
    if (size_constant_at_end) {
      ...
      // This is OK because if size_constant_at_end is false, NumSizeKeyFrames
      // is equal or smaller than new_props.NumKeyFrames so
      // new_props.Values[NumSizeKeyFrames - 1] and
      // new_props.KeyTimes[NumSizeKeyFrames - 1] exist.
      SizeKeyFrameDeltas[i] =
          (new_props.Values[i] - SizeKeyFrameValues[i]) /
    ...
    float last_size =
        SizeKeyFrameValues[NumSizeKeyFrames - 1] +
        SizeKeyFrameDeltas[NumSizeKeyFrames - 1] *
    ...
    if (size_rand_zero) {
      if (RandomSizeEntries) {
        // Reuse RandomSizeEntries if the right size, otherwise release and
        // reallocate.
...
void ParticleBufferClass::Reset_Rotations(
    ParticlePropertyStruct<float> &new_props, float orient_rnd) {
  unsigned int i;  // Used in loops
  ...
  /*
  ** NOTE: Input rotations are in rotations per second. These will be converted
  *to rotations per millisecond.
  */

  RotationRandom = new_props.Rand * 0.001f;
  InitialOrientationRandom = orient_rnd;

  // If both randomizers are effectively zero and rotation is constant zero,
  // then all arrays are NULL.
  static const float eps_orientation =
      2.77777778e-4f;  // Epsilon is equivalent to 0.1 degree
  static const float eps_rotation =
      2.77777778e-4f;  // Epsilon is equivalent to one rotation per hour (in
                       // rotations / second)
  bool orientation_rand_zero = fabs(orient_rnd) < eps_orientation;
  bool rotation_rand_zero = fabs(new_props.Rand) < eps_rotation;
  if (orientation_rand_zero && rotation_rand_zero &&
      new_props.NumKeyFrames == 0 && fabs(new_props.Start) < eps_rotation) {
    // Release Arrays,
    REF_PTR_RELEASE(Orientation);
    ...
    // Reuse RotationKeyFrameValues, RotationKeyFrameTimes,
    // RotationKeyFrameDeltas and OrientationKeyFrameValues if the right size,
    // otherwise release and reallocate.
    unsigned int new_num_key_frames =
        key + 1;  // Includes start keyframe (keytime == 0).
    ...
    for (i = 1; i < NumRotationKeyFrames; i++) {
      unsigned int im1 = i - 1;
      RotationKeyFrameTimes[i] =
          (unsigned int)(new_props.KeyTimes[im1] * 1000.0f);
    ...
    for (i = 0; i < NumRotationKeyFrames - 1; i++) {
      HalfRotationKeyFrameDeltas[i] =
          0.5f *
          ((RotationKeyFrameValues[i + 1] - RotationKeyFrameValues[i]) /
    ...
    if (rotation_constant_at_end) {
      ...
      // This is OK because if rotation_constant_at_end is false,
      // NumRotationKeyFrames is equal or smaller than new_props.NumKeyFrames so
      // new_props.Values[NumRotationKeyFrames - 1] and
      // new_props.KeyTimes[NumRotationKeyFrames - 1] exist.
      HalfRotationKeyFrameDeltas[i] =
          0.5f * (new_props.Values[i] * 0.001f - RotationKeyFrameValues[i]) /
          (new_props.KeyTimes[i] * 1000.0f - (float)RotationKeyFrameTimes[i]);
    }

    // Calculate orientation keyframes by integrating the rotation at each
    // keyframe
    OrientationKeyFrameValues[0] = 0.0f;
    for (i = 1; i < NumRotationKeyFrames; i++) {
      float delta_t =
          (float)(RotationKeyFrameTimes[i] - RotationKeyFrameTimes[i - 1]);
      OrientationKeyFrameValues[i] =
          OrientationKeyFrameValues[i - 1] +
          delta_t * (RotationKeyFrameValues[i - 1] +
                     HalfRotationKeyFrameDeltas[i - 1] * delta_t);
    }

    // Set up rotation randomizer table
    if (rotation_rand_zero) {
      if (RandomRotationEntries) {
        // Reuse RandomRotationEntries if the right size, otherwise release and
        // reallocate.
      ...
      if (RandomRotationEntries) {
        // Reuse RandomRotationEntries if the right size, otherwise release and
        // reallocate.
        if (NumRandomRotationEntriesMinus1 !=
            (default_randomizer_entries - 1)) {
          delete[] RandomRotationEntries;
    ...
    if (orientation_rand_zero) {
      if (RandomOrientationEntries) {
        // Reuse RandomOrientationEntries if the right size, otherwise release
        // and reallocate.
      ...
      if (RandomOrientationEntries) {
        // Reuse RandomOrientationEntries if the right size, otherwise release
        // and reallocate.
        if (NumRandomOrientationEntriesMinus1 !=
            (default_randomizer_entries - 1)) {
          delete[] RandomOrientationEntries;
          RandomOrientationEntries =
              W3DNEWARRAY float[default_randomizer_entries];
        }
      } else {
        RandomOrientationEntries =
            W3DNEWARRAY float[default_randomizer_entries];
...
void ParticleBufferClass::Reset_Frames(
    ParticlePropertyStruct<float> &new_props) {
  unsigned int i;  // Used in loops
  static Random3Class rand_gen;
  float oo_intmax = 1.0f / (float)INT_MAX;
  ...
  if (frame_rand_zero && new_props.NumKeyFrames == 0) {
    ...
    // Reuse FrameKeyFrameValues, FrameKeyFrameTimes and FrameKeyFrameDeltas if
    // the right size, otherwise release and reallocate.
    unsigned int new_num_key_frames =
        key + 1;  // Includes start keyframe (keytime == 0).
    ...
    for (i = 0; i < NumFrameKeyFrames - 1; i++) {
      FrameKeyFrameDeltas[i] =
          (FrameKeyFrameValues[i + 1] - FrameKeyFrameValues[i]) /
    ...
    if (frame_constant_at_end) {
      ...
      // This is OK because if frame_constant_at_end is false, NumFrameKeyFrames
      // is equal or smaller than new_props.NumKeyFrames so
      // new_props.Values[NumFrameKeyFrames - 1] and
      // new_props.KeyTimes[NumFrameKeyFrames - 1] exist.
      FrameKeyFrameDeltas[i] =
          (new_props.Values[i] - FrameKeyFrameValues[i]) /
    ...
    if (frame_rand_zero) {
      if (RandomFrameEntries) {
        // Reuse RandomFrameEntries if the right size, otherwise release and
        // reallocate.
...
void ParticleBufferClass::Update_Visual_Particle_State(void) {
  // NOTE: The visual state (color/alpha/size) is "stateless" in that each time
  // it is calculated without referring to what it was before. This is important
  // for when we optimize the particle systems/pointgroups in the future to
  // chunk triangles into reusable small buffers.
  ...
  for (part = Start; part < sub1_end; part++) {
    ...
    if (color) {
      // We go from older to younger particles, so we go backwards from the last
      // keyframe until age >= keytime. This loop must terminate because the 0th
      // keytime is 0.
      for (; part_age < ColorKeyFrameTimes[ckey]; ckey--);

      color[part] = ColorKeyFrameValues[ckey] +
                    ColorKeyFrameDeltas[ckey] *
                        (float)(part_age - ColorKeyFrameTimes[ckey]) +
    ...
    if (alpha) {
      ...
      alpha[part] = AlphaKeyFrameValues[akey] +
                    AlphaKeyFrameDeltas[akey] *
                        (float)(part_age - AlphaKeyFrameTimes[akey]) +
    ...
    if (size) {
      ...
      size[part] = SizeKeyFrameValues[skey] +
                   SizeKeyFrameDeltas[skey] *
                       (float)(part_age - SizeKeyFrameTimes[skey]) +
                   RandomSizeEntries[part & NumRandomSizeEntriesMinus1];

      // Size (unlike color and alpha) isn't clamped in the engine, so we need
      // to clamp negative values to zero here:
      size[part] = (size[part] >= 0.0f) ? size[part] : 0.0f;
    }

    // Ensure the current rotation keyframe is correct, and calculate
    // orientation state
    if (orientation) {
      // We go from older to younger particles, so we go backwards from the last
      // keyframe until age >= keytime. This loop must terminate because the 0th
      ...
      float tmp_orient =
          OrientationKeyFrameValues[rkey] +
          (RotationKeyFrameValues[rkey] +
           HalfRotationKeyFrameDeltas[rkey] * f_delta_t) *
              f_delta_t +
          RandomRotationEntries[part & NumRandomRotationEntriesMinus1] *
              (float)part_age +
    ...
    if (frame) {
      ...
      float tmp_frame = FrameKeyFrameValues[fkey] +
                        FrameKeyFrameDeltas[fkey] *
                            (float)(part_age - FrameKeyFrameTimes[fkey]) +
...
void ParticleBufferClass::Get_New_Particles(void) {
  ...
  // position is the current frame position, prev_pos is the previous frames
  // position (only if we have enabled pingpong position buffers)
  Vector3 *position;
  Vector3 *prev_pos;
  if (PingPongPosition) {
  ...
  for (; NewParticleQueueCount;) {
    ...
    if (HasAccel) {
      ...
      position[NewEnd] =
          new_particle.Position + (new_particle.Velocity * fp_age);
...
void ParticleBufferClass::Get_Color_Key_Frames(
    ParticlePropertyStruct<Vector3> &colors) const {
  int real_keyframe_count =
      (NumColorKeyFrames > 0) ? (NumColorKeyFrames - 1) : 0;
  bool create_last_keyframe = false;

  //
  //	Determine if there is a keyframe at the very end of the particle's
  //lifetime
  //
...
void ParticleBufferClass::Get_Opacity_Key_Frames(
    ParticlePropertyStruct<float> &opacities) const {
  int real_keyframe_count =
      (NumAlphaKeyFrames > 0) ? (NumAlphaKeyFrames - 1) : 0;
  bool create_last_keyframe = false;
...
void ParticleBufferClass::Get_Size_Key_Frames(
    ParticlePropertyStruct<float> &sizes) const {
...
void ParticleBufferClass::Get_Rotation_Key_Frames(
    ParticlePropertyStruct<float> &rotations) const {
  int real_keyframe_count =
      (NumRotationKeyFrames > 0) ? (NumRotationKeyFrames - 1) : 0;
  bool create_last_keyframe = false;

  /*
  ** NOTE: Rotations are stored internally in rotations per millisecond. These
  *will be converted to rotations per second.
  */
  ...
  // Convert the rotation values from rotations per millisecond to rotations per
  // second.
  rotations.Start =
      RotationKeyFrameValues ? RotationKeyFrameValues[0] * 1000.0f : 0;
  ...
  if (real_keyframe_count > 0) {
    ...
    for (index = 1; index < NumRotationKeyFrames; index++) {
      rotations.KeyTimes[index - 1] =
          ((float)RotationKeyFrameTimes[index]) / 1000;
    ...
    if (create_last_keyframe) {
      ...
      rotations.Values[index - 1] =
          (start_rotation + (delta * time_delta)) * 1000.0f;
    }
  }
...
void ParticleBufferClass::Get_Frame_Key_Frames(
    ParticlePropertyStruct<float> &frames) const {
  int real_keyframe_count =
      (NumFrameKeyFrames > 0) ? (NumFrameKeyFrames - 1) : 0;
  bool create_last_keyframe = false;
...
void ParticleBufferClass::Set_LOD_Max_Screen_Size(int lod_level,
