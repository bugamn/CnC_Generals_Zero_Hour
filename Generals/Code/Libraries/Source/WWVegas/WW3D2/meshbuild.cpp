 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * MeshBuilderClass::VertClass::Reset -- reset this vertex *
 *   MeshBuilderClass::FaceClass::Reset -- rest this face *
 *   MeshBuilderClass::FaceClass::Compute_Plane -- compute the plane for this
 *face             * MeshBuilderClass::FaceClass::Is_Degenerate -- check if a
 *face is degenerate               * MeshBuilderClass::MeshStatsStruct::Reset --
 *reset the stats to all false                  *
 *   MeshBuilderClass::MeshBuilderClass -- constructor *
 *   MeshBuilderClass::~MeshBuilderClass -- destructor * MeshBuilderClass::Free
 *-- release all memory in use                                       *
 ...
 *   MeshBuilderClass::Compute_Face_Normals -- computes all of the face normals
 *from the index * MeshBuilderClass::Verify_Face_Normals -- checks if any faces
 *have flipped                 * MeshBuilderClass::Compute_Vertex_Normals --
 *Computes the vertex normals for the mesh      *
 *   MeshBuilderClass::Remove_Degenerate_Faces -- discard invalid or duplicated
 *faces          * MeshBuilderClass::Compute_Mesh_Stats -- compute some stats
 *about the mesh                 * MeshBuilderClass::Compute_Bounding_Box --
 *computes an axis-aligned bounding box for the m *
 *   MeshBuilderClass::Compute_Bounding_Sphere -- computes a bounding sphere for
 *the mesh      * MeshBuilderClass::Optimize_Mesh -- "optimize" the mesh *
 *   MeshBuilderClass::Strip_Optimize_Mesh -- optimize the mesh for triangle
 *strips            * MeshBuilderClass::Grow_Face_Array -- increases the size of
 *the face array                 * MeshBuilderClass::Sort_Vertices -- Sorts
 *vertices according to the given key array        *
 *   MeshBuilderClass::Sort_Vertices_By_Bone_Index -- sorts verts by bone index
 ** MeshBuilderClass::Sort_Vertices_By_Vertex_Material -- sorts verts by vertex
 *mtl in pass0  *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
COMPARE_FUNC_TYPE
    Texture_Compare_Funcs[MeshBuilderClass::MAX_PASSES]
                         [MeshBuilderClass::MAX_STAGES] = {
...
** appears that the artists occasionally accidentally duplicate a face which
*causes
** problems in the stripping algorithm...
**
************************************************************************************/
class FaceHasherClass
    : public HashCalculatorClass<MeshBuilderClass::FaceClass> {
 public:
  virtual bool Items_Match(const MeshBuilderClass::FaceClass &a,
                           const MeshBuilderClass::FaceClass &b) {
    // Note: if we want this to detect duplicates that are "rotated", must
    // change both this function and the Compute_Hash function...
    return ((a.VertIdx[0] == b.VertIdx[0]) && (a.VertIdx[1] == b.VertIdx[1]) &&
            (a.VertIdx[2] == b.VertIdx[2]));
  }

  virtual void Compute_Hash(const MeshBuilderClass::FaceClass &item) {
    HashVal = (int)(item.VertIdx[0] * 12345.6f + item.VertIdx[1] * 1714.38484f +
                    item.VertIdx[2] * 27561.3f) &
              1023;
  }

  virtual int Num_Hash_Bits(void) { return 10; }

  virtual int Num_Hash_Values(void) { return 1; }
...
** template due to the special considerations needed to properly handle
*smoothing
...
class VertexArrayClass {
 public:
  ...
  VertexArrayClass(int maxsize, int match_normals = 0) {
    ...
    memset(HashTable, 0,
           sizeof(MeshBuilderClass::VertClass *) * HASH_TABLE_SIZE);
  ...
  int Submit_Vertex(const MeshBuilderClass::VertClass &vert) {
    ...
    for (x = xstart - EPSILON; x <= xstart + EPSILON + 0.0000001;
         x += EPSILON) {
      for (y = ystart - EPSILON; y <= ystart + EPSILON + 0.000001;
           y += EPSILON) {
          ...
          while (test_vert) {
            if (Verts_Shading_Match(vert, *test_vert)) {
              if (shadeindex == 0xFFFFFFFF) {
                ...
                // face's smoothing group since we are going to be smoothed with
                // it.
    ...
    if (shadeindex == 0xFFFFFFFF) {
      ...
      // This is a new vertex,so store the face's smoothing group into
      // SharedSmGroup
  ...
  int Verts_Match(const MeshBuilderClass::VertClass &v0,
    ...
    if (MatchNormals == 0) {
      int smgroup_match =
          ((v0.SmGroup & v1.SmGroup) || (v0.SmGroup == v1.SmGroup));
  ...
  int Verts_Shading_Match(const MeshBuilderClass::VertClass &v0,
                          const MeshBuilderClass::VertClass &v1) {
    float dv = (v0.Position - v1.Position).Length();
    int smgroup_match =
        ((v0.SmGroup & v1.SmGroup) || (v0.SmGroup == v1.SmGroup));

    if ((dv < EPSILON) && (smgroup_match) && (v0.Id == v1.Id)) {
      return 1;
    }
    return 0;
  }
...
void MeshBuilderClass::FaceClass::Compute_Plane(void) {
  ...
  Normal = Vector3::Cross_Product((Verts[1].Position - Verts[0].Position),
                                  (Verts[2].Position - Verts[0].Position));
  Normal.Normalize();
#else
  Vector3::Normalized_Cross_Product((Verts[1].Position - Verts[0].Position),
                                    (Verts[2].Position - Verts[0].Position),
                                    &Normal);
#endif
 ...
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * assumes we have already built the unique vertex table and
 *installed their indices           *
 *                                                                                             *
 * HISTORY: * 7/7/98     GTH : Created. *
 *=============================================================================================*/
 ...
 * HISTORY: *
 *=============================================================================================*/
MeshBuilderClass::MeshBuilderClass(int pass_count, int face_count_guess,
 ...
 * HISTORY: * 5/15/98    GTH : Created. *
 *=============================================================================================*/
void MeshBuilderClass::Reset(int passcount, int face_count_guess,
                             int face_count_growth_rate) {
  Free();
 ...
 * MeshBuilderClass::Build_Mesh -- process the mesh *
 *                                                                                             *
 *    This function finds all sharable vertices, calculates vertex normals,
 *sorts the          * faces by material, sorts the faces into stripping
 *order...                               *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 7/10/98    GTH : Created. * 10/19/98   GTH : Modified to use the
 *FaceClass::Compute_Plane function                    *
 *=============================================================================================*/
...
#ifdef ALLOW_TEMPORARIES
    Vector3 normal = Vector3::Cross_Product((v1.Position - v0.Position),
                                            (v2.Position - v0.Position));
    normal.Normalize();
#else
    Vector3 normal;
    Vector3::Normalized_Cross_Product((v1.Position - v0.Position),
                                      (v2.Position - v0.Position), &normal);
#endif
...
/***********************************************************************************************
 * MeshBuilderClass::Compute_Vertex_Normals -- Computes the vertex normals for
 *the mesh        *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * This function should only be called after the mesh has been
 *"optimized".  The algorithm  * relies on non-smooth vertices having been
 *split...                                       *
 *                                                                                             *
 * HISTORY: * 5/15/98    GTH : Created. *
 *=============================================================================================*/
  ...
  if (WorldInfo != NULL && WorldInfo->Are_Meshes_Smoothed()) {
    for (vertidx = 0; vertidx < VertCount; vertidx++) {
      if (Verts[vertidx].ShadeIndex == vertidx) {
        Verts[vertidx].Normal += WorldInfo->Get_Shared_Vertex_Normal(
            Verts[vertidx].Position, Verts[vertidx].SharedSmGroup);
      }
    }
  }

  /*
  ** Propogate the accumulated normals to all of the other verts which share
  *them
  */
  for (vertidx = 0; vertidx < VertCount; vertidx++) {
...
void MeshBuilderClass::Remove_Degenerate_Faces(void) {
  ...
  UniqueArrayClass<MeshBuilderClass::FaceClass> uniquefaces(
      FaceCount, FaceCount / 4, &facehasher);
  ...
  for (pass = 0; pass < MAX_PASSES; pass++) {
    for (stage = 0; stage < MAX_STAGES; stage++) {
      for (face_index = 0; face_index < FaceCount; face_index++) {
        if (Faces[face_index].TextureIndex[pass][stage] !=
            tex_index[pass][stage]) {
...
/***********************************************************************************************
 * MeshBuilderClass::Compute_Bounding_Box -- computes an axis-aligned bounding
 *box for the mes *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 *                                                                                             *
 *    Generates the array of unique vertices and sets the vertex indices in each
 *face.  Then   * all of the faces are sorted by material and then into
 *stripping order.                   *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void MeshBuilderClass::Optimize_Mesh(bool compute_normals) {
  ...
  **
  ** NOTE: Most meshes should be able to use *one* vertex material and *one*
  *shader
  ** We should throw up a warning flag in the asset manager whenever we
  *encounter
  ...
  */
  qsort(Faces, FaceCount, sizeof(FaceClass),
        Texture_Compare_Funcs[PolyOrderPass][PolyOrderStage]);
 ...
 *                                                                                             *
 * This algorithm was copied/modified from the surrender srModeller code so that
 *we don't have * to pass our models through the modeller at run-time. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 4/6/98     GTH : Created. * 10/20/98   GTH : Modified to strip
 *based on the desired texture channel                   * 2/11/99    GTH :
 *Added strip stats tracking                                               *
 *=============================================================================================*/
void MeshBuilderClass::Strip_Optimize_Mesh(void) {
  ...
  for (i = 0; i < FaceCount; i++) {
    ...
    for (j = 0; j < 3; j++) {
      // build the WingedEdgePolyStruct for this face.  basically, for each
      // edge, see if we already have the edge and if so, link to it.  otherwise
  ...
  while (polysinserted < FaceCount) {
    ...
    // first attempt to minimize material switches by choosing a polygon with
    // same material as the starting poly. Basically what we want is the poly
    // with same material with least shared edges and most recent vertices (as
    // we'd like to start the strip from a 'corner polygon'). This pass is done
    // only if mesh has multiple materials. The second pass scans through all
    // polygons.
    ...
    for (findpass = startpass; findpass < 2; findpass++) {
      ...
      for (i = 0; i < FaceCount; i++) {
        ...
        if (premap[i] == -1) {
          ...
          if (findpass == 0 &&
              Faces[i].TextureIndex[PolyOrderPass][PolyOrderStage] != lastmat)
            continue;
    ...
    newmat[polysinserted] =
        Faces[startpoly].TextureIndex[PolyOrderPass][PolyOrderStage];

    // Add the selected polygon to the new polygon list.
    // for each edge of start poly, see if the "other" polygon using that edge
    // is untaken, if so, store the new polygon such that that edge index is
    // last
    ...
    for (int edge_index = 0; (edge_index < 3) && !found_shared_edge;
         edge_index++) {
      for (int side_index = 0; (side_index < 2) && !found_shared_edge;
           side_index++) {
        // if this polygon is not the startpoly and it is not "taken", then this
        // edge is ok!
        ...
        if ((poly != -1) && (poly != startpoly) && (premap[poly] == -1)) {
          ...
          for (int vidx = 0; vidx < 3; vidx++) {
            if ((newpoly->VertIdx[vidx] !=
                 pedges[startpoly].Edge[edge_index]->Vertex[0]) &&
                (newpoly->VertIdx[vidx] !=
                 pedges[startpoly].Edge[edge_index]->Vertex[1])) {
          ...
          // rotate the vertex indices until first_vert is the index of
          // VertIdx[0]
    ...
    // Increment the count. Mark the vertices as used (i.e. update their
    // timestamps)
    ...
    while (nextpoly != -1) {
      ...
      for (i = 0; i < 3; i++) {
        ...
        if ((pedges[startpoly].Edge[i]->Vertex[0] == vFIFO[0] &&
             pedges[startpoly].Edge[i]->Vertex[1] == vFIFO[1]) ||
            (pedges[startpoly].Edge[i]->Vertex[1] == vFIFO[0] &&
             pedges[startpoly].Edge[i]->Vertex[0] == vFIFO[1])) {
          for (j = 0; j < 2; j++) {
            // if poly 'j' attached to this edge has not been used already, use
            // it!
    ...
    found:;
      ...
      int nw = -1;
      for (i = 0; i < 3; i++) {
        if (Faces[nextpoly].VertIdx[i] != vFIFO[0] &&
            Faces[nextpoly].VertIdx[i] != vFIFO[1]) {
          nw = i;
          break;
        }
      }
      ...
      newmat[polysinserted] =
          Faces[nextpoly].TextureIndex[PolyOrderPass][PolyOrderStage];
  ...
  for (i = 0; i < FaceCount; i++) {
    ...
    for (int pass = 0; pass < MAX_PASSES; pass++) {
      for (int stage = 0; stage < MAX_STAGES; stage++) {
        newpolys[new_idx].TextureIndex[pass][stage] =
            Faces[old_idx].TextureIndex[pass][stage];
      }
    ...
    // just checking
    assert(newmat[new_idx] ==
           Faces[old_idx].TextureIndex[PolyOrderPass][PolyOrderStage]);
  }
...
inline int tex_compare(const void *elem1, const void *elem2, int pass,
                       int stage) {
  MeshBuilderClass::FaceClass *f0 = (MeshBuilderClass::FaceClass *)elem1;
  MeshBuilderClass::FaceClass *f1 = (MeshBuilderClass::FaceClass *)elem2;
  ...
  */
  if (f0->Verts[0].VertexMaterialIndex[pass] <
      f1->Verts[0].VertexMaterialIndex[pass])
    return -1;
  if (f0->Verts[0].VertexMaterialIndex[pass] >
      f1->Verts[0].VertexMaterialIndex[pass])
    return 1;
