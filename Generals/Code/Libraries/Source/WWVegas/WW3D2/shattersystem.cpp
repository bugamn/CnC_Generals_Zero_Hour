...
class MeshMtlParamsClass {
 public:
  ...
  /*
  ** NOTE: currently I do not fully support stage 1 textures or texture arrays
  *in any pass.
  ...
  */
  ...
  //	Vector3i *
  //UVIndexArray[MeshMatDescClass::MAX_PASSES];
};
...
class VertexClass {
 public:
  ...
  unsigned DCG[MeshMatDescClass::MAX_PASSES];
  unsigned DIG[MeshMatDescClass::MAX_PASSES];
  Vector2 TexCoord[MeshMatDescClass::MAX_PASSES]
                  [MeshMatDescClass::MAX_TEX_STAGES];

  static void Lerp(const VertexClass &v0, const VertexClass &v1, float lerp,
                   VertexClass *res);
  static void Intersect_Plane(const VertexClass &v0, const VertexClass &v1,
                              const PlaneClass &plane, VertexClass *res);
};
...
class PolygonClass {
 public:
  ...
  int Which_Side(const PlaneClass &plane) const;
  void Split(const PlaneClass &plane, PolygonClass &front,
...
MeshMtlParamsClass::MeshMtlParamsClass(MeshModelClass *model) {
  ...
  for (int ipass = 0; ipass < MeshMatDescClass::MAX_PASSES; ipass++) {
    ...
    for (int istage = 0; istage < MeshMatDescClass::MAX_TEX_STAGES; istage++) {
      UV[ipass][istage] =
          const_cast<Vector2 *>(model->Get_UV_Array(ipass, istage));
    }
  }
}
...
VertexClass::VertexClass(void)
    : Position(0, 0, 0), Normal(0, 0, 1), PassCount(0) {
  for (int ipass = 0; ipass < MeshMatDescClass::MAX_PASSES; ipass++) {
...
VertexClass &VertexClass::operator=(const VertexClass &that) {
  if (this != &that) {
    ...
    for (int ipass = 0; ipass < PassCount; ipass++) {
      DCG[ipass] = that.DCG[ipass];
      DIG[ipass] = that.DIG[ipass];
      for (int istage = 0; istage < MeshMatDescClass::MAX_TEX_STAGES;
           istage++) {
        TexCoord[ipass][istage] = that.TexCoord[ipass][istage];
      }
    }
...
void VertexClass::Lerp(const VertexClass &v0, const VertexClass &v1, float lerp,
                       VertexClass *res) {
  ...
  for (int ipass = 0; ipass < v0.PassCount; ipass++) {
    ...
    for (int istage = 0; istage < MeshMatDescClass::MAX_TEX_STAGES; istage++) {
      Vector2::Lerp(v0.TexCoord[ipass][istage], v1.TexCoord[ipass][istage],
                    lerp, &(res->TexCoord[ipass][istage]));
    }
  }
}
...
void VertexClass::Intersect_Plane(const VertexClass &p0, const VertexClass &p1,
                                  const PlaneClass &plane, VertexClass *res) {
  float alpha = 0.0f;
...
void PolygonClass::Compute_Plane(void) {
  ...
  for (i = 0; i < NumVerts; i++) {
    ...
    nx += (double)(Verts[i].Position.Y - Verts[j].Position.Y) *
          (double)(Verts[i].Position.Z + Verts[j].Position.Z);
    ny += (double)(Verts[i].Position.Z - Verts[j].Position.Z) *
          (double)(Verts[i].Position.X + Verts[j].Position.X);
    nz += (double)(Verts[i].Position.X - Verts[j].Position.X) *
          (double)(Verts[i].Position.Y + Verts[j].Position.Y);
...
void PolygonClass::Split(const PlaneClass &plane, PolygonClass &front,
                         PolygonClass &back) const {
  front = *this;
  back = *this;
  ...
  int side = BPT_ON;
  int i;
  for (i = 0; (i < NumVerts) && ((side = Verts[i].Which_Side(plane)) == BPT_ON);
       i++);
...
void ShatterSystem::Init(void) {
  ...
  */
  ...
  /*
  ** Search for ShatterPattern hierarchy tree objects, beginning with
  *ShatterPattern00
...
#endif
  while (htree != NULL) {
    if ((htree->Num_Pivots() > 1) &&
        (htree->Num_Pivots() < MAX_MESH_FRAGMENTS)) {
      int leaf_counter = 0;
...
void ShatterSystem::Shatter_Mesh(MeshClass *mesh, const Vector3 &point,
  ...
  for (ipass = 0; ipass < model->Get_Pass_Count(); ipass++) {
    ...
    for (istage = 0; istage < MeshMatDescClass::MAX_TEX_STAGES; istage++) {
      if (model->Has_Texture_Array(ipass, istage)) {
        WWDEBUG_SAY(
            ("Failed to shatter model: %s.  Texture array in pass: %d stage: "
             "%d\n",
  ...
  */
  ...
  Mshatter_to_world.Look_At(point, point + direction,
                            0.0f);  // TODO: could put random roll
  ...
  */
  ...
  /*
  ** Scaling matrices.  This could be simpler if Matrix3D had a full inverse
  *function.
  */
  SphereClass sphere;
  ...
  float scale_factor =
      5.0f / sphere.Radius;  // mesh scales to 5x shatter pattern.
  ...
  */
  ...
  VectorProcessorClass::Transform(verts, src_verts, Mobj_to_shatter,
    ...
    */
    ...
    for (ivert = 0; ivert < 3; ivert++) {
      ...
      SHATTER_DEBUG_SAY(("position: %f %f %f\n", verts[vert_index].X,
                         verts[vert_index].Y, verts[vert_index].Z));
      SHATTER_DEBUG_SAY(("normal: %f %f %f\n", src_vnorms[vert_index].X,
      ...
      for (ipass = 0; ipass < MeshMatDescClass::MAX_PASSES; ipass++) {
        if (mtl_params.DCG[ipass] != NULL) {
          ...
          SHATTER_DEBUG_SAY(("DCG: pass: %d : %f %f %f\n", ipass,
                             mtl_params.DCG[ipass][vert_index].X,
                             mtl_params.DCG[ipass][vert_index].Y,
                             mtl_params.DCG[ipass][vert_index].Z));
        }
        ...
        if (mtl_params.DIG[ipass] != NULL) {
          ...
          SHATTER_DEBUG_SAY(("DIG: pass: %d : %f %f %f\n", ipass,
                             mtl_params.DIG[ipass][vert_index].X,
                             mtl_params.DIG[ipass][vert_index].Y,
                             mtl_params.DIG[ipass][vert_index].Z));
        }
        ...
        for (istage = 0; istage < MeshMatDescClass::MAX_TEX_STAGES; istage++) {
          if (mtl_params.UV[ipass][istage] != NULL) {
            ...
            SHATTER_DEBUG_SAY(("UV pass: %d stage: %d: %f %f\n", ipass, istage,
                               mtl_params.UV[ipass][istage][vert_index].X,
                               mtl_params.UV[ipass][istage][vert_index].Y));
          }
        }
  ...
  for (int ipool = 0; ipool < MAX_MESH_FRAGMENTS; ipool++) {
    if (ClipPools[ipool].Count() > 0) {
      ...
      */
      ...
      for (ipass = 0; ipass < model->Get_Pass_Count(); ipass++) {
        new_mesh->Set_Vertex_Material(model->Peek_Single_Material(ipass), false,
                                      ipass);
      ...
      for (ipoly = 0; ipoly < ClipPools[ipool].Count(); ipoly++) {
        ...
        for (ivert = 0; ivert < poly.Get_Vertex_Count(); ivert++) {
          ...
          for (ipass = 0; ipass < mtl_params.PassCount; ipass++) {
            ...
            /*
            ** If there were vertex colors for this pass in the original mesh,
            *then
            ** copy the color out of the vertex into the new mesh.
            */
            if (mtl_params.DCG[ipass] != NULL) {
              SHATTER_DEBUG_SAY(("DCG: pass:%d: %f %f %f\n", ipass,
                                 vert.DCG[ipass].X, vert.DCG[ipass].Y,
                                 vert.DCG[ipass].Z));
              /* OLD CODE
            ...
            if (mtl_params.DIG[ipass] != NULL) {
              SHATTER_DEBUG_SAY(("DIG: pass:%d: %f %f %f\n", ipass,
                                 vert.DIG[ipass].X, vert.DIG[ipass].Y,
                                 vert.DIG[ipass].Z));
            ...
            /*
            ** If there were UV coordinates in the original mesh for either
            *stage,
            ** then copy the vertex's uv's into into the new mesh.
            */
            for (istage = 0; istage < MeshMatDescClass::MAX_TEX_STAGES;
                 istage++) {
              if (mtl_params.UV[ipass][istage] != NULL) {
                SHATTER_DEBUG_SAY(("UV: pass:%d stage: %d: %f %f\n", ipass,
                                   istage, vert.TexCoord[ipass][istage].X,
