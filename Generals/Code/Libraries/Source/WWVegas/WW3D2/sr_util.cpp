 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * Set_SR_Transform -- copies our object transform into a Surrender
 *object                   * Set_SR_Camera_Transform -- copies our camera
 *transform into a Surrender object            * Get_SR_Camera_Transform --
 *creates a Matrix3D from a surrender camera transform           *
...
/***********************************************************************************************
 * Set_SR_Camera_Transform -- copies our camera transform into a Surrender
 *object              *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * Get_SR_Camera_Transform -- creates a Matrix3D from a surrender camera
 *transform             *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void Convert_Westwood_Matrix(const Matrix3D& wtm, srMatrix3* set_sr_tm,
...
void Multiply_Westwood_And_Surrender_Matrix(const Matrix3D& n,
                                            const srMatrix4& m,
                                            srMatrix4& srtm_d) {
  srtm_d[0].make(
  ...
  srtm_d[3].make(
      m[3][0] * n[0][0] + m[3][1] * n[1][0] + m[3][2] * n[2][0],
...
void Get_Camera_Frustum_Corners(const CameraClass* camera, Vector3 points[8]) {
  ...
  // points are numbered: upper left 0, upper right 1, lower left 2, lower
  // right 3. The far plane's points are numbered from 4 to 7 in an analogous
  // fashion.
  // (remember: the camera space has x going to the right, y up and z
  // backwards).
  Vector2 vpmin, vpmax;
  double znear, zfar;
  camera->Get_View_Plane(vpmin,
                         vpmax);  // Normalized view plane at a depth of 1.0
 ...
 *                                                                        *
 * OUTPUT:	(parameter) Vector3 points[8] - array of corner points.       *
 *				returns false of the intersection between the
 *clamped range   * and the frustum is empty. *
 *                                                                        *
 * WARNINGS:	points must be an array of length 8 at least, or bad       *
 *             things will happen (this is not checked by the compiler).  *
 *                                                                        *
