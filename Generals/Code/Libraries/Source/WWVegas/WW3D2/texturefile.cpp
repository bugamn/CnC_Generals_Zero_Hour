...
void TextureFileClass::Switch_Mipmaping_Debug() {
  ...
  while (n) {
    if (mipmaps)
      n->params.setMipmap(MIPMAP_DEFAULT);
    else
      n->params.setMipmap(MIPMAP_NONE);
    n = n->Succ;
  }
}
...
int TextureFileClass::Get_Total_Texture_Count() { return texture_count; }
...
StringClass
TextureFileClass::List_Missing_Files()  // Print out a list of missing files
{
...
TextureFileClass::TextureFileClass(const char* filename)
  ...
  // Load locked surface (or not) according to current locked surface reduction
  // factor set in texture loader
  Load_Locked_Surface();
  ...
  if (WW3D::Get_Texture_Thumbnail_Mode() !=
      WW3D::TEXTURE_THUMBNAIL_MODE_RESIZING) {
...
TextureFileClass::TextureFileClass(const TextureFileClass& src) { WWASSERT(0); }
...
srTextureIFace::FrameHandle TextureFileClass::getTextureFrameHandle(void) {
  if (flags.test(GENERATESURFACE_FAILURE)) return 0;
...
void TextureFileClass::getMipmapData(MultiRequest& m) {
  ...
  /*
  ** If the temporary surface pointer points to a surface (this will be a new
  *surface which just
  ** got loaded in this frame) use it and release it - done.
  ** Otherwise, if we have a locked surface, then normally the current reduction
  *factor should
  ** be equal or greater to the locked surface reduction factor so we can use
  *the locked surface
  ** (possibly scaled down). If the current reduction factor is less than that
  *of the locked
  ** surface, this means that in the past we have loaded up a surface larger
  *than the locked
  ** surface, and it has just been thrown out of the API/GERD/HW texture cache.
  *In this case we
  ** have no choice but to scale the locked surface up to fill the surface
  *handed to us, and we
  ** will request a background load task for the real surface.
  ** If there is no locked surface, this means that the texture is never reduced
  *or loaded in
  ** the background - we perform a normal blocking load of the surface, use it,
  *and release it
  ** (if you want behavior similar to "cached mode" in srTextureFile you should
  *set the locked
  ...
  if (TempSurfacePtr) {
    ...
    if (LockedSurface) {
      ...
      if (CurrentReductionFactor < LockedSurfaceReductionFactor) {
        ...
        // If int_desired_reduction_factor is smaller than the locked surface
        // reduction factor (meaning we want a larger surface), request to load
        // it.
        ...
        // This else clause should only be entered in cases where
        // Process_Reduction() is not called every frame on the texture for some
        // reason. Such textures will not resize, but this at least ensures that
...
void TextureFileClass::setupDefaultValues(void) {
  if (flags.test(DIRTY_DEFAULTS)) {
    ...
    ** If the temporary surface pointer points to a surface (this will be a new
    *surface which
    ** just got loaded in this frame by Apply_New_Surface) get defaults from it
    *- done.
    ** Otherwise, if we have a locked surface, then normally the current
    *reduction factor should
    ** be equal or greater to the locked surface reduction factor so we can get
    *defaults from
    ** the locked surface (possibly scaled down). If the current reduction
    *factor is less than
    ** that of the locked surface, this means that in the past we have loaded up
    *a surface
    ** larger than the locked surface, and it has just been thrown out of the
    *API/GERD/HW
    ** texture cache. In this case we set the current reduction factor to be
    *equal to that of
    ** the locked surface, and get defaults from the locked surface.
    ** If there is no locked surface, this means that the texture is never
    *reduced or loaded in
    ** the background - we perform a normal blocking load of the surface, store
    *it in the temp
    ** surface pointer (so getMipmapData can also use it), and get defaults from
    *it.
    */
    if (TempSurfacePtr) {
      setupDefaultValuesFromSurface(TempSurfacePtr);
      TextureSize = Calculate_Size(TempSurfacePtr);
      NonReducedTextureSize =
          Calculate_Size(TempSurfacePtr, CurrentReductionFactor);
    } else {
      if (LockedSurface) {
        setupDefaultValuesFromSurface(LockedSurface);
        if (CurrentReductionFactor >= LockedSurfaceReductionFactor) {
          // Scale size down by difference between current and locked reduction
          // factors
          unsigned int diff =
              CurrentReductionFactor - LockedSurfaceReductionFactor;
...
// This is used by an object to request a reduction factor on all its textures.
// Only the smallest reduction factor requested in a given frame is preserved.
void TextureFileClass::Request_Reduction(float reduction_factor) {
  ...
  // If this is the first request in this time-stamp, overwrite desired
  // reduction factor - otherwise current factor is the lesser of current and
  // requested.
...
// This is used during rendering - if a textures desired reduction level is
// "different enough" from its current one, we fix it: either by updating from
// the locked surface (if the reduction factor is equal or greater to that of
// the locked surface) or by asking the texture loader to load a new reduction
// level of the texture in the background. NOTE - if there is no locked surface,
// we do nothing (since textures without locked surfaces do not perform
// reduction).
void TextureFileClass::Process_Reduction(void) {
  // Check if this is the first Process_Reduction() call this frame - otherwise
  // do nothing
  if (TimeStampOfLastProcessReductionCall != _CurrentTimeStamp) {
    // If we are doing reductions and the difference between the current and
    // desired reduction factors is above the threshold, fix it.
    if (LockedSurface && fabs(float(CurrentReductionFactor) -
                              DesiredReductionFactor) > _SwitchThreshold) {
...
float TextureFileClass::_Get_Switch_Threshold(void) { return _SwitchThreshold; }
...
void TextureFileClass::Load_Temp_Surface(void) {
  ...
  //
  TextureFileCache* cache =
      WW3DAssetManager::Get_Instance()->Texture_File_Cache();
  if (cache != NULL) {
    cache->Validate_Texture(FileName);
    TempSurfacePtr = cache->Get_Surface(Get_File_Name(),
                                        0);  // No reduction!
  } else {
...
void TextureFileClass::Fill_Multi_Request_From_Surface(
    MultiRequest& m, srColorSurfaceIFace* surface) {
...
void TextureFileClass::Set_Texture_Flash(bool b) {
  ...
  if (flash) {
    ...
    // Restore the original surface... the surface will be released after
    // applying to use
    invalidateFrameHandle(TextureFrameHandle);
    Release_Temp_Surface();
...
bool TextureFileClass::Get_Texture_Flash() const { return flash; }
  ...
  while (n) {
    if (n->flash) {
      ...
      if (s) {
        ...
        n->TempSurfacePtr
            ->addReference();  // Surface will be release after the use
      }
    }
    n = n->Succ;
  }
...
// If false is passed, the texture will load the largest LOD size and stick to
// that
...
void TextureFileClass::Load_Locked_Surface() {
  ...
  LockedSurfaceReductionFactor = 0;
  if (FileName) {
    TextureLoader::Load_Locked_Surface_Immediate(FileName, LockedSurface,
                                                 LockedSurfaceReductionFactor);

    if (LockedSurface) {
      // If we have a locked surface, set the current reduction factor to be
      // equal to the locked surface reduction factor (this is the highest
      // resolution that we can render with at this time)
      CurrentReductionFactor = LockedSurfaceReductionFactor;
