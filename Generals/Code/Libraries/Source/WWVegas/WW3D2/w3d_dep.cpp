 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * Get_W3D_Dependencies -- Scans a W3D file to determine which
 *other files it depends on.    * Scan_Chunk -- Chooses the correct chunk loader
 *for this chunk.                            * Scan_Mesh -- Scans a mesh for
 *references to other files.                                  * Scan_Mesh_Header
 *-- Scans a mesh's header for file references.                            *
 *   Scan_Mesh_Textures -- Scans a mesh's textures. * Scan_HTree -- Scans an
 *HTree for references to other files.                               * Scan_Anim
 *-- Scans an animation for references to other files. * Scan_Compressed_Anim --
 *Scans an animation for references to other files.                 *
 ...
 *   Scan_HLOD -- Scans an HLOD for references to other files. * Get_W3D_Name --
 *Gets a W3D object name from a W3D filename.                               *
...
static void Scan_Chunk(ChunkLoadClass &cload, StringList &files,
                       const char *w3d_name);

static void Scan_Mesh(ChunkLoadClass &cload, StringList &files,
                      const char *w3d_name);
static void Scan_Mesh_Header(ChunkLoadClass &cload, StringList &files,
                             const char *w3d_name);
static void Scan_Mesh_Textures(ChunkLoadClass &cload, StringList &files,
                               const char *w3d_name);

static void Scan_Anim(ChunkLoadClass &cload, StringList &files,
                      const char *w3d_name);
static void Scan_Compressed_Anim(ChunkLoadClass &cload, StringList &files,
                                 const char *w3d_name);
static void Scan_HModel(ChunkLoadClass &cload, StringList &files,
                        const char *w3d_name);
static void Scan_Emitter(ChunkLoadClass &cload, StringList &files,
                         const char *w3d_name);
static void Scan_Aggregate(ChunkLoadClass &cload, StringList &files,
                           const char *w3d_name);
static void Scan_HLOD(ChunkLoadClass &cload, StringList &files,
                      const char *w3d_name);
...
/***********************************************************************************************
 * Get_W3D_Dependencies -- Scans a W3D file to determine which other files it
 *depends on.      *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * Scan_Compressed_Anim -- Scans a compressed animation mesh for references to
 *other files.    *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
  ...
  while (cload.Open_Chunk()) {
    ...
    if (cload.Cur_Chunk_ID() == W3D_CHUNK_AGGREGATE_INFO) {
      ...
      if (strcmp(chunk.BaseModelName, w3d_name) != 0) {
        ...
        if (*base_model) files.push_back(base_model);
      }
      ...
      for (i = 0; i < chunk.SubobjectCount; ++i) {
        ...
        if (strcmp(subchunk.SubobjectName, w3d_name) != 0) {
          ...
          if (*subobject) files.push_back(subobject);
        }
      }
    }
    cload.Close_Chunk();
...
static void Scan_HLOD(ChunkLoadClass &cload, StringList &files,
  ...
  while (cload.Open_Chunk()) {
    ...
    if (cload.Cur_Chunk_ID() == W3D_CHUNK_HLOD_HEADER) {
      ...
      if (strcmp(hdr.HierarchyName, w3d_name) != 0) {
        const char *hierarchy = Make_W3D_Filename(hdr.HierarchyName);
        if (*hierarchy) files.push_back(hierarchy);
      }
    }
    cload.Close_Chunk();
  }
...
static void Get_W3D_Name(const char *filename, char *w3d_name) {
  ...
  if (!end) end = start + strlen(start);  // point to the null character
  ...
  strncpy(w3d_name, start,
          num_chars < W3D_NAME_LEN ? num_chars : W3D_NAME_LEN - 1);
  strupr(w3d_name);
}
