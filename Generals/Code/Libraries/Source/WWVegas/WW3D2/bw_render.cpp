...
void BW_Render::Fill(unsigned char c) { pixel_buffer.Fill(c); }

// Sets the vertex coordinate buffer location and scales the vertex locations to
// the kjkj
void BW_Render::Set_Vertex_Locations(Vector2* vertices_, int count) {
  vertices = vertices_;

  float half_scale = pixel_buffer.Scale() * 0.5f;
  VectorProcessorClass::MulAdd(reinterpret_cast<float*>(vertices), half_scale,
                               half_scale, count * 2);
}

// --------------------------------------------------------------------

static inline bool Cull(const Vector2& c1, const Vector2& c2,
                        const Vector2& c3) {
  float x1 = c2[0] - c1[0];
  float y1 = c2[1] - c1[1];
  float x2 = c3[0] - c1[0];
...
void BW_Render::Render_Triangle_Strip(const unsigned long* indices,
  ...
  for (int n = 0; n < index_count; ++n) {
    ...
    Vector3i yci(WWMath::Float_To_Long(corner_1[1]),
                 WWMath::Float_To_Long(corner_2[1]),
...
void BW_Render::Render_Preprocessed_Triangle(Vector3& xcf, Vector3i& yci) {
  ...
  if (ycnt) {
    ...
    while (ycnt > 0) {
      pixel_buffer.Set_H_Line(WWMath::Float_To_Long(x_left),
                              WWMath::Float_To_Long(x_right), y);
