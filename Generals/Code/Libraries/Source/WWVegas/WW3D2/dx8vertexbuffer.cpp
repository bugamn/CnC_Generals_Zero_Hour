...
VertexBufferClass::VertexBufferClass(unsigned type_, unsigned FVF,
                                     unsigned short vertex_count_)
    : VertexCount(vertex_count_), type(type_), engine_refs(0) {
  ...
  WWDEBUG_SAY(("New VB, %d vertices, size %d bytes\n", VertexCount,
               VertexCount * fvf_info->Get_FVF_Size()));
  WWDEBUG_SAY(("Total VB count: %d, total %d vertices, total size %d bytes\n",
               _VertexBufferCount, _VertexBufferTotalVertices,
...
VertexBufferClass::WriteLockClass::WriteLockClass(
  ...
  switch (VertexBuffer->Type()) {
...
#ifdef VERTEX_BUFFER_LOG
    {
      ...
      WWDEBUG_SAY(
          ("VertexBuffer->Lock(start_index: 0, index_range: 0(%d), fvf_size: "
           "%d, fvf: %s)\n",
           VertexBuffer->Get_Vertex_Count(),
           VertexBuffer->FVF_Info().Get_FVF_Size(), fvf_name));
    }
#endif
      DX8_Assert();
      DX8_ErrorCode(static_cast<DX8VertexBufferClass*>(VertexBuffer)
                        ->Get_DX8_Vertex_Buffer()
                        ->Lock(0, 0, (unsigned char**)&Vertices,
                               0));  // Default (no) flags
      break;
    case BUFFER_TYPE_SORTING:
      Vertices =
          static_cast<SortingVertexBufferClass*>(VertexBuffer)->VertexBuffer;
      break;
    default:
      WWASSERT(0);
      break;
...
VertexBufferClass::AppendLockClass::AppendLockClass(
    VertexBufferClass* VertexBuffer, unsigned start_index, unsigned index_range)
    : VertexBufferLockClass(VertexBuffer) {
  DX8_THREAD_ASSERT();
  WWASSERT(VertexBuffer);
  WWASSERT(!VertexBuffer->Engine_Refs());
  ...
  switch (VertexBuffer->Type()) {
...
#ifdef VERTEX_BUFFER_LOG
    {
      ...
      VertexBuffer->FVF_Info().Get_FVF_Name(fvf_name);
      WWDEBUG_SAY(
          ("VertexBuffer->Lock(start_index: %d, index_range: %d, fvf_size: %d, "
           "fvf: %s)\n",
           start_index, index_range, VertexBuffer->FVF_Info().Get_FVF_Size(),
           fvf_name));
    }
#endif
      DX8_Assert();
      DX8_ErrorCode(
          static_cast<DX8VertexBufferClass*>(VertexBuffer)
              ->Get_DX8_Vertex_Buffer()
              ->Lock(start_index * VertexBuffer->FVF_Info().Get_FVF_Size(),
...
DX8VertexBufferClass::DX8VertexBufferClass(const Vector3* vertices,
    ...
    : VertexBufferClass(BUFFER_TYPE_DX8,
                        D3DFVF_XYZ | D3DFVF_TEX1 | D3DFVF_NORMAL, VertexCount),
      VertexBuffer(NULL) {
  WWASSERT(vertices);
  WWASSERT(normals);
  WWASSERT(tex_coords);
...
DX8VertexBufferClass::DX8VertexBufferClass(
    const Vector3* vertices, const Vector3* normals, const Vector4* diffuse,
    const Vector2* tex_coords, unsigned short VertexCount, UsageType usage)
    : VertexBufferClass(
          BUFFER_TYPE_DX8,
...
DX8VertexBufferClass::DX8VertexBufferClass(const Vector3* vertices,
                                           ...
                                           UsageType usage)
    : VertexBufferClass(BUFFER_TYPE_DX8,
                        D3DFVF_XYZ | D3DFVF_TEX1 | D3DFVF_DIFFUSE, VertexCount),
      VertexBuffer(NULL) {
  WWASSERT(vertices);
  WWASSERT(tex_coords);
  WWASSERT(diffuse);
...
void DX8VertexBufferClass::Create_Vertex_Buffer(UsageType usage) {
  ...
  WWDEBUG_SAY(
      ("CreateVertexBuffer(fvfsize=%d, vertex_count=%d, "
       "D3DUSAGE_WRITEONLY|%s|%s, fvf: %s, %s)\n",
       ...
       fvf_name, dynamic ? "D3DPOOL_DEFAULT" : "D3DPOOL_MANAGED"));
  _DX8VertexBufferCount++;
  WWDEBUG_SAY(("Current vertex buffer count: %d\n", _DX8VertexBufferCount));
#endif

  unsigned usage_flags =
      D3DUSAGE_WRITEONLY | ((usage & USAGE_DYNAMIC) ? D3DUSAGE_DYNAMIC : 0) |
  ...
  HRESULT ret = DX8Wrapper::_Get_D3D_Device8()->CreateVertexBuffer(
      FVF_Info().Get_FVF_Size() * VertexCount, usage_flags,
      FVF_Info().Get_FVF(),
  ...
  // Vertex buffer creation failed.  Must be out of memory. Try releasing all
  // our D3D assets and re-creating them.
...
void DX8VertexBufferClass::Copy(const Vector3* loc, const Vector3* norm,
                                const Vector2* uv, unsigned first_vertex,
                                unsigned count) {
  WWASSERT(loc);
  WWASSERT(norm);
  WWASSERT(uv);
...
void DX8VertexBufferClass::Copy(const Vector3* loc, unsigned first_vertex,
                                unsigned count) {
  WWASSERT(loc);
  WWASSERT(count <= VertexCount);
...
void DX8VertexBufferClass::Copy(const Vector3* loc, const Vector2* uv,
...
void DX8VertexBufferClass::Copy(const Vector3* loc, const Vector3* norm,
                                const Vector2* uv, const Vector4* diffuse,
                                unsigned first_vertex, unsigned count) {
  WWASSERT(loc);
  WWASSERT(norm);
  WWASSERT(uv);
...
void DX8VertexBufferClass::Copy(const Vector3* loc, const Vector2* uv,
                                const Vector4* diffuse, unsigned first_vertex,
                                unsigned count) {
  WWASSERT(loc);
  WWASSERT(uv);
  WWASSERT(diffuse);
...
DynamicVBAccessClass::DynamicVBAccessClass(unsigned t, unsigned fvf,
                                           unsigned short vertex_count_)
    : Type(t),
...
void DynamicVBAccessClass::Allocate_DX8_Dynamic_Buffer() {
  ...
  // If requesting more vertices than dynamic vertex buffer can fit, delete the
  // vb and adjust the size to the new count.
  if (VertexCount > _DynamicDX8VertexBufferSize) {
    REF_PTR_RELEASE(_DynamicDX8VertexBuffer);
    _DynamicDX8VertexBufferSize = VertexCount;
    if (_DynamicDX8VertexBufferSize < DEFAULT_VB_SIZE)
      _DynamicDX8VertexBufferSize = DEFAULT_VB_SIZE;
  }
  ...
  if (!_DynamicDX8VertexBuffer) {
    ...
    _DynamicDX8VertexBuffer = NEW_REF(
        DX8VertexBufferClass, (dynamic_fvf_type, _DynamicDX8VertexBufferSize,
                               (DX8VertexBufferClass::UsageType)usage));
    _DynamicDX8VertexBufferOffset = 0;
  }

  // Any room at the end of the buffer?
  if (((unsigned)VertexCount + _DynamicDX8VertexBufferOffset) >
...
DynamicVBAccessClass::WriteLockClass::WriteLockClass(
  ...
  switch (DynamicVBAccess->Get_Type()) {
...
#ifdef VERTEX_BUFFER_LOG
    {
      ...
      WWDEBUG_SAY(
          ("DynamicVertexBuffer->Lock(start_index: %d, index_range: %d, "
           "fvf_size: %d, fvf: %s)\n",
           DynamicVBAccess->VertexBufferOffset,
           DynamicVBAccess->Get_Vertex_Count(),
           DynamicVBAccess->VertexBuffer->FVF_Info().Get_FVF_Size(), fvf_name));
    }
#endif
    ...
    case BUFFER_TYPE_DYNAMIC_SORTING:
      Vertices =
          static_cast<SortingVertexBufferClass*>(DynamicVBAccess->VertexBuffer)
              ->VertexBuffer;
