 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * RenderObjClass::RenderObjClass -- constructor *
 *   RenderObjClass::RenderObjClass -- copy constructor *
 *   RenderObjClass::operator == -- assignment operator *
 *   RenderObjClass::Calculate_Texture_Reduction_Factor -- calculate texture
 *reduction factor  * RenderObjClass::Set_Texture_Reduction_Factor -- set
 *texture reduction factor.             * RenderObjClass::Get_Screen_Size -- get
 *normalized area of object.                         * RenderObjClass::Get_Scene
 *-- returns the (add_ref'd) scene pointer                        *
 ...
 ** RenderObjClass::Validate_Transform -- If the transform is dirty, this causes
 *it to be re- * RenderObjClass::Get_Sub_Object -- returns pointer to first
 *sub-obj with given name        * RenderObjClass::Add_Sub_Object_To_Bone -- add
 *an object to a named bone                   * RenderObjClass::Prepare_LOD --
 *prepare object for predictive and texture LOD processing.  *
 *   RenderObjClass::Get_Cost -- get object rendering cost for predictive LOD
 *processing.      * RenderObjClass::Update_Sub_Object_Bits -- updates our bits
 *according to our sub-objects   * RenderObjClass::Update_Sub_Object_Transforms
 *-- re-evaluate the transforms my sub-objects * RenderObjClass::Add -- Generic
 *add for render objects                                     *
 ...
 *   RenderObjClass::Notify_Removed -- notifies an object that it has been
 *removed             * RenderObjClass::Update_Cached_Bounding_Volumes --
 ...
 *   RenderObjClass::Intersect_Sphere -- tests for intersection with the
 *bounding sphere       * RenderObjClass::Intersect_Sphere_Quick -- tests for
 *intersection with the bounding sphere * RenderObjClass::Build_Dependency_List
 *-- Generates a list of files this obj depends on.   *
 *   RenderObjClass::Build_Texture_List -- Builds a list of texture files this
 *obj depends on. * RenderObjClass::Add_Dependencies_To_List -- Add dependent
 *files to the list.              *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
static inline bool Check_Is_Transform_Identity(const Matrix3D &m) {
  ...
  unsigned d = ((unsigned &)m[0][0] ^ (unsigned &)one) |
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 11/5/97    GTH : Created. * 1/28/98    EHC : Created. *
 *=============================================================================================*/
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 11/5/97    GTH : Created. * 2/25/99    GTH : Created. *
 *=============================================================================================*/
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 04/08/99    NH : Created. *
 *=============================================================================================*/
/*
float RenderObjClass::Calculate_Texture_Reduction_Factor(float norm_screensize)
{
        // NOTE: The texture reduction factor represents the number of powers of
two that the texture
        // must be reduced by in both dimensions. The reason that such an
inherently integral quantity
        // is represented as a float is for the texture reduction algorithms to
incorporate hysteresis
        // properly.
        float reduction = sqrt(Get_Native_Screen_Size() / norm_screensize);
        reduction = MAX(1.0f, reduction);

        // We want to calculate the log base 2. Since the standard libraries
have no log-base-2
        // function, we use the following: log-base-2(x) = log(x)/log(2) where
log is the natural
...
void RenderObjClass::Set_Container(RenderObjClass *con) {
  // Either we arent currently in a container or we are clearing our container,
  // otherwise Houston, there is a problem!
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 2/25/99    GTH : Created. * 07/14/2001 SKB : Add
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 6/15/99    GTH : Created. *
 *=============================================================================================*/
void RenderObjClass::Validate_Transform(void) const {
  /*
  ** Recurse up the tree to see if any of my parents are saying that their
  *sub-object
...
/***********************************************************************************************
 * RenderObjClass::Get_Sub_Object -- returns pointer to first sub-obj with given
 *name          *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS:	If the bone name is unknown then this function will add the the
 *object to the   * root transform rather than failing.  This is due to the fact
 *that GetBoneIndex  * returns the root tranform for unknown bones.
 **
 *                                                                                             *
 * HISTORY: * 3/4/99     GTH : Created. *
 *=============================================================================================*/
...
/***********************************************************************************************
 * RenderObjClass::Remove_Sub_Objects_From_Bone -- remove all objects from a
 *named bone        *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * RenderObjClass::Prepare_LOD -- prepare object for predictive and texture LOD
 *processing.    *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
float RenderObjClass::Get_Cost(void) const {
  ...
  // If polycount is zero set Cost to a small nonzero amount to avoid divisions
  // by zero.
...
/***********************************************************************************************
 * RenderObjClass::Calculate_Cost_Value_Arrays -- calc arrays for predictive LOD
 *processing.   *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNING:	This member function assumes that the arrays passed are large
 *enough (# LODs for   * the cost array and # LODs + 1 for the value array) to
 *contain the desired data.    *
 *                                                                                             *
 * NOTE:	This member function is usually only called internally inside
 *LOD objects for         * initialization purposes, or externally by
 *specialized LOD objects like the G          * predictive LOD PIP proxy. *
 *                                                                                             *
...
/***********************************************************************************************
 * RenderObjClass::Update_Sub_Object_Bits -- updates our bits according to our
 *sub-objects     *
 *                                                                                             *
 * This should be called by any object that contains other objects whenever a
 *sub object is    * added or removed.  It updates the status of the attribute
 *bits which are supposed to be     * the union of all of the sub-objects
 *attributes.  (I.e. if one of our sub-objects is         * translucent, then we
 *should be marked as translucent).                                      *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
...
/***********************************************************************************************
 * RenderObjClass::Update_Sub_Object_Transforms -- re-evaluate the transforms my
 *sub-objects   *
 *                                                                                             *
 * The default implementation is empty, derived classes which have sub-objects
 *should          * implement it to update the transforms of their sub-objects *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 11/04/1997 GH  : Created. * 2/25/99    GTH : Moved to the base
 *RenderObjClass                                         *
 *=============================================================================================*/
...
void RenderObjClass::Remove(void) {
  // All render objects have their scene pointers set.  To check if this is a
  // "top level" object, (i.e. directly in the scene) you see if its Container
  // pointer is NULL.
#if 1
  if (Container == NULL) {
    if (Scene != NULL) {
 ...
 * RenderObjClass::Notify_Added -- notifies the object that it is in a scene *
 *                                                                                             *
 * This function will be called whenever an object is directly or indirectly
 *added to a scene  * An example of "indirect" addition would be if you were
 *added as a sub-object to an HModel   * that was already in a scene. *
 *                                                                                             *
 * Override this function if you want to register your object for
 *per-frame-updating or        * as a VertexProcessor.  (See the Register method
 *of SceneClass, ParticleBufferClass, etc)    *
 *                                                                                             *
 ...
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * Due to implementation details of some derived scenes, the
 *SceneClass calls this function.   * Please dont move the call to this to
 *RenderObjClass::Add                                    * Derived classes
 *should also call the base class to ensure that the scene pointer is set     *
 *                                                                                             *
 * HISTORY: * 2/25/99    GTH : Created. *
 *=============================================================================================*/
 ...
 **
 *                                                                                             *
 * Works similar to the Notify_Added function.  You can override and Unregister
 *yourself from  * any scene based special processing.  Container objects must
 *recurse to their sub objects.   *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * Derived classes should also call the base class to ensure that
 *the scene pointer is cleared *
 *                                                                                             *
 * HISTORY: * 2/25/99    GTH : Created. *
 *=============================================================================================*/
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 28/8/97    NH : Created. * 2/25/99    GTH : Moved into
 *RenderObjClass                                                *
 *=============================================================================================*/
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT: A properly configured Intersection object specifying ray direction &
 *vector          *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * HISTORY: * 2/25/99    GTH : Moved into RenderObjClass *
 *=============================================================================================*/
bool RenderObjClass::Intersect(IntersectionClass *Intersection,
                               IntersectionResultClass *Final_Result) {
  // do the quick sphere test just to make sure it is worth the more expensive
  // intersection test
  if (Intersect_Sphere_Quick(Intersection, Final_Result)) {
    CastResultStruct castresult;
    LineSegClass lineseg;

    Vector3 end = *Intersection->RayLocation +
                  *Intersection->RayDirection * Intersection->MaxDistance;
    ...
    if (Cast_Ray(ray)) {
      lineseg.Compute_Point(ray.Result->Fraction,
                            &(Final_Result->Intersection));
...
/***********************************************************************************************
 * RenderObjClass::Intersect_Sphere -- tests for intersection with the bounding
 *sphere         *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * RenderObjClass::Intersect_Sphere_Quick -- tests for intersection with the
 *bounding sphere   *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 2/25/99    GTH : Created. *
 *=============================================================================================*/
bool RenderObjClass::Intersect_Sphere_Quick(
    IntersectionClass *Intersection, IntersectionResultClass *Final_Result) {
  SphereClass sphere = Get_Bounding_Sphere();
  return Intersection->Intersect_Sphere_Quick(sphere, Final_Result);
}

/***********************************************************************************************
 * RenderObjClass::Build_Dependency_List -- Generates a list of files this obj
 *depends on.     *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 *=============================================================================================*/
bool RenderObjClass::Build_Dependency_List(
...
/***********************************************************************************************
 * RenderObjClass::Build_Texture_List -- Builds a list of texture files this obj
 *depends on.   *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
bool RenderObjClass::Build_Texture_List(
    DynamicVectorClass<StringClass> &texture_file_list, bool recursive) {
  if (recursive) {
    //
    // Loop through all this object's subobj's
    //
...
/****************************************************************************************
        ...
        NOTE: For now, the render obj PersistFactory is going to cheat by simply
storing the name of the render object that was saved.  At load time, it will ask
the asset manager for that object again.  If the asset manager fails to
...
PersistClass *RenderObjPersistFactoryClass::Load(ChunkLoadClass &cload) const {
      ...
      case RENDOBJFACTORY_CHUNKID_VARIABLES:
        ...
        while (cload.Open_Micro_Chunk()) {
          switch (cload.Cur_Micro_Chunk_ID()) {
            READ_MICRO_CHUNK(cload, RENDOBJFACTORY_VARIABLE_OBJPOINTER,
                             old_obj);
            READ_MICRO_CHUNK(cload, RENDOBJFACTORY_VARIABLE_TRANSFORM, tm);
            READ_MICRO_CHUNK_STRING(cload, RENDOBJFACTORY_VARIABLE_NAME, name,
                                    sizeof(name));
          }
  ...
  if (strlen(name) == 0) {
    ...
    if (++count < 10) {
      WWDEBUG_SAY(
          ("RenderObjPersistFactory attempted to load an un-named render "
           "object!\r\n"));
  ...
  RenderObjClass *new_obj =
      WW3DAssetManager::Get_Instance()->Create_Render_Obj(name);
  ...
  if (new_obj == NULL) {
    ...
    if (++count < 10) {
      ...
      WWDEBUG_SAY(
          ("Either the asset for this object is gone or you tried to save a "
           "procedural object.\r\n"));
      WWDEBUG_SAY(("Replacing it with a NULL render object!\r\n"));
    }
    strcpy(name, "NULL");
...
void RenderObjPersistFactoryClass::Save(ChunkSaveClass &csave,
...
bool RenderObjClass::Save(ChunkSaveClass &csave) {
  ...
  // Yes this looks like a design flaw but the way we're saving render objects
  // is a "shortcut".  We specifically designed this capability into the
  // persistant object system so that we could avoid making all render object's
  // save and load themselves if possible.
  WWASSERT(0);
  return true;
}
