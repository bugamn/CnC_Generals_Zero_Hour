...
bool DX8TextureCategoryClass::m_gForceMultiply =
    false;  // Forces opaque materials to use the multiply blend - pseudo
            // transparent effect.  jba.
// ----------------------------------------------------------------------------
...
// void Whatever(const Vector3* locations,unsigned vertex_count,const Vector3i*
// polygon_indices,unsigned polygon_count);
void Whatever(Vector3i* added_polygon_indices, unsigned& added_polygon_count,
              const Vector3* locations, unsigned vertex_count,
              const Vector3i* polygon_indices, unsigned polygon_count);
...
#define VERTEX_BUFFER_OVERFLOW \
  0xffff  //'Generals' flag to signal when a mesh didn't fit in streaming vertex
          //buffer.
...
class PolyRenderTaskClass : public AutoPoolClass<PolyRenderTaskClass, 256> {
 public:
  PolyRenderTaskClass(DX8PolygonRendererClass* p_renderer, MeshClass* p_mesh)
      : Renderer(p_renderer), Mesh(p_mesh), NextVisible(NULL) {
    WWASSERT(Renderer != NULL);
    WWASSERT(Mesh != NULL);
    Mesh->Add_Ref();
  }

  ~PolyRenderTaskClass(void) { Mesh->Release_Ref(); }
...
class MatPassTaskClass : public AutoPoolClass<MatPassTaskClass, 256> {
 public:
  MatPassTaskClass(MaterialPassClass* pass, MeshClass* mesh)
      : MaterialPass(pass), Mesh(mesh), NextVisible(NULL) {
...
DX8TextureCategoryClass::DX8TextureCategoryClass(
    DX8FVFCategoryContainer* container_, TextureClass** texs, ShaderClass shd,
    VertexMaterialClass* mat, int pass_)
    : pass(pass_),
      shader(shd),
...
DX8TextureCategoryClass::~DX8TextureCategoryClass() {
  // Unregistering the mesh where polygon renderers are connected to kills all
  // polygon renderers
...
void DX8TextureCategoryClass::Add_Render_Task(
    DX8PolygonRendererClass* p_renderer, MeshClass* p_mesh) {
...
void DX8TextureCategoryClass::Add_Polygon_Renderer(
    ...
    DX8PolygonRendererClass* add_after_this) {
...
void DX8TextureCategoryClass::Remove_Polygon_Renderer(
    DX8PolygonRendererClass* p_renderer) {
...
void DX8FVFCategoryContainer::Remove_Texture_Category(
    DX8TextureCategoryClass* tex_category) {
  for (unsigned pass = 0; pass < passes; ++pass) {
    texture_category_list[pass].Remove(tex_category);
  }
  for (pass = 0; pass < passes; pass++) {
    // If any of the texture category lists has anything in it, no need to
...
void DX8FVFCategoryContainer::Render_Procedural_Material_Passes(void) {
  ...
  while (mpr != NULL) {
    ...
    if (mesh->Get_Base_Vertex_Offset() ==
        VERTEX_BUFFER_OVERFLOW)  // check if this mesh is valid
    ...
    mpr->Peek_Mesh()->Render_Material_Pass(mpr->Peek_Material_Pass(),
                                           index_buffer);
...
void DX8TextureCategoryClass::Log(bool only_visible) {
  ...
  for (int stage = 0; stage < MAX_TEXTURE_STAGES; ++stage) {
    work2.Format("	texture[%d]: %x (%s)\n", stage, textures[stage],
                 textures[stage] ? textures[stage]->Get_Name() : "-");
    work += work2;
  }
  work2.Format("	material: %x (%s)\n	shader: %x\n", material,
               material ? material->Get_Name() : "-", shader);
  work += work2;
  WWDEBUG_SAY((work));

  work.Format("	%8s %8s %6s %6s %6s %5s %s\n", "idx_cnt", "poly_cnt", "i_offs",
              "min_vi", "vi_rng", "ident", "name");
  WWDEBUG_SAY((work));
...
DX8FVFCategoryContainer::~DX8FVFCategoryContainer() {
  ...
  for (unsigned p = 0; p < passes; ++p) {
    while (DX8TextureCategoryClass* tex =
               texture_category_list[p].Remove_Head()) {
      delete tex;
    }
  }
}
...
DX8FVFCategoryContainer::Find_Matching_Texture_Category(
  ...
  // Find texture category which matches ref_category's properties but has
  // 'texture' on given pass and stage.
  ...
  while (!dest_it.Is_Done()) {
    if (dest_it.Peek_Obj()->Peek_Texture(stage) == texture) {
      ...
      for (unsigned int s = 0; s < MeshMatDescClass::MAX_TEX_STAGES; s++) {
        if (stage != s) {
          all_textures_same =
              all_textures_same && (dest_tex_category->Peek_Texture(s) ==
                                    ref_category->Peek_Texture(s));
        }
      }
      if (all_textures_same &&
          Equal_Material(dest_tex_category->Peek_Material(),
                         ref_category->Peek_Material()) &&
...
DX8TextureCategoryClass*
DX8FVFCategoryContainer::Find_Matching_Texture_Category(
    VertexMaterialClass* vmat, unsigned pass,
    DX8TextureCategoryClass* ref_category) {
  // Find texture category which matches ref_category's properties but has
  // 'vmat' on given pass
  DX8TextureCategoryClass* dest_tex_category = NULL;
...
void DX8FVFCategoryContainer::Change_Polygon_Renderer_Texture(
    DX8PolygonRendererList& polygon_renderer_list, TextureClass* texture,
  ...
  // Find source texture category, then find all polygon renderers who belong to
  // that category and move them to destination category.
  ...
  while (!src_it.Is_Done()) {
    ...
    if (src_tex_category->Peek_Texture(stage) == texture) {
      ...
      while (!poly_it.Is_Done()) {
        // If source texture category contains polygon renderer, move to
        ...
        if (prc == src_tex_category) {
          DX8TextureCategoryClass* dest_tex_category =
              Find_Matching_Texture_Category(new_texture, pass, stage,
                                             src_tex_category);
          ...
          if (!dest_tex_category) {
            ...
            DX8TextureCategoryClass* new_tex_category =
                W3DNEW DX8TextureCategoryClass(
                    this, tmp_textures, src_tex_category->Get_Shader(),
                    const_cast<VertexMaterialClass*>(
                        src_tex_category->Peek_Material()),
                    pass);

            /*
            ** Add the texture category object into the list, immediately after
            *any existing
            ** texture category object which uses the same texture.  This will
            *result in
            ** the list always having matching texture categories next to each
            *other.
            */
            ...
            while (!tex_it.Is_Done()) {
              ...
              if (tex_it.Peek_Obj()->Peek_Texture(0) == tmp_textures[0]) {
                texture_category_list[pass].Add_After(new_tex_category,
                                                      tex_it.Peek_Obj());
...
void DX8FVFCategoryContainer::Change_Polygon_Renderer_Material(
    DX8PolygonRendererList& polygon_renderer_list, VertexMaterialClass* vmat,
    VertexMaterialClass* new_vmat, unsigned pass) {
  WWASSERT(pass < passes);
    ...
    if (src_tex_category->Peek_Material() == vmat) {
      ...
      while (!poly_it.Is_Done()) {
        ...
        if (prc == src_tex_category) {
          foundtexture = true;
          DX8TextureCategoryClass* dest_tex_category =
              Find_Matching_Texture_Category(new_vmat, pass, src_tex_category);
          ...
          if (!dest_tex_category) {
            ...
            DX8TextureCategoryClass* new_tex_category =
                W3DNEW DX8TextureCategoryClass(
                    this, tmp_textures, src_tex_category->Get_Shader(),
                    const_cast<VertexMaterialClass*>(new_vmat), pass);
...
unsigned DX8FVFCategoryContainer::Define_FVF(MeshModelClass* mmc,
  ...
  if ((!!mmc->Get_Flag(MeshGeometryClass::SORT)) &&
  ...
  if (!mmc->Needs_Vertex_Normals()) {  // enable_lighting ||
                                       // mmc->Get_Flag(MeshModelClass::PRELIT_MASK))
                                       // {
    return fvf;
  }
...
void DX8RigidFVFCategoryContainer::Log(bool only_visible) {
  ...
  if (vertex_buffer) {
    ...
    work.Format("VB size (used/total): %d/%d FVF: %s\n", used_vertices,
                vertex_buffer->Get_Vertex_Count(), fvfname);
    WWDEBUG_SAY((work));
  } else {
...
void DX8RigidFVFCategoryContainer::Render(void) {
  ...
  for (unsigned p = 0; p < passes; ++p) {
    SNAPSHOT_SAY(("Pass: %d\n", p));
    while (DX8TextureCategoryClass* tex =
               visible_texture_category_list[p].Remove_Head()) {
      tex->Render();
    }
  }
...
bool DX8RigidFVFCategoryContainer::Check_If_Mesh_Fits(MeshModelClass* mmc) {
  if (!vertex_buffer)
    return true;  // No VB created - mesh will fit as a new vb will be created
                  // when inserting
  unsigned required_vertices = mmc->Get_Vertex_Count();
  unsigned available_vertices =
      vertex_buffer->Get_Vertex_Count() - used_vertices;
  ...
  if (required_vertices <= available_vertices &&
...
class Vertex_Split_Table {
 ...
 public:
  Vertex_Split_Table(MeshModelClass* mmc_)
      : mmc(mmc_), npatch_enable(false), allocated_polygon_array(false) {
    ...
    memcpy(polygon_array, mmc->Get_Polygon_Array(),
           mmc->Get_Polygon_Count() * sizeof(Vector3i));
    if (gap_filler) {
      memcpy(polygon_array + mmc->Get_Polygon_Count(),
  ...
  const Vector3* Get_Vertex_Array() const { return mmc->Get_Vertex_Array(); }
  ...
  unsigned Get_Vertex_Count() const { return mmc->Get_Vertex_Count(); }
  ...
  TextureClass* Peek_Texture(unsigned idx, unsigned pass, unsigned stage) {
    if (mmc->Has_Texture_Array(pass, stage)) {
      if (idx >= unsigned(mmc->Get_Polygon_Count())) {
        ...
        return mmc->Get_Gap_Filler()->Get_Texture_Array(
            pass, stage)[idx - mmc->Get_Polygon_Count()];
      }
  ...
  VertexMaterialClass* Peek_Material(unsigned idx, unsigned pass) {
    if (mmc->Has_Material_Array(pass)) {
      if (idx >= unsigned(mmc->Get_Polygon_Count())) {
        WWASSERT(mmc->Get_Gap_Filler());
        return mmc->Get_Gap_Filler()->Get_Material_Array(
            pass)[idx - mmc->Get_Polygon_Count()];
      }
  ...
  ShaderClass Peek_Shader(unsigned idx, unsigned pass) {
    if (mmc->Has_Shader_Array(pass)) {
      ...
      if (idx >= unsigned(mmc->Get_Polygon_Count())) {
        WWASSERT(mmc->Get_Gap_Filler());
        shader = mmc->Get_Gap_Filler()->Get_Shader_Array(
            pass)[idx - mmc->Get_Polygon_Count()];
      } else
...
void DX8RigidFVFCategoryContainer::Add_Mesh(MeshModelClass* mmc_) {
  ...
  /*
  ** This FVFCategoryContainer doesn't have a vertex buffer yet so allocate one
  *big
  ...
  */
  if (!vertex_buffer) {
    ...
    if (sorting) {
      ...
      WWASSERT(vertex_buffer->FVF_Info().Get_FVF() ==
               FVF);  // Only one sorting FVF type!
    } else {
      vertex_buffer = NEW_REF(
          DX8VertexBufferClass,
          (FVF, vb_size,
           (DX8Caps::Support_NPatches() && WW3D::Get_NPatches_Level() > 1)
  ...
  */
  ...
  VertexBufferClass::AppendLockClass l(vertex_buffer, used_vertices,
                                       split_table.Get_Vertex_Count());
...
void DX8FVFCategoryContainer::Insert_To_Texture_Category(
    Vertex_Split_Table& split_table, TextureClass** texs,
    VertexMaterialClass* mat, ShaderClass shader, int pass,
    unsigned vertex_offset) {
  /*
  ** Try to find a DX8TextureCategoryClass in this FVF container which matches
  *the
  ...
  */
  ...
  while (!it.Is_Done()) {
    ...
    // Compare all stage's textures
    bool all_textures_same = true;
    for (unsigned int stage = 0; stage < MeshMatDescClass::MAX_TEX_STAGES;
         stage++) {
      all_textures_same = all_textures_same &&
                          (tex_category->Peek_Texture(stage) == texs[stage]);
    }
    if (all_textures_same &&
        Equal_Material(tex_category->Peek_Material(), mat) &&
        tex_category->Get_Shader() == shader) {
      used_indices += tex_category->Add_Mesh(split_table, vertex_offset,
                                             used_indices, index_buffer, pass);
  ...
  if (!fit_in_existing_category) {
    DX8TextureCategoryClass* new_tex_category =
        W3DNEW DX8TextureCategoryClass(this, texs, shader, mat, pass);
    used_indices += new_tex_category->Add_Mesh(
        split_table, vertex_offset, used_indices, index_buffer, pass);

    /*
    ** Add the texture category object into the list, immediately after any
    *existing
...
struct Textures_Material_And_Shader_Booking_Struct {
  TextureClass* added_textures[MeshMatDescClass::MAX_TEX_STAGES]
  ...
  bool Add_Textures_Material_And_Shader(TextureClass** texs,
    ...
    for (unsigned a = 0; a < added_type_count; ++a) {
      ...
      for (unsigned int stage = 0; stage < MeshMatDescClass::MAX_TEX_STAGES;
           stage++) {
        all_textures_same =
            all_textures_same && (texs[stage] == added_textures[stage][a]);
      }
      if (all_textures_same && Equal_Material(mat, added_materials[a]) &&
          shd == added_shaders[a]) {
        return false;
      }
    }
...
void DX8FVFCategoryContainer::Generate_Texture_Categories(
    Vertex_Split_Table& split_table, unsigned vertex_offset) {
  int polygon_count = split_table.Get_Polygon_Count();
  int index_count = polygon_count * 3 * split_table.Get_Pass_Count();

  /*
  ** If we don't have an index buffer yet, allocate one.  Make it hold at least
  *12000 entries,
  ...
  */
  ...
  for (unsigned pass = 0; pass < split_table.Get_Pass_Count(); ++pass) {
    Textures_Material_And_Shader_Booking_Struct
        textures_material_and_shader_booking;
    ...
    for (int i = 0; i < polygon_count; ++i) {
      ...
      if (!textures_material_and_shader_booking
               .Add_Textures_Material_And_Shader(textures, mat, shader))
        continue;

      Insert_To_Texture_Category(split_table, textures, mat, shader, pass,
                                 vertex_offset);
    }
...
void DX8SkinFVFCategoryContainer::Render(void) {
  ...
  DX8Wrapper::Set_Vertex_Buffer(
      NULL);  // Free up the reference to the current vertex buffer
  ...
  if (maxVertexCount > 65535) {  // clamp vertex count to maximum size that can
                                 // be indexed by 16-bit index
    maxVertexCount = 65535;
  }

  DynamicVBAccessClass vb(
      sorting ? BUFFER_TYPE_DYNAMIC_SORTING : BUFFER_TYPE_DYNAMIC_DX8,
      dynamic_fvf_type, maxVertexCount);
  ...
  while (renderedVertexCount < VisibleVertexCount) {
    mesh = remainingMesh;
    {
      ...
      while (mesh != NULL) {
        ...
        if (vertex_offset + mesh_vertex_count > maxVertexCount ||
            remainingMesh) {  // flag mesh so we know it didn't fit in the
                              // vertex buffer
        ...
        if (_TempVertexBuffer.Length() < mesh_vertex_count)
          _TempVertexBuffer.Resize(mesh_vertex_count);
        if (_TempNormalBuffer.Length() < mesh_vertex_count)
          _TempNormalBuffer.Resize(mesh_vertex_count);
    ...
    // Flush the meshes which fit in the vertex buffer, applying all texture
    // variations
    for (unsigned pass = 0; pass < passes; ++pass) {
      SNAPSHOT_SAY(("Pass: %d\n", pass));
  ...
  for (unsigned pass = 0; pass < passes; ++pass) {
    while (DX8TextureCategoryClass* tex =
               visible_texture_category_list[pass].Remove_Head()) {
    }
  }
...
bool DX8SkinFVFCategoryContainer::Check_If_Mesh_Fits(MeshModelClass* mmc) {
  if (!index_buffer)
    return true;  // No IB created - mesh will fit as a new ib will be created
                  // when inserting
  ...
  if ((required_polygons * 3 * mmc->Get_Pass_Count()) <=
      index_buffer->Get_Index_Count() - used_indices) {
    return true;
  }
  return false;
}
...
void DX8SkinFVFCategoryContainer::Add_Visible_Skin(MeshClass* mesh) {
  if (mesh->Peek_Next_Visible_Skin() != NULL || mesh == VisibleSkinTail) {
    DEBUG_CRASH(
        ("Mesh %s is already a visible skin, and we tried to add it again... "
         "please notify Mark W or Steven J immediately!\n",
         mesh->Get_Name()));
    return;
  }
  if (VisibleSkinHead == NULL) VisibleSkinTail = mesh;
...
unsigned DX8TextureCategoryClass::Add_Mesh(Vertex_Split_Table& split_table,
                                           unsigned vertex_offset,
  ...
  /*
  ** Count the polygons in the given mesh in the given pass which match this
  *texture category
  */
  unsigned polygons = 0;
  ...
  for (int i = 0; i < poly_count; ++i) {
    ...
    for (unsigned int stage = 0; stage < MeshMatDescClass::MAX_TEX_STAGES;
      ...
      all_textures_same =
          all_textures_same &&
          (split_table.Peek_Texture(i, pass, stage) == textures[stage]);
    }
    VertexMaterialClass* mat = split_table.Peek_Material(i, pass);
  ...
  /*
  ** Add the indices for the polygons that match into this renderer's dx8 index
  *table.
  */
  if (polygons) {
    ...
    if (index_buffer->Type() == BUFFER_TYPE_SORTING ||
        index_buffer->Type() == BUFFER_TYPE_DYNAMIC_SORTING) {
      stripify = false;
    }
#endif;
    const Vector3i* src_indices =
        (const Vector3i*)split_table.Get_Polygon_Array(
            pass);  // mmc->Get_Polygon_Array();
    ...
    if (stripify) {
      ...
      for (int i = 0; i < poly_count; ++i) {
        ...
        if (all_textures_same && Equal_Material(mat, material) &&
            shd == shader) {
      ...
      int* strips =
          StripOptimizerClass::Stripify(triangles, triangle_index_count / 3);
      delete[] triangles;
      ...
      if (index_count < unsigned(strip[0])) {
        ...
        DX8PolygonRendererClass* p_renderer = W3DNEW DX8PolygonRendererClass(
            index_count, split_table.Get_Mesh_Model_Class(), this,
            vertex_offset, index_offset, true, pass);
        PolygonRendererList.Add_Tail(p_renderer);

        {
          IndexBufferClass::AppendLockClass l(index_buffer, index_offset,
                                              index_count);
          ...
          /*
          ** Iterate over the polys for this pass, adding each one that matches
          *this texture+material+shader
          */
          ...
          /*
          ** Remember the min and max vertex indices that these polygons used
          *(for optimization)
          */
    ...
    // Need to check stripify again as it may be changed to false by the
    // previous statement
    if (!stripify) {
      DX8PolygonRendererClass* p_renderer = W3DNEW DX8PolygonRendererClass(
          index_count, split_table.Get_Mesh_Model_Class(), this, vertex_offset,
          index_offset, false, pass);
      PolygonRendererList.Add_Tail(p_renderer);
      ...
      /*
      ** Iterate over the polys for this pass, adding each one that matches this
      ...
      */
      ...
      WWASSERT((vmax - vmin) <
               split_table.Get_Mesh_Model_Class()->Get_Vertex_Count());

      /*
      ** Remember the min and max vertex indices that these polygons used (for
      *optimization)
      */
      p_renderer->Set_Vertex_Index_Range(vmin, vmax - vmin + 1);
...
void DX8TextureCategoryClass::Render(void) {
  ...
  if (!WW3D::Expose_Prelit()) {
    ...
    for (unsigned i = 0; i < MAX_TEXTURE_STAGES; ++i) {
      SNAPSHOT_SAY(
          ("Set_Texture(%d,%s)\n", i,
           Peek_Texture(i) ? Peek_Texture(i)->Get_Texture_Name() : "NULL"));
  ...
  VertexMaterialClass* vmaterial = (VertexMaterialClass*)
      Peek_Material();  // ugly cast from const but we'll restore it after
                        // changes so okay. -MW
  ...
  // if we want to allow other translucent polygons behind this mesh, we need to
  // disable z-write but this will cause sorting errors on this mesh.
  ...
  while (prt) {
    ...
    */
...
#ifdef WWDEBUG
    // Debug rendering: if it exists, expose prelighting on this mesh by
    // disabling all base textures.
    if (WW3D::Expose_Prelit()) {
    ...
    /*
    ** If the user is not installing LightEnvironmentClasses, we leave the
    *lighting render
    ** states untouched.  This way they can set a couple global lights that
    ...
    */
    ...
    if (mesh->Peek_Model()->Get_Flag(MeshModelClass::ALIGNED)) {
      ...
      TheDX8MeshRenderer.Peek_Camera()->Get_Transform().Get_Z_Vector(
          &camera_z_vector);
      mesh->Get_Transform().Get_Translation(&mesh_position);

      tmp_world.Obj_Look_At(mesh_position, mesh_position + camera_z_vector,
                            0.0f);
      ...
      TheDX8MeshRenderer.Peek_Camera()->Get_Transform().Get_Translation(
          &camera_position);
      mesh->Get_Transform().Get_Translation(&mesh_position);
    ...
    */
    if ((!!mesh->Peek_Model()->Get_Flag(MeshGeometryClass::SORT)) &&
        WW3D::Is_Sorting_Enabled()) {
      renderer->Render_Sorted(mesh->Get_Base_Vertex_Offset(),
                              mesh->Get_Bounding_Sphere());
    } else {
      // non-transparent mesh that will be rendered immediately.  Okay to adjust
      // the shader/material if necessary
      if (mesh->Get_Alpha_Override() != 1.0 ||
          (mesh->Get_User_Data() &&
           *(int*)mesh->Get_User_Data() ==
               RenderObjClass::
                   USER_DATA_MATERIAL_OVERRIDE)) {  // mesh has material
                                                    // override of some kind
        ...
        LinearOffsetTextureMapperClass* oldMapper =
            (LinearOffsetTextureMapperClass*)vmaterial->Peek_Mapper();
        if (mesh->Get_User_Data() &&
          ...
          RenderObjClass::Material_Override* matOverride =
              (RenderObjClass::Material_Override*)mesh->Get_User_Data();
          oldUVOffsetSyncTime = oldMapper->Get_LastUsedSyncTime();
          oldMapper->Set_LastUsedSyncTime(
              WW3D::Get_Sync_Time());  // make sure zero time passes for the
                                       // mapper.
        ...
        if (mesh->Get_Alpha_Override() != 1.0) {
          if (mesh->Is_Additive()) {  // additvie blended mesh can't switch to
                                      // alpha or we will get a black outline.
            // so adjust diffuse color instead.
            vmaterial->Set_Diffuse(mesh->Get_Alpha_Override(),
          ...
          DX8Wrapper::Set_DX8_Render_State(
              D3DRS_ALPHAREF, (int)((float)0x60 * mesh->Get_Alpha_Override()));
        ...
        DX8Wrapper::Set_Material(
            NULL);  // force a reset of vertex material since we secretly
                    // changed opacity or uv offset
    ...
    /*
    ** Move to the next render task.  Note that the delete should be fast
    *because prt's are pooled
    */
...
void DX8MeshRendererClass::Clear_Pending_Delete_Lists() {
  while (DX8TextureCategoryClass* category =
             texture_category_delete_list.Remove_Head()) {
    delete category;
  }
  while (DX8FVFCategoryContainer* container =
             fvf_category_container_delete_list.Remove_Head()) {
    delete container;
  }
}
...
static void Add_Rigid_Mesh_To_Container(FVFCategoryList* container_list,
                                        unsigned fvf, MeshModelClass* mmc) {
  WWASSERT(container_list);
  DX8FVFCategoryContainer* container = NULL;
  bool sorting =
      ((!!mmc->Get_Flag(MeshModelClass::SORT)) && WW3D::Is_Sorting_Enabled());

  FVFCategoryListIterator it(container_list);
  while (!it.Is_Done()) {
    container = it.Peek_Obj();
    if (sorting == container->Is_Sorting() &&
        container->Check_If_Mesh_Fits(mmc)) {
...
void DX8MeshRendererClass::Register_Mesh_Type(MeshModelClass* mmc) {
  ...
  WWDEBUG_SAY(("Registering mesh: %s (%d polys, %d verts + %d gap polygons)\n",
               mmc->Get_Name(), mmc->Get_Polygon_Count(),
               mmc->Get_Vertex_Count(), mmc->Get_Gap_Filler_Polygon_Count()));
#endif
  ...
  if (skin) {
    ...
    */
    ...
    DX8FVFCategoryContainer* new_container =
        W3DNEW DX8SkinFVFCategoryContainer(sorting);
    ...
    */
    WWASSERT_PRINT(_RegisteredMeshList.Contains(mmc) == false,
                   ("Mesh name: %s", mmc->Get_Name()));

    /*
    ** If the previous step didn't add the mesh, then we have to actually
    *process this mesh
    */
    if (!_RegisteredMeshList.Contains(mmc)) {
      ...
      */
      ...
      if (i == texture_category_container_lists_rigid.Count()) {
        /*
        ** We couldn't find an existing FVF category container so we have to add
        *one.  Future
      ...
      /*
      ** Done processing the mesh, add its polygon renderers to the global
      *registered mesh list
      */
...
void DX8MeshRendererClass::Flush(void) {
  ...
  for (int i = 0; i < texture_category_container_lists_rigid.Count(); ++i) {
    Render_FVF_Category_Container_List(
        *texture_category_container_lists_rigid[i]);
  }
...
static void Log_Container_List(FVFCategoryList& container_list,
                               bool only_visible) {
  FVFCategoryListIterator it(&container_list);
  while (!it.Is_Done()) {
    it.Peek_Obj()->Log(only_visible);
...
void DX8MeshRendererClass::Log_Statistics_String(bool only_visible) {
  ...
  for (int i = 0; i < texture_category_container_lists_rigid.Count(); ++i) {
    Log_Container_List(*texture_category_container_lists_rigid[i],
                       only_visible);
  }
...
void DX8MeshRendererClass::Invalidate(bool shutdown) {
  ...
  if (!shutdown) texture_category_container_list_skin = W3DNEW FVFCategoryList;
