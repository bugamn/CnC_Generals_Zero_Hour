...
/* We have chunking logic which handles N segments at a time. To simplify the
*subdivision logic,
** we will ensure that N is a power of two and that N >=
*2^MAX_STREAK_SUBDIV_LEVELS, so that the
...
StreakRendererClass &StreakRendererClass::operator=(
    const StreakRendererClass &that) {
  if (this != &that) {
...
StreakRendererClass::~StreakRendererClass(void) { REF_PTR_RELEASE(Texture); }
...
void StreakRendererClass::Init(const W3dEmitterLinePropertiesStruct &props) {
  ...
  int texture_mode = ((props.Flags & W3D_ELINE_TEXTURE_MAP_MODE_MASK) >>
                      W3D_ELINE_TEXTURE_MAP_MODE_OFFSET);
  switch (texture_mode) {
...
void StreakRendererClass::Render(RenderInfoClass &rinfo,
                                 ...
                                 unsigned int num_points, Vector3 *points,
...
void StreakRendererClass::subdivision_util(unsigned int point_cnt,
                                           ...
                                           float *subdiv_tex_v) {
  // CAUTION: freezing the random offsets will make it more readily apparent
  // that the offsets are in camera space rather than worldspace.
  ...
  StreakSubdivision
      stack[2 * MAX_STREAK_SUBDIV_LEVELS];  // Maximum number needed
  int tos = 0;

  for (unsigned int pointIndex = 0; pointIndex < point_cnt - 1; pointIndex++) {
    // Subdivide the (pointIndex, pointIndex + 1) segment. Produce pointIndex
    // and all subdivided points up to (not including) pointIndex + 1.
    tos = 0;
    ...
    for (; tos >= 0;) {
      if (stack[tos].Level == SubdivisionLevel) {
        ...
        if (freeze_random) {
          randvec.Set(randomize * oo_int_max, randomize * oo_int_max,
                      randomize * oo_int_max);
        } else {
          randomizer.Get_Vector(randvec);
        }
        stack[tos + 1].StartPos = stack[tos].StartPos;
        stack[tos + 1].EndPos =
            (stack[tos].StartPos + stack[tos].EndPos) * 0.5f +
            randvec * stack[tos].Rand;
        stack[tos + 1].StartTexV = stack[tos].StartTexV;
        stack[tos + 1].EndTexV =
            (stack[tos].StartTexV + stack[tos].EndTexV) * 0.5f;
...
void StreakRendererClass::RenderStreak(
    RenderInfoClass &rinfo, const Matrix3D &transform, unsigned int num_points,
    Vector3 *points,
  ...
  */
  ...
  // We reduce the chunk size to take account of subdivision levels (so that the
  // # of points after subdivision will be no higher than the allowed maximum).
  // We know this will not reduce the chunk size below 2, since the chunk size
  // must be at least two to the power of the maximum allowable number of
  // subdivisions. The plus 1 is because #points = #segments + 1.
  unsigned int chunk_size = (STREAK_CHUNK_SIZE >> SubdivisionLevel) + 1;
  if (chunk_size > num_points) chunk_size = num_points;

  // Chunk through the points (we increment by chunk_size - 1 because the last
  // point of this chunk must be reused as the first point of the next chunk.
  // This is also the reason we stop when chunkIndex = NumPoints - 1: the last
  // point has already been processed in the previous iteration so we don't need
  // another one).
  for (unsigned int chunkIndex = 0; chunkIndex < num_points - 1;
       chunkIndex += (chunk_size - 1)) {
    unsigned int point_cnt = num_points - chunkIndex;
    point_cnt = MIN(point_cnt, chunk_size);

    // We use these different loop indices (which loop INSIDE a chunk) to
    // improve readability:
    ...
    */
    ...
    Matrix3D view2(view[0].X, view[0].Y, view[0].Z, view[0].W, view[1].X,
    ...
    VectorProcessorClass::Transform(&xformed_pts[0], &points[chunkIndex],
                                    modelview, point_cnt);

    /*
    ** Prepare v parameter per point - used for texture mapping (esp. tiled
    *mapping mode)
    */
    ...
    //				for (pointIndex = 0; pointIndex < point_cnt;
    //pointIndex++)
    //				{
    //					// All 0
    //					base_tex_v[pointIndex] = 0.0f;
    ...
    //				{
    //					// Increasing V
    //					base_tex_v[pointIndex] =
    //(float)(pointIndex + chunkIndex) * TextureTileFactor;
    //				}
    //				u_values[0] = 0.0f;
    //				u_values[1] = 0.0f;
    //				break;
    ...
    ** We find the midpoint for each section, apply a random offset, and
    *recurse. We also find
    ** the average V coordinate of the endpoints which is the midpoint V (for
    *tiled texture
    ** mapping).
    */
    ...
    subdivision_util(point_cnt, xformed_pts, base_tex_v, &sub_point_cnt,
                     xformed_subdiv_pts, subdiv_tex_v);
    ...
    */
    ...
    // For each line segment find the two silhouette planes from eyepoint to the
    // line segment cylinder. To simplify we do not find the tangent planes but
    // intersect the cylinder with a plane passing through its axis and
    // perpendicular to the eye vector, find the edges of the resulting
    // rectangle, and create planes through these edges and the eyepoint. Note
    // that these planes are represented as a single normal rather than a normal
    // and a distance; this is because they pass through the origin (eyepoint)
    // so their distance is always zero.

    // Since the line has thickness, each segment has two edges. We name these
    // 'top' and 'bottom' - note however that the top/bottom distinction does
    // not relate to screen up/down and remains consistent throughout the
    // segmented line.
    enum SegmentEdge {
    ...
    // We have dummy segments for "before the first point" and "after the last
    // point" - in these segments the top and bottom edge are the same - they
    // are a perpendicular plane defined by the endpoint vertices. This is so we
    // can merge intersections properly for the first and last points.
    ...
    struct LineSegmentIntersection {
      unsigned int
          PointCount;  // How many points does this intersection represent
    ...
    // The number of intersections is the number of points minus 2. However, we
    // store intersection records for the first and last point, even though they
    // are not really intersections. The reason we do this is for the
    // intersection merging - the vertices for the first and last points can get
    // merged just like any other intersection. Also, we have a dummy
    // intersection record before the first point - this is because we want
    // "previous segments" for the first point and each intersection only has an
    // index for the next segment.
    LineSegmentIntersection intersection[MAX_STREAK_POINT_BUFFER_SIZE + 1]
                                        [NUM_EDGES];

    for (segmentIndex = 1; segmentIndex < point_cnt;
         segmentIndex++) {  // #segments = #points - 1 (+ 2 dummy segments)
      ...
      // We temporarily store the segment direction in the segment's StartPlane
      // (since it is used to calculate the StartPlane later).
      ...
      Vector3 nearest =
          curr_point + segdir * -Vector3::Dot_Product(segdir, curr_point);
      ...
      // Find planes through top/bottom points and eyepoint. In addition to the
      // two points, we know that the planes are parallel to the line segment.
      Vector3 top_normal;
      ...
      // If the visual angle between the previous and current line segments (we
      // use the angle between the planes defined by each line segment and the
      // eyepoint) is less than 90 degrees, switch the top and bottom edges for
      // the current and subsequent segments and mark the intersection as having
      // a fold
      if (segmentIndex > 1) {
        Vector3 prev_plane;
        Vector3::Cross_Product(points[segmentIndex - 2], curr_point,
                               &prev_plane);
    ...
    // The two dummy segments for the clipping edges of the first and last real
    // segments will be defined later, with the first and last intersections.
    ...
    */
    ...
    unsigned int numsegs =
        point_cnt - 1;  // Doesn't include the two dummy segments
    unsigned int num_intersections[NUM_EDGES];

    // These include the 1st, last point "intersections", not the pre-first
    // dummy intersection
    num_intersections[TOP_EDGE] = point_cnt;
    num_intersections[BOTTOM_EDGE] = point_cnt;

    // Initialize pre-first point dummy intersection record (only NextSegmentID
    // will be used).
    intersection[0][TOP_EDGE].PointCount = 0;  // Should never be used
    intersection[0][TOP_EDGE].NextSegmentID =
        0;  // Points to first dummy segment
    ...
    // Find closest point to 1st top/bottom segment edge plane, and convert to
    // direction vector and dummy segment edge plane.
    ...
    segdir.Normalize();  // Is this needed? Probably not - remove later when all
                         // works
    Vector3 start_pl;
    ...
    if (dp > 0.0f) {
      segment[0].StartPlane = segment[0].EdgePlane[TOP_EDGE] =
          segment[0].EdgePlane[BOTTOM_EDGE] = start_pl;
    } else {
      segment[0].StartPlane = segment[0].EdgePlane[TOP_EDGE] =
          segment[0].EdgePlane[BOTTOM_EDGE] = -start_pl;
    }
    ...
    unsigned int last_isec =
        num_intersections[TOP_EDGE];  // Same # top, bottom intersections

    intersection[last_isec][TOP_EDGE].PointCount = 1;
    intersection[last_isec][TOP_EDGE].NextSegmentID =
        numsegs + 1;  // Last dummy segment
    ...
    intersection[last_isec][BOTTOM_EDGE].NextSegmentID =
        numsegs + 1;  // Last dummy segment
    ...
    if (dp > 0.0f) {
      segment[numsegs + 1].StartPlane =
          segment[numsegs + 1].EdgePlane[TOP_EDGE] =
    ...
    // Calculate midpoint segment intersections. There are 2 segment
    // intersections for each point: top and bottom (due to the fact that the
    // segments have width, so they have a top edge and a bottom edge). Note
    // that the top/bottom distinction does not relate to screen up/down. Since
    // each segment edge is represented by a plane passing through the origin
    // (eyepoint), the intersection of two such is a line passing through the
    // origin, which is represented as a normalized direction vector. We use
    // both segment intersections to define the startplane for the segment which
    // begins at that intersection.

    float vdp;

    for (intersectionIndex = 2; intersectionIndex < num_intersections[TOP_EDGE];
         intersectionIndex++) {
      // Relevant midpoint:
      ...
      intersection[intersectionIndex][TOP_EDGE].NextSegmentID =
          intersectionIndex;
      intersection[intersectionIndex][TOP_EDGE].Point = midpoint;

      //			intersection[intersectionIndex][TOP_EDGE].TexV =
      //mid_tex_v;
      intersection[intersectionIndex][TOP_EDGE].TexV =
          personalities[intersectionIndex] & 1;  // LORENZEN LORENZEN

      intersection[intersectionIndex][BOTTOM_EDGE].PointCount = 1;
      intersection[intersectionIndex][BOTTOM_EDGE].NextSegmentID =
          intersectionIndex;
      intersection[intersectionIndex][BOTTOM_EDGE].Point = midpoint;

      //			intersection[intersectionIndex][BOTTOM_EDGE].TexV
      //= mid_tex_v;
      intersection[intersectionIndex][BOTTOM_EDGE].TexV =
          personalities[intersectionIndex] & 1;  // LORENZEN LORENZEN

      // Intersection calculation: if the top/bottom planes of both adjoining
      // segments are not very close to being parallel, intersect them to get
      // top/bottom intersection lines. If the planes are almost parallel, pick
      // one, find the point on the plane closest to the midpoint, and convert
      // that point to a line direction vector.
      ...
      if (fabs(vdp) < parallel_factor) {
        // Not parallel - intersect planes to get line (get vector, normalize
        // it, ensure it is pointing towards the midpoint)
        ...
        if (Vector3::Dot_Product(
                intersection[intersectionIndex][TOP_EDGE].Direction, midpoint) <
            0.0f) {
          intersection[intersectionIndex][TOP_EDGE].Direction =
              -intersection[intersectionIndex][TOP_EDGE].Direction;
        }
        ...
        // Parallel (or almost): find point on av. plane closest to midpoint,
        // convert to line

        // Ensure average calculation is numerically stable:
        Vector3 pl;
        if (vdp > 0.0f) {
          pl = segment[intersectionIndex - 1].EdgePlane[TOP_EDGE] +
               segment[intersectionIndex].EdgePlane[TOP_EDGE];
        } else {
          pl = segment[intersectionIndex - 1].EdgePlane[TOP_EDGE] -
               segment[intersectionIndex].EdgePlane[TOP_EDGE];
        ...
        intersection[intersectionIndex][TOP_EDGE].Direction =
            midpoint - pl * Vector3::Dot_Product(pl, midpoint);
        intersection[intersectionIndex][TOP_EDGE].Direction.Normalize();
      ...
      if (fabs(vdp) < parallel_factor) {
        ...
        Vector3 pl;
        if (vdp > 0.0f) {
          pl = segment[intersectionIndex - 1].EdgePlane[BOTTOM_EDGE] +
               segment[intersectionIndex].EdgePlane[BOTTOM_EDGE];
        } else {
          pl = segment[intersectionIndex - 1].EdgePlane[BOTTOM_EDGE] -
               segment[intersectionIndex].EdgePlane[BOTTOM_EDGE];
      ...
      Vector3::Cross_Product(
          intersection[intersectionIndex][TOP_EDGE].Direction,
          intersection[intersectionIndex][BOTTOM_EDGE].Direction, &start_pl);
      start_pl.Normalize();
      dp =
          Vector3::Dot_Product(segment[intersectionIndex].StartPlane, start_pl);
      if (dp > 0.0f) {
    ...
    /*
    ** Intersection merging: when an intersection is inside an adjacent segment
    *and certain
    ** other conditions hold true, we need to merge intersections to avoid
    *visual glitches
    ** caused by the polys folding over on themselves.
    */

    if (Is_Merge_Intersections()) {
      // Since we are merging the intersections in-place, we have two index
      // variables, a "read index" and a "write index".
      unsigned int intersectionIndex_r;
      unsigned int intersectionIndex_w;

      // The merges will be repeated in multiple passes until none are
      // performed. The reason for this is that one merge may cause the need for
      // another merge elsewhere.
      bool merged = true;
      ...
      while (merged) {
        ...
        SegmentEdge edge;
        for (edge = FIRST_EDGE; edge <= MAX_EDGE;
             edge = (SegmentEdge)((int)edge + 1)) {
          // Merge top and bottom edge intersections: loop through the
          // intersections from the first intersection to the penultimate
          // intersection, for each intersection check if it needs to be merged
          // with the next one (which is why the loop doesn't go all the way to
          // the last intersection). We start at 1 because 0 is the dummy
          // "pre-first-point" intersection.
          unsigned int num_isects =
              num_intersections[edge];  // Capture here because will change
                                        // inside loop
          for (intersectionIndex_r = 1, intersectionIndex_w = 1;
               intersectionIndex_r < num_isects;
               intersectionIndex_r++, intersectionIndex_w++) {
            // Check for either of two possible reasons to merge this
            // intersection with the next: either the segment on the far side of
            // the next intersection overlaps this intersection, or the previous
            // segment overlaps the next intersection. Note that some other
            // conditions need to be true as well.

            // Note: intersectionIndex_r is used for anything at or after the
            // current position, intersectionIndex_w is used for anything before
            // the current position (previous positions have potentially already
            // been merged). Note: intersectionIndex_r is used for anything at
            // or after the current position, intersectionIndex_w is used for
            // anything before the current position (previous positions have
            // potentially already been merged).
            LineSegmentIntersection *curr_int =
                &(intersection[intersectionIndex_r][edge]);
            LineSegmentIntersection *next_int =
                &(intersection[intersectionIndex_r + 1][edge]);
            LineSegmentIntersection *write_int =
                &(intersection[intersectionIndex_w][edge]);
            LineSegmentIntersection *prev_int =
                &(intersection[intersectionIndex_w - 1][edge]);
            ...
            // If this intersection is inside both the start plane and the
            // segment edge plane of the segment after the next intersection,
            // merge this edge intersection and the next. We repeat merging
            // until no longer needed. NOTE - we do not merge across a fold.
            while ((!next_int->Fold &&
                    (Vector3::Dot_Product(curr_int->Direction,
                                          next_seg->StartPlane) > 0.0f) &&
                    (Vector3::Dot_Product(curr_int->Direction,
                                          next_seg->EdgePlane[edge]) > 0.0f)) ||
                   (!curr_int->Fold &&
                    (Vector3::Dot_Product(next_int->Direction,
                                          -curr_seg->StartPlane) > 0.0f) &&
                    (Vector3::Dot_Product(next_int->Direction,
                                          prev_seg->EdgePlane[edge]) > 0.0f))) {
              // First calculate location of merged intersection - this is so we
              // can abort the merge if it yields funky results.

              // Find mean point (weighted so all points have same weighting)
              unsigned int new_count =
                  curr_int->PointCount + next_int->PointCount;
              ...
              Vector3 new_point =
                  curr_int->Point * curr_factor + next_int->Point * next_factor;
              float new_tex_v =
                  curr_int->TexV * curr_factor + next_int->TexV * next_factor;

              // Calculate new intersection direction by intersecting prev_seg
              // with next_seg
              bool new_parallel;
              Vector3 new_direction;
              vdp = Vector3::Dot_Product(prev_seg->EdgePlane[edge],
                                         next_seg->EdgePlane[edge]);
              if (fabs(vdp) < parallel_factor) {
                // Not parallel - intersect planes to get line (get vector,
                // normalize it, ensure it is pointing towards the current
                // point)
                Vector3::Cross_Product(prev_seg->EdgePlane[edge],
                                       next_seg->EdgePlane[edge],
                                       &new_direction);
                ...
                // Parallel (or almost). If the current intersection is not
                // parallel, take the average plane and intersect it with the
                // skipped plane. If the current intersection is parallel, find
                // the average plane, and find the direction vector on it
                // closest to the current intersections direction vector.
                ...
                if (curr_int->Parallel) {
                  new_direction = new_direction -
                                  pl * Vector3::Dot_Product(pl, new_direction);
                  new_direction.Normalize();
                } else {
                  Vector3::Cross_Product(curr_seg->EdgePlane[edge], pl,
                                         &new_direction);
                  new_direction.Normalize();
                }
              ...
              // Now check to see if the merge caused any funky results - if so
              // abort it. Currently we check to see if the distance of the
              // direction from the two points is larger than the radius times
              // the merge_abort factor.
              if (MergeAbortFactor > 0.0f) {
                ...
                Vector3 diff_curr =
                    curr_int->Point -
                    new_direction *
                        Vector3::Dot_Product(curr_int->Point, new_direction);
                if (diff_curr.Length2() > abort_dist2) break;
                Vector3 next_curr =
                    next_int->Point -
                    new_direction *
                        Vector3::Dot_Product(next_int->Point, new_direction);
              ...
              // Advance intersectionIndex_r to shift subsequent entries
              // backwards in result.
              intersectionIndex_r++;
          ...
          // If intersectionIndex_r is exactly equal to num_isects (rather than
          // being larger by one) at this point, this means that the last
          // intersection was not merged with the previous one. In this case, we
          // need to do one last copy:
          if (intersectionIndex_r == num_isects) {
            LineSegmentIntersection *write_int =
                &(intersection[intersectionIndex_w][edge]);
            LineSegmentIntersection *curr_int =
    ...
    ** Since we can have top/bottom intersections merged, we need to skip points
    *if both the top
    ** and bottom intersections are merged, generate triangle fans if one of the
    *sides is merged
    ...
    */
    ...
    unsigned int vnum =
        num_intersections[TOP_EDGE] + num_intersections[BOTTOM_EDGE];
    ...
    // GENERALIZE FOR WHEN NO TEXTURE (DO NOT SET UV IN THESE CASES? NEED TO
    // GENERALIZE FOR DIFFERENT TEXTURING MODES ANYWAY).

    // "Prime the pump" with two vertices (pick nearest point on each direction
    // line):
    ...
    unsigned int bottom_int_idx =
        1;  // Skip "pre-first-point" dummy intersection
    pointIndex = 0;
    unsigned int residual_top_points = intersection[1][TOP_EDGE].PointCount;
    unsigned int residual_bottom_points =
        intersection[1][BOTTOM_EDGE].PointCount;

    // Reduce both pointcounts by the same amount so the smaller one is 1 (skip
    // points)
    ...
    for (;;) {
      if (residual_top_points == 1 && residual_bottom_points == 1) {
        ...
        residual_bottom_points =
            intersection[bottom_int_idx][BOTTOM_EDGE].PointCount;

        // Advance point index (must do here because the new point index is used
        // below):
        pointIndex++;

        // Generate two vertices for next point by picking nearest point on each
        // direction line
        Vector3 &top_dir = intersection[top_int_idx][TOP_EDGE].Direction;
        top = top_dir * Vector3::Dot_Product(points[pointIndex], top_dir);
        Vector3 &bottom_dir =
            intersection[bottom_int_idx][BOTTOM_EDGE].Direction;
        bottom =
            bottom_dir * Vector3::Dot_Product(points[pointIndex], bottom_dir);
        ...
        vertexArray[vertexIndex].v1 =
            intersection[bottom_int_idx][BOTTOM_EDGE].TexV;
        vertexIndex++;
      } else {
        // Exactly one of the pointcounts is greater than one - advance it and
        // draw one triangle
        if (residual_top_points > 1) {
          ...
          // Advance point index (must do here because the new point index is
          // used below):
          pointIndex++;

          // Generate bottom vertex by picking nearest point on bottom direction
          // line
          Vector3 &bottom_dir =
              intersection[bottom_int_idx][BOTTOM_EDGE].Direction;
          bottom =
          ...
          vertexArray[vertexIndex].u1 = u_values[0];
          vertexArray[vertexIndex].v1 =
              intersection[top_int_idx][TOP_EDGE].TexV;
          vertexIndex++;
        }
      }

      // Reduce both pointcounts by the same amount so the smaller one is 1
      // (skip points)
      ...
      pointIndex += delta;
      // Exit conditions
      if ((top_int_idx >= num_intersections[TOP_EDGE] &&
           residual_top_points == 1) ||
          (bottom_int_idx >= num_intersections[BOTTOM_EDGE] &&
           residual_bottom_points == 1)) {
        // Debugging check - if either intersection index is before end, both of
        // them should be and the points should be before the end.
    ...
    */
    ...
    // If color is not white or opacity not 100%, enable gradient in shader and
    // in renderer - otherwise disable.
    // unsigned int rgba;
    ...
    //COLORS THEM ALL TO THE COLOR,OPACITY MEMBERS ///////////////// 			unsigned
    //int perPointARGB; 			colorIndex = MIN(vertexIndex / 2, point_cnt);
    //			perPointARGB = DX8Wrapper::Convert_Color(
    //colors[colorIndex] );// twice as many verts as points? or so?
    //			vertexArray[vertexIndex].diffuse = perPointARGB;
    //			vertexArray[vertexIndex].u1 = (float)((vertexIndex&2) ==
    //2); 			vertexArray[vertexIndex].v1 = (float)((vertexIndex&1) == 1);
    //		}

    // Enable sorting if sorting has not been disabled and line is translucent
    // and alpha testing is not enabled.
    ...
    */
    ...
    // Copy in the data to the  VB
    {
      ...
      for (i = 0; i < vnum; i++) {
        ...
        argb = DX8Wrapper::Convert_Color_Clamp(
            colors[segIdx]);  // twice as many verts as points? or so?
        ...
        *(unsigned int *)(vb + fvfinfo.Get_Diffuse_Offset()) =
            vertexArray[i].diffuse;

        *(unsigned int *)(vb + fvfinfo.Get_Diffuse_Offset()) = argb;

        //			((Vector2*)(vb+fvfinfo.Get_Tex_Offset(0)))->V
        //=	(float)( ((i+oddEven)&1) == 1 );//mirror tiling
        //			((Vector2*)(vb+fvfinfo.Get_Tex_Offset(0)))->U
        //=	(float)( ((i+oddEven)&2) == 2 );//mirror tiling
    ...
    if (sorting) {
      SortingRendererClass::Insert_Triangles(obj_sphere, 0, triangleIndex, 0,
                                             vnum);
    } else {
