...
TextureClass *TexBufferClass::Peek_Element(int index) { return Array[index]; }
...
bool UVBufferClass::operator==(const UVBufferClass &that) {
  // NOTE: this only works if you've properly called Update_CRC after filling
  // the array
  return (CRC == that.CRC);
}
...
MeshMatDescClass::MeshMatDescClass(void)
    : PassCount(1), VertexCount(0), PolyCount(0) {
    ...
    for (int pass = 0; pass < MAX_PASSES; pass++) {
      for (int stage = 0; stage < MAX_TEX_STAGES; stage++) {
        ...
        if (that.TextureArray[pass][stage]) {
          TextureArray[pass][stage] =
              NEW_REF(TexBufferClass, (*that.TextureArray[pass][stage]));
        }
      }

      //			REF_PTR_SET(UVIndex [pass], that.UVIndex
      //[pass]);
      ...
      // The only time we add-ref these arrays are when we make alternate
      // material representations within this mesh... Then we re-use the same
      // arrays in different passes...
      REF_PTR_RELEASE(MaterialArray[pass]);
      if (that.MaterialArray[pass]) {
        MaterialArray[pass] =
            NEW_REF(MatBufferClass, (*that.MaterialArray[pass]));
      }
      REF_PTR_RELEASE(ShaderArray[pass]);
      if (that.ShaderArray[pass]) {
        ShaderArray[pass] =
            NEW_REF(ShareBufferClass<ShaderClass>, (*that.ShaderArray[pass]));
      }
    }
  }
  return *this;
...
MeshMatDescClass::~MeshMatDescClass(void) { Reset(0, 0, 0); }
...
void MeshMatDescClass::Init_Alternate(MeshMatDescClass &default_materials,
                                      MeshMatDescClass &alternate_materials) {
  ...
  // Copy the uv-arrays from the alternate materials to start.  Needed uv arrays
  // from the default material set will be brought over as encountered below
  for (int i = 0; i < alternate_materials.Get_UV_Array_Count(); i++) {
    REF_PTR_SET(UV[i], alternate_materials.UV[i]);
  }

  // add-ref the arrays in default_materials except when the same array is
  // present in alternate_materials
  for (int pass = 0; pass < MAX_PASSES; pass++) {
    for (int stage = 0; stage < MAX_TEX_STAGES; stage++) {
      // UV Coorindate arrays, Each UVSource[pass][stage] which is -1 in the
      // alternate_materials but not -1 in the default_materials causes us to
      // copy over a uv array from the default_materials and set its index into
      // our UVSource array.
      if (alternate_materials.UVSource[pass][stage] == -1) {
        if (default_materials.UVSource[pass][stage] != -1) {
          // Look up the uv array in default_materials that we need to bring
          // over.
          ...
          // If we already have it, just set the source index.  Otherwise
          // add-ref it into a new slot in our uv array and set that index.
          if (found_index != -1) {
      ...
      // Texture pointer(s):  If alternate_materials has either a single texture
      // or an array of textures, then add-ref only the texture data it
      // contains.  Otherwise, add-ref the data in default_materials.
      if ((alternate_materials.Texture[pass][stage] != NULL) ||
          (alternate_materials.TextureArray[pass][stage])) {
        REF_PTR_SET(Texture[pass][stage],
                    alternate_materials.Texture[pass][stage]);
        REF_PTR_SET(TextureArray[pass][stage],
                    alternate_materials.TextureArray[pass][stage]);
      } else {
        REF_PTR_SET(Texture[pass][stage],
                    default_materials.Texture[pass][stage]);
        REF_PTR_SET(TextureArray[pass][stage],
                    default_materials.TextureArray[pass][stage]);
      }
    }
    ...
    // Shaders, currently I can't tell if the alternate data has a shader...
    // Can't override the shader for now.
    Shader[pass] = default_materials.Shader[pass];
    REF_PTR_SET(ShaderArray[pass], default_materials.ShaderArray[pass]);

    // Vertex Materials.  If alternate_materials has either a single or array of
    // materials, then copy them
    if ((alternate_materials.Material[pass] != NULL) ||
      ...
      if (default_materials.Material[pass]) {
        Material[pass] =
            NEW_REF(VertexMaterialClass, (*(default_materials.Material[pass])));
      } else {
        if (default_materials.MaterialArray[pass]) {
          WWDEBUG_SAY(
              ("Unimplemented case: mesh has more than one default vertex "
               "material but no alternate vertex materials have been "
               "defined.\r\n"));
        }
        Material[pass] = NULL;
      }
...
void MeshMatDescClass::Set_Single_Texture(TextureClass *tex, int pass,
                                          int stage) {
...
void MeshMatDescClass::Set_Material(int vidx, VertexMaterialClass *vmat,
                                    int pass) {
...
void MeshMatDescClass::Set_Texture(int pidx, TextureClass *tex, int pass,
                                   int stage) {
...
TextureClass *MeshMatDescClass::Get_Texture(int pidx, int pass,
                                            int stage) const {
...
TextureClass *MeshMatDescClass::Peek_Texture(int pidx, int pass,
                                             int stage) const {
  if (TextureArray[pass][stage]) {
...
TexBufferClass *MeshMatDescClass::Get_Texture_Array(int pass, int stage,
                                                    bool create) {
  if (create && TextureArray[pass][stage] == NULL) {
    TextureArray[pass][stage] =
        NEW_REF(TexBufferClass, (PolyCount, "MeshMatDescClass::TextureArray"));
  }
...
MatBufferClass *MeshMatDescClass::Get_Material_Array(int pass, bool create) {
  if (create && MaterialArray[pass] == NULL) {
    MaterialArray[pass] = NEW_REF(
        MatBufferClass, (VertexCount, "MeshMatDescClass::MaterialArray"));
  }
...
ShaderClass *MeshMatDescClass::Get_Shader_Array(int pass, bool create) {
  if (create && ShaderArray[pass] == NULL) {
    ShaderArray[pass] = NEW_REF(ShareBufferClass<ShaderClass>,
                                (PolyCount, "MeshMatDescClass::ShaderArray"));
...
void MeshMatDescClass::Install_UV_Array(int pass, int stage, Vector2 *uvs,
                                        int count) {
  /*
...
void MeshMatDescClass::Post_Load_Process(bool lighting_enabled) {
  /*
  ** Configure all vertex materials to source the uv coordinates and colors from
  *the correct arrays
  ...
  */
  for (int pass = 0; pass < PassCount; pass++) {
    ...
    */
    ...
    // Analyze material array types and apply hacks for supporting SR-lighting
    // pipeline if possible.

    if (!ColorArray[0] && !ColorArray[1])
      continue;  // If no color arrays, we don't have a problem
    ...
    for (int vidx = 0; vidx < VertexCount; vidx++) {
      ...
      if (mtl_diffuse.X != single_diffuse.X ||
          mtl_diffuse.Y != single_diffuse.Y ||
          mtl_diffuse.Z != single_diffuse.Z) {
        single_diffuse_used = false;
      }
      if (mtl_ambient.X != single_ambient.X ||
          mtl_ambient.Y != single_ambient.Y ||
          mtl_ambient.Z != single_ambient.Z) {
        single_ambient_used = false;
      }
      if (mtl_emissive.X != single_emissive.X ||
          mtl_emissive.Y != single_emissive.Y ||
    ...
    // If both DCG and DIG arrays are submitted, multiply them together to DCG
    // channel
    if ((DCGSource[pass] != VertexMaterialClass::MATERIAL) &&
        (ColorArray[0] != NULL) &&
        (DIGSource[pass] != VertexMaterialClass::MATERIAL) &&
        (ColorArray[1] != NULL)) {
    ...
    if ((DCGSource[pass] != VertexMaterialClass::MATERIAL) &&
      ...
      for (int vidx = 0; vidx < VertexCount; vidx++) {
        ...
        // If only diffuse is used apply diffuse to color channel and set
        // diffuse source to color 1
        ...
        // If diffuse and ambient are used, apply diffuse to color channel and
        // set diffuse and ambient sources to color 1. (this is not completely
        // correct if diffuse and ambient are different but is probably the most
        // reasonable thing to do. Why set diffuse and ambient differently
        // anyway?)
        if (diffuse_used && ambient_used && !emissive_used) {
          Vector4 diffuse = DX8Wrapper::Convert_Color(diffuse_array[vidx]);
          diffuse.X *= mtl_diffuse.X;
        ...
        // If only ambient is used apply ambient to color channel and set
        // ambient source to color 1
        if (!diffuse_used && ambient_used && !emissive_used) {
          Vector4 diffuse = DX8Wrapper::Convert_Color(diffuse_array[vidx]);
        ...
        // If only emissive is used apply emissive to color channel, set diffuse
        // source to color 1, and turn off lighting
        if (!diffuse_used && !ambient_used && emissive_used) {
          Vector4 diffuse = DX8Wrapper::Convert_Color(diffuse_array[vidx]);
          ...
          mtl->Set_Diffuse_Color_Source(VertexMaterialClass::COLOR1);
          mtl->Set_Emissive_Color_Source(VertexMaterialClass::MATERIAL);
          // MW:  Vegas guys asked me to disable this because it can cause
          // z-fighting if lighting is disabled in multi-pass
    ...
    /*
    ** If a DCG array is present, pre multiply the alpha value from the material
    *into
    ** the vertex color array.  Experimentation on GeForce hardware showed that
    *we
    ** don't need to pre-multiply the color values; hopefully this is the
    *behavior on
    ** other hardware as well!
    */
    /*		if ((DCGSource[pass] != VertexMaterialClass::MATERIAL) &&
       (ColorArray[0] != NULL)) { unsigned * diffuse_array =
       ColorArray[0]->Get_Array(); Vector3 mtl_diffuse; float mtl_opacity
       = 1.0f;
                            ...
                            for (int vidx=0; vidx<VertexCount; vidx++) {
                                    ...
                                    Vector4
       diffuse=DX8Wrapper::Convert_Color(diffuse_array[vidx]); diffuse.X *=
       mtl_diffuse.X; diffuse.Y *= mtl_diffuse.Y; diffuse.Z *= mtl_diffuse.Z;
                                    diffuse.W *= mtl_opacity;
                                    diffuse_array[vidx]=DX8Wrapper::Convert_Color(diffuse);
    ...
    */		/*
		...
		*/
    /*		if ((DIGSource[pass] != VertexMaterialClass::MATERIAL) &&
       (ColorArray[1] != NULL)) {

                            unsigned * emissive_array =
       ColorArray[1]->Get_Array(); Vector3 mtl_emissive;
                            ...
                            for (int vidx=0; vidx<VertexCount; vidx++) {
                                    ...
                                    Vector4
       emissive=DX8Wrapper::Convert_Color(emissive_array[vidx]); emissive.X *=
       mtl_emissive.X; emissive.Y *= mtl_emissive.Y; emissive.Z *=
       mtl_emissive.Z; emissive_array[vidx]=DX8Wrapper::Convert_Color(emissive);
...
void MeshMatDescClass::Configure_Material(VertexMaterialClass *mtl, int pass,
...
bool MeshMatDescClass::Do_Mappers_Need_Normals(void) {
  if (DX8Caps::Support_NPatches() && WW3D::Get_NPatches_Level() > 1)
    return true;

  for (int pass = 0; pass < PassCount; pass++) {
    /*
    ** Check the materials on this pass to see if any have mappers which require
    *normals
    */
    if (Material[pass] != NULL) {
