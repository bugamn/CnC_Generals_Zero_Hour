 ...
 **
 *                                                                                             *
 * Rebuilds the give image to better pack characters and to insure a square
 *power of two size  * Must be called AFTER Make_Proportional() so each chars
 *minimal bounding box is known        * Will only create a new texture of size
 *128x128 or 256x256, dependent on original width      *
 *                                                                                             *
 ***********************************************************************************************/
SurfaceClass *Font3DDataClass::Minimize_Font_Image(SurfaceClass *surface) {
  ...
  // determine new width make the size of the new image either 128x128 or
  // 256x256, dependent on the width of the original image
  int new_width;
  ...
  SurfaceClass *new_surface =
      NEW_REF(SurfaceClass, (new_width, new_height, WW3D_FORMAT_A4R4G4B4));
  // SurfaceClass	*new_surface0 = NEW_REF(SurfaceClass,(new_width,
  // new_height,sd.Format));
  ...
  // for each character, copy minimum bounding area to (new_x, new_y) in the new
  // image
  for (int char_index = 0; char_index < 256; char_index++) {
    // find the lop left coordinate and the height and width of the char's
    // bounding box (must convert the normalized uv tables to pixels and round
    // off)
    ...
    if (width != 0) {
      ...
      if (new_x + width > new_width) {
        ...
        // we assert because we have already modified tables for some of the
        // chars
 ...
 **
 *                                                                                             *
 * Modifys U and Width tables to convert a monospace font into a proportional
 *font.  Hieght	  * remains the same.  Performed by getting the current
 *mono-space bounding box and bringing	  * in the left and right edges to the
 *first non-transparent ( != 0 ) pixel.  Then the U and	  * width tables are
 *updated with the new values.  The image itself is not modified unless...
 **
 * 																														  *
 * we complete by calling Minimize_Font_Image to shink the image & insure a
 *power of 2 square  *
 *                                                                                             *
 ***********************************************************************************************/
SurfaceClass *Font3DDataClass::Make_Proportional(SurfaceClass *surface) {
  ...
  for (int char_index = 0; char_index < 256; char_index++) {
    ...
    //	find minimum bounding box by finding the minimum and maximum non-0 x
    //pixel location
    Vector2i minb(x0, y0);
    Vector2i maxb(x1, y1);
 ...
 **
 *                                                                                             *
 * Loads a targa font image file, arranged as 16x16 characters, and builds u v
 *tables to 		  * find each character.  Converts the mono-space font
 *into a proportional font, then uploads	  * the image to the scene as a
 *textur material.                                                *
 *                                                                                             *
 ***********************************************************************************************/
  ...
  if (sd.Width > 8 * sd.Height) {
    ...
    for (int char_index = 0; char_index < 256; char_index++) {
      if (char_index >= 0x7F) {
        ...
        while ((column < width) && (surface->Is_Transparent_Column(column)))
          column++;
        int start = column;
    ...
    // mono-spaced width and height in normalized screen units
    for (int char_index = 0; char_index < 256; char_index++) {
      UOffsetTable[char_index] =
          (float)((char_index % 16) * mono_pixel_width) / font_width;
      VOffsetTable[char_index] =
          (float)((char_index / 16) * mono_pixel_height) / font_height;
 ...
 *                                                                                             *
 ***********************************************************************************************/
Font3DInstanceClass::~Font3DInstanceClass(void) { REF_PTR_RELEASE(FontData); }
 ...
 **
 *                                                                                             *
 * Finds the normalized screenspace width of a character string - useful for
 *checking before   * printing to avoid overflowing the screen.
 ** *
 ***********************************************************************************************/
