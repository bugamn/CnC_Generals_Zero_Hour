...
bool DX8Wrapper::Init(void *hwnd) {
  ...
  */
  D3DInterface =
      Direct3DCreate8(D3D_SDK_VERSION);  // TODO: handle failure cases...
  if (!D3DInterface) return false;
  IsInitted = true;
...
void DX8Wrapper::Invalidate_Cached_Render_States(void) {
  ...
  for (a = 0; a < MAX_TEXTURE_STAGES; ++a) {
    ...
    if (_Get_D3D_Device8()) _Get_D3D_Device8()->SetTexture(a, NULL);
    if (Textures[a] != NULL) Textures[a]->Release();
    Textures[a] = NULL;
  }
...
bool DX8Wrapper::Create_Device(void) {
  WWASSERT(
      D3DDevice ==
      NULL);  // for now, once you've created a device, you're stuck with it!
  ...
  if (DX8Wrapper_PreserveFPU) vertex_processing_type |= D3DCREATE_FPU_PRESERVE;

  if (FAILED(D3DInterface->CreateDevice(CurRenderDevice, WW3D_DEVTYPE, _Hwnd,
...
bool DX8Wrapper::Reset_Device(bool reload_assets) {
  ...
  if ((IsInitted) && (D3DDevice != NULL)) {
    ...
    if (hr != D3DERR_DEVICELOST) {
      ...
      if (hr != D3D_OK) return false;  // reset failed.
    } else
...
void DX8Wrapper::Release_Device(void) {
  if (D3DDevice) {
    ...
    DX8CALL(SetStreamSource(
        0, NULL, 0));  // release reference count on last rendered vertex buffer
    DX8CALL(SetIndices(
        NULL, 0));  // release reference count on last rendered index buffer
...
bool DX8Wrapper::Set_Render_Device(int dev, int width, int height, int bits,
  ...
  */
  ...
  WWDEBUG_SAY(
      ("Attempting Set_Render_Device: name: %s (%s:%s), width: %d, height: %d, "
       "windowed: %d\n",
       _RenderDeviceNameTable[CurRenderDevice],
  ...
  if (resize_window) {
    ...
    if ((rect.right - rect.left) != ResolutionWidth ||
      ...
      if (!windowed)
        ::SetWindowPos(_Hwnd, HWND_TOPMOST, 0, 0, rect.right - rect.left,
                       rect.bottom - rect.top, SWP_NOSIZE | SWP_NOMOVE);
      else
  ...
  */
  ...
  _PresentParameters.SwapEffect =
      IsWindowed ? D3DSWAPEFFECT_DISCARD
                 : D3DSWAPEFFECT_FLIP;  // Shouldn't this be D3DSWAPEFFECT_FLIP?
...
bool DX8Wrapper::Test_Z_Mode(D3DFORMAT colorbuffer, D3DFORMAT backbuffer,
  ...
  if (FAILED(D3DInterface->CheckDeviceFormat(D3DADAPTER_DEFAULT, WW3D_DEVTYPE,
    ...
    WWDEBUG_SAY(
        ("CheckDeviceFormat failed.  Colorbuffer format = %d  Zbufferformat = "
         "%d\n",
         colorbuffer, zmode));
    return false;
  }
  ...
  if (FAILED(D3DInterface->CheckDepthStencilMatch(
    ...
    WWDEBUG_SAY(
        ("CheckDepthStencilMatch failed.  Colorbuffer format = %d  Backbuffer "
...
void DX8Wrapper::Clear(bool clear_color, bool clear_z_stencil,
  ...
  // will fail
  bool has_stencil =
      (_PresentParameters.AutoDepthStencilFormat == D3DFMT_D15S1 ||
       _PresentParameters.AutoDepthStencilFormat == D3DFMT_D24S8 ||
       _PresentParameters.AutoDepthStencilFormat == D3DFMT_D24X4S4);
  ...
  if (flags) {
    DX8CALL(
        Clear(0, NULL, flags, Convert_Color(color, dest_alpha), z, stencil));
  }
}
...
void DX8Wrapper::Draw_Sorting_IB_VB(unsigned primitive_type,
                                     ...
                                     vertex_count);
  {
    ...
    VertexFormatXYZNDUV2 *src =
        static_cast<SortingVertexBufferClass *>(render_state.vertex_buffer)
            ->VertexBuffer;
...
void DX8Wrapper::Draw(unsigned primitive_type, unsigned short start_index,
  ...
  SNAPSHOT_SAY(
      ("DX8 - draw %s polygons (%d vertices)\n", polygon_count, vertex_count));
  ...
  if (vertex_count < 3) {
    ...
    switch (render_state.vertex_buffer_type) {
      ...
      case BUFFER_TYPE_SORTING:
        vertex_count = render_state.vertex_buffer->Get_Vertex_Count() -
                       render_state.index_base_offset -
                       render_state.vba_offset - min_vertex_index;
        break;
        ...
        case BUFFER_TYPE_DYNAMIC_DX8: {
          /*				if
             ...
             WWASSERT_PRINT(0,"OVERFLOWING INDEX BUFFER");
                                                  ///@todo: MUST FIND OUT WHY
             THIS HAPPENS WITH LOTS OF PARTICLES ON BIG FIGHT!  -MW break;
                                          }*/
          DX8_RECORD_RENDER(polygon_count, vertex_count, render_state.shader);
...
IDirect3DTexture8 *DX8Wrapper::_Create_DX8_Texture(
    IDirect3DSurface8 *surface, TextureClass::MipCountType mip_level_count) {
  DX8_THREAD_ASSERT();
  DX8_Assert();
  IDirect3DTexture8 *texture = NULL;
  ...
  DX8_ErrorCode(D3DXLoadSurfaceFromSurface(tex_surface, NULL, NULL, surface,
                                           NULL, NULL, D3DX_FILTER_BOX, 0));
  tex_surface->Release();
...
IDirect3DSurface8 *DX8Wrapper::_Create_DX8_Surface(const char *filename_) {
  ...
  surface = TextureLoader::Load_Surface_Immediate(filename_,
...
void DX8Wrapper::Compute_Caps(D3DFORMAT display_format,
...
TextureClass *DX8Wrapper::Create_Render_Target(int width, int height,
  ...
  // If the user requested a render-target texture and this device does not
  // support that feature, return NULL
  HRESULT hr;

  if (alpha) {
    // user wants a texture with destination alpha channel - only 1 such format
    // ever exists on current hardware	- D3DFMT_A8R8G8B8
    hr = D3DInterface->CheckDeviceFormat(D3DADAPTER_DEFAULT, WW3D_DEVTYPE,
  ...
  TextureClass *tex =
      NEW_REF(TextureClass, (width, height, format, TextureClass::MIP_LEVELS_1,
...
IDirect3DSwapChain8 *DX8Wrapper::Create_Additional_Swap_Chain(
    HWND render_window) {
  DX8_Assert();
