...
Render2DClass::~Render2DClass() { REF_PTR_RELEASE(Texture); }
...
void Render2DClass::Set_Texture(const char *filename) {
  TextureClass *tex = WW3DAssetManager::Get_Instance()->Get_Texture(
      filename, TextureClass::MIP_LEVELS_1);
  Set_Texture(tex);
  if (tex != NULL) {
    SET_REF_OWNER(tex);
    tex->Release_Ref();
...
void Render2DClass::Enable_Grayscale(bool b) { IsGrayScale = b; }
...
void Render2DClass::Add_Quad(const Vector2 &v0, const Vector2 &v1,
                             const Vector2 &v2, const Vector2 &v3,
                             const RectClass &uv, unsigned long color) {
...
void Render2DClass::Add_Quad_Backfaced(const Vector2 &v0, const Vector2 &v1,
                                       const Vector2 &v2, const Vector2 &v3,
                                       const RectClass &uv,
                                       unsigned long color) {
...
void Render2DClass::Add_Quad_VGradient(const Vector2 &v0, const Vector2 &v1,
                                       const Vector2 &v2, const Vector2 &v3,
                                       const RectClass &uv,
...
void Render2DClass::Add_Quad_HGradient(const Vector2 &v0, const Vector2 &v1,
                                       const Vector2 &v2, const Vector2 &v3,
                                       const RectClass &uv,
...
** Add Tri
*/
void Render2DClass::Add_Tri(const Vector2 &v0, const Vector2 &v1,
...
void Render2DClass::Add_Line(const Vector2 &a, const Vector2 &b, float width,
                             unsigned long color) {
...
void Render2DClass::Add_Line(const Vector2 &a, const Vector2 &b, float width,
  ...
  Add_Quad(a - corner_offset, a + corner_offset, b - corner_offset,
           b + corner_offset, uv, color);
  // Add_Quad_HGradient(RectClass( a.X -corner_offset.X ,a.Y,b.X,b.Y ), color,
  // color2); Add_Quad_HGradient( const RectClass & screen, unsigned long
  // left_color, unsigned long right_color );
}

void Render2DClass::Add_Line(const Vector2 &a, const Vector2 &b, float width,
                             unsigned long color, unsigned long color2) {
...
void Render2DClass::Add_Line(const Vector2 &a, const Vector2 &b, float width,
  ...
  Add_Quad_HGradient(a - corner_offset, a + corner_offset, b - corner_offset,
                     b + corner_offset, uv, color, color2);
  // Add_Quad_HGradient(RectClass( a.X -corner_offset.X ,a.Y,b.X,b.Y ), color,
  // color2);
}

void Render2DClass::Add_Rect(const RectClass &rect, float border_width,
                             uint32 border_color, uint32 fill_color) {
  //
  //	First add the outline
  //
  if (border_width > 0) Add_Outline(rect, border_width, border_color);
...
void Render2DClass::Add_Outline(const RectClass &rect, float width,
                                unsigned long color) {
...
void Render2DClass::Add_Outline(const RectClass &rect, float width,
                                const RectClass &uv, unsigned long color) {
  //
  //	Pretty straight forward, simply add the four side of the rectangle as
  //lines.
  //
  ...
  Add_Line(Vector2(rect.Right, rect.Top), Vector2(rect.Right, rect.Bottom - 1),
           width, color);
...
void Render2DClass::Render(void) {
  ...
  DynamicVBAccessClass vb(BUFFER_TYPE_DYNAMIC_DX8, dynamic_fvf_type,
                          Vertices.Count());
  {
  ...
  DynamicIBAccessClass ib(BUFFER_TYPE_DYNAMIC_DX8, Indices.Count());
  {
    ...
    for (int i = 0; i < Indices.Count(); i++) mem[i] = Indices[i];
  }

  DX8Wrapper::Set_Vertex_Buffer(vb);
  DX8Wrapper::Set_Index_Buffer(ib, 0);

  if (IsGrayScale) {  // special case added to draw grayscale non-alpha blended
                      // images.
    DX8Wrapper::Set_Shader(ShaderClass::_PresetOpaqueShader);
    DX8Wrapper::Apply_Render_State_Changes();  // force update of all regular
                                               // W3D states.
    if (DX8Caps::Support_DOT3()) {  // Override W3D states with customizations
                                    // for grayscale
  ...
  if (IsGrayScale) ShaderClass::Invalidate();  // force both stages to be reset.
}
...
Render2DTextClass::~Render2DTextClass() { REF_PTR_RELEASE(Font); }
...
void Render2DTextClass::Draw_Char(WCHAR ch, unsigned long color) {
  ...
  if (IsClippedEnabled &&
      (Cursor.X < ClipRect.Left || Cursor.X + char_spacing > ClipRect.Right ||
       Cursor.Y < ClipRect.Top || Cursor.Y + char_height > ClipRect.Bottom)) {
    is_clipped = true;
  }

  if (ch != (WCHAR)' ' && !is_clipped) {
    RectClass screen(Cursor.X, Cursor.Y, Cursor.X + Font->Char_Width(ch),
...
void Render2DTextClass::Draw_Text(const WCHAR *text, unsigned long color) {
  ...
  while (*text) {
    ...
    // if the current char is a space, and the next word length puts us past our
    // Width, wrap
