 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * OBBoxClass::OBBoxClass -- Constructor that computes the box for
 *a set of point            * OBBoxClass::Init_From_Box_Points -- Create an
 *OBBox from 8 corners of  a box              * OBBoxClass::Init_Random --
 *initalize a random oriented box                                *
 *   Oriented_Boxes_Intersect_On_Axis -- test if two boxes intersect on given
 *axis             * Oriented_Boxes_Intersect -- test if two oriented boxes
 *intersect                          * Oriented_Boxes_Collide_On_Axis -- test if
 *two boxes collide on the given axis             * Oriented_Boxes_Collide --
 *test if two oriented boxes collide                              *
 *   Oriented_Box_Intersects_Tri_On_Axis -- tests if the box and tri intersect
 *on the axis     * Oriented_Box_Intersects_Tri -- tests if the given box and
 *tri intersect                   *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
/***********************************************************************************************
 * OBBoxClass::OBBoxClass -- Constructor that computes the box for a set of
 *points             *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void OBBoxClass::Init_From_Box_Points(Vector3* points, int num) {
  ...
  */
  ...
  for (i = 0; i < num; i++) {
    ...
    float xprj =
        float(WWMath::Fabs(axis0.X * dx + axis0.Y * dy + axis0.Z * dz));
    if (xprj > Extent.X) Extent.X = xprj;

    float yprj =
        float(WWMath::Fabs(axis1.X * dx + axis1.Y * dy + axis1.Z * dz));
    if (yprj > Extent.Y) Extent.Y = yprj;

    float zprj =
        float(WWMath::Fabs(axis2.X * dx + axis2.Y * dy + axis2.Z * dz));
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 4/7/99     GTH : Created. *
 *=============================================================================================*/
bool Oriented_Boxes_Intersect_On_Axis(const OBBoxClass& box0,
 ...
 * HISTORY: * 4/7/99     GTH : Created. *
 *=============================================================================================*/
bool Oriented_Boxes_Collide_On_Axis(const OBBoxClass& box0, const Vector3& v0,
                                    const OBBoxClass& box1, const Vector3& v1,
                                    const Vector3& axis, float dt) {
  float ra, rb, rsum;
 ...
 * HISTORY: * 4/7/99     GTH : Created. *
 *=============================================================================================*/
bool Oriented_Boxes_Collide(const OBBoxClass& box0, const Vector3& v0,
                            const OBBoxClass& box1, const Vector3& v1,
                            float dt) {
  bool intersect = true;
...
/***********************************************************************************************
 * Oriented_Box_Intersects_Tri_On_Axis -- tests if the box and tri intersect on
 *the axis       *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 4/7/99     GTH : Created. *
 *=============================================================================================*/
bool Oriented_Box_Intersects_Tri_On_Axis(const OBBoxClass& box,
                                         const TriClass& tri, Vector3& axis) {
  ...
  lep = 0;
  tmp = Vector3::Dot_Product(r1, axis);
  if (tmp < lep) lep = tmp;
  tmp = Vector3::Dot_Product(r2, axis);
  if (tmp < lep) lep = tmp;
  lep += dist;
