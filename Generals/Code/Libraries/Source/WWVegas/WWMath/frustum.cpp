 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * FrustumClass::Init -- Initialize a frustum object *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
 ...
 *                                                                                             *
 * INPUT: * camera - camera transform, note that the camera looks down the -Z
 *axis                      * vpmin - min corner of the z=-1.0 view plane (not
 *necessarily the near clip plane)           * vpmax - max corner of the z=-1.0
 *view plane (not necessarily the near clip plane)           * znear - near clip
 *plane (should be negative, negated if it is not)                          *
 * zfar - far clip plane (should be negative, negated if it is not) *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * The vpmin and vpmax variables are the min and max of a view-plane
 *at z=-1.0                 *
 *                                                                                             *
 *                                                                                             *
 * HISTORY: * 2/17/2000  gth : Created. *
 *=============================================================================================*/
void FrustumClass::Init(const Matrix3D& camera, const Vector2& vpmin,
                        const Vector2& vpmax, float znear, float zfar) {
  int i;
  ...
  // When looking at the frustum from the position of the camera, the near four
  // corners are numbered: upper left 0, upper right 1, lower left 2, lower
  // right 3. The far plane's Frustum corners are numbered from 4 to 7 in an
  // analogous fashion.
  // (remember: the camera space has x going to the right, y up and z
  // backwards).

  // calculate a proper z-vector assuming our right-handed coordinate system
  Vector3 zv;
  Vector3::Cross_Product(CameraTransform.Get_X_Vector(),
                         CameraTransform.Get_Y_Vector(), &zv);

  // compare correct z-vector with the one in the camera matrix.  If they point
  // in opposite directions, we have a reflected camera matrix.
  if (Vector3::Dot_Product(CameraTransform.Get_Z_Vector(), zv) <
      0) {  // flip the frustum corners horizontally for a reflected matrix
  ...
  // Transform the eight corners of the view frustum from camera space to world
  // space.
  for (i = 0; i < 8; i++) {
  ...
  // find the bounding box of the entire frustum (may be used for sloppy quick
  // rejection)
