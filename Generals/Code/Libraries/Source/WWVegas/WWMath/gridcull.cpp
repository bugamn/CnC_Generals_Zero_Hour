 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * GridCullSystemClass::GridCullSystemClass -- Constructor *
 *   GridCullSystemClass::~GridCullSystemClass -- Destructor *
 *   GridCullSystemClass::Collect_Objects -- Collect all objects touching the
 *given point      * GridCullSystemClass::Collect_Objects -- Collect all objects
 *touching the given AABox      * GridCullSystemClass::Collect_Objects --
 *Collect all objects touching the given OBBox      *
 *   GridCullSystemClass::Collect_Objects -- Collect all objects touching the
 *given Frustum    * GridCullSystemClass::Re_Partition -- re-compute grid
 *parameters for the given volume      *
 *   GridCullSystemClass::Collect_And_Unlink_All -- collects all objects and
 *removes them from * GridCullSystemClass::Update_Culling -- updates an objects
 *position in the grid            * GridCullSystemClass::Load -- load function *
 *   GridCullSystemClass::Save -- Save function *
 *   GridCullSystemClass::Reset_Statistics -- reset debugging stats *
 *   GridCullSystemClass::Get_Statistics -- returns reference to the statistics
 *structure      * GridCullSystemClass::Add_Object_Internal -- links an object
 *into the system               * GridCullSystemClass::Remove_Object_Internal --
 *unlinks an object from the system          * GridCullSystemClass::link_object
 *-- figures out which cell the object is in and links it  *
 *   GridCullSystemClass::unlink_object -- unlinks the object from the cell it
 *is in           * GridCullSystemClass::link_object_to_list -- grid list link
 *function                       * GridCullSystemClass::unlink_object_from_list
 *-- grid list unlink function                 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
GridLinkClass::GridLinkClass(GridCullSystemClass *system)
    : CullLinkClass(system), GridAddress(-1), Prev(NULL), Next(NULL) {}
...
/***********************************************************************************************
 * GridCullSystemClass::Re_Partition -- re-compute grid parameters for the given
 *volume        *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 * INPUT: * input_min - 'min' corner of the world * input_max - 'max' corner of
 *the world                                                       * objdim -
 *largest object dimesion that will be allowed in the grid *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: *
 ...
 * HISTORY: * 4/27/2000  gth : Created. *
 *=============================================================================================*/
void GridCullSystemClass::Re_Partition(const Vector3 &input_min,
                                       const Vector3 &input_max, float objdim) {
  /*
  ...
  */
  ...
  while (!done) {
    ...
    */
    int bigdim = 0;
    if (CellDim[1] / MinCellSize[1] > CellDim[bigdim] / MinCellSize[bigdim])
      bigdim = 1;
    if (CellDim[2] / MinCellSize[2] > CellDim[bigdim] / MinCellSize[bigdim])
      bigdim = 2;
  ...
  */
  CullableClass *obj;
  for (obj = Get_First_Collected_Object_Internal(); obj != NULL;
 ...
 *removes them from t *
 *                                                                                             *
 * This is used when re-partitioning the grid.  All objects are pulled out and
 *linked into     * the collection list.  When the grid has been re-initialized,
 *the objects are put back in.   *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
...
/***********************************************************************************************
 * GridCullSystemClass::Update_Culling -- updates an objects position in the
 *grid              *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * GridCullSystemClass::Remove_Object_Internal -- unlinks an object from the
 *system            *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * GridCullSystemClass::link_object -- figures out which cell the object is in
 *and links it    *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void GridCullSystemClass::link_object(CullableClass *obj, int address) {
  ...
  */
  ...
  if ((box.Extent.X > MaxObjExtent) || (box.Extent.Y > MaxObjExtent) ||
...
/***********************************************************************************************
 * GridCullSystemClass::unlink_object -- unlinks the object from the cell it is
 *in             *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void GridCullSystemClass::collect_objects_in_leaf(const AABoxClass &box,
                                                  CullableClass *head) {
  if (head != NULL) {
    GridListIterator it(head);
    for (; !it.Is_Done(); it.Next()) {
...
void GridCullSystemClass::collect_objects_in_leaf(const OBBoxClass &obbox,
  ...
  if (head != NULL) {
    ...
    for (; !it.Is_Done(); it.Next()) {
      CullableClass *obj = it.Peek_Obj();
      if (CollisionMath::Overlap_Test(obbox, obj->Get_Cull_Box()) !=
          CollisionMath::OUTSIDE) {
        Add_To_Collection(obj);
      }
    }
