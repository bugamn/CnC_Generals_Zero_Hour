 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * aabtri_separation_test -- test the projected extents for
 *separation                       * aabtri_check_axis -- project the aab and
 *tri onto an arbitrary axis                       * aabtri_check_cross_axis --
 *projects aab and tri onto a "cross" axis                       *
 ...
 *   aabtri_intersect_cross_axis -- intersection check for a "cross-product"
 *axis              * aabtri_intersect_basis_axis -- intersection check for a
 *basis axis                        * aabtri_intersect_normal_axis --
 *intersection check for the triangle normal                *
 *   CollisionMath::Intersection_Test -- Intersection check for an AABox and a
 *triangle        *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
/******************************************************************************************
   ...
   This code is basically a special-case optimization of the OBBox->Triangle
collision detection code.  There are many dot and cross products that can be
simplified due to the fact that we know the axes of the boxes are always the
same and are aligned with the world coordinate axes.
...
the projection of the extent of the box + the projection of the extent of the
        ...
        the BTCollisionStruct.  The compiler was generating lots of extra code
for the constructor of this object and testing determined that re-using the same
static struct was slightly faster anyway. NOTE: this makes the code not
Thread-Safe!!!!
...
/*
** BoxTriColStruct
** Scratchpad variables for the AABox-Triangle collision detection functions.
*One instance
...
struct BTCollisionStruct {
  ...
  void Init(const AABoxClass &box, const Vector3 &move, const TriClass &tri,
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 4/8/99     GTH : Created. * 7/12/99    GTH : Converted original
 *OBBox code to AABox                                   *
 *=============================================================================================*/
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 4/8/99     GTH : Created. * 7/12/99    GTH : converted to AABox *
 *=============================================================================================*/
static inline bool aabtri_check_axis(void) {
  ...
  axismove =
      Vector3::Dot_Product(CollisionContext.Move, CollisionContext.TestAxis);
  ...
  leb0 = CollisionContext.Box->Extent.X *
             WWMath::Fabs(CollisionContext.TestAxis.X) +
         CollisionContext.Box->Extent.Y *
             WWMath::Fabs(CollisionContext.TestAxis.Y) +
         CollisionContext.Box->Extent.Z *
             WWMath::Fabs(CollisionContext.TestAxis.Z);
  leb1 = leb0 + axismove;
...
inline void VERIFY_CROSS(const Vector3 &a, const Vector3 &b,
                         const Vector3 &cross) {
#ifdef WWDEBUG
  Vector3 tmp_cross;
 ...
 * HISTORY: * 4/8/99     GTH : Created. *
 *=============================================================================================*/
bool CollisionMath::Collide(const AABoxClass &box, const Vector3 &move,
                            const TriClass &tri, CastResultStruct *result) {
  ...
  */
  ...
  if (aabtri_check_basis_axis(box.Extent.X, CollisionContext.AE[0][0],
                              CollisionContext.AE[0][1]))
    goto exit;
  ...
  */
  ...
  if (aabtri_check_basis_axis(box.Extent.Y, CollisionContext.AE[1][0],
                              CollisionContext.AE[1][1]))
    goto exit;
  ...
  */
  ...
  if (aabtri_check_basis_axis(box.Extent.Z, CollisionContext.AE[2][0],
                              CollisionContext.AE[2][1]))
    goto exit;

  /*
  ** AXIS_A0xE0
  */
  CollisionContext.AxE[0][0].Set(0, -CollisionContext.E[0].Z,
                                 CollisionContext.E[0].Y);
  VERIFY_CROSS(Vector3(1, 0, 0), CollisionContext.E[0],
               CollisionContext.AxE[0][0]);
  ...
  if (CollisionContext.TestAxis.Length2() > AXISLEN_EPSILON2) {
    ...
    leb0 = box.Extent[1] * WWMath::Fabs(CollisionContext.AE[2][0]) +
           box.Extent[2] * WWMath::Fabs(CollisionContext.AE[1][0]);
  ...
  ** AXIS_A0xE1
  */
  CollisionContext.AxE[0][1].Set(0, -CollisionContext.E[1].Z,
                                 CollisionContext.E[1].Y);
  VERIFY_CROSS(Vector3(1, 0, 0), CollisionContext.E[1],
               CollisionContext.AxE[0][1]);
  CollisionContext.TestAxis = CollisionContext.AxE[0][1];
  CollisionContext.TestAxisId = AXIS_A0E1;
  if (CollisionContext.TestAxis.Length2() > AXISLEN_EPSILON2) {
    dp = -CollisionContext.AN[0];
    leb0 = box.Extent[1] * WWMath::Fabs(CollisionContext.AE[2][1]) +
           box.Extent[2] * WWMath::Fabs(CollisionContext.AE[1][1]);
  ...
  */
  ...
  CollisionContext.AxE[0][2].Set(0, -CollisionContext.E[2].Z,
                                 CollisionContext.E[2].Y);
  VERIFY_CROSS(Vector3(1, 0, 0), CollisionContext.E[2],
               CollisionContext.AxE[0][2]);
  CollisionContext.TestAxis = CollisionContext.AxE[0][2];
  CollisionContext.TestAxisId = AXIS_A0E2;

  if (CollisionContext.TestAxis.Length2() > AXISLEN_EPSILON2) {
    dp = -CollisionContext.AN[0];
    leb0 = box.Extent[1] * WWMath::Fabs(CollisionContext.AE[2][2]) +
           box.Extent[2] * WWMath::Fabs(CollisionContext.AE[1][2]);
    if (aabtri_check_cross_axis(dp, 1, leb0)) goto exit;
  }
  ...
  ** AXIS_A1xE0
  */
  CollisionContext.AxE[1][0].Set(CollisionContext.E[0].Z, 0,
                                 -CollisionContext.E[0].X);
  VERIFY_CROSS(Vector3(0, 1, 0), CollisionContext.E[0],
               CollisionContext.AxE[1][0]);
  CollisionContext.TestAxis = CollisionContext.AxE[1][0];
  CollisionContext.TestAxisId = AXIS_A1E0;
  if (CollisionContext.TestAxis.Length2() > AXISLEN_EPSILON2) {
    dp = CollisionContext.AN[1];
    leb0 = box.Extent[0] * WWMath::Fabs(CollisionContext.AE[2][0]) +
           box.Extent[2] * WWMath::Fabs(CollisionContext.AE[0][0]);
    if (aabtri_check_cross_axis(dp, 2, leb0)) goto exit;
  }
  ...
  ** AXIS_A1xE1
  */
  CollisionContext.AxE[1][1].Set(CollisionContext.E[1].Z, 0,
                                 -CollisionContext.E[1].X);
  VERIFY_CROSS(Vector3(0, 1, 0), CollisionContext.E[1],
               CollisionContext.AxE[1][1]);
  CollisionContext.TestAxis = CollisionContext.AxE[1][1];
  CollisionContext.TestAxisId = AXIS_A1E1;
  if (CollisionContext.TestAxis.Length2() > AXISLEN_EPSILON2) {
    dp = -CollisionContext.AN[1];
    leb0 = box.Extent[0] * WWMath::Fabs(CollisionContext.AE[2][1]) +
           box.Extent[2] * WWMath::Fabs(CollisionContext.AE[0][1]);
    if (aabtri_check_cross_axis(dp, 1, leb0)) goto exit;
  }
  ...
  ** AXIS_A1xE2
  */
  CollisionContext.AxE[1][2].Set(CollisionContext.E[2].Z, 0,
                                 -CollisionContext.E[2].X);
  VERIFY_CROSS(Vector3(0, 1, 0), CollisionContext.E[2],
               CollisionContext.AxE[1][2]);
  CollisionContext.TestAxis = CollisionContext.AxE[1][2];
  CollisionContext.TestAxisId = AXIS_A1E2;
  if (CollisionContext.TestAxis.Length2() > AXISLEN_EPSILON2) {
    dp = -CollisionContext.AN[1];
    leb0 = box.Extent[0] * WWMath::Fabs(CollisionContext.AE[2][2]) +
           box.Extent[2] * WWMath::Fabs(CollisionContext.AE[0][2]);
    if (aabtri_check_cross_axis(dp, 1, leb0)) goto exit;
  }
  ...
  ** AXIS_A2xE0
  */
  CollisionContext.AxE[2][0].Set(-CollisionContext.E[0].Y,
                                 CollisionContext.E[0].X, 0);
  VERIFY_CROSS(Vector3(0, 0, 1), CollisionContext.E[0],
  ...
  if (CollisionContext.TestAxis.Length2() > AXISLEN_EPSILON2) {
    ...
    leb0 = box.Extent[0] * WWMath::Fabs(CollisionContext.AE[1][0]) +
           box.Extent[1] * WWMath::Fabs(CollisionContext.AE[0][0]);
    if (aabtri_check_cross_axis(dp, 2, leb0)) goto exit;
  }
  ...
  ** AXIS_A2xE1
  */
  CollisionContext.AxE[2][1].Set(-CollisionContext.E[1].Y,
                                 CollisionContext.E[1].X, 0);
  VERIFY_CROSS(Vector3(0, 0, 1), CollisionContext.E[1],
               CollisionContext.AxE[2][1]);
  CollisionContext.TestAxis = CollisionContext.AxE[2][1];
  CollisionContext.TestAxisId = AXIS_A2E1;
  if (CollisionContext.TestAxis.Length2() > AXISLEN_EPSILON2) {
    dp = -CollisionContext.AN[2];
    leb0 = box.Extent[0] * WWMath::Fabs(CollisionContext.AE[1][1]) +
           box.Extent[1] * WWMath::Fabs(CollisionContext.AE[0][1]);
    if (aabtri_check_cross_axis(dp, 1, leb0)) goto exit;
  }
  ...
  ** AXIS_A2xE2
  */
  CollisionContext.AxE[2][2].Set(-CollisionContext.E[2].Y,
                                 CollisionContext.E[2].X, 0);
  VERIFY_CROSS(Vector3(0, 0, 1), CollisionContext.E[2],
               CollisionContext.AxE[2][2]);
  CollisionContext.TestAxis = CollisionContext.AxE[2][2];
  CollisionContext.TestAxisId = AXIS_A2E2;
  if (CollisionContext.TestAxis.Length2() > AXISLEN_EPSILON2) {
    dp = -CollisionContext.AN[2];
    leb0 = box.Extent[0] * WWMath::Fabs(CollisionContext.AE[1][2]) +
           box.Extent[1] * WWMath::Fabs(CollisionContext.AE[0][2]);
    if (aabtri_check_cross_axis(dp, 1, leb0)) goto exit;
  }
  ...
  */
  if (!CollisionContext.StartBad) {
    ...
    CollisionContext.TestAxis.Set(0, -CollisionContext.Move.Z,
                                  CollisionContext.Move.Y);  // A0 X Move
    VERIFY_CROSS(Vector3(1, 0, 0), CollisionContext.Move,
                 CollisionContext.TestAxis);
    if (CollisionContext.TestAxis.Length2() > AXISLEN_EPSILON2) {
      if (aabtri_check_axis()) goto exit;
    }
    CollisionContext.TestAxis.Set(CollisionContext.Move.Z, 0,
                                  -CollisionContext.Move.X);  // A1 X Move
    VERIFY_CROSS(Vector3(0, 1, 0), CollisionContext.Move,
                 CollisionContext.TestAxis);
    if (CollisionContext.TestAxis.Length2() > AXISLEN_EPSILON2) {
      if (aabtri_check_axis()) goto exit;
    }
    CollisionContext.TestAxis.Set(-CollisionContext.Move.Y,
                                  CollisionContext.Move.X, 0);  // A2 X Move
    VERIFY_CROSS(Vector3(0, 0, 1), CollisionContext.Move,
                 CollisionContext.TestAxis);
    if (CollisionContext.TestAxis.Length2() > AXISLEN_EPSILON2) {
      if (aabtri_check_axis()) goto exit;
    }
...
exit:
  ...
  ** move.
  */
  if ((CollisionContext.MaxFrac <= result->Fraction) &&
      (CollisionContext.MaxFrac < 1.0f)) {
    /*
    ...
    */
    if ((WWMath::Fabs(CollisionContext.MaxFrac - result->Fraction) >
         WWMATH_EPSILON) ||
        (Vector3::Dot_Product(tmp_norm, move) <
         Vector3::Dot_Product(result->Normal, move))) {
      result->Normal = tmp_norm;
#pragma message("fatal assert disabled for demo")
      // WWASSERT(WWMath::Fabs(result->Normal.Length() - 1.0f) <
      // WWMATH_EPSILON);
    }
...
/*
** AABTIntersectStruct
** Scratchpad variables for the AABox-Triangle intersection functions.  One
*instance
** of this structure will be used for all of the local variables and its pointer
*will be
** handed of to various inline functions for the axis tests.
 ...
 *=============================================================================================*/
static inline bool aabtri_intersect_cross_axis(Vector3 &axis, float dp,
                                               float leb0) {
  float p0;  // distance from box center to vertex 0
  float lp;  // leading edge of the polygon.
 ...
 * HISTORY: * 5/4/99     GTH : Created. *
 *=============================================================================================*/
static inline bool aabtri_intersect_basis_axis(Vector3 &axis, float leb0,
                                               float dp1, float dp2) {
  float dist;  // separation along the axis
  float lp;    // leading edge of the polygon.
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 1/20/00    gth : copied from OBBox version and removed some
 *Dot-products                  *
 *=============================================================================================*/
bool CollisionMath::Intersection_Test(const AABoxClass &box,
  ...
  */
  ...
  if (aabtri_intersect_basis_axis(axis, box.Extent.X, IntersectContext.AE[0][0],
                                  IntersectContext.AE[0][1]))
    return false;
  ...
  */
  ...
  if (aabtri_intersect_basis_axis(axis, box.Extent.Y, IntersectContext.AE[1][0],
                                  IntersectContext.AE[1][1]))
    return false;
  ...
  */
  ...
  if (aabtri_intersect_basis_axis(axis, box.Extent.Z, IntersectContext.AE[2][0],
                                  IntersectContext.AE[2][1]))
    return false;

  /*
  ** AXIS_A0xE0
  */
  Vector3::Cross_Product(Vector3(1, 0, 0), IntersectContext.E[0],
                         &IntersectContext.AxE[0][0]);
  axis = IntersectContext.AxE[0][0];
  if (axis.Length2() > AXISLEN_EPSILON2) {
    dp = IntersectContext.AN[0];
    leb0 = box.Extent[1] * WWMath::Fabs(IntersectContext.AE[2][0]) +
           box.Extent[2] * WWMath::Fabs(IntersectContext.AE[1][0]);
  ...
  ** AXIS_A0xE1
  */
  Vector3::Cross_Product(Vector3(1, 0, 0), IntersectContext.E[1],
                         &IntersectContext.AxE[0][1]);
  axis = IntersectContext.AxE[0][1];
  if (axis.Length2() > AXISLEN_EPSILON2) {
    dp = -IntersectContext.AN[0];
    leb0 = box.Extent[1] * WWMath::Fabs(IntersectContext.AE[2][1]) +
           box.Extent[2] * WWMath::Fabs(IntersectContext.AE[1][1]);
    if (aabtri_intersect_cross_axis(axis, dp, leb0)) return false;
  }
  ...
  ** AXIS_A0xE2
  */
  Vector3::Cross_Product(Vector3(1, 0, 0), IntersectContext.E[2],
                         &IntersectContext.AxE[0][2]);
  ...
  if (axis.Length2() > AXISLEN_EPSILON2) {
    dp = -IntersectContext.AN[0];
    leb0 = box.Extent[1] * WWMath::Fabs(IntersectContext.AE[2][2]) +
           box.Extent[2] * WWMath::Fabs(IntersectContext.AE[1][2]);
    if (aabtri_intersect_cross_axis(axis, dp, leb0)) return false;
  }
  ...
  ** AXIS_A1xE0
  */
  Vector3::Cross_Product(Vector3(0, 1, 0), IntersectContext.E[0],
                         &IntersectContext.AxE[1][0]);
  axis = IntersectContext.AxE[1][0];
  if (axis.Length2() > AXISLEN_EPSILON2) {
    dp = IntersectContext.AN[1];
    leb0 = box.Extent[0] * WWMath::Fabs(IntersectContext.AE[2][0]) +
           box.Extent[2] * WWMath::Fabs(IntersectContext.AE[0][0]);
    if (aabtri_intersect_cross_axis(axis, dp, leb0)) return false;
  }
  ...
  ** AXIS_A1xE1
  */
  Vector3::Cross_Product(Vector3(0, 1, 0), IntersectContext.E[1],
                         &IntersectContext.AxE[1][1]);
  axis = IntersectContext.AxE[1][1];
  if (axis.Length2() > AXISLEN_EPSILON2) {
    dp = -IntersectContext.AN[1];
    leb0 = box.Extent[0] * WWMath::Fabs(IntersectContext.AE[2][1]) +
           box.Extent[2] * WWMath::Fabs(IntersectContext.AE[0][1]);
    if (aabtri_intersect_cross_axis(axis, dp, leb0)) return false;
  }
  ...
  ** AXIS_A1xE2
  */
  Vector3::Cross_Product(Vector3(0, 1, 0), IntersectContext.E[2],
                         &IntersectContext.AxE[1][2]);
  axis = IntersectContext.AxE[1][2];
  IntersectContext.AE[0][2] = IntersectContext.E[2].X;
  if (axis.Length2() > AXISLEN_EPSILON2) {
    dp = -IntersectContext.AN[1];
    leb0 = box.Extent[0] * WWMath::Fabs(IntersectContext.AE[2][2]) +
           box.Extent[2] * WWMath::Fabs(IntersectContext.AE[0][2]);
    if (aabtri_intersect_cross_axis(axis, dp, leb0)) return false;
  }
  ...
  ** AXIS_A2xE0
  */
  Vector3::Cross_Product(Vector3(0, 0, 1), IntersectContext.E[0],
                         &IntersectContext.AxE[2][0]);
  axis = IntersectContext.AxE[2][0];
  if (axis.Length2() > AXISLEN_EPSILON2) {
    dp = IntersectContext.AN[2];
    leb0 = box.Extent[0] * WWMath::Fabs(IntersectContext.AE[1][0]) +
           box.Extent[1] * WWMath::Fabs(IntersectContext.AE[0][0]);
    if (aabtri_intersect_cross_axis(axis, dp, leb0)) return false;
  }
  ...
  ** AXIS_A2xE1
  */
  Vector3::Cross_Product(Vector3(0, 0, 1), IntersectContext.E[1],
                         &IntersectContext.AxE[2][1]);
  axis = IntersectContext.AxE[2][1];
  if (axis.Length2() > AXISLEN_EPSILON2) {
    dp = -IntersectContext.AN[2];
    leb0 = box.Extent[0] * WWMath::Fabs(IntersectContext.AE[1][1]) +
           box.Extent[1] * WWMath::Fabs(IntersectContext.AE[0][1]);
    if (aabtri_intersect_cross_axis(axis, dp, leb0)) return false;
  }
  ...
  ** AXIS_A2xE2
  */
  Vector3::Cross_Product(Vector3(0, 0, 1), IntersectContext.E[2],
                         &IntersectContext.AxE[2][2]);
  axis = IntersectContext.AxE[2][2];
  if (axis.Length2() > AXISLEN_EPSILON2) {
    dp = -IntersectContext.AN[2];
    leb0 = box.Extent[0] * WWMath::Fabs(IntersectContext.AE[1][2]) +
           box.Extent[1] * WWMath::Fabs(IntersectContext.AE[0][2]);
    if (aabtri_intersect_cross_axis(axis, dp, leb0)) return false;
  }
