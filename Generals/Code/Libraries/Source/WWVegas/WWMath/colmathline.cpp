...
/*
** Structure used in the line->box test.  There was a lot of common code between
*the axis-
** aligned and oriented box tests so I package all of the truely relevant
*information into
** this struct and pass it into a function local to this module.  In the case of
*oriented
** boxes, the ray must be transformed into the box's coordinate system prior to
*the call
...
/*
** Enumeration which can be used to categorize a point with respect to the
*projection
** of a box onto an axis.  The point will either be to the negative side of the
*span, in the
...
bool CollisionMath::Collide(const LineSegClass& line, const AAPlaneClass& plane,
...
bool CollisionMath::Collide(const LineSegClass& line, const PlaneClass& plane,
                            CastResultStruct* result) {
  float num, den, t;
...
bool CollisionMath::Collide(const LineSegClass& line, const TriClass& tri,
...
bool CollisionMath::Collide(const LineSegClass& line, const SphereClass& sphere,
                            CastResultStruct* res) {
  ...
  if (disc < 0.0f) {
    ...
    if (frac < 0.0f) frac = (clen + d) / line.Get_Length();
...
bool CollisionMath::Collide(const LineSegClass& line, const AABoxClass& box,
                            CastResultStruct* res) {
...
bool CollisionMath::Collide(const LineSegClass& line, const OBBoxClass& box,
                            CastResultStruct* result) {
  // set up the test struct
  BoxTestStruct test;
  test.Min = box.Center - box.Extent;
