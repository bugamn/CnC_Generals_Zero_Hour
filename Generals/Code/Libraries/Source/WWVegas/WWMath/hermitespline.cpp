...
** Save-Load stuff
*/
SimplePersistFactoryClass<HermiteSpline3DClass, WWMATH_CHUNKID_HERMITESPLINE3D>
    _HermiteSpline3DFactory;
SimplePersistFactoryClass<HermiteSpline1DClass, WWMATH_CHUNKID_HERMITESPLINE1D>
    _HermiteSpline1DFactory;
...
** Hermite Spline
*/
const HermiteSpline3DClass &HermiteSpline3DClass::operator=(
    const HermiteSpline3DClass &that) {
  //
...
void HermiteSpline3DClass::Evaluate(float time, Vector3 *set_val) {
  ...
  // if the tangents are marked dirty, give derived classes a chance to
  // recompute them
  if (TangentsDirty) {
    Update_Tangents();
  }
...
void HermiteSpline3DClass::Set_Tangents(int i, const Vector3 &in_tan,
...
void HermiteSpline3DClass::Get_Tangents(int i, Vector3 *set_in,
                                        Vector3 *set_out) {
  assert(i >= 0);
  assert(i < Keys.Count());
