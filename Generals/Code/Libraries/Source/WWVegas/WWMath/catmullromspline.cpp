 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * CatmullRomSpline3DClass::Update_Tangents -- computes the
 *tangents at each key             * CatmullRomSpline3DClass::Get_Factory --
 *returns the factory for CatmullRomSpline3D        *
 *   CatmullRomSpline3DClass::Save -- save this curve *
 *   CatmullRomSpline3DClass::Load -- load this curve *
 *   CatmullRomSpline1DClass::Update_Tangents -- Computes the tangents at each
 *key             * CatmullRomSpline1DClass::Get_Factory -- returns the factory
 *for CatmullRomSpline1D        * CatmullRomSpline1DClass::Save -- Save this
 *curve                                          * CatmullRomSpline1DClass::Load
 *-- Load this curve                                          *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
** Save-Load stuff
*/
SimplePersistFactoryClass<CatmullRomSpline3DClass,
                          WWMATH_CHUNKID_CATMULLROMSPLINE3D>
    _CatmullRomSpline3DFactory;
SimplePersistFactoryClass<CatmullRomSpline1DClass,
 ...
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * I'm not sure about the tangents for the endpoints of the curve. *
 *                                                                                             *
 * HISTORY: * 3/7/2000   gth : Created. *
 *=============================================================================================*/
void CatmullRomSpline3DClass::Update_Tangents(void) {
  ...
  if (IsLooping) {
    ...
    Tangents[end].InTangent.X =
        0.25f * (Keys[end].Point.X - Keys[end - 1].Point.X);
    Tangents[end].InTangent.Y =
        0.25f * (Keys[end].Point.Y - Keys[end - 1].Point.Y);
    Tangents[end].InTangent.Z =
        0.25f * (Keys[end].Point.Z - Keys[end - 1].Point.Z);
  }
  ...
  for (int i = 1; i < Keys.Count() - 1; i++) {
    Tangents[i].InTangent.X =
        0.5f * (Keys[i + 1].Point.X - Keys[i - 1].Point.X);
    Tangents[i].InTangent.Y =
        0.5f * (Keys[i + 1].Point.Y - Keys[i - 1].Point.Y);
    Tangents[i].InTangent.Z =
        0.5f * (Keys[i + 1].Point.Z - Keys[i - 1].Point.Z);
...
/***********************************************************************************************
 * CatmullRomSpline3DClass::Get_Factory -- returns the factory for
 *CatmullRomSpline3D          *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * CatmullRomSpline1DClass::Get_Factory -- returns the factory for
 *CatmullRomSpline1D          *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
