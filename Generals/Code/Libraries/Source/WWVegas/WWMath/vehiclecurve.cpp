...
// Save-Load stuff
//////////////////////////////////////////////////////////////////////
SimplePersistFactoryClass<VehicleCurveClass, WWMATH_CHUNKID_VEHICLECURVE>
...
//	Local prototypes
//////////////////////////////////////////////////////////////////////
bool Find_Tangent(const Vector3 &center, float radius, const Vector3 &point,
                  bool clockwise, float *result);
float Get_Angle_Delta(float angle1, float angle2, bool clockwise);
void Find_Turn_Arc(const Matrix3D &transform, float radius,
                   const Vector3 &prev_pt, const Vector3 &curr_pt,
                   const Vector3 &next_pt, Vector3 *arc_center,
                   bool *is_right_turn);
void Find_Tangents(float radius, const Vector3 &prev_pt, const Vector3 &curr_pt,
                   const Vector3 &next_pt, const Vector3 &arc_center,
                   bool is_right_turn, float *point_angle,
                   float *angle_in_delta, float *angle_out_delta);
...
void Find_Turn_Arc(const Matrix3D &transform, float radius,
  ...
  //
  float angle1 =
      ::WWMath::Atan2((prev_pt.Y - curr_pt.Y), prev_pt.X - curr_pt.X);
  angle1 = WWMath::Wrap(angle1, 0, DEG_TO_RADF(360));

  float angle2 =
      ::WWMath::Atan2((next_pt.Y - curr_pt.Y), next_pt.X - curr_pt.X);
...
void Find_Tangents(float radius, const Vector3 &prev_pt, const Vector3 &curr_pt,
  ...
  //
  (*point_angle) =
      ::WWMath::Atan2(curr_pt.X - arc_center.X, -(curr_pt.Y - arc_center.Y));
  ...
  if (valid_in) {
    (*angle_in_delta) =
        ::Get_Angle_Delta(angle_in, (*point_angle), is_right_turn);
  } else {
    (*angle_in_delta) = 0;
  }
...
void VehicleCurveClass::Update_Arc_List(void) {
  ...
  for (int index = 1; index < count - 1; index++) {
    ...
    ::Find_Turn_Arc(tm, m_Radius, last_path_pt, curr_pt, next_pt, &arc_center,
                    &is_right_turn);
    ...
    float point_angle = 0;
    ::Find_Tangents(m_Radius, last_path_pt, curr_pt, next_pt, arc_center,
                    is_right_turn, &point_angle, &angle_in_delta,
                    &angle_out_delta);
    ...
    point_in.X =
        arc_center.X + (m_Radius * ::WWMath::Sin(point_angle + angle_in_delta));
    point_in.Y = arc_center.Y +
                 (m_Radius * -::WWMath::Cos(point_angle + angle_in_delta));

    Vector3 point_out(0, 0, 0);
    point_out.X = arc_center.X +
                  (m_Radius * ::WWMath::Sin(point_angle + angle_out_delta));
    point_out.Y = arc_center.Y +
                  (m_Radius * -::WWMath::Cos(point_angle + angle_out_delta));

    //
    //	Sanity check to ensure the vehicle doesn't try to go the long way around
    //the
    // turn arc...
    //
    if (angle_in_delta > DEG_TO_RADF(200) ||
        angle_out_delta > DEG_TO_RADF(200) ||
        angle_in_delta < -DEG_TO_RADF(200) ||
        angle_out_delta < -DEG_TO_RADF(200)) {
      //
...
//
//////////////////////////////////////////////////////////////////////
void VehicleCurveClass::Evaluate(float time, Vector3 *set_val) {
  ...
  //
  float arc_length0 =
      arc_info0.radius * WWMath::Fabs(arc_info0.angle_out_delta);
  float arc_length1 = arc_info1.radius * WWMath::Fabs(arc_info1.angle_in_delta);
  float other_length =
      ((arc_info1.point_in - arc_info0.point_out).Length()) / 2;
  ...
  if (seg_time < time1) {
    ...
    set_val->Y =
        arc_info0.center.Y + (arc_info0.radius * -::WWMath::Cos(angle));

    m_Sharpness = WWMath::Clamp(
        WWMath::Fabs(arc_info0.angle_out_delta) / DEG_TO_RADF(15), 0, 1.0F);
    m_SharpnessPos.X = set_val->X;
    m_SharpnessPos.Y = set_val->Y;
    m_SharpnessPos.Z = Keys[index0].Point.Z +
                       (Keys[index1].Point.Z - Keys[index0].Point.Z) * seg_time;

    m_LastTime =
        Keys[index0].Time + (Keys[index1].Time - Keys[index0].Time) * time1;
    ...
    // set_val->X = arc_info0.point_out.X + (arc_info1.point_in.X -
    // arc_info0.point_out.X) * percent; set_val->Y = arc_info0.point_out.Y +
    // (arc_info1.point_in.Y - arc_info0.point_out.Y) * percent;

    m_Sharpness = WWMath::Clamp(
        WWMath::Fabs(arc_info1.angle_out_delta) / DEG_TO_RADF(15), 0, 1.0F);
    m_SharpnessPos = arc_info1.point_in;

    m_LastTime =
        Keys[index0].Time + (Keys[index1].Time - Keys[index0].Time) * time2;
    ...
    set_val->X = arc_info1.center.X + (arc_info1.radius * ::WWMath::Sin
    (angle)); set_val->Y = arc_info1.center.Y + (arc_info1.radius *
    -::WWMath::Cos (angle));			*/
    ...
    set_val->Y =
        arc_info1.center.Y + (arc_info1.radius * -::WWMath::Cos(angle));
...
//
////////////////////////////////////////////////////////////////////////////////////////////
bool VehicleCurveClass::Save(ChunkSaveClass &csave) {
...
//
////////////////////////////////////////////////////////////////////////////////////////////
bool VehicleCurveClass::Load(ChunkLoadClass &cload) {
...
//
///////////////////////////////////////////////////////////////////////
void VehicleCurveClass::Load_Variables(ChunkLoadClass &cload) {
  //
