 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * CollisionMath::Intersection_Test -- Test intersection between
 *two AABoxes                 * CollisionMath::Overlap_Test -- Tests overlap
 *between an AABox and a sphere                * CollisionMath::Overlap_Test --
 *Tests overlap between an AABox and a triangle              *
 *   CollisionMath::Overlap_Test -- Tests overlap between an AABox and a line
 *segment          * CollisionMath::Collide -- Collision test for a moving AABox
 *and a plane                   * aab_separation_test -- tests two AAB's for
 *separation on an axis                          * CollisionMath::Collide --
 *Collision test for two moving AABoxes                           *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
CollisionMath::OverlapType CollisionMath::Overlap_Test(
    const AABoxClass &box, const SphereClass &sphere) {
 ...
 *segment            *
 *                                                                                             *
 * This function uses separating axes to determine whether an AABox and a line
 *segment overlap * I wrote it when we found that ray-casting was taking a huge
 *amount of time in Renegade.     * Here are some statistics comparing this
 *function to the previous function which used the    * same algorithm that
 *Collide(box,ray) uses.                                                  *
 *                                                                                             *
 *                  collide algorithm (Gems)                separating axis
 *(this one)         *
 * -----------------------------------------------------------------------------------------
 ** number of tests      10000                                  10000 * avg
 *cycles             641                                    464 * outside cycles
 *652                                    390                           * overlap
 *cycles         610                                    683 *
 *                                                                                             *
 * The idea for this test is that it will early-exit when it can while the old
 *test can't.     * When we are passing a ray down our culling trees, it is
 *common to encounter boxes that      * are not intersecting the ray.  The
 *faster we can reject these, the better!                  *
 *                                                                                             *
 * INPUT: * box - box to test * line - line to test *
 *                                                                                             *
 * OUTPUT: * overlap status of the line with respect to the box, either INSIDE,
 *OUTSIDE, or OVERLAPPED   *
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 11/19/99   gth : Created. *
 *=============================================================================================*/
CollisionMath::OverlapType CollisionMath::Overlap_Test(
    const AABoxClass &box, const LineSegClass &line) {
  // If both endpoints are in, return INSIDE
  // If either was inside, return OVERLAPPED
  int inside_point_count = 0;
  if (CollisionMath::Overlap_Test(box, line.Get_P0()) == INSIDE)
    inside_point_count++;
  if (CollisionMath::Overlap_Test(box, line.Get_P1()) == INSIDE)
    inside_point_count++;
  ...
  if (inside_point_count == 2) {
    ...
    // -----------------------------------------------
    // box_proj = fabs(extent.X*axis.X) + fabs(extent.Y*axis.Y) +
    // fabs(extent.Z*axis.Z) p0_proj = fabs(dot_product(dp0,axis)) dp_proj =
    // fabs(dot_product(dp,axis)) if (p0_proj > 0) { 	if (p0_proj > box_proj -
    // WWMath::Min(dp_proj,0.0f)) return OUTSIDE; } else { 	if (-p0_proj >
    // box_proj + WWMath::Max(dp_proj,0.0f)) return OUTSIDE;
    // }
    //
    // In practice, there are optimizations you can make on each of the axes
    // that we need to test (see below).
    ...
    // I'm manually optimizing the cross-product and taking advantage of the
    // fact that 'dp' will always project to zero for this axis.
    Vector3 axis;
    axis.Set(0, -line.Get_Dir().Z,
             line.Get_Dir().Y);  // == (1,0,0) cross (x,y,z)
    box_proj = WWMath::Fabs(axis.Y * box.Extent.Y) +
               WWMath::Fabs(axis.Z * box.Extent.Z);
    p0_proj = Vector3::Dot_Product(axis, dp0);
    if (WWMath::Fabs(p0_proj) > box_proj) return OUTSIDE;

    // Project box and line onto (y cross line)
    axis.Set(line.Get_Dir().Z, 0,
             -line.Get_Dir().X);  // == (0,1,0) cross (x,y,z)
    box_proj = WWMath::Fabs(axis.X * box.Extent.X) +
               WWMath::Fabs(axis.Z * box.Extent.Z);
    p0_proj = Vector3::Dot_Product(axis, dp0);
    if (WWMath::Fabs(p0_proj) > box_proj) return OUTSIDE;

    // Project box and line onto (z cross line)
    axis.Set(-line.Get_Dir().Y, line.Get_Dir().X,
             0);  // == (0,0,1) cross (x,y,z)
    box_proj = WWMath::Fabs(axis.X * box.Extent.X) +
               WWMath::Fabs(axis.Y * box.Extent.Y);
    p0_proj = Vector3::Dot_Product(axis, dp0);
    if (WWMath::Fabs(p0_proj) > box_proj) return OUTSIDE;
  }
  return OVERLAPPED;
 ...
 * HISTORY: * 11/19/99   gth : Created. *
 *=============================================================================================*/
bool CollisionMath::Collide(const AABoxClass &box, const Vector3 &move_vector,
                            const PlaneClass &plane, CastResultStruct *result) {
  float frac;
  ...
  if (frac < result->Fraction) {
    ...
    if (result->ComputeContactPoint) {
      ...
      result->ContactPoint =
          box.Center + result->Fraction * move_vector + move_extent * move_dir;
    }
    return true;
  }
...
struct AABCollisionStruct {
  AABCollisionStruct(const AABoxClass &box0, const Vector3 &move0,
 ...
 *                                                                                             *
 * This function is very similar to the obb_separation_test.  If a flaw is found
 *in either,    * we should update the other... *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 ...
 * HISTORY: * 11/19/99   gth : Created. *
 *=============================================================================================*/
static inline bool aab_separation_test(AABCollisionStruct &context, int axis) {
 ...
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * currently there is no parateter for the movement of the second
 *box.  the internal code      * can handle it though. *
 *                                                                                             *
 * HISTORY: * 11/19/99   gth : Created. *
 *=============================================================================================*/
bool CollisionMath::Collide(const AABoxClass &box, const Vector3 &move,
                            const AABoxClass &box2, CastResultStruct *result) {
  /*
  **	Test the X-axis
