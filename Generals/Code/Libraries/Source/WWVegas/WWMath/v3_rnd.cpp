...
void Vector3HollowSphereRandomizer::Get_Vector(Vector3 &vector) {
  // Generate vectors in a 2x2x2 origin-centered cube, discard the ones not in a
  // unit-radius sphere and scale the result to Radius.
  float v_l2;
  for (;;) {
...
Vector3SolidCylinderRandomizer::Vector3SolidCylinderRandomizer(float extent,
                                                               float radius) {
