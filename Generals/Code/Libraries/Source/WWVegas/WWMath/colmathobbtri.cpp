 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * obbtri_collision_separation_test -- test the projected extents
 *for separation             * obbtri_check_collision_axis -- project the obb
 *and tri onto an arbitrary axis             * obbtri_check_collision_cross_axis
 *-- intersection check for a "cross-product" axis        *
 *   obbtri_check_collision_basis_axis -- intersection check for a basis axis *
 *   obbtri_check_collision_normal_axis -- intersection check for the triangle
 *normal          * eval_side -- returns -1,0,+1 depending on the signe of val
 *and side                       * obbtri_compute_contact_normal -- computes the
 *normal of the collision                     * eval_A0_point -- contact point
 *parameters for A0xEx                                       * eval_A1_point --
 *contact point parameters for A1xEx                                       *
 ...
 *   obbtri_intersection_separation_test -- test the projected extents for
 *intersection        * obbtri_check_intersection_cross_axis -- intersection
 *check for a "cross-product" axis     * obbtri_check_intersection_basis_axis --
 *intersection check for a basis axis               *
 *   obbtri_check_intersection_normal_axis -- intersection check for the
 *triangle normal       * CollisionMath::Intersection_Test -- Intersection check
 *for an OBBox and a triangle        *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
/*
** Separating Axes have to be rejected if their length is smaller than some
*epsilon.
** Otherwise, erroneous results can be reported.
*/
#define AXISLEN_EPSILON2 \
  WWMATH_EPSILON *WWMATH_EPSILON  // squared length of a separating axis must be
                                  // larger than this
...
/******************************************************************************************
   ...
   As with most of the collision detection functions, this code is based on the
theorem that given any two non-intersecting convex polyhedra, a separating
plane/axis can be found that will be defined by one of the face normals of one
of the polyhedra or the cross product of an edge from each polyhedra.  The key
optimization for boxes is that many of these axes are the same.  The normal to a
face is the same as the axis for four of the edges of the box, etc.

        This test must be done using the tri normal, the three basis vectors
        for the box, and three vectors defined by the cross products of the
basis and edge vectors.  When testing the basis vectors, there will be two
extents for the tri (the other two vectors projected onto the axis being
tested). The appropriate extent must be used in the test (largest value which is
pointing towards the direction of the box). In addition, I've found that I must
test axes defined by the cross products of	the move vector and the box
axes.  These tests catch "false collisions" where the box passes very close to
the triangle but does not actually hit it.

        Each axis test will use the following logic:
        Project D onto the axis being used, it is the separation distance.  If
the projection of the extent of the box + the projection of the extent of the
...
/**
** BoxTriColStruct
** Scratchpad variables for the OBBox-Triangle collision detection functions.
*One instance
** of this structure will be used for all of the local variables and its pointer
*will be
** handed of to various inline functions for the axis tests.
** Note that much of the code needs the un-normalized triangle normal.  For this
*reason,
** I have to compute N rather than copying it from the triangle.  (commenting
*this to
** avoid re-generating a difficult to find bug that I had)
**
** Note that the MaxFrac variable starts negative so that we can accept slightly
*negative
** fractions (interpenetrations).  But we clamp the returned result to 0.0 so
*that we never
** allow an object to get more embedded, just to possibly break itself free if
*it is within
** the epsilon.  This is important because sometimes objects seem to intersect
*simply due to
** floating point roundoff error...
*/
struct BTCollisionStruct {
  BTCollisionStruct(const OBBoxClass &box, const Vector3 &move,
                    const TriClass &tri, const Vector3 &trimove)
      : Box(box), Tri(tri), BoxMove(move), TriMove(trimove) {
    Reset();
  }

  void Reset(void) {
    StartBad = true;  // true until an axis clears it
    MaxFrac = -1.0f;  // maximum move allowed so far (accept slightly negative
                      // but clamp to zero at end)
    ...
    Vector3::Subtract(*Tri.V[0], Box.Center,
                      &D);  // vector from center of box to vertex 0
    Vector3::Subtract(BoxMove, TriMove,
                      &Move);  // move vector relative to stationary triangle
 ...
 **
 *                                                                                             *
 * Once the extents are projected onto the axis, this function contains * the
 *logic that determines the allowed fraction. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: * true = objects are SEPARATED *
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 4/8/99     GTH : Created. *
 *=============================================================================================*/
static inline bool obbtri_collision_separation_test(BTCollisionStruct &context,
                                                    float lp, float leb0,
                                                    float leb1) {
  /*
  ...
  **     - fraction = How far I can move before embedding (can be negative if
  *started embedded)
  **     - If (I can take entire move)
  **       - accept entire move
  **     - Else If (I can move more than I could have before; *negative always
  *fails!)
  **       - update fraction
  **   - Else
  ...
  */
  ...
  if (lp - leb0 <= 0.0f) {
    eps = COLLISION_EPSILON *
          context.TestAxis
              .Length();  // trying to only compute epsilon if I have to
  }
...
static inline bool obbtri_check_collision_axis(BTCollisionStruct &context) {
  ...
  leb0 =
      context.Box.Extent.X *
          WWMath::Fabs(Vector3::Dot_Product(context.TestAxis, context.A[0])) +
      context.Box.Extent.Y *
          WWMath::Fabs(Vector3::Dot_Product(context.TestAxis, context.A[1])) +
      context.Box.Extent.Z *
          WWMath::Fabs(Vector3::Dot_Product(context.TestAxis, context.A[2]));
  leb1 = leb0 + axismove;

  // compute coordinate of "leading edge of the triangle" relative to the box
  // center.
  lp = 0;
 ...
 **
 *                                                                                             *
 * Assumes that the axis given is one generated from a cross product of one of
 *the edge and    * basis vectors.  In this case, the box extent can be
 *optimized and only two triangle verts   * need to be checked. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: * true = objects are SEPARATED *
 ...
 * HISTORY: * 4/8/99     GTH : Created. *
 *=============================================================================================*/
static inline bool obbtri_check_collision_cross_axis(BTCollisionStruct &context,
                                                     float dp, int dpi,
                                                     float leb0) {
...
/***********************************************************************************************
 * obbtri_check_collision_basis_axis -- projects the obb and tri onto a basis
 *axis             *
 *                                                                                             *
 ...
 * HISTORY: * 4/8/99     GTH : Created. *
 *=============================================================================================*/
static inline bool obbtri_check_collision_basis_axis(BTCollisionStruct &context,
                                                     float leb0, float dp1,
                                                     float dp2) {
  float dist;      // separation along the axis
  float axismove;  // size of the move along the axis.
  float leb1;      // final coordinate of the leading edge of the box
  ...
  // compute coordinate of "leading edge of the polygon" relative to the box
  // center.
  lp = 0;
  context.TestPoint = 0;
  if (dp1 < lp) {
...
/***********************************************************************************************
 * obbtri_check_collision_normal_axis -- project the box and tri onto the
 *tri-normal           *
 *                                                                                             *
...
static inline void obbtri_compute_contact_normal(
    const BTCollisionStruct &context, CastResultStruct *result) {
 ...
 * HISTORY: * 4/8/99     GTH : Created. *
 *=============================================================================================*/
static inline void eval_A0_point(const BTCollisionStruct &context, float *x,
                                 int edge) {
  float yval, den;
  Vector3 DxE;
 ...
 * HISTORY: * 4/8/99     GTH : Created. *
 *=============================================================================================*/
static inline void eval_A1_point(const BTCollisionStruct &context, float *x,
                                 int edge) {
  float yval, den;
  Vector3 DxE;
 ...
 * HISTORY: * 4/8/99     GTH : Created. *
 *=============================================================================================*/
static inline void eval_A2_point(const BTCollisionStruct &context, float *x,
                                 int edge) {
  float yval, den;
  Vector3 DxE;
 ...
 * HISTORY: * 4/8/99     GTH : Created. *
 *=============================================================================================*/
static inline void obbtri_compute_contact_point(BTCollisionStruct &context,
  ...
  switch (context.AxisId) {
    ...
    case AXIS_A2:
      ...
      Vector3::Add(result->ContactPoint, result->Fraction * context.TriMove,
                   &(result->ContactPoint));
      return;

    case AXIS_A0E0:  // one of the x-aligned edges of the box is contacting
                     // edge0
      eval_A0_point(context, x, 0);
      break;

    case AXIS_A0E1:  // one of the x-aligned edges of the box is contacting
                     // edge1
      eval_A0_point(context, x, 1);
      break;

    case AXIS_A0E2:  // one of the x-aligned edges of the box is contacting
                     // edge2
      eval_A0_point(context, x, 2);
      break;

    case AXIS_A1E0:  // one of the y-aligned edges of the box is contacting
                     // edge0
      eval_A1_point(context, x, 0);
      break;

    case AXIS_A1E1:  // one of the y-aligned edges of the box is contacting
                     // edge1
      eval_A1_point(context, x, 1);
      break;

    case AXIS_A1E2:  // one of the y-aligned edges of the box is contacting
                     // edge2
      eval_A1_point(context, x, 2);
      break;

    case AXIS_A2E0:  // one of the z-aligned edges of the box is contacting
                     // edge0
      eval_A2_point(context, x, 0);
      break;

    case AXIS_A2E1:  // one of the z-aligned edges of the box is contacting
                     // edge1
      eval_A2_point(context, x, 1);
      break;

    case AXIS_A2E2:  // one of the z-aligned edges of the box is contacting
                     // edge3
  ...
  ** the box.  I have tracked at least one of these cases to a situation where
  *the
  ** separating axis is the triangle normal but one of the AxE axes also lines
  *up
  ...
  ** the same up to the six decimal places that MSVC will show me in the
  *debugger.
  ** However, the fraction computed for the 0,0,5 axis was larger by some very
  *small
  ** amount.  This causes it to be used as the "separating axis".  Looks like
  *floating
  ** point roundoff error to me.  The problem is that since the axis was
  *perpendicular
  ** to the triangle, the "nearest-point" logic chose V0 which resulted in the
  ** calculation for x[0] being way off.  The equations for finding the contact
  *point
  ** in the AxE axis case assume that the axis is not normal to the triangle
  *since
  ** that should be handled in the simpler routine for the N separating axis...
  ** Since I am at a loss for how to deal with this problem, I'm going to clamp
  *all
  ...
  */
  result->ContactPoint.X = context.Box.Center.X + x[0] * context.A[0].X +
                           x[1] * context.A[1].X + x[2] * context.A[2].X;

  result->ContactPoint.Y = context.Box.Center.Y + x[0] * context.A[0].Y +
                           x[1] * context.A[1].Y + x[2] * context.A[2].Y;

  result->ContactPoint.Z = context.Box.Center.Z + x[0] * context.A[0].Z +
                           x[1] * context.A[1].Z + x[2] * context.A[2].Z;

  Vector3::Add(result->ContactPoint, result->Fraction * context.BoxMove,
               &(result->ContactPoint));
}
 ...
 * HISTORY: * 4/8/99     GTH : Created. *
 *=============================================================================================*/
bool CollisionMath::Collide(const OBBoxClass &box, const Vector3 &move,
                            const TriClass &tri, const Vector3 &trimove,
                            CastResultStruct *result) {
  ...
  */
  ...
  if (obbtri_check_collision_basis_axis(context, box.Extent.X, context.AE[0][0],
                                        context.AE[0][1]))
    goto exit;
  ...
  */
  ...
  if (obbtri_check_collision_basis_axis(context, box.Extent.Y, context.AE[1][0],
                                        context.AE[1][1]))
    goto exit;
  ...
  */
  ...
  if (obbtri_check_collision_basis_axis(context, box.Extent.Z, context.AE[2][0],
                                        context.AE[2][1]))
    goto exit;
  ...
  */
  ...
  if (context.TestAxis.Length2() > AXISLEN_EPSILON2) {
    dp = context.AN[0];
    leb0 = box.Extent[1] * WWMath::Fabs(context.AE[2][0]) +
           box.Extent[2] * WWMath::Fabs(context.AE[1][0]);
  ...
  */
  ...
  if (context.TestAxis.Length2() > AXISLEN_EPSILON2) {
    dp = -context.AN[0];
    leb0 = box.Extent[1] * WWMath::Fabs(context.AE[2][1]) +
           box.Extent[2] * WWMath::Fabs(context.AE[1][1]);
  ...
  */
  ...
  if (context.TestAxis.Length2() > AXISLEN_EPSILON2) {
    dp = -context.AN[0];
    leb0 = box.Extent[1] * WWMath::Fabs(context.AE[2][2]) +
           box.Extent[2] * WWMath::Fabs(context.AE[1][2]);
    if (obbtri_check_collision_cross_axis(context, dp, 1, leb0)) goto exit;
  }
  ...
  */
  ...
  if (context.TestAxis.Length2() > AXISLEN_EPSILON2) {
    dp = context.AN[1];
    leb0 = box.Extent[0] * WWMath::Fabs(context.AE[2][0]) +
           box.Extent[2] * WWMath::Fabs(context.AE[0][0]);
    if (obbtri_check_collision_cross_axis(context, dp, 2, leb0)) goto exit;
  }
  ...
  */
  ...
  if (context.TestAxis.Length2() > AXISLEN_EPSILON2) {
    dp = -context.AN[1];
    leb0 = box.Extent[0] * WWMath::Fabs(context.AE[2][1]) +
           box.Extent[2] * WWMath::Fabs(context.AE[0][1]);
    if (obbtri_check_collision_cross_axis(context, dp, 1, leb0)) goto exit;
  }
  ...
  */
  ...
  if (context.TestAxis.Length2() > AXISLEN_EPSILON2) {
    dp = -context.AN[1];
    leb0 = box.Extent[0] * WWMath::Fabs(context.AE[2][2]) +
           box.Extent[2] * WWMath::Fabs(context.AE[0][2]);
    if (obbtri_check_collision_cross_axis(context, dp, 1, leb0)) goto exit;
  }
  ...
  */
  ...
  if (context.TestAxis.Length2() > AXISLEN_EPSILON2) {
    dp = context.AN[2];
    leb0 = box.Extent[0] * WWMath::Fabs(context.AE[1][0]) +
           box.Extent[1] * WWMath::Fabs(context.AE[0][0]);
    if (obbtri_check_collision_cross_axis(context, dp, 2, leb0)) goto exit;
  }
  ...
  */
  ...
  if (context.TestAxis.Length2() > AXISLEN_EPSILON2) {
    dp = -context.AN[2];
    leb0 = box.Extent[0] * WWMath::Fabs(context.AE[1][1]) +
           box.Extent[1] * WWMath::Fabs(context.AE[0][1]);
    if (obbtri_check_collision_cross_axis(context, dp, 1, leb0)) goto exit;
  }
  ...
  */
  ...
  if (context.TestAxis.Length2() > AXISLEN_EPSILON2) {
    dp = -context.AN[2];
    leb0 = box.Extent[0] * WWMath::Fabs(context.AE[1][2]) +
           box.Extent[1] * WWMath::Fabs(context.AE[0][2]);
    if (obbtri_check_collision_cross_axis(context, dp, 1, leb0)) goto exit;
  }
...
#pragma message( \
    "(gth) disabling an assert in obb->tri collision, investigate later\n")
#if 0
  ...
  */
  ...
  if ((context.MaxFrac < 1.0f) && (context.MaxFrac <= result->Fraction)) {
    if ((WWMath::Fabs(context.MaxFrac - result->Fraction) > WWMATH_EPSILON) ||
        (Vector3::Dot_Product(*(tri.N), move) <
         Vector3::Dot_Product(result->Normal, move))) {
...
/***********************************************************************************************
        ...
        The following code implements a simple boolean intersection check.  It
uses the same algorithm as the collision function but can avoid some of the
calculations.  For a very simple implementation of this algorithm, see
Oriented_Box_Intersects_Tri in obbox.h

***********************************************************************************************/

/**
** BTIntersectStruct
** Scratchpad variables for the OBBox-Triangle intersection functions.  One
*instance
** of this structure will be used for all of the local variables and its pointer
*will be
** handed of to various inline functions for the axis tests.
 ...
 *                                                                                             *
 * Once the extents are projected onto the axis, this function contains * the
 *logic that determines whether the box and triangle intersect. *
 *                                                                                             *
 * INPUT: * context - the BTIntersectStruct containing the data for this
 *intersection test              * lp - the leading edge of the polygon
 *projected onto the axis                                * leb0 - the leading
 *edge of the box projected onto the axis                                  *
 *                                                                                             *
 * OUTPUT: * true = objects are separated *
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 2/3/2000   gth : Created. *
 *=============================================================================================*/
static inline bool obbtri_intersection_separation_test(
    BTIntersectStruct &context, float lp, float leb0) {
  /*
  ** Only compute the normalized epsilon if we need to.
  ** - compute 'EPSILON' normalized to the length of the axis
  ** - If (I'm no more than 'EPSILON' embedded in the polygon) then the box and
  *tri are separated
  */
  float eps = 0.0f;
  if (lp - leb0 <= 0.0f) {
...
/***********************************************************************************************
 * obbtri_check_intersection_cross_axis -- intersection check for a
 *"cross-product" axis       *
 *                                                                                             *
 * axis being checked is a cross product between a triangle edge and a box basis
 *vector        *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * true = the objects are SEPARATED *
 *                                                                                             *
 * HISTORY: * 5/4/99     GTH : Created. *
 *=============================================================================================*/
static inline bool obbtri_check_intersection_cross_axis(
    BTIntersectStruct &context, float dp, float leb0) {
  float p0;  // distance from box center to vertex 0
  float lp;  // leading edge of the polygon.
 ...
 * INPUT: *
 *                                                                                             *
 * OUTPUT: * true = the objects are SEPARATED *
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 5/4/99     GTH : Created. *
...
static inline bool obbtri_check_intersection_basis_axis(
    BTIntersectStruct &context, float leb0, float dp1, float dp2) {
  float dist;  // separation along the axis
  float lp;    // leading edge of the polygon.
...
/***********************************************************************************************
 * obbtri_check_intersection_normal_axis -- intersection check for the triangle
 *normal         *
 *                                                                                             *
...
/***********************************************************************************************
 * CollisionMath::Intersection_Test -- Intersection check for an OBBox and a
 *triangle          *
 *                                                                                             *
 * INPUT: * box - obbox to be tested * tri - triangle to be tested *
 *                                                                                             *
 * OUTPUT: * true = objects INTERSECT *
 *                                                                                             *
 * WARNINGS: * note that the other inline functions are "quick-reject" functions
 *which return true when    * the objects are separated. *
 *                                                                                             *
 * HISTORY: * 5/4/99     GTH : Created. *
 *=============================================================================================*/
bool CollisionMath::Intersection_Test(const OBBoxClass &box,
  ...
  */
  ...
  context.AE[0][0] = Vector3::Dot_Product(context.A[0], context.E[0]);
  context.AE[0][1] = Vector3::Dot_Product(context.A[0], context.E[1]);
  if (obbtri_check_intersection_basis_axis(context, box.Extent.X,
                                           context.AE[0][0], context.AE[0][1]))
    return false;
  ...
  */
  ...
  context.AE[1][0] = Vector3::Dot_Product(context.A[1], context.E[0]);
  context.AE[1][1] = Vector3::Dot_Product(context.A[1], context.E[1]);
  if (obbtri_check_intersection_basis_axis(context, box.Extent.Y,
                                           context.AE[1][0], context.AE[1][1]))
    return false;
  ...
  */
  ...
  context.AE[2][0] = Vector3::Dot_Product(context.A[2], context.E[0]);
  context.AE[2][1] = Vector3::Dot_Product(context.A[2], context.E[1]);
  if (obbtri_check_intersection_basis_axis(context, box.Extent.Z,
                                           context.AE[2][0], context.AE[2][1]))
    return false;
