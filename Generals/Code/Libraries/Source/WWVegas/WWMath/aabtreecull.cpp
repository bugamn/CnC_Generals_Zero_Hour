...
enum {
  AABTREE_CHUNK_VERSION =
      0x00000001,  // version wrapper, contains 32bit version #
  AABTREE_CHUNK_AABNODE = 0x00000101,  // generic aab-node wrapper
  AABTREE_CHUNK_AABNODE_INFO,          // OBSOLETE! generic aab-node definition
                                       // (IOAABNodeStruct)
  ...
  AABTREE_CHUNK_NODE_INDEX =
      0x00000200,  // wrapper around the node index for an object
...
AABTreeCullSystemClass::AABTreeCullSystemClass(void)
    : ObjectCount(0), NodeCount(0), IndexedNodes(NULL) {
...
void AABTreeCullSystemClass::Add_Object_Internal(CullableClass *obj,
                                                 int node_index) {
  WWASSERT_PRINT((obj->Get_Culling_System() == NULL),
                 "AABTreeCullSystemClass::Add -- Object is already in another "
                 "culling system!\n");
...
int AABTreeCullSystemClass::Object_Count(void) const { return ObjectCount; }

int AABTreeCullSystemClass::Partition_Node_Count_Recursive(
    AABTreeNodeClass *node) const {
  int curcount = 1;
  if (node->Front) {
...
void AABTreeCullSystemClass::Partition_Tree_Depth_Recursive(
    AABTreeNodeClass *node, int cur_depth, int &max_depth) const {
  cur_depth++;
...
void AABTreeCullSystemClass::Add_Object_Recursive(AABTreeNodeClass *node,
  ...
  if (big_child && small_child &&
      (big_child->Compute_Volume() < small_child->Compute_Volume())) {
...
void AABTreeCullSystemClass::Add_Loaded_Object(AABTreeNodeClass *node,
  ...
  WWASSERT_PRINT((obj->Get_Culling_System() == NULL),
                 "AABTreeCullSystemClass::Add_Loaded_Object -- Object is "
                 "already in another culling system!\n");
...
void AABTreeCullSystemClass::Re_Partition(
    const AABoxClass &bounds, SimpleDynVecClass<AABoxClass> &boxes) {
  /*
  ** transfer all objects to a temporary node
  */
  AABTreeNodeClass *dummy_node = new AABTreeNodeClass;
...
const AABTreeCullSystemClass::StatsStruct &
AABTreeCullSystemClass::Get_Statistics(void) {
  return Stats;
}
...
void AABTreeCullSystemClass::Collect_Objects_Recursive(AABTreeNodeClass *node,
  ...
  ** completely contained inside the given box, jump into the collection
  *function
  ** that doesn't do any more volume checking...
  */
  CollisionMath::OverlapType overlap =
      CollisionMath::Overlap_Test(box, node->Box);
  ...
  */
  if (node->Object) {
    CullableClass *obj = get_first_object(node);
    while (obj) {
      if (CollisionMath::Overlap_Test(box, obj->Get_Cull_Box()) !=
...
void AABTreeCullSystemClass::Collect_Objects_Recursive(
    AABTreeNodeClass *node, const FrustumClass &frustum, int planes_passed) {
  /*
  ** Cull the bounding volume of this node against the frustum.
  ** If it is culled, stop descending the tree.
  */
  CollisionMath::OverlapType overlap =
      CollisionMath::Overlap_Test(frustum, node->Box, planes_passed);
  if (overlap == CollisionMath::OUTSIDE) {
    NODE_REJECTED();
    return;
  } else if (overlap == CollisionMath::INSIDE) {
  ...
  */
  if (node->Object) {
    CullableClass *obj = get_first_object(node);
    while (obj) {
      if (CollisionMath::Overlap_Test(frustum, obj->Get_Cull_Box()) !=
          CollisionMath::OUTSIDE) {
        Add_To_Collection(obj);
      }
      obj = get_next_object(obj);
...
void AABTreeCullSystemClass::Collect_Objects_Recursive(
    AABTreeNodeClass *node, const SphereClass &sphere) {
  /*
  ** Is the point inside this volume?
  */
  if (CollisionMath::Overlap_Test(node->Box, sphere) ==
      CollisionMath::OUTSIDE) {
    NODE_REJECTED();
    return;
  }
  ...
  */
  if (node->Object) {
    CullableClass *obj = get_first_object(node);
    while (obj) {
      if (CollisionMath::Overlap_Test(obj->Get_Cull_Box(), sphere) !=
          CollisionMath::OUTSIDE) {
        Add_To_Collection(obj);
      }
      obj = get_next_object(obj);
...
void AABTreeCullSystemClass::Load(ChunkLoadClass &cload) {
  WWASSERT_PRINT(
      Object_Count() == 0,
      "Remove all objects from AAB-Culling system before loading!\n");
...
void AABTreeCullSystemClass::Load_Nodes(AABTreeNodeClass *node,
  ...
  // Older files will contain a chunk named AABTREE_CHUNK_AABNODE_INFO while
  // newer files will contain AABTREE_CHUNK_AABNODE_VARIABLES which contains the
  // IOAABNodeStruct in a micro-chunk.
...
float AABTreeNodeClass::Compute_Volume(void) { return Box.Volume(); }
...
void AABTreeNodeClass::Remove_Object(CullableClass *obj) {
  ...
  while (curobj) {
    ...
    if (curobj == obj) {
      ...
      if (prevobj) {
        AABTreeLinkClass *prevlink =
            (AABTreeLinkClass *)prevobj->Get_Cull_Link();
...
/******************************************************************************************
**
** Partitioning code which partitions the objects in the tree and passes them
*into
** the new children
**
******************************************************************************************/
...
void AABTreeNodeClass::Split_Objects(
    const AABTreeNodeClass::SplitChoiceStruct &sc, AABTreeNodeClass *front,
  ...
  while (Object) {
    ...
    if (CollisionMath::Overlap_Test(sc.Plane, box.Center) ==
        CollisionMath::FRONT) {
...
/******************************************************************************************
**
** Partitioning code which searches for the best split for a given set of boxes.
*This
** code is re-used by both types of partitioning (partitioning contained objects
*or
...
void AABTreeNodeClass::Select_Splitting_Plane(
    SplitChoiceStruct *sc, SimpleDynVecClass<AABoxClass> &boxes) {
...
void AABTreeNodeClass::Compute_Score(AABTreeNodeClass::SplitChoiceStruct *sc,
                                     SimpleDynVecClass<AABoxClass> &boxes) {
  /*
  ** Suitability of a plane as a partition plane is based on the following
  *factors:
...
AABTreeIterator::AABTreeIterator(AABTreeCullSystemClass *tree)
    : Tree(tree), CurNodeIndex(0) {
  WWASSERT(Tree != NULL);
}

void AABTreeIterator::Reset(void) { CurNodeIndex = 0; }
...
int AABTreeIterator::Get_Current_Node_Index(void) { return CurNodeIndex; }
...
/*
  ...
  Here is the existing data in each Node:
        ...
        uint32					Index;
  // Index of this node AABoxClass				Box;
  // Bounding box of the node AABTreeNodeClass *	Parent;
  // parent of this node AABTreeNodeClass *	Front;
  // front node AABTreeNodeClass *	Back;
  // back node CullableClass *		Object;				//
  objects in this node uint32					UserData;
  // 32bit field for the user, initialized to 0
        ...
        uint16					Index;
        short						x,y,z,w,l,h;
  // Need origin and scale factors for the boxes stored in tree uint16
  ParentIndex; uint16					FrontIndex; uint16
  BackIndex; CullableClass *		Object; uint32
  UserData;
