 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * Euler_Integrate -- uses Eulers method to integrate a system of
 *ODE's                      * Midpoint_Integrate -- midpoint method
 *(Runge-Kutta 2) for integration                     * Runge_Kutta_Integrate --
 ...
 *                                                                                             *
 * INPUT: * odesys - pointer to the ODE system to integrate * dt - size of the
 *timestep                                                                   *
 *                                                                                             *
 * OUTPUT: * state vector in odesys will be updated for the next timestep *
 *                                                                                             *
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 08/11/1997 GH  : Created. * 6/25/99    GTH : Updated to the new
 *integrator system                                     *
 *=============================================================================================*/
 ...
 *                                                                                             *
 * INPUT: * sys - pointer to the ODE system to integrate * dt - size of the
 *timestep                                                                   *
 *                                                                                             *
 * OUTPUT: * state vector in odesys will be updated for the next timestep *
 *                                                                                             *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 08/11/1997 GH  : Created. * 6/25/99    GTH : Converted to the new
 *Integrator system                                   *
 *=============================================================================================*/
void IntegrationSystem::Runge_Kutta5_Integrate(ODESystemClass* odesys,
                                               float dt) {
  int i;
  int veclen;
  ...
  */
  ...
  for (i = 0; i < veclen; i++) {
    ytmp[i] = Y0[i] +
              dt * (b51 * dydt[i] + b52 * ak2[i] + b53 * ak3[i] + b54 * ak4[i]);
  }

  // Fifth step
  odesys->Compute_Derivatives(a5 * dt, &ytmp, &ak5);
  for (i = 0; i < veclen; i++) {
    ytmp[i] = Y0[i] + dt * (b61 * dydt[i] + b62 * ak2[i] + b63 * ak3[i] +
                            b64 * ak4[i] + b65 * ak5[i]);
  }
  ...
  for (i = 0; i < veclen; i++) {
    Y1[i] =
        Y0[i] + dt * (c1 * dydt[i] + c3 * ak3[i] + c4 * ak4[i] + c6 * ak6[i]);
  }

  // Error approximation!
  // (maybe I should use this someday? nah not going to use this integrator
  // anyway...)
  for (i = 0; i < veclen; i++) {
    yerr[i] = dt * (dc1 * dydt[i] + dc3 * ak3[i] + dc4 * ak4[i] + dc5 * ak5[i] +
                    dc6 * ak6[i]);
  }
