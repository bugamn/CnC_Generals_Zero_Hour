...
/*
** Force-Link this module because the linker can't detect that we actually need
*it...
*/
...
** Save-Load stuff
*/
SimplePersistFactoryClass<CardinalSpline3DClass,
                          WWMATH_CHUNKID_CARDINALSPLINE3D>
    _CardinalSpline3DFactory;
SimplePersistFactoryClass<CardinalSpline1DClass,
                          WWMATH_CHUNKID_CARDINALSPLINE1D>
    _CardinalSpline1DFactory;
...
void CardinalSpline3DClass::Update_Tangents(void) {
  ...
  // First and Last Key:
  // Only need to compute the OutTangent for key[0] and the InTangent for
  // key[end]
  ...
  if (IsLooping) {
    // This really only works if the start and end points have the same
    // position...
    Tangents[0].OutTangent.X =
        (1.0f - Tightness[0]) * (Keys[1].Point.X - Keys[end - 1].Point.X);
    Tangents[0].OutTangent.Y =
        (1.0f - Tightness[0]) * (Keys[1].Point.Y - Keys[end - 1].Point.Y);
    Tangents[0].OutTangent.Z =
        (1.0f - Tightness[0]) * (Keys[1].Point.Z - Keys[end - 1].Point.Z);
    Tangents[end].InTangent = Tangents[0].OutTangent;

  } else {
    Tangents[0].OutTangent.X =
        (1.0f - Tightness[0]) * (Keys[1].Point.X - Keys[0].Point.X);
    Tangents[0].OutTangent.Y =
        (1.0f - Tightness[0]) * (Keys[1].Point.Y - Keys[0].Point.Y);
    Tangents[0].OutTangent.Z =
        (1.0f - Tightness[0]) * (Keys[1].Point.Z - Keys[0].Point.Z);

    Tangents[end].InTangent.X =
        (1.0f - Tightness[0]) * (Keys[end].Point.X - Keys[end - 1].Point.X);
    Tangents[end].InTangent.Y =
        (1.0f - Tightness[0]) * (Keys[end].Point.Y - Keys[end - 1].Point.Y);
    Tangents[end].InTangent.Z =
        (1.0f - Tightness[0]) * (Keys[end].Point.Z - Keys[end - 1].Point.Z);
  }

  float total_time =
      (Keys[1].Time - Keys[0].Time) + (Keys[end].Time - Keys[end - 1].Time);
  ...
  for (int i = 1; i < Keys.Count() - 1; i++) {
    Tangents[i].InTangent.X =
        (1.0f - Tightness[i]) * (Keys[i + 1].Point.X - Keys[i - 1].Point.X);
    Tangents[i].InTangent.Y =
        (1.0f - Tightness[i]) * (Keys[i + 1].Point.Y - Keys[i - 1].Point.Y);
    Tangents[i].InTangent.Z =
        (1.0f - Tightness[i]) * (Keys[i + 1].Point.Z - Keys[i - 1].Point.Z);
    Tangents[i].OutTangent = Tangents[i].InTangent;

    float in_factor = 2.0f * (Keys[i].Time - Keys[i - 1].Time) /
                      (Keys[i + 1].Time - Keys[i - 1].Time);
    float out_factor = 2.0f * (Keys[i + 1].Time - Keys[i].Time) /
                       (Keys[i + 1].Time - Keys[i - 1].Time);
...
void CardinalSpline1DClass::Update_Tangents(void) {
  ...
  if (IsLooping) {
    // This really only works if the start and end points have the same
    // position...
    Tangents[0].OutTangent =
        (1.0f - Tightness[0]) * (Keys[1].Point - Keys[end - 1].Point);
    Tangents[end].InTangent = Tangents[0].OutTangent;

  } else {
    Tangents[0].OutTangent =
        (1.0f - Tightness[0]) * (Keys[1].Point - Keys[0].Point);
    Tangents[end].InTangent =
        (1.0f - Tightness[0]) * (Keys[end].Point - Keys[end - 1].Point);
  }
  ...
  // inner knots
  for (int i = 1; i < Keys.Count() - 1; i++) {
    Tangents[i].InTangent =
        (1.0f - Tightness[i]) * (Keys[i + 1].Point - Keys[i - 1].Point);
    Tangents[i].OutTangent = Tangents[i].InTangent;
