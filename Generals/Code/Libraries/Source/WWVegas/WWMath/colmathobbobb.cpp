 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * obb_intersect_box0_basis -- intersection test for a basis vector
 *from box0                * obb_intersect_box1_basis -- intersection test for a
 *basis vector from box1                * obb_intersect_axis -- intersection
 *test for a axis                                        * intersect_obb_obb --
 *test two OBBoxes for intersection                                    *
 *   CollisionMath::Intersection_Test -- test two obb's for intersection *
 *   CollisionMath::Intersection_Test -- test an OBB for intersection with an
 *AAB              * CollisionMath::Intersection_Test -- Test an AAB for
 *intersection with an OBB              * obb_separation_test -- test the
 *projections of two obb's for separation                   *
 ...
 *   obb_compute_projections -- computes projections of two boxes onto an
 *arbitrary axis       * compute_contact_normal -- computes the contact normal
 *(after contact is detected)         * eval_side -- returns -1,0,1 depending on
 *ab and side                                      * compute_contact_point --
 *computes the contact point (after contact is detected)           *
 *   collide_obb_obb -- test two obb's for collision * CollisionMath::Collide --
 *collide two OBB's                                               *
...
of the polyhedra or the cross product of an edge from each polyhedra.

        In the case of two oriented 3D boxes, 15 separating axes must be tested.
        Each of the basis vectors from box A, each of the basis vectors from box
B, and the cross products of any combination of a basis vector from A and a
basis vector from B.  Some of these separating axis tests can be optimized.  For
example, if the axis being tested is a basis vector from the first box, then
that box's extent does not need to be projected onto the axis...

        The first batch of functions in this module implement a intersection
test. A boolean is returned indicating whether the two boxes intersect each
other in any way.
...
struct ObbIntersectionStruct {
  ObbIntersectionStruct(const OBBoxClass &box0, const OBBoxClass &box1)
      : Box0(box0), Box1(box1) {
    Vector3::Subtract(box1.Center, box0.Center,
                      &C);  // vector from center of box0 to center of box1
 ...
 *                                                                                             *
 * The optimization here is that the projection of the first box is known since
 *the axis is    * one of its basis vectors. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 ...
 *=============================================================================================*/
static bool obb_intersect_box0_basis(ObbIntersectionStruct &context,
                                     int axis_index) {
 ...
 *                                                                                             *
 * The "optimization" here is that the extent for the second box is known since
 *the axis is    * one of its basis vectors. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 5/4/99     GTH : Created. *
 *=============================================================================================*/
static bool obb_intersect_box1_basis(ObbIntersectionStruct &context,
                                     int axis_index) {
  // ra = box0 projection onto the axis
  // rb = box1 projection onto the axis
 ...
 *                                                                                             *
 * Checks intersection on an arbitrary axis where you've already computed the
 *projectsions.    * Many of the later tests in the OBB-OBB algorigthm fall into
 *here since there are            * optimizations in computing the projections
 *but they are all specific to the axis being used *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 ...
 * HISTORY: * 5/4/99     GTH : Created. *
 *=============================================================================================*/
static inline bool obb_intersect_axis(ObbIntersectionStruct &context,
                                      const Vector3 &axis, float ra, float rb) {
  float rsum = ra + rb;
 ...
 *                                                                                             *
 * This function works in a very similar (but simplified) way as the Collide
 *function.  See    * the comments in that function for more clues regarding the
 *math involved...                 *
 *                                                                                             *
 * Due to the re-usage of intermediate calculations, this function is ~2x faster
 *than the      * equivalent Oriented_Boxes_Intersect. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 5/25/99    GTH : Created. *
 *=============================================================================================*/
bool CollisionMath::Intersection_Test(const OBBoxClass &box0,
                                      const OBBoxClass &box1) {
 ...
 * HISTORY: * 5/25/99    GTH : Created. *
 *=============================================================================================*/
bool CollisionMath::Intersection_Test(const AABoxClass &box0,
                                      const OBBoxClass &box1) {
...
/********************************************************************************
        ...
        This batch of functions implement collision detection for moving
oriented boxes.  Assuming that the two arbitrarily oriented boxes are moving at
a constant velocity along a path and not rotating, the time of collision can be
found. The OBB-ABB and ABB-OBB functions are also implemented in a way that
re-uses the OBB-OBB code.

        For the code which computes the point of collision and collision normal,
you'll have to refer to the paper by Dave Eberly on oriented bounding boxes. The
        formulas for the collision point are the only part of this I was unable
to derive myself (they are pretty nasty...)
...
struct ObbCollisionStruct {
  ObbCollisionStruct(const OBBoxClass &box0, const Vector3 &move0,
                     const OBBoxClass &box1, const Vector3 &move1)
      : StartBad(true),  // Startbad is true until one of the axes clears it
        AxisId(INTERSECTION),  // AxisId will be the axis that allowed the
                               // longest move
    ...
    Vector3::Subtract(box1.Center, box0.Center,
                      &C);  // vector from center of box0 to center of box1
    Vector3::Subtract(move1, move0,
                      &M);  // move vector relative to stationary box0
 ...
 *=============================================================================================*/
static inline bool obb_separation_test(ObbCollisionStruct &context, float ra,
                                       float rb, float u0, float u1) {
  float tmp;
  float rsum = ra + rb;
 ...
 * HISTORY: * 4/8/99     GTH : Created. *
 *=============================================================================================*/
static inline bool obb_check_axis(ObbCollisionStruct &context, float ra,
                                  float rb) {
 ...
 * HISTORY: * 4/8/99     GTH : Created. *
 *=============================================================================================*/
static inline void obb_compute_projections(const ObbCollisionStruct &context,
                                           float *ra, float *rb) {
  *ra = context.Box0.Extent.X *
            WWMath::Fabs(Vector3::Dot_Product(context.A[0], context.TestAxis)) +
        context.Box0.Extent.Y *
            WWMath::Fabs(Vector3::Dot_Product(context.A[1], context.TestAxis)) +
        context.Box0.Extent.Z *
            WWMath::Fabs(Vector3::Dot_Product(context.A[2], context.TestAxis));

  *rb = context.Box1.Extent.X *
            WWMath::Fabs(Vector3::Dot_Product(context.B[0], context.TestAxis)) +
        context.Box1.Extent.Y *
            WWMath::Fabs(Vector3::Dot_Product(context.B[1], context.TestAxis)) +
        context.Box1.Extent.Z *
            WWMath::Fabs(Vector3::Dot_Product(context.B[2], context.TestAxis));
}

/***********************************************************************************************
 * compute_contact_normal -- computes the contact normal (after contact is
 *detected)           *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 4/8/99     GTH : Created. *
 *=============================================================================================*/
static inline void compute_contact_normal(ObbCollisionStruct &context,
...
/***********************************************************************************************
 * compute_contact_point -- computes the contact point (after contact is
 *detected)             *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 4/8/99     GTH : Created. *
 *=============================================================================================*/
static inline void compute_contact_point(ObbCollisionStruct &context,
                                         CastResultStruct *result) {
  int i, j;
  ...
  switch (context.AxisId) {
    ...
    case AXIS_A0B0:
      x[1] =
          -eval_side(context.AB[2][0], context.Side) * context.Box0.Extent[1];
      x[2] = eval_side(context.AB[1][0], context.Side) * context.Box0.Extent[2];
      y[1] =
          -eval_side(context.AB[0][2], context.Side) * context.Box1.Extent[1];
      ...
      if (WWMath::Fabs(den) > 0.0f) {
        ...
        x[0] += context.AB[0][0] *
                (Vector3::Dot_Product(-context.B[0], dcnew) +
                 context.AB[1][0] * x[1] + context.AB[2][0] * x[2]);
    ...
    case AXIS_A0B1:
      x[1] =
          -eval_side(context.AB[2][1], context.Side) * context.Box0.Extent[1];
      ...
      y[2] =
          -eval_side(context.AB[0][0], context.Side) * context.Box1.Extent[2];

      den = (1.0f - context.AB[0][1] * context.AB[0][1]);
      if (WWMath::Fabs(den) > 0.0f) {
        x[0] = Vector3::Dot_Product(context.A[0], dcnew);
        x[0] += context.AB[0][1] *
                (Vector3::Dot_Product(-context.B[1], dcnew) +
                 context.AB[1][1] * x[1] + context.AB[2][1] * x[2]);
    ...
    case AXIS_A0B2:
      x[1] =
          -eval_side(context.AB[2][2], context.Side) * context.Box0.Extent[1];
      x[2] = eval_side(context.AB[1][2], context.Side) * context.Box0.Extent[2];
      y[0] =
          -eval_side(context.AB[0][1], context.Side) * context.Box1.Extent[0];
      ...
      if (WWMath::Fabs(den) > 0.0f) {
        x[0] = Vector3::Dot_Product(context.A[0], dcnew);
        x[0] += context.AB[0][2] *
                (Vector3::Dot_Product(-context.B[2], dcnew) +
                 context.AB[1][2] * x[1] + context.AB[2][2] * x[2]);
    ...
    case AXIS_A1B0:
      ...
      x[2] =
          -eval_side(context.AB[0][0], context.Side) * context.Box0.Extent[2];
      y[1] =
          -eval_side(context.AB[1][2], context.Side) * context.Box1.Extent[1];
      ...
      if (WWMath::Fabs(den) > 0.0f) {
        ...
        x[1] += context.AB[1][0] *
                (Vector3::Dot_Product(-context.B[0], dcnew) +
                 context.AB[0][0] * x[0] + context.AB[2][0] * x[2]);
    ...
    case AXIS_A1B1:
      ...
      x[2] =
          -eval_side(context.AB[0][1], context.Side) * context.Box0.Extent[2];
      y[0] = eval_side(context.AB[1][2], context.Side) * context.Box1.Extent[0];
      y[2] =
          -eval_side(context.AB[1][0], context.Side) * context.Box1.Extent[2];
      ...
      if (WWMath::Fabs(den) > 0.0f) {
        ...
        x[1] += context.AB[1][1] *
                (Vector3::Dot_Product(-context.B[1], dcnew) +
                 context.AB[0][1] * x[0] + context.AB[2][1] * x[2]);
    ...
    case AXIS_A1B2:
      ...
      x[2] =
          -eval_side(context.AB[0][2], context.Side) * context.Box0.Extent[2];
      y[0] =
          -eval_side(context.AB[1][1], context.Side) * context.Box1.Extent[0];
      ...
      if (WWMath::Fabs(den) > 0.0f) {
        ...
        x[1] += context.AB[1][2] *
                (Vector3::Dot_Product(-context.B[2], dcnew) +
                 context.AB[0][2] * x[0] + context.AB[2][2] * x[2]);
    ...
    case AXIS_A2B0:
      x[0] =
          -eval_side(context.AB[1][0], context.Side) * context.Box0.Extent[0];
      x[1] = eval_side(context.AB[0][0], context.Side) * context.Box0.Extent[1];
      y[1] =
          -eval_side(context.AB[2][2], context.Side) * context.Box1.Extent[1];
      ...
      if (WWMath::Fabs(den) > 0.0f) {
        ...
        x[2] += context.AB[2][0] *
                (Vector3::Dot_Product(-context.B[0], dcnew) +
                 context.AB[0][0] * x[0] + context.AB[1][0] * x[1]);
    ...
    case AXIS_A2B1:
      x[0] =
          -eval_side(context.AB[1][1], context.Side) * context.Box0.Extent[0];
      ...
      y[2] =
          -eval_side(context.AB[2][0], context.Side) * context.Box1.Extent[2];
      ...
      if (WWMath::Fabs(den) > 0.0f) {
        ...
        x[2] += context.AB[2][1] *
                (Vector3::Dot_Product(-context.B[1], dcnew) +
                 context.AB[0][1] * x[0] + context.AB[1][1] * x[1]);
    ...
    case AXIS_A2B2:
      x[0] =
          -eval_side(context.AB[1][2], context.Side) * context.Box0.Extent[0];
      x[1] = eval_side(context.AB[0][2], context.Side) * context.Box0.Extent[1];
      y[0] =
          -eval_side(context.AB[2][1], context.Side) * context.Box1.Extent[0];
      ...
      if (WWMath::Fabs(den) > 0.0f) {
        ...
        x[2] += context.AB[2][2] *
                (Vector3::Dot_Product(-context.B[2], dcnew) +
                 context.AB[0][2] * x[0] + context.AB[1][2] * x[1]);
  ...
  result->ContactPoint.X = context.Box0.Center.X + x[0] * context.A[0].X +
                           x[1] * context.A[1].X + x[2] * context.A[2].X;

  result->ContactPoint.Y = context.Box0.Center.Y + x[0] * context.A[0].Y +
                           x[1] * context.A[1].Y + x[2] * context.A[2].Y;

  result->ContactPoint.Z = context.Box0.Center.Z + x[0] * context.A[0].Z +
                           x[1] * context.A[1].Z + x[2] * context.A[2].Z;

  Vector3::Add(result->ContactPoint, result->Fraction * context.Move0,
               &(result->ContactPoint));
}
...
bool collide_obb_obb(ObbCollisionStruct &context, CastResultStruct *result) {
  ...
  //      = |ey*A1*(A0xB0)| + |ez*A2*(A0xB0)|
  //      A0*(A0xB0)=0 = |ey*B0*(A1xA0)| + |ez*B0*(A2xA0)|
  //      A*(BxC)=B*(CxA)=C*(AxB) = |-ey*A2*B0| +     |ez*A1*B0|
  //      A0xA1=A2 = |ey*AB[2][0]| +   |ez*AB[1][0]|
  //      already computed these dot products!
  //
  /////////////////////////////////////////////////////////////////////////
 ...
 * HISTORY: * 4/8/99     GTH : Created. *
 *=============================================================================================*/
bool CollisionMath::Collide(const OBBoxClass &box0, const Vector3 &move0,
                            const OBBoxClass &box1, const Vector3 &move1,
                            CastResultStruct *result) {
 ...
 *=============================================================================================*/
bool CollisionMath::Collide(const OBBoxClass &box0, const Vector3 &move0,
                            const AABoxClass &box1, const Vector3 &move1,
                            CastResultStruct *result) {
  OBBoxClass obbox1(box1.Center, box1.Extent);
 ...
 * HISTORY: * 5/25/99    GTH : Created. *
 *=============================================================================================*/
bool CollisionMath::Collide(const AABoxClass &box0, const Vector3 &move0,
                            const OBBoxClass &box1, const Vector3 &move1,
                            CastResultStruct *result) {
  OBBoxClass obbox0(box0.Center, box0.Extent);
