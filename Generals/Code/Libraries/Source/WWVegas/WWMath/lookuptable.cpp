...
/*
** Save-Load stuff.  Lookup tables are basically saved 1D curve classes.  They
*are turned
** into tables at load time.  For future expansion, the curve is wrapped in a
*chunk.
*/
enum {
...
LookupTableClass::LookupTableClass(int sample_count)
    : MinInputValue(0.0f), MaxInputValue(0.0f), OutputSamples(sample_count) {}
...
void LookupTableClass::Init(const char *name, Curve1DClass *curve) {
  ...
  for (int i = 0; i < OutputSamples.Length(); i++) {
    float x = MinInputValue + (MaxInputValue - MinInputValue) * (float)i /
                                  (float)(OutputSamples.Length() - 1);
    float y;
    curve->Evaluate(x, &y);
    OutputSamples[i] = y;
  }
...
void LookupTableMgrClass::Shutdown(void) { Reset(); }
...
LookupTableClass *LookupTableMgrClass::Get_Table(const char *name,
                                                 bool try_to_load) {
...
void LookupTableMgrClass::Save_Table_Desc(ChunkSaveClass &csave,
                                          Curve1DClass *curve,
...
void LookupTableMgrClass::Load_Table_Desc(ChunkLoadClass &cload,
