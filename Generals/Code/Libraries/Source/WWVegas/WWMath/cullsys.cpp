...
// NOTE: THE Get_() functions currently are the same as the Peek_() functions
// (e.g., they do not add a Ref). This is wrong and will be fixed.
CullableClass* CullSystemClass::Get_First_Collected_Object_Internal(void) {
  return CollectionHead;
}

CullableClass* CullSystemClass::Get_Next_Collected_Object_Internal(
    CullableClass* obj) {
  if (obj != NULL) {
...
CullableClass* CullSystemClass::Peek_Next_Collected_Object_Internal(
    CullableClass* obj) {
  if (obj != NULL) {
    return obj->NextCollected;
  }
