...
enum { CHUNKID_VARIABLES = 0x00000100, CHUNKID_OBJECTS, CHUNKID_OBJECT };

enum { VARID_NEXTDEFID = 0x01 };
...
HashTemplateClass<StringClass, DynamicVectorClass<DefinitionClass *> *>
    *DefinitionMgrClass::DefinitionHash;
...
//
//////////////////////////////////////////////////////////////////////////////////
DefinitionMgrClass::DefinitionMgrClass(void) { return; }
...
DefinitionClass *DefinitionMgrClass::Find_Typed_Definition(const char *name,
  ...
  if (DefinitionHash == NULL) {
    WWDEBUG_SAY(
        ("DefinitionMgrClass::Find_Typed_Definition () failed due to a NULL "
         "DefinitionHash.\n"));
    return NULL;
  }
  ...
  // Check the hash table first. The hash table is built as we need the
  // definitions, so if definition is not in the table, it will be added there.
  ...
  DynamicVectorClass<DefinitionClass *> *defs =
      DefinitionHash->Get(name_string);
  if (defs) {
      ...
      if (curr_def != NULL) {
        ...
        if ((curr_class_id == class_id) ||
          ...
          if (::stricmp(curr_def->Get_Name(), name) == 0) {
            definition = curr_def;
            // Add the definition to the hash table, so that it can be quickly
            // accessed the next time it is needed.
            if (!defs) {
...
void DefinitionMgrClass::List_Available_Definitions(int superclass_id) {
  ...
  for (definition = Get_First(superclass_id, DefinitionMgrClass::ID_SUPERCLASS);
       definition != NULL;
       definition = Get_Next(definition, superclass_id,
                             DefinitionMgrClass::ID_SUPERCLASS)) {
    WWDEBUG_SAY(("  >%s<\n", definition->Get_Name()));
  }
...
DefinitionClass *DefinitionMgrClass::Get_First(uint32 id, ID_TYPE type) {
  ...
  //
  for (int index = 0; (definition == NULL) && (index < _DefinitionCount);
       index++) {
    DefinitionClass *curr_def = _SortedDefinitionArray[index];
    if (curr_def != NULL) {
      //
      ...
      if ((type == ID_SUPERCLASS) &&
        ...
        definition = curr_def;
      } else if ((type == ID_CLASS) && (curr_def->Get_Class_ID() == id)) {
        definition = curr_def;
      }
    }
  }
...
DefinitionClass *DefinitionMgrClass::Get_Next(DefinitionClass *curr_def,
  ...
  for (int index = curr_def->m_DefinitionMgrLink + 1;
       (definition == NULL) && (index < _DefinitionCount); index++) {
    DefinitionClass *curr_def = _SortedDefinitionArray[index];
    if (curr_def != NULL) {
      //
      //	Is this the definition we were looking for?
...
void DefinitionMgrClass::Free_Definitions(void) {
  ...
  if (DefinitionHash) {
    HashTemplateIterator<StringClass, DynamicVectorClass<DefinitionClass *> *>
        ite(*DefinitionHash);
...
//
////////////////////////////////////////////////////////////////////////////
void DefinitionMgrClass::Prepare_Definition_Array(void) {
  if (_DefinitionCount + 1 > _MaxDefinitionCount) {
    ...
    //
    ::memcpy(new_array, _SortedDefinitionArray,
             _DefinitionCount * sizeof(DefinitionClass *));
...
void DefinitionMgrClass::Register_Definition(DefinitionClass *definition) {
  ...
  if (definition != NULL && definition->m_DefinitionMgrLink == -1 &&
    ...
    if (is_valid) {
      //
      //	Re-index all the definitions that got bumped one cell due to
      //this insertion.
      //
...
void DefinitionMgrClass::Unregister_Definition(DefinitionClass *definition) {
  ...
  // WWASSERT (definition->m_DefinitionMgrLink >= 0 &&
  // definition->m_DefinitionMgrLink < _DefinitionCount);

  if (definition != NULL && definition->m_DefinitionMgrLink != -1) {
    //
    //	Re-index the definitions that come after this definition in the list
    //
    for (int index = definition->m_DefinitionMgrLink;
         index < _DefinitionCount - 1; index++) {
...
bool DefinitionMgrClass::Load_Objects(ChunkLoadClass &cload) {
  ...
  if (_DefinitionCount > 0) {
    ::qsort(_SortedDefinitionArray, _DefinitionCount, sizeof(DefinitionClass *),
...
//
////////////////////////////////////////////////////////////////
int __cdecl DefinitionMgrClass::fnCompareDefinitionsCallback(
