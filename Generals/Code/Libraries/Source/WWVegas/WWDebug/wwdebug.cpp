...
void WWDebug_DBWin32_Message_Handler(const char* str) {
  ...
  hSharedFile = CreateFileMapping((HANDLE)-1, NULL, PAGE_READWRITE, 0, 4096,
                                  "DBWIN_BUFFER");
  if (!hSharedFile) {
    // MessageBox(NULL, "DebugTrace: Unable to create file mapping object
  ...
  if (!lpszSharedMem) {
    // MessageBox(NULL, "DebugTrace: Unable to map shared memory", "Error",
    // MB_OK);
    CloseHandle(heventDBWIN);
    CloseHandle(heventData);
    return;
