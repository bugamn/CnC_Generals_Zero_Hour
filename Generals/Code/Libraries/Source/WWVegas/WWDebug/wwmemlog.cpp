 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * WWMemoryLogClass::Allocate_Memory -- allocates memory *
...
/*
** Name for each memory category.  I'm padding the array with some "undefined"
*strings in case
** someone forgets to set the name when adding a new category.
*/
static char *_MemoryCategoryNames[] = {
    "UNKNOWN",     "Geometry",    "Animation",   "Texture",     "Pathfind",
    "Vis",         "Sound",       "CullingData", "Strings",     "GameData",
    "PhysicsData", "W3dData",     "<undefined>", "<undefined>", "<undefined>",
    "<undefined>", "<undefined>", "<undefined>",
};

/**
** MemoryCounterClass
** This object will store statistics for each memory category.  It can provide
*things like
** the current amount of allocated memory and the peak amount of allocated
*memory.
*/
...
** This object is used to keep track of the "active memory category".  Whenever
*memory is allocated
** it will be charged to the current active memory category.  To be thread-safe,
*there will be
...
class ActiveCategoryStackClass : public VectorClass<int> {
 public:
  ActiveCategoryStackClass(void)
      : VectorClass<int>(MAX_CATEGORY_STACK_DEPTH), ThreadID(-1), Count(0) {}
...
/**
** ActiveCategoryClass
** This is a dynamic vector of ActiveCategoryStackClasses which adds a new stack
*each time
** a new thread is encountered.  It also is able to return to you the active
*category for
** the currently active thread automatically.
*/
const int MAX_CATEGORY_STACKS =
    256;  // maximum number of threads we expect to encounter...

class ActiveCategoryClass : public VectorClass<ActiveCategoryStackClass> {
 public:
  ActiveCategoryClass(void)
      : VectorClass<ActiveCategoryStackClass>(MAX_CATEGORY_STACKS), Count(0) {}
...
/**
** MemLogClass
** This class ties all of the logging datastructures together into a single
*object
...
/**
** Static Variables
** _TheMemLog - object which encapsulates all logging. will be allocated on
*first use
** _MemLogMutex - handle to the mutex used to arbtirate access to the logging
*data structures
...
**
***************************************************************************************************/
ActiveCategoryStackClass &ActiveCategoryStackClass::operator=(
    const ActiveCategoryStackClass &that) {
  if (this != &that) {
...
int WWMemoryLogClass::Get_Category_Count(void) { return MEM_COUNT; }
...
static void __cdecl _MemLogCleanup(void) { delete _TheMemLog; }
...
MemLogClass *WWMemoryLogClass::Get_Log(void) {
  ...
  if (_TheMemLog == NULL) {
    ...
    /*
    ** This was me trying to be clever and fix the memory leak in the memlog.
    *Unfortunately, the Get_Log member can be called
    ** during the process of exiting the process (IYSWIM) and you get it trying
    *to re-allocate the MemLogClass I just freed.
    ** Solution is just to disable memlog when I'm trying to find memory leaks.
    *ST - 6/18/2001 9:51PM
    */
    if (!_MemLogAllocated) {
      atexit(&Release_Log);
...
/***********************************************************************************************
 * WWMemoryLogClass::Release_Log -- Free the memory used by WWMemoryLogClass so
 *it doesn't leak*
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 ...
 *                                                                                             *
 *           It's messy, but I assume there's a reason it's not statically
 *allocated...        * OK, now I get it *
 *                                                                                             *
 * HISTORY: * 6/13/2001 8:55PM ST : Created *
 *=============================================================================================*/
...
**
** PLEASE NOTE: The user is expected to implement global new and delete
*functions in his own
** code which call WWMemoryLogClass::Allocate_Memory and
*WWMemoryLogClass::Release_Memory.
** This was the only solution I could come up given that some APPS have their
*own new and delete
** functions or enable the CRT ones.  It was also not an option to move this
*entire system into
** the APP because I wanted all of our LIBs to participate in the memory usage
*logging...
**
***************************************************************************************************/

const int WWMEMLOG_KEY0 = (unsigned('G') << 24) | (unsigned('g') << 16) |
                          (unsigned('0') << 8) | unsigned('l');
const int WWMEMLOG_KEY1 = (unsigned('~') << 24) | (unsigned('_') << 16) |
                          (unsigned('d') << 8) | unsigned('3');

/**
** MemoryLogStruct
** This structure is added to the beginning of each memory allocation to
*facilitate
** tracking which category the memory belongs to when it is freed.  The size of
** this struct is also 16 bytes so that we wont be seriously affecting the
*alignment
** of allocated memory...
*/
struct MemoryLogStruct {
  ...
  int Key0;  // if this is not equal to WWMEMLOG_KEY0 then we don't have a valid
             // log
 ...
 *                                                                                             *
 *    This function adds a header to the memory allocated so that when the
 *memory is freed     * the proper memory category size can be decremented.  The
 *application using this logging  * system should call this function from inside
 *its overloaded 'new' operator.              *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
    ...
    */
    ...
    if (ptr != NULL) {
      ...
      */
      ...
      /*
      ** Write our logging structure into the beginning of the buffer.  I'm
      *using
      ** placement new syntax to initialize the log structure right in the
      *memory buffer
      */
      new (ptr) MemoryLogStruct(active_category, size);

      /*
      ** Return the allocated memory to the user, skipping past our log
      *structure.
      */
 ...
 * WWMemoryLogClass::Release_Memory -- frees memory *
 *                                                                                             *
 *    This function checks for a wwmemlog header and decrements the relevant
 *memory category.  * It should be called in the application's custom delete
 *operator.                         *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void WWMemoryLogClass::Release_Memory(void *ptr) {
  ...
  if (ptr) {
    ...
    */
    MemoryLogStruct *memlog =
        (MemoryLogStruct *)((char *)ptr - sizeof(MemoryLogStruct));
    if (memlog->Is_Valid_Memory_Log()) {
      ...
      */
      WWMemoryLogClass::Register_Memory_Released(memlog->Category,
                                                 memlog->Size);
      free((void *)memlog);
...
int WWMemoryLogClass::Get_Allocate_Count() { return AllocateCount; }

// Return allocate count since last reset
int WWMemoryLogClass::Get_Free_Count() { return FreeCount; }
