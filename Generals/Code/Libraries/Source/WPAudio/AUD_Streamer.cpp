...
static int recalcBuffering(AudioStreamer *as, TimeStamp buffering,
                           AudioFormat *format);
...
static int streamNextFrame(AudioChannel *channel) {
  ...
  STM_AccessAdvance(as->out,
                    as->pending_bytes);  // mark these bytes as having been used
...
static int recalcBuffering(AudioStreamer *as, TimeStamp buffering,
  ...
  as->frame_size =
      AudioFormatBytes(format, AudioChannelFrameTime(as->channel) * 2);
...
int AudioStreamerInit(void) {
  ...
  thread =
      AUD_ThreadCreate("Streamer Thread", AUD_THREAD_PRI_HIGH, service_streams);
...
void AudioStreamerDestroy(AudioStreamer *as) {
  ...
  msg_assert(!(as->flags & AS_PLAYING),
             ("Trying to destroy as stream while is is still playing"));
...
int AudioStreamerOpen(AudioStreamer *as, File *file) {
  ...
  FLAGS_SET(as->flags,
            AS_NO_FILE_CLOSE);  // we did not open this file so do not close it
  as->file = file;
...
int AudioStreamerStart(AudioStreamer *as) {
  ...
  AUD_ThreadBeginCriticalSection(thread);
  as->stream_pos = AudioFormatSeekToPos(as->file, &as->format, as->stream_pos,
                                        as->data_start);
  ...
  time = max(AudioChannelBufferTime(as->channel) * 2,
             AUD_ThreadGetInterval(thread) * 4);
  ...
  as->sample.Bytes =
      AudioFormatBytes(&as->format, AudioChannelFrameTime(as->channel));
...
int AudioStreamerIsFading(AudioStreamer *as) {
  ...
  return AudioStreamerIsPlaying(as) &&
         !AudioAttribsVolumeAdjusted(&as->channel->ChannelAttribs);
}
...
void AudioStreamerWaitForFade(AudioStreamer *as) {
  DBG_ASSERT_TYPE(as, AudioStreamer);
  TimeStamp time_out =
      AudioGetTime() + as->channel->ChannelAttribs.VolumeLevel.duration;
...
static void streamDump(AudioStreamer *as, int index,
  ...
  if (out_pos > in_pos) {
    for (i = out_pos + 1; i < vRANGE; i++) gauge[i] = '*';

    for (i = 0; i < in_pos; i++) gauge[i] = '*';
  } else {
    for (i = out_pos + 1; i < in_pos; i++) gauge[i] = '*';
  }
  ...
  TimeStamp time =
      AudioFormatTime(&as->format, as->total_bytes - as->stream_pos);
  ...
  sprintf(buffer, "%.12s %4d Kb %.31s \n", AudioStreamerName(as), total / 1024,
          name);
  print(buffer);
  ...
  sprintf(buffer, " Status: %s %s %s %d:%02d:%03d \n",
          as->flags & AS_PLAYING
              ? "Play  "
              : (as->flags & AS_PAUSED ? "Paused" : "Stop  "),
          as->flags & AS_FILL ? "Fill" : "    ",
          Locked(&as->lock) ? "Locked" : "      ", minutes, seconds, time);
  print(buffer);
