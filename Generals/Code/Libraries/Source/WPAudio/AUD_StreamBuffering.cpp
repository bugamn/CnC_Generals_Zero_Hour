...
static void STM_access_init(STM_STREAM *stm, STM_ACCESS *access, int id) {
  ...
  access->stream = stm;
  DBG_CODE(LockInit(&access->in_service); /* we use the ref to stop nesting */
  )
  LockInit(&access->ref);
...
void STM_StreamDestroy(STM_STREAM *stm) {
  ...
  DBG_ASSERT_TYPE(stm, STM_STREAM);
  DBG_ASSERT(
      !(stm->flags & mSTM_STREAM_FAKE)); /* you can not destroy a fake stream */
  DBG_ASSERT(!Locked(&stm->ref)); /* you are trying to destroy a stream that is
  ...
  DBG_CODE(
      ...
      for (i = 0; i < vSTM_MAX_ACCESSORS; i++) {
        DBG_INVALIDATE_TYPE(&stm->access[i]); /* ensure that nobody tries to use
                                                 an accessor after it is dead */
      }
...
void STM_StreamReset(STM_STREAM *stm) {
  ...
  DBG_ASSERT_TYPE(stm, STM_STREAM);
  DBG_ASSERT(
      !Locked(&stm->ref)); /* you cannot reset the stream will it's in use */
...
void STM_StreamAddSBuffer(STM_STREAM *stm, STM_SBUFFER *sbuf) {
  ...
  W_FlagsClear(
      stm->flags,
      mSTM_STREAM_RESET_DONE); /* make sure that a reset is performed again */
}
...
int STM_StreamCreateSBuffers(STM_STREAM *stm, int num_buffers, int buf_size,
                             int align) {
  int bcount = 0;
  STM_SBUFFER *buf;
...
STM_ACCESS *STM_StreamAcquireAccess(STM_STREAM *stm, int access_id) {
  ...
  DBG_ASSERT(access_id <
             vSTM_MAX_ACCESSORS); /* legal values are vSTM_ACCESS_ID_OUT or
                                     vSTM_ACCESS_ID_IN */
  ...
  /* if this is the first time the stream has been acquired then perform a reset
   * on the buffers */
...
void STM_SBufferDestroy(STM_SBUFFER *sbuf) {
  ...
  DBG_ASSERT(!ListNodeInList(
      &sbuf->nd)); /* you must remove the buffer from its stream */
...
void STM_SBufferRemove(STM_SBUFFER *sbuf) {
  ...
  DBG_ASSERT(
      ListNodeInList(&sbuf->nd));   /* buffer is not presently in a stream */
...
void STM_AccessRelease(STM_ACCESS *access) {
  ...
  DBG_ASSERT_TYPE(access, STM_ACCESS);
  DBG_ASSERT(
      Locked(&access->ref)); /* already released, or was never acquired */
...
int STM_AccessSetMode(STM_ACCESS *access, int mode) {
  ...
  DBG_ASSERT(
      mode <
      vSTM_ACCESS_MAX_MODES); /* see vSTM_ACCESS_MODE_?? in wcore/stream.h */
...
int STM_AccessFileTransfer(STM_ACCESS *access, File *file, int bytes,
                           int *transfered) {
  int transfer;
...
int STM_AccessAdvance(STM_ACCESS *access, int bytes_to_advance) {
  int bytes_advanced = 0, bytes_available, advance;
  int total_bytes;
  int ok = TRUE;
  ...
  do {
    ...
    if (access->flags & mSTM_ACCESS_TOP_OF_START) {
      access->Block.Bytes = bytes_available; /* this must be a valid range as
                                                total_bytes cannot be greater
                                                then the total stream bytes */
    } else {
      int actual_bytes = buf_region->Bytes -
                         ((uint)access->Block.Data - (uint)buf_region->Data);
    ...
    if (advance >= access->Block.Bytes) {
      ...
      if (access->SBuffer == access->start_buffer) {
        ...
        access->Block.Data = access->SBuffer->data_region
                                 .Data; /* this is the top of the buffer */
      } else {
  ...
  if ((access->access_pos) &&
      (STM_AccessMode(access) == vSTM_ACCESS_MODE_UPDATE)) {
...
uint STM_AccessTotalBytes(STM_ACCESS *access) {
  uint total_bytes, bytes_in, bytes_out;
...
STM_ACCESS *STM_AccessUpStreamAccessor(STM_ACCESS *access) {
  ...
  DBG_ASSERT(access->id < vSTM_MAX_ACCESSORS); /* something has gone wrong. id
