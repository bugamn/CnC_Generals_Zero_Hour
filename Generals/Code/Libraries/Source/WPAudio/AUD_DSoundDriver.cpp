...
*** *
*** *
*** *	audioLoad              :   load system ** *	audioUnload            :
*unload system                                 ** *	audioOpen              :
*open a unit                                   ** *	audioClose             :
*close a unit                                  **
**                                                                          **
**	audioOpenChannel      :   open an audio channel **
**	audioCloseChannel     :   close an audio channel ** *	audioStart :
*satrt a sample                                ** *	audioStop              :
*stop a sample                                 ** *	audioPause             :
*pause a sample                                ** *	audioResume            :
*resume a sample                               ** *	audioCheck             :
*check if a sample is playing                  ** *	audioUpdate            :
*update sample attributes                      ** *	audioQueueIt          :
*queue up sample to play next                  ** *	AUD_command           :
...
#define vAUD_DRV_POLL_INTERVAL \
  (SECONDS(1) / 30) /* this frequency will determine the buffer sizes */
...
typedef int(CB_TRANSFER)(struct AudioDriverChannelTag *, uint8 *dst,
                         int *dst_bytes, uint8 *src, int *src_bytes);
...
typedef struct AudioDriverChannelTag {
  ...
  /* play */
  volatile int
      pcm_pos; /* pcm playback position relative to the end of the sample */
  ...
  volatile ulong
      write_pos;         /* current write posistion in buffer according to DS */
...
static AudioDriver _AUD_driver = {NULL, /* use for extended data */
                                  audioOpenChannel,
                                  audioCloseChannel,
                                  audioStart,
                                  audioStop,
...
// Table was generated with the function 1000*LOG2(n/100) where n is the range
// 0..100 0 = no volume, 100 = full volume, and the range is linear ( 50 is half
// volume). LOG2() is log to the base 2
static int AUD_log_table[101] =
    {  // 1....2.....3......4....5......6.....7....8.....9......0
...
// static		int			AUD_set_dev_format ( AudioDevice
// *dev, AudioFormat *format );
...
static int PCM_transfer(AUD_DRV_CHAN *, uint8 *dst, int *dst_bytes, uint8 *src,
                        int *src_bytes) {
  int transfer;
...
static int ADPCM_transfer(AUD_DRV_CHAN *ci, uint8 *dst, int *dst_bytes,
                          uint8 *src, int *src_bytes) {
  int sbytes = 0;
  int dbytes = 0;
  int stransfered = 0;
  int dtransfered = 0;
...
static S32 AILCALLBACK fetch_CB(U32 user, void FAR *dest, S32 bytes,
                                S32 offset) {
...
static int MP3_transfer(AUD_DRV_CHAN *ci, uint8 *dst, int *dst_bytes,
                        uint8 *src, int *src_bytes) {
  int sbytes = 0;
  int dbytes = 0;
  int stransfered = 0;
  ...
  if (!data->mp3.asi) {
    // start MP3 streamer
    if (!(data->mp3.asi =
              (ASISTREAM *)ASI_stream_open((U32)data, fetch_CB, 0))) {
      // bad MP3 stream
      return FALSE;
    }
  }
  ...
  data->pending = data->in_bytes_left ||
                  (data->mp3.asi->output_cursor < data->mp3.asi->frame_size);
...
static int AUD_same_format(AudioFormat *f1, AudioFormat *f2) {
  return (f1->Channels == f2->Channels && f1->Rate == f2->Rate &&
...
static int AUD_create_pcm_buffer(AUD_DRV_CHAN *ci, AudioFormat *format) {
  ...
  desc.dwFlags =
      (DSBCAPS_CTRLVOLUME | DSBCAPS_CTRLPAN |
       DSBCAPS_CTRLFREQUENCY);  // Need default controls vol, pan, pitch

  /* now we work out the size that the buffer needs to be. This is based on the
  polling interval and the number of frames.

  A frame size is the that size that when played back will take X polling
  intervals ( over sampling). This ensures that the polling code will not miss
  any frames. A frame is also the minimum update size.

          frame size = ((bytes per second) * (poll interval per second) * over
  sample
  */

  ci->frame_bytes = ((pcmwf.nAvgBytesPerSec * (vAUD_DRV_POLL_INTERVAL / 10)) /
                     (TIMER_HZ / 10)) *
                    vAUD_DRV_OVER_SAMPLE;
  ...
  msg_assert(caps.dwBufferBytes == desc.dwBufferBytes,
             ("Error: Asked for buffer of %d bytes, but got %d bytes",
...
#ifdef _DEBUG
  msg_assert(AUD_same_format(&ci->format, format),
             ("Sample \"%s\" has incompatible format with the previous "
              "sample.\nCannot seamlessly merge samples of differing playback "
              "parameters!!",
              ci->chan->sample_name));
#endif  //_DEBUG
...
static int AUD_lock_buffer(AUD_DRV_CHAN *ci, ulong pos, ulong transfer,
                           uint8 **dst, ulong *bytes1, uint8 **dst2,
                           ulong *bytes2) {
  int retval;
  int retry_count = 3;
...
retry:
  ...
  if ((retval = ci->dsbuf->Lock(pos, transfer, (void **)dst, bytes1,
                                (void **)dst2, bytes2, 0)) != DS_OK) {
...
static int AUD_fill_buffer(AUD_DRV_CHAN *ci, int total_bytes) {
  ...
  while (total_bytes > 0) {
    ...
    if (!AUD_lock_buffer(ci, ci->dst_pos, bytes, &lpos1, &lbytes1, &lpos2,
                         &lbytes2)) {
      goto done;
    }
    ...
    while (bytes) {
      ...
      if (ci->src_left == 0) {
         ...
         */
        ci->src_left = -1; /* unless the call back adds new source we assume no
                              more data */
      ...
      if (ci->src_left <= 0 && !ci->transfer_data.pending) {
        ...
        if ((src_bytes = ci->src_left) <= 0) {
          ...
          if (!ci->transfer(ci, dst, (int *)&transfer, (uint8 *)ci->src_pos,
                            &src_bytes)) {
            goto done;
          }
        }
...
static int audioLoad(AudioSystem *system) {
  ...
  result = AUD_sound_object->SetCooperativeLevel(AudioGetWindowsHandle(),
                                                 DSSCL_PRIORITY);
  ...
  if (!thread) {
    LockInit(&drv_ref);
    if (!(thread =
              AUD_ThreadCreate("Audio Service Thread", AUD_THREAD_PRI_REALTIME,
                               AUD_service_device))) {
      return ERROR_CODE_FAIL;
    }
    LockAcquire(&drv_ref);
...
static int audioOpen(AudioDevice *dev) {
  ...
  AudioServiceSetMustServiceInterval(
      &dev->mixerUpdate,
      vAUD_DRV_POLL_INTERVAL * vAUD_DRV_OVER_SAMPLE * vAUD_DRV_FRAMES);
  AudioServiceSetResetInterval(
      &dev->mixerUpdate,
      vAUD_DRV_POLL_INTERVAL * vAUD_DRV_OVER_SAMPLE * vAUD_DRV_FRAMES * 3);
  ...
  /* start the primary buffer playing */
  {
    ...
    if (AUD_sound_object->CreateSoundBuffer(&dsbdesc, &AUD_primary_buffer,
                                            NULL) != DS_OK) {
...
static int audioOpenChannel(AudioChannel *chan) {
  ...
  AudioChannelSetFormat(
      ci->chan,
      &ci->chan->Device->DefaultFormat); /* initially create sound buffer with
                                            default format */
  return vNO_ERROR;
}
...
static int audioStart(AudioChannel *chan) {
  ...
  if (ci->dsbuf->Play(0, 0, DSBPLAY_LOOPING) !=
      DS_OK) /* must do looping or things will go horribly wrong */
  {
...
static int audioQueueIt(AudioChannel *) { return ERROR_CODE_FAIL; }
...
/*						         :NOTICE:
        There are no public functions in a driver. If you have any then you are
   doing something wrong. -Tr
*/

void *GetDirectSoundObject(void) { return (void *)AUD_sound_object; }

void *GetPrimaryBuffer(void) { return (void *)AUD_primary_buffer; }
...
void AudioDeviceDumpDXSNDDriver(AudioDevice *dev, void (*print)(const char *)) {
  ...
  while (chan) {
    sprintf(string, "Channel 0x%0x, F=%6d, P=%d, I=%04d, T=%6d, B=%6d\n", chan,
            chan->drvData->frame_bytes, chan->drvData->frame,
            chan->drvData->poll_interval, chan->drvData->min_fill,
...
static int IMA_decode_block(AUD_DRV_CHAN *ci) {
  ...
  for (i = 0; i < data->channels; i++) {
    ...
    if ((data->ima[i].index = in[2]) > 88) {
      msg_assert(FALSE,
                 ("IMA ADPCM Format Error (bad index value) in wav file %s",
                  ci->chan->sample_name));
      return FALSE;
    }
    ...
    if (in[3]) {
      msg_assert(
          FALSE,
          ("IMA ADPCM Format Error (synchronization error) in wav file %s ",
           ci->chan->sample_name));
      return FALSE;
    }
    in += 4;
...
static const int adaptionTable[] = {230, 230, 230, 230, 307, 409, 512, 614,
                                    768, 614, 512, 409, 307, 230, 230, 230};

static short MSAdpcmDecode(unsigned char nibble, MS_DATA *ms, int sample1,
                           int sample2) {
  int newsample;
  int predsample;
...
static int MS_decode_block(AUD_DRV_CHAN *ci) {
  ...
  if (channels == 1) {
    ...
    while (bytes) {
      ...
      *out++ = MSAdpcmDecode(byte >> 4, &data->ms[chan], out[-channels],
                             out[-channels2]);
      if (++chan == channels) {
        chan = 0;
      }

      *out++ = MSAdpcmDecode(byte & 0x0f, &data->ms[chan], out[-channels],
                             out[-channels2]);
      if (++chan == channels) {
        chan = 0;
      }
...
DXDEC void AILCALL AIL_mem_free_lock(void FAR *ptr) { AudioMemFree(ptr); }
