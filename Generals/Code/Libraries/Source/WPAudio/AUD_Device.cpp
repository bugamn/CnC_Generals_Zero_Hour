...
static AudioDevice *audioCreateDevice(AudioSystem *system, int unit,
                                      AudioFormat *format);
...
static AudioAttribsNode *audioDeviceAttribsListFind(AudioDevice *idev,
                                                    AudioAttribs *attribs);
...
static void audioDeviceAttribsListUpdate(AudioDevice *idev) {
  AudioAttribsNode *node =
      (AudioAttribsNode *)ListFirstItem(&idev->attribsList);
...
static AudioDevice *audioCreateDevice(AudioSystem *system, int unit,
  ...
  AudioServiceSetMustServiceInterval(&dev->attribsUpdate,
                                     AUDIO_DEFAULT_SERVICE_INTERVAL * 5);
  AudioServiceSetResetInterval(&dev->attribsUpdate,
                               AUDIO_DEFAULT_SERVICE_INTERVAL * 5);
...
static int audioDeviceHandler(AudioDevice *dev) {
  ...
  if (!dev->GroupAttribs ||
      !(change = AudioAttribsChanged(dev->GroupAttribs))) {
...
AudioSystem *AudioLoadSystem(AudioSystemMaster *master) {
  ...
  DBG_ASSERT(!(master->flags &
               mAUDIO_SYSTEM_MASTER_LOADED));  //  master was already loaded
...
void AudioUnloadSystem(AudioSystem *system) {
  ...
  DBG_ASSERT(!Locked(&system->lock));  //  trying to unload a system while it is
                                       //  still being used
...
AudioSystem *AudioFirstSystem(AudioSearch *sh) {
  ...
  DBG_CODE(
      if (sh->item) { DBG_ASSERT_TYPE((AudioSystem *)sh->item, AudioSystem); })
...
AudioDevice *AudioFirstDevice(AudioSearch *sh) {
  ...
  DBG_CODE(
      if (sh->item) { DBG_ASSERT_TYPE((AudioDevice *)sh->item, AudioDevice); })
...
AudioChannel *AudioDeviceGetChannel(AudioDevice *dev, AudioChannelType type) {
  ...
  AudioSearch sh;
  int lowestPriorityPlaying, lowestPriorityPaused;
  AudioChannel *lowestPlaying, *lowestPaused;
  ...
  if (type == AUDIO_CHANNEL_TYPE_ANY) {
    //	you must know what type of channel you want so AUDIO_CHANNEL_ANY is
    //taken 	to mean a standard channel
  ...
  while (channel) {
    if (channel->Type == type) {
      if (!(FLAGS_CHECK(channel->Control.Status,
                        mAUDIO_CTRL_ACTIVE | mAUDIO_CTRL_INUSE))) {
  ...
  {
    if (!(channel = lowestPaused) ||
        (lowestPriorityPaused > lowestPriorityPlaying)) {
...
AudioChannel *AudioDeviceReserveChannel(AudioDevice *dev,
                                        AudioChannelType newType) {
  AudioChannel *chan;
  ...
  DBG_ASSERT(
      newType !=
      AUDIO_CHANNEL_TYPE_ALL);  //  this does not make sense, think about it
...
void AudioServiceSetMustServiceInterval(AudioServiceInfo *asi,
                                        TimeStamp interval) {
  DBG_ASSERT_TYPE(asi, AudioServiceInfoStruct);
...
void AudioServicePerform(AudioServiceInfo *asi, TimeStamp now) {
  ...
  if (asi->longestInterval > asi->longestReset) {
    asi->longestInterval =
        0;  //  reset the longest count if it is extremely long
  }
...
void AudioServiceInfoDump(AudioServiceInfo *asi, char *string) {
  ...
  sprintf(string, "%05dms (%04d,%04d,%04d,~%2d) %c \n",
          (int)asi->serviceInterval, (int)asi->lastInterval,
          (int)asi->longestIntervalforPeriod, (int)asi->longestInterval,
          (int)asi->missCount, anim[asi->animPos]);
}
...
void AudioDeviceDump(AudioDevice *dev, void (*print)(char *), int names) {
  ...
  sprintf(string, "%s (%d)\n", dev->system->master->Name, dev->Unit);
  print(string);
    ...
    if (format->Compression == AUDIO_COMPRESS_MP3) {
      ...
      sprintf(temp, "%2d",
              format->Compression == AUDIO_COMPRESS_NONE
                  ? format->SampleWidth * 8
                  : 4);
    }

    sprintf(fstr, "%02d-%s-%c%c%c", format->Rate / 1000, temp,
            format->Channels == 1 ? 'm' : 's', chan->format_changed ? '*' : ' ',
            chan->drv_format_changed ? '+' : ' ');

    // sprintf ( string, "Chan:TYPE: STATE : FORMAT :PRI :VOL:\n");
    sprintf(string, "#%2d :%s:%s:%s:%4d:%3d:%.31s\n", count, type, state, fstr,
            chan->Control.Priority,
            AudioLevelApply(&chan->attribs.VolumeLevel, 100), name);
