...
const char *CompressionManager::getCompressionNameByType(
...
const char *CompressionManager::getDecompressionNameByType(
    CompressionType compType) {
...
CompressionType CompressionManager::getCompressionType(const void *mem,
                                                       Int len) {
  if (len < 8) return COMPRESSION_NONE;

  if (memcmp(mem, "NOX\0", 4) == 0) return COMPRESSION_NOXLZH;

  if (memcmp(mem, "ZL1\0", 4) == 0) return COMPRESSION_ZLIB1;
  if (memcmp(mem, "ZL2\0", 4) == 0) return COMPRESSION_ZLIB2;
  if (memcmp(mem, "ZL3\0", 4) == 0) return COMPRESSION_ZLIB3;
  if (memcmp(mem, "ZL4\0", 4) == 0) return COMPRESSION_ZLIB4;
  if (memcmp(mem, "ZL5\0", 4) == 0) return COMPRESSION_ZLIB5;
  if (memcmp(mem, "ZL6\0", 4) == 0) return COMPRESSION_ZLIB6;
  if (memcmp(mem, "ZL7\0", 4) == 0) return COMPRESSION_ZLIB7;
  if (memcmp(mem, "ZL8\0", 4) == 0) return COMPRESSION_ZLIB8;
  if (memcmp(mem, "ZL9\0", 4) == 0) return COMPRESSION_ZLIB9;
  if (memcmp(mem, "EAB\0", 4) == 0) return COMPRESSION_BTREE;
  if (memcmp(mem, "EAH\0", 4) == 0) return COMPRESSION_HUFF;
  if (memcmp(mem, "EAR\0", 4) == 0) return COMPRESSION_REFPACK;
...
Int CompressionManager::compressData(CompressionType compType, void *srcVoid,
                                     Int srcLen, void *destVoid, Int destLen) {
  if (destLen < 8) return 0;
...
Int CompressionManager::decompressData(void *srcVoid, Int srcLen,
                                       void *destVoid, Int destLen) {
  ...
  if (compType >= COMPRESSION_ZLIB1 && compType <= COMPRESSION_ZLIB9) {
    ...
    if (err == Z_OK || err == Z_STREAM_END) {
      ...
      DEBUG_LOG(
          ("ZLib decompression error (src is level %d, %d bytes long) %d\n",
           level, srcLen, err));
      return 0;
    }
  }
...
void DoCompressTest(void) {
  ...
  for (i = 0; i < COMPRESSION_MAX + 1; ++i) {
    s_compressGathers[i] = new PerfGather(
        CompressionManager::getCompressionNameByType((CompressionType)i));
    s_decompressGathers[i] = new PerfGather(
        CompressionManager::getDecompressionNameByType((CompressionType)i));
  }
  ...
  while (it != TheMapCache->end()) {
    ...
    if (f) {
      ...
      for (i = COMPRESSION_MIN; i <= COMPRESSION_MAX; ++i) {
        ...
        Int maxCompressedSize = CompressionManager::getMaxCompressedSize(
            origSize, (CompressionType)i);
        ...
        for (Int j = 0; j < NUM_TIMES; ++j) {
          ...
          compressedLen = CompressionManager::compressData(
              (CompressionType)i, buf, origSize, compressedBuf,
              maxCompressedSize);
          s_compressGathers[i]->stopTimer();
          s_decompressGathers[i]->startTimer();
          decompressedLen = CompressionManager::decompressData(
              compressedBuf, compressedLen, uncompressedBuf, origSize);
        ...
        if (decompressedLen == origSize) {
          ...
          if (ret != 0) {
            DEBUG_CRASH(
                ("orig buffer does not match compressed+uncompressed output - "
                 "ret was %d\n",
                 ret));
          }
        }
  ...
  for (i = COMPRESSION_MIN; i <= COMPRESSION_MAX; ++i) {
    ...
    for (std::map<AsciiString, CompData>::iterator cd = s_sizes.begin();
         cd != s_sizes.end(); ++cd) {
    ...
    DEBUG_LOG(("Min ratio: %g%%, Max ratio: %g%%\n", minCompression * 100.0f,
               maxCompression * 100.0f));
    DEBUG_LOG(("\n"));
  }
