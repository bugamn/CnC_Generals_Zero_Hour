...
static void HUFF_writebits(struct HuffEncodeContext *EC,
                           struct HUFFMemStruct *dest, unsigned int bitpattern,
                           unsigned int len) {
  if (len > 16) {
...
static void HUFF_treechase(struct HuffEncodeContext *EC, unsigned int node,
                           unsigned int bits) {
  if (node < HUFFCODES)
...
static int HUFF_minrep(struct HuffEncodeContext *EC, unsigned int remaining,
                       unsigned int r) {
  ...
  if (r) {
    ...
    if (EC->count[EC->clue + r]) {
      ...
      min1 = HUFF_minrep(EC, newremaining, r - 1) +
             EC->bitsarray[EC->clue + r] * use;
...
static void HUFF_writenum(struct HuffEncodeContext *EC,
                          struct HUFFMemStruct *dest, unsigned int num) {
  unsigned int dphuf;
  unsigned int dbase;
...
static void HUFF_writeexp(struct HuffEncodeContext *EC,
                          struct HUFFMemStruct *dest, unsigned int code) {
...
static void HUFF_analysis(struct HuffEncodeContext *EC, unsigned int opt,
  ...
  */
  ...
  while (bptr1 < EC->bufptr) {
    ...
    if (i == i1) {
      i2 = 0;
      bptr2 = bptr1 + 30000;
      if (bptr2 > (EC->bufptr)) bptr2 = EC->bufptr;
  ...
  */
  ...
  for (i = 0; i < HUFFCODES; ++i) {
    i1 = i;
    i2 = 0;
    if (EC->count[i] < EC->count[i3]) i3 = i;
    ...
    for (i = 1; i < EC->clues; ++i) {
      ...
      if (EC->count[EC->clue + i]) {
        ...
        if ((i1 <= EC->bitsarray[EC->clue + i]) ||
            (EC->count[EC->clue + i] * (i1 - EC->bitsarray[EC->clue + i]) <
             (i / 2))) {
    ...
    if (EC->dclues) {
      ...
      if (di <= EC->maxdelta) {
        if (di >= EC->mindelta) {
          ...
          if (i < i1) di = (i1 - i - 1) * 2 + EC->dclue + 1;
          if (count2[di] > thres) {
            if (count2[i] < 4) ++i3;
            if (EC->bitsarray[di] < EC->bitsarray[i]) ++i3;
            if (EC->bitsarray[di] == EC->bitsarray[i])
              if (EC->count[di] > EC->count[i]) ++i3;
          }
        }
      }
      if (i3)
  ...
  */
  ...
  while (i1 > chainsaw) {
    ...
    if (i1 > chainsaw) {
      ...
      while (i1 < HUFFCODES) {
        if (EC->count[i1]) {
          if (EC->bitsarray[i1] < chainsaw) break;
        }
        ++i1;
      }
      ...
      {
        if (EC->count[i]) {
          if ((EC->bitsarray[i] < chainsaw) &&
              (EC->bitsarray[i] > EC->bitsarray[i1]))
            i1 = i;
        }
      }
  ...
  */
  ...
  if ((~opt) & 8) {
    i1 = 8;
    for (i = 0; i < HUFFCODES; ++i) EC->bitsarray[i] = i1;
  }
  ...
  */
  ...
  for (i = 0; i <= HUFFMAXBITS; ++i) EC->bitnum[i] = 0;
  for (i = 0; i < HUFFCODES; ++i) {
    if (EC->bitsarray[i] <= HUFFMAXBITS) ++EC->bitnum[EC->bitsarray[i]];
  }
  ...
  for (i2 = 1; i2 <= HUFFMAXBITS; ++i2) {
    if (EC->bitnum[i2]) {
      for (i1 = 0; i1 < HUFFCODES; ++i1)
        if (EC->bitsarray[i1] == i2) EC->sortptr[i++] = i1;
      i3 = i2;
    }
  }
  EC->mostbits = i3;
...
static void HUFF_pack(struct HuffEncodeContext *EC, struct HUFFMemStruct *dest,
                      unsigned int opt) {
  unsigned char *bptr1;
  unsigned char *bptr2;
  unsigned int i;
  ...
  if (uptype == 38) {
    if (uptype == 34) {
      ...
      ibits = 0;
      if ((opt & 16) && (!EC->chainused)) ibits = 1;
      ...
      if (!ibits) HUFF_writenum(EC, dest, (unsigned int)EC->mostbits);
    } else {
    ...
    for (i = 0; i < HUFFCODES; ++i) EC->qleapcode[i] = 0;
  ...
  if (!EC->clues) EC->clue = HUFFBIGNUM;
  ...
  */
  ...
  while (bptr1 < EC->bufptr) {
    ...
    if (i == i1) {
      ...
      if ((ncode <= repn) && (ncode <= irep)) {
        while (i2--) HUFF_writecode(EC, dest, i1);
      } else {
        if (repn < irep) {
          HUFF_writebits(EC, dest, EC->patternarray[EC->clue],
                         EC->bitsarray[EC->clue]);
          ...
          while (i3) {
            if (EC->count[EC->clue + i3]) {
              ...
              irep = irep + rep1 * EC->bitsarray[EC->clue + i3];
              remaining = remaining - rep1 * i3;
              while (rep1--) HUFF_writecode(EC, dest, EC->clue + i3);
            }
            --i3;
          }
        }
    ...
    i1 = i;
    if (!i3) HUFF_writecode(EC, dest, i);
...
static int HUFF_packfile(struct HuffEncodeContext *EC,
                         ...
                         struct HUFFMemStruct *outfile, int ulen, int deltaed) {
  unsigned int i;
  ...
  chainsaw = 15;
  EC->masks[0] = 0;
  for (i = 1; i < 17; ++i) EC->masks[i] = (EC->masks[i - 1] << 1) + 1;
...
int GCALL HUFF_encode(void *compresseddata, const void *source, int sourcesize,
                      int *opts) {
  int plen = 0;
  struct HUFFMemStruct infile;
