...
static unsigned int matchlen(unsigned char *s, unsigned char *d,
...
#define HASH(cptr)                                     \
  (int)((((unsigned int)(unsigned char)cptr[0] << 8) | \
         ((unsigned int)(unsigned char)cptr[2])) ^     \
        ((unsigned int)(unsigned char)cptr[1] << 4))

static int refcompress(unsigned char *from, int len, unsigned char *dest,
                       int maxback, int quick) {
  unsigned int tlen;
  unsigned int tcost;
  ...
  if ((unsigned int)maxback > (unsigned int)131071) maxback = 131071;
  ...
  while (len >= 0) {
    ...
    //        ccost = 0;
    //        if ((run<4) && ((run+blen)>=4))
    //            ccost = 1;  // extra packet cost to switch out of literal into
    //            reference
    ...
    if (bcost >= blen || len < 4) {
      ...
      {
        *to++ = (unsigned char)(0xc0 + ((boffset >> 16) << 4) +
                                (((blen - 5) >> 8) << 2) + run);
  ...
  *to++ =
      (unsigned char)(0xfc + run); /* end of stream command + 0..3 literal */
  if (run) {
    memcpy(to, rptr, run);
    to += run;
  }
...
int GCALL REF_encode(void *compresseddata, const void *source, int sourcesize,
                     int *opts) {
  int maxback = 131072;
  int quick = 0;
  int plen;
  ...
  plen = hlen + refcompress((unsigned char *)source, sourcesize,
                            (unsigned char *)compresseddata + hlen, maxback,
                            quick);
  return (plen);
}
