...
static void BTREE_writebits(struct BTreeEncodeContext *EC,
                            ...
                            unsigned int bitpattern, unsigned int len) {
  if (len > 16) {
...
static void BTREE_adjcount(unsigned char *s, unsigned char *bend,
                           BTREEWORD *count) {
#ifdef __WATCOMC__
...
static void BTREE_joinnodes(struct BTreeEncodeContext *EC, unsigned char *cluep,
                            unsigned char *rightp, unsigned char *joinp,
                            unsigned int clue) {
  unsigned char *s;
  unsigned char *d;
  unsigned char *bend;
...
static unsigned int BTREE_findbest(BTREEWORD *countptr, unsigned char *tryq,
                                   unsigned int *bestn, unsigned int *bestval,
                                   int ratio) {
  unsigned int i;
  unsigned int i1;
  unsigned int i2;
      ...
      for (i1 = 0; i1 < 256; ++i1) {
        if (*countptr++ > (BTREEWORD)i) {
          if (tryq[i1]) {
            ...
            while (bestval[bestsize - 1] < (bestval[1] / ratio)) --bestsize;
            if (bestsize < 48)
              i = bestval[1] / ratio;
            else
...
static void BTREE_treepack(struct BTreeEncodeContext *EC,
                           struct BTREEMemStruct *dest, unsigned int passes,
                           unsigned int multimax, unsigned int quick,
                           int zerosuppress) {
  unsigned char *bend;
  unsigned char *ptr1;
  ...
  if (!treebuf) return; /* failure Insufficient memory for work buffer */

  EC->buf1 = (unsigned char *)galloc(buf1size);
  if (!EC->buf1) return; /* failure Insufficient memory for work buffer */

  EC->buf2 = (unsigned char *)galloc(buf2size);
  if (!EC->buf2) return; /* failure Insufficient memory for work buffer */
  ...
  for (i = 0; i < BTREECODES; ++i) count2[i] = 0;
  ...
  for (i = 0; i < BTREECODES; ++i) EC->clueq[i] = 0;
  ...
  */
  ...
  for (i = 0; i < BTREECODES; ++i) sortptr[i] = i;
  ...
  if (count2[i]) BTREE_joinnodes(EC, EC->clueq, EC->right, EC->join, clue);
  ...
  while (ptr1 < bend) BTREE_writebits(EC, dest, (unsigned int)*ptr1++, 8);
...
static int BTREE_compressfile(struct BTreeEncodeContext *EC,
                              ...
                              struct BTREEMemStruct *outfile, int ulen,
                              int zerosuppress) {
  unsigned int i;
...
int GCALL BTREE_encode(void *compresseddata, const void *source, int sourcesize,
                       int *opts) {
  int plen;
  struct BTREEMemStruct infile;
