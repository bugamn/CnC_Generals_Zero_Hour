...
static const char *messageToString(unsigned int message) {
  ...
  switch (message) {
    ...
    case WM_INPUTLANGCHANGEREQUEST:
      return "WM_INPUTLANGCHANGEREQUES";
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam,
  ...
  try {
    ...
    char testString[256];
    sprintf(testString, "\n%d: %s (%X,%X)", msgCount++,
            messageToString(message), wParam, lParam);
    ...
    switch (message) {
      ...
      case WM_NCHITTEST:
        ...
        if (!ApplicationIsWindowed) return HTCLIENT;
        break;
...
#define PBT_APMRESUMESUSPEND 0x0007
      ...
      case WM_SYSCOMMAND:
        ...
        switch (wParam) {
          ...
          case SC_MONITORPOWER:
            if (FALSE == ApplicationIsWindowed) return 1;
            break;
        }
        break;
      ...
      case WM_SETFOCUS: {
        ...
        if (TheWin32Mouse) TheWin32Mouse->lostFocus(FALSE);
      ...
      case WM_SIZE:
        ...
        if (!gInitializing) gDoPaint = false;
        break;

      //-------------------------------------------------------------------------
      case WM_KILLFOCUS: {
        if (TheKeyboard) TheKeyboard->resetKeys();
        if (TheWin32Mouse) TheWin32Mouse->lostFocus(TRUE);
      ...
      case WM_ACTIVATEAPP: {
        ...
        if ((bool)wParam != isWinMainActive) {
          ...
          if (TheGameEngine) TheGameEngine->setIsActive(isWinMainActive);

          Reset_D3D_Device(isWinMainActive);
          if (isWinMainActive) {  // restore mouse cursor to our custom version.
            if (TheWin32Mouse)
      ...
      case WM_ACTIVATE: {
        ...
        if (active == WA_INACTIVE) {
          ClipCursor(NULL);
          if (TheAudio) TheAudio->loseFocus();
        }  // end if
        else {
          if (TheMouse) TheMouse->setMouseLimits();

          if (TheAudio) TheAudio->regainFocus();
      ...
      case WM_SETCURSOR: {
        if (TheWin32Mouse && (HWND)wParam == ApplicationHWnd)
          TheWin32Mouse->setCursor(TheWin32Mouse->getMouseCursor());
        return TRUE;  // tell Windows not to reset mouse cursor image to
                      // default.
      }
      ...
      case WM_PAINT: {
        if (gDoPaint) {
          ...
          if (gLoadScreenBitmap != NULL) {
            ...
            HBITMAP savBitmap =
                (HBITMAP)::SelectObject(tmpDC, gLoadScreenBitmap);
            ::BitBlt(dc, 0, 0, DEFAULT_XRESOLUTION, DEFAULT_YRESOLUTION, tmpDC,
                     0, 0, SRCCOPY);
      ...
      case WM_ERASEBKGND: {
        if (!gDoPaint)
          return TRUE;  // we don't need to erase the background because we
                        // always draw entire window.
        break;
      }
...
// to trap a different device change (for instance, removal of a mouse) - jkmcd
    ...
    RELEASE_CRASH((
        "Uncaught exception in Main::WndProc... probably should not happen\n"));
    // no rethrow
  }

  // In full-screen mode, only pass these messages onto the default windows
  // handler. Appears to fix issues with dual monitor systems but doesn't seem
  // safe?
...
//=============================================================================
static Bool initializeAppWindows(HINSTANCE hInstance, Int nCmdShow,
                                 Bool runWindowed) {
  DWORD windowStyle;
  ...
  HWND hWnd = CreateWindow(
      TEXT("Game Window"), TEXT("Command and Conquer Generals"), windowStyle,
      (GetSystemMetrics(SM_CXSCREEN) / 2) -
          (startWidth / 2),  // original position X
      (GetSystemMetrics(SM_CYSCREEN) / 2) -
          (startHeight / 2),  // original position Y
      // Lorenzen nudged the window higher
      // so the constantdebug report would
      // not get obliterated by assert windows, thank you.
      //(GetSystemMetrics( SM_CXSCREEN ) / 2) - (startWidth / 2),   //this works
      //with any screen res (GetSystemMetrics( SM_CYSCREEN ) / 25) -
      //(startHeight / 25),//this works with any screen res
      rect.right - rect.left, rect.bottom - rect.top, 0L, 0L, hInstance, 0L);
...
void munkeeFunc(void) { CDAPFN_ENDMARK(munkeeFunc); }
  ...
  try {
    ...
    */
    ...
    while (argc < 20 && token != NULL) {
      ...
      // added a preparse step for this flag because it affects window creation
      // style
      if (stricmp(token, "-win") == 0) ApplicationIsWindowed = true;
    ...
    // Force to be loaded from a file, not a resource so same exe can be used in
    // germany and retail.
    gLoadScreenBitmap =
        (HBITMAP)LoadImage(hInstance, "Install_Final.bmp", IMAGE_BITMAP, 0, 0,
                           LR_SHARED | LR_LOADFROMFILE);

    // register windows class and create application window
    if (initializeAppWindows(hInstance, nCmdShow, ApplicationIsWindowed) ==
        false)
      return 0;
