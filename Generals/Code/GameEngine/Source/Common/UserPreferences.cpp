...
Bool UserPreferences::getBool(AsciiString key, Bool defaultValue) const {
  ...
  val.toLower();
  return (val == "1" || val == "t" || val == "true" || val == "y" ||
          val == "yes" || val == "ok");
}
...
void QuickMatchPreferences::setMapSelected(const AsciiString& mapName,
                                           Bool selected) {
...
Bool QuickMatchPreferences::isMapSelected(const AsciiString& mapName) {
  Int ret;
  QuickMatchPreferences::const_iterator it =
      find(AsciiStringToQuotedPrintable(mapName));
  if (it == end()) {
    return TRUE;
  }
...
void QuickMatchPreferences::setLastLadder(const AsciiString& addr,
                                          UnsignedShort port) {
  AsciiString strVal;
...
void QuickMatchPreferences::setColor(Int val) { setInt("Color", val); }

Int QuickMatchPreferences::getColor(void) { return getInt("Color", 0); }

void QuickMatchPreferences::setSide(Int val) { setInt("Side", val); }

Int QuickMatchPreferences::getSide(void) { return getInt("Side", 0); }
...
Bool CustomMatchPreferences::getDisallowAsianText(void) {
  ...
  if (it == end()) {
    // since English Win98 machines don't have a Unicode font installed by
    // default, we're forced to disable asian chat by default for English
    // builds.
    if (GetRegistryLanguage().compareNoCase("chinese") == 0 ||
        GetRegistryLanguage().compareNoCase("korean") == 0)
      return FALSE;
    else
      return TRUE;
  }
...
Int GameSpyMiscPreferences::getLocale(void) { return getInt("Locale", 0); }

void GameSpyMiscPreferences::setLocale(Int val) { setInt("Locale", val); }
...
void IgnorePreferences::setIgnore(const AsciiString& userName, Int profileID,
                                  Bool ignore) {
  AsciiString strVal;
...
bool LadderPreferences::write(void) {
  ...
  for (lpIt = m_ladders.begin(), count = 0;
       lpIt != m_ladders.end() && count < MAX_LADDERS; ++lpIt, ++count) {
    LadderPref p = lpIt->second;
    AsciiString ladName;
    AsciiString ladData;
    ladName.format("%s:%d", AsciiStringToQuotedPrintable(p.address).str(),
                   p.port);
    ladData.format("%s:%d", UnicodeStringToQuotedPrintable(p.name).str(),
                   p.lastPlayDate);
...
void LadderPreferences::addRecentLadder(LadderPref ladder) {
  for (LadderPrefMap::iterator it = m_ladders.begin(); it != m_ladders.end();
       ++it) {
    if (it->second == ladder) {
      m_ladders.erase(it);
      break;
