...
void StatsCollector::collectMsgStats(const GameMessage *msg) {
  ...
  if (ThePlayerList->getLocalPlayer()->getPlayerIndex() !=
      msg->getPlayerIndex())
    return;
...
void StatsCollector::update(void) {
  if (m_lastUpdate + (TheGlobalData->m_playStats * LOGICFRAMES_PER_SECOND) >
...
void StatsCollector::incrementScrollMoveCount(void) { ++m_scrollMapCommands; }

void StatsCollector::incrementAttackCount(void) { ++m_attackCommands; }

void StatsCollector::incrementBuildCount(void) { ++m_buildCommands; }
void StatsCollector::incrementMoveCount(void) { ++m_moveCommands; }
...
void StatsCollector::writeFileEnd(void) {
  ...
  if (!f) {
    DEBUG_ASSERTCRASH(
        f, ("Unable to open file %s to write", m_statsFileName.str()));
    return;
  }

  m_timeCount +=
      (TheGameLogic->getFrame() - m_lastUpdate) / LOGICFRAMES_PER_SECOND;
  writeStatInfo();
  fprintf(f, "---------------------------------------------------\n");
  ...
  fprintf(f, "===================================================\n");
  fprintf(f,
          "* Times are in Game Seconds which are based off of frames. Current "
          "fps is set to %d\n",
  ...
  if (TheGlobalData->m_benchmarkTimer > 0) {
    ...
    fprintf(f, "    FPS = %.2f\n",
            ((Real)TheGameLogic->getFrame() - (Real)m_startFrame) /
                (Real)TheGlobalData->m_benchmarkTimer);
  }
#endif
  ...
  if (TheGlobalData->m_saveStats) {
    ...
    if (TheNetwork) {
      if (TheLAN) {
        ...
        full.format("%s%s_%d_%d", players.str(), name.str(), game->getSeed(),
...
void StatsCollector::writeInitialFileInfo() {
  ...
  fprintf(f, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n", 0, m_buildCommands,
          m_moveCommands, m_attackCommands, m_scrollMapCommands, 0,
          /*other commands*/ 0, m->countMoney(), m_playerUnits, m_AIUnits);
  // initial stats
  // we don't want a file pointer open for seconds on end... we'll open it each
  // time.
  fclose(f);
}
...
void StatsCollector::writeStatInfo() {
  ...
  fprintf(f, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n", m_timeCount,
          m_buildCommands, m_moveCommands, m_attackCommands,
          m_scrollMapCommands, m_scrollTime / LOGICFRAMES_PER_SECOND,
          /*other commands*/ 0, m->countMoney(), m_playerUnits, m_AIUnits);
