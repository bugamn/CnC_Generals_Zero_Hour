...
XferSave::~XferSave(void) {
  ...
  if (m_fileFP != NULL) {
    DEBUG_CRASH(
        ("Warning: Xfer file '%s' was left open\n", m_identifier.str()));
    close();

  }  // end if

  //
  // the block stack should be empty, if it's not that means we started blocks
  // but never called enough matching end blocks
  //
  if (m_blockStack != NULL) {
    // tell the user there is an error
    DEBUG_CRASH(
        ("Warning: XferSave::~XferSave - m_blockStack was not NULL!\n"));
...
//-------------------------------------------------------------------------------------------------
/** Write a placeholder at the current location in the file and store this
 * location internally.  The next endBlock that is called will back up to the
 * most recently stored beginBlock and write the difference in file bytes from
 * the endBlock call to the location of this beginBlock.  The current file
 * position will then return to the location at which endBlock was called */
//-------------------------------------------------------------------------------------------------
Int XferSave::beginBlock(void) {
  // sanity
  DEBUG_ASSERTCRASH(m_fileFP != NULL,
                    ("Xfer begin block - file pointer for '%s' is NULL\n",
                     m_identifier.str()));

  // get the current file position so we can back up here for the next end block
  // call
  ...
  //	{
  //
  //		DEBUG_CRASH(( "XferSave - Begin block, out of memory - can't
  //save block stack data\n" )); 		return XFER_OUT_OF_MEMORY;
  //
  //	}  // end if
...
//-------------------------------------------------------------------------------------------------
/** Do the tail end as described in beginBlock above.  Back up to the last begin
 * block, write the file difference from current position to the last begin
 * position, and put current file position back to where it was */
//-------------------------------------------------------------------------------------------------
void XferSave::endBlock(void) {
  // sanity
  DEBUG_ASSERTCRASH(
      m_fileFP != NULL,
      ("Xfer end block - file pointer for '%s' is NULL\n", m_identifier.str()));

  // sanity, make sure we have a block started
  if (m_blockStack == NULL) {
    DEBUG_CRASH(
        ("Xfer end block called, but no matching begin block was found\n"));
  ...
  // write the size in bytes between the block position and what is our current
  // file position
  XferBlockSize blockSize =
      currentFilePos - top->filePos - sizeof(XferBlockSize);
  if (fwrite(&blockSize, sizeof(XferBlockSize), 1, m_fileFP) != 1) {
    DEBUG_CRASH(
        ("Error writing block size to file '%s'\n", m_identifier.str()));
...
void XferSave::skip(Int dataSize) {
  ...
  DEBUG_ASSERTCRASH(
      m_fileFP != NULL,
      ("XferSave - file pointer for '%s' is NULL\n", m_identifier.str()));
...
void XferSave::xferAsciiString(AsciiString *asciiStringData) {
  ...
  if (asciiStringData->getLength() > 255) {
    DEBUG_CRASH(
        ("XferSave cannot save this unicode string because it's too long.  "
         "Change the size of the length header (but be sure to preserve save "
         "file compatability\n"));
  ...
  // save string data
  if (len > 0) xferUser((void *)asciiStringData->str(), sizeof(Byte) * len);
...
void XferSave::xferImplementation(void *data, Int dataSize) {
  ...
  if (fwrite(data, dataSize, 1, m_fileFP) != 1) {
    DEBUG_CRASH(
        ("XferSave - Error writing to file '%s'\n", m_identifier.str()));
    throw XFER_WRITE_ERROR;
