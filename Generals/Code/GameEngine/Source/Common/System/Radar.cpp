...
Bool RadarObject::isTemporarilyHidden() const {
  ...
  if (draw->getStealthLook() == STEALTHLOOK_INVISIBLE ||
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void RadarObject::crc(Xfer *xfer) {}  // end crc
...
void RadarObject::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_LOAD) {
    ...
    if (m_object == NULL) {
      DEBUG_CRASH(
          ("RadarObject::xfer - Unable to find object for radar data\n"));
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void RadarObject::loadPostProcess(void) {}  // end loadPostProcess
...
void Radar::update(void) {
  ...
  //
  // traverse the radar event list, if an event has a creationFrame it means
  // that it exists ... check to see if it's time for that event to die and if
  // so, just clear out the active status
  //
  ...
  if (m_queueTerrainRefreshFrame != 0 &&
      TheGameLogic->getFrame() - m_queueTerrainRefreshFrame >
          RADAR_QUEUE_TERRAIN_REFRESH_DELAY) {
    // refresh the terrain
...
void Radar::newMap(TerrainLogic *terrain) {
  ...
  DEBUG_ASSERTCRASH(m_radarWindow,
                    ("Radar::newMap - Unable to find radar game window\n"));
  ...
  if (terrainSamples == 0) terrainSamples = 1;
  if (waterSamples == 0) waterSamples = 1;
...
//-------------------------------------------------------------------------------------------------
/** Add an object to the radar list.  The object will be sorted in the list to
 * be grouped using it's radar priority */
//-------------------------------------------------------------------------------------------------
void Radar::addObject(Object *obj) {
  // get the radar priority for this object
  RadarPriorityType newPriority = obj->getRadarPriority();
  if (isPriorityVisible(newPriority) == FALSE) return;
  ...
  if (obj->isKindOf(KINDOF_DISGUISER)) {
    // Because we have support for disguised units pretending to be units from
    // another team, we need to intercept it here and make sure it's rendered
    // appropriately based on which client is rendering it.
    ...
    if (update) {
      if (update->isDisguised()) {
        ...
        if (player->getRelationship(clientPlayer->getDefaultTeam()) != ALLIES &&
            clientPlayer->isPlayerActive()) {
          // Neutrals and enemies will see this disguised unit as the team it's
          // disguised as.
          player = disguisedPlayer;
          if (player) useIndicatorColor = false;
        }
  ...
  if (obj->getContain()) {
    // To handle Stealth garrison, ask containers what color they are drawing
    // with to the local player. Local is okay because radar display is not
    // synced.
    player = obj->getContain()->getApparentControllingPlayer(
  ...
  //
  // we will put this on either the local object list for objects that belong to
  // the local player, or on the regular object list for all other objects
  //
  ...
  if (*list == NULL)
    *list = newObj;  // trivial case, an empty list
    ...
    for (currObject = *list; currObject; currObject = nextObject) {
      ...
      //
      // if there is no previous object, or the previous priority is less than
      // the our new priority, and the current object in the list has a priority
...
Bool Radar::deleteFromList(Object *obj, RadarObject **list) {
  ...
  for (radarObject = *list; radarObject;
...
void Radar::removeObject(Object *obj) {
  // sanity
  if (obj->friend_getRadarData() == NULL) return;
  ...
  if (deleteFromList(obj, &m_localObjectList) == TRUE)
    ...
    // sanity
    DEBUG_ASSERTCRASH(
        0, ("Radar: Tried to remove object '%s' which was not found\n",
            obj->getTemplate()->getName().str()));
...
//-------------------------------------------------------------------------------------------------
/** Translate a 2D spot on the radar (from (0,0) to
 * (RADAR_CELL_WIDTH,RADAR_CELL_HEIGHT) to a 3D spot in the world on the terrain
...
Bool Radar::radarToWorld(const ICoord2D *radar, Coord3D *world) {
  ...
  // sanity
  if (radar == NULL || world == NULL) return FALSE;
  ...
  // more sanity
  if (x < 0) x = 0;
  if (x >= RADAR_CELL_WIDTH) x = RADAR_CELL_WIDTH - 1;
  if (y < 0) y = 0;
  if (y >= RADAR_CELL_HEIGHT) y = RADAR_CELL_HEIGHT - 1;
...
Bool Radar::worldToRadar(const Coord3D *world, ICoord2D *radar) {
  // sanity
  if (world == NULL || radar == NULL) return FALSE;

  // sanity check the world position
  //	if( world->x < m_mapExtent.lo.x || world->x > m_mapExtent.hi.x ||
  //			world->y < m_mapExtent.lo.y || world->y >
  //m_mapExtent.hi.y ) 		return FALSE;
  // This is actually an insanity check.  Nobody uses the return value, so this
  // just leaves garbage in the return pointer.  The reason the question gets
  // asked is there are 60 partition cells to 128 radar cells (for example), and
  // the radar wants to draw a horizontal line.  This line ends up three pixels
  // long at the right side, so the radar gives up and doesn't draw the middle
  // one. We bind to on radar anyway, and we only ask if we are on radar, so
  // don't intentionally add edge weirdness.
  ...
  // keep it in bounds
  if (radar->x < 0) radar->x = 0;
  if (radar->x >= RADAR_CELL_WIDTH) radar->x = RADAR_CELL_WIDTH - 1;
  if (radar->y < 0) radar->y = 0;
  if (radar->y >= RADAR_CELL_HEIGHT) radar->y = RADAR_CELL_HEIGHT - 1;
...
// ------------------------------------------------------------------------------------------------
/** Translate an actual pixel location (relative pixel with (0,0) being the top
 * left of the radar area) to the "logical" radar coords that would cover the
 * entire area of display on the screen.  This is needed because some maps are
 * "long" or "tall" and need a translation to any radar image that has been
 * scaled to preserve the map aspect ratio */
// ------------------------------------------------------------------------------------------------
Bool Radar::localPixelToRadar(const ICoord2D *pixel, ICoord2D *radar) {
  // sanity
  if (pixel == NULL || radar == NULL) return FALSE;
  ...
  //
  // act like we're going to draw and find the aspect ratio adjusted points of
  // the terrain radar positions
  //
  ICoord2D start = {0, 0};
  ICoord2D ul, lr;
  ...
  // if the pixel is outsize of the adjusted radar area there are no logical
  // coords
  ...
  if (scaledWidth >= scaledHeight) {
    ...
    radar->y =
        REAL_TO_INT(((pixel->y - ul.y) / INT_TO_REAL(scaledHeight)) * size.y);

    //
    // radar->y now refers to a point that was "as if" the map was square,
    // translate to radar note that y is inverted to have the radar align with
    // the world (+x = right, -y = down)
    //
    ...
    // conversion for scaled Y direction in map
    radar->x =
        REAL_TO_INT(((pixel->x - ul.x) / INT_TO_REAL(scaledWidth)) * size.x);

    // radar->x now refers to a point that was "as if" the map was square,
    // translate to radar
    radar->x = radar->x * RADAR_CELL_WIDTH / size.x;

    //
    // just normal conversion from full stretched to radar cells, note that y is
    // inverted to have the radar align with the world (+x = right, -y = down)
    //
...
// ------------------------------------------------------------------------------------------------
/** Translate a screen mouse position to world coords if the screen position is
 * within the radar window and that spot in the radar corresponds to a point in
 * the world */
// ------------------------------------------------------------------------------------------------
Bool Radar::screenPixelToWorld(const ICoord2D *pixel, Coord3D *world) {
  // sanity
  if (pixel == NULL || world == NULL) return FALSE;

  // if we have no radar window can't do the conversion
  if (m_radarWindow == NULL) return FALSE;
  ...
  ICoord2D radar;
  if (localPixelToRadar(&localPixel, &radar) == FALSE) return FALSE;
...
// ------------------------------------------------------------------------------------------------
/** Given the pixel coordinates, see if there is an object that is exactly in
 * this spot represented on the radar */
// ------------------------------------------------------------------------------------------------
Object *Radar::objectUnderRadarPixel(const ICoord2D *pixel) {
  ...
  ICoord2D radar;
  if (localPixelToRadar(pixel, &radar) == FALSE) return NULL;

  // object we will return
  Object *obj = NULL;

  //
  // scan the objects on the radar list and return any object that maps its
  // world location to the radar location
  //
  ...
  if (obj == NULL) obj = searchListForRadarLocationMatch(m_objectList, &radar);
...
// ------------------------------------------------------------------------------------------------
/** Search the object list for an object that maps to the given logical radar
 * coords */
// ------------------------------------------------------------------------------------------------
Object *Radar::searchListForRadarLocationMatch(RadarObject *listHead,
                                               ICoord2D *radarMatch) {
  // sanity
  if (listHead == NULL || radarMatch == NULL) return NULL;

  // scan the list
  RadarObject *radarObject;
  ICoord2D radar;
  for (radarObject = listHead; radarObject;
       radarObject = radarObject->friend_getNext()) {
    // get object
    ...
    if (obj == NULL) {
      DEBUG_CRASH(
          ("Radar::searchListForRadarLocationMatch - NULL object encountered "
           "in list\n"));
      continue;
    ...
    if (radar.x >= radarMatch->x - 1 && radar.x <= radarMatch->x + 1 &&
        radar.y >= radarMatch->y - 1 && radar.y <= radarMatch->y + 1)
      return obj;
...
// ------------------------------------------------------------------------------------------------
/** Given the RELATIVE SCREEN start X and Y, the width and height of the area to
 * draw the whole radar in, compute what the upper left (ul) and lower right
 * (lr) local coordinates are that represent the actual terrain image part of
 * the radar that will preserve the aspect ratio of the map */
// ------------------------------------------------------------------------------------------------
void Radar::findDrawPositions(Int startX, Int startY, Int width, Int height,
  ...
  /*
          ...
          if( m_mapExtent.width() > m_mapExtent.height() )
          {
                  ...
                  //
                  ul->x = 0;
                  ul->y = (height - (m_mapExtent.height() / m_mapExtent.width()
     * height)) / 2.0f; lr->x = width; lr->y = height - ul->y;
          ...
          else if(  m_mapExtent.height() > m_mapExtent.width() )
          {
                  ...
                  ul->x = (width - (m_mapExtent.width() / m_mapExtent.height() *
     width)) / 2.0f; ul->y = 0; lr->x = width - ul->x; lr->y = height;
...
//-------------------------------------------------------------------------------------------------
void Radar::createEvent(const Coord3D *world, RadarEventType type,
                        Real secondsToLive) {
  // sanity
  ...
  if (radarColorLookupTable[i].event == RADAR_EVENT_INVALID) {
    ...
    DEBUG_CRASH(
        ("Radar::createEvent - Event not found in color table, using default "
         "colors\n"));
    color[0] = color1;
    color[1] = color2;
...
void Radar::createPlayerEvent(Player *player, const Coord3D *world,
  ...
  if (color[1].red < 0) color[1].red = 0;
  color[1].green -= REAL_TO_INT(color[0].green * darkScale);
  if (color[1].green < 0) color[1].green = 0;
  color[1].blue -= REAL_TO_INT(color[0].blue * darkScale);
  if (color[1].blue < 0) color[1].blue = 0;
...
void Radar::internalCreateEvent(const Coord3D *world, RadarEventType type,
  ...
  static Real secondsBeforeDieToFade =
      0.5f;  ///< this many seconds before we hit the die frame we start to fade
             ///< away

  // sanity
  if (world == NULL || color1 == NULL || color2 == NULL) return;
  ...
  m_event[m_nextFreeRadarEvent].dieFrame =
      TheGameLogic->getFrame() + LOGICFRAMES_PER_SECOND * secondsToLive;
  m_event[m_nextFreeRadarEvent].fadeFrame =
      m_event[m_nextFreeRadarEvent].dieFrame -
  ...
  if (type != RADAR_EVENT_BEACON_PULSE) m_lastRadarEvent = m_nextFreeRadarEvent;

  //
  // increment the next radar event index, wrapping to the beginning.  If we
  // ever have so many events that they fill up the buffer the oldest ones will
  // just drop off, eh ... should be fine.
  //
  m_nextFreeRadarEvent++;
  if (m_nextFreeRadarEvent >= MAX_RADAR_EVENTS) m_nextFreeRadarEvent = 0;
...
Bool Radar::getLastEventLoc(Coord3D *eventPos) {
  ...
  if (m_lastRadarEvent != -1) {
    if (eventPos) *eventPos = m_event[m_lastRadarEvent].worldLoc;
    return TRUE;
...
// ------------------------------------------------------------------------------------------------
/** Try to create a radar event for "we're under attack".  This will be called
 * every time actual damage is dealt to an object that the player owns that
 * shows up on the radar. We don't want to create under attack events every time
 * we are damaged and we also want to limit them based on time and local area of
 * other recent attack events */
// ------------------------------------------------------------------------------------------------
void Radar::tryUnderAttackEvent(const Object *obj) {
  ...
  if (eventCreated) {
    ...
    //
    ///@todo Should make an INI data driven table for radar event strings, and
    ///audio events
    //
    // UI feedback for being under attack (note that we display these messages
    // and audio queues even if we don't have a radar)
    //
    ...
    if (obj->isKindOf(KINDOF_INFANTRY) || obj->isKindOf(KINDOF_VEHICLE)) {
      ...
      if (obj->isKindOf(KINDOF_HARVESTER)) {
        ...
        unitAttackSound =
            TheAudio->getMiscAudio()->m_radarHarvesterUnderAttackSound;
      } else {
        // display message
        ...
        // play audio event
        unitAttackSound =
            TheAudio->getMiscAudio()->m_radarStructureUnderAttackSound;
      }
      ...
      TheAudio->addAudioEvent(&unitAttackSound);
             ...
             obj->isKindOf(KINDOF_MP_COUNT_FOR_VICTORY)) {
      ...
      else if (ThePlayerList->getLocalPlayer()->getRelationship(
                   obj->getTeam()) == ALLIES)
      ...
      // play audio event
      static AudioEventRTS structureAttackSound =
          TheAudio->getMiscAudio()->m_radarStructureUnderAttackSound;
      ...
      // play audio event
      static AudioEventRTS underAttackSound =
          TheAudio->getMiscAudio()->m_radarStructureUnderAttackSound;
...
/** Try to create a radar event for "infiltration".
                This happens whenever a unit is hijacked, defected, converted to
   carbomb, hacked, or otherwise snuck into */
// ------------------------------------------------------------------------------------------------
void Radar::tryInfiltrationEvent(const Object *obj) {
  // We should only be warned against infiltrations that are taking place
  // against us.
  if (obj->getControllingPlayer() != ThePlayerList->getLocalPlayer()) return;
  ...
  ///audio events
  //
  // UI feedback for being under attack (note that we display these messages and
  // audio queues even if we don't have a radar)
  //
  Player *player = ThePlayerList->getLocalPlayer();
  ...
  static AudioEventRTS infiltrationWarningSound =
      TheAudio->getMiscAudio()->m_radarInfiltrationSound;
...
Bool Radar::tryEvent(RadarEventType event, const Coord3D *pos) {
  // sanity
  if (event <= RADAR_EVENT_INVALID || event >= RADAR_EVENT_NUM_EVENTS ||
      pos == NULL)
    return FALSE;

  // see if there was an event of this type within the given range within the
  // given time
  ...
  // that wasn't too long ago, if there was we won't create another and get
  // outta here
  //
  for (Int i = 0; i < MAX_RADAR_EVENTS; ++i) {
    ...
    if (m_event[i].type == event) {
      ...
      if (distSquared <= closeEnoughDistanceSq) {
        // finally only reject making a new event of this existing one is
        // "recent enough"
...
// ------------------------------------------------------------------------------------------------
/** Queue a refresh of the radar terrain, we have this so that if there is code
 * that rapidly needs to refresh the radar, it should use this so we aren't
 * continually rebuilding the radar graphic because that process is slow.  If
 * you need to update the terrain on the radar immediately use refreshTerrain()
 */
// ------------------------------------------------------------------------------------------------
void Radar::queueTerrainRefresh(void) {
  //
  // we just simply overwrite the frame we have recorded for a radar refresh. If
  // there was already one there, it's simply just forgotten and whatever
  // changes we wanted to see with that refresh will have to wait until enough
  // time has passed to show these changes as well.  why you ask ... well,
  // because if we're calling this in close enough proximity for us to overwrite
  // something, we're changing the terrain features quite often and can't afford
  // the expense of rebuilding the radar visual
  //
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void Radar::crc(Xfer *xfer) {}  // end crc
...
static void xferRadarObjectList(Xfer *xfer, RadarObject **head) {
  ...
  // sanity
  DEBUG_ASSERTCRASH(head != NULL,
                    ("xferRadarObjectList - Invalid parameters\n"));
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    if (*head != NULL) {
      ...
      // we immediately destroy 'em, but they don't go away just yet. so just
      // ignore 'em if possible.
      for (radarObject = *head; radarObject;
           radarObject = radarObject->friend_getNext()) {
        if (!radarObject->friend_getObject()->isDestroyed()) {
          DEBUG_CRASH(
              ("xferRadarObjectList - List head should be NULL, or contain "
               "only destroyed objects\n"));
          throw SC_INVALID_DATA;
        }
      }
#else
      DEBUG_CRASH(
          ("xferRadarObjectList - List head should be NULL, but isn't\n"));
      throw SC_INVALID_DATA;
#endif
    }  // end if
    ...
    for (UnsignedShort i = 0; i < count; ++i) {
      ...
      if (*head == NULL)
        *head = radarObject;
        ...
        RadarObject *other;
        for (other = *head; other->friend_getNext() != NULL;
             other = other->friend_getNext()) {
        }  // end for, other
...
void Radar::xfer(Xfer *xfer) {
  ...
  if (eventCount != eventCountVerify) {
    DEBUG_CRASH(
        ("Radar::xfer - size of MAX_RADAR_EVENTS has changed, you must version "
         "this xfer method to accomodate the new array size.  Was '%d' and is "
         "now '%d'\n",
...
void Radar::loadPostProcess(void) {
  //
  // refresh the radar texture now that all the objects (specifically bridges)
  // have been loaded with their correct damage states from save game file
  //
  refreshTerrain(TheTerrainLogic);
