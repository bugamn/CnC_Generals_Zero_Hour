...
void UnicodeString::validate() const {
  ...
  DEBUG_ASSERTCRASH(wcslen(m_data->peek()) + 1 <= m_data->m_numCharsAllocated,
                    ("str is too long for storage"));
}
#endif
...
void UnicodeString::ensureUniqueBufferOfSize(int numCharsNeeded,
  ...
  if (m_data && m_data->m_refCount == 1 &&
    ...
    // us)
    if (strToCopy) wcscpy(m_data->peek(), strToCopy);
    if (strToCat) wcscat(m_data->peek(), strToCat);
    return;
  }
  ...
  UnicodeStringData* newData =
      (UnicodeStringData*)TheDynamicMemoryAllocator->allocateBytesDoNotZero(
          actualBytes, "STR_UnicodeString::ensureUniqueBufferOfSize");
  newData->m_refCount = 1;
  newData->m_numCharsAllocated =
      (actualBytes - sizeof(UnicodeStringData)) / sizeof(WideChar);
  ...
  if (strToCopy) wcscpy(newData->peek(), strToCopy);
  if (strToCat) wcscat(newData->peek(), strToCat);
...
void UnicodeString::translate(const AsciiString& stringSrc) {
  ...
  for (Int i = 0; i < len; i++) concat((WideChar)stringSrc.getCharAt(i));
  validate();
}
...
Bool UnicodeString::nextToken(UnicodeString* tok, UnicodeString delimiters) {
  ...
  if (delimiters.isEmpty()) delimiters = UnicodeString(L" \t\n\r");
