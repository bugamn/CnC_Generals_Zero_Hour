...
// Desc:   The Xfer system is capable of setting up operations to work with
// blocks of data
//				 from other subsystems.  It can work things such
//as file reading, file writing, 				 CRC computations etc
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void Xfer::xferVersion(XferVersion *versionData, XferVersion currentVersion) {
  ...
  // sanity, after the xfer, version data is never allowed to be higher than the
  // current version
  if (*versionData > currentVersion) {
    DEBUG_CRASH(
        ("XferVersion - Unknown version '%d' should be no higher than '%d'\n",
...
void Xfer::xferAsciiString(AsciiString *asciiStringData) {
  xferImplementation((void *)asciiStringData->str(),
                     sizeof(Byte) * asciiStringData->getLength());
...
void Xfer::xferUnicodeString(UnicodeString *unicodeStringData) {
  xferImplementation((void *)unicodeStringData->str(),
                     sizeof(WideChar) * unicodeStringData->getLength());
...
void Xfer::xferSTLObjectIDList(std::list<ObjectID> *objectIDListData) {
  //
  // the fact that this is a list and a little higher level than a simple data
  // type is reason enough to have every one of these versioned
  //
  ...
  if (getXferMode() == XFER_SAVE || getXferMode() == XFER_CRC) {
    ...
    if (objectIDListData->size() != 0) {
      DEBUG_CRASH(
          ("Xfer::xferSTLObjectIDList - object list should be empty before "
           "loading\n"));
    ...
    DEBUG_CRASH(
        ("xferSTLObjectIDList - Unknown xfer mode '%d'\n", getXferMode()));
...
void Xfer::xferSTLIntList(std::list<Int> *intListData) {
  ...
  if (getXferMode() == XFER_SAVE || getXferMode() == XFER_CRC) {
    ...
    if (intListData->size() != 0) {
      DEBUG_CRASH(
          ("Xfer::xferSTLIntList - int list should be empty before loading\n"));
      throw XFER_LIST_NOT_EMPTY;
...
void Xfer::xferScienceType(ScienceType *science) {
  // sanity
  DEBUG_ASSERTCRASH(science != NULL,
                    ("xferScienceType - Invalid parameters\n"));
  ...
  if (getXferMode() == XFER_SAVE) {
    ...
    *science = TheScienceStore->getScienceFromInternalName(scienceName);
    if (*science == SCIENCE_INVALID) {
      DEBUG_CRASH(
          ("xferScienceType - Unknown science '%s'\n", scienceName.str()));
...
void Xfer::xferScienceVec(ScienceVec *scienceVec) {
  // sanity
  DEBUG_ASSERTCRASH(scienceVec != NULL,
                    ("xferScienceVec - Invalid parameters\n"));
  ...
  if (getXferMode() == XFER_SAVE) {
    for (ScienceVec::const_iterator it = scienceVec->begin();
         it != scienceVec->end(); ++it) {
...
void Xfer::xferKindOf(KindOfType *kindOfData) {
  ...
  if (getXferMode() == XFER_SAVE) {
    ...
    if (bit != -1) *kindOfData = (KindOfType)bit;
...
void Xfer::xferUpgradeMask(Int64 *upgradeMaskData) {
  ...
  if (getXferMode() == XFER_SAVE) {
    ...
    for (upgradeTemplate = TheUpgradeCenter->firstUpgradeTemplate();
         upgradeTemplate; upgradeTemplate = upgradeTemplate->friend_getNext()) {
      // if the mask of this upgrade is set, it counts
      if (BitTest(*upgradeMaskData, upgradeTemplate->getUpgradeMask())) count++;
