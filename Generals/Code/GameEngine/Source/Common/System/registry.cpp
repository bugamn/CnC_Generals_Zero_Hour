...
Bool getStringFromRegistry(HKEY root, AsciiString path, AsciiString key,
  ...
  if ((returnValue = RegOpenKeyEx(root, path.str(), 0, KEY_READ, &handle)) ==
      ERROR_SUCCESS) {
    returnValue = RegQueryValueEx(handle, key.str(), NULL, &type,
...
Bool setStringInRegistry(HKEY root, AsciiString path, AsciiString key,
  ...
  if ((returnValue = RegCreateKeyEx(root, path.str(), 0, "REG_NONE",
...
Bool GetStringFromRegistry(AsciiString path, AsciiString key,
  ...
  if (getStringFromRegistry(HKEY_LOCAL_MACHINE, fullPath.str(), key.str(),
                            val)) {
    return TRUE;
  }

  return getStringFromRegistry(HKEY_CURRENT_USER, fullPath.str(), key.str(),
                               val);
}
...
Bool GetUnsignedIntFromRegistry(AsciiString path, AsciiString key,
  ...
  DEBUG_LOG(("GetUnsignedIntFromRegistry - looking in %s for key %s\n",
             fullPath.str(), key.str()));
  if (getUnsignedIntFromRegistry(HKEY_LOCAL_MACHINE, fullPath.str(), key.str(),
                                 val)) {
    return TRUE;
  }

  return getUnsignedIntFromRegistry(HKEY_CURRENT_USER, fullPath.str(),
                                    key.str(), val);
}
