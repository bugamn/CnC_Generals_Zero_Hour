...
ArchiveFile::ArchiveFile() { m_rootDirectory.clear(); }
...
void ArchiveFile::getFileListInDirectory(const AsciiString &currentDirectory,
  ...
  while (token.getLength() > 0) {
    DetailedArchivedDirectoryInfoMap::const_iterator it =
  ...
  getFileListInDirectory(dirInfo, originalDirectory, searchName, filenameList,
                         searchSubdirectories);
}

void ArchiveFile::getFileListInDirectory(
    const DetailedArchivedDirectoryInfo *dirInfo,
    const AsciiString &currentDirectory, const AsciiString &searchName,
    FilenameList &filenameList, Bool searchSubdirectories) const {
  DetailedArchivedDirectoryInfoMap::const_iterator diriter =
      dirInfo->m_directories.begin();
  while (diriter != dirInfo->m_directories.end()) {
    ...
    getFileListInDirectory(tempDirInfo, tempdirname, searchName, filenameList,
                           searchSubdirectories);
    diriter++;
  }
...
const ArchivedFileInfo *ArchiveFile::getArchivedFileInfo(
