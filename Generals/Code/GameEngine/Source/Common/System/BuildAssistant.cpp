...
// Desc:   Singleton class to encapsulate some of the more common functions or
// rules
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
ObjectSellInfo::~ObjectSellInfo(void) {}  // end ~ObjectSellInfo
...
static Bool isDozer(Object *obj) {
  ...
  if (obj->isKindOf(KINDOF_DOZER)) return TRUE;
...
void BuildAssistant::init(void) {
  //
  // allocate our array of positions that we use to assist ourselves when
  // constructing a tiled array of locations to build things like walls
  //
...
void BuildAssistant::update(void) {
  ...
  for (it = m_sellList.begin(); it != m_sellList.end(); /*empty*/) {
    ...
    //
    // if object is not found, remove it from the list immediately ... this is
    // valid as the object maybe was destroyed by other means during the sell
    // process
    //
    if (obj == NULL) {
      sellInfo->deleteInstance();
    ...
    if (TheGameLogic->getFrame() - sellInfo->m_sellFrame >=
        FRAMES_TO_ALLOW_SCAFFOLD) {
      Real previousConstructionPercent = obj->getConstructionPercent();

      // do the constructoin
      obj->setConstructionPercent(previousConstructionPercent -
                                  (100.0f / TOTAL_FRAMES_TO_SELL_OBJECT));

      //
      // did we cross the threshold from positive to negative ... if so, we will
      // clear visual construction indicators
      //
      if (previousConstructionPercent > 0.0f &&
          obj->getConstructionPercent() <= 0.0f) {
        ...
        //
        // set the animation durations so that the regular build up loop
        // animations can be done a bit faster for selling
        //
    ...
    //
    // after we've reached zero ... the object has "sunk" back down into the
    // ground ... but we need to keep the object around for a little while
    // longer so that the scaffold animation can finish up and sink itself back
    // into the ground
    //
    if (obj->getConstructionPercent() <= -50.0f) {
      ...
      if (player) {
        ...
        // 0 is the init, and means you have no override.  We would be marked
        // unsellable if someone wanted us to not sell
        if (obj->getTemplate()->getRefundValue() != 0)
          sellValue = obj->getTemplate()->getRefundValue();
        else
          sellValue =
              REAL_TO_UNSIGNEDINT(obj->getTemplate()->calcCostToBuild(player) *
      ...
      if (pui) pui->cancelAndRefundAllProduction();

      // for debugging
      // UnicodeString msg;
      // msg.format( L"'%S' has been sold\n",
      // obj->getTemplate()->getName().str() ); TheInGameUI->message( msg );
...
//-------------------------------------------------------------------------------------------------
Object *BuildAssistant::buildObjectNow(Object *constructorObject,
                                       const ThingTemplate *what,
                                       const Coord3D *pos, Real angle,
                                       Player *owningPlayer) {
  // sanity
  if (what == NULL || pos == NULL) return NULL;

  if (owningPlayer == NULL) return NULL;  // Invalid pointer.  Won't happen.

  // sanity
  if (constructorObject) {
    DEBUG_ASSERTCRASH(constructorObject->getControllingPlayer() == owningPlayer,
                      ("buildObjectNow: Constructor object player is not the "
                       "same as the controlling player passed in\n"));

  }  // end if

  // Need to validate that we can make this in case someone fakes their
  // CommandSet A Null constructorObject is used by the script engine to cheat,
  // so let it slide
  if (constructorObject && !isPossibleToMakeUnit(constructorObject, what))
    return NULL;

  // clear out any objects from the building area that are "auto-clearable" when
  // building
  clearRemovableForConstruction(what, pos, angle);

  if (moveObjectsForConstruction(what, pos, angle, owningPlayer) == FALSE) {
    // totally bogus. We tried to move our units out of the way, but they
    // wouldn't. Chode-boys.
  ...
  if (isDozer(constructorObject) && isLineBuildTemplate(what) == FALSE) {
    ...
    // create the new object.  We need to construct it with UnderConstruction
    // set, since we are going to insta build it, but we don't want to send
    // double construction type events like power creation.
    // onStructureConstructionComplete is called below, so we need to simulate
    // the proper object creation flow from the start.  Be like Dozer.
    ...
    Object *obj = TheThingFactory->newObject(
        what, owningPlayer->getDefaultTeam(), startingStatus);
    ...
    if (obj->isKindOf(KINDOF_STRUCTURE)) {
      //
      // this is a cheat since this method is currently used by the solo AI to
      // make buildings out of thin air
      //
      owningPlayer->onStructureCreated(constructorObject, obj);
      owningPlayer->onStructureConstructionComplete(constructorObject, obj,
                                                    FALSE);
      ...
      // Call the voice created for the 1st object -- because it's possible to
      // create multiple objects like redguards!
      AudioEventRTS sound =
          *obj->getTemplate()->getPerUnitSound("VoiceCreated");
    ...
    // Creation is another valid and essential time to call this. This building
    // now Looks.
...
//-------------------------------------------------------------------------------------------------
/** This method will create a line of objects end to end along the line defined
 * in 3D space from start to end.  This is especially useful in building walls
 */
//-------------------------------------------------------------------------------------------------
void BuildAssistant::buildObjectLineNow(Object *constructorObject,
                                        const ThingTemplate *what,
                                        const Coord3D *start,
                                        const Coord3D *end, Real angle,
                                        Player *owningPlayer) {
  TileBuildInfo *tileBuildInfo;

  // sanity
  if (what == NULL || start == NULL || end == NULL) return;
  ...
  tileBuildInfo = buildTiledLocations(what, angle, start, end, objectSize,
                                      maxObjects, constructorObject);

  // create an object at each position
  for (Int i = 0; i < tileBuildInfo->tilesUsed; i++)
    buildObjectNow(constructorObject, what, &tileBuildInfo->positions[i], angle,
                   owningPlayer);

}  // end buildObjectLineNow

//-------------------------------------------------------------------------------------------------
/** This structure is passed along to the checkSampleBuildLocation while
 * iterating the footprint of an area in order to determine if the entire
 * location is a legal place to build */
//-------------------------------------------------------------------------------------------------
 ...
 */
//-------------------------------------------------------------------------------------------------
static void checkSampleBuildLocation(const Coord3D *samplePoint,
                                     void *userData) {
  TerrainType *terrain;
  ...
  if (!cell) {
    ...
    if ((type == PathfindCell::CELL_WATER) ||
  ...
  //
  // record the highest and lowest Z points from all the samples and do not
  // allow building when the difference between them is too great
  //
  if (samplePoint->z < sampleData->loZ) sampleData->loZ = samplePoint->z;
  if (samplePoint->z > sampleData->hiZ) sampleData->hiZ = samplePoint->z;

  // too close to edge of map?
  if (TheGlobalData->m_MinDistFromEdgeOfMapForBuild > 0.0f) {
    if (samplePoint->x < sampleData->mapRegion.lo.x +
                             TheGlobalData->m_MinDistFromEdgeOfMapForBuild ||
        samplePoint->x > sampleData->mapRegion.hi.x -
                             TheGlobalData->m_MinDistFromEdgeOfMapForBuild ||
        samplePoint->y < sampleData->mapRegion.lo.y +
                             TheGlobalData->m_MinDistFromEdgeOfMapForBuild ||
        samplePoint->y > sampleData->mapRegion.hi.y -
                             TheGlobalData->m_MinDistFromEdgeOfMapForBuild) {
      sampleData->terrainRestricted = TRUE;
    }
  }
...
//-------------------------------------------------------------------------------------------------
/** This function will call the user callback at each "sample point" across the
 * footprint of where this object would be in the world.  The smaller the sample
 * resolution param is the more samples will be taken across the footprint area
 */
//-------------------------------------------------------------------------------------------------
void BuildAssistant::iterateFootprint(
    const ThingTemplate *build, Real buildOrientation, const Coord3D *worldPos,
    Real sampleResolution, IterateFootprintFunc func, void *funcUserData) {
  // sanity
  if (build == NULL || worldPos == NULL || func == NULL) return;

  //
  // create a transform matrix for the object position at the specified angle,
  // we will iterate the object footprints in "local object extent space" and
  // transform those points using this matrix into the world coords for the
  // object at the real location and specified angle
  //
  Matrix3D transform;
  ...
  Real halfFootprintHeight, halfFootprintWidth;
  if (build->getTemplateGeometryInfo().getGeomType() == GEOMETRY_BOX) {
    ...
    halfFootprintWidth = build->getTemplateGeometryInfo().getMajorRadius();
           ...
           build->getTemplateGeometryInfo().getGeomType() ==
               GEOMETRY_CYLINDER) {
    halfFootprintHeight =
        build->getTemplateGeometryInfo().getBoundingCircleRadius();
    halfFootprintWidth =
        build->getTemplateGeometryInfo().getBoundingCircleRadius();
    ...
    DEBUG_ASSERTCRASH(0,
                      ("iterateFootprint: Undefined geometry '%d' for '%s'\n",
                       build->getTemplateGeometryInfo().getGeomType(),
                       build->getName().str()));
    return;
  ...
  // footprint area, if our sample point is outside of the actual extent
  // footprint area because of this we snap it back to sample on the exact edge
  //
  Real x, y;
  Vector3 v;
  for (y = -halfFootprintHeight; y < halfFootprintHeight + sampleResolution;
       y += sampleResolution) {
    // snap it to the actual extent since we can go over by one sample
    // resolution
    if (y > halfFootprintHeight) y = halfFootprintHeight;

    for (x = -halfFootprintWidth; x < halfFootprintWidth + sampleResolution;
         x += sampleResolution) {
      // snap it to the actual extent since we can go over by one sample
      // resolution
      if (x > halfFootprintWidth) x = halfFootprintWidth;
      ...
      if (build->getTemplateGeometryInfo().getGeomType() == GEOMETRY_SPHERE ||
        ...
        if (vector.length() >
            halfFootprintWidth)  // could be height too, radius is all the same
                                 // for circles
...
Bool BuildAssistant::isLocationClearOfObjects(const Coord3D *worldPos,
                                              ...
                                              Real angle, Object *builderObject,
                                              UnsignedInt options,
                                              Player *thePlayer) {
  ObjectIterator *iter = ThePartitionManager->iteratePotentialCollisions(
      worldPos, build->getTemplateGeometryInfo(), angle);
  Object *them;
  ...
  for (them = iter->first(); them; them = iter->next()) {
    ...
    if (isRemovableForConstruction(them) == TRUE) continue;

    // ignore land mines, cluster mines and demo traps, since you can build on
    // them doing so will damage them during construction, by the way
    if (them->isKindOf(KINDOF_MINE)) continue;

    // same story for inert things: ie, radiation fields, pings, and projectile
    // streams (!)...
    if (them->isKindOf(KINDOF_INERT)) continue;
  ...
  Real range = 2 * (build->getTemplateGeometryInfo().getMajorRadius() +
                    build->getTemplateGeometryInfo().getMinorRadius());

  PartitionFilterAcceptByKindOf f1(MAKE_KINDOF_MASK(KINDOF_STRUCTURE),
                                   KINDOFMASK_NONE);
  PartitionFilter *filters[] = {&f1, NULL};

  ObjectIterator *iter2 = ThePartitionManager->iterateObjectsInRange(
      worldPos, range, FROM_BOUNDINGSPHERE_2D, filters);
  ...
  if (thePlayer && thePlayer->isSkirmishAIPlayer()) {
    // Skirmish ai adds a little extra around the edges so it doesn't build
  ...
  if (myBounds.getGeomType() != GEOMETRY_BOX) {
    myBounds.set(GEOMETRY_BOX, false, 40, myBounds.getMajorRadius(),
                 myBounds.getMajorRadius());
  } else {
  ...
  if (myFactoryExitWidth > 0) {
    ...
    Real offset = build->getTemplateGeometryInfo().getMajorRadius() +
                  myFactoryExitWidth / 2.0f;
  ...
  for (them = iter2->first(); them; them = iter2->next()) {
    ...
    if (hisBounds.getGeomType() != GEOMETRY_BOX) {
      hisBounds.set(GEOMETRY_BOX, false, 40, hisBounds.getMajorRadius(),
                    hisBounds.getMajorRadius());
    } else {
    ...
    if (themFactoryExitWidth > 0) {
      ...
      Real offset = them->getGeometryInfo().getMajorRadius() +
                    themFactoryExitWidth / 2.0f;
      hisExitPos.x += c * offset;
      hisExitPos.y += s * offset;
    }
    if (ThePartitionManager->geomCollidesWithGeom(
            them->getPosition(), hisBounds, them->getOrientation(), worldPos,
            myBounds, angle)) {
      TheTerrainVisual->addFactionBib(them, true);
      return false;
    }
    ...
    if (them->isKindOf(KINDOF_IMMOBILE)) {
      ...
      if (checkMyExit &&
          ThePartitionManager->geomCollidesWithGeom(
              them->getPosition(), hisBounds, them->getOrientation(),
              &myExitPos, myGeom, angle)) {
        TheTerrainVisual->addFactionBib(them, true);
        return false;
      }
      // Check for overlap of his exit rectangle with my geom info
      if (checkHisExit && ThePartitionManager->geomCollidesWithGeom(
                              &hisExitPos, hisGeom, them->getOrientation(),
                              worldPos, myBounds, angle)) {
        TheTerrainVisual->addFactionBib(them, true);
        return false;
      }
      // Check both exit rectangles together.
      if (checkMyExit && checkHisExit &&
          ThePartitionManager->geomCollidesWithGeom(
              &hisExitPos, hisGeom, them->getOrientation(), &myExitPos, myGeom,
              angle)) {
        TheTerrainVisual->addFactionBib(them, true);
        return false;
      }
...
//-------------------------------------------------------------------------------------------------
/** Query if we can build at this location.  Note that 'build' may be null and
 * is NOT required to be valid to know if a location is legal to build at.
 * 'builderObject' is used for queries that require a pathfind check and should
 * be NULL if not required */
//-------------------------------------------------------------------------------------------------
LegalBuildCode BuildAssistant::isLocationLegalToBuild(
    const Coord3D *worldPos, const ThingTemplate *build, Real angle,
    UnsignedInt options, Object *builderObject, Player *player) {
  ...
  // check shroud level
  // This should be the first check, since returning other errors for shrouded
  // areas could be used to game the system
  if (BitTest(options, SHROUD_REVEALED)) {
    {
      ...
      DEBUG_ASSERTCRASH(playerIndex >= 0,
                        ("isLocationLegalToBuild() needs a builderObject with "
                         "a team to check for shroud"));
      if (ThePartitionManager->getShroudStatusForPlayer(playerIndex, x, y) !=
          CELLSHROUD_CLEAR) {
        return LBC_SHROUD;
      }
    }
  ...
  //
  // if NO_OBJECT_OVERLAP is set, we are not allowed to construct 'build' if it
  // would overlap any immobile objects, or an enemy object.  Friendly objects
  // should politely "move out of the way" when you build something where
  // they're standing
  //
  if (BitTest(options, NO_OBJECT_OVERLAP)) {
    if (!isLocationClearOfObjects(worldPos, build, angle, builderObject,
                                  NO_OBJECT_OVERLAP, player)) {
      return LBC_OBJECTS_IN_THE_WAY;
    }

  }  // end if
  //
  // if NO_ENEMY_OBJECT_OVERLAP is set, we are not allowed to construct 'build'
  // if it would overlap any enemy objects.  Friendly objects are ignored.
  //
  if (BitTest(options, NO_ENEMY_OBJECT_OVERLAP)) {
    if (!isLocationClearOfObjects(worldPos, build, angle, builderObject,
                                  NO_ENEMY_OBJECT_OVERLAP, player)) {
      return LBC_OBJECTS_IN_THE_WAY;
    }

  }  // end if

  if (build->isKindOf(KINDOF_CANNOT_BUILD_NEAR_SUPPLIES) &&
      TheGlobalData->m_SupplyBuildBorder > 0) {
    // special case for supply centers: can't build too close to supply sources
    PartitionFilterAcceptByKindOf f1(MAKE_KINDOF_MASK(KINDOF_SUPPLY_SOURCE),
                                     KINDOFMASK_NONE);
    PartitionFilter *filters[] = {&f1, NULL};

    // see if there are any reasonably close by
    Real range = build->getTemplateGeometryInfo().getBoundingCircleRadius() +
                 TheGlobalData->m_SupplyBuildBorder * 2;
    Object *tooClose = ThePartitionManager->getClosestObject(
        worldPos, range, FROM_BOUNDINGSPHERE_2D, filters);
    if (tooClose != NULL) {
      ...
      if (ThePartitionManager->geomCollidesWithGeom(
              worldPos, build->getTemplateGeometryInfo(), angle,
              tooClose->getPosition(), tooCloseGeom,
              tooClose->getOrientation())) {
        TheTerrainVisual->addFactionBib(tooClose, true,
                                        TheGlobalData->m_SupplyBuildBorder);
        return LBC_TOO_CLOSE_TO_SUPPLIES;
      }
    }
  }

  // if clear path is requestsed check to see if the builder object can get
  // there
  if (BitTest(options, CLEAR_PATH) && builderObject) {
    AIUpdateInterface *ai = builderObject->getAIUpdateInterface();

    //
    // if there is no AI interface for this object, it cannot possible pass a
    // clear path check since it will never be able to move there
    //
    /**todo remove this if we need to change the semantics of this function of
    the builderObject
    // actually being able to get to the destination */
    //
    if (ai == NULL) return LBC_NO_CLEAR_PATH;

    //
    // check for an available path using one of two methods (the quick less
    // accurate one, or the slow more accurate one)
    //
    if (BitTest(options, USE_QUICK_PATHFIND)) {
      if (ai->isQuickPathAvailable(worldPos) == FALSE) return LBC_NO_CLEAR_PATH;

    }  // end if
    else {
      if (ai->isPathAvailable(worldPos) == FALSE) return LBC_NO_CLEAR_PATH;
  ...
  if (BitTest(options, TERRAIN_RESTRICTIONS)) {
    //
    // we will take "samples" at this resolution across the footprint of where
    // we are going to build the structure
    //
    ...
    //
    // check the footprint of where the structure would go to be clear of any
    // non-buildable tiles and to make sure there isn't a restricted tile and to
    // make sure it's "flat" enough
    //
    SampleBuildData sampleData;
    TheTerrainLogic->getExtent(&sampleData.mapRegion);
    sampleData.terrainRestricted = FALSE;
    sampleData.hiZ =
        terrainExtent.lo.z;  // note we set hi point to lowest point
    sampleData.loZ =
        terrainExtent.hi.z;  // note we set lo point to highest point

    // quick check at triple res.
    iterateFootprint(build, angle, worldPos, 3 * sampleResolution,
                     checkSampleBuildLocation, &sampleData);
    if (sampleData.terrainRestricted == TRUE) return LBC_RESTRICTED_TERRAIN;
    // check if the height across the whole footprint area is too varied (not
    // flat enough)
    if (sampleData.hiZ - sampleData.loZ >
        TheGlobalData->m_allowedHeightVariationForBuilding)
...
//-------------------------------------------------------------------------------------------------
/** Tiling wall object helper function, we can use this to "tile" walls when
 * building. This function will fill out the ARRAY world positions named
 * 'positions' starting at the 'start' point in the world and going to the 'end'
 * point in the world.  No more than 'maxTiles' will be considered and if the
 * position of any one of the objects in the chain is illegal the chain stops
 * there.  The number of objects actually used for the tiling is returned along
 * with a pointer to the array of positions in the tile build info.
 *
 * REQUIRES: Note that the array at 'postions' must be large enough to hold
 * 'maxTiles' entries of positions
 */
//-------------------------------------------------------------------------------------------------
BuildAssistant::TileBuildInfo *BuildAssistant::buildTiledLocations(
  ...
  //
  // we will fill out our own internal array of positions, it better be big
  // enough to accomodate max tiles, if it's not lets make it bigger!
  //
  if (maxTiles > m_buildPositionSize) {
    ...
    //
    // lets try to at least keep sanity here so that we don't have a completely
    // unbounded allocation spot in the code here
    //
    DEBUG_ASSERTCRASH(m_buildPositionSize < 200,
                      ("Do you really need to tile this many objects!!!"));
  ...
  //
  // get the lengh of the placement vector in the world, we'll use this to see
  // how many objects we'll need to fill the entire line
  //
  Real placementLength = placementVector.length();

  //
  // first see how many objects we're going to need to go from the placement
  // line start to the placement line end, we are guaranteed to have one thing
  // (+1)
  //
  Int tilesNeeded = REAL_TO_INT(placementLength / tilingSize) + 1;

  // we have a max that we are allowed to build to
  if (tilesNeeded > maxTiles) tilesNeeded = maxTiles;

  // we're guaranteed to have at least one object so lets take care of that
  // position now
  positions[0] = *start;

  //
  // march down each object and set the position based on its position in the
  // line. note that we're skipping the first one because we've already set it
  // explicitly and are guaranteed to have at least one object
  //
  Int tilesUsed =
      1;  // actual objects "tiled", we are guaranteed to have at least one
  Coord3D pos;
  ...
  for (Int i = 1; i < tilesNeeded; i++) {
    ...
    // check for a legal position to be at and stop the tiling process if that
    // becomes broken
  ...
  // return a struct filled out with the actual tiles used and the array of
  // locations
...
//-------------------------------------------------------------------------------------------------
/** Is the template passed in one of those wall type structures that we "build"
 * in the world after drawing a line of where we want the object(s) placed.  The
 * objects are placed "tiled" in a line */
//-------------------------------------------------------------------------------------------------
Bool BuildAssistant::isLineBuildTemplate(const ThingTemplate *tTemplate) {
  // sanity
  if (tTemplate == NULL) return FALSE;

  if (tTemplate->isKindOf(KINDOF_LINEBUILD)) return TRUE;
...
//-------------------------------------------------------------------------------------------------
/** This method will check to make sure it is possible to build the requested
 * unit.  The builder object MUST be present since all construction comes comes
 * from either a Dozer or a production building.  The player must have satisfied
 * the prereqs for 'whatToBuild' This does NOT check available money (see
 * canMakeUnit) */
//-------------------------------------------------------------------------------------------------
Bool BuildAssistant::isPossibleToMakeUnit(
    Object *builder, const ThingTemplate *whatToBuild) const {
  // sanity
  if (builder == NULL || whatToBuild == NULL) return FALSE;
  ...
  if (commandSet == NULL) {
    DEBUG_ASSERTLOG(0, ("Can't build a '%s' from the builder '%s' because '%s' "
                        "doesn't have any command set defined\n",
  ...
  //
  // scan the command set, we must find whatToBuild as one of the "build"
  // commands available in the command set.  We want to have all players run
  // this logic on all their machines so that nobody can hack one game and cheat
  // to make stuff that they can't usually make
  //
  ...
  if (foundCommand == NULL) return FALSE;

  // make sure that the player can actually make this unit by checking prereqs
  // and such
  Player *player = builder->getControllingPlayer();
  if (player->canBuild(foundCommand->getThingTemplate()) == FALSE) return FALSE;
...
// ------------------------------------------------------------------------------------------------
/** Count all the units of a given type that are in any production queues for a
 * player */
// ------------------------------------------------------------------------------------------------
static void countInProduction(Object *obj, void *userData) {
  ...
  ProductionUpdateInterface *pui =
      ProductionUpdate::getProductionUpdateInterfaceFromObject(obj);
  if (pui) {
    ProductionCountData *productionCountData = (ProductionCountData *)userData;

    // add the count of this type that are in the queue
    productionCountData->count +=
        pui->countUnitTypeInQueue(productionCountData->type);
...
//-------------------------------------------------------------------------------------------------
/** This method will check to make sure it is possible to build the requested
 * unit. and that the player has enough money for 'whatToBuild' */
//-------------------------------------------------------------------------------------------------
CanMakeType BuildAssistant::canMakeUnit(
    Object *builder, const ThingTemplate *whatToBuild) const {
  // sanity
  if (builder == NULL || whatToBuild == NULL) return CANMAKE_NO_PREREQ;

  if (builder->testScriptStatusBit(OBJECT_STATUS_SCRIPT_DISABLED) ||
      builder->testScriptStatusBit(OBJECT_STATUS_SCRIPT_UNPOWERED))
    return CANMAKE_FACTORY_IS_DISABLED;

  if (!isPossibleToMakeUnit(builder, whatToBuild)) return CANMAKE_NO_PREREQ;

  ProductionUpdateInterface *pu = builder->getProductionUpdateInterface();
  if (pu != NULL) {
    CanMakeType cmt = pu->canQueueCreateUnit(whatToBuild);
    if (cmt != CANMAKE_OK) return cmt;
  }
  ...
  if (whatToBuild->getMaxSimultaneousOfType() != 0) {
    ...
    const Bool ignoreUnderConstruction =
        FALSE;  // Most people don't want to count under construction, but I
                // totally do
    Int existingCount;
    player->countObjectsByThingTemplate(
        1, &whatToBuild, ignoreDead, &existingCount, ignoreUnderConstruction);
    ...
    if (productionCountData.count + existingCount >=
        whatToBuild->getMaxSimultaneousOfType())
      return CANMAKE_MAXED_OUT_FOR_PLAYER;
  }
...
// ------------------------------------------------------------------------------------------------
/** Some objects will be automatically removed when something is built on them
 * so they aren't considered as obstacles when placing them */
// ------------------------------------------------------------------------------------------------
Bool BuildAssistant::isRemovableForConstruction(Object *obj) {
  ...
  if (obj->isKindOf(KINDOF_SHRUBBERY)) return TRUE;

  // anything with a kindof marked as cleared by build can be removed
  if (obj->isKindOf(KINDOF_CLEARED_BY_BUILD)) return TRUE;
...
// ------------------------------------------------------------------------------------------------
/** Given that we are about to build 'whatToBuild' remove all the objects that
 * are in the 'footprint' of where the build will take place that can be
 * auto-removed for construction */
// ------------------------------------------------------------------------------------------------
void BuildAssistant::clearRemovableForConstruction(
    const ThingTemplate *whatToBuild, const Coord3D *pos, Real angle) {
  ObjectIterator *iter = ThePartitionManager->iteratePotentialCollisions(
      pos, whatToBuild->getTemplateGeometryInfo(), angle);
  MemoryPoolObjectHolder hold(iter);
  for (Object *them = iter->first(); them; them = iter->next()) {
    // UI feedback objects (always selectable) never get destroyed by
    // construction, but also should never trip any of the other side effects of
    // placing them in isRemovableForConstruction()
    if (isRemovableForConstruction(them) == TRUE &&
        !them->isKindOf(KINDOF_ALWAYS_SELECTABLE))
...
// ------------------------------------------------------------------------------------------------
/** clearRemovable is set up to delete objects that should cease to exist (for
   instance, trees).
        * moveObjects will move objects that are owned by the player. It will
   also return false if the association with the object is enemy. (We can move
   neutral things) */
// ------------------------------------------------------------------------------------------------
Bool BuildAssistant::moveObjectsForConstruction(
    const ThingTemplate *whatToBuild, const Coord3D *pos, Real angle,
  ...
  ObjectIterator *iter =
      ThePartitionManager->iteratePotentialCollisions(pos, gi, angle);
  ...
  for (Object *them = iter->first(); them; them = iter->next()) {
    ...
    // Skip KINDOF_ALWAYS_SELECTABLE and isRemovableForConstruction, because if
    // it is RemovableForConstruction, it just got destroyed, and won't actually
    // be gone until the end of the frame.  jba.
    if (!them->isKindOf(KINDOF_ALWAYS_SELECTABLE) &&
        !isRemovableForConstruction(them)) {
      Relationship rel = playerToBuild->getRelationship(them->getTeam());

      if (rel == NEUTRAL || rel == ALLIES) {
        // Pick a point outside of the construction and tell those fools to move
        // there. Pick an arbitrary direction and tell the unit to move there,
        // at a distance greater than the object's radius.
        AIUpdateInterface *ai = them->getAIUpdateInterface();
        if (ai) {
          // Vary the distance to move between one half the diameter of the
          // building (roughly) and 1.5 times the diameter of the building
          ...
          // note that this is an extra-special case... even if the unit's mood
          // is "sleep" it still needs to move here. (units with an ai mood of
          // "sleep" won't respond to any AI commands. this is a special case
          // that ignores the "sleep" ai mood.) (srj)
...
void BuildAssistant::sellObject(Object *obj) {
  ...
  if (obj->isKindOf(KINDOF_STRUCTURE) == FALSE) return;

  // if object already has an entry in the sell list, we shouldn't try to sell
  // it again
  ...
  // set the construction percent of this object just below 100.0% so we can
  // start counting down
  ...
  //
  // set the model condition in the drawable for this object that will show the
  // buildup scaffold and adjust the model height by construction percent
  //
  obj->setModelConditionFlags(
      MAKE_MODELCONDITION_MASK2(MODELCONDITION_PARTIALLY_CONSTRUCTED,
                                MODELCONDITION_ACTIVELY_BEING_CONSTRUCTED));

  //
  // set this object as under de-construction (sold).  It is still a legal
  // target, since you get the money at the completion of sale.
  //
  obj->setStatus(
      ObjectStatusBits(OBJECT_STATUS_SOLD | OBJECT_STATUS_UNSELECTABLE));

  // for everybody, unselect them at this time.  You can't just deselect a
  // drawable.  Selection is a logic property.
  TheGameLogic->deselectObject(obj, PLAYERMASK_ALL, TRUE);

  //
  // set the animation durations so that the regular build up loop animations
  // can be done a bit faster for selling
  //
  Drawable *draw = obj->getDrawable();
  if (draw)
    draw->setAnimationLoopDuration(
        (unsigned)(TOTAL_FRAMES_TO_SELL_OBJECT / 2.0f));
  ...
  if (obj->getAI()) obj->getAI()->aiIdle(CMD_FROM_AI);
