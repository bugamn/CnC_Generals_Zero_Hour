...
//=============================================================================
/*static*/ void GeometryInfo::parseGeometryType(INI* ini, void* /*instance*/,
                                                void* store,
                                                const void* /*userData*/) {
  ((GeometryInfo*)store)->m_type =
      (GeometryType)INI::scanIndexList(ini->getNextToken(), GeometryNames);
  ((GeometryInfo*)store)->calcBoundingStuff();
}

//=============================================================================
/*static*/ void GeometryInfo::parseGeometryIsSmall(INI* ini, void* /*instance*/,
                                                   void* store,
                                                   const void* /*userData*/) {
...
//=============================================================================
/*static*/ void GeometryInfo::parseGeometryHeight(INI* ini, void* /*instance*/,
                                                  void* store,
                                                  const void* /*userData*/) {
...
//=============================================================================
/*static*/ void GeometryInfo::parseGeometryMajorRadius(
...
//=============================================================================
/*static*/ void GeometryInfo::parseGeometryMinorRadius(
    INI* ini, void* /*instance*/, void* store, const void* /*userData*/) {
...
//-----------------------------------------------------------------------------
static Real calcPointToLineDistSquared(const Coord3D& pt,
                                       const Coord3D& lineStart,
...
Bool GeometryInfo::isIntersectedByLineSegment(const Coord3D& loc,
  ...
  DEBUG_CRASH(
      ("this call does not work properly for nonspheres yet. use with "
       "caution."));
...
//=============================================================================
// given an object with this geom, located at 'pos', and another obj with the
// given pos & geom, calc the min and max pitches from this to that.
void GeometryInfo::calcPitches(const Coord3D& thisPos, const GeometryInfo& that,
                               const Coord3D& thatPos, Real& minPitch,
                               Real& maxPitch) const {
  Coord3D thisCenter;
  getCenterPosition(thisPos, thisCenter);

  Real dxy =
      sqrt(sqr(thatPos.x - thisCenter.x) + sqr(thatPos.y - thisCenter.y));

  Real dz;

  /** @todo srj -- this could be better, by calcing it for all the corners, not
     just top-center and bottom-center... oh well */
...
//=============================================================================
// given an object with this geom, SET how far above the object's canonical
// position its max z should extend.
...
//=============================================================================
// given an object with this geom, how far above the object's canonical position
// does its max z extend?
...
//=============================================================================
// given an object with this geom, how far below the object's canonical position
// does its max z extend?
...
//=============================================================================
// given an object with this geom, located at 'pos', where is the "center" of
// the geometry?
...
//=============================================================================
void GeometryInfo::get2DBounds(const Coord3D& geomCenter, Real angle,
...
void GeometryInfo::clipPointToFootprint(const Coord3D& geomCenter,
  ...
  switch (m_type) {
    ...
    case GEOMETRY_BOX: {
      ptToClip.x = clamp(geomCenter.x - m_majorRadius, ptToClip.x,
                         geomCenter.x + m_majorRadius);
      ptToClip.y = clamp(geomCenter.y - m_minorRadius, ptToClip.y,
                         geomCenter.y + m_minorRadius);
      break;
    }
  };
}
...
Bool GeometryInfo::isPointInFootprint(const Coord3D& geomCenter,
  ...
  switch (m_type) {
    ...
    case GEOMETRY_BOX: {
      return isWithin(geomCenter.x - m_majorRadius, pt.x,
                      geomCenter.x + m_majorRadius) &&
             isWithin(geomCenter.y - m_minorRadius, pt.y,
                      geomCenter.y + m_minorRadius);
    }
  };
  return false;
  ...
  switch (m_type) {
    ...
    case GEOMETRY_BOX: {
      ...
      m_boundingSphereRadius =
          sqrt(sqr(m_majorRadius) + sqr(m_minorRadius) + sqr(m_height * 0.5));
      break;
    }
  };
}
...
//=============================================================================
void GeometryInfo::tweakExtents(ExtentModType extentModType,
                                Real extentModAmount) {
  switch (extentModType) {
    ...
    case EXTENTMOD_MINOR:
      ...
      break;
    case EXTENTMOD_TYPE:
      m_type = (GeometryType)((m_type +
                               ((extentModType == EXTENTMOD_TYPE) ? 1 : 0)) %
                              GEOMETRY_NUM_TYPES);
      break;
  }
  m_isSmall = false;
...
AsciiString GeometryInfo::getDescriptiveString() const {
  AsciiString msg;
  msg.format("%d/%d(%g %g %g)", (Int)m_type, (Int)m_isSmall, m_majorRadius,
             m_minorRadius, m_height);
  return msg;
}
#endif
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void GeometryInfo::crc(Xfer* xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void GeometryInfo::loadPostProcess(void) {}  // end loadPostProcess