...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
SaveGameInfo::~SaveGameInfo(void) {}  // end ~SaveGameInfo
...
GameState::SnapshotBlock *GameState::findBlockInfoByToken(AsciiString token,
  ...
  for (it = m_snapshotBlockList[which].begin();
    ...
    // check for match
    if (blockInfo->blockName == token) return blockInfo;
...
#define DATE_BUFFER_SIZE 256
  ...
  if (GetVersionEx(&osvi)) {  // check if we're running Win9x variant since they
    ...
    if (osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS) {
      ...
      GetDateFormat(LOCALE_SYSTEM_DEFAULT, DATE_SHORTDATE, &timeVal, NULL,
  ...
  GetDateFormatW(LOCALE_SYSTEM_DEFAULT, DATE_SHORTDATE, &timeVal, NULL,
                 dateBuffer, sizeof(dateBuffer));
  ...
  if (GetVersionEx(&osvi)) {  // check if we're running Win9x variant since they
    ...
    if (osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS) {
      ...
      GetTimeFormat(LOCALE_SYSTEM_DEFAULT,
                    ...
                    &timeVal, NULL, timeBuffer, sizeof(timeBuffer));
...
#define TIME_BUFFER_SIZE 256
  ...
  GetTimeFormatW(LOCALE_SYSTEM_DEFAULT, TIME_NOSECONDS, &timeVal, NULL,
...
GameState::~GameState(void) {
  ...
  for (Int i = 0; i < SNAPSHOT_MAX; ++i) m_snapshotBlockList[i].clear();
...
static void findHighFileNumber(AsciiString filename, void *userData) {
  ...
  if (filename.endsWithNoCase(SAVE_GAME_EXTENSION) == FALSE) return;
  ...
  //
  if (fileNumber == 0 && nameOnly.compare(ZERO_NAME_ONLY) != 0) return;

  // compare against the file number we're keeping tally of in the user data
  // parameter
  Int *highFileNumber = (Int *)userData;
  if (fileNumber >= *highFileNumber) *highFileNumber = fileNumber;
...
#define COCKBEER_DONT_USE_NORMAL_FILE_NAMES_THANKS
  ...
  if (searchType == HIGHEST_NUMBER) {
    ...
    while (TRUE) {
      ...
      if (_access(fullPath.str(), 0) == -1) return filename;

      // test the text filename
      i++;

      // check for at the max limit (this is highly unlikely but possible)
      if (i > MAX_SAVE_FILE_NUMBER) return AsciiString::TheEmptyString;
...
SaveCode GameState::loadGame(AvailableGameInfo gameInfo) {
  ...
  if (doesSaveGameExist(gameInfo.filename) == FALSE) return SC_FILE_NOT_FOUND;
...
//-------------------------------------------------------------------------------------------------
AsciiString GameState::getFilePathInSaveDirectory(
    const AsciiString &leaf) const {
...
// ------------------------------------------------------------------------------------------------
AsciiString GameState::realMapPathToPortableMapPath(
    const AsciiString &in) const {
  AsciiString prefix;
  if (in.startsWithNoCase(getSaveDirectory())) {
    ...
    DEBUG_CRASH(
        ("Map file was not found in any of the expected directories; this is "
         "impossible"));
...
// ------------------------------------------------------------------------------------------------
AsciiString GameState::portableMapPathToRealMapPath(
    const AsciiString &in) const {
  AsciiString prefix;
...
void GameState::getSaveGameInfoFromFile(AsciiString filename,
  ...
  while (done == FALSE) {
    ...
    if (token.compareNoCase(SAVE_FILE_EOF) == 0) {
      ...
      // needed
      DEBUG_CRASH(
          ("GameState::getSaveGameInfoFromFile - Game info not found in file "
           "'%s'\n",
           filename.str()));
      done = TRUE;
      ...
      if (blockInfo == NULL) throw SC_UNKNOWN_BLOCK;
      ...
      if (stricmp(token.str(), GAME_STATE_BLOCK_STRING) == 0) {
        ...
        try {
          ...
          DEBUG_CRASH(
              ("GameState::getSaveGameInfoFromFile - Error loading block '%s' "
               "in file '%s'\n",
...
static void addGameToAvailableList(AsciiString filename, void *userData) {
  ...
  // sanity
  DEBUG_ASSERTCRASH(listHead != NULL,
...
void GameState::iterateSaveFiles(IterateSaveFileCallback callback,
  ...
  while (done == FALSE) {
    ...
    // on to the next file
    if (FindNextFile(hFile, &item) == 0) done = TRUE;
...
void GameState::friend_xferSaveDataForCRC(Xfer *xfer, SnapshotType which) {
  DEBUG_LOG(
      ("GameState::friend_xferSaveDataForCRC() - SnapshotType %d\n", which));
  SaveGameInfo *gameInfo = getSaveGameInfo();
...
void GameState::xferSaveData(Xfer *xfer, SnapshotType which) {
  // sanity
  if (xfer == NULL) throw SC_INVALID_XFER;
