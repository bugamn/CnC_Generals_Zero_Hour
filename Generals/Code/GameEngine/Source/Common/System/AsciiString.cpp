...
void AsciiString::validate() const {
  ...
  DEBUG_ASSERTCRASH(m_data->m_refCount < 32000,
                    ("m_refCount is suspiciously large"));
  DEBUG_ASSERTCRASH(m_data->m_numCharsAllocated > 0,
                    ("m_numCharsAllocated is zero"));
  //	DEBUG_ASSERTCRASH(m_data->m_numCharsAllocated < 1024,
  //("m_numCharsAllocated suspiciously large"));
  DEBUG_ASSERTCRASH(
...
// -----------------------------------------------------
void AsciiString::ensureUniqueBufferOfSize(int numCharsNeeded,
                                           Bool preserveData,
                                           const char* strToCopy,
                                           const char* strToCat) {
  validate();

  if (m_data && m_data->m_refCount == 1 &&
      m_data->m_numCharsAllocated >= numCharsNeeded) {
    // no buffer manhandling is needed (it's already large enough, and unique to
    // us)
    if (strToCopy) strcpy(m_data->peek(), strToCopy);
    if (strToCat) strcat(m_data->peek(), strToCat);
    return;
  }

  int minBytes = sizeof(AsciiStringData) + numCharsNeeded * sizeof(char);
  if (minBytes > MAX_LEN) throw ERROR_OUT_OF_MEMORY;

  int actualBytes =
      TheDynamicMemoryAllocator->getActualAllocationSize(minBytes);
  AsciiStringData* newData =
      (AsciiStringData*)TheDynamicMemoryAllocator->allocateBytesDoNotZero(
          actualBytes, "STR_AsciiString::ensureUniqueBufferOfSize");
  newData->m_refCount = 1;
  newData->m_numCharsAllocated =
      (actualBytes - sizeof(AsciiStringData)) / sizeof(char);
#if defined(_DEBUG) || defined(_INTERNAL)
  newData->m_debugptr =
      newData->peek();  // just makes it easier to read in the debugger
#endif
  ...
  if (strToCopy) strcpy(newData->peek(), strToCopy);
  if (strToCat) strcat(newData->peek(), strToCat);
...
AsciiString::AsciiString(const char* s) : m_data(0) {
  // DEBUG_ASSERTCRASH(isMemoryManagerOfficiallyInited(), ("Initializing
  // AsciiStrings prior to main (ie, as static vars) can cause memory leak
  // reporting problems. Are you sure you want to do this?\n"));
...
void AsciiString::translate(const UnicodeString& stringSrc) {
  validate();
  /// @todo srj put in a real translation here; this will only work for 7-bit
  /// ascii
  clear();
  Int len = stringSrc.getLength();
  for (Int i = 0; i < len; i++) concat((char)stringSrc.getCharAt(i));
  validate();
}
...
void AsciiString::concat(const char* s) {
  validate();
  int addlen = strlen(s);
  if (addlen == 0) return;  // my, that was easy
...
Bool AsciiString::startsWith(const char* p) const {
  if (*p == 0) return true;  // everything starts with the empty string

  int lenThis = getLength();
  int lenThat = strlen(p);
  if (lenThis < lenThat) return false;  // that must be smaller than this
...
Bool AsciiString::endsWith(const char* p) const {
  if (*p == 0) return true;  // everything ends with the empty string
...
Bool AsciiString::nextToken(AsciiString* tok, const char* seps) {
  if (this->isEmpty() || tok == this) return false;
