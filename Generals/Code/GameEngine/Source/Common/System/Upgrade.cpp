...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
Upgrade::~Upgrade(void) {}  // end ~Upgrade

// ------------------------------------------------------------------------------------------------
/** CRC */
// ------------------------------------------------------------------------------------------------
void Upgrade::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void Upgrade::loadPostProcess(void) {}  // end loadPostProcess
...
const FieldParse UpgradeTemplate::m_upgradeFieldParseTable[] = {
    ...
    {"DisplayName", INI::parseAsciiString, NULL,
     offsetof(UpgradeTemplate, m_displayNameLabel)},
    {"Type", INI::parseIndexList, UpgradeTypeNames,
     offsetof(UpgradeTemplate, m_type)},
    ...
    {"ButtonImage", INI::parseAsciiString, NULL,
     offsetof(UpgradeTemplate, m_buttonImageName)},
    {"ResearchSound", INI::parseAudioEventRTS, NULL,
     offsetof(UpgradeTemplate, m_researchSound)},
    {"UnitSpecificSound", INI::parseAudioEventRTS, NULL,
     offsetof(UpgradeTemplate, m_unitSpecificSound)},
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
UpgradeTemplate::~UpgradeTemplate(void) {}  // end ~UpgradeTemplate

//-------------------------------------------------------------------------------------------------
/** Calculate the time it takes (in logic frames) for a player to build this
 * UpgradeTemplate */
//-------------------------------------------------------------------------------------------------
...
void UpgradeTemplate::friend_makeVeterancyUpgrade(VeterancyLevel v) {
  m_type = UPGRADE_TYPE_OBJECT;  // veterancy "upgrades" are always per-object,
                                 // not per-player
...
void UpgradeTemplate::cacheButtonImage() {
  if (m_buttonImageName.isNotEmpty()) {
    ...
    DEBUG_ASSERTCRASH(m_buttonImage,
                      ("UpgradeTemplate: %s is looking for button image %s but "
                       "can't find it. Skipping...",
...
//-------------------------------------------------------------------------------------------------
const UpgradeTemplate *UpgradeCenter::findVeterancyUpgrade(
    VeterancyLevel level) const {
...
UpgradeTemplate *UpgradeCenter::findNonConstUpgradeByKey(NameKeyType key) {
  ...
  for (upgrade = m_upgradeList; upgrade; upgrade = upgrade->friend_getNext())
    if (upgrade->getUpgradeNameKey() == key) return upgrade;
...
//-------------------------------------------------------------------------------------------------
const UpgradeTemplate *UpgradeCenter::findUpgrade(
...
UpgradeTemplate *UpgradeCenter::newUpgrade(const AsciiString &name) {
  ...
  if (defaultUpgrade) *newUpgrade = *defaultUpgrade;
  ...
  // Make a unique bitmask for this new template by keeping track of what bits
  // have been assigned damn MSFT! proper ANSI syntax for a proper 64-bit
  // constant is "1LL", but MSVC doesn't recognize it
  Int64 newMask = 1i64 << m_nextTemplateMaskBit;
  m_nextTemplateMaskBit++;
  DEBUG_ASSERTCRASH(
      m_nextTemplateMaskBit < 64,
      ("Can't have over 64 types of Upgrades and have a Bitfield function."));
...
void UpgradeCenter::linkUpgrade(UpgradeTemplate *upgrade) {
  ...
  if (m_upgradeList) m_upgradeList->friend_setPrev(upgrade);
...
Bool UpgradeCenter::canAffordUpgrade(Player *player,
                                     ...
                                     Bool displayReason) const {
  // sanity
  if (player == NULL || upgradeTemplate == NULL) return FALSE;
...
std::vector<AsciiString> UpgradeCenter::getUpgradeNames(void) const {
  ...
  for (UpgradeTemplate *upgrade = m_upgradeList; upgrade;
       upgrade = upgrade->friend_getNext())
...
void UpgradeCenter::parseUpgradeDefinition(INI *ini) {
  ...
  UpgradeTemplate *upgrade =
      TheUpgradeCenter->findNonConstUpgradeByKey(NAMEKEY(name));
  if (upgrade == NULL) {
    // allocate a new item
  ...
  // sanity
  DEBUG_ASSERTCRASH(
      upgrade, ("parseUpgradeDefinition: Unable to allocate upgrade '%s'\n",
                name.str()));
