...
Int XferLoad::beginBlock(void) {
  ...
  if (fread(&blockSize, sizeof(XferBlockSize), 1, m_fileFP) != 1) {
    DEBUG_CRASH(
        ("Xfer - Error reading block size for '%s'\n", m_identifier.str()));
    return 0;
...
// ------------------------------------------------------------------------------------------------
void XferLoad::endBlock(void) {}  // end endBlock
...
void XferLoad::skip(Int dataSize) {
  ...
  DEBUG_ASSERTCRASH(
      m_fileFP != NULL,
      ("XferLoad::skip - file pointer for '%s' is NULL\n", m_identifier.str()));

  // sanity
  DEBUG_ASSERTCRASH(
      dataSize >= 0,
      ("XferLoad::skip - dataSize '%d' must be greater than 0\n", dataSize));

  // skip datasize in the file from the current position
  if (fseek(m_fileFP, dataSize, SEEK_CUR) != 0) throw XFER_SKIP_ERROR;
...
void XferLoad::xferSnapshot(Snapshot *snapshot) {
  ...
  // add this snapshot to the game state for later post processing if not
  // restricted
...
void XferLoad::xferAsciiString(AsciiString *asciiStringData) {
  ...
  if (len > 0) xferUser(buffer, sizeof(Byte) * len);
...
void XferLoad::xferUnicodeString(UnicodeString *unicodeStringData) {
  ...
  if (len > 0) xferUser(buffer, sizeof(WideChar) * len);
  buffer[len] = 0;  // terminate
...
void XferLoad::xferImplementation(void *data, Int dataSize) {
  ...
  DEBUG_ASSERTCRASH(
      m_fileFP != NULL,
      ("XferLoad - file pointer for '%s' is NULL\n", m_identifier.str()));

  // read data from file
  if (fread(data, dataSize, 1, m_fileFP) != 1) {
    DEBUG_CRASH(
        ("XferLoad - Error reading from file '%s'\n", m_identifier.str()));
