...
BOOL(__stdcall *gsSymGetLineFromAddr)(IN HANDLE hProcess, IN DWORD dwAddr,
...
BOOL InitSymbolInfo() {
  ...
  gsSymGetLineFromAddr = (BOOL(__stdcall *)(
      IN HANDLE, IN DWORD, OUT PDWORD,
      OUT PIMAGEHLP_LINE))GetProcAddress(hInstDebugHlp, "SymGetLineFromAddr");
...
void MakeStackTrace(DWORD myeip, DWORD myesp, DWORD myebp, int skipFrames,
  ...
  stack_frame.AddrFrame.Offset = myebp;
  {
    ...
    */
    ...
    while (b_ret && skip) {
      b_ret = StackWalk(IMAGE_FILE_MACHINE_I386, process, thread, &stack_frame,
                        NULL,  //&gsContext,
...

  {
    ...
    */
    ...
    while (stillgoing && skip) {
      stillgoing =
          StackWalk(IMAGE_FILE_MACHINE_I386, process, thread, &stack_frame,
                    NULL,  //&gsContext,
                    NULL, SymFunctionTableAccess, SymGetModuleBase, NULL) != 0;
      skip--;
    }
