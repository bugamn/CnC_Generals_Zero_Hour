...
void ArchiveFileSystem::loadIntoDirectoryTree(
    const ArchiveFile *archiveFile, const AsciiString &archiveFilename,
    Bool overwrite) {
  FilenameList filenameList;

  archiveFile->getFileListInDirectory(AsciiString(""), AsciiString(""),
  ...
  while (it != filenameList.end()) {
    ...
    while (infoInPath && (!token.find('.') || path.find('.'))) {
      ArchivedDirectoryInfoMap::iterator tempiter =
          dirInfo->m_directories.find(token);
    ...
    if (dirInfo->m_files.find(token) == dirInfo->m_files.end() || overwrite) {
      ...
      path2 = debugpath;
      path2.concat(token);
      //			DEBUG_LOG(("ArchiveFileSystem::loadIntoDirectoryTree
      //- adding file %s, archived in %s\n", path2.str(),
      //archiveFilename.str()));
...
void ArchiveFileSystem::loadMods() {
  if (TheGlobalData->m_modBIG.isNotEmpty()) {
    ...
    if (archiveFile != NULL) {
      DEBUG_LOG((
          "ArchiveFileSystem::loadMods - loading %s into the directory tree.\n",
      ...
      DEBUG_LOG(
          ("ArchiveFileSystem::loadMods - %s inserted into the archive file "
           "map.\n",
           TheGlobalData->m_modBIG.str()));
    } else {
      DEBUG_LOG(
          ("ArchiveFileSystem::loadMods - could not openArchiveFile(%s)\n",
           TheGlobalData->m_modBIG.str()));
    }
  }
...
Bool ArchiveFileSystem::doesFileExist(const Char *filename) const {
  ...
  while (!token.find('.') || path.find('.')) {
    ArchivedDirectoryInfoMap::const_iterator tempiter =
        dirInfo->m_directories.find(token);
    if (tempiter != dirInfo->m_directories.end()) {
...
AsciiString ArchiveFileSystem::getArchiveFilenameForFile(
  ...
  while (!token.find('.') || path.find('.')) {
    ArchivedDirectoryInfoMap::const_iterator it =
        dirInfo->m_directories.find(token);
    if (it != dirInfo->m_directories.end()) {
      ...
      // token.str(), debugpath.str())); DEBUG_LOG(("directories in %s in
      // archive file system are:\n", debugpath.str()));
      // ArchivedDirectoryInfoMap::const_iterator it =
      // dirInfo->m_directories.begin(); while (it !=
      // dirInfo->m_directories.end()) { 	DEBUG_LOG(("\t%s\n",
      //it->second.m_directoryName.str())); 	it++;
      //}
...
void ArchiveFileSystem::getFileListInDirectory(
  ...
  while (it != m_archiveFileMap.end()) {
    it->second->getFileListInDirectory(currentDirectory, originalDirectory,
