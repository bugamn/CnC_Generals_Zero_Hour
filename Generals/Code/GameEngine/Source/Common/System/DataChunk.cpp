...
Bool CachedFileInputStream::open(AsciiString path) {
  ...
  if (CompressionManager::isDataCompressed(m_buffer, m_size) == 0) {
    // DEBUG_LOG(("CachedFileInputStream::open() - file %s is uncompressed at %d
    // bytes!\n", path.str(), m_size));
  } else {
    Int uncompLen = CompressionManager::getUncompressedSize(m_buffer, m_size);
    // DEBUG_LOG(("CachedFileInputStream::open() - file %s is compressed!  It
    // should go from %d to %d\n", path.str(), 	m_size, uncompLen));
    char *uncompBuffer = NEW char[uncompLen];
    Int actualLen = CompressionManager::decompressData(m_buffer, m_size,
                                                       uncompBuffer, uncompLen);
...
UnsignedInt CachedFileInputStream::tell(void) { return m_pos; }
...
Bool CachedFileInputStream::eof(void) { return m_size == m_pos; }
...
DataChunkOutput::DataChunkOutput(OutputStream *pOut) : m_pOut(pOut) {
...
DataChunkTableOfContents::DataChunkTableOfContents(void)
    : m_list(NULL), m_nextID(1), m_listLength(0), m_headerOpened(false) {}
...
UnsignedInt DataChunkTableOfContents::getID(const AsciiString &name) {
  ...
  DEBUG_CRASH(
      ("name not found in DataChunkTableOfContents::getName for name %s\n",
       name.str()));
  return 0;
}
...
AsciiString DataChunkTableOfContents::getName(UnsignedInt id) {
  ...
  DEBUG_CRASH(
      ("name not found in DataChunkTableOfContents::getName for id %d\n", id));
...
Bool DataChunkInput::parse(void *userData) {
  ...
  if (m_chunkStack) parentLabel = m_contents.getName(m_chunkStack->id);

  while (atEndOfFile() == false) {
    if (m_chunkStack) {  // If we are parsing chunks in a chunk, check current
                         // length.
      if (m_chunkStack->dataLeft < CHUNK_HEADER_BYTES) {
        DEBUG_ASSERTCRASH(m_chunkStack->dataLeft == 0,
                          ("Unexpected extra data in chunk."));
        break;
      }
    }
    // open the chunk
    label = openDataChunk(&ver);
    if (atEndOfFile()) {  // FILE * returns eof after you read past end of file,
                          // so check.
      break;
    }
    ...
    for (parser = m_parserList; parser; parser = parser->next) {
      ...
      if (parser->label == label) {
        ...
        if (parentLabel != parser->parentLabel) scopeOK = false;
        ...
        if (scopeOK) {
          ...
          if (parser->parser(*this, &info, userData) == false) return false;
          break;
        }
      }
    }
...
Bool DataChunkInput::atEndOfChunk(void) {
  if (m_chunkStack) {
    if (m_chunkStack->dataLeft <= 0) return true;
    return false;
  }
...
Real DataChunkInput::readReal(void) {
  Real r;
  DEBUG_ASSERTCRASH(m_chunkStack->dataLeft >= sizeof(Real),
...
Int DataChunkInput::readInt(void) {
  Int i;
  DEBUG_ASSERTCRASH(m_chunkStack->dataLeft >= sizeof(Int),
                    ("Read past end of chunk."));
...
Byte DataChunkInput::readByte(void) {
  Byte b;
  DEBUG_ASSERTCRASH(m_chunkStack->dataLeft >= sizeof(Byte),
                    ("Read past end of chunk."));
...
Dict DataChunkInput::readDict() {
  UnsignedShort len;
  DEBUG_ASSERTCRASH(m_chunkStack->dataLeft >= sizeof(UnsignedShort),
                    ("Read past end of chunk."));
...
AsciiString DataChunkInput::readAsciiString(void) {
  ...
  if (len > 0) {
    ...
    m_file->read(str, len);
    decrementDataLeft(len);
    // add null delimiter to string.  Note that getBufferForRead allocates space
