...
void SubsystemInterface::UPDATE(void) {
  ...
  if (m_curUpdateTime > 0.00001) {
    ...
    //	m_name.str(), m_curUpdateTime*1000, subTime*1000,
    //(m_curUpdateTime-subTime)*1000	));
...
#ifdef DUMP_PERF_STATS
  for (SubsystemList::iterator it = m_allSubsystems.begin();
       it != m_subsystems.end(); ++it) {
    if ((*it) == sys) {
...
void SubsystemInterfaceList::initSubsystem(SubsystemInterface* sys,
                                           ...
                                           const char* dirpath, Xfer* pXfer,
                                           AsciiString name) {
  sys->setName(name);
  sys->init();

  INI ini;
  if (path1) ini.load(path1, INI_LOAD_OVERWRITE, pXfer);
  if (path2) ini.load(path2, INI_LOAD_OVERWRITE, pXfer);
  if (dirpath) ini.loadDirectory(dirpath, TRUE, INI_LOAD_OVERWRITE, pXfer);
...
void SubsystemInterfaceList::postProcessLoadAll() {
  for (SubsystemList::iterator it = m_subsystems.begin();
       it != m_subsystems.end(); ++it) {
    (*it)->postProcessLoad();
  }
}
...
void SubsystemInterfaceList::resetAll() {
  //	for (SubsystemList::iterator it = m_subsystems.begin(); it !=
  //m_subsystems.end(); ++it)
  for (SubsystemList::reverse_iterator it = m_subsystems.rbegin();
       it != m_subsystems.rend(); ++it) {
    (*it)->reset();
  }
}
...
AsciiString SubsystemInterfaceList::dumpTimesForAll() {
  ...
  for (SubsystemList::reverse_iterator it = m_allSubsystems.rbegin();
       it != m_allSubsystems.rend(); ++it) {
    SubsystemInterface* sys = *it;
