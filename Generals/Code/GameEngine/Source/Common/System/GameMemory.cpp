...
typedef std::map<const char *, UsedNPeak, std::less<const char *> >
    UsedNPeakMap;
...
static void *sysAllocate(Int numBytes) {
  ...
  if (!p) throw ERROR_OUT_OF_MEMORY;
#ifdef MEMORYPOOL_DEBUG
  {
...
static void memset32(void *ptr, Int value, Int bytesToFill) {
  ...
  Int *p = (Int *)ptr;
  for (++wordsToFill; --wordsToFill;) *p++ = value;

  Byte *b = (Byte *)p;
  for (++bytesToFill; --bytesToFill;) *b++ = (Byte)value;
}
...
#ifdef MEMORYPOOL_DEBUG
  const char
      *m_debugLiteralTagString;  ///< ptr to the tagstring for this block.
...
inline void MemoryPoolSingleBlock::setNextFreeBlock(MemoryPoolSingleBlock *b) {
  // DEBUG_ASSERTCRASH(m_owningBlob != NULL && b->m_owningBlob != NULL, ("must
  // be called on blob block"));
...
inline void MemoryPoolSingleBlock::setNextRawBlock(MemoryPoolSingleBlock *b) {
  DEBUG_ASSERTCRASH(m_owningBlob == NULL && (!b || b->m_owningBlob == NULL),
                    ("must be called on raw block"));
  m_nextBlock = b;
#ifdef MPSB_DLINK
  if (b) b->m_prevBlock = this;
#endif
}
...
*/
inline void MemoryPoolSingleBlock::debugSetCheckpointInfo(
...
/*static*/ void BlockCheckpointInfo::doBlockCheckpointReport(
  ...
  if (!bi) {
    DEBUG_LOG(("%s,%32s,%6s,%6s,%6s,%s\n", PREPEND, "POOLNAME", "BLKSZ",
               "ALLOC", "FREED", "BLOCKNAME"));
  } else {
    ...
    if (bi->shouldBeInReport(flags, startCheckpoint, endCheckpoint)) {
      DEBUG_LOG(("%s,%32s,%6d,%6d,%6d,%s\n", PREPEND, poolName, bi->m_blockSize,
...
/*static*/ BlockCheckpointInfo *BlockCheckpointInfo::addToList(
    BlockCheckpointInfo **pHead, const char *debugLiteralTagString,
    Int allocCheckpoint, Int blockSize) {
  ...
  if (freed) {
    DEBUG_ASSERTCRASH(debugLiteralTagString != NULL,
...
void MemoryPoolSingleBlock::initBlock(
...
#ifdef MEMORYPOOL_DEBUG
  {
    ...
    m_debugFlags = 0;
    if (!theMainInitFlag) debugIgnoreLeaksForThisBlock();
    DEBUG_ASSERTCRASH(debugLiteralTagString != NULL,
                      ("null tagstrings are not allowed"));
...
void MemoryPoolSingleBlock::removeBlockFromList(MemoryPoolSingleBlock **pHead) {
  DEBUG_ASSERTCRASH(this->m_owningBlob == NULL,
  ...
  DEBUG_ASSERTCRASH(
      ...
      ("this function should only be used on raw blocks"));
  if (this->m_prevBlock) {
    DEBUG_ASSERTCRASH(this->m_prevBlock->m_owningBlob == NULL,
                      ("this function should only be used on raw blocks"));
  ...
  if (this->m_nextBlock) {
    DEBUG_ASSERTCRASH(this->m_nextBlock->m_owningBlob == NULL,
                      ("this function should only be used on raw blocks"));
  ...
  for (MemoryPoolSingleBlock *cur = *pHead; cur; cur = cur->m_nextBlock) {
    DEBUG_ASSERTCRASH(cur->m_owningBlob == NULL,
                      ("this function should only be used on raw blocks"));
    if (cur == this) {
      if (prev) {
...
Int MemoryPoolSingleBlock::debugSingleBlockReportLeak(const char *owner) {
  ...
  if (m_debugFlags & IGNORE_LEAKS) return 0;

  // it's free, ergo, not leaked.
  if (m_debugLiteralTagString == FREE_SINGLEBLOCK_TAG_STRING) return 0;
...
void MemoryPoolSingleBlock::debugVerifyBlock() {
  ...
  DEBUG_ASSERTCRASH(!m_nextBlock || m_nextBlock->m_owningBlob == m_owningBlob,
                    ("owning blob mismatch..."));
#ifdef MPSB_DLINK
  DEBUG_ASSERTCRASH(!m_prevBlock || m_prevBlock->m_owningBlob == m_owningBlob,
                    ("owning blob mismatch..."));
#endif
  debugCheckUnderrun();
  debugCheckOverrun();
...
Bool MemoryPoolSingleBlock::debugCheckUnderrun() {
  ...
  for (Int i = 0; i < WALLCOUNT; i++, p++) {
    if (*p != m_wallPattern + i) {
      DEBUG_CRASH(
          ("memory underrun for block \"%s\" (expected %08x, got %08x)\n",
...
Bool MemoryPoolSingleBlock::debugCheckOverrun() {
  ...
  for (Int i = 0; i < WALLCOUNT; i++, p++) {
    if (*p != m_wallPattern - i) {
      DEBUG_CRASH(
          ("memory overrun for block \"%s\" (expected %08x, got %08x)\n",
           m_debugLiteralTagString, m_wallPattern + i, *p));
      return true;
    }
  }
...
void MemoryPoolSingleBlock::debugFillInWalls() {
  ...
  for (i = 0; i < WALLCOUNT; i++) *p++ = m_wallPattern + i;
...
void MemoryPoolBlob::addBlobToList(MemoryPoolBlob **ppHead,
  ...
  if (*ppTail != NULL) (*ppTail)->m_nextBlob = this;

  if (*ppHead == NULL) *ppHead = this;
...
void MemoryPoolBlob::debugMemoryVerifyBlob() {
  ...
  DEBUG_ASSERTCRASH(
      m_usedBlocksInBlob >= 0 && m_usedBlocksInBlob <= m_totalBlocksInBlob,
...
Bool MemoryPoolBlob::debugIsBlockInBlob(void *pBlockPtr) {
  ...
  char *blockData = m_blockData;
  for (Int i = m_totalBlocksInBlob - 1; i >= 0; i--) {
    if ((char *)block == blockData) return true;
...
Checkpointable::Checkpointable()
    : m_firstCheckpointInfo(NULL), m_cpiEverFailed(false) {}
#endif
...
BlockCheckpointInfo *Checkpointable::debugAddCheckpointInfo(
    const char *debugLiteralTagString, Int allocCheckpoint, Int blockSize) {
...
void Checkpointable::debugCheckpointReport(Int flags, Int startCheckpoint,
  ...
  if (m_cpiEverFailed) {
    DEBUG_LOG(
        ("  *** WARNING *** info on freed blocks may be inaccurate or "
         "incomplete!\n"));
  }
...
MemoryPoolBlob *MemoryPool::createBlob(Int allocationCount) {
  ...
  MemoryPoolBlob *blob = new (::sysAllocate(sizeof MemoryPoolBlob))
  ...
  DEBUG_ASSERTCRASH(m_firstBlobWithFreeBlocks == NULL,
...
void *MemoryPool::allocateBlockDoNotZeroImplementation(
  ...
  if (m_firstBlobWithFreeBlocks != NULL &&
    ...
    for (MemoryPoolBlob *blob = m_firstBlob; blob != NULL;
         blob = blob->getNextInList()) {
      if (blob->hasAnyFreeBlocks()) break;
    }
  ...
  BlockCheckpointInfo *bi = debugAddCheckpointInfo(
      ...
      getAllocationSize());
  if (bi) block->debugSetCheckpointInfo(bi);
#endif
...
void MemoryPool::freeBlock(void *pBlockPtr) {
  if (!pBlockPtr) return;  // my, that was easy
  ...
  if (bi) bi->debugSetFreepoint(m_factory->getCurCheckpoint());
#endif
  ...
  if (!m_firstBlobWithFreeBlocks) m_firstBlobWithFreeBlocks = blob;
...
Int MemoryPool::releaseEmpties() {
  ...
  for (MemoryPoolBlob *blob = m_firstBlob; blob;) {
    ...
    if (blob->getUsedBlockCount() == 0) released += freeBlob(blob);
    blob = pNext;
  }
  return released;
}
...
Bool MemoryPool::debugIsBlockInPool(void *pBlockPtr) {
  ...
  for (MemoryPoolBlob *blob = m_firstBlob; blob; blob = blob->getNextInList()) {
    if (blob->debugIsBlockInBlob(pBlockPtr)) check1 = true;

    if (blob == ownerBlob) check2 = true;
  }
...
const char *MemoryPool::debugGetBlockTagString(void *pBlockPtr) {
  ...
  if (!pBlockPtr) return FREE_SINGLEBLOCK_TAG_STRING;
...
DynamicMemoryAllocator::DynamicMemoryAllocator()
    : m_factory(NULL),
      ...
      m_usedBlocksInDma(0),
      m_rawBlocks(NULL) {
  for (Int i = 0; i < MAX_DYNAMICMEMORYALLOCATOR_SUBPOOLS; i++) m_pools[i] = 0;
}
...
MemoryPool *DynamicMemoryAllocator::findPoolForSize(Int allocSize) {
  for (Int i = 0; i < m_numPools; i++) {
    ...
    if (allocSize <= m_pools[i]->getAllocationSize()) return m_pools[i];
  }
  return NULL;
}
...
void DynamicMemoryAllocator::debugIgnoreLeaksForThisBlock(void *pBlockPtr) {
  ...
  if (block->getOwningBlob()) {
    ...
    DEBUG_CRASH(
        ("cannot currently ignore leaks for raw blocks (allocation too "
         "large)\n"));
  }
}
#endif
...
void *DynamicMemoryAllocator::allocateBytesDoNotZeroImplementation(
...
#ifdef MEMORYPOOL_DEBUG
  {
    ...
    if (thePeakDMA < theTotalDMA) thePeakDMA = theTotalDMA;
#ifdef INTENSE_DMA_BOOKKEEPING
    if (isMemoryManagerOfficiallyInited() && doingIntenseDMA == 0) {
      ++doingIntenseDMA;
      UsedNPeak &up = TheUsedNPeakMap[debugLiteralTagString];
      up.used += numBytes;
      if (up.peak < up.used) up.peak = up.used;
      up.waste += waste;
      if (up.peakwaste < up.waste) up.peakwaste = up.waste;
      --doingIntenseDMA;
    }
#endif
  }
...
void DynamicMemoryAllocator::freeBytes(void *pBlockPtr) {
  ...
  if (block->getOwningBlob()) {
#ifdef MEMORYPOOL_DEBUG
    {
      USE_PERF_TIMER(MemoryPoolDebugging)
      DEBUG_ASSERTCRASH(
          findPoolForSize(used) == block->getOwningBlob()->getOwningPool(),
          ("pool mismatch"));
#ifdef INTENSE_DMA_BOOKKEEPING
      if (doingIntenseDMA == 0)
#endif
...
void DynamicMemoryAllocator::reset() {
  ...
  while (m_rawBlocks) freeBytes(m_rawBlocks->getUserData());
...
MemoryPool *MemoryPoolFactory::createMemoryPool(const char *poolName,
  ...
  pool = new (::sysAllocate(sizeof MemoryPool))
...
/**
   ...
   call this directly.
*/
DynamicMemoryAllocator *MemoryPoolFactory::createDynamicMemoryAllocator(
    Int numSubPools, const PoolInitRec pParms[]) {
  DynamicMemoryAllocator *dma;

  dma = new (::sysAllocate(sizeof DynamicMemoryAllocator))
...
void MemoryPoolFactory::adjustTotals(const char *tagString, Int usedDelta,
  ...
  if (m_peakUsedBytes < m_usedBytes) m_peakUsedBytes = m_usedBytes;
  if (m_peakPhysBytes < m_physBytes) m_peakPhysBytes = m_physBytes;

  int found = 0;  // if no matches found, goes into slot zero
  for (int i = 1; i < MAX_SPECIAL_USED; ++i)  // start at 1, not zero
  {
    if (s_specialPrefixes[i] == NULL) break;
...
Bool MemoryPoolFactory::debugIsBlockInAnyPool(void *pBlock) {
  ...
  for (MemoryPool *pool = m_firstPoolInFactory; pool;
       pool = pool->getNextPoolInList()) {
    if (pool->debugIsBlockInPool(pBlock)) return true;
  }

  for (DynamicMemoryAllocator *dma = m_firstDmaInFactory; dma;
       dma = dma->getNextDmaInList()) {
    if (dma->debugIsBlockInDma(pBlock)) return true;
  }
...
void MemoryPoolFactory::debugMemoryReport(Int flags, Int startCheckpoint,
  ...
  if (flags & REPORT_POOL_OVERFLOW) {
    ...
    for (MemoryPool *pool = m_firstPoolInFactory; pool;
      ...
      if (pool->getPeakBlockCount() > pool->getInitialBlockCount()) {
        DEBUG_LOG(
            ("*** Pool %s overflowed initial allocation of %d (peak allocation "
             "was %d)\n",
    ...
    for (pool = m_firstPoolInFactory; pool; pool = pool->getNextPoolInList()) {
      ...
      if (peak < initial / 2 && (initial - peak) > 4096) {
        DEBUG_LOG(
            ("*** Pool %s used less than half its initial allocation of %d "
             "(peak allocation was %d, wasted %dk)\n",
             pool->getPoolName(), pool->getInitialBlockCount(),
  ...
  if (flags & REPORT_SIMPLE_LEAKS) {
    ...
    DEBUG_ASSERTCRASH(!any,
                      ("There were %d memory leaks. Please fix them.\n", any));
    DEBUG_LOG(("------------------------------------------\n"));
...
/*
        This is a trick that is intended to force MSVC to link this file (and
   thus, these definitions of new/delete) ahead of all others. (We do debug
   checking to ensure that's the case)
*/
#if defined(_DEBUG)
...
void initMemoryManager() {
  if (TheMemoryPoolFactory == NULL) {
    ...
    TheMemoryPoolFactory = new (::sysAllocate(sizeof MemoryPoolFactory))
