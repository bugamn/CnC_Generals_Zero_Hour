...
static char theBuffer[LARGE_BUFFER];  // make it big to avoid weird overflow
                                      // bugs in debug mode
...
const char *TheDebugLevels[DEBUG_LEVEL_MAX] = {"NET"};
#endif
...
int MessageBoxWrapper(LPCSTR lpText, LPCSTR lpCaption, UINT uType) {
  ...
  if (!threadHWND) return (uType & MB_ABORTRETRYIGNORE) ? IDIGNORE : IDYES;
...
// ----------------------------------------------------------------------------
/**
        Empty the buffer passed in, then optionally prepend the current
   TickCount value in string form, depending on the setting of theDebugFlags.
*/
static const char *prepBuffer(const char *format, char *buffer) {
  buffer[0] = 0;
...
// doLogOutput
/**
        send a string directly to the log file and/or console without further
   processing.
*/
// ----------------------------------------------------------------------------
#ifdef DEBUG_LOGGING
...
// doCrashBox
/*
        present a messagebox with the given message. Depending on user
   selection, we exit the app, break into debugger, or continue execution.
*/
// ----------------------------------------------------------------------------
static int doCrashBox(const char *buffer, Bool logResult) {
  int result;
  ...
  if (!ignoringAsserts()) {
    // result = MessageBoxWrapper(buffer, "Assertion Failure",
    // MB_ABORTRETRYIGNORE|MB_TASKMODAL|MB_ICONWARNING|MB_DEFBUTTON3);
    result =
        MessageBoxWrapper(buffer, "Assertion Failure",
                          MB_ABORTRETRYIGNORE | MB_TASKMODAL | MB_ICONWARNING);
  } else {
    result = IDIGNORE;
  }
  ...
  switch (result) {
    case IDABORT:
#ifdef DEBUG_LOGGING
      if (logResult) DebugLog("[Abort]\n");
#endif
      _exit(1);
      break;
    case IDRETRY:
#ifdef DEBUG_LOGGING
      if (logResult) DebugLog("[Retry]\n");
#endif
      ::DebugBreak();
      break;
    case IDIGNORE:
...
#ifdef ALLOW_DEBUG_UTILS
/**
        Initialize the debug utilities. This should be called once, as near to
   the start of the app as possible, before anything else (since other code will
        probably want to make use of it).
*/
void DebugInit(int flags) {
  //	if (theDebugFlags != 0)
  //		::MessageBox(NULL, "Debug already inited", "",
  //MB_OK|MB_APPLMODAL);

  // just quietly allow multiple calls to this, so that static ctors can call
  // it.
...
void DebugLog(const char *format, ...) {
  ...
  if (theDebugFlags == 0)
    MessageBoxWrapper("DebugLog - Debug not inited properly", "",
                      MB_OK | MB_TASKMODAL);
  ...
  if (strlen(theBuffer) >= sizeof(theBuffer))
    MessageBoxWrapper("String too long for debug buffer", "",
                      MB_OK | MB_TASKMODAL);
...
/**
        Print a character string to the logfile and/or console, then halt
   execution while presenting the user with an exit/debug/ignore dialog
   containing the same text message.
*/
void DebugCrash(const char *format, ...) {
  // Note: You might want to make this thread safe, but we cannot. The reason is
  // that there is an implicit requirement on other threads that the message
  // loop be running.
  ...
  if (theDebugFlags == 0) {
    ...
    MessageBoxWrapper("DebugCrash - Debug not inited properly", "",
                      MB_OK | MB_TASKMODAL);
  }
  ...
  if (strlen(theCrashBuffer) >= sizeof(theCrashBuffer)) {
    ...
    MessageBoxWrapper("String too long for debug buffers", "",
                      MB_OK | MB_TASKMODAL);
  }
  ...
  if (result == IDIGNORE && TheCurrentIgnoreCrashPtr != NULL) {
    int yn;
    if (!ignoringAsserts()) {
      yn = MessageBoxWrapper("Ignore this crash from now on?", "",
                             MB_YESNO | MB_TASKMODAL);
    } else {
      yn = IDYES;
    }
    if (yn == IDYES) *TheCurrentIgnoreCrashPtr = 1;
    if (TheKeyboard) TheKeyboard->resetKeys();
    if (TheMouse) TheMouse->reset();
  }
}
#endif
...
#ifdef ALLOW_DEBUG_UTILS
/**
        Shut down the debug utilities. This should be called once, as near to
   the end of the app as possible, after everything else (since other code will
        probably want to make use of it).
*/
void DebugShutdown() {
#ifdef DEBUG_LOGGING
...
// ----------------------------------------------------------------------------
/**
        Get the current values for the flags passed to DebugInit. Most code will
   never need to use this; the most common usage would be to temporarily enable
   or disable the DEBUG_FLAG_PREPEND_TIME bit for complex logfile messages.
*/
int DebugGetFlags() { return theDebugFlags; }

// ----------------------------------------------------------------------------
// DebugSetFlags
// ----------------------------------------------------------------------------
/**
        Set the current values for the flags passed to DebugInit. Most code will
   never need to use this; the most common usage would be to temporarily enable
   or disable the DEBUG_FLAG_PREPEND_TIME bit for complex logfile messages.
*/
void DebugSetFlags(int flags) { theDebugFlags = flags; }
...
// ----------------------------------------------------------------------------
void SimpleProfiler::stopAndLog(const char *msg, int howOftenToLog,
                                int howOftenToResetAvg) {
  stop();
  ...
  DEBUG_ASSERTLOG(m_numSessions % howOftenToLog != 0,
                  ("%s: %f msec, total %f msec, avg %f msec\n", msg, getTime(),
...
double SimpleProfiler::getAverageTime() {
  ...
  return (double)m_totalAllSessions * 1000.0 /
         ((double)m_freq * (double)m_numSessions);
}
...
// ----------------------------------------------------------------------------
/**
        Halt the application, EVEN IN FINAL RELEASE BUILDS. This should be
   called only when a crash is guaranteed by continuing, and no meaningful
   continuation of processing is possible, even by throwing an exception.
*/
...
void ReleaseCrash(const char *reason) {
  ...
  if (theReleaseCrashLogFile) {
    ...
    void *stacktrace[STACKTRACE_SIZE];
    ::FillStackAddresses(stacktrace, STACKTRACE_SIZE, STACKTRACE_SKIP);
    ::StackDumpFromAddresses(stacktrace, STACKTRACE_SIZE,
                             releaseCrashLogOutput);
  ...
  buff[8191] = 0;
  ::MessageBox(NULL, buff, "Technical Difficulties...",
               MB_OK | MB_SYSTEMMODAL | MB_ICONERROR);
#else
  // crash error messaged changed 3/6/03 BGC
  //	::MessageBox(NULL, "Sorry, a serious error occurred.", "Technical
  //Difficulties...", MB_OK|MB_TASKMODAL|MB_ICONERROR);

  if (!GetRegistryLanguage().compareNoCase("german2") ||
      !GetRegistryLanguage().compareNoCase("german")) {
    ::MessageBox(
        NULL,
        "Es ist ein gravierender Fehler aufgetreten. Solche Fehler können "
        "durch viele verschiedene Dinge wie Viren, überhitzte Hardware und "
        "Hardware, die den Mindestanforderungen des Spiels nicht entspricht, "
        "ausgelöst werden. Tipps zur Vorgehensweise findest du in den Foren "
        "unter www.generals.ea.com, Informationen zum Technischen Kundendienst "
        "im Handbuch zum Spiel.",
        "Fehler...", MB_OK | MB_TASKMODAL | MB_ICONERROR);
  } else {
    ::MessageBox(NULL,
                 "You have encountered a serious error.  Serious errors can be "
                 "caused by many things including viruses, overheated hardware "
                 "and hardware that does not meet the minimum specifications "
                 "for the game. Please visit the forums at www.generals.ea.com "
                 "for suggested courses of action or consult your manual for "
...
void ReleaseCrashLocalized(const AsciiString &p, const AsciiString &m) {
  ...
  if (TheSystemIsUnicode) {
    ::MessageBoxW(NULL, mesg.str(), prompt.str(),
                  MB_OK | MB_SYSTEMMODAL | MB_ICONERROR);
  } else {
    ...
    ::MessageBoxA(NULL, mesgA.str(), promptA.str(),
                  MB_OK | MB_TASKMODAL | MB_ICONERROR);
  }
