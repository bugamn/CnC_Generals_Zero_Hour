...
NameKeyGenerator::NameKeyGenerator() {
  ...
  for (Int i = 0; i < SOCKET_COUNT; ++i) m_sockets[i] = NULL;
...
void NameKeyGenerator::init() {
  DEBUG_ASSERTCRASH(m_nextID == (UnsignedInt)NAMEKEY_INVALID,
                    ("NameKeyGen already inited"));
...
inline UnsignedInt calcHashForString(const char *p) {
  ...
  Byte *pp = (Byte *)p;
  while (*pp) result = (result << 5) + result + *pp++;
  return result;
}
...
AsciiString NameKeyGenerator::keyToName(NameKeyType key) {
  for (Int i = 0; i < SOCKET_COUNT; ++i) {
    for (Bucket *b = m_sockets[i]; b; b = b->m_nextInSocket) {
      if (key == b->m_key) return b->m_nameString;
    }
  }
...
NameKeyType NameKeyGenerator::nameToKey(const char *nameString) {
  ...
  for (b = m_sockets[hash]; b; b = b->m_nextInSocket) {
    if (strcmp(nameString, b->m_nameString.str()) == 0) return b->m_key;
  }
  ...
  for (Int i = 0; i < SOCKET_COUNT; ++i) {
    ...
    for (b = m_sockets[i]; b; b = b->m_nextInSocket) ++numInThisSocket;

    if (numInThisSocket > maxThresh) ++numOverThresh;
  }

  // if more than a small percent of the sockets are getting deep, probably want
  // to increase the socket count.
  if (numOverThresh > SOCKET_COUNT / 20) {
    DEBUG_CRASH(
        ("hmm, might need to increase the number of bucket-sockets for "
         "NameKeyGenerator (numOverThresh %d = %f%%)\n",
...
NameKeyType StaticNameKey::key() const {
  if (m_key == NAMEKEY_INVALID) {
    ...
    if (TheNameKeyGenerator) m_key = TheNameKeyGenerator->nameToKey(m_name);
  }
  return m_key;
}
