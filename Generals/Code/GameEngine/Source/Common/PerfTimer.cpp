...
//-------------------------------------------------------------------------------------------------
void GetPrecisionTimerTicksPerSec(Int64* t) { *t = s_ticksPerSec; }
...
void InitPrecisionTimer() {
  ...
  DEBUG_LOG(("TheTicksToGetTicks is %d (%f usec)\n", (int)TheTicksToGetTicks,
             TheTicksToGetTicks / s_ticksPerUSec));
#endif
}
...
//-------------------------------------------------------------------------------------------------
// PerfMetrics class. Basically, request a handle with your name and it will
// return. We use a vector of pairs of asciistrings for this
 ...
 public:
  AsciiString& getStatsString(const AsciiString& id) {
    for (int i = 0; i < m_outputStats.size(); ++i) {
      if (m_outputStats[i].first == id) return m_outputStats[i].second;
    }
...
PerfGather::PerfGather(const char* identifier)
  ...
  m_ignore = FALSE;
  //
  DEBUG_ASSERTCRASH(strchr(m_identifier, ',') == NULL,
                    ("PerfGather names must not contain commas"));
  addToList();
}

//-------------------------------------------------------------------------------------------------
PerfGather::~PerfGather() { removeFromList(); }
...
/*static*/ void PerfGather::dumpAll(UnsignedInt frame) {
  ...
  if (frame >= 1 && frame <= 30) {
    // always skip the first second or so, since it loads everything and skews
    // the results horribly
  } else {
    if (s_lastDumpedFrame == 0) {
      fprintf(s_perfStatsFile, "Frame");
      if (s_perfDumpOptions & PERF_GROSSTIME) {
        for (const PerfGather* head = getHeadPtr(); head != NULL;
             head = head->m_next) {
    ...
    // there's nothing magic about this value, it's purely determined from
    // sample dumps...
    ...
    if (s_perfDumpOptions & PERF_GROSSTIME) {
      for (const PerfGather* head = getHeadPtr(); head != NULL;
        ...
        t /= s_ticksPerUSec;
        if (t > CLIP_BIG_SPIKES) t = CLIP_BIG_SPIKES;
...
//-------------------------------------------------------------------------------------------------
// This function will queue up stuff to draw on the next frame. We also need to
// adjust the perf timers to not include time spent paused by the script engine.
...
//-------------------------------------------------------------------------------------------------
PerfTimer::PerfTimer(const char* identifier, Bool crashWithInfo, Int startFrame,
                     Int endFrame)
    : m_identifier(identifier),
  ...
  if (m_crashWithInfo) {
    DEBUG_CRASH(
        ("%s\n"
         ...
         1.0f * m_callCount / (m_lastFrame - m_startFrame + 1), m_callCount,
...
void PerfTimer::showMetrics(void) {
  ...
  outputStats.format("%s: %.2fms / call, %.2fms / frame \n", m_identifier,
                     avgTimePerCall, avgTimePerFrame);
  m_callCount = 0;
  m_runningTime = 0;
