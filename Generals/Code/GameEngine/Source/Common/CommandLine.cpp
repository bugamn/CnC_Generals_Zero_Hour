...
Int parsePreload(char *args[], int num) {
  if (TheWritableGlobalData) TheWritableGlobalData->m_preloadAssets = TRUE;
  return 1;
}
...
Int parseVTune(char *args[], int num) {
  if (TheWritableGlobalData) TheWritableGlobalData->m_vTune = TRUE;
  return 1;
}
/// end stuff for VTUNE
...
void parseCommandLine(int argc, char *argv[]) {
  ...
  // look at the next argument(s), to accomodate multi-arg parameters, e.g. "-p
  // 1234".
  int arg = 1, param;
  Bool found;
  ...
  while (arg < argc) {
    // Look at arg #i
    found = false;
    for (param = 0; !found && param < sizeof(params) / sizeof(params[0]);
         ++param) {
