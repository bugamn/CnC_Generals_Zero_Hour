...
#define PROFILE_ERROR_LIMIT \
  0.94f  // fraction of profiled result needed to get a match.  Allows some room
         // for error/fluctuation.

// Hack to get access to a static method on the W3DDevice side. -MW
extern Bool testMinimumRequirements(ChipsetType *videoChipType,
                                    CpuType *cpuType, Int *cpuFreq, Int *numRAM,
...
static const FieldParse TheStaticGameLODFieldParseTable[] = {
    ...
    {"MinimumProcessorFps", INI::parseInt, NULL,
     offsetof(StaticGameLODInfo, m_minProcessorFPS)},
    {"SampleCount2D", INI::parseInt, NULL,
     offsetof(StaticGameLODInfo, m_sampleCount2D)},
    {"SampleCount3D", INI::parseInt, NULL,
     offsetof(StaticGameLODInfo, m_sampleCount3D)},
    {"StreamCount", INI::parseInt, NULL,
     offsetof(StaticGameLODInfo, m_streamCount)},
    {"MaxParticleCount", INI::parseInt, NULL,
     offsetof(StaticGameLODInfo, m_maxParticleCount)},
    {"UseShadowVolumes", INI::parseBool, NULL,
     offsetof(StaticGameLODInfo, m_useShadowVolumes)},
    {"UseShadowDecals", INI::parseBool, NULL,
     offsetof(StaticGameLODInfo, m_useShadowDecals)},
    {"UseCloudMap", INI::parseBool, NULL,
     offsetof(StaticGameLODInfo, m_useCloudMap)},
    {"UseLightMap", INI::parseBool, NULL,
     offsetof(StaticGameLODInfo, m_useLightMap)},
    {"ShowSoftWaterEdge", INI::parseBool, NULL,
     offsetof(StaticGameLODInfo, m_showSoftWaterEdge)},
    {"MaxTankTrackEdges", INI::parseInt, NULL,
     offsetof(StaticGameLODInfo, m_maxTankTrackEdges)},
    {"MaxTankTrackOpaqueEdges", INI::parseInt, NULL,
     offsetof(StaticGameLODInfo, m_maxTankTrackOpaqueEdges)},
    {"MaxTankTrackFadeDelay", INI::parseInt, NULL,
     offsetof(StaticGameLODInfo, m_maxTankTrackFadeDelay)},
    {"UseBuildupScaffolds", INI::parseBool, NULL,
     offsetof(StaticGameLODInfo, m_useBuildupScaffolds)},
    {"UseTreeSway", INI::parseBool, NULL,
     offsetof(StaticGameLODInfo, m_useTreeSway)},
    {"UseEmissiveNightMaterials", INI::parseBool, NULL,
     offsetof(StaticGameLODInfo, m_useEmissiveNightMaterials)},
    {"TextureReductionFactor", INI::parseInt, NULL,
     offsetof(StaticGameLODInfo, m_textureReduction)},
};

static const char *StaticGameLODNames[] = {"Low", "Medium", "High", "Custom"};
...
static const FieldParse TheDynamicGameLODFieldParseTable[] = {
    ...
    {"ParticleSkipMask", INI::parseInt, NULL,
     offsetof(DynamicGameLODInfo, m_dynamicParticleSkipMask)},
    {"DebrisSkipMask", INI::parseInt, NULL,
     offsetof(DynamicGameLODInfo, m_dynamicDebrisSkipMask)},
    {"SlowDeathScale", INI::parseReal, NULL,
     offsetof(DynamicGameLODInfo, m_slowDeathScale)},
    {"MinParticlePriority", INI::parseIndexList, ParticlePriorityNames,
     offsetof(DynamicGameLODInfo, m_minDynamicParticlePriority)},
    {"MinParticleSkipPriority", INI::parseIndexList, ParticlePriorityNames,
     offsetof(DynamicGameLODInfo, m_minDynamicParticleSkipPriority)},
};

static const char *DynamicGameLODNames[] = {"Low", "Medium", "High",
                                            "VeryHigh"};
...
static const char *CPUNames[] = {"XX", "P3", "P4", "K7", NULL};

// Keep this in sync with enum in GameLOD.h
static const char *VideoNames[] = {"XX",   "V2",   "V3",   "V4",   "V5",  "TNT",
                                   "TNT2", "GF2",  "R100", "PS11", "GF3", "GF4",
                                   "PS14", "R200", "PS20", "R300", NULL};
...
void INI::parseLODPreset(INI *ini) {
  ...
  if (TheGameLODManager) {
    StaticGameLODLevel index =
        (StaticGameLODLevel)TheGameLODManager->getStaticGameLODIndex(name);
...
GameLODManager::GameLODManager(void) {
  ...
  for (Int i = 0; i < STATIC_GAME_LOD_CUSTOM; i++) m_numLevelPresets[i] = 0;
};
...
BenchProfile *GameLODManager::newBenchProfile(void) {
  ...
  DEBUG_CRASH(
      ("GameLODManager::newBenchProfile - Too many profiles defined\n"));
  return NULL;
}
...
LODPresetInfo *GameLODManager::newLODPreset(StaticGameLODLevel index) {
  ...
  DEBUG_CRASH(
      ("GameLODManager::newLODPreset - Too many presets defined for '%s'\n",
...
void GameLODManager::init(void) {
  ...
  ini.load(AsciiString("Data\\INI\\GameLODPresets.ini"), INI_LOAD_OVERWRITE,
           NULL);

  // Get Presets for custom LOD level by pulling them out of initial globaldata
  // (which should have all settings already applied).
  ...
  StaticGameLODLevel userSetDetail =
      (StaticGameLODLevel)optionPref.getStaticGameDetail();

  m_idealDetailLevel =
      (StaticGameLODLevel)optionPref.getIdealStaticGameDetail();

  // always get this data in case we need it later.
  testMinimumRequirements(NULL, &m_cpuType, &m_cpuFreq, &m_numRAM, NULL, NULL,
                          NULL);
  ...
  if (m_idealDetailLevel == STATIC_GAME_LOD_UNKNOWN ||
    ...
    if (m_cpuType == XX || TheGlobalData->m_forceBenchmark) {
      ...
      testMinimumRequirements(NULL, NULL, NULL, NULL, &m_intBenchIndex,
                              &m_floatBenchIndex, &m_memBenchIndex);

      if (TheGlobalData->m_forceBenchmark) {  // we want to see the numbers.  So
                                              // dump them to a logfile.
        FILE *fp = fopen("Benchmark.txt", "w");
        if (fp) {
          fprintf(fp, "BenchProfile = %s %d %f %f %f", CPUNames[m_cpuType],
                  m_cpuFreq, m_intBenchIndex, m_floatBenchIndex,
                  m_memBenchIndex);
          fclose(fp);
        }
      }
      ...
      m_compositeBenchIndex =
          m_intBenchIndex +
          m_floatBenchIndex;  ///@todo: Need to scale these based on our apps
                              /// usage of int/float/mem ops.
      ...
      for (Int k = 0; k < m_numBenchProfiles; k++) {
        ...
        if (m_intBenchIndex / prof->m_intBenchIndex >= PROFILE_ERROR_LIMIT &&
          ...
          for (Int i = STATIC_GAME_LOD_HIGH; i >= STATIC_GAME_LOD_LOW; i--) {
            ...
            if (currentLevel >= i)
              break;  // we already found a higher level than the remaining
                      // presets so no need to keep searching.
          }
        }
        prof++;
      }
  ...
  if (userSetDetail == STATIC_GAME_LOD_CUSTOM) {
    TheWritableGlobalData->m_textureReductionFactor =
        optionPref.getTextureReduction();
    TheWritableGlobalData->m_useShadowVolumes =
        optionPref.get3DShadowsEnabled();
    ...
    TheWritableGlobalData->m_showSoftWaterEdge =
        optionPref.getSmoothWaterEnabled();
    TheWritableGlobalData->m_useDrawModuleLOD =
        optionPref.getExtraAnimationsDisabled();
...
void GameLODManager::refreshCustomStaticLODLevel(void) {
  ...
  lodInfo->m_useTreeSway =
      lodInfo->m_useBuildupScaffolds;  // Borrow same setting.
...
Int GameLODManager::getStaticGameLODIndex(AsciiString name) {
  for (Int i = 0; i < STATIC_GAME_LOD_COUNT; ++i) {
    if (name.compareNoCase(StaticGameLODNames[i]) == 0) return i;
  }
...
void INI::parseStaticGameLODDefinition(INI *ini) {
  ...
  if (TheGameLODManager) {
    ...
    if (index != STATIC_GAME_LOD_UNKNOWN) {
      StaticGameLODInfo *lodInfo =
          &(TheGameLODManager->m_staticGameLODInfo[index]);
...
void INI::parseStaticGameLODLevel(INI *ini, void *, void *store, const void *) {
  ...
  DEBUG_CRASH(
      ("invalid GameLODLevel token %s -- expected LOW/MEDIUM/HIGH\n", tok));
  throw INI_INVALID_DATA;
}

const char *GameLODManager::getStaticGameLODLevelName(
...
StaticGameLODLevel GameLODManager::findStaticLODLevel(void) {
  ...
  if (m_idealDetailLevel == STATIC_GAME_LOD_UNKNOWN) {
    ...
    // get system configuration - only need vide chip type, got rest in
    // ::init().
    testMinimumRequirements(&m_videoChipType, NULL, NULL, NULL, NULL, NULL,
                            NULL);
    ...
    for (Int i = STATIC_GAME_LOD_HIGH; i >= STATIC_GAME_LOD_LOW; i--) {
      LODPresetInfo *preset =
          &m_lodPresets[i][0];  // pointer to first preset at this LOD level.
      for (Int j = 0; j < m_numLevelPresets[i]; j++) {
        if (m_cpuType == preset->m_cpuType &&
            ((Real)m_cpuFreq / (Real)preset->m_mhz >=
             PROFILE_ERROR_LIMIT) &&  // make sure we're within 5% or higher
      ...
      if (m_idealDetailLevel >= i)
        break;  // we already found a higher level than the remaining presets so
                // no need to keep searching.
    }
    // Save ideal detail level for future usage
    OptionPreferences optionPref;
    optionPref["IdealStaticGameLOD"] =
        getStaticGameLODLevelName(m_idealDetailLevel);
    if (getStaticLODLevel() ==
        STATIC_GAME_LOD_UNKNOWN)  // save for future usage.
      optionPref["StaticGameLOD"] =
          getStaticGameLODLevelName(m_idealDetailLevel);
    optionPref.write();
  }
...
Bool GameLODManager::setStaticLODLevel(StaticGameLODLevel level) {
  ...
  if (level == STATIC_GAME_LOD_UNKNOWN ||
      (level != STATIC_GAME_LOD_CUSTOM && m_currentStaticLOD == level))
    return FALSE;  // level is already applied.  Custom levels are always
                   // applied since random options could change.
...
void GameLODManager::applyStaticLODLevel(StaticGameLODLevel level) {
  ...
  // save previous info for this level since it may be overwritten by
  // refreshCustomStaticLODLevel().
  ...
  Bool requestedTrees =
      m_memPassed;  // only use trees if memory requirement passed.
  if (level == STATIC_GAME_LOD_CUSTOM) {
    requestedTextureReduction = lodInfo->m_textureReduction;
    requestedTrees = lodInfo->m_useTrees;
  } else if (level >=
             STATIC_GAME_LOD_LOW) {  // normal non-custom level gets texture
                                     // reduction based on recommendation
  ...
  if (TheGlobalData) {
    ...
    // Check if texture resolution changed.  No need to apply when current is
    // unknown because display will do it
    if (requestedTextureReduction != m_currentTextureReduction) {
      TheWritableGlobalData->m_textureReductionFactor =
          requestedTextureReduction;
      if (TheGameClient)
        TheGameClient->adjustLOD(
            0);  // apply the new setting stored in globaldata
    }
    ...
    if (m_currentStaticLOD == STATIC_GAME_LOD_UNKNOWN ||
      ...
      if (TheGameClient) {
        ...
        TheGameClient
            ->allocateShadows();  // allocate those shadows that are enabled.
      }
    }
    ...
    if (m_currentStaticLOD == STATIC_GAME_LOD_UNKNOWN ||
        lodInfo->m_showSoftWaterEdge != prevLodInfo->m_showSoftWaterEdge) {
      if (TheTerrainVisual) TheTerrainVisual->setShoreLineDetail();
    }

    TheWritableGlobalData->m_maxTankTrackEdges = lodInfo->m_maxTankTrackEdges;
    TheWritableGlobalData->m_maxTankTrackOpaqueEdges =
        lodInfo->m_maxTankTrackOpaqueEdges;
    TheWritableGlobalData->m_maxTankTrackFadeDelay =
        lodInfo->m_maxTankTrackFadeDelay;
  ...
  if (TheTerrainVisual) TheTerrainVisual->setTerrainTracksDetail();
}
...
void INI::parseDynamicGameLODDefinition(INI *ini) {
  ...
  if (TheGameLODManager) {
    ...
    if (index != DYNAMIC_GAME_LOD_UNKNOWN) {
      DynamicGameLODInfo *lodInfo =
          &(TheGameLODManager->m_dynamicGameLODInfo[index]);
...
/**Parse an LOD level*/
void INI::parseDynamicGameLODLevel(INI *ini, void *, void *store,
                                   const void *) {
  const char *tok = ini->getNextToken();
...
Int GameLODManager::getDynamicGameLODIndex(AsciiString name) {
  for (Int i = 0; i < DYNAMIC_GAME_LOD_COUNT; ++i) {
    if (name.compareNoCase(DynamicGameLODNames[i]) == 0) return i;
  }
...
const char *GameLODManager::getDynamicGameLODLevelName(
    DynamicGameLODLevel level) {
  return DynamicGameLODNames[level];
}

/**Given an average fps, return the optimal dynamic LOD level that matches this
 * fps.*/
DynamicGameLODLevel GameLODManager::findDynamicLODLevel(Real averageFPS) {
  Int ifps = (Int)(averageFPS);  // convert to integer.

  for (Int i = DYNAMIC_GAME_LOD_VERY_HIGH; i >= DYNAMIC_GAME_LOD_LOW;
       i--) {  // check which of the LOD levels matches our fps
    if (m_dynamicGameLODInfo[i].m_minFPS < ifps) return (DynamicGameLODLevel)i;
  }
  return DYNAMIC_GAME_LOD_LOW;  // none of the low levels were slow enough so
                                // pick the lowest.
}
...
void GameLODManager::applyDynamicLODLevel(DynamicGameLODLevel level) {
  ...
  m_dynamicParticleSkipMask =
      m_dynamicGameLODInfo[level].m_dynamicParticleSkipMask;
  ...
  m_minDynamicParticlePriority =
      m_dynamicGameLODInfo[level].m_minDynamicParticlePriority;
  m_minDynamicParticleSkipPriority =
      m_dynamicGameLODInfo[level].m_minDynamicParticleSkipPriority;
}
...
Bool GameLODManager::didMemPass(void) { return m_memPassed; }
