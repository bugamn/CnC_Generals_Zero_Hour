...
// Desc:	 Object and drawable modules and actions.  These are simply just
// class
//				 instances that we can assign to objects,
//drawables, and things to contain 				 data and code for specific events, or just to
//hold data
///////////////////////////////////////////////////////////////////////////////////////////////////
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
// this method should NEVER be overridden by user code, only via the
// MAKE_STANDARD_MODULE_xxx macros! it should also NEVER be called directly;
// it's only for use by ModuleFactory!
/*static*/ ModuleData* Module::friend_newModuleData(INI* ini) {
  ModuleData* data = MSGNEW("Module::friend_newModuleData")
      ModuleData;  // no need to memorypool these since we never allocate more
                   // than one of each
  if (ini)
    ini->initFromINI(data,
                     0);  // this is just so that an "end" token is required
  return data;
}
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void Module::crc(Xfer* xfer) {}  // end crc
...
/** load post process */
// ------------------------------------------------------------------------------------------------
void Module::loadPostProcess(void) {}  // end loadPostProcess
...
ObjectModule::ObjectModule(Thing* thing, const ModuleData* moduleData)
  ...
  DEBUG_ASSERTCRASH(m_object,
                    ("Thing passed to ObjectModule is not an Object!\n"));

}  // end ObjectModule

//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
ObjectModule::~ObjectModule(void) {}  // end ~ObjectModule
...
DrawableModule::DrawableModule(Thing* thing, const ModuleData* moduleData)
  ...
  DEBUG_ASSERTCRASH(m_drawable,
                    ("Thing passed to DrawableModule is not a Drawable!\n"));

}  // end ~DrawableModule

//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
DrawableModule::~DrawableModule(void) {}  // end ~DrawableModule
...
//-------------------------------------------------------------------------------------------------
void UpgradeMuxData::getUpgradeActivationMasks(Int64& activation,
                                               Int64& conflicting) const {
  // already computed.
  if (!m_activationUpgradeNames.empty() || !m_conflictingUpgradeNames.empty()) {
    ...
    for (it = m_activationUpgradeNames.begin();
      ...
      if (!theTemplate && !it->isEmpty() && !it->isNone()) {
        DEBUG_CRASH(("An upgrade module references %s, which is not an Upgrade",
                     it->str()));
        throw INI_INVALID_DATA;
      }
