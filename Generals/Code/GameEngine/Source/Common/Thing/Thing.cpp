...
//						are logic side representations
//while drawables are client 						side.  Common data will be held in the Thing
//defined here 						and systems that need to work with both of them will work with
//						"Things"
//
//-----------------------------------------------------------------------------
...
//=============================================================================
const ThingTemplate* Thing::getTemplate() const { return m_template; }
...
//=============================================================================
// the nice thing about this is that we don't have to recalc out cached terrain
// stuff.
void Thing::setPositionZ(Real z) {
  ...
  DEBUG_ASSERTCRASH(
      !(_isnan(getPosition()->x) || _isnan(getPosition()->y) ||
...
void Thing::setPosition(const Coord3D* pos) {
  ...
  if (!m_template->isKindOf(KINDOF_STICK_TO_TERRAIN_SLOPE)) {
    ...
    // DEBUG_ASSERTCRASH(!(_isnan(pos->x) || _isnan(pos->y) || _isnan(pos->z)),
    // ("Drawable/Object position NAN! '%s'\n", m_template->getName().str() ));
    ...
    m_cachedPos = *pos;
    m_cacheFlags &=
        ~(VALID_ALTITUDE_TERRAIN |
          VALID_ALTITUDE_SEALEVEL);  // but don't clear the dir flags.
...
void Thing::setOrientation(Real angle) {
  ...
  if (m_template->isKindOf(KINDOF_STICK_TO_TERRAIN_SLOPE)) {
    ...
    m_transform.Set(x.x, y.x, z.x, pos.x, x.y, y.y, z.y, pos.y, x.z, y.z, z.z,
                    pos.z);
  }

  // DEBUG_ASSERTCRASH(-PI <= angle && angle <= PI, ("Please pass only
  // normalized (-PI..PI) angles to setOrientation (%f).\n", angle));
...
//-------------------------------------------------------------------------------------------------
Bool Thing::isKindOf(KindOfType t) const { return getTemplate()->isKindOf(t); }
...
//=============================================================================
/** If we treat this as airborne, then they slide down slopes.  This checks
whether they are high enough that we should let them act like they're flying.
jba. */
//=============================================================================
Bool Thing::isSignificantlyAboveTerrain() const {
  // If it's high enough that it will take more than 3 frames to return to the
  // ground, then it's significantly airborne.  jba
