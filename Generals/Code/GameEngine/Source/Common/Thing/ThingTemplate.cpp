...
/*
        NOTE NOTE NOTE -- s_objectFieldParseTable and
   s_objectReskinFieldParseTable must be updated in tandem!

        "s_objectReskinFieldParseTable" is intended to be used for parsing the
   "ObjectReskin" keyword, and should only allow you to specify things that
   affect the appearance of a template.

        The idea is that some units are functionally the same, but different
   visually; rather than replicate all the settings, you can specify it as "this
   one is like that one, but looks different".
        ...
        So: if you add/remove/modify any settings that deal with visual
   appearance, you *may* want to add 'em to the reskin table... but do so VERY
   CAUTIOUSLY and with careful deliberation (and after checking around for other
   opinions).

*/

// NOTE NOTE NOTE -- s_objectFieldParseTable and s_objectReskinFieldParseTable
// must be updated in tandem -- see comment above
const FieldParse ThingTemplate::s_objectFieldParseTable[] = {
    {"DisplayName", INI::parseAndTranslateLabel, NULL,
     offsetof(ThingTemplate, m_displayName)},
    {"RadarPriority", INI::parseByteSizedIndexList, RadarPriorityNames,
     offsetof(ThingTemplate, m_radarPriority)},
    {"TransportSlotCount", INI::parseUnsignedByte, NULL,
     offsetof(ThingTemplate, m_transportSlotCount)},
    {"FenceWidth", INI::parseReal, NULL, offsetof(ThingTemplate, m_fenceWidth)},
    {"FenceXOffset", INI::parseReal, NULL,
     offsetof(ThingTemplate, m_fenceXOffset)},
    ...
    {"VisionRange", INI::parseReal, NULL,
     offsetof(ThingTemplate, m_visionRange)},
    {"ShroudClearingRange", INI::parseReal, NULL,
     offsetof(ThingTemplate, m_shroudClearingRange)},

    {"PlacementViewAngle", INI::parseAngleReal, NULL,
     offsetof(ThingTemplate, m_placementViewAngle)},

    {"FactoryExitWidth", INI::parseReal, NULL,
     offsetof(ThingTemplate, m_factoryExitWidth)},
    {"FactoryExtraBibWidth", INI::parseReal, NULL,
     offsetof(ThingTemplate, m_factoryExtraBibWidth)},

    {"SkillPointValue", ThingTemplate::parseIntList, (void*)LEVEL_COUNT,
     offsetof(ThingTemplate, m_skillPointValues)},
    {"ExperienceValue", ThingTemplate::parseIntList, (void*)LEVEL_COUNT,
     offsetof(ThingTemplate, m_experienceValues)},
    {"ExperienceRequired", ThingTemplate::parseIntList, (void*)LEVEL_COUNT,
     offsetof(ThingTemplate, m_experienceRequired)},
    {"IsTrainable", INI::parseBool, NULL,
     offsetof(ThingTemplate, m_isTrainable)},

    {"Side", INI::parseAsciiString, NULL,
     offsetof(ThingTemplate, m_defaultOwningSide)},

    // NOTE NOTE NOTE -- s_objectFieldParseTable and
    // s_objectReskinFieldParseTable must be updated in tandem -- see comment
    // above
    {"Prerequisites", ThingTemplate::parsePrerequisites, 0, 0},
    {"Buildable", INI::parseByteSizedIndexList, BuildableStatusNames,
     offsetof(ThingTemplate, m_buildable)},
    {"BuildCost", INI::parseUnsignedShort, NULL,
     offsetof(ThingTemplate, m_buildCost)},
    {"BuildTime", INI::parseReal, NULL, offsetof(ThingTemplate, m_buildTime)},
    {"RefundValue", INI::parseUnsignedShort, NULL,
     offsetof(ThingTemplate, m_refundValue)},
    {"BuildCompletion", INI::parseByteSizedIndexList, BuildCompletionNames,
     offsetof(ThingTemplate, m_buildCompletion)},
    {"EnergyProduction", INI::parseInt, NULL,
     offsetof(ThingTemplate, m_energyProduction)},
    {"EnergyBonus", INI::parseInt, NULL,
     offsetof(ThingTemplate, m_energyBonus)},
    {"IsForbidden", INI::parseBool, NULL,
     offsetof(ThingTemplate, m_isForbidden)},
    {"IsPrerequisite", INI::parseBool, NULL,
     offsetof(ThingTemplate, m_isPrerequisite)},
    {"DisplayColor", INI::parseColorInt, NULL,
     offsetof(ThingTemplate, m_displayColor)},
    {"EditorSorting", INI::parseByteSizedIndexList, EditorSortingNames,
     offsetof(ThingTemplate, m_editorSorting)},
    {"KindOf", KindOfMaskType::parseFromINI, NULL,
     offsetof(ThingTemplate, m_kindof)},
    {"CommandSet", INI::parseAsciiString, NULL,
     offsetof(ThingTemplate, m_commandSetString)},
    {"BuildVariations", INI::parseAsciiStringVector, NULL,
     offsetof(ThingTemplate, m_buildVariations)},

    // NOTE NOTE NOTE -- s_objectFieldParseTable and
    // s_objectReskinFieldParseTable must be updated in tandem -- see comment
    // above
    {"Behavior", ThingTemplate::parseModuleName,
     (const void*)MODULETYPE_BEHAVIOR,
     offsetof(ThingTemplate, m_behaviorModuleInfo)},
    {"Body", ThingTemplate::parseModuleName, (const void*)999,
     offsetof(ThingTemplate, m_behaviorModuleInfo)},
    {"Draw", ThingTemplate::parseModuleName, (const void*)MODULETYPE_DRAW,
     offsetof(ThingTemplate, m_drawModuleInfo)},
    {"ClientUpdate", ThingTemplate::parseModuleName,
    ...
    {"SelectPortrait", INI::parseAsciiString, NULL,
     offsetof(ThingTemplate, m_selectedPortraitImageName)},
    {"ButtonImage", INI::parseAsciiString, NULL,
     offsetof(ThingTemplate, m_buttonImageName)},

    // Code renderer handles these states now.
    //{ "InventoryImageEnabled",	INI::parseAsciiString,	NULL,
    //offsetof( ThingTemplate, m_inventoryImage[ INV_IMAGE_ENABLED ] ) }, {
    //"InventoryImageDisabled",	INI::parseAsciiString,	NULL, offsetof(
    //ThingTemplate, m_inventoryImage[ INV_IMAGE_DISABLED ] ) }, {
    //"InventoryImageHilite",		INI::parseAsciiString,	NULL,
    //offsetof( ThingTemplate, m_inventoryImage[ INV_IMAGE_HILITE ] ) }, {
    //"InventoryImagePushed",		INI::parseAsciiString,	NULL,
    //offsetof( ThingTemplate, m_inventoryImage[ INV_IMAGE_PUSHED ] ) },

    {"UpgradeCameo1", INI::parseAsciiString, NULL,
     offsetof(ThingTemplate, m_upgradeCameoUpgradeNames[0])},
    {"UpgradeCameo2", INI::parseAsciiString, NULL,
     offsetof(ThingTemplate, m_upgradeCameoUpgradeNames[1])},
    {"UpgradeCameo3", INI::parseAsciiString, NULL,
     offsetof(ThingTemplate, m_upgradeCameoUpgradeNames[2])},
    {"UpgradeCameo4", INI::parseAsciiString, NULL,
     offsetof(ThingTemplate, m_upgradeCameoUpgradeNames[3])},
    {"UpgradeCameo5", INI::parseAsciiString, NULL,
     offsetof(ThingTemplate, m_upgradeCameoUpgradeNames[4])},

    // NOTE NOTE NOTE -- s_objectFieldParseTable and
    // s_objectReskinFieldParseTable must be updated in tandem -- see comment
    // above

    {"VoiceSelect", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceSelect])},
    {"VoiceGroupSelect", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceGroupSelect])},
    {"VoiceMove", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceMove])},
    {"VoiceAttack", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceAttack])},
    {"VoiceEnter", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceEnter])},
    {"VoiceFear", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceFear])},
    {"VoiceSelectElite", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceSelectElite])},
    {"VoiceCreated", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceCreated])},
    {"VoiceTaskUnable", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceTaskUnable])},
    {"VoiceTaskComplete", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceTaskComplete])},
    {"VoiceMeetEnemy", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceMeetEnemy])},
    {"VoiceGarrison", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceGarrison])},
#ifdef ALLOW_SURRENDER
    {"VoiceSurrender", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceSurrender])},
#endif
    {"VoiceDefect", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceDefect])},
    {"VoiceAttackSpecial", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceAttackSpecial])},
    {"VoiceAttackAir", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceAttackAir])},
    {"VoiceGuard", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_voiceGuard])},
    {"SoundMoveStart", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundMoveStart])},
    {"SoundMoveStartDamaged", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate,
              m_audioarray.m_audio[TTAUDIO_soundMoveStartDamaged])},
    {"SoundMoveLoop", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundMoveLoop])},
    {"SoundMoveLoopDamaged", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate,
              m_audioarray.m_audio[TTAUDIO_soundMoveLoopDamaged])},
    {"SoundDie", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundDie])},
    {"SoundCrush", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundCrush])},
    {"SoundAmbient", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundAmbient])},
    {"SoundAmbientDamaged", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate,
              m_audioarray.m_audio[TTAUDIO_soundAmbientDamaged])},
    {"SoundAmbientReallyDamaged", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate,
              m_audioarray.m_audio[TTAUDIO_soundAmbientReallyDamaged])},
    {"SoundAmbientRubble", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundAmbientRubble])},
    {"SoundStealthOn", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundStealthOn])},
    {"SoundStealthOff", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundStealthOff])},
    {"SoundCreated", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundCreated])},
    {"SoundOnDamaged", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundOnDamaged])},
    {"SoundOnReallyDamaged", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate,
              m_audioarray.m_audio[TTAUDIO_soundOnReallyDamaged])},
    {"SoundDieFire", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundDieFire])},
    {"SoundDieToxin", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundDieToxin])},
    {"SoundEnter", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundEnter])},
    {"SoundExit", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundExit])},
    {"SoundPromotedVeteran", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate,
              m_audioarray.m_audio[TTAUDIO_soundPromotedVeteran])},
    {"SoundPromotedElite", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundPromotedElite])},
    {"SoundPromotedHero", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundPromotedHero])},
    {"SoundFallingFromPlane", INI::parseDynamicAudioEventRTS, NULL,
     offsetof(ThingTemplate, m_audioarray.m_audio[TTAUDIO_soundFalling])},

    {"UnitSpecificSounds", ThingTemplate::parsePerUnitSounds, NULL,
     offsetof(ThingTemplate, m_perUnitSounds)},
    {"UnitSpecificFX", ThingTemplate::parsePerUnitFX, NULL,
     offsetof(ThingTemplate, m_perUnitFX)},
    {"Scale", INI::parseReal, NULL, offsetof(ThingTemplate, m_assetScale)},
    {"Geometry", GeometryInfo::parseGeometryType, NULL,
     offsetof(ThingTemplate, m_geometryInfo)},
    {"GeometryMajorRadius", GeometryInfo::parseGeometryMajorRadius, NULL,
     offsetof(ThingTemplate, m_geometryInfo)},
    {"GeometryMinorRadius", GeometryInfo::parseGeometryMinorRadius, NULL,
     offsetof(ThingTemplate, m_geometryInfo)},
    {"GeometryHeight", GeometryInfo::parseGeometryHeight, NULL,
     offsetof(ThingTemplate, m_geometryInfo)},
    {"GeometryIsSmall", GeometryInfo::parseGeometryIsSmall, NULL,
     offsetof(ThingTemplate, m_geometryInfo)},
    {"Shadow", INI::parseBitString8, TheShadowNames,
     offsetof(ThingTemplate, m_shadowType)},
    {"ShadowSizeX", INI::parseReal, NULL,
     offsetof(ThingTemplate, m_shadowSizeX)},
    {"ShadowSizeY", INI::parseReal, NULL,
     offsetof(ThingTemplate, m_shadowSizeY)},
    {"ShadowOffsetX", INI::parseReal, NULL,
     offsetof(ThingTemplate, m_shadowOffsetX)},
    {"ShadowOffsetY", INI::parseReal, NULL,
     offsetof(ThingTemplate, m_shadowOffsetY)},
    {"ShadowTexture", INI::parseAsciiString, NULL,
     offsetof(ThingTemplate, m_shadowTextureName)},
    {"OcclusionDelay", INI::parseDurationUnsignedInt, NULL,
     offsetof(ThingTemplate, m_occlusionDelay)},
    ...
    {"InstanceScaleFuzziness", INI::parseReal, NULL,
     offsetof(ThingTemplate, m_instanceScaleFuzziness)},
    {"StructureRubbleHeight", INI::parseUnsignedByte, NULL,
     offsetof(ThingTemplate, m_structureRubbleHeight)},
    {"ThreatValue", INI::parseUnsignedShort, NULL,
     offsetof(ThingTemplate, m_threatValue)},
    {"MaxSimultaneousOfType", INI::parseUnsignedShort, NULL,
     offsetof(ThingTemplate, m_maxSimultaneousOfType)},
    {"CrusherLevel", INI::parseUnsignedByte, NULL,
     offsetof(ThingTemplate, m_crusherLevel)},
    {"CrushableLevel", INI::parseUnsignedByte, NULL,
     offsetof(ThingTemplate, m_crushableLevel)},
...
// ------------------------------------------------------------------------------------------------
const ModuleInfo::Nugget* ModuleInfo::getNuggetWithTag(
    const AsciiString& tag) const {
  std::vector<Nugget>::const_iterator it;
  for (it = m_info.begin(); it != m_info.end(); ++it)
    if ((*it).m_moduleTag == tag) return &(*it);
...
void ModuleInfo::addModuleInfo(ThingTemplate* thingTemplate,
                               ...
                               const ModuleData* data, Int interfaceMask,
                               Bool inheritable) {
  //
  // there must be a module tag present, and it must be unique across all module
  // infos for this thing template
  //
  ...
  if (nugget != NULL) {
    // compare this nugget tag against the tag for the new data we're going to
    // submit
    DEBUG_ASSERTCRASH(
        nugget->m_moduleTag != moduleTag,
        ("addModuleInfo - ERROR defining module '%s' on thing template '%s'.  "
         "The module '%s' has the tag '%s' which must be unique among all "
         "modules for this object, but the tag '%s' is also already on module "
         "'%s' within this object.\n\nPlease make unique tag names within an "
         "object definition\n",
         name.str(), thingTemplate->getName().str(), name.str(),
  ...
  nugget =
      thingTemplate->getClientUpdateModuleInfo().getNuggetWithTag(moduleTag);
  if (nugget != NULL) {
    // compare this nugget tag against the tag for the new data we're going to
    // submit
    DEBUG_ASSERTCRASH(
...
Bool ModuleInfo::clearModuleDataWithTag(const AsciiString& tagToClear,
  ...
  for (std::vector<Nugget>::iterator it = m_info.begin(); it != m_info.end();
       /* empty */) {
    if (it->m_moduleTag == tagToClear) {
      DEBUG_ASSERTCRASH(
          !cleared,
          ("Hmm, multiple clears in ModuleInfo::clearModuleDataWithTag, should "
           "this be possible?"));
...
//-------------------------------------------------------------------------------------------------
void ThingTemplate::parseModuleName(INI* ini, void* instance, void* store,
                                    const void* userData) {
  ...
  try {
    ...
    DEBUG_CRASH(
        ("[LINE: %d - FILE: '%s'] Module tag not found for module '%s' on "
         "thing template '%s'.  Module tags are required and must be unique "
         "for all modules within an object definition\n",
         ini->getLineNum(), ini->getFilename().str(), tokenStr.str(),
         self->getName().str()));
    throw;
  }
  ...
  if (ini->getLoadType() == INI_LOAD_CREATE_OVERRIDES) {
    if (self->m_moduleParsingMode == MODULEPARSE_ADD_REMOVE_REPLACE) {
      ...
      DEBUG_CRASH(
          ("[LINE: %d - FILE: '%s'] You must use AddModule to add modules in "
           "override INI files.\n",
  ...
  if (self->m_moduleParsingMode == MODULEPARSE_ADD_REMOVE_REPLACE &&
    ...
    DEBUG_CRASH(
        ("[LINE: %d - FILE: '%s'] ReplaceModule must replace modules with "
         "another module of the same type, but you are attempting to replace a "
         "%s with a %s for Object %s.\n",
  ...
  if (self->m_moduleParsingMode == MODULEPARSE_ADD_REMOVE_REPLACE &&
    ...
    DEBUG_CRASH(
        ("[LINE: %d - FILE: '%s'] ReplaceModule must specify a new, unique tag "
         "for the replaced module, but you are not doing so for %s (%s) for "
         "Object %s.\n",
         ini->getLineNum(), ini->getFilename().str(), moduleTagStr.str(),
         self->m_moduleBeingReplacedName.str(), self->getName().str()));
    throw INI_INVALID_DATA;
  }
  ...
  mi->addModuleInfo(self, tokenStr, moduleTagStr, data, interfaceMask,
                    inheritable);
}

//-------------------------------------------------------------------------------------------------
void ThingTemplate::parseIntList(INI* ini, void* instance, void* store,
                                 const void* userData) {
...
//-------------------------------------------------------------------------------------------------
static void parsePrerequisiteUnit(INI* ini, void* instance, void* /*store*/,
                                  const void* /*userData*/) {
  std::vector<ProductionPrerequisite>* v =
      (std::vector<ProductionPrerequisite>*)instance;

  ProductionPrerequisite prereq;
  Bool orUnitWithPrevious = FALSE;
  for (const char* token = ini->getNextToken(); token != NULL;
...
//-------------------------------------------------------------------------------------------------
static void parsePrerequisiteScience(INI* ini, void* instance, void* /*store*/,
                                     const void* /*userData*/) {
  std::vector<ProductionPrerequisite>* v =
      (std::vector<ProductionPrerequisite>*)instance;
...
//-------------------------------------------------------------------------------------------------
void ThingTemplate::parsePrerequisites(INI* ini, void* instance, void* store,
                                       const void* userData) {
  ThingTemplate* self = (ThingTemplate*)instance;
...
//-------------------------------------------------------------------------------------------Static
static void parseArbitraryFXIntoMap(INI* ini, void* instance, void* /* store */,
                                    const void* userData) {
  PerUnitFXMap* mapFX = (PerUnitFXMap*)instance;
  const char* name = (const char*)userData;
  const char* token = ini->getNextToken();
  const FXList* fxl = TheFXListStore->findFXList(token);  // could be null!
  DEBUG_ASSERTCRASH(fxl != NULL || stricmp(token, "None") == 0,
                    ("FXList %s not found!\n", token));
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void ThingTemplate::parsePerUnitFX(INI* ini, void* instance, void* store,
                                   const void* userData) {
  ...
  static const FieldParse myFieldParse[] = {
      {0, parseArbitraryFXIntoMap, NULL, 0}, {0, 0, 0, 0}};

  ini->initFromINI(fxmap, myFieldParse);
}

//-------------------------------------------------------------------------------------------Static
static void parseArbitrarySoundsIntoMap(INI* ini, void* instance,
                                        void* /* store */,
                                        const void* userData) {
  ...
  AudioEventRTS a;
  if (token) a.setEventName(token);
  mapSounds->insert(std::make_pair(AsciiString(name), a));
}

//-------------------------------------------------------------------------------------------------
/** Parse Additional per unit sounds such as TankTurretMove and
 * TankTurretMoveLoop. */
//-------------------------------------------------------------------------------------------------
void ThingTemplate::parsePerUnitSounds(INI* ini, void* instance, void* store,
                                       const void* userData) {
  PerUnitSoundMap* mapSounds = (PerUnitSoundMap*)store;
  mapSounds->clear();

  static const FieldParse myFieldParse[] = {
      {0, parseArbitrarySoundsIntoMap, NULL, 0}, {0, 0, 0, 0}};
...
//-------------------------------------------------------------------------------------------------
void ThingTemplate::parseAddModule(INI* ini, void* instance, void* store,
                                   const void* userData) {
  // don't care about the result.
  ThingTemplate* self = (ThingTemplate*)instance;

  ModuleParseMode oldMode = (ModuleParseMode)self->m_moduleParsingMode;
  if (oldMode != MODULEPARSE_NORMAL) throw INI_INVALID_DATA;
...
//-------------------------------------------------------------------------------------------------
void ThingTemplate::parseRemoveModule(INI* ini, void* instance, void* store,
                                      const void* userData) {
  ThingTemplate* self = (ThingTemplate*)instance;
...
//-------------------------------------------------------------------------------------------------
void ThingTemplate::parseReplaceModule(INI* ini, void* instance, void* store,
                                       const void* userData) {
  ThingTemplate* self = (ThingTemplate*)instance;
  ...
  if (!removed) {
    DEBUG_CRASH(
        ("[LINE: %d - FILE: '%s'] ReplaceModule %s was not found for %s; "
         "cannot continue.\n",
         ini->getLineNum(), ini->getFilename().str(), modToRemove,
         self->getName().str()));
    throw INI_INVALID_DATA;
  }
...
//-------------------------------------------------------------------------------------------------
void ThingTemplate::parseInheritableModule(INI* ini, void* instance,
...
Bool ThingTemplate::removeModuleInfo(const AsciiString& moduleToRemove,
  ...
  if (m_behaviorModuleInfo.clearModuleDataWithTag(moduleToRemove,
                                                  clearedModuleNameOut)) {
    DEBUG_ASSERTCRASH(
        !removed, ("Hmm, multiple removed in ThingTemplate::removeModuleInfo, "
                   "should this be possible?"));
    removed = true;
  }
  if (m_drawModuleInfo.clearModuleDataWithTag(moduleToRemove,
                                              clearedModuleNameOut)) {
    DEBUG_ASSERTCRASH(
        !removed, ("Hmm, multiple removed in ThingTemplate::removeModuleInfo, "
                   "should this be possible?"));
  ...
  if (m_clientUpdateModuleInfo.clearModuleDataWithTag(moduleToRemove,
                                                      clearedModuleNameOut)) {
    DEBUG_ASSERTCRASH(
        !removed, ("Hmm, multiple removed in ThingTemplate::removeModuleInfo, "
                   "should this be possible?"));
...
void ArmorTemplateSet::parseArmorTemplateSet(INI* ini) {
  static const FieldParse myFieldParse[] = {
      {"Conditions", ArmorSetFlags::parseFromINI, NULL,
       offsetof(ArmorTemplateSet, m_types)},
      {"Armor", INI::parseArmorTemplate, NULL,
       offsetof(ArmorTemplateSet, m_template)},
...
//-------------------------------------------------------------------------------------------------
void ThingTemplate::parseArmorTemplateSet(INI* ini, void* instance,
                                          void* /*store*/,
                                          const void* /*userData*/) {
  ThingTemplate* self = (ThingTemplate*)instance;
...
#if defined(_DEBUG) || defined(_INTERNAL)
  if (ini->getLoadType() != INI_LOAD_CREATE_OVERRIDES) {
    for (ArmorTemplateSetVector::const_iterator it =
             self->m_armorTemplateSets.begin();
         it != self->m_armorTemplateSets.end(); ++it) {
...
//-------------------------------------------------------------------------------------------------
void ThingTemplate::parseWeaponTemplateSet(INI* ini, void* instance,
                                           void* /*store*/,
                                           const void* /*userData*/) {
  ThingTemplate* self = (ThingTemplate*)instance;
...
#if defined(_DEBUG) || defined(_INTERNAL)
  if (ini->getLoadType() != INI_LOAD_CREATE_OVERRIDES) {
    for (WeaponTemplateSetVector::const_iterator it =
             self->m_weaponTemplateSets.begin();
         it != self->m_weaponTemplateSets.end(); ++it) {
      if (it->getNthConditionsYes(0) == ws.getNthConditionsYes(0)) {
...
ThingTemplate::ThingTemplate()
  ...
  m_crushableLevel =
      255;  // Unspecified, this object is unable to be crushed by anything!
}
...
AIUpdateModuleData* ThingTemplate::friend_getAIModuleInfo(void) {
  ...
  for (int j = 0; j < numModInfos; ++j) {
    if (m_behaviorModuleInfo.getNthData(j) &&
        m_behaviorModuleInfo.getNthData(j)->isAiModuleData()) {
...
#define AUDIO_TEST(y)                                                   \
  if (!get##y()->getEventName().isEmpty() &&                            \
      get##y()->getEventName().compareNoCase("NoSound") != 0) {         \
    DEBUG_ASSERTLOG(TheAudio->isValidAudioEvent(get##y()),              \
                    ("Invalid Sound '%s' in Object '%s'. (%s?)\n", #y,  \
                     getName().str(), get##y()->getEventName().str())); \
  }
...
#undef AUDIO_TEST
  ...
  for (PerUnitSoundMap::const_iterator it = perUnitSounds->begin();
       it != perUnitSounds->end(); ++it) {
    if (!it->second.getEventName().isEmpty() &&
        it->second.getEventName().compareNoCase("NoSound") != 0) {
      DEBUG_ASSERTCRASH(
          TheAudio->isValidAudioEvent(&it->second),
          ("Invalid UnitSpecificSound '%s' in Object '%s'. (%s?)",
           it->first.str(), getName().str(), it->second.getEventName().str()));
    }
  }
#endif
}
...
void ThingTemplate::validate() {
  ...
  if (getName() == "DefaultThingTemplate") return;

  // neutron missile is an old special case....
  if (getName() == "NeutronMissile") return;

  // another cheesy special case....
  if (getName() == "FlamethrowerProjectileStream") return;

  // drawable-only templates are exempt from further checks.
  if (isKindOf(KINDOF_DRAWABLE_ONLY)) return;

  // build-variation templates are exempt from further checks.
  if (!m_buildVariations.empty()) return;
  ...
  if (isKindOf(KINDOF_STRUCTURE) && !isImmobile) {
    DEBUG_CRASH(
        ("Structure %s is not marked immobile, but probably should be -- "
         "please fix it. (If we ever add mobile structures, this debug sniffer "
  ...
  if (isKindOf(KINDOF_STICK_TO_TERRAIN_SLOPE) && !isImmobile) {
    DEBUG_CRASH(
        ("item %s is marked STICK_TO_TERRAIN_SLOPE but not IMMOBILE -- please "
         "fix it.\n",
         getName().str()));
  }
  ...
  if (isKindOf(KINDOF_STRUCTURE)) {
    if (m_armorTemplateSets.empty() ||
        (m_armorTemplateSets.size() == 1 &&
         m_armorTemplateSets[0].getArmorTemplate() == NULL)) {
      DEBUG_CRASH(
          ("Structure %s has no armor, but probably should (StructureArmor) -- "
           "please fix it.)\n",
           getName().str()));
    }
    for (ArmorTemplateSetVector::const_iterator it =
             m_armorTemplateSets.begin();
         it != m_armorTemplateSets.end(); ++it) {
...
void ThingTemplate::resolveNames() {
  ...
  for (i = 0; i < m_prereqInfo.size(); i++) {
    Int count =
        m_prereqInfo[i].getAllPossibleBuildFacilityTemplates(tmpls, MAX_BF);
    for (j = 0; j < count; j++) {
      ...
      // DEBUG_LOG(("BF: %s is a buildfacility for
  ...
  if (TheMappedImageCollection) {
    ...
    if (m_buttonImageName.isNotEmpty()) {
      ...
      DEBUG_ASSERTCRASH(
          m_buttonImage,
          ("%s is looking for ButtonImage %s but can't find it. Skipping...",
           getName().str(), m_buttonImageName.str()));
...
void ThingTemplate::initForLTA(const AsciiString& name) {
  ...
  m_behaviorModuleInfo.addModuleInfo(
      this, "DestroyDie", moduleTag,
      TheModuleFactory->newModuleDataFromINI(NULL, "DestroyDie",
                                             MODULETYPE_BEHAVIOR, moduleTag),
      (MODULEINTERFACE_DIE), false);

  moduleTag.format("LTA_%sInactiveBody", m_LTAName.str());
  m_behaviorModuleInfo.addModuleInfo(
      this, "InactiveBody", moduleTag,
      TheModuleFactory->newModuleDataFromINI(NULL, "InactiveBody",
                                             MODULETYPE_BEHAVIOR, moduleTag),
      (MODULEINTERFACE_BODY), false);

  moduleTag.format("LTA_%sW3DDefaultDraw", m_LTAName.str());
  m_drawModuleInfo.addModuleInfo(
      this, "W3DDefaultDraw", moduleTag,
      TheModuleFactory->newModuleDataFromINI(NULL, "W3DDefaultDraw",
                                             MODULETYPE_DRAW, moduleTag),
  ...
  m_kindof = KINDOFMASK_NONE;
  m_assetScale = 1.0f;
  m_instanceScaleFuzziness =
      0.0f;  ///< tolerance to randomly vary scale per instance
...
//=============================================================================
const ArmorTemplateSet* ThingTemplate::findArmorTemplateSet(
    const ArmorSetFlags& t) const {
  return m_armorTemplateSetFinder.findBestInfo(m_armorTemplateSets, t);
}

//=============================================================================
const WeaponTemplateSet* ThingTemplate::findWeaponTemplateSet(
    const WeaponSetFlags& t) const {
...
Bool ThingTemplate::canPossiblyHaveAnyWeapon() const {
  for (WeaponTemplateSetVector::const_iterator it =
           m_weaponTemplateSets.begin();
       it != m_weaponTemplateSets.end(); ++it) {
    if (it->hasAnyWeapons()) return true;
  }
  return false;
}
...
Int ThingTemplate::getSkillPointValue(Int level) const {
  ...
  if (value == USE_EXP_VALUE_FOR_SKILL_VALUE) value = getExperienceValue(level);

  return value;
}

//-----------------------------------------------------------------------------
const ThingTemplate* ThingTemplate::getBuildFacilityTemplate(
    const Player* player) const {
  if (getPrereqCount() > 0) {
    return m_prereqInfo[0].getExistingBuildFacilityTemplate(
        player);  // might return null
  } else {
    return NULL;
  }
}
...
const FXList* ThingTemplate::getPerUnitFX(const AsciiString& fxName) const {
  ...
  if (it == m_perUnitFX.end()) {
    DEBUG_CRASH(("Unknown FX name (%s) asked for in ThingTemplate (%s). ",
                 fxName.str(), m_nameString.str()));
    return NULL;
  }
...
//-------------------------------------------------------------------------------------------------
const AudioEventRTS* ThingTemplate::getPerUnitSound(
    const AsciiString& soundName) const {
...
Bool ThingTemplate::isEquivalentTo(const ThingTemplate* tt) const {
  ...
  if (this->getFinalOverride() == tt->getFinalOverride()) return true;

  // This reskinned from that?
  if (this->m_reskinnedFrom == tt) return true;

  // That reskinned from this?
  if (this == tt->m_reskinnedFrom) return true;

  // This reskinned from that reskinned from?
  // Kris: added case (chassis 2 compared to chassis 3 -- NULL possible if not
  // reskinned)
  ...
  for (i = 0; i < numVariations; ++i)
    if (m_buildVariations[i].compareNoCase(tt->getName()) == 0) return true;

  numVariations = tt->m_buildVariations.size();
  for (i = 0; i < numVariations; ++i)
    if (tt->m_buildVariations[i].compareNoCase(getName()) == 0) return true;
...
//-------------------------------------------------------------------------------------------------
/** NOTE that we're not paying attention to m_override here, instead the
 * portions that retrieve template data values use the get() wrappers, which
 * *DO* pay attention to the override values */
//-------------------------------------------------------------------------------------------------
Int ThingTemplate::calcCostToBuild(const Player* player) const {
  ...
  return getBuildCost() * factionModifier *
         player->getHandicap()->getHandicap(Handicap::BUILDCOST, this);
}
...
Int ThingTemplate::calcTimeToBuild(const Player* player) const {
  ...
  Real EnergyPercent =
      player->getEnergy()->getEnergySupplyRatio();  // I'm at 80% Energy
  if (EnergyPercent > 1.0f)
    EnergyPercent = 1.0f;  // getEnergySupplyRatio() returns a true ratio, but
                           // we don't care about excess.
  Real EnergyShort = 1.0f - EnergyPercent;  // so I am 20% short
  EnergyShort *=
      TheGlobalData->m_LowEnergyPenaltyModifier;  // which is a 40% penalty, or
                                                  // a 10% penalty
  Real penaltyRate = 1.0f - EnergyShort;
  penaltyRate =
      max(penaltyRate,
          TheGlobalData->m_MinLowEnergyProductionSpeed);  // bind so 0% does not
                                                          // dead stop you

  if (EnergyPercent < 1.0f)  // and make 99% look like 80% (eg) since most of
                             // the time you are down only a little
    penaltyRate =
  ...
  buildTime /= penaltyRate;  // and voila.  Makes sense, designers have control,
                             // all is happy.
  ...
  if (getBuildCompletion() == BC_APPEARS_AT_RALLY_POINT) {
    const ThingTemplate* tmpl =
        getBuildFacilityTemplate(player);  // could be null if none exist
    Int count = 0;
    if (tmpl) {
      player->countObjectsByThingTemplate(1, &tmpl, false, &count);
      Real factoryMult = TheGlobalData->m_MultipleFactory;
      if (factoryMult > 0.0f) {
        for (int i = 0; i < count - 1; i++) buildTime *= factoryMult;
      }
    }
  }
