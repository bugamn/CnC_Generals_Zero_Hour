...
const char *lastReplayFileName =
    "00000000";  // a name the user is unlikely to ever type, but won't cause
                 // panic & confusion
...
void RecorderClass::logGameStart(AsciiString options) {
  ...
  if (TheNetwork && TheGlobalData->m_saveStats) {
    // if (TheLAN)
    {
      ...
      if (!logFP) {
        // try again locally
        TheWritableGlobalData->m_baseStatsDir =
...
void RecorderClass::updatePlayback() {
  cullBadCommands();  // Remove any bad commands that have been inserted by the
                      // local user that shouldn't be executed during playback.
  ...
  if (m_doingAnalysis) curFrame = m_nextFrame;
...
/**
 * Stop the currently running playback. This is probably due either to the user
 * exiting out of the playback or reaching the end of the playback file.
 */
...
/**
 * Update function for recording a game. Basically all the pertinant logic
 * commands for this frame are written out to a file.
 */
void RecorderClass::updateRecord() {
  ...
  while (msg != NULL) {
    if (msg->getType() == GameMessage::MSG_NEW_GAME &&
      ...
      Int maxFPS = 0;
      if (msg->getArgumentCount() >= 4) maxFPS = msg->getArgument(3)->integer;
...
/**
 * Start a new file for recording. This will always overwrite the
 * "LastReplay.rep" file with the new one. So don't call this unless you really
 * mean it.
 */
void RecorderClass::startRecording(GameDifficulty diff, Int originalGameMode,
                                   Int rankPoints, Int maxFPS) {
  DEBUG_ASSERTCRASH(m_file == NULL,
                    ("Starting to record game while game is in progress."));
  ...
  fwrite(&frames, sizeof(UnsignedInt), 1,
         m_file);  // reserve space for duration in frames

  Bool b = FALSE;
  fwrite(&b, sizeof(Bool), 1,
         m_file);  // reserve space for flag (true if we desync)
  fwrite(&b, sizeof(Bool), 1,
         m_file);  // reserve space for flag (true if we quit early)
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    fwrite(&b, sizeof(Bool), 1,
           m_file);  // reserve space for flag (true if player i disconnects)
  }
  ...
  */
  ...
  if (TheNetwork) {
    if (TheLAN) {
      ...
      DEBUG_ASSERTCRASH(game,
                        ("Starting a LAN game with no LANGameInfo object!"));
  ...
  /*
  /// @todo fix this to use starting spots and player alliances when those are
  put in the game. for (Int i = 0; i < numPlayers; ++i) { Player *player =
  ThePlayerList->getNthPlayer(i); if (player == NULL) { continue;
          }
...
/**
 * This will stop the current recording session and close the file. This should
 * always be called at the end of every game.
 */
void RecorderClass::stopRecording() {
  ...
  if (TheNetwork) {
    // if (TheLAN)
    {
      if (m_wasDesync) cleanUpReplayFile();
      m_wasDesync = FALSE;
    }
  }
  if (m_file != NULL) {
...
/**
 * Write this game message to the record file. This also writes the game
 * message's execution frame.
 */
void RecorderClass::writeToFile(GameMessage *msg) {
  ...
  // DEBUG_LOG(("RecorderClass::writeToFile - Adding %s command from player %d
  // to TheCommandList on frame %d\n", commandName.str(), msg->getPlayerIndex(),
...
/**
 * Read in a replay header, for (1) populating a replay listbox or (2) starting
 * playback.  In case (2), set FILE *m_file.
 */
Bool RecorderClass::readReplayHeader(ReplayHeader &header) {
  ...
  if (strncmp(genrep, "GENREP", 6)) {
    DEBUG_LOG(
        ("RecorderClass::readReplayHeader - replay file did not have GENREP at "
         "the start.\n"));
    fclose(m_file);
    m_file = NULL;
    return FALSE;
  ...
  // Read the date and time.  We don't really do anything with this either. Oh
  // well.
  ...
  if (header.localPlayerIndex < -1 || header.localPlayerIndex >= MAX_SLOTS) {
    DEBUG_LOG(
        ("RecorderClass::readReplayHeader - invalid local slot number.\n"));
    m_gameInfo.endGame();
    m_gameInfo.reset();
    fclose(m_file);
    m_file = NULL;
...
void CRCInfo::addCRC(UnsignedInt val) {
  ...
  // DEBUG_LOG(("CRCInfo::addCRC() - crc %8.8X pushes list to %d entries
...
UnsignedInt CRCInfo::readCRC(void) {
  ...
  // !m_data.empty(), m_data.size()));
  return val;
}

void RecorderClass::handleCRCMessage(UnsignedInt newCRC, Int playerIndex,
                                     Bool fromPlayback) {
  if (fromPlayback) {
    // DEBUG_LOG(("RecorderClass::handleCRCMessage() - Adding CRC of %X from %d
    // to m_crcInfo\n", newCRC, playerIndex));
  ...
  if (!p || (p->getPlayerNameKey() == NAMEKEY(playerName))) samePlayer = TRUE;
  if (samePlayer || (localPlayerIndex < 0)) {
    UnsignedInt playbackCRC = m_crcInfo->readCRC();
    // DEBUG_LOG(("RecorderClass::handleCRCMessage() - Comparing CRCs of
    // %8.8X/%8.8X from %d\n", newCRC, playbackCRC, playerIndex));
    if (TheGameLogic->getFrame() > 0 && newCRC != playbackCRC &&
        !m_crcInfo->sawCRCMismatch()) {
      m_crcInfo->setSawCRCMismatch();

      // Since we don't seem to have any *visible* desyncs when replaying games,
      // but get this warning virtually every replay, the assumption is our CRC
      // checking is faulty.  Since we're at the tail end of patch season, let's
      // just disable the message, and hope the users believe the problem is
      // fixed. -MDC 3/20/2003
      // TheInGameUI->message("GUI:CRCMismatch");
      DEBUG_CRASH(
          ("Replay has gone out of sync!  All bets are off!\nOld:%8.8X "
           "New:%8.8X\nFrame:%d",
  ...
  // DEBUG_LOG(("RecorderClass::handleCRCMessage() - Skipping CRC of %8.8X from
  // %d (our index is %d)\n", newCRC, playerIndex, localPlayerIndex));
}

/**
 * Return true if this version of the file is the same as our version of the
 * game
 */
Bool RecorderClass::testVersionPlayback(AsciiString filename) {
  ...
  Bool versionStringDiff =
      header.versionString != TheVersion->getUnicodeVersion();
  Bool versionTimeStringDiff =
      header.versionTimeString != TheVersion->getUnicodeBuildTime();
  Bool versionNumberDiff =
...
Bool RecorderClass::playbackFile(AsciiString filename) {
  ...
  if (exeDifferent) {
    ...
    if (versionStringDiff) {
      tempStr.format("   Version [%ls] vs [%ls]\n",
                     TheVersion->getUnicodeVersion().str(),
  ...
  if (header.localPlayerIndex >= 0) {
    DEBUG_LOG(("Local player is %ls (slot %d, IP %8.8X)\n",
               m_gameInfo.getSlot(header.localPlayerIndex)->getName().str(),
               header.localPlayerIndex,
               m_gameInfo.getSlot(header.localPlayerIndex)->getIP()));
  }
#endif
  ...
  if (!m_doingAnalysis) {
    ...
    if (maxFPS != 0) msg->appendIntegerArgument(maxFPS);
...
/**
 * Read a unicode string from the current file position. The string is assumed
 * to be 0-terminated.
 */
...
/**
 * Read an ascii string from the current file position. The string is assumed to
 * be 0-terminated.
 */
...
/**
 * Read the frame number for the next command in the playback file. If the end
 * of the file is reached, the playback is stopped and the next frame is said to
 * be -1.
 */
...
/**
 * This reads the next command from the replay file and appends it to
 * TheCommandList.
 */
void RecorderClass::appendNextCommand() {
  ...
  // don't debug log this if we're debugging sync errors, as it will cause diff
  // problems between a game and it's replay...
#ifdef DEBUG_LOGGING
  Bool logCommand = true;
#ifdef DEBUG_CRC
  if (!m_doingAnalysis) logCommand = false;
#endif
  if (logCommand) {
    DEBUG_LOG(
        ("RecorderClass::appendNextCommand - Adding %s command from player %d "
         "to TheCommandList on frame %d\n",
         commandName.str(),
  ...
  for (Int j = 0; j < totalArgs; ++j) {
    ...
    if (argsLeftForType == 0) {
      ...
      // parserArgType is allowed to be NULL here, this is the case if there are
      // no more arguments.
      if (parserArgType != NULL) {
        argsLeftForType = parserArgType->getArgCount();
        lasttype = parserArgType->getType();
  ...
  if (type == ARGUMENTDATATYPE_INTEGER) {
    ...
    if (m_doingAnalysis) {
      DEBUG_LOG(("Coord3D argument: %g %g %g (%8.8X %8.8X %8.8X)\n", loc.x,
                 loc.y, loc.z, *(int *)&loc.x, *(int *)&loc.y, *(int *)&loc.z));
    }
#endif
  } else if (type == ARGUMENTDATATYPE_PIXEL) {
    ICoord2D pixel;
    ...
    if (m_doingAnalysis) {
      DEBUG_LOG(("Pixel Region argument: %d,%d -> %d,%d\n", reg.lo.x, reg.lo.y,
             ...
             ARGUMENTDATATYPE_TIMESTAMP) {  // Not to be confused with Terrance
                                            // Stamp... Kneel before Zod!!!
    UnsignedInt stamp;
...
/**
 * This needs to be called for every frame during playback. Basically it
 * prevents the user from inserting.
 */
void RecorderClass::cullBadCommands() {
  if (m_doingAnalysis) return;
...
AsciiString RecorderClass::getLastReplayFileName() {
  ...
  if (TheNetwork && TheGlobalData->m_saveStats) {
    ...
    if (game) {
      ...
      full.format("%s%s_%d_%d", players.str(), mapName.str(), game->getSeed(),
                  game->getLocalSlotNum());
      AsciiString testString;
      testString.format("%s%s%s", getReplayDir().str(), full.str(),
                        replayExtention);
      ...
      while (test < 20) {
        ...
        testString.format("%s%s%s", getReplayDir().str(), fullPlusNum.str(),
                          replayExtention);
        fp = fopen(testString.str(), "rb");
        if (fp) {
          fclose(fp);
 ...
 */
RecorderModeType RecorderClass::getMode() { return m_mode; }

///< Show or Hide the Replay controls
void RecorderClass::initControls() {
  NameKeyType parentReplayControlID = TheNameKeyGenerator->nameToKey(
      AsciiString("ReplayControl.wnd:ParentReplayControl"));
  GameWindow *parentReplayControl =
      TheWindowManager->winGetWindowFromId(NULL, parentReplayControlID);
...
Bool RecorderClass::isMultiplayer(void) {
  if (m_mode == RECORDERMODETYPE_PLAYBACK) {
    ...
    for (int i = 0; i < MAX_SLOTS; ++i) {
      ...
      if (slot && slot->isOccupied())  ///< slots default to closed for
                                       ///< non-networked games
        return true;
    }
  }
  ...
  if (TheNetwork || TheSkirmishGameInfo) return true;
 ...
 */
RecorderClass *createRecorder() { return NEW RecorderClass; }
