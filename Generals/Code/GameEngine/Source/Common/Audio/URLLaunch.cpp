...
HRESULT GetShellOpenCommand(LPTSTR ptszShellOpenCommand,
  ...
  do {
    ...
    lResult =
        RegOpenKeyEx(HKEY_CLASSES_ROOT, _T( ".html" ), 0, KEY_READ, &hKey);
    ...
    lResult =
        RegQueryValueEx(hKey, NULL, 0, NULL, (BYTE *)szFileType, &dwLength);
    ...
    lResult =
        RegOpenKeyEx(HKEY_CLASSES_ROOT, szKeyName, 0, KEY_READ, &hFileKey);
    ...
    lResult = RegQueryValueEx(hFileKey, NULL, 0, NULL,
  ...
  //
  // If there was no application associated with .html files by extension, look
  // for an application associated with the http protocol
  //
  if (!fFoundExtensionCommand) {
    ...
    do {
      //
      // Find the command for the shell's open verb associated with the http
      // protocol
      //
      lResult =
          RegOpenKeyEx(HKEY_CLASSES_ROOT, _T( "http\\shell\\open\\command" ), 0,
                       KEY_READ, &hKey);
      ...
      lResult = RegQueryValueEx(hKey, NULL, 0, NULL,
                                (BYTE *)ptszShellOpenCommand, &dwLength);
    } while (FALSE);
  }
...
HRESULT LaunchURL(LPCWSTR pszURL) {
  ...
  if (NULL != pszParam) {
    *pszParam = _T('\0');
    ...
    wsprintf(szLaunchCommand, _T( "%s%ws%s" ), szShellOpenCommand, pszURL,
             pszParam + 4);
  } else {
  ...
  if (!CreateProcess(szExe, szLaunchCommand, NULL, NULL, FALSE, 0, NULL, NULL,
                     &StartUp, &ProcInfo)) {
