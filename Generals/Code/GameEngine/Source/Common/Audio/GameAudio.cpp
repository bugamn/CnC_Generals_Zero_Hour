...
static const char *TheSpeakerTypes[] = {
    "2 Speakers",   "Headphones", "Surround Sound", "4 Speaker", "5.1 Surround",
    "7.1 Surround", NULL};

static const Int TheSpeakerTypesCount =
    sizeof(TheSpeakerTypes) / sizeof(TheSpeakerTypes[0]);

static void parseSpeakerType(INI *ini, void *instance, void *store,
                             const void *userData);

// Field Parse table for Audio Settings
// ///////////////////////////////////////////////////////////
static const FieldParse audioSettingsFieldParseTable[] = {
    {"AudioRoot", INI::parseAsciiString, NULL,
     offsetof(AudioSettings, m_audioRoot)},
    {"SoundsFolder", INI::parseAsciiString, NULL,
     offsetof(AudioSettings, m_soundsFolder)},
    {"MusicFolder", INI::parseAsciiString, NULL,
     offsetof(AudioSettings, m_musicFolder)},
    {"StreamingFolder", INI::parseAsciiString, NULL,
     offsetof(AudioSettings, m_streamingFolder)},
    {"SoundsExtension", INI::parseAsciiString, NULL,
     offsetof(AudioSettings, m_soundsExtension)},
    ...
    {"OutputChannels", INI::parseInt, NULL,
     offsetof(AudioSettings, m_outputChannels)},
    {"SampleCount2D", INI::parseInt, NULL,
     offsetof(AudioSettings, m_sampleCount2D)},
    {"SampleCount3D", INI::parseInt, NULL,
     offsetof(AudioSettings, m_sampleCount3D)},
    {"StreamCount", INI::parseInt, NULL,
     offsetof(AudioSettings, m_streamCount)},

    {"Preferred3DHW1", INI::parseAsciiString, NULL,
     offsetof(AudioSettings, m_preferred3DProvider[0])},
    {"Preferred3DHW2", INI::parseAsciiString, NULL,
     offsetof(AudioSettings, m_preferred3DProvider[1])},
    {"Preferred3DHW3", INI::parseAsciiString, NULL,
     offsetof(AudioSettings, m_preferred3DProvider[2])},
    {"Preferred3DHW4", INI::parseAsciiString, NULL,
     offsetof(AudioSettings, m_preferred3DProvider[3])},

    {"Preferred3DSW", INI::parseAsciiString, NULL,
     offsetof(AudioSettings, m_preferred3DProvider[4])},

    {"Default2DSpeakerType", parseSpeakerType, NULL,
     offsetof(AudioSettings, m_defaultSpeakerType2D)},
    {"Default3DSpeakerType", parseSpeakerType, NULL,
     offsetof(AudioSettings, m_defaultSpeakerType3D)},

    {"MinSampleVolume", INI::parsePercentToReal, NULL,
     offsetof(AudioSettings, m_minVolume)},
    {"GlobalMinRange", INI::parseInt, NULL,
     offsetof(AudioSettings, m_globalMinRange)},
    {"GlobalMaxRange", INI::parseInt, NULL,
     offsetof(AudioSettings, m_globalMaxRange)},
    {"TimeBetweenDrawableSounds", INI::parseDurationUnsignedInt, NULL,
     offsetof(AudioSettings, m_drawableAmbientFrames)},
    {"TimeToFadeAudio", INI::parseDurationUnsignedInt, NULL,
     offsetof(AudioSettings, m_fadeAudioFrames)},
    {"AudioFootprintInBytes", INI::parseUnsignedInt, NULL,
     offsetof(AudioSettings, m_maxCacheSize)},
    {"Relative2DVolume", INI::parsePercentToReal, NULL,
     offsetof(AudioSettings, m_relative2DVolume)},
    {"DefaultSoundVolume", INI::parsePercentToReal, NULL,
     offsetof(AudioSettings, m_defaultSoundVolume)},
    {"Default3DSoundVolume", INI::parsePercentToReal, NULL,
     offsetof(AudioSettings, m_default3DSoundVolume)},
    {"DefaultSpeechVolume", INI::parsePercentToReal, NULL,
     offsetof(AudioSettings, m_defaultSpeechVolume)},
    {"DefaultMusicVolume", INI::parsePercentToReal, NULL,
     offsetof(AudioSettings, m_defaultMusicVolume)},
    {"MicrophoneDesiredHeightAboveTerrain", INI::parseReal, NULL,
     offsetof(AudioSettings, m_microphoneDesiredHeightAboveTerrain)},
    {"MicrophoneMaxPercentageBetweenGroundAndCamera", INI::parsePercentToReal,
     NULL,
     offsetof(AudioSettings, m_microphoneMaxPercentageBetweenGroundAndCamera)},
    {"ZoomMinDistance", INI::parseReal, NULL,
     offsetof(AudioSettings, m_zoomMinDistance)},
    {"ZoomMaxDistance", INI::parseReal, NULL,
     offsetof(AudioSettings, m_zoomMaxDistance)},
    {"ZoomSoundVolumePercentageAmount", INI::parsePercentToReal, NULL,
     offsetof(AudioSettings, m_zoomSoundVolumePercentageAmount)},
...
AudioManager::~AudioManager() {
  ...
  for (it = m_allAudioEventInfo.begin(); it != m_allAudioEventInfo.end();
       ++it) {
  ...
  if (m_savedValues) delete[] m_savedValues;
}

//-------------------------------------------------------------------------------------------------
void AudioManager::init() {
  INI ini;
  ini.load(AsciiString("Data\\INI\\AudioSettings.ini"), INI_LOAD_OVERWRITE,
           NULL);

  ini.load(AsciiString("Data\\INI\\Default\\Music.ini"), INI_LOAD_OVERWRITE,
           NULL);
  ini.load(AsciiString("Data\\INI\\Music.ini"), INI_LOAD_OVERWRITE, NULL);

  ini.load(AsciiString("Data\\INI\\Default\\SoundEffects.ini"),
           INI_LOAD_OVERWRITE, NULL);
  ini.load(AsciiString("Data\\INI\\SoundEffects.ini"), INI_LOAD_OVERWRITE,
           NULL);

  ini.load(AsciiString("Data\\INI\\Default\\Speech.ini"), INI_LOAD_OVERWRITE,
           NULL);
  ini.load(AsciiString("Data\\INI\\Speech.ini"), INI_LOAD_OVERWRITE, NULL);

  ini.load(AsciiString("Data\\INI\\Default\\Voice.ini"), INI_LOAD_OVERWRITE,
           NULL);
  ini.load(AsciiString("Data\\INI\\Voice.ini"), INI_LOAD_OVERWRITE, NULL);

  // do the miscellaneous sound files last so that we find the audioeventrts
  // associated with the events.
  ini.load(AsciiString("Data\\INI\\MiscAudio.ini"), INI_LOAD_OVERWRITE, NULL);

  // determine if one of the music tracks exists. Since their now BIGd, one
  // implies all. If they don't exist, then attempt to load them from the CD.
  if (!isMusicAlreadyLoaded()) {
    ...
    while (TRUE) {
      ...
      // We loop infinitely on the splash screen if we don't allow breaking out
      // of this loop.
      // #if !defined( _DEBUG ) && !defined( _INTERNAL )
        ...
        if (OSDisplayWarningBox("GUI:InsertCDPrompt", "GUI:InsertCDMessage",
                                ...
                                OSDOF_SYSTEMMODAL | OSDOF_EXCLAMATIONICON) ==
            OSDBT_CANCEL) {
  ...
  // Set our system volumes from the user's preferred settings, not the
  // defaults.
  m_systemMusicVolume =
      getAudioSettings() ? getAudioSettings()->m_preferredMusicVolume : 0.55f;
  m_systemSoundVolume =
      getAudioSettings() ? getAudioSettings()->m_preferredSoundVolume : 0.75f;
  m_systemSound3DVolume =
      getAudioSettings() ? getAudioSettings()->m_preferred3DSoundVolume : 0.75f;
  m_systemSpeechVolume =
      getAudioSettings() ? getAudioSettings()->m_preferredSpeechVolume : 0.55f;
...
void AudioManager::update() {
  ...
  Real desiredHeight =
      TheAudio->getAudioSettings()->m_microphoneDesiredHeightAboveTerrain;
  Real maxPercentage = TheAudio->getAudioSettings()
                           ->m_microphoneMaxPercentageBetweenGroundAndCamera;

  Coord3D lookTo;
  lookTo.set(forward.X, forward.Y, forward.Z);

  // Kris: At this point, the microphone is calculated to be at the ground
  // position where the camera is looking at. Instead we want to move the
  // microphone towards the camera. Hopefully, it'll be a desired altitude, but
  // if it gets too close to the camera (or even past it), that would be
  // undesirable. Therefore, we have a backup method of making sure we only go a
  // certain percentage towards the camera or the desired height, whichever
  // occurs first.
  ...
  if (cameraPos.z <= desiredHeight || groundToCameraVector.z <= 0.0f) {
    ...
    // Calculate the stopping position of the groundToCameraVector when we force
    // z to be m_microphoneDesiredHeightAboveTerrain
  ...
  // Set the microphone to be the ground position adjusted for terrain plus the
  ...
  // Now determine if we would like to boost the volume based on the camera
  // being close to the microphone!
  Real maxBoostScalar =
      TheAudio->getAudioSettings()->m_zoomSoundVolumePercentageAmount;
...
//-------------------------------------------------------------------------------------------------
void AudioManager::getInfoForAudioEvent(
  ...
  eventToFindAndFill->setAudioEventInfo(
      findAudioEventInfo(eventToFindAndFill->getEventName()));
}

//-------------------------------------------------------------------------------------------------
AudioHandle AudioManager::addAudioEvent(const AudioEventRTS *eventToAdd) {
  if (eventToAdd->getEventName().isEmpty() ||
      eventToAdd->getEventName() == AsciiString("NoSound")) {
    return AHSV_NoSound;
  }
  ...
  switch (eventToAdd->getAudioEventInfo()->m_soundType) {
    ...
    case AT_SoundEffect:
      ...
      break;
    case AT_Streaming:
      if (!isOn(AudioAffect_Speech)) return AHSV_NoSound;
      break;
  }

  // if we're currently playing uninterruptable speech, then disallow the
  // addition of this sample
  if (getDisallowSpeech() &&
      eventToAdd->getAudioEventInfo()->m_soundType == AT_Streaming) {
    return AHSV_NoSound;
  }

  AudioEventRTS *audioEvent =
      MSGNEW("AudioEventRTS") AudioEventRTS(*eventToAdd);  // poolify
  audioEvent->setPlayingHandle(allocateNewHandle());
  audioEvent->generateFilename();  // which file are we actually going to play?
  ((AudioEventRTS *)eventToAdd)
      ->setPlayingAudioIndex(audioEvent->getPlayingAudioIndex());
  audioEvent->generatePlayInfo();  // generate pitch shift and volume shift now
                                   // as well
...
//-------------------------------------------------------------------------------------------------
void AudioManager::removeDisabledEvents() { removeAllDisabledAudio(); }

//-------------------------------------------------------------------------------------------------
Bool AudioManager::isCurrentlyPlaying(AudioHandle audioEvent) { return true; }

//-------------------------------------------------------------------------------------------------
UnsignedInt AudioManager::translateSpeakerTypeToUnsignedInt(
...
//-------------------------------------------------------------------------------------------------
AsciiString AudioManager::translateUnsignedIntToSpeakerType(
...
void AudioManager::set3DVolumeAdjustment(Real volumeAdjustment) {
  m_sound3DVolume =
      volumeAdjustment * m_scriptSound3DVolume * m_systemSound3DVolume;

  // clamp
  if (m_sound3DVolume < 0.0f) m_sound3DVolume = 0.0f;

  if (m_sound3DVolume > 1.0f) m_sound3DVolume = 1.0f;
...
//-------------------------------------------------------------------------------------------------
const MiscAudio *AudioManager::getMiscAudio(void) const { return m_miscAudio; }

//-------------------------------------------------------------------------------------------------
MiscAudio *AudioManager::friend_getMiscAudio(void) { return m_miscAudio; }
...
//-------------------------------------------------------------------------------------------------
void AudioManager::findAllAudioEventsOfType(
...
Bool AudioManager::isCurrentProviderHardwareAccelerated() {
  for (Int i = 0; i < MAX_HW_PROVIDERS; ++i) {
    if (getProviderName(getSelectedProvider()) ==
...
Bool AudioManager::isCurrentSpeakerTypeSurroundSound() {
  return (getSpeakerType() ==
          TheAudio->getAudioSettings()->m_defaultSpeakerType3D);
}
...
Bool AudioManager::shouldPlayLocally(const AudioEventRTS *audioEvent) {
  ...
  if (!BitTest(ei->m_type,
               (ST_PLAYER | ST_ALLIES | ST_ENEMIES | ST_EVERYONE))) {
  ...
  Player *owningPlayer =
      ThePlayerList->getNthPlayer(audioEvent->getPlayerIndex());

  if (BitTest(ei->m_type, ST_PLAYER) && BitTest(ei->m_type, ST_UI) &&
      owningPlayer == NULL) {
    DEBUG_ASSERTCRASH(!TheGameLogic->isInGameLogicUpdate(),
                      ("Playing %s sound -- player-based UI sound without "
                       "specifying a player.\n"));
    return TRUE;
  }
  ...
  if (owningPlayer == NULL) {
    DEBUG_CRASH(
        ("Sound '%s' expects an owning player, but the audio event that "
         "created it didn't specify one.\n",
         ei->m_audioName.str()));
    return FALSE;
  }
  ...
  if (BitTest(ei->m_type, ST_ALLIES)) {
    // We have to also check that the owning player isn't the local player,
    // because PLAYER wasn't specified, or we wouldn't have gotten here.
    return (owningPlayer != localPlayer) &&
           owningPlayer->getRelationship(localTeam) == ALLIES;
  }
...
void AudioManager::loseFocus(void) {
  DEBUG_ASSERTLOG(m_savedValues == NULL,
                  ("AudioManager::loseFocus() - leak - jkmcd\n"));
...
void AudioManager::regainFocus(void) {
  ...
  setVolume(m_savedValues[0],
            (AudioAffect)(AudioAffect_Music | AudioAffect_SystemSetting));
  setVolume(m_savedValues[1],
            (AudioAffect)(AudioAffect_Sound | AudioAffect_SystemSetting));
  setVolume(m_savedValues[2],
            (AudioAffect)(AudioAffect_Sound3D | AudioAffect_SystemSetting));
  setVolume(m_savedValues[3],
            (AudioAffect)(AudioAffect_Speech | AudioAffect_SystemSetting));
...
void INI::parseAudioSettingsDefinition(INI *ini) {
  ini->initFromINI(TheAudio->friend_getAudioSettings(),
                   TheAudio->getFieldParseTable());

  // time to override the volume settings, default 3-D provider, and speaker
  // setup with the ones stored from the prefs
  ...
  TheAudio->friend_getAudioSettings()->m_preferredSoundVolume =
      prefs.getSoundVolume() / 100.0f;
  TheAudio->friend_getAudioSettings()->m_preferred3DSoundVolume =
      prefs.get3DSoundVolume() / 100.0f;
  TheAudio->friend_getAudioSettings()->m_preferredSpeechVolume =
      prefs.getSpeechVolume() / 100.0f;
  TheAudio->friend_getAudioSettings()->m_preferredMusicVolume =
      prefs.getMusicVolume() / 100.0f;
}
...
//-------------------------------------------------------------------------------------------------
void parseSpeakerType(INI *ini, void *instance, void *store,
                      const void *userData) {
  AsciiString str;
