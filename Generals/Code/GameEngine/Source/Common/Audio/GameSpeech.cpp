...
//===============================
/**
 *	The Speech struct holds all information about a line of dialog. While
 *Speech itself is not accessible from outside SpeechInterface, the embedded
 *struct SpeechInfo is accessible to the game code. Place data in SpeechInfo
 *that is useful to the game code in determining what speech to play.
 */
//===============================
...
//===============================
/**
 *	This is an internal structure used by the SpeechManager. It holds
 *information about the playback of a line of dialog.
 */
//===============================
...
class Speaker : public SpeakerInterface {
 ...
 public:
  ...
  virtual void setDelay(
      Int delay);  ///< Set speaker's delay between speeches( in milliseconds)
  virtual void setBuffering(
      Int buffer_time);  ///< Set speaker's amount of buffer (in milliseconds)
  virtual Int getPriority(void);  ///< Get speaker's priority level
  virtual Int getDelay(
      void);  ///< Get speaker's delay between speeches( in milliseconds)
  virtual Int getBuffering(
      void);  ///< Get speaker's amount of buffer (in milliseconds)
  ...
  virtual Bool hasSaid(
      Speech *speech);  ///< is line of dialog no longer pending
  virtual Bool isGoingToSay(
      Speech *speech);           ///< check in line of dialog is pending
  ...
  void init(char *name, Int priority, Int btime,
            Int delay);  ///< initialized speaker for use
  void deinit(void);
...
class SpeechManager : public SpeechInterface {
 protected:
  ...
  struct AudioDeviceTag
      *m_device;  ///< The audio device we were initilaized with

  LList m_speakers;     ///< list of speakers created by this manager
  HashSpeech m_speech;  ///< This is a hash table of all speeches we currently
                        ///< have loaded.
  VecAsciiString
      m_speechNames;  ///< The purpose of this is to return to someone asking
                      ///< for the name of some element.
  VecSpeech
      m_temporarySpeeches;  ///< This is a vector of speeches that are _like_
                            ///< something in the hash, but differ by priority.

  AsciiString getFilenameForPlay(
      Speech *speech);  ///< Return the filename corresponding to this speech
 ...
 public:
  ...
  virtual void update(
      void);  ///< Services speech tasks. Called by AudioInterface
  ...
  virtual SpeechInfo *getSpeechInfo(
      Speech *speech);  ///< Returns speech info by speech
  ...
  virtual Speech *getSpeech(
      const Char *speech_name);  ///< Converts speech name to speech
  virtual const Char *getSpeechName(
      Speech *speech);  ///< Converts speech to speech name

  // volume control
  virtual void fadeIn(void);   ///< Fade all speech in
  virtual void fadeOut(void);  ///< Fade all speech out
  virtual void fade(
      Real fade_value);  ///< Fade all speech by a specified amout (1.0 .. 0.0 )
  virtual Bool isFading(
      void);  ///< Returns whether or not any speech is in the process of a fade
  virtual void waitForFade(void);  ///< Returns when all fading has finished
  virtual void setVolume(
      Real new_volume);  ///< Set new volume level for all speech 1.0 (loudest)
                         ///< 0.0 (silent)
  virtual Real getVolume(void);  ///< Return current volume level for all speech

  /// Speaker factory
  virtual SpeakerInterface *createSpeaker(Char *name, Int priority,
                                          Int buffer_time, Int delay);

  virtual Speech *addNewSpeech(
      SpeechInfo *pSpeechToAdd);  ///< Create a speech specified by pSpeechToAdd
  virtual Speech *addTemporaryDialog(
      Speech &temporarySpeech);  ///< Speech to be taken over by
  // playback control
  ...
  virtual void turnOff(
      void);  ///< Turns off all speech. No more speech will play
  virtual void turnOn(void);                  ///< Turns speech on.
  virtual Bool waitToStop(Int milliseconds);  ///< Returns when all speech has
                                              ///< completely stopped playing
  virtual Bool say(
      Speech *pSpeechToSay);  ///< Attempts to add a speech to the speach queue.
  virtual Bool say(AsciiString speechName);  ///< Attempts to find and add a
                                             ///< speech to the speech queue

  // info
  virtual Bool isOn(
      void);  ///< Returns whether or not speech is turned on at present
  virtual struct AudioAttribs *getMasterAttribs(
      void);  ///< Returns the master attribute control for all speech
  virtual struct AudioAttribs *getMasterFadeAttribs(
      void);  ///< Returns the master fade control for all speech
  virtual void addDialogEvent(
      const AudioEventRTS *eventRTS, Speech *speech,
      AudioEventRTS *returnEvent);  ///< Add an event. speech can be NULL
  virtual void removeDialogEvent(
      const AudioEventRTS *eventRTS,
      AudioEventRTS
          *eventToUse);  ///< remove an event, eventToUse shouldn't be NULL.

  void removeSpeaker(Speaker *speaker);
  void addSpeaker(Speaker *speaker);

  // empty string means that this sound wasn't found or some error occurred.
  // CHECK FOR EMPTY STRING.
  virtual AsciiString getFilenameForPlayFromAudioEvent(
      const AudioEventRTS *eventToGetFrom);
};
...
SpeechInterface *CreateSpeechInterface(void) { return NEW SpeechManager; }

//============================================================================
// Speech::Speech
//============================================================================

Speech::Speech() : id(INVALID_ID), valid(FALSE), volume(TheAudio->VOLUME_MIN) {}
...
Int SpeechManager::numSpeeches(void) { return m_count; }
...
void SpeechManager::fade(Real new_volume) {
  if (m_masterFadeAttribs) {
    AudioAttribsAdjustVolume(m_masterFadeAttribs,
                             TheAudio->convertRealVolume(new_volume));
  }
}
...
void SpeechManager::setVolume(Real new_volume) {
  ...
  if (m_masterAttribs) {
    AudioAttribsSetVolume(m_masterAttribs,
                          TheAudio->convertRealVolume(new_volume));
  }
}
...
Real SpeechManager::getVolume(void) { return m_volume; }
...
void SpeechManager::turnOn(void) { m_on = TRUE; }

//============================================================================
// SpeechManager::waitToStop
//============================================================================
Bool SpeechManager::waitToStop(Int milliseconds) { return TRUE; }
...
Bool SpeechManager::say(Speech *pSpeechToSay) {
  ...
  currentSpeaker->say(pSpeechToSay, pSpeechToSay->priority,
                      pSpeechToSay->timeout, pSpeechToSay->interrupt);
  return true;
}
...
Bool SpeechManager::isOn(void) { return m_on; }
...
SpeakerInterface *SpeechManager::createSpeaker(Char *name, Int priority,
                                               Int btime, Int delay) {
...
void SpeechManager::addDialogEvent(const AudioEventRTS *eventRTS,
                                   Speech *speech, AudioEventRTS *returnEvent) {
  if (!eventRTS) {
    return;
  }
...
void SpeechManager::removeDialogEvent(const AudioEventRTS *eventRTS,
  ...
  while (node && isPlaying) {
    ...
    if (pCurrSpeaker->saying() == useSpeech ||
        pCurrSpeaker->isGoingToSay(useSpeech)) {
...
AsciiString SpeechManager::getFilenameForPlayFromAudioEvent(
    const AudioEventRTS *eventToGetFrom) {
  if (!eventToGetFrom) {
...
AsciiString SpeechManager::getFilenameForPlay(Speech *speech) {
  ...
  Int numSamples =
      regularSamples + eveningSamples + morningSamples + nightSamples;

  // using a random number generator, select a random sample from all the
  // samples that correspond to this sound name
  ...
  if (soundToPlay < regularSamples) {
    ...
    strcpy(
        name,
        speechInfo->m_dialogFilesEvening[soundToPlay - regularSamples].str());
  } else if (soundToPlay < (regularSamples + eveningSamples + morningSamples)) {
    strcpy(name, speechInfo
                     ->m_dialogFilesMorning[soundToPlay - regularSamples -
                                            eveningSamples]
                     .str());
  } else if (soundToPlay < numSamples) {
    strcpy(name, speechInfo
                     ->m_dialogFilesNight[soundToPlay - regularSamples -
                                          eveningSamples - morningSamples]
                     .str());
  } else {
    return AsciiString::TheEmptyString;
  }
...
void Speaker::say(Speech *speech, Int priority, Int timeout, Int interrupt) {
  ...
  if (timeout) {
    item->timeout =
        m_paused ? MSECONDS(timeout) : AudioGetTime() + MSECONDS(timeout);
  }
...
void Speaker::update(void) {
  ...
  if (!m_paused && m_currentSpeech == NULL &&
      (AudioStreamerEndTimeStamp(m_stream) + m_delay) <= now) {
    ...
    if (next) {
      ...
      AudioStreamerSetMaxVolume(
          m_stream, TheAudio->convertRealVolume(next->speech->volume));
      AudioStreamerSetVolume(m_stream,
                             TheAudio->convertRealVolume(next->speech->volume));

      AudioStreamerOpen(m_stream,
                        getNextFilenameFromSpeech(next->speech).str());
      ...
      // REMOVE_GDF --> this must be converted to INI or something
                              GDI_Asset *asset = TheGameData->openAsset(
      next->speech->handle );
                              ...
                              if( asset )
                              {
                                      ...
                                      {
                                              if ( AudioStreamerOpen( m_stream,
      m_file ))
                                              {
                                                      if ( AudioStreamerStart (
      m_stream ) )
                                                      {
                                                              m_delay =
      m_delayTime;
...
Int Speaker::getPriority(void) { return m_priority; }

//============================================================================
// Speaker::setDelay
//============================================================================

void Speaker::setDelay(Int delay) { m_delay = MSECONDS(delay); }

//============================================================================
// Speaker::getDelay
//============================================================================

Int Speaker::getDelay(void) { return IN_MSECONDS(m_delay); }
  ...
  if (!m_paused) {
    ...
    while (item) {
      if (!(item->flags & SpeechItem::PAUSED)) {
        item->flags =
            SpeechItem::Flags((Int)item->flags | (Int)SpeechItem::PAUSED);
        if (item->timeout) {
...
Bool Speaker::hasSaid(Speech *speech) {
  return findItem(speech) == NULL &&
         (m_currentSpeech == NULL || m_currentSpeech->speech != speech);
}
...
Bool Speaker::isGoingToSay(Speech *speech) { return findItem(speech) != NULL; }

//============================================================================
// Speaker::isTalking
//============================================================================

Bool Speaker::isTalking(void) { return m_currentSpeech != NULL; }
...
static AsciiString getNextFilenameFromSpeech(Speech *pSpeechToPlay) {
  ...
  if (pSpeechToPlay->info.m_sequentialStartIndex < 0) {
    indexToPlay =
        GameClientRandomValue(pSpeechToPlay->info.m_randomStartIndex,
