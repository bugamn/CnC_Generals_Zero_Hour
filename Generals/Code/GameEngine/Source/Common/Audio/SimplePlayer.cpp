...
CSimplePlayer::~CSimplePlayer() {
  DEBUG_ASSERTCRASH(0 == m_cBuffersOutstanding,
                    ("CSimplePlayer destructor m_cBuffersOutstanding != 0"));
...
///////////////////////////////////////////////////////////////////////////////
HRESULT CSimplePlayer::Play(LPCWSTR pszUrl, DWORD dwSecDuration,
                            HANDLE hCompletionEvent, HRESULT *phrCompletion) {
  HRESULT hr;

  //
  // If the URL is not a UNC path, a full path, or an Internet-style URL then
  // assume it is a relative local file name that needs to be expanded to a full
  // path.
  //
  WCHAR wszFullUrl[MAX_PATH];

  if ((0 == wcsstr(pszUrl, L"\\\\")) && (0 == wcsstr(pszUrl, L":\\")) &&
      (0 == wcsstr(pszUrl, L"://"))) {
    //
  ...
  for (i = 0; i < wAttrCnt; i++) {
    ...
    hr = m_pHeader->GetAttributeByIndex(i, &wStream, NULL, &cchNamelen, &type,
                                        NULL, &cbLength);
    if (FAILED(hr)) {
    ...
    hr = m_pHeader->GetAttributeByIndex(i, &wStream, pwszName, &cchNamelen,
                                        &type, pValue, &cbLength);
    if (FAILED(hr)) {
      DEBUG_LOG(("GetAttributeByIndex Failed (hr=0x%08x)\n", hr));
      break;
    }
  ...
  mmr = waveOutOpen(&m_hwo, WAVE_MAPPER, &m_wfx, (DWORD)WaveProc, (DWORD)this,
                    CALLBACK_FUNCTION);
  mmr = MMSYSERR_NOERROR;
  ...
  switch (Status) {
    ...
    case WMT_NO_RIGHTS: {
      ...
      if (SUCCEEDED(hr)) {
        ...
        swprintf(wszURL, L"%s&filename=%s&embedded=false", pValue,
                 pwszEscapedURL);
        ...
        if (FAILED(hr)) {
          DEBUG_LOG(
              ("Unable to launch web browser to retrieve playback license "
               "(hr=0x%08x)\n",
               hr));
        }
...
///////////////////////////////////////////////////////////////////////////////
void CALLBACK CSimplePlayer::WaveProc(HWAVEOUT hwo, UINT uMsg, DWORD dwInstance,
...
void CSimplePlayer::RemoveWaveHeaders() {
  ...
  while (NULL != m_whdrHead) {
    tmp = m_whdrHead->next;
    DEBUG_ASSERTCRASH(m_whdrHead->pwh->dwFlags & WHDR_DONE,
                      ("RemoveWaveHeaders!"));
