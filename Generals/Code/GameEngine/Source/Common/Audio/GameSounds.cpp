...
void SoundManager::postProcessLoad() {
  // The AudioManager should actually be live now, so go ahead and get the info
  // we need from it here
}
...
//-------------------------------------------------------------------------------------------------
Real SoundManager::getCameraAudibleDistance(void) { return 1.0f; }
...
//-------------------------------------------------------------------------------------------------
void SoundManager::notifyOf2DSampleStart(void) { ++m_numPlaying2DSamples; }

//-------------------------------------------------------------------------------------------------
void SoundManager::notifyOf3DSampleStart(void) { ++m_numPlaying3DSamples; }
...
//-------------------------------------------------------------------------------------------------
AsciiString SoundManager::getFilenameForPlayFromAudioEvent(
...
Bool SoundManager::canPlayNow(AudioEventRTS *event) {
  ...
  // 3) Are we violating our voice count or are we playing above the limit? (If
  // so, stop now) 4) is there an avaiable channel open? 5) if not, then
  // determine if there is anything of lower priority that we can kill 6) if
  // not, are we an interrupt-sound type? if so, are there any sounds of our
  // type playing right now that we can interrupt? potentially here: Are there
  // any sounds that are playing that are now beyond their distance? if so, kill
  // them and start our sound if not, we're done. Can't play dude.

  if (event->isPositionalAudio() &&
      !BitTest(event->getAudioEventInfo()->m_type, ST_GLOBAL) &&
      event->getAudioEventInfo()->m_priority != AP_CRITICAL) {
...
#ifdef INTENSIVE_AUDIO_DEBUG
      DEBUG_LOG(
          ("- culled due to no channels available and non-interrupting.\n"));
#endif
      return false;
    }
  }
...
Bool SoundManager::violatesVoice(AudioEventRTS *event) {
  if (event->getAudioEventInfo()->m_type & ST_VOICE) {
    return (event->getObjectID() &&
            TheAudio->isObjectPlayingVoice(event->getObjectID()));
  }
  return false;
}
