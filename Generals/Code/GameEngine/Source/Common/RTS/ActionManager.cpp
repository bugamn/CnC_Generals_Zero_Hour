...
// ////////////////////////////////////////////////////////////////////////
// Author: Colin Day
// Desc:   TheActionManager is a convenient place for us to wrap up all sorts of
// logical
//				 queries about what objects can do in the world
//and to other objects.  The purpose 				 of having a central place for this logic
//assists us in making these logical kind 				 of queries in the user interface and
//allows us to use the same code to validate 				 commands as they come in over the
//network interface in order to do the 				 real action.
///////////////////////////////////////////////////////////////////////////////////////////////////
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
static Bool appearsToContainFriendlies(const Object *obj,
  ...
  if (otherContain) {
    const Player *otherPlayer =
        otherContain->getApparentControllingPlayer(obj->getControllingPlayer());
    //	if( otherPlayer && otherPlayer->getRelationship( obj->getTeam() ) !=
    //ENEMIES )
    // the above test is wrong; we want to know how WE consider THEM, not how
    // THEY consider US
    if (otherPlayer && obj->getTeam()->getRelationship(
                           otherPlayer->getDefaultTeam()) != ENEMIES) {
      return TRUE;
    }
  }
  return FALSE;
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
static Bool isObjectShroudedForAction(const Object *source,
                                      const Object *target,
                                      CommandSourceType commandSource) {
  /// @todo - reenable this when we can avoid breaking scripted actions.
  // In order to support ai and scripted actions in singler player, we have to
  // disable this for now. We can re-enable it when we can tell that this is a
  // player generated action.  jba.
  //	return false;
  // GS Keeping this comment to show we now have commandSource, so everything
  ...
  if (source && target && source->getControllingPlayer()) {
    if (source->getControllingPlayer()->getPlayerType() == PLAYER_HUMAN &&
        ...
        target->getShroudedStatus(
            source->getControllingPlayer()->getPlayerIndex()) >=
            OBJECTSHROUD_FOGGED) {
      return TRUE;
    }
  }
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
ActionManager::ActionManager(void) {}  // end ActionManager
...
Bool ActionManager::canGetRepairedAt(const Object *obj,
                                     ...
                                     CommandSourceType commandSource) {
  // sanity
  if (obj == NULL || repairDest == NULL) return FALSE;
  ...
  if (BitTest(obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION) == TRUE ||
      BitTest(repairDest->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION) ==
          TRUE)
    return FALSE;

  // Can't get repaired at something being sold
  if (repairDest->testStatus(OBJECT_STATUS_SOLD)) return FALSE;

  // only vehicles can go get repaired at something
  if (obj->isKindOf(KINDOF_VEHICLE) == FALSE) return FALSE;

  // vehicles can only be repaired at something that is designated as a repair
  // pad
  if (obj->isKindOf(KINDOF_AIRCRAFT)) {
    ...
    if (repairDest->isKindOf(KINDOF_REPAIR_PAD) == FALSE) return FALSE;
  }

  // if I am at full health, I can't get repair there
  BodyModuleInterface *body = obj->getBodyModule();
  if (body->getHealth() == body->getMaxHealth()) return FALSE;

  // if the target is in the shroud, we can't do anything
  if (isObjectShroudedForAction(obj, repairDest, commandSource)) return FALSE;
...
Bool ActionManager::canTransferSuppliesAt(const Object *obj,
  ...
  // sanity
  if (obj == NULL || transferDest == NULL) return FALSE;
  ...
  // nothing can be done with things that are under construction
  if (BitTest(obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION) == TRUE ||
      BitTest(transferDest->getStatusBits(),
              OBJECT_STATUS_UNDER_CONSTRUCTION) == TRUE)
    return FALSE;

  // Can't transfer at something being sold
  if (transferDest->testStatus(OBJECT_STATUS_SOLD)) return FALSE;
  ...
  if (supplyTruck == NULL) return FALSE;
  ...
  SupplyWarehouseDockUpdate *warehouseModule =
      (SupplyWarehouseDockUpdate *)transferDest->findUpdateModule(
          key_warehouseUpdate);
  if (warehouseModule)
  ...
  // if it is a supply center, I must have boxes, and must be controlled by the
  // same player (not merely an ally... otherwise you may find yourself funding
  // your allies. ick.)
  static const NameKeyType key_centerUpdate = NAMEKEY("SupplyCenterDockUpdate");
  SupplyCenterDockUpdate *centerModule =
      (SupplyCenterDockUpdate *)transferDest->findUpdateModule(
          key_centerUpdate);
  if (centerModule)
  ...
  if ((warehouseModule == NULL) && (centerModule == NULL)) return FALSE;

  // We do not check ClearToApproach, as it is a temporary failure that is
  // handled in the state logic.  This function is for Legality, not
  // conditionals. however, we DO check for the unit to be available (cf
  // Chinook) (srj)
  if (!supplyTruck->isAvailableForSupplying()) return FALSE;
  ...
  if (objPlayer) {
    if (objPlayer->getPlayerType() == PLAYER_HUMAN &&
        transferDest->getShroudedStatus(objPlayer->getPlayerIndex()) ==
...
Bool ActionManager::canDockAt(const Object *obj, const Object *dockDest,
  ...
  for (BehaviorModule **u = dockDest->getBehaviorModules(); *u; ++u) {
    if ((di = (*u)->getDockUpdateInterface()) != NULL) break;
  }
  if (di == NULL)
  ...
  */
  ...
  if (canTransferSuppliesAt(obj, dockDest) == TRUE) return TRUE;

  // units and infantry can dock with a railed transport
  static const NameKeyType key = NAMEKEY("RailedTransportDockUpdate");
  RailedTransportDockUpdate *fdu =
      (RailedTransportDockUpdate *)dockDest->findUpdateModule(key);
  if (fdu) {
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
Bool ActionManager::canGetHealedAt(const Object *obj, const Object *healDest,
                                   CommandSourceType commandSource) {
  // sanity
  if (obj == NULL || healDest == NULL) return FALSE;
  ...
  if (healDest->isEffectivelyDead()) return FALSE;

  // nothing can be done with things that are under construction
  if (BitTest(obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION) == TRUE ||
      BitTest(healDest->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION) ==
          TRUE)
    return FALSE;

  // Can't get healed at something being sold
  if (healDest->testStatus(OBJECT_STATUS_SOLD)) return FALSE;

  // only infantry can go get "healed" somewhere (vehicles get "repaired")
  if (obj->isKindOf(KINDOF_INFANTRY) == FALSE) return FALSE;

  // infantry can only be healed at something that is designated as a heal pad
  if (healDest->isKindOf(KINDOF_HEAL_PAD) == FALSE) return FALSE;

  // if the target is in the shroud, we can't do anything
  if (isObjectShroudedForAction(obj, healDest, commandSource)) return FALSE;
...
Bool ActionManager::canRepairObject(const Object *obj,
                                    ...
                                    CommandSourceType commandSource) {
  // sanity
  if (obj == NULL || objectToRepair == NULL) return FALSE;
  ...
  if (r == ENEMIES) return FALSE;

  //
  // can't repair dead things ... the exception is bridges and bridge towers,
  // which can be destroyed and die, but can be repaired to bring the bring
  // "back to life"
  //
  ...
  if (objectToRepair->isKindOf(KINDOF_BRIDGE) ||
      objectToRepair->isKindOf(KINDOF_BRIDGE_TOWER))
    return FALSE;

  // nothing can be done with things that are under construction
  if (BitTest(obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION) == TRUE ||
      BitTest(objectToRepair->getStatusBits(),
              OBJECT_STATUS_UNDER_CONSTRUCTION) == TRUE)
    return FALSE;

  // we cannot manually repair things that are regeneration holes
  if (objectToRepair->isKindOf(KINDOF_REBUILD_HOLE) == TRUE) return FALSE;

  // only Dozers can go repair things
  if (obj->isKindOf(KINDOF_DOZER) == FALSE) return FALSE;

  // dozers can only repair buildings
  if (objectToRepair->isKindOf(KINDOF_STRUCTURE) == FALSE) return FALSE;
  ...
  if (obj->getContainedBy()) {
    // We can't heal things while in a transport (especially our own transport,
    // you cheater)
    return FALSE;
  }
...
Bool ActionManager::canResumeConstructionOf(
    const Object *obj, const Object *objectBeingConstructed,
    CommandSourceType commandSource) {
  // sanity
  if (obj == NULL || objectBeingConstructed == NULL) return FALSE;
  ...
  // if the objectBeingConstructed is not actually under construction we can't
  // resume that!
  ...
  //
  // if the object being constructed is actively being built by another dozer we
  // cannot add more effectiveness to the construction so we'll say no (this
  // might change for workers in the future)
  //
  Object *builder =
      TheGameLogic->findObjectByID(objectBeingConstructed->getBuilderID());
  if (builder) {
    ...
    if (ai) {
      ...
      DEBUG_ASSERTCRASH(
          dozerAI, ("Builder object doest not have a DozerAI interface!\n"));

      if (dozerAI) {
        if (dozerAI->getCurrentTask() == DOZER_TASK_BUILD &&
            dozerAI->getTaskTarget(DOZER_TASK_BUILD) ==
                objectBeingConstructed->getID())
          return FALSE;
  ...
  //
  // all is well, the objectBeingConstructeds' builder object has gone away or
  // is no longer building the object anymore
  //
...
Bool ActionManager::canEnterObject(const Object *obj,
                                   ...
                                   CanEnterType mode) {
  // sanity
  if (obj == NULL || objectToEnter == NULL) return FALSE;
  ...
  if (objectToEnter->isEffectivelyDead()) return FALSE;
  ...
  // nothing can be done with things that are under construction
  if (BitTest(obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION) == TRUE ||
      BitTest(objectToEnter->getStatusBits(),
              OBJECT_STATUS_UNDER_CONSTRUCTION) == TRUE) {
    return FALSE;
  }

  // Can't enter something being sold
  if (objectToEnter->testStatus(OBJECT_STATUS_SOLD)) return FALSE;
  if (obj->isKindOf(
          KINDOF_IGNORED_IN_GUI)  // As in, Angry Mob Members, Cargo Planes
  ...
  // Special case for unmanned vehicles. Any infantry unit can take over any
  // unmanned vehicle!
  if (obj->isKindOf(KINDOF_INFANTRY) &&
      objectToEnter->isDisabledByType(DISABLED_UNMANNED)) {
    return TRUE;
  }

  // Special case for aircraft.
  if (obj->isKindOf(KINDOF_AIRCRAFT) &&
      objectToEnter->isKindOf(KINDOF_AIRFIELD)) {
    if (!obj->isAboveTerrain()) return FALSE;

    if (obj->getControllingPlayer() == objectToEnter->getControllingPlayer()) {
      // Kris -- added code to prevent aircraft from landing in any airstrips
      ...
      for (BehaviorModule **i = objectToEnter->getBehaviorModules(); *i; ++i) {
        ...
        if (pp->hasReservedSpace(obj->getID())) return TRUE;

        if (pp->shouldReserveDoorWhenQueued(obj->getTemplate()) &&
            pp->hasAvailableSpaceFor(obj->getTemplate()))
          return TRUE;
      }
    }
    return FALSE;
  ...
  for (BehaviorModule **m = obj->getBehaviorModules(); *m; ++m) {
    ...
    if (collide->wouldLikeToCollideWith(objectToEnter)) {
      // I thought this was a little confusing that it would return TRUE here
      // before getting to any of the other checks. The key is that it usually
      // doesn't return TRUE because most things aren't trying to collide with
      // objects. This is different for terrorist converting carbombs, and
      // pilots entering vehicles. In these cases, the vehicles don't have
      // transport capacities, therefore returning true here foregoes that
      // checking later on.
      return TRUE;
    }
  }
  ...
  if (mode == COMBATDROP_INTO) {
    // we don't care about valid container-ness, but we DO care about faction
    // structures... we aren't allowed to combat drop into them
    if (objectToEnter->isFactionStructure()) return FALSE;
  } else {
    ...
    if (objectToEnter->getControllingPlayer() != obj->getControllingPlayer()) {
      ...
      if (nonStealthContainCount > 0) return FALSE;
    ...
    /// @todo srj -- seems like we should check always (not just for checkCap),
    /// but scared to change now -- check later
...
CanAttackResult ActionManager::getCanAttackObject(
    const Object *obj, const Object *objectToAttack,
  ...
  // has any weapons that are capable of inflicting damage. Special damage types
  // are rejected such as hack weapons... others can be added.
  ...
  // Special case code for stinger sites: Stinger sites have no weapons, instead
  // -- their spawns are the weapons, in this case the stinger soldiers.
  if (obj->isKindOf(KINDOF_SPAWNS_ARE_THE_WEAPONS)) {
    ...
    if (spawnInterface) {
      ...
      Object *slave =
          spawnInterface->getClosestSlave(objectToAttack->getPosition());
...
Bool ActionManager::canConvertObjectToCarBomb(const Object *obj,
  ...
  for (BehaviorModule **m = obj->getBehaviorModules(); *m; ++m) {
    ...
    if (collide->wouldLikeToCollideWith(objectToConvert) &&
        collide->isCarBombCrateCollide()) {
      return TRUE;
    }
  }
  return FALSE;
...
Bool ActionManager::canHijackVehicle(
    const Object *obj, const Object *objectToHijack,
    CommandSourceType commandSource)  // LORENZEN
{
  ...
  for (BehaviorModule **m = obj->getBehaviorModules(); *m; ++m) {
    ...
    if (collide->wouldLikeToCollideWith(objectToHijack) &&
        collide->isHijackedVehicleCrateCollide()) {
      return TRUE;
    }
  }
...
Bool ActionManager::canMakeObjectDefector(
    const Object *obj, const Object *objectToMakeDefector,
    CommandSourceType commandSource)  // LORENZEN
{
  // sanity
...
Bool ActionManager::canCaptureBuilding(const Object *obj,
  ...
  //	{
  //		SpecialPowerModuleInterface *disableSPI =
  //obj->findSpecialPowerModuleInterface(
  //SPECIAL_BLACKLOTUS_DISABLE_VEHICLE_HACK ); 		if ( disableSPI &&
  //disableSPI->isBusy() ) 			return FALSE; 		SpecialPowerModuleInterface *cashSPI =
  //obj->findSpecialPowerModuleInterface( SPECIAL_BLACKLOTUS_STEAL_CASH_HACK );
  //		if ( cashSPI && cashSPI->isBusy() )
  //			return FALSE;
  //	}

  SpecialPowerModuleInterface *spInterface =
      obj->findSpecialPowerModuleInterface(SPECIAL_INFANTRY_CAPTURE_BUILDING);
  if (!spInterface)
    spInterface = obj->findSpecialPowerModuleInterface(
        SPECIAL_BLACKLOTUS_CAPTURE_BUILDING);
  if (!spInterface) return false;
  ...
  // the player into thinking it isn't actually an enemy.
  if (appearsToContainFriendlies(obj, objectToCapture)) return FALSE;
...
Bool ActionManager::canDisableVehicleViaHacking(const Object *obj,
  ...
  // sanity
  if (obj == NULL || objectToHack == NULL) return FALSE;

  if (checkSourceRequirements) {
    // Make sure our object has the capability of performing this special
    // ability.
  ...
  //  This is the althernate way to one-at-a-time BlackLotus' specials; we'll
  //  keep it commented her until Dustin decides, or until 12/10/02
  //	SpecialPowerModuleInterface *captureSPI =
  //obj->findSpecialPowerModuleInterface( SPECIAL_BLACKLOTUS_CAPTURE_BUILDING );
  //	if ( captureSPI && captureSPI->isBusy() )
  //		return FALSE;
  //	SpecialPowerModuleInterface *cashSPI =
  //obj->findSpecialPowerModuleInterface( SPECIAL_BLACKLOTUS_STEAL_CASH_HACK );
  //	if ( cashSPI && cashSPI->isBusy() )
  //		return FALSE;
  ...
  if (objectToHack->isKindOf(KINDOF_AIRCRAFT) ||
      objectToHack->isAirborneTarget()) {
    return false;
  }

  // if the target is in the shroud, we can't do anything
  if (isObjectShroudedForAction(obj, objectToHack, commandSource)) return FALSE;
  ...
  if (r == ENEMIES) {
    ...
    // Also check if the object is a container containing stealth units tricking
    // the player into thinking it isn't actually an enemy.
    if (appearsToContainFriendlies(obj, objectToHack)) return FALSE;
...
// ------------------------------------------------------------------------------------------------
Bool ActionManager::canPickUpPrisoner(const Object *obj, const Object *prisoner,
                                      CommandSourceType commandSource) {
  // sanity
  if (obj == NULL || prisoner == NULL) return FALSE;

  // only pow trucks can pick up anything
  if (obj->isKindOf(KINDOF_POW_TRUCK) == FALSE) return FALSE;

  // only infantry can be picked up
  if (prisoner->isKindOf(KINDOF_INFANTRY) == FALSE) return FALSE;

  // prisoner cannot be contained inside anything
  if (prisoner->getContainedBy()) return FALSE;

  // prisoner must be in a surrendered state
  const AIUpdateInterface *ai = prisoner->getAI();
  if (ai == NULL || ai->isSurrendered() == FALSE) return FALSE;
  ...
  Player *surrenderedToPlayer =
      (idx >= 0) ? ThePlayerList->getNthPlayer(idx) : NULL;
  if (surrenderedToPlayer != NULL &&
      surrenderedToPlayer != obj->getControllingPlayer())
    return FALSE;

  // we must be enemies
  if (obj->getRelationship(prisoner) != ENEMIES) return FALSE;
...
Bool ActionManager::canStealCashViaHacking(const Object *obj,
  ...
  //obj->findSpecialPowerModuleInterface(
  //SPECIAL_BLACKLOTUS_DISABLE_VEHICLE_HACK ); 	if ( disableSPI &&
  //disableSPI->isBusy() ) 		return FALSE;
  ...
  if (BitTest(objectToHack->getStatusBits(),
              OBJECT_STATUS_UNDER_CONSTRUCTION) == TRUE) {
    return FALSE;
  }
  ...
  if (r == ENEMIES) {
    ...
    if (!objectToHack->isKindOf(KINDOF_CAPTURABLE) ||
        objectToHack->isKindOf(KINDOF_REBUILD_HOLE)) {
      return FALSE;
    }
...
Bool ActionManager::canDisableBuildingViaHacking(
    const Object *obj, const Object *objectToHack,
    CommandSourceType commandSource) {
  // sanity
  if (obj == NULL || objectToHack == NULL) return FALSE;
  ...
  SpecialPowerModuleInterface *spInterface =
      obj->findSpecialPowerModuleInterface(SPECIAL_HACKER_DISABLE_BUILDING);
  if (!spInterface || spInterface->getPercentReady() < 1.0f) {
    // Special not ready or non-existent.
    return FALSE;
  }
  ...
  // Make sure the building is considered hackable (temp: using capturable)
  //  An exception is any TechFactionBuilding that is not explicitly immune to
  //  capture
  if ((!objectToHack->isKindOf(KINDOF_CAPTURABLE) ||
       objectToHack->isKindOf(KINDOF_REBUILD_HOLE)) &&
      !(objectToHack->isKindOf(KINDOF_FS_TECHNOLOGY) &&
        !objectToHack->isKindOf(KINDOF_IMMUNE_TO_CAPTURE))) {
    return FALSE;
  }

  if (objectToHack->isKindOf(KINDOF_REBUILD_HOLE) ||
      objectToHack->testStatus(OBJECT_STATUS_UNDER_CONSTRUCTION))
    return FALSE;
...
// ------------------------------------------------------------------------------------------------
Bool ActionManager::canBribeUnit(const Object *obj, const Object *objectToBribe,
                                 CommandSourceType commandSource) {
  return FALSE;
}
...
Bool ActionManager::canDoSpecialPowerAtLocation(
    const Object *obj, const Coord3D *loc, CommandSourceType commandSource,
    const SpecialPowerTemplate *spTemplate, const Object *objectInWay,
  ...
  if (mod) {
    ...
    // First check terrain type, if it is cared about.  Don't return a true,
    // since there are more checks.
    switch (spTemplate->getSpecialPowerType()) {
      case SPECIAL_PARADROP_AMERICA:
      case SPECIAL_CRATE_DROP: {
        if (TheTerrainLogic->isUnderwater(loc->x, loc->y)) return FALSE;
      }
    }
    ...
    switch (spTemplate->getSpecialPowerType()) {
      ...
      case SPECIAL_CLEANUP_AREA:
        // Don't allow "damaging" special powers in shrouded areas, but Fogged
        // are okay.
        return ThePartitionManager->getShroudStatusForPlayer(
...
Bool ActionManager::canDoSpecialPowerAtObject(
    ...
    const SpecialPowerTemplate *spTemplate, UnsignedInt commandOptions,
    Bool checkSourceRequirements) {
  if (checkSourceRequirements) {
    // First check, if our object can do this special power.
    if (!obj->hasSpecialPower(spTemplate->getSpecialPowerType())) {
    ...
    switch (spTemplate->getSpecialPowerType()) {
      ...
      case SPECIAL_TANKHUNTER_TNT_ATTACK:
        if (target->isKindOf(KINDOF_STRUCTURE) ||
            (target->isKindOf(KINDOF_VEHICLE) &&
      ...
      case SPECIAL_HACKER_DISABLE_BUILDING:
        ...
        if (target->isKindOf(KINDOF_STRUCTURE) && r == ENEMIES) {
          // Make sure the building is considered hackable (temp: using
          // capturable)
          if (!target->isKindOf(KINDOF_CAPTURABLE) ||
              target->isKindOf(KINDOF_REBUILD_HOLE)) {
            return FALSE;
          }
          return true;
        }
      ...
      case SPECIAL_CASH_HACK:
        ...
        if (target->isKindOf(KINDOF_STRUCTURE) && r == ENEMIES) {
          ...
          if (BitTest(target->getStatusBits(),
      ...
      case SPECIAL_DISGUISE_AS_VEHICLE:
        if (target->isKindOf(KINDOF_VEHICLE) &&
          ...
          // Don't allow it to disguise as another bomb truck -- that's just
          // plain dumb. if( target->getTemplate() != obj->getTemplate() )
          {
            // Don't allow it to disguise as a train -- they don't have
            // KINDOF_TRAIN yet, but if added, please change this code so it'll
            // be faster!
            static const NameKeyType key = NAMEKEY("RailroadBehavior");
            RailroadBehavior *rBehavior =
                (RailroadBehavior *)target->findUpdateModule(key);
            if (!rBehavior) {
              return true;
            }
          }
      ...
      case SPECIAL_DEFECTOR:
        ...
        if (!target->isKindOf(KINDOF_STRUCTURE)) {
          // only attacking type things will defect (no dozers, supply trucks,
          // workers...)
          // srj sez: I don't know why this is commented out, but it should
          // remain thus, because it is not necessarily the case that dozers,
          // workers, etc. cannot attack; they may "attack" Mines to disarm
          // them...
      ...
      case SPECIAL_REMOTE_CHARGES:
      case SPECIAL_TIMED_CHARGES: {
        if (target->isEffectivelyDead() || target->isKindOf(KINDOF_BRIDGE) ||
        ...
        if (target->isKindOf(KINDOF_STRUCTURE) ||
          ...
          SpecialAbilityUpdate *spUpdate =
              obj->findSpecialAbilityUpdate(spTemplate->getSpecialPowerType());
          if (spUpdate) {
            // Make sure we have enough equipment to place an additional charge.
            if (spUpdate->getSpecialObjectCount() <
                spUpdate->getSpecialObjectMax()) {
              // Also restrict the unit from placing more than one charge on the
              // same building. We accomplish this by having the stickybomb
              // update store the target as the producer ID.
              if (spUpdate->findSpecialObjectWithProducerID(target)) {
                return false;
              }

              // HERE'S THE TOUGH CASE...
              // We also don't want to allow a unit that can place timed charges
              // on a building to be able to place remote charges (or
              // vice-versa). So we're going to look for the other special
              // ability update and reject if the other one has it planted...
              if (spTemplate->getSpecialPowerType() == SPECIAL_REMOTE_CHARGES) {
                spUpdate = obj->findSpecialAbilityUpdate(SPECIAL_TIMED_CHARGES);
              } else if (spTemplate->getSpecialPowerType() ==
                         SPECIAL_TIMED_CHARGES) {
                spUpdate =
                    obj->findSpecialAbilityUpdate(SPECIAL_REMOTE_CHARGES);
              }

              // If we have a valid pointer at this point, we found the other
              // special. Make sure it isn't planted on the same target.
              if (spUpdate &&
                  spUpdate->findSpecialObjectWithProducerID(target)) {
                return false;
              }
...
Bool ActionManager::canFireWeaponAtObject(const Object *obj,
  ...
  CanAttackResult result = obj->getAbleToAttackSpecificObject(
      ATTACK_NEW_TARGET, target, commandSource);
...
//------------------------------------------------------------------------------------------------
Bool ActionManager::canFireWeapon(const Object *obj, const WeaponSlotType slot,
                                  CommandSourceType commandSource) {
  // Sanity check
  if (obj == NULL) {
    return false;
...
Bool ActionManager::canOverrideSpecialPowerDestination(
    const Object *obj, const Coord3D *loc, SpecialPowerType spType,
    CommandSourceType commandSource) {
  SpecialPowerUpdateInterface *spuInterface =
      obj->findSpecialPowerWithOverridableDestinationActive(spType);
  if (spuInterface) {
