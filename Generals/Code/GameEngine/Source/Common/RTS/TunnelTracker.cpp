...
// /////////////////////////////////////////////////////////// The part of a
// Player's brain that holds the communal Passenger list of all tunnels. Author:
// Graham Smallwood, March, 2002
...
// ------------------------------------------------------------------------
TunnelTracker::~TunnelTracker() { m_tunnelIDs.clear(); }

// ------------------------------------------------------------------------
void TunnelTracker::iterateContained(ContainIterateFunc func, void *userData,
                                     Bool reverse) {
  if (reverse) {
...
Object *TunnelTracker::getCurNemesis(void) {
  ...
  if (m_nemesisTimestamp + 4 * LOGICFRAMES_PER_SECOND <
...
Bool TunnelTracker::isValidContainerFor(const Object *obj,
  ...
  // October 11, 2002 -- Kris : Dustin wants ALL units to be able to use
  // tunnels!
...
void TunnelTracker::removeFromContain(Object *obj, Bool exposeStealthUnits) {
  ContainedItemsList::iterator it =
      std::find(m_containList.begin(), m_containList.end(), obj);
...
Bool TunnelTracker::isInContainer(Object *obj) {
  return (std::find(m_containList.begin(), m_containList.end(), obj) !=
          m_containList.end());
}
...
void TunnelTracker::onTunnelDestroyed(const Object *deadTunnel) {
  ...
  if (m_tunnelCount == 0) {
    ...
    // Otherwise, make sure nobody inside remembers the dead tunnel as the one
    // they entered (scripts need to use so there must be something valid here)
    for (ContainedItemsList::iterator it = m_containList.begin();
         it != m_containList.end();) {
      Object *obj = *it;
      ++it;
      if (obj->getContainedBy() == deadTunnel) obj->onContainedBy(validTunnel);
    }
  }
}
...
void TunnelTracker::destroyObject(Object *obj, void *) {
  // Now that tunnels consider ContainedBy to be "the tunnel you entered", I
  // need to say goodbye llike other contain types so they don't look us up on
  // their deletion and crash
  obj->onRemovedFrom(obj->getContainedBy());
  TheGameLogic->destroyObject(obj);
}

// ------------------------------------------------------------------------
// heal all the objects within the tunnel system using the iterateContained
// function
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void TunnelTracker::crc(Xfer *xfer) {}  // end crc
...
void TunnelTracker::loadPostProcess(void) {
  ...
  if (m_containList.size() != 0) {
    DEBUG_CRASH(
        ("TunnelTracker::loadPostProcess - m_containList should be empty but "
         "is not\n"));
    throw SC_INVALID_DATA;

  }  // end if

  // translate each object ids on the xferContainList into real object pointers
  // in the contain list
  Object *obj;
  ...
  for (it = m_xferContainList.begin(); it != m_xferContainList.end(); ++it) {
    ...
    if (obj == NULL) {
      DEBUG_CRASH(
          ("TunnelTracker::loadPostProcess - Unable to find object ID '%d'\n",
           *it));
      throw SC_INVALID_DATA;
    ...
    // Crap.  This is in OpenContain as a fix, but not here.
    {
      ...
      if (TheAI) TheAI->pathfinder()->removeObjectFromPathfindMap(obj);
    }
  }  // end for, it
