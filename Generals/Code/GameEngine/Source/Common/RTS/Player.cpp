...
static void findClosestKindOf(Object *obj, void *userData) {
  ...
  Real distSq = ThePartitionManager->getDistanceSquared(closestData->m_source,
                                                        obj, FROM_CENTER_2D);
...
#define CRCDUMPBATTLEPLANBONUSES(x, y, z) \
  dumpBattlePlanBonuses(x, #x, y, z, __FILE__, __LINE__, FALSE)
#define DUMPBATTLEPLANBONUSES(x, y, z) \
  dumpBattlePlanBonuses(x, #x, y, z, __FILE__, __LINE__, TRUE)
AsciiString kindofMaskAsAsciiString(KindOfMaskType m) {
  ...
  for (Int i = KINDOF_FIRST; i < KINDOF_COUNT; ++i) {
    if (m.test(i)) {
      if (s.isNotEmpty()) s.concat("|");
      s.concat(kindofNames[i]);
    }
  }
  if (s.isEmpty()) s = "KINDOF_INVALID";
  return s;
}
void dumpBattlePlanBonuses(const BattlePlanBonuses *b, AsciiString name,
                           const Player *p, const Object *o, AsciiString fname,
                           Int line, Bool doDebugLog) {
  CRCDEBUG_LOG(
      ("dumpBattlePlanBonuses() %s:%d %s\n  Player %d(%ls) object %d(%s) "
       "armor:%g/%8.8X bombardment:%d, holdTheLine:%d, searchAndDestroy:%d "
       "sight:%g/%8.8X, valid:%s invalid:%s\n",
       fname.str(), line, name.str(), (p) ? p->getPlayerIndex() : -1,
       (p) ? ((Player *)p)->getPlayerDisplayName().str() : L"<No Name>",
       (o) ? o->getID() : -1,
       (o) ? o->getTemplate()->getName().str() : "<No Name>", b->m_armorScalar,
       AS_INT(b->m_armorScalar), b->m_bombardment, b->m_holdTheLine,
       b->m_searchAndDestroy, b->m_sightRangeScalar,
       AS_INT(b->m_sightRangeScalar),
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
PlayerRelationMap::PlayerRelationMap(void) {}  // end PlayerRelationMap
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void PlayerRelationMap::crc(Xfer *xfer) {}  // end crc
...
void PlayerRelationMap::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (playerRelationIt = m_map.begin(); playerRelationIt != m_map.end();
         ++playerRelationIt) {
      // write player index
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void PlayerRelationMap::loadPostProcess(void) {}  // end loadPostProcess
...
void Player::init(const PlayerTemplate *pt) {
  DEBUG_ASSERTCRASH(
      m_playerTeamPrototypes.size() == 0,
      ("Player::m_playerTeamPrototypes is not empty at game start!\n"));
  m_skillPointsModifier = 1.0f;
  m_attackedFrame = 0;
  ...
  if (pt) {
    ...
    // neutral is always "allied" with self -- this is the only thing ever
    // allied with neutral!
...

  {
    SpecialPowerReadyTimerListIterator it =
        m_specialPowerReadyTimerList.begin();
  ...
  KindOfPercentProductionChangeListIt it =
      m_kindOfPercentProductionChangeList.begin();
...
Player::~Player() {
  ...
  // delete the relation maps (the destructor clears the actual map if any data
  // is present)
...
Relationship Player::getRelationship(const Team *that) const {
  ...
  if (that) {
    ...
    if (!m_teamRelations->m_map.empty()) {
      TeamRelationMapType::const_iterator it =
          m_teamRelations->m_map.find(that->getID());
    ...
    if (!m_playerRelations->m_map.empty()) {
      ...
      if (thatPlayer != NULL) {
        PlayerRelationMapType::const_iterator it =
            m_playerRelations->m_map.find(thatPlayer->getPlayerIndex());
...
Bool Player::removeTeamRelationship(const Team *that) {
  if (!m_teamRelations->m_map.empty()) {
    if (that == NULL) {
      ...
      TeamRelationMapType::iterator it =
          m_teamRelations->m_map.find(that->getID());
      if (it != m_teamRelations->m_map.end()) {
...
//=============================================================================
void Player::addToPriorityBuildList(AsciiString templateName, Coord3D *pos,
                                    Real angle) {
  BuildListInfo *newInfo = newInstance(BuildListInfo);
...
void Player::initFromDict(const Dict *d) {
  ...
  const PlayerTemplate *pt =
      ThePlayerTemplateStore->findPlayerTemplate(NAMEKEY(tmplname));
  DEBUG_ASSERTCRASH(pt != NULL,
                    ("PlayerTemplate %s not found -- this is an obsolete map "
                     "(please open and resave in WB)\n",
                     tmplname.str()));
  ...
  if (d->getBool(TheKey_playerIsSkirmish, &exists)) {
    // srj sez: check to ensure the map actually has a skirmish player...
    // ordinarily it should, but poorly-formed user maps might not, which would
    // be bad, and could crash us later. so if it doesn't actually have a
    // skirmish player defined for this side, declare it nonskirmish... the
    // player won't really work, but it's better than crashing.
    for (Int spIdx = 0; spIdx < TheSidesList->getNumSkirmishSides(); ++spIdx) {
      AsciiString spTemplateName =
          TheSidesList->getSkirmishSideInfo(spIdx)->getDict()->getAsciiString(
              TheKey_playerFaction);
      const PlayerTemplate *spt =
          ThePlayerTemplateStore->findPlayerTemplate(NAMEKEY(spTemplateName));
    ...
    DEBUG_ASSERTCRASH(skirmish, ("Could not find skirmish player for side "
                                 "%s... quietly making into nonskirmish.",
                                 getSide().str()));
    ...
    if (TheSidesList->getNumSkirmishSides() > 0) {
      ...
      for (i = 0; i < TheSidesList->getNumSkirmishSides(); i++) {
        AsciiString templateName =
            TheSidesList->getSkirmishSideInfo(i)->getDict()->getAsciiString(
                TheKey_playerFaction);
        pt = ThePlayerTemplateStore->findPlayerTemplate(NAMEKEY(templateName));
        if (pt && pt->getSide() == mySide) {
          qualTemplatePlayerName.format("%s%d",
                                        TheSidesList->getSkirmishSideInfo(i)
                                            ->getDict()
                                            ->getAsciiString(TheKey_playerName)
                                            .str(),
                                        m_mpStartIndex);
          found = true;
          break;
      ...
      if (found && TheSidesList->getSkirmishSideInfo(i)->getScriptList()) {
        ...
        ScriptList *scripts =
            TheSidesList->getSkirmishSideInfo(i)
                ->getScriptList()
                ->duplicateAndQualify(qualifier, qualTemplatePlayerName, pname);
        if (TheSidesList->getSideInfo(getPlayerIndex())->getScriptList()) {
          TheSidesList->getSideInfo(getPlayerIndex())
              ->getScriptList()
              ->deleteInstance();
        }
  ...
  if (skirmish) {
    ...
    Bool found = false;
    AsciiString qualTemplatePlayerName;
    for (skirmishNdx = 0; skirmishNdx < TheSidesList->getNumSkirmishSides();
         skirmishNdx++) {
      AsciiString templateName = TheSidesList->getSkirmishSideInfo(skirmishNdx)
                                     ->getDict()
                                     ->getAsciiString(TheKey_playerFaction);
      pt = ThePlayerTemplateStore->findPlayerTemplate(NAMEKEY(templateName));
      if (pt && pt->getSide() == mySide) {
        qualTemplatePlayerName.format(
            "%s%d",
            TheSidesList->getSkirmishSideInfo(skirmishNdx)
                ->getDict()
                ->getAsciiString(TheKey_playerName)
                .str(),
            m_mpStartIndex);
    ...
    if (!found) {
      ...
      ScriptList *scripts =
          TheSidesList->getSkirmishSideInfo(skirmishNdx)
              ...
              ->duplicateAndQualify(qualifier, qualTemplatePlayerName, pname);
      ScriptList *slist =
          TheSidesList->getSideInfo(getPlayerIndex())->getScriptList();
      if (slist) {
        slist->deleteInstance();
      }
      TheSidesList->getSideInfo(getPlayerIndex())->setScriptList(scripts);
      for (i = 0; i < TheSidesList->getNumTeams(); i++) {
        if (TheSidesList->getTeamInfo(i)->getDict()->getAsciiString(
                TheKey_teamOwner) == pname) {
      ...
      AsciiString originalPlayerName =
          TheSidesList->getSkirmishSideInfo(skirmishNdx)
              ->getDict()
              ->getAsciiString(TheKey_playerName);
      for (i = 0; i < TheSidesList->getNumSkirmishTeams(); i++) {
        if (TheSidesList->getSkirmishTeamInfo(i)->getDict()->getAsciiString(
                TheKey_teamOwner) == originalPlayerName) {
          Dict teamDict(*TheSidesList->getSkirmishTeamInfo(i)->getDict());
          AsciiString teamName = teamDict.getAsciiString(TheKey_teamName);
          AsciiString newName;
          newName.format("%s%d", teamDict.getAsciiString(TheKey_teamName).str(),
                         m_mpStartIndex);
          ...
          NameKeyType nameKeys[] = {
              TheKey_teamOnCreateScript,        TheKey_teamOnIdleScript,
              TheKey_teamOnUnitDestroyedScript, TheKey_teamOnDestroyedScript,
              TheKey_teamEnemySightedScript,    TheKey_teamAllClearScript,
          ...
          // Now do the TheKey_teamGenericScriptHookN (where N can be from 0
          // to 15.)
          for (j = 0; j < MAX_GENERIC_SCRIPTS; ++j) {
            AsciiString keyName;
            keyName.format(
                "%s%d",
                TheNameKeyGenerator->keyToName(TheKey_teamGenericScriptHook)
                    .str(),
                j);
  ...
  /// @todo Ack!  the todo in PlayerList::reset() mentioning the need for a
  /// Player::reset() really needs to get done.
  m_playerRelations->m_map.clear();  // For now, it has been decided to just fix
                                     // this one.  Dear god me must reset.
  m_teamRelations->m_map.clear();    // For now, it has been decided to just fix
                                     // this one.  Dear god me must reset.

  Int i;
  for (i = 0; i < MAX_PLAYER_COUNT;
       ++i)  // For now, it has been decided to just fix this one.  Dear god me
             // must reset.
  {
    m_attackedBy[i] = false;
    m_visionSpiedBy[i] = 0;
...
void Player::becomingTeamMember(Object *obj, Bool yes) {
  ...
  // energy production/consumption hooks, note we ignore things that are
  // UNDER_CONSTRUCTION
  ...
  // when we capture a building, we need to see if there's an AutoDepositUpdate
  // hooked to it, if so, award the cash bonus
  if (this != ThePlayerList->getNeutralPlayer() && yes) {
    NameKeyType key_AutoDepositUpdate = NAMEKEY("AutoDepositUpdate");
    AutoDepositUpdate *adu =
        (AutoDepositUpdate *)obj->findUpdateModule(key_AutoDepositUpdate);
    if (adu != NULL) {
  ...
  if (obj->isKindOf(KINDOF_DOZER) && obj->getAIUpdateInterface() &&
...
void Player::becomingLocalPlayer(Bool yes) {
  if (yes) {
    // This changes the color of the little dot on the upper right side of the
    // screen indicating which team is under control.
    if (TheGameClient) {
      RGBColor rgb;
      rgb.setFromInt(m_color);
      TheGameClient->setTeamColor(REAL_TO_INT(rgb.red * 255),
                                  REAL_TO_INT(rgb.green * 255),
                                  REAL_TO_INT(rgb.blue * 255));
    }
    ...
    if (ThePartitionManager) {
      ...
      for (Object *object = iter->first(); object; object = iter->next()) {
        // Added support for updating the perceptions of garrisoned buildings
        // containing enemy stealth units. When changing teams, it is necessary
        ...
        if (object->isKindOf(KINDOF_DISGUISER)) {
          // KM -- August 2002:
          // Added support for disguised objects, based on relationships, we
          // either show the real color or the disguised color.
          Drawable *draw = object->getDrawable();
          if (draw) {
            static NameKeyType key_StealthUpdate = NAMEKEY("StealthUpdate");
            StealthUpdate *update =
                (StealthUpdate *)object->findUpdateModule(key_StealthUpdate);
            if (update && update->isDisguised()) {
              Player *disguisedPlayer = ThePlayerList->getNthPlayer(
                  update->getDisguisedPlayerIndex());
              if (getRelationship(object->getTeam()) != ALLIES &&
                  isPlayerActive()) {
                // Neutrals and enemies will see this disguised unit as the team
                // it's disguised as.
                if (TheGlobalData->m_timeOfDay == TIME_OF_DAY_NIGHT)
                  draw->setIndicatorColor(
                      disguisedPlayer->getPlayerNightColor());
                else
                  draw->setIndicatorColor(disguisedPlayer->getPlayerColor());
              } else {
                // Otherwise, the color will show up as the team it really
                // belongs to.
                if (TheGlobalData->m_timeOfDay == TIME_OF_DAY_NIGHT)
 ...
 */
void Player::computeSuperweaponTarget(const SpecialPowerTemplate *power,
                                      Coord3D *retPos, Int playerNdx,
                                      Real weaponRadius) {
  if (m_ai) {
...
static void doFindCommandCenter(Object *obj, void *userData) {
  ...
  if (info->cmdCenter == NULL && obj->isKindOf(KINDOF_COMMANDCENTER) &&
...
void Player::onUnitCreated(Object *factory, Object *unit) {
  ...
  if (m_ai) m_ai->onUnitProduced(factory, unit);
#endif
}  // end onUnitCreated
...
Bool Player::isSupplySourceSafe(Int minSupplies) {
  // ai query
  if (m_ai) return m_ai->isSupplySourceSafe(minSupplies);
  return true;
}  // isSupplySourceSafe
...
Bool Player::isSupplySourceAttacked(void) {
  // ai query
  if (m_ai) return m_ai->isSupplySourceAttacked();
  return false;
}  // isSupplySourceSafe
...
void Player::setTeamDelaySeconds(Int delay) {
  // ai action
  if (m_ai) m_ai->setTeamDelaySeconds(delay);
}  // guardSupplyCenter
...
void Player::guardSupplyCenter(Team *team, Int minSupplies) {
  // ai action
  if (m_ai) m_ai->guardSupplyCenter(team, minSupplies);
}  // guardSupplyCenter
...
void Player::preTeamDestroy(const Team *team) {
  // ai notification callback
  if (m_ai) m_ai->aiPreTeamDestroy(team);
}  // preTeamDestroy
...
//-------------------------------------------------------------------------------------------------
void Player::onStructureConstructionComplete(Object *builder, Object *structure,
                                             Bool isRebuild) {
  ...
  if (isRebuild == FALSE) {
    ...
    m_scoreKeeper.addMoneySpent(
        structure->getTemplate()->calcCostToBuild(this));
  }
...
#if !defined(_PLAYTEST)
  // ai notification callback
  if (m_ai) m_ai->onStructureProduced(builder, structure);
#endif

  // the GUI needs to re-evaluate the information being displayed to the user
  // now
  if (TheControlBar) TheControlBar->markUIDirty();

  // This object may require us to play some EVA sounds.
  Player *localPlayer = ThePlayerList->getLocalPlayer();
  if (localPlayer == structure->getControllingPlayer() ||
      localPlayer->getRelationship(structure->getTeam()) != ENEMIES)
    return;
...
void Player::addTeamToList(TeamPrototype *team) {
  for (PlayerTeamList::const_iterator it = m_playerTeamPrototypes.begin();
       it != m_playerTeamPrototypes.end(); ++it) {
    if (team == *it) return;  // already present
  }
...
void Player::countObjectsByThingTemplate(Int numTmplates,
                                         ...
                                         Bool ignoreDead, Int *counts,
                                         Bool ignoreUnderConstruction) const {
  Int i;

  for (i = 0; i < numTmplates; ++i) counts[i] = 0;

  for (PlayerTeamList::const_iterator it = m_playerTeamPrototypes.begin();
       it != m_playerTeamPrototypes.end(); ++it) {
    (*it)->countObjectsByThingTemplate(numTmplates, things, ignoreDead, counts,
                                       ignoreUnderConstruction);
  }
}
...
//=============================================================================
Object *Player::findClosestByKindOf(Object *queryObject, KindOfMaskType setMask,
...
Bool Player::hasAnyBuildFacility(void) const {
  for (PlayerTeamList::const_iterator it = m_playerTeamPrototypes.begin();
       it != m_playerTeamPrototypes.end(); ++it) {
    if ((*it)->hasAnyBuildFacility()) return true;
  }
  return false;
}
...
//=============================================================================
Bool Player::getListInScoreScreen() { return m_listInScoreScreen; }

//=============================================================================
UnsignedInt Player::getSupplyBoxValue() {
  /// @todo This would be the hookup for difficulty level modifiers and special
  /// economy buildings
  return TheGlobalData->m_baseValuePerSupplyBox;
}

//=============================================================================
Real Player::getProductionCostChangePercent(
    AsciiString buildTemplateName) const {
  ProductionChangeMap::const_iterator it =
      m_productionCostChanges.find(NAMEKEY(buildTemplateName));
...
//=============================================================================
Real Player::getProductionTimeChangePercent(
    AsciiString buildTemplateName) const {
  ProductionChangeMap::const_iterator it =
      m_productionTimeChanges.find(NAMEKEY(buildTemplateName));
...
//=============================================================================
VeterancyLevel Player::getProductionVeterancyLevel(
    AsciiString buildTemplateName) const {
  NameKeyType templateNameKey = NAMEKEY(buildTemplateName);
  ProductionVeterancyMap::const_iterator it =
      m_productionVeterancyLevels.find(templateNameKey);
...
void Player::setUnitsShouldHunt(Bool unitsShouldHunt,
  ...
  Coord3D pos;
  ThePartitionManager->getMostValuableLocation(getPlayerIndex(), ALLOW_ENEMIES,
                                               VOT_CashValue, &pos);
  for (PlayerTeamList::iterator it = m_playerTeamPrototypes.begin();
       it != m_playerTeamPrototypes.end(); ++it) {
    for (DLINK_ITERATOR<Team> iter = (*it)->iterate_TeamInstanceList();
      ...
      for (DLINK_ITERATOR<Object> iterObj = team->iterate_TeamMemberList();
           !iterObj.done(); iterObj.advance()) {
...
void Player::killPlayer(void) {
  ...
  m_isPlayerDead =
      TRUE;  // this is so OCLs don't ever again spawn useful units for us.

  for (it = m_playerTeamPrototypes.begin(); it != m_playerTeamPrototypes.end();
       ++it) {
    for (DLINK_ITERATOR<Team> iter = (*it)->iterate_TeamInstanceList();
         !iter.done(); iter.advance()) {
      Team *team = iter.cur();
  ...
  if (TheGameLogic->isInSinglePlayerGame()) {
    if (getPlayerType() == PLAYER_COMPUTER) {
      // This is an AI player in a solo mission - leave him alive so he can be
      // used later. jba.
      m_isPlayerDead =
          FALSE;  // this is so we can later spawn useful units for us.
      return;
    }
  }
  if (isLocalPlayer() && !TheGameLogic->isInShellGame()) {
    ...
    if (TheControlBar) {
      if (isPlayerActive()) {
        ...
        TheControlBar->setControlBarSchemeByPlayerTemplate(
            ThePlayerTemplateStore->findPlayerTemplate(
                NAMEKEY("FactionObserver")));
      }
    }
  }
...
void Player::transferAssetsFromThat(Player *that) {
  ...
  for (PlayerTeamList::iterator it = that->m_playerTeamPrototypes.begin();
    ...
    for (DLINK_ITERATOR<Team> iter = (*it)->iterate_TeamInstanceList();
      ...
      for (DLINK_ITERATOR<Object> iterObj = team->iterate_TeamMemberList();
           !iterObj.done(); iterObj.advance()) {
        Object *obj = iterObj.cur();
        if (!obj || obj->getTemplate()->isEquivalentTo(
                        beaconTemplate))  // don't transfer NULL objs or beacons
        {
          continue;
        }
  ...
  for (std::list<Object *>::iterator itObjs = objsToTransfer.begin();
       itObjs != objsToTransfer.end(); ++itObjs) {
...
void Player::garrisonAllUnits(CommandSourceType source) {
  ...
  /// @todo srj -- we should really use iterateAllObjects() here instead, but I
  /// have no time to
  // test such a change... make someday
  ObjectIterator *iterBuilding = ThePartitionManager->iterateObjectsInRange(
      &pos, 1e9f, FROM_CENTER_3D, filters, ITER_SORTED_NEAR_TO_FAR);
  ...
  for (PlayerTeamList::iterator it = m_playerTeamPrototypes.begin();
    ...
    for (DLINK_ITERATOR<Team> iter = (*it)->iterate_TeamInstanceList();
      ...
      for (DLINK_ITERATOR<Object> iterObj = team->iterate_TeamMemberList();
        ...
        for (Object *theBuilding = iterBuilding->first(); theBuilding;
             theBuilding = iterBuilding->next()) {
          ...
          if (!TheActionManager->canEnterObject(obj, theBuilding, source,
                                                CHECK_CAPACITY)) {
            continue;
          }
  ...
  for (PlayerTeamList::iterator it = m_playerTeamPrototypes.begin();
    ...
    for (DLINK_ITERATOR<Team> iter = (*it)->iterate_TeamInstanceList();
      ...
      for (DLINK_ITERATOR<Object> iterObj = team->iterate_TeamMemberList();
        ...
        // Tell everything that has stuff in it to kick them all out.  You can't
        // tell the people to get out of what they are in, because they may not
        // know, or they may be two transports deep, and their transport may not
        // know where they are. And check for building so you don't unload
        // transports in the ungarrison command
        if (obj->isKindOf(KINDOF_STRUCTURE)) ai->aiEvacuate(FALSE, source);
      }
    }
  }
}
    ...
    for (DLINK_ITERATOR<Team> iter = (*it)->iterate_TeamInstanceList();
      ...
      for (DLINK_ITERATOR<Object> iterObj = team->iterate_TeamMemberList();
        ...
        if (idle) {
          ...
          // Here is the special bit for this exit style, force wanting on
          // SupplyTruck types
          if (ai->isIdle()) {
            SupplyTruckAIInterface *supplyTruckAI =
                ai->getSupplyTruckAIInterface();
            if (supplyTruckAI) supplyTruckAI->setForceWantingState(true);
          }
        }
      }
    }
...
void Player::doBountyForKill(const Object *killer, const Object *victim) {
  ...
  Int costToBuild =
      victim->getTemplate()->calcCostToBuild(victim->getControllingPlayer());
...
Bool Player::addSkillPoints(Int delta) {
  ...
  Int levelCap = min(TheGameLogic->getRankLevelLimit(),
                     TheRankInfoStore->getRankLevelCount());
  Int pointCap = TheRankInfoStore->getRankInfo(levelCap)
                     ->m_skillPointsNeeded;  // Cap at lowest point of cap
                                             // level, not highest.

  Bool levelGained = FALSE;
  m_skillPoints = min(pointCap, (m_skillPoints + delta));
  while (m_skillPoints >= m_levelUp) {
    // LevelUp gets increased as a side effect of setRankLevel, and this won't
    // infinitely loop, because when there are no more levels to be gained,
    // m_levelUp is set to INT_MAX.
...
Bool Player::addSkillPointsForKill(const Object *killer, const Object *victim) {
  ...
  // srj sez: per dustin, no experience (et al) for killing things under
  // construction.
  if (victim->testStatus(OBJECT_STATUS_UNDER_CONSTRUCTION)) return false;
...
void Player::resetSciences() {
  ...
  for (Int i = 1; i <= m_rankLevel; ++i) {
    ...
    if (rank) {
      for (ScienceVec::const_iterator it = rank->m_sciencesGranted.begin();
           it != rank->m_sciencesGranted.end(); ++it) {
        addScience(*it);
      }
    }
  }

  for (ScienceVec::const_iterator it = m_sciences.begin();
       it != m_sciences.end(); ++it)
...
Bool Player::addScience(ScienceType science) {
  if (hasScience(science)) return false;

  // DEBUG_LOG(("Adding Science
      ...
      for (DLINK_ITERATOR<Object> iterObj = team->iterate_TeamMemberList();
        ...
        for (BehaviorModule **m = obj->getBehaviorModules(); *m; ++m) {
          ...
          if (sp->getRequiredScience() == science) {
            // Turn on the power, and set it to be instantly ready, because that
            // is cool.
...
Bool Player::attemptToPurchaseScience(ScienceType science) {
  if (!isCapableOfPurchasingScience(science)) {
    DEBUG_CRASH(
        ("isCapableOfPurchasingScience: need other prereqs/points to purchase, "
         "request is ignored!\n"));
    return false;
  }
  ...
  if (!TheScienceStore->isScienceGrantable(science)) {
    DEBUG_CRASH(
        ("Cannot grant science %s, since it is marked as nonGrantable.\n",
         TheScienceStore->getInternalNameForScience(science).str()));
    return false;  // it's not grantable, so tough, can't have it, even via this
                   // method.
  }
...
void Player::resetRank() {
  ...
  m_levelDown = 0;
  m_sciences.clear();
  m_sciencePurchasePoints =
      getPlayerTemplate()
          ? getPlayerTemplate()->getIntrinsicSciencePurchasePoints()
          : 0;
  const RankInfo *curRank = TheRankInfoStore->getRankInfo(m_rankLevel);
  m_sciencePurchasePoints +=
      curRank ? curRank->m_sciencePurchasePointsGranted : 0;
  m_generalName = TheGameText ? TheGameText->fetch("SCIENCE:GeneralName")
...
Bool Player::setRankLevel(Int newLevel) {
  ...
  if (newLevel == m_rankLevel) return false;
  ...
  DEBUG_ASSERTCRASH(m_skillPoints >= m_levelDown && m_skillPoints < m_levelUp,
                    ("hmm, wrong"));
  // DEBUG_LOG(("Rank %d, Skill %d, down %d, up %d\n",m_rankLevel,m_skillPoints,
  // m_levelDown, m_levelUp));

  if (TheControlBar != NULL) {
    if (m_levelUp) {
      // Play an EVA sound
      if (isLocalPlayer()) TheEva->setShouldPlay(EVA_GeneralLevelUp);
    }
...
Bool Player::isScienceDisabled(ScienceType t) const {
  return std::find(m_sciencesDisabled.begin(), m_sciencesDisabled.end(), t) !=
         m_sciencesDisabled.end();
}

//=============================================================================
Bool Player::isScienceHidden(ScienceType t) const {
  return std::find(m_sciencesHidden.begin(), m_sciencesHidden.end(), t) !=
         m_sciencesHidden.end();
}
...
void Player::setScienceAvailability(ScienceType science,
  ...
  if (type == SCIENCE_DISABLED) {
    ...
    // Do nothing, because being available means that the science isn't in one
    // of the hidden or disabled vecs.
  }
}

//=============================================================================
ScienceAvailabilityType Player::getScienceAvailabilityTypeFromString(
...
Bool Player::canBuild(const ThingTemplate *tmplate) const {
  ...
  if (!allowedToBuild(tmplate)) return false;

  if (tmplate->getBuildable() == BSTATUS_NO) return false;

  if (tmplate->getBuildable() == BSTATUS_IGNORE_PREREQUISITES) return true;

  if (tmplate->getBuildable() == BSTATUS_ONLY_BY_AI &&
  ...
  // else BSTATUS tmplate->getBuildable() == BSTATUS_YES
  {
    ...
    for (Int i = 0; i < tmplate->getPrereqCount(); i++) {
      ...
      if (pre->isSatisfied(this) == false) prereqsOK = false;
    }

#if defined(_DEBUG) || defined(_INTERNAL)
    if (ignoresPrereqs()) prereqsOK = true;
#endif
...
Upgrade *Player::findUpgrade(const UpgradeTemplate *upgradeTemplate) {
  ...
  for (upgrade = m_upgradeList; upgrade; upgrade = upgrade->friend_getNext())
    if (upgrade->getTemplate() == upgradeTemplate) return upgrade;
...
//=================================================================================================
/**
        Does the player have this completed upgrade.  This form is exposed so
   Objects can do quick lookups.
*/
//=================================================================================================
...
Upgrade *Player::addUpgrade(const UpgradeTemplate *upgradeTemplate,
  ...
  if (u == NULL) {
    ...
    if (m_upgradeList) m_upgradeList->friend_setPrev(u);
    m_upgradeList = u;
...
//=================================================================================================
/**
        An upgrade just finished, do things like tell all objects to recheck
   UpgradeModules
*/
  ...
  for (PlayerTeamList::iterator it = m_playerTeamPrototypes.begin();
    ...
    for (DLINK_ITERATOR<Team> iter = (*it)->iterate_TeamInstanceList();
      ...
      for (DLINK_ITERATOR<Object> iterObj = team->iterate_TeamMemberList();
        ...
        // Dear copy-paste monkeys, the meat of this iterate-all-player-objects
        // loop goes twixt the MEAT comments
...
void Player::removeUpgrade(const UpgradeTemplate *upgradeTemplate) {
  ...
  if (upgrade) {
    ...
    if (upgrade->getStatus() == UPGRADE_STATUS_COMPLETE) onUpgradeRemoved();
...
Bool Player::okToPlayRadarEdgeSound(void) {
  return (!TheVictoryConditions->hasSinglePlayerBeenDefeated(this) &&
          !m_isPlayerDead && !TheInGameUI->isClientQuiet() &&
          TheGameLogic->isInGameLogicUpdate() && TheGameLogic->getFrame() > 0);
}
...
void Player::removeRadar(Bool disableProof) {
  ...
  // decrement count
  DEBUG_ASSERTCRASH(m_radarCount > 0,
                    ("removeRadar: An Object is taking its radar away, but the "
                     "player radar count says they don't have radar!\n"));
  --m_radarCount;
...
Bool Player::hasRadar() const {
  if (m_radarDisabled && (m_disableProofRadarCount == 0))
    return FALSE;  // Nope, no matter how many you have, if I say no, you don't
                   // have it
...
static void doPowerDisable(Object *obj, void *userData) {
  ...
  if (obj && obj->isKindOf(KINDOF_POWERED)) {
    if (disabling)
      obj->setDisabled(
          DISABLED_UNDERPOWERED);  // set disabled has a pauseAllSpecialPowers
                                   // that prevents double pausing
    else
...
void Player::onPowerBrownOutChange(Bool brownOut) {
  ...
  if (brownOut)
    ...
    enableRadar();  // This doesn't give radar necessarily, it just removes the
                    // restriction
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
// SEVERAL OBJECTS (LIKE MULTIPLE COMMAND CENTERS) MAY HAVE MATCHING SETS OF
// SPECIAL POWERS THIS KEEPS THEM IN SYNC SO IT LOOKS LIKE EACH SPECIAL POWER IS
// SHARED BETWEEN ALL
...
void Player::resetOrStartSpecialPowerReadyFrame(
    const SpecialPowerTemplate *temp) {
...
//-------------------------------------------------------------------------------------------------
UnsignedInt Player::getOrStartSpecialPowerReadyFrame(
    const SpecialPowerTemplate *temp) {
  UnsignedInt lookupID = temp->getID();
  UnsignedInt now = TheGameLogic->getFrame();
...
void Player::friend_applyDifficultyBonusesForObject(Object *obj,
  ...
  if (TheGameLogic->isInSinglePlayerGame()) {
    Real healthFactor =
        TheGlobalData
            ->m_soloPlayerHealthBonusForDifficulty[getPlayerType()]
                                                  [getPlayerDifficulty()];
    ...
    static const WeaponBonusConditionType
        wbonus[PLAYERTYPE_COUNT][DIFFICULTY_COUNT] = {
    ...
    if (apply)
      obj->setWeaponBonusCondition(
          wbonus[getPlayerType()][getPlayerDifficulty()]);
    else
      obj->clearWeaponBonusCondition(
          wbonus[getPlayerType()][getPlayerDifficulty()]);
  }
}
...
void Player::changeBattlePlan(BattlePlanStatus plan, Int delta,
...
static void localApplyBattlePlanBonusesToObject(Object *obj, void *userData) {
  ...
  DEBUG_LOG(
      ("localApplyBattlePlanBonusesToObject() - looking at object %d (%s)\n",
       (objectToValidate) ? objectToValidate->getID() : INVALID_ID,
       (objectToValidate) ? objectToValidate->getTemplate()->getName().str()
                          : "<No Object>"));
  ...
  if (isProjectile) {
    ...
    DEBUG_LOG(("Object is a projectile - looking at object %d (%s) instead\n",
               (objectToValidate) ? objectToValidate->getID() : INVALID_ID,
               (objectToValidate)
                   ? objectToValidate->getTemplate()->getName().str()
                   : "<No Object>"));
  }
  if (objectToValidate && objectToValidate->isAnyKindOf(bonus->m_validKindOf)) {
    ...
    if (!objectToValidate->isAnyKindOf(bonus->m_invalidKindOf)) {
      ...
      if (!isProjectile) {
        DEBUG_LOG(
            ("Is not projectile.  Armor scalar is %g\n", bonus->m_armorScalar));
        // Really important to not apply certain bonuses like health
        // augmentation to projectiles!
        if (bonus->m_armorScalar != 1.0f) {
          ...
          CRCDEBUG_LOG(
              ("Applying armor scalar of %g (%8.8X) to object %d (%ls) owned "
               "by player %d\n",
               bonus->m_armorScalar, AS_INT(bonus->m_armorScalar),
               objectToModify->getID(),
               objectToModify->getTemplate()->getDisplayName().str(),
               objectToModify->getControllingPlayer()->getPlayerIndex()));
          DEBUG_LOG(
              ("After apply, armor scalar is %g\n", body->getDamageScalar()));
        }
        if (bonus->m_sightRangeScalar != 1.0f) {
          objectToModify->setVisionRange(obj->getVisionRange() *
                                         bonus->m_sightRangeScalar);
          objectToModify->setShroudClearingRange(obj->getShroudClearingRange() *
                                                 bonus->m_sightRangeScalar);
        }
      }

      if (bonus->m_bombardment > 0) {
        objectToModify->setWeaponBonusCondition(
            WEAPONBONUSCONDITION_BATTLEPLAN_BOMBARDMENT);
      } else {
        objectToModify->clearWeaponBonusCondition(
            WEAPONBONUSCONDITION_BATTLEPLAN_BOMBARDMENT);
      }
      if (bonus->m_holdTheLine > 0) {
        objectToModify->setWeaponBonusCondition(
...
void Player::removeBattlePlanBonusesForObject(Object *obj) const {
  ...
  *bonus = *m_battlePlanBonuses;
  ...
  bonus->m_sightRangeScalar = 1.0f / __max(bonus->m_sightRangeScalar, 0.01f);
  bonus->m_bombardment =
      -ALL_PLANS;  // Safe to remove as it clears the weapon bonus flag
  bonus->m_searchAndDestroy =
      -ALL_PLANS;  // Safe to remove as it clears the weapon bonus flag
  bonus->m_holdTheLine =
      -ALL_PLANS;  // Safe to remove as it clears the weapon bonus flag
...
//-------------------------------------------------------------------------------------------------
void Player::applyBattlePlanBonusesForPlayerObjects(
    const BattlePlanBonuses *bonus) {
  DUMPBATTLEPLANBONUSES(bonus, this, NULL);
  ...
  if (!m_battlePlanBonuses) {
    ...
    *m_battlePlanBonuses = *bonus;
    ...
    // Just apply the differences by multiplying the scalars together (kindofs
    // won't change) These bonuses are used for new objects that are created or
    // objects that are transferred to our team.
    ...
    m_battlePlanBonuses->m_bombardment =
        MAX(0, m_battlePlanBonuses->m_bombardment);
    m_battlePlanBonuses->m_holdTheLine += bonus->m_holdTheLine;
    m_battlePlanBonuses->m_holdTheLine =
        MAX(0, m_battlePlanBonuses->m_holdTheLine);
    m_battlePlanBonuses->m_searchAndDestroy += bonus->m_searchAndDestroy;
    m_battlePlanBonuses->m_searchAndDestroy =
        MAX(0, m_battlePlanBonuses->m_searchAndDestroy);
  }
...
void Player::processCreateTeamGameMessage(Int hotkeyNum, GameMessage *msg) {
  // GameMessage arguments are the object ID's of the objects that are to be in
  // this team.
...
//-------------------------------------------------------------------------------------------------
/** return the hotkey squad that a unit is in, or NO_HOTKEY_SQUAD if it isn't in
 * one */
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
/** Remove an object from any hotkey squads its on. (Should never be more than
 * one, but do them */
...
void Player::addKindOfProductionCostChange(KindOfMaskType kindOf,
  ...
  KindOfPercentProductionChange *newTof =
      newInstance(KindOfPercentProductionChange);
...
void Player::removeKindOfProductionCostChange(KindOfMaskType kindOf,
  ...
  DEBUG_ASSERTCRASH(FALSE, ("removeKindOfProductionCostChange was called with "
                            "kindOf=%d and percent=%f. We could not find the "
                            "entry in the list with these variables. CLH.",
                            kindOf, percent));
}

//-------------------------------------------------------------------------------------------------
/** getProductionCostChangeBasedOnKindOf gets the cost percentage change based
 * off of Kindof Mask */
//-------------------------------------------------------------------------------------------------
...
void Player::setAttackedBy(Int playerNdx) {
  DEBUG_ASSERTCRASH(playerNdx >= 0,
                    ("Player::setAttackedBy Player index is %d", playerNdx));
...
static void callHandleShroud(Object *obj, void *) {
  // I feel I have to disapprove of the naming of this gathering of cell
  // functions.  It is called by death, alliance change, containment, spy
  // change, and dynamic view range as well as partition cell change.
  if (obj) obj->handlePartitionCellMaintenance();
}

// ------------------------------------------------------------------------------------------------
void Player::setUnitsVisionSpied(Bool setting, PlayerIndex byWhom) {
  Bool needRefresh = FALSE;  // If this setting is an edge trigger on the
                             // reference count, I need
  // to tell all of my guys to handleShroud so they will start/stop looking for
  // the new team.

  if (setting) {
    m_visionSpiedBy[byWhom] = m_visionSpiedBy[byWhom] + 1;
    if (m_visionSpiedBy[byWhom] == 1) needRefresh = TRUE;
  } else {
    m_visionSpiedBy[byWhom] = m_visionSpiedBy[byWhom] - 1;
    if (m_visionSpiedBy[byWhom] == 0) needRefresh = TRUE;
  }
...
// ------------------------------------------------------------------------------------------------
PlayerMaskType Player::getVisionSpiedMask() const { return m_visionSpiedMask; }

// ------------------------------------------------------------------------------------------------
Bool Player::isPlayerObserver(void) const { return m_observer; }

// ------------------------------------------------------------------------------------------------
Bool Player::isPlayerDead(void) const { return m_isPlayerDead; }
...
void Player::crc(Xfer *xfer) {
  ...
  CRCDEBUG_LOG(("Player %d[%ls] %s battle plans\n", m_playerIndex,
                m_playerDisplayName.str(),
                (battlePlanBonus) ? "has" : "doesn't have"));
  if (m_battlePlanBonuses) {
 ...
 * 4: Player stores a list of specialpowerreadyframe timers, used by
 * specialpowermodules abroad 5: ??? (Profit) 6: Store m_unitsShouldHunt, set to
 * true after the script "Tell player to hunt" is called. 7: added Preorder flag
...
void Player::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    for (upgrade = m_upgradeList; upgrade;
    ...
    for (UnsignedShort i = 0; i < upgradeCount; ++i) {
      ...
      // add upgrade to player, the status is invalid, but that's OK cause we're
      // about to xfer it
  ...
  //
  // team prototypes ... this is only the fact that team prototypes are on this
  // player it is not the team prototype data itself
  //
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (it = m_playerTeamPrototypes.begin();
         it != m_playerTeamPrototypes.end(); ++it) {
      prototype = *it;
  ...
  for (buildListInfo = m_pBuildList; buildListInfo;
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    //
    // destroy any build list that we got from loading the bare bones map, note
    // that deleting the head of these structures automatically deletes any
    // links attached
    //
    if (m_pBuildList) m_pBuildList->deleteInstance();
    m_pBuildList = NULL;
  ...
  if ((aiPlayerPresent == TRUE && m_ai == NULL) ||
      (aiPlayerPresent == FALSE && m_ai != NULL)) {
    DEBUG_CRASH(("Player::xfer - m_ai present/missing mismatch\n"));
    throw SC_INVALID_DATA;
    ;

  }  // end if
  if (m_ai) xfer->xferSnapshot(m_ai);
#else
  ...
  Bool resourceGatheringManagerPresent =
      m_resourceGatheringManager ? TRUE : FALSE;
  xfer->xferBool(&resourceGatheringManagerPresent);
  if ((resourceGatheringManagerPresent == TRUE &&
       m_resourceGatheringManager == NULL) ||
      (resourceGatheringManagerPresent == FALSE &&
       m_resourceGatheringManager != NULL)) {
    DEBUG_CRASH(
        ("Player::xfer - m_resourceGatheringManager present/missing "
         "mismatch\n"));
    throw SC_INVALID_DATA;
  ...
  if (m_tunnelSystem) xfer->xferSnapshot(m_tunnelSystem);
  ...
  // sciences
  if (version >= 5) {
    // m_sciences will contain some intrinsic sciences and stuff, which we don't
    // want. so nuke 'em for load.
    if (xfer->getXferMode() == XFER_LOAD) m_sciences.clear();
  ...
  UnsignedShort percentProductionChangeCount =
      m_kindOfPercentProductionChangeList.size();
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (it = m_kindOfPercentProductionChangeList.begin();
         it != m_kindOfPercentProductionChangeList.end(); ++it) {
      // get entry data
      entry = *it;
    ...
    if (m_kindOfPercentProductionChangeList.size() != 0) {
      DEBUG_CRASH(
          ("Player::xfer - m_kindOfPercentProductionChangeList should be empty "
           "but is not\n"));
      throw SC_INVALID_DATA;
  ...
  if (version < 4) {
    ...
    xfer->xferUnsignedShort(
        &timerListSize);  // HANDY LITTLE SHORT TO SIZE MY LIST
    if (xfer->getXferMode() == XFER_SAVE) {
      SpecialPowerReadyTimerType *timer;
      SpecialPowerReadyTimerListIterator it;
      for (it = m_specialPowerReadyTimerList.begin();
      ...
      if (m_specialPowerReadyTimerList.size() !=
          0)  // sanity, list must be empty right now
      {
        DEBUG_CRASH(
            ("Player::xfer - m_specialPowerReadyTimerList should be empty but "
             "is not\n"));
        throw SC_INVALID_DATA;
      }  // end if
  ...
  Bool battlePlanBonus = m_battlePlanBonuses != NULL;
  xfer->xferBool(
      &battlePlanBonus);  // If we're loading, it just replaces the bool
  if (xfer->getXferMode() == XFER_LOAD) {
    if (m_battlePlanBonuses) {
      m_battlePlanBonuses->deleteInstance();
      m_battlePlanBonuses = NULL;
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void Player::loadPostProcess(void) {}  // end loadPostProcess
