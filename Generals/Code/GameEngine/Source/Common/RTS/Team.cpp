...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
TeamRelationMap::TeamRelationMap(void) {}  // end TeamRelationMap
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void TeamRelationMap::crc(Xfer *xfer) {}  // end crc
...
void TeamRelationMap::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (teamRelationIt = m_map.begin(); teamRelationIt != m_map.end();
         ++teamRelationIt) {
      // write team ID
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void TeamRelationMap::loadPostProcess(void) {}  // end loadPostProcess
...
// ------------------------------------------------------------------------
// STATIC FUNCTIONS ///////////////////////////////////////////////////////////
static Bool locoSetMatches(LocomotorSurfaceTypeMask lstm,
                           UnsignedInt surfaceBitFlags) {
...
// ------------------------------------------------------------------------
TeamFactory::~TeamFactory() { clear(); }

// ------------------------------------------------------------------------
void TeamFactory::init(void) { clear(); }
...
// ------------------------------------------------------------------------
void TeamFactory::initTeam(const AsciiString &name, const AsciiString &owner,
                           Bool isSingleton, Dict *d) {
  ...
  DEBUG_ASSERTCRASH(
      pOwner, ("no owner found for team %s (%s)\n", name.str(), owner.str()));
  if (!pOwner) pOwner = ThePlayerList->getNeutralPlayer();
  /*TeamPrototype *tp =*/newInstance(TeamPrototype)(
      this, name, pOwner, isSingleton, d, ++m_uniqueTeamPrototypeID);
  if (isSingleton) {
...
void TeamFactory::removeTeamPrototypeFromList(TeamPrototype *team) {
  NameKeyType nk = NAMEKEY(team->getName());
  TeamPrototypeMap::iterator it = m_prototypes.find(nk);
  if (it != m_prototypes.end()) m_prototypes.erase(it);
}
...
TeamPrototype *TeamFactory::findTeamPrototype(const AsciiString &name) {
  ...
  TeamPrototypeMap::iterator it = m_prototypes.find(nk);
  if (it != m_prototypes.end()) return it->second;
...
TeamPrototype *TeamFactory::findTeamPrototypeByID(TeamPrototypeID id) {
  ...
  for (it = m_prototypes.begin(); it != m_prototypes.end(); ++it) {
    prototype = it->second;
    if (prototype->getID() == id) return prototype;
...
Team *TeamFactory::findTeamByID(TeamID teamID) {
  // simple case
  if (teamID == TEAM_ID_INVALID) return NULL;
    ...
    if (t) {
      if (tp->getTemplateInfo()->m_executeActions) {
        const Script *script = TheScriptEngine->findScriptByName(
            tp->getTemplateInfo()->m_productionCondition);
        if (script) {
...
void TeamFactory::teamAboutToBeDeleted(Team *team) {
  for (TeamPrototypeMap::iterator it = m_prototypes.begin();
       it != m_prototypes.end(); ++it) {
    it->second->teamAboutToBeDeleted(team);
  }
  if (ThePlayerList) ThePlayerList->teamAboutToBeDeleted(team);
}

// ------------------------------------------------------------------------
/** CRC */
// ------------------------------------------------------------------------
void TeamFactory::crc(Xfer *xfer) {}  // end crc
...
void TeamFactory::xfer(Xfer *xfer) {
  ...
  //
  // prototypes cannot change in number during run time so the count should be
  // the same as that already loaded into us from a map load
  //
  ...
  Team *team;
  for( prototypeIt = m_prototypes.begin(); prototypeIt != m_prototypes.end();
  ++prototypeIt )
  {
          prototype = prototypeIt->second;
  fprintf( fp, "Prototype '%s' for player index '%d'\n",
  prototype->getName().str(),
  prototype->getControllingPlayer()->getPlayerIndex() ); for(
  DLINK_ITERATOR<Team> teamIt = prototype->iterate_TeamInstanceList();
  !teamIt.done(); teamIt.advance() )
          {
                  team = teamIt.cur();
  fprintf( fp, "  Team Instance '%s', id is '%d'\n", team->getName().str(),
  team->getID() ); for( DLINK_ITERATOR<Object> objIt =
  team->iterate_TeamMemberList(); !objIt.done(); objIt.advance() )
                  {
                          obj = objIt.cur();
...
void TeamFactory::loadPostProcess(void) {
  // set the next unique team and prototype ID to just over the highest one in
  // use
  m_uniqueTeamID = 0;
  ...
  for (prototypeIt = m_prototypes.begin(); prototypeIt != m_prototypes.end();
    ...
    for (DLINK_ITERATOR<Team> iter = prototype->iterate_TeamInstanceList();
      ...
      team = iter.cur();
      if (team->getID() >= m_uniqueTeamID) m_uniqueTeamID = team->getID() + 1;
  ...
  !teamIt.done(); teamIt.advance() )
          {
                  team = teamIt.cur();
  fprintf( fp, "  Team Instance '%s', id is '%d'\n", team->getName().str(),
  team->getID() ); team->reverse_TeamMemberList(); for( DLINK_ITERATOR<Object>
  objIt = team->iterate_TeamMemberList(); !objIt.done(); objIt.advance() )
                  {
                          obj = objIt.cur();
  fprintf( fp, "    Member '%s', id '%d'\n",
  obj->getTemplate()->getName().str(), obj->getID() );
...
TeamTemplateInfo::TeamTemplateInfo(Dict *d) : m_numUnitsInfo(0) {
  ...
  for (int i = 0; i < MAX_GENERIC_SCRIPTS; ++i) {
    ...
    keyName.format(
        "%s%d",
        TheNameKeyGenerator->keyToName(TheKey_teamGenericScriptHook).str(), i);
  ...
  m_startReinforceWaypoint =
      d->getAsciiString(TheKey_teamReinforcementOrigin, &exists);
...
/** CRC */
// ------------------------------------------------------------------------
void TeamTemplateInfo::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------
void TeamTemplateInfo::loadPostProcess(void) {}  // end loadPostProcess
...
// ------------------------------------------------------------------------
TeamPrototype::TeamPrototype(TeamFactory *tf, const AsciiString &name,
                             Player *ownerPlayer, Bool isSingleton, Dict *d,
                             TeamPrototypeID id)
    : m_id(id),
      m_factory(tf),
      m_name(name),
  ...
  DEBUG_ASSERTCRASH(!(m_owningPlayer == NULL),
                    ("bad args to TeamPrototype ctor"));
  if (m_factory) m_factory->addTeamPrototypeToList(this);

  if (m_owningPlayer) m_owningPlayer->addTeamToList(this);
...
TeamPrototype::~TeamPrototype() {
  ...
  if (m_owningPlayer) m_owningPlayer->removeTeamFromList(this);

  if (m_factory) m_factory->removeTeamPrototypeFromList(this);
...
// ------------------------------------------------------------------------
Player *TeamPrototype::getControllingPlayer() const { return m_owningPlayer; }

// ------------------------------------------------------------------------
Team *TeamPrototype::findTeamByID(TeamID teamID) {
  for (DLINK_ITERATOR<Team> iter = iterate_TeamInstanceList(); !iter.done();
       iter.advance()) {
    if (iter.cur()->getID() == teamID) return iter.cur();
  }
  return NULL;
}
...
void TeamPrototype::setControllingPlayer(Player *newController) {
  DEBUG_ASSERTCRASH(newController,
                    ("Attempted to set NULL player as team-owner, illegal."));
  if (!newController) {
    return;
  }
...
// ------------------------------------------------------------------------
void TeamPrototype::countObjectsByThingTemplate(
    Int numTmplates, const ThingTemplate *const *things, Bool ignoreDead,
    Int *counts, Bool ignoreUnderConstruction) const {
  for (DLINK_ITERATOR<Team> iter = iterate_TeamInstanceList(); !iter.done();
       iter.advance()) {
    iter.cur()->countObjectsByThingTemplate(numTmplates, things, ignoreDead,
                                            counts, ignoreUnderConstruction);
  }
}
...
void TeamPrototype::teamAboutToBeDeleted(Team *team) {
  for (DLINK_ITERATOR<Team> iter = iterate_TeamInstanceList(); !iter.done();
    ...
    iter.cur()->removeOverrideTeamRelationship(team ? team->getID()
                                                    : TEAM_ID_INVALID);
  }
}
...
Script *TeamPrototype::getGenericScript(Int scriptToRetrieve) {
  if (!m_retrievedGenericScripts) {
    m_retrievedGenericScripts =
        TRUE;  // set this to true so we won't do the lookup again.
    ...
    for (Int i = 0; i < MAX_GENERIC_SCRIPTS; ++i) {
      ...
      if (!m_teamTemplate.m_teamGenericScripts[i].isEmpty()) {
        tmpScript = TheScriptEngine->findScriptByName(
            m_teamTemplate.m_teamGenericScripts[i]);
        if (tmpScript) {
          scriptToSave = tmpScript->duplicate();
        } else {
          DEBUG_CRASH(
              ("We attempted to find a generic script, but couldn't. ('%s')",
               m_teamTemplate.m_teamGenericScripts[i].str()));
        }
      }
...
void TeamPrototype::decreaseAIPriorityForFailure(void) const {
  m_teamTemplate.m_productionPriority -=
      m_teamTemplate.m_productionPriorityFailureDecrease;
}
  ...
  while (!done) {
    ...
    for (DLINK_ITERATOR<Team> iter = iterate_TeamInstanceList(); !iter.done();
      ...
      if (iter.cur()->getFirstItemIn_TeamMemberList() == NULL) {
        ...
        if (iter.cur()->getControllingPlayer() &&
            iter.cur()->getControllingPlayer()->getDefaultTeam() ==
                iter.cur()) {
        ...
        break;  // Not sure what state the iterator is in after deleting a
                // member of the list. jba
      }
    }
  }
}
...
Bool TeamPrototype::evaluateProductionCondition(void) {
  ...
  if (m_productionConditionScript) {
    ...
    if (TheGameLogic->getFrame() <
        m_productionConditionScript->getFrameToEvaluate()) {
      return false;
    }
    Int delaySeconds = m_productionConditionScript->getDelayEvalSeconds();

    if (delaySeconds > 0) {
      m_productionConditionScript->setFrameToEvaluate(
          TheGameLogic->getFrame() + delaySeconds * LOGICFRAMES_PER_SECOND);
    }
    return TheScriptEngine->evaluateConditions(m_productionConditionScript,
                                               NULL, getControllingPlayer());
  }
  ...
  const Script *pScript =
      TheScriptEngine->findScriptByName(m_teamTemplate.m_productionCondition);
  if (pScript) {
    // Check difficulty.
    ...
    // Make a copy of the script locally, just for paranoia's sake.  We can't be
    // sure exactly what order the teams & scripts will get reset, so be safe.
...
/** CRC */
// ------------------------------------------------------------------------
void TeamPrototype::crc(Xfer *xfer) {}  // end crc
...
void TeamPrototype::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    //
    // there is no need to check to make sure the instance list is empty here
    // ... see the large bock comment below where we find a team given an id
    //
    ...
    for (UnsignedShort i = 0; i < teamInstanceCount; ++i) {
      ...
      //
      // find this team, if it's not there create it ... note that there will in
      // fact be some teams in the instance when were are loading as they are
      // creating during the map load.  But since the team ids are reset on the
      // map load, they are created with exactly the same team IDs they had
      // before
      //
...
/** Load post process */
// ------------------------------------------------------------------------
void TeamPrototype::loadPostProcess(void) {}  // end loadPostProcess
...
Team::Team(TeamPrototype *proto, TeamID id)
  ...
  if (proto) {
    ...
    if (!proto->getTemplateInfo()->m_scriptOnAllClear.isEmpty() ||
      ...
      m_checkEnemySighted = true;  // Only keep track of enemy sighted if there
                                   // is a script that cares.
    }
...
// ------------------------------------------------------------------------
Team::~Team() {
  //	DEBUG_ASSERTCRASH(getFirstItemIn_TeamMemberList() == NULL, ("Team still
  //has members in existence"));
  ...
  // this test is valid, but will generate a 'false positive' during game
  // teardown DEBUG_ASSERTCRASH(!(getControllingPlayer() &&
  // getControllingPlayer()->getDefaultTeam()==this),("I am still someones
  // default team -- sure you want to delete me?"));
...
void Team::setControllingPlayer(Player *newController) {
  ...
  // This function is used by one script, and it is kind of odd.  The actual
  // units are not getting captured, the team they are on is being reassigned to
  // a new player. The Team doesn't change, it just starts to return a different
  // answer when you ask for the controlling player.  I don't want to make the
  // major change of onCapture on everyone, so I will do the minor fix for the
  // specific bug, which is harmless even when misused.

  // Tell all members to redo their looking status, as their Player has changed,
  // but they don't know.
...
void Team::getTeamAsAIGroup(AIGroup *pAIGroup) {
  ...
  // Should this clear out the pAIGroup that it receives? I don't think so, but
  // that would go here if so. jkmcd
...
Int Team::getTargetableCount() const {
  ...
  for (DLINK_ITERATOR<Object> iter = iterate_TeamMemberList(); !iter.done();
    ...
    if (obj->isEffectivelyDead() || (obj->getAIUpdateInterface() == NULL &&
  ...
  if (!m_playerRelations->m_map.empty()) {
    if (playerIndex == PLAYER_INDEX_INVALID) {
      ...
      PlayerRelationMapType::iterator it =
          m_playerRelations->m_map.find(playerIndex);
      if (it != m_playerRelations->m_map.end()) {
...
void Team::countObjectsByThingTemplate(Int numTmplates,
  ...
  for (DLINK_ITERATOR<Object> iter = iterate_TeamMemberList(); !iter.done();
    ...
    for (Int i = 0; i < numTmplates; ++i) {
      ...
      if (ignoreDead && iter.cur()->isEffectivelyDead()) continue;

      if (ignoreUnderConstruction &&
          (BitTest(iter.cur()->getStatusBits(),
...
Bool Team::hasAnyBuildings() const {
  for (DLINK_ITERATOR<Object> iter = iterate_TeamMemberList(); !iter.done();
       iter.advance()) {
    if (iter.cur()->isEffectivelyDead()) continue;

    if (iter.cur()->isDestroyed()) continue;

    if (iter.cur()->isKindOf(KINDOF_STRUCTURE)) return true;
  }
  return false;
}
...
Bool Team::hasAnyBuildings(KindOfMaskType kindOf) const {
  for (DLINK_ITERATOR<Object> iter = iterate_TeamMemberList(); !iter.done();
    ...
    if (iter.cur()->isKindOfMulti(kindOf, KINDOFMASK_NONE)) return true;
  }
  return false;
}
...
Bool Team::hasAnyObjects() const {
  for (DLINK_ITERATOR<Object> iter = iterate_TeamMemberList(); !iter.done();
    ...
    if (iter.cur()->isKindOf(KINDOF_INERT)) {
      // inert stuff doesn't count.  This is for radiation fields, which are
      // living so they can be attacked by ambulances.
      continue;
    }
...
void Team::updateState(void) {
  ...
  if (m_created) {
    ...
    if (!pInfo->m_scriptOnDestroyed.isEmpty()) {
      ...
      m_destroyThreshold =
          m_curUnits - (m_curUnits * pInfo->m_destroyedThreshold);
  ...
  if (m_checkEnemySighted) {
    ...
    for (DLINK_ITERATOR<Object> iter = iterate_TeamMemberList(); !iter.done();
      ...
      PartitionFilterRelationship filterTeam(
          iter.cur(), PartitionFilterRelationship::ALLOW_ENEMIES);

      // and only stuff that is not dead
      PartitionFilterAlive filterAlive;
      PartitionFilterSameMapStatus filterMapStatus(iter.cur());

      PartitionFilter *filters[] = {&filterTeam, &filterAlive, &filterMapStatus,
                                    NULL};
      Real visionRange = iter.cur()->getVisionRange();
      anyAliveInTeam = true;
      Object *pObj = ThePartitionManager->getClosestObject(
          iter.cur(), visionRange, FROM_CENTER_2D, filters);
      if (pObj) {
        m_seeEnemy = true;
        break;
      }
...
Bool Team::someInsideSomeOutside(PolygonTrigger *pTrigger,
  ...
  for (DLINK_ITERATOR<Object> iter = iterate_TeamMemberList(); !iter.done();
    ...
    // In this particular case, this is unnecessary. However, unless it is a
    // performance hit, please leave it.
    anyConsidered = true;
  }
...
const Coord3D *Team::getEstimateTeamPosition(void) {
  // this doesn't actually calculate the team position, but rather estimates it
  // by returning the position of the first member of the team
...
void Team::deleteTeam(Bool ignoreDead) {
  // First off, if this Team is the Player's default team, we need to Evacuate
  // everyone or else Garrisoned buildings will fall victim to this deletion as
  // well, since they were added to the Default when captured.  Design intends
  // with this script to kill the people out from inside. If the thing is a
  // transport, everything will still work, as the issue at hand is the
  // container's wanting to change sides when emptied.  The bug is that the
  // people in the Garrisoned building are deleted, and that changes the Team of
  // the building, and then the DLink walks down the new team and deletes the
  // wrong stuff.  Like every tree and civialian building on the map. Of course,
  // to prevent the exact same DLINK jumping bug, I must first record what guys
  // I am going to Evacuate, or else after the first occupied building is
  // emptied, he will move his Next into the same damn wrong team.
  if (this == getControllingPlayer()->getDefaultTeam()) {
    ...
    for (DLINK_ITERATOR<Object> iter = iterate_TeamMemberList(); !iter.done();
      ...
      if (obj->getContain() && (obj->getContain()->getContainCount() > 0)) {
        // Write them all down, so the DLINK track jumping doesn't screw me up
        // here as well.
  ...
  for (DLINK_ITERATOR<Object> iter = iterate_TeamMemberList(); !iter.done();
    ...
    // The reason the comment says the team will be deleted is because it will
    // see that it is empty. So yes, the Team will survive for a while, but it
    // was going to anyway.  This is a script flag, so if they use it on a team
    // with things that can't die, then yeah, the Team will last forever.  But
    // then it is user error.
    if (ignoreDead && obj->isEffectivelyDead()) continue;
...
static Bool isInBuildVariations(const ThingTemplate *ttWithVariations,
  ...
  for (std::vector<AsciiString>::const_iterator it = bv.begin(); it != bv.end();
       ++it) {
    if (b->getName() == *it) return true;
  }
  return false;
}
...
Object *Team::tryToRecruit(const ThingTemplate *tTemplate,
                           const Coord3D *teamHome, Real maxDist) {
  ...
  for (obj = TheGameLogic->getFirstObject(); obj; obj = obj->getNextObject()) {
    if (!obj->getTemplate()->isEquivalentTo(tTemplate)) {
      // it might be ok, if tTemplate is really just a "build-variations"
      // template...
      if (!isInBuildVariations(tTemplate, obj->getTemplate())) continue;
    }
    if (obj->getControllingPlayer() != myPlayer) continue;
    ...
    if (obj->getAIUpdateInterface() &&
        !obj->getAIUpdateInterface()->isRecruitable()) {
...
void Team::evacuateTeam(void) {
  ...
  for (objIt = objectsToProcess.begin(); objIt != objectsToProcess.end();
       ++objIt) {
    Object *obj = *objIt;
    ContainModuleInterface *cmi = obj->getContain();
    if (cmi) {
...
void Team::killTeam(void) {
  ...
  const ThingTemplate *beaconTemplate = TheThingFactory->findTemplate(
      getControllingPlayer()->getPlayerTemplate()->getBeaconTemplate());
  ...
  for (DLINK_ITERATOR<Object> iter = iterate_TeamMemberList(); !iter.done();
    ...
    if (!obj || obj->isDestroyed() ||
        (obj->isEffectivelyDead() &&
         !obj->getTemplate()->isEquivalentTo(beaconTemplate))) {
      continue;
    }
...
// ------------------------------------------------------------------------
/// @todo This should give a "team move" command, not individual move orders
/// (MSB)
...
Bool Team::hasAnyBuildFacility() const {
  for (DLINK_ITERATOR<Object> iter = iterate_TeamMemberList(); !iter.done();
       iter.advance()) {
    const ThingTemplate *objtmpl = iter.cur()->getTemplate();
    if (objtmpl->isBuildFacility()) return true;
  }
  return false;
}
...
void Team::updateGenericScripts(void) {
  ...
  for (Int i = 0; i < MAX_GENERIC_SCRIPTS; ++i) {
    if (m_shouldAttemptGenericScript[i]) {
      // Does the condition succeed? If so, run it. If it is a run once script,
      // also mark that we shouldn't run it again.
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void Team::crc(Xfer *xfer) {}  // end crc
...
void Team::xfer(Xfer *xfer) {
  ...
  // xfer id, this is a sanity check as team id m_id should always be valid at
  // this point
  TeamID teamID = m_id;
  xfer->xferUser(&teamID, sizeof(TeamID));
  if (teamID != m_id) {
    DEBUG_CRASH(
        ("Team::xfer - TeamID mismatch.  Xfered '%d' but should be '%d'\n",
         teamID, m_id));
    throw SC_INVALID_DATA;
  ...
  for (DLINK_ITERATOR<Object> objIt = iterate_TeamMemberList();
       objIt.done() == FALSE; objIt.advance())
    memberCount++;
  ...
  // waypoint
  UnsignedInt currentWaypointID =
      m_currentWaypoint ? m_currentWaypoint->getID() : 0;
  ...
  if (shouldAttemptGenericScriptCount != MAX_GENERIC_SCRIPTS) {
    DEBUG_CRASH(
        ("Team::xfer - The number of allowable Generic scripts has changed, "
         "and this chunk needs to be versioned."));
    throw SC_INVALID_DATA;
  }
...
void Team::loadPostProcess(void) {
  //
  // now that all objects have actually been loaded, populate the member list
  // with real object pointers
  //
  Object *obj;
  ...
  for (it = m_xferMemberIDList.begin(); it != m_xferMemberIDList.end(); ++it) {
    ...
    if (obj == NULL) {
      DEBUG_CRASH(
          ("Team::loadPostProcess - Unable to post process object to member "
           "list, object ID = '%d'\n",
           *it));
      throw SC_INVALID_DATA;
    ...
    //
    // we are now disabling this code since the objects set their team during
    // their own xfer function which will actually put it on the team ...
    // however, we will sanity check everything here to make sure that all the
    // objects that should be on the team, are in fact on the team
    //
    if (isInList_TeamMemberList(obj) == FALSE) {
      DEBUG_CRASH(
          ("Team::loadPostProcess - Object '%s'(%d) should be in team list but "
           "is not\n",
           obj->getTemplate()->getName().str(), obj->getID()));
      throw SC_INVALID_DATA;
  ...
  // since we prepended the object member pointers, reverse that list so it's
  // just like before
