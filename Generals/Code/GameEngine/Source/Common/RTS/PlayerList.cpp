...
PlayerList::PlayerList() : m_local(NULL), m_playerCount(0) {
  ...
  for (Int i = 0; i < MAX_PLAYER_COUNT; i++) m_players[i] = NEW Player(i);
  init();
}
...
PlayerList::~PlayerList() {
  ...
  for (Int i = 0; i < MAX_PLAYER_COUNT; ++i) delete m_players[i];
}
...
void PlayerList::newGame() {
  ...
  for (i = 0; i < TheSidesList->getNumSides(); i++) {
    ...
    /// @todo The Player class should have a reset() method, instead of directly
    /// calling initFromDict() (MSB)
  ...
  if (!setLocal) {
    DEBUG_ASSERTCRASH(TheNetwork, ("*** Map has no human player... picking "
                                   "first nonneutral player for control\n"));
    for (i = 0; i < TheSidesList->getNumSides(); i++) {
  ...
  for (i = 0; i < TheSidesList->getNumSides(); i++) {
    Dict *d = TheSidesList->getSideInfo(i)->getDict();
    Player *p =
        findPlayerWithNameKey(NAMEKEY(d->getAsciiString(TheKey_playerName)));
...
void PlayerList::init() {
  ...
  for (int i = 1; i < MAX_PLAYER_COUNT; i++) m_players[i]->init(NULL);
...
void PlayerList::setLocalPlayer(Player *player) {
  ...
  if (player) {
    DEBUG_LOG(("\n----------\n"));
    // did you know? you can use "%ls" to print a doublebyte string, even in a
    // single-byte printf...
    DEBUG_LOG(
        ("Switching local players. The new player is named '%ls' (%s) and owns "
         "the following objects:\n",
         player->getPlayerDisplayName().str(),
         TheNameKeyGenerator->keyToName(player->getPlayerNameKey()).str()));
    for (Object *obj = player->getFirstOwnedObject(); obj;
         obj = obj->getNextOwnedObject()) {
      DEBUG_LOG(("Obj %08lx is of type %s", obj,
...
Player *PlayerList::getPlayerFromMask(PlayerMaskType mask) {
  ...
  for (i = 0; i < MAX_PLAYER_COUNT; i++) {
    ...
    if (player && player->getPlayerMask() == mask) return player;
...
//-------------------------------------------------------------------------------------------------
PlayerMaskType PlayerList::getPlayersWithRelationship(
    Int srcPlayerIndex, UnsignedInt allowedRelationships) {
  PlayerMaskType retVal = 0;

  if (allowedRelationships == 0) return retVal;

  Player *srcPlayer = getNthPlayer(srcPlayerIndex);
  if (!srcPlayer) return retVal;
...
void PlayerList::crc(Xfer *xfer) {
  ...
  for (Int i = 0; i < m_playerCount; ++i) xfer->xferSnapshot(m_players[i]);
}
...
void PlayerList::xfer(Xfer *xfer) {
  ...
  //
  // sanity, the player count read from the file should match our player count
  // that was setup from the bare bones map load since that data can't change
  // during run time
  //
  ...
  for (Int i = 0; i < playerCount; ++i) xfer->xferSnapshot(m_players[i]);
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void PlayerList::loadPostProcess(void) {}  // end postProcessLoad
