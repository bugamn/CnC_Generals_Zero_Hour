...
// /////////////////////////////////////////////////////////// The part of a
// Player's brain that keeps track of all Resource type Objects and makes
...
ResourceGatheringManager::~ResourceGatheringManager() {
  m_supplyWarehouses.erase(m_supplyWarehouses.begin(),
                           m_supplyWarehouses.end());
...
static Real computeRelativeCost(Object *queryObject, Object *destObject,
                                Real *pureDistanceSquared) {
  /** @todo This gets filled with Pathfinding computations, analysis of Boxes
     remaining, Threat calculations, paths of other trucks, and other fancy
     stuff.
  */

  // A good score is a very small number.

  if (queryObject == NULL || destObject == NULL) return FLT_MAX;
  ...
  if (!dockInterface->isClearToApproach(queryObject)) return FLT_MAX;
  ...
  Real distSquared = ThePartitionManager->getDistanceSquared(
      queryObject, destObject, FROM_CENTER_3D);

  // I need the distance, but I don't want to count on the coincidence that
  // the abstract 'cost' this function returns happens to be just the distance,
  // since it could become more complicated
  if (pureDistanceSquared) *pureDistanceSquared = distSquared;
...
Object *ResourceGatheringManager::findBestSupplyWarehouse(Object *queryObject) {
  ...
  if ((queryObject == NULL) || (queryObject->getAI() == NULL)) return NULL;
  ...
  if (supplyTruckAI) {
    ...
    if (dock) {
      ...
      SupplyWarehouseDockUpdate *warehouseModule =
          (SupplyWarehouseDockUpdate *)dock->findUpdateModule(
              key_warehouseUpdate);
    ...
    // Design wants a harvester to give up and return to base if it is "too far"
    // to the warehouse. Note, the "PreferedDock" will override this, and there
    // is no distance max on Centers.
  ...
  while (iterator != m_supplyWarehouses.end()) {
    ...
    if (currentWarehouse == NULL) {
      ...
      Real currentCost =
          computeRelativeCost(queryObject, currentWarehouse, &distanceSquared);
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void ResourceGatheringManager::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void ResourceGatheringManager::loadPostProcess(void) {}  // end loadPostProcess
