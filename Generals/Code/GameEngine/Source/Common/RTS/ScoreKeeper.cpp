...
//
//	purpose:	Score Keeper class will be an object attached to each
//player 						that will maintain accurate counts for the various stats we 						want to
//show on the score screen.  The information in here 						could also be used for the
//observer screen
//
//-----------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////
...
ScoreKeeper::ScoreKeeper(void) { reset(0); }
...
void ScoreKeeper::addObjectBuilt(const Object *o) {
  ...
  if (o->getTemplate()->isKindOfMulti(scoringBuildingMask, KINDOFMASK_NONE)) {
    ...
    addToCount = TRUE;
  } else if (o->getTemplate()->isKindOfMulti(scoringBuildingCreateMask,
                                             KINDOFMASK_NONE)) {
    ++m_totalBuildingsBuilt;
    addToCount = TRUE;
  } else if (o->getTemplate()->isKindOf(KINDOF_INFANTRY) ||
             o->getTemplate()->isKindOf(KINDOF_VEHICLE)) {
    if (o->getTemplate()->isKindOf(KINDOF_SCORE) ||
        o->getTemplate()->isKindOf(KINDOF_SCORE_CREATE)) {
      ++m_totalUnitsBuilt;
      addToCount = TRUE;
    }
  }
  ...
  if (addToCount) {
    ...
    if (it != m_objectsBuilt.end()) existingCount = it->second;
...
Int ScoreKeeper::getTotalUnitsBuilt(KindOfMaskType validMask,
  ...
  Int count = 0;
  for (ObjectCountMapIt it = m_objectsBuilt.begin(); it != m_objectsBuilt.end();
       ++it) {
...
Int ScoreKeeper::getTotalObjectsBuilt(const ThingTemplate *pTemplate) {
  ...
  for (ObjectCountMapIt it = m_objectsBuilt.begin(); it != m_objectsBuilt.end();
       ++it) {
    const ThingTemplate *theTemplate = it->first;
    if (theTemplate->isEquivalentTo(pTemplate)) ++count;
  }
  return count;
}
...
void ScoreKeeper::addObjectCaptured(const Object *o) {
  ...
  if (addToCount) {
    ...
    if (it != m_objectsCaptured.end()) existingCount = it->second;
...
void ScoreKeeper::addObjectDestroyed(const Object *o) {
  ...
  } else if (o->getTemplate()->isKindOfMulti(scoringBuildingDestroyMask,
                                             KINDOFMASK_NONE)) {
    if (!(o->testStatus(OBJECT_STATUS_UNDER_CONSTRUCTION))) {
      ++m_totalBuildingsDestroyed[playerIdx];
      addToCount = TRUE;
             ...
             o->getTemplate()->isKindOf(KINDOF_VEHICLE)) {
    if (o->getTemplate()->isKindOf(KINDOF_SCORE) ||
        o->getTemplate()->isKindOf(KINDOF_SCORE_DESTROY)) {
      if (!(o->testStatus(OBJECT_STATUS_UNDER_CONSTRUCTION))) {
  ...
  if (addToCount) {
    ...
    if (it != m_objectsDestroyed[playerIdx].end()) existingCount = it->second;
...
void ScoreKeeper::addObjectLost(const Object *o) {
  ...
  if (addToCount) {
    ...
    if (it != m_objectsLost.end()) existingCount = it->second;
...
Int ScoreKeeper::getTotalBuildingsDestroyed(void) {
  ...
  for (int i = 0; i < MAX_PLAYER_COUNT; ++i) {
    ...
    // for (ObjectCountMapIt it = m_objectsDestroyed[i].begin(); it !=
    // m_objectsDestroyed[i].end(); ++it)
    //		{
    //
    //			count += it->second;
    //		}
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void ScoreKeeper::crc(Xfer *xfer) {}  // end ScoreKeeper
...
void ScoreKeeper::xfer(Xfer *xfer) {
  ...
  if (destroyedArraySize != MAX_PLAYER_COUNT) {
    DEBUG_CRASH(
        ("ScoreKeeper::xfer - size of objects destroyed array has changed\n"));
    throw SC_INVALID_DATA;
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void ScoreKeeper::loadPostProcess(void) {}  // end loadPostProcess
