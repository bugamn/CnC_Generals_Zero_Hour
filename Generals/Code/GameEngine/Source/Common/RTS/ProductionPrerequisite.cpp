...
void ProductionPrerequisite::resolveNames() {
  for (Int i = 0; i < m_prereqUnits.size(); i++) {
    //
    // note that this will find the template at the "top most" level (not
    // override sub-temlates), which is what we want ... we conceptually only
    // have one template for any given thing, it's only the *data* that is
    // overridden
    //
    m_prereqUnits[i].unit =
        TheThingFactory->findTemplate(m_prereqUnits[i].name);  // might be null

    /** @todo for now removing this assert until we can completely remove
    the GDF stuff, the problem is that some INI files refer to GDF names, and
    they aren't yet loaded in the world builder but will all go away later
    anyway etc */
...
//-----------------------------------------------------------------------------
Int ProductionPrerequisite::calcNumPrereqUnitsOwned(
    const Player *player, Int counts[MAX_PREREQ]) const {
  const ThingTemplate *tmpls[MAX_PREREQ];
  Int cnt = m_prereqUnits.size();
  if (cnt > MAX_PREREQ) cnt = MAX_PREREQ;
  for (int i = 0; i < cnt; i++) tmpls[i] = m_prereqUnits[i].unit;
  player->countObjectsByThingTemplate(cnt, tmpls, false, counts);
  return cnt;
}

//-----------------------------------------------------------------------------
Int ProductionPrerequisite::getAllPossibleBuildFacilityTemplates(
    const ThingTemplate *tmpls[], Int maxtmpls) const {
  Int count = 0;
  for (int i = 0; i < m_prereqUnits.size(); i++) {
    if (i > 0 && !(m_prereqUnits[i].flags & UNIT_OR_WITH_PREV)) break;
    if (count >= maxtmpls) break;
...
const ThingTemplate *ProductionPrerequisite::getExistingBuildFacilityTemplate(
  ...
  if (m_prereqUnits.size()) {
    ...
    for (int i = 0; i < cnt; i++) {
      if (i > 0 && !(m_prereqUnits[i].flags & UNIT_OR_WITH_PREV)) break;
      if (ownCount[i]) return m_prereqUnits[i].unit;
    }
  }
  return NULL;
}
...
Bool ProductionPrerequisite::isSatisfied(const Player *player) const {
  ...
  for (i = 0; i < m_prereqSciences.size(); i++) {
    if (!player->hasScience(m_prereqSciences[i])) return false;
  }
 ...
 * to be an alternate prereq to the previously added unit, otherwise this
 * becomes a new 'block' and is required in ADDDITION to other entries. Return
 * FALSE if no space left to add unit */
//-------------------------------------------------------------------------------------------------
 ...
 * FALSE if no space left to add unit */
//-------------------------------------------------------------------------------------------------
void ProductionPrerequisite::addUnitPrereq(
    const std::vector<AsciiString> &units) {
...
// not satisfied yet
UnicodeString ProductionPrerequisite::getRequiresList(
    const Player *player) const {
  ...
  for (i = 1; i < cnt; i++) {
    if (m_prereqUnits[i].flags & UNIT_OR_WITH_PREV) {
      orRequirements[i] =
          TRUE;  // set the flag for this unit to be "ored" with previous
      ownCount[i] += ownCount[i - 1];  // lump 'em together for prereq purposes
      ownCount[i - 1] = -1;            // flag for "ignore me"
    }
  }
  ...
  for (i = 0; i < cnt; i++) {
    ...
    if (ownCount[i] == 0) {
      ...
      // TheControlBar->findCommandButton(unit->getName()); if (cmdButton)
  ...
  // gotta have all the prereq sciences.
  for (i = 0; i < m_prereqSciences.size(); i++) {
    if (!player->hasScience(m_prereqSciences[i])) hasSciences = FALSE;
  }
  ...
  if (hasSciences == FALSE) {
    ...
    requiresList.concat(
        TheGameText->fetch("CONTROLBAR:GeneralsPromotion", NULL));
  }
