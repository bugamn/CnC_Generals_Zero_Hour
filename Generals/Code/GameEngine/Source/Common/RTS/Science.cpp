...
void ScienceStore::reset() {
  ...
  for (ScienceInfoVec::iterator it = m_sciences.begin(); it != m_sciences.end();
       /*++it*/) {
    ScienceInfo* si = *it;
...
//-----------------------------------------------------------------------------
ScienceType ScienceStore::getScienceFromInternalName(
    const AsciiString& name) const {
  if (name.isEmpty()) return SCIENCE_INVALID;
...
AsciiString ScienceStore::getInternalNameForScience(ScienceType science) const {
  if (science == SCIENCE_INVALID) return AsciiString::TheEmptyString;
...
std::vector<AsciiString> ScienceStore::friend_getScienceNames() const {
  ...
  for (ScienceInfoVec::const_iterator it = m_sciences.begin();
...
void ScienceInfo::addRootSciences(ScienceVec& v) const {
  if (m_prereqSciences.empty()) {
    ...
    for (ScienceVec::const_iterator it = m_prereqSciences.begin();
         it != m_prereqSciences.end(); ++it) {
      const ScienceInfo* si = TheScienceStore->findScienceInfo(*it);
      if (si) si->addRootSciences(v);
    }
  }
}
...
/*static*/ void ScienceStore::friend_parseScienceDefinition(INI* ini) {
  ...
  if (TheScienceStore) {
    static const FieldParse myFieldParse[] = {
        {"PrerequisiteSciences", INI::parseScienceVector, NULL,
         offsetof(ScienceInfo, m_prereqSciences)},
        {"SciencePurchasePointCost", INI::parseInt, NULL,
         offsetof(ScienceInfo, m_sciencePurchasePointCost)},
        {"IsGrantable", INI::parseBool, NULL,
         offsetof(ScienceInfo, m_grantable)},
        {"DisplayName", INI::parseAndTranslateLabel, NULL,
         offsetof(ScienceInfo, m_name)},
        {"Description", INI::parseAndTranslateLabel, NULL,
         offsetof(ScienceInfo, m_description)},
        {0, 0, 0, 0}};

    ScienceInfo* info = NULL;

    // see if the science already exists. (can't use findScienceInfo() since it
    // is const and should remain so.)
    for (ScienceInfoVec::iterator it = TheScienceStore->m_sciences.begin();
         it != TheScienceStore->m_sciences.end(); ++it) {
      // note that we don't use getFinalOverride here. this is correct and
      // as-desired.
      if ((*it)->m_science == st) {
        info = *it;
        break;
    ...
    if (ini->getLoadType() == INI_LOAD_CREATE_OVERRIDES) {
      ...
      if (info == NULL) {
        ...
        // copy data from final override to 'newInfo' as a set of initial
        // default values
        ...
        *newInfo = *info;
        ...
        info = newInfo;
        // TheScienceStore->m_sciences.push_back(info);	// NO, BAD, WRONG --
...
//-----------------------------------------------------------------------------
Bool ScienceStore::getNameAndDescription(ScienceType st, UnicodeString& name,
...
Bool ScienceStore::playerHasPrereqsForScience(const Player* player,
  ...
  const ScienceInfo* si = findScienceInfo(st);
  if (si) {
    for (ScienceVec::const_iterator it2 = si->m_prereqSciences.begin();
         it2 != si->m_prereqSciences.end(); ++it2) {
...
Bool ScienceStore::playerHasRootPrereqsForScience(const Player* player,
  ...
  const ScienceInfo* si = findScienceInfo(st);
  if (si) {
    for (ScienceVec::const_iterator it2 = si->m_rootSciences.begin();
         it2 != si->m_rootSciences.end(); ++it2) {
      if (!player->hasScience(*it2)) {
        return false;
      }
    }
...
//-----------------------------------------------------------------------------
/** return a list of the sciences the given player can purchase now, and a list
   he might be able to purchase in the future, but currently lacks prereqs or
   points for. (either might be an empty list) */
void ScienceStore::getPurchasableSciences(
    const Player* player, ScienceVec& purchasable,
...
ScienceType ScienceStore::friend_lookupScience(const char* scienceName) const {
  ...
  if (!isValidScience(st)) {
    DEBUG_CRASH(
        ("Science name %s not known! (Did you define it in Science.ini?)",
         scienceName));
    throw INI_INVALID_DATA;
  }
  return st;
