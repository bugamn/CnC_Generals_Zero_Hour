 ...
 */
void State::friend_onCondition(StateTransFuncPtr test, StateID toStateID,
                               void *userData, const char *description) {
  m_transitions.push_back(
      TransitionInfo(test, toStateID, userData, description));
}
...
StateReturnType State::friend_checkForTransitions(StateReturnType status) {
  ...
  DEBUG_ASSERTCRASH(!IS_STATE_SLEEP(status),
                    ("Please handle sleep states prior to this"));
  ...
  switch (status) {
    ...
    case STATE_CONTINUE:
      ...
      if (!m_transitions.empty()) {
        for (std::vector<TransitionInfo>::const_iterator it =
                 m_transitions.begin();
          ...
          if (it->test(this, it->userData)) {
            ...
            if (getMachine()->getWantsDebugOutput()) {
              DEBUG_LOG(
                  ("%d '%s' -- '%s' condition '%s' returned true!\n",
                   TheGameLogic->getFrame(),
                   getMachineOwner()->getTemplate()->getName().str(),
                   getMachine()->getName().str(),
...
StateReturnType State::friend_checkForSleepTransitions(StateReturnType status) {
  ...
  DEBUG_ASSERTCRASH(IS_STATE_SLEEP(status),
                    ("Please only pass sleep states here"));

  // check transition condition list
  if (m_transitions.empty()) return status;

  for (std::vector<TransitionInfo>::const_iterator it = m_transitions.begin();
       it != m_transitions.end(); ++it) {
    if (!it->test(this, it->userData)) continue;
...
Bool StateMachine::getWantsDebugOutput() const {
  ...
  if (TheObjectIDToDebug != 0 && getOwner() != NULL &&
      getOwner()->getID() == TheObjectIDToDebug) {
    return true;
  }
#endif
...
void StateMachine::internalClear() {
  ...
  if (getWantsDebugOutput()) {
    DEBUG_LOG(("%d '%s' -- '%s' %x internalClear()\n", TheGameLogic->getFrame(),
               m_owner->getTemplate()->getName().str(), m_name.str(), this));
  }
#endif
}
...
#ifdef STATE_MACHINE_DEBUG
    if (m_currentState)
      DEBUG_LOG((" cur state '%s'\n", m_currentState->getName().str()));
    DEBUG_LOG(
        ("machine is locked (by %s), cannot be cleared (Please don't ignore; "
         "this generally indicates a potential logic flaw)\n",
         m_lockedby));
#endif
    return;
  }
...
StateReturnType StateMachine::resetToDefaultState() {
  ...
  if (!m_defaultStateInited) {
    DEBUG_CRASH(
        ("you may not call resetToDefaultState before initDefaultState"));
    return STATE_FAILURE;
  }
  ...
  DEBUG_ASSERTCRASH(
      ...
      ("StateMachine::resetToDefaultState() Error setting default state"));
...
StateReturnType StateMachine::updateStateMachine() {
  ...
  if (m_currentState) {
    ...
    // it is possible that the state's update() method may cause the state to be
    // destroyed
    if (m_currentState == NULL) {
      return STATE_FAILURE;
    }
    ...
    // -- however, foo() called setState() to State B. thus our current state is
    // not the same.
    // -- thus, if the state changed, we must ignore any sleep result and
    // pretend we got STATE_CONTINUE, so that the new state will be called
    // immediately.
    ...
    DEBUG_CRASH(
        ("State machine has no current state -- did you remember to call "
         "initDefaultState?"));
    return STATE_FAILURE;
  }
}
 ...
 */
void StateMachine::defineState(StateID id, State *state, StateID successID,
                               StateID failureID,
                               const StateConditionInfo *conditions) {
#ifdef STATE_MACHINE_DEBUG
  DEBUG_ASSERTCRASH(m_stateMap.find(id) == m_stateMap.end(),
                    ("duplicate state ID in statemachine %s\n", m_name.str()));
#endif
  ...
  while (conditions && conditions->test != NULL) {
    state->friend_onCondition(conditions->test, conditions->toStateID,
                              conditions->userData);
    ++conditions;
  }

  if (m_defaultStateID == INVALID_STATE_ID) m_defaultStateID = id;
}
...
StateReturnType StateMachine::internalSetState(StateID newStateID) {
  ...
  // if we're not setting the "done" state ID we will continue with the actual
  // transition
  if (newStateID != MACHINE_DONE_STATE_ID) {
    ...
    if (newStateID == INVALID_STATE_ID) {
      ...
      if (newStateID == INVALID_STATE_ID) {
        DEBUG_CRASH(
            ("you may NEVER set the current state to an invalid state id."));
        return STATE_FAILURE;
      }
    }
    ...
    if (getWantsDebugOutput()) {
      ...
      DEBUG_LOG(("%d '%s' -- '%s' %x exit ", TheGameLogic->getFrame(),
                 m_owner->getTemplate()->getName().str(), m_name.str(), this));
      if (m_currentState) {
  ...
  // invoke the old state's onExit()
  if (m_currentState) m_currentState->onExit(EXIT_NORMAL);
  ...
  if (m_currentState) {
    // onEnter() could conceivably change m_currentState, so save it for a
    // moment...
    State *stateBeforeEnter = m_currentState;

    StateReturnType status = m_currentState->onEnter();

    // it is possible that the state's onEnter() method may cause the state to
    // be destroyed
    if (m_currentState == NULL) {
      return STATE_FAILURE;
    }
    ...
    return STATE_CONTINUE;  // irrelevant return code, but we must return
                            // something
  }
}
...
StateReturnType StateMachine::initDefaultState() {
  DEBUG_ASSERTCRASH(!m_locked,
                    ("Machine is locked here, but probably should not be"));
  if (m_defaultStateInited) {
    DEBUG_CRASH(
        ("you may not call initDefaultState twice for the same StateMachine"));
    return STATE_FAILURE;
  } else {
...
void StateMachine::internalSetGoalPosition(const Coord3D *pos) {
  if (pos) {
    m_goalPosition = *pos;
    // Don't clear the goal object, or everything breaks.  Like construction of
    // buildings.
  }
}
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void StateMachine::crc(Xfer *xfer) {}  // end crc
...
void StateMachine::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_LOAD) {
    // We are going to jump into the current state.	We don't call onEnter or
    // onExit, because the state was already active when we saved.
  ...
  if (snapshotAllStates) {
    ...
    if (saveCount != count) {
      DEBUG_CRASH(
          ("State count mismatch - %d expected, %d read", count, saveCount));
    ...
    for (i = m_stateMap.begin(); i != m_stateMap.end(); ++i) {
      ...
      if (id != state->getID()) {
        DEBUG_CRASH(
            ("State ID mismatch - %d expected, %d read", state->getID(), id));
        throw SC_INVALID_DATA;
      }
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void StateMachine::loadPostProcess(void) {}  // end loadPostProcess
