...
#define IS_FRAME_OK_TO_LOG                                       \
  TheGameLogic->isInGame() && !TheGameLogic->isInShellGame() &&  \
      !TheDebugIgnoreSyncErrors && TheCRCFirstFrameToLog >= 0 && \
      TheCRCFirstFrameToLog <= TheGameLogic->getFrame() &&       \
...
CRCVerification::CRCVerification() {
  ...
  if (g_verifyClientCRC && (IS_FRAME_OK_TO_LOG)) {
    m_startCRC = TheGameLogic->getCRC(CRC_RECALC,
                                      (g_clientDeepCRC) ? "clientPre.crc" : "");
  } else {
    m_startCRC = 0;
  }
/**/
...
CRCVerification::~CRCVerification() {
  ...
  if (g_verifyClientCRC && (IS_FRAME_OK_TO_LOG)) {
    endCRC = TheGameLogic->getCRC(CRC_RECALC,
                                  (g_clientDeepCRC) ? "clientPost.crc" : "");
  }
  DEBUG_ASSERTCRASH(
      !TheGameLogic->isInGame() || m_startCRC == endCRC,
      ("GameLogic changed outside of GameLogic::update() on frame %d!",
       TheGameLogic->getFrame()));
  if (TheGameLogic->isInMultiplayerGame() && m_startCRC != endCRC) {
    if (TheInGameUI) {
      TheInGameUI->message(
          UnicodeString(L"GameLogic changed outside of GameLogic::update() - "
                        L"call Matt (x36804)!"));
    }
...
void outputCRCDebugLines(void) {
  ...
  if (numDebugStrings >= MaxStrings) start = nextDebugString - MaxStrings;
...
void addCRCDebugLine(const char *fmt, ...) {
  ...
  if (IS_FRAME_OK_TO_LOG) {
    ...
    sprintf(DebugStrings[nextDebugString], "%d:%d ", TheGameLogic->getFrame(),
            lastCRCDebugIndex++);
    ...
    ++nextDebugString;
    ++numDebugStrings;
    if (nextDebugString == MaxStrings) nextDebugString = 0;
  }
}

void addCRCGenLine(const char *fmt, ...) {
  if (dumped || !(IS_FRAME_OK_TO_LOG)) return;
...
void dumpVector3(const Vector3 *v, AsciiString name, AsciiString fname,
                 Int line) {
  if (dumped) return;
  ...
  addCRCDebugLine("dumpVector3() %s:%d %s %8.8X %8.8X %8.8X\n", fname.str(),
                  line, name.str(), AS_INT(v->X), AS_INT(v->Y), AS_INT(v->Z));
}

void dumpCoord3D(const Coord3D *c, AsciiString name, AsciiString fname,
                 Int line) {
  if (dumped) return;
  ...
  addCRCDebugLine("dumpCoord3D() %s:%d %s %8.8X %8.8X %8.8X\n", fname.str(),
                  line, name.str(), AS_INT(c->x), AS_INT(c->y), AS_INT(c->z));
}
...
void dumpMatrix3D(const Matrix3D *m, AsciiString name, AsciiString fname,
  ...
  for (Int i = 0; i < 3; ++i)
    addCRCDebugLine("      0x%08X 0x%08X 0x%08X 0x%08X\n",
                    AS_INT(matrix[(i << 2) + 0]), AS_INT(matrix[(i << 2) + 1]),
                    AS_INT(matrix[(i << 2) + 2]), AS_INT(matrix[(i << 2) + 3]));
}
