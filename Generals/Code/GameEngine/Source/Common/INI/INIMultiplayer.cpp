...
void INI::parseMultiplayerSettingsDefinition(INI* ini) {
  if (TheMultiplayerSettings) {
    ...
    if (ini->getLoadType() == INI_LOAD_CREATE_OVERRIDES) {
      DEBUG_ASSERTCRASH(false,
                        ("Creating an override of MultiplayerSettings!"));
    }
  }  // end if
  else {
  ...
  ini->initFromINI(TheMultiplayerSettings,
                   TheMultiplayerSettings->getFieldParse());
}
...
void INI::parseMultiplayerColorDefinition(INI* ini) {
  ...
  ini->initFromINI(multiplayerColorDefinition,
                   multiplayerColorDefinition->getFieldParse());

  multiplayerColorDefinition->setColor(
      multiplayerColorDefinition->getRGBValue());
  multiplayerColorDefinition->setNightColor(
      multiplayerColorDefinition->getRGBNightValue());
}
