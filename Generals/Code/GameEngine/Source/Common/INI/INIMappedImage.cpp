...
void INI::parseMappedImageDefinition(INI* ini) {
  ...
  Image* image =
      const_cast<Image*>(TheMappedImageCollection->findImageByName(name));
  if (image)
    DEBUG_ASSERTCRASH(
        !image->getRawTextureData(),
        ("We are trying to parse over an existing image that contains a "
         "non-null rawTextureData, you should fix that"));
  ...
  if (image == NULL) {
    ...
    DEBUG_ASSERTCRASH(
        image,
        ("parseMappedImage: unable to allocate image for '%s'\n", name.str()));
