...
void Dict::validate() const {
  ...
  DEBUG_ASSERTCRASH(m_data->m_numPairsAllocated > 0,
                    ("m_numPairsAllocated is zero"));
  DEBUG_ASSERTCRASH(m_data->m_numPairsUsed >= 0, ("m_numPairsUsed is neg"));
  DEBUG_ASSERTCRASH(m_data->m_numPairsAllocated >= m_data->m_numPairsUsed,
                    ("m_numPairsAllocated too small"));
  DEBUG_ASSERTCRASH(m_data->m_numPairsAllocated < 1024,
                    ("m_numPairsAllocated suspiciously large"));
}
#endif
...
// -----------------------------------------------------
Dict::DictPair* Dict::ensureUnique(int numPairsNeeded, Bool preserveData,
                                   DictPair* pairToTranslate) {
  ...
  if (numPairsNeeded > MAX_LEN) throw ERROR_OUT_OF_MEMORY;
  ...
  if (numPairsNeeded > 0) {
    ...
    // note: be certain to alloc with zero; we'll take advantage of the fact
    // that all-zero is a bit-pattern that happens to init all our pairs to
    // legal values: type BOOL, key INVALID, value FALSE.
    newData = (Dict::DictPairData*)TheDynamicMemoryAllocator->allocateBytes(
        actualBytes, "Dict::ensureUnique");
    newData->m_refCount = 1;
    newData->m_numPairsAllocated =
        (actualBytes - sizeof(Dict::DictPairData)) / sizeof(Dict::DictPair);
  ...
  Int delta;
  if (pairToTranslate && m_data) delta = pairToTranslate - m_data->peek();

  releaseData();
  m_data = newData;

  if (pairToTranslate && m_data) pairToTranslate = m_data->peek() + delta;
...
void Dict::releaseData() {
  if (m_data) {
    if (--m_data->m_refCount == 0) {
      Dict::DictPair* src = m_data->peek();
      for (Int i = 0; i < m_data->m_numPairsUsed; ++i, ++src) src->clear();
...
Bool Dict::getBool(NameKeyType key, Bool* exists /*=NULL*/) const {
  ...
  DEBUG_ASSERTCRASH(
      exists != NULL,
      ("dict key missing, or of wrong type\n"));  // only assert if they didn't
                                                  // check result
...
Bool Dict::getNthBool(Int n) const {
  ...
  if (m_data) {
    ...
    if (pair && pair->getType() == DICT_BOOL) return *pair->asBool();
  }
...
Int Dict::getNthInt(Int n) const {
  ...
  if (m_data) {
    DictPair* pair = &m_data->peek()[n];
    if (pair && pair->getType() == DICT_INT) return *pair->asInt();
  }
  DEBUG_CRASH(("dict key missing, or of wrong type\n"));
  return 0;
}
...
Real Dict::getNthReal(Int n) const {
  ...
  if (m_data) {
    DictPair* pair = &m_data->peek()[n];
    if (pair && pair->getType() == DICT_REAL) return *pair->asReal();
  }
  DEBUG_CRASH(("dict key missing, or of wrong type\n"));
  return 0.0f;
}
...
void Dict::copyPairFrom(const Dict& that, NameKeyType key) {
  ...
  if (thatPair) {
    ...
    if (this->findPairByKey(key)) this->remove(key);
  }
  this->validate();
}
