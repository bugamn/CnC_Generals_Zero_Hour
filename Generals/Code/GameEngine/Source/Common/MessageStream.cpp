...
GameMessage::~GameMessage() {
  ...
  if (m_list) m_list->removeMessage(this);
}
...
GameMessageArgumentDataType GameMessage::getArgumentDataType(Int argIndex) {
  ...
  int i = 0;
  for (GameMessageArgument *a = m_argList; a && (i < argIndex);
       a = a->m_next, ++i);
...
/**
 * Allocate a new argument, add it to the argument list, and increment the total
 * arg count
 */
...
void GameMessageList::insertMessage(GameMessage *msg,
  ...
  else  // if the friend wasn't there, then messageToInsertAfter is the last
        // message. Update it to be msg.
    m_lastMessage = msg;
...
TranslatorID MessageStream::attachTranslator(GameMessageTranslator *translator,
  ...
  for (ss = m_firstTranslator; ss; ss = ss->m_next)
    if (ss->m_priority > newSS->m_priority) break;
...
GameMessageTranslator *MessageStream::findTranslator(TranslatorID id) {
  ...
  for (translatorData = m_firstTranslator; translatorData;
       translatorData = translatorData->m_next) {
    if (translatorData->m_id == id) return translatorData->m_translator;
  }
  ...
  for (ss = m_firstTranslator; ss; ss = ss->m_next) {
    for (msg = m_firstMessage; msg; msg = next) {
      if (ss->m_translator
        ...
        GameMessageDisposition disp =
            ss->m_translator->translateGameMessage(msg);
        next = msg->next();
 ...
 * Destructor
 */
CommandList::~CommandList() { destroyAllMessages(); }
 ...
 * Primarily used by TheMessageStream to put the final messages that reach the
 * end of the stream on TheCommandList. Since TheGameClient will update faster
 * than TheNetwork and TheGameLogic, messages will accumulate on this list.
 */
 ...
 */
void buildRegion(const ICoord2D *anchor, const ICoord2D *dest,
                 IRegion2D *region) {
