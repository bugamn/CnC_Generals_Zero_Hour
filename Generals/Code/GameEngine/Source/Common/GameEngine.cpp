...
void initSubsystem(SUBSYSTEM *&sysref, AsciiString name, SUBSYSTEM *sys,
                   Xfer *pXfer, const char *path1 = NULL,
...
Int GameEngine::getFramesPerSecondLimit(void) { return m_maxFPS; }
...
void GameEngine::setFramesPerSecondLimit(Int fps) {
  DEBUG_LOG(
      ("GameEngine::setFramesPerSecondLimit() - setting max fps to %d "
       "(TheGlobalData->m_useFpsLimit == %d)\n",
...
}  /// @todo: I changed this to take argc & argv so we can parse those after the
   /// GDF is loaded.  We need to rethink this immediately as it is a nasty hack
...
//	const char *buildType = "Release";
    ...
    // apparently)
    ini.load(AsciiString("Data\\INI\\Default\\Water.ini"), INI_LOAD_OVERWRITE,
             &xferCRC);
    ...
    initSubsystem(TheDeepCRCSanityCheck, "TheDeepCRCSanityCheck",
                  MSGNEW("GameEngineSubystem") DeepCRCSanityCheck, NULL, NULL,
                  NULL, NULL);
#endif  // DEBUG_CRC
    ...
    initSubsystem(TheScienceStore, "TheScienceStore",
                  ...
                  "Data\\INI\\Default\\Science.ini", "Data\\INI\\Science.ini");
    ...
    initSubsystem(TheSidesList, "TheSidesList",
                  MSGNEW("GameEngineSubsystem") SidesList(), NULL);
    initSubsystem(TheCaveSystem, "TheCaveSystem",
                  MSGNEW("GameEngineSubsystem") CaveSystem(), NULL);
    initSubsystem(TheRankInfoStore, "TheRankInfoStore",
                  MSGNEW("GameEngineSubsystem") RankInfoStore(), &xferCRC, NULL,
                  "Data\\INI\\Rank.ini");
    ...
    initSubsystem(TheDamageFXStore, "TheDamageFXStore",
                  MSGNEW("GameEngineSubsystem") DamageFXStore(), &xferCRC, NULL,
                  "Data\\INI\\DamageFX.ini");
    initSubsystem(TheArmorStore, "TheArmorStore",
                  MSGNEW("GameEngineSubsystem") ArmorStore(), &xferCRC, NULL,
                  "Data\\INI\\Armor.ini");
    initSubsystem(TheBuildAssistant, "TheBuildAssistant",
                  MSGNEW("GameEngineSubsystem") BuildAssistant, NULL);
    initSubsystem(TheThingFactory, "TheThingFactory", createThingFactory(),
                  &xferCRC, "Data\\INI\\Default\\Object.ini", NULL,
                  "Data\\INI\\Object");
    ...
    initSubsystem(TheTeamFactory, "TheTeamFactory",
                  MSGNEW("GameEngineSubsystem") TeamFactory(), NULL);
    initSubsystem(TheCrateSystem, "TheCrateSystem",
                  MSGNEW("GameEngineSubsystem") CrateSystem(), &xferCRC,
                  "Data\\INI\\Default\\Crate.ini", "Data\\INI\\Crate.ini");
    initSubsystem(ThePlayerList, "ThePlayerList",
                  MSGNEW("GameEngineSubsystem") PlayerList(), NULL);
    ...
    if (!TheGlobalData->m_playIntro) TheWritableGlobalData->m_afterIntro = TRUE;
...
void GameEngine::reset(void) {
  ...
  if (TheGameLogic->isInMultiplayerGame()) deleteNetwork = true;

  TheSubsystemList->resetAll();

  if (deleteNetwork) {
    DEBUG_ASSERTCRASH(TheNetwork, ("Deleting NULL TheNetwork!"));
    if (TheNetwork) delete TheNetwork;
    TheNetwork = NULL;
  }
  if (background) {
...
// I'm disabling this in internal because many people need alt-tab capability.
// If you happen to be doing performance tuning, please just change this on your
// local system. -MDC
#if defined(_DEBUG) || defined(_INTERNAL)
...
/**	MW - 6-10-03: I added this function in order to verify that users who
quit the application by not using the menus (Alt-F4, etc.) are not doing it in
the middle of an internet game.  Quitting in this way is considered cheating so
...
void GameEngine::checkAbnormalQuitting(void) {
  if (TheRecorder->isMultiplayer() &&
      TheGameLogic->isInInternetGame()) {  // Should not be quitting at this
                                           // time, record it as a cheat.
    ...
    DEBUG_LOG(
        ("req.addDesync=%d, req.addDiscon=%d, addedInDesync=%d,%d,%d, "
         ...
         addedInDesyncs4, addedInDiscons2, addedInDiscons3, addedInDiscons4));
