...
// #define DETERMINISTIC				// to allow repetition
// for debugging
...
static const Real theMultFactor =
    1.0f / (powf(2, 8 * sizeof(UnsignedInt)) - 1.0f);

// Initial seed values.
static UnsignedInt theGameClientSeed[6] = {0xf22d0e56L, 0x883126e9L,
                                           0xc624dd2fL, 0x702c49cL,
                                           0x9e353f7dL, 0x6fdf3b64L};

static UnsignedInt theGameAudioSeed[6] = {0xf22d0e56L, 0x883126e9L,
                                          0xc624dd2fL, 0x702c49cL,
                                          0x9e353f7dL, 0x6fdf3b64L};

static UnsignedInt theGameLogicBaseSeed = 0;
static UnsignedInt theGameLogicSeed[6] = {0xf22d0e56L, 0x883126e9L,
                                          0xc624dd2fL, 0x702c49cL,
                                          0x9e353f7dL, 0x6fdf3b64L};

// Add with carry. SUM is replaced with A + B + C, C is replaced with 1  if
// there was a carry, 0 if there wasn't. A carry occurred if the sum is  less
// than one of the inputs. This is addition, so carry can never be  more than
// one.
...
UnsignedInt GetGameLogicRandomSeed(void) { return theGameLogicBaseSeed; }
...
*/
void GameClientRandomVariable::setRange(Real low, Real high,
                                        DistributionType type) {
  DEBUG_ASSERTCRASH(
      !(m_type == CONSTANT && m_low != m_high),
...
Real GameClientRandomVariable::getValue(void) const {
  switch (m_type) {
    ...
    default:
      ...
      DEBUG_CRASH(
          ("unsupported DistributionType in "
           "GameClientRandomVariable::getValue\n"));
      return 0.0f;
  }
}
...
/**
        define the range of random values, and the distribution of values
*/
void GameLogicRandomVariable::setRange(Real low, Real high,
                                       DistributionType type) {
  DEBUG_ASSERTCRASH(
      !(m_type == CONSTANT && m_low != m_high),
...
Real GameLogicRandomVariable::getValue(void) const {
  switch (m_type) {
    ...
    default:
      ...
      DEBUG_CRASH(
          ("unsupported DistributionType in "
           "GameLogicRandomVariable::getValue\n"));
      return 0.0f;
  }
}
