 ...
 *   /i,/ignore
 * list ignored players /i,/ignore +name1 -name2		ignore name1,
 * stop ignoring name2 /m,/me:
 * shorthand for an action /o,/on:
 * find command to look up a user's location /f,/find:
 * find command to look up a user's location /p,/page:
 * page user(s) /r,/reply:
 * reply to last page /raw:
 * raw IRC command (only in debug & internal) /oper:
 * become an IRC op (only in debug & internal) /quit:
 * send the IRC quit command to exit WOL
 */
static Bool handleSlashCommands(UnicodeString message, Bool isAction,
  ...
  if (message.getCharAt(0) == L'/')
  {
          ...
          case L'I':
                  ...
                  if (token.compareNoCase(L"i") == 0 ||
token.compareNoCase(L"ignore") == 0)
                  {
                          if (remainder.isEmpty())
                          {
                                  // List the people we're ignoring
                                  TheWOL->addText(TheGameText->fetch("WOL:BeginIgnoreList"));
                                  set<AsciiString> *ignoreList =
getIgnoreList(); if (ignoreList)
                                  {
                                          UnicodeString msg;
                                          UnicodeString uName;
                                          AsciiSetIter iter =
ignoreList->begin(); while (iter != ignoreList->end())
                                          {
                                                  uName.translate(*iter);
                                                  msg.format(TheGameText->fetch("WOL:IgnoredUser"),
uName.str()); TheWOL->addText(msg); iter++;
                                          }
                                  }
...
1); name.translate(token); doIgnore = 1;
                                  }
                                  else if (token.getCharAt(0) == L'-')
                                  {
                                          // Listen to someone again
                                          token = UnicodeString(token.str() +
1); name.translate(token); doIgnore = 0;
                                  }
                                  else
                                  {
                                          // Ignore somebody
...
uName.str()); TheWOL->addText(msg);
          ...
          case L'O':
                  remainder.nextToken(&token);
                  if (token.compareNoCase(L"on") == 0 ||
token.compareNoCase(L"o") == 0)
                  {
                          remainder.nextToken(&token);
          ...
          case L'P':
                  remainder.nextToken(&token);
                  if (token.compareNoCase(L"page") == 0 ||
token.compareNoCase(L"p") == 0)
                  {
                          remainder.nextToken(&token);
                          AsciiString userName;
                          userName.translate(token);
                          ...
                          else
                          {
                                  // Send page command
                                  IChat *ichat = TheWOL->getIChat();
                                  ichat->RequestGlobalUnicodePage(&user,
remainder.str());
                          }
                          return true; // show it anyway
                  }
...
static handleUnicodeMessage(const char *nick, UnicodeString msg, Bool isPublic,
                            Bool isAction);

Bool GameSpySendChat(UnicodeString message, Bool isAction,
                     GameWindow *playerListbox) {
  RoomType roomType = StagingRoom;
  if (TheGameSpyChat->getCurrentGroupRoomID()) roomType = GroupRoom;
  ...
  if (!message.isEmpty()) {
    ...
    if (!playerListbox) {
      // Public message
      if (isAction) {
        peerMessageRoom(TheGameSpyChat->getPeer(), roomType,
                        UnicodeStringToQuotedPrintable(message).str(),
                        ActionMessage);
        // if (roomType == StagingRoom)
        // handleUnicodeMessage(TheGameSpyChat->getloginName().str(), message,
        // true, true);
      } else {
        peerMessageRoom(TheGameSpyChat->getPeer(), roomType,
                        UnicodeStringToQuotedPrintable(message).str(),
    ...
    if (selections[0] == -1) {
      ...
      if (!names.isEmpty()) {
        if (isAction) {
          peerMessagePlayer(TheGameSpyChat->getPeer(), names.str(),
                            UnicodeStringToQuotedPrintable(message).str(),
                            ActionMessage);
        } else {
          peerMessagePlayer(TheGameSpyChat->getPeer(), names.str(),
...
void RoomMessageCallback(PEER peer, RoomType roomType, const char *nick,
  ...
  handleUnicodeMessage(nick, QuotedPrintableToUnicodeString(message), true,
...
void PlayerMessageCallback(PEER peer, const char *nick, const char *message,
  ...
  handleUnicodeMessage(nick, QuotedPrintableToUnicodeString(message), false,
                       (messageType == ActionMessage));
}
...
static handleUnicodeMessage(const char *nick, UnicodeString msg, Bool isPublic,
  ...
  if (isPublic && isAction) {
    ...
    style = (isOwner) ? GSCOLOR_CHAT_PRIVATE_OWNER_EMOTE
...
void GameSpyAddText(UnicodeString message, GameSpyColors color) {
  ...
  if (!textWindow) textWindow = listboxLobbyChat;
  if (!textWindow) textWindow = listboxGameSetupChat;
