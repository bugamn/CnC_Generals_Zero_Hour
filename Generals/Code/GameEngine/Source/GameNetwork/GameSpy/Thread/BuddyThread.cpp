...
GameSpyBuddyMessageQueueInterface::createNewMessageQueue(void) {
...
void callbackWrapper(GPConnection *con, void *arg, void *param) {
  ...
  BuddyThreadClass *thread =
      MESSAGE_QUEUE->getThread()
          ? MESSAGE_QUEUE->getThread()
          : NULL /*(TheGameSpyBuddyMessageQueue)?TheGameSpyBuddyMessageQueue->getThread():NULL*/
      ;
  if (!thread) return;
...
GameSpyBuddyMessageQueue::GameSpyBuddyMessageQueue() { m_thread = NULL; }

GameSpyBuddyMessageQueue::~GameSpyBuddyMessageQueue() { endThread(); }
...
BuddyThreadClass *GameSpyBuddyMessageQueue::getThread(void) { return m_thread; }
...
void BuddyThreadClass::Thread_Function() {
  try {
    ...
    while (running) {
      ...
      if (TheGameSpyBuddyMessageQueue->getRequest(incomingRequest)) {
        switch (incomingRequest.buddyRequestType) {
          case BuddyRequest::BUDDYREQUEST_LOGIN:
            ...
            m_isConnected =
                (gpConnect(con, incomingRequest.arg.login.nick,
                           incomingRequest.arg.login.email,
                           incomingRequest.arg.login.password,
                           (incomingRequest.arg.login.hasFirewall)
                               ? GP_FIREWALL
                               : GP_NO_FIREWALL,
                           GP_BLOCKING, callbackWrapper,
                           (void *)CALLBACK_CONNECT) == GP_NO_ERROR);
          ...
          case BuddyRequest::BUDDYREQUEST_RELOGIN:
            m_isConnecting = true;
            m_isConnected =
                (gpConnect(con, m_nick.c_str(), m_email.c_str(), m_pass.c_str(),
                           GP_FIREWALL, GP_BLOCKING, callbackWrapper,
                           (void *)CALLBACK_CONNECT) == GP_NO_ERROR);
            m_isConnecting = false;
            break;
          ...
          case BuddyRequest::BUDDYREQUEST_LOGINNEW: {
            ...
            m_isNewAccount = TRUE;
            m_isConnected =
                (gpConnectNewUser(con, incomingRequest.arg.login.nick,
                                  incomingRequest.arg.login.email,
                                  incomingRequest.arg.login.password,
                                  (incomingRequest.arg.login.hasFirewall)
                                      ? GP_FIREWALL
      ...
      if (isConnected == GP_CONNECTED && res == GP_NO_ERROR) gpProcess(con);
...
#undef ERRORCODE
  ...
  if (arg->fatal == GP_FATAL) {
    ...
    strncpy(errorResponse.arg.error.errorString, arg->errorString,
            MAX_BUDDY_CHAT_LEN);
...
static void getNickForMessage(GPConnection *con, GPGetInfoResponseArg *arg,
                              void *param) {
...
void BuddyThreadClass::messageCallback(GPConnection *con,
  ...
  gpGetInfo(con, arg->profile, GP_CHECK_CACHE, GP_BLOCKING,
            (GPCallback)getNickForMessage, &messageResponse);
...
void BuddyThreadClass::connectCallback(GPConnection *con,
  ...
  if (arg->result == GP_NO_ERROR) {
    ...
    if (!TheGameSpyPeerMessageQueue->isConnected() &&
        !TheGameSpyPeerMessageQueue->isConnecting()) {
    ...
    if (!TheGameSpyPeerMessageQueue->isConnected() &&
      ...
      if (m_lastErrorCode == GP_NEWUSER_BAD_NICK) {
        ...
        DEBUG_LOG(
            ("User Error: Create Account instead of Login.  Fixing them...\n"));
        BuddyRequest req;
      ...
      DEBUG_LOG(
          ("Buddy connect failed (%d/%d): posting a failed chat connect\n",
...
static void getInfoResponseForRequest(GPConnection *con,
...
static void getInfoResponseForStatus(GPConnection *connection,
                                     GPGetInfoResponseArg *arg, void *param) {
  BuddyResponse *resp = (BuddyResponse *)param;
  resp->profile = arg->profile;
...
void BuddyThreadClass::statusCallback(GPConnection *con,
  ...
  DEBUG_LOG(("Got buddy status for %d(%s) - status %d\n", status.profile,
             response.arg.status.nick, status.status));
