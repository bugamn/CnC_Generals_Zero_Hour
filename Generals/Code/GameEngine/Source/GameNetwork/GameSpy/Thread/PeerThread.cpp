...
// enable this for trying to track down why SBServers are losing their keyvals
// -MDC 2/20/2003
...
// we should always be using broadcast keys from now on.  Remove the old code
// sometime when we're not in a rush, ok? -MDC 2/14/2003
...
GameSpyPeerMessageQueueInterface::createNewMessageQueue(void) {
...
class PeerThreadClass : public ThreadClass {
 public:
  ...
  void trackStatsForPlayer(RoomType roomType, const char *nick, const char *key,
                           const char *val);
...
#ifdef USE_BROADCAST_KEYS
const char *PeerThreadClass::s_keys[6] = {"b_locale", "b_wins", "b_losses",
                                          "b_points", "b_side", "b_pre"};
char PeerThreadClass::s_valueBuffers[6][20] = {"", "", "", "", "", ""};
const char *PeerThreadClass::s_values[6] = {
    s_valueBuffers[0], s_valueBuffers[1], s_valueBuffers[2],
...
int PeerThreadClass::lookupStatForPlayer(RoomType roomType, const char *nick,
                                         const char *key) {
  ...
  switch (roomType) {
    case GroupRoom:
      ...
      if (it != m_groupRoomStats.end()) return it->second;
      break;
    case StagingRoom:
      it = m_stagingRoomStats.find(fullKey);
      if (it != m_stagingRoomStats.end()) return it->second;
      break;
  }
  return 0;
}
...
void PeerThreadClass::pushStatsToRoom(PEER peer) {
  DEBUG_LOG(
      ("PeerThreadClass::pushStatsToRoom(): stats are "
       "%s=%s,%s=%s,%s=%s,%s=%s,%s=%s,%s=%s\n",
       s_keys[0], s_values[0], s_keys[1], s_values[1], s_keys[2], s_values[2],
       s_keys[3], s_values[3], s_keys[4], s_values[4], s_keys[5], s_values[5]));
...
void PeerThreadClass::getStatsFromRoom(PEER peer, RoomType roomType) {
  peerGetRoomKeys(peer, GroupRoom, "*", NumKeys, s_keys, getRoomKeysCallback,
                  this, PEERFalse);
}
#endif  // USE_BROADCAST_KEYS
...
void PeerThreadClass::clearServers(void) { m_stagingServers.clear(); }
...
Int PeerThreadClass::findServer(SBServer server) {
  ...
  for (std::map<Int, SBServer>::iterator it = m_stagingServers.begin();
    ...
    if (it->second == server) {
      ...
      if (!strcmp(oldName, newName) && oldPrivateIP == newPrivateIP &&
          oldPublicIP == newPublicIP && oldPrivatePort == newPrivatePort) {
  ...
  if (serverToRemove) {
    // this is the same as another game - it has just migrated to another port.
    // Remove the old and replace it.
    PeerResponse resp;
...
void nickErrorCallbackWrapper(PEER peer, Int type, const char *nick,
                              void *param) {
  if (param != NULL) {
...
static void joinRoomCallback(PEER peer, PEERBool success, PEERJoinResult result,
...
GameSpyPeerMessageQueue::~GameSpyPeerMessageQueue() { endThread(); }
...
Bool GameSpyPeerMessageQueue::getResponse(PeerResponse &resp) {
  ...
  if (m_responses.empty()) return false;
  resp = m_responses.front();
  m_responses.pop();
  return true;
}

PeerThreadClass *GameSpyPeerMessageQueue::getThread(void) { return m_thread; }

//-------------------------------------------------------------------------
static void disconnectedCallback(PEER peer, const char *reason, void *param);
static void roomMessageCallback(PEER peer, RoomType roomType, const char *nick,
                                const char *message, MessageType messageType,
                                void *param);
static void playerMessageCallback(PEER peer, const char *nick,
                                  const char *message, MessageType messageType,
                                  void *param);
static void playerJoinedCallback(PEER peer, RoomType roomType, const char *nick,
                                 void *param);
static void playerLeftCallback(PEER peer, RoomType roomType, const char *nick,
                               const char *reason, void *param);
static void playerChangedNickCallback(PEER peer, RoomType roomType,
                                      const char *oldNick, const char *newNick,
                                      void *param);
static void playerInfoCallback(PEER peer, RoomType roomType, const char *nick,
                               unsigned int IP, int profileID, void *param);
static void playerFlagsChangedCallback(PEER peer, RoomType roomType,
                                       const char *nick, int oldFlags,
                                       int newFlags, void *param);
static void listingGamesCallback(PEER peer, PEERBool success, const char *name,
                                 SBServer server, PEERBool staging, int msg,
                                 Int percentListed, void *param);
...
static void playerUTMCallback(PEER peer, const char *nick, const char *command,
                              const char *parameters, PEERBool authenticated,
                              void *param);
static void gameStartedCallback(PEER peer, UnsignedInt IP, const char *message,
                                void *param);
static void globalKeyChangedCallback(PEER peer, const char *nick,
                                     const char *key, const char *val,
                                     void *param);
static void roomKeyChangedCallback(PEER peer, RoomType roomType,
                                   const char *nick, const char *key,
                                   const char *val, void *param);

// convenience function to set buddy status
static void updateBuddyStatus(GameSpyBuddyStatus status, Int groupRoom = 0,
                              std::string gameName = "") {
  ...
  DEBUG_LOG(("updateBuddyStatus %d:%s\n", req.arg.status.status,
             req.arg.status.statusString));
  TheGameSpyBuddyMessageQueue->addRequest(req);
}

static void createRoomCallback(PEER peer, PEERBool success,
...
static void QRServerKeyCallback(PEER peer, int key, qr2_buffer_t buffer,
                                void *param) {
  // DEBUG_LOG(("QR_SERVER_KEY | %d (%s)\n", key,
  // qr2_registered_key_list[key]));
  ...
  if (!t->isHosting()) t->stopHostingAlready(peer);
...
#define ADDINT(x)                  \
  ...
  switch (key) {
    ...
    case GAMENAME_KEY: {
      ...
      tmp.append(" ");
      tmp.append(
          WideCharStringToMultiByte(t->getLocalStagingServerName().c_str()));
      ADD(tmp.c_str());
    } break;
    case MAPNAME_KEY:
  ...
  DEBUG_LOG(("QR_SERVER_KEY | %d (%s) = [%s]\n", key,
             qr2_registered_key_list[key], val.str()));
}

static void QRPlayerKeyCallback(PEER peer, int key, int index,
...
static void QRTeamKeyCallback(PEER peer, int key, int index,
                              qr2_buffer_t buffer, void *param) {
...
static void QRKeyListCallback(PEER peer, qr2_key_type type,
...
static int QRCountCallback(PEER peer, qr2_key_type type, void *param) {
  ...
  if (type == key_player) {
    DEBUG_LOG(("QR_COUNT | %s = %d\n", KeyTypeToString(type),
               t->getNumPlayers() + t->getNumObservers()));
...
static void QRAddErrorCallback(PEER peer, qr2_error_t error, char *errorString,
                               void *param) {
  DEBUG_LOG(
...
static void KickedCallback(PEER peer, RoomType roomType, const char *nick,
                           const char *reason, void *param) {
...
static void AuthenticateCDKeyCallback(PEER peer, int result,
...
static SerialAuthResult doCDKeyAuthentication(PEER peer) {
  ...
  if (GetStringFromRegistry("\\ergc", "", s) && s.isNotEmpty()) {
    ...
    CheckServers(peer);
#endif  // SERVER_DEBUGGING
    peerAuthenticateCDKey(peer, s.str(), AuthenticateCDKeyCallback, &retval,
                          PEERTrue);
#ifdef SERVER_DEBUGGING
...
void checkQR2Queries(PEER peer, SOCKET sock) {
  ...
  while (1) {
    ...
    if (SOCKET_ERROR == error || 0 == error) return;
    // else we have data
    error = recvfrom(sock, indata, INBUF_LEN - 1, 0, (struct sockaddr *)&saddr,
                     &saddrlen);
...
void PeerThreadClass::Thread_Function() {
  try {
    ...
    const char *allKeys = "\\pid_\\mapname\\gamever\\gamename" \
            ...
            "\\" NAME__STR "_" "\\" WINS__STR "_" "\\" LOSSES__STR "_" "\\"
    FACTION__STR "_" "\\" COLOR__STR "_";
    */
    ...
    **********/
    ...
    // Set the title.
    /////////////////
    if (!peerSetTitle(peer, gameName, secretKey, gameName, secretKey,
                      GetRegistryVersion(), 30, PEERTrue, pingRooms,
                      crossPingRooms)) {
    ...
    while (running) {
      ...
      if (TheGameSpyPeerMessageQueue->getRequest(incomingRequest)) {
        DEBUG_LOG((
            "TheGameSpyPeerMessageQueue->getRequest() got request of type %d\n",
        ...
        switch (incomingRequest.peerRequestType) {
          case PeerRequest::PEERREQUEST_LOGIN: {
            ...
            peerConnect(peer, incomingRequest.nick.c_str(),
                        incomingRequest.login.profileID,
                        nickErrorCallbackWrapper, connectCallbackWrapper, this,
                        PEERTrue);
#ifdef SERVER_DEBUGGING
            ...
            // if (m_isConnected)
            //{
            //	GetLocalChatConnectionAddress("peerchat.gamespy.com", 6667,
            //localIP);
            //}
          }
          ...
          case PeerRequest::PEERREQUEST_JOINGROUPROOM:
            ...
            DEBUG_LOG(("Requesting to join room %d in thread %X\n",
                       m_localRoomID, this));
            peerJoinGroupRoom(peer, incomingRequest.groupRoom.id,
                              joinRoomCallback, (void *)this, PEERTrue);
            break;
          ...
          case PeerRequest::PEERREQUEST_JOINSTAGINGROOM: {
            ...
            if (server) {
              peerJoinStagingRoom(peer, server,
                                  incomingRequest.password.c_str(),
                                  joinRoomCallback, (void *)this, PEERTrue);
            } else {
              PeerResponse resp;
              resp.peerResponseType =
                  PeerResponse::PEERRESPONSE_JOINSTAGINGROOM;
          ...
          case PeerRequest::PEERREQUEST_MESSAGEPLAYER: {
            std::string s =
                WideCharStringToMultiByte(incomingRequest.text.c_str());
            peerMessagePlayer(peer, incomingRequest.nick.c_str(), s.c_str(),
                              (incomingRequest.message.isAction)
                                  ? ActionMessage
                                  : NormalMessage);
          } break;
          ...
          case PeerRequest::PEERREQUEST_MESSAGEROOM: {
            std::string s =
                WideCharStringToMultiByte(incomingRequest.text.c_str());
            peerMessageRoom(peer, (m_groupRoomID) ? GroupRoom : StagingRoom,
                            s.c_str(),
                            (incomingRequest.message.isAction) ? ActionMessage
                                                               : NormalMessage);
          } break;
          ...
          case PeerRequest::PEERREQUEST_PUSHSTATS: {
...
#ifdef USE_BROADCAST_KEYS
            _snprintf(s_valueBuffers[0], 20, "%d",
                      incomingRequest.statsToPush.locale);
            _snprintf(s_valueBuffers[1], 20, "%d",
                      incomingRequest.statsToPush.wins);
            _snprintf(s_valueBuffers[2], 20, "%d",
                      incomingRequest.statsToPush.losses);
            _snprintf(s_valueBuffers[3], 20, "%d",
                      incomingRequest.statsToPush.rankPoints);
            _snprintf(s_valueBuffers[4], 20, "%d",
                      incomingRequest.statsToPush.side);
            _snprintf(s_valueBuffers[5], 20, "%d",
                      incomingRequest.statsToPush.preorder);
            pushStatsToRoom(peer);
#else
            const char *keys[6] = {"locale", "wins", "losses",
                                   "points", "side", "pre"};
            char valueStrings[6][20];
            char *values[6] = {valueStrings[0], valueStrings[1],
                               valueStrings[2], valueStrings[3],
                               valueStrings[4], valueStrings[5]};
            ...
            _snprintf(values[3], 20, "%d",
                      incomingRequest.statsToPush.rankPoints);
            _snprintf(values[4], 20, "%d", incomingRequest.statsToPush.side);
            _snprintf(values[5], 20, "%d",
                      incomingRequest.statsToPush.preorder);
            peerSetGlobalKeys(peer, 6, (const char **)keys,
                              (const char **)values);
          ...
          case PeerRequest::PEERREQUEST_SETGAMEOPTIONS: {
            ...
            DEBUG_LOG(
                ("peerStateChanged(): Marking game options state as changed - "
                 "%d players, %d observers\n",
            ...
            prev = now;
#endif
            STATECHANGED_LOG(("peerStateChanged() at time %d (difference of %d
ms)\n", now, diff));
            */

            peerUTMRoom(
                peer, StagingRoom, "SL/", incomingRequest.options.c_str(),
                PEERFalse);  // send the full string to people in the room
          } break;
          ...
          case PeerRequest::PEERREQUEST_CREATESTAGINGROOM: {
            ...
            if (bind(qr2Sock, (sockaddr *)&saddr, sizeof(saddr)) != 0) {
              DEBUG_LOG(
                  ("Could not bind to %d!  Falling back to GameSpy's default "
                   "port\n",
                   preferredQRPort));
              closesocket(qr2Sock);
              qr2Sock = INVALID_SOCKET;
            ...
            peerCreateStagingRoomWithSocket(
                peer, compositeGame.c_str(), MAX_SLOTS,
                incomingRequest.password.c_str(), qr2Sock, preferredQRPort,
                createRoomCallback, (void *)&res, PEERTrue);
            // peerCreateStagingRoomWithSocket(peer,
            // WideCharStringToMultiByte(incomingRequest.text.c_str()).c_str(),
            // MAX_SLOTS, incomingRequest.password.c_str(), qr2Sock,
            // preferredQRPort, createRoomCallback, (void *)&res, PEERTrue);
            DEBUG_LOG(
                ("PEERREQUEST_CREATESTAGINGROOM - creating staging room, name "
                 "is %ls, passwd is %s, result = %d\n",
                 incomingRequest.text.c_str(), incomingRequest.password.c_str(),
                 res));

            PeerResponse resp;
            resp.peerResponseType =
                PeerResponse::PEERRESPONSE_CREATESTAGINGROOM;
            ...
            if (res != PEERJoinSuccess && res != PEERAlreadyInRoom) {
              ...
              if (incomingRequest.stagingRoomCreation.restrictGameList) {
                ...
                peerJoinGroupRoom(peer, oldGroupID, joinRoomCallback,
                                  (void *)this, PEERTrue);
              }
              m_isHosting = FALSE;
              m_localStagingServerName = L"";
              ...
              s_lastStateChangedHeartbeat =
                  timeGetTime();  // wait the full interval before updating
                                  // state
              s_wantStateChangedHeartbeat = FALSE;
              m_isHosting = TRUE;
              m_allowObservers =
                  incomingRequest.stagingRoomCreation.allowObservers;
              m_mapName = "";
              for (Int i = 0; i < MAX_SLOTS; ++i) {
                m_playerNames[i] = "";
                m_playerWins[i] = 0;
              ...
              updateBuddyStatus(
                  BUDDY_STAGING, 0,
                  WideCharStringToMultiByte(m_localStagingServerName.c_str()));
            }
          } break;
          ...
          case PeerRequest::PEERREQUEST_STARTGAMELIST: {
            ...
            peerStartListingGames(
                peer, allKeysArray, NumKeys,
                (incomingRequest.gameList.restrictGameList ? "~" : NULL),
                listingGamesCallback, this);
          } break;
          ...
          case PeerRequest::PEERREQUEST_UTMPLAYER: {
            if (incomingRequest.nick.length() > 0) {
              peerUTMPlayer(peer, incomingRequest.nick.c_str(),
                            incomingRequest.id.c_str(),
                            incomingRequest.options.c_str(), PEERFalse);
            }
          } break;
          ...
          case PeerRequest::PEERREQUEST_UTMROOM: {
            peerUTMRoom(
                ...
                incomingRequest.id.c_str(), incomingRequest.options.c_str(),
                PEERFalse);
          } break;
...
#endif
          STATECHANGED_LOG(
              ("peerStateChanged() at time %d (difference of %d ms)\n", now,
               diff));
        }
      }
...
static void qmProfileIDCallback(PEER peer, PEERBool success, const char *nick,
                                int profileID, void *param) {
  Int *i = (Int *)param;
  if (!i || !success || !nick) return;

  *i = profileID;
}

static Int matchbotProfileID = 0;
void quickmatchEnumPlayersCallback(PEER peer, PEERBool success,
                                   RoomType roomType, int index,
      ...
      if (TheGameSpyPeerMessageQueue->getRequest(incomingRequest)) {
        switch (incomingRequest.peerRequestType) {
          case PeerRequest::PEERREQUEST_WIDENQUICKMATCHSEARCH: {
            if (m_qmStatus != QM_IDLE && m_qmStatus != QM_STOPPED &&
                m_sawMatchbot) {
              peerMessagePlayer(peer, m_matchbotName.c_str(), "\\WIDEN",
                                NormalMessage);
            }
          } break;
          ...
          default: {
            DEBUG_CRASH(("Unanticipated request %d to peer thread!",
                         incomingRequest.peerRequestType));
          } break;
        }
      }
      ...
      if (!done) {
        ...
        switch (m_qmStatus) {
          case QM_JOININGQMCHANNEL: {
            ...
            DEBUG_LOG(("Requesting to join room %d in thread %X\n",
                       m_localRoomID, this));
            peerJoinGroupRoom(peer, m_localRoomID, joinRoomCallback,
                              (void *)this, PEERTrue);
            if (m_roomJoined) {
              resp.peerResponseType =
                  PeerResponse::PEERRESPONSE_QUICKMATCHSTATUS;
              ...
              peerEnumPlayers(peer, GroupRoom, quickmatchEnumPlayersCallback,
                              this);
            } else {
              resp.peerResponseType =
                  PeerResponse::PEERRESPONSE_QUICKMATCHSTATUS;
          ...
          case QM_LOOKINGFORBOT: {
            if (m_sawEndOfEnumPlayers) {
              if (m_sawMatchbot) {
                ...
                msg.append(buf);
                _snprintf(buf, 63, "\\PointsMin\\%d",
                          m_qmInfo.QM.minPointPercentage);
                msg.append(buf);
                _snprintf(buf, 63, "\\PointsMax\\%d",
                          m_qmInfo.QM.maxPointPercentage);
                msg.append(buf);
                ...
                _snprintf(
                    buf, 63, "\\IP\\%d",
                    ntohl(peerGetLocalIP(
                        peer)));  // not ntohl(localIP), as we need EXTERNAL
                ...
                DEBUG_LOG(("Sending QM options of [%s] to %s\n", msg.c_str(),
                           m_matchbotName.c_str()));
                peerMessagePlayer(peer, m_matchbotName.c_str(), msg.c_str(),
                                  NormalMessage);
...
static void getPlayerProfileIDCallback(PEER peer, PEERBool success,
                                       const char *nick, int profileID,
                                       void *param) {
...
static void stagingRoomPlayerEnum(PEER peer, PEERBool success,
                                  RoomType roomType, int index,
                                  const char *nick, int flags, void *param) {
  DEBUG_LOG(("Enum: success=%d, index=%d, nick=%s, flags=%d\n", success, index,
             nick, flags));
  if (!nick || !success) return;
...
/////////////////////////////////////////////////////////////////
static void listGroupRoomsCallback(PEER peer, PEERBool success, int groupID,
                                   SBServer server, const char *name,
                                   int numWaiting, int maxWaiting, int numGames,
  ...
  if (success) {
    DEBUG_LOG(("Saw group room of %d (%s) at address %X %X\n", groupID, name,
               server, (server) ? server->keyvals : 0));
    PeerResponse resp;
...
void roomMessageCallback(PEER peer, RoomType roomType, const char *nick,
  ...
  DEBUG_LOG(("Saw text [%hs] (%ls) %d chars Orig was %s (%d chars)\n", nick,
             resp.text.c_str(), resp.text.length(), message, strlen(message)));
...
void gameStartedCallback(PEER peer, UnsignedInt IP, const char *message,
                         void *param) {
  PeerResponse resp;
    ...
    if (resp.message.isPrivate && resp.message.profileID == matchbotProfileID) {
      ...
      if (cmd && strcmp(cmd, "MBOT:MATCHED") == 0) {
        ...
        if (numPlayers > 1) {
          ...
          t->handleQMMatch(peer, atoi(mapNumStr), atoi(seedStr), playerStr,
                           playerIPStr, playerSideStr, playerColorStr,
                           playerNATStr);
        }
      } else if (cmd && strcmp(cmd, "MBOT:WORKING") == 0) {
        Int poolSize = 0;
        char *poolStr = strtok_r(NULL, " ", &lastStr);
        if (poolStr) poolSize = atoi(poolStr);
        PeerResponse resp;
        resp.peerResponseType = PeerResponse::PEERRESPONSE_QUICKMATCHSTATUS;
...
static void getPlayerInfo(PeerThreadClass *t, PEER peer, const char *nick,
                          Int &id, UnsignedInt &IP, std::string &locale,
                          Int &wins, Int &losses, Int &rankPoints, Int &side,
                          Int &preorder, RoomType roomType, Int &flags) {
  if (!t || !nick) return;
  ...
  DEBUG_LOG(
      ("getPlayerInfo(%d) - %s has locale %s, wins:%d, losses:%d, "
       "rankPoints:%d, side:%d, preorder:%d\n",
...
static void roomKeyChangedCallback(PEER peer, RoomType roomType,
  ...
  PeerThreadClass *t = (PeerThreadClass *)param;
  DEBUG_ASSERTCRASH(t, ("No Peer thread!"));
  DEBUG_ASSERTCRASH(nick && key && val,
                    ("Bad values %X %X %X\n", nick, key, val));
  if (!t || !nick || !key || !val) {
...
void getRoomKeysCallback(PEER peer, PEERBool success, RoomType roomType,
  ...
  if (!t || !nick || !num || !success || !keys || !values) return;
...
static void listingGamesCallback(PEER peer, PEERBool success, const char *name,
  ...
  if (!t || !success || (!name && (msg == PEER_ADD || msg == PEER_UPDATE))) {
    DEBUG_LOG(
        ("Bailing from listingGamesCallback() - success=%d, name=%X, "
         "server=%X, msg=%X\n",
  ...
  if (server && (msg == PEER_ADD || msg == PEER_UPDATE)) {
    DEBUG_ASSERTCRASH(
        server->keyvals,
        ("Looking at an already-freed server for msg type %d!", msg));
    if (!server->keyvals) {
      msg = PEER_REMOVE;
    }
  }
  ...
  if (server && success && (msg == PEER_ADD || msg == PEER_UPDATE)) {
    ...
    const char *newname =
        SBServerGetStringValue(server, "gamename", (char *)name);
    if (strcmp(newname, "ccgenerals")) name = newname;
    DEBUG_LOG(("Game name is now '%s'\n", name));
  }

  DEBUG_LOG(
      ("listingGamesCallback - got percent complete %d\n", percentListed));
  if (percentListed == 100) {
    if (!t->getSawCompleteGameList()) {
      t->setSawCompleteGameList(TRUE);
      PeerResponse completeResp;
      completeResp.peerResponseType =
          PeerResponse::PEERRESPONSE_STAGINGROOMLISTCOMPLETE;
  ...
  if (server && (msg == PEER_ADD || msg == PEER_UPDATE)) {
    ...
    const char *pingStr =
        SBServerGetStringValue(server, PINGSTR_STR, "FFFFFFFFFFFFFFFF");
    UnsignedShort ladPort =
        (UnsignedShort)SBServerGetIntValue(server, LADPORT_STR, 0);
    ...
    for (Int i = 0; i < MAX_SLOTS; ++i) {
      resp.stagingRoomPlayerNames[i] =
          SBServerGetPlayerStringValue(server, i, NAME__STR, "");
      resp.stagingRoom.wins[i] =
          SBServerGetPlayerIntValue(server, i, WINS__STR, 0);
      resp.stagingRoom.losses[i] =
          SBServerGetPlayerIntValue(server, i, LOSSES__STR, 0);
      resp.stagingRoom.profileID[i] =
          SBServerGetPlayerIntValue(server, i, "pid", 0);
      resp.stagingRoom.color[i] =
          SBServerGetPlayerIntValue(server, i, COLOR__STR, 0);
      resp.stagingRoom.faction[i] =
          SBServerGetPlayerIntValue(server, i, FACTION__STR, 0);
#ifdef DEBUG_LOGGING
      if (resp.stagingRoomPlayerNames[i].length()) {
        DEBUG_LOG(("Player %d raw stuff: [%s] [%d] [%d] [%d]\n", i,
                   resp.stagingRoomPlayerNames[i].c_str(),
                   resp.stagingRoom.wins[i], resp.stagingRoom.losses[i],
                   resp.stagingRoom.profileID[i]));
      }
#endif
    }
    ...
    DEBUG_ASSERTCRASH(resp.stagingRoomPlayerNames[0].empty() == false,
                      ("No host!"));
    DEBUG_LOG(("Raw stuff: [%s] [%s] [%s] [%d] [%d]\n", verStr, exeStr, iniStr,
               hasPassword, allowObservers));
    DEBUG_LOG(("Raw stuff: [%s] [%s] [%d]\n", pingStr, ladIPStr, ladPort));
    DEBUG_LOG(("Saw game with stuff %s %d %X %X %X %s\n",
               resp.stagingRoomMapName.c_str(), hasPassword, verVal, exeVal,
               iniVal, SBServerGetStringValue(server, "password", "missing")));
#ifdef PING_TEST
    PING_LOG(("%s\n", pingStr));
#endif
  }
  ...
  if (msg == PEER_ADD || msg == PEER_UPDATE) {
    if (!resp.stagingRoom.exeCRC || !resp.stagingRoom.iniCRC) {
      ...
      if (msg == PEER_UPDATE) {
        ...
        DEBUG_LOG(
            ("Add/update a 0/0 server %X (%d, %s) - requesting full update to "
             "see if that helps.\n",
