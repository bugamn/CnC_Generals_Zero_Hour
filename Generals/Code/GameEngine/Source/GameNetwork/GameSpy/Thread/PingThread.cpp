...
Bool Pinger::areThreadsRunning(void) {
  for (Int i = 0; i < NumWorkerThreads; ++i) {
    if (m_workerThreads[i]) {
      if (m_workerThreads[i]->Is_Running()) return true;
    }
  }
  return false;
}
...
Int Pinger::getPing(AsciiString hostname) {
  ...
  if (it != m_pingMap.end()) return it->second;
...
AsciiString Pinger::getPingString(Int timeout) {
  ...
  for (std::map<std::string, Int>::const_iterator it = m_pingMap.begin();
       it != m_pingMap.end(); ++it) {
    Int ping = it->second;
    if (ping < 0 || ping > timeout) ping = timeout;
...
Int PingThreadClass::doPing(UnsignedInt IP, Int timeout) {
   ...
   */
  lpfnIcmpCreateFile = (void *(__stdcall *)(void))GetProcAddress(
  ...
  if (dwReplyCount != 0) {
    ...
    //   bit.  How lame.
    if (pingTime > timeout) pingTime = timeout;
...
cleanup:
  ...
  // Shut down...
  if (hICMP_DLL) FreeLibrary((HINSTANCE)hICMP_DLL);
