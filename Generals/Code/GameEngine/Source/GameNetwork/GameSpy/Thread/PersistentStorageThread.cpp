...
// ////////////////////////////////////////////////////// GameSpy Persistent
// Storage thread This thread communicates with GameSpy's persistent storage
// server and talks through a message queue with the rest of the game. Author:
// Matthew D. Campbell, July 2002
...
GameSpyPSMessageQueueInterface::createNewMessageQueue(void) {
...
class PSThreadClass : public ThreadClass {
 ...
 private:
  ...
  Bool tryLogin(Int id, std::string nick, std::string password,
                std::string email);
  Bool m_loginOK;
...
GameSpyPSMessageQueue::~GameSpyPSMessageQueue() { endThread(); }
...
PSThreadClass *GameSpyPSMessageQueue::getThread(void) { return m_thread; }
...
Bool PSThreadClass::tryConnect(void) {
  ...
  // this may block for 1-2 seconds (according to GS) so it's nice we're not in
  // the UI thread :)
  result = InitStatsConnection(0);

#ifdef DEBUG_LOGGING
  static const char *retValStrings[6] = {"GE_NOERROR", "GE_NOSOCKET",
                                         "GE_NODNS",   "GE_NOCONNECT",
                                         "GE_BUSY",    "GE_DATAERROR"};
#endif  // DEBUG_LOGGING

  if (result != GE_NOERROR) {
    DEBUG_LOG(("InitStatsConnection() returned %d (%s)\n", result,
               retValStrings[result]));
    return false;
  } else {
...
static void persAuthCallback(int localid, int profileid, int authenticated,
  ...
  if (t) t->persAuthCallback(authenticated != 0);
}

Bool PSThreadClass::tryLogin(Int id, std::string nick, std::string password,
                             std::string email) {
  char validate[33];
  DEBUG_LOG((
      "PSThreadClass::tryLogin id = %d, nick = %s, password = %s, email = %s\n",
      id, nick.c_str(), password.c_str(), email.c_str()));
  /***********
  We'll go ahead and start the authentication, using a Presence & Messaging SDK
  profileid / password.  To generate the new validation token, we'll need to
  pass in the password for the profile we are authenticating. Again, if this is
  done in a client/server setting, with the Persistent Storage access being done
  on the server, and the P&M SDK is used on the client, the server will need to
  send the challenge (GetChallenge(NULL)) to the client, the client will create
  the validation token using GenerateAuth, and send it back to the server for
  use in PreAuthenticatePlayerPM
  ***********/
  char *munkeeHack = strdup(
      password.c_str());  // GenerateAuth takes a char*, not a const char* :P
  ...
  We pass the same authentication callback as for the first user, but a
  different localid this time.
  ************/
  m_loginOK = false;
...
static void getPersistentDataCallback(int localid, int profileid,
                                      persisttype_t type, int index,
                                      int success, char *data, int len,
                                      void *instance) {
  DEBUG_LOG(
      ("Data get callback: localid: %d profileid: %d success: %d len: %d data: "
       "%s\n",
  ...
  if (profileid == MESSAGE_QUEUE->getLocalPlayerID()) {
    ...
    userPrefFilename.format("GeneralsOnline\\MiscPref%d.ini",
    ...
    if (addedInDesyncs2 || addedInDesyncs3 || addedInDesyncs4 ||
        addedInDiscons2 || addedInDiscons3 || addedInDiscons4) {
      DEBUG_LOG(
          ("We have a previous discon we can attempt to update!  Bummer...\n"));
      ...
      req.player =
          GameSpyPSMessageQueueInterface::parsePlayerKVPairs((len) ? data : "");
...
static void setPersistentDataLocaleCallback(int localid, int profileid,
                                            persisttype_t type, int index,
                                            int success, void *instance) {
...
static void setPersistentDataCallback(int localid, int profileid,
                                      persisttype_t type, int index,
                                      int success, void *instance) {
  DEBUG_LOG(("Data save callback: localid: %d profileid: %d success: %d\n",
             localid, profileid, success));
...
void preAuthCDCallback(int localid, int profileid, int authenticated,
                       char *errmsg, void *instance) {
...
static void getPreorderCallback(int localid, int profileid, persisttype_t type,
      ...
      if (TheGameSpyPSMessageQueue->getRequest(req)) {
        switch (req.requestType) {
          ...
          case PSRequest::PSREQUEST_READPLAYERSTATS: {
            ...
            if (!MESSAGE_QUEUE->getLocalPlayerID()) {
              MESSAGE_QUEUE->setLocalPlayerID(
                  req.player.id);  // first request is for ourselves
              ...
              DEBUG_LOG(("Setting email/nick/password = %s/%s/%s\n",
                         req.email.c_str(), req.nick.c_str(),
                         req.password.c_str()));
            ...
            if (tryConnect()) {
              ...
              incrOpCount();
              GetPersistDataValues(0, req.player.id, pd_public_rw, 0, "",
                                   getPersistentDataCallback, this);
            } else {
              ...
              if (initialConnection) {
                // we haven't gotten stats for ourselves - try again
                DEBUG_LOG(
                    ("Requesting retry for reading local player's stats\n"));
                PSRequest req;
                req.requestType = PSRequest::PSREQUEST_READPLAYERSTATS;
                req.player.id = MESSAGE_QUEUE->getLocalPlayerID();
                TheGameSpyPSMessageQueue->addRequest(req);
          ...
          case PSRequest::PSREQUEST_UPDATEPLAYERLOCALE: {
            ...
            if (tryConnect() &&
                tryLogin(req.player.id, req.nick, req.password, req.email)) {
              char kvbuf[256];
              sprintf(kvbuf, "\\locale\\%d", req.player.locale);
              incrOpCount();
              SetPersistDataValues(0, req.player.id, pd_public_rw, 0, kvbuf,
                                   setPersistentDataLocaleCallback, this);
            }
          } break;
          case PSRequest::PSREQUEST_UPDATEPLAYERSTATS: {
            /*
            ** NOTE THAT THIS IS HIGHLY DEPENDENT ON INI ORDERING FOR THE
            *PLAYERTEMPLATES!!!
            */
            ...
            DEBUG_LOG(
                ("req.addDesync=%d, req.addDiscon=%d, addedInDesync=%d,%d,%d, "
                 "addedInDiscon=%d,%d,%d\n",
                 req.addDesync, req.addDiscon, addedInDesyncs2, addedInDesyncs3,
                 addedInDesyncs4, addedInDiscons2, addedInDiscons3,
                 addedInDiscons4));
            ...
            if (req.addDesync || req.addDiscon) {
              ...
              if (req.lastHouse == 2) {
                ...
                DEBUG_LOG(("house 2 req.addDesync || req.addDiscon: %d %d\n",
                           addedInDesyncs2 + req.addDesync,
                           addedInDiscons2 + req.addDiscon));
                ...
                DEBUG_LOG(("house 3 req.addDesync || req.addDiscon: %d %d\n",
                           addedInDesyncs3 + req.addDesync,
                           addedInDiscons3 + req.addDiscon));
              } else {
                ...
                DEBUG_LOG(("house 4 req.addDesync || req.addDiscon: %d %d\n",
                           addedInDesyncs4 + req.addDesync,
                           addedInDiscons4 + req.addDiscon));
              }
              pref.write();
            ...
            DEBUG_LOG(("House2: %d/%d/%d, House3: %d/%d/%d, House4: %d/%d/%d\n",
                       req.player.desyncs[2], req.player.discons[2],
                       req.player.games[2], req.player.desyncs[3],
                       req.player.discons[3], req.player.games[3],
                       req.player.desyncs[4], req.player.discons[4],
                       req.player.games[4]));
            if (tryConnect() &&
                tryLogin(req.player.id, req.nick, req.password, req.email)) {
              DEBUG_LOG(("Logged in!\n"));
              ...
              char *munkeeHack =
                  strdup(GameSpyPSMessageQueueInterface::formatPlayerKVPairs(
                             req.player)
                             .c_str());  // GS takes a char* for some reason
              incrOpCount();
              DEBUG_LOG(("Setting values %s\n", munkeeHack));
              SetPersistDataValues(0, req.player.id, pd_public_rw, 0,
                                   munkeeHack, setPersistentDataCallback, this);
              free(munkeeHack);
            } else {
          ...
          case PSRequest::PSREQUEST_READCDKEYSTATS: {
            ...
            if (tryConnect()) {
              ...
              char *munkeeHack =
                  strdup(req.cdkey.c_str());  // GenerateAuth takes a char*, not
                                              // a const char* :P

              GenerateAuth(GetChallenge(NULL), munkeeHack,
                           validationToken);            // validation token
              GenerateAuth("", munkeeHack, cdkeyHash);  // cdkey hash

              free(munkeeHack);

              PreAuthenticatePlayerCD(0, "preorder", cdkeyHash, validationToken,
                                      preAuthCDCallback, &cdAuthInfo);

              while (running && IsStatsConnected() && !cdAuthInfo.done)
                PersistThink();

              DEBUG_LOG(
                  ("Looking for preorder status for %d (success=%d, done=%d) "
                   "from CDKey %s with hash %s\n",
                   cdAuthInfo.id, cdAuthInfo.success, cdAuthInfo.done,
                   req.cdkey.c_str(), cdkeyHash));
              if (cdAuthInfo.done && cdAuthInfo.success)
                GetPersistDataValues(0, cdAuthInfo.id, pd_public_ro, 0,
                                     "\\preorder", getPreorderCallback, this);
              else
                decrOpCount();
            }
          } break;
...
//-------------------------------------------------------------------------
PSPlayerStats::PSPlayerStats(void) { reset(); }
...
PSPlayerStats GameSpyPSMessageQueueInterface::parsePlayerKVPairs(
    std::string kvPairs) {
  PSPlayerStats s;
  kvPairs.append("\\");
  ...
  while (1) {
    ...
    if (firstMarker < 0) break;
    Int secondMarker = kvPairs.find_first_of('\\', firstMarker + 1);
    if (secondMarker < 0) break;
    Int thirdMarker = kvPairs.find_first_of('\\', secondMarker + 1);
    if (thirdMarker < 0) break;
    Int generalMarker =
        kvPairs.find_last_not_of("0123456789", secondMarker - 1);
    std::string k, v, g;
    ...
    // DEBUG_ASSERTCRASH(generalMarker >= 0, ("Unknown KV Pair in persistent
    // storage: [%s] = [%s]\n", k.c_str(), v.c_str()));
    // DEBUG_ASSERTCRASH(generalMarker  < 0, ("Unknown KV Pair in persistent
    // storage for PlayerTemplate %d: [%s] = [%s]\n", generalMarker, k.c_str(),
    // v.c_str()));
  }
...
std::string GameSpyPSMessageQueueInterface::formatPlayerKVPairs(
    PSPlayerStats stats) {
  char kvbuf[256];
  std::string s = "";
  ...
  for (Int ptIdx = 0; ptIdx < ThePlayerTemplateStore->getPlayerTemplateCount();
       ++ptIdx) {
    //		const PlayerTemplate* pTemplate =
    //ThePlayerTemplateStore->getNthPlayerTemplate(ptIdx); 		const GeneralPersona*
    //pGeneral =
    //TheChallengeGenerals->getGeneralByTemplateName(pTemplate->getName()); 		BOOL
    //isReported = pGeneral ? pGeneral->isStartingEnabled() : FALSE; 		if(
    //!isReported ) 			continue;  //don't report unplayable templates (observer,
    //boss, etc.)
