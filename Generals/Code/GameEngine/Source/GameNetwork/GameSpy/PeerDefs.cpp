...
void GameSpyInfo::readAdditionalDisconnects(void) {
  m_additionalDisconnects =
      GetAdditionalDisconnectsFromUserFile(m_localProfileID);
...
Int GameSpyInfo::getAdditionalDisconnects(void) {
  DEBUG_LOG(
      ("GameSpyInfo::getAdditionalDisconnects() would have returned %d.  "
       "Returning 0 instead.\n",
       m_additionalDisconnects));
  return 0;
}
...
Bool GameSpyInfo::amIHost(void) { return m_isHosting; }

GameSpyStagingRoom *GameSpyInfo::getCurrentStagingRoom(void) {
  if (m_isHosting || m_joinedStagingRoom) return &m_localStagingRoom;

  StagingRoomMap::iterator it = m_stagingRooms.find(m_joinedStagingRoom);
  if (it != m_stagingRooms.end()) return it->second;
...
void GameSpyInfo::setGameOptions(void) {
  ...
  Int i;
  AsciiString mapName =
      TheGameState->realMapPathToPortableMapPath(m_localStagingRoom.getMap());
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (!slot->isOccupied()) {
      ...
      if (slot->isHuman()) {
        ...
        req.gameOptions
            .wins[req.gameOptions.numObservers + req.gameOptions.numPlayers] =
            wins;
        req.gameOptions
            .losses[req.gameOptions.numObservers + req.gameOptions.numPlayers] =
            losses;
        req.gameOptions.profileID[req.gameOptions.numObservers +
                                  req.gameOptions.numPlayers] = profileID;
        req.gameOptions.faction[req.gameOptions.numObservers +
                                req.gameOptions.numPlayers] =
            slot->getPlayerTemplate();
        req.gameOptions
            .color[req.gameOptions.numObservers + req.gameOptions.numPlayers] =
            slot->getColor();
        ...
        req.gameOptsPlayerNames[i] =
            playerName.str();  // name is unused - we go off of the profileID
        req.gameOptions
            .wins[req.gameOptions.numObservers + req.gameOptions.numPlayers] =
            0;
        req.gameOptions
  ...
  req.gameOptions.maxPlayers =
      numOpenSlots + req.gameOptions.numPlayers + req.gameOptions.numObservers;
...
void GameSpyInfo::addGroupRoom(GameSpyGroupRoom room) {
  if (room.m_groupID == 0) {
    ...
    Int numRooms = 0;
    for (iter = getGroupRoomList()->begin(); iter != getGroupRoomList()->end();
         ++iter) {
    ...
    if (!names.empty() && names.size() != numRooms) {
      ...
      for (iter = TheGameSpyInfo->getGroupRoomList()->begin();
        ...
        GameSpyGroupRoom room = iter->second;
        if (room.m_groupID != TheGameSpyConfig->getQMChannel()) {
          room.m_translatedName.format(L"%ls %d", names[nameIndex].str(),
                                       timesThrough);
          nameIndex = (nameIndex + 1) % names.size();
          m_groupRooms[room.m_groupID] = room;
          if (!nameIndex) {
            // we've looped through the name list already.  increment the
            // timesThrough counter
            ++timesThrough;
          }
        }
      }
...
void GameSpyInfo::joinBestGroupRoom(void) {
  if (m_currentGroupRoomID) {
    DEBUG_LOG(
        ("Bailing from GameSpyInfo::joinBestGroupRoom() - we were already in a "
         "room\n"));
    m_currentGroupRoomID = 0;
    return;
  }
  ...
  if (m_groupRooms.size()) {
    ...
    while (iter != m_groupRooms.end()) {
      ...
      if (TheGameSpyConfig->getQMChannel() != room.m_groupID &&
          minPlayers > 25 && room.m_numWaiting < minPlayers) {
    ...
    if (minID > 0) {
      ...
      GSMessageBoxOk(TheGameText->fetch("GUI:Error"),
                     TheGameText->fetch("GUI:GSGroupRoomJoinFail"), NULL);
    }
  } else {
    GSMessageBoxOk(TheGameText->fetch("GUI:Error"),
                   TheGameText->fetch("GUI:GSGroupRoomJoinFail"), NULL);
...
void GameSpyInfo::updatePlayerInfo(PlayerInfo pi, AsciiString oldNick) {
  if (!oldNick.isEmpty()) playerLeftGroupRoom(oldNick);

  m_playerInfoMap[pi.m_name] = pi;

  if (pi.m_preorder != 0) markPlayerAsPreorder(pi.m_profileID);
}
...
void GameSpyInfo::clearStagingRoomList(void) {
  ...
  if (numRoomsRemoved > 0) {
    // m_stagingRoomsDirty = true; // only consider ourselves dirty if we
    // actually removed some games.
  }
}
...
void GameSpyInfo::markAsStagingRoomHost(void) {
  ...
  m_localStagingRoom
      .adjustSlotsForMap();  // close slots that the map can't hold. BGC
}
...
void GameSpyInfo::setMOTD(const AsciiString &motd) { m_rawMotd = motd; }

const AsciiString &GameSpyInfo::getMOTD(void) { return m_rawMotd; }

void GameSpyInfo::setConfig(const AsciiString &config) { m_rawConfig = config; }

const AsciiString &GameSpyInfo::getConfig(void) { return m_rawConfig; }

// --------------------------------------------------------------
void SetUpGameSpy(const char *motdBuffer, const char *configBuffer) {
  if (!motdBuffer) motdBuffer = "";
  ...
  CreateDirectory(dir.str(), NULL);
  dir.format("%sGeneralsOnline\\Ladders",
             TheGlobalData->getPath_UserData().str());
  CreateDirectory(dir.str(), NULL);

  TheGameSpyBuddyMessageQueue =
      GameSpyBuddyMessageQueueInterface::createNewMessageQueue();
  TheGameSpyBuddyMessageQueue->startThread();

  TheGameSpyPeerMessageQueue =
      GameSpyPeerMessageQueueInterface::createNewMessageQueue();
...
void TearDownGameSpy(void) {
  ...
  if (TheGameSpyInfo && TheGameSpyInfo->getLocalProfileID()) {
    //		/* This was done on the score screen, so there is no need to do
    //it now.
    //		 *
    PSPlayerStats localPSStats = TheGameSpyPSMessageQueue->findPlayerStatsByID(
        TheGameSpyInfo->getLocalProfileID());
    if (localPSStats.id != 0) {
      GameSpyMiscPreferences mPref;
      mPref.setCachedStats(
          GameSpyPSMessageQueueInterface::formatPlayerKVPairs(localPSStats)
              .c_str());
      mPref.write();
    }
    //		*/
  ...
  // End our threads before we kill off the singletons they reference.  No
  // crashy-crash for you!
  if (TheGameSpyPSMessageQueue) TheGameSpyPSMessageQueue->endThread();
  if (TheGameSpyBuddyMessageQueue) TheGameSpyBuddyMessageQueue->endThread();
  if (TheGameSpyPeerMessageQueue) TheGameSpyPeerMessageQueue->endThread();
  if (ThePinger) ThePinger->endThreads();
...
IgnoreList GameSpyInfo::returnIgnoreList(void) { return m_ignoreList; }
...
Bool PlayerInfo::isIgnored(void) {
  return (m_profileID) ? TheGameSpyInfo->isSavedIgnored(m_profileID)
                       : TheGameSpyInfo->isIgnored(m_name);
}
...
void GameSpyInfo::setDisallowAsianText(Bool val) { m_disallowAsainText = val; }

void GameSpyInfo::setDisallowNonAsianText(Bool val) {
  m_disallowNonAsianText = val;
}

Bool GameSpyInfo::getDisallowAsianText(void) { return m_disallowAsainText; }
...
/**This function is used to force an update of player's gamespy stats with an
additional disconnection.  This is used upon starting a new game so that if user
disconnects prior to finishing game, the disconnection stays on the server.  If
he completes the game, we remove this extra disconnection inside of
populatePlayerInfo() on the ScoreScreen.  This seems like the only secure way to
handle this issue since users can abort the process before we can detect/log
disconnections.*/
void GameSpyInfo::updateAdditionalGameSpyDisconnections(Int count) {
  if (TheRecorder->isMultiplayer() && TheGameLogic->isInInternetGame()) {
    ...
    for (ptIdx = 0; ptIdx < ThePlayerTemplateStore->getPlayerTemplateCount();
         ++ptIdx) {
    ...
    if (anyAI || stats.id == 0 || myTemplate->isObserver() ||
        player->getPlayerType() != PLAYER_HUMAN || player->isPlayerObserver())
      return;
    ...
    if (disCons < 0) {
      DEBUG_LOG(
          ("updateAdditionalGameSpyDisconnections() - disconnection count "
           "below zero\n"));
      return;  // something is wrong here
    }
    stats.discons[ptIdx] =
        disCons;  // add an additional disconnection to their stats.
    ...
    GameSpyMiscPreferences mPref;
    mPref.setCachedStats(
        GameSpyPSMessageQueueInterface::formatPlayerKVPairs(stats).c_str());
    mPref.write();
  }
}
