...
// ////////////////////////////////////////////////////// Generals GameSpy
// GameInfo-related code Author: Matthew D. Campbell, July 2002
...
BOOL(__stdcall *SnmpExtensionInitPtr)(
    IN DWORD dwUpTimeReference, OUT HANDLE *phSubagentTrapEvent,
    OUT AsnObjectIdentifier *pFirstSupportedRegion);
BOOL(__stdcall *SnmpExtensionQueryPtr)(IN BYTE bPduType,
                                       IN OUT RFC1157VarBindList *pVarBindList,
...
/***********************************************************************************************
 * Get_Local_Chat_Connection_Address -- Which address are we using to talk to
 *the chat server? *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 10/27/00 3:24PM ST : Created *
 *=============================================================================================*/
Bool GetLocalChatConnectionAddress(AsciiString serverName,
  ...
  */
  static char _conn_state[][32] = {
      "?",           "CLOSED",    "LISTENING", "SYN_SENT",   "SEN_RECEIVED",
      "ESTABLISHED", "FIN_WAIT",  "FIN_WAIT2", "CLOSE_WAIT", "LAST_ACK",
      "CLOSING",     "TIME_WAIT", "DELETE_TCB"};
  ...
  *((unsigned long *)(&serverAddress[0])) = temp;
  ...
  DEBUG_LOG(("Host address is %d.%d.%d.%d\n", serverAddress[3],
             serverAddress[2], serverAddress[1], serverAddress[0]));
  ...
  */
  SnmpExtensionInitPtr = (int(__stdcall *)(
      unsigned long, void **,
      AsnObjectIdentifier *))GetProcAddress(mib_ii_dll, "SnmpExtensionInit");
  SnmpExtensionQueryPtr = (int(__stdcall *)(
      unsigned char, SnmpVarBindList *, long *,
      long *))GetProcAddress(mib_ii_dll, "SnmpExtensionQuery");
  SnmpUtilMemAllocPtr = (void *(__stdcall *)(unsigned long))GetProcAddress(
      snmpapi_dll, "SnmpUtilMemAlloc");
  SnmpUtilMemFreePtr =
      (void(__stdcall *)(void *))GetProcAddress(snmpapi_dll, "SnmpUtilMemFree");
  if (SnmpExtensionInitPtr == NULL || SnmpExtensionQueryPtr == NULL ||
      SnmpUtilMemAllocPtr == NULL || SnmpUtilMemFreePtr == NULL) {
  ...
  RFC1157VarBindList *bind_list_ptr =
      (RFC1157VarBindList *)SnmpUtilMemAllocPtr(sizeof(RFC1157VarBindList));
  RFC1157VarBind *bind_ptr =
      (RFC1157VarBind *)SnmpUtilMemAllocPtr(sizeof(RFC1157VarBind));

  /*
  ** OK, here we go. Try to initialise the .dll
  */
  DEBUG_LOG(("About to init INETMIB1.DLL\n"));
  int ok = SnmpExtensionInitPtr(GetCurrentTime(), &trap_handle,
  ...
  */
  ...
  unsigned int *mib_ii_name_ptr =
      (unsigned int *)SnmpUtilMemAllocPtr(sizeof(mib_ii_name));
  ...
  */
  while (true) {
    if (!SnmpExtensionQueryPtr(SNMP_PDU_GETNEXT, bind_list_ptr, &error_status,
                               &error_index)) {
      // if (!SnmpExtensionQueryPtr(ASN_RFC1157_GETNEXTREQUEST, bind_list_ptr,
    ...
    */
    ...
    switch (conn_entry_type) {
      /*
      ** 1. First field in the entry. Need to create a new connection info
      *struct
      ...
      */
      case tcpConnLocalAddress:
        DEBUG_ASSERTCRASH(index < connectionVector.size(),
                          ("Bad connection index"));
        connectionVector[index].LocalIP =
            *((unsigned long *)bind_ptr->value.asnValue.address.stream);
        index++;
        break;
      ...
      */
      case tcpConnLocalPort:
        ...
        // connectionVector[index]->LocalPort =
        // ntohs(connectionVector[index]->LocalPort);
        index++;
        break;
      ...
      */
      case tcpConnRemAddress:
        DEBUG_ASSERTCRASH(index < connectionVector.size(),
                          ("Bad connection index"));
        connectionVector[index].RemoteIP =
            *((unsigned long *)bind_ptr->value.asnValue.address.stream);
        index++;
        break;
      ...
      */
      case tcpConnRemPort:
        ...
        // connectionVector[index]->RemotePort =
        // ntohs(connectionVector[index]->RemotePort);
        index++;
        break;
    }
  }
  ...
  /*
  ** Right, we got the lot. Lets see if any of them have the same address as the
  *chat
  ...
  for (Int i = 0; i < connectionVector.size(); ++i) {
    ...
    */
    if (!found && memcmp(remoteAddress, serverAddress, 4) == 0) {
      ...
      if (serverPort == 0 ||
          serverPort == (unsigned int)connection.RemotePort) {
        ...
        */
        ...
        DEBUG_LOG(
            ("Connection has different port. Port is %d, looking for %d\n",
...
void GameSpyStagingRoom::cleanUpSlotPointers(void) {
  for (Int i = 0; i < MAX_SLOTS; ++i) setSlotPointer(i, &m_GameSpySlot[i]);
}

GameSpyGameSlot *GameSpyStagingRoom::getGameSpySlot(Int index) {
  GameSlot *slot = getSlot(index);
  DEBUG_ASSERTCRASH(slot && (slot == &(m_GameSpySlot[index])),
                    ("Bad game slot pointer\n"));
  return (GameSpyGameSlot *)slot;
}

void GameSpyStagingRoom::init(void) { GameInfo::init(); }
...
Int GameSpyStagingRoom::getLocalSlotNum(void) const {
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (slot->isPlayer(localName)) return i;
  }
  return -1;
}
...
void GameSpyStagingRoom::startGame(Int gameID) {
  ...
  DEBUG_ASSERTCRASH(m_transport == NULL,
                    ("m_transport is not NULL when it should be"));
    ...
    if (m_GameSpySlot[i].isHuman()) {
      ...
      if (m_isQM) {
        if (getLocalSlotNum() == i)
          m_GameSpySlot[i].setProfileID(
              TheGameSpyInfo
                  ->getLocalProfileID());  // hehe - we know our own.  the rest,
                                           // they'll tell us.
      } else {
  ...
  if (numHumans < 2) {
    launchGame();
    if (TheGameSpyInfo) TheGameSpyInfo->leaveStagingRoom();
  } else
...
AsciiString GameSpyStagingRoom::generateGameSpyGameResultsPacket(void) {
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (p) {
      ...
      if (!slot->disconnected()) {
        if (slot->getTeamNumber() != lastTeamAtGameEnd ||
            numTeamsAtGameEnd == 0) {
      ...
      if (m_GameSpySlot[i].isAI()) ++numAIs;
    }
  }
  ...
  AsciiString results;
  results.format(
      "\\seed\\%d\\hostname\\%s\\mapname\\%s\\numplayers\\%d\\duration\\%"
      "d\\gamemode\\exiting\\localplayer\\%d",
      getSeed(), m_GameSpySlot[0].getLoginName().str(), mapName.str(),
      numPlayers, endFrame, localSlotNum);
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (p) {
      ...
      AsciiString playerName =
          (slot->isHuman()) ? slot->getLoginName() : "AIPlayer";
      ...
      if (side == "America") side = "USA";

      AsciiString playerStr;
      playerStr.format(
          "\\player_%d\\%s\\pid_%d\\%d\\team_%d\\%d\\result_%d\\%s\\side_%d\\%"
          "s",
          playerID, playerName.str(), playerID, gsPlayerID, playerID,
          slot->getTeamNumber(), playerID, result.str(), playerID, side.str());
...
AsciiString GameSpyStagingRoom::generateLadderGameResultsPacket(void) {
  ...
  AsciiString results;
  results.format(
      "seed=%d,slotNum=%d,sawDesync=%d,sawGameEnd=%d,winningTeam=%d,disconEnd=%"
      "d,duration=%d,numPlayers=%d,isQM=%d,map=%s",
      getSeed(), localSlotNum, TheNetwork->sawCRCMismatch(), sawGameEnd,
      winningTeam, (numTeamsAtGameEnd < 2), endFrame, numPlayers, m_isQM,
      TheGameState->realMapPathToPortableMapPath(getMap()).str());

  AsciiString tempStr;
  tempStr.format(",ladderIP=%s,ladderPort=%d", getLadderIP().str(),
                 getLadderPort());
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (p[i]) {
      ...
      Int gsPlayerID = slot->getProfileID();
      PSPlayerStats stats =
          TheGameSpyPSMessageQueue->findPlayerStatsByID(gsPlayerID);
      ...
      AsciiString playerStr;
      playerStr.format(",player%d=%s,playerID%d=%d,locale%d=%d", playerID,
                       playerName.str(), playerID, gsPlayerID, playerID,
                       stats.locale);
      results.concat(playerStr);
      playerStr.format(",unitsKilled%d=%d,unitsLost%d=%d,unitsBuilt%d=%d",
                       playerID, unitsKilled, playerID, unitsLost, playerID,
                       unitsBuilt);
      results.concat(playerStr);
      playerStr.format(
          ",buildingsKilled%d=%d,buildingsLost%d=%d,buildingsBuilt%d=%d",
          playerID, buildingsKilled, playerID, buildingsLost, playerID,
          buildingsBuilt);
      results.concat(playerStr);
      playerStr.format(
          ",fps%d=%d,cash%d=%d,capturedTech%d=%d,discon%d=%d,side%d=%s,team%d=%"
          "d",
          playerID, fps, playerID, earnings, playerID, techCaptured, playerID,
          disconnected, playerID, p[i]->getPlayerTemplate()->getSide().str(),
          playerID, slot->getTeamNumber());
      results.concat(playerStr);
  ...
  // Add a trailing size value (so the server can ensure it got the entire
  // packet)
  results.concat(",size=");
...
void GameSpyStagingRoom::launchGame(void) {
  ...
  DEBUG_ASSERTCRASH(TheNetwork == NULL,
                    ("For some reason TheNetwork isn't NULL at the start of "
                     "this game.  Better look into that."));
  ...
  TheNetwork->setLocalAddress(
      getLocalIP(), (TheNAT) ? TheNAT->getSlotPort(getLocalSlotNum()) : 8888);
  if (TheNAT)
...
void GameSpyStagingRoom::reset(void) {
  ...
  if (this == TheGameSpyGame) {
    WindowLayout *theLayout =
        TheShell->findScreenByFilename("Menus/GameSpyGameOptionsMenu.wnd");
    if (theLayout) {
