...
static Int timeThroughOnline =
    0;  // used to avoid having old callbacks cause problems
...
void StartDownloadingPatches(void) {
  ...
  WindowLayout *layout;
  layout =
      TheWindowManager->winCreateLayout(AsciiString("Menus/DownloadMenu.wnd"));
  layout->runInit();
  layout->hide(FALSE);
  layout->bringForward();
...
static void patchBeforeOnlineCallback(void) { StartDownloadingPatches(); }
...
static void startOnline(void) {
  ...
  DEBUG_ASSERTCRASH(checksLeftBeforeOnline == 0,
                    ("starting online with pending callbacks"));
  ...
  TheScriptEngine->signalUIInteract(
      TheShellHookNames[SHELL_SCRIPT_HOOK_MAIN_MENU_ONLINE_SELECTED]);

  DEBUG_ASSERTCRASH(!TheGameSpyBuddyMessageQueue,
                    ("TheGameSpyBuddyMessageQueue exists!"));
  DEBUG_ASSERTCRASH(!TheGameSpyPeerMessageQueue,
                    ("TheGameSpyPeerMessageQueue exists!"));
...
static void queuePatch(Bool mandatory, AsciiString downloadURL) {
  ...
  DEBUG_LOG(("download URL split: %d [%s] [%s] [%s] [%s] [%s] [%s]\n", success,
             connectionType.str(), server.str(), user.str(), pass.str(),
  ...
  while (it != queuedDownloads.end()) {
    if (it->localFile == q.localFile)
      return;  // don't add it if it exists already (because we can check
               // multiple times)
    ++it;
  }
...
static GHTTPBool motdCallback(GHTTPRequest request, GHTTPResult result,
  ...
  if (!checksLeftBeforeOnline) startOnline();
...
static GHTTPBool configCallback(GHTTPRequest request, GHTTPResult result,
  ...
  if (result != GHTTPSuccess || bufferLen < 100) {
    if (!checkingForPatchBeforeGameSpy) return GHTTPTrue;
    --checksLeftBeforeOnline;
    if (onlineCancelWindow && !checksLeftBeforeOnline) {
      TheWindowManager->winDestroy(onlineCancelWindow);
      onlineCancelWindow = NULL;
  ...
  AsciiString fname;
  fname.format("%sGeneralsOnline\\Config.txt",
...
static GHTTPBool gamePatchCheckCallback(GHTTPRequest request,
                                        GHTTPResult result, char *buffer,
                                        int bufferLen, void *param) {
  Int run = (Int)param;
  if (run != timeThroughOnline) {
    DEBUG_CRASH(("Old callback being called!"));
    return GHTTPTrue;
...
static GHTTPBool overallStatsCallback(GHTTPRequest request, GHTTPResult result,
                                      char *buffer, int bufferLen,
                                      void *param) {
  DEBUG_LOG(
      ("overallStatsCallback() - Result=%d, len=%d\n", result, bufferLen));
  if (result != GHTTPSuccess) {
    return GHTTPTrue;
  }
  ...
  while (message.nextToken(&line, "\n")) {
    ...
    if (stats) {
      ...
      if (totalLine.isNotEmpty() && winsLine.isNotEmpty() &&
          lossesLine.isNotEmpty()) {
...
static GHTTPBool numPlayersOnlineCallback(GHTTPRequest request,
                                          GHTTPResult result, char *buffer,
                                          int bufferLen, void *param) {
...
void CheckOverallStats(void) {
  ghttpGet("http://gamestats.gamespy.com/ccgenerals/display.html", GHTTPFalse,
           overallStatsCallback, NULL);
}
...
void CheckNumPlayersOnline(void) {
  ghttpGet(
      ...
      "arcadecount2.dll?svcname=ccgenerals",
...
int asyncGethostbyname(char *szName) {
  ...
  if (stat == 0) {
    ...
    s_asyncDNSThreadHandle = CreateThread(NULL, 0, asyncGethostbynameThreadFunc,
                                          szName, 0, &threadid);
...
// GameSpy's HTTP SDK has had at least 1 crash bug, so we're going to just bail
// and never try again if they crash us.  We won't be able to get back online
// again (we'll time out) but at least we'll live.
...
void HTTPThinkWrapper(void) {
  ...
  if (isHttpOk) {
    try {
      ...
      isHttpOk = FALSE;  // we can't abort the login, since we might be done
                         // with the required checks and are fetching extras. If
                         // it is a required check, we'll time out normally.
    }
  }
}
...
void StopAsyncDNSCheck(void) {
  if (s_asyncDNSThreadHandle) {
    ...
    DEBUG_ASSERTCRASH(
        res,
        ("Could not terminate the Async DNS Lookup thread!"));  // Thread still
                                                                // not killed!
  }
  s_asyncDNSThreadHandle = NULL;
  s_asyncDNSLookupInProgress = FALSE;
...
void StartPatchCheck(void) {
  ...
  onlineCancelWindow =
      MessageBoxCancel(TheGameText->fetch("GUI:CheckingForPatches"),
...
static void reallyStartPatchCheck(void) {
  ...
  DEBUG_LOG(("Config: [%s]\n", configURL.c_str()));
  DEBUG_LOG(("MOTD: [%s]\n", motdURL.c_str()));
  ghttpGet(gameURL.c_str(), GHTTPFalse, gamePatchCheckCallback,
           (void *)timeThroughOnline);
  ghttpGet(mapURL.c_str(), GHTTPFalse, gamePatchCheckCallback,
           (void *)timeThroughOnline);
  ghttpHead(configURL.c_str(), GHTTPFalse, configHeadCallback,
            (void *)timeThroughOnline);
  ghttpGet(motdURL.c_str(), GHTTPFalse, motdCallback,
           (void *)timeThroughOnline);
