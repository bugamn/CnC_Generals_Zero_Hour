...
static void gameTooltip(GameWindow *window, WinInstanceData *instData,
                        UnsignedInt mouse) {
  Int x, y, row, col;
  ...
  if (col == COLUMN_PING) {
    ...
    s.format(
        ...
        L"remote pings",
        room->getPingAsInt(), TheGameSpyConfig->getPingCutoffGood(),
        TheGameSpyConfig->getPingCutoffBad(),
        TheGameSpyInfo->getPingString().str(), room->getPingString().str());
    TheMouse->setCursorTooltip(
        s, 10, NULL, 2.0f);  // the text and width are the only params used. the
                             // others are the default values.
#else
    TheMouse->setCursorTooltip(
        TheGameText->fetch("TOOLTIP:PingInfo"), 10, NULL,
        2.0f);  // the text and width are the only params used.  the others are
                // the default values.
#endif
    return;
  }
  if (col == COLUMN_NUMPLAYERS) {
    TheMouse->setCursorTooltip(
        TheGameText->fetch("TOOLTIP:NumberOfPlayers"), 10, NULL,
        2.0f);  // the text and width are the only params used.  the others are
                // the default values.
    return;
  }
  if (col == COLUMN_PASSWORD) {
    if (room->getHasPassword()) {
      ...
      TheMouse->setCursorTooltip(
          checkTooltip, 10, NULL,
          2.0f);  // the text and width are the only params used.  the others
  ...
  UnicodeString tmp;
  tooltip.format(TheGameText->fetch("TOOLTIP:GameInfoGameName"),
                 room->getGameName().str());
  if (room->getLadderPort() != 0) {
    const LadderInfo *linfo =
        TheLadderList->findLadder(room->getLadderIP(), room->getLadderPort());
    if (linfo) {
      tmp.format(TheGameText->fetch("TOOLTIP:GameInfoLadderName"),
                 linfo->name.str());
      tooltip.concat(tmp);
    }
  }
  if (room->getExeCRC() != TheGlobalData->m_exeCRC ||
      room->getIniCRC() != TheGlobalData->m_iniCRC) {
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (slot && slot->isHuman()) {
      tmp.format(TheGameText->fetch("TOOLTIP:GameInfoPlayer"),
                 slot->getName().str(), slot->getWins(), slot->getLosses());
      tooltip.concat(tmp);
      ++numPlayers;
  ...
  TheMouse->setCursorTooltip(
      tooltip, 10, NULL, 2.0f);  // the text and width are the only params used.
                                 // the others are the default values.
}

static Bool isSmall = TRUE;

GameWindow *GetGameListBox(void) { return listboxLobbyGamesLarge; }

GameWindow *GetGameInfoListBox(void) { return NULL; }

NameKeyType GetGameListBoxID(void) { return listboxLobbyGamesLargeID; }

NameKeyType GetGameInfoListBoxID(void) { return NAMEKEY_INVALID; }
...
void GrabWindowInfo(void) {
  ...
  //	parentGameListSmallID = NAMEKEY(
  //"WOLCustomLobby.wnd:ParentGameListSmall" ); 	parentGameListSmall =
  //TheWindowManager->winGetWindowFromId(NULL, parentGameListSmallID);
  ...
  //	listboxLobbyGamesSmall = TheWindowManager->winGetWindowFromId(NULL,
  //listboxLobbyGamesSmallID);
  ...
  listboxLobbyGamesLarge =
      TheWindowManager->winGetWindowFromId(NULL, listboxLobbyGamesLargeID);
  listboxLobbyGamesLarge->winSetTooltipFunc(gameTooltip);
  //
  //	listboxLobbyGameInfoID = NAMEKEY( "WOLCustomLobby.wnd:ListboxGameInfo"
  //); 	listboxLobbyGameInfo = TheWindowManager->winGetWindowFromId(NULL,
  //listboxLobbyGameInfoID);
  ...
  buttonSortAlpha =
      TheWindowManager->winGetWindowFromId(parent, buttonSortAlphaID);
  buttonSortPing =
      TheWindowManager->winGetWindowFromId(parent, buttonSortPingID);
  buttonSortBuddies =
      TheWindowManager->winGetWindowFromId(parent, buttonSortBuddiesID);
  windowSortAlpha =
      TheWindowManager->winGetWindowFromId(parent, windowSortAlphaID);
  windowSortPing =
      TheWindowManager->winGetWindowFromId(parent, windowSortPingID);
  windowSortBuddies =
      TheWindowManager->winGetWindowFromId(parent, windowSortBuddiesID);
...
static void populateBuddyGames(void) {
  ...
  for (BuddyInfoMap::const_iterator bit = m->begin(); bit != m->end(); ++bit) {
    ...
    if (info.m_status == GP_STAGING) {
      ...
      for (StagingRoomMap::iterator srmIt = srm->begin(); srmIt != srm->end();
           ++srmIt) {
...
static void clearBuddyGames(void) {
  if (theBuddyGames) delete theBuddyGames;
  theBuddyGames = NULL;
}
...
struct GameSortStruct {
  bool operator()(GameSpyStagingRoom *g1, GameSpyStagingRoom *g2) {
    ...
    Bool g1Good = (g1->getExeCRC() != TheGlobalData->m_exeCRC ||
                   g1->getIniCRC() != TheGlobalData->m_iniCRC);
    Bool g2Good = (g1->getExeCRC() != TheGlobalData->m_exeCRC ||
                   g1->getIniCRC() != TheGlobalData->m_iniCRC);
    if (g1Good ^ g2Good) {
      return g1Good;
    }
    ...
    Bool g1UnknownLadder =
        (g1->getLadderPort() &&
         TheLadderList->findLadder(g1->getLadderIP(), g1->getLadderPort()) ==
             NULL);
    Bool g2UnknownLadder =
        (g2->getLadderPort() &&
         TheLadderList->findLadder(g2->getLadderIP(), g2->getLadderPort()) ==
             NULL);
    ...
    Bool g1Full = (g1->getNumNonObserverPlayers() == g1->getMaxPlayers() ||
                   g1->getNumPlayers() == MAX_SLOTS);
    Bool g2Full = (g2->getNumNonObserverPlayers() == g2->getMaxPlayers() ||
                   g2->getNumPlayers() == MAX_SLOTS);
    if (g1Full ^ g2Full) {
      return g2Full;
    }
...
static Int insertGame(GameWindow *win, GameSpyStagingRoom *game, Bool showMap) {
  ...
  if (game->getNumNonObserverPlayers() == game->getMaxPlayers() ||
      game->getNumPlayers() == MAX_SLOTS) {
    gameColor = GameSpyColor[GSCOLOR_GAME_FULL];
  }
  if (game->getExeCRC() != TheGlobalData->m_exeCRC ||
      game->getIniCRC() != TheGlobalData->m_iniCRC) {
  ...
  if (TheGameSpyInfo->getDisallowAsianText()) {
    ...
    if (!hasUnicode) return -1;
  }

  Int index = GadgetListBoxAddEntryText(win, game->getGameName(), gameColor, -1,
                                        COLUMN_NAME);
  ...
  if (showMap) {
    ...
    const LadderInfo *li =
        TheLadderList->findLadder(game->getLadderIP(), game->getLadderPort());
    if (li) {
      GadgetListBoxAddEntryText(win, li->name, gameColor, index, COLUMN_LADDER);
    } else if (game->getLadderPort()) {
      GadgetListBoxAddEntryText(win, TheGameText->fetch("GUI:UnknownLadder"),
                                gameColor, index, COLUMN_LADDER);
    } else {
      GadgetListBoxAddEntryText(win, TheGameText->fetch("GUI:NoLadder"),
                                gameColor, index, COLUMN_LADDER);
    }
  } else {
    GadgetListBoxAddEntryText(win, UnicodeString(L" "), gameColor, index,
                              COLUMN_MAP);
    GadgetListBoxAddEntryText(win, UnicodeString(L" "), gameColor, index,
                              COLUMN_LADDER);
  }

  s.format(L"%d/%d", game->getReportedNumPlayers(),
           game->getReportedMaxPlayers());
  ...
  if (ping < TheGameSpyConfig->getPingCutoffGood()) {
    GadgetListBoxAddEntryImage(win, pingImages[0], index, COLUMN_PING, width,
                               height);
  } else if (ping < TheGameSpyConfig->getPingCutoffBad()) {
    GadgetListBoxAddEntryImage(win, pingImages[1], index, COLUMN_PING, width,
                               height);
  } else {
    GadgetListBoxAddEntryImage(win, pingImages[2], index, COLUMN_PING, width,
                               height);
  }
...
void RefreshGameListBox(GameWindow *win, Bool showMap) {
  ...
  // populate listbox
  for (SortedGameList::iterator sglIt = sgl.begin(); sglIt != sgl.end();
       ++sglIt) {
  ...
  GadgetListBoxSetSelected(
      win,
      indexToSelect);  // even for -1, so we can disable the 'Join Game' button
                       //	if(prevPos > 10)
...
void RefreshGameInfoListBox(GameWindow *mainWin, GameWindow *win) {
  ...
  //
  //		// game name
  ////		GadgetListBoxAddEntryText(listboxLobbyGameInfo,
  ///theRoom->getGameName(), GameSpyColor[GSCOLOR_DEFAULT], -1);
  //
  //		const LadderInfo * li =
  //TheLadderList->findLadder(theRoom->getLadderIP(), theRoom->getLadderPort());
  //		if (li)
  //		{
  //			UnicodeString tmp;
  //			tmp.format(TheGameText->fetch("TOOLTIP:LadderName"),
  //li->name.str()); 			GadgetListBoxAddEntryText(listboxLobbyGameInfo, tmp,
  //GameSpyColor[GSCOLOR_DEFAULT], -1);
  //		}
  //		else if (theRoom->getLadderPort())
  //		{
  //			GadgetListBoxAddEntryText(listboxLobbyGameInfo,
  //TheGameText->fetch("TOOLTIP:UnknownLadder"), GameSpyColor[GSCOLOR_DEFAULT],
  //-1);
  //		}
  //		else
  //		{
  //			GadgetListBoxAddEntryText(listboxLobbyGameInfo,
  //TheGameText->fetch("TOOLTIP:NoLadder"), GameSpyColor[GSCOLOR_DEFAULT], -1);
  //		}
  //
  //		if (theRoom->getExeCRC() != TheGlobalData->m_exeCRC ||
  //theRoom->getIniCRC() != TheGlobalData->m_iniCRC)
  //		{
  //			GadgetListBoxAddEntryText(listboxLobbyGameInfo,
  //TheGameText->fetch("TOOLTIP:InvalidGameVersionSingleLine"),
  //GameSpyColor[GSCOLOR_DEFAULT], -1);
  //		}
  //
  //		// map name
  ...
  //		}
  //
  //		GadgetListBoxAddEntryText(listboxLobbyGameInfo, mapName,
  //GameSpyColor[GSCOLOR_DEFAULT], -1);
  //
  //		// player list (rank, win/loss, side)
  //		for (Int i=0; i<MAX_SLOTS; ++i)
  //		{
  //			const GameSpyGameSlot *slot =
  //theRoom->getGameSpySlot(i); 			if (slot && slot->isHuman())
  //			{
  //				UnicodeString theName, theRating,
  //thePlayerTemplate; 				Int colorIdx = slot->getColor(); 				theName =
  //slot->getName(); 				theRating.format(L" (%d-%d)", slot->getWins(),
  //slot->getLosses()); 				const PlayerTemplate * pt =
  //ThePlayerTemplateStore->getNthPlayerTemplate(slot->getPlayerTemplate()); 				if
  //(pt)
  //				{
  //					thePlayerTemplate =
  //pt->getDisplayName();
  ...
  //				{
  //					thePlayerTemplate =
  //TheGameText->fetch("GUI:Random");
  //				}
  //
  //				UnicodeString theText;
  //				theText.format(L"%ls - %ls - %ls",
  //theName.str(), thePlayerTemplate.str(), theRating.str());
  //
  //				Int theColor = GameSpyColor[GSCOLOR_DEFAULT];
  //				const MultiplayerColorDefinition *mcd =
  //TheMultiplayerSettings->getColor(colorIdx); 				if (mcd)
  //				{
