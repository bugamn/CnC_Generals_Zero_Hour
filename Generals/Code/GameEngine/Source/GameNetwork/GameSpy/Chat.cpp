...
static const FieldParse GameSpyColorFieldParse[] = {
    ...
    {"GameCRCMismatch", INI::parseColorInt, NULL,
     OFFSET(GSCOLOR_GAME_CRCMISMATCH)},
    ...
    {"ChatOwnerEmote", INI::parseColorInt, NULL,
     OFFSET(GSCOLOR_CHAT_OWNER_EMOTE)},
    {"ChatPriv", INI::parseColorInt, NULL, OFFSET(GSCOLOR_CHAT_PRIVATE)},
    {"ChatPrivEmote", INI::parseColorInt, NULL,
     OFFSET(GSCOLOR_CHAT_PRIVATE_EMOTE)},
    {"ChatPrivOwner", INI::parseColorInt, NULL,
     OFFSET(GSCOLOR_CHAT_PRIVATE_OWNER)},
    {"ChatPrivOwnerEmote", INI::parseColorInt, NULL,
     OFFSET(GSCOLOR_CHAT_PRIVATE_OWNER_EMOTE)},
...
Bool GameSpyInfo::sendChat(UnicodeString message, Bool isAction,
                           GameWindow *playerListbox) {
  RoomType roomType = StagingRoom;
  if (getCurrentGroupRoom()) roomType = GroupRoom;
  ...
  if (!message.isEmpty()) {
    ...
    if (selections[0] == -1) {
      ...
      for (int i = 0; i < maxSel; i++) {
        if (selections[i] != -1) {
          aStr.translate(GadgetListBoxGetText(
              playerListbox, selections[i],
              GadgetListBoxGetNumColumns(playerListbox) - 1));
...
void GameSpyInfo::addChat(PlayerInfo p, UnicodeString msg, Bool isPublic,
                          Bool isAction, GameWindow *win) {
  Int style;
  if (isSavedIgnored(p.m_profileID) || isIgnored(p.m_name)) return;
...
Int GameSpyInfo::addText(UnicodeString message, Color c, GameWindow *win) {
  ...
  if (!win) {
    ...
    if (m_textWindows.empty()) return -1;
