  ...
  while (raw.nextToken(&line, "\n")) {
    ...
    if (!lad && line.startsWith("<Ladder ")) {
      ...
      while (lad->name.getLength() > 20)
        lad->name.removeLastChar();  // Per Harvard's request, ladder names are
                                     // limited to 20 chars
      ...
      AsciiStringList::iterator aIt =
          std::find(outStringList.begin(), outStringList.end(), faction);
      if (aIt != outStringList.end()) {
        // valid faction - now check for dupes
        aIt = std::find(lad->validFactions.begin(), lad->validFactions.end(),
                        faction);
    ...
    */
      ...
      DEBUG_LOG(
          ("Saw a ladder: name=%ls, addr=%s:%d, players=%dv%d, pass=%s, "
           "replay=%d, homepage=%s\n",
           lad->name.str(), lad->address.str(), lad->port, lad->playersPerTeam,
           lad->playersPerTeam, lad->cryptedPassword.str(), lad->submitReplay,
           lad->homepageURL.str()));
      // end of a ladder
      if (lad->playersPerTeam >= 1 && lad->playersPerTeam <= MAX_SLOTS / 2) {
        if (lad->validFactions.size() == 0) {
          ...
          for (AsciiStringListIterator it = validFactions.begin();
            ...
            DEBUG_LOG(("Faction %s has marker %s corresponding to str %ls\n",
                       faction.str(), marker.str(),
                       TheGameText->fetch(marker).str()));
          }
        }
        ...
        if (lad->validMaps.size() == 0) {
          ...
          for (std::list<AsciiString>::const_iterator it = qmMaps.begin();
               it != qmMaps.end(); ++it) {
      ...
      if (mapName.isNotEmpty()) {
        mapName.format("%s\\%s\\%s.map", TheMapCache->getMapDir().str(),
                       mapName.str(), mapName.str());
        mapName = TheGameState->portableMapPathToRealMapPath(
            TheGameState->realMapPathToPortableMapPath(mapName));
        mapName.toLower();
        std::list<AsciiString> qmMaps = TheGameSpyConfig->getQMMaps();
...
LadderList::LadderList() {
  ...
  DEBUG_LOG(
      ("After looking for ladders, we have %d local, %d special && %d normal\n",
       m_localLadders.size(), m_specialLadders.size(),
       m_standardLadders.size()));
}

LadderList::~LadderList() {
  LadderInfoList::iterator it;
  for (it = m_specialLadders.begin(); it != m_specialLadders.end();
       it = m_specialLadders.begin()) {
    delete *it;
    m_specialLadders.pop_front();
  }
  for (it = m_standardLadders.begin(); it != m_standardLadders.end();
       it = m_standardLadders.begin()) {
    delete *it;
    m_standardLadders.pop_front();
  }
  for (it = m_localLadders.begin(); it != m_localLadders.end();
       it = m_localLadders.begin()) {
    delete *it;
...
void LadderList::loadLocalLadders(void) {
  AsciiString dirname;
  dirname.format("%sGeneralsOnline\\Ladders\\",
...
void LadderList::checkLadder(AsciiString fname, Int index) {
  ...
  // ladders are QM-only at this point, which kinda invalidates the whole
  // concept of local ladders.  Oh well.
  ...
  // li->name =
  // UnicodeString(MultiByteToWideCharSingleLine(fname.reverseFind('\\')+1).c_str());
