...
class GameSpyConfig : public GameSpyConfigInterface {
 public:
  ...
  virtual Bool getManglerLocation(Int index, AsciiString& host,
                                  UnsignedShort& port);
...
Bool SectionChecker::isInSection() {
  Bool ret = FALSE;
  for (SectionList::const_iterator it = m_bools.begin(); it != m_bools.end();
       ++it) {
    ret = ret || **it;
  }
  return ret;
...
// QM Bot thinks that they have the same number of maps.
      ...
      if (line.nextToken(&key, " =") && line.nextToken(&val, " =")) {
        if (key == "restricted") {
          ...
          DEBUG_LOG(
              ("Unknown key '%s' = '%s' in Custom block of GameSpy Config\n",
               key.str(), val.str()));
        }
      } else {
...
Bool GameSpyConfig::getManglerLocation(Int index, AsciiString& host,
                                       UnsignedShort& port) {
