 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 3/15/01 5:03PM ST : Created *
 *=============================================================================================*/
...
FirewallHelperClass::FirewallHelperClass(void) {
  ...
  m_sourcePortPool = 4096 + ((timeGetTime() / 1000) %
                             1000);  // do this to make sure we don't use the
                                     // same source port before a previous
                                     // connection has had a chance to time out.
}
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 4/16/02 BGC : Created *
 *=============================================================================================*/
FirewallHelperClass::~FirewallHelperClass() { reset(); }
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 3/29/01 1:04AM ST : Created *
 *=============================================================================================*/
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 3/15/01 6:47PM ST : Created *
 *=============================================================================================*/
Bool FirewallHelperClass::detectFirewall(void) {
  ...
  if (TheWritableGlobalData->m_firewallBehavior == FIREWALL_TYPE_UNKNOWN) {
    ...
    DEBUG_LOG(
        ("FirewallHelperClass::detectFirewall - firewall behavior already "
         "specified as %d, port allocation delta is %d, skipping detection.\n",
...
/***********************************************************************************************
 * FHC::getNextTemporarySourcePort -- Get a throwaway source port for temporary
 *use        *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 3/15/01 12:06PM ST : Created *
 *=============================================================================================*/
  ...
  while (tries--) {
    ...
    */
    ...
    if (result) {
      ...
      DEBUG_LOG(
          ("FirewallHelperClass::getNextTemporarySourcePort - failed to open "
           "socket on port %d\n"));
    }
  }
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT:    Address of mangler server * Source port to send *from* *
 *                                                                                             *
 * OUTPUT:   True if sent OK *
 *                                                                                             *
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 3/15/01 12:47PM ST : Created *
 *=============================================================================================*/
Bool FirewallHelperClass::sendToManglerFromPort(UnsignedInt address,
  ...
  */
  ...
  /*
          ...
          for (Int i = 0; i < sizeof(ManglerData); ++i) {
                  DEBUG_LOG(("%02x", *(((unsigned char *)(&(packet.data))) +
     i)));
          }
          DEBUG_LOG(("\n"));
  */
  ...
  */
  CRC crc;
  crc.computeCRC((unsigned char *)(&(packet.data.magic)),
                 sizeof(ManglerData) - sizeof(unsigned int));
  packet.data.CRC = htonl(crc.get());

  packet.length = sizeof(ManglerData);

  DEBUG_LOG(
      ("FirewallHelperClass::sendToManglerFromPort - Sending from port %d to "
       "%d.%d.%d.%d:%d\n",
       (UnsignedInt)port, (address & 0xFF000000) >> 24,
       (address & 0xFF0000) >> 16, (address & 0xFF00) >> 8, (address & 0xFF),
       MANGLER_PORT));
  /*
          DEBUG_LOG(("Buffer = "));
          for (i = 0; i < sizeof(ManglerData); ++i) {
  ...
  */
  ...
  if (spareSocket == NULL) {
    DEBUG_ASSERTCRASH(spareSocket != NULL,
                      ("Could not find spare socket for send."));
    DEBUG_LOG(
        ("FirewallHelperClass::sendToManglerFromPort - failed to find the "
         "spare socket for port %d\n",
         port));
    return FALSE;
  }

  spareSocket->udp->Write((UnsignedByte *)&packet, sizeof(ManglerData), address,
                          MANGLER_PORT);

  return (TRUE);
}

SpareSocketStruct *FirewallHelperClass::findSpareSocketByPort(
    UnsignedShort port) {
  DEBUG_LOG(
      ("FirewallHelperClass::findSpareSocketByPort - trying to find spare "
       "socket with port %d\n",
       port));
  for (Int i = 0; i < MAX_SPARE_SOCKETS; ++i) {
...
/***********************************************************************************************
 * FHC::Get_Mangler_Response_On_Port -- Get the manglers response to a specific
 *query           *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 3/15/01 12:51PM ST : Created *
 *=============================================================================================*/
UnsignedShort FirewallHelperClass::getManglerResponse(UnsignedShort packetID,
  ...
  for (Int i = 0; i < MAX_SPARE_SOCKETS; ++i) {
    if (m_spareSockets[i].udp != NULL) {
      ...
      Int retval = m_spareSockets[i].udp->Read((unsigned char *)message,
                                               sizeof(ManglerData), &addr);
      if (retval > 0) {
        CRC crc;
        crc.computeCRC((unsigned char *)(&(message->data.magic)),
                       sizeof(ManglerData) - sizeof(unsigned int));
        if (crc.get() != htonl(message->data.CRC)) {
          DEBUG_LOG(
              ("FirewallHelperClass::getManglerResponse - Saw message, CRC "
               "mismatch.  Expected CRC %u, computed CRC %u\n",
        ...
        DEBUG_LOG(
            ("FirewallHelperClass::getManglerResponse - Saw message of %d "
             "bytes from mangler %d on port %u\n",
             retval, i, m_spareSockets[i].port));
        DEBUG_LOG(
            ("FirewallHelperClass::getManglerResponse - Message has packet ID "
             "%d 0x%08X, looking for packet id %d 0x%08X\n",
             message->data.PacketID, message->data.PacketID, packetID,
             packetID));
        if (message->data.PacketID == packetID) {
          DEBUG_LOG(
              ("FirewallHelperClass::getManglerResponse - packet ID's match, "
               "returning message\n"));
          msg = message;
          message->length = 0;
        }
        if (ntohs(message->data.PacketID) == packetID) {
          DEBUG_LOG(
              ("FirewallHelperClass::getManglerResponse - NETWORK BYTE ORDER "
               "packet ID's match, returning message\n"));
          msg = message;
          message->length = 0;
        }
      }
  ...
  if (msg == NULL) {
    for (i = 0; i < MAX_SPARE_SOCKETS; ++i) {
      if ((m_messages[i].length != 0) &&
          (m_messages[i].data.PacketID == packetID)) {
        msg = &(m_messages[i]);
        msg->length = 0;
      }
    }
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 3/22/01 10:23PM ST : Created *
 *=============================================================================================*/
...
/***********************************************************************************************
 * FirewallHelperClass::flagNeedToRefresh -- Flag that the next time we log in
 *we need to      * refresh our firewall settings. *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 2/19/03 4:30PM BGC : Created *
 *=============================================================================================*/
void FirewallHelperClass::flagNeedToRefresh(Bool flag) {
  OptionPreferences pref;

  (pref)["FirewallNeedToRefresh"] =
      flag ? AsciiString("TRUE") : AsciiString("FALSE");
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 3/22/01 10:25PM ST : Created *
 *=============================================================================================*/
void FirewallHelperClass::readFirewallBehavior(void) {
#if (0)
  m_lastBehavior = (FirewallBehaviorType)ConfigINI.Get_Int(
      "MultiPlayer", "FirewallSettings", FIREWALL_UNKNOWN);
  m_lastSourcePortAllocationDelta =
      ConfigINI.Get_Int("MultiPlayer", "FirewallDelta", 1);
#endif  //(0)
}

/***********************************************************************************************
 * FHC::detectFirewallBehavior -- What is that wacky firewall doing to our
 *packet headers?   *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 3/15/01 12:30PM ST : Created *
 *=============================================================================================*/
...
/* static */ void FirewallHelperClass::getManglerName(Int manglerIndex,
                                                      Char *nameBuf) {
  AsciiString host;
  UnsignedShort port;
...
Bool FirewallHelperClass::detectionBeginUpdate() {
  ...
  */
  ...
  /*
  ** If the user specified a particular port to use then we act as if there is
  *no firewall.
  */
    ...
    */
    if (!found) {
      ...
      DEBUG_LOG(("Found mangler address at %d.%d.%d.%d\n",
                 mangler_addresses[m][0], mangler_addresses[m][1],
                 mangler_addresses[m][2], mangler_addresses[m][3]));
    }

  } while ((m_numManglers < MAX_NUM_MANGLERS) &&
           ((timeGetTime() - m_timeoutStart) < m_timeoutLength));
  ...
  DEBUG_LOG((
      "FirewallHelperClass::detectionBeginUpdate - Testing for Netgear bug\n"));

  /*
  ** See if the user specified a netgear firewall - that will save us the
  *trouble.
  */
  if (TheGlobalData->m_firewallSendDelay) {
    UnsignedInt addbehavior = FIREWALL_TYPE_NETGEAR_BUG;
    ...
    m_behavior = (FirewallBehaviorType)addbehavior;
    DEBUG_LOG(
        ("FirewallHelperClass::detectionBeginUpdate - Netgear bug specified by "
         "command line or SendDelay flag\n"));
  } else {
    DEBUG_LOG(
        ("FirewallHelperClass::detectionBeginUpdate - Netgear bug not "
         "specified\n"));
  }
...
Bool FirewallHelperClass::detectionTest1Update() {
  ...
  */
  if (m_mangledPorts[0] == 0 || m_mangledPorts[0] == m_sparePorts[0]) {
    if (m_mangledPorts[0] == m_sparePorts[0]) {
      m_sourcePortAllocationDelta = 0;
      DEBUG_LOG(
          ("FirewallHelperClass::detectionTest1Update - Non-mangled response "
           "from mangler, quitting test.\n"));
    }
    if ((m_mangledPorts[0] == 0) &&
        ((timeGetTime() - m_timeoutStart) < m_timeoutLength)) {
      // we are still waiting for a response and haven't timed out yet.
      DEBUG_LOG(
          ("FirewallHelperClass::detectionTest1Update - waiting for response "
           "from mangler.\n"));
      return FALSE;
    }
    if ((m_mangledPorts[0] == 0) &&
      ...
      DEBUG_LOG(
          ("FirewallHelperClass::detectionTest1Update - timed out waiting for "
           "response from mangler.\n"));
    }
    // either we have received a non-mangled response or we timed out waiting
    // for a response.
  ...
  **
  ** Second test. See if the ports are mangled differently for different
  *destination IPs.
  **
  ** We can use the spare socket from the last test and send to a different
  *mangler.
  **
  */
...
Bool FirewallHelperClass::detectionTest2Update() {
  ...
  if (m_mangledPorts[1] == 0) {
    ...
    DEBUG_LOG(
        ("FirewallHelperClass::detectionTest2Update - timed out waiting for "
         "mangler response\n"));
    m_currentState = DETECTIONSTATE_DONE;
    return TRUE;
  }
  ...
  */
  if (m_mangledPorts[1] == 0 || m_mangledPorts[1] == m_sparePorts[0]) {
    ...
    if (m_mangledPorts[1] == 0) {
      DEBUG_LOG(
          ("FirewallHelperClass::detectionTest2Update - got no response from "
           "mangler\n"));
    } else {
      DEBUG_LOG(
          ("FirewallHelperClass::detectionTest2Update - got a mangler "
           "response, no port mangling\n"));
    }
    DEBUG_LOG(
        ("FirewallHelperClass::detectionTest2Update - Setting behavior to "
         "SIMPLE, done testing\n"));
    return TRUE;
  }
  ...
  if (m_mangledPorts[0] == m_mangledPorts[1]) {
    DEBUG_LOG(
        ("FirewallHelperClass::detectionTest2Update - port mangling doesn't "
         "depend on destination IP, setting to DUMB_MANGLING\n"));
    UnsignedInt addBehavior = (UnsignedInt)FIREWALL_TYPE_DUMB_MANGLING;
    addBehavior |= (UnsignedInt)m_behavior;
    m_behavior = (FirewallBehaviorType)addBehavior;
  } else {
    DEBUG_LOG(
        ("FirewallHelperClass::detectionTest2Update - port mangling depends on "
         "destination IP, setting to SMART_MANGLING\n"));
...
Bool FirewallHelperClass::detectionTest3Update() {
  ...
  */
  if (m_currentTry < 3) {
    ...
    ** We should use a non-linear set of source ports so we can detect the NAT32
    *relative offset
    ** case.
    */
    for (Int i = 0; i < NUM_TEST_PORTS; i++) {
      ...
      if (!openSpareSocket(m_sparePorts[i])) {
        ...
        */
        ...
        DEBUG_LOG(
            ("FirewallHelperClass::detectionTest3Update - Failed to open all "
             "the spare sockets, bailing test\n"));
        m_currentState = DETECTIONSTATE_DONE;
        return TRUE;
      }
    }
    ...
    **
    ** Keep sending from each port until we get a response to all the sends.
    *There's a implied
    ...
    */
    ...
    DEBUG_LOG(
        ("FirewallHelperClass::detectionTest3Update - Sending to %d manglers\n",
         NUM_TEST_PORTS));
    ...
    DEBUG_LOG(
        ("FirewallHelperClass::detectionTest3Update - Waiting for mangler "
         "responses\n"));
    return FALSE;
  }

  DEBUG_LOG(
      ("FirewallHelperClass::detectionTest3Update - Failed to complete test, "
       "quitting\n"));
  m_currentState = DETECTIONSTATE_DONE;
  return TRUE;
}
...
Bool FirewallHelperClass::detectionTest3WaitForResponsesUpdate() {
  ...
  if (m_numResponses < NUM_TEST_PORTS) {
    if ((timeGetTime() - m_timeoutStart) > m_timeoutLength) {
      ...
      */
      ...
      DEBUG_LOG(
          ("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - timed "
           "out waiting, bailing test\n"));
      m_currentState = DETECTIONSTATE_DONE;
      return TRUE;
    }
    return FALSE;
  ...
  */
  if (m_numResponses < 4) {
    if (m_lastSourcePortAllocationDelta != 0 &&
        (int)m_lastBehavior > (int)FIREWALL_TYPE_SIMPLE) {
      /*
    ...
    DEBUG_LOG(
        ("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - didn't "
         "get enough responses, using %d as the source port allocation delta, "
         "finished test\n"));
    m_currentState = DETECTIONSTATE_DONE;
    return TRUE;
  }
  ...
  Int delta = getNATPortAllocationScheme(
      m_numResponses, m_sparePorts, m_mangledPorts, relative_delta, looks_good);
  DEBUG_LOG(
      ("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - "
       "getNATPortAllocationScheme returned %d\n",
       delta));
  ...
  if (delta) {
    ...
    */
    ...
    if (relative_delta) {
      DEBUG_LOG(
          ("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - "
           "detected RELATIVE PORT ALLOCATION\n"));
      addbehavior = (UnsignedInt)FIREWALL_TYPE_RELATIVE_PORT_ALLOCATION;
    } else {
      DEBUG_LOG(
          ("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - "
           "detected SIMPLE PORT ALLOCATION\n"));
    ...
    DEBUG_LOG(
        ("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - setting "
         "source port delta to %d\n",
         delta));
  } else {
    DEBUG_LOG(
        ("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - didn't "
    ...
    if (m_lastSourcePortAllocationDelta != 0 &&
      ...
      */
      DEBUG_LOG(
          ("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - using "
           "the port allocation delta we have from before which is %d\n",
           m_lastSourcePortAllocationDelta));
      m_sourcePortAllocationDelta = m_lastSourcePortAllocationDelta;
    }
    ++m_currentTry;
  ...
  DEBUG_LOG(
      ("FirewallHelperClass::detectionTest3WaitForRepsonsesUpdate - starting "
       "4th test\n"));
  /*
  ** Fourth test.
  **
  ** Test to see if the NAT mangles differently per destination port at the same
  *IP.
  */
  if ((m_behavior & FIREWALL_TYPE_SMART_MANGLING) != 0) {
    if ((m_behavior & FIREWALL_TYPE_SIMPLE_PORT_ALLOCATION) != 0) {
      DEBUG_LOG(
          ("FirewallHelperClass::detectionTest3WaitForRepsonsesUpdate - simple "
           "port allocation, Testing to see if the NAT mangles differently per "
           "destination port at the same IP\n"));
      ...
      */
      ...
      /*
      ** NAT32 uses different mangled source ports for different destination
      *ports.
      */
      DEBUG_LOG(
          ("FirewallHelperClass::detectionTest3WaitForRepsonsesUpdate - "
      ...
      DEBUG_LOG(
          ("FirewallHelperClass::detectionTest3WaitForRepsonsesUpdate - adding "
           "DESTINATION PORT DELTA to behavior\n"));
      addbehavior |= (UnsignedInt)m_behavior;
      m_behavior = (FirewallBehaviorType)addbehavior;
    }
  } else {
    DEBUG_LOG(
        ("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - We don't "
         "have smart mangling, skipping test 4, entering test 5\n"));
  }

  DEBUG_LOG(
      ("FirewallHelperClass::detectionTest3WaitForRepsonsesUpdate - entering "
       "test 5\n"));
...
Bool FirewallHelperClass::detectionTest4Stage1Update() {
  ...
  if (m_mangledPorts[0] == 0) {
    if ((timeGetTime() - m_timeoutStart) > m_timeoutLength) {
      ...
      m_currentState = DETECTIONSTATE_DONE;
      DEBUG_LOG(
          ("FirewallHelperClass::detectionTest4Stage1Update - timed out "
           "waiting for mangler response, quitting\n"));
      return TRUE;
    }
    return FALSE;
  }
  ...
  */
  ...
  /*
  ** We can't get a response from a different destination port so the only way
  *to detect
  ** this behavior is to check the next mangled port allocation to see if it's
  *double
...
Bool FirewallHelperClass::detectionTest4Stage2Update() {
  ...
  if (m_mangledPorts[1] == 0) {
    if ((timeGetTime() - m_timeoutStart) > m_timeoutLength) {
      ...
      m_currentState = DETECTIONSTATE_DONE;
      DEBUG_LOG(
          ("FirewallHelperClass::detectionTest4Stage2Update - timed out "
           "waiting for the second mangler response, quitting\n"));
      return TRUE;
    }
    return FALSE;
  }
  ...
  if (m_mangledPorts[1] != m_mangledPorts[0] + m_sourcePortAllocationDelta) {
    DEBUG_LOG(
        ("FirewallHelperClass::detectionTest4Stage2Update - NAT uses different "
         "source ports for different destination ports\n"));
    ...
    if (m_mangledPorts[1] == m_mangledPorts[0] + m_sourcePortAllocationDelta) {
      DEBUG_LOG(
          ("FirewallHelperClass::detectionTest4Stage2Update - NAT uses the "
           "same source port for different destination ports\n"));
    } else {
      DEBUG_LOG(
          ("FirewallHelperClass::detectionTest4Stage2Update - Unable to "
           "complete destination port mangling test\n"));
...
Bool FirewallHelperClass::detectionTest5Update() {
  /*
  ** We have done all the tests we *have* to. There's other info that it would
  *be nice to know though.
  **
  ** Test for the netgear bug behavior.
  */
#if (0)
  // moved to before test 1.  Moved because this flag could be specified for
  // another firewall for testing purposes and never get this far because it has
  // behavior that doesn't require all the tests to be performed. BGC 10/1/02
  DEBUG_LOG((
      "FirewallHelperClass::detectionTest5Update - Testing for Netgear bug\n"));
  ...
  */
  if (TheGlobalData->m_firewallSendDelay) {
    ...
    m_behavior = (FirewallBehaviorType)addbehavior;
    DEBUG_LOG(
        ("FirewallHelperClass::detectionTest5Update - Netgear bug specified by "
         "command line or SendDelay flag\n"));
  } else {
    DEBUG_LOG(
        ("FirewallHelperClass::detectionTest5Update - Netgear bug not "
         "specified\n"));
  }
#endif  // #if (0)

  DEBUG_LOG(
      ("FirewallHelperClass::detectionTest5Update - All done, behavior is: "));
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT:    Number of ports we should analyze * List of original port numbers *
 *           List of mangled port numbers * relative_delta (out) Is the delta
 *relative to the original port number?           * looks_good (out) Do all the
 *values point to the same delta?                       *
 *                                                                                             *
 * OUTPUT:   Port allocation delta *
 *                                                                                             *
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 3/15/01 4:45PM ST : Created *
 *=============================================================================================*/
Int FirewallHelperClass::getNATPortAllocationScheme(
  ...
  ** Stupid bubble sort will do. original_ports may be out of oder after the
  *sort.
  */
  ...
  /*
  ** Now start looking for patterns in the port numbers. Possible patterns
  *include.
  **
  ...
  */
  if (diff1 == diff2 && diff2 == diff3) {
    DEBUG_LOG(
        ("Looks good for absolute allocation sequence delta of %d\n", diff1));
    relativeDelta = FALSE;
    looksGood = TRUE;
    return (diff1);
  }
  ...
  */
  ...
  /*
  ** 4. Check for relative scheme skipping 'n' ports. NAT32 behaves this way, it
  *skips 100 ports
  ** each time.
  */
  ...
  */
  if (diff1 == diff2 || diff1 == diff3) {
    DEBUG_LOG(
        ("Detected probable broken relative port range delta of %d\n", diff1));
    relativeDelta = TRUE;
    looksGood = FALSE;
    return (diff1);
  }
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 3/16/01 11:43AM ST : Created *
 *=============================================================================================*/
...
/***********************************************************************************************
 * FHC::Get_Firewall_Retries -- How many retries is it likely to take before we
 *connect?       *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
...
Bool FirewallHelperClass::openSpareSocket(UnsignedShort port) {
  ...
  if (m_spareSockets[i].udp == NULL) {
    DEBUG_LOG(
        ("FirewallHelperClass::openSpareSocket - failed to create UDP "
         "object\n"));
    return FALSE;
  }
  ...
  if (m_spareSockets[i].udp->Bind((UnsignedInt)0, port) != 0) {
    DEBUG_CRASH(
        ("FirewallHelperClass::openSpareSocket - Failed to init spare socket"));
    return FALSE;
  }

  m_spareSockets[i].port = port;
  DEBUG_LOG(
      ("FirewallHelperClass::openSpareSocket - port %d is open for send\n",
       port));
  return TRUE;
}
