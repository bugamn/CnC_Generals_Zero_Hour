...
void DisconnectManager::update(ConnectionManager *conMgr) {
  ...
  // The game logic stalls on the frame we are currently waiting for commands
  // on, so we have to check for the current logic frame being one higher than
  ...
  if (m_disconnectState != DISCONNECTSTATETYPE_SCREENOFF) {
    ...
    if (m_pingFrame < TheGameLogic->getFrame()) {
      ...
      if ((curTime - m_lastFrameTime) >
          10000)  /// @todo: plug in some better measure here
      {
        ...
        if (ThePinger) {
          ...
          std::list<AsciiString>::iterator it =
              TheGameSpyConfig->getPingServers().begin();
          ...
          DEBUG_LOG(
              ("DisconnectManager::update() - requesting %d pings of %d from "
               "%s\n",
    ...
    if (ThePinger) {
      ...
      while (ThePinger->getResponse(resp)) {
        if (m_pingFrame != TheGameLogic->getFrame()) {
          ...
          DEBUG_LOG(
              ("DisconnectManager::update() - discarding ping of %d from %s "
               "(%d reps)\n",
               resp.avgPing, resp.hostname.c_str(), resp.repetitions));
        } else {
          // right frame
          DEBUG_LOG(
              ("DisconnectManager::update() - keeping ping of %d from %s (%d "
               "reps)\n",
               resp.avgPing, resp.hostname.c_str(), resp.repetitions));
...
UnsignedInt DisconnectManager::getPingFrame() { return m_pingFrame; }

Int DisconnectManager::getPingsSent() { return m_pingsSent; }

Int DisconnectManager::getPingsRecieved() { return m_pingsRecieved; }
...
void DisconnectManager::updateDisconnectStatus(ConnectionManager *conMgr) {
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if (conMgr->isPlayerConnected(i)) {
      ...
      if (slot != -1) {
        time_t curTime = timeGetTime();
        time_t newTime = TheGlobalData->m_networkPlayerTimeoutTime -
                         (curTime - m_playerTimeouts[slot]);

        // if someone is more than 2/3 timed out, lets get our frame numbers
        // sync'd up.  Also if someone is voted out lets do the same thing.

        if (m_haveNotifiedOtherPlayersOfCurrentFrame == FALSE) {
          if ((newTime < TheGlobalData->m_networkPlayerTimeoutTime / 3) ||
          ...
          DEBUG_LOG(
              ("DisconnectManager::updateDisconnectStatus - curTime = %d, "
               "m_timeOfDisconnectScreenOn = %d, curTime - "
               "m_timeOfDisconnectScreenOn = %d\n",
               curTime, m_timeOfDisconnectScreenOn,
               curTime - m_timeOfDisconnectScreenOn));

          if (m_timeOfDisconnectScreenOn != 0) {
            if ((curTime - m_timeOfDisconnectScreenOn) >
        ...
        if ((newTime < 0) || (isPlayerVotedOut(slot, conMgr) == TRUE)) {
          newTime = 0;
          DEBUG_LOG(
              ("DisconnectManager::updateDisconnectStatus - player "
               "%d(translated slot %d) has been voted out or timed out\n",
               i, slot));
          if (allOnSameFrame(conMgr) == TRUE) {
            DEBUG_LOG(
                ("DisconnectManager::updateDisconnectStatus - all on same "
                 "frame\n"));
            if (isLocalPlayerNextPacketRouter(conMgr) == TRUE) {
              DEBUG_LOG(
                  ("DisconnectManager::updateDisconnectStatus - local player "
                   "is next packet router\n"));
              DEBUG_LOG(
                  ("DisconnectManager::updateDisconnectStatus - about to do "
                   "the disconnect procedure for player %d\n",
                   i));
              ...
              DEBUG_LOG(
                  ("DisconnectManager::updateDisconnectStatus - local player "
                   "is not the next packet router\n"));
            }
          } else {
            DEBUG_LOG(
                ("DisconnectManager::updateDisconnectStatus - not all on same "
                 "frame\n"));
          }
        }
...
void DisconnectManager::updateWaitForPacketRouter(ConnectionManager *conMgr) {
  /*
          time_t curTime = timeGetTime();
          time_t newTime = TheGlobalData->m_networkPlayerTimeoutTime - (curTime
     - m_packetRouterTimeout); if (newTime < 0) { newTime = 0;

                  // The guy that we were hoping would be the new packet router
     isn't.  We're screwed, get out of the game.

                  DEBUG_LOG(("DisconnectManager::updateWaitForPacketRouter -
     timed out waiting for new packet router, quitting game\n"));
                  TheNetwork->quitGame();
          }
...
void DisconnectManager::processDisconnectKeepAlive(NetCommandMsg *msg,
  ...
  Int slot =
      translatedSlotPosition(cmdMsg->getPlayerID(), conMgr->getLocalPlayerID());
  if (slot != -1) {
...
void DisconnectManager::processDisconnectPlayer(NetCommandMsg *msg,
  ...
  DEBUG_LOG(
      ("DisconnectManager::processDisconnectPlayer - Got disconnect player "
       "command from player %d.  Disconnecting player %d on frame %d\n",
       msg->getPlayerID(), cmdMsg->getDisconnectSlot(),
       cmdMsg->getDisconnectFrame()));
...
void DisconnectManager::processPacketRouterQuery(NetCommandMsg *msg,
  ...
  DEBUG_LOG(
      ("DisconnectManager::processPacketRouterQuery - got a packet router "
       "query command from player %d\n",
       msg->getPlayerID()));

  if (conMgr->getPacketRouterSlot() == conMgr->getLocalPlayerID()) {
    NetPacketRouterAckCommandMsg *ackmsg =
        newInstance(NetPacketRouterAckCommandMsg);
    ...
    DEBUG_LOG(
        ("DisconnectManager::processPacketRouterQuery - We are the new packet "
         "router, responding with an packet router ack. Local player is %d\n",
         ackmsg->getPlayerID()));
    conMgr->sendLocalCommandDirect(ackmsg, 1 << cmdMsg->getPlayerID());
    ackmsg->detach();
  } else {
    DEBUG_LOG(
        ("DisconnectManager::processPacketRouterQuery - We are NOT the new "
         "packet router, these are not the droids you're looking for.\n"));
  }
}
...
void DisconnectManager::processPacketRouterAck(NetCommandMsg *msg,
  ...
  DEBUG_LOG(
      ("DisconnectManager::processPacketRouterAck - got packet router ack "
       "command from player %d\n",
       msg->getPlayerID()));

  if (conMgr->getPacketRouterSlot() == cmdMsg->getPlayerID()) {
    DEBUG_LOG(
        ("DisconnectManager::processPacketRouterAck - packet router command is "
         "from who it should be.\n"));
    resetPacketRouterTimeout();
    Int currentPacketRouterSlot = conMgr->getPacketRouterSlot();
    Int currentPacketRouterIndex = 0;
    while ((currentPacketRouterSlot !=
            conMgr->getPacketRouterFallbackSlot(currentPacketRouterIndex)) &&
           (currentPacketRouterIndex < MAX_SLOTS)) {
      ++currentPacketRouterIndex;
    }
    DEBUG_ASSERTCRASH((currentPacketRouterIndex < MAX_SLOTS),
                      ("Invalid packet router index"));

    DEBUG_LOG(
        ("DisconnectManager::processPacketRouterAck - New packet router "
         "confirmed, resending pending commands\n"));
    conMgr->resendPendingCommands();
    m_currentPacketRouterIndex = currentPacketRouterIndex;
    DEBUG_LOG(
        ("DisconnectManager::processPacketRouterAck - Setting disconnect state "
         "to screen on.\n"));
    m_disconnectState =
        DISCONNECTSTATETYPE_SCREENON;  ///< set it to screen on so that the next
                                       ///< call to AllCommandsReady can set up
                                       ///< everything for the next frame
                                       ///< properly.
  }
}
...
void DisconnectManager::processDisconnectVote(NetCommandMsg *msg,
  ...
  DEBUG_LOG(
      ("DisconnectManager::processDisconnectVote - Got a disconnect vote for "
       "player %d command from player %d\n",
       cmdMsg->getSlot(), cmdMsg->getPlayerID()));
  Int transSlot =
      translatedSlotPosition(msg->getPlayerID(), conMgr->getLocalPlayerID());

  if (isPlayerInGame(transSlot, conMgr) == FALSE) {
    // if they've been timed out, voted out, disconnected, don't count their
    // vote.
    return;
  }

  applyDisconnectVote(cmdMsg->getSlot(), cmdMsg->getVoteFrame(),
                      cmdMsg->getPlayerID(), conMgr);
}
...
void DisconnectManager::processDisconnectFrame(NetCommandMsg *msg,
  ...
  if (m_disconnectFrames[playerID] >= cmdMsg->getDisconnectFrame()) {
    // this message isn't valid, we have a disconnect frame that is later than
    // this already.
    return;
  }
  ...
  if (m_disconnectFramesReceived[playerID] == TRUE) {
    DEBUG_LOG(
        ("DisconnectManager::processDisconnectFrame - Got two disconnect "
         "frames without an intervening disconnect screen off command from "
         "player %d. Frames are %d and %d\n",
         playerID, m_disconnectFrames[playerID], cmdMsg->getDisconnectFrame()));
  }

  DEBUG_LOG(
      ("DisconnectManager::processDisconnectFrame - about to call "
       "resetPlayersVotes for player %d\n",
       playerID));
  ...
  DEBUG_LOG(
      ("DisconnectManager::processDisconnectFrame - Got a disconnect frame for "
       "player %d, frame = %d, local player is %d, local disconnect frame = "
       "%d, command id = %d\n",
       cmdMsg->getPlayerID(), cmdMsg->getDisconnectFrame(),
       conMgr->getLocalPlayerID(),
       m_disconnectFrames[conMgr->getLocalPlayerID()], cmdMsg->getID()));

  if (playerID == conMgr->getLocalPlayerID()) {
    DEBUG_LOG(
        ("DisconnectManager::processDisconnectFrame - player %d is the local "
         "player\n",
         playerID));
    // we just got the message from the local player, check to see if we need to
    // send commands to anyone we already have heard from.
    for (Int i = 0; i < MAX_SLOTS; ++i) {
      if (i != playerID) {
        Int transSlot = translatedSlotPosition(i, conMgr->getLocalPlayerID());
        if (isPlayerInGame(transSlot, conMgr) == TRUE) {
          if ((m_disconnectFrames[i] < m_disconnectFrames[playerID]) &&
              (m_disconnectFramesReceived[i] == TRUE)) {
            DEBUG_LOG(
                ("DisconnectManager::processDisconnectFrame - I have more "
                 "frames than player %d, my frame = %d, their frame = %d\n",
                 i, m_disconnectFrames[conMgr->getLocalPlayerID()],
            ...
            conMgr->sendFrameDataToPlayer(i, m_disconnectFrames[i]);
             ...
             (m_disconnectFramesReceived[playerID] == TRUE)) {
    DEBUG_LOG(
        ("DisconnectManager::processDisconnectFrame - I have more frames than "
         "player %d, my frame = %d, their frame = %d\n",
         playerID, m_disconnectFrames[conMgr->getLocalPlayerID()],
         m_disconnectFrames[playerID]));
...
void DisconnectManager::processDisconnectScreenOff(NetCommandMsg *msg,
  ...
  DEBUG_LOG(
      ("DisconnectManager::processDisconnectScreenOff - got a screen off "
       "command from player %d for frame %d\n",
  ...
  if (newFrame >= m_disconnectFrames[playerID]) {
    DEBUG_LOG(
        ("DisconnectManager::processDisconnectScreenOff - resetting the "
         "disconnect screen status for player %d\n",
         playerID));
    m_disconnectFramesReceived[playerID] = FALSE;
    m_disconnectFrames[playerID] =
        newFrame;  // just in case we get packets out of order and the
                   // disconnect screen off message gets here before the
                   // disconnect frame message.

    DEBUG_LOG(
        ("DisconnectManager::processDisconnectScreenOff - about to call "
         "resetPlayersVotes for player %d\n",
         playerID));
...
void DisconnectManager::applyDisconnectVote(Int slot, UnsignedInt frame,
                                            Int fromSlot,
                                            ConnectionManager *conMgr) {
  ...
  DEBUG_LOG(
      ("DisconnectManager::applyDisconnectVote - added a vote to disconnect "
       "slot %d, from slot %d, for frame %d, current votes are %d\n",
...
void DisconnectManager::allCommandsReady(UnsignedInt frame,
                                         ConnectionManager *conMgr,
                                         Bool waitForPacketRouter) {
  if (m_disconnectState != DISCONNECTSTATETYPE_SCREENOFF) {
    DEBUG_LOG(
        ("DisconnectManager::allCommandsReady - setting screen state to "
         "off.\n"));
    ...
    DEBUG_LOG(
        ("DisconnectManager::allCommandsReady - resetting "
...
void DisconnectManager::sendKeepAlive(ConnectionManager *conMgr) {
  ...
  if (((curTime - m_lastKeepAliveSendTime) > 500) ||
      (m_lastKeepAliveSendTime == -1)) {
    NetDisconnectKeepAliveCommandMsg *msg =
...
void DisconnectManager::turnOnScreen(ConnectionManager *conMgr) {
  ...
  DEBUG_LOG(
      ("DisconnectManager::turnOnScreen - turning on screen on frame %d\n",
...
void DisconnectManager::disconnectPlayer(Int slot, ConnectionManager *conMgr) {
  DEBUG_LOG(
      ("DisconnectManager::disconnectPlayer - Disconnecting slot number %d on "
       "frame %d\n",
       slot, TheGameLogic->getFrame()));
  DEBUG_ASSERTCRASH((slot >= 0) && (slot < MAX_SLOTS),
                    ("Attempting to disconnect an invalid slot number"));
  ...
  if (transSlot != -1) {
    ...
    DEBUG_ASSERTCRASH((retcode != PLAYERLEAVECODE_UNKNOWN),
                      ("Invalid player leave code"));

    if (retcode == PLAYERLEAVECODE_PACKETROUTER) {
      DEBUG_LOG(
          ("DisconnectManager::disconnectPlayer - disconnecting player was "
           "packet router.\n"));
...
void DisconnectManager::sendDisconnectCommand(Int slot,
                                              ConnectionManager *conMgr) {
  DEBUG_LOG(
      ("DisconnectManager::sendDisconnectCommand - Sending disconnect command "
       "for slot number %d\n",
       slot));
  DEBUG_ASSERTCRASH(
      (slot >= 0) && (slot < MAX_SLOTS),
      ("Attempting to send a disconnect command for an invalid slot number"));
  if ((slot < 0) || (slot >= (MAX_SLOTS))) {
    return;
  }
...
void DisconnectManager::sendVoteCommand(Int slot, ConnectionManager *conMgr) {
  ...
  conMgr->sendLocalCommandDirect(msg,
                                 0xff & ~(1 << conMgr->getLocalPlayerID()));
...
void DisconnectManager::voteForPlayerDisconnect(Int slot,
  ...
  if (m_playerVotes[transSlot][conMgr->getLocalPlayerID()].vote == FALSE) {
    ...
    // we use the game logic frame cause we might not have sent out our own
    // disconnect frame yet.
    applyDisconnectVote(transSlot, TheGameLogic->getFrame(),
                        conMgr->getLocalPlayerID(), conMgr);
  }
}

void DisconnectManager::recalculatePacketRouterIndex(
    ConnectionManager *conMgr) {
  Int currentPacketRouterSlot = conMgr->getPacketRouterSlot();
  m_currentPacketRouterIndex = 0;
  while ((currentPacketRouterSlot !=
          conMgr->getPacketRouterFallbackSlot(m_currentPacketRouterIndex)) &&
         (m_currentPacketRouterIndex < MAX_SLOTS)) {
    ++m_currentPacketRouterIndex;
  }
  DEBUG_ASSERTCRASH((m_currentPacketRouterIndex < MAX_SLOTS),
                    ("Invalid packet router index"));
}
...
Bool DisconnectManager::allOnSameFrame(ConnectionManager *conMgr) {
  ...
  for (Int i = 0; (i < MAX_SLOTS) && (retval == TRUE); ++i) {
    ...
    if ((conMgr->isPlayerConnected(i) == TRUE) &&
      ...
      // ok, i is someone who is in the game and hasn't timed out yet or been
      // voted out.
...
Bool DisconnectManager::isLocalPlayerNextPacketRouter(
    ConnectionManager *conMgr) {
  ...
  while ((transSlot != -1) && (isPlayerInGame(transSlot, conMgr) == FALSE)) {
    ...
    if ((packetRouterSlot >= MAX_SLOTS) || (packetRouterSlot < 0)) {
      ...
      DEBUG_CRASH(
          ("no more players left to be the packet router, this shouldn't "
           "happen."));
      return FALSE;
    }
...
void DisconnectManager::sendPlayerDestruct(Int slot,
  ...
  DEBUG_LOG(
      ("Queueing DestroyPlayer %d for frame %d on frame %d as command %d\n",
       slot, TheNetwork->getExecutionFrame() + 1, TheGameLogic->getFrame(),
       currentID));
...
// the 'slot' variable is supposed to be a translated slot position. (translated
// slot meaning that it is the player's position in the disconnect menu)
...
Bool DisconnectManager::isPlayerInGame(Int slot, ConnectionManager *conMgr) {
  Int transSlot = untranslatedSlotPosition(slot, conMgr->getLocalPlayerID());
  DEBUG_ASSERTCRASH((transSlot >= 0) && (transSlot < MAX_SLOTS),
                    ("invalid slot number"));
  if (((transSlot < 0) || (transSlot >= MAX_SLOTS)) ||
      conMgr->isPlayerConnected(transSlot) == FALSE) {
    return FALSE;
  }
...
void DisconnectManager::playerHasAdvancedAFrame(Int slot, UnsignedInt frame) {
  // if they have advanced beyond the frame they had been previously
  // disconnecting on.
  if (frame >= m_disconnectFrames[slot]) {
    m_disconnectFrames[slot] = frame;  // just in case we get a disconnect frame
...
Int DisconnectManager::countVotesForPlayer(Int slot) {
  ...
  Int retval = 0;
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    // using TheGameLogic->getFrame() cause we might not have sent our
    // disconnect frame yet.
...
void DisconnectManager::resetPlayersVotes(Int playerID, UnsignedInt frame,
                                          ConnectionManager *conMgr) {
  DEBUG_LOG(
      ("DisconnectManager::resetPlayersVotes - resetting player %d's votes on "
       "frame %d\n",
       playerID, frame));

  // we need to reset this player's votes that happened before or on the given
  // frame.
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if (m_playerVotes[i][playerID].frame <= frame) {
      DEBUG_LOG(
          ("DisconnectManager::resetPlayersVotes - resetting player %d's vote "
           "for player %d from frame %d on frame %d\n",
  ...
  DEBUG_LOG(
      ("DisconnectManager::resetPlayersVotes - after adjusting votes, player "
       "%d has %d votes\n",
       playerID, numVotes));
