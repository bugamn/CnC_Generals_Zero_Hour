...
/**
 * Connection message - encapsulating info kept by the connection layer about
 * each packet.  These structs make up the in/out buffers at the connection
 * layer.
 */
#pragma pack(push, 1)
...
static const int CmdMsgLen =
    6;  //< Minimum size of a command packet (Int + Unsigned Short)
 ...
 * implements the interface to all Network operations such as message stream
...
class Network : public NetworkInterface {
 public:
  ...
  void liteupdate(
      void);  ///< Do a lightweight update to send packets and pass messages.
  ...
  UnsignedInt getPacketArrivalCushion(
      void);  ///< Returns the smallest packet arrival cushion since this was
              ///< last called.
  ...
  void sendFile(AsciiString path, UnsignedByte playerMask,
  ...
  void notifyOthersOfCurrentFrame();  ///< Tells all the other players what
                                      ///< frame we are on.
  void notifyOthersOfNewFrame(
      UnsignedInt
          frame);  ///< Tells all the other players that we are on a new frame.

  Int getExecutionFrame();  ///< Returns the next valid frame for simultaneous
                            ///< command execution.
 ...
 protected:
  void
  GetCommandsFromCommandList();  ///< Remove commands from TheCommandList and
                                 ///< put them on the Network command list.
  void SendCommandsToConnectionManager();  ///< Send the new commands to the
                                           ///< ConnectionManager
  Bool AllCommandsReady(
      UnsignedInt frame);  ///< Do we have all the commands for the given frame?
  void RelayCommandsToCommandList(
      UnsignedInt frame);  ///< Put the commands for the given frame onto
                           ///< TheCommandList.
  Bool isTransferCommand(
      GameMessage *msg);  ///< Is this a command that needs to be transfered to
                          ///< the other clients?
  Bool processCommand(
      GameMessage *msg);  ///< Whatever needs to be done as a result of this
                          ///< command, do it now.
  void processFrameSynchronizedNetCommand(
      NetCommandRef
          *msg);  ///< If there is a network command that needs to be executed
                  ///< at the same frame number on all clients, it happens here.
  void processRunAheadCommand(
      NetRunAheadCommandMsg *msg);  ///< Do what needs to be done when we get a
                                    ///< new run ahead command.
  void processDestroyPlayerCommand(
      NetDestroyPlayerCommandMsg *
          msg);  ///< Do what needs to be done when we need to destroy a player.
  void
  endOfGameCheck();  ///< Checks to see if its ok to leave this game.  If it is,
                     ///< send the apropriate command to the game logic.
  ...
  Int m_frameRate;
  Int m_lastExecutionFrame;  ///< The highest frame number that a command could
                             ///< have been executed on.
  Int m_lastFrameCompleted;
  Bool m_didSelfSlug;
  __int64 m_perfCountFreq;  ///< The frequency of the performance counter.

  __int64 m_nextFrameTime;  ///< When did we execute the last frame?  For
                            ///< slugging the GameLogic...

  Bool m_frameDataReady;  ///< Is the frame data for the next frame ready to be
                          ///< executed by TheGameLogic?
 ...
 */
NetworkInterface *NetworkInterface::createNetwork() { return NEW Network; }
...
void Network::init() {
  ...
  m_lastFrame = 0;
  m_runAhead =
      min(max(30, MIN_RUNAHEAD),
          MAX_FRAMES_AHEAD / 2);  ///< @todo: don't hard-code the run-ahead.
  m_frameRate = 30;
  m_lastExecutionFrame =
      m_runAhead - 1;  // subtract 1 since we're starting on frame 0
...
void Network::setSawCRCMismatch(void) {
  ...
  m_messageWindow =
      TheWindowManager->winCreateFromScript("Menus/CRCMismatch.wnd");
...
/**
 * Take a user list and build the connection queues and player lists and stuff
 * like that.
 */
void Network::parseUserList(const GameInfo *game) {
  ...
  // Now that we have the players in this game, we need to reset the FrameData
  // stuff.
  m_conMgr->destroyGameMessages();
  m_conMgr->zeroFrames(
      1, m_runAhead - 1);  ///< we zero out m_runAhead frames +1 because the
                           ///< game actually starts at frame 1.
}
...
/**
 * Tell the network which ip address the user has chosen to use.  Well ok, they
 * probably didn't choose it explicitly, but regardless, this is the one we're
 * going to use.
 */
...
/**
 * Take commands from TheCommandList and give them to the connection manager for
 * transport.
 */
void Network::GetCommandsFromCommandList() {
  ...
  while (msg != NULL) {
    next = msg->next();
    if (isTransferCommand(msg)) {  // Is this something we should be sending to
                                   // the other players?
      if (m_localStatus == NETLOCALSTATUS_INGAME) {
        m_conMgr->sendLocalGameMessage(msg, getExecutionFrame());
      }
      TheCommandList->removeMessage(
          msg);  // This does not destroy msg's prev and next pointers, so they
...
/**
 * This is where any processing that needs to be done for specific game
 * messages. Also check here to see if the logic frame number has changed to see
 * if we need to send our info for the last frame to the other players. Return
 * true if the message should be "eaten" by the network.
 */
Bool Network::processCommand(GameMessage *msg) {
  if ((m_lastFrame != TheGameLogic->getFrame()) ||
      (m_localStatus == NETLOCALSTATUS_PREGAME)) {
    // If this is the start of a new game logic frame, then tell the connection
    // manager that the last frame is over and that it should now produce a
    // frame info packet for the other players.

    if (m_localStatus == NETLOCALSTATUS_PREGAME) {
      // a sort-of-hack that prevents extraneous frames from being executed
      // before the game actually starts. Idealy this shouldn't be necessary,
      // but I don't think its hurting anything by being here.
      if (TheGameLogic->getFrame() == 1) {
        m_localStatus = NETLOCALSTATUS_INGAME;
        NetCommandList *netcmdlist = m_conMgr->getFrameCommandList(
            0);  // clear out frame 0 since we skipped it
    ...
    // Only send frame info packets for frames where we are actually going to be
    // in the game. The reason is so we don't have to wait for frame info
    // packets to be sent that aren't going to matter anyways when we actually
    // try to leave the game ourselves.
    if (m_localStatus == NETLOCALSTATUS_INGAME) {
      ...
      for (Int i = m_lastFrameCompleted + 1; i < executionFrame; ++i) {
        ...
        // DEBUG_LOG(("Network::processCommand - calling processFrameTick for
        // frame %d\n", i));
  ...
  // This has to happen after the check to see if this is the start of a new
  // logic frame. The reason is that we have to send all the frame info packets
  // necessary to get to the frame where everyone else is going to see that we
  // left.
  if ((msg->getType() == GameMessage::MSG_CLEAR_GAME_DATA) &&
      (m_localStatus == NETLOCALSTATUS_INGAME)) {
    Int executionFrame = getExecutionFrame();
    DEBUG_LOG(
        ("Network::processCommand - local player leaving, executionFrame = %d, "
         "player leaving on frame %d\n",
         executionFrame, executionFrame + 1));

    m_conMgr->handleLocalPlayerLeaving(executionFrame + 1);
    m_conMgr->processFrameTick(
        executionFrame);  // This is the last command we will execute, so send
                          // the command count. Also, we are guaranteed not to
                          // send any more commands for this frame since the
                          // local status will change to "Leaving" so we don't
                          // have to worry about messing up the other players.
    m_conMgr->processFrameTick(executionFrame +
                               1);  // since we send it for executionFrame+1, we
                                    // need to process both ticks
...
Bool Network::AllCommandsReady(UnsignedInt frame) {
  ...
  return m_conMgr->allCommandsReady(
      frame);  // && m_conMgr->allCRCsReady(frame);
}
...
void Network::RelayCommandsToCommandList(UnsignedInt frame) {
  ...
  while (msg != NULL) {
    ...
    if (cmdType == NETCOMMANDTYPE_GAMECOMMAND) {
      // DEBUG_LOG(("Network::RelayCommandsToCommandList - appending command %d
      // of type %s to command list on frame %d\n", msg->getCommand()->getID(),
      // ((NetGameCommandMsg
      // *)msg->getCommand())->constructGameMessage()->getCommandAsAsciiString().str(),
      // TheGameLogic->getFrame()));
      TheCommandList->appendMessage(
          ((NetGameCommandMsg *)msg->getCommand())->constructGameMessage());
    } else {
...
/**
 * This is where network commands that need to be executed on the same frame
 * should be executed.
 */
void Network::processFrameSynchronizedNetCommand(NetCommandRef *msg) {
  ...
  if (cmdMsg->getNetCommandType() == NETCOMMANDTYPE_PLAYERLEAVE) {
    PlayerLeaveCode retval =
        m_conMgr->processPlayerLeave((NetPlayerLeaveCommandMsg *)cmdMsg);
    if (retval == PLAYERLEAVECODE_LOCAL) {
      DEBUG_LOG(
          ("Network::processFrameSynchronizedNetCommand - Local player left "
           "the game on frame %d.\n",
           TheGameLogic->getFrame()));
      m_localStatus = NETLOCALSTATUS_LEFT;
    } else if (retval == PLAYERLEAVECODE_PACKETROUTER) {
      DEBUG_LOG(
          ("Network::processFrameSynchronizedNetCommand - Packet router left "
           "the game on frame %d\n",
           TheGameLogic->getFrame()));
    } else {
      DEBUG_LOG(
          ("Network::processFrameSynchronizedNetCommand - Client left the game "
           "on frame %d\n",
           TheGameLogic->getFrame()));
    }
    ...
    DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                    ("command to set run ahead to %d and frame rate to %d on "
                     "frame %d actually executed on frame %d\n",
                     netmsg->getRunAhead(), netmsg->getFrameRate(),
                     netmsg->getExecutionFrame(), TheGameLogic->getFrame()));
    ...
    // DEBUG_LOG(("CRC command (%8.8X) on frame %d actually executed on frame
    // %d\n", netmsg->getCRC(), netmsg->getExecutionFrame(),
    // TheGameLogic->getFrame()));
  }
}
...
void Network::processRunAheadCommand(NetRunAheadCommandMsg *msg) {
  ...
  time_t frameGrouping =
      (1000 * m_runAhead) /
      m_frameRate;  // number of miliseconds between packet sends
  frameGrouping =
      frameGrouping /
      2;  // since we only want the latency for one way to be a factor.
          //	DEBUG_LOG(("Network::processRunAheadCommand - trying to set frame
          //grouping to %d.  run ahead = %d, m_frameRate = %d\n", frameGrouping,
          //m_runAhead, m_frameRate));
  if (frameGrouping < 1) {
...
void Network::processDestroyPlayerCommand(NetDestroyPlayerCommandMsg *msg) {
  ...
  if (playerIndex >= MAX_SLOTS) return;
...
void Network::update(void) {
  //
  // 1. Take Commands off TheCommandList, hand them off to the
  // ConnectionManager.
  ...
  GetCommandsFromCommandList();  // Remove commands from TheCommandList and send
                                 // them to the connection manager.
  if (m_conMgr != NULL) {
    if (m_localStatus == NETLOCALSTATUS_INGAME) {
      m_conMgr->updateRunAhead(m_runAhead, m_frameRate, m_didSelfSlug,
                               getExecutionFrame());
      m_didSelfSlug = FALSE;
    }
    // m_conMgr->update(); // Do the priority thing, packetize thing. This also
    // calls the Transport::update function.
    //  depacketizes the incoming packets and puts them on the Network command
    //  list.
  }

  liteupdate();

  if ((m_localStatus == NETLOCALSTATUS_LEFT)) {  // || (m_localStatus ==
                                                 // NETLOCALSTATUS_LEAVING)) {
    endOfGameCheck();
  }

  if (AllCommandsReady(
          TheGameLogic->getFrame())) {  // If all the commands are ready for the
                                        // next frame...
    m_conMgr->handleAllCommandsReady();
    //		DEBUG_LOG(("Network::update - frame %d is ready\n",
    //TheGameLogic->getFrame()));
    if (timeForNewFrame()) {  // This needs to come after any other pre-frame
                              // execution checks as this changes the timing
                              // variables.
      RelayCommandsToCommandList(
          TheGameLogic->getFrame());  // Put the commands for the next frame on
                                      // TheCommandList.
      m_frameDataReady =
          TRUE;  // Tell the GameEngine to run the commands for the new frame.
    }
  }
}
...
Bool Network::timeForNewFrame() {
  ...
  /*
   * If we're pushing up against the edge of our run ahead, we should slow the
   * framerate down a bit to avoid being frozen by spikes in network lag.  This
   * will happen if another user's computer is running too far behind us, so we
   * need to slow down to let them catch up.
   */
  if (m_conMgr != NULL) {
    Real cushion = m_conMgr->getMinimumCushion();
    Real runAheadPercentage =
        m_runAhead * (TheGlobalData->m_networkRunAheadSlack /
                      (Real)100.0);  // If we are at least 50% into our slack,
                                     // we need to slow down.
    if (cushion < runAheadPercentage) {
      //			DEBUG_LOG(("Average cushion = %f, run ahead
      //percentage = %f.  Adjusting frameDelay from %I64d to ", cushion,
      //runAheadPercentage, frameDelay));
      frameDelay +=
          frameDelay / 10;  // temporarily decrease the frame rate by 20%.
                            //			DEBUG_LOG(("%I64d\n", frameDelay));
      m_didSelfSlug = TRUE;
      //		} else {
      //			DEBUG_LOG(("Average cushion = %f, run ahead
  ...
  if (curTime >= m_nextFrameTime) {
    //		DEBUG_LOG(("Allowing a new frame, frameDelay = %I64d, curTime -
    //m_nextFrameTime = %I64d\n", frameDelay, curTime - m_nextFrameTime));

    //		if (m_nextFrameTime + frameDelay < curTime) {
    if ((m_nextFrameTime + (2 * frameDelay)) < curTime) {
      // If we get too far behind on our framerate we need to reset the
      // nextFrameTime thing.
      m_nextFrameTime = curTime;
      //			DEBUG_LOG(("Initializing m_nextFrameTime to
      //%I64d\n", m_nextFrameTime));
    } else {
  ...
  //	DEBUG_LOG(("Slowing down frame rate. frame rate = %d, frame delay =
  //%I64d, curTime - m_nextFrameTime = %I64d\n", m_frameRate, frameDelay,
  //curTime - m_nextFrameTime));
  return FALSE;
}

/**
 * Returns true if the game commands for the next frame have been put on the
 * command list.
 */
...
/**
 * returns the number of incoming bytes per second averaged over the last 30
 * sec.
 */
...
/**
 * returns the number of incoming packets per second averaged over the last 30
 * sec.
 */
...
/**
 * returns the number of outgoing bytes per second averaged over the last 30
 * sec.
 */
...
/**
 * returns the number of outgoing packets per second averaged over the last 30
 * sec.
 */
...
/**
 * returns the number of bytes received per second that are not from a generals
 * client averaged over 30 sec.
 */
...
/**
 * returns the number of packets received per second that are not from a
 * generals client averaged over 30 sec.
 */
...
// send a file.  woohoo.
void Network::sendFile(AsciiString path, UnsignedByte playerMask,
                       UnsignedShort commandID) {
...
Bool Network::areAllQueuesEmpty(void) { return m_conMgr->canILeave(); }
