...
static Bool doFileTransfer(AsciiString filename, MapTransferLoadScreen *ls,
                           Int mask) {
  ...
  if (mask) {
    ...
    while (!fileTransferDone) {
      ...
      // get the progress for each player, and take the min for our overall
      // progress
      ...
      for (i = 1; i < MAX_SLOTS; ++i) {
        if (TheGameInfo->getConstSlot(i)->isHuman() &&
          ...
          Int slotTransferPercent =
              TheNetwork->getFileTransferProgress(i, filename);
          fileTransferPercent = min(fileTransferPercent, slotTransferPercent);

          if (slotTransferPercent == 0)
            ls->processProgress(i, slotTransferPercent,
      ...
      Int now = timeGetTime();
      if (now > startTime + timeoutPeriod)  // bail if we don't finish in a
...
Bool DoAnyMapTransfers(GameInfo *game) {
  ...
  if (ok) ok = doFileTransfer(game->getMap(), ls, mask);
  delete ls;
  ls = NULL;
  if (!ok) TheShell->showShell();
  return ok;
}
