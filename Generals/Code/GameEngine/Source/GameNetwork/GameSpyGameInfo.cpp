...
// ////////////////////////////////////////////////////// GameSpy game setup
// state info Author: Matthew D. Campbell, December 2001
...
void GameSpyStartGame(void) {
  if (TheGameSpyGame) {
    ...
    for (i = 0; i < MAX_SLOTS; ++i) {
      ...
      if (slot && slot->isOccupied()) numUsers++;
    }
...
void GameSpyLaunchGame(void) {
  if (TheGameSpyGame) {
    ...
    /*
    if (!TheGameSpyGame->amIHost())
            TheNetwork->setLocalAddress((207<<24) | (138<<16) | (47<<8) | 15,
    8088); else
    */
    TheNetwork->setLocalAddress(
        TheGameSpyGame->getLocalIP(),
        TheNAT->getSlotPort(TheGameSpyGame->getLocalSlotNum()));
...
AsciiString GameSpyGameInfo::generateGameResultsPacket(void) {
  ...
  Bool sawGameEnd =
      (endFrame > 0);  // && localSlot->lastFrameInGame() <= endFrame);
  Int winningTeam = -1;
  Int numPlayers = 0;
  ...
  results.format(
      ...
      "d,duration=%d,numPlayers=%d,isQM=%d",
      getSeed(), localSlotNum, TheNetwork->sawCRCMismatch(), sawGameEnd,
      winningTeam, (numTeamsAtGameEnd != 0), endFrame, numPlayers, m_isQM);
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (p) {
      ...
      playerStr.format(",player%d=%s,playerID%d=%d,locale%d=%s", playerID,
      ...
      playerStr.format(",fps%d=%d,cash%d=%d,capturedTech%d=%d,discon%d=%d",
                       playerID, fps, playerID, earnings, playerID,
                       techCaptured, playerID, disconnected);
      results.concat(playerStr);
