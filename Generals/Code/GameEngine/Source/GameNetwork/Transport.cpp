...
Transport::~Transport(void) { reset(); }
...
Bool Transport::init(UnsignedInt ip, UnsignedShort port) {
  ...
  if (TheGlobalData->m_packetLoss) m_usePacketLoss = true;
#endif
...
Bool Transport::doSend() {
  ...
  for (i = 0; i < MAX_MESSAGES; ++i) {
    if (m_outBuffer[i].length != 0) {
      int bytesSent = 0;
      // Send this message
      if ((bytesSent = m_udpsock->Write(
               (unsigned char *)(&m_outBuffer[i]),
               m_outBuffer[i].length + sizeof(TransportMessageHeader),
               m_outBuffer[i].addr, m_outBuffer[i].port)) > 0) {
        // DEBUG_LOG(("Sending %d bytes to %d:%d\n", m_outBuffer[i].length +
        // sizeof(TransportMessageHeader), m_outBuffer[i].addr,
        // m_outBuffer[i].port));
        m_outgoingPackets[m_statisticsSlot]++;
        m_outgoingBytes[m_statisticsSlot] +=
            m_outBuffer[i].length + sizeof(TransportMessageHeader);
        m_outBuffer[i].length = 0;  // Remove from queue
        //				DEBUG_LOG(("Transport::doSend - sent %d
        //butes to %d.%d.%d.%d:%d\n", bytesSent, 					(m_outBuffer[i].addr >> 24) &
        //0xff, 					(m_outBuffer[i].addr >> 16) & 0xff, 					(m_outBuffer[i].addr >> 8) &
        //0xff, 					m_outBuffer[i].addr & 0xff, 					m_outBuffer[i].port));
      } else {
        // DEBUG_LOG(("Could not write to socket!!!  Not discarding
        // message!\n"));
        retval = FALSE;
...
Bool Transport::doRecv() {
  ...
  while ((len = m_udpsock->Read(buf, MAX_MESSAGE_LEN, &from)) > 0) {
    ...
    //ntohl(from.sin_addr.S_un.S_addr), ntohs(from.sin_port)));
...
Bool Transport::isGeneralsPacket(TransportMessage *msg) {
  ...
  if (msg->length < 0 || msg->length > MAX_MESSAGE_LEN) return false;
  ...
  if (crc.get() != msg->header.crc) return false;

  if (msg->header.magic != GENERALS_MAGIC_NUMBER) return false;
...
Real Transport::getIncomingBytesPerSecond(void) {
  ...
  for (int i = 0; i < MAX_TRANSPORT_STATISTICS_SECONDS; ++i) {
    if (i != m_statisticsSlot) val += m_incomingBytes[i];
  }
...
Real Transport::getIncomingPacketsPerSecond(void) {
  Real val = 0.0;
  for (int i = 0; i < MAX_TRANSPORT_STATISTICS_SECONDS; ++i) {
    if (i != m_statisticsSlot) val += m_incomingPackets[i];
  }
  return val / (MAX_TRANSPORT_STATISTICS_SECONDS - 1);
}
...
Real Transport::getOutgoingBytesPerSecond(void) {
  Real val = 0.0;
  for (int i = 0; i < MAX_TRANSPORT_STATISTICS_SECONDS; ++i) {
    if (i != m_statisticsSlot) val += m_outgoingBytes[i];
  }
  return val / (MAX_TRANSPORT_STATISTICS_SECONDS - 1);
}
...
Real Transport::getOutgoingPacketsPerSecond(void) {
  Real val = 0.0;
  for (int i = 0; i < MAX_TRANSPORT_STATISTICS_SECONDS; ++i) {
    if (i != m_statisticsSlot) val += m_outgoingPackets[i];
  }
  return val / (MAX_TRANSPORT_STATISTICS_SECONDS - 1);
}
...
Real Transport::getUnknownBytesPerSecond(void) {
  Real val = 0.0;
  for (int i = 0; i < MAX_TRANSPORT_STATISTICS_SECONDS; ++i) {
    if (i != m_statisticsSlot) val += m_unknownBytes[i];
  }
  return val / (MAX_TRANSPORT_STATISTICS_SECONDS - 1);
}
...
Real Transport::getUnknownPacketsPerSecond(void) {
  Real val = 0.0;
  for (int i = 0; i < MAX_TRANSPORT_STATISTICS_SECONDS; ++i) {
    if (i != m_statisticsSlot) val += m_unknownPackets[i];
  }
  return val / (MAX_TRANSPORT_STATISTICS_SECONDS - 1);
}
