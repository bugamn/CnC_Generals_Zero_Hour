  ...
  if (AmIHost()) {
    ...
    if (i != MAX_SLOTS) {
      ...
      const MapMetaData *mapData =
          TheMapCache->findMap(m_currentGame->getMap());
...
void LANAPI::OnGameStart(void) {
  // DEBUG_LOG(("Map is '%s', preview is '%s'\n", m_currentGame->getMap().str(),
  // GetPreviewFromMap(m_currentGame->getMap()).str())); DEBUG_LOG(("Map is
  // '%s', INI is '%s'\n", m_currentGame->getMap().str(),
  // GetINIFromMap(m_currentGame->getMap()).str()));
  ...
  if (m_currentGame) {
    ...
    AsciiString option;
    option.format("%d",
                  m_currentGame->getLANSlot(m_currentGame->getLocalSlotNum())
                      ->getPlayerTemplate());
    ...
    if (!filesOk || TheMapCache->findMap(m_currentGame->getMap()) == NULL) {
      DEBUG_LOG(
          ("After transfer, we didn't really have the map.  Bailing...\n"));
      ...
      OnChat(UnicodeString::TheEmptyString, 0,
             TheGameText->fetch("GUI:CouldNotTransferMap"), LANCHAT_SYSTEM);
      return;
    }
...
void LANAPI::OnGameOptions(UnsignedInt playerIP, Int playerSlot,
                           AsciiString options) {
  ...
  if (playerSlot == 0 && !m_currentGame->amIHost()) {
    ...
    AsciiString oldOptions = GameInfoToAsciiString(
        m_currentGame);  // save these off for if we get booted
      ...
      if (options.compare("HELLO") == 0) {
        ...
        if (key == "Color") {
          ...
          if ((val >= FirewallHelperClass::FIREWALL_TYPE_SIMPLE) &&
            ...
            DEBUG_LOG(
                ("NAT behavior set to %d for player %d\n", val, playerSlot));
            change = true;
          } else {
        ...
        if (change) {
          if (shouldUnaccept) m_currentGame->resetAccepted();
...
void LANAPI::OnSlotList( ReturnType ret, LANGameInfo *theGame )
{
        ...
        if (!foundMe)
        {
                ...
                // We're counting on the fact that OnPlayerLeave winds up
calling reset on TheLAN. OnPlayerLeave(m_name); return;
        }
...
void LANAPI::OnGameJoin(ReturnType ret, LANGameInfo *theGame) {
  if (ret == RET_OK) {
    ...
    options.format(
        "NAT=%d",
        FirewallHelperClass::FIREWALL_TYPE_SIMPLE);  // BGC: This is a LAN game,
...
void LANAPI::OnPlayerLeave(UnicodeString player) {
  DEBUG_ASSERTCRASH(!m_inLobby && m_currentGame, ("Game info is gone!"));
  if (m_inLobby || !m_currentGame || m_currentGame->isGameInProgress()) return;
  ...
  if (m_name.compare(player) == 0) {
    ...
    if (m_currentGame && m_currentGame->isInGame() &&
        m_currentGame->getLocalSlotNum() >= 0) {
      LANPreferences pref;
      AsciiString option;
      option.format("%d",
                    m_currentGame->getLANSlot(m_currentGame->getLocalSlotNum())
...
void LANAPI::OnGameCreate(ReturnType ret) {
  if (ret == RET_OK) {
    ...
    // RequestGameAnnounce( ); // can't do this here, since we don't have a map
    // set
  } else {
    if (m_inLobby) {
      switch (ret) {
        case RET_GAME_EXISTS:
          GadgetListBoxAddEntryText(listboxChatWindow,
                                    TheGameText->fetch("LAN:ErrorGameExists"),
                                    chatSystemColor, -1, -1);
          break;
        case RET_BUSY:
          GadgetListBoxAddEntryText(listboxChatWindow,
                                    TheGameText->fetch("LAN:ErrorBusy"),
                                    chatSystemColor, -1, -1);
          break;
        default:
          GadgetListBoxAddEntryText(listboxChatWindow,
                                    TheGameText->fetch("LAN:ErrorUnknown"),
                                    chatSystemColor, -1, -1);
      }
    }
  }
...
void LANAPI::OnPlayerList(LANPlayer *playerList) {
  if (m_inLobby) {
    ...
    if (selectedIndex != -1)
      selectedIP = (UnsignedInt)GadgetListBoxGetItemData(listboxPlayers,
                                                         selectedIndex, 0);
    ...
    while (player) {
      Int addedIndex = GadgetListBoxAddEntryText(
          listboxPlayers, player->getName(), playerColor, -1, -1);
      GadgetListBoxSetItemData(listboxPlayers, (void *)player->getIP(),
                               addedIndex, 0);

      if (selectedIP == player->getIP()) indexToSelect = addedIndex;
...
void LANAPI::OnChat(UnicodeString player, UnsignedInt ip, UnicodeString message,
                    ChatType format) {
  ...
  switch (format) {
    case LANAPIInterface::LANCHAT_SYSTEM:
      ...
      index = GadgetListBoxAddEntryText(chatWindow, unicodeChat,
                                        chatSystemColor, -1, -1);
      break;
    ...
    default: {
        ...
        if (slotNum >= 0) {
          ...
          if (gs) {
            ...
            MultiplayerColorDefinition *def =
                TheMultiplayerSettings->getColor(colorIndex);
            if (def) chatColor = def->getColor();
          }
        }
      }
      ...
      unicodeChat.concat(message);
      if (ip == m_localIP)
        index = GadgetListBoxAddEntryText(chatWindow, unicodeChat, chatColor,
                                          -1, -1);
      else
        index = GadgetListBoxAddEntryText(chatWindow, unicodeChat, chatColor,
                                          -1, -1);
