...
// Props to Steve Tall for figuring all the NAT and Firewall behavior patterns
// out, making my job a LOT easier. Desc:   Resolves NAT'd IPs and port numbers
// for the other players in a game.
///////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 * his own choice.
 */
//										m_connectionPairs[num
//nodes]	[round]			 [node index]
/* static */ Int
    NAT::m_connectionPairs[MAX_SLOTS - 1][MAX_SLOTS - 1][MAX_SLOTS] = {
        {
            // 2 nodes
...
/* static */ Int NAT::m_timeBetweenRetries =
    500;  // .5 seconds between retries sounds good to me.
/* static */ time_t NAT::m_manglerRetryTimeInterval =
    300;                                                // sounds good to me.
/* static */ Int NAT::m_maxAllowedManglerRetries = 25;  // works for me.
/* static */ time_t NAT::m_keepaliveInterval =
    15000;  // 15 seconds between keepalive packets seems good.
/* static */ time_t NAT::m_timeToWaitForPort =
    15000;  // wait for 15 seconds for the other player's port number.
/* static */ time_t NAT::m_timeForRoundTimeout =
    15000;  // wait for at most 15 seconds for each connection round to finish.
...
NAT::NAT() {
  ...
  for (Int i = 0; i < MAX_SLOTS; i++) m_sourcePorts[i] = 0;
  m_spareSocketPort = 0;
...
NATStateType NAT::update() {
  ...
  if (m_NATState == NATSTATE_DONE) {
    ...
    // check for timeout.  Timing out is not a fatal error - it just means we
    // didn't get the other player's stats.  We'll see 0/0 as his record, but we
    // can still play him just fine.
    UnsignedInt now = timeGetTime();
    if (now > s_startStatWaitTime + MS_TO_WAIT_FOR_STATS) {
      DEBUG_LOG(
          ("Timed out waiting for stats.  Let's just start the dang game.\n"));
      timedOut = TRUE;
    }
    ...
    if (allConnectionsDoneThisRound() == TRUE) {
      ...
      ++m_connectionRound;
      //			m_roundTimeout = timeGetTime() +
      //TheGameSpyConfig->getRoundTimeout();
      m_roundTimeout = timeGetTime() + m_timeForRoundTimeout;
      DEBUG_LOG(
          ("NAT::update - done with connection round, moving on to round %d\n",
           m_connectionRound));

      // we finished that round, now check to see if we're done, or if there are
      // more rounds to go.
      if (allConnectionsDone() == TRUE) {
        // we're all done, time to go back home.
        m_NATState = NATSTATE_WAITFORSTATS;

        // 2/19/03 BGC - we have successfully negotaited a NAT thingy, so our
        // behavior must be correct so therefore we don't need to refresh our
        // NAT even if we previously thought we had to.
    ...
    if (state == NATCONNECTIONSTATE_FAILED) {
      ...
      TheEstablishConnectionsMenu->endMenu();
      if (TheFirewallHelper != NULL) {
        // we failed NAT negotiation, perhaps we need to redetect our firewall
        // settings. We don't trust the user to do it for themselves so we force
        // them to do it next time the log in. 2/19/03 - ok, we don't want to do
        // this right away, if the user tries to play in another game before
        // they log out and log back in the game won't have a chance at working.
        // so we need to simply flag it so that when they log out the firewall
        // behavior gets blown away.
        TheFirewallHelper->flagNeedToRefresh(TRUE);
        //				TheWritableGlobalData->m_firewallBehavior
        //= FirewallHelperClass::FIREWALL_TYPE_UNKNOWN;
        //				TheFirewallHelper->writeFirewallBehavior();

        delete TheFirewallHelper;
        TheFirewallHelper = NULL;
      }
      // we failed to connect, so we don't have to pass on the transport to the
      // network.
      if (m_transport != NULL) {
        delete m_transport;
        m_transport = NULL;
...
// if we didn't get a response, check to see if its time to send another packet
// to it
NATConnectionState NAT::connectionUpdate() {
  ...
  if (m_beenProbed == FALSE) {
    if (timeGetTime() >= m_nextPortSendTime) {
      ...
      //targetSlot);
      sendMangledPortNumberToTarget(m_sourcePorts[m_targetNodeNumber],
                                    targetSlot);
      //			m_nextPortSendTime = timeGetTime() +
      //TheGameSpyConfig->getRetryInterval();
  ...
  if (timeGetTime() >= m_nextKeepaliveTime) {
    for (Int node = 0; node < m_numNodes; ++node) {
      if (m_myConnections[node] == TRUE) {
        ...
        DEBUG_ASSERTCRASH(slot != NULL,
                          ("Trying to send keepalive to a NULL slot"));
        if (slot != NULL) {
          UnsignedInt ip = slot->getIP();
          DEBUG_LOG(
              ("NAT::connectionUpdate - sending keep alive to node %d at "
               "%d.%d.%d.%d:%d\n",
               node, ip >> 24, (ip >> 16) & 0xff, (ip >> 8) & 0xff, ip & 0xff,
               slot->getPort()));
          m_transport->queueSend(ip, slot->getPort(),
                                 (const unsigned char *)"KEEPALIVE",
                                 strlen("KEEPALIVE") + 1);
        }
      }
    }
    //		m_nextKeepaliveTime = timeGetTime() +
    //TheGameSpyConfig->getKeepaliveInterval();
    ...
    if (m_transport->m_inBuffer[i].length > 0) {
...
#endif
      DEBUG_LOG(
          ("NAT::connectionUpdate - got a packet from %d.%d.%d.%d:%d, length = "
           "%d\n",
           ip >> 24, (ip >> 16) & 0xff, (ip >> 8) & 0xff, ip & 0xff,
           m_transport->m_inBuffer[i].port, m_transport->m_inBuffer[i].length));
      ...
      if (memcmp(data, "PROBE", strlen("PROBE")) == 0) {
        ...
        if (fromNode == m_targetNodeNumber) {
          DEBUG_LOG(
              ("NAT::connectionUpdate - probe was sent by our target, setting "
               "connection state %d to done.\n",
               m_targetNodeNumber));
...
#endif
            DEBUG_LOG(
                ("NAT::connectionUpdate - incomming packet has different from "
                 "address than we expected, incoming: %d.%d.%d.%d expected: "
                 "%d.%d.%d.%d\n",
                 fromIP >> 24, (fromIP >> 16) & 0xff, (fromIP >> 8) & 0xff,
                 fromIP & 0xff, slotIP >> 24, (slotIP >> 16) & 0xff,
                 (slotIP >> 8) & 0xff, slotIP & 0xff));
            targetSlot->setIP(fromIP);
          }
          if (m_transport->m_inBuffer[i].port != targetSlot->getPort()) {
            DEBUG_LOG(
                ("NAT::connectionUpdate - incoming packet came from a "
                 "different port than we expected, incoming: %d expected: %d\n",
      ...
      if (memcmp(data, "KEEPALIVE", strlen("KEEPALIVE")) == 0) {
        ...
        DEBUG_LOG(
            ("NAT::connectionUpdate - got keepalive from %d.%d.%d.%d:%d\n",
             ip >> 24, (ip >> 16) & 0xff, (ip >> 8) && 0xff, ip & 0xff,
             m_transport->m_inBuffer[i].port));
        m_transport->m_inBuffer[i].length = 0;
      }
    }
  }
  ...
  if (m_connectionStates[m_localNodeNumber] ==
    ...
    if ((m_timeTillNextSend != -1) && (m_timeTillNextSend <= timeGetTime())) {
      if (m_numRetries > m_maxNumRetriesAllowed) {
        DEBUG_LOG(
            ("NAT::connectionUpdate - too many retries, connection failed.\n"));
        setConnectionState(m_localNodeNumber, NATCONNECTIONSTATE_FAILED);

        notifyUsersOfConnectionFailed(m_localNodeNumber);
      } else {
        DEBUG_LOG(
            ("NAT::connectionUpdate - trying to send another probe (#%d) to "
             "our target\n",
             m_numRetries + 1));
        // Send a probe.
        sendAProbe(targetSlot->getIP(), targetSlot->getPort(),
                   m_localNodeNumber);
        //				m_timeTillNextSend = timeGetTime() +
        //TheGameSpyConfig->getRetryInterval();
        m_timeTillNextSend = timeGetTime() + m_timeBetweenRetries;

        // tell the target they've been probed. In other words, our port is
        // open.
  ...
  // we are waiting for a response from the mangler to tell us what port we're
  // using.
  if (m_connectionStates[m_localNodeNumber] ==
    ...
    if (mangledPort != 0) {
      // we got a response.  now we need to start probing (unless of course we
      // have a netgear)
      ...
      if (timeGetTime() >= m_manglerRetryTime) {
        ++m_manglerRetries;
        //				if (m_manglerRetries >
        //TheGameSpyConfig->getMaxManglerRetries()) {
        if (m_manglerRetries > m_maxAllowedManglerRetries) {
          ...
          DEBUG_LOG(
              ("NAT::connectionUpdate - couldn't talk with the mangler using "
               "default port number\n"));
          sendMangledPortNumberToTarget(
              getSlotPort(m_connectionNodes[m_localNodeNumber].m_slotIndex),
              targetSlot);
          m_sourcePorts[m_targetNodeNumber] =
              getSlotPort(m_connectionNodes[m_localNodeNumber].m_slotIndex);
          setConnectionState(m_localNodeNumber,
                             NATCONNECTIONSTATE_WAITINGFORRESPONSE);
        } else {
          if (TheFirewallHelper != NULL) {
            DEBUG_LOG(
                ("NAT::connectionUpdate - trying to send to the mangler again. "
                 "mangler address: %d.%d.%d.%d, from port: %d, packet ID:%d\n",
                 m_manglerAddress >> 24, (m_manglerAddress >> 16) & 0xff,
                 (m_manglerAddress >> 8) & 0xff, m_manglerAddress & 0xff,
                 m_spareSocketPort, m_packetID));
            TheFirewallHelper->sendToManglerFromPort(
                m_manglerAddress, m_spareSocketPort, m_packetID);
          }
          //					m_manglerRetryTime =
          //TheGameSpyConfig->getRetryInterval() + timeGetTime();
  ...
  if (m_connectionStates[m_localNodeNumber] ==
    ...
    if (timeGetTime() > m_timeoutTime) {
      DEBUG_LOG(
          ("NAT::connectionUpdate - waiting too long to get the other player's "
           "port number, failed.\n"));
      setConnectionState(m_localNodeNumber, NATCONNECTIONSTATE_FAILED);
...
void NAT::establishConnectionPaths() {
  ...
  DEBUG_LOG(
      ("NAT::establishConnectionPaths - using %d as our starting port number\n",
  ...
  DEBUG_ASSERTCRASH(m_slotList != NULL,
                    ("NAT::establishConnectionPaths - don't have a slot list"));
  if (m_slotList == NULL) {
    return;
  }
    ...
    if (m_slotList[i] != NULL) {
      if (m_slotList[i]->isHuman()) {
        DEBUG_LOG(("NAT::establishConnectionPaths - slot %d is %ls\n", i,
                   m_slotList[i]->getName().str()));
        ++m_numNodes;
      }
    }
  }
  DEBUG_LOG(
      ("NAT::establishConnectionPaths - number of nodes: %d\n", m_numNodes));
  ...
  // nodes with a netgear bug behavior need to be matched up first. This
  // prevents the NAT table from being reset for connections to other nodes.
  // This also happens to be the reason why I call them "nodes" rather than
  // "slots" or "players" as the ordering has to be messed with to get the
  // netgears to make love, not war.
  DEBUG_LOG(
      ("NAT::establishConnectionPaths - about to set up the node list\n"));
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    if ((m_slotList != NULL) && (m_slotList[i] != NULL)) {
      if ((m_slotList[i]->getNATBehavior() &
           FirewallHelperClass::FIREWALL_TYPE_NETGEAR_BUG) != 0) {
        if (otherNetgearNum == -1) {
          // this is the start of a new pair, put it in as the first non -1 node
          // connection pair thing.
          Int nodeindex = 0;
          while (
              (m_connectionPairs[m_connectionPairIndex][0][nodeindex] == -1) ||
              (m_connectionNodes[nodeindex].m_slotIndex != -1)) {
            ++nodeindex;
          }
          m_connectionNodes[nodeindex].m_slotIndex = i;
          m_connectionNodes[nodeindex].m_behavior =
              m_slotList[i]->getNATBehavior();
          connectionAssigned[i] = TRUE;
          otherNetgearNum = nodeindex;
          DEBUG_LOG(
              ("NAT::establishConnectionPaths - first netgear in pair. "
               "assigning node %d to slot %d (%ls)\n",
               nodeindex, i, m_slotList[i]->getName().str()));
        } else {
          // this is the second in the pair of netgears, pair this up with the
          // other one for the first round.
          Int nodeindex = 0;
          while (m_connectionPairs[m_connectionPairIndex][0][nodeindex] !=
                 otherNetgearNum) {
            ++nodeindex;
          }
          m_connectionNodes[nodeindex].m_slotIndex = i;
          ...
          otherNetgearNum = -1;
          DEBUG_LOG(
              ("NAT::establishConnectionPaths - second netgear in pair. "
               "assigning node %d to slot %d (%ls)\n",
               nodeindex, i, m_slotList[i]->getName().str()));
        }
      }
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    ...
    DEBUG_LOG(
        ("NAT::establishConnectionPaths - assigning node %d to slot %d (%ls)\n",
         nodeindex, i, m_slotList[i]->getName().str()));
    m_connectionNodes[nodeindex].m_slotIndex = i;
...
// sanity check
  ...
  for (i = 0; i < m_numNodes; ++i) {
    DEBUG_ASSERTCRASH(connectionAssigned[i] == TRUE,
                      ("connection number %d not assigned", i));
  }
#endif
  ...
  Int playerNum = 0;
  for (i = 0; i < MAX_SLOTS; ++i) {
    while ((i < MAX_SLOTS) && (m_slotList[i] != NULL) &&
           !(m_slotList[i]->isHuman())) {
      ++i;
    }
    if (i >= MAX_SLOTS) {
      break;
    ...
    if (i != TheGameSpyGame->getLocalSlotNum()) {
      TheEstablishConnectionsMenu->setPlayerName(playerNum,
                                                 m_slotList[i]->getName());
      TheEstablishConnectionsMenu->setPlayerStatus(
          playerNum, NATCONNECTIONSTATE_WAITINGTOBEGIN);
      ++playerNum;
    }
  }
...
void NAT::attachSlotList(GameSlot *slotList[], Int localSlot,
  ...
  DEBUG_LOG(
      ("NAT::attachSlotList - initting the transport socket with address "
       "%d.%d.%d.%d:%d\n",
       m_localIP >> 24, (m_localIP >> 16) & 0xff, (m_localIP >> 8) & 0xff,
       m_localIP & 0xff, getSlotPort(localSlot)));

  m_startingPortNumber =
      NETWORK_BASE_PORT_NUMBER + ((timeGetTime() / 1000) % 20000);
...
void NAT::generatePortNumbers(GameSlot *slotList[], Int localSlot) {
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if (slotList[i] != NULL) {
      if ((i == localSlot) &&
          (TheWritableGlobalData->m_firewallPortOverride != 0)) {
...
Transport *NAT::getTransport() { return m_transport; }
...
void NAT::doThisConnectionRound() {
  DEBUG_LOG((
      "NAT::doThisConnectionRound - starting process for connection round %d\n",
      m_connectionRound));
  ...
  for (i = 0; i < m_numNodes; ++i) {
    Int targetNodeNumber =
        m_connectionPairs[m_connectionPairIndex][m_connectionRound][i];
    DEBUG_LOG(
        ("NAT::doThisConnectionRound - node %d needs to connect to node %d\n",
         i, targetNodeNumber));
    if (targetNodeNumber != -1) {
      if (i == m_localNodeNumber) {
        m_targetNodeNumber = targetNodeNumber;
        DEBUG_LOG(
            ("NAT::doThisConnectionRound - Local node is connecting to node "
             "%d\n",
             m_targetNodeNumber));
        UnsignedInt targetSlotIndex =
            m_connectionNodes[(m_connectionPairs[m_connectionPairIndex]
                                                [m_connectionRound][i])]
                .m_slotIndex;
        GameSlot *targetSlot = m_slotList[targetSlotIndex];
        GameSlot *localSlot =
            m_slotList[m_connectionNodes[m_localNodeNumber].m_slotIndex];

        DEBUG_ASSERTCRASH(localSlot != NULL, ("local slot is NULL"));
        DEBUG_ASSERTCRASH(
            targetSlot != NULL,
            ("trying to negotiate with a NULL target slot, slot is %d",
             m_connectionPairs[m_connectionPairIndex][m_connectionRound][i]));
        DEBUG_LOG(
            ("NAT::doThisConnectionRound - Target slot index = %d (%ls)\n",
             targetSlotIndex, m_slotList[targetSlotIndex]->getName().str()));
        DEBUG_LOG(
            ("NAT::doThisConnectionRound - Target slot has NAT behavior 0x%8X, "
             "local slot has NAT behavior 0x%8X\n",
        ...
        DEBUG_LOG(
            ("NAT::doThisConnectionRound - Target slot has IP %d.%d.%d.%d  "
             "Local slot has IP %d.%d.%d.%d\n",
             targetIP >> 24, (targetIP >> 16) & 0xff, (targetIP >> 8) & 0xff,
             targetIP & 0xff, localIP >> 24, (localIP >> 16) & 0xff,
             (localIP >> 8) & 0xff, localIP & 0xff));

        if (((targetSlot->getNATBehavior() &
              FirewallHelperClass::FIREWALL_TYPE_NETGEAR_BUG) == 0) &&
            ((localSlot->getNATBehavior() &
              FirewallHelperClass::FIREWALL_TYPE_NETGEAR_BUG) != 0)) {
          // we have a netgear bug type behavior and the target does not, so we
          // need them to send to us first to avoid having our NAT table reset.

          DEBUG_LOG(
              ("NAT::doThisConnectionRound - Local node has a netgear and the "
               "target node does not, need to delay our probe.\n"));
          m_timeTillNextSend = -1;
        }

        // figure out which port number I'm using for this connection
        // this merely starts to talk to the mangler server, we have to keep
        // calling the update function till we get a response.
        DEBUG_LOG(
            ("NAT::doThisConnectionRound - About to attempt to get the next "
             "mangled source port\n"));
        ...
        //TheGameSpyConfig->getPortTimeout();
        m_timeoutTime = timeGetTime() + m_timeToWaitForPort;
      } else {
        // this is someone else that needs to connect to someone, so wait till
        // they tell us that they're done.
        setConnectionState(i, NATCONNECTIONSTATE_WAITINGFORRESPONSE);
      }
    } else {
      // no one to connect to, so this one is done.
      DEBUG_LOG(
          ("NAT::doThisConnectionRound - node %d has no one to connect to, so "
           "they're done\n",
           i));
...
void NAT::sendAProbe(UnsignedInt ip, UnsignedShort port, Int fromNode) {
  DEBUG_LOG(
      ("NAT::sendAProbe - sending a probe from port %d to %d.%d.%d.%d:%d\n",
       getSlotPort(m_connectionNodes[m_localNodeNumber].m_slotIndex), ip >> 24,
       (ip >> 16) & 0xff, (ip >> 8) & 0xff, ip & 0xff, port));
  AsciiString str;
  str.format("PROBE%d", fromNode);
  m_transport->queueSend(ip, port, (unsigned char *)str.str(),
                         str.getLength() + 1);
  m_transport->doSend();
}

// find the next mangled source port, and then send it to the other player.
// if this requires talking to the mangler, we'll have to wait till a later
// update to send our port out.
void NAT::sendMangledSourcePort() {
  UnsignedShort sourcePort =
      getSlotPort(m_connectionNodes[m_localNodeNumber].m_slotIndex);

  FirewallHelperClass::tFirewallBehaviorType fwType =
      m_slotList[m_connectionNodes[m_localNodeNumber].m_slotIndex]
          ->getNATBehavior();
  GameSlot *targetSlot =
      m_slotList[m_connectionNodes[m_targetNodeNumber].m_slotIndex];
  DEBUG_ASSERTCRASH(targetSlot != NULL,
                    ("NAT::sendMangledSourcePort - targetSlot is NULL"));
  if (targetSlot == NULL) {
    DEBUG_LOG(
        ("NAT::sendMangledSourcePort - targetSlot is NULL, failed this "
         "connection\n"));
    setConnectionState(m_localNodeNumber, NATCONNECTIONSTATE_FAILED);
    return;
  }
  ...
  GameSlot *localSlot =
      m_slotList[m_connectionNodes[m_localNodeNumber].m_slotIndex];
  DEBUG_ASSERTCRASH(localSlot != NULL,
                    ("NAT::sendMangledSourcePort - localSlot is NULL, WTF?"));
  if (localSlot == NULL) {
    DEBUG_LOG(
        ("NAT::sendMangledSourcePort - localSlot is NULL, failed this "
         "connection\n"));
    setConnectionState(m_localNodeNumber, NATCONNECTIONSTATE_FAILED);
    return;
  }
  ...
  if (targetSlot->getIP() == localSlot->getIP()) {
...
#endif
    DEBUG_LOG(
        ("NAT::sendMangledSourcePort - target and I are behind the same NAT, "
         "no mangling\n"));
    DEBUG_LOG(
        ("NAT::sendMangledSourcePort - I am %ls, target is %ls, my IP is "
         "%d.%d.%d.%d, target IP is %d.%d.%d.%d\n",
         localSlot->getName().str(), targetSlot->getName().str(), localip >> 24,
         (localip >> 16) & 0xff, (localip >> 8) & 0xff, localip & 0xff,
         targetip >> 24, (targetip >> 16) & 0xff, (targetip >> 8) & 0xff,
         targetip & 0xff));
    ...
    setConnectionState(m_localNodeNumber,
                       NATCONNECTIONSTATE_WAITINGFORMANGLEDPORT);
    // In case you're wondering, we don't set the m_previousSourcePort here
    // because this will be a different source address than what other nodes
    // will likely see (unless of course there are more than two of us behind
    // the same NAT, but we won't worry about that cause theres no mangling in
    // that case anyways)
    return;
  }
  ...
  if ((fwType == 0) || (fwType == FirewallHelperClass::FIREWALL_TYPE_SIMPLE)) {
    ...
    DEBUG_LOG(
        ("NAT::sendMangledSourcePort - no mangling, just using the source "
         "port\n"));
    sendMangledPortNumberToTarget(sourcePort, targetSlot);
  ...
  // check to see if our NAT keeps the same source port for different
  // destinations. if this is the case, and we've already worked out what our
  // mangled port number is then we don't have to figure it out again.
  if (((fwType & FirewallHelperClass::FIREWALL_TYPE_DESTINATION_PORT_DELTA) ==
       0) &&
      ((fwType & FirewallHelperClass::FIREWALL_TYPE_SMART_MANGLING) == 0)) {
    DEBUG_LOG(
        ("NAT::sendMangledSourcePort - our firewall doesn't NAT based on "
         "destination address, checking for old connections from this "
         "address\n"));
    if (m_previousSourcePort != 0) {
      DEBUG_LOG(
          ("NAT::sendMangledSourcePort - Previous source port was %d, using "
           "that one\n",
      ...
      DEBUG_LOG(
          ("NAT::sendMangledSourcePort - Previous source port not found\n"));
    }
  }

  // At this point we know that our NAT uses some kind of relative port mapping
  // scheme, so we need to talk to the mangler to find out where we are now so
  // we can find out where we'll be on the next port allocation.

  // get the address of the mangler we need to talk to.
  Char manglerName[256];
  FirewallHelperClass::getManglerName(1, manglerName);
  DEBUG_LOG(
      ("NAT::sendMangledSourcePort - about to call gethostbyname for mangler "
       "at %s\n",
       manglerName));
  ...
  if (hostInfo == NULL) {
    DEBUG_LOG(
        ("NAT::sendMangledSourcePort - gethostbyname failed for mangler "
         "address %s\n",
         manglerName));
  ...
  DEBUG_LOG(("NAT::sendMangledSourcePort - mangler %s address is %d.%d.%d.%d\n",
             manglerName, m_manglerAddress >> 24,
             (m_manglerAddress >> 16) & 0xff, (m_manglerAddress >> 8) & 0xff,
             m_manglerAddress & 0xff));

  DEBUG_LOG(("NAT::sendMangledSourcePort - NAT behavior = 0x%08x\n", fwType));

  //	m_manglerRetryTime = TheGameSpyConfig->getRetryInterval() +
  //timeGetTime();
  m_manglerRetryTime = m_manglerRetryTimeInterval + timeGetTime();
  m_manglerRetries = 0;
  ...
  if (TheFirewallHelper != NULL) {
    ...
    TheFirewallHelper->sendToManglerFromPort(m_manglerAddress,
                                             m_spareSocketPort, m_packetID);
    //		m_manglerRetryTime = TheGameSpyConfig->getRetryInterval() +
    //timeGetTime();
    m_manglerRetryTime = m_manglerRetryTimeInterval + timeGetTime();
...
void NAT::processManglerResponse(UnsignedShort mangledPort) {
  DEBUG_LOG(
      ("NAT::processManglerResponse - Work out what my NAT'd port will be\n"));

  GameSlot *targetSlot =
      m_slotList[m_connectionNodes[m_targetNodeNumber].m_slotIndex];
  DEBUG_ASSERTCRASH(targetSlot != NULL,
                    ("NAT::processManglerResponse - targetSlot is NULL"));
  if (targetSlot == NULL) {
    DEBUG_LOG(
        ("NAT::processManglerResponse - targetSlot is NULL, failed this "
         "connection\n"));
    setConnectionState(m_localNodeNumber, NATCONNECTIONSTATE_FAILED);
    return;
  }
  ...
  if ((fwType & FirewallHelperClass::FIREWALL_TYPE_SIMPLE_PORT_ALLOCATION) !=
      0) {
  ...
  if (targetSlot->getPort() == 0) {
    ...
    // in this case we should have already sent a PROBE, so we'll just change
    // the state and leave it at that.
    setConnectionState(m_localNodeNumber,
                       NATCONNECTIONSTATE_WAITINGFORRESPONSE);
  }
}
...
Bool NAT::allConnectionsDoneThisRound() {
  ...
  for (Int i = 0; (i < m_numNodes) && (retval == TRUE); ++i) {
    if ((m_connectionStates[i] != NATCONNECTIONSTATE_DONE) &&
        (m_connectionStates[i] != NATCONNECTIONSTATE_FAILED)) {
      retval = FALSE;
    }
  }
  return retval;
...
void NAT::probed(Int nodeNumber) {
  GameSlot *localSlot =
      m_slotList[m_connectionNodes[m_localNodeNumber].m_slotIndex];
  DEBUG_ASSERTCRASH(localSlot != NULL,
                    ("NAT::probed - localSlot is NULL, WTF?"));
  if (localSlot == NULL) {
  ...
  if (m_beenProbed == FALSE) {
    ...
    if ((localSlot->getNATBehavior() &
         FirewallHelperClass::FIREWALL_TYPE_NETGEAR_BUG) != 0) {
      DEBUG_LOG(
          ("NAT::probed - we have a NETGEAR and we were just probed for the "
           "first time\n"));
      GameSlot *targetSlot =
          m_slotList[m_connectionNodes[m_targetNodeNumber].m_slotIndex];
      DEBUG_ASSERTCRASH(targetSlot != NULL,
                        ("NAT::probed - targetSlot is NULL"));
      if (targetSlot == NULL) {
        DEBUG_LOG(
            ("NAT::probed - targetSlot is NULL, failed this connection\n"));
        setConnectionState(m_localNodeNumber, NATCONNECTIONSTATE_FAILED);
        return;
      }
...
void NAT::gotMangledPort(Int nodeNumber, UnsignedShort mangledPort) {
  // if we've already finished the connection, then we don't need to process
  // this.
  if (m_connectionStates[m_localNodeNumber] == NATCONNECTIONSTATE_DONE) {
    DEBUG_LOG(
        ("NAT::gotMangledPort - got a mangled port, but we've already finished "
         "this connection, ignoring.\n"));
    return;
  }
  ...
  GameSlot *targetSlot =
      m_slotList[m_connectionNodes[m_targetNodeNumber].m_slotIndex];
  DEBUG_ASSERTCRASH(targetSlot != NULL,
                    ("NAT::gotMangledPort - targetSlot is NULL"));
  if (targetSlot == NULL) {
    DEBUG_LOG(
        ("NAT::gotMangledPort - targetSlot is NULL, failed this connection\n"));
    setConnectionState(m_localNodeNumber, NATCONNECTIONSTATE_FAILED);
    return;
  }
  ...
  GameSlot *localSlot =
      m_slotList[m_connectionNodes[m_localNodeNumber].m_slotIndex];
  DEBUG_ASSERTCRASH(localSlot != NULL,
                    ("NAT::gotMangledPort - localSlot is NULL, WTF?"));
  if (localSlot == NULL) {
    DEBUG_LOG(
        ("NAT::gotMangledPort - localSlot is NULL, failed this connection\n"));
    setConnectionState(m_localNodeNumber, NATCONNECTIONSTATE_FAILED);
    return;
  }
  ...
  if (nodeNumber != m_targetNodeNumber) {
    DEBUG_LOG(
        ("NAT::gotMangledPort - got a mangled port number for someone that "
         "isn't my target. node = %d, target node = %d\n",
  ...
  DEBUG_LOG(
      ("NAT::gotMangledPort - got mangled port number %d from our target node "
       "(%ls)\n",
  ...
  if (((localSlot->getNATBehavior() &
      ...
      (((localSlot->getNATBehavior() &
         FirewallHelperClass::FIREWALL_TYPE_NETGEAR_BUG) != 0) &&
       ((targetSlot->getNATBehavior() &
         FirewallHelperClass::FIREWALL_TYPE_NETGEAR_BUG) != 0))) {
#ifdef DEBUG_LOGGING
    UnsignedInt ip = targetSlot->getIP();
#endif
    DEBUG_LOG(
        ("NAT::gotMangledPort - don't have a netgear or we have already been "
         "probed, or both my target and I have a netgear, send a PROBE. "
         "Sending to %d.%d.%d.%d:%d\n",
         ip >> 24, (ip >> 16) & 0xff, (ip >> 8) & 0xff, ip & 0xff,
         targetSlot->getPort()));
    ...
    DEBUG_LOG(
        ("NAT::gotMangledPort - we are a netgear, not sending a PROBE yet.\n"));
  }
}
...
void NAT::gotInternalAddress(Int nodeNumber, UnsignedInt address) {
  ...
  DEBUG_ASSERTCRASH(targetSlot != NULL,
                    ("NAT::gotInternalAddress - targetSlot is NULL"));
  if (targetSlot == NULL) {
    return;
  }
  ...
  GameSlot *localSlot =
      m_slotList[m_connectionNodes[m_localNodeNumber].m_slotIndex];
  DEBUG_ASSERTCRASH(localSlot != NULL,
                    ("NAT::gotInternalAddress - localSlot is NULL, WTF?"));
  if (localSlot == NULL) {
    return;
  }
  ...
  if (nodeNumber != m_targetNodeNumber) {
    DEBUG_LOG(
        ("NAT::gotInternalAddress - got a internal address for someone that "
         "isn't my target. node = %d, target node = %d\n",
         nodeNumber, m_targetNodeNumber));
    return;
  }
  ...
  if (localSlot->getIP() == targetSlot->getIP()) {
    ...
    DEBUG_LOG(
        ("NAT::gotInternalAddress - target and local players have same "
         "external address, using internal address.\n"));
...
void NAT::notifyTargetOfProbe(GameSlot *targetSlot) {
  ...
  DEBUG_LOG(
      ("NAT::notifyTargetOfProbe - notifying %ls that we have probed them.\n",
       targetSlot->getName().str()));
}
...
void NAT::notifyUsersOfConnectionDone(Int nodeIndex) {
  ...
  DEBUG_ASSERTCRASH(
      localSlot != NULL,
      ("NAT::notifyUsersOfConnectionDone - localSlot is NULL, WTF?"));
  if (localSlot == NULL) {
    DEBUG_LOG(
        ("NAT::notifyUsersOfConnectionDone - localSlot is NULL, failed this "
         "connection\n"));
    setConnectionState(m_localNodeNumber, NATCONNECTIONSTATE_FAILED);
    return;
  }
...
void NAT::notifyUsersOfConnectionFailed(Int nodeIndex) {
  ...
  DEBUG_ASSERTCRASH(
      localSlot != NULL,
      ("NAT::notifyUsersOfConnectionFailed - localSlot is NULL, WTF?"));
  if (localSlot == NULL) {
    DEBUG_LOG(
        ("NAT::notifyUsersOfConnectionFailed - localSlot is NULL, failed this "
         "connection\n"));
    setConnectionState(m_localNodeNumber, NATCONNECTIONSTATE_FAILED);
    return;
  }
  ...
  /*
          ...
          DEBUG_LOG(("NAT::notifyUsersOfConnectionFailed - sending %s to
     room\n", options.str()));
  */
...
void NAT::processGlobalMessage(Int slotNum, const char *options) {
  ...
  DEBUG_LOG(
      ("NAT::processGlobalMessage - got message from slot %d, message is "
       "\"%s\"\n",
       slotNum, ptr));
  if (!strncmp(ptr, "PROBED", strlen("PROBED"))) {
    // format: PROBED<node number>
    // a probe has been sent at us, if we are waiting because of a netgear or
    // something, we should start sending our own probes.
    ...
    if (node == m_targetNodeNumber) {
      ...
      DEBUG_LOG(
          ("NAT::processGlobalMessage - probed by node %d, not our target\n",
           node));
    }
  } else if (!strncmp(ptr, "CONNDONE", strlen("CONNDONE"))) {
    // format: CONNDONE<node number>
    // we should get the node number of the player who's connection is done from
    // the options and mark that down as part of the connectionStates.
    ...
    */
    ...
    if (m_connectionPairs[m_connectionPairIndex][m_connectionRound][node] ==
        sendingNode) {
      //			Int node = atoi(ptr + strlen("CONNDONE"));
      DEBUG_LOG(
          ("NAT::processGlobalMessage - got a CONNDONE message for node %d\n",
           node));
      if ((node >= 0) && (node <= m_numNodes)) {
        DEBUG_LOG(
            ("NAT::processGlobalMessage - node %d's connection is complete, "
             "setting connection state to done\n",
             node));
        setConnectionState(node, NATCONNECTIONSTATE_DONE);
      }
    } else {
      DEBUG_LOG(
          ("NAT::processGlobalMessage - got a connection done message that "
           "isn't from this round. node: %d sending node: %d\n",
           node, sendingNode));
    }
  } else if (!strncmp(ptr, "CONNFAILED", strlen("CONNFAILED"))) {
    // format: CONNFAILED<node number>
    // we should get the node number of the player who's connection failed from
    // the options and mark that down as part of the connectionStates.
    Int node = atoi(ptr + strlen("CONNFAILED"));
    if ((node >= 0) && (node < m_numNodes)) {
      DEBUG_LOG(
          ("NAT::processGlobalMessage - node %d's connection failed, setting "
    ...
    // we should get the node number and the mangled port number of the client
    // we are supposed to be communicating with and start probing them. No, that
    // was not meant to be a phallic reference, you sicko.
    ...
    DEBUG_LOG(
        ("NAT::processGlobalMessage - got port message from node %d, port: %d, "
         "internal address: %d.%d.%d.%d\n",
         node, port, addr >> 24, (addr >> 16) & 0xff, (addr >> 8) & 0xff,
         addr & 0xff));
...
void NAT::setConnectionState(Int nodeNumber, NATConnectionState state) {
  ...
  if (i == MAX_SLOTS) {
    DEBUG_ASSERTCRASH(i < MAX_SLOTS,
                      ("Didn't find the node number in the slot list"));
    return;
  }
