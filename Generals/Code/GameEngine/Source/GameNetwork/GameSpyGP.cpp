...
void GPRecvBuddyMessageCallback(GPConnection *pconnection,
                                GPRecvBuddyMessageArg *arg, void *param) {
  DEBUG_LOG(("GPRecvBuddyMessageCallback: message from %d is %s\n",
             arg->profile, arg->message));

  // gpGetInfo(pconn, arg->profile, GP_DONT_CHECK_CACHE, GP_BLOCKING,
  // (GPCallback)Whois, NULL); printf("MESSAGE (%d): %s: %s\n", msgCount,whois,
  // arg->message);
}

static void buddyTryReconnect(void) { TheGameSpyChat->reconnectProfile(); }
...
#undef ERRORCODE
  ...
  if (arg->fatal) {
    ...
    // if we're still connected to the chat server, tell the user.  He can
    // always hit the buddy button to try reconnecting.  Oh yes, also hide the
    // buddy popup.
    GameSpyCloseOverlay(GSOVERLAY_BUDDY);
    if (TheGameSpyChat->isConnected()) {
      GSMessageBoxYesNo(TheGameText->fetch("GUI:GPErrorTitle"),
                        TheGameText->fetch("GUI:GPDisconnected"),
                        buddyTryReconnect, NULL);
    }
  } else {
    DEBUG_LOG(("-----\n"));
    DEBUG_LOG(("GP ERROR\n"));
...
void GPRecvBuddyStatusCallback(GPConnection *connection,
...
void GPRecvBuddyRequestCallback(GPConnection *connection,
                                GPRecvBuddyRequestArg *arg, void *param) {
  DEBUG_LOG(
      ("GPRecvBuddyRequestCallback: %d wants to be our buddy because '%s'\n",
