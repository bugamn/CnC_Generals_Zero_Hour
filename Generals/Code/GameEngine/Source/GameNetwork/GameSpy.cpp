...
AsciiString GameSpyThreadClass::getNextShellScreen(void) {
  ...
  if (m.Failed()) return AsciiString::TheEmptyString;
...
void createRoomCallback(PEER peer, PEERBool success, PEERJoinResult result,
...
class GameSpyChat : public GameSpyChatInterface {
 public:
  ...
  virtual void login(AsciiString loginName,
                     AsciiString password = AsciiString::TheEmptyString,
                     AsciiString email = AsciiString::TheEmptyString);
  virtual void reconnectProfile(void);
  virtual void disconnectFromChat(void);

  virtual void UTMRoom(RoomType roomType, const char *key, const char *val,
                       Bool authenticate = FALSE);
  virtual void UTMPlayer(const char *name, const char *key, const char *val,
                         Bool authenticate = FALSE);
  virtual void startGame(void);
  virtual void leaveRoom(RoomType roomType);
  virtual void setReady(Bool ready);
  virtual void enumPlayers(RoomType roomType, peerEnumPlayersCallback callback,
                           void *userData);
  ...
  virtual void createStagingRoom(AsciiString gameName, AsciiString password,
                                 Int maxPlayers);
  virtual void joinBestGroupRoom(void);

  void loginQuick(AsciiString loginName);
  void loginProfile(AsciiString loginName, AsciiString password,
                    AsciiString email);
  ...
  void _GPConnectCallback(GPConnection *pconnection, GPConnectResponseArg *arg,
                          void *param);
  void _GPReconnectCallback(GPConnection *pconnection,
                            GPConnectResponseArg *arg, void *param);
...
void GameSpyChatInterface::clearGroupRoomList(void) { m_groupRooms.clear(); }

GameSpyChat::GameSpyChat() { m_peer = NULL; }

GameSpyChat::~GameSpyChat() {
  reset();
  if (thread.Is_Running()) thread.Stop();
  ...
  if (!m.Failed() && m_peer) {
    ...
    if (TheGameSpyThread->showLocaleSelect()) {
      ...
      layout = TheWindowManager->winCreateLayout(
          AsciiString("Menus/PopupLocaleSelect.wnd"));
      layout->runInit();
      layout->hide(FALSE);
      layout->bringForward();
    ...
    if (TheFirewallHelper != NULL) {
      if (TheFirewallHelper->behaviorDetectionUpdate()) {
        TheGlobalData->m_firewallBehavior =
            TheFirewallHelper->getFirewallBehavior();
    ...
    if (m_loginTimeout && now > m_loginTimeout) {
      // login timed out
      m_loginTimeout = 0;
      GSMessageBoxOk(UnicodeString(L"Error connecting"),
                     UnicodeString(L"Timed out connecting"), NULL);

      // Enable controls again
      // EnableLoginControls(TRUE);

      if (TheGameSpyGame) delete TheGameSpyGame;
      TheGameSpyGame = NULL;
...
void GameSpyChat::UTMRoom(RoomType roomType, const char *key, const char *val,
                          Bool authenticate) {
  peerUTMRoom(m_peer, roomType, key, val,
              (authenticate) ? PEERTrue : PEERFalse);
}

void GameSpyChat::UTMPlayer(const char *name, const char *key, const char *val,
                            Bool authenticate) {
...
void GameSpyChat::enumPlayers(RoomType roomType,
...
void GameSpyChat::stopListingGames(void) { peerStopListingGames(m_peer); }

void GameSpyChat::joinGroupRoom(Int ID) {
  if (m_joiningGroupRoom || m_joiningStagingRoom) return;
...
void GameSpyChat::joinStagingRoom(GServer server, AsciiString password) {
  ...
  peerJoinStagingRoom(m_peer, server, password.str(), JoinRoomCallback, server,
                      PEERFalse);
}

void GameSpyChat::createStagingRoom(AsciiString gameName, AsciiString password,
                                    Int maxPlayers) {
  if (m_joiningGroupRoom || m_joiningStagingRoom) return;
  ...
  DEBUG_LOG(
      ("GameSpyChat::createStagingRoom - creating staging room, name is %s\n",
       m_loginName.str()));

  peerCreateStagingRoom(m_peer, m_loginName.str(), maxPlayers, password.str(),
                        createRoomCallback, (void *)oldGroupID, PEERFalse);
}
  ...
  if (m_groupRooms.size()) {
    ...
    while (iter != m_groupRooms.end()) {
      ...
      DEBUG_LOG(("Group room %d: %s (%d, %d, %d, %d)\n", room.m_groupID,
                 room.m_name.str(), room.m_numWaiting, room.m_maxWaiting,
    ...
    if (minID > 0) {
      ...
      GSMessageBoxOk(UnicodeString(L"Oops"),
                     UnicodeString(L"No empty group rooms"), NULL);
    }
  } else {
    GSMessageBoxOk(UnicodeString(L"Oops"), UnicodeString(L"No group rooms"),
                   NULL);
  }
}
...
void DisconnectedCallback(PEER peer, const char *reason, void *param) {
  ...
  GSMessageBoxOk(TheGameText->fetch("GUI:GSErrorTitle"),
                 TheGameText->fetch("GUI:GSDisconnected"), NULL);
...
void ReadyChangedCallback(PEER peer, const char *nick, PEERBool ready,
                          void *param) {
  if (TheGameSpyGame) {
    ...
    if (slotNum >= 0) {
      ...
      if (slot) {
        ...
        if (TheGameSpyGame->amIHost()) {
          peerUTMRoom(TheGameSpyChat->getPeer(), StagingRoom, "SL/",
                      GameInfoToAsciiString(TheGameSpyGame).str(), PEERFalse);
        }
...
void GameStartedCallback(PEER peer, unsigned int IP, const char *message,
                         void *param) {
  GameSpyStartGame();
}
  ...
  if (roomType == StagingRoom && TheGameSpyGame) {
    ...
    for (Int i = 1; i < MAX_SLOTS; ++i) {
      ...
      if (slot && slot->getState() == SLOT_OPEN) {
        ...
        UnsignedInt ip;
        PEERBool success =
            peerGetPlayerInfoNoWait(TheGameSpyChat->getPeer(), nick, &ip, &id);
        DEBUG_LOG(
            ("PlayerJoinedCallback - result from peerGetPlayerInfoNoWait, %d: "
             "ip=%d.%d.%d.%d, id=%d\n",
             success, (ip >> 24) & 0xff, (ip >> 16) & 0xff, (ip >> 8) & 0xff,
             ip & 0xff, id));
...
void PingCallback(PEER peer, const char *nick, int ping, void *param) {
  DEBUG_LOG(("PingCallback\n"));
}

void CrossPingCallback(PEER peer, const char *nick1, const char *nick2,
                       int crossPing, void *param) {
    ...
    if (slotNum == 0 && !TheGameSpyGame->amIHost()) {
      ...
      } else if (!strcmp(command, "HWS"))  // HostWantsStart
      {
        ...
        if (slot->isAccepted() == false) {
          GameSpyAddText(TheGameText->fetch("GUI:HostWantsToStart"),
                         GSCOLOR_DEFAULT);
        }
      }
    }
      ...
      if (!strcmp(command, "REQ")) {
        ...
        if (key == "Color") {
          ...
          if ((val >= FirewallHelperClass::FIREWALL_TYPE_SIMPLE) &&
            ...
            DEBUG_LOG(
                ("Setting NAT behavior to %d for player %d\n", val, slotNum));
            change = true;
          } else {
        ...
        if (change) {
          if (shouldUnaccept) TheGameSpyGame->resetAccepted();
          peerUTMRoom(TheGameSpyChat->getPeer(), StagingRoom, "SL/",
                      GameInfoToAsciiString(TheGameSpyGame).str(), PEERFalse);
          WOLDisplaySlotList();
          DEBUG_LOG(
              ("Slot value is color=%d, PlayerTemplate=%d, startPos=%d, "
               "team=%d, IP=0x%8.8X\n",
               slot->getColor(), slot->getPlayerTemplate(), slot->getStartPos(),
               slot->getTeamNumber(), slot->getIP()));
...
void GOABasicCallback(PEER peer, PEERBool playing, char *outbuf, int maxlen,
                      void *param) {
  _snprintf(outbuf, maxlen,
            "\\gamename\\c&cgenerals\\gamever\\%s\\location\\%d",
...
void GOAInfoCallback(PEER peer, PEERBool playing, char *outbuf, int maxlen,
                     void *param) {
  _snprintf(outbuf, maxlen,
            "\\hostname\\%s\\hostport\\%d\\mapname\\%s\\gametype\\%"
            "s\\numplayers\\%d\\maxplayers\\%d\\gamemode\\%s",
            TheGameSpyChat->getLoginName().str(), 0,
            TheGameSpyGame->getMap().str(), "battle",
            TheGameSpyGame->getNumPlayers(), TheGameSpyGame->getMaxPlayers(),
            "wait");
  outbuf[maxlen - 1] = 0;
...
void GOARulesCallback(PEER peer, PEERBool playing, char *outbuf, int maxlen,
                      void *param) {
...
void GOAPlayersCallback(PEER peer, PEERBool playing, char *outbuf, int maxlen,
                        void *param) {
  AsciiString buf, tmp;
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    DEBUG_LOG(
        ("GOAPlayersCallback - about to call getSlot for player %d\n", i));
    GameSlot *slot = TheGameSpyGame->getSlot(i);
    AsciiString name;
    ...
    tmp.format(
        "\\player_%d\\%s\\color_%d\\%d\\faction_%d\\%d\\team_%d\\%d\\pos_%d\\%"
        "d",
        i, name.str(), i, slot->getColor(), i, slot->getPlayerTemplate(), i,
        slot->getTeamNumber(), i, slot->getStartPos());
    buf.concat(tmp);
  }
  ...
  switch (roomType) {
    case GroupRoom: {
      ...
      if (success) {
        ...
        DEBUG_LOG(
            ("Joined group room, active screen was %s\n", windowFile.str()));
        ...
        GSMessageBoxOk(UnicodeString(L"Oops"),
                       UnicodeString(L"Unable to join group room"), NULL);
      }
      ...
      if (TheGameSpyChat->getUsingProfile()) {
        if (TheGameSpyChat->getCurrentGroupRoomID()) {
          AsciiString s;
          s.format("c&cgenerals://0.0.0.0:0/%d",
                   TheGameSpyChat->getCurrentGroupRoomID());
    ...
    case StagingRoom: {
      ...
      if (success) {
        ...
        localIP = ntohl(
            localIP);  // The IP returned from GetLocalChatConnectionAddress is
                       // in network byte order.
        options.format("IP=%d", localIP);
        peerUTMPlayer(TheGameSpyChat->getPeer(), hostName.str(), "REQ/",
                      options.str(), PEERFalse);
        ...
        // let the user know
        GSMessageBoxOk(UnicodeString(L"Oops"),
                       UnicodeString(L"Unable to join game"), NULL);
...
void createRoomCallback(PEER peer, PEERBool success, PEERJoinResult result,
  ...
  if (success) {
    ...
    UnsignedInt localIP = peerGetLocalIP(TheGameSpyChat->getPeer());
    DEBUG_LOG(
        ("createRoomCallback - peerGetLocalIP returned %d.%d.%d.%d as the "
         "local IP\n",
         localIP >> 24, (localIP >> 16) & 0xff, (localIP >> 8) & 0xff,
         localIP & 0xff));
    //		GetLocalChatConnectionAddress("peerchat.gamespy.com", 6667,
    //localIP); 		DEBUG_LOG(("createRoomCallback - GetLocalChatConnectionAddress
    //returned %d.%d.%d.%d as the local IP\n", 								localIP >> 24, (localIP >> 16) &
    //0xff, (localIP >> 8) & 0xff, localIP & 0xff));
    localIP =
        ntohl(localIP);  // The IP returned from GetLocalChatConnectionAddress
    ...
    if (it != TheMapCache->end()) {
      ...
      TheGameSpyGame
          ->adjustSlotsForMap();  // BGC- adjust the slots for the new map.
    }
    ...
    GSMessageBoxOk(UnicodeString(L"Oops"),
                   UnicodeString(L"Unable to create game"), NULL);
  }
...
/////////////////////////////////////////////////////////////////
void ListGroupRoomsCallback(PEER peer, PEERBool success, int groupID,
                            GServer server, const char *name, int numWaiting,
                            int maxWaiting, int numGames, int numPlaying,
                            void *param) {
...
static void nickErrorCallback(PEER peer, int type, const char *nick,
                              void *param) {
  ((GameSpyChat *)TheGameSpyChat)->_nickErrorCallback(peer, type, nick, param);
}

static void GPConnectCallback(GPConnection *pconnection,
                              GPConnectResponseArg *arg, void *param) {
  ((GameSpyChat *)TheGameSpyChat)->_GPConnectCallback(pconnection, arg, param);
}

static void GPReconnectCallback(GPConnection *pconnection,
                                GPConnectResponseArg *arg, void *param) {
  ((GameSpyChat *)TheGameSpyChat)
      ->_GPReconnectCallback(pconnection, arg, param);
}
...
void GameSpyChat::_connectCallback(PEER peer, PEERBool success, void *param) {
  ...
  if (!success) {
    GSMessageBoxOk(UnicodeString(L"Error connecting"),
                   UnicodeString(L"Failed to connect"), NULL);
...
void GameSpyChat::_nickErrorCallback(PEER peer, int type, const char *nick,
                                     void *param) {
  // Let the user know.
  /////////////////////
  if (type == PEER_IN_USE) {
    ...
    DEBUG_LOG(
        ("Nickname taken: origName=%s, tries=%d\n", origName.str(), intVal));

    if (intVal < 10) {
      nickStr.format("%s{%d}", origName.str(), intVal);
      // Retry the connect with a similar nick.
      DEBUG_LOG(
          ("GameSpyChat::_nickErrorCallback - nick was taken, setting to %s\n",
           nickStr.str()));
      m_loginName = nickStr;
      peerRetryWithNick(peer, nickStr.str());
    } else {
      GSMessageBoxOk(
          UnicodeString(L"Error connecting"),
          UnicodeString(
              L"That nickname is already taken; please choose another one."),
          NULL);
      // Cancel the connect.
    ...
    GSMessageBoxOk(UnicodeString(L"Error connecting"),
                   UnicodeString(L"That nickname contains at least 1 invalid "
                                 L"character, please choose another one."),
                   NULL);
    // Cancel the connect.
    peerRetryWithNick(peer, NULL);
...
void GameSpyChat::_GPConnectCallback(GPConnection *pconnection,
                                     GPConnectResponseArg *arg, void *param) {
  ...
  *res = arg->result;
  ...
  if (*res != GP_NO_ERROR) {
    ...
    GSMessageBoxOk(UnicodeString(L"Error connecting"),
                   UnicodeString(L"Error connecting to buddy server"), NULL);
    gpDisconnect(TheGPConnection);
    gpDestroy(TheGPConnection);
    m_loginTimeout = 0;
    return;
  ...
  // Connect to chat.
  ///////////////////
  peerConnect(m_peer, m_loginName.str(), m_profileID, nickErrorCallback,
              connectCallback, NULL, PEERFalse);
}

static Bool inGPReconnect = false;
void GameSpyChat::_GPReconnectCallback(GPConnection *pconnection,
                                       GPConnectResponseArg *arg, void *param) {
  inGPReconnect = false;
  DEBUG_LOG(("GPConnectCallback:\n"));
  ...
  if (arg->result != GP_NO_ERROR) {
    ...
    // yay!  we're back in!
    GSMessageBoxOk(UnicodeString(L"Connected!"),
                   UnicodeString(L"Reonnected to buddy server"), NULL);
  }
}

void GameSpyChat::loginProfile(AsciiString loginName, AsciiString password,
                               AsciiString email) {
  // Connect to GP.
  ///////////////////
  m_profileID = 0;
  ...
  gpSetCallback(TheGPConnection, GP_RECV_BUDDY_REQUEST,
                (GPCallback)GPRecvBuddyRequestCallback, NULL);
  gpSetCallback(TheGPConnection, GP_RECV_BUDDY_MESSAGE,
                (GPCallback)GPRecvBuddyMessageCallback, NULL);
  gpSetCallback(TheGPConnection, GP_RECV_BUDDY_STATUS,
                (GPCallback)GPRecvBuddyStatusCallback, NULL);
  ...
  m_password = password;
  m_email = email;
  gpConnect(TheGPConnection, loginName.str(), email.str(), password.str(),
            GP_FIREWALL, GP_NON_BLOCKING, (GPCallback)GPConnectCallback, &res);
  /*
  if (res != GP_NO_ERROR)
  {
          // couldn't connect.  bummer.
  ...
  peerConnect(TheGameSpyChat->getPeer(), loginName.str(), GameSpyLocalProfile,
  nickErrorCallback, connectCallback, NULL, PEERFalse);
  */
}

void GameSpyChat::reconnectProfile(void) {
  if (inGPReconnect) return;
  ...
  gpConnect(TheGPConnection, m_loginName.str(), m_email.str(), m_password.str(),
            GP_FIREWALL, GP_NON_BLOCKING, (GPCallback)GPReconnectCallback,
            NULL);
}
...
void GameSpyChat::loginQuick(AsciiString login) {
  ...
  peerConnect(m_peer, login.str(), 0, nickErrorCallback, connectCallback, NULL,
              PEERFalse);
}

void GameSpyChat::login(AsciiString loginName, AsciiString password,
                        AsciiString email) {
  MutexClass::LockClass m(TheGameSpyMutex);
  if (ISMAINTHREAD) {
  ...
  // Set the title.
  /////////////////
  if (!peerSetTitle(m_peer, "gmtest", "HA6zkS", "gmtest", "HA6zkS", 15,
                    pingRooms, crossPingRooms)) {
    GSMessageBoxOk(UnicodeString(L"Error setting title"),
                   UnicodeString(L"Error setting title"), NULL);
    peerShutdown(m_peer);
    m_peer = NULL;
    return;
  }
