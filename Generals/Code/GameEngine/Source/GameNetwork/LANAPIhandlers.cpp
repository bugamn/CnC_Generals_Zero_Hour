...
void LANAPI::handleGameAnnounce(LANMessage *msg, UnsignedInt senderIP) {
  if (senderIP == m_localIP) {
    ...
    if (m_currentGame == NULL) {
      ...
      Bool success =
          ParseGameOptionsString(game, AsciiString(msg->GameInfo.options));
...
void LANAPI::handleRequestJoin(LANMessage *msg, UnsignedInt senderIP) {
  UnsignedInt responseIP =
      senderIP;  // need this cause the player may or may not be
  ...
  if (!m_inLobby && m_currentGame && m_currentGame->getIP(0) == m_localIP) {
    if (m_currentGame->isGameInProgress()) {
      ...
      DEBUG_LOG(
          ("LANAPI::handleRequestJoin - join denied because game already "
           "started.\n"));
    } else {
      int player;
      Bool canJoin = true;
      ...
      if (TheGlobalData->m_netMinPlayers > 0) {
        ...
        if (msg->GameToJoin.iniCRC != TheGlobalData->m_iniCRC ||
          ...
          DEBUG_LOG(
              ("LANAPI::handleRequestJoin - join denied because of CRC "
               "mismatch. CRCs are them/us INI:%X/%X exe:%X/%X\n",
      ...
      for (player = 0; canJoin && player < MAX_SLOTS; ++player) {
        ...
        if (s.isNotEmpty()) {
          ...
          if (!strncmp(s.str(), msg->GameToJoin.serial, g_maxSerialLength)) {
            ...
            DEBUG_LOG(
                ("LANAPI::handleRequestJoin - join denied because of duplicate "
                 "serial # (%s).\n",
                 s.str()));
            break;
          }
      ...
      for (player = 0; player < MAX_SLOTS; ++player) {
        if (m_currentGame->getLANSlot(player)->isOccupied() &&
            !(m_currentGame->getLANSlot(player)->getPlayerTemplate() ==
      ...
      if (numPlayers < numStartingSpots) {
        for (player = 0; canJoin && player < MAX_SLOTS; ++player) {
          if (m_currentGame->getLANSlot(player)->isOpen()) {
            ...
            wcsncpy(reply.GameJoined.gameName, m_currentGame->getName().str(),
                    g_lanGameNameLength);
            ...
            DEBUG_LOG(
                ("LANAPI::handleRequestJoin - added player %ls at ip 0x%08x to "
                 "the game\n",
                 msg->name, senderIP));
      ...
      if (canJoin && player == MAX_SLOTS) {
        reply.LANMessageType = LANMessage::MSG_JOIN_DENY;
        wcsncpy(reply.GameNotJoined.gameName, m_currentGame->getName().str(),
                g_lanGameNameLength);
        ...
        reply.GameNotJoined.playerIP = senderIP;
        DEBUG_LOG((
            "LANAPI::handleRequestJoin - join denied because game is full.\n"));
      }
    }
  } else {
    reply.LANMessageType = LANMessage::MSG_JOIN_DENY;
  ...
  if (msg->GameJoined.playerIP == m_localIP)  // Is it for us?
  {
    if (m_pendingAction == ACT_JOIN)  // Are we trying to join?
    {
      ...
      if (!m_currentGame) {
        ...
        AsciiString entry;
        entry.format(
            "%d.%d.%d.%d:%s", senderIP >> 24, (senderIP & 0xff0000) >> 16,
            (senderIP & 0xff00) >> 8, senderIP & 0xff,
            UnicodeStringToQuotedPrintable(m_currentGame->getSlot(0)->getName())
                .str());
...
void LANAPI::handleJoinDeny(LANMessage *msg, UnsignedInt senderIP) {
  if (msg->GameJoined.playerIP == m_localIP)  // Is it for us?
  {
    if (m_pendingAction == ACT_JOIN)  // Are we trying to join?
    {
      OnGameJoin(msg->GameNotJoined.reason,
                 LookupGame(UnicodeString(msg->GameNotJoined.gameName)));
      m_pendingAction = ACT_NONE;
      m_expiration = 0;
    }
  }
    ...
    for (player = 0; player < MAX_SLOTS; ++player) {
      if (m_currentGame->getIP(player) == senderIP) {
        if (player == 0) {
          ...
          /// @todo re-add myself to lobby?  Or just keep me there all the time?
          /// If we send a LOBBY_ANNOUNCE things'll work out...
  ...
  if (m_inLobby) {
    ...
    for (player = 0; player < MAX_SLOTS; ++player) {
      if (m_currentGame && m_currentGame->getIP(player) == senderIP) {
        OnChat(UnicodeString(msg->name), m_currentGame->getIP(player),
               UnicodeString(msg->Chat.message), msg->Chat.chatType);
        break;
      }
    }
...
void LANAPI::handleGameStart(LANMessage *msg, UnsignedInt senderIP) {
  if (!m_inLobby && m_currentGame && m_currentGame->getIP(0) == senderIP &&
...
void LANAPI::handleInActive(LANMessage *msg, UnsignedInt senderIP) {
  if (m_inLobby || (m_currentGame == NULL) ||
      (m_currentGame->isGameInProgress())) {
    return;
  }
  ...
  // don't want to unaccept the host, that's silly.  They can't hit start
  // alt-tabbed anyways.
