...
static void messageBoxOK(void) {
  DEBUG_ASSERTCRASH(
      messageBoxWindow,
      ("Message box window went away without being there in the first place!"));
 ...
 * pointers to it and its callbacks.
 */
void GSMessageBoxOkCancel(UnicodeString title, UnicodeString message,
                          GameWinMsgBoxFunc newOkFunc,
 ...
 * pointers to it and its callbacks.
 */
void GSMessageBoxYesNo(UnicodeString title, UnicodeString message,
                       GameWinMsgBoxFunc newYesFunc,
                       GameWinMsgBoxFunc newNoFunc) {
  ClearGSMessageBoxes();
  messageBoxWindow =
...
void GameSpyOpenOverlay(GSOverlayType overlay) {
  if (overlay == GSOVERLAY_BUDDY) {
    if (!TheGameSpyBuddyMessageQueue->isConnected()) {
      ...
      if (TheGameSpyBuddyMessageQueue->getLocalProfileID()) {
        ...
        // no profile
        GSMessageBoxOk(TheGameText->fetch("GUI:GPErrorTitle"),
                       TheGameText->fetch("GUI:GPNoProfile"), NULL);
      }
      return;
    }
  ...
  if (overlayLayouts[overlay]) {
    ...
    overlayLayouts[overlay] =
        TheWindowManager->winCreateLayout(AsciiString(gsOverlays[overlay]));
...
void ReOpenPlayerInfo(void) { reOpenPlayerInfoFlag = TRUE; }
void CheckReOpenPlayerInfo(void) {
  if (!reOpenPlayerInfoFlag) return;
