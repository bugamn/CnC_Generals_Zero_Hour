...
FrameMetrics::FrameMetrics() {
  ...
  for (Int i = 0; i < MAX_FRAMES_AHEAD; i++) m_pendingLatencies[i] = 0;
  //
...
void FrameMetrics::doPerFrameMetrics(UnsignedInt frame) {
  ...
  if ((curTime - m_lastFpsTimeThing) >= 1000) {
    ...
    //			DEBUG_LOG(("FrameMetrics::doPerFrameMetrics - adding %f
    //to fps history. average before: %f ", m_fpsList[m_fpsListIndex],
    //m_averageFps));
    //		}
    m_averageFps -=
        ((m_fpsList[m_fpsListIndex])) /
        TheGlobalData->m_networkFPSHistoryLength;  // subtract out the old value
                                                   // from the average.
    m_fpsList[m_fpsListIndex] = TheDisplay->getAverageFPS();
    //		m_fpsList[m_fpsListIndex] = TheGameClient->getFrame() -
    //m_fpsStartingFrame;
    m_averageFps +=
        ((Real)(m_fpsList[m_fpsListIndex])) /
        TheGlobalData
            ->m_networkFPSHistoryLength;  // add the new value to the average.
...
void FrameMetrics::processLatencyResponse(UnsignedInt frame) {
  ...
  m_latencyList[latencyListIndex] =
      (Real)timeDiff / (Real)1000;  // convert to seconds from milliseconds.
  m_averageLatency += m_latencyList[latencyListIndex] /
                      TheGlobalData->m_networkLatencyHistoryLength;

  if (frame % 16 == 0) {
    //		DEBUG_LOG(("ConnectionManager::processFrameInfoAck - average
    //latency = %f\n", m_averageLatency));
  }
}
...
Int FrameMetrics::getAverageFPS() { return (Int)m_averageFps; }

Real FrameMetrics::getAverageLatency() { return m_averageLatency; }

Int FrameMetrics::getMinimumCushion() { return m_minimumCushion; }