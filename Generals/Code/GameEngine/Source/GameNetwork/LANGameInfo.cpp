...
LANGameSlot::LANGameSlot() { m_lastHeard = 0; }
...
LANGameInfo::LANGameInfo() {
  ...
  m_next = NULL;
  //
  for (Int i = 0; i < MAX_SLOTS; ++i) setSlotPointer(i, &m_LANSlot[i]);
...
void LANGameInfo::setSlot(Int slotNum, LANGameSlot slotInfo) {
  DEBUG_ASSERTCRASH(slotNum >= 0 && slotNum < MAX_SLOTS,
                    ("LANGameInfo::setSlot - Invalid slot number"));
...
LANGameSlot *LANGameInfo::getLANSlot(Int slotNum) {
  DEBUG_ASSERTCRASH(slotNum >= 0 && slotNum < MAX_SLOTS,
                    ("LANGameInfo::getLANSlot - Invalid slot number"));
...
const LANGameSlot *LANGameInfo::getConstLANSlot(Int slotNum) const {
  DEBUG_ASSERTCRASH(slotNum >= 0 && slotNum < MAX_SLOTS,
                    ("LANGameInfo::getConstLANSlot - Invalid slot number"));
  if (slotNum < 0 || slotNum >= MAX_SLOTS) return NULL;
...
Int LANGameInfo::getLocalSlotNum(void) const {
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (slot->isLocalPlayer()) return i;
  }
  return -1;
}
...
Int LANGameInfo::getSlotNum(UnicodeString userName) {
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (slot->isUser(userName)) return i;
  }
  return -1;
}
...
void LANGameInfo::setMap(AsciiString mapName) { GameInfo::setMap(mapName); }

void LANGameInfo::setSeed(Int seed) { GameInfo::setSeed(seed); }
...
void LANDisplayGameList(GameWindow *gameListbox, LANGameInfo *gameList) {
  ...
  if (gameListbox) {
    ...
    if (selectedIndex != -1) {
      selectedPtr = (LANGameInfo *)GadgetListBoxGetItemData(gameListbox,
                                                            selectedIndex, 0);
    }
    ...
    while (gameList) {
      ...
      Int addedIndex = GadgetListBoxAddEntryText(
          gameListbox, txtGName,
          (gameList->isGameInProgress()) ? gameInProgressColor : gameColor, -1,
          -1);
      GadgetListBoxSetItemData(gameListbox, (void *)gameList, addedIndex, 0);

      if (selectedPtr == gameList) indexToSelect = addedIndex;
...
Bool ParseGameOptionsString(LANGameInfo *game, AsciiString options) {
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (slot && slot->isHuman()) {
      ...
      // slot->getUser()->getLogin().str(), slot->getUser()->getHost().str(),
      // slot->getName().str()));
  ...
  if (ParseAsciiStringToGameInfo(game, options)) {
    ...
    if (!TheLAN->AmIHost() && isInGame) {
      ...
      // DEBUG_LOG(("wasInGame:%d isInGame:%d hadMap:%d hasMap:%d oldMap:%s
      // newMap:%s\n", wasInGame, isInGame, hadMap, hasMap, oldMap.str(),
      // game->getMap().str()));
      if ((oldMapCRC ^ newMapCRC) /*(hasMap ^ hadMap)*/ ||
          (!wasInGame && isInGame)) {
        // it changed.  send it
    ...
    for (i = 0; i < MAX_SLOTS; ++i) {
      ...
      if (slot->isHuman()) {
        ...
        if (mapIt != oldLogins.end()) slot->setLogin(mapIt->second);
        mapIt = oldMachines.find(slot->getName());
        if (mapIt != oldMachines.end()) slot->setHost(mapIt->second);
