...
/**
 * Constructor.  isLocal tells it whether its the frame data manager for the
 * local player or not.
 */
 ...
 */
void FrameDataManager::reset() { init(); }
...
void FrameDataManager::addNetCommandMsg(NetCommandMsg *msg) {
  ...
  DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                  ("FrameDataManager::addNetCommandMsg - about to add a "
                   "command of type %s for frame %d, frame index %d\n",
  ...
  if (m_isLocal) {
    // If this is the local connection, adjust the frame command count.
    m_frameData[frameindex].setFrameCommandCount(
        m_frameData[frameindex].getCommandCount());
  }
}
...
FrameDataReturnType FrameDataManager::allCommandsReady(UnsignedInt frame,
                                                       Bool debugSpewage) {
  UnsignedInt frameindex = frame % FRAME_DATA_LENGTH;
  // DEBUG_ASSERTCRASH(m_frameData[frameindex].getFrame() == frame || frame ==
  // 256, ("Looking at old commands!"));
...
void FrameDataManager::resetFrame(UnsignedInt frame, Bool isAdvancing) {
  ...
  DEBUG_ASSERTCRASH(
      m_frameData[frameindex].getCommandCount() == 0,
      ("we just reset the frame data and the command count is not zero, huh?"));
}
...
/**
 * Set both the command count and the frame command count to 0 for the given
 * frames.
 */
 ...
 */
UnsignedInt FrameDataManager::getQuitFrame() { return m_quitFrame; }
