...
Bool DoesCommandRequireACommandID(NetCommandType type) {
  if ((type == NETCOMMANDTYPE_GAMECOMMAND) ||
      ...
      (type == NETCOMMANDTYPE_RUNAHEAD) || (type == NETCOMMANDTYPE_CHAT) ||
      (type == NETCOMMANDTYPE_DISCONNECTVOTE) ||
      (type == NETCOMMANDTYPE_LOADCOMPLETE) ||
      (type == NETCOMMANDTYPE_TIMEOUTSTART) ||
      (type == NETCOMMANDTYPE_WRAPPER) || (type == NETCOMMANDTYPE_FILE) ||
...
/**
 * Returns true if this type of network command requires the ack to be sent
 * directly to the player rather than going through the packet router.  This
 * should really only be used by commands used on the disconnect screen.
 */
