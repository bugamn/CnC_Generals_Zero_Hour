...
/**
 * Assign this connection a user.  This is the user to whome we send all our
 * packetized goodies.
 */
 ...
 */
User *Connection::getUser() { return m_user; }

/**
 * Add this network command to the send queue for this connection.
 * The relay is the mask specifying the people the person we are sending to
 * should send to. The relay mostly has to do with the packet router.
 */
void Connection::sendNetCommandMsg(NetCommandMsg *msg, UnsignedByte relay) {
  static NetPacket *packet = NULL;

  // this is done so we don't have to allocate and delete a packet every time we
  // send a message.
  if (packet == NULL) {
  ...
  if (m_netCommandList != NULL) {
    // check to see if this command will fit in a packet.  If not, we need to
    // split it up. we are splitting up the command here so that the retry logic
    // will not try to resend the ENTIRE command (i.e. multiple packets work of
    // data) and only do the retry one wrapper command at a time.
    packet->reset();
    ...
    if (!msgFits) {
      ...
      NetPacketList packetList =
          NetPacket::ConstructBigCommandPacketList(origref);
      ...
      while (tempPacketPtr != packetList.end()) {
        ...
        while (ref1 != NULL) {
          NetCommandRef *ref2 =
              m_netCommandList->addMessage(ref1->getCommand());
          ref2->setRelay(relay);
    ...
    if (ref != NULL) {
      /*
                              ...
                              if (msg->getNetCommandType() ==
      NETCOMMANDTYPE_GAMECOMMAND) { DEBUG_LOG(("Connection::sendNetCommandMsg -
      added game command %d to net command list for frame %d.\n", msg->getID(),
      msg->getExecutionFrame())); } else if (msg->getNetCommandType() ==
      NETCOMMANDTYPE_FRAMEINFO) { DEBUG_LOG(("Connection::sendNetCommandMsg -
      added frame info for frame %d\n", msg->getExecutionFrame()));
                              }
...
void Connection::clearCommandsExceptFrom(Int playerIndex) {
  ...
  while (tmp) {
    ...
    if (msg->getPlayerID() != playerIndex) {
      DEBUG_LOG(
          ("Connection::clearCommandsExceptFrom(%d) - clearing a command from "
           "%d for frame %d\n",
           playerIndex, tmp->getCommand()->getPlayerID(),
           tmp->getCommand()->getExecutionFrame()));
...
/**
 * This is the good part. We take all the network commands queued up for this
 * connection, packetize them and put them on the transport's send queue for
 * actual sending.
 */
UnsignedInt Connection::doSend() {
  ...
  if ((curtime - m_lastTimeSent) < m_frameGrouping) {
    //		DEBUG_LOG(("not sending packet, time = %d, m_lastFrameSent = %d,
  ...
  while ((msg != NULL) && couldQueue) {
    ...
    // add the command messages until either we run out of messages or the
    // packet is full.
    while ((msg != NULL) && notDone) {
      NetCommandRef *next =
          msg->getNext();  // Need this since msg could be deleted
    ...
    /// @todo Make the act of giving the transport object a packet to send more
    /// efficient.  Make the transport take a NetPacket object rather than the
    /// raw data, thus avoiding an extra memcpy.
    if (packet->getNumCommands() > 0) {
      // If the packet actually has any information to give, give it to the
      // transport object for transmission.
      couldQueue =
          m_transport->queueSend(packet->getAddr(), packet->getPort(),
                                 packet->getData(), packet->getLength());
      m_lastTimeSent = curtime;
    }
    if (packet != NULL) {
      packet
          ->deleteInstance();  // delete the packet now that we're done with it.
    }
  }
...
NetCommandRef *Connection::processAck(UnsignedShort commandID,
  ...
  while ((temp != NULL) &&
         ((temp->getCommand()->getID() != commandID) ||
          (temp->getCommand()->getPlayerID() != originalPlayerID))) {
...
#if defined(_DEBUG) || defined(_INTERNAL)
  Bool doDebug = FALSE;
  if (temp->getCommand()->getNetCommandType() ==
  ...
  m_averageLatency -=
      ((Real)(m_latencies[index])) / CONNECTION_LATENCY_HISTORY_LENGTH;
  ...
  if (doDebug == TRUE) {
    DEBUG_LOG(
        ("Connection::processAck - disconnect frame command %d found, removing "
         "from command list.\n",
         commandID));
  }
#endif
...
void Connection::doRetryMetrics() {
  ...
  if ((curTime - m_retryMetricsTime) > 10000) {
    ...
    ++numSeconds;
    //		DEBUG_LOG(("Retries in the last 10 seconds = %d, average latency
...
void Connection::debugPrintCommands() {
  ...
  while (ref != NULL) {
    DEBUG_LOG(
        ("Connection::debugPrintCommands - ID: %d\tType: %s\tRelay: 0x%X for "
         "frame %d\n",
