...
// This function assumes that all of the fields are either of default value or
// are present in the raw data.
NetCommandRef *NetPacket::ConstructNetCommandMsgFromRawData(
    UnsignedByte *data, UnsignedShort dataLength) {
...
NetPacketList NetPacket::ConstructBigCommandPacketList(NetCommandRef *ref) {
  ...
  if (!DoesCommandRequireACommandID(msg->getNetCommandType())) {
    DEBUG_CRASH(
        ("Trying to wrap a command that doesn't have a unique command ID"));
    return NULL;
  }

  UnsignedInt bufferSize = GetBufferSizeNeededForCommand(
      msg);  // need to implement.  I have a drinking problem.
  UnsignedByte *bigPacketData = NULL;

  NetPacketList packetList;

  // create the buffer for the huge message and fill the buffer with that
  // message.
  ...
  // get the amount of space needed for the wrapper message, not including the
  // wrapped command data.
  ...
  while (currentChunk < numChunks) {
    ...
    if (packet->addCommand(ref) == FALSE) {
      DEBUG_LOG_LEVEL(
          DEBUG_LEVEL_NET,
          ("NetPacket::BeginBigCommandPacketList - failed to add a wrapper "
...
UnsignedInt NetPacket::GetGameCommandSize(NetCommandMsg *msg) {
  ...
  while (arg != NULL) {
    msglen += 2 * sizeof(UnsignedByte);  // for the type and number of args of
...
UnsignedInt NetPacket::GetFileCommandSize(NetCommandMsg *msg) {
  ...
  UnsignedInt msglen = 0;
  msglen +=
      sizeof(UnsignedByte) + sizeof(UnsignedByte);  // 'T' and command type
  msglen += sizeof(UnsignedByte) + sizeof(UnsignedByte);   // 'P' and player ID
  msglen += sizeof(UnsignedByte) + sizeof(UnsignedShort);  // 'C' and command ID
  msglen += sizeof(UnsignedByte) + sizeof(UnsignedByte);   // 'R' and relay
  ...
  msglen += filemsg->getPortableFilename().getLength() +
            1;                    // PORTABLE filename and the terminating 0
...
void NetPacket::FillBufferWithGameCommand(UnsignedByte *buffer,
  ...
  // DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::FillBufferWithGameCommand for
  // command ID %d\n", cmdMsg->getID()));
  ...
  //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::addGameMessage -
  //added game message, frame %d, player %d, command ID %d\n", m_lastFrame,
  //m_lastPlayerID, m_lastCommandID));

  if (gmsg) gmsg->deleteInstance();
  gmsg = NULL;
}
...
void NetPacket::FillBufferWithAckCommand(UnsignedByte *buffer,
  ...
  //("NetPacket::FillBufferWithAckCommand - adding ack for command %d for player
  ...
  //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("outgoing - added ACK,
  //original player %d, command id %d\n", origPlayerID, cmdID));
}
...
void NetPacket::FillBufferWithFrameCommand(UnsignedByte *buffer,
  ...
  UnsignedShort offset = 0;
  //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::addFrameCommand -
  //adding frame command for frame %d, command count = %d, command id = %d\n",
  ...
  //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("player = %d",
  //m_lastPlayerID));
  ...
  // frameinfodebug
  //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("outgoing - added frame %d,
  //player %d, command count = %d, command id = %d\n",
  //cmdMsg->getExecutionFrame(), cmdMsg->getPlayerID(),
  //cmdMsg->getCommandCount(), cmdMsg->getID()));
}
...
void NetPacket::FillBufferWithPlayerLeaveCommand(UnsignedByte *buffer,
  ...
  UnsignedShort offset = 0;
  //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
  //("NetPacket::addPlayerLeaveCommand - adding player leave command for player
...
void NetPacket::FillBufferWithRunAheadMetricsCommand(UnsignedByte *buffer,
                                                     NetCommandRef *msg) {
  NetRunAheadMetricsCommandMsg *cmdMsg =
      (NetRunAheadMetricsCommandMsg *)(msg->getCommand());
  UnsignedShort offset = 0;
  //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
  //("NetPacket::addRunAheadMetricsCommand - adding run ahead metrics for player
  //%d, fps = %d, latency = %f\n", cmdMsg->getPlayerID(),
  //cmdMsg->getAverageFps(), cmdMsg->getAverageLatency()));
...
void NetPacket::FillBufferWithRunAheadCommand(UnsignedByte *buffer,
  ...
  UnsignedShort offset = 0;
  // DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::FillBufferWithRunAheadCommand
  ...
  //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket - added run ahead
  //command, frame %d, player id %d command id %d\n", m_lastFrame,
  //m_lastPlayerID, m_lastCommandID));
}
...
void NetPacket::FillBufferWithKeepAliveCommand(UnsignedByte *buffer,
                                               NetCommandRef *msg) {
  NetKeepAliveCommandMsg *cmdMsg =
      (NetKeepAliveCommandMsg *)(msg->getCommand());
  UnsignedShort offset = 0;
...
void NetPacket::FillBufferWithDisconnectKeepAliveCommand(UnsignedByte *buffer,
                                                         NetCommandRef *msg) {
  NetDisconnectKeepAliveCommandMsg *cmdMsg =
      (NetDisconnectKeepAliveCommandMsg *)(msg->getCommand());
  UnsignedShort offset = 0;
...
void NetPacket::FillBufferWithDisconnectPlayerCommand(UnsignedByte *buffer,
                                                      NetCommandRef *msg) {
  NetDisconnectPlayerCommandMsg *cmdMsg =
      (NetDisconnectPlayerCommandMsg *)(msg->getCommand());
  UnsignedShort offset = 0;
  //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
  //("NetPacket::addDisconnectPlayerCommand - adding run ahead command\n"));
...
void NetPacket::FillBufferWithPacketRouterQueryCommand(UnsignedByte *buffer,
                                                       NetCommandRef *msg) {
  NetPacketRouterQueryCommandMsg *cmdMsg =
      (NetPacketRouterQueryCommandMsg *)(msg->getCommand());
  UnsignedShort offset = 0;
  //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
  //("NetPacket::addPacketRouterQueryCommand - adding packet router query
  //command\n"));
...
void NetPacket::FillBufferWithPacketRouterAckCommand(UnsignedByte *buffer,
                                                     NetCommandRef *msg) {
  NetPacketRouterAckCommandMsg *cmdMsg =
      (NetPacketRouterAckCommandMsg *)(msg->getCommand());
  UnsignedShort offset = 0;
  //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
  //("NetPacket::addPacketRouterAckCommand - adding packet router query
  //command\n"));
...
void NetPacket::FillBufferWithDisconnectChatCommand(UnsignedByte *buffer,
                                                    NetCommandRef *msg) {
  NetDisconnectChatCommandMsg *cmdMsg =
      (NetDisconnectChatCommandMsg *)(msg->getCommand());
  UnsignedShort offset = 0;
  //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
  //("NetPacket::addDisconnectChatCommand - adding run ahead command\n"));
...
void NetPacket::FillBufferWithDisconnectVoteCommand(UnsignedByte *buffer,
                                                    NetCommandRef *msg) {
  NetDisconnectVoteCommandMsg *cmdMsg =
      (NetDisconnectVoteCommandMsg *)(msg->getCommand());
  UnsignedShort offset = 0;
  //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
  //("NetPacket::addDisconnectVoteCommand - adding run ahead command\n"));
...
void NetPacket::FillBufferWithFileAnnounceMessage(UnsignedByte *buffer,
                                                  NetCommandRef *msg) {
  NetFileAnnounceCommandMsg *cmdMsg =
      (NetFileAnnounceCommandMsg *)(msg->getCommand());
  UnsignedInt offset = 0;
...
void NetPacket::FillBufferWithFileProgressMessage(UnsignedByte *buffer,
                                                  NetCommandRef *msg) {
  NetFileProgressCommandMsg *cmdMsg =
      (NetFileProgressCommandMsg *)(msg->getCommand());
  UnsignedInt offset = 0;
...
void NetPacket::FillBufferWithFrameResendRequestMessage(UnsignedByte *buffer,
                                                        NetCommandRef *msg) {
  NetFrameResendRequestCommandMsg *cmdMsg =
      (NetFrameResendRequestCommandMsg *)(msg->getCommand());
  UnsignedInt offset = 0;
...
Bool NetPacket::addFrameResendRequestCommand(NetCommandRef *msg) {
  ...
  if (isRoomForFrameResendRequestMessage(msg)) {
    ...
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("NetPacket::addFrameResendRequest - added frame resend request "
         "command from player %d for frame %d, command id = %d\n",
...
Bool NetPacket::addDisconnectScreenOffCommand(NetCommandRef *msg) {
  ...
  if (isRoomForDisconnectScreenOffMessage(msg)) {
    ...
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("NetPacket::addDisconnectScreenOff - added disconnect screen off "
         "command from player %d for frame %d, command id = %d\n",
...
Bool NetPacket::addDisconnectFrameCommand(NetCommandRef *msg) {
  ...
  if (isRoomForDisconnectFrameMessage(msg)) {
    ...
    DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                    ("NetPacket::addDisconnectFrame - added disconnect frame "
                     "command from player %d for frame %d, command id = %d\n",
...
Bool NetPacket::isRoomForFileMessage(NetCommandRef *msg) {
  ...
  ++len;  // 'D'
  len += cmdMsg->getPortableFilename().getLength() +
         1;                    // PORTABLE filename + the terminating 0
...
Bool NetPacket::addFileAnnounceCommand(NetCommandRef *msg) {
  ...
  if (isRoomForFileAnnounceMessage(msg)) {
    ...
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("Adding file announce message for fileID %d, ID %d to packet\n",
         cmdMsg->getFileID(), cmdMsg->getID()));
    return TRUE;
  }
  DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                  ("No room to add file announce message to packet\n"));
  return FALSE;
}
...
Bool NetPacket::addTimeOutGameStartMessage(NetCommandRef *msg) {
  ...
  if (isRoomForLoadCompleteMessage(msg)) {
    ...
    //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("Added keep alive command to
    //packet.\n"));
...
Bool NetPacket::addDisconnectVoteCommand(NetCommandRef *msg) {
  ...
  //	DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::addDisconnectVoteCommand -
  //entering...\n"));
  ...
  if (isRoomForDisconnectVoteMessage(msg)) {
    ...
    //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
    //("NetPacket::addDisconnectVoteCommand - added disconnect vote command,
    //player id %d command id %d, voted slot %d\n", m_lastPlayerID,
    //m_lastCommandID, slot));
...
/**
 * Returns true if there is room for this player disconnect command in this
 * packet.
 */
...
Bool NetPacket::addDisconnectChatCommand(NetCommandRef *msg) {
  ...
  if (isRoomForDisconnectChatMessage(msg)) {
    ...
    memcpy(m_packet + m_packetLen, unitext.str(),
           length * sizeof(UnsignedShort));
    m_packetLen += length * sizeof(UnsignedShort);

    //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket - added disconnect
    //chat command\n"));
...
Bool NetPacket::addChatCommand(NetCommandRef *msg) {
  ...
  if (isRoomForChatMessage(msg)) {
    ...
    //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket - added chat
    //command\n"));
...
Bool NetPacket::addPacketRouterAckCommand(NetCommandRef *msg) {
  ...
  if (isRoomForPacketRouterAckMessage(msg)) {
    ...
    //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket - added packet
    //router ack command, player id %d\n", m_lastPlayerID));
...
/**
 * Returns true if there is room for this packet router ack command in this
 * packet.
 */
...
Bool NetPacket::addPacketRouterQueryCommand(NetCommandRef *msg) {
  ...
  if (isRoomForPacketRouterQueryMessage(msg)) {
    ...
    //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket - added packet
    //router query command, player id %d\n", m_lastPlayerID));
...
/**
 * Returns true if there is room for this packet router query command in this
 * packet.
 */
...
Bool NetPacket::addDisconnectPlayerCommand(NetCommandRef *msg) {
  ...
  //	DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::addDisconnectPlayerCommand
  //- entering...\n"));
  //  need type, player id, relay, command id, slot number
  if (isRoomForDisconnectPlayerMessage(msg)) {
    ...
    //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
    //("NetPacket::addDisconnectPlayerCommand - added disconnect player command,
    //player id %d command id %d, disconnecting slot %d\n", m_lastPlayerID,
    //m_lastCommandID, slot));
...
Bool NetPacket::addRunAheadCommand(NetCommandRef *msg) {
  ...
  if (isRoomForRunAheadMessage(msg)) {
    NetRunAheadCommandMsg *cmdMsg =
        (NetRunAheadCommandMsg *)(msg->getCommand());
    // DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::addRunAheadCommand - adding
    // run ahead command\n"));
...
Bool NetPacket::addDestroyPlayerCommand(NetCommandRef *msg) {
  ...
  if (isRoomForDestroyPlayerMessage(msg)) {
    ...
    // DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket - added CRC:0x%8.8X info
    // command, frame %d, player id %d command id %d\n", newCRC, m_lastFrame,
    // m_lastPlayerID, m_lastCommandID));
...
Bool NetPacket::addRunAheadMetricsCommand(NetCommandRef *msg) {
  ...
  if (isRoomForRunAheadMetricsMessage(msg)) {
    NetRunAheadMetricsCommandMsg *cmdMsg =
        (NetRunAheadMetricsCommandMsg *)(msg->getCommand());
    //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
    //("NetPacket::addRunAheadMetricsCommand - adding run ahead metrics for
    //player %d, fps = %d, latency = %f\n", cmdMsg->getPlayerID(),
    //cmdMsg->getAverageFps(), cmdMsg->getAverageLatency()));
...
Bool NetPacket::addPlayerLeaveCommand(NetCommandRef *msg) {
  ...
  if (isRoomForPlayerLeaveMessage(msg)) {
    NetPlayerLeaveCommandMsg *cmdMsg =
        (NetPlayerLeaveCommandMsg *)(msg->getCommand());
    //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
    //("NetPacket::addPlayerLeaveCommand - adding player leave command for
    //player %d\n", cmdMsg->getLeavingPlayerID()));
...
Bool NetPacket::addFrameCommand(NetCommandRef *msg) {
  ...
  if (isFrameRepeat(msg)) {
    ...
    m_lastCommand->setRelay(msg->getRelay());
    ++m_lastFrame;  // need this cause we're actually advancing to the next
                    // frame by adding this command.
    ++m_numCommands;
    // frameinfodebug
    //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("outgoing - added frame %d,
    //player %d, command count = %d, command id = %d, repeat\n", m_lastFrame,
    //m_lastPlayerID, 0, m_lastCommandID));
    return TRUE;
  }
...
Bool NetPacket::isFrameRepeat(NetCommandRef *msg) {
  ...
  if (m_lastCommand->getCommand()->getNetCommandType() !=
      NETCOMMANDTYPE_FRAMEINFO) {
    return FALSE;
  }
  NetFrameCommandMsg *framemsg = (NetFrameCommandMsg *)(msg->getCommand());
  NetFrameCommandMsg *lastmsg =
      (NetFrameCommandMsg *)(m_lastCommand->getCommand());
...
Bool NetPacket::addAckBothCommand(NetCommandRef *msg) {
  ...
  return addAckCommand(msg, ackmsg->getCommandID(),
                       ackmsg->getOriginalPlayerID());
}
...
Bool NetPacket::addAckStage1Command(NetCommandRef *msg) {
  NetAckStage1CommandMsg *ackmsg =
      (NetAckStage1CommandMsg *)(msg->getCommand());
  return addAckCommand(msg, ackmsg->getCommandID(),
                       ackmsg->getOriginalPlayerID());
}
...
Bool NetPacket::addAckStage2Command(NetCommandRef *msg) {
  NetAckStage2CommandMsg *ackmsg =
      (NetAckStage2CommandMsg *)(msg->getCommand());
  return addAckCommand(msg, ackmsg->getCommandID(),
                       ackmsg->getOriginalPlayerID());
}
 ...
 */
Bool NetPacket::addAckCommand(NetCommandRef *msg, UnsignedShort commandID,
  ...
  if (isRoomForAckMessage(msg)) {
    NetCommandMsg *cmdMsg = msg->getCommand();
    //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::addAckCommand -
...
Bool NetPacket::isAckRepeat(NetCommandRef *msg) {
  ...
  if (m_lastCommand->getCommand()->getNetCommandType() !=
      msg->getCommand()->getNetCommandType()) {
    return FALSE;
  }
...
Bool NetPacket::isAckBothRepeat(NetCommandRef *msg) {
  ...
  NetAckBothCommandMsg *lastAck =
      (NetAckBothCommandMsg *)(m_lastCommand->getCommand());
...
Bool NetPacket::isAckStage1Repeat(NetCommandRef *msg) {
  ...
  NetAckStage2CommandMsg *lastAck =
      (NetAckStage2CommandMsg *)(m_lastCommand->getCommand());
  if (lastAck->getCommandID() != (ack->getCommandID() - 1)) {
    return FALSE;
  }
  if (lastAck->getOriginalPlayerID() != ack->getOriginalPlayerID()) {
...
Bool NetPacket::addGameCommand(NetCommandRef *msg) {
  ...
  //	DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::addGameCommand for command
  //ID %d\n", cmdMsg->getID()));

  if (isRoomForGameMessage(msg, gmsg)) {
    // Now we know there is enough room, put the new game message into the
    // packet.

    Bool needNewCommandID =
        FALSE;  // this is to allow us to force the starting command ID to be
                // respecified with this command.
    ...
    if (m_lastPlayerID != cmdMsg->getPlayerID()) {
      ...
      m_packet[m_packetLen] = cmdMsg->getPlayerID();
      m_packetLen += sizeof(UnsignedByte);
      // since if we have a new player we need to respecify the starting command
      // ID, lets force the command id to be different.
      needNewCommandID = TRUE;
...
void NetPacket::writeGameMessageArgumentToPacket(
    GameMessageArgumentDataType type, GameMessageArgumentType arg) {
  if (type == ARGUMENTDATATYPE_INTEGER) {
...
Bool NetPacket::isRoomForGameMessage(NetCommandRef *msg, GameMessage *gmsg) {
  ...
  if (msglen > (MAX_PACKET_SIZE - m_packetLen)) {
    return FALSE;  // there isn't enough room in this packet for this new
                   // message.
  }
  return TRUE;
}
...
NetCommandList *NetPacket::getCommandList() {
  ...
  //	DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::getCommandList, packet
  //length = %d\n", m_packetLen));
  retval->init();

  // These need to be the same as the default values for m_lastPlayerID,
  // m_lastFrame, etc.
  UnsignedByte playerID = 0;
  UnsignedInt frame = 0;
  ...
  while (i < m_packetLen) {
    if (m_packet[i] == 'T') {
      ...
      // command of type %d(%s)\n", commandType,
      // GetAsciiNetCommandType((NetCommandType)commandType).str()));

      switch ((NetCommandType)commandType) {
        case NETCOMMANDTYPE_GAMECOMMAND:
          msg = readGameMessage(m_packet, i);
          // DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read game command from player %d
          // for frame %d\n", playerID, frame));
          break;
        case NETCOMMANDTYPE_ACKBOTH:
        ...
        case NETCOMMANDTYPE_FRAMEINFO:
          ...
          DEBUG_LOG_LEVEL(
              DEBUG_LEVEL_NET,
              ("read frame %d from player %d, command count = %d, relay = "
               "0x%X\n",
        ...
        case NETCOMMANDTYPE_PLAYERLEAVE:
          ...
          //				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read
          //player leave message from player %d for execution on frame %d\n",
          //playerID, frame));
          break;
        case NETCOMMANDTYPE_RUNAHEADMETRICS:
        ...
        case NETCOMMANDTYPE_RUNAHEAD:
          ...
          //				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read
          //run ahead message from player %d for execution on frame %d\n",
          //playerID, frame));
          break;
        case NETCOMMANDTYPE_DESTROYPLAYER:
          msg = readDestroyPlayerMessage(m_packet, i);
          //				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read
          //CRC info message from player %d for execution on frame %d\n",
          //playerID, frame));
          break;
        case NETCOMMANDTYPE_KEEPALIVE:
          msg = readKeepAliveMessage(m_packet, i);
          //				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read
          //keep alive message from player %d\n", playerID));
          break;
        case NETCOMMANDTYPE_DISCONNECTKEEPALIVE:
        ...
        case NETCOMMANDTYPE_DISCONNECTPLAYER:
          ...
          //				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read
          //disconnect player message from player %d\n", playerID));
          break;
        case NETCOMMANDTYPE_PACKETROUTERQUERY:
          msg = readPacketRouterQueryMessage(m_packet, i);
          //				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read
          //packet router query message from player %d\n", playerID));
          break;
        case NETCOMMANDTYPE_PACKETROUTERACK:
          msg = readPacketRouterAckMessage(m_packet, i);
          //				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read
          //packet router ack message from player %d\n", playerID));
          break;
        case NETCOMMANDTYPE_DISCONNECTCHAT:
          msg = readDisconnectChatMessage(m_packet, i);
          //				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read
          //disconnect chat message from player %d\n", playerID));
          break;
        case NETCOMMANDTYPE_DISCONNECTVOTE:
          msg = readDisconnectVoteMessage(m_packet, i);
          //				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read
          //disconnect vote message from player %d\n", playerID));
          break;
        case NETCOMMANDTYPE_CHAT:
          msg = readChatMessage(m_packet, i);
          //				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read
          //chat message from player %d\n", playerID));
          break;
        case NETCOMMANDTYPE_PROGRESS:
          msg = readProgressMessage(m_packet, i);
          //				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read
          //Progress message from player %d\n", playerID));
          break;
        case NETCOMMANDTYPE_LOADCOMPLETE:
          msg = readLoadCompleteMessage(m_packet, i);
          //				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read
          //LoadComplete message from player %d\n", playerID));
          break;
        case NETCOMMANDTYPE_TIMEOUTSTART:
          msg = readTimeOutGameStartMessage(m_packet, i);
          //				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read
          //TimeOutGameStart message from player %d\n", playerID));
          break;
        case NETCOMMANDTYPE_WRAPPER:
          DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                          ("read Wrapper message from player %d\n", playerID));
          msg = readWrapperMessage(m_packet, i);
          DEBUG_LOG_LEVEL(
              DEBUG_LEVEL_NET,
              ("Done reading Wrapper message from player %d - wrapped command "
               "was %d\n",
               playerID, ((NetWrapperCommandMsg *)msg)->getWrappedCommandID()));
          break;
        case NETCOMMANDTYPE_FILE:
          DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                          ("read file message from player %d\n", playerID));
          msg = readFileMessage(m_packet, i);
          break;
        case NETCOMMANDTYPE_FILEANNOUNCE:
          DEBUG_LOG_LEVEL(
              DEBUG_LEVEL_NET,
              ("read file announce message from player %d\n", playerID));
          msg = readFileAnnounceMessage(m_packet, i);
          break;
        case NETCOMMANDTYPE_FILEPROGRESS:
          DEBUG_LOG_LEVEL(
              DEBUG_LEVEL_NET,
              ("read file progress message from player %d\n", playerID));
          msg = readFileProgressMessage(m_packet, i);
          break;
        case NETCOMMANDTYPE_DISCONNECTFRAME:
          DEBUG_LOG_LEVEL(
              DEBUG_LEVEL_NET,
              ("read disconnect frame message from player %d\n", playerID));
          msg = readDisconnectFrameMessage(m_packet, i);
          break;
        case NETCOMMANDTYPE_DISCONNECTSCREENOFF:
          DEBUG_LOG_LEVEL(
              DEBUG_LEVEL_NET,
              ("read disconnect screen off message from player %d\n",
               playerID));
        ...
        case NETCOMMANDTYPE_FRAMERESENDREQUEST:
          DEBUG_LOG_LEVEL(
              DEBUG_LEVEL_NET,
              ("read frame resend request message from player %d\n", playerID));
      ...
      if (msg == NULL) {
        DEBUG_CRASH(
            ("Didn't read a message from the packet. Things are about to go "
             "wrong."));
        continue;
      }
      ...
      //			DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("frame = %d,
      //player = %d, command type = %d, id = %d\n", frame, playerID,
      ...
      if (ref != NULL) {
        ...
        DEBUG_LOG_LEVEL(
            DEBUG_LEVEL_NET,
            ("NetPacket::getCommandList - failed to set relay for message %d\n",
             msg->getID()));
      }
      ...
      msg->detach();  // Need to detach from new NetCommandMsg created by the
                      // "readXMessage" above.

      // since the message is part of the list now, we don't have to keep track
      // of it.  So we'll just set it to NULL.
      msg = NULL;
      ...
      if (commandType == NETCOMMANDTYPE_ACKSTAGE1) {
        ...
        NetAckStage1CommandMsg *last =
            (NetAckStage1CommandMsg *)(lastCommand->getCommand());
        ((NetAckStage1CommandMsg *)msg)->setCommandID(last->getCommandID() + 1);
        ((NetAckStage1CommandMsg *)msg)
            ->setOriginalPlayerID(last->getOriginalPlayerID());
      } else if (commandType == NETCOMMANDTYPE_ACKSTAGE2) {
        msg = newInstance(NetAckStage2CommandMsg)();
        NetAckStage2CommandMsg *last =
            (NetAckStage2CommandMsg *)(lastCommand->getCommand());
        ((NetAckStage2CommandMsg *)msg)->setCommandID(last->getCommandID() + 1);
        ((NetAckStage2CommandMsg *)msg)
            ->setOriginalPlayerID(last->getOriginalPlayerID());
      } else if (commandType == NETCOMMANDTYPE_ACKBOTH) {
        msg = newInstance(NetAckBothCommandMsg)();
        NetAckBothCommandMsg *last =
            (NetAckBothCommandMsg *)(lastCommand->getCommand());
        ((NetAckBothCommandMsg *)msg)->setCommandID(last->getCommandID() + 1);
        ((NetAckBothCommandMsg *)msg)
            ->setOriginalPlayerID(last->getOriginalPlayerID());
      } else if (commandType == NETCOMMANDTYPE_FRAMEINFO) {
        ...
        DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                        ("Read a repeated frame command, frame = %d, player = "
                         "%d, commandID = %d\n",
      ...
      // we don't recognize this command, but we have to increment i so we don't
      // fall into an infinite loop.
      DEBUG_CRASH(("Unrecognized packet entry, ignoring."));
      DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                      ("NetPacket::getCommandList - Unrecognized packet entry "
                       "at index %d\n",
                       i));
      dumpPacketToLog();
      ++i;
...
/**
 * Reads the data portion of a game message from the given position in the
 * packet.
 */
NetCommandMsg *NetPacket::readGameMessage(UnsignedByte *data, Int &i) {
  ...
  for (j = 0; j < totalArgCount; ++j) {
    ...
    if (argsLeftForType == 0) {
      DEBUG_ASSERTCRASH(
          parserArgType != NULL,
          ("parserArgType was NULL when it shouldn't have been."));
      if (parserArgType == NULL) {
        return NULL;
      }
...
void NetPacket::readGameMessageArgumentFromPacket(
    GameMessageArgumentDataType type, NetGameCommandMsg *msg,
    UnsignedByte *data, Int &i) {
  if (type == ARGUMENTDATATYPE_INTEGER) {
...
NetCommandMsg *NetPacket::readDisconnectKeepAliveMessage(UnsignedByte *data,
                                                         Int &i) {
  NetDisconnectKeepAliveCommandMsg *msg =
      newInstance(NetDisconnectKeepAliveCommandMsg);

  return msg;
}

/**
 * Reads the disconnect player data.  Which is the slot number of the player
 * being disconnected.
 */
NetCommandMsg *NetPacket::readDisconnectPlayerMessage(UnsignedByte *data,
                                                      Int &i) {
  NetDisconnectPlayerCommandMsg *msg =
...
NetCommandMsg *NetPacket::readPacketRouterQueryMessage(UnsignedByte *data,
                                                       Int &i) {
  NetPacketRouterQueryCommandMsg *msg =
      newInstance(NetPacketRouterQueryCommandMsg);
...
NetCommandMsg *NetPacket::readDisconnectChatMessage(UnsignedByte *data,
  ...
  // DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::readDisconnectChatMessage -
  // read message, message is %ls\n", unitext.str()));
...
NetCommandMsg *NetPacket::readChatMessage(UnsignedByte *data, Int &i) {
  ...
  // DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::readChatMessage - read
  // message, message is %ls\n", unitext.str()));
...
/**
 * Reads the disconnect vote data.  Which is the slot number of the player being
 * disconnected.
 */
...
/**
 * Reads the Progress data.  Which is the slot number of the player being
 * disconnected.
 */
...
NetCommandMsg *NetPacket::readWrapperMessage(UnsignedByte *data, Int &i) {
  ...
  DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                  ("NetPacket::readWrapperMessage - wrapped command ID == %d\n",
                   wrappedCommandID));
  ...
  DEBUG_LOG_LEVEL(
      DEBUG_LEVEL_NET,
      ("NetPacket::readWrapperMessage - chunk number = %d\n", chunkNumber));
  ...
  DEBUG_LOG_LEVEL(
      DEBUG_LEVEL_NET,
      ("NetPacket::readWrapperMessage - number of chunks = %d\n", numChunks));
  ...
  DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                  ("NetPacket::readWrapperMessage - total data length = %d\n",
                   totalDataLength));
  ...
  DEBUG_LOG_LEVEL(
      DEBUG_LEVEL_NET,
      ("NetPacket::readWrapperMessage - data length = %d\n", dataLength));
  ...
  DEBUG_LOG_LEVEL(
      DEBUG_LEVEL_NET,
      ("NetPacket::readWrapperMessage - data offset = %d\n", dataOffset));
...
NetCommandMsg *NetPacket::readFileMessage(UnsignedByte *data, Int &i) {
  ...
  *c = 0;
  ++i;
  msg->setPortableFilename(
      AsciiString(filename));  // it's transferred as a portable filename
...
NetCommandMsg *NetPacket::readDisconnectFrameMessage(UnsignedByte *data,
  ...
  DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::readDisconnectFrameMessage - "
                                    "read disconnect frame for frame %d\n",
                                    disconnectFrame));
...
/**
 * Returns the number of commands in this packet.  Only valid if the packet is
 * locally constructed.
 */
Int NetPacket::getNumCommands() { return m_numCommands; }

/**
 * Returns the address that this packet is to be sent to.  Only valid if the
 * packet is locally constructed.
 */
UnsignedInt NetPacket::getAddr() { return m_addr; }

/**
 * Returns the port that this packet is to be sent to.  Only valid if the packet
 * is locally constructed.
 */
UnsignedShort NetPacket::getPort() { return m_port; }

/**
 * Returns the data of this packet.
 */
UnsignedByte *NetPacket::getData() { return m_packet; }

/**
 * Returns the length of the packet.
 */
Int NetPacket::getLength() { return m_packetLen; }
...
void NetPacket::dumpPacketToLog() {
  DEBUG_LOG_LEVEL(
      DEBUG_LEVEL_NET,
      ("NetPacket::dumpPacketToLog() - packet is %d bytes\n", m_packetLen));
  ...
  for (Int dumpindex = 0; dumpindex < numLines; ++dumpindex) {
    ...
    for (Int dumpindex2 = 0;
         (dumpindex2 < 8) && ((dumpindex * 8 + dumpindex2) < m_packetLen);
         ++dumpindex2) {
      DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                      ("%02x '%c' ", m_packet[dumpindex * 8 + dumpindex2],
                       m_packet[dumpindex * 8 + dumpindex2]));
    }
    DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("\n"));
  }
