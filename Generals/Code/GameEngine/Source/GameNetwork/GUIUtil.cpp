...
void EnableSlotListUpdates(Bool val) { winInitialized = val; }

Bool AreSlotListUpdatesEnabled(void) { return winInitialized; }

// -----------------------------------------------------------------------------

void EnableAcceptControls(Bool Enabled, GameInfo *myGame,
                          GameWindow *comboPlayer[], GameWindow *comboColor[],
                          GameWindow *comboPlayerTemplate[],
                          GameWindow *comboTeam[], GameWindow *buttonAccept[],
                          GameWindow *buttonStart,
                          GameWindow *buttonMapStartPosition[], Int slotNum) {
  if (slotNum == -1 || slotNum >= MAX_SLOTS)
    slotNum = myGame->getLocalSlotNum();

  Bool isObserver = myGame->getConstSlot(slotNum)->getPlayerTemplate() ==
  ...
  if (!isObserver) canChooseStartSpot = TRUE;
  for (Int i = 0; i < MAX_SLOTS && !canChooseStartSpot && myGame->amIHost();
       ++i) {
...
void ShowUnderlyingGUIElements(Bool show, const char *layoutFilename,
                               const char *parentName,
                               const char **gadgetsToHide,
  ...
  while (*text) {
    ...
    // DEBUG_ASSERTCRASH(win, ("Cannot find %s to show/hide it",
    // gadgetName.str()));
    if (win) {
      win->winHide(!show);
    }
...
void PopulateColorComboBox(Int comboBox, GameWindow *comboArray[],
  ...
  for (Int i = 0; i < numColors; i++) availableColors.push_back(true);

  for (i = 0; i < MAX_SLOTS; i++) {
    GameSlot *slot = myGame->getSlot(i);
    if (slot && (i != comboBox) && (slot->getColor() >= 0) &&
        (slot->getColor() < numColors)) {
      DEBUG_ASSERTCRASH(slot->getColor() >= 0,
                        ("We've tried to access array %d and that ain't good",
                         slot->getColor()));
  ...
  Int newIndex =
      GadgetComboBoxAddEntry(comboArray[comboBox],
  ...
  for (Int c = 0; c < numColors; ++c) {
    ...
    if (!def || availableColors[c] == false) continue;
  ...
  if (wasObserver) GadgetComboBoxSetSelectedPos(comboArray[comboBox], 0);
}
...
void PopulatePlayerTemplateComboBox(Int comboBox, GameWindow *comboArray[],
  ...
  Int newIndex = GadgetComboBoxAddEntry(
      comboArray[comboBox], TheGameText->fetch("GUI:Random"), def->getColor());
  ...
  for (Int c = 0; c < numPlayerTemplates; ++c) {
    ...
    newIndex = GadgetComboBoxAddEntry(
        comboArray[comboBox], TheGameText->fetch(side), def->getColor());
    GadgetComboBoxSetItemData(comboArray[comboBox], newIndex, (void *)c);
  }
  seenSides.clear();
  ...
  if (allowObservers) {
    ...
    newIndex = GadgetComboBoxAddEntry(comboArray[comboBox],
                                      TheGameText->fetch("GUI:Observer"),
                                      def->getColor());
    GadgetComboBoxSetItemData(comboArray[comboBox], newIndex,
                              (void *)PLAYERTEMPLATE_OBSERVER);
  }
#endif
  GadgetComboBoxSetSelectedPos(comboArray[comboBox], 0);
}
...
void PopulateTeamComboBox(Int comboBox, GameWindow *comboArray[],
  ...
  MultiplayerColorDefinition *def =
      TheMultiplayerSettings->getColor(PLAYERTEMPLATE_RANDOM);
  Int newIndex = GadgetComboBoxAddEntry(
      comboArray[comboBox], TheGameText->fetch("Team:0"), def->getColor());
  GadgetComboBoxSetItemData(comboArray[comboBox], newIndex, (void *)-1);
  ...
  for (Int c = 0; c < numTeams; ++c) {
    ...
    newIndex =
        GadgetComboBoxAddEntry(comboArray[comboBox], teamName, def->getColor());
    GadgetComboBoxSetItemData(comboArray[comboBox], newIndex, (void *)c);
  }
  GadgetComboBoxSetSelectedPos(comboArray[comboBox], 0);
}
...
void UpdateSlotList(GameInfo *myGame, GameWindow *comboPlayer[],
                    ...
                    GameWindow *comboTeam[], GameWindow *buttonAccept[],
                    GameWindow *buttonStart,
                    GameWindow *buttonMapStartPosition[]) {
  if (!AreSlotListUpdatesEnabled()) return;
  ...
  if (myGame) {
    for (int i = 0; i < MAX_SLOTS; i++) {
      ...
      if (myGame->amIHost() && slot && slot->isAI()) {
        EnableAcceptControls(TRUE, myGame, comboPlayer, comboColor,
                             comboPlayerTemplate, comboTeam, buttonAccept,
                             buttonStart, buttonMapStartPosition, i);
      } else if (slot && myGame->getLocalSlotNum() == i) {
        if (slot->isAccepted() && !myGame->amIHost()) {
          EnableAcceptControls(FALSE, myGame, comboPlayer, comboColor,
                               comboPlayerTemplate, comboTeam, buttonAccept,
          ...
          if (slot->hasMap()) {
            ...
            EnableAcceptControls(willTransfer, myGame, comboPlayer, comboColor,
                                 comboPlayerTemplate, comboTeam, buttonAccept,
                                 buttonStart, buttonMapStartPosition);
          }
        }
      ...
      if (slot && slot->isHuman()) {
        ...
        if (buttonAccept && buttonAccept[i]) buttonAccept[i]->winHide(TRUE);
      }
      /*
      ...
      if (!myGame->amIHost()) {
        if (comboPlayer[i]) comboPlayer[i]->winEnable(FALSE);
      }
      // if( i == myGame->getLocalSlotNum())
      if ((comboColor[i] != NULL) &&
          BitTest(comboColor[i]->winGetStatus(), WIN_STATUS_ENABLED))
        PopulateColorComboBox(i, comboColor, myGame,
                              myGame->getConstSlot(i)->getPlayerTemplate() ==
                                  PLAYERTEMPLATE_OBSERVER);
      Int max, idx;
      if (comboColor[i] != NULL) {
      ...
      if (comboPlayerTemplate[i] != NULL) {
        ...
        for (idx = 0; idx < max; ++idx) {
          Int playerTemplate =
              (Int)GadgetComboBoxGetItemData(comboPlayerTemplate[i], idx);
