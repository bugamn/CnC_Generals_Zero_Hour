...
static const UnsignedShort lobbyPort =
    8086;  ///< This is the UDP port used by all LANAPI communication

AsciiString GetMessageTypeString(UnsignedInt type);

const UnsignedInt LANAPI::s_resendDelta =
    10 * 1000;  ///< This is how often we announce ourselves to the world
/*
...
LANAPI::LANAPI(void) : m_transport(NULL) {
  DEBUG_LOG(
      ("LANAPI::LANAPI() - max game option size is %d, sizeof(LANMessage)=%d, "
       "MAX_PACKET_SIZE=%d\n",
...
void LANAPI::sendMessage(LANMessage *msg, UnsignedInt ip /* = 0 */) {
  if (ip != 0) {
    m_transport->queueSend(ip, lobbyPort, (unsigned char *)msg,
                           sizeof(LANMessage) /*, 0, 0 */);
    ...
    for (Int i = 0; i < MAX_SLOTS; ++i) {
      if (i != localSlot) {
        ...
        if ((slot != NULL) && (slot->isHuman())) {
          m_transport->queueSend(slot->getIP(), lobbyPort, (unsigned char *)msg,
                                 sizeof(LANMessage) /*, 0, 0 */);
        }
      }
    }
  } else {
    m_transport->queueSend(m_broadcastAddr, lobbyPort, (unsigned char *)msg,
                           sizeof(LANMessage) /*, 0, 0 */);
  }
}
...
void LANAPI::update(void) {
  ...
  for (i = 0; i < MAX_MESSAGES && !LANbuttonPushed; ++i) {
    if (m_transport->m_inBuffer[i].length > 0) {
      ...
      // GetMessageTypeString(msg->LANMessageType).str(), 	msg->name,
      //msg->userName, msg->hostName));
      switch (msg->LANMessageType) {
          // Location specification
        case LANMessage::MSG_REQUEST_LOCATIONS:  // Hey, where is everybody?
          //				DEBUG_LOG(("LANAPI::update - got a
          //MSG_REQUEST_LOCATIONS from 0x%08x\n", senderIP));
          handleRequestLocations(msg, senderIP);
          break;
        case LANMessage::MSG_GAME_ANNOUNCE:  // Here someone is, and here's his
                                             // game info!
          //				DEBUG_LOG(("LANAPI::update - got a
          //MSG_GAME_ANNOUNCE from 0x%08x\n", senderIP));
          handleGameAnnounce(msg, senderIP);
          break;
        case LANMessage::MSG_LOBBY_ANNOUNCE:  // Hey, I'm in the lobby!
          //				DEBUG_LOG(("LANAPI::update - got a
          //MSG_LOBBY_ANNOUNCE from 0x%08x\n", senderIP));
          handleLobbyAnnounce(msg, senderIP);
          break;
        case LANMessage::MSG_REQUEST_GAME_INFO:
          //				DEBUG_LOG(("LANAPI::update - got a
          //MSG_REQUEST_GAME_INFO from 0x%08x\n", senderIP));
        ...
        case LANMessage::MSG_GAME_OPTIONS:  // Here's some info about the game.
          //				DEBUG_LOG(("LANAPI::update - got a
          //MSG_GAME_OPTIONS from 0x%08x\n", senderIP));
          handleGameOptions(msg, senderIP);
          break;
        case LANMessage::MSG_INACTIVE:  // someone is telling us that we're
                                        // inactive.
  ...
  if (now > s_resendDelta + m_lastResendTime) {
    ...
    if (m_inLobby) {
      ...
      // game is in progress - RequestGameAnnounce will check if we should send
      // it
      RequestGameAnnounce();
    }
  }
  ...
  while (game) {
    if (game != m_currentGame &&
        game->getLastHeard() + s_resendDelta * 2 < now) {
      // He's gone!
      removeGame(game);
  ...
  if (m_currentGame && !m_currentGame->isGameInProgress()) {
    if (!AmIHost() &&
        (m_currentGame->getLastHeard() + s_resendDelta * 16 < now)) {
      ...
      wcsncpy(msg.name, m_currentGame->getPlayerName(0).str(),
              g_lanPlayerNameLength);
      ...
      for (int p = 1; p < MAX_SLOTS; ++p) {
        if (m_currentGame->getIP(p) &&
            m_currentGame->getPlayerLastHeard(p) + s_resendDelta * 8 < now) {
          LANMessage msg;
          fillInLANMessage(&msg);
          UnicodeString theStr;
          theStr.format(TheGameText->fetch("LAN:PlayerDropped"),
                        m_currentGame->getPlayerName(p).str());
          msg.LANMessageType = LANMessage::MSG_REQUEST_GAME_LEAVE;
          wcsncpy(msg.name, m_currentGame->getPlayerName(p).str(),
                  g_lanPlayerNameLength);
          msg.name[g_lanPlayerNameLength] = 0;
          handleRequestGameLeave(&msg, m_currentGame->getIP(p));
          OnChat(UnicodeString::TheEmptyString, m_localIP, theStr,
                 LANCHAT_SYSTEM);
        }
      }
    }
  ...
  if (m_gameStartTime && m_gameStartSeconds && m_gameStartTime <= now) {
    ...
    //		DEBUG_LOG(("m_gameStartTime=%d, now=%d,
    //m_gameStartSeconds=%d\n", m_gameStartTime, now, m_gameStartSeconds));
    ResetGameStartTimer();
    RequestGameStart();
  }
...
void LANAPI::RequestGameLeave(void) {
  ...
  msg.LANMessageType = LANMessage::MSG_REQUEST_GAME_LEAVE;
  fillInLANMessage(&msg);
  wcsncpy(msg.GameToLeave.gameName,
          (m_currentGame) ? m_currentGame->getName().str() : L"",
          g_lanGameNameLength);
  msg.GameToLeave.gameName[g_lanGameNameLength] = 0;
  sendMessage(&msg);
  m_transport->update();  // Send immediately, before OnPlayerLeave below resets
                          // everything.
...
void LANAPI::RequestGameAnnounce(void) {
  ...
  if (m_currentGame && !(m_currentGame->getIsDirectConnect())) {
    if (m_currentGame->getIP(0) == m_localIP ||
        (m_currentGame->isGameInProgress() && TheNetwork &&
         TheNetwork->isPacketRouter()))  // if we're in game we should reply if
                                         // we're the packet router
    {
      LANMessage reply;
      ...
      wcsncpy(reply.GameInfo.gameName, m_currentGame->getName().str(),
              g_lanGameNameLength);
...
void LANAPI::RequestAccept(void) {
  if (m_inLobby || !m_currentGame) return;

  LANMessage msg;
  fillInLANMessage(&msg);
  msg.LANMessageType = LANMessage::MSG_SET_ACCEPT;
  msg.Accept.isAccepted = true;
  wcsncpy(msg.Accept.gameName, m_currentGame->getName().str(),
          g_lanGameNameLength);
...
void LANAPI::RequestHasMap(void) {
  ...
  msg.MapStatus.hasMap =
      m_currentGame->getSlot(m_currentGame->getLocalSlotNum())->hasMap();
  wcsncpy(msg.MapStatus.gameName, m_currentGame->getName().str(),
          g_lanGameNameLength);
  msg.MapStatus.gameName[g_lanGameNameLength] = 0;
  CRC mapNameCRC;
  // mapNameCRC.computeCRC(m_currentGame->getMap().str(),
  // m_currentGame->getMap().getLength());
  AsciiString portableMapName =
      TheGameState->realMapPathToPortableMapPath(m_currentGame->getMap());
  ...
  if (!msg.MapStatus.hasMap) {
    ...
    if (mapData) {
      ...
      if (mapData->m_isOfficial) willTransfer = FALSE;
    } else {
      mapDisplayName.format(
          L"%hs", TheGameState->getMapLeafName(m_currentGame->getMap()).str());
      willTransfer = WouldMapTransfer(m_currentGame->getMap());
    }
    if (willTransfer)
      text.format(TheGameText->fetch("GUI:LocalPlayerNoMapWillTransfer"),
                  mapDisplayName.str());
    else
...
void LANAPI::RequestGameOptions(AsciiString gameOptions, Bool isPublic,
                                UnsignedInt ip /* = 0 */) {
  DEBUG_ASSERTCRASH(gameOptions.getLength() < m_lanMaxOptionsLength,
                    ("Game options string is too long!"));
  ...
  // We can request game options (side, color, etc) while we don't have a slot
  // yet.  Of course, we don't need to call OnGameOptions for those, so it's
  // okay to silently fail.
  // DEBUG_ASSERTCRASH(player != MAX_SLOTS, ("Requested game options, but we're
  // not in slot list!");
}
...
void LANAPI::RequestGameCreate(UnicodeString gameName, Bool isDirectConnect) {
  ...
  /// @todo: make sure LAN games with identical names don't crash things like in
  /// RA2.

  if ((!m_inLobby || m_currentGame) && !isDirectConnect) {
    DEBUG_ASSERTCRASH(m_inLobby && m_currentGame,
                      ("Can't create a game while in one!"));
  ...
  while (s.getLength() > g_lanGameNameLength) s.removeLastChar();
  ...
  newSlot.setPort(
      NETWORK_BASE_PORT_NUMBER);  // LAN game, everyone has a unique IP, so it's
                                  // ok to use the same port.
  ...
  /*
          ...
          for (player=0; player<MAX_SLOTS; ++player)
          {
                  wcscpy(msg.GameInfo.name[player],
     myGame->getPlayerName(player).str()); msg.GameInfo.ip[player] =
     myGame->getIP(player); msg.GameInfo.playerAccepted[player] =
     myGame->getAccepted(player);
          }
...
void LANAPI::RequestSlotList( void )
{
        ...
        for (player = 0; player < MAX_SLOTS; ++player)
        {
                wcsncpy(reply.GameInfo.name[player],
m_currentGame->getPlayerName(player).str(), g_lanPlayerNameLength);
                reply.GameInfo.name[player][g_lanPlayerNameLength] = 0;
                reply.GameInfo.ip[player] = m_currentGame->getIP(player);
                reply.GameInfo.playerAccepted[player] =
m_currentGame->getSlot(player)->isAccepted();
        }
        wcsncpy(reply.GameInfo.gameName, m_currentGame->getName().str(),
g_lanGameNameLength); reply.GameInfo.gameName[g_lanGameNameLength] = 0;
        reply.GameInfo.inProgress = m_currentGame->isGameInProgress();
...
void LANAPI::addGame(LANGameInfo *game) {
  if (!m_games) {
    ...
    if (game->getName().compareNoCase(m_games->getName()) < 0) {
      ...
      LANGameInfo *g = m_games;
      while (g->getNext() &&
             g->getNext()->getName().compareNoCase(game->getName()) > 0) {
        g = g->getNext();
      }
...
void LANAPI::addPlayer(LANPlayer *player) {
  if (!m_lobbyPlayers) {
    ...
    if (player->getName().compareNoCase(m_lobbyPlayers->getName()) < 0) {
      ...
      LANPlayer *p = m_lobbyPlayers;
      while (p->getNext() &&
             p->getNext()->getName().compareNoCase(player->getName()) > 0) {
        p = p->getNext();
      }
