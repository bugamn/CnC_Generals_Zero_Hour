...
// ////////////////////////////////////////////////////// GameSpy Persistent
// Storage callbacks, utils, etc Author: Matthew D. Campbell, March 2002
...
class GameSpyPlayerInfo : public GameSpyPlayerInfoInterface {
 public:
  GameSpyPlayerInfo() {
    m_locale.clear();
    m_wins = m_losses = m_operationCount = 0;
...
void GameSpyPlayerInfo::threadReadFromServer(void) {
  ...
  if (gameSpyInitPersistentStorageConnection()) {
    ...
    DEBUG_LOG(("GameSpyPlayerInfo::readFromServer() operation count = %d\n",
               m_operationCount));
    GetPersistDataValues(0, TheGameSpyChat->getProfileID(), pd_public_rw, 0,
                         "\\locale\\wins\\losses", getPersistentDataCallback,
                         &m_operationCount);
  } else {
...
void GameSpyPlayerInfo::setLocale(AsciiString locale, Bool setOnServer) {
  ...
  if (!TheGameSpyChat->getProfileID() || !setOnServer) return;
...
void GameSpyPlayerInfo::setValue(AsciiString key, AsciiString val,
                                 Bool setOnServer) {
  if (!setOnServer) return;
...
void GameSpyPlayerInfo::threadSetLocale(AsciiString val) {
  MutexClass::LockClass m(TheGameSpyMutex);
  if (!gameSpyInitPersistentStorageConnection()) return;
  ...
  DEBUG_LOG(("GameSpyPlayerInfo::set%s() operation count = %d\n", key.str(),
             m_operationCount));
  SetPersistDataValues(0, TheGameSpyChat->getProfileID(), pd_public_rw, 0,
                       writable, setPersistentDataCallback, &m_operationCount);
  free(writable);
}
...
static void getPersistentDataCallback(int localid, int profileid,
  ...
  peerSetGlobalKeys(TheGameSpyChat->getPeer(), 3, (const char **)keys,
                    (const char **)values);
  peerSetGlobalWatchKeys(TheGameSpyChat->getPeer(), GroupRoom, 3, keys,
                         PEERTrue);
  peerSetGlobalWatchKeys(TheGameSpyChat->getPeer(), StagingRoom, 3, keys,
                         PEERTrue);
...
static Bool gameSpyInitPersistentStorageConnection(void) {
  if (IsStatsConnected()) return true;
  ...
  **********/
  ...
  if (TheGameSpyChat->getProfileID()) {
    ...
    /***********
    We'll go ahead and start the authentication, using a Presence & Messaging
    SDK profileid / password.  To generate the new validation token, we'll need
    to pass in the password for the profile we are authenticating. Again, if
    this is done in a client/server setting, with the Persistent Storage access
    being done on the server, and the P&M SDK is used on the client, the server
    will need to send the challenge (GetChallenge(NULL)) to the client, the
    ...
    ***********/
    char *munkeeHack =
        strdup(TheGameSpyChat->getPassword()
                   .str());  // GenerateAuth takes a char*, not a const char* :P
    ...
    ************/
    PreAuthenticatePlayerPM(0, TheGameSpyChat->getProfileID(), validate,
                            persAuthCallback, NULL);
  } else {
    return isProfileAuthorized;
  }

  UnsignedInt timeoutTime = timeGetTime() + 5000;
  while (!isProfileAuthorized && timeGetTime() < timeoutTime &&
         IsStatsConnected()) {
    PersistThink();
    msleep(10);
  }
