...
Int NetCommandWrapperListNode::getPercentComplete(void) {
  if (isComplete())
    ...
    return min(99, REAL_TO_INT(((Real)m_numChunksPresent) /
                               ((Real)m_numChunks) * 100.0f));
}

UnsignedShort NetCommandWrapperListNode::getCommandID() { return m_commandID; }
...
void NetCommandWrapperListNode::copyChunkData(NetWrapperCommandMsg *msg) {
  if (msg == NULL) {
    DEBUG_CRASH(
        ("Trying to copy data from a non-existent wrapper command message"));
    return;
  }
  ...
  DEBUG_ASSERTCRASH(
      ...
      ("MunkeeChunk %d of %d\n", msg->getChunkNumber(), m_numChunks));
  if (msg->getChunkNumber() >= m_numChunks) return;
...
UnsignedByte *NetCommandWrapperListNode::getRawData() { return m_data; }
...
NetCommandWrapperList::NetCommandWrapperList() { m_list = NULL; }
...
void NetCommandWrapperList::init() { m_list = NULL; }
...
void NetCommandWrapperList::processWrapper(NetCommandRef *ref) {
  ...
  while ((temp != NULL) &&
         (temp->getCommandID() != msg->getWrappedCommandID())) {
    temp = temp->m_next;
  }
...
NetCommandList *NetCommandWrapperList::getReadyCommands() {
  ...
  while (temp != NULL) {
    next = temp->m_next;
    if (temp->isComplete()) {
      NetCommandRef *msg = NetPacket::ConstructNetCommandMsgFromRawData(
          temp->getRawData(), temp->getRawDataLength());
