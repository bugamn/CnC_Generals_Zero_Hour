 ...
 * Destructor.
 */
NetCommandList::~NetCommandList() { reset(); }
...
void NetCommandList::appendList(NetCommandList *list) {
  ...
  // Need to do it this way because of the reference counting that needs to
  // happen in appendMessage.
 ...
 */
NetCommandRef *NetCommandList::getFirstMessage() { return m_first; }
 ...
 * Initialize the list.
 */
void NetCommandList::init() { reset(); }
...
/**
 * Insert sorts msg.  Assumes that all the previous message inserts were done
 * using this function. The message is sorted in based first on command type,
 * then player id, and then command id.
 */
NetCommandRef *NetCommandList::addMessage(NetCommandMsg *cmdMsg) {
  if (cmdMsg == NULL) {
    DEBUG_ASSERTCRASH(
        cmdMsg != NULL,
        ("NetCommandList::addMessage - command message was NULL"));
    return NULL;
  }
  ...
  if (m_lastMessageInserted != NULL) {
    // Messages that are inserted in order should just be put in one right after
    // the other. So saving the placement of the last message inserted can give
    // us a huge boost in efficiency.
    NetCommandRef *theNext = m_lastMessageInserted->getNext();
    if ((m_lastMessageInserted->getCommand()->getNetCommandType() ==
         msg->getCommand()->getNetCommandType()) &&
        (m_lastMessageInserted->getCommand()->getPlayerID() ==
         msg->getCommand()->getPlayerID()) &&
        (m_lastMessageInserted->getCommand()->getID() <
         msg->getCommand()->getID()) &&
        ((theNext == NULL) ||
         ((theNext->getCommand()->getNetCommandType() >
           msg->getCommand()->getNetCommandType()) ||
          (theNext->getCommand()->getPlayerID() >
           msg->getCommand()->getPlayerID()) ||
          (theNext->getCommand()->getID() > msg->getCommand()->getID())))) {
      // Make sure this command isn't already in the list.
      if (isEqualCommandMsg(m_lastMessageInserted->getCommand(),
                            msg->getCommand())) {
      ...
      if (theNext == NULL) {
        // this means that m_lastMessageInserted == m_last, so m_last should
        // point to the msg that is being inserted.
  ...
  if (msg->getCommand()->getNetCommandType() >
      m_last->getCommand()->getNetCommandType()) {
  ...
  if (msg->getCommand()->getNetCommandType() <
      m_first->getCommand()->getNetCommandType()) {
    // Make sure this command isn't already in the list.
  ...
  while ((tempmsg != NULL) && (msg->getCommand()->getNetCommandType() >
                               tempmsg->getCommand()->getNetCommandType())) {
  ...
  while ((tempmsg != NULL) &&
         (msg->getCommand()->getNetCommandType() ==
          tempmsg->getCommand()->getNetCommandType()) &&
         (msg->getCommand()->getPlayerID() >
          tempmsg->getCommand()->getPlayerID())) {
    tempmsg = tempmsg->getNext();
  }
  ...
  // If the command type doesn't require a command ID, sort by whatever it
  // should be sorted by.
  while ((tempmsg != NULL) &&
         (msg->getCommand()->getNetCommandType() ==
          tempmsg->getCommand()->getNetCommandType()) &&
         (msg->getCommand()->getPlayerID() ==
          tempmsg->getCommand()->getPlayerID()) &&
         (msg->getCommand()->getSortNumber() >
          tempmsg->getCommand()->getSortNumber())) {
    tempmsg = tempmsg->getNext();
  }
...
NetCommandRef *NetCommandList::findMessage(NetCommandMsg *msg) {
  ...
  while ((retval != NULL) &&
         (isEqualCommandMsg(retval->getCommand(), msg) == FALSE)) {
...
NetCommandRef *NetCommandList::findMessage(UnsignedShort commandID,
  ...
  NetCommandRef *retval = m_first;
  while (retval != NULL) {
    if (DoesCommandRequireACommandID(
            retval->getCommand()->getNetCommandType())) {
      if ((retval->getCommand()->getID() == commandID) &&
          (retval->getCommand()->getPlayerID() == playerID)) {
        return retval;
      }
    }
    retval = retval->getNext();
...
Bool NetCommandList::isEqualCommandMsg(NetCommandMsg *msg1,
  ...
  if (DoesCommandRequireACommandID(msg1->getNetCommandType()) !=
      DoesCommandRequireACommandID(msg2->getNetCommandType())) {
    return FALSE;
  }

  // At this point we know that the commands both do or do not require a command
  ...
  // If we've gotten this far, we know that the commands do not require a
  // command id. So now our equality checking becomes type-specific.
