...
void GameSlot::saveOffOriginalInfo(void) {
  DEBUG_LOG((
      "GameSlot::saveOffOriginalInfo() - orig was color=%d, pos=%d, house=%d\n",
...
static Int getSlotIndex(const GameSlot *slot) {
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if (TheGameInfo->getConstSlot(i) == slot) return i;
  }
  return -1;
}
...
static Bool isSlotLocalAlly(const GameSlot *slot) {
  ...
  if (slotIndex < 0 || localIndex < 0) return FALSE;

  // if slot is us, ally
  if (slotIndex == localIndex) return TRUE;

  // if slot is same team as us, ally
  if (slot->getTeamNumber() == localSlot->getTeamNumber() &&
      slot->getTeamNumber() >= 0)
    return TRUE;
...
UnicodeString GameSlot::getApparentPlayerTemplateDisplayName(void) const {
  if (TheMultiplayerSettings &&
      TheMultiplayerSettings->showRandomPlayerTemplate() &&
  ...
  DEBUG_LOG(
      ("Fetching player template display name for player template %d (orig is "
       "%d)\n",
       m_playerTemplate, m_origPlayerTemplate));
  if (m_playerTemplate < 0) {
    return TheGameText->fetch("GUI:Random");
  }
  return ThePlayerTemplateStore->getNthPlayerTemplate(m_playerTemplate)
      ->getDisplayName();
}
...
void GameSlot::setState(SlotState state, UnicodeString name, UnsignedInt IP) {
  if (!(isAI() && (state == SLOT_EASY_AI || state == SLOT_MED_AI ||
    ...
    if (state == SLOT_OPEN && TheGameSpyGame &&
        TheGameSpyGame->getConstSlot(0) == this) {
...
// Various tests
Bool GameSlot::isHuman(void) const { return m_state == SLOT_PLAYER; }
...
Bool GameSlot::isOpen(void) const { return m_state == SLOT_OPEN; }
...
void GameInfo::reset(void) {
  ...
  //	m_localIP = 0; // BGC - actually we don't want this to be reset since
  //the m_localIP is
  // set properly in the constructor of LANGameInfo which uses this as a base
  // class.
  m_mapCRC = 0;
  m_mapSize = 0;
  //
...
void GameInfo::markPlayerAsPreorder(Int index) {
  if (index >= 0 && index < MAX_SLOTS) m_preorderMask |= 1 << index;
}

void GameInfo::clearSlotList(void) {
  for (int i = 0; i < MAX_SLOTS; ++i) {
    if (m_slot[i]) m_slot[i]->setState(SLOT_CLOSED);
  }
}
...
Int GameInfo::getNumPlayers(void) const {
  Int numPlayers = 0;
  for (int i = 0; i < MAX_SLOTS; ++i) {
    if (m_slot[i] && m_slot[i]->isOccupied()) numPlayers++;
  }
  return numPlayers;
}
...
Int GameInfo::getMaxPlayers(void) const {
  ...
  if (it == TheMapCache->end()) return -1;
  MapMetaData data = it->second;
  return data.m_numPlayers;
}

void GameInfo::enterGame(void) {
  DEBUG_ASSERTCRASH(!m_inGame && !m_inProgress,
                    ("Entering game at a bad time!"));
  reset();
  m_inGame = true;
  m_inProgress = false;
}
...
void GameInfo::setSlot(Int slotNum, GameSlot slotInfo) {
  DEBUG_ASSERTCRASH(slotNum >= 0 && slotNum < MAX_SLOTS,
                    ("GameInfo::setSlot - Invalid slot number"));
  if (slotNum < 0 || slotNum >= MAX_SLOTS) return;

  DEBUG_ASSERTCRASH(m_slot[slotNum], ("NULL slot pointer"));
  if (!m_slot[slotNum]) return;
  ...
  *m_slot[slotNum] = slotInfo;
  ...
  DEBUG_LOG(
      ("GameInfo::setSlot - setting slot %d to be player %ls with IP "
       "%d.%d.%d.%d\n",
       slotNum, slotInfo.getName().str(), ip >> 24, (ip >> 16) & 0xff,
       (ip >> 8) & 0xff, ip & 0xff));
}

GameSlot *GameInfo::getSlot(Int slotNum) {
  DEBUG_ASSERTCRASH(slotNum >= 0 && slotNum < MAX_SLOTS,
                    ("GameInfo::getSlot - Invalid slot number"));
  if (slotNum < 0 || slotNum >= MAX_SLOTS) return NULL;
...
Int GameInfo::getLocalSlotNum(void) const {
  DEBUG_ASSERTCRASH(m_inGame,
                    ("Looking for local game slot while not in game"));
  if (!m_inGame) return -1;
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (slot->isPlayer(m_localIP)) return i;
  }
  return -1;
}
...
Int GameInfo::getSlotNum(AsciiString userName) const {
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    const GameSlot *slot = getConstSlot(i);
    if (slot->isPlayer(uName)) return i;
  }
  return -1;
}
...
void GameInfo::setMapContentsMask(Int mask) { m_mapMask = mask; }
...
void GameInfo::setMapCRC(UnsignedInt mapCRC) {
  ...
  if (m_inGame && getLocalSlotNum() >= 0) {
    ...
    AsciiString lowerMap = m_mapName;
    lowerMap.toLower();
    // DEBUG_LOG(("GameInfo::setMapCRC - looking for map file \"%s\" in the map
    // cache\n", lowerMap.str()));
...
Bool GameInfo::isColorTaken(Int colorIdx, Int slotToIgnore) const {
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    const GameSlot *slot = getConstSlot(i);
    if (slot && slot->getColor() == colorIdx && i != slotToIgnore) return true;
  }
  return false;
}
...
void GameInfo::resetAccepted(void) {
  ...
  if (slot) slot->setAccept();
  for (int i = 1; i < MAX_SLOTS; i++) {
    slot = getSlot(i);
    if (slot) slot->unAccept();
  }
}
...
void GameInfo::adjustSlotsForMap() {
  ...
  if (md != NULL) {
    ...
    for (i = 0; i < MAX_SLOTS; ++i) {
      // we have room for more players, if this slot is unoccupied, set it to
      // open.
...
Bool GameInfo::isSkirmish(void) {
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if (i == getLocalSlotNum()) continue;

    if (getConstSlot(i)->isHuman()) return FALSE;

    if (getConstSlot(i)->isAI()) {
      if (isSlotLocalAlly(getConstSlot(i))) return FALSE;
      sawAI = TRUE;
    }
  }
  return sawAI;
...
Bool GameInfo::isMultiPlayer(void) {
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (getConstSlot(i)->isHuman()) return TRUE;
  }
...
Bool GameInfo::isSandbox(void) {
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if (i == localSlotNum) continue;

    const GameSlot *slot = getConstSlot(i);
    if (slot->isOccupied() &&
        (slot->getTeamNumber() < 0 || slot->getTeamNumber() != localTeam))
      return FALSE;
  }
  return TRUE;
}
...
AsciiString GameInfoToAsciiString(const GameInfo *game) {
  if (!game) return AsciiString::TheEmptyString;
  ...
  optionsString.format("M=%2.2x%s;MC=%X;MS=%d;SD=%d;C=%d;",
                       game->getMapContentsMask(), newMapName.str(),
                       game->getMapCRC(), game->getMapSize(), game->getSeed(),
                       game->getCRCInterval());
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (slot && slot->isHuman()) {
      AsciiString name =
          WideCharStringToMultiByte(slot->getName().str()).c_str();

      str.format("H%s,%X,%d,%c%c,%d,%d,%d,%d,%d:", name.str(), slot->getIP(),
                 slot->getPort(), (slot->isAccepted() ? 'T' : 'F'),
                 (slot->hasMap() ? 'T' : 'F'), slot->getColor(),
                 slot->getPlayerTemplate(), slot->getStartPos(),
      ...
      if (slot->getState() == SLOT_EASY_AI)
        ...
        c = 'H';
      str.format("C%c,%d,%d,%d,%d:", c, slot->getColor(),
                 slot->getPlayerTemplate(), slot->getStartPos(),
                 slot->getTeamNumber());
  ...
  DEBUG_ASSERTCRASH(
      !TheLAN || (optionsString.getLength() < m_lanMaxOptionsLength),
      ("WARNING: options string is longer than expected!  Length is %d, but "
       "max is %d!\n",
...
Bool ParseAsciiStringToGameInfo(GameInfo *game, AsciiString options) {
  ...
  while ((keyValPair = strtok_r(bufPtr, ";", &strPos)) != NULL) {
    ...
    if (key.compare("M") == 0) {
      ...
      sawMap = true;
      DEBUG_LOG(
          ("ParseAsciiStringToGameInfo - map name is %s\n", mapName.str()));
      ...
      seed = atoi(val.str());
      sawSeed = true;
      //			DEBUG_LOG(("ParseAsciiStringToGameInfo - random
      //seed is %d\n", seed));
      ...
      char *freeMe = NULL;
      AsciiString rawSlot;
      //			Bool slotsOk = true;	//flag that lets us know
      //whether or not the slot list is good.

      //			DEBUG_LOG(("ParseAsciiStringToGameInfo - Parsing
      //slot list\n"));
      for (int i = 0; i < MAX_SLOTS; ++i) {
        rawSlot = strtok_r(rawSlotBuf, ":", &pos);
        if (rawSlotBuf) freeMe = rawSlotBuf;
        rawSlotBuf = NULL;
        switch (*rawSlot.str()) {
          case 'H': {
            //							DEBUG_LOG(("ParseAsciiStringToGameInfo
            //- Human player\n"));
            char *slotPos = NULL;
            // Parse out the Name
            AsciiString slotValue(
                strtok_r((char *)rawSlot.str(), ",", &slotPos));
            if (slotValue.isEmpty()) {
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - slotValue name is empty, "
                   "quitting\n"));
              break;
            }
            UnicodeString name;
            name.set(
                MultiByteToWideCharSingleLine(slotValue.str() + 1).c_str());
            ...
            if (slotValue.isEmpty()) {
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - slotValue IP address is "
                   "empty, quitting\n"));
              break;
            }
            ...
            if (slotValue.isEmpty()) {
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - slotValue port is empty, "
                   "quitting\n"));
              break;
            }
            ...
            DEBUG_LOG(
                ("ParseAsciiStringToGameInfo - port is %d\n", playerPort));
            ...
            if (slotValue.getLength() != 2) {
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - slotValue accepted is "
                   "mis-sized, quitting\n"));
              break;
            }
            ...
            if (*svs == 'T') {
              newSlot[i].setAccept();
              // DEBUG_LOG(("ParseAsciiStringToGameInfo - player has
              // accepted\n"));
            } else if (*svs == 'F') {
              newSlot[i].unAccept();
              // DEBUG_LOG(("ParseAsciiStringToGameInfo - player has not
              // accepted\n"));
            }
            ++svs;
            if (*svs == 'T') {
              ...
              // DEBUG_LOG(("ParseAsciiStringToGameInfo - player does not have
              // map\n"));
            }
            ...
            if (slotValue.isEmpty()) {
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - slotValue color is empty, "
                   "quitting\n"));
              break;
            }
            ...
            if (color < -1 || color >= TheMultiplayerSettings->getNumColors()) {
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - player color was invalid, "
                   "quitting\n"));
              break;
            }
            newSlot[i].setColor(color);
            // DEBUG_LOG(("ParseAsciiStringToGameInfo - player color set to
            // %d\n", color));
            ...
            if (slotValue.isEmpty()) {
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - slotValue player template is "
                   "empty, quitting\n"));
              break;
            }
            ...
            if (playerTemplate < PLAYERTEMPLATE_MIN ||
              ...
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - player template value is "
                   "invalid, quitting\n"));
              break;
            }
            newSlot[i].setPlayerTemplate(playerTemplate);
            // DEBUG_LOG(("ParseAsciiStringToGameInfo - player template is
            // %d\n", playerTemplate));
            ...
            if (slotValue.isEmpty()) {
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - slotValue start position is "
                   "empty, quitting\n"));
              break;
            }
            ...
            if (startPos < -1 || startPos >= MAX_SLOTS) {
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - player start position is "
                   "invalid, quitting\n"));
              break;
            }
            newSlot[i].setStartPos(startPos);
            // DEBUG_LOG(("ParseAsciiStringToGameInfo - player start position is
            // %d\n", startPos));
            ...
            if (slotValue.isEmpty()) {
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - slotValue team number is "
                   "empty, quitting\n"));
              break;
            }
            ...
            if (team < -1 || team >= MAX_SLOTS / 2) {
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - team number is invalid, "
                   "quitting\n"));
              break;
            }
            ...
            if (slotValue.isEmpty()) {
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - NAT behavior is empty, "
                   "quitting\n"));
              break;
            }
            FirewallHelperClass::FirewallBehaviorType NATType =
                (FirewallHelperClass::FirewallBehaviorType)atoi(
                    slotValue.str());
            if ((NATType < FirewallHelperClass::FIREWALL_MIN) ||
                (NATType > FirewallHelperClass::FIREWALL_MAX)) {
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - NAT behavior is invalid, "
                   "quitting\n"));
              break;
            }
            newSlot[i].setNATBehavior(NATType);
            DEBUG_LOG(
                ("ParseAsciiStringToGameInfo - NAT behavior is %X\n", NATType));
          }  // case 'H':
          break;
          case 'C': {
            ...
            if (slotValue.isEmpty()) {
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - slotValue AI Type is empty, "
                   "quitting\n"));
              break;
            }
            ...
            switch (*(slotValue.str() + 1)) {
              ...
              default: {
                optionsOk = false;
                DEBUG_LOG(
                    ("ParseAsciiStringToGameInfo - Unknown AI, quitting\n"));
              } break;
            ...
            if (slotValue.isEmpty()) {
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - slotValue start pos is empty, "
                   "quitting\n"));
              break;
            }
            Int startPos = atoi(slotValue.str());
            ...
            for (Int j = 0; j < i; ++j) {
              if (startPos >= 0 && startPos == newSlot[i].getStartPos()) {
                isStartPosBad = TRUE;  // can't have multiple people using the
                                       // same start pos
              }
            }
            if (isStartPosBad) {
              optionsOk = false;
              DEBUG_LOG(
                  ("ParseAsciiStringToGameInfo - start pos is invalid, "
                   "quitting\n"));
              break;
            }
            newSlot[i].setStartPos(startPos);
  ...
  if (optionsOk && sawMap && sawMapCRC && sawMapSize && sawSeed &&
      sawSlotlist && sawCRC) {
    ...
    // DEBUG_LOG(("ParseAsciiStringToGameInfo - game options all good, setting
    // info\n"));

    for (Int i = 0; i < MAX_SLOTS; i++) game->setSlot(i, newSlot[i]);
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void SkirmishGameInfo::crc(Xfer *xfer) {}  // end crc
...
void SkirmishGameInfo::xfer(Xfer *xfer) {
  ...
  Int slot = MAX_SLOTS;
  xfer->xferInt(&slot);
  DEBUG_ASSERTCRASH(slot == MAX_SLOTS,
                    ("MAX_SLOTS changed, need to change version. jba."));
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void SkirmishGameInfo::loadPostProcess(void) {}  // end loadPostProcess
