 ...
 */
UnsignedInt FrameData::getFrame() { return m_frame; }

/**
 * Assign the frame number this frame data is associated with.
 */
void FrameData::setFrame(UnsignedInt frame) { m_frame = frame; }

/**
 * Returns true if all the frame command count is equal to the number of
...
FrameDataReturnType FrameData::allCommandsReady(Bool debugSpewage) {
  ...
  if (debugSpewage) {
    if ((m_lastFailedFrameCC != m_frameCommandCount) ||
        (m_lastFailedCC != m_commandCount)) {
      DEBUG_LOG(
          ("FrameData::allCommandsReady - failed, frame command count = %d, "
           "command count = %d\n",
  ...
  if (m_commandCount > m_frameCommandCount) {
    DEBUG_LOG(
        ("FrameData::allCommandsReady - There are more commands than there "
         "should be (%d, should be %d).  Commands in command list are...\n",
    ...
    while (ref != NULL) {
      DEBUG_LOG(
          ("%s, frame = %d, id = %d\n",
           ...
           ref->getCommand()->getExecutionFrame(), ref->getCommand()->getID()));
      ref = ref->getNext();
    }
    DEBUG_LOG(("FrameData::allCommandsReady - End of command list.\n"));
    DEBUG_LOG(
        ("FrameData::allCommandsReady - about to clear the command list\n"));
    reset();
    DEBUG_LOG(
        ("FrameData::allCommandsReady - command list cleared. command list "
         "length = %d, command count = %d, frame command count = %d\n",
 ...
 */
UnsignedInt FrameData::getFrameCommandCount() { return m_frameCommandCount; }

/**
 * return the number of commands received so far.
 */
UnsignedInt FrameData::getCommandCount() { return m_commandCount; }
...
void FrameData::addCommand(NetCommandMsg *msg) {
  ...
  // We don't need to worry about setting the relay since its not getting sent
  // anywhere.
  ...
  ++m_commandCount;
  // DEBUG_LOG(("added command %d, type = %d(%s), command count = %d, frame
  // command count = %d\n", msg->getID(), msg->getNetCommandType(),
  // GetAsciiNetCommandType(msg->getNetCommandType()).str(), m_commandCount,
  // m_frameCommandCount));
}
 ...
 */
NetCommandList *FrameData::getCommandList() { return m_commandList; }
