...
ConnectionManager::~ConnectionManager(void) {
  ...
  //	m_transport = NULL; // Network will delete transports; we just forget
  //them
  if (m_transport != NULL) {
    delete m_transport;
    m_transport = NULL;
  ...
  if (TheDisconnectMenu != NULL) {
    // This is done here since TheDisconnectMenu should only be there if we are
    // in a network game.
...
void ConnectionManager::init() {
...
#endif
  m_packetRouterSlot = 0;  /// @todo The LAN/WOL interface should be telling us
                           /// who the packet router is based on machine specs
                           /// passed around through game options.
  for (i = 0; i < MAX_SLOTS; ++i) {
  ...
  //	m_averageFps = 30;			// since 30 fps is the desired
  //rate, we'll start off at that. 	m_averageLatency = (Real)0.2; // 200ms seems
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    m_latencyAverages[i] =
        0.0;  // using zero since all floating point standards should be able to
              // specify 0.0 accurately.
  }
...
Bool ConnectionManager::isPlayerConnected(Int playerID) {
  return (playerID == m_localSlot ||
          (m_connections[playerID] && !m_connections[playerID]->isQuitting()));
}
 ...
 * the start of a game since there won't be any commands for the first few
 * frames due to runahead.
 */
void ConnectionManager::zeroFrames(UnsignedInt startingFrame,
                                   UnsignedInt numFrames) {
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if (m_frameData[i] != NULL) {
      //			DEBUG_LOG(("Calling zeroFrames on player %d,
      //starting frame %d, numFrames %d\n", i, startingFrame, numFrames));
 ...
 * Get those commands and relay them to the appropriate Connection(s). We make
 * the assumption that a command will only be relayed once.
 */
void ConnectionManager::doRelay() {
  ...
  for (Int i = 0; i < MAX_MESSAGES; ++i) {
    if (m_transport->m_inBuffer[i].length != 0) {
      ...
      // make a NetPacket out of this data so it can be broken up into
      // individual commands.
      packet = newInstance(NetPacket)(&(m_transport->m_inBuffer[i]));

      // DEBUG_LOG(("ConnectionManager::doRelay() - got a packet with %d
      // commands\n", packet->getNumCommands())); LOGBUFFER( packet->getData(),
      // packet->getLength() );
      ...
      // Iterate through the commands in this packet and send them to the proper
      // connections.
      while (cmd != NULL) {
        // DEBUG_LOG(("ConnectionManager::doRelay() - Looking at a command of
        // type %s\n",
 ...
 * Return TRUE if the command should not be relayed. Return FALSE if it should
 * be relayed.
 */
Bool ConnectionManager::processNetCommand(NetCommandRef *ref) {
  ...
  if ((m_connections[msg->getPlayerID()] == NULL) &&
      (msg->getPlayerID() != m_localSlot)) {
    // if this is from a player that is no longer in the game, then ignore them.
    return TRUE;
  }

  if (msg->getNetCommandType() == NETCOMMANDTYPE_WRAPPER) {
    processWrapper(ref);  // need to send the NetCommandRef since we have to
                          // construct the relay for the wrapped command.
    return FALSE;
  }
  ...
  if (msg->getNetCommandType() == NETCOMMANDTYPE_PROGRESS) {
    // DEBUG_LOG(("ConnectionManager::processNetCommand - got a progress net
  ...
  if (msg->getNetCommandType() == NETCOMMANDTYPE_TIMEOUTSTART) {
    DEBUG_LOG(
        ("ConnectionManager::processNetCommand - got a TimeOut GameStart net "
         "command from player %d\n",
         msg->getPlayerID()));
  ...
  if (msg->getNetCommandType() == NETCOMMANDTYPE_LOADCOMPLETE) {
    DEBUG_LOG(
        ("ConnectionManager::processNetCommand - got a Load Complete net "
         "command from player %d\n",
         msg->getPlayerID()));
...
void ConnectionManager::processFrameResendRequest(
    NetFrameResendRequestCommandMsg *msg) {
  ...
  if ((m_connections[playerID] == NULL) ||
      (m_connections[playerID]->isQuitting() == TRUE)) {
    return;
  }
...
void ConnectionManager::processWrapper(NetCommandRef *ref) {
  NetWrapperCommandMsg *wrapperMsg =
      (NetWrapperCommandMsg *)(ref->getCommand());
  UnsignedShort commandID = wrapperMsg->getWrappedCommandID();
  DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                  ("ConnectionManager::processWrapper() - wrapped commandID is "
                   "%d, commandID is %d\n",
  ...
  DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("ConnectionManager::processWrapper() - "
                                    "origProgress[%d] == %d for command %d\n",
...
/**
 * A client has sent us their run ahead metrics, lets store them away for future
 * calculations.
 */
void ConnectionManager::processRunAheadMetrics(
    NetRunAheadMetricsCommandMsg *msg) {
  ...
  if ((player >= 0) && (player < MAX_SLOTS) && (isPlayerConnected(player))) {
    ...
    // DEBUG_LOG(("ConnectionManager::processRunAheadMetrics - player %d, fps =
    // %d, latency = %f\n", player, msg->getAverageFps(),
    // msg->getAverageLatency()));
    if (m_fpsAverages[player] > 100) {
      // limit the reported frame rate average to 100.  This is done because if
      // a user alt-tab's out of the game their frame rate climbs to in the
      // neighborhood of 300, that was deemed "ugly" by the powers that be.
      m_fpsAverages[player] = 100;
    }
  }
}

void ConnectionManager::processDisconnectChat(
    NetDisconnectChatCommandMsg *msg) {
  ...
  //	DEBUG_LOG(("ConnectionManager::processDisconnectChat - got message from
  //player %d, message is %ls\n", playerID, unitext.str()));
...
void ConnectionManager::processChat(NetChatCommandMsg *msg) {
  ...
  } else if (((m_connections[playerID] != NULL) &&
              (m_connections[playerID]->isQuitting() == FALSE))) {
    name = m_connections[playerID]->getUser()->GetName();
    // DEBUG_LOG(("connection is non-NULL, using %ls\n", name.str()));
  }
  unitext.format(L"[%ls] %ls", name.str(), msg->getText().str());
  //	DEBUG_LOG(("ConnectionManager::processChat - got message from player %d
  //(mask %8.8X), message is %ls\n", playerID, msg->getPlayerMask(),
  //unitext.str()));
  ...
  Bool canSeeChat =
      amIObserver ||
      !fromObserver && !TheGameInfo->getConstSlot(playerID)->isMuted();
...
void ConnectionManager::processFile(NetFileCommandMsg *msg) {
#ifdef _INTERNAL
  UnicodeString log;
  log.format(L"Saw file transfer: '%hs' of %d bytes from %d",
             msg->getPortableFilename().str(), msg->getFileLength(),
             msg->getPlayerID());
  DEBUG_LOG(("%ls\n", log.str()));
#endif
  ...
  if (msg->getFilename().endsWith(".tga") &&
      CompressionManager::isDataCompressed(buf, len)) {
    Int uncompLen = CompressionManager::getUncompressedSize(buf, len);
    UnsignedByte *uncompBuffer = NEW UnsignedByte[uncompLen];
    Int actualLen =
        CompressionManager::decompressData(buf, len, uncompBuffer, uncompLen);
  ...
  File *fp = TheFileSystem->openFile(msg->getRealFilename().str(),
  ...
  DEBUG_LOG(
      ("ConnectionManager::processFile() - sending a "
       "NetFileProgressCommandMsg\n"));
  ...
  Int progressMask = 0xff ^ (1 << m_localSlot);
  NetFileProgressCommandMsg *progressMsg =
      newInstance(NetFileProgressCommandMsg);
...
void ConnectionManager::processFileAnnounce(NetFileAnnounceCommandMsg *msg) {
  DEBUG_LOG(
      ("ConnectionManager::processFileAnnounce() - expecting '%s' (%s) in "
       "command %d\n",
       msg->getPortableFilename().str(), msg->getRealFilename().str(),
       msg->getFileID()));
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if ((1 << i) & msg->getPlayerMask()) {
      ...
      s_fileProgressMap[i][msg->getFileID()] =
          100;  // they don't need to get it, so they're already done.
    }
  }
}
...
void ConnectionManager::processFileProgress(NetFileProgressCommandMsg *msg) {
  DEBUG_LOG(
      ("ConnectionManager::processFileProgress() - command %d is at %d%%\n",
       msg->getFileID(), msg->getProgress()));
  Int oldProgress = s_fileProgressMap[msg->getPlayerID()][msg->getFileID()];

  s_fileProgressMap[msg->getPlayerID()][msg->getFileID()] =
      max(oldProgress, msg->getProgress());
}
...
void ConnectionManager::processFrameInfo(NetFrameCommandMsg *msg) {
  ...
  if ((playerID >= 0) && (playerID < MAX_SLOTS)) {
    if (m_frameData[playerID] != NULL) {
      //			DEBUG_LOG(("ConnectionManager::processFrameInfo
      //- player %d, frame %d, command count %d, received on frame %d\n",
      //playerID, msg->getExecutionFrame(), msg->getCommandCount(),
      //TheGameLogic->getFrame()));
      m_frameData[playerID]->setFrameCommandCount(msg->getExecutionFrame(),
                                                  msg->getCommandCount());
    }
  }
}
...
/**
 * We just got a stage 1 ack from someone.  So we should remove it from the
 * connection that sent it so it doesn't keep resending it.
 */
void ConnectionManager::processAckStage1(NetCommandMsg *msg) {
#if defined(_DEBUG) || defined(_INTERNAL)
  Bool doDebug = (msg->getNetCommandType() == NETCOMMANDTYPE_DISCONNECTFRAME)
                     ? TRUE
                     : FALSE;
#endif
  ...
  if (doDebug == TRUE) {
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("ConnectionManager::processAck - processing ack for command %d from "
         "player %d\n",
  ...
  if ((playerID >= 0) && (playerID < NUM_CONNECTIONS)) {
    ...
    DEBUG_ASSERTCRASH(
        (playerID >= 0) && (playerID < NUM_CONNECTIONS),
        ("ConnectionManager::processAck - %d is an invalid player number"));
  }

  if (ref != NULL) {
    if (ref->getCommand()->getNetCommandType() == NETCOMMANDTYPE_FRAMEINFO) {
      m_frameMetrics.processLatencyResponse(
          ((NetFrameCommandMsg *)(ref->getCommand()))->getExecutionFrame());
    }
...
/**
 * We just got a stage 2 ack from someone.  So remove it from the pending
 * commands list so it doesn't get sent in the case of a new packet router.
 */
void ConnectionManager::processAckStage2(NetCommandMsg *msg) {
  ...
  if (ref != NULL) {
    // DEBUG_LOG(("ConnectionManager::processAckStage2 - removing command %d
    // from the pending commands list.\n", commandID));
    DEBUG_ASSERTCRASH((m_localSlot == playerID),
                      ("Found a command in the pending commands list that "
                       "wasn't originated by the local player"));
    m_pendingCommands->removeMessage(ref);
    ref->deleteInstance();
    ref = NULL;
  } else {
    // DEBUG_LOG(("ConnectionManager::processAckStage2 - Couldn't find command
    // %d from player %d in the pending commands list.\n", commandID,
    // playerID));
  }

  ref = m_relayedCommands->findMessage(commandID, playerID);
  if (ref != NULL) {
    // DEBUG_LOG(("ConnectionManager::processAckStage2 - found command ID %d
    // from player %d in the relayed commands list.\n", commandID, playerID));
    UnsignedByte relay = ref->getRelay();
    // DEBUG_LOG(("ConnectionManager::processAckStage2 - relay was %d and is now
    // ", relay));
    relay = relay & ~(1 << msg->getPlayerID());
    // DEBUG_LOG(("%d\n", relay));
    if (relay == 0) {
      // DEBUG_LOG(("ConnectionManager::processAckStage2 - relay is 0, removing
      // command from the relayed commands list.\n"));
      m_relayedCommands->removeMessage(ref);
      NetAckStage2CommandMsg *ackmsg =
          newInstance(NetAckStage2CommandMsg)(ref->getCommand());
...
/**
 * We just got a "both" ack from someone.  So process it as both a stage 1 and
 * stage 2 ack.
 */
...
/**
 * A player has just left our game. Delete their connection and frame data
 * manager. return codes are: PLAYERLEAVECODE_UNKNOWN - player didn't have a
 * valid slot number. PLAYERLEAVECODE_CLIENT - someone in the game that wasn't
 * us or the packet router. PLAYERLEAVECODE_LOCAL - We are leaving the game, we
 * could also be the packet router. PLAYERLEAVECODE_PACKETROUTER - The packet
 * router left the game.
 *
 * If we are leaving and are also the packet router, it will return the
 * PLAYERLEAVECODE_LOCAL return code.
 */
PlayerLeaveCode ConnectionManager::processPlayerLeave(
    NetPlayerLeaveCommandMsg *msg) {
  UnsignedByte playerID = msg->getLeavingPlayerID();
  if ((playerID != m_localSlot) && (m_connections[playerID] != NULL)) {
    DEBUG_LOG(
        ("ConnectionManager::processPlayerLeave() - setQuitting() on player %d "
         "on frame %d\n",
  ...
  if ((playerID != m_localSlot) && (m_frameData[playerID] != NULL) &&
    ...
    DEBUG_LOG(
        ("ConnectionManager::processPlayerLeave - setQuitFrame on player %d "
         "for frame %d\n",
         playerID, TheGameLogic->getFrame() + 1));
    m_frameData[playerID]->setQuitFrame(TheGameLogic->getFrame() +
                                        FRAMES_TO_KEEP + 1);
  }
  ...
  DEBUG_LOG(
      ("ConnectionManager::processPlayerLeave() - just disconnected player %d "
       "with ret code %d\n",
       playerID, code));
  if (code == PLAYERLEAVECODE_PACKETROUTER) resendPendingCommands();

  PopulateInGameDiplomacyPopup();
  return code;
}

UnsignedInt ConnectionManager::getPacketRouterFallbackSlot(
    Int packetRouterNumber) {
  ...
  for (Int i = 0; (i < MAX_SLOTS) && retval; ++i) {
    if (m_connections[i] != NULL) {
      if (m_connections[i]->isQueueEmpty() == FALSE) {
        // DEBUG_LOG(("ConnectionManager::areAllQueuesEmpty() -
        // m_connections[%d] is not empty\n", i));
...
Bool ConnectionManager::canILeave() { return areAllQueuesEmpty(); }

/**
 * The local player is leaving. Tell the local player as well as the other
 * players to remove this player at the specified frame.
 */
void ConnectionManager::handleLocalPlayerLeaving(UnsignedInt frame) {
  ...
  DEBUG_LOG(
      ("ConnectionManager::handleLocalPlayerLeaving - Local player leaving on "
       "frame %d\n",
       frame));
  DEBUG_ASSERTCRASH(m_packetRouterSlot >= 0,
                    ("ConnectionManager::handleLocalPlayerLeaving, packet "
                     "router is %d, illegal value.",
                     m_packetRouterSlot));
...
void ConnectionManager::ackCommand(NetCommandRef *ref, UnsignedInt localSlot) {
  ...
  // Make send relay a bitmask for the connections that the relay will actually
  // be sent to. This is necessary to determine whether or not we have to wait
  // to send a stage 2 ack.
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if (((m_connections[i] != NULL) &&
         (m_connections[i]->isQuitting() == FALSE))) {
  ...
  if (sendRelay == 0) {
    NetAckBothCommandMsg *bothmsg =
        newInstance(NetAckBothCommandMsg)(ref->getCommand());
    ackmsg = bothmsg;
    ...
    if (doDebug) {
      DEBUG_LOG_LEVEL(
          DEBUG_LEVEL_NET,
          ("ConnectionManager::ackCommand - doing ack both for command %d from "
           "player %d\n",
           bothmsg->getCommandID(), bothmsg->getOriginalPlayerID()));
    }
#endif
  } else {
    NetAckStage1CommandMsg *stage1msg =
        newInstance(NetAckStage1CommandMsg)(ref->getCommand());
    ackmsg = stage1msg;
    ...
    if (doDebug) {
      DEBUG_LOG_LEVEL(
          DEBUG_LEVEL_NET,
          ("ConnectionManager::ackCommand - doing ack stage 1 for command %d "
           "from player %d\n",
  ...
  ackmsg->setPlayerID(
      localSlot);  // Tell the player who this ack is coming from.

  if (CommandRequiresDirectSend(msg) && CommandRequiresAck(msg)) {
    // Send this ack directly back to the sending player, don't go through the
    // packet router.
    if ((msg->getPlayerID() >= 0) && (msg->getPlayerID() < MAX_SLOTS)) {
      if (m_connections[msg->getPlayerID()] != NULL) {
        m_connections[msg->getPlayerID()]->sendNetCommandMsg(
            ackmsg, 1 << msg->getPlayerID());
      }
    }
  } else {
    // The local connection may be the packet router, in that case, the
    // connection would be NULL.  So do something about it!
    if ((m_packetRouterSlot >= 0) && (m_packetRouterSlot < MAX_SLOTS)) {
      if (m_connections[m_packetRouterSlot] != NULL) {
        //				DEBUG_LOG(("ConnectionManager::ackCommand
        //- acking command %d from player %d to packet router.\n", commandID,
        //m_packetRouterSlot));
        m_connections[m_packetRouterSlot]->sendNetCommandMsg(
            ackmsg, 1 << m_packetRouterSlot);
      } else if (m_localSlot == m_packetRouterSlot) {
        // we are the packet router, send the ack to the player that sent the
        // command.
        if ((msg->getPlayerID() >= 0) && (msg->getPlayerID() < MAX_SLOTS)) {
          if (m_connections[msg->getPlayerID()] != NULL) {
            //						DEBUG_LOG(("ConnectionManager::ackCommand
            //- acking command %d from player %d directly to player.\n",
            //commandID, msg->getPlayerID()));
            m_connections[msg->getPlayerID()]->sendNetCommandMsg(
                ackmsg, 1 << msg->getPlayerID());
          } else {
            //					DEBUG_ASSERTCRASH(m_connections[msg->getPlayerID()]
            //!= NULL, ("Connection to player is NULL"));
          }
        } else {
          DEBUG_ASSERTCRASH(
              (msg->getPlayerID() >= 0) && (msg->getPlayerID() < MAX_SLOTS),
              ("Command sent by an invalid player ID."));
        }
      } else {
        DEBUG_ASSERTCRASH(m_connections[m_packetRouterSlot] != NULL,
                          ("Connection to packet router is NULL"));
      }
    } else {
      DEBUG_ASSERTCRASH(
          (m_packetRouterSlot >= 0) && (m_packetRouterSlot < MAX_SLOTS),
          ("I don't know who the packet router is."));
    }
  }
...
/**
 * This is where we relay a command from one client to others (including
 * ourselves). This should only be done by the current packet router.
 */
void ConnectionManager::sendRemoteCommand(NetCommandRef *msg) {
  ...
  DEBUG_LOG_LEVEL(
      DEBUG_LEVEL_NET,
      ("ConnectionManager::sendRemoteCommand - sending net command %d of type "
       "%s from player %d, relay is 0x%x\n",
       msg->getCommand()->getID(),
       GetAsciiNetCommandType(msg->getCommand()->getNetCommandType()).str(),
       msg->getCommand()->getPlayerID(), msg->getRelay()));

  UnsignedByte relay = msg->getRelay();
  if ((relay & (1 << m_localSlot)) &&
      (m_frameData[msg->getCommand()->getPlayerID()] != NULL)) {
    if (IsCommandSynchronized(msg->getCommand()->getNetCommandType())) {
      DEBUG_LOG_LEVEL(
          DEBUG_LEVEL_NET,
          ("ConnectionManager::sendRemoteCommand - adding net command of type "
           "%s to player %d for frame %d\n",
           GetAsciiNetCommandType(msg->getCommand()->getNetCommandType()).str(),
           msg->getCommand()->getPlayerID(),
           msg->getCommand()->getExecutionFrame()));
      m_frameData[msg->getCommand()->getPlayerID()]->addNetCommandMsg(
          msg->getCommand());
    }
  }

  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if ((relay & (1 << i)) && ((m_connections[i] != NULL) &&
                               (m_connections[i]->isQuitting() == FALSE))) {
  ...
  if ((actualRelay != 0) && (CommandRequiresAck(msg->getCommand()) == TRUE)) {
    ...
    if (ref != NULL) {
      ...
      // DEBUG_LOG(("ConnectionManager::sendRemoteCommand - command %d added to
      // relayed commands with relay %d\n", msg->getCommand()->getID(),
      // ref->getRelay()));
    }
  }
  ...
  if (IsCommandSynchronized(msg->getCommand()->getNetCommandType())) {
    //		DEBUG_LOG(("ConnectionManager::sendRemoteCommand - about to call
    //allCommandsReady\n"));
    if (allCommandsReady(msg->getCommand()->getExecutionFrame(), TRUE)) {
      UnsignedInt cushion =
          msg->getCommand()->getExecutionFrame() - TheGameLogic->getFrame();
      if ((cushion < m_smallestPacketArrivalCushion) ||
          (m_smallestPacketArrivalCushion == -1)) {
        m_smallestPacketArrivalCushion = cushion;
      }
      m_frameMetrics.addCushion(cushion);
      //			DEBUG_LOG(("Adding %d to cushion for frame
...
void ConnectionManager::update(Bool isInGame) {
  ...
  if ((m_localAddr == 0) || (m_localPort == 0)) {
    ...
    DEBUG_ASSERTCRASH((m_localAddr != 0) && (m_localPort != 0),
                      ("ConnectionManager doesn't have a local address."));
    return;
  }
  ...
  // take the packets from the transport, break them up into commands, and give
  // them to the appropriate connections.
  doRelay();
  ...
  for (Int i = 0; i < NUM_CONNECTIONS; ++i) {
    if (m_connections[i] != NULL) {
      /*
      if (m_connections[i]->isQueueEmpty() == FALSE) {
//				DEBUG_LOG(("ConnectionManager::update - calling
doSend on connection %d\n", i));
      }
      */
      ...
      if (m_connections[i]->isQuitting() && m_connections[i]->isQueueEmpty()) {
        DEBUG_LOG(
            ("ConnectionManager::update - deleting connection for slot %d\n",
             i));
        m_connections[i]->deleteInstance();
        m_connections[i] = NULL;
      }
    ...
    if ((m_frameData[i] != NULL) && (m_frameData[i]->getIsQuitting() == TRUE)) {
      if (m_frameData[i]->getQuitFrame() == TheGameLogic->getFrame()) {
        DEBUG_LOG(
            ("ConnectionManager::update - deleting frame data for slot %d on "
             "quitting frame %d\n",
...
void ConnectionManager::updateRunAhead(Int oldRunAhead, Int frameRate,
  ...
  if ((lasttimesent == 0) || ((curTime - lasttimesent) >
                              TheGlobalData->m_networkRunAheadMetricsTime)) {
    if (m_localSlot == m_packetRouterSlot) {
      // We are the packet router, time to compute a new run ahead for this
      // game.
      m_latencyAverages[m_localSlot] = m_frameMetrics.getAverageLatency();

      // since we are now using the display frame rate rather than the logic
      // frame rate to get our average FPS, it doesn't make sense to send the
      // desired logic frame rate if we "slugged" ourself.
      //			if (didSelfSlug) {
      ...
      //			}
      if (didSelfSlug) {
        // DEBUG_LOG(("ConnectionManager::updateRunAhead - local player run
        // ahead metrics, fps = %d, actual fps = %d, latency = %f, didSelfSlug =
        // true\n", m_fpsAverages[m_localSlot], m_frameMetrics.getAverageFPS(),
        // m_latencyAverages[m_localSlot]));
      } else {
        // DEBUG_LOG(("ConnectionManager::updateRunAhead - local player run
        // ahead metrics, fps = %d, latency = %f, didSelfSlug = false\n",
      ...
      DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                      ("ConnectionManager::updateRunAhead - max latency = %f, "
                       "min fps = %d, min fps player = %d old FPS = %d\n",
                       getMaximumLatency(), minFps, minFpsPlayer, frameRate));
      if ((minFps >= ((frameRate * 9) / 10)) && (minFps < frameRate)) {
        // if the minimum fps is within 10% of the desired framerate, then keep
        // the current minimum fps.
        minFps = frameRate;
      }
      if (minFps < 5) {
      ...
      DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("ConnectionManager::updateRunAhead - "
                                        "minFps after adjustment is %d\n",
                                        minFps));
      Int newRunAhead = (Int)((getMaximumLatency() / 2.0) * (Real)minFps);
      newRunAhead += (newRunAhead * TheGlobalData->m_networkRunAheadSlack) /
                     100;  // Add in 10% of slack to the run ahead in case of
                           // network hiccups.
      ...
      if (newRunAhead > (MAX_FRAMES_AHEAD / 2)) {
        newRunAhead =
            MAX_FRAMES_AHEAD / 2;  // dont let run ahead get out of hand.
      }
      ...
      // needs to be set to the greater of getExecutionFrame and
      // TheGameLogic->getFrame() + oldRunAhead This prevents the case of... run
      // ahead starts at 30 run ahead changes to 10 at frame 31 (the command was
      // created on frame 1) run ahead changes to 10 at frame 56 (the command
      // was created on frame 46) notice that 56 is within the previous run
      // ahead of 30 which has triggered the frame command count being set for
      // frames 1 through 60 since run ahead didn't change for the first time
      // till frame 31.  This creates an extra command for frame 56 that isn't
      // accounted for in the frame command count that is sent out in the
      // NetFrameCommandMsg.  sheesh.
      ...
      // DEBUG_LOG(("ConnectionManager::updateRunAhead - new run ahead = %d, new
      // frame rate = %d, execution frame %d\n", newRunAhead, minFps,
      // msg->getExecutionFrame()));
      sendLocalCommand(
          msg, 0xff ^ (1 << minFpsPlayer));  // Send the packet to everyone but
                                             // the lowest FPS player.
      ...
      if (DoesCommandRequireACommandID(msg2->getNetCommandType())) {
         ...
         * What happens is that the two run ahead commands get sent to different
         * players using different command ID's.  So player 1 has the run ahead
         * command as command x and player 2 has the command as command x+1.
         * This is all good except when it comes to players being disconnected.
         * With the new disconnect scheme player 1 could potentially send his
         * run ahead command to player 2 (or the other way around) to let player
         * 2 catch up to him.  So if player 2 has his run ahead command as x+1
         * and now he gets player 1's command list with the run ahead command
         * listed as command x, he won't see them as being the same command and
         * will now think he has two different run ahead commands for that frame
         * and thus his command list will have an extra command and he will
         * never be able to recover. So to fix this we have to use the same
         * command ID for both run ahead commands.  That way when the commands
         * are copied places for the disconnect screen they will be seen as the
      ...
      // Let the player with the slowest FPS run a little faster than the other
      // computers... just in case they are able to.  Then we might be able to
      // run the game faster which would be good.
      ...
      // We are not the packet router, send our metrics info to the packet
      // router.
      NetRunAheadMetricsCommandMsg *msg =
          newInstance(NetRunAheadMetricsCommandMsg);
      msg->setPlayerID(m_localSlot);
      if (DoesCommandRequireACommandID(msg->getNetCommandType())) {
        msg->setID(GenerateNextCommandID());
      }
      ...
      //			}
      if (didSelfSlug) {
        // DEBUG_LOG(("ConnectionManager::updateRunAhead - average latency = %f,
        // average fps = %d, actual fps = %d, didSelfSlug = true\n",
        // m_frameMetrics.getAverageLatency(), m_frameMetrics.getAverageFPS(),
        // m_frameMetrics.getAverageFPS()));
      } else {
        // DEBUG_LOG(("ConnectionManager::updateRunAhead - average latency = %f,
        // average fps = %d, didSelfSlug = false\n",
        // m_frameMetrics.getAverageLatency(), m_frameMetrics.getAverageFPS()));
      }
      m_connections[m_packetRouterSlot]->sendNetCommandMsg(
          msg, 1 << m_packetRouterSlot);
      msg->detach();
    }
...
Real ConnectionManager::getMaximumLatency() {
  // This works for 2 player games because the latency for the packet router is
  // always 0.
  Real lat1 = 0.0;
  Real lat2 = 0.0;
...
/**
 * The commands for the given frame are all ready, time to send out our command
 * count for that frame.
 */
void ConnectionManager::processFrameTick(UnsignedInt frame) {
  if ((m_frameData[m_localSlot] == NULL) ||
      (m_frameData[m_localSlot]->getIsQuitting() == TRUE)) {
  ...
  DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                  ("ConnectionManager::processFrameTick - sending frame info "
                   "for frame %d, ID %d, command count %d\n",
...
void ConnectionManager::initTransport() {
  DEBUG_ASSERTCRASH((m_transport == NULL),
                    ("m_transport already exists when trying to init it."));
...
/**
 * This is where the commands from the local client are sent to the other
 * clients in the game.  This is also where the local commands are put into the
 * frame data for future execution.
 */
...
/**
 * This is a NetCommandMsg that originated on the local computer. Send this to
 * everyone specified in the relay field.  Commands sent in this way go through
 * the packet router.
 */
void ConnectionManager::sendLocalCommand(
    NetCommandMsg *msg, UnsignedByte relay /* = 0xff by default*/) {
  if (CommandRequiresDirectSend(msg) || (m_packetRouterSlot < 0) ||
      (m_packetRouterSlot >= MAX_SLOTS) ||
      (m_connections[m_packetRouterSlot] == NULL)) {
  ...
  DEBUG_LOG_LEVEL(
      DEBUG_LEVEL_NET,
      ("ConnectionManager::sendLocalCommand - sending net command %d of type "
       "%s\n",
       msg->getID(), GetAsciiNetCommandType(msg->getNetCommandType()).str()));

  if (relay & (1 << m_localSlot)) {
    DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                    ("ConnectionManager::sendLocalCommand - adding net command "
                     "of type %s to player %d for frame %d\n",
                     GetAsciiNetCommandType(msg->getNetCommandType()).str(),
                     msg->getPlayerID(), msg->getExecutionFrame()));
    m_frameData[m_localSlot]->addNetCommandMsg(msg);
  }

  // Send the packet to everyone else
  if (m_localSlot == m_packetRouterSlot) {
    // I am the packet router, I need to send this packet to everyone
    // individually.
    for (Int i = 0; i < MAX_SLOTS; ++i) {
      ...
      if (((m_connections[i] != NULL) &&
           (m_connections[i]->isQuitting() == FALSE)) &&
          (relay & (1 << i))) {
        // Set the relay mask to only go to this player so he knows not to relay
        // it to anyone else.
        UnsignedByte temprelay = 1 << i;
        m_connections[i]->sendNetCommandMsg(
            msg, temprelay);  // This will create a new copy of netmsg for this
                              // connection.
      }
    }
  } else {
    // Send the command to everyone else via the packet router.
    UnsignedByte temprelay =
        relay &
        ~(1 << m_localSlot);  // Tell the packet router to relay the message to
                              // everyone but myself. Hopefully the packet
                              // router is smart enough to not send it to slots
    ...
    if (CommandRequiresAck(msg)) {
      ...
      // DEBUG_LOG(("ConnectionManager::sendLocalCommand - added command %d to
...
/**
 * This is a NetCommandMsg that originated on the local computer.  Send this to
 * everyone specified in the relay field.  Commands sent in this way do not go
...
void ConnectionManager::sendLocalCommandDirect(NetCommandMsg *msg,
  ...
  if (((relay & (1 << m_localSlot)) != 0) &&
      (m_frameData[m_localSlot] != NULL)) {
    if (IsCommandSynchronized(msg->getNetCommandType()) == TRUE) {
      DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                      ("ConnectionManager::sendLocalCommandDirect - adding net "
                       "command of type %s to player %d for frame %d\n",
                       GetAsciiNetCommandType(msg->getNetCommandType()).str(),
                       msg->getPlayerID(), msg->getExecutionFrame()));
      m_frameData[m_localSlot]->addNetCommandMsg(msg);
    }
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if ((relay & (1 << i)) != 0) {
      if ((m_connections[i] != NULL) &&
        ...
        DEBUG_LOG_LEVEL(
            DEBUG_LEVEL_NET,
            ("ConnectionManager::sendLocalCommandDirect - Sending direct "
             "command %d of type %s to player %d\n",
             msg->getID(),
             GetAsciiNetCommandType(msg->getNetCommandType()).str(), i));
      }
    }
  }
...
/**
 * Returns true if all the commands for the given frame are ready to be
 * executed.
 */
Bool ConnectionManager::allCommandsReady(UnsignedInt frame,
  ...
  for (Int i = 0; (i < MAX_SLOTS) && retval; ++i) {
    if ((m_frameData[i] != NULL) &&
      ...
      /*
                              if (!(m_frameData[i]->allCommandsReady(frame,
      (frame != commandsReadyDebugSpewage) && (justTesting == FALSE)))) { if
      ((frame != commandsReadyDebugSpewage) && (justTesting == FALSE)) {
                                              DEBUG_LOG(("ConnectionManager::allCommandsReady,
      frame %d player %d not ready.\n", frame, i)); commandsReadyDebugSpewage =
      frame;
                                      }
                                      retval = FALSE;
                              } else {
      //
      DEBUG_LOG(("ConnectionManager::allCommandsReady, frame %d player %d is
      ready.\n", frame, i));
                              }
      */

      frameRetVal = m_frameData[i]->allCommandsReady(
          frame,
          (frame != commandsReadyDebugSpewage) && (justTesting == FALSE));
  ...
  if (frameRetVal == FRAMEDATA_RESEND) {
    // this frame's data is really screwed up, we need to clean it out so it can
    // be resent to us.
    for (i = 0; i < MAX_SLOTS; ++i) {
  ...
  if ((retval == TRUE) && (justTesting == FALSE)) {
    ...
    retval =
        m_disconnectManager
            ->allowedToContinue();  // allow the disconnect manager to keep us
                                    // on this frame in case we are waiting for
                                    // a new packet router or something.
  }
...
/**
 * Only call this after making sure that all the commands are there for this
 * frame. After calling this the commands for this frame will be removed from
 * the connection.
 *
 * BGC - To account for the case where the host disconnects without sending the
 *       same commands to all players, we now have to keep around the last 'run
 * ahead' frames so we can potentially send those commands to the other players
 * in the game so they can catch up.
 */
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if (m_frameData[i] != NULL) {
      ...
      if (frame > FRAMES_TO_KEEP) {
        m_frameData[i]->resetFrame(
            frame - FRAMES_TO_KEEP);  // After getting the commands for that
                                      // frame from this FrameDataManager
                                      // object, we need to tell it that we're
                                      // done with the messages for that frame.
        DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("getFrameCommandList - called reset "
                                          "frame on player %d for frame %d\n",
...
void ConnectionManager::setFrameGrouping(time_t frameGrouping) {
  // Since we are the packet router, we should send more packets per second
  // since we may become the latency bottleneck for sending packets from one
  // player to the next. This is probably ok since the packet router should have
  // the fastest connection of all the players in the game.
  if (m_localSlot == m_packetRouterSlot) {
...
void ConnectionManager::doKeepAlive() {
  ...
  while ((nextIndex <= numSeconds) && (nextIndex < MAX_SLOTS)) {
    //		DEBUG_LOG(("ConnectionManager::doKeepAlive - trying to send keep
    //alive message to player %d\n", nextIndex));
    if (m_connections[nextIndex] != NULL) {
      ...
      //			DEBUG_LOG(("ConnectionManager::doKeepAlive -
      //sending keep alive message to player %d\n", nextIndex));
...
PlayerLeaveCode ConnectionManager::disconnectPlayer(Int slot) {
  ...
  DEBUG_LOG(
      ("ConnectionManager::disconnectPlayer - disconnecting slot %d on frame "
       "%d\n",
  ...
  if (TheGameInfo) {
    ...
    if (gSlot && !gSlot->lastFrameInGame()) {
      DEBUG_LOG(
          ("ConnectionManager::disconnectPlayer(%d) - slot is last in the game "
           "on frame %d\n",
           slot, TheGameLogic->getFrame()));
  ...
  if ((m_frameData[slot] != NULL) &&
      (m_frameData[slot]->getIsQuitting() == FALSE)) {
    DEBUG_LOG((
        "ConnectionManager::disconnectPlayer - deleting player %d frame data\n",
        slot));
  ...
  if (m_connections[slot] != NULL && !m_connections[slot]->isQuitting()) {
    DEBUG_LOG((
        "ConnectionManager::disconnectPlayer - deleting player %d connection\n",
        slot));
  ...
  if (slot == m_packetRouterSlot) {
    Int index = 0;
    while ((index < (MAX_SLOTS - 1)) &&
           (m_packetRouterFallback[index] != m_packetRouterSlot)) {
      ++index;
    }
    ++index;
  ...
  Int fallbackindex = 0;
  while ((fallbackindex < MAX_SLOTS) &&
         (m_packetRouterFallback[fallbackindex] != slot)) {
    ++fallbackindex;
  }
...
void ConnectionManager::quitGame() {
  ...
  NetDisconnectPlayerCommandMsg *disconnectMsg =
      newInstance(NetDisconnectPlayerCommandMsg);
  ...
  // DEBUG_LOG(("ConnectionManager::disconnectLocalPlayer - about to send
  // disconnect command\n"));
  sendLocalCommandDirect(disconnectMsg, 0xff ^ (1 << m_localSlot));

  // DEBUG_LOG(("ConnectionManager::disconnectLocalPlayer - about to flush
  // connections\n"));
  flushConnections();  // need to do this so our packet actually gets sent
                       // before the connections are deleted.
  // DEBUG_LOG(("ConnectionManager::disconnectLocalPlayer - done flushing
  // connections\n"));

  disconnectMsg->detach();

  // if we get here, we hit Quit on the disconnect screen.  Mark everyone as
  // having disconnected from us so the online stats can give us appropriate
  // feedback.
  if (TheGameInfo) {
    for (Int i = 0; i < MAX_SLOTS; ++i) {
...
void ConnectionManager::flushConnections() {
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if (m_connections[i] != NULL) {
      //			DEBUG_LOG(("ConnectionManager::flushConnections
      //- flushing connection to player %d\n", i));
      /*
...
void ConnectionManager::resendPendingCommands() {
  ...
  while (ref != NULL) {
    // DEBUG_LOG(("ConnectionManager::resendPendingCommands - resending command
...
UnsignedInt ConnectionManager::getLocalPlayerID() { return m_localSlot; }

UnicodeString ConnectionManager::getPlayerName(Int playerNum) {
  UnicodeString retval;
  if (playerNum == m_localSlot) {
    retval = m_localUser->GetName();
  } else if (((m_connections[playerNum] != NULL) &&
              (m_connections[playerNum]->isQuitting() == FALSE))) {
...
/**
 * Take a user list and make connections and frame data manager objects for each
 * of the players. For now, this is also how we'll determine the packet router
 * fallback plan.
 */
void ConnectionManager::parseUserList(const GameInfo *game) {
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    const GameSlot *slot = game->getConstSlot(
        i);  // badness, but since we cast right back to const, we should be ok
    if (slot->isHuman()) {
  ...
  {
          DEBUG_CRASH(("FAILED parseUserList - network game won't work as
expected\n")); return;
  }
  ...
  DEBUG_LOG(("ConnectionManager::parseUserList - looking for local user at
%d.%d.%d.%d:%d\n", (m_localAddr >> 24) & 0xff, (m_localAddr >> 16) & 0xff,
          (m_localAddr >> 8) & 0xff,
          m_localAddr & 0xff,
          m_localPort));
  ...
  while ( (userStr=strtok_r(listPtr, ",", &listPos)) != NULL )
  {
          ...
          {
                  DEBUG_LOG(("ConnectionManager::parseUserList - (numUsers = %d)
FAILED parseUserList with list [%s]\n", numUsers, buf)); return;
          }
...
//		if ((m_localAddr != addr) || (m_localPort != port)) {
          if (loginName.compare(nameStr) != 0) {
                  ...
                  m_connections[numUsers]->setUser(newInstance(User)(nameStr,
addr, port));

                  m_frameData[numUsers] = newInstance(FrameDataManager)(FALSE);

                  DEBUG_LOG(("ConnectionManager::parseUserList - User %d is
%s\n", numUsers, nameStr)); } else { m_localSlot = numUsers;
...
%s\n", numUsers, nameStr)); DEBUG_LOG(("Local user is %d\n", m_localSlot));
  ...
  if (numUsers < 2 || m_localSlot == -1)
  {
          DEBUG_LOG(("ConnectionManager::parseUserList - FAILED (local user =
%d, num players = %d) with list [%s]\n", m_localSlot, numUsers, buf)); return;
  }
...
/**
 * Return the number of incoming packets per second averaged over the last 30
 * sec.
 */
...
/**
 * Return the number of outgoing packets per second averaged over the last 30
 * sec.
 */
...
/**
 * Return the number of bytes not from generals clients received per second
 * averaged over the last 30 sec.
 */
...
/**
 * Return the number ov packets not from generals clients received per second
 * averaged over the last 30 sec.
 */
...
void ConnectionManager::sendChat(UnicodeString text, Int playerMask,
  ...
  DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                  ("Chat message has ID of %d, mask of %8.8X, text of %ls\n",
...
UnsignedShort ConnectionManager::sendFileAnnounce(AsciiString path,
  ...
  if (!theFile || !theFile->size()) {
    ...
    if (TheLAN)
      TheLAN->OnChat(UnicodeString(L"sendFile"), 0, log,
  ...
  NetFileAnnounceCommandMsg *announceMsg =
      newInstance(NetFileAnnounceCommandMsg);
  ...
  DEBUG_LOG_LEVEL(
      DEBUG_LEVEL_NET,
      ("ConnectionManager::sendFileAnnounce() - creating announce message with "
       "ID of %d from %d to mask %X for '%s' going to %X as command %d\n",
       announceMsg->getID(), announceMsg->getPlayerID(), announceMask,
       announceMsg->getRealFilename().str(), announceMsg->getPlayerMask(),
       announceMsg->getFileID()));

  processFileAnnounce(announceMsg);  // set up things for the host

  DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                  ("Sending file announce to %X\n", announceMask));
...
void ConnectionManager::sendFile(AsciiString path, UnsignedByte playerMask,
                                 UnsignedShort commandID) {
  File *theFile = TheLocalFileSystem->openFile(path.str());
  if (!theFile || !theFile->size()) {
    UnicodeString log;
  ...
  Int compressedLen =
      path.endsWith(".tga")
          ? CompressionManager::getMaxCompressedSize(
                len, CompressionManager::getPreferredCompression())
          : 0;
  Int compressedSize = 0;
  if (compressedLen)
    compressedSize = CompressionManager::compressData(
        CompressionManager::getPreferredCompression(), buf, len, compressedBuf,
        compressedLen);
  ...
  if (compressedBuf) {
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("Compressed '%s' from %d to %d (%g%%) before transfer\n", path.str(),
         len, compressedSize, (Real)compressedSize / (Real)len * 100.0f));
  ...
  DEBUG_LOG_LEVEL(
      DEBUG_LEVEL_NET,
      ("ConnectionManager::sendFile() - creating file message with ID of %d "
       "for '%s' going to %X from %d, size of %d\n",
       fileMsg->getID(), fileMsg->getRealFilename().str(), playerMask,
       fileMsg->getPlayerID(), fileMsg->getFileLength()));
...
Int ConnectionManager::getFileTransferProgress(Int playerID, AsciiString path) {
  ...
  while (commandIt != s_fileCommandMap.end()) {
    // DEBUG_LOG(("ConnectionManager::getFileTransferProgress(%s): looking at
    // existing transfer of '%s'\n", 	path.str(), commandIt->second.str()));
  ...
  DEBUG_LOG_LEVEL(
      DEBUG_LEVEL_NET,
      ("ConnectionManager::getFileTransferProgress: path %s not found\n",
       path.str()));
  return 0;
}
...
void ConnectionManager::debugPrintConnectionCommands() {
  DEBUG_LOG_LEVEL(
      DEBUG_LEVEL_NET,
      ("ConnectionManager::debugPrintConnectionCommands - begin commands\n"));
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if (m_connections[i] != NULL) {
      DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                      ("ConnectionManager::debugPrintConnectionCommands - "
                       "commands for connection %d\n",
                       i));
      m_connections[i]->debugPrintCommands();
    }
  }
  DEBUG_LOG_LEVEL(
      DEBUG_LEVEL_NET,
      ("ConnectionManager::debugPrintConnectionCommands - end commands\n"));
}
#endif
...
void ConnectionManager::notifyOthersOfCurrentFrame(Int frame) {
  ...
  DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                  ("ConnectionManager::notifyOthersOfCurrentFrame - sending "
                   "disconnect frame of %d, command ID = %d\n",
                   frame, msg->getID()));
  sendLocalCommandDirect(msg, 0xff ^ (1 << m_localSlot));
  ...
  DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                  ("ConnectionManager::notifyOthersOfCurrentFrame - start "
                   "screen on debug stuff\n"));
#if defined(_DEBUG) || defined(_INTERNAL)
...
void ConnectionManager::notifyOthersOfNewFrame(UnsignedInt frame) {
  NetDisconnectScreenOffCommandMsg *msg =
      newInstance(NetDisconnectScreenOffCommandMsg);
...
void ConnectionManager::sendFrameDataToPlayer(UnsignedInt playerID,
  ...
  DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                  ("ConnectionManager::sendFrameDataToPlayer - sending frame "
                   "data to player %d starting with frame %d\n",
                   playerID, startingFrame));
  for (UnsignedInt frame = startingFrame; frame < TheGameLogic->getFrame();
       ++frame) {
    sendSingleFrameToPlayer(playerID, frame);
  }
  DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("ConnectionManager::sendFrameDataToPlayer "
                                    "- done sending commands to player %d\n",
                                    playerID));
}
...
void ConnectionManager::sendSingleFrameToPlayer(UnsignedInt playerID,
  ...
  if ((TheGameLogic->getFrame() - FRAMES_TO_KEEP) > frame) {
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("ConnectionManager::sendSingleFrameToPlayer - player %d requested "
         "frame %d when we are on frame %d, this is too far in the past.\n",
  ...
  DEBUG_LOG_LEVEL(
      DEBUG_LEVEL_NET,
      ("ConnectionManager::sendFrameDataToPlayer - sending data for frame %d\n",
       frame));
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if ((m_frameData[i] != NULL) &&
        (i != playerID)) {  // no need to send his own commands to him.
      ...
      if (list != NULL) {
        ...
        while (ref != NULL) {
          DEBUG_LOG_LEVEL(
              DEBUG_LEVEL_NET,
              ("ConnectionManager::sendFrameDataToPlayer - sending command %d "
               "from player %d to player %d using relay 0x%x\n",
               ref->getCommand()->getID(), i, playerID, relay));
          sendLocalCommandDirect(ref->getCommand(), relay);
          ref = ref->getNext();
        }
      }
      UnsignedInt frameCommandCount =
          m_frameData[i]->getFrameCommandCount(frame);
      NetFrameCommandMsg *msg = newInstance(NetFrameCommandMsg);
      msg->setExecutionFrame(frame);
      ...
      msg->setPlayerID(i);
      DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET,
                      ("ConnectionManager::sendFrameDataToPlayer - sending "
                       "frame info from player %d to player %d for frame %d "
                       "with command count %d and ID %d and relay %d\n",
                       i, playerID, msg->getExecutionFrame(),
                       msg->getCommandCount(), msg->getID(), relay));
      sendLocalCommandDirect(msg, relay);
      msg->detach();
    }
  }
...
UnsignedInt ConnectionManager::getNextPacketRouterSlot(UnsignedInt playerID) {
  Int index = 0;
  while ((index < (MAX_SLOTS - 1)) &&
         (m_packetRouterFallback[index] != playerID)) {
    ++index;
  }
  ++index;
...
void ConnectionManager::requestFrameDataResend(Int playerID,
                                               UnsignedInt frame) {
  NetFrameResendRequestCommandMsg *msg =
      newInstance(NetFrameResendRequestCommandMsg);
  msg->setPlayerID(m_localSlot);
