...
// ////////////////////////////////////////////////////////////////// Author:
// Matthew D. Campbell, May 2002 Desc:   Tree sway client update module
///////////////////////////////////////////////////////////////////////////////////////////////////
...
{
  // don't do updateSway here; wait till the first time we go thru our update
  // loop.
  // updateSway();
}
...
void SwayClientUpdate::updateSway() {
  ...
  m_curAngleLimit =
      info.m_intensity * GameClientRandomValueReal(1.0f - delta, 1.0f + delta);
  m_curDelta = 2 * PI / info.m_breezePeriod *
               GameClientRandomValueReal(1.0f - delta, 1.0f + delta);
  m_leanAngle =
      info.m_lean * GameClientRandomValueReal(1.0f - delta, 1.0f + delta);
...
void SwayClientUpdate::clientUpdate(void) {
  ...
  if (info.m_breezeVersion != m_curVersion) {
    ...
    if (!draw || !draw->isVisible()) return;
  }

  m_curValue += m_curDelta;
  if (m_curValue > 2 * PI) m_curValue -= 2 * PI;
  ...
  if (obj && (obj->getStatusBits() & OBJECT_STATUS_BURNED) != 0) stopSway();
}
