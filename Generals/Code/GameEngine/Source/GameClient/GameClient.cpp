...
GameClient::~GameClient() {
  ...
  // ------------------------------------------\n")); for (Int oog=0;
  // oog<preloadTextureNamesGlobalHack2.size(); ++oog)
  //{
  ...
  // ------------------------------------------------\n"));
  if (TheCampaignManager) delete TheCampaignManager;
...
void GameClient::init(void) {
  setFrameRate(MSEC_PER_LOGICFRAME_REAL);  // from GameCommon.h... tell W3D what
                                           // our expected framerate is
  ...
  if (TheGlobalLanguageData &&
      TheGlobalLanguageData->m_drawGroupInfoFont.name.isNotEmpty()) {
    TheDrawGroupInfo->m_fontName =
        TheGlobalLanguageData->m_drawGroupInfoFont.name;
    TheDrawGroupInfo->m_fontSize =
        TheGlobalLanguageData->m_drawGroupInfoFont.size;
    TheDrawGroupInfo->m_fontIsBold =
        TheGlobalLanguageData->m_drawGroupInfoFont.bold;
  }
  ...
  // allocate and load image collection for the GUI and just load the 256x256
  // ones for now
  TheMappedImageCollection = MSGNEW("GameClientSubsystem") ImageCollection;
  TheMappedImageCollection->load(512);

  // now that we have all the images loaded ... load any animation definitions
  // from those images
  ...
  if (TheMessageStream) {
    //
    // NOTE: Make sure m_translators[] is large enough to accomodate all the
    // translators you are loading here.  See MAX_CLIENT_TRANSLATORS
    //
    ...
    m_translators[m_numTranslators++] = TheMessageStream->attachTranslator(
        MSGNEW("GameClientSubsystem") WindowTranslator, 10);
    m_translators[m_numTranslators++] = TheMessageStream->attachTranslator(
        MSGNEW("GameClientSubsystem") MetaEventTranslator, 20);
    m_translators[m_numTranslators++] = TheMessageStream->attachTranslator(
        MSGNEW("GameClientSubsystem") HotKeyTranslator, 25);
    m_translators[m_numTranslators++] = TheMessageStream->attachTranslator(
        MSGNEW("GameClientSubsystem") PlaceEventTranslator, 30);
    m_translators[m_numTranslators++] = TheMessageStream->attachTranslator(
        MSGNEW("GameClientSubsystem") GUICommandTranslator, 40);
    m_translators[m_numTranslators++] = TheMessageStream->attachTranslator(
        MSGNEW("GameClientSubsystem") SelectionTranslator, 50);
    m_translators[m_numTranslators++] = TheMessageStream->attachTranslator(
        MSGNEW("GameClientSubsystem") LookAtTranslator, 60);
    m_translators[m_numTranslators] = TheMessageStream->attachTranslator(
        MSGNEW("GameClientSubsystem") CommandTranslator, 70);
    // we keep a pointer to the command translator because it's useful
    m_commandTranslator = (CommandTranslator *)TheMessageStream->findTranslator(
        m_translators[m_numTranslators++]);
    m_translators[m_numTranslators++] = TheMessageStream->attachTranslator(
        MSGNEW("GameClientSubsystem") HintSpyTranslator, 100);

    //
    // the client message translator should probably remain as the last reaction
    // of the client before the messages are given to the network for
    // processing.  This lets all systems in the client give events that can be
    // processed by the client message translator
    //
    m_translators[m_numTranslators++] = TheMessageStream->attachTranslator(
        MSGNEW("GameClientSubsystem") GameClientMessageDispatcher, 999999999);
  }

  // create the font library
  TheFontLibrary = createFontLibrary();
  if (TheFontLibrary) TheFontLibrary->init();
  ...
  TheHeaderTemplateManager =
      MSGNEW("GameClientSubsystem") HeaderTemplateManager;
...
void GameClient::registerDrawable(Drawable *draw) {
  // assign this drawable a unique ID, this will add it to the fast lookup table
  // too
...
void GameClient::update(void) {
  ...
  GameMessage *frameMsg =
      TheMessageStream->appendMessage(GameMessage::MSG_FRAME_TICK);
  ...
  // Initial Game Codition.  We must show the movie first and then we can
  // display the shell
  if (TheGlobalData->m_afterIntro && !TheDisplay->isMoviePlaying()) {
    if (playSizzle) {
      ...
      if (TheGameLODManager && !TheGameLODManager->didMemPass()) {
        ...
        WindowLayout *legal =
            TheWindowManager->winCreateLayout("Menus/LegalPage.wnd");
        if (legal) {
          legal->hide(FALSE);
          legal->bringForward();
  ...
  if (!freezeTime) {
...
#endif
    {
      // localPlayerIndex=TheGhostObjectManager->getLocalPlayerIndex();	//always
      // use the first local player set since normally can't change.  Doesn't
      // work with debug "CTRL_SHIFT_SPACE"
      ...
      for (Int i = 0; i < numPlayers; i++) {
        ...
        if (player->getPlayerIndex() != localPlayerIndex)
          nonLocalPlayerIndices[numNonLocalPlayers++] =
              player->getPlayerIndex();
      }
      // update ghostObjects which don't have drawables or objects.
      TheGhostObjectManager->updateOrphanedObjects(nonLocalPlayerIndices,
                                                   numNonLocalPlayers);
#else
    ...
    while (draw) {  // update() could free the Drawable, so go ahead and grab
                    // 'next'
        ...
        if (object) {
          ...
          if (ss >= OBJECTSHROUD_FOGGED && draw->getShroudClearFrame() != 0) {
            ...
            if (TheGameLogic->getFrame() <
                limit + draw->getShroudClearFrame()) {
  ...
  // need to draw the first frame, then don't draw again until
  // TheGlobalData->m_noDraw
  if (TheGlobalData->m_noDraw > TheGameLogic->getFrame() &&
...
/** -----------------------------------------------------------------------------------------------
 * Call the given callback function for each object contained within the given
 * region.
 */
void GameClient::iterateDrawablesInRegion(Region3D *region,
  ...
  for (draw = m_drawableList; draw; draw = nextDrawable) {
    ...
    if (region == NULL || (pos.x >= region->lo.x && pos.x <= region->hi.x &&
 ...
 * This method is the primary interface for accessing objects, and should be
 * used instead of pointers to "attach" objects to each other.
 */
...
void GameClient::destroyDrawable(Drawable *draw) {
  ...
  //
  // because drawables and objects are tightly coupled, not only MUST we
  // maintain our links in all instances, but it is NECESSARY for the client to
  // actually modify data in the logic, that is the pointer in an object to
  // *this* drawable
  //
  Object *obj = draw->getObject();
  if (obj) {
    DEBUG_ASSERTCRASH(obj->getDrawable() == draw,
                      ("Object/Drawable pointer mismatch!\n"));
...
void GameClient::selectDrawablesInGroup(Int group) {
  /*
          ...
          // create a message that will assign a group ID to all the selected
     drawables, this
          // way when we do things with this current selected group of units, we
     only have
     ...
     GameMessage::MSG_CREATE_SELECTED_GROUP );
          ...
          while( draw )
          {
                  ...
                  if( object && draw->getDrawableGroup() == group &&
     object->isLocallyControlled() && !object->isContained() )
                  {
                          //Only select the object if it is locally controlled
     and not contained by anything. TheInGameUI->selectDrawable(draw);
                          teamMsg->appendObjectIDArgument(
     draw->getObject()->getID() );
                  }
                  else
                  {
...
void GameClient::addTextBearingDrawable(Drawable *tbd) {
  if (tbd != NULL) m_textBearingDrawableList.push_back(tbd);
}
// ------------------------------------------------------------------------------------------------
void GameClient::flushTextBearingDrawables(void) {
  ...
  for (TextBearingDrawableListIterator it = m_textBearingDrawableList.begin();
       it != m_textBearingDrawableList.end(); ++it)
    (*it)->drawUIText();
...
GameMessage::Type GameClient::evaluateContextCommand(
    Drawable *draw, const Coord3D *pos,
...
//-------------------------------------------------------------------------------------------------
/** Preload assets for the currently loaded map.  Those assets include all the
 * damage states for every building loaded, as well as any faction
 * units/structures we can build and all their damage states */
//-------------------------------------------------------------------------------------------------
void GameClient::preloadAssets(TimeOfDay timeOfDay) {
  MEMORYSTATUS before, after;
  GlobalMemoryStatus(&before);

  // first, for every drawable in the map load the assets for all states we care
  // about
  Drawable *draw;
  for (draw = firstDrawable(); draw; draw = draw->getNextDrawable())
  ...
  //
  // now create a temporary drawble for each of the faction things we can
  // create, preload their assets, and dump the drawable
  //
  AsciiString side;
  ...
  for (tTemplate = TheThingFactory->firstTemplate(); tTemplate;
    ...
    if (tTemplate->isKindOf(KINDOF_PRELOAD) == FALSE &&
        !TheGlobalData->m_preloadEverything)
      continue;
  ...
  DEBUG_LOG(("Preloading memory dwAvailPageFile %d --> %d : %d\n",
             before.dwAvailPageFile, after.dwAvailPageFile,
             before.dwAvailPageFile - after.dwAvailPageFile));
  DEBUG_LOG(("Preloading memory dwAvailPhys     %d --> %d : %d\n",
             before.dwAvailPhys, after.dwAvailPhys,
             before.dwAvailPhys - after.dwAvailPhys));
  DEBUG_LOG(("Preloading memory dwAvailVirtual  %d --> %d : %d\n",
             before.dwAvailVirtual, after.dwAvailVirtual,
             before.dwAvailVirtual - after.dwAvailVirtual));
  /*
  ...
  DEBUG_LOG(("Preloading memory dwMemoryLoad    %d --> %d : %d\n",
          before.dwMemoryLoad, after.dwMemoryLoad, before.dwMemoryLoad -
  after.dwMemoryLoad)); DEBUG_LOG(("Preloading memory dwTotalPageFile %d --> %d
  : %d\n", before.dwTotalPageFile, after.dwTotalPageFile, before.dwTotalPageFile
  - after.dwTotalPageFile)); DEBUG_LOG(("Preloading memory dwTotalPhys     %d
  --> %d : %d\n", before.dwTotalPhys , after.dwTotalPhys, before.dwTotalPhys -
  after.dwTotalPhys)); DEBUG_LOG(("Preloading memory dwTotalVirtual  %d --> %d :
  %d\n", before.dwTotalVirtual , after.dwTotalVirtual, before.dwTotalVirtual -
  after.dwTotalVirtual));
  */
  ...
  char *textureNames[] = {"ptspruce01.tga",
                          "exrktflame.tga",
                          "cvlimo3_d2.tga",
...
GameClient::DrawableTOCEntry *GameClient::findTOCEntryByName(AsciiString name) {
  for (DrawableTOCListIterator it = m_drawableTOC.begin();
       it != m_drawableTOC.end(); ++it)
...
static Bool shouldSaveDrawable(const Drawable *draw) {
  if (draw->testDrawableStatus(DRAWABLE_STATUS_NO_SAVE)) {
    if (draw->getObject() == NULL) {
      ...
      DEBUG_CRASH(
          ("You should not ever set DRAWABLE_STATUS_NO_SAVE for a Drawable "
           "with an object. (%s)\n",
...
void GameClient::xferDrawableTOC(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (Drawable *draw = getDrawableList(); draw;
         draw = draw->getNextDrawable()) {
      if (!shouldSaveDrawable(draw)) continue;
 ...
 * 3: Added block markers around drawable data, no version checking is done and
 *therefore this version breaks compatibility with previous versions. (CBD)
 */
// ------------------------------------------------------------------------------------------------
void GameClient::xfer(Xfer *xfer) {
  ...
  //
  // !!!DON'T DO THIS!!! ----> xfer->xferDrawableID( &m_nextDrawableID ); <----
  // !!!DON'T DO THIS!!!
  ...
  for (draw = getDrawableList(); draw; draw = draw->getNextDrawable()) {
    if (xfer->getXferMode() == XFER_SAVE && !shouldSaveDrawable(draw)) continue;
    drawableCount++;
  }
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (draw = getDrawableList(); draw; draw = draw->getNextDrawable()) {
      ...
      if (tocEntry == NULL) {
        DEBUG_CRASH(
            ("GameClient::xfer - Drawable TOC entry not found for '%s'\n",
             draw->getTemplate()->getName().str()));
    ...
    for (UnsignedShort i = 0; i < drawableCount; ++i) {
      ...
      if (tocEntry == NULL) {
        DEBUG_CRASH(
            ("GameClient::xfer - No TOC entry match for id '%d'\n", tocID));
        throw SC_INVALID_DATA;
      ...
      if (thingTemplate == NULL) {
        DEBUG_CRASH(
            ("GameClient::xfer - Unrecognized thing template '%s', skipping.  "
             "ENGINEERS - Are you *sure* it's OK to be ignoring this object "
             "from the save file???  Think hard about it!\n",
             tocEntry->name.str()));
        xfer->skip(dataSize);
        continue;
      ...
      //
      // if we have an attached object ID, we won't create a new drawable, we'll
      // use the one that has been created and attached to the object already
      //
      if (objectID != INVALID_ID) {
        ...
        if (object == NULL) {
          DEBUG_CRASH(
              ("GameClient::xfer - Cannot find object '%d' that is supposed to "
               "be attached to this drawable '%s'\n",
        ...
        if (draw == NULL) {
          DEBUG_CRASH(
              ("GameClient::xfer - There is no drawable attached to the object "
        ...
        // srj sez: some objects (eg, diguised bombtrucks) may have an
        // "abnormal" drawable. so check.
        //
        // note carefully: we do NOT want to use isEquivalentTo() here, because
        // different object reskins SHOULD count as different templates for our
        // purposes here (which are purely visual). however, we do need to
        // compare getFinalOverride, because draw->getTemplate() is always gonna
        // return the final override, while TheThingFactory->findTemplate does
        // not.
        //
        const ThingTemplate *drawTemplate = draw->getTemplate();
        if (drawTemplate->getFinalOverride() !=
        ...
        //
        // there was no object attached to this drawable when we saved, we need
        // to create a whole brand new drawable now
        //
        draw = TheThingFactory->newDrawable(thingTemplate);
        ...
        if (draw == NULL) {
          DEBUG_CRASH(
              ("GameClient::xfer - Unable to create drawable for '%s'\n",
  ...
  if (version >= 2) {
    if (xfer->getXferMode() == XFER_SAVE) {
      ...
      for (BriefingList::const_iterator bIt = bList->begin();
           bIt != bList->end(); ++bIt) {
    ...
    } else  // XFER_LOAD
    {
      ...
      UpdateDiplomacyBriefingText(AsciiString::TheEmptyString,
                                  TRUE);  // clear out briefing list first
      while (numEntries-- > 0) {
        AsciiString tempStr;
        xfer->xferAsciiString(&tempStr);
        DEBUG_LOG(("'%s'\n", tempStr.str()));
...
void GameClient::loadPostProcess(void) {
  //
  // due to the fact that during the load process we have called newDrawable for
  // drawables without objects, and then overwrote their ids with data from the
  // save file, our allocater id may be far higher than it needs to be.  We'll
  // pull it back down as low as we can
  //
  Drawable *draw;
  for (draw = getDrawableList(); draw; draw = draw->getNextDrawable())
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void GameClient::crc(Xfer *xfer) {}  // end crc
