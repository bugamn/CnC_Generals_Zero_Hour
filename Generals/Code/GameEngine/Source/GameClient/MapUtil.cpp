...
static std::vector<ICoord2D>
    m_boundaries;           ///< All the boundaries we use for the map
static Int m_dataSize = 0;  ///< size of m_data.
static UnsignedByte *m_data =
    0;  ///< array of z(height) values in the height map.
...
static Bool ParseObjectDataChunk(DataChunkInput &file, DataChunkInfo *info,
                                 void *userData) {
  ...
  if (pThisOne->getProperties()->getType(TheKey_waypointID) == Dict::DICT_INT) {
    ...
    (*m_waypoints)[pThisOne->getWaypointName()] = loc;
             ...
             pThisOne->getThingTemplate()->isKindOf(KINDOF_TECH_BUILDING)) {
    m_techPositions.push_back(loc);
  } else if (pThisOne->getThingTemplate() &&
             pThisOne->getThingTemplate()->isKindOf(
...
static Bool ParseObjectsDataChunk(DataChunkInput &file, DataChunkInfo *info,
                                  void *userData) {
...
static Bool ParseWorldDictDataChunk(DataChunkInput &file, DataChunkInfo *info,
                                    void *userData) {
  worldDict = file.readDict();
  return true;
}

static Bool ParseSizeOnly(DataChunkInput &file, DataChunkInfo *info,
                          void *userData) {
...
static Bool ParseSizeOnlyInChunk(DataChunkInput &file, DataChunkInfo *info,
                                 void *userData) {
...
AsciiString MapCache::getMapDir() const { return AsciiString("Maps"); }
...
AsciiString MapCache::getMapExtension() const { return AsciiString("map"); }
  ...
  while (it != end()) {
    if (it->first.startsWithNoCase(mapDir.str())) {
      ...
      fprintf(fp, "  extentMin = X:%2.2f Y:%2.2f Z:%2.2f\n", md.m_extent.lo.x,
              md.m_extent.lo.y, md.m_extent.lo.z);
      fprintf(fp, "  extentMax = X:%2.2f Y:%2.2f Z:%2.2f\n", md.m_extent.hi.x,
              md.m_extent.hi.y, md.m_extent.hi.z);
...
void MapCache::updateCache(void) {
  ...
  loadStandardMaps();  // we shall overwrite info from matching user maps to
                       // prevent munkees from getting rowdy :)
...
Bool MapCache::loadUserMaps() {
  ...
  while (iter != filenameList.end()) {
    ...
    if (!s) {
      ...
      for (Int i = 0; i < strlen(mapExtension); ++i) fname.removeLastChar();
      ...
      if (!skipMap) {
        if (!tempfilename.endsWithNoCase(endingStr.str())) {
          DEBUG_CRASH(("Found map '%s' in wrong spot (%s)", fname.str(),
                       tempfilename.str()));
        } else {
  ...
  if (clearUnseenMaps(mapDir)) return TRUE;

  return parsedAMap;
}

// Bool MapCache::addMap( AsciiString dirName, AsciiString fname, WinTimeStamp
// timestamp, UnsignedInt filesize, Bool isOfficial )
Bool MapCache::addMap(AsciiString dirName, AsciiString fname,
  ...
  if (it != end()) {
    ...
    if ((md.m_filesize == filesize) && (md.m_CRC != 0)) {
      //			DEBUG_LOG(("MapCache::addMap - found match for
      //map %s\n", lowerFname.str()));
      return FALSE;  // OK, it checks out.
    }
    DEBUG_LOG(("%s didn't match file in MapCache\n", fname.str()));
    DEBUG_LOG(("size: %d / %d\n", filesize, md.m_filesize));
    DEBUG_LOG(("time1: %d / %d\n", fileInfo->timestampHigh,
               md.m_timestamp.m_highTimeStamp));
    DEBUG_LOG(("time2: %d / %d\n", fileInfo->timestampLow,
               md.m_timestamp.m_lowTimeStamp));
    //		DEBUG_LOG(("size: %d / %d\n", filesize, md.m_filesize));
    //		DEBUG_LOG(("time1: %d / %d\n", timestamp.m_highTimeStamp,
    //md.m_timestamp.m_highTimeStamp)); 		DEBUG_LOG(("time2: %d / %d\n",
  ...
  loadMap(fname);  // Just load for querying the data, since we aren't playing
                   // this map.
  ...
  if (!exists || munkee.isEmpty()) {
    ...
    for (Int i = 0; i < 4; ++i) stringFileName.removeLastChar();
  ...
  DEBUG_LOG(("  extent = (%2.2f,%2.2f) -> (%2.2f,%2.2f)\n", md.m_extent.lo.x,
             md.m_extent.lo.y, md.m_extent.hi.x, md.m_extent.hi.y));
...
//-------------------------------------------------------------------------------------------------
Int populateMapListboxNoReset(GameWindow *listbox, Bool useSystemMaps,
                              Bool isMultiplayer, AsciiString mapToSelect) {
  ...
  typedef std::set<UnicodeString, rts::less_than_nocase<UnicodeString> >
      MapNameList;
  ...
  while (numMapsListed < TheMapCache->size()) {
    ...
    while (tempit != tempCache.end()) {
      ...
      {
              DEBUG_LOG(("populateMapListbox(): looking at %s (displayName =
      %ls), mp = %d (== %d?) mapDir=%s (ok=%d)\n", it->first.str(),
      it->second.m_displayName.str(), it->second.m_isMultiplayer, isMultiplayer,
                      mapDir.str(), it->first.startsWith(mapDir.str())));
      }
      */

      DEBUG_ASSERTCRASH(it != TheMapCache->end(),
                        ("Map %s not found in map cache.", *tempit));
      if (it->first.startsWithNoCase(mapDir.str()) &&
          isMultiplayer == it->second.m_isMultiplayer &&
          !it->second.m_displayName.isEmpty()) {
        /// @todo: mapDisplayName =
        /// TheGameText->fetch(it->second.m_displayName.str());
        mapDisplayName = it->second.m_displayName;
        Int index = -1;
        Int imageItemData = -1;
        if (numColumns > 1 && it->second.m_isMultiplayer) {
          Int numEasy =
              battleHonors->getEnduranceMedal(it->first.str(), SLOT_EASY_AI);
          Int numMedium =
              battleHonors->getEnduranceMedal(it->first.str(), SLOT_MED_AI);
          Int numBrutal =
              battleHonors->getEnduranceMedal(it->first.str(), SLOT_BRUTAL_AI);
          if (numBrutal) {
            imageItemData = 3;
            index = GadgetListBoxAddEntryImage(listbox, brutalImage, index, 0,
                                               w, h, TRUE);
          } else if (numMedium) {
            imageItemData = 2;
            index = GadgetListBoxAddEntryImage(listbox, mediumImage, index, 0,
                                               w, h, TRUE);
          } else if (numEasy) {
            imageItemData = 1;
            index = GadgetListBoxAddEntryImage(listbox, easyImage, index, 0, w,
                                               h, TRUE);
          } else {
            imageItemData = 0;
            index =
                GadgetListBoxAddEntryImage(listbox, NULL, index, 0, w, h, TRUE);
          }
        }
        index = GadgetListBoxAddEntryText(listbox, mapDisplayName, color, index,
                                          numColumns - 1);
        ...
        // now set the char* as the item data.  this works because the map cache
        // isn't being modified while a map listbox is up.
  ...
  if (selectionIndex >= 0) {
    ...
    if (selectionIndex >= bottomIndex) {
      ...
      // The trouble is that rowsonscreen/2 can be zero if bottom is 1 and top
      // is zero
...
/** Load the listbox with all the map files available to play */
//-------------------------------------------------------------------------------------------------
Int populateMapListbox(GameWindow *listbox, Bool useSystemMaps,
                       Bool isMultiplayer, AsciiString mapToSelect) {
  if (!TheMapCache) return -1;
  ...
  return populateMapListboxNoReset(listbox, useSystemMaps, isMultiplayer,
                                   mapToSelect);
}
...
AsciiString getDefaultMap(Bool isMultiplayer) {
  if (!TheMapCache) return AsciiString::TheEmptyString;
  TheMapCache->updateCache();
...
// ------------------------------------------------------------------------------------------------
static void copyFromBigToDir(const AsciiString &infile,
                             const AsciiString &outfile) {
  // open the map file

  File *file = TheFileSystem->openFile(infile.str(), File::READ | File::BINARY);
  if (file == NULL) {
    DEBUG_CRASH(
        ("copyFromBigToDir - Error opening source file '%s'\n", infile.str()));
  ...
  if (file->read(buffer, fileSize) < fileSize) {
    DEBUG_CRASH(
        ("copyFromBigToDir - Error reading from file '%s'\n", infile.str()));
    throw SC_INVALID_DATA;
  }  // end if
  // close the BIG file
  file->close();

  File *filenew = TheFileSystem->openFile(
      outfile.str(), File::WRITE | File::CREATE | File::BINARY);

  if (!filenew || filenew->write(buffer, fileSize) < fileSize) {
    DEBUG_CRASH(
        ("copyFromBigToDir - Error writing to file '%s'\n", outfile.str()));
    throw SC_INVALID_DATA;
  }  // end if
...
Image *getMapPreviewImage(AsciiString mapName) {
  ...
  if (!image) {
    if (!TheFileSystem->doesFileExist(tgaName.str())) return NULL;
    AsciiString mapPreviewDir;
    mapPreviewDir.format(MAP_PREVIEW_DIR_PATH,
  ...
  /*
          ...
          Region2D uv;
          mapPreviewImage =
  TheMappedImageCollection->findImageByName("MapPreview"); if(mapPreviewImage)
  ...
  // allocate our terrain texture
          ...
          if (theInputStream.open(AsciiString(mapName.str())))
          {
                  ...
                  DataChunkInput file( pStrm );
                  if (file.isValidFileType()) {	// Backwards compatible files
  aren't valid data chunk files.
                          // Read the waypoints.
                          file.registerParser( AsciiString("MapPreview"),
  AsciiString::TheEmptyString, parseMapPreviewChunk ); if (!file.parse(NULL)) {
                                  DEBUG_ASSERTCRASH(false,("Unable to read
  MapPreview info.")); mapPreviewImage->deleteInstance(); return NULL;
                          }
                  }
...
Bool parseMapPreviewChunk(DataChunkInput &file, DataChunkInfo *info,
                          void *userData) {
  /*
          ICoord2D size;
          ...
          surface = (TextureClass
     *)mapPreviewImage->getRawTextureData()->Get_Surface_Level();
          ...
          DEBUG_ASSERTCRASH(file.atEndOfChunk(), ("Unexpected data left
     over.")); DEBUG_LOG(("EndMapPreviewInfo\n")); REF_PTR_RELEASE(surface);
          return true;
  */
  return FALSE;
}
