...
//-------------------------------------------------------------------------------------------------
/** Clip a line to the region provided.  The source line runs from p1 to p2, and
 *is clipped using the clipRegion.  The resulting line goes from c1 to c2
 *
 * Return values:
...
// Subject 1.03
Bool IntersectLine2D(const Coord2D *a, const Coord2D *b, const Coord2D *c,
                     const Coord2D *d, Coord2D *intersection) {
  if (!a || !b || !c || !d) {
...
// determines whether a point lies within a rectangle. Doesnt' determine whether
// the shape is actually a rectangle or not.
Bool PointInsideRect2D(const Coord2D *bl, const Coord2D *tl, const Coord2D *br,
                       const Coord2D *tr, const Coord2D *inputPoint) {
...
Bool PointInsideRect3D(const Coord3D *bl, const Coord3D *tl, const Coord3D *br,
                       const Coord3D *tr, const Coord3D *inputPoint) {
...
// This function uses even-odd winding to determine whether a point is inside an
// area.
Bool PointInsideArea2D(const Coord2D *ptToTest, const Coord2D *area,
                       const Int numPointsInArea) {
  int numIntersections = 0;
  for (int i = 0; i < numPointsInArea; ++i) {
    if (IntersectLine2D(ptToTest, &reallyFarPoint, &area[i],
                        &area[(i + 1) % numPointsInArea])) {
      ++numIntersections;
    }
  }
...
// This function uses even-odd winding to determine whether a point is inside an
// area.
Bool PointInsideArea2D(const Coord3D *ptToTest, const Coord3D *area,
                       Int numPointsInArea) {
  int numIntersections = 0;
...
Bool Coord3DInsideRect2D(const Coord3D *inputPoint, const Coord2D *tl,
                         const Coord2D *br) {
...
void ScaleRect2D(Coord2D *tl, Coord2D *br, Real scaleFactor) {
  scaleFactor = scaleFactor - 1.0f;  // We are starting with tl,br, so scaling
                                     // it by 1 means adding 0 to it.
...
void ShortestDistancePointToSegment2D(const Coord2D *a, const Coord2D *b,
                                      const Coord2D *pt, Real *outDistance,
                                      Coord2D *outPosition, Real *outU) {
  if (!a || !b || !pt) {
    return;
  }
