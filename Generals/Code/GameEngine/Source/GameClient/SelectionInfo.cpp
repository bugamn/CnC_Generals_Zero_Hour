...
//-------------------------------------------------------------------------------------------------
/**
 * Given a list of currently selected things and a list of things that are
 * currently under the selection (pointer or drag), generate some useful
 * information about each.
 */
extern Bool contextCommandForNewSelection(
    const DrawableList *currentlySelectedDrawables,
    const DrawableList *newlySelectedDrawables, SelectionInfo *outSelectionInfo,
    Bool selectionIsPoint) {
  if (!(currentlySelectedDrawables && newlySelectedDrawables &&
        outSelectionInfo))
    return FALSE;
  ...
  DrawableListCIt it;
  for (it = currentlySelectedDrawables->begin();
       it != currentlySelectedDrawables->end(); ++it) {
    if (!(*it)) {
      continue;
    }
  ...
  for (it = newlySelectedDrawables->begin();
       it != newlySelectedDrawables->end(); ++it) {
    if (!(*it)) {
      continue;
    }
    ...
    if (TheActionManager->canPlayerGarrison(localPlayer, obj,
                                            CMD_FROM_PLAYER)) {
  ...
  DEBUG_ASSERTCRASH(outSelectionInfo->currentCountEnemies <= 1,
                    ("Selection bug. jkmcd"));
  DEBUG_ASSERTCRASH(outSelectionInfo->currentCountFriends <= 1,
                    ("Selection bug. jkmcd"));
  DEBUG_ASSERTCRASH(outSelectionInfo->currentCountCivilians <= 1,
                    ("Selection bug. jkmcd"));
  ...
  if (TheGlobalData->m_useAlternateMouse) {
    // context sensitive commands never apply when selecting in alternate mouse
    // mode
    return FALSE;
  }
  ...
  if (outSelectionInfo->currentCountMine > 0) {
    if (outSelectionInfo->newCountEnemies > 0) {
      if (outSelectionInfo->newCountEnemies == 1 && selectionIsPoint) {
        return TheGameClient->evaluateContextCommand(
                   newEnemy, newEnemy->getPosition(),
                   CommandTranslator::EVALUATE_ONLY) !=
    ...
    if (outSelectionInfo->newCountMine > 0) {
      if (outSelectionInfo->newCountMine == 1 && selectionIsPoint &&
          !TheInGameUI->isInPreferSelectionMode()) {
        return TheGameClient->evaluateContextCommand(
                   newMine, newMine->getPosition(),
                   CommandTranslator::EVALUATE_ONLY) !=
               GameMessage::MSG_INVALID;
      }
    ...
    if (outSelectionInfo->newCountFriends > 0) {
      if (outSelectionInfo->newCountFriends == 1 && selectionIsPoint) {
        return TheGameClient->evaluateContextCommand(
                   newFriendly, newFriendly->getPosition(),
                   CommandTranslator::EVALUATE_ONLY) !=
               GameMessage::MSG_INVALID;
      }
      return FALSE;
    ...
    if (outSelectionInfo->currentCountMineInfantry > 0 &&
        outSelectionInfo->newCountGarrisonableBuildings == 1) {
      return TRUE;
    }
    ...
    if (outSelectionInfo->newCountCivilians > 0) {
      if (outSelectionInfo->newCountCivilians == 1 && selectionIsPoint) {
        return TheGameClient->evaluateContextCommand(
                   newCivilian, newCivilian->getPosition(),
                   CommandTranslator::EVALUATE_ONLY) !=
               GameMessage::MSG_INVALID;
      }
      return FALSE;
...
UnsignedInt getPickTypesForContext(Bool forceAttackMode) {
  ...
  if (forceAttackMode) types |= PICK_TYPE_FORCEATTACKABLE;

  //
  // if we have a gui context command that allows for a shrubbery target then we
  // want to pick that type too (generally shrubbery aren't pickable cause it
  // would get in the way with movement and general selection)
  //
...
UnsignedInt getPickTypesForCurrentSelection(Bool forceAttackMode) {
  ...
  for (DrawableListCIt cit = allSelectedDrawables->begin();
       cit != allSelectedDrawables->end(); ++cit) {
    Drawable *draw = *cit;
    if (!draw) {
      continue;
    }
    ...
    // srj sez: thanks to new, area-effect disarming, we NO LONGER want to do
    // this...
...
//-------------------------------------------------------------------------------------------------
void translatePickTypesToKindof(UnsignedInt pickTypes,
                                KindOfMaskType &outMask) {
...
Bool addDrawableToList(Drawable *draw, void *userData) {
  ...
  if (!pds->drawableListToFill) return FALSE;

  if (!draw->getTemplate()->isAnyKindOf(pds->kindofsToMatch)) return FALSE;

  if (!draw->isSelectable()) return FALSE;
