...
void DrawableIconInfo::clear() {
  for (int i = 0; i < MAX_ICONS; ++i) {
    if (m_icon[i]) m_icon[i]->deleteInstance();
    m_icon[i] = NULL;
    m_keepTillFrame[i] = 0;
  }
}
...
static const char* drawableIconIndexToName(DrawableIconType iconIndex) {
  DEBUG_ASSERTCRASH(
      iconIndex >= ICON_FIRST && iconIndex < MAX_ICONS,
...
static DrawableIconType drawableIconNameToIndex(const char* iconName) {
  DEBUG_ASSERTCRASH(iconName != NULL,
                    ("drawableIconNameToIndex - Illegal name\n"));
...
/*static*/ void Drawable::initStaticImages() {
  if (s_staticImagesInited) return;
  ...
  s_animationTemplates[ICON_DEFAULT_HEAL] = TheAnim2DCollection->findTemplate(
      TheDrawableIconNames[ICON_DEFAULT_HEAL]);
  s_animationTemplates[ICON_STRUCTURE_HEAL] = TheAnim2DCollection->findTemplate(
      TheDrawableIconNames[ICON_STRUCTURE_HEAL]);
  s_animationTemplates[ICON_VEHICLE_HEAL] = TheAnim2DCollection->findTemplate(
      TheDrawableIconNames[ICON_VEHICLE_HEAL]);
#ifdef ALLOW_DEMORALIZE
  s_animationTemplates[ICON_DEMORALIZED] =
      TheAnim2DCollection->findTemplate(TheDrawableIconNames[ICON_DEMORALIZED]);
#endif
  s_animationTemplates[ICON_BOMB_TIMED] =
      TheAnim2DCollection->findTemplate(TheDrawableIconNames[ICON_BOMB_TIMED]);
  s_animationTemplates[ICON_BOMB_REMOTE] =
      TheAnim2DCollection->findTemplate(TheDrawableIconNames[ICON_BOMB_REMOTE]);
  s_animationTemplates[ICON_DISABLED] =
      TheAnim2DCollection->findTemplate(TheDrawableIconNames[ICON_DISABLED]);
  s_animationTemplates[ICON_BATTLEPLAN_BOMBARD] =
      TheAnim2DCollection->findTemplate(
          TheDrawableIconNames[ICON_BATTLEPLAN_BOMBARD]);
  s_animationTemplates[ICON_BATTLEPLAN_HOLDTHELINE] =
      TheAnim2DCollection->findTemplate(
          TheDrawableIconNames[ICON_BATTLEPLAN_HOLDTHELINE]);
  s_animationTemplates[ICON_BATTLEPLAN_SEARCHANDDESTROY] =
      TheAnim2DCollection->findTemplate(
          TheDrawableIconNames[ICON_BATTLEPLAN_SEARCHANDDESTROY]);
  s_animationTemplates[ICON_EMOTICON] =
      NULL;  // Emoticons can be anything, so we'll need to handle it
             // dynamically.
  s_animationTemplates[ICON_ENTHUSIASTIC] = TheAnim2DCollection->findTemplate(
      TheDrawableIconNames[ICON_ENTHUSIASTIC]);
  s_animationTemplates[ICON_ENTHUSIASTIC_SUBLIMINAL] =
      TheAnim2DCollection->findTemplate(
          TheDrawableIconNames[ICON_ENTHUSIASTIC_SUBLIMINAL]);
  s_animationTemplates[ICON_CARBOMB] =
      TheAnim2DCollection->findTemplate(TheDrawableIconNames[ICON_CARBOMB]);
...
//--- A MACRO TO APPLY TO TheTacticalView->getZoom() ------ To Clamp the return
//to a visually pleasing size
//--- so that icons, emoticons, health bars, pips, etc, look reasonably solid
//and don't shimmer or tweed #define CLAMP_ICON_ZOOM_FACTOR(n) (MAX(0.80f,
// MIN(1.00f, n)))
...
Drawable::Drawable(const ThingTemplate* thingTemplate,
  ...
  // register drawable with the GameClient ... do this first before we start
  // doing anything complex that uses any of the drawable data so that we have
  // and ID!!  It's ok to initialize members of the drawable before this
  ...
  m_constructDisplayString->setFont(
      TheFontLibrary->getFont(TheInGameUI->getDrawableCaptionFontName(),
                              TheGlobalLanguageData->adjustFontSize(
                                  TheInGameUI->getDrawableCaptionPointSize()),
  ...
  for (i = 0; i < NUM_DRAWABLE_MODULE_TYPES; ++i) m_modules[i] = NULL;
  ...
  // Real fuzzyScale = ( 1.0f + GameClientRandomValueReal( -scaleFuzziness,
  // scaleFuzziness ));
  ...
  m_modules[MODULETYPE_DRAW - FIRST_DRAWABLE_MODULE_TYPE] =
      MSGNEW("ModulePtrs") Module * [drawMI.getCount() + 1];  // pool[]ify
  ...
  for (modIdx = 0; modIdx < drawMI.getCount(); ++modIdx) {
    ...
    if (TheGlobalData->m_useDrawModuleLOD &&
        newModData->getMinimumRequiredGameLOD() >
            TheGameLODManager->getStaticLODLevel())
      continue;
    *m++ = TheModuleFactory->newModule(this, drawMI.getNthName(modIdx),
                                       newModData, MODULETYPE_DRAW);
  }
  *m = NULL;
  ...
  if (cuMI.getCount()) {
    ...
    m_modules[MODULETYPE_CLIENT_UPDATE - FIRST_DRAWABLE_MODULE_TYPE] =
        MSGNEW("ModulePtrs") Module * [cuMI.getCount() + 1];  // pool[]ify
    ...
    for (modIdx = 0; modIdx < cuMI.getCount(); ++modIdx) {
      ...
      /// @todo srj -- this is evil, we shouldn't look at the module name
      /// directly!
      ...
      *m++ = TheModuleFactory->newModule(this, cuMI.getNthName(modIdx),
                                         newModData, MODULETYPE_CLIENT_UPDATE);
    }
    *m = NULL;
  }
  ...
  for (i = 0; i < NUM_DRAWABLE_MODULE_TYPES; ++i) {
    for (Module** m = m_modules[i]; m && *m; ++m) (*m)->onObjectCreated();
  }
...
Drawable::~Drawable() {
  ...
  if (m_iconInfo) m_iconInfo->deleteInstance();

  if (m_selectionFlashEnvelope) m_selectionFlashEnvelope->deleteInstance();

  if (m_colorTintEnvelope) m_colorTintEnvelope->deleteInstance();
...
void Drawable::onDestroy(void) {
  ...
  for (Int i = 0; i < NUM_DRAWABLE_MODULE_TYPES; i++) {
    for (Module** m = m_modules[i]; m && *m; ++m) (*m)->onDelete();
...
Bool Drawable::getShouldAnimate(Bool considerPower) const {
  ...
  if (obj) {
    if (considerPower &&
        obj->testScriptStatusBit(OBJECT_STATUS_SCRIPT_UNPOWERED))
      return FALSE;
    ...
    if (obj->isDisabled()) {
      if (obj->isDisabledByType(DISABLED_HACKED) ||
          ...
          // srj sez: unmanned things also should not animate. (eg, gattling
          // tanks, which have a slight barrel animation even when at rest). if
          // this causes a problem, we will need to fix gattling tanks in
          // another way.
      ...
      if (considerPower && obj->isDisabledByType(DISABLED_UNDERPOWERED)) {
        ...
        // By checking for the others first, we prevent underpower from allowing
        // a True on an addition disable type
        return FALSE;
      }
    }
  }
...
Bool Drawable::clientOnly_getFirstRenderObjInfo(Coord3D* pos,
  ...
  const ObjectDrawInterface* di =
      (dm && *dm) ? (*dm)->getObjectDrawInterface() : NULL;
  if (di) {
    return di->clientOnly_getRenderObjInfo(pos, boundingSphereRadius,
                                           transform);
  }
  return false;
}
...
//-------------------------------------------------------------------------------------------------
Bool Drawable::getProjectileLaunchOffset(
    WeaponSlotType wslot, Int specificBarrelToUse, Matrix3D* launchPos,
    WhichTurretType tur, Coord3D* turretRotPos, Coord3D* turretPitchPos) const {
  for (const DrawModule** dm = getDrawModules(); *dm; ++dm) {
    const ObjectDrawInterface* di = (*dm)->getObjectDrawInterface();
    if (di && di->getProjectileLaunchOffset(m_conditionState, wslot,
                                            specificBarrelToUse, launchPos, tur,
...
void Drawable::setAnimationLoopDuration(UnsignedInt numFrames) {
  for (DrawModule** dm = getDrawModules(); *dm; ++dm) {
    ...
    if (di) di->setAnimationLoopDuration(numFrames);
  }
}
...
void Drawable::setAnimationCompletionTime(UnsignedInt numFrames) {
  for (DrawModule** dm = getDrawModules(); *dm; ++dm) {
    ObjectDrawInterface* di = (*dm)->getObjectDrawInterface();
    if (di) di->setAnimationCompletionTime(numFrames);
  }
}
...
//-------------------------------------------------------------------------------------------------
/**
        This call asks, "In the current animation (if any) how far along are
   you, from 0.0f to 1.0f".
*/
  ...
  for (const DrawModule** dm = getDrawModules(); *dm; ++dm) {
    ...
    if (di) {
      Int subcount = di->getPristineBonePositionsForConditionState(
          m_conditionState, boneNamePrefix, startIndex, positions, transforms,
          maxBones);
...
Int Drawable::getCurrentClientBonePositions(const char* boneNamePrefix,
  ...
  for (const DrawModule** dm = getDrawModules(); *dm; ++dm) {
    ...
    const ObjectDrawInterface* di = (*dm)->getObjectDrawInterface();
    if (di) {
      Int subcount = di->getCurrentBonePositions(
...
//-------------------------------------------------------------------------------------------------
void Drawable::attachToParticleSystem(Particle* p) { m_particle = p; }
...
void Drawable::setTerrainDecal(TerrainDecalType type) {
  if (m_terrainDecalType == type) return;
...
void Drawable::setTerrainDecalSize(Real x, Real y) {
  ...
  if (*dm) (*dm)->setTerrainDecalSize(x, y);
}
...
//-------------------------------------------------------------------------------------------------
/** Clear drawable's "selected" status, if not already clear.  Also update
 * running total count of selected drawables. */
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
/** Gathering point for all things besides actual selection that must happen on
 * selection */
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
/** fades the object out gradually...how gradually is determined by number of
 * frames */
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
/** fades the object in gradually...how gradually is determined by number of
 * frames */
//-------------------------------------------------------------------------------------------------
...
void Drawable::reactToBodyDamageStateChange(BodyDamageType newState) {
  ...
  clearAndSetModelConditionFlags(
      MAKE_MODELCONDITION_MASK3(MODELCONDITION_DAMAGED,
                                MODELCONDITION_REALLY_DAMAGED,
                                MODELCONDITION_RUBBLE),
      newDamage);
...
void Drawable::updateDrawable(void) {
...

  {
    ...
    if (m_fadeMode != FADING_NONE) {
      Real numer = (m_fadeMode == FADING_IN)
                       ? (m_timeElapsedFade)
                       : (m_timeToFade - m_timeElapsedFade);

      setDrawableOpacity(numer / (Real)m_timeToFade);
      ++m_timeElapsedFade;

      if (m_timeElapsedFade > m_timeToFade) m_fadeMode = FADING_NONE;
    }
  }
...

  {
    if (m_expirationDate != 0 && now >= m_expirationDate) {
      DEBUG_ASSERTCRASH(
          obj == NULL,
          ("Drawables with Objects should not have expiration dates!"));
  ...
  {
    if (m_flashCount > 0 &&
        (TheGameClient->getFrame() % DRAWABLE_FRAMES_PER_FLASH) == 0) {
      RGBColor tmp;
  ...
  if (m_prevTintStatus != m_tintStatus)  // edge test
  {
    if (testTintStatus(TINT_STATUS_DISABLED)) {
      if (m_colorTintEnvelope == NULL)
        m_colorTintEnvelope = newInstance(TintEnvelope);
      m_colorTintEnvelope->play(&DARK_GRAY_DISABLED_COLOR, 30, 30,
    ...
    //			if (m_colorTintEnvelope == NULL)
    //				m_colorTintEnvelope = newInstance(TintEnvelope);
    //			m_colorTintEnvelope->play( &SICKLY_GREEN_POISONED_COLOR,
    //30, 30, SUSTAIN_INDEFINITELY);
    //		}
    //		else if ( testTintStatus( TINT_STATUS_IRRADIATED) )
    //		{
    //			if (m_colorTintEnvelope == NULL)
    //				m_colorTintEnvelope = newInstance(TintEnvelope);
    //			m_colorTintEnvelope->play( &RED_IRRADIATED_COLOR, 30,
    //30, SUSTAIN_INDEFINITELY);
    //		}
    else {
  ...
  if (obj) {
    if (!obj->isEffectivelyDead())
      clearTintStatus(
          TINT_STATUS_IRRADIATED);  // so the res glow stops when not exposed
  }
  ...
  // If we have an ambient sound, and we aren't currently playing it, attempt to
  // play it now
  if (m_ambientSound && m_ambientSoundEnabled &&
      !m_ambientSound->m_event.getEventName().isEmpty() &&
      !m_ambientSound->m_event.isCurrentlyPlaying()) {
    startAmbientSound();
  }
}

//-------------------------------------------------------------------------------------------------
void Drawable::flashAsSelected(
    const RGBColor*
...
void Drawable::applyPhysicsXform(Matrix3D* mtx) {
  ...
  if (!obj || obj->isDisabledByType(DISABLED_HELD) ||
      !TheGlobalData->m_showClientPhysics) {
    return;
  }

  Bool frozen = TheTacticalView->isTimeFrozen() &&
                !TheTacticalView->isCameraMovementFinished();
  frozen = frozen || TheScriptEngine->isTimeFrozenDebug() ||
...
Bool Drawable::calcPhysicsXform(PhysicsXformInfo& info) {
  ...
  if (hasPhysicsXform) {
    ...
    if (info.m_totalYaw > -1e-20f && info.m_totalYaw < 1e-20f)
      info.m_totalYaw = 0.f;
    if (info.m_totalZ > -1e-20f && info.m_totalZ < 1e-20f) info.m_totalZ = 0.f;
  }
...
void Drawable::calcPhysicsXformThrust(const Locomotor* locomotor,
                                      PhysicsXformInfo& info) {
  if (m_locoInfo == NULL) m_locoInfo = newInstance(DrawableLocoInfo);
  ...
  //
  // this is a kind of quick thrust implementation cause we need scud missiles
  // to wobble *now*, we deal with just adjusting pitch, yaw, and roll just a
  // little bit
  //
  ...
  if (WOBBLE_RATE) {
    if (m_locoInfo->m_wobble >= 1.0f) {
      ...
      if (m_locoInfo->m_pitch >= MAX_WOBBLE) m_locoInfo->m_wobble = -1.0f;
      ...
      if (m_locoInfo->m_pitch <= MIN_WOBBLE) m_locoInfo->m_wobble = 1.0f;
...
void Drawable::calcPhysicsXformHoverOrWings(const Locomotor* locomotor,
  ...
  m_locoInfo->m_pitchRate +=
      ((-PITCH_STIFFNESS * m_locoInfo->m_pitch) +
       (-PITCH_DAMPING * m_locoInfo->m_pitchRate));  // spring/damper
  m_locoInfo->m_rollRate +=
      ((-ROLL_STIFFNESS * m_locoInfo->m_roll) +
       (-ROLL_DAMPING * m_locoInfo->m_rollRate));  // spring/damper
  ...
  m_locoInfo->m_accelerationPitchRate +=
      ((-PITCH_STIFFNESS * (m_locoInfo->m_accelerationPitch)) +
       (-PITCH_DAMPING *
        m_locoInfo->m_accelerationPitchRate));  // spring/damper
  m_locoInfo->m_accelerationPitch += m_locoInfo->m_accelerationPitchRate;

  m_locoInfo->m_accelerationRollRate +=
      ((-ROLL_STIFFNESS * m_locoInfo->m_accelerationRoll) +
       (-ROLL_DAMPING * m_locoInfo->m_accelerationRollRate));  // spring/damper
  ...
  if (physics->isMotive()) {
    ...
    // cause the chassis to pitch & roll in reaction to
    // acceleration/deceleration
    Real forwardAccel = dir->x * accel->x + dir->y * accel->y;
    m_locoInfo->m_accelerationPitchRate +=
        -(FORWARD_ACCEL_COEFF * forwardAccel);
...
void Drawable::calcPhysicsXformTreads(const Locomotor* locomotor,
  ...
  Coord3D normal;
  /*	Real hheight = */ TheTerrainLogic->getLayerHeight(
      pos->x, pos->y, obj->getLayer(), &normal);
  ...
  Object* overlapped =
      TheGameLogic->findObjectByID(physics->getCurrentOverlap());
  ...
  if (overlapped) {
    ...
    if (centerDistSqr < sqr(maxCenterDist)) {
      ...
      Real rough =
          (vel->x * vel->x + vel->y * vel->y) * OVERLAP_ROUGH_VIBRATION_FACTOR;
      if (rough > MAX_ROUGH_VIBRATION) rough = MAX_ROUGH_VIBRATION;
      ...
      if (overlapped->isKindOf(KINDOF_LOW_OVERLAPPABLE) ||
          ...
          (overlapped->getBodyModule()->getFrontCrushed() &&
           overlapped->getBodyModule()->getBackCrushed())) {
        flat = true;
      ...
      if (amount < FLATTENED_OBJECT_HEIGHT && flat == false) {
        ...
        // not proportional to actual geometry to avoid overlay steep inclines,
        // etc
        Coord3D v;
        v.x = dx / centerDist;
        v.y = dy / centerDist;
  ...
  } else  // no overlap this frame
  {
    ...
    if (physics->getPreviousOverlap() != INVALID_ID &&
  ...
  if (overlapped || m_locoInfo->m_overlapZ <= 0.0f) {
    m_locoInfo->m_pitchRate +=
        ((-PITCH_STIFFNESS * (m_locoInfo->m_pitch - groundPitch)) +
         (-PITCH_DAMPING * m_locoInfo->m_pitchRate));  // spring/damper
    if (m_locoInfo->m_pitchRate > 0.0f) m_locoInfo->m_pitchRate *= 0.5f;

    m_locoInfo->m_rollRate +=
        ((-ROLL_STIFFNESS * (m_locoInfo->m_roll - groundRoll)) +
         (-ROLL_DAMPING * m_locoInfo->m_rollRate));  // spring/damper
  }
  ...
  if (damageInfo) {
    if (obj->getBodyModule()->getLastDamageTimestamp() >
            m_lastDamageTimestamp &&
        damageInfo->in.m_amount > RECOIL_DAMAGE) {
...
void Drawable::calcPhysicsXformWheels(const Locomotor* locomotor,
  ...
  const Real MAX_SUSPENSION_EXTENSION =
      locomotor->getMaxWheelExtension();  //-2.3f;
  //	const Real MAX_SUSPENSION_COMPRESSION =
  //locomotor->getMaxWheelCompression(); //1.4f;
  ...
  if (airborne) {
    if (DO_WHEELS) {
      ...
      if (pos->z - hheight > -MAX_SUSPENSION_EXTENSION) {
        m_locoInfo->m_wheelInfo.m_rearLeftHeightOffset +=
            (MAX_SUSPENSION_EXTENSION -
             m_locoInfo->m_wheelInfo.m_rearLeftHeightOffset) /
            2.0f;
        m_locoInfo->m_wheelInfo.m_rearRightHeightOffset +=
            (MAX_SUSPENSION_EXTENSION -
             m_locoInfo->m_wheelInfo.m_rearRightHeightOffset) /
            2.0f;
      } else {
        m_locoInfo->m_wheelInfo.m_rearLeftHeightOffset +=
            (0 - m_locoInfo->m_wheelInfo.m_rearLeftHeightOffset) / 2.0f;
        m_locoInfo->m_wheelInfo.m_rearRightHeightOffset +=
            (0 - m_locoInfo->m_wheelInfo.m_rearRightHeightOffset) / 2.0f;
      }
    }
    ...
    Real pitchHeight =
        length * Sin(m_locoInfo->m_pitch + m_locoInfo->m_accelerationPitch -
                     groundPitch);
    Real rollHeight = width * Sin(m_locoInfo->m_roll +
                                  m_locoInfo->m_accelerationRoll - groundRoll);
  ...
  if (!airborne && curSpeed > maxSpeed / 10) {
    ...
    if (fabs(m_locoInfo->m_pitchRate) < factor * BOUNCE_ANGLE_KICK / 4 &&
        fabs(m_locoInfo->m_rollRate) < factor * BOUNCE_ANGLE_KICK / 8) {
      // do the bouncy.
  ...
  if (DO_WHEELS) {
    ...
    m_locoInfo->m_wheelInfo.m_framesAirborne =
        m_locoInfo->m_wheelInfo.m_framesAirborneCounter;
...
// we're going to add the angle to the current wheel rotation ... but we're
// going to divide that number to add small angles.  This allows for "smoother"
// wheel turning transitions ... and when the AI has things move in a straight
// line, since it's constantly telling the object to go left, go straight, go
// right, go straight, etc, this smaller angle we'll be adding covers the
// constant wheel shifting left and right when moving in a relatively straight
// line
//
#define WHEEL_SMOOTHNESS \
  10.0f  // higher numbers add smaller angles, make it more "smooth"
    m_locoInfo->m_wheelInfo.m_wheelAngle +=
        (newInfo.m_wheelAngle - m_locoInfo->m_wheelInfo.m_wheelAngle) /
        WHEEL_SMOOTHNESS;

    const Real SPRING_FACTOR = 0.9f;
    if (pitchHeight < 0) {  // Front raising up
      newInfo.m_frontLeftHeightOffset =
          SPRING_FACTOR * (pitchHeight / 3 + pitchHeight / 2);
      newInfo.m_frontRightHeightOffset =
          SPRING_FACTOR * (pitchHeight / 3 + pitchHeight / 2);
      ...
      newInfo.m_rearLeftHeightOffset =
          SPRING_FACTOR * (-pitchHeight / 2 + -pitchHeight / 3);
      newInfo.m_rearRightHeightOffset =
          SPRING_FACTOR * (-pitchHeight / 2 + -pitchHeight / 3);
    }
    if (rollHeight > 0) {  // Right raising up
      newInfo.m_frontRightHeightOffset +=
          -SPRING_FACTOR * (rollHeight / 3 + rollHeight / 2);
      newInfo.m_rearRightHeightOffset +=
          -SPRING_FACTOR * (rollHeight / 3 + rollHeight / 2);
    ...
    if (newInfo.m_frontLeftHeightOffset <
        m_locoInfo->m_wheelInfo.m_frontLeftHeightOffset) {
    ...
    if (newInfo.m_frontRightHeightOffset <
        m_locoInfo->m_wheelInfo.m_frontRightHeightOffset) {
      // If it's going down, dampen the movement a bit
      m_locoInfo->m_wheelInfo.m_frontRightHeightOffset +=
          (newInfo.m_frontRightHeightOffset -
           m_locoInfo->m_wheelInfo.m_frontRightHeightOffset) /
    ...
    if (newInfo.m_rearLeftHeightOffset <
        m_locoInfo->m_wheelInfo.m_rearLeftHeightOffset) {
      // If it's going down, dampen the movement a bit
      m_locoInfo->m_wheelInfo.m_rearLeftHeightOffset +=
          (newInfo.m_rearLeftHeightOffset -
    ...
    if (newInfo.m_rearRightHeightOffset <
        m_locoInfo->m_wheelInfo.m_rearRightHeightOffset) {
      // If it's going down, dampen the movement a bit
      m_locoInfo->m_wheelInfo.m_rearRightHeightOffset +=
          (newInfo.m_rearRightHeightOffset -
    ...
    if (m_locoInfo->m_wheelInfo.m_frontLeftHeightOffset <
        MAX_SUSPENSION_EXTENSION) {
      m_locoInfo->m_wheelInfo.m_frontLeftHeightOffset =
          MAX_SUSPENSION_EXTENSION;
    }
    if (m_locoInfo->m_wheelInfo.m_frontRightHeightOffset <
        MAX_SUSPENSION_EXTENSION) {
      m_locoInfo->m_wheelInfo.m_frontRightHeightOffset =
          MAX_SUSPENSION_EXTENSION;
    }
    if (m_locoInfo->m_wheelInfo.m_rearLeftHeightOffset <
        MAX_SUSPENSION_EXTENSION) {
      m_locoInfo->m_wheelInfo.m_rearLeftHeightOffset = MAX_SUSPENSION_EXTENSION;
    }
    if (m_locoInfo->m_wheelInfo.m_rearRightHeightOffset <
        MAX_SUSPENSION_EXTENSION) {
      m_locoInfo->m_wheelInfo.m_rearRightHeightOffset =
          MAX_SUSPENSION_EXTENSION;
    }
...
//-------------------------------------------------------------------------------------------------
/** decodes the current previous damage type and sets the ambient sound set from
 * that. */
//-------------------------------------------------------------------------------------------------
...
void Drawable::validatePos() const {
  ...
  if (getObject()) {
    ...
    if (ourPos->x != objPos->x || ourPos->y != objPos->y ||
        ourPos->z != objPos->z) {
  ...
  if (look != m_stealthLook) {
    ...
    switch (look) {
      ...
      case STEALTHLOOK_VISIBLE_FRIENDLY_DETECTED: {
        ...
        Object* obj = getObject();
        if (obj) {
          // Try to get the stealthupdate module and see if the opacity value is
          // overriden.
          ...
          if (stealth) {
            if (stealth->isDisguised()) {
              // Disguised objects drive the opacity level directly, hence the
              // break.
              m_hiddenByStealth = false;
              break;
            } else {
        ...
        m_stealthOpacity =
            opacity;  // make as partially transparent as this while pulsing
        m_hiddenByStealth = false;

        /** @todo srj -- evil hack here... this whole heat-vision thing is
           fucked. don't want it on mines but no good way to do that. hack for
           now. */
        if (look == STEALTHLOOK_VISIBLE_FRIENDLY_DETECTED &&
            !isKindOf(KINDOF_MINE))
      ...
      case STEALTHLOOK_VISIBLE_DETECTED:
        ...
        if (isKindOf(KINDOF_MINE))
          ...
          m_heatVisionOpacity =
              1.0f;  // Draw() will fade until it is set to 1 again
        break;
...
void Drawable::draw(View* view) {
  ...
  else if (m_heatVisionOpacity >
           VERY_TRANSPARENT_HEATVISION)  // keep fading any heatvision unless
                                         // something has set it to zero
...
static Bool computeHealthRegion(const Drawable* draw, IRegion2D& region) {
  ...
  if (!TheTacticalView->worldToScreen(&p, &screenCenter)) return FALSE;
...
Bool Drawable::drawsAnyUIText(void) {
  ...
  const Object* obj = getObject();
  if (!obj || !obj->isLocallyControlled()) return FALSE;
  ...
  if (obj->getFormationID() != NO_FORMATION_ID) return TRUE;

  return FALSE;
}

// ------------------------------------------------------------------------------------------------
/** This is called as part of the "post draw" phase when drawable a drawable. It
 * is there that we should overlay on the screen any 2D elements for purposes of
 * user interface information (such as a heatlh bar, veterency levels, etc.) */
// ------------------------------------------------------------------------------------------------
void Drawable::drawIconUI(void) {
  if (TheGameLogic->getDrawIconUI() &&
      (TheScriptEngine->getFade() == ScriptEngine::FADE_NONE)) {
    IRegion2D healthBarRegionStorage;
    const IRegion2D* healthBarRegion = NULL;
    if (computeHealthRegion(this, healthBarRegionStorage))
      healthBarRegion =
          &healthBarRegionStorage;  // both data and a PointerAsFlag for logic
    ...
    if (obj->isEffectivelyDead() ||
        obj->isKindOf(
            KINDOF_IGNORED_IN_GUI))  // object explicitly wants nothing to do
                                     // with these icons, so...
      return;
    drawHealing(
        healthBarRegion);  // call so dead things can kill their healing icons
    drawBombed(healthBarRegion);

    // Disabled for multiplay!
    // drawBattlePlans( healthBarRegion );

    if (drawsAnyUIText()) TheGameClient->addTextBearingDrawable(this);
...
DrawableIconInfo* Drawable::getIconInfo() {
  if (m_iconInfo == NULL) m_iconInfo = newInstance(DrawableIconInfo);
  return m_iconInfo;
}

//------------------------------------------------------------------------------------------------
void Drawable::clearEmoticon() {
  if (!hasIconInfo()) return;
...
void Drawable::setEmoticon(const AsciiString& name, Int duration) {
  ...
  if (animTemplate) {
    DEBUG_ASSERTCRASH(
        getIconInfo()->m_icon[ICON_EMOTICON] == NULL,
        ("Drawable::setEmoticon - Emoticon isn't empty, need to refuse to set "
         "or destroy the old one in favor of the new one\n"));
    if (getIconInfo()->m_icon[ICON_EMOTICON] == NULL) {
      getIconInfo()->m_icon[ICON_EMOTICON] =
          newInstance(Anim2D)(animTemplate, TheAnim2DCollection);
      getIconInfo()->m_keepTillFrame[ICON_EMOTICON] =
          duration >= 0 ? TheGameLogic->getFrame() + duration : FOREVER;
    }
  }
}
...
void Drawable::drawEmoticon(const IRegion2D* healthBarRegion) {
  if (hasIconInfo() && getIconInfo()->m_icon[ICON_EMOTICON]) {
    UnsignedInt now = TheGameLogic->getFrame();
    if (healthBarRegion &&
        getIconInfo()->m_keepTillFrame[ICON_EMOTICON] >= now) {
      // Draw the emoticon.
      Int barWidth = healthBarRegion->hi.x - healthBarRegion->lo.x;
      // Int barHeight = healthBarRegion.hi.y - healthBarRegion.lo.y;
      Int frameWidth =
          getIconInfo()->m_icon[ICON_EMOTICON]->getCurrentFrameWidth();
      Int frameHeight =
          getIconInfo()->m_icon[ICON_EMOTICON]->getCurrentFrameHeight();

#ifdef SCALE_ICONS_WITH_ZOOM_ML
      // adjust the width to be a % of the health bar region size
      Int size = REAL_TO_INT(barWidth * 0.3f);
      frameHeight = REAL_TO_INT((INT_TO_REAL(size) / INT_TO_REAL(frameWidth)) *
                                frameHeight);
      frameWidth = size;
#endif
      // given our scaled width and height we need to find the top left point to
      // draw the image at
      ICoord2D screen;
      screen.x = (Int)(healthBarRegion->lo.x + (barWidth * 0.5f) -
                       (frameWidth * 0.5f));
      screen.y = healthBarRegion->hi.y - frameHeight;
      getIconInfo()->m_icon[ICON_EMOTICON]->draw(screen.x, screen.y, frameWidth,
                                                 frameHeight);
    } else {
...
void Drawable::drawAmmo(const IRegion2D* healthBarRegion) {
  ...
  Int numTotal;
  Int numFull;
  if (!obj->getAmmoPipShowingInfo(numTotal, numFull)) return;

  if (!s_fullAmmo || !s_emptyAmmo) return;

#ifdef SCALE_ICONS_WITH_ZOOM_ML
  Real scale = TheGlobalData->m_ammoPipScaleFactor /
               CLAMP_ICON_ZOOM_FACTOR(TheTacticalView->getZoom());
#else
  Real scale = 1.0f;
#endif
  ...
  if (!TheTacticalView->worldToScreen(&pos, &screenCenter)) return;

  Real bounding = obj->getGeometryInfo().getBoundingSphereRadius() * scale;
  // Int posx = screenCenter.x +
  // REAL_TO_INT(TheGlobalData->m_ammoPipScreenOffset.x*bounding) - totalWidth;
  //**CHANGING CODE: Left justify with health bar min
  Int posx = healthBarRegion->lo.x;
  Int posy = screenCenter.y +
             REAL_TO_INT(TheGlobalData->m_ammoPipScreenOffset.y * bounding);
  for (Int i = 0; i < numTotal; ++i) {
    TheDisplay->drawImage(i < numFull ? s_fullAmmo : s_emptyAmmo, posx,
                          posy + 1, posx + boxWidth, posy + 1 + boxHeight);
...
void Drawable::drawContained(const IRegion2D* healthBarRegion) {
  ...
  Int numTotal;
  Int numFull;
  if (!container->getContainerPipsToShow(numTotal, numFull)) return;
  ...
  if (contained) {
    for (ContainedItemsList::const_iterator it = contained->begin();
         it != contained->end(); ++it) {
      if ((*it)->isKindOf(KINDOF_INFANTRY)) ++numInfantry;
    }
  }
  ...
  pos.z += TheGlobalData->m_containerPipWorldOffset.z +
           obj->getGeometryInfo().getMaxHeightAbovePosition();
  if (!TheTacticalView->worldToScreen(&pos, &screenCenter)) return;
  ...
  // Int posx = screenCenter.x +
  // REAL_TO_INT(TheGlobalData->m_containerPipScreenOffset.x*bounding) -
  // totalWidth;
  //**CHANGING CODE: Left justify with health bar min
  Int posx = healthBarRegion->lo.x;
  Int posy =
      screenCenter.y +
      REAL_TO_INT(TheGlobalData->m_containerPipScreenOffset.y * bounding);
  ...
  for (Int i = 0; i < numTotal; ++i) {
    ...
    if (i < numFull)
      TheDisplay->drawImage(
          s_fullContainer, posx, posy, posx + boxWidth, posy + boxHeight,
          (i < numInfantry) ? INFANTRY_COLOR : NON_INFANTRY_COLOR);
    else
      TheDisplay->drawImage(s_emptyContainer, posx, posy + 1, posx + boxWidth,
                            posy + 1 + boxHeight);
    posx += boxWidth + SPACING;
  }
}
...
void Drawable::drawBattlePlans(const IRegion2D* healthBarRegion) {
  ...
  if (player && player->getNumBattlePlansActive() > 0 &&
      player->doesObjectQualifyForBattlePlan(obj)) {
    if (player->getBattlePlansActiveSpecific(PLANSTATUS_BOMBARDMENT)) {
      if (!getIconInfo()->m_icon[ICON_BATTLEPLAN_BOMBARD]) {
        getIconInfo()->m_icon[ICON_BATTLEPLAN_BOMBARD] = newInstance(Anim2D)(
            s_animationTemplates[ICON_BATTLEPLAN_BOMBARD], TheAnim2DCollection);
      }
      // Int barHeight = healthBarRegion.hi.y - healthBarRegion.lo.y;
      Int frameWidth = getIconInfo()
                           ->m_icon[ICON_BATTLEPLAN_BOMBARD]
                           ->getCurrentFrameWidth();
      Int frameHeight = getIconInfo()
                            ->m_icon[ICON_BATTLEPLAN_BOMBARD]
      ...
      getIconInfo()->m_icon[ICON_BATTLEPLAN_BOMBARD]->draw(
          screen.x, screen.y, frameWidth, frameHeight);
    } else {
    ...
    if (player->getBattlePlansActiveSpecific(PLANSTATUS_HOLDTHELINE)) {
      if (!getIconInfo()->m_icon[ICON_BATTLEPLAN_HOLDTHELINE]) {
        getIconInfo()->m_icon[ICON_BATTLEPLAN_HOLDTHELINE] = newInstance(
            Anim2D)(s_animationTemplates[ICON_BATTLEPLAN_HOLDTHELINE],
                    TheAnim2DCollection);
      }
      // draw the icon
      Int frameWidth = getIconInfo()
                           ->m_icon[ICON_BATTLEPLAN_HOLDTHELINE]
                           ->getCurrentFrameWidth();
      Int frameHeight = getIconInfo()
                            ->m_icon[ICON_BATTLEPLAN_HOLDTHELINE]
                            ->getCurrentFrameHeight();
      ...
      screen.x = healthBarRegion->lo.x;
      screen.y = healthBarRegion->lo.y + frameHeight;
      getIconInfo()->m_icon[ICON_BATTLEPLAN_HOLDTHELINE]->draw(
          screen.x + frameWidth, screen.y, frameWidth, frameHeight);
    } else {
    ...
    if (player->getBattlePlansActiveSpecific(PLANSTATUS_SEARCHANDDESTROY)) {
      if (!getIconInfo()->m_icon[ICON_BATTLEPLAN_SEARCHANDDESTROY]) {
        getIconInfo()->m_icon[ICON_BATTLEPLAN_SEARCHANDDESTROY] = newInstance(
            Anim2D)(s_animationTemplates[ICON_BATTLEPLAN_SEARCHANDDESTROY],
                    TheAnim2DCollection);
      }
      // draw the icon
      Int frameWidth = getIconInfo()
                           ->m_icon[ICON_BATTLEPLAN_SEARCHANDDESTROY]
                           ->getCurrentFrameWidth();
      Int frameHeight = getIconInfo()
                            ->m_icon[ICON_BATTLEPLAN_SEARCHANDDESTROY]
                            ->getCurrentFrameHeight();
      ...
      screen.x = healthBarRegion->lo.x;
      screen.y = healthBarRegion->lo.y + frameHeight;
      getIconInfo()->m_icon[ICON_BATTLEPLAN_SEARCHANDDESTROY]->draw(
          screen.x + (frameWidth * 2), screen.y, frameWidth, frameHeight);
    } else {
...
void Drawable::drawUIText() {
  ...
  if (computeHealthRegion(this, healthBarRegionStorage))
    healthBarRegion =
        &healthBarRegionStorage;  // both data and a PointerAsFlag for logic in
                                  // the methods below

  if (!healthBarRegion) return;
  ...
  Color color = TheDrawGroupInfo->m_usePlayerColor
                    ? owner->getPlayerColor()
                    : TheDrawGroupInfo->m_colorForText;
  ...
  if (obj->getFormationID() != NO_FORMATION_ID) {
    ...
    ICoord2D screenCenter;
    obj->getHealthBoxPosition(p);
    if (!TheTacticalView->worldToScreen(&p, &screenCenter)) return;

    Real healthBoxWidth, healthBoxHeight;
    if (!obj->getHealthBoxDimensions(healthBoxHeight, healthBoxWidth)) return;

    Real scale = 1.3f / CLAMP_ICON_ZOOM_FACTOR(TheTacticalView->getZoom());
    screenCenter.x += (healthBoxWidth * scale * 0.5f) + 10;

    DisplayString* formationMarker =
        TheDisplayStringManager->getFormationLetterString();
    // static DisplayString *formationMarker =
    // TheDisplayStringManager->getGroupNumeralString( 5 );
    if (formationMarker)
...
void Drawable::drawHealing(const IRegion2D* healthBarRegion) {
  ...
  if (obj->isKindOf(KINDOF_NO_HEAL_ICON) ||
      BitTest(obj->getStatusBits(), OBJECT_STATUS_SOLD))
    return;
  ...
  if (body->getHealth() != body->getMaxHealth()) {
    ...
    //		if( lastDamage != NULL && lastDamage->in.m_damageType ==
    //DAMAGE_HEALING
    //			&&(TheGameLogic->getFrame() -
    //body->getLastHealingTimestamp()) <= HEALING_ICON_DISPLAY_TIME
    //			)
    if (TheGameLogic->getFrame() >
            HEALING_ICON_DISPLAY_TIME &&  // because so many things init health
                                          // early in game
        (TheGameLogic->getFrame() - body->getLastHealingTimestamp() <=
         HEALING_ICON_DISPLAY_TIME))
  ...
  //
  // if we are to show healing make sure we have the animation for it allocated,
  // otherwise free any animation we may have allocated back to the animation
  // memory pool
  //
  if (showHealing)  /// @todo HERE, WE NEED TO LEAVE STUFF ALONE, IF WE ARE
                    /// ALREADY SHOWING HEALING
  {
    if (healthBarRegion != NULL) {
      if (getIconInfo()->m_icon[typeIndex] == NULL)
        getIconInfo()->m_icon[typeIndex] = newInstance(Anim2D)(
            s_animationTemplates[typeIndex], TheAnim2DCollection);

      // draw the animation if present
      if (getIconInfo()->m_icon[typeIndex] != NULL) {
        //
        // we are going to draw the healing icon relative to the size of the
        // health bar region since that region takes into account hit point size
        // and zoom factor of the camera too
        //
        Int barWidth = healthBarRegion->hi.x - healthBarRegion->lo.x;

        Int frameWidth =
            getIconInfo()->m_icon[typeIndex]->getCurrentFrameWidth();
        Int frameHeight =
            getIconInfo()->m_icon[typeIndex]->getCurrentFrameHeight();

#ifdef SCALE_ICONS_WITH_ZOOM_ML
        // adjust the width to be a % of the health bar region size
        Int size = REAL_TO_INT(barWidth * scale);
        frameHeight = REAL_TO_INT(
            (INT_TO_REAL(size) / INT_TO_REAL(frameWidth)) * frameHeight);
        frameWidth = size;
#endif
        // given our scaled width and height we need to find the top left point
        // to draw the image at
        ICoord2D screen;
        screen.x = REAL_TO_INT(healthBarRegion->lo.x + (barWidth * 0.75f) -
                               (frameWidth * 0.5f));
        screen.y = REAL_TO_INT(healthBarRegion->lo.y - frameHeight);
        getIconInfo()->m_icon[typeIndex]->draw(screen.x, screen.y, frameWidth,
                                               frameHeight);
      }
    }
  } else {
...
void Drawable::drawEnthusiastic(const IRegion2D* healthBarRegion) {
  const Object* obj = getObject();
  //
  // if we are to show effect make sure we have the animation for it allocated,
  // otherwise free any animation we may have allocated back to the animation
  // memory pool
  //
  // only display if have enthusiasm

  if (obj->testWeaponBonusCondition(WEAPONBONUSCONDITION_ENTHUSIASTIC) ==
          TRUE &&
    ...
    if (getIconInfo()->m_icon[iconIndex] == NULL)
      getIconInfo()->m_icon[iconIndex] = newInstance(Anim2D)(
          s_animationTemplates[iconIndex], TheAnim2DCollection);
    ...
    if (getIconInfo()->m_icon[iconIndex] != NULL) {
      ...
      //
      Int barWidth =
          healthBarRegion->hi.x - healthBarRegion->lo.x;  // used for position

      // based on our own kind of we have certain icons to display at a size
      // scale
      Real scale;
      ...
      Int frameWidth =
          getIconInfo()->m_icon[iconIndex]->getCurrentFrameWidth() * scale;
      Int frameHeight =
          getIconInfo()->m_icon[iconIndex]->getCurrentFrameHeight() * scale;
      ...
      frameWidth = size;
#endif
      // given our scaled width and height we need to find the bottom left point
      // to draw the image at
      ICoord2D screen;
      screen.x = REAL_TO_INT(healthBarRegion->lo.x + (barWidth * 0.25f) -
                             (frameWidth * 0.5f));
      screen.y = healthBarRegion->hi.y;
      getIconInfo()->m_icon[iconIndex]->draw(screen.x, screen.y, frameWidth,
                                             frameHeight);
    }
  } else {
  ...
  if (ai->isDemoralized()) {
    ...
    if (healthBarRegion) {
      ...
      if (getIconInfo()->m_icon[ICON_DEMORALIZED] == NULL)
        getIconInfo()->m_icon[ICON_DEMORALIZED] = newInstance(Anim2D)(
            s_animationTemplates[ICON_DEMORALIZED], TheAnim2DCollection);

      if (getIconInfo()->m_icon[ICON_DEMORALIZED]) {
        Int frameWidth =
            getIconInfo()->m_icon[ICON_DEMORALIZED]->getCurrentFrameWidth();
        Int frameHeight =
            getIconInfo()->m_icon[ICON_DEMORALIZED]->getCurrentFrameHeight();
        ...
        screen.x = healthBarRegion->lo.x;
        screen.y = healthBarRegion->hi.y;
        getIconInfo()->m_icon[ICON_DEMORALIZED]->draw(screen.x, screen.y,
...
void Drawable::drawBombed(const IRegion2D* healthBarRegion) {
  ...
  if (obj->testWeaponSetFlag(WEAPONSET_CARBOMB) &&
    ...
    if (!getIconInfo()->m_icon[ICON_CARBOMB])
      getIconInfo()->m_icon[ICON_CARBOMB] = newInstance(Anim2D)(
          s_animationTemplates[ICON_CARBOMB], TheAnim2DCollection);
    ...
    if (getIconInfo()->m_icon[ICON_CARBOMB]) {
      ...
      if (healthBarRegion) {
        ...
        Int frameWidth =
            getIconInfo()->m_icon[ICON_CARBOMB]->getCurrentFrameWidth();
        Int frameHeight =
            getIconInfo()->m_icon[ICON_CARBOMB]->getCurrentFrameHeight();
        ...
        // to draw the image at
        ICoord2D screen;
        screen.x = REAL_TO_INT(healthBarRegion->lo.x + (barWidth * 0.5f) -
                               (frameWidth * 0.5f));
        screen.y = REAL_TO_INT(healthBarRegion->lo.y + barHeight * 0.5f);

        getIconInfo()->m_icon[ICON_CARBOMB]->draw(screen.x, screen.y,
                                                  frameWidth, frameHeight);
  ...
  StickyBombUpdate* update =
      (StickyBombUpdate*)obj->findUpdateModule(key_StickyBombUpdate);
  if (update) {
    // This case is tricky. The object that is bombed doesn't know it... but the
    // bomb itself does. So what we do is get it's target, then determine if the
    // target has the icon or not.
    Object* target = update->getTargetObject();
    if (target) {
      if (update->isTimedBomb()) {
        // Timed bomb
        if (!getIconInfo()->m_icon[ICON_BOMB_TIMED]) {
          getIconInfo()->m_icon[ICON_BOMB_REMOTE] = newInstance(Anim2D)(
              s_animationTemplates[ICON_BOMB_REMOTE], TheAnim2DCollection);
          getIconInfo()->m_icon[ICON_BOMB_TIMED] = newInstance(Anim2D)(
              s_animationTemplates[ICON_BOMB_TIMED], TheAnim2DCollection);

          // Because this is a counter icon that ranges from 0-60 seconds, we
          // need to calculate which frame to start the animation from. Because
          // timers are second based -- 1000 ms equal 1 frame. So we simply
          // calculate the time via detonation frame.
          //
          //  srj sez: this may sound familiar somehow, but let me reiterate,
          //  just in case you missed it:
          //
          //  hardcoding is bad.
          //
          //  the anim got changed and now is only 20 seconds max, so the
          ...
          UnsignedInt seconds =
              REAL_TO_INT_CEIL((dieFrame - now) * SECONDS_PER_LOGICFRAME_REAL);

          UnsignedInt numFrames = getIconInfo()
                                      ->m_icon[ICON_BOMB_TIMED]
                                      ->getAnimTemplate()
                                      ->getNumFrames();
          // this anim goes from "N" seconds down to zero, so the max seconds we
          // can use is N-1.
          if (seconds > numFrames - 1) seconds = numFrames - 1;

          getIconInfo()->m_icon[ICON_BOMB_TIMED]->setMinFrame(numFrames -
                                                              seconds - 1);
        ...
        if (getIconInfo()->m_icon[ICON_BOMB_TIMED]) {
          //
          // we are going to draw the healing icon relative to the size of the
          // health bar region since that region takes into account hit point
          // size and zoom factor of the camera too
          //
          if (healthBarRegion) {
            Int barWidth = healthBarRegion->hi.x - healthBarRegion->lo.x;
            Int barHeight = healthBarRegion->hi.y - healthBarRegion->lo.y;
            ...
            Int frameWidth =
                getIconInfo()->m_icon[ICON_BOMB_TIMED]->getCurrentFrameWidth();
            Int frameHeight =
                getIconInfo()->m_icon[ICON_BOMB_TIMED]->getCurrentFrameHeight();
            ...
            // given our scaled width and height we need to find the top left
            ...
            getIconInfo()->m_icon[ICON_BOMB_REMOTE]->draw(
                screen.x, screen.y, frameWidth, frameHeight);
            getIconInfo()->m_keepTillFrame[ICON_BOMB_REMOTE] = now + 1;
            getIconInfo()->m_icon[ICON_BOMB_TIMED]->draw(
                screen.x, screen.y, frameWidth, frameHeight);
        ...
        if (getIconInfo()->m_icon[ICON_BOMB_REMOTE]) {
          ...
          if (healthBarRegion) {
            ...
            Int frameWidth =
                getIconInfo()->m_icon[ICON_BOMB_REMOTE]->getCurrentFrameWidth();
            Int frameHeight = getIconInfo()
                                  ->m_icon[ICON_BOMB_REMOTE]
                                  ->getCurrentFrameHeight();
...
void Drawable::drawDisabled(const IRegion2D* healthBarRegion) {
  ...
  if (obj->isDisabledByType(DISABLED_HACKED) ||
    ...
    if (healthBarRegion) {
      ...
      Int frameWidth =
          getIconInfo()->m_icon[ICON_DISABLED]->getCurrentFrameWidth();
      Int frameHeight =
          getIconInfo()->m_icon[ICON_DISABLED]->getCurrentFrameHeight();
      ...
      getIconInfo()->m_icon[ICON_DISABLED]->draw(screen.x, screen.y, frameWidth,
                                                 frameHeight);
...
//-------------------------------------------------------------------------------------------------
/** Draw construction percent for drawables that have objects that are "under
 * construction" */
//-------------------------------------------------------------------------------------------------
void Drawable::drawConstructPercent(const IRegion2D* healthBarRegion) {
  ...
  // construction is partially complete, allocate a display string if we need
  // one
  ...
  if (m_lastConstructDisplayed != obj->getConstructionPercent()) {
    ...
    buffer.format(TheGameText->fetch("CONTROLBAR:UnderConstructionDesc"),
                  obj->getConstructionPercent());
    m_constructDisplayString->setText(buffer);

    // record this percent as our last displayed so we don't un-necessarily
    // rebuild the string
...
void Drawable::drawCaption(const IRegion2D* healthBarRegion) {
  if (!m_captionDisplayString) return;
  ...
  // draw background
  {
    ...
    TheDisplay->drawFillRect(xPos, yPos, width + 2, height + 2,
                             GameMakeColor(0, 0, 0, 125));
    TheDisplay->drawOpenRect(xPos, yPos, width + 2, height + 2, 1.0,
                             GameMakeColor(20, 20, 20, 255));
  }
...
void Drawable::drawVeterancy(const IRegion2D* healthBarRegion) {
  ...
  //
  // take the center position of the object, go down to it's bottom extent, and
  // project that point to the screen, that will be the "center" of our
  // veterancy box
  //
  ...
  // draw the image
  TheDisplay->drawImage(image, screenCenter.x + 1, screenCenter.y + 1,
                        screenCenter.x + 1 + vetBoxWidth,
                        screenCenter.y + 1 + vetBoxHeight);
...
void Drawable::drawHealthBar(const IRegion2D* healthBarRegion) {
  ...
  //
  // only draw health for selected drawbles and drawables that have been moused
  // over by the cursor
  //
  if (TheGlobalData->m_showObjectHealth &&
      (isSelected() ||
    ...
    if (obj->isKindOf(KINDOF_FORCEATTACKABLE)) {
      // Currently (Nov 2002), everything that is forceattackable are civ
      // fences, and they all have a single hit point and they aren't
      // selectable. However, a bug is when you force attack it, it shows the
      // healthbar. Well, this stops it, however, should force attackable
      // kindofs change, then this will require reevaluation.
      return;
    }
    ...
    if (maxHealth == 0.0f || health == 0.0f) return;

    // what is our health ratio
    Real healthRatio = health / maxHealth;

    //
    // what color will we use for the health bar based on our ratio, this makes
    // it slowly go from green to red, (or from blue to cyan if under
    // construction, or disabled)
    //
    ...
    if (BitTest(obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION) ||
      ...
      outlineColor = GameMakeColor(0, healthRatio * 128.0f, 128,
                                   255);  // dark blue to dark cyan
    ...
    } else  // red to green
    {
      ...
      if (healthRatio >= 0.5f) {
        ...
        inColor.green = 1.0f;
        //				color = GameMakeColor       (  255 -
        //((healthRatio - 0.5f) / 0.5f) * 255, 255, 0, 255 ); 				outlineColor =
        //GameMakeColor( (255 - ((healthRatio - 0.5f) / 0.5f) * 255) * 0.5, 255
        //* 0.5, 0, 255 );
      } else {
        inColor.red = 1.0f;
        inColor.green = 1.0f - ((0.5f - healthRatio) / 0.5f);
        //				color = GameMakeColor( 255, 255 - ((0.5f
        //- healthRatio) / 0.5f) * 255, 0, 255 ); 				outlineColor = GameMakeColor(
        //255 * 0.5, (255 - ((0.5f - healthRatio) / 0.5f) * 255) * 0.5, 0, 255
        //);
      }
      ...
      if (m_conditionState.test(MODELCONDITION_REALLY_DAMAGED) ==
          TRUE) {  // average the above color with red
        inColor.red = (1.0f + inColor.red) * 0.5f;
        inColor.green *= 0.5f;
      } else if (m_conditionState.test(MODELCONDITION_DAMAGED) ==
                 FALSE) {  // average the above color with green
        inColor.green = (1.0f + inColor.green) * 0.5f;
        inColor.red *= 0.5f;
      }

      color = GameMakeColor(255.0 * inColor.red, 255.0 * inColor.green,
                            255.0 * inColor.blue, 255);
      outlineColor = GameMakeColor(255.0 * outColor.red, 255.0 * outColor.green,
                                   255.0 * outColor.blue, 255);
    }

    ///		Real scale = 1.3f / TheTacticalView->getZoom();
    Real healthBoxWidth = healthBarRegion->hi.x - healthBarRegion->lo.x;

    Real healthBoxHeight =
        max(3, healthBarRegion->hi.y - healthBarRegion->lo.y);
    ...
    TheDisplay->drawFillRect(
        healthBarRegion->lo.x + 1, healthBarRegion->lo.y + 1,
        (healthBoxWidth - 2) * healthRatio, healthBoxHeight - 2, color);
  }  // end if
...
void Drawable::clearAndSetModelConditionState(ModelConditionFlagType clr,
  ...
  if (clr != MODELCONDITION_INVALID) c.set(clr);
  if (set != MODELCONDITION_INVALID) s.set(set);
...
DrawModule** Drawable::getDrawModules() {
  ...
  if (m_isModelDirty) {
    if (s_modelLockCount > 0) {
      DEBUG_CRASH(
          ("Should not need to update dirty stuff while locked-for-iteration. "
           "Ignoring."));
      // this shouldn't happen, but if it does, just return the current (dirty)
      // scenario. we must NOT update the condition state, as someone is relying
      // on the current list of W3D render objects not being munged. (srj)
    } else {
      for (DrawModule** dm2 = dm; *dm2; ++dm2) {
        ObjectDrawInterface* di = (*dm2)->getObjectDrawInterface();
        if (di) di->replaceModelConditionState(m_conditionState);
      }
...
void Drawable::replaceModelConditionFlags(const ModelConditionFlags& flags,
                                          Bool forceReplace) {
  //
  // this is a no-op if the new flags are the same as our existing flags (unless
  // we have the forceReplace parameter set, in which case we will force the
  // setting of the new flags)
  //
  if (forceReplace == FALSE && m_conditionState == flags) return;

  m_conditionState = flags;
#ifdef DIRTY_CONDITION_FLAGS
  // when forcing a replace we won't use dirty flags, we will immediately do an
  // update now
...
void Drawable::setIndicatorColor(Color color) {
  for (DrawModule** dm = getDrawModules(); *dm; ++dm) {
    ObjectDrawInterface* di = (*dm)->getObjectDrawInterface();
    if (di) di->replaceIndicatorColor(color);
  }
}

// ------------------------------------------------------------------------------------------------
const GeometryInfo& Drawable::getDrawableGeometryInfo() const {
  return getObject() ? getObject()->getGeometryInfo()
                     : getTemplate()->getTemplateGeometryInfo();
}
...
void Drawable::setID(DrawableID id) {
  ...
  if (m_id != INVALID_DRAWABLE_ID) {
    ...
    if (m_ambientSound) m_ambientSound->m_event.setDrawableID(m_id);
  }
...
DrawableID Drawable::getID(void) const {
  // we should never be getting the ID of a drawable who doesn't yet have and ID
  // assigned to it
  DEBUG_ASSERTCRASH(
      m_id != 0, ("Drawable::getID - Using ID before it was assigned!!!!\n"));
...
//-------------------------------------------------------------------------------------------------
void Drawable::friend_bindToObject(
    Object* obj)  ///< bind this drawable to an object ID
{
  m_object = obj;
  if (getObject()) {
...
Bool Drawable::handleWeaponFireFX(WeaponSlotType wslot, Int specificBarrelToUse,
  ...
  for (DrawModule** dm = getDrawModules(); *dm; ++dm) {
    ObjectDrawInterface* di = (*dm)->getObjectDrawInterface();
    if (di && di->handleWeaponFireFX(wslot, specificBarrelToUse, fxl,
                                     weaponSpeed, victimPos, damageRadius))
      return true;
  }
  return false;
}
 ...
 * If this Drawable is attached to an Object, return the Object's transform
 * instead.
 */
//-------------------------------------------------------------------------------------------------
...
UnicodeString Drawable::getCaptionText(void) {
  if (m_captionDisplayString) return m_captionDisplayString->getText();
  ...
  if (trySound && m_ambientSound) {
    ...
    if (info) {
      if (BitTest(info->m_type, ST_GLOBAL) || info->m_priority == AP_CRITICAL) {
        ...
        m_ambientSound->m_event.setPlayingHandle(
            TheAudio->addAudioEvent(&m_ambientSound->m_event));
      } else {
      ...
      DEBUG_CRASH(("Ambient sound %s missing! Skipping...",
                   m_ambientSound->m_event.getEventName().str()));
      m_ambientSound->deleteInstance();
      m_ambientSound = NULL;
    }
  }
...
//-------------------------------------------------------------------------------------------------
// Attach and start playing an ambient sound to this drawable. Calculates states
// automatically.
//-------------------------------------------------------------------------------------------------
...
void Drawable::prependToList(Drawable** pListHead) {
  ...
  if (*pListHead) (*pListHead)->m_prevDrawable = this;
  *pListHead = this;
}
...
void Drawable::removeFromList(Drawable** pListHead) {
  if (m_nextDrawable) m_nextDrawable->m_prevDrawable = m_prevDrawable;
...
void Drawable::updateHiddenStatus() {
  ...
  if (hidden) TheInGameUI->deselectDrawable(this);

  for (DrawModule** dm = getDrawModules(); *dm; ++dm) {
    ObjectDrawInterface* di = (*dm)->getObjectDrawInterface();
    if (di) di->setHidden(hidden != 0);
  }
}
...
void Drawable::updateDrawableClipStatus(UnsignedInt shotsRemaining,
  ...
  for (DrawModule** dm = getDrawModules(); *dm; ++dm) {
    ObjectDrawInterface* di = (*dm)->getObjectDrawInterface();
    if (di) di->updateProjectileClipStatus(shotsRemaining, maxShots, slot);
  }
}
...
void Drawable::updateDrawableSupplyStatus(Int maxSupply, Int currentSupply) {
  for (DrawModule** dm = getDrawModules(); *dm; ++dm) {
    ObjectDrawInterface* di = (*dm)->getObjectDrawInterface();
    if (di) di->updateDrawModuleSupplyStatus(maxSupply, currentSupply);
  }
}
...
void Drawable::notifyDrawableDependencyCleared() {
  for (DrawModule** dm = getDrawModules(); *dm; ++dm) {
    ObjectDrawInterface* di = (*dm)->getObjectDrawInterface();
    if (di) di->notifyDrawModuleDependencyCleared();
  }
}
...
void Drawable::setSelectable(Bool selectable) {
  ...
  if (!selectable) TheInGameUI->deselectDrawable(this);

  for (DrawModule** dm = getDrawModules(); *dm; ++dm) {
    ObjectDrawInterface* di = (*dm)->getObjectDrawInterface();
    if (di) di->setSelectable(selectable);
  }
}
...
//-------------------------------------------------------------------------------------------------
/** Preload all our assets that we can for all our possible states in this time
 * of day */
//-------------------------------------------------------------------------------------------------
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void Drawable::crc(Xfer* xfer) {}  // end crc
...
void Drawable::xferDrawableModules(Xfer* xfer) {
  ...
  //
  // when using dirty condition flags ... we want to make sure that the modules
  // are updated to the current state of the drawable
  //
#ifdef DIRTY_CONDITION_FLAGS
  if (xfer->getXferMode() == XFER_SAVE)
    getDrawModules();  // will re-evaluate modules that are dirty and update
                       // them
#endif
  ...
  for (UnsignedShort curModuleType = 0; curModuleType < moduleTypes;
    ...
    for (m = m_modules[curModuleType]; m && *m; ++m) moduleCount++;
    ...
    if (xfer->getXferMode() == XFER_SAVE) {
      ...
      for (m = m_modules[curModuleType]; m && *m; ++m) {
        ...
        moduleIdentifier =
            TheNameKeyGenerator->keyToName((*m)->getModuleTagNameKey());
        DEBUG_ASSERTCRASH(moduleIdentifier != AsciiString::TheEmptyString,
                          ("Drawable::xferDrawableModules - module name key "
                           "does not translate to a string!\n"));
      ...
      for (UnsignedShort j = 0; j < moduleCount; ++j) {
        ...
        //
        // if we didn't find the module, it's quite possible that we have
        // removed it from the object definition in a future patch, if that is
        // so, we need to skip the module data in the file
        //
        if (module == NULL) {
          // for testing purposes, this module better be found
          DEBUG_CRASH((
              "Drawable::xferDrawableModules - Module '%s' was indicated in "
              "file, but not found on Drawable %s %d\n",
...
void Drawable::xfer(Xfer* xfer) {
  ...
  if (xfer->getXferMode() == XFER_LOAD && m_ambientSound) {
    TheAudio->killAudioEventImmediately(
        m_ambientSound->m_event.getPlayingHandle());
    m_ambientSound->deleteInstance();
    m_ambientSound = NULL;
  }
  ...
  // condition state, note that when we're loading we need to force a replace of
  // these flags
  if (version >= 2) {
  ...
  if (xfer->getXferMode() == XFER_LOAD) setTerrainDecal(decal);
  ...
  if (xfer->getXferMode() == XFER_LOAD) {
    if (m_object) {
      if (objectID != m_object->getID()) {
        DEBUG_CRASH((
            "Drawable::xfer - Drawable '%s' is attached to wrong object '%s'\n",
            getTemplate()->getName().str(),
            m_object->getTemplate()->getName().str()));
      ...
      if (objectID != INVALID_ID) {
        ...
        DEBUG_CRASH(
            ("Drawable::xfer - Drawable '%s' is not attached to an object but "
             "should be attached to object '%s' with id '%d'\n",
             getTemplate()->getName().str(),
             obj ? obj->getTemplate()->getName().str() : "Unknown", objectID));
#endif
        throw SC_INVALID_DATA;
  ...
  // particle
  // we don't need to worry about this, the particle itself will set it upon
  // loading
  ...
  if (version < 2) {
    ...
    // condition state, note that when we're loading we need to force a replace
    // of these flags
    m_conditionState.xfer(xfer);
    if (xfer->getXferMode() == XFER_LOAD)
      replaceModelConditionFlags(m_conditionState, TRUE);
  ...
  if (hasIconInfo()) {
    for (UnsignedByte i = 0; i < MAX_ICONS; ++i)
      if (getIconInfo()->m_icon[i]) iconCount++;
  }
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    for (UnsignedByte i = 0; i < MAX_ICONS; ++i) {
      ...
      if (!hasIconInfo() || getIconInfo()->m_icon[i] == NULL) continue;
    ...
    // destroy any icons that might be present right now in favor of what we'll
    // load from the file
    if (hasIconInfo()) getIconInfo()->clear();
  ...
  if (xfer->getXferMode() == XFER_LOAD) {
    ...
    // it's correct value, and the drawable updated (hide shadows and such).
    // jba.
    m_stealthLook = STEALTHLOOK_NONE;
  ...
  //
  // when saving we should never have dirty modules, but when loading we will
  // force the modules to be dirty just to be sure that they get re-evaluated
  // after the load
  //
#ifdef DIRTY_CONDITION_FLAGS
  if (xfer->getXferMode() == XFER_SAVE)
    DEBUG_ASSERTCRASH(m_isModelDirty == FALSE,
                      ("Drawble::xfer - m_isModelDirty is not FALSE!\n"));
  else
    m_isModelDirty = TRUE;
#endif
...
/*static*/ void Drawable::friend_lockDirtyStuffForIteration() {
  if (s_modelLockCount == 0) {
    ...
    {
      for (Drawable* d = TheGameClient->firstDrawable(); d != NULL;
           d = d->getNextDrawable()) {
...
/*static*/ void Drawable::friend_unlockDirtyStuffForIteration() {
  if (s_modelLockCount > 0) --s_modelLockCount;
}
#endif
...
//-------------------------------------------------------------------------------------------------
void TintEnvelope::play(const RGBColor* peak, UnsignedInt atackFrames,
  ...
  if (delta.Length() <=
      FADE_RATE_EPSILON)  // we are practically already at this color
  ...
  switch (m_envState) {
    ...
    case (ENVELOPE_STATE_DECAY):  // much more likely than attack
    {
      if (m_decayRate.Length() > m_currentColor.Length() ||
          m_currentColor.Length() <= FADE_RATE_EPSILON)  // we are at rest
      {
        m_envState = ENVELOPE_STATE_REST;
        m_affect = FALSE;
      } else {
        Vector3::Add(
            m_decayRate, m_currentColor,
            &m_currentColor);  // Add the decayRate to the current color;
        m_affect = TRUE;
      }
      break;
    }
    case (ENVELOPE_STATE_ATTACK): {
      ...
      if (m_attackRate.Length() > delta.Length() ||
          delta.Length() <= FADE_RATE_EPSILON)  // we are at the peak
      {
        if (m_sustainCounter) {
          m_envState = ENVELOPE_STATE_SUSTAIN;
        } else {
        ...
        Vector3::Add(
            m_attackRate, m_currentColor,
            &m_currentColor);  // Add the attackRate to the current color;
        m_affect = TRUE;
      }
    ...
    default: {
      // do nothing, we are sustaining until externally triggered to release
      // (decay)
      break;
    }
  }
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void TintEnvelope::crc(Xfer* xfer) {}  // end crc
...
/** Load Post Process */
// ------------------------------------------------------------------------------------------------
void TintEnvelope::loadPostProcess(void) {}  // end loadPostProcess
