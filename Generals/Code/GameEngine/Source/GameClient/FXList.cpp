...
class SoundFXNugget : public FXNugget {
 ...
 public:
  ...
  virtual void doFXObj(const Object* primary,
  ...
  static void parse(INI* ini, void* instance, void* /*store*/,
    ...
    static const FieldParse myFieldParse[] = {
        {"Name", INI::parseAsciiString, NULL,
         offsetof(SoundFXNugget, m_soundName)},
        {0, 0, 0, 0}};
...
class TracerFXNugget : public FXNugget {
 ...
 public:
  ...
  virtual void doFXPos(const Coord3D* primary, const Matrix3D* primaryMtx,
                       ...
                       const Real /*overrideRadius*/) const {
    if (m_probability <= GameClientRandomValueReal(0, 1)) return;

    if (primary && secondary) {
      Drawable* tracer = TheThingFactory->newDrawable(
          TheThingFactory->findTemplate(m_tracerName));
      if (!tracer) return;

      // Kris -- Redid this section Sept 18, 2002
      // Calculate tracer orientations to face from primary to secondary
      // position. This should be the direction that the projectile is being
      // fired towards. It doesn't make sense that the old stuff made use of the
      // muzzle fx bone orientation (because it's a subobject). It had other
      // problems because of elevation variations the tracers would stay on the
      // ground. tracer->setTransformMatrix(primaryMtx);
      Matrix3D tracerMtx;
      Vector3 pos(primary->x, primary->y, primary->z);
      Vector3 dir(secondary->x - primary->x, secondary->y - primary->y,
                  secondary->z - primary->z);
      dir.Normalize();  // This is fantastically crucial for calling
  ...
  static void parse(INI* ini, void* instance, void* /*store*/,
                    const void* /*userData*/) {
    static const FieldParse myFieldParse[] = {
        {"TracerName", INI::parseAsciiString, NULL,
         offsetof(TracerFXNugget, m_tracerName)},
        {"BoneName", INI::parseAsciiString, NULL,
         offsetof(TracerFXNugget, m_boneName)},
        {"Speed", INI::parseVelocityReal, NULL,
         offsetof(TracerFXNugget, m_speed)},
        ...
        {"Probability", INI::parseReal, NULL,
         offsetof(TracerFXNugget, m_probability)},
        {0, 0, 0, 0}};
...
class RayEffectFXNugget : public FXNugget {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(RayEffectFXNugget,
                                          "RayEffectFXNugget")
 public:
  RayEffectFXNugget() {
  ...
  static void parse(INI* ini, void* instance, void* /*store*/,
    ...
    static const FieldParse myFieldParse[] = {
        {"Name", INI::parseAsciiString, NULL,
         offsetof(RayEffectFXNugget, m_templateName)},
        {"PrimaryOffset", INI::parseCoord3D, NULL,
         offsetof(RayEffectFXNugget, m_primaryOffset)},
        {"SecondaryOffset", INI::parseCoord3D, NULL,
         offsetof(RayEffectFXNugget, m_secondaryOffset)},
        {0, 0, 0, 0}};
...
class LightPulseFXNugget : public FXNugget {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(LightPulseFXNugget,
                                          "LightPulseFXNugget")
 public:
  LightPulseFXNugget()
      : m_radius(0),
  ...
  virtual void doFXObj(const Object* primary,
    ...
    if (primary) {
      ...
      TheDisplay->createLightPulse(primary->getPosition(), &m_color, 1, radius,
  ...
  virtual void doFXPos(const Coord3D* primary, const Matrix3D* /*primaryMtx*/,
                       ...
                       const Real /*overrideRadius*/) const {
    if (primary) {
      TheDisplay->createLightPulse(primary, &m_color, 1, m_radius,
                                   m_increaseFrames, m_decreaseFrames);
    } else {
      DEBUG_CRASH(("You must have a primary source for this effect"));
    }
  ...
  static void parse(INI* ini, void* instance, void* /*store*/,
                    const void* /*userData*/) {
    static const FieldParse myFieldParse[] = {
        {"Color", INI::parseRGBColor, NULL,
         offsetof(LightPulseFXNugget, m_color)},
        {"Radius", INI::parseReal, NULL,
         offsetof(LightPulseFXNugget, m_radius)},
        {"RadiusAsPercentOfObjectSize", INI::parsePercentToReal, NULL,
         offsetof(LightPulseFXNugget, m_boundingCirclePct)},
        {"IncreaseTime", INI::parseDurationUnsignedInt, NULL,
         offsetof(LightPulseFXNugget, m_increaseFrames)},
        {"DecreaseTime", INI::parseDurationUnsignedInt, NULL,
         offsetof(LightPulseFXNugget, m_decreaseFrames)},
        {0, 0, 0, 0}};
...
class ViewShakeFXNugget : public FXNugget {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(ViewShakeFXNugget,
                                          "ViewShakeFXNugget")
 public:
  ...
  virtual void doFXPos(const Coord3D* primary, const Matrix3D* /*primaryMtx*/,
                       ...
                       const Real /*overrideRadius*/) const {
    if (primary) {
      if (TheTacticalView) TheTacticalView->shake(primary, m_shake);
    } else {
      DEBUG_CRASH(("You must have a primary source for this effect"));
    }
  }
 ...
 protected:
  static void parseShakeType(INI* ini, void* instance, void* store,
                             const void* /*userData*/) {
...
class TerrainScorchFXNugget : public FXNugget {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(TerrainScorchFXNugget,
 ...
 public:
  ...
  static void parse(INI* ini, void* instance, void* /*store*/,
                    const void* /*userData*/) {
    static const FieldParse myFieldParse[] = {
        {"Type", parseScorchType, NULL,
         offsetof(TerrainScorchFXNugget, m_scorch)},
        {"Radius", INI::parseReal, NULL,
         offsetof(TerrainScorchFXNugget, m_radius)},
        {0, 0, 0, 0}};
 ...
 protected:
  static void parseScorchType(INI* ini, void* instance, void* store,
                              const void* /*userData*/) {
...
class ParticleSystemFXNugget : public FXNugget {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(ParticleSystemFXNugget,
 ...
 public:
  ...
  virtual void doFXObj(const Object* primary, const Object* secondary) const {
    if (primary) {
      if (m_ricochet && secondary) {
        // HERE WE MUST BUILD A MATRIX WHICH WILL ORIENT THE NEW PARTICLE SYSTEM
        // TO FACE AWAY FROM THE SECONDARY OBJECT THE RESULT SHOULD LOOK LIKE
        // THE DIRECTION OF THE "ATTACK" IS CARRIED THROUGH LIKE A RICOCHET
        ...
        // if we have an object, then adjust the offset and direction by the
        // object's transformation matrix, so that (say) an offset of +10 in the
        // z axis "follows" the orientation of the object.
        reallyDoFX(primary->getPosition(), primary->getTransformMatrix(),
                   primary, 0.0f);
    } else {
      DEBUG_CRASH(("You must have a primary source for this effect"));
    }
  ...
  static void parse(INI* ini, void* instance, void* /*store*/,
    ...
    static const FieldParse myFieldParse[] = {
        {"Name", INI::parseAsciiString, NULL,
         offsetof(ParticleSystemFXNugget, m_name)},
        {"Count", INI::parseInt, NULL,
         offsetof(ParticleSystemFXNugget, m_count)},
        {"Offset", INI::parseCoord3D, NULL,
         offsetof(ParticleSystemFXNugget, m_offset)},
        {"Radius", INI::parseGameClientRandomVariable, NULL,
         offsetof(ParticleSystemFXNugget, m_radius)},
        {"Height", INI::parseGameClientRandomVariable, NULL,
         offsetof(ParticleSystemFXNugget, m_height)},
        {"InitialDelay", INI::parseGameClientRandomVariable, NULL,
         offsetof(ParticleSystemFXNugget, m_delay)},
        {"RotateX", INI::parseAngleReal, NULL,
         offsetof(ParticleSystemFXNugget, m_rotateX)},
        {"RotateY", INI::parseAngleReal, NULL,
         offsetof(ParticleSystemFXNugget, m_rotateY)},
        {"RotateZ", INI::parseAngleReal, NULL,
         offsetof(ParticleSystemFXNugget, m_rotateZ)},
        {"OrientToObject", INI::parseBool, NULL,
         offsetof(ParticleSystemFXNugget, m_orientToObject)},
        {"Ricochet", INI::parseBool, NULL,
         offsetof(ParticleSystemFXNugget, m_ricochet)},
        {"AttachToObject", INI::parseBool, NULL,
         offsetof(ParticleSystemFXNugget, m_attachToObject)},
        {"CreateAtGroundHeight", INI::parseBool, NULL,
         offsetof(ParticleSystemFXNugget, m_createAtGroundHeight)},
        {"UseCallersRadius", INI::parseBool, NULL,
         offsetof(ParticleSystemFXNugget, m_useCallersRadius)},
        {0, 0, 0, 0}};
 ...
 protected:
  void reallyDoFX(const Coord3D* primary, const Matrix3D* mtx,
                  const Object* thingToAttachTo, Real overrideRadius) const {
    ...
    const ParticleSystemTemplate* tmp =
        TheParticleSystemManager->findTemplate(m_name);
      ...
      for (Int i = 0; i < m_count; i++) {
        ...
        if (sys) {
          ...
          if (m_createAtGroundHeight && TheTerrainLogic) {
            // old way:
            // newPos.z = TheTerrainLogic->getGrsoundHeight( newPos.x, newPos.y
            // ) + 1;// The plus one prevents scissoring with terrain

            // new way: now we allow bridges in the GroundHeight.
            PathfindLayerEnum layer =
                TheTerrainLogic->getLayerForDestination(&newPos);
            newPos.z =
                TheTerrainLogic->getLayerHeight(newPos.x, newPos.y, layer);
          } else
          ...
          if (m_rotateX != 0.0f) sys->rotateLocalTransformX(m_rotateX);
          if (m_rotateY != 0.0f) sys->rotateLocalTransformY(m_rotateY);
          if (m_rotateZ != 0.0f) sys->rotateLocalTransformZ(m_rotateZ);
          ...
          if (delayInMsec >= 0.0f) {
            UnsignedInt delayInFrames =
                REAL_TO_INT_CEIL(ConvertDurationFromMsecsToFrames(delayInMsec));
            sys->setInitialDelay(delayInFrames);
          }

          if (m_useCallersRadius && overrideRadius) {
            ParticleSystemInfo::EmissionVolumeType type =
                sys->getEmisionVolumeType();
...
class FXListAtBonePosFXNugget : public FXNugget {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(FXListAtBonePosFXNugget,
 ...
 public:
  ...
  static void parse(INI* ini, void* instance, void* /*store*/,
    ...
    static const FieldParse myFieldParse[] = {
        ...
        {"BoneName", INI::parseAsciiString, NULL,
         offsetof(FXListAtBonePosFXNugget, m_boneName)},
        {"OrientToBone", INI::parseBool, NULL,
         offsetof(FXListAtBonePosFXNugget, m_orientToBone)},
        {0, 0, 0, 0}};
 ...
 protected:
  void doFxAtBones(const Object* obj, Int start) const {
    ...
    if (draw) {
      // yes, BONEPOS_CURRENT_CLIENT_ONLY -- this is client-only, so should be
      // safe to do.
      Int count = draw->getCurrentClientBonePositions(
          m_boneName.str(), start, bonePos, boneMtx, MAX_BONE_POINTS);
...
//-------------------------------------------------------------------------------------------------
void FXList::clear() {
  for (FXNuggetList::iterator it = m_nuggets.begin(); it != m_nuggets.end();
       ++it) {
    if (*it) (*it)->deleteInstance();
  }
  m_nuggets.clear();
}

//-------------------------------------------------------------------------------------------------
void FXList::doFXPos(const Coord3D* primary, const Matrix3D* primaryMtx,
                     const Real primarySpeed, const Coord3D* secondary,
                     const Real overrideRadius) const {
  if (ThePartitionManager->getShroudStatusForPlayer(
          ThePlayerList->getLocalPlayer()->getPlayerIndex(), primary) !=
      CELLSHROUD_CLEAR)
    return;

  for (FXNuggetList::const_iterator it = m_nuggets.begin();
       it != m_nuggets.end(); ++it) {
    (*it)->doFXPos(primary, primaryMtx, primarySpeed, secondary,
                   overrideRadius);
  }
}
...
void FXList::doFXObj(const Object* primary, const Object* secondary) const {
  if (primary && primary->getShroudedStatus(
                     ThePlayerList->getLocalPlayer()->getPlayerIndex()) >
                     OBJECTSHROUD_PARTIAL_CLEAR)
    return;  // the primary object is fogged or shrouded so don't bother with
             // the effect.

  for (FXNuggetList::const_iterator it = m_nuggets.begin();
       it != m_nuggets.end(); ++it) {
    // HERE THE PRIMARY IS THE GUY RECEIVING THE FX, AND SECONDARY MIGHT BE THE
    // GUY DEALING IT
...
//-------------------------------------------------------------------------------------------------
FXListStore::~FXListStore() { m_fxmap.clear(); }
