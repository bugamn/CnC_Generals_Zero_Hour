...
LanguageFilter::~LanguageFilter() { m_wordList.clear(); }
...
void LanguageFilter::init() {
  ...
  File *file1 =
      TheFileSystem->openFile(BadWordFileName, File::READ | File::BINARY);
  if (file1 == NULL) {
    return;
  }
...
void LanguageFilter::reset() { init(); }
...
void LanguageFilter::filterLine(UnicodeString &line) {
  ...
  while (
      newLine.nextToken(&token, UnicodeString(L" ;,.!?:=\\/><`~()&^%#\n\t"))) {
    ...
    Int len = token.getLength();  // need to get the length of the original
                                  // word, not the unhaxor'd word.
...
void LanguageFilter::unHaxor(UnicodeString &word) {
  ...
  for (Int i = 0; i < len; ++i) {
    ...
    if ((c == L'p') || (c == L'P')) {
      if (((i + 1) < len) && ((word.getCharAt(i + 1) == L'h') ||
...
LanguageFilter *createLanguageFilter() { return NEW LanguageFilter; }
