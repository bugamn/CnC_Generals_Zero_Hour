...
const FieldParse CampaignManager::m_campaignFieldParseTable[] = {
    ...
    {"FirstMission", INI::parseAsciiString, NULL,
     offsetof(Campaign, m_firstMission)},
    {"CampaignNameLabel", INI::parseAsciiString, NULL,
     offsetof(Campaign, m_campaignNameLabel)},
    {"FinalVictoryMovie", INI::parseAsciiString, NULL,
     offsetof(Campaign, m_finalMovieName)},
...
void INI::parseCampaignDefinition(INI *ini) {
  ...
  DEBUG_ASSERTCRASH(
      TheCampaignManager,
      ("parseCampaignDefinition: Unable to Get TheCampaignManager\n"));
  ...
  // sanity
  DEBUG_ASSERTCRASH(
      campaign, ("parseCampaignDefinition: Unable to allocate campaign '%s'\n",
                 name.str()));
...
Campaign::~Campaign(void) {
  ...
  while (it != m_missions.end()) {
    ...
    if (mission) mission->deleteInstance();
  }
}

AsciiString Campaign::getFinalVictoryMovie(void) { return m_finalMovieName; }
...
Mission *Campaign::getMission(AsciiString missionName) {
  if (missionName.isEmpty()) return NULL;
  MissionListIt it;
  it = m_missions.begin();
  // we've reached the end of the campaign
  while (it != m_missions.end()) {
    Mission *mission = *it;
    if (mission->m_name.compare(missionName) == 0) return mission;
    ++it;
  }
...
Mission *Campaign::getNextMission(Mission *current) {
  ...
  while (it != m_missions.end()) {
    Mission *mission = *it;
    if (mission->m_name.compare(name) == 0) return mission;
    ++it;
  }
  //	DEBUG_ASSERTCRASH(FALSE, ("GetNextMission couldn't find %s",
  //current->m_nextMission.str()));
  return NULL;
}
...
CampaignManager::~CampaignManager(void) {
  ...
  while (it != m_campaignList.end()) {
    ...
    if (campaign) campaign->deleteInstance();
  }
}
...
//-----------------------------------------------------------------------------
Mission *CampaignManager::getCurrentMission(void) { return m_currentMission; }

//-----------------------------------------------------------------------------
Mission *CampaignManager::gotoNextMission(void) {
  if (!m_currentCampaign || !m_currentMission) return NULL;
...
AsciiString CampaignManager::getCurrentMap(void) {
  if (!m_currentMission) return AsciiString::TheEmptyString;
...
Int CampaignManager::getCurrentMissionNumber(void) {
  ...
  if (m_currentCampaign) {
    ...
    for (it = m_currentCampaign->m_missions.begin();
         it != m_currentCampaign->m_missions.end(); ++it) {
      number++;
      if (*it == m_currentMission) return number;
    }
  }
...
//-----------------------------------------------------------------------------
void CampaignManager::parseMissionPart(INI *ini, void *instance, void *store,
                                       const void *userData) {
  static const FieldParse myFieldParse[] = {
      {"Map", INI::parseAsciiString, NULL, offsetof(Mission, m_mapName)},
      {"NextMission", INI::parseAsciiString, NULL,
       offsetof(Mission, m_nextMission)},
      {"IntroMovie", INI::parseAsciiString, NULL,
       offsetof(Mission, m_movieLabel)},
      {"ObjectiveLine0", INI::parseAsciiString, NULL,
       offsetof(Mission, m_missionObjectivesLabel[0])},
      {"ObjectiveLine1", INI::parseAsciiString, NULL,
       offsetof(Mission, m_missionObjectivesLabel[1])},
      {"ObjectiveLine2", INI::parseAsciiString, NULL,
       offsetof(Mission, m_missionObjectivesLabel[2])},
      {"ObjectiveLine3", INI::parseAsciiString, NULL,
       offsetof(Mission, m_missionObjectivesLabel[3])},
      {"ObjectiveLine4", INI::parseAsciiString, NULL,
       offsetof(Mission, m_missionObjectivesLabel[4])},
      {"BriefingVoice", INI::parseAudioEventRTS, NULL,
       offsetof(Mission, m_briefingVoice)},
      {"UnitNames0", INI::parseAsciiString, NULL,
       offsetof(Mission, m_unitNames[0])},
      {"UnitNames1", INI::parseAsciiString, NULL,
       offsetof(Mission, m_unitNames[1])},
      {"UnitNames2", INI::parseAsciiString, NULL,
       offsetof(Mission, m_unitNames[2])},
      {"LocationNameLabel", INI::parseAsciiString, NULL,
       offsetof(Mission, m_locationNameLabel)},
...
void CampaignManager::xfer(Xfer *xfer) {
  ...
  if (m_currentCampaign) currentCampaign = m_currentCampaign->m_name;
  xfer->xferAsciiString(&currentCampaign);

  // current mission
  AsciiString currentMission;
  if (m_currentMission) currentMission = m_currentMission->m_name;
  xfer->xferAsciiString(&currentMission);

  // version 2 and above has rank points!
  if (version >= 2) xfer->xferInt(&m_currentRankPoints);

  if (version >= 3) xfer->xferUser(&m_difficulty, sizeof(m_difficulty));
...
//-----------------------------------------------------------------------------
Mission::Mission(void) { m_voiceLength = 0; }
