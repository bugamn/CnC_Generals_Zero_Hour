...
// static PerfTimer s_particleSys("ParticleSys::update", false,
// PERFMETRICS_LOGIC_STARTFRAME, PERFMETRICS_LOGIC_STOPFRAME);
//-------------------------------------------------------------------------------------------------
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void ParticleInfo::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void ParticleInfo::loadPostProcess(void) {}  // end loadPostProcess
...
void Particle::computeAlphaRate(void) {
  ...
  Real delta = m_alphaKey[m_alphaTargetKey].value -
               m_alphaKey[m_alphaTargetKey - 1].value;
  UnsignedInt time = m_alphaKey[m_alphaTargetKey].frame -
                     m_alphaKey[m_alphaTargetKey - 1].frame;
...
void Particle::computeColorRate(void) {
  ...
  UnsignedInt time = m_colorKey[m_colorTargetKey].frame -
                     m_colorKey[m_colorTargetKey - 1].frame;
  Real delta = m_colorKey[m_colorTargetKey].color.red -
               m_colorKey[m_colorTargetKey - 1].color.red;
  m_colorRate.red = delta / time;

  delta = m_colorKey[m_colorTargetKey].color.green -
          m_colorKey[m_colorTargetKey - 1].color.green;
  m_colorRate.green = delta / time;

  delta = m_colorKey[m_colorTargetKey].color.blue -
...
Particle::Particle(ParticleSystem *system, const ParticleInfo *info) {
  ...
  // set up alpha
  for (int i = 0; i < MAX_KEYFRAMES; i++) m_alphaKey[i] = info->m_alphaKey[i];
  ...
  // set up colors
  for (i = 0; i < MAX_KEYFRAMES; i++) m_colorKey[i] = info->m_colorKey[i];
  ...
  if (system->isUsingDrawables()) {
    const ThingTemplate *tmpl =
        TheThingFactory->findTemplate(system->getParticleTypeName());
    DEBUG_ASSERTCRASH(
        tmpl, ("Drawable %s not found\n", system->getParticleTypeName().str()));
    if (tmpl) {
      m_drawable = TheThingFactory->newDrawable(tmpl);
      if (m_drawable) m_drawable->attachToParticleSystem(this);
    }
  }
  ...
  // add this particle to the Particle System list, retaining local creation
  // order
  m_system->addParticle(this);

  // DEBUG_ASSERTLOG(!(totalParticleCount % 100 == 0), ( "TotalParticleCount =
...
Particle::~Particle() {
  ...
  if (m_drawable) TheGameClient->destroyDrawable(m_drawable);
  m_drawable = NULL;

  // if this particle was controlling another particle system, destroy that
  // system
...
Bool Particle::update(void) {
  ...
  if (m_alphaTargetKey < MAX_KEYFRAMES && m_alphaKey[m_alphaTargetKey].frame) {
    if (TheGameClient->getFrame() - m_createTimestamp >=
        m_alphaKey[m_alphaTargetKey].frame) {
  ...
  if (m_colorTargetKey < MAX_KEYFRAMES && m_colorKey[m_colorTargetKey].frame) {
    if (TheGameClient->getFrame() - m_createTimestamp >=
        m_colorKey[m_colorTargetKey].frame) {
...
void Particle::doWindMotion(void) {
  ...
  if (windMotion == ParticleSystemInfo::WIND_MOTION_NOT_USED) return;
  ...
  // when we're attached objects and drawables we offset by that position as
  // well
  ...
  //
  // compute a vector from the system position in the world to the particle ...
  // we will use this to compute how much force we apply
  //
  Coord3D v;
  ...
  //
  // given the distance from the wind position to the particle ... figure out
  // how much force we're going to apply to it.  When it's further away (outside
  // of the full force distance) we will apply only a fraction of the force
  //
...
Bool Particle::isInvisible(void) {
  ...
  switch (m_system->getShaderType()) {
    case ParticleSystemInfo::ADDITIVE:
      ...
      if (m_colorKey[m_colorTargetKey].frame == 0) {
        if (m_color.red < 0.01f && m_color.green < 0.01f &&
            m_color.blue < 0.01f)
          return true;
      }
      return false;
    ...
    case ParticleSystemInfo::MULTIPLY:
      ...
      // check that we're not in the process of going to another color
      if (m_colorKey[m_colorTargetKey].frame == 0) {
        if (m_color.red > 0.99f && m_color.green > 0.99f &&
            m_color.blue > 0.99f)
          return true;
      }
      return false;
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void Particle::crc(Xfer *xfer) {}  // end crc
...
void Particle::xfer(Xfer *xfer) {
  ...
  DrawableID drawableID =
      m_drawable ? m_drawable->getID() : INVALID_DRAWABLE_ID;
  xfer->xferDrawableID(&drawableID);
  if (xfer->getXferMode() == XFER_LOAD && drawableID != INVALID_DRAWABLE_ID) {
    //
    // save drawable pointer, note that this xfer block is after all the
    // drawable stuff so we don't need to post process anything all the correct
    // drawables are loaded and available to us
    //
    ...
    if (m_drawable == NULL) {
      DEBUG_CRASH(
          ("Particle::xfer - Unable to find matching drawable id for "
           "particle\n"));
  ...
  ParticleSystemID systemUnderControlID =
      m_systemUnderControl ? m_systemUnderControl->getSystemID()
...
void Particle::loadPostProcess(void) {
  ...
  if (m_systemUnderControlID != INVALID_PARTICLE_SYSTEM_ID) {
    ...
    // find system
    system =
        TheParticleSystemManager->findParticleSystem(m_systemUnderControlID);
    ...
    if (m_systemUnderControlID == NULL) {
      DEBUG_CRASH(
          ("Particle::loadPostProcess - Unable to find system under control "
           "pointer\n"));
      throw SC_INVALID_DATA;
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void ParticleSystemInfo::crc(Xfer *xfer) {}  // end crc
...
void ParticleSystemInfo::xfer(Xfer *xfer) {
  ...
  switch (m_emissionVelocityType) {
    // --------------------------------------------------------------------------------------------
    case ORTHO:
      xfer->xferUser(&m_emissionVelocity.ortho.x,
                     sizeof(GameClientRandomVariable));
      xfer->xferUser(&m_emissionVelocity.ortho.y,
                     sizeof(GameClientRandomVariable));
      xfer->xferUser(&m_emissionVelocity.ortho.z,
                     sizeof(GameClientRandomVariable));
      break;

    // --------------------------------------------------------------------------------------------
    case SPHERICAL:
      xfer->xferUser(&m_emissionVelocity.spherical.speed,
                     sizeof(GameClientRandomVariable));
      break;

    // --------------------------------------------------------------------------------------------
    case HEMISPHERICAL:
      xfer->xferUser(&m_emissionVelocity.hemispherical.speed,
                     sizeof(GameClientRandomVariable));
      break;

    // --------------------------------------------------------------------------------------------
    case CYLINDRICAL:
      xfer->xferUser(&m_emissionVelocity.cylindrical.radial,
                     sizeof(GameClientRandomVariable));
      xfer->xferUser(&m_emissionVelocity.cylindrical.normal,
                     sizeof(GameClientRandomVariable));
      break;

    // --------------------------------------------------------------------------------------------
    case OUTWARD:
      xfer->xferUser(&m_emissionVelocity.outward.speed,
                     sizeof(GameClientRandomVariable));
      xfer->xferUser(&m_emissionVelocity.outward.otherSpeed,
                     sizeof(GameClientRandomVariable));
      break;
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void ParticleSystemInfo::loadPostProcess(void) {}  // end loadPostProcess
...
ParticleSystem::ParticleSystem(const ParticleSystemTemplate *sysTemplate,
  ...
  ///@todo: further formalize this parameter with an UnsignedInt field in the
  ///editor
  m_volumeParticleDepth = DEFAULT_VOLUME_PARTICLE_DEPTH;
  ...
  m_windMotionStartAngle = GameClientRandomValueReal(m_windMotionStartAngleMin,
                                                     m_windMotionStartAngleMax);
  m_windMotionEndAngle = GameClientRandomValueReal(m_windMotionEndAngleMin,
                                                   m_windMotionEndAngleMax);
  m_windAngle =
      GameClientRandomValueReal(m_windMotionStartAngle, m_windMotionEndAngle);
  ...
  // DEBUG_ASSERTLOG(!(m_totalParticleSystemCount % 10 == 0), (
  // "TotalParticleSystemCount = %d\n", m_totalParticleSystemCount ));
}
...
ParticleSystem::~ParticleSystem() {
  ...
  if (m_slaveSystem) {
    DEBUG_ASSERTCRASH(
        m_slaveSystem->getMaster() == this,
        ("~ParticleSystem: Our slave doesn't have us as a master!\n"));
  ...
  if (m_masterSystem) {
    DEBUG_ASSERTCRASH(
        m_masterSystem->getSlave() == this,
        ("~ParticleSystem: Our master doesn't have us as a slave!\n"));
  ...
  while (m_systemParticlesHead) m_systemParticlesHead->deleteInstance();

  m_attachedToDrawableID = INVALID_DRAWABLE_ID;
  m_attachedToObjectID = INVALID_ID;

  // if this system was controlled by a particle, detach
  if (m_controlParticle) m_controlParticle->detachControlledParticleSystem();
...
void ParticleSystem::setMaster(ParticleSystem *master) {
  ...
  m_masterSystemID =
      master ? master->getSystemID() : INVALID_PARTICLE_SYSTEM_ID;
...
void ParticleSystem::setSaveable(Bool b) {
  m_isSaveable = b;
  if (m_slaveSystem) m_slaveSystem->setSaveable(b);
}

// ------------------------------------------------------------------------------------------------
/** (Re)start a stopped particle system */
// ------------------------------------------------------------------------------------------------
void ParticleSystem::start(void) { m_isStopped = false; }

// ------------------------------------------------------------------------------------------------
/** Stop a particle system from emitting */
// ------------------------------------------------------------------------------------------------
void ParticleSystem::stop(void) { m_isStopped = true; }
...
void ParticleSystem::destroy(void) {
  m_isDestroyed = true;
  if (m_slaveSystem) {
    m_slaveSystem->destroy();  // If we don't it will leak forever.  We are
...
const Coord3D *ParticleSystem::computeParticleVelocity(const Coord3D *pos) {
  ...
  switch (m_emissionVelocityType) {
    ...
    // "outward" velocity is directed along the surface normal of the emission
    // volume
    case OUTWARD: {
  ...
  switch (m_emissionVolumeType) {
    case CYLINDER: {
      ...
      if (m_isEmissionVolumeHollow)
        ...
        radius =
            GameClientRandomValueReal(0.0f, m_emissionVolume.cylinder.radius);
    ...
    case SPHERE: {
      ...
      if (m_isEmissionVolumeHollow)
        ...
        radius =
            GameClientRandomValueReal(0.0f, m_emissionVolume.sphere.radius);
    ...
    case BOX: {
      if (m_isEmissionVolumeHollow) {
        ...
        if (side % 3 == 0) {
          // generate X, Y
          newPos.x = GameClientRandomValueReal(-m_emissionVolume.box.halfSize.x,
                                               m_emissionVolume.box.halfSize.x);
          newPos.y = GameClientRandomValueReal(-m_emissionVolume.box.halfSize.y,
                                               m_emissionVolume.box.halfSize.y);
          if (side == 0) {
          ...
          newPos.y = GameClientRandomValueReal(-m_emissionVolume.box.halfSize.y,
                                               m_emissionVolume.box.halfSize.y);
          newPos.z = GameClientRandomValueReal(-m_emissionVolume.box.halfSize.z,
                                               m_emissionVolume.box.halfSize.z);
          if (side == 1) {
...
Particle *ParticleSystem::createParticle(const ParticleInfo *info,
                                         ...
                                         Bool forceCreate) {
  //
  // if we aren't absolutely forcing this particle to be created (which is
  // needed when loading and creating particle systems from the save games) we
  // need to check a few restrictions before this particle can really be created
  //
  if (forceCreate == FALSE) {
    if (TheGlobalData->m_useFX == FALSE) return NULL;
    ...
    //
    // Check if particle is below priorities we allow for this FPS or if it
    // being skipped because all particesl are being skipped (excluding special
    // fps independent particles at getMinDynamicParticleSkipPriority())
    //
    ...
    // ALWAYS_RENDER particles are exempt from all count limits, and are always
    // created, regardless of LOD issues.
    if (priority != ALWAYS_RENDER) {
      ...
      if (numInExcess > 0) {
        if (TheParticleSystemManager->removeOldestParticles(
          ...
          return NULL;  // could not remove enough particles, don't create new
                        // stuff
      }

      if (TheGlobalData->m_maxParticleCount == 0) return NULL;
    }
 ...
 * particleNum and particleCount are used to get 'tween frame particles emitted
 * in the correct place. (jkmcd) */
// ------------------------------------------------------------------------------------------------
const ParticleInfo *ParticleSystem::generateParticleInfo(Int particleNum,
  ...
  // NOTE: position MUST be computed before velocity, in case OUTWARD velocity
  // is specified, which must know where the particle is in space.
  ...
  if (m_isIdentity == false) {
    ...
    Coord3D
        emissionAdjustment;  // this is the adjustment for inter-frame emission
    // @todo : This should work, if m_lastPos = m_pos is removed from here but
    // it doesn't.
    ...
    emissionAdjustment.x = (1 - (INT_TO_REAL(particleNum) / particleCount)) *
                           (m_pos.x - m_lastPos.x);
    emissionAdjustment.y = (1 - (INT_TO_REAL(particleNum) / particleCount)) *
                           (m_pos.y - m_lastPos.y);
    emissionAdjustment.z = (1 - (INT_TO_REAL(particleNum) / particleCount)) *
                           (m_pos.z - m_lastPos.z);
  ...
  info.m_size =
      m_startSize.getValue() * m_sizeCoeff * TheGlobalData->m_particleScale;
  info.m_sizeRate =
      m_sizeRate.getValue() * m_sizeCoeff * TheGlobalData->m_particleScale;
  info.m_sizeRateDamping = m_sizeRateDamping.getValue();

  // Keeping a running tally makes each successive particle spawned start a bit
  // bigger (or smaller).
...
Bool ParticleSystem::update(Int localPlayerIndex) {
  if (TheGlobalData->m_useFX == FALSE) return false;
  ...
  if (m_delayLeft) {
    ...
    if (m_delayLeft == 0) m_startTimestamp = TheGameClient->getFrame();
  ...
  // if this system is attached to a Drawable/Object, update the current
  // transform matrix so generated particles' are relative to the parent
  // Drawable's position and orientation
  ...
  if (m_attachedToDrawableID) {
    Drawable *attachedTo =
        TheGameClient->findDrawableByID(m_attachedToDrawableID);

    if (attachedTo) {
      if (attachedTo->getFullyObscuredByShroud()) isShrouded = true;
  ...
  if (m_attachedToObjectID) {
    ...
    if (attachedTo) {
      if (!isShrouded)
        isShrouded = (attachedTo->getShroudedStatus(localPlayerIndex) >=
                      OBJECTSHROUD_FOGGED);
  ...
  // If we are a slave system, do not generate particles ourselves - our master
  // will force us to
  //
        ...
        if (m_burstDelayLeft == 0) {
          ...
          for (Int i = 0; i < count; i++) {
            ...
            if (!m_isEmitAboveGroundOnly ||
                (info->m_pos.z >= TheTerrainLogic->getGroundHeight(
                                      info->m_pos.x, info->m_pos.y))) {
  ...
  //
  if (m_isDestroyed && !m_systemParticlesHead) return false;

  // monitor particle system lifetime
  if (m_isForever == false) {
    // decrement lifetime if not zero
    if (m_systemLifetimeLeft) m_systemLifetimeLeft--;

    // if there are still particles "in the air", don't destroy yet
    if (getParticleCount()) return true;

    // check if time is up
    if (m_systemLifetimeLeft == 0) return false;
  }
...
void ParticleSystem::updateWindMotion(void) {
  switch (m_windMotion) {
    ...
    case ParticleSystemInfo::WIND_MOTION_PING_PONG: {
      ...
      DEBUG_ASSERTCRASH(startAngle < endAngle,
                        ("updateWindMotion: startAngle must be < endAngle\n"));

      // how big is the total angle span
      Real totalSpan = endAngle - startAngle;
      Real halfSpan = totalSpan / 2.0f;

      // given our current angle ... how far away from the "center" of the span
      // are we
      Real diffFromCenter = fabs(halfSpan - m_windAngle + startAngle);

      //
      // given our distance from the center ... we need to compute how much we
      // will change the angle.  When we are closer to the center we change it
      // faster (more), and when we are near the edges we change is slower
      // (less)
      //
      Real change = (1.0f - (diffFromCenter / halfSpan)) * m_windAngleChange;

// we will always change a little bit
#define MINIMUM_CHANGE \
  0.005f  // lower #'s have softer swings at the edge angles
      if (change < MINIMUM_CHANGE) change = MINIMUM_CHANGE;

      //
      // if we are moving toward the end angle we add the change, if we're
      // moving away from it we subtract it
      //
      if (m_windMotionMovingToEndAngle) {
        ...
        if (m_windAngle >= endAngle) {
          ...
          m_windAngleChange = GameClientRandomValueReal(m_windAngleChangeMin,
                                                        m_windAngleChangeMax);

          // pick new start and end angles
          m_windMotionStartAngle = GameClientRandomValueReal(
              m_windMotionStartAngleMin, m_windMotionStartAngleMax);
          m_windMotionEndAngle = GameClientRandomValueReal(
              m_windMotionEndAngleMin, m_windMotionEndAngleMax);
    ...
    case ParticleSystemInfo::WIND_MOTION_CIRCULAR: {
      // give us a wind angle change if one hasn't been specifed (this plays
      // nice with the particle editor)
...
void ParticleSystem::addParticle(Particle *particleToAdd) {
  if (particleToAdd->m_inSystemList) return;
...
void ParticleSystem::removeParticle(Particle *particleToRemove) {
  if (!particleToRemove->m_inSystemList) return;

  // remove links from prev & next objs
  if (particleToRemove->m_systemNext)
    particleToRemove->m_systemNext->m_systemPrev =
        particleToRemove->m_systemPrev;
  if (particleToRemove->m_systemPrev)
    particleToRemove->m_systemPrev->m_systemNext =
        particleToRemove->m_systemNext;
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
ParticleInfo ParticleSystem::mergeRelatedParticleSystems(
    ParticleSystem *masterParticleSystem, ParticleSystem *slaveParticleSystem,
    Bool slaveNeedsFullPromotion) {
  if (!masterParticleSystem || !slaveParticleSystem) {
    DEBUG_CRASH(
        ("masterParticleSystem or slaveParticleSystem was NULL. Should not "
         "happen. JKMCD"));
    ParticleInfo bogus;
  ...
  if (slaveNeedsFullPromotion) {
    ...
    slaveParticleSystem->m_emissionVelocity =
        masterParticleSystem->m_emissionVelocity;
    slaveParticleSystem->m_emissionVelocityType =
        masterParticleSystem->m_emissionVelocityType;
    slaveParticleSystem->m_emissionVolume =
        masterParticleSystem->m_emissionVolume;
    slaveParticleSystem->m_emissionVolumeType =
        masterParticleSystem->m_emissionVolumeType;
    slaveParticleSystem->m_isEmissionVolumeHollow =
        masterParticleSystem->m_isEmissionVolumeHollow;

    slaveParticleSystem->m_startSize.setRange(
        masterParticleSystem->m_startSize.getMinimumValue() *
            slaveParticleSystem->m_startSize.getMinimumValue(),
        masterParticleSystem->m_startSize.getMaximumValue() *
            slaveParticleSystem->m_startSize.getMaximumValue(),
        masterParticleSystem->m_startSize.getDistributionType());

    slaveParticleSystem->m_sizeRate.setRange(
        masterParticleSystem->m_sizeRate.getMinimumValue() *
            slaveParticleSystem->m_sizeRate.getMinimumValue(),
        masterParticleSystem->m_sizeRate.getMaximumValue() *
            slaveParticleSystem->m_sizeRate.getMaximumValue(),
        masterParticleSystem->m_sizeRate.getDistributionType());

    slaveParticleSystem->m_sizeRateDamping.setRange(
        masterParticleSystem->m_sizeRateDamping.getMinimumValue() *
            slaveParticleSystem->m_sizeRateDamping.getMinimumValue(),
        masterParticleSystem->m_sizeRateDamping.getMaximumValue() *
            slaveParticleSystem->m_sizeRateDamping.getMaximumValue(),
        masterParticleSystem->m_sizeRateDamping.getDistributionType());

    //		slaveParticleSystem->m_burstCount.setRange(masterParticleSystem->m_burstCount.getMinimumValue()
    /// 2, 																							 masterParticleSystem->m_burstCount.getMaximumValue() / 2,
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void ParticleSystem::setControlParticle(Particle *p) { m_controlParticle = p; }

// ------------------------------------------------------------------------------------------------
/** CRC */
// ------------------------------------------------------------------------------------------------
void ParticleSystem::crc(Xfer *xfer) {}  // end crc
...
void ParticleSystem::xfer(Xfer *xfer) {
  ...
  // we never save destroyed particle systems so there is no need to consider
  // m_isDestroyed m_isDestroyed <-- do nothing with me
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (particle = m_systemParticlesHead; particle;
    ...
    for (UnsignedInt i = 0; i < particleCount; ++i) {
      ...
      DEBUG_ASSERTCRASH(
          particle,
          ("ParticleSyste::xfer - Unable to create particle for loading\n"));
...
void ParticleSystem::loadPostProcess(void) {
  ...
  if (m_slaveSystemID != INVALID_PARTICLE_SYSTEM_ID) {
    ...
    if (m_slaveSystem != NULL) {
      DEBUG_CRASH(
          ("ParticleSystem::loadPostProcess - m_slaveSystem is not NULL but "
           "should be\n"));
      throw SC_INVALID_DATA;
    ...
    // assign system
    m_slaveSystem =
        TheParticleSystemManager->findParticleSystem(m_slaveSystemID);

    // sanity
    if (m_slaveSystem == NULL || m_slaveSystem->isDestroyed() == TRUE) {
      DEBUG_CRASH(
          ("ParticleSystem::loadPostProcess - m_slaveSystem is NULL or "
           "destroyed\n"));
      throw SC_INVALID_DATA;
  ...
  if (m_masterSystemID != INVALID_PARTICLE_SYSTEM_ID) {
    ...
    if (m_masterSystem != NULL) {
      DEBUG_CRASH(
          ("ParticleSystem::loadPostProcess - m_masterSystem is not NULL but "
           "should be\n"));
      throw SC_INVALID_DATA;
    ...
    // assign system
    m_masterSystem =
        TheParticleSystemManager->findParticleSystem(m_masterSystemID);

    // sanity
    if (m_masterSystem == NULL || m_masterSystem->isDestroyed() == TRUE) {
      DEBUG_CRASH(
          ("ParticleSystem::loadPostProcess - m_masterSystem is NULL or "
           "destroyed\n"));
      throw SC_INVALID_DATA;
...
const FieldParse ParticleSystemTemplate::m_fieldParseTable[] = {
    {"Priority", INI::parseIndexList, ParticlePriorityNames,
     offsetof(ParticleSystemTemplate, m_priority)},
    {"IsOneShot", INI::parseBool, NULL,
     offsetof(ParticleSystemTemplate, m_isOneShot)},
    {"Shader", INI::parseIndexList, ParticleShaderTypeNames,
     offsetof(ParticleSystemTemplate, m_shaderType)},
    {"Type", INI::parseIndexList, ParticleTypeNames,
     offsetof(ParticleSystemTemplate, m_particleType)},
    {"ParticleName", INI::parseAsciiString, NULL,
     offsetof(ParticleSystemTemplate, m_particleTypeName)},
    {"AngleX", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_angleX)},
    {"AngleY", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_angleY)},
    {"AngleZ", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_angleZ)},
    {"AngularRateX", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_angularRateX)},
    {"AngularRateY", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_angularRateY)},
    {"AngularRateZ", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_angularRateZ)},
    {"AngularDamping", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_angularDamping)},

    {"VelocityDamping", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_velDamping)},
    {"Gravity", INI::parseReal, NULL,
     offsetof(ParticleSystemTemplate, m_gravity)},
    {"SlaveSystem", INI::parseAsciiString, NULL,
     offsetof(ParticleSystemTemplate, m_slaveSystemName)},
    {"SlavePosOffset", INI::parseCoord3D, NULL,
     offsetof(ParticleSystemTemplate, m_slavePosOffset)},
    {"PerParticleAttachedSystem", INI::parseAsciiString, NULL,
     offsetof(ParticleSystemTemplate, m_attachedSystemName)},

    {"Lifetime", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_lifetime)},
    {"SystemLifetime", INI::parseUnsignedInt, NULL,
     offsetof(ParticleSystemTemplate, m_systemLifetime)},

    {"Size", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_startSize)},
    {"StartSizeRate", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_startSizeRate)},
    {"SizeRate", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_sizeRate)},
    {"SizeRateDamping", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_sizeRateDamping)},

    {"Alpha1", ParticleSystemTemplate::parseRandomKeyframe, NULL,
     offsetof(ParticleSystemTemplate, m_alphaKey[0])},
    {"Alpha2", ParticleSystemTemplate::parseRandomKeyframe, NULL,
     offsetof(ParticleSystemTemplate, m_alphaKey[1])},
    {"Alpha3", ParticleSystemTemplate::parseRandomKeyframe, NULL,
     offsetof(ParticleSystemTemplate, m_alphaKey[2])},
    {"Alpha4", ParticleSystemTemplate::parseRandomKeyframe, NULL,
     offsetof(ParticleSystemTemplate, m_alphaKey[3])},
    {"Alpha5", ParticleSystemTemplate::parseRandomKeyframe, NULL,
     offsetof(ParticleSystemTemplate, m_alphaKey[4])},
    {"Alpha6", ParticleSystemTemplate::parseRandomKeyframe, NULL,
     offsetof(ParticleSystemTemplate, m_alphaKey[5])},
    {"Alpha7", ParticleSystemTemplate::parseRandomKeyframe, NULL,
     offsetof(ParticleSystemTemplate, m_alphaKey[6])},
    {"Alpha8", ParticleSystemTemplate::parseRandomKeyframe, NULL,
     offsetof(ParticleSystemTemplate, m_alphaKey[7])},

    {"Color1", ParticleSystemTemplate::parseRGBColorKeyframe, NULL,
     offsetof(ParticleSystemTemplate, m_colorKey[0])},
    {"Color2", ParticleSystemTemplate::parseRGBColorKeyframe, NULL,
     offsetof(ParticleSystemTemplate, m_colorKey[1])},
    {"Color3", ParticleSystemTemplate::parseRGBColorKeyframe, NULL,
     offsetof(ParticleSystemTemplate, m_colorKey[2])},
    {"Color4", ParticleSystemTemplate::parseRGBColorKeyframe, NULL,
     offsetof(ParticleSystemTemplate, m_colorKey[3])},
    {"Color5", ParticleSystemTemplate::parseRGBColorKeyframe, NULL,
     offsetof(ParticleSystemTemplate, m_colorKey[4])},
    {"Color6", ParticleSystemTemplate::parseRGBColorKeyframe, NULL,
     offsetof(ParticleSystemTemplate, m_colorKey[5])},
    {"Color7", ParticleSystemTemplate::parseRGBColorKeyframe, NULL,
     offsetof(ParticleSystemTemplate, m_colorKey[6])},
    {"Color8", ParticleSystemTemplate::parseRGBColorKeyframe, NULL,
     offsetof(ParticleSystemTemplate, m_colorKey[7])},

    //	{ "COLOR",
    //ParticleSystemTemplate::parseRandomRGBColor,	NULL, offsetof(
    //ParticleSystemTemplate, m_color ) },
    {"ColorScale", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_colorScale)},

    {"BurstDelay", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_burstDelay)},
    {"BurstCount", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_burstCount)},

    {"InitialDelay", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_initialDelay)},

    {"DriftVelocity", INI::parseCoord3D, NULL,
     offsetof(ParticleSystemTemplate, m_driftVelocity)},
    {"VelocityType", INI::parseIndexList, EmissionVelocityTypeNames,
     offsetof(ParticleSystemTemplate, m_emissionVelocityType)},
    {"VelOrthoX", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_emissionVelocity.ortho.x)},
    {"VelOrthoY", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_emissionVelocity.ortho.y)},
    {"VelOrthoZ", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_emissionVelocity.ortho.z)},

    {"VelSpherical", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_emissionVelocity.spherical.speed)},
    {"VelHemispherical", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_emissionVelocity.hemispherical.speed)},

    {"VelCylindricalRadial", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_emissionVelocity.cylindrical.radial)},
    {"VelCylindricalNormal", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_emissionVelocity.cylindrical.normal)},

    {"VelOutward", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_emissionVelocity.outward.speed)},
    {"VelOutwardOther", INI::parseGameClientRandomVariable, NULL,
     offsetof(ParticleSystemTemplate, m_emissionVelocity.outward.otherSpeed)},

    {"VolumeType", INI::parseIndexList, EmissionVolumeTypeNames,
     offsetof(ParticleSystemTemplate, m_emissionVolumeType)},
    {"VolLineStart", INI::parseCoord3D, NULL,
     offsetof(ParticleSystemTemplate, m_emissionVolume.line.start)},
    {"VolLineEnd", INI::parseCoord3D, NULL,
     offsetof(ParticleSystemTemplate, m_emissionVolume.line.end)},

    {"VolBoxHalfSize", INI::parseCoord3D, NULL,
     offsetof(ParticleSystemTemplate, m_emissionVolume.box.halfSize)},

    {"VolSphereRadius", INI::parseReal, NULL,
     offsetof(ParticleSystemTemplate, m_emissionVolume.sphere.radius)},

    {"VolCylinderRadius", INI::parseReal, NULL,
     offsetof(ParticleSystemTemplate, m_emissionVolume.cylinder.radius)},
    {"VolCylinderLength", INI::parseReal, NULL,
     offsetof(ParticleSystemTemplate, m_emissionVolume.cylinder.length)},

    {"IsHollow", INI::parseBool, NULL,
     offsetof(ParticleSystemTemplate, m_isEmissionVolumeHollow)},
    {"IsGroundAligned", INI::parseBool, NULL,
     offsetof(ParticleSystemTemplate, m_isGroundAligned)},
    {"IsEmitAboveGroundOnly", INI::parseBool, NULL,
     offsetof(ParticleSystemTemplate, m_isEmitAboveGroundOnly)},
    {"IsParticleUpTowardsEmitter", INI::parseBool, NULL,
     offsetof(ParticleSystemTemplate, m_isParticleUpTowardsEmitter)},

    {"WindMotion", INI::parseIndexList, WindMotionNames,
     offsetof(ParticleSystemTemplate, m_windMotion)},

    {"WindAngleChangeMin", INI::parseReal, NULL,
     offsetof(ParticleSystemTemplate, m_windAngleChangeMin)},
    {"WindAngleChangeMax", INI::parseReal, NULL,
     offsetof(ParticleSystemTemplate, m_windAngleChangeMax)},

    {"WindPingPongStartAngleMin", INI::parseReal, NULL,
     offsetof(ParticleSystemTemplate, m_windMotionStartAngleMin)},
    {"WindPingPongStartAngleMax", INI::parseReal, NULL,
     offsetof(ParticleSystemTemplate, m_windMotionStartAngleMax)},

    {"WindPingPongEndAngleMin", INI::parseReal, NULL,
     offsetof(ParticleSystemTemplate, m_windMotionEndAngleMin)},
    {"WindPingPongEndAngleMax", INI::parseReal, NULL,
     offsetof(ParticleSystemTemplate, m_windMotionEndAngleMax)},
...
// ------------------------------------------------------------------------------------------------
/** If returns non-NULL, it is a slave system for use ... the create slaves
 * parameter tells *this* slave system whether or not it should create any
 * slaves itself automatically during its own constructor */
// ------------------------------------------------------------------------------------------------
ParticleSystem *ParticleSystemTemplate::createSlaveSystem(
    Bool createSlaves) const {
  ...
  if (m_slaveTemplate)
    slave = TheParticleSystemManager->createParticleSystem(m_slaveTemplate,
                                                           createSlaves);
...
void ParticleSystemManager::init(void) {
  ...
  INI ini;
  ini.load(AsciiString("Data\\INI\\ParticleSystem.ini"), INI_LOAD_OVERWRITE,
           NULL);
  ...
  for (Int i = 0; i < NUM_PARTICLE_PRIORITIES; ++i) {
    ...
    DEBUG_ASSERTCRASH(
        m_allParticlesHead[i] == NULL,
        ("INIT: ParticleSystem all particles head[%d] is not NULL!\n", i));
    DEBUG_ASSERTCRASH(
        m_allParticlesTail[i] == NULL,
        ("INIT: ParticleSystem all particles tail[%d] is not NULL!\n", i));
...
void ParticleSystemManager::reset(void) {
  ...
  for (Int i = 0; i < NUM_PARTICLE_PRIORITIES; ++i) {
    ...
    DEBUG_ASSERTCRASH(
        m_allParticlesHead[i] == NULL,
        ("RESET: ParticleSystem all particles head[%d] is not NULL!\n", i));
    DEBUG_ASSERTCRASH(
        m_allParticlesTail[i] == NULL,
        ("RESET: ParticleSystem all particles tail[%d] is not NULL!\n", i));
...
// ------------------------------------------------------------------------------------------------
/** Given a file containing particle system properties, create a new instance of
 * it */
// ------------------------------------------------------------------------------------------------
ParticleSystem *ParticleSystemManager::createParticleSystem(
    const ParticleSystemTemplate *sysTemplate, Bool createSlaves) {
  // sanity
  if (sysTemplate == NULL) return NULL;

  m_uniqueSystemID = (ParticleSystemID)((UnsignedInt)m_uniqueSystemID + 1);
  ParticleSystem *sys =
      newInstance(ParticleSystem)(sysTemplate, m_uniqueSystemID, createSlaves);
  return sys;
}

// ------------------------------------------------------------------------------------------------
/// given a template, instantiate a particle system attached to the given
/// object, and return its ID
// ------------------------------------------------------------------------------------------------
ParticleSystemID ParticleSystemManager::createAttachedParticleSystemID(
    const ParticleSystemTemplate *sysTemplate, Object *attachTo,
    Bool createSlaves) {
  ParticleSystem *pSystem =
      TheParticleSystemManager->createParticleSystem(sysTemplate, createSlaves);
  if (pSystem && attachTo) pSystem->attachToObject(attachTo);
...
ParticleSystem *ParticleSystemManager::findParticleSystem(ParticleSystemID id) {
  if (id == INVALID_PARTICLE_SYSTEM_ID) return NULL;  // my, that was easy

  ParticleSystem *system = NULL;

  for (ParticleSystemListIt it = m_allParticleSystemList.begin();
       it != m_allParticleSystemList.end(); ++it) {
    system = *it;
    if (!system) {
      continue;
    }
...
void ParticleSystemManager::destroyParticleSystemByID(ParticleSystemID id) {
  ...
  if (pSystem) pSystem->destroy();
}

// ------------------------------------------------------------------------------------------------
/** Locate an existing ParticleSystemTemplate */
// ------------------------------------------------------------------------------------------------
ParticleSystemTemplate *ParticleSystemManager::findTemplate(
...
// ------------------------------------------------------------------------------------------------
ParticleSystemTemplate *ParticleSystemManager::newTemplate(
...
// ------------------------------------------------------------------------------------------------
/** Find a particle system's parent. Should really only be called by
 * TheScriptEngine */
// ------------------------------------------------------------------------------------------------
...
void ParticleSystemManager::destroyAttachedSystems(Object *obj) {
  ...
  for (ParticleSystemListIt it = m_allParticleSystemList.begin();
    ...
    if (system->getAttachedObject() == obj->getID()) system->destroy();
  }
}
...
void ParticleSystemManager::addParticle(Particle *particleToAdd,
                                        ParticlePriorityType priority) {
  if (particleToAdd->m_inOverallList) return;
...
void ParticleSystemManager::removeParticle(Particle *particleToRemove) {
  if (!particleToRemove->m_inOverallList) return;
  ...
  if (particleToRemove->m_overallNext)
    particleToRemove->m_overallNext->m_overallPrev =
        particleToRemove->m_overallPrev;
  if (particleToRemove->m_overallPrev)
    particleToRemove->m_overallPrev->m_overallNext =
        particleToRemove->m_overallNext;
...
// ------------------------------------------------------------------------------------------------
void ParticleSystemManager::friend_addParticleSystem(
    ParticleSystem *particleSystemToAdd) {
...
// ------------------------------------------------------------------------------------------------
void ParticleSystemManager::friend_removeParticleSystem(
    ParticleSystem *particleSystemToRemove) {
  ParticleSystemListIt it =
      std::find(m_allParticleSystemList.begin(), m_allParticleSystemList.end(),
                particleSystemToRemove);
...
// ------------------------------------------------------------------------------------------------
/** Remove the oldest N number of particles from the lowest priority lists
 * first.  We will not remove particles from any priorities higher or equal to
 * the priorityCap parameter. */
// ------------------------------------------------------------------------------------------------
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void ParticleSystemManager::crc(Xfer *xfer) {}  // end crc
...
void ParticleSystemManager::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    ParticleSystemListIt it;
    for (it = m_allParticleSystemList.begin();
         it != m_allParticleSystemList.end(); ++it) {
      systemCount--;
      // get system
      system = *it;
      ...
      if (system->isDestroyed() == TRUE || system->isSaveable() == FALSE) {
        ...
        xfer->xferAsciiString(
            &mtString);  // write null string as key for destroyed system.
        continue;
      }
    ...
    for (UnsignedInt i = 0; i < systemCount; ++i) {
      ...
      if (systemTemplate == NULL) {
        DEBUG_CRASH(
            ("ParticleSystemManager::xfer - Unknown particle system template "
             "'%s'\n",
             systemName.str()));
        throw SC_INVALID_DATA;
      ...
      if (system == NULL) {
        DEBUG_CRASH(
            ("ParticleSystemManager::xfer - Unable to allocate particle system "
             "'%s'\n",
             systemName.str()));
        throw SC_INVALID_DATA;
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void ParticleSystemManager::loadPostProcess(void) {}  // end loadPostProcess
...
void ParticleSystemDebugDisplay(DebugDisplayInterface *dd, void *, FILE *fp) {
  ...
  ParticleSystemManager::ParticleSystemList list =
  ...
  for (it = list.begin(); it != list.end(); ++it) {
    ...
    if (templateMapIt == templateMap.end()) {
      ...
      templateMapParticleCount.insert(
          std::make_pair(templateName, (*it)->getParticleCount()));
    } else {
  ...
  for (templateMapIt = templateMap.begin(); templateMapIt != templateMap.end();
       ++templateMapIt) {
    templateMapParticleCountIt =
        templateMapParticleCount.find(templateMapIt->first);

    dd->printf("  %s: %d instances", templateMapIt->first.str(),
               templateMapIt->second);
    if (templateMapParticleCountIt != templateMapParticleCount.end())
      dd->printf("  (Avg per system %.2f)",
                 INT_TO_REAL(templateMapParticleCountIt->second) /
                     templateMapIt->second);
    dd->printf("\n");
  }
}
