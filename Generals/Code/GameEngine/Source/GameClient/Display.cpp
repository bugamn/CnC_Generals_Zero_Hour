...
void Display::drawViews(void) {
  for (View *v = m_viewList; v; v = v->getNextView()) v->drawView();
}
...
void Display::updateViews(void) {
  for (View *v = m_viewList; v; v = v->getNextView()) v->updateView();
}
...
Bool Display::setDisplayMode(UnsignedInt xres, UnsignedInt yres,
  ...
  TheTacticalView->setWidth((Real)oldViewWidth / (Real)oldDisplayWidth *
                            (Real)xres);
  TheTacticalView->setHeight((Real)oldViewHeight / (Real)oldDisplayHeight *
                             (Real)yres);
  TheTacticalView->setOrigin(
      (Real)oldViewOriginX / (Real)oldDisplayWidth * (Real)xres,
...
void Display::playLogoMovie(AsciiString movieName, Int minMovieLength,
  ...
  m_elapsedMovieTime = timeGetTime();  // we're using time get time becuase
                                       // legal want's actual "Seconds"
...
void Display::stopMovie(void) {
  ...
  if (!m_currentlyPlayingMovie.isEmpty()) {
    ...
    // Removing this sync-error cause MDC
    ...
    if (m_videoStream->isFrameReady()) {
      ...
      if (m_videoStream->frameIndex() != m_videoStream->frameCount() - 1)
        ...
        if (m_elapsedCopywriteTime == 0 && m_elapsedCopywriteTime >= 0) {
          ...
          m_copyrightDisplayString->setText(
              TheGameText->fetch("GUI:EACopyright"));
          if (TheGlobalLanguageData &&
              TheGlobalLanguageData->m_copyrightFont.name.isNotEmpty()) {
            FontDesc *fontdesc = &TheGlobalLanguageData->m_copyrightFont;
            m_copyrightDisplayString->setFont(TheFontLibrary->getFont(
                fontdesc->name,
                TheGlobalLanguageData->adjustFontSize(fontdesc->size),
                fontdesc->bold));
          } else
            m_copyrightDisplayString->setFont(TheFontLibrary->getFont(
                "Courier", TheGlobalLanguageData->adjustFontSize(12), TRUE));
...
void Display::reset() {
  ...
  for (View *v = m_viewList; v; v = v->getNextView()) v->reset();
}
