...
class IMEManager : public IMEManagerInterface {
 public:
  ...
  virtual Bool isAttachedTo(
      GameWindow *window);  ///< Is the manager attached toa window
  virtual GameWindow *getWindow(
      void);  ///< Returns the window we are currently attached to
  virtual Bool isComposing(
      void);  ///< Manager is currently composing new input string
  virtual void getCompositionString(
      UnicodeString &string);  ///< Return the current composition string
  virtual Int getCompositionCursorPosition(
      void);                       ///< Returns the composition cursor position
  ...
  virtual UnicodeString *getCandidate(
      Int index);                           ///< Returns the candidate string
  ...
  virtual Int getCandidatePageStart();  ///< Returns the index of the first
                                        ///< visibel candidate
  ...
  virtual Int result(
      void);  ///< result return value of last serviced IME message
 ...
 protected:
  ...
  WideChar convertCharToWide(
      WPARAM mbchar);  ///< Convert multibyte character to wide
  void updateCompositionString(
      void);  ///< Update the context of the composition string from the IMM
  ...
  switch (message) {
    case WM_IME_NOTIFY: {
      ...
      DEBUG_LOG(("IMM: %s(0x%04x) - %s(0x%04x) - 0x%08x\n", messageText,
                 message, notifyName, wParam, lParam));
      break;
    }
    case WM_IME_CONTROL: {
      ...
      DEBUG_LOG(("IMM: %s(0x%04x) - %s(0x%04x) - 0x%08x\n", messageText,
                 message, controlName, wParam, lParam));
      break;
    }
    ...
    case WM_IME_REQUEST: {
      ...
      DEBUG_LOG(("IMM: %s(0x%04x) - %s(0x%04x) - 0x%08x\n", messageText,
                 message, requestName, wParam, lParam));
      break;
    }
#endif
    case WM_IME_SETCONTEXT: {
      ...
      DEBUG_LOG(("IMM: %s(0x%04x) - 0x%08x - %s(0x%04x)\n", messageText,
                 message, wParam, flags.str(), lParam));
      break;
    }
    default:
      if (messageText) {
...
IMEManagerInterface *CreateIMEManagerInterface(void) { return NEW IMEManager; }
...
void IMEManager::init(void) {
  ...
  m_statusWindow =
      TheWindowManager->winCreateFromScript(AsciiString("IMEStatusWindow.wnd"));
...
Bool IMEManager::serviceIMEMessage(void *windowsHandle, UnsignedInt message,
  ...
  switch (message) {
    ...
    case WM_IME_COMPOSITION: {
      ...
      // should process this message if it displays composition characters
      // itself. Otherwise, it should send the message to the IME window. This
      // message has no return value. wParam = DBCS character. lParam = change
      // indicator.
      //
                                   ...
                                   // character is translated. -MW
      {
        if (m_window) {
          ...
          while (m_compositionCharsDisplayed >
                 0) {  // if cursor has moved since start of composition, we
                       // need to move it back using backspace message
            TheWindowManager->winSendInputMsg(m_window, GWM_CHAR, KEY_BACKSPACE,
                                              KEY_STATE_DOWN);
            m_compositionCharsDisplayed--;
          }
        ...
        if (m_window) {
          ...
          m_composing = TRUE;  // reset this flag before calling GWM_IME_CHAR
                 ...
                 GCS_COMPSTR) {  // we are supposed to display the composition
                                 // character without moving the cursor. (it's a
                                 // candidate).
        if (m_window) {
          m_composing = FALSE;  // reset this flag before calling GWM_IME_CHAR
    ...
    case WM_IME_NOTIFY: {
      ...
      switch (wParam) {
        case IMN_OPENCANDIDATE: {
          ...
          // function. The application receives this notification message
          // through the WM_IME_NOTIFY message.
          //
        ...
        case IMN_GUIDELINE:  // This message is sent when an IME is about to
                             // show an error message or other data.
        {
        ...
        case IMN_SETCONVERSIONMODE:  // This message is sent when the conversion
                                     // mode of the input context is updated.
        {
#ifdef DEBUG_IME
        ...
        case IMN_SETSENTENCEMODE:  // This message is sent when the sentence
                                   // mode of the input context is updated.
        {
#ifdef DEBUG_IME
          printSentenceStatus();
        ...
        /*
                                                // pass the reset on through to
           the installed IME case IMN_SETCANDIDATEPOS:        //This message is
           sent when the IME is about to move the candidate window. case
           IMN_SETSTATUSWINDOWPOS:     //This message is sent when the status
           window position in the input context is updated. return FALSE; case
           IMN_OPENSTATUSWINDOW:       //This message is sent when an IME is
           about to create the status window. DEBUG_LOG(("Open Status Window"));
                                                        return FALSE;
                                                case IMN_CLOSESTATUSWINDOW:
           //This message is sent to the application when an input method editor
           (IME) is about to close the status window. DEBUG_LOG(("Close Status
           Window")); return FALSE; case IMN_SETOPENSTATUS:          //This
           message is sent when the open status of the input context is updated.
                                                case IMN_SETCOMPOSITIONFONT:
           //This message is sent when the font of the input context is updated.
                                                case IMN_SETCOMPOSITIONWINDOW:
           //This message is sent when the style or position of the composition
           window is updated. case IMN_PRIVATE:                //This message is
           for your own use, it seems.
        */
        default:
          m_result = 1;
          return TRUE;
...
Int IMEManager::result(void) { return m_result; }
...
GameWindow *IMEManager::getWindow(void) { return m_window; }
  ...
  if (m_context) {
    ...
    if (result >= 0) {
      ...
      m_compositionCursorPos =
          (ImmGetCompositionStringW(m_context, GCS_CURSORPOS, NULL, 0) &
           0xffff);
    } else {
      // read MBCS instead
      ...
      if (result > 0) {
        ...
        if (convRes < 0) {
          ...
          m_compositionCursorPos =
              (ImmGetCompositionString(m_context, GCS_CURSORPOS, NULL, 0) &
               0xffff);
...
#ifdef DEBUG_IME
        DEBUG_LOG(
            ("IME: Candidate Update: Candidates = %d, pageSize = %d pageStart "
             "= %d, selected = %d\n",
