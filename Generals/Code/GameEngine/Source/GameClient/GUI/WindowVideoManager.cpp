...
//						WindowVideoManager will take
//care of setting up the window, 						creating/destroying the buffers, and the
//ability to pause/stop 						movies.
//
//	To Use:		Create a manager and initialize it. Make sure the
//manager's Update 						is called every frame or how ever often it needs to be
//updated. 						Call reset if the manager needs to be cleared. 						Play a movie in a
//window by passing a window pointer, a movie name, 						and the playtype. 						If a user
//trys to play a two different movies on the same window, 						only the last one
//added will play. 						It's important when destroying a window, that window is
//removed 						from the manager (Or call Reset if you know no other windows are
//						playing a movie).
//
//-----------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////
...
WindowVideo::~WindowVideo(void) {
  ...
  if (m_win) m_win->winGetInstanceData()->setVideoBuffer(NULL);
  m_win = NULL;
...
void WindowVideo::init(GameWindow *win, AsciiString movieName,
  ...
  if (m_win) m_win->winGetInstanceData()->setVideoBuffer(m_videoBuffer);
}
...
void WindowVideo::setWindowState(WindowVideoStates state) {
  ...
  if ((m_state == WINDOW_VIDEO_STATE_PLAY ||
...
void WindowVideoManager::update(void) {
  ...
  if (m_pauseAllMovies || m_stopAllMovies) return;
  ...
  while (it != m_playingVideos.end()) {
    ...
    if (winVid->getState() == WINDOW_VIDEO_STATE_HIDDEN &&
        (win->winIsHidden() == FALSE)) {
      resumeMovie(win);
    }
...
void WindowVideoManager::playMovie(GameWindow *win, AsciiString movieName,
...
void WindowVideoManager::pauseMovie(GameWindow *win) {
  ...
  if (it != m_playingVideos.end()) {
    WindowVideo *winVid = it->second;
    if (winVid) winVid->setWindowState(WINDOW_VIDEO_STATE_PAUSE);
  }
}
void WindowVideoManager::hideMovie(GameWindow *win) {
  ...
  if (it != m_playingVideos.end()) {
    WindowVideo *winVid = it->second;
    if (winVid) winVid->setWindowState(WINDOW_VIDEO_STATE_HIDDEN);
  }
}
...
void WindowVideoManager::resumeMovie(GameWindow *win) {
  ...
  if (it != m_playingVideos.end()) {
    WindowVideo *winVid = it->second;
    if (winVid) winVid->setWindowState(WINDOW_VIDEO_STATE_PLAY);
  }
  m_pauseAllMovies = FALSE;
  m_stopAllMovies = FALSE;
}
...
void WindowVideoManager::stopMovie(GameWindow *win) {
  ...
  if (it != m_playingVideos.end()) {
    WindowVideo *winVid = it->second;
    if (winVid) winVid->setWindowState(WINDOW_VIDEO_STATE_STOP);
  }
}
...
Int WindowVideoManager::getWinState(GameWindow *win) {
  ...
  if (it != m_playingVideos.end()) {
    WindowVideo *winVid = it->second;
    if (winVid) return winVid->getState();
  }
