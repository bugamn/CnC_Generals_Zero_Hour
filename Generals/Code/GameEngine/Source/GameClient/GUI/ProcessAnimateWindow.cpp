...
//
//	purpose:	Holds all the process modules for the Animate Window
//Class
//
//-----------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////
...
ProcessAnimateWindowSlideFromRight::ProcessAnimateWindowSlideFromRight(void) {
  ...
  m_slowDownRatio =
      0.67f;  // how fast the windows slow down (smaller slows quicker)
...
void ProcessAnimateWindowSlideFromRight::initReverseAnimateWindow(
    AnimateWindow *animWin, UnsignedInt maxDelay) {
  if (!animWin) {
    DEBUG_ASSERTCRASH(animWin, ("animWin was passed into initAnimateWindow as "
                                "a NULL Pointer... bad bad bad!"));
    return;
  }
...
//-----------------------------------------------------------------------------
void ProcessAnimateWindowSlideFromRight::initAnimateWindow(
  ...
  // it's set that the window is passed in as it's current position being it's
  // rest position so save off the rest position
  GameWindow *win = animWin->getGameWindow();
  if (!win) {
    DEBUG_ASSERTCRASH(win, ("animWin contains a NULL Pointer for it's "
                            "GameWindow... Whatup wit dat?"));
    return;
  }
  ...
  // set the initial positions for the window. In this case, off the Right of
  // the screen
  ...
  animWin->setAnimData(startPos, endPos, curPos, restPos, vel,
                       timeGetTime() + animWin->getDelay(), 0);
}

//-----------------------------------------------------------------------------
Bool ProcessAnimateWindowSlideFromRight::updateAnimateWindow(
    AnimateWindow *animWin) {
  if (!animWin) {
    DEBUG_ASSERTCRASH(animWin, ("animWin was passed into updateAnimateWindow "
                                "as a NULL Pointer... bad bad bad!"));
    return TRUE;
  }

  // if the window has finished animating into position, return
  if (animWin->isFinished()) return TRUE;

  // if the window hasn't started animating...return that we're not finished
  if (timeGetTime() < animWin->getStartTime()) return FALSE;
  // it's set that the window is passed in as it's current position being it's
  // rest position so save off the rest position
  GameWindow *win = animWin->getGameWindow();
  if (!win) {
...
Bool ProcessAnimateWindowSlideFromRight::reverseAnimateWindow(
    AnimateWindow *animWin) {
  if (!animWin) {
    DEBUG_ASSERTCRASH(animWin, ("animWin was passed into updateAnimateWindow "
                                "as a NULL Pointer... bad bad bad!"));
  ...
  if (vel.x > -m_maxVel.x) vel.x = -m_maxVel.x;
  animWin->setVel(vel);
  return FALSE;
}
...
void ProcessAnimateWindowSlideFromLeft::initAnimateWindow(
    AnimateWindow *animWin) {
  ICoord2D restPos = {0, 0};
  ICoord2D startPos = {0, 0};
  ICoord2D curPos = {0, 0};
  ...
  // set the initial positions for the window. In this case, off the Left of the
  // screen
  Int travelDistance = TheDisplay->getWidth();  // / 4 * 3;
  startPos.x = curPos.x = restPos.x - travelDistance;
  startPos.y = curPos.y = restPos.y;
...
Bool ProcessAnimateWindowSlideFromLeft::updateAnimateWindow(
    AnimateWindow *animWin) {
  if (!animWin) {
    DEBUG_ASSERTCRASH(animWin, ("animWin was passed into updateAnimateWindow "
                                "as a NULL Pointer... bad bad bad!"));
...
Bool ProcessAnimateWindowSlideFromLeft::reverseAnimateWindow(
    AnimateWindow *animWin) {
  if (!animWin) {
    DEBUG_ASSERTCRASH(animWin, ("animWin was passed into updateAnimateWindow "
                                "as a NULL Pointer... bad bad bad!"));
...
void ProcessAnimateWindowSlideFromTop::initAnimateWindow(
    AnimateWindow *animWin) {
  ICoord2D restPos = {0, 0};
  ICoord2D startPos = {0, 0};
  ICoord2D curPos = {0, 0};
  ...
  // set the initial positions for the window. In this case, off the Top of the
  // screen
  Int travelDistance = TheDisplay->getWidth();  // / 4 * 3;
...
Bool ProcessAnimateWindowSlideFromTop::updateAnimateWindow(
    AnimateWindow *animWin) {
  if (!animWin) {
    DEBUG_ASSERTCRASH(animWin, ("animWin was passed into updateAnimateWindow "
                                "as a NULL Pointer... bad bad bad!"));
...
Bool ProcessAnimateWindowSlideFromTop::reverseAnimateWindow(
    AnimateWindow *animWin) {
  if (!animWin) {
    DEBUG_ASSERTCRASH(animWin, ("animWin was passed into updateAnimateWindow "
                                "as a NULL Pointer... bad bad bad!"));
  ...
  if (vel.y < -m_maxVel.y) vel.y = -m_maxVel.y;
  animWin->setVel(vel);
  return FALSE;
}
...
void ProcessAnimateWindowSlideFromBottom::initAnimateWindow(
    AnimateWindow *animWin) {
  ICoord2D restPos = {0, 0};
  ICoord2D startPos = {0, 0};
  ICoord2D curPos = {0, 0};
  ...
  // set the initial positions for the window. In this case, off the Bottom of
  // the screen
  Int travelDistance = TheDisplay->getWidth();  // / 4 * 3;
  startPos.x = curPos.x = restPos.x;
  startPos.y = curPos.y = restPos.y + travelDistance;
...
Bool ProcessAnimateWindowSlideFromBottom::updateAnimateWindow(
    AnimateWindow *animWin) {
  if (!animWin) {
    DEBUG_ASSERTCRASH(animWin, ("animWin was passed into updateAnimateWindow "
                                "as a NULL Pointer... bad bad bad!"));
...
Bool ProcessAnimateWindowSlideFromBottom::reverseAnimateWindow(
    AnimateWindow *animWin) {
  if (!animWin) {
    DEBUG_ASSERTCRASH(animWin, ("animWin was passed into updateAnimateWindow "
                                "as a NULL Pointer... bad bad bad!"));
...
void ProcessAnimateWindowSlideFromBottomTimed::initReverseAnimateWindow(
    AnimateWindow *animWin, UnsignedInt maxDelay) {
  ICoord2D restPos = {0, 0};
  ICoord2D startPos = {0, 0};
  ICoord2D curPos = {0, 0};
  ...
  DEBUG_LOG(("initReverseAnimateWindow at %d (%d->%d)\n", now, now,
             now + m_maxDuration));
  animWin->setAnimData(startPos, endPos, curPos, restPos, vel, now,
                       now + m_maxDuration);
}

void ProcessAnimateWindowSlideFromBottomTimed::initAnimateWindow(
    AnimateWindow *animWin) {
  ICoord2D restPos = {0, 0};
  ICoord2D startPos = {0, 0};
  ICoord2D curPos = {0, 0};
  ...
  DEBUG_LOG(("initAnimateWindow at %d (%d->%d)\n", now, now + delay,
             now + m_maxDuration + delay));
  animWin->setAnimData(startPos, endPos, curPos, restPos, vel, now + delay,
                       now + m_maxDuration + delay);
}

Bool ProcessAnimateWindowSlideFromBottomTimed::updateAnimateWindow(
    AnimateWindow *animWin) {
  if (!animWin) {
    DEBUG_ASSERTCRASH(animWin, ("animWin was passed into updateAnimateWindow "
                                "as a NULL Pointer... bad bad bad!"));
  ...
  DEBUG_LOG(("(%d,%d) -> (%d,%d) -> (%d,%d) at %g\n", startPos.x, startPos.y,
             curPos.x, curPos.y, endPos.x, endPos.y, percentDone));
...
Bool ProcessAnimateWindowSlideFromBottomTimed::reverseAnimateWindow(
    AnimateWindow *animWin) {
...
//-----------------------------------------------------------------------------
void ProcessAnimateWindowSpiral::initReverseAnimateWindow(
    AnimateWindow *animWin, UnsignedInt maxDelay) {
  if (!animWin) {
    DEBUG_ASSERTCRASH(animWin, ("animWin was passed into initAnimateWindow as "
                                "a NULL Pointer... bad bad bad!"));
...
void ProcessAnimateWindowSlideFromTopFast::initAnimateWindow(
    AnimateWindow *animWin) {
  ICoord2D restPos = {0, 0};
  ICoord2D startPos = {0, 0};
  ICoord2D curPos = {0, 0};
  ...
  // set the initial positions for the window. In this case, off the Top of the
  // screen Int travelDistance = TheDisplay->getWidth();// / 4 * 3;
  startPos.x = curPos.x = restPos.x;
...
Bool ProcessAnimateWindowSlideFromTopFast::updateAnimateWindow(
    AnimateWindow *animWin) {
  if (!animWin) {
    DEBUG_ASSERTCRASH(animWin, ("animWin was passed into updateAnimateWindow "
                                "as a NULL Pointer... bad bad bad!"));
...
Bool ProcessAnimateWindowSlideFromTopFast::reverseAnimateWindow(
    AnimateWindow *animWin) {
  if (!animWin) {
    DEBUG_ASSERTCRASH(animWin, ("animWin was passed into updateAnimateWindow "
                                "as a NULL Pointer... bad bad bad!"));
...
ProcessAnimateWindowSlideFromRightFast::ProcessAnimateWindowSlideFromRightFast(
  ...
  m_slowDownThreshold = 60;  // when widnows get this close to their resting
                             // positions they start to slow down
  m_slowDownRatio =
      0.77f;  // how fast the windows slow down (smaller slows quicker)
...
//-----------------------------------------------------------------------------
void ProcessAnimateWindowSlideFromRightFast::initAnimateWindow(
    AnimateWindow *animWin) {
  ICoord2D restPos = {0, 0};
  ICoord2D startPos = {0, 0};
  ICoord2D size = {0, 0};
...
//-----------------------------------------------------------------------------
Bool ProcessAnimateWindowSlideFromRightFast::updateAnimateWindow(
    AnimateWindow *animWin) {
  if (!animWin) {
    DEBUG_ASSERTCRASH(animWin, ("animWin was passed into updateAnimateWindow "
                                "as a NULL Pointer... bad bad bad!"));
...
Bool ProcessAnimateWindowSlideFromRightFast::reverseAnimateWindow(
    AnimateWindow *animWin) {
  if (!animWin) {
    DEBUG_ASSERTCRASH(animWin, ("animWin was passed into updateAnimateWindow "
                                "as a NULL Pointer... bad bad bad!"));
