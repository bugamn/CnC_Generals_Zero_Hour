...
const FieldParse
    GameWindowTransitionsHandler::m_gameWindowTransitionsFieldParseTable[] = {

        {"Window", GameWindowTransitionsHandler::parseWindow, NULL, NULL},
        {"FireOnce", INI::parseBool, NULL,
         offsetof(TransitionGroup, m_fireOnce)},
...
void INI::parseWindowTransitions(INI *ini) {
  ...
  DEBUG_ASSERTCRASH(
      ...
      ("parseWindowTransitions: TheTransitionHandler doesn't exist yet\n"));
  if (!TheTransitionHandler) return;
  ...
  DEBUG_ASSERTCRASH(
      g,
      ("parseWindowTransitions: Unable to allocate group '%s'\n", name.str()));
...
Transition *getTransitionForStyle(Int style) {
  switch (style) {
    ...
    default:
      DEBUG_ASSERTCRASH(
          FALSE,
          ("getTransitionForStyle:: An invalid style was passed in. Style = %d",
           style));
      return NULL;
  }
  return NULL;
...
TransitionWindow::~TransitionWindow(void) {
  m_win = NULL;
  if (m_transition) delete m_transition;
...
Bool TransitionWindow::init(void) {
  ...
  //	DEBUG_ASSERTCRASH( m_win, ("TransitionWindow::init Failed to find window
  //%s", m_winName.str())); 	if( !m_win ) 		return FALSE;
...
void TransitionWindow::update(Int frame) {
  ...
  if (m_transition) m_transition->update(frame - m_currentFrameDelay);
}

Bool TransitionWindow::isFinished(void) {
  if (m_transition) return m_transition->isFinished();
  return TRUE;
}

void TransitionWindow::reverse(Int totalFrames) {
  // m_currentFrameDelay = totalFrames - (m_transition->getFrameLength() +
  // m_frameDelay);
  if (m_transition) m_transition->reverse();
}

void TransitionWindow::skip(void) {
  if (m_transition) m_transition->skip();
}
...
void TransitionGroup::update(void) {
  m_currentFrame +=
      m_directionMultiplier;  // we go forward or backwards depending.
  TransitionWindowList::iterator it = m_transitionWindowList.begin();
  while (it != m_transitionWindowList.end()) {
    TransitionWindow *tWin = *it;
...
Bool TransitionGroup::isFinished(void) {
  ...
  while (it != m_transitionWindowList.end()) {
    TransitionWindow *tWin = *it;
    if (tWin->isFinished() == FALSE) return FALSE;
    it++;
  }
...
void TransitionGroup::reverse(void) {
  ...
  while (it != m_transitionWindowList.end()) {
    ...
    if (winFrames > totalFrames) totalFrames = winFrames;
    it++;
  }
...
Bool TransitionGroup::isReversed(void) {
  if (m_directionMultiplier < 0) return TRUE;
  return FALSE;
}
...
void GameWindowTransitionsHandler::load(void) {
  ...
  ini.load(AsciiString("Data\\INI\\WindowTransitions.ini"), INI_LOAD_OVERWRITE,
           NULL);
}
...
void GameWindowTransitionsHandler::update(void) {
  ...
  if (m_currentGroup && !m_currentGroup->isFinished()) m_currentGroup->update();
...
void GameWindowTransitionsHandler::setGroup(AsciiString groupName,
  ...
  if (m_currentGroup) {
    ...
    if (m_pendingGroup) m_pendingGroup->init();
    return;
  }
...
void GameWindowTransitionsHandler::reverse(AsciiString groupName) {
  ...
  if (m_currentGroup) m_currentGroup->skip();
  if (m_pendingGroup) m_pendingGroup->skip();
...
void GameWindowTransitionsHandler::remove(AsciiString groupName,
  ...
  TransitionGroup *g = findGroup(groupName);
  if (m_pendingGroup == g) {
    if (skipPending) m_pendingGroup->skip();
  ...
  if (m_currentGroup == g) {
    m_currentGroup->skip();
    m_currentGroup = NULL;
    if (m_pendingGroup) m_currentGroup = m_pendingGroup;
  }
}
...
TransitionGroup *GameWindowTransitionsHandler::getNewGroup(AsciiString name) {
  ...
  if (findGroup(name)) {
    DEBUG_ASSERTCRASH(FALSE, ("GameWindowTransitionsHandler::getNewGroup - We "
                              "already have a group %s",
                              name.str()));
    return NULL;
  }
...
Bool GameWindowTransitionsHandler::isFinished(void) {
  if (m_currentGroup) return m_currentGroup->isFinished();
  return TRUE;
}
...
// PRIVATE FUNCTIONS //////////////////////////////////////////////////////////
//-----------------------------------------------------------------------------
TransitionGroup *GameWindowTransitionsHandler::findGroup(
  ...
  while (it != m_transitionGroupList.end()) {
    TransitionGroup *g = *it;
    if (groupName.compareNoCase(g->getName()) == 0) return g;
    it++;
  }
  return NULL;
}
...
void GameWindowTransitionsHandler::parseWindow(INI *ini, void *instance,
  ...
  static const FieldParse myFieldParse[] = {
      {"WinName", INI::parseAsciiString, NULL,
       offsetof(TransitionWindow, m_winName)},
      {"Style", INI::parseLookupList, TransitionStyleNames,
       offsetof(TransitionWindow, m_style)},
      {"FrameDelay", INI::parseInt, NULL,
       offsetof(TransitionWindow, m_frameDelay)},
      {NULL, NULL, NULL, 0}  // keep this last
  };
