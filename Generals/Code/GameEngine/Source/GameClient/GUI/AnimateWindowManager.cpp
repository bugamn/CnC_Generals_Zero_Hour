...
//
//	purpose:	This will contain the logic behind the different
//animations that 						can happen to a window.
//
//-----------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////
...
AnimateWindow::~AnimateWindow(void) { m_win = NULL; }

void AnimateWindow::setAnimData(ICoord2D startPos, ICoord2D endPos,
                                ICoord2D curPos, ICoord2D restPos, Coord2D vel,
...
AnimateWindowManager::~AnimateWindowManager(void) {
  if (m_slideFromRight) delete m_slideFromRight;
  if (m_slideFromRightFast) delete m_slideFromRightFast;
  if (m_slideFromLeft) delete m_slideFromLeft;
  if (m_slideFromTop) delete m_slideFromTop;
  if (m_slideFromTopFast) delete m_slideFromTopFast;
  if (m_slideFromBottom) delete m_slideFromBottom;
  if (m_spiral) delete m_spiral;
  if (m_slideFromBottomTimed) delete m_slideFromBottomTimed;
...
void AnimateWindowManager::update(void) {
  ...
  if (m_needsUpdate) {
    ...
    while (it != m_winMustFinishList.end()) {
      ...
      if (processAnim) {
        if (m_reverse) {
          if (!processAnim->reverseAnimateWindow(animWin)) m_needsUpdate = TRUE;
        } else {
          if (!processAnim->updateAnimateWindow(animWin)) m_needsUpdate = TRUE;
        }
      }
  ...
  while (it != m_winList.end()) {
    ...
    processAnim = getProcessAnimate(animWin->getAnimType());
    if (m_reverse) {
      if (processAnim) processAnim->reverseAnimateWindow(animWin);
    } else {
      if (processAnim) processAnim->updateAnimateWindow(animWin);
    }
    it++;
  }
}
...
void AnimateWindowManager::registerGameWindow(GameWindow *win,
  ...
  if (!win) {
    DEBUG_CRASH(
        ("Win was NULL as it was passed into registerGameWindow... not good "
         "indeed"));
    return;
  }
  if (animType <= WIN_ANIMATION_NONE || animType >= WIN_ANIMATION_COUNT) {
    DEBUG_CRASH(
        ("an Invalid WIN_ANIMATION type was passed into registerGameWindow... "
         "please fix me "));
    return;
  }
...
void AnimateWindowManager::reverseAnimateWindow(void) {
  ...
  while (it != m_winMustFinishList.end()) {
    ...
    if (animWin->getDelay() > maxDelay) maxDelay = animWin->getDelay();
    it++;
  }
  ...
  while (it != m_winList.end()) {
    ...
    if (processAnim) processAnim->initReverseAnimateWindow(animWin);
    animWin->setFinished(FALSE);
    it++;
  }
}
...
void AnimateWindowManager::resetToRestPosition(void) {
  ...
  while (it != m_winMustFinishList.end()) {
    ...
    if (win) win->winSetPosition(restPos.x, restPos.y);
    it++;
  }
  it = m_winList.begin();
  while (it != m_winList.end()) {
