...
struct LayoutScriptParse {
  char *name;
  Bool (*parse)(char *token, char *buffer, UnsignedInt version,
...
//
const char *WindowStatusNames[] = {"ACTIVE",        "TOGGLE",
                                   "DRAGABLE",      "ENABLED",
                                   "HIDDEN",        "ABOVE",
                                   "BELOW",         "IMAGE",
                                   "TABSTOP",       "NOINPUT",
...
WinDrawData disabledUpButtonDrawData[MAX_DRAW_DATA];  ///< for list boxes and
                                                      ///< combo boxes
WinDrawData
...
WinDrawData disabledDownButtonDrawData[MAX_DRAW_DATA];  ///< for list boxes and
                                                        ///< combo boxes
WinDrawData hiliteDownButtonDrawData[MAX_DRAW_DATA];    ///< for list boxes and
                                                        ///< combo boxes
WinDrawData
...
WinDrawData
    disabledSliderThumbDrawData[MAX_DRAW_DATA];  ///< for sliders and list boxes
                                                 ///< and combo boxes
WinDrawData
    hiliteSliderThumbDrawData[MAX_DRAW_DATA];  ///< for sliders and list boxes
                                               ///< and combo boxes
...
static GameWindow *peekWindow(void) {
  if (stackPtr == windowStack) return NULL;
...
//=============================================================================
static Bool parseTooltip(char *token, WinInstanceData *instData, char *buffer,
                         void *data) {
 ...
 * if present */
//=============================================================================
static Bool parseScreenRect(char *token, char *buffer, Int *x, Int *y,
...
//=============================================================================
static Bool parseFont(char *token, WinInstanceData *instData, char *buffer,
                      void *data) {
  char *c, *ptr;
  char *seps = " ,\n\r\t";
  char *stringSeps = ":,\n\r\t\"";
...
//=============================================================================
static Bool parseName(char *token, WinInstanceData *instData, char *buffer,
                      void *data) {
  char *c, *ptr;
  //	char *seps = " ,\n\r\t";
  char *stringSeps = "\"";
...
//=============================================================================
static Bool parseStatus(char *token, WinInstanceData *instData, char *buffer,
                        void *data) {
  instData->m_status = 0;
...
//=============================================================================
static Bool parseStyle(char *token, WinInstanceData *instData, char *buffer,
                       void *data) {
  instData->m_style = 0;
...
//=============================================================================
static Bool parseText(char *token, WinInstanceData *instData, char *buffer,
                      void *data) {
  char *ptr = buffer;
  char *c;
  char *stringSeps = "\n\r\t\"";
  ...
  if (strlen(c) >= MAX_TEXT_LABEL) {
    DEBUG_LOG(
        ("Text label '%s' is too long, max is '%d'\n", c, MAX_TEXT_LABEL));
    assert(0);
    return FALSE;
 ...
 * drop shadow colors */
//=============================================================================
static Bool parseTextColor(char *token, WinInstanceData *instData, char *buffer,
                           void *data) {
  char *c;
  char *seps = " :,\n\r\t";
  UnsignedInt r, g, b, a;
...
//=============================================================================
static Bool parseDrawData(char *token, WinInstanceData *instData, char *buffer,
                          void *data) {
  Int i;
  UnsignedInt r, g, b, a;
  ...
  if (!strcmp(type, "VERTSLIDER") || !strcmp(type, "HORZSLIDER")) {
    ...
    *data = &lData;
    ...
    if (c) {
      ...
      if (eData.secretText != FALSE) eData.secretText = TRUE;
    } else
    ...
    *data = &eData;
    ...
    if (tData.centered != FALSE) tData.centered = TRUE;
...
static GameWindow *createGadget(char *type, GameWindow *parent, Int status,
  ...
  if (!strcmp(type, "PUSHBUTTON")) {
    ...
    c = strchr(filename, ':');
    if (c) *c = 0;  // terminate after filename (format is filename:gadgetname)
    assert(TheNameKeyGenerator);
    if (TheNameKeyGenerator)
      rData->screen =
          (Int)(TheNameKeyGenerator->nameToKey(AsciiString(filename)));
...
static GameWindow *createWindow(
    char *type, Int id, Int status, Int x, Int y, Int width, Int height,
    WinInstanceData *instData, void *data, GameWinSystemFunc system,
    GameWinInputFunc input, GameWinTooltipFunc tooltip, GameWinDrawFunc draw) {
  ...
  if (window) {
    if (system) window->winSetSystemFunc(system);
    if (input) window->winSetInputFunc(input);
    if (tooltip) window->winSetTooltipFunc(tooltip);
    if (draw) window->winSetDrawFunc(draw);
...
static Bool parseChildWindows(GameWindow *window, File *inFile, char *buffer) {
  ...
  if (lastWindow != window) {
    DEBUG_LOG(
        ("parseChildWindows: unmatched window on stack.  Corrupt stack or bad "
         "source\n"));
    return FALSE;
  }
  ...
  if (BitTest(window->winGetStyle(), GWS_TAB_CONTROL))
    GadgetTabControlFixupSubPaneList(
        window);  // all children created, so re-fill SubPane array with
                  // children
  ...
  while (TRUE) {
    ...
    if (parse->parse == NULL) {
      ...
      if (asciibuf.compare("DATA") == 0) {
        ...
        if (window == NULL) goto cleanupAndExit;
...
//-------------------------------------------------------------------------------------------------
Bool parseInit(char *token, char *buffer, UnsignedInt version,
               WindowLayoutInfo *info) {
  char *c;
  char *seps = " \n\r\t";
...
//-------------------------------------------------------------------------------------------------
Bool parseUpdate(char *token, char *buffer, UnsignedInt version,
                 WindowLayoutInfo *info) {
  char *c;
  char *seps = " \n\r\t";
...
//-------------------------------------------------------------------------------------------------
Bool parseShutdown(char *token, char *buffer, UnsignedInt version,
                   WindowLayoutInfo *info) {
  char *c;
  char *seps = " \n\r\t";
  ...
  while (TRUE) {
    ...
    for (parse = layoutScriptTable; parse && parse->name; parse++) {
      if (asciitoken.compare(parse->name) == 0) {
        ...
        // parse it
        if (parse->parse(token, c, version, info) == FALSE) return FALSE;
