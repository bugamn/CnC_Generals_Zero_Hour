...
// //////////////////////////////////////////////////////////////// Author:
// Colin Day, March 2002 Desc:   Context sensitive GUI for when you select
// mutiple objects.  What we do is show
...
void ControlBar::addCommonCommands(Drawable *draw, Bool firstDrawable) {
  ...
  if (commandSet == NULL) {
    //
    // if there is no command set for this drawable, none of the selected
    // drawables can possibly have matching commands so we'll get rid of them
    // all
    //
    for (i = 0; i < MAX_COMMANDS_PER_SET; i++) {
      m_commonCommands[i] = NULL;
      ...
      // missing blanks with the images
  ...
  //
  // easy case, if we're adding the first drawable we simply just add any of the
  // commands in its set that can be multi-select commands to the common command
  // set
  //
  if (firstDrawable == TRUE) {
    ...
    for (i = 0; i < MAX_COMMANDS_PER_SET; i++) {
      ...
      if (command &&
          BitTest(command->getOptions(), OK_FOR_MULTI_SELECT) == TRUE) {
    ...
    for (i = 0; i < MAX_COMMANDS_PER_SET; i++) {
      ...
      Bool attackMove =
          (command && command->getCommandType() == GUI_COMMAND_ATTACK_MOVE) ||
          (m_commonCommands[i] &&
           m_commonCommands[i]->getCommandType() == GUI_COMMAND_ATTACK_MOVE);

      // Kris: When any units have attack move, they all get it. This is to
      // allow combat units to be selected with the odd dozer or pilot and still
      // retain that ability.
      if (attackMove && !m_commonCommands[i]) {
        ...
        //
        // if this command does not match the command that is in the common
        // command set then *neither* this command OR the command in the common
        // command set are really common commands, so we will remove the one
        // that has been stored in the common set
        //
        ...
        //
        // hide the window control cause it should have been made visible from a
        // command that was placed in this common 'slot' earlier
        //
        m_commandWindows[i]->winHide(TRUE);
      }
...
//-------------------------------------------------------------------------------------------------
/** Populate the visible command bar with commands that are common to all the
 * objects that are selected in the UI */
//-------------------------------------------------------------------------------------------------
void ControlBar::populateMultiSelect(void) {
  ...
  DEBUG_ASSERTCRASH(TheInGameUI->getSelectCount() > 1,
                    ("populateMultiSelect: Can't populate multiselect context "
                     "cause there are only '%d' things selected\n",
  ...
  // sanity
  DEBUG_ASSERTCRASH(selectedDrawables->empty() == FALSE,
                    ("populateMultiSelect: Drawable list is empty\n"));
  ...
  for (DrawableListCIt it = selectedDrawables->begin();
    ...
    if (draw->getObject()->isKindOf(
            KINDOF_IGNORED_IN_GUI))  // ignore these guys
      continue;

    //
    // add command for this drawable, note that we also sanity check to make
    // sure the drawable has an object as all interesting drawables that we can
    // select should actually have an object underneath it so that we can do
    // interesting things with it ... otherwise we should have never selected
    // it. NOTE that we're not considering objects that are currently in the
    // process of being sold as those objects can't be issued anymore commands
    //
    if (draw && draw->getObject() &&
        BitTest(draw->getObject()->getStatusBits(), OBJECT_STATUS_SOLD) ==
            FALSE) {
      // add the common commands of this drawable to the common command set
      ...
      //
      // keep track of the portrait images, if all units selected have the same
      // portrait we will display it in the right HUD, otherwise we won't
      //
...
void ControlBar::updateContextMultiSelect(void) {
  ...
  for (DrawableListCIt it = selectedDrawables->begin();
    ...
    // for each of the visible command windows make sure the object can execute
    // the command
    for (i = 0; i < MAX_COMMANDS_PER_SET; i++) {
  ...
  //
  // for each command, if any objects can do the command we enable the window,
  // otherwise we disable it
  //
  for (i = 0; i < MAX_COMMANDS_PER_SET; i++) {
    // don't consider hidden commands
    if (m_commandWindows[i]->winIsHidden() == TRUE) continue;

    // don't consider slots that don't have commands
    if (m_commonCommands[i] == NULL) continue;

    // check the count of objects that can do the command and enable/disable the
    // control,
