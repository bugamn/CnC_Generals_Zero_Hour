...
// //////////////////////////////////////////////////////////////////// Author:
// Colin Day, March 2002 Desc:   Methods specific to the control bar unit
// commands
///////////////////////////////////////////////////////////////////////////////////////////////////
...
//-------------------------------------------------------------------------------------------------
/** Note, this iterate callback assumes that the inventory exit buttons appear
 * in a continuous order in the layout of the command set */
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
/** Used for the callback iterator on transport contents to do the actual GUI
 * fill */
//-------------------------------------------------------------------------------------------------
void ControlBar::populateInvDataCallback(Object *obj, void *userData) {
  ...
  //
  // if we're beyond the max the GUI can support, design needs to change the
  // parameters of the transport object to carry less things
  //
  ...
  // obj->getTemplate()->getInventoryImageName( INV_IMAGE_HILITE ) );
  // GadgetButtonSetHiliteImage( control, image );
  // image = TheMappedImageCollection->findImageByName(
  // obj->getTemplate()->getInventoryImageName( INV_IMAGE_PUSHED ) );
...
//-------------------------------------------------------------------------------------------------
/** Transports have an extra special manipulation of the user interface.  They
 * get to look at the available command set, and any of the commands that are
 * TransportExit commands *AND* there is actually an object to represent that
 * slot contained in the transport, the inventory picture of the contained
 * object will be displayed in the window control for that TransportExit
 * command.  Also, transports will HIDE any TransportExit controls found in the
 * command set that represent slots that *DO NOT EXIST* for the transport (that
 * is, the transport can only hold 4 things, but the GUI has buttons for 8
 * things).  For slots that are empty but present in the transport the UI will
 * show a disabled button to show the user that there is an open "slot" */
//-------------------------------------------------------------------------------------------------
void ControlBar::doTransportInventoryUI(Object *transport,
  ...
  // static const CommandButton *exitCommand = findCommandButton(
  // "Command_TransportExit" );

  // sanity
  if (transport == NULL || commandSet == NULL) return;
  ...
  //
  // first, hide any windows in 'm_commandWindows' that correspond to
  // TransportExit commands within the 'commandSet' that are overflow slots (the
  // ui could be showing more inventory exit button slots than there are slots
  // in the transport)
  //
  // The extra slots bit means that a tank that takes up three slots will make
  // two transport buttons disappear off the end to show he takes up more room.
  ...
  for (Int i = 0; i < MAX_COMMANDS_PER_SET; i++) {
    ...
    if (commandButton &&
        commandButton->getCommandType() == GUI_COMMAND_EXIT_CONTAINER) {
      // record the index of the control for the first inventory exit command we
      // found
      if (firstInventoryIndex == -1) firstInventoryIndex = i;

      //
      // since we're assuming all inventory exit commands appear in a continuous
      // order, we need to also need to keep track of what is the last valid
      // inventory commadn index
      //
      lastInventoryIndex = i;

      // increment our count of available inventory exit commands found for the
      // set
      inventoryCommandCount++;

      // show the window, but disable by default unless something is actually
      // loaded in there
      m_commandWindows[i]->winHide(FALSE);
      m_commandWindows[i]->winEnable(FALSE);

      // Clear any potential veterancy rank, or else we'll see it when it's
      // empty!
      GadgetButtonDrawOverlayImage(m_commandWindows[i], NULL);

      // Unmanned vehicles don't have any commands available -- in fact they are
      // hidden!
      if (transport->isDisabledByType(DISABLED_UNMANNED)) {
        m_commandWindows[i]->winHide(TRUE);
      }

      // if we've counted more UI spots than the transport can hold, hide this
      // command window
      if (inventoryCommandCount > transportMax)
        m_commandWindows[i]->winHide(TRUE);

      //
      // set the inventory exit command into the window (even if it's one of the
      // hidden ones it's OK cause we'll never see it to click it
      //
  ...
  // After Every change to the m_commandWIndows, we need to show fill in the
  // missing blanks with the images removed from multiplayer branch
  // showCommandMarkers();

  //
  // now, iterate the contained items of the transport and for each one we find
  // we will populate a user interface button with its inventory picture and
  // store the inventory data inside the control bar so we can respond to the
  // button when its clicked
  //
  ...
  //
  // save the last recorded inventory count so we know when we have to redo the
  // gui when something exits or enters
  //
...
void ControlBar::populateCommand(Object *obj) {
  ...
  for (i = 0; i < MAX_COMMANDS_PER_SET; i++) {
    ...
    if (commandButton == NULL) {
      ...
      //
      // inventory exit commands were a special case already taken care above
      // ... we needed to iterage through the containment for transport objects
      // and fill out any available inventory exit buttons on the UI
      //
      if (commandButton->getCommandType() != GUI_COMMAND_EXIT_CONTAINER) {
        ...
        // show up cause we can never pick "another" general technology
        // throughout the game
        //
        if (BitTest(commandButton->getOptions(), NEED_SPECIAL_POWER_SCIENCE)) {
          ...
          if (power && power->getRequiredScience() != SCIENCE_INVALID) {
            if (player->hasScience(power->getRequiredScience()) == FALSE) {
              ...
              // determined by the command button specifying a vector of
              // sciences in the command button.
              Int bestIndex = -1;
              ScienceType science;
  ...
  //
  // for objects that have a production exit interface, we may have a rally
  // point set. if we do, we want to show that rally point in the world
  //
  ExitInterface *exit = obj->getObjectExitInterface();
  if (exit) {
    //
    // if a rally point is set, show the rally point, if we don't have it set
    // hide any rally point we might have visible
    //
...
void ControlBar::populateBuildQueue(Object *producer) {
  /// @todo srj -- remove hard-coding here, please
  static const CommandButton *cancelUnitCommand =
      findCommandButton("Command_CancelUnitCreate");
  /// @todo srj -- remove hard-coding here, please
  static const CommandButton *cancelUpgradeCommand =
      findCommandButton("Command_CancelUpgradeCreate");
  ...
  for (i = 0; i < MAX_BUILD_QUEUE_BUTTONS; i++) {
    ...
    // Clear the status because this button doesn't use it -- and if it's set,
    // it'll become invisible meaning the image that was there will be showed.
  ...
  if (pu == NULL) return;  // sanity
  const ProductionEntry *production;
  Int windowIndex = 0;
  const Image *image;
  for (production = pu->firstProduction(); production;
       production = pu->nextProduction(production)) {
    // don't go above how many queue windows we have
    if (windowIndex >= MAX_BUILD_QUEUE_BUTTONS) break;  // exit for
    ...
    if (production->getProductionType() == PRODUCTION_UNIT) {
      ...
      m_queueData[windowIndex].control->winSetStatus(
      ...
      // No longer used.
      // image = TheMappedImageCollection->findImageByName(
      // production->getProductionObject()->getInventoryImageName(
      // INV_IMAGE_HILITE ) ); GadgetButtonSetHiliteSelectedImage( m_queueData[
      // windowIndex ].control, image ); image =
      // TheMappedImageCollection->findImageByName(
      // production->getProductionObject()->getInventoryImageName(
      // INV_IMAGE_PUSHED ) ); GadgetButtonSetHiliteImage( m_queueData[
      // windowIndex ].control, image );

      // Show the veterancy rank of the object being constructed in the queue
      const Image *image =
          calculateVeterancyOverlayForThing(production->getProductionObject());
      GadgetButtonDrawOverlayImage(m_queueData[windowIndex].control, image);
      //
      // note we're not setting a disabled image into the queue button ... when
      // there is nothing in the queue we set the button to disabled, we want to
      // leave the disabled queue button graphic we already have in place
      //
      //		image = TheMappedImageCollection->findImageByName(
      //production->getProductionObject()->getInventoryImageName(
      //INV_IMAGE_DISABLED ) ); 		GadgetButtonSetDisabledImage( m_queueData[
      //windowIndex ].control, image );
      ...
      m_queueData[windowIndex].upgradeToResearch =
          production->getProductionUpgrade();
      ...
      // No longer used
      // image = TheMappedImageCollection->findImageByName(
      // ut->getQueueImageName( UpgradeTemplate::UPGRADE_HILITE ) );
      // GadgetButtonSetHiliteSelectedImage( m_queueData[ windowIndex ].control,
      // image ); image = TheMappedImageCollection->findImageByName(
      // ut->getQueueImageName( UpgradeTemplate::UPGRADE_PUSHED ) );
      // GadgetButtonSetHiliteImage( m_queueData[ windowIndex ].control, image
      // );
      //
      //  note we're not setting a disabled image into the queue button ... when
      //  there is nothing in the queue we set the button to disabled, we want
      //  to leave the disabled queue button graphic we already have in place
      //
      //		image = TheMappedImageCollection->findImageByName(
      //ut->getQueueImageName( UpgradeTemplate::UPGRADE_DISABLED ) );
      //		GadgetButtonSetDisabledImage( m_queueData[ windowIndex
      //].control, image );
  ...
  //
  // save the count of things being produced in the build queue, when it changes
  // we will repopulate the queue to visually show the change
  //
...
void ControlBar::updateContextCommand(void) {
  ...
  //
  // the contents of objects are ususally showed on the UI, when those contents
  // change we always to update the UI
  //
  ...
  ProductionUpdateInterface *pu =
      obj ? obj->getProductionUpdateInterface() : NULL;

  //
  // when we have a production update, we show the build queue when there is
  // actually something in the queue, otherwise we show the selection portrait
  // for the object ... so if the queue is visible we need to check to see if we
  // should hide it and show the portrait, and if the queue is hidden, we need
  // to check and see if it should become shown
  //
  ...
  if (m_contextParent[CP_BUILD_QUEUE]->winIsHidden() == FALSE) {
    ...
    //
    // when showing a production queue, when the production count changes of the
    // producer object (the thing we have selected for the control bar) we will
    // repopulate the windows to visually show the new production linup
    //
    if (pu) {
      ...
      //
      // update the build percentage on the first thing (the thing that's being
      // built) in the queue
      //
      ...
      if (produce) {
        ...
        GameWindow *win = TheWindowManager->winGetWindowFromId(
            m_contextParent[CP_BUILD_QUEUE], winID);

        DEBUG_ASSERTCRASH(win, ("updateContextCommand: Unable to find first "
                                "build queue button\n"));
        //				UnicodeString text;
        //
        //				text.format( L"%.0f%%",
        //produce->getPercentComplete() ); 				GadgetButtonSetText( win, text );

        GadgetButtonDrawInverseClock(win, produce->getPercentComplete(),
                                     m_buildUpClockColor);
  ...
  for (i = 0; i < MAX_COMMANDS_PER_SET; i++) {
    ...
    //`tbd: fix the bug here, that is that if we don't change the unit, we won't
    //attempt to show
    // these.
    ...
    if (command->getCommandType() == GUI_COMMAND_EXIT_CONTAINER) {
      win->winSetStatus(WIN_STATUS_ALWAYS_COLOR);  // Don't let these buttons
                                                   // render in grayscale ever!
      continue;
    } else {
    ...
    CommandAvailability availability =
        getCommandAvailability(command, obj, win);
    ...
    // Determine by the production type of this button, whether or not the
    // created object will have a veterancy rank
    const Image *image =
        calculateVeterancyOverlayForThing(command->getThingTemplate());
    GadgetButtonDrawOverlayImage(win, image);

    //
    // for check-like commands we will keep the push button "pushed" or
    // "unpushed" depending on the current running status of the command
    //
    if (BitTest(command->getOptions(), CHECK_LIKE)) {
      // sanity, check like commands should have windows that are check like as
      // well
      DEBUG_ASSERTCRASH(BitTest(win->winGetStatus(), WIN_STATUS_CHECK_LIKE),
                        ("updateContextCommand: Error, gadget window for "
                         "command '%s' is not check-like!\n",
                         command->getName().str()));
...
//-------------------------------------------------------------------------------------------------
const Image *ControlBar::calculateVeterancyOverlayForThing(
...
static Int getRappellerCount(Object *obj) {
  ...
  const ContainedItemsList *items =
      obj->getContain() ? obj->getContain()->getContainedItemsList() : NULL;
  if (items) {
...
//-------------------------------------------------------------------------------------------------
/** What's the status between 'obj' and the 'command' at present.  Can we do it?
 * Are we already doing it?  Can ya dig it? */
//-------------------------------------------------------------------------------------------------
CommandAvailability ControlBar::getCommandAvailability(
    const CommandButton *command, Object *obj, GameWindow *win,
  ...
  if (obj == NULL) return COMMAND_HIDDEN;  // probably better than crashing....

  Player *player = obj->getControllingPlayer();

  if (obj->testScriptStatusBit(OBJECT_STATUS_SCRIPT_DISABLED) ||
      obj->testScriptStatusBit(OBJECT_STATUS_SCRIPT_UNPOWERED)) {
    // if the object status is disabled or unpowered, you cannot do anything to
    // it.
  ...
  // It's possible for command buttons to be a single use only type of a button
  // -- like detonating a nuke from a convoy truck.
  ...
  // if we are only disabled by being underpowered, and this button doesn't
  // care, well, fix it
  if (disabled && BitTest(command->getOptions(), IGNORES_UNDERPOWERED) &&
  ...
  if (pu && pu->firstProduction() &&
      BitTest(command->getOptions(), NOT_QUEUEABLE)) {
    // This button is designated so that it is incapable of building this
    // upgrade/object when anything is in the production queue.
    return COMMAND_RESTRICTED;
  }

  Bool queueMaxed =
      pu ? (pu->getProductionCount() == MAX_BUILD_QUEUE_BUTTONS) : FALSE;

  switch (command->getCommandType()) {
    case GUI_COMMAND_DOZER_CONSTRUCT: {
      // if the command is a dozer construct task and the object dozer is
      // building anything this command is not available
      if (command->getThingTemplate()) {
        BuildableStatus bStatus = command->getThingTemplate()->getBuildable();
        if (bStatus == BSTATUS_NO ||
            (bStatus == BSTATUS_ONLY_BY_AI &&
             obj->getControllingPlayer()->getPlayerType() != PLAYER_COMPUTER))
          return COMMAND_HIDDEN;
      }

      // sanity, non dozer object
      if (obj->isKindOf(KINDOF_DOZER) == FALSE) return COMMAND_RESTRICTED;

      // get the dozer ai update interface
      DozerAIInterface *dozerAI = NULL;
      if (obj->getAIUpdateInterface() == NULL) return COMMAND_RESTRICTED;

      dozerAI = obj->getAIUpdateInterface()->getDozerAIInterface();

      DEBUG_ASSERTCRASH(
          dozerAI != NULL,
          ("Something KINDOF_DOZER must have a Dozer-like AIUpdate"));
      if (dozerAI == NULL) return COMMAND_RESTRICTED;
    ...
    case GUI_COMMAND_SELL: {
      // if this is a sell command, is the object marked as "This cannot be
      // sold?" if so, remove the button, otherwise, its available
    ...
    case GUI_COMMAND_UNIT_BUILD: {
      ...
      CanMakeType makeType =
          TheBuildAssistant->canMakeUnit(obj, command->getThingTemplate());
      if (makeType == CANMAKE_MAXED_OUT_FOR_PLAYER ||
          makeType == CANMAKE_PARKING_PLACES_FULL) {
        // Disable the button if the player has a max amount of these units in
        // build queue or existance.
        return COMMAND_RESTRICTED;
      }
    ...
    case GUI_COMMAND_PLAYER_UPGRADE: {
      ...
      if (TheUpgradeCenter->canAffordUpgrade(
              player, command->getUpgradeTemplate()) == FALSE)
        return COMMAND_RESTRICTED;  // COMMAND_CANT_AFFORD;
    ...
    case GUI_COMMAND_OBJECT_UPGRADE: {
      ...
      if (pu == NULL) {
        DEBUG_CRASH(
            ("Objects that have Object-Level Upgrades must also have "
             "ProductionUpdate. Just cuz."));
        return COMMAND_RESTRICTED;
      }

      //
      // if this object already has this upgrade, or is researching it already
      // in the queue we will disable the button so you can't build another one
      //
    ...
    case GUI_COMMAND_FIRE_WEAPON: {
      ...
      if (ai == NULL) return COMMAND_RESTRICTED;

      // ask the ai if the weapon is ready to fire
      const Weapon *w = obj->getWeaponInWeaponSlot(command->getWeaponSlot());

      // changed this to Log rather than Crash, because this can legitimately
      // happen now for dozers and workers with mine-clearing stuff... (srj)
      // DEBUG_ASSERTLOG( w, ("Unit %s's CommandButton %s is trying to access
      // weaponslot %d, but doesn't have a weapon there in its FactionUnit ini
      // entry.\n", 	obj->getTemplate()->getName().str(),
      //command->getName().str(), (Int)command->getWeaponSlot() ) );

      UnsignedInt now = TheGameLogic->getFrame();

      /// @Kris -- We need to show the button as always available for anything
      /// with a 0 clip reload time.
      ...
      if (w == NULL                           // No weapon
          ...
          || w->getPossibleNextShotFrame() ==
                 now  // Weapon ready, but could fire this exact frame (handle
                      // button flicker since it may be going to fire anyway)
          /// @todo srj -- not sure why this next check is necessary, but the
          /// Comanche missile buttons will flicker without it. figure out
          /// someday.
          /// @todo ml  -- and note: that the "now-1" below causes
          /// zero-clip-reload weapons to never be ready, so I added this If you
          /// make changes to this code, make sure that the DragonTank's
          /// firewall weapon can be retargeted while active, that is, while the
          /// tank is squirting out flames all over the floor, you can click the
          /// firewall button (or "F"), and re-target the firewall without
          /// having to stop or move in-betwen.. Thanks for reading
          || (w->getPossibleNextShotFrame() == now - 1)) {
        if (w != NULL) {
          // only draw the clock when reloading a clip, not when merely between
          // shots, since that's usually a tiny amount of time
          ...
          // if this is a mine-clearing button but we don't have the right
          // weaponset, just declare it available... we'll switch weaponsets
          // when the time comes
          if ((command->getOptions() & USES_MINE_CLEARING_WEAPONSET) != 0 &&
    ...
    case GUI_COMMAND_COMBATDROP: {
      if (getRappellerCount(obj) <= 0) return COMMAND_RESTRICTED;
      break;
    }

    case GUI_COMMAND_EXIT_CONTAINER: {
      //
      // this method is really used as a per frame update to see if we should
      // enable disable a control ... inventory of objects shows as buttons have
      // that enable disable logic handled elsewhere, where if the contained
      // count of the entire container changes the UI is completely repopulated
      //
      break;
    }
    ...
    case GUI_COMMAND_EXECUTE_RAILED_TRANSPORT: {
      ...
      if (dui == NULL || dui->isDockOpen() == FALSE) return COMMAND_RESTRICTED;
      break;
    }
    ...
    case GUI_COMMAND_SPECIAL_POWER: {
      ...
      SpecialPowerModuleInterface *mod =
          obj->getSpecialPowerModule(command->getSpecialPowerTemplate());

      if (mod == NULL) {
        // sanity ... we must have a module for the special power, if we don't
        // somebody probably forgot to put it in the object
        DEBUG_CRASH(
            ("Object does not contain special power module (%s) to execute.  "
             "Did you forget to add it to the object INI?\n",
    ...
    case GUI_COMMAND_TOGGLE_OVERCHARGE: {
      ...
      for (BehaviorModule **bmi = obj->getBehaviorModules(); *bmi; ++bmi) {
        ...
        if (obi) {
          if (obi->isOverchargeActive()) return COMMAND_ACTIVE;
        }
      }
      break;
    }
    ...
    case GUI_COMMAND_SWITCH_WEAPON: {
      ...
      DEBUG_ASSERTCRASH(
          w, ("Unit %s's CommandButton %s is trying to access weaponslot %d, "
              "but doesn't have a weapon there in its FactionUnit ini entry.",
              obj->getTemplate()->getName().str(), command->getName().str(),
              (Int)command->getWeaponSlot()));

      if (w == NULL) return COMMAND_RESTRICTED;
      ...
      for (DrawableListCIt it = selected->begin(); it != selected->end();
        ...
        if (draw && draw->getObject() &&
          ...
          WeaponSlotType wslot =
              draw->getObject()->getCurrentWeapon()->getWeaponSlot();
          if (wslot != command->getWeaponSlot()) return COMMAND_AVAILABLE;
        }
      }
    ...
    case GUI_COMMAND_STOP: {
      ...
      static NameKeyType key_BattlePlanUpdate = NAMEKEY("BattlePlanUpdate");
      BattlePlanUpdate *bpUpdate =
          (BattlePlanUpdate *)obj->findUpdateModule(key_BattlePlanUpdate);
      if (bpUpdate &&
          bpUpdate->getActiveBattlePlan() != PLANSTATUS_BOMBARDMENT) {
        return COMMAND_RESTRICTED;
      }
      return COMMAND_AVAILABLE;
    }
