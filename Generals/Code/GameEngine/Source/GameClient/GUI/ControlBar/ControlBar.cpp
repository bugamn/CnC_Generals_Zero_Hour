...
const FieldParse CommandButton::s_commandButtonFieldParseTable[] = {
    ...
    {"Command", CommandButton::parseCommand, NULL,
     offsetof(CommandButton, m_command)},
    {"Options", INI::parseBitString32, TheCommandOptionNames,
     offsetof(CommandButton, m_options)},
    {"Object", INI::parseThingTemplate, NULL,
     offsetof(CommandButton, m_thingTemplate)},
    {"Upgrade", INI::parseUpgradeTemplate, NULL,
     offsetof(CommandButton, m_upgradeTemplate)},
    {"WeaponSlot", INI::parseLookupList, TheWeaponSlotTypeNamesLookupList,
     offsetof(CommandButton, m_weaponSlot)},
    {"MaxShotsToFire", INI::parseInt, NULL,
     offsetof(CommandButton, m_maxShotsToFire)},
    {"Science", INI::parseScienceVector, NULL,
     offsetof(CommandButton, m_science)},
    {"SpecialPower", INI::parseSpecialPowerTemplate, NULL,
     offsetof(CommandButton, m_specialPower)},
    {"TextLabel", INI::parseAsciiString, NULL,
     offsetof(CommandButton, m_textLabel)},
    {"DescriptLabel", INI::parseAsciiString, NULL,
     offsetof(CommandButton, m_descriptionLabel)},
    {"PurchasedLabel", INI::parseAsciiString, NULL,
     offsetof(CommandButton, m_purchasedLabel)},
    {"ConflictingLabel", INI::parseAsciiString, NULL,
     offsetof(CommandButton, m_conflictingLabel)},
    {"ButtonImage", INI::parseAsciiString, NULL,
     offsetof(CommandButton, m_buttonImageName)},
    {"CursorName", INI::parseAsciiString, NULL,
     offsetof(CommandButton, m_cursorName)},
    {"InvalidCursorName", INI::parseAsciiString, NULL,
     offsetof(CommandButton, m_invalidCursorName)},
    {"ButtonBorderType", INI::parseLookupList,
     CommandButtonMappedBorderTypeNames,
     offsetof(CommandButton, m_commandButtonBorder)},
    {"RadiusCursorType", INI::parseIndexList, TheRadiusCursorNames,
     offsetof(CommandButton, m_radiusCursor)},
    {"UnitSpecificSound", INI::parseAudioEventRTS, NULL,
     offsetof(CommandButton, m_unitSpecificSound)},

    {NULL, NULL, NULL, 0}  // keep this last

};
static void commandButtonTooltip(GameWindow *window, WinInstanceData *instData,
                                 UnsignedInt mouse) {
...
void ControlBar::populatePurchaseScience(Player *player) {
  ...
  Int i;
  if (TheScriptEngine->isGameEnding()) return;
  // get command set
  if (!player || !player->getPlayerTemplate() ||
      ...
      player->getPlayerTemplate()
          ...
          .isEmpty())
    return;
  commandSet1 = TheControlBar->findCommandSet(
      player->getPlayerTemplate()
          ->getPurchaseScienceCommandSetRank1());  // TEMP WILL CHANGE TO PROPER
                                                   // WAY ONCE WORKING
  commandSet3 = TheControlBar->findCommandSet(
      player->getPlayerTemplate()
          ->getPurchaseScienceCommandSetRank3());  // TEMP WILL CHANGE TO PROPER
                                                   // WAY ONCE WORKING
  commandSet8 = TheControlBar->findCommandSet(
      player->getPlayerTemplate()
          ->getPurchaseScienceCommandSetRank8());  // TEMP WILL CHANGE TO PROPER
                                                   // WAY ONCE WORKING
  ...
  if (commandSet1 == NULL || commandSet3 == NULL || commandSet8 == NULL) return;
      ...
      if (!commandButton->getScienceVec().empty()) {
        ...
        if (player->isScienceDisabled(st)) {
          ...
          if (!player->hasScience(st) &&
              ...
              TheScienceStore->getSciencePurchaseCost(st) <=
                  player->getSciencePurchasePoints()) {
            m_sciencePurchaseWindowsRank1[i]->winEnable(TRUE);
          }
          if (player->hasScience(st)) {
            m_sciencePurchaseWindowsRank1[i]->winSetStatus(
                WIN_STATUS_ALWAYS_COLOR);
          } else {
            m_sciencePurchaseWindowsRank1[i]->winClearStatus(
                WIN_STATUS_ALWAYS_COLOR);
          }
  ...
  for (i = 0; i < MAX_PURCHASE_SCIENCE_RANK_3; i++) {
    ...
    if (commandButton == NULL) {
      ...
      if (player->isScienceDisabled(st)) {
        ...
        if (player->hasScience(st)) {
          m_sciencePurchaseWindowsRank3[i]->winSetStatus(
              WIN_STATUS_ALWAYS_COLOR);
        } else {
          m_sciencePurchaseWindowsRank3[i]->winClearStatus(
              WIN_STATUS_ALWAYS_COLOR);
        }
        if (!TheScienceStore->playerHasRootPrereqsForScience(player, st))
          m_sciencePurchaseWindowsRank3[i]->winHide(TRUE);
  ...
  for (i = 0; i < MAX_PURCHASE_SCIENCE_RANK_8; i++) {
    ...
    if (commandButton == NULL) {
      ...
      if (player->isScienceDisabled(st)) {
        ...
        if (player->hasScience(st)) {
          m_sciencePurchaseWindowsRank8[i]->winSetStatus(
              WIN_STATUS_ALWAYS_COLOR);
        } else {
          m_sciencePurchaseWindowsRank8[i]->winClearStatus(
              WIN_STATUS_ALWAYS_COLOR);
        }
        if (!TheScienceStore->playerHasRootPrereqsForScience(player, st))
          m_sciencePurchaseWindowsRank8[i]->winHide(TRUE);
  ...
  win = TheWindowManager->winGetWindowFromId(
      m_contextParent[CP_PURCHASE_SCIENCE],
      TheNameKeyGenerator->nameToKey(
          "GeneralsExpPoints.wnd:StaticTextRankPointsAvailable"));
  if (win) {
  ...
  win = TheWindowManager->winGetWindowFromId(
      m_contextParent[CP_PURCHASE_SCIENCE],
      TheNameKeyGenerator->nameToKey(
          "GeneralsExpPoints.wnd:ProgressBarExperience"));
  if (win) {
    Int progress;
    progress =
        ((player->getSkillPoints() - player->getSkillPointsLevelDown()) * 100) /
  ...
  //
  // to avoid a one frame delay where windows may become enabled/disabled, run
  // the update at once to get it all in the correct state immediately
  //
  ...
  /*
          ...
          TheScienceStore->getPurchasableSciences(player, purchasable,
     potential); GadgetListBoxReset(win); for (ScienceVec::const_iterator it =
     purchasable.begin(); it != purchasable.end(); ++it)
          {
                  ScienceType st = *it;
                  UnicodeString u;
          ...
          for (ScienceVec::const_iterator it2 = potential.begin(); it2 !=
     potential.end(); ++it2)
          {
                  ScienceType st = *it2;
          ...
          GadgetListBoxAddEntryText(win, UnicodeString(L"Cancel"), color, -1,
     -1);*/
}
...
void ControlBar::updateContextPurchaseScience(void) {
  ...
  //	win = TheWindowManager->winGetWindowFromId( m_contextParent[
  //CP_PURCHASE_SCIENCE ], TheNameKeyGenerator->nameToKey(
  //"ControlBar.wnd:TextEntryGeneralName" ) ); 	if(win)
  //	{
  ...
  /*
          ...
          if( selected != -1 )
          {
                  ...
                  AsciiString sci;
                  sci.translate(usci);
                  ScienceType st = usci.getCharAt(0) == '(' ? SCIENCE_INVALID :
     TheScienceStore->getScienceFromInternalName(sci);

                  if (st != SCIENCE_INVALID)
                  {
                          GameMessage *msg = TheMessageStream->appendMessage(
     GameMessage::MSG_PURCHASE_SCIENCE ); msg->appendIntegerArgument( st );
                  }
...
//-------------------------------------------------------------------------------------------------
void CommandButton::parseCommand(INI *ini, void *instance, void *store,
                                 const void *userData) {
...
CommandButton::CommandButton(void) {
  ...
  // The purpose is to initialize these variable to values that are zero or
  // empty
...
Bool CommandButton::isValidRelationshipTarget(Relationship r) const {
  UnsignedInt mask = 0;
  if (r == ENEMIES)
    mask |= NEED_TARGET_ENEMY_OBJECT;
  else if (r == ALLIES)
    mask |= NEED_TARGET_ALLY_OBJECT;
  else if (r == NEUTRAL)
    mask |= NEED_TARGET_NEUTRAL_OBJECT;
...
Bool CommandButton::isValidObjectTarget(const Player *sourcePlayer,
  ...
  if (!sourcePlayer || !targetObj) return false;
...
Bool CommandButton::isValidToUseOn(const Object *sourceObj,
  ...
  if (m_upgradeTemplate) {
    // @todo: Make a const version of pui. We're not altering the production
    // queue, so this const-cast is okay.
    ...
    if (pui) {
      ...
      while (pe) {
        if (pe->getProductionUpgrade() != NULL) return false;
        pe = pui->nextProduction(pe);
      }
      return sourceObj->affectedByUpgrade(m_upgradeTemplate) &&
             !sourceObj->hasUpgrade(m_upgradeTemplate);
    }
  ...
  if (BitTest(m_options, COMMAND_OPTION_NEED_OBJECT_TARGET)) {
    return TheActionManager->canDoSpecialPowerAtObject(
        sourceObj, targetObj, commandSource, m_specialPower, m_options, false);
  }

  if (BitTest(m_options, NEED_TARGET_POS)) {
    return TheActionManager->canDoSpecialPowerAtLocation(
        sourceObj, targetLocation, commandSource, m_specialPower, NULL,
        m_options, false);
  }

  return TheActionManager->canDoSpecialPower(sourceObj, m_specialPower,
                                             commandSource, m_options, false);
}

//-------------------------------------------------------------------------------------------------
Bool CommandButton::isReady(const Object *sourceObj) const {
  SpecialPowerModuleInterface *mod =
      sourceObj->getSpecialPowerModule(m_specialPower);
  if (mod && mod->getPercentReady() == 1.0f) return true;

  if (m_upgradeTemplate && sourceObj->affectedByUpgrade(m_upgradeTemplate) &&
      !sourceObj->hasUpgrade(m_upgradeTemplate))
    return true;
...
Bool CommandButton::isValidObjectTarget(const Drawable *source,
  ...
  return isValidObjectTarget(source ? source->getObject() : NULL,
                             target ? target->getObject() : NULL);
}
...
//-------------------------------------------------------------------------------------------------
/** These are the fields you can define in a command set, they correspond to
 * physical buttons in the GUI */
//-------------------------------------------------------------------------------------------------
const FieldParse CommandSet::m_commandSetFieldParseTable[] = {

    {"1", CommandSet::parseCommandButton, (void *)0,
     offsetof(CommandSet, m_command)},
    {"2", CommandSet::parseCommandButton, (void *)1,
     offsetof(CommandSet, m_command)},
    {"3", CommandSet::parseCommandButton, (void *)2,
     offsetof(CommandSet, m_command)},
    {"4", CommandSet::parseCommandButton, (void *)3,
     offsetof(CommandSet, m_command)},
    {"5", CommandSet::parseCommandButton, (void *)4,
     offsetof(CommandSet, m_command)},
    {"6", CommandSet::parseCommandButton, (void *)5,
     offsetof(CommandSet, m_command)},
    {"7", CommandSet::parseCommandButton, (void *)6,
     offsetof(CommandSet, m_command)},
    {"8", CommandSet::parseCommandButton, (void *)7,
     offsetof(CommandSet, m_command)},
    {"9", CommandSet::parseCommandButton, (void *)8,
     offsetof(CommandSet, m_command)},
    {"10", CommandSet::parseCommandButton, (void *)9,
     offsetof(CommandSet, m_command)},
    {"11", CommandSet::parseCommandButton, (void *)10,
     offsetof(CommandSet, m_command)},
    {"12", CommandSet::parseCommandButton, (void *)11,
     offsetof(CommandSet, m_command)},
    {NULL, NULL, NULL, 0}  // keep this last
...
//-------------------------------------------------------------------------------------------------
void CommandSet::parseCommandButton(INI *ini, void *instance, void *store,
                                    const void *userData) {
  const char *token = ini->getNextToken();

  // get find the command button from this name
  const CommandButton *commandButton =
      TheControlBar->findCommandButton(AsciiString(token));
  if (commandButton == NULL) {
    DEBUG_CRASH(
  ...
  // sanity
  DEBUG_ASSERTCRASH(
      buttonIndex < MAX_COMMANDS_PER_SET,
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
CommandSet::CommandSet(const AsciiString &name) : m_name(name), m_next(NULL) {
  for (Int i = 0; i < MAX_COMMANDS_PER_SET; i++) m_command[i] = NULL;
}
...
const CommandButton *CommandSet::getCommandButton(Int i) const {
  ...
  // Check for TheGameLogic == null, cause it is in Worldbuilder, and wb gets
  // command bar info. jba.
...
ControlBar::ControlBar(void) {
  ...
  for (i = 0; i < NUM_CONTEXT_PARENTS; i++) m_contextParent[i] = NULL;
  for (i = 0; i < MAX_COMMANDS_PER_SET; i++) {
  ...
  for (i = 0; i < MAX_RIGHT_HUD_UPGRADE_CAMEOS; i++) m_rightHUDUpgradeCameos[i];
  ...
  updateCommanBarBorderColors(GAME_COLOR_UNDEFINED, GAME_COLOR_UNDEFINED,
                              GAME_COLOR_UNDEFINED, GAME_COLOR_UNDEFINED);
...
ControlBar::~ControlBar(void) {
  ...
  m_animateWindowManagerForGenShortcuts = NULL;
  if (m_animateWindowManager) delete m_animateWindowManager;
  m_animateWindowManager = NULL;

  if (m_generalsScreenAnimate) delete m_generalsScreenAnimate;
  m_generalsScreenAnimate = NULL;

  if (m_controlBarSchemeManager) delete m_controlBarSchemeManager;
  m_controlBarSchemeManager = NULL;
...
//-------------------------------------------------------------------------------------------------
/** Initialzie the control bar, this is our interface to the context sinsitive
 * GUI */
//-------------------------------------------------------------------------------------------------
void ControlBar::init(void) {
  ...
  ini.load(AsciiString("Data\\INI\\Default\\CommandButton.ini"),
           INI_LOAD_OVERWRITE, NULL);
  ini.load(AsciiString("Data\\INI\\CommandButton.ini"), INI_LOAD_OVERWRITE,
           NULL);
  ...
  // Added this check because the builder uses the ControlBar, but doesn't care
  // about the GUI.
  if (TheWindowManager) {
    //
    // the control bar has several windows that make up our context sensitive
    // interface, we want those parent windows so that we can easily hide and
    // show them to make the interface context sensitive
    //
    NameKeyType id;
    ...
    m_contextParent[CP_MASTER]->winGetPosition(&m_defaultControlBarPosition.x,
                                               &m_defaultControlBarPosition.y);

    m_scienceLayout =
        TheWindowManager->winCreateLayout("GeneralsExpPoints.wnd");
    m_scienceLayout->hide(TRUE);
    id = TheNameKeyGenerator->nameToKey("GeneralsExpPoints.wnd:GenExpParent");

    m_contextParent[CP_PURCHASE_SCIENCE] = TheWindowManager->winGetWindowFromId(
        NULL, id);  // m_scienceLayout->getFirstWindow();

    id = TheNameKeyGenerator->nameToKey(
        "ControlBar.wnd:UnderConstructionWindow");
    m_contextParent[CP_UNDER_CONSTRUCTION] =
        TheWindowManager->winGetWindowFromId(NULL, id);
    ...
    m_contextParent[CP_OBSERVER_LIST] =
        TheWindowManager->winGetWindowFromId(NULL, id);

    id = TheNameKeyGenerator->nameToKey(
        "ControlBar.wnd:ObserverPlayerInfoWindow");
    m_contextParent[CP_OBSERVER_INFO] =
        TheWindowManager->winGetWindowFromId(NULL, id);
    ...
    for (i = 0; i < MAX_COMMANDS_PER_SET; i++) {
      ...
      // removed from multiplayer branch
      //			windowName.format(
      //"ControlBar.wnd:CommandMarker%02d", i + 1 ); 			id =
      //TheNameKeyGenerator->nameToKey( windowName.str() ); 			m_commandMarkers[ i
      //] = 				TheWindowManager->winGetWindowFromId( m_contextParent[ CP_COMMAND ],
      //id );
      //			// set the size and position to make sure their
      //in the same place as the buttons.
      //			m_commandMarkers[i]->winSetPosition(commandPos.x
      //-2, commandPos.y - 2); 			m_commandMarkers[i]->winSetSize(commandSize.x +
      //2, commandSize.y + 2);
    ...
    for (i = 0; i < MAX_PURCHASE_SCIENCE_RANK_1; i++) {
      ...
      id = TheNameKeyGenerator->nameToKey(windowName.str());
      m_sciencePurchaseWindowsRank1[i] = TheWindowManager->winGetWindowFromId(
          m_contextParent[CP_PURCHASE_SCIENCE], id);
      m_sciencePurchaseWindowsRank1[i]->winSetStatus(
    ...
    for (i = 0; i < MAX_PURCHASE_SCIENCE_RANK_3; i++) {
      ...
      id = TheNameKeyGenerator->nameToKey(windowName.str());
      m_sciencePurchaseWindowsRank3[i] = TheWindowManager->winGetWindowFromId(
          m_contextParent[CP_PURCHASE_SCIENCE], id);
      m_sciencePurchaseWindowsRank3[i]->winSetStatus(
          WIN_STATUS_USE_OVERLAY_STATES);
    }  // end for i
    ...
    for (i = 0; i < MAX_PURCHASE_SCIENCE_RANK_8; i++) {
      ...
      id = TheNameKeyGenerator->nameToKey(windowName.str());
      m_sciencePurchaseWindowsRank8[i] = TheWindowManager->winGetWindowFromId(
          m_contextParent[CP_PURCHASE_SCIENCE], id);
      m_sciencePurchaseWindowsRank8[i]->winSetStatus(
          WIN_STATUS_USE_OVERLAY_STATES);
    }  // end for i
    ...
    setControlCommand(m_communicatorButton,
                      findCommandButton("NonCommand_Communicator"));
    ...
    win = TheWindowManager->winGetWindowFromId(
        NULL,
        TheNameKeyGenerator->nameToKey("ControlBar.wnd:ButtonPlaceBeacon"));
    if (win) {
    ...
    if (win) {
      setControlCommand(win,
                        findCommandButton("NonCommand_GeneralsExperience"));
      win->winSetTooltipFunc(commandButtonTooltip);
    }
    win = TheWindowManager->winGetWindowFromId(
        NULL, TheNameKeyGenerator->nameToKey("ControlBar.wnd:ButtonLarge"));
    if (win) {
    ...
    win = TheWindowManager->winGetWindowFromId(
        NULL, TheNameKeyGenerator->nameToKey("ControlBar.wnd:GeneralsExp"));
    if (win) {
      win->winSetTooltipFunc(commandButtonTooltip);
    }

    m_radarAttackGlowWindow = TheWindowManager->winGetWindowFromId(
        NULL, TheNameKeyGenerator->nameToKey("ControlBar.wnd:WinUAttack"));

    win = TheWindowManager->winGetWindowFromId(
        NULL, TheNameKeyGenerator->nameToKey(
                  AsciiString("ControlBar.wnd:BackgroundMarker")));
    win->winGetScreenPosition(&m_controlBarForegroundMarkerPos.x,
                              &m_controlBarForegroundMarkerPos.y);
    win = TheWindowManager->winGetWindowFromId(
        NULL, TheNameKeyGenerator->nameToKey(
                  AsciiString("ControlBar.wnd:BackgroundMarker")));
    win->winGetScreenPosition(&m_controlBarBackgroundMarkerPos.x,
                              &m_controlBarBackgroundMarkerPos.y);

    if (!m_videoManager) m_videoManager = NEW WindowVideoManager;
    ...
    m_rankVeteranIcon =
        TheMappedImageCollection
            ? TheMappedImageCollection->findImageByName("SSChevron1L")
            : NULL;
    m_rankEliteIcon =
        TheMappedImageCollection
            ? TheMappedImageCollection->findImageByName("SSChevron2L")
            : NULL;
    m_rankHeroicIcon =
        TheMappedImageCollection
            ? TheMappedImageCollection->findImageByName("SSChevron3L")
            : NULL;
...
void ControlBar::reset(void) {
  ...
  // do not destroy the rally drawable, it will get destroyed with everythign
  // else during a reset
  m_rallyPointDrawableID = INVALID_DRAWABLE_ID;
  if (m_radarAttackGlowWindow) m_radarAttackGlowWindow->winEnable(TRUE);
  m_radarAttackGlowOn = FALSE;
  m_remainingRadarAttackGlowFrames = 0;
  ...
  if (m_videoManager) m_videoManager->reset();
  ...
  if (TheTransitionHandler) TheTransitionHandler->remove("ControlBarArrow");
  m_genArrow = NULL;
...
//-------------------------------------------------------------------------------------------------
/** Update phase, we can track if our selected object is destroyed, update
 * button percentages, status, enabled status etc */
//-------------------------------------------------------------------------------------------------
void ControlBar::update(void) {
  ...
  if (m_animateWindowManager) {
    if (m_animateWindowManager->isFinished() &&
        m_animateWindowManager->isReversed()) {
      Int id = (Int)TheNameKeyGenerator->nameToKey(
          AsciiString("ControlBar.wnd:ControlBarParent"));
  ...
  if (m_animateWindowManagerForGenShortcuts && m_specialPowerShortcutParent) {
    if (m_animateWindowManagerForGenShortcuts->isFinished() &&
        m_animateWindowManagerForGenShortcuts->isReversed()) {
  ...
  if (m_isObserverCommandBar) {
    ...
    if (TheInGameUI->getSelectCount() > 1) {
      ...
      drawToEvaluateFor = TheGameClient->findDrawableByID(
          TheInGameUI->getSoloNexusSelectedDrawableID());
  ...
  if (m_flash) {
    ...
    for (Int i = 0; i < MAX_COMMANDS_PER_SET; ++i) {
      ...
      if (button != NULL) {
        const CommandButton *commandButton =
            (const CommandButton *)GadgetButtonGetData(button);
        if (commandButton != NULL) {
          if (commandButton->getFlashCount() > 0 &&
              TheGameClient->getFrame() % 10 == 0) {
  ...
  //
  // first, if the UI is dirty repopulate the UI with what the user should see
  // for all the selected drawables
  //
  if (m_UIDirty) {
    evaluateContextUI();
  ...
  if (ThePlayerList && ThePlayerList->getLocalPlayer() &&
    ...
    const ThingTemplate *thing =
        TheThingFactory->findTemplate(ThePlayerList->getLocalPlayer()
                                          ->getPlayerTemplate()
                                          ->getBeaconTemplate());
    ThePlayerList->getLocalPlayer()->countObjectsByThingTemplate(1, &thing,
                                                                 false, &count);
    static NameKeyType beaconPlacementButtonID =
        NAMEKEY("ControlBar.wnd:ButtonPlaceBeacon");
    GameWindow *win =
        TheWindowManager->winGetWindowFromId(NULL, beaconPlacementButtonID);
    if (win) {
  ...
  //
  // most control bar contexts have one selected thing that we switch on and
  // update based on if that thing changed in some way ... the exception is when
  // multi selected
  //
  ...
  // if nothing is selected get out of here except if we're in the Purchase
  // science context... that requires us to not have anything selected
  if (m_currentSelectedDrawable == NULL) {
    // we better be in the default none context
    DEBUG_ASSERTCRASH(m_currContext == CB_CONTEXT_NONE,
                      ("ControlBar::update no selection, but not we're not "
                       "showing the default NONE context\n"));
    return;
  ...
  Object *obj = NULL;
  if (m_currentSelectedDrawable) obj = m_currentSelectedDrawable->getObject();
  if (obj == NULL) {
    switchToContext(CB_CONTEXT_NONE, NULL);
    return;
...
void ControlBar::onDrawableDeselected(Drawable *draw) {
  ...
  //
  // always when becoming unselected should we remove any build placement icons
  // because if we have some and are in the middle of a build process, it must
  // obiously be over now because we are no longer selecting the dozer or worker
  //
...
const Image *ControlBar::getStarImage(void) {
  if (m_lastFlashedAtPointValue >
          ThePlayerList->getLocalPlayer()->getSciencePurchasePoints() ||
      ThePlayerList->getLocalPlayer()->getSciencePurchasePoints() <= 0)
    m_genStarFlash = FALSE;
  else
    m_lastFlashedAtPointValue =
        ThePlayerList->getLocalPlayer()->getSciencePurchasePoints();
  ...
  if (TheGameLogic->getFrame() % LOGICFRAMES_PER_SECOND >
...
//-------------------------------------------------------------------------------------------------
/** Given the drawables that we have selected into our context sensitive UI,
 * evaluate and perform all UI manipulations to make the GUI show to the user
 * what we want them to see */
//-------------------------------------------------------------------------------------------------
void ControlBar::evaluateContextUI(void) {
  //
  // the UI has been "evaluated" and is now displaying the most current and
  // correct information to the player
  //
  m_UIDirty = FALSE;

  // if our purchase science window is up, we will want to update it by
  // repopulating it.
  if (!m_contextParent[CP_PURCHASE_SCIENCE]->winIsHidden())
    showPurchaseScience();
  ...
  if (TheInGameUI->getSelectCount() == 0) return;

  // get the list of drawable IDs from the in game UI
  const DrawableList *selectedDrawables =
      TheInGameUI->getAllSelectedDrawables();

  // sanity
  if (selectedDrawables->empty() == TRUE) return;
  ...
  if (!TheInGameUI->areSelectedObjectsControllable()) {
    ...
    if (obj->getControllingPlayer() &&
        ...
        obj->getControllingPlayer()
                ->getPlayerTemplate()
                ->getBeaconTemplate()
                .compare(obj->getTemplate()->getName()) == 0) {
    ...
    if (contain && contain->getContainMax() > 0) {
      const Player *otherPlayer = contain->getApparentControllingPlayer(
          ThePlayerList->getLocalPlayer());
      if (!otherPlayer) otherPlayer = obj->getControllingPlayer();
      Player *player = ThePlayerList->getLocalPlayer();
      ...
      // Note: All following checks already account for the fact that this
      // object isn't ours.

      // The only case we can actually see a non-controlled controlbar is a
      // neutral garrisonable structure.
  ...
  //
  // when we have multiple things selected, we will only display the common
  // commands in the center command bar that can be displayed with multi-units
  // selected
  //
  ...
  } else if (drawToEvaluateFor)  // either we have exactly one drawable, or we
                                 // have isolated one to evaluate for...
  {
    ...
    if (BitTest(obj->getStatusBits(), OBJECT_STATUS_SOLD)) return;

    static const NameKeyType key_OCLUpdate = NAMEKEY("OCLUpdate");
    OCLUpdate *update = (OCLUpdate *)obj->findUpdateModule(key_OCLUpdate);

    //
    // a command center is context sensitive itself, if a side has *NOT* been
    // chosen we display the side select interface for command centers only, but
    // note how under construction is more important than anything
    //
    ...
    if (contextSelected == FALSE) {
      ...
      if (cmi && cmi->isGarrisonable() &&
          obj->getCommandSetString().isEmpty()) {
        // Kris: This is a convenient section to graft an inventory commandset
        // for garrisoned troops. However, we only want to use this if we DON'T
        // have a commandset defined. If we do, then trust that the commandset
        // will handle it!
...
CommandButton *ControlBar::findNonConstCommandButton(const AsciiString &name) {
  for (const CommandButton *command = m_commandButtons; command;
       command = command->getNext())
    if (command->getName() == name)
      return const_cast<CommandButton *>(
          (const CommandButton *)command->getFinalOverride());
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
CommandButton *ControlBar::newCommandButtonOverride(
    CommandButton *buttonToOverride) {
  if (!buttonToOverride) {
    return NULL;
  }
...
/*static*/ void ControlBar::parseCommandSetDefinition(INI *ini) {
  ...
  if (commandSet == NULL) {
    ...
    // command set.
    DEBUG_CRASH(("[LINE: %d in '%s'] Duplicate commandset %s found!",
  ...
  DEBUG_ASSERTCRASH(
      commandSet,
      ("parseCommandSetDefinition: Unable to allocate set '%s'\n", name.str()));
...
const CommandSet *ControlBar::findCommandSet(const AsciiString &name) {
  ...
  if (set) set = (CommandSet *)set->friend_getFinalOverride();
  return set;
}

//-------------------------------------------------------------------------------------------------
/** Allocate a new command set, link to list, initialize to default, and return
 * it */
//-------------------------------------------------------------------------------------------------
...
CommandSet *ControlBar::newCommandSetOverride(CommandSet *setToOverride) {
  ...
  // !!! DO NOT DO THIS !!! -- > set->friend_addToList(&m_commandSets); <-- !!!
  // DO NOT DO THIS !!!
...
//-------------------------------------------------------------------------------------------------
CBCommandStatus ControlBar::processContextSensitiveButtonClick(
    GameWindow *button, GadgetGameMessage gadgetMessage) {
...
/** Process a button click for the context sensitive GUI */
//-------------------------------------------------------------------------------------------------
CBCommandStatus ControlBar::processContextSensitiveButtonTransition(
    GameWindow *button, GadgetGameMessage gadgetMessage) {
  // call command processing method
...
//-------------------------------------------------------------------------------------------------
/** Switch the user interface to the new context specified and fill out any of
 * the art and/or buttons that we need to for the new context using data from
...
void ControlBar::switchToContext(ControlBarContext context, Drawable *draw) {
  ...
  if (TheHotKeyManager) TheHotKeyManager->reset();
  ...
  if (IsInGameChatActive() == FALSE && TheGameLogic &&
  ...
  switch (context) {
    ...
    case CB_CONTEXT_NONE: {
      ...
      // if there is a current selected drawable then we wil display a selection
      // portrait if present
      if (draw) {
        // Get the current thing template.
        const ThingTemplate *thing = draw->getTemplate();

        // Special case -- if we are a GLA hole, then get the rebuild building
        // template
        Object *obj = draw->getObject();
    ...
    case CB_CONTEXT_COMMAND: {
      ...
      //
      // for objects that are able to create units, we show a build queue if we
      // actually actually have something in production, otherwise we show the
      // selection portrait
      //
      if (obj) {
    ...
    case CB_CONTEXT_OCL_TIMER: {
      // show or hide the right window groups
      //			m_contextParent[ CP_PURCHASE_SCIENCE ]->winHide(
      //TRUE );
      m_contextParent[CP_COMMAND]->winHide(TRUE);
      m_contextParent[CP_BUILD_QUEUE]->winHide(TRUE);
      m_contextParent[CP_BEACON]->winHide(TRUE);
    ...
    case CB_CONTEXT_MULTI_SELECT: {
      ...
      //			m_contextParent[ CP_PURCHASE_SCIENCE ]->winHide(
      //TRUE );
      m_contextParent[CP_COMMAND]->winHide(
          FALSE);  // multi select shows common commands
      m_contextParent[CP_BUILD_QUEUE]->winHide(TRUE);
      m_contextParent[CP_BEACON]->winHide(TRUE);
      m_contextParent[CP_UNDER_CONSTRUCTION]->winHide(TRUE);
      m_contextParent[CP_OCL_TIMER]->winHide(TRUE);
    ...
    default: {
      DEBUG_ASSERTCRASH(
          0, ("ControlBar::switchToContext, unknown context '%d'\n", context));
      break;
...
void ControlBar::setCommandBarBorder(GameWindow *button,
                                     CommandButtonMappedBorderType type) {
  if (!button) return;
...
void ControlBar::setControlCommand(GameWindow *button,
  ...
  //
  // set the button gadget control to be a normal button or a check like button
  // if the command says it needs one
  //
  ...
  //
  // set the imagry ... note that for 99% of the command buttons it's sufficient
  // to specify only the disabled, enabled, hilite, and hilite pushed images.
  // For push-like buttons we actually utilize all the state available to a
  // GameWindow.  We will replicate the hilite pushed image to be the enabled
  // pushed image ... and we will also replicate the disabled image to be the
  // disabled pushed image.  For complete control over all the states of these
  // buttons we would add additional lines to the INI for a command button and
  // store those additional images in the command button
  //
  ...
  //); 	GadgetButtonSetDisabledSelectedImage( button,
  //commandButton->getDisabledImage() ); }  //end if if(
  // commandButton->getHiliteImage() ) 	GadgetButtonSetHiliteImage( button,
  //commandButton->getHiliteImage() ); if( commandButton->getPushedImage() )
  //{
  //	GadgetButtonSetHiliteSelectedImage( button,
  //commandButton->getPushedImage() ); 	GadgetButtonSetEnabledSelectedImage(
  //button, commandButton->getPushedImage() ); }  // end if

  // set the text
  if (commandButton->getTextLabel().isEmpty() == FALSE ||
      !commandButton->getScienceVec().empty()) {
  ...
  setCommandBarBorder(button,
                      commandButton->getCommandButtonMappedBorderType());

  if (TheHotKeyManager) {
    AsciiString hotKey =
        TheHotKeyManager->searchHotKey(commandButton->getTextLabel());
    if (hotKey.isNotEmpty()) TheHotKeyManager->addHotKey(button, hotKey);
  }
...
void CommandButton::cacheButtonImage() {
  ...
  if (m_buttonImageName.isNotEmpty()) {
    ...
    DEBUG_ASSERTCRASH(m_buttonImage, ("CommandButton: %s is looking for button "
                                      "image %s but can't find it. Skipping...",
                                      m_name.str(), m_buttonImageName.str()));
...
void ControlBar::postProcessCommands(void) {
  for (CommandButton *button = m_commandButtons; button;
 ...
 * NOTE that parent may be NULL, it only helps to speed up the search for a
 * particular window ID */
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
/** show/hide the portrait image by object.  We like to use this method as
 * opposed to the plain image one above so that we can build more intelligence
 * into what portrait to show for an object given its current state or object
 * type */
//-------------------------------------------------------------------------------------------------
void ControlBar::setPortraitByObject(Object *obj) {
  if (obj) {
    if (obj->isKindOf(KINDOF_SHOW_PORTRAIT_WHEN_CONTROLLED) &&
    ...
    if (draw && draw->getStealthLook() == STEALTHLOOK_DISGUISED_ENEMY) {
      ...
      if (thing->isKindOf(KINDOF_SHOW_PORTRAIT_WHEN_CONTROLLED)) {
        // If a bomb truck disguises as a civ vehicle, don't use it's portrait
        // (or else you'll see the terrorist).
        setPortraitByObject(NULL);
        return;
      }
      ...
      if (stealth && stealth->isDisguised()) {
        ...
        player =
            ThePlayerList->getNthPlayer(stealth->getDisguisedPlayerIndex());
      }
    }
...
// ------------------------------------------------------------------------------------------------
/** Show a rally point marker at the world location specified.  If no location
 * is specified any marker that we might have visible is hidden */
// ------------------------------------------------------------------------------------------------
  ...
  if (loc == NULL) {
    ...
    if (m_rallyPointDrawableID == INVALID_DRAWABLE_ID) {
      const ThingTemplate *ttn =
          TheThingFactory->findTemplate("RallyPointMarker");
      marker = TheThingFactory->newDrawable(ttn);
      DEBUG_ASSERTCRASH(
          marker, ("showRallyPoint: Unable to create rally point drawable\n"));
      if (marker) {
    ...
    // sanity
    DEBUG_ASSERTCRASH(marker,
                      ("showRallyPoint: No rally point marker found\n"));

    // set the position of the rally point drawble to the position passed in
    marker->setPosition(loc);
    marker->setOrientation(
        TheGlobalData->m_downwindAngle);  // To blow down wind -- ML
...
void ControlBar::setControlBarSchemeByPlayer(Player *p) {
  ...
  static NameKeyType buttonPlaceBeaconID =
      NAMEKEY("ControlBar.wnd:ButtonPlaceBeacon");
  static NameKeyType buttonIdleWorkerID =
      NAMEKEY("ControlBar.wnd:ButtonIdleWorker");
  static NameKeyType buttonGeneralID = NAMEKEY("ControlBar.wnd:ButtonGeneral");
  GameWindow *buttonPlaceBeacon =
      TheWindowManager->winGetWindowFromId(NULL, buttonPlaceBeaconID);
  GameWindow *buttonIdleWorker =
      TheWindowManager->winGetWindowFromId(NULL, buttonIdleWorkerID);
  GameWindow *buttonGeneral =
      TheWindowManager->winGetWindowFromId(NULL, buttonGeneralID);
  ...
  if (!p->isPlayerActive()) {
    ...
    if (buttonPlaceBeacon) buttonPlaceBeacon->winHide(TRUE);
    if (buttonIdleWorker) buttonIdleWorker->winHide(TRUE);
    if (buttonGeneral) buttonGeneral->winEnable(FALSE);
  } else {
    switchToContext(CB_CONTEXT_NONE, NULL);
    m_isObserverCommandBar = FALSE;
    ...
    if (buttonPlaceBeacon)
      buttonPlaceBeacon->winHide(
          (TheGameLogic->getGameMode() != GAME_LAN &&
           TheGameLogic->getGameMode() != GAME_INTERNET) ||
          !TheGameInfo->isMultiPlayer());
    if (buttonIdleWorker) buttonIdleWorker->winHide(FALSE);
    if (buttonGeneral) {
...
void ControlBar::setControlBarSchemeByPlayerTemplate(const PlayerTemplate *pt) {
  ...
  if (pt == ThePlayerTemplateStore->findPlayerTemplate(
...
void ControlBar::updateBuildQueueDisabledImages(const Image *image) {
  ...
  for (i = 0; i < MAX_BUILD_QUEUE_BUTTONS; i++) {
    // get window commented out cause I believe we already set this.  We'll see
    // in a few minutes
    m_queueData[i].control = TheWindowManager->winGetWindowFromId(
        m_contextParent[CP_BUILD_QUEUE], buildQueueIDs[i]);
...
void ControlBar::updateRightHUDImage(const Image *image) {
  if (!m_rightHUDWindow || !image) return;
  m_rightHUDWindow->winSetEnabledImage(0, image);
}
...
void ControlBar::updateCommanBarBorderColors(Color build, Color action,
                                             Color upgrade, Color system) {
...
void ControlBar::hideCommunicator(Bool b) {
  // sanity
  if (m_communicatorButton != NULL) m_communicatorButton->winHide(b);
}

// ---------------------------------------------------------------------------------------
// Outside hook so when the genera's head is pushed, we can switch to the
// purchase science context
void ControlBar::updatePurchaseScience(void) {
  ...
  //		if (m_generalsScreenAnimate->isFinished() && !wasFinished &&
  //m_generalsScreenAnimate->isReversed()) 			m_contextParent[ CP_PURCHASE_SCIENCE
  //]->winHide(TRUE);
  //	}
}
...
void ControlBar::showPurchaseScience(void) {
  ...
  m_genStarFlash = FALSE;
  if (!m_contextParent[CP_PURCHASE_SCIENCE]->winIsHidden()) return;
  ...
  // m_generalsScreenAnimate->registerGameWindow( m_contextParent[
  // CP_PURCHASE_SCIENCE ], WIN_ANIMATION_SLIDE_TOP, TRUE, 200 );
}
...
void ControlBar::switchControlBarStage(ControlBarStages stage) {
  ...
  switch (stage) {
    ...
    case CONTROL_BAR_STAGE_HIDDEN:
      ...
      break;
    default:
      DEBUG_ASSERTCRASH(FALSE, ("ControlBar::switchControlBarStage we were "
                                "passed in a stage that's not supported %d",
                                stage));
  }
}
void ControlBar::setDefaultControlBarConfig(void) {
  ...
  //		m_controlBarSchemeManager->setControlBarSchemeByPlayerTemplate(ThePlayerList->getLocalPlayer()->getPlayerTemplate(),
  //FALSE);
  //	}
...
void ControlBar::setSquishedControlBarConfig(void) {
  if (m_currentControlBarStage == CONTROL_BAR_STAGE_SQUISHED) return;
  ...
  m_controlBarSchemeManager->setControlBarSchemeByPlayerTemplate(
      ThePlayerList->getLocalPlayer()->getPlayerTemplate(), TRUE);
}
...
void ControlBar::updateSlotExitImage(const Image *image) {
  ...
  if (cmdButton) cmdButton->setButtonImage(image);

  cmdButton = findNonConstCommandButton("Command_TransportExit");
  if (cmdButton) cmdButton->setButtonImage(image);
}

void ControlBar::updateUpDownImages(
    const Image *toggleButtonUpIn, const Image *toggleButtonUpOn,
    const Image *toggleButtonUpPushed, const Image *toggleButtonDownIn,
    const Image *toggleButtonDownOn, const Image *toggleButtonDownPushed,
...
void ControlBar::initSpecialPowershortcutBar(Player *player) {
  ...
  m_currentlyUsedSpecialPowersButtons =
      pt->getSpecialPowerShortcutButtonCount();
  ...
  m_specialPowerShortcutParent = TheWindowManager->winGetWindowFromId(
      NULL, id);  // m_scienceLayout->getFirstWindow();
  ...
  m_currentlyUsedSpecialPowersButtons = MIN(
      pt->getSpecialPowerShortcutButtonCount(), MAX_SPECIAL_POWER_SHORTCUTS);
  for (i = 0; i < m_currentlyUsedSpecialPowersButtons; i++) {
    ...
    m_specialPowerShortcutButtons[i]->winSetStatus(
        WIN_STATUS_USE_OVERLAY_STATES);
...
void ControlBar::populateSpecialPowerShortcut(Player *player) {
  ...
  Int i;
  if (!player || !player->getPlayerTemplate() || !player->isLocalPlayer() ||
      m_currentlyUsedSpecialPowersButtons == 0 ||
      m_specialPowerShortcutButtons == NULL ||
      m_specialPowerShortcutButtonParents == NULL)
    return;
  for (i = 0; i < m_currentlyUsedSpecialPowersButtons; ++i) {
  ...
  // get command set
  if (player->getPlayerTemplate()
          ->getSpecialPowerShortcutCommandSet()
          .isEmpty())
    return;
  commandSet = TheControlBar->findCommandSet(
      player->getPlayerTemplate()
          ->getSpecialPowerShortcutCommandSet());  // TEMP WILL CHANGE TO PROPER
                                                   // WAY ONCE WORKING
  ...
  for (i = 0; i < m_currentlyUsedSpecialPowersButtons; i++) {
    ...
    if (commandButton == NULL) {
      ...
      // show up cause we can never pick "another" general technology throughout
      // the game
      //
      if (BitTest(commandButton->getOptions(), NEED_SPECIAL_POWER_SCIENCE)) {
        const SpecialPowerTemplate *power =
            commandButton->getSpecialPowerTemplate();
        ...
        if (power && power->getRequiredScience() != SCIENCE_INVALID) {
          if (player->hasScience(power->getRequiredScience()) == FALSE) {
            ...
            // The player does have the special power! Now determine if the
            // images require enhancement based on upgraded versions. This is
            // determined by the command button specifying a vector of sciences
            // in the command button.
            Int bestIndex = -1;
            ScienceType science;
            for (Int scienceIndex = 0;
                 scienceIndex < commandButton->getScienceVec().size();
                 ++scienceIndex) {
              science = commandButton->getScienceVec()[scienceIndex];

              // Keep going until we reach the end or don't have the required
              // science!
            ...
            if (bestIndex != -1) {
              ...
              // Now we have to search through the command buttons to find a
              // matching purchase science button.
              for (const CommandButton *command = m_commandButtons; command;
                   command = command->getNext()) {
      ...
      // populate the visible button with data from the command button
      setControlCommand(m_specialPowerShortcutButtons[currentButton],
                        commandButton);
...
void ControlBar::updateSpecialPowerShortcut(void) {
  ...
  if (m_specialPowerShortcutParent->winIsHidden()) return;
  ...
  for (Int i = 0; i < m_currentlyUsedSpecialPowersButtons; i++) {
    ...
    if (ThePlayerList->getLocalPlayer()->findNaturalCommandCenter())
      availability = getCommandAvailability(
          command, ThePlayerList->getLocalPlayer()->findNaturalCommandCenter(),
          win);
...
void ControlBar::animateSpecialPowerShortcut(Bool isOn) {
  if (!m_specialPowerShortcutParent || !m_animateWindowManagerForGenShortcuts ||
      !m_currentlyUsedSpecialPowersButtons)
    return;
  ...
  if (isOn) {
    m_animateWindowManagerForGenShortcuts->reset();
    m_animateWindowManagerForGenShortcuts->registerGameWindow(
        m_specialPowerShortcutParent, WIN_ANIMATION_SLIDE_RIGHT, TRUE, 500, 0);
  } else {
...
void ControlBar::showSpecialPowerShortcut(void) {
  if (TheScriptEngine->isGameEnding() || !m_specialPowerShortcutParent ||
      !m_specialPowerShortcutButtons || !ThePlayerList ||
      !ThePlayerList->getLocalPlayer())
    return;
  Bool dontAnimate = TRUE;
  for (Int i = 0; i < m_currentlyUsedSpecialPowersButtons; ++i) {
...
void ControlBar::hideSpecialPowerShortcut(void) {
  if (!m_specialPowerShortcutParent) return;
