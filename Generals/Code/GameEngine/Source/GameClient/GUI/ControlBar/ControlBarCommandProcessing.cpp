...
// ////////////////////////////////////////////////////////// Author: Colin Day,
// March 2002 Desc:   This file contain just the method responsible for
// processing the actual command
...
//-------------------------------------------------------------------------------------------------
/** Process a button transition message from the window system that should be
 * for one of our GUI commands */
//-------------------------------------------------------------------------------------------------
CBCommandStatus ControlBar::processCommandTransitionUI(
    GameWindow *control, GadgetGameMessage gadgetMessage) {
...
//-------------------------------------------------------------------------------------------------
/** Process a button selected message from the window system that should be for
 * one of our GUI commands */
//-------------------------------------------------------------------------------------------------
CBCommandStatus ControlBar::processCommandUI(GameWindow *control,
  ...
  const CommandButton *commandButton =
      (const CommandButton *)GadgetButtonGetData(control);

  // sanity, we won't process messages if we have no source object,
  // unless we're CB_CONTEXT_PURCHASE_SCIENCE or
  ...
  //
  // get the object that is driving the context sensitive UI if we're not in a
  ...
  //@todo Kris -- Special case code so convoy trucks can detonate nuke trucks --
  //if other things need this, rethink it.
  if (obj && BitTest(commandButton->getOptions(), SINGLE_USE_COMMAND)) {
    /** @todo Added obj check because Single Use and Multi Select crash when
     * used together, but with this check they just won't work.  When the
     * "rethinking" occurs, this can get fixed.  Right now it is unused. Convoy
     * Truck needs Multi Select so Single Use is turned off, and noone else has
     * it.
     */

    // Make sure the command button is marked as used if it's a single use
    // command. That way we can never press the button again. This was added
    // specifically for nuke convoy trucks. When you click to detonate the nuke,
    // it takes a few seconds to detonate in order to play a sound. But we want
    // to disable the button after the first click.
    obj->markSingleUseCommandUsed();  // Yeah, an object can only use one single
                                      // use command...
  }
  ...
  if (BitTest(commandButton->getOptions(), COMMAND_OPTION_NEED_TARGET)) {
    if (commandButton->getOptions() & USES_MINE_CLEARING_WEAPONSET) {
      TheMessageStream->appendMessage(
          GameMessage::MSG_SET_MINE_CLEARING_DETAIL);
    }

    // June 06, 2002 -- Major change
    // I've added support for specific context sensitive commands which need
    // targets just like other options may need. When we need a target, the user
    // must move the cursor to a position where he wants the GUI command to take
    // place. Older commands such as napalm strikes or daisy cutter drops simply
    // needed the user to click anywhere he desired.
    //
    // Now, we have new commands that will only work when the user clicks on
    // valid targets to interact with. For example, the terrorist can jack a car
    // and convert it into a carbomb, but he has to click on a valid car. In
    // this case the doCommandOrHint code will determine if the mode is valid or
    // not and the cursor modes will be set appropriately.
    ...
    switch (commandButton->getCommandType()) {
      ...
      case GUI_COMMAND_DOZER_CONSTRUCT: {
        // sanity
        if (m_currentSelectedDrawable == NULL) break;

        // Kris: September 27, 2002
        // Make sure we have enough CASH to build it WHEN we click the button to
        // build it, before actually previewing the purchase, otherwise, cancel
        // altogether.
        ...
        // tell the UI that we want to build something so we get a building at
        // the cursor
        TheInGameUI->placeBuildAvailable(commandButton->getThingTemplate(),
      ...
      case GUI_COMMAND_DOZER_CONSTRUCT_CANCEL: {
        ...
        // sanity check, the building must be under our control to cancel
        // construction
        ...
        // do the message
        TheMessageStream->appendMessage(
            GameMessage::MSG_DOZER_CANCEL_CONSTRUCT);
      ...
      case GUI_COMMAND_UNIT_BUILD: {
        ...
        if (cmt == CANMAKE_NO_MONEY) {
          ...
          DEBUG_ASSERTCRASH(0, ("Cannot create '%s' because the factory object "
                                "'%s' returns false for canMakeUnit\n",
        ...
        if (pu == NULL) {
          DEBUG_ASSERTCRASH(0, ("Cannot create '%s' because the factory object "
                                "'%s' is not capable of producting units\n",
                                whatToBuild->getName().str(),
                                factory->getTemplate()->getName().str()));
          break;
        ...
        GameMessage *msg =
            TheMessageStream->appendMessage(GameMessage::MSG_QUEUE_UNIT_CREATE);
      ...
      case GUI_COMMAND_CANCEL_UNIT_BUILD: {
        ...
        for (i = 0; i < MAX_BUILD_QUEUE_BUTTONS; i++)
          if (m_queueData[i].control == control) break;
        ...
        // sanity
        if (m_queueData[i].type != PRODUCTION_UNIT) break;
        ...
        // sanity, we must control the producer ... if this isn't true they
        // might be hacking the game
      ...
      case GUI_COMMAND_PLAYER_UPGRADE: {
        ...
        // sanity
        if (obj == NULL || upgradeT == NULL) break;

        // make sure the player can really make this
        if (TheUpgradeCenter->canAffordUpgrade(ThePlayerList->getLocalPlayer(),
                                               upgradeT, TRUE) == FALSE) {
          break;
        }
        ...
        // send the message
        GameMessage *msg =
            TheMessageStream->appendMessage(GameMessage::MSG_QUEUE_UPGRADE);
      ...
      case GUI_COMMAND_OBJECT_UPGRADE: {
        ...
        DEBUG_ASSERTCRASH(
            upgradeT,
            ("Undefined upgrade '%s' in object upgrade command\n", "UNKNOWN"));
        // sanity
        ...
        if (TheUpgradeCenter->canAffordUpgrade(ThePlayerList->getLocalPlayer(),
                                               upgradeT, TRUE) == FALSE) {
          // Kris: Disabled because we can get a valid reason for not being able
          // to afford the upgrade! TheInGameUI->message( "upgrade unsupported
          // in commandprocessing." );
          break;
        }
        ...
        if (obj) objID = obj->getID();

        // make sure that the this object can actually build the upgrade
        if (obj && (obj->hasUpgrade(upgradeT) == TRUE ||
                    obj->affectedByUpgrade(upgradeT) == FALSE))
          break;
      ...
      case GUI_COMMAND_CANCEL_UPGRADE: {
        ...
        // sanity
        if (m_queueData[i].type != PRODUCTION_UPGRADE) break;
        ...
        // sanity
        if (upgradeT == NULL || producer == NULL) break;

        // send the message
        GameMessage *msg =
            TheMessageStream->appendMessage(GameMessage::MSG_CANCEL_UPGRADE);
        msg->appendIntegerArgument(upgradeT->getUpgradeNameKey());
      ...
      case GUI_COMMAND_ATTACK_MOVE:
        TheMessageStream->appendMessage(
            GameMessage::MSG_META_TOGGLE_ATTACKMOVE);
        break;
      ...
      case GUI_COMMAND_EXIT_CONTAINER: {
        ...
        //
        // find the object ID that wants to exit by scanning through the
        // transport data and looking for the matching control button
        //
        ...
        if (objWantingExit == NULL) {
          //
          // remove from inventory data to avoid future matches ... the
          // inventory update cycle of the UI will repopulate any buttons as the
          // contents of objects change so this is only an edge case that will
          // be visually corrected next frame
          //
        ...
        GameMessage *exitMsg =
            TheMessageStream->appendMessage(GameMessage::MSG_EXIT);
        exitMsg->appendObjectIDArgument(
            objWantingExit->getID());  // 0 is the thing inside coming out
      ...
      case GUI_COMMAND_EXECUTE_RAILED_TRANSPORT: {
        TheMessageStream->appendMessage(
            GameMessage::MSG_EXECUTE_RAILED_TRANSPORT);
        break;
      ...
      case GUI_COMMAND_SWITCH_WEAPON: {
        // command needs no additional data, send the message
        GameMessage *msg =
            TheMessageStream->appendMessage(GameMessage::MSG_SWITCH_WEAPONS);
        ...
        pickAndPlayUnitVoiceResponse(TheInGameUI->getAllSelectedDrawables(),
                                     GameMessage::MSG_SWITCH_WEAPONS, &info);
      ...
      case GUI_COMMAND_FIRE_WEAPON: {
        // command needs no additional data, send the message
        GameMessage *msg =
            TheMessageStream->appendMessage(GameMessage::MSG_DO_WEAPON);
        msg->appendIntegerArgument(commandButton->getWeaponSlot());
      ...
      case GUI_COMMAND_SPECIAL_POWER_FROM_COMMAND_CENTER: {
        ...
        if (cmdCenter == NULL) break;

        // command needs no additional data, send the message
        GameMessage *msg =
            TheMessageStream->appendMessage(GameMessage::MSG_DO_SPECIAL_POWER);
      ...
      case GUI_COMMAND_PURCHASE_SCIENCE: {
        // loop through all the sciences on the button and select the one we
        // don't have
        ...
        GameMessage *msg =
            TheMessageStream->appendMessage(GameMessage::MSG_PURCHASE_SCIENCE);
