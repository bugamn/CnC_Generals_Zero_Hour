...
// ///////////////////////////////////////////////////////// Author: Colin Day,
// March 2002 Desc:   Methods specific to the control bar garrison display
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void ControlBar::populateButtonProc(Object *obj, void *userData) {
  ...
  // sanity
  DEBUG_ASSERTCRASH(
      info->buttonIndex < MAX_STRUCTURE_INVENTORY_BUTTONS,
  ...
  // set the UI button that will allow us to press it and cause the object to
  // exit the container
  const Image *image;
  ...
  GadgetButtonDrawOverlayImage(info->inventoryButtons[info->buttonIndex],
                               image);
...
void ControlBar::populateStructureInventory(Object *building) {
  ...
  // reset hotkeys -- seeing it only is reset in switching contexts. This is a
  // special case because we are building the hotkeys on the fly sort of... and
  // it changes as guys enter and leave. Taking this call out will cause
  // multiple hotkeys to be added.
  ...
  DEBUG_ASSERTCRASH(
      contain,
      ("Object in structure inventory does not contain a Contain Module\n"));
  if (!contain) return;
  ...
  for (i = 0; i < MAX_STRUCTURE_INVENTORY_BUTTONS; i++) {
    ...
    //
    // disable the button for now, it will be enabled if there is something
    // there for its contents
    //
    ...
    //
    // if the structure can hold a lesser amount inside it than what the GUI
    // displays we will completely hide the buttons that can't contain anything
    //
    if (i + 1 > contain->getContainMax()) m_commandWindows[i]->winHide(TRUE);
  ...
  //
  // iterate each of the objects inside the container and put them in a button,
  // note we're iterating in reverse order here
  //
  ...
  //
  // save how many items were contained by the object at this time so that we
  // can update it if they change in the future while selected
  //
...
void ControlBar::updateContextStructureInventory(void) {
  ...
  //
  // we're visible, so there is something selected.  It is possible that we had
  // a building selected that can be garrisoned, and while it was selected the
  // enemy occupied it. in that case we want to unselect the building so that we
  ...
  if (source->isLocallyControlled() == FALSE &&
    ...
    if (draw) TheInGameUI->deselectDrawable(draw);
    return;

  }  // end if

  //
  // if the object being displayed in the interface has a different count than
  // we last knew about we need to repopulate the buttons of the interface
  //
  ContainModuleInterface *contain = source->getContain();
  DEBUG_ASSERTCRASH(
      contain, ("No contain module defined for object in the iventory bar\n"));
  if (!contain) return;
