...
//
//	purpose:	We want a "squished" control bar, this is the methods
//that will do it
//
//-----------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////
...
const FieldParse ControlBarResizer::m_controlBarResizerParseTable[] = {
    {"AltPosition", INI::parseICoord2D, NULL,
     offsetof(ResizerWindow, m_altPos)},
...
void ControlBarResizer::init(void) {
  ...
  ini.load(AsciiString("Data\\INI\\ControlBarResizer.ini"), INI_LOAD_OVERWRITE,
           NULL);
}
...
ResizerWindow *ControlBarResizer::findResizerWindow(AsciiString name) {
  ...
  while (it != m_resizerWindowsList.end()) {
    ...
    if (!rWin) {
      DEBUG_ASSERTCRASH(
          FALSE,
          ("There's no resizerWindow in ControlBarResizer::findResizerWindow"));
      it++;
      continue;
    }
...
ResizerWindow *ControlBarResizer::newResizerWindow(AsciiString name) {
  ...
  if (!newRwin) return NULL;

  newRwin->m_name = name;
  GameWindow *win = NULL;
  win = TheWindowManager->winGetWindowFromId(
      NULL, TheNameKeyGenerator->nameToKey(name));
  if (!win) {
    DEBUG_ASSERTCRASH(win,
                      ("ControlBarResizer::newResizerWindow could not find "
                       "window %s Are you sure that window is loaded yet?",
                       name.str()));
    delete newRwin;
    return NULL;
  }
...
void ControlBarResizer::sizeWindowsDefault(void) {
  ...
  while (it != m_resizerWindowsList.end()) {
    ResizerWindow *rWin = *it;
    if (!rWin) {
      DEBUG_ASSERTCRASH(FALSE, ("There's no resizerWindow in "
                                "ControlBarResizer::sizeWindowsDefault"));
      it++;
      continue;
    }
    win = TheWindowManager->winGetWindowFromId(
        NULL, TheNameKeyGenerator->nameToKey(rWin->m_name));
    if (!win) {
      it++;
      continue;
    }
    ...
    DEBUG_LOG(("sizeWindowsDefault:%s pos X:%d pos Y: %d size X:%d sizeY: %d",
               rWin->m_name.str(), rWin->m_defaultPos.x, rWin->m_defaultPos.y,
               rWin->m_defaultSize.x, rWin->m_defaultSize.y));
    it++;
  }
}
void ControlBarResizer::sizeWindowsAlt(void) {
  ...
  while (it != m_resizerWindowsList.end()) {
    ...
    DEBUG_LOG(("sizeWindowsAlt:%s pos X:%d pos Y: %d size X:%d sizeY: %d",
               rWin->m_name.str(), rWin->m_altPos.x * x, rWin->m_altPos.y * y,
               rWin->m_altSize.x * x, rWin->m_altSize.y * y));
    it++;
  }
}
...
void INI::parseControlBarResizerDefinition(INI *ini) {
  ...
  //	if( !resizer )
  //	{
  //		//We don't need it if we're in the builder... which doesn't have
  //this. 		return;
  //	}
  ...
  //		DEBUG_ASSERTCRASH( rWin, ("parseControlBarResizerDefinition:
  //unable to allocate ResizerWindow for '%s'\n", 															name.str()) );
  //
  //	}  // end if
  //
