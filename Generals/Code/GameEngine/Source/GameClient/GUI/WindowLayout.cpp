...
WindowLayout::~WindowLayout(void) {
  //
  // it is the users responsability to remove windows from the layout beforing
  // destroying the layout itself.  This allows for maximum flexibility of the
  // window layouts and you can use them in any you see fit, as long as they are
  // clean when they go away
  //
  DEBUG_ASSERTCRASH(
      m_windowList == NULL,
      ("Window layout being destroyed still has window references"));
  DEBUG_ASSERTCRASH(
      m_windowTail == NULL,
      ("Window layout being destroyed still has window references"));
...
void WindowLayout::addWindow(GameWindow *window) {
  ...
  if (win == NULL) {
    ...
    if (m_windowList) m_windowList->winSetPrevInLayout(window);
    m_windowList = window;
    ...
    if (m_windowTail == NULL) m_windowTail = window;
...
void WindowLayout::removeWindow(GameWindow *window) {
  ...
  if (win) {
    ...
    if (next) next->winSetPrevInLayout(prev);
    if (prev)
    ...
    if (m_windowTail == win) m_windowTail = prev;
