...
static GameWindow *curWindow =
    NULL; /**< so we can keep track of the input
  ...
  switch (msg) {
    ...
    case GWM_CHAR:
      if (BitTest(mData2, KEY_STATE_DOWN) &&
        ...
        return MSG_IGNORED;  // text extries shouldn't care about CTRL+* or
                             // ALT+*
      }
...
WindowMsgHandledType GadgetTextEntrySystem(GameWindow *window, UnsignedInt msg,
  ...
  switch (msg) {
    ...
    case GEM_SET_TEXT: {
      ...
      for (Int i = 0; i < len; i++) e->sText->appendChar(L'*');
    ...
    case GWM_DESTROY:
      ...
      if (e->constructList) TheWindowManager->winDestroy(e->constructList);
    ...
    case GWM_INPUT_FOCUS:
      if (mData1 == FALSE) {
        ...
        if (e->constructList) e->constructList->winHide(TRUE);
        e->constructText->setText(UnicodeString::TheEmptyString);
        e->conCharPos = 0;
        if (TheIMEManager && TheIMEManager->isAttachedTo(window))
        ...
        curWindow = window;
        /// @todo need to enable this for IME support
        if (TheIMEManager) TheIMEManager->attach(window);
...
&& (	(e->aSCIIOnly == FALSE )					&&
                                ...
                                (e->numericalOnly == FALSE ) ) )
        {
                ...
                // always update the construction buffer after a key has come
through here. TheWindowManager->winStrcpy( e->constructText, (WideChar
*)ourIME->Composition_Get() ); e->conCharPos = NoxStrlen( e->constructText );
                ...
                else
                {
                        ...
                        e->constructList->winGetSize( &constructSize.x,
&constructSize.y ); list->slider->winGetSize( &sliderSize.x, &sliderSize.y );

                        TheWindowManager->winSendSystemMsg( e->constructList,
GLM_DEL_ALL, 0, 0 );
                        ...
                        for( i=0; i<listCount; i++ )
                        {
                                ...
                                WideChar *text = (WideChar
*)ourIME->CandidateList_GetItem( i );

                                TheWindowManager->winGetTextSize(
e->constructList->instData.font, text, NULL, &tempWidth, 0 );
                                ...
                                TheWindowManager->winSendSystemMsg(
e->constructList, GLM_ADD_ENTRY, (WindowMsgData)&tmp, -1 );
                        }
...
UnicodeString GadgetTextEntryGetText(GameWindow *textentry) {
  // sanity
  if (textentry == NULL) return UnicodeString::TheEmptyString;
  ...
  TheWindowManager->winSendSystemMsg(textentry, GEM_GET_TEXT, 0,
                                     (WindowMsgData)&result);
  return result;
