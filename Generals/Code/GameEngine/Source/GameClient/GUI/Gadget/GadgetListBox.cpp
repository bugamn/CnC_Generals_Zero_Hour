...
// DEFINES ////////////////////////////////////////////////////////////////////
// Sets up the user's OS set doubleclick time so if they don't like it... they
// can change it in their OS.
...
static Int getListboxEntryBasedOnCoord(GameWindow *window, Int x, Int y,
                                       Int &row, Int &column) {
  Int pos;
  Int winx, winy, i;
  ...
  for (i = 0;; i++) {
    ...
    if (list->listData[i].listHeight > (y - winy + list->displayPos)) break;
  }
...
Int GadgetListBoxGetEntryBasedOnXY(GameWindow *listbox, Int x, Int y, Int &row,
                                   Int &column) {
...
static Int getListboxTopEntry(ListboxData *list) {
  ...
  for (entry = 0;; entry++) {
    if (list->listData[entry].listHeight > list->displayPos) return entry;

    if (entry >= list->endPos) return 0;
  }
...
static Int getListboxBottomEntry(ListboxData *list) {
  ...
  for (entry = list->endPos - 1;; entry--) {
    if (list->listData[entry].listHeight ==
        list->displayPos + list->displayHeight)
      return entry;
    if (list->listData[entry].listHeight <
            list->displayPos + list->displayHeight &&
...
static void adjustDisplay(GameWindow *window, Int adjustment,
  ...
  if (list->slider != NULL) {
    ...
    sData->maxVal =
        list->totalHeight - (list->displayHeight - TOTAL_OUTLINE_HEIGHT) + 1;
    ...
    sData->numTicks =
        (float)((sliderSize.y - sliderChildSize.y) / (float)sData->maxVal);

    if (updateSlider)
      TheWindowManager->winSendSystemMsg(list->slider, GSM_SET_SLIDER,
                                         (sData->maxVal - list->displayPos), 0);
  }
...
static void computeTotalHeight(GameWindow *window) {
  ...
  for (i = 0; i < list->endPos; i++) {
    if (!list->listData[i].cell) continue;
    tempHeight = 0;
    ...
    for (Int j = 0; j < list->columns; j++) {
      ...
      if (list->listData[i].cell[j].cellType == LISTBOX_TEXT) {
        if (BitTest(window->winGetStatus(), WIN_STATUS_ONE_LINE) == TRUE) {
          ...
          DisplayString *displayString =
              (DisplayString *)list->listData[i].cell[j].data;
          if (displayString) displayString->getSize(NULL, &cellHeight);
        }  // else
      }  // if
      ...
      if (cellHeight > tempHeight) tempHeight = cellHeight;
    }  // for
...
// addImageEntry ==============================================================
/** Add Images to position and column. Row and Column are both based from
   starting Position 0 */
//=============================================================================
static Int addImageEntry(const Image *image, Color color, Int row, Int column,
                         GameWindow *window, Bool overwrite, Int width,
                         Int height) {
  //	WinInstanceData *instData = window->winGetInstanceData();
  ListboxData *list = (ListboxData *)window->winGetUserData();
  ...
  if (column >= list->columns || row >= list->listLength) {
    DEBUG_ASSERTCRASH(false,
                      ("Tried to add Image to Listbox at invalid position"));
    return -1;
  }
  ...
  // Check and see if we have allocated cells for that row yet, if not, allocate
  // them
  if (!listRow->cell) {
  ...
  if (listRow->cell[column].cellType == LISTBOX_TEXT) {
    TheDisplayStringManager->freeDisplayString(
        (DisplayString *)listRow->cell[column].data);
  }
...
// startingRow will get moved to startingRow+1, etc.  This assumes there is
// space!!!!!
static Int moveRowsDown(ListboxData *list, Int startingRow) {
  ...
  if (list->multiSelect) {
    ...
    while (list->selections[i] >= 0) {
      if (startingRow <= list->selections[i]) list->selections[i]++;
      i++;
    }
  } else {
    if (list->selectPos >= startingRow) list->selectPos++;
  }
...
//=============================================================================
static Int addEntry(UnicodeString *string, Int color, Int row, Int column,
  ...
  if (column >= list->columns || row >= list->listLength) {
    DEBUG_ASSERTCRASH(false,
                      ("Tried to add text to Listbox at invalid position"));
    return -1;
  }
  ...
  ListEntryRow *listRow = &list->listData[row];
  // Here I've decided to just overright what's in the row, if that's not what
  // we want, change it here Check and see if we have allocated cells for that
  // row yet, if not, allocate them
  if (!listRow->cell) {
    listRow->cell = NEW ListEntryCell[list->columns];
    memset(listRow->cell, 0, list->columns * sizeof(ListEntryCell));
    rowsAdded = 1;
  ...
  if (!listRow->cell[column].data)
    listRow->cell[column].data =
        (void *)TheDisplayStringManager->newDisplayString();
...
WindowMsgHandledType GadgetListBoxInput(GameWindow *window, UnsignedInt msg,
  ...
  switch (msg) {
    ...
    case GWM_CHAR: {
      switch (mData1) {
        ...
        case KEY_SPACE: {
          if (BitTest(mData2, KEY_STATE_UP)) {
            ...
            TheWindowManager->winSendSystemMsg(
                window->winGetOwner(), GLM_DOUBLE_CLICKED,
          ...
          if (BitTest(mData2, KEY_STATE_DOWN)) {
            if (list->selectPos == -1) {
              ...
              while (1) {
                ...
                Int cellTop =
                    cellBottom - list->listData[list->selectPos].height;
                Int displayTop = list->displayPos;
                Int displayBottom = list->displayPos + list->displayHeight -
                                    1;  // account for the border
            ...
            TheWindowManager->winSendSystemMsg(
                window->winGetOwner(), GLM_SELECTED, (WindowMsgData)window,
                list->selectPos);
          }  // end if
            ...
            if (list->selectPos == -1) {
              ...
              while (1) {
                if (list->listData[list->selectPos].listHeight -
                    ...
                    list->displayPos) {
                  list->displayPos =
                      list->listData[list->selectPos].listHeight + 1;
                  adjustDisplay(window, -1, TRUE);
                } else if (list->listData[list->selectPos].listHeight >
                           list->displayPos + list->displayHeight) {
                  list->displayPos =
                      list->listData[list->selectPos].listHeight -
                      list->displayHeight;
                  adjustDisplay(window, 1, TRUE);
                } else {
                  adjustDisplay(window, 0, TRUE);
        ...
        default: {
          Bool foundIt = false;
          if (BitTest(mData2, KEY_STATE_DOWN)) {
            // set the position to start looking for the line of text with this
            // character
            Int position = list->selectPos;
            // only search the max number of times so that we're not looping
            // over and over
            for (Int i = 0; i < list->endPos; ++i) {
              // start at the next position
              ++position;
              // if we've reached the end of the list, start at the beginning
              if (position >= list->endPos) position = 0;
              ...
              if (!cell || cell->cellType != LISTBOX_TEXT) continue;
              ...
              for (j = 0; j < TheKeyboard->MAX_KEY_STATES; ++j) {
                if (dString->getText().getCharAt(0) ==
                  ...
                  // list->displayPos = list->listData[position].listHeight -
                  // list->displayHeight;
    ...
    case GWM_LEFT_UP: {
      ...
      TheWindowManager->winSendSystemMsg(window->winGetOwner(), GLM_SELECTED,
    ...
    case GWM_RIGHT_DOWN:
      doAudioFeedback(window);
      break;  // if we're in game, we want to eat this message because we're
              // right clicking on a listbox
    case GWM_RIGHT_UP: {
      TheWindowManager->winSetFocus(window);
      Int pos;
      Int mousex = mData1 & 0xFFFF;
      ...
      rc.mouseY = mousey;
      TheWindowManager->winSendSystemMsg(
          window->winGetOwner(), GLM_RIGHT_CLICKED, (WindowMsgData)window,
          (WindowMsgData)&rc);
      break;
...
//=============================================================================
WindowMsgHandledType GadgetListBoxMultiInput(GameWindow *window,
                                             UnsignedInt msg,
                                             WindowMsgData mData1,
                                             WindowMsgData mData2) {
  ListboxData *list = (ListboxData *)window->winGetUserData();
  ...
  switch (msg) {
    ...
    case GWM_LEFT_UP:
      // case GWM_LEFT_CLICK:
      {
        ...
        TheWindowManager->winSendSystemMsg(window->winGetOwner(), GLM_SELECTED,
                                           (WindowMsgData)window, selectPos);
        break;
    ...
    case GWM_RIGHT_UP: {
      ...
      //			Int mousex = mData1 & 0xFFFF;
                              ...
                              // Adjust for title if present
                              if( instData->getTextLength() )
                                      y += TheWindowManager->winFontHeight(
      instData->getFont() ) + 1;
                              ...
                              for( i = 0; ; i++ )
                              {
      ...
      ].listHeight > (list->displayPos + list->displayHeight ) )
                                              {
                                                      selectPos = -1;
                                                      break;
                                              }
                                      ...
                                      if( list->listData[i].listHeight > (mousey
      - y + list->displayPos) ) break;
                              }
                              ...
                              TheWindowManager->winSendSystemMsg(
      window->winGetOwner(), GLM_RIGHT_CLICKED, (WindowMsgData)window, selectPos
      );*/
      TheWindowManager->winSetFocus(window);
      Int pos;
      Int mousex = mData1 & 0xFFFF;
    ...
    case GWM_WHEEL_UP:
      ...
      if (list->upButton) {
        if (list->displayPos > 0) adjustDisplay(window, -1, TRUE);
      }
      break;
  ...
  switch (msg) {
    ...
    case GLM_GET_TEXT: {
      ...
      if (pos->x >= list->columns || pos->y >= list->listLength ||
        ...
        tAndC->string =
            ((DisplayString *)list->listData[pos->y].cell[pos->x].data)
                ->getText();
    ...
    case GLM_DEL_ALL: {
      ...
      //
      // Loop through and destroy any display strings we've allocated that
      // aren't used
    ...
    case GLM_DEL_ENTRY: {
      ...
      if (list->endPos <= (Int)mData1) break;
    ...
    case GLM_ADD_ENTRY: {
      ...
      if (addInfo->row >= list->insertPos) addInfo->row = -1;
      ...
      } else if (addInfo->row != -1 && !addInfo->overwrite &&
                 list->insertPos == list->listLength) {
        // We're inserting into the middle with no space - see if we can scroll
        // the window
        if (list->autoPurge)
          TheWindowManager->winSendSystemMsg(window, GLM_SCROLL_BUFFER, 1, 0);
        else
      ...
      if (success) {
        if (addInfo->type == LISTBOX_TEXT) {
          addedIndex =
              addEntry((UnicodeString *)addInfo->data, mData2, addInfo->row,
                       addInfo->column, window, addInfo->overwrite);
        } else if (addInfo->type == LISTBOX_IMAGE) {
          addedIndex =
              addImageEntry((const Image *)addInfo->data, mData2, addInfo->row,
                            addInfo->column, window, addInfo->overwrite,
                            addInfo->width, addInfo->height);
        } else
          success = FALSE;
      }
      ...
      if (success) {
        ...
        if (list->multiSelect) {
          ...
          while (list->selections[i] >= 0) {
            if ((row = list->selections[i]) != 0) list->selections[i] = -1;
          ...
          if (row == list->selectPos) list->selectPos = -1;
        }
    ...
    case GLM_TOGGLE_MULTI_SELECTION: {
      ...
      if (!list->listData[mData1].cell) break;
    ...
    case GLM_SET_SELECTION: {
      ...
      DEBUG_ASSERTCRASH(list->multiSelect || selectCount == 1,
                        ("Bad selection size"));
      ...
      if (list->multiSelect) {
        ...
        if (list->listData[list->selectPos].listHeight < list->displayPos) {
          TheWindowManager->winSendSystemMsg(window, GLM_UPDATE_DISPLAY,
                   ...
                   (list->displayPos + list->displayHeight)) {
          if (list->selectPos > 0)
            list->displayPos = list->listData[list->selectPos].listHeight -
                               list->displayHeight;
          else
            list->displayPos = 0;
    ...
    case GLM_SCROLL_BUFFER: {
      ...
      if (list->multiSelect) {
        ...
        if (list->selectPos > 0) list->selectPos -= mData1;
      }

      if (list->displayPos > 0) adjustDisplay(window, (-1 * mData1), TRUE);
    ...
    case GGM_RESIZED: {
      ...
      if (list->upButton) list->upButton->winGetSize(&upSize.x, &upSize.y);
      if (list->slider) {
        list->slider->winGetSize(&sliderSize.x, &sliderSize.y);
        child = list->slider->winGetChild();
        if (child) child->winGetSize(&sliderChildSize.x, &sliderChildSize.y);
      }
      ...
      if (instData->getTextLength()) {
        list->displayHeight -=
            TheWindowManager->winFontHeight(instData->getFont());
      }
      ...
      if (list->columns == 1) {
        ...
        if (!list->columnWidthPercentage) break;
        if (!list->columnWidth) break;
        ...
        for (Int i = 0; i < list->columns; i++) {
          list->columnWidth[i] =
              list->columnWidthPercentage[i] * totalWidth / 100;
        }  // for
      }  // else
    ...
    case GWM_DESTROY: {
      ...
      for (i = 0; i < list->listLength; i++) {
        // We're now onto a row of cells we are not using anymore Pull off the
        // cells and loop through them
        ListEntryCell *cells = list->listData[i].cell;
        for (int j = list->columns - 1; j >= 0; j--) {
          if (!cells) break;
          if (cells[j].cellType == LISTBOX_TEXT) {
            // If we can delete the stuff that won't be showing up in the new
            // listData struture
            if (cells[j].data) {
              TheDisplayStringManager->freeDisplayString(
                  (DisplayString *)cells[j].data);
          ...
          // Null out the data pointers so they're not destroyed when we free up
          // this listdata
          cells[j].userData = NULL;
          cells[j].data = NULL;
        }
      ...
      if (list->columnWidth) delete[] (list->columnWidth);
      if (list->columnWidthPercentage) delete[] (list->columnWidthPercentage);
      if (list->multiSelect) delete[] (list->selections);
      delete (list);
      break;
    ...
    case GSM_SLIDER_TRACK: {
      ...
      if (list->displayPos < 0) list->displayPos = 0;
...
void GadgetListBoxSetColors(
    ...
    Color hiliteColor, Color hiliteBorderColor, Color hiliteSelectedItemColor,
  ...
  GadgetListBoxSetEnabledSelectedItemBorderColor(
      listbox, enabledSelectedItemBorderColor);
  ...
  GadgetListBoxSetDisabledSelectedItemBorderColor(
      listbox, disabledSelectedItemBorderColor);
  ...
  if (slider) {
    ...
    GadgetSliderSetEnabledBorderColor(
        slider, GadgetListBoxGetEnabledBorderColor(listbox));

    // Disabled
    GadgetSliderSetDisabledColor(slider,
                                 GadgetListBoxGetDisabledColor(listbox));
    GadgetSliderSetDisabledBorderColor(
        slider, GadgetListBoxGetDisabledBorderColor(listbox));

    // Hilite
    GadgetSliderSetHiliteColor(slider, GadgetListBoxGetHiliteColor(listbox));
    GadgetSliderSetHiliteBorderColor(
        slider, GadgetListBoxGetHiliteBorderColor(listbox));

    // up button ------------------------------------------------------------

    // enabled
    GadgetButtonSetEnabledColor(upButton, GadgetSliderGetEnabledColor(slider));
    GadgetButtonSetEnabledBorderColor(
        upButton, GadgetSliderGetEnabledBorderColor(slider));
    GadgetButtonSetEnabledSelectedColor(
        upButton, GadgetSliderGetEnabledSelectedThumbColor(slider));
    GadgetButtonSetEnabledSelectedBorderColor(
        upButton, GadgetSliderGetEnabledSelectedThumbBorderColor(slider));

    // disabled
    GadgetButtonSetDisabledColor(upButton,
                                 GadgetSliderGetDisabledColor(slider));
    GadgetButtonSetDisabledBorderColor(
        upButton, GadgetSliderGetDisabledBorderColor(slider));
    GadgetButtonSetDisabledSelectedColor(
        upButton, GadgetSliderGetDisabledSelectedThumbColor(slider));
    GadgetButtonSetDisabledSelectedBorderColor(
        upButton, GadgetSliderGetDisabledSelectedThumbBorderColor(slider));

    // hilite
    GadgetButtonSetHiliteColor(upButton, GadgetSliderGetHiliteColor(slider));
    GadgetButtonSetHiliteBorderColor(upButton,
                                     GadgetSliderGetHiliteBorderColor(slider));
    GadgetButtonSetHiliteSelectedColor(
        upButton, GadgetSliderGetHiliteSelectedThumbColor(slider));
    GadgetButtonSetHiliteSelectedBorderColor(
        upButton, GadgetSliderGetHiliteSelectedThumbBorderColor(slider));

    // down button ----------------------------------------------------------

    // enabled
    GadgetButtonSetEnabledColor(downButton,
                                GadgetSliderGetEnabledColor(slider));
    GadgetButtonSetEnabledBorderColor(
        downButton, GadgetSliderGetEnabledBorderColor(slider));
    GadgetButtonSetEnabledSelectedColor(
        downButton, GadgetSliderGetEnabledSelectedThumbColor(slider));
    GadgetButtonSetEnabledSelectedBorderColor(
        downButton, GadgetSliderGetEnabledSelectedThumbBorderColor(slider));

    // disabled
    GadgetButtonSetDisabledColor(downButton,
                                 GadgetSliderGetDisabledColor(slider));
    GadgetButtonSetDisabledBorderColor(
        downButton, GadgetSliderGetDisabledBorderColor(slider));
    GadgetButtonSetDisabledSelectedColor(
        downButton, GadgetSliderGetDisabledSelectedThumbColor(slider));
    GadgetButtonSetDisabledSelectedBorderColor(
        downButton, GadgetSliderGetDisabledSelectedThumbBorderColor(slider));

    // hilite
    GadgetButtonSetHiliteColor(downButton, GadgetSliderGetHiliteColor(slider));
    GadgetButtonSetHiliteBorderColor(downButton,
                                     GadgetSliderGetHiliteBorderColor(slider));
    GadgetButtonSetHiliteSelectedColor(
        downButton, GadgetSliderGetHiliteSelectedThumbColor(slider));
    GadgetButtonSetHiliteSelectedBorderColor(
        downButton, GadgetSliderGetHiliteSelectedThumbBorderColor(slider));
...
/** Get the text for a list box entry */
//=============================================================================
UnicodeString GadgetListBoxGetTextAndColor(GameWindow *listbox, Color *color,
                                           Int row, Int column) {
  *color = 0;
  // sanity
  ...
  pos.x = column;
  pos.y = row;
  TheWindowManager->winSendSystemMsg(listbox, GLM_GET_TEXT, (WindowMsgData)&pos,
                                     (WindowMsgData)&tAndC);
...
Int GadgetListBoxAddEntryText(GameWindow *listbox, UnicodeString text,
                              ...
                              Bool overwrite) {
  if (!listbox) return -1;
  if (text.isEmpty()) text = UnicodeString(L" ");
  Int index;
  ...
  index = (Int)TheWindowManager->winSendSystemMsg(
      listbox, GLM_ADD_ENTRY, (WindowMsgData)&addInfo, color);

  // DEBUG_ASSERTLOG(!listData->scrollIfAtEnd, ("Adding line %d (orig end was
  // %d, newEntryOffset is %d, (%d-%d)?=%d, isFull=%d/%d ll=%d, end=%d\n",
  // index, oldBottomIndex, newEntryOffset, index, oldBottomIndex,
  // newEntryOffset, wasFull, GadgetListBoxIsFull(listbox),
  // listData->listLength, listData->endPos));
...
/** Add a new string entry into the listbox at the insert position */
//=============================================================================
Int GadgetListBoxAddEntryImage(GameWindow *listbox, const Image *image, Int row,
                               Int column, Int hight, Int width, Bool overwrite,
                               Color color) {
  Int index;
  AddMessageStruct addInfo;
  addInfo.row = row;
...
Int GadgetListBoxAddEntryImage(GameWindow *listbox, const Image *image, Int row,
                               Int column, Bool overwrite, Color color) {
  return GadgetListBoxAddEntryImage(listbox, image, row, column, -1, -1,
                                    overwrite, color);
}
...
void GadgetListBoxSetFont(GameWindow *g, GameFont *font) {
  ...
  if (dString) dString->setFont(font);
    ...
    for (Int i = 0; i < listData->listLength; i++) {
      if (listData->listData[i].cell)
        for (Int j = 0; j < listData->columns; j++) {
          if (listData->listData[i].cell[j].cellType == LISTBOX_TEXT &&
              listData->listData[i].cell[j].data) {
...
void GadgetListboxCreateScrollbar(GameWindow *listbox) {
  ...
  // do we have a title
  if (listbox->winGetTextLength()) title = TRUE;
  ...
  listData->upButton = TheWindowManager->gogoGadgetPushButton(
      listbox, status | WIN_STATUS_ACTIVE | WIN_STATUS_ENABLED,
      width - buttonWidth - 2, top + 2, buttonWidth, buttonHeight, &winInstData,
      NULL, TRUE);
  ...
  listData->downButton = TheWindowManager->gogoGadgetPushButton(
      listbox, status | WIN_STATUS_ACTIVE | WIN_STATUS_ENABLED,
      width - buttonWidth - 2, (top + bottom - buttonHeight - 2), buttonWidth,
      buttonHeight, &winInstData, NULL, TRUE);
  ...
  // Create Slider
  listData->slider = TheWindowManager->gogoGadgetSlider(
      listbox, status | WIN_STATUS_ACTIVE | WIN_STATUS_ENABLED,
      width - sliderButtonWidth - 2, (top + buttonHeight + 3),
      sliderButtonWidth, bottom - (2 * buttonHeight) - 6, &winInstData, &sData,
      NULL, TRUE);
...
void GadgetListBoxAddMultiSelect(GameWindow *listbox) {
  ...
  DEBUG_ASSERTCRASH(listboxData && listboxData->selections == NULL,
                    ("selections is not NULL"));
  listboxData->selections = NEW Int[listboxData->listLength];
  DEBUG_LOG(
      ("Enable list box multi select: listLength (select) = %d * %d = %d "
       "bytes;\n",
  ...
  memset(listboxData->selections, -1, listboxData->listLength * sizeof(Int));
...
void GadgetListBoxSetListLength(GameWindow *listbox, Int newLength) {
  ...
  if (!listboxData) return;
  DEBUG_ASSERTCRASH(listboxData->columns > 0,
                    ("We need at least one Column in the listbox"));
  if (listboxData->columns < 1) return;

  Int columns = listboxData->columns;
  ListEntryRow *newData = NEW ListEntryRow[newLength];
  DEBUG_ASSERTCRASH(newData,
                    ("Unable to allocate new data structures for the Listbox"));
  if (!newData) return;
  Int i;
  ...
  if (newLength >= listboxData->listLength) {
    memcpy(newData, listboxData->listData,
           listboxData->listLength * sizeof(ListEntryRow));
  } else {
    ...
    if (listboxData->insertPos > newLength) listboxData->insertPos = newLength;
  ...
  // Loop through and destroy any display strings we've allocated that aren't
  // used
  for (i = 0; i < listboxData->listLength; i++) {
    // We're now onto a row of cells we are not using anymore Pull off the cells
    ...
    if (i >= newLength) delete (listboxData->listData[i].cell);
    listboxData->listData[i].cell = NULL;
  }

  listboxData->listLength = newLength;

  if (listboxData->listData) delete (listboxData->listData);
...
Int GadgetListBoxGetNumEntries(GameWindow *listbox) {
  ...
  ListboxData *listboxData = (ListboxData *)listbox->winGetUserData();
  if (listboxData) return listboxData->endPos;

  return 0;
}  // end GadgetListBoxGetNumEntries

//-------------------------------------------------------------------------------------------------
/** Get the selected item(s) of a listbox.  For a single select listbox the
 * parameter should be a single integer pointer.  For a multi select listbox the
 * parameter should be an (Int *), this array returned will be an array of
 * indices of the selected items of the list box.  An entry of -1 in this array
 * is the "end" of the selected list, and this list will never be larger than
 * the max items in the list box */
//-------------------------------------------------------------------------------------------------
void GadgetListBoxGetSelected(GameWindow *listbox, Int *selectList) {
  ...
  TheWindowManager->winSendSystemMsg(listbox, GLM_GET_SELECTION, 0,
                                     (WindowMsgData)selectList);
...
void GadgetListBoxSetSelected(GameWindow *listbox, Int selectIndex) {
  ...
  TheWindowManager->winSendSystemMsg(listbox, GLM_SET_SELECTION,
                                     (WindowMsgData)(&selectIndex), 1);
...
//-------------------------------------------------------------------------------------------------
void GadgetListBoxSetSelected(GameWindow *listbox, const Int *selectList,
                              Int selectCount) {
  // sanity
  if (listbox == NULL) return;
  // set selected index via system message
  TheWindowManager->winSendSystemMsg(listbox, GLM_SET_SELECTION,
                                     (WindowMsgData)selectList, selectCount);
}
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void GadgetListBoxSetItemData(GameWindow *listbox, void *data, Int row,
                              Int column) {
  ICoord2D pos;
  pos.x = column;
  pos.y = row;
  ...
  if (listbox)
    TheWindowManager->winSendSystemMsg(
        listbox, GLM_SET_ITEM_DATA, (WindowMsgData)&pos, (WindowMsgData)data);
...
void *GadgetListBoxGetItemData(GameWindow *listbox, Int row, Int column) {
  ...
  if (listbox) {
    TheWindowManager->winSendSystemMsg(
        listbox, GLM_GET_ITEM_DATA, (WindowMsgData)&pos, (WindowMsgData)&data);
  }
  return (data);
}
...
bool GadgetListBoxIsFull(GameWindow *window) {
  ...
  if (listData->listData[entry].listHeight >=
