  ...
  switch (msg) {
    ...
    case GWM_MOUSE_ENTERING: {
      ...
      if (window->winGetParent() &&
          BitTest(window->winGetParent()->winGetStyle(), GWS_HORZ_SLIDER)) {
        WinInstanceData *instDataParent =
            window->winGetParent()->winGetInstanceData();
    ...
    case GWM_LEFT_DOWN: {
      ...
      //
      // for 'check-like' buttons we have "dual state", we flip the selected
      // status in that case instead of just turning it on like normal ... also
      // note that selected messages are sent immediately
      //
    ...
    case GWM_RIGHT_DOWN: {
      ...
      if (BitTest(instData->getStatus(), WIN_STATUS_RIGHT_CLICK)) {
        ...
        //
        // for 'check-like' buttons we have "dual state", we flip the selected
        // status in that case instead of just turning it on like normal ...
        // also note that selected messages are sent immediately
        //
        if (BitTest(window->winGetStatus(), WIN_STATUS_CHECK_LIKE)) {
          if (BitTest(instData->m_state, WIN_STATE_SELECTED))
            BitClear(instData->m_state, WIN_STATE_SELECTED);
      ...
      switch (mData1) {
        ...
        case KEY_SPACE: {
          if (BitTest(mData2, KEY_STATE_UP)) {
            //
            // note check like selected messages aren't sent here ... they are
            // sent on the down press
            //
            if (BitTest(instData->getState(), WIN_STATE_SELECTED) &&
                BitTest(window->winGetStatus(), WIN_STATUS_CHECK_LIKE) ==
                    FALSE) {
              TheWindowManager->winSendSystemMsg(
                  instData->getOwner(), GBM_SELECTED, (WindowMsgData)window, 0);
            ...
            //
            // for 'check-like' buttons we have "dual state", we flip the
            // selected status in that case instead of just turning it on like
            // normal ... also note that selected messages are sent immediately
            //
            if (BitTest(window->winGetStatus(), WIN_STATUS_CHECK_LIKE)) {
              if (BitTest(instData->m_state, WIN_STATE_SELECTED))
                BitClear(instData->m_state, WIN_STATE_SELECTED);
...
WindowMsgHandledType GadgetPushButtonSystem(GameWindow *window, UnsignedInt msg,
  ...
  switch (msg) {
    ...
    case GWM_INPUT_FOCUS:
      ...
      TheWindowManager->winSendSystemMsg(instData->getOwner(), GGM_FOCUS_CHANGE,
                                         (WindowMsgData)mData1,
...
// ------------------------------------------------------------------------------------------------
/** Set the visual status of a button to make it looked checked/unchecked ... DO
 * NOT send any actual button selected messages, this is ONLY VISUAL */
// ------------------------------------------------------------------------------------------------
void GadgetCheckLikeButtonSetVisualCheck(GameWindow *g, Bool checked) {
  ...
  if (BitTest(g->winGetStatus(), WIN_STATUS_CHECK_LIKE) == FALSE) {
    DEBUG_CRASH(
        ("GadgetCheckLikeButtonSetVisualCheck: Window is not 'CHECK-LIKE'\n"));
    return;
