  ...
  switch (msg) {
    ...
    case GWM_CHAR: {
      switch (mData1) {
        ...
        // --------------------------------------------------------------------
        default:
          return TheWindowManager->winSendInputMsg(editBox, GWM_CHAR, mData1,
                                                   mData2);
    ...
    case GWM_LEFT_UP: {
      ...
      if (listBox) {
        ...
        if (listBox->winIsHidden()) {
          ...
          // those entries and not additional blank lines.  Also, just so it
          // looks pretty, hide the list box's sliders if we don't need to
          // scroll.
          ...
          newSize.y =
              ((TheWindowManager->winFontHeight(listInstData->getFont())) *
               multiplier) +
              multiplier * 2 + 4;
         ...
         ------------------------------------------------------------------------
                      case GWM_MOUSE_ENTERING:
                      {
         ...
         ) )
                              {
                                      ...
                                      BitSet( instData->m_state,
         WIN_STATE_HILITED ); TheWindowManager->winSendSystemMsg(
         window->winGetOwner(), GBM_MOUSE_ENTERING, (WindowMsgData)window, 0 );
...
WindowMsgHandledType GadgetComboBoxSystem(GameWindow *window, UnsignedInt msg,
  ...
  switch (msg) {
    ...
    case GCM_SET_TEXT: {
      if (comboData->editBox)
        GadgetTextEntrySetText(comboData->editBox,
                               *(const UnicodeString *)mData1);
      break;
    ...
    // ------------------------------------------------------------------------
    // if we get sent an edit done message from the text box, lets notify the
    // parent
    case GEM_EDIT_DONE: {
    ...
    // ------------------------------------------------------------------------
    // Set the selection in the listbox, this will trigger the listbox selected
    // message which will set the edit box.
    ...
    case GCM_GET_SELECTION: {
      if (comboData->listBox)
        ...
        DEBUG_ASSERTCRASH(0,
                          ("We don't have a listbox as part of the combo box"));
        *(Int *)mData2 = -1;
      }
      break;
    }  // case GCM_GET_SELECTION:
    ...
    case GCM_SET_ITEM_DATA: {
      if (comboData->listBox) {
        GadgetListBoxSetItemData(comboData->listBox, (void *)mData2,
                                 (Int)mData1);
      }
    ...
    case GCM_GET_ITEM_DATA: {
      if (comboData->listBox) {
        *(void **)mData2 =
            GadgetListBoxGetItemData(comboData->listBox, (Int)mData1, 0);
      }
      break;
    }
    // ------------------------------------------------------------------------
    ...
    case GLM_SELECTED: {
      if ((GameWindow *)mData1 == comboData->listBox) {
        ...
        Color color;
        tempUString =
            GadgetListBoxGetTextAndColor(comboData->listBox, &color, mData2, 0);
    ...
    // ------------------------------------------------------------------------
    case GCM_DEL_ALL: {
      if (comboData->listBox) GadgetListBoxReset(comboData->listBox);
      if (comboData->editBox)
        GadgetTextEntrySetText(comboData->editBox,
    ...
    case GCM_ADD_ENTRY: {
      ...
      if (listBox) {
        ...
        addedIndex = GadgetListBoxAddEntryText(
            listBox, *(UnicodeString *)mData1, mData2, -1, 0);
    ...
    // ------------------------------------------------------------------------
    case GWM_DESTROY: {
      TheWindowManager->winSetLoneWindow(
          NULL);  // if we are transitioning screens, close all combo boxes
      if (comboData) {
        delete (comboData);
        comboData = NULL;
      }
 ...
 * buttons */
//=============================================================================
void GadgetComboBoxSetColors(
    GameWindow *comboBox, Color enabledColor, Color enabledBorderColor,
  ...
  GadgetComboBoxSetEnabledSelectedItemBorderColor(
      comboBox, enabledSelectedItemBorderColor);
  // disabled
  GadgetComboBoxSetDisabledColor(comboBox, disabledColor);
  GadgetComboBoxSetDisabledBorderColor(comboBox, disabledBorderColor);
  GadgetComboBoxSetDisabledSelectedItemColor(comboBox,
                                             disabledSelectedItemColor);
  GadgetComboBoxSetDisabledSelectedItemBorderColor(
      comboBox, disabledSelectedItemBorderColor);
  // hilite
  ...
  GadgetComboBoxSetHiliteSelectedItemBorderColor(comboBox,
  ...
  if (editBox) {
    ...
    GadgetButtonSetEnabledSelectedBorderColor(editBox,
    ...
    GadgetButtonSetDisabledSelectedBorderColor(editBox,
    ...
    GadgetButtonSetHiliteSelectedBorderColor(editBox,
                                             hiliteSelectedItemBorderColor);
  }
  ...
  if (dropDownButton) {
    ...
    GadgetButtonSetEnabledSelectedColor(dropDownButton,
    ...
    GadgetButtonSetDisabledSelectedColor(dropDownButton,
                                         disabledSelectedItemColor);
  ...
  if (listBox) {
    GadgetListBoxSetColors(
        listBox, enabledColor, enabledBorderColor, enabledSelectedItemColor,
        enabledSelectedItemBorderColor, disabledColor, disabledBorderColor,
        disabledSelectedItemColor, disabledSelectedItemBorderColor, hiliteColor,
        hiliteBorderColor, hiliteSelectedItemColor,
        hiliteSelectedItemBorderColor);
  }
...
//=============================================================================
void GadgetComboBoxSetLettersAndNumbersOnly(GameWindow *comboBox,
...
void GadgetComboBoxSetAsciiOnly(GameWindow *comboBox, Bool isAsciiOnly) {
  ...
  if (comboData->entryData) comboData->entryData->aSCIIOnly = isAsciiOnly;
}
...
UnicodeString GadgetComboBoxGetText(GameWindow *comboBox) {
  // sanity
  if (comboBox == NULL) return UnicodeString::TheEmptyString;
...
//=============================================================================
Int GadgetComboBoxAddEntry(GameWindow *comboBox, UnicodeString text,
                           Color color) {
  // sanity
  if (comboBox == NULL) return -1;
  return (Int)TheWindowManager->winSendSystemMsg(comboBox, GCM_ADD_ENTRY,
                                                 (WindowMsgData)&text, color);
}
// GadgetComboBoxReset =======================================================
...
void GadgetComboBoxSetFont(GameWindow *comboBox, GameFont *font) {
  ...
  GameWindow *listBox = GadgetComboBoxGetListBox(comboBox);
  if (listBox) listBox->winSetFont(font);

  // set the Text Entry gadget's font
  GameWindow *editBox = GadgetComboBoxGetEditBox(comboBox);
  if (editBox) editBox->winSetFont(font);
...
//=============================================================================
void GadgetComboBoxSetEnabledTextColors(GameWindow *comboBox, Color color,
                                        Color borderColor) {
  // sanity
  if (comboBox == NULL) return;
...
//=============================================================================
void GadgetComboBoxSetDisabledTextColors(GameWindow *comboBox, Color color,
                                         Color borderColor) {
  ComboBoxData *comboBoxData = (ComboBoxData *)comboBox->winGetUserData();
  // sanity
  if (comboBox == NULL) return;
...
//=============================================================================
void GadgetComboBoxSetHiliteTextColors(GameWindow *comboBox, Color color,
                                       Color borderColor) {
  // sanity
  if (comboBox == NULL) return;
...
//=============================================================================
void GadgetComboBoxSetIMECompositeTextColors(GameWindow *comboBox, Color color,
                                             Color borderColor) {
  // sanity
  if (comboBox == NULL) return;
...
void GadgetComboBoxGetSelectedPos(GameWindow *comboBox, Int *selectedIndex) {
  ...
  TheWindowManager->winSendSystemMsg(comboBox, GCM_GET_SELECTION, 0,
                                     (WindowMsgData)selectedIndex);
}

// GadgetComboBoxSetSelectedPos ===============================================
/** Convenience wrapper function for setting the selected Position, if don't
   hide is set to true, the listbox won't be forced to hide when the Selected
   call is passed back */
//=============================================================================
void GadgetComboBoxSetSelectedPos(GameWindow *comboBox, Int selectedIndex,
                                  Bool dontHide) {
  // sanity
  if (comboBox == NULL) return;

  // get selected indeces via system message
  TheWindowManager->winSendSystemMsg(comboBox, GCM_SET_SELECTION, selectedIndex,
                                     dontHide);
}
// GadgetComboBoxSetItemData ==================================================
/** Convenience wrapper function for setting the Item data for the listbox under
 * the combo box */
//=============================================================================
void GadgetComboBoxSetItemData(GameWindow *comboBox, Int index, void *data) {
  if (comboBox)
    TheWindowManager->winSendSystemMsg(comboBox, GCM_SET_ITEM_DATA, index,
                                       (WindowMsgData)data);
}
// GadgetComboBoxGetItemData ==================================================
/** Convenience wrapper function for getting the Item data from the listbox
 * under the combo Box */
//=============================================================================
void *GadgetComboBoxGetItemData(GameWindow *comboBox, Int index) {
  ...
  if (comboBox) {
    TheWindowManager->winSendSystemMsg(comboBox, GCM_GET_ITEM_DATA, index,
                                       (WindowMsgData)&data);
  }
  return (data);
}
...
Int GadgetComboBoxGetLength(GameWindow *combobox) {
  ComboBoxData *comboboxData = (ComboBoxData *)combobox->winGetUserData();
  if (comboboxData) return comboboxData->entryCount;
