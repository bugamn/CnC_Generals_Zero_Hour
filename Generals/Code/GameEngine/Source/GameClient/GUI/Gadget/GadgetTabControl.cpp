...
//
// File name:
// \projects\RTS\code\gameengine\Source\GameClient\GUI\Gadget\GadgetTabControl.cpp
//
...
//=============================================================================
WindowMsgHandledType GadgetTabControlInput(GameWindow *tabControl,
                                           UnsignedInt msg,
  ...
  Int mouseX = LOLONGTOSHORT(mData1) -
               tabX;  // mData1 is packedMouseCoords in screen space
  ...
  switch (msg) {
    case GWM_LEFT_DOWN: {
      if ((mouseX < tabsLeft) || (mouseX > tabsRight) || (mouseY < tabsTop) ||
          (mouseY >
           tabsBottom)) {  // I eat input on myself that isn't a tab (a button
                           // click would mean I don't see the input ever.)
        return MSG_HANDLED;
      }

      Int distanceIn;
      Int tabSize;
      if ((tabData->tabEdge == TP_RIGHT_SIDE) ||
          (tabData->tabEdge ==
           TP_LEFT_SIDE)) {  // scan down to find which button
        distanceIn = mouseY - tabsTop;
        tabSize = tabData->tabHeight;
      } else {  // scan right to find which button
        distanceIn = mouseX - tabsLeft;
        tabSize = tabData->tabWidth;
      }
      Int tabPressed = distanceIn / tabSize;
      if (!tabData->subPaneDisabled[tabPressed] &&
...
//=============================================================================
WindowMsgHandledType GadgetTabControlSystem(GameWindow *tabControl,
                                            UnsignedInt msg,
                                            WindowMsgData mData1,
                                            WindowMsgData mData2) {
  switch (msg) {
    ...
    case GGM_RESIZED: {  // On resize, we need to upkeep the pane sizes and tabs
                         // since they are bound to us
    ...
    case GBM_SELECTED: {  // Pass buttons messages up
...
void GadgetTabControlComputeTabRegion(
    GameWindow *tabControl)  ///< Recalc the tab positions based on userData
{
  ...
  if ((tabData->tabEdge == TP_TOP_SIDE) ||
    ...
    if (tabData->tabOrientation == TP_CENTER) {
      horzOffset = winWidth - (2 * tabData->paneBorder) -
                   (tabData->tabCount * tabData->tabWidth);
      horzOffset /= 2;
    ...
    if (tabData->tabOrientation == TP_CENTER) {
      vertOffset = winHeight - (2 * tabData->paneBorder) -
                   (tabData->tabCount * tabData->tabHeight);
      vertOffset /= 2;
    } else if (tabData->tabOrientation == TP_BOTTOMRIGHT) {
      vertOffset = winHeight - (2 * tabData->paneBorder) -
                   (tabData->tabCount * tabData->tabHeight);
  ...
  if (tabData->tabEdge == TP_TOP_SIDE) {
    ...
    tabData->tabsRightLimit = tabData->paneBorder + horzOffset +
                              (tabData->tabWidth * tabData->tabCount);
  } else if (tabData->tabEdge == TP_BOTTOM_SIDE) {
    tabData->tabsTopLimit =
        winHeight - tabData->paneBorder - tabData->tabHeight;
    ...
    tabData->tabsBottomLimit = tabData->paneBorder + vertOffset +
                               (tabData->tabHeight * tabData->tabCount);
...
void GadgetTabControlComputeSubPaneSize(GameWindow *tabControl, Int *width,
...
void GadgetTabControlCreateSubPanes(
    GameWindow
        *tabControl)  ///< Create User Windows attached to userData as Panes
{  // These two funcs are called after all the Editor set data is updated
  ...
  for (Int paneIndex = 0; paneIndex < NUM_TAB_PANES; paneIndex++) {
    ...
    {
      tabData->subPanes[paneIndex] = TheWindowManager->winCreate(
          tabControl, WIN_STATUS_NONE, x, y, width, height,
          PassSelectedButtonsToParentSystem, NULL);
      ...
      tabData->subPanes[paneIndex]->winEnable(
          BitTest(tabControl->winGetStatus(), WIN_STATUS_ENABLED));
    } else  // this one exists, tabCount will control keeping extra panes
            // perma-hidden
    {
...
void GadgetTabControlFixupSubPaneList(GameWindow *tabControl) {
  ...
  if (child) {  // need to write down children, and they are reversed from our
                // array
