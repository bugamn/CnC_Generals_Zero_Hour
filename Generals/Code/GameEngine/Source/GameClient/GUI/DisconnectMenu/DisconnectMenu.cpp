...
char *DisconnectMenu::m_packetRouterTimeoutControlName =
    "DisconnectScreen.wnd:StaticPacketRouterTimeout";
char *DisconnectMenu::m_packetRouterTimeoutLabelControlName =
    "DisconnectScreen.wnd:StaticPacketRouterTimeoutLabel";
char *DisconnectMenu::m_textDisplayControlName =
    "DisconnectScreen.wnd:ListboxTextDisplay";

static const Color chatNormalColor = GameMakeColor(255, 0, 0, 255);

DisconnectMenu *TheDisconnectMenu = NULL;

DisconnectMenu::DisconnectMenu() { m_disconnectManager = NULL; }
...
void DisconnectMenu::setPlayerName(Int playerNum, UnicodeString name) {
  NameKeyType id =
      TheNameKeyGenerator->nameToKey(m_playerNameTextControlNames[playerNum]);
  ...
  id = TheNameKeyGenerator->nameToKey(
      m_playerTimeoutTextControlNames[playerNum]);
...
void DisconnectMenu::showPlayerControls(Int slot) {
  NameKeyType id =
      TheNameKeyGenerator->nameToKey(m_playerNameTextControlNames[slot]);
  GameWindow *control = TheWindowManager->winGetWindowFromId(NULL, id);
  if (control != NULL) {
...
void DisconnectMenu::showPacketRouterTimeout() {
  NameKeyType id =
      TheNameKeyGenerator->nameToKey(m_packetRouterTimeoutLabelControlName);
  GameWindow *control = TheWindowManager->winGetWindowFromId(NULL, id);
  ...
  if (control != NULL) {
    GadgetStaticTextSetText(
        control, UnicodeString(L""));  // start it off with a blank string.
    control->winHide(FALSE);
  }
}
...
void DisconnectMenu::setPacketRouterTimeoutTime(time_t newTime) {
  NameKeyType id =
      TheNameKeyGenerator->nameToKey(m_packetRouterTimeoutControlName);
  GameWindow *control = TheWindowManager->winGetWindowFromId(NULL, id);
...
void DisconnectMenu::showChat(UnicodeString text) {
  NameKeyType displayID =
      TheNameKeyGenerator->nameToKey(m_textDisplayControlName);
  GameWindow *displayControl =
      TheWindowManager->winGetWindowFromId(NULL, displayID);
...
void DisconnectMenu::quitGame() { TheNetwork->quitGame(); }
...
void DisconnectMenu::updateVotes(Int slot, Int votes) {
  NameKeyType id =
      TheNameKeyGenerator->nameToKey(m_playerVoteCountControlNames[slot]);
  GameWindow *control = TheWindowManager->winGetWindowFromId(NULL, id);
