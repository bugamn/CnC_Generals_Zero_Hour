...
GameWindow::GameWindow(void) {
  ...
  // We use to set the default tooltip func to
  // TheWindowManager->getDefaultTooltip() but I removed this so that we can set
  // in GUI edit a text string that will be the default tool tip for a control.
...
GameWindow::~GameWindow(void) {
  ...
  if (m_editData) delete m_editData;
  m_editData = NULL;
...
GameWindow *GameWindow::findFirstLeaf(void) {
  ...
  while (leaf->m_parent) leaf = leaf->m_parent;

  // Find the first leaf
  while (leaf->m_child) leaf = leaf->m_child;
...
GameWindow *GameWindow::findLastLeaf(void) {
  ...
  while (leaf->m_child) {
    ...
    while (leaf->m_next) leaf = leaf->m_next;
...
GameWindow *GameWindow::findNextLeaf(void) {
  ...
  if (leaf->m_next) {
    if (leaf->m_next->m_status & WIN_STATUS_TAB_STOP) return leaf->m_next;

    for (leaf = leaf->m_next; leaf; leaf = leaf->m_child)
      if (leaf->m_child == NULL || BitTest(leaf->m_status, WIN_STATUS_TAB_STOP))
        return leaf;
...
Int GameWindow::winBringToTop(void) {
  ...
  if (parent) {
    ...
    for (current = TheWindowManager->winGetWindowList(); current != this;
         current = current->m_next)
      if (current == NULL) return WIN_ERR_INVALID_PARAMETER;
...
Int GameWindow::winActivate(void) {
  ...
  if (returnCode != WIN_ERR_OK) return returnCode;
...
Int GameWindow::winGetPosition(Int *x, Int *y) {
  // sanity
  if (x == NULL || y == NULL) return WIN_ERR_INVALID_PARAMETER;
...
Bool GameWindow::winPointInWindow(Int x, Int y) {
  ...
  if (x >= winX && x <= winX + width && y >= winY && y <= winY + height)
    return TRUE;
...
Int GameWindow::winSetSize(Int width, Int height) {
  ...
  TheWindowManager->winSendSystemMsg(this, GGM_RESIZED, (WindowMsgData)width,
...
Int GameWindow::winGetSize(Int *width, Int *height) {
  // sanity
  if (width == NULL || height == NULL) return WIN_ERR_INVALID_PARAMETER;
...
Int GameWindow::winHide(Bool hide) {
  if (hide) {
    ...
    //
    if (!BitTest(m_status, WIN_STATUS_NO_FLUSH)) freeImages();
...
Int GameWindow::winSetInstanceData(WinInstanceData *data) {
  ...
  if (data->getTextLength()) m_instData.setText(data->getText());
...
Bool GameWindow::winIsChild(GameWindow *child) {
  while (child) {
    if (this == child->m_parent) return TRUE;
...
//=============================================================================
GameWindow *GameWindow::winGetNext(void) { return m_next; }  // end winGetNext
...
//=============================================================================
GameWindow *GameWindow::winGetPrev(void) { return m_prev; }  // end winGetPrev

// GameWindow::winSetNextInLayout =============================================
/** Set next window in layout */
//=============================================================================
void GameWindow::winSetNextInLayout(GameWindow *next) { m_nextLayout = next; }

// GameWindow::winSetPrevInLayout =============================================
/** Set previous window in layout pointer */
//=============================================================================
void GameWindow::winSetPrevInLayout(GameWindow *prev) { m_prevLayout = prev; }

// GameWindow::winSetLayout ===================================================
/** Set this window as belonging to layout 'layout' */
//=============================================================================
void GameWindow::winSetLayout(WindowLayout *layout) { m_layout = layout; }

// GameWindow::winGetLayout ===================================================
/** Get layout this window is a part of, if any */
//=============================================================================
WindowLayout *GameWindow::winGetLayout(void) { return m_layout; }

// GameWindow::winGetNextInLayout =============================================
/** Get next window in layout list if any */
//=============================================================================
GameWindow *GameWindow::winGetNextInLayout(void) { return m_nextLayout; }

// GameWindow::winGetPrevInLayout =============================================
/** Get prev window in layout list if any */
//=============================================================================
GameWindow *GameWindow::winGetPrevInLayout(void) { return m_prevLayout; }
...
//=============================================================================
Int GameWindow::winSetCallbacks(GameWinInputFunc input, GameWinDrawFunc draw,
...
//=============================================================================
GameWindow *GameWindow::winPointInChild(Int x, Int y, Bool ignoreEnableCheck,
                                        Bool playDisabledSound) {
  GameWindow *parent;
  GameWindow *child;
  ICoord2D origin;
  ...
  for (child = m_child; child; child = child->m_next) {
    ...
    if (x >= origin.x && x <= origin.x + child->m_size.x && y >= origin.y &&
        y <= origin.y + child->m_size.y) {
      Bool enabled =
          ignoreEnableCheck || BitTest(child->m_status, WIN_STATUS_ENABLED);
...
//=============================================================================
GameWindow *GameWindow::winPointInAnyChild(Int x, Int y, Bool ignoreHidden,
  ...
  for (child = m_child; child; child = child->m_next) {
    ...
    if (x >= origin.x && x <= origin.x + child->m_size.x && y >= origin.y &&
      ...
      if (!(ignoreHidden == TRUE &&
            BitTest(child->m_status, WIN_STATUS_HIDDEN)))
        return child->winPointInChild(x, y, ignoreEnableCheck);
...
///< Input that blocks all (mouse) input like a wall, instead of passing like it
///< wasn't there
WindowMsgHandledType GameWinBlockInput(GameWindow *window, UnsignedInt msg,
                                       WindowMsgData mData1,
                                       WindowMsgData mData2) {
  if (msg == GWM_CHAR || msg == GWM_MOUSE_POS) return MSG_IGNORED;
...
//=============================================================================
void GameWinDefaultTooltip(GameWindow *window, WinInstanceData *instData,
                           UnsignedInt mouse) {
  return;
