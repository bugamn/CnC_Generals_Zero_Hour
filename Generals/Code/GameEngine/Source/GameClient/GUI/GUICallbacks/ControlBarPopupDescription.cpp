...
void ControlBarPopupDescriptionUpdateFunc(WindowLayout *layout, void *param) {
  if (TheScriptEngine->isGameEnding()) TheControlBar->hideBuildTooltipLayout();

  if (theAnimateWindowManager && !TheControlBar->getShowBuildTooltipLayout() &&
      !theAnimateWindowManager->isReversed())
    theAnimateWindowManager->reverseAnimateWindow();
  else if (!TheControlBar->getShowBuildTooltipLayout() &&
           (!TheGlobalData->m_animateWindows || !useAnimation))
    TheControlBar->deleteBuildTooltipLayout();

  if (useAnimation && theAnimateWindowManager &&
      TheGlobalData->m_animateWindows) {
    ...
    if (theAnimateWindowManager && theAnimateWindowManager->isFinished() &&
        !wasFinished && theAnimateWindowManager->isReversed()) {
...
void ControlBar::showBuildTooltipLayout(GameWindow *cmdButton) {
  ...
  if (BitTest(cmdButton->winGetStyle(), GWS_PUSH_BUTTON)) {
    const CommandButton *commandButton =
        (const CommandButton *)GadgetButtonGetData(cmdButton);
...
// to work as a solo game!
    ...
    if (TheGameLogic->isInReplayGame()) return;
#endif

    if (TheInGameUI->isQuitMenuVisible()) return;

    if (TheDisconnectMenu && TheDisconnectMenu->isScreenVisible()) return;
    ...
    m_showBuildToolTipLayout = TRUE;
    //	m_buildToolTipLayout = TheWindowManager->winCreateLayout(
    ...
    if (!BitTest(cmdButton->winGetStyle(), GWS_USER_WINDOW) &&
        !BitTest(cmdButton->winGetStyle(), GWS_STATIC_TEXT))
      return;
  ...
  if (useAnimation && TheGlobalData->m_animateWindows) {
    ...
    theAnimateWindowManager->registerGameWindow(
        m_buildToolTipLayout->getFirstWindow(), WIN_ANIMATION_SLIDE_RIGHT_FAST,
        TRUE, 200);
  }
}
...
void ControlBar::repopulateBuildTooltipLayout(void) {
  ...
  if (!BitTest(prevWindow->winGetStyle(), GWS_PUSH_BUTTON)) return;
  const CommandButton *commandButton =
      (const CommandButton *)GadgetButtonGetData(prevWindow);
  populateBuildTooltipLayout(commandButton);
}

void ControlBar::populateBuildTooltipLayout(const CommandButton *commandButton,
                                            GameWindow *tooltipWin) {
  if (!m_buildToolTipLayout) return;

  Player *player = ThePlayerList->getLocalPlayer();
  UnicodeString name, cost, descrip;
  UnicodeString
    requires
  = UnicodeString::TheEmptyString, requiresList;
  ...
  if (commandButton) {
    ...
    const UpgradeTemplate *upgradeTemplate =
        commandButton->getUpgradeTemplate();
      ...
      for (Int j = 0; j < commandButton->getScienceVec().size(); ++j) {
        ...
        if (commandButton->getCommandType() != GUI_COMMAND_PURCHASE_SCIENCE) {
          if (!player->hasScience(st) && j > 0) {
            // If we're not looking at a command button that purchases a
            // science, then it means we are looking at a command button that
            // can USE the science. This means we want to get the description
            // for the previous science -- the one we can use, not purchase!
            st = commandButton->getScienceVec()[j - 1];
          }

          // Now that we got the science for the button that executes the
          // science, we need to generate a simpler help text!
          fireScienceButton = true;

          break;
        } else if (!player->hasScience(st)) {
          // Purchase science case. The first science we run into that we don't
          // have, that's the one we'll want to show!
          break;
        }
      }
    ...
    if (commandButton->getDescriptionLabel().isNotEmpty()) {
      ...
      if (selectedObject) {
        ...
        if (commandButton->getCommandType() == GUI_COMMAND_TOGGLE_OVERCHARGE) {
          {
            ...
            for (BehaviorModule **bmi = selectedObject->getBehaviorModules();
                 *bmi; ++bmi) {
              obi = (*bmi)->getOverchargeBehaviorInterface();
              if (obi) {
                descrip.concat(L"\n");
                if (obi->isOverchargeActive())
                  descrip.concat(TheGameText->fetch(
        ...
        // for not being able to buy stuff.
        else if (thingTemplate) {
          CanMakeType makeType = TheBuildAssistant->canMakeUnit(
              selectedObject, commandButton->getThingTemplate());
          switch (makeType) {
            case CANMAKE_NO_MONEY:
              descrip.concat(L"\n\n");
              descrip.concat(
                  TheGameText->fetch("TOOLTIP:TooltipNotEnoughMoneyToBuild"));
              break;
            ...
            case CANMAKE_MAXED_OUT_FOR_PLAYER:
              ...
              //	descrip.concat( L"\n\n" );
              //	descrip.concat( TheGameText->fetch(
              //"TOOLTIP:TooltipCannotBuildDueToPrerequisites" ) ); 	break;
          }
        }

        // Special case: When building upgrades
        else if (upgradeTemplate &&
                 !player->hasUpgradeInProduction(upgradeTemplate)) {
          if (commandButton->getCommandType() == GUI_COMMAND_PLAYER_UPGRADE ||
            ...
            ProductionUpdateInterface *pui =
                selectedObject->getProductionUpdateInterface();
            if (pui && pui->getProductionCount() == MAX_BUILD_QUEUE_BUTTONS) {
              descrip.concat(L"\n\n");
              descrip.concat(TheGameText->fetch(
                  "TOOLTIP:TooltipCannotPurchaseBecauseQueueFull"));
            } else if (!TheUpgradeCenter->canAffordUpgrade(
                           ThePlayerList->getLocalPlayer(), upgradeTemplate,
                           FALSE)) {
              descrip.concat(L"\n\n");
              descrip.concat(
                  TheGameText->fetch("TOOLTIP:TooltipNotEnoughMoneyToBuild"));
    ...
    if (thingTemplate &&
      ...
      // We are either looking at building a unit or a structure that may or may
      // not have any prerequisites.

      // Format the cost only when we have to pay for it.
      cost.format(TheGameText->fetch("TOOLTIP:Cost"),
                  thingTemplate->calcCostToBuild(player));

      // ask each prerequisite to give us a list of the non satisfied
      // prerequisites
      ...
      if (!requires.isEmpty()) {
        UnicodeString requireFormat =
            TheGameText->fetch("CONTROLBAR:Requirements");
        requires.format(requireFormat.str(), requires.str());
        if (!descrip.isEmpty()) descrip.concat(L"\n");
        descrip.concat(requires);
      }
    } else if (upgradeTemplate) {
      // We are looking at an upgrade purchase icon. Maybe we already purchased
      // it?

      Bool hasUpgradeAlready = false;
      Bool hasConflictingUpgrade = false;
      Bool playerUpgradeButton =
          commandButton->getCommandType() == GUI_COMMAND_PLAYER_UPGRADE;
      Bool objectUpgradeButton =
          commandButton->getCommandType() == GUI_COMMAND_OBJECT_UPGRADE;
        ...
        if (draw) {
          ...
          if (object) {
            ...
            if (objectUpgradeButton) {
              hasConflictingUpgrade =
                  !object->affectedByUpgrade(upgradeTemplate);
            }
          }
        }
      }
      if (hasConflictingUpgrade && !hasUpgradeAlready) {
        if (commandButton->getConflictingLabel().isNotEmpty()) {
          ...
          descrip = TheGameText->fetch("TOOLTIP:HasConflictingUpgradeDefault");
                 ...
                 (playerUpgradeButton || objectUpgradeButton)) {
        // See if we can fetch the "already upgraded" text for this upgrade. If
        // not.... use the default "fill me in".
    ...
    if (tooltipWin ==
    ...
    } else if (tooltipWin == TheWindowManager->winGetWindowFromId(
                                 m_buildToolTipLayout->getFirstWindow(),
      ...
      if (playerToDisplay && playerToDisplay->getEnergy()) {
        ...
        descrip.format(descrip, energy->getProduction(),
      ...
      DEBUG_ASSERTCRASH(
          FALSE, ("ControlBar::populateBuildTooltipLayout We attempted to call "
                  "the popup tooltip on a game window that has yet to be hand "
                  "coded in as this fuction was/is designed for only buttons "
                  "but has been hacked to work with GameWindows."));
      return;
    }
  }
  ...
  if (win) {
    ...
    //		{
    //
    //			parent->winSetPosition(pos.x, pos.y -  (102 - (newSize.y
    //+ size.y + diffSize) ));
    //		}
    //		else
    ...
    parent->winSetPosition(pos.x,
                           (pos.y - diffSize) + (offset.y - lastOffset.y));
...
void ControlBar::hideBuildTooltipLayout() {
  if (theAnimateWindowManager && theAnimateWindowManager->isReversed()) return;
  if (useAnimation && theAnimateWindowManager &&
      TheGlobalData->m_animateWindows)
    theAnimateWindowManager->reverseAnimateWindow();
  else
...
void ControlBar::deleteBuildTooltipLayout(void) {
  ...
  //	m_buildToolTipLayout->deleteInstance();
  //	m_buildToolTipLayout = NULL;
  if (theAnimateWindowManager) delete theAnimateWindowManager;
  theAnimateWindowManager = NULL;
}
