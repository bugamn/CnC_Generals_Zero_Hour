...
static NameKeyType comboBoxPlayerID[MAX_SLOTS] = {
    NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID,
    NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID};

static NameKeyType comboBoxColorID[MAX_SLOTS] = {
    NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID,
    NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID};

static NameKeyType comboBoxPlayerTemplateID[MAX_SLOTS] = {
    NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID,
    NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID};

static NameKeyType comboBoxTeamID[MAX_SLOTS] = {
    NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID,
    NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID};

// static NameKeyType buttonStartPositionID[MAX_SLOTS] = {
// NAMEKEY_INVALID,NAMEKEY_INVALID, 																										NAMEKEY_INVALID,NAMEKEY_INVALID,
//																										NAMEKEY_INVALID,NAMEKEY_INVALID,
//																										NAMEKEY_INVALID,NAMEKEY_INVALID
//};

static NameKeyType buttonMapStartPositionID[MAX_SLOTS] = {
    NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID,
    NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID};
...
SkirmishPreferences::SkirmishPreferences(void) {
  // note, the superclass will put this in the right dir automatically, this is
  // just a leaf name
  load("Skirmish.ini");
}
...
Int SkirmishPreferences::getPreferredColor(void) {
  ...
  if (ret < -1 || ret >= TheMultiplayerSettings->getNumColors()) ret = -1;
...
Int SkirmishPreferences::getPreferredFaction(void) {
  ...
  ret = atoi(it->second.str());
  if (ret < PLAYERTEMPLATE_MIN ||
      ret >= ThePlayerTemplateStore->getPlayerTemplateCount())
    ret = PLAYERTEMPLATE_RANDOM;

  if (ret >= 0) {
    const PlayerTemplate *fac =
        ThePlayerTemplateStore->getNthPlayerTemplate(ret);
    if (!fac)
      ret = PLAYERTEMPLATE_RANDOM;
...
Bool SkirmishPreferences::write(void) {
  ...
  (*this)["UserName"] = UnicodeStringToQuotedPrintable(
      TheSkirmishGameInfo->getConstSlot(0)->getName());

  setSlotList();

  //	NameKeyType sliderGameSpeedID = TheNameKeyGenerator->nameToKey(
  //AsciiString( "SkirmishGameOptionsMenu.wnd:SliderGameSpeed" ) );
  GameWindow *sliderGameSpeed = TheWindowManager->winGetWindowFromId(
      parentSkirmishGameOptions, sliderGameSpeedID);
                                                                                                        ...
                                                                                                        UnsignedInt mouse)
{
        ...
        {
                if (window && window ==
GadgetComboBoxGetEditBox(comboBoxPlayer[i]))
                {
                        idx = i;
                        break;
                }
        ...
        /// @todo get this to display the correct tooltip for the player field
CCB tooltip.format(TheGameText->fetch("TOOLTIP:LANPlayer") );
        TheMouse->setCursorTooltip( tooltip );
}
*/

void setFPSTextBox(Int sliderPos) {
  if (!staticTextGameSpeed) return;
  UnicodeString text;
...
void reallyDoStart(void) {
  ...
  if (maxFPS > GREATER_NO_FPS_LIMIT) maxFPS = 1000;
  ...
  if (md) {
    isSkirmish = md->m_isMultiplayer;  // we can now select solo campaign maps
                                       // in Skirmish.
  }
  ...
  if (isSkirmish) {
    ...
    msg->appendIntegerArgument(
        DIFFICULTY_NORMAL);  // not really used; just specified so we can add
                             // the game speed last
    msg->appendIntegerArgument(0);  // not really used; just specified so we can
                                    // add the game speed last
...
void CheckForCDAtGameStart(gameStartCallback callback) {
  if (!IsFirstCDPresent()) {
    ...
    ExMessageBoxOkCancel(TheGameText->fetch("GUI:InsertCDPrompt"),
                         TheGameText->fetch("GUI:InsertCDMessage"), callback,
                         checkCDCallback, cancelStartBecauseOfNoCD);
  } else {
    callback();
  }
}
...
static void startPressed(void) {
  ...
  if (it == TheMapCache->end()) {
    ...
    MessageBoxOk(TheGameText->fetch("GUI:ErrorStartingGame"),
                 TheGameText->fetch("GUI:CantFindMap"), NULL);
  }
...
else if (playerCount < 2 && !sandboxOk)
{
          sandboxOk = TRUE;
          MessageBoxOk(TheGameText->fetch("GUI:ErrorStartingGame"),
TheGameText->fetch("GUI:SandboxWarning"), NULL);
}
  */
  else {
    isReady = TRUE;
...
/////////////////////////////////////////////////////
void MapSelectorTooltip(GameWindow *window, WinInstanceData *instData,
                        UnsignedInt mouse) {
  Int x, y;
  ...
  ICoord2DList::iterator it =
      TheSupplyAndTechImageLocations.m_techPosList.begin();
  ICoord2DList::iterator it2 =
      TheSupplyAndTechImageLocations.m_supplyPosList.begin();
  ...
  if (image) {
    ...
    while (it != TheSupplyAndTechImageLocations.m_techPosList.end()) {
      if ((x > (pixelX + it->x) && x < (pixelX + it->x + SUPPLY_TECH_SIZE)) &&
        ...
        TheMouse->setCursorTooltip(TheGameText->fetch("TOOLTIP:TechBuilding"),
                                   -1, NULL);  //, 1.5f
        return;
      }
      it++;
  ...
  if (image2) {
    ...
    while (it2 != TheSupplyAndTechImageLocations.m_supplyPosList.end()) {
      if ((x > (pixelX + it2->x) && x < (pixelX + it2->x + SUPPLY_TECH_SIZE)) &&
        ...
        TheMouse->setCursorTooltip(TheGameText->fetch("TOOLTIP:SupplyDock"), -1,
                                   NULL);  // , 1.5f
        break;
      }
      it2++;
...
void positionStartSpotControls(GameWindow *win, GameWindow *mapWindow,
                               Coord3D *pos, MapMetaData *mmd,
                               GameWindow *buttonMapStartPositions[]) {
  if (!win || !mmd || !mapWindow || !buttonMapStartPositions) return;
  DEBUG_ASSERTCRASH(win && mmd, ("positionStartSpotControls:: we don't have a "
                                 "window to position or any mapmetadata"));
  ...
  // When we actually draw the map, save off it's screen position and use that
  // instead of the map window's position/size
  Real position;
  position =
      (pos->x - mmd->m_extent.lo.x) / (mmd->m_extent.hi.x - mmd->m_extent.lo.x);
  gadgetPos.x =
      (position * smallWidth) - gadgetSize.x / 2 + ul.x;  // + winMapPos.x;

  position =
      (pos->y - mmd->m_extent.lo.y) / (mmd->m_extent.hi.y - mmd->m_extent.lo.y);
  gadgetPos.y = ((1 - position) * smallHeight) - gadgetSize.y / 2 +
                ul.y;  // + winMapPos.y;

  // loop through and make sure we're not on top of anyone else
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if (buttonMapStartPositions[i] == win) break;
    ICoord2D tempPos;
...
void positionAdditionalImages(MapMetaData *mmd, GameWindow *mapWindow,
  ...
  while (it != mmd->m_supplyPositions.end()) {
    ...
    // instead of the map window's position/size
    Real position;
    position = (it->x - mmd->m_extent.lo.x) /
               (mmd->m_extent.hi.x - mmd->m_extent.lo.x);
    markerPos.x = (position * smallWidth) - SUPPLY_TECH_SIZE / 2 +
                  ul.x;  // + winMapPos.x;

    position = (it->y - mmd->m_extent.lo.y) /
               (mmd->m_extent.hi.y - mmd->m_extent.lo.y);
    markerPos.y = ((1 - position) * smallHeight) - SUPPLY_TECH_SIZE / 2 +
                  ul.y;  // + winMapPos.y;
...
void positionStartSpots(AsciiString mapName,
                        GameWindow *buttonMapStartPositions[],
                        GameWindow *mapWindow) {
  ...
  if (it == TheMapCache->end()) {
    ...
    if (mapWindow != NULL) {
      ...
      if (image) {
        ...
        if (!unknownImage)
          unknownImage =
              TheMappedImageCollection->findImageByName("UnknownMap");
        if (unknownImage) {
          mapWindow->winSetStatus(WIN_STATUS_IMAGE);
          mapWindow->winSetEnabledImage(0, unknownImage);
        } else {
    ...
    for (Int i = 0; i < mmd.m_numPlayers && mmd.m_isMultiplayer; ++i) {
      waypointName.format("Player_%d_Start",
                          i + 1);  // start pos waypoints are 1-based
      ...
      if (wmIt != mmd.m_waypoints.end()) {
        ...
        DEBUG_ASSERTCRASH(FALSE,
                          ("positionStartSpots:: someone messed with the map "
                           "cash.  We couldn't find waypoint <%s> in map <%s>",
                           waypointName.str(), lowerMap.str()));
      }
    }
    // hide the rest
    for (; i < MAX_SLOTS; ++i) {
...
void updateMapStartSpots(GameInfo *myGame,
                         GameWindow *buttonMapStartPositions[],
                         Bool onLoadScreen) {
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    GadgetButtonSetText(buttonMapStartPositions[i],
                        UnicodeString::TheEmptyString);
    if (!onLoadScreen) {
      buttonMapStartPositions[i]->winSetTooltip(
          TheGameText->fetch("TOOLTIP:StartPosition"));
    }
  }
  for (i = 0; i < MAX_SLOTS; ++i) {
    GameSlot *gs = myGame->getSlot(i);
    if (onLoadScreen) {
      if (gs->getApparentStartPos() >= 0 &&
          gs->getApparentStartPos() < mmd.m_numPlayers &&
          gs->getPlayerTemplate() > PLAYERTEMPLATE_MIN) {
        AsciiString displayNumber;
        displayNumber.format("NUMBER:%d", i + 1);
        GadgetButtonSetText(buttonMapStartPositions[gs->getApparentStartPos()],
                            TheGameText->fetch(displayNumber));
      }
    } else {
      if (gs->getStartPos() >= 0 && gs->getStartPos() < mmd.m_numPlayers &&
          gs->getPlayerTemplate() > PLAYERTEMPLATE_MIN) {
        AsciiString displayNumber;
        displayNumber.format("NUMBER:%d", i + 1);
        GadgetButtonSetText(buttonMapStartPositions[gs->getStartPos()],
                            TheGameText->fetch(displayNumber));
        // Added By Sadullah Nader
    ...
    if (color >= -1 && color < TheMultiplayerSettings->getNumColors()) {
      ...
      if (color != -1) {
        for (Int i = 0; i < MAX_SLOTS; i++) {
          ...
          if (checkSlot && color == checkSlot->getColor() &&
              slot != checkSlot) {
...
static void handlePlayerTemplateSelection(int index) {
  ...
  if (myGame) {
    GameSlot *slot = myGame->getSlot(index);
    if (!slot) return;
    if (playerTemplate == slot->getPlayerTemplate()) return;
...
static void handleStartPositionSelection(int index, Int position) {
  ...
  if (myGame) {
    GameSlot *slot = myGame->getSlot(index);
    if (position == slot->getStartPos()) return;
    if (position < 0) {
...
static void handleTeamSelection(int index) {
  ...
  if (myGame) {
    GameSlot *slot = myGame->getSlot(index);
    if (!slot) return;
    if (team == slot->getTeamNumber()) return;
...
void InitSkirmishGameGadgets(void) {
  ...
  parentSkirmishGameOptionsID = TheNameKeyGenerator->nameToKey(
      AsciiString("SkirmishGameOptionsMenu.wnd:SkirmishGameOptionsMenuParent"));
  buttonExitID = TheNameKeyGenerator->nameToKey(
      AsciiString("SkirmishGameOptionsMenu.wnd:ButtonBack"));
  buttonStartID = TheNameKeyGenerator->nameToKey(
      AsciiString("SkirmishGameOptionsMenu.wnd:ButtonStart"));
  textEntryMapDisplayID = TheNameKeyGenerator->nameToKey(
      AsciiString("SkirmishGameOptionsMenu.wnd:TextEntryMapDisplay"));
  buttonSelectMapID = TheNameKeyGenerator->nameToKey(
      AsciiString("SkirmishGameOptionsMenu.wnd:ButtonSelectMap"));
  buttonResetID = TheNameKeyGenerator->nameToKey(
      AsciiString("SkirmishGameOptionsMenu.wnd:ButtonReset"));
  windowMapID = TheNameKeyGenerator->nameToKey(
      AsciiString("SkirmishGameOptionsMenu.wnd:MapWindow"));
  staticTextGameSpeedID = TheNameKeyGenerator->nameToKey(
      AsciiString("SkirmishGameOptionsMenu.wnd:StaticTextGameSpeed"));
  // Initialize the pointers to our gadgets
  parentSkirmishGameOptions =
      TheWindowManager->winGetWindowFromId(NULL, parentSkirmishGameOptionsID);
  DEBUG_ASSERTCRASH(parentSkirmishGameOptions,
                    ("Could not find the parentSkirmishGameOptions"));
  buttonSelectMap = TheWindowManager->winGetWindowFromId(
      parentSkirmishGameOptions, buttonSelectMapID);
  DEBUG_ASSERTCRASH(buttonSelectMap, ("Could not find the buttonSelectMap"));
  buttonStart = TheWindowManager->winGetWindowFromId(parentSkirmishGameOptions,
                                                     buttonStartID);
  DEBUG_ASSERTCRASH(buttonStart, ("Could not find the buttonStart"));
  buttonExit = TheWindowManager->winGetWindowFromId(parentSkirmishGameOptions,
                                                    buttonExitID);
  DEBUG_ASSERTCRASH(buttonExit, ("Could not find the buttonExit"));
  textEntryMapDisplay = TheWindowManager->winGetWindowFromId(
      parentSkirmishGameOptions, textEntryMapDisplayID);
  DEBUG_ASSERTCRASH(textEntryMapDisplay,
                    ("Could not find the textEntryMapDisplay"));
  buttonReset = TheWindowManager->winGetWindowFromId(parentSkirmishGameOptions,
                                                     buttonResetID);
  DEBUG_ASSERTCRASH(buttonReset, ("Could not find the buttonReset"));
  staticTextGameSpeed = TheWindowManager->winGetWindowFromId(
      parentSkirmishGameOptions, staticTextGameSpeedID);
  DEBUG_ASSERTCRASH(staticTextGameSpeed,
                    ("Could not find the staticTextGameSpeed"));

  textEntryPlayerNameID = TheNameKeyGenerator->nameToKey(
      AsciiString("SkirmishGameOptionsMenu.wnd:TextEntryPlayerName"));
  textEntryPlayerName =
      TheWindowManager->winGetWindowFromId(NULL, textEntryPlayerNameID);
  DEBUG_ASSERTCRASH(textEntryPlayerName,
                    ("Could not find the textEntryPlayerName"));

  windowMap = TheWindowManager->winGetWindowFromId(parentSkirmishGameOptions,
                                                   windowMapID);
  DEBUG_ASSERTCRASH(
      windowMap, ("Could not find the SkirmishGameOptionsMenu.wnd:MapWindow"));
  ...
  for (Int i = 0; i < MAX_SLOTS; i++) {
    ...
    if (i != 0) {
      ...
      comboBoxPlayer[i] = TheWindowManager->winGetWindowFromId(
          parentSkirmishGameOptions, comboBoxPlayerID[i]);
    ...
    if (i == 0) {
      GadgetTextEntrySetText(textEntryPlayerName,
                             TheGameText->fetch("GUI:Player"));
      // GadgetComboBoxAddEntry(comboBoxPlayer[i], TheGameText->fetch(
      // "GUI:Start" ), white);
      // GadgetComboBoxSetSelectedPos(comboBoxPlayer[i],0);
    } else {
      GadgetComboBoxAddEntry(comboBoxPlayer[i], TheGameText->fetch("GUI:Open"),
                             white);  // leave this first
      GadgetComboBoxSetItemData(comboBoxPlayer[i], 0, (void *)SLOT_OPEN);
      GadgetComboBoxAddEntry(comboBoxPlayer[i],
                             TheGameText->fetch("GUI:Closed"),
                             white);  // leave this first
      GadgetComboBoxSetItemData(comboBoxPlayer[i], 1, (void *)SLOT_CLOSED);
      GadgetComboBoxAddEntry(comboBoxPlayer[i],
                             TheGameText->fetch("GUI:EasyAI"), white);
      GadgetComboBoxSetItemData(comboBoxPlayer[i], 2, (void *)SLOT_EASY_AI);
      GadgetComboBoxAddEntry(comboBoxPlayer[i],
                             TheGameText->fetch("GUI:MediumAI"), white);
      GadgetComboBoxSetItemData(comboBoxPlayer[i], 3, (void *)SLOT_MED_AI);
      GadgetComboBoxAddEntry(comboBoxPlayer[i],
                             TheGameText->fetch("GUI:HardAI"), white);
    ...
    comboBoxColor[i] = TheWindowManager->winGetWindowFromId(
        parentSkirmishGameOptions, comboBoxColorID[i]);
    DEBUG_ASSERTCRASH(comboBoxColor[i],
                      ("Could not find the comboBoxColor[%d]", i));
    ...
    comboBoxPlayerTemplate[i] = TheWindowManager->winGetWindowFromId(
        parentSkirmishGameOptions, comboBoxPlayerTemplateID[i]);
    DEBUG_ASSERTCRASH(comboBoxPlayerTemplate[i],
                      ("Could not find the comboBoxPlayerTemplate[%d]", i));

    tmpString.format("SkirmishGameOptionsMenu.wnd:ComboBoxTeam%d", i);
    comboBoxTeamID[i] = TheNameKeyGenerator->nameToKey(tmpString);
    comboBoxTeam[i] = TheWindowManager->winGetWindowFromId(
        parentSkirmishGameOptions, comboBoxTeamID[i]);
    DEBUG_ASSERTCRASH(comboBoxTeam[i],
                      ("Could not find the comboBoxTeam[%d]", i));

    //		tmpString.format("SkirmishGameOptionsMenu.wnd:ButtonStartPosition%d",
    //i); 		buttonStartPositionID[i] = TheNameKeyGenerator->nameToKey( tmpString
    //); 		buttonStartPosition[i] = TheWindowManager->winGetWindowFromId(
    //parentSkirmishGameOptions, buttonStartPositionID[i] );
    //		DEBUG_ASSERTCRASH(buttonStartPosition[i], ("Could not find the
    //ButtonStartPosition[%d]",i ));
    //
    ...
    buttonMapStartPosition[i] = TheWindowManager->winGetWindowFromId(
        parentSkirmishGameOptions, buttonMapStartPositionID[i]);
    DEBUG_ASSERTCRASH(buttonMapStartPosition[i],
                      ("Could not find the ButtonMapStartPosition[%d]", i));
  }

  for (i = 0; i < MAX_SLOTS; ++i) {
    PopulateColorComboBox(i, comboBoxColor, TheSkirmishGameInfo);
    GadgetComboBoxSetSelectedPos(comboBoxColor[i], 0);
    PopulatePlayerTemplateComboBox(i, comboBoxPlayerTemplate,
                                   TheSkirmishGameInfo, FALSE);
    PopulateTeamComboBox(i, comboBoxTeam, TheSkirmishGameInfo);

    //		if (buttonStartPosition[i])
    //			buttonStartPosition[i]->winHide(TRUE); // not using
    //these right now
  }
...
void skirmishUpdateSlotList(void) {
  if (doUpdateSlotList) {
    ...
    std::map<AsciiString, MapMetaData>::iterator it =
        TheMapCache->find(lowerMap);
    if (it != TheMapCache->end()) {
      GadgetStaticTextSetText(textEntryMapDisplay, it->second.m_displayName);
    }
    GadgetTextEntrySetText(textEntryPlayerName,
                           TheSkirmishGameInfo->getSlot(0)->getName());
    UpdateSlotList(TheSkirmishGameInfo, comboBoxPlayer, comboBoxColor,
                   comboBoxPlayerTemplate, comboBoxTeam, NULL, buttonStart,
...
static const char *perPlayerGadgetsToHide[] = {
    "ComboBoxPlayer", "ComboBoxTeam", "ComboBoxColor", "ComboBoxPlayerTemplate",
    NULL  // keep this last
};
...
void updateSkirmishGameOptions(void) {
  ...
  if (isSkirmish) {
    ShowUnderlyingGUIElements(TRUE, layoutFilename, parentName, gadgetsToHide,
                              perPlayerGadgetsToHide);
  } else {
    ShowUnderlyingGUIElements(FALSE, layoutFilename, parentName, gadgetsToHide,
                              perPlayerGadgetsToHide);
    for (Int i = 1; i < MAX_SLOTS; ++i) {
      GadgetComboBoxSetSelectedPos(comboBoxPlayer[i], 0);
    }
...
void SkirmishGameOptionsMenuInit(WindowLayout *layout, void *userData) {
  if (TheGameEngine->getQuitting()) return;

  sliderGameSpeedID = TheNameKeyGenerator->nameToKey(
      AsciiString("SkirmishGameOptionsMenu.wnd:SliderGameSpeed"));
  ...
  GameWindow *sliderGameSpeed = TheWindowManager->winGetWindowFromId(
      parentSkirmishGameOptions, sliderGameSpeedID);
  Int sliderPos = max(
      15, min(61, prefs.getInt("FPS", TheGlobalData->m_framesPerSecondLimit)));
  ...
  /* hey, for now we're also going to disable the map select button until it
   * doesn't crash */
  ...
  //	TheShell->registerWithAnimateManager(buttonExit,
  //WIN_ANIMATION_SLIDE_RIGHT, TRUE, 1);
  doUpdateSlotList = TRUE;
  skirmishUpdateSlotList();
  justEntered = TRUE;
  ...
  GameWindow *win = TheWindowManager->winGetWindowFromId(
      NULL,
      TheNameKeyGenerator->nameToKey("SkirmishGameOptionsMenu.wnd:SubParent"));
  if (win) win->winHide(TRUE);
  buttonPushed = FALSE;
}  // void SkirmishGameOptionsMenuInit( WindowLayout *layout, void *userData )
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType SkirmishGameOptionsMenuInput(GameWindow *window,
                                                  UnsignedInt msg,
        ...
        case KEY_ESC: {
          ...
          if (BitTest(state, KEY_STATE_UP)) {
            TheWindowManager->winSendSystemMsg(
                window, GBM_SELECTED, (WindowMsgData)buttonExit, buttonExitID);
          }  // end if
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType SkirmishGameOptionsMenuSystem(GameWindow *window,
                                                   UnsignedInt msg,
                                                   WindowMsgData mData1,
                                                   WindowMsgData mData2) {
  ...
  switch (msg) {
    ...
    case GBM_SELECTED: {
      GameWindow *control = (GameWindow *)mData1;
      Int controlID = control->winGetWindowId();
      ///				static NameKeyType buttonResetFPSID =
      ///TheNameKeyGenerator->nameToKey( AsciiString(
      ///"SkirmishGameOptionsMenu.wnd:ButtonResetFPS" ) );
      ...
      //				{
      //					static NameKeyType
      //sliderGameSpeedID = TheNameKeyGenerator->nameToKey( AsciiString(
      //"SkirmishGameOptionsMenu.wnd:SliderGameSpeed" ) ); 					GameWindow
      //*sliderGameSpeed = TheWindowManager->winGetWindowFromId(
      //parentSkirmishGameOptions, sliderGameSpeedID ); 					GadgetSliderSetPosition(
      //sliderGameSpeed, TheGlobalData->m_framesPerSecondLimit );
      //				}
      else if (controlID == buttonSelectMapID) {
        sandboxOk = FALSE;
        // buttonBack->winEnable( false );
        skirmishMapSelectLayout = TheWindowManager->winCreateLayout(
            AsciiString("Menus/SkirmishMapSelectMenu.wnd"));
        ...
        for (Int i = 0; i < MAX_SLOTS; i++) {
          if (controlID == buttonMapStartPositionID[i]) {
            ...
            if (playerIdxInPos >= 0) {
              ...
              if (playerIdxInPos == TheSkirmishGameInfo->getLocalSlotNum() ||
                  (TheSkirmishGameInfo->amIHost() && slot && slot->isAI())) {
    ...
    case GEM_UPDATE_TEXT: {
      ...
      if (controlID == textEntryPlayerNameID) {
        TheSkirmishGameInfo->getSlot(0)->setName(
...
void populateSkirmishBattleHonors(void) {
  GameWindow *list = TheWindowManager->winGetWindowFromId(
      NULL, NAMEKEY("SkirmishGameOptionsMenu.wnd:ListboxInfo"));
  if (!list) return;
  ...
  UnicodeString uStr;
  GameWindow *streakWindow = TheWindowManager->winGetWindowFromId(
      NULL, NAMEKEY("SkirmishGameOptionsMenu.wnd:StaticTextStreakValue"));
  if (streakWindow) {
    uStr.format(L"%d", stats.getWinStreak());
    GadgetStaticTextSetText(streakWindow, uStr);
  }
  GameWindow *bestStreakWindow = TheWindowManager->winGetWindowFromId(
      NULL, NAMEKEY("SkirmishGameOptionsMenu.wnd:StaticTextBestStreakValue"));
  if (bestStreakWindow) {
    uStr.format(L"%d", stats.getBestWinStreak());
    GadgetStaticTextSetText(bestStreakWindow, uStr);
  }
  GameWindow *winsWindow = TheWindowManager->winGetWindowFromId(
      NULL, NAMEKEY("SkirmishGameOptionsMenu.wnd:StaticTextWinsValue"));
  if (winsWindow) {
    uStr.format(L"%d", stats.getWins());
    GadgetStaticTextSetText(winsWindow, uStr);
  }
  GameWindow *lossesWindow = TheWindowManager->winGetWindowFromId(
      NULL, NAMEKEY("SkirmishGameOptionsMenu.wnd:StaticTextLossesValue"));
  if (lossesWindow) {
  ...
  GadgetListBoxAddEntryImage(list, NULL, 0, 0, 10, 10, TRUE,
                             GameMakeColor(255, 255, 255, 255));
  row = 1;
  InsertBattleHonor(
      list, TheMappedImageCollection->findImageByName("HonorCampaignChina"),
      (honors & BATTLE_HONOR_CAMPAIGN_CHINA), BATTLE_HONOR_CAMPAIGN_CHINA, row,
      column);
  InsertBattleHonor(
      list, TheMappedImageCollection->findImageByName("HonorCampaignGLA"),
      (honors & BATTLE_HONOR_CAMPAIGN_GLA), BATTLE_HONOR_CAMPAIGN_GLA, row,
      column);
  InsertBattleHonor(
      list, TheMappedImageCollection->findImageByName("HonorCampaignUSA"),
      (honors & BATTLE_HONOR_CAMPAIGN_USA), BATTLE_HONOR_CAMPAIGN_USA, row,
      column);

  InsertBattleHonor(
      list, TheMappedImageCollection->findImageByName("HonorAirWing"),
      (honors & BATTLE_HONOR_AIR_WING), BATTLE_HONOR_AIR_WING, row, column);
  InsertBattleHonor(
      list, TheMappedImageCollection->findImageByName("HonorBattleTank"),
      (honors & BATTLE_HONOR_BATTLE_TANK), BATTLE_HONOR_BATTLE_TANK, row,
      column);
  GadgetListBoxAddEntryImage(list, NULL, 2, 0, 10, 10, TRUE,
                             GameMakeColor(255, 255, 255, 255));
  row = 3;
  // endurance medals
  ...
  for (it = TheMapCache->begin(); it != TheMapCache->end(); ++it) {
    if (!it->second.m_isOfficial || !it->second.m_isMultiplayer) continue;

    Bool easy = TRUE;
    Bool med = TRUE;
    Bool hard = TRUE;
    if (stats.getEnduranceMedal(it->first, SLOT_EASY_AI) == 0) easy = FALSE;
    if (stats.getEnduranceMedal(it->first, SLOT_MED_AI) == 0) med = FALSE;
    if (stats.getEnduranceMedal(it->first, SLOT_BRUTAL_AI) == 0) hard = FALSE;

    if (!easy && !med && !hard) missingEasy = TRUE;

    if (!med && !hard) missingMedium = TRUE;

    if (!hard) missingBrutal = TRUE;
  }
  if (!missingBrutal) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("Challenge_Gold"), TRUE,
        BATTLE_HONOR_ENDURANCE, row, column);
  } else if (!missingMedium) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("Challenge_Silver"),
        TRUE, BATTLE_HONOR_ENDURANCE, row, column);
  } else if (!missingEasy) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("Challenge_Bronz"),
        TRUE, BATTLE_HONOR_ENDURANCE, row, column);
  } else {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("Challenge_Bronz"),
        FALSE, BATTLE_HONOR_ENDURANCE, row, column);
  }

  InsertBattleHonor(
      list, TheMappedImageCollection->findImageByName("Apocalypse"),
      (honors & BATTLE_HONOR_APOCALYPSE), BATTLE_HONOR_APOCALYPSE, row, column);

  if (honors & BATTLE_HONOR_BLITZ5) {
    InsertBattleHonor(list,
                      TheMappedImageCollection->findImageByName("HonorBlitz5"),
                      TRUE, BATTLE_HONOR_BLITZ5, row, column);
  } else if (honors & BATTLE_HONOR_BLITZ10) {
    InsertBattleHonor(list,
                      TheMappedImageCollection->findImageByName("HonorBlitz10"),
                      TRUE, BATTLE_HONOR_BLITZ10, row, column);
  } else {
    InsertBattleHonor(list,
                      TheMappedImageCollection->findImageByName("HonorBlitz10"),
                      FALSE, BATTLE_HONOR_BLITZ10, row, column);
  }

  uStr.format(L"%10d", stats.getBestWinStreak());
  if (honors & BATTLE_HONOR_STREAK_25) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("HonorStreak_G"), TRUE,
        BATTLE_HONOR_STREAK_25, row, column, uStr);
  } else if (honors & BATTLE_HONOR_STREAK_10) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("HonorStreak_S"), TRUE,
        BATTLE_HONOR_STREAK_10, row, column, uStr);
  } else if (honors & BATTLE_HONOR_STREAK_3) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("HonorStreak_B"), TRUE,
        BATTLE_HONOR_STREAK_3, row, column, uStr);
  } else {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("HonorStreak_B"), FALSE,
        BATTLE_HONOR_STREAK_3, row, column);
  }

  /*
  InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("Loyalty_USA"), (honors &
  BATTLE_HONOR_LOYALTY_USA), BATTLE_HONOR_LOYALTY_USA, row, column);
  InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("Loyalty_China"), (honors &
  BATTLE_HONOR_LOYALTY_CHINA), BATTLE_HONOR_LOYALTY_CHINA, row, column);
  InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("Loyalty_GLA"), (honors &
  BATTLE_HONOR_LOYALTY_GLA), BATTLE_HONOR_LOYALTY_GLA, row, column);
  */
  ...
  {
          InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("HonorChallenge7"), TRUE,
                  BATTLE_HONOR_CHALLENGE, row, column);
  }
  else if (BitTest(challenge, BH_CHALLENGE_MASK_6))
  {
          InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("HonorChallenge6"), TRUE,
                  BATTLE_HONOR_CHALLENGE, row, column);
  }
  else if (BitTest(challenge, BH_CHALLENGE_MASK_5))
  {
          InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("HonorChallenge5"), TRUE,
                  BATTLE_HONOR_CHALLENGE, row, column);
  }
  else if (BitTest(challenge, BH_CHALLENGE_MASK_4))
  {
          InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("HonorChallenge4"), TRUE,
                  BATTLE_HONOR_CHALLENGE, row, column);
  }
  else if (BitTest(challenge, BH_CHALLENGE_MASK_3))
  {
          InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("HonorChallenge3"), TRUE,
                  BATTLE_HONOR_CHALLENGE, row, column);
  }
  else if (BitTest(challenge, BH_CHALLENGE_MASK_2))
  {
          InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("HonorChallenge2"), TRUE,
                  BATTLE_HONOR_CHALLENGE, row, column);
  }
  else if (BitTest(challenge, BH_CHALLENGE_MASK_1))
  {
          InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("HonorChallenge1"), TRUE,
                  BATTLE_HONOR_CHALLENGE, row, column);
  }
  else
  {
          InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("HonorChallenge1"), FALSE,
                  BATTLE_HONOR_CHALLENGE, row, column);
  }
  */
  ...
  if (isPreorder != 0) {
    InsertBattleHonor(list,
                      TheMappedImageCollection->findImageByName("OfficersClub"),
                      TRUE, BATTLE_HONOR_OFFICERSCLUB, row, column);
  }
}
