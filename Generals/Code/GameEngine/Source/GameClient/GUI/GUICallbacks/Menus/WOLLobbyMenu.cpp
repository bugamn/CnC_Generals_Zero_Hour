...
Bool handleLobbySlashCommands(UnicodeString uText) {
  ...
  if (token == "host") {
    ...
    TheGameSpyInfo->sendChat(UnicodeString(uText.str() + 4), TRUE,
                             listboxLobbyPlayers);
    return TRUE;  // was a slash command
  ...
  NAMEKEY("WOLCustomLobby.wnd:ButtonGameListToggle"); GameWindow *button =
  TheWindowManager->winGetWindowFromId(parent, buttonID); if (button)
          {
  ...
  else if (token == "adjustchat")
  {
          ...
          GameWindow *slider = TheWindowManager->winGetWindowFromId(parent,
  sliderID); if (slider)
          {
  ...
  */
    ...
    g_fakeCRC = !g_fakeCRC;
    TheGameSpyInfo->addText(UnicodeString(L"Toggled CRC fakery"),
...
void SetLobbyAttemptHostJoin(Bool start) { s_tryingToHostOrJoin = start; }
...
static void playerTooltip(GameWindow *window, WinInstanceData *instData,
  ...
  Bool isLocalPlayer =
      (TheGameSpyInfo->getLocalName().compareNoCase(info->m_name) == 0);
  ...
  UnicodeString tooltip = UnicodeString::
      TheEmptyString;  // TheGameText->fetch("TOOLTIP:PlayersInLobby");
  if (isLocalPlayer) {
    tooltip.format(TheGameText->fetch("TOOLTIP:LocalPlayer"), uName.str());
  } else {
    // not us
    if (TheGameSpyInfo->getBuddyMap()->find(info->m_profileID) !=
  ...
  Int rank = 0;
  Int i = 0;
  while (info->m_rankPoints >= TheRankPointValues->m_ranks[i + 1]) ++i;
  rank = i;
  AsciiString sideName = "GUI:RandomSide";
  if (info->m_side > 0) {
    const PlayerTemplate *fac =
        ThePlayerTemplateStore->getNthPlayerTemplate(info->m_side);
    if (fac) {
    ...
    if (room.m_groupID != TheGameSpyConfig->getQMChannel()) {
      ...
      if (room.m_groupID == TheGameSpyInfo->getCurrentGroupRoom()) {
        Int selected = GadgetComboBoxAddEntry(
            lb, room.m_translatedName, GameSpyColor[GSCOLOR_CURRENTROOM]);
...
const Image *LookupSmallRankImage(Int side, Int rankPoints) {
  ...
  DEBUG_ASSERTLOG(img, ("*** Could not load small rank image '%s' from "
...
static Int insertPlayerInListbox(const PlayerInfo &info, Color color) {
  ...
  if (info.m_profileID)
  {
          PSPlayerStats psStats =
  TheGameSpyPSMessageQueue->findPlayerStatsByID(info.m_profileID); if
  (psStats.id)
          {
                  ...
                  Int numGames = 0;
                  for(it = psStats.games.begin(); it != psStats.games.end();
  ++it)
                  {
  ...
  */
  ...
  Int index =
      GadgetListBoxAddEntryImage(listboxLobbyPlayers, preorderImg, -1, 0, w, h);
...
void PopulateLobbyPlayerListbox(void) {
  ...
  if (listboxLobbyPlayers) {
    ...
    for (it = players->begin(); it != players->end(); ++it) {
      PlayerInfo info = it->second;
      if (info.m_flags & PEER_FLAG_OP ||
          TheGameSpyConfig->isPlayerVIP(info.m_profileID)) {
        Int index = insertPlayerInListbox(
            info, info.isIgnored() ? GameSpyColor[GSCOLOR_PLAYER_IGNORED]
    ...
    if (indicesToSelect.size()) {
      ...
      for (i = 0, indexIt = indicesToSelect.begin();
           indexIt != indicesToSelect.end(); ++i, ++indexIt) {
        newIndices[i] = *indexIt;
        DEBUG_LOG(("Queueing up index %d to re-select\n", *indexIt));
      }
      GadgetListBoxSetSelected(listboxLobbyPlayers, newIndices,
                               indicesToSelect.size());
      delete[] newIndices;
    }
...
void WOLLobbyMenuInit(WindowLayout *layout, void *userData) {
  ...
  if (!TheGameSpyInfo->getCurrentGroupRoom()) {
    if (groupRoomToJoin) {
      DEBUG_LOG(
          ("WOLLobbyMenuInit() - rejoining group room %d\n", groupRoomToJoin));
    ...
    DEBUG_LOG(
        ("WOLLobbyMenuInit() - not joining group room because we're already in "
         "one\n"));
  }
  ...
  // animate controls
  //	TheShell->registerWithAnimateManager(parent, WIN_ANIMATION_SLIDE_TOP,
  //TRUE);
...
//-------------------------------------------------------------------------------------------------
/** refreshGameList
                The Bool is used to force refresh if the refresh button was
   hit.*/
//-------------------------------------------------------------------------------------------------
...
static void refreshPlayerList(Bool forceRefresh) {
  ...
  if (forceRefresh ||
      ((playerListRefreshTime == 0) ||
       ((playerListRefreshTime + refreshInterval) <= timeGetTime()))) {
      ...
      switch (resp.peerResponseType) {
        ...
        case PeerResponse::PEERRESPONSE_MESSAGE: {
          TheGameSpyInfo->addChat(resp.nick.c_str(), resp.message.profileID,
                                  ...
                                  resp.message.isAction, listboxLobbyChat);
        } break;
        case PeerResponse::PEERRESPONSE_DISCONNECT: {
          sawImportantMessage = TRUE;
          ...
          if (resp.joinStagingRoom.ok == PEERTrue && isHostPresent) {
            ...
            switch (resp.joinStagingRoom.result) {
              ...
              case PEERInviteOnlyRoom:  // The room is invite only.
                ...
                break;
              case PEERBannedFromRoom:  // The local user is banned from the
                                        // room.
                s = TheGameText->fetch("GUI:JoinFailedBannedFromRoom");
                break;
              case PEERBadPassword:  // An incorrect password (or none) was
                                     // given for a passworded room.
                s = TheGameText->fetch("GUI:JoinFailedBadPassword");
                break;
              case PEERAlreadyInRoom:  // The local user is already in or
                                       // entering a room of the same type.
                s = TheGameText->fetch("GUI:JoinFailedAlreadyInRoom");
                break;
              case PEERNoConnection:  // Can't join a room if there's no chat
                                      // connection.
        ...
        case PeerResponse::PEERRESPONSE_STAGINGROOM: {
          ...
          switch (resp.stagingRoom.action) {
            case PEER_CLEAR:
              TheGameSpyInfo->clearStagingRoomList();
              // TheGameSpyInfo->addText( UnicodeString(L"gameList:
              // PEER_CLEAR"), GameSpyColor[GSCOLOR_DEFAULT], listboxLobbyChat
              // );
              break;
            case PEER_ADD:
            case PEER_UPDATE: {
              ...
              // if (ParseAsciiStringToGameInfo(&room,
              // resp.stagingRoomMapName.c_str())) if
              // (ParseAsciiStringToGameInfo(&room,
              // resp.stagingServerGameOptions.c_str()))
              Bool serverOk = TRUE;
              if (!resp.stagingRoomMapName.length()) {
                serverOk = FALSE;
              }
              // fix for ghost game problem - need to iterate over all
              // resp.stagingRoomPlayerNames[i]
              Bool sawSelf = FALSE;
              // for (Int i=0; i<MAX_SLOTS; ++i)
              //{
              if (TheGameSpyInfo->getLocalName() ==
              ...
              if (serverOk) {
                ...
                room.setMap(
                    TheGameState->portableMapPathToRealMapPath(mapName));
                ...
                for (i = 0; i < MAX_SLOTS; ++i) {
                  ...
                  if (slot) {
                    ...
                    if (resp.stagingRoom.profileID[i] == SLOT_EASY_AI) {
                      ...
                      ++numPlayers;
                    } else if (resp.stagingRoom.profileID[i] ==
                               SLOT_BRUTAL_AI) {
                ...
                if (resp.stagingRoom.action == PEER_ADD) {
                  ...
                  // TheGameSpyInfo->addText( UnicodeString(L"gameList:
                  // PEER_ADD"), GameSpyColor[GSCOLOR_DEFAULT], listboxLobbyChat
                  // );
                } else {
                  TheGameSpyInfo->updateStagingRoom(room);
                  // TheGameSpyInfo->addText( UnicodeString(L"gameList:
                  // PEER_UPDATE"), GameSpyColor[GSCOLOR_DEFAULT],
                  // listboxLobbyChat );
                }
              } else {
                room.setID(resp.stagingRoom.id);
                ...
                // TheGameSpyInfo->addText( UnicodeString(L"gameList:
                // PEER_UPDATE FAILED"), GameSpyColor[GSCOLOR_DEFAULT],
                // listboxLobbyChat );
              }
              break;
            }
            case PEER_REMOVE:
              ...
              TheGameSpyInfo->removeStagingRoom(room);
              // TheGameSpyInfo->addText( UnicodeString(L"gameList:
              // PEER_REMOVE"), GameSpyColor[GSCOLOR_DEFAULT], listboxLobbyChat
              // );
              break;
            default:
              // TheGameSpyInfo->addText( UnicodeString(L"gameList: Unknown"),
              // GameSpyColor[GSCOLOR_DEFAULT], listboxLobbyChat );
              break;
          }
        } break;
      }
...
WindowMsgHandledType WOLLobbyMenuSystem(GameWindow *window, UnsignedInt msg,
  ...
  switch (msg) {
    ...
    case GWM_CREATE: {
      ...
      //				sliderChatAdjustID =
      //NAMEKEY("WOLCustomLobby.wnd:SliderChatAdjust");
    ...
    case GBM_SELECTED: {
      ...
      if (HandleSortButton((NameKeyType)controlID)) break;

      // If we back out, just bail - we haven't gotten far enough to need to log
      // out
      if (controlID == buttonBackID) {
        if (s_tryingToHostOrJoin) break;

        // Leave any group room, then pop off the screen
        TheGameSpyInfo->leaveGroupRoom();

        SetLobbyAttemptHostJoin(
            TRUE);  // pretend, since we don't want to queue up another action
        buttonPushed = true;
        ...
        if (selected >= 0) {
          ...
          if (selectedID > 0) {
            ...
            if (srmIt != srm->end()) {
              ...
              if (!roomToJoin ||
                  roomToJoin->getExeCRC() != TheGlobalData->m_exeCRC ||
                  roomToJoin->getIniCRC() != TheGlobalData->m_iniCRC) {
                // bad crc.  don't go.
                DEBUG_LOG(
                    ("WOLLobbyMenuSystem - CRC mismatch with the game I'm "
                     "trying to join. My CRC's - EXE:0x%08X INI:0x%08X  Their "
                     "CRC's - EXE:0x%08x INI:0x%08x\n",
                     TheGlobalData->m_exeCRC, TheGlobalData->m_iniCRC,
                     roomToJoin->getExeCRC(), roomToJoin->getIniCRC()));
#if defined(_DEBUG) || defined(_INTERNAL)
                if (TheGlobalData->m_netMinPlayers) {
                  GSMessageBoxOk(
                      TheGameText->fetch("GUI:JoinFailedDefault"),
        ...
        if (!txtInput.isEmpty()) {
          ...
          TheGameSpyInfo->sendChat(
              txtInput, FALSE,
              listboxLobbyPlayers);  // 'emote' button now just sends text
        }
      }
      ...
      if (controlID == comboLobbyGroupRoomsID) {
        ...
        if (rowSelected >= 0) {
          ...
          DEBUG_LOG(("ItemData was %d, current Group Room is %d\n", groupID,
                     TheGameSpyInfo->getCurrentGroupRoom()));
    ...
    case GLM_DOUBLE_CLICKED: {
      ...
      if (controlID == GetGameListBoxID()) {
        ...
        if (rowSelected >= 0) {
          ...
          TheWindowManager->winSendSystemMsg(
              window, GBM_SELECTED, (WindowMsgData)button, buttonJoinID);
        }
      }
      break;
    }  // case GLM_DOUBLE_CLICKED:
