...
static void updateLadderDetails(Int ladderID, GameWindow *staticTextLadderName,
                                GameWindow *listboxLadderDetails);
...
enum PasswordMode { PASS_NONE, PASS_ENTRY, PASS_ERROR };
...
static void setPasswordMode(PasswordMode mode) {
  ...
  switch (mode) {
    case PASS_NONE:
      if (passwordParent) passwordParent->winHide(TRUE);
      if (badPasswordParent) badPasswordParent->winHide(TRUE);
      if (buttonOk) buttonOk->winEnable(TRUE);
      if (buttonCancel) buttonCancel->winEnable(TRUE);
      if (textEntryPassword) textEntryPassword->winEnable(FALSE);
      if (listboxLadderSelect) listboxLadderSelect->winEnable(TRUE);
      TheWindowManager->winSetFocus(listboxLadderSelect);
      break;
    case PASS_ENTRY:
      if (passwordParent) passwordParent->winHide(FALSE);
      if (badPasswordParent) badPasswordParent->winHide(TRUE);
      if (buttonOk) buttonOk->winEnable(FALSE);
      if (buttonCancel) buttonCancel->winEnable(FALSE);
      if (textEntryPassword) {
        textEntryPassword->winEnable(TRUE);
        GadgetTextEntrySetText(textEntryPassword,
                               UnicodeString::TheEmptyString);
      }
      if (listboxLadderSelect) listboxLadderSelect->winEnable(FALSE);
      TheWindowManager->winSetFocus(textEntryPassword);
      break;
    case PASS_ERROR:
      if (passwordParent) passwordParent->winHide(TRUE);
      if (badPasswordParent) badPasswordParent->winHide(FALSE);
      if (buttonOk) buttonOk->winEnable(FALSE);
      if (buttonCancel) buttonCancel->winEnable(FALSE);
      if (textEntryPassword) textEntryPassword->winEnable(FALSE);
      if (listboxLadderSelect) listboxLadderSelect->winEnable(FALSE);
...
void PopupLadderSelectInit(WindowLayout *layout, void *userData) {
  ...
  listboxLadderSelect =
      TheWindowManager->winGetWindowFromId(parent, listboxLadderSelectID);

  listboxLadderDetailsID =
      NAMEKEY("PopupLadderSelect.wnd:ListBoxLadderDetails");
  listboxLadderDetails =
      TheWindowManager->winGetWindowFromId(parent, listboxLadderDetailsID);

  staticTextLadderNameID =
      NAMEKEY("PopupLadderSelect.wnd:StaticTextLadderName");
  staticTextLadderName =
      TheWindowManager->winGetWindowFromId(parent, staticTextLadderNameID);
  ...
  passwordParent =
      TheWindowManager->winGetWindowFromId(parent, passwordParentID);
  buttonPasswordOkID = NAMEKEY("PopupLadderSelect.wnd:ButtonPasswordOk");
  buttonPasswordCancelID =
      NAMEKEY("PopupLadderSelect.wnd:ButtonPasswordCancel");
  textEntryPasswordID = NAMEKEY("PopupLadderSelect.wnd:PasswordEntry");
  textEntryPassword =
      TheWindowManager->winGetWindowFromId(parent, textEntryPasswordID);

  // bad password popup
  badPasswordParentID = NAMEKEY("PopupLadderSelect.wnd:BadPasswordParent");
  badPasswordParent =
      TheWindowManager->winGetWindowFromId(parent, badPasswordParentID);
...
/** Input callback */
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType PopupLadderSelectInput(GameWindow *window, UnsignedInt msg,
...
/** System callback */
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType PopupLadderSelectSystem(GameWindow *window,
                                             UnsignedInt msg,
                                             WindowMsgData mData1,
                                             WindowMsgData mData2) {
  switch (msg) {
    ...
    case GBM_SELECTED: {
      ...
      if (controlID == buttonOkID) {
        ...
        ladderIndex =
            (Int)GadgetListBoxGetItemData(listboxLadderSelect, selectPos, 0);
        ...
        if (!li || li->cryptedPassword.isEmpty()) {
          // eh?  something's not right.  just pretend they typed something
          // wrong...
    ...
    case GLM_DOUBLE_CLICKED: {
      ...
      if (controlID == listboxLadderSelectID) {
        TheWindowManager->winSendSystemMsg(parent, GBM_SELECTED,
                                           (WindowMsgData)buttonOk,
                                           buttonOk->winGetWindowId());
      }
      break;
    }
    ...
    case GEM_EDIT_DONE: {
      ...
      if (controlID == textEntryPasswordID) {
        TheWindowManager->winSendSystemMsg(
            parent, GBM_SELECTED,
            (WindowMsgData)(TheWindowManager->winGetWindowFromId(
                passwordParent, buttonPasswordOkID)),
            buttonPasswordOkID);
      }
      break;
    }
...
static void updateLadderDetails(Int selID, GameWindow *staticTextLadderName,
                                GameWindow *listboxLadderDetails) {
  if (!staticTextLadderName || !listboxLadderDetails) return;
  ...
  // name
  line.format(TheGameText->fetch("GUI:LadderNameAndSize"), info->name.str(),
              info->playersPerTeam, info->playersPerTeam);
  GadgetStaticTextSetText(staticTextLadderName, line);

  // location
  if (!info->location.isEmpty())
    GadgetListBoxAddEntryText(listboxLadderDetails, info->location,
                              captionColor, -1);
  ...
  if (!info->description.isEmpty())
    GadgetListBoxAddEntryText(listboxLadderDetails, info->description, color,
                              -1);

  // requires password?
  if (info->cryptedPassword.isNotEmpty()) {
    GadgetListBoxAddEntryText(listboxLadderDetails,
                              TheGameText->fetch("GUI:LadderHasPassword"),
                              captionColor, -1);
  }
  ...
  if (info->randomFactions) {
    GadgetListBoxAddEntryText(listboxLadderDetails,
                              TheGameText->fetch("GUI:LadderRandomFactions"),
                              captionColor, -1);
  } else {
    GadgetListBoxAddEntryText(listboxLadderDetails,
                              TheGameText->fetch("GUI:LadderFactions"),
                              captionColor, -1);
  }

  // factions
  AsciiStringList validFactions = info->validFactions;
  for (AsciiStringListIterator it = validFactions.begin();
       it != validFactions.end(); ++it) {
    AsciiString marker;
    marker.format("INI:Faction%s", it->str());
    GadgetListBoxAddEntryText(listboxLadderDetails, TheGameText->fetch(marker),
                              color, -1);
  }
  ...
  if (info->randomMaps) {
    GadgetListBoxAddEntryText(listboxLadderDetails,
                              TheGameText->fetch("GUI:LadderRandomMaps"),
                              captionColor, -1);
  } else {
    GadgetListBoxAddEntryText(listboxLadderDetails,
                              TheGameText->fetch("GUI:LadderMaps"),
                              captionColor, -1);
  }
  ...
  for (it = validMaps.begin(); it != validMaps.end(); ++it) {
    ...
    if (md) {
      GadgetListBoxAddEntryText(listboxLadderDetails, md->m_displayName, color,
                                -1);
    }
  }
}
...
WindowMsgHandledType RCGameDetailsMenuSystem(GameWindow *window,
                                             UnsignedInt msg,
                                             WindowMsgData mData1,
                                             WindowMsgData mData2) {
      ...
      if (controlID == ladderInfoID) {
        ...
        if (srmIt != srm->end()) {
          ...
          if (linfo) {
            ...
            GameWindow *st = TheWindowManager->winGetWindowFromId(
                NULL, NAMEKEY("PopupLadderDetails.wnd:StaticTextLadderName"));
            GameWindow *lb = TheWindowManager->winGetWindowFromId(
                NULL, NAMEKEY("PopupLadderDetails.wnd:ListBoxLadderDetails"));
