...
struct ScoreGather {
  Int m_totalMoneyEarned;     ///< The total money that was harvested, refined,
                              ///< received in crates
  Int m_totalMoneySpent;      ///< The total money spent on units, buildings,
                              ///< repairs
  Int m_totalUnitsDestroyed;  ///< The total number of enimies that we've killed
  Int m_totalUnitsBuilt;  ///< The total number of units we've created (created
                          ///< meaning that we built from a building)
  Int m_totalUnitsLost;   ///< The total number of our units lost
  Int m_totalBuildingsDestroyed;  ///< The total number of Buildings we've
                                  ///< destroyed
  Int m_totalBuildingsBuilt;      ///< The total number of buildings we've
                                  ///< constructed
  Int m_totalBuildingsLost;       ///< The total number of our buildings lost
  const Image *m_sideImage;
};
void populateSideInfo(UnicodeString side, ScoreGather *sg, Int pos,
                      Color color);
//-----------------------------------------------------------------------------
// PUBLIC FUNCTIONS ///////////////////////////////////////////////////////////
//-----------------------------------------------------------------------------
...
void ScoreScreenEnableControls(Bool enable) {
  ...
  GameWindow *buttonSaveReplay =
      TheWindowManager->winGetWindowFromId(parent, buttonSaveReplayID);
  if ((buttonSaveReplay != NULL) &&
      (buttonSaveReplay->winIsHidden() == FALSE)) {
    if (!canSaveReplay) enable = FALSE;
...
void ScoreScreenInit(WindowLayout *layout, void *userData) {
  if (TheGameSpyInfo) {
    DEBUG_LOG(("ScoreScreenInit(): TheGameSpyInfo->stuff(%s/%s/%s)\n",
               TheGameSpyInfo->getLocalBaseName().str(),
               TheGameSpyInfo->getLocalEmail().str(),
               TheGameSpyInfo->getLocalPassword().str()));
  }
  ...
  buttonOkID =
      TheNameKeyGenerator->nameToKey(AsciiString("ScoreScreen.wnd:ButtonOk"));
  ...
  //	buttonRehostID = TheNameKeyGenerator->nameToKey( AsciiString(
  //"ScoreScreen.wnd:ButtonRehost" ) );
  ...
  //	buttonRehost = TheWindowManager->winGetWindowFromId( parent,
  //buttonRehostID );
  chatBoxBorder = TheWindowManager->winGetWindowFromId(parent, chatBoxBorderID);
  buttonContinue =
      TheWindowManager->winGetWindowFromId(parent, buttonContinueID);
  ...
  canSaveReplay = FALSE;
  if (TheRecorder->getMode() == RECORDERMODETYPE_RECORD) canSaveReplay = TRUE;
  GameWindow *buttonSaveReplay =
      TheWindowManager->winGetWindowFromId(parent, buttonSaveReplayID);
  ...
  if (TheGameLogic->isInReplayGame()) {
    if (buttonSaveReplay) buttonSaveReplay->winHide(TRUE);
        ...
        case KEY_ESC: {
          ...
          if (BitTest(state, KEY_STATE_UP)) {
            TheWindowManager->winSendSystemMsg(
                window, GBM_SELECTED, (WindowMsgData)buttonOk, buttonOkID);
  ...
  switch (msg) {
    ...
    case GBM_SELECTED: {
      ...
      if (controlID == buttonOkID) {
        ...
        if (!buttonIsFinishCampaign) ReplayWasPressed = TRUE;
#if !defined(_PLAYTEST)
        ...
        DEBUG_ASSERTCRASH(saveReplayLayout,
                          ("Unable to get save replay menu layout.\n"));
      ...
      for (Int i = 0; i < MAX_SLOTS; ++i) {
        ...
        if (controlID == TheNameKeyGenerator->nameToKey(name)) {
          Bool notBuddy = TRUE;
          Int playerID = (Int)GadgetButtonGetData(
              TheWindowManager->winGetWindowFromId(NULL, controlID));
          ...
          if (notBuddy) {
            ...
            wcsncpy(req.arg.addbuddy.text, buddyAddstr.str(),
                    MAX_BUDDY_CHAT_LEN);
...
void initSkirmish(void) {
  ...
  grabMultiPlayerInfo();
  if (textEntryChat) textEntryChat->winHide(TRUE);
  if (buttonEmote) buttonEmote->winHide(TRUE);
  if (chatBoxBorder) chatBoxBorder->winHide(TRUE);
  if (buttonBuddies) buttonBuddies->winHide(TRUE);
  if (buttonContinue) buttonContinue->winHide(TRUE);
  if (listboxChatWindowScoreScreen) listboxChatWindowScoreScreen->winHide(TRUE);
  if (staticTextGameSaved) staticTextGameSaved->winHide(TRUE);
  //	if (buttonRehost)
...
void initSinglePlayer(void) {
  ...
  TheCampaignManager->setRankPoints(
      ThePlayerList->getLocalPlayer()->getSkillPoints());
...
void finishSinglePlayerInit(void) {
  ...
  if (copyProtectOK && TheCampaignManager->isVictorious()) {
    ...
    if (TheCampaignManager->getCurrentMap().isEmpty()) {
      GadgetButtonSetText(buttonContinue,
                          TheGameText->fetch("GUI:EndCampaign"));
      ...
      if (campaign) {
        ...
        if (buttonOk) buttonOk->winHide(TRUE);
        if (buttonContinue) buttonContinue->winHide(TRUE);
        if (textEntryChat) textEntryChat->winHide(TRUE);
        if (buttonEmote) buttonEmote->winHide(TRUE);
        ...
        if (campaign->getFinalVictoryMovie().isNotEmpty()) {
          ...
          if (TheGameLODManager) {
            ...
            if (TheGameLODManager->findStaticLODLevel() ==
                STATIC_GAME_LOD_LOW) {
              useLowRes = TRUE;
            }
          ...
          if (!useLowRes) PlayMovieAndBlock(vidName);
        }
      }
    } else {
      GadgetButtonSetText(buttonContinue,
                          TheGameText->fetch("GUI:SaveAndContinue"));

      // auto save game
      TheGameState->missionSave();
      if (staticTextGameSaved) staticTextGameSaved->winHide(FALSE);
    }
  } else {
  ...
  if (buttonOk) buttonOk->winHide(FALSE);
  if (buttonContinue) buttonContinue->winHide(FALSE);
  if (textEntryChat) textEntryChat->winHide(TRUE);
  if (buttonEmote) buttonEmote->winHide(TRUE);
  if (listboxChatWindowScoreScreen) listboxChatWindowScoreScreen->winHide(TRUE);
  if (chatBoxBorder) chatBoxBorder->winHide(TRUE);
...
void initLANMultiPlayer(void) {
  ...
  if (staticTextGameSaved) staticTextGameSaved->winHide(TRUE);
  if (textEntryChat) textEntryChat->winHide(FALSE);
  if (buttonEmote) buttonEmote->winHide(FALSE);
  if (buttonContinue) buttonContinue->winHide(TRUE);
  if (listboxChatWindowScoreScreen)
    listboxChatWindowScoreScreen->winHide(FALSE);
  if (chatBoxBorder) chatBoxBorder->winHide(FALSE);
  if (buttonBuddies) buttonBuddies->winHide(TRUE);
}
...
static Bool isSlotLocalAlly(GameInfo *game, const GameSlot *slot) {
  ...
  if (!localSlot) return TRUE;

  if (slot == localSlot) return TRUE;

  if (slot->getTeamNumber() < 0) return FALSE;
...
static void updateSkirmishBattleHonors(SkirmishBattleHonors &stats) {
  ...
  if (stats.getWinStreak() >= 3) stats.setHonors(BATTLE_HONOR_STREAK_3);
  if (stats.getWinStreak() >= 10) stats.setHonors(BATTLE_HONOR_STREAK_10);
  if (stats.getWinStreak() >= 25) stats.setHonors(BATTLE_HONOR_STREAK_25);

  const ThingTemplate *pTemplate =
      TheThingFactory->findTemplate("GLAScudStorm");
  if (s->getTotalObjectsBuilt(pTemplate) > 0) stats.setBuiltSCUD();

  pTemplate = TheThingFactory->findTemplate("AmericaParticleCannonUplink");
  if (s->getTotalObjectsBuilt(pTemplate) > 0) stats.setBuiltParticleCannon();

  pTemplate = TheThingFactory->findTemplate("ChinaNuclearMissileLauncher");
  if (s->getTotalObjectsBuilt(pTemplate) > 0) stats.setBuiltNuke();
  ...
  if (/*TheGameInfo->isSkirmish() &&*/ s->getTotalUnitsBuilt(validMask,
                                                             invalidMask) >= 50)
  ...
  if (/*TheGameInfo->isSkirmish() &&*/ (TheGameLogic->getFrame() /
                                        LOGICFRAMES_PER_SECOND / 60) < 5) {
    stats.setHonors(BATTLE_HONOR_BLITZ5);
  }

  if (/*TheGameInfo->isSkirmish() &&*/ (TheGameLogic->getFrame() /
                                        LOGICFRAMES_PER_SECOND / 60) < 10) {
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (slot->isAI() && !isSlotLocalAlly(TheGameInfo, slot)) {
      if (TheGameInfo->getConstSlot(i)->getState() == SLOT_EASY_AI) ++numEasy;
      if (TheGameInfo->getConstSlot(i)->getState() == SLOT_MED_AI) ++numMedium;
  ...
  if (/*!anyAlliedAI &&*/ (numEasy || numMedium || numBrutal)) {
    ...
    Int oldBrutal =
        stats.getEnduranceMedal(TheGameInfo->getMap(), SLOT_BRUTAL_AI);
    if (numEasy) {
...
static void updateMPBattleHonors(Int &honors, PSPlayerStats &stats) {
  ...
  if (stats.winsInARow >= 3) honors |= BATTLE_HONOR_STREAK_3;

  //	BATTLE_HONOR_STREAK_10			= 0x0000008,
  if (stats.winsInARow >= 10) honors |= BATTLE_HONOR_STREAK_10;

  //	BATTLE_HONOR_STREAK_25			= 0x0000010,
  if (stats.winsInARow >= 25) honors |= BATTLE_HONOR_STREAK_25;

  //	BATTLE_HONOR_LOYALTY_USA		= 0x0000020,
  if (stats.gamesInRowWithLastGeneral >= 20 &&
      localPlayer->getSide() == "America")
    honors |= BATTLE_HONOR_LOYALTY_USA;

  //	BATTLE_HONOR_LOYALTY_CHINA	= 0x0000040,
  if (stats.gamesInRowWithLastGeneral >= 20 &&
      localPlayer->getSide() == "China")
...
static void updateChallengeMedals(Int &medals) {
  if (!TheGameInfo->isSkirmish()) return;
...
/** Populate the various windows with the information about the game based on
   each player's score keeper. */
//-------------------------------------------------------------------------------------------------
void populatePlayerInfo(Player *player, Int pos) {
  if (!player || pos > MAX_SLOTS) return;
  Color color = player->getPlayerColor();
  ScoreKeeper *scoreKpr = player->getScoreKeeper();
  if (!scoreKpr) {
    DEBUG_ASSERTCRASH(FALSE, ("Player %s does not have a scoreKeeper",
  ...
  win = TheWindowManager->winGetWindowFromId(
      parent, TheNameKeyGenerator->nameToKey(winName));
  DEBUG_ASSERTCRASH(
      win, ("Could not find window %s on the score screen", winName.str()));
...
TheNameKeyGenerator->nameToKey( winName ) ); DEBUG_ASSERTCRASH(win,("Could not
find window %s on the score screen", winName.str())); winValue.format(L"%d",
scoreKpr->calculateScore()); GadgetStaticTextSetText(win, winValue);
  win->winSetEnabledTextColors(color, win->winGetEnabledTextBorderColor());
  win->winHide(FALSE);
*/
  // set the Buttons
  ...
  //	win =  TheWindowManager->winGetWindowFromId( parent,
  //TheNameKeyGenerator->nameToKey( winName ) ); 	DEBUG_ASSERTCRASH(win,("Could
  //not find window %s on the score screen", winName.str())); 	if(screenType ==
  //SCORESCREEN_INTERNET && TheGameSpyInfo &&
  //TheGameSpyInfo->getLocalProfileID() != 0)
  //	{
  ...
  //
  //		PlayerInfoMap::iterator blah =
  //TheGameSpyInfo->getPlayerInfoMap()->find(aName); 		if(blah->second.m_profileID
  //!= 0)
  //		{
  //
  //			BuddyInfoMap::iterator bIt =
  //TheGameSpyInfo->getBuddyMap()->find(blah->second.m_profileID); 			if(bIt !=
  //TheGameSpyInfo->getBuddyMap()->end()  ||
  //(TheGameSpyInfo->getLocalProfileID() == blah->second.m_profileID))
  //			{
  //				// already our buddy
  //				win->winHide(TRUE);
  //			}
  ...
  //			{
  //
  //				GadgetButtonSetData(win, (void *)
  //blah->second.m_profileID); 				win->winEnable(TRUE); 				win->winHide(FALSE);
  //			}
  //		}
  //		else
  //			win->winHide(TRUE);
  ...
  if (screenType == SCORESCREEN_SKIRMISH && player->isLocalPlayer()) {
    if (TheGameInfo->isSandbox() ||
      ...
      if (player->isPlayerActive()) {
        DEBUG_LOG((
            "Skipping skirmish stats update: sandbox:%d defeat:%d victory:%d\n",
            TheGameInfo->isSandbox(),
            TheVictoryConditions->isLocalAlliedDefeat(),
            TheVictoryConditions->isLocalAlliedVictory()));
        return;
      }
    ...
    if (TheVictoryConditions->isLocalAlliedVictory()) {
      ...
      stats.setBestWinStreak(
          max(stats.getBestWinStreak(), stats.getWinStreak()));
  ...
  if (screenType == SCORESCREEN_INTERNET) {
    ...
    if (localID) {
      ...
      if (player->isLocalPlayer()) {
        GameSpyGameSlot *localSlot =
            TheGameSpyGame->getGameSpySlot(localSlotNum);
        if (localSlot) {
          if (TheVictoryConditions->amIObserver()) {
            // nothing to track
            DEBUG_LOG(
                ("populatePlayerInfo() - not tracking stats for observer\n"));
            return;
          }
          PSPlayerStats stats =
              TheGameSpyPSMessageQueue->findPlayerStatsByID(localID);
          ...
          for (Int i = 0; i < MAX_SLOTS; ++i) {
            ...
            if (slot->isHuman()) {
              if (i != localSlotNum) {
                latestHumanInGame =
                    max(latestHumanInGame, slot->lastFrameInGame());
              }
            }
            if (slot->isAI()) {
              anyAI = TRUE;
          ...
          for (i = 0; i < MAX_SLOTS; ++i) {
            const GameSlot *slot = TheGameInfo->getConstSlot(i);
            DEBUG_LOG(
                ("latestHumanInGame=%d, slot->isOccupied()=%d, "
                 "slot->disconnected()=%d, slot->isAI()=%d, "
                 "slot->lastFrameInGame()=%d\n",
                 latestHumanInGame, slot->isOccupied(), slot->disconnected(),
                 slot->isAI(), slot->lastFrameInGame()));
            ...
            if (slot->isOccupied() && !slot->disconnected() &&
                i != localSlotNum &&
                (slot->isAI() ||
                 (slot->lastFrameInGame() >=
                  lastFrameOfGame /*TheGameLogic->getFrame()*/ - 1))) {
          ...
          if (!sawAnyDisconnects) {
            DEBUG_LOG(
                ("Didn't see any disconnects - making gameEndedInDisconnect == "
                 "FALSE\n"));
            gameEndedInDisconnect = FALSE;
          }
          ...
          if (gameEndedInDisconnect) {
            if (latestHumanInGame == TheNetwork->getPingFrame()) {
              // we pinged on the last frame someone was there - i.e. game ended
              // in a disconnect. check if we were to blame.
              if (TheNetwork->getPingsRecieved() <
                  max(1, TheNetwork->getPingsSent() /
                             2))  /// @todo: what's a good percent of pings to
                                  /// have gotten?
              {
                DEBUG_LOG(
                    ("We were to blame.  Leaving gameEndedInDisconnect = "
                     "true\n"));
              } else {
                DEBUG_LOG(
                    ("We were not to blame.  Changing gameEndedInDisconnect = "
                     "false\n"));
                gameEndedInDisconnect = FALSE;
              }
            } else {
              DEBUG_LOG(
                  ("gameEndedInDisconnect, and we didn't ping on last frame.  "
                   "What's up with that?\n"));
            }
          }
          ...
          if (TheVictoryConditions->isLocalAlliedDefeat() ||
              TheVictoryConditions->isLocalAlliedVictory()) {
            sawEndOfGame = TRUE;
          }
          ...
          // send ladder results (even if we end the game in the first N
          // seconds)
          if (TheGameSpyGame->getLadderPort() &&
              TheGameSpyGame->getLadderIP().isNotEmpty()) {
            ...
            gameResReq.results =
                TheGameSpyGame->generateLadderGameResultsPacket().str();
            DEBUG_ASSERTCRASH(TheGameResultsQueue,
                              ("No Game Results queue!\n"));
            if (TheGameResultsQueue) {
              TheGameResultsQueue->addRequest(gameResReq);
            }
          }

          if (TheVictoryConditions->getEndFrame() <
                  LOGICFRAMES_PER_SECOND * 25 &&
              TheVictoryConditions->getEndFrame()) {
            DEBUG_LOG(
                ("populatePlayerInfo() - not tracking stats for short game\n"));
            return;
          }

          // generate and send a gameres packet
          AsciiString resultsPacket =
              TheGameSpyGame->generateGameSpyGameResultsPacket();
          DEBUG_LOG(
              ("About to send results packet: %s\n", resultsPacket.str()));
          PSRequest grReq;
          ...
          DEBUG_LOG(("myTemplate = %X(%s)\n", myTemplate,
                     myTemplate->getName().str()));
          for (ptIdx = 0;
               ptIdx < ThePlayerTemplateStore->getPlayerTemplateCount();
               ++ptIdx) {
            const PlayerTemplate *nthTemplate =
                ThePlayerTemplateStore->getNthPlayerTemplate(ptIdx);
            DEBUG_LOG(("nthTemplate = %X(%s)\n", nthTemplate,
                       nthTemplate->getName().str()));
          ...
          if (stats.id == 0) {
            ...
            // what we'll do is just update our disconnects in the registry if
            // we disconnected. other than that, there's not much we can do.  :P
            if (gameEndedInDisconnect || TheNetwork->sawCRCMismatch()) {
              ...
              */
              DEBUG_LOG(
                  ("populatePlayerInfo() - need to save off info for "
                   "disconnect games!\n"));
            ...
            DEBUG_CRASH(
                ("populatePlayerInfo() - not tracking stats - we haven't "
                 "gotten the original stuff yet\n"));
            return;
          }
          ...
          if (TheNetwork->sawCRCMismatch()) {
            ...
            stats.maxDesyncsInARow =
                max(stats.desyncsInARow, stats.maxDesyncsInARow);
          } else if (gameEndedInDisconnect) {
            stats.lossesInARow = 0;
            ...
            stats.maxDisconsInARow =
                max(stats.disconsInARow, stats.maxDisconsInARow);
            ...
            stats.maxLossesInARow =
                max(stats.lossesInARow, stats.maxLossesInARow);
          }

          stats.earnings[ptIdx] += s->getTotalMoneyEarned();
          stats.duration[ptIdx] += TheGameLogic->getFrame() /
                                   LOGICFRAMES_PER_SECOND / 60;  // in minutes
          stats.games[ptIdx]++;
          // since we raise this number when game starts, we need to lower it
          // back on completion
          Int disCons;
          disCons = stats.discons[ptIdx];
          disCons -= 1;
          if (disCons >= 0) stats.discons[ptIdx] = disCons;

          stats.gamesAsRandom +=
              (localSlot->getOriginalPlayerTemplate() == PLAYERTEMPLATE_RANDOM);

          if (stats.lastGeneral != ptIdx) stats.gamesInRowWithLastGeneral = 0;
          stats.gamesInRowWithLastGeneral++;
          stats.lastGeneral = ptIdx;

          Int gameSize = 0;
          for (i = 0; i < MAX_SLOTS; ++i) {
            if (TheGameSpyGame->getConstSlot(i)->isOccupied() &&
                TheGameSpyGame->getConstSlot(i)->getPlayerTemplate() !=
          ...
          stats.lastFPS =
              TheDisplay->getAverageFPS();  ///@todo: need something more than
                                            ///this, really. :(

          stats.surrenders[ptIdx] += TheGameInfo->haveWeSurrendered() ||
                                     !TheVictoryConditions->getEndFrame();
          ...
          const ThingTemplate *pTemplate =
              TheThingFactory->findTemplate("GLAScudStorm");
          if (s->getTotalObjectsBuilt(pTemplate) > 0) stats.builtSCUD = 1;

          pTemplate =
              TheThingFactory->findTemplate("AmericaParticleCannonUplink");
          if (s->getTotalObjectsBuilt(pTemplate) > 0)
            stats.builtParticleCannon = 1;

          pTemplate =
              TheThingFactory->findTemplate("ChinaNuclearMissileLauncher");
          if (s->getTotalObjectsBuilt(pTemplate) > 0) stats.builtNuke = 1;
          ...
          mPref.setCachedStats(
              GameSpyPSMessageQueueInterface::formatPlayerKVPairs(stats)
                  .c_str());
          mPref.write();
        }
      }
...
/** We Grab information about the players differently in Multiplayer.  We only
   want the players listed in the slots */
//-------------------------------------------------------------------------------------------------
void grabMultiPlayerInfo(void) {
  ...
  ScoreMapIt it;
  scores.clear();
  Int adder = 1;  // Varible used to add on an offset to the score to make sure
                  // we don't add people to the same map

  player = ThePlayerList->getLocalPlayer();
  if (player) {
    const Image *image =
        TheMappedImageCollection->findImageByName("MutiPlayer_ScoreScreen");
    if (image) {
  ...
  // Add each player and score to the map. THis allows us to sort the players
  // based on score.
  for (Int i = 0; i < MAX_SLOTS; ++i) {
  ...
  DEBUG_ASSERTCRASH(count == playerCount,
                    ("For some reason we added %d players to the scores map, "
                     "but only read %d",
                     playerCount, count));
}
...
void grabSinglePlayerInfo(void) {
  ...
  AsciiString side;
  // okay, there's all kinds of hard coding going on here.  THe reason why!
  // well, We have no way of telling what sides we have in the game.  Hence, the
  // hardcoding.
    ...
    for (Int i = 0; i < MAX_PLAYER_COUNT; ++i) {
      ...
      if (player && player != localPlayer &&
        ...
        if ((TheGameLogic->isInSinglePlayerGame() == FALSE) ||
          ...
          if ((isFriend == TRUE && localPlayer->getRelationship(
                                       player->getDefaultTeam()) == ALLIES) ||
              (isFriend == FALSE && localPlayer->getRelationship(
                                        player->getDefaultTeam()) == ENEMIES)) {
...
winName.format("ScoreScreen.wnd:StaticTextScore%d", i);
    win =  TheWindowManager->winGetWindowFromId( parent,
TheNameKeyGenerator->nameToKey( winName ) ); DEBUG_ASSERTCRASH(win,("Could not
find window %s on the score screen", winName.str())); win->winHide(TRUE);
*/
    ...
    //		win =  TheWindowManager->winGetWindowFromId( parent,
    //TheNameKeyGenerator->nameToKey( winName ) ); 		DEBUG_ASSERTCRASH(win,("Could
    //not find window %s on the score screen", winName.str()));
    //		win->winHide(TRUE);
  }
}
...
*/
  ...
  ////	else
  ////		win->winHide(TRUE);
  //	if(screenType ==	SCORESCREEN_INTERNET && TheGameSpyInfo &&
  //TheGameSpyInfo->getLocalProfileID() != 0)
  //	{
  //		// Get the stats for the player
  //
...
*/
  ...
  //TheNameKeyGenerator->nameToKey( winName ) ); 	DEBUG_ASSERTCRASH(win,("Could
  //not find window %s on the score screen", winName.str())); 	if(screenType ==
  //SCORESCREEN_INTERNET) 		win->winHide(FALSE); 	else 		win->winHide(TRUE);
}
