...
static void doGameStart(void) {
  ...
  // TheScriptEngine->setGlobalDifficulty(s_AIDiff); // CANNOT DO THIS! REPLAYS
  // WILL BREAK!!!
  ...
  /// @todo: when Campaign & skirmish are separated, make campaign have fixed
  /// seed and skirmish random.
  InitRandom(0);
  /*
    ...
    switch (TheScriptEngine->getGlobalDifficulty()) {
      case DIFFICULTY_EASY: {
        NameKeyType radioButtonEasyAIID = TheNameKeyGenerator->nameToKey(
            AsciiString("MapSelectMenu.wnd:RadioButtonEasyAI"));
        GameWindow *radioButtonEasyAI =
            TheWindowManager->winGetWindowFromId(parent, radioButtonEasyAIID);
      ...
      case DIFFICULTY_NORMAL: {
        NameKeyType radioButtonMediumAIID = TheNameKeyGenerator->nameToKey(
            AsciiString("MapSelectMenu.wnd:RadioButtonMediumAI"));
        GameWindow *radioButtonMediumAI =
            TheWindowManager->winGetWindowFromId(parent, radioButtonMediumAIID);
      ...
      case DIFFICULTY_HARD: {
        NameKeyType radioButtonHardAIID = TheNameKeyGenerator->nameToKey(
            AsciiString("MapSelectMenu.wnd:RadioButtonHardAI"));
        GameWindow *radioButtonHardAI =
...
void MapSelectMenuInit(WindowLayout *layout, void *userData) {
  ...
  NameKeyType buttonBackID = TheNameKeyGenerator->nameToKey(
      AsciiString("MapSelectMenu.wnd:ButtonBack"));
  GameWindow *buttonBack =
      TheWindowManager->winGetWindowFromId(NULL, buttonBackID);

  NameKeyType buttonOKID =
      TheNameKeyGenerator->nameToKey(AsciiString("MapSelectMenu.wnd:ButtonOK"));
  ...
  TheShell->registerWithAnimateManager(buttonBack, WIN_ANIMATION_SLIDE_RIGHT,
                                       TRUE, 0);
  TheShell->registerWithAnimateManager(buttonOK, WIN_ANIMATION_SLIDE_LEFT, TRUE,
                                       0);

  SetDifficultyRadioButton();

  radioButtonSystemMapsID =
      TheNameKeyGenerator->nameToKey("MapSelectMenu.wnd:RadioButtonSystemMaps");
  radioButtonUserMapsID =
      TheNameKeyGenerator->nameToKey("MapSelectMenu.wnd:RadioButtonUserMaps");
...
void MapSelectMenuUpdate(WindowLayout *layout, void *userData) {
  if (startGame && TheShell->isAnimFinished()) doGameStart();

  // We'll only be successful if we've requested to
  if (isShuttingDown && TheShell->isAnimFinished()) shutdownComplete(layout);
  ...
  switch (msg) {
    ...
    case GWM_CREATE: {
      ...
      buttonOK = TheNameKeyGenerator->nameToKey(
          AsciiString("MapSelectMenu.wnd:ButtonOK"));
      listboxMap = TheNameKeyGenerator->nameToKey(
          AsciiString("MapSelectMenu.wnd:ListboxMap"));
      radioButtonEasyAI = TheNameKeyGenerator->nameToKey(
          AsciiString("MapSelectMenu.wnd:RadioButtonEasyAI"));
      radioButtonMediumAI = TheNameKeyGenerator->nameToKey(
          AsciiString("MapSelectMenu.wnd:RadioButtonMediumAI"));
      radioButtonHardAI = TheNameKeyGenerator->nameToKey(
          AsciiString("MapSelectMenu.wnd:RadioButtonHardAI"));
      break;
    ...
    case GBM_SELECTED: {
      ...
      static NameKeyType singlePlayerID =
          NAMEKEY("MapSelectMenu.wnd:ButtonSinglePlayer");
      static NameKeyType multiplayerID =
          NAMEKEY("MapSelectMenu.wnd:ButtonMultiplayer");
      if (controlID == singlePlayerID) {
        ...
        GameWindow *mapWindow =
            TheWindowManager->winGetWindowFromId(NULL, listboxMap);
        ...
        if (selected != -1) {
          ...
          if (mapFname) setupGameStart(mapFname);
        }  // end if
    ...
    case GLM_DOUBLE_CLICKED: {
      ...
      if (controlID == listboxMap) {
        ...
        if (rowSelected >= 0) {
          ...
          NameKeyType buttonOKID = TheNameKeyGenerator->nameToKey(
              AsciiString("MapSelectMenu.wnd:ButtonOK"));
          GameWindow *buttonOK =
              TheWindowManager->winGetWindowFromId(NULL, buttonOKID);
