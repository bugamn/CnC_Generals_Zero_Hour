...
extern void MapSelectorTooltip(GameWindow *window, WinInstanceData *instData,
                               UnsignedInt mouse);
...
void SendStatsToOtherPlayers(const GameInfo *game) {
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (slot->isHuman() && i != localIndex) {
      ...
      DEBUG_LOG(
          ("SendStatsToOtherPlayers() - sending to '%s', data of\n\t'%s'\n",
...
static NameKeyType buttonAcceptID[MAX_SLOTS] = {
    NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID,
    NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID};
...
static NameKeyType buttonMapStartPositionID[MAX_SLOTS] = {
    NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID,
    NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID};
static NameKeyType genericPingWindowID[MAX_SLOTS] = {
    NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID,
    NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID};
...
void PopBackToLobby(void) {
  ...
  if (TheGameSpyInfo)  // this can be blown away by a disconnect on the map
                       // transfer screen
  {
  ...
  DEBUG_LOG(
      ("PopBackToLobby() - parentWOLGameSetup is %X\n", parentWOLGameSetup));
...
void WOLPositionStartSpots(void) {
  ...
  if (WOLMapSelectLayout != NULL) {
    ...
    // get the controls.
    NameKeyType listboxMapID = TheNameKeyGenerator->nameToKey(
        AsciiString("WOLMapSelectMenu.wnd:ListboxMap"));
    GameWindow *listboxMap =
        TheWindowManager->winGetWindowFromId(NULL, listboxMapID);
    ...
    if (listboxMap != NULL) {
      ...
      if (selected != -1) {
        ...
        const char *mapFname =
            (const char *)GadgetListBoxGetItemData(listboxMap, selected);
    ...
    positionStartSpots(TheGameSpyInfo->getCurrentStagingRoom(),
                       buttonMapStartPosition, win);
  }
}
...
static void playerTooltip(GameWindow *window, WinInstanceData *instData,
                          UnsignedInt mouse) {
  Int slotIdx = -1;
  for (Int i = 0; i < MAX_SLOTS; ++i) {
  ...
  PlayerInfoMap::iterator pmIt =
      TheGameSpyInfo->getPlayerInfoMap()->find(aName);
  ...
  PSPlayerStats stats =
      TheGameSpyPSMessageQueue->findPlayerStatsByID(profileID);
  if (stats.id == 0) {
    TheMouse->setCursorTooltip(uName, -1, NULL, 1.5f);
    return;
  }
  ...
  playerInfo.format(TheGameText->fetch("TOOLTIP:StagingPlayerInfo"),
                    ...
                    slot->getPingAsInt(), totalWins, totalLosses, totalDiscons,
                    favoriteSide.str());
  ...
  if (isLocalPlayer) {
    ...
    // not us
    if (TheGameSpyInfo->getBuddyMap()->find(profileID) !=
  ...
  TheMouse->setCursorTooltip(
      tooltip, -1, NULL, 1.5f);  // the text and width are the only params used.
                                 // the others are the default values.
}

void gameAcceptTooltip(GameWindow *window, WinInstanceData *instData,
                       UnsignedInt mouse) {
  Int x, y;
  ...
  if ((x > winPosX && x < (winPosX + winWidth)) &&
      (y > winPosY && y < (winPosY + winHeight))) {
    TheMouse->setCursorTooltip(TheGameText->fetch("TOOLTIP:GameAcceptance"), -1,
                               NULL);
  }
}

void pingTooltip(GameWindow *window, WinInstanceData *instData,
                 UnsignedInt mouse) {
  Int x, y;
  x = LOLONGTOSHORT(mouse);
  y = HILONGTOSHORT(mouse);
  ...
  if ((x > winPosX && x < (winPosX + winWidth)) &&
      (y > winPosY && y < (winPosY + winHeight))) {
    TheMouse->setCursorTooltip(TheGameText->fetch("TOOLTIP:ConnectionSpeed"),
                               -1, NULL);
  }
}
...
static void handleColorSelection(int index) {
  ...
  if (myGame) {
    ...
    if (TheGameSpyInfo->amIHost()) {
      ...
      if (!slot->isPlayer(TheGameSpyInfo->getLocalName())) return;
...
static void handlePlayerTemplateSelection(int index) {
  ...
  if (myGame) {
    ...
    if (oldTemplate == PLAYERTEMPLATE_OBSERVER) {
      ...
      // is becoming observer, so populate color & team with random only, and
      // disable
      GadgetComboBoxSetSelectedPos(comboBoxColor[index], 0);
      GadgetComboBoxSetSelectedPos(comboBoxTeam[index], 0);
      slot->setStartPos(-1);
...
static void handleStartPositionSelection(Int player, int startPos) {
  ...
  if (myGame) {
    ...
    if (startPos == slot->getStartPos()) return;
    Bool skip = FALSE;
    if (startPos < 0) {
      skip = TRUE;
    }
...
static void StartPressed(void) {
  ...
  for (int i = 0; i < MAX_SLOTS; i++) {
    if ((myGame->getSlot(i)->isAccepted() == FALSE) &&
        (myGame->getSlot(i)->isHuman() == TRUE)) {
      isReady = FALSE;
      if (!myGame->getSlot(i)->hasMap() && !willTransfer) {
        UnicodeString msg;
        msg.format(TheGameText->fetch("GUI:PlayerNoMap"),
                   myGame->getSlot(i)->getName().str(), mapDisplayName.str());
        TheGameSpyInfo->addText(msg, GameSpyColor[GSCOLOR_DEFAULT],
                                listboxGameSetupChat);
        allHaveMap = FALSE;
      }
    }
    if (myGame->getSlot(i)->isOccupied() &&
        myGame->getSlot(i)->getPlayerTemplate() != PLAYERTEMPLATE_OBSERVER) {
      if (myGame->getSlot(i)->isHuman()) humanCount++;
      playerCount++;
    }
  }
  ...
  if (!md || md->m_numPlayers < playerCount) {
    if (myGame->amIHost()) {
      UnicodeString text;
      text.format(TheGameText->fetch("LAN:TooManyPlayers"),
                  (md) ? md->m_numPlayers : 0);
      TheGameSpyInfo->addText(text, GameSpyColor[GSCOLOR_DEFAULT],
                              listboxGameSetupChat);
    }
    return;
  }
  ...
  if (isReady) {
    ...
    GameWindow *buttonBuddy = TheWindowManager->winGetWindowFromId(
        NULL, NAMEKEY("GameSpyGameOptionsMenu.wnd:ButtonCommunicator"));
    ...
    *TheGameSpyGame = *myGame;
    ...
    TheGameSpyInfo->addText(TheGameText->fetch("GUI:NotifiedStartIntent"),
...
void WOLDisplayGameOptions(void) {
  ...
  if (!parentWOLGameSetup || !theGame) return;
  ...
  const MapMetaData *md =
      TheMapCache->findMap(TheGameSpyInfo->getCurrentStagingRoom()->getMap());
  ...
  updateMapStartSpots(TheGameSpyInfo->getCurrentStagingRoom(),
...
//-------------------------------------------------------------------------------------------------
void WOLDisplaySlotList(void) {
  if (!parentWOLGameSetup || !TheGameSpyInfo->getCurrentStagingRoom()) return;
  ...
  UpdateSlotList(game, comboBoxPlayer, comboBoxColor, comboBoxPlayerTemplate,
                 comboBoxTeam, buttonAccept, buttonStart,
                 buttonMapStartPosition);
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (slot && slot->isHuman()) {
      ...
      if (genericPingWindow[i]) genericPingWindow[i]->winHide(TRUE);
    }
  }
}
...
void InitWOLGameGadgets(void) {
  ...
  parentWOLGameSetupID = TheNameKeyGenerator->nameToKey(
      AsciiString("GameSpyGameOptionsMenu.wnd:GameSpyGameOptionsMenuParent"));
  buttonBackID = TheNameKeyGenerator->nameToKey(
      AsciiString("GameSpyGameOptionsMenu.wnd:ButtonBack"));
  buttonStartID = TheNameKeyGenerator->nameToKey(
      AsciiString("GameSpyGameOptionsMenu.wnd:ButtonStart"));
  textEntryChatID = TheNameKeyGenerator->nameToKey(
      AsciiString("GameSpyGameOptionsMenu.wnd:TextEntryChat"));
  textEntryMapDisplayID = TheNameKeyGenerator->nameToKey(
      AsciiString("GameSpyGameOptionsMenu.wnd:TextEntryMapDisplay"));
  listboxGameSetupChatID = TheNameKeyGenerator->nameToKey(AsciiString(
      "GameSpyGameOptionsMenu.wnd:ListboxChatWindowGameSpyGameSetup"));
  buttonEmoteID = TheNameKeyGenerator->nameToKey(
      AsciiString("GameSpyGameOptionsMenu.wnd:ButtonEmote"));
  buttonSelectMapID = TheNameKeyGenerator->nameToKey(
      AsciiString("GameSpyGameOptionsMenu.wnd:ButtonSelectMap"));
  windowMapID = TheNameKeyGenerator->nameToKey(
      AsciiString("GameSpyGameOptionsMenu.wnd:MapWindow"));
  windowMapSelectMapID = TheNameKeyGenerator->nameToKey(
      AsciiString("WOLMapSelectMenu.wnd:WinMapPreview"));

  NameKeyType staticTextTitleID =
      NAMEKEY("GameSpyGameOptionsMenu.wnd:StaticTextGameName");

  // Initialize the pointers to our gadgets
  parentWOLGameSetup =
      TheWindowManager->winGetWindowFromId(NULL, parentWOLGameSetupID);
  buttonEmote =
      TheWindowManager->winGetWindowFromId(parentWOLGameSetup, buttonEmoteID);
  buttonSelectMap = TheWindowManager->winGetWindowFromId(parentWOLGameSetup,
                                                         buttonSelectMapID);
  buttonStart =
      TheWindowManager->winGetWindowFromId(parentWOLGameSetup, buttonStartID);
  buttonBack =
      TheWindowManager->winGetWindowFromId(parentWOLGameSetup, buttonBackID);
  listboxGameSetupChat = TheWindowManager->winGetWindowFromId(
      parentWOLGameSetup, listboxGameSetupChatID);
  textEntryChat =
      TheWindowManager->winGetWindowFromId(parentWOLGameSetup, textEntryChatID);
  textEntryMapDisplay = TheWindowManager->winGetWindowFromId(
      parentWOLGameSetup, textEntryMapDisplayID);
  windowMap =
      TheWindowManager->winGetWindowFromId(parentWOLGameSetup, windowMapID);
  DEBUG_ASSERTCRASH(windowMap,
                    ("Could not find the parentWOLGameSetup.wnd:MapWindow"));
  ...
  GameWindow *staticTextTitle = TheWindowManager->winGetWindowFromId(
      parentWOLGameSetup, staticTextTitleID);
  if (staticTextTitle) {
  ...
  for (Int i = 0; i < MAX_SLOTS; i++) {
    ...
    comboBoxPlayer[i] = TheWindowManager->winGetWindowFromId(
        parentWOLGameSetup, comboBoxPlayerID[i]);
    ...
    staticTextPlayer[i] = TheWindowManager->winGetWindowFromId(
        parentWOLGameSetup, staticTextPlayerID[i]);
    staticTextPlayer[i]->winSetTooltipFunc(playerTooltip);
    if (TheGameSpyInfo->amIHost()) staticTextPlayer[i]->winHide(TRUE);

    if (i == 0 && TheGameSpyInfo->amIHost()) {
      UnicodeString uName;
      uName.translate(TheGameSpyInfo->getLocalName());
      GadgetComboBoxAddEntry(comboBoxPlayer[i], uName,
                             GameSpyColor[GSCOLOR_PLAYER_OWNER]);
      GadgetComboBoxSetSelectedPos(comboBoxPlayer[0], 0);
    } else {
      GadgetComboBoxAddEntry(comboBoxPlayer[i], TheGameText->fetch("GUI:Open"),
                             GameSpyColor[GSCOLOR_PLAYER_NORMAL]);
      GadgetComboBoxAddEntry(comboBoxPlayer[i],
                             TheGameText->fetch("GUI:Closed"),
                             GameSpyColor[GSCOLOR_PLAYER_NORMAL]);
      GadgetComboBoxAddEntry(comboBoxPlayer[i],
                             TheGameText->fetch("GUI:EasyAI"),
                             GameSpyColor[GSCOLOR_PLAYER_NORMAL]);
      GadgetComboBoxAddEntry(comboBoxPlayer[i],
                             TheGameText->fetch("GUI:MediumAI"),
                             GameSpyColor[GSCOLOR_PLAYER_NORMAL]);
      GadgetComboBoxAddEntry(comboBoxPlayer[i],
                             TheGameText->fetch("GUI:HardAI"),
                             GameSpyColor[GSCOLOR_PLAYER_NORMAL]);
    ...
    comboBoxColor[i] = TheWindowManager->winGetWindowFromId(parentWOLGameSetup,
                                                            comboBoxColorID[i]);
    ...
    comboBoxPlayerTemplate[i] = TheWindowManager->winGetWindowFromId(
        parentWOLGameSetup, comboBoxPlayerTemplateID[i]);
    ...
    buttonAccept[i] = TheWindowManager->winGetWindowFromId(parentWOLGameSetup,
                                                           buttonAcceptID[i]);
    DEBUG_ASSERTCRASH(buttonAccept[i],
                      ("Could not find the buttonAccept[%d]", i));
    ...
    genericPingWindow[i] = TheWindowManager->winGetWindowFromId(
        parentWOLGameSetup, genericPingWindowID[i]);
    DEBUG_ASSERTCRASH(genericPingWindow[i],
                      ("Could not find the genericPingWindow[%d]", i));
    genericPingWindow[i]->winSetTooltipFunc(pingTooltip);

    //		tmpString.format("GameSpyGameOptionsMenu.wnd:ButtonStartPosition%d",
    ...
    //parentWOLGameSetup, buttonStartPositionID[i] );
    ...
    buttonMapStartPosition[i] = TheWindowManager->winGetWindowFromId(
        parentWOLGameSetup, buttonMapStartPositionID[i]);
    ...
    if (i != 0 && buttonAccept[i]) buttonAccept[i]->winHide(TRUE);
  }

  if (buttonAccept[0]) buttonAccept[0]->winEnable(TRUE);

  if (buttonBack != NULL) {
    buttonBack->winEnable(TRUE);
  }
  // GadgetButtonSetEnabledColor(buttonAccept[0],
  // GameSpyColor[GSCOLOR_ACCEPT_TRUE]);
}
...
void WOLGameSetupMenuInit(WindowLayout *layout, void *userData) {
  if (TheGameSpyGame && TheGameSpyGame->isGameInProgress()) {
    ...
    if (TheGameSpyInfo->isDisconnectedAfterGameStart(&disconReason)) {
      ...
      DEBUG_LOG(
          ("WOLGameSetupMenuInit() - game was in progress, and we were "
           "disconnected, so pop immediate back to main menu\n"));
      TheShell->popImmediate();
      return;
    }

    // If we init while the game is in progress, we are really returning to the
    // menu after the game.  So, we pop the menu and go back to the lobby. Whee!
    DEBUG_LOG(
        ("WOLGameSetupMenuInit() - game was in progress, so pop immediate back "
         "to lobby\n"));
    TheShell->popImmediate();
  ...
  // The dialog needs to react differently depending on whether it's the host or
  // not.
  ...
  if (TheGameSpyInfo->amIHost()) {
    ...
    hostSlot->setNATBehavior((FirewallHelperClass::FirewallBehaviorType)
                                 natPref.getFirewallBehavior());
  ...
  initDone = true;
  TheGameSpyInfo->setGameOptions();
  // TheShell->registerWithAnimateManager(parentWOLGameSetup,
  // WIN_ANIMATION_SLIDE_TOP, TRUE);
  WOLPositionStartSpots();
...
static void shutdownComplete(WindowLayout *layout) {
  ...
  if (nextScreen != NULL) {
    if (!TheGameSpyPeerMessageQueue ||
        !TheGameSpyPeerMessageQueue->isConnected()) {
      DEBUG_LOG(
          ("GameSetup shutdownComplete() - skipping push because we're "
           "disconnected\n"));
    } else {
  ...
  if (TheShell->isAnimFinished() && !buttonPushed &&
      ...
      while (allowedMessages-- && !sawImportantMessage &&
        ...
        switch (resp.peerResponseType) {
          case PeerResponse::PEERRESPONSE_DISCONNECT: {
            ...
            NameKeyType listboxChatWindowScoreScreenID =
                NAMEKEY("ScoreScreen.wnd:ListboxChatWindowScoreScreen");
            GameWindow *listboxChatWindowScoreScreen =
                TheWindowManager->winGetWindowFromId(
                    NULL, listboxChatWindowScoreScreenID);
            if (listboxChatWindowScoreScreen) {
              GadgetListBoxAddEntryText(listboxChatWindowScoreScreen,
                                        TheGameText->fetch(disconMunkee),
                                        GameSpyColor[GSCOLOR_DEFAULT], -1);
            } else {
              // still ingame
              TheInGameUI->message(disconMunkee);
            }
            TheGameSpyInfo->markAsDisconnectedAfterGameStart(
                resp.discon.reason);
          }
        }
      }

      return;  // if we're in game, all we care about is if we've been
               // disconnected from the chat server
    }

    Bool isHosting =
        TheGameSpyInfo->amIHost();  // only while in game setup screen
    isHosting = isHosting || (TheGameSpyGame && TheGameSpyGame->isInGame() &&
                              TheGameSpyGame->amIHost());  // while in game
    if (!isHosting && !lastSlotlistTime && timeGetTime() > enterTime + 10000) {
      ...
      if (TheGameSpyPeerMessageQueue->isConnected()) {
        ...
        if (TheGameSpyGame) TheGameSpyGame->reset();
        TheGameSpyInfo->leaveStagingRoom();
        // TheGameSpyInfo->joinBestGroupRoom();
        GSMessageBoxOk(TheGameText->fetch("GUI:HostLeftTitle"),
                       TheGameText->fetch("GUI:HostLeft"));
        nextScreen = "Menus/WOLCustomLobby.wnd";
        TheShell->pop();
      }
      return;
    ...
    if (TheNAT != NULL) {
      ...
      if (NATState == NATSTATE_DONE) {
        ...
        // TheShell->pop();
        TheGameSpyGame->launchGame();
        if (TheGameSpyInfo)  // this can be blown away by a disconnect on the
                             // map transfer screen
          TheGameSpyInfo->leaveStagingRoom();
        return;
        ...
        GSMessageBoxOk(TheGameText->fetch("GUI:Error"),
                       TheGameText->fetch("GUI:NATNegotiationFailed"));
        nextScreen = "Menus/WOLCustomLobby.wnd";
        TheShell->pop();
        return;
      }
    ...
    while (allowedMessages-- && !sawImportantMessage) {
      ...
      switch (resp.peerResponseType) {
        case PeerResponse::PEERRESPONSE_FAILEDTOHOST: {
          ...
          TheGameSpyInfo->addText(TheGameText->fetch("GUI:GSFailedToHost"),
                                  GameSpyColor[GSCOLOR_DEFAULT], NULL);
        } break;
        case PeerResponse::PEERRESPONSE_GAMESTART: {
          sawImportantMessage = TRUE;
          GameSpyStagingRoom *myGame = TheGameSpyInfo->getCurrentStagingRoom();
          if (!myGame || !myGame->isInGame()) break;

          if (!TheGameSpyGame) break;
        ...
        case PeerResponse::PEERRESPONSE_PLAYERJOIN: {
          ...
          if (p.m_profileID) {
            if (TheGameSpyPSMessageQueue->findPlayerStatsByID(p.m_profileID)
                    .id == 0) {
              PSRequest req;
            ...
            if (TheNAT) {
              ...
              for (Int player = 0; player < MAX_SLOTS; ++player) {
                if (game->getSlot(player)->isOccupied() &&
                    game->getSlot(player)->getPlayerTemplate() !=
              ...
              if (openSlotIndex >= 0) {
                ...
                game->resetAccepted();  // BGC - need to unaccept everyone if
        ...
        case PeerResponse::PEERRESPONSE_PLAYERLEFT: {
          ...
          if (TheNAT ==
              NULL)  // don't update slot list if we're trying to start a game
          {
            GameInfo *game = TheGameSpyInfo->getCurrentStagingRoom();
        ...
        case PeerResponse::PEERRESPONSE_MESSAGE: {
          TheGameSpyInfo->addChat(resp.nick.c_str(), resp.message.profileID,
                                  ...
                                  !resp.message.isPrivate,
                                  resp.message.isAction, listboxGameSetupChat);
        } break;
        ...
        case PeerResponse::PEERRESPONSE_ROOMUTM: {
          ...
          if (g_debugSlots) {
            DEBUG_LOG(
                ("About to process a room UTM.  Command is '%s', command "
                 "options is '%s'\n",
          ...
          if (!strcmp(resp.command.c_str(), "SL")) {
            // slotlist
            GameSpyStagingRoom *game = TheGameSpyInfo->getCurrentStagingRoom();
            Bool isValidSlotList =
                game && game->getSlot(0) &&
                game->getSlot(0)->isPlayer(resp.nick.c_str()) &&
                !TheGameSpyInfo->amIHost();
            if (!isValidSlotList) {
              ...
              if (!game) {
                ...
                if (!game->getSlot(0)) {
                  ...
                  if (TheGameSpyInfo->amIHost()) {
                    ...
                    SLOTLIST_DEBUG_LOG(
                        ("Not from the host!  isHuman:%d, name:'%ls', "
                         "sender:'%s'\n",
                         game->getSlot(0)->isHuman(),
                         game->getSlot(0)->getName().str(), resp.nick.c_str()));
                  }
                }
              }
            ...
            {
              Int oldLocalSlotNum =
                  (game->isInGame()) ? game->getLocalSlotNum() : -1;
              ...
              if (TheNAT) {
                for (i = 0; i < MAX_SLOTS; ++i) {
                  if (game && game->getSlot(i)) {
                    ...
                    DEBUG_ASSERTLOG(
                        newIP == ips[i],
                        ("IP was different for player %d (%X --> %X)\n", i,
                         ips[i], newIP));
                    DEBUG_ASSERTLOG(
                        newPort == ports[i],
                        ("Port was different for player %d (%d --> %d)\n", i,
                         ports[i], newPort));
#endif
              ...
              if (!optionsOK) {
                ...
                SLOTLIST_DEBUG_LOG(
                    ("Options are good, local slot is %d\n", newLocalSlotNum));
                if (!isInGame) {
              ...
              if (isInGame) {
                ...
                if ((oldMapCRC ^ newMapCRC) || (!wasInGame && isInGame)) {
                  ...
                  UnicodeString hostName =
                      TheGameSpyInfo->getCurrentStagingRoom()
                          ->getSlot(0)
                          ->getName();
                  ...
                  req.options =
                      (game->getSlot(newLocalSlotNum)->hasMap()) ? "1" : "0";
                  TheGameSpyPeerMessageQueue->addRequest(req);
                  if (!game->getSlot(newLocalSlotNum)->hasMap()) {
                    UnicodeString text;
                    UnicodeString mapDisplayName;
                    const MapMetaData *mapData =
                        TheMapCache->findMap(game->getMap());
                    Bool willTransfer = TRUE;
                    if (mapData) {
                      mapDisplayName.format(L"%ls",
                                            mapData->m_displayName.str());
                      willTransfer = !mapData->m_isOfficial;
                    } else {
                      mapDisplayName.format(
                          L"%hs",
                          TheGameState->getMapLeafName(game->getMap()).str());
                    ...
                    if (willTransfer)
                      ...
                      text.format(TheGameText->fetch("GUI:LocalPlayerNoMap"),
                                  mapDisplayName.str());
                    TheGameSpyInfo->addText(text, GameSpyColor[GSCOLOR_DEFAULT],
                                            listboxGameSetupChat);
                  }
                ...
                if (lastSlotlistTime) {
                  ...
                  buttonPushed = true;
                  DEBUG_LOG(
                      ("Can't see ourselves in slotlist %s\n", options.str()));
                  TheGameSpyInfo->getCurrentStagingRoom()->reset();
                  TheGameSpyInfo->leaveStagingRoom();
                  // TheGameSpyInfo->joinBestGroupRoom();
                  GSMessageBoxOk(TheGameText->fetch("GUI:GSErrorTitle"),
                                 TheGameText->fetch("GUI:GSKicked"));
                  nextScreen = "Menus/WOLCustomLobby.wnd";
                  TheShell->pop();
                }
              }
            ...
            // host wants to start
            GameInfo *game = TheGameSpyInfo->getCurrentStagingRoom();
            if (game && game->isInGame() && game->getSlot(0) &&
                game->getSlot(0)->isPlayer(resp.nick.c_str())) {
              Int slotNum = game->getLocalSlotNum();
              GameSlot *slot = game->getSlot(slotNum);
              if (slot && (slot->isAccepted() == false)) {
                TheGameSpyInfo->addText(
                    TheGameText->fetch("GUI:HostWantsToStart"),
                    GameSpyColor[GSCOLOR_DEFAULT], listboxGameSetupChat);
              }
            }
            ...
            if (!TheGameSpyInfo->amIHost()) {
              ...
              AsciiString token;
              for (Int i = 0; i < MAX_SLOTS; ++i) {
                GameSpyGameSlot *slot =
                    TheGameSpyInfo->getCurrentStagingRoom()->getGameSpySlot(i);
        ...
        case PeerResponse::PEERRESPONSE_PLAYERUTM: {
          ...
          if (!strcmp(resp.command.c_str(), "STATS")) {
            PSPlayerStats stats = TheGameSpyPSMessageQueue->parsePlayerKVPairs(
                resp.commandOptions.c_str());
            if (stats.id &&
                (TheGameSpyPSMessageQueue->findPlayerStatsByID(stats.id).id ==
                 0))
          ...
          if (game) {
            ...
            if ((slotNum >= 0) && (slotNum < MAX_SLOTS) &&
              ...
              if (TheNAT != NULL) {
                TheNAT->processGlobalMessage(slotNum,
                                             resp.commandOptions.c_str());
              }
            }
            if (slotNum == 0 && !TheGameSpyInfo->amIHost()) {
              ...
              if (!strcmp(resp.command.c_str(), "accept")) {
                ...
                if (!hasMap) {
                  ...
                  if (willTransfer)
                    text.format(
                        ...
                        game->getSlot(slotNum)->getName().str(),
                        mapDisplayName.str());
                  else
                ...
                if (key == "Color") {
                  if (val >= -1 &&
                      val < TheMultiplayerSettings->getNumColors() &&
                      val != slot->getColor() &&
                      slot->getPlayerTemplate() != PLAYERTEMPLATE_OBSERVER) {
                    Bool colorAvailable = TRUE;
                    if (val != -1) {
                    ...
                    if (colorAvailable) slot->setColor(val);
                    change = true;
                  } else {
                  ...
                  if (val >= -1 && val < MAX_SLOTS &&
                      val != slot->getStartPos() &&
                      slot->getPlayerTemplate() != PLAYERTEMPLATE_OBSERVER) {
                    Bool startPosAvailable = TRUE;
                    if (val != -1) {
                      for (Int i = 0; i < MAX_SLOTS; i++) {
                        GameSlot *checkSlot = game->getSlot(i);
                        if (val == checkSlot->getStartPos() &&
                            slot != checkSlot) {
                    ...
                    if (startPosAvailable) slot->setStartPos(val);
                    change = true;
                    shouldUnaccept = true;
                  } else {
                  ...
                  if (val >= -1 && val < MAX_SLOTS / 2 &&
                      val != slot->getTeamNumber() &&
                      slot->getPlayerTemplate() != PLAYERTEMPLATE_OBSERVER) {
                  ...
                  if (uVal != slot->getIP()) {
                    DEBUG_LOG(
                        ("setting IP of player %ls from 0x%08x to be 0x%08x",
                         slot->getName().str(), slot->getIP(), uVal));
                    slot->setIP(uVal);
                    change = true;
                    shouldUnaccept = true;
                  } else {
                  ...
                  if ((val >= FirewallHelperClass::FIREWALL_MIN) &&
                    ...
                    slot->setNATBehavior(
                        (FirewallHelperClass::FirewallBehaviorType)val);
                    DEBUG_LOG(("Setting NAT behavior to %d for player %d\n",
                               val, slotNum));
                    change = true;
                  } else {
                    DEBUG_LOG(
                        ("Rejecting invalid NAT behavior %d from player %d\n",
                         val, slotNum));
                  }
                } else if (key == "Ping") {
                ...
                if (change) {
                  if (shouldUnaccept) game->resetAccepted();

                  TheGameSpyInfo->setGameOptions();

                  WOLDisplaySlotList();
                  DEBUG_LOG(
                      ("Slot value is color=%d, PlayerTemplate=%d, "
                       "startPos=%d, team=%d, IP=0x%8.8X\n",
...
WindowMsgHandledType WOLGameSetupMenuInput(GameWindow *window, UnsignedInt msg,
  ...
  (NameKeyType)control->winGetWindowId(); DEBUG_LOG(("GWM_RIGHT_UP for control
  ...
  ----------------------------------------------------------------------------------------
                          ...
                          {
                                  //
                                  // send a simulated selected event to the
  parent window of the
                                  // back/exit button
                                  //
                                  if( BitTest( state, KEY_STATE_UP ) )
                                  {
                                          TheWindowManager->winSendSystemMsg(
  window, GBM_SELECTED, (WindowMsgData)buttonBack, buttonBackID ); }  // end if
  ...
  */
  return MSG_IGNORED;
}  // WindowMsgHandledType WOLGameSetupMenuInput( GameWindow *window,
   // UnsignedInt msg,
...
Bool handleGameSetupSlashCommands(UnicodeString uText) {
  ...
  if (token == "host") {
    UnicodeString s;
    s.format(L"Hosting qr2:%d thread:%d", getQR2HostingStatus(),
             isThreadHosting);
    ...
    TheGameSpyInfo->addText(UnicodeString(L"Toggled SlotList debug"),
                            GameSpyColor[GSCOLOR_DEFAULT], NULL);
    return TRUE;  // was a slash command
...
static Int getFirstSelectablePlayer(const GameInfo *game) {
  ...
  if (!game->amIHost() ||
      slot && slot->getPlayerTemplate() != PLAYERTEMPLATE_OBSERVER)
    return game->getLocalSlotNum();

  for (Int i = 0; i < MAX_SLOTS; ++i) {
    slot = game->getConstSlot(i);
    if (slot && slot->isAI()) return i;
  }
...
WindowMsgHandledType WOLGameSetupMenuSystem(GameWindow *window, UnsignedInt msg,
  ...
  switch (msg) {
    ...
    case GCM_SELECTED: {
      ...
      for (Int i = 0; i < MAX_SLOTS; i++) {
        if (controlID == comboBoxColorID[i]) {
          ...
          if (i == myGame->getLocalSlotNum()) break;
          // Get
          Int pos = -1;
            ...
            if (myGame->getSlot(i)->getState() == SLOT_PLAYER) {
              ...
              if (wasAI ^ isAI)
                PopulatePlayerTemplateComboBox(
                    ...
                    wasAI && myGame->getAllowObservers());
              TheGameSpyInfo->setGameOptions();
              WOLDisplaySlotList();
            }
          }
    ...
    case GBM_SELECTED: {
      ...
      if (controlID == buttonBackID) {
        ...
        if (!txtInput.isEmpty())
          TheGameSpyInfo->sendChat(
              txtInput, FALSE, NULL);  // 'emote' button is now carriage-return
        ...
        if (TheGameSpyInfo->amIHost()) {
          ...
          GameSlot *localSlot =
              TheGameSpyInfo->getCurrentStagingRoom()->getSlot(
                  TheGameSpyInfo->getCurrentStagingRoom()->getLocalSlotNum());
          if (localSlot) {
            localSlot->setAccept();
          }
          UnicodeString hostName =
              TheGameSpyInfo->getCurrentStagingRoom()->getSlot(0)->getName();
          AsciiString asciiName;
          asciiName.translate(hostName);
          PeerRequest req;
          req.peerRequestType = PeerRequest::PEERREQUEST_UTMPLAYER;
        ...
        for (Int i = 0; i < MAX_SLOTS; i++) {
          if (controlID == buttonMapStartPositionID[i]) {
            ...
            if (playerIdxInPos >= 0) {
              ...
              if (nextPlayer < 0) nextPlayer = getFirstSelectablePlayer(game);
              handleStartPositionSelection(nextPlayer, i);
            }
          }
        }
  ...
  return MSG_HANDLED;
}  // WindowMsgHandledType WOLGameSetupMenuSystem( GameWindow *window,
   // UnsignedInt msg,
