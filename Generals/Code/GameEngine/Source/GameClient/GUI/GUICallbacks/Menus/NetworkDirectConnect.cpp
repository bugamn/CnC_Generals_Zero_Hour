...
void UpdateRemoteIPList() {
  ...
  Int numFields = swscanf(newIP.str(), L"%d.%d.%d.%d", &(n1[0]), &(n1[1]),
                          &(n1[2]), &(n1[3]));
  ...
  for (Int i = 0; i < numEntries; ++i) {
    if (i != currentSelection) {
      ...
      if (currentSelection == -1) {
        ...
        swscanf(oldIP.str(), L"%d.%d.%d.%d", &(n2[0]), &(n2[1]), &(n2[2]),
                &(n2[3]));
        ...
        // check to see if this is a duplicate or if this is not a properly
...
void HostDirectConnectGame() {
  ...
  localIPString.format(L"%d.%d.%d.%d", localIP >> 24,
                       (localIP & 0xff0000) >> 16, (localIP & 0xff00) >> 8,
                       localIP & 0xff);
  ...
  while (name.getLength() > g_lanPlayerNameLength) name.removeLastChar();
...
void JoinDirectConnectGame() {
  ...
  DEBUG_LOG(
      ("JoinDirectConnectGame - joining at %d.%d.%d.%d\n", ip1, ip2, ip3, ip4));
...
void NetworkDirectConnectInit(WindowLayout *layout, void *userData) {
  ...
  buttonBackID = TheNameKeyGenerator->nameToKey(
      AsciiString("NetworkDirectConnect.wnd:ButtonBack"));
  buttonHostID = TheNameKeyGenerator->nameToKey(
      AsciiString("NetworkDirectConnect.wnd:ButtonHost"));
  buttonJoinID = TheNameKeyGenerator->nameToKey(
      AsciiString("NetworkDirectConnect.wnd:ButtonJoin"));
  editPlayerNameID = TheNameKeyGenerator->nameToKey(
      AsciiString("NetworkDirectConnect.wnd:EditPlayerName"));
  comboboxRemoteIPID = TheNameKeyGenerator->nameToKey(
      AsciiString("NetworkDirectConnect.wnd:ComboboxRemoteIP"));
  staticLocalIPID = TheNameKeyGenerator->nameToKey(
      AsciiString("NetworkDirectConnect.wnd:StaticLocalIP"));
  ...
  comboboxRemoteIP =
      TheWindowManager->winGetWindowFromId(NULL, comboboxRemoteIPID);
  ...
  if (TheLAN == NULL) {
    //		DEBUG_ASSERTCRASH(TheLAN != NULL, ("TheLAN is null initializing
    //the direct connect screen."));
    TheLAN = NEW LANAPI();
  ...
  ipstr.format(L"%d.%d.%d.%d", ip >> 24, (ip & 0xff0000) >> 16,
               (ip & 0xff00) >> 8, ip & 0xff);
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType NetworkDirectConnectInput(GameWindow *window,
                                               UnsignedInt msg,
