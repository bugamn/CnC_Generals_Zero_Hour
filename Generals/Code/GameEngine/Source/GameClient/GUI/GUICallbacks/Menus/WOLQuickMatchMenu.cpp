...
static GameWindow *comboBoxDisabledLadder =
    NULL;  // enable and disable this, but never use it.  it is a stand-in for
           // comboBoxLadder for when there are no ladders
...
static Bool isInfoShown(void) {
  static NameKeyType parentStatsID =
      NAMEKEY("WOLQuickMatchMenu.wnd:ParentStats");
  GameWindow *parentStats =
      TheWindowManager->winGetWindowFromId(parentWOLQuickMatch, parentStatsID);
  if (parentStats) return !parentStats->winIsHidden();
  return FALSE;
}
...
static void hideOptionsGadgets(Bool doIt) {
  static NameKeyType parentOptionsID =
      NAMEKEY("WOLQuickMatchMenu.wnd:ParentOptions");
  GameWindow *parentOptions = TheWindowManager->winGetWindowFromId(
      parentWOLQuickMatch, parentOptionsID);
  if (parentOptions) {
    parentOptions->winHide(doIt);
    if (comboBoxSide) comboBoxSide->winHide(doIt);
    if (comboBoxColor) comboBoxColor->winHide(doIt);
    if (comboBoxNumPlayers) comboBoxNumPlayers->winHide(doIt);
    if (comboBoxLadder) comboBoxLadder->winHide(doIt);
    if (comboBoxDisabledLadder) comboBoxDisabledLadder->winHide(doIt);
    if (comboBoxMaxPing) comboBoxMaxPing->winHide(doIt);
    if (comboBoxMaxDisconnects) comboBoxMaxDisconnects->winHide(doIt);
  }
}
...
static void enableOptionsGadgets(Bool doIt) {
  ...
  if (parentOptions) {
    ...
    if (comboBoxColor) comboBoxColor->winEnable(doIt);
    if (comboBoxNumPlayers) comboBoxNumPlayers->winEnable(doIt);
    if (comboBoxLadder) comboBoxLadder->winEnable(doIt);
    if (comboBoxDisabledLadder) comboBoxDisabledLadder->winEnable(FALSE);
    if (comboBoxMaxPing) comboBoxMaxPing->winEnable(doIt);
    if (comboBoxMaxDisconnects) comboBoxMaxDisconnects->winEnable(doIt);
  }
}
...
static Int MAX_DISCONNECTS[MAX_DISCONNECTS_COUNT] = {
    MAX_DISCONNECTS_ANY, MAX_DISCONNECTS_5, MAX_DISCONNECTS_10,
    MAX_DISCONNECTS_25, MAX_DISCONNECTS_50};

void UpdateStartButton(void) {
  if (!comboBoxLadder || !buttonStart || !listboxMapSelect) return;
...
static void populateQMColorComboBox(QuickMatchPreferences &pref) {
  ...
  MultiplayerColorDefinition *def =
      TheMultiplayerSettings->getColor(PLAYERTEMPLATE_RANDOM);
  Int newIndex = GadgetComboBoxAddEntry(
      comboBoxColor, TheGameText->fetch("GUI:???"), def->getColor());
  ...
  for (Int c = 0; c < numColors; ++c) {
    ...
    newIndex =
        GadgetComboBoxAddEntry(comboBoxColor, colorName, def->getColor());
...
static void populateQMSideComboBox(Int favSide, const LadderInfo *li = NULL) {
  ...
  MultiplayerColorDefinition *def =
      TheMultiplayerSettings->getColor(PLAYERTEMPLATE_RANDOM);
  Int newIndex = GadgetComboBoxAddEntry(
      comboBoxSide, TheGameText->fetch("GUI:Random"), def->getColor());
  GadgetComboBoxSetItemData(comboBoxSide, newIndex,
  ...
  for (Int c = 0; c < numPlayerTemplates; ++c) {
    ...
    if (fac->getStartingBuilding().isEmpty()) continue;

    AsciiString side;
    side.format("SIDE:%s", fac->getSide().str());
    if (seenSides.find(side) != seenSides.end()) continue;
    ...
    newIndex = GadgetComboBoxAddEntry(comboBoxSide, TheGameText->fetch(side),
                                      def->getColor());
    GadgetComboBoxSetItemData(comboBoxSide, newIndex, (void *)c);

    if (c == favSide) entryToSelect = newIndex;
  }
  seenSides.clear();
...
void PopulateQMLadderListBox(GameWindow *win) {
  if (!parentWOLQuickMatch || !comboBoxLadder) return;
  ...
  index = GadgetListBoxAddEntryText(win, TheGameText->fetch("GUI:NoLadder"),
                                    normalColor, -1);
  ...
  const LadderInfo *info =
      TheLadderList->findLadder(lastLadderAddr, lastLadderPort);
  ...
  for (LadderPrefMap::const_iterator cit = recentLadders.begin();
       cit != recentLadders.end(); ++cit) {
    AsciiString addr = cit->second.address;
    UnsignedShort port = cit->second.port;
    if (addr == lastLadderAddr && port == lastLadderPort) continue;
...
void PopulateQMLadderComboBox(void) {
  ...
  index = GadgetComboBoxAddEntry(
      comboBoxLadder, TheGameText->fetch("GUI:NoLadder"), normalColor);
  ...
  index = GadgetComboBoxAddEntry(
      comboBoxLadder, TheGameText->fetch("GUI:ChooseLadder"), normalColor);
  GadgetComboBoxSetItemData(comboBoxLadder, index, (void *)-1);

  GadgetComboBoxSetSelectedPos(comboBoxLadder, selectedPos);
  isPopulatingLadderBox = false;

  populateQMSideComboBox(pref.getSide(),
                         getLadderInfo());  // this will set side to random and
...
static void populateQuickMatchMapSelectListbox(QuickMatchPreferences &pref) {
  ...
  for (std::list<AsciiString>::const_iterator it = maps.begin();
    ...
    if (md && md->m_numPlayers >= numPlayers) {
      ...
      if (li && li->randomMaps) isSelected = TRUE;
      Int width = 10;
      Int height = 10;
      const Image *img = (isSelected) ? selectedImage : unselectedImage;
      if (img) {
        width = min(GadgetListBoxGetColumnWidth(listboxMapSelect, 0),
                    img->getImageWidth());
        height = width;
      }
      Int index = GadgetListBoxAddEntryImage(listboxMapSelect, img, -1, 0,
                                             height, width);
      GadgetListBoxAddEntryText(
          listboxMapSelect, displayName,
          GameSpyColor[(isSelected) ? GSCOLOR_MAP_SELECTED
...
static void saveQuickMatchOptions(void) {
  ...
  if (!li ||
      !li->randomMaps)  // don't save the map as selected if we couldn't choose
  {
    Int row = 0;
    Int entries = GadgetListBoxGetNumEntries(listboxMapSelect);
    while (row < entries) {
      const MapMetaData *md = (const MapMetaData *)GadgetListBoxGetItemData(
          listboxMapSelect, row, 1);
      if (md)
        pref.setMapSelected(md->m_fileName, (Bool)GadgetListBoxGetItemData(
                                                listboxMapSelect, row));
      row++;
    }
  }
...
void WOLQuickMatchMenuInit(WindowLayout *layout, void *userData) {
  ...
  if (TheGameSpyGame && TheGameSpyGame->isGameInProgress()) {
    ...
    if (TheGameSpyInfo->isDisconnectedAfterGameStart(&disconReason)) {
      ...
      DEBUG_LOG(
          ("WOLQuickMatchMenuInit() - game was in progress, and we were "
  ...
  parentWOLQuickMatchID =
      NAMEKEY("WOLQuickMatchMenu.wnd:WOLQuickMatchMenuParent");
  ...
  // textEntryMaxDisconnectsID = NAMEKEY(
  // "WOLQuickMatchMenu.wnd:TextEntryMaxDisconnects" ); textEntryMaxPointsID =
  // NAMEKEY( "WOLQuickMatchMenu.wnd:TextEntryMaxPointPercent" );
  // textEntryMinPointsID = NAMEKEY(
  // "WOLQuickMatchMenu.wnd:TextEntryMinPointPercent" );
  ...
  comboBoxMaxDisconnectsID =
      NAMEKEY("WOLQuickMatchMenu.wnd:ComboBoxMaxDisconnects");
  staticTextNumPlayersID =
      NAMEKEY("WOLQuickMatchMenu.wnd:StaticTextNumPlayers");
  comboBoxSideID = NAMEKEY("WOLQuickMatchMenu.wnd:ComboBoxSide");
  comboBoxColorID = NAMEKEY("WOLQuickMatchMenu.wnd:ComboBoxColor");

  parentWOLQuickMatch =
      TheWindowManager->winGetWindowFromId(NULL, parentWOLQuickMatchID);
  buttonBack =
      TheWindowManager->winGetWindowFromId(parentWOLQuickMatch, buttonBackID);
  buttonStart =
      TheWindowManager->winGetWindowFromId(parentWOLQuickMatch, buttonStartID);
  buttonStop =
      TheWindowManager->winGetWindowFromId(parentWOLQuickMatch, buttonStopID);
  buttonWiden =
      TheWindowManager->winGetWindowFromId(parentWOLQuickMatch, buttonWidenID);
  quickmatchTextWindow = TheWindowManager->winGetWindowFromId(
      parentWOLQuickMatch, listboxQuickMatchID);
  listboxMapSelect = TheWindowManager->winGetWindowFromId(parentWOLQuickMatch,
                                                          listboxMapSelectID);
  // textEntryMaxDisconnects = TheWindowManager->winGetWindowFromId(
  // parentWOLQuickMatch, textEntryMaxDisconnectsID ); textEntryMaxPoints =
  // TheWindowManager->winGetWindowFromId( parentWOLQuickMatch,
  // textEntryMaxPointsID ); textEntryMinPoints =
  // TheWindowManager->winGetWindowFromId( parentWOLQuickMatch,
  // textEntryMinPointsID );
  textEntryWaitTime = TheWindowManager->winGetWindowFromId(parentWOLQuickMatch,
                                                           textEntryWaitTimeID);
  comboBoxMaxPing = TheWindowManager->winGetWindowFromId(parentWOLQuickMatch,
                                                         comboBoxMaxPingID);
  comboBoxNumPlayers = TheWindowManager->winGetWindowFromId(
      parentWOLQuickMatch, comboBoxNumPlayersID);
  comboBoxLadder = TheWindowManager->winGetWindowFromId(parentWOLQuickMatch,
                                                        comboBoxLadderID);
  comboBoxMaxDisconnects = TheWindowManager->winGetWindowFromId(
      parentWOLQuickMatch, comboBoxMaxDisconnectsID);
  TheGameSpyInfo->registerTextWindow(quickmatchTextWindow);
  staticTextNumPlayers = TheWindowManager->winGetWindowFromId(
      parentWOLQuickMatch, staticTextNumPlayersID);
  comboBoxSide =
      TheWindowManager->winGetWindowFromId(parentWOLQuickMatch, comboBoxSideID);
  comboBoxColor = TheWindowManager->winGetWindowFromId(parentWOLQuickMatch,
                                                       comboBoxColorID);
  ...
  if (TheLadderList->getStandardLadders()->size() == 0 &&
    ...
    index =
        GadgetComboBoxAddEntry(comboBoxDisabledLadder,
                               TheGameText->fetch("GUI:NoLadder"), normalColor);
    GadgetComboBoxSetItemData(comboBoxDisabledLadder, index, 0);
    GadgetComboBoxSetSelectedPos(comboBoxDisabledLadder, index);

    isPopulatingLadderBox = FALSE;

    /** This code would actually *hide* the combo box, but it doesn't look as
    good.  Left here since someone will want to
    ...
    comboBoxLadder = NULL;
    GameWindow *staticTextLadder = TheWindowManager->winGetWindowFromId(
    parentWOLQuickMatch, NAMEKEY("WOLQuickMatchMenu.wnd:StaticTextLadder") ); if
    (staticTextLadder) staticTextLadder->winHide(TRUE);
    */
  }

  GameWindow *buttonBuddies =
      TheWindowManager->winGetWindowFromId(NULL, buttonBuddiesID);
  if (buttonBuddies) buttonBuddies->winEnable(TRUE);

  GameWindow *staticTextTitle = TheWindowManager->winGetWindowFromId(
      parentWOLQuickMatch, NAMEKEY("WOLQuickMatchMenu.wnd:StaticTextTitle"));
  if (staticTextTitle) {
    UnicodeString tmp;
  ...
  // fill in preferences
  selectedImage =
      TheMappedImageCollection->findImageByName("CustomMatch_selected");
  unselectedImage =
      TheMappedImageCollection->findImageByName("CustomMatch_deselected");
  QuickMatchPreferences pref;
  ...
  GadgetComboBoxSetSelectedPos(comboBoxNumPlayers,
                               max(0, pref.getNumPlayers()));

  GadgetComboBoxReset(comboBoxMaxDisconnects);
  GadgetComboBoxAddEntry(comboBoxMaxDisconnects, TheGameText->fetch("GUI:Any"),
                         c);
  ...
  i = pref.getMaxPing();
  if (i < 0) i = 0;
  if (i >= maxPingEntries) i = maxPingEntries - 1;
...
void WOLQuickMatchMenuShutdown(WindowLayout *layout, void *userData) {
  ...
  if (!TheGameEngine->getQuitting()) saveQuickMatchOptions();
  ...
  if (TheShell->isAnimFinished() && !buttonPushed &&
    ...
    if (TheNAT != NULL) {
      ...
      if (NATState == NATSTATE_DONE) {
        ...
        return;  // don't do any more processing this frame, in case the screen
                 // goes away
      ...
      switch (resp.peerResponseType) {
          ...
          if (!stricmp(resp.command.c_str(), "STATS")) {
            ...
            for (Int i = 0; i < MAX_SLOTS; ++i) {
              ...
              if (slot && slot->isHuman() &&
                  (slot->getLoginName().compareNoCase(nick) == 0)) {
          ...
          FirewallHelperClass::FIREWALL_TYPE_DESTINATION_PORT_DELTA))
                  {
                          ...
                          DEBUG_LOG(("Setting NAT behavior to %d for player
          %d\n", val, slotNum)); change = true;
                  }
                  else
                  {
                          DEBUG_LOG(("Rejecting invalid NAT behavior %d from
          player %d\n", val, slotNum));
                  }
          }
          */
        ...
        case PeerResponse::PEERRESPONSE_QUICKMATCHSTATUS: {
          ...
          switch (resp.qmStatus.status) {
            case QM_IDLE:
              // TheGameSpyInfo->addText(UnicodeString(L"Status: QM_IDLE"),
              // GameSpyColor[GSCOLOR_DEFAULT], quickmatchTextWindow);
              break;
            case QM_JOININGQMCHANNEL:
              TheGameSpyInfo->addText(TheGameText->fetch("QM:JOININGQMCHANNEL"),
                                      GameSpyColor[GSCOLOR_DEFAULT],
                                      quickmatchTextWindow);
              break;
            case QM_LOOKINGFORBOT:
              TheGameSpyInfo->addText(TheGameText->fetch("QM:LOOKINGFORBOT"),
                                      GameSpyColor[GSCOLOR_DEFAULT],
                                      quickmatchTextWindow);
              break;
            case QM_SENTINFO:
              TheGameSpyInfo->addText(TheGameText->fetch("QM:SENTINFO"),
                                      GameSpyColor[GSCOLOR_DEFAULT],
                                      quickmatchTextWindow);
              break;
            case QM_WORKING: {
              UnicodeString s;
              s.format(TheGameText->fetch("QM:WORKING"),
                       resp.qmStatus.poolSize);
            ...
            case QM_POOLSIZE: {
              UnicodeString s;
              s.format(TheGameText->fetch("QM:POOLSIZE"),
                       resp.qmStatus.poolSize);
              TheGameSpyInfo->addText(s, GameSpyColor[GSCOLOR_DEFAULT],
                                      quickmatchTextWindow);
            } break;
            case QM_WIDENINGSEARCH:
              TheGameSpyInfo->addText(TheGameText->fetch("QM:WIDENINGSEARCH"),
                                      GameSpyColor[GSCOLOR_DEFAULT],
                                      quickmatchTextWindow);
              buttonWiden->winEnable(FALSE);
              break;
            case QM_MATCHED: {
              TheGameSpyInfo->addText(TheGameText->fetch("QM:MATCHED"),
                                      GameSpyColor[GSCOLOR_DEFAULT],
                                      quickmatchTextWindow);
              buttonWiden->winEnable(FALSE);
              ...
              for (i = 0; i < MAX_SLOTS; ++i) {
                if (!resp.stagingRoomPlayerNames[i].empty()) ++numPlayers;
              }
              ...
              for (std::list<AsciiString>::const_iterator it = maps.begin();
                ...
                if (md && md->m_numPlayers >= numPlayers) {
                  ...
                  if (resp.qmStatus.mapIdx-- == 0) break;
                }
              }
#endif
              ...
              if (!numPlayersPerTeam) numPlayersPerTeam = 1;
              ...
              for (i = 0; i < MAX_SLOTS; ++i) {
                ...
                if (resp.stagingRoomPlayerNames[i].empty()) {
                  ...
                  // slot->setProfileID(0);
                  slot->setNATBehavior(
                      (FirewallHelperClass::FirewallBehaviorType)
                          resp.qmStatus.nat[i]);
                  slot->setLocale("");
                  slot->setTeamNumber(i / numPlayersPerTeam);
                  if (i == 0) TheGameSpyGame->setGameName(uName);
                }
              }
              ...
              GameWindow *buttonBuddies =
                  TheWindowManager->winGetWindowFromId(NULL, buttonBuddiesID);
              if (buttonBuddies) buttonBuddies->winEnable(FALSE);
              GameSpyCloseOverlay(GSOVERLAY_BUDDY);
            } break;
            case QM_INCHANNEL:
              TheGameSpyInfo->addText(TheGameText->fetch("QM:INCHANNEL"),
                                      GameSpyColor[GSCOLOR_DEFAULT],
                                      quickmatchTextWindow);
              break;
            case QM_NEGOTIATINGFIREWALLS:
              TheGameSpyInfo->addText(
                  TheGameText->fetch("QM:NEGOTIATINGFIREWALLS"),
                  GameSpyColor[GSCOLOR_DEFAULT], quickmatchTextWindow);
              break;
            case QM_STARTINGGAME:
              TheGameSpyInfo->addText(TheGameText->fetch("QM:STARTINGGAME"),
                                      GameSpyColor[GSCOLOR_DEFAULT],
                                      quickmatchTextWindow);
              break;
            case QM_COULDNOTFINDBOT:
              TheGameSpyInfo->addText(TheGameText->fetch("QM:COULDNOTFINDBOT"),
                                      GameSpyColor[GSCOLOR_DEFAULT],
                                      quickmatchTextWindow);
              buttonWiden->winEnable(FALSE);
              buttonStart->winHide(FALSE);
            ...
            case QM_COULDNOTFINDCHANNEL:
              TheGameSpyInfo->addText(
                  TheGameText->fetch("QM:COULDNOTFINDCHANNEL"),
                  GameSpyColor[GSCOLOR_DEFAULT], quickmatchTextWindow);
              buttonWiden->winEnable(FALSE);
              buttonStart->winHide(FALSE);
              buttonStop->winHide(TRUE);
            ...
            case QM_COULDNOTNEGOTIATEFIREWALLS:
              TheGameSpyInfo->addText(
                  TheGameText->fetch("QM:COULDNOTNEGOTIATEFIREWALLS"),
                  GameSpyColor[GSCOLOR_DEFAULT], quickmatchTextWindow);
              buttonWiden->winEnable(FALSE);
              buttonStart->winHide(FALSE);
              buttonStop->winHide(TRUE);
              ...
              break;
            case QM_STOPPED:
              TheGameSpyInfo->addText(TheGameText->fetch("QM:STOPPED"),
                                      GameSpyColor[GSCOLOR_DEFAULT],
                                      quickmatchTextWindow);
              buttonWiden->winEnable(FALSE);
              buttonStart->winHide(FALSE);
    ...
    if (frameTime > 100 || responses.size() > 20) {
      ...
      munkee.format(L"inQM:%d %d ms, %d messages", s_inQM, frameTime,
                    responses.size());
      TheGameSpyInfo->addText(munkee, GameSpyColor[GSCOLOR_DEFAULT],
                              quickmatchTextWindow);
      PERF_LOG(("%ls\n", munkee.str()));
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType WOLQuickMatchMenuSystem(GameWindow *window,
                                             UnsignedInt msg,
                                             WindowMsgData mData1,
                                             WindowMsgData mData2) {
  ...
  switch (msg) {
      ...
      if (controlID == comboBoxLadderID && !isPopulatingLadderBox) {
        if (pos >= 0) {
          ...
          if (ladderID == 0) {
            ...
            GadgetComboBoxSetSelectedPos(comboBoxNumPlayers,
                                         max(0, pref.getNumPlayers() / 2 - 1));
            comboBoxNumPlayers->winEnable(TRUE);
            populateQMSideComboBox(
                pref.getSide());  // this will set side to random and disable if
                                  // necessary
          } else if (ladderID > 0) {
            ...
            populateQMSideComboBox(
                pref.getSide(),
                li);  // this will set side to random and disable if necessary
          } else {
            // "Choose a ladder" selected - open overlay
            PopulateQMLadderComboBox();  // this restores the non-"Choose a
                                         // ladder" selection
    ...
    case GBM_SELECTED: {
      ...
      GameWindow *control = (GameWindow *)mData1;
      Int controlID = control->winGetWindowId();
      static NameKeyType buttonOptionsID =
          NAMEKEY("WOLQuickMatchMenu.wnd:ButtonOptions");
      ...
      if (controlID == buttonStopID) {
        ...
        buttonStop->winHide(TRUE);
        enableOptionsGadgets(TRUE);
        TheGameSpyInfo->addText(TheGameText->fetch("GUI:QMAborted"),
                                GameSpyColor[GSCOLOR_DEFAULT],
                                quickmatchTextWindow);
      } else if (controlID == buttonOptionsID) {
        GameWindow *win = TheWindowManager->winGetWindowFromId(
            parentWOLQuickMatch, buttonOptionsID);
        if (isInfoShown()) {
...
#if VARIABLE_NUMBER_OF_MAPS
        for (MapListboxIndex::iterator idxIt = mapListboxIndex.begin();
             idxIt != mapListboxIndex.end(); ++idxIt) {
          Int index = (*idxIt);
          if (index >= 0) {
            req.qmMaps.push_back(
                GadgetListBoxGetItemData(listboxMapSelect, index, 0));
          } else {
        ...
        for (Int i = 0; i < numMaps; ++i) {
          req.qmMaps.push_back(
              GadgetListBoxGetItemData(listboxMapSelect, i, 0));
        }
#endif
        ...
        AsciiString a;
        //					u =
        //GadgetTextEntryGetText(textEntryMaxDisconnects); 					a.translate(u);
        //					req.QM.maxDiscons =
        //atoi(a.str()); 					u = GadgetTextEntryGetText(textEntryMaxPoints);
        //					a.translate(u);
        req.QM.maxPointPercentage = max(100, maxPoints);
        //					u =
        //GadgetTextEntryGetText(textEntryMinPoints); 					a.translate(u);
        ...
        if (ladderInfo && ladderInfo->randomFactions) {
          Int sideNum =
              GameClientRandomValue(0, ladderInfo->validFactions.size() - 1);
          DEBUG_LOG(("Looking for %d out of %d random sides\n", sideNum,
                     ladderInfo->validFactions.size()));
        ...
        if (ladderIndex > 0) {
          // save the ladder as being played upon even if we cancel out of
          // matching early...
          LadderPreferences ladPref;
        ...
        for (Int i = 0; i < numMaps; ++i) {
          ...
          GadgetListBoxAddEntryText(
              listboxMapSelect, GadgetListBoxGetText(listboxMapSelect, i, 1),
              GameSpyColor[GSCOLOR_MAP_SELECTED], i, 1);
        }
        ...
        for (Int i = 0; i < numMaps; ++i) {
          ...
          GadgetListBoxAddEntryText(
              listboxMapSelect, GadgetListBoxGetText(listboxMapSelect, i, 1),
              GameSpyColor[GSCOLOR_MAP_UNSELECTED], i, 1);
        }
    ...
    case GLM_SELECTED: {
      ...
      if (controlID == listboxMapSelectID) {
        ...
        if (selected >= 0 && (!li || !li->randomMaps)) {
          Bool wasSelected =
              (Bool)GadgetListBoxGetItemData(control, selected, 0);
          GadgetListBoxSetItemData(control, (void *)(!wasSelected), selected,
                                   0);
          Int width = 10;
          Int height = 10;
          const Image *img = (!wasSelected) ? selectedImage : unselectedImage;
          if (img) {
            width = min(GadgetListBoxGetColumnWidth(control, 0),
                        img->getImageWidth());
            height = width;
          }
        ...
        if (selected >= 0) GadgetListBoxSetSelected(control, -1);
      }
      UpdateStartButton();
      break;
    }  // case GLM_SELECTED
