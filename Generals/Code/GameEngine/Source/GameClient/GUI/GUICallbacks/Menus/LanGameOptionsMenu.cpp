...
extern void MapSelectorTooltip(GameWindow *window, WinInstanceData *instData,
                               UnsignedInt mouse);
extern void gameAcceptTooltip(GameWindow *window, WinInstanceData *instData,
                              UnsignedInt mouse);
...
static void playerTooltip(GameWindow *window, WinInstanceData *instData,
  ...
  if (!player) {
    DEBUG_ASSERTCRASH(TheLAN->GetMyGame()->getIP(i) == 0,
                      ("No player info in listbox!"));
    TheMouse->setCursorTooltip(UnicodeString::TheEmptyString);
    return;
  }
  UnicodeString tooltip;
  tooltip.format(TheGameText->fetch("TOOLTIP:LANPlayer"),
                 player->getName().str(), player->getLogin().str(),
                 player->getHost().str());
...
void StartPressed(void) {
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (slot && slot->isOccupied() &&
      ...
      if (slot && slot->isHuman()) numHumans++;
      numUsers++;
    }
  }
  ...
  if (!md || md->m_numPlayers < numUsers) {
    if (TheLAN->AmIHost()) {
      ...
      TheLAN->OnChat(UnicodeString(L"SYSTEM"), TheLAN->GetLocalIP(), text,
    ...
    if (slot->isHuman() && !slot->isAccepted()) {
      ...
      if (!willTransfer) {
        if (!slot->hasMap()) {
          ...
          GadgetListBoxAddEntryText(listboxChatWindowLanGame, msg,
                                    chatSystemColor, -1, 0);
          allHaveMap = false;
        }
      }
  ...
  if (isReady) {
    ...
    if (allHaveMap) {
      GadgetListBoxAddEntryText(listboxChatWindowLanGame,
                                TheGameText->fetch("GUI:NotifiedStartIntent"),
                                chatSystemColor, -1, 0);
...
static void handleColorSelection(int index) {
  ...
  if (myGame) {
    ...
    if (myGame->amIHost()) {
      ...
      if (!slot->isLocalPlayer() || !AreSlotListUpdatesEnabled()) return;
...
void lanUpdateSlotList(void) {
  if (!AreSlotListUpdatesEnabled() || s_isIniting) return;
  UpdateSlotList(TheLAN->GetMyGame(), comboBoxPlayer, comboBoxColor,
                 comboBoxPlayerTemplate, comboBoxTeam, buttonAccept,
                 buttonStart, buttonMapStartPosition);
...
void InitLanGameGadgets(void) {
  ...
  parentLanGameOptionsID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanGameOptionsMenu.wnd:LanGameOptionsMenuParent"));
  buttonBackID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanGameOptionsMenu.wnd:ButtonBack"));
  buttonStartID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanGameOptionsMenu.wnd:ButtonStart"));
  textEntryChatID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanGameOptionsMenu.wnd:TextEntryChat"));
  textEntryMapDisplayID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanGameOptionsMenu.wnd:TextEntryMapDisplay"));
  listboxChatWindowLanGameID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanGameOptionsMenu.wnd:ListboxChatWindowLanGame"));
  buttonEmoteID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanGameOptionsMenu.wnd:ButtonEmote"));
  buttonSelectMapID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanGameOptionsMenu.wnd:ButtonSelectMap"));
  windowMapID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanGameOptionsMenu.wnd:MapWindow"));

  // Initialize the pointers to our gadgets
  parentLanGameOptions =
      TheWindowManager->winGetWindowFromId(NULL, parentLanGameOptionsID);
  DEBUG_ASSERTCRASH(parentLanGameOptions,
                    ("Could not find the parentLanGameOptions"));
  buttonEmote =
      TheWindowManager->winGetWindowFromId(parentLanGameOptions, buttonEmoteID);
  DEBUG_ASSERTCRASH(buttonEmote, ("Could not find the buttonEmote"));
  buttonSelectMap = TheWindowManager->winGetWindowFromId(parentLanGameOptions,
                                                         buttonSelectMapID);
  DEBUG_ASSERTCRASH(buttonSelectMap, ("Could not find the buttonSelectMap"));
  buttonStart =
      TheWindowManager->winGetWindowFromId(parentLanGameOptions, buttonStartID);
  DEBUG_ASSERTCRASH(buttonStart, ("Could not find the buttonStart"));
  buttonBack =
      TheWindowManager->winGetWindowFromId(parentLanGameOptions, buttonBackID);
  DEBUG_ASSERTCRASH(buttonBack, ("Could not find the buttonBack"));
  listboxChatWindowLanGame = TheWindowManager->winGetWindowFromId(
      parentLanGameOptions, listboxChatWindowLanGameID);
  DEBUG_ASSERTCRASH(listboxChatWindowLanGame,
                    ("Could not find the listboxChatWindowLanGame"));
  textEntryChat = TheWindowManager->winGetWindowFromId(parentLanGameOptions,
                                                       textEntryChatID);
  DEBUG_ASSERTCRASH(textEntryChat, ("Could not find the textEntryChat"));
  textEntryMapDisplay = TheWindowManager->winGetWindowFromId(
      parentLanGameOptions, textEntryMapDisplayID);
  DEBUG_ASSERTCRASH(textEntryMapDisplay,
                    ("Could not find the textEntryMapDisplay"));

  windowMap =
      TheWindowManager->winGetWindowFromId(parentLanGameOptions, windowMapID);
  DEBUG_ASSERTCRASH(windowMap,
                    ("Could not find the LanGameOptionsMenu.wnd:MapWindow"));
  ...
  for (Int i = 0; i < MAX_SLOTS; i++) {
    ...
    if (localSlotNum == i) {
      ...
      GadgetComboBoxAddEntry(comboBoxPlayer[i],
                             TheGameText->fetch("GUI:Closed"), white);
      GadgetComboBoxAddEntry(comboBoxPlayer[i],
    ...
    */
    ...
    comboBoxColor[i] = TheWindowManager->winGetWindowFromId(
        parentLanGameOptions, comboBoxColorID[i]);
    ...
    comboBoxPlayerTemplate[i] = TheWindowManager->winGetWindowFromId(
        parentLanGameOptions, comboBoxPlayerTemplateID[i]);
    ...
    buttonAccept[i] = TheWindowManager->winGetWindowFromId(parentLanGameOptions,
                                                           buttonAcceptID[i]);
    ...
    //
    //		tmpString.format("LanGameOptionsMenu.wnd:ButtonStartPosition%d",
    ...
    //parentLanGameOptions, buttonStartPositionID[i] );
    ...
    buttonMapStartPosition[i] = TheWindowManager->winGetWindowFromId(
        parentLanGameOptions, buttonMapStartPositionID[i]);
...
void LanGameOptionsMenuInit(WindowLayout *layout, void *userData) {
  ...
  if (TheLAN->AmIHost()) {
    ...
    if (it != TheMapCache->end()) {
      ...
      TheLAN->GetMyGame()
          ->adjustSlotsForMap();  // BGC- adjust the slots for the selected map.
    }
    ...
    TheLAN->GetMyGame()->setMapCRC(
        TheLAN->GetMyGame()->getMapCRC());  // force a recheck
    TheLAN->GetMyGame()->setMapSize(
        TheLAN->GetMyGame()->getMapSize());  // of if we have the map
  ...
  for (Int i = start; i < MAX_SLOTS; ++i) {
    ...
    if (!TheLAN->AmIHost()) comboBoxPlayer[i]->winEnable(FALSE);
  ...
  //			buttonStartPosition[i]->winHide(TRUE); // not picking
  //start spots this way any more
  //	}
  //
  // Show the Menu
  layout->hide(FALSE);
  ...
  // animate controls
  // TheShell->registerWithAnimateManager(buttonBack, WIN_ANIMATION_SLIDE_RIGHT,
  // TRUE, 1);
...
void updateGameOptions(void) {
  ...
  if (theGame && AreSlotListUpdatesEnabled()) {
    const GameSlot *localSlot =
        theGame->getConstSlot(theGame->getLocalSlotNum());
    const MapMetaData *mapData =
        TheMapCache->findMap(TheLAN->GetMyGame()->getMap());
    ...
    if (old.compare(mapDisplayName) != 0) LanPositionStartSpots();
...
void LanGameOptionsMenuShutdown(WindowLayout *layout, void *userData) {
  ...
  if (TheLAN) TheLAN->ResetGameStartTimer();
...
void LanGameOptionsMenuUpdate(WindowLayout *layout, void *userData) {
  if (LANisShuttingDown && TheShell->isAnimFinished() &&
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType LanGameOptionsMenuInput(GameWindow *window,
                                             UnsignedInt msg,
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType LanGameOptionsMenuSystem(GameWindow *window,
                                              UnsignedInt msg,
                                              WindowMsgData mData1,
                                              WindowMsgData mData2) {
        ...
        if (controlID == comboBoxColorID[i]) {
          ...
          if (pos != SLOT_PLAYER && pos >= 0) {
            if (myGame->getLANSlot(i)->getState() == SLOT_PLAYER) {
              ...
              if (wasAI || isAI) myGame->resetAccepted();
              if (wasAI ^ isAI)
                PopulatePlayerTemplateComboBox(i, comboBoxPlayerTemplate,
                                               myGame, wasAI);
              if (!s_isIniting) {
    ...
    case GBM_SELECTED: {
      ...
      if (controlID == buttonBackID) {
        ...
        mapSelectLayout = TheWindowManager->winCreateLayout(
            AsciiString("Menus/LanMapSelectMenu.wnd"));
        ...
        if (TheLAN->AmIHost()) {
          ...
          EnableAcceptControls(TRUE, TheLAN->GetMyGame(), comboBoxPlayer,
...
void PostToLanGameOptions(PostToLanGameType post) {
  if (post >= POST_TO_LAN_GAME_TYPE_COUNT) return;
  LanPositionStartSpots();
  switch (post) {
      //-------------------------------------------------------------------------------------------------
    case SEND_GAME_OPTS: {
