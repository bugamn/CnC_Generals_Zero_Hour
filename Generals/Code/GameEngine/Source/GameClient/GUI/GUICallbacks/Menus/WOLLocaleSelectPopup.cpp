...
void WOLLocaleSelectInit(WindowLayout *layout, void *userData) {
  parentLocaleSelectID = TheNameKeyGenerator->nameToKey(
      AsciiString("PopupLocaleSelect.wnd:ParentLocaleSelect"));
  buttonOkID = TheNameKeyGenerator->nameToKey(
      AsciiString("PopupLocaleSelect.wnd:ButtonOk"));
  listboxLocaleID = TheNameKeyGenerator->nameToKey(
      AsciiString("PopupLocaleSelect.wnd:ListBoxLocaleSelect"));
  parentLocaleSelect =
      TheWindowManager->winGetWindowFromId(NULL, parentLocaleSelectID);
  ...
  for (int i = LOC_MIN; i <= LOC_MAX; ++i) {
    ...
    GadgetListBoxAddEntryText(listboxLocale, TheGameText->fetch(id.str()),
...
// Int getRegistryNicknameOffset(AsciiString nick); /// @todo: mdc remove this
// once we can save ini pref files
//-------------------------------------------------------------------------------------------------
  ...
  switch (msg) {
    ...
    case GBM_SELECTED: {
      ...
      if (controlID == buttonOkID) {
        ...
        if (selected < 0) return MSG_HANDLED;  // can't select nothing!
        PSRequest psReq;
