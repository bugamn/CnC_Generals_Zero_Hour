...
static void playerTooltip(GameWindow *window, WinInstanceData *instData,
  ...
  UnsignedInt playerIP =
      (UnsignedInt)GadgetListBoxGetItemData(window, row, col);
...
void LanLobbyMenuInit(WindowLayout *layout, void *userData) {
  ...
  parentLanLobbyID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanLobbyMenu.wnd:LanLobbyMenuParent"));
  buttonBackID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanLobbyMenu.wnd:ButtonBack"));
  buttonClearID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanLobbyMenu.wnd:ButtonClear"));
  buttonHostID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanLobbyMenu.wnd:ButtonHost"));
  buttonJoinID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanLobbyMenu.wnd:ButtonJoin"));
  buttonDirectConnectID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanLobbyMenu.wnd:ButtonDirectConnect"));
  buttonEmoteID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanLobbyMenu.wnd:ButtonEmote"));
  staticToolTipID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanLobbyMenu.wnd:StaticToolTip"));
  textEntryPlayerNameID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanLobbyMenu.wnd:TextEntryPlayerName"));
  textEntryChatID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanLobbyMenu.wnd:TextEntryChat"));
  listboxPlayersID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanLobbyMenu.wnd:ListboxPlayers"));
  listboxChatWindowID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanLobbyMenu.wnd:ListboxChatWindowLanLobby"));
  listboxGamesID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanLobbyMenu.wnd:ListboxGames"));
  staticTextGameInfoID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanLobbyMenu.wnd:StaticTextGameInfo"));
  ...
  buttonDirectConnect =
      TheWindowManager->winGetWindowFromId(NULL, buttonDirectConnectID);
  ...
  listboxChatWindow =
      TheWindowManager->winGetWindowFromId(NULL, listboxChatWindowID);
  listboxGames = TheWindowManager->winGetWindowFromId(NULL, listboxGamesID);
  staticTextGameInfo =
      TheWindowManager->winGetWindowFromId(NULL, staticTextGameInfoID);
  ...
  if (!IP) {
    ...
    */
    ...
    // GadgetListBoxAddEntryText(listboxChatWindow, str, chatSystemColor, -1,
    // 0);
    IP = IPlist->getIP();
  } else {
    /*
  ...
  GadgetListBoxAddEntryText(listboxChatWindow, unicodeChat, chatSystemColor, -1,
  0);
  ...
  while (IPlist)
  {
          ...
          GadgetListBoxAddEntryText(listboxChatWindow, unicodeChat,
  chatSystemColor, -1, 0); IPlist = IPlist->getNext();
  }
  */
  ...
  GameWindow *win = TheWindowManager->winGetWindowFromId(
      NULL, TheNameKeyGenerator->nameToKey("LanLobbyMenu.wnd:GadgetParent"));
  if (win) win->winHide(TRUE);

  // animate controls
  // TheShell->registerWithAnimateManager(parentLanLobby,
  // WIN_ANIMATION_SLIDE_TOP, TRUE);
  //	TheShell->registerWithAnimateManager(buttonHost,
  //WIN_ANIMATION_SLIDE_LEFT, TRUE, 600);
  //	TheShell->registerWithAnimateManager(buttonJoin,
  //WIN_ANIMATION_SLIDE_LEFT, TRUE, 400);
  //	TheShell->registerWithAnimateManager(buttonDirectConnect,
...
void LanLobbyMenuShutdown(WindowLayout *layout, void *userData) {
  LANPreferences prefs;
  prefs["UserName"] = UnicodeStringToQuotedPrintable(
...
void LanLobbyMenuUpdate(WindowLayout *layout, void *userData) {
  ...
  if (LANSocketErrorDetected == TRUE) {
    ...
    MessageBoxOk(TheGameText->fetch("GUI:NetworkError"),
                 TheGameText->fetch("GUI:SocketError"), NULL);
  ...
  switch (msg) {
    ...
    case GBM_SELECTED: {
      ...
      if (controlID == buttonBackID) {
        ...
        TheWindowManager->winSendSystemMsg(
            window, GEM_UPDATE_TEXT, (WindowMsgData)textEntryPlayerName, 0);
        ...
        if (rowSelected >= 0) {
          ...
          GadgetListBoxAddEntryText(
              listboxChatWindow, TheGameText->fetch("LAN:ErrorNoGameSelected"),
              chatSystemColor, -1, 0);
        }
    ...
    case GEM_UPDATE_TEXT: {
      ...
      if (controlID == textEntryPlayerNameID) {
        ...
        if (!txtInput.isEmpty() &&
            txtInput.getCharAt(txtInput.getLength() - 1) == L',')
          txtInput.removeLastChar();  // we use , for strtok's so we can't allow
                                      // them in names.  :(
