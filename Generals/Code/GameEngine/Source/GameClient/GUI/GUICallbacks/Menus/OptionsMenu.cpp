...
Int OptionPreferences::getCampaignDifficulty(void) {
  ...
  if (it == end()) return TheScriptEngine->getGlobalDifficulty();

  Int factor = atoi(it->second.str());
  if (factor < DIFFICULTY_EASY) factor = DIFFICULTY_EASY;
  if (factor > DIFFICULTY_HARD) factor = DIFFICULTY_HARD;
...
void OptionPreferences::setLANIPAddress(UnsignedInt IP) {
  AsciiString tmp;
  tmp.format("%d.%d.%d.%d", ((IP & 0xff000000) >> 24), ((IP & 0xff0000) >> 16),
             ((IP & 0xff00) >> 8), (IP & 0xff));
  (*this)["IPAddress"] = tmp;
}
...
Bool OptionPreferences::getAlternateMouseModeEnabled(void) {
  ...
  if (it == end()) return TheGlobalData->m_useAlternateMouse;
...
Real OptionPreferences::getScrollFactor(void) {
  ...
  if (it == end()) return TheGlobalData->m_keyboardDefaultScrollFactor;
...
Int OptionPreferences::getIdealStaticGameDetail(void) {
  ...
  if (it == end()) return STATIC_GAME_LOD_UNKNOWN;
...
Int OptionPreferences::getStaticGameDetail(void) {
  ...
  if (it == end()) return TheGameLODManager->getStaticLODLevel();
...
Bool OptionPreferences::getSendDelay(void) {
  ...
  if (it == end()) return TheGlobalData->m_firewallSendDelay;
...
Int OptionPreferences::getFirewallBehavior() {
  ...
  if (it == end()) return TheGlobalData->m_firewallBehavior;
...
UnsignedShort OptionPreferences::getFirewallPortOverride() {
  ...
  if (override < 0 || override > 65535) override = 0;
  return override;
}
...
AsciiString OptionPreferences::getPreferred3DProvider(void) {
  ...
  if (it == end())
    return TheAudio->getAudioSettings()
        ->m_preferred3DProvider[MAX_HW_PROVIDERS];
  return it->second;
}
...
AsciiString OptionPreferences::getSpeakerType(void) {
  ...
  if (it == end())
    return TheAudio->translateUnsignedIntToSpeakerType(
...
Bool OptionPreferences::getCloudShadowsEnabled(void) {
  ...
  if (it == end()) return TheGlobalData->m_useCloudMap;
...
Bool OptionPreferences::getLightmapEnabled(void) {
  ...
  if (it == end()) return TheGlobalData->m_useLightMap;
...
Bool OptionPreferences::getSmoothWaterEnabled(void) {
  ...
  if (it == end()) return TheGlobalData->m_showSoftWaterEdge;
...
Bool OptionPreferences::getTreesEnabled(void) {
  ...
  if (it == end()) return TheGlobalData->m_useTrees;
...
Bool OptionPreferences::getExtraAnimationsDisabled(void) {
  ...
  if (it == end()) return TheGlobalData->m_useDrawModuleLOD;
...
Bool OptionPreferences::getDynamicLODEnabled(void) {
  ...
  if (it == end()) return TheGlobalData->m_enableDynamicLOD;
...
Bool OptionPreferences::getFPSLimitEnabled(void) {
  ...
  if (it == end()) return TheGlobalData->m_useFpsLimit;
...
Bool OptionPreferences::get3DShadowsEnabled(void) {
  ...
  if (it == end()) return TheGlobalData->m_useShadowVolumes;
...
Bool OptionPreferences::get2DShadowsEnabled(void) {
  ...
  if (it == end()) return TheGlobalData->m_useShadowDecals;
...
Bool OptionPreferences::getBuildingOcclusionEnabled(void) {
  ...
  if (it == end()) return TheGlobalData->m_enableBehindBuildingMarkers;
...
Int OptionPreferences::getParticleCap(void) {
  ...
  if (it == end()) return TheGlobalData->m_maxParticleCount;
...
Int OptionPreferences::getTextureReduction(void) {
  ...
  if (it == end()) return -1;  // unknown texture reduction
...
static void setDefaults(void) {
  ...
  if ((TheGameLogic->isInGame() == FALSE) ||
      (TheGameLogic->isInShellGame() == TRUE)) {
    ...
    switch (TheGameLODManager->getStaticLODLevel()) {
      ...
      default:
        DEBUG_ASSERTCRASH(FALSE,
                          ("Tried to set comboBoxDetail to a value of %d ",
  ...
  if ((TheGameLogic->isInGame() == FALSE) ||
      (TheGameLogic->isInShellGame() == TRUE) && !TheGameSpyInfo) {
    ...
    for (Int i = 0; i < numResolutions; ++i) {
      ...
      if (xres == 800 &&
          yres == 600)  // keep track of default mode in case we need it.
      {
        defaultResIndex = i;
        break;
      }
    ...
    GadgetComboBoxSetSelectedPos(
        comboBoxResolution,
        defaultResIndex);  // should be 800x600 (our lowest supported mode)
  }
  ...
  //	GadgetSliderSetPosition(sliderScrollSpeed, ((valMax - valMin) / 2 +
  //valMin));
  ...
  Real maxVolume = MAX(TheAudio->getAudioSettings()->m_defaultSoundVolume,
                       TheAudio->getAudioSettings()->m_default3DSoundVolume);
  ...
  GadgetSliderSetPosition(
      sliderVoiceVolume,
      REAL_TO_INT(TheAudio->getAudioSettings()->m_defaultSpeechVolume *
                  100.0f));
  ...
  if ((TheGameLogic->isInGame() == FALSE) ||
    ...
    //
    GadgetCheckBoxSetChecked(checkSmoothWater,
                             TheGlobalData->m_showSoftWaterEdge);

    //-------------------------------------------------------------------------------------------------
    // Extra Animations (tree sway and buildups) checkbox
    //
    GadgetCheckBoxSetChecked(checkExtraAnimations,
                             !TheGlobalData->m_useDrawModuleLOD);

    //-------------------------------------------------------------------------------------------------
    // DisableDynamicLOD
    //
    GadgetCheckBoxSetChecked(checkNoDynamicLod,
                             !TheGlobalData->m_enableDynamicLOD);
    ...
    //
    GadgetCheckBoxSetChecked(checkBuildingOcclusion,
                             TheGlobalData->m_enableBehindBuildingMarkers);

    //-------------------------------------------------------------------------------------------------
    // Particle Cap slider
    //
    GadgetSliderSetPosition(sliderParticleCap,
...
static void saveOptions(void) {
  ...
  //	(*pref)["3DAudioProvider"] =
  //TheAudio->getProviderName(TheAudio->getSelectedProvider());
  //
  //-------------------------------------------------------------------------------------------------
  //	// speaker type
  ...
  //	(*pref)["SpeakerType"] =
  //TheAudio->translateUnsignedIntToSpeakerType(TheAudio->getSpeakerType());
  //
  //-------------------------------------------------------------------------------------------------
  // language filter
  ...
  //-------------------------------------------------------------------------------------------------
  // send Delay
  TheWritableGlobalData->m_firewallSendDelay =
      GadgetCheckBoxIsChecked(checkSendDelay);
  ...
  if (index == CUSTOMDETAIL) {
    ...
    // Texture resolution slider
    {
      ...
      if (TheGlobalData->m_textureReductionFactor != val) {
        TheGameClient->adjustLOD(
            val -
            TheGlobalData->m_textureReductionFactor);  // apply the new setting
      }
    }

    TheWritableGlobalData->m_useShadowVolumes =
        GadgetCheckBoxIsChecked(check3DShadows);
    (*pref)["UseShadowVolumes"] = TheWritableGlobalData->m_useShadowVolumes
                                      ? AsciiString("yes")
                                      : AsciiString("no");

    TheWritableGlobalData->m_useShadowDecals =
        GadgetCheckBoxIsChecked(check2DShadows);
    (*pref)["UseShadowDecals"] = TheWritableGlobalData->m_useShadowDecals
                                     ? AsciiString("yes")
                                     : AsciiString("no");

    TheWritableGlobalData->m_useCloudMap =
        GadgetCheckBoxIsChecked(checkCloudShadows);
    (*pref)["UseCloudMap"] =
        TheGlobalData->m_useCloudMap ? AsciiString("yes") : AsciiString("no");

    TheWritableGlobalData->m_useLightMap =
        GadgetCheckBoxIsChecked(checkGroundLighting);
    (*pref)["UseLightMap"] =
        TheGlobalData->m_useLightMap ? AsciiString("yes") : AsciiString("no");

    TheWritableGlobalData->m_showSoftWaterEdge =
        GadgetCheckBoxIsChecked(checkSmoothWater);
    (*pref)["ShowSoftWaterEdge"] = TheGlobalData->m_showSoftWaterEdge
                                       ? AsciiString("yes")
                                       : AsciiString("no");

    TheWritableGlobalData->m_useDrawModuleLOD =
        !GadgetCheckBoxIsChecked(checkExtraAnimations);
    TheWritableGlobalData->m_useTreeSway =
        !TheWritableGlobalData->m_useDrawModuleLOD;  // borrow same setting.
    (*pref)["ExtraAnimations"] = TheGlobalData->m_useDrawModuleLOD
                                     ? AsciiString("no")
                                     : AsciiString("yes");

    TheWritableGlobalData->m_enableDynamicLOD =
        !GadgetCheckBoxIsChecked(checkNoDynamicLod);
    (*pref)["DynamicLOD"] = TheGlobalData->m_enableDynamicLOD
                                ? AsciiString("yes")
                                : AsciiString("no");

    // Never write this out
    // TheWritableGlobalData->m_useFpsLimit = !GadgetCheckBoxIsChecked(
    // checkUnlockFps );
    //(*pref)["FPSLimit"] = TheGlobalData->m_useFpsLimit ? AsciiString("yes") :
    //AsciiString("no");

    TheWritableGlobalData->m_enableBehindBuildingMarkers =
        GadgetCheckBoxIsChecked(checkBuildingOcclusion);
    (*pref)["BuildingOcclusion"] =
        TheWritableGlobalData->m_enableBehindBuildingMarkers
            ? AsciiString("yes")
            : AsciiString("no");

    TheWritableGlobalData->m_useTrees = GadgetCheckBoxIsChecked(checkProps);
    (*pref)["ShowTrees"] = TheWritableGlobalData->m_useTrees
                               ? AsciiString("yes")
                               : AsciiString("no");
  ...
  GadgetComboBoxGetSelectedPos(comboBoxDetail, &index);
  // The levels stored by the LOD Manager are inverted compared to GUI so find
  // correct one:
  switch (index) {
    case HIGHDETAIL:
    ...
    case MEDIUMDETAIL:
      levelChanged =
          TheGameLODManager->setStaticLODLevel(STATIC_GAME_LOD_MEDIUM);
      break;
    case LOWDETAIL:
    ...
    case CUSTOMDETAIL:
      levelChanged =
          TheGameLODManager->setStaticLODLevel(STATIC_GAME_LOD_CUSTOM);
      break;
    default:
      DEBUG_ASSERTCRASH(
          FALSE,
          ("LOD passed in was %d, %d is not a supported LOD", index, index));
      break;
  }

  if (levelChanged)
    (*pref)["StaticGameLOD"] = TheGameLODManager->getStaticGameLODLevelName(
        TheGameLODManager->getStaticLODLevel());
  ...
  if (index < TheDisplay->getDisplayModeCount() && index >= 0) {
    ...
    if (TheGlobalData->m_xResolution != xres ||
        TheGlobalData->m_yResolution != yres) {
      if (TheDisplay->setDisplayMode(xres, yres, bitDepth,
        ...
        // Save new settings for a dialog box confirmation after options are
        // accepted
  ...
  //-------------------------------------------------------------------------------------------------
  // HTTP Proxy
  GameWindow *textEntryHTTPProxy = TheWindowManager->winGetWindowFromId(
      NULL, NAMEKEY("OptionsMenu.wnd:TextEntryHTTPProxy"));
  ...
  GameWindow *textEntryFirewallPortOverride =
      TheWindowManager->winGetWindowFromId(
          NULL, NAMEKEY("OptionsMenu.wnd:TextEntryFirewallPortOverride"));
  ...
  (*pref)["UseAlternateMouse"] = TheWritableGlobalData->m_useAlternateMouse
                                     ? AsciiString("yes")
                                     : AsciiString("no");
  ...
  if (val != -1) {
    ...
    DEBUG_LOG(("Scroll Spped val %d, keyboard scroll factor %f\n", val,
               TheGlobalData->m_keyboardScrollFactor));
    AsciiString prefString;
    prefString.format("%d", val);
  ...
  if (val != -1) {
    ...
    TheAudio->setVolume(val / 100.0f, (AudioAffect)(AudioAffect_Music |
                                                    AudioAffect_SystemSetting));
  }
  ...
  if (val != -1) {
    // Both 2D and 3D sound effects are sharing the same slider. However, there
    // is a relative slider that gets applied to one of these values to lower
    // that sound volume.
    ...
    TheAudio->setVolume(
        sound2DVolume,
        (AudioAffect)(AudioAffect_Sound | AudioAffect_SystemSetting));
    TheAudio->setVolume(
        sound3DVolume,
        (AudioAffect)(AudioAffect_Sound3D | AudioAffect_SystemSetting));
  ...
  if (val != -1) {
    ...
    TheAudio->setVolume(val / 100.0f, (AudioAffect)(AudioAffect_Speech |
                                                    AudioAffect_SystemSetting));
  }
...
static void showAdvancedOptions() { WinAdvancedDisplay->winHide(FALSE); }

static void acceptAdvancedOptions() { WinAdvancedDisplay->winHide(TRUE); }
...
static void cancelAdvancedOptions() {
  ...
  switch (TheGameLODManager->getStaticLODLevel()) {
    ...
    case STATIC_GAME_LOD_CUSTOM:
      ...
      break;
    default:
      DEBUG_ASSERTCRASH(FALSE, ("Tried to set comboBoxDetail to a value of %d ",
                                TheGameLODManager->getStaticLODLevel()));
  };
...
void OptionsMenuInit(WindowLayout *layout, void *userData) {
  ...
  comboBoxLANIPID =
      TheNameKeyGenerator->nameToKey(AsciiString("OptionsMenu.wnd:ComboBoxIP"));
  comboBoxLANIP = TheWindowManager->winGetWindowFromId(NULL, comboBoxLANIPID);
  comboBoxOnlineIPID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:ComboBoxOnlineIP"));
  comboBoxOnlineIP =
      TheWindowManager->winGetWindowFromId(NULL, comboBoxOnlineIPID);
  checkAlternateMouseID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:CheckAlternateMouse"));
  checkAlternateMouse =
      TheWindowManager->winGetWindowFromId(NULL, checkAlternateMouseID);
  sliderScrollSpeedID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:SliderScrollSpeed"));
  sliderScrollSpeed =
      TheWindowManager->winGetWindowFromId(NULL, sliderScrollSpeedID);
  comboBoxAntiAliasingID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:ComboBoxAntiAliasing"));
  comboBoxAntiAliasing =
      TheWindowManager->winGetWindowFromId(NULL, comboBoxAntiAliasingID);
  comboBoxResolutionID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:ComboBoxResolution"));
  comboBoxResolution =
      TheWindowManager->winGetWindowFromId(NULL, comboBoxResolutionID);
#ifdef _PLAYTEST
  if (comboBoxResolution) comboBoxResolution->winEnable(FALSE);
#endif _PLAYTEST
  comboBoxDetailID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:ComboBoxDetail"));
  comboBoxDetail = TheWindowManager->winGetWindowFromId(NULL, comboBoxDetailID);

  checkLanguageFilterID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:CheckLanguageFilter"));
  checkLanguageFilter =
      TheWindowManager->winGetWindowFromId(NULL, checkLanguageFilterID);
  checkSendDelayID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:CheckSendDelay"));
  checkSendDelay = TheWindowManager->winGetWindowFromId(NULL, checkSendDelayID);
  buttonFirewallRefreshID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:ButtonFirewallRefresh"));
  buttonFirewallRefresh =
      TheWindowManager->winGetWindowFromId(NULL, buttonFirewallRefreshID);
  checkDrawAnchorID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:CheckBoxDrawAnchor"));
  checkDrawAnchor =
      TheWindowManager->winGetWindowFromId(NULL, checkDrawAnchorID);
  checkMoveAnchorID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:CheckBoxMoveAnchor"));
  checkMoveAnchor =
      TheWindowManager->winGetWindowFromId(NULL, checkMoveAnchorID);

  // Replay camera
  checkSaveCameraID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:CheckBoxSaveCamera"));
  checkSaveCamera =
      TheWindowManager->winGetWindowFromId(NULL, checkSaveCameraID);
  checkUseCameraID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:CheckBoxUseCamera"));
  checkUseCamera = TheWindowManager->winGetWindowFromId(NULL, checkUseCameraID);

  //	// Speakers and 3-D Audio
  //	checkAudioSurroundID   = TheNameKeyGenerator->nameToKey( AsciiString(
  //"OptionsMenu.wnd:CheckAudioSurround" ) ); 	checkAudioSurround     =
  //TheWindowManager->winGetWindowFromId( NULL, checkAudioSurroundID );
  //	checkAudioHardwareID   = TheNameKeyGenerator->nameToKey( AsciiString(
  //"OptionsMenu.wnd:CheckAudioHardware" ) ); 	checkAudioHardware     =
  //TheWindowManager->winGetWindowFromId( NULL, checkAudioHardwareID );
  //
  // Volume Controls
  sliderMusicVolumeID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:SliderMusicVolume"));
  sliderMusicVolume =
      TheWindowManager->winGetWindowFromId(NULL, sliderMusicVolumeID);
  sliderSFXVolumeID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:SliderSFXVolume"));
  sliderSFXVolume =
      TheWindowManager->winGetWindowFromId(NULL, sliderSFXVolumeID);
  sliderVoiceVolumeID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:SliderVoiceVolume"));
  sliderVoiceVolume =
      TheWindowManager->winGetWindowFromId(NULL, sliderVoiceVolumeID);
  sliderGammaID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:SliderGamma"));
  sliderGamma = TheWindowManager->winGetWindowFromId(NULL, sliderGammaID);

  //	checkBoxLowTextureDetailID = TheNameKeyGenerator->nameToKey(
  //AsciiString( "OptionsMenu.wnd:CheckLowTextureDetail" ) );
  //	checkBoxLowTextureDetail      = TheWindowManager->winGetWindowFromId(
  //NULL, checkBoxLowTextureDetailID );

  WinAdvancedDisplayID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:WinAdvancedDisplayOptions"));
  WinAdvancedDisplay =
      TheWindowManager->winGetWindowFromId(NULL, WinAdvancedDisplayID);

  ButtonAdvancedAcceptID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:ButtonAdvanceAccept"));
  ButtonAdvancedAccept =
      TheWindowManager->winGetWindowFromId(NULL, ButtonAdvancedAcceptID);

  ButtonAdvancedCancelID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:ButtonAdvanceBack"));
  ButtonAdvancedCancel =
      TheWindowManager->winGetWindowFromId(NULL, ButtonAdvancedCancelID);

  sliderTextureResolutionID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:LowResSlider"));
  sliderTextureResolution =
      TheWindowManager->winGetWindowFromId(NULL, sliderTextureResolutionID);

  check3DShadowsID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:Check3DShadows"));
  check3DShadows = TheWindowManager->winGetWindowFromId(NULL, check3DShadowsID);

  check2DShadowsID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:Check2DShadows"));
  check2DShadows = TheWindowManager->winGetWindowFromId(NULL, check2DShadowsID);

  checkCloudShadowsID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:CheckCloudShadows"));
  checkCloudShadows =
      TheWindowManager->winGetWindowFromId(NULL, checkCloudShadowsID);

  checkGroundLightingID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:CheckGroundLighting"));
  checkGroundLighting =
      TheWindowManager->winGetWindowFromId(NULL, checkGroundLightingID);

  checkSmoothWaterID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:CheckSmoothWater"));
  checkSmoothWater =
      TheWindowManager->winGetWindowFromId(NULL, checkSmoothWaterID);

  checkExtraAnimationsID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:CheckExtraAnimations"));
  checkExtraAnimations =
      TheWindowManager->winGetWindowFromId(NULL, checkExtraAnimationsID);

  checkNoDynamicLodID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:CheckNoDynamicLOD"));
  checkNoDynamicLod =
      TheWindowManager->winGetWindowFromId(NULL, checkNoDynamicLodID);

  checkUnlockFpsID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:CheckUnlockFPS"));
  checkUnlockFps = TheWindowManager->winGetWindowFromId(NULL, checkUnlockFpsID);

  checkBuildingOcclusionID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:CheckBehindBuilding"));
  checkBuildingOcclusion =
      TheWindowManager->winGetWindowFromId(NULL, checkBuildingOcclusionID);

  checkPropsID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:CheckShowProps"));
  checkProps = TheWindowManager->winGetWindowFromId(NULL, checkPropsID);

  sliderParticleCapID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:ParticleCapSlider"));
  sliderParticleCap =
      TheWindowManager->winGetWindowFromId(NULL, sliderParticleCapID);

  WinAdvancedDisplay->winHide(TRUE);

  Color color = GameMakeColor(255, 255, 255, 255);

  enum AliasingMode { OFF = 0, LOW, HIGH, NUM_ALIASING_MODES };

  NameKeyType versionID = TheNameKeyGenerator->nameToKey(
      AsciiString("OptionsMenu.wnd:LabelVersion"));
  GameWindow *labelVersion =
      TheWindowManager->winGetWindowFromId(NULL, versionID);
  UnicodeString versionString;
  versionString.format(TheGameText->fetch("Version:Format2").str(),
                       (GetRegistryVersion() >> 16),
                       (GetRegistryVersion() & 0xffff));

  if (TheVersion->showFullVersion()) {
    if (TheVersion) {
      UnicodeString version;
      version.format(L"(%s) %s -- %s", versionString.str(),
                     TheVersion->getFullUnicodeVersion().str(),
                     TheVersion->getUnicodeBuildTime().str());
  ...
  Int defaultResIndex =
      0;  // index of default video mode that should always exist
  if (!selectedResolution.isEmpty()) {  // try to parse 2 integers out of string
    if (sscanf(selectedResolution.str(), "%d%d", &selectedXRes,
               &selectedYRes) != 2) {
      selectedXRes = 800;
      selectedYRes = 600;
    }
  ...
  for (i = 0; i < numResolutions; ++i) {
    ...
    if (xres == 800 &&
        yres == 600)  // keep track of default mode in case we need it.
      defaultResIndex = i;
    if (xres == selectedXRes && yres == selectedYRes) selectedResIndex = i;
  }
  ...
  GadgetComboBoxAddEntry(comboBoxDetail, TheGameText->fetch("GUI:Medium"),
                         color);
  GadgetComboBoxAddEntry(comboBoxDetail, TheGameText->fetch("GUI:Low"), color);
  GadgetComboBoxAddEntry(comboBoxDetail, TheGameText->fetch("GUI:Custom"),
                         color);
  ...
  if (test == "Yes" ||
      (test.isEmpty() && TheInGameUI->getDrawRMBScrollAnchor())) {
  ...
  if (test == "Yes" ||
      (test.isEmpty() && TheInGameUI->getMoveRMBScrollAnchor())) {
  ...
  //	// Audio Init shiznat
  //	GadgetCheckBoxSetChecked(checkAudioHardware,
  //TheAudio->getHardwareAccelerated());
  //	GadgetCheckBoxSetChecked(checkAudioSurround,
  //TheAudio->getSpeakerSurround());

  // set the mouse mode
  GadgetCheckBoxSetChecked(checkAlternateMouse,
                           TheGlobalData->m_useAlternateMouse);
  ...
  GadgetSliderSetPosition(sliderMusicVolume,
                          REAL_TO_INT(pref->getMusicVolume()));
  ...
  GadgetSliderSetPosition(sliderVoiceVolume,
                          REAL_TO_INT(pref->getSpeechVolume()));
  ...
  if ((TheGameLogic->isInGame() && TheGameLogic->getGameMode() != GAME_SHELL) ||
    ...
    if (comboBoxOnlineIP) comboBoxOnlineIP->winEnable(FALSE);
    checkSendDelay->winEnable(FALSE);
    buttonFirewallRefresh->winEnable(FALSE);

    if (comboBoxDetail) comboBoxDetail->winEnable(FALSE);
      ...
      switch (key) {
        ...
        case KEY_ESC: {
          ...
          if (BitTest(state, KEY_STATE_UP)) {
            ...
            GameWindow *button =
                TheWindowManager->winGetWindowFromId(window, buttonID);
...
WindowMsgHandledType OptionsMenuSystem(GameWindow *window, UnsignedInt msg,
  ...
  switch (msg) {
    ...
    case GWM_CREATE: {
      ...
      buttonBack = TheNameKeyGenerator->nameToKey(
          AsciiString("OptionsMenu.wnd:ButtonBack"));
      buttonDefaults = TheNameKeyGenerator->nameToKey(
          AsciiString("OptionsMenu.wnd:ButtonDefaults"));
      buttonAccept = TheNameKeyGenerator->nameToKey(
          AsciiString("OptionsMenu.wnd:ButtonAccept"));
      buttonKeyboardOptionsMenu = TheNameKeyGenerator->nameToKey(
          AsciiString("OptionsMenu.wnd:ButtonKeyboardOptions"));
    ...
    case GBM_SELECTED: {
      ...
      if (controlID == buttonBack) {
        ...
        if (!TheGameLogic->isInGame() || TheGameLogic->isInShellGame())
          destroyQuitMenu();  // if we're in a game, the change res then enter
                              // the same kind of game, we nee the quit menu to
                              // be gone.
        ...
        // setting the behavior to unknown will force the firewall helper to
        // detect the firewall behavior the next time we log into
        // gamespy/WOL/whatever.
        char num[16];
        num[0] = 0;
