...
void PopupJoinGameInit(WindowLayout *layout, void *userData) {
  ...
  NameKeyType staticTextGameNameID = TheNameKeyGenerator->nameToKey(
      AsciiString("PopupJoinGame.wnd:StaticTextGameName"));
  GameWindow *staticTextGameName =
      TheWindowManager->winGetWindowFromId(parentPopup, staticTextGameNameID);
  ...
  GameSpyStagingRoom *ourRoom = TheGameSpyInfo->findStagingRoomByID(
      TheGameSpyInfo->getCurrentStagingRoomID());
  if (ourRoom)
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType PopupJoinGameInput(GameWindow *window, UnsignedInt msg,
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType PopupJoinGameSystem(GameWindow *window, UnsignedInt msg,
                                         WindowMsgData mData1,
                                         WindowMsgData mData2) {
  switch (msg) {
    // --------------------------------------------------------------------------------------------
    ...
    case GEM_EDIT_DONE: {
      ...
      if (controlID == textEntryGamePasswordID) {
        ...
        GadgetTextEntrySetText(textEntryGamePassword,
