...
void RefreshGameInfoWindow(GameInfo *gameInfo, UnicodeString gameName) {
  static const Image *randomIcon =
      TheMappedImageCollection->findImageByName("GameinfoRANDOM");
  static const Image *observerIcon =
      TheMappedImageCollection->findImageByName("GameinfoOBSRVR");
  if (!gameInfoWindowLayout || !gameInfo) return;

  parent->winHide(FALSE);
  parent->winBringToTop();

  // Set the game name
  GadgetStaticTextSetText(staticTextGameName,
  ...
  if (it != TheMapCache->end()) {
    ...
    // can happen if the map will have to be transferred... so use the leaf name
    // (srj)
  ...
  for (Int i = 0; i < MAX_SLOTS; i++) {
    ...
    if (!slot || (slot->isOccupied() == FALSE)) continue;
    color = slot->getColor();
    ...
    if (slot->isAI()) {
      switch (slot->getState()) {
        case SLOT_EASY_AI: {
          addedRow = GadgetListBoxAddEntryText(listBoxPlayers,
        ...
        case SLOT_MED_AI: {
          addedRow = GadgetListBoxAddEntryText(
              listBoxPlayers, TheGameText->fetch("GUI:MediumAI"), playerColor,
              -1, 1);
          break;
        }
        case SLOT_BRUTAL_AI: {
      ...
      addedRow = GadgetListBoxAddEntryText(listBoxPlayers, slot->getName(),
                                           playerColor, -1, 1);
    }
    Int playerTemplate = slot->getPlayerTemplate();
    if (playerTemplate == PLAYERTEMPLATE_OBSERVER) {
      GadgetListBoxAddEntryImage(listBoxPlayers, observerIcon, addedRow, 0, 22,
                                 25);
    } else if (playerTemplate < 0 ||
               playerTemplate >=
                   ThePlayerTemplateStore->getPlayerTemplateCount()) {
      ///< @todo: When we get art that shows player's side, then we'll actually
      ///< draw the art instead of putting in text
      GadgetListBoxAddEntryImage(listBoxPlayers, randomIcon, addedRow, 0, 22,
                                 25);
      // GadgetListBoxAddEntryText(listBoxPlayers,TheGameText->fetch("GUI:???"),playerColor,addedRow,
      // 0);
    } else {
      const PlayerTemplate *fact =
          ThePlayerTemplateStore->getNthPlayerTemplate(playerTemplate);
      GadgetListBoxAddEntryImage(listBoxPlayers, fact->getSideIconImage(),
                                 addedRow, 0, 22, 25);
      // GadgetListBoxAddEntryText(listBoxPlayers,fact->getDisplayName(),playerColor,addedRow,
      // 0);
    }
  }
}
...
void GameInfoWindowInit(WindowLayout *layout, void *userData) {
  parentID =
      TheNameKeyGenerator->nameToKey("GameInfoWindow.wnd:ParentGameInfo");
  staticTextGameNameID =
      TheNameKeyGenerator->nameToKey("GameInfoWindow.wnd:StaticTextGameName");
  staticTextMapNameID =
      TheNameKeyGenerator->nameToKey("GameInfoWindow.wnd:StaticTextMapName");
  listBoxPlayersID =
      TheNameKeyGenerator->nameToKey("GameInfoWindow.wnd:ListBoxPlayers");
  winCratesID = TheNameKeyGenerator->nameToKey("GameInfoWindow.wnd:WinCrates");
  winSuperWeaponsID =
      TheNameKeyGenerator->nameToKey("GameInfoWindow.wnd:WinSuperWeapons");
  winFreeForAllID =
      TheNameKeyGenerator->nameToKey("GameInfoWindow.wnd:WinFreeForAll");

  parent = TheWindowManager->winGetWindowFromId(NULL, parentID);
  staticTextGameName =
      TheWindowManager->winGetWindowFromId(parent, staticTextGameNameID);
  staticTextMapName =
      TheWindowManager->winGetWindowFromId(parent, staticTextMapNameID);
  listBoxPlayers =
      TheWindowManager->winGetWindowFromId(parent, listBoxPlayersID);
  winCrates = TheWindowManager->winGetWindowFromId(parent, winCratesID);
  winSuperWeapons =
      TheWindowManager->winGetWindowFromId(parent, winSuperWeaponsID);
