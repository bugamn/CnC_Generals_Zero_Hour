...
static const char *gadgetsToHide[] = {
    "MapWindow",
    //"StaticTextTitle",
    "StaticTextTeam", "StaticTextFaction", "StaticTextColor",
...
static void showLANGameOptionsUnderlyingGUIElements(Bool show) {
  ShowUnderlyingGUIElements(show, layoutFilename, parentName, gadgetsToHide,
                            perPlayerGadgetsToHide);
  GameWindow *win = TheWindowManager->winGetWindowFromId(
      NULL,
      TheNameKeyGenerator->nameToKey("LanGameOptionsMenu.wnd:ButtonBack"));
  if (win) win->winEnable(show);
}
...
void LanMapSelectMenuInit(WindowLayout *layout, void *userData) {
  ...
  buttonBack = TheNameKeyGenerator->nameToKey(
      AsciiString("LanMapSelectMenu.wnd:ButtonBack"));
  buttonOK = TheNameKeyGenerator->nameToKey(
      AsciiString("LanMapSelectMenu.wnd:ButtonOK"));
  listboxMap = TheNameKeyGenerator->nameToKey(
      AsciiString("LanMapSelectMenu.wnd:ListboxMap"));
  winMapPreviewID = TheNameKeyGenerator->nameToKey(
      AsciiString("LanMapSelectMenu.wnd:WinMapPreview"));

  radioButtonSystemMapsID = TheNameKeyGenerator->nameToKey(
      "LanMapSelectMenu.wnd:RadioButtonSystemMaps");
  radioButtonUserMapsID = TheNameKeyGenerator->nameToKey(
      "LanMapSelectMenu.wnd:RadioButtonUserMaps");
  ...
  if (mapList) {
    ...
    populateMapListbox(mapList, usesSystemMapDir, TRUE,
      ...
      if (controlID == radioButtonSystemMapsID) {
        ...
        if (selected != -1) {
          ...
          if (it != TheMapCache->end()) {
            ...
            TheLAN->GetMyGame()->adjustSlotsForMap();  // BGC- adjust the slots
                                                       // for the new map.
          }
