...
static const char *rankNames[] = {
    ...
    "Major",   "Colonel",  "General",  "Brigadier",  "Commander",
};
...
static const Image *lookupRankImage(AsciiString side, Int rank) {
  ...
  if (rank < 0 || rank >= MAX_RANKS) return NULL;
  ...
  if (img) {
    ...
    DEBUG_LOG(
        ("*** Could not load rank image '%s' from TheMappedImageCollection!\n",
         fullImageName.str()));
  }
  return img;
}
...
void GetAdditionalDisconnectsFromUserFile(PSPlayerStats *stats) {
  ...
  if (TheGameSpyInfo->getAdditionalDisconnects() > 0 &&
      !getTotalDisconnectsFromFile(stats->id)) {
    DEBUG_LOG(("Clearing additional disconnects\n"));
    TheGameSpyInfo->clearAdditionalDisconnects();
  }
...
RankPoints::RankPoints(void) {
  ...
  m_ranks[RANK_CORPORAL] =
      TheGameSpyConfig->getPointsForRank(RANK_CORPORAL);  // 5
  m_ranks[RANK_SERGEANT] =
      TheGameSpyConfig->getPointsForRank(RANK_SERGEANT);  // 10
  m_ranks[RANK_LIEUTENANT] =
      TheGameSpyConfig->getPointsForRank(RANK_LIEUTENANT);  // 20
  m_ranks[RANK_CAPTAIN] =
      TheGameSpyConfig->getPointsForRank(RANK_CAPTAIN);                  // 50
  m_ranks[RANK_MAJOR] = TheGameSpyConfig->getPointsForRank(RANK_MAJOR);  // 100
  m_ranks[RANK_COLONEL] =
      TheGameSpyConfig->getPointsForRank(RANK_COLONEL);  // 200
  m_ranks[RANK_BRIGADIER_GENERAL] =
      TheGameSpyConfig->getPointsForRank(RANK_BRIGADIER_GENERAL);  // 500
  m_ranks[RANK_GENERAL] =
      TheGameSpyConfig->getPointsForRank(RANK_GENERAL);  // 1000
  m_ranks[RANK_COMMANDER_IN_CHIEF] =
      TheGameSpyConfig->getPointsForRank(RANK_COMMANDER_IN_CHIEF);  // 2000
  ...
  for (AsciiStringList::iterator sit = sidesList.begin();
       sit != sidesList.end(); ++sit) {
    for (Int i = 0; i < MAX_RANKS; ++i) lookupRankImage(*sit, i);
  }
  for (Int i = 0; i < MAX_RANKS; ++i) lookupRankImage("Random", i);
#endif
}
...
void BattleHonorTooltip(GameWindow *window, WinInstanceData *instData,
                        UnsignedInt mouse) {
  Int x, y, row, col;
  ...
  if (BitTest(battleHonor, BATTLE_HONOR_NOT_GAINED)) {
    if (BitTest(battleHonor, BATTLE_HONOR_STREAK_3))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorStreak3Disabled"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_LOYALTY_USA))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorLoyaltyUSADisabled"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_LOYALTY_CHINA))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorLoyaltyChinaDisabled"), -1,
          NULL, tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_LOYALTY_GLA))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorLoyaltyGLADisabled"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_BATTLE_TANK))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorBattleTankDisabled"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_AIR_WING))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorAirWingDisabled"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_ENDURANCE))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorEnduranceDisabled"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_CAMPAIGN_USA))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorCampaignUSADisabled"), -1,
          NULL, tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_CAMPAIGN_CHINA))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorCampaignChinaDisabled"), -1,
          NULL, tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_CAMPAIGN_GLA))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorCampaignGLADisabled"), -1,
          NULL, tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_BLITZ10))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorBlitz10Disabled"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_FAIR_PLAY))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorFairPlayDisabled"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_APOCALYPSE))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorApocalypseDisabled"), -1, NULL,
          tooltipWidth);
    /*
    else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_USA_B))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloUSABDisabled"), -1, NULL,
    tooltipWidth ); else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_USA_S))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloUSASDisabled"), -1, NULL,
    tooltipWidth ); else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_USA_G))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloUSAGDisabled"), -1, NULL,
    tooltipWidth ); else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_CHINA_B))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloChinaBDisabled"), -1, NULL,
    tooltipWidth ); else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_CHINA_S))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloChinaSDisabled"), -1, NULL,
    tooltipWidth ); else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_CHINA_G))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloChinaGDisabled"), -1, NULL,
    tooltipWidth ); else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_GLA_B))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloGLABDisabled"), -1, NULL,
    tooltipWidth ); else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_GLA_S))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloGLASDisabled"), -1, NULL,
    tooltipWidth ); else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_GLA_G))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloGLAGDisabled"), -1, NULL,
    tooltipWidth );
            */
    else if (BitTest(battleHonor, BATTLE_HONOR_CHALLENGE))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorChallengeDisabled"), -1, NULL,
          tooltipWidth);
  } else {
    if (BitTest(battleHonor, BATTLE_HONOR_LADDER_CHAMP))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorLadderChamp"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_STREAK_3))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorStreak3"), -1, NULL,
          tooltipWidth);
    // else if(BitTest(battleHonor, BATTLE_HONOR_STREAK_5))
    // TheMouse->setCursorTooltip(
    // TheGameText->fetch("TOOLTIP:BattleHonorStreak5"), -1, NULL, tooltipWidth
    // );
    else if (BitTest(battleHonor, BATTLE_HONOR_STREAK_10))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorStreak10"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_STREAK_25))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorStreak25"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_LOYALTY_USA))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorLoyaltyUSA"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_LOYALTY_CHINA))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorLoyaltyChina"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_LOYALTY_GLA))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorLoyaltyGLA"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_BATTLE_TANK))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorBattleTank"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_AIR_WING))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorAirWing"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_ENDURANCE))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorEndurance"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_CAMPAIGN_USA))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorCampaignUSA"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_CAMPAIGN_CHINA))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorCampaignChina"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_CAMPAIGN_GLA))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorCampaignGLA"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_BLITZ5))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorBlitz5"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_BLITZ10))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorBlitz10"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_FAIR_PLAY))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorFairPlay"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_APOCALYPSE))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorApocalypse"), -1, NULL,
          tooltipWidth);
    else if (BitTest(battleHonor, BATTLE_HONOR_OFFICERSCLUB))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorOfficersClub"), -1, NULL,
          tooltipWidth);
    /*
    else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_USA_B))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloUSAB"), -1, NULL, tooltipWidth );
    else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_USA_S))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloUSAS"), -1, NULL, tooltipWidth );
    else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_USA_G))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloUSAG"), -1, NULL, tooltipWidth );
    else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_CHINA_B))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloChinaB"), -1, NULL, tooltipWidth
    ); else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_CHINA_S))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloChinaS"), -1, NULL, tooltipWidth
    ); else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_CHINA_G))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloChinaG"), -1, NULL, tooltipWidth
    ); else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_GLA_B))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloGLAB"), -1, NULL, tooltipWidth );
    else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_GLA_S))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloGLAS"), -1, NULL, tooltipWidth );
    else if(BitTest(battleHonor, BATTLE_HONOR_SOLO_GLA_G))
            TheMouse->setCursorTooltip(
    TheGameText->fetch("TOOLTIP:BattleHonorSoloGLAG"), -1, NULL, tooltipWidth );
            */
    else if (BitTest(battleHonor, BATTLE_HONOR_CHALLENGE))
      TheMouse->setCursorTooltip(
          TheGameText->fetch("TOOLTIP:BattleHonorChallenge"), -1, NULL,
          tooltipWidth);
  }
}

static Int rowsToSkip = 0;
void ResetBattleHonorInsertion(void) { rowsToSkip = 0; }
void InsertBattleHonor(GameWindow *list, const Image *image, Bool enabled,
  ...
  Int height =
      MAX_BATTLE_HONOR_IMAGE_HEIGHT * (TheDisplay->getHeight() / 600.0f);
  ...
  if (!enabled) itemData |= BATTLE_HONOR_NOT_GAINED;

  GadgetListBoxAddEntryImage(list, image, row, column, height, width, TRUE,
                             color);
  GadgetListBoxSetItemData(list, (void *)itemData, row, column);

  /*
  ** removing text, since every place that adds text has alternate displays of
  the same thing if (!text.isEmpty())
  {
          GadgetListBoxAddEntryText(list, text, GameSpyColor[GSCOLOR_DEFAULT],
  row+1, column, TRUE ); GadgetListBoxSetItemData(list, (void *)itemData, row+1,
  column ); rowsToSkip++;
  }
  */
...
static void populateBattleHonors(const PSPlayerStats &stats, Int battleHonors,
                                 Int gamesInRow, Int lastGen, Int challenge,
                                 GameWindow *list) {
  if (!list) return;
  ...
  InsertBattleHonor(list, TheMappedImageCollection->findImageByName("FairPlay"),
                    isFairPlayer, BATTLE_HONOR_FAIR_PLAY, row, column);

  InsertBattleHonor(list,
                    TheMappedImageCollection->findImageByName("HonorAirWing"),
                    BitTest(battleHonors, BATTLE_HONOR_AIR_WING),
                    BATTLE_HONOR_AIR_WING, row, column);
  InsertBattleHonor(
      list, TheMappedImageCollection->findImageByName("HonorBattleTank"),
      BitTest(battleHonors, BATTLE_HONOR_BATTLE_TANK), BATTLE_HONOR_BATTLE_TANK,
      row, column);
  InsertBattleHonor(list,
                    TheMappedImageCollection->findImageByName("Apocalypse"),
                    BitTest(battleHonors, BATTLE_HONOR_APOCALYPSE),
  ...
  Bool isLoyal;
  isLoyal =
  ThePlayerTemplateStore->getNthPlayerTemplate(lastGen)->getSide().compareNoCase(
  "america") == 0 && gamesInRow >= 20; InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("Loyalty_USA"), isLoyal,
          BATTLE_HONOR_LOYALTY_USA, row, column);
  isLoyal =
  ThePlayerTemplateStore->getNthPlayerTemplate(lastGen)->getSide().compareNoCase(
  "china") == 0 && gamesInRow >= 20; InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("Loyalty_China"), isLoyal,
          BATTLE_HONOR_LOYALTY_CHINA, row, column);
  isLoyal =
  ThePlayerTemplateStore->getNthPlayerTemplate(lastGen)->getSide().compareNoCase(
  "gla") == 0 && gamesInRow >= 20; InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("Loyalty_GLA"), isLoyal,
          BATTLE_HONOR_LOYALTY_GLA, row, column);
          */

  // insertBattleHonor(list,
  // TheMappedImageCollection->findImageByName("Endurance"),
  // BitTest(battleHonors, BATTLE_HONOR_ENDURANCE), BATTLE_HONOR_ENDURANCE, row,
  // column);

  /*
  InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("Campaign_USA"),
  BitTest(battleHonors, BATTLE_HONOR_CAMPAIGN_USA), BATTLE_HONOR_CAMPAIGN_USA,
  row, column); InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("Campaign_China"),
  BitTest(battleHonors, BATTLE_HONOR_CAMPAIGN_CHINA),
          BATTLE_HONOR_CAMPAIGN_CHINA, row, column);
  InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("Campaign_GLA"),
  BitTest(battleHonors, BATTLE_HONOR_CAMPAIGN_GLA), BATTLE_HONOR_CAMPAIGN_GLA,
  row, column);
  */
...
Int CalculateRank(const PSPlayerStats &stats) {
  if (stats.id == 0 || !TheRankPointValues) return 0;
  PerGeneralMap::const_iterator it;
  Int rankPoints = 0;
  Int numGames = 0;
  ...
  rankPoints +=
      (numGames / 60) * TheRankPointValues->m_hourSpentOnlineMultiplier;
  ...
  if (BitTest(stats.battleHonors, BATTLE_HONOR_CAMPAIGN_USA |
                                      BATTLE_HONOR_CAMPAIGN_CHINA |
                                      BATTLE_HONOR_CAMPAIGN_GLA)) {
    rankPoints += 1 * TheRankPointValues->m_completedSoloCampaigns;
  }

  rankPoints = max(0, rankPoints);  // clip off negative values, since discons
                                    // can push us below 0.

  return rankPoints;
}

static GameWindow *findWindow(GameWindow *parent, AsciiString baseWindow,
                              AsciiString gadgetName) {
  AsciiString fullPath;
  fullPath.format("%s:%s", baseWindow.str(), gadgetName.str());
  GameWindow *res =
      TheWindowManager->winGetWindowFromId(parent, NAMEKEY(fullPath));
...
void PopulatePlayerInfoWindows(AsciiString parentWindowName) {
  ...
  if (parentWindowName == "PopupPlayerInfo.wnd") {
    ...
    if (lookAtPlayerID <= 0 || !parent) return;
  }
  ...
  Int i = 0;
  while (rankPoints >= TheRankPointValues->m_ranks[i + 1]) ++i;
  currentRank = i;
  ...
  if (win) {
    ...
    uStr.format(TheGameText->fetch("GUI:PlayerStatistics"),
                lookAtPlayerName.c_str(), TheGameText->fetch(localeID).str());
  ...
  win = findWindow(NULL, parentWindowName, "StaticTextFavoriteSideValue");
  {
    ...
    if (numGames == 0)
      ...
      const PlayerTemplate *fac =
          ThePlayerTemplateStore->getNthPlayerTemplate(favorite);
      if (fac) {
        AsciiString side;
  ...
  if (win) {
    Int numGames = 0;
    for (it = stats.buildingsKilled.begin(); it != stats.buildingsKilled.end();
         ++it) {
      numGames += it->second;
    }
    uStr.format(L"%d", numGames);
  ...
  if (win) {
    Int numGames = 0;
    for (it = stats.buildingsLost.begin(); it != stats.buildingsLost.end();
         ++it) {
      numGames += it->second;
    }
    uStr.format(L"%d", numGames);
  ...
  if (win) {
    Int numGames = 0;
    for (it = stats.buildingsBuilt.begin(); it != stats.buildingsBuilt.end();
         ++it) {
      numGames += it->second;
    }
    uStr.format(L"%d", numGames);
  ...
  if (win) {
    uStr.format(TheGameText->fetch("GUI:WinPercent"),
                REAL_TO_INT(numWins / (Real)numGames * 100.0f));
    GadgetStaticTextSetText(win, uStr);
  }
  ...
  if (win && TheRankPointValues) {
    if (currentRank == MAX_RANKS - 1) {
      ...
      GadgetProgressBarSetProgress(
          win, 100 *
                   INT_TO_REAL(rankPoints -
                               TheRankPointValues->m_ranks[currentRank]) /
                   (TheRankPointValues->m_ranks[currentRank + 1] -
                    TheRankPointValues->m_ranks[currentRank]));
    }
  }
  ...
  if (win && TheRankPointValues) {
    if (rankPoints == 0)
      win->winSetEnabledImage(0,
                              lookupRankImage(AsciiString::TheEmptyString, 0));
    else
  ...
  if (win) {
    if (weHaveStats) {
      ...
      win->winHide(FALSE);
      GadgetStaticTextSetText(win,
                              TheGameText->fetch("GUI:FetchingPlayerInfo"));
    }
  }

  win = findWindow(NULL, parentWindowName, "ListboxInfo");
  if (win) {
    populateBattleHonors(stats, stats.battleHonors,
                         stats.gamesInRowWithLastGeneral, stats.lastGeneral,
                         stats.challengeMedals, win);
  }
}
    ...
    if (TheGameSpyPSMessageQueue->getResponse(resp)) {
      switch (resp.responseType) {
        ...
        case PSResponse::PSRESPONSE_PLAYERSTATS: {
          DEBUG_LOG(
              ("LocalProfileID %d, resp.player.id %d, resp.player.locale %d\n",
               TheGameSpyInfo->getLocalProfileID(), resp.player.id,
               resp.player.locale));
          /*
          if(resp.player.id == TheGameSpyInfo->getLocalProfileID() &&
          resp.player.locale < LOC_MIN)
          {
          ...
          */
          if (resp.player.id == TheGameSpyInfo->getLocalProfileID()) {
            ...
            Int wins = 0, losses = 0;
            PerGeneralMap::const_iterator it;
            for (it = resp.player.wins.begin(); it != resp.player.wins.end();
                 ++it) {
              wins += it->second;
            }
            for (it = resp.player.losses.begin();
                 it != resp.player.losses.end(); ++it) {
              losses += it->second;
            }
            ...
            Int numGames = 0;
            Int favorite = 0;
            for (it = resp.player.games.begin(); it != resp.player.games.end();
                 ++it) {
              if (it->second >= numGames) {
                numGames = it->second;
                favorite = it->first;
            ...
            Bool isPreorder = TheGameSpyInfo->didPlayerPreorder(
                TheGameSpyInfo->getLocalProfileID());
            req.statsToPush.preorder = isPreorder;

            DEBUG_LOG(
                ("PEERREQUEST_PUSHSTATS: stats will be %d,%d,%d,%d,%d,%d\n",
                 req.statsToPush.locale, req.statsToPush.wins,
                 req.statsToPush.losses, req.statsToPush.rankPoints,
                 req.statsToPush.side, req.statsToPush.preorder));
          ...
          while (it != TheGameSpyInfo->getPlayerInfoMap()->end()) {
            ...
            if (info && info->m_profileID == resp.player.id) {
              ...
              Int numGames = 0;
              Int favorite = 0;
              for (it = resp.player.games.begin();
                   it != resp.player.games.end(); ++it) {
                if (it->second >= numGames) {
                  numGames = it->second;
                  favorite = it->first;
                }
...
void GameSpyPlayerInfoOverlayInit(WindowLayout *layout, void *userData) {
  ...
  buttonCloseID =
      TheNameKeyGenerator->nameToKey("PopupPlayerInfo.wnd:ButtonClose");
  buttonBuddiesID =
      TheNameKeyGenerator->nameToKey("PopupPlayerInfo.wnd:ButtonCommunicator");
  listboxInfoID =
      TheNameKeyGenerator->nameToKey("PopupPlayerInfo.wnd:ListboxInfo");
  // buttonOptionsID = TheNameKeyGenerator->nameToKey(
  // "PopupPlayerInfo.wnd:ButtonOptions" );
  buttonSetLocaleID =
      TheNameKeyGenerator->nameToKey("PopupPlayerInfo.wnd:ButtonSetLocale");
  buttonDeleteAccountID =
      TheNameKeyGenerator->nameToKey("PopupPlayerInfo.wnd:ButtonDeleteAccount");
  checkBoxAsianFontID =
      TheNameKeyGenerator->nameToKey("PopupPlayerInfo.wnd:CheckBoxAsianText");
  checkBoxNonAsianFontID = TheNameKeyGenerator->nameToKey(
      "PopupPlayerInfo.wnd:CheckBoxNonAsianText");
  ...
  // buttonbuttonOptions = TheWindowManager->winGetWindowFromId( parent,
  // buttonOptionsID);
  buttonSetLocale =
      TheWindowManager->winGetWindowFromId(parent, buttonSetLocaleID);
  buttonDeleteAccount =
      TheWindowManager->winGetWindowFromId(parent, buttonDeleteAccountID);
  checkBoxAsianFont =
      TheWindowManager->winGetWindowFromId(parent, checkBoxAsianFontID);
  checkBoxNonAsianFont =
      TheWindowManager->winGetWindowFromId(parent, checkBoxNonAsianFontID);
  ...
  // GadgetListBoxAddEntryText(listboxInfo, UnicodeString(L"Working"),
  // GameSpyColor[GSCOLOR_DEFAULT], -1);
  ...
  if (lookAtPlayerID == TheGameSpyInfo->getLocalProfileID()) {
    ...
    buttonDeleteAccount->winHide(
        TRUE);  // set back to false when we have this worked out.
  ...
  GadgetCheckBoxSetChecked(checkBoxNonAsianFont,
                           !pref.getDisallowNonAsianText());
  ...
  if (GetVersionEx(&osvi)) {  // check if we're running Win9x variant since they
    ...
    if (osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS) {
      if (checkBoxAsianFont) checkBoxAsianFont->winEnable(FALSE);
      if (checkBoxNonAsianFont) checkBoxNonAsianFont->winEnable(FALSE);
    }
  }
...
void GameSpyPlayerInfoOverlayUpdate(WindowLayout *layout, void *userData) {
  if (raiseMessageBox) RaiseGSMessageBox();
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType GameSpyPlayerInfoOverlayInput(GameWindow *window,
                                                   UnsignedInt msg,
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType GameSpyPlayerInfoOverlaySystem(GameWindow *window,
                                                    UnsignedInt msg,
                                                    WindowMsgData mData1,
                                                    WindowMsgData mData2) {
  ...
  switch (msg) {
    ...
    case GBM_SELECTED: {
      ...
      //					RefreshGameListBoxes();
      //					GameSpyCloseOverlay(
      //GSOVERLAY_PLAYERINFO ); 					GameSpyOpenOverlay( GSOVERLAY_OPTIONS );
      //				}
        ...
        RefreshGameListBoxes();
        GameSpyCloseOverlay(GSOVERLAY_PLAYERINFO);
        MessageBoxYesNo(TheGameText->fetch("GUI:DeleteAccount"),
                        TheGameText->fetch("GUI:AreYouSureDeleteAccount"),
                        messageBoxYes, NULL);
        ...
        pref.write();
        if (TheGameSpyInfo) TheGameSpyInfo->setDisallowAsianText(isChecked);
        if (isChecked && !GadgetCheckBoxIsChecked(checkBoxNonAsianFont)) {
          ...
          pref.write();
          if (TheGameSpyInfo) TheGameSpyInfo->setDisallowNonAsianText(FALSE);
        }
        ...
        pref.write();
        if (TheGameSpyInfo) TheGameSpyInfo->setDisallowNonAsianText(isChecked);
        if (isChecked && !GadgetCheckBoxIsChecked(checkBoxAsianFont)) {
          ...
          pref.write();
          if (TheGameSpyInfo) TheGameSpyInfo->setDisallowAsianText(FALSE);
        }
      }
