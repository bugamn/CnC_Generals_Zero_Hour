...
// At this point I don't give a damn about how good this way is.  I'm doing it
// anyway.
enum {
...
void InitBuddyControls(Int type) {
  ...
  switch (type) {
    ...
    case BUDDY_WINDOW_BUDDIES:
      buddyControls.textEntryEditID = TheNameKeyGenerator->nameToKey(
          AsciiString("WOLBuddyOverlay.wnd:TextEntryChat"));
      buddyControls.textEntryEdit = TheWindowManager->winGetWindowFromId(
          NULL, buddyControls.textEntryEditID);
      buddyControls.listboxBuddiesID = TheNameKeyGenerator->nameToKey(
          AsciiString("WOLBuddyOverlay.wnd:ListboxBuddies"));
      buddyControls.listboxChatID = TheNameKeyGenerator->nameToKey(
          AsciiString("WOLBuddyOverlay.wnd:ListboxBuddyChat"));
      buddyControls.listboxBuddies = TheWindowManager->winGetWindowFromId(
          NULL, buddyControls.listboxBuddiesID);
      buddyControls.listboxChat = TheWindowManager->winGetWindowFromId(
          NULL, buddyControls.listboxChatID);
      GadgetTextEntrySetText(buddyControls.textEntryEdit,
                             UnicodeString.TheEmptyString);
      buddyControls.isInit = TRUE;
      break;
    case BUDDY_WINDOW_DIPLOMACY:
      buddyControls.textEntryEditID = TheNameKeyGenerator->nameToKey(
          AsciiString("Diplomacy.wnd:TextEntryChat"));
      buddyControls.textEntryEdit = TheWindowManager->winGetWindowFromId(
          NULL, buddyControls.textEntryEditID);
      buddyControls.listboxBuddiesID = TheNameKeyGenerator->nameToKey(
          AsciiString("Diplomacy.wnd:ListboxBuddies"));
      buddyControls.listboxChatID = TheNameKeyGenerator->nameToKey(
          AsciiString("Diplomacy.wnd:ListboxBuddyChat"));
      buddyControls.listboxBuddies = TheWindowManager->winGetWindowFromId(
          NULL, buddyControls.listboxBuddiesID);
      buddyControls.listboxChat = TheWindowManager->winGetWindowFromId(
          NULL, buddyControls.listboxChatID);
    ...
    default:
      DEBUG_ASSERTCRASH(
          FALSE, ("Well, you really shouldn't have gotten here, if you really "
                  "care about GUI Bugs, search for this string, you you don't "
                  "care, call chris (who probably doesn't care either"));
  }
}
  ...
  switch (msg) {
    case GLM_RIGHT_CLICKED: {
      ...
      if (controlID == buddyControls.listboxBuddiesID) {
        ...
        GPProfile profileID =
            (GPProfile)GadgetListBoxGetItemData(control, rc->pos, 0);
        RCItemType itemType =
            (RCItemType)(Int)GadgetListBoxGetItemData(control, rc->pos, 1);
        ...
        if (itemType == ITEM_BUDDY)
          ...
          rcLayout = TheWindowManager->winCreateLayout(
              AsciiString("Menus/RCBuddyRequestMenu.wnd"));
        else
          rcLayout = TheWindowManager->winCreateLayout(
    ...
    case GEM_EDIT_DONE: {
      GameWindow *control = (GameWindow *)mData1;
      Int controlID = control->winGetWindowId();
      if (controlID != buddyControls.textEntryEditID) return MSG_IGNORED;

      // see if someone's selected
      Int selected = -1;
      GadgetListBoxGetSelected(buddyControls.listboxBuddies, &selected);
      if (selected >= 0) {
        GPProfile selectedProfile = (GPProfile)GadgetListBoxGetItemData(
            buddyControls.listboxBuddies, selected);
        BuddyInfoMap *m = TheGameSpyInfo->getBuddyMap();
        BuddyInfoMap::iterator recipIt = m->find(selectedProfile);
        if (recipIt == m->end()) break;

        DEBUG_LOG(("Trying to send a buddy message to %d.\n", selectedProfile));
        if (TheGameSpyGame && TheGameSpyGame->isInGame() &&
            TheGameSpyGame->isGameInProgress() &&
            !ThePlayerList->getLocalPlayer()->isPlayerActive()) {
          DEBUG_LOG(("I'm dead - gotta look for cheats.\n"));
          for (Int i = 0; i < MAX_SLOTS; ++i) {
            DEBUG_LOG(("Slot[%d] profile is %d\n", i,
                       TheGameSpyGame->getGameSpySlot(i)->getProfileID()));
            if (TheGameSpyGame->getGameSpySlot(i)->getProfileID() ==
                selectedProfile) {
              // can't send to someone in our game if we're dead/observing.
              // security breach and all that.  no seances for you.
        ...
        if (buddyControls.listboxChat) {
          GadgetListBoxAddEntryText(
              buddyControls.listboxChat,
              TheGameText->fetch("Buddy:SelectBuddyToChat"),
              GameSpyColor[GSCOLOR_DEFAULT], -1, -1);
        }
      }
      break;
...
static void insertChat(BuddyMessage msg) {
  if (buddyControls.listboxChat) {
    ...
    UnicodeString s;
    // UnicodeString timeStr = UnicodeString(_wctime( (const time_t
    // *)&msg.m_timestamp ));
    UnicodeString timeStr;
    if (localSender /*&& recipientIt != m->end()*/) {
      s.format(L"[%hs -> %hs] %s", TheGameSpyInfo->getLocalBaseName().str(),
               msg.m_recipientNick.str(), msg.m_message.str());
      Int index =
          GadgetListBoxAddEntryText(buddyControls.listboxChat, s,
                                    GameSpyColor[GSCOLOR_PLAYER_SELF], -1, -1);
      GadgetListBoxAddEntryText(buddyControls.listboxChat, timeStr,
                                GameSpyColor[GSCOLOR_PLAYER_SELF], index, 1);
      ...
      if (!msg.m_senderID) {
        ...
        Int index =
            GadgetListBoxAddEntryText(buddyControls.listboxChat, s,
                                      GameSpyColor[GSCOLOR_DEFAULT], -1, -1);
        GadgetListBoxAddEntryText(buddyControls.listboxChat, timeStr,
                                  GameSpyColor[GSCOLOR_DEFAULT], index, 1);
      } else {
        s.format(L"[%hs] %s", msg.m_senderNick.str(), msg.m_message.str());
        Int index = GadgetListBoxAddEntryText(
            buddyControls.listboxChat, s, GameSpyColor[GSCOLOR_PLAYER_BUDDY],
            -1, -1);
        GadgetListBoxAddEntryText(buddyControls.listboxChat, timeStr,
                                  GameSpyColor[GSCOLOR_PLAYER_BUDDY], index, 1);
      }
    }
  }
}
...
void updateBuddyInfo(void) {
  ...
  if (!buddyControls.isInit) return;

  int selected;
  GPProfile selectedProfile = 0;
  int visiblePos =
      GadgetListBoxGetTopVisibleEntry(buddyControls.listboxBuddies);

  GadgetListBoxGetSelected(buddyControls.listboxBuddies, &selected);
  if (selected >= 0)
    selectedProfile = (GPProfile)GadgetListBoxGetItemData(
        buddyControls.listboxBuddies, selected);
  ...
  for (bIt = buddies->begin(); bIt != buddies->end(); ++bIt) {
    ...
    formatStr.translate(
        info.m_name.str());  //, info.m_status, info.m_statusString.str(),
                             //info.m_locationString.str());
    ...
    int index = GadgetListBoxAddEntryText(buddyControls.listboxBuddies,
                                          formatStr, nameColor, -1, -1);
    ...
    if (!info.m_statusString.compareNoCase(L"Offline") ||
      ...
      GroupRoomMap::iterator gIt = TheGameSpyInfo->getGroupRoomList()->find(
          _wtoi(info.m_locationString.str()));
    ...
    GadgetListBoxAddEntryText(buddyControls.listboxBuddies, formatStr,
                              GameSpyColor[GSCOLOR_DEFAULT], index, 1);
    GadgetListBoxSetItemData(buddyControls.listboxBuddies, (void *)(profileID),
                             index, 0);
    GadgetListBoxSetItemData(buddyControls.listboxBuddies, (void *)(ITEM_BUDDY),
                             index, 1);

    if (profileID == selectedProfile) selected = index;
  }
  ...
  for (bIt = buddies->begin(); bIt != buddies->end(); ++bIt) {
    ...
    GadgetListBoxAddEntryText(buddyControls.listboxBuddies, formatStr,
                              GameSpyColor[GSCOLOR_DEFAULT], index, 1);
    GadgetListBoxSetItemData(buddyControls.listboxBuddies,
                             (void *)(ITEM_REQUEST), index, 1);
    ...
    if (TheGameSpyBuddyMessageQueue->getResponse(resp)) {
      switch (resp.buddyResponseType) {
        ...
        case BuddyResponse::BUDDYRESPONSE_DISCONNECT: {
          ...
          showNotificationBox(AsciiString::TheEmptyString,
        ...
        case BuddyResponse::BUDDYRESPONSE_MESSAGE: {
          ...
          UnicodeString s;
          s.format(TheGameText->fetch("Buddy:MessageNotification"), nick.str(),
                   snippet.str());
          lastNotificationWasStatus = FALSE;
          numOnlineInNotification = 0;
        ...
        case BuddyResponse::BUDDYRESPONSE_REQUEST: {
          ...
          lastNotificationWasStatus = FALSE;
          numOnlineInNotification = 0;
          showNotificationBox(info.m_name,
                              TheGameText->fetch("Buddy:AddNotification"));
        } break;
        case BuddyResponse::BUDDYRESPONSE_STATUS: {
          ...
          info.m_statusString = UnicodeString(
              MultiByteToWideCharSingleLine(resp.arg.status.statusString)
                  .c_str());
          info.m_locationString = UnicodeString(
              MultiByteToWideCharSingleLine(resp.arg.status.location).c_str());
          (*m)[resp.profile] = info;
          ...
          if ((newStatus == GP_OFFLINE && seenPreviously) ||
              (newStatus == GP_ONLINE &&
               (oldStatus == GP_OFFLINE || !seenPreviously)))
          // if (!info.m_statusString.compareNoCase(L"Offline") ||
          //! info.m_statusString.compareNoCase(L"Online"))
          {
            // insert status into box
            ...
            if (newStatus != GP_OFFLINE) ++numOnlineInNotification;
            ...
            showNotificationBox(info.m_name,
                                TheGameText->fetch("Buddy:OnlineNotification"));
          }
        } break;
      }
    }
...
void showNotificationBox(AsciiString nick, UnicodeString message) {
  ...
  //		return;
  if (!noticeLayout)
    noticeLayout = TheWindowManager->winCreateLayout(
        "Menus/PopupBuddyListNotification.wnd");
  noticeLayout->hide(FALSE);
  if (buttonNotificationID == NAMEKEY_INVALID) {
    buttonNotificationID = TheNameKeyGenerator->nameToKey(
        "PopupBuddyListNotification.wnd:ButtonNotification");
  }
  GameWindow *win =
      TheWindowManager->winGetWindowFromId(NULL, buttonNotificationID);
  if (!win) {
    deleteNotificationBox();
    return;
  }
  ...
  if (nick.isNotEmpty()) message.format(message, nick.str());
...
void WOLBuddyOverlayInit(WindowLayout *layout, void *userData) {
  parentID = TheNameKeyGenerator->nameToKey(
      AsciiString("WOLBuddyOverlay.wnd:BuddyMenuParent"));
  buttonHideID = TheNameKeyGenerator->nameToKey(
      AsciiString("WOLBuddyOverlay.wnd:ButtonHide"));
  buttonAddBuddyID = TheNameKeyGenerator->nameToKey(
      AsciiString("WOLBuddyOverlay.wnd:ButtonAdd"));
  buttonDeleteBuddyID = TheNameKeyGenerator->nameToKey(
      AsciiString("WOLBuddyOverlay.wnd:ButtonDelete"));
  // textEntryID = TheNameKeyGenerator->nameToKey( AsciiString(
  // "WOLBuddyOverlay.wnd:TextEntryChat" ) ); listboxBuddyID =
  // TheNameKeyGenerator->nameToKey( AsciiString(
  // "WOLBuddyOverlay.wnd:ListboxBuddies" ) ); listboxChatID =
  // TheNameKeyGenerator->nameToKey( AsciiString(
  // "WOLBuddyOverlay.wnd:ListboxBuddyChat" ) );
  buttonAcceptBuddyID = TheNameKeyGenerator->nameToKey(
      AsciiString("WOLBuddyOverlay.wnd:ButtonYes"));
  buttonDenyBuddyID = TheNameKeyGenerator->nameToKey(
      AsciiString("WOLBuddyOverlay.wnd:ButtonNo"));
  radioButtonBuddiesID = TheNameKeyGenerator->nameToKey(
      AsciiString("WOLBuddyOverlay.wnd:RadioButtonBuddies"));
  radioButtonIgnoreID = TheNameKeyGenerator->nameToKey(
      AsciiString("WOLBuddyOverlay.wnd:RadioButtonIgnore"));
  parentBuddiesID = TheNameKeyGenerator->nameToKey(
      AsciiString("WOLBuddyOverlay.wnd:BuddiesParent"));
  parentIgnoreID = TheNameKeyGenerator->nameToKey(
      AsciiString("WOLBuddyOverlay.wnd:IgnoreParent"));
  listboxIgnoreID = TheNameKeyGenerator->nameToKey(
      AsciiString("WOLBuddyOverlay.wnd:ListboxIgnore"));

  parent = TheWindowManager->winGetWindowFromId(NULL, parentID);
  buttonHide = TheWindowManager->winGetWindowFromId(parent, buttonHideID);
  buttonAddBuddy =
      TheWindowManager->winGetWindowFromId(parent, buttonAddBuddyID);
  buttonDeleteBuddy =
      TheWindowManager->winGetWindowFromId(parent, buttonDeleteBuddyID);
  //	textEntry = TheWindowManager->winGetWindowFromId( parent,  textEntryID);
  // listboxBuddy = TheWindowManager->winGetWindowFromId( parent,
  // listboxBuddyID); listboxChat = TheWindowManager->winGetWindowFromId(
  // parent,  listboxChatID);
  buttonAcceptBuddy =
      TheWindowManager->winGetWindowFromId(parent, buttonAcceptBuddyID);
  buttonDenyBuddy =
      TheWindowManager->winGetWindowFromId(parent, buttonDenyBuddyID);
  radioButtonBuddies =
      TheWindowManager->winGetWindowFromId(parent, radioButtonBuddiesID);
  radioButtonIgnore =
      TheWindowManager->winGetWindowFromId(parent, radioButtonIgnoreID);
...
void WOLBuddyOverlayUpdate(WindowLayout *layout, void *userData) {
  if (!TheGameSpyBuddyMessageQueue ||
      !TheGameSpyBuddyMessageQueue->isConnected())
        ...
        case KEY_ESC: {
          ...
          if (BitTest(state, KEY_STATE_UP)) {
            TheWindowManager->winSendSystemMsg(
                window, GBM_SELECTED, (WindowMsgData)buttonHide, buttonHideID);
  ...
  switch (msg) {
    ...
    case GLM_RIGHT_CLICKED: {
      ...
      if (controlID == listboxIgnoreID) {
        ...
        GPProfile profileID =
            (GPProfile)GadgetListBoxGetItemData(control, rc->pos);
        UnicodeString nick = GadgetListBoxGetText(control, rc->pos);
        BuddyInfoMap *buddies = TheGameSpyInfo->getBuddyMap();
        BuddyInfoMap::iterator bIt;
        ...
        rcData->m_id = profileID;
        rcData->m_nick.translate(nick);
        rcData->m_itemType = (isBuddy)
                                 ? ITEM_BUDDY
                                 : ((isRequest) ? ITEM_REQUEST : ITEM_NONBUDDY);
        setUnignoreText(rcLayout, rcData->m_nick, rcData->m_id);
        rcMenu->winSetUserData((void *)rcData);
        TheWindowManager->winSetLoneWindow(rcMenu);
      }
    ...
    case GBM_SELECTED: {
      ...
      if (controlID == buttonHideID) {
        ...
        */
        ...
        if (selected >= 0)
                selectedName =
        TheNameKeyGenerator->keyToName((NameKeyType)(int)GadgetListBoxGetItemData(listbox,
        selected));
    ...
    case GLM_DOUBLE_CLICKED: {
      ...
      if( controlID == listboxBuddyID )
      {
              ...
              if (rowSelected >= 0)
              {
                      ...
                      GameWindow *listboxWindow =
      TheWindowManager->winGetWindowFromId( parent, listboxBuddyID );

                              // get text of buddy name
                      buddyName = GadgetListBoxGetText( listboxWindow,
      rowSelected,0 ); GPProfile buddyID = (GPProfile)GadgetListBoxGetItemData(
      listboxWindow, rowSelected, 0 );
                      ...
                      if (index >= 0)
                      {
                              ...
                              gpGetBuddyStatus(TheGPConnection, rowSelected,
      &status);

                              UnicodeString string;
                              string.format(L"To join %s in %hs:",
      buddyName.str(), status.locationString); GameSpyAddText(string,
      GSCOLOR_DEFAULT);

                              if (status.status == GP_CHATTING)
                              {
                                      AsciiString location =
      status.locationString; AsciiString val; location.nextToken(&val, "/");
                                      ...
                                      if (!val.isEmpty())
                                      {
                                              ...
                                              if
      (TheGameSpyChat->getCurrentGroupRoomID() == groupRoom)
                                              {
                                                      // already there
                                                      string.format(L"
      nothing"); GameSpyAddText(string, GSCOLOR_DEFAULT);
                                              }
                                              else
                                              {
                                                      GroupRoomMap *rooms =
      TheGameSpyChat->getGroupRooms(); if (rooms)
                                                      {
                                                              Bool needToJoin =
      true; GroupRoomMap::iterator it = rooms->find(groupRoom); if (it !=
      rooms->end())
                                                              {
                                                                      // he's in
      a different room if (TheGameSpyChat->getCurrentGroupRoomID())
                                                                      {
                                                                              string.format(L"
      leave group room"); GameSpyAddText(string, GSCOLOR_DEFAULT);

                                                                              TheGameSpyChat->leaveRoom(GroupRoom);
                                                                      }
                                                                      else if
      (TheGameSpyGame->isInGame())
                                                                      {
                                                                              if
      (TheGameSpyGame->isGameInProgress())
                                                                              {
      ...
      (needToJoin)
                                                                      {
                                                                              string.format(L"
      join lobby %d", groupRoom); TheGameSpyChat->joinGroupRoom(groupRoom);
                                                                              GameSpyAddText(string,
      GSCOLOR_DEFAULT);
                                                                      }
                                                              }
                                                      }
                      ...
                      {
                              DEBUG_CRASH(("No buddy associated with that
      ProfileID")); GameSpyUpdateBuddyOverlay();
                      }
              }
      }
      */
...
WindowMsgHandledType PopupBuddyNotificationSystem(GameWindow *window,
                                                  UnsignedInt msg,
                                                  WindowMsgData mData1,
                                                  WindowMsgData mData2) {
  switch (msg) {
...
void RequestBuddyAdd(Int profileID, AsciiString nick) {
  ...
  message.m_recipientID = TheGameSpyInfo->getLocalProfileID();
  message.m_recipientNick = TheGameSpyInfo->getLocalBaseName();
  message.m_message.format(TheGameText->fetch("Buddy:InviteSentToPlayer"),
                           nick.str());
...
WindowMsgHandledType WOLBuddyOverlayRCMenuSystem(GameWindow *window,
                                                 UnsignedInt msg,
                                                 WindowMsgData mData1,
                                                 WindowMsgData mData2) {
  switch (msg) {
    ...
    case GBM_SELECTED: {
      ...
      if (!rcData) break;
      DEBUG_ASSERTCRASH(rcData,
                        ("WOLBuddyOverlayRCMenuSystem GBM_SELECTED:: we're "
                         "attempting to read the GameSpyRCMenuData from the "
                         "window, but the data's not there"));
      ...
      if (controlID == buttonAddID) {
        ...
        if (isRequest) {
          ...
          // if the profile ID is not from a buddy and we're okaying his
          // request, then request to add him to our list automatically CLH
          // 2-18-03
        ...
        BuddyInfoMap *buddies = (isBuddy)
                                    ? TheGameSpyInfo->getBuddyMap()
                                    : TheGameSpyInfo->getBuddyRequestMap();
...
void setUnignoreText(WindowLayout *layout, AsciiString nick, GPProfile id) {
  ...
  GameWindow *win =
      TheWindowManager->winGetWindowFromId(layout->getFirstWindow(), ID);
  if (win) {
...
void refreshIgnoreList(void) {
  ...
  while (it != tempMap.end()) {
    ...
    Int pos = GadgetListBoxAddEntryText(listboxIgnore, name,
                                        GameMakeColor(255, 100, 100, 255), -1);
  ...
  //TheGameSpyInfo->getPlayerInfoMap()->find(aName); 	if (it !=
  //TheGameSpyInfo->getPlayerInfoMap()->end()) 		profileID =
  //it->second.m_profileID;
}
