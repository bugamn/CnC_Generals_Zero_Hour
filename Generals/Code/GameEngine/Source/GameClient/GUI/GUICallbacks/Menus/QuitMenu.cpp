...
static void initGadgetsFullQuit(void) {
  buttonExit =
      TheNameKeyGenerator->nameToKey(AsciiString("QuitMenu.wnd:ButtonExit"));
  buttonRestart =
      TheNameKeyGenerator->nameToKey(AsciiString("QuitMenu.wnd:ButtonRestart"));
  buttonReturn =
      TheNameKeyGenerator->nameToKey(AsciiString("QuitMenu.wnd:ButtonReturn"));
  buttonOptions =
      TheNameKeyGenerator->nameToKey(AsciiString("QuitMenu.wnd:ButtonOptions"));
  buttonSaveLoad = TheNameKeyGenerator->nameToKey(
      AsciiString("QuitMenu.wnd:ButtonSaveLoad"));

  buttonRestartWin = TheWindowManager->winGetWindowFromId(NULL, buttonRestart);
  buttonSaveLoadWin =
      TheWindowManager->winGetWindowFromId(NULL, buttonSaveLoad);
...
static void initGadgetsNoSaveQuit(void) {
  buttonExit =
      TheNameKeyGenerator->nameToKey(AsciiString("QuitNoSave.wnd:ButtonExit"));
  buttonRestart = TheNameKeyGenerator->nameToKey(
      AsciiString("QuitNoSave.wnd:ButtonRestart"));
  buttonReturn = TheNameKeyGenerator->nameToKey(
      AsciiString("QuitNoSave.wnd:ButtonReturn"));
  buttonOptions = TheNameKeyGenerator->nameToKey(
      AsciiString("QuitNoSave.wnd:ButtonOptions"));
...
static void exitQuitMenu() {
  ...
  if (!TheGameLogic->isInMultiplayerGame()) TheGameLogic->setGamePaused(FALSE);
...
static void noExitQuitMenu() { quitConfirmationWindow = NULL; }
...
static void surrenderQuitMenu() {
  ...
  if (TheVictoryConditions->isLocalAlliedVictory()) return;
...
void HideQuitMenu(void) {
  // Note: This is called as a safety a lot, without checking for the prescence
...
void ToggleQuitMenu() {
  ...
  if ((saveLoadMenuLayout != NULL) &&
    ...
    TheWindowManager->winSendSystemMsg(saveLoadMenuLayout->getFirstWindow(),
                                       GBM_SELECTED,
                                       (WindowMsgData)saveLoadBack, NULL);
  ...
  if (isVisible && quitMenuLayout) {
    ...
    // Added By Sadullah Nader
    // Bug: When Toggling the quit menu off, the quit confirmation should also
    // go away
    if (quitConfirmationWindow)
      TheWindowManager->winDestroy(quitConfirmationWindow);
    quitConfirmationWindow = NULL;
    ...
    if (TheGameLogic->isInMultiplayerGame() || TheGameLogic->isInReplayGame()) {
      ...
      if (!noSaveLoadQuitMenuLayout)
        noSaveLoadQuitMenuLayout = TheWindowManager->winCreateLayout(
            AsciiString("Menus/QuitNoSave.wnd"));
      ...
      if (!fullQuitMenuLayout)
        fullQuitMenuLayout = TheWindowManager->winCreateLayout(
            AsciiString("Menus/QuitMenu.wnd"));
    ...
    if (TheInGameUI->getInputEnabled() == FALSE) {
      if (buttonSaveLoadWin) buttonSaveLoadWin->winEnable(FALSE);
      buttonOptionsWin->winEnable(FALSE);
    } else if (buttonSaveLoadWin) {
      if (buttonSaveLoadWin) buttonSaveLoadWin->winEnable(TRUE);
  ...
  switch (msg) {
    ...
    case GBM_SELECTED: {
      ...
      if (controlID == buttonSaveLoad) {
        //
        // these commented lines (12-11-2002) will allow access to load only
        // when were viewing an in-game cinema ... but it's brittle, so I'm
        // disableing it for now and just using the grey button for the whole
        // save/load button for now during a cinema
        //
        ...
        //
        // if input is disabled we are in an in-game cinematic and can load
        // only, since we are in the quit menu we are paused and therefore input
        // is disabled all the time ... in order to figure out if the *game* has
        // input disabled we need to query for the input enabled memory in the
        // game logic which represents the input enabled status of the game if
        // we were not currently paused
        //
        //				if(
