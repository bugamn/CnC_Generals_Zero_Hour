...
class GameSpyLoginPreferences : public UserPreferences {
 public:
  GameSpyLoginPreferences() {
    m_emailPasswordMap.clear();
    m_emailNickMap.clear();
  }
  ...
  void addLogin(AsciiString email, AsciiString nick, AsciiString password,
                AsciiString date);
...
Bool GameSpyLoginPreferences::load(AsciiString fname) {
  if (!UserPreferences::load(fname)) return false;
...
AsciiString GameSpyLoginPreferences::getDateForEmail(AsciiString email,
  ...
  if (fullDate.getLength() != 8) return AsciiString::TheEmptyString;
...
void GameSpyLoginPreferences::addLogin(AsciiString email, AsciiString nick,
  ...
  if (std::find(m_emailNickMap[email].begin(), m_emailNickMap[email].end(),
                nick) == m_emailNickMap[email].end())
    m_emailNickMap[email].push_back(nick);
...
static NameKeyType checkBoxRememberPasswordID =
    NAMEKEY_INVALID;  // checkbox to remember information or not
...
void EnableLoginControls(Bool state) {
  if (buttonLogin) buttonLogin->winEnable(state);
  if (buttonCreateAccount) buttonCreateAccount->winEnable(state);
  if (buttonUseAccount) buttonUseAccount->winEnable(state);
  if (buttonDontUseAccount) buttonDontUseAccount->winEnable(state);
  if (comboBoxEmail) comboBoxEmail->winEnable(state);
  if (comboBoxLoginName) comboBoxLoginName->winEnable(state);
  if (textEntryLoginName) textEntryLoginName->winEnable(state);
  if (textEntryPassword) textEntryPassword->winEnable(state);
  if (checkBoxRememberPassword) checkBoxRememberPassword->winEnable(state);
  if (buttonTOS) buttonTOS->winEnable(state);

  if (textEntryMonth) textEntryMonth->winEnable(state);
  if (textEntryDay) textEntryDay->winEnable(state);
  if (textEntryYear) textEntryYear->winEnable(state);
}
...
void WOLLoginMenuInit(WindowLayout *layout, void *userData) {
  ...
  GameWindow *esrbTitle = TheWindowManager->winGetWindowFromId(
      NULL, NAMEKEY("GameSpyLoginProfile.wnd:StaticTextESRBTop"));
  GameWindow *esrbParent = TheWindowManager->winGetWindowFromId(
      NULL, NAMEKEY("GameSpyLoginProfile.wnd:ParentESRB"));
  ...
  parentWOLLoginID = TheNameKeyGenerator->nameToKey(
      "GameSpyLoginProfile.wnd:WOLLoginMenuParent");
  buttonBackID =
      TheNameKeyGenerator->nameToKey("GameSpyLoginProfile.wnd:ButtonBack");
  buttonLoginID =
      TheNameKeyGenerator->nameToKey("GameSpyLoginProfile.wnd:ButtonLogin");
  buttonCreateAccountID = TheNameKeyGenerator->nameToKey(
      "GameSpyLoginProfile.wnd:ButtonCreateAccount");
  buttonUseAccountID = TheNameKeyGenerator->nameToKey(
      "GameSpyLoginProfile.wnd:ButtonUseAccount");
  buttonDontUseAccountID = TheNameKeyGenerator->nameToKey(
      "GameSpyLoginProfile.wnd:ButtonDontUseAccount");
  buttonTOSID =
      TheNameKeyGenerator->nameToKey("GameSpyLoginProfile.wnd:ButtonTOS");
  parentTOSID =
      TheNameKeyGenerator->nameToKey("GameSpyLoginProfile.wnd:ParentTOS");
  buttonTOSOKID =
      TheNameKeyGenerator->nameToKey("GameSpyLoginProfile.wnd:ButtonTOSOK");
  listboxTOSID =
      TheNameKeyGenerator->nameToKey("GameSpyLoginProfile.wnd:ListboxTOS");
  comboBoxEmailID =
      TheNameKeyGenerator->nameToKey("GameSpyLoginProfile.wnd:ComboBoxEmail");
  comboBoxLoginNameID = TheNameKeyGenerator->nameToKey(
      "GameSpyLoginProfile.wnd:ComboBoxLoginName");
  textEntryLoginNameID = TheNameKeyGenerator->nameToKey(
      "GameSpyLoginProfile.wnd:TextEntryLoginName");
  textEntryPasswordID = TheNameKeyGenerator->nameToKey(
      "GameSpyLoginProfile.wnd:TextEntryPassword");
  checkBoxRememberPasswordID = TheNameKeyGenerator->nameToKey(
      "GameSpyLoginProfile.wnd:CheckBoxRememberInfo");
  textEntryMonthID =
      TheNameKeyGenerator->nameToKey("GameSpyLoginProfile.wnd:TextEntryMonth");
  textEntryDayID =
      TheNameKeyGenerator->nameToKey("GameSpyLoginProfile.wnd:TextEntryDay");
  textEntryYearID =
      TheNameKeyGenerator->nameToKey("GameSpyLoginProfile.wnd:TextEntryYear");
  ...
  buttonCreateAccount =
      TheWindowManager->winGetWindowFromId(NULL, buttonCreateAccountID);
  buttonUseAccount =
      TheWindowManager->winGetWindowFromId(NULL, buttonUseAccountID);
  buttonDontUseAccount =
      TheWindowManager->winGetWindowFromId(NULL, buttonDontUseAccountID);
  ...
  comboBoxLoginName =
      TheWindowManager->winGetWindowFromId(NULL, comboBoxLoginNameID);
  textEntryLoginName =
      TheWindowManager->winGetWindowFromId(NULL, textEntryLoginNameID);
  textEntryPassword =
      TheWindowManager->winGetWindowFromId(NULL, textEntryPasswordID);
  checkBoxRememberPassword =
      TheWindowManager->winGetWindowFromId(NULL, checkBoxRememberPasswordID);
  ...
  if (parentWOLLogin) {
    ...
    // DEBUG_ASSERTCRASH(buttonDontUseAccount,	("buttonDontUseAccount
    // missing!"));
    ...
    // TheShell->registerWithAnimateManager(parentWOLLogin,
    // WIN_ANIMATION_SLIDE_TOP, TRUE);
    /**/
    //		TheShell->registerWithAnimateManager(buttonTOS,
    //WIN_ANIMATION_SLIDE_BOTTOM, TRUE);
    // TheShell->registerWithAnimateManager(buttonCreateAccount,
    // WIN_ANIMATION_SLIDE_LEFT, TRUE);
    // TheShell->registerWithAnimateManager(buttonDontUseAccount,
    // WIN_ANIMATION_SLIDE_LEFT, TRUE);
    //		TheShell->registerWithAnimateManager(buttonBack,
    //WIN_ANIMATION_SLIDE_BOTTOM, TRUE);
    /**/
#ifdef ALLOW_NON_PROFILED_LOGIN
  } else {
    ...
    parentWOLLoginID = TheNameKeyGenerator->nameToKey(
        "GameSpyLoginQuick.wnd:WOLLoginMenuParent");
    buttonBackID =
        TheNameKeyGenerator->nameToKey("GameSpyLoginQuick.wnd:ButtonBack");
    buttonLoginID =
        TheNameKeyGenerator->nameToKey("GameSpyLoginQuick.wnd:ButtonLogin");
    buttonCreateAccountID = TheNameKeyGenerator->nameToKey(
        "GameSpyLoginQuick.wnd:ButtonCreateAccount");
    buttonUseAccountID = TheNameKeyGenerator->nameToKey(
        "GameSpyLoginQuick.wnd:ButtonUseAccount");
    buttonDontUseAccountID = TheNameKeyGenerator->nameToKey(
        "GameSpyLoginQuick.wnd:ButtonDontUseAccount");
    buttonTOSID =
        TheNameKeyGenerator->nameToKey("GameSpyLoginQuick.wnd:ButtonTOS");
    parentTOSID =
        TheNameKeyGenerator->nameToKey("GameSpyLoginQuick.wnd:ParentTOS");
    buttonTOSOKID =
        TheNameKeyGenerator->nameToKey("GameSpyLoginQuick.wnd:ButtonTOSOK");
    listboxTOSID =
        TheNameKeyGenerator->nameToKey("GameSpyLoginQuick.wnd:ListboxTOS");
    comboBoxEmailID =
        TheNameKeyGenerator->nameToKey("GameSpyLoginQuick.wnd:ComboBoxEmail");
    textEntryLoginNameID = TheNameKeyGenerator->nameToKey(
        "GameSpyLoginQuick.wnd:TextEntryLoginName");
    textEntryPasswordID = TheNameKeyGenerator->nameToKey(
        "GameSpyLoginQuick.wnd:TextEntryPassword");
    checkBoxRememberPasswordID = TheNameKeyGenerator->nameToKey(
        "GameSpyLoginQuick.wnd:CheckBoxRememberPassword");

    parentWOLLogin =
        TheWindowManager->winGetWindowFromId(NULL, parentWOLLoginID);
    buttonBack = TheWindowManager->winGetWindowFromId(NULL, buttonBackID);
    buttonLogin = TheWindowManager->winGetWindowFromId(NULL, buttonLoginID);
    buttonCreateAccount =
        TheWindowManager->winGetWindowFromId(NULL, buttonCreateAccountID);
    ...
    //		TheShell->registerWithAnimateManager(buttonCreateAccount,
    //WIN_ANIMATION_SLIDE_LEFT, TRUE);
    //		TheShell->registerWithAnimateManager(buttonUseAccount,
    //WIN_ANIMATION_SLIDE_LEFT, TRUE);
    //		TheShell->registerWithAnimateManager(buttonBack,
    //WIN_ANIMATION_SLIDE_RIGHT, TRUE);
  }
#endif  // ALLOW_NON_PROFILED_LOGIN
  ...
  if (GameSpyUseProfiles) {
    ...
    while (eIt != cachedEmails.end()) {
      ...
      Int pos = GadgetComboBoxAddEntry(comboBoxEmail, uniEmail,
                                       GameSpyColor[GSCOLOR_DEFAULT]);
      if (*eIt == lastEmail) selectedPos = pos;
    ...
    while (nIt != cachedNicks.end()) {
      ...
      Int pos = GadgetComboBoxAddEntry(comboBoxLoginName, uniNick,
                                       GameSpyColor[GSCOLOR_DEFAULT]);
      if (*nIt == lastName) selectedPos = pos;
...
void WOLLoginMenuUpdate(WindowLayout *layout, void *userData) {
    ...
    if (!loggedInOK && TheGameSpyPeerMessageQueue->getResponse(resp)) {
      switch (resp.peerResponseType) {
        ...
        case PeerResponse::PEERRESPONSE_DISCONNECT: {
          ...
          DEBUG_LOG(
              ("Tearing down GameSpy from "
               "WOLLoginMenuUpdate(PEERRESPONSE_DISCONNECT)\n"));
          TearDownGameSpy();
  ...
  if (TheGameSpyInfo && !buttonPushed && loginAttemptTime &&
      (loginAttemptTime + loginTimeoutInMS < timeGetTime())) {
    // timed out a login attempt, so say so
    loginAttemptTime = 0;
    UnicodeString title, body;
    AsciiString disconMunkee;
    disconMunkee.format(
        "GUI:GSDisconReason4");  // ("could not connect to server")
    title = TheGameText->fetch("GUI:GSErrorTitle");
    body = TheGameText->fetch(disconMunkee);
    GSMessageBoxOk(title, body);
    EnableLoginControls(TRUE);
    ...
    AsciiString config = TheGameSpyInfo->getConfig();
    DEBUG_LOG(
        ("Tearing down GameSpy from WOLLoginMenuUpdate(login timeout)\n"));
    TearDownGameSpy();
    SetUpGameSpy(motd.str(), config.str());
  }
...
static Bool isNickOkay(UnicodeString nick) {
  static const WideChar *legalIRCChars =
  ...
  if (len == 1 && nick.getCharAt(0) == L'-') return FALSE;

  WideChar newChar = nick.getCharAt(len - 1);
  if (wcschr(legalIRCChars, newChar) == NULL) return FALSE;
...
static Bool isAgeOkay(AsciiString &month, AsciiString &day, AsciiString year) {
  ...
  if (monthInt > 12 || dayInt > 31) return FALSE;
...
#define DATE_BUFFER_SIZE 256
  ...
  GetDateFormat(LOCALE_SYSTEM_DEFAULT, 0, NULL, "yyyy", dateBuffer,
                DATE_BUFFER_SIZE);
  ...
  GetDateFormat(LOCALE_SYSTEM_DEFAULT, 0, NULL, "MM", dateBuffer,
                DATE_BUFFER_SIZE);
  ...
  GetDateFormat(LOCALE_SYSTEM_DEFAULT, 0, NULL, "dd", dateBuffer,
                DATE_BUFFER_SIZE);
  sysVal = atoi(dateBuffer);
  userVal = atoi(day.str());
  if (sysVal - userVal < 0) return FALSE;
...
WindowMsgHandledType WOLLoginMenuSystem(GameWindow *window, UnsignedInt msg,
  ...
  switch (msg) {
    ...
    // someone typed in a combo box.  Clear password (or fill it in if the typed
    // name matches a known login name)
    case GCM_UPDATE_TEXT: {
      ...
      if (controlID == comboBoxEmailID) {
        ...
        while (nIt != cachedNicks.end()) {
          UnicodeString uniNick;
          uniNick.translate(*nIt);
          GadgetComboBoxAddEntry(comboBoxLoginName, uniNick,
                                 GameSpyColor[GSCOLOR_DEFAULT]);
          selectedPos = 0;
          ++nIt;
        }
    ...
    case GBM_SELECTED: {
      ...
      // If we back out, just bail - we haven't gotten far enough to need to log
      // out
      if (controlID == buttonBackID) {
        ...
        // actually attempt to create an account based on info entered
          ...
          if (!isAgeOkay(month, day, year)) {
            GSMessageBoxOk(TheGameText->fetch("GUI:AgeFailedTitle"),
                           TheGameText->fetch("GUI:AgeFailed"));
            break;
          }
          ...
          if (!email.isEmpty() && !login.isEmpty() && !password.isEmpty()) {
            ...
            if (checkBoxRememberPassword &&
                GadgetCheckBoxIsChecked(checkBoxRememberPassword)) {
        ...
        useWebBrowserForTOS =
            FALSE;  // loginPref->getBool("UseTOSBrowser", TRUE);
        if (useWebBrowserForTOS && (TheWebBrowser != NULL)) {
          ...
          // Okay, no web browser.  This means we're looking at a UTF-8 text
          // file.
          ...
          if (theFile) {
            ...
            if (bytesRead == size && size > 2) {
              ...
              while (asciiBuf.nextToken(&asciiLine, "\r\n")) {
                ...
                uniLine = UnicodeString(
                    MultiByteToWideCharSingleLine(asciiLine.str()).c_str());
...
case GEM_UPDATE_TEXT:
      {
              ...
              {
                      UnicodeString munkee = GadgetTextEntryGetText(
textEntryLoginName ); if ( !isNickOkay( munkee ) )
                      {
