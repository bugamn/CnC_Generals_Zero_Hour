...
void DifficultySelectInit(WindowLayout *layout, void *userData) {
  ...
  buttonCancelID =
      TheNameKeyGenerator->nameToKey("DifficultySelect.wnd:ButtonCancel");
  buttonCancel = TheWindowManager->winGetWindowFromId(parent, buttonCancelID);
  radioButtonEasyAIID = TheNameKeyGenerator->nameToKey(
      AsciiString("DifficultySelect.wnd:RadioButtonEasy"));
  radioButtonEasyAI =
      TheWindowManager->winGetWindowFromId(parent, radioButtonEasyAIID);
  radioButtonMediumAIID = TheNameKeyGenerator->nameToKey(
      AsciiString("DifficultySelect.wnd:RadioButtonMedium"));
  radioButtonMediumAI =
      TheWindowManager->winGetWindowFromId(parent, radioButtonMediumAIID);
  radioButtonHardAIID = TheNameKeyGenerator->nameToKey(
      AsciiString("DifficultySelect.wnd:RadioButtonHard"));
  radioButtonHardAI =
  ...
  //"SkirmishMapSelectMenu.wnd:SkrimishMapSelectMenuParent" ); 	NameKeyType
  //parentID = TheNameKeyGenerator->nameToKey( parentName ); 	parent =
  //TheWindowManager->winGetWindowFromId( NULL, parentID );
  //
...
WindowMsgHandledType DifficultySelectInput(GameWindow *window, UnsignedInt msg,
  ...
  //
  //					//
  //					// send a simulated selected event to
  ...
  //"SkirmishMapSelectMenu.wnd:ButtonBack" ); 						NameKeyType buttonID =
  //TheNameKeyGenerator->nameToKey( buttonName ); 						GameWindow *button =
  //TheWindowManager->winGetWindowFromId( window, buttonID );
  //
  //						TheWindowManager->winSendSystemMsg(
  //window, GBM_SELECTED, 																								(WindowMsgData)button, buttonID );
  //
  //					}  // end if
  //
  //					// don't let key fall through anywhere
  ...
  switch (msg) {
    ...
    case GBM_SELECTED: {
      ...
      if (controlID == buttonOkID) {
        ...
        pref.write();
        // TheScriptEngine->setGlobalDifficulty(s_AIDiff); // CANNOT DO THIS!
        // REPLAYS WILL BREAK!
