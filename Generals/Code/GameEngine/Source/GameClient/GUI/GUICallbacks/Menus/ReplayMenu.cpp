...
void PopulateReplayFileListbox(GameWindow *listbox) {
  ...
  Color colors[COLOR_MAX] = {
      GameMakeColor(255, 255, 255, 255), GameMakeColor(128, 128, 128, 255),
      GameMakeColor(255, 255, 255, 255), GameMakeColor(128, 128, 128, 255)};
  ...
  TheFileSystem->getFileListInDirectory(TheRecorder->getReplayDir(),
                                        asciisearch, replayFilenames, TRUE);
  ...
  for (it = replayFilenames.begin(); it != replayFilenames.end(); ++it) {
    ...
    Bool success =
        TheRecorder && TheMapCache && TheRecorder->readReplayHeader(header);
    if (success) {
      ReplayGameInfo info;
      if (ParseAsciiStringToGameInfo(&info, header.gameOptions)) {
        ...
        for (Int tmp = 0; tmp < TheRecorder->getReplayExtention().getLength();
             ++tmp)
        ...
        //					// MP game
        //					time_t totalSeconds =
        //header.endTime - header.startTime; 					Int mins = totalSeconds/60; 					Int
        //secs = totalSeconds%60; 					Real fps = header.frameDuration/totalSeconds;
        //					extraStr.format(L"%d:%d (%g fps)
        //%hs", mins, secs, fps, header.desyncGame?"OOS ":"");
        //
        //					for (Int i=0; i<MAX_SLOTS; ++i)
        //					{
        //						const GameSlot *slot =
        //info.getConstSlot(i); 						if (slot && slot->isHuman())
        //						{
        //							if (i)
        //								extraStr.concat(L",
        //"); 							if (header.playerDiscons[i]) 								extraStr.concat(L'*');
        ...
        Int insertionIndex =
            GadgetListBoxAddEntryText(listbox, replayNameToShow, color, -1, 0);
        GadgetListBoxAddEntryText(listbox, displayTimeBuffer, color,
                                  insertionIndex, 1);
        GadgetListBoxAddEntryText(listbox, header.versionString, color,
                                  insertionIndex, 2);
        GadgetListBoxAddEntryText(listbox, mapStr, color, insertionIndex, 3);
        // GadgetListBoxAddEntryText(listbox, extraStr, color, insertionIndex,
        // 4);
      }
    }
  }
...
void ReplayMenuInit(WindowLayout *layout, void *userData) {
  ...
  parentReplayMenuID = TheNameKeyGenerator->nameToKey(
      AsciiString("ReplayMenu.wnd:ParentReplayMenu"));
  buttonLoadID = TheNameKeyGenerator->nameToKey(
      AsciiString("ReplayMenu.wnd:ButtonLoadReplay"));
  buttonBackID =
      TheNameKeyGenerator->nameToKey(AsciiString("ReplayMenu.wnd:ButtonBack"));
  listboxReplayFilesID = TheNameKeyGenerator->nameToKey(
      AsciiString("ReplayMenu.wnd:ListboxReplayFiles"));
  buttonDeleteID = TheNameKeyGenerator->nameToKey(
      AsciiString("ReplayMenu.wnd:ButtonDeleteReplay"));
  buttonCopyID = TheNameKeyGenerator->nameToKey(
      AsciiString("ReplayMenu.wnd:ButtonCopyReplay"));

  parentReplayMenu =
      TheWindowManager->winGetWindowFromId(NULL, parentReplayMenuID);
  buttonLoad =
      TheWindowManager->winGetWindowFromId(parentReplayMenu, buttonLoadID);
  buttonBack =
      TheWindowManager->winGetWindowFromId(parentReplayMenu, buttonBackID);
  listboxReplayFiles = TheWindowManager->winGetWindowFromId(
      parentReplayMenu, listboxReplayFilesID);
  buttonDelete =
      TheWindowManager->winGetWindowFromId(parentReplayMenu, buttonDeleteID);
  buttonCopy =
      TheWindowManager->winGetWindowFromId(parentReplayMenu, buttonCopyID);
  ...
  buttonAnalyzeReplay = TheWindowManager->gogoGadgetPushButton(
      parentReplayMenu, WIN_STATUS_ENABLED | WIN_STATUS_IMAGE, 4, 4, 180, 26,
      &instData, NULL, TRUE);
#endif
  ...
  GameWindow *win = TheWindowManager->winGetWindowFromId(
      NULL, TheNameKeyGenerator->nameToKey("ReplayMenu.wnd:GadgetParent"));
...
void ReplayMenuUpdate(WindowLayout *layout, void *userData) {
  ...
  if (callCopy) copyReplay();
  if (callDelete) deleteReplay();
...
void reallyLoadReplay(void) {
  ...
  if (selected < 0) {
    MessageBoxOk(TheGameText->fetch("GUI:NoFileSelected"),
                 TheGameText->fetch("GUI:PleaseSelectAFile"), NULL);
    return;
  }
    ...
    case GLM_DOUBLE_CLICKED: {
      ...
      if (controlID == listboxReplayFilesID) {
        ...
        if (rowSelected >= 0) {
          UnicodeString filename;
          filename =
              GetReplayFilenameFromListbox(listboxReplayFiles, rowSelected);
    ...
    case GBM_SELECTED: {
      ...
      if (controlID == buttonAnalyzeReplay->winGetWindowId()) {
        if (listboxReplayFiles) {
          ...
          GadgetListBoxGetSelected(listboxReplayFiles, &selected);
          if (selected < 0) {
            MessageBoxOk(UnicodeString(L"Blah Blah"),
                         UnicodeString(L"Please select something munkee boy"),
                         NULL);
            break;
          }
        ...
        if (listboxReplayFiles) {
          ...
          if (TheRecorder->testVersionPlayback(asciiFilename)) {
            MessageBoxOkCancel(
                TheGameText->fetch("GUI:OlderReplayVersionTitle"),
                TheGameText->fetch("GUI:OlderReplayVersion"), reallyLoadReplay,
                NULL);
          } else {
            TheRecorder->playbackFile(asciiFilename);
        ...
        filename = GetReplayFilenameFromListbox(listboxReplayFiles, selected);
        MessageBoxYesNo(TheGameText->fetch("GUI:DeleteFile"),
                        TheGameText->fetch("GUI:AreYouSureDelete"),
                        deleteReplayFlag, NULL);
        ...
        filename = GetReplayFilenameFromListbox(listboxReplayFiles, selected);
        MessageBoxYesNo(TheGameText->fetch("GUI:CopyReplay"),
                        TheGameText->fetch("GUI:AreYouSureCopy"),
                        copyReplayFlag, NULL);
      }
      break;
    }  // end selected
...
void deleteReplay(void) {
  ...
  translate.translate(
      GetReplayFilenameFromListbox(listboxReplayFiles, selected));
...
void copyReplay(void) {
  ...
  if (CopyFile(filename.str(), newFilename.str(), FALSE) == 0) {
    wchar_t buffer[1024];
    FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, buffer,
                   sizeof(buffer), NULL);
    UnicodeString errorStr;
    errorStr.set(buffer);
    errorStr.trim();
