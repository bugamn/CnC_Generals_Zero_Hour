...
// shows whether or not a correctly formatted hotkey assignment is in the text
// area
Bool absolute = false;
...
// keeps track of whether or not each text modifier is being currently displayed
...
void fillCommandListBox(MappableKeyCategories cat) {
  if (!listBoxCommandList) return;

  GadgetListBoxReset(listBoxCommandList);
  Color color = GameMakeColor(255, 255, 255, 255);

  for (const MetaMapRec *rec = TheMetaMap->getFirstMetaMapRec(); rec;
       rec = rec->m_next) {
    if (rec->m_category == cat)
      GadgetListBoxAddEntryText(listBoxCommandList, rec->m_displayName, color,
                                -1, -1);
  }
}
...
void KeyboardOptionsMenuInit(WindowLayout *layout, void *userData) {
  ...
  parentKeyboardOptionsMenuID = TheNameKeyGenerator->nameToKey(
      "KeyboardOptionsMenu.wnd:ParentKeyboardOptionsMenu");
  parentKeyboardOptionsMenu =
      TheWindowManager->winGetWindowFromId(NULL, parentKeyboardOptionsMenuID);

  // get ids for our children controls
  buttonBackID = TheNameKeyGenerator->nameToKey(
      AsciiString("KeyboardOptionsMenu.wnd:ButtonBack"));
  buttonBack = TheWindowManager->winGetWindowFromId(parentKeyboardOptionsMenu,
                                                    buttonBackID);

  comboBoxCategoryListID = TheNameKeyGenerator->nameToKey(
      "KeyboardOptionsMenu.wnd:ComboBoxCategoryList");
  comboBoxCategoryList = TheWindowManager->winGetWindowFromId(
      /*parentKeyboardOptionsMenu*/ NULL, comboBoxCategoryListID);

  listBoxCommandListID = TheNameKeyGenerator->nameToKey(
      "KeyboardOptionsMenu.wnd:ListBoxCommandList");
  listBoxCommandList =
      TheWindowManager->winGetWindowFromId(NULL, listBoxCommandListID);

  staticTextDescriptionID = TheNameKeyGenerator->nameToKey(
      "KeyboardOptionsMenu.wnd:StaticTextDescription");
  staticTextDescription =
      TheWindowManager->winGetWindowFromId(NULL, staticTextDescriptionID);

  staticTextCurrentHotkeyID = TheNameKeyGenerator->nameToKey(
      "KeyboardOptionsMenu.wnd:StaticTextCurrentHotkey");
  staticTextCurrentHotkey =
      TheWindowManager->winGetWindowFromId(NULL, staticTextCurrentHotkeyID);

  buttonResetAllID =
      TheNameKeyGenerator->nameToKey("KeyboardOptionsMenu.wnd:ButtonResetAll");
  buttonResetAll = TheWindowManager->winGetWindowFromId(NULL, buttonResetAllID);

  textEntryAssignHotkeyID = TheNameKeyGenerator->nameToKey(
      "KeyboardOptionsMenu.wnd:TextEntryAssignHotkey");
  textEntryAssignHotkey =
      TheWindowManager->winGetWindowFromId(NULL, textEntryAssignHotkeyID);

  buttonAssignID =
      TheNameKeyGenerator->nameToKey("KeyboardOptionsMenu.wnd:ButtonAssign");
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType KeyboardOptionsMenuInput(GameWindow *window,
                                              UnsignedInt msg,
                                              WindowMsgData mData1,
                                              WindowMsgData mData2) {
  switch (msg) {
  ...
  switch (msg) {
    ...
    case GCM_SELECTED: {
      ...
      if (controlID == comboBoxCategoryListID) {
        ...
        GadgetStaticTextSetText(staticTextDescription,
                                TheGameText->fetch("GUI:NULL"));

        // reset current hotkey text
        GadgetStaticTextSetText(staticTextCurrentHotkey,
                                TheGameText->fetch("GUI:NULL"));
    ...
    case GLM_SELECTED: {
      ...
      if (controlID == listBoxCommandListID) {
        ...
        UnicodeString str;
        str = GadgetListBoxGetText(listBoxCommandList,
                                   selected /*, Int column = 0*/);
        for (const MetaMapRec *rec = TheMetaMap->getFirstMetaMapRec(); rec;
             rec = rec->m_next) {
          if (rec->m_displayName == str) {
            ...
            for (const LookupListRec *keyName = KeyNames; keyName->name;
                 keyName++) {
...
WindowMsgHandledType KeyboardTextEntryInput(GameWindow *window, UnsignedInt msg,
  ...
  switch (msg) {
    ...
    case GWM_IME_CHAR: {
      ...
      if (ch == VK_RETURN) {
        // Done with this edit
        TheWindowManager->winSendSystemMsg(window->winGetOwner(), GEM_EDIT_DONE,
      ...
      if (ch) {
        ...
        if (e->numericalOnly) {
          if (TheWindowManager->winIsDigit(ch) == 0) return MSG_HANDLED;
        } else if (e->alphaNumericalOnly) {
          if (TheWindowManager->winIsAlNum(ch) == 0) return MSG_HANDLED;
        } else if (e->aSCIIOnly) {
        ...
        if (e->text->getTextLength() <= 1) {
          ...
          TheWindowManager->winSendSystemMsg(
              window->winGetOwner(), GEM_UPDATE_TEXT, (WindowMsgData)window, 0);
          return MSG_HANDLED;
        }
        // else check is modifiers are persent
          ...
          // if not, reset textEntry
            ...
            if ((shiftDown | ctrlDown | altDown) && (!absolute)) {
              char test =
                  e->text->getText().getCharAt(e->text->getTextLength() - 1);
              // only replace letter if not the same as last char of string
              // (removes flickering)
              if (test != ch) {
      ...
      switch (mData1) {
          ...
          case KEY_ENTER:
                  ...
                  if( BitTest( mData2, KEY_STATE_DOWN ) )
                  {
                          ...
                          {
                                  TheWindowManager->winSendSystemMsg(
          window->winGetOwner(), GEM_EDIT_DONE, (WindowMsgData)window, 0 );
                          }
                  }
        ...
        case KEY_DEL: {
          ...
          /*default:
          {
                  ...
                  if( ch && ( BitTest( mData2, KEY_STATE_DOWN ) ) )
                  {
                          ...
                          {
                                  e->text->setText(
          UnicodeString::TheEmptyString ); e->text->appendChar( ch ); e->charPos
          = e->text->getTextLength(); TheWindowManager->winSendSystemMsg(
          window->winGetOwner(), GEM_UPDATE_TEXT, (WindowMsgData)window, 0 );
                                  return MSG_HANDLED;
                          }
                          //else check is modifiers are persent
                          ...
                          {
                                  char c =
          e->text->getText().getCharAt(e->text->getTextLength() - 1 ); if(c ==
          '+' )
                                  {
                                          e->text->appendChar( ch );
                                          e->charPos++;
          ...
          window->winGetOwner(), GEM_UPDATE_TEXT, (WindowMsgData)window, 0 );
                                  ...
                                  {
                                          //if any of the modifiers are down,
          just replace letter if( ( shiftDown | ctrlDown | altDown ) && (
          !absolute ) )
                                          {
                                                  char test =
          e->text->getText().getCharAt(e->text->getTextLength() - 1);
                                                  // only replace letter if not
          the same as last char of string (removes flickering) if( test != ch )
                                                  {
                                                          e->text->removeLastChar();
                                                          e->text->appendChar(
          ch ); TheWindowManager->winSendSystemMsg( window->winGetOwner(),
                                                                                                                                                                                          GEM_UPDATE_TEXT,
                                          ...
                                          else
                                          {
                                                  e->text->setText(
          UnicodeString::TheEmptyString ); e->text->appendChar( ch ); e->charPos
          = 1; TheWindowManager->winSendSystemMsg( window->winGetOwner(),
                                                                                                                                                                                  GEM_UPDATE_TEXT,
                                                                                                                                                                                  (WindowMsgData)window,
                                                                                                                                                                                  0 );
                                          }
