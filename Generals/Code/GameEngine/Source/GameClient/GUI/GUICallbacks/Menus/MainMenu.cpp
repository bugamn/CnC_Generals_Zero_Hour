...
// #define _PLAYTEST
// #endif
// 10-20  GS  Made this a project setting so we can set it in one place.  (It
// has spread to several files, inclding MessageStream.h)
#ifdef _INTERNAL
...
static void checkCDBeforeCampaign(GameDifficulty diff) {
  if (!IsFirstCDPresent()) {
    ...
    ExMessageBoxOkCancel(TheGameText->fetch("GUI:InsertCDPrompt"),
                         TheGameText->fetch("GUI:InsertCDMessage"),
                         (void *)diff, checkCDCallback,
                         cancelStartBecauseOfNoCD);
  } else {
...
void initialHide(void) {
  ...
  win = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionGLA"));
  if (win) win->winHide(TRUE);
  win = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionChina"));
  if (win) win->winHide(TRUE);
  win = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionUS"));
  if (win) win->winHide(TRUE);
  win = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinGrowMarker"));
  if (win) win->winHide(TRUE);
  win = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionTraining"));
  if (win) win->winHide(TRUE);
  win = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionTrainingSmall"));
  if (win) win->winHide(TRUE);
  win = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionTrainingMedium"));
  if (win) win->winHide(TRUE);

  win = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionSkirmish"));
  if (win) win->winHide(TRUE);
  win = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionSkirmishSmall"));
  if (win) win->winHide(TRUE);
  win = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionSkirmishMedium"));
  if (win) win->winHide(TRUE);
  ...
  win = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionUSSmall"));
  if (win) win->winHide(TRUE);
  win = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionUSMedium"));
  if (win) win->winHide(TRUE);
  ...
  win = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionGLASmall"));
  if (win) win->winHide(TRUE);
  win = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionGLAMedium"));
  if (win) win->winHide(TRUE);
  ...
  win = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionChinaSmall"));
  if (win) win->winHide(TRUE);
  win = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionChinaMedium"));
  if (win) win->winHide(TRUE);
}
//-------------------------------------------------------------------------------------------------
...
void MainMenuInit(WindowLayout *layout, void *userData) {
  ...
  for (Int i = 0; i < DROPDOWN_COUNT; ++i) dropDownWindows[i] = NULL;

  // get ids for our windows
  mainMenuID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:MainMenuParent"));
  //	campaignID = TheNameKeyGenerator->nameToKey(
  //AsciiString("MainMenu.wnd:ButtonCampaign") );
  skirmishID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonSkirmish"));
  onlineID =
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:ButtonOnline"));
  networkID =
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:ButtonNetwork"));
  optionsID =
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:ButtonOptions"));
  exitID =
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:ButtonExit"));
  motdID =
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:ButtonMOTD"));
  worldBuilderID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonWorldBuilder"));
  //	NameKeyType versionID = TheNameKeyGenerator->nameToKey(
  //AsciiString("MainMenu.wnd:LabelVersion") );
  getUpdateID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonGetUpdate"));
  buttonTRAININGID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonTRAINING"));
  buttonUSAID =
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:ButtonUSA"));
  buttonGLAID =
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:ButtonGLA"));
  buttonChinaID =
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:ButtonChina"));
  buttonUSARecentSaveID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonUSARecentSave"));
  buttonUSALoadGameID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonUSALoadGame"));
  buttonGLARecentSaveID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonGLARecentSave"));
  buttonGLALoadGameID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonGLALoadGame"));
  buttonChinaRecentSaveID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonChinaRecentSave"));
  buttonChinaLoadGameID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonChinaLoadGame"));
  buttonSinglePlayerID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonSinglePlayer"));
  buttonMultiPlayerID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonMultiplayer"));
  buttonMultiBackID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonMultiBack"));
  buttonSingleBackID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonSingleBack"));
  buttonLoadReplayBackID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonLoadReplayBack"));
  buttonReplayID =
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:ButtonReplay"));
  buttonLoadReplayID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonLoadReplay"));
  buttonLoadID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonLoadGame"));
  buttonCreditsID =
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:ButtonCredits"));

  buttonEasyID =
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:ButtonEasy"));
  buttonMediumID =
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:ButtonMedium"));
  buttonHardID =
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:ButtonHard"));
  buttonDiffBackID = TheNameKeyGenerator->nameToKey(
      AsciiString("MainMenu.wnd:ButtonDiffBack"));

  // get pointers to the window buttons
  parentMainMenu = TheWindowManager->winGetWindowFromId(NULL, mainMenuID);
  // buttonCampaign = TheWindowManager->winGetWindowFromId( parentMainMenu,
  // campaignID );
  buttonSinglePlayer = TheWindowManager->winGetWindowFromId(
      parentMainMenu, buttonSinglePlayerID);
  buttonMultiPlayer =
      TheWindowManager->winGetWindowFromId(parentMainMenu, buttonMultiPlayerID);
  buttonSkirmish =
      TheWindowManager->winGetWindowFromId(parentMainMenu, skirmishID);
  buttonOnline = TheWindowManager->winGetWindowFromId(parentMainMenu, onlineID);
  buttonNetwork =
      TheWindowManager->winGetWindowFromId(parentMainMenu, networkID);
  buttonOptions =
      TheWindowManager->winGetWindowFromId(parentMainMenu, optionsID);
  ...
  buttonWorldBuilder =
      TheWindowManager->winGetWindowFromId(parentMainMenu, worldBuilderID);
  buttonReplay =
      TheWindowManager->winGetWindowFromId(parentMainMenu, buttonReplayID);
  buttonLoadReplay =
      TheWindowManager->winGetWindowFromId(parentMainMenu, buttonLoadReplayID);
  buttonLoad =
      TheWindowManager->winGetWindowFromId(parentMainMenu, buttonLoadID);
  buttonCredits =
      TheWindowManager->winGetWindowFromId(parentMainMenu, buttonCreditsID);

  buttonEasy =
      TheWindowManager->winGetWindowFromId(parentMainMenu, buttonEasyID);
  buttonMedium =
      TheWindowManager->winGetWindowFromId(parentMainMenu, buttonMediumID);
  buttonHard =
      TheWindowManager->winGetWindowFromId(parentMainMenu, buttonHardID);
  buttonDiffBack =
      TheWindowManager->winGetWindowFromId(parentMainMenu, buttonDiffBackID);

  //	GameWindow *labelVersion = TheWindowManager->winGetWindowFromId(
  //parentMainMenu, versionID );

  getUpdate = TheWindowManager->winGetWindowFromId(parentMainMenu, getUpdateID);
  buttonTRAINING =
      TheWindowManager->winGetWindowFromId(parentMainMenu, buttonTRAININGID);
  ...
  buttonChina =
      TheWindowManager->winGetWindowFromId(parentMainMenu, buttonChinaID);
  buttonUSARecentSave = TheWindowManager->winGetWindowFromId(
      parentMainMenu, buttonUSARecentSaveID);
  buttonUSALoadGame =
      TheWindowManager->winGetWindowFromId(parentMainMenu, buttonUSALoadGameID);
  buttonGLARecentSave = TheWindowManager->winGetWindowFromId(
      parentMainMenu, buttonGLARecentSaveID);
  buttonGLALoadGame =
      TheWindowManager->winGetWindowFromId(parentMainMenu, buttonGLALoadGameID);
  buttonChinaRecentSave = TheWindowManager->winGetWindowFromId(
      parentMainMenu, buttonChinaRecentSaveID);
  buttonChinaLoadGame = TheWindowManager->winGetWindowFromId(
      parentMainMenu, buttonChinaLoadGameID);

  dropDownWindows[DROPDOWN_SINGLE] = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:MapBorder")));
  dropDownWindows[DROPDOWN_MULTIPLAYER] = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:MapBorder1")));
  dropDownWindows[DROPDOWN_MAIN] = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:MapBorder2")));
  dropDownWindows[DROPDOWN_LOADREPLAY] = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:MapBorder3")));
  dropDownWindows[DROPDOWN_DIFFICULTY] = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:MapBorder4")));
  for (i = 1; i < DROPDOWN_COUNT; ++i) dropDownWindows[i]->winHide(TRUE);
  ...
  buttonCompressTest = TheWindowManager->gogoGadgetPushButton(
      parentMainMenu, WIN_STATUS_ENABLED | WIN_STATUS_IMAGE, 25, 175, 400, 400,
      &instData, NULL, TRUE);
#endif  // TEST_COMPRESSION
  ...
  instData.setTooltipText(UnicodeString(L"Only Used in Debug and Internal!"));
  buttonCampaign = TheWindowManager->gogoGadgetPushButton(
      parentMainMenu, WIN_STATUS_ENABLED, 25, 54, 180, 26, &instData, NULL,
      TRUE);

//	if (TheVersion)
//	{
//		UnicodeString version;
//		version.format(L"%s\n%s", TheVersion->getUnicodeVersion().str(),
//TheVersion->getUnicodeBuildTime().str()); 		GadgetStaticTextSetText(
...
//);
  ...
  // TheShell->registerWithAnimateManager(buttonCampaign,
  // WIN_ANIMATION_SLIDE_LEFT, TRUE, 800);
  // TheShell->registerWithAnimateManager(buttonSkirmish,
  // WIN_ANIMATION_SLIDE_LEFT, TRUE, 600);
  //	TheShell->registerWithAnimateManager(buttonSinglePlayer,
  //WIN_ANIMATION_SLIDE_LEFT, TRUE, 400);
  //	TheShell->registerWithAnimateManager(buttonMultiPlayer,
  //WIN_ANIMATION_SLIDE_LEFT, TRUE, 200);
  //	TheShell->registerWithAnimateManager(buttonOptions,
  //WIN_ANIMATION_SLIDE_LEFT, TRUE, 1);
  ...
  if (!checkedForUpdate)
  {
          ...
          if (isConnected && !(state & INTERNET_CONNECTION_MODEM_BUSY))
          {
                  ...
                  DEBUG_LOG(("Looking for a patch for productID=%d,
  versionStr=%s, distribution=%d\n", gameProductID, gameVersionUniqueIDStr,
  gameDistributionID)); ptCheckForPatch( gameProductID, gameVersionUniqueIDStr,
  gameDistributionID, patchAvailableCallback, PTFalse, NULL );
                  //ptCheckForPatch( productID, versionUniqueIDStr,
  distributionID, mapPackAvailableCallback, PTFalse, NULL );
          }
  }
  if (getUpdate != NULL)
  {
  ...
  if (TheMapCache) TheMapCache->updateCache();
  ...
  */
  ...
  if (!didMinSpecCheck) {
    ...
    if (!TheDisplay->testMinSpecRequirements(
            &videoPassed, &cpuPassed, &memPassed)) {  // system failed the test
      MessageBoxOk(TheGameText->fetch("GUI:MinSpecFailedTitle"),
                   TheGameText->fetch("GUI:MinSpecFailedMessage"), NULL);
    }
    didMinSpecCheck = true;
  }
#endif
  GameWindow *rule = TheWindowManager->winGetWindowFromId(
      parentMainMenu, TheNameKeyGenerator->nameToKey(
                          AsciiString("MainMenu.wnd:MainMenuRuler")));
  if (rule) rule->winHide(TRUE);
...
void MainMenuShutdown(WindowLayout *layout, void *userData) {
  if (!startGame) isShuttingDown = TRUE;
  ...
  if (!startGame) TheShell->reverseAnimatewindow();
  // TheShell->reverseAnimatewindow();
...
void DeclineResolution() {
  ...
  if (TheDisplay->setDisplayMode(oldDispSettings.xRes, oldDispSettings.yRes,
                                 oldDispSettings.bitDepth,
                                 oldDispSettings.windowed)) {
    dispChanged = FALSE;
...
void DoResolutionDialog() {
  ...
  resolutionNew.format(L": %dx%d\n", newDispSettings.xRes,
                       newDispSettings.yRes);

  resTimerString.concat(resolutionNew);

  resAcceptMenu = TheWindowManager->gogoMessageBox(
      CORNER, CORNER, -1, -1, MSG_BOX_OK | MSG_BOX_CANCEL,
      TheGameText->fetch("GUI:Resolution"), resTimerString, NULL, NULL,
      AcceptResolution, DeclineResolution);
}

/* This function is not being currently used because we do not need a timer on
the
// dialog box.
//-------------------------------------------------------------------------------------------------
//ResolutionDialogUpdate() - if resolution dialog box is shown, this must count
10 seconds for
//	accepting resolution changes otherwise we go back to previous display
settings
//-------------------------------------------------------------------------------------------------
void ResolutionDialogUpdate()
{
        ...
        //------------------------------------------------------------------------------------------------------
        DEBUG_LOG(("Resolution Timer :  started at %d,  current time at %d,
frameTicker is %d\n", timeStarted, time(NULL) , currentTime));
}
*/
...
void MainMenuUpdate(WindowLayout *layout, void *userData) {
  ...
  if (DontShowMainMenu && justEntered) justEntered = FALSE;
  ...
  */
  ...
  //	if(localAnimateWindowManager && pendingDropDown != DROPDOWN_NONE &&
  //localAnimateWindowManager->isFinished())
  //	{
  ...
  //		pendingDropDown = DROPDOWN_NONE;
  //	}
  //	else if(localAnimateWindowManager && dropDown == DROPDOWN_NONE &&
  //pendingDropDown == DROPDOWN_NONE && localAnimateWindowManager->isReversed()
  //&& localAnimateWindowManager->isFinished())
  //	{
  //		localAnimateWindowManager->reset();
...
WindowMsgHandledType MainMenuInput(GameWindow *window, UnsignedInt msg,
  ...
  if (!notShown) return MSG_IGNORED;
...
WindowMsgHandledType MainMenuSystem(GameWindow *window, UnsignedInt msg,
  ...
  switch (msg) {
    ...
    case GWM_DESTROY: {
      ...
      TearDownGameSpy();
      StopAsyncDNSCheck();  // kill off the async DNS check thread in case it is
                            // still running
      break;
    ...
    case GBM_MOUSE_ENTERING: {
      ...
      if (controlID == onlineID) {
        TheScriptEngine->signalUIInteract(
            TheShellHookNames[SHELL_SCRIPT_HOOK_MAIN_MENU_ONLINE_HIGHLIGHTED]);
      } else if (controlID == networkID) {
        TheScriptEngine->signalUIInteract(
            TheShellHookNames[SHELL_SCRIPT_HOOK_MAIN_MENU_NETWORK_HIGHLIGHTED]);
      } else if (controlID == optionsID) {
        TheScriptEngine->signalUIInteract(
            TheShellHookNames[SHELL_SCRIPT_HOOK_MAIN_MENU_OPTIONS_HIGHLIGHTED]);
      } else if (controlID == exitID) {
        TheScriptEngine->signalUIInteract(
            TheShellHookNames[SHELL_SCRIPT_HOOK_MAIN_MENU_EXIT_HIGHLIGHTED]);
      }
        ...
        if (campaignSelected || dontAllowTransitions) break;
        // TheTransitionHandler->remove("MainMenuSinglePlayerMenu");
    ...
    case GBM_MOUSE_LEAVING: {
      ...
      if (controlID == onlineID) {
        ...
        TheScriptEngine->signalUIInteract(
            TheShellHookNames[SHELL_SCRIPT_HOOK_MAIN_MENU_EXIT_UNHIGHLIGHTED]);
      } else if (controlID == buttonTRAININGID) {
    ...
    case GBM_SELECTED: {
      ...
      if (control == buttonCampaign) {
        ...
        //SLLT_LOAD_ONLY;
        //        WindowLayout *saveLoadMenuLayout =
        //        TheShell->getSaveLoadMenuLayout();
        //				DEBUG_ASSERTCRASH( saveLoadMenuLayout,
        //("Unable to get save load menu layout.\n") );
        ...
        TheTransitionHandler->reverse(
            "MainMenuMultiPlayerMenuTransitionToNext");
        ...
        TheScriptEngine->signalUIInteract(
            TheShellHookNames[SHELL_SCRIPT_HOOK_MAIN_MENU_NETWORK_SELECTED]);
      }  // end else if
      else if (controlID == optionsID) {
        if (dontAllowTransitions) break;
        dontAllowTransitions = TRUE;
        // buttonPushed = TRUE;
        TheScriptEngine->signalUIInteract(
            TheShellHookNames[SHELL_SCRIPT_HOOK_MAIN_MENU_OPTIONS_SELECTED]);

        // load the options menu
        WindowLayout *optLayout = TheShell->getOptionsLayout(TRUE);
        DEBUG_ASSERTCRASH(optLayout != NULL,
                          ("unable to get options menu layout"));
        optLayout->runInit();
...
#if defined _DEBUG
        if (_spawnl(_P_NOWAIT, "WorldBuilderD.exe", "WorldBuilderD.exe", NULL) <
            0)
          MessageBoxOk(TheGameText->fetch("GUI:WorldBuilder"),
                       TheGameText->fetch("GUI:WorldBuilderLoadFailed"), NULL);
#elif defined _INTERNAL
        if (_spawnl(_P_NOWAIT, "WorldBuilderI.exe", "WorldBuilderI.exe", NULL) <
            0)
          MessageBoxOk(TheGameText->fetch("GUI:WorldBuilder"),
                       TheGameText->fetch("GUI:WorldBuilderLoadFailed"), NULL);
#else
        if (_spawnl(_P_NOWAIT, "WorldBuilder.exe", "WorldBuilder.exe", NULL) <
            0)
          MessageBoxOk(TheGameText->fetch("GUI:WorldBuilder"),
                       TheGameText->fetch("GUI:WorldBuilderLoadFailed"), NULL);
#endif
        ...
        // If we ever want to add a dialog before we exit out of the game,
        // uncomment this line and kill the quitCallback() line below.
        // #if defined(_DEBUG) || defined(_INTERNAL)
        ...
        //				layout =
        //TheWindowManager->winCreateLayout( AsciiString(
        //"Menus/DifficultySelect.wnd" ) ); 				layout->runInit(); 				layout->hide(
        //FALSE ); 				layout->bringForward();
  ...
  switch (showSide) {
    case SHOW_TRAINING:
      ...
      TheTransitionHandler->setGroup(
          "MainMenuSinglePlayerTrainingMenuFromDiff");
      break;
    case SHOW_USA:
