...
// ------------------------------------------------------------------------------------------------
/** Given the current layout and selection in the game listbox, update the main
 * save/load menu buttons to be enabled or disabled */
// ------------------------------------------------------------------------------------------------
static void updateMenuActions(void) {
  // for loading only, disable the save button, otherwise enable it
  GameWindow *saveButton =
      TheWindowManager->winGetWindowFromId(NULL, buttonSaveKey);
  DEBUG_ASSERTCRASH(saveButton,
                    ("SaveLoadMenuInit: Unable to find save button\n"));
  ...
  // GameWindow *listboxGames = TheWindowManager->winGetWindowFromId( NULL,
  // NAMEKEY( "PopupSaveLoad.wnd:ListboxGames" ) );
  ...
  selectedGameInfo =
      (AvailableGameInfo *)GadgetListBoxGetItemData(listboxGames, selected);
  GameWindow *buttonLoad =
      TheWindowManager->winGetWindowFromId(NULL, buttonLoadKey);
  buttonLoad->winEnable(selectedGameInfo != NULL);
  GameWindow *buttonDelete =
      TheWindowManager->winGetWindowFromId(NULL, buttonDeleteKey);
...
void SaveLoadMenuInit(WindowLayout *layout, void *userData) {
  ...
  if (userData) currentLayoutType = *((SaveLoadLayoutType *)userData);
  ...
  NameKeyType parentID =
      TheNameKeyGenerator->nameToKey("PopupSaveLoad.wnd:SaveLoadMenu");
  parent = TheWindowManager->winGetWindowFromId(NULL, parentID);
  TheWindowManager->winSetFocus(parent);
  TheWindowManager->winSetModal(parent);

  // enable the menu action buttons
  buttonFrame = TheWindowManager->winGetWindowFromId(
      parent, NAMEKEY("PopupSaveLoad.wnd:MenuButtonFrame"));
  buttonFrame->winEnable(TRUE);

  // get confirmation windows and hide
  overwriteConfirm = TheWindowManager->winGetWindowFromId(
      parent, NAMEKEY("PopupSaveLoad.wnd:OverwriteConfirmParent"));
  overwriteConfirm->winHide(TRUE);
  loadConfirm = TheWindowManager->winGetWindowFromId(
      parent, NAMEKEY("PopupSaveLoad.wnd:LoadConfirmParent"));
  loadConfirm->winHide(TRUE);
  saveDesc = TheWindowManager->winGetWindowFromId(
      parent, NAMEKEY("PopupSaveLoad.wnd:SaveDescParent"));
  saveDesc->winHide(TRUE);
  deleteConfirm = TheWindowManager->winGetWindowFromId(
      parent, NAMEKEY("PopupSaveLoad.wnd:DeleteConfirmParent"));
  editDesc = TheWindowManager->winGetWindowFromId(
      parent, NAMEKEY("PopupSaveLoad.wnd:EntryDesc"));
  // get the listbox that will have the save games in it
  listboxGames = TheWindowManager->winGetWindowFromId(NULL, listboxGamesKey);
  DEBUG_ASSERTCRASH(listboxGames != NULL,
                    ("SaveLoadMenuInit - Unable to find games listbox\n"));
...
void SaveLoadMenuFullScreenInit(WindowLayout *layout, void *userData) {
  ...
  // set keyboard focus to main parent and set modal
  NameKeyType parentID =
      TheNameKeyGenerator->nameToKey("SaveLoad.wnd:SaveLoadMenu");
  parent = TheWindowManager->winGetWindowFromId(NULL, parentID);
  TheWindowManager->winSetFocus(parent);
  //	TheWindowManager->winSetModal( parent );
  ...
  // enable the menu action buttons
  buttonFrame = TheWindowManager->winGetWindowFromId(
      parent, NAMEKEY("SaveLoad.wnd:MenuButtonFrame"));
  buttonFrame->winEnable(TRUE);

  // get confirmation windows and hide
  overwriteConfirm = TheWindowManager->winGetWindowFromId(
      parent, NAMEKEY("SaveLoad.wnd:OverwriteConfirmParent"));
  overwriteConfirm->winHide(TRUE);
  loadConfirm = TheWindowManager->winGetWindowFromId(
      parent, NAMEKEY("SaveLoad.wnd:LoadConfirmParent"));
  loadConfirm->winHide(TRUE);
  saveDesc = TheWindowManager->winGetWindowFromId(
      parent, NAMEKEY("SaveLoad.wnd:SaveDescParent"));
  saveDesc->winHide(TRUE);

  editDesc = TheWindowManager->winGetWindowFromId(
      parent, NAMEKEY("SaveLoad.wnd:EntryDesc"));
  deleteConfirm = TheWindowManager->winGetWindowFromId(
      parent, NAMEKEY("SaveLoad.wnd:DeleteConfirmParent"));
  // get the listbox that will have the save games in it
  listboxGames = TheWindowManager->winGetWindowFromId(NULL, listboxGamesKey);
  DEBUG_ASSERTCRASH(listboxGames != NULL,
                    ("SaveLoadMenuInit - Unable to find games listbox\n"));
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
WindowMsgHandledType SaveLoadMenuInput(GameWindow *window, UnsignedInt msg,
      ...
      switch (key) {
        ...
        case KEY_ESC: {
          ...
          if (BitTest(state, KEY_STATE_UP)) {
            GameWindow *button =
                TheWindowManager->winGetWindowFromId(parent, buttonBackKey);
            TheWindowManager->winSendSystemMsg(
                window, GBM_SELECTED, (WindowMsgData)button, buttonBackKey);
...
static AvailableGameInfo *getSelectedSaveFileInfo(GameWindow *window) {
  // get the listbox
  // GameWindow *listboxGames = TheWindowManager->winGetWindowFromId( window,
  // listboxGamesKey );
  DEBUG_ASSERTCRASH(listboxGames != NULL,
                    ("SaveLoadMenuInit - Unable to find games listbox\n"));
...
static void doLoadGame(void) {
  ...
  // GameWindow *listboxGames = TheWindowManager->winGetWindowFromId( parent,
  // NAMEKEY( "PopupSaveLoad.wnd:ListboxGames" ) );
  DEBUG_ASSERTCRASH(listboxGames,
                    ("doLoadGame: Unable to find game listbox\n"));

  // get selected game info
  AvailableGameInfo *selectedGameInfo = getSelectedSaveFileInfo(listboxGames);
  DEBUG_ASSERTCRASH(selectedGameInfo,
                    ("doLoadGame: No selected game info found\n"));

  // when loading a game we also close the quit/esc menu for the user when
  // in-game
  ...
  //
  // load game, note the *copy* of the selected game info is passed here because
  // we will loose these allocated user data pointers attached as listbox item
  // data when the engine resets
  //
...
static void closeSaveMenu(GameWindow *window) {
  if (isPopup) {
    ...
    if (saveLoadMenuLayout) saveLoadMenuLayout->hide(TRUE);
  } else
    TheShell->hideShell();
...
static void processLoadButtonPress(GameWindow *window) {
  ...
  if (selectedGameInfo) {
    //
    // if we're in the shell we do not need a confirmation dialog that states we
    // will lose the current loaded game data cause we're not in a game
    //
    if (TheShell->isShellActive() == TRUE) {
      ...
      // GameWindow *buttonFrame  = TheWindowManager->winGetWindowFromId(
      // parent, NAMEKEY( "PopupSaveLoad.wnd:MenuButtonFrame" ) ); GameWindow
      // *listboxGames = TheWindowManager->winGetWindowFromId( parent, NAMEKEY(
      // "PopupSaveLoad.wnd:ListboxGames" ) ); GameWindow *loadConfirm  =
      // TheWindowManager->winGetWindowFromId( parent, NAMEKEY(
  ...
  switch (msg) {
    ...
    case GLM_DOUBLE_CLICKED: {
      ...
      GameWindow *listboxGames =
          TheWindowManager->winGetWindowFromId(window, listboxGamesKey);
      DEBUG_ASSERTCRASH(listboxGames != NULL,
                        ("SaveLoadMenuInit - Unable to find games listbox\n"));
    ...
    case GLM_SELECTED: {
      ...
      //
      // handle games listbox, when certain items are selected in the listbox
      // only some commands are available
      //
      if (control == listboxGames) updateMenuActions();
    ...
    case GBM_SELECTED: {
      ...
      if (controlID == buttonLoadKey) {
        ...
        if (selectedGameInfo == NULL) {
          ...
          // GameWindow *saveDesc = TheWindowManager->winGetWindowFromId(
          // parent, NAMEKEY( "PopupSaveLoad.wnd:SaveDescParent" ) );
          saveDesc->winHide(FALSE);

          // set the description text entry field to default value
          // GameWindow *editDesc = TheWindowManager->winGetWindowFromId(
          // saveDesc, NAMEKEY( "PopupSaveLoad.wnd:EntryDesc" ) );
          setEditDescription(editDesc);

          // disable the listbox of games
          // GameWindow *listboxGames = TheWindowManager->winGetWindowFromId(
          // parent, NAMEKEY( "PopupSaveLoad.wnd:ListboxGames" ) );
          listboxGames->winEnable(FALSE);
        ...
        GadgetListBoxGetSelected(listboxGames, &selected);
        AvailableGameInfo *selectedGameInfo;
        selectedGameInfo = (AvailableGameInfo *)GadgetListBoxGetItemData(
            listboxGames, selected);
        ...
        if (selectedGameInfo) {
          ...
          // GameWindow *deleteConfirm = TheWindowManager->winGetWindowFromId(
          // parent, NAMEKEY( "PopupSaveLoad.wnd:DeleteConfirmParent" ) );
        ...
        if (controlID == buttonDeleteConfirm) {
          ...
          AsciiString filepath = TheGameState->getFilePathInSaveDirectory(
              selectedGameInfo->filename);

          // delete the file
          DeleteFile(filepath.str());

          // repopulate the listbox
          TheGameState->populateSaveGameListbox(listboxGames,
                                                currentLayoutType);
        ...
        updateMenuActions();
               ...
               controlID == buttonOverwriteConfirm) {
        // GameWindow *listboxGames = TheWindowManager->winGetWindowFromId(
        // parent, NAMEKEY( "PopupSaveLoad.wnd:ListboxGames" ) );

        // hide save confirm dialog
        // GameWindow *overwriteConfirm  = TheWindowManager->winGetWindowFromId(
        // parent, NAMEKEY( "PopupSaveLoad.wnd:OverwriteConfirmParent" ) );
        overwriteConfirm->winHide(TRUE);
        ...
        if (controlID == buttonOverwriteConfirm) {
          ...
          //					GameWindow *saveDesc =
          //TheWindowManager->winGetWindowFromId( parent, NAMEKEY(
          //"PopupSaveLoad.wnd:SaveDescParent" ) ); 					saveDesc->winHide( FALSE );
          ...
          DEBUG_ASSERTCRASH(
              selectedGameInfo,
              ("SaveLoadMenuSystem: Internal error, listbox entry to overwrite "
               "game has no item data set into listbox element\n"));
          ...
          //
          // given the context of this menu figure out which type of save game
          // we're acutally saving right now.  As it turns out, when this menu
          // is used in the save only mode it means that the save is a mission
          // save between maps because you can only save the game between maps
          // and can of course not load one
          //
          ...
          if (selectedGameInfo) filename = selectedGameInfo->filename;
          TheGameState->saveGame(
              filename, selectedGameInfo->saveGameInfo.description, fileType);

          /*
                                                  // set the description text
             entry field to default value GameWindow *editDesc =
             TheWindowManager->winGetWindowFromId( saveDesc, NAMEKEY(
             "PopupSaveLoad.wnd:EntryDesc" ) ); setEditDescription( editDesc );
          */
        }  // end if
        else if (controlID == buttonOverwriteCancel) {
          // GameWindow *buttonFrame  = TheWindowManager->winGetWindowFromId(
        ...
        // GameWindow *saveDesc = TheWindowManager->winGetWindowFromId( parent,
        // NAMEKEY( "PopupSaveLoad.wnd:SaveDescParent" ) );

        // get description text
        // GameWindow *entryDesc = TheWindowManager->winGetWindowFromId(
        // saveDesc, NAMEKEY( "PopupSaveLoad.wnd:EntryDesc" ) );
        ...
        // get save filename
        AvailableGameInfo *selectedGameInfo =
            getSelectedSaveFileInfo(listboxGames);

        //
        // given the context of this menu figure out which type of save game
        // we're acutally saving right now.  As it turns out, when this menu is
        // used in the save only mode it means that the save is a mission save
        // between maps because you can only save the game between maps and can
        // of course not load one
        //
        SaveFileType fileType;
        if (currentLayoutType == SLLT_SAVE_AND_LOAD)
          fileType = SAVE_FILE_TYPE_NORMAL;
        ...
        updateMenuActions();
               ...
               controlID == buttonLoadCancel) {
        // hide confirm dialog
        // GameWindow *loadConfirm  = TheWindowManager->winGetWindowFromId(
        // parent, NAMEKEY( "PopupSaveLoad.wnd:LoadConfirmParent" ) );
        loadConfirm->winHide(TRUE);
