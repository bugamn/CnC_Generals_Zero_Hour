...
void ShowInGameChat(Bool immediate) {
  ...
  if (chatWindow) {
    ...
    chatWindow =
        TheWindowManager->winCreateFromScript(AsciiString("InGameChat.wnd"));

    static NameKeyType textEntryChatID =
        TheNameKeyGenerator->nameToKey("InGameChat.wnd:TextEntryChat");
    ...
    static NameKeyType chatTypeStaticTextID =
        TheNameKeyGenerator->nameToKey("InGameChat.wnd:StaticTextChatType");
    chatTypeStaticText =
        TheWindowManager->winGetWindowFromId(NULL, chatTypeStaticTextID);
  }
...
void ResetInGameChat(void) {
  if (chatWindow) TheWindowManager->winDestroy(chatWindow);
  chatWindow = NULL;
  chatTextEntry = NULL;
    ...
    switch (inGameChatType) {
      case INGAME_CHAT_EVERYONE:
        if (ThePlayerList->getLocalPlayer()->isPlayerActive())
          GadgetStaticTextSetText(chatTypeStaticText,
                                  TheGameText->fetch("Chat:Everyone"));
        else
          GadgetStaticTextSetText(chatTypeStaticText,
                                  TheGameText->fetch("Chat:Observers"));
        break;
      case INGAME_CHAT_ALLIES:
        GadgetStaticTextSetText(chatTypeStaticText,
                                TheGameText->fetch("Chat:Allies"));
        break;
      case INGAME_CHAT_PLAYERS:
        GadgetStaticTextSetText(chatTypeStaticText,
                                TheGameText->fetch("Chat:Players"));
        break;
    }
  }
}
...
void ToggleInGameChat(Bool immediate) {
  ...
  if (!TheGameInfo->isMultiPlayer() && TheGlobalData->m_netMinPlayers) return;
            ...
            if (player && localPlayer) {
              switch (inGameChatType) {
                case INGAME_CHAT_EVERYONE:
                  ...
                  break;
                case INGAME_CHAT_ALLIES:
                  if ((player->getRelationship(localPlayer->getDefaultTeam()) ==
                           ALLIES &&
                       localPlayer->getRelationship(player->getDefaultTeam()) ==
                           ALLIES) ||
                      player == localPlayer)
                    playerMask |= (1 << i);
                  break;
                case INGAME_CHAT_PLAYERS:
                  if (player == localPlayer) playerMask |= (1 << i);
                  break;
              }
            }
          }
...
WindowMsgHandledType InGameChatSystem(GameWindow *window, UnsignedInt msg,
  ...
  switch (msg) {
    ...
    case GBM_SELECTED: {
      ...
      static NameKeyType buttonClearID = TheNameKeyGenerator->nameToKey(
          AsciiString("InGameChat.wnd:ButtonClear"));
