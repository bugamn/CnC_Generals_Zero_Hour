...
// /////////////////////////////////////////////////////////////////// Author:
// Colin Day - October 2001 Desc: GUI Control bar at the bottom of the screen
// that houses the
...
WindowMsgHandledType LeftHUDInput(GameWindow *window, UnsignedInt msg,
  ...
  //
  // if the player doesn't have a radar, or the radar is hidden, and the radar
  // is not being forced to on, we just eat input over the radar window
  //
  if (!TheRadar->isRadarForced() &&
      (TheRadar->isRadarHidden() || !player->hasRadar()))
    return MSG_HANDLED;

  // If the middle mouse button is depressed, then just let the message fall all
  // the way back to the usual middle mouse button processing. jkmcd
  if (TheMouse->getMouseStatus()->middleState == MBS_Down) return MSG_IGNORED;

  switch (msg) {
    /** @todo
            This is wrong.  The radar should be in the message stream, and eat
       all messages and propagate them as a new message with the coords
       converted to world coords and the message flagged as being from the
       radar.  This would let all of the normal processing occur, and allow
    ...
    */
    ...
    case GWM_MOUSE_LEAVING: {
      //
      // consider changing the mouse cursor if we are not in the process of
      // firing targeted "superweapons" which we can use the radar itself to
      // fire
      //
      ...
      if (targeting == FALSE) {
        const DrawableList *drawableList =
            TheInGameUI->getAllSelectedLocalDrawables();
    ...
    case GWM_MOUSE_POS: {
      ...
      if ((TheRadar->isRadarHidden() == FALSE || TheRadar->isRadarForced()) &&
        ...
        /*
        //
        // this is an example piece of code to find the object under the pixel
        position
        // of the radar ... should we in the future wish to allow commands to be
        executed
        // on objects throught he radar.  note tho that this is extremely hard
        to do because
        // the pixels on the radar are very small and it's hard to do accurate
        targeting
        //

                                        Object *obj =
        TheRadar->objectUnderRadarPixel( &mouse ); UnicodeString msg; if( obj )
                                                msg.format( L"Object under mouse
        on radar '%S'(%d)", obj->getTemplate()->getName().str(), obj->getID() );
                                        else
                                                msg.format( L"Mouse (%d,%d) in
        Radar window L(%d,%d)", mouse.x, mouse.y, radar.x, radar.y );
        ...
        */
        ...
        if (command &&
          ...
          // Else we are not super targeting, so we have to try to refresh the
          // move cursor. We can't just do this on Enter and Exit, because
          // hotkeys allow state to change while we are in the radar.
          const DrawableList *drawableList =
              TheInGameUI->getAllSelectedLocalDrawables();
          Mouse::MouseCursor cur = Mouse::ARROW;
          ...
          if (!(drawableList->empty() || msg == GWM_MOUSE_LEAVING)) {
            if (command &&
                command->getCommandType() == GUI_COMMAND_ATTACK_MOVE) {
              cur = Mouse::ATTACKMOVETO;
            } else {
              cur = Mouse::MOVETO;
            }
    ...
    case GWM_LEFT_DOWN: {
      ...
      if ((TheRadar->isRadarHidden() == FALSE || TheRadar->isRadarForced()) &&
        ...
        // Having drawables and being in attack move mode means that we should
        // attack move. Having drawables and not being in attack move mode means
        // that we should move.

        const DrawableList *drawableList =
            TheInGameUI->getAllSelectedLocalDrawables();  // locally-owned only
        ...
        if (command &&
            ...
            BitTest(command->getOptions(), NEED_TARGET_POS)) {
          // do the command
          TheGameClient->evaluateContextCommand(NULL, &world,
                                                CommandTranslator::DO_COMMAND);

        }  // end if
        else if (command &&
                 command->getCommandType() == GUI_COMMAND_ATTACK_MOVE) {
          // Attack move has changed from a modifier to a command, so it moves
          // up here.

          GameMessage *msg =
              TheMessageStream->appendMessage(GameMessage::MSG_DO_ATTACKMOVETO);
          ...
          // GS Leaving commented out to show that isInAttackMoveToMode is NEVER
          // SET.  It's a command now, not a modifier.
          //					if
          ...
          //						// Play the unit voice
          //response 						pickAndPlayUnitVoiceResponse(drawableList,
          //GameMessage::MSG_DO_ATTACKMOVETO); 						break;
          //					}

          newMsg = TheMessageStream->appendMessage(GameMessage::MSG_DO_MOVETO);
          newMsg->appendLocationArgument(world);
          // Play the unit voice response
          pickAndPlayUnitVoiceResponse(drawableList,
...
WindowMsgHandledType ControlBarSystem(GameWindow *window, UnsignedInt msg,
  ...
  if (TheScriptEngine && TheScriptEngine->isGameEnding()) return MSG_IGNORED;
  switch (msg) {
    // --------------------------------------------------------------------------------------------
    case GWM_CREATE: {
      // get ids for our children controls
      buttonCommunicator = TheNameKeyGenerator->nameToKey(
          AsciiString("ControlBar.wnd:PopupCommunicator"));
    ...
    case GBM_MOUSE_LEAVING: {
      ...
      TheControlBar->processContextSensitiveButtonTransition(
    ...
    case GBM_SELECTED_RIGHT: {
      ...
      static NameKeyType beaconDeleteButtonID =
          NAMEKEY("ControlBar.wnd:ButtonDeleteBeacon");
      static NameKeyType beaconClearTextButtonID =
          NAMEKEY("ControlBar.wnd:ButtonClearBeaconText");
      static NameKeyType beaconGeneralButtonID =
          NAMEKEY("ControlBar.wnd:ButtonGeneral");
      //			static NameKeyType buttonSmallID =
      //NAMEKEY("ControlBar.wnd:ButtonSmall"); 			static NameKeyType buttonMediumID
      //= NAMEKEY("ControlBar.wnd:ButtonMedium");
      static NameKeyType buttonLargeID = NAMEKEY("ControlBar.wnd:ButtonLarge");
      static NameKeyType buttonOptions =
          NAMEKEY("ControlBar.wnd:ButtonOptions");
      static NameKeyType buttonIdleWorker =
          NAMEKEY("ControlBar.wnd:ButtonIdleWorker");

      Int controlID = control->winGetWindowId();
      if (controlID == buttonCommunicator) {
        ToggleDiplomacy(FALSE);
      } else if (controlID == beaconPlacementButtonID &&
      ...
      } else if (controlID == beaconDeleteButtonID &&
                 TheGameLogic->isInMultiplayerGame()) {
        TheMessageStream->appendMessage(GameMessage::MSG_REMOVE_BEACON);
      } else if (controlID == beaconClearTextButtonID &&
                 TheGameLogic->isInMultiplayerGame()) {
      ...
      //			{
      //				TheControlBar->switchControlBarStage(
      ...
      //			}
        ...
        //
        // all buttons from all the context sensitive user interface windows are
        // part of the control bar, send the button processing that way
        //
    ...
    case GEM_EDIT_DONE: {
      ...
      if (controlID == textID) {
        ...
        if (TheInGameUI->getSelectCount() == 1) {
          GameMessage *msg =
              TheMessageStream->appendMessage(GameMessage::MSG_SET_BEACON_TEXT);
...
void ShowControlBar(Bool immediate) {
  showReplayControls();
  if (TheControlBar) TheControlBar->showSpecialPowerShortcut();
  if (TheWindowManager) {
    ...
    if (window) {
      ...
      if (TheControlBar->m_animateWindowManager && !immediate) {
        ...
        // TheControlBar->m_animateWindowManager->registerGameWindow(window,
        // WIN_ANIMATION_SLIDE_BOTTOM_TIMED, TRUE, 1000, 0);
...
void HideControlBar(Bool immediate) {
  hideReplayControls();
  if (TheControlBar) TheControlBar->hideSpecialPowerShortcut();
  if (TheWindowManager) {
    Int id = (Int)TheNameKeyGenerator->nameToKey(
        AsciiString("ControlBar.wnd:ControlBarParent"));
    GameWindow *window = TheWindowManager->winGetWindowFromId(NULL, id);
  ...
  if (TheWindowManager) {
    ...
    if (window) {
      if (window->winIsHidden()) {
        ...
        // now hidden, we're making it visible again so shrink viewport under
        // the window
        TheTacticalView->setHeight((Int)(TheDisplay->getHeight() * 0.80f));
        ...
        if (TheControlBar->m_animateWindowManager && !immediate) {
          TheControlBar->m_animateWindowManager->reset();
          // TheControlBar->m_animateWindowManager->registerGameWindow(window,
          // WIN_ANIMATION_SLIDE_BOTTOM_TIMED, FALSE, 500, 0);
          TheControlBar->m_animateWindowManager->registerGameWindow(
              window, WIN_ANIMATION_SLIDE_BOTTOM, TRUE, 500, 0);
          TheControlBar->animateSpecialPowerShortcut(TRUE);
        }
