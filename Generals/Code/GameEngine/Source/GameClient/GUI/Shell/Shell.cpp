...
Shell::Shell(void) {
  ...
  m_screenCount = 0;
  for (i = 0; i < MAX_SHELL_STACK; i++) m_screenStack[i] = NULL;
...
Shell::~Shell(void) {
  ...
  if (m_schemeManager) delete m_schemeManager;
...
void Shell::init(void) {
  ...
  ini.load(AsciiString("Data\\INI\\Default\\ShellMenuScheme.ini"),
           INI_LOAD_OVERWRITE, NULL);
  ini.load(AsciiString("Data\\INI\\ShellMenuScheme.ini"), INI_LOAD_OVERWRITE,
           NULL);

  if (m_schemeManager) m_schemeManager->init();
...
//-------------------------------------------------------------------------------------------------
/** Update shell system cycle.  All windows are updated that are on the stack,
 * starting with the top layout and progressing to the bottom one */
//-------------------------------------------------------------------------------------------------
void Shell::update(void) {
  ...
  //
  // we keep the shell updates fixed in time so that we can write consitent
...
void Shell::hide(Bool hide) {
  ...
  for (i = 0; i < MAX_SHELL_STACK; i++)
    if (m_screenStack[i]) m_screenStack[i]->hide(hide);
...
void Shell::push(AsciiString filename, Bool shutdownImmediate) {
  ...
  if (TheGameSpyInfo) GameSpyCloseAllOverlays();
  ...
  if (currentTop && !currentTop->isHidden()) {
    ...
    // just call shutdownComplete() which will immediately cause the push to
    // happen
...
//-------------------------------------------------------------------------------------------------
/** Pop top layout of the stack.  Note that we don't actually do the pop right
 * here, we instead run the layout shutdown.  That shutdown() in turn notifies
 * the shell when the shutdown is complete and at that point we do the actual
 * pop */
//-------------------------------------------------------------------------------------------------
void Shell::pop(void) {
...
void Shell::popImmediate(void) {
  ...
  // do NOT set pending pop, we are going to force a pop after the shutdown is
  // run
  m_pendingPop = FALSE;
...
//-------------------------------------------------------------------------------------------------
/** Run the initialize function for the top of the stack just as though it was
 * pushed on the stack.  We want this behavior when we want to act like the top
 * was just pushed on the stack, but it's already there (ie going from in game
 * back to the pre-game shell menus */
//-------------------------------------------------------------------------------------------------
void Shell::showShell(Bool runInit) {
  ...
  //GameMessage::MSG_NEW_GAME ); 		msg->appendIntegerArgument(GAME_SHELL);
  //	}
  //	else
  //	{
  //
  ...
  //
  //		DEBUG_ASSERTCRASH(m_background,("We Couldn't Load
  //Menus/BlankWindow.wnd")); 		m_background->hide(FALSE); 		if (top())
  //			top()->bringForward();
  //
  //	}
...
void Shell::showShellMap(Bool useShellMap) {
  ...
  if (TheGlobalData->m_initialFile.isNotEmpty() || !TheGameLogic) return;
...
//-------------------------------------------------------------------------------------------------
/** Run the shutdown() function for the top of the stack just like we're going
 * to pop it off but DO NOT pop it off the stack.  We want this behavior when
 * leaving the pre-game menus and entering the game and want the shell to still
 * exist and contain the stack information but don't want it to go away */
//-------------------------------------------------------------------------------------------------
...
WindowLayout *Shell::top(void) {
  // emtpy stack
  if (m_screenCount == 0) return NULL;
...
void Shell::doPush(AsciiString layoutFile) {
  ...
  DEBUG_ASSERTCRASH(newScreen != NULL,
                    ("Shell unable to load pending push layout\n"));
...
void Shell::doPop(Bool impendingPush) {
  ...
  DEBUG_ASSERTCRASH(currentTop,
                    ("Shell: No top of stack and we want to pop!\n"));
...
void Shell::registerWithAnimateManager(GameWindow *win, AnimTypes animType,
                                       Bool needsToFinish,
                                       UnsignedInt delayMS) {
  if (!m_animateWindowManager) {
    DEBUG_CRASH(
        ("We called registerWithAnimateManager and we don't have an Animate "
         "Manager created"));
    return;
  }
  if (TheGlobalData->m_animateWindows)
    m_animateWindowManager->registerGameWindow(win, animType, needsToFinish,
                                               500, delayMS);
}

Bool Shell::isAnimFinished(void) {
  // check the new way also.
  if (!TheTransitionHandler->isFinished()) return FALSE;
...
WindowLayout *Shell::getSaveLoadMenuLayout(void) {
  ...
  if (m_saveLoadMenuLayout == NULL)
    m_saveLoadMenuLayout = TheWindowManager->winCreateLayout(
        AsciiString("Menus/PopupSaveLoad.wnd"));
...
WindowLayout *Shell::getPopupReplayLayout(void) {
  ...
  if (m_popupReplayLayout == NULL)
    m_popupReplayLayout =
        TheWindowManager->winCreateLayout(AsciiString("Menus/PopupReplay.wnd"));

  // sanity
  DEBUG_ASSERTCRASH(m_popupReplayLayout,
                    ("Unable to create replay save menu layout\n"));
...
WindowLayout *Shell::getOptionsLayout(Bool create) {
  ...
  if ((m_optionsLayout == NULL) && (create == TRUE)) {
    m_optionsLayout =
        TheWindowManager->winCreateLayout(AsciiString("Menus/OptionsMenu.wnd"));

    // sanity
    DEBUG_ASSERTCRASH(m_optionsLayout,
                      ("Unable to create options menu layout\n"));
  }
