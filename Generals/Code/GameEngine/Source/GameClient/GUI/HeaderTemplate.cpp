...
//
//	purpose:	The header template system is used to maintain a unified
//look across 						windows.  It also allows Localization to customize the looks based
//						on language fonts.
//
//-----------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////
...
const FieldParse HeaderTemplateManager::m_headerFieldParseTable[] = {
    {"Font", INI::parseQuotedAsciiString, NULL,
     offsetof(HeaderTemplate, m_fontName)},
...
void INI::parseHeaderTemplateDefinition(INI *ini) {
  ...
  if (hTemplate == NULL) {
    ...
    DEBUG_CRASH(("[LINE: %d in '%s'] Duplicate header Template %s found!",
...
void HeaderTemplateManager::init(void) {
  ...
  if (GetVersionEx(&osvi)) {  // check if we're running Win9x variant since they
    ...
    if (osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS) {
      ...
      tempName.format("Data\\%s\\HeaderTemplate9x.ini",
                      GetRegistryLanguage().str());
      if (TheFileSystem->doesFileExist(tempName.str())) fname = tempName;
    }
  }
...
HeaderTemplate *HeaderTemplateManager::findHeaderTemplate(AsciiString name) {
  ...
  while (it != m_headerTemplateList.end()) {
    HeaderTemplate *hTemplate = *it;
    if (hTemplate->m_name.compare(name) == 0) return hTemplate;
    ++it;
  }
  return NULL;
}
...
HeaderTemplate *HeaderTemplateManager::newHeaderTemplate(AsciiString name) {
  ...
  DEBUG_ASSERTCRASH(newHTemplate, ("Unable to create a new Header Template in "
                                   "HeaderTemplateManager::newHeaderTemplate"));
  if (!newHTemplate) return NULL;
...
GameFont *HeaderTemplateManager::getFontFromTemplate(AsciiString name) {
  ...
  if (!ht) {
    // DEBUG_LOG(("HeaderTemplateManager::getFontFromTemplate - Could not find
    // header %s\n", name.str()));
    return NULL;
  }
...
HeaderTemplate *HeaderTemplateManager::getFirstHeader(void) {
  HeaderTemplateListIt it = m_headerTemplateList.begin();
  if (it == m_headerTemplateList.end()) return NULL;
...
void HeaderTemplateManager::populateGameFonts(void) {
  ...
  while (it != m_headerTemplateList.end()) {
    ...
    GameFont *font = TheFontLibrary->getFont(hTemplate->m_fontName, pointSize,
                                             hTemplate->m_bold);
    DEBUG_ASSERTCRASH(
        font,
        ("HeaderTemplateManager::populateGameFonts - Could not find font %s %d",
         hTemplate->m_fontName, hTemplate->m_point));
