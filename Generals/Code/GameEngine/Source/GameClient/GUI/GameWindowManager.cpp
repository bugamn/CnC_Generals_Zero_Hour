...
// messages to windows.  You may want to disable this if you feel the mouse
// position messages are "spamming" your window and making a particular debuggin
// situation difficult.  Make sure you do enable this before you check in again
// tho because it is necessary for any code that needs to look at objects or
// anything under the radar cursor
//
...
void GameWindowManager::processDestroyList(void) {
  ...
  for (; doDestroy; doDestroy = next) {
    ...
    if (m_mouseCaptor == doDestroy) winRelease(doDestroy);

    if (m_keyboardFocus == doDestroy) winSetFocus(NULL);

    if ((m_modalHead != NULL) && (doDestroy == m_modalHead->window))
      winUnsetModal(m_modalHead->window);

    if (m_currMouseRgn == doDestroy) m_currMouseRgn = NULL;

    if (m_grabWindow == doDestroy) m_grabWindow = NULL;

    // send the destroy message to the window we're about to kill
    winSendSystemMsg(doDestroy, GWM_DESTROY, 0, 0);

    // free the memory
    if (doDestroy) doDestroy->deleteInstance();
...
//-------------------------------------------------------------------------------------------------
/** Generic function to simply propagate only button press messages to parent
 * and let it deal with it */
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType PassSelectedButtonsToParentSystem(GameWindow *window,
                                                       UnsignedInt msg,
                                                       WindowMsgData mData1,
                                                       WindowMsgData mData2) {
  // sanity
  if (window == NULL) return MSG_IGNORED;

  if ((msg == GBM_SELECTED) || (msg == GBM_SELECTED_RIGHT) ||
      (msg == GBM_MOUSE_ENTERING) || (msg == GBM_MOUSE_LEAVING) ||
      (msg == GEM_EDIT_DONE)) {
 ...
 * and let it deal with it */
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType PassMessagesToParentSystem(GameWindow *window,
                                                UnsignedInt msg,
                                                WindowMsgData mData1,
                                                WindowMsgData mData2) {
  // sanity
...
GameWindowManager::~GameWindowManager(void) {
  ...
  winDestroyAll();
  freeStaticStrings();
  if (TheTransitionHandler) delete TheTransitionHandler;
...
void GameWindowManager::update(void) {
  // Process windows waiting to be destroyed
  processDestroyList();
  if (TheTransitionHandler) TheTransitionHandler->update();
}  // end update
...
void GameWindowManager::addWindowToParent(GameWindow *window,
  ...
  if (parent) {
    ...
    if (parent->m_child) parent->m_child->m_prev = window;
...
void GameWindowManager::windowHiding(GameWindow *window) {
  ...
  if (m_keyboardFocus == window) m_keyboardFocus = NULL;

  // if this is the modal head, unset it
  if (m_modalHead && m_modalHead->window == window) winUnsetModal(window);

  // if this is the captor, it shall no longer be
  if (m_mouseCaptor == window) winCapture(NULL);
...
//-------------------------------------------------------------------------------------------------
void GameWindowManager::hideWindowsInRange(GameWindow *baseWindow, Int first,
                                           Int last, Bool hideFlag) {
  Int i;
  GameWindow *window;

  for (i = first; i <= last; i++) {
    window = winGetWindowFromId(baseWindow, i);
    if (window) window->winHide(hideFlag);
...
//-------------------------------------------------------------------------------------------------
void GameWindowManager::enableWindowsInRange(GameWindow *baseWindow, Int first,
                                             Int last, Bool enableFlag) {
  Int i;
  GameWindow *window;

  for (i = first; i <= last; i++) {
    window = winGetWindowFromId(baseWindow, i);
    if (window) window->winEnable(enableFlag);
...
Int GameWindowManager::winCapture(GameWindow *window) {
  if (m_mouseCaptor != NULL) return WIN_ERR_MOUSE_CAPTURED;
...
Int GameWindowManager::winRelease(GameWindow *window) {
  if (window == m_mouseCaptor) m_mouseCaptor = NULL;
...
GameWindow *GameWindowManager::winGetWindowFromId(GameWindow *window, Int id) {
  if (window == NULL) window = m_windowList;
...
Int GameWindowManager::winSetFocus(GameWindow *window) {
  ...
  if (window)
    if (BitTest(window->winGetStatus(), WIN_STATUS_NO_FOCUS)) return 0;
  ...
  if ((m_keyboardFocus) && (m_keyboardFocus != window)) {
    ...
    winSendSystemMsg(m_keyboardFocus, GWM_INPUT_FOCUS, FALSE,
                     (WindowMsgData)&wf);
  }
  ...
  if (m_keyboardFocus) {
    for (;;) {
      winSendSystemMsg(window, GWM_INPUT_FOCUS, TRUE,
  ...
  if (wantsFocus == FALSE) m_keyboardFocus = NULL;
...
//-------------------------------------------------------------------------------------------------
WinInputReturnCode GameWindowManager::winProcessMouseEvent(
    GameWindowMessage msg, ICoord2D *mousePos, void *data) {
  WinInputReturnCode returnCode = WIN_INPUT_NOT_USED;
  ...
  if (m_mouseCaptor) {
    ...
    if (sendMousePosMessages == TRUE || msg != GWM_MOUSE_POS) {
      ...
      if (win) {
        ...
        // if used clear the event
        if (winSendInputMsg(m_mouseCaptor, msg, packedMouseCoords, 0) ==
            MSG_HANDLED)
          returnCode = WIN_INPUT_USED;
    ...
    if (m_grabWindow) {
      ...
      switch (msg) {
        ...
        case GWM_LEFT_DRAG: {
          if (BitTest(m_grabWindow->m_status, WIN_STATUS_DRAGABLE)) {
            ...
            // Move the window, but keep it completely visible within screen
            // boundaries
            IRegion2D newRegion;
            ICoord2D grabSize;
            ...
            newRegion.lo.x += dx;
            newRegion.lo.y += dy;
            if (newRegion.lo.x < 0) newRegion.lo.x = 0;
            if (newRegion.lo.y < 0) newRegion.lo.y = 0;
      ...
      if (m_modalHead && m_modalHead->window) {
        ...
        for (window = m_windowList; window; window = window->m_next) {
          if (BitTest(window->m_status, WIN_STATUS_ABOVE) &&
            ...
            childWindow = window->winPointInAnyChild(mousePos->x, mousePos->y,
                                                     TRUE, TRUE);
      ...
      if (window) {
        ...
        if (sendMousePosMessages == TRUE || msg != GWM_MOUSE_POS) {
          ...
          while (winSendInputMsg(tempWin, msg, packedMouseCoords, 0) ==
                 MSG_IGNORED) {
            tempWin = tempWin->m_parent;
            if (tempWin == NULL) break;
          ...
          if (m_loneWindow && m_loneWindow == oldLoneWindow &&
            ...
            if (!m_loneWindow->winIsChild(tempWin)) winSetLoneWindow(NULL);
            /*
                            ComboBoxData *cData = (ComboBoxData
               *)m_comboBoxOpen->winGetUserData();
          ...
          if (tempWin) {
            ...
            if (msg == GWM_LEFT_DOWN) {
              //						if( tempWin !=
              //windowList ) 							WinActivate( tempWin );
      ...
      if (toolTipWindow == NULL) {
        if (isHidden(window) == FALSE) toolTipWindow = window;
      ...
      if (tooltipsOn) {
        //				if( toolTipWindow &&
        //toolTipWindow->winGetParent() && BitTest(
        //toolTipWindow->winGetParent()->winGetInstanceData()->getStyle(),
        //GWS_COMBO_BOX )) 					toolTipWindow = toolTipWindow->winGetParent();
        if (toolTipWindow) {
          // do we have a callback to call for the tooltip
          if (toolTipWindow->m_tooltip)
            toolTipWindow->m_tooltip(toolTipWindow, &toolTipWindow->m_instData,
                                     packedMouseCoords);
  ...
  if ((m_grabWindow == NULL) && (window != m_currMouseRgn)) {
    if (m_mouseCaptor) {
      if (m_mouseCaptor->winIsChild(m_currMouseRgn))
        winSendInputMsg(m_currMouseRgn, GWM_MOUSE_LEAVING, packedMouseCoords,
                        0);
...
Int GameWindowManager::drawWindow(GameWindow *window) {
  ...
  if (window == NULL) return WIN_ERR_INVALID_WINDOW;
  ...
  if (BitTest(window->m_status, WIN_STATUS_HIDDEN) == FALSE) {
    ...
    while (child && child->m_next) child = child->m_next;
...
void GameWindowManager::winRepaint(void) {
  ...
  for (window = m_windowTail; window; window = next) {
    ...
    if (BitTest(window->m_status, WIN_STATUS_BELOW)) drawWindow(window);
  }
  ...
  for (window = m_windowTail; window; window = next) {
    ...
    if (BitTest(window->m_status, WIN_STATUS_ABOVE | WIN_STATUS_BELOW) == FALSE)
      drawWindow(window);
  }
  ...
  for (window = m_windowTail; window; window = next) {
    ...
    if (BitTest(window->m_status, WIN_STATUS_ABOVE)) drawWindow(window);
  }

  if (TheTransitionHandler) TheTransitionHandler->draw();
}  // end WinRepaint
...
void GameWindowManager::dumpWindow(GameWindow *window) {
  ...
  for (child = window->m_child; child; child = child->m_next) dumpWindow(child);
...
//-------------------------------------------------------------------------------------------------
GameWindow *GameWindowManager::winCreate(GameWindow *parent, UnsignedInt status,
  ...
  if (window == NULL) {
...
#ifndef FINAL
    {
      ...
      for (win = m_windowList; win; win = win->m_next) dumpWindow(win);
    }
#endif
  ...
  if (instData) window->winSetInstanceData(instData);

  // set default font
  if (TheGlobalLanguageData &&
      TheGlobalLanguageData->m_defaultWindowFont.name.isNotEmpty()) {
    window->winSetFont(
        winFindFont(TheGlobalLanguageData->m_defaultWindowFont.name,
...
Int GameWindowManager::winDestroy(GameWindow *window) {
  ...
  if (BitTest(window->m_status, WIN_STATUS_DESTROYED)) return WIN_ERR_OK;

  BitSet(window->m_status, WIN_STATUS_DESTROYED);
  window->freeImages();

  if (m_mouseCaptor == window) winRelease(window);

  if (m_keyboardFocus == window) winSetFocus(NULL);

  if ((m_modalHead != NULL) && (window == m_modalHead->window))
    winUnsetModal(m_modalHead->window);

  if (m_currMouseRgn == window) m_currMouseRgn = NULL;

  if (m_grabWindow == window) m_grabWindow = NULL;
  ...
  //
  if (window->m_layout) window->m_layout->removeWindow(window);
...
//-------------------------------------------------------------------------------------------------
GameWindow *GameWindowManager::gogoMessageBox(
    Int x, Int y, Int width, Int height, UnsignedShort buttonFlags,
  ...
  return gogoMessageBox(x, y, width, height, buttonFlags, titleString,
                        bodyString, yesCallback, noCallback, okCallback,
...

{
  ...
  parent = TheWindowManager->winGetWindowFromId(
      trueParent, TheNameKeyGenerator->nameToKey(tempName));
  TheWindowManager->winSetModal(trueParent);
  TheWindowManager->winSetFocus(
      NULL);  // make sure we lose focus from other windows even if we refuse
              // focus ourselves
  ...
  if (x >= 0 && y >= 0) parent->winSetPosition(x, y);
  ...
  GameWindow *buttonOk =
      TheWindowManager->winGetWindowFromId(parent, buttonOkID);
  ...
  GameWindow *buttonYes =
      TheWindowManager->winGetWindowFromId(parent, buttonYesID);
  // buttonNo in the second position
  tempName = menuName;
  tempName.concat("ButtonNo");
  NameKeyType buttonNoID = TheNameKeyGenerator->nameToKey(tempName);
  GameWindow *buttonNo =
      TheWindowManager->winGetWindowFromId(parent, buttonNoID);
  ...
  GameWindow *buttonCancel =
      TheWindowManager->winGetWindowFromId(parent, buttonCancelID);
  buttonCancel->winGetPosition(&buttonX[2], &buttonY[2]);

  // we shouldn't have button OK and Yes on the same dialog
  if ((buttonFlags & (MSG_BOX_OK | MSG_BOX_YES)) ==
      (MSG_BOX_OK | MSG_BOX_YES)) {
  ...
  if ((buttonFlags & (MSG_BOX_NO | MSG_BOX_CANCEL)) ==
      (MSG_BOX_NO | MSG_BOX_CANCEL)) {
    // If we have both the No and Cancel button, then the no should go in the
    // middle position
    ...
    // else if we just have the Cancel button, well, it should always go in the
    // right spot
    buttonCancel->winSetPosition(buttonX[2], buttonY[2]);
    buttonCancel->winHide(FALSE);
  }
  ...
  GameWindow *staticTextTitle =
      TheWindowManager->winGetWindowFromId(parent, staticTextTitleID);
...
//-------------------------------------------------------------------------------------------------
GameWindow *GameWindowManager::gogoGadgetPushButton(
    GameWindow *parent, UnsignedInt status, Int x, Int y, Int width, Int height,
    WinInstanceData *instData, GameFont *defaultFont, Bool defaultVisual) {
  GameWindow *button;
  ...
  button = TheWindowManager->winCreate(parent, status, x, y, width, height,
                                       GadgetPushButtonSystem, instData);
  if (button == NULL) {
  ...
  if (text.getLength()) GadgetButtonSetText(button, text);
...
//-------------------------------------------------------------------------------------------------
GameWindow *GameWindowManager::gogoGadgetCheckbox(
    GameWindow *parent, UnsignedInt status, Int x, Int y, Int width, Int height,
    WinInstanceData *instData, GameFont *defaultFont, Bool defaultVisual)

{
  ...
  // create the button window
  checkbox = TheWindowManager->winCreate(parent, status, x, y, width, height,
                                         GadgetCheckBoxSystem, instData);
  if (checkbox == NULL) {
  ...
  // assign text from label
  UnicodeString text = winTextLabelToText(instData->m_textLabelString);
  if (text.getLength()) GadgetCheckBoxSetText(checkbox, text);
...
//-------------------------------------------------------------------------------------------------
GameWindow *GameWindowManager::gogoGadgetRadioButton(
    GameWindow *parent, UnsignedInt status, Int x, Int y, Int width, Int height,
    WinInstanceData *instData, RadioButtonData *rData, GameFont *defaultFont,
    Bool defaultVisual)

{
  ...
  if (BitTest(instData->getStyle(), GWS_RADIO_BUTTON) == FALSE) {
    DEBUG_LOG(
        ("Cann't create radioButton gadget, instance data not radioButton "
         "type\n"));
    assert(0);
    return NULL;
  ...
  // create the button window
  radioButton = TheWindowManager->winCreate(parent, status, x, y, width, height,
                                            GadgetRadioButtonSystem, instData);
  if (radioButton == NULL) {
  ...
  // assign text from label
  UnicodeString text = winTextLabelToText(instData->m_textLabelString);
  if (text.getLength()) GadgetRadioSetText(radioButton, text);
...
GameWindow *GameWindowManager::gogoGadgetTabControl(
    GameWindow *parent, UnsignedInt status, Int x, Int y, Int width, Int height,
    WinInstanceData *instData, TabControlData *tData, GameFont *defaultFont,
    Bool defaultVisual)

{
  ...
  tabControl = TheWindowManager->winCreate(parent, status, x, y, width, height,
                                           GadgetTabControlSystem, instData);
  if (tabControl == NULL) {
...
//-------------------------------------------------------------------------------------------------
GameWindow *GameWindowManager::gogoGadgetListBox(
    GameWindow *parent, UnsignedInt status, Int x, Int y, Int width, Int height,
    WinInstanceData *instData, ListboxData *listboxDataTemplate,
...

{
  ...
  // create the listbox
  listbox = winCreate(parent, status, x, y, width, height, GadgetListBoxSystem,
                      instData);
  ...
  DEBUG_ASSERTCRASH(listboxDataTemplate,
                    ("listboxDataTemplate not initialized"));
  ...
  if (instData->getTextLength()) title = TRUE;
  ...
  if (title) listboxData->displayHeight -= winFontHeight(instData->getFont());
  ...
  if (listboxData->scrollBar) GadgetListboxCreateScrollbar(listbox);
  //
  ...
  if (listboxData->columns == 1) {
    ...
    if (!listboxData->columnWidthPercentage) return NULL;
    listboxData->columnWidth = NEW Int[listboxData->columns];
    if (!listboxData->columnWidth) return NULL;
    ...
    for (Int i = 0; i < listboxData->columns; i++) {
      listboxData->columnWidth[i] =
          listboxData->columnWidthPercentage[i] * totalWidth / 100;
    }  // for
  }  // else
  // assign the default images/colors
...
//-------------------------------------------------------------------------------------------------
GameWindow *GameWindowManager::gogoGadgetSlider(
    GameWindow *parent, UnsignedInt status, Int x, Int y, Int width, Int height,
    WinInstanceData *instData, SliderData *sliderData, GameFont *defaultFont,
    Bool defaultVisual)

{
  ...
  if (BitTest(instData->getStyle(), GWS_HORZ_SLIDER)) {
    ...
    if (BitTest(slider->winGetStatus(), WIN_STATUS_IMAGE) &&
        !(parent && BitTest(parent->winGetStyle(), GWS_SCROLL_LISTBOX)))
  ...
  if (BitTest(instData->getStyle(), GWS_HORZ_SLIDER))
    button = gogoGadgetPushButton(
        slider, statusFlags, 0, HORIZONTAL_SLIDER_THUMB_POSITION,
        HORIZONTAL_SLIDER_THUMB_WIDTH, HORIZONTAL_SLIDER_THUMB_HEIGHT,
        &buttonInstData, NULL, TRUE);
  else
    button = gogoGadgetPushButton(slider, statusFlags, 0, 0, width, width + 1,
                                  &buttonInstData, NULL, TRUE);
...
//-------------------------------------------------------------------------------------------------
GameWindow *GameWindowManager::gogoGadgetComboBox(
    GameWindow *parent, UnsignedInt status, Int x, Int y, Int width, Int height,
    WinInstanceData *instData, ComboBoxData *comboBoxDataTemplate,
    GameFont *defaultFont, Bool defaultVisual) {
  GameWindow *comboBox;
  ...
  // do we have a title
  if (comboBox->winGetTextLength()) title = TRUE;
  ...
  comboBoxData->dropDownButton = TheWindowManager->gogoGadgetPushButton(
      comboBox, status | WIN_STATUS_ACTIVE | WIN_STATUS_ENABLED,
      width - buttonWidth, 0, buttonWidth, height, &winInstData, NULL, TRUE);
  comboBoxData->dropDownButton->winSetTooltipFunc(
      comboBox->winGetTooltipFunc());
  ...
  comboBoxData->entryData =
      (EntryData *)comboBoxData->editBox->winGetUserData();
  // ----------------------------------------------------------------------
  // Create list box
  // ----------------------------------------------------------------------
  winInstData.init();
  ...
  comboBoxData->listBox = TheWindowManager->gogoGadgetListBox(
      comboBox, status | WIN_STATUS_ABOVE | WIN_STATUS_ONE_LINE, 0, height,
      width, height, &winInstData, comboBoxData->listboxData,
      winInstData.m_font, FALSE);
  comboBoxData->listBox->winHide(TRUE);
  delete (comboBoxData->listboxData);
  comboBoxData->listboxData =
      (ListboxData *)comboBoxData->listBox->winGetUserData();
...
//-------------------------------------------------------------------------------------------------
GameWindow *GameWindowManager::gogoGadgetProgressBar(
    GameWindow *parent, UnsignedInt status, Int x, Int y, Int width, Int height,
    WinInstanceData *instData, GameFont *defaultFont, Bool defaultVisual)

{
  ...
  if (BitTest(instData->getStyle(), GWS_PROGRESS_BAR) == FALSE) {
    DEBUG_LOG(
        ("Cann't create progressBar gadget, instance data not progressBar "
         "type\n"));
    assert(0);
    return NULL;
  ...
  // create the button window
  progressBar = TheWindowManager->winCreate(parent, status, x, y, width, height,
                                            GadgetProgressBarSystem, instData);
  if (progressBar == NULL) {
...
//-------------------------------------------------------------------------------------------------
GameWindow *GameWindowManager::gogoGadgetStaticText(
    GameWindow *parent, UnsignedInt status, Int x, Int y, Int width, Int height,
    WinInstanceData *instData, TextData *textData, GameFont *defaultFont,
    Bool defaultVisual)

{
  ...
  if (textWin != NULL) {
    ...
    data->text->setWordWrapCentered(
        BitTest(instData->getStatus(), WIN_STATUS_WRAP_CENTERED));
    ...
    // assign text from label
    UnicodeString text = winTextLabelToText(instData->m_textLabelString);
    if (text.getLength()) GadgetStaticTextSetText(textWin, text);
...
GameWindow *GameWindowManager::gogoGadgetTextEntry(
    GameWindow *parent, UnsignedInt status, Int x, Int y, Int width, Int height,
    WinInstanceData *instData, EntryData *entryData, GameFont *defaultFont,
    Bool defaultVisual)

{
  ...
  // create the window
  entry = winCreate(parent, status, x, y, width, height, GadgetTextEntrySystem,
                    instData);
  if (entry == NULL) {
  ...
  if (entryData->text) data->text->setText(entryData->text->getText());
  if (entryData->sText) data->sText->setText(entryData->sText->getText());
  ...
  if (OurLanguage == LANGUAGE_ID_KOREAN ||
    ...
    data->constructList = gogoGadgetListBox(
        NULL,
        WIN_STATUS_ABOVE | WIN_STATUS_HIDDEN | WIN_STATUS_NO_FOCUS |
            WIN_STATUS_ONE_LINE,
        0, height, 110, 119, &boxInstData, &lData, NULL, TRUE);
  ...
  // assign text from label
  UnicodeString text = winTextLabelToText(instData->m_textLabelString);
  if (text.getLength()) GadgetTextEntrySetText(entry, text);
...
void GameWindowManager::assignDefaultGadgetLook(GameWindow *gadget,
  ...
  static Color lightGreen =
      TheWindowManager->winMakeColor(128, 255, 128, alpha);
  ...
  static Color lightPurple =
      TheWindowManager->winMakeColor(255, 128, 255, alpha);
  ...
  static Color lightYellow =
      TheWindowManager->winMakeColor(255, 255, 128, alpha);
  ...
  if (defaultFont)
    ...
    if (TheGlobalLanguageData &&
      ...
      gadget->winSetFont(TheWindowManager->winFindFont(
          AsciiString("Times New Roman"), 14, FALSE));
  }

  // if we don't want to assign default colors/images get out of here
  if (assignVisual == FALSE) return;
  ...
  if (BitTest(instData->getStyle(), GWS_PUSH_BUTTON)) {
    ...
    GadgetButtonSetEnabledSelectedImage(
        gadget, winFindImage("PushButtonEnabledSelected"));
    ...
    GadgetButtonSetDisabledSelectedImage(
        gadget, winFindImage("PushButtonDisabledSelected"));
    ...
    gadget->winSetIMECompositeTextColors(imeCompositeText,
                                         imeCompositeTextBorder);
    ...
    GadgetCheckBoxSetEnabledUncheckedBoxImage(
        gadget, winFindImage("CheckBoxEnabledBoxUnselected"));
    ...
    GadgetCheckBoxSetEnabledCheckedBoxImage(
        gadget, winFindImage("CheckBoxEnabledBoxSelected"));
    ...
    GadgetCheckBoxSetDisabledUncheckedBoxImage(
        gadget, winFindImage("CheckBoxDisabledBoxUnselected"));
    ...
    GadgetCheckBoxSetDisabledCheckedBoxImage(
        gadget, winFindImage("CheckBoxDisabledBoxSelected"));
    ...
    GadgetCheckBoxSetHiliteUncheckedBoxImage(
        gadget, winFindImage("CheckBoxHiliteBoxUnselected"));
    ...
    GadgetCheckBoxSetHiliteCheckedBoxImage(
        gadget, winFindImage("CheckBoxHiliteBoxSelected"));
    ...
    GadgetRadioSetEnabledUncheckedBoxImage(
        gadget, winFindImage("RadioButtonEnabledBoxUnselected"));
    ...
    GadgetRadioSetEnabledCheckedBoxImage(
        gadget, winFindImage("RadioButtonEnabledBoxSelected"));
    ...
    GadgetRadioSetDisabledUncheckedBoxImage(
        gadget, winFindImage("RadioButtonDisabledBoxUnselected"));
    ...
    GadgetRadioSetDisabledCheckedBoxImage(
        gadget, winFindImage("RadioButtonDisabledBoxSelected"));
    ...
    GadgetRadioSetHiliteCheckedBoxImage(
        gadget, winFindImage("RadioButtonHiliteBoxSelected"));
    ...
    // enabled
    GadgetSliderSetEnabledImageLeft(gadget,
                                    winFindImage("HSliderEnabledLeftEnd"));
    GadgetSliderSetEnabledImageRight(gadget,
                                     winFindImage("HSliderEnabledRightEnd"));
    GadgetSliderSetEnabledImageCenter(
        gadget, winFindImage("HSliderEnabledRepeatingCenter"));
    GadgetSliderSetEnabledImageSmallCenter(
        gadget, winFindImage("HSliderEnabledSmallRepeatingCenter"));
    GadgetSliderSetEnabledColor(gadget, red);
    GadgetSliderSetEnabledBorderColor(gadget, lightRed);

    // disabled
    GadgetSliderSetDisabledImageLeft(gadget,
                                     winFindImage("HSliderDisabledLeftEnd"));
    GadgetSliderSetDisabledImageRight(gadget,
                                      winFindImage("HSliderDisabledRightEnd"));
    GadgetSliderSetDisabledImageCenter(
        gadget, winFindImage("HSliderDisabledRepeatingCenter"));
    GadgetSliderSetDisabledImageSmallCenter(
        gadget, winFindImage("HSliderDisabledSmallRepeatingCenter"));
    GadgetSliderSetDisabledColor(gadget, red);
    GadgetSliderSetDisabledBorderColor(gadget, lightRed);

    // hilite
    GadgetSliderSetHiliteImageLeft(gadget,
                                   winFindImage("HSliderHiliteLeftEnd"));
    GadgetSliderSetHiliteImageRight(gadget,
                                    winFindImage("HSliderHiliteRightEnd"));
    GadgetSliderSetHiliteImageCenter(
        gadget, winFindImage("HSliderHiliteRepeatingCenter"));
    GadgetSliderSetHiliteImageSmallCenter(
        gadget, winFindImage("HSliderHiliteSmallRepeatingCenter"));
    ...
    //
    // enabled
    GadgetSliderSetEnabledThumbImage(gadget,
                                     winFindImage("HSliderThumbEnabled"));
    GadgetSliderSetEnabledThumbColor(gadget,
                                     GadgetSliderGetEnabledColor(gadget));
    GadgetSliderSetEnabledThumbBorderColor(
        gadget, GadgetSliderGetEnabledBorderColor(gadget));
    GadgetSliderSetEnabledSelectedThumbImage(
        gadget, winFindImage("HSliderThumbEnabled"));
    GadgetSliderSetEnabledSelectedThumbColor(
        gadget, GadgetSliderGetEnabledBorderColor(gadget));
    GadgetSliderSetEnabledSelectedThumbBorderColor(
        gadget, GadgetSliderGetEnabledColor(gadget));

    // disabled
    GadgetSliderSetDisabledThumbImage(gadget,
                                      winFindImage("HSliderThumbDisabled"));
    GadgetSliderSetDisabledThumbColor(gadget,
                                      GadgetSliderGetDisabledColor(gadget));
    GadgetSliderSetDisabledThumbBorderColor(
        gadget, GadgetSliderGetDisabledBorderColor(gadget));
    GadgetSliderSetDisabledSelectedThumbImage(
        gadget, winFindImage("HSliderThumbDisabled"));
    GadgetSliderSetDisabledSelectedThumbColor(
        gadget, GadgetSliderGetDisabledBorderColor(gadget));
    GadgetSliderSetDisabledSelectedThumbBorderColor(
        gadget, GadgetSliderGetDisabledColor(gadget));
    ...
    GadgetSliderSetHiliteThumbBorderColor(
        gadget, GadgetSliderGetHiliteBorderColor(gadget));
    GadgetSliderSetHiliteSelectedThumbImage(
        gadget, winFindImage("HSliderThumbHiliteSelected"));
    GadgetSliderSetHiliteSelectedThumbColor(
        gadget, GadgetSliderGetHiliteBorderColor(gadget));
    GadgetSliderSetHiliteSelectedThumbBorderColor(
        gadget, GadgetSliderGetHiliteColor(gadget));

  }  // end if
  else if (BitTest(instData->getStyle(), GWS_VERT_SLIDER)) {
    // enabled
    GadgetSliderSetEnabledImageTop(gadget,
                                   winFindImage("VSliderEnabledTopEnd"));
    GadgetSliderSetEnabledImageBottom(gadget,
                                      winFindImage("VSliderEnabledBottomEnd"));
    GadgetSliderSetEnabledImageCenter(
        gadget, winFindImage("VSliderEnabledRepeatingCenter"));
    GadgetSliderSetEnabledImageSmallCenter(
        gadget, winFindImage("VSliderEnabledSmallRepeatingCenter"));
    GadgetSliderSetEnabledColor(gadget, red);
    GadgetSliderSetEnabledBorderColor(gadget, lightRed);

    // disabled
    GadgetSliderSetDisabledImageTop(gadget,
                                    winFindImage("VSliderDisabledTopEnd"));
    GadgetSliderSetDisabledImageBottom(
        gadget, winFindImage("VSliderDisabledBottomEnd"));
    GadgetSliderSetDisabledImageCenter(
        gadget, winFindImage("VSliderDisabledRepeatingCenter"));
    GadgetSliderSetDisabledImageSmallCenter(
        gadget, winFindImage("VSliderDisabledSmallRepeatingCenter"));
    GadgetSliderSetDisabledColor(gadget, red);
    GadgetSliderSetDisabledBorderColor(gadget, lightRed);

    // hilite
    GadgetSliderSetHiliteImageTop(gadget, winFindImage("VSliderHiliteTopEnd"));
    GadgetSliderSetHiliteImageBottom(gadget,
                                     winFindImage("VSliderHiliteBottomEnd"));
    GadgetSliderSetHiliteImageCenter(
        gadget, winFindImage("VSliderHiliteRepeatingCenter"));
    GadgetSliderSetHiliteImageSmallCenter(
        gadget, winFindImage("VSliderHiliteSmallRepeatingCenter"));
    GadgetSliderSetHiliteColor(gadget, red);
    GadgetSliderSetHiliteBorderColor(gadget, lightRed);
    ...
    // enabled
    GadgetSliderSetEnabledThumbImage(gadget,
                                     winFindImage("VSliderThumbEnabled"));
    GadgetSliderSetEnabledThumbColor(gadget,
                                     GadgetSliderGetEnabledColor(gadget));
    GadgetSliderSetEnabledThumbBorderColor(
        gadget, GadgetSliderGetEnabledBorderColor(gadget));
    GadgetSliderSetEnabledSelectedThumbImage(
        gadget, winFindImage("VSliderThumbEnabled"));
    GadgetSliderSetEnabledSelectedThumbColor(
        gadget, GadgetSliderGetEnabledBorderColor(gadget));
    GadgetSliderSetEnabledSelectedThumbBorderColor(
        gadget, GadgetSliderGetEnabledColor(gadget));
    ...
    // disabled
    GadgetSliderSetDisabledThumbImage(gadget,
                                      winFindImage("VSliderThumbDisabled"));
    GadgetSliderSetDisabledThumbColor(gadget,
                                      GadgetSliderGetDisabledColor(gadget));
    GadgetSliderSetDisabledThumbBorderColor(
        gadget, GadgetSliderGetDisabledBorderColor(gadget));
    GadgetSliderSetDisabledSelectedThumbImage(
        gadget, winFindImage("VSliderThumbDisabled"));
    GadgetSliderSetDisabledSelectedThumbColor(
        gadget, GadgetSliderGetDisabledBorderColor(gadget));
    GadgetSliderSetDisabledSelectedThumbBorderColor(
        gadget, GadgetSliderGetDisabledColor(gadget));
    ...
    GadgetSliderSetHiliteThumbBorderColor(
        gadget, GadgetSliderGetHiliteBorderColor(gadget));
    GadgetSliderSetHiliteSelectedThumbImage(
        gadget, winFindImage("VSliderThumbHiliteSelected"));
    GadgetSliderSetHiliteSelectedThumbColor(
        gadget, GadgetSliderGetHiliteBorderColor(gadget));
    GadgetSliderSetHiliteSelectedThumbBorderColor(
        gadget, GadgetSliderGetHiliteColor(gadget));
    ...
    GadgetListBoxSetEnabledSelectedItemImageLeft(
        gadget, winFindImage("ListBoxEnabledSelectedItemLeftEnd"));
    GadgetListBoxSetEnabledSelectedItemImageRight(
        gadget, winFindImage("ListBoxEnabledSelectedItemRightEnd"));
    GadgetListBoxSetEnabledSelectedItemImageCenter(
        gadget, winFindImage("ListBoxEnabledSelectedItemRepeatingCenter"));
    GadgetListBoxSetEnabledSelectedItemImageSmallCenter(
        gadget, winFindImage("ListBoxEnabledSelectedItemSmallRepeatingCenter"));

    // disabled
    GadgetListBoxSetDisabledImage(gadget, winFindImage("ListBoxDisabled"));
    GadgetListBoxSetDisabledSelectedItemImageLeft(
        gadget, winFindImage("ListBoxDisabledSelectedItemLeftEnd"));
    GadgetListBoxSetDisabledSelectedItemImageRight(
        gadget, winFindImage("ListBoxDisabledSelectedItemRightEnd"));
    GadgetListBoxSetDisabledSelectedItemImageCenter(
        gadget, winFindImage("ListBoxDisabledSelectedItemRepeatingCenter"));
    GadgetListBoxSetDisabledSelectedItemImageSmallCenter(
        gadget,
        winFindImage("ListBoxDisabledSelectedItemSmallRepeatingCenter"));

    // hilited
    GadgetListBoxSetHiliteImage(gadget, winFindImage("ListBoxHilite"));
    GadgetListBoxSetHiliteSelectedItemImageLeft(
        gadget, winFindImage("ListBoxHiliteSelectedItemLeftEnd"));
    GadgetListBoxSetHiliteSelectedItemImageRight(
        gadget, winFindImage("ListBoxHiliteSelectedItemRightEnd"));
    GadgetListBoxSetHiliteSelectedItemImageCenter(
        gadget, winFindImage("ListBoxHiliteSelectedItemRepeatingCenter"));
    GadgetListBoxSetHiliteSelectedItemImageSmallCenter(
    ...
    if (slider) {
      ...
      // enabled
      GadgetSliderSetEnabledImageTop(slider,
                                     winFindImage("VSliderLargeEnabledTopEnd"));
      GadgetSliderSetEnabledImageBottom(
          slider, winFindImage("VSliderLargeEnabledBottomEnd"));
      GadgetSliderSetEnabledImageCenter(
          slider, winFindImage("VSliderLargeEnabledRepeatingCenter"));
      GadgetSliderSetEnabledImageSmallCenter(
          slider, winFindImage("VSliderLargeEnabledSmallRepeatingCenter"));
      GadgetSliderSetEnabledThumbImage(
          slider, winFindImage("VSliderLargeThumbEnabled"));
      GadgetSliderSetEnabledSelectedThumbImage(
          slider, winFindImage("VSliderLargeThumbEnabled"));

      // disabled
      GadgetSliderSetDisabledImageTop(
          slider, winFindImage("VSliderLargeDisabledTopEnd"));
      GadgetSliderSetDisabledImageBottom(
          slider, winFindImage("VSliderLargeDisabledBottomEnd"));
      GadgetSliderSetDisabledImageCenter(
          slider, winFindImage("VSliderLargeDisabledRepeatingCenter"));
      GadgetSliderSetDisabledImageSmallCenter(
          slider, winFindImage("VSliderLargeDisabledSmallRepeatingCenter"));
      GadgetSliderSetDisabledThumbImage(
          slider, winFindImage("VSliderLargeThumbDisabled"));
      GadgetSliderSetDisabledSelectedThumbImage(
          slider, winFindImage("VSliderLargeThumbDisabled"));

      // hilite
      GadgetSliderSetHiliteImageTop(slider,
                                    winFindImage("VSliderLargeHiliteTopEnd"));
      GadgetSliderSetHiliteImageBottom(
          slider, winFindImage("VSliderLargeHiliteBottomEnd"));
      GadgetSliderSetHiliteImageCenter(
          slider, winFindImage("VSliderLargeHiliteRepeatingCenter"));
      GadgetSliderSetHiliteImageSmallCenter(
          slider, winFindImage("VSliderLargeHiliteSmallRepeatingCenter"));
      GadgetSliderSetHiliteThumbImage(slider,
                                      winFindImage("VSliderLargeThumbHilite"));
      GadgetSliderSetHiliteSelectedThumbImage(
          slider, winFindImage("VSliderLargeThumbHilite"));

      // up button ------------------------------------------------------------

      // enabled
      GadgetButtonSetEnabledImage(upButton,
                                  winFindImage("VSliderLargeUpButtonEnabled"));
      GadgetButtonSetEnabledSelectedImage(
          upButton, winFindImage("VSliderLargeUpButtonEnabled"));

      // disabled
      GadgetButtonSetDisabledImage(
          upButton, winFindImage("VSliderLargeUpButtonDisabled"));
      GadgetButtonSetDisabledSelectedImage(
          upButton, winFindImage("VSliderLargeUpButtonDisabled"));

      // hilite
      GadgetButtonSetHiliteImage(upButton,
                                 winFindImage("VSliderLargeUpButtonHilite"));
      GadgetButtonSetHiliteSelectedImage(
          upButton, winFindImage("VSliderLargeUpButtonHiliteSelected"));

      // down button ----------------------------------------------------------

      // enabled
      GadgetButtonSetEnabledImage(
          downButton, winFindImage("VSliderLargeDownButtonEnabled"));
      GadgetButtonSetEnabledSelectedImage(
          downButton, winFindImage("VSliderLargeDownButtonEnabled"));

      // disabled
      GadgetButtonSetDisabledImage(
          downButton, winFindImage("VSliderLargeDownButtonDisabled"));
      GadgetButtonSetDisabledSelectedImage(
          downButton, winFindImage("VSliderLargeDownButtonDisabled"));

      // hilite
      GadgetButtonSetHiliteImage(downButton,
                                 winFindImage("VSliderLargeDownButtonHilite"));
      GadgetButtonSetHiliteSelectedImage(
          downButton, winFindImage("VSliderLargeDownButtonHiliteSelected"));

    }  // end if

  }  // end else if
  else if (BitTest(instData->getStyle(), GWS_COMBO_BOX)) {
    //		ComboBoxData *comboBoxData = (ComboBoxData
    //*)gadget->winGetUserData();
    ...
    GadgetComboBoxSetEnabledSelectedItemImageLeft(
        gadget, winFindImage("ListBoxEnabledSelectedItemLeftEnd"));
    GadgetComboBoxSetEnabledSelectedItemImageRight(
        gadget, winFindImage("ListBoxEnabledSelectedItemRightEnd"));
    GadgetComboBoxSetEnabledSelectedItemImageCenter(
        gadget, winFindImage("ListBoxEnabledSelectedItemRepeatingCenter"));
    GadgetComboBoxSetEnabledSelectedItemImageSmallCenter(
        gadget, winFindImage("ListBoxEnabledSelectedItemSmallRepeatingCenter"));

    // disabled
    GadgetComboBoxSetDisabledImage(gadget, winFindImage("ListBoxDisabled"));
    GadgetComboBoxSetDisabledSelectedItemImageLeft(
        gadget, winFindImage("ListBoxDisabledSelectedItemLeftEnd"));
    GadgetComboBoxSetDisabledSelectedItemImageRight(
        gadget, winFindImage("ListBoxDisabledSelectedItemRightEnd"));
    GadgetComboBoxSetDisabledSelectedItemImageCenter(
        gadget, winFindImage("ListBoxDisabledSelectedItemRepeatingCenter"));
    GadgetComboBoxSetDisabledSelectedItemImageSmallCenter(
        gadget,
        winFindImage("ListBoxDisabledSelectedItemSmallRepeatingCenter"));

    // hilited
    GadgetComboBoxSetHiliteImage(gadget, winFindImage("ListBoxHilite"));
    GadgetComboBoxSetHiliteSelectedItemImageLeft(
        gadget, winFindImage("ListBoxHiliteSelectedItemLeftEnd"));
    GadgetComboBoxSetHiliteSelectedItemImageRight(
        gadget, winFindImage("ListBoxHiliteSelectedItemRightEnd"));
    GadgetComboBoxSetHiliteSelectedItemImageCenter(
        gadget, winFindImage("ListBoxHiliteSelectedItemRepeatingCenter"));
    GadgetComboBoxSetHiliteSelectedItemImageSmallCenter(
        gadget, winFindImage("ListBoxHiliteSelectedItemSmallRepeatingCenter"));
    ...
    if (dropDownButton) {
      // enabled background
      GadgetButtonSetEnabledImage(dropDownButton,
                                  winFindImage("PushButtonEnabled"));
      // enabled selected button
      GadgetButtonSetEnabledSelectedImage(
          dropDownButton, winFindImage("PushButtonEnabledSelected"));

      // Disabled background
      GadgetButtonSetDisabledImage(dropDownButton,
                                   winFindImage("PushButtonDisabled"));
      // Disabled selected button
      GadgetButtonSetDisabledSelectedImage(
          dropDownButton, winFindImage("PushButtonDisabledSelected"));

      // Hilite background
      GadgetButtonSetHiliteImage(dropDownButton,
                                 winFindImage("PushButtonHilite"));
      // Hilite selected button
      GadgetButtonSetHiliteSelectedImage(
    ...
    if (editBox) {
      // enabled
      GadgetTextEntrySetEnabledImageLeft(
          editBox, winFindImage("TextEntryEnabledLeftEnd"));
      GadgetTextEntrySetEnabledImageRight(
          editBox, winFindImage("TextEntryEnabledRightEnd"));
      GadgetTextEntrySetEnabledImageCenter(
          editBox, winFindImage("TextEntryEnabledRepeatingCenter"));
      GadgetTextEntrySetEnabledImageSmallCenter(
          editBox, winFindImage("TextEntryEnabledSmallRepeatingCenter"));

      // disabled
      GadgetTextEntrySetDisabledImageLeft(
          editBox, winFindImage("TextEntryDisabledLeftEnd"));
      GadgetTextEntrySetDisabledImageRight(
          editBox, winFindImage("TextEntryDisabledRightEnd"));
      GadgetTextEntrySetDisabledImageCenter(
          editBox, winFindImage("TextEntryDisabledRepeatingCenter"));
      GadgetTextEntrySetDisabledImageSmallCenter(
          editBox, winFindImage("TextEntryDisabledSmallRepeatingCenter"));

      // hilited
      GadgetTextEntrySetHiliteImageLeft(editBox,
                                        winFindImage("TextEntryHiliteLeftEnd"));
      GadgetTextEntrySetHiliteImageRight(
          editBox, winFindImage("TextEntryHiliteRightEnd"));
      GadgetTextEntrySetHiliteImageCenter(
          editBox, winFindImage("TextEntryHiliteRepeatingCenter"));
      GadgetTextEntrySetHiliteImageSmallCenter(
          editBox, winFindImage("TextEntryHiliteSmallRepeatingCenter"));
    }
    ...
    if (listBox) {
      ...
      GadgetListBoxSetEnabledSelectedItemImageLeft(
          listBox, winFindImage("ListBoxEnabledSelectedItemLeftEnd"));
      GadgetListBoxSetEnabledSelectedItemImageRight(
          listBox, winFindImage("ListBoxEnabledSelectedItemRightEnd"));
      GadgetListBoxSetEnabledSelectedItemImageCenter(
          listBox, winFindImage("ListBoxEnabledSelectedItemRepeatingCenter"));
      GadgetListBoxSetEnabledSelectedItemImageSmallCenter(
          listBox,
          winFindImage("ListBoxEnabledSelectedItemSmallRepeatingCenter"));

      // disabled
      GadgetListBoxSetDisabledImage(listBox, winFindImage("ListBoxDisabled"));
      GadgetListBoxSetDisabledSelectedItemImageLeft(
          listBox, winFindImage("ListBoxDisabledSelectedItemLeftEnd"));
      GadgetListBoxSetDisabledSelectedItemImageRight(
          listBox, winFindImage("ListBoxDisabledSelectedItemRightEnd"));
      GadgetListBoxSetDisabledSelectedItemImageCenter(
          listBox, winFindImage("ListBoxDisabledSelectedItemRepeatingCenter"));
      GadgetListBoxSetDisabledSelectedItemImageSmallCenter(
          listBox,
          winFindImage("ListBoxDisabledSelectedItemSmallRepeatingCenter"));
      ...
      GadgetListBoxSetHiliteSelectedItemImageLeft(
          listBox, winFindImage("ListBoxHiliteSelectedItemLeftEnd"));
      GadgetListBoxSetHiliteSelectedItemImageRight(
          listBox, winFindImage("ListBoxHiliteSelectedItemRightEnd"));
      GadgetListBoxSetHiliteSelectedItemImageCenter(
          listBox, winFindImage("ListBoxHiliteSelectedItemRepeatingCenter"));
      GadgetListBoxSetHiliteSelectedItemImageSmallCenter(
          listBox,
          winFindImage("ListBoxHiliteSelectedItemSmallRepeatingCenter"));
      ...
      if (slider) {
        ...
        // enabled
        GadgetSliderSetEnabledImageTop(
            slider, winFindImage("VSliderLargeEnabledTopEnd"));
        GadgetSliderSetEnabledImageBottom(
            slider, winFindImage("VSliderLargeEnabledBottomEnd"));
        GadgetSliderSetEnabledImageCenter(
            slider, winFindImage("VSliderLargeEnabledRepeatingCenter"));
        ...
        GadgetSliderSetHiliteImageSmallCenter(
            slider, winFindImage("VSliderLargeHiliteSmallRepeatingCenter"));
        GadgetSliderSetHiliteThumbImage(
            slider, winFindImage("VSliderLargeThumbHilite"));
        GadgetSliderSetHiliteSelectedThumbImage(
            slider, winFindImage("VSliderLargeThumbHilite"));
        ...
        // hilite
        GadgetButtonSetHiliteImage(
            downButton, winFindImage("VSliderLargeDownButtonHilite"));
        GadgetButtonSetHiliteSelectedImage(
            downButton, winFindImage("VSliderLargeDownButtonHiliteSelected"));
    ...
    GadgetProgressBarSetEnabledImageLeft(
        gadget, winFindImage("ProgressBarEnabledLeftEnd"));
    GadgetProgressBarSetEnabledImageRight(
        gadget, winFindImage("ProgressBarEnabledRightEnd"));
    GadgetProgressBarSetEnabledImageCenter(
        gadget, winFindImage("ProgressBarEnabledRepeatingCenter"));
    GadgetProgressBarSetEnabledImageSmallCenter(
        gadget, winFindImage("ProgressBarEnabledSmallRepeatingCenter"));
    GadgetProgressBarSetEnabledBarColor(gadget, yellow);
    GadgetProgressBarSetEnabledBarBorderColor(gadget, white);
    GadgetProgressBarSetEnabledBarImageLeft(
        gadget, winFindImage("ProgressBarEnabledBarLeftEnd"));
    GadgetProgressBarSetEnabledBarImageRight(
        gadget, winFindImage("ProgressBarEnabledBarRightEnd"));
    GadgetProgressBarSetEnabledBarImageCenter(
        gadget, winFindImage("ProgressBarEnabledBarRepeatingCenter"));
    GadgetProgressBarSetEnabledBarImageSmallCenter(
        gadget, winFindImage("ProgressBarEnabledBarSmallRepeatingCenter"));

    // disabled
    GadgetProgressBarSetDisabledColor(gadget, darkGray);
    GadgetProgressBarSetDisabledBorderColor(gadget, lightGray);
    GadgetProgressBarSetDisabledImageLeft(
        gadget, winFindImage("ProgressBarDisabledLeftEnd"));
    GadgetProgressBarSetDisabledImageRight(
        gadget, winFindImage("ProgressBarDisabledRightEnd"));
    GadgetProgressBarSetDisabledImageCenter(
        gadget, winFindImage("ProgressBarDisabledRepeatingCenter"));
    GadgetProgressBarSetDisabledImageSmallCenter(
        gadget, winFindImage("ProgressBarDisabledSmallRepeatingCenter"));
    GadgetProgressBarSetDisabledBarColor(gadget, lightGray);
    GadgetProgressBarSetDisabledBarBorderColor(gadget, white);
    GadgetProgressBarSetDisabledBarImageLeft(
        gadget, winFindImage("ProgressBarDisabledBarLeftEnd"));
    GadgetProgressBarSetDisabledBarImageRight(
        gadget, winFindImage("ProgressBarDisabledBarRightEnd"));
    GadgetProgressBarSetDisabledBarImageCenter(
        gadget, winFindImage("ProgressBarDisabledBarRepeatingCenter"));
    GadgetProgressBarSetDisabledBarImageSmallCenter(
        gadget, winFindImage("ProgressBarDisabledBarSmallRepeatingCenter"));

    // Hilite
    GadgetProgressBarSetHiliteColor(gadget, green);
    GadgetProgressBarSetHiliteBorderColor(gadget, darkGreen);
    GadgetProgressBarSetHiliteImageLeft(
        gadget, winFindImage("ProgressBarHiliteLeftEnd"));
    GadgetProgressBarSetHiliteImageRight(
        gadget, winFindImage("ProgressBarHiliteRightEnd"));
    GadgetProgressBarSetHiliteImageCenter(
        gadget, winFindImage("ProgressBarHiliteRepeatingCenter"));
    GadgetProgressBarSetHiliteImageSmallCenter(
    ...
    GadgetProgressBarSetHiliteBarImageLeft(
        gadget, winFindImage("ProgressBarHiliteBarLeftEnd"));
    ...
    GadgetProgressBarSetHiliteBarImageCenter(
        gadget, winFindImage("ProgressBarHiliteBarRepeatingCenter"));
    GadgetProgressBarSetHiliteBarImageSmallCenter(
        gadget, winFindImage("ProgressBarHiliteBarSmallRepeatingCenter"));
    ...
    // disabled
    GadgetStaticTextSetDisabledImage(gadget,
                                     winFindImage("StaticTextDisabled"));
    ...
    // enabled
    GadgetTextEntrySetEnabledImageLeft(gadget,
                                       winFindImage("TextEntryEnabledLeftEnd"));
    GadgetTextEntrySetEnabledImageRight(
        gadget, winFindImage("TextEntryEnabledRightEnd"));
    GadgetTextEntrySetEnabledImageCenter(
        gadget, winFindImage("TextEntryEnabledRepeatingCenter"));
    GadgetTextEntrySetEnabledImageSmallCenter(
        gadget, winFindImage("TextEntryEnabledSmallRepeatingCenter"));
    GadgetTextEntrySetEnabledColor(gadget, red);
    GadgetTextEntrySetEnabledBorderColor(gadget, lightRed);

    // disabled
    GadgetTextEntrySetDisabledImageLeft(
        gadget, winFindImage("TextEntryDisabledLeftEnd"));
    GadgetTextEntrySetDisabledImageRight(
        gadget, winFindImage("TextEntryDisabledRightEnd"));
    GadgetTextEntrySetDisabledImageCenter(
        gadget, winFindImage("TextEntryDisabledRepeatingCenter"));
    GadgetTextEntrySetDisabledImageSmallCenter(
    ...
    // hilited
    GadgetTextEntrySetHiliteImageLeft(gadget,
                                      winFindImage("TextEntryHiliteLeftEnd"));
    GadgetTextEntrySetHiliteImageRight(gadget,
                                       winFindImage("TextEntryHiliteRightEnd"));
    GadgetTextEntrySetHiliteImageCenter(
        gadget, winFindImage("TextEntryHiliteRepeatingCenter"));
    GadgetTextEntrySetHiliteImageSmallCenter(
        gadget, winFindImage("TextEntryHiliteSmallRepeatingCenter"));
...
//-------------------------------------------------------------------------------------------------
GameWindow *GameWindowManager::getWindowUnderCursor(Int x, Int y,
                                                    Bool ignoreEnabled) {
  if (m_mouseCaptor) {
  ...
  if (m_modalHead && m_modalHead->window) {
    ...
    for (window = m_windowList; window; window = window->m_next) {
      if (BitTest(window->m_status, WIN_STATUS_ABOVE) &&
          !BitTest(window->m_status, WIN_STATUS_HIDDEN) &&
          x >= window->m_region.lo.x && x <= window->m_region.hi.x &&
          y >= window->m_region.lo.y && y <= window->m_region.hi.y) {
  ...
  if (window) {
    if (BitTest(window->m_status, WIN_STATUS_NO_INPUT)) {
      ...
      window = NULL;
               ...
               !(BitTest(window->m_status, WIN_STATUS_ENABLED))) {
      window = NULL;
    }
  }
...
Bool GameWindowManager::initTestGUI(void) {
  ...
  UnsignedInt statusFlags =
      WIN_STATUS_ENABLED | WIN_STATUS_DRAGABLE | WIN_STATUS_IMAGE;
  WinInstanceData instData;

  // make some windows inside each other in the upper left
  window = TheWindowManager->winCreate(NULL, statusFlags, 0, 0, 100, 100, NULL,
                                       NULL);
  window->winSetInputFunc(testGrab);
  window->winSetEnabledColor(
      0, TheWindowManager->winMakeColor(255, 254, 255, 255));
  window->winSetEnabledBorderColor(
      0, TheWindowManager->winMakeColor(0, 0, 0, 255));
  window = TheWindowManager->winCreate(window, statusFlags, 10, 10, 50, 50,
                                       NULL, NULL);
  window->winSetInputFunc(testGrab);
  window->winSetEnabledColor(
      0, TheWindowManager->winMakeColor(128, 128, 128, 255));
  window->winSetEnabledBorderColor(
      0, TheWindowManager->winMakeColor(0, 0, 0, 255));
  ...
  window = TheWindowManager->gogoGadgetPushButton(
      NULL, WIN_STATUS_ENABLED | WIN_STATUS_IMAGE, 200, 100, 100, 30, &instData,
      NULL, TRUE);
  ...
  window = TheWindowManager->gogoGadgetPushButton(
      NULL, WIN_STATUS_ENABLED, 330, 100, 100, 30, &instData, NULL, TRUE);

  // make a push button
  instData.init();
  BitSet(instData.m_style, GWS_PUSH_BUTTON | GWS_MOUSE_TRACK);
  instData.m_textLabelString = "Disabled";
  window = TheWindowManager->gogoGadgetPushButton(NULL, 0, 450, 100, 100, 30,
                                                  &instData, NULL, TRUE);
  ...
  window = TheWindowManager->gogoGadgetCheckbox(
      NULL, WIN_STATUS_ENABLED | WIN_STATUS_IMAGE, 200, 150, 100, 30, &instData,
      NULL, TRUE);
  ...
  instData.m_textLabelString = "Check";
  window = TheWindowManager->gogoGadgetCheckbox(
      NULL, WIN_STATUS_ENABLED, 330, 150, 100, 30, &instData, NULL, TRUE);

  // make window to hold radio buttons
  window = TheWindowManager->winCreate(
      NULL, WIN_STATUS_ENABLED | WIN_STATUS_DRAGABLE, 200, 200, 250, 45, NULL);
  window->winSetInputFunc(testGrab);
  window->winSetEnabledColor(0,
                             TheWindowManager->winMakeColor(50, 50, 50, 200));
  window->winSetEnabledBorderColor(
      0, TheWindowManager->winMakeColor(254, 254, 254, 255));
  ...
  rData.group = 1;
  radio = TheWindowManager->gogoGadgetRadioButton(
      window, WIN_STATUS_ENABLED | WIN_STATUS_IMAGE, 10, 10, 100, 30, &instData,
      &rData, NULL, TRUE);
  ...
  radio = TheWindowManager->gogoGadgetRadioButton(window, WIN_STATUS_ENABLED,
                                                  130, 10, 100, 30, &instData,
                                                  &rData, NULL, TRUE);
  ...
  window = TheWindowManager->gogoGadgetListBox(NULL, WIN_STATUS_ENABLED, 200,
                                               250, 100, 100, &instData,
                                               &listData, NULL, TRUE);
  ...
  GadgetListBoxAddEntryText(window, UnicodeString(L"More text"),
                            TheWindowManager->winMakeColor(105, 105, 255, 255),
                            -1, 0);
  GadgetListBoxAddEntryText(window, UnicodeString(L"Nothing"),
                            TheWindowManager->winMakeColor(105, 105, 255, 255),
                            -1, 0);
  GadgetListBoxAddEntryText(window, UnicodeString(L"Seasons"),
                            TheWindowManager->winMakeColor(105, 205, 255, 255),
                            -1, 0);
  GadgetListBoxAddEntryText(window, UnicodeString(L"Misery"),
                            TheWindowManager->winMakeColor(235, 105, 255, 255),
                            -1, 0);
  GadgetListBoxAddEntryText(window, UnicodeString(L"Natural"),
                            TheWindowManager->winMakeColor(105, 205, 45, 255),
                            -1, 0);
  window->winSetFont(
      TheFontLibrary->getFont(AsciiString("Times New Roman"), 12, FALSE));
  ...
  instData.init();
  instData.m_style = GWS_SCROLL_LISTBOX | GWS_MOUSE_TRACK;
  window = TheWindowManager->gogoGadgetListBox(
      NULL, WIN_STATUS_ENABLED | WIN_STATUS_IMAGE, 75, 250, 100, 100, &instData,
      &listData, NULL, TRUE);
  GadgetListBoxAddEntryText(window, UnicodeString(L"Listbox text"),
                            TheWindowManager->winMakeColor(255, 255, 255, 255),
                            -1, -1);
  ...
  window = TheWindowManager->gogoGadgetSlider(NULL, WIN_STATUS_ENABLED, 360,
                                              250, 11, 100, &instData,
  ...
  instData.init();
  instData.m_style = GWS_VERT_SLIDER | GWS_MOUSE_TRACK;
  window = TheWindowManager->gogoGadgetSlider(
      NULL, WIN_STATUS_ENABLED | WIN_STATUS_IMAGE, 400, 250, 11, 100, &instData,
      &sliderData, NULL, TRUE);
  ...
  window = TheWindowManager->gogoGadgetSlider(NULL, WIN_STATUS_ENABLED, 200,
                                              400, 200, 11, &instData,
                                              &sliderData, NULL, TRUE);
  ...
  instData.m_style = GWS_HORZ_SLIDER | GWS_MOUSE_TRACK;
  window = TheWindowManager->gogoGadgetSlider(
      NULL, WIN_STATUS_ENABLED | WIN_STATUS_IMAGE, 200, 420, 200, 11, &instData,
      &sliderData, NULL, TRUE);
  ...
  window = TheWindowManager->gogoGadgetProgressBar(
      NULL, WIN_STATUS_ENABLED, 200, 450, 250, 15, &instData, NULL, TRUE);

  // make a progress bar
  instData.init();
  instData.m_style = GWS_PROGRESS_BAR | GWS_MOUSE_TRACK;
  window = TheWindowManager->gogoGadgetProgressBar(
      NULL, WIN_STATUS_ENABLED | WIN_STATUS_IMAGE, 200, 470, 250, 15, &instData,
      NULL, TRUE);
  ...
  window = TheWindowManager->gogoGadgetStaticText(NULL, WIN_STATUS_ENABLED, 200,
                                                  490, 300, 25, &instData,
                                                  &textData, NULL, TRUE);
  ...
  window = TheWindowManager->gogoGadgetStaticText(
      NULL, WIN_STATUS_ENABLED | WIN_STATUS_IMAGE, 200, 520, 300, 25, &instData,
      &textData, NULL, TRUE);
  window->winSetEnabledTextColors(
      TheWindowManager->winMakeColor(128, 128, 255, 255),
      TheWindowManager->winMakeColor(255, 255, 255, 255));
  ...
  window = TheWindowManager->gogoGadgetTextEntry(NULL, WIN_STATUS_ENABLED, 450,
                                                 270, 400, 30, &instData,
                                                 &entryData, NULL, TRUE);
  ...
  instData.m_style = GWS_ENTRY_FIELD | GWS_MOUSE_TRACK;
  instData.m_textLabelString = "Entry";
  window = TheWindowManager->gogoGadgetTextEntry(
      NULL, WIN_STATUS_ENABLED | WIN_STATUS_IMAGE, 450, 310, 400, 30, &instData,
      &entryData, NULL, TRUE);
...
void GameWindowManager::winNextTab(GameWindow *window) {
  if (m_tabList.size() == 0 || m_modalHead) return;
...
void GameWindowManager::clearTabList(void) { m_tabList.clear(); }
