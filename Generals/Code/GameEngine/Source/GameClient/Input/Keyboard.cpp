...
void Keyboard::createStreamMessages(void) {
  // santiy
  if (TheMessageStream == NULL) return;
    ...
    if (m_keys[index].key == KEY_TAB) {
      if (BitTest(m_keyStatus[KEY_LALT].state, KEY_STATE_DOWN) ||
        ...
        m_keys[index].status = KeyboardIO::STATUS_USED;
               ...
               m_keys[index].key == KEY_LALT || m_keys[index].key == KEY_RALT)
  ...
  for (key = 0; key < 256; key++) {
    if (BitTest(m_keyStatus[key].state, KEY_STATE_DOWN)) {
      if ((m_inputFrame - m_keyStatus[key].sequence) >
        ...
        m_keyStatus[key].sequence =
            m_inputFrame - (Keyboard::KEY_REPEAT_DELAY + 2);
...
#define _set_keyname_(k, s, s2, z)                      \
  (m_keyNames[z].stdKey = k, m_keyNames[z].shifted = s, \
   m_keyNames[z].shifted2 = s2)
  ...
  if (low == 0x040c || low == 0x080c || low == 0x0c0c || low == 0x100c ||
      low == 0x140c)
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
Keyboard::~Keyboard(void) {}  // end ~Keyboard
...
//-------------------------------------------------------------------------------------------------
void Keyboard::reset(void) {}  // end reset
...
WideChar Keyboard::translateKey(WideChar keyCode) {
  if (keyCode > 0x00FF) return keyCode;
  ...
  switch (ubKeyCode) {
    ...
    default:
      ...
      if (isShift() ||
          getCapsState() && GameIsAlpha(m_keyNames[ubKeyCode].stdKey)) {
...
Bool Keyboard::isShift() {
  if (m_modifiers & KEY_STATE_LSHIFT || m_modifiers & KEY_STATE_RSHIFT ||
      m_modifiers & KEY_STATE_SHIFT2) {
    return TRUE;
  }
  return FALSE;
...
WideChar Keyboard::getPrintableKey(UnsignedByte key, Int state) {
  if ((key < 0 || key >= KEY_NAMES_COUNT) ||
      (state < 0 || state >= MAX_KEY_STATES))
    return L'';
  if (state == 0)
