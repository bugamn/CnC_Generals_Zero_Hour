...
static const FieldParse TheMouseCursorFieldParseTable[] = {
    {"CursorText", INI::parseAsciiString, NULL,
     offsetof(CursorInfo, cursorText)},
    {"CursorTextColor", INI::parseRGBAColorInt, NULL,
     offsetof(CursorInfo, cursorTextColor)},
    {"CursorTextDropColor", INI::parseRGBAColorInt, NULL,
     offsetof(CursorInfo, cursorTextDropColor)},
    {"W3DModel", INI::parseAsciiString, NULL,
     offsetof(CursorInfo, W3DModelName)},
    ...
    {"HotSpot", INI::parseICoord2D, NULL,
     offsetof(CursorInfo, hotSpotPosition)},
...
static const FieldParse TheMouseFieldParseTable[] = {
    {"TooltipFontName", INI::parseAsciiString, NULL,
     offsetof(Mouse, m_tooltipFontName)},
    {"TooltipFontSize", INI::parseInt, NULL,
     offsetof(Mouse, m_tooltipFontSize)},
    {"TooltipFontIsBold", INI::parseBool, NULL,
     offsetof(Mouse, m_tooltipFontIsBold)},
    {"TooltipAnimateBackground", INI::parseBool, NULL,
     offsetof(Mouse, m_tooltipAnimateBackground)},
    {"TooltipFillTime", INI::parseInt, NULL,
     offsetof(Mouse, m_tooltipFillTime)},
    {"TooltipDelayTime", INI::parseInt, NULL,
     offsetof(Mouse, m_tooltipDelayTime)},
    {"TooltipTextColor", INI::parseRGBAColorInt, NULL,
     offsetof(Mouse, m_tooltipColorText)},
    {"TooltipHighlightColor", INI::parseRGBAColorInt, NULL,
     offsetof(Mouse, m_tooltipColorHighlight)},
    {"TooltipShadowColor", INI::parseRGBAColorInt, NULL,
     offsetof(Mouse, m_tooltipColorShadow)},
    {"TooltipBackgroundColor", INI::parseRGBAColorInt, NULL,
     offsetof(Mouse, m_tooltipColorBackground)},
    {"TooltipBorderColor", INI::parseRGBAColorInt, NULL,
     offsetof(Mouse, m_tooltipColorBorder)},
    {"TooltipWidth", INI::parsePercentToReal, NULL,
     offsetof(Mouse, m_tooltipWidth)},
    {"CursorMode", INI::parseInt, NULL, offsetof(Mouse, m_currentRedrawMode)},
    {"UseTooltipAltTextColor", INI::parseBool, NULL,
     offsetof(Mouse, m_useTooltipAltTextColor)},
    {"UseTooltipAltBackColor", INI::parseBool, NULL,
     offsetof(Mouse, m_useTooltipAltBackColor)},
    {"AdjustTooltipAltColor", INI::parseBool, NULL,
     offsetof(Mouse, m_adjustTooltipAltColor)},
    {"OrthoCamera", INI::parseBool, NULL, offsetof(Mouse, m_orthoCamera)},
    {"OrthoZoom", INI::parseReal, NULL, offsetof(Mouse, m_orthoZoom)},
    {"DragTolerance", INI::parseUnsignedInt, NULL,
     offsetof(Mouse, m_dragTolerance)},
    {"DragTolerance3D", INI::parseUnsignedInt, NULL,
     offsetof(Mouse, m_dragTolerance3D)},
    {"DragToleranceMS", INI::parseUnsignedInt, NULL,
     offsetof(Mouse, m_dragToleranceMS)},
...
void Mouse::updateMouseData() {
  ...
  if (index != 0) m_deadInputFrame = m_inputFrame;
...
void Mouse::processMouseEvent(Int index) {
  ...
  if (index == 0) checkForDrag();

  // add Mouse Position Changes to Master Position
  moveMouse(m_mouseEvents[index].pos.x, m_mouseEvents[index].pos.y,
            movementType);
...
void Mouse::checkForDrag(void) {
  if (m_currMouse.leftState && ((m_prevMouse.leftEvent == GWM_LEFT_DOWN) ||
                                (m_prevMouse.leftEvent == GWM_LEFT_DRAG))) {
    m_currMouse.leftEvent = GWM_LEFT_DRAG;
  }

  if (m_currMouse.rightState && ((m_prevMouse.rightEvent == GWM_RIGHT_DOWN) ||
                                 (m_prevMouse.rightEvent == GWM_RIGHT_DRAG))) {
    m_currMouse.rightEvent = GWM_RIGHT_DRAG;
  }
...
Bool Mouse::isClick(const ICoord2D *anchor, const ICoord2D *dest,
  ...
  if (abs(delta.x) > m_dragTolerance || abs(delta.y) > m_dragTolerance ||
...
CursorInfo::CursorInfo(void) {
  ...
  cursorTextDropColor.red = cursorTextDropColor.blue =
      cursorTextDropColor.green = 0;
...
/**Had to move this out of main init() because I need this data to properly
initialize the Win32 version of the mouse (by preloading resources before D3D
device is created).*/
...
void Mouse::mouseNotifyResolutionChange(void) {
  ...
  if (TheGlobalLanguageData &&
      TheGlobalLanguageData->m_tooltipFontName.name.isNotEmpty()) {
    m_tooltipDisplayString->setFont(TheFontLibrary->getFont(
        ...
        TheGlobalLanguageData->adjustFontSize(
            TheGlobalLanguageData->m_tooltipFontName.size),
...
void Mouse::createStreamMessages(void) {
  // santiy
  if (TheMessageStream == NULL) return;  // no place to put messages
  ...
  Int delay = m_tooltipDelayTime;
  if (m_tooltipDelay >= 0) delay = m_tooltipDelay;
  ...
  if (now - m_stillTime >= delay) {
    ...
    // DEBUG_LOG(("%d %d %d %d\n", TheGameClient->getFrame(), delay, now,
    // m_stillTime));
    m_displayTooltip = FALSE;
  }
  ...
  for (Int i = 0; i < m_eventsThisFrame; ++i) {
    processMouseEvent(i);
    if (m_currMouse.deltaPos.x || m_currMouse.deltaPos.y) m_stillTime = now;

    // button messages
    msg = NULL;
    switch (m_currMouse.leftEvent) {
      case GWM_LEFT_DOWN:
        msg = TheMessageStream->appendMessage(
            GameMessage::MSG_RAW_MOUSE_LEFT_BUTTON_DOWN);
        msg->appendPixelArgument(m_currMouse.pos);
        msg->appendIntegerArgument(TheKeyboard->getModifierFlags());
      ...
      case GWM_LEFT_DOUBLE_CLICK:
        msg = TheMessageStream->appendMessage(
            GameMessage::MSG_RAW_MOUSE_LEFT_DOUBLE_CLICK);
        msg->appendPixelArgument(m_currMouse.pos);
        msg->appendIntegerArgument(TheKeyboard->getModifierFlags());
        msg->appendIntegerArgument(m_currMouse.time);
        break;
      ...
      case GWM_LEFT_DRAG:
        msg = TheMessageStream->appendMessage(
            GameMessage::MSG_RAW_MOUSE_LEFT_DRAG);
        msg->appendPixelArgument(m_currMouse.pos);
    ...
    switch (m_currMouse.middleEvent) {
      ...
      case GWM_MIDDLE_DOWN:
        msg = TheMessageStream->appendMessage(
            GameMessage::MSG_RAW_MOUSE_MIDDLE_BUTTON_DOWN);
        msg->appendPixelArgument(m_currMouse.pos);
        msg->appendIntegerArgument(TheKeyboard->getModifierFlags());
        msg->appendIntegerArgument(m_currMouse.time);
        break;
      ...
      case GWM_MIDDLE_DOUBLE_CLICK:
        msg = TheMessageStream->appendMessage(
            GameMessage::MSG_RAW_MOUSE_MIDDLE_DOUBLE_CLICK);
        msg->appendPixelArgument(m_currMouse.pos);
        msg->appendIntegerArgument(TheKeyboard->getModifierFlags());
        msg->appendIntegerArgument(m_currMouse.time);
        break;
      ...
      case GWM_MIDDLE_UP:
        msg = TheMessageStream->appendMessage(
            GameMessage::MSG_RAW_MOUSE_MIDDLE_BUTTON_UP);
        msg->appendPixelArgument(m_currMouse.pos);
        msg->appendIntegerArgument(TheKeyboard->getModifierFlags());
        msg->appendIntegerArgument(m_currMouse.time);
        break;
      ...
      case GWM_MIDDLE_DRAG:
        msg = TheMessageStream->appendMessage(
            GameMessage::MSG_RAW_MOUSE_MIDDLE_DRAG);
        msg->appendPixelArgument(m_currMouse.pos);
        msg->appendPixelArgument(m_currMouse.deltaPos);
        msg->appendIntegerArgument(TheKeyboard->getModifierFlags());
        break;
    ...
    switch (m_currMouse.rightEvent) {
      ...
      case GWM_RIGHT_DOWN:
        msg = TheMessageStream->appendMessage(
            GameMessage::MSG_RAW_MOUSE_RIGHT_BUTTON_DOWN);
        msg->appendPixelArgument(m_currMouse.pos);
        msg->appendIntegerArgument(TheKeyboard->getModifierFlags());
        msg->appendIntegerArgument(m_currMouse.time);
        break;
      ...
      case GWM_RIGHT_DOUBLE_CLICK:
        msg = TheMessageStream->appendMessage(
            GameMessage::MSG_RAW_MOUSE_RIGHT_DOUBLE_CLICK);
        msg->appendPixelArgument(m_currMouse.pos);
        msg->appendIntegerArgument(TheKeyboard->getModifierFlags());
        msg->appendIntegerArgument(m_currMouse.time);
        break;
      ...
      case GWM_RIGHT_DRAG:
        msg = TheMessageStream->appendMessage(
            GameMessage::MSG_RAW_MOUSE_RIGHT_DRAG);
        msg->appendPixelArgument(m_currMouse.pos);
        msg->appendPixelArgument(m_currMouse.deltaPos);
        msg->appendIntegerArgument(TheKeyboard->getModifierFlags());
        break;
...
void Mouse::setCursorTooltip(UnicodeString tooltip, Int delay,
  ...
  if (!tooltip.isEmpty() && width != m_lastTooltipWidth) {
    ...
    // DEBUG_LOG(("Setting tooltip width to %d pixels (%g%% of the normal
  ...
  if (forceRecalc ||
      !m_isTooltipEmpty && tooltip.compare(m_tooltipDisplayString->getText())) {
  ...
  if (color) {
    if (m_useTooltipAltTextColor) {
      if (m_adjustTooltipAltColor) {
        m_tooltipTextColor.red =
            REAL_TO_INT((color->red + 1.0f) * 255.0f / 2.0f);
        m_tooltipTextColor.green =
            REAL_TO_INT((color->green + 1.0f) * 255.0f / 2.0f);
        m_tooltipTextColor.blue =
            REAL_TO_INT((color->blue + 1.0f) * 255.0f / 2.0f);
      } else {
...
// ------------------------------------------------------------------------------------------------
/** Set the text for the mouse cursor ... note that this is *NOT* the tooltip
 * text we can set to be at the mouse position */
// ------------------------------------------------------------------------------------------------
void Mouse::setMouseText(UnicodeString text, const RGBAColorInt *color,
                         const RGBAColorInt *dropColor) {
  // sanity, if no display string has been created, get out of here
  if (m_cursorTextDisplayString == NULL) return;

  // set the text into the cursor display string
  m_cursorTextDisplayString->setText(text);

  // save the colors to draw in
  if (color) m_cursorTextColor = *color;
  if (dropColor) m_cursorTextDropColor = *dropColor;
...
void Mouse::drawTooltip(void) {
  ...
  /// @todo: Still need to put in display logic so it puts the tool tips in a
  /// visable position on the edge of the screen
  if (m_displayTooltip && TheDisplay && m_tooltipDisplayString &&
    ...
    Int boxWidth =
        (m_tooltipAnimateBackground) ? (min(width, m_highlightPos)) : width;

#define GMC(x) GameMakeColor(x.red, x.green, x.blue, x.alpha)
#define COLOR(x) GMC(m_tooltipColor##x)
    TheDisplay->drawFillRect(
        xPos, yPos, boxWidth + 2, height + 2,
        GMC(m_tooltipBackColor));  // GameMakeColor(0,0,0,125));
    TheDisplay->drawOpenRect(xPos, yPos, boxWidth + 2, height + 2, 1.0,
                             COLOR(Border));  // GameMakeColor(20,20,20,255));
    ...
    m_tooltipDisplayString->draw(
        xPos + 2, yPos + 1, GMC(m_tooltipTextColor),
        COLOR(
            Shadow));  // GameMakeColor(220,220,220,255),GameMakeColor(20,20,20,125));
    ...
    m_tooltipDisplayString->setClipRegion(&clipRegion);
    m_tooltipDisplayString->draw(
        xPos + 2, yPos + 1, COLOR(Highlight),
        COLOR(
            Shadow));  // GameMakeColor(255,255,0,255),GameMakeColor(20,20,20,125));

    // get ready for the next part of the anim
    if (m_highlightPos < width + HIGHLIGHT_WIDTH) {
      UnsignedInt now = timeGetTime();
      m_highlightPos =
          (width * (now - m_highlightUpdateStart)) / m_tooltipFillTime;
    }
  }  // end if
...
// ------------------------------------------------------------------------------------------------
/** Draw the cursor text at the mouse position.  Note that this is *NOT* the
 * tooltip text */
// ------------------------------------------------------------------------------------------------
void Mouse::drawCursorText(void) {
  ...
  color = GameMakeColor(m_cursorTextColor.red, m_cursorTextColor.green,
                        m_cursorTextColor.blue, m_cursorTextColor.alpha);
  dropColor =
      GameMakeColor(m_cursorTextDropColor.red, m_cursorTextDropColor.green,
                    m_cursorTextDropColor.blue, m_cursorTextDropColor.alpha);
...
Int Mouse::getCursorIndex(const AsciiString &name) {
  if (name.isEmpty()) return INVALID_MOUSE_CURSOR;
  ...
  static char *CursorININames[NUM_MOUSE_CURSORS] = {
      //"InvalidMouseCursor",  // this entry is not actually a mouse cursor, but
      //just a
  ...
  for (Int i = 0; i < NUM_MOUSE_CURSORS; ++i) {
    if (name.compareNoCase(CursorININames[i]) == 0) return i;
  }

  DEBUG_CRASH(
      ("Mouse::getCursorIndex - Invalid cursor name '%s'\n", name.str()));
...
void Mouse::setCursor(MouseCursor cursor) {
  ...
  if (m_cursorTextDisplayString != NULL) {
    ...
    //
    // if there is text present into the cursor info, set it into the mouse,
    // otherwise just clear out the text for the cursor text
    //
