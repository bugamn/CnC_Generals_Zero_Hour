...
const FieldParse TerrainRoadType::m_terrainRoadFieldParseTable[] = {
    ...
    {"RoadWidthInTexture", INI::parseReal, NULL,
...
const FieldParse TerrainRoadType::m_terrainBridgeFieldParseTable[] = {
    ...
    {"BridgeScale", INI::parseReal, NULL,
     offsetof(TerrainRoadType, m_bridgeScale)},
    {"ScaffoldObjectName", INI::parseAsciiString, NULL,
     offsetof(TerrainRoadType, m_scaffoldObjectName)},
    {"ScaffoldSupportObjectName", INI::parseAsciiString, NULL,
     offsetof(TerrainRoadType, m_scaffoldSupportObjectName)},
    {"RadarColor", INI::parseRGBColor, NULL,
     offsetof(TerrainRoadType, m_radarColor)},

    {"TransitionEffectsHeight", INI::parseReal, NULL,
     offsetof(TerrainRoadType, m_transitionEffectsHeight)},
    {"NumFXPerType", INI::parseInt, NULL,
     offsetof(TerrainRoadType, m_numFXPerType)},

    {"BridgeModelName", INI::parseAsciiString, NULL,
     offsetof(TerrainRoadType, m_bridgeModelName)},
    {"Texture", INI::parseAsciiString, NULL,
     offsetof(TerrainRoadType, m_texture)},
    {"BridgeModelNameDamaged", INI::parseAsciiString, NULL,
     offsetof(TerrainRoadType, m_bridgeModelNameDamaged)},
    {"TextureDamaged", INI::parseAsciiString, NULL,
     offsetof(TerrainRoadType, m_textureDamaged)},
    {"BridgeModelNameReallyDamaged", INI::parseAsciiString, NULL,
     offsetof(TerrainRoadType, m_bridgeModelNameReallyDamaged)},
    {"TextureReallyDamaged", INI::parseAsciiString, NULL,
     offsetof(TerrainRoadType, m_textureReallyDamaged)},
    {"BridgeModelNameBroken", INI::parseAsciiString, NULL,
     offsetof(TerrainRoadType, m_bridgeModelNameBroken)},
    {"TextureBroken", INI::parseAsciiString, NULL,
     offsetof(TerrainRoadType, m_textureBroken)},

    {"TowerObjectNameFromLeft", INI::parseAsciiString, NULL,
     offsetof(TerrainRoadType, m_towerObjectName[BRIDGE_TOWER_FROM_LEFT])},
    {"TowerObjectNameFromRight", INI::parseAsciiString, NULL,
     offsetof(TerrainRoadType, m_towerObjectName[BRIDGE_TOWER_FROM_RIGHT])},
    {"TowerObjectNameToLeft", INI::parseAsciiString, NULL,
     offsetof(TerrainRoadType, m_towerObjectName[BRIDGE_TOWER_TO_LEFT])},
    {"TowerObjectNameToRight", INI::parseAsciiString, NULL,
     offsetof(TerrainRoadType, m_towerObjectName[BRIDGE_TOWER_TO_RIGHT])},

    {"DamagedToSound", INI::parseAsciiString, NULL,
     offsetof(TerrainRoadType, m_damageToSoundString[BODY_DAMAGED])},
    {"RepairedToSound", INI::parseAsciiString, NULL,
     offsetof(TerrainRoadType, m_repairedToSoundString[BODY_DAMAGED])},
...
// ------------------------------------------------------------------------------------------------
/** In the form of
 * Label = Transition:<Damage|Repair> ToState:<BODYTYPE> EffectNum:<INT>
 * OCL:<OCL NAME> */
// ------------------------------------------------------------------------------------------------
/*static*/ void TerrainRoadType::parseTransitionToOCL(INI *ini, void *instance,
                                                      void *store,
                                                      const void *userData) {
  const char *token;
  ...
  BodyDamageType state =
      (BodyDamageType)ini->scanIndexList(token, TheBodyDamageTypeNames);
  ...
  if (damageTransition)
    theInstance->friend_setDamageToOCLString(state, effectNum,
                                             AsciiString(token));
  else
    theInstance->friend_setRepairedToOCLString(state, effectNum,
                                               AsciiString(token));
 ...
 * FX:<FXLIST NAME> */
// ------------------------------------------------------------------------------------------------
/*static*/ void TerrainRoadType::parseTransitionToFX(INI *ini, void *instance,
                                                     void *store,
                                                     const void *userData) {
  const char *token;
  TerrainRoadType *theInstance = (TerrainRoadType *)instance;
  ...
  if (damageTransition)
    theInstance->friend_setDamageToFXString(state, effectNum,
                                            AsciiString(token));
  else
    theInstance->friend_setRepairedToFXString(state, effectNum,
                                              AsciiString(token));
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
TerrainRoadType::~TerrainRoadType(void) {}  // end ~TerrainRoadType
...
TerrainRoadType *TerrainRoadCollection::findRoad(AsciiString name) {
  ...
  for (road = m_roadList; road; road = nextRoad(road)) {
    if (road->getName() == name) return road;
...
TerrainRoadType *TerrainRoadCollection::findBridge(AsciiString name) {
  ...
  for (bridge = m_bridgeList; bridge; bridge = nextBridge(bridge)) {
    if (bridge->getName() == name) return bridge;
...
TerrainRoadType *TerrainRoadCollection::newBridge(AsciiString name) {
  ...
  if (defaultBridge) {
    ...
    bridge->friend_setBridgeModelNameDamaged(
        defaultBridge->getBridgeModelNameDamaged());
    bridge->friend_setBridgeModelNameReallyDamaged(
        defaultBridge->getBridgeModelNameReallyDamaged());
    bridge->friend_setBridgeModelNameBroken(
        defaultBridge->getBridgeModelNameBroken());
    bridge->friend_setTextureDamaged(defaultBridge->getTextureDamaged());
    bridge->friend_setTextureReallyDamaged(
        defaultBridge->getTextureReallyDamaged());
    bridge->friend_setTextureBroken(defaultBridge->getTextureBroken());

    bridge->friend_setTransitionEffectsHeight(
        defaultBridge->getTransitionEffectsHeight());
    bridge->friend_setNumFXPerType(defaultBridge->getNumFXPerType());
    for (Int state = BODY_PRISTINE; state < BODYDAMAGETYPE_COUNT; state++) {
      bridge->friend_setDamageToSoundString(
          (BodyDamageType)state,
          bridge->getDamageToSoundString((BodyDamageType)state));
      bridge->friend_setRepairedToSoundString(
          (BodyDamageType)state,
          bridge->getRepairedToSoundString((BodyDamageType)state));

      for (Int i = 0; i < MAX_BRIDGE_BODY_FX; i++) {
        bridge->friend_setDamageToOCLString(
            (BodyDamageType)state, i,
            bridge->getDamageToOCLString((BodyDamageType)state, i));
        bridge->friend_setDamageToFXString(
            (BodyDamageType)state, i,
            bridge->getDamageToOCLString((BodyDamageType)state, i));
        bridge->friend_setRepairedToOCLString(
            (BodyDamageType)state, i,
            bridge->getDamageToOCLString((BodyDamageType)state, i));
        bridge->friend_setRepairedToFXString(
            (BodyDamageType)state, i,
            bridge->getDamageToOCLString((BodyDamageType)state, i));
...
TerrainRoadType *TerrainRoadCollection::nextBridge(TerrainRoadType *bridge) {
  DEBUG_ASSERTCRASH(bridge->isBridge() == TRUE,
                    ("nextBridge, bridge is not a bridge\n"));
