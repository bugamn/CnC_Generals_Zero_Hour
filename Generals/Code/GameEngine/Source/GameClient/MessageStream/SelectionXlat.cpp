...
static Bool currentlyLookingForSelection() {
  // This needs to check if we are currently targetting for special weapons
  // fire.
...
/*friend*/ Bool selectFriendsWrapper(Drawable *draw, void *userData) {
  ...
  return info->translator->selectFriends(draw, info->createTeamMsg,
                                         info->dragSelecting) != 0;
...
Bool CanSelectDrawable(const Drawable *draw, Bool dragSelecting) {
  ...
  // Added this to support attacking cargo planes without being able to select
  // them. I added the KINDOF_FORCEATTACKABLE to them, but unsure if it's
  // possible to select something without the KINDOF_SELECTABLE -- so doing a
  // LATE code change. My gut says we should simply have the KINDOF_SELECTABLE
  // check only... but best to be safe.
  if (!obj->isKindOf(KINDOF_SELECTABLE) &&
  ...
  //
  // structures cannot be selected by a drag select, you must individually pick
  // them NOTE that this is really a convenience for the multi select context
  // sensitive UI, later we might want to allow you to drag select buildings if
  // only one building is actually in the selection area, but don't forget
  // complications like holding down a key to "add" to an already existing
  // selection list
  //
  // not allowing you to have multiple buildings selected drastically simplifies
  // the user interface ... including all those context sensitive commands that
  // we can just assume are for a single building selected.
  //
  if (dragSelecting && draw->isKindOf(KINDOF_STRUCTURE)) {
    return FALSE;
  }

  // You cannot select something that has a logic override of unselectability or
  // masked
  if (BitTest(obj->getStatusBits(),
              OBJECT_STATUS_UNSELECTABLE | OBJECT_STATUS_MASKED)) {
    return FALSE;
  }
  ...
  // Now allowing the selection of everything including enemies... but only if
  // not drag selecting. In fact the only way you can drag select is if the unit
  // is on your team.
 ...
 */
Bool SelectionTranslator::selectFriends(Drawable *draw,
                                        GameMessage *createTeamMsg,
                                        Bool dragSelecting) {
...
//-----------------------------------------------------------------------------
Bool SelectionTranslator::killThemKillThemAll(Drawable *draw,
                                              GameMessage *killThemAllMsg) {
  if (draw) {
    Object *obj = draw->getObject();
    if (obj) {
      // enforce an optional selection size limit
 ...
 * LEFT_DOWN -> LEFT_UP  to  LEFT_DOWN -> { LEFT_UP, AREA_SELECTION, or
 * DRAWABLE_PICKED }
 */
  ...
  switch (t) {
    ...
    case GameMessage::MSG_RAW_MOUSE_POSITION: {
      ...
      if (m_leftMouseButtonIsDown) {
        ...
        if (delta.x > TheMouse->m_dragTolerance ||
            delta.y > TheMouse->m_dragTolerance) {
        ...
        if (m_dragSelecting) {
          ...
          GameMessage *hintMsg = TheMessageStream->appendMessage(
              GameMessage::MSG_AREA_SELECTION_HINT);
      ...
      {
        // insert Mouseover hint into stream for CommandTranslator and HintSpy
        // to see.
        GameMessage *mouseoverMessage;

        // Kris: We want to show information such as the popup text on objects
        // that are forceattackable even
        //       when we're not in force attackable mode!
        UnsignedInt pickType =
            getPickTypesForContext(true /*TheInGameUI->isInForceAttackMode()*/);

        Drawable *underCursor = TheTacticalView->pickDrawable(
            &pixel, TheInGameUI->isInForceAttackMode(), (PickType)pickType);
        Object *objUnderCursor = underCursor ? underCursor->getObject() : NULL;

        if (objUnderCursor &&
            (!objUnderCursor->isEffectivelyDead() ||
             objUnderCursor->isKindOf(KINDOF_ALWAYS_SELECTABLE))) {
          mouseoverMessage = TheMessageStream->appendMessage(
              GameMessage::MSG_MOUSEOVER_DRAWABLE_HINT);
        ...
        } else  // else this is a mouseover terrain
        {
          ...
          mouseoverMessage = TheMessageStream->appendMessage(
              GameMessage::MSG_MOUSEOVER_LOCATION_HINT);
    ...
    case GameMessage::MSG_MOUSE_LEFT_DOUBLE_CLICK: {
      ...
      if (TheInGameUI->isInForceAttackMode()) break;

      const IRegion2D &region = msg->getArgument(0)->pixelRegion;

      // Single point. If there's a unit in there, double click will select all
      // of them.
      if (region.height() == 0 && region.width() == 0) {
        Bool selectAcrossMap =
            (BitTest(modifiers, KEY_STATE_ALT) ? TRUE : FALSE);

        // only allow things that are selectable. Also, we aren't allowed to
        Drawable *picked = TheTacticalView->pickDrawable(&region.lo, FALSE,
                                                         PICK_TYPE_SELECTABLE);

        // If there wasn't anyone to pick, then we want to propagate this double
        // click.
        if (picked == NULL) break;

        if (!picked->isMassSelectable()) break;

        Object *pickedObj = picked->getObject();

        // We have to have an object in order to be able to do interesting
        // double click stuff on him. Also, if it is a structure, it is already
        // selected, so don't select all the units like him.
        if (pickedObj == NULL || !pickedObj->isLocallyControlled()) break;

        // Ok. The logic is a little bit weird here. What we need to do is
        // deselect everything except for this one picked thing. Store off the
        // old selection, pick the single clicked thing. Then if
        ...
        // Yay. Either select across the screen or the world depending on
        // selectAcrossMap
        if (selectAcrossMap)
        ...
        // emit "picked" message
        GameMessage *pickMsg =
            TheMessageStream->appendMessage(GameMessage::MSG_AREA_SELECTION);
        pickMsg->appendDrawableIDArgument(
            picked->getID());  /// note we are putting in a drawable id

        if (TheInGameUI->isInPreferSelectionMode() &&
            !listOfSelectedDrawables.empty()) {
          GameMessage *selectMore = TheMessageStream->appendMessage(
              GameMessage::MSG_CREATE_SELECTED_GROUP_NO_SOUND);
          selectMore->appendBooleanArgument(FALSE);
          for (DrawableListIt it = listOfSelectedDrawables.begin();
               it != listOfSelectedDrawables.end(); ++it) {
            Drawable *draw = *it;
    ...
    case GameMessage::MSG_MOUSEOVER_DRAWABLE_HINT: {
      ...
      GameMessage::Type msgType = TheGameClient->evaluateContextCommand(
          draw, draw->getPosition(), CommandTranslator::EVALUATE_ONLY);
    ...
    case GameMessage::MSG_MOUSE_LEFT_CLICK: {
      // If the quit menu is visible, we need to not process left clicks through
      // the selection translator.
      if (TheInGameUI->isQuitMenuVisible()) {
        disp = DESTROY_MESSAGE;
        break;
      }

      // Basically, we need to first determine if there are any drawables in the
      // region of interest. If there aren't then this click should move
      // forward.
      IRegion2D selectionRegion = msg->getArgument(0)->pixelRegion;
      Bool isPoint =
          (selectionRegion.height() == 0 && selectionRegion.width() == 0);

      DrawableList drawablesThatWillSelect;
      PickDrawableStruct pds;
      pds.drawableListToFill = &drawablesThatWillSelect;
      TheTacticalView->iterateDrawablesInRegion(&selectionRegion,
                                                addDrawableToList, &pds);

      if (drawablesThatWillSelect.empty()) {
        break;
      }

      // if there were drawables in the region, then we should determine if
      // there is a context sensitive command that should take place. If there
      // is, then this isn't a selection thing
      ...
      SelectionInfo si;
      if (contextCommandForNewSelection(currentList, &drawablesThatWillSelect,
                                        &si, isPoint)) {
        break;
      }

      // There isn't a context command, so this is a selection thing. Now, based
      // on the keys, determine whether or not we should create a new group, or
      // append these guys to our existing group.

      Bool addToGroup = TheInGameUI->isInPreferSelectionMode();

      if (si.currentCountEnemies > 0 || si.currentCountCivilians > 0 ||
          si.currentCountFriends > 0 || si.currentCountMineBuildings > 0) {
      ...
      } else if (si.newCountEnemies > 0 && si.newCountCivilians > 0 &&
                 si.newCountFriends > 0) {
        // No go here
        break;
      ...
      if (!(si.selectMine || si.selectEnemies || si.selectCivilians ||
            si.selectFriends)) {
        break;
      }
      ...
      if (TheInGameUI->isInPreferSelectionMode() && isPoint &&
          areAllSelected(drawablesThatWillSelect)) {
        // If this was a point, shift was pressed and we already have that unit
        // selected, then we need to deselect those units.
        GameMessage *newMsg = TheMessageStream->appendMessage(
            GameMessage::MSG_REMOVE_FROM_SELECTED_GROUP);
        Drawable *draw = NULL;
        DrawableListIt it;
        for (it = drawablesThatWillSelect.begin();
        ...
        if (newDrawablesSelected == 1 && draw) {
          ...
          if (TheHandOfGodSelectionMode && draw) {
            ...
            if (obj) {
              TheAudio->addAudioEvent(
                  &TheAudio->getMiscAudio()->m_noCanDoSound);
              GameMessage *msg = TheMessageStream->appendMessage(
                  GameMessage::MSG_DEBUG_KILL_OBJECT);
              msg->appendObjectIDArgument(obj->getID());
            }
            disp = DESTROY_MESSAGE;
            break;
            ...
            if (obj) {
              TheAudio->addAudioEvent(
                  &TheAudio->getMiscAudio()->m_noCanDoSound);
              GameMessage *msg = TheMessageStream->appendMessage(
                  GameMessage::MSG_DEBUG_HURT_OBJECT);
              msg->appendObjectIDArgument(obj->getID());
            }
            disp = DESTROY_MESSAGE;
            break;
          ...
          if (TheDebugSelectionMode && draw && draw->getObject()) {
            if (TheObjectIDToDebug == 0) {
              ...
              AsciiString msg;
              msg.format("Item %s %08x selected for debugging",
      ...
      if (disp == DESTROY_MESSAGE) TheInGameUI->clearAttackMoveToMode();
    ...
    case GameMessage::MSG_RAW_MOUSE_LEFT_BUTTON_DOWN: {
      // cannot actually start area selection yet - have to wait for cursor to
      // move a bit
    ...
    case GameMessage::MSG_RAW_MOUSE_LEFT_BUTTON_UP: {
      ...
      if (m_dragSelecting) {
        ...
        buildRegion(&m_selectFeedbackAnchor, &msg->getArgument(0)->pixel,
                    &selectionRegion);
    ...
    case GameMessage::MSG_RAW_MOUSE_RIGHT_BUTTON_UP: {
      ...
      Bool isClick = TRUE;
      if (isClick && abs(delta.x) > TheMouse->m_dragTolerance ||
          abs(delta.y) > TheMouse->m_dragTolerance) {
        isClick = FALSE;
      }

      if (isClick && currentTime - m_lastClick > TheMouse->m_dragToleranceMS) {
        isClick = FALSE;
      }

      if (isClick && cameraPos.length() > TheMouse->m_dragTolerance3D) {
        isClick = FALSE;
      }

      // right click behavior (not right drag)
      if (isClick) {
        // Added support to cancel the GUI command without deselecting the
        // unit(s) involved when you right click.
        if (TheInGameUI->getGUICommand()) {
          ...
          // No GUI command mode, so deselect everyone if we're in regular mouse
          // mode. In alternate mouse mode, right click still cancels building
          // placement.
          if (!TheGlobalData->m_useAlternateMouse ||
              TheInGameUI->getPendingPlaceSourceObjectID() != INVALID_ID) {
            deselectAll();
          }
        }
      }
          ...
          if (player) {
            ...
            if (selectedSquad != NULL) {
              ...
              if (numObjs > 0) {
                ...
                TheTacticalView->lookAt(
                    objlist[numObjs - 1]->getDrawable()->getPosition());
              }
            }
          }
        } else {
          TheInGameUI->deselectAllDrawables(
              false);  // No need to post message because we're just creating a
                       // new group!

          // no need to send two messages for selecting the same group.
          TheMessageStream->appendMessage(
              (GameMessage::Type)(GameMessage::MSG_SELECT_TEAM0 + group));
          Player *player = ThePlayerList->getLocalPlayer();
          if (player) {
            Squad *selectedSquad = player->getHotkeySquad(group);
            if (selectedSquad != NULL) {
    ...
    case GameMessage::MSG_META_ADD_TEAM9: {
      ...
      if (group >= 0 && group < 10) {
        ...
        if (now - m_lastGroupSelTime < 20 && group == m_lastGroupSelGroup) {
          ...
          // no need to send two messages for selecting the same group.
          TheMessageStream->appendMessage(
              (GameMessage::Type)(GameMessage::MSG_ADD_TEAM0 + group));
          Player *player = ThePlayerList->getLocalPlayer();
          if (player) {
            Squad *selectedSquad = player->getHotkeySquad(group);
            if (selectedSquad != NULL) {
    ...
    case GameMessage::MSG_META_OPTIONS: {
      // stop drawing selection feedback, as we're going to ignore the
      // selection.
      m_leftMouseButtonIsDown = FALSE;
      // let this message drop through, the commandXLat will show the options
      // screen itself.
      break;
    }
    ...
    case GameMessage::MSG_META_DEMO_TOGGLE_HAND_OF_GOD_MODE: {
      ...
      TheInGameUI->message(UnicodeString(L"Hand-Of-God Mode is %s"),
                           TheHandOfGodSelectionMode ? L"ON" : L"OFF");
      disp = DESTROY_MESSAGE;
      break;
    }
#endif
    ...
    case GameMessage::MSG_META_DEMO_TOGGLE_HURT_ME_MODE: {
      ...
      TheInGameUI->message(UnicodeString(L"Hurt-Me Mode is %s"),
                           TheHurtSelectionMode ? L"ON" : L"OFF");
      disp = DESTROY_MESSAGE;
      break;
    }
#endif
    ...
    case GameMessage::MSG_META_DEMO_DEBUG_SELECTION: {
      ...
      TheInGameUI->message(UnicodeString(L"Debug-Selected-Item Mode is %s"),
                           TheDebugSelectionMode ? L"ON" : L"OFF");
#ifdef DEBUG_OBJECT_ID_EXISTS
