...
// ///////////////////////////////////////////////////////////////// Author:
// Colin Day, March 2002 Desc:   Translator for commands activated from the
// selection GUI, such as special unit
//				 actions, that require additional clicks in the
//world like selecting a target 				 object or location
///////////////////////////////////////////////////////////////////////////////////////////////////
...
// PRIVATE
// ////////////////////////////////////////////////////////////////////////////////////////
static enum CommandStatus { COMMAND_INCOMPLETE = 0, COMMAND_COMPLETE };
...
//-------------------------------------------------------------------------------------------------
static Object *validUnderCursor(const ICoord2D *mouse,
  ...
  if (pick && pick->getObject()) {
    ...
    if (!command->isValidObjectTarget(player, pickObj)) pickObj = NULL;
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
static CommandStatus doFireWeaponCommand(const CommandButton *command,
                                         const ICoord2D *mouse) {
  // sanity
  if (command == NULL || mouse == NULL) return COMMAND_COMPLETE;

  //
  // for single object selections get the source ID and sanity check for illegal
  // object and bail along the way
  //
  ...
  if (TheInGameUI->getSelectCount() == 1) {
    ...
    // sanity
    if (draw == NULL || draw->getObject() == NULL) return COMMAND_COMPLETE;
  ...
  if (BitTest(command->getOptions(), NEED_TARGET_POS)) {
    ...
    msg =
        TheMessageStream->appendMessage(GameMessage::MSG_DO_WEAPON_AT_LOCATION);
    ...
    if (target) {
      msg =
          TheMessageStream->appendMessage(GameMessage::MSG_DO_WEAPON_AT_OBJECT);
      msg->appendIntegerArgument(command->getWeaponSlot());
    ...
    //-----------------------------------------------------------------
    // DEBUG_ASSERTCRASH( 0, ("doFireWeaponCommand: Command options say it
    // doesn't need additional user input '%s'\n", 											command->m_name.str()) );
...
static CommandStatus doGuardCommand(const CommandButton *command,
  ...
  if (TheInGameUI->getSelectCount() == 0) return COMMAND_COMPLETE;
...
//-------------------------------------------------------------------------------------------------
static CommandStatus doAttackMoveCommand(const CommandButton *command,
                                         const ICoord2D *mouse) {
  // sanity
  if (command == NULL || mouse == NULL) return COMMAND_COMPLETE;

  //
  // we can only set rally points for structures ... and we never multiple
  // select structures so we must be sure there is only one thing selected (that
  // thing we will set the point on)
  //
  Drawable *draw = TheInGameUI->getFirstSelectedDrawable();
...
/** Do the set rally point command */
//-------------------------------------------------------------------------------------------------
static CommandStatus doSetRallyPointCommand(const CommandButton *command,
                                            const ICoord2D *mouse) {
  // sanity
  if (command == NULL || mouse == NULL) return COMMAND_COMPLETE;
  ...
  DEBUG_ASSERTCRASH(TheInGameUI->getSelectCount() == 1,
                    ("doSetRallyPointCommand: The selected count is not 1, we "
                     "can only set a rally point on a *SINGLE* building\n"));
  Drawable *draw = TheInGameUI->getFirstSelectedDrawable();
  ...
  // send the message to set the rally point
  GameMessage *msg =
      TheMessageStream->appendMessage(GameMessage::MSG_SET_RALLY_POINT);
...
//-------------------------------------------------------------------------------------------------
static CommandStatus doPlaceBeacon(const CommandButton *command,
                                   const ICoord2D *mouse) {
  // sanity
  if (command == NULL || mouse == NULL) return COMMAND_COMPLETE;
  ...
  // send the message to set the rally point
  GameMessage *msg =
      TheMessageStream->appendMessage(GameMessage::MSG_PLACE_BEACON);
  msg->appendLocationArgument(world);
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
GameMessageDisposition GUICommandTranslator::translateGameMessage(
    const GameMessage *msg) {
  GameMessageDisposition disp = KEEP_MESSAGE;

  // only pay attention to clicks in this translator if there is a pending GUI
  // command
  const CommandButton *command = TheInGameUI->getGUICommand();
  if (command == NULL) return disp;
  ...
  switch (msg->getType()) {
    ...
    case GameMessage::MSG_RAW_MOUSE_LEFT_BUTTON_DOWN: {
      //
      //
      // it is necessary to use this input when there is a pending gui command,
      // we don't wan't it to fall through to the rest of the system when we're
      // in pending gui command "mode" because things like selection rectangles
      // will start when we want to stay totally within the gui command "mode"
      // here
      //
      ...
      if (command && !command->isContextCommand()) {
        switch (command->getCommandType()) {
          ...
          case GUI_COMMAND_GUARD_WITHOUT_PURSUIT: {
            commandStatus = doGuardCommand(
                command, GUARDMODE_GUARD_WITHOUT_PURSUIT, &mouse);
            break;
          }
          ...
          case GUI_COMMAND_GUARD_FLYING_UNITS_ONLY: {
            commandStatus = doGuardCommand(
                command, GUARDMODE_GUARD_FLYING_UNITS_ONLY, &mouse);
            break;
          }
        ...
        // get out of GUI command mode if we completed the command one way or
        // another
        if (commandStatus == COMMAND_COMPLETE) TheInGameUI->setGUICommand(NULL);
      }  // end if
  ...
  // If we're destroying the message, it means we used it. Therefore, destroy
  // the current attack move instruction as well.
