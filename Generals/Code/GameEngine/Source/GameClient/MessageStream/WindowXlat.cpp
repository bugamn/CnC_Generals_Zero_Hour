...
//						 on the stream from the input
//devices and acts on anything 						 relevant to the windowing system.
//
//-----------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////
...
static GameWindowMessage rawMouseToWindowMessage(const GameMessage *msg) {
  ...
  switch (msg->getType()) {
    ...
    // ------------------------------------------------------------------------
    // Strange, but true. The window stuff really doesn't care about double
    // clicks, so just treat it as a down click.. Kinda like a second click.
...
//=============================================================================
GameMessageDisposition WindowTranslator::translateGameMessage(
  ...
  switch (msg->getType()) {
    ...
    case GameMessage::MSG_META_TOGGLE_ATTACKMOVE: {
      // Basically, we're cheating here. The mouse no longer sends us useless
      // spam.
      ...
      // Force it to keep the message, regardless of what the window thinks it
      // did with the input.
      return KEEP_MESSAGE;
    }
    ...
    defined(_INTERNAL)  // debug hack to view object under mouse stats
      ...
      if (TheWindowManager)
        returnCode =
            TheWindowManager->winProcessMouseEvent(gwm, &mousePos, NULL);

      if (TheShell && TheShell->isShellActive()) returnCode = WIN_INPUT_USED;
    ...
    case GameMessage::MSG_RAW_MOUSE_RIGHT_DRAG: {
      ...
      if (TheWindowManager)
        returnCode =
            TheWindowManager->winProcessMouseEvent(gwm, &mousePos, &delta);
    ...
    case GameMessage::MSG_RAW_MOUSE_WHEEL: {
      ...
      if (TheWindowManager)
        returnCode =
            TheWindowManager->winProcessMouseEvent(gwm, &mousePos, &wheelPos);
    ...
    case GameMessage::MSG_RAW_KEY_UP: {
      ...
      if (returnCode != WIN_INPUT_USED && (key == KEY_ESC) &&
          (BitTest(state, KEY_STATE_UP)) && TheDisplay->isMoviePlaying() &&
  ...
  if (returnCode == WIN_INPUT_USED &&
      !forceKeepMessage)  // || (TheShell && TheShell->isShellActive()))
  {
