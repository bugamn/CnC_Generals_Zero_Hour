...
//-------------------------------------------------------------------------------------------------
/** Translator to process raw input messages into the "place something"
 * message(s) */
//-------------------------------------------------------------------------------------------------
GameMessageDisposition PlaceEventTranslator::translateGameMessage(
  ...
  switch (msg->getType()) {
    ...
    case GameMessage::MSG_RAW_MOUSE_LEFT_BUTTON_DOWN: {
      // if we're in a building placement mode, do the place and send to all
      // players
      ...
      if (build && TheInGameUI->isPlacementAnchored() == FALSE) {
        ...
        //
        // placing things causes a dozer to go over and build it ... get the
        // dozer in question from the in game UI
        //
        ...
        /*
        //
        // This block of code checks for valid placement on a down mouse click,
        but since we can
        // rotate a building into a valid location, this check prevents us from
        placing things
        // down in some legal locations
        //
                                        // get the type of thing we want to
        build const ThingTemplate *whatToBuild =
        TheInGameUI->getPendingPlaceType();

                                        //
                                        // if the spot at which they choose to
        place this thing is illegal we won't start
                                        // the placement anchor, instead we play
        a "can't do that" sound
                                        //
                                        LegalBuildCode lbc;
                                        lbc =
        TheBuildAssistant->isLocationLegalToBuild( &world, whatToBuild,
                                        ...
                                        if( lbc != LBC_OK )
                                        {
                                                static const Sound *noCanDoSound
        = TheAudio->Sounds->getSound( "NoCanDoSound" );

                                                // play a can't do that sound
                                                TheAudio->Sounds->playSound(
        noCanDoSound );

                                                // display a message to the user
        as to why you can't build there TheInGameUI->displayCantBuildMessage(
        lbc );
    ...
    case GameMessage::MSG_MOUSE_LEFT_CLICK: {
      ...
      // (srj sez: not sure if this is always necessary... more of a failsafe to
      // make it go away.)
      ...
      if (build && TheInGameUI->isPlacementAnchored()) {
        ...
        // get the angle of the drawable at the cursor to use as the initial
        // angle
        ...
        Object *builderObj = TheGameLogic->findObjectByID(
            TheInGameUI->getPendingPlaceSourceObjectID());
        if (builderObj) builderID = builderObj->getID();

        // Kris: September 27, 2002
        // Make sure we have enough CASH to build it! It's possible that between
        // the time we initiated it and the time we confirm it, a hacker has
        // stolen some of our cash!
        ...
        if (cmt != CANMAKE_OK) {
          ...
          // get out of pending placement mode, this will also clear the arrow
          // anchor status
          TheInGameUI->placeBuildAvailable(NULL, NULL);
          break;
        }
        ...
        lbc = TheBuildAssistant->isLocationLegalToBuild(
            ...
            builderObj, NULL);
        if (lbc == LBC_OK) {
          /** @todo Do not send local player id as argument once we have player
          ids tied into all messages automatically */

          // create the right kind of message
          if (isLineBuild)
            placeMsg = TheMessageStream->appendMessage(
          ...
          AudioEventRTS sound =
              *builderObj->getTemplate()->getPerUnitSound("VoiceNoBuild");
    ...
    case GameMessage::MSG_RAW_MOUSE_POSITION: {
      ...
      if (TheInGameUI->isPlacementAnchored()) {
        const Int PLACEMENT_DRAG_THRESHOLD_DIST =
            5;  // in pixels away from anchor point
        ICoord2D mouse = msg->getArgument(0)->pixel;

        //
        // we will only process placement end point sets (clicking, and dragging
        // to set angles) if we have moved far enough away from the start point
        //
        ICoord2D start;
