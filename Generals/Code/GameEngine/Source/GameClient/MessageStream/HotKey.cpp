...
//-----------------------------------------------------------------------------
GameMessageDisposition HotKeyTranslator::translateGameMessage(
  ...
  if (t == GameMessage::MSG_RAW_KEY_UP) {
    ...
    if (newModState != 0) return disp;
    WideChar key =
        TheKeyboard->getPrintableKey(msg->getArgument(0)->integer, 0);
    UnicodeString uKey;
    uKey.set(&key);
    AsciiString aKey;
    aKey.translate(uKey);
...
//-----------------------------------------------------------------------------
HotKeyManager::~HotKeyManager(void) { m_hotKeyMap.clear(); }

//-----------------------------------------------------------------------------
void HotKeyManager::init(void) { m_hotKeyMap.clear(); }

//-----------------------------------------------------------------------------
void HotKeyManager::reset(void) { m_hotKeyMap.clear(); }
...
Bool HotKeyManager::executeHotKey(const AsciiString &keyIn) {
  ...
  key.toLower();
  HotKeyMap::iterator it = m_hotKeyMap.find(key);
  if (it == m_hotKeyMap.end()) return FALSE;
  ...
  if (!BitTest(win->winGetStatus(), WIN_STATUS_HIDDEN)) {
    if (BitTest(win->winGetStatus(), WIN_STATUS_ENABLED)) {
      ...
      // here we make the same click sound that the GUI uses when you click a
      // button
...
AsciiString HotKeyManager::searchHotKey(const UnicodeString &uStr) {
  if (uStr.isEmpty()) return AsciiString::TheEmptyString;
