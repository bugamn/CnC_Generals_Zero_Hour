...
static enum { DIR_UP = 0, DIR_DOWN, DIR_LEFT, DIR_RIGHT };
...
void LookAtTranslator::setScrolling(Int x) {
  if (!TheInGameUI->getInputEnabled()) return;
  ...
  m_scrollType = x;
  if (TheStatsCollector) TheStatsCollector->startScrollTime();
}
...
void LookAtTranslator::stopScrolling(void) {
  ...
  if (TheStatsCollector) TheStatsCollector->endScrollTime();
}
...
LookAtTranslator::LookAtTranslator()
  ...
  DEBUG_ASSERTCRASH(!TheLookAtTranslator,
                    ("Already have a LookAtTranslator - why do you need two?"));
  TheLookAtTranslator = this;
}

//-----------------------------------------------------------------------------
LookAtTranslator::~LookAtTranslator() {
  if (TheLookAtTranslator == this) TheLookAtTranslator = NULL;
}
...
//-----------------------------------------------------------------------------
/**
 * The LookAt Translator is responsible for camera movements. It is directly
 * responsible for right mouse button scrolling, and CTRL-<F key> bookmarking.
 * It also responds to certain LOOKAT message on the message stream.
 */
GameMessageDisposition LookAtTranslator::translateGameMessage(
  ...
  switch (t) {
    ...
    case GameMessage::MSG_RAW_KEY_UP: {
      ...
      if (TheShell && TheShell->isShellActive()) break;
      ...
      if (TheInGameUI->isSelecting() ||
          (m_isScrolling && m_scrollType != SCROLL_KEY))
        break;

      // see if we need to start/stop scrolling
      Int numDirs = 0;
      for (Int i = 0; i < 4; ++i) {
        if (scrollDir[i]) numDirs++;
      }
    ...
    case GameMessage::MSG_RAW_MOUSE_RIGHT_BUTTON_DOWN: {
      ...
      if (!TheGlobalData->m_useAlternateMouse && !TheInGameUI->isSelecting() &&
          !m_isScrolling) {
    ...
    case GameMessage::MSG_RAW_MOUSE_MIDDLE_BUTTON_UP: {
      ...
      if (!didMove &&
          TheGameClient->getFrame() - m_timestamp < CLICK_DURATION) {
    ...
    case GameMessage::MSG_RAW_MOUSE_POSITION: {
      if (m_currentPos.x != msg->getArgument(0)->pixel.x ||
          m_currentPos.y != msg->getArgument(0)->pixel.y)
        m_lastMouseMoveFrame = TheGameLogic->getFrame();
      ...
      if (TheInGameUI->getInputEnabled() == FALSE) {
        ...
        if (m_isScrolling) stopScrolling();
        break;
      }
      ...
      if (!TheGlobalData->m_windowed) {
        if (m_isScrolling) {
          if (m_scrollType == SCROLL_SCREENEDGE &&
              (m_currentPos.x >= edgeScrollSize &&
               m_currentPos.y >= edgeScrollSize &&
               m_currentPos.y < height - edgeScrollSize &&
               m_currentPos.x < width - edgeScrollSize)) {
            stopScrolling();
          }
        } else {
      ...
      if (m_isChangingFOV) {
        ...
        TheTacticalView->setFieldOfView(TheTacticalView->getFieldOfView() +
                                        angle);
        m_anchor = msg->getArgument(0)->pixel;
      }
#endif
    ...
    case GameMessage::MSG_RAW_MOUSE_WHEEL: {
      ...
      if (spin > 0) {
        for (; spin > 0; spin--) TheTacticalView->zoomIn();
      } else {
        for (; spin < 0; spin++) TheTacticalView->zoomOut();
      }
    }
    ...
    case GameMessage::MSG_FRAME_TICK: {
        ...
        if (m_isScrolling) {
          switch (m_scrollType) {
            case SCROLL_RMB: {
              ...
              offset.x = TheGlobalData->m_horizontalScrollSpeedFactor *
                         (m_currentPos.x - m_anchor.x);
              offset.y = TheGlobalData->m_verticalScrollSpeedFactor *
                         (m_currentPos.y - m_anchor.y);
              Coord2D vec;
              vec.x = offset.x;
              vec.y = offset.y;
              vec.normalize();
              // Add in the window scroll amount as the minimum.
              offset.x += TheGlobalData->m_horizontalScrollSpeedFactor * vec.x *
                          sqr(TheGlobalData->m_keyboardScrollFactor);
              offset.y += TheGlobalData->m_verticalScrollSpeedFactor * vec.y *
                          sqr(TheGlobalData->m_keyboardScrollFactor);
            } break;
            case SCROLL_KEY: {
              if (scrollDir[DIR_UP]) {
                offset.y -= TheGlobalData->m_verticalScrollSpeedFactor *
                            SCROLL_AMT * TheGlobalData->m_keyboardScrollFactor;
              }
      ...
      // if (TheGlobalData->m_saveCameraInReplay /*&& TheRecorder->getMode() !=
      // RECORDERMODETYPE_PLAYBACK *//**/&&
      // (TheGameLogic->isInSinglePlayerGame() ||
      // TheGameLogic->isInSkirmishGame())/**/) if
      // (TheGlobalData->m_saveCameraInReplay &&
      // (TheGameLogic->isInMultiplayerGame() ||
      // TheGameLogic->isInSinglePlayerGame() ||
      // TheGameLogic->isInSkirmishGame()))
      if (TheGlobalData->m_saveCameraInReplay &&
          (TheGameLogic->isInSinglePlayerGame() ||
           TheGameLogic->isInSkirmishGame())) {
        ViewLocation currentView;
        TheTacticalView->getLocation(&currentView);
        GameMessage *msg =
            TheMessageStream->appendMessage(GameMessage::MSG_SET_REPLAY_CAMERA);
    ...
    case GameMessage::MSG_META_DEMO_ENSHROUD: {
      // Need to first undo the permanent Look laid down by DEMO_DESHROUD, then
      // blast a shroud dollop.
      ThePartitionManager->undoRevealMapForPlayerPermanently(
          ThePlayerList->getLocalPlayer()->getPlayerIndex());
      ThePartitionManager->shroudMapForPlayer(
          ThePlayerList->getLocalPlayer()->getPlayerIndex());
      break;
    }
#endif  // #if defined(_DEBUG) || defined(_INTERNAL)
    ...
    case GameMessage::MSG_META_DEMO_LOCK_CAMERA_TO_PLANES: {
      ...
      if (m_lastPlaneID) first = TheGameClient->findDrawableByID(m_lastPlaneID);

      if (first == NULL) first = TheGameClient->firstDrawable();
      ...
      if (first) {
        ...
        while (!done) {
          ...
          d = d->getNextDrawable();
          if (d == NULL) d = TheGameClient->firstDrawable();

          // if we've found an airborne object, lock onto it
          // "isAboveTerrain" only indicates that we are currently in the air,
          // but that could be the case if we are a buggy jumping a hill, or a
          // unit being paradropped. the right thing would be to look at the
          // locomotors. so this isn't really right, but will suffice for demo
          // purposes.
          if (d->getObject() && d->getObject()->isAboveTerrain()) {
            ...
            for (BehaviorModule **u = d->getObject()->getBehaviorModules(); *u;
                 ++u) {
