...
/// optimization disabled for debugging purposes")
#endif
...
static const LookupListRec GameMessageMetaTypeNames[] = {
    ...
    {"BEGIN_CAMERA_ROTATE_LEFT",
     GameMessage::MSG_META_BEGIN_CAMERA_ROTATE_LEFT},
    {"END_CAMERA_ROTATE_LEFT", GameMessage::MSG_META_END_CAMERA_ROTATE_LEFT},
    {"BEGIN_CAMERA_ROTATE_RIGHT",
     GameMessage::MSG_META_BEGIN_CAMERA_ROTATE_RIGHT},
    ...
    {"DEMO_TOGGLE_BEHIND_BUILDINGS",
     GameMessage::MSG_META_DEMO_TOGGLE_BEHIND_BUILDINGS},
    ...
    {"DEMO_GIVE_SCIENCEPURCHASEPOINTS",
     GameMessage::MSG_META_DEMO_GIVE_SCIENCEPURCHASEPOINTS},
    ...
    {"DEMO_TOGGLE_GRAPHICALFRAMERATEBAR",
     GameMessage::MSG_META_DEMO_TOGGLE_GRAPHICALFRAMERATEBAR},
    ...
    {"DEMO_TOGGLE_PROJECTILEDEBUG",
     GameMessage::MSG_META_DEMO_TOGGLE_PROJECTILEDEBUG},
    {"DEMO_LOD_DECREASE", GameMessage::MSG_META_DEMO_LOD_DECREASE},
    {"DEMO_LOD_INCREASE", GameMessage::MSG_META_DEMO_LOD_INCREASE},
    {"DEMO_TOGGLE_SHADOW_VOLUMES",
     GameMessage::MSG_META_DEMO_TOGGLE_SHADOW_VOLUMES},
    ...
    {"DEMO_TOGGLE_HAND_OF_GOD_MODE",
     GameMessage::MSG_META_DEMO_TOGGLE_HAND_OF_GOD_MODE},
    {"DEMO_DEBUG_SELECTION", GameMessage::MSG_META_DEMO_DEBUG_SELECTION},
    {"DEMO_LOCK_CAMERA_TO_SELECTION",
     GameMessage::MSG_META_DEMO_LOCK_CAMERA_TO_SELECTION},
    ...
    {"DEMO_NEXT_OBJECTIVE_MOVIE",
     GameMessage::MSG_META_DEMO_NEXT_OBJECTIVE_MOVIE},
    {"DEMO_PLAY_OBJECTIVE_MOVIE1",
     GameMessage::MSG_META_DEMO_PLAY_OBJECTIVE_MOVIE1},
    {"DEMO_PLAY_OBJECTIVE_MOVIE2",
     GameMessage::MSG_META_DEMO_PLAY_OBJECTIVE_MOVIE2},
    {"DEMO_PLAY_OBJECTIVE_MOVIE3",
     GameMessage::MSG_META_DEMO_PLAY_OBJECTIVE_MOVIE3},
    {"DEMO_PLAY_OBJECTIVE_MOVIE4",
     GameMessage::MSG_META_DEMO_PLAY_OBJECTIVE_MOVIE4},
    {"DEMO_PLAY_OBJECTIVE_MOVIE5",
     GameMessage::MSG_META_DEMO_PLAY_OBJECTIVE_MOVIE5},
    {"DEMO_PLAY_OBJECTIVE_MOVIE6",
     GameMessage::MSG_META_DEMO_PLAY_OBJECTIVE_MOVIE6},
    ...
    {"DEMO_LOCK_CAMERA_TO_PLANES",
     GameMessage::MSG_META_DEMO_LOCK_CAMERA_TO_PLANES},
    ...
    {"DEMO_TOGGLE_MOTION_BLUR_ZOOM",
     GameMessage::MSG_META_DEMO_TOGGLE_MOTION_BLUR_ZOOM},
    ...
    {"DEMO_TOGGLE_SPECIAL_POWER_DELAYS",
     GameMessage::MSG_META_DEMO_TOGGLE_SPECIAL_POWER_DELAYS},
    ...
    {"DEMO_TOGGLE_CAMERA_DEBUG",
     GameMessage::MSG_META_DEMO_TOGGLE_CAMERA_DEBUG},
    ...
    {"DEMO_TOGGLE_FEATHER_WATER",
     GameMessage::MSG_META_DEBUG_TOGGLE_FEATHER_WATER},

    {"DEMO_INCR_ANIM_SKATE_SPEED",
     GameMessage::MSG_META_DEBUG_INCR_ANIM_SKATE_SPEED},
    {"DEMO_DECR_ANIM_SKATE_SPEED",
     GameMessage::MSG_META_DEBUG_DECR_ANIM_SKATE_SPEED},
    {"DEMO_CYCLE_EXTENT_TYPE", GameMessage::MSG_META_DEBUG_CYCLE_EXTENT_TYPE},
    {"DEMO_INCR_EXTENT_MAJOR",
     GameMessage::MSG_META_DEBUG_INCREASE_EXTENT_MAJOR},
    {"DEMO_DECR_EXTENT_MAJOR",
     GameMessage::MSG_META_DEBUG_DECREASE_EXTENT_MAJOR},
    {"DEMO_INCR_EXTENT_MAJOR_LARGE",
     GameMessage::MSG_META_DEBUG_INCREASE_EXTENT_MAJOR_BIG},
    {"DEMO_DECR_EXTENT_MAJOR_LARGE",
     GameMessage::MSG_META_DEBUG_DECREASE_EXTENT_MAJOR_BIG},
    {"DEMO_INCR_EXTENT_MINOR",
     GameMessage::MSG_META_DEBUG_INCREASE_EXTENT_MINOR},
    {"DEMO_DECR_EXTENT_MINOR",
     GameMessage::MSG_META_DEBUG_DECREASE_EXTENT_MINOR},
    {"DEMO_INCR_EXTENT_MINOR_LARGE",
     GameMessage::MSG_META_DEBUG_INCREASE_EXTENT_MINOR_BIG},
    {"DEMO_DECR_EXTENT_MINOR_LARGE",
     GameMessage::MSG_META_DEBUG_DECREASE_EXTENT_MINOR_BIG},
    {"DEMO_INCR_EXTENT_HEIGHT",
     GameMessage::MSG_META_DEBUG_INCREASE_EXTENT_HEIGHT},
    ...
    {"DEMO_DECR_EXTENT_HEIGHT_LARGE",
     GameMessage::MSG_META_DEBUG_DECREASE_EXTENT_HEIGHT_BIG},
    {"DEMO_TOGGLE_NETWORK", GameMessage::MSG_META_DEBUG_TOGGLE_NETWORK},
    {"DEBUG_DUMP_PLAYER_OBJECTS",
     GameMessage::MSG_META_DEBUG_DUMP_PLAYER_OBJECTS},
    {"DEBUG_DUMP_ALL_PLAYER_OBJECTS",
     GameMessage::MSG_META_DEBUG_DUMP_ALL_PLAYER_OBJECTS},
    ...
    {"DEMO_PERFORM_STATISTICAL_DUMP",
     GameMessage::MSG_META_DEMO_PERFORM_STATISTICAL_DUMP},
...
static const FieldParse TheMetaMapFieldParseTable[] = {
    ...
    {"Transition", INI::parseLookupList, TransitionNames,
     offsetof(MetaMapRec, m_transition)},
    {"Modifiers", INI::parseLookupList, ModifierNames,
     offsetof(MetaMapRec, m_modState)},
    {"UseableIn", INI::parseBitString32, TheCommandUsableInNames,
     offsetof(MetaMapRec, m_usableIn)},
    {"Category", INI::parseLookupList, CategoryListName,
     offsetof(MetaMapRec, m_category)},
    {"Description", INI::parseAndTranslateLabel, 0,
     offsetof(MetaMapRec, m_description)},
    {"DisplayName", INI::parseAndTranslateLabel, 0,
     offsetof(MetaMapRec, m_displayName)},
...
MetaEventTranslator::MetaEventTranslator()
    : m_lastKeyDown(MK_NONE), m_lastModState(0) {
...
static const char *findGameMessageNameByType(GameMessage::Type type) {
  for (const LookupListRec *metaNames = GameMessageMetaTypeNames;
       metaNames->name; metaNames++)
    if (metaNames->value == (Int)type) return metaNames->name;

  DEBUG_CRASH(
      ("MetaTypeName %d not found -- did you remember to add it to "
       "GameMessageMetaTypeNames[] ?\n"));
  return "???";
}

//-------------------------------------------------------------------------------------------------
GameMessageDisposition MetaEventTranslator::translateGameMessage(
  ...
  if (t == GameMessage::MSG_RAW_KEY_DOWN || t == GameMessage::MSG_RAW_KEY_UP) {
    ...
    // for our purposes here, we don't care to distinguish between right and
    // left keys, so just fudge a little to simplify things.
    Int newModState = 0;
    ...
    for (const MetaMapRec *map = TheMetaMap->getFirstMetaMapRec(); map;
         map = map->m_next) {
      ...
      //
      // if this command is *only* usable in the game, we will ignore it if the
      // game client has not yet incremented to frame 1 (keeps us from doing
      // in-game commands during a map load, which throws the input system into
      // wack because there isn't a client frame for the input event, and in the
      // case of a command that pauses the game, like the quit menu, the client
      // frame will never get beyond 0 and we lose the ability to process any
      // input
      //
      if (map->m_usableIn == COMMANDUSABLE_GAME &&
          TheGameClient->getFrame() < 1)
        continue;

      // if the shell is active, and this command is not usable in shell,
      // continue
      if (TheShell && TheShell->isShellActive() &&
          !(map->m_usableIn & COMMANDUSABLE_SHELL))
        continue;

      // if the shell is not active and this command is not usable in the game,
      // continue
      if (TheShell && !TheShell->isShellActive() &&
      ...
      if (map->m_key == MK_NONE && newModState != m_lastModState &&
        ...
        // findGameMessageNameByType(map->m_meta)));
      ...
      if (map->m_key == key && map->m_modState == newModState &&
          ((map->m_transition == UP && (keyState & KEY_STATE_UP)) ||
           (map->m_transition == DOWN && (keyState & KEY_STATE_DOWN))  //||
           //(map->m_transition == DOUBLEDOWN && (keyState & KEY_STATE_DOWN) &&
           //m_lastKeyDown == key)
           )) {
        if (keyState & KEY_STATE_AUTOREPEAT) {
          // if it's an autorepeat of a "known" key, don't generate the
          // meta-event, but DO eat the keystroke so no one else can mess with
          // it
          // DEBUG_LOG(("Frame %d: MetaEventTranslator::translateGameMessage()
          // auto-repeat: %s\n", TheGameLogic->getFrame(),
          // findGameMessageNameByType(map->m_meta)));
        } else {
          /*GameMessage *metaMsg =*/TheMessageStream->appendMessage(
              map->m_meta);
          // DEBUG_LOG(("Frame %d: MetaEventTranslator::translateGameMessage()
          // normal: %s\n", TheGameLogic->getFrame(),
          // findGameMessageNameByType(map->m_meta)));
    ...
    if (t == GameMessage::MSG_RAW_KEY_DOWN) m_lastKeyDown = key;
    m_lastModState = newModState;
  }

  if (t > GameMessage::MSG_RAW_MOUSE_BEGIN &&
      t < GameMessage::MSG_RAW_MOUSE_END) {
    Int index = 0;
    switch (t) {
      ...
      case GameMessage::MSG_RAW_MOUSE_RIGHT_BUTTON_UP: {
        ...
        if (t == GameMessage::MSG_RAW_MOUSE_LEFT_BUTTON_UP) {
          if (m_nextUpShouldCreateDoubleClick[index])
            newMessage = TheMessageStream->insertMessage(
                GameMessage::MSG_MOUSE_LEFT_DOUBLE_CLICK,
                const_cast<GameMessage *>(msg));
          else
            newMessage = TheMessageStream->insertMessage(
        ...
        if (abs(pixelRegion.hi.x - pixelRegion.lo.x) <
                TheMouse->m_dragTolerance &&
            abs(pixelRegion.hi.y - pixelRegion.lo.y) <
                TheMouse->m_dragTolerance) {
...
//-------------------------------------------------------------------------------------------------
MetaMap::MetaMap() : m_metaMaps(NULL) {}
...
GameMessage::Type MetaMap::findGameMessageMetaType(const char *name) {
  ...
  DEBUG_CRASH(
      ("MetaTypeName %s not found -- did you remember to add it to "
       "GameMessageMetaTypeNames[] ?",
       name));
...
/*static */ void MetaMap::parseMetaMap(INI *ini) {
  ...
  if (t == GameMessage::MSG_INVALID) throw INI_INVALID_DATA;

  MetaMapRec *map = TheMetaMap->getMetaMapRec(t);
  if (map == NULL) throw INI_INVALID_DATA;
