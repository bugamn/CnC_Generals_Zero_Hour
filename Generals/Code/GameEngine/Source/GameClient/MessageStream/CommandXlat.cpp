...
void countObjects(Object *obj, void *userData) {
  ...
  if (!numObjects || !obj) return;

  DEBUG_LOG(
      ("Looking at obj %d (%s) - isEffectivelyDead()==%d, isDestroyed==%d, "
       "numObjects==%d\n",
       obj->getID(), obj->getTemplate()->getName().str(),
       obj->isEffectivelyDead(), obj->isDestroyed(), *numObjects));
...
void printObjects(Object *obj, void *userData) {
  ...
  if (obj->getName().isEmpty())
    line.format("  Obj#%d %s - (%g,%g) %s", obj->getID(),
                obj->getTemplate()->getName().str(), obj->getPosition()->x,
                obj->getPosition()->y, statusStr.str());
  else
    line.format("  Obj#%d %s (%s) - (%g,%g) %s", obj->getID(),
                obj->getTemplate()->getName().str(), obj->getName().str(),
...
//-------------------------------------------------------------------------------------------------
static CanAttackResult canObjectForceAttack(Object *obj, const Object *victim,
                                            const Coord3D *pos) {
  CanAttackResult result;
  result =
      obj->isAbleToAttack() ? ATTACKRESULT_POSSIBLE : ATTACKRESULT_NOT_POSSIBLE;
  if (result == ATTACKRESULT_NOT_POSSIBLE) {
    return ATTACKRESULT_NOT_POSSIBLE;
  }

  if (victim) {
    result = obj->getAbleToAttackSpecificObject(ATTACK_NEW_TARGET_FORCED,
                                                victim, CMD_FROM_PLAYER);

    // Special case -- objects with spawn weapons have to do different checks.
    // Stinger site with stinger soldiers is the catalyst example.
    if (result != ATTACKRESULT_POSSIBLE &&
        result != ATTACKRESULT_POSSIBLE_AFTER_MOVING &&
        obj->isKindOf(KINDOF_SPAWNS_ARE_THE_WEAPONS)) {
      SpawnBehaviorInterface *spawnInterface = obj->getSpawnBehaviorInterface();
      if (spawnInterface) {
        // We found the spawn interface, now get the closest slave to the
        // target.
        Object *slave = spawnInterface->getClosestSlave(victim->getPosition());
        if (slave) {
          result = slave->getAbleToAttackSpecificObject(
              ATTACK_NEW_TARGET_FORCED, victim, CMD_FROM_PLAYER);
        }
      }
    }
    return result;
  } else {
    // Almost every combat unit can force attack a position. The exceptions
    // include stationary units that try to force attack in a location beyond
    // their reach (range, LOS, etc).
    if (pos) {
      Object *testObj = obj;
      if (obj->isKindOf(KINDOF_IMMOBILE) ||
          obj->isKindOf(KINDOF_SPAWNS_ARE_THE_WEAPONS)) {
        SpawnBehaviorInterface *spawnInterface =
            obj->getSpawnBehaviorInterface();
        if (spawnInterface) {
          // We found the spawn interface, now get the closest slave to the
          // target.
      ...
      // Now evaluate the testObj to see if it is capable of force attacking the
      // pos.
      result = testObj->getAbleToUseWeaponAgainstTarget(
          ATTACK_NEW_TARGET, victim, pos, CMD_FROM_PLAYER);
      return result;
    }
  }
  return ATTACKRESULT_NOT_POSSIBLE;
...
//-------------------------------------------------------------------------------------------------
static CanAttackResult canAnyForceAttack(const DrawableList *allSelected,
                                         const Object *victim,
                                         const Coord3D *pos) {
  // check to make sure that allSelected can attack obj.
  for (DrawableListCIt cit = allSelected->begin(); cit != allSelected->end();
       ++cit) {
    Drawable *draw = *cit;
    if (!draw) {
      continue;
...
void pickAndPlayUnitVoiceResponse(const DrawableList *list,
  ...
  // when we have a null event -- but if any of the units can crush the
  // specified target, then it changes to VoiceCrush.
  for (DrawableListCIt it = list->begin(); it != list->end(); ++it) {
    Object *obj = (*it)->getObject();

    if (obj->isKindOf(KINDOF_IGNORED_IN_GUI)) continue;

    // Use the object instead of the drawable to get the thing template from.
    // This way, we get the sounds even if the thing is disguised as something
    // else. (Ala bomb truck.)
    ...
    switch (msgType) {
      ...
      case GameMessage::MSG_DO_SALVAGE: {
        ...
        if (ai) {
          ...
          // Default to voice move (it'll be selected if we can't find a crush
          // case as we iterate through the rest of the drawables)
          if (!soundToPlayPtr) {
          ...
          if (TheInGameUI->isInForceMoveToMode() && target) {
            if (obj->canCrushOrSquish(target)) {
              // Change it to voice crush because we are intentionally trying to
              // crush this!
          ...
          if (msgType == GameMessage::MSG_DO_SALVAGE) {
            const AudioEventRTS *tempSound =
                templ->getPerUnitSound("VoiceSalvage");
        ...
        if (info && info->m_weaponSlot) {
          switch (*info->m_weaponSlot) {
            ...
            case SECONDARY_WEAPON:
              soundToPlayPtr =
                  templ->getPerUnitSound("VoiceSecondaryWeaponMode");
              break;
            case TERTIARY_WEAPON:
              soundToPlayPtr =
                  templ->getPerUnitSound("VoiceTertiaryWeaponMode");
              break;
          }
          objectWithSound = obj;
          skip = true;
      ...
      case GameMessage::MSG_DO_WEAPON_AT_OBJECT: {
        ...
        if (weapon) {
          switch (weapon->getDamageType()) {
              // this stays, even if ALLOW_SURRENDER is not defed, since
              // flashbangs still use 'em
            case DAMAGE_SURRENDER:
              if (target && target->isKindOf(KINDOF_STRUCTURE)) {
                // We are attempting to take over a building with rangers and
                // flashbangs!
        ...
        if (weapon) {
          switch (weapon->getDamageType()) {
            ...
            case DAMAGE_DISARM:
              ...
              objectWithSound = obj;
              break;
            // these are specific to the guicommand based ground attacks, the
            // toxin sprinkler and the firestorm wall thing hence the additional
            // check for it being a non-primary weapon
            case DAMAGE_FLAME:
      ...
      case GameMessage::MSG_DO_SPECIAL_POWER_AT_OBJECT: {
        if (info && info->m_specialPowerType != SPECIAL_INVALID) {
          SpecialPowerModuleInterface *spmInterface =
              obj->findSpecialPowerModuleInterface(info->m_specialPowerType);
          if (spmInterface) {
            objectWithSound = obj;
      ...
      default:
        DEBUG_LOG(
            ("Requested to add voice of message type %d, but don't know how - "
             "jkmcd\n",
             msgType));
        break;
    }
    if (skip) {
      // The unit sound doesn't have any sort of priority or special case, so
      // simply play the first one that comes along.
      break;
    }

  }  // next drawable in list

  if (!soundToPlayPtr) return;
  ...
  if (objectWithSound) {
    ...
    // This seems really hacky, and MarkL admits that it is. However, we do this
    // so that we can "randomly" pick a different sound the next time, if we
    // have 3 or more sounds. - jkmcd
    ((AudioEventRTS *)soundToPlayPtr)
        ->setPlayingAudioIndex(soundToPlay.getPlayingAudioIndex());
...
void findCommandCenterOrMostExpensiveBuilding(Object *obj, void *vccl) {
  ...
  if (obj->isKindOf(KINDOF_COMMANDCENTER)) {
    ...
    Int costToBuild =
        obj->getTemplate()->calcCostToBuild(obj->getControllingPlayer());
...
//------------------------------------------------------------------------------------
/**
 * Create DO_MOVE_TO messages for each selected object, instructing it to move
 * to the given location.
 */
GameMessage::Type CommandTranslator::issueMoveToLocationCommand(
    const Coord3D *pos, Drawable *drawableInWay,
  ...
  if (TheStatsCollector) TheStatsCollector->incrementMoveCount();
...
GameMessage::Type CommandTranslator::createAttackMessage(
    Drawable *draw, Drawable *other, CommandEvaluateType commandType) {
  GameMessage::Type msgType = GameMessage::MSG_INVALID;

  // the drawable must have an object to be able to attack
  if (draw->getObject() == NULL) return msgType;

  // the target must have an object to be attacked
  if (other->getObject() == NULL) return msgType;
  ...
  if (commandType == DO_COMMAND) {
    ...
    attackmsg->appendObjectIDArgument(
        other->getObject()->getID());  // must pass object IDs to logic
...
//------------------------------------------------------------------------------------
/**
 * Create DO_ATTACK_GROUND_OBJECT messages for each selected object, instructing
...
GameMessage::Type CommandTranslator::issueAttackCommand(
    Drawable *target, CommandEvaluateType commandType, GUICommandType command) {
  GameMessage::Type msgType = GameMessage::MSG_INVALID;

  if (target == NULL) return msgType;
  ...
  if (m_teamExists) {
    // DEBUG_LOG(("issuing team-attack cmd against
    ...
    switch (command) {
      ...
      default:
        DEBUG_ASSERTCRASH(0,
                          ("issueAttackCommand was passed in a GUICommandType "
                           "type that isn't supported yet..."));
        return msgType;
    }
    ...
    if (commandType == DO_COMMAND) {
      ...
      attackMsg->appendObjectIDArgument(
          targetObj->getID());  // must pass target object ID to logic

      // if we have a stats collector, inrement the stats
      if (TheStatsCollector) TheStatsCollector->incrementAttackCount();
    }  // end if
  } else {
...
//-------------------------------------------------------------------------------------------------
GameMessage::Type CommandTranslator::issueSpecialPowerCommand(
  ...
  Drawable *sourceDraw = ignoreSelObj ? ignoreSelObj->getDrawable()
  ...
  if (BitTest(command->getOptions(), COMMAND_OPTION_NEED_OBJECT_TARGET)) {
    ...
    if (commandType == DO_COMMAND) {
      ...
      // say   something like " I think I'll put some dynamite on that there
      // tank."
      PickAndPlayInfo info;
      info.m_drawTarget = target;
      info.m_specialPowerType =
    ...
    if (commandType == DO_COMMAND) {
      ...
      ObjectID targetID = (target && target->getObject())
                              ? target->getObject()->getID()
                              : INVALID_ID;
      msg->appendObjectIDArgument(targetID);
      msg->appendIntegerArgument(command->getOptions());
      msg->appendObjectIDArgument(specificSource);

      // say   something like " I think I'll put a timed charge on the ground,
      // here."
      PickAndPlayInfo info;
      info.m_drawTarget = target;
      info.m_specialPowerType =
    ...
    if (commandType == DO_COMMAND) {
      ...
      // say   something like " I think I'll set down my laptop and hack some
      // cash from a bank in the Cayman Islands."
      PickAndPlayInfo info;
      info.m_drawTarget = target;
      info.m_specialPowerType =
          command->getSpecialPowerTemplate()->getSpecialPowerType();
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
GameMessage::Type CommandTranslator::issueCombatDropCommand(
    const CommandButton *command, CommandEvaluateType commandType,
  ...
  if (target != NULL &&
    ...
    // OBJECT BASED SPECIAL
    if (!command->isValidObjectTarget(TheInGameUI->getFirstSelectedDrawable(),
                                      target))
      return GameMessage::MSG_INVALID;
...
GameMessage::Type CommandTranslator::issueFireWeaponCommand(
  ...
  if (BitTest(command->getOptions(), COMMAND_OPTION_NEED_OBJECT_TARGET)) {
    ...
    if (!target || !target->getObject()) return msgType;
    ...
    if (BitTest(command->getOptions(), ATTACK_OBJECTS_POSITION)) {
      // Actually, you know what.... we want to attack the object's location
      // instead.
    ...
    if (commandType == DO_COMMAND) {
      GameMessage *msg;
      msg = TheMessageStream->appendMessage(msgType);
      DEBUG_ASSERTCRASH((command->getSpecialPowerTemplate()),
                        ("No Special Power Weapon here to 'do' with! ML"));
      msg->appendIntegerArgument(command->getSpecialPowerTemplate()->getID());
    }
  }
...
//-------------------------------------------------------------------------------------------------
GameMessage::Type CommandTranslator::createEnterMessage(
    Drawable *enter, CommandEvaluateType commandType) {
  GameMessage::Type msgType = GameMessage::MSG_ENTER;

  // if we're just evaluating then get out of here without actually doing the
  // action
  if (commandType == EVALUATE_ONLY) return msgType;

  if (!enter || !enter->getObject()) return msgType;

  // sanity
  DEBUG_ASSERTCRASH(commandType == DO_COMMAND,
                    ("createEnterMessage - commandType is not DO_COMMAND\n"));
  ...
  if (m_teamExists) {
    ...
    enterMsg->appendObjectIDArgument(
        INVALID_ID);  // 0 means current "selection team" of this player
...
//-------------------------------------------------------------------------------------------------
GameMessage::Type CommandTranslator::evaluateForceAttack(
    Drawable *draw, const Coord3D *pos, CommandEvaluateType type) {
  // evaluateForceAttack is used to determine whether or not we can force attack
  // the given target, and if we can, to issue the appropriate command.
...
// ------------------------------------------------------------------------------------------------
/** This method and the order of operations in the check here, determine what
 * command would actually happen (if type parameter == DO_COMMAND) if the user
 * clicked on the drawable 'draw'.  If type == DO_HINT, then the user hasn't
 * actually clicked, but has moused over the drawable 'draw' and we want to
 * generate a hint message as to what the actual command would be if clicked
...
GameMessage::Type CommandTranslator::evaluateContextCommand(
  ...
  // This piece of code is used to prevent interaction with unselectable objects
  // or masked objects. When we call this function, we typically pass in both a
  // position and a drawable (if applicable), so if the drawable is invalid...
  // then convert it to a position to be evaluated instead. Added: shrubberies
  // are the exception for interactions... Removed: GS Took out
  // ObjectStatusUnselectable, since that status only prevents selection, not
  // everything
  if (obj == NULL || (BitTest(obj->getStatusBits(), OBJECT_STATUS_MASKED) &&
                      !obj->isKindOf(KINDOF_SHRUBBERY) &&
                      !obj->isKindOf(KINDOF_FORCEATTACKABLE))) {
    // Nulling out the draw and obj pointer will force the remainder of this
    // code to evaluate a position interaction.
    draw = NULL;
    obj = NULL;
  }  //  end if

  // If the thing is a mine, and is locally controlled, then we should issue a
  // moveto to its location.
  ...
  if (TheInGameUI->isInForceMoveToMode()) {
    ...
    // setting the drawableInWay to draw will allow us to force attack in the
    // issue move command if there is a location to which we should attack.
    drawableInWay = draw;
  }

  GameMessage::Type msgType = GameMessage::MSG_INVALID;

  // Then we should determine if the game currently prefers selection events. If
  // it does, then return the invalid message.
  if (obj) {
  ...
  // Kris: Now that we can select non-controllable units/structures, don't allow
  ...
  if (TheInGameUI->areSelectedObjectsControllable() ||
    ...
    if (command && (command->isContextCommand() ||
                    ...
                    command->getCommandType() ==
                        GUI_COMMAND_SPECIAL_POWER_FROM_COMMAND_CENTER)) {
      if (obj && obj->isKindOf(KINDOF_SHRUBBERY) &&
          !BitTest(command->getOptions(), ALLOW_SHRUBBERY_TARGET)) {
        // If our object is a shrubbery, and we don't allow targetting it...
        // then null it out. Nulling out the draw and obj pointer will force the
        // remainder of this code to evaluate a position interaction.
        draw = NULL;
        obj = NULL;
      }

      if (obj && obj->isKindOf(KINDOF_MINE) &&
          !BitTest(command->getOptions(), ALLOW_MINE_TARGET)) {
        // If our object is a mine, and we don't allow targetting it... then
        // null it out. Nulling out the draw and obj pointer will force the
        // remainder of this code to evaluate a position interaction.
        draw = NULL;
        obj = NULL;
      }
      ...
      // Added relationship tests to make sure we're not attempting a
      // context-command on a restricted relationship. This case prevents rebels
      // from using tranq darts on allies.
      if (obj &&
          BitTest(command->getOptions(), COMMAND_OPTION_NEED_OBJECT_TARGET)) {
        Relationship relationship =
            ThePlayerList->getLocalPlayer()->getRelationship(obj->getTeam());
        switch (relationship) {
          case ALLIES:
      ...
      switch (command->getCommandType()) {
        // Kris: June 06, 2002
        // This is a GUI command button that triggers a mode. In any of these
        // modes, only one specific action can occur. If the mouse isn't over a
        // valid target, then the conditions aren't met and the code will cause
        // an invalid version of the cursor to be shown -- and should the user
        // click, the action won't take place.
        case GUICOMMANDMODE_CONVERT_TO_CARBOMB:
          currentlyValid = TheInGameUI->canSelectedObjectsDoAction(
              InGameUI::ACTIONTYPE_CONVERT_OBJECT_TO_CARBOMB, obj,
              InGameUI::SELECTION_ANY);
          break;
        case GUICOMMANDMODE_HIJACK_VEHICLE:
          currentlyValid = TheInGameUI->canSelectedObjectsDoAction(
              InGameUI::ACTIONTYPE_HIJACK_VEHICLE, obj,
              InGameUI::SELECTION_ANY);
          break;
#ifdef ALLOW_SURRENDER
        case GUICOMMANDMODE_PICK_UP_PRISONER:
          currentlyValid = TheInGameUI->canSelectedObjectsDoAction(
              InGameUI::ACTIONTYPE_PICK_UP_PRISONER, obj,
              InGameUI::SELECTION_ANY);
          break;
#endif
        case GUI_COMMAND_SPECIAL_POWER_FROM_COMMAND_CENTER: {
          ...
          if (cmdCenter)
            currentlyValid = TheInGameUI->canSelectedObjectsDoSpecialPower(
                command, obj, pos, InGameUI::SELECTION_ANY,
                command->getOptions(), cmdCenter);
          else
        ...
        case GUI_COMMAND_SPECIAL_POWER:
          currentlyValid = TheInGameUI->canSelectedObjectsDoSpecialPower(
              command, obj, pos, InGameUI::SELECTION_ANY, command->getOptions(),
              NULL);
          break;
        case GUI_COMMAND_FIRE_WEAPON:
          currentlyValid = TheInGameUI->canSelectedObjectsEffectivelyUseWeapon(
              command, obj, pos, InGameUI::SELECTION_ANY);
          break;
        case GUI_COMMAND_COMBATDROP:
          currentlyValid = !obj ? TRUE
                                : TheInGameUI->canSelectedObjectsDoAction(
                                      InGameUI::ACTIONTYPE_COMBATDROP_INTO, obj,
                                      InGameUI::SELECTION_ANY);
          break;
      }
          ...
          switch (command->getCommandType()) {
            ...
            case GUI_COMMAND_SPECIAL_POWER_FROM_COMMAND_CENTER: {
              Object *cmdCenter =
                  ThePlayerList->getLocalPlayer()->findNaturalCommandCenter();
              if (cmdCenter)
                msgType = issueSpecialPowerCommand(command, type, draw, pos,
                                                   cmdCenter);
              break;
            }
            case GUI_COMMAND_SPECIAL_POWER:  // lorenzen
              msgType =
                  issueSpecialPowerCommand(command, type, draw, pos, NULL);
              break;
            case GUI_COMMAND_FIRE_WEAPON:
    ...
    // ********************************************************************************************
    else if (TheInGameUI->canSelectedObjectsOverrideSpecialPowerDestination(
                 pos, InGameUI::SELECTION_ANY, SPECIAL_INVALID)) {
      if (type == DO_COMMAND || type == EVALUATE_ONLY) {
        // do the command
    ...
    // ********************************************************************************************
             ...
             TheInGameUI->canSelectedObjectsDoAction(
                 InGameUI::ACTIONTYPE_RESUME_CONSTRUCTION, obj,
                 InGameUI::SELECTION_ANY)) {
      if (type == DO_COMMAND || type == EVALUATE_ONLY) {
        // do the command
    ...
    // ********************************************************************************************
             ...
             TheInGameUI->canSelectedObjectsDoAction(
                 InGameUI::ACTIONTYPE_DOCK_AT, obj, InGameUI::SELECTION_ALL)) {
      //
      // The actual logic is simply to AIUpdate::dock with the target, the hint
      // is the only part that needs to be more specific.
      //
      if (type == DO_COMMAND || type == EVALUATE_ONLY) {
    ...
    // ********************************************************************************************
             ...
             TheInGameUI->canSelectedObjectsDoAction(
                 InGameUI::ACTIONTYPE_REPAIR_OBJECT, obj,
                 InGameUI::SELECTION_ANY)) {
      if (type == DO_COMMAND || type == EVALUATE_ONLY) {
        // do the command
    ...
    // ********************************************************************************************
             ...
             TheInGameUI->canSelectedObjectsDoAction(
                 InGameUI::ACTIONTYPE_GET_REPAIRED_AT, obj,
                 InGameUI::SELECTION_ANY)) {
      if (type == DO_COMMAND || type == EVALUATE_ONLY) {
        // do the command
    ...
    // ********************************************************************************************
             ...
             TheInGameUI->canSelectedObjectsDoAction(
                 InGameUI::ACTIONTYPE_GET_HEALED_AT, obj,
                 InGameUI::SELECTION_ANY)) {
      if (type == DO_COMMAND || type == EVALUATE_ONLY) {
        // do the command
    ...
    // ********************************************************************************************
             ...
             TheInGameUI->canSelectedObjectsDoAction(
                 InGameUI::ACTIONTYPE_HIJACK_VEHICLE, draw->getObject(),
                 InGameUI::SELECTION_ANY)) {
      if (type == DO_COMMAND || type == EVALUATE_ONLY) {
        // Now, this just tricks the AI  into making the hijacker run towards
        // the target vehicle
        // I must add a test to keep him from actually entering an enemy vehicle
        // (contained)... Lorenzen
        msgType = createEnterMessage(draw, type);
    ...
    // ********************************************************************************************
             ...
             TheInGameUI->canSelectedObjectsDoAction(
                 InGameUI::ACTIONTYPE_ENTER_OBJECT, obj,
                 InGameUI::SELECTION_ANY, true)) {
      if (type == DO_COMMAND || type == EVALUATE_ONLY) {
        // issue the command
        msgType = createEnterMessage(draw, type);
    ...
    // ********************************************************************************************
    else if (draw && (result = TheInGameUI->getCanSelectedObjectsAttack(
                          InGameUI::ACTIONTYPE_ATTACK_OBJECT, obj,
                          InGameUI::SELECTION_ANY,
    ...
    // ********************************************************************************************
    else if (draw && TheInGameUI->canSelectedObjectsDoAction(
                         InGameUI::ACTIONTYPE_CAPTURE_BUILDING, obj,
                         InGameUI::SELECTION_ANY)) {
      //@TODO: Kris
      ...
      const CommandSet *set =
          TheControlBar->findCommandSet(source->getCommandSetString());
      if (set) {
    ...
    // ********************************************************************************************
                         ...
                         InGameUI::ACTIONTYPE_DISABLE_VEHICLE_VIA_HACKING, obj,
                         InGameUI::SELECTION_ANY)) {
      if (type == DO_COMMAND || type == EVALUATE_ONLY) {
        //@TODO: Kris
        // PRELIMINARY CODE FOR HOOKING IN AUTO SPECIALS --- WILL BE REDONE!
    ...
    // ********************************************************************************************
                         ...
                         InGameUI::ACTIONTYPE_STEAL_CASH_VIA_HACKING, obj,
                         InGameUI::SELECTION_ANY)) {
      if (type == DO_COMMAND || type == EVALUATE_ONLY) {
        //@TODO: Kris
        // PRELIMINARY CODE FOR HOOKING IN AUTO SPECIALS --- WILL BE REDONE!
    ...
    // ********************************************************************************************
                         ...
                         InGameUI::ACTIONTYPE_DISABLE_BUILDING_VIA_HACKING, obj,
                         InGameUI::SELECTION_ANY)) {
      if (type == DO_COMMAND || type == EVALUATE_ONLY) {
        //@TODO: Kris
        // PRELIMINARY CODE FOR HOOKING IN AUTO SPECIALS --- WILL BE REDONE!
    ...
    // ********************************************************************************************
      ...
      if (type == DO_COMMAND || type == EVALUATE_ONLY) {
        // issue the command
        msgType =
            issueAttackCommand(draw, type, GUICOMMANDMODE_PICK_UP_PRISONER);
    ...
    // ********************************************************************************************
             ...
             TheInGameUI->canSelectedObjectsDoAction(
                 InGameUI::ACTIONTYPE_SET_RALLY_POINT, NULL,
                 InGameUI::SELECTION_ALL, FALSE)) {
      msgType = GameMessage::MSG_SET_RALLY_POINT;

      if (type == DO_COMMAND) {
        const DrawableList *allSelectedDrawables =
            TheInGameUI->getAllSelectedDrawables();

        for (DrawableList::const_iterator it = allSelectedDrawables->begin();
             it != allSelectedDrawables->end(); ++it) {
          Drawable *draw = (*it);
    ...
    // ********************************************************************************************
      ...
      // no "interesting" command to do with the picked drawable ... which is
      // determined by what we return in this function by default
      //

      // Before we issue a move order or hint, check to see if we can even move
      // there!
      Bool validQuickPath = FALSE;
      // Make sure to only to the check if the shroud is CLEARED.  If it is
      // fogged or shrouded, SKIP THE CHECK.  jba [3/11/2003]
      if (ThePartitionManager->getShroudStatusForPlayer(
              ThePlayerList->getLocalPlayer()->getPlayerIndex(), pos) !=
          CELLSHROUD_CLEAR) {
      ...
      if (type == DO_COMMAND || type == EVALUATE_ONLY) {
        // issue command
        // Note: If draw is valid, then its one of ours and we don't have
        // something more specific to do. Therefore, lets not issue a move
        // command, and instead we'll return that there wasn't a command for us
        // to perform.
        ...
        if (!validQuickPath) {
          ...
          // THIS CODE WILL NEVER EVER GET CALLED! -- it's a context command now
          // (READ: rip code out) Attack move
 ...
 */
GameMessageDisposition CommandTranslator::translateGameMessage(
    const GameMessage *msg) {
  GameMessage::Type t = msg->getType();
  GameMessageDisposition disp = KEEP_MESSAGE;
  // We want to always be able to get to the options menu even during no input
  // times and a clear game data message should always go through
  if (t != GameMessage::MSG_META_OPTIONS &&
      t != GameMessage::MSG_CLEAR_GAME_DATA &&
  ...
  switch (t) {
    ...
    case GameMessage::MSG_META_SELECT_NEXT_UNIT: {
      ...
      if (TheGameClient->firstDrawable() == NULL) break;
      ...
      if (TheInGameUI->getSelectCount() == 0) {
        ...
        for (temp = TheGameClient->firstDrawable();
             temp->getNextDrawable() != NULL; temp = temp->getNextDrawable()) {
        }
        ...
        for (temp; temp != NULL; temp = temp->getPrevDrawable()) {
          ...
          if (!object) {
            break;
          } else if (object && object->isMobile() &&
                     object->isLocallyControlled() && !object->isContained() &&
                     !object->isKindOf(KINDOF_DRONE)) {
            // create a new group.
            GameMessage *teamMsg = TheMessageStream->appendMessage(
                GameMessage::MSG_CREATE_SELECTED_GROUP);

            // New group or add to group? Passed in value is true if we are
        ...
        if (selectedObject->isLocallyControlled()) {
          ...
          for (; temp != selectedDrawable; temp = temp->getPrevDrawable()) {
            ...
            // if temp is null, set it to the last drawable and loop back to
            // selected drawable
            if (temp == NULL) {
              for (temp = selectedDrawable; temp->getNextDrawable() != NULL;
                   temp = temp->getNextDrawable()) {
              }
              hack = TRUE;
            }
            // else search for a previous selectable drawable
              ...
              if (tempObject && tempObject->isMobile() &&
                  tempObject->isLocallyControlled() &&
                  !tempObject->isContained() &&
                  !tempObject->isKindOf(KINDOF_DRONE)) {
                newDrawable = temp;
                break;
      ...
      if (TheInGameUI->getSelectCount() == 0) {
        ...
        if (selectedObject->isLocallyControlled()) {
          ...
          for (temp; temp != selectedDrawable; temp = temp->getNextDrawable()) {
            ...
            // if temp is null, set it to the first drawable and loop forward to
            // selected drawable
            if (temp == NULL) {
              temp = TheGameClient->firstDrawable();
              hack = TRUE;
              const Object *tempObject = temp->getObject();
              // must take case of this case here or else the loop will break
              // without getting newDrawable
              if (tempObject && temp->getNextDrawable() == selectedDrawable &&
                  !temp->isSelected() && tempObject->isMobile() &&
                  tempObject->isLocallyControlled() &&
                  !tempObject->isContained() &&
                  !tempObject->isKindOf(KINDOF_DRONE)) {
                newDrawable = temp;
    ...
    case GameMessage::MSG_META_SELECT_NEXT_WORKER: {
      ...
      if (TheInGameUI->getSelectCount() == 0) {
        ...
        for (temp; temp != NULL; temp = temp->getPrevDrawable()) {
          ...
          else if (object && object->isLocallyControlled() &&
        ...
        if (selectedObject->isLocallyControlled()) {
          ...
          for (; temp != selectedDrawable; temp = temp->getPrevDrawable()) {
            ...
            // else search for a previous selectable drawable
              ...
              const Object *tempObject = temp->getObject();
              if (tempObject && tempObject->isLocallyControlled() &&
                  !tempObject->isContained() &&
                  tempObject->isKindOf(KINDOF_DOZER)) {
                newDrawable = temp;
                break;
                // temp = selectedDrawable; // same as break
              }
    ...
    case GameMessage::MSG_META_SELECT_HERO: {
      ...
      if (hero->isContained()) hero = hero->getContainedBy();

      Drawable *heroDraw = hero->getDrawable();

      if (heroDraw == NULL) break;
    ...
    case GameMessage::MSG_META_VIEW_LAST_RADAR_EVENT: {
      ...
      //			if( TheRadar->isRadarForced() || (
      //TheRadar->isRadarHidden() == false && player->hasRadar() ) )
      {
        Coord3D lastEvent;
    ...
    case GameMessage::MSG_META_SELECT_ALL: {
      ...
      while (draw) {
        ...
        // Only select the object if it is locally controlled and not contained
        // by anything.
        ...
        if (object && object->isMobile() && object->isLocallyControlled() &&
            !object->isContained() &&
            !object->isAnyKindOf(disqualifyingKindofs) &&
            !object->isEffectivelyDead() && object->isMassSelectable()) {
    ...
    case GameMessage::MSG_META_DEPLOY:
      ...
      DEBUG_ASSERTCRASH(FALSE,
                        ("unimplemented meta command MSG_META_DEPLOY !"));
#endif
...
#ifdef _DEBUG
      DEBUG_ASSERTCRASH(FALSE,
                        ("unimplemented meta command MSG_META_FOLLOW !"));
#endif
      /// @todo srj implement me
      disp = DESTROY_MESSAGE;
      break;
    ...
    //-----------------------------------------------------------------------------------------
    /* MDC - no such thing as chat to players right now - not until we have a
    diplomacy screen case GameMessage::MSG_META_CHAT_PLAYERS: if
    ...
    */
    ...
    case GameMessage::MSG_META_DIPLOMACY:
      ...
      } else if (TheShell && TheShell->isShellActive() &&
...
#endif
      {
        ...
        const ThingTemplate *thing =
            TheThingFactory->findTemplate(ThePlayerList->getLocalPlayer()
                                              ->getPlayerTemplate()
                                              ->getBeaconTemplate());
        ThePlayerList->getLocalPlayer()->countObjectsByThingTemplate(
            1, &thing, false, &count);
        DEBUG_LOG(
            ("MSG_META_PLACE_BEACON - Player already has %d beacons active\n",
             count));
        if (count < TheMultiplayerSettings->getMaxBeaconsPerPlayer()) {
          const CommandButton *commandButton =
              TheControlBar->findCommandButton("Command_PlaceBeacon");
    ...
    case GameMessage::MSG_META_TOGGLE_LOWER_DETAILS: {
      if (TheGlobalData) {
        ...
        if (isLowDetails) {
          ...
          oldBehindBuildingMarkers =
              TheGameLogic->getShowBehindBuildingMarkers();
          TheGameLogic->setShowBehindBuildingMarkers(FALSE);

          oldMaxParticleCount = TheGlobalData->m_maxParticleCount;
          TheWritableGlobalData->m_maxParticleCount =
              DROPPED_MAX_PARTICLE_COUNT;
    ...
    case GameMessage::MSG_META_BEGIN_CAMERA_ROTATE_LEFT:
      DEBUG_ASSERTCRASH(!TheInGameUI->isCameraRotatingLeft(),
                        ("Setting rotate camera left, but it's already set!"));
      TheInGameUI->setCameraRotateLeft(true);
      break;
    case GameMessage::MSG_META_END_CAMERA_ROTATE_LEFT:
      DEBUG_ASSERTCRASH(
          TheInGameUI->isCameraRotatingLeft(),
          ("Clearing rotate camera left, but it's already clear!"));
      TheInGameUI->setCameraRotateLeft(false);
      break;
    case GameMessage::MSG_META_BEGIN_CAMERA_ROTATE_RIGHT:
      DEBUG_ASSERTCRASH(!TheInGameUI->isCameraRotatingRight(),
                        ("Setting rotate camera right, but it's already set!"));
      TheInGameUI->setCameraRotateRight(true);
      break;
    case GameMessage::MSG_META_END_CAMERA_ROTATE_RIGHT:
      DEBUG_ASSERTCRASH(
          TheInGameUI->isCameraRotatingRight(),
          ("Clearing rotate camera right, but it's already clear!"));
      TheInGameUI->setCameraRotateRight(false);
      break;
    case GameMessage::MSG_META_BEGIN_CAMERA_ZOOM_IN:
      DEBUG_ASSERTCRASH(!TheInGameUI->isCameraZoomingIn(),
                        ("Setting zoom camera in, but it's already set!"));
      TheInGameUI->setCameraZoomIn(true);
      break;
    case GameMessage::MSG_META_END_CAMERA_ZOOM_IN:
      DEBUG_ASSERTCRASH(TheInGameUI->isCameraZoomingIn(),
                        ("Clearing zoom camera in, but it's already clear!"));
      TheInGameUI->setCameraZoomIn(false);
      break;
    case GameMessage::MSG_META_BEGIN_CAMERA_ZOOM_OUT:
      DEBUG_ASSERTCRASH(!TheInGameUI->isCameraZoomingOut(),
                        ("Setting zoom camera out, but it's already set!"));
      TheInGameUI->setCameraZoomOut(true);
      break;
    case GameMessage::MSG_META_END_CAMERA_ZOOM_OUT:
      DEBUG_ASSERTCRASH(TheInGameUI->isCameraZoomingOut(),
                        ("Clearing zoom camera out, but it's already clear!"));
    ...
    case GameMessage::MSG_META_BEGIN_FORCEMOVE:
      DEBUG_ASSERTCRASH(!TheInGameUI->isInForceMoveToMode(),
                        ("forceMoveToMode mismatch"));
    ...
    case GameMessage::MSG_META_BEGIN_WAYPOINTS:
      DEBUG_ASSERTCRASH(!TheInGameUI->isInWaypointMode(),
                        ("Setting m_waypointMode but it's already set!"));
    ...
    case GameMessage::MSG_META_END_WAYPOINTS:
      DEBUG_ASSERTCRASH(TheInGameUI->isInWaypointMode(),
                        ("Clearing m_waypointMode but it's already clear!"));
    ...
    case GameMessage::MSG_SELECT_TEAM9: {
      ...
      for (DrawableListIt it = listOfUnits.begin(); it != listOfUnits.end();
           /* empty */) {
        Drawable *draw = *it;
      ...
      if (!listOfUnits.empty()) {
        pickAndPlayUnitVoiceResponse(&listOfUnits,
                                     GameMessage::MSG_CREATE_SELECTED_GROUP);
      }
    ...
    // --------------------------------------------------------------------------------------------
    // An object is mouseover'd.  A hint of a command may be generated if
    ...
    case GameMessage::MSG_MOUSEOVER_DRAWABLE_HINT: {
      ...
      if (TheInGameUI->getSelectCount() > 0 ||
          (command &&
           command->getCommandType() ==
               GUI_COMMAND_SPECIAL_POWER_FROM_COMMAND_CENTER))  // If something
                                                                // is selected
      {
        /// @todo This as well as the one in GameMessage::MSG_DRAWABLE_PICKED
        /// below should possibly have a generalized CanAttack instead of simply
        /// checking isEnemyOf
        Drawable *draw =
            TheGameClient->findDrawableByID(msg->getArgument(0)->drawableID);
        if (draw) {
          //
          // given the drawable that we have moused over, pretend that we have
          // actually "picked" that drawable, depending on what we have
          // selected, our mode, and what "picked" is we would do a command ...
          // but here we only will generate the hint message, *not* the actual
          // command
          //
          ...
          // Do not eat this message, as it itself has another purpose in
          // HintSpy
        }
      }
    } break;
    ...
    //-----------------------------------------------------------------------------
    // Terrain is mouseover'd.  A hint of a command may be generated if
    // something is selected
    //
    case GameMessage::MSG_MOUSEOVER_LOCATION_HINT: {
      ...
      //
      // This message occurs whenever the mouse cursor moves off of a drawable,
      // in which case we want to turn off the pick hint.
      //
      if (TheInGameUI->isInForceAttackMode()) {
    ...
    case GameMessage::MSG_RAW_MOUSE_RIGHT_BUTTON_UP: {
      // register this event for determining if the click was fast or short
      // enough not to be a drag
    ...
    case GameMessage::MSG_MOUSE_RIGHT_CLICK: {
      ...
      if (TheGlobalData->m_useAlternateMouse &&
          TheMouse->isClick(&m_mouseRightDragAnchor, &m_mouseRightDragLift,
                            m_mouseRightDown, m_mouseRightUp)) {
        Bool isPoint = (msg->getArgument(0)->pixelRegion.height() == 0 &&
                        msg->getArgument(0)->pixelRegion.width() == 0);

        // NOTE: RIGHT_CLICK is not transmitted if AREA_SELECTION or
        // DRAWABLE_PICKED occurs. If we see this msg, no object was clicked on,
        // therefore clicked on ground. Issue move command to all currently
        // selected objects.

        // sanity
        if (TheTacticalView == NULL) break;
        ...
        if (isPoint && controllable) {
          UnsignedInt pickType =
              getPickTypesForContext(TheInGameUI->isInForceAttackMode());
          Drawable *draw = TheTacticalView->pickDrawable(
              &msg->getArgument(0)->pixelRegion.lo,
              TheInGameUI->isInForceAttackMode(), (PickType)pickType);
          if (TheInGameUI->isInForceAttackMode()) {
    ...
    case GameMessage::MSG_MOUSE_LEFT_CLICK: {
      ...
      // NOTE: LEFT_CLICK is not transmitted if AREA_SELECTION or
      // DRAWABLE_PICKED occurs. If we see this msg, no object was clicked on,
      // therefore clicked on ground. Issue move command to all currently
      // selected objects.
      ...
      const CommandButton *command = TheInGameUI->getGUICommand();
      // maintain this as the set of GUI button initiated commands that require
      // a left click action in alt mouse mode
      Bool isFiringGUICommand =
          (command &&
           (command->getCommandType() == GUI_COMMAND_SPECIAL_POWER ||
            command->getCommandType() ==
                GUI_COMMAND_SPECIAL_POWER_FROM_COMMAND_CENTER ||
      ...
      // in alternate mouse mode, this left click is only actioned here if we're
      // firing a gui command
    ...
    case GameMessage::MSG_META_DEMO_SWITCH_TEAMS: {
      if (TheGameLogic->isInGame()) {
        ...
        do {
          ++idx;
          if (idx >= ThePlayerList->getPlayerCount()) idx = 0;

          if (idx == idxOrig) {
            break;
          }

        } while (ThePlayerList->getNthPlayer(idx) ==
        ...
        TheControlBar->initSpecialPowershortcutBar(
            ThePlayerList->getLocalPlayer());
    ...
    case GameMessage::MSG_META_HELP: {
      // for demo, they don't want the help menu showing up.  I left this as a
      // block comment because we might want to add back in the help menu at
      // some point in time.
      /*
                              if (TheWindowManager && TheNameKeyGenerator)
                              {
                                      GameWindow *motd =
         TheWindowManager->winGetWindowFromId(NULL,
         (Int)TheNameKeyGenerator->nameToKey(AsciiString("MOTD.wnd:MOTD"))); if
         (motd) motd->winHide(!motd->winIsHidden());
                              }*/
    ...
    case GameMessage::MSG_META_DEMO_TOGGLE_MOTION_BLUR_ZOOM: {
      Int mode;
      if ((mode = TheTacticalView->getViewFilterType()) ==
          FT_VIEW_MOTION_BLUR_FILTER) {  // mode already set, turn it off
    ...
    //-----------------------------------------------------------------------------------------
    case GameMessage::
        MSG_META_DEMO_TOGGLE_BW_VIEW: {  // We're not testing BW mode anymore,
                                         // so use this message for toggling
                                         // wireframe mode.
      static mode = 0;
      if (mode == 0) {  // First turn on wireframe
      ...
      if (mode == 1) {
        if ((TheTacticalView->getViewFilterType()) ==
            FT_VIEW_CROSSFADE) {  // mode already set, turn it off
          TheTacticalView->setViewFilterMode(FM_NULL_MODE);
          TheTacticalView->setViewFilter(FT_NULL_FILTER);
    ...
    case GameMessage::MSG_META_DEMO_CYCLE_LOD_LEVEL: {
      ...
      if (level < DYNAMIC_GAME_LOD_LOW) level = DYNAMIC_GAME_LOD_VERY_HIGH;

      TheGameLODManager->setDynamicLODLevel((DynamicGameLODLevel)level);

      UnicodeString lodName;
      lodName.format(L"Dynamic Game Detail %hs",
                     TheGameLODManager->getDynamicGameLODLevelName(
                         (DynamicGameLODLevel)level));
    ...
    case GameMessage::MSG_META_DEMO_TOGGLE_SHADOW_VOLUMES: {
      TheWritableGlobalData->m_useShadowVolumes =
          !TheGlobalData->m_useShadowVolumes;
      TheWritableGlobalData->m_useShadowDecals =
    ...
    case GameMessage::MSG_META_DEMO_KILL_ALL_ENEMIES: {
      for (Drawable *d = TheGameClient->firstDrawable(); d;
        ...
        if (obj && obj->getControllingPlayer() &&
            obj->getControllingPlayer()->getRelationship(
                ThePlayerList->getLocalPlayer()->getDefaultTeam()) == ENEMIES) {
          obj->kill();
        }
      }
      disp = DESTROY_MESSAGE;
    ...
    case GameMessage::MSG_META_DEBUG_INCR_ANIM_SKATE_SPEED: {
      ...
      TheInGameUI->message(UnicodeString(L"Skate Distance Override is now %f"),
        ...
        for (DrawableListCIt it = list->begin(); it != list->end(); ++it) {
          ...
          if (pDraw) {
            ...
            if (pObject) {
              ...
              AsciiString msg;
              msg.format("Extent %s --> %s   %d %g",
                         oldGeometry.getDescriptiveString().str(),
                         newGeometry.getDescriptiveString().str(),
                         extentModType, extentModAmount);
    ...
    case GameMessage::MSG_META_DEMO_TOGGLE_TRACKMARKS: {
      TheWritableGlobalData->m_makeTrackMarks =
    ...
    case GameMessage::MSG_META_DEMO_KILL_AREA_SELECTION: {
      for (int i = 1; i < msg->getArgumentCount(); i++) {
        Object *obj =
            TheGameLogic->findObjectByID(msg->getArgument(i)->objectID);
        if (obj) {
          obj->kill();
        }
      }
    ...
    case GameMessage::MSG_META_DEMO_NEXT_OBJECTIVE_MOVIE: {
      if (TheGameLogic->isInGame()) {
        if (t == GameMessage::MSG_META_DEMO_NEXT_OBJECTIVE_MOVIE) {
          m_objective++;
          if (m_objective > 6) m_objective = 1;
        } else {
          m_objective =
              t - GameMessage::MSG_META_DEMO_PLAY_OBJECTIVE_MOVIE1 + 1;
        }
        AsciiString name;
    ...
    case GameMessage::MSG_META_DEMO_TOGGLE_MILITARY_SUBTITLES: {
      TheInGameUI->militarySubtitle("MSG:Testing",
                                    10000);  // use some innocuous string
      disp = DESTROY_MESSAGE;
      break;
    }
    ...
    case GameMessage::MSG_META_DEMO_TOGGLE_AI_DEBUG: {
      TheWritableGlobalData->m_debugAI =
          (AIDebugOptions)((Int)TheGlobalData->m_debugAI + 1);
      ...
      TheInGameUI->message(UnicodeString(L"Debug AI Mode is %s"),
                           TheGlobalData->m_debugAI ? line.str() : L"OFF");
      disp = DESTROY_MESSAGE;
      break;
    }
    ...
    case GameMessage::MSG_META_DEMO_GIVE_SCIENCEPURCHASEPOINTS: {
      ...
      if (player) player->addSciencePurchasePoints(1);
    ...
    case GameMessage::MSG_META_DEMO_GIVE_ALL_SCIENCES: {
      Player *player = ThePlayerList->getLocalPlayer();
      if (player) {
        // cheese festival: do NOT imitate this code. it is for debug purposes
        // only.
        ...
        for (int i = 0; i < v.size(); ++i) {
          ...
          if (st != SCIENCE_INVALID &&
              TheScienceStore->isScienceGrantable(st)) {
    ...
    case GameMessage::MSG_META_DEMO_GIVE_RANKLEVEL: {
      Player *player = ThePlayerList->getLocalPlayer();
      if (player) player->setRankLevel(player->getRankLevel() + 1);
    ...
    case GameMessage::MSG_META_DEMO_TOGGLE_CAMERA_DEBUG: {
      ...
      TheInGameUI->message(UnicodeString(L"Debug Camera Mode is %s"),
                           TheGlobalData->m_debugCamera ? L"On" : L"OFF");
      disp = DESTROY_MESSAGE;
      break;
    }
    ...
    case GameMessage::MSG_META_DEBUG_DUMP_ALL_PLAYER_OBJECTS: {
      ...
      for (Int i = 0; i < MAX_PLAYER_COUNT; ++i) {
        ...
        if (!pPlayer || !pPlayer->getPlayerTemplate() ||
            !pPlayer->getPlayerTemplate()->isPlayableSide())
          continue;

        Int numObjects = 0;
        pPlayer->iterateObjects(countObjects, &numObjects);
        line.format("Player %d (%ls) has %d non-dead objects", i,
                    pPlayer->getPlayerDisplayName().str(), numObjects);
        TheScriptEngine->AppendDebugMessage(line, FALSE);

        if (numObjects &&
            (numObjects <= 5 ||
             t == GameMessage::MSG_META_DEBUG_DUMP_ALL_PLAYER_OBJECTS)) {
          line = "Objects are:";
          TheScriptEngine->AppendDebugMessage(line, FALSE);
    ...
    case GameMessage::MSG_META_DEMO_TOGGLE_VISIONDEBUG: {
      TheWritableGlobalData->m_debugVisibility =
          !TheGlobalData->m_debugVisibility;
      TheInGameUI->message(UnicodeString(L"Debug Vision Mode is %s"),
                           TheGlobalData->m_debugVisibility ? L"On" : L"OFF");
      disp = DESTROY_MESSAGE;
      break;
    }
    ...
    case GameMessage::MSG_META_DEMO_TOGGLE_PROJECTILEDEBUG: {
      TheWritableGlobalData->m_debugProjectilePath =
          !TheGlobalData->m_debugProjectilePath;
      TheInGameUI->message(
          UnicodeString(L"Debug Projectile Path Mode is %s"),
          TheGlobalData->m_debugProjectilePath ? L"On" : L"OFF");
      disp = DESTROY_MESSAGE;
      break;
    }
    ...
    case GameMessage::MSG_META_DEMO_TOGGLE_THREATDEBUG: {
      TheWritableGlobalData->m_debugThreatMap =
          !TheGlobalData->m_debugThreatMap;
      if (TheGlobalData->m_debugThreatMap) {
        TheWritableGlobalData->m_debugCashValueMap = false;
      }
      TheInGameUI->message(UnicodeString(L"Debug Threat Map is %s"),
                           TheGlobalData->m_debugThreatMap ? L"On" : L"OFF");
      disp = DESTROY_MESSAGE;
      break;
    }
    ...
    case GameMessage::MSG_META_DEMO_TOGGLE_CASHMAPDEBUG: {
      TheWritableGlobalData->m_debugCashValueMap =
          !TheGlobalData->m_debugCashValueMap;
      if (TheGlobalData->m_debugCashValueMap) {
        TheWritableGlobalData->m_debugThreatMap = false;
      }
      TheInGameUI->message(UnicodeString(L"Debug Cash Value Map is %s"),
                           TheGlobalData->m_debugCashValueMap ? L"On" : L"OFF");
      disp = DESTROY_MESSAGE;
      break;
    }
    ...
    case GameMessage::MSG_META_DEMO_TOGGLE_GRAPHICALFRAMERATEBAR: {
      TheWritableGlobalData->m_debugShowGraphicalFramerate =
          !TheGlobalData->m_debugShowGraphicalFramerate;
      disp = DESTROY_MESSAGE;
      break;
    }
    ...
    case GameMessage::MSG_META_DEBUG_SHOW_EXTENTS:
      TheWritableGlobalData->m_showCollisionExtents =
          1 - TheGlobalData->m_showCollisionExtents;
      TheInGameUI->message(
          UnicodeString(L"Show Object Extents %s"),
    ...
    case GameMessage::MSG_META_DEBUG_SHOW_HEALTH: {
      TheWritableGlobalData->m_showObjectHealth =
          1 - TheGlobalData->m_showObjectHealth;
    ...
    case GameMessage::MSG_META_DEMO_TOGGLE_SPECIAL_POWER_DELAYS: {
      if (TheGlobalData) {
        TheWritableGlobalData->m_specialPowerUsesDelay =
            1 - TheGlobalData->m_specialPowerUsesDelay;
        TheInGameUI->message(
            UnicodeString(L"Special Power (Superweapon) Delay: %s"),
    ...
    case GameMessage::MSG_META_DEMO_TEST_SURRENDER: {
      GameMessage *msg =
          TheMessageStream->appendMessage(GameMessage::MSG_DO_SURRENDER);
      msg->appendObjectIDArgument(
          INVALID_ID);  // zero means "surrender to anyone"
    ...
    case GameMessage::MSG_META_DEMO_TOGGLE_AVI: {
      if (TheDisplay) TheDisplay->toggleMovieCapture();
      disp = DESTROY_MESSAGE;
      break;
    }
    ...
    case GameMessage::MSG_META_DEMO_PERFORM_STATISTICAL_DUMP:
      ...
      TheInGameUI->message(UnicodeString(L"Statistics dump made on frame: %d"),
