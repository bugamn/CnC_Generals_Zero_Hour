...
//-----------------------------------------------------------------------------------
//EvaSideSounds
static void parseSideSoundsList(INI *ini, void *instance, void *store,
                                const void *userData) {
  ...
  // This could be made more efficient, but to be honest, it shouldn't be that
  // slow.
...
const FieldParse EvaSideSounds::s_evaSideSounds[] = {
    ...
    {"Sounds", INI::parseSoundsList, NULL,
     offsetof(EvaSideSounds, m_soundNames)},
    {0, 0, 0, 0},
};
...
const FieldParse EvaCheckInfo::s_evaEventInfo[] = {
    {"Priority", INI::parseUnsignedInt, NULL,
     offsetof(EvaCheckInfo, m_priority)},
    {"TimeBetweenChecksMS", INI::parseDurationUnsignedInt, NULL,
     offsetof(EvaCheckInfo, m_framesBetweenChecks)},
    {"ExpirationTimeMS", INI::parseDurationUnsignedInt, NULL,
     offsetof(EvaCheckInfo, m_framesToExpire)},
    {"SideSounds", parseSideSoundsList, NULL,
     offsetof(EvaCheckInfo, m_evaSideSounds)},
    {0, 0, 0, 0},
...
const ShouldPlayFunc Eva::s_shouldPlayFuncs[] = {
    Eva::shouldPlayLowPower,       Eva::shouldPlayGenericHandler,
    Eva::shouldPlayGenericHandler, Eva::shouldPlayGenericHandler,
    Eva::shouldPlayGenericHandler, Eva::shouldPlayGenericHandler,
    Eva::shouldPlayGenericHandler, Eva::shouldPlayGenericHandler,
    Eva::shouldPlayGenericHandler, Eva::shouldPlayGenericHandler,
    Eva::shouldPlayGenericHandler, Eva::shouldPlayGenericHandler,
    ...
    Eva::shouldPlayGenericHandler, Eva::shouldPlayGenericHandler,
    Eva::shouldPlayGenericHandler, Eva::shouldPlayGenericHandler,
    Eva::shouldPlayGenericHandler, NULL,
};
...
void Eva::update() {
  ...
  // Don't update for the first few frames. This way, we don't have to deal with
  // our initial power being 0, etc.
  if (frame < 2) {
    return;
  }
  ...
  // Reset all of the flags that have been set to true that haven't actually
  // been probed, because they will need to trigger again to be valid messages.
...
EvaMessage Eva::nameToMessage(const AsciiString &name) {
  ...
  DEBUG_CRASH(
      ("Invalid requested Eva message translation :%s: jkmcd", name.str()));
  return EVA_COUNT;
}
...
EvaCheckInfo *Eva::newEvaCheckInfo(AsciiString name) {
  ...
  EvaCheckInfoPtrVecIt it;
  for (it = m_allCheckInfos.begin(); it != m_allCheckInfos.end(); ++it) {
    if (*it && (*it)->m_message == mesg) return NULL;
  }
...
const EvaCheckInfo *Eva::getEvaCheckInfo(AsciiString name) {
  ...
  EvaCheckInfoPtrVecIt it;
  for (it = m_allCheckInfos.begin(); it != m_allCheckInfos.end(); ++it) {
    if (*it && (*it)->m_message == mesg) return *it;
  }
...
Bool Eva::shouldPlayLowPower(Player *localPlayer) {
  ...
  //{
  //	if ( ! (localPlayer->canBuild(chinaReactorTemplate) ||
  //localPlayer->canBuild(americanReactorTemplate)) ) 		return FALSE
  //}
...
void Eva::playMessage(EvaMessage messageToTest, UnsignedInt currentFrame) {
  ...
  check.m_timeForNextCheck =
      currentFrame + check.m_evaInfo->m_framesBetweenChecks;
...
void Eva::processPlayingMessages(UnsignedInt currentFrame) {
  ...
  EvaCheckVecIt it;
  for (it = m_checks.begin(); it != m_checks.end(); /* empty */) {
    // These are requests that will be available next frame because they've
    // played.
    ...
    // These are requests that never got a chance to play and have since
    // expired.
    if (it->m_triggeredOnFrame + it->m_evaInfo->m_framesToExpire <=
            currentFrame &&
        !it->m_alreadyPlayed) {
      it = m_checks.erase(it);
      continue;
  ...
  // Okay. No one is currently playing anything, so lets find an event and
  // trigger it.
  ...
  for (Int i = 0; i < numSides; ++i) {
    if (side.compareNoCase(storedIt->m_evaInfo->m_evaSideSounds[i].m_side) ==
        0) {
      // Its this one.
      if (storedIt->m_evaInfo->m_evaSideSounds[i].m_soundNames.size() > 0) {
        Int soundToPlay = GameClientRandomValue(
            0, storedIt->m_evaInfo->m_evaSideSounds[i].m_soundNames.size() - 1);
        m_evaSpeech.setEventName(
            storedIt->m_evaInfo->m_evaSideSounds[i].m_soundNames[soundToPlay]);
      } else {
        // clear it.
  ...
  storedIt->m_timeForNextCheck =
      currentFrame + storedIt->m_evaInfo->m_framesBetweenChecks;

  // Now that we correctly filter messages, we need to set the player index for
  // who should hear the sound to the local player.
