...
GameMessageDisposition GameClientMessageDispatcher::translateGameMessage(
  ...
  if (msg->getType() == GameMessage::MSG_FRAME_TICK) return KEEP_MESSAGE;

  // DEBUG_LOG(("GameClientMessageDispatcher::translateGameMessage() - eating a
  // %s on frame %d\n",
  //((GameMessage *)msg)->getCommandAsAsciiString().str(),
