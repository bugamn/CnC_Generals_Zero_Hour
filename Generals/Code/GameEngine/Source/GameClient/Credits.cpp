...
const FieldParse CreditsManager::m_creditsFieldParseTable[] = {
    ...
    {"ScrollRateEveryFrames", INI::parseInt, NULL,
     offsetof(CreditsManager, m_scrollRatePerFrames)},
    {"ScrollDown", INI::parseBool, NULL,
     offsetof(CreditsManager, m_scrollDown)},
    {"TitleColor", INI::parseColorInt, NULL,
     offsetof(CreditsManager, m_titleColor)},
    {"MinorTitleColor", INI::parseColorInt, NULL,
     offsetof(CreditsManager, m_positionColor)},
    {"NormalColor", INI::parseColorInt, NULL,
     offsetof(CreditsManager, m_normalColor)},
    {"Style", INI::parseLookupList, CreditStyleNames,
     offsetof(CreditsManager, m_currentStyle)},
...
void INI::parseCredits(INI *ini) {
  ...
  DEBUG_ASSERTCRASH(
      TheCredits, ("parseCredits: TheCredits has not been ininialized yet.\n"));
  if (!TheCredits) return;
...
CreditsManager::CreditsManager(void) {
  ...
  m_scrollDown = TRUE;  // if TRUE text will come from the top to the bottom if
                        // False, it will go from the bottom up
  m_framesSinceStarted = 0;
  m_titleColor = m_positionColor = m_normalColor =
...
void CreditsManager::load(void) {
  ...
  if (m_scrollRatePerFrames <= 0) m_scrollRatePerFrames = 1;
  if (m_scrollRate <= 0) m_scrollRate = 1;
...
void CreditsManager::update(void) {
  ...
  if (m_framesSinceStarted % m_scrollRatePerFrames != 0) return;
  ...
  Int offsetStartMultiplyer =
      m_scrollDown ? -1 : 0;  // if we're scrolling from the top, we need to
                              // subtract the height
  ...
  if (!((m_scrollDown && (yTest >= start)) ||
        (!m_scrollDown && (yTest <= start))))
    return;

  if (m_displayedCreditLineList.size() == 0 &&
      m_creditLineListIt == m_creditLineList.end())
    m_isFinished = TRUE;

  if (m_creditLineListIt == m_creditLineList.end()) return;
  ...
  switch (cLine->m_style) {
    case CREDIT_STYLE_TITLE: {
      ...
      if (TheGlobalLanguageData && !cLine->m_text.isEmpty()) {
        ...
        ds->setFont(TheFontLibrary->getFont(
            TheGlobalLanguageData->m_creditsTitleFont.name,
            TheGlobalLanguageData->adjustFontSize(
                TheGlobalLanguageData->m_creditsTitleFont.size),
    ...
    case CREDIT_STYLE_POSITION: {
      ...
      if (TheGlobalLanguageData && !cLine->m_text.isEmpty()) {
        ...
        if (!ds) return;
        ds->setFont(TheFontLibrary->getFont(
            TheGlobalLanguageData->m_creditsPositionFont.name,
            TheGlobalLanguageData->adjustFontSize(
                TheGlobalLanguageData->m_creditsPositionFont.size),
  ...
  if (m_creditLineListIt != m_creditLineList.end()) m_creditLineListIt++;
}
    ...
    switch (cLine->m_style) {
      ...
      case CREDIT_STYLE_NORMAL: {
        if (cLine->m_displayString)
          cLine->m_displayString->draw(cLine->m_pos.x, cLine->m_pos.y, color,
                                       bColor, 1, 1);
      } break;
      case CREDIT_STYLE_COLUMN: {
...
void CreditsManager::parseBlank(INI *ini, void *instance, void *store,
                                const void *userData) {
  CreditsManager *cManager = (CreditsManager *)instance;
  cManager->addBlank();
}

void CreditsManager::parseText(INI *ini, void *instance, void *store,
                               const void *userData) {
...
void CreditsManager::addText(AsciiString text) {
  ...
  switch (m_currentStyle) {
    ...
    case CREDIT_STYLE_COLUMN: {
      ...
      if (rIt == m_creditLineList.rend() ||
          rcLine->m_style != CREDIT_STYLE_COLUMN ||
    ...
    default:
      DEBUG_ASSERTCRASH(FALSE,
                        ("CreditsManager::addText we tried to add a credit "
                         "text with the wrong style before it.  Style is %d\n",
                         m_currentStyle));
      delete cLine;
  }
}
...
UnicodeString CreditsManager::getUnicodeString(AsciiString str) {
  UnicodeString uStr;
  if (str.compare("<BLANK>") == 0) return UnicodeString::TheEmptyString;
