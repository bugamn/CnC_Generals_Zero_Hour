...
static Bool similarUnitSelection(Drawable *test, void *userData) {
  ...
  if (test) {
    ...
    if (data->isCarBomb && !isEquivalent &&
        object->testStatus(OBJECT_STATUS_IS_CARBOMB)) {
      isEquivalent = TRUE;
    }

    // only select objects if not already selected
    if (object && isEquivalent && object->isLocallyControlled() &&
        !object->isContained() && !(object->getDrawable()->isSelected()) &&
        object->isMassSelectable()  // And only if they can be multiply
                                    // selected. (otherwise the drawable will
                                    // be, but the object will not be)
    ) {
      ...
      if (TheInGameUI->getMaxSelectCount() > 0 &&
        ...
        if (!TheInGameUI->getDisplayedMaxWarning()) {
          ...
          UnicodeString msg;
          msg.format(TheGameText->fetch("GUI:MaxSelectionSize").str(),
                     TheInGameUI->getMaxSelectCount());
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
SuperweaponInfo::SuperweaponInfo(ObjectID id, UnsignedInt timestamp,
                                 Bool hiddenByScript, Bool hiddenByScience,
                                 Bool ready,
                                 const AsciiString &superweaponNormalFont,
                                 Int superweaponNormalPointSize,
                                 Bool superweaponNormalBold, Color c,
  ...
  setFont(superweaponNormalFont, superweaponNormalPointSize,
          superweaponNormalBold);
}
...
void SuperweaponInfo::setFont(const AsciiString &superweaponNormalFont,
  ...
  m_nameDisplayString->setFont(TheFontLibrary->getFont(
      superweaponNormalFont,
      TheGlobalLanguageData->adjustFontSize(superweaponNormalPointSize),
      superweaponNormalBold));
...
void SuperweaponInfo::drawName(Int x, Int y, Color color, Color dropColor) {
  if (color == 0) color = m_color;
  m_nameDisplayString->draw(x - m_nameDisplayString->getWidth(), y, color,
                            dropColor);
}
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void InGameUI::crc(Xfer *xfer) {}  // end crc

// ------------------------------------------------------------------------------------------------
/** Xfer method
 * Version Info:
 * 1: Initial version
 * 2: Save NamedTimers, but not specifically their Info structs.  We'll recreate
 * them.
 */
// ------------------------------------------------------------------------------------------------
void InGameUI::xfer(Xfer *xfer) {
  ...
  if (version >= 2) {
    // Saving the named timer infos and their friends so we get script timers
    // back after we load
    xfer->xferInt(&m_namedTimerLastFlashFrame);
    xfer->xferBool(&m_namedTimerUsedFlashColor);
    xfer->xferBool(&m_showNamedTimers);

    // For the timers themselves, all I need to save is the things that are used
    // in the call to addNamedTimer. It is okay to do this, because
    // SuperweaponInfos pushes things on to a map; addNamedTimer is just a more
    // organized way to push things on the namedTimer Map.
    // addNamedTimer needs (const AsciiString& timerName, const UnicodeString&
    // text, Bool isCountdown)
    if (xfer->getXferMode() == XFER_SAVE) {
      Int timerCount = m_namedTimers.size();
      xfer->xferInt(&timerCount);
      for (NamedTimerMapIt timerIter = m_namedTimers.begin();
           timerIter != m_namedTimers.end(); ++timerIter) {
  ...
  // xfer->xferBool(&m_inputEnabled);	// no, don't save this yet. somewhat
  // problematic.
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    for (Int playerIndex = 0; playerIndex < MAX_PLAYER_COUNT; ++playerIndex) {
      for (SuperweaponMap::iterator mapIt = m_superweapons[playerIndex].begin();
        ...
        for (SuperweaponList::iterator listIt = swList.begin();
             listIt != swList.end(); ++listIt) {
          SuperweaponInfo *swInfo = *listIt;

          // since this list tends to be somewhat sparse, we write stuff out
          // pretty explicitly.
          xfer->xferInt(&playerIndex);

          AsciiString templateName =
              swInfo->getSpecialPowerTemplate()->getName();
    ...
    for (;;) {
      ...
      const SpecialPowerTemplate *powerTemplate =
          TheSpecialPowerStore->findSpecialPowerTemplate(templateName);
      ...
      // srj sez: due to order-of-operation stuff, sometimes these will already
      // exist, sometimes not. not sure why. so handle both cases.
      SuperweaponInfo *swInfo =
          findSWInfo(playerIndex, powerName, id, powerTemplate);
      if (swInfo == NULL) {
        const Player *player = ThePlayerList->getNthPlayer(playerIndex);
        swInfo = newInstance(SuperweaponInfo)(
            id, timestamp, hiddenByScript, hiddenByScience, ready,
            m_superweaponNormalFont, m_superweaponNormalPointSize,
            m_superweaponNormalBold, player->getPlayerColor(), powerTemplate);
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void InGameUI::loadPostProcess(void) {}  // end loadPostProcess
...
void InGameUI::setMouseCursor(Mouse::MouseCursor c) {
  ...
  if (m_mouseMode == MOUSEMODE_GUI_COMMAND && c != Mouse::ARROW &&
      c != Mouse::SCROLL)
    m_mouseModeCursor = c;
}
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
SuperweaponInfo *InGameUI::findSWInfo(
    Int playerIndex, const AsciiString &powerName, ObjectID id,
    const SpecialPowerTemplate *powerTemplate) {
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void InGameUI::addSuperweapon(Int playerIndex, const AsciiString &powerName,
                              ObjectID id,
                              const SpecialPowerTemplate *powerTemplate) {
  if (powerTemplate == NULL) return;

  // srj sez: don't allow adding the same superweapon more than once. it can
  // happen. not sure how. (srj)
  SuperweaponInfo *swInfo =
      findSWInfo(playerIndex, powerName, id, powerTemplate);
  ...
  Bool hiddenByScience =
      (powerTemplate->getRequiredScience() != SCIENCE_INVALID) &&
      (player->hasScience(powerTemplate->getRequiredScience()) == false);
  ...
  SuperweaponInfo *info = newInstance(SuperweaponInfo)(
      ...
      hiddenByScience,  // Aaayeeee! This is meaningless and just clogs up the
                        // works, sez srj, nuke or repair or SHIP WITH(tm), ASAP
                        //  THe trouble is: There is no mechanism to clear this
                        //  bit when the science is granted, thus, the timer
                        //  never, ever, ever get drawn.... unless the owning
                        //  object is post-science constructed.
      FALSE,  // ready
      m_superweaponNormalFont, m_superweaponNormalPointSize,
      m_superweaponNormalBold, player->getPlayerColor(), powerTemplate);
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
Bool InGameUI::removeSuperweapon(Int playerIndex, const AsciiString &powerName,
                                 ObjectID id,
                                 const SpecialPowerTemplate *powerTemplate) {
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void InGameUI::objectChangedTeam(const Object *obj, Int oldPlayerIndex,
                                 Int newPlayerIndex) {
  ...
  if (obj && oldPlayerIndex >= 0 && newPlayerIndex >= 0) {
    ...
    for (BehaviorModule **m = obj->getBehaviorModules(); *m; ++m) {
      ...
      SuperweaponMap::iterator mapIt =
          m_superweapons[oldPlayerIndex].find(powerName);
      Bool found = false;
      ...
      if (!found) {
        if (TheGameLogic->getFrame() == 0 &&
            BitTest(obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION) ==
                FALSE &&
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void InGameUI::showNamedTimerDisplay(Bool show) { m_showNamedTimers = show; }

//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
const FieldParse InGameUI::s_fieldParseTable[] = {
    {"MaxSelectionSize", INI::parseInt, NULL,
     offsetof(InGameUI, m_maxSelectCount)},

    {"MessageColor1", INI::parseColorInt, NULL,
     offsetof(InGameUI, m_messageColor1)},
    {"MessageColor2", INI::parseColorInt, NULL,
     offsetof(InGameUI, m_messageColor2)},
    {"MessagePosition", INI::parseICoord2D, NULL,
     offsetof(InGameUI, m_messagePosition)},
    {"MessageFont", INI::parseAsciiString, NULL,
     offsetof(InGameUI, m_messageFont)},
    {"MessagePointSize", INI::parseInt, NULL,
     offsetof(InGameUI, m_messagePointSize)},
    {"MessageBold", INI::parseBool, NULL, offsetof(InGameUI, m_messageBold)},
    {"MessageDelayMS", INI::parseInt, NULL,
     offsetof(InGameUI, m_messageDelayMS)},

    {"MilitaryCaptionColor", INI::parseRGBAColorInt, NULL,
     offsetof(InGameUI, m_militaryCaptionColor)},
    {"MilitaryCaptionPosition", INI::parseICoord2D, NULL,
     offsetof(InGameUI, m_militaryCaptionPosition)},

    {"MilitaryCaptionTitleFont", INI::parseAsciiString, NULL,
     offsetof(InGameUI, m_militaryCaptionTitleFont)},
    {"MilitaryCaptionTitlePointSize", INI::parseInt, NULL,
     offsetof(InGameUI, m_militaryCaptionTitlePointSize)},
    {"MilitaryCaptionTitleBold", INI::parseBool, NULL,
     offsetof(InGameUI, m_militaryCaptionTitleBold)},

    {"MilitaryCaptionFont", INI::parseAsciiString, NULL,
     offsetof(InGameUI, m_militaryCaptionFont)},
    {"MilitaryCaptionPointSize", INI::parseInt, NULL,
     offsetof(InGameUI, m_militaryCaptionPointSize)},
    {"MilitaryCaptionBold", INI::parseBool, NULL,
     offsetof(InGameUI, m_militaryCaptionBold)},

    {"MilitaryCaptionRandomizeTyping", INI::parseBool, NULL,
     offsetof(InGameUI, m_militaryCaptionRandomizeTyping)},
    {"MilitaryCaptionSpeed", INI::parseInt, NULL,
     offsetof(InGameUI, m_militaryCaptionSpeed)},
    {"MilitaryCaptionDelayMS", INI::parseInt, NULL,
     offsetof(InGameUI, m_militaryCaptionDelayMS)},

    {"MilitaryCaptionPosition", INI::parseICoord2D, NULL,
     offsetof(InGameUI, m_militaryCaptionPosition)},

    {"SuperweaponCountdownPosition", INI::parseCoord2D, NULL,
     offsetof(InGameUI, m_superweaponPosition)},
    {"SuperweaponCountdownFlashDuration", INI::parseDurationReal, NULL,
     offsetof(InGameUI, m_superweaponFlashDuration)},
    {"SuperweaponCountdownFlashColor", INI::parseColorInt, NULL,
     offsetof(InGameUI, m_superweaponFlashColor)},

    {"SuperweaponCountdownNormalFont", INI::parseAsciiString, NULL,
     offsetof(InGameUI, m_superweaponNormalFont)},
    {"SuperweaponCountdownNormalPointSize", INI::parseInt, NULL,
     offsetof(InGameUI, m_superweaponNormalPointSize)},
    {"SuperweaponCountdownNormalBold", INI::parseBool, NULL,
     offsetof(InGameUI, m_superweaponNormalBold)},

    {"SuperweaponCountdownReadyFont", INI::parseAsciiString, NULL,
     offsetof(InGameUI, m_superweaponReadyFont)},
    {"SuperweaponCountdownReadyPointSize", INI::parseInt, NULL,
     offsetof(InGameUI, m_superweaponReadyPointSize)},
    {"SuperweaponCountdownReadyBold", INI::parseBool, NULL,
     offsetof(InGameUI, m_superweaponReadyBold)},

    {"NamedTimerCountdownPosition", INI::parseCoord2D, NULL,
     offsetof(InGameUI, m_namedTimerPosition)},
    {"NamedTimerCountdownFlashDuration", INI::parseDurationReal, NULL,
     offsetof(InGameUI, m_namedTimerFlashDuration)},
    {"NamedTimerCountdownFlashColor", INI::parseColorInt, NULL,
     offsetof(InGameUI, m_namedTimerFlashColor)},

    {"NamedTimerCountdownNormalFont", INI::parseAsciiString, NULL,
     offsetof(InGameUI, m_namedTimerNormalFont)},
    {"NamedTimerCountdownNormalPointSize", INI::parseInt, NULL,
     offsetof(InGameUI, m_namedTimerNormalPointSize)},
    {"NamedTimerCountdownNormalBold", INI::parseBool, NULL,
     offsetof(InGameUI, m_namedTimerNormalBold)},
    {"NamedTimerCountdownNormalColor", INI::parseColorInt, NULL,
     offsetof(InGameUI, m_namedTimerNormalColor)},

    {"NamedTimerCountdownReadyFont", INI::parseAsciiString, NULL,
     offsetof(InGameUI, m_namedTimerReadyFont)},
    {"NamedTimerCountdownReadyPointSize", INI::parseInt, NULL,
     offsetof(InGameUI, m_namedTimerReadyPointSize)},
    {"NamedTimerCountdownReadyBold", INI::parseBool, NULL,
     offsetof(InGameUI, m_namedTimerReadyBold)},
    {"NamedTimerCountdownReadyColor", INI::parseColorInt, NULL,
     offsetof(InGameUI, m_namedTimerReadyColor)},

    {"FloatingTextTimeOut", INI::parseDurationUnsignedInt, NULL,
     offsetof(InGameUI, m_floatingTextTimeOut)},
    {"FloatingTextMoveUpSpeed", INI::parseVelocityReal, NULL,
     offsetof(InGameUI, m_floatingTextMoveUpSpeed)},
    {"FloatingTextVanishRate", INI::parseVelocityReal, NULL,
     offsetof(InGameUI, m_floatingTextMoveVanishRate)},

    {"PopupMessageColor", INI::parseColorInt, NULL,
     offsetof(InGameUI, m_popupMessageColor)},

    {"DrawableCaptionFont", INI::parseAsciiString, NULL,
     offsetof(InGameUI, m_drawableCaptionFont)},
    {"DrawableCaptionPointSize", INI::parseInt, NULL,
     offsetof(InGameUI, m_drawableCaptionPointSize)},
    {"DrawableCaptionBold", INI::parseBool, NULL,
     offsetof(InGameUI, m_drawableCaptionBold)},
    {"DrawableCaptionColor", INI::parseColorInt, NULL,
     offsetof(InGameUI, m_drawableCaptionColor)},

    {"DrawRMBScrollAnchor", INI::parseBool, NULL,
     offsetof(InGameUI, m_drawRMBScrollAnchor)},
    {"MoveRMBScrollAnchor", INI::parseBool, NULL,
     offsetof(InGameUI, m_moveRMBScrollAnchor)},

    {"AttackDamageAreaRadiusCursor",
     RadiusDecalTemplate::parseRadiusDecalTemplate, NULL,
     offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_ATTACK_DAMAGE_AREA])},
    {"AttackScatterAreaRadiusCursor",
     RadiusDecalTemplate::parseRadiusDecalTemplate, NULL,
     offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_ATTACK_SCATTER_AREA])},
    {"AttackContinueAreaRadiusCursor",
     RadiusDecalTemplate::parseRadiusDecalTemplate, NULL,
     offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_ATTACK_CONTINUE_AREA])},
    {"FriendlySpecialPowerRadiusCursor",
     RadiusDecalTemplate::parseRadiusDecalTemplate, NULL,
     offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_FRIENDLY_SPECIALPOWER])},
    {"OffensiveSpecialPowerRadiusCursor",
     RadiusDecalTemplate::parseRadiusDecalTemplate, NULL,
     offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_OFFENSIVE_SPECIALPOWER])},
    {"SuperweaponScatterAreaRadiusCursor",
     RadiusDecalTemplate::parseRadiusDecalTemplate, NULL,
     offsetof(InGameUI,
              m_radiusCursors[RADIUSCURSOR_SUPERWEAPON_SCATTER_AREA])},

    {"GuardAreaRadiusCursor", RadiusDecalTemplate::parseRadiusDecalTemplate,
     NULL, offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_GUARD_AREA])},
    {"EmergencyRepairRadiusCursor",
     RadiusDecalTemplate::parseRadiusDecalTemplate, NULL,
     offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_EMERGENCY_REPAIR])},

    {"ParticleCannonRadiusCursor",
     RadiusDecalTemplate::parseRadiusDecalTemplate, NULL,
     offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_PARTICLECANNON])},
    {"A10StrikeRadiusCursor", RadiusDecalTemplate::parseRadiusDecalTemplate,
     NULL, offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_A10STRIKE])},
    {"CarpetBombRadiusCursor", RadiusDecalTemplate::parseRadiusDecalTemplate,
     NULL, offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_CARPETBOMB])},
    {"DaisyCutterRadiusCursor", RadiusDecalTemplate::parseRadiusDecalTemplate,
     NULL, offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_DAISYCUTTER])},
    {"ParadropRadiusCursor", RadiusDecalTemplate::parseRadiusDecalTemplate,
     NULL, offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_PARADROP])},
    {"SpySatelliteRadiusCursor", RadiusDecalTemplate::parseRadiusDecalTemplate,
     NULL, offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_SPYSATELLITE])},

    {"NuclearMissileRadiusCursor",
     RadiusDecalTemplate::parseRadiusDecalTemplate, NULL,
     offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_NUCLEARMISSILE])},
    {"EMPPulseRadiusCursor", RadiusDecalTemplate::parseRadiusDecalTemplate,
     NULL, offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_EMPPULSE])},
    {"ArtilleryRadiusCursor", RadiusDecalTemplate::parseRadiusDecalTemplate,
     NULL, offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_ARTILLERYBARRAGE])},
    {"NapalmStrikeRadiusCursor", RadiusDecalTemplate::parseRadiusDecalTemplate,
     NULL, offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_NAPALMSTRIKE])},
    {"ClusterMinesRadiusCursor", RadiusDecalTemplate::parseRadiusDecalTemplate,
     NULL, offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_CLUSTERMINES])},

    {"ScudStormRadiusCursor", RadiusDecalTemplate::parseRadiusDecalTemplate,
     NULL, offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_SCUDSTORM])},
    {"AnthraxBombRadiusCursor", RadiusDecalTemplate::parseRadiusDecalTemplate,
     NULL, offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_ANTHRAXBOMB])},
    {"AmbushRadiusCursor", RadiusDecalTemplate::parseRadiusDecalTemplate, NULL,
     offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_AMBUSH])},
    {"RadarRadiusCursor", RadiusDecalTemplate::parseRadiusDecalTemplate, NULL,
     offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_RADAR])},
    {"SpyDroneRadiusCursor", RadiusDecalTemplate::parseRadiusDecalTemplate,
     NULL, offsetof(InGameUI, m_radiusCursors[RADIUSCURSOR_SPYDRONE])},
  ...
  if (TheGlobalLanguageData) {
    if (TheGlobalLanguageData->m_drawableCaptionFont.name.isNotEmpty()) {
      ...
      m_drawableCaptionPointSize =
          TheGlobalLanguageData->m_drawableCaptionFont.size;
    ...
    if (TheGlobalLanguageData->m_militaryCaptionTitleFont.name.isNotEmpty()) {
      m_militaryCaptionTitleFont =
          TheGlobalLanguageData->m_militaryCaptionTitleFont.name;
      m_militaryCaptionTitlePointSize =
          TheGlobalLanguageData->m_militaryCaptionTitleFont.size;
      m_militaryCaptionTitleBold =
          TheGlobalLanguageData->m_militaryCaptionTitleFont.bold;
    }
    ...
    if (TheGlobalLanguageData->m_militaryCaptionFont.name.isNotEmpty()) {
      ...
      m_militaryCaptionPointSize =
          TheGlobalLanguageData->m_militaryCaptionFont.size;
      m_militaryCaptionBold = TheGlobalLanguageData->m_militaryCaptionFont.bold;
    }

    if (TheGlobalLanguageData->m_superweaponCountdownNormalFont.name
            .isNotEmpty()) {
      m_superweaponNormalFont =
          TheGlobalLanguageData->m_superweaponCountdownNormalFont.name;
      m_superweaponNormalPointSize =
          TheGlobalLanguageData->m_superweaponCountdownNormalFont.size;
      m_superweaponNormalBold =
          TheGlobalLanguageData->m_superweaponCountdownNormalFont.bold;
    }

    if (TheGlobalLanguageData->m_superweaponCountdownReadyFont.name
            .isNotEmpty()) {
      m_superweaponReadyFont =
          TheGlobalLanguageData->m_superweaponCountdownReadyFont.name;
      m_superweaponReadyPointSize =
          TheGlobalLanguageData->m_superweaponCountdownReadyFont.size;
      m_superweaponReadyBold =
          TheGlobalLanguageData->m_superweaponCountdownReadyFont.bold;
    }

    if (TheGlobalLanguageData->m_namedTimerCountdownNormalFont.name
            .isNotEmpty()) {
      m_namedTimerNormalFont =
          TheGlobalLanguageData->m_namedTimerCountdownNormalFont.name;
      m_namedTimerNormalPointSize =
          TheGlobalLanguageData->m_namedTimerCountdownNormalFont.size;
      m_namedTimerNormalBold =
          TheGlobalLanguageData->m_namedTimerCountdownNormalFont.bold;
    }

    if (TheGlobalLanguageData->m_namedTimerCountdownReadyFont.name
            .isNotEmpty()) {
      m_namedTimerReadyFont =
          TheGlobalLanguageData->m_namedTimerCountdownReadyFont.name;
      m_namedTimerReadyPointSize =
          TheGlobalLanguageData->m_namedTimerCountdownReadyFont.size;
      m_namedTimerReadyBold =
          TheGlobalLanguageData->m_namedTimerCountdownReadyFont.bold;
    }
  }
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void InGameUI::setRadiusCursor(RadiusCursorType cursorType,
                               const SpecialPowerTemplate *specPowTempl,
                               WeaponSlotType weaponSlot) {
  if (cursorType == m_curRcType) return;

  m_curRadiusCursor.clear();
  m_curRcType = RADIUSCURSOR_NONE;

  if (cursorType == RADIUSCURSOR_NONE) return;

  Object *obj = NULL;
  if (m_pendingGUICommand &&
      m_pendingGUICommand->getCommandType() ==
  ...
  m_radiusCursors[cursorType].createRadiusDecal(pos, radius, controller,
                                                m_curRadiusCursor);
...
void InGameUI::handleRadiusCursor() {
  if (!m_curRadiusCursor.isEmpty()) {
    ...
    //
    // if the mouse is in the radar window, the position in the world is that
    // which is represented by the radar, otherwise we use the mouse position
    // itself transformed from screen to world But only if the radar is on.
    //
    Bool radarOn =
        TheRadar->isRadarForced() ||
        (!TheRadar->isRadarHidden() && ThePlayerList->getLocalPlayer() &&
         ThePlayerList->getLocalPlayer()->hasRadar());

    if (!radarOn || (TheRadar->screenPixelToWorld(&mouseIO->pos, &pos) ==
                     FALSE))  // if radar off, or point not on radar
      TheTacticalView->screenToTerrain(&mouseIO->pos, &pos);

    m_curRadiusCursor.setPosition(
        pos);  // world space position of center of decal
    m_curRadiusCursor.update();
  }
}

//-------------------------------------------------------------------------------------------------
/** Handle the placement "icons" that appear at the cursor when we're putting
 * down a structure to build.  Note that this has additional logic to also show
 * a line of objects because when we build "walls" we want to draw a line of
 * repeating wall pieces on the map where we want to put all of them */
//-------------------------------------------------------------------------------------------------
...
void InGameUI::evaluateSoloNexus(Drawable *newlyAddedDrawable) {
  ...
  if (newlyAddedDrawable) {
    ...
    if (newObj && !(newObj->isKindOf(KINDOF_MOB_NEXUS) ||
                    newObj->isKindOf(KINDOF_IGNORED_IN_GUI)))
      return;
  }
  ...
  for (DrawableListCIt it = m_selectedDrawables.begin();
    ...
    } else if (!obj->isKindOf(
                   KINDOF_IGNORED_IN_GUI))  // darn! a non-angrymobster!
    {
      m_soloNexusSelectedDrawableID = INVALID_DRAWABLE_ID;
      return;
    }
...
void InGameUI::handleBuildPlacements(void) {
  //
  // if we're in the process of placing something we need up update one or more
  // drawables based on the position of the mouse
  //
  if (m_pendingPlaceType) {
    ...
    // location the icon will be at (anchored is the start, otherwise it's the
    // mouse)
    if (isPlacementAnchored()) {
      ...
      if (start.x != end.x || start.y != end.y) {
        ...
        // project the start and the end points of the line anchor into the 3D
        // world
    ...
    /**@todo this whole orientation vector thing is LAME! Must replace, all I
    want to to do is set a simple angle and have it automatically change, ug! */
    TheTacticalView->screenToTerrain(&loc, &world);
    m_placeIcon[0]->setPosition(&world);
    m_placeIcon[0]->setOrientation(angle);

    //
    // check to see if this is a legal location to build something at and tint
    // or "un-tint" the cursor icons as appropriate.  This involves a pathfind
    // which could be expensive so we don't want to do it on every frame
    // (althought that would be ideal) If we discover there are cases that this
    // is just too slow we should increase the delay time between checks or we
    // need to come up with a way of recording what is valid and what isn't or
    // "fudge" the results to feel "ok"
    //
    if (TheGameClient->getFrame() & 0x1) {
      TheTerrainVisual->removeAllBibs();

      Object *builderObject =
          TheGameLogic->findObjectByID(getPendingPlaceSourceObjectID());

      LegalBuildCode lbc;
      lbc = TheBuildAssistant->isLocationLegalToBuild(
          &world, m_pendingPlaceType, angle,
    ...
    //
    // we have additional place icons when we're placing down a line of walls or
    // other similarly placed object ... for those we will have them be oriented
    // the same way as the first one, but we'll set their positions so that they
    // "tile" end to end
    //
    if (isPlacementAnchored() &&
      ...
      Real objectSize =
          m_pendingPlaceType->getTemplateGeometryInfo().getMajorRadius() * 2.0f;
      ...
      Object *builderObject = TheGameLogic->findObjectByID(
          TheInGameUI->getPendingPlaceSourceObjectID());

      //
      // given the start/end points in the world and the the angle of the wall,
      // fill out an array of positions that "tile" this wall across the
      // landscape
      //
      BuildAssistant::TileBuildInfo *tileBuildInfo;
      tileBuildInfo = TheBuildAssistant->buildTiledLocations(
          m_pendingPlaceType, angle, &worldStart, &worldEnd, objectSize,
          maxObjects, builderObject);

      // create any necessary drawables we need to "fill out" the line
      for (i = 0; i < tileBuildInfo->tilesUsed; i++) {
        if (m_placeIcon[i] == NULL)
          m_placeIcon[i] = TheThingFactory->newDrawable(
              m_pendingPlaceType, DRAWABLE_STATUS_NO_STATE_PARTICLES);
      ...
      //
      // march down each drawable and set the position based on its position in
      // the line and set their angles all the same
      //
      for (i = 0; i < tileBuildInfo->tilesUsed; i++) {
...
void InGameUI::preDraw(void) {
  // handle any "icons" for the act of building things and placing them in the
  // world
...
void InGameUI::update(void) {
  ...
  //
  // remove any message strings that have expired, note that the oldest strings
  // are always at the end of the array (higher index numbers) so we can just
  // remove things from the rear and never have to worry about shifting entries
  // cause we check every frame
  //
  ...
  for (i = MAX_UI_MESSAGES - 1; i >= 0; i--) {
    if (currLogicFrame - m_uiMessages[i].timestamp > messageTimeout) {
      ...
      amount =
          REAL_TO_INT(((currLogicFrame - m_uiMessages[i].timestamp) * 0.01f));
      if (a - amount < 0)
        a = 0;
      else
        a -= amount;
  ...
  if (m_militarySubtitle)  // if we have a subtitle, work on it
  {
    ...
    if ((Int)m_militarySubtitle->lifetime < (Int)currLogicFrame) {
      ...
      // start fading the alpha on this color down
      amount =
          REAL_TO_INT(((currLogicFrame - m_militarySubtitle->lifetime) * 0.1f));
      if (a - amount < 0) {
        removeMilitarySubtitle();
      } else {
        a -= amount;
      ...
      if (m_militarySubtitle->incrementOnFrame < currLogicFrame) {
        ...
        WideChar tempWChar =
            m_militarySubtitle->subtitle.getCharAt(m_militarySubtitle->index);
        // if that letter is a return, add a new line
        if (tempWChar == L'\n') {
          // increment the Block position's Y value to draw it on the next line
          Int height;
          m_militarySubtitle
              ->displayStrings[m_militarySubtitle->currentDisplayString]
              ->getSize(NULL, &height);
          m_militarySubtitle->blockPos.y =
              m_militarySubtitle->blockPos.y + height;

          // Now add a new display string
          m_militarySubtitle->currentDisplayString++;
          if (!(m_militarySubtitle->currentDisplayString >=
                MAX_SUBTITLE_LINES)) {
            m_militarySubtitle->blockPos.x = m_militarySubtitle->position.x;
            m_militarySubtitle
                ->displayStrings[m_militarySubtitle->currentDisplayString] =
                TheDisplayStringManager->newDisplayString();
            m_militarySubtitle
                ->displayStrings[m_militarySubtitle->currentDisplayString]
                ->reset();
            m_militarySubtitle
                ->displayStrings[m_militarySubtitle->currentDisplayString]
                ->setFont(TheFontLibrary->getFont(
                    m_militaryCaptionFont,
                    TheGlobalLanguageData->adjustFontSize(
                        m_militaryCaptionPointSize),
                    m_militaryCaptionBold));

            m_militarySubtitle->blockDrawn = TRUE;
            m_militarySubtitle->incrementOnFrame =
                currLogicFrame + (Int)(((Real)LOGICFRAMES_PER_SECOND *
                                        m_militaryCaptionDelayMS) /
                                       1000.0f);
          } else {
            // if we've exceeded the allocated number of display strings, this
            // will force us to essentially truncate the remaining text
            m_militarySubtitle->index =
                m_militarySubtitle->subtitle.getLength();
            DEBUG_CRASH(
                ("You're Only Allowed to use %d lines of subtitle text\n",
                 MAX_SUBTITLE_LINES));
          }
        } else {
          // okay, we're not a \n, lets append this character to the display
          // string
          m_militarySubtitle
              ->displayStrings[m_militarySubtitle->currentDisplayString]
              ->appendChar(tempWChar);
          // increment the draw position of the block
          Int width;
          m_militarySubtitle
              ->displayStrings[m_militarySubtitle->currentDisplayString]
              ->getSize(&width, NULL);
          m_militarySubtitle->blockPos.x =
              m_militarySubtitle->position.x + width;
          ...
          if (TheGlobalLanguageData)
            m_militarySubtitle->incrementOnFrame =
                currLogicFrame + TheGlobalLanguageData->m_militaryCaptionSpeed;
          else
            m_militarySubtitle->incrementOnFrame =
                currLogicFrame + m_militaryCaptionSpeed;
        }
        // increment the index
        m_militarySubtitle->index++;
        if (m_militarySubtitle->index >=
            m_militarySubtitle->subtitle.getLength()) {
          // We're at the end of the subtitle, set everything to persist till
          // the subtitle has expired
          m_militarySubtitle->incrementOnFrame =
              m_militarySubtitle->lifetime + 1;
        }
        /*
                                                        else
                                                                {
           ...
           randomize the space between printing of characters
                                                                        if(GameClientRandomValueReal(0,1)
           < 0.95f)
                                                                        {
  ...
  static NameKeyType moneyWindowKey =
      TheNameKeyGenerator->nameToKey("ControlBar.wnd:MoneyDisplay");
  static NameKeyType powerWindowKey =
      TheNameKeyGenerator->nameToKey("ControlBar.wnd:PowerWindow");

  GameWindow *moneyWin =
      TheWindowManager->winGetWindowFromId(NULL, moneyWindowKey);
  GameWindow *powerWin =
      TheWindowManager->winGetWindowFromId(NULL, powerWindowKey);
  //	if( moneyWin == NULL )
  //	{
  //		NameKeyType moneyWindowKey = TheNameKeyGenerator->nameToKey(
  //"ControlBar.wnd:MoneyDisplay" );
  //
  //		moneyWin = TheWindowManager->winGetWindowFromId( NULL,
  //moneyWindowKey );
  //
  //	}  // end if
  ...
  if (moneyPlayer) {
    ...
    if (lastMoney != currentMoney) {
      ...
      buffer.format(TheGameText->fetch("GUI:ControlBarMoneyDisplay"),
                    currentMoney);
  ...
  for (std::list<WindowLayout *>::iterator it = m_windowLayouts.begin();
       it != m_windowLayouts.end(); ++it) {
  ...
  if (m_cameraRotatingLeft && !m_cameraRotatingRight) {
    ...
    TheTacticalView->setAngle(TheTacticalView->getAngle() -
                              TheGlobalData->m_keyboardCameraRotateSpeed);
  }
...
void InGameUI::reset(void) {
  ...
  for (i = 0; i < MAX_PLAYER_COUNT; ++i) {
    for (SuperweaponMap::iterator mapIt = m_superweapons[i].begin();
         mapIt != m_superweapons[i].end(); ++mapIt) {
      for (SuperweaponList::iterator listIt = mapIt->second.begin();
           listIt != mapIt->second.end(); ++listIt) {
        SuperweaponInfo *info = *listIt;
        info->deleteInstance();
  ...
  for (NamedTimerMapIt timerIt = m_namedTimers.begin();
       timerIt != m_namedTimers.end(); ++timerIt) {
...
//-------------------------------------------------------------------------------------------------
/** Same as the unicode message method, but this takes an ascii string which is
 * assumed to me a string manager label */
//-------------------------------------------------------------------------------------------------
// srj sez: passing as const-ref screws up varargs for some reason. dunno why.
// just pass by value.
void InGameUI::message(AsciiString stringManagerLabel, ...) {
  ...
  if (_vsnwprintf(buf, sizeof(buf) / sizeof(WideChar) - 1,
                  stringManagerString.str(), args) < 0)
...
void InGameUI::message(UnicodeString format, ...) {
  ...
  WideChar buf[UnicodeString::MAX_FORMAT_BUF_LEN];
  if (_vsnwprintf(buf, sizeof(buf) / sizeof(WideChar) - 1, format.str(), args) <
      0)
    throw ERROR_OUT_OF_MEMORY;
  formattedMessage.set(buf);
  va_end(args);
...
void InGameUI::addMessageText(const UnicodeString &formattedMessage,
  ...
  if (m_uiMessages[MAX_UI_MESSAGES - 1].displayString)
    TheDisplayStringManager->freeDisplayString(
        m_uiMessages[MAX_UI_MESSAGES - 1].displayString);
  ...
  //
  // set the new message in index 0, note that we need to allocate a display
  // string, but we do not need to free the one that is already there because it
  // has been moved "up" an index
  //
  ...
  m_uiMessages[0].displayString->setFont(TheFontLibrary->getFont(
      m_messageFont, TheGlobalLanguageData->adjustFontSize(m_messagePointSize),
      m_messageBold));
  m_uiMessages[0].displayString->setText(m_uiMessages[0].fullText);

  //
  // assign a color for this string instance that will stay with it no matter
  // what line it is rendered on
  //
...
void InGameUI::createMoveHint(const GameMessage *msg) {
  ...
  // wrap around
  if (m_nextMoveHint == InGameUI::MAX_MOVE_HINTS) m_nextMoveHint = 0;
}
...
void InGameUI::createGarrisonHint(const GameMessage *msg) {
  Drawable *draw =
      TheGameClient->findDrawableByID(msg->getArgument(0)->drawableID);
  if (draw) {
    draw->onSelected();
  }
}
...
//-------------------------------------------------------------------------------------------------
/** Details of what is mouse hovered over right now are in this message. Terrain
 * might result in just a tooltip.  An object might get a tooltip and show its
 * hit points.
 */
//-------------------------------------------------------------------------------------------------
void InGameUI::createMouseoverHint(const GameMessage *msg) {
  if (m_isScrolling || m_isSelecting) return;  // no mouseover for you
  ...
  while (window) {
    ...
    // check to see if it or any of its parents are opaque.  If so, we can't
    // select anything.
  ...
  if (msg->getType() == GameMessage::MSG_MOUSEOVER_DRAWABLE_HINT) {
    ...
    if (obj) {
      // Ahh, here is a wierd exception: if the moused-over drawable is a
      // mob-member (e.g. AngryMob), Lets fool the UI into creating the hint for
      //the NEXUS instead...
      if (obj->isKindOf(KINDOF_IGNORED_IN_GUI)) {
        static NameKeyType key_MobMemberSlavedUpdate =
            NAMEKEY("MobMemberSlavedUpdate");
        MobMemberSlavedUpdate *MMSUpdate =
            (MobMemberSlavedUpdate *)obj->findUpdateModule(
                key_MobMemberSlavedUpdate);
        if (MMSUpdate) {
          Object *slaver =
              TheGameLogic->findObjectByID(MMSUpdate->getSlaverID());
          if (slaver) {
            Drawable *slaverDraw = slaver->getDrawable();
            if (slaverDraw) m_mousedOverDrawableID = slaverDraw->getID();
            // if this fails, not to worry... it has already defaulted to
            // INVALID_DRAWABLE_ID, above
          }
        }
      } else
        m_mousedOverDrawableID = draw->getID();

#if defined(_DEBUG) || defined(_INTERNAL)  // Extra hacky, sorry, but I need to
                                           // use this in constantdebug report
      ...
      if (contain)
        player = contain->getApparentControllingPlayer(
            ThePlayerList->getLocalPlayer());

      if (player == NULL) player = obj->getControllingPlayer();

      Bool disguised = false;
      if (obj->isKindOf(KINDOF_DISGUISER)) {
        // Because we have support for disguised units pretending to be units
        // from another team, we need to intercept it here and make sure it's
        // rendered appropriately based on which client is rendering it.
        static NameKeyType key_StealthUpdate = NAMEKEY("StealthUpdate");
        StealthUpdate *update =
            (StealthUpdate *)obj->findUpdateModule(key_StealthUpdate);
        if (update) {
          if (update->isDisguised()) {
            Player *clientPlayer = ThePlayerList->getLocalPlayer();
            Player *disguisedPlayer =
                ThePlayerList->getNthPlayer(update->getDisguisedPlayerIndex());
            if (player->getRelationship(clientPlayer->getDefaultTeam()) !=
                    ALLIES &&
                clientPlayer->isPlayerActive()) {
              // Neutrals and enemies will see this disguised unit as the team
              // it's disguised as.
              player = disguisedPlayer;
              const ThingTemplate *disguisedTemplate =
                  update->getDisguisedTemplate();
              if (disguisedTemplate) {
                thingTemplate = disguisedTemplate;
                disguised = true;
              }
            }
            // Otherwise, the color will show up as the team it really belongs
            // to (already set above).
          }
        }
      }
      ...
      if (TheGlobalData->m_debugAI) {
        ...
        if (!objName.isEmpty()) {
          if (!teamName.isEmpty()) {
            str.format(L"%hs(%hs): %s", teamName.str(), objName.str(),
                       str.str());
          } else {
      ...
      // Add on dollar amount of warehouse contents so people don't freak out
      // until the art is hooked up
      ...
      if (player) {
        ...
        // if (TheRecorder->isMultiplayer() && player->getPlayerType() ==
        // PLAYER_HUMAN)
        if (TheRecorder->isMultiplayer() && player->isPlayableSide())
          tooltip.format(L"%s\n%s", str.str(),
                         ((Player *)player)->getPlayerDisplayName().str());
        else
          tooltip = str;

        Int localPlayerIndex =
            ThePlayerList ? ThePlayerList->getLocalPlayer()->getPlayerIndex()
                          : 0;

        Int x, y;
        ThePartitionManager->worldToCell(obj->getPosition()->x,
                                         obj->getPosition()->y, &x, &y);
        if (ThePartitionManager->getShroudStatusForPlayer(
                localPlayerIndex, x, y) == CELLSHROUD_CLEAR) {
          RGBColor rgb;
          if (disguised) {
            ...
            if (obj) {
              ...
              if (contain && contain->isGarrisonable()) {
                const Player *play = contain->getApparentControllingPlayer(
                    ThePlayerList->getLocalPlayer());
                if (play) rgb.setFromInt(play->getPlayerColor());
              }
            }
          }
  ...
  if (m_mouseMode == MOUSEMODE_DEFAULT && !m_isScrolling && !m_isSelecting &&
      !TheInGameUI->getSelectCount() &&
      (TheRecorder->getMode() != RECORDERMODETYPE_PLAYBACK ||
       TheLookAtTranslator->hasMouseMovedRecently())) {
...
//-------------------------------------------------------------------------------------------------
/** A command would be given if a click were to happen, so give a preview hint
 * of what it would be. Changing the mouse cursor is an example
 */
void InGameUI::createCommandHint(const GameMessage *msg) {
  ...
  const Drawable *draw =
      TheGameClient->findDrawableByID(m_mousedOverDrawableID);
  GameMessage::Type t = msg->getType();
  // #ifdef DO_SHROUD_PROJECTION
  if (draw && (t == GameMessage::MSG_DO_ATTACK_OBJECT_HINT ||
               t == GameMessage::MSG_DO_ATTACK_OBJECT_AFTER_MOVING_HINT)) {
    const Object *obj = draw->getObject();
    Int localPlayerIndex =
        ThePlayerList ? ThePlayerList->getLocalPlayer()->getPlayerIndex() : 0;
#if defined(_DEBUG) || defined(_INTERNAL)
    ObjectShroudStatus ss = (!obj || !TheGlobalData->m_shroudOn)
                                ? OBJECTSHROUD_CLEAR
                                : obj->getShroudedStatus(localPlayerIndex);
#else
    ObjectShroudStatus ss =
        (!obj) ? OBJECTSHROUD_CLEAR : obj->getShroudedStatus(localPlayerIndex);
#endif
    if (ss == OBJECTSHROUD_SHROUDED) {
      t = GameMessage::MSG_DO_MOVETO_HINT;  // if the object is hidden, switch
  ...
  switch (m_mouseMode) {
    case MOUSEMODE_DEFAULT: {
      // This section of code only gets called when there is no specific cursor
      // mode happening.
      ...
      switch (t) {
        case GameMessage::MSG_DO_MOVETO_HINT: {
          if (!drawSelectable && srcObj && srcObj->isLocallyControlled() &&
    ...
    case MOUSEMODE_GUI_COMMAND: {
      ...
      // set the mouse cursor for commands that need a targeting or to normal
      // with no command
      if (m_pendingGUICommand) {
        if (m_pendingGUICommand->isContextCommand() ||
            ...
            m_pendingGUICommand->getCommandType() ==
                GUI_COMMAND_SPECIAL_POWER_FROM_COMMAND_CENTER) {
          // Here is the hook for when we are in a context sensitive command
          // mode. We can either do the specified command mode command or
          // nothing! Whether or not the command is valid or not was determined
          // in evaluateContextCommand which is called first, and posts the
          // appropriate message.
        ...
        } else if (BitTest(m_pendingGUICommand->getOptions(),
                           COMMAND_OPTION_NEED_TARGET)) {
          Int index =
              TheMouse->getCursorIndex(m_pendingGUICommand->getCursorName());
          if (index != Mouse::INVALID_MOUSE_CURSOR)
            setMouseCursor((Mouse::MouseCursor)index);
          else
            setMouseCursor(Mouse::CROSS);
...
/// are we scrolling?
//-------------------------------------------------------------------------------------------------
Bool InGameUI::isScrolling(void) { return m_isScrolling; }
...
/// are we selecting?
//-------------------------------------------------------------------------------------------------
Bool InGameUI::isSelecting(void) { return m_isSelecting; }

//-------------------------------------------------------------------------------------------------
/// get scroll amount
//-------------------------------------------------------------------------------------------------
void InGameUI::setScrollAmount(Coord2D amt) { m_scrollAmt = amt; }

//-------------------------------------------------------------------------------------------------
/// get scroll amount
//-------------------------------------------------------------------------------------------------
Coord2D InGameUI::getScrollAmount(void) { return m_scrollAmt; }

//-------------------------------------------------------------------------------------------------
/** Like the building "placement" mode, clicking on some buttons in the UI
 * require us to provide additional data by clicking on a target object/location
 * in the world.  This is where we enable that "mode" so that we can get the
 * additional data needed for a command from the user */
//-------------------------------------------------------------------------------------------------
void InGameUI::setGUICommand(const CommandButton *command) {
  if (TheRecorder->getMode() == RECORDERMODETYPE_PLAYBACK) return;

  // sanity
  if (command) {
    if (BitTest(command->getOptions(), COMMAND_OPTION_NEED_TARGET) == FALSE) {
      DEBUG_ASSERTCRASH(0, ("setGUICommand: Command '%s' does not need "
  ...
  // set the mouse cursor for commands that need a targeting or to normal with
  // no command
  if (command && BitTest(command->getOptions(), COMMAND_OPTION_NEED_TARGET) &&
      !command->isContextCommand()) {
    setMouseCursor(Mouse::ARROW);  // This occurs on the mouse-up of a panel
                                   // button, so make an arrow
    // the mouseoverhint code will take care of the cursor context, once the
    // mouse leaves the panel but we will set the radius cursor here, so you can
    // see it bleeding out from beneath the panel

    setRadiusCursor(
        command
            ->getRadiusCursorType(),  //*****************************************************************
        command->getSpecialPowerTemplate(), command->getWeaponSlot());
  } else {
    if (TheMouse) {
      setMouseCursor(Mouse::ARROW);
    }
...
//-------------------------------------------------------------------------------------------------
/** User has clicked on a built item that requires placement in the world.  We
 * will record what that thing is so that the we can catch the next click in the
 * world and try to place the object there */
//-------------------------------------------------------------------------------------------------
void InGameUI::placeBuildAvailable(const ThingTemplate *build,
  ...
  //
  // if we're setting another place available, but we're somehow already in the
  // placement mode, get out of it before we start a new one
  //
  ...
  if (buildDrawable) sourceObject = buildDrawable->getObject();
  if (sourceObject) m_pendingPlaceSourceObjectID = sourceObject->getID();

  //
  // hack, change our cursor to at least something different ... also note that
  // it's possible to not have the mouse yet, as some UI systems as part of
  // initialization make sure that there isn't anything valid for to "place
  // build"
  //
  if (TheMouse) {
    if (build) {
      ...
      // capture the mouse for our window, windows is lame and changes it if we
      // don't
      TheMouse->capture();
      ...
      draw = TheThingFactory->newDrawable(build,
                                          DRAWABLE_STATUS_NO_STATE_PARTICLES);
      if (sourceObject) {
        if (TheGlobalData->m_timeOfDay == TIME_OF_DAY_NIGHT)
          draw->setIndicatorColor(
              sourceObject->getControllingPlayer()->getPlayerNightColor());
        else
          draw->setIndicatorColor(
              sourceObject->getControllingPlayer()->getPlayerColor());
      }
      DEBUG_ASSERTCRASH(draw,
                        ("Unable to create icon at cursor for placement '%s'\n",
                         build->getName().str()));

      //
      // set the initial angle of the free floating building to the property
      // from INI we have this so we can have the "cool" face the user until
      // they click and pick an actual direction for placement
      //
      Real angle = build->getPlacementViewAngle();

      // don't forget to take into account the current view angle
      // angle += TheTacticalView->getAngle();	Don't do this - makes odd angled
      // building placements.  jba.
      ...
      DEBUG_ASSERTCRASH(
          m_placeIcon[0] == NULL,
          ("placeBuildAvailable, build icon array is not empty!"));
      m_placeIcon[0] = draw;
...
//-------------------------------------------------------------------------------------------------
/** Start the angle selection interface for selecting building angles when
 * placing them */
//-------------------------------------------------------------------------------------------------
void InGameUI::setPlacementStart(const ICoord2D *start) {
  // if we have a start point we turn "on" the interface, otherwise we turn it
  // "off"
  if (start) {
...
//-------------------------------------------------------------------------------------------------
/** Get the start and end anchor points for the building angle selection
 * interface */
//-------------------------------------------------------------------------------------------------
void InGameUI::getPlacementPoints(ICoord2D *start, ICoord2D *end) {
  if (start) *start = m_placeAnchorStart;
  if (end) *end = m_placeAnchorEnd;
...
Real InGameUI::getPlacementAngle(void) {
  if (m_placeIcon[0]) return m_placeIcon[0]->getOrientation();
...
void InGameUI::deselectDrawable(Drawable *draw) {
  if (draw->isSelected()) {
    ...
    DrawableListIt findIt =
        std::find(m_selectedDrawables.begin(), m_selectedDrawables.end(), draw);

    // sanity
    DEBUG_ASSERTCRASH(findIt != m_selectedDrawables.end(),
                      ("deselectDrawable: Drawable not found in the selected "
                       "drawable list '%s'\n",
...
void InGameUI::deselectAllDrawables(Bool postMsg) {
  ...
  ///@todo don't we want to not emit this message if there wasn't a group at
  ///all? (CBD)
  /** @todo also, we probably are sending this message too much, we should come
  up with some kind of "selections are dirty" status that we can check once per
  frame and send the correct group info over the network ... could be tricky tho
  (or impossible) given the order of operations of things happening in the code
  (CBD) */
  if (postMsg) {
    GameMessage *groupMsg = TheMessageStream->appendMessage(
        GameMessage::MSG_DESTROY_SELECTED_GROUP);
...
Bool InGameUI::isDrawableSelected(DrawableID idToCheck) const {
  for (DrawableListCIt it = m_selectedDrawables.begin();
       it != m_selectedDrawables.end(); ++it) {
    if ((*it)->getID() == idToCheck) return TRUE;
...
Bool InGameUI::isAnySelectedKindOf(KindOfType kindOf) const {
  ...
  for (DrawableListCIt it = m_selectedDrawables.begin();
       it != m_selectedDrawables.end(); ++it) {
    /** @todo, it seems like we might want to keep a list of drawable pointers
    so we don't have to do this lookup ... it seems "tightly coupled" to me
    (CBD) */
    // get the drawable from the ID
    draw = *it;
    if (draw && draw->isKindOf(kindOf)) return TRUE;
...
void InGameUI::setInputEnabled(Bool enable) {
  ...
  if (wasEnabled && !enable) {
    /*
            when input is disabled, clear out all the special "modes" we can be
       in, since we can miss the "exit mode" message during the cinematic. e.g.,
       hold down the ctrl key when a cinematic begins, then release it during
       the cinematic... since input is disabled, we never see the keyup and thus
       think we're still in forceattack when its done, until you jiggle that key
       again. (admittedly, this code will actually do the wrong thing if you
       were to hold down the ctrl key thru the whole cinematic, but that's even
       more unlikely...)
    */
...
void InGameUI::postDraw(void) {
  ...
  if (m_messagesOn) {
    ...
    for (i = MAX_UI_MESSAGES - 1; i >= 0; i--) {
      if (m_uiMessages[i].displayString) {
        // make drop color black, but use the alpha setting of the fill color
        // specified (for fading)
        GameGetColorComponents(m_uiMessages[i].color, &r, &g, &b, &a);
        dropColor = GameMakeColor(0, 0, 0, a);

        // draw the text
        m_uiMessages[i].displayString->draw(x, y, m_uiMessages[i].color,
                                            dropColor);
  ...
  if (m_militarySubtitle) {
    ...
    for (Int i = 0; i <= m_militarySubtitle->currentDisplayString; i++) {
      m_militarySubtitle->displayStrings[i]->draw(
          pos.x, pos.y, m_militarySubtitle->color, dropColor);
      Int height;
    ...
    if (m_militarySubtitle->blockDrawn) {
      ICoord2D size;
      size.y = m_militarySubtitle
                   ->displayStrings[m_militarySubtitle->currentDisplayString]
                   ->getFont()
                   ->height;
      size.x = size.y * 0.8f;
      TheDisplay->drawFillRect(m_militarySubtitle->blockPos.x,
                               m_militarySubtitle->blockPos.y, size.x, size.y,
                               m_militarySubtitle->color);
    }
  }
        ...
        for (SuperweaponList::iterator listIt = mapIt->second.begin();
          ...
          if (info && !info->m_hiddenByScript && !info->m_hiddenByScience) {
            ...
            if (startY >= bottomMargin) {
              ...
              info->setFont(m_superweaponReadyFont,
                            m_superweaponNormalPointSize,
                            m_superweaponNormalBold);
            ...
            if (owningObject) {
              // We don't draw our timers until we are finished with
              // construction. It is important that let the SpecialPowerUpdate
              // is add its timer in its contructor,, since the science for it
              // could be added before construction is finished, And thus the
              // timer set to READY before the timer is first drawn, here
              if (owningObject->testStatus(OBJECT_STATUS_UNDER_CONSTRUCTION))
                continue;

              SpecialPowerModuleInterface *module =
                  owningObject->getSpecialPowerModule(
                      info->getSpecialPowerTemplate());
              if (module) {
                // found one - draw it
                Bool isReady = module->isReady();
                Int readySecs;

                // IsReady includes disabledness, so if you have a 0 timer
                // disabled super, you don't want the UnsignedInt to wrap around
                // to hundreds of millions of seconds.
                if (module->getReadyFrame() < TheGameLogic->getFrame())
                  readySecs = 0;
                else
                  readySecs =
                      (module->getReadyFrame() - TheGameLogic->getFrame()) /
                      LOGICFRAMES_PER_SECOND;
                // Yes, integer math.  We can't have float imprecision display
                // 4:01 on a disabled superweapon.

                // Similarly, only checking timers is not truly indicitive of
                // readyness.
                Bool changeBolding = (readySecs != info->m_timestamp) ||
                                     (isReady != info->m_ready) ||
                                     info->m_forceUpdateText;
                if (changeBolding) {
                  if (isReady) {
                    ...
                    if (info->m_timestamp == 0) {
                      info->setFont(m_superweaponNormalFont,
                                    m_superweaponNormalPointSize,
                                    m_superweaponNormalBold);
                    }
                  }
                  ...
                  name.format(L"%ls: ",
                              TheGameText->fetch(strIndex.str()).str());
                ...
                // No matter how many specialpowermodules register their timers
                // with me, I will only draw the timer of the first valid one in
                // my list, since they all have the same template, ans they all
  ...
  if (TheGameLogic->getFrame() > 0 && m_showNamedTimers) {
    ...
    Color bgColor = GameMakeColor(0, 0, 0, 255);
    for (NamedTimerMapIt mapIt = m_namedTimers.begin();
         mapIt != m_namedTimers.end(); ++mapIt) {
      ...
      if (info) {
        ...
        if (framesLeft > 0) readyFrame += framesLeft;
        Int readySecs = (Int)(SECONDS_PER_LOGICFRAME_REAL *
                              (readyFrame - TheGameLogic->getFrame()));
        if ((info->isCountdown && readySecs != info->timestamp) ||
            (!info->isCountdown && framesLeft != info->timestamp)) {
          if (!readySecs && info->isCountdown) {
            // go bold - we're good to go
            info->displayString->setFont(
                TheFontLibrary->getFont(m_namedTimerReadyFont,
                                        TheGlobalLanguageData->adjustFontSize(
                                            m_namedTimerReadyPointSize),
                                        m_namedTimerReadyBold));
          } else {
            ...
            if (info->timestamp == 0 || info->isCountdown) {
              info->displayString->setFont(
                  TheFontLibrary->getFont(m_namedTimerNormalFont,
                                          TheGlobalLanguageData->adjustFontSize(
                                              m_namedTimerNormalPointSize),
                                          m_namedTimerNormalBold));
            }
        ...
        // draw the text
        Int drawX = startX;
        if (reverseXDir) drawX -= info->displayString->getWidth();
        if (!readySecs && info->isCountdown) {
          if (m_namedTimerFlashDuration != 0.0f) {
            if (TheGameLogic->getFrame() >=
                m_namedTimerLastFlashFrame + (Int)(m_namedTimerFlashDuration)) {
              m_namedTimerUsedFlashColor = !m_namedTimerUsedFlashColor;
              m_namedTimerLastFlashFrame = TheGameLogic->getFrame();
            }
            info->displayString->draw(drawX, startY,
  ...
  if (TheLookAtTranslator && m_drawRMBScrollAnchor) {
    ...
    if (anchor) {
      ...
      TheDisplay->drawFillRect(anchor->x - w * r - 1, anchor->y - h - 1,
                               w * 2 * r + 3, h * 2 + 3, dropColor);
      TheDisplay->drawFillRect(anchor->x - w - 1, anchor->y - h * r - 1,
                               w * 2 + 3, h * 2 * r + 3, dropColor);
      TheDisplay->drawFillRect(anchor->x - w * r, anchor->y - h, w * 2 * r + 1,
                               h * 2 + 1, mainColor);
      TheDisplay->drawFillRect(anchor->x - w, anchor->y - h * r, w * 2 + 1,
                               h * 2 * r + 1, mainColor);
    }
  }
}  // end postDraw
...
void InGameUI::expireHint(HintType type, UnsignedInt hintIndex) {
  if (type == MOVE_HINT) {
    // sanity
    if (hintIndex < 0 || hintIndex >= MAX_MOVE_HINTS) return;
...
void InGameUI::createReplayControl(void) {
  m_replayWindow =
      TheWindowManager->winCreateFromScript(AsciiString("ReplayControl.wnd"));
...
void InGameUI::stopMovie(void) {
  ...
  if (!m_currentlyPlayingMovie.isEmpty()) {
    // TheScriptEngine->notifyOfCompletedVideo(m_currentlyPlayingMovie); //
    // removing sync error source -MDC
...
// ------------------------------------------------------------------------------------------------
VideoBuffer *InGameUI::videoBuffer(void) { return m_videoBuffer; }
...
void InGameUI::playCameoMovie(const AsciiString &movieName) {
  ...
  GameWindow *window = TheWindowManager->winGetWindowFromId(
      NULL,
      TheNameKeyGenerator->nameToKey(AsciiString("ControlBar.wnd:RightHUD")));
...
void InGameUI::stopCameoMovie(void) {
  // RightHUD
  // GameWindow *window =
  // TheWindowManager->winGetWindowFromId(NULL,TheNameKeyGenerator->nameToKey(
  // AsciiString("ControlBar.wnd:CameoMovieWindow") ));
  GameWindow *window = TheWindowManager->winGetWindowFromId(
      NULL,
      TheNameKeyGenerator->nameToKey(AsciiString("ControlBar.wnd:RightHUD")));
  //	window->winHide(FALSE);
...
// InGameUI::videoBuffer
// ------------------------------------------------------------------------------------------------
VideoBuffer *InGameUI::cameoVideoBuffer(void) { return m_cameoVideoBuffer; }
...
void InGameUI::militarySubtitle(const AsciiString &label, Int duration) {
  ...
  if (title.isEmpty() || duration <= 0) {
    DEBUG_CRASH(
        ("Trying to create a military subtitle but either title is empty (%ls) "
         "or duration is <= 0 (%d)",
         title.str(), duration));
    return;
  }
  ...
  const int messageTimeout =
      currLogicFrame +
      (Int)(((Real)LOGICFRAMES_PER_SECOND * duration) / 1000.0f);

  // disable tooltips until this frame, cause we don't want to collide with the
  // military subtitles.
  TheInGameUI->disableTooltipsUntil(messageTimeout);

  // calculate where this screen position should be since the position being
  // passed in is based off 8x6
  Coord2D multiplyer;
  ...
  m_militarySubtitle->blockPos.x = m_militarySubtitle->position.x =
      m_militaryCaptionPosition.x * multiplyer.x;
  m_militarySubtitle->blockPos.y = m_militarySubtitle->position.y =
      m_militaryCaptionPosition.y * multiplyer.y;
  m_militarySubtitle->incrementOnFrame =
      currLogicFrame +
      (Int)(((Real)LOGICFRAMES_PER_SECOND * m_militaryCaptionDelayMS) /
            1000.0f);
  ...
  m_militarySubtitle->displayStrings[0] =
      TheDisplayStringManager->newDisplayString();
  m_militarySubtitle->displayStrings[0]->reset();
  m_militarySubtitle->displayStrings[0]->setFont(TheFontLibrary->getFont(
      m_militaryCaptionTitleFont,
      TheGlobalLanguageData->adjustFontSize(m_militaryCaptionTitlePointSize),
      m_militaryCaptionTitleBold));
  m_militarySubtitle->color =
      GameMakeColor(m_militaryCaptionColor.red, m_militaryCaptionColor.green,
...
void InGameUI::removeMilitarySubtitle(void) {
  ...
  for (Int i = 0; i <= m_militarySubtitle->currentDisplayString; i++) {
    TheDisplayStringManager->freeDisplayString(
        m_militarySubtitle->displayStrings[i]);
    m_militarySubtitle->displayStrings[i] = NULL;
  }
...
//------------------------------------------------------------------------------
// Checks to see if an object can interact with an object in a non-hostile
// manner. This is currently used by the selection translator to determine
// whether to do something to an object or select it instead based on the
// context of what is currently selected.
//------------------------------------------------------------------------------
Bool InGameUI::canSelectedObjectsNonAttackInteractWithObject(
    const Object *objectToInteractWith, SelectionRules rule) const {
  for (int i = 1; i < NUM_ACTIONTYPES; i++) {
    if (i != ACTIONTYPE_ATTACK_OBJECT) {
      if (canSelectedObjectsDoAction((ActionType)i, objectToInteractWith,
                                     rule)) {
        return TRUE;
      }
    }
...
CanAttackResult InGameUI::getCanSelectedObjectsAttack(
    ActionType action, const Object *objectToInteractWith, SelectionRules rule,
    Bool additionalChecking) const {
  // Setting a rally point doesn't require an object to interact with.
  if ((objectToInteractWith == NULL) !=
      (action == ACTIONTYPE_SET_RALLY_POINT)) {
    ...
    switch (action) {
      case ACTIONTYPE_ATTACK_OBJECT: {
        ...
        CanAttackResult result = TheActionManager->getCanAttackObject(
            other->getObject(), objectToInteractWith, CMD_FROM_PLAYER,
      ...
      default:
        DEBUG_CRASH(
            ("Called InGameUI::getCanSelectedObjectsAttack() with actiontype "
             "%d. Only accepts attack types! Should you be calling "
             "InGameUI::canSelectedObjectsDoAction() instead?"));
  ...
  for (DrawableListCIt it = selected->begin(); it != selected->end(); ++it) {
    ...
    switch (action) {
      case ACTIONTYPE_NONE:
        ...
        return TRUE;
      case ACTIONTYPE_GET_REPAIRED_AT:
        success = TheActionManager->canGetRepairedAt(
            other->getObject(), objectToInteractWith, CMD_FROM_PLAYER);
        break;
      case ACTIONTYPE_DOCK_AT:
        success = TheActionManager->canDockAt(
            other->getObject(), objectToInteractWith, CMD_FROM_PLAYER);
        break;
      case ACTIONTYPE_GET_HEALED_AT:
        success = TheActionManager->canGetHealedAt(
            other->getObject(), objectToInteractWith, CMD_FROM_PLAYER);
        if (success) {
          ContainModuleInterface *contain = objectToInteractWith->getContain();
          if (contain && contain->isHealContain()) {
            // This container is only used for the purposes of healing and we
            // cannot enter it normally -- this is NOT a transport!
            success = false;
          }
        }
        break;
      case ACTIONTYPE_REPAIR_OBJECT: {
        ObjectID currentRepairer =
            objectToInteractWith->getSoleHealingBenefactor();
        success =
            (TheActionManager->canRepairObject(
                 other->getObject(), objectToInteractWith, CMD_FROM_PLAYER) &&
             (currentRepairer == INVALID_ID ||
              currentRepairer == other->getObject()->getID()));
        // unless someone else is already healing it...
        // please note that this add'l test is left out of canRepairObject()
        // since canRepairObject gets called from within the
        // Dozer/WorkerAIUpdates' stateMachines as they continue the repair
        // process. This remains true.
        break;
      }
      case ACTIONTYPE_RESUME_CONSTRUCTION:
        success = TheActionManager->canResumeConstructionOf(
            other->getObject(), objectToInteractWith, CMD_FROM_PLAYER);
        break;
      case ACTIONTYPE_COMBATDROP_INTO:
        success = TheActionManager->canEnterObject(
            other->getObject(), objectToInteractWith, CMD_FROM_PLAYER,
            COMBATDROP_INTO);
        break;
      case ACTIONTYPE_ENTER_OBJECT:
        // additionalChecking is TRUE only if we want to check if transport is
        // full first.
        success = TheActionManager->canEnterObject(
            other->getObject(), objectToInteractWith, CMD_FROM_PLAYER,
            additionalChecking ? CHECK_CAPACITY : DONT_CHECK_CAPACITY);
        break;
      case ACTIONTYPE_ATTACK_OBJECT:
        DEBUG_CRASH(
            ("Called InGameUI::canSelectedObjectsDoAction() with "
             "ACTIONTYPE_ATTACK_OBJECT. You must use "
             "InGameUI::getCanSelectedObjectsAttack() instead."));
        return FALSE;
      case ACTIONTYPE_HIJACK_VEHICLE:
        success = TheActionManager->canHijackVehicle(
            other->getObject(), objectToInteractWith, CMD_FROM_PLAYER);
        break;
      case ACTIONTYPE_CONVERT_OBJECT_TO_CARBOMB:
        success = TheActionManager->canConvertObjectToCarBomb(
            other->getObject(), objectToInteractWith, CMD_FROM_PLAYER);
        break;
      case ACTIONTYPE_CAPTURE_BUILDING:
        success = TheActionManager->canCaptureBuilding(
            other->getObject(), objectToInteractWith, CMD_FROM_PLAYER);
        break;
      case ACTIONTYPE_DISABLE_VEHICLE_VIA_HACKING:
        success = TheActionManager->canDisableVehicleViaHacking(
            other->getObject(), objectToInteractWith, CMD_FROM_PLAYER);
        break;
#ifdef ALLOW_SURRENDER
      case ACTIONTYPE_PICK_UP_PRISONER:
        success = TheActionManager->canPickUpPrisoner(
            other->getObject(), objectToInteractWith, CMD_FROM_PLAYER);
        break;
#endif
      case ACTIONTYPE_STEAL_CASH_VIA_HACKING:
        success = TheActionManager->canStealCashViaHacking(
            other->getObject(), objectToInteractWith, CMD_FROM_PLAYER);
        break;
      case ACTIONTYPE_DISABLE_BUILDING_VIA_HACKING:
        success = TheActionManager->canDisableBuildingViaHacking(
            other->getObject(), objectToInteractWith, CMD_FROM_PLAYER);
        break;
      case ACTIONTYPE_MAKE_DEFECTOR:
        success = TheActionManager->canMakeObjectDefector(
            other->getObject(), objectToInteractWith, CMD_FROM_PLAYER);
        break;
      case ACTIONTYPE_SET_RALLY_POINT: {
        ...
        success = (obj->isKindOf(KINDOF_AUTO_RALLYPOINT) &&
...
//------------------------------------------------------------------------------
Bool InGameUI::canSelectedObjectsDoSpecialPower(
    const CommandButton *command, const Object *objectToInteractWith,
    const Coord3D *position, SelectionRules rule, UnsignedInt commandOptions,
  ...
  Bool doAtObject =
      BitTest(command->getOptions(), COMMAND_OPTION_NEED_OBJECT_TARGET);
  ...
  if (ignoreSelDraw) tmpList.push_back(ignoreSelDraw);

  const DrawableList *selected =
      (tmpList.size() > 0) ? &tmpList : TheInGameUI->getAllSelectedDrawables();
  ...
  for (DrawableListCIt it = selected->begin(); it != selected->end(); ++it) {
    ...
    if (!doAtObject && !doAtPosition) {
      if (TheActionManager->canDoSpecialPower(other->getObject(), spTemplate,
                                              CMD_FROM_PLAYER,
                                              commandOptions)) {
      ...
      if (TheActionManager->canDoSpecialPowerAtLocation(
              other->getObject(), position, CMD_FROM_PLAYER, spTemplate,
              objectToInteractWith, commandOptions)) {
...
Bool InGameUI::canSelectedObjectsOverrideSpecialPowerDestination(
  ...
  for (DrawableListCIt it = selected->begin(); it != selected->end(); ++it) {
    ...
    if (TheActionManager->canOverrideSpecialPowerDestination(
            other->getObject(), loc, spType, CMD_FROM_PLAYER)) {
      if (rule == SELECTION_ANY) {
        return true;
      }
      qualify++;
...
//------------------------------------------------------------------------------
Bool InGameUI::canSelectedObjectsEffectivelyUseWeapon(
    const CommandButton *command, const Object *objectToInteractWith,
    const Coord3D *position, SelectionRules rule) const {
  // Get the special power template.
  ...
  for (DrawableListCIt it = selected->begin(); it != selected->end(); ++it) {
    ...
    if (!doAtObject && !doAtPosition) {
      if (TheActionManager->canFireWeapon(other->getObject(), slot,
                                          CMD_FROM_PLAYER)) {
        // This is the no target version
        if (rule == SELECTION_ANY) {
          return true;
      ...
      if (TheActionManager->canFireWeaponAtObject(other->getObject(),
                                                  objectToInteractWith,
                                                  CMD_FROM_PLAYER, slot)) {
        // This requires a object target
        if (rule == SELECTION_ANY) {
          return true;
        }
      ...
      if (TheActionManager->canFireWeaponAtLocation(
              other->getObject(), position, CMD_FROM_PLAYER, slot,
              objectToInteractWith)) {
        // This requires a valid location.
        if (rule == SELECTION_ANY) {
          return true;
...
Int InGameUI::selectAcrossRegion(IRegion2D *region) {
  ...
  /* loop through all the selected drawables and create a set of all the
     objects, so that you only iterate once through each type of object
  */
  ...
  for (DrawableListCIt it = selected->begin(); it != selected->end(); ++it) {
    ...
    draw = *it;
    if (draw && draw->getObject() && draw->getObject()->isLocallyControlled()) {
      // Use the Object's thing template, doing so will prevent wierdness for
      // disguised vehicles.
  ...
  if (drawableList.size() == 0)
    return -1;  // nothing useful selected to begin with - don't bother
                // iterating
  ...
  for (iter = drawableList.begin(); iter != drawableList.end(); ++iter) {
    ...
    if (region)
      newSelectionCount += TheTacticalView->iterateDrawablesInRegion(
          region, similarUnitSelection, (void *)&data);
    else {
      // loop over the map
  ...
  if (newSelectionCount > 0) {
    ...
    GameMessage *teamMsg = TheMessageStream->appendMessage(
    ...
    for (DrawableListCIt it = data.newlySelectedDrawables.begin();
         it != data.newlySelectedDrawables.end(); ++it) {
      draw = *it;
...
Int InGameUI::selectAcrossMap() {
  ...
  if (numSelected == -1) {
    ...
    if (!draw || !draw->getObject() ||
        !draw->getObject()->isKindOf(KINDOF_STRUCTURE)) {
...
//-------------------------------------------------------------------------------------------------
/** Selects matching units, either on screen or across map.  When called by
   pressing 'T', their is not a way to tell if the game is supposed to select
   across the screen, or across the map.  For mouse clicks, i.e. Alt + click or
   double click, we can directly call selectAcrossScreen or selectAcrossMap */
//-------------------------------------------------------------------------------------------------
Int InGameUI::selectMatchingUnits() {
  /// When implementing this, obey TheInGameUI->getMaxSelectCount() if it is > 0
  Int numSelected = selectAcrossScreen();
  if (numSelected == -1) return numSelected;
  if (numSelected == 0) {
  ...
  for( DrawableListCIt it = selected->begin(); it != selected->end(); ++it )
  {
          ...
          draw = *it;
          if( draw && draw->getObject() &&
  draw->getObject()->isLocallyControlled() )
          {


                  //drawableList.insert(
  draw->getObject()->getTemplate()->getName() ); drawableList.insert(
  draw->getTemplate() );
          }
  }
  ...
  data.m_message = TheMessageStream->appendMessage(
  GameMessage::MSG_CREATE_SELECTED_GROUP );
  ...
  for( iter = drawableList.begin(); iter != drawableList.end(); ++iter )
  {
          ...
          m_selectedAcrossScreen = TheTacticalView->iterateDrawablesInRegion(
  &region, InGameUI::typeSelection, (void *) &data); setDisplayedMaxWarning(
  FALSE );
  }
  ...
  "GUI:SelectedAcrossScreen" ); TheInGameUI->message( message );
  ...
  GameMessage::MSG_CREATE_SELECTED_GROUP );
          ...
          Bool check = FALSE;
          for( DrawableListCIt it1 = selected->begin(); it1 != selected->end();
  ++it1 )
          {
                  draw = *it1;
          ...
          {
                  UnicodeString message = TheGameText->fetch(
  "GUI:NothingSelected" ); TheInGameUI->message( message );
                  setSelectedAcrossScreen( false );
          ...
          for( iter = drawableList.begin(); iter != drawableList.end(); ++iter )
          {
                  ...
                  while( temp )
                  {
                          ...
                          if( object && object->isLocallyControlled()
                                          && !object->isContained() &&
  temp->getTemplate()->isEquivalentTo( templateName ) )
                          {

                                  // enforce optional unit cap
                                  if ( getMaxSelectCount() > 0 &&
  TheInGameUI->getSelectCount() >= getMaxSelectCount())
                                  {
                                          if ( !getDisplayedMaxWarning() )
                                          {
                                                  ...
                                                  msg.format(TheGameText->fetch("GUI:MaxSelectionSize").str(),
  TheInGameUI->getMaxSelectCount()); message(msg);
                                          }
                                  }
                                  else
                                  {
                                          selectDrawable( temp );
                                          teamMsg->appendObjectIDArgument(
  temp->getObject()->getID() ); setDisplayedMaxWarning( FALSE );
                                  }
                          }
                          temp = temp->getNextDrawable();
                  }
 ...
 */
//-----------------------------------------------------------------------------------
void InGameUI::buildRegion(const ICoord2D *anchor, const ICoord2D *dest,
                           IRegion2D *region) {
...
//-------------------------------------------------------------------------------------------------
void InGameUI::addFloatingText(const UnicodeString &text, const Coord3D *pos,
                               Color color) {
  if (TheGameLogic->getDrawIconUI()) {
    ...
    if (m_floatingTextTimeOut <= 0)
      newFTD->m_frameTimeOut =
          TheGameLogic->getFrame() + DEFAULT_FLOATING_TEXT_TIMEOUT;
    else
...
void InGameUI::DEBUG_addFloatingText(const AsciiString &text,
                                     const Coord3D *pos, Color color) {
...
try_again:
  for (FloatingTextListIt it = m_floatingTextList.begin();
       it != m_floatingTextList.end(); ++it) {
  ...
  newFTD->m_dString->setFont(
      TheWindowManager->winFindFont(AsciiString("Arial"), POINTSIZE, FALSE));
...
void InGameUI::updateFloatingText(void) {
  ...
  UnsignedInt currLogicFrame =
      TheGameLogic->getFrame();  // the current logic frame
  UnsignedByte r, g, b,
      a;       // we'll need to break apart our color so we can modify the alpha
  Int amount;  // The amout we'll change the alpha
  static UnsignedInt lastLogicFrameUpdate =
      currLogicFrame;  // We need to make sure our current frame is different
                       // then our last frame we updated.

  // only update the position if we're incrementing frames
  if (lastLogicFrameUpdate == currLogicFrame) return;

  lastLogicFrameUpdate = currLogicFrame;

  // Loop through our floating text list
  for (FloatingTextListIt it = m_floatingTextList.begin();
       it != m_floatingTextList.end();) {
    ftd = *it;
    ...
    if (currLogicFrame > ftd->m_frameTimeOut) {
      ...
      amount = REAL_TO_INT((currLogicFrame - ftd->m_frameTimeOut) *
...
void InGameUI::drawFloatingText(void) {
  ...
  for (FloatingTextListIt it = m_floatingTextList.begin();
    ...
    Int pCX, pCY;
    ThePartitionManager->worldToCell(ftd->m_pos3D.x, ftd->m_pos3D.y, &pCX,
                                     &pCY);

    // translate it's 3d pos into a 2d screen pos
    if (TheTacticalView->worldToScreen(&ftd->m_pos3D, &pos) && ftd->m_dString &&
        ThePartitionManager->getShroudStatusForPlayer(playerNdx, pCX, pCY) ==
            CELLSHROUD_CLEAR) {
...
//-------------------------------------------------------------------------------------------------
void InGameUI::popupMessage(const AsciiString &message, Int x, Int y, Int width,
                            Bool pause, Bool pauseMusic) {
...
void InGameUI::popupMessage(const AsciiString &identifier, Int x, Int y,
                            Int width, Color textColor, Bool pause,
                            Bool pauseMusic) {
  if (m_popupMessageData) clearPopupMessageData();
  ...
  // x and why are passed in as a percentage of the screen, convert to screen
  // coords
  if (x > 100) x = 100;
  if (x < 0) x = 0;
  ...
  if (pause) TheGameLogic->setGamePaused(TRUE, pauseMusic);

  m_popupMessageData->layout =
      TheWindowManager->winCreateLayout(AsciiString("InGamePopupMessage.wnd"));
...
//-------------------------------------------------------------------------------------------------
FloatingTextData::~FloatingTextData(void) {
  if (m_dString) TheDisplayStringManager->freeDisplayString(m_dString);
  m_dString = NULL;
}
...
void InGameUI::addWorldAnimation(Anim2DTemplate *animTemplate,
  ...
  // sanity
  if (animTemplate == NULL || pos == NULL || durationInSeconds <= 0.0f) return;
  ...
  wad->m_anim = anim;
  wad->m_expireFrame =
      TheGameLogic->getFrame() + (durationInSeconds * LOGICFRAMES_PER_SECOND);
...
void InGameUI::updateAndDrawWorldAnimations(void) {
  ...
  for (WorldAnimationListIterator it = m_worldAnimationList.begin();
    ...
    if (TheGameLogic->isGamePaused() == FALSE) {
      //
      // see if it's time to expire this animation based on animation type and
      // options or the expire frame
      //
    ...
    //
    // don't bother going forward with the draw process if this location is
    // shrouded for the local player
    //
    Int playerIndex = ThePlayerList->getLocalPlayer()->getPlayerIndex();
    if (ThePartitionManager->getShroudStatusForPlayer(
            playerIndex, &wad->m_worldPos) != CELLSHROUD_CLEAR) {
      ++it;
      continue;
    ...
    if (BitTest(wad->m_options, WORLD_ANIM_FADE_ON_EXPIRE)) {
      ...
      if (framesTillExpire < FRAMES_BEFORE_EXPIRE_TO_FADE) {
        ...
        Real alpha = INT_TO_REAL(framesTillExpire) /
                     INT_TO_REAL(FRAMES_BEFORE_EXPIRE_TO_FADE);
    ...
    if (TheTacticalView->worldToScreen(&wad->m_worldPos, &screen) == TRUE) {
      ...
      Real zoomScale =
          TheTacticalView->getMaxZoom() / TheTacticalView->getZoom();
      width *= zoomScale;
      height *= zoomScale;

      // adjust the screen position to draw so the image is centered at the
      // location
      screen.x -= width / 2;
      screen.y -= height / 2;
...
Object *InGameUI::findIdleWorker(Object *obj) {
  ...
  if (m_idleWorkers[index].empty()) return NULL;
...
void InGameUI::removeIdleWorker(Object *obj, Int playerNumber) {
  ...
  if (playerNumber < 0 ||
      playerNumber >=
          MAX_PLAYER_COUNT)  // we're leaving the game, so this is all screwed
    return;

  if (m_idleWorkers[playerNumber].empty()) return;
...
void InGameUI::selectNextIdleWorker(void) {
  ...
  if (m_idleWorkers[index].empty()) {
    DEBUG_ASSERTCRASH(
        FALSE, ("InGameUI::selectNextIdleWorker We're trying to select a "
                "worker when our list is empty for player %ls",
                ThePlayerList->getLocalPlayer()->getPlayerDisplayName().str()));
    return;
  }
  ...
  if (getSelectCount() == 0 || getSelectCount() > 1) {
    ...
    if (!selectThisObject) selectThisObject = *m_idleWorkers[index].begin();
  }
  DEBUG_ASSERTCRASH(
      selectThisObject,
      ("InGameUI::selectNextIdleWorker Could not select the next IDLE worker"));
  if (selectThisObject) {
    // If our idle worker is contained by anything, we need to select the
    // container instead.
    ...
    deselectAllDrawables();
    GameMessage *teamMsg =
        TheMessageStream->appendMessage(GameMessage::MSG_CREATE_SELECTED_GROUP);

    // New group or add to group? Passed in value is true if we are creating a
    // new group.
    ...
    /*// removed becuase we're already playing a select sound... left in, just
    in case i"m wrong.
    // play the units sound
                    const AudioEventRTS *soundEvent =
    selectThisObject->getTemplate()->getVoiceSelect(); if (soundEvent)
                    {
...
void InGameUI::showIdleWorkerLayout(void) {
  if (!m_idleWorkerWin) {
    m_idleWorkerWin = TheWindowManager->winGetWindowFromId(
        NULL,
        TheNameKeyGenerator->nameToKey("ControlBar.wnd:ButtonIdleWorker"));
    DEBUG_ASSERTCRASH(m_idleWorkerWin, ("InGameUI::showIdleWorkerLayout could "
                                        "not find IdleWorker.wnd to load "));
    return;
  }
  ...
  //		GadgetButtonSetText(m_idleWorkerWin,
  //UnicodeString::TheEmptyString); 	else
  //	{
...
void InGameUI::recreateControlBar(void) {
  GameWindow *win = TheWindowManager->winGetWindowFromId(
      NULL, TheNameKeyGenerator->nameToKey(AsciiString("ControlBar.wnd")));
  if (win) win->deleteInstance();
...
void InGameUI::disableTooltipsUntil(UnsignedInt frameNum) {
  if (frameNum > m_tooltipsDisabledUntil) m_tooltipsDisabledUntil = frameNum;
}

void InGameUI::clearTooltipsDisabled() { m_tooltipsDisabledUntil = 0; }
...
WindowMsgHandledType IdleWorkerSystem(GameWindow *window, UnsignedInt msg,
  ...
  switch (msg) {
    ...
    case GBM_SELECTED: {
      ...
      static NameKeyType buttonSelectID =
          NAMEKEY("IdleWorker.wnd:ButtonSelectNextIdleWorker");
