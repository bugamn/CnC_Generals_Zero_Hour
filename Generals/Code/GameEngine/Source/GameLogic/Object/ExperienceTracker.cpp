...
Int ExperienceTracker::getExperienceValue(const Object *killer) const {
  ...
  if (killer->getRelationship(m_parent) == ALLIES) return 0;
...
void ExperienceTracker::setMinVeterancyLevel(VeterancyLevel newLevel) {
  // This does not check for IsTrainable, because this function is for explicit
  // setting, so the setter is assumed to know what they are doing.  The game
  // function of addExperiencePoints cares about Trainability.
  if (m_currentLevel < newLevel) {
    VeterancyLevel oldLevel = m_currentLevel;
    m_currentLevel = newLevel;
    m_currentExperience = m_parent->getTemplate()->getExperienceRequired(
        m_currentLevel);  // Minimum for this level
    if (m_parent) m_parent->onVeterancyLevelChanged(oldLevel, newLevel);
  }
}
...
void ExperienceTracker::setVeterancyLevel(VeterancyLevel newLevel) {
  // This does not check for IsTrainable, because this function is for explicit
  // setting, so the setter is assumed to know what they are doing.  The game
  // function of addExperiencePoints cares about Trainability, if flagged thus.
  if (m_currentLevel != newLevel) {
    VeterancyLevel oldLevel = m_currentLevel;
    m_currentLevel = newLevel;
    m_currentExperience = m_parent->getTemplate()->getExperienceRequired(
...
//-------------------------------------------------------------------------------------------------
Bool ExperienceTracker::gainExpForLevel(Int levelsToGain,
                                        Bool canScaleForBonus) {
  Int newLevel = (Int)m_currentLevel + levelsToGain;
  if (newLevel > LEVEL_LAST) newLevel = LEVEL_LAST;
  // gain what levels we can, even if we can't use 'em all
  if (newLevel > m_currentLevel) {
    Int experienceNeeded =
        m_parent->getTemplate()->getExperienceRequired(newLevel) -
        m_currentExperience;
...
Bool ExperienceTracker::canGainExpForLevel(Int levelsToGain) const {
  Int newLevel = (Int)m_currentLevel + levelsToGain;
  // return true if we can gain levels, even if we can't gain ALL the levels
  // requested
  if (newLevel > LEVEL_LAST) newLevel = LEVEL_LAST;
...
//-------------------------------------------------------------------------------------------------
void ExperienceTracker::addExperiencePoints(Int experienceGain,
                                            Bool canScaleForBonus) {
  ...
  if (!isTrainable()) return;  // safety

  VeterancyLevel oldLevel = m_currentLevel;

  Int amountToGain = experienceGain;
  if (canScaleForBonus) amountToGain *= m_experienceScalar;
  ...
  while (((levelIndex + 1) < LEVEL_COUNT) &&
         m_currentExperience >=
             m_parent->getTemplate()->getExperienceRequired(levelIndex + 1)) {
    // If there is a higher level to qualify for, and I qualify for it, advance
    // the index
    levelIndex++;
  }
...
void ExperienceTracker::setExperienceAndLevel(Int experienceIn) {
  ...
  while (((levelIndex + 1) < LEVEL_COUNT) &&
         m_currentExperience >=
             m_parent->getTemplate()->getExperienceRequired(levelIndex + 1)) {
    // If there is a level to qualify for, and I qualify for it, advance the
    // index
    levelIndex++;
  }
  ...
  if (oldLevel != m_currentLevel) {
    ...
    m_parent->onVeterancyLevelChanged(
        oldLevel, m_currentLevel);  //<<== paradox! this may be a level lost!
  }
}
...
//-----------------------------------------------------------------------------
void ExperienceTracker::loadPostProcess(void) {}  // end loadPostProcess
