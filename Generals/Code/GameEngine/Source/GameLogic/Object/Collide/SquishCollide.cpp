...
/** Do the collision */
//-------------------------------------------------------------------------------------------------
void SquishCollide::onCollide(Object *other, const Coord3D *loc,
  ...
  if (ai && ai->getGoalObject() == other) {
    // We are actually targeting the other object to do something to! Don't
    // allow them to crush us in the following special circumstances:

    // Hijacking!
    static NameKeyType key_HijackerUpdate = NAMEKEY("HijackerUpdate");
    HijackerUpdate *hijackUpdate =
        (HijackerUpdate *)self->findUpdateModule(key_HijackerUpdate);
    if (hijackUpdate) {
      return;
    }

    // TNT hunter placing a charge!
    SpecialAbilityUpdate *saUpdate =
        self->findSpecialAbilityUpdate(SPECIAL_TANKHUNTER_TNT_ATTACK);
  ...
  if (other->getCrusherLevel() > 0 &&
      other->getRelationship(getObject()) != ALLIES) {
    ...
    if (!ThePartitionManager->geomCollidesWithGeom(
            other->getPosition(), other->getGeometryInfo(),
            other->getOrientation(), self->getPosition(), myGeom,
            self->getOrientation())) {
      return;
    }
