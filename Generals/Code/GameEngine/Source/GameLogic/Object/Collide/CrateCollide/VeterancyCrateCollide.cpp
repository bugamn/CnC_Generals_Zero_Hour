...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
VeterancyCrateCollide::VeterancyCrateCollide(Thing *thing,
...
Int VeterancyCrateCollide::getLevelsToGain() const {
  const VeterancyCrateCollideModuleData *d =
      getVeterancyCrateCollideModuleData();
  if (!d || !d->m_addsOwnerVeterancy) return 1;
...
Bool VeterancyCrateCollide::isValidToExecute(const Object *other) const {
  ...
  if (!et || !et->canGainExpForLevel(levelsToGain)) return false;

  if (d->m_isPilot) {
    if (other->getControllingPlayer() != getObject()->getControllingPlayer()) {
      // This is a pilot and we are checking to make sure the pilot is entering
      // a vehicle on the same team. If it's not, then don't allow it.. this is
      // particularly the case for pilots attempting to enter civilian vehicles.
      return false;
    }

    if (other->isUsingAirborneLocomotor()) {
      // Can't upgrade a helicopter or plane, but we will think we can for a
      // moment while it is on the ground from being built.
      return false;
    }
  }
...
Bool VeterancyCrateCollide::executeCrateBehavior(Object *other) {
  ...
  AIUpdateInterface *ai =
      (AIUpdateInterface *)getObject()->getAIUpdateInterface();
  const VeterancyCrateCollideModuleData *md =
      getVeterancyCrateCollideModuleData();
  ...
  if (range == 0) {
    ...
    if (other != NULL) {
      other->getExperienceTracker()->gainExpForLevel(levelsToGain,
                                                     (!md->m_isPilot));
    }
  } else {
    ...
    ObjectIterator *iter = ThePartitionManager->iterateObjectsInRange(
        other, range, FROM_CENTER_2D, filters, ITER_FASTEST);
    MemoryPoolObjectHolder hold(iter);

    for (Object *potentialObject = iter->first(); potentialObject;
         potentialObject = iter->next()) {
      // This function will give just enough exp for the Object to gain a level,
      // if it can
      potentialObject->getExperienceTracker()->gainExpForLevel(
          levelsToGain, (!md->m_isPilot));
    }
  }

  // In order to make things easier for the designers, we are going to transfer
  // the terrorist name to the car... so the designer can control the car with
  // their scripts.
  if (md->m_isPilot) {
