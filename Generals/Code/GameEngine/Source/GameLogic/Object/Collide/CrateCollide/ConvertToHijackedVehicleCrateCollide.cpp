...
// Desc:   A crate (actually a terrorist - mobile crate) that makes the target
// vehicle switch
//				 sides, and kills its driver
//	@todo	 Needs to set the science of that vehicle (dozer) so still can
//build same stuff as always
//
///////////////////////////////////////////////////////////////////////////////////////////////////
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
Bool ConvertToHijackedVehicleCrateCollide::isValidToExecute(
  ...
  if (other->isKindOf(KINDOF_TRANSPORT)) {
    ...
    if (other->getContain() && other->getContain()->getContainCount() > 0) {
      return FALSE;  // dustin sez: do not jack vehicles that may carry hostile
                     // passengers
    }
  }
...
Bool ConvertToHijackedVehicleCrateCollide::executeCrateBehavior(Object *other) {
  ...
  // AIUpdateInterface in order to prevent an unintentional conversion simply by
  // having the terrorist walk too close to it. Assume ai is valid because
  // CrateCollide::isValidToExecute(other) checks it.
  ...
  other->setStatus(
      OBJECT_STATUS_HIJACKED);  // I claim this car in the name of the GLA
  ...
  // Just in case this target is a dozer, lets make him stop al his dozer tasks,
  // like building and repairing, So the previous owner does not benefit from
  // these tasks
  ...
  // In order to make things easier for the designers, we are going to transfer
  // the hijacker's name to the car... so the designer can control the car with
  // their scripts.
  ...
  if (targetExp && jackerExp) {
    VeterancyLevel highestLevel =
        MAX(targetExp->getVeterancyLevel(), jackerExp->getVeterancyLevel());
  ...
  // I we have made it this far, we are going to ride in this vehicle for a
  // while get the name of the hijackerupdate
  static NameKeyType key_HijackerUpdate = NAMEKEY("HijackerUpdate");
  HijackerUpdate *hijackerUpdate =
      (HijackerUpdate *)obj->findUpdateModule(key_HijackerUpdate);
  if (hijackerUpdate) {
  ...
  obj->leaveGroup();
  if (ai) {
    // By setting him to idle, we will prevent him from entering the target
    // after this gets called.
    ai->aiIdle(CMD_FROM_AI);
  }

  // This is kinda special... we will endow our new ride with our vision and
  // shroud range, since we are driving
