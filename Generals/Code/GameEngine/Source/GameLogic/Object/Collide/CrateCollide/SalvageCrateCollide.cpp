...
// Desc:   The Savlage system can give a Weaponset bonus, a level, or money.
// Salvagers create them
//					by killing marked units, and only
//WeaponSalvagers can get the WeaponSet bonus
///////////////////////////////////////////////////////////////////////////////////////////////////
...
Bool SalvageCrateCollide::isValidToExecute(const Object *other) const {
  ...
  if (!other->getTemplate()->isKindOf(KINDOF_SALVAGER)) return FALSE;
...
Bool SalvageCrateCollide::executeCrateBehavior(Object *other) {
  if (eligibleForWeaponSet(other) && testWeaponChance()) {
    ...
    // other->getTemplate()->getPerUnitSound( "VoiceSalvage" ); soundToPlay =
    // *soundToPlayPtr; soundToPlay.setObjectID( other->getID() );
    // TheAudio->addAudioEvent( &soundToPlay );
...
Bool SalvageCrateCollide::eligibleForWeaponSet(Object *other) {
  ...
  if (!other->isKindOf(KINDOF_WEAPON_SALVAGER)) return FALSE;
  if (other->testWeaponSetFlag(WEAPONSET_CRATEUPGRADE_TWO)) return FALSE;
...
Bool SalvageCrateCollide::eligibleForLevel(Object *other) {
  ...
  if (!other->getExperienceTracker()->isTrainable()) return FALSE;
...
Bool SalvageCrateCollide::testWeaponChance() {
  ...
  if (randomNumber < md->m_weaponChance) return TRUE;
...
Bool SalvageCrateCollide::testLevelChance() {
  ...
  Real randomNumber = GameLogicRandomValueReal(0, 1);
  if (randomNumber < md->m_levelChance) return TRUE;
...
void SalvageCrateCollide::doMoney(Object *other) {
  ...
  Int money;
  if (md->m_minimumMoney !=
      md->m_maximumMoney)  // Random value doesn't like to get a constant range
  ...
  if (money > 0) {
    ...
    // Display cash income floating over the crate.  Position is me, everything
    // else is them.
    ...
    Color color = other->getControllingPlayer()->getPlayerColor() |
