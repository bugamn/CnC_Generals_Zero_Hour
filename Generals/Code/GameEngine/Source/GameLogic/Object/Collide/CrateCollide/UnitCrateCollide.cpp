...
Bool UnitCrateCollide::executeCrateBehavior(Object *other) {
  ...
  ThingTemplate const *unitType = TheThingFactory->findTemplate(
      getUnitCrateCollideModuleData()->m_unitType);
  ...
  for (Int unitIndex = 0; unitIndex < unitCount; unitIndex++) {
    ...
    if (newObj) {
      ...
      /// @todo As a user of the future findLegalPositionAround, I wouldn't mind
      /// not having to specify range.  I just want a non colliding point.
      FindPositionOptions fpOptions;
      fpOptions.minRadius = 0.0f;
      fpOptions.maxRadius = 20.0f;
      ThePartitionManager->findPositionAround(&creationPoint, &fpOptions,
                                              &creationPoint);
