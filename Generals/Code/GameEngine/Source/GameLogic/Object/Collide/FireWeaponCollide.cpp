...
void FireWeaponCollideModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"CollideWeapon", INI::parseWeaponTemplate, NULL,
       offsetof(FireWeaponCollideModuleData, m_collideWeaponTemplate)},
      {"FireOnce", INI::parseBool, NULL,
       offsetof(FireWeaponCollideModuleData, m_fireOnce)},
      {"RequiredStatus", INI::parseBitString32, TheObjectStatusBitNames,
       offsetof(FireWeaponCollideModuleData, m_requiredStatus)},
      {"ForbiddenStatus", INI::parseBitString32, TheObjectStatusBitNames,
       offsetof(FireWeaponCollideModuleData, m_forbiddenStatus)},
      {0, 0, 0, 0}};
...
FireWeaponCollide::FireWeaponCollide(Thing *thing, const ModuleData *moduleData)
    : CollideModule(thing, moduleData), m_collideWeapon(NULL) {
  m_collideWeapon = TheWeaponStore->allocateNewWeapon(
      getFireWeaponCollideModuleData()->m_collideWeaponTemplate,
      PRIMARY_WEAPON);
  m_everFired = FALSE;
}
...
FireWeaponCollide::~FireWeaponCollide(void) {
  if (m_collideWeapon) m_collideWeapon->deleteInstance();
}

//-------------------------------------------------------------------------------------------------
/** The die callback. */
//-------------------------------------------------------------------------------------------------
void FireWeaponCollide::onCollide(Object *other, const Coord3D *loc,
                                  const Coord3D *normal) {
  if (other == NULL) return;  // Don't shoot the ground

  Object *me = getObject();

  // This will fire at you every frame, because multiple people could be
  // colliding and we want to hurt them all.  Another solution would be to keep
  // a Map of other->objetIDs and delays for each individually.  However, this
  // solution here is so quick and simple that it warrants the "do it eventually
  // if we need it" clause.
  if (shouldFireWeapon()) {
...
Bool FireWeaponCollide::shouldFireWeapon() {
  ...
  if ((status & d->m_requiredStatus) != d->m_requiredStatus) return FALSE;

  if ((status & d->m_forbiddenStatus) != 0) return FALSE;

  if (m_everFired && d->m_fireOnce) return FALSE;  // can only fire once ever
