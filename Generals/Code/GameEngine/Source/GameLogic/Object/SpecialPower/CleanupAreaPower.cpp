...
void CleanupAreaPowerModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"MaxMoveDistanceFromLocation", INI::parseReal, NULL,
       offsetof(CleanupAreaPowerModuleData, m_cleanupMoveRange)},
      {0, 0, 0, 0}};
...
//-------------------------------------------------------------------------------------------------
void CleanupAreaPower::doSpecialPowerAtLocation(const Coord3D *loc,
  ...
  CleanupHazardUpdate *update =
      (CleanupHazardUpdate *)obj->findUpdateModule(key_CleanupHazardUpdate);
  if (update) {
    update->setCleanupAreaParameters(loc, data->m_cleanupMoveRange);
  } else {
    // This case will only happen should the ambulance not have a cleanuphazard
    // update module.
    DEBUG_CRASH(
        ("%s is attempting to use CleanupAreaPower, but requires a "
         "CleanupHazardUpdate module to work!",
