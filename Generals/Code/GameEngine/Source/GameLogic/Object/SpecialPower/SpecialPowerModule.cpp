...
// /////////////////////////////////////////////////////////////////// Author:
// Colin Day, April 2002 Desc:	 Special power module interface
///////////////////////////////////////////////////////////////////////////////////////////////////
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
/* static */ void SpecialPowerModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  BehaviorModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"SpecialPowerTemplate", INI::parseSpecialPowerTemplate, NULL,
       offsetof(SpecialPowerModuleData, m_specialPowerTemplate)},
      {"UpdateModuleStartsAttack", INI::parseBool, NULL,
       offsetof(SpecialPowerModuleData, m_updateModuleStartsAttack)},
      {"StartsPaused", INI::parseBool, NULL,
       offsetof(SpecialPowerModuleData, m_startsPaused)},
      {"InitiateSound", INI::parseAudioEventRTS, NULL,
       offsetof(SpecialPowerModuleData, m_initiateSound)},
      {0, 0, 0, 0}};
...
SpecialPowerModule::SpecialPowerModule(Thing *thing,
  ...
  if (BitTest(getObject()->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION) ==
      FALSE) {
    // A sharedNSync special only startPowerRecharges when first scienced or
    // when executed, Since a new modue with same SPTemplates may construct at
    // any time.
  ...
  // Some Special powers need to be activated by an Upgrade, so prevent the
  // timer from going until then
  const SpecialPowerModuleData *md = (const SpecialPowerModuleData *)moduleData;
  if (md->m_startsPaused) pauseCountdown(TRUE);
  ...
  if (m_pausedCount == 0 &&
    ...
    TheInGameUI->addSuperweapon(
        getObject()->getControllingPlayer()->getPlayerIndex(), getPowerName(),
        getObject()->getID(),
...
void SpecialPowerModule::resolveSpecialPower(void) {
  ...
  // if we're pre-built, and not from a command center, and a building register
  us with the UI if(
  ...
  && TheGameLogic->getFrame() == 0 && getObject()->getControllingPlayer() &&
            getObject()->isKindOf( KINDOF_COMMANDCENTER ) == FALSE &&
                  getObject()->isKindOf( KINDOF_STRUCTURE ) )
  {
          //KM: The KINDOF_STRUCTURE check was made to prevent scripted bombers
  from registering their
...
void SpecialPowerModule::onSpecialPowerCreation(void) {
  // THIS gets called by addScience(), that is, when the General has purchased a
  // new special power, and this module is thus activated.
  ...
  if (getSpecialPowerTemplate()->isSharedNSync()) {
    ...
    if (player) {
      player->expressSpecialPowerReadyFrame(getSpecialPowerTemplate(),
                                            TheGameLogic->getFrame());
      m_availableOnFrame =
          player->getOrStartSpecialPowerReadyFrame(getSpecialPowerTemplate());
    }
  }
...
ScienceType SpecialPowerModule::getRequiredScience(void) const {
  return getSpecialPowerModuleData()
      ->m_specialPowerTemplate->getRequiredScience();
}  // end ~SpecialPowerModule

//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
const SpecialPowerTemplate *SpecialPowerModule::getSpecialPowerTemplate(
    void) const {
...
//-------------------------------------------------------------------------------------------------
Bool SpecialPowerModule::isModuleForPower(
    const SpecialPowerTemplate *specialPowerTemplate) const {
  // get the module data
  const SpecialPowerModuleData *modData = getSpecialPowerModuleData();

  //
  // if the special power template defined in the module data matches the
  // template we want to check then we are for it!
  //
...
Bool SpecialPowerModule::isReady() const {
  ...
  if (TheGlobalData->m_specialPowerUsesDelay == FALSE) return TRUE;
#endif
      ...
      if (modData->m_specialPowerTemplate->isSharedNSync())
        return (TheGameLogic->getFrame() >=
                player->getOrStartSpecialPowerReadyFrame(
                    modData->m_specialPowerTemplate));
    }
  }

  return (m_pausedCount == 0) &&
         (TheGameLogic->getFrame() >= m_availableOnFrame);
...
Real SpecialPowerModule::getPercentReady() const {
#if defined(_DEBUG) || defined(_INTERNAL)
  if (TheGlobalData->m_specialPowerUsesDelay == FALSE) return 1.0f;
#endif

  // easy case ... is ready
  if (isReady()) return 1.0f;

  if (m_pausedCount > 0) return m_pausedPercent;

  // get the module data
  const SpecialPowerModuleData *modData = getSpecialPowerModuleData();

  // sanity
  if (modData->m_specialPowerTemplate == NULL) return 0.0f;
  ...
  Real percent =
      1.0f - ((readyFrame - TheGameLogic->getFrame()) /
...
//-------------------------------------------------------------------------------------------------
/** A special power has been used ... start the recharge process by computing
 * the frame we will become fully available on in the future again */
//-------------------------------------------------------------------------------------------------
void SpecialPowerModule::startPowerRecharge() {
#if defined(_DEBUG) || defined(_INTERNAL)
  // this is a cheat ... remove this for release!
  if (TheGlobalData->m_specialPowerUsesDelay == FALSE) return;
#endif
  ...
  // Here, we make sure that general specials work as one between command
  // centers
  //  only factory type faction buildings should do this, and only with generals
  //  powers (in general) but there are no restrictions on the use of
  //  SharedNSync on specialPowerTemplates at large
  if (modData->m_specialPowerTemplate->isSharedNSync()) {
    player->resetOrStartSpecialPowerReadyFrame(modData->m_specialPowerTemplate);
  } else {
    // set the frame we will be 100% available on now
    m_availableOnFrame =
        TheGameLogic->getFrame() + getSpecialPowerTemplate()->getReloadTime();
  }
}

//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void SpecialPowerModule::initiateIntentToDoSpecialPower(
    const Object *targetObj, const Coord3D *targetPos,
    UnsignedInt commandOptions, Int locationCount) {
  Bool valid = false;
  ...
  for (BehaviorModule **u = getObject()->getBehaviorModules(); *u; ++u) {
    ...
    if (spu) {
      ...
      if (isModuleForPower(
              getSpecialPowerModuleData()->m_specialPowerTemplate)) {
        spu->initiateIntentToDoSpecialPower(
            getSpecialPowerModuleData()->m_specialPowerTemplate, targetObj,
            targetPos, commandOptions, locationCount);
        valid = true;
      }
    }
  }

  // If we depend on our update module to trigger the special power, make sure
  // we have the appropriate update module!
  if (!valid && getSpecialPowerModuleData()->m_updateModuleStartsAttack) {
    DEBUG_CRASH(
        ("Object does not contain a special power module to execute.  Did you "
         "forget to add it to the object INI?\n"));
    // DEBUG_CRASH(( "Object does not contain special power module (%s) to
    // execute.  Did you forget to add it to the object INI?\n",
...
void SpecialPowerModule::createViewObject(const Coord3D *location) {
  ...
  if (modData == NULL || powerTemplate == NULL) return;
  ...
  const ThingTemplate *viewObjectTemplate =
      TheThingFactory->findTemplate(objectName);
  if (viewObjectTemplate == NULL) return;

  Object *viewObject = TheThingFactory->newObject(
      viewObjectTemplate,
      getObject()->getControllingPlayer()->getDefaultTeam());
  ...
  DeletionUpdate *dup =
      (DeletionUpdate *)viewObject->findUpdateModule(key_DeletionUpdate);
  if (dup) {
...
void SpecialPowerModule::aboutToDoSpecialPower(const Coord3D *location) {
  ...
  SpecialPowerType type = getSpecialPowerModuleData()
                              ->m_specialPowerTemplate->getSpecialPowerType();

  // Only play the EVA sounds if this is not the local player, and the local
  // player doesn't consider the person an enemy. Kris: Actually, all players
  // need to hear these warnings.
  // Player *localPlayer = ThePlayerList->getLocalPlayer();
  // if (localPlayer != getObject()->getControllingPlayer() &&
  // localPlayer->getRelationship(getObject()->getTeam()) != ENEMIES) {
  ...
  AudioEventRTS audioEvent =
      *modData->m_specialPowerTemplate->getInitiateSound();
  audioEvent.setObjectID(getObject()->getID());
  TheAudio->addAudioEvent(&audioEvent);

  // play sound at target location if specified
  if (location) {
    AudioEventRTS soundAtLocation =
        *modData->m_specialPowerTemplate->getInitiateAtTargetSound();
    soundAtLocation.setPosition(location);
    soundAtLocation.setPlayerIndex(
        getObject()->getControllingPlayer()->getPlayerIndex());
...
//-------------------------------------------------------------------------------------------------
// By default, special powers are not triggered by it's update module -- in
// which case it triggers it and resets its timer immediately. When the update
// module triggers it, then all we do is initiate the special power, and trust
// that the update module will do the rest.
//-------------------------------------------------------------------------------------------------
void SpecialPowerModule::doSpecialPower(UnsignedInt commandOptions) {
  ...
  // This tells the update module that we want to do our special power. The
  // update modules will then start processing each frame.
  initiateIntentToDoSpecialPower(NULL, NULL, commandOptions);

  // Only trigger the special power immediately if the updatemodule doesn't
  // start the attack. An example of a case that wouldn't trigger immediately is
  // for a unit that needs to close to range before firing the special attack. A
  // case that would trigger immediately is the napalm strike. If we don't call
  // this now, it's up to the update module to do so.
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void SpecialPowerModule::doSpecialPowerAtMultipleLocations(
    const Coord3D *locations, Int locCount, UnsignedInt commandOptions) {
  if (m_pausedCount > 0 || getObject()->isDisabled()) {
    return;
  }
...
void SpecialPowerModule::pauseCountdown(Bool pause) {
  ...
  {
    if (m_pausedCount == 0) {
      // Only record this with the first pausing, otherwise upon final unpausing
      // you would get credited the time between pauses as time served.
      m_pausedOnFrame = TheGameLogic->getFrame();
      m_pausedPercent = getPercentReady();
    }

    ++m_pausedCount;
  } else if (m_pausedCount > 0)  // Else if unpausing, but only if I am in fact
                                 // paused, so multiple unpauses don't break.
  {
    --m_pausedCount;
...
UnsignedInt SpecialPowerModule::getReadyFrame(void) const {
  if (getSpecialPowerTemplate()->isSharedNSync()) {
    ...
    if (obj) {
      Player *player = getObject()->getControllingPlayer();
      if (player)
        return player->getOrStartSpecialPowerReadyFrame(
