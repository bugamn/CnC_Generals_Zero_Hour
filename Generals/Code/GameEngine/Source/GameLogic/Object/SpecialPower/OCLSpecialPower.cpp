...
enum { CREATE_ABOVE_LOCATION_HEIGHT = 300 };
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
static void parseOCLUpgradePair(INI* ini, void* /*instance*/, void* store,
                                const void* /*userData*/) {
  ...
  std::vector<OCLSpecialPowerModuleData::Upgrades>* s =
      (std::vector<OCLSpecialPowerModuleData::Upgrades>*)store;
  s->push_back(up);
}

//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
/* static */ void OCLSpecialPowerModuleData::buildFieldParse(
    MultiIniFieldParse& p) {
  SpecialPowerModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"UpgradeOCL", parseOCLUpgradePair, NULL,
       offsetof(OCLSpecialPowerModuleData, m_upgradeOCL)},
      {"OCL", INI::parseObjectCreationList, NULL,
       offsetof(OCLSpecialPowerModuleData, m_defaultOCL)},
      {"CreateLocation", INI::parseIndexList, TheOCLCreateLocTypeNames,
       offsetof(OCLSpecialPowerModuleData, m_createLoc)},
      {0, 0, 0, 0}};
...
const ObjectCreationList* OCLSpecialPower::findOCL() const {
  ...
  if (controller != NULL) {
    for (std::vector<OCLSpecialPowerModuleData::Upgrades>::const_iterator it =
             d->m_upgradeOCL.begin();
         it != d->m_upgradeOCL.end(); ++it) {
      if (controller->hasScience(it->m_science)) return it->m_ocl;
    }
  }
...
void OCLSpecialPower::doSpecialPowerAtLocation(const Coord3D* loc,
  ...
  switch (modData->m_createLoc) {
    case CREATE_AT_EDGE_NEAR_SOURCE:
      creationCoord =
          TheTerrainLogic->findClosestEdgePoint(getObject()->getPosition());
    ...
    case CREATE_AT_LOCATION:
      // this is the case where the special power stuff originates at the
    ...
    case CREATE_ABOVE_LOCATION:
      // this is the case where the special power stuff originates above the
      // location of the mouse click
      creationCoord = *loc;
      creationCoord.z += CREATE_ABOVE_LOCATION_HEIGHT;
      ObjectCreationList::create(ocl, getObject(), &creationCoord, loc);
...
void OCLSpecialPower::doSpecialPower(UnsignedInt commandOptions) {
  ...
  const ObjectCreationList* ocl = findOCL();
  ObjectCreationList::create(ocl, getObject(), &creationCoord, &creationCoord,
                             false);
}
