...
// Desc:   Contain module that acts as transport normally, but when full it
// redirects queries to the first passenger All of this redirection stuff makes
// it so that while I am normally a transport for Overlord subObjects, once I
// have a passenger, _I_ become a transport of their type. So, the answer to
// this question depends on if it is my passenger asking, or theirs. As always,
// I can't use convience functions that get redirected on a ? like this.
...
void OverlordContain::onBodyDamageStateChange(
    const DamageInfo *damageInfo, BodyDamageType oldState,
    BodyDamageType newState)  ///< state change callback
{
  // I can't use any convienience functions, as they will all get routed to the
  // bunker I may carry. I want just me. Oh, and I don't want this function
  // trying to do death.  That is more complicated and will be handled on my
  // death.
...
ContainModuleInterface *OverlordContain::getRedirectedContain() const {
  ...
  if (m_containListSize < 1) return NULL;

  if (!m_redirectionActivated)
    return NULL;  // Shut off early to allow death to happen without my bunker
                  // having
  // trouble finding me to say goodbye as messages get sucked up the pipe to
  // him.

  Object *myGuy = m_containList.front();
  if (myGuy) return myGuy->getContain();
...
void OverlordContain::onDie(const DamageInfo *damageInfo) {
  // Do you mean me the Overlord, or my behavior of passing stuff on to my
  // passengers?
  if (getRedirectedContain() == NULL) {
    TransportContain::onDie(damageInfo);
    return;
  }
  // Everything is fine if I am empty or carrying a regular guy.  If I have a
  // redirected contain
  //  set up, then I need to handle the order of death explicitly, or things
  //  will become confused when I stop redirecting in the middle of the process.
  //  Or I will get confused as my commands get sucked up the pipe.
...
void OverlordContain::onDelete(void) {
  ...
  // Without my throwing the redirect switch, teardown deletion will get
  // confused and fire off a bunch of asserts
...
void OverlordContain::onCapture(Player *oldOwner, Player *newOwner) {
  if (m_containListSize < 1) return;

  // Need to capture our specific rider.  He will then kick passengers out if he
  // is a Transport
  Object *myGuy = m_containList.front();
...
Bool OverlordContain::isGarrisonable() const {
  if (getRedirectedContain() == NULL) return FALSE;
...
void OverlordContain::removeAllContained(Bool exposeStealthUnits) {
  ...
  const ContainedItemsList *fullList =
      getRedirectedContain()->getContainedItemsList();
...
//-------------------------------------------------------------------------------------------------
void OverlordContain::iterateContained(ContainIterateFunc func, void *userData,
                                       Bool reverse) {
  // Do you mean me the Overlord, or my behavior of passing stuff on to my
  // passengers?
  if (getRedirectedContain() == NULL) {
...
//-------------------------------------------------------------------------------------------------
Bool OverlordContain::isValidContainerFor(const Object *obj,
...
UnsignedInt OverlordContain::getContainCount() const {
  ...
  // Do you mean me the Overlord, or my behavior of passing stuff on to my
  // passengers?
  if (redir == NULL) return TransportContain::getContainCount();
...
Int OverlordContain::getContainMax() const {
  // Do you mean me the Overlord, or my behavior of passing stuff on to my
  // passengers?
  if (getRedirectedContain() == NULL) return TransportContain::getContainMax();
...
Bool OverlordContain::isEnclosingContainerFor(const Object *obj) const {
  // All of this redirection stuff makes it so that while I am normally a
  // transport for Overlord subObjects, once I have a passenger, _I_ become a
  // transport of their type. So, the answer to this question depends on if it
  // is my passenger asking, or theirs. As always, I can't use convience
  // functions that get redirected on a ? like this.
  if (m_containListSize > 0 && obj == m_containList.front()) return FALSE;
...
Bool OverlordContain::isDisplayedOnControlBar() const {
  ...
  // passengers?
  if (getRedirectedContain() == NULL)
    return FALSE;  // No need to call up inheritance, this is a module based
                   // question, and I say no.
...
const Object *OverlordContain::friend_getRider() const {
  // The draw order dependency bug for riders means that our draw module needs
  // to cheat to get
  // around it.	So this is another function that knows it is getting around
  // redirection to ask
  // an Overlord specific function.

  if (m_containListSize > 0) return m_containList.front();
