...
// Desc:   The OpenContain ContainModule allows objects to be contained inside
// of other
//				 objects.  There is a set of functionality that
//will be common to 				 all container modules that provides the actual containment
//				 implementations, those implementations are
//found here
///////////////////////////////////////////////////////////////////////////////////////////////////
...
OpenContainModuleData::OpenContainModuleData(void) {
  m_containMax = CONTAIN_MAX_UNKNOWN;  // means we don't care, infinite,
                                       // unassigned, whatever
  ...
  m_doorOpenTime = 1;
  m_allowInsideKindOf.clear();
  m_allowInsideKindOf.flip();    // everything is allowed
...
/*static*/ void OpenContainModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"ContainMax", INI::parseInt, NULL,
       offsetof(OpenContainModuleData, m_containMax)},
      {"EnterSound", INI::parseAudioEventRTS, NULL,
       offsetof(OpenContainModuleData, m_enterSound)},
      {"ExitSound", INI::parseAudioEventRTS, NULL,
       offsetof(OpenContainModuleData, m_exitSound)},
      {"DamagePercentToUnits", INI::parsePercentToReal, NULL,
       offsetof(OpenContainModuleData, m_damagePercentageToUnits)},
      {"AllowInsideKindOf", KindOfMaskType::parseFromINI, NULL,
       offsetof(OpenContainModuleData, m_allowInsideKindOf)},
      {"ForbidInsideKindOf", KindOfMaskType::parseFromINI, NULL,
       offsetof(OpenContainModuleData, m_forbidInsideKindOf)},
      {"PassengersAllowedToFire", INI::parseBool, NULL,
       offsetof(OpenContainModuleData, m_passengersAllowedToFire)},
      {"PassengersInTurret", INI::parseBool, NULL,
       offsetof(OpenContainModuleData, m_passengersInTurret)},
      {"NumberOfExitPaths", INI::parseInt, NULL,
       offsetof(OpenContainModuleData, m_numberOfExitPaths)},
      {"DoorOpenTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(OpenContainModuleData, m_doorOpenTime)},
      {"AllowAlliesInside", INI::parseBool, NULL,
       offsetof(OpenContainModuleData, m_allowAlliesInside)},
      {"AllowEnemiesInside", INI::parseBool, NULL,
       offsetof(OpenContainModuleData, m_allowEnemiesInside)},
      {"AllowNeutralInside", INI::parseBool, NULL,
       offsetof(OpenContainModuleData, m_allowNeutralInside)},
      {0, 0, 0, 0}};
  p.add(dataFieldParse);
  p.add(DieMuxData::getFieldParse(),
        offsetof(OpenContainModuleData, m_dieMuxData));
}
...
void OpenContain::containReactToTransformChange() {
  // Our transform changed, which means our bones moved, and we keep people
  // positioned on bones.
  redeployOccupants();
}
...
UpdateSleepTime OpenContain::update(void) {
  ...
  if (m_doorCloseCountdown) {
    ...
    if (m_doorCloseCountdown == 0)
      getObject()->clearAndSetModelConditionState(
          MODELCONDITION_DOOR_1_OPENING, MODELCONDITION_DOOR_1_CLOSING);
  }

  if (!m_objectEnterExitInfo.empty()) pruneDeadWanters();
...
void OpenContain::addOrRemoveObjFromWorld(Object *obj, Bool add) {
  //
  // it's expensive to add and remove structures from the pathfinder, but we
  // really shouldn't be doing it anyway as it doesn't make much sense to
  // contain a structure, but we'll check for it here and print a warning
  //
  if (obj->isKindOf(KINDOF_STRUCTURE))
    DEBUG_LOG(
        ("WARNING: Containing/Removing structures like '%s' is potentially a "
         "very expensive and slow operation\n",
  ...
  if (add) {
    ...
    if (obj->getDrawable()) obj->getDrawable()->setDrawableHidden(true);
  ...
  // enclosing container (eg, b52), we need to be able to show/hide the
  // contained riders...
  if (obj->getContain()) {
    const ContainedItemsList *items =
        obj->getContain()->getContainedItemsList();
    if (items) {
...
void OpenContain::addToContain(Object *rider) {
  ...
  if (!isValidContainerFor(rider, false)) {
    ...
    DEBUG_CRASH(
        ("OpenContain::addToContain() - Object %s not valid for container %s!",
         reportObject ? reportObject->getTemplate()->getName().str() : "NULL",
         getObject()->getTemplate()->getName().str()));
  }
#endif

  // this object cannot be contained by this module if it is already contained
  // in something
  if (rider->getContainedBy() != NULL) {
    DEBUG_LOG(
        ("'%s' is trying to contain '%s', but '%s' is already contained by "
         "'%s'\n",
         getObject()->getTemplate()->getName().str(),
...
void OpenContain::removeFromContain(Object *rider, Bool exposeStealthUnits) {
  ...
  if (containedBy != getObject()) {
    DEBUG_LOG((
        "'%s' is trying to un-contain '%s', but '%s' is really contained by "
        "'%s'\n",
        getObject()->getTemplate()->getName().str(),
        rider->getTemplate()->getName().str(),
        rider->getTemplate()->getName().str(),
  ...
  ContainedItemsList::iterator it =
      std::find(m_containList.begin(), m_containList.end(), rider);
...
//-------------------------------------------------------------------------------------------------
void OpenContain::iterateContained(ContainIterateFunc func, void *userData,
                                   Bool reverse) {
  if (reverse) {
    // note that this has to be smart enough to handle items in the list being
    // deleted via the callback function.
    for (ContainedItemsList::reverse_iterator it = m_containList.rbegin();
         it != m_containList.rend();) {
      // save the rider...
      Object *rider = *it;

      // incr the iterator BEFORE calling the func (if the func removes the
      // rider, the iterator becomes invalid)
      ++it;
...
void OpenContain::removeFromContainViaIterator(ContainedItemsList::iterator it,
  ...
  /*
          #ifdef _DEBUG
                  TheInGameUI->message( UnicodeString( L"'%S(%d)' no longer
     contains '%S(%d)'" ), getObject()->getTemplate()->getName().str(),
                                                                                                          getObject()->getID(),
  ...
  */
  ...
  if (rider->isKindOf(KINDOF_STEALTH_GARRISON)) {
    ...
    if (exposeStealthUnits) {
      ...
      StealthUpdate *stealth =
          (StealthUpdate *)rider->findUpdateModule(key_StealthUpdate);
      if (stealth) {
  ...
  // trigger an onRemoving event for 'm_object' no longer containing
  // 'itemToRemove->m_object'
  DEBUG_ASSERTCRASH(getObject()->getContain() == this,
                    ("hmm, wrong container"));
  if (getObject()->getContain()) {
...
void OpenContain::scatterToNearbyPosition(Object *rider) {
  ...
  //
  // for now we will just set the position of the object that is being removed
  // from us at a random angle away from our center out some distance
  //
  ...
  // we maintain the angle of each players screen in the player structure or
  // something
  //
  ...
  pos.z =
      TheTerrainLogic->getLayerHeight(pos.x, pos.y, theContainer->getLayer());
...
Real OpenContain::getContainedItemsMass() const {
  ...
  Real mass = 0;
  for (ContainedItemsList::const_iterator it = m_containList.begin();
       it != m_containList.end(); ++it) {
    PhysicsBehavior *phys = (*it)->getPhysics();
    if (phys) mass += phys->getMass();
  }
  return mass;
}
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void OpenContain::onCollide(Object *other, const Coord3D *loc,
  ...
  if (ai->getEnterTarget() != getObject()) return;

  // last-minute change: don't allow units from multiple (different) players to
  // occupy the same unit. so eject everyone else if they aren't controlled by
  // the same player. (srj)
  for (ContainedItemsList::iterator it = m_containList.begin();
       it != m_containList.end();) {
    // save the rider...
    ...
    if (rider->getControllingPlayer() != other->getControllingPlayer()) {
      if (rider->getAI()) {
        if (rider->isKindOf(KINDOF_STEALTH_GARRISON)) {
          // aiExit is needed to walk away from the building well, but it
          // doesn't take the Unstealth flag
  ...
  if (!isValidContainerFor(other, TRUE)) return;
...
{
  // This uses my literal list, and not the gettor, because we don't want to get
  // redirected some place fancy.
  for (ContainedItemsList::iterator it = m_containList.begin();
       it != m_containList.end();) {
    Object *rider = *it;
    ++it;
...
void OpenContain::onDie(const DamageInfo *damageInfo) {
  if (!getOpenContainModuleData()->m_dieMuxData.isDieApplicable(getObject(),
                                                                damageInfo))
    return;
  ...
  // Leaving this commented out to show it can't work.  We are about to die, so
  // they will have zero chance to hit an exitState::Update.  At least we would
  // clean them up in onDelete.
...
Bool OpenContain::isValidContainerFor(const Object *obj,
  ...
  //
  // check relationship, note that this behavior is defined as the relation
  // between 'obj' and the container 'us', and not the reverse
  //
  Bool relationshipRestricted = FALSE;
  Relationship r = obj->getRelationship(us);
  switch (r) {
    case ALLIES:
      if (modData->m_allowAlliesInside == FALSE) relationshipRestricted = TRUE;
      break;

    case ENEMIES:
      if (modData->m_allowEnemiesInside == FALSE) relationshipRestricted = TRUE;
      break;

    case NEUTRAL:
      if (modData->m_allowNeutralInside == FALSE) relationshipRestricted = TRUE;
      break;

    default:
      DEBUG_CRASH(
          ("isValidContainerFor: Undefined relationship (%d) between '%s' and "
           "'%s'",
  ...
  if (relationshipRestricted == TRUE) return FALSE;
...
// ------------------------------------------------------------------------------------------------
/**
        This new call is going to replace all game callings of
   removeFromContain. Consider rFC a 'system' call, while this is a 'game' call.
   rFC will pull from the contained list and just plop the guy in the world,
   this function will then override that location with cool game stuff.

        Exit and Evacuate can fail, removeAllContain and removeFromContain
   cannot.
*/
void OpenContain::exitObjectViaDoor(Object *exitObj, ExitDoorType exitDoor) {
  ...
  if (numberExits > 0) {
    // We have ExitStart/End specified in art to use when we kick people out. If
    // >1, then we have many and we need to decide which one to use.
    ...
    if (numberExits > 1) {
      ...
      m_whichExitPath =
          (m_whichExitPath % numberExits) + 1;  // To cycle from 1 to n
    ...
    // Can fall problem - When units exit from planes, they are airborne for a
    // while as they drop. Airborne units dont' do pathfinding, and this is
    // fine. However, amphibious transports unload 3 feet off the ground.  This
    // is enough to trigger the airborne flag, and they don't pathfind.  So they
    // all unload at the same exact point, and life is ugly.  So temporarily, we
    // turn off the allowToFall flag, so they aren't considered airborne, and
    // their final destination (adjustToPossibleDestination) considers them as
    // pathfinding, so they don't stack up.  Then we turn the flag back, so if
    // they are parachuting, they can fly down
    ...
    // Per JohnA: We need to set our layer to match our transports layer, or
    // we'll try to pick a spot on the ground.
    ...
    if (ai) {
      ...
      // The units often come out at the same position, and need to ignore
      // collisions briefly as they move out.  jba.
    ...
    exitPath.push_back(endPosition);  // Do it twice, in case units stack up due
                                      // to brief flying.  jba.
...
// ------------------------------------------------------------------------------------------------
/**
        This call is used to evacuate units from a tunnel network via a
   persistent state (guard tunnel networ). The main difference between this and
   exitObjectViaDoor is that this exit doesn't change the current ai state, so
   the guard state doesn't get blown away. jba.
*/
...
Bool OpenContain::isPassengerAllowedToFire() const {
  ...
  // If we are ourselves contained, our passengers need to check with them if
  // they get past us
...
//-------------------------------------------------------------------------------------------------
/** Check to see if our internal knowledge of the artwork matches the current
 * state of the model being displayed.  When changes occur (from changing the
 * time of day or going to a new damage state, we will want to redeploy all our
 * occupants to be at new fire points that are reflected in the new artwork */
//-------------------------------------------------------------------------------------------------
void OpenContain::monitorConditionChanges(void) {
  ...
  if (draw) {
    ...
    if (currCondition != m_conditionState) stateChanged = TRUE;
  }
...
void OpenContain::redeployOccupants(void) {
  //
  // because the state has changed, we will must give the deploy logic the
  // opportunity to look for a new set of bones in the art ... so if we had them
  // flagged as not having any before (for optimization reasons) we want to
  // check at least one more time again
  //
  m_noFirePointsInArt = false;
  m_firePointStart = -1;
  m_firePointNext = 0;
  ...
  //
  // redeploy the occupants, note that we're starting at the last occupant and
  // going backwards to the start ... that reflects the order in which they
  // actually went into the building since all list building constructs at the
  // head
  //
  for (ContainedItemsList::const_reverse_iterator it =
           getContainList().rbegin();
       it != getContainList().rend(); ++it) {
...
void OpenContain::putObjAtNextFirePoint(Object *obj) {
  //
  // first, if we need to load the 3D point data from the art do so, if we've
  // already determined there is no firepoints in the current art we will avoid
  // searching for it all the time
  //
  if (m_firePointSize == 0 && m_noFirePointsInArt == false) {
    m_firePointSize = getObject()->getMultiLogicalBonePosition(
        "FIREPOINT", MAX_FIRE_POINTS, NULL, m_firePoints, TRUE);

    //
    // if there is still no firepoints in the art, we'll set a flag so that we
    // don't ever go through the art stuff again
    //
    if (m_firePointSize == 0) m_noFirePointsInArt = TRUE;
  ...
  if (getOpenContainModuleData()->m_passengersInTurret) {
    ...
    char suffix[8];
    itoa(m_firePointNext + 1, suffix,
         10);  //+1 from bone names starting at 1, not zero like my array
    ...
    getObject()->getSingleLogicalBonePositionOnTurret(
        TURRET_MAIN, firepoint.str(), NULL, &matrix);
  } else {
  ...
  m_firePointNext++;
  if (m_firePointNext >= m_firePointSize) m_firePointNext = 0;

}  // end putObjAtNextFirePoint

//-------------------------------------------------------------------------------------------------
/**
        this is used for containers that must do something to allow people to
   enter or exit... eg, land (for Chinook), open door (whatever)... it's called
   with wants=WANTS_TO_ENTER when something is in the enter state, and
   wants=WANTS_NOTHING when the unit has either entered, or given up...
*/
void OpenContain::onObjectWantsToEnterOrExit(Object *obj,
  ...
  if (wants == WANTS_NEITHER) {
    ...
    if (it != m_objectEnterExitInfo.end()) m_objectEnterExitInfo.erase(it);
  } else {
...
void OpenContain::orderAllPassengersToExit(CommandSourceType commandSource) {
  for (ContainedItemsList::const_iterator it = getContainedItemsList()->begin();
    ...
    // call it
    if (rider->getAI()) rider->getAI()->aiExit(getObject(), commandSource);
  }
}
...
void OpenContain::processDamageToContained() {
  ...
  if (items) {
    ...
    while (*it) {
      ...
      Real damage = object->getBodyModule()->getMaxHealth() *
                    getOpenContainModuleData()->m_damagePercentageToUnits;
      ...
      if (!object->isEffectivelyDead() &&
          getOpenContainModuleData()->m_damagePercentageToUnits == 1.0f)
        object->kill();  // in case we are carrying flame proof troops we have
                         // been asked to kill
    }
  }
}
...
//-------------------------------------------------------------------------------------------------
Bool OpenContain::isEnclosingContainerFor(const Object *) const { return TRUE; }
...
const Coord3D *OpenContain::getRallyPoint(void) const {
  if (m_rallyPointExists) return &m_rallyPoint;
...
void OpenContain::xfer(Xfer *xfer) {
...
#if 1
      // srj sez: yeah, it SHOULD be, but a few rogue things come into existence
      // (via their ctor) preloaded with stuff (eg, "CabooseFullOfTerrorists").
      // so just nuke 'em here. sigh...
      m_containListSize = 0;
      for (ContainedItemsList::iterator it = m_containList.begin();
           it != m_containList.end();
           /*++it*/) {
        // clear as go along, otherwise the obj will try to remove itself,
        // munging things
        Object *tmp = *it;
...
#else
      DEBUG_CRASH(
          ("OpenContain::xfer - Contain list should be empty before load but "
           "is not\n"));
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (it = m_objectEnterExitInfo.begin(); it != m_objectEnterExitInfo.end();
         ++it) {
      // object id
      objectID = (*it).first;
      xfer->xferObjectID(&objectID);
    ...
    if (m_objectEnterExitInfo.empty() == FALSE) {
      DEBUG_CRASH(
          ("OpenContain::xfer - m_objectEnterExitInfo should be empty, but is "
           "not\n"));
      throw SC_INVALID_DATA;
...
void OpenContain::loadPostProcess(void) {
  ...
  if (m_containList.empty() == FALSE) {
    DEBUG_CRASH(
        ("OpenContain::loadPostProcess - Contain list should be empty before "
         "load but is not\n"));
    throw SC_INVALID_DATA;
  ...
  for (idIt = m_xferContainIDList.begin(); idIt != m_xferContainIDList.end();
       ++idIt) {
    // find this object
    ...
    if (obj == NULL) {
      DEBUG_CRASH(
          ("OpenContain::loadPostProcess - Unable to find object to put on "
           "contain list\n"));
      throw SC_INVALID_DATA;
    ...
    if (isEnclosingContainerFor(obj)) addOrRemoveObjFromWorld(obj, FALSE);
