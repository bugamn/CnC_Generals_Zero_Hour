...
void ParachuteContainModuleData::buildFieldParse(MultiIniFieldParse& p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"PitchRateMax", INI::parseAngularVelocityReal, NULL,
       offsetof(ParachuteContainModuleData, m_pitchRateMax)},
      {"RollRateMax", INI::parseAngularVelocityReal, NULL,
       offsetof(ParachuteContainModuleData, m_rollRateMax)},
      {"LowAltitudeDamping", INI::parseReal, NULL,
       offsetof(ParachuteContainModuleData, m_lowAltitudeDamping)},
      {"ParachuteOpenDist", INI::parseReal, NULL,
       offsetof(ParachuteContainModuleData, m_paraOpenDist)},
      {"KillWhenLandingInWaterSlop", INI::parseReal, NULL,
       offsetof(ParachuteContainModuleData, m_killWhenLandingInWaterSlop)},
      {"FreeFallDamagePercent", INI::parsePercentToReal, NULL,
       offsetof(ParachuteContainModuleData, m_freeFallDamagePercent)},
      {"ParachuteOpenSound", INI::parseAudioEventRTS, NULL,
       offsetof(ParachuteContainModuleData, m_parachuteOpenSound)},
      {0, 0, 0, 0}};
...
ParachuteContain::ParachuteContain(Thing* thing, const ModuleData* moduleData)
  ...
  if (d) {
    m_pitchRate =
        GameLogicRandomValueReal(-d->m_pitchRateMax, d->m_pitchRateMax);
...
void ParachuteContain::onDrawableBoundToObject() {
  ...
  if (draw) draw->setDrawableHidden(!m_opened);
}
...
void ParachuteContain::updateBonePositions() {
  if (m_needToUpdateParaBones) {
    ...
    if (parachuteDraw) {
      if (parachuteDraw->getPristineBonePositions(
              "PARA_COG", 0, &m_paraSwayBone, NULL, 1) != 1) {
        DEBUG_CRASH(("PARA_COG not found\n"));
        m_paraSwayBone.zero();
      }

      if (parachuteDraw->getPristineBonePositions(
              "PARA_ATTCH", 0, &m_paraAttachBone, NULL, 1) != 1) {
        DEBUG_CRASH(("PARA_ATTCH not found\n"));
        m_paraAttachBone.zero();
      }
    }
    // DEBUG_LOG(("updating para bone positions
  ...
  if (m_needToUpdateRiderBones) {
    ...
    if (riderDraw) {
      if (riderDraw->getPristineBonePositions("PARA_MAN", 0, &m_riderAttachBone,
                                              NULL, 1) != 1) {
        // DEBUG_LOG(("*** No parachute-attach bone... using object
        // height!\n"));
        m_riderAttachBone.zero();
        m_riderAttachBone.z +=
            riderDraw->getDrawableGeometryInfo().getMaxHeightAbovePosition();
      }
    }

    // DEBUG_LOG(("updating rider bone positions
    // %d...\n",TheGameLogic->getFrame()));
  }
}
...
void ParachuteContain::updateOffsetsFromBones() {
  ...
  getObject()->convertBonePosToWorldPos(&m_paraSwayBone, NULL,
                                        &m_paraSwayOffset, NULL);
  m_paraSwayOffset.x -= objPos->x;
  m_paraSwayOffset.y -= objPos->y;
  m_paraSwayOffset.z -= objPos->z;

  getObject()->convertBonePosToWorldPos(&m_paraAttachBone, NULL,
                                        &m_paraAttachOffset, NULL);
  ...
  if (rider) {
    ...
    rider->convertBonePosToWorldPos(&m_riderAttachBone, NULL,
                                    &m_riderAttachOffset, NULL);
...
*/
Bool ParachuteContain::isValidContainerFor(const Object* rider,
  ...
  // by other means)
  if (transportSlotCount == 0 && !rider->isKindOf(KINDOF_INFANTRY) &&
      !rider->isKindOf(KINDOF_PARACHUTABLE))
    return false;

  // we can only "hold" one item at a time.
  if (getContainCount() > 0) return false;
...
void ParachuteContain::containReactToTransformChange() {
  // a bit of a cheese festival here... hidden is a flag, not a counter, so when
  // we are dumped from a plane, we might get drawn for a frame before our
  // update is called, meaning we should be briefly visible. put this here to
  // ensure we stay hidden appropriately.
  Drawable* draw = getObject()->getDrawable();
  if (draw) draw->setDrawableHidden(!m_opened);
}
...
UpdateSleepTime ParachuteContain::update(void) {
  ...
  if (m_startZ == NO_START_Z) {
    ...
    Real groundHeight = TheTerrainLogic->getGroundHeight(
        parachute->getPosition()->x, parachute->getPosition()->y);
  ...
  if (!m_opened) {
    ...
    if (fabs(m_startZ - parachute->getPosition()->z) >= d->m_paraOpenDist) {
      m_opened = true;
      parachute->clearAndSetModelConditionState(MODELCONDITION_FREEFALL,
                                                MODELCONDITION_PARACHUTING);
      m_needToUpdateParaBones = true;
      if (rider) {
        rider->clearAndSetModelConditionState(MODELCONDITION_FREEFALL,
                                              MODELCONDITION_PARACHUTING);
        m_needToUpdateRiderBones = true;
      ...
      // When a parachute opens, it should look for a good place to land.  This
      // could be explicitly set by a DeliverPayload, otherwise any place clear
      // is good.
      if (parachuteAI) {
  ...
  if (ai && !parachute->isEffectivelyDead()) {
    ...
    if (locomotor) {
      ...
      if (m_opened) {
        ...
        const Real PITCH_DAMPING =
            locomotor->getPitchDamping() + altitudeDamping;
        const Real ROLL_DAMPING = locomotor->getRollDamping() + altitudeDamping;
        m_pitchRate += ((-PITCH_STIFFNESS * m_pitch) +
                        (-PITCH_DAMPING * m_pitchRate));  // spring/damper
        m_rollRate += ((-ROLL_STIFFNESS * m_roll) +
                       (-ROLL_DAMPING * m_rollRate));  // spring/damper

        m_pitch += m_pitchRate;
        m_roll += m_rollRate;

        if (m_isLandingOverrideSet) {
          // Need to wait until after opening to do this, or else we are sending
          // the message to the Freefall locomotor.
  ...
  PathfindLayerEnum newLayer =
      TheTerrainLogic->getHighestLayerForDestination(paraPos);
  getObject()->setLayer(newLayer);
  if (rider) rider->setLayer(newLayer);

  // If we have lost our passenger for whatever reason, die early.  Otherwise we
  // just sit around forever.
  if (getContainCount() == 0) getObject()->kill();

  // the collide system doesn't always collide us with the ground if we fall
  // into water. so force the issue.
  Real waterZ;
...
void ParachuteContain::onContaining(Object* rider) {
  ...
  rider->clearAndSetModelConditionState(MODELCONDITION_PARACHUTING,
...
void ParachuteContain::onRemoving(Object* rider) {
  ...
  rider->clearModelConditionFlags(MAKE_MODELCONDITION_MASK2(
      MODELCONDITION_FREEFALL, MODELCONDITION_PARACHUTING));
  m_needToUpdateRiderBones = true;
  ...
  if (physics) {
    ...
    Coord3D force;
    force.zero();
    physics->applyForce(&force);  // force its physics to wake up... should be
                                  // done when DISABLED_HELD is cleared, but it
                                  // not, and scared to do it now.
  }
  ...
  if (TheTerrainLogic->isUnderwater(riderPos->x, riderPos->y, &waterZ,
                                    &terrainZ) &&
    ...
    damageInfo.in.m_damageType =
        DAMAGE_WATER;  // use this instead of UNRESISTABLE so we don't get a
                       // dusty damage effect
...
void ParachuteContain::positionContainedObjectsRelativeToContainer() {
  for (ContainedItemsList::const_iterator it = getContainList().begin();
       it != getContainList().end(); ++it) {
    positionRider(*it);
  }
}
    ...
    if (rider) {
      ...
      if (d->m_freeFallDamagePercent > 0.0f) {
        ...
        extraDamageInfo.in.m_amount =
            rider->getBodyModule()->getMaxHealth() * d->m_freeFallDamagePercent;
      ...
      if (physics) {
        ...
        Coord3D force;
        force.zero();
        physics->applyForce(&force);  // force its physics to wake up... should
                                      // be done when DISABLED_HELD is cleared,
                                      // but it not, and scared to do it now.
      }
    }
  }
...
//-------------------------------------------------------------------------------------------------
void ParachuteContain::onCollide(Object* other, const Coord3D* loc,
  ...
  if (other == NULL) {
    ...
    if (getObject()->getContainedBy() != NULL) return;
