...
// //////////////////////////////////////////////////////////////////////////
// Author: Colin Day
// Desc:   Objects that are contained inside a heal contain ... get healed!  oh
// my!
///////////////////////////////////////////////////////////////////////////////////////////////////
...
/*static*/ void HealContainModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"TimeForFullHeal", INI::parseDurationUnsignedInt, NULL,
       offsetof(HealContainModuleData, m_framesForFullHeal)},
      {0, 0, 0, 0}};
...
HealContain::HealContain(Thing *thing, const ModuleData *moduleData)
    : OpenContain(thing, moduleData) {}  // end HealContain

// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
HealContain::~HealContain(void) {}  // end ~HealContain
...
UpdateSleepTime HealContain::update(void) {
  ...
  //
  // for each of our objects, we give them a little health each frame so that
  // when the the TimeTillHealed is up, the object will exit and be fully healed
  //
  Bool doneHealing;
  Object *obj;
  ...
  while (it != getContainList().end()) {
    ...
    // increment the iterator, which allows failure to not cause an infinite
    // loop
    ++it;
    ...
    if (doneHealing == TRUE) {
      ...
      if (exitDoor != DOOR_NONE_AVAILABLE) exitObjectViaDoor(obj, exitDoor);
    }  // end if
