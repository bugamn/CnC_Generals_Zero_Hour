...
GarrisonContainModuleData::GarrisonContainModuleData(void) {
  ...
  m_doIHealObjects =
      false;  ///< if T, then I heal objects that are inside of me
  m_framesForFullHeal =
      1.0f;  ///< the number of frames something inside of me takes to heal
...
Int GarrisonContain::findClosestFreeGarrisonPointIndex(
    Int conditionIndex, const Coord3D *targetPos) {
  DEBUG_ASSERTCRASH(m_garrisonPointsInitialized,
                    ("garrisonPoints are not inited"));
...
// ------------------------------------------------------------------------------------------------
/** Given the object, return the garrison point index the object is placed at
 * ... if any */
// ------------------------------------------------------------------------------------------------
Int GarrisonContain::getObjectGarrisonPointIndex(Object *obj) {
  // sanity
  if (obj == NULL) return GARRISON_INDEX_INVALID;

  for (Int i = 0; i < MAX_GARRISON_POINTS; ++i)
    if (m_garrisonPointData[i].object == obj) return i;
...
// ------------------------------------------------------------------------------------------------
void GarrisonContain::putObjectAtGarrisonPoint(Object *obj, ObjectID targetID,
                                               Int conditionIndex,
                                               Int pointIndex) {
  DEBUG_ASSERTCRASH(m_garrisonPointsInitialized,
                    ("garrisonPoints are not inited"));
  ...
  if (obj == NULL || pointIndex < 0 || pointIndex >= MAX_GARRISON_POINTS ||
    ...
    DEBUG_CRASH(
        ("GarrisionContain::putObjectAtGarrisionPoint - Invalid arguments\n"));
    return;
  ...
  if (m_garrisonPointData[pointIndex].object != NULL) {
    DEBUG_CRASH(
        ("GarrisonContain::putObjectAtGarrisonPoint - Garrison Point '%d' is "
         "not empty\n",
         pointIndex));
    return;
  ...
  //
  // create a drawable that has a gun barrel which will show there is an object
  // at this garrison point ready to shoot
  //
  static const ThingTemplate *muzzle =
      TheThingFactory->findTemplate("GarrisonGun");
  DEBUG_ASSERTCRASH(muzzle, ("Warning, Object 'GarrisonGun' not found and is "
                             "need for Garrison gun effects\n"));
  if (muzzle) {
    ...
    if (draw) {
      ...
      if (containerDrawable)
        draw->setFullyObscuredByShroud(
            containerDrawable->getFullyObscuredByShroud());
...
// ------------------------------------------------------------------------------------------------
/** Given the current state of the structure, return the condition index we are
 * to use from the garrison point position arrays */
// ------------------------------------------------------------------------------------------------
Int GarrisonContain::findConditionIndex(void) {
  ...
  switch (bodyDamage) {
    ...
    default:
      ...
      DEBUG_CRASH(
          ("GarrisonContain::findConditionIndex - Unknown body damage type "
           "'%d'\n",
           bodyDamage));
      break;
...
//-------------------------------------------------------------------------------------------------
// The weapon system would like to perform a range check assuming the object is
// placed in the best possible available garrison position. If found, we change
// sourcePos to that position.
//-------------------------------------------------------------------------------------------------
Bool GarrisonContain::calcBestGarrisonPosition(Coord3D *sourcePos,
  ...
  // find which garrison point position array we will used based on body
  // condition
  ...
  if (placeIndex == GARRISON_INDEX_INVALID) {
    DEBUG_CRASH(
        ("GarrisonContain::calcBestGarrisonPosition - Unable to find suitable "
         "garrison point.\n"));
    return FALSE;
  }
...
//-------------------------------------------------------------------------------------------------
// The AI is entering the aim state and would like to move the unit to the best
// position, perform a range check, and if it succeeds, leave him there --
// otherwise, remove him immediately.
//-------------------------------------------------------------------------------------------------
Bool GarrisonContain::attemptBestFirePointPosition(Object *source,
  ...
  // Okay, now we have positioned the object in the best position for the
  // victim. Now check if we are able to fire on our victim.
...
Bool GarrisonContain::attemptBestFirePointPosition(Object *source,
  ...
  // Okay, now we have positioned the object in the best position for the
  // targetPos. Now check if we are able to fire on our targetPos.
...
//-------------------------------------------------------------------------------------------------
/** Place the object at the "best" garrison point position so it's on the same
 * "side" of the structure that its target is */
//-------------------------------------------------------------------------------------------------
void GarrisonContain::putObjectAtBestGarrisonPoint(Object *obj, Object *target,
                                                   const Coord3D *targetPos) {
  // sanity
  if (obj == NULL || (target == NULL && targetPos == NULL)) return;
  ...
  if (getObjectGarrisonPointIndex(obj) != GARRISON_INDEX_INVALID) return;
  ...
  DEBUG_ASSERTCRASH(placeIndex != GARRISON_INDEX_INVALID,
                    ("GarrisonContain::putObjectAtBestGarrisonPoint - Unable "
                     "to find suitable garrison point for '%s'\n",
                     obj->getTemplate()->getName().str()));

  // put it here
  putObjectAtGarrisonPoint(obj, target ? target->getID() : INVALID_ID,
                           conditionIndex, placeIndex);

}  // end putObjectAtBestGarrisonPoint

// ------------------------------------------------------------------------------------------------
/** Remove the object from the garrison point position and replace at the center
 * of the building */
// ------------------------------------------------------------------------------------------------
void GarrisonContain::removeObjectFromGarrisonPoint(Object *obj, Int index) {
  ...
  if (removeIndex < 0 || removeIndex >= MAX_GARRISON_POINTS) {
    //
    // this is not an error, if a search was ordered, we may very well not find
    // the object at a garrison point and therefore can't remove it
    //
    return;
  ...
  // set the position of the object to back to the center of the garrisoned
  // building
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
GarrisonContain::~GarrisonContain(void) {}  // end ~GarrisonContain
...
Bool GarrisonContain::isValidContainerFor(const Object *obj,
  ...
  if (getObject()->getBodyModule()->getHealth() <= 0.0f) return false;
...
// ------------------------------------------------------------------------------------------------
/** Any objects that are sitting at the garrison points which no longer have
 * targets need to be moved to the center of the building and taken off the
 * garrison point */
// ------------------------------------------------------------------------------------------------
void GarrisonContain::removeInvalidObjectsFromGarrisonPoints(void) {
  Object *obj;

  if (m_garrisonPointsInUse == 0) return;  // my, that was easy
...
// ------------------------------------------------------------------------------------------------
/** Are there any objects in the center that have now obtained targets and need
 * to move to a garrison point */
// ------------------------------------------------------------------------------------------------
void GarrisonContain::addValidObjectsToGarrisonPoints(void) {
  const ContainedItemsList &containList = getContainList();

  if (containList.empty()) return;

  for (ContainedItemsList::const_iterator it = containList.begin();
       it != containList.end(); ++it) {
    Object *obj = *it;
    AIUpdateInterface *ai = obj->getAIUpdateInterface();
    if (ai) {
      ...
      //
      // add this object to the garrison point that is closest to its target if
...
// ------------------------------------------------------------------------------------------------
/** Every frame this method is called.  It keeps any of the attacking units at
 * any of the fire points closest to their active target and shuffles them
 * around to any open garrison points that are available if they are closer.  We
 * will also track our targets position and orient any effect stuff we need to
 * (gun barrel / muzzle flash) */
// ------------------------------------------------------------------------------------------------
void GarrisonContain::trackTargets(void) {
  ...
  for (ContainedItemsList::const_iterator it = containList.begin();
    ...
    // only consider objects that are actually at garrison points for
    // re-shuffling
    ...
    if (ourIndex != GARRISON_INDEX_INVALID) {
      ...
      if (ai) {
        Object *victim = ai->getCurrentVictim();
        // even though the target position can't change in some cases, still
        // must do this code at least once.
        const Coord3D *victimPos = ai->getCurrentVictimPos();

        if (victim || victimPos) {
          if (victim) victimPos = victim->getPosition();
          const Coord3D *ourPos = obj->getPosition();

          // find the closest free (of all remaining) garrison points to our
          // target
          Int newIndex =
              findClosestFreeGarrisonPointIndex(conditionIndex, victimPos);
          ...
          if (newIndex != GARRISON_INDEX_INVALID) {
            ...
            Real newDistSq = calcDistSqr(
                *victimPos, m_garrisonPoint[conditionIndex][newIndex]);

            // if the newly chosen index is closer than our current index,
            // switch
            if (newDistSq < currentDistSq) {
              ...
              putObjectAtGarrisonPoint(obj,
                                       victim ? victim->getID() : INVALID_ID,
          ...
          //
          // we are now either at a new garrison fire point, or we have remained
          // at our existing point still tracking our target.  Orient the effect
          // drawable which shows the gun barrel and muzzle flash towards our
          // target position
          //
...
// ------------------------------------------------------------------------------------------------
/** Remove all the objects at garrison points back to the center and redeploy
 * them among the garrison points.  NOTE that we are preserving the frame in
 * which the object was put at the garrison point originally as this method is
 * used when the model condition changes which could shuffle the garrison point
 * positions but that shouldn't logically change when an object was placed at
 * the point */
// ------------------------------------------------------------------------------------------------
  ...
  for (i = 0; i < MAX_GARRISON_POINTS; ++i) {
    if (garrisonPointDataCopy[i].object) {
      ...
      if (index != GARRISON_INDEX_INVALID)
        m_garrisonPointData[index].placeFrame =
            garrisonPointDataCopy[i].placeFrame;
...
void GarrisonContain::updateEffects(void) {
  ...
  for (ContainedItemsList::const_iterator it = containList.begin();
    ...
    //
    // did the object fire last frame, if so make a muzzle flash if needed at
    // the garrison point
    //
    if (obj->getLastShotFiredFrame() == currentFrame - 1) {
      Int garrisonIndex = getObjectGarrisonPointIndex(obj);

      // only consider doing muzzle flash logic if the object is actually at a
      // garrison point
      if (garrisonIndex != GARRISON_INDEX_INVALID) {
        // set the model condition for the effect object to show the muzzle
        // flash
  ...
  for (Int i = 0; i < MAX_GARRISON_POINTS; ++i) {
    if (m_garrisonPointData[i].effect &&
        m_garrisonPointData[i].lastEffectFrame != 0 &&
        currentFrame - m_garrisonPointData[i].lastEffectFrame >
            MUZZLE_FLASH_LIFETIME) {
      // clear the model condition
      m_garrisonPointData[i].effect->clearModelConditionState(
...
// set the safe occlusion frame to way way way in the future so we never see it
// during death
#define HUGE_FRAME_IN_FUTURE (LOGICFRAMES_PER_SECOND * 1000)
      contained->setSafeOcclusionFrame(TheGameLogic->getFrame() +
                                       HUGE_FRAME_IN_FUTURE);

    }  // end if

  }  // end for, it

  // are there any objects at the garrison points who now need to go back to the
  ...
  // any units that have just fired need to have a muzzle flash display out of
  // the fire point
  updateEffects();

  //
  // given all the objects that are at the garrison points shooting at
  // something, if their target moves around the structure and closer to another
  // open garrison point we want to shuffle our object to the new closest
  // garrison point.  We'll also track the target here and set orientation for
  // any effects we need to
  //
  trackTargets();
  ...
  if (modData->m_mobileGarrison && (getObject()->isMobile() == TRUE)) {
    ...
    // sanity information
    DEBUG_ASSERTCRASH(
        getObject()->isMobile() == FALSE,
        ("GarrisonContain::update - Objects with garrison contain can be "
         "spec'd as 'mobile' in the INI. Do you really want to do this? \n"));
  }
...
void GarrisonContain::healObjects(void) {
  ...
  if (!modData->m_doIHealObjects) return;
...
void GarrisonContain::healSingleObject(Object *obj, Real framesForFullHeal) {
  ...
  if (TheGameLogic->getFrame() - obj->getContainedByFrame() >=
      framesForFullHeal) {
    ...
    //
    // given the *whole* time it would take to heal this object, lets pretend
    // that the object is at zero health ... and give it a sliver of health as
    // if it were at 0 health and would be fully healed at 'framesForFullHeal'
    //
...
// ------------------------------------------------------------------------------------------------
const Player *GarrisonContain::getApparentControllingPlayer(
    const Player *observingPlayer) const {
  const Player *myPlayer = getObject()->getControllingPlayer();

  if (m_hideGarrisonedStateFromNonallies && m_originalTeam && myPlayer &&
      observingPlayer) {
    Relationship r =
        myPlayer->getRelationship(observingPlayer->getDefaultTeam());
    if (r != ALLIES) return m_originalTeam->getControllingPlayer();
  }
  return myPlayer;
}
...
void GarrisonContain::recalcApparentControllingPlayer(void) {
  ...
  if (getObject()->getTeam() == NULL) m_originalTeam = NULL;
  ...
  if (getContainCount() > 0) {
    ...
    // Check to see if all the contained units are stealthy.  Need to set this
    // flag before the capture, since the Radar refresh in setTeam will want to
    // use it to decide our color.
    Bool detected = (rider->getStatusBits() & OBJECT_STATUS_DETECTED);
    m_hideGarrisonedStateFromNonallies =
        (!detected && (getStealthUnitsContained() == getContainCount()));
  ...
  // Only allow the garrison state to be set if the client team knows that it is
  // garrisoned.
  ...
  if (draw) {
    ...
    // now that we have an object inside us, we need to get all the garrison
    // point positions if we don't already have them.
...
void GarrisonContain::loadGarrisonPoints(void) {
  ...
  //
  // initialize all the garrison points to the center of the object, this
  // assumes that we will never move (these points are cached)
  //
  ...
  //
  // in order to get all the garrison point positions we will actually switch
  // the model condition through the garrisoned pristine, damaged, and really
  // damaged states.  This is especially important because at the time or
  // "garrisoning" a model condition may not actually switch to a garrisoned
  // state because of units that garrison things all stealthy.  By caching all
  // the bone positions once we will have access to any of the points for any
  // condition state at any time
  //
  {
    Int conditionIndex;
    Int count = 0;
    ...
    count = structure->getMultiLogicalBonePosition(
        "FIREPOINT", MAX_GARRISON_POINTS, m_garrisonPoint[conditionIndex],
        NULL);
  ...
  if (gBonesFound && modData->m_mobileGarrison &&
      (getObject()->isMobile() == TRUE)) {
    DEBUG_ASSERTCRASH(getObject()->isMobile() == FALSE,
                      ("GarrisonContain::update - You have specified this "
                       "garrisonContain as mobile,\n yet you want garrison "
                       "point placement bones... \n what are you thinking?"));
  }

}  // end loadGarrisonPoints

// ------------------------------------------------------------------------------------------------
/** Validate any exit rally point that has been chosen (if any).  If it's not
 * valid, try to find a new one */
// ------------------------------------------------------------------------------------------------
void GarrisonContain::validateRallyPoint(void) {
  ...
  if (m_rallyValid == TRUE) {
    ...
    if (ThePartitionManager->findPositionAround(&m_exitRallyPoint, &options,
                                                &result) == FALSE)
      m_rallyValid = FALSE;
  ...
  if (m_rallyValid == FALSE) {
    ...
    options.maxRadius = options.minRadius *
                        1.8f;  // arbitrary max distance away, change as needed
    options.ignoreObject = getObject();
    options.relationshipObject = getObject();
    m_rallyValid = ThePartitionManager->findPositionAround(
        getObject()->getPosition(), &options, &m_exitRallyPoint);
  }  // end if

}  // end validateRallyPoint

// ------------------------------------------------------------------------------------------------
/** Remove all contents of this container.  We will try to do so with
 * intelligent garrison logic, but if all else fails no matter, we need to get
 * all things out after this call is complete */
// ------------------------------------------------------------------------------------------------
void GarrisonContain::removeAllContained(Bool exposeStealthUnits) {
  //
  // we will call this when we are destroying the object (either normally or
  // through a game reset/exit etc) *and* also when we have received an
  // "evacuate" command from the player. We will attempt to find a spot for all
  // the contents to scatter to that is around the structure, but if such a spot
  // is not found, we'll just scatter in a random direction from the building.
  //

  // only even bother doing this if we have contents inside us just cause it's a
  // waste
  if (getContainCount() > 0) {
...
void GarrisonContain::exitObjectViaDoor(Object *exitObj,
  ...
  // We don't use the ExitPath system of the general OpenContain, we just send
  // people out.  The direction of outing has been picked by Design to be the
  // Screen Down at the default camera angle.
  ...
  // Garrison doesn't have reserveDoor or exitDelay, so if we do nothing,
  // everyone will appear on top of each other and get stuck inside each others'
  // extent (except for the first guy).  So we'll scatter the start point around
  // a little to make it better.
  startPosition = *getObject()->getPosition();
  // In the case of cliff bunkers, the units start in a cliff.  So we want to
  // adjust.
  AIUpdateInterface *ai = exitObj->getAI();
  if (ai) {
    Locomotor *loco = ai->getCurLocomotor();
    if (loco && !TheAI->pathfinder()->validMovementTerrain(LAYER_GROUND, loco,
                                                           &startPosition)) {
      // try front & back.
      ...
      if (!TheAI->pathfinder()->validMovementTerrain(LAYER_GROUND, loco,
                                                     &startPosition)) {
  ...
  ///< @todo This really should be automatically wrapped up in an activation
  ...
  if (ai) {
    TheAI->pathfinder()->adjustToPossibleDestination(
        exitObj, ai->getLocomotorSet(), &endPosition);
...
void GarrisonContain::onContaining(Object *obj) {
  ...
  //
  // the team of the building is now the same as those that have garrisoned it,
  // be sure to save our original team tho so that we can revert back to it when
  // all the occupants are gone
  //
  recalcApparentControllingPlayer();
...
void GarrisonContain::onRemoving(Object *obj) {
  ...
  // if we have nothing left inside of us then we are once again back to our
  // original team
  if (getContainCount() == 0) {
  ...
  ///@todo: we should probably not draw the unit to begin with and just have
  ///them pop out.
  obj->setSafeOcclusionFrame(TheGameLogic->getFrame() +
                             obj->getTemplate()->getOcclusionDelay());
...
void GarrisonContain::moveObjectsWithMe(void) {
  ...
  if (!modData->m_mobileGarrison) return;
...
//-------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void GarrisonContain::onBodyDamageStateChange(const DamageInfo *,
                                              BodyDamageType,
                                              BodyDamageType newState) {
  // This is an event triggered on edge, so we just need to look at the
  // newState. We know a change happened. And we don't need to check Rubble,
  // since death exiting is already handled. This is Garrison specific.
  if (newState == BODY_REALLYDAMAGED &&
      !getObject()->isKindOf(KINDOF_GARRISONABLE_UNTIL_DESTROYED)) {
    if (getContainCount() > 0) orderAllPassengersToExit(CMD_FROM_AI);
  }
}
...
void GarrisonContain::onObjectCreated() {
  GarrisonContainModuleData *self =
      (GarrisonContainModuleData *)getGarrisonContainModuleData();

  Int count = self->m_initialRoster.count;
  const ThingTemplate *rosterTemplate =
      TheThingFactory->findTemplate(self->m_initialRoster.templateName);
  Object *object = getObject();

  for (int i = 0; i < count; i++) {
    // We are creating a garrison that comes with an initial roster, so add it
    // now!
    Object *payload = TheThingFactory->newObject(
        rosterTemplate, object->getControllingPlayer()->getDefaultTeam());
    if (object->getContain() &&
        object->getContain()->isValidContainerFor(payload, true)) {
      object->getContain()->addToContain(payload);
    } else {
      DEBUG_CRASH(
          ("DeliverPayload: PutInContainer %s is full, or not valid for the "
           "payload %s!",
...
void GarrisonContain::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_LOAD) {
    if (teamID != TEAM_ID_INVALID) {
      ...
      if (m_originalTeam == NULL) {
        DEBUG_CRASH(
            ("GarrisonContain::xfer - Unable to find original team by id\n"));
  ...
  xfer->xferUser(
      m_garrisonPoint,
      sizeof(Coord3D) * MAX_GARRISON_POINT_CONDITIONS * MAX_GARRISON_POINTS);
...
void GarrisonContain::loadPostProcess(void) {
  ...
  for (Int i = 0; i < MAX_GARRISON_POINTS; i++) {
    ...
    if (m_garrisonPointData[i].objectID != INVALID_ID) {
      m_garrisonPointData[i].object =
          TheGameLogic->findObjectByID(m_garrisonPointData[i].objectID);
      if (m_garrisonPointData[i].object == NULL) {
        DEBUG_CRASH(
            ("GarrisonContain::loadPostProcess - Unable to find object for "
             "point data\n"));
        throw SC_INVALID_DATA;
    ...
    if (m_garrisonPointData[i].effectID != INVALID_ID) {
      m_garrisonPointData[i].effect =
          TheGameClient->findDrawableByID(m_garrisonPointData[i].effectID);
      if (m_garrisonPointData[i].effect == NULL) {
        DEBUG_CRASH(
            ("GarrisonContain::loadPostProcess - Unable to find effect for "
             "point data\n"));
        throw SC_INVALID_DATA;
