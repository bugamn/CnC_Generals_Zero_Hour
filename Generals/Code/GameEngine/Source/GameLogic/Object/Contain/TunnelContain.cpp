...
// stored: the Owning Player's
//					TunnelTracker.  All queries about
//capacity and contents are also redirected.
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void TunnelContain::removeFromContain(Object *obj, Bool exposeStealthUnits) {
  ...
  if (owningPlayer == NULL)
    return;  // game tear down.  We do the onRemove* stuff first because this is
             // allowed to fail but that still needs to be done
...
void TunnelContain::removeAllContained(Bool exposeStealthUnits) {
  ...
  const ContainedItemsList *fullList =
      owningPlayer->getTunnelSystem()->getContainedItemsList();
...
//-------------------------------------------------------------------------------------------------
void TunnelContain::iterateContained(ContainIterateFunc func, void *userData,
                                     Bool reverse) {
  Player *owningPlayer = getObject()->getControllingPlayer();
...
void TunnelContain::onSelling() {
  ...
  if (tunnelTracker == NULL) return;

  // We are the last tunnel, so kick everyone out.  This makes tunnels act like
  // Palace and Bunker rather than killing the occupants as if the last tunnel
  // died.
  if (tunnelTracker->friend_getTunnelCount() == 1)
    removeAllContained(FALSE);  // Can't be order to exit, as I have no time to
                                // organize their exits.
  // If they don't go right now, I will delete them in a moment

  // Unregister after the kick out, or else the unregistering will activate a
  // cavein-kill. We need to do this in case someone sells their last two
...
Bool TunnelContain::isValidContainerFor(const Object *obj,
  ...
  Player *owningPlayer = getObject()->getControllingPlayer();
  return owningPlayer->getTunnelSystem()->isValidContainerFor(obj,
                                                              checkCapacity);
}
...
void TunnelContain::onDie(const DamageInfo *damageInfo) {
  ...
  if (!getTunnelContainModuleData()->m_dieMuxData.isDieApplicable(getObject(),
                                                                  damageInfo))
    return;

  if (!m_isCurrentlyRegistered) return;  // it isn't registered as a tunnel
...
UpdateSleepTime TunnelContain::update(void) {
  ...
  if (controllingPlayer) {
    ...
    if (body) {
      ...
      if (info) {
        if (body->getLastDamageTimestamp() + LOGICFRAMES_PER_SECOND >
