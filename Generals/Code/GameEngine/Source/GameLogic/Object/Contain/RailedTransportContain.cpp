...
// /////////////////////////////////////////////////////////////// Author: Colin
// Day, August 2002 Desc: Railed Transport Contain Module
///////////////////////////////////////////////////////////////////////////////////////////////////
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
static RailedTransportDockUpdateInterface *
getRailedTransportDockUpdateInterface(Object *obj) {
  // sanity
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
RailedTransportContain::RailedTransportContain(Thing *thing,
                                               const ModuleData *moduleData)
    : TransportContain(thing, moduleData) {}  // end RailedTransportContain
...
void RailedTransportContain::onRemoving(Object *obj) {
  ...
  // once we have removed all our contents we are "open" for docking again for
  // more transportation
  if (getContainCount() == 0) {
    DockUpdateInterface *dui = getObject()->getDockUpdateInterface();
    if (dui) dui->setDockOpen(TRUE);
...
Bool RailedTransportContain::isSpecificRiderFreeToExit(Object *obj) {
  ...
  // if we are in transit we cannot exit, when we're in transit, the dock is
  // closed
  DockUpdateInterface *dui = us->getDockUpdateInterface();
  if (dui && dui->isDockOpen() == FALSE) return FALSE;

  // we can now exit, note we're not extending the base class cause *we* handle
  // it all
  return TRUE;
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void RailedTransportContain::exitObjectViaDoor(Object *newObj,
                                               ExitDoorType exitDoor) {
  RailedTransportDockUpdateInterface *rtdui =
