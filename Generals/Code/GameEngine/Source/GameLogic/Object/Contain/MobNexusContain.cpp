...
MobNexusContainModuleData::MobNexusContainModuleData() {
  ...
  //
  // by default we say that MobNexae can have infantry inside them, this will be
  // totally overwritten by any data provided from the INI entry tho
  //
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void MobNexusContainModuleData::parseInitialPayload(INI* ini, void* instance,
                                                    void* store,
                                                    const void* /*userData*/) {
...
void MobNexusContainModuleData::buildFieldParse(MultiIniFieldParse& p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"Slots", INI::parseInt, NULL,
       offsetof(MobNexusContainModuleData, m_slotCapacity)},
      {"ScatterNearbyOnExit", INI::parseBool, NULL,
       offsetof(MobNexusContainModuleData, m_scatterNearbyOnExit)},
      {"OrientLikeContainerOnExit", INI::parseBool, NULL,
       offsetof(MobNexusContainModuleData, m_orientLikeContainerOnExit)},
      {"KeepContainerVelocityOnExit", INI::parseBool, NULL,
       offsetof(MobNexusContainModuleData, m_keepContainerVelocityOnExit)},
      {"ExitBone", INI::parseAsciiString, NULL,
       offsetof(MobNexusContainModuleData, m_exitBone)},
      {"ExitPitchRate", INI::parseAngularVelocityReal, NULL,
       offsetof(MobNexusContainModuleData, m_exitPitchRate)},
      {"InitialPayload", parseInitialPayload, NULL, 0},
      {"HealthRegen%PerSec", INI::parseReal, NULL,
       offsetof(MobNexusContainModuleData, m_healthRegen)},
      {0, 0, 0, 0}};
...
*/
Bool MobNexusContain::isValidContainerFor(const Object* rider,
                                          Bool checkCapacity) const {
  // sanity
  if (!rider) return false;

  // The point of this new code is to determine when something has a negative 1
  // contain max, to look at the object inside of it to use that as the valid
  ...
  // plane, we want to check the infantry, not the parachute.
  //	const MobNexusContainModuleData *modData =
  //getMobNexusContainModuleData();

  // Check if we are a fake container, and if so, get an object inside it to see
  // what kind this object *is*.
  if (rider->getContain() &&
    ...
    // blech! This case may or may not occur... in which case, just use the
    // supplied object.
  }
  ...
  // order matters: we want to know if IT considers ME to be an ally (a reversal
  // of the usual situation)
  if (rider->getRelationship(getObject()) != ALLIES) return false;

  Int mobNexusSlotCount = rider->getTransportSlotCount();

  // if 0, this object isn't MobNexusable.
  if (mobNexusSlotCount == 0) return false;

  if (checkCapacity) {
    return (m_extraSlotsInUse + getContainCount() + mobNexusSlotCount <=
            getContainMax());
  } else {
    return true;
  }
...
void MobNexusContain::onContaining(Object* rider) {
  ...
  DEBUG_ASSERTCRASH(mobNexusSlotCount > 0,
                    ("Hmm, this object isnt MobNexusable"));
  m_extraSlotsInUse += mobNexusSlotCount - 1;
  DEBUG_ASSERTCRASH(
      m_extraSlotsInUse >= 0 &&
          m_extraSlotsInUse + getContainCount() <= getContainMax(),
      ("Hmm, bad slot count"));
  ...
  if (getContainCount() == 1) {
    ...
    if (draw) draw->setModelConditionState(MODELCONDITION_LOADED);
...
void MobNexusContain::onRemoving(Object* rider) {
  ...
  if (!d->m_exitBone.isEmpty()) {
    ...
    if (draw) {
      ...
      if (draw->getPristineBonePositions(d->m_exitBone.str(), 0, &bonePos, NULL,
                                         1) == 1) {
  ...
  if (d->m_scatterNearbyOnExit) scatterToNearbyPosition(rider);
  ...
  if (getContainCount() == 0) {
    ...
    if (draw) draw->clearModelConditionState(MODELCONDITION_LOADED);
...
void MobNexusContain::onObjectCreated() {
  MobNexusContainModuleData* self =
      (MobNexusContainModuleData*)getMobNexusContainModuleData();
  ...
  for (int i = 0; i < count; i++) {
    // We are creating a MobNexus that comes with a initial payload, so add it
    // now!
    ...
    if (object->getContain() &&
      ...
      DEBUG_CRASH(
          ("DeliverPayload: PutInContainer %s is full, or not valid for the "
           "payload %s!",
           object->getName().str(), self->m_initialPayload.name.str()));
    }
  }
}
...
UpdateSleepTime MobNexusContain::update() {
  MobNexusContainModuleData* moduleData =
      (MobNexusContainModuleData*)getModuleData();

  if (moduleData && moduleData->m_healthRegen) {
    ContainModuleInterface* contain = getObject()->getContain();
    if (contain) {
      // This MobNexus has a health regeneration value, so go through and heal
      // all inside.
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
ExitDoorType MobNexusContain::reserveDoorForExit(const ThingTemplate* objType,
                                                 Object* specificObject) {
  if (specificObject == NULL) return DOOR_1;  // I can, in general, exit people.

  // This is an override, not an extend.  I will check for game legality for
  // okaying the call to exitObjectViaDoor.
  Object* me = getObject();

  // this is present solely for some MobNexuss to override, so that they can
  // land before allowing people to exit...
  AIUpdateInterface* ai = me->getAIUpdateInterface();
  if (ai && ai->getAiFreeToExit(me) != FREE_TO_EXIT) return DOOR_NONE_AVAILABLE;

  // I can always kick people out if I am in the air, I know what I'm doing
  if (me->isUsingAirborneLocomotor()) return DOOR_1;
...
Bool MobNexusContain::tryToEvacuate(Bool exposeStealthedUnits) {
  ...
  while (it != getContainList().end()) {
    // get the object
    Object* obj = *it;
    // increment the iterator, since removal will pull this guy from the list
    // somewhere else and we might not actually kick anyone so we don't want to
    // loop forever.
    ++it;
