...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void TransportContainModuleData::parseInitialPayload(INI* ini, void* instance,
                                                     void* store,
                                                     const void* /*userData*/) {
...
void TransportContainModuleData::buildFieldParse(MultiIniFieldParse& p) {
  ...
  static const FieldParse dataFieldParse[] = {
      ...
      {"GoAggressiveOnExit", INI::parseBool, NULL,
       offsetof(TransportContainModuleData, m_goAggressiveOnExit)},
      {"ResetMoodCheckTimeOnExit", INI::parseBool, NULL,
       offsetof(TransportContainModuleData, m_resetMoodCheckTimeOnExit)},
      {"DestroyRidersWhoAreNotFreeToExit", INI::parseBool, NULL,
      ...
      {"ExitDelay", INI::parseDurationUnsignedInt, NULL,
...
Bool TransportContain::isValidContainerFor(const Object* rider,
  ...
  if (checkCapacity) {
    return (m_extraSlotsInUse + getContainCount() + transportSlotCount <=
            getContainMax());
  } else {
    return true;
  }
...
UpdateSleepTime TransportContain::update() {
  ...
  if (m_payloadCreated == FALSE) createPayload();
...
Bool TransportContain::isSpecificRiderFreeToExit(Object* specificObject) {
  if (specificObject == NULL) return TRUE;  // I can, in general, exit people.
  ...
  if (ai && ai->getAiFreeToExit(specificObject) != FREE_TO_EXIT) return FALSE;

  // I can always kick people out if I am in the air, I know what I'm doing
  if (me->isUsingAirborneLocomotor()) return TRUE;

  const Coord3D* myPosition = me->getPosition();
  if (!specificObject->getAIUpdateInterface()) return FALSE;

  const Locomotor* hisLocomotor =
      specificObject->getAIUpdateInterface()->getCurLocomotor();
  if (hisLocomotor == FALSE) return FALSE;
...
Bool TransportContain::isExitBusy()
    const  ///< Contain style exiters are getting the ability to space out
           ///< exits, so ask this before reserveDoor as a kind of no-commitment
           ///< check.
{
