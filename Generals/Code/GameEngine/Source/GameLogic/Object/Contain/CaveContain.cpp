...
// Desc:   A version of OpenContain that overrides where the passengers are
// stored: one of CaveSystem's
//					entries. Changing entry is a script or
//ini command.  All queries about capacity and 					contents are also redirected.
//They change sides like Garrison too.
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void CaveContain::removeFromContain(Object *obj, Bool exposeStealthUnits) {
  ...
  // This must come before the onRemov*, because CaveContain's version has a
  // edge-0 triggered event. If that were to go first, the number would still be
  // 1 at that time.  Noone else cares about order.
  ...
  if (getObject()->getContain()) getObject()->getContain()->onRemoving(obj);
...
//-------------------------------------------------------------------------------------------------
void CaveContain::iterateContained(ContainIterateFunc func, void *userData,
                                   Bool reverse) {
  TunnelTracker *myTracker =
      TheCaveSystem->getTunnelTrackerForCaveIndex(m_caveIndex);
...
Bool CaveContain::isValidContainerFor(const Object *obj,
...
void CaveContain::onDie(const DamageInfo *damageInfo) {
  ...
  if (!getCaveContainModuleData()->m_dieMuxData.isDieApplicable(getObject(),
                                                                damageInfo))
    return;
...
void CaveContain::onBuildComplete(void) {
  if (!shouldDoOnBuildComplete()) return;
...
void CaveContain::recalcApparentControllingPlayer(void) {
  ...
  // This is called from onContaining, so a one is the edge trigger to do
  // capture stuff
  if (getContainCount() == 1) {
    ContainedItemsList::const_iterator it = getContainedItemsList()->begin();
    Object *rider = *it;

    // This also gets called during reset from the PlayerList, so we might not
    // actually have players. Check like the hokey trick mentioned above
  ...
  if (controller) {
    if (TheGlobalData->m_timeOfDay == TIME_OF_DAY_NIGHT)
      getObject()->getDrawable()->setIndicatorColor(
          controller->getPlayerNightColor());
    else
      getObject()->getDrawable()->setIndicatorColor(
          controller->getPlayerColor());
  }
}
...
/////////////////////////////////////////////////////////////////////////////////////
void CaveContain::changeTeamOnAllConnectedCaves(Team *newTeam,
                                                Bool setOriginalTeams) {
  TunnelTracker *myTracker =
      TheCaveSystem->getTunnelTrackerForCaveIndex(m_caveIndex);
  const std::list<ObjectID> *allCaves = myTracker->getContainerList();
  for (std::list<ObjectID>::const_iterator iter = allCaves->begin();
       iter != allCaves->end(); iter++) {
    ...
    if (currentCave) {
      ...
      // triggers the change, he needs to tell everyone, so anyone can do the
      // un-change.
      CaveInterface *caveModule = findCave(currentCave);
      if (caveModule == NULL) continue;
      if (setOriginalTeams)
...
/////////////////////////////////////////////////////////////////////////////////////
void CaveContain::setOriginalTeam(Team *oldTeam) { m_originalTeam = oldTeam; }
...
void CaveContain::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_LOAD) {
    if (teamID != TEAM_ID_INVALID) {
      ...
      if (m_originalTeam == NULL) {
        DEBUG_CRASH(
            ("CaveContain::xfer - Unable to find original team by id\n"));
