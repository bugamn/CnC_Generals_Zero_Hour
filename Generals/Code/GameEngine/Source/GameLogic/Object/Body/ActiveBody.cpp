...
// ------------------------------------------------------------------------------------------------
/** Body particle systems are particle systems that are automatically created
 * and attached to an object as the damage state changes for that object.  We
 * keep a list of these so that when we transition from one state to another we
 * can kill any old particle systems that we need to before we create new ones
 */
// ------------------------------------------------------------------------------------------------
class BodyParticleSystem : public MemoryPoolObject {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(BodyParticleSystem,
                                          "BodyParticleSystem")
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
BodyParticleSystem::~BodyParticleSystem(void) {}  // end ~BodyParticleSystem
...
static BodyDamageType calcDamageState(Real health, Real maxHealth) {
  if (!TheGlobalData) return BODY_PRISTINE;
...
void ActiveBodyModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"MaxHealth", INI::parseReal, NULL,
       offsetof(ActiveBodyModuleData, m_maxHealth)},
      {"InitialHealth", INI::parseReal, NULL,
       offsetof(ActiveBodyModuleData, m_initialHealth)},
      {0, 0, 0, 0}};
...
ActiveBody::ActiveBody(Thing *thing, const ModuleData *moduleData)
    : BodyModule(thing, moduleData),
      ...
      m_backCrushed(false),
      m_lastDamageTimestamp(0xffffffff),   // So we don't think we just got
                                           // damaged on the first frame
      m_lastHealingTimestamp(0xffffffff),  // So we don't think we just got
                                           // healed on the first frame
...
void ActiveBody::setCorrectDamageState() {
  ...
  if (m_curDamageState == BODY_RUBBLE &&
    ...
    /** @todo I had to change this to a Z only version to keep it from
       disappearing from the PartitionManager for a frame.  That didn't used to
       happen.
    */
    ...
    // //Lorenzen
    // THis allows projectiles shot from infantry that are inside rubble to get
    // out of said rubble safely
...
void ActiveBody::setDamageState(BodyDamageType newState) {
  ...
  Real desiredHealth =
      m_maxHealth * ratio - 1;  // -1 because < not <= in calcState
...
void ActiveBody::validateArmorAndDamageFX() const {
  const ArmorTemplateSet *set =
      getObject()->getTemplate()->findArmorTemplateSet(m_curArmorSetFlags);
...
Real ActiveBody::estimateDamage(DamageInfoInput &damageInfo) const {
  ...
  Real amount =
      m_curArmor.adjustDamage(damageInfo.m_damageType, damageInfo.m_amount);
...
void ActiveBody::doDamageFX(const DamageInfo *damageInfo) {
  ...
  if (damageInfo->in.m_damageType == DAMAGE_UNRESISTABLE &&
      (damageInfo->in.m_deathType == DEATH_POISONED ||
       damageInfo->in.m_deathType == DEATH_POISONED_BETA)) {
    // PoisonedBehavior sends DAMAGE_UNRESISTABLE (rather that DAMAGE_POISON) to
    // avoid reinfection, but it prevents us from using the appropriate
    // DamageFX. This is pretty greasy; I should really augment DamageInfo to
    // have some "visible damage type" field, but that's too risky at this
    // point. oh well, the carnage continues. (srj)
  ...
  if (damageTypeToUse == DAMAGE_RADIATION) {
    ...
    if (obj) {
      ...
      if (draw) {
        ...
        // We allow the Tinting Class in Drawable to turn this effect off, which
        // it does every update
      }
    }
  }
  ...
  if (m_curDamageFX) {
    ...
    if (damageTypeToUse == m_lastDamageFXDone && m_nextDamageFXTime > now)
      return;
    Object *source = TheGameLogic->findObjectByID(
        damageInfo->in.m_sourceID);  // might be null, I guess
    m_lastDamageFXDone = damageTypeToUse;
    m_nextDamageFXTime =
        now + m_curDamageFX->getDamageFXThrottleTime(damageTypeToUse, source);
    m_curDamageFX->doDamageFX(damageTypeToUse,
                              damageInfo->out.m_actualDamageDealt, source,
                              getObject());
  }
}
...
void ActiveBody::attemptDamage(DamageInfo *damageInfo) {
  ...
  Real amount = m_curArmor.adjustDamage(damageInfo->in.m_damageType,
  ...
  switch (damageInfo->in.m_damageType) {
    ...
    case DAMAGE_KILLPILOT: {
      // This type of damage doesn't actually damage the unit, but it does kill
      // it's pilot, in the case of a vehicle.
      if (obj->isKindOf(KINDOF_VEHICLE)) {
        // Make it unmanned, so units can easily check the ability to "take
        // control of it"
        obj->setDisabled(DISABLED_UNMANNED);

        TheGameLogic->deselectObject(obj, PLAYERMASK_ALL, TRUE);

        // Convert it to the neutral team so it renders gray giving visual
        // representation that it is unmanned.
  ...
  if (allowModifier) {
    if (damageInfo->in.m_damageType != DAMAGE_UNRESISTABLE) {
      ...
      // defensive battle plan) And remember not to adjust unresistable damage,
      // just like the armor code can't.
...
//*****************************************************************************************
//*****************************************************************************************
// THIS CODE HAS BEEN DISABLED FOR THE MULTIPLAYER PLAY
// TEST!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!**
//*****************************************************************************************
//		// if we were "killed" by surrender damage...
//		if (damageInfo->in.m_damageType == DAMAGE_SURRENDER &&
//m_currentHealth <= 0.0f && obj->isKindOf(KINDOF_CAN_SURRENDER))
//		{
...
//*****************************************************************************************
    ...
    // m_lastDamageTimestamp is initialized to FFFFFFFFFF, so doing a < compare
    // is problematic. jba.
    if (m_lastDamageTimestamp != TheGameLogic->getFrame() &&
        m_lastDamageTimestamp != TheGameLogic->getFrame() - 1) {
      m_lastDamageInfo = *damageInfo;
      m_lastDamageCleared = false;
      m_lastDamageTimestamp = TheGameLogic->getFrame();
    } else {
      // Multiple damages applied in one/next frame.  We prefer the one that
      // tells who the attacker is.
      Object *srcObj1 =
          TheGameLogic->findObjectByID(m_lastDamageInfo.in.m_sourceID);
      Object *srcObj2 = TheGameLogic->findObjectByID(damageInfo->in.m_sourceID);
      if (srcObj2) {
        if (srcObj1) {
          if (srcObj2->isKindOf(KINDOF_VEHICLE) ||
              srcObj2->isKindOf(KINDOF_INFANTRY) ||
    ...
    if (m_curDamageState != oldState) {
      ...
      if (m_curDamageState == BODY_DAMAGED) {
        ...
        AudioEventRTS reallyDamaged =
            *obj->getTemplate()->getSoundOnReallyDamaged();
...
void ActiveBody::attemptHealing(DamageInfo *damageInfo) {
  ...
  if (obj->isKindOf(KINDOF_BRIDGE) == FALSE &&
      obj->isKindOf(KINDOF_BRIDGE_TOWER) == FALSE && obj->isEffectivelyDead())
    return;
...
void ActiveBody::setMaxHealth(Real maxHealth,
                              MaxHealthChangeType healthChangeType) {
  ...
  switch (healthChangeType) {
    ...
    case ADD_CURRENT_HEALTH_TOO: {
      ...
      // This could kill you if max health is reduced (if we ever have that
      // ability to add buffer health like in D&D) 400/500 (80%) + 100 becomes
      // 500/600 (83%) 200/500 (40%) - 100 becomes 100/400 (25%)
  ...
  // value is now outside of the max health range we will set it back down to
  // the new cap.  Note that we are *NOT* going through any healing or damage
  // methods here and are doing a direct set
  //
...
// ------------------------------------------------------------------------------------------------
/** Given the current damage state of the object, evaluate the visual model
 * conditions that have a visual impact on the object */
// ------------------------------------------------------------------------------------------------
void ActiveBody::evaluateVisualCondition() {
  ...
  //
  // destroy any particle systems that were attached to our body for the old
  // state and create new particle systems for the new state
  //
  updateBodyParticleSystems();
}

// ------------------------------------------------------------------------------------------------
/** Create up to maxSystems particle systems of type particleSystemName and
 * attach to bones specified by the bone base name.  If there are more bones
 * than maxSystems then the bones will be randomly selected */
// ------------------------------------------------------------------------------------------------
void ActiveBody::createParticleSystems(
    const AsciiString &boneBaseName,
    const ParticleSystemTemplate *systemTemplate, Int maxSystems) {
  Object *us = getObject();

  // sanity
  if (systemTemplate == NULL) return;

  // get the bones
  enum { MAX_BONES = 16 };
  Coord3D bonePositions[MAX_BONES];
  Int numBones = us->getMultiLogicalBonePosition(boneBaseName.str(), MAX_BONES,
                                                 bonePositions, NULL, FALSE);

  // if no bones found nothing else to do
  if (numBones == 0) return;

  //
  // if we don't have enough bones to go up to maxSystems, we will change
  // maxSystems to be the number of bones we actually have (we don't want
  // systems doubling up on bones)
  //
  if (numBones < maxSystems) maxSystems = numBones;

  //
  // create an array that we'll use to mark which bone positions have already
  // been used, this is necessary when we have more bones than particle systems
  // we're going to create, in which case we place the particle systems at
  // random bone locations but don't want to repeat any
  //
  ...
  for (Int i = 0; i < maxSystems; ++i) {
    ...
    // MDC: moving to GameLogicRandomValue.  This does not need to be synced,
    // but having it so makes searches *so* much nicer.
    // Int boneIndex = GameLogicRandomValue( 0, maxSystems - i - 1 );

    // DTEH: Moved back to GameClientRandomValue because of desync problems.
    // July 27th 2003.
    ...
    for (Int j = 0; j < numBones; j++) {
      ...
      if (usedBoneIndices[j] == TRUE) continue;
      ...
      if (count == boneIndex) {
        ...
        // we won't use this index, increment count until we find a suitable
        // index to use
        ++count;
    ...
    DEBUG_ASSERTCRASH(j != numBones,
                      ("ActiveBody::createParticleSystems, Unable to select "
    ...
    if (particleSystem) {
      ...
      // create a new body particle system entry and keep this particle system
      // in it
...
void ActiveBody::deleteAllParticleSystems(void) {
  ...
  while (m_particleSystems) {
    ...
    particleSystem = TheParticleSystemManager->findParticleSystem(
...
void ActiveBody::updateBodyParticleSystems(void) {
  static const ParticleSystemTemplate *fireSmallTemplate =
      TheParticleSystemManager->findTemplate(
          TheGlobalData->m_autoFireParticleSmallSystem);
  static const ParticleSystemTemplate *fireMediumTemplate =
      TheParticleSystemManager->findTemplate(
          TheGlobalData->m_autoFireParticleMediumSystem);
  static const ParticleSystemTemplate *fireLargeTemplate =
      TheParticleSystemManager->findTemplate(
          TheGlobalData->m_autoFireParticleLargeSystem);
  static const ParticleSystemTemplate *smokeSmallTemplate =
      TheParticleSystemManager->findTemplate(
          TheGlobalData->m_autoSmokeParticleSmallSystem);
  static const ParticleSystemTemplate *smokeMediumTemplate =
      TheParticleSystemManager->findTemplate(
          TheGlobalData->m_autoSmokeParticleMediumSystem);
  static const ParticleSystemTemplate *smokeLargeTemplate =
      TheParticleSystemManager->findTemplate(
          TheGlobalData->m_autoSmokeParticleLargeSystem);
  static const ParticleSystemTemplate *aflameTemplate =
      TheParticleSystemManager->findTemplate(
          TheGlobalData->m_autoAflameParticleSystem);
  Int countModifier;
  ...
  //
  // when we're aflame, we use a slightly different set of particle systems that
  // are auto created that lends itself to more fire and bigger fire
  //
  ...
  //
  // remove any particle systems we have currently in the list in favor of any
  // new ones that we're going to autopopulate ourselves with
  //
  deleteAllParticleSystems();
  ...
  createParticleSystems(
      TheGlobalData->m_autoFireParticleSmallPrefix, fireSmall,
      TheGlobalData->m_autoFireParticleSmallMax * countModifier);

  // medium fire bones
  createParticleSystems(
      TheGlobalData->m_autoFireParticleMediumPrefix, fireMedium,
      TheGlobalData->m_autoFireParticleMediumMax * countModifier);

  // large fire bones
  createParticleSystems(
      TheGlobalData->m_autoFireParticleLargePrefix, fireLarge,
      TheGlobalData->m_autoFireParticleLargeMax * countModifier);

  // small smoke bones
  createParticleSystems(
      TheGlobalData->m_autoSmokeParticleSmallPrefix, smokeSmall,
      TheGlobalData->m_autoSmokeParticleSmallMax * countModifier);

  // medium smoke bones
  createParticleSystems(
      TheGlobalData->m_autoSmokeParticleMediumPrefix, smokeMedium,
      TheGlobalData->m_autoSmokeParticleMediumMax * countModifier);

  // large smoke bones
  createParticleSystems(
      TheGlobalData->m_autoSmokeParticleLargePrefix, smokeLarge,
      TheGlobalData->m_autoSmokeParticleLargeMax * countModifier);

  // actively on fire
  if (getObject()->testStatus(OBJECT_STATUS_AFLAME))
    createParticleSystems(
        TheGlobalData->m_autoAflameParticlePrefix, aflameTemplate,
        TheGlobalData->m_autoAflameParticleMax * countModifier);
 ...
 * No game logic should go in here.  This is the low level math and flag
 * maintenance. Game stuff goes in attemptDamage and attemptHealing.
 */
//-------------------------------------------------------------------------------------------------
void ActiveBody::internalChangeHealth(Real delta) {
  ...
  if (m_currentHealth < lowEndCap) m_currentHealth = lowEndCap;
  ...
  if (m_curDamageState != oldState) {
    //
    // show a visual change in the model for the damage state, we do not show
    // visual changes for damage states when things are under construction
    // because we just don't have all the art states for that during buildup
    // animation
    //
    if (BitTest(getObject()->getStatusBits(),
                OBJECT_STATUS_UNDER_CONSTRUCTION) == FALSE)
      evaluateVisualCondition();

  }  // end if

  // mark the bit according to our health. (if our AI is dead but our health
  // improves, it will still re-flag this bit in the AIDeadState every frame.)
  getObject()->setEffectivelyDead(m_currentHealth <= 0);
}

//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
Real ActiveBody::getHealth() const { return m_currentHealth; }

//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
BodyDamageType ActiveBody::getDamageState() const { return m_curDamageState; }
...
void ActiveBody::setIndestructible(Bool indestructible) {
  ...
  if (us->isKindOf(KINDOF_BRIDGE)) {
    BridgeBehaviorInterface *bbi =
        BridgeBehavior::getBridgeBehaviorInterfaceFromObject(us);
    if (bbi) {
      Object *tower;
      ...
      for (Int i = 0; i < BRIDGE_MAX_TOWERS; ++i) {
        ...
        if (tower) {
          ...
          if (body) body->setIndestructible(indestructible);
...
void ActiveBody::onVeterancyLevelChanged(VeterancyLevel oldLevel,
  ...
  if (oldLevel < newLevel) {
    ...
    switch (newLevel) {
      case LEVEL_VETERAN:
        veterancyChanged =
            *getObject()->getTemplate()->getSoundPromotedVeteran();
        break;
      case LEVEL_ELITE:
...
void ActiveBody::setAflame(Bool) {
  //
  // All this does now is act like a major body state change. It is called after
  // Aflame has been set or cleared as an Object Status
  //
  updateBodyParticleSystems();
}
...
void ActiveBody::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    if (m_particleSystems != NULL) {
      DEBUG_CRASH(
          ("ActiveBody::xfer - m_particleSystems should be empty, but is "
           "not\n"));
    ...
    for (UnsignedShort i = 0; i < particleSystemCount; ++i) {
      ...
      newEntry->m_next =
          m_particleSystems;  // the list will be reversed, but we don't care
      m_particleSystems = newEntry;
