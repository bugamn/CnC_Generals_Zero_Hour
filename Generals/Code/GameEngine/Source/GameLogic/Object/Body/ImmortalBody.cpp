...
void ImmortalBody::internalChangeHealth(Real delta) {
  ...
  // extend functionality, but I go first because I can't let you die and then
  // fix it, I must prevent
  ActiveBody::internalChangeHealth(delta);

  // nothing -- never mark it as dead.
  DEBUG_ASSERTCRASH((getHealth() > 0 && !getObject()->isEffectivelyDead()),
