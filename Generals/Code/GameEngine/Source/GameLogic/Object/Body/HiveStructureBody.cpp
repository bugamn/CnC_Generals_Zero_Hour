...
// ////////////////////////////////////////////////////////////////////////
// Desc:	 Hive structure bodies are structure bodies with the ability to
// propagate specified
//         damage types to slaves when available. If there are no slaves, the
//         the structure will take the damage.
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void HiveStructureBody::attemptDamage(DamageInfo *damageInfo) {
  ...
  if (getDamageTypeFlag(data->m_damageTypesToPropagateToSlaves,
                        damageInfo->in.m_damageType)) {
    // We have the right type of damage types incoming to propagate to slaves.
    // Do we have slaves?
    ...
    if (spawnInterface) {
      ...
      if (shooter) {
        ...
        if (slave) {
          ...
          return;
        } else if (getDamageTypeFlag(data->m_damageTypesToSwallow,
                                     damageInfo->in.m_damageType)) {
      ...
      DEBUG_CRASH(
          ("%s has a HiveStructureBody module, which requires a SpawnBehavior "
           "module. Thus it is unable to propagate damage to slaves.",
           hive->getTemplate()->getName().str()));
    }
  }
