...
static const char* TheLocomotorPriorityNames[] = {"MOVES_BACK", "MOVES_MIDDLE",
                                                  "MOVES_FRONT",
...
//-------------------------------------------------------------------------------------------------
static Real calcSlowDownDist(Real curSpeed, Real desiredSpeed,
                             Real maxBraking) {
...
static Real tryToRotateVector3D(
    Real maxAngle,  // if negative, it's a percent (0...1) of the dist to rotate
                    // 'em
    const Vector3& inCurDir, const Vector3& inGoalDir, Vector3& actualDir) {
...
//-------------------------------------------------------------------------------------------------
static Real tryToOrientInThisDirection3D(Object* obj, Real maxTurnRate,
                                         const Vector3& desiredDir) {
  Vector3 actualDir;
  Real relAngle = tryToRotateVector3D(maxTurnRate,
                                      obj->getTransformMatrix()->Get_X_Vector(),
                                      desiredDir, actualDir);
  if (relAngle != 0.0f) {
    Vector3 objPos(obj->getPosition()->x, obj->getPosition()->y,
                   obj->getPosition()->z);
...
//-------------------------------------------------------------------------------------------------
inline Real tryToOrientInThisDirection3D(Object* obj, Real maxTurnRate,
                                         const Coord3D* dir) {
  return tryToOrientInThisDirection3D(obj, maxTurnRate,
                                      Vector3(dir->x, dir->y, dir->z));
}

//-----------------------------------------------------------------------------
static void calcDirectionToApplyThrust(const Object* obj,
                                       const PhysicsBehavior* physics,
                                       const Coord3D& ingoalPos, Real maxAccel,
                                       Vector3& goalDir) {
  /*
          our meta-goal here is to calculate the direction we should apply our
     motive force in order to minimize the angle between (our velocity) and
     (direction towards goalpos).

          this is complicated by the fact that we generally have an intrinsic
     velocity already, that must be accounted for, and by the fact that we can
     only apply force in our forward-x-direction (with a thrust-angle-range),
     and (due to limited range) might not be able to apply the force in the
     optimal direction!
  */
  ...
  if (isNearlyZero(vecToGoal.Length2())) {
    // goal pos is essentially same as current pos, so just stay the same &
    // return
  ...
  */
  Vector3 curVel(physics->getVelocity()->x, physics->getVelocity()->y,
                 physics->getVelocity()->z);
...
LocomotorTemplate::LocomotorTemplate() {
  ...
  //	m_pitchDamping = 0;
  //	m_rollDamping = 0;
  // it's highly unlikely you want zero for the defaults for stiffness and
  // damping... (srj) for stiffness: stiffness of the "springs" in the
  // suspension 0 = no stiffness, 1 = totally stiff (huh huh, he said "stiff")
...
void LocomotorTemplate::validate() {
  ...
  // DEBUG_ASSERTCRASH(m_lift == 0.0f || m_lift >
  // fabs(TheGlobalData->m_gravity), ("Lift is too low to counteract
  // gravity!")); DEBUG_ASSERTCRASH(m_liftDamaged == 0.0f || m_liftDamaged >
  // fabs(TheGlobalData->m_gravity), ("LiftDamaged is too low to counteract
  // gravity!")); DEBUG_ASSERTCRASH(m_preferredHeight == 0.0f || (m_behaviorZ ==
  // Z_SURFACE_RELATIVE_HEIGHT || m_behaviorZ == Z_ABSOLUTE_HEIGHT ||
  // m_appearance == LOCO_THRUST),
  //	("You must use Z_SURFACE_RELATIVE_HEIGHT or Z_ABSOLUTE_HEIGHT (or
  //THRUST) to use preferredHeight"));

  // for 'damaged' stuff that was omitted, set 'em to be the same as
  // 'undamaged'...
  if (m_maxSpeedDamaged < 0.0f) m_maxSpeedDamaged = m_maxSpeed;

  if (m_maxTurnRateDamaged < 0.0f) m_maxTurnRateDamaged = m_maxTurnRate;

  if (m_accelerationDamaged < 0.0f) m_accelerationDamaged = m_acceleration;

  if (m_liftDamaged < 0.0f) m_liftDamaged = m_lift;

  if (m_appearance == LOCO_WINGS) {
    if (m_minSpeed <= 0.0f) {
      DEBUG_CRASH(
          ("WINGS should always have positive minSpeeds (otherwise, they "
           "hover)"));
      m_minSpeed = 0.01f;
    }
  ...
  if (m_appearance == LOCO_THRUST) {
    if (m_behaviorZ != Z_NO_Z_MOTIVE_FORCE || m_lift != 0.0f ||
        m_liftDamaged != 0.0f) {
      DEBUG_CRASH(("THRUST locos may not use ZAxisBehavior or lift!\n"));
      throw INI_INVALID_DATA;
    }
    if (m_maxSpeed <= 0.0f) {
      // if one of these was omitted, it defaults to zero... just quietly heal
      // it here, rather than crashing
    ...
    if (m_maxSpeedDamaged <= 0.0f) {
      ...
      // it here, rather than crashing
      DEBUG_LOG(
          ("THRUST locos may not have zero m_maxSpeedDamaged; healing...\n"));
...
//-------------------------------------------------------------------------------------------------
static void parseFrictionPerSec(INI* ini, void* /*instance*/, void* store,
                                const void* /*userData*/) {
...
const FieldParse* LocomotorTemplate::getFieldParse() const {
  static const FieldParse TheFieldParse[] = {
      {"Surfaces", INI::parseBitString32, TheLocomotorSurfaceTypeNames,
       offsetof(LocomotorTemplate, m_surfaces)},
      {"Speed", INI::parseVelocityReal, NULL,
       offsetof(LocomotorTemplate, m_maxSpeed)},
      {"SpeedDamaged", INI::parseVelocityReal, NULL,
       offsetof(LocomotorTemplate, m_maxSpeedDamaged)},
      {"TurnRate", INI::parseAngularVelocityReal, NULL,
       offsetof(LocomotorTemplate, m_maxTurnRate)},
      {"TurnRateDamaged", INI::parseAngularVelocityReal, NULL,
       offsetof(LocomotorTemplate, m_maxTurnRateDamaged)},
      {"Acceleration", INI::parseAccelerationReal, NULL,
       offsetof(LocomotorTemplate, m_acceleration)},
      {"AccelerationDamaged", INI::parseAccelerationReal, NULL,
       offsetof(LocomotorTemplate, m_accelerationDamaged)},
      {"Lift", INI::parseAccelerationReal, NULL,
       offsetof(LocomotorTemplate, m_lift)},
      {"LiftDamaged", INI::parseAccelerationReal, NULL,
       offsetof(LocomotorTemplate, m_liftDamaged)},
      {"Braking", INI::parseAccelerationReal, NULL,
       offsetof(LocomotorTemplate, m_braking)},
      {"MinSpeed", INI::parseVelocityReal, NULL,
       offsetof(LocomotorTemplate, m_minSpeed)},
      {"MinTurnSpeed", INI::parseVelocityReal, NULL,
       offsetof(LocomotorTemplate, m_minTurnSpeed)},
      {"PreferredHeight", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_preferredHeight)},
      {"PreferredHeightDamping", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_preferredHeightDamping)},
      {"CirclingRadius", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_circlingRadius)},
      {"Extra2DFriction", parseFrictionPerSec, NULL,
       offsetof(LocomotorTemplate, m_extra2DFriction)},
      {"SpeedLimitZ", INI::parseVelocityReal, NULL,
       offsetof(LocomotorTemplate, m_speedLimitZ)},
      {"MaxThrustAngle", INI::parseAngleReal, NULL,
       offsetof(LocomotorTemplate,
                m_maxThrustAngle)},  // yes, angle, not angular-vel
      {"ZAxisBehavior", INI::parseIndexList, TheLocomotorBehaviorZNames,
       offsetof(LocomotorTemplate, m_behaviorZ)},
      {"Appearance", INI::parseIndexList, TheLocomotorAppearanceNames,
       offsetof(LocomotorTemplate, m_appearance)},
      {"GroupMovementPriority", INI::parseIndexList, TheLocomotorPriorityNames,
       offsetof(LocomotorTemplate, m_movePriority)},

      {"AccelerationPitchLimit", INI::parseAngleReal, NULL,
       offsetof(LocomotorTemplate, m_accelPitchLimit)},
      {"BounceAmount", INI::parseAngularVelocityReal, NULL,
       offsetof(LocomotorTemplate, m_bounceKick)},
      {"PitchStiffness", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_pitchStiffness)},
      {"RollStiffness", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_rollStiffness)},
      {"PitchDamping", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_pitchDamping)},
      {"RollDamping", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_rollDamping)},
      {"ThrustRoll", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_thrustRoll)},
      {"ThrustWobbleRate", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_wobbleRate)},
      {"ThrustMinWobble", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_minWobble)},
      {"ThrustMaxWobble", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_maxWobble)},
      {"PitchInDirectionOfZVelFactor", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_pitchByZVelCoef)},
      {"ForwardVelocityPitchFactor", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_forwardVelCoef)},
      {"LateralVelocityRollFactor", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_lateralVelCoef)},
      {"ForwardAccelerationPitchFactor", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_forwardAccelCoef)},
      {"LateralAccelerationRollFactor", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_lateralAccelCoef)},
      {"UniformAxialDamping", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_uniformAxialDamping)},
      {"TurnPivotOffset", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_turnPivotOffset)},
      {"Apply2DFrictionWhenAirborne", INI::parseBool, NULL,
       offsetof(LocomotorTemplate, m_apply2DFrictionWhenAirborne)},
      {"DownhillOnly", INI::parseBool, NULL,
       offsetof(LocomotorTemplate, m_downhillOnly)},
      {"AllowAirborneMotiveForce", INI::parseBool, NULL,
       offsetof(LocomotorTemplate, m_allowMotiveForceWhileAirborne)},
      {"LocomotorWorksWhenDead", INI::parseBool, NULL,
       offsetof(LocomotorTemplate, m_locomotorWorksWhenDead)},
      {"AirborneTargetingHeight", INI::parseInt, NULL,
       offsetof(LocomotorTemplate, m_airborneTargetingHeight)},
      {"StickToGround", INI::parseBool, NULL,
       offsetof(LocomotorTemplate, m_stickToGround)},
      {"CanMoveBackwards", INI::parseBool, NULL,
       offsetof(LocomotorTemplate, m_canMoveBackward)},
      {"HasSuspension", INI::parseBool, NULL,
       offsetof(LocomotorTemplate, m_hasSuspension)},
      {"FrontWheelTurnAngle", INI::parseAngleReal, NULL,
       offsetof(LocomotorTemplate, m_wheelTurnAngle)},
      {"MaximumWheelExtension", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_maximumWheelExtension)},
      {"MaximumWheelCompression", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_maximumWheelCompression)},
      {"CloseEnoughDist", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_closeEnoughDist)},
      {"CloseEnoughDist3D", INI::parseBool, NULL,
       offsetof(LocomotorTemplate, m_isCloseEnoughDist3D)},
      {"SlideIntoPlaceTime", INI::parseDurationReal, NULL,
       offsetof(LocomotorTemplate, m_ultraAccurateSlideIntoPlaceFactor)},

      {"WanderWidthFactor", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_wanderWidthFactor)},
      {"WanderLengthFactor", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_wanderLengthFactor)},
      {"WanderAboutPointRadius", INI::parseReal, NULL,
       offsetof(LocomotorTemplate, m_wanderAboutPointRadius)},
...
LocomotorTemplate* LocomotorStore::findLocomotorTemplate(NameKeyType namekey) {
  if (namekey == NAMEKEY_INVALID) return NULL;
...
//-------------------------------------------------------------------------------------------------
const LocomotorTemplate* LocomotorStore::findLocomotorTemplate(
    NameKeyType namekey) const {
  if (namekey == NAMEKEY_INVALID) return NULL;
...
//-------------------------------------------------------------------------------------------------
LocomotorTemplate* LocomotorStore::newOverride(
    LocomotorTemplate* locoTemplate) {
  if (locoTemplate == NULL) return NULL;

  // allocate new template
  LocomotorTemplate* newTemplate = newInstance(LocomotorTemplate);

  // copy data from final override to 'newTemplate' as a set of initial default
  // values
...
/*static*/ void LocomotorStore::parseLocomotorTemplateDefinition(INI* ini) {
  if (!TheLocomotorStore) throw INI_INVALID_DATA;
  ...
  if (loco) {
    if (ini->getLoadType() == INI_LOAD_CREATE_OVERRIDES) {
      loco = TheLocomotorStore->newOverride(
          (LocomotorTemplate*)loco->friend_getFinalOverride());
    }
    isOverride = true;
  } else {
  ...
  // if this is an override, then we want the pointer on the existing named
  // locomotor to point us to the override, so don't add it to the map.
  if (!isOverride) TheLocomotorStore->m_locomotorTemplates[namekey] = loco;
}
...
Locomotor::Locomotor(const LocomotorTemplate* tmpl) {
  ...
  m_offsetIncrement = (PI / 40) * (GameLogicRandomValueReal(0.8f, 1.2f) /
                                   m_template->m_wanderLengthFactor);
  setFlag(OFFSET_INCREASING, GameLogicRandomValue(0, 1));
  m_donutTimer = TheGameLogic->getFrame() +
                 DONUT_TIME_DELAY_SECONDS * LOGICFRAMES_PER_SECOND;
}
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void Locomotor::crc(Xfer* xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void Locomotor::loadPostProcess(void) {}  // end loadPostProcess
...
Real Locomotor::getMaxSpeedForCondition(BodyDamageType condition) const {
  ...
  if (IS_CONDITION_BETTER(condition,
                          TheGlobalData->m_movementPenaltyDamageState))
...
Real Locomotor::getMaxTurnRate(BodyDamageType condition) const {
  ...
  if (turn > m_maxTurnRate) turn = m_maxTurnRate;

  const Real TURN_FACTOR = 2;
  if (getFlag(ULTRA_ACCURATE)) turn *= TURN_FACTOR;  // monster turning ability
...
Real Locomotor::getBraking() const {
  ...
  if (braking > m_maxBraking) braking = m_maxBraking;
...
Real Locomotor::getMaxLift(BodyDamageType condition) const {
  ...
  if (lift > m_maxLift) lift = m_maxLift;
...
void Locomotor::locoUpdate_moveTowardsAngle(Object* obj, Real goalAngle) {
  ...
  if (obj == NULL || m_template == NULL) return;
...
//	DEBUG_ASSERTLOG(obj->getID() != TheObjectIDToDebug,
//("locoUpdate_moveTowardsAngle %f (%f deg), spd %f
//(%f)\n",goalAngle,goalAngle*180/PI,physics->getSpeed(),physics->getForwardSpeed2D()));
  ...
  if (minSpeed > 0) {
    ...
    Bool blocked = false;
    locoUpdate_moveTowardsPosition(obj, desiredPos, onPathDistToGoal, minSpeed,
                                   &blocked);

    // don't need to call handleBehaviorZ() here, since
    // locoUpdate_moveTowardsPosition() will do so
    return;
  } else {
    DEBUG_ASSERTCRASH(m_template->m_appearance != LOCO_THRUST,
                      ("THRUST should always have minspeeds!\n"));
    Coord3D desiredPos = *obj->getPosition();
...
PhysicsTurningType Locomotor::rotateTowardsPosition(Object* obj,
  ...
  PhysicsTurningType rotating =
      rotateObjAroundLocoPivot(obj, goalPos, turnRate, relAngle);
  return rotating;
}
...
void Locomotor::setPhysicsOptions(Object* obj) {
  ...
  // crank up the friction in ultra-accurate mode to increase movement
  // precision.
  ...
  physics->setAllowAirborneFriction(
      getApply2DFrictionWhenAirborne());  // you'd think we wouldn't want
                                          // friction in the air, but it's
                                          // needed for realistic behavior.
  physics->setStickToGround(
      getStickToGround());  // walking guys aren't allowed to catch huge (or
                            // even small) air.
}
...
void Locomotor::locoUpdate_moveTowardsPosition(Object* obj,
  ...
  // sanity, we cannot use desired speed that is greater than our max speed we
  // are capable of moving at
  if (desiredSpeed > maxSpeed) desiredSpeed = maxSpeed;

  Real distToStopAtMaxSpeed = (maxSpeed / getBraking()) * (maxSpeed) / 2.0f;
  if (onPathDistToGoal > PATHFIND_CELL_SIZE_F &&
      onPathDistToGoal > distToStopAtMaxSpeed) {
...
#ifdef DEBUG_OBJECT_ID_EXISTS
//	DEBUG_ASSERTLOG(obj->getID() != TheObjectIDToDebug,
//("locoUpdate_moveTowardsPosition %f %f %f (dtg %f, spd %f), speed %f
//(%f)\n",goalPos.x,goalPos.y,goalPos.z,onPathDistToGoal,desiredSpeed,physics->getSpeed(),physics->getForwardSpeed2D()));
  ...
  if (BitTest(m_template->m_surfaces, LOCOMOTORSURFACE_AIR) == false &&
      !TheAI->pathfinder()->validMovementTerrain(obj->getLayer(), this,
                                                 obj->getPosition()) &&
  ...
  // If the actual distance is farther, then use the actual distance so we get
  // there.
  ...
  if (*blocked) {
    physics->scrubVelocity2D(
        desiredSpeed);  // stop if we are about to run into the blocking object.
    Real turnRate = getMaxTurnRate(obj->getBodyModule()->getDamageState());
    if (m_template->m_wanderWidthFactor == 0.0f) {
      *blocked =
          (TURN_NONE != rotateObjAroundLocoPivot(obj, goalPos, turnRate));
    }

    // it is very important to be sure to call this in all situations, even if
    // not moving in 2d space.
  ...
  if (
      ...
      // we actually really want them to, because it allows much more precise
  ...
  if (getAllowMotiveForceWhileAirborne() || !treatAsAirborne) {
    switch (m_template->m_appearance) {
      case LOCO_LEGS_TWO:
        moveTowardsPositionLegs(obj, physics, goalPos, onPathDistToGoal,
                                desiredSpeed);
        break;
      case LOCO_CLIMBER:
        moveTowardsPositionClimb(obj, physics, goalPos, onPathDistToGoal,
                                 desiredSpeed);
        break;
      case LOCO_WHEELS_FOUR:
        moveTowardsPositionWheels(obj, physics, goalPos, onPathDistToGoal,
                                  desiredSpeed);
        break;
      case LOCO_TREADS:
        moveTowardsPositionTreads(obj, physics, goalPos, onPathDistToGoal,
                                  desiredSpeed);
        break;
      case LOCO_HOVER:
        moveTowardsPositionHover(obj, physics, goalPos, onPathDistToGoal,
                                 desiredSpeed);
        break;
      case LOCO_WINGS:
        moveTowardsPositionWings(obj, physics, goalPos, onPathDistToGoal,
                                 desiredSpeed);
        break;
      case LOCO_THRUST:
        moveTowardsPositionThrust(obj, physics, goalPos, onPathDistToGoal,
                                  desiredSpeed);
        break;
      case LOCO_OTHER:
      default:
        moveTowardsPositionOther(obj, physics, goalPos, onPathDistToGoal,
                                 desiredSpeed);
        break;
    }
  }
  ...
  handleBehaviorZ(obj, physics, goalPos);
  // Objects that are braking don't follow the normal physics, so they end up at
...
#define MIN_VEL (PATHFIND_CELL_SIZE_F / (LOGICFRAMES_PER_SECOND))
    ...
    if (obj->isKindOf(KINDOF_PROJECTILE)) {
      ...
      if (vel > dist) vel = dist;  // do not overcompensate!
      // Normalize.
      if (dist > 0.001f) {
        dist = 1.0f / dist;
        dx *= dist;
...
//-------------------------------------------------------------------------------------------------
void Locomotor::moveTowardsPositionTreads(Object* obj, PhysicsBehavior* physics,
                                          const Coord3D& goalPos,
                                          Real onPathDistToGoal,
                                          Real desiredSpeed) {
  // sanity, we cannot use desired speed that is greater than our max speed we
  ...
  //
  // Modulate speed according to turning. The more we have to turn, the slower
  // we go
  //
  const Real QUAETERPI = PI / 4.0f;
  Real angleCoeff = (Real)fabs(relAngle) / QUAETERPI;
  if (angleCoeff > 1.0f) angleCoeff = 1.0;
  ...
  if (onPathDistToGoal < slowDownDist && !getFlag(IS_BRAKING) &&
      !getFlag(NO_SLOW_DOWN_AS_APPROACHING_DEST)) {
    setFlag(IS_BRAKING, true);
    m_brakingFactor = 1.1f;
  }

  if (onPathDistToGoal > PATHFIND_CELL_SIZE_F &&
      onPathDistToGoal > 2.0 * slowDownDist) {
    setFlag(IS_BRAKING, false);
  }
  ...
  // DEBUG_LOG(("Actual speed %f, Braking factor %f, slowDownDist %f, Pathdist
  // %f, goalSpeed %f\n", 	actualSpeed, m_brakingFactor, slowDownDist,
  //onPathDistToGoal, goalSpeed));
  ...
  if (speedDelta != 0.0f) {
    ...
    Real acceleration =
        (speedDelta > 0.0f) ? maxAcceleration : -m_brakingFactor * getBraking();
    ...
    */
    ...
    if (fabs(accelForce) > fabs(maxForceNeeded)) accelForce = maxForceNeeded;
...
//-------------------------------------------------------------------------------------------------
void Locomotor::moveTowardsPositionWheels(Object* obj, PhysicsBehavior* physics,
                                          const Coord3D& goalPos,
                                          Real onPathDistToGoal,
                                          Real desiredSpeed) {
  BodyDamageType bdt = obj->getBodyModule()->getDamageState();
  ...
  //	Real relAngle = ThePartitionManager->getRelativeAngle2D( obj, &goalPos
  //); 	Real desiredAngle = angle + relAngle;
  Real desiredAngle = atan2(goalPos.y - obj->getPosition()->y,
                            goalPos.x - obj->getPosition()->x);
  Real relAngle = stdAngleDiff(desiredAngle, angle);

  Bool moveBackwards = false;

  // Wheeled vehicles can only turn while moving, so make sure the turn speed is
  // reasonable.
  ...
  if (actualSpeed == 0.0f) {
    ...
    if (m_template->m_canMoveBackward && fabs(relAngle) > PI / 2) {
      ...
      setFlag(DOING_THREE_POINT_TURN,
              onPathDistToGoal > 5 * obj->getGeometryInfo().getMajorRadius());
    }
  }
  ...
  const Real PROJECT_FRAMES =
      LOGICFRAMES_PER_SECOND / 2;  // Project out 1/2 second.
  if (fabs(relAngle) > FIFTEEN_DEGREES) {
    // If we're turning more than 10 degrees, check & see if we're moving into
    ...
    if (!TheAI->pathfinder()->validMovementTerrain(obj->getLayer(), this,
                                                   &halfPos) ||
        !TheAI->pathfinder()->validMovementTerrain(obj->getLayer(), this,
                                                   &nextPos)) {
  ...
  if (onPathDistToGoal < effectiveSlowDownDist && !getFlag(IS_BRAKING) &&
      !getFlag(NO_SLOW_DOWN_AS_APPROACHING_DEST)) {
    setFlag(IS_BRAKING, true);
    m_brakingFactor = 1.1f;
  }
  ...
  if (turnFactor < 0) {
    turnFactor =
        -turnFactor;  // in case we're sliding backwards in a 3 pt turn.
  }
  if (turnFactor > 1.0f) turnFactor = 1.0f;
  ...
  if (speedDelta != 0.0f) {
    ...
    Real acceleration;
    if (moveBackwards) {
      acceleration = (speedDelta < 0.0f) ? -maxAcceleration
                                         : m_brakingFactor * getBraking();
    } else {
      acceleration = (speedDelta > 0.0f) ? maxAcceleration
                                         : -m_brakingFactor * getBraking();
    }
    ...
    */
    ...
    // DEBUG_LOG(("Braking %d, actualSpeed %f, goalSpeed %f, delta %f, accel
    // %f\n", getFlag(IS_BRAKING), actualSpeed, goalSpeed, speedDelta,
    // accelForce));
...
#define no_IGNORE_INVALID
  ...
  // Right now we ignore invalid positions, so when units clip the edge of a
  // building or cliff they don't get stuck.  jba. 12SEPT02
  return false;
#else
  Int dx = 0;
  Int dy = 0;
  ...
  if (dx || dy) {
    ...
    Real dot = (velocity.x * correctionNormalized.x) +
               (velocity.y * correctionNormalized.y);
    if (dot > .25f) {
...
Real Locomotor::calcMinTurnRadius(BodyDamageType condition,
  ...
  */
  ...
  if (timeToTravelThatDist)
    *timeToTravelThatDist =
        (minSpeed > 0.0f) ? (minTurnRadius / minSpeed) : 0.0f;

  return minTurnRadius;
}

//-------------------------------------------------------------------------------------------------
void Locomotor::moveTowardsPositionLegs(Object* obj, PhysicsBehavior* physics,
                                        const Coord3D& goalPos,
                                        Real onPathDistToGoal,
                                        Real desiredSpeed) {
  ...
  Real maxAcceleration =
      getMaxAcceleration(obj->getBodyModule()->getDamageState());

  // sanity, we cannot use desired speed that is greater than our max speed we
  // are capable of moving at
  Real maxSpeed =
      getMaxSpeedForCondition(obj->getBodyModule()->getDamageState());
  if (desiredSpeed > maxSpeed) desiredSpeed = maxSpeed;
  ...
  if (m_template->m_wanderWidthFactor != 0.0f) {
    ...
    // This is the wander offline code - it forces the desired angle away from
    // the goal, so we wander back & forth.  jba.
  ...
  Real slowDownDist =
      calcSlowDownDist(actualSpeed, m_template->m_minSpeed, getBraking());
...
//-------------------------------------------------------------------------------------------------
void Locomotor::moveTowardsPositionClimb(Object* obj, PhysicsBehavior* physics,
                                         const Coord3D& goalPos,
                                         Real onPathDistToGoal,
                                         Real desiredSpeed) {
  Real maxAcceleration =
...
//-------------------------------------------------------------------------------------------------
void Locomotor::moveTowardsPositionWings(Object* obj, PhysicsBehavior* physics,
                                         const Coord3D& goalPos,
                                         Real onPathDistToGoal,
                                         Real desiredSpeed) {
#ifdef CIRCLE_FOR_LANDING
  if (m_circleThresh > 0.0f) {
    ...
    if (fabs(dz) > m_circleThresh) {
      ...
      Real angleTowardPos = (isNearlyZero(dx) && isNearlyZero(dy))
                                ? obj->getOrientation()
                                : atan2(dy, dx);
...
//-------------------------------------------------------------------------------------------------
void Locomotor::moveTowardsPositionHover(Object* obj, PhysicsBehavior* physics,
                                         const Coord3D& goalPos,
                                         Real onPathDistToGoal,
                                         Real desiredSpeed) {
  // handle the 2D component.
  ...
  if (TheTerrainLogic->isUnderwater(newPosition.x, newPosition.y)) {
    if (!getFlag(OVER_WATER)) {
      // Change my model condition because I used to not be over water, but now
      // I am
...
//-------------------------------------------------------------------------------------------------
void Locomotor::moveTowardsPositionThrust(Object* obj, PhysicsBehavior* physics,
                                          const Coord3D& goalPos,
                                          Real onPathDistToGoal,
                                          Real desiredSpeed) {
  BodyDamageType bdt = obj->getBodyModule()->getDamageState();
  ...
  if (getBraking() > 0) {
    // Real slowDownDist = (actualForwardSpeed - m_template->m_minSpeed) /
    // getBraking();
    Real slowDownDist = calcSlowDownDist(actualForwardSpeed,
                                         m_template->m_minSpeed, getBraking());
    if (onPathDistToGoal < slowDownDist &&
        !getFlag(NO_SLOW_DOWN_AS_APPROACHING_DEST))
  ...
  if (m_preferredHeight != 0.0f && !getFlag(PRECISE_Z_POS)) {
    // If we have a preferred flight height, and we haven't been told explicitly
    // to ignore it...
    ...
    delta = fabs(delta);
    if (delta > MAX_VERTICAL_DAMP_RANGE) delta = MAX_VERTICAL_DAMP_RANGE;
  ...
  Real maxAccel = (forwardSpeedDelta > 0.0f || getBraking() == 0)
                      ? getMaxAcceleration(bdt)
                      : -getBraking();
  Real maxTurnRate = getMaxTurnRate(bdt);

  // what direction do we need to thrust in, in order to reach the goalpos?
  Vector3 desiredThrustDir;
  calcDirectionToApplyThrust(obj, physics, localGoalPos, maxAccel,
                             desiredThrustDir);
  ...
  Vector3 thrustDir;
  Real thrustAngle = tryToRotateVector3D(maxThrustAngle, forwardDir,
                                         desiredThrustDir, thrustDir);

  // note that we are trying to orient in the direction of our vel, not the dir
  // of our thrust.
  ...
  if (forwardSpeedDelta != 0.0f || thrustAngle != 0.0f) {
    ...
    // DEBUG_LOG(("accel %f (max %f) vel %f (max %f) damping
...
//-------------------------------------------------------------------------------------------------
Real Locomotor::calcLiftToUseAtPt(Object* obj, PhysicsBehavior* physics,
                                  Real curZ, Real surfaceAtPt,
  ...
  */
  ...
  Real maxNetLift =
      maxGrossLift +
      TheGlobalData->m_gravity;  // note that gravity is always negative.
  if (maxNetLift < 0) maxNetLift = 0;
  Real curVelZ = physics->getVelocity()->z;
  // going down, braking is limited by net lift; going up, braking is limited by
  // gravity
  Real maxAccel;
  ...
  if (fabs(maxAccel) > TINY_ACCEL) {
    ...
    // calc how far it will take for us to go from cur speed to zero speed, at
    // max accel. Real brakeDist = calcSlowDownDist(curVelZ, 0, maxAccel); in
    // theory, the above is the correct calculation, but in practice, doesn't
    // work in some situations (eg, opening of USA01 map). Why, I dunno. But for
    // now I have gone back to the old, looks-incorrect-to-me-but-works calc.
    // (srj)
  ...
  if (getFlag(ULTRA_ACCURATE)) {
    ...
    // down, which sucked.) if you need to bump this back to zero, check it
    // carefully...
...
PhysicsTurningType Locomotor::rotateObjAroundLocoPivot(Object* obj,
  ...
  if (getFlag(IS_BRAKING))
    offset = 0.0f;  // When braking we do exact movement towards goal, instead
                    // of physics.
  // Rotating about pivot moves the object, and can make us miss our goal, so it
  // is disabled.  jba.
  if (offset != 0.0f) {
    ...
    // If we are very close to the goal, we twitch due to rounding error.  So
    // just return. jba.
...
#endif
    /// @todo srj -- there's probably a more efficient & more direct way to do
    /// this. find it.
    Matrix3D mtx;
    Matrix3D tmp(1);
...
//-------------------------------------------------------------------------------------------------
/*
        return true if we can maintain the position without being called every
   frame (eg, we are resting on the ground), false if not (eg, we are hovering
   or circling)
*/
Bool Locomotor::handleBehaviorZ(Object* obj, PhysicsBehavior* physics,
                                const Coord3D& goalPos) {
  ...
  switch (m_template->m_behaviorZ) {
    ...
    case Z_SEA_LEVEL:
      ...
      if (!obj->isDisabledByType(DISABLED_HELD)) {
        ...
        if (TheTerrainLogic->isUnderwater(pos.x, pos.y, &waterZ)) {
          ...
          pos.z =
              TheTerrainLogic->getLayerHeight(pos.x, pos.y, obj->getLayer());
        }
        obj->setPosition(&pos);
      }
      break;
    ...
    case Z_FIXED_ABSOLUTE_HEIGHT:
      requiresConstantCalling = TRUE;
      {
        Coord3D pos = *obj->getPosition();
        Bool surfaceRel =
            (m_template->m_behaviorZ == Z_FIXED_SURFACE_RELATIVE_HEIGHT);
    ...
    case Z_RELATIVE_TO_GROUND_AND_BUILDINGS:
      requiresConstantCalling = TRUE;
      {
        // srj sez: use getGroundOrStructureHeight(), because someday it will
        // cache building heights...
        Coord3D pos = *obj->getPosition();
        Real surfaceHt =
            ThePartitionManager->getGroundOrStructureHeight(pos.x, pos.y);
    ...
    case Z_SMOOTH_RELATIVE_TO_HIGHEST_LAYER:
      requiresConstantCalling = TRUE;
      {
        if (m_preferredHeight != 0.0f || getFlag(PRECISE_Z_POS)) {
          ...
          Real surfaceHt;
          Coord3D normal;
          const Bool clip = false;  // return the height, even if off the edge
                                    // of the bridge proper.
          surfaceHt = TheTerrainLogic->getLayerHeight(pos.x, pos.y, layerAtDest,
                                                      &normal, clip);

          Real preferredHeight = m_preferredHeight + surfaceHt;
          if (getFlag(PRECISE_Z_POS)) preferredHeight = goalPos.z;

          Real delta = preferredHeight - pos.z;
          delta *= getPreferredHeightDamping();
          preferredHeight = pos.z + delta;

          Real liftToUse = calcLiftToUseAtPt(obj, physics, pos.z, surfaceHt,
                                             preferredHeight);

          // DEBUG_LOG(("HandleBZ %d LiftToUse
    ...
    case Z_ABSOLUTE_HEIGHT:
      requiresConstantCalling = TRUE;
      {
        if (m_preferredHeight != 0.0f || getFlag(PRECISE_Z_POS)) {
          ...
          Bool surfaceRel =
              (m_template->m_behaviorZ == Z_SURFACE_RELATIVE_HEIGHT);
          Real surfaceHt = surfaceRel ? getSurfaceHtAtPt(pos.x, pos.y) : 0.0f;
          Real preferredHeight =
              m_preferredHeight + (surfaceRel ? surfaceHt : 0);
          if (getFlag(PRECISE_Z_POS)) preferredHeight = goalPos.z;
...
//-------------------------------------------------------------------------------------------------
void Locomotor::moveTowardsPositionOther(Object* obj, PhysicsBehavior* physics,
                                         const Coord3D& goalPos,
                                         Real onPathDistToGoal,
                                         Real desiredSpeed) {
  BodyDamageType bdt = obj->getBodyModule()->getDamageState();
  ...
  if (getFlag(ULTRA_ACCURATE) &&
      fabs(goalPos.y - pos->y) <=
          goalSpeed * m_template->m_ultraAccurateSlideIntoPlaceFactor &&
      fabs(goalPos.x - pos->x) <=
          goalSpeed * m_template->m_ultraAccurateSlideIntoPlaceFactor) {
...
#ifdef DEBUG_OBJECT_ID_EXISTS
//	DEBUG_ASSERTLOG(obj->getID() != TheObjectIDToDebug,
//("locoUpdate_maintainCurrentPosition %f %f %f, speed %f
//(%f)\n",m_maintainPos.x,m_maintainPos.y,m_maintainPos.z,physics->getSpeed(),physics->getForwardSpeed2D()));
  ...
  // but we do need to do this even if not moving, for hovering/Thrusting
  // things.
...
void Locomotor::maintainCurrentPositionThrust(Object* obj,
  ...
  /// @todo srj -- should these also use the "circling radius" stuff, like
  /// wings?
...
void Locomotor::maintainCurrentPositionWings(Object* obj,
  ...
  physics->setTurning(TURN_NONE);
  if (physics->isMotive() &&
      obj->isAboveTerrain())  // no need to stop something that isn't moving (or
                              // is just sitting on the ground)
  {
    // aim for the spot on the opposite side of the circle.
    BodyDamageType bdt = obj->getBodyModule()->getDamageState();
    Real turnRadius = m_template->m_circlingRadius;
    if (turnRadius == 0.0f) turnRadius = calcMinTurnRadius(bdt, NULL);

    // find the direction towards our "maintain pos"
    const Coord3D* pos = obj->getPosition();
    Real dx = m_maintainPos.x - pos->x;
    Real dy = m_maintainPos.y - pos->y;
    Real angleTowardMaintainPos = (isNearlyZero(dx) && isNearlyZero(dy))
                                      ? obj->getOrientation()
                                      : atan2(dy, dx);
    ...
    moveTowardsPositionWings(obj, physics, desiredPos, 0,
                             m_template->m_minSpeed);
  }
}
...
void Locomotor::maintainCurrentPositionHover(Object* obj,
  ...
  {
    DEBUG_ASSERTCRASH(
        m_template->m_minSpeed == 0.0f,
        ("HOVER should always have zero minSpeeds (otherwise, they WING)"));
...
//-------------------------------------------------------------------------------------------------
LocomotorSet::~LocomotorSet() { clear(); }

// ------------------------------------------------------------------------------------------------
/** CRC */
// ------------------------------------------------------------------------------------------------
void LocomotorSet::crc(Xfer* xfer) {}  // end crc
...
void LocomotorSet::xfer(Xfer* xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    for (LocomotorVector::iterator it = m_locomotors.begin();
         it != m_locomotors.end(); ++it) {
      Locomotor* loco = *it;
    ...
    if (m_locomotors.empty() == FALSE) {
      DEBUG_CRASH(
          ("LocomotorSet::xfer - vector is not empty, but should be\n"));
    ...
    for (UnsignedShort i = 0; i < count; ++i) {
      ...
      const LocomotorTemplate* lt =
          TheLocomotorStore->findLocomotorTemplate(NAMEKEY(name));
      if (lt == NULL) {
        DEBUG_CRASH(
            ("LocomotorSet::xfer - template %s not found\n", name.str()));
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void LocomotorSet::loadPostProcess(void) {}  // end loadPostProcess
...
void LocomotorSet::clear() {
  for (int i = 0; i < m_locomotors.size(); ++i) {
    if (m_locomotors[i]) m_locomotors[i]->deleteInstance();
  }
  m_locomotors.clear();
  m_validLocomotorSurfaces = 0;
  m_downhillOnly = FALSE;
...
void LocomotorSet::addLocomotor(const LocomotorTemplate* lt) {
  ...
  if (loco) {
    ...
    {
      DEBUG_ASSERTCRASH(!m_downhillOnly,
                        ("LocomotorSet, YOU CAN NOT MIX DOWNHILL-ONLY "
                         "LOCOMOTORS WITH NON-DOWNHILL-ONLY ONES."));
    }
  }
}
