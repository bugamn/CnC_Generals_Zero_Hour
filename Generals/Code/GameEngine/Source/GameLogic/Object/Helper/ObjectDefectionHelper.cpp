...
// //////////////////////////////////////////////////////////////// Author:
// Colin Day, Steven Johnson - September 2002 Desc:   Object helper module
///////////////////////////////////////////////////////////////////////////////////////////////////
...
UpdateSleepTime ObjectDefectionHelper::update() {
  ...
  if (!obj->getIsUndetectedDefector()) return UPDATE_SLEEP_FOREVER;
  ...
  if (now >= m_defectionDetectionEnd) {
    ...
    if (draw && m_doDefectorFX) {
      ...
      AudioEventRTS defectorVulnerableSound =
  ...
  if (obj->isEffectivelyDead() ||
    ...
    // PLEASE NOTE:
    // checking the is_attacking statusbit above, only handles weapon related
    // attacks... I also set m_undetectedDefector = FALSE in the
    // groupDoSpecialPower[...]( ) functions;
    obj->friend_setUndetectedDefector(FALSE);
    return UPDATE_SLEEP_FOREVER;  // hey, we're done.
  }
  ...
  {
    Bool lastPhase = (((Int)m_defectionDetectionFlashPhase) &
                      1);  // were we in a flashy phase last frame?
    UnsignedInt timeLeft = m_defectionDetectionEnd - now;
    m_defectionDetectionFlashPhase +=
        0.5f * (1.0f - ((Real)timeLeft / DEFECTION_DETECTION_TIME_MAX));
    Bool thisPhase = (((Int)m_defectionDetectionFlashPhase) &
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void ObjectDefectionHelper::startDefectionTimer(UnsignedInt numFrames,
                                                Bool withDefectorFX) {
  Object* obj = getObject();
