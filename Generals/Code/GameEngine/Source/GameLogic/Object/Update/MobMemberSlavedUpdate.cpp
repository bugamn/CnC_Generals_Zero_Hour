...
// Desc:  Slaved unit(s) remain close to their master. Used by angry Mob members
// (various)
///////////////////////////////////////////////////////////////////////////////////////////////////
...
//-------------------------------------------------------------------------------------------------
MobMemberSlavedUpdate::MobMemberSlavedUpdate(Thing *thing,
                                             const ModuleData *moduleData)
    : UpdateModule(thing, moduleData) {
  m_slaver = INVALID_ID;
  m_framesToWait = GameLogicRandomValue(0, 20);

  // MDC: moving to GameLogicRandomValue.  This does not need to be synced, but
  // having it so makes searches *so* much nicer.
  ...
  // MDC: moving to GameLogicRandomValue.  This does not need to be synced, but
  // having it so makes searches *so* much nicer.
  // getObject()->getDrawable()->setInstanceScale(GameLogicRandomValueReal( 5.0f,
  // 1.5f ));
}
...
void MobMemberSlavedUpdate::onObjectCreated() {
  const MobMemberSlavedUpdateModuleData *data =
      getMobMemberSlavedUpdateModuleData();
  m_squirrellinessRatio =
      MIN(MAX_SQUIRRELLINESS, MAX(0, data->m_squirrellinessRatio));
}
...
UpdateSleepTime MobMemberSlavedUpdate::update(void) {
  ...
  if (master == NULL) {
    ...
    me->kill();
    return UPDATE_SLEEP_NONE;  // you cannot return SLEEP_FOREVER unless you
  ...
  if (flags.anyIntersectionWith(
          MAKE_MODELCONDITION_MASK(MODELCONDITION_WEAPONSET_PLAYER_UPGRADE))) {
  ...
  if (++m_framesToWait < 16) return UPDATE_SLEEP_NONE;
  ...
  if (catchUpRadiusSquared > sqr(data->m_mustCatchUpRadius)) {
    ...
    {
      if (masterPathDistToGoal >
          myPathDistToGoal)  // I'm getting ahead of master, need to slow down
        myAI->chooseLocomotorSet(LOCOMOTORSET_WANDER);
      else
        myAI->chooseLocomotorSet(
            LOCOMOTORSET_PANIC);  // I'm lagging, so I need to snap to it!

      Coord3D nuPos = *masterAI->getGoalPosition();

      if (nuPos.length() < 1.0f)  // if a nasty error has sent me to map origin
      {
        myAI->aiMoveToPosition(master->getPosition(),
                               CMD_FROM_AI);  // NASTY BEEHIVE EFFECT
      } else {
        ...
        if (goalDelta.length() >
            5.0f *
                PATHFIND_CELL_SIZE_F)  // only if I am not headed there already
        {
          myAI->aiMoveToPosition(&nuPos,
                                 CMD_FROM_AI);  // Whither thou goest... THis
                                                // causes the mob to reconverge
        }
      }
    ...
    if (catchUpRadiusSquared >
        sqr(data->m_mustCatchUpRadius * 3))  // I am critically far, now!
    {
      ...
      if (m_catchUpCrisisTimer > data->m_catchUpCrisisBailTime) {
        ...
        //				Coord3D masterPosition =
        //*master->getPosition(); 				master->setPosition( me->getPosition() ); 				if (
        //! masterAI->isMoving() )
        //				{
        //					masterAI->aiMoveToPosition(
        //&masterPosition, CMD_FROM_AI );
        //				}
        //				m_catchUpCrisisTimer = 0;
  ...
  } else if (myAI->isMoving())  // we're all on a trip, together
  {
    ...
    //		TheAI->pathfinder()->adjustToPossibleDestination(me,
    //myAI->getLocomotorSet(), &destination); 		myAI->aiMoveToPosition(
    //&destination, CMD_FROM_AI ); // reconverge
    //	}
  ...
  } else  // give me something to do while I'm standing here...
  {
    ...
    SpawnBehaviorInterface *spawnerBehavior =
        master->getSpawnBehaviorInterface();

    if (spawnerBehavior)  // if I have a mommy
    {
      if (masterAI->isIdle())  // if controlling player has pressed stop, we
                               // stop! That's it!
      {
      ...
      if (spawnerBehavior->maySpawnSelfTaskAI(
              m_squirrellinessRatio))  // if mommy says it is okay
      {
        if (myAI->getLastCommandSource() !=
            CMD_FROM_AI)  // I may have been told to attack directly more
                          // recently
        {
          Object *newTarget = myAI->getNextMoodTarget(FALSE, FALSE);
          if (newTarget &&
              (newTarget !=
               victim))  // if there is someone else around to attack
          {
            victim = newTarget;
            myAI->aiAttackObject(newTarget, 999,
                                 CMD_FROM_AI);  // go ahead and do it
      ...
      if (!victim)  // If I still don't have anyone to shoot at
      {
        ...
        } else if (!masterAI->isAttacking())  // there Is no previous target and
                                              // master isn't attacking
        {
      ...
      DEBUG_ASSERTCRASH(
          (spawnerBehavior != NULL),
          ("Hey!, why for this mob member got no spawner? MLorenzen"));
    }
  }
