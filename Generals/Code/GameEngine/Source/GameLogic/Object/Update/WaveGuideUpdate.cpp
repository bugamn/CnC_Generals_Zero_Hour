...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
/*static*/ void WaveGuideUpdateModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  UpdateModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {

      {"WaveDelay", INI::parseDurationReal, NULL,
       offsetof(WaveGuideUpdateModuleData, m_waveDelay)},
      {"YSize", INI::parseReal, NULL,
       offsetof(WaveGuideUpdateModuleData, m_ySize)},
      {"LinearWaveSpacing", INI::parseReal, NULL,
       offsetof(WaveGuideUpdateModuleData, m_linearWaveSpacing)},
      {"WaveBendMagnitude", INI::parseReal, NULL,
       offsetof(WaveGuideUpdateModuleData, m_waveBendMagnitude)},
      {"WaterVelocity", INI::parseVelocityReal, NULL,
       offsetof(WaveGuideUpdateModuleData, m_waterVelocity)},
      {"PreferredHeight", INI::parseReal, NULL,
       offsetof(WaveGuideUpdateModuleData, m_preferredHeight)},
      {"ShorelineEffectDistance", INI::parseReal, NULL,
       offsetof(WaveGuideUpdateModuleData, m_shorelineEffectDistance)},
      {"DamageRadius", INI::parseReal, NULL,
       offsetof(WaveGuideUpdateModuleData, m_damageRadius)},
      {"DamageAmount", INI::parseReal, NULL,
       offsetof(WaveGuideUpdateModuleData, m_damageAmount)},
      {"ToppleForce", INI::parseReal, NULL,
       offsetof(WaveGuideUpdateModuleData, m_toppleForce)},
      {"RandomSplashSound", INI::parseAudioEventRTS, NULL,
       offsetof(WaveGuideUpdateModuleData, m_randomSplashSound)},
      {"RandomSplashSoundFrequency", INI::parseInt, NULL,
       offsetof(WaveGuideUpdateModuleData, m_randomSplashSoundFrequency)},
      {"BridgeParticle", INI::parseParticleSystemTemplate, NULL,
       offsetof(WaveGuideUpdateModuleData, m_bridgeParticle)},
      {"BridgeParticleAngleFudge", INI::parseAngleReal, NULL,
       offsetof(WaveGuideUpdateModuleData, m_bridgeParticleAngleFudge)},
      {"LoopingSound", INI::parseAudioEventRTS, NULL,
       offsetof(WaveGuideUpdateModuleData, m_loopingSound)},
      {0, 0, 0, 0}};
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
WaveGuideUpdate::~WaveGuideUpdate(void) {}  // end ~WaveGuideUpdate

// ------------------------------------------------------------------------------------------------
/** Start the waveguide moving along its waypoint path, bringing water
 * destruction and havok upon the land */
// ------------------------------------------------------------------------------------------------
Bool WaveGuideUpdate::startMoving(void) {
  ...
  if (waypoint) {
    //
    // traverse the entire waypoint path, there must be one link in the path,
    // and at any of the links there cannot be multiple choices of where to go
    // next
    //
    ...
    while (verify) {
      ...
      if (verify->getNumLinks() > 1) {
        DEBUG_CRASH(
            ("WaveGuideUpdate::startMoving - The waypoint path cannot have "
             "multiple link choices at any node\n"));
        return FALSE;
    ...
    if (next == NULL) {
      DEBUG_CRASH(
          ("WaveGuideUpdate:startMoving - There must be a linked waypoint path "
           "to follow\n"));
      return FALSE;
    ...
    if (ai) {
      ...
      //
      // make sure it doesn't stop or slow down at each waypoint along the path
      // if the path we're following consists of multiple nodes
      //
...
Bool WaveGuideUpdate::initWaveGuide(void) {
  ...
  if (startMoving() == FALSE) return FALSE;  // don't begin moving

  // compute the wave shape points
  computeWaveShapePoints();

  static const ParticleSystemTemplate *wave1 =
      TheParticleSystemManager->findTemplate("WaveSpray01");
  static const ParticleSystemTemplate *wave2 =
      TheParticleSystemManager->findTemplate("WaveSpray02");
  static const ParticleSystemTemplate *wave3 =
      TheParticleSystemManager->findTemplate("WaveSpray03");
...
void WaveGuideUpdate::computeWaveShapePoints(void) {
  ...
  //
  // build an array of points that will represent the wavefront.  these points
  // are created in local object space and can be put through the transform
  // matrix of the object to get the world position given the waveguides'
  ...
  for (Int y = -halfY; y < halfY; y += modData->m_linearWaveSpacing) {
    // sanity
    if (m_shapePointCount >= MAX_WAVEGUIDE_SHAPE_POINTS) break;  // exit for

    //
    // if there is a bend magnitude we will use a form of "y = x^2" to make a
    // parabola that forms the wavefront ... otherwise we just make a straight
    // line
    //
    if (modData->m_waveBendMagnitude)
      m_shapePoints[m_shapePointCount].x =
          -(y * y) / modData->m_waveBendMagnitude;
    else
...
// ------------------------------------------------------------------------------------------------
/** Given the current position and orientation of the wave guide, transform all
 * the wave shape points so we can quickly access them for mutliple reasons */
// ------------------------------------------------------------------------------------------------
void WaveGuideUpdate::transformWaveShape(void) {
  ...
  for (i = 0; i < m_shapePointCount; i++) {
    ...
    m_transformedShapePoints[i].z = TheTerrainLogic->getGroundHeight(
        m_transformedShapePoints[i].x, m_transformedShapePoints[i].y);
...
void WaveGuideUpdate::doShapeEffects(void) {
  ...
  //
  // the particle systems that make up the wave shape need to maintain a
  // position that is just above the ground
  //
  Int i, j;
  ParticleSystem *particleSys;
  Coord3D pos;
  for (i = 0; i < m_shapePointCount; i++) {
    for (j = 0; j < MAX_SHAPE_EFFECTS; j++) {
      ...
      particleSys =
          TheParticleSystemManager->findParticleSystem(m_shapeEffects[i][j]);
      if (particleSys) {
        //
        // we have the height of the terrain at this shape point in the
        // transformed shape point array ... just set the LOCAL position for the
        // particle system to reflect this Z *IF* that Z will not take us above
        // the target water height
        //
...
void WaveGuideUpdate::doWaterMotion(void) {
  ...
  // iterate the waveguide shape points ... at certain points we will create
  // water "up force"
  Int i;
  //	const Matrix3D *transform = waveGuide->getTransformMatrix();
  for (i = 0; i < m_shapePointCount; i++) {
    // push up the water here
    TheTerrainVisual->addWaterVelocity(
        m_transformedShapePoints[i].x, m_transformedShapePoints[i].y,
        modData->m_waterVelocity, modData->m_preferredHeight);
...
// ------------------------------------------------------------------------------------------------
/** Any points in our wave that are on the shoreline that are close enough to
 * the effect points for the shore will do those effects */
// ------------------------------------------------------------------------------------------------
void WaveGuideUpdate::doShoreEffects(void) {
  // this is expensive to do *all* the time
  if (TheGameLogic->getFrame() & 0x1) return;
  ...
  //
  // setup an array of points just behind the front of the wave where the first
  // wave "crest" is rising up ... we will use these points to make collide with
  // shore effects
  //
  ...
  //
  // go across the shape of our wave ... when we detect a transition from
  // underground to above ground or vice verse, that is a shoreline ... play an
  // effect between those points
  //
  static const ParticleSystemTemplate *left =
      TheParticleSystemManager->findTemplate("WaveSplashLeft01");
  static const ParticleSystemTemplate *right =
      TheParticleSystemManager->findTemplate("WaveSplashRight01");
  ...
  for (i = 0; i < m_shapePointCount; i++) {
    ...
    if (terrainZ > modData->m_preferredHeight) {
      ...
      if (underWater == TRUE && i != 0) {
        Coord3D *prevPoint =
            &effectPoints[i - 1];  // the prev point is actuall on the water so
                                   // we'll use it

        particleSystem = TheParticleSystemManager->createParticleSystem(right);
        if (particleSystem) particleSystem->setPosition(prevPoint);
      ...
      if (underWater == FALSE && i != 0) {
        ...
        if (particleSystem) particleSystem->setPosition(point);
...
// ------------------------------------------------------------------------------------------------
/** Do damage to things that have fallen victim in the path of this enourmous
 * wave */
// ------------------------------------------------------------------------------------------------
void WaveGuideUpdate::doDamage(void) {
  ...
  for (Int i = 0; i < m_shapePointCount; i++) {
    ...
    ObjectIterator *iter = ThePartitionManager->iterateObjectsInRange(
        &m_transformedShapePoints[i], modData->m_damageRadius, FROM_CENTER_2D,
        NULL);
    ...
    for (obj = iter->first(); obj; obj = iter->next()) {
      ...
      if (obj->isKindOf(KINDOF_WAVEGUIDE)) continue;

      //
      // ignore bridge towers ... water waves will destroy bridges by colliding
      // with the actual bridge object in the *center* of the bridge, not the
      // towers
      //
      if (obj->isKindOf(KINDOF_BRIDGE_TOWER)) continue;

      // get other object position
      objPos = obj->getPosition();

      //
      // only damage objects that are below the preferred height of the wave
      // doing the damage, bridges are an exception as their object is raised
      // above the ground, but we'll say the water is destroying the foundation
      // of it
      //
      if (objPos->z > modData->m_preferredHeight &&
          obj->isKindOf(KINDOF_BRIDGE) == FALSE)
        continue;
      ...
      //
      // get the cosine of the angle between the our forward direction and the
      // vector to the obj otherwise known as a dot product
      //
      ...
      //
      // we will only do damage if the object is "behind" the wave ... that is,
      // behind the 180 degrees mark behind us which we figure out by seeing if
      // the cosine of the angle between us (dot product) is less than zero
      //
      if (angle < 0) {
        // if object was not wet before we kill it and play effects
        if (BitTest(obj->getStatusBits(), OBJECT_STATUS_WET) == FALSE) {
          static const ParticleSystemTemplate *splash =
              TheParticleSystemManager->findTemplate("WaveHit01");
          ParticleSystem *particleSystem;

          // create particle system at position
          particleSystem =
              TheParticleSystemManager->createParticleSystem(splash);
          if (particleSystem) {
            Coord3D pos;

            //
            // we will use the position of the shape point (in local space) with
            // the Z of the object we're killin
            //
          ...
          toppleVector.x =
              obj->getPosition()->x - m_transformedShapePoints[i].x;
          toppleVector.y =
              obj->getPosition()->y - m_transformedShapePoints[i].y;
          toppleVector.z = 0;
          obj->topple(&toppleVector, modData->m_toppleForce,
          ...
          //
          // set flooded condition for model and turn off shadows cause we're
          // inside water, note that we want to do this after we damage the
          // object so that we switch off the shadows on the new model data
          // loaded after damage is dealt
          //
          ...
          //
          // Temp demo hack, replace bridges destroyed with the special
          // destroyed bridge art
          //
          if (obj->isKindOf(KINDOF_BRIDGE)) {
            const ThingTemplate *ttn =
                TheThingFactory->findTemplate("WaterWaveBridge");
            ...
            if (newBridge) {
              ...
              Bridge *oldBridge =
                  TheTerrainLogic->findBridgeAt(obj->getPosition());
              if (oldBridge) {
                ...
                // compute the angle of the object based on the angle of the
                // bridge
                Coord2D v;
              ...
              // create and attach a wave hit bridge particle system to the new
              // object
              ParticleSystem *particleSystem =
                  TheParticleSystemManager->createParticleSystem(
                      modData->m_bridgeParticle);
              if (particleSystem) {
                Coord3D u, x, y, z;
                Matrix3D transform;
                ...
                //
                // angle is rotated, becuase we computed from 'from' and 'to'
                // points of the bridge going *across* the valley, not pointing
                // *down* it
                //
                ...
                transform.Set(x.x, y.x, z.x, obj->getPosition()->x, x.y, y.y,
                              z.y, obj->getPosition()->y, x.z, y.z, z.z,
                              obj->getPosition()->z);
...
UpdateSleepTime WaveGuideUpdate::update(void) {
  ...
  if (waveGuide->isDisabled()) return UPDATE_SLEEP_NONE;

  // get module data
  const WaveGuideUpdateModuleData *modData = getWaveGuideUpdateModuleData();

  // set the frame we became active on if not set
  if (m_activeFrame == 0) m_activeFrame = TheGameLogic->getFrame();
  ...
  if (TheGameLogic->getFrame() - m_splashSoundFrame >
      LOGICFRAMES_PER_SECOND / 2.0f) {
  ...
  //
  // transform the wave shape points once for the current position ... we have
  // this array because we access this data several times and don't want to
  ...
  if (v.x * v.x + v.y * v.y <= distSquared) {
    static const ParticleSystemTemplate *waveSplash =
...
void WaveGuideUpdate::xfer(Xfer *xfer) {
  ...
  xfer->xferUser(&m_transformedShapePoints,
                 sizeof(Coord3D) * MAX_WAVEGUIDE_SHAPE_POINTS);

  // particle shape effects
  xfer->xferUser(m_shapeEffects, sizeof(ParticleSystemID) *
