...
void StealthDetectorUpdateModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"DetectionRate", INI::parseDurationUnsignedInt, NULL,
       offsetof(StealthDetectorUpdateModuleData, m_updateRate)},
      {"DetectionRange", INI::parseReal, NULL,
       offsetof(StealthDetectorUpdateModuleData, m_detectionRange)},
      {"InitiallyDisabled", INI::parseBool, NULL,
       offsetof(StealthDetectorUpdateModuleData, m_initiallyDisabled)},
      {"PingSound", INI::parseAudioEventRTS, NULL,
       offsetof(StealthDetectorUpdateModuleData, m_pingSound)},
      {"LoudPingSound", INI::parseAudioEventRTS, NULL,
       offsetof(StealthDetectorUpdateModuleData, m_loudPingSound)},
      {"IRBeaconParticleSysName", INI::parseParticleSystemTemplate, NULL,
       offsetof(StealthDetectorUpdateModuleData, m_IRBeaconParticleSysTmpl)},
      {"IRParticleSysName", INI::parseParticleSystemTemplate, NULL,
       offsetof(StealthDetectorUpdateModuleData, m_IRParticleSysTmpl)},
      {"IRBrightParticleSysName", INI::parseParticleSystemTemplate, NULL,
       offsetof(StealthDetectorUpdateModuleData, m_IRBrightParticleSysTmpl)},
      {"IRGridParticleSysName", INI::parseParticleSystemTemplate, NULL,
       offsetof(StealthDetectorUpdateModuleData, m_IRGridParticleSysTmpl)},
      {"IRParticleSysBone", INI::parseAsciiString, NULL,
       offsetof(StealthDetectorUpdateModuleData, m_IRParticleSysBone)},
      {"ExtraRequiredKindOf", KindOfMaskType::parseFromINI, NULL,
       offsetof(StealthDetectorUpdateModuleData, m_extraDetectKindof)},
      {"ExtraForbiddenKindOf", KindOfMaskType::parseFromINI, NULL,
       offsetof(StealthDetectorUpdateModuleData, m_extraDetectKindofNot)},
      {"CanDetectWhileGarrisoned", INI::parseBool, NULL,
       offsetof(StealthDetectorUpdateModuleData, m_canDetectWhileGarrisoned)},
      {"CanDetectWhileContained", INI::parseBool, NULL,
       offsetof(StealthDetectorUpdateModuleData, m_canDetectWhileTransported)},
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
StealthDetectorUpdate::StealthDetectorUpdate(Thing *thing,
                                             const ModuleData *moduleData)
    : UpdateModule(thing, moduleData) {
  const StealthDetectorUpdateModuleData *data =
      getStealthDetectorUpdateModuleData();
  ...
  setWakeFrame(getObject(), m_enabled ? UPDATE_SLEEP(GameLogicRandomValue(
                                            1, data->m_updateRate))
...
void StealthDetectorUpdate::setSDEnabled(Bool enabled) {
  ...
  setWakeFrame(getObject(),
               m_enabled ? UPDATE_SLEEP_NONE : UPDATE_SLEEP_FOREVER);
}
...
class PartitionFilterStealthedOrStealthGarrisoned : public PartitionFilter {
 public:
  ...
  virtual const char *debugGetName() {
    return "PartitionFilterStealthedOrStealthGarrisoned";
  }
#endif
};

Bool PartitionFilterStealthedOrStealthGarrisoned::allow(Object *objOther) {
  if (!objOther) return FALSE;

  if (objOther->getStatusBits() & OBJECT_STATUS_STEALTHED) return TRUE;

  ContainModuleInterface *contain = objOther->getContain();
  if (contain && contain->isGarrisonable() &&
      contain->getStealthUnitsContained())
    return TRUE;
...
UpdateSleepTime StealthDetectorUpdate::update(void) {
  ...
  // We have to wait until we are fully constructed, but we will detect the
  // moment we finish
  if (self->testStatus(OBJECT_STATUS_UNDER_CONSTRUCTION))
    return UPDATE_SLEEP_NONE;

  // We turn off forever the moment we are sold.
  if (self->testStatus(OBJECT_STATUS_SOLD)) return UPDATE_SLEEP_FOREVER;
  ...
  // filterStatus(OBJECT_STATUS_STEALTHED, 0);
  PartitionFilterRelationship filterTeam(
      self, PartitionFilterRelationship::ALLOW_ENEMIES |
                PartitionFilterRelationship::ALLOW_NEUTRAL);
  PartitionFilterAcceptByKindOf filterKindof(data->m_extraDetectKindof,
                                             data->m_extraDetectKindofNot);
  PartitionFilterSameMapStatus filterMapStatus(getObject());
  PartitionFilter *filters[] = {&filterStealthOrStealthGarrisoned, &filterTeam,
                                &filterKindof, &filterMapStatus, NULL};
  ...
  for (Object *them = iter->first(); them; them = iter->next()) {
    ...
    StealthUpdate *stealth =
        (StealthUpdate *)them->findUpdateModule(key_StealthUpdate);
    if (stealth) {
      // we have found someone
      foundSomeone = TRUE;

      //
      // if this object was not previously detected it is now being revealed and
      // we want to do some UI feedback
      //
      if (them->testStatus(OBJECT_STATUS_DETECTED) == FALSE) {
        ...
        if (ThePlayerList->getLocalPlayer() == self->getControllingPlayer() &&
          ...
          //
          // do a radar event, for mines we only make events if there weren't
          // other mine events within close proximity and time to other mines
          //
          if (them->isKindOf(KINDOF_MINE))
            doFeedback = TheRadar->tryEvent(RADAR_EVENT_STEALTH_DISCOVERED,
                                            them->getPosition());
          else
            TheRadar->createEvent(them->getPosition(),
                                  RADAR_EVENT_STEALTH_DISCOVERED);

          // do audio and UI message if we need to do feedback
          if (doFeedback) {
            // audio msg
            static AudioEventRTS discoveredSound =
                TheAudio->getMiscAudio()->m_stealthDiscoveredSound;
            discoveredSound.setPlayerIndex(
                self->getControllingPlayer()->getPlayerIndex());
            TheAudio->addAudioEvent(&discoveredSound);
            // ui msg
            TheInGameUI->message(
                TheGameText->fetch("MESSAGE:StealthDiscovered"));
        ...
        if (ThePlayerList->getLocalPlayer() == them->getControllingPlayer() &&
          ...
          //
          if (them->isKindOf(KINDOF_MINE))
            doFeedback = TheRadar->tryEvent(RADAR_EVENT_STEALTH_NEUTRALIZED,
                                            them->getPosition());
          else
            TheRadar->createEvent(them->getPosition(),
          ...
          if (doFeedback) {
            // audio msg
            static AudioEventRTS neutralizedSound =
                TheAudio->getMiscAudio()->m_stealthNeutralizedSound;
            neutralizedSound.setPlayerIndex(
                them->getControllingPlayer()->getPlayerIndex());
            TheAudio->addAudioEvent(&neutralizedSound);
            // ui msg
            TheInGameUI->message(
                TheGameText->fetch("MESSAGE:StealthNeutralized"));
      ...
      // updateRate PLUS 1 is necessary to ensure it stays detected 'till we are
      // called again...
      ...
      if (data->m_IRGridParticleSysTmpl) {
        const ParticleSystemTemplate *gridTemplate =
            data->m_IRGridParticleSysTmpl;
        if (gridTemplate) {
    ...
    else  // perhaps they are garrisoning something stealthy, eh?
    {
      ...
      if (contain && contain->isGarrisonable() &&
        ...
        for (ContainedItemsList::const_iterator it =
             ...
             it != contain->getContainedItemsList()->end(); ++it) {
          rider = *it;
          ...
          if (stealth) {
            // we have found someone
            foundSomeone = TRUE;
            if (self->getControllingPlayer() != rider->getControllingPlayer() &&
                self->getRelationship(rider) != ALLIES) {
  ...
  // Make sure the detector is visible to the local player before we add effects
  // or sounds.
  if (data->m_IRGridParticleSysTmpl &&
      self->getShroudedStatus(
    ...
    if (myDraw)
      myDraw->getPristineBonePositions(data->m_IRParticleSysBone.str(), 0,
                                       &bonePosition, NULL, 1);
    ...
    if (pingTemplate) {
      ParticleSystem *sys =
          TheParticleSystemManager->createParticleSystem(pingTemplate);
      if (sys) {
        if (myDraw)
    ...
    const ParticleSystemTemplate *beaconTemplate =
        data->m_IRBeaconParticleSysTmpl;
    if (beaconTemplate) {
      ParticleSystem *sys = TheParticleSystemManager->createParticleSystem(
          beaconTemplate);  // BEACON
      if (sys) {
        if (myDraw)
          sys->attachToDrawable(myDraw);
        else
