...
//-------------------------------------------------------------------------------------------------
/*static*/ void PointDefenseLaserUpdateModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  ModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"WeaponTemplate", INI::parseWeaponTemplate, NULL,
       offsetof(PointDefenseLaserUpdateModuleData, m_weaponTemplate)},
      {"PrimaryTargetTypes", KindOfMaskType::parseFromINI, NULL,
       offsetof(PointDefenseLaserUpdateModuleData, m_primaryTargetKindOf)},
      {"SecondaryTargetTypes", KindOfMaskType::parseFromINI, NULL,
       offsetof(PointDefenseLaserUpdateModuleData, m_secondaryTargetKindOf)},
      {"ScanRate", INI::parseDurationUnsignedInt, NULL,
       offsetof(PointDefenseLaserUpdateModuleData, m_scanFrames)},
      {"ScanRange", INI::parseReal, NULL,
       offsetof(PointDefenseLaserUpdateModuleData, m_scanRange)},
      {"PredictTargetVelocityFactor", INI::parseReal, NULL,
       offsetof(PointDefenseLaserUpdateModuleData, m_velocityFactor)},
      {0, 0, 0, 0}};
  p.add(dataFieldParse);
}

//-------------------------------------------------------------------------------------------------
PointDefenseLaserUpdate::PointDefenseLaserUpdate(Thing *thing,
  ...
  setWakeFrame(
      getObject(),
      UPDATE_SLEEP_NONE);  // No starting sleep, but we want to sleep later.
}
...
void PointDefenseLaserUpdate::onObjectCreated() {
  ...
  if (!data->m_weaponTemplate) {
    DEBUG_CRASH(
        ("PointDefenseLaserUpdate for %s doesn't have a valid weapon template",
  ...
  if (data->m_scanRange <= attackRange) {
    DEBUG_CRASH(
        ("PointDefenseLaserUpdate for %s requires the scan range (%.1f) being "
...
UpdateSleepTime PointDefenseLaserUpdate::update() {
  ...
  // The point defense laser typically has short range, high rate of fire, and
  // shoots at incoming projectiles that move fast. This amounts to a
  // potentially very expensive system. Instead of frantically scanning for
  // targets, we will scan less frequently (data->m_scanFrames) in a larger
  // radius (data->m_scanRange). When this occurs, we'll store the "best"
  // target, and track only that target until the next update or if it is
  // killed.
...
Object *PointDefenseLaserUpdate::scanClosestTarget() {
  ...
  for (Object *other = iter->first(); other; other = iter->next()) {
    ...
    // order matters: we want to know if I consider it to be an enemy, not vice
    // versa
    ...
    if (fDist <= fireRange) {
      ...
      // Determine where the target will be based on current velocity using
      // (m_velocityFactor * frames)
      if (data->m_velocityFactor != 0.0f && !other->isKindOf(KINDOF_IMMOBILE)) {
        ...
        if (physics) {
          ...
          fDist = sqrt(ThePartitionManager->getDistanceSquared(me, other,
                                                               FROM_CENTER_2D));
        }
      }
