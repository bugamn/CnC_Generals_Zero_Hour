...
AIUpdateModuleData::AIUpdateModuleData() {
  ...
  for (int i = 0; i < MAX_TURRETS; i++) m_turretData[i] = NULL;
...
AIUpdateModuleData::~AIUpdateModuleData() {
  for (int i = 0; i < MAX_TURRETS; i++) {
    if (m_turretData[i]) {
      ...
      if (td) td->deleteInstance();
    }
  }
}

//-------------------------------------------------------------------------------------------------
const LocomotorTemplateVector *AIUpdateModuleData::findLocomotorTemplateVector(
    LocomotorSetType t) const {
  if (m_locomotorTemplates.empty()) return NULL;
...
/*static*/ void AIUpdateModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"Turret", AIUpdateModuleData::parseTurret, NULL,
       offsetof(AIUpdateModuleData, m_turretData[0])},
      {"AltTurret", AIUpdateModuleData::parseTurret, NULL,
       offsetof(AIUpdateModuleData, m_turretData[1])},
      {"AutoAcquireEnemiesWhenIdle", INI::parseBitString32,
       TheAutoAcquireEnemiesNames,
       offsetof(AIUpdateModuleData, m_autoAcquireEnemiesWhenIdle)},
      {"MoodAttackCheckRate", INI::parseDurationUnsignedInt, NULL,
       offsetof(AIUpdateModuleData, m_moodAttackCheckRate)},
#ifdef ALLOW_SURRENDER
      {"SurrenderDuration", INI::parseDurationUnsignedInt, NULL,
       offsetof(AIUpdateModuleData, m_surrenderDuration)},
#endif
      {0, 0, 0, 0}};
...
//-------------------------------------------------------------------------------------------------
/*static*/ void AIUpdateModuleData::parseTurret(INI *ini, void *instance,
                                                void *store,
                                                const void * /*userData*/) {
...
//-------------------------------------------------------------------------------------------------
/*static*/ void AIUpdateModuleData::parseLocomotorSet(
  ...
  if (!self) {
    DEBUG_CRASH(
        ("Attempted to specify a locomotor for an object without an AIUpdate "
         "block."));
    throw INI_INVALID_DATA;
  }

  LocomotorSetType set = (LocomotorSetType)INI::scanIndexList(
      ini->getNextToken(), TheLocomotorSetNames);
  ...
  for (const char *locoName = ini->getNextToken(); locoName;
       locoName = ini->getNextTokenOrNull()) {
    if (!*locoName || !stricmp(locoName, "None")) continue;

    NameKeyType locoKey = NAMEKEY(locoName);
    const LocomotorTemplate *lt =
        TheLocomotorStore->findLocomotorTemplate(locoKey);
    if (!lt) {
...
AIUpdateInterface::AIUpdateInterface(Thing *thing, const ModuleData *moduleData)
  ...
  for (i = 0; i < MAX_TURRETS; i++) m_turretAI[i] = NULL;
  ...
  for (i = 0; i < MAX_TURRETS; i++) {
    if (getAIUpdateModuleData()->m_turretData[i]) {
      m_turretAI[i] = newInstance(TurretAI)(
          getObject(), getAIUpdateModuleData()->m_turretData[i],
          (WhichTurretType)i);
    }
  }
...
//=============================================================================
void AIUpdateInterface::setSurrendered(const Object *objWeSurrenderedTo,
                                       Bool surrendered) {
  if (surrendered) {
    ...
    const Player *playerWeSurrenderedTo =
        objWeSurrenderedTo ? objWeSurrenderedTo->getControllingPlayer() : NULL;
    m_surrenderedPlayerIndex =
        playerWeSurrenderedTo ? playerWeSurrenderedTo->getPlayerIndex() : -1;

    if (!wasSurrendered) {
      // aiIdle(CMD_FROM_AI);
      //  srj sez: calling aiIdle() won't work, since we are probably
      //  "effectivelyDead"... meaning we won't respong to aiDoCommand! so go
      //  straight to the metal here:
      getStateMachine()->clear();
      getStateMachine()->setState(AI_IDLE);
      setLastCommandSource(CMD_FROM_AI);

      // Play our sound surrendered
      AudioEventRTS surrenderSound =
          *getObject()->getTemplate()->getVoiceSurrender();
      surrenderSound.setObjectID(getObject()->getID());
      TheAudio->addAudioEvent(&surrenderSound);
    }
  } else {
    // GS During the act of surrendering, we dipped to 0 and then were manually
    // set to have hit points. That made us alive but marked as Dead.  Gotta
    // undo that.
...
void AIUpdateInterface::setGoalPositionClipped(const Coord3D *in,
  ...
  if (in) {
    ...
    if (cmdSource == CMD_FROM_PLAYER) {
      ...
      if (getObject()->isKindOf(KINDOF_AIRCRAFT) &&
          getObject()->isSignificantlyAboveTerrain() &&
          m_curLocomotor != NULL) {
        // aircraft must stay further away from the map edges, to prevent
        // getting "lost"
...
/* Called by the pathfinder when it processes the pathfind queue.  Basically,
it's our turn to call use the PathfindServicesInterface to do a pathfind
operation.  This shouldn't be called (and in fact is very hard to do because
PathfindServicesInterace is private to the pathfinder) except by the pathfinder
during pathfind queue processing.  jba */
//-------------------------------------------------------------------------------------------------
void AIUpdateInterface::doPathfind(PathfindServicesInterface *pathfinder) {
  ...
  if (m_isSafePath) {
    ...
    m_path = pathfinder->findSafePath(
        getObject(), m_locomotorSet, getObject()->getPosition(), &pos1, &pos2,
  ...
  if (m_isApproachPath) {
    destroyPath();
    m_path = pathfinder->findClosestPath(
        getObject(), m_locomotorSet, getObject()->getPosition(),
        &m_requestedDestination, m_isBlockedAndStuck, 0.2f, FALSE);
    if (isDoingGroundMovement() && getPath()) {
      TheAI->pathfinder()->updateGoal(getObject(),
                                      getPath()->getLastNode()->getPosition(),
  ...
  if (m_isAttackPath) {
    ...
    if (computeAttackPath(pathfinder, victim, &m_requestedDestination)) {
      ...
      // CRCDEBUG_LOG(("AIUpdateInterface::doPathfind() - m_isAttackPath = TRUE
      // after computeAttackPath\n"));
      m_isAttackPath = TRUE;
      return;
    }
    // CRCDEBUG_LOG(("AIUpdateInterface::doPathfind() - m_isAttackPath = FALSE
    // after computeAttackPath()\n"));
    m_isAttackPath = FALSE;
    if (victim) {
      m_requestedDestination = *victim->getPosition();
      /* find a pathable destination near the victim.*/
      TheAI->pathfinder()->adjustToPossibleDestination(
          getObject(), getLocomotorSet(), &m_requestedDestination);
...
/* Requests a path to be found.  Note that if it is possible to do it without
having to use the pathfinder (air units just move point to point) it generates
the path immediately.  Otherwise the path will be processed when we get to the
front of the pathfind queue. jba */
//-------------------------------------------------------------------------------------------------
void AIUpdateInterface::requestPath(Coord3D *destination, Bool isFinalGoal) {
  ...
  // getObject()->getTemplate()->getName().str(), getObject()));
  m_requestedDestination = *destination;
  m_isFinalGoal = isFinalGoal;
  CRCDEBUG_LOG(
      ("AIUpdateInterface::requestPath() - m_isAttackPath = FALSE for object "
       "%d\n",
       getObject()->getID()));
  m_isAttackPath = FALSE;
  m_requestedVictimID = INVALID_ID;
  ...
  if (m_pathTimestamp > TheGameLogic->getFrame() - 3) {
    ...
    // DEBUG_LOG(("%d Pathfind - repathing in less than 3 frames.  Waiting 1
    // second\n", TheGameLogic->getFrame()));
...
void AIUpdateInterface::requestAttackPath(ObjectID victimID,
  ...
  CRCDEBUG_LOG(
      ("AIUpdateInterface::requestAttackPath() - m_isAttackPath = TRUE for "
       "object %d\n",
       getObject()->getID()));
  ...
  if (m_pathTimestamp > TheGameLogic->getFrame() - 3) {
    /* Requesting path very quickly.  Can cause a spin. */
    // DEBUG_LOG(("%d Pathfind - repathing in less than 3 frames.  Waiting 2
    // second\n",TheGameLogic->getFrame()));
...
void AIUpdateInterface::requestApproachPath(Coord3D *destination) {
  ...
  m_isFinalGoal = TRUE;
  CRCDEBUG_LOG(
      ("AIUpdateInterface::requestApproachPath() - m_isAttackPath = FALSE for "
       "object %d\n",
       getObject()->getID()));
  m_isAttackPath = FALSE;
  m_requestedVictimID = INVALID_ID;
...
void AIUpdateInterface::requestSafePath(ObjectID repulsor) {
  ...
  m_isFinalGoal = FALSE;
  CRCDEBUG_LOG(
      ("AIUpdateInterface::requestSafePath() - m_isAttackPath = FALSE for "
       "object %d\n",
       getObject()->getID()));
  m_isAttackPath = FALSE;
  m_requestedVictimID = INVALID_ID;
...
void AIUpdateInterface::onObjectCreated() {
  ...
  // can't do this in the ctor because makeStateMachine is a protected virtual
  // func, and overrides to virtual funcs don't exist in our ctor. (look it up.)
...
AIUpdateInterface::~AIUpdateInterface(void) {
  ...
  for (int i = 0; i < MAX_TURRETS; i++) {
    if (m_turretAI[i]) m_turretAI[i]->deleteInstance();
    m_turretAI[i] = NULL;
  }
  m_stateMachine = NULL;
...
//=============================================================================
void AIUpdateInterface::setTurretTargetObject(WhichTurretType tur, Object *o,
                                              Bool forceAttacking) {
  if (m_turretAI[tur]) {
...
//=============================================================================
Bool AIUpdateInterface::isWeaponSlotOnTurretAndAimingAtTarget(
    WeaponSlotType wslot, const Object *victim) const {
  for (int i = 0; i < MAX_TURRETS; i++) {
...
Bool AIUpdateInterface::getTurretRotAndPitch(WhichTurretType tur,
  ...
  if (m_turretAI[tur]) {
    if (turretAngle) *turretAngle = m_turretAI[tur]->getTurretAngle();
    if (turretPitch) *turretPitch = m_turretAI[tur]->getTurretPitch();
    return TRUE;
  }
  return FALSE;
}
...
//=============================================================================
WhichTurretType AIUpdateInterface::getWhichTurretForWeaponSlot(
    WeaponSlotType wslot, Real *turretAngle, Real *turretPitch) const {
  for (int i = 0; i < MAX_TURRETS; ++i) {
    if (m_turretAI[i] && m_turretAI[i]->isWeaponSlotOnTurret(wslot)) {
      if (turretAngle) *turretAngle = m_turretAI[i]->getTurretAngle();
      if (turretPitch) *turretPitch = m_turretAI[i]->getTurretPitch();
...
Real AIUpdateInterface::getCurLocomotorSpeed() const {
  if (m_curLocomotor != NULL)
    return m_curLocomotor->getMaxSpeedForCondition(
...
Bool AIUpdateInterface::chooseLocomotorSetExplicit(LocomotorSetType wst) {
  const LocomotorTemplateVector *set =
      getAIUpdateModuleData()->findLocomotorTemplateVector(wst);
  if (set) {
    m_locomotorSet.clear();
    m_curLocomotor = NULL;
    for (Int i = 0; i < set->size(); ++i) {
      const LocomotorTemplate *lt = set->at(i);
      if (lt) m_locomotorSet.addLocomotor(lt);
    }
...
void AIUpdateInterface::chooseGoodLocomotorFromCurrentSet(void) {
  ...
  Locomotor *newLoco = TheAI->pathfinder()->chooseBestLocomotorForPosition(
      getObject()->getLayer(), &m_locomotorSet, getObject()->getPosition());
  ...
  if (newLoco == NULL) {
    if (prevLoco != NULL) {
      /* due to physics, we might slight into a cell for which we have no loco
              ...
              this may look a little funny, but as a practical matter, it works
         well, since the pathfinder will prevent us from doing any significant
         "wrong" terrain. */
      newLoco = prevLoco;
    } else {
      /* this can happen for a newly-created object, which might come into being
         in the middle of an obstacle. for now, we just fake it and choose a
         ground locomotor. */
  ...
  if (prevLoco != m_curLocomotor) {
    ...
    if (getGroup()) getGroup()->recomputeGroupSpeed();
...
void AIUpdateInterface::doSurrenderUpdateStuff() {
  ...
  */
  ...
  if (m_surrenderedFramesLeft > 0) --m_surrenderedFramesLeft;
  ...
  //
  // when we leave a surrendered state we give ourselves an idle command ... why
  // you ask? Well during the surrender sequence we might have started moving
  // towards a POW truck come to pick us up, but now we should stop and be all
  // normal again
  //
...
void AIUpdateInterface::setQueueForPathTime(Int frames) {
  ...
  if (frames >= UPDATE_SLEEP_NONE && getWakeFrame() > UPDATE_SLEEP(frames)) {
    if (m_isInUpdate) {
      // we're changing this while in our own update (probably via a move
      // state). just do nothing, since update will calculate the correct sleep
      // behavior at the end.
    } else {
...
//-------------------------------------------------------------------------------------------------
void AIUpdateInterface::friend_notifyStateMachineChanged() { wakeUpNow(); }
...
UpdateSleepTime AIUpdateInterface::update(void) {
  // DEBUG_LOG(("AIUpdateInterface frame %d:
  ...
  m_completedWaypoint = NULL;  // Reset so state machine update can set it if we
                               // just completed the path.

  // assume we can sleep forever, unless the state machine (or turret, etc)
  // demand otherwise
  ...
  if (IS_STATE_SLEEP(stRet)) {
    ...
    if (frames < subMachineSleep) subMachineSleep = UPDATE_SLEEP(frames);
  } else {
  ...
  // only be set via our statemachine (via friend_startingMove or
  // friend_endMove), which we just called. thus we should never have worry
  // about waking ourselves up when this changes, since if it changes the code
  // will always flow thru here anyway. (srj)
  if (m_movementComplete) {
    ...
    if (TheAI->pathfinder()->goalPosition(getObject(), &goalPos)) {
      // Pop to goal - This shouldn't happen (often), but make sure we got to
      // where we're going.
      ...
      TheAI->pathfinder()->updateGoal(getObject(), &goalPos,
                                      getObject()->getLayer());
    }
    m_movementComplete = FALSE;
    ignoreObstacle(NULL);
  ...
  if (!obj->isEffectivelyDead() && !obj->isDisabledByType(DISABLED_PARALYZED) &&
      !obj->isDisabledByType(DISABLED_UNMANNED) &&
      !obj->isDisabledByType(DISABLED_EMP) &&
      !obj->isDisabledByType(DISABLED_HACKED)) {
    // If we are dead, don't let the turrets do anything anymore, or else they
    // will keep attacking
    for (int i = 0; i < MAX_TURRETS; ++i) {
      if (m_turretAI[i]) {
        UpdateSleepTime tmp = m_turretAI[i]->updateTurretAI();
        if (tmp < subMachineSleep) subMachineSleep = tmp;
      }
    }
  }

  // must do death check outside of the state machine update, to avoid
  // corruption
  if (isAiInDeadState() &&
      !(getStateMachine()->getCurrentStateID() == AI_DEAD)) {
    /// @todo Yikes! If we are not interruptable, and we die, what do we do?
    /// (MSB)
    getStateMachine()->clear();
    getStateMachine()->setState(AI_DEAD);
    getStateMachine()->lock("AIUpdateInterface::update");
    // strangely, dead things need to NOT sleep at all. (but they don't stay
    // dead for long, so this is not too bad.)
    subMachineSleep = UPDATE_SLEEP_NONE;
  }
...
#ifdef ALLOW_DEMORALIZE
  RELEASE_CRASH(
      ("If ALLOW_DEMORALIZE is ever defined, this code must be redone to do "
       "proper SLEEPY updates. (srj)"));
...
#ifdef ALLOW_SURRENDER
  RELEASE_CRASH(
      ("If ALLOW_SURRENDER is ever defined, this code must be redone to do "
       "proper SLEEPY updates. (srj)"));
...
1. If the paths were assigned when both units were in the same ai group, we use
the path priority assigned.
2. If not, the unit that is in front has the higher priority.
3. If exactly tied (usually beacause both units got unfortunately snapped to the
same location), ObjectID is used to break the tie.
*/
Bool AIUpdateInterface::hasHigherPathPriority(
    AIUpdateInterface *otherAI) const {
  Object *other = otherAI->getObject();

  // Vehicles always have higher priority than infantry.
  if (getObject()->isKindOf(KINDOF_VEHICLE) &&
      other->isKindOf(KINDOF_INFANTRY)) {
    return TRUE;
  }
  // Vehicles always have higher priority than infantry.
  if (getObject()->isKindOf(KINDOF_INFANTRY) &&
      other->isKindOf(KINDOF_VEHICLE)) {
    return FALSE;
  }
  ...
  Real dotProduct =
      combinedDir.x * vectorToOther.x + combinedDir.y * vectorToOther.y;
  if (dotProduct > 0)
    return FALSE;  // other is ahead of us along our directional vector.
...
/* Returns TRUE if we are blocked from moving by the other object.*/
{
  ...
  if (obj->isKindOf(KINDOF_INFANTRY) && other->isKindOf(KINDOF_INFANTRY)) {
    ...
    if (dotProduct <= 0.25)
      return FALSE;  // we are not moving in the same direction.
#endif
  }
  ...
  // collisionAngle*180/PI, otherAngle*180/PI,
  ...
  if (collisionAngle > angleLimit || collisionAngle < -angleLimit) {
    ...
    if (otherMoving && (otherAngle > angleLimit || otherAngle < -angleLimit)) {
      ...
      return FALSE;  // Off angle, and they're not moving, so we aren't moving
                     // into each other.
    }
  }
...
/* Returns TRUE if we need to rotate to point in our path's direcion.*/
{
  ...
  if (this->getCurLocomotor() &&
      this->getCurLocomotor()->getWanderWidthFactor() > 0.0f)
  ...
  if (getPath()) {
    ...
    CRCDEBUG_LOG(
        ("AIUpdateInterface::needToRotate() - calling computePointOnPath() for "
         "object %d\n",
         getObject()->getID()));
    getPath()->computePointOnPath(getObject(), m_locomotorSet,
                                  *getObject()->getPosition(), info);
    deltaAngle =
        ThePartitionManager->getRelativeAngle2D(getObject(), &info.posOnPath);
  }
...
Also determines whether objects are blocked, and if so, if they are stuck.
jba.*/
Bool AIUpdateInterface::processCollision(PhysicsBehavior *physics,
  ...
  if (m_ignoreCollisionsUntil > TheGameLogic->getFrame()) return FALSE;

  if (m_canPathThroughUnits) return FALSE;

  AIUpdateInterface *aiOther = other->getAI();
  if (aiOther == NULL) return FALSE;
    ...
    if (blocked) {
      ...
      if (!aiOther->isMovingAwayFrom(getObject())) {
        if (other->isKindOf(KINDOF_INFANTRY) &&
            !getObject()->isKindOf(KINDOF_INFANTRY)) {
...
#define dont_MOVE_AROUND  // It just causes more problems than it fixes. jba.
        ...
        if (m_curLocomotor != NULL &&
            (other->isKindOf(KINDOF_INFANTRY) ==
             getObject()->isKindOf(KINDOF_INFANTRY))) {
          Real myMaxSpeed = m_curLocomotor->getMaxSpeedForCondition(
              getObject()->getBodyModule()->getDamageState());
          Locomotor *hisLoco = aiOther->getCurLocomotor();
          if (hisLoco) {
            Real hisMaxSpeed = hisLoco->getMaxSpeedForCondition(
                other->getBodyModule()->getDamageState());
        ...
        if (aiOther->blockedBy(getObject())) {
          if (!aiOther->needToRotate()) {
            ...
            if (!hasHigherPathPriority(aiOther)) {
              ...
              // m_isBlockedAndStuck = TRUE;
              //  Intense logging jba.
              //  DEBUG_LOG(("Blocked&Stuck other is blockedByUs, has higher
              //  priority\n"));
            }
          }
        } else {
    ...
    if (isAiInDeadState()) {
      ...
      if (getObject()->isKindOf(KINDOF_INFANTRY) &&
          other->canCrushOrSquish(getObject(), TEST_SQUISH_ONLY)) {
        return TRUE;
      }
    }
    ...
    Real curDSqr = dx * dx + dy * dy;
    if (!otherMoving &&
        curDSqr < PATHFIND_CELL_SIZE_F * PATHFIND_CELL_SIZE_F * 0.25f) {
      if (this->getCurrentStateID() == AI_BUSY) {
        return false;
      }
      if (getObject()->testStatus(OBJECT_STATUS_IS_USING_ABILITY)) {
        return false;  // we are doing a special ability.  Shouldn't move at
                       // this time.  jba.
      }
      // jba intense debug
      // DEBUG_LOG(("*****Units ended up on top of each other.  Shouldn't
      // happen.\n"));
      if (isIdle()) {
        Coord3D safePosition = *getObject()->getPosition();

        TheAI->pathfinder()->adjustToPossibleDestination(
            getObject(), getLocomotorSet(), &safePosition);
        aiMoveToPosition(&safePosition, CMD_FROM_AI);
      }
      if (aiOther->isIdle()) {
        TheAI->pathfinder()->adjustToPossibleDestination(
            other, aiOther->getLocomotorSet(), &otherPos);
...
Bool AIUpdateInterface::canComputeQuickPath(void) {
  ...
  // Note - if a truck happens to pop into the air and gets a move to command,
  // it still needs to pathfind.  So only skip pathfinding for airborne things
  // that can fly... jba.
...
Bool AIUpdateInterface::computeQuickPath(const Coord3D *destination) {
  ...
  destroyPath();
  if (getObject()->isKindOf(KINDOF_AIRCRAFT) &&
      !getObject()->isKindOf(KINDOF_PROJECTILE)) {
 ...
 */
Bool AIUpdateInterface::computePath(PathfindServicesInterface *pathServices,
                                    Coord3D *destination) {
  ...
  if (!extent.isInRegionNoZ(destination)) {
    ...
    if (!extent.isInRegionNoZ(&pos)) {
      // We're starting off the map.  Since we're off the map, we can't pathfind
      // so just build a path.
      return computeQuickPath(destination);
    }
  }

  // Special case of exit factory. jba.
  if ((m_stateMachine->getCurrentStateID() == AI_FOLLOW_EXITPRODUCTION_PATH) &&
      canPathThroughUnits()) {
    Bool ok = computeQuickPath(destination);
    if (ok) {
      TheAI->pathfinder()->moveAlliesAwayFromDestination(getObject(),
                                                         *destination);
  ...
  if (!m_isFinalGoal &&
      TheAI->pathfinder()->isLinePassable(
          getObject(), surfaces, getObject()->getLayer(),
          *getObject()->getPosition(), originalDestination, false, true)) {
    return computeQuickPath(destination);
  }

  PathfindLayerEnum destinationLayer =
      TheTerrainLogic->getLayerForDestination(destination);
  if (TheAI->pathfinder()->validMovementPosition(
          getObject()->getCrusherLevel() > 0, destinationLayer, m_locomotorSet,
          destination) == FALSE) {
    theNewPath = NULL;
  } else {
    ...
    if (m_isBlockedAndStuck) {
      ...
      theNewPath = pathServices->findPath(
          getObject(), m_locomotorSet, getObject()->getPosition(), destination);
    }
  }
  if (theNewPath == NULL && m_path == NULL) {
    Real pathCostFactor = 0.0f;
    theNewPath = pathServices->findClosestPath(
        getObject(), m_locomotorSet, getObject()->getPosition(), destination,
        m_isBlockedAndStuck, pathCostFactor, FALSE);
    m_retryPath = true;
  }
  ...
  if (theNewPath) {
    ...
    if (getCurLocomotor() && getCurLocomotor()->isUltraAccurate()) {
      // Move exactly to the destination.  Normal ground pathfinding moves to a
      // gridded location.
      theNewPath->updateLastNode(&originalDestination);
    }
    setLocomotorGoalPositionOnPath();
    if (!getObject()->isKindOf(
            KINDOF_NO_COLLIDE))  // If I don't collide with things, I don't need
 ...
 */
Bool AIUpdateInterface::computeAttackPath(
    PathfindServicesInterface *pathServices, const Object *victim,
    const Coord3D *victimPos) {
  ...
  if (!victim && !victimPos) {
    // CRCDEBUG_LOG(("AIUpdateInterface::computeAttackPath() - victim is
    // NULL\n"));
    return FALSE;
  }
  ...
  if (victim != NULL) {
    if (weapon->isWithinAttackRange(source, victim)) {
      ...
      if (!viewBlocked) {
        destroyPath();
        // CRCDEBUG_LOG(("AIUpdateInterface::computeAttackPath() - target is in
        // range and visible\n"));
        return TRUE;
      }
    }
    ...
    if (weapon->isWithinAttackRange(source, victimPos)) {
      ...
      if (isDoingGroundMovement()) {
        viewBlocked = TheAI->pathfinder()->isAttackViewBlockedByObstacle(
            source, *source->getPosition(), NULL, *victimPos);
      }
      if (!viewBlocked) {
        destroyPath();
        // CRCDEBUG_LOG(("AIUpdateInterface::computeAttackPath() target pos is
  ...
  if (weapon->isContactWeapon()) {
    ...
    // approach the target position.	jba.
    ...
    if (sqr(dx) + sqr(dy) < sqr(PATHFIND_CELL_SIZE_F * 2)) {
      if (m_path) {
        m_path->updateLastNode(
            victimPos);  // jam in the coordinates of the target.
      }
    }
    ...
    // CRCDEBUG_LOG(("AIUpdateInterface::computeAttackPath() is contact
    // weapon\n"));
    return ok;
  }
  ...
  if (victim != NULL) {
    if (victim->isKindOf(KINDOF_BRIDGE)) {
      ...
      Real distSqr1 = ThePartitionManager->getDistanceSquared(
          source, &info.attackPoint1, FROM_BOUNDINGSPHERE_3D);
      Real distSqr2 = ThePartitionManager->getDistanceSquared(
          source, &info.attackPoint2, FROM_BOUNDINGSPHERE_3D);
  ...
  localVictimPos.z = TheTerrainLogic->getLayerHeight(
      localVictimPos.x, localVictimPos.y, victimLayer);

  if (getObject()->isAboveTerrain() && !landBound) {
    // for now, airborne objects don't pathfind
    // build a trivial one-node path containing destination

    weapon->computeApproachTarget(getObject(), victim, &localVictimPos, 0,
                                  localVictimPos);
    // DEBUG_ASSERTCRASH(weapon->isGoalPosWithinAttackRange(getObject(),
    // &localVictimPos, victim, victimPos, NULL),
    ...
    // compute a ground-based path
    m_path = pathServices->findAttackPath(getObject(), m_locomotorSet,
                                          getObject()->getPosition(), victim,
                                          &localVictimPos, weapon);
    TheAI->pathfinder()->setIgnoreObstacleID(INVALID_ID);
    if (m_path && m_path->getBlockedByAlly()) {
      if (!getObject()->isKindOf(
              KINDOF_NO_COLLIDE))  // If I don't collide with things, I don't
                                   // need to tell them to get out of the way
...
void AIUpdateInterface::destroyPath(void) {
  // destroy previous path
  if (m_path) m_path->deleteInstance();

  m_path = NULL;
  m_waitingForPath = FALSE;  // we no longer need it.
  // CRCDEBUG_LOG(("AIUpdateInterface::destroyPath() - m_isAttackPath = FALSE
  // for object %d\n", getObject()->getID()));
  m_isAttackPath = FALSE;
  setLocomotorGoalNone();
}
...
Bool AIUpdateInterface::isPathAvailable(const Coord3D *destination) const {
  ...
  return TheAI->pathfinder()->quickDoesPathExist(m_locomotorSet, myPos,
                                                 destination);
...
Bool AIUpdateInterface::isValidLocomotorPosition(const Coord3D *pos) const {
  return TheAI->pathfinder()->validMovementPosition(
      getObject()->getCrusherLevel() > 0, getObject()->getLayer(),
      m_locomotorSet, pos);
}
...
UpdateSleepTime AIUpdateInterface::doLocomotor(void) {
  ...
  if (getObject()->isKindOf(KINDOF_IMMOBILE)) return UPDATE_SLEEP_FOREVER;
  ...
  if (m_curLocomotor) {
      ...
      switch (m_locomotorGoalType) {
        case POSITION_EXPLICIT: {
          ...
          if (speed == FAST_AS_POSSIBLE || speed > myMaxSpeed)
            speed = myMaxSpeed;
          m_curLocomotor->locoUpdate_moveTowardsPosition(
              getObject(), m_locomotorGoalData, 0.0f, speed, &blocked);
          m_doFinalPosition = FALSE;
        } break;
        ...
        case POSITION_ON_PATH: {
          if (!getPath()) {
            ...
            DEBUG_LOG(("Waiting %d, state %s\n", m_waitingForPath,
                       getStateMachine()->getCurrentStateName().str()));
          ...
          if (!isDoingGroundMovement()) {
            // airborne locomotor.  Get the goal and distance direct to the
            // goal, don't consider obstacles.
            onPathDistToGoal = getPath()->computeFlightDistToGoal(
                getObject()->getPosition(), goalPos);
          } else {
            // Compute the actual goal position along the path to move towards.
            // Consider obstacles, and follow the intermediate path points.
            ClosestPointOnPathInfo info;
            CRCDEBUG_LOG(
                ("AIUpdateInterface::doLocomotor() - calling "
                 "computePointOnPath() for %s\n",
                 DescribeObject(getObject()).str()));
            getPath()->computePointOnPath(getObject(), m_locomotorSet,
                                          *getObject()->getPosition(), info);
            onPathDistToGoal = info.distAlongPath;
            goalPos = info.posOnPath;
            // layer is a possible bridge in the path.  Check & set the layer if
            // applicable.
          ...
          m_curLocomotor->locoUpdate_moveTowardsPosition(
              getObject(), goalPos, onPathDistToGoal + getPathExtraDistance(),
              speed, &blocked);
        ...
        case ANGLE: {
          m_curLocomotor->locoUpdate_moveTowardsAngle(getObject(),
                                                      m_locomotorGoalData.x);
          m_doFinalPosition = FALSE;
        } break;
        ...
        case NONE: {
          if (m_doFinalPosition) {
            Coord3D pos = *getObject()->getPosition();
            Bool onGround = !getObject()->isAboveTerrain() &&
            ...
            if (dSqr < DARN_CLOSE) {
              m_doFinalPosition = FALSE;
              if (onGround)
                m_finalPosition.z = TheTerrainLogic->getGroundHeight(
                    m_finalPosition.x, m_finalPosition.y);
              else
              ...
              if (dist < 1) dist = 1;
              pos.x += 2 * PATHFIND_CELL_SIZE_F * dx /
                       (dist * LOGICFRAMES_PER_SECOND);
              pos.y += 2 * PATHFIND_CELL_SIZE_F * dy /
                       (dist * LOGICFRAMES_PER_SECOND);
              if (onGround)
    ...
    if (getObject()->getHeightAboveTerrain() >
        m_curLocomotor->getAirborneTargetingHeight())
  ...
  if (m_curLocomotor != NULL && m_locomotorGoalType == NONE &&
      m_doFinalPosition == FALSE && m_isBlocked == FALSE &&
...
//-------------------------------------------------------------------------------------------------
void AIUpdateInterface::setLocomotorGoalPositionExplicit(
    const Coord3D &newPos) {
  m_locomotorGoalType = POSITION_EXPLICIT;
  m_locomotorGoalData = newPos;
#ifdef _DEBUG
  if (_isnan(m_locomotorGoalData.x) || _isnan(m_locomotorGoalData.y) ||
      _isnan(m_locomotorGoalData.z)) {
...
//-------------------------------------------------------------------------------------------------
void AIUpdateInterface::setLocomotorGoalNone() { m_locomotorGoalType = NONE; }
...
Bool AIUpdateInterface::isDoingGroundMovement(void) const {
  ...
  // if we're airborne and "allowed to fall", we are probably deliberately in
  // midair due to rappel or accident...
  const PhysicsBehavior *physics = getObject()->getPhysics();
  if (getObject()->isAboveTerrain() && physics != NULL &&
      physics->getAllowToFall()) {
    return FALSE;
  }

  // After all exceptions, we must be doing ground movement.
  // DEBUG_ASSERTLOG(getObject()->isSignificantlyAboveTerrain(), ("Object %s is
  // significantly airborne but also doing ground movement.
  // What?\n",getObject()->getTemplate()->getName().str()));
  return TRUE;
}

//-------------------------------------------------------------------------------------------------
/** Some aircraft (comanche in particular, which hover) shouldn't stack
destinations. Others, like missles, should stack destinations. AdjustDestination
in pathfinder unstacks destinations, and this routine identifies non-ground
units that should unstack. */
...
Bool AIUpdateInterface::getTreatAsAircraftForLocoDistToGoal() const {
  ...
  if (getPathExtraDistance() > PATHFIND_CLOSE_ENOUGH) {
    // We are following a waypoint or other multiple point path, so use the
    // "easy" success criteria.
    treatAsAircraft = TRUE;
  }
  if (m_curLocomotor && m_curLocomotor->getAppearance() == LOCO_HOVER) {
    // Hovercrafts are very sloppy.  So use aircraft tests for distance to goal.
    // jba.
    treatAsAircraft = TRUE;
  }
  ...
  switch (m_locomotorGoalType) {
    ...
    case POSITION_ON_PATH:
      if (!getPath()) {
        ...
        return 0.0f;
      } else if (m_curLocomotor->isCloseEnoughDist3D() ||
                 getObject()->isKindOf(KINDOF_PROJECTILE)) {
        ...
        Real distance =
            ThePartitionManager->getDistanceSquared(me, dest, FROM_CENTER_3D);
        return sqrt(distance);  // Other paths return dots of normalized
                                // vectors, so one sqrt ain't so bad
      } else {
        Coord3D goalPos;
        Bool treatAsAircraft = getTreatAsAircraftForLocoDistToGoal();
        Real dist;
        if (treatAsAircraft) {
          // airborne locomotor.  Get the goal and distance direct to the goal,
          // don't consider obstacles.
          dist = getPath()->computeFlightDistToGoal(getObject()->getPosition(),
                                                    goalPos);
        } else {
          // Ground based locomotor.
          ClosestPointOnPathInfo info;
          CRCDEBUG_LOG(
              ("AIUpdateInterface::getLocomotorDistanceToGoal() - calling "
               "computePointOnPath() for object %d\n",
               getObject()->getID()));
          getPath()->computePointOnPath(getObject(), m_locomotorSet,
                                        *getObject()->getPosition(), info);
          goalPos = info.posOnPath;
        ...
        // If the actual distance is farther, then use the actual distance so we
        // get there.
        Real dx = goalPos.x - getObject()->getPosition()->x;
        Real dy = goalPos.y - getObject()->getPosition()->y;
...
void AIUpdateInterface::joinTeam(void) {
  ...
  if (getObject()->isMobile() == FALSE) return;
...
//-------------------------------------------------------------------------------------------------
Bool AIUpdateInterface::isAllowedToRespondToAiCommands(
    const AICommandParms *parms) const {
  // the dead don't listen very well
  // (unless they are seeking to feed on the brains of the living)
  // [urrr, need brains]
  if (getObject()->isEffectivelyDead()) return FALSE;

  // We're catching the sleep mood here. AI Units that are asleep actually
  // ignore all commands. (See the AI Mood matrix for more info)
  UnsignedInt moodParms = getMoodMatrixValue();
  if ((moodParms & MM_Controller_AI) && (moodParms & MM_Mood_Sleep) &&
...
void AIUpdateInterface::aiDoCommand(const AICommandParms *parms) {
  ...
  if (isSurrendered()) {
    if (parms->m_cmdSource != CMD_FROM_AI) return;
  ...
  switch (parms->m_cmd) {
    ...
    case AICMD_FOLLOW_WAYPOINT_PATH_AS_TEAM_EXACT:
      privateFollowWaypointPathAsTeamExact(parms->m_waypoint,
                                           parms->m_cmdSource);
      break;
    case AICMD_FOLLOW_PATH:
      privateFollowPath(&parms->m_coords, parms->m_obj, parms->m_cmdSource,
                        FALSE);
      break;
    ...
    case AICMD_FORCE_ATTACK_OBJECT:
      privateForceAttackObject(parms->m_obj, parms->m_intValue,
                               parms->m_cmdSource);
      break;
    ...
    case AICMD_ATTACK_POSITION:
      privateAttackPosition(&parms->m_pos, parms->m_intValue,
                            parms->m_cmdSource);
      break;
    case AICMD_ATTACKMOVE_TO_POSITION:
      privateAttackMoveToPosition(&parms->m_pos, parms->m_intValue,
                                  parms->m_cmdSource);
      break;
    case AICMD_ATTACKFOLLOW_WAYPOINT_PATH:
      privateAttackFollowWaypointPath(parms->m_waypoint, parms->m_intValue,
                                      FALSE, parms->m_cmdSource);
      break;
    case AICMD_ATTACKFOLLOW_WAYPOINT_PATH_AS_TEAM:
      privateAttackFollowWaypointPath(parms->m_waypoint, parms->m_intValue,
                                      TRUE, parms->m_cmdSource);
      break;
    case AICMD_HUNT:
    ...
    case AICMD_GUARD_POSITION:
      privateGuardPosition(&parms->m_pos, (GuardMode)parms->m_intValue,
                           parms->m_cmdSource);
      break;
    case AICMD_GUARD_OBJECT:
      privateGuardObject(parms->m_obj, (GuardMode)parms->m_intValue,
                         parms->m_cmdSource);
      break;
    case AICMD_GUARD_TUNNEL_NETWORK:
      privateGuardTunnelNetwork((GuardMode)parms->m_intValue,
                                parms->m_cmdSource);
      break;
    case AICMD_GUARD_AREA:
      privateGuardArea(parms->m_polygon, (GuardMode)parms->m_intValue,
                       parms->m_cmdSource);
      break;
    ...
    case AICMD_COMMANDBUTTON_OBJ:
      privateCommandButtonObject(parms->m_commandButton, parms->m_obj,
                                 parms->m_cmdSource);
      break;
    case AICMD_COMMANDBUTTON_POS:
      privateCommandButtonPosition(parms->m_commandButton, &parms->m_pos,
                                   parms->m_cmdSource);
      break;
    case AICMD_WANDER:
...
void AIUpdateInterface::privateMoveToPosition(const Coord3D *pos,
  ...
  // Resetting the locomotor here was initially added for scripting purposes. It
  // has been moved to the responsibility of the script to reset the locomotor
  // before moving. This is needed because other systems (like the battle drone)
  // change the locomotor based on what it's trying to do, and doesn't want to
  // get reset when ordered to move. chooseLocomotorSet(LOCOMOTORSET_NORMAL);
...
//----------------------------------------------------------------------------------------
void AIUpdateInterface::privateFaceObject(Object *obj,
                                          CommandSourceType cmdSource) {
...
//----------------------------------------------------------------------------------------
void AIUpdateInterface::privateRappelInto(Object *target, const Coord3D &pos,
                                          CommandSourceType cmdSource) {
  // Resetting the locomotor here was initially added for scripting purposes. It
  // has been moved to the responsibility of the script to reset the locomotor
  // before moving. This is needed because other systems (like the battle drone)
 ...
 * If transportExits, transport returns and deletes itself.
 */
void AIUpdateInterface::privateMoveToAndEvacuateAndExit(
    const Coord3D *pos, CommandSourceType cmdSource) {
  if (getObject()->isMobile() == FALSE) return;
  ...
  static NameKeyType key_DeliverPayloadAIUpdate =
      NAMEKEY("DeliverPayloadAIUpdate");
  DeliverPayloadAIUpdate *dp =
      (DeliverPayloadAIUpdate *)getObject()->findUpdateModule(
          key_DeliverPayloadAIUpdate);
  if (dp) {
...
void AIUpdateInterface::privateIdle(CommandSourceType cmdSource) {
  if (getObject()->isKindOf(KINDOF_PROJECTILE)) return;
...
//----------------------------------------------------------------------------------------
// Definition of busy -- when explicitly in the busy state. Moving or attacking
// is not considered busy!
//----------------------------------------------------------------------------------------
...
Bool AIUpdateInterface::isClearingMines() const {
  // if we are attacking with an anti-mine weapon, we are clearing mines,
  // regardless of our target.

  if (!getObject()->testStatus(OBJECT_STATUS_IS_ATTACKING)) return FALSE;

  const Weapon *weapon = getObject()->getCurrentWeapon();
  if (!weapon) return FALSE;

  if ((weapon->getAntiMask() & WEAPON_ANTI_MINE) == 0) return FALSE;
...
void AIUpdateInterface::privateMoveAwayFromUnit(Object *unit,
                                                CommandSourceType cmdSource) {
  // the dead don't listen very well
  if (isAiInDeadState() || (getObject()->isMobile() == FALSE) ||
  ...
  Path *newPath = TheAI->pathfinder()->getMoveAwayFromPath(
      getObject(), unit, unitPath, obj2, path2);
  if (newPath == NULL && !canPathThroughUnits()) {
    setCanPathThroughUnits(TRUE);
    newPath = TheAI->pathfinder()->getMoveAwayFromPath(getObject(), unit,
                                                       unitPath, obj2, path2);
  }
  ...
  if (newPath) {
    ...
    m_path = newPath;
    wakeUpNow();
    m_stateMachine->setTemporaryState(AI_MOVE_OUT_OF_THE_WAY,
 ...
 * Start following the path from the given point
 */
void AIUpdateInterface::privateFollowWaypointPathExact(
    const Waypoint *way, CommandSourceType cmdSource) {
  if (getObject()->isMobile() == FALSE) return;
 ...
 * Start following the path from the given point
 */
void AIUpdateInterface::privateFollowWaypointPathAsTeam(
    const Waypoint *way, CommandSourceType cmdSource) {
  if (getObject()->isMobile() == FALSE) return;
 ...
 * Start following the path from the given point
 */
void AIUpdateInterface::privateFollowWaypointPathAsTeamExact(
    const Waypoint *way, CommandSourceType cmdSource) {
  if (getObject()->isMobile() == FALSE) return;
...
void AIUpdateInterface::privateFollowPathAppend(const Coord3D *pos,
  ...
  if (getAIStateType() == AI_FOLLOW_PATH &&
      getStateMachine()->getGoalPathSize() > 0 && effectivelyMoving) {
    ...
    // Our unit is moving to a point already so simply add our waypoint after
    // that point and convert it to a waypoint command!
...
void AIUpdateInterface::privateFollowPath(const std::vector<Coord3D> *path,
  ...
  // start us following
  getStateMachine()->setState(exitProduction ? AI_FOLLOW_EXITPRODUCTION_PATH
                                             : AI_FOLLOW_PATH);
}
 ...
 * Attack given object
 */
void AIUpdateInterface::privateAttackObject(Object *victim, Int maxShotsToFire,
                                            CommandSourceType cmdSource) {
  // Resetting the locomotor here was initially added for scripting purposes. It
  // has been moved to the responsibility of the script to reset the locomotor
  // before moving. This is needed because other systems (like the battle drone)
  ...
  // do this after setting it as the current state, as the max-shots-to-fire is
  // reset in AttackState::onEnter()
  Weapon *weapon = getObject()->getCurrentWeapon();
  if (weapon) weapon->setMaxShotCount(maxShotsToFire);
}
 ...
 */
void AIUpdateInterface::privateAttackTeam(const Team *team, Int maxShotsToFire,
                                          CommandSourceType cmdSource) {
  // Resetting the locomotor here was initially added for scripting purposes. It
  // has been moved to the responsibility of the script to reset the locomotor
  // before moving. This is needed because other systems (like the battle drone)
...
void AIUpdateInterface::privateAttackPosition(const Coord3D *pos,
  ...
  if (continueRange > 0.0f) {
    // ick. set this bit so we can find the mine to go target, even if
    // stealthed. (srj)
    getObject()->setStatus(OBJECT_STATUS_IGNORING_STEALTH, true);
    PartitionFilterPossibleToAttack filterAttack(ATTACK_NEW_TARGET, getObject(),
                                                 cmdSource);
    ...
    Object *victim = ThePartitionManager->getClosestObject(
        &localPos, continueRange, FROM_CENTER_2D, filters);
  ...
  // if it's a contact weapon, we must be able to path to the target pos. if
  // not, find a spot close by. this fixes an obscure bug with mine-clearing: if
  // you tell someone to clear mines and put the centerpoint inside a building,
  // the dozer/worker will just go thru the building to that spot. ick. so if
  // you find that this clause (below) is problematic, you'll probbaly have to
  // find another way to fix this mine-clearing bug. (srj)
  if (weapon && weapon->isContactWeapon() && !isPathAvailable(&localPos)) {
    FindPositionOptions fpOptions;
    fpOptions.minRadius = 0.0f;
    fpOptions.maxRadius = 100.0f;
    fpOptions.sourceToPathToDest =
        getObject();  // This makes it find a place forWhom can get to.
    Coord3D tmp;
  ...
  // Set the goal object to NULL because if we are attacking a location, we need
  // to be able to move up to it properly. When this isn't set, the move aborts
  // before getting into firing range, thus deadlocks.
  getStateMachine()->setGoalObject(NULL);
 ...
 */
void AIUpdateInterface::privateAttackMoveToPosition(
    const Coord3D *pos, Int maxShotsToFire, CommandSourceType cmdSource) {
  if (m_isAiDead || getObject()->isMobile() == FALSE) return;
 ...
 */
void AIUpdateInterface::privateAttackFollowWaypointPath(
    const Waypoint *way, Int maxShotsToFire, Bool asTeam,
    CommandSourceType cmdSource) {
  if (m_isAiDead || getObject()->isMobile() == FALSE) return;
  ...
  getStateMachine()->setState(
      (asTeam ? AI_ATTACKFOLLOW_WAYPOINT_PATH_AS_TEAM
 ...
 * Pick up prisoner
 */
void AIUpdateInterface::privatePickUpPrisoner(Object *prisoner,
                                              CommandSourceType cmdSource) {
 ...
 * Return prisoners
 */
void AIUpdateInterface::privateReturnPrisoners(Object *prison,
                                               CommandSourceType cmdSource) {
...
void AIUpdateInterface::privateGetHealed(Object *healDepot,
  ...
  if (TheActionManager->canGetHealedAt(getObject(), healDepot, cmdSource) ==
      FALSE)
    return;
...
void AIUpdateInterface::privateGetRepaired(Object *repairDepot,
  ...
  if (TheActionManager->canGetRepairedAt(getObject(), repairDepot, cmdSource) ==
      FALSE)
    return;
...
void AIUpdateInterface::privateEnter(Object *obj, CommandSourceType cmdSource) {
  ...
  if (TheActionManager->canEnterObject(me, obj, cmdSource,
                                       DONT_CHECK_CAPACITY)) {
    getStateMachine()->clear();
    getStateMachine()->setGoalObject(obj);
    setLastCommandSource(cmdSource);
...
//----------------------------------------------------------------------------------------
void AIUpdateInterface::privateCombatDrop(Object *target, const Coord3D &pos,
                                          CommandSourceType cmdSource) {
...
void AIUpdateInterface::privateExit(Object *objectToExit,
  ...
  if (!objectToExit) return;

  // we must go thru this state (rather than calling exitObjectViaDoor
  // directly!), because a few containers might need to delay to allow us to
  // exit (eg, Chinooks must land), meaning we might have to wait a bit, and
  // coordinate with the container by actually NOTIFYING it that we want to
  // exit...
  getStateMachine()->clear();
...
void AIUpdateInterface::doQuickExit(const std::vector<Coord3D> *path) {
  ...
  getStateMachine()->setTemporaryState(AI_FOLLOW_EXITPRODUCTION_PATH,
                                       10 * LOGICFRAMES_PER_SECOND);
  if (locked) {
...
// ------------------------------------------------------------------------------------------------
void AIUpdateInterface::privateExecuteRailedTransport(
    CommandSourceType cmdSource) {
...
void AIUpdateInterface::privateGoProne(const DamageInfo *damageInfo,
                                       CommandSourceType) {
  static NameKeyType proneModuleKey =
      TheNameKeyGenerator->nameToKey("ProneUpdate");
  ProneUpdate *proneModule =
      (ProneUpdate *)getObject()->findUpdateModule(proneModuleKey);

  if (proneModule) proneModule->goProne(damageInfo);
}
...
void AIUpdateInterface::privateWander(const Waypoint *way,
  ...
  // before moving. This is needed because other systems (like the battle drone)
  // change the locomotor based on what it's trying to do, and doesn't want to
  // get reset when ordered to move. chooseLocomotorSet(LOCOMOTORSET_WANDER);
...
void AIUpdateInterface::privatePanic(const Waypoint *way,
  ...
  // before moving. This is needed because other systems (like the battle drone)
  // change the locomotor based on what it's trying to do, and doesn't want to
  // get reset when ordered to move. chooseLocomotorSet(LOCOMOTORSET_PANIC);
...
void AIUpdateInterface::privateHackInternet(CommandSourceType cmdSource) {
  ...
  HackInternetAIUpdate *ai =
      (HackInternetAIUpdate *)getObject()->findUpdateModule(
          key_HackInternetAIUpdate);
  if (ai) {
    ai->hackInternet();
  } else {
    DEBUG_CRASH(
        ("Unit %s is expecting a 'Update = HackInternetAIUpdate' entry in "
         "FactionUnit.ini",
         getObject()->getTemplate()->getName().str()));
  }
}
...
void AIUpdateInterface::transferAttack(ObjectID fromID, ObjectID toID) {
  if (m_currentVictimID == fromID) m_currentVictimID = toID;
...
// if we are attacking a position (and NOT an object), return it. otherwise
// return null.
 ...
 */
void AIUpdateInterface::setAttitude(AttitudeType tude) { m_attitude = tude; }

/**
 * Get the current behavior modifier state
 */
AttitudeType AIUpdateInterface::getAttitude(void) const { return m_attitude; }
...
Object *AIUpdateInterface::getEnterTarget() {
  ...
  if (stateType != AI_ENTER && stateType != AI_GUARD_TUNNEL_NETWORK &&
...
UnsignedInt AIUpdateInterface::getMoodMatrixValue(void) const {
  ...
  // seems like a weird way to get my controlling object, but I don't see
  // another
  if (!getStateMachine()) {
    return returnVal;
  }
  ...
  if (player->getPlayerType() == PLAYER_HUMAN) {
    ...
    switch (getAttitude()) {
      ...
      case AI_NORMAL:
        ...
        break;
      case AI_ALERT:
        returnVal |= MM_Mood_Alert;
        break;
      case AI_AGGRESSIVE:
        returnVal |= MM_Mood_Aggressive;
        break;
      default:
        DEBUG_CRASH(
            ("Unknown mood '%d' in getMoodMatrixValue. (Team '%s'). Using "
             "normal. (jkmcd)",
             getAttitude(), getObject()->getTeam()->getName().str()));
        returnVal |= MM_Mood_Normal;
        break;
    }
  }
...
//-------------------------------------------------------------------------------------------------
UnsignedInt AIUpdateInterface::getMoodMatrixActionAdjustment(
    MoodMatrixAction action) const {
  // Angry Mob Members (but not Nexi) are never subject to moods. In particular,
  // they must never, ever, ever convert a move into an attack move, or Bad
  // Things will happend, since MobMemberSlavedUpdate expects a moveto to remain
  // a moveto. Mark L sez that members do not, in fact, need any mood adjustment
  // whatsoever, since the mood of the nexus wants to control all this anyway.
  // Unfortunately, there is no KINDOF_MOB_MEMBER, and we don't want to add one
  // at the eleventh hour... this, however, is a unique and safe combination
  // that applies only to mob members. (srj)
  if (getObject()->isKindOf(KINDOF_INFANTRY) &&
      getObject()->isKindOf(KINDOF_IGNORED_IN_GUI)) {
  ...
  if (moodMatrix & MM_Controller_Player) {
    // Player-controlled units can always do actions (from a mood perspective,
    // at any rate)
...
Object *AIUpdateInterface::getNextMoodTarget(Bool calledByAI,
  ...
  if (obj->isEffectivelyDead()) return NULL;

  if (obj->testStatus(OBJECT_STATUS_IS_USING_ABILITY)) {
    return NULL;  // we are doing a special ability.  Shouldn't auto-acquire a
                  // target at this time.  jba.
  }
  ...
  // srj sez: this should ignore calledDuringIdle, despite what the name of the
  // bit implies.
  if (isAttacking() &&
      BitTest(d->m_autoAcquireEnemiesWhenIdle, AAS_Idle_Not_While_Attacking)) {
    return NULL;
  }
  ...
  if (calledDuringIdle) {
    if ((obj->getStatusBits() & OBJECT_STATUS_STEALTHED) != 0) {
      if ((getAIUpdateModuleData()->m_autoAcquireEnemiesWhenIdle &
           AAS_Idle_Stealthed) == 0) {
        // He's stealthed and idle, but if he's garrisoned, then that's a whole
        // different matter....
  ...
  if (calledByAI &&
    ...
    if (teamVictim && getAttitude() >= AI_NORMAL) return teamVictim;
  }

  DEBUG_ASSERTCRASH(m_nextMoodCheckTime != 0,
                    ("m_nextMoodCheckTime should never be zero here."));

  if (calledByAI) {
    // make sure it's time to check again.
    if (now < m_nextMoodCheckTime) return NULL;
    ...
    if (m_randomlyOffsetMoodCheck) {
      ...
      m_nextMoodCheckTime =
          (UnsignedInt)((Int)m_nextMoodCheckTime +
                        GameLogicRandomValue(-halfRate, halfRate));
      m_randomlyOffsetMoodCheck = FALSE;
    }
  }

  // Use Guard Outer, which typically corresponds to the total range
  Real rangeToFindWithin = TheAI->getAdjustedVisionRangeForObject(
      obj, AI_VISIONFACTOR_OWNERTYPE | AI_VISIONFACTOR_MOOD);

  if (rangeToFindWithin <= 0.0f) return NULL;

  // If we are contained by an object, add it's bounding radius so that large
  // buildings can auto acquire everything in outer ranges. Calculating this
  // from the center is bad... although this code makes it possible to acquire a
  // target outside of range, but in that case, it'll just fail and continue.
  const Object *container = obj->getContainedBy();
  if (container) {
  ...
  if ((moodMatrixVal & MM_Controller_AI) && (moodMatrixVal & MM_Mood_Passive)) {
    ...
    return TheGameLogic->findObjectByID(
        bmi->getLastDamageInfo()->in.m_sourceID);
  }
  ...
  // look for targets that are already within attack range (as opposed to vision
  // range).
  if (calledByAI &&
  ...
  // Instead of shroud affecting the ability to attack, it affects the ability
  // to target. The same checks apply as the old WeaponSet check (now commented
  // out, search for getShroudedStatus)
  ...
  Object *newVictim =
      TheAI->findClosestEnemy(obj, rangeToFindWithin, flags, getAttackInfo());

  /*
  DEBUG_LOG(("GNMT frame %d: %s %08lx (con %s %08lx) uses range %f, flags %08lx,
  %s finds %s %08lx\n", now, obj->getTemplate()->getName().str(), obj, container
  ? container->getTemplate()->getName().str() : "", container,
          rangeToFindWithin,
          flags,
          getAttackInfo() != NULL && getAttackInfo() !=
  TheScriptEngine->getDefaultAttackInfo() ? "ATTACKINFO," : "", newVictim ?
  newVictim->getTemplate()->getName().str() : "", newVictim
  ));
  */

  if (newVictim) {
    CRCDEBUG_LOG(
        ("AIUpdateInterface::getNextMoodTarget() - %d is attacking %d\n",
...
void AIUpdateInterface::evaluateMoraleBonus(void) {
  ...
  for (BehaviorModule **u = us->getBehaviorModules(); *u; ++u) {
    ...
    if (hui && hui->isInHorde()) horde = TRUE;

  }  // end for

  // do we have nationalism
  ///@todo Find a better way to represent nationalism without hardcoding here
  ///(CBD)
  static const UpgradeTemplate *nationalismTemplate =
      TheUpgradeCenter->findUpgrade("Upgrade_Nationalism");
  DEBUG_ASSERTCRASH(nationalismTemplate != NULL,
                    ("AIUpdateInterface::evaluateMoraleBonus - Nationalism "
                     "upgrade not found\n"));
  ...
  /*
     ...
     horde, nationalism, demoralized ); TheInGameUI->message( msg );
  */
...
void AIUpdateInterface::setDemoralized(UnsignedInt durationInFrames) {
  ...
  if ((prevDemoralizedFrames == 0 && m_demoralizedFramesLeft > 0) ||
    ...
    // evaluate demoralization, nationalism, and horde effect as they are all
    // intertwined
    evaluateMoraleBonus();
...
void AIUpdateInterface::privateCommandButton(const CommandButton *commandButton,
  ...
  // First of all, it's quite possible to get this far with an object incapable
  // of performing such a task. Scripts will have entire teams of multiple unit
  // types and want to order units to do something... if they can, great.. if
  // not, ignore.
  ...
  if (owner) {
    ...
    if (ai) {
      ...
      const CommandSet *commandSet =
          TheControlBar->findCommandSet(owner->getCommandSetString());
      if (commandSet) {
        for (int i = 0; i < MAX_COMMANDS_PER_SET; i++) {
          const CommandButton *aCommandButton = commandSet->getCommandButton(i);
          if (commandButton == aCommandButton) {
            // We found the matching command button so now order the unit to do
            // what the button wants.
            switch (commandButton->getCommandType()) {
              ...
              default:
                if (owner->getName().isNotEmpty()) {
                  DEBUG_ASSERTCRASH(0,
                                    ("AIUpdate::privateCommandButton() -- unit "
                                     "%s ('%s'), command %s not implemented.",
                                     owner->getTemplate()->getName().str(),
                                     owner->getName().str(),
                                     commandButton->getTextLabel().str()));
                } else {
                  DEBUG_ASSERTCRASH(0, ("AIUpdate::privateCommandButton() -- "
                                        "unit %s, command %s not implemented.",
                                        owner->getTemplate()->getName().str(),
                                        commandButton->getTextLabel().str()));
                }
            }
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void AIUpdateInterface::privateCommandButtonPosition(
    const CommandButton *commandButton, const Coord3D *pos,
    CommandSourceType cmdSource) {
  if (!commandButton) {
    return;
  }
          ...
          if (commandButton == aCommandButton) {
            ...
            switch (commandButton->getCommandType()) {
              ...
              default:
                if (owner->getName().isNotEmpty()) {
                  DEBUG_ASSERTCRASH(
                      0, ("AIUpdate::privateCommandButtonPosition() -- unit %s "
                          "('%s'), command %s not implemented.",
                          owner->getTemplate()->getName().str(),
                          owner->getName().str(),
                          commandButton->getTextLabel().str()));
                } else {
                  DEBUG_ASSERTCRASH(0,
                                    ("AIUpdate::privateCommandButtonPosition() "
                                     "-- unit %s, command %s not implemented.",
                                     owner->getTemplate()->getName().str(),
                                     commandButton->getTextLabel().str()));
                }
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void AIUpdateInterface::privateCommandButtonObject(
    const CommandButton *commandButton, Object *obj,
    CommandSourceType cmdSource) {
  if (!commandButton) {
    return;
  }
        ...
        if (commandButton == aCommandButton) {
          ...
          switch (commandButton->getCommandType()) {
            ...
            default: {
              ...
              DEBUG_ASSERTCRASH(
                  0, ("AIUpdate::privateCommandButtonPosition() -- unit %s %s, "
                      "command %s at unit %s %s not implemented.",
                      myName.str(), myNickname.str(),
                      commandButton->getTextLabel().str(), targetName.str(),
                      targetNickname.str()));
            }
          }
        }
...
// ------------------------------------------------------------------------------------------------
AIGroup *AIUpdateInterface::getGroup(void) { return getObject()->getGroup(); }
...
void AIUpdateInterface::crc(Xfer *x) {
  CRCGEN_LOG(
      ("AIUpdateInterface::crc() begin - %8.8X\n", ((XferCRC *)x)->getCRC()));
  // extend base class
  UpdateModule::crc(x);
  ...
  CRCGEN_LOG(
      ("AIUpdateInterface::crc() end - %8.8X\n", ((XferCRC *)x)->getCRC()));
...
void AIUpdateInterface::xfer(Xfer *xfer) {
  ...
  if (m_waypointCount < 0 || m_waypointCount > MAX_WAYPOINTS) {
    DEBUG_CRASH(("Invalid waypoint count %d, max = %d", m_waypointCount,
                 MAX_WAYPOINTS));
  ...
  // Not needed - jba.
  // Int					m_blockedFrames;
  // ///< Number of frames we've been blocked. Real
  // m_curMaxBlockedSpeed;				///< Max speed we can
  // have and not run into blocking things. Bool
  // m_isBlocked; Bool				m_isBlockedAndStuck;
  // ///< True if we are stuck & need to recompute path. Bool
  // m_isInUpdate; Bool				m_fixLocoInPostProcess;
  ...
  if (xfer->getXferMode() == XFER_LOAD && version < 4) {
    ...
    AsciiString setName;
    if (m_curLocomotorSet > LOCOMOTORSET_INVALID &&
        m_curLocomotorSet < LOCOMOTORSET_COUNT)
      setName = TheLocomotorSetNames[m_curLocomotorSet];

    xfer->xferAsciiString(&setName);

    if (setName.isNotEmpty())
      m_curLocomotorSet = (LocomotorSetType)INI::scanIndexList(
          setName.str(), TheLocomotorSetNames);
...
void AIUpdateInterface::loadPostProcess(void) {
  ...
  if (m_fixLocoInPostProcess && m_curLocomotorSet != LOCOMOTORSET_INVALID) {
    ...
    // Set the current to invalid, because chooseLocomotorSet aborts if it is
  ...
  if (!isMoving()) {
    ...
    TheAI->pathfinder()->updateGoal(getObject(), getObject()->getPosition(),
                                    getObject()->getLayer());
    ...
    if (m_pathfindGoalCell.x >= 0 && m_pathfindGoalCell.y >= 0) {
      Coord3D goalPos;
      goalPos.x = m_pathfindGoalCell.x * PATHFIND_CELL_SIZE_F +
                  PATHFIND_CELL_SIZE_F * 0.5f;
      goalPos.y = m_pathfindGoalCell.y * PATHFIND_CELL_SIZE_F +
                  PATHFIND_CELL_SIZE_F * 0.5f;
      m_pathfindGoalCell.x = -1;
      m_pathfindGoalCell.y = -1;
      TheAI->pathfinder()->updateGoal(getObject(), &goalPos,
...
Int AIUpdateInterface::friend_getWaypointGoalPathSize() const {
  //
  // it is VERY IMPORTANT to check for the current state type as being
  // follow-path, because "getGoalPath" and friends are used for other things
  // (eg, jet takeoff and landing). if you don't do this check, you will end up
  // with really bizarre behavior in obscure jet-related cases, and our users
  // will all laugh at us.
  //
  // the goalpath should really be completely private, but at this point, this
  // ugly scheme has to be lived with. (srj)
  //
  if (getAIStateType() != AI_FOLLOW_PATH) return 0;
