...
void NeutronMissileUpdateModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"DistanceToTravelBeforeTurning", INI::parseReal, NULL,
       offsetof(NeutronMissileUpdateModuleData, m_initialDist)},
      {"MaxTurnRate", INI::parseAngularVelocityReal, NULL,
       offsetof(NeutronMissileUpdateModuleData, m_maxTurnRate)},
      {"ForwardDamping", INI::parseReal, NULL,
       offsetof(NeutronMissileUpdateModuleData, m_forwardDamping)},
      {"RelativeSpeed", INI::parseReal, NULL,
       offsetof(NeutronMissileUpdateModuleData, m_relativeSpeed)},
      {"TargetFromDirectlyAbove", INI::parseReal, NULL,
       offsetof(NeutronMissileUpdateModuleData, m_targetFromDirectlyAbove)},
      {"LaunchFX", INI::parseFXList, NULL,
       offsetof(NeutronMissileUpdateModuleData, m_launchFX)},
      {"SpecialSpeedTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(NeutronMissileUpdateModuleData, m_specialSpeedTime)},
      {"SpecialSpeedHeight", INI::parseReal, NULL,
       offsetof(NeutronMissileUpdateModuleData, m_specialSpeedHeight)},
      {"SpecialAccelFactor", INI::parseReal, NULL,
       offsetof(NeutronMissileUpdateModuleData, m_specialAccelFactor)},
      {"SpecialJitterDistance", INI::parseReal, NULL,
       offsetof(NeutronMissileUpdateModuleData, m_specialJitterDistance)},
      {"IgnitionFX", INI::parseFXList, NULL,
       offsetof(NeutronMissileUpdateModuleData, m_ignitionFX)},
      {"DeliveryDecal", RadiusDecalTemplate::parseRadiusDecalTemplate, NULL,
       offsetof(NeutronMissileUpdateModuleData, m_deliveryDecalTemplate)},
      {"DeliveryDecalRadius", INI::parseReal, NULL,
       offsetof(NeutronMissileUpdateModuleData, m_deliveryDecalRadius)},
      {0, 0, 0, 0}};
...
//-------------------------------------------------------------------------------------------------
NeutronMissileUpdate::~NeutronMissileUpdate(void) { m_deliveryDecal.clear(); }
...
//-------------------------------------------------------------------------------------------------
void NeutronMissileUpdate::projectileLaunchAtObjectOrPosition(
  ...
  if (launcher) {
    ...
    if (phys) m_vel = *phys->getVelocity();
  }
...
//-------------------------------------------------------------------------------------------------
void NeutronMissileUpdate::projectileFireAtObjectOrPosition(
  ...
  if (victim) {
    // CalcTarget will add half the target's height.  But in this case, we are
    // aiming at the ground and need to stay aiming at the ground.
    m_targetPos = *victim->getPosition();
    m_intermedPos = m_targetPos;
    m_intermedPos.z +=
        getNeutronMissileUpdateModuleData()->m_targetFromDirectlyAbove;
  } else {
  ...
  getNeutronMissileUpdateModuleData()
      ->m_deliveryDecalTemplate.createRadiusDecal(
          m_targetPos,
          getNeutronMissileUpdateModuleData()->m_deliveryDecalRadius,
...
void NeutronMissileUpdate::doLaunch(void) {
  if (!m_isLaunched) {
    ...
    if (!launcher->getDrawable() ||
        !launcher->getDrawable()->getProjectileLaunchOffset(
            m_attach_wslot, m_attach_specificBarrelToUse, &attachTransform,
            TURRET_INVALID, NULL)) {
      DEBUG_CRASH(("ProjectileLaunchPos %d %d not found!\n", m_attach_wslot,
                   m_attach_specificBarrelToUse));
    ...
    //
    // the missile on the raising up launch platform is actually 45 degrees from
    // the missile that is flying around the world ... we want to rotate it "on
    // end and in place" so that we don't see any decals on the side of the
    // missile 'pop' to the new angle
    ...
    FXList::doFXObj(getNeutronMissileUpdateModuleData()->m_launchFX,
                    getObject());
  ...
  FXList::doFXObj(getNeutronMissileUpdateModuleData()->m_ignitionFX,
                  getObject());

  if (m_exhaustSysTmpl != NULL)
    TheParticleSystemManager->createAttachedParticleSystemID(m_exhaustSysTmpl,
                                                             getObject());
...
static Real calcTransform(const Object *obj, const Coord3D *pos,
                          Real maxTurnRate, Matrix3D *newTransform) {
  ...
  Vector3 objDir =
      obj->getTransformMatrix()->Rotate_Vector(Vector3(1.0f, 0.0f, 0.0f));
...
void NeutronMissileUpdate::doAttack(void) {
  ...
  if (m_noTurnDistLeft > 0.0f) {
  ...
  } else {
    {
      ...
      //
      Real relAngle = calcTransform(
          getObject(), m_reachedIntermediatePos ? &m_targetPos : &m_intermedPos,
          getNeutronMissileUpdateModuleData()->m_maxTurnRate, &mx);

      //
      // Modulate speed according to turning. The more we have to turn, the
      // slower we go
      //
  ...
  if (d->m_specialSpeedTime > 0 &&
      now <= m_frameAtLaunch + d->m_specialSpeedTime) {
    ...
    if (elapsed < d->m_specialSpeedTime) {
      ...
      newPos.z =
          m_heightAtLaunch +
          (sqr(accelFactor * timeFrac) / accelFactor) * d->m_specialSpeedHeight;
      ...
      if (d->m_specialJitterDistance > 0.0f) {
        ...
        Vector3 vectmp;
        vectmp.X = 0;
        // MDC: moving to GameLogicRandomValue.  This does not need to be
        // synced, but having it so makes searches *so* much nicer.
  ...
  // pos.z)); Real vm = sqrt(m_vel.x*m_vel.x+m_vel.y*m_vel.y+m_vel.z*m_vel.z);
...
Bool NeutronMissileUpdate::projectileHandleCollision(Object *other) {
  ...
  if (other != NULL && projectileGetLauncherID() == other->getID()) return true;
...
UpdateSleepTime NeutronMissileUpdate::update(void) {
  ...
  if (!m_reachedIntermediatePos) {
    Real distSqr = ThePartitionManager->getDistanceSquared(
        getObject(), &m_intermedPos, FROM_CENTER_3D);
    Real boundSqr =
        sqr(getObject()->getGeometryInfo().getBoundingSphereRadius());
    if (distSqr <= boundSqr) {
      m_reachedIntermediatePos = true;
  ...
  Bool oldPosValid =
      (m_state == ATTACK);  // not valid till *after* we've launched
  switch (m_state) {
    case PRELAUNCH:
  ...
  if (m_noTurnDistLeft > 0.0f && oldPosValid) {
    ...
    Real distThisTurn =
        sqrt(sqr(newPos.x - oldPos.x) + sqr(newPos.y - oldPos.y) +
             sqr(newPos.z - oldPos.z));
    // DEBUG_LOG(("noTurnDist goes from %f to
    // %f\n",m_noTurnDistLeft,m_noTurnDistLeft-distThisTurn));
    m_noTurnDistLeft -= distThisTurn;
  }

  if (m_state != PRELAUNCH && m_state != DEAD &&
...
void NeutronMissileUpdate::xfer(Xfer *xfer) {
  ...
  AsciiString name = m_exhaustSysTmpl ? m_exhaustSysTmpl->getName()
  ...
  if (xfer->getXferMode() == XFER_LOAD) {
    ...
    if (name.isEmpty() == FALSE) {
      ...
      if (m_exhaustSysTmpl == NULL) {
        DEBUG_CRASH(
            ("NeutronMissileUpdate::xfer - Unable to find particle system "
             "'%s'\n",
             name.str()));
