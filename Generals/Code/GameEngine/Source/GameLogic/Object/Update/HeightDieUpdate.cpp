...
// //////////////////////////////////////////////////////////////////////
// Author: Objects that will die when the are a certain height above the terrain
// or objects Desc:   Colin Day, April 2002
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void HeightDieUpdateModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"TargetHeight", INI::parseReal, NULL,
       offsetof(HeightDieUpdateModuleData, m_targetHeightAboveTerrain)},
      {"TargetHeightIncludesStructures", INI::parseBool, NULL,
       offsetof(HeightDieUpdateModuleData, m_targetHeightIncludesStructures)},
      {"OnlyWhenMovingDown", INI::parseBool, NULL,
       offsetof(HeightDieUpdateModuleData, m_onlyWhenMovingDown)},
      {"DestroyAttachedParticlesAtHeight", INI::parseReal, NULL,
       offsetof(HeightDieUpdateModuleData, m_destroyAttachedParticlesAtHeight)},
      {"SnapToGroundOnDeath", INI::parseBool, NULL,
       offsetof(HeightDieUpdateModuleData, m_snapToGroundOnDeath)},
      {"InitialDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(HeightDieUpdateModuleData, m_initialDelay)},
      {0, 0, 0, 0}
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
HeightDieUpdate::~HeightDieUpdate(void) {}  // end ~HeightDieUpdate
...
UpdateSleepTime HeightDieUpdate::update(void) {
  ...
  if (m_earliestDeathFrame > now) return UPDATE_SLEEP_NONE;
  ...
  if (m_hasDied == FALSE) {
    ...
    if (modData->m_targetHeightIncludesStructures) {
      ...
      if (layer != LAYER_GROUND) {
        Real layerHeight =
            TheTerrainLogic->getLayerHeight(pos->x, pos->y, layer);
        if (layerHeight > terrainHeightAtPos) terrainHeightAtPos = layerHeight;
      }
    }

    //
    // our target height to die at is by default the height specified in the INI
    // entry above the terrain ... we may change our target height if we care
    // about dying above objects under us (see below)
    //
    Real targetHeight =
        terrainHeightAtPos + modData->m_targetHeightAboveTerrain;

    //
    // if we consider objects under us ... we will die when we are the specified
    // distance above those objects
    //
    if (modData->m_targetHeightIncludesStructures == TRUE) {
      // scan all objects in the radius of our extent and find the tallest
      // height among them
      ...
      ObjectIterator *iter = ThePartitionManager->iterateObjectsInRange(
          getObject(), range, FROM_BOUNDINGSPHERE_3D, filters);
      ...
      //
      // our target height is either the height above the terrain as specified
      // by the INI entry for the object that has this update ... or it is the
      // building height of the tallest thing under us
      //
    ...
    if (pos->z < targetHeight && directionOK) {
      ...
      // AND: even if we're not snapping to ground, be sure we don't go BELOW
      // ground
  ...
  //
  // if our height is below the destroy attached particles height above the
  // terrain, clean them up from the particle system
  //
  if (m_particlesDestroyed == FALSE &&
      pos->z < modData->m_destroyAttachedParticlesAtHeight &&
