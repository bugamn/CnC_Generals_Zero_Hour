...
/*static*/ void OCLUpdateModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"OCL", INI::parseObjectCreationList, NULL,
       offsetof(OCLUpdateModuleData, m_ocl)},
      {"MinDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(OCLUpdateModuleData, m_minDelay)},
      {"MaxDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(OCLUpdateModuleData, m_maxDelay)},
      {"CreateAtEdge", INI::parseBool, NULL,
       offsetof(OCLUpdateModuleData, m_isCreateAtEdge)},
      {0, 0, 0, 0}};
...
UpdateSleepTime OCLUpdate::update(void) {
  ...
  if (shouldCreate()) {
    if (m_nextCreationFrame == 0) {
      // You don't get to actually spread the first try, you start on a timer,
      // then go
    ...
    ObjectCreationList::create(getOCLUpdateModuleData()->m_ocl, getObject(),
                               &creationCoord, getObject()->getPosition());
  }
  return UPDATE_SLEEP_NONE;
}
...
Bool OCLUpdate::shouldCreate() {
  if (TheGameLogic->getFrame() < m_nextCreationFrame) return FALSE;  // too soon
...
void OCLUpdate::setNextCreationFrame() {
  UnsignedInt delay =
      GameLogicRandomValue(getOCLUpdateModuleData()->m_minDelay,
...
Real OCLUpdate::getCountdownPercent() const {
  ...
  return 1.0f - ((m_nextCreationFrame - now) /
                 (float)(m_nextCreationFrame - m_timerStartedFrame));
}
