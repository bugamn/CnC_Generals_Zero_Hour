...
UpdateSleepTime ProjectileStreamUpdate::update(void) {
  ...
  if (considerDying()) TheGameLogic->destroyObject(getObject());

  return UPDATE_SLEEP_NONE;
}

void ProjectileStreamUpdate::addProjectile(ObjectID sourceID, ObjectID newID) {
  DEBUG_ASSERTCRASH(m_owningObject == INVALID_ID || m_owningObject == sourceID,
                    ("Two objects are trying to use the same Projectile "
                     "stream."));  // Don't cross the streams!
  if (m_owningObject == INVALID_ID) m_owningObject = sourceID;
  ...
  DEBUG_ASSERTCRASH(m_nextFreeIndex != m_firstValidIndex,
                    ("Need to increase the allowed number of simultaneous "
                     "particles in ProjectileStreamUpdate."));
}

void ProjectileStreamUpdate::cullFrontOfList() {
  while ((m_firstValidIndex != m_nextFreeIndex) &&
         (TheGameLogic->findObjectByID(m_projectileIDs[m_firstValidIndex]) ==
          NULL)) {
    // Chew off the front if they are gone.  Don't chew on the middle, as bad
    // ones there are just a break in the chain
...
Bool ProjectileStreamUpdate::considerDying() {
  if (m_firstValidIndex == m_nextFreeIndex && m_owningObject != INVALID_ID) {
    // If I have no projectiles to watch, and my master is dead, then yes, I
    // want to die
    if (TheGameLogic->findObjectByID(m_owningObject) == NULL) return TRUE;
  }
...
void ProjectileStreamUpdate::getAllPoints(Vector3 *points, Int *count) {
  ...
  while (pointIndex != m_nextFreeIndex) {
    // Go through the array I think of as good.  Holes in the middle get 0,0,0.
    // I write to pointCount because I am unrolling a circular array into the
    // same sized flat one since I am writing anyway.
    Object *projectile =
        TheGameLogic->findObjectByID(m_projectileIDs[pointIndex]);
    ...
    if (projectile) {
      ...
      if (obj &&
          obj->isKindOf(KINDOF_VEHICLE))  // this makes the stream skim along my
                                          // roof, if I have a roof
      {
        const Coord3D *pos = obj->getPosition();
        Real myTop =
            obj->getGeometryInfo().getMaxHeightAbovePosition() + pos->z + 0.5f;
        Coord3D delta;
