...
// Updated: Kris Morness, July 2002 -- Add support for advanced scout drone
// abilities Desc:  Slaved unit(s) remain close to their master. Used by scout
// drones, and used by stinger
//        soldiers that are close to a stinger site. It's important to note that
//        any slaved units
//				can use any or all features, some of which are
//specialized.
///////////////////////////////////////////////////////////////////////////////////////////////////
...
#define STRAY_MULTIPLIER \
  2.0f  // Multiplier from stating diestance from tunnel, to max distance from
const Real CLOSE_ENOUGH =
    15;  // Our moveTo commands and pathfinding can't handle people in the way,
         // so quit trying to hump someone on your spot
...
UpdateSleepTime SlavedUpdate::update(void) {
  ...
  if (m_slaver == INVALID_ID) return UPDATE_SLEEP_NONE;
  ...
  if (!master || master->isEffectivelyDead() ||
      master->isDisabledByType(DISABLED_UNMANNED)) {
    stopSlavedEffects();
    ...
    if (masterTeam->getRelationship(myTeam) !=
        ALLIES) {  // slaver must have been hijacked or something..	// we
                   // will join his team
      me->defect(masterTeam, 0);
    }
  }

  if (data->m_stayOnSameLayerAsMaster) me->setLayer(master->getLayer());
  ...
  // Calculate the health percentage of the master -- there are two places we
  // care. NOTE: Health percentage will always be 100 should the drone be
  ...
  if (healthPercentage <= data->m_repairWhenHealthBelowPercentage) {
    // 1ST PRIORITY: Go to the master's position to repair him because he needs
    // it.
    doRepairLogic();
    return UPDATE_SLEEP_NONE;
  }
  ...
  if (data->m_attackRange) {
    // 2ND PRIORITY: Go to the master's current victim (as close as wander
    // distance allows)
    if (target) {
  ...
  if (data->m_scoutRange) {
    // 3RD PRIORITY: Hover above master's current move destination (as close as
    // wander distance allows).
    if (masterAI->getPath()) {
      const Coord3D *masterDest =
          masterAI->getPath()->getLastNode()->getPosition();

      // Check to see if master is close to the goal position.
      Real distSqr = ThePartitionManager->getDistanceSquared(
          master, masterDest, FROM_BOUNDINGSPHERE_2D);
      if (distSqr >
          (data->m_guardMaxRange * 0.5f) * (data->m_guardMaxRange * 0.5f)) {
        // If the master's distance to destination is more than half of the
        // guarding range of the slave, then order the slave to scout it.
        endRepair();
  ...
  if (healthPercentage < 100) {
    // 3RD PRIORITY: Go to the master's position to repair him (because we're
    // idle)
    doRepairLogic();
    return UPDATE_SLEEP_NONE;
  }
  ...
  m_guardPointOffset.z =
      TheTerrainLogic->getGroundHeight(pinnedPosition.x, pinnedPosition.y);

  if (data->m_guardMaxRange) {
    // 3RD PRIORITY: Guard the master's area.
    if (myAI->isIdle() &&
        ThePartitionManager->getDistanceSquared(
            me, &pinnedPosition, FROM_CENTER_3D) > CLOSE_ENOUGH_SQR) {
      // I'm idle and too far away.
      endRepair();
      doGuardLogic(&pinnedPosition);
    } else if (ThePartitionManager->getDistanceSquared(me, master,
                                                       FROM_CENTER_3D) >
...
void SlavedUpdate::doAttackLogic(const Object *target) {
  ...
  // First, determine the attack position. If the target is too far away, then
  // we'll calculate the closest allowable position.
  const Coord3D *targetPos = target->getPosition();
  Real dist = ThePartitionManager->getDistanceSquared(me, targetPos,
  ...
  if (dist > sqr(data->m_attackRange)) {
    ...
    // Now that we have calculated the vector relative to me, add it to my
    // position to get my goal.
  ...
  if (data->m_attackWanderRange) {
    ...
    m_guardPointOffset.z =
        TheTerrainLogic->getGroundHeight(attackPosition.x, attackPosition.y);
  }
...
//-------------------------------------------------------------------------------------------------
// We are ordered to attempt to get as close as possible to my master's movement
// destination point.
//-------------------------------------------------------------------------------------------------
void SlavedUpdate::doScoutLogic(const Coord3D *mastersDestination) {
  ...
  // First, determine the scout position. If our master's destination is too far
  // away, then we'll calculate the closest allowable position.
  Real dist = ThePartitionManager->getDistanceSquared(me, mastersDestination,
                                                      FROM_BOUNDINGSPHERE_2D);
  ...
  if (data->m_scoutWanderRange) {
    ...
    m_guardPointOffset.z =
        TheTerrainLogic->getGroundHeight(scoutPosition.x, scoutPosition.y);
  }
...
void SlavedUpdate::doRepairLogic() {
  ...
  Real distSqr = ThePartitionManager->getDistanceSquared(
      me, master, FROM_BOUNDINGSPHERE_2D);
  ...
  if (closeEnough) {
    ...
    Bool closeEnoughForZPrecision =
        distSqr < sqr(master->getGeometryInfo().getBoundingSphereRadius() * 2);
    ...
    Real altitude = GameLogicRandomValueReal(data->m_repairMinAltitude,
...
void SlavedUpdate::endRepair() {
  if (m_repairState != REPAIRSTATE_NONE) {
    ...
    // Set the packing state and clear any other states (to force the arm to
    // fully retract)
  ...
  switch (repairState) {
    ...
    case REPAIRSTATE_READY: {
      switch (m_repairState) {
        ...
        default:
          ...
          m_framesToWait = GameLogicRandomValue(data->m_minReadyFrames,
                                                data->m_maxReadyFrames);
          break;
      }
      break;
    }
    case REPAIRSTATE_WELDING: {
      if (m_repairState == REPAIRSTATE_READY) {
        ...
        m_framesToWait =
            GameLogicRandomValue(data->m_minWeldFrames, data->m_maxWeldFrames);

        // Make sparks!
        if (!data->m_weldingSysName.isEmpty()) {
          const ParticleSystemTemplate *tmp =
              TheParticleSystemManager->findTemplate(data->m_weldingSysName);
          if (tmp) {
            ParticleSystem *weldingSys =
...
void SlavedUpdate::moveToNewRepairSpot() {
  ...
  if (data->m_repairRange) {
    ...
    m_guardPointOffset.z = TheTerrainLogic->getGroundHeight(
        m_guardPointOffset.x, m_guardPointOffset.y);
    Real altitude = GameLogicRandomValueReal(data->m_repairMinAltitude,
                                             data->m_repairMaxAltitude);
...
void SlavedUpdate::stopSlavedEffects() {
  ...
  /// @todo Just a thought.  Our Status bits on objects really need to be
  /// reference counts so you don't clear someone else's flag
