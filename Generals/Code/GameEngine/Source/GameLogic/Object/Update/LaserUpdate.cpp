...
/*static*/ void LaserUpdateModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"MuzzleParticleSystem", INI::parseAsciiString, NULL,
       offsetof(LaserUpdateModuleData, m_particleSystemName)},
      {"TargetParticleSystem", INI::parseAsciiString, NULL,
       offsetof(LaserUpdateModuleData, m_targetParticleSystemName)},
      {"ParentFireBoneName", INI::parseAsciiString, NULL,
       offsetof(LaserUpdateModuleData, m_parentFireBoneName)},
      {"ParentFireBoneOnTurret", INI::parseAsciiString, NULL,
       offsetof(LaserUpdateModuleData, m_parentFireBoneOnTurret)},
      {0, 0, 0, 0}};
...
void LaserUpdate::clientUpdate(void) {
  ...
  if (m_decaying) {
    ...
    m_currentWidthScalar =
        1.0f - (Real)(now - m_decayStartFrame) /
                   (Real)(m_decayFinishFrame - m_decayStartFrame);
    m_dirty = true;
    ...
    UnsignedInt now = TheGameLogic->getFrame();
    m_currentWidthScalar = (Real)(now - m_widenStartFrame) /
                           (Real)(m_widenFinishFrame - m_widenStartFrame);
    m_dirty = true;
...
//-------------------------------------------------------------------------------------------------
void LaserUpdate::initLaser(const Object *parent, const Coord3D *startPos,
                            const Coord3D *endPos, Int sizeDeltaFrames) {
  ...
  if (parent && data->m_parentFireBoneName.isNotEmpty()) {
    ...
    if (data->m_parentFireBoneOnTurret) {
      if (!parent->getSingleLogicalBonePositionOnTurret(
              TURRET_MAIN, data->m_parentFireBoneName.str(), &m_startPos,
              NULL)) {
      ...
      if (!parent->getSingleLogicalBonePosition(
              data->m_parentFireBoneName.str(), &m_startPos, NULL)) {
        // failed to find the required bone, so just die
        TheGameClient->destroyDrawable(getDrawable());
        return;
      }
  ...
  if (!m_particleSystemID) {
    // If we don't have a particle system for the lense flare (muzzle flare),
    // create it.
    if (data->m_particleSystemName.isNotEmpty()) {
      const ParticleSystemTemplate *tmp =
          TheParticleSystemManager->findTemplate(data->m_particleSystemName);
      if (tmp) {
    ...
    if (data->m_targetParticleSystemName.isNotEmpty()) {
      const ParticleSystemTemplate *tmp =
          TheParticleSystemManager->findTemplate(
              data->m_targetParticleSystemName);
      if (tmp) {
        system = TheParticleSystemManager->createParticleSystem(tmp);
        if (system) {
  ...
  if (m_targetParticleSystemID) {
    system =
        TheParticleSystemManager->findParticleSystem(m_targetParticleSystemID);
    if (system) {
...
Real LaserUpdate::getCurrentLaserRadius() const {
  ...
  for (const DrawModule **d = draw->getDrawModules(); *d; ++d) {
    ...
    if (ldi) {
      //***NOTE***
      // While it appears the logic is accessing client data, it is actually
      // accessing template module data from the client. This value is INI
      // constant thus can't change. It's grouped with other laser defining
      // attributes and having it there makes it easier for artists.
