...
// Desc:   The action of this dock update is identifying who is docking and
// either taking Boxes away or giving them
///////////////////////////////////////////////////////////////////////////////////////////////////
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
/*static*/ void SupplyWarehouseDockUpdateModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  DockUpdateModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"StartingBoxes", INI::parseInt, NULL,
       offsetof(SupplyWarehouseDockUpdateModuleData, m_startingBoxesData)},
      {"DeleteWhenEmpty", INI::parseBool, NULL,
       offsetof(SupplyWarehouseDockUpdateModuleData, m_deleteWhenEmpty)},
      {0, 0, 0, 0}};
...
void SupplyWarehouseDockUpdate::onObjectCreated() {
  ...
  if (draw) {
    draw->updateDrawableSupplyStatus(
        getSupplyWarehouseDockUpdateModuleData()->m_startingBoxesData,
        m_boxesStored);
  }
}

Bool SupplyWarehouseDockUpdate::action(Object *docker, Object *drone) {
  if (m_boxesStored == 0) return FALSE;
  ...
  Real closeEnoughSqr =
      sqr(docker->getGeometryInfo().getBoundingCircleRadius() * 2);
  Real curDistSqr = ThePartitionManager->getDistanceSquared(
      docker, getObject(), FROM_BOUNDINGSPHERE_2D);
  ...
  --m_boxesStored;  // so the docker sees that I am shy by one box (or empty)
                    // from within his gainOneBox()

  SupplyTruckAIInterface *ai =
      docker->getAIUpdateInterface()->getSupplyTruckAIInterface();
  if (ai && ai->gainOneBox(m_boxesStored)) {
    if (m_boxesStored == 0 &&
        getSupplyWarehouseDockUpdateModuleData()->m_deleteWhenEmpty) {
    ...
    ++m_boxesStored;  // take it back, since there was noone to gain the box
                      // this is important so that I have one less boxes as
                      // perceived by the docker when he gains one

  return FALSE;
}

void SupplyWarehouseDockUpdate::setDockCrippled(Bool setting) {
  // At this level, Crippling means I kill any activeDocker between enter and
  // exit.
  if (setting) {
    if (m_activeDocker != INVALID_ID) {
      ...
      if (victim) {
        if (m_dockerInside) {
          if (!victim->isUsingAirborneLocomotor()) victim->kill();
        } else {
          // Else, he was between Approach and Enter.  Lucky guy.  Tell him to
          // stop, but then remind him that he wants to try again later
          SupplyTruckAIInterface *supplyTruckAI =
              victim->getAI()->getSupplyTruckAIInterface();
          if (supplyTruckAI) {
...
void SupplyWarehouseDockUpdate::setCashValue(Int cashValue) {
  // A script can tell us our set value, and we need to figure out the boxes
  // needed to provide that.
  m_boxesStored =
      ceil(cashValue / (float)TheGlobalData->m_baseValuePerSupplyBox);
  Drawable *draw = getObject()->getDrawable();
  if (draw) {
    draw->updateDrawableSupplyStatus(
        getSupplyWarehouseDockUpdateModuleData()->m_startingBoxesData,
...
void SupplyWarehouseDockUpdate::loadPostProcess(void) {
  ...
  const SupplyWarehouseDockUpdateModuleData *modData =
      getSupplyWarehouseDockUpdateModuleData();
  Object *us = getObject();
