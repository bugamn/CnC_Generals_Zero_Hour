...
RepairDockUpdateModuleData::RepairDockUpdateModuleData(void) {
  m_framesForFullHeal =
      1.0f;  // 1 frame, instant heal by default (keeps away from divide by 0's)

}  // end RepairDockUpdateModuleData

// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
/*static*/ void RepairDockUpdateModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  DockUpdateModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"TimeForFullHeal", INI::parseDurationReal, NULL,
       offsetof(RepairDockUpdateModuleData, m_framesForFullHeal)},
      {0, 0, 0, 0}};
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
RepairDockUpdate::~RepairDockUpdate(void) {}  // end ~RepairDockUpdate
...
Bool RepairDockUpdate::action(Object *docker, Object *drone) {
  ...
  //
  // no matter if the object is damaged just a little bit, or on the brink of
  // death, we will heal the object over a fixed amount of *TIME* that is
  // specified in the INI file.  Whenever we get a new docker, given its current
  // health we figure out how much health we will add to this docked object each
  // frame so that it is fully healed after the correct amount of time has
  // passed
  //
  if (m_lastRepair == 0) {
    ...
    //
    // figure out how much health we need to add each frame to this object so
    // that it's fully healed at the right time
    //
    m_healthToAddPerFrame = (body->getMaxHealth() - body->getHealth()) /
                            modData->m_framesForFullHeal;
