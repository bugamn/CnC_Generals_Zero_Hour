...
/*static*/ void DockUpdateModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"NumberApproachPositions", INI::parseInt, NULL,
       offsetof(DockUpdateModuleData, m_numberApproachPositionsData)},
      {"AllowsPassthrough", INI::parseBool, NULL,
       offsetof(DockUpdateModuleData, m_isAllowPassthrough)},
      {0, 0, 0, 0}
...
DockUpdate::DockUpdate(Thing *thing, const ModuleData *moduleData)
  ...
  for (Int vectorIndex = 0; vectorIndex < m_approachPositions.size();
       ++vectorIndex) {
...
Bool DockUpdate::isClearToApproach(Object const *docker) const {
  // If we allow infinite approaches, we don't even need to look up.  Just say
  // yes. The reserve code will handle appending a free spot to the end.
  if (m_numberApproachPositions == DYNAMIC_APPROACH_VECTOR_FLAG) return TRUE;

  ObjectID dockerID = docker->getID();

  for (Int positionIndex = 0; positionIndex < m_approachPositionOwners.size();
       ++positionIndex) {
...
Bool DockUpdate::reserveApproachPosition(Object *docker, Coord3D *position,
                                         Int *index) {
  // load dock positions if not loaded yet
  if (m_positionsLoaded == FALSE) loadDockPositions();
  ...
  // If I make it out of the loop, I am full, so dynamic approach buildings
  // should make a new entry instead of saying no
  if (m_numberApproachPositions == DYNAMIC_APPROACH_VECTOR_FLAG) {
    Coord3D zero;
    zero.zero();
...
Bool DockUpdate::advanceApproachPosition(Object *docker, Coord3D *position,
                                         Int *index) {
  // load dock positions if not loaded yet
  if (m_positionsLoaded == FALSE) loadDockPositions();
  ...
  if (m_approachPositionOwners[(*index) - 1] != INVALID_ID) return FALSE;
...
Bool DockUpdate::isClearToAdvance(Object const *docker, Int dockerIndex) const {
  ...
  Bool nextSpotFree = (dockerIndex > 0) &&
                      (m_approachPositionOwners[dockerIndex - 1] == INVALID_ID);
...
void DockUpdate::getEnterPosition(Object *docker, Coord3D *position) {
  ...
  // If I don't have a bone, you are fine where you are, unless you fly, in
  // which case I should recenter you
  Coord3D zero;
  zero.zero();
...
void DockUpdate::onEnterReached(Object *docker) {
  ...
  me->clearAndSetModelConditionFlags(
      ...
      MAKE_MODELCONDITION_MASK2(MODELCONDITION_DOCKING_BEGINNING,
...
void DockUpdate::onDockReached(Object *docker) {
  Object *me = getObject();
  me->clearAndSetModelConditionState(MODELCONDITION_DOCKING_BEGINNING,
                                     MODELCONDITION_DOCKING_ACTIVE);
  docker->clearAndSetModelConditionState(MODELCONDITION_DOCKING_BEGINNING,
                                         MODELCONDITION_DOCKING_ACTIVE);
}
...
void DockUpdate::onExitReached(Object *docker) {
  Object *me = getObject();
  me->clearAndSetModelConditionFlags(
      MAKE_MODELCONDITION_MASK2(MODELCONDITION_DOCKING_ACTIVE,
                                MODELCONDITION_DOCKING),
      MAKE_MODELCONDITION_MASK(MODELCONDITION_DOCKING_ENDING));
  docker->clearAndSetModelConditionFlags(
      MAKE_MODELCONDITION_MASK2(MODELCONDITION_DOCKING_ACTIVE,
  ...
  if (dockerID == m_activeDocker)
    ...
    //
    // we only assert here if the dock is open, for closed docks it's OK to
    // allow somebody to continue moving to the exit position cause they are
    // leaving after all
    //
    if (isDockOpen())
      DEBUG_ASSERTCRASH(FALSE,
                        ("Fiddle.  Someone said goodbye to a dock when the "
                         "dock didn't think it was talking to that someone."));
  }
}
...
void DockUpdate::cancelDock(Object *docker) {
  ...
  if (m_activeDocker == dockerID) {
    ...
    m_activeDocker = INVALID_ID;
    m_dockerInside = FALSE;
    // clear any model conditions related to docking that may be set on us and
    // them. (Normal clear is part of each stage, but we won't get there.)
    ...
    if (dockingObject) dockingObject->clearModelConditionFlags(clear);
  }
}

void DockUpdate::setDockCrippled(Bool setting) {
  // At this level, Crippling means I will accept Approach requests, but I will
  // never grant Enter clearence.
  m_dockCrippled = setting;
}

UpdateSleepTime DockUpdate::update() {
  if (m_activeDocker == INVALID_ID && !m_dockCrippled) {
    // if setDockCrippled has been called, I will never give enterance
    // permission.
    ...
    if (docker && docker->isKindOf(KINDOF_DOZER) &&
        docker->isKindOf(KINDOF_HARVESTER))  // a worker
    {
      ModelConditionFlags test;
      test.set(MODELCONDITION_DOCKING_BEGINNING);
      Drawable *dockerDraw = docker->getDrawable();
      if (dockerDraw &&
          dockerDraw->getModelConditionFlags().anyIntersectionWith(test))
        dockerDraw->clearModelConditionFlags(
            MAKE_MODELCONDITION_MASK(MODELCONDITION_MOVING));
    }
  }
...
Coord3D DockUpdate::computeApproachPosition(Int positionIndex,
  ...
  Coord3D bestPosition;     // This answer is the best, as it includes
                            // findPositionAround
  Coord3D workingPosition;  // But if findPositionAround fails, we need to say
                            // something.

  FindPositionOptions fpOptions;
  // Start with the pristine bone, then convert it to the world, then find a
  // clean spot around it.

  Object *us = getObject();
  us->convertBonePosToWorldPos(&m_approachPositions[positionIndex], NULL,
  ...
  if (m_numberApproachPositionBones == 0) {
    ...
    // A Boneless building wants to bias towards the caller for the arbitrary
    // position
    Vector3 offset(theirPosition.x - ourPosition.x,
                   theirPosition.y - ourPosition.y,
                   theirPosition.z - ourPosition.z);
    offset.Normalize();
...
void DockUpdate::loadDockPositions() {
  ...
  if (myDrawable) {
    myDrawable->getPristineBonePositions("DockStart", 0, &m_enterPosition, NULL,
                                         1);
    myDrawable->getPristineBonePositions("DockAction", 0, &m_dockPosition, NULL,
                                         1);
    myDrawable->getPristineBonePositions("DockEnd", 0, &m_exitPosition, NULL,
                                         1);
    if (m_numberApproachPositions != DYNAMIC_APPROACH_VECTOR_FLAG) {
      ...
      m_numberApproachPositionBones = myDrawable->getPristineBonePositions(
          "DockWaiting", 1, approachBones, NULL, m_numberApproachPositions);
      if (m_numberApproachPositions ==
          m_approachPositions.size())  // safeguard: will always be true
      {
        for (Int copyIndex = 0; copyIndex < m_numberApproachPositions;
             ++copyIndex) {
...
void DockUpdate::xfer(Xfer *xfer) {
  ...
  for (Int vectorIndex = 0; vectorIndex < vectorSize; ++vectorIndex) {
    // Okay, this is cool.  On save, the size and a bunch of coords will be
    // written. on load, vectorSize will be at 0 from the .size, but will then
    // get set by the xfer, and properly control the number of Coords.
