...
//-------------------------------------------------------------------------------------------------
/*static*/ void PilotFindVehicleUpdateModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  ModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"ScanRate", INI::parseDurationUnsignedInt, NULL,
       offsetof(PilotFindVehicleUpdateModuleData, m_scanFrames)},
      {"ScanRange", INI::parseReal, NULL,
       offsetof(PilotFindVehicleUpdateModuleData, m_scanRange)},
      {"MinHealth", INI::parseReal, NULL,
       offsetof(PilotFindVehicleUpdateModuleData, m_minHealth)},
      {0, 0, 0, 0}};
  p.add(dataFieldParse);
}

//-------------------------------------------------------------------------------------------------
PilotFindVehicleUpdate::PilotFindVehicleUpdate(Thing *thing,
...
UpdateSleepTime PilotFindVehicleUpdate::update() {
  ...
  const PilotFindVehicleUpdateModuleData *data =
      getPilotFindVehicleUpdateModuleData();
  ...
  if (upgradeUnit) {
    ...
    if (!m_didMoveToBase &&
        obj->getControllingPlayer()->getAiBaseCenter(&pos)) {
...
Object *PilotFindVehicleUpdate::scanClosestTarget() {
  ...
  PartitionFilterAcceptByKindOf kindFilter(MAKE_KINDOF_MASK(KINDOF_VEHICLE),
                                           KINDOFMASK_NONE);
