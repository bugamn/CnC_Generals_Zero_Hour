...
EnemyNearUpdate::EnemyNearUpdate(Thing *thing, const ModuleData *moduleData)
  ...
  m_enemyScanDelay += GameLogicRandomValue(
      0, getEnemyNearUpdateModuleData()->m_enemyScanDelayTime);
}
...
void EnemyNearUpdate::checkForEnemies(void) {
  ...
  if (m_enemyScanDelay == 0) {
    ...
    Object *enemy =
        TheAI->findClosestEnemy(getObject(), visionRange, AI::CAN_SEE);
...
UpdateSleepTime EnemyNearUpdate::update() {
  ...
  if (m_enemyNear && !enemyWasNear) {
    ...
    if (draw) draw->setModelConditionState(MODELCONDITION_ENEMYNEAR);
  } else if (!m_enemyNear && enemyWasNear) {
    // change the state of the art to an idle state
    Drawable *draw = getObject()->getDrawable();
    if (draw) draw->clearModelConditionState(MODELCONDITION_ENEMYNEAR);
  }
