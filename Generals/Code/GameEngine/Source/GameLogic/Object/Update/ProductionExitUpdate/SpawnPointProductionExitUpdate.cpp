...
SpawnPointProductionExitUpdate::SpawnPointProductionExitUpdate(
  ...
  m_spawnPointCount = 0;
  for (Int positionIndex = 0; positionIndex < MAX_SPAWN_POINTS;
       positionIndex++) {
...
void SpawnPointProductionExitUpdate::exitObjectViaDoor(Object *newObj,
  ...
  if (!m_bonesInitialized) initializeBonePositions();

  Object *creationObject = getObject();
  if (creationObject) {
    for (Int positionIndex = 0; positionIndex < m_spawnPointCount;
         positionIndex++) {
      if (m_spawnPointOccupier[positionIndex] == INVALID_ID) break;
    }
    ...
    createPoint.z = TheTerrainLogic ? TheTerrainLogic->getLayerHeight(
                                          createPoint.x, createPoint.y,
                                          creationObject->getLayer())
                                    : 0.0f;
...
//-------------------------------------------------------------------------------------------------
ExitDoorType SpawnPointProductionExitUpdate::reserveDoorForExit(
    const ThingTemplate *objType, Object *specificObject) {
  if (!m_bonesInitialized) initializeBonePositions();

  if (!m_bonesInitialized) return DOOR_NONE_AVAILABLE;  // Init failure

  revalidateOccupiers();

  for (Int positionIndex = 0; positionIndex < m_spawnPointCount;
       positionIndex++) {
    if (m_spawnPointOccupier[positionIndex] == INVALID_ID) return DOOR_1;
  }
...
//-------------------------------------------------------------------------------------------------
void SpawnPointProductionExitUpdate::unreserveDoorForExit(
    ExitDoorType exitDoor) {
  /* nothing */
}
...
void SpawnPointProductionExitUpdate::initializeBonePositions() {
  ...
  const SpawnPointProductionExitUpdateModuleData *md =
      getSpawnPointProductionExitUpdateModuleData();
  m_spawnPointCount = myDrawable->getPristineBonePositions(
      md->m_spawnPointBoneNameData.str(), 1, NULL, boneTransforms,
      MAX_SPAWN_POINTS);
  ...
  for (matrixIndex = 0; matrixIndex < m_spawnPointCount; matrixIndex++) {
    ...
    me->convertBonePosToWorldPos(NULL, currentTransform, NULL,
                                 currentTransform);

    // Then save the world coord and angle
    m_worldCoordSpawnPoints[matrixIndex].x =
        currentTransform->Get_X_Translation();
    m_worldCoordSpawnPoints[matrixIndex].y =
        currentTransform->Get_Y_Translation();
...
void SpawnPointProductionExitUpdate::revalidateOccupiers() {
  for (Int positionIndex = 0; positionIndex < m_spawnPointCount;
       positionIndex++) {
    if (m_spawnPointOccupier[positionIndex] == INVALID_ID) continue;

    if (TheGameLogic->findObjectByID(m_spawnPointOccupier[positionIndex]) ==
        NULL)
      m_spawnPointOccupier[positionIndex] = INVALID_ID;
  }
}
...
void SpawnPointProductionExitUpdate::xfer(Xfer *xfer) {
  ...
  //
  // we can ignore all the data with the bone position arrays and spawn point
  // counts because this module will load them lazily
  //
  //	Bool m_bonesInitialized;
  /////< To prevent creation bugs, only init the World coords when first asked
  //for one 	Int m_spawnPointCount;
  /////< How many in the array are actually live and valid 	Coord3D
  //m_worldCoordSpawnPoints[MAX_SPAWN_POINTS];///< Where my little friends will
  //be created 	Real m_worldAngleSpawnPoints[MAX_SPAWN_POINTS];		///< And
  //what direction they should face
