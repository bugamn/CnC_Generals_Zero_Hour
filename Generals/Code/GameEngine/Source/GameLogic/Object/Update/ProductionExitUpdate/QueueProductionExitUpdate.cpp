...
//					This instance refuses to spit a second
//out until the first is clear
///////////////////////////////////////////////////////////////////////////////////////////////////
...
QueueProductionExitUpdate::QueueProductionExitUpdate(
  ...
  const QueueProductionExitUpdateModuleData *md =
      getQueueProductionExitUpdateModuleData();
  if (md)  // sanity
  {
...
//-------------------------------------------------------------------------------------------------
void QueueProductionExitUpdate::exitObjectViaDoor(Object *newObj,
  ...
  if (creationObject) {
    ...
    if (creationInAir && !md->m_allowAirborneCreationData)
      loc.Z = TheTerrainLogic ? TheTerrainLogic->getGroundHeight(loc.X, loc.Y)
                              : 0.0f;
    ...
    //
    // Objects that are created in the air from producers that are in the air
    // get a kick that will make their starting speed equal mine.
    //
    ...
    if ((myPhysics != NULL) && creationInAir && (newObjectPhysics != NULL)) {
      ...
      Real pitchRate =
          newObjectPhysics->getCenterOfMassOffset() * STARTING_PITCH_COEFF;
...
//-------------------------------------------------------------------------------------------------
ExitDoorType QueueProductionExitUpdate::reserveDoorForExit(
...
//-------------------------------------------------------------------------------------------------
void QueueProductionExitUpdate::exitObjectByBudding(Object *newObj,
                                                    Object *budHost) {
  ...
  if (ai) {
    ai->aiMoveToPosition(
        newObj->getPosition(),
        CMD_FROM_AI);  // IOW, I cant stay here, if somebody else is here
  }
...
Bool QueueProductionExitUpdate::getNaturalRallyPoint(Coord3D &rallyPoint,
                                                     Bool offset) const {
  const QueueProductionExitUpdateModuleData *data =
      getQueueProductionExitUpdateModuleData();
  Vector3 p;
