...
// Desc:   Allows hijacker to kepp with his hijacked vehicle (though hidden)
// until it dies, then to become a hijacker once more
//
/////////////////////////////////////////////////////////////////////////////////////////////////
...
UpdateSleepTime HijackerUpdate::update(void) {
  ...
  if (m_isInVehicle) {
    ...
    // Continually reset position of hijacker to match the position of the
    // target.
    ...
    if (target) {
      ...
      // So, if while I am driving this American war vehicle, I gain skill
      // points, I get to keep them when I wreck the vehicle
    ...
    } else  // the car we have been "driving" is dead now, and has safely
            // ejected us
    {
      {
        ...
        if (obj->getDrawable()) {
          // so it is time to unhide ourselves and be a pedestrian hijacker
          // again
          obj->getDrawable()->setDrawableHidden(false);
        }

        // We won't come back here until and unless we have hijacked another
        // vehicle
        ...
        if (m_wasTargetAirborne) {
          const ThingTemplate *putInContainerTmpl =
              TheThingFactory->findTemplate(
                  getHijackerUpdateModuleData()->m_parachuteName);
          DEBUG_ASSERTCRASH(
              putInContainerTmpl,
              ("DeliverPayload: PutInContainer %s not found!",
               getHijackerUpdateModuleData()->m_parachuteName.str()));
          if (putInContainerTmpl) {
            Object *container =
                TheThingFactory->newObject(putInContainerTmpl, obj->getTeam());
