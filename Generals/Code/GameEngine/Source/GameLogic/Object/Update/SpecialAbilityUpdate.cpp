...
//-------------------------------------------------------------------------------------------------
SpecialAbilityUpdate::SpecialAbilityUpdate(Thing *thing,
  ...
  m_doDisableFXParticles =
      TRUE;  // true always, unless small unit causes it to toggle on-off
  setWakeFrame(getObject(), UPDATE_SLEEP_FOREVER);
  //  This is the althernate way to one-at-a-time BlackLotus' specials; we'll
  //  keep it commented her until Dustin decides, or until 12/10/02
  //	setBusy( FALSE );
}

//-------------------------------------------------------------------------------------------------
SpecialAbilityUpdate::~SpecialAbilityUpdate(void) { onExit(true); }

/*------------------------------------------------------------------------------------------------
void SpecialAbilityUpdate::update( void )

This is the brains of the entire special ability update. There are several
optional steps and variations that can be processed for any particular type of
special ability. A special ability that has every option will do the following
in order:

        1 -- APPROACH: If I'm not close enough to the target, then approach it
        2 -- UNPACK: If I need to unpack before I can prepare, then do so now
(this uses the model condition unpack). 3 -- PREPARE: If I need to perform a
task for a period of time before I can trigger my special ability, then do so
now. A good example is aiming with a targetting laser for a few seconds before
firing your special weapon. 4 -- TRIGGER: Once preparation is complete, fire
your special ability now. 5 -- PACK: If I need to pack after finishing my
attack, do so now. 6 -- FINISH: Clean up the states, and turn off the update.

Variations:

        Persistent Specials -- A persistent special will continually trigger
it's effect every so often and never end. A good example of this is the disable
building hack. The hacker will run up to the target building, unpack, prepare
(firing hack stream), then after a period of time, the building becomes
disabled. But because it's persistent, we reset the preparation and trigger the
building disabled code over and over again -- which is on a timer. No Target
Specials -- You can link two different main specials together. Colonel Burton
has the ability to lay C4 charges on multiple targets. Activating these specials
require a target. The non-target version is actually the detonator, which goes
through all the special objects and detonates each one of them.

Options:
        SpecialPowerTemplate -- Defines the special power template that links to
a command. StartAbilityRange		 -- Specify how far you want your unit
to be from the target before start your special ability. AbilityAbortRange
-- After starting an attack, it'll allow preparation unless the target goes
                                beyond this range. If this happens, the ability
is aborted outright. PreparationTime			 -- How long it takes to
prepare your special once in position and unpacked. PersistentPrepTime	 -- This
value defines whether or not you are using a persistent special. Once the
special ability is triggered, it'll wait until this specified delay occurs and
it'll trigger it again, for ever until the unit dies, the target dies, or the
unit decides to do something else. PackTime
-- How long it takes to pack up the unit after triggering a non persistent
                                                                                                        special ability or after ordering the unit to do something else, or the
                                                                                                        target dies (if applicable).
        PackUnpackVariationFactor -- Randomizes the pack and unpack time by
specified range. The closer to zero, the smaller the variation. 0.2 would have a
pack or unpack range of xTime +/- 20%. This is important because it represents
averages. UnpackTime					 -- Same as PackTime,
except used once entering range of target. SkipPackingWithNoTarget -- This
option is used by the No Target Special variation, and only uses
        ...
        SpecialObject				 -- Defines the special object
the unit will create via the special ability. In one case, it creates and
maintains the laser or binary stream during preparation. In Colonel Burton's
case, it keeps track of the C4 charges that he has placed. MaxSpecialObjects
-- Defines the max number of special objects that can exist at any given time.
The laser example only has one, but the C4 charges can have more.
        SpecialObjectsPersistent -- If this flag is set, then the objects will
remain should the owner decides to do something else... C4 charges are a good
example. EffectDuration			 -- Defines the duration of the special
ability. In the case of disabling the building (hacker), this value will dictate
how long the building will be disabled should the hacker die or stop the attack.
        UniqueSpecialObjectTargets -- Prevents the owner from placing multiple
special objects on the same target. C4 charges once again.
        SpecialObjectsPersistWhenOwnerDies -- If the owner dies, the special
objects will remain in the world. Timed C4 charges is a good example, but remote
C4 charges is a bad example -- because it requires the owner to detonate them.
        FlipObjectAfterPacking -- Simply rotates the object 180 degrees after
packing (due to special animation case). FlipObjectAfterUnPacking -- Simply
rotates the object 180 degrees after unpacking (due to special	animation case).
Used by colonel burton after planting charge.
...
UpdateSleepTime SpecialAbilityUpdate::update(void) {
  ...
  // Note: This will be complicated because one difficult example is handling
  // Burton when he dies while
  //       bombs are in the world.

  // Validation of special objects makes sure that they still exist. Sometimes
  // special objects can be destroyed or removed, or detonate, etc! When they go
  // missing, they are removed from the special object list and free up slots
  // for those units that can have multiple. We need to do this now because it's
  // possible the special objects need to be checked while the the special
  // ability is over (thus inactive).
  validateSpecialObjects();

  // Important! This check will see if there has been any commands issued by
  // either the player or script. When told to do something else, we need to
  // immediately cleanup our special ability. This also means some things might
  // be left around like timed charges to detonate.
  ...
  if (m_targetID != INVALID_ID) {
    ...
    if (target != NULL) {
      const SpecialAbilityUpdateModuleData *data =
  ...
  if (!isPreparationComplete()) {
    // The special ability has fired, now continue to process the special
    // ability until it expires
    m_prepFrames--;
...
//-------------------------------------------------------------------------------------------------
void SpecialAbilityUpdate::initiateIntentToDoSpecialPower(
    const SpecialPowerTemplate *specialPowerTemplate, const Object *targetObj,
    const Coord3D *targetPos, UnsignedInt commandOptions, Int locationCount) {
  const SpecialAbilityUpdateModuleData *data =
      getSpecialAbilityUpdateModuleData();
  ...
  getObject()->clearModelConditionFlags(MAKE_MODELCONDITION_MASK4(
      MODELCONDITION_UNPACKING, MODELCONDITION_PACKING, MODELCONDITION_FIRING_A,
  ...
  // Determine whether we are triggering a command (rather than executing
  // special at location or target)
  m_noTargetCommand = !targetObj && !targetPos;

  if (data->m_unpackTime == 0 ||
      m_noTargetCommand && data->m_skipPackingWithNoTarget) {
    // Only unpack if we need to -- setting it to unpacked will skip step 2 in
    // the update
  ...
  // Prevent other mutually exclusive specials from running (kill them now if
  // we're starting something else)
  SpecialAbilityUpdate *disableSA;
  disableSA = getObject()->findSpecialAbilityUpdate(
      SPECIAL_BLACKLOTUS_DISABLE_VEHICLE_HACK);
  if (disableSA && disableSA != this) disableSA->onExit(FALSE);
  disableSA =
      getObject()->findSpecialAbilityUpdate(SPECIAL_BLACKLOTUS_STEAL_CASH_HACK);
  if (disableSA && disableSA != this) disableSA->onExit(FALSE);
  disableSA = getObject()->findSpecialAbilityUpdate(
...
//-------------------------------------------------------------------------------------------------
Bool SpecialAbilityUpdate::isPowerCurrentlyInUse(
    const CommandButton *command) const {
  if (command) {
    if (command->getSpecialPowerTemplate() &&
      ...
      if (!BitTest(command->getOptions(), CONTEXTMODE_COMMAND)) {
        // This is the detonate charge button. Treat it backwards saying it's in
        // use when we don't have any special objects (charges). That way, the
  ...
  if (m_packingState != STATE_NONE) {
    // exception for powers with zero reload time... they are ready to use
    // immediately!
...
void SpecialAbilityUpdate::onExit(Bool cleanup) {
  ...
  if (!data->m_specialObjectsPersistent ||
    ...
    // Delete special objects that aren't considered persistent whenever we turn
    // off leave the special ability update.
    killSpecialObjects();
  }
  ...
  // no, actually, we DON'T want to call this here, since onExit is always
  // called (directly or indirectly) from update()... and calling setWakeFrame()
  // from your own update() method is a no-no (since it would just be ignored in
  // favor of the return value from update() anyway). just set m_active to
  // false, and we'll put ourselves to sleep.
  //	setWakeFrame(getObject(), UPDATE_SLEEP_FOREVER);
}

//-------------------------------------------------------------------------------------------------
// Some special abilities requires packing or unpacking. When setup, all this
// function does is decrement the counter and clear the model state when
// finished. Returns TRUE if we are still packing or unpacking.
//-------------------------------------------------------------------------------------------------
Bool SpecialAbilityUpdate::handlePackingProcessing() {
  ...
  if (m_animFrames > 0) {
    ...
    if (m_animFrames == 0) {
      ...
      getObject()->clearModelConditionFlags(MAKE_MODELCONDITION_MASK2(
          MODELCONDITION_UNPACKING, MODELCONDITION_PACKING));
    ...
    if (getSpecialAbilityUpdateModuleData()->m_loseStealthOnTrigger &&
        m_animFrames <
            getSpecialAbilityUpdateModuleData()->m_preTriggerUnstealthFrames) {
      static NameKeyType key_StealthUpdate = NAMEKEY("StealthUpdate");
      StealthUpdate *stealth =
          (StealthUpdate *)getObject()->findUpdateModule(key_StealthUpdate);
      if (stealth) {
...
void SpecialAbilityUpdate::startPacking(Bool success) {
  ...
  Real variation =
      GameLogicRandomValueReal(1.0f - data->m_packUnpackVariationFactor,
                               1.0f + data->m_packUnpackVariationFactor);
  m_animFrames = data->m_packTime * variation;

  // Set the animation state
  getObject()->clearAndSetModelConditionFlags(
      MAKE_MODELCONDITION_MASK2(MODELCONDITION_UNPACKING,
                                MODELCONDITION_RAISING_FLAG),
  ...
  if (draw) draw->setAnimationCompletionTime(m_animFrames);
  AIUpdateInterface *ai = getObject()->getAIUpdateInterface();
  if (ai) ai->aiBusy(CMD_FROM_AI);

  if (success) {
    AudioEventRTS event;
    switch (data->m_specialPowerTemplate->getSpecialPowerType()) {
      // case SPECIAL_HACKER_DISABLE_BUILDING:// Awaiting Mical's new sound
      //	event = *getObject()->getTemplate()->getPerUnitSound(
      //"VoiceDisableBuildingComplete" ); 	break;
      case SPECIAL_BLACKLOTUS_CAPTURE_BUILDING:
        event = *getObject()->getTemplate()->getPerUnitSound(
...
Bool SpecialAbilityUpdate::isWithinStartAbilityRange() const {
  ...
  // Quickly convert very short range approachs to "contact" class requiring
  ...
  if (m_targetID != INVALID_ID) {
    ...
    if (target) {
      fDistSquared = ThePartitionManager->getDistanceSquared(
          self, target, FROM_BOUNDINGSPHERE_2D);
    }
  } else if (m_targetPos.x || m_targetPos.y ||
             m_targetPos.z)  // It's zero if not used...
  {
    fDistSquared = ThePartitionManager->getDistanceSquared(
        self, &m_targetPos, FROM_BOUNDINGSPHERE_2D);
  } else {
  ...
  Real fStartRangeSquared =
      data->m_startAbilityRange * data->m_startAbilityRange;
  if (fDistSquared <= fStartRangeSquared) {
    if (range == 0.0f && m_targetID != INVALID_ID) {
      ...
      ObjectIterator *iter = ThePartitionManager->iteratePotentialCollisions(
          self->getPosition(), self->getGeometryInfo(), 0.0f);
    ...
    if (data->m_approachRequiresLOS) {
      ...
      ObjectIterator *iter = ThePartitionManager->iterateObjectsInRange(
          self, range, FROM_BOUNDINGSPHERE_2D, filters,
          ITER_SORTED_NEAR_TO_FAR);
      for (Object *theTarget = iter->first(); theTarget;
           theTarget = iter->next()) {
        // LOS check succeeded.
        if (target == theTarget) {
          return true;
...
Bool SpecialAbilityUpdate::isWithinAbilityAbortRange() const {
  ...
  // Check to see how far we are from the target!
  Real fStartRangeSquared =
      data->m_abilityAbortRange * data->m_abilityAbortRange;
  if (fDistSquared <= fStartRangeSquared) {
    if (range == 0.0f && m_targetID != INVALID_ID) {
      // We want to ensure we collided with our target first!
      ObjectIterator *iter = ThePartitionManager->iteratePotentialCollisions(
...
void SpecialAbilityUpdate::startPreparation() {
  ...
  switch (spTemplate->getSpecialPowerType()) {
    case SPECIAL_MISSILE_DEFENDER_LASER_GUIDED_MISSILES: {
      Object *target = TheGameLogic->findObjectByID(m_targetID);
      if (target) {
        // Specialized code that specifically creates and looks up a laser
        // update.
        Object *specialObject = createSpecialObject();
        if (specialObject) {
          if (!initLaser(specialObject, target)) return;
        }
      }
      break;
    }
    case SPECIAL_INFANTRY_CAPTURE_BUILDING: {
      ...
      Drawable *draw = getObject()->getDrawable();
      if (draw) draw->setAnimationCompletionTime(data->m_preparationFrames);
    ...
    case SPECIAL_BLACKLOTUS_STEAL_CASH_HACK: {
      ...
      if (target) {
        ...
        // Warn the victim so he might have a chance to react!
        if (spTemplate->getSpecialPowerType() ==
                SPECIAL_BLACKLOTUS_CAPTURE_BUILDING &&
            target && target->isLocallyControlled()) {
          TheEva->setShouldPlay(EVA_BuildingBeingStolen);
        }
      }
      break;
  ...
  // Trigger the reset timer on the special power because we're officially
  // starting it now!
  SpecialPowerModuleInterface *spmInterface = getMySPM();
  if (spmInterface) {
    spmInterface->markSpecialPowerTriggered(
        NULL);  // Null for not creating a view object
  }
...
Bool SpecialAbilityUpdate::initLaser(Object *specialObject, Object *target) {
  ...
  Coord3D startPos;
  if (!getObject()->getSingleLogicalBonePosition(
          data->m_specialObjectAttachToBoneName.str(), &startPos, NULL)) {
...
Bool SpecialAbilityUpdate::continuePreparation() {
  ...
  switch (spTemplate->getSpecialPowerType()) {
    ...
    case SPECIAL_BLACKLOTUS_DISABLE_VEHICLE_HACK: {
      ...
      for (std::list<ObjectID>::iterator it = m_specialObjectIDList.begin();
    ...
    case SPECIAL_BLACKLOTUS_CAPTURE_BUILDING:
      ...
      if (data->m_doCaptureFX) {
        ...
        {
          Bool lastPhase = (((Int)m_captureFlashPhase) &
                            1);  // were we in a flashy phase last frame?
          ...
          Bool thisPhase = (((Int)m_captureFlashPhase) &
                            1);  // are we in a flashy phase this frame?
          ...
          if (lastPhase && (!thisPhase)) {
            ...
            targetDraw->flashAsSelected(
                &myHouseColor);  // In MY house color, not his!
      ...
      SpecialPowerModuleInterface *spmInterface = getMySPM();
      if (spmInterface && spTemplate->getSpecialPowerType() ==
                              SPECIAL_INFANTRY_CAPTURE_BUILDING) {
        // these keep resetting the recharge timer while in use. (srj) // only
        // for infantry capture, not black lotus which resets in
        // triggerAbilityEffect()
...
void SpecialAbilityUpdate::triggerAbilityEffect() {
  ...
  // Award experience to units for triggering the ability (optional and ini
  // specified). NOTE: Be award of persistant abilities that call trigger over
  // and over again!
  ...
  // Also add skill points. If unspecified, it'll use the award experience to
  // units for triggering value.
  Int skillPoints = data->m_skillPointsForTriggering != -1
                        ? data->m_skillPointsForTriggering
  ...
  switch (spTemplate->getSpecialPowerType()) {
    ...
    case SPECIAL_TIMED_CHARGES: {
      ...
      if (charge) {
        ...
        StickyBombUpdate *update =
            (StickyBombUpdate *)charge->findUpdateModule(key_StickyBombUpdate);
        if (!update) {
          DEBUG_ASSERTCRASH(0, ("Unit '%s' attempted to place %s on %s but the "
                                "bomb requires a StickyBombUpdate module.",
        ...
        // Setting the producer ID allows the sticky bomb update module to
        // initialize and setup timers, etc.
    ...
    case SPECIAL_BLACKLOTUS_DISABLE_VEHICLE_HACK: {
      ...
      target->setDisabledUntil(
          DISABLED_HACKED, TheGameLogic->getFrame() + data->m_effectDuration);
      ...
      if ((targetFootprintArea < 300) && target->isKindOf(KINDOF_STRUCTURE)) {
        m_doDisableFXParticles =
            !m_doDisableFXParticles;  // toggle if small building
      ...
      if (m_doDisableFXParticles) {
        ...
        if (tmp) {
          ...
          if (sys) {
            ...
            sys->setSystemLifetime(
                data->m_effectDuration *
                durationInterleaveFactor);  // lifetime of the system, not the
                                            // particles
          }
        }
      }
    ...
    case SPECIAL_BLACKLOTUS_CAPTURE_BUILDING: {
      ...
      // Just in case we are capturing a building which is already garrisoned by
      // other
      ContainModuleInterface *contain = target->getContain();
      if (contain && contain->isGarrisonable()) {
        contain->removeAllContained(TRUE);
        break;  // we do not want to set a neutral building to our team if we
                // are not in it, that would be confusing!
      }
      ...
      target->defect(object->getControllingPlayer()->getDefaultTeam(),
                     1);  // one frame of flash!

      SpecialPowerModuleInterface *spmInterface = getMySPM();
      if (spmInterface && spTemplate->getSpecialPowerType() ==
                              SPECIAL_BLACKLOTUS_CAPTURE_BUILDING) {
        // only for black lotus, not infantry capture which resets in
        // contunueprep()
        spmInterface->startPowerRecharge();
      }
      break;
    ...
    case SPECIAL_BLACKLOTUS_STEAL_CASH_HACK: {
      ...
      if (targetMoney && objectMoney) {
        ...
        if (cash > 0) {
          ...
          if (controller) controller->getScoreKeeper()->addMoneyEarned(cash);
    ...
    case SPECIAL_REMOTE_CHARGES: {
      static NameKeyType key_StickyBombUpdate = NAMEKEY("StickyBombUpdate");
      if (m_targetID == INVALID_ID && !m_targetPos.x && !m_targetPos.y &&
          !m_targetPos.z) {
        // If there is no target object nor position, then we are detonating the
        // existing charges.
        ...
        for (i = m_specialObjectIDList.begin();
          ...
          if (specialObject) {
            StickyBombUpdate *update =
                (StickyBombUpdate *)specialObject->findUpdateModule(
                    key_StickyBombUpdate);
            if (update) {
              // Blow it up!!!
              update->detonate();
              okToLoseStealth = FALSE;
              // Note: while the objects are detonating, they will still exist
              // in the game. Our update will be responsible for validating
              // their existance and removing them.. in case either the enemy
              // player cleans one up, or after it's gone.
            }
          }
        }
      } else {
        ...
        if (charge) {
          StickyBombUpdate *update =
              (StickyBombUpdate *)charge->findUpdateModule(
                  key_StickyBombUpdate);
          if (!update) {
            DEBUG_ASSERTCRASH(
                0, ("Unit '%s' attempted to place remote charge but the charge "
                    "'%s' requires a StickyBombUpdate module.",
                    object->getTemplate()->getName().str(),
                    charge->getTemplate()->getName().str()));
            killSpecialObjects();
            return;
...
Object *SpecialAbilityUpdate::createSpecialObject() {
  ...
  if (thingTemplate) {
    ...
    specialObject =
        TheThingFactory->newObject(thingTemplate, getObject()->getTeam());
    if (specialObject) {
      ...
      // specialObject->setProducer( getObject() ); --This causes it to be an
      // enemy which is naughty.
...
void SpecialAbilityUpdate::killSpecialObjects() {
  ...
  std::list<ObjectID>::iterator i;
  for (i = m_specialObjectIDList.begin(); i != m_specialObjectIDList.end();
       ++i) {
...
void SpecialAbilityUpdate::finishAbility() {
  ...
  // The flee range code is used only if the unit has completed a special
  // ability against a target. If we don't have a valid target, then we don't
  // flee -- the specific example is when Colonel Burton uses his remote charge
  // special ability. With a target, he will plant the charge then flee. Without
  // a target, he simply detonates it without fleeing.
  Bool validTarget = m_targetPos.x || m_targetPos.y || m_targetPos.z ||
                     m_targetID != INVALID_ID;
  if (data->m_fleeRangeAfterCompletion && validTarget) {
    ...
    if (ai) {
      ...
      // Now check for mines.  Normally we are fleeing from a bomb we just
      // planted. It is not good to run back towards the previous mine we just
      // planted about the time it explodes. jba.
      Object *obj = getObject();
      if (obj) {
        Player *contPlayer = obj->getControllingPlayer();
        if (contPlayer) {
          PartitionFilterSamePlayer filterPlayer(
              contPlayer);  // Look for our own mines.
          PartitionFilterAcceptByKindOf filterKind(
              MAKE_KINDOF_MASK(KINDOF_MINE), KINDOFMASK_NONE);
          PartitionFilter *filters[] = {&filterKind, &filterPlayer, NULL};
          Object *mine = ThePartitionManager->getClosestObject(
              &pos, data->m_fleeRangeAfterCompletion, FROM_CENTER_2D,
              filters);  // could be null. this is ok.
          if (mine) {
            dir.set(pos.x - mine->getPosition()->x,
                    pos.y - mine->getPosition()->y, 0);
            dir.normalize();
            dir.scale(data->m_fleeRangeAfterCompletion);
...
//-------------------------------------------------------------------------------------------------
Object *SpecialAbilityUpdate::findSpecialObjectWithProducerID(
    const Object *target) {
  Object *specialObject;
  std::list<ObjectID>::iterator i;
  for (i = m_specialObjectIDList.begin(); i != m_specialObjectIDList.end();
...
void SpecialAbilityUpdate::endPreparation() {
  ...
  // Based on the special that we just finished preparing (either by failure or
  // success), do we want to keep the "special objects" created? Some specials
  // will -- others won't. Note that persistant specials will not call this
  // until preparation is complete (not recycling).
  const SpecialAbilityUpdateModuleData *data =
      getSpecialAbilityUpdateModuleData();
  const SpecialPowerTemplate *spTemplate = data->m_specialPowerTemplate;
  switch (spTemplate->getSpecialPowerType()) {
