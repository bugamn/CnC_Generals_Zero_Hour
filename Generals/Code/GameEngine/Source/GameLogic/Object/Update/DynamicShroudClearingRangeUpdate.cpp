...
//-------------------------------------------------------------------------------------------------
/*static*/ void DynamicShroudClearingRangeUpdateModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  UpdateModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"ChangeInterval", INI::parseDurationUnsignedInt, NULL,
       offsetof(DynamicShroudClearingRangeUpdateModuleData, m_changeInterval)},
      {"GrowInterval", INI::parseDurationUnsignedInt, NULL,
       offsetof(DynamicShroudClearingRangeUpdateModuleData, m_growInterval)},
      {"ShrinkDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(DynamicShroudClearingRangeUpdateModuleData, m_shrinkDelay)},
      {"ShrinkTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(DynamicShroudClearingRangeUpdateModuleData, m_shrinkTime)},
      {"GrowDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(DynamicShroudClearingRangeUpdateModuleData, m_growDelay)},
      {"GrowTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(DynamicShroudClearingRangeUpdateModuleData, m_growTime)},
      {"FinalVision", INI::parseReal, NULL,
       offsetof(DynamicShroudClearingRangeUpdateModuleData, m_finalVision)},
      {"GridDecalTemplate", RadiusDecalTemplate::parseRadiusDecalTemplate, NULL,
       offsetof(DynamicShroudClearingRangeUpdateModuleData,
                m_gridDecalTemplate)},
      {0, 0, 0, 0}};
...
//  |<-----------------------------------stateCountDown---------------------------------->|
//  |  shrinktime    |                                     |   growtime  |
//  growdelay    | |                shrinkStartDeadline sustain
//  growStartDeadline|
//
//-------------------------------------------------------------------------------------------------
DynamicShroudClearingRangeUpdate::DynamicShroudClearingRangeUpdate(
  ...
  DynamicShroudClearingRangeUpdateModuleData *md =
      (DynamicShroudClearingRangeUpdateModuleData *)moduleData;
  ...
  DEBUG_ASSERTCRASH((m_sustainDeadline >= m_shrinkStartDeadline),
                    ("DynamicShroudClearingRangeUpdate\nYOU WILL NEVER REACH "
                     "FULL SHROUD CLEARING RANGE WITH THESE TIMES AND DELAYS"));
  DEBUG_ASSERTCRASH((m_growStartDeadline >= m_shrinkStartDeadline),
                    ("DynamicShroudClearingRangeUpdate\nYOU WILL NEVER REACH "
                     "FULL SHROUD CLEARING RANGE WITH THESE TIMES AND DELAYS"));

  m_doneForeverFrame =
      TheGameLogic->getFrame() +
      m_stateCountDown;  // a failsafe to force a shutdown on schedule
  ...
  m_nativeClearingRange =
      obj->getShroudClearingRange();  // capture this before we start monkeying
                                      // with it
...
void DynamicShroudClearingRangeUpdate::createGridDecals(
    const RadiusDecalTemplate &tmpl, Real radius, const Coord3D &pos) {
  for (int d = 0; d < GRID_FX_DECAL_COUNT; ++d) {
    m_gridDecal[d].clear();
    tmpl.createRadiusDecal(pos, radius, getObject()->getControllingPlayer(),
                           m_gridDecal[d]);
...
void DynamicShroudClearingRangeUpdate::animateGridDecals(void) {
  ...
  Real radius =
      m_currentClearingRange + ((m_totalFrames - m_stateCountDown) * 2);
  Real angle = 0.0f;
...
UpdateSleepTime DynamicShroudClearingRangeUpdate::update(void) {
  ...
  const DynamicShroudClearingRangeUpdateModuleData *md =
      getDynamicShroudClearingRangeUpdateModuleData();
  ...
  switch (m_state) {
    ...
    case DSCRU_GROWING: {
      ...
      m_currentClearingRange +=
          m_nativeClearingRange / max(1.0f, (Real)md->m_growTime);
    ...
    case DSCRU_SHRINKING: {
      m_currentClearingRange -= (m_nativeClearingRange - md->m_finalVision) /
                                max(1.0f, (Real)md->m_shrinkTime);
      break;
    }
  ...
  if (m_stateCountDown > 0)
    m_stateCountDown--;  // it is important that this gets called every frame
                         // without sleeping
  // beacuse it handles animation and may need to respond to changing vision
  // range from scripts & stuff

  if (m_changeIntervalCountdown > 0)
    m_changeIntervalCountdown--;
  else {  // reset per change timer
    m_changeIntervalCountdown =
        (m_state == DSCRU_GROWING ? md->m_growInterval : md->m_changeInterval);
    me->setShroudClearingRange(m_currentClearingRange);
    if (m_state == DSCRU_DONE_FOREVER) {  // We are done forever, and have done
                                          // the final update, so sleep.+
  ...
  //	if( !m_shrinkStarted )
  //	{
  //		//To allow a start delay of zero, check before dec, and look for
  //trigger after 		if( m_shrinkStartCountdown > 0 ) 			m_shrinkStartCountdown--; 		if(
  //m_shrinkStartCountdown == 0 )
  //		{
  //			m_shrinkStarted = TRUE;
  //
  //			// Timer for how often to change
  //			m_changeIntervalCountdown = md->m_changeInterval;
  //			// Marker to make sure we end up precisely correct
  //regardless of interval 			m_shrinkDeadline = TheGameLogic->getFrame() +
  //md->m_shrinkTime;
  //			// Figure out how much to change each interval
  //			Real visionDiff = me->getShroudClearingRange() -
  //md->m_finalVision; 			m_visionChangePerInterval =
  //-visionDiff/(md->m_shrinkTime/md->m_changeInterval);
  //		}
  //		else
  //		{
  ...
  //	m_changeIntervalCountdown =
  //getDynamicShroudClearingRangeUpdateModuleData()->m_changeInterval;
  //	UnsignedInt currentFrame = TheGameLogic->getFrame();

  //	if( currentFrame > m_shrinkDeadline )
  //	{
  //		// Time to stop doing anything.
  //		me->setShroudClearingRange(
  //getDynamicShroudClearingRangeUpdateModuleData()->m_finalVision );
  ...
  //		me->setShroudClearingRange( me->getShroudClearingRange() +
  //m_visionChangePerInterval );
  //	}
  //	return UPDATE_SLEEP_NONE;
}
