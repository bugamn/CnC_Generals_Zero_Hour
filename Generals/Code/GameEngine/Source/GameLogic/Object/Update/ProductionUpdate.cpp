...
// ///////////////////////////////////////////////////////////////////// Author:
// Colin Day, March 2002 Desc:   This module allows things to be "constructed"
// from a building
///////////////////////////////////////////////////////////////////////////////////////////////////
...
static const ModelConditionFlagType theOpeningFlags[DOOR_COUNT_MAX] = {
    MODELCONDITION_DOOR_1_OPENING, MODELCONDITION_DOOR_2_OPENING,
    MODELCONDITION_DOOR_3_OPENING, MODELCONDITION_DOOR_4_OPENING};

static const ModelConditionFlagType theClosingFlags[DOOR_COUNT_MAX] = {
    MODELCONDITION_DOOR_1_CLOSING, MODELCONDITION_DOOR_2_CLOSING,
    MODELCONDITION_DOOR_3_CLOSING, MODELCONDITION_DOOR_4_CLOSING};

static const ModelConditionFlagType theWaitingOpenFlags[DOOR_COUNT_MAX] = {
    MODELCONDITION_DOOR_1_WAITING_OPEN, MODELCONDITION_DOOR_2_WAITING_OPEN,
    MODELCONDITION_DOOR_3_WAITING_OPEN, MODELCONDITION_DOOR_4_WAITING_OPEN};
...
//-------------------------------------------------------------------------------------------------
/*static*/ void ProductionUpdateModuleData::parseAppendQuantityModifier(
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
/*static*/ void ProductionUpdateModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  UpdateModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"MaxQueueEntries", INI::parseInt, NULL,
       offsetof(ProductionUpdateModuleData, m_maxQueueEntries)},
      {"NumDoorAnimations", INI::parseInt, NULL,
       offsetof(ProductionUpdateModuleData, m_numDoorAnimations)},
      {"DoorOpeningTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(ProductionUpdateModuleData, m_doorOpeningTime)},
      {"DoorWaitOpenTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(ProductionUpdateModuleData, m_doorWaitOpenTime)},
      {"DoorCloseTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(ProductionUpdateModuleData, m_doorClosingTime)},
      {"ConstructionCompleteDuration", INI::parseDurationUnsignedInt, NULL,
       offsetof(ProductionUpdateModuleData, m_constructionCompleteDuration)},
      {"QuantityModifier", parseAppendQuantityModifier, NULL,
       offsetof(ProductionUpdateModuleData, m_quantityModifiers)},
      {"DisabledTypesToProcess", DisabledMaskType::parseFromINI, NULL,
       offsetof(ProductionUpdateModuleData, m_disabledTypesToProcess)},
      {0, 0, 0, 0}};
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
ProductionEntry::~ProductionEntry(void) {}  // end ~ProductionEntry
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
CanMakeType ProductionUpdate::canQueueUpgrade(
    const UpgradeTemplate *upgrade) const {
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
CanMakeType ProductionUpdate::canQueueCreateUnit(
    const ThingTemplate *unitType) const {
  /// @todo srj -- this is horrible, but the "right" way to do it is to move
  // ProductionUpdate to be part of ParkingPlaceBehavior, which I don't
  ...
  for (BehaviorModule **i = getObject()->getBehaviorModules(); *i; ++i) {
    if ((pp = (*i)->getParkingPlaceBehaviorInterface()) != NULL) {
      if (pp->shouldReserveDoorWhenQueued(unitType) &&
          !pp->hasAvailableSpaceFor(unitType))
...
Bool ProductionUpdate::queueUpgrade(const UpgradeTemplate *upgrade) {
  ...
  if (isUpgradeInQueue(upgrade) == TRUE) return FALSE;

  // STOP cheaters by making sure they can actually build this
  if (!getObject()->canProduceUpgrade(upgrade)) return FALSE;

  //
  // you cannot queue a player upgrade production if you are producing one
  // already somewhere else (or that somewhere else could even possibly be here)
  //
  if (upgrade->getUpgradeType() == UPGRADE_TYPE_PLAYER &&
  ...
  production->m_productionID =
      PRODUCTIONID_INVALID;  // not needed for upgrades, you can only have one
...
void ProductionUpdate::cancelUpgrade(const UpgradeTemplate *upgrade) {
  ...
  if (upgrade->getUpgradeType() == UPGRADE_TYPE_PLAYER &&
      player->hasUpgradeInProduction(upgrade) == FALSE)
    return;

  //
  // find the production entry for this upgrade in the queue here, there can
  // only be one of this type in the queue
  //
  ProductionEntry *production;
  ...
  //
  // remove the IN_PRODUCTION status of this upgrade from the player, object
  // upgrades don't have any other IN_PRODUCTION status other than their
  // existence in the build queue
  //
  if (upgrade->getUpgradeType() == UPGRADE_TYPE_PLAYER)
...
//-------------------------------------------------------------------------------------------------
/** Queue the prodcution of a unit.  Returns TRUE if unit was added to queue,
 * FALSE if it was not */
//-------------------------------------------------------------------------------------------------
Bool ProductionUpdate::queueCreateUnit(const ThingTemplate *unitType,
  ...
  // Check to see if there is a quantity modifier entry. What this means is if
  // we are building a particular unit and it has a modifier, we build that
  // number of objects instead of just one. A good example is the Chinese
  // barracks building redguards -- they are built 4 at a time, but the user or
  // script only pays for one and builds four for that price!
  production->m_productionQuantityTotal = 1;
  production->m_productionQuantityProduced = 0;
  for (std::vector<QuantityModifier>::const_iterator it =
           data->m_quantityModifiers.begin();
       it != data->m_quantityModifiers.end(); ++it) {
    const ThingTemplate *productionTemplate =
        TheThingFactory->findTemplate(it->m_templateName);
...
void ProductionUpdate::updateDoors() {
  ...
  for (Int i = 0; i < DOOR_COUNT_MAX; ++i) {
    //
    // if we have an open door, see if enough time has passed for us to close it
    // ... likewise if we were closing a door we will clear the closing
    // condition after an amount of time has passed
    //
...
UpdateSleepTime ProductionUpdate::update(void) {
  ...
  if (d->m_numDoorAnimations > 0) updateDoors();
  ...
  // if we have dirty bits that need to be set and cleared, do it here all at
  // once
  if (m_flagsDirty == TRUE) {
    Drawable *draw = us->getDrawable();

    if (draw) draw->clearAndSetModelConditionFlags(m_clearFlags, m_setFlags);
  ...
  if (production == NULL) return UPDATE_SLEEP_NONE;

  //
  // if we've become OBJECT_STATUS_SOLD, halt all production ... leave things in
  // the queue because if the sell process completes we get money back for them,
  // for now we will be just frozen in time Actually, there will be nothing in
  // the queue since everything gets cancel/refunded at the start of sell, but
  // we still don't want to do anything here.
  //
  ...
  //
  // you can disallow types of units on the fly in scripts, so if there is
  // something in the queue that suddenly can't be build then get rid of it
  //
  if (production->getProductionType() == PRODUCTION_UNIT &&
  ...
  if (production->m_type == PRODUCTION_UNIT)
    totalProductionFrames =
        production->m_objectToProduce->calcTimeToBuild(player);
  else
    totalProductionFrames =
        production->m_upgradeToResearch->calcTimeToBuild(player);

  // figure out our percent complete
  production->m_percentComplete =
      INT_TO_REAL(production->m_framesUnderConstruction) /
      INT_TO_REAL(totalProductionFrames) * 100.0f;
  ...
  if (production->m_percentComplete >= 100.0f) {
    ...
    if (production->m_type == PRODUCTION_UNIT) {
      ...
      if (exitInterface) {
        //
        // only exit if the exit interface will let us ... if we can't we'll
        // just keep this unit in the production and keep "overbuilding" it,
        // every frame we will check to see if we can exit and then move onto
        // production of the next one
        //
        Int numberToTry =
            production
                ->getProductionQuantityRemaining();  // this will change
                                                     // midloop, so it can't be
                                                     // the For clause
        for (int i = 0; i < numberToTry; i++) {
          ExitDoorType exitDoor = production->getExitDoor();
          if (exitDoor == DOOR_NONE_AVAILABLE) {
            exitDoor = exitInterface->reserveDoorForExit(
                production->m_objectToProduce, NULL);
            production->setExitDoor(exitDoor);
          }

          if (exitDoor != DOOR_NONE_AVAILABLE) {
            // note, could be DOOR_NONE_NEEDED! so door could be null. (srj)
            DoorInfo *door = (exitDoor >= 0 && exitDoor < DOOR_COUNT_MAX)
                                 ? &m_doors[exitDoor]
                                 : NULL;

            //
            // if the producing structure has a door opening animation we will
            // set the condition bits to open the door (note that we must also
            // take care to remove any condition that had us previously closing
            // a door)
            //
            const ProductionUpdateModuleData *d =
                getProductionUpdateModuleData();
            if (d->m_numDoorAnimations > 0 && door != NULL) {
              // if the door is closed, open it
              if (door->m_doorOpenedFrame == 0 &&
                  door->m_doorWaitOpenFrame == 0 &&
                  door->m_doorClosedFrame == 0) {
            ...
            //
            // make a new object, note that for production buildings that have
            // door animations we will not make the object until the door has
            // been totally opened
            //
            if (d->m_numDoorAnimations == 0 || door == NULL ||
                door->m_doorWaitOpenFrame != 0) {
              Object *newObj = TheThingFactory->newObject(
                  production->m_objectToProduce,
                  creationBuilding->getControllingPlayer()->getDefaultTeam());

              newObj->setProducer(creationBuilding);

              // call the exit interface to do the rally point and position
              // stuff
              exitInterface->exitObjectViaDoor(newObj, exitDoor);

              // since we successfully exited via this door, we should NOT call
              // unreserveDoorForExit when we toss this production entry. so set
              // it back to an innocuous value.
              production->setExitDoor(DOOR_NONE_AVAILABLE);

              // Now, play the sound associated with the new object's
              // production.
              AudioEventRTS voiceCreate =
                  *newObj->getTemplate()->getVoiceCreated();
              ...
              // onCreates have been called on newObj, and after that the owner
              // was set, so now is the time to call the game side of
              // CreateModules
              ...
              if (production->getProductionQuantity() ==
                  production->getProductionQuantityRemaining()) {
                // Call the voice created for the 1st object -- because it's
                // possible to create multiple objects like redguards!
                AudioEventRTS sound =
                    *newObj->getTemplate()->getPerUnitSound("VoiceCreate");
                sound.setObjectID(newObj->getID());
                TheAudio->addAudioEvent(&sound);
              }

              // We created one guy, but we may want to do more so we should
              // stay in this node of production.
              //  This is last so the voice check can easily check for "first"
              //  guy
        ...
        }  // end of trying to exit all the things we were planning on
           // attempting

        if (production->getProductionQuantityRemaining() == 0) {
          // remove this production entry so we can go on to the next if we are
          // totally finished
          removeFromProductionQueue(production);
        ...
        DEBUG_ASSERTCRASH(0,
                          ("Cannot create '%s', there is no ExitUpdate "
                           "interface defined for producer object '%s'\n",
      ...
      // we finished an upgrade, lets add that money spent on it to the
      // scorekeeper
      ...
      TheScriptEngine->notifyOfCompletedUpgrade(
          ...
          upgrade->getUpgradeName(), us->getID());
      ...
      if (us->isLocallyControlled()) {
        ...
        UnicodeString upgradeName =
            TheGameText->fetch(upgrade->getDisplayNameLabel().str());
      ...
      // update the upgrade status in the player or give the upgrade to the
      // object
      if (upgrade->getUpgradeType() == UPGRADE_TYPE_PLAYER) {
      ...
      if (selectedObject) {
        ...
        for (Int i = 0; i < MAX_UPGRADE_CAMEO_UPGRADES; i++) {
          ...
          const UpgradeTemplate *testUpgrade =
...
void ProductionUpdate::addToProductionQueue(ProductionEntry *production) {
  ...
  if (m_productionQueue == NULL) m_productionQueue = production;
  ...
  // when something is in the queue ... we are in the actively constructing
  // state
...
void ProductionUpdate::removeFromProductionQueue(ProductionEntry *production) {
  ...
  // when we go back to zero things in our queue, we clear the constructing
  // state
  Drawable *draw = getObject()->getDrawable();
  if (draw) {
    ModelConditionFlags condition = draw->getModelConditionFlags();

    if (m_productionCount == 0 &&
        condition.test(MODELCONDITION_ACTIVELY_CONSTRUCTING) == TRUE) {
  ...
  /*
          // Debugging
          UnicodeString msg;
          if( production->getProductionType() ==
     ProductionEntry::PRODUCTION_UNIT )
          {
                  msg.format( L"Removed unit '%S'(%d)",
     production->getProductionObject()->getName().str(),
                                                                                                                                                                          production->getProductionID() );
                  TheInGameUI->message( msg );
          }
          else
          {
                  msg.format( L"Remove upgrade '%S'",
     production->getProductionUpgrade()->getName().str() );
                  TheInGameUI->message( msg );
          }
  */
...
//-------------------------------------------------------------------------------------------------
/** Is the upgrade already in the production queue.  Note that you can only have
 * one production entry for any given upgrade in the queue */
//-------------------------------------------------------------------------------------------------
Bool ProductionUpdate::isUpgradeInQueue(const UpgradeTemplate *upgrade) const {
  const ProductionEntry *production;

  for (production = firstProduction(); production;
...
// ------------------------------------------------------------------------------------------------
UnsignedInt ProductionUpdate::countUnitTypeInQueue(
    const ThingTemplate *unitType) const {
  UnsignedInt count = 0;
  const ProductionEntry *production;
...
// ------------------------------------------------------------------------------------------------
/** Cancel each of the production items in the queue.  By going through the
 * actual cancel methods, the cost of each of those items will be refunded to
 * the player */
// ------------------------------------------------------------------------------------------------
void ProductionUpdate::cancelAndRefundAllProduction(void) {
  // Empirically, in release the code can loop forever.  So we limit to 100
  // passes. jba. [8/31/2003]
  const Int productionLimit =
      100;  // With luck, we never queue up 100 units. [8/31/2003]
  Int i;
  for (i = 0; i < productionLimit; i++) {
    ...
    if (m_productionQueue) {
      if (m_productionQueue->getProductionType() == PRODUCTION_UNIT)
        ...
        DEBUG_CRASH(
            ("ProductionUpdate::cancelAndRefundAllProduction - Unknown "
             "production type '%d'\n",
...
void ProductionUpdate::setHoldDoorOpen(ExitDoorType exitDoor, Bool holdIt) {
  if (exitDoor >= DOOR_1 && exitDoor < DOOR_COUNT_MAX) {
    ...
    if (holdIt && door.m_doorOpenedFrame == 0 &&
        door.m_doorWaitOpenFrame == 0 && door.m_doorClosedFrame == 0) {
...
// ------------------------------------------------------------------------------------------------
/** Helper method to retrieve a production update interface from an object if
 * one is present */
// ------------------------------------------------------------------------------------------------
/*static*/ ProductionUpdateInterface *
ProductionUpdate::getProductionUpdateInterfaceFromObject(Object *obj) {
  // sanity
...
void ProductionUpdate::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    if (m_productionQueue != NULL) {
      DEBUG_CRASH(
          ("ProductionUpdate::xfer - m_productionQueue is not empty, but "
           "should be\n"));
    ...
    for (UnsignedShort i = 0; i < productionCount; ++i) {
      ...
      // make any existing tail pointer now point to us, and we point back to
      // them
      if (m_productionQueueTail) {
        m_productionQueueTail->m_next = production;
        production->m_prev = m_productionQueueTail;
