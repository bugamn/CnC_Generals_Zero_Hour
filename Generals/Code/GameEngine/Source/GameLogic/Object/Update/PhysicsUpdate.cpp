...
// Air friction used to be completely separate and unclamped, so this constant
// must be split. The fact it defaults to 0 shows it.
...
//-------------------------------------------------------------------------------------------------
static Real angleBetweenVectors(const Coord3D& inCurDir,
                                const Coord3D& inGoalDir) {
  Vector3 curDir;
  curDir.X = inCurDir.x;
  curDir.Y = inCurDir.y;
  curDir.Z = inCurDir.z;
...
PhysicsBehaviorModuleData::PhysicsBehaviorModuleData() {
  ...
  /*
          thru some bizarre editing mishap, we have been double-apply
     pitch/roll/yaw rates to objects for, well, a long time, it looks like. I
     have corrected that problem in the name of efficiency, but to maintain the
     same visual appearance without having to edit every freaking INI in the
     world at this point, I am just multiplying all the results by a factor so
     that the effect is the same (but with less execution time). I have put this
     factor into INI in the unlikely event we ever need to change it, but
     defaulting it to 2 is, in fact, the right thing for now... (srj)
  */
  m_pitchRollYawFactor = 2.0f;
  m_vehicleCrashesIntoBuildingWeaponTemplate =
      TheWeaponStore->findWeaponTemplate("VehicleCrashesIntoBuildingWeapon");
  m_vehicleCrashesIntoNonBuildingWeaponTemplate =
      TheWeaponStore->findWeaponTemplate("VehicleCrashesIntoNonBuildingWeapon");
}

//-------------------------------------------------------------------------------------------------
static void parseHeightToSpeed(INI* ini, void* /*instance*/, void* store,
                               const void* /*userData*/) {
  // don't bother trying to remember how far we've fallen; instead,
  // back-calc it from our speed & gravity... v = sqrt(2*g*h)
...
//-------------------------------------------------------------------------------------------------
/*static*/ void PhysicsBehaviorModuleData::buildFieldParse(
    MultiIniFieldParse& p) {
  UpdateModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"Mass", INI::parsePositiveNonZeroReal, NULL,
       offsetof(PhysicsBehaviorModuleData, m_mass)},

      {"ForwardFriction", parseFrictionPerSec, NULL,
       offsetof(PhysicsBehaviorModuleData, m_forwardFriction)},
      {"LateralFriction", parseFrictionPerSec, NULL,
       offsetof(PhysicsBehaviorModuleData, m_lateralFriction)},
      {"ZFriction", parseFrictionPerSec, NULL,
       offsetof(PhysicsBehaviorModuleData, m_ZFriction)},
      {"AerodynamicFriction", parseFrictionPerSec, NULL,
       offsetof(PhysicsBehaviorModuleData, m_aerodynamicFriction)},

      {"CenterOfMassOffset", INI::parseReal, NULL,
       offsetof(PhysicsBehaviorModuleData, m_centerOfMassOffset)},
      {"AllowBouncing", INI::parseBool, NULL,
       offsetof(PhysicsBehaviorModuleData, m_allowBouncing)},
      {"AllowCollideForce", INI::parseBool, NULL,
       offsetof(PhysicsBehaviorModuleData, m_allowCollideForce)},
      {"KillWhenRestingOnGround", INI::parseBool, NULL,
       offsetof(PhysicsBehaviorModuleData, m_killWhenRestingOnGround)},

      {"MinFallHeightForDamage", parseHeightToSpeed, NULL,
       offsetof(PhysicsBehaviorModuleData, m_minFallSpeedForDamage)},
      {"FallHeightDamageFactor", INI::parseReal, NULL,
       offsetof(PhysicsBehaviorModuleData, m_fallHeightDamageFactor)},
      {"PitchRollYawFactor", INI::parseReal, NULL,
       offsetof(PhysicsBehaviorModuleData, m_pitchRollYawFactor)},

      {"VehicleCrashesIntoBuildingWeaponTemplate", INI::parseWeaponTemplate,
       NULL,
       offsetof(PhysicsBehaviorModuleData,
                m_vehicleCrashesIntoBuildingWeaponTemplate)},
      {"VehicleCrashesIntoNonBuildingWeaponTemplate", INI::parseWeaponTemplate,
       NULL,
       offsetof(PhysicsBehaviorModuleData,
                m_vehicleCrashesIntoNonBuildingWeaponTemplate)},
...
static ProjectileUpdateInterface* getPui(Object* obj) {
  if (!obj->isKindOf(KINDOF_PROJECTILE)) return NULL;

  ProjectileUpdateInterface* objPui = NULL;
  for (BehaviorModule** u = obj->getBehaviorModules(); *u; ++u) {
    if ((objPui = (*u)->getProjectileUpdateInterface()) != NULL) return objPui;
  }
  return NULL;
}

//-------------------------------------------------------------------------------------------------
void PhysicsBehavior::onObjectCreated() { m_pui = getPui(getObject()); }
...
Real PhysicsBehavior::getAerodynamicFriction() const {
  Real f =
      getPhysicsBehaviorModuleData()->m_aerodynamicFriction + m_extraFriction;
...
void PhysicsBehavior::applyForce(const Coord3D* force) {
  DEBUG_ASSERTCRASH(!(_isnan(force->x) || _isnan(force->y) || _isnan(force->z)),
  ...
  // DEBUG_ASSERTCRASH(!(_isnan(m_accel.x) || _isnan(m_accel.y) ||
  // _isnan(m_accel.z)), ("PhysicsBehavior::applyForce accel NAN!\n"));
  // DEBUG_ASSERTCRASH(!(_isnan(m_vel.x) || _isnan(m_vel.y) || _isnan(m_vel.z)),
  // ("PhysicsBehavior::applyForce vel NAN!\n"));
  // DEBUG_ASSERTCRASH(fabs(force->z) < 3, ("unlikely z-force"));
#ifdef SLEEPY_PHYSICS
  if (getFlag(IS_IN_UPDATE)) {
    // we're applying a force from inside our own update (probably for a bounce
    // or friction). just do nothing, since update will calculate the correct
    // sleep behavior at the end.
  } else {
    // when a force is applied by an external module, we must wake up, even if
    // the force is zero.
    setWakeFrame(getObject(), UPDATE_SLEEP_NONE);
  }
#endif
...
#ifdef SLEEPY_PHYSICS
  DEBUG_ASSERTCRASH(!getFlag(IS_IN_UPDATE),
                    ("hmm, should not happen, may not work"));
...
void PhysicsBehavior::applyFrictionalForces() {
  if (getFlag(APPLY_FRICTION2D_WHEN_AIRBORNE) ||
    ...
    applyYPRDamping(1.0f +
                    aerodynamics);  // since aero is negated, this results
                                    // in 1.0-getAerodynamicFriction()
  }
}

//-------------------------------------------------------------------------------------------------
Bool PhysicsBehavior::handleBounce(Real oldZ, Real newZ, Real groundZ,
                                   Coord3D* bounceForce) {
...
void PhysicsBehavior::setPitchRate(Real pitch) {
  ...
  setFlag(HAS_PITCHROLLYAW,
          (m_pitchRate != 0.0f || m_rollRate != 0.0f || m_yawRate != 0.0f));
}
 ...
 * @todo Currently, only translations are integrated. Rotations should also be
 * integrated. (MSB)
 */
...
UpdateSleepTime PhysicsBehavior::update() {
  ...
  if (!obj->isDisabledByType(DISABLED_HELD)) {
    ...
    if (getFlag(HAS_PITCHROLLYAW)) {
      /*
              ...
              the matrix... because doing such a thing is highly
         order-dependent, and furthermore, you'd have to use Euler angles (Not
         the Get_?_Rotation calls) to be able to reconstruct 'em, and that's too
         slow to do for every object every frame.
      ...
      */
      ...
      if (offset != 0.0f) {
        ...
        Real remainingAngle =
            (offset > 0) ? ((PI / 2) - pitchAngle) : (-(PI / 2) + pitchAngle);
        Real s = Sin(remainingAngle);
        pitchRateToUse *= s;
      }

      // update rotation
      /// @todo Rotation should use torques, and integrate just like forces
      /// (MSB)
    ...
    Real groundZ = TheTerrainLogic->getLayerHeight(
        mtx.Get_X_Translation(), mtx.Get_Y_Translation(), obj->getLayer());
    gotBounceForce =
        handleBounce(oldPosZ, mtx.Get_Z_Translation(), groundZ, &bounceForce);

    // remember our z-vel prior to doing ground-slam adjustment
    activeVelZ = m_vel.z;
    if (mtx.Get_Z_Translation() <= groundZ) {
      // Note - when vehicles are going down a slope, they will maintain a small
      // negative z velocity as they go down.  So don't slam it to 0 if they
      // aren't slamming into the ground.
  ...
  if (getFlag(WAS_AIRBORNE_LAST_FRAME) && !airborneAtEnd &&
      !getFlag(IMMUNE_TO_FALLING_DAMAGE)) {
    ...
    if (netSpeed > 0.0f && m_pui == NULL) {
      ...
      // going down hills don't injure themselves (unless the hill is really
      // steep)
      const Real MIN_ANGLE_TAN = 3.0f;  //	roughly 71 degrees
      const Real TINY_DELTA = 0.01f;
      if ((fabs(m_vel.x) <= TINY_DELTA ||
           fabs(activeVelZ / m_vel.x) >= MIN_ANGLE_TAN) &&
          (fabs(m_vel.y) <= TINY_DELTA ||
           fabs(activeVelZ / m_vel.y) >= MIN_ANGLE_TAN)) {
        ...
        // DEBUG_LOG(("Dealing %f (%f %f) points of falling damage to
        // %s!\n",damageAmt,damageInfo.out.m_actualDamageDealt,
        // damageInfo.out.m_actualDamageClipped,obj->getTemplate()->getName().str()));
  ...
  if (d->m_killWhenRestingOnGround && !airborneAtEnd && isVerySmall3D(m_vel)) {
    if (!obj->isKindOf(KINDOF_DRONE) || obj->isEffectivelyDead() ||
...
#ifdef SLEEPY_PHYSICS
  if (isZero3D(m_vel) && isZero3D(m_accel) && !getFlag(HAS_PITCHROLLYAW) &&
      !isMotive() &&
      (getObject()->getLayer() == LAYER_GROUND &&
       !getObject()->isAboveTerrain()) &&
      getCurrentOverlap() == INVALID_ID && getPreviousOverlap() == INVALID_ID &&
...
Real PhysicsBehavior::getForwardSpeed2D() const {
  ...
  //	DEBUG_ASSERTCRASH( speedSquared != 0, ("zero speedSquared will overflow
  //sqrtf()!") );// lorenzen... sanity check

  Real speed = (Real)sqrtf(speedSquared);

  if (dot >= 0.0f) return speed;
...
void PhysicsBehavior::scrubVelocityZ(Real desiredVelocity) {
  if (fabs(desiredVelocity) < 0.001f) {
    ...
    if ((desiredVelocity < 0 && m_vel.z < desiredVelocity) ||
        (desiredVelocity > 0 && m_vel.z > desiredVelocity)) {
...
void PhysicsBehavior::setAngles(Real yaw, Real pitch, Real roll) {
  ...
  // here we DO want to use in-place-etc, cuz we're not adding to any existing
  // rot/etc
...
Real PhysicsBehavior::getMass() const {
  ...
  if (contain) mass += contain->getContainedItemsMass();
  return mass;
}
...
void PhysicsBehavior::doBounceSound(const Coord3D& prevPos) {
  if (!m_bounceSound) return;

  const Real NORMAL_VEL_Z = 0.25f;
  const Real NORMAL_MASS = 50.0f;

  // get the per-unit sound for the collision which was stuffed in on Object
  // creation.
  AudioEventRTS collisionSound = m_bounceSound->m_event;

  // Real vel = fabs(getVelocity()->z);
  //  can't use velocity, because it's already been updated this frame, and will
  //  be zero... (srj)
...
#ifdef FIX_AUDIO
  Real volAdjust =
      NormalizeToRange(MuLaw(vel, NORMAL_VEL_Z, 500), -1, 1, 0.25, 1.0);
  volAdjust *=
      NormalizeToRange(MuLaw(mass, NORMAL_MASS, 500), -1, 1, 0.25, 1.0);
 ...
 * push them apart.
 * Note that this call only applies forces to our object, not to "other".  Since
 * the forces should be equal and opposite, this could be optimized.
...
void PhysicsBehavior::onCollide(Object* other, const Coord3D* loc,
                                const Coord3D* normal) {
  // USE_PERF_TIMER(PhysicsBehavioronCollide)
  if (m_pui != NULL) {
    // projectiles always get a chance to handle their own collisions, and not
    // go thru here
    if (m_pui->projectileHandleCollision(other)) return;
  }
  ...
  if (obj->testStatus(OBJECT_STATUS_PARACHUTING) &&
      other->testStatus(OBJECT_STATUS_PARACHUTING)) {
    return;
  }
  ...
  if (ai != NULL && ai->getIgnoredObstacleID() == other->getID()) {
    ...
    // Before we return, check for a very special case of an infantry colliding
    // with an unmanned vehicle. If this is the case, it'll become its new
    // pilot!
    if (obj->isKindOf(KINDOF_INFANTRY) &&
        other->isDisabledByType(DISABLED_UNMANNED)) {
      // This is in fact the case, and we are doing it here because it applies
      // to all infantry in any unmanned vehicle. This could be done via a
      // special/new module, but doing it here doesn't require a new module
      // update to every infantry.
      other->clearDisabled(DISABLED_UNMANNED);

      // We need to be able to test whether an object on a team has been
      // captured, so set here that this object was captured.
      ...
      // transfer the name of the infantry to the vehicle... so the designer can
      // control the vehicle with their scripts.
  ...
  if (ai) {
    ...
    if (!((obj->isEffectivelyDead() ||
           obj->testStatus(OBJECT_STATUS_PARACHUTING)) &&
          otherImmobile)) {
  ...
  // if objects are coincident, dist is zero, so force would be infinite --
  // clearly not what we want. so just cap it here for now. (srj)
  ...
  if (getAllowCollideForce()) {
    ...
    if (otherImmobile && !obj->isDestroyed()) {
      ...
      // if other is immobile, we must apply enough force to at least stop our
      // motion, and preferably negate it, otherwise we will pass thru the
      // object. so cheat, and act like we're bouncing off the ground.
      ...
      if (mag < MINBOUNCESPEED) mag = MINBOUNCESPEED;
      factor = -mag * getMass() * stiffness;

      // if we are moving down, we may want to blow ourselves into
      // smithereens....
      if (delta.z < 0.0f && obj->getPosition()->z >=
                                TheGlobalData->m_defaultStructureRubbleHeight) {
        if (other->isKindOf(KINDOF_STRUCTURE)) {
          // fall into a building. if a vehicle, blow up. then destroy ourself
          // (not die), regardless.
          if (obj->isKindOf(KINDOF_VEHICLE)) {
            TheWeaponStore->createAndFireTempWeapon(
                getPhysicsBehaviorModuleData()
                    ->m_vehicleCrashesIntoBuildingWeaponTemplate,
                obj, obj->getPosition());
          }
          TheGameLogic->destroyObject(obj);
          ...
          // fall into a nonbuilding -- whatever. if we're a vehicle, quietly do
          // a little damage.
          if (obj->isKindOf(KINDOF_VEHICLE)) {
            TheWeaponStore->createAndFireTempWeapon(
                getPhysicsBehaviorModuleData()
      ...
      // nuke the velocity. why? very simple: we want to ignore the previous vel
      // in favor of this. in theory, we could be clever and calculate the right
      // force to apply to achieve this, but then if we were still colliding
      // next frame, we'd get a sudden 'aceleration' of bounce that would look
      // freakish. so cheat.
      m_vel.x = 0;
      m_vel.y = 0;
      m_vel.z = 0;
...
/** Do the collision
 *  Cases:
 *  * The crusheeOther is !OVERLAPPABLE or !CRUSHABLE.
 * Result: return false
 *  * The crusherMe is !CRUSHER.
 * Result: return false
 *  * crusher is CRUSHER, crushee is CRUSHABLE but allied.
 * Result: return false
 *  * crusher is CRUSHER, crushee is OVERLAPPABLE but not CRUSHABLE.  Result:
 * return false
 *  * crushee has no PhysicsBehavior.
 * Result: return false
 *  * crusher is CRUSHER, crushee is CRUSHABLE, but is too hard.
 * Result: return false
 *  * crusher is CRUSHER, crushee is CRUSHABLE, hardness ok.
 * Result: crush, return true
 *
...
Bool PhysicsBehavior::checkForOverlapCollision(Object* other) {
  // This is the most Supreme Truth... that unless I am moving right now, I may
  // not crush anyhing!
  if (isVerySmall3D(*getVelocity())) return false;

  Object* crusherMe = getObject();
  Object* crusheeOther = other;

  // Determine if we can crush the other object.
  Bool selfCrushingOther =
      crusherMe->canCrushOrSquish(crusheeOther, TEST_CRUSH_ONLY);
  Bool selfBeingCrushed =
      crusheeOther->canCrushOrSquish(crusherMe, TEST_CRUSH_ONLY);

  if (selfCrushingOther && selfBeingCrushed) {
    // Is it possible to crush and be crushed at the same time?
    DEBUG_CRASH(
        ("%s (Crusher:%d, Crushable:%d) is attempting to crush %s (Crusher:%d, "
         "Crushable:%d) but it is reciprocating -- shouldn't be possible!",
         crusherMe->getTemplate()->getName().str(),
         crusherMe->getCrusherLevel(), crusherMe->getCrushableLevel(),
         crusheeOther->getTemplate()->getName().str(),
         crusheeOther->getCrusherLevel(), crusheeOther->getCrushableLevel()));
    return false;
  }

  // if we are being crushed, then skip all this and return true;
  // this allows us to NOT react in the normal way and just be passive to the
  // overlap...
  if (selfBeingCrushed) {
    return true;
  }
  ...
  if (!crusherPhysics->wasPreviouslyOverlapped(crusheeOther)) {
    ...
    damageInfo.in.m_amount =
        0.0f;  // yes, that's right -- we don't want to do damage, just to
               // trigger the minor DamageFX, if any
  ...
  if (!(frontCrushed && backCrushed)) {
    ...
    Real crushPointOffsetDistance =
        crusheeOther->getGeometryInfo().getMajorRadius() / 2;
    ...
    /// @todo GS To account for different sized crushers, this should be redone
    /// as a box or circle test, not a point
    // First decide which crush point has the shortest perp to our direction
    // ray.
    //		Real bestPerp = 9999;
    ...
    if (frontCrushed || backCrushed) {
      ...
      Coord3D frontVector, backVector, centerVector;
      {
        ...
        frontVector.y -=
            crusherPos->y;  // vector from me to the front crush point
        frontVector.z = 0;
        ...
        dirVector.y =
            rayLength *
            dir->y;  // vector from me to point of perp along direction ray
        dirVector.z = 0;
        ...
        perpVector.y =
            dirVector.y -
            frontVector.y;  // vector from the front point perp to my direction
        perpVector.z = 0;
      ...
      }
      {
        ...
        perpVector.y =
            dirVector.y -
            backVector.y;  // vector from the front point perp to my direction
        perpVector.z = 0;
      ...
      }
      {
        ...
        perpVector.y =
            dirVector.y -
            centerVector.y;  // vector from the front point perp to my direction
        perpVector.z = 0;

        centerPerpLength = perpVector.length();
      }

      // Now find the shortest.  Use the straightline distance to crush point as
      // tie breaker
      if ((frontPerpLength <= centerPerpLength) &&
          (frontPerpLength <= backPerpLength)) {
      ...
      } else if ((backPerpLength <= centerPerpLength) &&
                 (backPerpLength <= frontPerpLength)) {
    ...
    Real distanceTooFarSquared =
        2.25 * crushPointOffsetDistance * crushPointOffsetDistance;
    // And just because there is only one crush point left doesn't
    // mean we automatically get it. (1.5x)^2 means if we are outside the
    // crushed
    ...
    if (crushIt) {
      // do a boat load of crush damage, and the onDie will handle cases like
      // crushed car object
      DamageInfo damageInfo;
      damageInfo.in.m_damageType = DAMAGE_CRUSH;
      damageInfo.in.m_deathType = DEATH_CRUSHED;
