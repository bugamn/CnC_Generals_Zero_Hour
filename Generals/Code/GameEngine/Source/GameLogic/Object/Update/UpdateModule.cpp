...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
UpdateSleepTime UpdateModule::frameToSleepTime(UnsignedInt frame1,
                                               UnsignedInt frame2,
                                               UnsignedInt frame3,
                                               UnsignedInt frame4) {
  DEBUG_ASSERTCRASH(frame1 != 0 && frame2 != 0 && frame3 != 0 && frame4 != 0,
                    ("probably should not pass zero to frameToSleepTime."));
...
void UpdateModule::setWakeFrame(Object *obj, UpdateSleepTime wakeDelay) {
  UnsignedInt now = TheGameLogic->getFrame();
  TheGameLogic->friend_awakenUpdateModule(obj, this,
                                          now + (UnsignedInt)wakeDelay);
}
...
/*
        ...
        now change the code for module "FOO" from being nonsleepy to being
   sleepy now reload that saved game as soon as "FOO" attempts to wake itself
   up, you're dead.

        this fix simply looks to see if the module in question is now sleepy in
   code, but was saved in sleepy form, and if so, quietly nudges a reasonable
...
#ifdef FIX_OLD_SAVES
  if (xfer->getXferMode() == XFER_LOAD && thisModuleIsNowSleepy &&
      m_nextCallFrameAndPhase == 0) {
#ifdef ALLOW_NONSLEEPY_UPDATES
    // when the file was saved, this module was nonsleepy, but now it is sleepy.
#else
    // note that 'm_nextCallFrameAndPhase' should only be zero for legacy save
    // files.
#endif
