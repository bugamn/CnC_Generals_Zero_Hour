...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
static const char *TheStructureCollapsePhaseNames[] = {"INITIAL", "DELAY",
                                                       "BURST", "FINAL",
...
static void parseFX(INI *ini, void *instance, void * /*store*/,
                    const void * /*userData*/) {
  StructureCollapseUpdateModuleData *self =
      (StructureCollapseUpdateModuleData *)instance;
  StructureCollapsePhaseType scphase =
      (StructureCollapsePhaseType)INI::scanIndexList(
          ini->getNextToken(), TheStructureCollapsePhaseNames);
...
static void parseOCL(INI *ini, void *instance, void * /*store*/,
  ...
  StructureCollapseUpdateModuleData *self =
      (StructureCollapseUpdateModuleData *)instance;
  StructureCollapsePhaseType stphase =
      (StructureCollapsePhaseType)INI::scanIndexList(
          ini->getNextToken(), TheStructureCollapsePhaseNames);
  for (const char *token = ini->getNextToken(); token != NULL;
       token = ini->getNextTokenOrNull()) {
...
//-------------------------------------------------------------------------------------------------
/*static*/ void StructureCollapseUpdateModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  UpdateModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"MinCollapseDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(StructureCollapseUpdateModuleData, m_minCollapseDelay)},
      {"MaxCollapseDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(StructureCollapseUpdateModuleData, m_maxCollapseDelay)},
      {"MinBurstDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(StructureCollapseUpdateModuleData, m_minBurstDelay)},
      {"MaxBurstDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(StructureCollapseUpdateModuleData, m_maxBurstDelay)},
      {"CollapseDamping", INI::parseReal, NULL,
       offsetof(StructureCollapseUpdateModuleData, m_collapseDamping)},
      {"MaxShudder", INI::parseReal, NULL,
       offsetof(StructureCollapseUpdateModuleData, m_maxShudder)},
      {"BigBurstFrequency", INI::parseInt, NULL,
       offsetof(StructureCollapseUpdateModuleData, m_bigBurstFrequency)},
      {"OCL", parseOCL, NULL, 0},
      {"FXList", parseFX, NULL, 0},
      {0, 0, 0, 0}};
  p.add(dataFieldParse);
  p.add(DieMuxData::getFieldParse(),
        offsetof(StructureCollapseUpdateModuleData, m_dieMuxData));
}

//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void StructureCollapseUpdate::beginStructureCollapse(
    const DamageInfo *damageInfo) {
  const StructureCollapseUpdateModuleData *d =
      getStructureCollapseUpdateModuleData();
  ...
  m_collapseFrame =
      now + GameLogicRandomValue(d->m_minCollapseDelay, d->m_maxCollapseDelay);
...
UpdateSleepTime StructureCollapseUpdate::update(void) {
  ...
  if (m_collapseState == COLLAPSESTATE_WAITINGFORCOLLAPSESTART) {
    ...
    Vector3 shudder;
    shudder.Set(GameClientRandomValueReal(-(d->m_maxShudder), d->m_maxShudder),
                GameClientRandomValueReal(-(d->m_maxShudder), d->m_maxShudder),
                0);
    ...
    if (now >= m_collapseFrame) {
      ...
      // debris, and debris is sync'd.
      m_burstFrame =
          now + GameLogicRandomValue(d->m_minBurstDelay, d->m_maxBurstDelay);
    }
  }
  ...
  if (m_collapseState == COLLAPSESTATE_COLLAPSING) {
    ...
    m_collapseVelocity -=
        TheGlobalData->m_gravity * (1.0 - d->m_collapseDamping);
    ...
    if (now >= m_burstFrame) {
      ...
      // debris, and debris is sync'd.
      m_burstFrame +=
          GameLogicRandomValue(d->m_minBurstDelay, d->m_maxBurstDelay);
    }

    //		if ((m_currentHeight +
    //building->getGeometryInfo().getMaxHeightAbovePosition()) <= 0)
    if ((m_currentHeight + building->getTemplate()
                               ->getTemplateGeometryInfo()
                               .getMaxHeightAbovePosition()) <= 0) {
