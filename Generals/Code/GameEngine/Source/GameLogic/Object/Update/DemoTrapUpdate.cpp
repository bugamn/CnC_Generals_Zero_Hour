...
//-------------------------------------------------------------------------------------------------
/*static*/ void DemoTrapUpdateModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  ModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"DefaultProximityMode", INI::parseBool, NULL,
       offsetof(DemoTrapUpdateModuleData, m_defaultsToProximityMode)},
      {"DetonationWeaponSlot", INI::parseLookupList,
       TheWeaponSlotTypeNamesLookupList,
       offsetof(DemoTrapUpdateModuleData, m_detonationWeaponSlot)},
      {"ProximityModeWeaponSlot", INI::parseLookupList,
       TheWeaponSlotTypeNamesLookupList,
       offsetof(DemoTrapUpdateModuleData, m_proximityModeWeaponSlot)},
      {"ManualModeWeaponSlot", INI::parseLookupList,
       TheWeaponSlotTypeNamesLookupList,
       offsetof(DemoTrapUpdateModuleData, m_manualModeWeaponSlot)},
      {"TriggerDetonationRange", INI::parseReal, NULL,
       offsetof(DemoTrapUpdateModuleData, m_triggerDetonationRange)},
      {"IgnoreTargetTypes", KindOfMaskType::parseFromINI, NULL,
       offsetof(DemoTrapUpdateModuleData, m_ignoreKindOf)},
      {"ScanRate", INI::parseDurationUnsignedInt, NULL,
       offsetof(DemoTrapUpdateModuleData, m_scanFrames)},
      {"AutoDetonationWithFriendsInvolved", INI::parseBool, NULL,
       offsetof(DemoTrapUpdateModuleData, m_friendlyDetonation)},
      {"DetonationWeapon", INI::parseWeaponTemplate, NULL,
       offsetof(DemoTrapUpdateModuleData, m_detonationWeaponTemplate)},
      {"DetonateWhenKilled", INI::parseBool, NULL,
       offsetof(DemoTrapUpdateModuleData, m_detonateWhenKilled)},
      {0, 0, 0, 0}};
...
void DemoTrapUpdate::onObjectCreated() {
  ...
  if (data->m_detonationWeaponSlot == data->m_proximityModeWeaponSlot ||
    ...
    DEBUG_CRASH(
        ("The demo trap requires three weaponslots: One for each of the "
         "detonation mode, proximity mode, and manual mode."));
  }
  ...
  if (data->m_defaultsToProximityMode) {
    ...
    getObject()->setWeaponLock(data->m_proximityModeWeaponSlot,
                               LOCKED_TEMPORARILY);
  } else {
    // lock it just till the weapon is empty or the attack is "done"
    getObject()->setWeaponLock(data->m_manualModeWeaponSlot,
                               LOCKED_TEMPORARILY);
  }
}
...
UpdateSleepTime DemoTrapUpdate::update() {
  ...
  if (me->testStatus(OBJECT_STATUS_UNDER_CONSTRUCTION) ||
      me->testStatus(OBJECT_STATUS_SOLD)) {
    return UPDATE_SLEEP_NONE;
  }
  ...
  // Reset timer here -- because if we are in manual mode, and switch, we want
  // instant gratification (if possible).
  ...
  ObjectIterator *iter = ThePartitionManager->iterateObjectsInRange(
      me->getPosition(), data->m_triggerDetonationRange, FROM_CENTER_2D);
  MemoryPoolObjectHolder hold(iter);

  Bool shallDetonate = false;

  // Now iterate through each object in range and check to see if it should
  // detonate us!
  for (Object *other = iter->first(); other; other = iter->next()) {
    ...
    if (fDist <=
        data->m_triggerDetonationRange * data->m_triggerDetonationRange) {
      // Yeehaw!
      shallDetonate = true;
  ...
  if (shallDetonate) {
    // Enemy in proximity and we are in proximity detonation mode, so trigger
    // the explosion and kill them!!! Muwahahaha!
    detonate();
  }
  return UPDATE_SLEEP_NONE;
}
...
void DemoTrapUpdate::detonate() {
  ...
  if (!me->testStatus(OBJECT_STATUS_UNDER_CONSTRUCTION) &&
      !me->testStatus(OBJECT_STATUS_SOLD))
    TheWeaponStore->createAndFireTempWeapon(data->m_detonationWeaponTemplate,
                                            me, me->getPosition());
