...
// Desc:   Update will change model state conditions based on special power
// state
///////////////////////////////////////////////////////////////////////////////////////////////////
...
MissileLauncherBuildingUpdate::MissileLauncherBuildingUpdate(
  ...
  m_timeoutFrame = 0;
  m_openIdleAudio =
      getMissileLauncherBuildingUpdateModuleData()->m_openIdleAudio;
...
void MissileLauncherBuildingUpdate::switchToState(DoorStateType dst) {
  if (m_doorState == dst) return;

  const MissileLauncherBuildingUpdateModuleData *d =
      getMissileLauncherBuildingUpdateModuleData();
  ...
  switch (dst) {
    ...
    case DOOR_OPEN:
      ...
      if (!m_openIdleAudio.isCurrentlyPlaying()) {
        m_openIdleAudio.setPlayingHandle(
            TheAudio->addAudioEvent(&m_openIdleAudio));
      }
      break;
    ...
    case DOOR_CLOSING:
...

      {
        ...
        if (m_timeoutFrame > now + delta / 2) m_timeoutFrame = now + delta / 2;
      }
  ...
  if (m_timeoutFrame > 0) {
    ...
    if (draw) draw->setAnimationLoopDuration(m_timeoutFrame - now);
  }
}

//-------------------------------------------------------------------------------------------------
void MissileLauncherBuildingUpdate::initiateIntentToDoSpecialPower(
...
//-------------------------------------------------------------------------------------------------
Bool MissileLauncherBuildingUpdate::isPowerCurrentlyInUse(
    const CommandButton *command) const {
  //@todo -- Implement me in such a way that it still works with Ctrl+s cheat
  //key... In fact, this code doesn't really care because in legit mode, the
  // game will never require this check to disable the cameo button.
  return false;
}
...
UpdateSleepTime MissileLauncherBuildingUpdate::update(void) {
  ...
  // If we are under construction, any decision we make about door status could
  // be wrong. Our special power module is randomly going to be initialized or
  // not (which would result in him reporting a 0 frame ready, which means we
  // will start open).
  ...
  if (!m_specialPowerModule) {
    m_specialPowerModule =
        getObject()->getSpecialPowerModule(d->m_specialPowerTemplate);
    DEBUG_ASSERTCRASH(m_specialPowerModule, ("Missing special power"));
  }

  if (m_specialPowerModule) {
    UnsignedInt readyFrame = m_specialPowerModule->getReadyFrame();
    UnsignedInt whenToStartOpening = (readyFrame >= d->m_doorOpenTime)
                                         ? (readyFrame - d->m_doorOpenTime)
                                         : 0;
    ...
    DEBUG_ASSERTCRASH(
        !TheGlobalData->m_specialPowerUsesDelay ||
            !(m_specialPowerModule->isReady() && m_doorState != DOOR_OPEN),
...
SpecialPowerTemplate *MissileLauncherBuildingUpdate::getTemplate() const {
  const MissileLauncherBuildingUpdateModuleData *data =
      getMissileLauncherBuildingUpdateModuleData();
