...
// Desc:	 An update that checks for a status bit to stealth the owning
// object
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void StealthUpdateModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"StealthDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(StealthUpdateModuleData, m_stealthDelay)},
      {"MoveThresholdSpeed", INI::parseVelocityReal, NULL,
       offsetof(StealthUpdateModuleData, m_stealthSpeed)},
      {"StealthForbiddenConditions", INI::parseBitString32,
       TheStealthLevelNames, offsetof(StealthUpdateModuleData, m_stealthLevel)},
      {"HintDetectableConditions", INI::parseBitString32,
       TheObjectStatusBitNames,
       offsetof(StealthUpdateModuleData, m_hintDetectableStates)},
      {"FriendlyOpacityMin", INI::parsePercentToReal, NULL,
       offsetof(StealthUpdateModuleData, m_friendlyOpacityMin)},
      {"FriendlyOpacityMax", INI::parsePercentToReal, NULL,
       offsetof(StealthUpdateModuleData, m_friendlyOpacityMax)},
      {"PulseFrequency", INI::parseDurationUnsignedInt, NULL,
       offsetof(StealthUpdateModuleData, m_pulseFrames)},
      {"DisguisesAsTeam", INI::parseBool, NULL,
       offsetof(StealthUpdateModuleData, m_teamDisguised)},
      {"RevealDistanceFromTarget", INI::parseReal, NULL,
       offsetof(StealthUpdateModuleData, m_revealDistanceFromTarget)},
      {"OrderIdleEnemiesToAttackMeUponReveal", INI::parseBool, NULL,
       offsetof(StealthUpdateModuleData,
                m_orderIdleEnemiesToAttackMeUponReveal)},
      {"DisguiseFX", INI::parseFXList, NULL,
       offsetof(StealthUpdateModuleData, m_disguiseFX)},
      {"DisguiseRevealFX", INI::parseFXList, NULL,
       offsetof(StealthUpdateModuleData, m_disguiseRevealFX)},
      {"DisguiseTransitionTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(StealthUpdateModuleData, m_disguiseTransitionFrames)},
      {"DisguiseRevealTransitionTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(StealthUpdateModuleData, m_disguiseRevealTransitionFrames)},
      {"InnateStealth", INI::parseBool, NULL,
       offsetof(StealthUpdateModuleData, m_innateStealth)},
...
StealthUpdate::StealthUpdate(Thing *thing, const ModuleData *moduleData)
  ...
  if (data->m_innateStealth) {
    // Giving innate stealth units this status bit allows other code to easily
...
Bool StealthUpdate::allowedToStealth() const {
  ...
  if (flags & STEALTH_NOT_WHILE_ATTACKING &&
      self->getStatusBits() & OBJECT_STATUS_IS_FIRING_WEAPON) {
    // Doesn't stealth while aggressive (includes approaching).
    return false;
  }

  if (flags & STEALTH_NOT_WHILE_USING_ABILITY &&
      self->getStatusBits() & OBJECT_STATUS_IS_USING_ABILITY) {
    // Doesn't stealth while using a special ability (starting with preparation,
    // which takes place after unpacking).
    return false;
  }

  // Do a quick preliminary test to see if we are restricted by firing
  // particular weapons and we fired a shot last frame or this frame.
  if (flags & STEALTH_NOT_WHILE_FIRING_WEAPON &&
      self->getStatusBits() & OBJECT_STATUS_IS_FIRING_WEAPON) {
    if ((flags & STEALTH_NOT_WHILE_FIRING_WEAPON) ==
  ...
  if ((flags & STEALTH_NOT_WHILE_MOVING) && physics != NULL &&
      physics->getVelocityMagnitude() >
          getStealthUpdateModuleData()->m_stealthSpeed)
    return false;
...
void StealthUpdate::hintDetectableWhileUnstealthed() {
  ...
  if (self && (md->m_hintDetectableStates & self->getStatusBits())) {
    if (self->getControllingPlayer() == ThePlayerList->getLocalPlayer()) {
      ...
      if (selfDraw) selfDraw->setHeatVisionOpacity(1.0f);
    }
  }
}
...
StealthLookType StealthUpdate::calcStealthedStatusForPlayer(
    const Object *obj, const Player *player) {
  /*
          ...
          If stealthed (and not detected), you appear as (sv) to allies and (i)
     to others. If detected, you always appears as (sd).

          Sorry, there is one more condition, stealthed, but visible to friendly
     folks, YET detected In this state we render outselves visible and we
     ovlerlay the detection effect as a warning we'll call this
  ...
  */
  ...
  if (obj->getStatusBits() & OBJECT_STATUS_STEALTHED) {
    ...
    Relationship r =
        team ? team->getRelationship(player->getDefaultTeam()) : NEUTRAL;
    if (!player->isPlayerActive()) {
      // Observer players are friends to everyone!
      r = ALLIES;
    }

    // srj sez: disguised stuff doesn't work well when combined with the normal
    // "detected" stuff. so special case it here.
    if (canDisguise()) {
    ...
    {
      if (r == ALLIES)  // if we're friendly to the given player, detection DOES
                        // matter though.
      ...
      if (r == ALLIES) {
        ...
        // srj sez: disguised stuff doesn't work well when combined with the
        // normal "detected" stuff. so special case it above.
        //				if(
        //getStealthUpdateModuleData()->m_teamDisguised )
        //				{
        //					return
...
UpdateSleepTime StealthUpdate::update(void) {
  ...
  if (m_xferRestoreDisguise == TRUE) {
    ...
    // hack! if drawable was hidden (such as if we're inside a container) we
    // must keep that state
    if (draw && draw->isDrawableEffectivelyHidden()) wasHidden = TRUE;
    ...
    if (wasHidden && draw) draw->setDrawableHidden(TRUE);
  ...
  /// @todo srj -- improve sleeping behavior. we currently just sleep when not
  /// enabled,
  // and demand every-frame attention when enabled. this could probably be
  // smartened.
  ...
  if (draw) {
    ...
    if (self->isKindOf(KINDOF_MINE)) {
      ...
      if (m_transitioningToDisguise) {
        factor = 1.0f - ((Real)m_disguiseTransitionFrames /
                         (Real)data->m_disguiseTransitionFrames);
      } else {
        factor = 1.0f - ((Real)m_disguiseTransitionFrames /
                         (Real)data->m_disguiseRevealTransitionFrames);
      }
  ...
  if (revealDistance > 0.0f) {
    ...
    if (ai) {
      ...
      if (target) {
        Real distSqrd = ThePartitionManager->getDistanceSquared(self, target,
                                                                FROM_CENTER_2D);
  ...
  if (m_detectionExpiresFrame > now) {
    ...
    // if this is the first time your clearing the detected status, play the
    // stealth on sound
    if ((self->getStatusBits() & OBJECT_STATUS_DETECTED)) {
      detectedStatusChangedThisFrame = TRUE;
  ...
  if (detectedStatusChangedThisFrame) {
    // do the trick where we tell our container to recals his apparent
    // controlling player since I may have just become either detected or
    // undetected
    if (self->isContained()) {
  ...
  if (draw) {
    StealthLookType stealthLook =
        calcStealthedStatusForPlayer(self, ThePlayerList->getLocalPlayer());
...
void setWakeupIfInRange(Object *obj, void *userData) {
  ...
  srcpos.sub(&dstpos);
  if (srcpos.length() > vision) return;

  ai->wakeUpAndAttemptToTarget();

  //	if( obj->isKindOf( KINDOF_SELECTABLE ) && ( obj->isAbleToAttack() ||
  //!obj->isKindOf( KINDOF_STRUCTURE ) )) { 		Drawable *draw = obj->getDrawable();
  //		if( draw ) {
...
void StealthUpdate::markAsDetected(UnsignedInt numFrames) {
  ...
  if (!numFrames) {
    // Kris:
    // If numFrames is zero (the default value), use the stealth delay specified
    // in the ini file.
  ...
  if (data->m_orderIdleEnemiesToAttackMeUponReveal) {
    // This can't be a partitionmanager thing, because we need to know which
    // objects can see us. Therefore, walk the play list, and for each player
    // that considers us an enemy, check if any of their units can see us.
...
void StealthUpdate::disguiseAsObject(const Object *target) {
  ...
  if (target && target->getControllingPlayer()) {
    ...
    StealthUpdate *stealth =
        (StealthUpdate *)target->findUpdateModule(key_StealthUpdate);
    ...
    m_enabled = true;
    m_transitioningToDisguise =
        true;  // Means we are gaining disguise over time.
    ...
    m_transitioningToDisguise =
        false;  // Means we are losing the disguise over time.
    m_disguiseHalfpointReached = false;
  }
...
void StealthUpdate::changeVisualDisguise() {
  ...
  Drawable *draw = self->getDrawable();
  // We need to maintain our selection across the un/disguise, so pull selected
  // out here.
  ...
  if (m_disguiseAsTemplate) {
    ...
    if (draw) {
      ...
      if (self->getControllingPlayer()->getRelationship(
              clientPlayer->getDefaultTeam()) != ALLIES &&
    ...
    AudioEventRTS sound =
        *self->getTemplate()->getPerUnitSound("DisguiseStarted");
    ...
    if (draw) {
      ...
      // UGH!
      // A concrete example is the bomb truck. Different payloads are displayed
      // based on which upgrades have been made. When the bomb truck disguises
      // as something else, these subobjects are lost because the vector is
      // stored in W3DDrawModule. When we revert back to the original bomb
...
void StealthUpdate::xfer(Xfer *xfer) {
  ...
  AsciiString name = m_disguiseAsTemplate ? m_disguiseAsTemplate->getName()
  ...
  if (xfer->getXferMode() == XFER_LOAD) {
    ...
    if (name.isEmpty() == FALSE) {
      ...
      if (m_disguiseAsTemplate == NULL) {
        DEBUG_CRASH(
            ("StealthUpdate::xfer - Unknown template '%s'\n", name.str()));
...
void StealthUpdate::loadPostProcess(void) {
  ...
  //
  // we will need to restore our disguise when the game is ready to run ... NOTE
  // that we cannot restore it here because if we called changeVisualDisguise()
  // it would destroy our drawable and create new stuff.  The destruction of a
  // drawable during a load is *very* bad ... it has a snapshot instance in the
  // game state, other things may be pointing at it etc.
  //
  if (isDisguised()) m_xferRestoreDisguise = TRUE;
