...
enum DozerActionType {
  DOZER_ACTION_PICK_ACTION_POS,  ///< pick a location "around" the target to do
                                 ///< our action
...
class DozerActionPickActionPosState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(DozerActionPickActionPosState,
 ...
 protected:
  ...
  Int m_failedAttempts; /**< counter for successive unsuccessfull attempts to
                           pick and move to an action position */
};
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void DozerActionPickActionPosState::crc(Xfer *xfer) {}  // end crc
...
StateReturnType DozerActionPickActionPosState::update(void) {
  ...
  Object *goalObject =
      TheGameLogic->findObjectByID(dozerAI->getTaskTarget(m_task));

  // if there is no goal, get out of this machine with a failure code (success
  // is done in the action state )
  if (goalObject == NULL) {
  ...
  if (pos)
    ...
    //
    // find the vector from goal object to us ... we will start our search on
    // this angle so that we "approach" a closer point rather than a point on a
    // random side
    //
    Coord2D v;
    ...
    if (ThePartitionManager->findPositionAround(
            goalObject->getPosition(), &fpOptions, &goalPos) == FALSE) {
      // return STATE_FAILURE; no, we don't ever want dozers to fail,
      // particularly if ai.
    ...
    //
    // we only ignore the goal object when we did the point selection in this
    // more "random" method cause we could pick a place inside the object
    //
...
class DozerActionMoveToActionPosState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(DozerActionMoveToActionPosState,
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void DozerActionMoveToActionPosState::crc(Xfer *xfer) {}  // end crc
...
//-------------------------------------------------------------------------------------------------
/** We are supposed to be on route to our action position now, see when we get
 * there or detect that we have encountered a problem that's going to cause it
 * to give up */
//-------------------------------------------------------------------------------------------------
StateReturnType DozerActionMoveToActionPosState::update(void) {
  ...
  // sanity
  if (goalObject == NULL || dozer == NULL) return STATE_FAILURE;

  AIUpdateInterface *ai = dozer->getAIUpdateInterface();
  ObjectID currentRepairer = goalObject->getSoleHealingBenefactor();
  if (m_task == DOZER_TASK_REPAIR && currentRepairer != INVALID_ID &&
      currentRepairer != dozer->getID())  // oops I guess someone beat me to it!
  {
    if (ai) {
      DozerAIInterface *dozerAI = ai->getDozerAIInterface();
      if (dozerAI) dozerAI->internalTaskComplete(m_task);
    }
    getMachine()->setGoalObject(NULL);
    return STATE_FAILURE;
  }

  // Double oops, someone else has taken over the build responsibility for this
  // building.  We can't double up. This can happen because a guy who is moving
  // to build who is told to build will idle for a frame after registering. Two
  // workers, two started buildings. Grab both workers, resume on one building
  // First will register, second will see first as active and say no
  // Grab both, click on other building
  // First will register, and idle for a frame, second will see first as not
  // active and say yes Next frame, first will start the build task, without
  // reasking validity Infinite number of workers can be told to build something
  // with just two in progress buildings
  if ((m_task == DOZER_TASK_BUILD) && goalObject &&
      (goalObject->getBuilderID() != dozer->getID())) {
    // Geebus.  Returning failure is ignored, so you have to explicitly make the
    // ai stop trying to restart the machine
    if (ai) {
      DozerAIInterface *dozerAI = ai->getDozerAIInterface();
      if (dozerAI) dozerAI->internalTaskComplete(m_task);
    }
  ...
  Real distSqr = ThePartitionManager->getDistanceSquared(
      dozer, goalPos, FROM_BOUNDINGSPHERE_2D);
  ...
  if (distSqr <= allowableDistanceSqr) {
    if (m_task == DOZER_TASK_BUILD) {
      //
      // the object is now no longer awaiting construction, it is being
      // constructed ... note that we might possibly be here multiple times for
      // a single object being built but the setting of the same model condition
  ...
  if (ai && ai->isIdle()) return STATE_FAILURE;
...
class DozerActionDoActionState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(DozerActionDoActionState,
                                          "DozerActionDoActionState")

 public:
  DozerActionDoActionState(StateMachine *machine, DozerTask task)
      : State(machine, "DozerActionDoActionState") {
    m_task = task;
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void DozerActionDoActionState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void DozerActionDoActionState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType DozerActionDoActionState::onEnter(void) {
  ...
  DozerAIInterface *dozerAI =
      dozer->getAIUpdateInterface()->getDozerAIInterface();
  if (!dozerAI) {
    return STATE_FAILURE;
  }
  // DozerAIUpdate *dozerAI = static_cast<DozerAIUpdate
  // *>(dozer->getAIUpdateInterface());
...
StateReturnType DozerActionDoActionState::update(void) {
  ...
  // DozerAIUpdate *dozerAI = static_cast<DozerAIUpdate
  // *>(dozer->getAIUpdateInterface());
  //	const UnsignedInt ACTION_TIME = LOGICFRAMES_PER_SECOND * 4 ; // frames
  //to spend here in this state doing the action
  ...
  switch (m_task) {
    ...
    case DOZER_TASK_BUILD: {
      // GS Moved this inside Build, since you are allowed to Repair things that
      // are not your player (canRepairObject handles it)
      if (dozer->getControllingPlayer() !=
          goalObject->getControllingPlayer())  // Yipes, SOmehow I have changed
                                               // sides in mid build!
        return STATE_FAILURE;

      // if we need to select the dock location and move there do so
      if (dozerAI->getBuildSubTask() == DOZER_SELECT_BUILD_DOCK_LOCATION) {
        const Coord3D *dockLocation =
            dozerAI->getDockPoint(m_task, DOZER_DOCK_POINT_ACTION);
        if (dockLocation) ai->aiMoveToPosition(dockLocation, CMD_FROM_AI);
      ...
      // if we're moving to the build dock location, when we become idle we are
      // there
      if (dozerAI->getBuildSubTask() == DOZER_MOVING_TO_BUILD_DOCK_LOCATION) {
        if (ai->isIdle()) {
          dozerAI->setBuildSubTask(DOZER_DO_BUILD_AT_DOCK);
          // Get the audio sound and start playing the construction sound (get
          // the sound from the building itself)
          dozerAI->startBuildingSound(
              goalObject->getTemplate()->getPerUnitSound("UnderConstruction"),
              goalObject->getID());
        }
      }  // end if
      ...
      if (dozerAI->getBuildSubTask() == DOZER_DO_BUILD_AT_DOCK) {
        ...
        Int framesToBuild = goalObject->getTemplate()->calcTimeToBuild(
            dozer->getControllingPlayer());
        Real percentProgressThisFrame = 100.0f / framesToBuild;
        goalObject->setConstructionPercent(
            goalObject->getConstructionPercent() + percentProgressThisFrame);

        //
        // every time we construct a piece of the goal object, the goal object
        // gets a little bit o health, note that we're bypassing the regular
        // healing/damage methods here and going straight for the change of the
        // health
        //
        BodyModuleInterface *body = goalObject->getBodyModule();
        body->internalChangeHealth(body->getMaxHealth() /
                                   INT_TO_REAL(framesToBuild));

        //
        // since we've just actually contributed a "piece" to this building,
        // we'll say that we are now the producer and the builder
        //
        ...
        if (goalObject->getConstructionPercent() >= 100.0f) {
          ...
          // object will now be idle instead of in one of the construction
          // actions
          goalObject->clearModelConditionFlags(MAKE_MODELCONDITION_MASK3(
          ...
          //
          // now that we're done with construction we evaluate the visual
          // condition of the object since while under construction visual
          // changes due to damage state do not occur.  Note that is was
          // important to call this after we cleared the model conditions
          // involving construction because part of this evaluation is to auto
          // populate the model with particle systems when special named bones
          // for the current given state are provided
          //
          ...
          if (player) {
            ...
            player->onStructureConstructionComplete(dozer, goalObject,
                                                    dozerAI->getIsRebuild());

            //
            // Now onCreates were called at construction start.  Now at finish
            // is when we want the Game side of OnCreate
            //
            for (BehaviorModule **m = goalObject->getBehaviorModules(); *m;
                 ++m) {
          ...
          // Creation is another valid and essential time to call this.  This
          // building now Looks.
          ...
          if (dozer->isLocallyControlled()) {
            ...
            UnicodeString format =
                TheGameText->fetch("DOZER:ConstructionComplete");
            UnicodeString objectName =
                goalObject->getTemplate()->getDisplayName();
            if (objectName.isEmpty()) {
              UnicodeString format =
                  TheGameText->fetch("INI:MissingDisplayName");

              objectName.format(format,
                                goalObject->getTemplate()->getName().str());
            ...
            TheRadar->createEvent(goalObject->getPosition(),
          ...
          const Coord3D *endPos =
              dozerAI->getDockPoint(m_task, DOZER_DOCK_POINT_END);
          Coord3D pos = *dozer->getPosition();
          if (endPos) {
            pos = *endPos;
          }
          // Our goal may be inside a building, if we are packing them in tight,
          // so try adjusting. jba.
          TheAI->pathfinder()->adjustToPossibleDestination(
              dozer, ai->getLocomotorSet(), &pos);
    ...
    case DOZER_TASK_REPAIR: {
      ...
      if (body->getHealth() == body->getMaxHealth()) {
        // issue repair complete message, we might want to remove this later
        // cause it could be annoying
        ...
        // if we are repairing a bridge, create scaffolding over the bridge if
        // we need to
        if (goalObject->isKindOf(KINDOF_BRIDGE_TOWER))
          dozerAI->createBridgeScaffolding(goalObject);

        // the builder is now actively repairing something, we'll borrow the
        ...
        //
        if (goalObject->isKindOf(KINDOF_BRIDGE_TOWER)) {
          BridgeTowerBehaviorInterface *btbi =
          ...
          Object *bridgeObject =
              TheGameLogic->findObjectByID(btbi->getBridgeID());
          DEBUG_ASSERTCRASH(bridgeObject,
                            ("Unable to find bridge center object\n"));
          BridgeBehaviorInterface *bbi =
              BridgeBehavior::getBridgeBehaviorInterfaceFromObject(
                  bridgeObject);
          DEBUG_ASSERTCRASH(bbi, ("Unable to find bridge interface from tower "
                                  "goal object during repair\n"));

          if (bbi->isScaffoldInMotion() == TRUE) canHeal = FALSE;
        ...
        if (canHeal) {
          ...
          Real health = body->getMaxHealth() *
                        dozerAI->getRepairHealthPerSecond() /
                        LOGICFRAMES_PER_SECOND;

          // try to give it a little bit-o-health
          if (!goalObject->attemptHealingFromSoleBenefactor(
                  health, dozer, 2))  // this frame and the next
          {
            // goalObject->setStatus( OBJECT_STATUS_UNDERGOING_REPAIR );
            // This bit used to be set way back in
            // DozerAIUpdate::privateRepair(), but it has been outmoded so that
            // several dozers/workers can target the same sick building for
            // repair, but the first one to begin this is done by
            // attemptHealingFromSoleBenefactor() which lets the beneficiary of
            // the healing remember who has been healing it, and will return
            // false to everybody else
            // or if the goalObject is already receiving healing, I must stop,
            // since my healing is getting rejected here
  ...
  if (complete == TRUE) {
    ...
    getMachineOwner()->setWeaponSetFlag(
        WEAPONSET_MINE_CLEARING_DETAIL);  // maybe go clear some mines, if I
                                          // feel like it
    // we're done
    return STATE_SUCCESS;
  ...
  getMachineOwner()->clearWeaponSetFlag(
      WEAPONSET_MINE_CLEARING_DETAIL);  // no mine clearing fun while I'm on the
                                        // job
...
class DozerActionStateMachine : public StateMachine {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(DozerActionStateMachine,
                                          "DozerActionStateMachine");
...
DozerActionStateMachine::DozerActionStateMachine(Object *owner, DozerTask task)
  ...
  defineState(DOZER_ACTION_PICK_ACTION_POS,
              newInstance(DozerActionPickActionPosState)(this, task),
              DOZER_ACTION_MOVE_TO_ACTION_POS, EXIT_MACHINE_WITH_FAILURE);
  defineState(DOZER_ACTION_MOVE_TO_ACTION_POS,
              newInstance(DozerActionMoveToActionPosState)(this, task),
              DOZER_ACTION_DO_ACTION, DOZER_ACTION_PICK_ACTION_POS);
  defineState(DOZER_ACTION_DO_ACTION,
              newInstance(DozerActionDoActionState)(this, task),
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void DozerActionStateMachine::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void DozerActionStateMachine::loadPostProcess(void) {}  // end loadPostProcess
...
static Object *findObjectToRepair(Object *dozer) {
  // sanity
  if (dozer == NULL) return NULL;
  if (!dozer->getAIUpdateInterface()) {
    return NULL;
  }
  const DozerAIInterface *dozerAI =
      dozer->getAIUpdateInterface()->getDozerAIInterface();
  ...
  for (obj = iter->first(); obj; obj = iter->next()) {
    ...
    if (closestRepairTarget == NULL) {
      ...
      closestRepairTargetDistSqr =
          ThePartitionManager->getDistanceSquared(dozer, obj, FROM_CENTER_2D);
...
static Object *findMine(Object *dozer) {
  ...
  // srj sez: only clear enemy or neutal mines. clearing allied mines (ie, OURS)
  // is really dumb. (and no, PossibleToAttack won't necessarily filter these
  // out.)
  PartitionFilterRelationship filterTeam(
      dozer, PartitionFilterRelationship::ALLOW_ENEMIES |
                 PartitionFilterRelationship::ALLOW_NEUTRAL);
  PartitionFilterPossibleToAttack filterAttack(ATTACK_NEW_TARGET, dozer,
                                               CMD_FROM_DOZER);
  PartitionFilterSameMapStatus filterMapStatus(dozer);
  PartitionFilter *filters[] = {&filterTeam, &filterAttack, &filterMapStatus,
                                NULL};
  Object *mine = ThePartitionManager->getClosestObject(
      dozer, dozerAI->getBoredRange(), FROM_CENTER_2D, filters);
...
enum {
  ...
  DOZER_PRIMARY_FORTIFY,  ///< dozer is fortifying a civilian building, making
                          ///< it stronger
  DOZER_PRIMARY_GO_HOME,  ///< dozer has nothing else to do so it will return a
                          ///< command center
};
...
class DozerPrimaryIdleState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(DozerPrimaryIdleState,
                                          "DozerPrimaryIdleState")
 ...
 protected:
  UnsignedInt
      m_idleTooLongTimestamp;  ///< when this is more than our idle too long
                               ///< time we try to do something about it
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void DozerPrimaryIdleState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void DozerPrimaryIdleState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType DozerPrimaryIdleState::onEnter(void) {
  //
  // upon entering this state we begin tracking how long we're idle for to try
  // to do something about it every once in a while
  //
  ...
  getMachineOwner()->setWeaponSetFlag(
      WEAPONSET_MINE_CLEARING_DETAIL);  // maybe go clear some mines, if I feel
                                        // like it
...
StateReturnType DozerPrimaryIdleState::update(void) {
  ...
  //
  // if our actual real AI state machine is not idle we're doing something so
  // let's reset our idle too long timestamp
  //
  if (!ai->isIdle()) m_idleTooLongTimestamp = TheGameLogic->getFrame();

  //
  // if we're just sitting here in idle, and there is no task pending ... after
  // so long we'll try to go to the nearest command center
  //
  if (TheGameLogic->getFrame() - m_idleTooLongTimestamp >
          dozerAI->getBoredTime() &&
      dozerAI->isAnyTaskPending() == FALSE) {
    //
    // to prevent us from doing this potentially expensive logic to find objects
    // every frame we'll only try it every once in a while so set our idle too
    // long timestamp to the current frame
    //
    m_idleTooLongTimestamp = TheGameLogic->getFrame();
    ...
    if (repairTarget) {
      ...
      //
      // in theory we would need to "interrupt" whatever it is the Dozer is
      // doing now, but we know we're in the idle state doing nothing so we
...
class DozerActionState : public State {
 ...
 public:
  DozerActionState(StateMachine *machine, DozerTask task)
    ...
    m_task = task;
    m_actionMachine =
        newInstance(DozerActionStateMachine)(getMachineOwner(), task);
    m_actionMachine->initDefaultState();
  }
  /*
          Note that we DON'T use CONVERT_SLEEP_TO_CONTINUE; since we're not
     doing anything else interesting in update, we can sleep when this machine
     sleeps
  */
  virtual StateReturnType update(void) {
    return m_actionMachine->updateStateMachine();
  }
...
inline DozerActionState::~DozerActionState(void) {
  if (m_actionMachine) m_actionMachine->deleteInstance();
}

// ------------------------------------------------------------------------------------------------
/** CRC */
// ------------------------------------------------------------------------------------------------
void DozerActionState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void DozerActionState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType DozerActionState::onEnter(void) {
  ...
  // note that during an onEnter, since the action machine is persistent,
  // anytime we transition into doing an action, we need to reset our state
  // machine to do that action as it may have been left in any state from the
  // previous time we ran it
  //
...
class DozerPrimaryGoingHomeState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(DozerPrimaryGoingHomeState,
                                          "DozerPrimaryGoingHomeState")
 ...
 public:
  DozerPrimaryGoingHomeState(StateMachine *machine)
      : State(machine, "DozerPrimaryGoingHomeState") {}
...
DozerPrimaryStateMachine::DozerPrimaryStateMachine(Object *owner)
  ...
  // order matters: first state is the default state.
  defineState(DOZER_PRIMARY_IDLE, newInstance(DozerPrimaryIdleState)(this),
              INVALID_STATE_ID, INVALID_STATE_ID, idleConditions);
  defineState(DOZER_PRIMARY_BUILD,
              newInstance(DozerActionState)(this, DOZER_TASK_BUILD),
              DOZER_PRIMARY_IDLE, DOZER_PRIMARY_IDLE);
  defineState(DOZER_PRIMARY_REPAIR,
              newInstance(DozerActionState)(this, DOZER_TASK_REPAIR),
              DOZER_PRIMARY_IDLE, DOZER_PRIMARY_IDLE);
  defineState(DOZER_PRIMARY_FORTIFY,
              newInstance(DozerActionState)(this, DOZER_TASK_FORTIFY),
              DOZER_PRIMARY_IDLE, DOZER_PRIMARY_IDLE);
  defineState(DOZER_PRIMARY_GO_HOME,
              newInstance(DozerPrimaryGoingHomeState)(this), DOZER_PRIMARY_IDLE,
              DOZER_PRIMARY_IDLE);
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
Bool DozerPrimaryStateMachine::isBuildMostImportant(State *thisState,
                                                    void *userData) {
  ...
  if (!ai->isIdle()) return FALSE;  // busy doing something else
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
Bool DozerPrimaryStateMachine::isRepairMostImportant(State *thisState,
                                                     void *userData) {
  Object *dozer = thisState->getMachineOwner();
  AIUpdateInterface *ai = dozer->getAIUpdateInterface();
  if (!ai) {
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
Bool DozerPrimaryStateMachine::isFortifyMostImportant(State *thisState,
                                                      void *userData) {
  Object *dozer = thisState->getMachineOwner();
  AIUpdateInterface *ai = dozer->getAIUpdateInterface();
  if (!ai) {
...
void DozerAIUpdateModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"RepairHealthPercentPerSecond", INI::parsePercentToReal, NULL,
       offsetof(DozerAIUpdateModuleData, m_repairHealthPercentPerSecond)},
      {"BoredTime", INI::parseDurationReal, NULL,
       offsetof(DozerAIUpdateModuleData, m_boredTime)},
      {"BoredRange", INI::parseReal, NULL,
       offsetof(DozerAIUpdateModuleData, m_boredRange)},
      {0, 0, 0, 0}};
...

{
  ...
  m_buildSubTask = DOZER_SELECT_BUILD_DOCK_LOCATION;  // irrelavant, but I want
                                                      // non-garbage value

  //
  // initialize the dozer machine to NULL, we want to do this and create it
  // during the update implementation because at this point we don't have the
  // object all setup
  //
  m_dozerMachine = NULL;
...
DozerAIUpdate::~DozerAIUpdate(void) {
  ...
  if (m_dozerMachine) m_dozerMachine->deleteInstance();
...
// ------------------------------------------------------------------------------------------------
/** Create the bridge scaffolding if necessary for the bridge that is attached
 * to this tower */
// ------------------------------------------------------------------------------------------------
void DozerAIUpdate::createBridgeScaffolding(Object *bridgeTower) {
  // sanity
  if (bridgeTower == NULL) return;

  // get the bridge behavior interface from the bridge object that this tower is
  // a part of
  BridgeTowerBehaviorInterface *btbi =
      BridgeTowerBehavior::getBridgeTowerBehaviorInterfaceFromObject(
          bridgeTower);
...
// ------------------------------------------------------------------------------------------------
/** Remove the bridge scaffolding from the bridge object that is attached to
 * this tower */
// ------------------------------------------------------------------------------------------------
...
UpdateSleepTime DozerAIUpdate::update(void) {
  //
  // NOTE: Any changes to DozerAIUpdate::* you probably want to reflect and copy
  // into WorkerAIUPdate:* as well ... sigh
  //

  //
  // now that we're really executing we have all the necessary object modules in
  // place to correctly create a state machine and set the default state
  //
  createMachines();
  ...
  if (getObject()->isEffectivelyDead()) return UPDATE_SLEEP_NONE;
  ...
  if (currentTask != DOZER_TASK_INVALID) {
    ...
    if (currentTask == DOZER_TASK_REPAIR &&
        TheActionManager->canRepairObject(getObject(), targetObject,
                                          getLastCommandSource()) == FALSE)
      invalidTask = TRUE;

    // cancel the task if it's now invalid
    if (invalidTask == TRUE) cancelTask(currentTask);

  }  // end if
  else
    getObject()->setWeaponSetFlag(
        WEAPONSET_MINE_CLEARING_DETAIL);  // maybe go clear some mines, if I
                                          // feel like it
...
//-------------------------------------------------------------------------------------------------
Object *DozerAIUpdate::construct(const ThingTemplate *what, const Coord3D *pos,
                                 Real angle, Player *owningPlayer,
                                 Bool isRebuild) {
  // !!! NOTE: If you modify this you must modify the worker too !!!
  // !!! Graham: Please please please have inspiration for how to *not*
  // duplicate this code
  ...
  // sanity
  if (what == NULL || pos == NULL || owningPlayer == NULL) return NULL;

  // sanity
  DEBUG_ASSERTCRASH(getObject()->getControllingPlayer() == owningPlayer,
                    ("Dozer::Construct - The controlling player of the Dozer "
                     "is not the owning player passed in\n"));
  ...
  if (isRebuild) BitSet(statusBits, OBJECT_STATUS_RECONSTRUCTING);
  ...
  // Flatten the terrain underneath the object, then adjust to the flattened
  // height. jba.
  ...
  // set a construction percent for the new object to zero and a status for
  // under construction
...
// ------------------------------------------------------------------------------------------------
/** Given our current task and repair target, can we accept this as a new repair
 * target */
// ------------------------------------------------------------------------------------------------
Bool DozerAIUpdate::canAcceptNewRepair(Object *obj) {
  ...
  if (getCurrentTask() != DOZER_TASK_REPAIR) return TRUE;

  // get current repair target
  Object *currentRepair =
      TheGameLogic->findObjectByID(m_task[DOZER_TASK_REPAIR].m_targetObjectID);

  if (currentRepair) {
    // check for same object
    if (currentRepair == obj) return FALSE;
    ...
    if (currentRepair->isKindOf(KINDOF_BRIDGE_TOWER) &&
      ...
      if (currentTowerInterface->getBridgeID() ==
          newTowerInterface->getBridgeID())
        return FALSE;
...
void DozerAIUpdate::privateRepair(Object *obj, CommandSourceType cmdSource) {
  ...
  if (canAcceptNewRepair(obj) == FALSE) return;

  // sanity, if we can't repair the object then get out of there
  if (TheActionManager->canRepairObject(dozer, obj, cmdSource) == FALSE) return;

  // if this object is already actively being repaired by another dozertype we
  // won't also try to go repair it
  ...
  // if this object is a bridge tower, we need to check the status of the bridge
  // in order to check for a 'duplicate repair'
  //
  // if( obj->isKindOf( KINDOF_BRIDGE_TOWER ) )
  //{
  //	BridgeTowerBehaviorInterface *btbi =
  //BridgeTowerBehavior::getBridgeTowerBehaviorInterfaceFromObject( obj );
  //	DEBUG_ASSERTCRASH( btbi, ("Unable to find bridge tower behavior
  //interface\n") );
  //
...
// ------------------------------------------------------------------------------------------------
void DozerAIUpdate::privateResumeConstruction(Object *obj,
                                              CommandSourceType cmdSource) {
  // sanity
  if (obj == NULL) return;

  // make sure we can resume construction on this
  if (TheActionManager->canResumeConstructionOf(getObject(), obj, cmdSource) ==
      FALSE)
    return;
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
/*static*/ Bool DozerAIUpdate::findGoodBuildOrRepairPosition(
    const Object *me, const Object *target, Coord3D &positionOut) {
  ...
  Coord3D bestPosition = theirPosition;     // This answer is the best, as it
                                            // includes findPositionAround
  Coord3D workingPosition = theirPosition;  // But if findPositionAround fails,
  ...
  // this is a little cheesy... the idea is that we can only choose a location
  // that is pretty close in z to the desired one. this prevents us from
  // choosing a space at the bottom of a cliff when the space we want is at the
  // top of the cliff. ideally we should do a funky terrain-zone compare but
  // that isn't well-exposed... (srj)
  ...
  fpOptions.sourceToPathToDest =
      me;  // This makes it find a place forWhom can get to.
  if (!me->isUsingAirborneLocomotor()) fpOptions.maxZDelta = MAX_Z_DELTA;
  if (me->isUsingAirborneLocomotor())
    fpOptions.ignoreObject =
        target;  // Flyers can ignore stuff, so they can approach right over the
                 // target if they want.

  Bool spotFound = ThePartitionManager->findPositionAround(
      &workingPosition, &fpOptions, &bestPosition);
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
/*static*/ Object *DozerAIUpdate::findGoodBuildOrRepairPositionAndTarget(
  ...
  if (target->isKindOf(KINDOF_BRIDGE)) {
    ...
    if (bbi) {
      ...
      for (Int i = 0; i < BRIDGE_MAX_TOWERS; ++i) {
        Object *tower =
            TheGameLogic->findObjectByID(bbi->getTowerID((BridgeTowerType)i));
        if (tower) {
          Coord3D tmp;
          ...
          if (found && ai->isPathAvailable(&tmp)) {
            Real thisDistSqr = sqr(me->getPosition()->x - tmp.x) +
                               sqr(me->getPosition()->y - tmp.y);
...
void DozerAIUpdate::newTask(DozerTask task, Object *target) {
  // sanity
  DEBUG_ASSERTCRASH(task >= 0 && task < DOZER_NUM_TASKS,
                    ("Illegal dozer task '%d'\n", task));

  // sanity
  if (target == NULL) return;

  //
  // special check for the build task, we should never be given more than one of
  // them ... for the other tasks we just forget what we were doing and the new
  // target takes precedence for the task
  //
  if (task == DOZER_TASK_BUILD || task == DOZER_TASK_REPAIR) {
    ...
    if (target == NULL) return;  // could happen for some bridges

    //
    // for building, we say that even "thinking" about building or rebuilding an
    // object sets us as the current builder of that object.  this allows any
    // dozers that are ordered later to resume construction on something to see
    // that somebody is already taking care of it and then they won't be even
    // try to resume a build since we don't allow multiple dozers/workers to
    // double up on construction efforts
    //
    ...
    Coord3D offset;
    offset.set(position.x - target->getPosition()->x,
               position.y - target->getPosition()->y, 0);
    offset.normalize();
    offset.scale(5 * PATHFIND_CELL_SIZE_F);
    position.add(
        &offset);  // move away from the dock point at the end of build.
  ...
  // reset the dozer behavior so that it can re-evluate which task to continue
  // working on
...
Bool DozerAIUpdate::isAnyTaskPending(void) {
  for (Int i = 0; i < DOZER_NUM_TASKS; i++)
    if (isTaskPending((DozerTask)i)) return TRUE;
...
//-------------------------------------------------------------------------------------------------
/** Clear a task from the Dozer for consideration, we can use this when a goal
 * object becomes invalid/destroyed etc. */
//-------------------------------------------------------------------------------------------------
...
void DozerAIUpdate::internalTaskCompleteOrCancelled(DozerTask task) {
  switch (task) {
    ...
    case DOZER_TASK_BUILD: {
      ...
      getObject()->clearModelConditionState(
          MODELCONDITION_ACTIVELY_CONSTRUCTING);

      // And the thing we were working on is no longer being actively built

      ///@todo This would be correct except that we don't have idle crane
      ///animations and it is December.
      //			Object* goalObject =
      //TheGameLogic->findObjectByID(m_task[task].m_targetObjectID); 			if
      //(goalObject != NULL)
      //			{
    ...
    case DOZER_TASK_REPAIR: {
      ...
      // Bridges have been made indestructible, so the code below was
      // meaningless. -- ML Object *obj = NULL; get object to reapir (if
      // present)
      ...
      //
      //
      //	 when we're done repairing bridges, tell the scaffolding to go
      //away and also remove 	 the undergoing repair status from the bridge object
      //itself
      //
      //	if( obj->isKindOf( KINDOF_BRIDGE_TOWER ) )
      //	{
      ...
      //		BridgeTowerBehaviorInterface *btbi =
      //BridgeTowerBehavior::getBridgeTowerBehaviorInterfaceFromObject( obj );
      //		DEBUG_ASSERTCRASH( btbi, ("Unable to find bridge tower
      //behavior interface\n") ); 		Object *bridge = TheGameLogic->findObjectByID(
      //btbi->getBridgeID() ); 		DEBUG_ASSERTCRASH( bridge, ("Unable to find
      //bridge object\n") ); 		bridge->clearStatus(
    ...
    default: {
      DEBUG_CRASH(
          ("internalTaskCompleteOrCancelled: Unknown Dozer task '%d'\n", task));
      break;
...
void DozerAIUpdate::onDelete(void) {
  ...
  for (i = DOZER_TASK_FIRST; i < DOZER_NUM_TASKS; ++i) {
    if (isTaskPending((DozerTask)i)) cancelTask((DozerTask)i);

  }  // end for i

  for (i = 0; i < DOZER_NUM_TASKS; i++) {
    Object *goalObject =
        TheGameLogic->findObjectByID(m_task[i].m_targetObjectID);
    if (goalObject != NULL) {
      goalObject->clearModelConditionState(
          MODELCONDITION_ACTIVELY_BEING_CONSTRUCTED);
    }
  }
}
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
const Coord3D *DozerAIUpdate::getDockPoint(DozerTask task,
                                           DozerDockPoint point) {
  // sanity
  if (task < 0 || task >= DOZER_NUM_TASKS) return NULL;

  // sanity
  if (point < 0 || point >= DOZER_NUM_DOCK_POINTS) return NULL;

  // if the point has been set (is valid) then return it
  if (m_dockPoint[task][point].valid) return &m_dockPoint[task][point].location;
...
Real DozerAIUpdate::getBoredRange(void) const {
  if (getObject()->getControllingPlayer() &&
    ...
    return TheAI->getAiData()->m_aiDozerBoredRadiusModifier *
           getDozerAIUpdateModuleData()->m_boredRange;
  }
...
void DozerAIUpdate::aiDoCommand(const AICommandParms *parms) {
  //
  // anytime we get a command, just remove any model condition that has us
  // actively building if we need to show that, that bit will be set anyway
  ...
  switch (parms->m_cmd) {
    case AICMD_MOVE_AWAY_FROM_UNIT: {
      ...
      // if we have no task right now, go idle so we can immediately respond to
      // this
    ...
    case AICMD_REPAIR: {
      // if we have no task right now, go idle so we can immediately respond to
      // this
      if (getCurrentTask() == DOZER_TASK_INVALID) aiIdle(CMD_FROM_AI);
    ...
    default: {
      ...
      if (parms->m_cmdSource == CMD_FROM_PLAYER &&
          getCurrentTask() != DOZER_TASK_INVALID)
        cancelTask(getCurrentTask());

      // issue the command
      AIUpdateInterface::aiDoCommand(parms);

      // when a player issues commands, this will cause the dozer to re-evaluate
      // what it's doing
      if (parms->m_cmdSource == CMD_FROM_PLAYER)
        m_dozerMachine->resetToDefaultState();
      break;
...
//------------------------------------------------------------------------------------------------
void DozerAIUpdate::startBuildingSound(const AudioEventRTS *sound,
