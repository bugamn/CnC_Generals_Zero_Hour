...
AIStateMachine *HackInternetAIUpdate::makeStateMachine() {
  return newInstance(HackInternetStateMachine)(getObject(),
                                               "HackInternetBasicAI");
}

//-------------------------------------------------------------------------------------------------
HackInternetAIUpdate::HackInternetAIUpdate(Thing *thing,
...
void HackInternetAIUpdate::hackInternet() {
  ...
  // m_hackInternetStateMachine = newInstance(HackInternetStateMachine)(
  // getObject() ); m_hackInternetStateMachine->initDefaultState();
#ifdef _DEBUG
...
//-------------------------------------------------------------------------------------------------
HackInternetStateMachine::HackInternetStateMachine(Object *owner,
                                                   AsciiString name)
    : AIStateMachine(owner, "HackInternetStateMachine") {
  // HackInternetAIUpdate *ai =
  // (HackInternetAIUpdate*)owner->getAIUpdateInterface();

  // order matters: first state is the default state.
  defineState(UNPACKING, newInstance(UnpackingState)(this), HACK_INTERNET,
              HACK_INTERNET);
  defineState(HACK_INTERNET, newInstance(HackInternetState)(this), PACKING,
              PACKING);
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void UnpackingState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void UnpackingState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType UnpackingState::onEnter() {
  ...
  owner->clearModelConditionFlags(
      MAKE_MODELCONDITION_MASK3(MODELCONDITION_PACKING, MODELCONDITION_FIRING_A,
  ...
  Real variation =
      GameLogicRandomValueReal(1.0f - variationFactor, 1.0f + variationFactor);
...
StateReturnType UnpackingState::update() {
  ...
  // This is a bit hacky, no pun intended, but if this Update is engeged
  // specialability (disablebuilding) The unpacking modelconditionflag gets
  // cleared by specialability::cleanup() after my onEnter() sets it! Why
  // HackInterent wasn't included within specialability I can't figure out,
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void PackingState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void PackingState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType PackingState::onEnter() {
  ...
  owner->clearAndSetModelConditionFlags(
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void HackInternetState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void HackInternetState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType HackInternetState::update() {
  ...
  if (m_framesRemaining > 0) {
    ...
    // We have waited the full amount of the delay, so hack some cash from the
    // heavens!
    ...
    if (money) {
      ...
      if (xp) {
        ...
        AudioEventRTS sound =
            *(owner->getTemplate()->getPerUnitSound("UnitCashPing"));
        sound.setObjectID(owner->getID());
        TheAudio->addAudioEvent(&sound);
      }
    }
