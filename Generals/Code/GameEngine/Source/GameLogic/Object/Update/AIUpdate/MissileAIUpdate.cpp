...
void MissileAIUpdateModuleData::buildFieldParse(MultiIniFieldParse& p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"TryToFollowTarget", INI::parseBool, NULL,
       offsetof(MissileAIUpdateModuleData, m_tryToFollowTarget)},
      {"FuelLifetime", INI::parseDurationUnsignedInt, NULL,
       offsetof(MissileAIUpdateModuleData, m_fuelLifetime)},
      {"IgnitionDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(MissileAIUpdateModuleData, m_ignitionDelay)},
      {"InitialVelocity", INI::parseVelocityReal, NULL,
       offsetof(MissileAIUpdateModuleData, m_initialVel)},
      {"DistanceToTravelBeforeTurning", INI::parseReal, NULL,
       offsetof(MissileAIUpdateModuleData, m_initialDist)},
      {"DistanceToTargetBeforeDiving", INI::parseReal, NULL,
       offsetof(MissileAIUpdateModuleData, m_diveDistance)},
      {"DistanceToTargetForLock", INI::parseReal, NULL,
       offsetof(MissileAIUpdateModuleData, m_lockDistance)},
      {"IgnitionFX", INI::parseFXList, NULL,
       offsetof(MissileAIUpdateModuleData, m_ignitionFX)},
      {"UseWeaponSpeed", INI::parseBool, NULL,
       offsetof(MissileAIUpdateModuleData, m_useWeaponSpeed)},
      {"DetonateOnNoFuel", INI::parseBool, NULL,
       offsetof(MissileAIUpdateModuleData, m_detonateOnNoFuel)},

      {"GarrisonHitKillRequiredKindOf", KindOfMaskType::parseFromINI, NULL,
       offsetof(MissileAIUpdateModuleData, m_garrisonHitKillKindof)},
      {"GarrisonHitKillForbiddenKindOf", KindOfMaskType::parseFromINI, NULL,
       offsetof(MissileAIUpdateModuleData, m_garrisonHitKillKindofNot)},
      {"GarrisonHitKillCount", INI::parseUnsignedInt, NULL,
       offsetof(MissileAIUpdateModuleData, m_garrisonHitKillCount)},
      {"GarrisonHitKillFX", INI::parseFXList, NULL,
       offsetof(MissileAIUpdateModuleData, m_garrisonHitKillFX)},
      {0, 0, 0, 0}};
...
//-------------------------------------------------------------------------------------------------
MissileAIUpdate::MissileAIUpdate(Thing* thing, const ModuleData* moduleData)
  ...
  m_nextTargetTrackTime =
      0x7fffffff;  // so that we never recalc target pos, by default
...
//-------------------------------------------------------------------------------------------------
MissileAIUpdate::~MissileAIUpdate() { tossExhaust(); }

//-------------------------------------------------------------------------------------------------
void MissileAIUpdate::onDelete(void) {
  //
  // there is no need to destroy the attached particle systems here because the
  // particle systems themselves detect that their "parent" object is gone and
  // destroy themselves. but to be clean, for the exhaust particle system, we
  // will invalidate the ID so that we can't do anything else with it since
  // we're being deleted
  //
  m_exhaustID = INVALID_PARTICLE_SYSTEM_ID;
}
...
void MissileAIUpdate::projectileLaunchAtObjectOrPosition(
  ...
  Weapon::positionProjectileForLaunch(getObject(), launcher, wslot,
                                      specificBarrelToUse);
...
//-------------------------------------------------------------------------------------------------
void MissileAIUpdate::projectileFireAtObjectOrPosition(
  ...
  m_isTrackingTarget = false;
  // Missiles do their thing by colliding with something and exploding.  So they
  // Move to the target instead of Attacking the target.
  if (victim && d->m_tryToFollowTarget) {
    ...
    m_isTrackingTarget = TRUE;  // Remember that I was originally shot at a
                                // moving object, so if the
...
Bool MissileAIUpdate::projectileHandleCollision(Object* other) {
  ...
  if (projectileIsArmed() == false) return true;
  ...
  if (other != NULL) {
    ...
    if (!m_detonationWeaponTmpl->shouldProjectileCollideWith(
            projectileLauncher, obj, other, m_victimID)) {
...
void MissileAIUpdate::doIgnitionState() {
  ...
  if (m_exhaustSysTmpl != NULL) {
    m_exhaustID = TheParticleSystemManager->createAttachedParticleSystemID(
        m_exhaustSysTmpl, getObject());
  }

  // arm the missile's "warhead"
  m_isArmed = true;
  UnsignedInt now = TheGameLogic->getFrame();
  m_fuelExpirationDate =
      d->m_fuelLifetime ? (now + d->m_fuelLifetime) : 0x7fffffff;
...
void MissileAIUpdate::doAttackState(Bool turnOK) {
  ...
  if (d->m_lockDistance > 0) {
    ...
    if (m_isTrackingTarget && (getGoalObject() != NULL)) {
      distanceToTargetSquared = ThePartitionManager->getDistanceSquared(
          getObject(), getGoalObject(), FROM_CENTER_2D);
    } else {
      distanceToTargetSquared = ThePartitionManager->getDistanceSquared(
          getObject(), getGoalPosition(), FROM_CENTER_2D);
    }
  ...
  if (curLoco && curLoco->getPreferredHeight() > 0) {
    ...
    Real distanceToTargetSquared = ThePartitionManager->getDistanceSquared(
        getObject(), getGoalPosition(), FROM_CENTER_2D);
  ...
  // If I was fired at a flyer and have lost target (most likely they died),
  // then I need to do something better than cloverleaf around their last spot.
  if (m_isTrackingTarget && (getGoalObject() == NULL)) airborneTargetGone();
}
...
void MissileAIUpdate::doKillState(void) {
  ...
  /* Kill state uses the OBJECT_STATUS_BRAKING, which is a flag to the locomotor
     to "Cheat" and move directly towards the target.  This ensures that the
     missile will hit. jba. */
  Locomotor* curLoco = getCurLocomotor();
  Object* obj = getObject();
  ...
  if (isIdle()) {
    ...
    if (getGoalObject() != NULL) {
      ...
      Real distanceToTargetSq = ThePartitionManager->getDistanceSquared(
          getObject(), getGoalObject(), FROM_CENTER_3D);
...
UpdateSleepTime MissileAIUpdate::update() {
  ...
  if (m_noTurnDistLeft > 0.0f && m_state >= IGNITION) {
    Real distThisTurn =
        sqrtf(sqr(newPos.x - m_prevPos.x) + sqr(newPos.y - m_prevPos.y) +
              sqr(newPos.z - m_prevPos.z));
...
#if 1
  // srj sez: doh, why was this never in place? I guess 'cuz so few "smart"
  // missiles ever go in a downward-towards-ground attitude. however, comanche
  // rocket pod missiles do, so this fix is needed for them to target bridges
  // correctly.
  ...
  if (projectileIsArmed() && oldLayer != LAYER_GROUND &&
...
void MissileAIUpdate::airborneTargetGone() {
  // I would really love it if this could retarget, but all of the targeting and
  // legality is done by the Weapon that fired me.  The safest thing for me to
  // do in this state is to just run out of gas.
