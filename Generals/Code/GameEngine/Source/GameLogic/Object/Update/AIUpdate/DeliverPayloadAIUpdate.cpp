...
// Desc:   State machine that controls the approach and deployment of airborne
// cargo
...
const FieldParse *DeliverPayloadData::getFieldParse() {
  static const FieldParse dataFieldParse[] = {
      ...
      {"DeliveryDistance", INI::parseReal, NULL,
       offsetof(DeliverPayloadData, m_distToTarget)},
      {"PreOpenDistance", INI::parseReal, NULL,
       offsetof(DeliverPayloadData, m_preOpenDistance)},
      {"MaxAttempts", INI::parseInt, NULL,
       offsetof(DeliverPayloadData, m_maxAttempts)},

      // Drop information
      {"DropDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(DeliverPayloadData, m_dropDelay)},
      {"DropOffset", INI::parseCoord3D, NULL,
       offsetof(DeliverPayloadData, m_dropOffset)},
      {"DropVariance", INI::parseCoord3D, NULL,
       offsetof(DeliverPayloadData, m_dropVariance)},
      {"InheritTransportVelocity", INI::parseBool, NULL,
       offsetof(DeliverPayloadData, m_inheritTransportVelocity)},
      {"ExitPitchRate", INI::parseAngularVelocityReal, NULL,
       offsetof(DeliverPayloadData, m_exitPitchRate)},
      {"ParachuteDirectly", INI::parseBool, NULL,
       offsetof(DeliverPayloadData, m_isParachuteDirectly)},

      // Visible payload information (payload assumed to be show visibly and
      // it's created only when dropped)
      {"VisibleItemsDroppedPerInterval", INI::parseInt, NULL,
       offsetof(DeliverPayloadData, m_visibleItemsDroppedPerInterval)},
      {"VisibleDropBoneBaseName", INI::parseAsciiString, NULL,
       offsetof(DeliverPayloadData, m_visibleDropBoneName)},
      {"VisibleSubObjectBaseName", INI::parseAsciiString, NULL,
       offsetof(DeliverPayloadData, m_visibleSubObjectName)},
      {"VisibleNumBones", INI::parseInt, NULL,
       offsetof(DeliverPayloadData, m_visibleNumBones)},
      {"VisiblePayloadTemplateName", INI::parseAsciiString, NULL,
       offsetof(DeliverPayloadData, m_visiblePayloadTemplateName)},
      {"VisiblePayloadWeaponTemplate", INI::parseWeaponTemplate, NULL,
       offsetof(DeliverPayloadData, m_visiblePayloadWeaponTemplate)},
      {"SelfDestructObject", INI::parseBool, NULL,
       offsetof(DeliverPayloadData, m_selfDestructObject)},

      // Weapon based payload
      {"FireWeapon", INI::parseBool, NULL,
       offsetof(DeliverPayloadData, m_fireWeapon)},

      // Specify an additional weaponslot to be fired while strafing
      {"DiveStartDistance", INI::parseReal, NULL,
       offsetof(DeliverPayloadData, m_diveStartDistance)},
      {"DiveEndDistance", INI::parseReal, NULL,
       offsetof(DeliverPayloadData, m_diveEndDistance)},
      {"StrafingWeaponSlot", INI::parseLookupList,
       TheWeaponSlotTypeNamesLookupList,
       offsetof(DeliverPayloadData, m_strafingWeaponSlot)},
      {"StrafeWeaponFX", INI::parseFXList, NULL,
       offsetof(DeliverPayloadData, m_strafeFX)},
      {"StrafeLength", INI::parseReal, NULL,
       offsetof(DeliverPayloadData, m_strafeLength)},

      {"DeliveryDecal", RadiusDecalTemplate::parseRadiusDecalTemplate, NULL,
       offsetof(DeliverPayloadData, m_deliveryDecalTemplate)},
      {"DeliveryDecalRadius", INI::parseReal, NULL,
       offsetof(DeliverPayloadData, m_deliveryDecalRadius)},
...
//-------------------------------------------------------------------------------------------------
DeliverPayloadAIUpdate::DeliverPayloadAIUpdate(Thing *thing,
...
//-------------------------------------------------------------------------------------------------
AIFreeToExitType DeliverPayloadAIUpdate::getAiFreeToExit(
    const Object *exiter) const {
  if (getObject()->isEffectivelyDead()) return NOT_FREE_TO_EXIT;

  return m_freeToExit ? FREE_TO_EXIT : NOT_FREE_TO_EXIT;
}

//-------------------------------------------------------------------------------------------------
void DeliverPayloadAIUpdate::killDeliveryDecal() { m_deliveryDecal.clear(); }

//-------------------------------------------------------------------------------------------------
Bool DeliverPayloadAIUpdate::isAllowedToRespondToAiCommands(
...
UpdateSleepTime DeliverPayloadAIUpdate::update(void) {
  ...
  if (m_diveState != DIVESTATE_POSTDIVE) {
    if (m_diveState == DIVESTATE_PREDIVE) {
      ...
      Real currentDistanceSquared = ThePartitionManager->getDistanceSquared(
          getObject(), getTargetPos(), FROM_CENTER_2D);
      if (currentDistanceSquared <= startDiveDistanceSquared) {
        m_diveState = DIVESTATE_DIVING;
        getObject()
            ->getAIUpdateInterface()
            ->getCurLocomotor()
            ->setUsePreciseZPos(true);

        AudioEventRTS soundDive =
            *(getObject()->getTemplate()->getPerUnitSound("StartDive"));
      ...
      if (m_data.m_strafingWeaponSlot != -1) {
        ...
        if (velocity->z < 5.0f) {
          ...
          Real diveRatio = (startDiveDistance - currentDistance) /
                           (startDiveDistance - endDiveDistance);
          ...
          strafePoint.z =
              TheTerrainLogic->getGroundHeight(strafePoint.x, strafePoint.y);

          // lock it just till the weapon is empty or the attack is "done"
          getObject()->setWeaponLock(m_data.m_strafingWeaponSlot,
                                     LOCKED_TEMPORARILY);
...
//-------------------------------------------------------------------------------------------------
void DeliverPayloadAIUpdate::deliverPayload(const Coord3D *moveToPos,
  ...
  m_deliveryDecal.clear();
  m_data.m_deliveryDecalTemplate.createRadiusDecal(
      *targetPos, m_data.m_deliveryDecalRadius,
      getObject()->getControllingPlayer(), m_deliveryDecal);
  ...
  // must make the state machine AFTER initing the other stuff, since it may
  // inquire of its values...
  m_deliverPayloadStateMachine =
      newInstance(DeliverPayloadStateMachine)(getObject());
...
//-------------------------------------------------------------------------------------------------
void DeliverPayloadAIUpdate::deliverPayloadViaModuleData(
    const Coord3D *moveToPos) {
  const DeliverPayloadAIUpdateModuleData *data =
      getDeliverPayloadAIUpdateModuleData();
...
//-------------------------------------------------------------------------------------------------
const ThingTemplate *
DeliverPayloadAIUpdate::getPutInContainerTemplateViaModuleData() const {
  AsciiString name =
      getDeliverPayloadAIUpdateModuleData()->m_putInContainerName;
  return name.isEmpty() ? NULL : TheThingFactory->findTemplate(name);
}

//-------------------------------------------------------------------------------------------------
Real DeliverPayloadAIUpdate::calcMinTurnRadius(
  ...
  /// @todo srj -- this should probably use min-speed, not max-speed... fix
  /// after E3
  ...
  */
  Real minTurnRadius =
      (maxTurnRate > 0.0f) ? (maxSpeed / maxTurnRate) : 999999.0f;

  if (timeToTravelThatDist) *timeToTravelThatDist = minTurnRadius / maxSpeed;
...
Bool DeliverPayloadAIUpdate::isCloseEnoughToTarget() {
  // In addition to testing distance, it is also sensitive to being in/outward
  // bound
  ////The new getPreOpenDistance() allows the deliver state to fire early, but
  ///only if inbound, /so the doors can open and payload can get ready...

  Real allowedDistanceSqr = sqr(getAllowedDistanceToTarget());
  Real currentDistanceSqr = ThePartitionManager->getDistanceSquared(
      getObject(), getTargetPos(), FROM_CENTER_2D);
  ...
  if (inBound)
    allowedDistanceSqr =
        sqr(getAllowedDistanceToTarget() + getPreOpenDistance());

  // DEBUG_LOG(("Dist to target is %f (allowed
  // %f)\n",sqrt(currentDistanceSqr),sqrt(allowedDistanceSqr)));

  if (allowedDistanceSqr > currentDistanceSqr) return TRUE;
...
Bool DeliverPayloadAIUpdate::isOffMap() const {
  ...
  if (!mapRegion.isInRegionNoZ(getObject()->getPosition())) return true;
...
//-------------------------------------------------------------------------------------------------
DeliverPayloadStateMachine::DeliverPayloadStateMachine(Object *owner)
    : StateMachine(owner, "DeliverPayloadStateMachine") {
  //	DeliverPayloadAIUpdate *ai =
  //(DeliverPayloadAIUpdate*)owner->getAIUpdateInterface();

  static const StateConditionInfo considerConditions[] = {
      StateConditionInfo(DeliverPayloadStateMachine::isOffMap,
                         RECOVER_FROM_OFF_MAP, NULL),
      StateConditionInfo(NULL, NULL, NULL)  // keep last
  };

  // order matters: first state is the default state.
  defineState(APPROACH, newInstance(ApproachState)(this), DELIVERING,
              CONSIDER_NEW_APPROACH);
  defineState(DELIVERING, newInstance(DeliveringState)(this), HEAD_OFF_MAP,
              CONSIDER_NEW_APPROACH);
  defineState(CONSIDER_NEW_APPROACH,
              newInstance(ConsiderNewApproachState)(this), APPROACH,
              HEAD_OFF_MAP, considerConditions);
  defineState(RECOVER_FROM_OFF_MAP, newInstance(RecoverFromOffMapState)(this),
              APPROACH, APPROACH);
  defineState(HEAD_OFF_MAP, newInstance(HeadOffMapState)(this), CLEAN_UP,
              CLEAN_UP);
  defineState(CLEAN_UP, newInstance(CleanUpState)(this), INVALID_STATE_ID,
              INVALID_STATE_ID);
}
...
//-------------------------------------------------------------------------------------------------
/*static*/ Bool DeliverPayloadStateMachine::isOffMap(State *thisState,
                                                     void *userData) {
...
StateReturnType ApproachState::update() {
  ...
  if (ai->isCloseEnoughToTarget()) return STATE_SUCCESS;

  if (ai->getAIStateType() != AI_MOVE_TO) {
    DEBUG_CRASH(
        ("hmm, bailed from moveto state early... should this be possible?"));
    ai->aiMoveToPosition(ai->getMoveToPos(), CMD_FROM_AI);
  }
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void DeliveringState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void DeliveringState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType DeliveringState::onEnter()  // Open the pod bay doors, Hal
{
  ...
  owner->clearAndSetModelConditionState(MODELCONDITION_DOOR_1_CLOSING,
...
StateReturnType DeliveringState::update()  // Kick a dude out every so often
{
  ...
  if (!ai->isCloseEnoughToTarget()) return STATE_FAILURE;

  const ContainedItemsList *items =
      owner->getContain() ? owner->getContain()->getContainedItemsList() : NULL;
  if ((!items || !items->size()) &&
      ai->getVisibleItemsDelivered() == ai->getData()->m_visibleNumBones) {
    // We are out of payload to drop AND our visible payload is empty. It's
    // possible for deliverers to have one or the other or even both.
  ...
  if (items) {
    ...
    if (ai->isFireWeapon()) {
      ...
      if (ai->getDropVariance().x > 0)
        pos.x += GameLogicRandomValueReal(-ai->getDropVariance().x,
                                          ai->getDropVariance().x);
      if (ai->getDropVariance().y > 0)
        pos.y += GameLogicRandomValueReal(-ai->getDropVariance().y,
                                          ai->getDropVariance().y);
      if (ai->getDropVariance().z > 0)
        pos.z += GameLogicRandomValueReal(-ai->getDropVariance().z,
                                          ai->getDropVariance().z);
      ...
      if (ai->getData()->m_isParachuteDirectly && contain) {
        contain->setOverrideDestination(
            ai->getTargetPos());  // go for the direct hit
      } else if (itemAI) {
        // This will most likely be ignored, as the item is still in the
        // transport, and therefore Held.
        itemAI->aiMoveToPosition(ai->getMoveToPos(), CMD_FROM_AI);
      }

      /// @todo srj -- urg. icky.
      static NameKeyType key_GenerateMinefieldBehavior =
          NAMEKEY("GenerateMinefieldBehavior");
      GenerateMinefieldBehavior *mfb =
          (GenerateMinefieldBehavior *)item->findUpdateModule(
              key_GenerateMinefieldBehavior);
      if (mfb) {
    ...
    while (attemptDrops &&
      ...
      if (draw) {
        ...
        if (ai->getData()->m_visibleSubObjectName.isNotEmpty()) {
          AsciiString name;
          name.format("%s%02d", ai->getData()->m_visibleSubObjectName.str(),
                      ai->getVisibleItemsDelivered() + 1);
        ...
        if (ai->getData()->m_visiblePayloadTemplateName.isNotEmpty()) {
          const ThingTemplate *thing = TheThingFactory->findTemplate(
              ai->getData()->m_visiblePayloadTemplateName.str());
          Object *payload = TheThingFactory->newObject(
              thing, owner->getControllingPlayer()->getDefaultTeam());
          ...
          if (payload) {
            ...
            if (ai->getData()->m_visibleDropBoneName.isNotEmpty()) {
              Coord3D pos;
              AsciiString bone;
              bone.format("%s%02d", ai->getData()->m_visibleDropBoneName.str(),
                          ai->getVisibleItemsDelivered() + 1);
              if (draw->getPristineBonePositions(
                      ai->getData()->m_visibleDropBoneName.str(),
                      ai->getVisibleItemsDelivered() + 1, &pos, NULL, 1) > 0) {
            ...
            // If we want the payload to inherit the transport's velocity, do so
            // now.
            if (ai->getData()->m_inheritTransportVelocity) {
            ...
            for (BehaviorModule **u = payload->getBehaviorModules(); *u; ++u) {
              ProjectileUpdateInterface *pui =
                  (*u)->getProjectileUpdateInterface();
              if (pui) {
                // Missile!
                const WeaponTemplate *weaponTemplate =
                    ai->getData()->m_visiblePayloadWeaponTemplate;
                if (!weaponTemplate) {
                  DEBUG_ASSERTCRASH(
                      0, ("%s tried to fire missile %s via DeliverPayload, and "
                          "is missing required weapon template in "
                          "ObjectCreationList.ini entry.",
                          owner->getTemplate()->getName().str(),
                          payload->getTemplate()->getName().str()));
                  break;
                }
                VeterancyLevel v = owner->getVeterancyLevel();
                pui->projectileFireAtObjectOrPosition(
                    NULL, ai->getTargetPos(), weaponTemplate,
                    weaponTemplate->getProjectileExhaust(v));
            ...
            if (!projectileFired) {
              ...
              if (ai->getData()->m_exitPitchRate != 0) {
                ...
                Real pitchRate =
                    payload->getPhysics()->getCenterOfMassOffset() *
                    ai->getData()->m_exitPitchRate;
                payload->getPhysics()->setPitchRate(pitchRate);
              }

              // If payload can move independently, order the move. A lot of
              // bombs don't have locomotors, in which case the "start move
              // sound" is played, but nothing else actually happens.
              if (payload->getAIUpdateInterface()) {
                payload->getAIUpdateInterface()->aiMoveToPosition(
                    ai->getMoveToPos(), CMD_FROM_AI);
              }
            }
          }
        }
...
void DeliveringState::onExit(StateExitType)  // Close the doors
{
  ...
  DeliverPayloadAIUpdate *ai =
      (DeliverPayloadAIUpdate *)owner->getAIUpdateInterface();
  if (ai) ai->friend_setFreeToExit(false);

  DEBUG_ASSERTCRASH(
      m_didOpen, ("Not enough time for the doors to open to drop anything!"));
  /// @todo srj -- for now, this assumes at most one door
  owner->clearAndSetModelConditionState(MODELCONDITION_DOOR_1_OPENING,
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void ConsiderNewApproachState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void ConsiderNewApproachState::loadPostProcess(void) {}  // end loadPostProcess

//-------------------------------------------------------------------------------------------------
StateReturnType
ConsiderNewApproachState::onEnter()  // Increment local counter o' futility
{
  Object *owner = getMachineOwner();
  DeliverPayloadAIUpdate *ai =
      (DeliverPayloadAIUpdate *)owner->getAIUpdateInterface();
  ...
  // ok, play some games here... the pathfinder is pretty dumb about things with
  // big turning radii, so we have to force the issue here... basically,
  // estimate our necessary turning radius based on loco values, move off far
  // enough so we can turn, then head back. (if we just say "head back
  // directly", the code will just keep turning in circles, not realizing that
  // our turning radius is too large for that ever to work.)
  Real minTurnRadius = ai->calcMinTurnRadius(NULL);

  // how far is "far enough"? we must be at least 2*radius dist away from our
  // target. (we add a little fudge since we may not be able to travel our max
  // speed while turning.)
  ...
  reApproachPoint.z = 0.0f;  // yeah, yeah, should be terrainpos, but doesn't
                             // really matter here...
...
//-------------------------------------------------------------------------------------------------
StateReturnType
ConsiderNewApproachState::update()  // Success if we should try again, failure
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void RecoverFromOffMapState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void RecoverFromOffMapState::loadPostProcess(void) {}  // end loadPostProcess

//-------------------------------------------------------------------------------------------------
StateReturnType
RecoverFromOffMapState::onEnter()  // Increment local counter o' futility
{
  Object *owner = getMachineOwner();
  DeliverPayloadAIUpdate *ai =
      (DeliverPayloadAIUpdate *)owner->getAIUpdateInterface();
  ...
  // a little cheesy... make a delay based on turn-radius time to simulate
  // offscreen maneuvering.
  Real timeToTravelThatDist;
  /*Real minTurnRadius =*/ai->calcMinTurnRadius(&timeToTravelThatDist);
  m_reEntryFrame =
      TheGameLogic->getFrame() + REAL_TO_INT_CEIL(timeToTravelThatDist);
  ...
  if (owner->getDrawable()) owner->getDrawable()->setDrawableHidden(true);
...
//-------------------------------------------------------------------------------------------------
StateReturnType
RecoverFromOffMapState::update()  // Success if we should try again, failure if
                                  // we should give up
{
  if (TheGameLogic->getFrame() < m_reEntryFrame) return STATE_CONTINUE;
  ...
  if (owner->getDrawable()) owner->getDrawable()->setDrawableHidden(false);

  Coord3D enterCoord =
      TheTerrainLogic->findClosestEdgePoint(owner->getPosition());
  if (owner->isAboveTerrain()) enterCoord.z = owner->getPosition()->z;
  owner->setPosition(&enterCoord);

  Real enterAngle = atan2(ai->getMoveToPos()->y - enterCoord.y,
                          ai->getMoveToPos()->x - enterCoord.x);
...
StateReturnType HeadOffMapState::onEnter()  // Give move order out of town
{
  ...
  if (ai->getData()->m_selfDestructObject)  // Lorenzen... this makes delivery
                                            // unit pop away like a bubble...
  {
    TheGameLogic->destroyObject(owner);
    return STATE_CONTINUE;
  }

  // Coord3D exitCoord = TheTerrainLogic->findClosestEdgePoint(
  // owner->getPosition() );
  ...
  Real HUGE_DIST = FUDGE * sqrt(sqr(terrainExtent.hi.x - terrainExtent.lo.x) +
                                sqr(terrainExtent.hi.y - terrainExtent.lo.y));

  exitCoord.x += dir->x * HUGE_DIST;
  exitCoord.y += dir->y * HUGE_DIST;

  ai->getCurLocomotor()->setAllowInvalidPosition(true);
  ai->getCurLocomotor()->setUltraAccurate(
      true);  // set ultra-accurate just so AI won't try to adjust our dest
  ai->aiMoveToPosition(&exitCoord, CMD_FROM_AI);

  // once we get into head-off-map state, we're done... don't respond to
  // anything else. this addresses the bug where a plane delivering team "foo"
  // hasn't made it off the map before all of team "foo" lands, whereupon team
  // "foo" is given a script command to (say) move to a given point... and since
  // the cargo plane might be a member of "foo" it would respond to said
  // command, which sucks and is confusing. might be better to have these guys
  // generally ignore most commands, but this is pretty safe, in that AFAIK
  // nothing with deliverpayload should ever respond to anything, not even IDLE,
...
StateReturnType HeadOffMapState::update() {
  ...
  if (ai->isOffMap()) return STATE_SUCCESS;
...
StateReturnType CleanUpState::onEnter()  // Delete my successful butt
{
  if (getMachineOwner()->getContain()) {
    DEBUG_ASSERTCRASH(getMachineOwner()->getContain()->getContainCount() == 0,
                      ("did not drop all items!"));
  }
