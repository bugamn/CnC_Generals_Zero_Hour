...
static const char *statenames[] = {
    ...
    "ST_REGROUPING",  ///< Wanting failed, so hang out at base until something
                      ///< changes.  Still on autopilot, but resting.
...
//-------------------------------------------------------------------------------------------------
SupplyTruckAIUpdate::SupplyTruckAIUpdate(Thing *thing,
  ...
  m_suppliesDepletedVoice =
      getSupplyTruckAIUpdateModuleData()->m_suppliesDepletedVoice;
}
...
UpdateSleepTime SupplyTruckAIUpdate::update(void) {
  ...
  UpdateSleepTime mine = IS_STATE_SLEEP(stRet)
                             ? UPDATE_SLEEP(GET_STATE_SLEEP_FRAMES(stRet))
                             : UPDATE_SLEEP_NONE;
...
//-------------------------------------------------------------------------------------------------
Bool SupplyTruckAIUpdate::isAvailableForSupplying() const { return true; }
...
Bool SupplyTruckAIUpdate::loseOneBox() {
  ...
  if (draw) {
    draw->updateDrawableSupplyStatus(
        getSupplyTruckAIUpdateModuleData()->m_maxBoxesData, m_numberBoxes);
  }
...
Bool SupplyTruckAIUpdate::gainOneBox(Int remainingStock) {
  if (getSupplyTruckAIUpdateModuleData() &&
      m_numberBoxes >= getSupplyTruckAIUpdateModuleData()->m_maxBoxesData)
    return FALSE;
  ++m_numberBoxes;
...
void SupplyTruckAIUpdate::privateIdle(CommandSourceType cmdSource) {
  ...
  if (cmdSource == CMD_FROM_PLAYER) setForceBusyState(TRUE);

  AIUpdateInterface::privateIdle(cmdSource);
}

//----------------------------------------------------------------------------------------
void SupplyTruckAIUpdate::privateDock(Object *dock,
...
class SupplyTruckBusyState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(SupplyTruckBusyState,
 ...
 public:
  SupplyTruckBusyState(StateMachine *machine)
      : State(machine, "SupplyTruckBusyState") {}
  virtual StateReturnType onEnter() {
    if (getMachineOwner() && getMachineOwner()->getAI()) {
      // Have to check, since constructor sets a state.  Phhbbt. Constructor =
      // set up, init = do first thing.
...
#ifdef DEBUG_SUPPLY_STATE
    TheInGameUI->DEBUG_addFloatingText("entering busy state",
...
#ifdef DEBUG_SUPPLY_STATE
    TheInGameUI->DEBUG_addFloatingText("exiting busy state",
                                       getMachineOwner()->getPosition(),
                                       GameMakeColor(255, 0, 0, 255));
#endif
  }
...
class SupplyTruckIdleState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(SupplyTruckIdleState,
                                          "SupplyTruckIdleState")
 protected:
  // snapshot interface STUBBED.
  virtual void crc(Xfer *xfer) {};
  virtual void xfer(Xfer *xfer) {};
 ...
 public:
  SupplyTruckIdleState(StateMachine *machine)
      : State(machine, "SupplyTruckIdleState") {}
  virtual StateReturnType onEnter();
  virtual StateReturnType update() { return STATE_CONTINUE; }
  virtual void onExit(StateExitType status) {
#ifdef DEBUG_SUPPLY_STATE
    TheInGameUI->DEBUG_addFloatingText("exiting idle state",
                                       getMachineOwner()->getPosition(),
                                       GameMakeColor(255, 0, 0, 255));
#endif
  }
...
#ifdef DEBUG_SUPPLY_STATE
  TheInGameUI->DEBUG_addFloatingText("entering idle state",
                                     getMachineOwner()->getPosition(),
                                     GameMakeColor(255, 0, 0, 255));
#endif
...
SupplyTruckStateMachine::SupplyTruckStateMachine(Object *owner)
    : StateMachine(owner, "SupplyTruckStateMachine") {
  ...
  defineState(ST_BUSY, newInstance(SupplyTruckBusyState)(this), ST_BUSY,
              ST_BUSY, busyConditions);
  defineState(ST_IDLE, newInstance(SupplyTruckIdleState)(this), ST_BUSY,
              ST_BUSY, idleConditions);
  defineState(ST_WANTING,
              newInstance(SupplyTruckWantsToPickUpOrDeliverBoxesState)(this),
              ST_BUSY, ST_REGROUPING, wantingConditions);
  defineState(ST_REGROUPING, newInstance(RegroupingState)(this), ST_WANTING,
              ST_BUSY, regroupingConditions);
  defineState(ST_DOCKING, newInstance(DockingState)(this), ST_BUSY, ST_BUSY,
              dockingConditions);
}
...
#ifdef DEBUG_SUPPLY_STATE
  TheInGameUI->DEBUG_addFloatingText("entering wanting state",
                                     getMachineOwner()->getPosition(),
                                     GameMakeColor(255, 0, 0, 255));
#endif
  return STATE_CONTINUE;
...
#ifdef DEBUG_SUPPLY_STATE
  TheInGameUI->DEBUG_addFloatingText("exiting wanting state",
                                     getMachineOwner()->getPosition(),
                                     GameMakeColor(255, 0, 0, 255));
#endif
}
...
StateReturnType SupplyTruckWantsToPickUpOrDeliverBoxesState::update() {
  ...
  if (!update->isAvailableForSupplying()) {
    DEBUG_CRASH(
        ("SupplyTruckWantsToPickUpOrDeliverBoxesState: "
         "isAvailableForSupplying==false; should not get here"));
  ...
  return STATE_FAILURE;  // we aren't going to wait right here, we will go back
                         // to base and wait for
...
StateReturnType RegroupingState::onEnter() {
#ifdef DEBUG_SUPPLY_STATE
  TheInGameUI->DEBUG_addFloatingText("entering regrouping state",
                                     getMachineOwner()->getPosition(),
                                     GameMakeColor(255, 0, 0, 255));
#endif
  // I have failed to find a dock, so my first choice is to go hang out at a
  // Supply Center (I may have failed to find a Warehouse).  My second choices
  // is to go to a ConYard.  My last choice is just to go to a friendly
  // building.

  Object *owner = getMachineOwner();
  AIUpdateInterface *ownerAI = owner->getAIUpdateInterface();
  Player *ownerPlayer = owner->getControllingPlayer();
  if (!ownerPlayer || !ownerAI) return STATE_FAILURE;

  ownerAI->ignoreObstacle(NULL);
  SupplyTruckAIInterface *update =
      owner->getAIUpdateInterface()->getSupplyTruckAIInterface();
  if (!update) {
    return STATE_FAILURE;
  }
  Int numBoxes = update->getNumberBoxes();
  // If we are forced to regroup, and we have boxes, we want to wait for the
  // player to rebuild a supply center & go to it.
  ...
  kindofnot.clear();
  // can't do best supply center of the player's resource brain, because that
  // adds canTransfer checks.
  destinationObject =
      ownerPlayer->findClosestByKindOf(owner, kindof, kindofnot);
  ...
  if (!ThePartitionManager->findPositionAround(destinationObject->getPosition(),
                                               &fpOptions, &destination))
    return STATE_FAILURE;

  ownerAI->aiMoveToPosition(&destination, CMD_FROM_AI);
  return STATE_CONTINUE;  // Remember to say continue when you change ai command
                          // inside a state
}
...
#ifdef DEBUG_SUPPLY_STATE
  TheInGameUI->DEBUG_addFloatingText("exiting regroup state",
                                     getMachineOwner()->getPosition(),
                                     GameMakeColor(255, 0, 0, 255));
#endif
}
...
StateReturnType DockingState::onEnter() {
#ifdef DEBUG_SUPPLY_STATE
  TheInGameUI->DEBUG_addFloatingText("entering docking state",
                                     getMachineOwner()->getPosition(),
                                     GameMakeColor(255, 0, 0, 255));
#endif
  Object *owner = getMachineOwner();
  ...
  // after we dock the first time, we clear this, and then follow our normal
  // state machine path
...
//-------------------------------------------------------------------------------------------------
StateReturnType DockingState::update() { return STATE_CONTINUE; }

//-------------------------------------------------------------------------------------------------
void DockingState::onExit(StateExitType status) {
#ifdef DEBUG_SUPPLY_STATE
  TheInGameUI->DEBUG_addFloatingText("exiting docking state",
                                     getMachineOwner()->getPosition(),
                                     GameMakeColor(255, 0, 0, 255));
#endif
}
...
//-------------------------------------------------------------------------------------------------
/* static */ Bool SupplyTruckStateMachine::isForcedIntoWantingState(
    State *thisState, void *userData) {
...
#ifdef DEBUG_SUPPLY_STATE
    AsciiString tmp;
    tmp.format("isForcedIntoWantingState returns true (%s)",
               statenames[thisState->getID()]);
    TheInGameUI->DEBUG_addFloatingText(tmp, owner->getPosition(),
                                       GameMakeColor(255, 0, 0, 255));
#endif
    return true;
  }
...
//-------------------------------------------------------------------------------------------------
/* static */ Bool SupplyTruckStateMachine::isForcedIntoBusyState(
    State *thisState, void *userData) {
  Object *owner = thisState->getMachineOwner();
  AIUpdateInterface *ai = owner->getAIUpdateInterface();
  if (!ai) return false;
...
#ifdef DEBUG_SUPPLY_STATE
    AsciiString tmp;
    tmp.format("isForcedIntoBusytate returns true (%s)",
               statenames[thisState->getID()]);
    TheInGameUI->DEBUG_addFloatingText(tmp, owner->getPosition(),
                                       GameMakeColor(255, 0, 0, 255));
#endif
...
//-------------------------------------------------------------------------------------------------
/* static */ Bool SupplyTruckStateMachine::ownerDocking(State *thisState,
                                                        void *userData) {
  Object *owner = thisState->getMachineOwner();
  AIUpdateInterface *ai = owner->getAIUpdateInterface();
  if (!ai) return false;
...
#ifdef DEBUG_SUPPLY_STATE
    AsciiString tmp;
    tmp.format("ownerDocking returns true (%s)",
               statenames[thisState->getID()]);
    TheInGameUI->DEBUG_addFloatingText(tmp, owner->getPosition(),
                                       GameMakeColor(255, 0, 0, 255));
#endif
...
//-------------------------------------------------------------------------------------------------
/* static */ Bool SupplyTruckStateMachine::ownerIdle(State *thisState,
                                                     void *userData) {
  Object *owner = thisState->getMachineOwner();
  AIUpdateInterface *ai = owner->getAIUpdateInterface();
  if (!ai) return false;
...
//-------------------------------------------------------------------------------------------------
/* static */ Bool SupplyTruckStateMachine::ownerAvailableForSupplying(
    State *thisState, void *userData) {
  Object *owner = thisState->getMachineOwner();
  AIUpdateInterface *ai = owner->getAIUpdateInterface();
  if (!ai) return false;
...
#ifdef DEBUG_SUPPLY_STATE
    AsciiString tmp;
    tmp.format("ownerAvailableForSupplying returns true (%s)",
               statenames[thisState->getID()]);
    TheInGameUI->DEBUG_addFloatingText(tmp, owner->getPosition(),
                                       GameMakeColor(255, 0, 0, 255));
#endif
...
//-------------------------------------------------------------------------------------------------
/* static */ Bool SupplyTruckStateMachine::ownerNotDockingOrIdle(
    State *thisState, void *userData) {
  Object *owner = thisState->getMachineOwner();
  AIUpdateInterface *ai = owner->getAIUpdateInterface();
  if (!ai) return false;
...
#ifdef DEBUG_SUPPLY_STATE
    AsciiString tmp;
    tmp.format("ownerNotDockingOrIdle returns true (%s)",
               statenames[thisState->getID()]);
    TheInGameUI->DEBUG_addFloatingText(tmp, owner->getPosition(),
                                       GameMakeColor(255, 0, 0, 255));
#endif
