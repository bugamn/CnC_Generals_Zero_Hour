...
// ////////////////////////////////////////////////////////////// Author: Mark
// Lorenzen, Sept 2002 Desc: The railroad track following railroad
// carriage/locomotive logic module
///////////////////////////////////////////////////////////////////////////////////////////////////
...
// disabled for debugging purposes") #pragma message("NO COMMENTS, PLEASE ABOUT
// THIS PRAGMA BEING LEFT ON... IT IS FOR A SPECIAL TEST...") #pragma
// message("IF QA EVER REPRODUCES A DESYNC (BELIEVED FIXED) I'LL NEED TO DEBUG
// WHILE PLAYING AGAINST") #pragma message("THAT MACHINE WITHOUT QUITTING TO
// START A DEBUGGABLE VERSION...") #pragma message("THIS WILL BE CORRECTED IN
// GOOD TIME. JUST TRUST ME, -ML")
#endif

#define FRAMES_UNPULLED_LONG_ENOUGH_TO_UNHITCH (2)

void TrainTrack::incReference() { ++m_refCount; }

Bool TrainTrack::releaseReference() { return (--m_refCount == 0); }
...
RailroadBehavior::RailroadBehavior(Thing *thing, const ModuleData *moduleData)
  ...
  m_isLeadCarraige =
      m_isLocomotive;  // for now, I am the lead, only if I am the locomotive
...
RailroadBehavior::~RailroadBehavior(void) {
  TheAudio->removeAudioEvent(
      m_runningSound.getPlayingHandle());  // no more chugchug when I'm dead

  if (m_track != NULL) {
    if (m_track->releaseReference()) delete m_track;
...
Bool RailroadBehavior::isRailroad() const {
  if (!m_track) return FALSE;  // I haven't even started yet!

  if (m_waitingInWings) return FALSE;
  if (m_endOfLine) return FALSE;
  if (m_isLeadCarraige) return TRUE;
  if (m_trailerID == INVALID_ID) return TRUE;

  // Explanation: I return that I am a railroad only if I am at one of the ends,
...
// ------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void RailroadBehavior::onCollide(Object *other, const Coord3D *loc,
                                 const Coord3D *normal) {
  Object *obj = getObject();
  if (!obj) return;     // sanity
  if (!other) return;   // sanity
  if (!loc) return;     // sanity
  if (!normal) return;  // sanity

  if (m_waitingInWings) return;
  if (m_endOfLine) return;
  ...
  for (BehaviorModule **m = other->getBehaviorModules(); *m; ++m) {
    ...
    if (collide->isRailroad()) {
      if (m_isLocomotive)  // yikes! I just rear ended a stranded train! Oh
                           // well...
      {
        other->kill();
      } else if (m_isLeadCarraige)  // yikes! I just coasted into some other
                                    // carriage
      {
        other->kill();
  ...
  if (other->isKindOf(KINDOF_STRUCTURE))  // now be careful here, we ignore
                                          // buildings, except for hostile,
                                          // nasty ones that can blow us up
  {
    ...
    DemoTrapUpdate *dtu =
        (DemoTrapUpdate *)other->findUpdateModule(key_DemoTrapUpdate);
    if (dtu) {
  ...
  const RailroadBehaviorModuleData *modData = getRailroadBehaviorModuleData();
  if (m_conductorState == WAIT_AT_STATION &&
      (m_pullInfo.speed <
       modData->m_runningGarrisonSpeedMax))  // they can grab on safely
  {
  ...
  if (other->getContainedBy() ==
      getObject())  // I dont mash my own sweet passengers
    return;
  ...
  Real overlap = ((usRadius + themRadius) - dist) +
                 1;  // the plus 1 makes them go just outside of me.
  dlt.normalize();
  if (!victimIsInfantry) {
    overlap /= 4;
  }
  ...
  AIUpdateInterface *ai = other->getAI();
  if (ai)
    ai->aiIdle(CMD_FROM_AI);  // this eliminates yadda by telling them to stop
                              // driving into me

  //---------------------------

  if (m_conductorState == WAIT_AT_STATION ||
      (m_conductorState == COAST &&
       m_pullInfo.speed < modData->m_runningGarrisonSpeedMax)) {
  ...
  if (other->isEffectivelyDead()) {  // we just run over debris, instead of
                                     // shoving it around
    delta.scale(MIN(0.3f, m_pullInfo.speed * 0.66f));
  } else {
    delta.scale(MIN(1.4f, m_pullInfo.speed *
                              0.66f));  // the faster I go, the harder I slam!
  ...
  heft.z = MAX(heft.z, TheTerrainLogic->getGroundHeight(heft.x, heft.y) +
                           2);  // lift them off the ground
  other->setPosition(&heft);

  delta.z = GameLogicRandomValueReal(
      0.05f, m_pullInfo.speed / 10);  // for some fake heft
  delta.scale(dot);
  ...
  if (dot > 0) theirPhys->setYawRate(deviationCOG * -0.06f * m_pullInfo.speed);
...
void RailroadBehavior::playImpactSound(Object *victim,
  ...
  if (!hasBounceSound) {
    ...
    } else if (victim->isKindOf(KINDOF_HUGE_VEHICLE) ||
  ...
  if (impact.getEventName().isEmpty()) return;
  ...
  // This sound really should only be played at the position where the collision
  // occurs. Therefore, set the player index of the sound to be the player index
...
void RailroadBehavior::loadTrackData(void) {
  if (m_track != NULL) return;  // lets do this only once!
  ...
  // m_anchorWaypointID COULD HAVE BEEN RECORDED IN XFER, WHICH MEANS I LOADED
  // MY TRACK DATA IN A PRIOR LIFE, SO LETS JUST RE_INIT THE TRACK BASED ON THAT
  // POINT, AUTOMAGICALLY
  ...
  DEBUG_ASSERTCRASH(
      anchorWaypoint,
      ("Railroad... couldn't find a waypoint close enough to be the anchor.\n "
       "You should put your train engine right on a waypoint,\n and make sure "
       "the path forms a valid track."));
  if (!anchorWaypoint) return;

  m_track = NEW(TrainTrack);  // this constructor inc's the refcount to 1

  // From now until the next carriage is added, this track is writable using
  // getWritablePointList(); This method will return NULL when refcount is 2 or
  // more getPointList returns the list as const to any caller each carriage
  // must increment the reference when this pointer is passed to it, any
  // subsequent carriage (destructor) will delete this memory here if it
  // releases refcount to zero
  ...
  while (scanner) {
    trackPoint
        .clear();  // clean up the local workspace to make debugging more fun
    ...
    if (scanner->getNumLinks()) {
      ...
      if (anotherWaypoint != NULL) {
        ...
        trackPoint.m_isTunnelOrBridge =
            anotherWaypoint->getName().endsWith("Tunnel");
...
UpdateSleepTime RailroadBehavior::update(void) {
  ...
  if (m_isLocomotive) {
    if (m_conductorState == APPLY_BRAKES) {
      ...
      if (m_waitAtStationTimer <= 0) {
        ...
        m_runningSound.setPlayingHandle(
            TheAudio->addAudioEvent(&m_runningSound));
      } else if (m_waitAtStationTimer == (modData->m_waitAtStationTime / 4)) {
        m_whistleSound.setPlayingHandle(
            TheAudio->addAudioEvent(&m_whistleSound));
      }

    } else if (m_conductorState == ACCELERATE) {
      conductorPullInfo.speed +=
          0.02f * conductorPullInfo.m_direction;  // push start multiplier
  ...
  if (m_wantsToBeLeadCarraige >
      FRAMES_UNPULLED_LONG_ENOUGH_TO_UNHITCH)  // if this flag survived until
                                               // now, I have lost my puller
  {
  ...
  if (m_isLeadCarraige) {
    ...
    // only normalize track position for a looping track, otherwise, let train
    // exit by exceeding tracklength
    while ((conductorPullInfo.trackDistance > m_track->m_length) &&
           m_track->m_isLooping)
    ...
    if (trailer) {
      ...
      RailroadBehavior *RGUpdate =
          (RailroadBehavior *)trailer->findUpdateModule(key_RGUpdate);
      if (RGUpdate) {
        RGUpdate->getPulled(&m_pullInfo);
      }
    } else {
      m_trailerID = INVALID_ID;  // so I will forget about my trailer and
                                 // designate myself as the caboose

      if (m_endOfLine) TheGameLogic->destroyObject(getObject());
    }
  } else if (m_wantsToBeLeadCarraige <=
             FRAMES_UNPULLED_LONG_ENOUGH_TO_UNHITCH)  // if I am not the lead
                                                      // carriage
  {
    m_wantsToBeLeadCarraige++;  // like every young carriage, I aspire to be the
                                // lead carriage some day unless getpulled() set
                                // this false, I will be on the next update!
                                // Joy!
  }
  ...
  if (draw) {
    ...
    if (drawPos->z <
        TheTerrainLogic->getGroundHeight(drawPos->x, drawPos->y) - 3.0f)
...
class PartitionFilterIsValidCarriage : public PartitionFilter {
 ...
 public:
  PartitionFilterIsValidCarriage(Object *obj,
                                 const RailroadBehaviorModuleData *data)
      : m_obj(obj), m_data(data) {}

#if defined(_DEBUG) || defined(_INTERNAL)
  virtual const char *debugGetName() {
    return "PartitionFilterIsValidCarriage";
  }
#endif

  virtual Bool allow(Object *objOther) {
    // must exist!
    if (m_obj == NULL || objOther == NULL) return FALSE;

    // must not be me!
    if (m_obj == objOther) return FALSE;
    // must have railroady goodness
    static NameKeyType key_rb = NAMEKEY("RailroadBehavior");
    RailroadBehavior *rb =
        (RailroadBehavior *)objOther->findUpdateModule(key_rb);
    if (!rb) return FALSE;
    ...
    if (m_obj->getRelationship(objOther) != ALLIES) return FALSE;
...
void RailroadBehavior::createCarriages(void) {
  if (!m_isLocomotive) {
    DEBUG_ASSERTCRASH(m_isLocomotive,
                      ("a not locomotive attempted to create carriages"));
    return;
  }
  ...
  // First we'll see if the map artist put some cars down on the track for us to
  // find
  ...
  hitchOffset.scale(
      -maxRadius);  // negative, since I want the back, not the front
  ...
  if (xferCarriage != NULL)
    ...
    closeCarriage = ThePartitionManager->getClosestObject(
        &myHitchLoc, maxRadius, FROM_CENTER_2D, filters);
  ...
  if (iter != list.end()) {
    ...
    if (temp) {
      ...
      if (firstCarriage) {
        ...
        static NameKeyType key_rb = NAMEKEY("RailroadBehavior");
        RailroadBehavior *rb =
            (RailroadBehavior *)firstCarriage->findUpdateModule(key_rb);
        if (rb) {
          if (closeCarriage)
            rb->hitchNewCarriagebyProximity(self->getID(), m_track);
          else
            rb->hitchNewCarriagebyTemplate(self->getID(), list, ++iter,
                                           m_track);  // ! increment iter here!
        } else {
          DEBUG_ASSERTCRASH(
              rb,
              ("%s is attempting to hitch carriage, %s without a "
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void RailroadBehavior::hitchNewCarriagebyTemplate(
    ObjectID locoID, const TemplateNameVector &list, TemplateNameIterator &iter,
    TrainTrack *track) {
  if (m_isLocomotive) {
    DEBUG_ASSERTCRASH(m_isLocomotive,
                      ("You can not hitch a locomotive in mid train, dude."));
    return;
  }
  ...
  m_track->incReference();  // ALWAYS INCREFERENCE WHENEVER YOU INHERIT THIS
                            // POINTER
  ...
  if (iter != list.end())  // this test is bogus
  {
    ...
    if (temp) {
      newCarriage = TheThingFactory->newObject(
          temp, locomotive->getTeam());  // just a little worried about this...
      newCarriage->setProducer(locomotive);
      m_trailerID = newCarriage->getID();

      static NameKeyType key_rb = NAMEKEY("RailroadBehavior");
      RailroadBehavior *rb =
          (RailroadBehavior *)newCarriage->findUpdateModule(key_rb);
      if (rb) {
        rb->hitchNewCarriagebyTemplate(locoID, list, ++iter, m_track);
      } else {
        DEBUG_ASSERTCRASH(rb,
                          ("%s could not hitch a %s without a "
                           "RailroadBehavior... \nwhat kind of nutty conductor "
                           "are you? \nThe next carriage would have been a %s.",
                           locomotive->getTemplate()->getName().str(),
                           newCarriage->getTemplate()->getName().str(), *iter));
      }
    }
  }
}
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void RailroadBehavior::hitchNewCarriagebyProximity(ObjectID locoID,
                                                   TrainTrack *track) {
  if (m_isLocomotive) {
    DEBUG_ASSERTCRASH(m_isLocomotive,
                      ("You can not hitch a locomotive in mid train, dude."));
  ...
  if (closeCarriage) {
    ...
    static NameKeyType key_rb = NAMEKEY("RailroadBehavior");
    RailroadBehavior *rb =
        (RailroadBehavior *)closeCarriage->findUpdateModule(key_rb);
    if (rb)
      rb->hitchNewCarriagebyProximity(self->getID(), m_track);
    else {
      DEBUG_ASSERTCRASH(
          rb, ("%s is attempting to hitch carriage, %s without a "
               "RailroadBehavior... \nwhat kind of nutty conductor are you? ",
               self->getTemplate()->getName().str(),
...
void RailroadBehavior::getPulled(PullInfo *info) {
  // ENFORCE MY STATUS AS A PULLEE, NOT A PULLER, and update my position, speed
  // etc.
  ...
  conductorPullInfo =
      *info;  // copy my puller to my conductor, so that if I ever get detached
  //( meaning that if this function stops getting called ), I will take over as
  //the lead carriage! Wheeeee!!
  ...
  if (trailer) {
    ...
    if (RGUpdate) {
      RGUpdate->getPulled(
          &m_pullInfo);  //< this is MY info, not the one passed to me
    }
  } else {
    m_trailerID = INVALID_ID;  // so I will forget about my trailer and
                               // designate myself as the caboose
...
void alignToTerrain(Real angle, const Coord3D &pos, const Coord3D &normal,
                    Matrix3D &mtx) {
  Coord3D x, y, z;
  ...
  DEBUG_ASSERTCRASH(fabs(x.x * z.x + x.y * z.y + x.z * z.z) < 0.0001,
                    ("dot is not zero\n"));
...
void RailroadBehavior::updatePositionTrackDistance(PullInfo *pullerInfo,
  ...
  myInfo->speed =
      pullerInfo->speed;  // YES, if I am getting pulled, I must obey puller
  myInfo->m_direction =
      pullerInfo
          ->m_direction;  // YES, if I am getting pulled, I must obey puller

  //	pullerInfo->previousWaypoint = pullerInfo->currentWaypoint;// to feed
  //edge test in update
  ...
  Coord3D carPosition;
  FindPosByPathDistance(&carPosition, myInfo->trackDistance, m_track->m_length,
                        TRUE);
  ...
  Real relAngle =
      stdAngleDiff(desiredAngle, obj->getTransformMatrix()->Get_Z_Rotation());
  ...
  Coord3D normal;
  Real enforceElevation =
      TheTerrainLogic->getGroundHeight(turnPos.x, turnPos.y, &normal);

  obj->setTransformMatrix(&mtx);

  if (!m_inTunnel) obj->setPositionZ(enforceElevation);
...
void RailroadBehavior::destroyTheWholeTrainNow(void) {
  ...
  if (trailer) {
    ...
    RailroadBehavior *RGUpdate =
        (RailroadBehavior *)trailer->findUpdateModule(key_RGUpdate);
    if (RGUpdate) {
      RGUpdate->destroyTheWholeTrainNow();  //< this is MY info, not the one
                                            //passed to me
    }
  }
}
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void RailroadBehavior::FindPosByPathDistance(Coord3D *pos, const Real dist,
                                             const Real length, Bool setState) {
  if (!m_track) return;
  ...
  while (pointIter != pointList->end()) {
    ...
    if (thisPoint && thisPoint->m_distanceFromFirst <
                         actualDistance)  // I am after this point, and
    {
      ...
      if (nextPoint && nextPoint->m_distanceFromFirst > actualDistance) {
        ...
        if (setState) {
          ...
          {
            // THE LOCOMOTIVE SNIFFS THE TRACK TO SMELL FOR THE NEXT STATION AND
            // TUNNEL
            if (edge) {
              ...
              if (thisPoint->m_isStation) {
                ...
                m_disembark = TRUE;
                TheAudio->removeAudioEvent(m_runningSound.getPlayingHandle());
              } else if (thisPoint->m_isPingPong &&
                         conductorPullInfo.m_mostRecentSpecialPointHandle !=
                             handleFound) {
          ...
          if (edge && !m_inTunnel) {  // play my clickety clack sound, `cause I
                                      // just rode over a join IN the tracks
            TheAudio->addAudioEvent(&m_clicketyClackSound);
            m_clicketyClackSound.setPosition(getObject()->getPosition());
            m_clicketyClackSound.setVolume((Real)conductorPullInfo.speed /
                                           10.0f);  // assumed max speed
          }
        }
  ...
  // DEBUG_ASSERTCRASH(FALSE,("Railroad could not find a position on the
  // path!"));
}
...
void RailroadBehavior::xfer(Xfer *xfer) {
  ...
  if (version >= 2) {
    ...
    // Bool m_carriagesCreated; ///< TRUE once we have made all the cars in the
    // train
    xfer->xferBool(&m_carriagesCreated);

    // Bool m_hasEverBeenHitched; /// has somebody ever hitched me? Remains
    // true, even after puller dies.
    xfer->xferBool(&m_hasEverBeenHitched);

    // Bool m_waitingInWings; /// I have not entered the real track yet, so
    // leave me alone
    xfer->xferBool(&m_waitingInWings);

    // Bool m_endOfLine;				/// I have reached the
    // end of a non looping track
    ...
    // Bool m_disembark; ///< If I wait at a station, I should also evacuate
    // everybody when I get theres
    xfer->xferBool(&m_disembark);

    // Bool m_inTunnel; ///< Am I in a tunnel, so I wil not snap to ground
    // height, until the next waypoint,
...
void RailroadBehavior::loadPostProcess(void) {
  ...
  // Bool m_trackDataLoaded; ///< have I TRIED to load track data, yet? I only
  // try once!
  m_trackDataLoaded = FALSE;
