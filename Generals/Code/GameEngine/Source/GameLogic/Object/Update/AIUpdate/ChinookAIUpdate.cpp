...
class ChinookEvacuateState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(ChinookEvacuateState,
                                          "ChinookEvacuateState")
 protected:
 ...
 public:
  ChinookEvacuateState(StateMachine* machine)
      : State(machine, "ChinookEvacuateState") {}
...
class ChinookHeadOffMapState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(ChinookHeadOffMapState,
                                          "ChinookHeadOffMapState")
  // I'm outta here
 protected:
  // snapshot interface	STUBBED - no member vars to save. jba.
  virtual void crc(Xfer* xfer) {};
 ...
 public:
  ChinookHeadOffMapState(StateMachine* machine)
      : State(machine, "ChinookHeadOffMapState") {}
...
class ChinookTakeoffOrLandingState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(ChinookTakeoffOrLandingState,
                                          "ChinookTakeoffOrLandingState")
 private:
  Coord3D m_destLoc;
  Bool m_landing;
 ...
 public:
  ...
  virtual StateReturnType onEnter() {
    ...
    PathfindLayerEnum layerAtDest =
        TheTerrainLogic->getHighestLayerForDestination(&m_destLoc,
                                                       onlyHealthyBridges);
    m_destLoc.z =
        TheTerrainLogic->getLayerHeight(m_destLoc.x, m_destLoc.y, layerAtDest);
    if (m_landing) {
      Coord3D tmp;
      FindPositionOptions options;
      options.maxRadius =
          obj->getGeometryInfo().getBoundingCircleRadius() * 100.0f;
      if (ThePartitionManager->findPositionAround(&m_destLoc, &options, &tmp)) {
        m_destLoc = tmp;
        TheAI->pathfinder()->adjustToLandingDestination(obj, &m_destLoc);
      }
      // recalc, since it may have changed. note that findPositionAround() will
      // ALWAYS return a position on the ground proper, so if our initial search
      // start pos was above a bridge, this will put us below the bridge, which
      // would be unfortunate. so recheck to be sure. (note that the
      // partitionmgr is 2d-only, so if it sez that the position on the ground
      // at that xy is clear, it will be clear for both the ground proper and
      // the bridge itself.) also note: don't call
      // objectInteractsWithBridgeLayer(), since it assumes that things that
      // aren't close in z shouldn't interact.
      tmp = m_destLoc;
      tmp.z = obj->getPosition()->z;
      layerAtDest = TheTerrainLogic->getHighestLayerForDestination(
          &tmp, onlyHealthyBridges);
      m_destLoc.z = TheTerrainLogic->getLayerHeight(m_destLoc.x, m_destLoc.y,
                                                    layerAtDest);
  ...
  virtual StateReturnType update() {
    Object* obj = getMachineOwner();
    if (obj->isEffectivelyDead()) return STATE_FAILURE;
  ...
  virtual void onExit(StateExitType status) {
    ...
    if (loco) {
      ...
      if (!obj->isEffectivelyDead()) loco->setMaxLift(BIGNUM);
    }
...
class ChinookCombatDropState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(ChinookCombatDropState,
 ...
 private:
  ...
  void removeDoneRappellers() {
    for (std::vector<RopeInfo>::iterator it = m_ropes.begin();
         it != m_ropes.end(); ++it) {
      for (std::list<ObjectID>::iterator oit = it->rappellerIDs.begin();
           oit != it->rappellerIDs.end();) {
        Object* rappeller = TheGameLogic->findObjectByID(*oit);
        if (rappeller == NULL || rappeller->isEffectivelyDead() ||
            !rappeller->isAboveTerrain()) {
  ...
  static void initRopeParms(Drawable* rope, Real length, Real width,
                            const RGBColor& color, Real wobbleLen,
                            Real wobbleAmp, Real wobbleRate) {
    RopeDrawInterface* tdi = NULL;
    for (DrawModule** d = rope->getDrawModules(); *d; ++d) {
      if ((tdi = (*d)->getRopeDrawInterface()) != NULL) {
        tdi->initRopeParms(length, width, color, wobbleLen, wobbleAmp,
                           wobbleRate);
      }
    }
  }
  ...
  static void setRopeSpeed(Drawable* rope, Real curSpeed, Real maxSpeed,
                           Real accel) {
    RopeDrawInterface* tdi = NULL;
    for (DrawModule** d = rope->getDrawModules(); *d; ++d) {
      if ((tdi = (*d)->getRopeDrawInterface()) != NULL) {
    ...
    if (version >= 2) {
      ...
      for (Int i = 0; i < numRopes; ++i) {
        ...
        if (xfer->getXferMode() == XFER_SAVE) {
          ...
          info.ropeID = info.ropeDrawable ? info.ropeDrawable->getID()
                                          : INVALID_DRAWABLE_ID;
        }
 ...
 public:
  ChinookCombatDropState(StateMachine* machine)
      : State(machine, "ChinookCombatDropState") {}

  // --------------
  virtual StateReturnType onEnter() {
    Object* obj = getMachineOwner();
    Drawable* draw = obj->getDrawable();
    if (draw == NULL) return STATE_FAILURE;
    ...
    const ThingTemplate* ropeTmpl =
        TheThingFactory->findTemplate(d->m_ropeName);

    const Int MAX_BONES = 32;
    Coord3D ropePos[MAX_BONES];
    Matrix3D dropMtx[MAX_BONES];

    Int ropeCount = draw->getPristineBonePositions("RopeStart", 1, ropePos,
                                                   NULL, MAX_BONES);
    Int dropCount =
        draw->getPristineBonePositions("RopeEnd", 1, NULL, dropMtx, MAX_BONES);

    Int numRopes = d->m_numRopes;
    if (numRopes > ropeCount) numRopes = ropeCount;
    if (numRopes > dropCount) numRopes = dropCount;
    if (numRopes <= 0) return STATE_FAILURE;

    m_ropes.clear();
    for (Int i = 0; i < numRopes; ++i) {
      RopeInfo info;

      obj->convertBonePosToWorldPos(NULL, &dropMtx[i], NULL,
                                    &info.dropStartMtx);

      info.ropeDrawable =
          ropeTmpl ? TheThingFactory->newDrawable(ropeTmpl) : NULL;
      if (info.ropeDrawable) {
        ...
        const Bool onlyHealthyBridges = true;  // ignore dead bridges.
        PathfindLayerEnum layerAtDest =
            TheTerrainLogic->getHighestLayerForDestination(&ropePos[i],
                                                           onlyHealthyBridges);

        info.ropeLenMax = ropePos[i].z -
                          TheTerrainLogic->getLayerHeight(
                              ropePos[i].x, ropePos[i].y, layerAtDest) -
                          d->m_ropeFinalHeight;

        initRopeParms(info.ropeDrawable, info.ropeLenMax, d->m_ropeWidth,
                      d->m_ropeColor, d->m_ropeWobbleLen, d->m_ropeWobbleAmp,
                      d->m_ropeWobbleRate);
      }

      info.nextDropTime =
          now +
          GameLogicRandomValue(d->m_perRopeDelayMin, d->m_perRopeDelayMax) -
          d->m_perRopeDelayMin;
  ...
  virtual StateReturnType update() {
    ...
    // ok, now check each rope: if it's empty, or we're at the next drop time,
    // spawn a new rappeller
    Int numRopesInUse = 0;
    for (std::vector<RopeInfo>::iterator it = m_ropes.begin();
         it != m_ropes.end(); ++it) {
        ...
        if (rappeller != NULL) {
          ...
          ExitDoorType exitDoor = exitInterface
                                      ? exitInterface->reserveDoorForExit(
                                            rappeller->getTemplate(), rappeller)
                                      : DOOR_NONE_AVAILABLE;
          ...
          AIUpdateInterface* rappellerAI =
              rappeller ? rappeller->getAIUpdateInterface() : NULL;
          if (rappellerAI) {
            rappellerAI->setDesiredSpeed(d->m_rappelSpeed);
            rappellerAI->aiRappelInto(getMachineGoalObject(),
                                      *getMachineGoalPosition(), CMD_FROM_AI);
          }

          it->rappellerIDs.push_back(rappeller->getID());

          it->nextDropTime = now + GameLogicRandomValue(d->m_perRopeDelayMin,
                                                        d->m_perRopeDelayMax);
        }
      }
    ...
    for (Int i = 0; i < m_ropes.size(); ++i) {
      if (m_ropes[i].ropeDrawable) {
        ...
        const Real initialSpeed =
            TheGlobalData->m_gravity * 30;  // give it a little kick
        setRopeSpeed(m_ropes[i].ropeDrawable, initialSpeed, d->m_ropeDropSpeed,
                     TheGlobalData->m_gravity);
        m_ropes[i].ropeDrawable->setExpirationDate(now + ROPE_EXPIRATION_TIME);
        m_ropes[i].ropeDrawable =
            NULL;  // we're done with it, so null it so we won't save it
      }
    }
...
class ChinookMoveToBldgState : public AIMoveToState {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(ChinookMoveToBldgState,
                                          "ChinookMoveToBldgState")
 private:
 ...
 public:
  ...
  virtual StateReturnType onEnter() {
    ...
    if (bldg != NULL && !bldg->isEffectivelyDead() &&
      ...
      m_newPreferredHeight =
          bldg->getGeometryInfo().getMaxHeightAbovePosition() +
          d->m_minDropHeight;
    ...
    m_destZ = TheTerrainLogic->getGroundHeight(destPos->x, destPos->y) +
              m_newPreferredHeight;
  ...
  virtual StateReturnType update() {
    ...
    if (status != STATE_CONTINUE &&
        fabs(obj->getPosition()->z - m_destZ) > THRESH)
...
class ChinookAIStateMachine : public AIStateMachine {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(ChinookAIStateMachine,
                                          "ChinookAIStateMachine");
...
//-------------------------------------------------------------------------------------------------
ChinookAIStateMachine::ChinookAIStateMachine(Object* owner, AsciiString name)
    : AIStateMachine(owner, name) {
  defineState(TAKING_OFF,
              newInstance(ChinookTakeoffOrLandingState)(this, false), AI_IDLE,
              AI_IDLE);
  defineState(LANDING, newInstance(ChinookTakeoffOrLandingState)(this, true),
              AI_IDLE, AI_IDLE);
  defineState(MOVE_TO_COMBAT_DROP, newInstance(ChinookMoveToBldgState)(this),
              DO_COMBAT_DROP, AI_IDLE);
  defineState(DO_COMBAT_DROP, newInstance(ChinookCombatDropState)(this),
              AI_IDLE, AI_IDLE);

  defineState(MOVE_TO_AND_LAND, newInstance(AIMoveToState)(this), LANDING,
              AI_IDLE);

  defineState(MOVE_TO_AND_EVAC, newInstance(AIMoveToState)(this), LAND_AND_EVAC,
              AI_IDLE);
  defineState(LAND_AND_EVAC,
              newInstance(ChinookTakeoffOrLandingState)(this, true),
              EVAC_AND_TAKEOFF, AI_IDLE);
  defineState(EVAC_AND_TAKEOFF, newInstance(ChinookEvacuateState)(this),
              TAKING_OFF, AI_IDLE);

  defineState(MOVE_TO_AND_EVAC_AND_EXIT, newInstance(AIMoveToState)(this),
              LAND_AND_EVAC_AND_EXIT, AI_IDLE);
  defineState(LAND_AND_EVAC_AND_EXIT,
              newInstance(ChinookTakeoffOrLandingState)(this, true),
              EVAC_AND_EXIT, AI_IDLE);
  defineState(EVAC_AND_EXIT, newInstance(ChinookEvacuateState)(this),
              TAKEOFF_AND_EXIT, AI_IDLE);
  defineState(TAKEOFF_AND_EXIT,
              newInstance(ChinookTakeoffOrLandingState)(this, false),
              HEAD_OFF_MAP, AI_IDLE);
  defineState(HEAD_OFF_MAP, newInstance(ChinookHeadOffMapState)(this), AI_IDLE,
              AI_IDLE);
}
...
ChinookAIUpdateModuleData::ChinookAIUpdateModuleData() {
  ...
  m_rappelSpeed =
      fabs(TheGlobalData->m_gravity) * LOGICFRAMES_PER_SECOND * 0.5f;
...
//-------------------------------------------------------------------------------------------------
/*static*/ void ChinookAIUpdateModuleData::buildFieldParse(
    MultiIniFieldParse& p) {
  SupplyTruckAIUpdateModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"RappelSpeed", INI::parseVelocityReal, 0,
       offsetof(ChinookAIUpdateModuleData, m_rappelSpeed)},
      {"RopeDropSpeed", INI::parseVelocityReal, 0,
       offsetof(ChinookAIUpdateModuleData, m_ropeDropSpeed)},
      {"RopeName", INI::parseAsciiString, 0,
       offsetof(ChinookAIUpdateModuleData, m_ropeName)},
      {"RopeFinalHeight", INI::parseReal, 0,
       offsetof(ChinookAIUpdateModuleData, m_ropeFinalHeight)},
      {"RopeWidth", INI::parseReal, 0,
       offsetof(ChinookAIUpdateModuleData, m_ropeWidth)},
      {"RopeWobbleLen", INI::parseReal, 0,
       offsetof(ChinookAIUpdateModuleData, m_ropeWobbleLen)},
      {"RopeWobbleAmplitude", INI::parseReal, 0,
       offsetof(ChinookAIUpdateModuleData, m_ropeWobbleAmp)},
      {"RopeWobbleRate", INI::parseAngularVelocityReal, 0,
       offsetof(ChinookAIUpdateModuleData, m_ropeWobbleRate)},
      {"RopeColor", INI::parseRGBColor, 0,
       offsetof(ChinookAIUpdateModuleData, m_ropeColor)},
      {"NumRopes", INI::parseUnsignedInt, 0,
       offsetof(ChinookAIUpdateModuleData, m_numRopes)},
      {"PerRopeDelayMin", INI::parseDurationUnsignedInt, 0,
       offsetof(ChinookAIUpdateModuleData, m_perRopeDelayMin)},
      {"PerRopeDelayMax", INI::parseDurationUnsignedInt, 0,
       offsetof(ChinookAIUpdateModuleData, m_perRopeDelayMax)},
      {"MinDropHeight", INI::parseReal, 0,
       offsetof(ChinookAIUpdateModuleData, m_minDropHeight)},
      {"WaitForRopesToDrop", INI::parseBool, 0,
       offsetof(ChinookAIUpdateModuleData, m_waitForRopesToDrop)},
      {0, 0, 0, 0}};
...
AIStateMachine* ChinookAIUpdate::makeStateMachine() {
  return newInstance(ChinookAIStateMachine)(getObject(),
                                            "ChinookAIStateMachine");
}

//-------------------------------------------------------------------------------------------------
ChinookAIUpdate::ChinookAIUpdate(Thing* thing, const ModuleData* moduleData)
    : SupplyTruckAIUpdate(thing, moduleData) {
  m_hasPendingCommand = false;
  m_flightStatus =
      CHINOOK_FLYING;  // yep, that's right, even if we start "on ground"
...
Bool ChinookAIUpdate::isIdle() const {
  ...
  // to be "idle"...
  if (m_hasPendingCommand) return false;
  ...
  if (result && m_flightStatus == CHINOOK_LANDED) {
    ...
    if (contain && contain->hasObjectsWantingToEnterOrExit()) result = false;
  }
...
Bool ChinookAIUpdate::isAvailableForSupplying() const {
  if (!SupplyTruckAIUpdate::isAvailableForSupplying()) return false;
...
Bool ChinookAIUpdate::isAllowedToAdjustDestination() const {
  if (m_flightStatus == CHINOOK_LANDED) return false;
...
ObjectID ChinookAIUpdate::getBuildingToNotPathAround() const {
  if (getAIStateType() == MOVE_TO_COMBAT_DROP ||
      getAIStateType() == DO_COMBAT_DROP) {
    const Object* goalObj = getStateMachine()->getGoalObject();
    if (goalObj) return goalObj->getID();
  }
...
AIFreeToExitType ChinookAIUpdate::getAiFreeToExit(const Object* exiter) const {
  if (m_flightStatus == CHINOOK_LANDED ||
      (m_flightStatus == CHINOOK_DOING_COMBAT_DROP &&
       exiter->isKindOf(KINDOF_CAN_RAPPEL)))
    return FREE_TO_EXIT;
...
Bool ChinookAIUpdate::chooseLocomotorSet(LocomotorSetType wst) {
  if (m_flightStatus == CHINOOK_LANDED) wst = LOCOMOTORSET_TAXIING;
...
UpdateSleepTime ChinookAIUpdate::update() {
  ...
  if (pp != NULL) {
    if (m_flightStatus == CHINOOK_LANDED && !m_hasPendingCommand &&
        getObject()->getBodyModule()->getHealth() ==
...
//-------------------------------------------------------------------------------------------------
void ChinookAIUpdate::setMyState(StateID cmd, Object* target,
                                 const Coord3D* pos,
 ...
 */
void ChinookAIUpdate::privateGetRepaired(Object* repairDepot,
                                         CommandSourceType cmdSource) {
  ...
  FindPositionOptions options;
  options.maxRadius =
      repairDepot->getGeometryInfo().getBoundingCircleRadius() * 100.0f;
  if (ThePartitionManager->findPositionAround(&pos, &options, &tmp)) pos = tmp;

  setMyState(MOVE_TO_AND_LAND, NULL, &pos, cmdSource);
}

//-------------------------------------------------------------------------------------------------
void ChinookAIUpdate::privateCombatDrop(Object* target, const Coord3D& pos,
                                        CommandSourceType cmdSource) {
  //
  // when there is a target present, we must verify that we can logically do the
  // action when we get commands from players (we'll assume AI knows what its
  // doing)
  //
  if (target != NULL && cmdSource == CMD_FROM_PLAYER &&
      TheActionManager->canEnterObject(getObject(), target, cmdSource,
                                       COMBATDROP_INTO) == FALSE)
    return;
  ...
  if (target == NULL) {
    ...
    FindPositionOptions options;
    options.maxRadius =
        getObject()->getGeometryInfo().getBoundingCircleRadius() * 100.0f;
...
void ChinookAIUpdate::aiDoCommand(const AICommandParms* parms) {
  ...
  if (m_flightStatus == CHINOOK_TAKING_OFF ||
      ...
      m_flightStatus == CHINOOK_DOING_COMBAT_DROP) {
    // have to wait for takeoff or landing (or rappel) to complete, just store
    // the sucker.
  ...
  switch (parms->m_cmd) {
    ...
    case AICMD_MOVE_TO_POSITION_AND_EVACUATE_AND_EXIT: {
      ...
      if (calcDistSqr(*getObject()->getPosition(), parms->m_pos) > THRESH_SQR &&
        ...
        setMyState((parms->m_cmd == AICMD_MOVE_TO_POSITION_AND_EVACUATE)
                       ? MOVE_TO_AND_EVAC
                       : MOVE_TO_AND_EVAC_AND_EXIT,
