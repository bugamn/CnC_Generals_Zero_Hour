...
// Desc:   State machine that allows assault transports (troop crawler) to
// deploy
//         troops, order them to attack, then return. Can do extra things like
//         ordering injured troops to return to the transport for healing
//         purposes.
...
//-------------------------------------------------------------------------------------------------
AssaultTransportAIUpdate::AssaultTransportAIUpdate(Thing *thing,
                                                   const ModuleData *moduleData)
    : AIUpdateInterface(thing, moduleData) {
  m_currentMembers = MAX_TRANSPORT_SLOTS;  // First time, max it out, to ensure
                                           // clearing arrays in reset.
  reset();
}
...
void AssaultTransportAIUpdate::aiDoCommand(const AICommandParms *parms) {
  ...
  if (parms->m_cmdSource != CMD_FROM_AI) {
    // Now the only time we care about anything is if we were ordered to attack
    // something or attack move.
    switch (parms->m_cmd) {
      ...
      case AICMD_ATTACK_OBJECT:
        // Reset because we have been ordered to do something.
        reset();
        // m_designatedTarget = parms->m_obj ? parms->m_obj->getID() :
        // INVALID_ID;
      ...
      case AICMD_IDLE:
        ...
        break;
      default:
        // Reset because we have been ordered to do something we're not
        // handling.
        reset();
        break;
    }
  ...
  // Note, in both cases, the transport will fire a dummy DEPLOY weapon that
  // will trigger the evacuation of the troops.
  AIUpdateInterface::aiDoCommand(parms);
}

//-------------------------------------------------------------------------------------------------
void AssaultTransportAIUpdate::beginAssault(
    const Object *designatedTarget) const {
  // The transport has determined it is in range to begin the assault (via
  // weapon system). Now order the evacuation of healthy troops, and let the
  // update handle moving them.
  if (designatedTarget) {
...
//-------------------------------------------------------------------------------------------------
UpdateSleepTime calcSleepTime() { return UPDATE_SLEEP_NONE; }

//-------------------------------------------------------------------------------------------------
UpdateSleepTime AssaultTransportAIUpdate::update(void) {
  Object *transport = getObject();
  // const AssaultTransportAIUpdateModuleData *data =
  // getAssaultTransportAIUpdateModuleData();
  ...
  // First removing dead members or members that have been ordered to do
  // something outside of this AI.
  if (m_currentMembers) {
    for (int i = 0; i < m_currentMembers; i++) {
      ...
      if (!member || member->isEffectivelyDead() ||
        ...
        if (ai) {
          // Important! Members of our assault transport must be allowed to
          // chase down designated enemies. Generally only player commands allow
          // this, so this flag allows AI commands to do the same. We need to
          // turn this off though, because this ex-member is no longer under
          // transport control.
  ...
  if (contain) {
    ...
    while (passengerIterator != passengerList->end()) {
      ...
      // It's possible to add members manually -- but if we already have 10
      // members, then wait!
      if (m_currentMembers < MAX_TRANSPORT_SLOTS) {
        ...
        if (passenger->getAI()) {
          // Important! Members of our assault transport must be allowed to
          // chase down designated enemies. Generally only player commands allow
          // this, so this flag allows AI commands to do the same.
          passenger->getAI()->setAllowedToChase(TRUE);
        }

        // Check if the passenger is wounded below threshhold (if so make sure
        // we heal him before ordering him to fight!)
  ...
  // If we're already in the process, reacquire the designated target again...
  ...
  if (designatedTarget) {
    ...
    for (int i = 0; i < m_currentMembers; i++) {
      ...
      if (member && ai) {
        ...
        if (contained && isMemberHealthy(member) && !m_newMember[i]) {
          // This contained member is healthy so order him to exit to start
          // fighting! New members are exempt!
          ...
          if (wounded) {
            ...
            if (!ai->isMoving()) {
              if (ai->getGoalObject() != designatedTarget) {
                // Okay, this dude is outside and waiting... order him to attack
                // the designated target
                ai->aiAttackObject(designatedTarget, NO_MAX_SHOTS_LIMIT,
                                   CMD_FROM_AI);
              }
            }
          }
    ...
    if (m_isAttackMove && getAIStateType() != AI_ATTACK_MOVE_TO) {
      ...
      aiAttackMoveToPosition(&m_attackMoveGoalPos, NO_MAX_SHOTS_LIMIT,
                             CMD_FROM_AI);
  ...
  if( !m_framesRemaining )
  {
          if( !isMoving() && fightingMembers && designatedTarget )
          {
                  ...
                  //Get centriod pos now that we know the number of fighting
  members. Real scale = 1.0f / (Real)fightingMembers; fighterCentroidPos.scale(
  scale );

                  Coord3D designatedTargetPos =
  *designatedTarget->getPosition();

                  //Calculate a vector from the target passed the fighters to be
  at a safe place
                  ...
                  Real distanceSqrd = ThePartitionManager->getDistanceSquared(
  transport, &transportGoalPos, FROM_CENTER_2D ); if( distanceSqrd > 40.0f
  * 40.0f )
                  {
...
Bool AssaultTransportAIUpdate::isAttackPointless() const {
  // If all members are new members (thus can't attack), and the transport
  // itself is still attacking, stop!
...
void AssaultTransportAIUpdate::giveFinalOrders() {
  // All members have been ejected outside already -- transfer the original
  // order to the troops
  for (int i = 0; i < m_currentMembers; i++) {
    Object *member = TheGameLogic->findObjectByID(m_memberIDs[i]);
    AIUpdateInterface *ai = member ? member->getAI() : NULL;
    if (member && ai) {
      Object *designatedTarget =
          TheGameLogic->findObjectByID(m_designatedTarget);

      if (m_isAttackObject && designatedTarget) {
        ai->aiAttackObject(designatedTarget, NO_MAX_SHOTS_LIMIT,
                           CMD_FROM_PLAYER);
      } else if (m_isAttackMove) {
        ai->aiAttackMoveToPosition(&m_attackMoveGoalPos, NO_MAX_SHOTS_LIMIT,
                                   CMD_FROM_PLAYER);
      }
