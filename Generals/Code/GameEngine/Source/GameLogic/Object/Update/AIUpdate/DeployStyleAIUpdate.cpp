...
// Desc:   A standard ai update that also handles units that must deploy to
// attack and pack before moving.
...
//-------------------------------------------------------------------------------------------------
DeployStyleAIUpdate::DeployStyleAIUpdate(Thing *thing,
...
Bool DeployStyleAIUpdate::isIdle() const {
  // we need to do this because we enter an idle state during deploy states in
  // these cases, but scripting relies on us never claiming to be "idle"...
  if (m_hasOutsideCommand) return false;
...
void DeployStyleAIUpdate::aiDoCommand(const AICommandParms *parms) {
  ...
  if (parms->m_cmdSource != CMD_FROM_AI) {
    ...
    if (m_state != DEPLOY && m_state != UNDEPLOY) {
      // Only issue the command if we're not in the process of
...
UpdateSleepTime DeployStyleAIUpdate::update(void) {
  ...
  Bool remainDeployed = m_isGuardingPosition && !designatedTarget &&
                        !isMoving() && !isWaitingForPath();
  ...
  switch (m_state) {
    ...
    case READY_TO_ATTACK:
      if (!remainDeployed &&
          (!inRange && isAttacking ||
           !isAttacking && (isWaitingForPath() || getPath()))) {
        WhichTurretType tur = getWhichTurretForCurWeapon();
        if (tur != TURRET_INVALID) {
        ...
        setMyState(UNDEPLOY);
      } else if (!designatedTarget && m_overriddenAttack &&
                 m_hasOutsideCommand) {
        // For multiple attacks we specifically order the unit to attack one
        // object. After object is dead, we revert to our original command.
  ...
  switch (m_state) {
    ...
    case DEPLOY:
    case UNDEPLOY:
      mine = m_frameToWakeForDeploy > now
                 ? UPDATE_SLEEP(m_frameToWakeForDeploy - now)
                 : UPDATE_SLEEP_NONE;
      aiIdle(CMD_FROM_AI);
      break;
...
void DeployStyleAIUpdate::setMyState(DeployStateTypes stateID) {
  ...
  switch (stateID) {
    case DEPLOY: {
      ...
      m_frameToWakeForDeploy +=
          TheGameLogic->getFrame();  // convert to absolute frame
