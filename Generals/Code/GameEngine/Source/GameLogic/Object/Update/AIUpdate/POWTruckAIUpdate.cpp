...
// ///////////////////////////////////////////////////////////////////// Author:
// Colin Day Desc:   AI for the POW Truck
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void POWTruckAIUpdateModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"BoredTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(POWTruckAIUpdateModuleData, m_boredTimeInFrames)},
      {"AtPrisonDistance", INI::parseReal, NULL,
       offsetof(POWTruckAIUpdateModuleData, m_hangAroundPrisonDistance)},
      {0, 0, 0, 0}};
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
POWTruckAIUpdate::~POWTruckAIUpdate(void) {}  // end ~POWTruckAIUpdate

//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void POWTruckAIUpdate::onDelete(void) {}  // end onDelete
...
void POWTruckAIUpdate::aiDoCommand(const AICommandParms *parms) {
  ...
  // if the command is from a player, stop whatever we might be doing and accept
  // the new command
...
UpdateSleepTime POWTruckAIUpdate::update(void) {
  ...
  if (getCurLocomotor()) getCurLocomotor()->setUltraAccurate(TRUE);
...
void POWTruckAIUpdate::setTask(POWTruckTask task, Object *taskObject) {
  ...
  // sanity, POW_TRUCK_TASK_COLLECTING_TARGET and
  // POW_TRUCK_TASK_RETURNING_PRISONERS require taskObject parameters
  if ((task == POW_TRUCK_TASK_COLLECTING_TARGET ||
       task == POW_TRUCK_TASK_RETURNING_PRISONERS) &&
      taskObject == NULL) {
  ...
  // to be clean, when leaving the POW_TRUCK_TASK_RETURNING_PRISONERS state,
  // clear out the prison ID we were watching
  if (oldTask == POW_TRUCK_TASK_RETURNING_PRISONERS) m_prisonID = INVALID_ID;
  ...
  if (task == POW_TRUCK_TASK_COLLECTING_TARGET) {
    ...
    DEBUG_ASSERTCRASH(taskObject->getAIUpdateInterface(),
                      ("POWTruckAIUpdate::setTask - '%s' has no ai module\n",
...
// ------------------------------------------------------------------------------------------------
/** Set this unit into automatic or manual AI mode, when automatic the AI will
 * look for prisoners, collect them, and bring them back automatically.  When in
 * manual, the player must direct the actions of the unit explicitly */
// ------------------------------------------------------------------------------------------------
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void POWTruckAIUpdate::privatePickUpPrisoner(Object *prisoner,
                                             CommandSourceType cmdSource) {
  // if 'prisoner' is not a valid target for picking up don't bother going
  // forward
  if (validateTarget(prisoner) == FALSE) return;

  //
  // we will now try to pick up this target ... if the command was from a player
  // then we are no longer in automatic mode and will not return to automatic
  // mode until the player issues the command for us to return back to prison
  //
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void POWTruckAIUpdate::privateReturnPrisoners(Object *prison,
                                              CommandSourceType cmdSource) {
  // when receiving this command from the player, we will always enter AUTOMATIC
  // ai mode
  if (cmdSource == CMD_FROM_PLAYER) setAIMode(AUTOMATIC);

  // if no prison is provided, find one if possible
  if (prison == NULL) prison = findBestPrison();
...
void POWTruckAIUpdate::updateWaiting(void) {
  ...
  //
  // if we're not idle we don't consider ourselves waiting ... we could be
  // moving from a players command, or just about anything ... so just keep
  // updating the m_enteredWaitingFrame to the current frame until we actually
  // become idle and start to wait
  //
  if (ai->isIdle() == FALSE) m_enteredWaitingFrame = TheGameLogic->getFrame();

  // get module data
  const POWTruckAIUpdateModuleData *modData = getPOWTruckAIUpdateModuleData();

  // if enough time has passed we're bored of waiting, try to find another
  // target
  if (TheGameLogic->getFrame() - m_enteredWaitingFrame >
      modData->m_boredTimeInFrames)
...
void POWTruckAIUpdate::updateFindTarget(void) {
  ...
  DEBUG_ASSERTCRASH(m_aiMode != MANUAL,
                    ("POWTruckAIUpdate::updateFindTarget - We shouldn't be "
                     "here with a manual ai mode\n"));
  if (m_aiMode == MANUAL) return;

  // don't do this too often
  if (TheGameLogic->getFrame() - m_lastFindFrame < FIND_DELAY) return;
  ...
  Object *us = getObject();
  AIUpdateInterface *ai = us->getAIUpdateInterface();
  DEBUG_ASSERTCRASH(ai,
                    ("POWTruckAIUpdate::updateFindTarget - '%s' has no ai\n",
                     us->getTemplate()->getName().str()));
...
void POWTruckAIUpdate::updateCollectingTarget(void) {
  ...
  // we should never become idle while collecting a target, if we do, find a
  // target
  Object *us = getObject();
  AIUpdateInterface *ai = us->getAIUpdateInterface();
  if (ai->isIdle()) {
  ...
  /*
          //
          // when we're close to the target, we tell the target to start walking
     toward
          // us ... if they're not idle we don't do this as they are already
     moving
          //
          Object *us = getObject();
          ...
          if( targetAI->isIdle() )
          {
                  ...
                  Real distSq = pow(
     us->getGeometryInfo().getBoundingSphereRadius() * 2.0f, 2 ); if(
     ThePartitionManager->getDistanceSquared( us, target, FROM_CENTER_2D ) <=
     distSq )
                  {
...
static void putContainedInPrison(Object *obj, void *userData) {
  ...
  // sanity
  DEBUG_ASSERTCRASH(returnData != NULL && returnData->source != NULL &&
                        returnData->dest != NULL,
...
void POWTruckAIUpdate::updateReturnPrisoners(void) {
  ...
  AIUpdateInterface *ai = us->getAIUpdateInterface();
  DEBUG_ASSERTCRASH(
      ai, ("POWTruckAIUpdate::updateReturnPrisoners - '%s' has no AI\n",
           us->getTemplate()->getName().str()));
  ...
  // we should not go idle, we should collide with the prison to return the
  // prisoners
  if (ai->isIdle()) doReturnPrisoners();
...
Bool POWTruckAIUpdate::validateTarget(const Object *target) {
  return TheActionManager->canPickUpPrisoner(getObject(), target,
...
void POWTruckAIUpdate::doReturnPrisoners(void) {
  ...
  AIUpdateInterface *ai = us->getAIUpdateInterface();
  DEBUG_ASSERTCRASH(ai,
                    ("POWTruckAIUpdate::doReturnPrisoners - '%s' has no AI\n",
                     us->getTemplate()->getName().str()));
...
void POWTruckAIUpdate::doReturnToPrison(Object *prison) {
  //
  // the great thing about this logic is that there isn't a state to "return to
  // prison", we just go into waiting, find a prison if we can, and tell the AI
  // to move there
  //
  setTask(POW_TRUCK_TASK_WAITING);
  ...
  Real distSq =
      ThePartitionManager->getDistanceSquared(us, prison, FROM_CENTER_2D);
  if (distSq <=
      modData->m_hangAroundPrisonDistance * modData->m_hangAroundPrisonDistance)
    return;
...
Object *POWTruckAIUpdate::findBestPrison(void) {
  ...
  if (prisonID == INVALID_ID) return NULL;
...
Object *POWTruckAIUpdate::findBestTarget(void) {
  ...
  for (other = TheGameLogic->getFirstObject(); other;
       other = other->getNextObject()) {
    //
    // run this target through our validation scheme and don't pick it in the
    // first place if it's not a valid target for us
    //
    if (validateTarget(other) == FALSE) continue;

    // ignore targets that we cannot pathfind to as we will never be able to
    // pick them up
    if (isQuickPathAvailable(other->getPosition()) == FALSE) continue;

    // is this target closer than the one we've found so far
    Real distSq =
        ThePartitionManager->getDistanceSquared(us, other, FROM_CENTER_2D);
    if (closestTarget == NULL || distSq < closestTargetDistSq) {
      ...
      if (TheAI->pathfinder()->quickDoesPathExist(
              ai->getLocomotorSet(), us->getPosition(), other->getPosition()) ==
          TRUE) {
...
// ------------------------------------------------------------------------------------------------
/** We are chosing to pass a structure through the iterate function to unload
 * the prisoners because it's more flexible in that, perhaps someday in the
 * future, we could fail to add an object to the prison (maybe it's full or
 * something), and in that case it's better to manually do actions each time a
 * successful add happens rather than figure it out from before counts and after
 * counts of the truck */
// ------------------------------------------------------------------------------------------------
...
static void putPrisonersInPrison(Object *obj, void *userData) {
  ...
  DEBUG_ASSERTCRASH(obj->getContainedBy() != NULL,
                    ("putPrisonersInPrison: Prisoner '%s' is not contained by "
                     "anything, it should be contained by a POW truck\n",
                     obj->getTemplate()->getName().str()));

  // extra super sanity, just so that we don't crash ... this is in the assert
  // above
  if (obj->getContainedBy() == NULL) return;
  ...
  prisonUnloadData->bounty +=
      TheGlobalData->m_prisonBountyMultiplier *
...
void POWTruckAIUpdate::unloadPrisonersToPrison(Object *prison) {
  ...
  DEBUG_ASSERTCRASH(
      prison->getContain(),
      ("POWTruckAIUpdate::unloadPrisonersToPrison - '%s' has no contain\n",
       prison->getTemplate()->getName().str()));
  DEBUG_ASSERTCRASH(
      prison->getContain()->asOpenContain(),
  ...
  truckContain->iterateContained(putPrisonersInPrison, &prisonUnloadData,
                                 FALSE);
  ...
  if (prison->isKindOf(KINDOF_COLLECTS_PRISON_BOUNTY) &&
    ...
    if (player) {
      ...
      if (money) {
        ...
        moneyString.format(TheGameText->fetch("GUI:AddCash"),
                           prisonUnloadData.bounty);
...
void POWTruckAIUpdate::loadPrisoner(Object *prisoner) {
  ...
  //
  // removed the surrendered status of these prisoners to false, that way if the
  // truck is killed, the prison blows up releasing all the prisoners, whatever,
  // they will behave as normal
  //
  AIUpdateInterface *prisonerAI = prisoner->getAIUpdateInterface();
  if (prisonerAI) prisonerAI->setSurrendered(NULL, FALSE);

  // done adding prisoner, for automatic AI find another target, for manual just
  // wait
  if (m_aiMode == AUTOMATIC)
    setTask(POW_TRUCK_TASK_FIND_TARGET);
  else
