...
// ////////////////////////////////////////////////////////////// Author: Colin
// Day, August 2002 Desc: Railed Transport AI
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void RailedTransportAIUpdateModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"PathPrefixName", INI::parseAsciiString, NULL,
       offsetof(RailedTransportAIUpdateModuleData, m_pathPrefixName)},
      {0, 0, 0, 0}};
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
RailedTransportAIUpdate::RailedTransportAIUpdate(Thing *thing,
...
void RailedTransportAIUpdate::loadWaypointData(void) {
  const RailedTransportAIUpdateModuleData *modData =
      getRailedTransportAIUpdateModuleData();
...
UpdateSleepTime RailedTransportAIUpdate::update(void) {
  ...
  if (m_waypointDataLoaded == FALSE) loadWaypointData();
  ...
  if (currentLocomotor) currentLocomotor->setUltraAccurate(TRUE);

  //
  // if we have no current path selected pick one and move to the end waypoint
  // of that path.  this will set us up in an initial position at the end of the
  // closest path so that stuff can be loaded into us
  //
  if (m_currentPath == INVALID_PATH && m_numPaths > 0)
    pickAndMoveToInitialLocation();

  //
  // if we're in transit, see if we're close enough to the destination waypoint
  // to be considered as "there" and open up the dock
  //
  if (m_inTransit) {
    // sanity
    ...
    Waypoint *waypoint =
        TheTerrainLogic->getWaypointByID(m_path[m_currentPath].endWaypointID);

    // sanity
    DEBUG_ASSERTCRASH(waypoint,
                      ("RailedTransportAIUpdate: Invalid target waypoint\n"));
...
void RailedTransportAIUpdate::aiDoCommand(const AICommandParms *parms) {
  ...
  if (isAllowedToRespondToAiCommands(parms) == FALSE) return;

  // we ignore all commands from the player except the one to start a transit
  // and to unload
...
void RailedTransportAIUpdate::setInTransit(Bool inTransit) {
  ...
  // open up the dock
  if (dui) dui->setDockOpen(!inTransit);
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void RailedTransportAIUpdate::privateExecuteRailedTransport(
    CommandSourceType cmdSource) {
  Object *us = getObject();

  //
  // find us our railed dock interface, note that we MUST go through the modules
  // here because if we just call the method
  // getReailedTransportDockUpdateInterface, it will execute the method for
  // *THIS AI UPDATE MODULE* which of course is not our dock update
  //
  RailedTransportDockUpdateInterface *rtdui = NULL;
  for (BehaviorModule **u = us->getBehaviorModules(); *u; ++u)
    if ((rtdui = (*u)->getRailedTransportDockUpdateInterface()) != NULL) break;

  // if we've in the process of loading or unloading anything we can't do a
  // transport sequence
  if (rtdui == NULL || rtdui->isLoadingOrUnloading()) return;

  // pick the next path
  if (++m_currentPath >= m_numPaths) m_currentPath = 0;

  // find the start waypoint for our current path
  Waypoint *startWaypoint =
      TheTerrainLogic->getWaypointByID(m_path[m_currentPath].startWaypointID);
  DEBUG_ASSERTCRASH(startWaypoint,
                    ("RailedTransportAIUpdate: Start waypoint not found\n"));
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void RailedTransportAIUpdate::privateEvacuate(Int exposeStealthUnits,
                                              CommandSourceType cmdSource) {
  Object *us = getObject();
...
void RailedTransportAIUpdate::xfer(Xfer *xfer) {
  ...
  if (m_numPaths > MAX_WAYPOINT_PATHS) {
    DEBUG_CRASH(
        ("m_numPaths %d exceeds limit %d.", m_numPaths, MAX_WAYPOINT_PATHS));
