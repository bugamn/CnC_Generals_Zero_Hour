...
// Needs to check legality of evacuate, and may move to a place that is better
// to evacuate at Author: Graham Smallwood, July 2002
 ...
 * Attack given object
 */
void TransportAIUpdate::privateAttackObject(Object *victim, Int maxShotsToFire,
                                            CommandSourceType cmdSource) {
  ContainModuleInterface *contain = getObject()->getContain();
  if (contain != NULL && contain->isPassengerAllowedToFire()) {
    // As an extension of the normal attack, I may want to tell my passengers to
    // attack too, but only if this is a direct command.  (As opposed to a
    // passive aquire)
    if (cmdSource == CMD_FROM_PLAYER || cmdSource == CMD_FROM_SCRIPT) {
      const ContainedItemsList *passengerList =
      ...
      while (passengerIterator != passengerList->end()) {
        ...
        // If I am an overlord with a gattling upgrade, I do not tell it to fire
        // if it is disabled
 ...
 * Attack given object
 */
void TransportAIUpdate::privateForceAttackObject(Object *victim,
                                                 Int maxShotsToFire,
                                                 CommandSourceType cmdSource) {
  ContainModuleInterface *contain = getObject()->getContain();
  if (contain != NULL && contain->isPassengerAllowedToFire()) {
  ...
  AIUpdateInterface::privateForceAttackObject(victim, maxShotsToFire,
                                              cmdSource);
}
 ...
 */
void TransportAIUpdate::privateAttackPosition(const Coord3D *pos,
                                              Int maxShotsToFire,
                                              CommandSourceType cmdSource) {
  ContainModuleInterface *contain = getObject()->getContain();
  if (contain != NULL && contain->isPassengerAllowedToFire()) {
    ...
    if (cmdSource == CMD_FROM_PLAYER || cmdSource == CMD_FROM_SCRIPT) {
      ...
      while (passengerIterator != passengerList->end()) {
        ...
        // If I am an overlord with a gattling upgrade, I do not tell it ti fire
        // if it is disabled
        if (passenger->isKindOf(KINDOF_PORTABLE_STRUCTURE)) {
          if (passenger->isDisabledByType(DISABLED_HACKED) ||
              passenger->isDisabledByType(DISABLED_EMP) ||
...
//-------------------------------------------------------------------------------------------------
AIFreeToExitType TransportAIUpdate::getAiFreeToExit(
