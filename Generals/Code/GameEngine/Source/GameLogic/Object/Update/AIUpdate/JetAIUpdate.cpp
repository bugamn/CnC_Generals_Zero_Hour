...
//-------------------------------------------------------------------------------------------------
enum TaxiType { FROM_HANGAR, FROM_PARKING, TO_PARKING };
...
static Bool isOutOfSpecialReloadAmmo(Object* jet) {
  ...
  for (Int i = 0; i < WEAPONSLOT_COUNT; i++) {
    ...
    if (weapon == NULL || weapon->getReloadType() != RETURN_TO_BASE_TO_RELOAD)
      continue;
    ++specials;
    if (weapon->getStatus() == OUT_OF_AMMO) ++out;
  }
...
static ParkingPlaceBehaviorInterface* getPP(ObjectID id,
                                            Object** airfieldPP = NULL) {
  if (airfieldPP) *airfieldPP = NULL;

  Object* airfield = TheGameLogic->findObjectByID(id);
  if (airfield == NULL || airfield->isEffectivelyDead() ||
      !airfield->isKindOf(KINDOF_AIRFIELD) ||
      airfield->testStatus(OBJECT_STATUS_SOLD))
    return NULL;

  if (airfieldPP) *airfieldPP = airfield;

  ParkingPlaceBehaviorInterface* pp = NULL;
  for (BehaviorModule** i = airfield->getBehaviorModules(); *i; ++i) {
    if ((pp = (*i)->getParkingPlaceBehaviorInterface()) != NULL) break;
  }
...
class PartitionFilterHasParkingPlace : public PartitionFilter {
 ...
 protected:
  ...
  virtual const char* debugGetName() {
    return "PartitionFilterHasParkingPlace";
  }
#endif
  virtual Bool allow(Object* objOther) {
    ParkingPlaceBehaviorInterface* pp = getPP(objOther->getID());
    if (pp != NULL && pp->reserveSpace(m_id, 0.0f, NULL)) return true;
    return false;
  }
};
...
static Object* findSuitableAirfield(Object* jet) {
  PartitionFilterAcceptByKindOf filterKind(MAKE_KINDOF_MASK(KINDOF_AIRFIELD),
                                           KINDOFMASK_NONE);
  PartitionFilterRejectByObjectStatus filterStatus(
      OBJECT_STATUS_UNDER_CONSTRUCTION, 0);
  PartitionFilterRejectByObjectStatus filterStatusTwo(
      OBJECT_STATUS_SOLD, 0);  // Independent to make it an OR
  PartitionFilterRelationship filterTeam(
      jet, PartitionFilterRelationship::ALLOW_ALLIES);
  ...
  return ThePartitionManager->getClosestObject(jet, HUGE_DIST, FROM_CENTER_2D,
                                               filters);
}
...
class JetAwaitingRunwayState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(JetAwaitingRunwayState,
 ...
 public:
  JetAwaitingRunwayState(StateMachine* machine, Bool landing)
      : m_landing(landing), State(machine, "JetAwaitingRunwayState") {}

  virtual StateReturnType onEnter() {
    Object* jet = getMachineOwner();
    JetAIUpdate* jetAI = (JetAIUpdate*)jet->getAIUpdateInterface();
    if (!jetAI) return STATE_FAILURE;
  ...
  virtual StateReturnType update() {
    ...
    if (!pp->reserveSpace(jet->getID(), jetAI->friend_getParkingOffset(),
                          NULL)) {
      DEBUG_ASSERTCRASH(
          m_landing, ("hmm, this should never happen for taking-off things"));
...
class JetOrHeliCirclingDeadAirfieldState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(JetOrHeliCirclingDeadAirfieldState,
                                          "JetOrHeliCirclingDeadAirfieldState")
 protected:
  // snapshot interface	 STUBBED.
  // The state will check immediately after a load game, but I think that's ok.
  // jba.
  virtual void crc(Xfer* xfer) {};
  virtual void xfer(Xfer* xfer) {
    XferVersion cv = 1;
 ...
 public:
  ...
  virtual StateReturnType onEnter() {
    ...
    // obscure case: if the jet wasn't spawned, but just placed directly on the
    // map, it might not have an owning airfield, and it might be trying to
    // return simply due to being idle, not out of ammo. so check and don't die
    // in that case, but just punt back out to idle.
    ...
    // Play the "low fuel" voice whenever the craft is circling above the
    // airfield.
    AudioEventRTS soundToPlay =
        *jet->getTemplate()->getPerUnitSound("VoiceLowFuel");
...
class JetOrHeliReturningToDeadAirfieldState : public AIInternalMoveToState {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(
      ...
      "JetOrHeliReturningToDeadAirfieldState")
 public:
  JetOrHeliReturningToDeadAirfieldState(StateMachine* machine)
...
// Subject 1.03
static Bool intersectInfiniteLine2D(Real ax, Real ay, Real ao, Real cx, Real cy,
                                    Real co, Real& ix, Real& iy) {
  Real bx = ax + Cos(ao);
  Real by = ay + Sin(ao);
  Real dx = cx + Cos(co);
  Real dy = cy + Sin(co);
...
class JetOrHeliTaxiState : public AIMoveOutOfTheWayState {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(JetOrHeliTaxiState,
                                          "JetOrHeliTaxiState")
 private:
  TaxiType m_taxiMode;

 public:
  JetOrHeliTaxiState(StateMachine* machine, TaxiType m)
      : m_taxiMode(m), AIMoveOutOfTheWayState(machine) {}
  ...
  virtual StateReturnType onEnter() {
    ...
    if (pp == NULL) return STATE_SUCCESS;  // no airfield? just skip this step.
    ...
    Coord3D intermedPt;
    Bool intermed = false;
    Real orient = atan2(ppinfo.runwayPrep.y - ppinfo.parkingSpace.y,
                        ppinfo.runwayPrep.x - ppinfo.parkingSpace.x);
    if (fabs(stdAngleDiff(orient, ppinfo.parkingOrientation)) > PI / 128) {
      ...
      intermed = intersectInfiniteLine2D(
          ppinfo.parkingSpace.x, ppinfo.parkingSpace.y,
          ppinfo.parkingOrientation, ppinfo.runwayPrep.x, ppinfo.runwayPrep.y,
          ppinfo.parkingOrientation + PI / 2, intermedPt.x, intermedPt.y);
    }
    ...
    if (m_taxiMode == TO_PARKING) {
      ...
      if (intermed) movePath->appendNode(&intermedPt, LAYER_GROUND);
...
class JetTakeoffOrLandingState : public AIFollowPathState {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(JetTakeoffOrLandingState,
                                          "JetTakeoffOrLandingState")
 private:
  Real m_maxLift;
  Real m_maxSpeed;
 ...
 public:
  JetTakeoffOrLandingState(StateMachine* machine, Bool landing)
      : m_landing(landing),
        AIFollowPathState(machine, "JetTakeoffOrLandingState") {}
  ...
  virtual StateReturnType onEnter() {
    ...
    // only check this for landing; we might have already given up the
    // reservation to the guy behind us for takeoff
    if (m_landing) {
      if (!pp->reserveRunway(jet->getID(), m_landing)) {
        DEBUG_CRASH(
            ("we should never get to this state unless we have a runway "
             "available"));
        return STATE_FAILURE;
      }
    }
    ...
    if (m_landing) {
      ...
      m_circleForLandingPos.z =
          (ppinfo.runwayEnd.z + ppinfo.runwayApproach.z) * 0.5f;
#else
    ...
    if (m_landing) {
      ...
      if (!m_landingSoundPlayed) {
        ...
        PathfindLayerEnum layer =
            TheTerrainLogic->getHighestLayerForDestination(jet->getPosition());
        Real groundZ = TheTerrainLogic->getLayerHeight(
            jet->getPosition()->x, jet->getPosition()->y, layer);

        if (zPos - zSlop <= groundZ) {
          m_landingSoundPlayed = TRUE;
          AudioEventRTS soundToPlay =
              TheAudio->getMiscAudio()->m_aircraftWheelScreech;
      ...
      ParkingPlaceBehaviorInterface* pp = getPP(jet->getProducerID());
      if (pp) pp->transferRunwayReservationToNextInLineForTakeoff(jet->getID());

      PhysicsBehavior* physics = jet->getPhysics();
      Real ratio = physics->getVelocityMagnitude() /
                   (m_maxSpeed * jetAI->friend_getTakeoffSpeedForMaxLift());
  ...
  virtual void onExit(StateExitType status) {
    ...
    if (loco) {
      ...
      if (!jet->isEffectivelyDead()) loco->setMaxLift(BIGNUM);
#ifdef CIRCLE_FOR_LANDING
    ...
    if (pp) pp->releaseRunway(jet->getID());
  }
};
...
class HeliTakeoffOrLandingState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(HeliTakeoffOrLandingState,
                                          "HeliTakeoffOrLandingState")
 protected:
  // snapshot interface
  virtual void crc(Xfer* xfer) {
    // empty. jba.
 ...
 public:
  HeliTakeoffOrLandingState(StateMachine* machine, Bool landing)
      : m_landing(landing),
        State(machine, "HeliTakeoffOrLandingState"),
        m_index(0) {
    m_parkingLoc.zero();
  }
...
// I have disabled this because it is no longer necessary and is a bit funky
// lookin' (srj)
#ifdef NOT_IN_USE
    ...
    if (jet->isKindOf(KINDOF_PRODUCED_AT_HELIPAD) || !m_landing) {
      TheAI->pathfinder()->adjustDestination(jet, jetAI->getLocomotorSet(),
                                             &m_path[m_index]);
    ...
    if (distSqr <= THRESH_SQR) ++m_index;

    if (m_index >= 2) return STATE_SUCCESS;
...
class JetOrHeliParkOrientState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(JetOrHeliParkOrientState,
                                          "JetOrHeliParkOrientState")
 protected:
  // snapshot interface STUBBED.
  virtual void crc(Xfer* xfer) {};
  virtual void xfer(Xfer* xfer) {
 ...
 public:
  JetOrHeliParkOrientState(StateMachine* machine)
      : State(machine, "JetOrHeliParkOrientState") {}
  ...
  virtual StateReturnType update() {
    ...
    ParkingPlaceBehaviorInterface* pp = getPP(jet->getProducerID());
    if (pp == NULL) return STATE_FAILURE;
    ...
    if (fabs(stdAngleDiff(jet->getOrientation(), ppinfo.parkingOrientation)) <=
        THRESH)
      return STATE_SUCCESS;
...
class JetPauseBeforeTakeoffState : public AIFaceState {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(JetPauseBeforeTakeoffState,
                                          "JetPauseBeforeTakeoffState")
 protected:
  // snapshot interface
  virtual void crc(Xfer* xfer) {
    // empty. jba.
 ...
 private:
  ...
  Bool findWaiter() {
    ...
    ParkingPlaceBehaviorInterface* pp =
        getPP(getMachineOwner()->getProducerID());
    if (pp) {
      Int count = pp->getRunwayCount();
      for (Int i = 0; i < count; ++i) {
        Object* otherJet =
            TheGameLogic->findObjectByID(pp->getRunwayReservation(i));
        if (otherJet == NULL || otherJet == jet) continue;
 ...
 public:
  ...
  virtual StateReturnType update() {
    ...
    if (findWaiter()) return STATE_CONTINUE;

    UnsignedInt now = TheGameLogic->getFrame();
    if (!m_resetTimer) {
      // we had to wait, but now everyone else is ready, so restart our
      // countdown.
      m_when = now + jetAI->friend_getTakeoffPause();
      if (m_waitedForTaxiID == INVALID_ID) {
        m_waitedForTaxiID =
            jet->getID();  // just so we don't pick up anyone else
    ...
    // once we start the final wait, release the runways for guys behind us, so
    // they can start taxiing
    ParkingPlaceBehaviorInterface* pp = getPP(jet->getProducerID());
    if (pp && now >= m_whenTransfer) {
      pp->transferRunwayReservationToNextInLineForTakeoff(jet->getID());
    }

    if (now >= m_when) return superStatus;
...
class JetOrHeliReloadAmmoState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(JetOrHeliReloadAmmoState,
                                          "JetOrHeliReloadAmmoState")
 private:
 ...
 public:
  JetOrHeliReloadAmmoState(StateMachine* machine)
      : State(machine, "JetOrHeliReloadAmmoState") {}
  ...
  virtual StateReturnType onEnter() {
    ...
    for (Int i = 0; i < WEAPONSLOT_COUNT; ++i) {
      ...
      if (rt > m_reloadTime) m_reloadTime = rt;
    }

    if (m_reloadTime < 1) m_reloadTime = 1;
  ...
  virtual StateReturnType update() {
    ...
    for (Int i = 0; i < WEAPONSLOT_COUNT; ++i) {
      ...
      if (now >= m_reloadDoneFrame)
        ...
        w->setClipPercentFull(
            (Real)(m_reloadTime - (m_reloadDoneFrame - now)) / m_reloadTime,
            false);

      if (w->getRemainingAmmo() != w->getClipSize()) allDone = false;
    }

    if (allDone) return STATE_SUCCESS;
...
/*
        ...
        Failure: we are unable to get close enough to a friendly airfield to
   land
*/
class JetOrHeliReturnForLandingState : public AIInternalMoveToState {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(JetOrHeliReturnForLandingState,
                                          "JetOrHeliReturnForLandingState")
 public:
  JetOrHeliReturnForLandingState(StateMachine* machine)
      : AIInternalMoveToState(machine, "JetOrHeliReturnForLandingState") {}
  ...
  virtual StateReturnType onEnter() {
    ...
    if (jet->isKindOf(KINDOF_PRODUCED_AT_HELIPAD)) {
      ...
      m_goalPosition = jetAI->friend_needsRunway() ? ppinfo.runwayApproach
                                                   : ppinfo.parkingSpace;
    }
    setAdjustsDestination(false);  // precision is necessary
...
class JetAIStateMachine : public AIStateMachine {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(JetAIStateMachine,
                                          "JetAIStateMachine");
...
//-------------------------------------------------------------------------------------------------
JetAIStateMachine::JetAIStateMachine(Object* owner, AsciiString name)
    : AIStateMachine(owner, name) {
  defineState(RETURNING_FOR_LANDING,
              newInstance(JetOrHeliReturnForLandingState)(this),
              LANDING_AWAIT_CLEARANCE, RETURN_TO_DEAD_AIRFIELD);
  defineState(TAKING_OFF_AWAIT_CLEARANCE,
              newInstance(JetAwaitingRunwayState)(this, false), TAXI_TO_TAKEOFF,
              AI_IDLE);
  defineState(TAXI_TO_TAKEOFF,
              newInstance(JetOrHeliTaxiState)(this, FROM_PARKING),
              PAUSE_BEFORE_TAKEOFF, AI_IDLE);
  defineState(PAUSE_BEFORE_TAKEOFF,
              newInstance(JetPauseBeforeTakeoffState)(this), TAKING_OFF,
              AI_IDLE);
  defineState(TAKING_OFF, newInstance(JetTakeoffOrLandingState)(this, false),
              AI_IDLE, AI_IDLE);
  defineState(LANDING_AWAIT_CLEARANCE,
              newInstance(JetAwaitingRunwayState)(this, true), LANDING,
              AI_IDLE);
  defineState(LANDING, newInstance(JetTakeoffOrLandingState)(this, true),
              TAXI_FROM_LANDING, AI_IDLE);
  defineState(TAXI_FROM_LANDING,
              newInstance(JetOrHeliTaxiState)(this, TO_PARKING),
              ORIENT_FOR_PARKING_PLACE, AI_IDLE);
  defineState(TAXI_FROM_HANGAR,
              newInstance(JetOrHeliTaxiState)(this, FROM_HANGAR),
              ORIENT_FOR_PARKING_PLACE, AI_IDLE);
  defineState(ORIENT_FOR_PARKING_PLACE,
              newInstance(JetOrHeliParkOrientState)(this), RELOAD_AMMO,
              AI_IDLE);
  defineState(RELOAD_AMMO, newInstance(JetOrHeliReloadAmmoState)(this), AI_IDLE,
              AI_IDLE);
  defineState(RETURN_TO_DEAD_AIRFIELD,
              newInstance(JetOrHeliReturningToDeadAirfieldState)(this),
              CIRCLING_DEAD_AIRFIELD, RETURN_TO_DEAD_AIRFIELD);
  defineState(CIRCLING_DEAD_AIRFIELD,
              newInstance(JetOrHeliCirclingDeadAirfieldState)(this), AI_IDLE,
              AI_IDLE);
}
...
class HeliAIStateMachine : public AIStateMachine {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(HeliAIStateMachine,
                                          "HeliAIStateMachine");
...
//-------------------------------------------------------------------------------------------------
HeliAIStateMachine::HeliAIStateMachine(Object* owner, AsciiString name)
    : AIStateMachine(owner, name) {
  defineState(RETURNING_FOR_LANDING,
              newInstance(JetOrHeliReturnForLandingState)(this),
              LANDING_AWAIT_CLEARANCE, RETURN_TO_DEAD_AIRFIELD);
  defineState(TAKING_OFF_AWAIT_CLEARANCE, newInstance(SuccessState)(this),
              TAKING_OFF, AI_IDLE);
  defineState(TAKING_OFF, newInstance(HeliTakeoffOrLandingState)(this, false),
              AI_IDLE, AI_IDLE);
  defineState(LANDING_AWAIT_CLEARANCE, newInstance(SuccessState)(this),
              ORIENT_FOR_PARKING_PLACE, AI_IDLE);
  defineState(ORIENT_FOR_PARKING_PLACE,
              newInstance(JetOrHeliParkOrientState)(this), LANDING, AI_IDLE);
  defineState(LANDING, newInstance(HeliTakeoffOrLandingState)(this, true),
              RELOAD_AMMO, AI_IDLE);
  defineState(RELOAD_AMMO, newInstance(JetOrHeliReloadAmmoState)(this), AI_IDLE,
              AI_IDLE);
  defineState(RETURN_TO_DEAD_AIRFIELD,
  ...
  defineState(CIRCLING_DEAD_AIRFIELD,
              ...
              AI_IDLE);
  defineState(TAXI_FROM_HANGAR,
              newInstance(JetOrHeliTaxiState)(this, FROM_HANGAR), AI_IDLE,
              AI_IDLE);
}
...
/*static*/ void JetAIUpdateModuleData::buildFieldParse(MultiIniFieldParse& p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"OutOfAmmoDamagePerSecond", INI::parsePercentToReal, NULL,
       offsetof(JetAIUpdateModuleData, m_outOfAmmoDamagePerSecond)},
      {"NeedsRunway", INI::parseBool, NULL,
       offsetof(JetAIUpdateModuleData, m_needsRunway)},
      {"KeepsParkingSpaceWhenAirborne", INI::parseBool, NULL,
       offsetof(JetAIUpdateModuleData, m_keepsParkingSpaceWhenAirborne)},
      {"TakeoffSpeedForMaxLift", INI::parsePercentToReal, NULL,
       offsetof(JetAIUpdateModuleData, m_takeoffSpeedForMaxLift)},
      {"TakeoffPause", INI::parseDurationUnsignedInt, NULL,
       offsetof(JetAIUpdateModuleData, m_takeoffPause)},
      {"MinHeight", INI::parseReal, NULL,
       offsetof(JetAIUpdateModuleData, m_minHeight)},
      {"ParkingOffset", INI::parseReal, NULL,
       offsetof(JetAIUpdateModuleData, m_parkingOffset)},
      {"SneakyOffsetWhenAttacking", INI::parseReal, NULL,
       offsetof(JetAIUpdateModuleData, m_sneakyOffsetWhenAttacking)},
      {"AttackLocomotorType", INI::parseIndexList, TheLocomotorSetNames,
       offsetof(JetAIUpdateModuleData, m_attackingLoco)},
      {"AttackLocomotorPersistTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(JetAIUpdateModuleData, m_attackLocoPersistTime)},
      {"AttackersMissPersistTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(JetAIUpdateModuleData, m_attackersMissPersistTime)},
      {"ReturnForAmmoLocomotorType", INI::parseIndexList, TheLocomotorSetNames,
       offsetof(JetAIUpdateModuleData, m_returningLoco)},
      {"LockonTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(JetAIUpdateModuleData, m_lockonTime)},
      {"LockonCursor", INI::parseAsciiString, NULL,
       offsetof(JetAIUpdateModuleData, m_lockonCursor)},
      {"LockonInitialDist", INI::parseReal, NULL,
       offsetof(JetAIUpdateModuleData, m_lockonInitialDist)},
      {"LockonFreq", INI::parseReal, NULL,
       offsetof(JetAIUpdateModuleData, m_lockonFreq)},
      {"LockonAngleSpin", INI::parseAngleReal, NULL,
       offsetof(JetAIUpdateModuleData, m_lockonAngleSpin)},
      {"LockonBlinky", INI::parseBool, NULL,
       offsetof(JetAIUpdateModuleData, m_lockonBlinky)},
      {"ReturnToBaseIdleTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(JetAIUpdateModuleData, m_returnToBaseIdleTime)},
      {0, 0, 0, 0}};
...
//-------------------------------------------------------------------------------------------------
JetAIUpdate::JetAIUpdate(Thing* thing, const ModuleData* moduleData)
    : AIUpdateInterface(thing, moduleData) {
  m_flags = 0;
  m_afterburnerSound =
      *(getObject()->getTemplate()->getPerUnitSound("Afterburner"));
...
Bool JetAIUpdate::isIdle() const {
  // we need to do this because we enter an idle state briefly between
  // takeoff/landing in these cases, but scripting relies on us never claiming
  // to be "idle"...
  if (getFlag(HAS_PENDING_COMMAND)) return false;
...
void JetAIUpdate::onDelete() {
  ...
  if (pp) pp->releaseSpace(getObject()->getID());
}

//-------------------------------------------------------------------------------------------------
void JetAIUpdate::getProducerLocation() {
  if (getFlag(HAS_PRODUCER_LOCATION)) return;
  ...
  /*
          if we aren't allowed to fly, then we should be parked (or at least
     taxiing), which implies we have a parking place reserved. If we don't, it's
     probably because we were directly spawned via script (or directly placed on
     the map). So, check to see if we have no parking place, and if not, quietly
     enable flight.
  */
  ParkingPlaceBehaviorInterface* pp = getPP(jet->getProducerID());
...
UpdateSleepTime JetAIUpdate::update() {
  ...
  // have to call our parent's isIdle, because we override it to never return
  // true when we have a pending command...
  UnsignedInt now = TheGameLogic->getFrame();

  // srj sez: not 100% sure on this. calling RELOAD_AMMO "idle" allows us to get
  // healed while reloading, but might have other side effects we didn't want.
  // if this does prove to cause a bug, be sure that jets (and ESPECIALLY
  // comanches) are still getting healed at airfields.
  if (AIUpdateInterface::isIdle() ||
      getStateMachine()->getCurrentStateID() == RELOAD_AMMO) {
    if (pp != NULL) {
      if (!getFlag(ALLOW_AIR_LOCO) && !getFlag(HAS_PENDING_COMMAND) &&
          jet->isKindOf(KINDOF_PRODUCED_AT_HELIPAD) &&
          jet->getBodyModule()->getHealth() ==
              jet->getBodyModule()->getMaxHealth()) {
    ...
    // note that we might still have weapons with ammo, but still be forced to
    // return to reload.
    if (isOutOfSpecialReloadAmmo(jet) && getFlag(ALLOW_AIR_LOCO)) {
      m_returnToBaseFrame = 0;

      // this is really a "just-in-case" to ensure the targeter list doesn't
      // spin out of control (srj)
      pruneDeadTargeters();
      ...
      getStateMachine()->setState(RETURNING_FOR_LANDING);
               ...
               // srj sez: if we are reloading ammo, wait will we are done
               // before processing the pending command.
               && getStateMachine()->getCurrentStateID() != RELOAD_AMMO) {
      m_returnToBaseFrame = 0;

      AICommandParms parms(
          AICMD_MOVE_TO_POSITION,
          CMD_FROM_AI);  // values don't matter, will be wiped by next line
      m_mostRecentCommand.reconstitute(parms);
      setFlag(HAS_PENDING_COMMAND, false);

      aiDoCommand(&parms);
    } else if (m_returnToBaseFrame != 0 && now >= m_returnToBaseFrame &&
               getFlag(ALLOW_AIR_LOCO)) {
      m_returnToBaseFrame = 0;
      DEBUG_ASSERTCRASH(
          isOutOfSpecialReloadAmmo(jet) == false,
          ("Hmm, this seems unlikely -- isOutOfSpecialReloadAmmo(jet)==false"));
      setFlag(USE_SPECIAL_RETURN_LOCO, false);
      setLastCommandSource(CMD_FROM_AI);
      getStateMachine()->setState(RETURNING_FOR_LANDING);
    } else if (m_returnToBaseFrame == 0 && d->m_returnToBaseIdleTime > 0 &&
               getFlag(ALLOW_AIR_LOCO)) {
  ...
  if (draw != NULL) {
    ...
    Bool needToCheckMinHeight =
        (id >= JETAISTATETYPE_FIRST && id <= JETAISTATETYPE_LAST) ||
        !jet->isAboveTerrain() || !getFlag(ALLOW_AIR_LOCO);
  ...
  if (!jet->isKindOf(KINDOF_PRODUCED_AT_HELIPAD)) {
    ...
    if (draw) {
      if (getFlag(TAKEOFF_IN_PROGRESS) || getFlag(LANDING_IN_PROGRESS) ||
          getObject()->isSignificantlyAboveTerrain() || isMoving() ||
          isWaitingForPath()) {
        if (!m_enginesOn) {
...
Bool JetAIUpdate::getSneakyTargetingOffset(Coord3D* offset) const {
  if (m_attackersMissExpireFrame != 0 &&
      TheGameLogic->getFrame() < m_attackersMissExpireFrame) {
    if (offset) {
      const JetAIUpdateModuleData* d = getJetAIUpdateModuleData();
...
void JetAIUpdate::pruneDeadTargeters() {
  if (!m_targetedBy.empty()) {
    for (std::list<ObjectID>::iterator it = m_targetedBy.begin();
         it != m_targetedBy.end();
         /* empty */) {
...
void JetAIUpdate::positionLockon() {
  ...
  if (dx || dy) m_lockonDrawable->setOrientation(atan2(dy, dx));
  ...
  if (lastPhase && (!thisPhase)) {
    ...
    if (d->m_lockonBlinky) m_lockonDrawable->setDrawableHidden(false);
  } else {
    if (d->m_lockonBlinky) m_lockonDrawable->setDrawableHidden(true);
  }
}

//----------------------------------------------------------------------------------------
void JetAIUpdate::buildLockonDrawableIfNecessary() {
  if (m_untargetableExpireFrame == 0) return;
...
void JetAIUpdate::addTargeter(ObjectID id, Bool add) {
  ...
  if (lockonTime != 0) {
    std::list<ObjectID>::iterator it =
        std::find(m_targetedBy.begin(), m_targetedBy.end(), id);
    if (add) {
...
Bool JetAIUpdate::isTemporarilyPreventingAimSuccess() const {
  return m_untargetableExpireFrame != 0 &&
         (TheGameLogic->getFrame() < m_untargetableExpireFrame);
}

//----------------------------------------------------------------------------------------
Bool JetAIUpdate::isAllowedToMoveAwayFromUnit() const {
  // parked (or landing) units don't get to do this.
  if (!getFlag(ALLOW_AIR_LOCO) || getFlag(TAKEOFF_IN_PROGRESS) ||
...
Bool JetAIUpdate::isDoingGroundMovement(void) const {
  // srj per jba: Air units should never be doing ground movement, even when
  // taxiing... (exception: see getTreatAsAircraftForLocoDistToGoal)
  return false;
}

//-------------------------------------------------------------------------------------------------
Bool JetAIUpdate::getTreatAsAircraftForLocoDistToGoal() const {
  // exception to isDoingGroundMovement: should never treat as aircraft for
  // dist-to-goal when taxiing.
 ...
 */
void JetAIUpdate::privateFollowPath(const std::vector<Coord3D>* path,
                                    Object* ignoreObject,
                                    CommandSourceType cmdSource,
                                    Bool exitProduction) {
  if (exitProduction) {
    getStateMachine()->clear();
    if (ignoreObject) ignoreObstacle(ignoreObject);
    ...
    AIUpdateInterface::privateFollowPath(path, ignoreObject, cmdSource,
                                         exitProduction);
  }
}

//----------------------------------------------------------------------------------------
void JetAIUpdate::privateFollowPathAppend(const Coord3D* pos,
                                          CommandSourceType cmdSource) {
...
//----------------------------------------------------------------------------------------
void JetAIUpdate::doLandingCommand(Object* airfield,
                                   CommandSourceType cmdSource) {
  if (getObject()->isKindOf(KINDOF_PRODUCED_AT_HELIPAD)) {
    ...
    options.maxRadius =
        airfield->getGeometryInfo().getBoundingCircleRadius() * 10.0f;
    if (ThePartitionManager->findPositionAround(&m_landingPosForHelipadStuff,
                                                &options, &tmp))
  ...
  for (BehaviorModule** i = airfield->getBehaviorModules(); *i; ++i) {
    ...
    if (getObject()->isKindOf(KINDOF_PRODUCED_AT_HELIPAD) ||
        pp->reserveSpace(getObject()->getID(), friend_getParkingOffset(),
                         NULL)) {
      // if we had a space at another airfield, release it
      ParkingPlaceBehaviorInterface* oldPP =
          getPP(getObject()->getProducerID());
      ...
      DEBUG_ASSERTCRASH(
          isOutOfSpecialReloadAmmo(getObject()) == false,
          ("Hmm, this seems unlikely -- isOutOfSpecialReloadAmmo(jet)==false"));
      setFlag(USE_SPECIAL_RETURN_LOCO, false);
      setFlag(ALLOW_INTERRUPT_AND_RESUME_OF_CUR_STATE_FOR_RELOAD, false);
      setLastCommandSource(cmdSource);
 ...
 */
void JetAIUpdate::privateEnter(Object* objectToEnter,
                               CommandSourceType cmdSource) {
  // we are already landing. just ignore it.
  if (getFlag(LANDING_IN_PROGRESS)) return;

  if (!TheActionManager->canEnterObject(getObject(), objectToEnter, cmdSource,
                                        DONT_CHECK_CAPACITY))
    return;
 ...
 */
void JetAIUpdate::privateGetRepaired(Object* repairDepot,
                                     CommandSourceType cmdSource) {
  // we are already landing. just ignore it.
  if (getFlag(LANDING_IN_PROGRESS)) return;
...
Bool JetAIUpdate::isParkedAt(const Object* obj) const {
  if (!getFlag(ALLOW_AIR_LOCO) &&
      !getObject()->isKindOf(KINDOF_PRODUCED_AT_HELIPAD) && obj != NULL) {
    Object* airfield;
    ParkingPlaceBehaviorInterface* pp =
        getPP(getObject()->getProducerID(), &airfield);
...
void JetAIUpdate::aiDoCommand(const AICommandParms* parms) {
  ...
  if (getFlag(TAKEOFF_IN_PROGRESS) || getFlag(LANDING_IN_PROGRESS)) {
    ...
    setFlag(HAS_PENDING_COMMAND, true);
    return;
  } else if (parms->m_cmd == AICMD_IDLE &&
             getStateMachine()->getCurrentStateID() == RELOAD_AMMO) {
    // uber-special-case... if we are told to idle, but are reloading ammo,
    // ignore it for now, since we're already doing "nothing" and responding to
    // this will cease our reload... don't just return, tho, in case we were
    // (say) reloading during a guard stint.
    setFlag(HAS_PENDING_COMMAND, true);
    return;
    ...
    switch (parms->m_cmd) {
      ...
      case AICMD_GET_REPAIRED:
        ...
        if (isParkedAt(parms->m_obj)) return;
...
void JetAIUpdate::friend_enableAfterburners(Bool v) {
  ...
  if (v) {
    ...
    if (!m_afterburnerSound.isCurrentlyPlaying()) {
      ...
      m_afterburnerSound.setPlayingHandle(
          TheAudio->addAudioEvent(&m_afterburnerSound));
    }
  } else {
...
void JetAIUpdate::xfer(Xfer* xfer) {
  ...
  // Set on create.
  // AudioEventRTS
  // m_afterburnerSound;		///< Sound when afterburners on
...
void JetAIUpdate::loadPostProcess(void) {
  // When drawables are created, so are their ambient sounds. After loading,
  // only turn off the ambient sound if the engine is off.
  if (!m_enginesOn) {
