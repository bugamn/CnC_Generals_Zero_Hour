...
// ///////////////////////////////////////////////////////////////////////////
enum {
  AS_DOZER,  ///< When not actively Gathering, or when actively building, I am a
             ///< dozer
  AS_SUPPLY_TRUCK  ///< When told explicitly by player or other object, I become
                   ///< a supply truck
};
...

{
  ...
  m_suppliesDepletedVoice =
      getWorkerAIUpdateModuleData()->m_suppliesDepletedVoice;
...
WorkerAIUpdate::~WorkerAIUpdate(void) {
  ...
  if (m_supplyTruckStateMachine) m_supplyTruckStateMachine->deleteInstance();

  if (m_workerMachine) m_workerMachine->deleteInstance();
}
...
//-------------------------------------------------------------------------------------------------
Bool WorkerAIUpdate::isAvailableForSupplying() const { return true; }
  ...
  if (m_workerMachine == NULL) {
    ...
    if (m_supplyTruckStateMachine == NULL) {
      m_supplyTruckStateMachine =
          newInstance(SupplyTruckStateMachine)(getObject());
      m_supplyTruckStateMachine->initDefaultState();
    }

    m_workerMachine->initDefaultState();  // this has to wait until all three
                                          // are in place since
...
UnsignedInt WorkerAIUpdate::getActionDelayForDock(Object *dock) {
  ...
  SupplyCenterDockUpdate *centerModule =
      (SupplyCenterDockUpdate *)dock->findUpdateModule(key_centerUpdate);
  if (centerModule) {
...
UpdateSleepTime WorkerAIUpdate::update(void) {
  ...
  // DO NOT set us as being to able to move with super precision off grid
  // locations Causes workers to get stuck.  jba.
  // if( getCurLocomotor() )
...
//-------------------------------------------------------------------------------------------------
Object *WorkerAIUpdate::construct(const ThingTemplate *what, const Coord3D *pos,
  ...
  // duplicate this code GS - Construct needs to be an AI primitive.  Inheriting
  // off of AIUpdate means you are writing a master brain that will call AI
  // primitives on the object, not something that does stuff itself.
  // SupplyTruckAI decides who to call AIDock on.  Worker should decide to
  // AIDock or AIConstruct or AIRepair.  Dozer should just use the latter two.
  // No construction logic should be in the inherited AIUpdates at all.
  ...
  //
  // set a bit that this object is under construction, it is important to do
  // this early before the hooks add/subtract power from a player are executed
  //
...
// ------------------------------------------------------------------------------------------------
/** We just exited from a supply truck task and are now idle, we should go back
   to Dozer idle mode */
// ------------------------------------------------------------------------------------------------
void WorkerAIUpdate::exitingSupplyTruckState() {
  if (m_workerMachine->getCurrentStateID() == AS_SUPPLY_TRUCK) {
    // We've been given a Dozer specific order that the Supply Truck machine
    // doesn't recognize as BUSY (because this command also recognizes its own
    // busy and is likewise waiting). Explicitly slap it upside the head.
    if (getObject()->getAIUpdateInterface()) {
      getObject()->getAIUpdateInterface()->aiIdle(CMD_FROM_AI);
    }
    m_workerMachine->setState(AS_DOZER);
    // To clarify, I leave supply truck mode when I notice I am doing something
    // not supply truck related.  When given a construct command, I wait to do
    // anything until I notice I'm not busy.  Both states are being polite, so I
    // must force the switch.
  }
}
...
void WorkerAIUpdate::privateIdle(CommandSourceType cmdSource) {
  // Leaving this commented out to show that although the regular supply truck
  // does this, the worker's dozer brain will get completely screwed.
...
void WorkerAIUpdate::privateDock(Object *dock, CommandSourceType cmdSource) {
  ...
  // If this is a command from a player, I will remember this as my favorite
  // dock to override ResourceManager searches.
  if ((cmdSource == CMD_FROM_PLAYER) && dock) {
    // Please note, there is not a separate Warehouse and Center memory by
    // Design.  Because we lack a UI way to click Warehouse and drag to center
    // to set up a specific path, the practical realization has been made that
    // you do not want separate memory.
...
void WorkerAIUpdate::privateRepair(Object *obj, CommandSourceType cmdSource) {
  ...
  //
  // if this object has already been targeted for repair by an object we won't
  // also try to go repair it
...
// ------------------------------------------------------------------------------------------------
void WorkerAIUpdate::privateResumeConstruction(Object *obj,
...
void WorkerAIUpdate::newTask(DozerTask task, Object *target) {
  ...
  m_preferredDock = INVALID_ID;  // If we are dozing, we don't want any supply
                                 // truck stuff going on. jba.
  ...
  if (task == DOZER_TASK_BUILD || task == DOZER_TASK_REPAIR) {
    ...
    Coord3D position;
    target = DozerAIUpdate::findGoodBuildOrRepairPositionAndTarget(me, target,
                                                                   position);
...
void WorkerAIUpdate::internalCancelTask(DozerTask task) {
  ...
  /// @todo we really need a stop command instead of making it move to it's
  /// current location
  ...
  switch (task) {
    ...
    case DOZER_TASK_REPAIR: {
      ...
      if (obj) {
        ...
        if (obj->isKindOf(KINDOF_BRIDGE_TOWER)) removeBridgeScaffolding(obj);
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
const Coord3D *WorkerAIUpdate::getDockPoint(DozerTask task,
  ...
  if (isClearingMines() && m_numberBoxes > 0) {
    ...
    if (draw) {
      draw->updateDrawableSupplyStatus(
          getWorkerAIUpdateModuleData()->m_maxBoxesData, m_numberBoxes);
    }
  }
}
...
Bool WorkerAIUpdate::loseOneBox() {
  if (m_numberBoxes == 0) return FALSE;
...
Bool WorkerAIUpdate::gainOneBox(Int remainingStock) {
  if (getWorkerAIUpdateModuleData() &&
      m_numberBoxes >= getWorkerAIUpdateModuleData()->m_maxBoxesData)
    return FALSE;
  ...
  if (remainingStock == 0) {
    Object *bestWarehouse = getObject()
                                ->getControllingPlayer()
                                ->getResourceGatheringManager()
                                ->findBestSupplyWarehouse(getObject());

    Bool playDepleted = FALSE;
    if (bestWarehouse) {
      // figure out whether the best one is considerably far from the previous
      // one (current position)
      Coord3D delta = *getObject()->getPosition();
      delta.sub(bestWarehouse->getPosition());
      if (delta.length() > getWarehouseScanDistance() / 4) playDepleted = TRUE;
    } else
      playDepleted = TRUE;

    if (playDepleted &&
        m_suppliesDepletedVoice.getEventName().isEmpty() == false) {
      m_suppliesDepletedVoice.setObjectID(getObject()->getID());
      m_suppliesDepletedVoice.setPlayingHandle(
          TheAudio->addAudioEvent(&m_suppliesDepletedVoice));
    }
  }
...
class ActAsSupplyTruckState : public State {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(ActAsSupplyTruckState,
                                          "ActAsSupplyTruckState")
 protected:
  // snapshot interface STUBBED.
  virtual void crc(Xfer *xfer) {};
  virtual void xfer(Xfer *xfer) {};
 ...
 public:
  ActAsSupplyTruckState(StateMachine *machine)
      : State(machine, "ActAsSupplyTruckState") {}
  virtual StateReturnType onEnter();
  virtual StateReturnType update();
  virtual StateReturnType onExit();
};
...
WorkerStateMachine::WorkerStateMachine(Object *owner)
    : StateMachine(owner, "WorkerStateMachine") {
  static const StateConditionInfo asDozerConditions[] = {
      StateConditionInfo(supplyTruckSubMachineWantsToEnter, AS_SUPPLY_TRUCK,
                         NULL),
  ...
  defineState(AS_DOZER, newInstance(ActAsDozerState)(this), INVALID_STATE_ID,
              INVALID_STATE_ID, asDozerConditions);
  defineState(AS_SUPPLY_TRUCK, newInstance(ActAsSupplyTruckState)(this),
              INVALID_STATE_ID, INVALID_STATE_ID, asTruckConditions);
}
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
Bool WorkerStateMachine::supplyTruckSubMachineWantsToEnter(State *thisState,
                                                           void *userData) {
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
Bool WorkerStateMachine::supplyTruckSubMachineReadyToLeave(State *thisState,
                                                           void *userData) {
  Object *owner = thisState->getMachineOwner();
  WorkerAIUpdate *update = (WorkerAIUpdate *)owner->getAIUpdateInterface();
  if (!update) {
  ...
  // so there is no transition out on the way in.  Active and Busy means it
  // isn't doing anything Supply related.
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
StateReturnType ActAsDozerState::onEnter() { return STATE_CONTINUE; }

// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
StateReturnType ActAsDozerState::update() { return STATE_CONTINUE; }
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
StateReturnType ActAsSupplyTruckState::onEnter() { return STATE_CONTINUE; }

// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
StateReturnType ActAsSupplyTruckState::update() { return STATE_CONTINUE; }
...
//------------------------------------------------------------------------------------------------
void WorkerAIUpdate::startBuildingSound(const AudioEventRTS *sound,
