...
// Desc:   Update for something that sits around until hit, then topples and
// stops
///////////////////////////////////////////////////////////////////////////////////////////////////
...
//-------------------------------------------------------------------------------------------------
// this is our "bounce" limit -- slightly less that 90 degrees, to account for
// slop.
...
ToppleUpdateModuleData::ToppleUpdateModuleData() {
  ...
  const Real VELOCITY_BOUNCE_PERCENT =
      0.3f;  // multiply the velocity by this when you bounce
  m_toppleFX = NULL;
  m_bounceFX = NULL;
  m_stumpName.clear();
...
void ToppleUpdateModuleData::buildFieldParse(MultiIniFieldParse& p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"ToppleFX", INI::parseFXList, NULL,
       offsetof(ToppleUpdateModuleData, m_toppleFX)},
      {"BounceFX", INI::parseFXList, NULL,
       offsetof(ToppleUpdateModuleData, m_bounceFX)},
      {"StumpName", INI::parseAsciiString, NULL,
       offsetof(ToppleUpdateModuleData, m_stumpName)},
      {"KillWhenStartToppling", INI::parseBool, NULL,
       offsetof(ToppleUpdateModuleData, m_killWhenStartToppled)},
      {"KillWhenFinishedToppling", INI::parseBool, NULL,
       offsetof(ToppleUpdateModuleData, m_killWhenToppled)},
      {"KillStumpWhenToppled", INI::parseBool, NULL,
       offsetof(ToppleUpdateModuleData, m_killStumpWhenToppled)},
      {"ToppleLeftOrRightOnly", INI::parseBool, NULL,
       offsetof(ToppleUpdateModuleData, m_toppleLeftOrRightOnly)},
      {"ReorientToppledRubble", INI::parseBool, NULL,
       offsetof(ToppleUpdateModuleData, m_reorientToppledRubble)},
      {"InitialVelocityPercent", INI::parsePercentToReal, NULL,
       offsetof(ToppleUpdateModuleData, m_initialVelocityPercent)},
      {"InitialAccelPercent", INI::parsePercentToReal, NULL,
       offsetof(ToppleUpdateModuleData, m_initialAccelPercent)},
      {"BounceVelocityPercent", INI::parsePercentToReal, NULL,
       offsetof(ToppleUpdateModuleData, m_bounceVelocityPercent)},
      {0, 0, 0, 0}};
...
static Real angleClosestTo(Real a1, Real a2, Real desired) {
  ...
  return (fabs(stdAngleDiff(desired, a1)) < fabs(stdAngleDiff(desired, a2)))
             ? a1
             : a2;
}
...
void ToppleUpdate::applyTopplingForce(const Coord3D* toppleDirection,
                                      Real toppleSpeed, UnsignedInt options) {
  if (getObject()->isEffectivelyDead()) return;
  ...
  // rotate around the z-axis so that our x-axis is perpendicular to the topple
  // direction. this is really a trick to ensure that relatively planar things
  // (eg, streetlights) fall parallel to the ground, so that they don't up
  // sticking thru the ground. yeah, it assumes the models are constructed
  // appropriately, but is a cheap way of minimizing the problem. (srj)
  Real curAngleX = normalizeAngle(getObject()->getOrientation());
  Real toppleAngle =
      normalizeAngle(atan2(m_toppleDirection.y, m_toppleDirection.x));
  if (d->m_toppleLeftOrRightOnly) {
    // it's a fence or such, and can only topple left or right, so pick the
    // closest
    toppleAngle =
        angleClosestTo(curAngleX + PI / 2, curAngleX - PI / 2, toppleAngle);
    m_toppleDirection.x = Cos(toppleAngle);
    m_toppleDirection.y = Sin(toppleAngle);

    // go ahead and remove it from the pathfinder now, rather than waiting for
    // the topple to finish.... since we might be in a slightly different
    // position when toppled, which can confuse the pathfinder and not
    // de-obstacle everything correctly
    TheAI->pathfinder()->removeObjectFromPathfindMap(getObject());
  }
  // desired angle is toppleAngle +/- pi/2, whichever is closer to curangle
  Real desiredAngleX =
      angleClosestTo(toppleAngle + PI / 2, toppleAngle - PI / 2, curAngleX);
  m_numAngleDeltaX = REAL_TO_INT_FLOOR(ANGULAR_LIMIT / (m_angularVelocity * 2));
  if (m_numAngleDeltaX < 1) m_numAngleDeltaX = 1;
  ...
  if (!d->m_stumpName.isEmpty()) {
    ...
    if (stump) {
      ...
      if (draw) {
        if (draw->getModelConditionFlags().test(MODELCONDITION_BURNED) ==
            TRUE) {
...
UpdateSleepTime ToppleUpdate::update() {
  ...
  DEBUG_ASSERTCRASH(m_toppleState != TOPPLE_UPRIGHT,
                    ("hmm, we should be sleeping here"));
  if ((m_toppleState == TOPPLE_UPRIGHT) || (m_toppleState == TOPPLE_DOWN))
    return UPDATE_SLEEP_FOREVER;

  const ToppleUpdateModuleData* d = getToppleUpdateModuleData();
  const Real VELOCITY_BOUNCE_LIMIT =
      0.01f;  // if the velocity after a bounce will be this or lower, just stop
              // at zero
  const Real VELOCITY_BOUNCE_SOUND_LIMIT =
      0.03f;  // and if this low, then skip the bounce sound
    ...
    if (BitTest(m_options, TOPPLE_OPTIONS_NO_BOUNCE) == TRUE ||
      ...
      if (d->m_killWhenToppled) {
        ...
        if (d->m_reorientToppledRubble) {
          // we have a separate rubble state that needs to be upright, and
          // centered on the new "center" pos...
          Vector3 pos;
          pos.X = 0;
          pos.Y = 0;
          ...
          // this relies on the fact that setOrientation always forces us
          // straight up in the Z axis!
...
/** Do the collision */
//-------------------------------------------------------------------------------------------------
void ToppleUpdate::onCollide(Object* other, const Coord3D* loc,
                             const Coord3D* normal) {
  // Note that other == null means "collide with ground"
  //
  if (other == NULL) return;

  //@todo JohnA -- Should you get around to adding trees to avoidance
  //pathfinding, then you'll want to change this code: if(
  // other->getCrusherLevel() > getObject()->getCrushableLevel() ) //<----proper
  // tree method
