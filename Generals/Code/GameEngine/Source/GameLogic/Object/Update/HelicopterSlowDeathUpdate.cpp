...
// //////////////////////////////////////////////////////////// Author: Colin
// Day, March 2002 Desc:   Helicoptor slow deaths
///////////////////////////////////////////////////////////////////////////////////////////////////
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
/*static*/ void HelicopterSlowDeathBehaviorModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  SlowDeathBehaviorModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"SpiralOrbitTurnRate", INI::parseAngularVelocityReal, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_spiralOrbitTurnRate)},
      {"SpiralOrbitForwardSpeed", INI::parseVelocityReal, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData,
                m_spiralOrbitForwardSpeed)},
      {"SpiralOrbitForwardSpeedDamping", INI::parseReal, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData,
                m_spiralOrbitForwardSpeedDamping)},
      {"MinSelfSpin", INI::parseAngularVelocityReal, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_minSelfSpin)},
      {"MaxSelfSpin", INI::parseAngularVelocityReal, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_maxSelfSpin)},
      {"SelfSpinUpdateDelay", INI::parseDurationReal, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_selfSpinUpdateDelay)},
      {"SelfSpinUpdateAmount", INI::parseAngleReal, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_selfSpinUpdateAmount)},
      {"FallHowFast", INI::parsePercentToReal, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_fallHowFast)},
      {"MinBladeFlyOffDelay", INI::parseDurationReal, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_minBladeFlyOffDelay)},
      {"MaxBladeFlyOffDelay", INI::parseDurationReal, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_maxBladeFlyOffDelay)},
      {"AttachParticle", INI::parseParticleSystemTemplate, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_attachParticleSystem)},
      {"AttachParticleBone", INI::parseAsciiString, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_attachParticleBone)},
      {"AttachParticleLoc", INI::parseCoord3D, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_attachParticleLoc)},
      {"BladeObjectName", INI::parseAsciiString, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_bladeObjectName)},
      {"BladeBoneName", INI::parseAsciiString, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_bladeBone)},
      {"OCLEjectPilot", INI::parseObjectCreationList, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_oclEjectPilot)},
      {"FXBlade", INI::parseFXList, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_fxBlade)},
      {"OCLBlade", INI::parseObjectCreationList, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_oclBlade)},
      {"FXHitGround", INI::parseFXList, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_fxHitGround)},
      {"OCLHitGround", INI::parseObjectCreationList, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_oclHitGround)},
      {"FXFinalBlowUp", INI::parseFXList, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_fxFinalBlowUp)},
      {"OCLFinalBlowUp", INI::parseObjectCreationList, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_oclFinalBlowUp)},
      {"DelayFromGroundToFinalDeath", INI::parseDurationReal, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData,
                m_delayFromGroundToFinalDeath)},
      {"FinalRubbleObject", INI::parseAsciiString, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_finalRubbleObject)},
      {"SoundDeathLoop", INI::parseAudioEventRTS, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_deathSound)},
      {"MaxBraking", INI::parseAccelerationReal, NULL,
       offsetof(HelicopterSlowDeathBehaviorModuleData, m_maxBraking)},
...
//
// we use these to guide the direction of the circular orbit, as well as the
// direction the object self spins
//
...
void HelicopterSlowDeathBehavior::beginSlowDeath(const DamageInfo *damageInfo) {
  ...
  // get the module data
  const HelicopterSlowDeathBehaviorModuleData *modData =
      getHelicopterSlowDeathBehaviorModuleData();
  ...
  //
  // record the current forward angle of the object to be the start angle of our
  // downward spin circle angle
  //
  ...
  if (modData->m_attachParticleSystem) {
    ParticleSystem *pSys = TheParticleSystemManager->createParticleSystem(
        modData->m_attachParticleSystem);
    if (pSys) {
      ...
      if (modData->m_attachParticleBone.isEmpty() == FALSE) {
        ...
        if (draw) {
          ...
          if (draw->getPristineBonePositions(
                  modData->m_attachParticleBone.str(), 0, &pos, NULL, 1))
        ...
        // use location coord specified ... it will be zero if not given which
        // is center of obj anyway
...
UpdateSleepTime HelicopterSlowDeathBehavior::update(void) {
  ...
  if (m_hitGroundFrame == 0) {
    //
    // change the angle of the helicoptor based on the spiral rate for the
    // turning on the large downward spin circle
    //

    // copter->setOrientation( copter->getOrientation() + m_selfSpin *
    // m_orbitDirection );
    ...
    //
    // over time we change the rate at which we self spin around our center of
    // gravity ... we will ping pong back and forth between the MinSelfSpin and
    // MaxSelfSpin defined in INI
    //
    if (modData->m_selfSpinUpdateDelay &&
        TheGameLogic->getFrame() - m_lastSelfSpinUpdateFrame >
            modData->m_selfSpinUpdateDelay) {
      ...
      if (m_selfSpinTowardsMax == TRUE) {
        ...
        if (m_selfSpin > modData->m_maxSelfSpin) {
          ...
          m_selfSpinTowardsMax =
              FALSE;  // now start changing spin towards min again
        ...
        if (m_selfSpin < modData->m_minSelfSpin) {
          ...
          m_selfSpinTowardsMax =
              TRUE;  // now start chaning spin towards max again
    ...
    DEBUG_ASSERTCRASH(physics, ("HelicopterSlowDeathBehavior: object '%s' does "
                                "not have a physics module\n",
                                copter->getTemplate()->getName().str()));

    //
    // apply a force to the helicopter pushing it in a forward motion
    // according to the forward angle & speed we are keeping track of to make
    // our downward circle (that forward angle is *NOT* the angle the object is
    // facing
    //
    Coord3D force;
    ...
    // update our forward angle for travelling along the large spiral downward
    // circle
    ...
    if (m_bladeFlyOffFrame > 0) {
      ...
      if (m_bladeFlyOffFrame <= 0) {
        ...
        if (draw) {
          draw->getPristineBonePositions(modData->m_bladeBone.str(), 0,
                                         &bladePos, NULL, 1);
        ...
        //"OCL_ComancheBlades" ); 			ObjectCreationList::create( ocl, &bladePos );
        ...
        // note that we call EjectPilotDie::ejectPilot() rather than
        // ObjectCreationList::create(), because the former makes the right
        // sounds, and also constrains to veteran-or-better status.
        //
        if (modData->m_oclEjectPilot &&
            copter->getVeterancyLevel() > LEVEL_REGULAR)
  ...
  Bool hitATree = FALSE;
  // Here we want to make sure we crash if we collide with a tree on the way
  // down
  ...
  if (m_hitGroundFrame == 0 && phys) {
    ...
    if (tree) {
      if (tree->isKindOf(KINDOF_SHRUBBERY)) hitATree = TRUE;
    }
  }
  ...
  if (m_hitGroundFrame == 0) {
    // srj sez: if we haven't yet hit the ground, adjust our layer properly so
    // we crash on bridges
    Coord3D tmpPt = *pos;
    tmpPt.z = 99999.0f;
    PathfindLayerEnum newLayer =
        TheTerrainLogic->getHighestLayerForDestination(&tmpPt);
    ...
    if (pos->z <= ground + 1.0f || hitATree) {
      ...
      Drawable *draw = copter->getDrawable();
      if (draw) draw->setModelConditionState(MODELCONDITION_SPECIAL_DAMAGED);
  ...
  if (m_hitGroundFrame && TheGameLogic->getFrame() - m_hitGroundFrame >
                              modData->m_delayFromGroundToFinalDeath) {
    // make effect
    ...
    const ThingTemplate *ttn =
        TheThingFactory->findTemplate(modData->m_finalRubbleObject);
