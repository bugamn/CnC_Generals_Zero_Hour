...
// Desc:   Updates stickybomb to stay on target while it counts down to
// detonation (lifetime update)
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void StickyBombUpdate::onObjectCreated() {
  // This first step is an initialization step. Immediately after the stickybomb
  // is created, the producerID is stored in the object to signify the shooter
  // of this weapon. From there, we can get the target. The target information
  // will be used to determine where the sticky bomb will go.
...
void StickyBombUpdate::init(const Object *target, const Object *bomber) {
  ...
  LifetimeUpdate *update =
      (LifetimeUpdate *)getObject()->findUpdateModule(key_LifetimeUpdate);
  if (update) {
    // we are a timer bomb
  ...
  if (target) {
    ...
    // let the bomb just stay where it was first put, so a mine clearing unit
    // can get to it later
...
UpdateSleepTime StickyBombUpdate::update(void) {
  // Continually reset position of stickybomb to match the position of the
  // target.
...
//-------------------------------------------------------------------------------------------------
void StickyBombUpdate::detonate() { getObject()->kill(); }
