...
class PartitionFilterHordeMember : public PartitionFilter {
 ...
 public:
  ...
  virtual Bool allow(Object* objOther) {
    ...
    // doh
    if (m_obj->isOffMap() != objOther->isOffMap()) return false;
...
/*static*/ void HordeUpdateModuleData::buildFieldParse(MultiIniFieldParse& p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"UpdateRate", INI::parseDurationUnsignedInt, NULL,
       offsetof(HordeUpdateModuleData, m_updateRate)},
      {"KindOf", KindOfMaskType::parseFromINI, NULL,
       offsetof(HordeUpdateModuleData, m_kindof)},
      {"Count", INI::parseInt, NULL,
       offsetof(HordeUpdateModuleData, m_minCount)},
      {"Radius", INI::parseReal, NULL,
       offsetof(HordeUpdateModuleData, m_minDist)},
      {"RubOffRadius", INI::parseReal, NULL,
       offsetof(HordeUpdateModuleData, m_rubOffRadius)},
      {"AlliesOnly", INI::parseBool, NULL,
       offsetof(HordeUpdateModuleData, m_alliesOnly)},
      {"ExactMatch", INI::parseBool, NULL,
       offsetof(HordeUpdateModuleData, m_exactMatch)},
      {"Action", INI::parseIndexList, TheHordeActionTypeNames,
       offsetof(HordeUpdateModuleData, m_action)},
      {"FlagSubObjectNames", INI::parseAsciiStringVector, NULL,
       offsetof(HordeUpdateModuleData, m_flagSubObjNames)},
      {0, 0, 0, 0}};
...
//-------------------------------------------------------------------------------------------------
/** @todo I think we should model the horde list ... so we can do all this
 * without doing all this scanning, plus we can give exactly 1 flag to the right
 * person in the center of the horde which I think would look better (CBD)
 *
 * Redesign occurred 10/15, where the flags have been removed replaced by a
 * teraindecal for each horde member Thank You for reading, MLorenzen
 */
// ------------------------------------------------------------------------------------------------
    ...
    case HORDEACTION_HORDE: {
      ...
      if (prevInHorde != m_inHorde) {
        ...
        if (ai)
          ...
          DEBUG_CRASH(
              ("HordeUpdate::joinOrLeaveHorde - We (%s) must have an AI to "
               "benefit from horde\n",
  ...
  if (!d->m_flagSubObjNames.empty()) {
    ...
    if (draw && obj) {
      for (std::vector<AsciiString>::const_iterator it =
               d->m_flagSubObjNames.begin();
           it != d->m_flagSubObjNames.end(); ++it)
...
UpdateSleepTime HordeUpdate::update(void) {
  ...
  // This is a sticky situation, where refreshing the model state (like from
  // default to damaged, for example) will rebuild the terraindecal and set its
  // size to the default size.... since Vehicles have a special size, we want to
  // keep it fresh, here, but not do the horde-ing test every frame...
  Bool isInfantry = (obj->isKindOf(KINDOF_INFANTRY));
  if (isInfantry ||
      (TheGameLogic->getFrame() > m_lastHordeRefreshFrame + md->m_updateRate)) {
    m_lastHordeRefreshFrame = TheGameLogic->getFrame();
    ...
    if ((iter->getCount() >=
         md->m_minCount - 1))  // we really are in the thick part of the horde
    {
      m_inHorde = TRUE;
      ...
      for (Object* other = iter->first(); other; other = iter->next()) {
        ...
        if (hui != NULL && hui->isTrueHordeMember()) {
          Real dist = ThePartitionManager->getDistanceSquared(
              getObject(), other, FROM_CENTER_2D);
    ...
    AIUpdateInterface* ai = getObject()->getAIUpdateInterface();
    if (ai) ai->evaluateMoraleBonus();
  }

  // This is a sticky situation, where refreshing the model state (like from
  // default to damaged, for example) will rebuild the terraindecal and set its
  // size to the default size.... since Vehicles have a special size, we want to
  // keep it fresh, here, but not do the horde-ing test every frame... This is a
  // weak solution, in that It causes this update to fight the defualt behavior
  // of the modelstate methods, But in the interest of not breaking the
  // modelstate changing logic for five hundred other units a week before
  // golden, this is my solution... If anyone gets this note on the next
  // project... please please please, fix the resetting of the
  ...
  if (draw && !obj->isEffectivelyDead()) {
    if (TheGameLogic->getDrawIconUI()) {
      if (m_inHorde &&
          !obj->isKindOf(
              KINDOF_PORTABLE_STRUCTURE))  // this not is a ride-on for overlord
      {
        TerrainDecalType nuType;  // uninitialized

        if (isInfantry) {
          if (obj->testWeaponBonusCondition(WEAPONBONUSCONDITION_NATIONALISM) ==
              TRUE)
