...
// Desc:   This system tracks the objects status with regards to firing, and
// whenever the object stops
//         firing, and all the conditions are met, then it'll create the
//         specified OCL.
///////////////////////////////////////////////////////////////////////////////////////////////////
...
//-------------------------------------------------------------------------------------------------
void FireOCLAfterWeaponCooldownUpdateModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  UpdateModuleData::buildFieldParse(p);
  static const FieldParse dataFieldParse[] = {
      {"WeaponSlot", INI::parseLookupList, TheWeaponSlotTypeNamesLookupList,
       offsetof(FireOCLAfterWeaponCooldownUpdateModuleData, m_weaponSlot)},
      {"OCL", INI::parseObjectCreationList, NULL,
       offsetof(FireOCLAfterWeaponCooldownUpdateModuleData, m_ocl)},
      {"MinShotsToCreateOCL", INI::parseUnsignedInt, NULL,
       offsetof(FireOCLAfterWeaponCooldownUpdateModuleData,
                m_minShotsRequired)},
      {"OCLLifetimePerSecond", INI::parseUnsignedInt, NULL,
       offsetof(FireOCLAfterWeaponCooldownUpdateModuleData,
                m_oclLifetimePerSecond)},
      {"OCLLifetimeMaxCap", INI::parseDurationUnsignedInt, NULL,
       offsetof(FireOCLAfterWeaponCooldownUpdateModuleData, m_oclMaxFrames)},
      {0, 0, 0, 0}};
  p.add(dataFieldParse);
  p.add(UpgradeMuxData::getFieldParse(),
        offsetof(FireOCLAfterWeaponCooldownUpdateModuleData, m_upgradeMuxData));
}
...
UpdateSleepTime FireOCLAfterWeaponCooldownUpdate::update(void) {
  const FireOCLAfterWeaponCooldownUpdateModuleData *data =
      getFireOCLAfterWeaponCooldownUpdateModuleData();
  ...
  if (weapon) {
    if (weapon->getWeaponSlot() != data->m_weaponSlot) {
      // Not the right weapon slot -- it's possible we switched weapons in which
      // case it's possible to fire the OCL.
      validThisFrame = false;
    }
  } else {
    // No weapon selected -- possible to switch off weapons, possible to have
    // just finished firing our weapon.
    validThisFrame = false;
  }
  ...
  if (validThisFrame) {
    if (weapon->getLastShotFrame() == now - 1) {
      ...
      if (m_consecutiveShots == 1) {
        // Our first shot -- so record the frame number so we can easily
        // calculate the time we've been firing!
        m_startFrame = now;
      }
    ...
    // We've stopped being valid -- which means we may have stopped firing our
    // weapon.
