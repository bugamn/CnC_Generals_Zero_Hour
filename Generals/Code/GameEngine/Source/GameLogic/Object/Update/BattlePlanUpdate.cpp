...
// Desc:   Update module to handle building states and battle plan execution &
// changes
///////////////////////////////////////////////////////////////////////////////////////////////////
...
//-------------------------------------------------------------------------------------------------
/*static*/ void BattlePlanUpdateModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  ModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"SpecialPowerTemplate", INI::parseSpecialPowerTemplate, NULL,
       offsetof(BattlePlanUpdateModuleData, m_specialPowerTemplate)},

      {"BombardmentPlanAnimationTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(BattlePlanUpdateModuleData, m_bombardmentPlanAnimationFrames)},
      {"HoldTheLinePlanAnimationTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(BattlePlanUpdateModuleData, m_holdTheLinePlanAnimationFrames)},
      {"SearchAndDestroyPlanAnimationTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(BattlePlanUpdateModuleData,
                m_searchAndDestroyPlanAnimationFrames)},
      {"TransitionIdleTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(BattlePlanUpdateModuleData, m_transitionIdleFrames)},

      {"BombardmentPlanUnpackSoundName", INI::parseAsciiString, NULL,
       offsetof(BattlePlanUpdateModuleData, m_bombardmentUnpackName)},
      {"BombardmentPlanPackSoundName", INI::parseAsciiString, NULL,
       offsetof(BattlePlanUpdateModuleData, m_bombardmentPackName)},
      {"BombardmentMessageLabel", INI::parseAsciiString, NULL,
       offsetof(BattlePlanUpdateModuleData, m_bombardmentMessageLabel)},
      {"BombardmentAnnouncementName", INI::parseAsciiString, NULL,
       offsetof(BattlePlanUpdateModuleData, m_bombardmentAnnouncementName)},
      {"SearchAndDestroyPlanUnpackSoundName", INI::parseAsciiString, NULL,
       offsetof(BattlePlanUpdateModuleData, m_searchAndDestroyUnpackName)},
      {"SearchAndDestroyPlanIdleLoopSoundName", INI::parseAsciiString, NULL,
       offsetof(BattlePlanUpdateModuleData, m_searchAndDestroyIdleName)},
      {"SearchAndDestroyPlanPackSoundName", INI::parseAsciiString, NULL,
       offsetof(BattlePlanUpdateModuleData, m_searchAndDestroyPackName)},
      {"SearchAndDestroyMessageLabel", INI::parseAsciiString, NULL,
       offsetof(BattlePlanUpdateModuleData, m_searchAndDestroyMessageLabel)},
      {"SearchAndDestroyAnnouncementName", INI::parseAsciiString, NULL,
       offsetof(BattlePlanUpdateModuleData,
                m_searchAndDestroyAnnouncementName)},
      {"HoldTheLinePlanUnpackSoundName", INI::parseAsciiString, NULL,
       offsetof(BattlePlanUpdateModuleData, m_holdTheLineUnpackName)},
      {"HoldTheLinePlanPackSoundName", INI::parseAsciiString, NULL,
       offsetof(BattlePlanUpdateModuleData, m_holdTheLinePackName)},
      {"HoldTheLineMessageLabel", INI::parseAsciiString, NULL,
       offsetof(BattlePlanUpdateModuleData, m_holdTheLineMessageLabel)},
      {"HoldTheLineAnnouncementName", INI::parseAsciiString, NULL,
       offsetof(BattlePlanUpdateModuleData, m_holdTheLineAnnouncementName)},

      {"ValidMemberKindOf", KindOfMaskType::parseFromINI, NULL,
       offsetof(BattlePlanUpdateModuleData, m_validMemberKindOf)},
      {"InvalidMemberKindOf", KindOfMaskType::parseFromINI, NULL,
       offsetof(BattlePlanUpdateModuleData, m_invalidMemberKindOf)},
      {"BattlePlanChangeParalyzeTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(BattlePlanUpdateModuleData, m_battlePlanParalyzeFrames)},
      {"HoldTheLinePlanArmorDamageScalar", INI::parseReal, NULL,
       offsetof(BattlePlanUpdateModuleData, m_holdTheLineArmorDamageScalar)},
      {"SearchAndDestroyPlanSightRangeScalar", INI::parseReal, NULL,
       offsetof(BattlePlanUpdateModuleData,
                m_searchAndDestroySightRangeScalar)},

      {"StrategyCenterSearchAndDestroySightRangeScalar", INI::parseReal, NULL,
       offsetof(BattlePlanUpdateModuleData,
                m_strategyCenterSearchAndDestroySightRangeScalar)},
      {"StrategyCenterSearchAndDestroyDetectsStealth", INI::parseBool, NULL,
       offsetof(BattlePlanUpdateModuleData,
                m_strategyCenterSearchAndDestroyDetectsStealth)},
      {"StrategyCenterHoldTheLineMaxHealthScalar", INI::parseReal, NULL,
       offsetof(BattlePlanUpdateModuleData,
                m_strategyCenterHoldTheLineMaxHealthScalar)},
      {"StrategyCenterHoldTheLineMaxHealthChangeType", INI::parseIndexList,
       TheMaxHealthChangeTypeNames,
       offsetof(BattlePlanUpdateModuleData,
                m_strategyCenterHoldTheLineMaxHealthChangeType)},

      {"VisionObjectName", INI::parseAsciiString, NULL,
       offsetof(BattlePlanUpdateModuleData, m_visionObjectName)},
...
BattlePlanUpdate::BattlePlanUpdate(Thing *thing, const ModuleData *moduleData)
    : UpdateModule(thing, moduleData), m_bonuses(NULL) {
...
void BattlePlanUpdate::onDelete() {
  ...
  // (and which point it doesn't really matter if we can remove the bonus or
  // not)
...
void BattlePlanUpdate::onObjectCreated() {
  ...
  if (!data->m_specialPowerTemplate) {
    DEBUG_CRASH(
        ("%s object's BattlePlanUpdate lacks access to the "
  ...
  m_searchAndDestroyAnnouncement.setEventName(
      data->m_searchAndDestroyAnnouncementName);
...
Bool BattlePlanUpdate::isPowerCurrentlyInUse(
  ...
  if (m_nextReadyFrame <= now) {
    switch (m_status) {
      case TRANSITIONSTATUS_IDLE:
        // There's only two cases where we are in an idle status -- upon
        // initialization when no plan has yet been selected. The other case is
        // after we've finished packing the previous plan up and are waiting to
      ...
      case TRANSITIONSTATUS_UNPACKING:
        // If we're unpacking, we are forcing the user to wait until the plan is
        // unpacked before allowing him to select a new plan. The plan doesn't
        // become active until we're finished unpacking.
      ...
      case TRANSITIONSTATUS_ACTIVE:
        // If we're active and the user has selected a different plan, then we
        // need to pack up.
        if (m_currentPlan != m_desiredPlan) {
          if (m_currentPlan == PLANSTATUS_BOMBARDMENT) {
            // Special case situation -- in bombardment status, we need to
            // center the turret prior to packing up, so handle it here.
            ...
            if (ai) {
              if (isTurretInNaturalPosition()) {
                ...
                // It's not centered, and not trying to center, so order it to
                // center.
      ...
      case TRANSITIONSTATUS_PACKING:
        // If we finished packing, then go idle until we can switch to our new
        // plan.
...
void BattlePlanUpdate::createVisionObject() {
  ...
  const ThingTemplate *tt =
      TheThingFactory->findTemplate(data->m_visionObjectName);
  DEBUG_ASSERTCRASH(
      tt, ("BattlePlanUpdate::setStatus - Invalid vision object name '%s'\n",
      ...
      switch (m_currentPlan) {
        ...
        case PLANSTATUS_SEARCHANDDESTROY:
          ...
          TheAudio->removeAudioEvent(
              m_searchAndDestroyUnpack.getPlayingHandle());
          break;
      }
      break;
  ...
  switch (newStatus) {
    ...
    case TRANSITIONSTATUS_UNPACKING: {
      ...
      TheRadar->createPlayerEvent(obj->getControllingPlayer(),
                                  obj->getPosition(), RADAR_EVENT_BATTLE_PLAN);
      ...
      switch (m_currentPlan) {
        case PLANSTATUS_BOMBARDMENT:
          ...
          obj->getDrawable()->setAnimationLoopDuration(
              data->m_bombardmentPlanAnimationFrames);
          ...
          if (m_bombardmentUnpack.getEventName().isNotEmpty()) {
            ...
            m_bombardmentUnpack.setPlayingHandle(
                TheAudio->addAudioEvent(&m_bombardmentUnpack));
          }

          // display a message to *all* users
          TheInGameUI->message(
              TheGameText->fetch(data->m_bombardmentMessageLabel));
        ...
        case PLANSTATUS_HOLDTHELINE:
          ...
          obj->getDrawable()->setAnimationLoopDuration(
              data->m_holdTheLinePlanAnimationFrames);
          ...
          if (m_holdTheLineUnpack.getEventName().isNotEmpty()) {
            ...
            m_holdTheLineUnpack.setPlayingHandle(
                TheAudio->addAudioEvent(&m_holdTheLineUnpack));
          }

          // display a message to *all* users
          TheInGameUI->message(
              TheGameText->fetch(data->m_holdTheLineMessageLabel));
        ...
        case PLANSTATUS_SEARCHANDDESTROY:
          ...
          obj->getDrawable()->setAnimationLoopDuration(
              data->m_searchAndDestroyPlanAnimationFrames);
          ...
          if (m_searchAndDestroyUnpack.getEventName().isNotEmpty()) {
            ...
            m_searchAndDestroyUnpack.setPlayingHandle(
                TheAudio->addAudioEvent(&m_searchAndDestroyUnpack));
          }

          // display a message to *all* users
          TheInGameUI->message(
              TheGameText->fetch(data->m_searchAndDestroyMessageLabel));
          if (m_searchAndDestroyAnnouncement.getEventName().isEmpty() ==
              FALSE) {
      ...
      switch (m_currentPlan) {
        ...
        case PLANSTATUS_SEARCHANDDESTROY:
          ...
          if (m_searchAndDestroyIdle.getEventName().isNotEmpty()) {
            ...
            m_searchAndDestroyIdle.setPlayingHandle(
                TheAudio->addAudioEvent(&m_searchAndDestroyIdle));
          }
          break;
      }
      break;
      ...
      switch (m_currentPlan) {
        case PLANSTATUS_BOMBARDMENT:
          ...
          if (m_bombardmentUnpack.getEventName().isNotEmpty()) {
            ...
            m_bombardmentPack.setPlayingHandle(
                TheAudio->addAudioEvent(&m_bombardmentPack));
          }
          break;
        case PLANSTATUS_HOLDTHELINE:
          ...
          if (m_holdTheLineUnpack.getEventName().isNotEmpty()) {
            ...
            m_holdTheLinePack.setPlayingHandle(
                TheAudio->addAudioEvent(&m_holdTheLinePack));
          }
          break;
        case PLANSTATUS_SEARCHANDDESTROY:
          ...
          if (m_searchAndDestroyUnpack.getEventName().isNotEmpty()) {
            ...
            m_searchAndDestroyPack.setPlayingHandle(
                TheAudio->addAudioEvent(&m_searchAndDestroyPack));
          }
          break;
      }
      break;
...
static void paralyzeTroop(Object *obj, void *userData) {
  const BattlePlanUpdateModuleData *data =
      (BattlePlanUpdateModuleData *)userData;
  if (obj->isAnyKindOf(data->m_validMemberKindOf)) {
    if (!obj->isAnyKindOf(data->m_invalidMemberKindOf)) {
      obj->setDisabledUntil(
          DISABLED_PARALYZED,
          TheGameLogic->getFrame() + data->m_battlePlanParalyzeFrames);
    }
  }
}
  ...
  if (player) {
    switch (m_planAffectingArmy) {
      case PLANSTATUS_BOMBARDMENT: {
        ...
        // The only building bonus is actually the turret, and that's already
        // handled!
        break;
      }
      case PLANSTATUS_HOLDTHELINE: {
        ...
        if (data->m_strategyCenterHoldTheLineMaxHealthScalar != 1.0f) {
          ...
          body->setMaxHealth(
              body->getMaxHealth() * 1.0f /
                  data->m_strategyCenterHoldTheLineMaxHealthScalar,
              data->m_strategyCenterHoldTheLineMaxHealthChangeType);
        }
        break;
      }
      case PLANSTATUS_SEARCHANDDESTROY: {
        ...
        if (data->m_strategyCenterSearchAndDestroySightRangeScalar != 1.0f) {
          obj->setVisionRange(
              obj->getVisionRange() * 1.0f /
              data->m_strategyCenterSearchAndDestroySightRangeScalar);
          obj->setShroudClearingRange(
              obj->getShroudClearingRange() * 1.0f /
              data->m_strategyCenterSearchAndDestroySightRangeScalar);
        }

        // Remove stealth detection
        if (data->m_strategyCenterSearchAndDestroyDetectsStealth) {
          static NameKeyType key_StealthDetectorUpdate =
              NAMEKEY("StealthDetectorUpdate");
          StealthDetectorUpdate *update =
              (StealthDetectorUpdate *)obj->findUpdateModule(
                  key_StealthDetectorUpdate);
          if (update) {
    ...
    switch (plan) {
      ...
      case PLANSTATUS_SEARCHANDDESTROY:
        ...
        m_bonuses->m_sightRangeScalar =
            data->m_searchAndDestroySightRangeScalar;
...
//------------------------------------------------------------------------------------------------
// Returns the currently active battle plan -- unpacked and ready... returns
// PLANSTATUS_NONE if in transition!
//------------------------------------------------------------------------------------------------
