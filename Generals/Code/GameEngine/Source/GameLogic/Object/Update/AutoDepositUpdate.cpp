...
AutoDepositUpdate::AutoDepositUpdate(Thing *thing, const ModuleData *moduleData)
  ...
  m_depositOnFrame = TheGameLogic->getFrame() +
                     getAutoDepositUpdateModuleData()->m_depositFrame;
...
void AutoDepositUpdate::awardInitialCaptureBonus(Player *player) {
  m_depositOnFrame = TheGameLogic->getFrame() +
                     getAutoDepositUpdateModuleData()->m_depositFrame;
  if (!player || !m_awardInitialCaptureBonus ||
      getAutoDepositUpdateModuleData()->m_initialCaptureBonus <= 0)
    return;

  player->getMoney()->deposit(
      getAutoDepositUpdateModuleData()->m_initialCaptureBonus);
...
UpdateSleepTime AutoDepositUpdate::update(void) {
  ...
  if (TheGameLogic->getFrame() >= m_depositOnFrame) {
    if (!m_initialized) {
      // Note - we have to set these in update, because during load the team is
      // set, and we don't want to award initial bonus on load.  jba :)
    ...
    if (getObject()->isNeutralControlled() ||
        getAutoDepositUpdateModuleData()->m_depositAmount <= 0)
    ...
    getObject()->getControllingPlayer()->getMoney()->deposit(
        getAutoDepositUpdateModuleData()->m_depositAmount);
    ...
    if (getAutoDepositUpdateModuleData()->m_depositAmount > 0) {
      ...
      pos.set(getObject()->getPosition());
      pos.z += 10.0f;  // add a little z to make it show up above the unit.
      Color color = getObject()->getControllingPlayer()->getPlayerColor() |
