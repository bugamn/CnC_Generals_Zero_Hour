...
//-------------------------------------------------------------------------------------------------
/*static*/ void CleanupHazardUpdateModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  ModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"WeaponSlot", INI::parseLookupList, TheWeaponSlotTypeNamesLookupList,
       offsetof(CleanupHazardUpdateModuleData, m_weaponSlot)},
      {"ScanRate", INI::parseDurationUnsignedInt, NULL,
       offsetof(CleanupHazardUpdateModuleData, m_scanFrames)},
      {"ScanRange", INI::parseReal, NULL,
       offsetof(CleanupHazardUpdateModuleData, m_scanRange)},
      {0, 0, 0, 0}};
  p.add(dataFieldParse);
}

//-------------------------------------------------------------------------------------------------
CleanupHazardUpdate::CleanupHazardUpdate(Thing *thing,
...
void CleanupHazardUpdate::onObjectCreated() {
  const CleanupHazardUpdateModuleData *data =
      getCleanupHazardUpdateModuleData();
  ...
  if (!weapon) {
    DEBUG_CRASH(
        ("CleanupHazardUpdate for %s doesn't have a valid weapon template",
  ...
  if (data->m_scanRange <= attackRange) {
    DEBUG_CRASH(
        ("CleanupHazardUpdate for %s requires the scan range (%.1f) being "
         "larger than the firing range (%.1f)",
         getObject()->getTemplate()->getName().str(), data->m_scanRange,
         attackRange));
  }
}
...
UpdateSleepTime CleanupHazardUpdate::update() {
  ...
  // Make sure we are "busy" for scripting purposes if the unit is cleaning up
  // an area.
  if (m_moveRange > 0.0f) {
    ...
    if (ai) {
      if (ai->isIdle()) {
        ...
        // Either the player or a script gave a NEW order so abandon the cleanup
        // area cause.
        m_moveRange = 0.0f;
  ...
  if (scanClosestTarget()) {
    ...
    // immediately in range, set the AI to idle so it can advance to the next
    // script!
    AIUpdateInterface *ai = obj->getAI();
    if (ai && (ai->isIdle() || ai->isBusy())) {
      Real fDist = sqrt(
          ThePartitionManager->getDistanceSquared(obj, &m_pos, FROM_CENTER_2D));
      if (fDist < 25.0f) {
...
void CleanupHazardUpdate::fireWhenReady() {
  ...
  if (target && m_moveRange == 0.0f) {
    ...
    Object *me = getObject();
    Real fDist = sqrt(
        ThePartitionManager->getDistanceSquared(me, target, FROM_CENTER_2D));
    if (fDist < fireRange) {
      // We are currently in range!
      m_inRange = true;
    } else {
      if (m_inRange) {
        // We were in range last frame, but the target has moved out of firing
        ...
        if (!m_nextScanFrames) {
          scanClosestTarget();
          m_nextScanFrames = data->m_scanFrames;
          target = NULL;  // Set target to NULL so we don't shoot at it (might
                          // be out of range)
        }
      } else {
        // Not in range
...
Object *CleanupHazardUpdate::scanClosestTarget() {
  ...
  PartitionFilterAcceptByKindOf kindFilter(
      MAKE_KINDOF_MASK(KINDOF_CLEANUP_HAZARD), KINDOFMASK_NONE);
  PartitionFilterSameMapStatus filterMapStatus(getObject());
  PartitionFilter *filters[] = {&kindFilter, &filterMapStatus, NULL};

  if (m_moveRange > 0.0f) {
    // Look for targets around the target position only (but add scan range and
    // move range). This case only happens when we are performing a cleanup area
    // command.
    bestTargetInRange = ThePartitionManager->getClosestObject(
        &m_pos, data->m_scanRange + m_moveRange, FROM_CENTER_2D, filters);
  } else {
    // Look for targets near me -- passive default.
    bestTargetInRange = ThePartitionManager->getClosestObject(
        me->getPosition(), data->m_scanRange, FROM_CENTER_2D, filters);
  }
...
//-------------------------------------------------------------------------------------------------
void CleanupHazardUpdate::setCleanupAreaParameters(const Coord3D *pos,
                                                   Real range) {
  Object *obj = getObject();
  AIUpdateInterface *ai = obj->getAI();

  // Setting the move range triggers that passive conditions for it to be
  // allowed to move from the specified position.
  m_moveRange = range;
  m_pos = *pos;

  if (ai) {
    // CMD_FROM_AI important because it'll abort when other types are used --
    // like if a player or script orders the unit to do something else, we need
    // a way to cancel this passive situation.
