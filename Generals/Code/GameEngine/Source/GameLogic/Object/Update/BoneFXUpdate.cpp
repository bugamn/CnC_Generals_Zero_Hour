...
void BoneFXUpdate::onObjectCreated() {
  ...
  BoneFXDamage *bfxd =
      (BoneFXDamage *)getObject()->findDamageModule(key_BoneFXDamage);
  if (bfxd == NULL) {
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
BoneFXUpdate::~BoneFXUpdate(void) { killRunningParticleSystems(); }
...
//-------------------------------------------------------------------------------------------------
/** Parse a random delay.  This is a number pair, where the numbers are a min
 * and max time in miliseconds. */
//-------------------------------------------------------------------------------------------------
static void parseGameClientRandomDelay(INI *ini, void *instance,
                                       GameClientRandomVariable *delay) {
  Real min, max;
  INI::parseDurationReal(ini, instance, &min, NULL);
  INI::parseDurationReal(ini, instance, &max, NULL);

  delay->setRange(min, max,
                  GameClientRandomVariable::DistributionType::UNIFORM);
}

static void parseGameLogicRandomDelay(INI *ini, void *instance,
...
//-------------------------------------------------------------------------------------------------
/** In the form of:
 * <BodyDamageState>FXList<index> = Bone:<BoneName> OnlyOnce:<Yes|No> <Min
 * delay> <Max delay> FXList:<FXListName> */
//-------------------------------------------------------------------------------------------------
void BoneFXUpdateModuleData::parseFXList(INI *ini, void *instance, void *store,
                                         const void *userData) {
  const char *token;
...
//-------------------------------------------------------------------------------------------------
/** In the form of:
 * <BodyDamageState>OCL<index> = Bone:<BoneName> OnlyOnce:<Yes|No> <Min delay>
 * <Max delay> OCL:<OCLName> */
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
/** In the form of:
 * <BodyDamageState>ParticleSystem<index> = <Bone:BoneName> OnlyOnce:<Yes|No>
 * <Min delay> <Max delay> PSys:<PSysName> */
//-------------------------------------------------------------------------------------------------
void BoneFXUpdateModuleData::parseParticleSystem(INI *ini, void *instance,
  ...
  ini->parseParticleSystemTemplate(ini, instance, &info->particleSysTemplate,
                                   NULL);
...
UpdateSleepTime BoneFXUpdate::update(void) {
  ...
  for (Int i = 0; i < BONE_FX_MAX_BONES; ++i) {
    ...
    if ((m_nextFXFrame[m_curBodyState][i] != -1) &&
        (m_nextFXFrame[m_curBodyState][i] <= now)) {
      doFXListAtBone(d->m_fxList[m_curBodyState][i].fx,
                     &(m_FXBonePositions[m_curBodyState][i]));
      computeNextLogicFXTime(&(d->m_fxList[m_curBodyState][i]),
                             m_nextFXFrame[m_curBodyState][i]);
    }
    if ((m_nextOCLFrame[m_curBodyState][i] != -1) &&
        (m_nextOCLFrame[m_curBodyState][i] <= now)) {
      doOCLAtBone(d->m_OCL[m_curBodyState][i].ocl,
                  &(m_OCLBonePositions[m_curBodyState][i]));
      computeNextLogicFXTime(&(d->m_OCL[m_curBodyState][i]),
                             m_nextOCLFrame[m_curBodyState][i]);
    }
    if ((m_nextParticleSystemFrame[m_curBodyState][i] != -1) &&
        (m_nextParticleSystemFrame[m_curBodyState][i] <= now)) {
      doParticleSystemAtBone(
          d->m_particleSystem[m_curBodyState][i].particleSysTemplate,
          &(m_PSBonePositions[m_curBodyState][i]));
      computeNextClientFXTime(&(d->m_particleSystem[m_curBodyState][i]),
                              m_nextParticleSystemFrame[m_curBodyState][i]);
    }
  }
...
void BoneFXUpdate::initTimes() {
  ...
  for (i = 0; i < BONE_FX_MAX_BONES; ++i) {
    if (d->m_fxList[m_curBodyState][i].locInfo.boneName.compare(
            AsciiString::TheEmptyString) != 0) {
      m_nextFXFrame[m_curBodyState][i] =
          now +
          REAL_TO_INT(d->m_fxList[m_curBodyState][i].gameLogicDelay.getValue());
    } else {
      m_nextFXFrame[m_curBodyState][i] = -1;
    }
    if (d->m_OCL[m_curBodyState][i].locInfo.boneName.compare(
            AsciiString::TheEmptyString) != 0) {
      m_nextOCLFrame[m_curBodyState][i] =
          now +
          REAL_TO_INT(d->m_OCL[m_curBodyState][i].gameLogicDelay.getValue());
    } else {
      m_nextOCLFrame[m_curBodyState][i] = -1;
    }
    if (d->m_particleSystem[m_curBodyState][i].locInfo.boneName.compare(
            AsciiString::TheEmptyString) != 0) {
      m_nextParticleSystemFrame[m_curBodyState][i] =
          now + REAL_TO_INT(d->m_particleSystem[m_curBodyState][i]
                                .gameClientDelay.getValue());
    } else {
...
void BoneFXUpdate::doFXListAtBone(const FXList *fxList,
  ...
  // Convert the bone's position relative to the origin of the building to the
  // current bone position in the world.
  Coord3D newPos;
...
void BoneFXUpdate::doOCLAtBone(const ObjectCreationList *ocl,
  ...
  const DamageInfo *lastDamageInfo =
      getObject()->getBodyModule()->getLastDamageInfo();
  if (lastDamageInfo &&
      getDamageTypeFlag(d->m_damageOCLTypes, lastDamageInfo->in.m_damageType) ==
          FALSE)
    return;
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void BoneFXUpdate::doParticleSystemAtBone(
  ...
  const DamageInfo *lastDamageInfo =
      getObject()->getBodyModule()->getLastDamageInfo();
  if (lastDamageInfo &&
      getDamageTypeFlag(d->m_damageParticleTypes,
                        lastDamageInfo->in.m_damageType) == FALSE)
    return;

  Object *building = getObject();

  ParticleSystem *psys =
      TheParticleSystemManager->createParticleSystem(particleSystemTemplate);
  if (psys != NULL) {
...
void BoneFXUpdate::computeNextClientFXTime(const BaseBoneListInfo *info,
  ...
  nextFrame =
      TheGameLogic->getFrame() + REAL_TO_INT(info->gameClientDelay.getValue());
}
...
void BoneFXUpdate::computeNextLogicFXTime(const BaseBoneListInfo *info,
  ...
  nextFrame =
      TheGameLogic->getFrame() + REAL_TO_INT(info->gameLogicDelay.getValue());
}

//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void BoneFXUpdate::killRunningParticleSystems() {
  for (std::vector<ParticleSystemID>::iterator it = m_particleSystemIDs.begin();
...
void BoneFXUpdate::resolveBoneLocations() {
  ...
  for (i = 0; i < BONE_FX_MAX_BONES; ++i) {
    if (d->m_fxList[m_curBodyState][i].locInfo.boneName.compare(
      ...
      drawable->getPristineBonePositions(info->locInfo.boneName.str(), 0,
                                         &m_FXBonePositions[m_curBodyState][i],
                                         NULL, 1);
    }
    ...
    if (d->m_particleSystem[m_curBodyState][i].locInfo.boneName.compare(
            AsciiString::TheEmptyString) != 0) {
      const BoneParticleSystemInfo *info =
          &(d->m_particleSystem[m_curBodyState][i]);
      drawable->getPristineBonePositions(info->locInfo.boneName.str(), 0,
                                         &m_PSBonePositions[m_curBodyState][i],
                                         NULL, 1);
    }
...
void BoneFXUpdate::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (it = m_particleSystemIDs.begin(); it != m_particleSystemIDs.end();
         ++it) {
      systemID = *it;
    ...
    if (m_particleSystemIDs.empty() == FALSE) {
      DEBUG_CRASH(
          ("BoneFXUpdate::xfer - m_particleSystemIDs should be empty but is "
           "not\n"));
  ...
  // next fx frame
  xfer->xferUser(m_nextFXFrame,
                 sizeof(Int) * BODYDAMAGETYPE_COUNT * BONE_FX_MAX_BONES);

  // next OCL farme
  xfer->xferUser(m_nextOCLFrame,
                 sizeof(Int) * BODYDAMAGETYPE_COUNT * BONE_FX_MAX_BONES);

  // next particle system frame
  xfer->xferUser(m_nextParticleSystemFrame,
                 sizeof(Int) * BODYDAMAGETYPE_COUNT * BONE_FX_MAX_BONES);

  // fx bone positions
  xfer->xferUser(m_FXBonePositions,
                 sizeof(Coord3D) * BODYDAMAGETYPE_COUNT * BONE_FX_MAX_BONES);

  // ocl bone positions
  xfer->xferUser(m_OCLBonePositions,
                 sizeof(Coord3D) * BODYDAMAGETYPE_COUNT * BONE_FX_MAX_BONES);

  // particle system bone positions
  xfer->xferUser(m_PSBonePositions,
                 sizeof(Coord3D) * BODYDAMAGETYPE_COUNT * BONE_FX_MAX_BONES);
