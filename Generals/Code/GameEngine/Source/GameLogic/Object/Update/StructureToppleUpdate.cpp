...
StructureToppleUpdate::StructureToppleUpdate(Thing *thing,
  ...
  m_accumulatedAngle =
      0.001f;  // Need to give it a little nudge in the right direction
  m_lastCrushedLocation = 0.0f;
  m_nextBurstFrame = -1;
  setWakeFrame(getObject(), UPDATE_SLEEP_FOREVER);

  // Get the extent height here, rather than after it dies -- when it switches
  // to dead state the rubble state has a tiny height.
...
//-------------------------------------------------------------------------------------------------
static void parseOCL(INI *ini, void *instance, void * /*store*/,
                     const void * /*userData*/) {
  StructureToppleUpdateModuleData *self =
      (StructureToppleUpdateModuleData *)instance;
  StructureTopplePhaseType stphase =
      (StructureTopplePhaseType)INI::scanIndexList(
          ini->getNextToken(), TheStructureTopplePhaseNames);
  for (const char *token = ini->getNextToken(); token != NULL;
    ...
    const ObjectCreationList *ocl =
        TheObjectCreationListStore->findObjectCreationList(
            token);  // could be null! this is OK!
    self->m_ocls[stphase].push_back(ocl);
  }
}

//-------------------------------------------------------------------------------------------------
static void parseAngleFX(INI *ini, void *instance, void * /* store */,
                         const void * /*userData*/) {
  StructureToppleUpdateModuleData *self =
      (StructureToppleUpdateModuleData *)instance;
  AngleFXInfo info;
...
//-------------------------------------------------------------------------------------------------
/*static*/ void StructureToppleUpdateModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  UpdateModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"MinToppleDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(StructureToppleUpdateModuleData, m_minToppleDelay)},
      {"MaxToppleDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(StructureToppleUpdateModuleData, m_maxToppleDelay)},
      {"MinToppleBurstDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(StructureToppleUpdateModuleData, m_minToppleBurstDelay)},
      {"MaxToppleBurstDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(StructureToppleUpdateModuleData, m_maxToppleBurstDelay)},
      {"StructuralIntegrity", INI::parseReal, NULL,
       offsetof(StructureToppleUpdateModuleData, m_structuralIntegrity)},
      {"StructuralDecay", INI::parseReal, NULL,
       offsetof(StructureToppleUpdateModuleData, m_structuralDecay)},
      {"DamageFXTypes", INI::parseDamageTypeFlags, NULL,
       offsetof(StructureToppleUpdateModuleData, m_damageFXTypes)},
      {"TopplingFX", INI::parseFXList, NULL,
       offsetof(StructureToppleUpdateModuleData, m_toppleFXList)},
      {"ToppleDelayFX", INI::parseFXList, NULL,
       offsetof(StructureToppleUpdateModuleData, m_toppleDelayFXList)},
      {"ToppleStartFX", INI::parseFXList, NULL,
       offsetof(StructureToppleUpdateModuleData, m_toppleStartFXList)},
      {"ToppleDoneFX", INI::parseFXList, NULL,
       offsetof(StructureToppleUpdateModuleData, m_toppleDoneFXList)},
      {"CrushingFX", INI::parseFXList, NULL,
       offsetof(StructureToppleUpdateModuleData, m_crushingFXList)},
      {"CrushingWeaponName", INI::parseAsciiString, NULL,
       offsetof(StructureToppleUpdateModuleData, m_crushingWeaponName)},
      {"OCL", parseOCL, NULL, 0},
      {"AngleFX", parseAngleFX, NULL, 0},
      {0, 0, 0, 0}};
  p.add(dataFieldParse);
  p.add(DieMuxData::getFieldParse(),
        offsetof(StructureToppleUpdateModuleData, m_dieMuxData));
}

//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void StructureToppleUpdate::beginStructureTopple(const DamageInfo *damageInfo) {
  const StructureToppleUpdateModuleData *d =
      getStructureToppleUpdateModuleData();

  if (d) {
    UnsignedInt now = TheGameLogic->getFrame();
    m_toppleFrame =
        now + GameLogicRandomValue(d->m_minToppleDelay, d->m_maxToppleDelay);
    ...
    if (attacker == NULL) {
      ...
      // Calculate the topple direction to be the opposite of the direction
      // fired from.
    ...
    Real averageRadius = (building->getGeometryInfo().getMajorRadius() +
                          building->getGeometryInfo().getMinorRadius()) /
                         2;
    Real explosionRadius = averageRadius * 0.90;

    m_delayBurstLocation.x =
        building->getPosition()->x + explosionRadius * Cos(toppleAngle);
    m_delayBurstLocation.y =
        building->getPosition()->y + explosionRadius * Sin(toppleAngle);
    m_delayBurstLocation.z = TheTerrainLogic->getGroundHeight(
        m_delayBurstLocation.x, m_delayBurstLocation.y);

    doToppleStartFX(building, damageInfo);
    m_nextBurstFrame = now + GameClientRandomValue(d->m_minToppleBurstDelay,
                                                   d->m_maxToppleBurstDelay);
...
void StructureToppleUpdate::onDie(const DamageInfo *damageInfo) {
  const StructureToppleUpdateModuleData *d =
      getStructureToppleUpdateModuleData();
  if (!d->m_dieMuxData.isDieApplicable(getObject(), damageInfo)) return;
...
UpdateSleepTime StructureToppleUpdate::update(void) {
  ...
  if (m_toppleState == TOPPLESTATE_WAITINGFORTOPPLESTART) {
    ...
    if (now >= m_nextBurstFrame) {
      ...
      // This uses a game client random value because the delay bursts are
      // purely visual and aural effects.
      m_nextBurstFrame = now + GameClientRandomValue(d->m_minToppleBurstDelay,
                                                     d->m_maxToppleBurstDelay);
    }
  ...
  if (m_toppleState == TOPPLESTATE_TOPPLING) {
    ...
    Real toppleAcceleration =
        TOPPLE_ACCELERATION_FACTOR *
        (Sin(m_accumulatedAngle) * (1.0 - m_structuralIntegrity));
    ...
    if (m_accumulatedAngle >= PI / 2) {
      ...
      if (lastDamageInfo == NULL ||
          getDamageTypeFlag(d->m_damageFXTypes,
                            lastDamageInfo->in.m_damageType))
  ...
  // The building is now flat on the ground and done with all the crushing and
  // all that.
...
void StructureToppleUpdate::doToppleDoneStuff() {
  ...
  BoneFXUpdate *bfxu =
      (BoneFXUpdate *)getObject()->findUpdateModule(key_BoneFXUpdate);
  if (bfxu != NULL) {
    bfxu->stopAllBoneFX();
  }
...
void StructureToppleUpdate::doAngleFX(Real curAngle, Real newAngle) {
  ...
  for (std::vector<AngleFXInfo>::const_iterator it = d->angleFX.begin();
       it != d->angleFX.end(); ++it) {
...
void StructureToppleUpdate::applyCrushingDamage(Real theta) {
  //	static const Real THETA_CEILING = PI/8; // This weapon won't do any
  //damage until theta is less than this value.
  static const Real THETA_CEILING =
      PI / 6;  // This weapon won't do any damage until theta is less than this
               // value.
  static const Real WEAPON_SPACING_PERPENDICULAR =
      25;  // The spacing between weapon firing locations,
  ...
  // Figure out the width of the projection of the boundary of the building
  // along the topple direction. Do this because the amount of ground that is
  // affected will be different if the building falls in different orientations.
  Real angle = orientationAngle - toppleAngle;
  Real minorComponent =
      building->getGeometryInfo().getMinorRadius() * Cos(angle);
  Real majorComponent =
      building->getGeometryInfo().getMajorRadius() * Sin(angle);
  ...
  const WeaponTemplate *wt =
      TheWeaponStore->findWeaponTemplate(d->m_crushingWeaponName);
  if (wt == NULL) {
    return;
  }
  ...
  /*
   * Fire explosions at regular intervals across the area that the building is
   * currently crushing.  The explosions occur across the face and along the
   * length of the building.
   */
  Real jcos;
  Real jsin;
  //	Coord3D target;
  for (Real j = m_lastCrushedLocation; j < maxDistance;
...
void StructureToppleUpdate::doDamageLine(Object *building,
                                         const WeaponTemplate *wt, Real jcos,
                                         Real jsin, Real facingWidth,
                                         Real toppleAngle) {
  const DamageInfo *lastDamageInfo =
      getObject()->getBodyModule()->getLastDamageInfo();
  static const Real WEAPON_SPACING_PARALLEL =
      25;  // The spacing between weapon firing locations,
  ...
  for (Real i = -facingWidth; i < facingWidth; i += WEAPON_SPACING_PARALLEL) {
    ...
    if (lastDamageInfo == NULL ||
        getDamageTypeFlag(d->m_damageFXTypes, lastDamageInfo->in.m_damageType))
      FXList::doFXPos(d->m_crushingFXList, &target);
  }

  // Make sure there are weapons fired and FX done on the edge of the building.
  target.x =
      building->getPosition()->x + jcos + (facingWidth * Sin(toppleAngle));
  target.y =
      building->getPosition()->y + jsin + (facingWidth * Cos(toppleAngle));
  target.z = TheTerrainLogic->getGroundHeight(target.x, target.y);
...
void StructureToppleUpdate::doToppleDelayBurstFX() {
  ...
  if (lastDamageInfo == NULL ||
      getDamageTypeFlag(d->m_damageFXTypes, lastDamageInfo->in.m_damageType)) {
    for (std::vector<FXBoneInfo>::const_iterator it = d->fxbones.begin();
         it != d->fxbones.end(); ++it) {
      ParticleSystem *sys = TheParticleSystemManager->createParticleSystem(
          it->particleSystemTemplate);
      if (sys != NULL) {
        Coord3D pos;
        if (drawable->getPristineBonePositions(it->boneName.str(), 0, &pos,
                                               NULL, 1) == 1) {
...
inline Bool inList(Int value, Int count, const Int idxList[]) {
  for (Int j = 0; j < count; ++j) {
    if (idxList[j] == value) return true;
  }
  return false;
}
