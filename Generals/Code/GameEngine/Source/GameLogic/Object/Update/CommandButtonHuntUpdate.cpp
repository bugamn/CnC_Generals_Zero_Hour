...
// Desc:   Update module to handle "Hunting" using a special power.  If the unit
// is idle, and the power is not active, it targets a new unit with the power.
// Note that this is an update rather than an AI state because many of the
// special abilities use the ai to perform portions of the special ability.
///////////////////////////////////////////////////////////////////////////////////////////////////
...
//-------------------------------------------------------------------------------------------------
/*static*/ void CommandButtonHuntUpdateModuleData::buildFieldParse(
  ...
  static const FieldParse dataFieldParse[] = {
      ...
      {"ScanRange", INI::parseReal, NULL,
       offsetof(CommandButtonHuntUpdateModuleData, m_scanRange)},
      {0, 0, 0, 0}};
...
CommandButtonHuntUpdate::CommandButtonHuntUpdate(Thing *thing,
    ...
    : UpdateModule(thing, moduleData), m_commandButton(NULL) {
...
UpdateSleepTime CommandButtonHuntUpdate::update() {
  ...
  if (ai->getLastCommandSource() != CMD_FROM_AI) {
    // If a script or the player (in this case should only be script, but either
    // way) we quit hunting.
    m_commandButton = NULL;
...
//-------------------------------------------------------------------------------------------------
UpdateSleepTime CommandButtonHuntUpdate::huntSpecialPower(
    AIUpdateInterface *ai) {
  Object *obj = getObject();
  const CommandButtonHuntUpdateModuleData *data =
      getCommandButtonHuntUpdateModuleData();
  if (!ai->isIdle()) {
...
Object *CommandButtonHuntUpdate::scanClosestTarget(void) {
  ...
  PartitionFilterRelationship filterTeam(
      me, PartitionFilterRelationship::ALLOW_ENEMIES);
  ...
  Bool isBlackLotusVehicleHack = (spTemplate->getSpecialPowerType() ==
                                  SPECIAL_BLACKLOTUS_DISABLE_VEHICLE_HACK);
  Bool isCaptureBuilding =
      (spTemplate->getSpecialPowerType() == SPECIAL_INFANTRY_CAPTURE_BUILDING);
  if (isCaptureBuilding) {
    filters[2] = NULL;  // It's ok (in fact necessary for oil derricks) to
                        // capture special buildings.
  }
  ...
  if (mod) {
    for (Object *other = iter->first(); other; other = iter->next()) {
      ...
      if (TheActionManager->canDoSpecialPowerAtObject(me, other, CMD_FROM_AI,
                                                      spTemplate, 0)) {
        if (isPlaceExplosive) {
          ...
          PartitionFilterSamePlayer filterPlayer(
              me->getControllingPlayer());  // Look for our own mines.
          ...
          Object *mine = ThePartitionManager->getClosestObject(
              other, range, FROM_BOUNDINGSPHERE_2D,
        ...
        Real distSqr = ThePartitionManager->getDistanceSquared(
            me, other, FROM_BOUNDINGSPHERE_2D);
...
void CommandButtonHuntUpdate::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_LOAD) {
    ...
    if (m_commandButtonName.isEmpty() == FALSE) {
      Object *us = getObject();
      const CommandSet *commandSet =
          TheControlBar->findCommandSet(us->getCommandSetString());
