...
void RadarUpgradeModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"DisableProof", INI::parseBool, NULL,
       offsetof(RadarUpgradeModuleData, m_isDisableProof)},
      {0, 0, 0, 0}};
...
RadarUpgrade::RadarUpgrade(Thing *thing, const ModuleData *moduleData)
    : UpgradeModule(thing, moduleData) {}  // end RadarUpgrade

//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
RadarUpgrade::~RadarUpgrade(void) {}  // end ~RadarUpgrade
...
void RadarUpgrade::onDelete(void) {
  ...
  // If we're currently disabled, we shouldn't do anything, because we've
  // already done it.
  if (getObject()->isDisabled()) return;

  // remove the radar from the player
  Player *player = getObject()->getControllingPlayer();
  if (player) player->removeRadar(md->m_isDisableProof);
...
void RadarUpgrade::upgradeImplementation(void) {
  ...
  RadarUpdate *radarUpdate =
      (RadarUpdate *)getObject()->findUpdateModule(radarUpdateKey);
