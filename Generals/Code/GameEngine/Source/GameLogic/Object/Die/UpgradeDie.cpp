...
// Desc:  When object dies, the parent object is freed of the specified object
// upgrade field.
///////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if (producer) {
    ...
    if (upgrade) {
      ...
      if (producer->hasUpgrade(upgrade)) {
        ...
        DEBUG_ASSERTCRASH(0,
                          ("Object %s just died, but is trying to free upgrade "
                           "%s in it's producer %s%s",
                           getObject()->getTemplate()->getName().str(),
                           getUpgradeDieModuleData()->m_upgradeName.str(),
                           producer->getTemplate()->getName().str(),
                           ", which the producer doesn't have. This is used in "
                           "cases where the producer builds an upgrade that "
                           "can die... like ranger building scout drones."));
      }
    }
  }
}
