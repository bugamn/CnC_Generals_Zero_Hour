...
//-------------------------------------------------------------------------------------------------
// Figure out which crush point was hit so the correct crushed object can be
// swapped in

// Figure out which crush point was hit so the correct crushed object can be
// swapped in
static CrushEnum crushLocationCheck(Object *crusherObject,
                                    Object *victimObject) {
  if ((crusherObject == NULL) || (victimObject == NULL)) return NO_CRUSH;
  ...
  Real crushPointOffsetDistance =
      victimObject->getGeometryInfo().getMajorRadius() * 0.5;
  ...
  // waited until we crossed that point so at this point we just need to know
  // which crush point is closest.
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
CrushDie::CrushDie(Thing *thing, const ModuleData *moduleData)
...
void CrushDie::onDie(const DamageInfo *damageInfo) {
  ...
  DEBUG_ASSERTCRASH(damageInfo->in.m_damageType == DAMAGE_CRUSH,
                    ("this should only be used for crush damage\n"));
  if (damageInfo->in.m_damageType != DAMAGE_CRUSH) return;

  Object *damageDealer =
      TheGameLogic->findObjectByID(damageInfo->in.m_sourceID);
  DEBUG_ASSERTCRASH(damageDealer,
                    ("You must have a damageDealer source for this effect"));

  CrushEnum crushType = damageDealer
                            ? crushLocationCheck(damageDealer, getObject())
                            : TOTAL_CRUSH;

  if (crushType != NO_CRUSH) {
    if (getCrushDieModuleData()
            ->m_crushSounds[crushType]
            .getEventName()
            .isEmpty() == false) {
      ...
      if (GameLogicRandomValue(0, 99) <
          getCrushDieModuleData()->m_crushSoundPercent[crushType]) {
        AudioEventRTS crushSound(
            getCrushDieModuleData()->m_crushSounds[crushType]);
    ...
    }
    {
      ...
      if (me) {
        me->getBodyModule()->setFrontCrushed(crushType == TOTAL_CRUSH ||
                                             crushType == FRONT_END_CRUSH);
        me->getBodyModule()->setBackCrushed(crushType == TOTAL_CRUSH ||
                                            crushType == BACK_END_CRUSH);

        ModelConditionFlags newCrushed;
        newCrushed.set(
            MODELCONDITION_FRONTCRUSHED,
            (crushType == TOTAL_CRUSH || crushType == FRONT_END_CRUSH));
        newCrushed.set(MODELCONDITION_BACKCRUSHED,
                       crushType == TOTAL_CRUSH || crushType == BACK_END_CRUSH);

        me->getDrawable()->clearAndSetModelConditionFlags(
            MAKE_MODELCONDITION_MASK2(MODELCONDITION_BACKCRUSHED,
                                      MODELCONDITION_FRONTCRUSHED),
            newCrushed);
      }
    }
