...
// Desc:   A chance to create a crate on death according to certain condition
// checks
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void CreateCrateDieModuleData::parseCrateData(INI *ini, void *instance,
                                              void * /*store*/,
                                              const void * /*userData*/) {
...
void CreateCrateDie::onDie(const DamageInfo *damageInfo) {
  ...
  for (AsciiStringListConstIterator iter =
    ...
    if (currentCrateData) {
      if (!testCreationChance(currentCrateData)) continue;  // always test this

      if ((currentCrateData->m_veterancyLevel != LEVEL_INVALID) &&
          !testVeterancyLevel(currentCrateData))
        continue;  // If this is set up to test and it fails

      if (KINDOFMASK_ANY_SET(currentCrateData->m_killedByTypeKindof) &&
      ...
      if (crate) {
        ...
        if (killer) {
          // If the killer is a computer controlled player, notify that the
          // crate exists.
          if (killer->getControllingPlayer() &&
              killer->getControllingPlayer()->getPlayerType() ==
                  PLAYER_COMPUTER) {
...
Bool CreateCrateDie::testKillerType(CrateTemplate const *currentCrateData,
                                    Object *killer) {
  if (killer == NULL) return FALSE;

  // Must match the whole group of bits set in the KilledBy description (most
  // likely One).
  if (!killer->getTemplate()->isKindOfMulti(
          currentCrateData->m_killedByTypeKindof, KINDOFMASK_NONE))
    return FALSE;

  return TRUE;
}

Bool CreateCrateDie::testKillerScience(CrateTemplate const *currentCrateData,
                                       Object *killer) {
  if (killer == NULL) return FALSE;
...
Object *CreateCrateDie::createCrate(CrateTemplate const *currentCrateData) {
  ...
  // CreationChance is used for the success of this block, but this block can
  // have any number of potential actual crates
  Real multipleCratePick = GameLogicRandomValueReal(0, 1);
  Real multipleCrateRunningTotal = 0;
  AsciiString crateName = "";

  for (crateCreationEntryConstIterator iter =
           currentCrateData->m_possibleCrates.begin();
       iter != currentCrateData->m_possibleCrates.end(); iter++) {
    multipleCrateRunningTotal += (*iter).crateChance;
    if (multipleCrateRunningTotal > multipleCratePick) {
      // Run through the list of possibles, and if the sum of the chances is
      // greater than my random pick, then this is the correct one.  (This
      // simulates contiguous %, or weighted distribution)
      crateName = (*iter).crateName;
      break;
    }
  }
  // At this point, I could very well have a "" for the type, if the Designer
  // didn't make the sum of chances 1

  ThingTemplate const *crateType = TheThingFactory->findTemplate(crateName);
  if (crateType == NULL) return NULL;
  ...
  fpOptions.flags =
      FPF_IGNORE_ALLY_OR_NEUTRAL_UNITS;  // So the dead guy won't block, nor
                                         // will his dead hulk.
  if (layer != LAYER_GROUND) {
    creationPoint = centerPoint;
    spotFound = true;
  } else if (ThePartitionManager->findPositionAround(&centerPoint, &fpOptions,
                                                     &creationPoint)) {
    spotFound = TRUE;
  } else {
    // If the tight ignore units scan fails, then try a great big scan so we
    // appear on the edge of the large dead thing (building rubble)
    fpOptions.minRadius = 0.0f;
    fpOptions.maxRadius = 125.0f;
    fpOptions.relationshipObject = NULL;
    fpOptions.flags = FPF_NONE;
    if (ThePartitionManager->findPositionAround(&centerPoint, &fpOptions,
                                                &creationPoint)) {
      spotFound = TRUE;
    }
  }
  ...
  if (spotFound) {
    ...
    if (crateDrawable) {
      ...
      crateDrawable->setTerrainDecalSize(
