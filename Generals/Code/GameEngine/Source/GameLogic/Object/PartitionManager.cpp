...
//					  objects in space, iterate objects in
//specified volumes, 					  regions, by types and other properties.
//
//-----------------------------------------------------------------------------
...
extern void addIcon(const Coord3D *pos, Real width, Int numFramesDuration,
                    RGBColor color);

const Real HUGE_DIST_SQR = (HUGE_DIST * HUGE_DIST);

#define DISABLE_INVALID_PREVENTION  // Steven, I had to turn this off because it
                                    // was causing problem with map border
                                    // resizing (USA04). -MW
...
/*
        Notes:
        -- collideLoc and collideNormal will only be filled in if result is
   true; they are undefined if result is false.
        ...
        -- collideLoc is not guaranteed to lie precisely on either 'a' or 'b'!
                        (this matters iff the objects are overlapping and thus
   have multiple/ambiguous collision points; we pick an 'in-between' spot to
   simplify matters, such that the same spot will be used for the collision in
   both directions)
        -- collideNormal is the normal the collision surface at 'a' at
   collideLoc. (thus the normal for b is exactly opposite in direction)
        -- collideNormal is guaranteed to be a unit vector
*/
typedef Bool (*CollideTestProc)(const CollideInfo *a, const CollideInfo *b,
                                CollideLocAndNormal *cinfo);

// if the dist is greater than maxDist, return false, and the output stuff is
// undefined.
typedef Bool (*DistCalcProc)(const Coord3D *posA, const Object *objA,
                             const Coord3D *posB, const Object *objB,
                             Real &abDistSqr, Coord3D &abVec, Real maxDistSqr);
    ...
    if (!(*fp)->allow(objOther)) {
      if (DoFilterProfiling) {
        ...
        for (idx = 0; idx < maxEver; ++idx)
          if (namesarr[idx] == (*fp)->debugGetName()) break;
        if (idx == maxEver) {
          namesarr[maxEver++] = (*fp)->debugGetName();
          DEBUG_ASSERTCRASH(maxEver < MAXR, ("hmm, enlarge this array"));
        }
        ++rejections[idx];
        if (allow) ++usefulRejections[idx];
        allow = false;
        if (maxEver < idx) maxEver = idx;
      } else {
        return false;
      }
    }
  ...
  if (DoFilterProfiling && calls > 0 && calls % 1000 == 0) {
    ...
    for (idx = 0; idx < maxEver; idx++) {
      DEBUG_LOG(("rejections[%s] = %d (useful = %d)\n", names[idx],
                 rejections[idx], usefulRejections[idx]));
    }
  }
...
//-----------------------------------------------------------------------------
inline void vecDiff_2D(const Coord3D *posA, const Coord3D *posB,
                       Coord3D *resultVec) {
...
//-----------------------------------------------------------------------------
inline void vecDiff_3D(const Coord3D *posA, const Coord3D *posB,
                       Coord3D *resultVec) {
  DEBUG_ASSERTCRASH(posA && posB && resultVec, ("null parm"));
  resultVec->x = posA->x - posB->x;
  resultVec->y = posA->y - posB->y;
...
static void rectToFourPoints(const CollideInfo *a,  // z is ignored
                             Coord2D pts[]);
static void testRotatedPointsAgainstRect(const Coord2D *pts,  // an array of 4
                                         const CollideInfo *a, Coord2D *avg,
                                         Int *avgTot);

static Bool xy_collideTest_Rect_Rect(const CollideInfo *a, const CollideInfo *b,
                                     CollideLocAndNormal *cinfo);
static Bool xy_collideTest_Rect_Circle(const CollideInfo *a,
                                       const CollideInfo *b,
                                       CollideLocAndNormal *cinfo);
static Bool xy_collideTest_Circle_Rect(const CollideInfo *a,
                                       const CollideInfo *b,
                                       CollideLocAndNormal *cinfo);
static Bool xy_collideTest_Circle_Circle(const Coord3D *a_pos,
                                         const Coord3D *b_pos,
                                         const Real a_radius,
                                         const Real b_radius,
                                         CollideLocAndNormal *cinfo);

static Bool collideTest_Sphere_Sphere(const CollideInfo *a,
                                      const CollideInfo *b,
                                      CollideLocAndNormal *cinfo);
static Bool collideTest_Sphere_Cylinder(const CollideInfo *a,
                                        const CollideInfo *b,
                                        CollideLocAndNormal *cinfo);
static Bool collideTest_Sphere_Box(const CollideInfo *a, const CollideInfo *b,
                                   CollideLocAndNormal *cinfo);
static Bool collideTest_Cylinder_Sphere(const CollideInfo *a,
                                        const CollideInfo *b,
                                        CollideLocAndNormal *cinfo);
static Bool collideTest_Cylinder_Cylinder(const CollideInfo *a,
                                          const CollideInfo *b,
                                          CollideLocAndNormal *cinfo);
static Bool collideTest_Cylinder_Box(const CollideInfo *a, const CollideInfo *b,
                                     CollideLocAndNormal *cinfo);
static Bool collideTest_Box_Sphere(const CollideInfo *a, const CollideInfo *b,
                                   CollideLocAndNormal *cinfo);
static Bool collideTest_Box_Cylinder(const CollideInfo *a, const CollideInfo *b,
                                     CollideLocAndNormal *cinfo);
static Bool collideTest_Box_Box(const CollideInfo *a, const CollideInfo *b,
                                CollideLocAndNormal *cinfo);

static Bool distCalcProc_CenterAndCenter_2D(const Coord3D *posA,
                                            const Object *objA,
                                            const Coord3D *posB,
                                            const Object *objB, Real &abDistSqr,
                                            Coord3D &abVec, Real maxDistSqr);
static Bool distCalcProc_BoundaryAndBoundary_2D(
    const Coord3D *posA, const Object *objA, const Coord3D *posB,
    const Object *objB, Real &abDistSqr, Coord3D &abVec, Real maxDistSqr);
static Bool distCalcProc_CenterAndCenter_3D(const Coord3D *posA,
                                            const Object *objA,
                                            const Coord3D *posB,
                                            const Object *objB, Real &abDistSqr,
                                            Coord3D &abVec, Real maxDistSqr);
...
static void testRotatedPointsAgainstRect(const Coord2D *pts,  // an array of 4
                                         const CollideInfo *a, Coord2D *avg,
                                         Int *avgTot) {
  // DEBUG_ASSERTCRASH(a->geom.getGeomType() == GEOMETRY_BOX, ("only boxes are
  // ok here"));
  Real major = a->geom.getMajorRadius();
  Real minor = (a->geom.getGeomType() == GEOMETRY_SPHERE)
...
static Bool xy_collideTest_Circle_Rect(const CollideInfo *a,
  ...
  if (cinfo) flipCoord3D(&cinfo->normal);
  return result;
}
...
static Bool xy_collideTest_Rect_Circle(const CollideInfo *a,
                                       ...
                                       CollideLocAndNormal *cinfo) {
#if 1
  /// @todo srj -- this is better than the other one, since it actually handles
  /// rotated rects,
...
#else
  // note, this actually tests the intersection of the the rect with the
  // circle's bounding box. in practice, this is usually good enough, since most
  // of our sphere/cyl shapes are small relative to boxes. (in fact, the WWMath
  ...
  if (circ_r >= rect_l && circ_l <= rect_r && circ_b >= rect_t &&
      circ_t <= rect_b) {
    if (cinfo) {
...
*/
static Bool xy_collideTest_Circle_Circle(const CollideInfo *a,
                                         const CollideInfo *b,
                                         CollideLocAndNormal *cinfo) {
  Coord3D diff;
  vecDiff_2D(&b->position, &a->position, &diff);
  Real distSqr = calcSqrDist_2D(&diff);
  Real touchingDistSqr =
      sqr(a->geom.getMajorRadius() + b->geom.getMajorRadius());
...
static Bool xy_collideTest_Rect_Rect(const CollideInfo *a, const CollideInfo *b,
  ...
  if (avgTot > 0) {
    if (cinfo) {
      ...
      // collisions "after the fact" and thus the rects already intersect,
      // meaning that it's tricky to find the normal. we could do it the "right"
      // way, which would be to back up the objects to find the perfect point
      // (see above), but we don't have velocity information at this point, so
      // we can't really do that. so, naturally, we provide a (poor) guess by
      // just doing the center-to-center normal. this is rarely the correct
      // surface normal for a box-box collision, but until I either (a) rewrite
      // everything to have predictive collisions, or (b) come up with a better
      // definition of a useful normal in this case, I'm not sure we can do a
      // whole lot better... (srj)
      vecDiff_2D(&b->position, &a->position, &cinfo->normal);
      cinfo->normal.normalize();
    }
    return true;
...
//-----------------------------------------------------------------------------
inline Bool z_collideTest_Sphere_Nonsphere(CollideTestProc xyproc,
                                           const CollideInfo *a,
                                           const CollideInfo *b,
                                           CollideLocAndNormal *cinfo) {
  // case 1: center of sphere is within the nonsphere z-space.
  if (a->position.z >= b->position.z &&
      a->position.z <= b->position.z + b->geom.getMaxHeightAbovePosition()) {
    if (xyproc(a, b, cinfo)) {
  ...
  if (a->position.z < b_bot &&
      a->position.z + a->geom.getMajorRadius() >= b_bot) {
    // find the radius of the slice of the sphere that is at b_bot
    CollideInfo amod = *a;
    amod.position.z = b_bot;
    amod.geom.setMajorRadius((Real)sqrtf(sqr(a->geom.getMajorRadius()) -
                                         sqr(b_bot - a->position.z)));
    if (xyproc(&amod, b, cinfo)) {
      // if you want to have 'end' collisions, you should add something like:
      //		if (circle a is contained by circle b) normal points
      //straight up;
      // but since that's currently never possible (and even if it was, wouldn't
      // matter) we don't bother with the calculation.
      return true;
    }
    return false;
  }
  ...
  if (a->position.z > b_top &&
      a->position.z - a->geom.getMajorRadius() <= b_top) {
    CollideInfo amod = *a;
    amod.position.z = b_top;
    amod.geom.setMajorRadius((Real)sqrtf(sqr(a->geom.getMajorRadius()) -
                                         sqr(a->position.z - b_top)));
    if (xyproc(&amod, b, cinfo)) {
      // if you want to have 'end' collisions, you should add something like:
      //		if (circle a is contained by circle b) normal points
      //straight down;
...
//-----------------------------------------------------------------------------
inline Bool z_collideTest_Nonsphere_Nonsphere(CollideTestProc xyproc,
                                              const CollideInfo *a,
                                              const CollideInfo *b,
                                              CollideLocAndNormal *cinfo) {
  // note: we already know that there is a z-intersection (our caller filters
  // that out) so we need not recheck that here.

  // Preflight for close objects.
  Real dSqr =
      sqr(a->position.x - b->position.x) + sqr(a->position.y - b->position.y);
  ...
  if (closeEnough || xyproc(a, b, cinfo)) {
    ...
    if (cinfo) {
      if (b->position.z > a->position.z)
        cinfo->loc.z = (b->position.z + a->position.z +
                        a->geom.getMaxHeightAbovePosition()) *
                       0.5f;
      else
        cinfo->loc.z = (a->position.z + b->position.z +
                        b->geom.getMaxHeightAbovePosition()) *
                       0.5f;
    }
    return true;
  }
...
static Bool collideTest_Sphere_Cylinder(const CollideInfo *a,
                                        const CollideInfo *b,
                                        CollideLocAndNormal *cinfo) {
  return z_collideTest_Sphere_Nonsphere(xy_collideTest_Circle_Circle, a, b,
                                        cinfo);
}
...
static Bool collideTest_Sphere_Box(const CollideInfo *a, const CollideInfo *b,
                                   CollideLocAndNormal *cinfo) {
  return z_collideTest_Sphere_Nonsphere(xy_collideTest_Circle_Rect, a, b,
                                        cinfo);
}
...
static Bool collideTest_Cylinder_Sphere(const CollideInfo *a,
                                        ...
                                        CollideLocAndNormal *cinfo) {
  Bool result =
      z_collideTest_Sphere_Nonsphere(xy_collideTest_Circle_Circle, b, a, cinfo);
  if (cinfo) flipCoord3D(&cinfo->normal);
  return result;
}
...
static Bool collideTest_Cylinder_Cylinder(const CollideInfo *a,
                                          const CollideInfo *b,
                                          CollideLocAndNormal *cinfo) {
  return z_collideTest_Nonsphere_Nonsphere(xy_collideTest_Circle_Circle, a, b,
                                           cinfo);
}
...
static Bool collideTest_Cylinder_Box(const CollideInfo *a, const CollideInfo *b,
                                     CollideLocAndNormal *cinfo) {
  return z_collideTest_Nonsphere_Nonsphere(xy_collideTest_Circle_Rect, a, b,
                                           cinfo);
}
...
static Bool collideTest_Box_Sphere(const CollideInfo *a, const CollideInfo *b,
                                   CollideLocAndNormal *cinfo) {
  Bool result =
      z_collideTest_Sphere_Nonsphere(xy_collideTest_Circle_Rect, b, a, cinfo);
  if (cinfo) flipCoord3D(&cinfo->normal);
  return result;
}
...
static Bool collideTest_Box_Cylinder(const CollideInfo *a, const CollideInfo *b,
                                     CollideLocAndNormal *cinfo) {
  return z_collideTest_Nonsphere_Nonsphere(xy_collideTest_Rect_Circle, a, b,
                                           cinfo);
}
...
static Bool collideTest_Box_Box(const CollideInfo *a, const CollideInfo *b,
                                CollideLocAndNormal *cinfo) {
  return z_collideTest_Nonsphere_Nonsphere(xy_collideTest_Rect_Rect, a, b,
                                           cinfo);
}
...
static Bool distCalcProc_CenterAndCenter_2D(const Coord3D *posA,
                                            ...
                                            const Object *objB, Real &abDistSqr,
                                            Coord3D &abVec, Real maxDistSqr) {
  // note that object positions are defined as the bottom center of the
  // geometry, thus we must add the radius to the z coord to get the proper
...
static Bool distCalcProc_BoundaryAndBoundary_2D(
  ...
  // if (abVec)
  {
    DEBUG_ASSERTCRASH(shrinkFactor >= 0.0f && shrinkFactor <= 1.0f,
                      ("Hmm, this should not be possible."));
    diff.x *= shrinkFactor;
    diff.y *= shrinkFactor;
    abVec = diff;
  }
...
static Bool distCalcProc_BoundaryAndBoundary_3D(
  ...
  Real shrinkFactor = 1.0f;
  Real shrunkenDistSqr = actualDistSqr;
  Real totalRad = (geomA ? geomA->getBoundingSphereRadius() : 0) +
                  (geomB ? geomB->getBoundingSphereRadius() : 0);
  if (totalRad > 0.0f) {
    Real actualDist = sqrtf(actualDistSqr);
    Real shrunkenDist = actualDist - totalRad;
    if (shrunkenDist <= 0.0f) {
...
static CollideTestProc theCollideTestProcs[] = {collideTest_Sphere_Sphere,
...
class PartitionContactListNode : public MemoryPoolObject {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(PartitionContactListNode,
                                          "PartitionContactListNode")
 ...
 public:
  ...
  PartitionData
      *m_other;  ///< the other object (or null for collisions with the terrain)
...
class PartitionContactList {
 private:
  /*
          socketcount should be prime (and "not too close to a power of 2) for
     best results.
 ...
 public:
  ...
  ~PartitionContactList() { resetContactList(); }
...
CellOutwardIterator::CellOutwardIterator(PartitionManager *mgr, Int x, Int y) {
  ...
  m_cellCenterY = y;
  m_cellRadius = 0;
  m_maxRadius = maxInt(maxInt(x, mgr->getCellCountX() - x),
                       maxInt(y, mgr->getCellCountY() - y));
...
try_again:
  ...
  if (m_cellRadius <= m_maxRadius) {
    if (m_iter > 0) {
      m_delta[m_axis] += m_inc;
      --m_iter;
      PartitionCell *cell = m_mgr->getCellAt(m_cellCenterX + m_delta[0],
                                             m_cellCenterY + m_delta[1]);
      if (!cell) goto try_again;
      if (skipEmpties && !cell->getFirstCoiInCell()) goto try_again;
      return cell;
    }
...
//-----------------------------------------------------------------------------
CellAndObjectIntersection::~CellAndObjectIntersection() {
  DEBUG_ASSERTCRASH(m_prevCoi == NULL && m_nextCoi == NULL,
                    ("destroying a linked COI"));
  DEBUG_ASSERTCRASH(!getModule(), ("destroying an in-use COI"));
}

//-----------------------------------------------------------------------------
void CellAndObjectIntersection::friend_addToCellList(
    CellAndObjectIntersection **pListHead) {
  DEBUG_ASSERTCRASH(
      m_prevCoi == NULL && m_nextCoi == NULL && *pListHead != this,
      ("trying to add a cell to list, but it appears to already be in a list"));

  this->m_nextCoi = *pListHead;
  if (*pListHead) (*pListHead)->m_prevCoi = this;
  *pListHead = this;
}

//-----------------------------------------------------------------------------
void CellAndObjectIntersection::friend_removeFromCellList(
    CellAndObjectIntersection **pListHead) {
#define DEBUG_ASSERTINLIST(c)                                                \
  DEBUG_ASSERTCRASH(                                                         \
      (c)->m_prevCoi != NULL || (c)->m_nextCoi != NULL || *pListHead == (c), \
      ("cell is not in list"));
...
void CellAndObjectIntersection::addCoverage(PartitionCell *cell,
  ...
  if (m_cell == NULL) cell->friend_addToCellList(this);
...
#endif
  /*
          You may be asking yourself: why do we model the shroud for all
     players, rather than just the local player? And the answer is: because this
     allows us to checksum these values for net games, to help prevent "shroud
     cheaters" (who use a trainer to disable the shroud on their system).
  */
...
//-----------------------------------------------------------------------------
PartitionCell::~PartitionCell() {
  DEBUG_ASSERTCRASH(m_firstCoiInCell == NULL && m_coiCount == 0,
                    ("destroying a nonempty PartitionCell"));
  // but don't destroy the Cois; they don't belong to us
}

//-----------------------------------------------------------------------------
void PartitionCell::invalidateShroudedStatusForAllCois(Int playerIndex) {
  for (CellAndObjectIntersection *coi = m_firstCoiInCell; coi;
       coi = coi->getNextCoi()) {
...
void PartitionCell::addLooker(Int playerIndex) {
  ...
  // The decreasing Algorithm: A 1 will go straight to -1, otherwise it just
  // gets decremented
  m_shroudLevel[playerIndex].m_currentShroud =
      min(m_shroudLevel[playerIndex].m_currentShroud - 1, -1);
...
void PartitionCell::removeLooker(Int playerIndex) {
  CellShroudStatus oldShroud = getShroudStatusForPlayer(playerIndex);
  // the increasing Algorithm: a -1 goes up to min(1,activeLevel), otherwise it
  // just gets incremented
  if (m_shroudLevel[playerIndex].m_currentShroud == -1)
    m_shroudLevel[playerIndex].m_currentShroud =
        min(m_shroudLevel[playerIndex].m_activeShroudLevel, (Short)1);
  else {
    DEBUG_ASSERTCRASH(m_shroudLevel[playerIndex].m_currentShroud < 0,
                      ("Someone is RemoveLooker-ing on a cell that is not "
                       "looked at.  This will make a permanent shroud blob."));
    m_shroudLevel[playerIndex].m_currentShroud++;
  }
  CellShroudStatus newShroud = getShroudStatusForPlayer(playerIndex);
...
void PartitionCell::addShrouder(Int playerIndex) {
  CellShroudStatus oldShroud = getShroudStatusForPlayer(playerIndex);
  // Increasing active shroud: activeLevel gets incremented, and CS is set to 1
  // if at zero do the algorithm
  m_shroudLevel[playerIndex].m_activeShroudLevel++;
...
void PartitionCell::removeShrouder(Int playerIndex) {
  // Decreasing active shroud: just decrement activeLevel.  This will never
  // result in a client change. Either it was passive shroud and is now active,
  // or it was being looked at and still is.
  m_shroudLevel[playerIndex].m_activeShroudLevel--;
  DEBUG_ASSERTCRASH(
      m_shroudLevel[playerIndex].m_activeShroudLevel >= 0,
      ("Shroud generation has gone negative.  This can't happen."));
}

//-----------------------------------------------------------------------------
// Bool PartitionCell::isShroudedForPlayer( Int playerIndex ) const
//{
// There isn't an absolute answer.  This cell is only shrouded in regards to a
// person
//	return (m_shroudLevel[playerIndex].m_currentShroud == 1);
//}

//-----------------------------------------------------------------------------
CellShroudStatus PartitionCell::getShroudStatusForPlayer(
...
void PartitionCell::addThreatValue(Int playerIndex, UnsignedInt threatValue) {
  if (playerIndex >= 0 && playerIndex < MAX_PLAYER_COUNT) {
    ...
    DEBUG_ASSERTCRASH(oldThreatVal <= oldThreatVal + threatValue,
                      ("adding new threat value overflowed allotted storage."));
#endif
...
void PartitionCell::removeThreatValue(Int playerIndex,
  ...
  if (playerIndex >= 0 && playerIndex < MAX_PLAYER_COUNT) {
    ...
    UnsignedInt oldThreatVal = m_threatValue[playerIndex];
    DEBUG_ASSERTCRASH(
        oldThreatVal >= oldThreatVal - threatValue,
        ("removing new threat value underflowed allotted storage."));
#endif
    m_threatValue[playerIndex] -= threatValue;
  }
}
...
void PartitionCell::addCashValue(Int playerIndex, UnsignedInt cashValue) {
  if (playerIndex >= 0 && playerIndex < MAX_PLAYER_COUNT) {
    ...
    DEBUG_ASSERTCRASH(oldCashVal <= oldCashVal + cashValue,
                      ("adding new cash value overflowed allotted storage."));
#endif
...
void PartitionCell::removeCashValue(Int playerIndex, UnsignedInt cashValue) {
  if (playerIndex >= 0 && playerIndex < MAX_PLAYER_COUNT) {
    ...
    UnsignedInt oldCashVal = m_cashValue[playerIndex];
    DEBUG_ASSERTCRASH(
        oldCashVal >= oldCashVal - cashValue,
        ("removing new cash value underflowed allotted storage."));
#endif
    m_cashValue[playerIndex] -= cashValue;
  }
}
...
void PartitionCell::validateCoiList() {
  ...
  for (CellAndObjectIntersection *coi = getFirstCoiInCell(); coi;
       prevCoi = coi, coi = nextCoi) {
    nextCoi = coi->getNextCoi();
    DEBUG_ASSERTCRASH(coi->getPrevCoi() == prevCoi, ("coi link mismatch"));
    DEBUG_ASSERTCRASH(prevCoi == NULL || prevCoi->getNextCoi() == coi,
                      ("coi link mismatch"));
    DEBUG_ASSERTCRASH((coi == getFirstCoiInCell()) == (prevCoi == NULL),
                      ("coi link mismatch"));
    DEBUG_ASSERTCRASH(nextCoi == NULL || nextCoi->getPrevCoi() == coi,
                      ("coi link mismatch"));
  }
}
#endif
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void PartitionCell::loadPostProcess(void) {}  // end loadPostProcess
...
PartitionData::~PartitionData() {
  ...
  if (ThePartitionManager && ThePartitionManager->isInListDirtyModules(this)) {
    // DEBUG_LOG(("remove pd %08lx from dirty list (%08lx
    // %08lx)\n",this,m_prevDirty,m_nextDirty));
    ThePartitionManager->removeFromDirtyModules(this);
    // DEBUG_ASSERTCRASH(!ThePartitionManager->isInListDirtyModules(this),
    // ("hmm\n"));
  }
}
...
void PartitionData::friend_setShroudednessPrevious(Int playerIndex,
  ...
  // if our status is anything but shrouded, assume we've never been seen by the
  // player.
  m_everSeenByPlayer[playerIndex] =
      (status == OBJECTSHROUD_SHROUDED) ? false : true;
...
ObjectShroudStatus PartitionData::getShroudedStatus(Int playerIndex) {
  // sanity
  DEBUG_ASSERTCRASH(
      playerIndex >= 0 && playerIndex < MAX_PLAYER_COUNT,
  ...
  if (!ThePartitionManager->getUpdatedSinceLastReset()) {
    ...
    // I'll set myself as invalid so the next time you ask I'll recompute.  (If
    // I just return Invalid, then the old state will persist unfairly over a
    // reset)
  ...
  if (m_shroudedness[playerIndex] == OBJECTSHROUD_INVALID ||
      m_shroudedness[playerIndex] == OBJECTSHROUD_INVALID_BUT_PREVIOUS_VALID) {
    Bool updateShroudednessPrevious = (m_shroudedness[playerIndex] !=
                                       OBJECTSHROUD_INVALID_BUT_PREVIOUS_VALID);
#else
  if (m_shroudedness[playerIndex] == OBJECTSHROUD_INVALID) {
#endif
    Int shroudedCells = 0;
    ...
    for (Int i = m_coiInUseCount; i; --i, ++coi) {
      if (coi->getCell()->getShroudStatusForPlayer(playerIndex) ==
          CELLSHROUD_SHROUDED)
        ++shroudedCells;
      else if (coi->getCell()->getShroudStatusForPlayer(playerIndex) ==
               CELLSHROUD_FOGGED)
        ++foggedCells;
    }

    if (m_coiInUseCount == 0) {
      m_shroudedness[playerIndex] =
          OBJECTSHROUD_SHROUDED;  // Stuff off the map have no COIs since they
                                  // aren't in the PartitionCell range
      m_everSeenByPlayer[playerIndex] =
          false;  // force object as never seen by the player
    } else if (shroudedCells == m_coiInUseCount) {
      m_shroudedness[playerIndex] =
          OBJECTSHROUD_SHROUDED;  // every cell I use is shrouded
      m_everSeenByPlayer[playerIndex] =
          false;  // force object as never seen by the player
      if (m_ghostObject &&
          m_shroudednessPrevious[playerIndex] ==
              OBJECTSHROUD_FOGGED) {  // we are shrouding an area that used to
                                      // be fogged so release our memory of what
                                      // was there.
        m_ghostObject->freeSnapShot(playerIndex);
      }
    } else if (shroudedCells + foggedCells == m_coiInUseCount) {
      m_shroudedness[playerIndex] =
          OBJECTSHROUD_FOGGED;        // object is visible but fogged.
      if (m_object && m_ghostObject)  // object does not exist for modules
                                      // holding only GhostObjects
      {
        // fogged but may not be visible if faction unit or faction building
        // that has not been seen before
        Player *player = ThePlayerList->getNthPlayer(playerIndex);
        if (player->getRelationship(m_object->getTeam()) ==
            NEUTRAL) {  // anything neutral that moves around will not be
                        // rendered inside fog.
          if (!m_object->isKindOf(KINDOF_IMMOBILE))
            m_shroudedness[playerIndex] = OBJECTSHROUD_SHROUDED;
        } else  // Not neutral
        {  // enemy unit will always be shrouded unless it's a building that's
           // already been seen by the player.  Fogged Mines are also always
          // shroued no matter what.
          if (!(m_object->isKindOf(KINDOF_IMMOBILE) &&
                m_everSeenByPlayer[playerIndex]) ||
              m_object->isKindOf(KINDOF_MINE))
            m_shroudedness[playerIndex] = OBJECTSHROUD_SHROUDED;
        }
        if (m_shroudedness[playerIndex] ==
            OBJECTSHROUD_FOGGED) {  // successfully applied fog to object so
                                    // check if we need to freeze it's state
          if (m_shroudednessPrevious[playerIndex] <
              OBJECTSHROUD_FOGGED) {  // object was not previously fogged but
                                      // now is fogged.
    ...
    } else if (shroudedCells == 0 &&
               foggedCells == 0) {  // Record that this object was seen by the
                                    // player.  This info will be used to show
                                    // fogged enemy faction buildings.
      m_everSeenByPlayer[playerIndex] = true;
      m_shroudedness[playerIndex] = OBJECTSHROUD_CLEAR;
      if (m_ghostObject &&
          m_shroudednessPrevious[playerIndex] ==
              OBJECTSHROUD_FOGGED) {  // object was previously fogged but now is
                                      // visible so we no longer
        // need a ghost object.
        m_ghostObject->freeSnapShot(playerIndex);
      }
    }  // no cell I use has anything
    else {  // Record that this object was seen by the player.  This info will
            // be used to show fogged enemy faction buildings.
      m_everSeenByPlayer[playerIndex] = true;
      m_shroudedness[playerIndex] =
          OBJECTSHROUD_PARTIAL_CLEAR;  // I am at least partially clear
                                       // otherwise
      if (m_ghostObject &&
          m_shroudednessPrevious[playerIndex] ==
              OBJECTSHROUD_FOGGED) {  // object was previously fogged but now is
...
void PartitionData::addSubPixToCoverage(PartitionCell *cell) {
  DEBUG_ASSERTCRASH(m_coiInUseCount < m_coiArrayCount,
                    ("not enough cois allocated for this object"));
  if (cell) {
    ...
    DEBUG_ASSERTCRASH(coiToUse != NULL || m_coiInUseCount < m_coiArrayCount,
                      ("not enough cois allocated for this object"));
...
// -----------------------------------------------------------------------------
void PartitionData::doRectFill(Real centerX, Real centerY, Real halfsizeX,
  ...
  Real stepSize =
      actualCellSize * 0.5f;  // in theory, should be getCellSize() exactly, but
                              // needs to be smaller to avoid aliasing problems
  Real ydx = s * stepSize;
  Real ydy = -c * stepSize;
  Real xdx = c * stepSize;
  Real xdy = s * stepSize;

  // GS 12-26-02  Oh my.  The step size has been fudged down to ensure that a
  // cell is not jumped over, but then the original step size was used to
  // determine the number of steps.  This error gets more pronounced on bigger
  // buildings.  ie a 100 X 160 registers cells for a 80 x 100.  If you fudge
  // one, you have to fudge the other.
  //	Real stepSizeInvTimes2 = 2.0f * ThePartitionManager->getCellSizeInv();
  Real stepSizeInvTimes2 = 2.0f * (1.0f / stepSize);

  /*
          srj fixes subtle bug: need +1 to get all of misaligned geometries
     accted for.
  */
  ...
  // GS 12-26-02  Oh my part 2.  Now that we are no longer greatly
  // underestimating our COI imprint, we don't need this fudge.  The fudge is
  // not in the calcMaxCOIForShape, so it just results in good COIs getting
  // dropped since we didn't allocate enough to handle this fudge.
  ...
  for (Int iy = 0; iy < numStepsY; ++iy, tl_x += ydx, tl_y += ydy) {
    ...
    for (Int ix = 0; ix < numStepsX; ++ix, x += xdx, y += xdy) {
      ...
      PartitionCell *cell = ThePartitionManager->getCellAt(
          cellx, celly);  // might be null if off the edge
      if (cell) {
...
// -----------------------------------------------------------------------------
void PartitionData::doCircleFill(Real centerX, Real centerY, Real radius) {
  DEBUG_ASSERTCRASH(m_coiInUseCount == 0, ("expected no coi in use here"));

  Int cellCenterX, cellCenterY;
  ThePartitionManager->worldToCell(centerX, centerY, &cellCenterX,
                                   &cellCenterY);
...
void PartitionData::doSmallFill(Real centerX, Real centerY, Real radius) {
  ...
  if (radius > halfCellSize) {
    DEBUG_CRASH(
        ("object is too large to use a 'small' geometry, truncating size to "
         "cellsize\n"));
    radius = halfCellSize;
  }

  Int cx1, cy1, cx2, cy2;
  ThePartitionManager->worldToCell(centerX - radius, centerY - radius, &cx1,
                                   &cy1);
  ThePartitionManager->worldToCell(centerX + radius, centerY + radius, &cx2,
                                   &cy2);
  ...
  for (int i = 0; i < m_coiInUseCount; i++) {
    for (int j = 0; j < i; j++) {
      DEBUG_ASSERTCRASH(m_coiArray[i].getCell() != m_coiArray[j].getCell(),
                        ("dup cells"));
    }
  }
#endif
...
// immobile structures, because otherwise they pass thru them, which looks
// dopey. if preventing dead-ai-collisions is necessary to fix something else,
// please ensure that the above case is cool before signing off. (srj)
  ...
  // DEBUG_LOG(("adding possible collision for
  // %s\n",getObject()->getTemplate()->getName().str()));

  CellAndObjectIntersection *myCoi = m_coiArray;
  for (Int i = m_coiInUseCount; i > 0; --i, ++myCoi) {
    PartitionCell *cell = myCoi->getCell();
    if (cell->getCoiCount() < 2) continue;

    for (CellAndObjectIntersection *coi = cell->getFirstCoiInCell(); coi;
         coi = coi->getNextCoi()) {
...
//-----------------------------------------------------------------------------
Bool PartitionData::collidesWith(const PartitionData *that,
  ...
  if (thisObj->isKindOf(KINDOF_NO_COLLIDE) ||
    ...
    return FALSE;  // A collision extent of zero size is still a point and can
                   // collide, but we don't always want to.

  CollideInfo thisInfo(thisObj->getPosition(), thisObj->getGeometryInfo(),
                       thisObj->getOrientation());
  CollideInfo thatInfo(thatObj->getPosition(), thatObj->getGeometryInfo(),
                       thatObj->getOrientation());

  // invariant for all geometries: first do z collision check.
  Real thisTop =
      thisInfo.position.z + thisInfo.geom.getMaxHeightAbovePosition();
  Real thisBot =
      thisInfo.position.z - thisInfo.geom.getMaxHeightBelowPosition();
  Real thatTop =
      thatInfo.position.z + thatInfo.geom.getMaxHeightAbovePosition();
  Real thatBot =
      thatInfo.position.z - thatInfo.geom.getMaxHeightBelowPosition();
  if (thisTop >= thatBot && thisBot <= thatTop) {
    ...
    //
    // NOTE: This assumes geometry enumerations that start at GEOMETRY_FIRST AND
    // depends on the order in which they appear in the enum list
    //
    CollideTestProc collideProc =
        theCollideTestProcs[(thisGeom - GEOMETRY_FIRST) * GEOMETRY_NUM_TYPES +
...
Bool PartitionManager::geomCollidesWithGeom(const Coord3D *pos1,
                                            ...
                                            Real angle1, const Coord3D *pos2,
  ...
  // invariant for all geometries: first do z collision check.
  if (thisInfo.position.z + thisInfo.geom.getMaxHeightAbovePosition() >=
          thatInfo.position.z &&
      thisInfo.position.z <=
          thatInfo.position.z + thatInfo.geom.getMaxHeightAbovePosition()) {
...
void PartitionData::updateCellsTouched() {
  ...
  DEBUG_ASSERTCRASH(obj != NULL || m_ghostObject != NULL,
                    ("must be attached to an Object here 1"));

  if (obj) {
    // we have no object using this PartitionData but we still have a
    // GhostObject so copy its data.
  ...
  ThePartitionManager->worldToCell(pos.x, pos.y, &currentCellIndexX,
                                   &currentCellIndexY);
  const PartitionCell *currentCell =
      ThePartitionManager->getCellAt(currentCellIndexX, currentCellIndexY);
  ...
  // if we have moved, our shroudedness status might be different for all
  // players, so it must all be invalidated.
...
//-----------------------------------------------------------------------------
Int PartitionData::calcMaxCoiForShape(GeometryType geom, Real majorRadius,
                                      Real minorRadius, Bool isSmall) {
  Int result;
  if (isSmall) {
#if defined(_DEBUG) || defined(_INTERNAL)
    ...
    DEBUG_ASSERTCRASH(
        chk <= 4,
        ("Small objects should be <= 4 cells, but I calced %s as %d\n",
         theObjName.str(), chk));
#endif
    result = 4;
  } else {
    switch (geom) {
      ...
      case GEOMETRY_BOX: {
        Real diagonal = (Real)(sqrtf(majorRadius * majorRadius +
...
void PartitionData::makeDirty(Bool needToUpdateCells) {
  ...
  if (!ThePartitionManager->isInListDirtyModules(this)) {
    ...
    // If I am in the list already, I may be in there for a half update, so if
    // my needs have increased I'll upgrade what I want.
    m_dirtyStatus = NEED_CELL_UPDATE_AND_COLLISION_CHECK;
  }
}
...
void PartitionData::allocCoiArray() {
  DEBUG_ASSERTCRASH(m_coiArrayCount == 0 && m_coiArray == NULL,
                    ("hmm, coi should probably be null here"));
  DEBUG_ASSERTCRASH(m_coiInUseCount == 0, ("hmm, coi count mismatch"));
  m_coiArrayCount = calcMaxCoiForObject();
  m_coiArray = MSGNEW("PartitionManager_COI")
      CellAndObjectIntersection[m_coiArrayCount];  // may throw!
  m_coiInUseCount = 0;
  makeDirty(true);
}
...
void PartitionData::attachToObject(Object *object) {
  ...
  // we only snapshot things that are immobile and have something to draw. Don't
  // need ghostobjects for others.
  if (object->isKindOf(KINDOF_IMMOBILE)) {
    ...
    // Objects with default draw modules usually don't need ghostobjects so skip
    // them to save memory.
  ...
  if (object) object->friend_setPartitionData(this);
  makeDirty(true);
}
...
void PartitionData::detachFromObject() {
  ...
  //
  // note 2: we used to do this in our dtor, but it's too late then, since we
  // require our Object to still be present in order to figure out what to
  // remove. so do it here, since detaching it should nuke it from the obj list
  // anyway. (srj)
  if (TheContactList) TheContactList->removeSpecificPartitionData(this);

  if (m_object) m_object->friend_setPartitionData(NULL);
  removeAllTouchedCells();
  freeCoiArray();
...
void PartitionData::attachToGhostObject(GhostObject *object) {
  ...
  m_coiArrayCount = calcMaxCoiForShape(
      object->getGeometryType(), object->getGeometryMajorRadius(),
      object->getGeometryMinorRadius(), object->getGeometrySmall());
  m_coiArray = MSGNEW("PartitionManager_COI")
      CellAndObjectIntersection[m_coiArrayCount];  // may throw!
  m_coiInUseCount = 0;
  makeDirty(true);

  if (m_ghostObject) m_ghostObject->updateParentObject(NULL, this);
}
...
void PartitionContactList::addToContactList(PartitionData *obj,
                                            PartitionData *other) {
  if (obj == other || obj == NULL || other == NULL) return;

  Object *obj_obj = obj->getObject();
  Object *other_obj = other->getObject();
  if (obj_obj == NULL || other_obj == NULL) return;
  ...
  for (PartitionContactListNode *cd = m_contactHash[hashValue]; cd;
       cd = cd->m_nextHash) {
...
void PartitionContactList::processContactList() {
  for (PartitionContactListNode *cd = m_contactList; cd; cd = cd->m_next) {
    if (cd->m_obj == NULL || cd->m_other == NULL) continue;

    // we know that their partitions overlap; determine if they REALLY collide
    // before proceeding...
    CollideLocAndNormal cinfo;
    if (!cd->m_obj->friend_collidesWith(cd->m_other, &cinfo)) continue;
    ...
    DEBUG_ASSERTCRASH(
        !(obj->isKindOf(KINDOF_IMMOBILE) && other->isKindOf(KINDOF_IMMOBILE)),
        ("we should never have collisions between two immobile things "
         "reported"));

    // the onCollide() calls can remove the object(s) from the partition mgr,
    // thus destroying the partitiondata for 'em. go ahead and null these out
    // here so we won't be tempted to use 'em (since they might be bogus).
    cd->m_obj = NULL;
    cd->m_other = NULL;
    ...
    // Before checking the "other" case, make sure that the previous collision
    // didn't absorb him. This becomes a conflict for pilots giving veterancy to
    // transports and pilots entering transports. Both would occur if these
    // isDestroyed checks were missing.
    if (!obj->isDestroyed() && !other->isDestroyed()) {
      other->onCollide(obj, &cinfo.loc, &cinfo.normal);
    }

    //
    // NOTE: it is VERY IMPORTANT (for performance reasons) to not re-dirty
    // immobile things.
    //
    // NOTE also that we re-get partitiondata from the object, since it might
    // have been removed from the partition system by the onCollide call...
    //
    if (!obj->isDestroyed() && obj->friend_getPartitionData() != NULL &&
        !obj->isKindOf(KINDOF_IMMOBILE)) {
      // DEBUG_LOG(("%d: re-dirtying collision of %s %08lx with %s
    ...
    if (!other->isDestroyed() && other->friend_getPartitionData() != NULL &&
...
//-----------------------------------------------------------------------------
PartitionManager::~PartitionManager() { shutdown(); }  // end ~PartitionManager

//-----------------------------------------------------------------------------
#ifdef PM_CACHE_TERRAIN_HEIGHT
static void calcHeights(const Region3D &world, Real cellSize, Int x, Int y,
                        Real &loZ, Real &hiZ) {
...
void PartitionManager::init() {
  ...
  if (m_cellSize < 1.0) m_cellSize = 1.0;
  ...
  if (TheTerrainLogic) {
    ...
    // DEBUG_ASSERTLOG(m_worldExtents.width() > 0 && m_worldExtents.height() >
    // 0, ("TheTerrainLogic must be loaded before ThePartitionManager"));
    //  you'd think it wouldn't be legal for terrainlogic to have zero area, but
    //  apparently that's what it resets itself to. let's just make the world a
    //  simple place and pretend it's nonzero in area, so that we can proceed
    //  without crashing.
...
//-----------------------------------------------------------------------------
#ifdef DUMP_PERF_STATS
void PartitionManager::getPMStats(double &gcoTimeThisFrameTotal,
                                  double &gcoTimeThisFrameAvg) {
  Int64 freq64;
  GetPrecisionTimerTicksPerSec(&freq64);

  double gcoTimeInMSecs =
      (double)s_timeInClosestObjectsThisFrame * 1000.0f / (double)freq64;

  gcoTimeThisFrameTotal = gcoTimeInMSecs;
  gcoTimeThisFrameAvg =
      gcoTimeInMSecs / (double)s_countInClosestObjectsThisFrame;
}
#endif
...
#ifdef _DEBUG
  // the above *should* remove all the touched cells (via unRegisterObject), but
  // let's check:
  DEBUG_ASSERTCRASH(m_moduleList == NULL, ("hmm, modules left over"));
  PartitionData *mod, *nextMod;
  for (mod = m_moduleList; mod; mod = nextMod) {
    nextMod = mod->getNext();
    DEBUG_ASSERTCRASH(mod->friend_getCoiInUseCount() == 0,
                      ("hmm, coi count mismatch"));
...
void PartitionManager::update() {
  // USE_PERF_TIMER(PartitionManager_update)
  {
    ...
    while (m_dirtyModules) {
      ...
      DEBUG_ASSERTCRASH(
          dirty->getObject() != NULL || dirty->getGhostObject() != NULL,
      ...
      Bool collideEm =
          dirty->isInNeedOfCollisionCheck() &&
          dirty->getObject();  // only update collisions if we have object
  ...
  if (TheGlobalData->m_debugThreatMap) {
    if (TheGameLogic->getFrame() %
        TheGlobalData->m_debugThreatMapTileDuration) {
      Int cellCount = m_cellCountX * m_cellCountY;
      for (int i = 0; i < cellCount; ++i) {
        UnsignedInt threat = m_cells[i].getThreatValue(
            ThePlayerList->getLocalPlayer()->getPlayerIndex());
        if (threat > 0) {
          Real threatMul =
              INT_TO_REAL(threat) / TheGlobalData->m_maxDebugThreat;
          if (threatMul > 1.0f) threatMul = 1.0f;

          Real size = TheGlobalData->m_partitionCellSize;
          Coord3D pos = {m_cells[i].getCellX() * size,
                         m_cells[i].getCellY() * size, 0};
          ...
          addIcon(&pos, size, TheGlobalData->m_debugThreatMapTileDuration,
                  color);
        }
      }
    }
  ...
  if (TheGlobalData->m_debugCashValueMap) {
    if (TheGameLogic->getFrame() %
        TheGlobalData->m_debugCashValueMapTileDuration) {
      Int cellCount = m_cellCountX * m_cellCountY;
      for (int i = 0; i < cellCount; ++i) {
        UnsignedInt value = m_cells[i].getCashValue(
            ThePlayerList->getLocalPlayer()->getPlayerIndex());
        if (value > 0) {
          Real valueMul = INT_TO_REAL(value) / TheGlobalData->m_maxDebugValue;
          if (valueMul > 1.0f) valueMul = 1.0f;
          ...
          addIcon(&pos, size, TheGlobalData->m_debugCashValueMapTileDuration,
                  color);
        }
      }
    }
...
void PartitionManager::registerObject(Object *object) {
  ...
  if (m_moduleList) m_moduleList->setPrev(mod);
  m_moduleList = mod;
...
void PartitionManager::unRegisterObject(Object *object) {
  ...
  if ((ghost = mod->getGhostObject()) != NULL &&
      mod->wasSeenByAnyPlayers() < MAX_PLAYER_COUNT) {
    if (TheContactList) TheContactList->removeSpecificPartitionData(mod);
...
/**
        Reveals the map for the given player, but does not override Shroud
   generation.  (Script)
*/
void PartitionManager::revealMapForPlayer(Int playerIndex) {
  ...
  // By adding a looker directly I don't hit the Ally logic of the normal
  // look/doShroudReveal
...
/**
        Reveals the map for the given player, AND permanently disables all
   Shroud generation (Observer Mode).
        */
void PartitionManager::revealMapForPlayerPermanently(Int playerIndex) {
  // By skipping the removeLooker, I consider myself as actively looking at
  // everything, so Shroud generation will no longer function By adding a looker
  // directly I don't hit the Ally logic of the normal look/doShroudReveal
  for (int i = 0; i < m_totalCellCount; ++i) {
    m_cells[i].addLooker(playerIndex);
  }
}
...
/**
                Adds a layer of permanent blindness.  Used solely to undo the
   permanent reveal for debugging
        */
void PartitionManager::undoRevealMapForPlayerPermanently(Int playerIndex) {
  // First make sure no lingering looks will leave holes when they aren't
  // wanted.
  processEntirePendingUndoShroudRevealQueue();

  // This will have amusing consequences if done without a preceding
  // revealMapForPlayerPermanently. Everything you own can become shrouded.
  for (int i = 0; i < m_totalCellCount; ++i) {
    m_cells[i].removeLooker(playerIndex);
  }
}
...
/**
        Resets the shroud for the given player with passive shroud (can
   re-explore).
        */
void PartitionManager::shroudMapForPlayer(Int playerIndex) {
  ...
  // By pulsing a blast of shroud like this, we will set everything not actively
  // looked at as Passive Shroud
  for (int i = 0; i < m_totalCellCount; ++i) {
...
CellShroudStatus PartitionManager::getShroudStatusForPlayer(Int playerIndex,
                                                            ...
                                                            Int y) const {
  if (playerIndex < 0)
    return CELLSHROUD_SHROUDED;  // Safety.  There are no Negative players, but
                                 // PlayerIndex is typedef'd to Int, not
                                 // UnsignedInt

  const PartitionCell *cell = getCellAt(x, y);
  return cell ? cell->getShroudStatusForPlayer(playerIndex)
              : CELLSHROUD_SHROUDED;
}

//-----------------------------------------------------------------------------
CellShroudStatus PartitionManager::getShroudStatusForPlayer(
...
Int PartitionManager::calcMinRadius(const ICoord2D &cur) {
  /*
          ...
          each object can be anywhere within a given cell (not just in the
     center!) so even objects in the same cell can still be cellSize*1.414
     apart, and objects in adjacent cells can be nearly-zero distance apart!
  */
  Real halfCell = m_cellSize * 0.5;
  Coord3D centerPos[4] = {{-halfCell, -halfCell},
                          {halfCell, -halfCell},
                          {-halfCell, halfCell},
                          {halfCell, halfCell}};
  ...
  Coord3D otherPos[4] = {{x - halfCell, y - halfCell},
                         {x + halfCell, y - halfCell},
                         {x - halfCell, y + halfCell},
                         {x + halfCell, y + halfCell}};

  /*
          this is ugly and there's probably a faster way, but we only do this
     once per PM reset, so it really shouldn't matter... (I hope)
  */
  ...
  for (int i = 0; i < 4; ++i) {
    for (int j = 0; j < 4; ++j) {
      ...
      if (minDistSqr > curDistSqr) minDistSqr = curDistSqr;
    }
  }
...
void PartitionManager::calcRadiusVec() {
  ...
  for (cur.y = -cy + 1; cur.y < cy; ++cur.y) {
    for (cur.x = -cx + 1; cur.x < cx; ++cur.x) {
      /*
              m_radiusVec[curRadius] contains a list of the cells (foo) that
         could contain objects that are <= (curRadius * cellSize) distance away
         from cell (0,0).
      */
      Int curRadius = calcMinRadius(cur);
      DEBUG_ASSERTCRASH(
          curRadius <= m_maxGcoRadius,
          ("expected max of %d but got %d\n", m_maxGcoRadius, curRadius));
      if (curRadius <= m_maxGcoRadius) m_radiusVec[curRadius].push_back(cur);
    }
  }
...
Object *PartitionManager::getClosestObjects(
    const Object *obj, const Coord3D *pos, Real maxDist,
    DistanceCalculationType dc, PartitionFilter **filters,
    SimpleObjectIterator
        *iterArg,  // if nonnull, append ALL satisfactory objects to the
                   // iterator (not just the single closest)
    Real *closestDistArg, Coord3D *closestVecArg) {
  ...
  DEBUG_ASSERTCRASH(theEntrancyCount == 0,
                    ("sorry, this routine is not reentrant"));
  ++theEntrancyCount;
#endif

  DEBUG_ASSERTCRASH((obj == NULL) != (pos == NULL),
  ...
  Real closestDistSqr = maxDist * maxDist;  // if it's not closer than this, we
                                            // shouldn't consider it anyway...
  Coord3D closestVec;
  ...
  /*
          ...
          you "false positives" for objects that are located just off the map...
     (srj)
  */
  ...
  /*
          ...
          contain objects that are <= (curRadius * cellSize) distance away from
     cell (0,0).
  */
  for (Int curRadius = 0; curRadius <= maxRadiusLimit; ++curRadius) {
    const OffsetVec &offsets = m_radiusVec[curRadius];
    if (offsets.empty()) continue;
    for (OffsetVec::const_iterator it = offsets.begin(); it != offsets.end();
         ++it) {
      PartitionCell *thisCell =
          getCellAt(cellCenterX + it->x, cellCenterY + it->y);
      if (thisCell == NULL) continue;

      for (CellAndObjectIntersection *thisCoi = thisCell->getFirstCoiInCell();
           thisCoi; thisCoi = thisCoi->getNextCoi()) {
        PartitionData *thisMod = thisCoi->getModule();
        Object *thisObj = thisMod->getObject();

        // never compare against ourself.
        if (thisObj == obj || thisObj == NULL) continue;

        // since an object can exist in multiple COIs, we use this to avoid
        // processing the same one more than once.
        if (thisMod->friend_getDoneFlag() == theIterFlag) continue;
        thisMod->friend_setDoneFlag(theIterFlag);

        Real thisDistSqr;
        Coord3D distVec;
        if (!(*distProc)(objPos, objToUse, thisObj->getPosition(), thisObj,
                         thisDistSqr, distVec, closestDistSqr))
          continue;

        if (!filtersAllow(filters, thisObj)) continue;
        ...
        if (iterArg) {
          ...
          // (note that we can't break out now 'cuz we have to finish examining
          // the rest of curRadius)
          closestObj = thisObj;
          closestDistSqr = thisDistSqr;
          closestVec = distVec;

          if (!foundAny) {
            // if not adding to iterArg, we want to stop once we have the
            // closest object.
  ...
  if (maxDist < HUGE_DIST) {
    ...
    // default value for "max" is largest possible, based on map size, so we
    // should never make it any larger than that
    if (max < iter.getMaxRadius()) iter.setMaxRadius(max);
  }
  ...
  while ((thisCell = iter.nextNonEmpty()) != NULL) {
    ...
    for (CellAndObjectIntersection *thisCoi = thisCell->getFirstCoiInCell();
      ...
      if (iterArg) {
        ...
        if (!foundAny) {
          // if not adding to iterArg, we want to stop once we have the closest
          // object. since all objects in this radius (and the next radius, due
          // to slop) might be slightly closer, we still have to check all of
          // them. so set the termination radius to be
          // our-current-radius-plus-1. (if we ARE adding to the iterArg, we
          // skip this, cuz we want to go all the way out to the original max we
          // specified as an arg.)
...
//-----------------------------------------------------------------------------
Object *PartitionManager::getClosestObject(const Object *obj, Real maxDist,
                                           DistanceCalculationType dc,
                                           PartitionFilter **filters,
                                           Real *closestDist,
                                           Coord3D *closestDistVec) {
  return getClosestObjects(obj, NULL, maxDist, dc, filters, NULL, closestDist,
                           closestDistVec);
}

//-----------------------------------------------------------------------------
Object *PartitionManager::getClosestObject(const Coord3D *pos, Real maxDist,
                                           DistanceCalculationType dc,
                                           PartitionFilter **filters,
                                           Real *closestDist,
                                           Coord3D *closestDistVec) {
  return getClosestObjects(NULL, pos, maxDist, dc, filters, NULL, closestDist,
                           closestDistVec);
}

//-----------------------------------------------------------------------------
void PartitionManager::getVectorTo(const Object *obj, const Object *otherObj,
                                   DistanceCalculationType dc, Coord3D &vec) {
  DistCalcProc distProc = theDistCalcProcs[dc];
  Real distSqr;
  (*distProc)(obj->getPosition(), obj, otherObj->getPosition(), otherObj,
              distSqr, vec, HUGE_DIST_SQR);
}

//-----------------------------------------------------------------------------
void PartitionManager::getVectorTo(const Object *obj, const Coord3D *pos,
                                   DistanceCalculationType dc, Coord3D &vec) {
  DistCalcProc distProc = theDistCalcProcs[dc];
  Real distSqr;
...
Real PartitionManager::getDistanceSquared(const Object *obj,
  ...
  Coord3D thisVec;
  (*distProc)(obj->getPosition(), obj, otherObj->getPosition(), otherObj,
              thisDistSqr, thisVec, HUGE_DIST_SQR);
  if (vec) *vec = thisVec;
  return thisDistSqr;
}

//-----------------------------------------------------------------------------
Real PartitionManager::getDistanceSquared(const Object *obj, const Coord3D *pos,
                                          DistanceCalculationType dc,
                                          Coord3D *vec) {
  DistCalcProc distProc = theDistCalcProcs[dc];
  Real thisDistSqr;
  Coord3D thisVec;
  (*distProc)(obj->getPosition(), obj, pos, NULL, thisDistSqr, thisVec,
              HUGE_DIST_SQR);
  if (vec) *vec = thisVec;
  return thisDistSqr;
}
...
//-----------------------------------------------------------------------------
// Gets the distance if obj were at goalPos.  Used to calculate attack position
// paths.
Real PartitionManager::getGoalDistanceSquared(const Object *obj,
  ...
  Real thisDistSqr;
  Coord3D thisVec;
  (*distProc)(goalPos, obj, otherObj->getPosition(), otherObj, thisDistSqr,
              thisVec, HUGE_DIST_SQR);
  if (vec) *vec = thisVec;
  return thisDistSqr;
}
...
Real PartitionManager::getGoalDistanceSquared(const Object *obj,
  ...
  Real thisDistSqr;
  Coord3D thisVec;
  (*distProc)(goalPos, obj, otherPos, NULL, thisDistSqr, thisVec,
              HUGE_DIST_SQR);
  if (vec) *vec = thisVec;
  return thisDistSqr;
}
...
Real PartitionManager::getRelativeAngle2D(const Object *obj,
  ...
  // Note this is assumes 2D, and is identical to dotting the perpendicular of v
  // with dir
  Real perpZ = dir->x * v.y - dir->y * v.x;
  if (perpZ < 0.0f) value = -value;

  // note: to make this 3D, 'dir' and 'v' can be normalized and dotted just as
  // they are to test sign, compute N = dir X v, then P = N x dir, then S = P .
  // v, where sign of S is sign of angle - MSB
...
SimpleObjectIterator *PartitionManager::iterateObjectsInRange(
    const Object *obj, Real maxDist, DistanceCalculationType dc,
...
SimpleObjectIterator *PartitionManager::iteratePotentialCollisions(
    const Coord3D *pos, const GeometryInfo &geom, Real angle, Bool use2D) {
  ...
  getClosestObjects(NULL, pos, maxDist,
                    use2D ? FROM_BOUNDINGSPHERE_2D : FROM_BOUNDINGSPHERE_3D,
                    filters, iter, NULL, NULL);
...
//-----------------------------------------------------------------------------
SimpleObjectIterator *PartitionManager::iterateAllObjects(
    PartitionFilter **filters) {
  MemoryPoolObjectHolder iterHolder;
  SimpleObjectIterator *iter = newInstance(SimpleObjectIterator);
  iterHolder.hold(iter);
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
Bool PartitionManager::tryPosition(const Coord3D *center, Real dist, Real angle,
  ...
  if ((options->flags & FPF_USE_HIGHEST_LAYER) != 0) {
    ...
    // ensure we are slightly above the bridge, to account for fudge & sloppy
    // art
  ...
  //
  // we don't usually find positions in the water unless we explicitly say
  // that's OK, or if the option is set we can only pick position underwater
  //
  if (BitTest(options->flags, FPF_IGNORE_WATER) == FALSE) {
    Bool isUnderwater = TheTerrainLogic->isUnderwater(pos.x, pos.y);

    //
    // if we want water spots only and this is underwater it's no good,
    // otherwise we want the default behavior where underwater spots are invalid
    //
  ...
  if (BitTest(options->flags, FPF_IGNORE_ALL_OBJECTS) == FALSE) {
    ...
    for (Object *them = iter->first(); them; them = iter->next()) {
      ...
      //
      // if we have a source that must path to the destination we will ignore
      // that too NOTE: If needed, we can reorganize the meaning of this method
      // to be more explicit on when to pay attention to any 'source' objects
      // and when not too but for now we're keeping the API very simple for the
      // kinds of queries that we're likely to do for an RTS (CBD)
      //
  ...
  //
  // finally ... if sourceToPathDest is valid ... the source object must be able
  // to find a path the the position we have selected.  We want to do this last
  // because a pathfind can sometimes be an relatively expensive operation
  //
  if (options->sourceToPathToDest) {
    const AIUpdateInterface *ai =
        options->sourceToPathToDest->getAIUpdateInterface();

    // check for path existence
    if (ai && TheAI->pathfinder()->quickDoesPathExist(
                  ai->getLocomotorSet(),
                  options->sourceToPathToDest->getPosition(), &pos) == FALSE)
      return FALSE;
...
//
// the following determines how fast we expand our concentric ring search for
// the find position methods
//
static Real ringSpacing = 5.0f;

//-------------------------------------------------------------------------------------------------
/** This method will attempt to find a legal postion from the center position
 * specified, at least minRadis away from it, but no more than maxRadius away.
 *
...
Bool PartitionManager::findPositionAround(const Coord3D *center,
                                          ...
                                          Coord3D *result) {
  // sanity
  if (center == NULL || result == NULL || options == NULL) return FALSE;
  ...
  DEBUG_ASSERTCRASH(!(BitTest(options->flags, FPF_IGNORE_WATER) == TRUE &&
                    ...
                    ("PartitionManager::findPositionAround - The options "
                     "FPF_WATER_ONLY and FPF_IGNORE_WATER are mutually "
  ...
  // start the search at the most inner ring (minRadius) and expand to outer
  // most ring (maxRadius)
  for (Real dist = options->minRadius; dist <= options->maxRadius;
       dist += ringSpacing) {
    //
    // given the spacing that we've been using for the angles on the 'idea'
    // inner circle, we will need more points on larger circles to cover more
    // ground
    //
    Real angleSpacing;
    if (dist == options->minRadius)
      ...
      angleSpacing = (ringSpacing / (dist + 1.0f)) *
                     (TWO_PI / 6.0f);  // larger float = more samples

    //
    // on this "ring", try all the angles available to us in a circle ... we'll
    // start at the 'startAngle', then try 'angleSpacing' to the left, then
    // 'angleSpacing' to the right, then '2 angleSpacings' to the left etc.,
    // ping ponging around the start angle until we've done them all the way
    // around the circle
    //
    ...
    for (Int i = 0; i < samples; ++i) {
      // try one "side"
      if (tryPosition(center, dist, startAngle + angleSpacing * i, options,
                      result) == TRUE)
        return TRUE;

      //
      // try the other "side" ... note for the first point at minRadius this
      // position is the same so we won't test it again in that case
      //
      if (i != 0)
        if (tryPosition(center, dist, startAngle - angleSpacing * i, options,
                        result) == TRUE)
...
//-----------------------------------------------------------------------------
// This is the main accessor of the shroud system.  At this level, allies are
// taken into consideration as specified by the caller.  Look/Unlook are the
// ones sending Ally info, as that is in Object where Allies make sense.
// AddLooker literally just adds a looker for the player you specify. This way,
// Full map reveals and Observer mode active look will not carry over to all
// allies.  They'll use the RevealWholeDamnMap series, which call addLooker
// directly.
void PartitionManager::doShroudReveal(Real centerX, Real centerY, Real radius,
  ...
  for (Int currentIndex = ThePlayerList->getPlayerCount() - 1;
       currentIndex >= 0; currentIndex--) {
    // Object's Look is the one who knows about allies.  Anyone can pask a
    // player mask to me and all of those players will have an active looker
    // applied to a bunch of cells
...
//-----------------------------------------------------------------------------
void PartitionManager::processPendingUndoShroudRevealQueue(
    Bool considerTimestamp) {
  // Keep going until the front one is in the future.  UndoShroudReveal on each
  // one you process. Again, you know these are in order, because we control the
  // adding of the Constant in-the-queue time.
  ...
  while (!m_pendingUndoShroudReveals.empty() &&
         (m_pendingUndoShroudReveals.front()->m_data < compareTime)) {
    SightingInfo *thisInfo = m_pendingUndoShroudReveals.front();

    undoShroudReveal(thisInfo->m_where.x, thisInfo->m_where.y,
                     thisInfo->m_howFar, thisInfo->m_forWhom);
...
void PartitionManager::processEntirePendingUndoShroudRevealQueue() {
  // Something major is about to happen, so we need to rush through our queue
  // and get everything in order. Examples are map resize, full map undo-reveal,
  // stuff like that.
...
//-----------------------------------------------------------------------------
void PartitionManager::undoShroudReveal(Real centerX, Real centerY, Real radius,
                                        PlayerMaskType playerMask) {
  Int cellCenterX, cellCenterY;
  ThePartitionManager->worldToCell(centerX, centerY, &cellCenterX,
                                   &cellCenterY);
...
//-----------------------------------------------------------------------------
void PartitionManager::queueUndoShroudReveal(Real centerX, Real centerY,
                                             Real radius,
                                             PlayerMaskType playerMask) {
...
//-----------------------------------------------------------------------------
void PartitionManager::doShroudCover(Real centerX, Real centerY, Real radius,
                                     PlayerMaskType playerMask) {
  Int cellCenterX, cellCenterY;
  ThePartitionManager->worldToCell(centerX, centerY, &cellCenterX,
                                   &cellCenterY);
  ...
  for (Int currentIndex = ThePlayerList->getPlayerCount() - 1;
       currentIndex >= 0; currentIndex--) {
    // Object's Shroud is the one who knows about allies.  Anyone can pask a
    // player mask to me and all of those players will have an active shrouder
    // applied to a bunch of cells
    const Player *currentPlayer = ThePlayerList->getNthPlayer(currentIndex);
    if (BitTest(playerMask, currentPlayer->getPlayerMask())) {
...
//-----------------------------------------------------------------------------
void PartitionManager::undoShroudCover(Real centerX, Real centerY, Real radius,
                                       PlayerMaskType playerMask) {
  Int cellCenterX, cellCenterY;
  ThePartitionManager->worldToCell(centerX, centerY, &cellCenterX,
                                   &cellCenterY);
...
//-----------------------------------------------------------------------------
void PartitionManager::doThreatAffect(Real centerX, Real centerY, Real radius,
...
//-----------------------------------------------------------------------------
void PartitionManager::undoThreatAffect(Real centerX, Real centerY, Real radius,
                                        UnsignedInt threatVal,
                                        PlayerMaskType playerMask) {
  Int cellCenterX, cellCenterY;
  ThePartitionManager->worldToCell(centerX, centerY, &cellCenterX,
...
//-----------------------------------------------------------------------------
void PartitionManager::doValueAffect(Real centerX, Real centerY, Real radius,
...
//-----------------------------------------------------------------------------
void PartitionManager::undoValueAffect(Real centerX, Real centerY, Real radius,
                                       UnsignedInt valueVal,
                                       PlayerMaskType playerMask) {
  Int cellCenterX, cellCenterY;
  ThePartitionManager->worldToCell(centerX, centerY, &cellCenterX,
...
//-----------------------------------------------------------------------------
#ifdef PM_CACHE_TERRAIN_HEIGHT
Bool PartitionManager::estimateTerrainExtremesAlongLine(const Coord3D &pos,
...
Int PartitionManager::iterateCellsAlongLine(const Coord3D &pos,
  ...
  for (Int curpixel = 0; curpixel <= numpixels; curpixel++) {
    PartitionCell *cell =
        ThePartitionManager->getCellAt(x, y);  // might be null if off the edge
    DEBUG_ASSERTCRASH(cell != NULL, ("off the map"));
    if (cell) {
      Int ret = (*proc)(cell, userData);
      if (ret != 0) return ret;  // bail early
    }
...
Int PartitionManager::iterateCellsBreadthFirst(const Coord3D *pos,
  ...
  // Call proc on each cell, and if it returns non-zero, then return the cell
  // index -1 means error, but we should add a define later for this.
...
Bool PartitionManager::isClearLineOfSightTerrain(const Object *obj,
  ...
  /*
          This looks tempting, and in theory would be better than the below
     code; however, it is not (yet) accurate enough for our targeting needs.
     what we really need to do is add a Bresenham line iterator to the
     TerrainLogic itself, which would probably suffice. Until then, this code is
          left in as an example of what NOT to do. (srj)
  */
  Real maxZ;
  Coord2D maxZPos;
  Bool valid = estimateTerrainExtremesAlongLine(pos, posOther, NULL, &maxZ,
                                                NULL, &maxZPos);
  DEBUG_ASSERTCRASH(
      valid, ("this should never happen unless both positions are off-map"));
  ...
  Real percent =
      (totalDist2D > TINY_DIST) ? (terrainDist2D / totalDist2D) : 0.0f;
...
void PartitionManager::xfer(Xfer *xfer) {
  ...
  //
  // cell size should match what we have allocated in the partition manager, if
  // it doesn't we need to increment the version here and edit this code to be
  // able to load the old cell size if changed after we release
  //
  if (cellSize != m_cellSize) {
    DEBUG_CRASH(
        ("Partition cell size has changed, this save game file is invalid\n"));
  ...
  // total cell count better match what we have allocated for cells in the
  // partition manager
  ...
  // when loading tell the partition manager to rethink and refresh all shroud
  // information
  if (xfer->getXferMode() == XFER_LOAD) {
    // tell partition manager to re-evaluate shroud things when next asked
    m_updatedSinceLastReset = FALSE;

    // refresh the shroud for the local player which will update the radar and
    // everything
  ...
  if (version >= 2) {
    ...
    // This xfer thing is still cool.  On save, I just wrote the number of
    // elements.  On load, the first line above was a decoy, and the second
    // found out the size.

    if (xfer->getXferMode() == XFER_LOAD) {
      // have to remove this assert, because during load there is a setTeam call
      // for each guy on a sub-team, and that results in a queued unlook, so we
      // actually have stuff in here at the start.  I am fairly certain that
      // setTeam should wait until loadPostProcess, but I ain't gonna change it
      // now.
      //			DEBUG_ASSERTCRASH(m_pendingUndoShroudReveals.size()
      //== 0, ("At load, we appear to not be in a reset state.") );

      // I have to split this up though, since on Load I need to make new
      // instances.
      ...
      // And on Save, I need to loop through, but not destroy anything, so I
      // pop-push n times, cycling through the queue once (No random access on
      // queues, and speed not a concern in saveload)
      for (Int infoIndex = 0; infoIndex < queueSize; infoIndex++) {
    ...
    // Version 1 save games will just not have any SightingInfos in the queue to
    // be undone.
  }
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void PartitionManager::loadPostProcess(void) {}  // end loadPostProcess
...
Real PartitionManager::getGroundOrStructureHeight(Real posx, Real posy) {
  ...
  // among them
  PartitionFilterAcceptByKindOf filter1(MAKE_KINDOF_MASK(KINDOF_STRUCTURE),
                                        KINDOFMASK_NONE);
  ...
  ObjectIterator *iter =
      iterateObjectsInRange(&pos, RANGE, FROM_BOUNDINGSPHERE_2D, filters);
  MemoryPoolObjectHolder hold(iter);
...
void PartitionManager::getMostValuableLocation(Int playerIndex,
  ...
  PlayerMaskType playerMask =
      ThePlayerList->getPlayersWithRelationship(playerIndex, whichPlayerTypes);
  ...
  outLocation->set(m_cells[greatestValueCell].getCellX() *
                       TheGlobalData->m_partitionCellSize,
                   m_cells[greatestValueCell].getCellY() *
                       TheGlobalData->m_partitionCellSize,
                   0);
}

//-------------------------------------------------------------------------------------------------
void PartitionManager::getNearestGroupWithValue(
    Int playerIndex, UnsignedInt whichPlayerTypes, ValueOrThreat valType,
    const Coord3D *sourceLocation, Int valueRequired, Bool greaterThan,
    Coord3D *outLocation) {
  if (!(sourceLocation && outLocation)) return;
  ...
  Int nearestGreat =
      iterateCellsBreadthFirst(sourceLocation, cellValueProc, &parms);
  if (nearestGreat != -1) {
    (*outLocation).x =
        m_cells[nearestGreat].getCellX() * TheGlobalData->m_partitionCellSize;
    (*outLocation).y =
        m_cells[nearestGreat].getCellY() * TheGlobalData->m_partitionCellSize;
    (*outLocation).z = 0;
  }
...
//-------------------------------------------------------------------------------------------------
void PartitionManager::storeFoggedCells(
    ShroudStatusStoreRestore &outPartitionStore, Bool storeToFog) const {
  Int i, j, p;
  ...
  for (p = 0; p < MAX_PLAYER_COUNT; ++p) {
    ...
    for (j = 0; j < m_cellCountY; ++j) {
      for (i = 0; i < m_cellCountX; ++i) {
        UnsignedByte &byteToWrite =
            outPartitionStore.m_foggedOrRevealed[p][j * m_cellCountX + i];

        if (storeToFog &&
            m_cells[j * m_cellCountX + i].getShroudStatusForPlayer(p) ==
                CELLSHROUD_FOGGED) {
...
//-------------------------------------------------------------------------------------------------
void PartitionManager::restoreFoggedCells(
    const ShroudStatusStoreRestore &inPartitionStore, Bool restoreToFog) {
  Int i, j, p;
  ...
  for (p = 0; p < MAX_PLAYER_COUNT; ++p) {
    for (j = 0; j < storeHeight; ++j) {
      ...
      for (i = 0; i < storeWidth; ++i) {
        ...
        UnsignedByte byteToRestore =
            inPartitionStore.m_foggedOrRevealed[p][j * storeWidth + i];
...
PartitionFilterRejectBuildings::PartitionFilterRejectBuildings(const Object *o)
    : m_self(o), m_acquireEnemies(false) {
...
Bool PartitionFilterRejectBuildings::allow(Object *other) {
  ...
  if (!other->isKindOf(KINDOF_STRUCTURE)) return true;
  ...
  const Player *otherPlayer =
      contain ? contain->getApparentControllingPlayer(myPlayer) : NULL;

  if (!otherPlayer) otherPlayer = other->getControllingPlayer();

  if (!otherPlayer) return false;

  Relationship relationship =
      myPlayer->getRelationship(otherPlayer->getDefaultTeam());
  if (relationship != ENEMIES) return false;

  // if I am a computer-controlled opponent, auto-aquire enemy buildings (if we
  // can see them!)
  if (m_acquireEnemies) return true;
...
Bool PartitionFilterRepulsor::allow(Object *other) {
  ...
  if (other->isEffectivelyDead()) return false;  // no dead enemies.
  ...
  if (other->isKindOf(KINDOF_STRUCTURE)) {
    ...
    if (other->isAbleToAttack()) return true;
    return false;
  }

  if (other->isKindOf(KINDOF_INERT)) return false;
...
Bool PartitionFilterGarrisonableByPlayer::allow(Object *other) {
  return TheActionManager->canPlayerGarrison(m_player, other,
                                             m_commandSource) == m_match;
}
...
Bool PartitionFilterValidCommandButtonTarget::allow(Object *other) {
  return (m_commandButton->isValidToUseOn(m_source, other, NULL,
                                          m_commandSource) == m_match);
}
...
//-----------------------------------------------------------------------------
PartitionFilterWouldCollide::PartitionFilterWouldCollide(
    const Coord3D &pos, const GeometryInfo &geom, Real angle, Bool desired)
    : m_position(pos),
      m_geom(geom),
      m_angle(angle),
...
Bool PartitionFilterWouldCollide::allow(Object *objOther) {
  ...
  CollideInfo thatInfo(objOther->getPosition(), objOther->getGeometryInfo(),
...
Bool PartitionFilterSamePlayer::allow(Object *objOther) {
  if (m_player == objOther->getControllingPlayer()) return true;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
PartitionFilterAcceptOnTeam::PartitionFilterAcceptOnTeam(const Team *team)
    : m_team(team) {}
...
Bool PartitionFilterAcceptOnSquad::allow(Object *objOther) {
  return (m_squad && m_squad->isOnSquad(objOther) &&
          !objOther->isEffectivelyDead());
}
...
Bool PartitionFilterLineOfSight::allow(Object *objOther) {
  ...
  if (!ThePartitionManager->isClearLineOfSightTerrain(
          m_obj, *m_obj->getPosition(), objOther, *objOther->getPosition()))
    return false;
...
PartitionFilterPossibleToAttack::PartitionFilterPossibleToAttack(
    AbleToAttackType t, const Object *obj, CommandSourceType commandSource)
    : m_attackType(t), m_obj(obj), m_commandSource(commandSource) {}
...
Bool PartitionFilterPossibleToAttack::allow(Object *objOther) {
  ...
  // don't do this here... done in getAbleToAttackSpecificObject, with
  // more/better checking for Disguise (srj)
  //  // stealthed items can't ever be attacked.
  //	UnsignedInt status = objOther->getStatusBits();
  //	if ((status & OBJECT_STATUS_STEALTHED) && !(status &
  //OBJECT_STATUS_DETECTED)) 		return false;

  // we should have already filtered out isAbleToAttack!
#ifdef _DEBUG
  // disable this assert for INTERNAL builds (srj)
  DEBUG_ASSERTCRASH(m_obj && m_obj->isAbleToAttack(),
                    ("if the object is unable to attack at all, you should "
                     "filter that out ahead of time!"));
#endif
  CanAttackResult result = m_obj->getAbleToAttackSpecificObject(
      m_attackType, objOther, m_commandSource);
...
Bool PartitionFilterLastAttackedBy::allow(Object *other) {
  // objOther is guaranteed to be non-null, so we don't need to check (srj)
  if (other->getID() == m_lastAttackedBy) return true;
...
Bool PartitionFilterAcceptByObjectStatus::allow(Object *objOther) {
  UnsignedInt status = objOther->getStatusBits();
  return ((status & m_mustBeSet) == m_mustBeSet) &&
         ((status & m_mustBeClear) == 0);
}
...
Bool PartitionFilterStealthedAndUndetected::allow(Object *objOther) {
  ...
  if (BitTest(objOther->getStatusBits(), OBJECT_STATUS_STEALTHED) &&
      !BitTest(objOther->getStatusBits(), OBJECT_STATUS_DETECTED)) {
    if (!objOther->isKindOf(KINDOF_DISGUISER)) {
      // We have a stealthed & undetected object.
      return m_allow;
    } else {
      // Exception case -- bomb trucks can't be considered stealthed units when
      // they are disguised as the enemy.
      static NameKeyType key_StealthUpdate = NAMEKEY("StealthUpdate");
      StealthUpdate *update =
          (StealthUpdate *)objOther->findUpdateModule(key_StealthUpdate);
      if (update && update->isDisguised()) {
        ...
        if (ourPlayer && otherPlayer) {
          if (ourPlayer->getRelationship(otherPlayer->getDefaultTeam()) ==
              ENEMIES) {
            // Our stealthed & undetected object is disguised as a unit
            // perceived to be our enemy, therefore it's not considered to be
            // stealthed!
            return !m_allow;
          }
          // Our object is disguised as a "friendly" object, therefore it's
          // considered to be stealthed.
          return m_allow;
        }
      }
    ...
    // This handles neutral containers that hold stealth units. This
    // specifically fixes a bug where hunt scripts would ignore this case --
    // units would acquire the building Jarmen Kell occupied even though it was
    // not stealth detected.
    const ContainModuleInterface *contain = objOther->getContain();
    if (contain) {
      const Player *victimApparentController =
          contain->getApparentControllingPlayer(m_obj->getControllingPlayer());
      // Check if it's stealthed!
      if (contain->getStealthUnitsContained() == contain->getContainCount()) {
        // Check if the first object inside is detected (if one is detected, all
        // are detected).
        ContainedItemsList::const_iterator it =
            contain->getContainedItemsList()->begin();
        Object *member = (*it);
        if (member &&
            !BitTest((*it)->getStatusBits(), OBJECT_STATUS_DETECTED)) {
          // Finally check the relationship!
          if (victimApparentController &&
              m_obj->getTeam()->getRelationship(
                  victimApparentController->getDefaultTeam()) == ENEMIES) {
            // Our object is a neutral building garrisoned by enemy units we
            // can't see, therefore it is stealthed.
            return m_allow;
          }
        }
      }
...
static void hLineAddLooker(Int x1, Int x2, Int y, void *playerIndexVoid) {
  if (y < 0 || y >= ThePartitionManager->m_cellCountY ||
      x1 >= ThePartitionManager->m_cellCountX || x2 < 0)
    return;

  Int playerIndex = (Int)(playerIndexVoid);

  PartitionCell *cell =
      &ThePartitionManager->m_cells[y * ThePartitionManager->m_cellCountX +
                                    x1];  // yes, this could be invalid. we'll
                                          // skip the bad ones.
  for (Int x = x1; x <= x2; ++x, ++cell) {
    if (x < 0 || x >= ThePartitionManager->m_cellCountX) continue;
...
static void hLineAddThreat(Int x1, Int x2, Int y, void *threatValueParms) {
  ...
  for (Int x = x1; x <= x2; ++x, ++cell) {
    ...
    cell->addThreatValue(parms->playerIndex,
                         REAL_TO_UNSIGNEDINT(parms->threatOrValue * mulVal));
  }
}
...
static void hLineRemoveThreat(Int x1, Int x2, Int y, void *threatValueParms) {
  ...
  for (Int x = x1; x <= x2; ++x, ++cell) {
    ...
    cell->removeThreatValue(parms->playerIndex,
                            REAL_TO_UNSIGNEDINT(parms->threatOrValue * mulVal));
  }
}
...
static void hLineAddValue(Int x1, Int x2, Int y, void *threatValueParms) {
  ...
  for (Int x = x1; x <= x2; ++x, ++cell) {
    ...
    cell->addCashValue(parms->playerIndex,
                       REAL_TO_UNSIGNEDINT(parms->threatOrValue * mulVal));
  }
}
...
static void hLineRemoveValue(Int x1, Int x2, Int y, void *threatValueParms) {
  ...
  for (Int x = x1; x <= x2; ++x, ++cell) {
    ...
    cell->removeCashValue(parms->playerIndex,
                          REAL_TO_UNSIGNEDINT(parms->threatOrValue * mulVal));
  }
}
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
SightingInfo::SightingInfo() { reset(); }
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
Bool SightingInfo::isInvalid() const { return m_howFar == 0.0f; }

// ------------------------------------------------------------------------------------------------
/** CRC */
// ------------------------------------------------------------------------------------------------
void SightingInfo::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void SightingInfo::loadPostProcess() {}  // end loadPostProcess

// ------------------------------------------------------------------------------------------------
SightingInfo::~SightingInfo() {}  // end loadPostProcess
