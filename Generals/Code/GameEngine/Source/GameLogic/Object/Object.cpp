...
static const ModelConditionFlags s_allWeaponFireFlags[WEAPONSLOT_COUNT] = {
    MAKE_MODELCONDITION_MASK5(
        MODELCONDITION_FIRING_A, MODELCONDITION_BETWEEN_FIRING_SHOTS_A,
        MODELCONDITION_RELOADING_A, MODELCONDITION_PREATTACK_A,
        MODELCONDITION_USING_WEAPON_A),
    MAKE_MODELCONDITION_MASK5(
        MODELCONDITION_FIRING_B, MODELCONDITION_BETWEEN_FIRING_SHOTS_B,
        MODELCONDITION_RELOADING_B, MODELCONDITION_PREATTACK_B,
        MODELCONDITION_USING_WEAPON_B),
    MAKE_MODELCONDITION_MASK5(
        MODELCONDITION_FIRING_C, MODELCONDITION_BETWEEN_FIRING_SHOTS_C,
...
AsciiString DescribeObject(const Object *obj) {
  ...
  if (obj->getName().isNotEmpty()) {
    ret.format("Object %d (%s) [%s, owned by player %d (%ls)]", obj->getID(),
               obj->getName().str(), obj->getTemplate()->getName().str(),
               obj->getControllingPlayer()->getPlayerIndex(),
               obj->getControllingPlayer()->getPlayerDisplayName().str());
  } else {
    ret.format("Object %d [%s, owned by player %d (%ls)]", obj->getID(),
...
Object::Object(const ThingTemplate *tt, ObjectStatusBits statusBits, Team *team)
  ...
  // Force the thing template to use the most overridden version of itself -
  // jkmcd Note that after this, the object will be using m_template, which
  // forces the usage of the most overridden version of tt, so this is okay.
  ...
  if (m_shroudClearingRange == -1.0f)
    m_shroudClearingRange =
        m_visionRange;  // Backwards compatible, and perfectly logical default
                        // to assign
  m_shroudRange = 0.0f;
  ...
  //
  Int totalModules =
      tt->getBehaviorModuleInfo().getCount() +
      NUM_SLEEP_HELPERS;  // need to take into account all the helper modules
  ...
  // the helpers are done first -- even before Behaviors! -- in case a module
  // needs to call something that uses them.
  static const NameKeyType smcHelperModuleDataTagNameKey =
      NAMEKEY("ModuleTag_SMCHelper");
  ...
  *curB++ = m_smcHelper;
  ...
  if (TheAI != NULL && TheAI->getAiData()->m_enableRepulsors &&
      isKindOf(KINDOF_CAN_BE_REPULSED)) {
    // if we can ever be a temporary-repulsor, make a repulsor helper. (srj)
    static const NameKeyType repulsorHelperModuleDataTagNameKey =
        NAMEKEY("ModuleTag_RepulsorHelper");
    ...
    m_repulsorHelper =
        newInstance(ObjectRepulsorHelper)(this, &repulsorModuleData);
  ...
  if (!tt->isKindOf(KINDOF_SHRUBBERY)) {
    static const NameKeyType defectionModuleDataTagNameKey =
        NAMEKEY("ModuleTag_DefectionHelper");
    ...
    m_defectionHelper =
        newInstance(ObjectDefectionHelper)(this, &defectionModuleData);
    *curB++ = m_defectionHelper;
  }

  if (tt->canPossiblyHaveAnyWeapon()) {
    // we only need a firingtracker and wshelper if we can possibly have a
    // weapon.
    static const NameKeyType weaponStatusModuleDataTagNameKey =
        NAMEKEY("ModuleTag_WeaponStatusHelper");
    static ObjectWeaponStatusHelperModuleData weaponStatusModuleData;
    weaponStatusModuleData.setModuleTagNameKey(
        weaponStatusModuleDataTagNameKey);
    m_wsHelper =
        newInstance(ObjectWeaponStatusHelper)(this, &weaponStatusModuleData);
    *curB++ = m_wsHelper;

    static const NameKeyType firingTrackerModuleDataTagNameKey =
        NAMEKEY("ModuleTag_FiringTrackerHelper");
    static FiringTrackerModuleData firingTrackerModuleData;
    firingTrackerModuleData.setModuleTagNameKey(
        firingTrackerModuleDataTagNameKey);
    m_firingTracker =
        newInstance(FiringTracker)(this, &firingTrackerModuleData);
  ...
  for (modIdx = 0; modIdx < mi.getCount(); ++modIdx) {
    ...
    BehaviorModule *newMod = (BehaviorModule *)TheModuleFactory->newModule(
        this, modName, mi.getNthData(modIdx), MODULETYPE_BEHAVIOR);
    *curB++ = newMod;
    ...
    if (ai) {
      DEBUG_ASSERTCRASH(
          m_ai == NULL,
          ("You should never have more than one AI module (srj)\n"));
      m_ai = ai;
    }
  ...
  *curB = NULL;
  ...
  if (ai) {
    ai->setAttitude(
        getTeam()->getPrototype()->getTemplateInfo()->m_initialTeamAttitude);
    if (m_team && m_team->getPrototype() &&
        m_team->getPrototype()->getAttackPriorityName().isNotEmpty()) {
  ...
  // If a valid team has been assigned me, then I have a Player I can ask about
  // my starting level
  const Player *controller = getControllingPlayer();
  m_experienceTracker->setVeterancyLevel(
      controller->getProductionVeterancyLevel(getTemplate()->getName()));
  ...
  m_healthBoxOffset
      .zero();  // this is used for units that are amorphous, like angry mob
  ...
  // disable occlusion for some time after object is created to allow them to
  // exit the factory/building.
  m_safeOcclusionFrame = TheGameLogic->getFrame() + tt->getOcclusionDelay();

  m_soleHealingBenefactorID =
      INVALID_ID;  ///< who is the only other object that can give me this
                   ///< non-stacking heal benefit?
  m_soleHealingBenefactorExpirationFrame =
      0;  ///< on what frame can I accept healing (thus to switch) from a new
          ///< benefactor
...
void Object::initObject() {
  ...
  // If I have a valid team assigned, I can run through my Upgrade modules with
  // his flags
  ...
  // If the player has battle plans (America Strategy Center), then apply those
  // bonuses to this object if applicable. Internally it validates certain kinds
  // of objects.
  const Player *controller = getControllingPlayer();
  if (controller) {
    if (!getReceivingDifficultyBonus() &&
  ...
  // For each special power module that we have, add it's type to the
  // specialpower bits. This is for optimal access later.
  for (BehaviorModule **m = m_behaviors; *m; ++m) {
    ...
    if (spTemplate) {
      SET_SPECIALPOWERMASK(m_specialPowerBits,
                           spTemplate->getSpecialPowerType());
    }
  }

  // Kris -- All missiles must be projectiles! This is the perfect place to
  // assert them! srj: yes, but only in debug...
#if defined(_DEBUG) || defined(_INTERNAL)
  if (!isKindOf(KINDOF_PROJECTILE)) {
    if (isKindOf(KINDOF_SMALL_MISSILE) || isKindOf(KINDOF_BALLISTIC_MISSILE)) {
      // Warning only...
      DEBUG_CRASH(
          ("Missile %s must also be a KindOf = PROJECTILE in addition to being "
           "either a SMALL_MISSILE or PROJECTILE_MISSILE -- call Kris (36844) "
           "for questions!",
           getTemplate()->getName().str()));
    }
  }
  ...
  if (!isKindOf(KINDOF_PROJECTILE) && !isKindOf(KINDOF_INERT)) {
    ...
    // We ignore projectiles cause they are frequently created & destroyed, and
    // are not of general interest.  Normal unit count tests consider tanks or
    // infantry or planes, etc. jba.
...
Object::~Object() {
  ...
  /// @todo Generalize the notion of objects entering and leaving the world, so
  /// we don't have to special case this
  ...
  //
  // remove from radar before we NULL out the team ... the order of ops are
  // critical here because the radar code will sometimes look at the team info
  // and it is assumed through the team and player code that the team is valid
  //
  if (m_radarData) TheRadar->removeObject(this);

  // emit message announcing object's destruction. Again, order is important; we
  // must do this before wiping out the team.
  ...
  if (m_partitionData) ThePartitionManager->unRegisterObject(this);
  ...
  if (m_experienceTracker) m_experienceTracker->deleteInstance();
  ...
  // Instead of removing it from the named cache, notify the script engine that
  // it has died. The script engine will remove it from the cache if necessary.
  // The script engine needs to take a crack at this in case it is the current
  // "This Object" pointer.
...
void Object::onDestroy() {
  // This is the old cleanUpContain safeguard.  Say goodbye so they don't try to
  // look us up.
  ...
  //
  // run the onDelete on all modules present so they each have an opportunity to
  // cleanup anything they need to ... including talking to any other modules
  //
  for (BehaviorModule **b = m_behaviors; *b; ++b) {
    (*b)->onDelete();
  }

  // Have to remove ourself from looking as well.  RebuildHoleWorkers definately
  // hit here.
...
void Object::setGeometryInfo(const GeometryInfo &geom) {
  m_geometryInfo = geom;
  if (m_partitionData) {
    // if our geometry changes, we unregister and re-register with the
    // partitionmgr so that our size gets updated appropriately. this shouldn't
    // be a problem unless setGeometryInfo gets called frequently. (srj)
    ThePartitionManager->unRegisterObject(this);
    ThePartitionManager->registerObject(this);
  }

  if (m_drawable) m_drawable->reactToGeometryChange();
}
...
void Object::restoreOriginalTeam() {
  if (m_team == NULL || m_originalTeamName.isEmpty()) return;

  Team *origTeam = TheTeamFactory->findTeam(m_originalTeamName);
  if (origTeam == NULL) {
    DEBUG_CRASH(
        ("Object original team (%s) could not be found or created! (srj)\n",
  ...
  if (m_team == origTeam) {
    DEBUG_CRASH(
        ("Object appears to still be on its original team, so why are we "
         "attempting to restore it? (srj)\n"));
    return;
  }
...
void Object::setOrRestoreTeam(Team *team, Bool restoring) {
  ...
  if (m_team) {
    ...
    if (draw) draw->changedTeam();
  }

  // This can't just go in ::defect, because some things just do setTeam.  The
  // act of setting a new team needs to tell the modules and do other important
  // stuff. And it needs to happen after the switch.
  ...
  Int oldPlayerIndex =
      (oldTeam) ? (oldTeam->getControllingPlayer()->getPlayerIndex()) : -1;
  Int newPlayerIndex =
      (m_team) ? (m_team->getControllingPlayer()->getPlayerIndex()) : -1;
...
void Object::setStatus(ObjectStatusBits bits, Bool set) {
  ...
  if (m_status != oldStatus) {
    if (set && (bits & OBJECT_STATUS_REPULSOR) != 0 &&
        m_repulsorHelper != NULL) {
      // Damaged repulsable civilians scare (repulse) other civs, but only
      // for a short amount of time... use the repulsor helper to turn off
      // repulsion shortly.
      m_repulsorHelper->sleepUntil(TheGameLogic->getFrame() +
                                   2 * LOGICFRAMES_PER_SECOND);
    }

    // when an object's construction status changes, it needs to have its
    // partition data updated, in order to maintain the shroud correctly.
    if ((m_status & OBJECT_STATUS_UNDER_CONSTRUCTION) !=
        (oldStatus & OBJECT_STATUS_UNDER_CONSTRUCTION)) {
      // CHECK FOR MINES, AND DETONATE THEM NOW
      ObjectIterator *iter = ThePartitionManager->iteratePotentialCollisions(
          getPosition(), getGeometryInfo(), getOrientation());
      ...
      if (m_partitionData) m_partitionData->makeDirty(true);
    }
  }
}
...
void Object::setScriptStatus(ObjectScriptStatusBit bit, Bool set) {
  ...
  if (m_scriptStatus != oldScriptStatus) {
    if ((m_scriptStatus & OBJECT_STATUS_SCRIPT_DISABLED) !=
        (oldScriptStatus & OBJECT_STATUS_SCRIPT_DISABLED)) {
      if (m_partitionData) {
        // if an object becomes disabled or unpowered, then you have to update
        // its partition data because it will change how far it can see.
    ...
    if ((m_scriptStatus & OBJECT_STATUS_SCRIPT_UNPOWERED) !=
        (oldScriptStatus & OBJECT_STATUS_SCRIPT_UNPOWERED)) {
      if (m_partitionData) {
        // if an object becomes disabled or unpowered, then you have to update
        // its partition data because it will change how far it can see.
        m_partitionData->makeDirty(true);
...
Bool Object::canCrushOrSquish(Object *otherObj,
                              CrushSquishTestType testType) const {
  ...
  if (isDisabledByType(DISABLED_UNMANNED)) {
    // Unmanned vehicles cannot crush troops. This was happening when Jarmen
    // Kell sniped the vehicle and booted the guys out while still moving, as
    // the vehicle is now on a different team.
    return false;
  }

  UnsignedByte crusherLevel = getCrusherLevel();

  // order matters: we want to know if I consider it to be an ally, not vice
  // versa
  ...
  if (testType == TEST_SQUISH_ONLY || testType == TEST_CRUSH_OR_SQUISH) {
    //****************************************************************************************
    // NOTE: This section of code is used by the pathfinder to determine if the
    // object should
    //      move to the target. I don't think it's the right place to check for
    //      this because the semantics check to see if we can squish something
    //      -- not approach it. However I'm not moving it for fear of some major
    //      breakage! -- KM
    ...
    //}
    // if( squisher )
    // NOTE2: *** IF YOU REENABLE THIS CODE -- Move the "if( !crusherLevel )
    // return false" below
    //                                        this squish section.
    //****************************************************************************************
    {
...
// ------------------------------------------------------------------------------------------------
void Object::topple(const Coord3D *toppleDirection, Real toppleSpeed,
                    UnsignedInt options) {
...
//=============================================================================
void Object::reloadAllAmmo(Bool now) { m_weaponSet.reloadAllAmmo(this, now); }

//=============================================================================
Bool Object::isOutOfAmmo() const { return m_weaponSet.isOutOfAmmo(); }

//=============================================================================
Bool Object::hasAnyWeapon() const { return m_weaponSet.hasAnyWeapon(); }
...
void Object::setFiringConditionForCurrentWeapon() const {
  if (m_drawable) {
    ...
    ModelConditionFlags c =
        m_weaponSet.getModelConditionForWeaponSlot(wslot, WSF_FIRING);
...
void Object::clearSpecialModelConditionStates() {
  clearModelConditionFlags(
      MAKE_MODELCONDITION_MASK(MODELCONDITION_SPECIAL_CHEERING));
  m_smcUntil = NEVER;
}
...
//	else
//	{
//		DEBUG_CRASH(("NULL Drawable at this point, you can't get
//modelconditionflags now.")); 		static ModelConditionFlags noFlags; 		return
//noFlags;
//	}
//}
...
Weapon *Object::getCurrentWeapon(WeaponSlotType *wslot) {
  if (!m_weaponSet.hasAnyWeapon()) return NULL;

  if (wslot) *wslot = m_weaponSet.getCurWeaponSlot();
...
//=============================================================================
/*
        NOTE: getAbleToAttackSpecificObject NO LONGER internally calls
   isAbleToAttack(), since that isn't an incredibly fast call, and this is
   called repeatedly in some inner loops where we already know that
   isAbleToAttack() == true. so you should always call isAbleToAttack prior to
   calling this! (srj)
*/
CanAttackResult Object::getAbleToAttackSpecificObject(
    AbleToAttackType t, const Object *target,
  ...
  return m_weaponSet.getAbleToAttackSpecificObject(t, this, target,
                                                   commandSource);
}

//=============================================================================
// Used for base defenses and otherwise stationary units to see if you can
// attack a position potentially out of range.
CanAttackResult Object::getAbleToUseWeaponAgainstTarget(
    AbleToAttackType attackType, const Object *victim, const Coord3D *pos,
    CommandSourceType commandSource) const {
  return m_weaponSet.getAbleToUseWeaponAgainstTarget(attackType, this, victim,
                                                     pos, commandSource);
}
...
Bool Object::chooseBestWeaponForTarget(const Object *target,
  ...
  return m_weaponSet.chooseBestWeaponForTarget(this, target, criteria,
                                               cmdSource);
}
...
void Object::fireCurrentWeapon(Object *target) {
  ...
  if (weapon && (weapon->getStatus() == READY_TO_FIRE)) {
    ...
    DEBUG_ASSERTCRASH(
        m_firingTracker,
        ("hey, we are firing but have no firing tracker. this is wrong."));
    if (m_firingTracker) m_firingTracker->shotFired(weapon, target->getID());
    if (reloaded)
...
void Object::fireCurrentWeapon(const Coord3D *pos) {
  ...
  if (weapon && (weapon->getStatus() == READY_TO_FIRE)) {
    ...
    DEBUG_ASSERTCRASH(
        m_firingTracker,
        ("hey, we are firing but have no firing tracker. this is wrong."));
    if (m_firingTracker) m_firingTracker->shotFired(weapon, INVALID_ID);
    if (reloaded)
      releaseWeaponLock(LOCKED_TEMPORARILY);  // release any temporary locks.
...
void Object::preFireCurrentWeapon(const Object *victim) {
  ...
  // If we are going to be capable of firing our weapon NEXT frame, set the
  // pre-attack up now. This gets called by AIAttackFireWeaponState::onEnter()..
  // but the update happens next frame.
  if (weapon &&
      TheGameLogic->getFrame() + 1 >= weapon->getPossibleNextShotFrame()) {
...
Relationship Object::getRelationship(const Object *that) const {
  ...
  if (myTeam && that) {
    if (getIsUndetectedDefector()) {
      return NEUTRAL;  // so my AI does not give away my position by auto
                       // acquire
    } else if (that->getIsUndetectedDefector()) {
      return ALLIES;  // so I treat undetecteddefectors like they were my very
                      // own
    } else {
...
Player *Object::getControllingPlayer() const {
  ...
  if (myTeam) return myTeam->getControllingPlayer();
...
void Object::setCustomIndicatorColor(Color c) {
  if (m_indicatorColor != c) {
    m_indicatorColor = c;
    if (m_drawable) m_drawable->changedTeam();
  }
}

//=============================================================================
void Object::removeCustomIndicatorColor() { setCustomIndicatorColor(0); }
...
//=============================================================================
// Object::getNightIndicatorColor - used to make blue/purple easier to see on
// night models.
//=============================================================================
...
inline Bool isPosDifferent(const Coord3D *a, const Coord3D *b) {
  // this is necessary because PhysicsBehavior may generate tiny changes even
  // when "standing still", due to roundoff errors. It's important that we only
  // invalidate the PartitionManager stuff when the pos/orientation really
  // changes (for efficiency purposes) so we must put in some cleverness...
  const Real THRESH = 0.01f;

  if (fabs(a->x - b->x) > THRESH) return true;

  if (fabs(a->y - b->y) > THRESH) return true;

  if (fabs(a->z - b->z) > THRESH) return true;
...
inline Bool isAngleDifferent(Real a, Real b) {
  ...
  if (fabs(a - b) > THRESH) return true;

  return false;
}

//-------------------------------------------------------------------------------------------------
void Object::reactToTurretChange(WhichTurretType turret, Real oldRotation,
                                 Real oldPitch) {
  ...
  if (rotationChange) {
    if (getContain()) getContain()->containReactToTransformChange();
  }
}
...
void Object::reactToTransformChange(const Matrix3D *oldMtx,
  ...
  if (_isnan(getPosition()->x) || _isnan(getPosition()->y) ||
      _isnan(getPosition()->z)) {
...
ObjectShroudStatus Object::getShroudedStatus(Int playerIndex) const {
  if (getTemplate()->isKindOf(KINDOF_ALWAYS_VISIBLE)) return OBJECTSHROUD_CLEAR;

  if (m_partitionData) return m_partitionData->getShroudedStatus(playerIndex);
...
void Object::attemptDamage(DamageInfo *damageInfo) {
  ...
  if (body) body->attemptDamage(damageInfo);

  /// @todo track damage dealt/attempted

  //
  // if actual damage occurred, and this is an object owned by the local player
  // we might do a radar event for under attack. Note that we do not even try to
  // do radar events for DAMAGE_PENALTY as that damage type is a type of damage
  ...
  if (damageInfo->out.m_actualDamageDealt > 0.0f &&
      ...
      !BitTest(damageInfo->in.m_sourcePlayerMask,
               getControllingPlayer()->getPlayerMask()) &&
      m_radarData != NULL &&
...
ObjectID Object::getSoleHealingBenefactor(void) const {
  ...
  if (now > m_soleHealingBenefactorExpirationFrame) return INVALID_ID;

  return m_soleHealingBenefactorID;
}

Bool Object::attemptHealingFromSoleBenefactor(
    Real amount, const Object *source,
    UnsignedInt duration) {  ///< for the non-stacking healers like ambulance
                             ///< and propaganda
  ...
  // Either it is ok to accept healing from any who offer or this is my guy,
  // calling again
...
Real Object::estimateDamage(DamageInfoInput &damageInfo) const {
  BodyModuleInterface *body = getBodyModule();
  if (body) return body->estimateDamage(damageInfo);
...
void Object::kill() {
  ...
  DEBUG_ASSERTCRASH(
      ...
      ("Attempting to kill an unKillable object (InactiveBody?)\n"));
...
//-------------------------------------------------------------------------------------------------
void Object::healCompletely() { attemptHealing(HUGE_DAMAGE_AMOUNT, NULL); }
...
void Object::setCaptured(Bool isCaptured) {
  ...
  // No need to see if we should skip updates, this flag has no effect on
  // skipping updates.
}

//-------------------------------------------------------------------------------------------------
Bool Object::isStructure(void) const { return isKindOf(KINDOF_STRUCTURE); }
...
void Object::setReceivingDifficultyBonus(Bool receive) {
  ...
  getControllingPlayer()->friend_applyDifficultyBonusesForObject(
      this, m_isReceivingDifficultyBonus);
}
...
//----------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void Object::setDisabled(DisabledType type) { setDisabledUntil(type, FOREVER); }
...
void Object::setDisabledUntil(DisabledType type, UnsignedInt frame) {
  ...
  if (type < 0 || type >= DISABLED_COUNT) {
    DEBUG_CRASH(
        ("Invalid disabled type value %d specified -- doesn't not exist!",
         type));
    return;
  }
  ...
  if (type == DISABLED_UNMANNED && !isKindOf(KINDOF_DRONE)) {
    ...
    TheAudio->addAudioEvent(&sound);
             ...
             type == DISABLED_HACKED) {
    // We've lost power -- make sure we aren't already out of power as the
    // sounds shouldn't happen if you were already disabled.
    if (!isDisabledByType(DISABLED_UNDERPOWERED) &&
        !isDisabledByType(DISABLED_EMP) && !isDisabledByType(DISABLED_HACKED)) {
  ...
  if (m_disabledTillFrame[type] != frame) {
    ...
    if (m_drawable) {
      if (isDisabled()) {
        // Held does not tint anybody.  If we are multiply disabled, the other
        // setting will hit the tint, and in clear, only-held and not-disabled
        // are both causes to untint. Doh. Also shouldn't be tinting when
        // disabled by scripting. Doh^2. Also shouldn't be CLEARING tinting if
        // we're disabling by held or script disabledness Doh^3. Unmanned is no
        // tint too
        if (type != DISABLED_HELD && type != DISABLED_SCRIPT_DISABLED &&
  ...
  if (type == DISABLED_UNMANNED && !isKindOf(KINDOF_DRONE)) {
    ...
    if (set && set->testWeaponSetFlag(WEAPONSET_CARBOMB)) {
      Object *sniper = TheGameLogic->findObjectByID(
          getBodyModule()->getLastDamageInfo()->in.m_sourceID);
      if (sniper) sniper->scoreTheKill(this);

      kill();
    } else {
      // This vehicle's pilot has been sniped, so we want to clear the veterancy
      // rating (if any)
      ExperienceTracker *xpTracker = getExperienceTracker();
      if (xpTracker) {
        xpTracker->setExperienceAndLevel(0);
      }
      // Not only that, but it also loses any healing bonuses it may have earned
      // in its prior life
      {
        static const NameKeyType key_AutoHealBehavior =
            NAMEKEY("AutoHealBehavior");
        AutoHealBehavior *autoHeal =
            (AutoHealBehavior *)(findUpdateModule(key_AutoHealBehavior));
        if (autoHeal) autoHeal->undoUpgrade();
      }
    }
  }

  // This will only be called if we were NOT disabled before coming into this
  // function.
  if (edgeCase) {
    onDisabledEdge(true);
  }
...
Bool Object::clearDisabled(DisabledType type) {
  ...
  if (type == DISABLED_UNDERPOWERED || type == DISABLED_EMP ||
      type == DISABLED_HACKED) {
    // We've regained power-- make sure we aren't still disabled by another
    // type.
    AudioEventRTS sound;
    if ((!isDisabledByType(DISABLED_UNDERPOWERED) ||
         type == DISABLED_UNDERPOWERED) &&
  ...
  ContainModuleInterface *contain = getContain();
  if (contain) {
    // We explicitly pass stuff in up in the set, so we need to turn it off if
    // it is a forever type
    Object *rider = (Object *)contain->friend_getRider();
  ...
  // to clarify, if I am NOT disabled by anything other than DISABLED_HELD, or
  // DISABLED_SCRIPT_DISABLED
  if (!isDisabled() ||
      getDisabledFlags().countInverseIntersection(exceptions) == 0) {
    if (m_drawable) m_drawable->clearTintStatus(TINT_STATUS_DISABLED);
  }
  ...
  for (int i = 0; i < DISABLED_COUNT; i++) {
    ...
    if (isDisabledByType(type)) {
      if (now >= m_disabledTillFrame[i]) {
        clearDisabled(type);  // This will also DECREMENT m_pauseCount in all
                              // specialpowers
        m_disabledMask.set(type, 0);
      }
    }
...
void Object::updateTriggerAreaFlags() {
  Int j = 0;
  // Update the flags, and remove any trigger areas that this object isn't
  // inside.
  for (Int i = 0; i < m_numTriggerAreasActive; i++) {
    if (!m_triggerInfo[j].isInside) continue;
...
//-------------------------------------------------------------------------------------------------
void Object::onCollide(Object *other, const Coord3D *loc,
...
//-------------------------------------------------------------------------------------------------
/**
        Our owning player is telling us to recheck our UpgradeModules, as an
...
void Object::updateUpgradeModules() {
  ...
  // We need to add in all of the already owned upgrades to handle "AND"
  // requiring upgrades. We combine all the masks in case someone has a Object
...
//-------------------------------------------------------------------------------------------------
// This function sucks.
// It was added for objects that can disguise as other objects and contain
// upgraded subobject overrides. A concrete example is the bomb truck. Different
// payloads are displayed based on which upgrades have been made. When the bomb
// truck disguises as something else, these subobjects are lost because the
// vector is stored in W3DDrawModule. When we revert back to the original bomb
// truck, we call this function to recalculate those upgraded subobjects.
//-------------------------------------------------------------------------------------------------
...
Bool Object::didEnter(const PolygonTrigger *pTrigger) const {
  if (!didEnterOrExit()) return false;

  DEBUG_ASSERTCRASH(
      !isKindOf(KINDOF_INERT),
      ("Asking whether an inert object entered or exited. This is invalid.\n"));
...
Bool Object::isInside(const PolygonTrigger *pTrigger) const {
  DEBUG_ASSERTCRASH(
      !isKindOf(KINDOF_INERT),
      ("Asking whether an inert is inside a trigger area. This is invalid.\n"));
...
ExitInterface *Object::getObjectExitInterface() const {
  ...
  for (BehaviorModule **umod = m_behaviors; *umod; ++umod) {
    if ((exitInterface = (*umod)->getUpdateExitInterface()) != NULL) break;
  }
...
void Object::setTriggerAreaFlagsForChangeInPosition() {
  ...
  if (isKindOf(KINDOF_PROJECTILE) || isKindOf(KINDOF_INERT)) return;
  ...
  for (i = 0; i < m_numTriggerAreasActive; i++) {
    if (!m_triggerInfo[i].pTrigger->pointInTrigger(m_iPos)) {
      ...
      if (m_team) m_team->setEnteredExited();
      TheGameLogic->updateObjectsChangedTriggerAreas();
#ifdef _DEBUG
  ...
  for (const PolygonTrigger *pTrig = PolygonTrigger::getFirstPolygonTrigger();
       pTrig; pTrig = pTrig->getNext()) {
    Bool skip = false;
    for (i = 0; i < m_numTriggerAreasActive; i++) {
      ...
      if (m_numTriggerAreasActive < MAX_TRIGGER_AREA_INFOS) {
        ...
        if (!didWarn) {
          ...
          TheScriptEngine->AppendDebugMessage(
              "***WARNING - Too many nested trigger areas. ***", true);
        }
      }
    }
  }
...
Bool Object::isInList(Object **pListHead) const {
  ...
  DEBUG_ASSERTCRASH(found == result,
                    ("inconsistent links in Object::isInList"));
#endif
  return result;
}
...
void Object::prependToList(Object **pListHead) {
  ...
  m_prev = NULL;
  m_next = *pListHead;
  if (*pListHead) (*pListHead)->m_prev = this;
  *pListHead = this;
}
...
void Object::friend_prepareForMapBoundaryAdjust(void) {
  ...
  // The whole PartitionManager and all of the Looker data is about to be blown
  // away, so forget what I think I have done
  m_partitionLastLook->reset();
  m_partitionLastShroud->reset();
...
void Object::calcNaturalRallyPoint(Coord2D *pt) {
  ...
  //
  // get the natural rally point from the template, this coord is in model space
  // relative to the model (0,0,0)
  //
  /*
...
Bool Object::isMobile() const {
  if (isKindOf(KINDOF_IMMOBILE)) return false;
...
void Object::scoreTheKill(const Object *victim) {
  // Do stuff that has nothing to do with experience points here, like tell our
  // Player we killed something
  /// @todo Multiplayer score hook location?

  Player *victimController = victim->getControllingPlayer();
  // if the other player is not a playable side (i.e. they are civilian,
  // observer, whatever) we shouldn't count the kill.
  if (victimController->isPlayableSide() == FALSE) {
    return;
  }

  if (victim->isKindOf(KINDOF_IGNORED_IN_GUI)) return;
  ...
  // Don't count kills that I do on my own buildings or units, cause thats just
  // silly.
  ...
  if (m_experienceTracker &&
    ...
    // srj sez: per dustin, no experience (et al) for killing things under
    // construction.
    if (!victim->testStatus(OBJECT_STATUS_UNDER_CONSTRUCTION)) {
      Int experienceValue =
          victim->getExperienceTracker()->getExperienceValue(this);
...
VeterancyLevel Object::getVeterancyLevel() const {
  return m_experienceTracker ? m_experienceTracker->getVeterancyLevel()
                             : LEVEL_REGULAR;
}
  ...
  if (m_drawable) {
    ...
    if (TheGlobalData) {
      if (TheGlobalData->m_forceModelsToFollowTimeOfDay) {
        set.set(MODELCONDITION_NIGHT,
                (TheGlobalData->m_timeOfDay == TIME_OF_DAY_NIGHT) ? 1 : 0);
      }

      if (TheGlobalData->m_forceModelsToFollowWeather) {
        set.set(MODELCONDITION_SNOW,
                (TheGlobalData->m_weather == WEATHER_SNOWY) ? 1 : 0);
      }
    }
    m_drawable->clearAndSetModelConditionFlags(clr, set);
  }
...
Bool Object::isSelectable() const {
  return getTemplate()->isKindOf(KINDOF_ALWAYS_SELECTABLE) ||
         (m_isSelectable && !testStatus(OBJECT_STATUS_UNSELECTABLE) &&
          !isEffectivelyDead() &&
          !getTemplate()->isKindOf(
              KINDOF_DRONE)  // Most drones are unselectable from being slaved,
                             // but the SpyDrone needs help
         );
}
...
void Object::setWeaponSetFlag(WeaponSetType wst) {
  ...
  m_weaponSet.updateWeaponSet(this);
  if (m_drawable) {
    m_drawable->setModelConditionState(
        TheWeaponSetTypeToModelConditionTypeMap[wst]);
  }
}
...
void Object::clearWeaponSetFlag(WeaponSetType wst) {
  ...
  m_weaponSet.updateWeaponSet(this);
  if (m_drawable) {
    m_drawable->clearModelConditionState(
        TheWeaponSetTypeToModelConditionTypeMap[wst]);
  }
}
...
void Object::onVeterancyLevelChanged(VeterancyLevel oldLevel,
  ...
  if (up) giveUpgrade(up);

  BodyModuleInterface *body = getBodyModule();
  if (body) body->onVeterancyLevelChanged(oldLevel, newLevel);

  Bool hideAnimationForStealth =
      (!isLocallyControlled() && testStatus(OBJECT_STATUS_STEALTHED));

  Bool doAnimation =
      (!hideAnimationForStealth && (newLevel > oldLevel) &&
       (!isKindOf(KINDOF_IGNORED_IN_GUI)));  // First, we plan to do the
                                             // animation if the level went up
  ...
  if (doAnimation && TheGameLogic->getDrawIconUI()) {
    if (TheAnim2DCollection &&
        TheGlobalData->m_levelGainAnimationName.isEmpty() == FALSE) {
      Anim2DTemplate *animTemplate = TheAnim2DCollection->findTemplate(
          TheGlobalData->m_levelGainAnimationName);

      Coord3D pos = *getPosition();
      pos.add(&m_healthBoxOffset);

      TheInGameUI->addWorldAnimation(
          animTemplate, &pos, WORLD_ANIM_FADE_ON_EXPIRE,
...
Bool Object::isAbleToAttack() const {
  ...
  // For things that may or may not be able to normally attack, but are under a
  // status condition
  if (getStatusBits() & OBJECT_STATUS_NO_ATTACK) return false;

  // if we're contained within a transport we cannot attack unless it
  // specifically allows us
  const Object *containedBy = getContainedBy();
  DEBUG_ASSERTCRASH(
      (containedBy == NULL) || (containedBy->getContain() != NULL),
      ("A %s thinks they are contained by something with no contain module!",
       getTemplate()->getName().str()));
  if (containedBy && containedBy->getContain() &&
      !containedBy->getContain()->isPassengerAllowedToFire())
    return false;

  // We can't fire if under construction
  if (testStatus(OBJECT_STATUS_UNDER_CONSTRUCTION)) return false;

  // or being sold
  if (testStatus(OBJECT_STATUS_SOLD)) return false;

  // We can't fire if we, as a portable structure, are aptly disabled
  if (isKindOf(KINDOF_PORTABLE_STRUCTURE) ||
  ...
  if (ai && !isKindOf(KINDOF_CAN_ATTACK)) {
    for (Int i = 0; i < WEAPONSLOT_COUNT; i++) {
      ...
      Real dummy;
      WhichTurretType tur =
          ai->getWhichTurretForWeaponSlot((WeaponSlotType)i, &dummy);
  ...
  if (isKindOf(KINDOF_CAN_ATTACK)) return true;

  // for garrisonned buildings that can attack sometimes
  if (getStatusBits() & OBJECT_STATUS_CAN_ATTACK) return true;

  // for weaponless transports.  This will make me think I can, but I will check
  // if I literally can by looking at passenger weapons in CanAttack.
  const ContainModuleInterface *contain = getContain();
  if (contain && contain->isPassengerAllowedToFire() &&
      contain->getContainCount() > 0)
    return true;

  // if we have AI and a weapon, assume we know how to use it
  if (getAIUpdateInterface() != NULL && m_weaponSet.hasAnyWeapon()) {
    // actually, we don't want to do this; we want the troop crawler to be
    // considered "able to attack" even if empty, so sayeth Dustin. (srj)
    //		// special case: if the only damage we do is DEPLOY, we must
    //have some guys contained. 		if
    //(m_weaponSet.hasSingleDamageType(DAMAGE_DEPLOY))
    //		{
...
void Object::maskObject(Bool mask) {
  ...
  //
  // when masking objects they become unselected ... we do this in any situation
  // for any player cause you aren't allowed to select masked objects, if the
  // object is not selected (ie, belongs to another player) it's no big deal
  // cause it won't be selected anyway
  //

  if (mask)
    TheGameLogic->deselectObject(this, ~getControllingPlayer()->getPlayerMask(),
                                 TRUE);
...
Bool Object::isUsingAirborneLocomotor(void) const {
  return (m_ai && m_ai->getCurLocomotor() &&
          ((m_ai->getCurLocomotor()->getLegalSurfaces() &
            LOCOMOTORSURFACE_AIR) != 0));
}

//-------------------------------------------------------------------------------------------------
// THIS FUNCTION BELONGS AT THE OBJECT LEVEL BECAUSE THERE IS AT LEAST ONE
// SPECIAL UNIT (ANGRY MOB) WHICH NEEDS LOGIC-SIDE POSITION CALC'S... IT WOULD
// PROBABLY BE WISE TO MOVE ALL THE HARD-CODED DEFAULTS BELOW INTO A NEW
// Drawable::getHealthBox..() WHICH USES GEOM0INFO, MODEL DATA, INI DATA, ETC.
...
Bool Object::getHealthBoxDimensions(Real &healthBoxHeight,
  ...
  Real size = MAX(20.0f, MIN(150.0f, (getGeometryInfo().getMajorRadius() +
                                      getGeometryInfo().getMinorRadius())));
...
void Object::updateObjValuesFromMapProperties(Dict *properties) {
  ...
  // adjust the vision distance of this object, overriding its default vision
  // distance
  ...
  // adjust the shroud clearing distance of this object, overriding its default
  // distance
  valInt = properties->getInt(TheKey_objectShroudClearingDistance, &exists);
  if (exists) {
    if (valInt < 0) valInt = 0.0f;
...
void Object::onDisabledEdge(Bool becomingDisabled) {
  ...
  if (controller) {
    //@todo jkmcd - Colin suggested we rewrite this to use the interface stuff.
    //I agree, but need
  ...
  if (powerToAdjust > 0) {
    // We can't affect something that consumes, or else we go low power which
    // removes the consumption which makes us not low power so we add the
    // consumption so we go low power... This check also guaards the IsDisabled
    // in friend_adjustPower above
...
void Object::crc(Xfer *xfer) {
  // This is evil - we cast the const Matrix3D * to a Matrix3D * because the
  // XferCRC class must use the same interface as the XferLoad class for save
  // game restore.  This only works because XferCRC does not modify its data.
  ...
  AsciiString tmp;
  Bool doLogging = g_logObjectCRCs /* && getControllingPlayer()->getPlayerType()
                                      == PLAYER_HUMAN */
      ;
  if (doLogging) {
    tmp.format("CRC of Object %d (%s), owned by player %d, ", m_id,
               getTemplate()->getName().str(),
               getControllingPlayer()->getPlayerIndex());
  ...
  if (doLogging) {
    ...
    CRCDEBUG_LOG(("CRC of Object %d (%s), owned by player %d, ", m_id,
                  getTemplate()->getName().str(),
                  getControllingPlayer()->getPlayerIndex()));
    DUMPMATRIX3D(mtx);
  }
...
#endif  // DEBUG_CRC
  if (m_experienceTracker) xfer->xferSnapshot(m_experienceTracker);
#ifdef DEBUG_CRC
  if (doLogging) {
    XferCRC tmpXfer;
    tmpXfer.open("tmp");
 ...
 * Version Info:
 * 1: Initial version
 * 2: Xfers m_singleUseCommandUsed... determines if the single use command
 * button has been used or not. 3: Xfers the solehealingbenefactor ID and
 * expiration frame 4: misc stuff that got missed somehow 5:
 * m_isReceivingDifficultyBonus 6: We do indeed need to save m_containedBy.  The
 * comment misrepresents what the contain module will do. 7: save full mtx, not
 * pos+orient.
 */
//-------------------------------------------------------------------------------------------------
void Object::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_LOAD) {
    // change the ID of the drawable attached to be the same ID as it was when
    // it was saved
  ...
  // sighting info, last look - must be saved cause we save
  // PartitionCell::m_shroudLevel
  xfer->xferSnapshot(m_partitionLastLook);

  // sighting info, last shroud - must be saved cause we save
  // PartitionCell::m_shroudLevel
  ...
  // New var added for version 2. Determines if the single use command button
  ...
  //
  // radar data ... when loading, we will remove all objects from the radar and
  // let the radar system load itself as a separate chunk of data from the save
  // file
  //
  ...
  //
  // we do not need to do anything with our m_containedBy pointer, the post
  // process of that objects contain module will actually re-do the contain
  // process again
  //
  // m_containedBy <-- do nothing with this right now
  if (version >= 6) {
    // No, the contain module is just going to friend_ reach in and set this for
    // us. Containers more complicated than Open (like Tunnel) can't do that.
    // Our variable, our responsibility.
  ...
  if (m_numTriggerAreasActive < 0 ||
      m_numTriggerAreasActive > MAX_TRIGGER_AREA_INFOS) {
    DEBUG_CRASH(("Invalid m_numTriggerAreasActive = %d, max is %d",
                 m_numTriggerAreasActive, MAX_TRIGGER_AREA_INFOS));
    throw SC_INVALID_DATA;
  }
  for (i = 0; i < m_numTriggerAreasActive; i++) {
    ...
    if (xfer->getXferMode() == XFER_LOAD) {
      //
      // CBD (11-13-2002) I'm disabling this because it appears there might be
      // some areas with empty names, see John A. for more info
      //
      // if (triggerName.isNotEmpty())
      m_triggerInfo[i].pTrigger =
          TheTerrainLogic->getTriggerAreaByName(triggerName);
    }
  ...
  for (BehaviorModule **b = m_behaviors; *b; ++b) ++moduleCount;
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (BehaviorModule **b = m_behaviors; *b; ++b) {
      ...
      moduleIdentifier =
          TheNameKeyGenerator->keyToName(module->getModuleTagNameKey());
      DEBUG_ASSERTCRASH(
          moduleIdentifier != AsciiString::TheEmptyString,
    ...
    for (UnsignedShort i = 0; i < moduleCount; ++i) {
      ...
      NameKeyType moduleIdentifierKey =
          TheNameKeyGenerator->nameToKey(moduleIdentifier);
      ...
      // it from the object definition in a future patch, if that is so, we need
      // to skip the module data in the file
      //
      if (module == NULL) {
        // for testing purposes, this module better be found
        DEBUG_CRASH(
            ("Object::xfer - Module '%s' was indicated in file, but not found "
             "on object '%s'(%d)\n",
  ...
  // AIGroup*		m_group;
  // ///< if non-NULL, we are part of this group of agents

  // don't need to save m_partitionData.
  DEBUG_ASSERTCRASH(
      !(xfer->getXferMode() == XFER_LOAD && m_partitionData == NULL),
      ("should not be in partitionmgr yet"));

  // don't need to be saved or loaded; are inited & cached for runtime only by
  // our ctor (srj)
  // m_repulsorHelper;
  // m_smcHelper;
  // m_wsHelper;
  ...
  if (version >= 4) {
    // xfer the weaponSetFlags FIRST, since we need 'em to restore the weaponSet
    // properly. (srj)
...
Bool Object::affectedByUpgrade(const UpgradeTemplate *upgradeT) const {
  ...
  for (BehaviorModule **module = m_behaviors; *module; ++module) {
    ...
    if (upgrade->wouldUpgrade(maskToCheck)) {
      // if any of my many upgrade modules would execute in response to this
      // flag, say yes.
      return TRUE;
    }
  }
...
void Object::giveUpgrade(const UpgradeTemplate *upgradeT) {
  if (upgradeT) {
    ...
    //
    // iterate through all the upgrade modules of this object and call the
    // method to grant a new upgrade
    //
    updateUpgradeModules();
  }
}  // end giveUpgrade
...
void Object::onCapture(Player *oldOwner, Player *newOwner) {
  // Everybody dhills when they captured so they don't keep doing something the
  // new player might not want him to be doing
  ...
  // rip through the behavior modules and call the onCapture for any modules
  // that care
  for (BehaviorModule **module = m_behaviors; *module; ++module)
  ...
  // onCapture is used now, so it better be called after ownership changes and
  // not before.
  //
  handlePartitionCellMaintenance();

  // Design needs the player to be able to sell buildings he steals from the
  // AI's build list, and this is the easiest fix.  The only snafu would be a
  // key building build listed by the AI that the player can capture and the AI
  // tries to capture back but needs to not sell.  In that case, a Cinematic
  // Unsellable version of the building needs to be made.  This fix has been
  // okayed as the most non-lethal in November.
  ...
  if (oldOwner != newOwner && newOwner->isSkirmishAIPlayer()) {
    // The skirmish ai doesn't know what to do with captured faction buildings
    // except sell them.
...
void Object::onDie(DamageInfo *damageInfo) {
  ...
  DEBUG_ASSERTCRASH(
      m_hasDiedAlready == false,
      ("Object::onDie has been called multiple times. This is invalid. jkmcd"));
  ...
  for (BehaviorModule **d = m_behaviors; *d; ++d) {
    ...
    if (die) die->onDie(damageInfo);
  }

  // When objects die we remove from the radar as they're really not interesting
  // anymore
  if (m_radarData) TheRadar->removeObject(this);
  ...
  // objects that were spawned from something, need to tell their spawner that
  // they have died
  Object *spawner = TheGameLogic->findObjectByID(getProducerID());
  if (spawner) {
    // get the spawn behavior interface of the spawner
    SpawnBehaviorInterface *spawnerBehavior =
        spawner->getSpawnBehaviorInterface();
    if (spawnerBehavior) spawnerBehavior->onSpawnDeath(getID(), damageInfo);
  }

  handlePartitionCellMaintenance();
  if (m_team) m_team->notifyTeamOfObjectDeath();
  // Play death sound here.

  AudioEventRTS deathSound = *getTemplate()->getSoundDie();
  // If we were killed by fire, or by poison, we should play those die sounds
  // instead of the usual sound
  if (damageInfo->in.m_deathType == DEATH_BURNED)
    deathSound = *getTemplate()->getSoundDieFire();
  else if (damageInfo->in.m_deathType == DEATH_POISONED ||
           damageInfo->in.m_deathType == DEATH_POISONED_BETA)
    deathSound = *getTemplate()->getSoundDieToxin();

  // If we didn't actually have a specialized die sound (for the case of fire or
  // poison, we should use the generic death sound)
  if (!TheAudio->isValidAudioEvent(&deathSound))
    deathSound = *getTemplate()->getSoundDie();

  // Use the position. Next frame, when this unit is gone, this sound will be
  // clipped because we can no longer automatically find its position. - jkmcd
  deathSound.setPosition(getPosition());
  PlayerIndex index =
      getControllingPlayer() ? getControllingPlayer()->getPlayerIndex() : 0;
  ...
  if (isLocallyControlled() && !selfInflicted)  // wasLocallyControlled? :-)
  {
    if (isKindOf(KINDOF_STRUCTURE) && isKindOf(KINDOF_MP_COUNT_FOR_VICTORY)) {
      ...
      // Create a fake radar event so the user can use the spacebar to quickly
      // jump to this!
  ...
  TheInGameUI->removeIdleWorker(this,
                                this->getControllingPlayer()->getPlayerIndex());
}

//-------------------------------------------------------------------------------------------------
/**
        A weapon cannot be in charge of maintaining condition flags as it is all
   event driven. I will maintain my ModelCondition myself if it should change.
   Firing is set by firing logic, so I don't include it here.  It is only the
   states that expire on timers that noone watches that I am concerned with.
*/
//-------------------------------------------------------------------------------------------------
void Object::adjustModelConditionForWeaponStatus() {
  ...
  for (int i = 0; i < WEAPONSLOT_COUNT; ++i) {
    ...
    if (i != m_weaponSet.getCurWeaponSlot()) {
      ...
      // srj sez: not 100% sure about this one, but the problem is: say we were
      // attacking, then issue a move command. if we didn't do this here, we
      // might still have a 'firing' pose, because his weapon might be in
      // 'reloading' mode. since we're not attacking, however, we really don't
      // care, so we just force the issue here. (This might still need tweaking
      // for the pursue state.)
      conditionToSet = WSF_NONE;
    } else {
      ...
      // then we might have a frame where we have reloaded and are
      // ready-to-fire, but haven't fired yet this frame. in that case, use
      // 'between' so we still have a firing pose, 'cuz if we use 'none' we will
      // 'pop' back to idle for a frame. (srj) additional note: only do if
      // aiming or firing, since we could also be in this state if we are
      // approaching or pursuing a target! (srj)
      if (newStatus == READY_TO_FIRE && conditionToSet == WSF_NONE &&
          testStatus(OBJECT_STATUS_IS_ATTACKING) &&
          (testStatus(OBJECT_STATUS_IS_AIMING_WEAPON) ||
           testStatus(OBJECT_STATUS_IS_FIRING_WEAPON))) {
        conditionToSet = WSF_BETWEEN;
      }
    }

    if (m_drawable) {
      m_drawable->updateDrawableClipStatus(
          w->getRemainingAmmo(), w->getClipSize(), w->getWeaponSlot());
      if (conditionToSet != WSF_INVALID &&
          conditionToSet != m_lastWeaponCondition[i]) {
        m_lastWeaponCondition[i] = conditionToSet;
        ModelConditionFlags c = m_weaponSet.getModelConditionForWeaponSlot(
            (WeaponSlotType)i, conditionToSet);
        m_drawable->clearAndSetModelConditionFlags(s_allWeaponFireFlags[i], c);
        if (conditionToSet == WSF_PREATTACK) {
          // in the preattack state, adjust the speed of the preattack anim to
          // match the actual time it will take
...
//-------------------------------------------------------------------------------------------------
/// We have moved a 'significant' amount, so do maintenence that can be
/// considered 'cell-based'
void Object::onPartitionCellChange() { handlePartitionCellMaintenance(); }
...
void Object::addValue() {
  if (!m_partitionLastValue->isInvalid()) {
    DEBUG_CRASH(
        ("An Object is adding value, but hasn't removed his previous value."));
    return;
  }

  if (!getControllingPlayer()) return;

  if (((getStatusBits() & OBJECT_STATUS_UNDER_CONSTRUCTION) != 0) ||
      (isEffectivelyDead()) || (getShroudClearingRange() <= 0.0f))
    return;
  ...
  m_partitionLastValue->m_howFar =
      getVisionRange();  // we are valuable all the way to where we can target.

  ThePartitionManager->doValueAffect(
      m_partitionLastValue->m_where.x, m_partitionLastValue->m_where.y,
      m_partitionLastValue->m_howFar, m_partitionLastValue->m_data,
...
void Object::removeValue() {
  if (m_partitionLastValue->isInvalid()) {
    // removing before adding is valid, cause we always remove before adding.
    // (So the first remove will occur before the first add)
    return;
  }

  ThePartitionManager->undoValueAffect(
      m_partitionLastValue->m_where.x, m_partitionLastValue->m_where.y,
      m_partitionLastValue->m_howFar, m_partitionLastValue->m_data,
      m_partitionLastValue->m_forWhom);
...
void Object::addThreat() {
  if (!m_partitionLastThreat->isInvalid()) {
    DEBUG_CRASH(
        ("An Object is adding threat, but hasn't removed his previous threat. "
         "(He hasn't finished the threat?)"));
    return;
  }
  ...
  m_partitionLastThreat->m_howFar =
      getVisionRange();  // we are threatening all the way to where we can
                         // target.

  ThePartitionManager->doThreatAffect(
      m_partitionLastThreat->m_where.x, m_partitionLastThreat->m_where.y,
      m_partitionLastThreat->m_howFar, m_partitionLastThreat->m_data,
...
void Object::removeThreat() {
  ...
  ThePartitionManager->undoThreatAffect(
      m_partitionLastThreat->m_where.x, m_partitionLastThreat->m_where.y,
      m_partitionLastThreat->m_howFar, m_partitionLastThreat->m_data,
      m_partitionLastThreat->m_forWhom);
...
void Object::look() {
  ...
  Player *controller = getControllingPlayer();
  if (controller) {
    // I removed the check for objects under construction by request of
    // designers since they want constructing objects to have a reduced sight
    // range now. -MW dead or blind things don't reveal shroud
    if ((!isDestroyed())  // Some things get Destroyed directly without hitting
                          // Death.
        && (!isEffectivelyDead()) && (getShroudClearingRange() > 0.0f)) {
      ...
      for (Int currentIndex = ThePlayerList->getPlayerCount() - 1;
        ...
        if (getControllingPlayer()->getRelationship(
                currentPlayer->getDefaultTeam()) == ALLIES) {
      ...
      if (isKindOf(KINDOF_REVEAL_TO_ALL)) lookingMask = PLAYERMASK_ALL;

      Coord3D pos = *getPosition();
      ThePartitionManager->doShroudReveal(
          pos.x, pos.y, getShroudClearingRange(), lookingMask);
      ...
      //			DEBUG_LOG(( "A %s looks at %f, %f for %x at
      //range %f\n", 									getTemplate()->getName().str(), 									pos.x, 									pos.y, 									lookingMask,
...
void Object::unlook() {
  if (m_partitionLastLook->isInvalid()) {
    // Your very first action will be an unlook, so of course you haven't looked
    // yet.  This is not an error This early return prevents an extra unlook if
    // you never looked.  Like you have 0 vision.
    return;
  }

  ThePartitionManager->queueUndoShroudReveal(
      m_partitionLastLook->m_where.x, m_partitionLastLook->m_where.y,
      m_partitionLastLook->m_howFar, m_partitionLastLook->m_forWhom);

  //			DEBUG_LOG(( "A %s queues an unlook at %f, %f for %x at
  //range %f\n", 									getTemplate()->getName().str(), 									m_partitionLastLook.m_where.x,
...
void Object::shroud() {
  if (!m_partitionLastShroud->isInvalid()) {
    DEBUG_CRASH(
        ("An Object is shrouding, but hasn't unshrouded the last one."));
    return;
  }

  Player *controller = getControllingPlayer();
  if (controller) {
    // things under construction don't  shroud. (srj), nor do dead or blind
    // things
    if (((getStatusBits() & OBJECT_STATUS_UNDER_CONSTRUCTION) == 0) &&
        (!isEffectivelyDead()) && (getShroudRange() > 0.0f)) {
      PlayerMaskType shroudingMask = 0;
      for (Int currentIndex = ThePlayerList->getPlayerCount() - 1;
           currentIndex >= 0; currentIndex--) {
        const Player *currentPlayer = ThePlayerList->getNthPlayer(currentIndex);
        // Build mask of NON-allies.  This is the Object-centric game level that
        // cares
        if (getControllingPlayer()->getRelationship(
                currentPlayer->getDefaultTeam()) != ALLIES) {
...
void Object::unshroud() {
  if (m_partitionLastShroud->isInvalid()) {
    // Your very first action will be an unlook, so of course you haven't looked
    // yet.  This is not an error This early return prevents an extra unlook if
    // you never looked.  Like you have 0 shroud generation.
    return;
  }

  ThePartitionManager->undoShroudCover(
      m_partitionLastShroud->m_where.x, m_partitionLastShroud->m_where.y,
      m_partitionLastShroud->m_howFar, m_partitionLastShroud->m_forWhom);
...
Real Object::getVisionRange() const {
  ...
  if (TheGlobalData->m_debugVisibility) {
    ...
    for (int i = 0; i < TheGlobalData->m_debugVisibilityTileCount; ++i) {
      pos.Rotate_Z(1.0f * i / TheGlobalData->m_debugVisibilityTileCount * 2 *
                   PI);
      Coord3D coord = {pos.X + getPosition()->x, pos.Y + getPosition()->y,
                       pos.Z + getPosition()->z};
...
Real Object::getShroudClearingRange() const {
  ...
  if ((getStatusBits() &
       OBJECT_STATUS_UNDER_CONSTRUCTION)) {  // structures under construction
                                             // have limited vision range.  For
                                             // now, base it
...
void Object::setShroudClearingRange(Real newShroudClearingRange) {
  if (newShroudClearingRange != m_shroudClearingRange) {
    // The partition cell refresh is a slow operation, so only do it if you
    // really have to. Range change is a valid reason to relook.
    m_shroudClearingRange = newShroudClearingRange;

    /*
            Complete and total monkey hack fix.

            The problem: newObject doesn't get an initial pos, so all objects
       start at 0,0,0. Most code paths instantly move 'em to a good pos, but in
       some cases, that is too late: If we have search-and-destroy battle plan,
       we will apply it at that point, and clear out a vision range based on our
       current (wrong) location. Doh!

            So, this just sez: if you are at 0,0,0, don't call
       handlePartitionCellMaintenance()... since you will either (1) be moved
       elsewhere immediately, thus forcing it to be called via another route
       anyway, or (2) not be moved, which means you are a very naughty and
       worthless object anyway and we should just ignore you.

            Proper fix for next version is to require initial pos to be passed
       in to newObject so that all objects can start at their proper initial
       position from the start of the ctor.
...
//-------------------------------------------------------------------------------------------------
/** Given a special power template, find the module in the object that can
 * implement it. There can be at most one */
//-------------------------------------------------------------------------------------------------
SpecialPowerModuleInterface *Object::getSpecialPowerModule(
    const SpecialPowerTemplate *specialPowerTemplate) const {
  // sanity
  if (specialPowerTemplate == NULL) return NULL;
  ...
  for (BehaviorModule **m = m_behaviors; *m; ++m) {
    ...
    if (sp->isModuleForPower(specialPowerTemplate)) return sp;
  }
...
void Object::doSpecialPower(const SpecialPowerTemplate *specialPowerTemplate,
  ...
  // sanity
  if (!forced && TheSpecialPowerStore->canUseSpecialPower(
                     this, specialPowerTemplate) == FALSE)
    return;

  // get the module and execute
  SpecialPowerModuleInterface *mod =
      getSpecialPowerModule(specialPowerTemplate);
  if (mod) mod->doSpecialPower(commandOptions);
}

//-------------------------------------------------------------------------------------------------
/** Execute special power */
//-------------------------------------------------------------------------------------------------
void Object::doSpecialPowerAtObject(
    const SpecialPowerTemplate *specialPowerTemplate, Object *obj,
    UnsignedInt commandOptions, Bool forced) {
  if (isDisabled()) return;
  ...
  SpecialPowerModuleInterface *mod =
      getSpecialPowerModule(specialPowerTemplate);
  if (mod) mod->doSpecialPowerAtObject(obj, commandOptions);
}

//-------------------------------------------------------------------------------------------------
/** Execute special power */
//-------------------------------------------------------------------------------------------------
void Object::doSpecialPowerAtLocation(
    const SpecialPowerTemplate *specialPowerTemplate, const Coord3D *loc,
    UnsignedInt commandOptions, Bool forced) {
  if (isDisabled()) return;
  ...
  SpecialPowerModuleInterface *mod =
      getSpecialPowerModule(specialPowerTemplate);
  if (mod) mod->doSpecialPowerAtLocation(loc, commandOptions);
}

//-------------------------------------------------------------------------------------------------
/** Execute special power */
//-------------------------------------------------------------------------------------------------
void Object::doSpecialPowerAtMultipleLocations(
    const SpecialPowerTemplate *specialPowerTemplate, const Coord3D *locations,
    Int locCount, UnsignedInt commandOptions, Bool forced) {
  if (isDisabled()) return;
  ...
  if (commandButton) {
    switch (commandButton->getCommandType()) {
      case GUI_COMMAND_SPECIAL_POWER:
        if (commandButton->getSpecialPowerTemplate()) {
          CommandOption commandOptions =
              (CommandOption)(commandButton->getOptions() |
                              COMMAND_FIRED_BY_SCRIPT);
          doSpecialPower(commandButton->getSpecialPowerTemplate(),
                         commandOptions, cmdSource == CMD_FROM_SCRIPT);
        }
        break;
      ...
      case GUI_COMMAND_SWITCH_WEAPON: {
        ...
        // GUI_COMMAND_SWITCH_WEAPON switches until un-switched, or switched to
        // something else.
      ...
      case GUI_COMMAND_FIRE_WEAPON: {
        WeaponSlotType weaponSlot = commandButton->getWeaponSlot();
        // GUI_COMMAND_FIRE_WEAPON merely fires till the weapon is empty or the
        // attack is "done"
      ...
      case GUI_COMMAND_PLAYER_UPGRADE: {
        ...
        DEBUG_ASSERTCRASH(
            upgradeT,
            ("Undefined upgrade '%s' in player upgrade command\n", "UNKNOWN"));
        // sanity
        if (upgradeT == NULL) break;
        if (upgradeT->getUpgradeType() == UPGRADE_TYPE_OBJECT) {
          if (hasUpgrade(upgradeT) || !affectedByUpgrade(upgradeT)) break;
        }
...
void Object::doCommandButtonAtObject(const CommandButton *commandButton,
                                     Object *obj, CommandSourceType cmdSource) {
  if (isDisabled()) return;
      ...
      case GUI_COMMAND_SPECIAL_POWER: {
        if (commandButton->getSpecialPowerTemplate()) {
          CommandOption commandOptions =
              (CommandOption)(commandButton->getOptions() |
                              COMMAND_FIRED_BY_SCRIPT);
          doSpecialPowerAtObject(commandButton->getSpecialPowerTemplate(), obj,
                                 commandOptions, cmdSource == CMD_FROM_SCRIPT);
        }
        break;
      }
...
void Object::doCommandButtonAtPosition(const CommandButton *commandButton,
  ...
  if (commandButton) {
    switch (commandButton->getCommandType()) {
      case GUI_COMMAND_SPECIAL_POWER: {
        if (commandButton->getSpecialPowerTemplate()) {
          CommandOption commandOptions =
              (CommandOption)(commandButton->getOptions() |
                              COMMAND_FIRED_BY_SCRIPT);
          doSpecialPowerAtLocation(commandButton->getSpecialPowerTemplate(),
                                   pos, commandOptions,
                                   cmdSource == CMD_FROM_SCRIPT);
        }
        break;
      }
      case GUI_COMMAND_ATTACK_MOVE:
        if (ai) {
          ai->aiAttackMoveToPosition(pos, commandButton->getMaxShotsToFire(),
                                     cmdSource);
        }
        break;
      case GUI_COMMAND_STOP:
      ...
      case GUI_COMMAND_DOZER_CONSTRUCT:
        TheBuildAssistant->buildObjectNow(this,
                                          commandButton->getThingTemplate(),
                                          pos, 0.0f, getControllingPlayer());
        break;
...
// ------------------------------------------------------------------------------------------------
/** Search our update modules for a production update interface and return it if
 * one is found */
// ------------------------------------------------------------------------------------------------
...
DockUpdateInterface *Object::getDockUpdateInterface(void) {
  ...
  for (BehaviorModule **u = m_behaviors; *u; ++u) {
    if ((dock = (*u)->getDockUpdateInterface()) != NULL) return dock;
  }
...
// ------------------------------------------------------------------------------------------------
SpecialPowerModuleInterface *Object::findSpecialPowerModuleInterface(
...
// ------------------------------------------------------------------------------------------------
// Simply find the special power module that is currently allowing plotting of
// positions to target.
// ------------------------------------------------------------------------------------------------
SpecialPowerUpdateInterface *
Object::findSpecialPowerWithOverridableDestinationActive(
    SpecialPowerType type) const {
  for (BehaviorModule **u = m_behaviors; *u; ++u) {
    SpecialPowerUpdateInterface *spInterface =
        (*u)->getSpecialPowerUpdateInterface();
    if (spInterface) {
...
// ------------------------------------------------------------------------------------------------
SpecialAbilityUpdate *Object::findSpecialAbilityUpdate(
    SpecialPowerType type) const {
  for (BehaviorModule **u = m_behaviors; *u; ++u) {
    SpecialPowerUpdateInterface *spInterface =
        (*u)->getSpecialPowerUpdateInterface();
...
SpecialPowerCompletionDie *Object::findSpecialPowerCompletionDie() const {
  static NameKeyType key_SpecialPowerCompletionDie =
      NAMEKEY("SpecialPowerCompletionDie");
...
Int Object::getNumConsecutiveShotsFiredAtTarget(const Object *victim) const {
  return m_firingTracker
             ? m_firingTracker->getNumConsecutiveShotsAtVictim(victim)
             : 0;
}
...
Bool Object::getSingleLogicalBonePosition(const char *boneName,
  ...
  if (m_drawable && m_drawable->getPristineBonePositions(boneName, 0, position,
                                                         transform, 1) == 1) {
    m_drawable->convertBonePosToWorldPos(position, transform, position,
                                         transform);
    return true;
  } else {
    if (position) *position = *getPosition();
    if (transform) *transform = *getTransformMatrix();
    return false;
  }
}
...
Bool Object::getSingleLogicalBonePositionOnTurret(WhichTurretType whichTurret,
  ...
  if (getDrawable() == NULL || getAI() == NULL) return FALSE;

  // We need to find the TurretBone's pristine position.
  getDrawable()->getProjectileLaunchOffset(PRIMARY_WEAPON, 1, NULL, whichTurret,
                                           &turretPosition, NULL);
  // And the required bone's pristine position
  if (getDrawable()->getPristineBonePositions(boneName, 0, &bonePosition, NULL,
                                              1) != 1)
    return FALSE;
  // Then we mojo the Logic position of the required bone like Missile firing
  // does.  Using the logic twist of the turret
  Real turretRotation;
  getAI()->getTurretRotAndPitch(whichTurret, &turretRotation, NULL);

  Matrix3D boneOffset(
      TRUE);  // This will be from the turret to the requested bone

  //	Vector3 bonePositionVector(	bonePosition.x - turretPosition.x,
  //															bonePosition.y
  //- turretPosition.y, 															bonePosition.z - turretPosition.z );
  Vector3 bonePositionVector(bonePosition.x, bonePosition.y, bonePosition.z);
  boneOffset.Translate(bonePositionVector);

  Matrix3D turnAdjustment(
      TRUE);  // this is the turret twist to be applied to the final answer

  turnAdjustment.Translate(turretPosition.x, turretPosition.y,
                           turretPosition.z);
...
Bool Object::canProduceUpgrade(const UpgradeTemplate *upgrade) {
  // We need to have the button to make the upgrade.  CommandSets are a weird
  // Logic/Client hybrid.
  ...
  for (Int buttonIndex = 0; buttonIndex < MAX_COMMANDS_PER_SET; buttonIndex++) {
    ...
    if (button &&
        ((button->getCommandType() == GUI_COMMAND_PLAYER_UPGRADE) ||
         (button->getCommandType() ==
          GUI_COMMAND_OBJECT_UPGRADE))  // Or else a button that requires an
                                        // upgrade will appear the same as a
                                        // button that gives an upgrade
        && button->getUpgradeTemplate() &&
        (button->getUpgradeTemplate() == upgrade))
      return TRUE;  // getUpgradeTemplate only returns something if it is
                    // actually an upgrade
  }
...
void Object::defect(Team *newTeam, UnsignedInt detectionTime) {
  if (isContained())  //@todo (KRIS?) make contained units unselectable, until
                      //then... lorenzen
  {
    return;
  }
  ...
  // But don't do this is if the new team is not a real team.  "'Enemy'
  // infiltration" wouldn't make sense, and we are probably just reverting a
  // cave or something.
  if (friend_getRadarData() &&
  ...
  if (m_defectionHelper) m_defectionHelper->startDefectionTimer(detectionTime);
  ...
  // make the new recruit the only selected thing, awaiting new command to move,
  // attack, etc...
  Drawable *dr = getDrawable();
  if (dr) {
    dr->flashAsSelected();  // This is the first of several flashes which get
                            // cue'd by doDefectorUpdateStuff()
    AudioEventRTS defectorTimerSound =
        TheAudio->getMiscAudio()->m_defectorTimerTickSound;
  ...
  for (Object *mine = TheGameLogic->getFirstObject(); mine;
       mine = mine->getNextObject()) {
...
RadarPriorityType Object::getRadarPriority(void) const {
  ...
  // certain properties ... here we will check for those properties unless the
  // INI setting of "not on radar" has been manually entered which explicitly
  // forbids an object from being on the radar ... by default objects get an
  // "invalid" priority on the radar and this means that we are free to decide
  // one here if we want
  //
  if (priority == RADAR_PRIORITY_INVALID) {
    ...
    if (cmi && cmi->isGarrisonable()) priority = RADAR_PRIORITY_STRUCTURE;

    // objects that are "capturable" show up on the radar
    if (isKindOf(KINDOF_CAPTURABLE)) priority = RADAR_PRIORITY_STRUCTURE;

  }  // end if

  // Carbombs will show up as units regardless of their default priority
  if (testStatus(OBJECT_STATUS_IS_CARBOMB)) priority = RADAR_PRIORITY_UNIT;
...
// ------------------------------------------------------------------------------------------------
AIGroup *Object::getGroup(void) { return m_group; }
