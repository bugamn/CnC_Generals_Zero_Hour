...
static void parseAllVetLevelsAsciiString(INI* ini, void* /*instance*/,
  ...
  for (Int i = LEVEL_FIRST; i <= LEVEL_LAST; ++i) s[i] = a;
}
...
static void parseAllVetLevelsFXList(INI* ini, void* /*instance*/, void* store,
  ...
  const FXList* fx = NULL;
  INI::parseFXList(ini, NULL, &fx, NULL);
  for (Int i = LEVEL_FIRST; i <= LEVEL_LAST; ++i) s[i] = fx;
}
...
static void parseAllVetLevelsPSys(INI* ini, void* /*instance*/, void* store,
  ...
  ConstParticleSystemTemplatePtr pst = NULL;
  INI::parseParticleSystemTemplate(ini, NULL, &pst, NULL);
  for (Int i = LEVEL_FIRST; i <= LEVEL_LAST; ++i) s[i] = pst;
}
...
const FieldParse WeaponTemplate::TheWeaponTemplateFieldParseTable[] = {
    ...
    {"WeaponSpeed", INI::parseVelocityReal, NULL,
     offsetof(WeaponTemplate, m_weaponSpeed)},
    {"MinWeaponSpeed", INI::parseVelocityReal, NULL,
     offsetof(WeaponTemplate, m_minWeaponSpeed)},
    {"ScaleWeaponSpeed", INI::parseBool, NULL,
     offsetof(WeaponTemplate, m_isScaleWeaponSpeed)},
    {"WeaponRecoil", INI::parseAngleReal, NULL,
     offsetof(WeaponTemplate, m_weaponRecoil)},
    {"MinTargetPitch", INI::parseAngleReal, NULL,
     offsetof(WeaponTemplate, m_minTargetPitch)},
    {"MaxTargetPitch", INI::parseAngleReal, NULL,
     offsetof(WeaponTemplate, m_maxTargetPitch)},
    {"ProjectileObject", INI::parseAsciiString, NULL,
     offsetof(WeaponTemplate, m_projectileName)},
    {"FireSound", INI::parseAudioEventRTS, NULL,
     offsetof(WeaponTemplate, m_fireSound)},
    {"FireSoundLoopTime", INI::parseDurationUnsignedInt, NULL,
     offsetof(WeaponTemplate, m_fireSoundLoopTime)},
    {"FireFX", parseAllVetLevelsFXList, NULL,
     offsetof(WeaponTemplate, m_fireFXs)},
    {"ProjectileDetonationFX", parseAllVetLevelsFXList, NULL,
     offsetof(WeaponTemplate, m_projectileDetonateFXs)},
    {"FireOCL", parseAllVetLevelsAsciiString, NULL,
     offsetof(WeaponTemplate, m_fireOCLNames)},
    {"ProjectileDetonationOCL", parseAllVetLevelsAsciiString, NULL,
     offsetof(WeaponTemplate, m_projectileDetonationOCLNames)},
    {"ProjectileExhaust", parseAllVetLevelsPSys, NULL,
     offsetof(WeaponTemplate, m_projectileExhausts)},
    {"VeterancyFireFX", parsePerVetLevelFXList, NULL,
     offsetof(WeaponTemplate, m_fireFXs)},
    {"VeterancyProjectileDetonationFX", parsePerVetLevelFXList, NULL,
     offsetof(WeaponTemplate, m_projectileDetonateFXs)},
    {"VeterancyFireOCL", parsePerVetLevelAsciiString, NULL,
     offsetof(WeaponTemplate, m_fireOCLNames)},
    {"VeterancyProjectileDetonationOCL", parsePerVetLevelAsciiString, NULL,
     offsetof(WeaponTemplate, m_projectileDetonationOCLNames)},
    {"VeterancyProjectileExhaust", parsePerVetLevelPSys, NULL,
     offsetof(WeaponTemplate, m_projectileExhausts)},
    {"ClipSize", INI::parseInt, NULL, offsetof(WeaponTemplate, m_clipSize)},
    {"ContinuousFireOne", INI::parseInt, NULL,
     offsetof(WeaponTemplate, m_continuousFireOneShotsNeeded)},
    {"ContinuousFireTwo", INI::parseInt, NULL,
     offsetof(WeaponTemplate, m_continuousFireTwoShotsNeeded)},
    {"ContinuousFireCoast", INI::parseDurationUnsignedInt, NULL,
     offsetof(WeaponTemplate, m_continuousFireCoastFrames)},
    {"AutoReloadWhenIdle", INI::parseDurationUnsignedInt, NULL,
     offsetof(WeaponTemplate, m_autoReloadWhenIdleFrames)},
    {"ClipReloadTime", INI::parseDurationUnsignedInt, NULL,
     offsetof(WeaponTemplate, m_clipReloadTime)},
    {"DelayBetweenShots", WeaponTemplate::parseShotDelay, NULL, 0},
    {"ShotsPerBarrel", INI::parseInt, NULL,
     offsetof(WeaponTemplate, m_shotsPerBarrel)},
    {"DamageDealtAtSelfPosition", INI::parseBool, NULL,
    ...
    {"AntiAirborneVehicle", INI::parseBitInInt32,
     (void*)WEAPON_ANTI_AIRBORNE_VEHICLE, offsetof(WeaponTemplate, m_antiMask)},
    ...
    {"AntiProjectile", INI::parseBitInInt32, (void*)WEAPON_ANTI_PROJECTILE,
     offsetof(WeaponTemplate, m_antiMask)},
    {"AntiSmallMissile", INI::parseBitInInt32, (void*)WEAPON_ANTI_SMALL_MISSILE,
     offsetof(WeaponTemplate, m_antiMask)},
    ...
    {"ContinueAttackRange", INI::parseReal, NULL,
     offsetof(WeaponTemplate, m_continueAttackRange)},
    {"SuspendFXDelay", INI::parseDurationUnsignedInt, NULL,
...
WeaponTemplate::~WeaponTemplate() {
  ...
  if (m_extraBonus) m_extraBonus->deleteInstance();
}

// ------------------------------------------------------------------------------------------------
void WeaponTemplate::reset(void) { m_historicDamage.clear(); }  // end reset
...
/*static*/ void WeaponTemplate::parseWeaponBonusSet(INI* ini, void* instance,
  ...
  if (!self->m_extraBonus) self->m_extraBonus = newInstance(WeaponBonusSet);
...
Int WeaponTemplate::getDelayBetweenShots(const WeaponBonus& bonus) const {
  ...
  // val:%d, bonusROF=%g/%8.8X\n", m_minDelayBetweenShots,
  // m_maxDelayBetweenShots, delayToUse, bonusROF, AS_INT(bonusROF)));
...
Real WeaponTemplate::estimateWeaponTemplateDamage(
  ...
  // this stays, even if ALLOW_SURRENDER is not defed, since flashbangs still
  // use 'em
...
Bool WeaponTemplate::shouldProjectileCollideWith(
  ...
  if (!projectile || !thingWeCollidedWith) return false;

  // if it's our intended victim, we want to collide with it, regardless of any
  // other considerations.
  if (intendedVictimID == thingWeCollidedWith->getID()) return true;

  if (projectileLauncher != NULL) {
    // Don't hit your own launcher, ever.
    if (projectileLauncher == thingWeCollidedWith) return false;
    ...
    if (launcherContainedBy == thingWeCollidedWith) return false;
  }
  ...
  if (thingWeCollidedWith->isKindOf(KINDOF_AIRFIELD)) {
    ...
    for (BehaviorModule** i = thingWeCollidedWith->getBehaviorModules(); *i;
      ...
      if (pp != NULL && pp->hasReservedSpace(intendedVictimID)) return false;
    }
  }
  ...
  if (thingWeCollidedWith->isKindOf(KINDOF_BALLISTIC_MISSILE))
    requiredMask |= WEAPON_COLLIDE_BALLISTIC_MISSILES;  // All missiles are also
                                                        // projectiles!

  // if any in requiredMask are present in collidemask, do the collision. (srj)
  if ((getProjectileCollideMask() & requiredMask) != 0) return true;
...
UnsignedInt WeaponTemplate::fireWeaponTemplate(
    ...
    const Object* victimObj, const Coord3D* victimPos, const WeaponBonus& bonus,
    Bool isProjectileDetonation, Bool ignoreRanges, Weapon* firingWeapon,
    ObjectID* projectileID) const {
  ...
  DEBUG_ASSERTCRASH(
      (m_primaryDamage > 0) || (victimObj == NULL),
  ...
  if (!ignoreRanges && !isLeechRangeWeapon()) {
    ...
    if (distSqr > attackRangeSqr) {
      ...
      // victim is out of range (%f vs %f) of this weapon -- why did we attempt
  ...
  if (sourceObj && sourceObj->getDrawable()) {
    ...
    if (TheGameLogic->getFrame() < firingWeapon->getSuspendFXFrame()) fx = NULL;

    Bool handled;

    if (!sourceObj
             ->isLocallyControlled()  // if user watching is not controller and
        &&
        sourceObj->testStatus(OBJECT_STATUS_STEALTHED)  // if unit is stealthed
                                                        // (like a Pathfinder)
        &&
        ...
        && !isPlayFXWhenStealthed()           // and not a weapon marked to
                                              // playwhenstealthed
    ) {
      handled =
  ...
  if (sourceObj) {
    ...
    if (oclToUse) ObjectCreationList::create(oclToUse, sourceObj, NULL);
  }
  ...
  } else  // must be a projectile
  {
    ...
    if (projectileID) *projectileID = projectile->getID();
    ...
    firingWeapon->newProjectileFired(
        sourceObj,
        projectile);  // The actual logic weapon needs to know this was created.

    if (m_scatterRadius > 0.0f || m_infantryInaccuracyDist > 0.0f &&
                                      victimObj &&
                                      victimObj->isKindOf(KINDOF_INFANTRY)) {
      // This weapon scatters, so clear the victimObj, as we are no longer
      // shooting it directly, and find a random point within the radius to
      // shoot at as victimPos
    ...
    for (BehaviorModule** u = projectile->getBehaviorModules(); *u; ++u) {
      if ((pui = (*u)->getProjectileUpdateInterface()) != NULL) break;
    }
    if (pui) {
      VeterancyLevel v = sourceObj->getVeterancyLevel();
...
void WeaponTemplate::dealDamageInternal(ObjectID sourceID, ObjectID victimID,
  ...
  if (getProjectileTemplate() == NULL || isProjectileDetonation) {
    ...
    if (radius > 0.0f) {
      ...
      // DEBUG_ASSERTCRASH(primaryVictim != NULL, ("weapons without radii should
      // always pass in specific victims"));
    ...
    for (; curVictim != NULL;
         curVictim = iter ? iter->nextWithNumeric(&curVictimDistSqr) : NULL) {
      Bool killSelf = false;
      if (source != NULL) {
        ...
        if (curVictim != primaryVictim) {
          ...
          if ((affects & WEAPON_AFFECTS_SELF) == 0) {
            // Remember that source is a missile for some units, and they don't
            // want to injure them'selves' either
            if (source == curVictim ||
          ...
          if (affects & WEAPON_DOESNT_AFFECT_SIMILAR) {
            ...
            // kill nearby members that are the same type as us. A good example
            // are a group of terrorists blowing themselves up. We don't want to
            // cause a domino effect that kills all of them.
          ...
          /*
                  The idea here is: if its our ally(/enemies), AND it's not the
             direct target, AND the weapon doesn't do radius-damage to
...
const WeaponTemplate* WeaponStore::findWeaponTemplate(AsciiString name) const {
  if (stricmp(name.str(), "None") == 0) return NULL;
...
/*static*/ void WeaponStore::parseWeaponTemplateDefinition(INI* ini) {
  ...
  if (weapon->m_projectileName.isNone()) weapon->m_projectileName.clear();

#if defined(_DEBUG) || defined(_INTERNAL)
  if (!weapon->getFireSound().getEventName().isEmpty() &&
      weapon->getFireSound().getEventName().compareNoCase("NoSound") != 0) {
    DEBUG_ASSERTCRASH(
        TheAudio->isValidAudioEvent(&weapon->getFireSound()),
        ("Invalid FireSound %s in Weapon '%s'.",
         weapon->getFireSound().getEventName().str(), weapon->getName().str()));
  }
#endif
}
...
void Weapon::computeBonus(const Object* source,
  ...
  if (extra) extra->appendBonuses(flags, bonus);
}
...
void Weapon::setClipPercentFull(Real percent, Bool allowReduction) {
  if (m_template->getClipSize() == 0) return;
...
Bool Weapon::isTooClose(const Object* source, const Object* target) const {
  Real minAttackRange = m_template->getMinimumAttackRange();
  if (minAttackRange == 0.0f) return false;
...
void Weapon::newProjectileFired(const Object* sourceObj,
  ...
  if (projectileStream == NULL) {
    ...
    if (projectileStream == NULL) return;
  ...
  if (draw) {
    ...
    if (update) {
      ...
      if (victimObj && !victimObj->isKindOf(KINDOF_PROJECTILE)) {
        // Infantry targets are positioned on the ground, so raise the beam up
        // so we're not shooting their feet. Projectiles are a different story,
        // target their exact position.
        pos.z += 10.0f;
      }
...
Bool Weapon::privateFireWeapon(const Object* sourceObj, Object* victimObj,
  ...
  if (projectileID) *projectileID = INVALID_ID;
  ...
  if (getStatus() != READY_TO_FIRE) return false;
...
Bool Weapon::isWithinTargetPitch(const Object* source,
  ...
  if (isContactWeapon() || !isPitchLimited()) return true;
...
Int Weapon::getPreAttackDelay(const Object* source,
  ...
  if (type == PREFIRE_PER_CLIP) {
    ...
    if (source->getNumConsecutiveShotsFiredAtTarget(victim) > 0)
      return 0;  // I only delay once an attack, and I have already shot this
                 // guy
  }
...
static void makeAssistanceRequest(Object* requestOf, void* userData) {
  ...
  if (requestOf == requestData->m_requestingObject) return;
  ...
  if (distSq > requestData->m_requestDistanceSquared) return;
  ...
  if (assistModule == NULL) return;

  // and say yes
  if (!assistModule->isFreeToAssist()) return;
...
/*static*/ void Weapon::calcProjectileLaunchPosition(const Object* launcher,
  ...
  if (!draw || !draw->getProjectileLaunchOffset(
    ...
    // specificBarrelToUse));
    DEBUG_CRASH(
        ("ProjectileLaunchPos %d %d not found!\n", wslot, specificBarrelToUse));
...
Bool Weapon::isClearFiringLineOfSightTerrain(const Object* source,
  ...
  return ThePartitionManager->isClearLineOfSightTerrain(NULL, origin, NULL,
                                                        victimPos);
}
...
Bool Weapon::isClearGoalFiringLineOfSightTerrain(const Object* source,
  ...
  // CRCDEBUG_LOG(("Weapon::isClearGoalFiringLineOfSightTerrain(Object) for
  // %s\n", DescribeObject(source).str())); DUMPCOORD3D(&origin);
  getFiringLineOfSightOrigin(source, origin);
  Coord3D victimPos;
  victim->getGeometryInfo().getCenterPosition(*victim->getPosition(),
...
void Weapon::xfer(Xfer* xfer) {
  ...
  if (version >= 2) {
    ...
    if (xfer->getXferMode() == XFER_LOAD) {
      ...
      if (m_template == NULL) throw INI_INVALID_DATA;
    }
  }
...
//-------------------------------------------------------------------------------------------------
/*static*/ void WeaponBonusSet::parseWeaponBonusSetPtr(
...
void WeaponBonusSet::appendBonuses(WeaponBonusConditionFlags flags,
                                   WeaponBonus& bonus) const {
  if (flags == 0) return;  // my, that was easy
