...
// Desc:   Keeps track of shots fired and people targeted for weapons that want
// a history of such a thing
///////////////////////////////////////////////////////////////////////////////////////////////////
...
Int FiringTracker::getNumConsecutiveShotsAtVictim(const Object *victim) const {
  ...
  if (victim->getID() != m_victimID) return 0;  // nope, not shooting at him

  return m_consecutiveShots;  // this is how any times I have shot at this hoser
                              // right now
}
...
void FiringTracker::shotFired(const Weapon *weaponFired, ObjectID victimID) {
  ...
  if (victimID == m_victimID) {
    ...
    // Switching targets within the coast time is valid, and we will not spin
    // down
    ++m_consecutiveShots;
    m_victimID = victimID;
  } else {
  ...
  if (autoReloadDelay > 0) m_frameToForceReload = now + autoReloadDelay;
  ...
  if (getObject()->testWeaponBonusCondition(
    ...
    // Check to go up
    if (m_consecutiveShots > shotsNeededOne) speedUp();
  }
...
UpdateSleepTime FiringTracker::update() {
  // DEBUG_ASSERTCRASH(m_frameToStartCooldown != 0 || m_frameToStopLoopingSound
  // != 0, ("hmm, should be asleep"));

  UnsignedInt now = TheGameLogic->getFrame();

  // I have been idle long enough that I should reload, so I do not hang around
  // with a near empty clip forever.
  ...
  if (m_frameToStartCooldown != 0 && now > m_frameToStartCooldown) {
    ...
    coolDown();  // if this is the coolest call to cooldown, it will set
                 // m_frameToStartCooldown to zero
...
UpdateSleepTime FiringTracker::calcTimeToSleep() {
  ...
  if (m_frameToStopLoopingSound == 0 && m_frameToStartCooldown == 0 &&
      m_frameToForceReload == 0)
...
void FiringTracker::speedUp() {
  ...
  if (self->testWeaponBonusCondition(
          WEAPONBONUSCONDITION_CONTINUOUS_FIRE_FAST)) {
  ...
  } else if (self->testWeaponBonusCondition(
                 WEAPONBONUSCONDITION_CONTINUOUS_FIRE_MEAN)) {
    const AudioEventRTS *soundToPlayPtr =
        self->getTemplate()->getPerUnitSound("VoiceRapidFire");
