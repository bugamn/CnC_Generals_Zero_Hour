...
void DumbProjectileBehaviorModuleData::buildFieldParse(MultiIniFieldParse& p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"MaxLifespan", INI::parseDurationUnsignedInt, NULL,
       offsetof(DumbProjectileBehaviorModuleData, m_maxLifespan)},
      {"TumbleRandomly", INI::parseBool, NULL,
       offsetof(DumbProjectileBehaviorModuleData, m_tumbleRandomly)},
      {"DetonateCallsKill", INI::parseBool, NULL,
       offsetof(DumbProjectileBehaviorModuleData, m_detonateCallsKill)},
      {"OrientToFlightPath", INI::parseBool, NULL,
       offsetof(DumbProjectileBehaviorModuleData, m_orientToFlightPath)},

      {"FirstHeight", INI::parseReal, NULL,
       offsetof(DumbProjectileBehaviorModuleData, m_firstHeight)},
      {"SecondHeight", INI::parseReal, NULL,
       offsetof(DumbProjectileBehaviorModuleData, m_secondHeight)},
      {"FirstPercentIndent", INI::parsePercentToReal, NULL,
       offsetof(DumbProjectileBehaviorModuleData, m_firstPercentIndent)},
      {"SecondPercentIndent", INI::parsePercentToReal, NULL,
       offsetof(DumbProjectileBehaviorModuleData, m_secondPercentIndent)},

      {"GarrisonHitKillRequiredKindOf", KindOfMaskType::parseFromINI, NULL,
       offsetof(DumbProjectileBehaviorModuleData, m_garrisonHitKillKindof)},
      {"GarrisonHitKillForbiddenKindOf", KindOfMaskType::parseFromINI, NULL,
       offsetof(DumbProjectileBehaviorModuleData, m_garrisonHitKillKindofNot)},
      {"GarrisonHitKillCount", INI::parseUnsignedInt, NULL,
       offsetof(DumbProjectileBehaviorModuleData, m_garrisonHitKillCount)},
      {"GarrisonHitKillFX", INI::parseFXList, NULL,
       offsetof(DumbProjectileBehaviorModuleData, m_garrisonHitKillFX)},

      {"FlightPathAdjustDistPerSecond", INI::parseVelocityReal, NULL,
...
static Bool calcTrajectory(
  ...
  // let's start by aiming directly for it. we know this isn't right (unless
  // gravity is zero, which it's not) but is a good starting point...
  Real theta = atan2(dz, horizDist);
  // if the angle isn't pretty shallow, we can get a better initial guess by
  ...
  */
  ...
  const Real MIN_ANGLE_DIFF =
      (PI /
       (180.0f * 16.0f));  // 1/16th of a degree. yes, we need that accuracy.
                           // Int numLoops = 0;
  while (theta_max > theta_min + MIN_ANGLE_DIFF) {
    //++numLoops;
    pitches[0] = theta;  // shallower angle
    pitches[1] =
        (theta >= 0.0) ? (PI / 2 - theta) : (-PI / 2 - theta);  // steeper angle

    DEBUG_ASSERTCRASH(pitches[0] <= PI / 2 && pitches[0] >= -PI / 2,
                      ("bad pitches[0] %f\n", rad2deg(pitches[0])));
    DEBUG_ASSERTCRASH(pitches[1] <= PI / 2 && pitches[1] >= -PI / 2,
                      ("bad pitches[1] %f\n", rad2deg(pitches[1])));
    ...
    if (root < 0.0f) {
      ...
      if (preferred == 0)
        tooClose = true;  // if this fails for the shallow case, it's 'cuz the
                          // result is too close
...
void DumbProjectileBehavior::projectileLaunchAtObjectOrPosition(
    const Object* victim, const Coord3D* victimPos, const Object* launcher,
    WeaponSlotType wslot, Int specificBarrelToUse,
    const WeaponTemplate* detWeap,
    const ParticleSystemTemplate* exhaustSysOverride) {
  const DumbProjectileBehaviorModuleData* d =
      getDumbProjectileBehaviorModuleData();

  DEBUG_ASSERTCRASH(specificBarrelToUse >= 0,
                    ("specificBarrelToUse must now be explicit"));
  ...
  Weapon::positionProjectileForLaunch(projectile, launcher, wslot,
                                      specificBarrelToUse);

  projectileFireAtObjectOrPosition(victim, victimPos, detWeap,
                                   exhaustSysOverride);
}

//-------------------------------------------------------------------------------------------------
// The actual firing of the missile once setup. Uses a Bezier curve with points
// parameterized in ini
//-------------------------------------------------------------------------------------------------
void DumbProjectileBehavior::projectileFireAtObjectOrPosition(
  ...
  if (detWeap && detWeap->isScaleWeaponSpeed()) {
    ...
    Real range = sqrt(ThePartitionManager->getDistanceSquared(
        projectile, &victimPosToUse, FROM_CENTER_2D));
    Real rangeRatio = (range - minRange) / (maxRange - minRange);
    m_flightPathSpeed =
        (rangeRatio * (weaponSpeed - minWeaponSpeed)) + minWeaponSpeed;
  } else {
  ...
  m_currentFlightPathStep =
      0;  // We are at the first point, because the launching put us there
}
...
Bool DumbProjectileBehavior::calcFlightPath(Bool recalcNumSegments) {
  ...
  Bool onMap = ThePartitionManager->estimateTerrainExtremesAlongLine(
      controlPoints[0], controlPoints[3], NULL, &highestInterveningTerrain,
      NULL, NULL);
  if (!onMap) {
    return false;
  }

  // X and Y for inner points are along the line between us, so normalize and
  // scale a vector between us, but only use the x and y of the result
  ...
  Vector3 firstPointAlongLine =
      targetVector * (targetDistance * d->m_firstPercentIndent);
  Vector3 secondPointAlongLine =
      targetVector * (targetDistance * d->m_secondPercentIndent);

  controlPoints[1].x =
      firstPointAlongLine.X +
      controlPoints[0]
          .x;  // add world start to offset along the origin based vector
  ...
  // Z's are determined using the highest intervening height so they won't hit
  // hills, low end bounded by current Zs
  highestInterveningTerrain =
      max(highestInterveningTerrain, controlPoints[0].z);
  highestInterveningTerrain =
      max(highestInterveningTerrain, controlPoints[3].z);
  controlPoints[1].z = highestInterveningTerrain + d->m_firstHeight;
  controlPoints[2].z = highestInterveningTerrain + d->m_secondHeight;

  // With four control points, we have a curve.  We will decide how many frames
  // we want to take to get to the target, and fill our vector with those curve
  // points.
  ...
  DEBUG_ASSERTCRASH(m_flightPathSegments == m_flightPath.size(),
                    ("m_flightPathSegments mismatch"));

#if defined(_DEBUG) || defined(_INTERNAL)
  if (TheGlobalData->m_debugProjectilePath) displayFlightPath();
#endif
...
Bool DumbProjectileBehavior::projectileHandleCollision(Object* other) {
  ...
  if (other != NULL) {
    Object* projectileLauncher =
        TheGameLogic->findObjectByID(projectileGetLauncherID());

    // if it's not the specific thing we were targeting, see if we should
    // incidentally collide...
    if (!m_detonationWeaponTmpl->shouldProjectileCollideWith(
            projectileLauncher, getObject(), other, m_victimID)) {
      // DEBUG_LOG(("ignoring projectile collision with %s at frame
      ...
      if (contain && contain->getContainCount() > 0 &&
        ...
        if (items) {
          for (ContainedItemsList::const_iterator it = items->begin();
               *it != NULL && numKilled < d->m_garrisonHitKillCount;) {
            Object* thingToKill = *it++;
            if (!thingToKill->isEffectivelyDead() &&
                thingToKill->isKindOfMulti(d->m_garrisonHitKillKindof,
                                           d->m_garrisonHitKillKindofNot)) {
              // DEBUG_LOG(("Killed a garrisoned unit (%08lx %s) via
  ...
  // mark ourself as "no collisions" (since we might still exist in slow death
  // mode)
...
void DumbProjectileBehavior::detonate() {
  ...
  if (m_detonationWeaponTmpl) {
    TheWeaponStore->handleProjectileDetonation(
        m_detonationWeaponTmpl, obj, obj->getPosition(), m_extraBonusFlags);
...
UpdateSleepTime DumbProjectileBehavior::update() {
  ...
  if (m_currentFlightPathStep >= m_flightPath.size()) {
    // No more steps to use. Would go out of bounds on vector, so have to do
    // something. We could allow physics to take over and make us fall, but the
    // point of this whole task is to guarentee where the shell explodes.  This
    // way, it _will_ explode at the target point.
    detonate();
    return UPDATE_SLEEP_NONE;
  }
    ...
    if (victim) {
      ...
      if (distVictimMovedSqr > 0.1f) {
        ...
        if (!calcFlightPath(false)) {
          DEBUG_CRASH(
              ("Hmm, recalc of flight path returned false... should this "
               "happen?"));
          detonate();
          return UPDATE_SLEEP_NONE;
        }
  ...
  if (d->m_orientToFlightPath && (!d->m_tumbleRandomly) &&
      m_currentFlightPathStep > 0) {
    // this seems reasonable; however, if this object has a PhysicsBehavior on
    // it, this calc will be wrong, since Physics is applying gravity, which we
    // duly ignore, but the prevPos won't be what we expect. get it from the
    // flight path instead. (srj)
    // Coord3D prevPos = *getObject()->getPosition();

    Coord3D prevPos = m_flightPath[m_currentFlightPathStep - 1];

    Vector3 curDir(flightStep.x - prevPos.x, flightStep.y - prevPos.y,
                   flightStep.z - prevPos.z);
    curDir.Normalize();  // buildTransformMatrix wants it this way

    Matrix3D orientMtx;
    orientMtx.buildTransformMatrix(
        Vector3(flightStep.x, flightStep.y, flightStep.z), curDir);
    getObject()->setTransformMatrix(&orientMtx);
  } else {
    getObject()->setPosition(&flightStep);
  }

  // note that we want to use getHighestLayerForDestination() here, so that
  // anything even slightly below the bridge translates into GROUND.
  // (getLayerForDestination just does a "closest" check)
  PathfindLayerEnum oldLayer = getObject()->getLayer();
  PathfindLayerEnum newLayer = TheTerrainLogic->getHighestLayerForDestination(
  ...
  if (oldLayer != LAYER_GROUND && newLayer == LAYER_GROUND) {
    ...
    tmp.z = 9999.0f;
    PathfindLayerEnum testLayer =
        TheTerrainLogic->getHighestLayerForDestination(&tmp);
  ...
  return UPDATE_SLEEP_NONE;  // This no longer flys with physics, so it needs to
                             // not sleep
}
...
void DumbProjectileBehavior::displayFlightPath() {
  ...
  for (Int pointIndex = 0; pointIndex < m_flightPath.size(); ++pointIndex) {
    addIcon(&m_flightPath[pointIndex],
            TheGlobalData->m_debugProjectileTileWidth,
  ...
  if (xfer->getXferMode() == XFER_LOAD) {
    if (weaponTemplateName == AsciiString::TheEmptyString)
      ...
      if (m_detonationWeaponTmpl == NULL) {
        DEBUG_CRASH(
            ("DumbProjectileBehavior::xfer - Unknown weapon template '%s'\n",
