...
// Desc:   Behavior that reacts to poison Damage by continuously damaging us
// further in an Update
///////////////////////////////////////////////////////////////////////////////////////////////////
...
// tinting is all handled in drawable, now, Graham look near the bottom of
...
PoisonedBehaviorModuleData::PoisonedBehaviorModuleData() {
  ...
  m_poisonDurationData =
      0;  // And how long after the last poison dose I am poisoned
}

//-------------------------------------------------------------------------------------------------
/*static*/ void PoisonedBehaviorModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  static const FieldParse dataFieldParse[] = {
      {"PoisonDamageInterval", INI::parseDurationUnsignedInt, NULL,
       offsetof(PoisonedBehaviorModuleData, m_poisonDamageIntervalData)},
      {"PoisonDuration", INI::parseDurationUnsignedInt, NULL,
       offsetof(PoisonedBehaviorModuleData, m_poisonDurationData)},
      {0, 0, 0, 0}};
...
UpdateSleepTime PoisonedBehavior::update() {
  ...
  if (m_poisonDamageFrame != 0 && now >= m_poisonDamageFrame) {
    ...
    damage.in.m_damageType =
        DAMAGE_UNRESISTABLE;  // Not poison, as that will infect us again
  ...
  // unless the poison killed us, then we continue to be a pulsating toxic pus
  // ball
  if (m_poisonOverallStopFrame != 0 && now >= m_poisonOverallStopFrame &&
...
UpdateSleepTime PoisonedBehavior::calcSleepTime() {
  // UPDATE_SLEEP requires a count-of-frames, not an absolute-frame, so subtract
  // 'now'
  UnsignedInt now = TheGameLogic->getFrame();
...
void PoisonedBehavior::startPoisonedEffects(const DamageInfo *damageInfo) {
  ...
  // We are going to take the damage dealt by the original poisoner every so
  // often for a while.
  m_poisonDamageAmount = damageInfo->out.m_actualDamageDealt;

  m_poisonOverallStopFrame = now + d->m_poisonDurationData;

  // If we are getting re-poisoned, don't reset the damage counter if running,
  // but do set it if unset
  if (m_poisonDamageFrame != 0)
    m_poisonDamageFrame =
        min(m_poisonDamageFrame, now + d->m_poisonDamageIntervalData);
  else
    m_poisonDamageFrame = now + d->m_poisonDamageIntervalData;
  ...
  if (myDrawable)
    myDrawable->setTintStatus(
        TINT_STATUS_POISONED);  // Graham, It has changed, see UpdateDrawable()
