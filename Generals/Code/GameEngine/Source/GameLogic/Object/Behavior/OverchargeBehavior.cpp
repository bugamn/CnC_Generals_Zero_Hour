...
// /////////////////////////////////////////////////////////////////// Author:
// Colin Day, June 2002 Desc:   Objects with this behavior module will get the
// ability to produce more power
//				 for a short amount of time, during this
//"overcharge" state object health is 				 slowly reduced
///////////////////////////////////////////////////////////////////////////////////////////////////
...
//-------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
/*static*/ void OverchargeBehaviorModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  UpdateModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"HealthPercentToDrainPerSecond", INI::parsePercentToReal, NULL,
       offsetof(OverchargeBehaviorModuleData, m_healthPercentToDrainPerSecond)},
      {"NotAllowedWhenHealthBelowPercent", INI::parsePercentToReal, NULL,
       offsetof(OverchargeBehaviorModuleData,
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
OverchargeBehavior::~OverchargeBehavior(void) {}  // end ~OverchargeBehavior
...
UpdateSleepTime OverchargeBehavior::update(void) {
  ...
  if (m_overchargeActive) {
    ...
    // get mod data
    const OverchargeBehaviorModuleData *modData =
        getOverchargeBehaviorModuleData();

    // do some damage
    BodyModuleInterface *body = us->getBodyModule();
    DamageInfo damageInfo;
    damageInfo.in.m_amount =
        (body->getMaxHealth() * modData->m_healthPercentToDrainPerSecond) /
    ...
    if (body->getHealth() <
        body->getMaxHealth() * modData->m_notAllowedWhenHealthBelowPercent) {
...
// ------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void OverchargeBehavior::onDamage(DamageInfo *damageInfo) {}  // end onDie
...
void OverchargeBehavior::enable(Bool enable) {
  ...
  if (enable == FALSE) {
    ...
    if (m_overchargeActive == TRUE) {
      // make sure to NOT extend rods for purpose of maintaining proper model
      // condition
      PowerPlantUpdateInterface *ppui;
      for (BehaviorModule **umi = getObject()->getBehaviorModules(); *umi;
           ++umi) {
        ppui = (*umi)->getPowerPlantUpdateInterface();
        if (ppui) ppui->extendRods(FALSE);
      }

      Player *player = us->getControllingPlayer();
      if (player) player->removePowerBonus(us);
    ...
    if (m_overchargeActive == FALSE) {
      // make sure to extend rods for purpose of maintaining proper model
      // condition
      PowerPlantUpdateInterface *ppui;
      for (BehaviorModule **umi = getObject()->getBehaviorModules(); *umi;
           ++umi) {
        ppui = (*umi)->getPowerPlantUpdateInterface();
        if (ppui) ppui->extendRods(TRUE);
      }

      // add the power bonus
      Player *player = us->getControllingPlayer();
      if (player) player->addPowerBonus(us);
...
void OverchargeBehavior::onDelete(void) {
  ...
  if (player) player->removePowerBonus(getObject());
...
void OverchargeBehavior::onCapture(Player *oldOwner, Player *newOwner) {
  ...
  if (oldOwner) oldOwner->removePowerBonus(getObject());

  // add power bonus to the new owner
  if (newOwner) newOwner->addPowerBonus(getObject());
...
void OverchargeBehavior::loadPostProcess(void) {
  ...
  // Our effect is a fire and forget effect, not an upgrade state that is itself
  // saved, so need to re-fire.
