...
static void checkForAutoHeal(Object *testObj, void *userData) {
  ...
  if (testObj->isEffectivelyDead()) return;

  if (testObj->getControllingPlayer() !=
      helper->m_theHealer->getControllingPlayer())
    return;

  if (testObj->isOffMap()) return;

  if (!testObj->isAnyKindOf(helper->m_kindOfToTest)) return;

  if (testObj->getBodyModule()->getHealth() >=
      testObj->getBodyModule()->getMaxHealth())
    return;
...
AutoHealBehavior::AutoHealBehavior(Thing *thing, const ModuleData *moduleData)
...

  {
    if (d->m_radiusParticleSystemTmpl) {
      ...
      particleSystem = TheParticleSystemManager->createParticleSystem(
          d->m_radiusParticleSystemTmpl);
      if (particleSystem) {
...
void AutoHealBehavior::onDamage(DamageInfo *damageInfo) {
  ...
  if (isUpgradeActive() && d->m_radius == 0.0f) {
    ...
    if (d->m_startHealingDelay > 0) {
      ...
      // We can only force an immediate wake if we are ready to heal.  Otherwise
      // we will heal on a timer AND at every damage input.
...
UpdateSleepTime AutoHealBehavior::update(void) {
  if (m_stopped) return UPDATE_SLEEP_FOREVER;

  Object *obj = getObject();
  const AutoHealBehaviorModuleData *d = getAutoHealBehaviorModuleData();

  // do not heal if our status bit is not on.
  // do not heal if our status is effectively dead.  There ain't no coming back,
  // man!
  ...
  if (d->m_affectsWholePlayer) {
    // Even newer system, I can ignore radius and iterate objects on the owning
    // player.  Faster than scanning range 10,000,000
    ...
    if (owningPlayer) {
      ...
      // Smack all objects with this function, and we will end up with a list of
      // Objects deserving of pulseHealObject
      owningPlayer->iterateObjects(checkForAutoHeal, &helper);

      for (ObjectPointerListIterator iter = objectsToHeal.begin();
           iter != objectsToHeal.end(); ++iter) {
    ...
    PartitionFilterRelationship relationship(
        obj, PartitionFilterRelationship::ALLOW_ALLIES);
    ...
    ObjectIterator *iter = ThePartitionManager->iterateObjectsInRange(
        obj->getPosition(), d->m_radius, FROM_CENTER_2D, filters);
    MemoryPoolObjectHolder hold(iter);
    for (obj = iter->first(); obj; obj = iter->next()) {
      // do not heal if we are at max health already
      BodyModuleInterface *body = obj->getBodyModule();
      if (body->getHealth() < body->getMaxHealth() &&
          obj->isAnyKindOf(d->m_kindOf)) {
        pulseHealObject(obj);
          ...
          if (TheAnim2DCollection &&
            ...
            if (animTemplate) {
              ...
              TheInGameUI->addWorldAnimation(
                  animTemplate, &iconPosition, WORLD_ANIM_FADE_ON_EXPIRE,
    ...
    return UPDATE_SLEEP(d->m_singleBurst ? UPDATE_SLEEP_FOREVER
                                         : d->m_healingDelay);
  }
}
...
void AutoHealBehavior::pulseHealObject(Object *obj) {
  ...
  if (data->m_radius == 0.0f)
    ...
    obj->attemptHealingFromSoleBenefactor(data->m_healingAmount, getObject(),
                                          data->m_healingDelay);

  if (data->m_unitHealPulseParticleSystemTmpl) {
    ParticleSystem *system = TheParticleSystemManager->createParticleSystem(
        data->m_unitHealPulseParticleSystemTmpl);
    if (system) {
  ...
  m_soonestHealFrame =
      TheGameLogic->getFrame() +
      data->m_healingDelay;  // In case onDamage tries to wake us up early
}
