...
// /////////////////////////////////////////////////////////////// Author: Colin
// Day, September 2002 Desc:   Bridge scaffold
///////////////////////////////////////////////////////////////////////////////////////////////////
...
// ------------------------------------------------------------------------------------------------
/** Set all of the target positions that we're care about as a moving scaffold
 * object */
// ------------------------------------------------------------------------------------------------
...
UpdateSleepTime BridgeScaffoldBehavior::update(void) {
  ...
  if (m_targetMotion == STM_STILL) return UPDATE_SLEEP_NONE;
  ...
  //
  // will this new position push us beyond our target destination, we will take
  // the vector from the new position to the destination and the vector from our
  // current present position tot he destination and dot them togehter ... if
  // the result is < 0 then we have will overshoot the distance if we use the
  // new position
  //
  ...
  if (tooFarVector.x * dirV.x + tooFarVector.y * dirV.y +
          tooFarVector.z * dirV.z <=
      0.0f) {
    // use the destination position
    newPos = m_targetPos;

    //
    // we have reached our target position, switch motion to the next position
    // in the chain (which may be stay still and don't move anymore)
    //
    switch (m_targetMotion) {
      case STM_RISE:
        setMotion(STM_BUILD_ACROSS);
...
// ------------------------------------------------------------------------------------------------
/** STATIC MEMBER:
 * Helper function to retrieve a bridge scaffold interface from an object if one
 * is present */
// ------------------------------------------------------------------------------------------------
...
BridgeScaffoldBehavior::getBridgeScaffoldBehaviorInterfaceFromObject(
  ...
  for (bmi = obj->getBehaviorModules(); *bmi; ++bmi) {
    ...
    if (bridgeScaffoldInterface) return bridgeScaffoldInterface;
