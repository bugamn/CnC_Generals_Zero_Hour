...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
PrisonVisual::~PrisonVisual(void) {}  // end ~PrisonVisual
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
/*static*/ void PrisonBehaviorModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  OpenContainModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {

      {"ShowPrisoners", INI::parseBool, NULL,
       offsetof(PrisonBehaviorModuleData, m_showPrisoners)},
      {"YardBonePrefix", INI::parseAsciiString, NULL,
       offsetof(PrisonBehaviorModuleData, m_prisonYardBonePrefix)},
      {0, 0, 0, 0}
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
PrisonBehavior::~PrisonBehavior(void) {}  // end ~PrisonBehavior
...
void PrisonBehavior::onDelete(void) {
  ...
  while (m_visualList) {
    ...
    if (draw) TheGameClient->destroyDrawable(draw);
...
void PrisonBehavior::onContaining(Object *obj) {
  ...
  if (modData->m_showPrisoners) addVisual(obj);
...
void PrisonBehavior::onRemoving(Object *obj) {
  ...
  const PrisonBehaviorModuleData *modData = getPrisonBehaviorModuleData();
  if (modData->m_showPrisoners) removeVisual(obj);
...
void PrisonBehavior::pickVisualLocation(Coord3D *pos) {
  ...
  Int yardBones =
      us->getMultiLogicalBonePosition(modData->m_prisonYardBonePrefix.str(),
                                      MAX_YARD_BONES, yardPositions, NULL);

  //
  // we must have at least 3 bone locations to make a yard polygon, otherwise
  // we'll default to the object position
  //
  if (yardBones >= 3) {
    ...
    //
    // now that we have a yard region, the default visual position will be in
    // the middle of the yard region instead of the position of our object
    //
...
void PrisonBehavior::removeVisual(Object *obj) {
  ...
  // find visual info in our list, once found, take this opportunity to remove
  // it from that list
...
void PrisonBehavior::xfer(Xfer *xfer) {
  ...
  PrisonVisual *visual;
  for (visual = m_visualList; visual; visual = visual->m_next) visualCount++;
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    if (m_visualList != NULL) {
      DEBUG_CRASH(
          ("PrisonBehavior::xfer - the visual list should be empty but is "
           "not\n"));
