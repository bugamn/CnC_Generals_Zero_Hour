...
// ////////////////////////////////////////////////////////////////// Author:
// Colin Day, July 2002 Desc:   Behavior module for the towers attached to
// bridges that can be targeted
///////////////////////////////////////////////////////////////////////////////////////////////////
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
BridgeTowerBehavior::BridgeTowerBehavior(Thing *thing,
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
BridgeTowerBehavior::~BridgeTowerBehavior(void) {}  // end ~BridgeTowerBehavior
...
void BridgeTowerBehavior::onDamage(DamageInfo *damageInfo) {
  ...
  for (bmi = bridge->getBehaviorModules(); *bmi; ++bmi) {
    ...
    if (bridgeInterface) break;

  }  // end for bmi
  DEBUG_ASSERTCRASH(
      bridgeInterface != NULL,
      ("BridgeTowerBehavior::onDamage - no 'BridgeBehaviorInterface' found\n"));
  if (bridgeInterface) {
    //
    // damage each of the other towers if the source of this damage isn't from
    // the bridge or other towers
    //
    Object *source = TheGameLogic->findObjectByID(damageInfo->in.m_sourceID);
    if (source == NULL || (source->isKindOf(KINDOF_BRIDGE) == FALSE &&
                           source->isKindOf(KINDOF_BRIDGE_TOWER) == FALSE)) {
      for (Int i = 0; i < BRIDGE_MAX_TOWERS; ++i) {
        Object *tower;

        tower = TheGameLogic->findObjectByID(
      ...
      //
      // damage bridge object, but make sure it's done through the bridge
      // interface so that it doesn't automatically propagate that damage to the
      // towers
      //
...
void BridgeTowerBehavior::onHealing(DamageInfo *damageInfo) {
  ...
  DEBUG_ASSERTCRASH(bridgeInterface != NULL,
                    ("BridgeTowerBehavior::onHealing - no "
                     "'BridgeBehaviorInterface' found\n"));
  if (bridgeInterface) {
    //
    // heal each of the other towers if the source of this healing isn't from
    // the bridge or other towers
    //
    Object *source = TheGameLogic->findObjectByID(damageInfo->in.m_sourceID);
    if (source == NULL || (source->isKindOf(KINDOF_BRIDGE) == FALSE &&
      ...
      //
      // heal bridge object, but make sure it's done through the bridge
      // interface so that it doesn't automatically propagate that healing to
      // the towers.
      //
      BodyModuleInterface *bridgeBody = bridge->getBodyModule();
      bridge->attemptHealing(healingPercentage * bridgeBody->getMaxHealth(),
                             getObject());
...
// ------------------------------------------------------------------------------------------------
BridgeTowerBehaviorInterface *
BridgeTowerBehavior::getBridgeTowerBehaviorInterfaceFromObject(Object *obj) {
  // sanity
  if (obj == NULL || obj->isKindOf(KINDOF_BRIDGE_TOWER) == FALSE) return NULL;
  ...
  for (bmi = obj->getBehaviorModules(); *bmi; ++bmi) {
    ...
    if (bridgeTowerInterface) return bridgeTowerInterface;
