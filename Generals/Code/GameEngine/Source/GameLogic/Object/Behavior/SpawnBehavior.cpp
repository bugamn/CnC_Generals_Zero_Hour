...
// Desc:   Update will create and monitor a group of spawned units and replace
// as needed
///////////////////////////////////////////////////////////////////////////////////////////////////
...
#define SPAWN_DELAY_MIN_FRAMES \
  (16)  // about as rapidly as you'd expect people to successively exit through
        // the same door
//-------------------------------------------------------------------------------------------------
SpawnBehavior::SpawnBehavior(Thing *thing, const ModuleData *moduleData)
  ...
  // each time m_spawn template is used, it will increment
  // m_templateNameIterator, thus scanning through the ordered list of template
  // names looping back to the beginning
  ...
  // The initializing of the initial bursters is handled in the first update
  // @todo invent an object::postConstructionProcess() some day
...
//-------------------------------------------------------------------------------------------------
SpawnBehavior::~SpawnBehavior(void) { m_replacementTimes.clear(); }
...
void SpawnBehavior::onDelete() {
  ...
  if (modData->m_spawnedRequireSpawner) {
    ...
    for (objectIDListIterator it = m_spawnIDs.begin(); it != m_spawnIDs.end();
         /*empty*/) {
      // get object
      obj = TheGameLogic->findObjectByID(*it);

      // increment iterator incase the id list is alterd because what we're
      // about to do
      ++it;

      //
      // destroy this if it's alive, in the usual case this object "dies" and is
      // not "destroyed".  on *death* we *KILL* our spawned things, but this is
      // here and will *DESTROY* our spawned things if we ourselves are
      // destroyed and they are still alive (such a case would be calling
      // destroy object on us directly)
      //
...
void SpawnBehavior::onDie(const DamageInfo *damageInfo) {
  ...
  for (objectIDListIterator iter = m_spawnIDs.begin(); iter != m_spawnIDs.end();
       iter++) {
    Object *currentSpawn = TheGameLogic->findObjectByID((*iter));
    if (currentSpawn) {
      // Go through all my spawns and see if they have a SlavedUpdate I can tell
      // I was killed to
      for (BehaviorModule **update = currentSpawn->getBehaviorModules();
           *update; ++update) {
      ...
      // our spawner has died, we must invalidate the ID now in the spawned
      // object
  ...
  if (modData->m_spawnedRequireSpawner) {
    ...
    for (objectIDListIterator it = m_spawnIDs.begin(); it != m_spawnIDs.end();
      ...
      if (obj && obj->isEffectivelyDead() == FALSE) obj->kill();
...
UpdateSleepTime SpawnBehavior::update(void) {
  ...
  if (!m_initialBurstTimesInited) {
    ...
    Bool runtimeProduced =
        getObject()->getProducerID() !=
        INVALID_ID;  // this was produced by a production module, rather than a
                     // script or worldbuilder
    ...
    for (int listIndex = 0; listIndex < md->m_spawnNumberData; listIndex++) {
      if (md->m_initialBurst > 0) {
        ...
        m_replacementTimes.push_back(
            runtimeProduced);  // Set all spawns to be created in rapid
                               // succession
      } else
        m_replacementTimes.push_back(
            listIndex);  // Set all spawns to be created as soon as possible
    }
  }
  ...
  // Go through the list and make a spawn for each number that is less than
  // now's frame
  if (shouldTryToSpawn()) {
...
Bool SpawnBehavior::maySpawnSelfTaskAI(Real maxSelfTaskersRatio) {
  if (m_spawnCount == 0) return FALSE;
  if (maxSelfTaskersRatio == 0) return FALSE;

  // if my last attack command was from player or script, I need to forbid my
  // spawn from disobeying that command otherwise (since my attack state was
  // autoacquired my ny own ai), let them deviate by the ratio specified.
  ...
  if (lastAttackCommandSource != CMD_FROM_AI) return FALSE;
...
Object *SpawnBehavior::getClosestSlave(const Coord3D *pos) {
  ...
  for (objectIDListIterator it = m_spawnIDs.begin(); it != m_spawnIDs.end();
    ...
    if (obj) {
      Real distance =
          ThePartitionManager->getDistanceSquared(obj, pos, FROM_CENTER_2D);
...
// ------------------------------------------------------------------------------------------------
CanAttackResult SpawnBehavior::getCanAnySlavesAttackSpecificTarget(
  ...
  for (objectIDListIterator it = m_spawnIDs.begin(); it != m_spawnIDs.end();
    ...
    if (obj) {
      CanAttackResult result =
          obj->getAbleToAttackSpecificObject(attackType, target, cmdSource);
      ...
      switch (result) {
        ...
        default:
          DEBUG_CRASH(
              ("SpawnBehavior::getCanAnySlavesAttackSpecificTarget encountered "
               "unhandled CanAttackResult of %d. Treating as not possible...",
               result));
          break;
      }
    }
...
// ------------------------------------------------------------------------------------------------
CanAttackResult SpawnBehavior::getCanAnySlavesUseWeaponAgainstTarget(
  ...
  for (objectIDListIterator it = m_spawnIDs.begin(); it != m_spawnIDs.end();
    ...
    Object *obj = TheGameLogic->findObjectByID(*it);
    if (obj) {
      CanAttackResult result = obj->getAbleToUseWeaponAgainstTarget(
          attackType, victim, pos, cmdSource);
      ...
      switch (result) {
        ...
        default:
          DEBUG_CRASH(
              ("SpawnBehavior::getCanAnySlavesUseWeaponAgainstTarget "
               "encountered unhandled CanAttackResult of %d. Treating as not "
               "possible...",
               result));
          break;
      }
...
static void findClosestOrphan(Object *obj, void *userData) {
  ...
  if (obj->getProducerID() != INVALID_ID) return;

  // is this the closest one so far
  Real distSq = ThePartitionManager->getDistanceSquared(orphanData->m_source,
                                                        obj, FROM_CENTER_2D);
...
Object *SpawnBehavior::reclaimOrphanSpawn(void) {
  ...
  //
  // iterate all the objects my controlling player has and look for any orphaned
  // things that we would normally spawn, if found we'll just make it our own
  //
  ...
  for (std::vector<AsciiString>::const_iterator tempName =
           md->m_spawnTemplateNameData.begin();
       tempName != md->m_spawnTemplateNameData.end(); ++tempName) {
    if (prevName.compare(*tempName))  // the list may have redundancy, this will
                                      // skip some of it
      continue;
...
Bool SpawnBehavior::createSpawn() {
  ...
  if (exitInterface == NULL) {
    DEBUG_ASSERTCRASH(
        exitInterface != NULL,
        ("Something cannot have SpawnBehavior without an exit interface"));
    return FALSE;
  }

  ExitDoorType exitDoor = exitInterface->reserveDoorForExit(NULL, NULL);
  if (exitDoor == DOOR_NONE_AVAILABLE) return FALSE;
  ...
  if (md->m_canReclaimOrphans && md->m_isOneShotData == FALSE) {
    ...
    if (newSpawn) reclaimedOrphan = TRUE;
  }
  ...
  if (newSpawn == NULL)  // and that we really want a new spawn, here.
  {
    ...
    // newSpawn = TheThingFactory->newObject( m_spawnTemplate,
    // parent->getControllingPlayer()->getDefaultTeam() );
    newSpawn = TheThingFactory->newObject(
        m_spawnTemplate,
        parent->getTeam());  // just a little worried about this...
    ...
    // Now, we can have a list of template names.
    // Each time m_spawn template is used, it will increment
    // m_templateNameIterator, thus scanning through the ordered list of
    // template names, and looping back to the beginning
  ...
  // from now on.
  for (BehaviorModule **update = newSpawn->getBehaviorModules(); *update;
       ++update) {
    SlavedUpdateInterface *sdu = (*update)->getSlavedUpdateInterface();
    if (sdu != NULL) {
  ...
  if (reclaimedOrphan == FALSE) {
    if (md->m_exitByBudding) {
      ...
      if (m_initialBurstCountdown > 0) {
        Object *barracks =
            TheGameLogic->findObjectByID(parent->getProducerID());
        if (barracks && barracks->isKindOf(KINDOF_STRUCTURE)) {
          ExitInterface *barracksExitInterface =
              barracks->getObjectExitInterface();
          if (barracksExitInterface) {
            ExitDoorType barracksDoor =
                barracksExitInterface->reserveDoorForExit(NULL, NULL);
            barracksExitInterface->exitObjectViaDoor(newSpawn, barracksDoor);
            newSpawn->setProducer(parent);  // let parents producer exit him,
      ...
      if (!barracksExitSuccess) {
        ...
        // there is probably a more elegant way to choose the budHost, but oh
        // well
        Object *budHost = NULL;
        ...
        for (iter = m_spawnIDs.begin(); iter != m_spawnIDs.end(); iter++) {
          ...
          if (curSpawn) {
            if (curSpawn == newSpawn) continue;

            tapeMeasure = ThePartitionManager->getDistanceSquared(
                curSpawn, parent, FROM_CENTER_2D);
        ...
        exitInterface->exitObjectByBudding(
            newSpawn, budHost);  // also handles the NULL pointer okay
      }
  ...
  if (md->m_isOneShotData) m_oneShotCountdown--;
...
//-------------------------------------------------------------------------------------------------
void SpawnBehavior::stopSpawning() { m_active = FALSE; }

//-------------------------------------------------------------------------------------------------
void SpawnBehavior::startSpawning() { m_active = TRUE; }
...
Bool SpawnBehavior::shouldTryToSpawn() {
  ...
  if (BitTest(getObject()->getStatusBits(), OBJECT_STATUS_RECONSTRUCTING) &&
    ...
    // If we are a Hole rebuild, not only should we not, but we should never ask
    // again.
    stopSpawning();
    return FALSE;
  }
  ...
  if (getObject()->isNeutralControlled()) return FALSE;
...
void SpawnBehavior::computeAggregateStates(void) {
  ...
  for (objectIDListIterator iter = m_spawnIDs.begin(); iter != m_spawnIDs.end();
    ...
    if (currentSpawn) {
      ...
      } else if (spawnVetLevel <
                 vetLevel)  // whoever has more, set the other guy
      {
  ...
  if (SomebodyIsSelected &&
      (!obj->getDrawable()->isSelected() || SomebodyIsNotSelected)) {
    GameMessage *teamMsg =
        TheMessageStream->appendMessage(GameMessage::MSG_CREATE_SELECTED_GROUP);
    teamMsg->appendBooleanArgument(
        FALSE);  // not creating new team so pass false
...
void SpawnBehavior::xfer(Xfer *xfer) {
  ...
  // spawn template
  name = m_spawnTemplate ? m_spawnTemplate->getName()
  ...
  // replacement times
  if (xfer->getXferMode() == XFER_LOAD) m_replacementTimes.clear();
