...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
/*static*/ void BridgeBehaviorModuleData::buildFieldParse(
    MultiIniFieldParse &p) {
  BehaviorModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"LateralScaffoldSpeed", INI::parseVelocityReal, NULL,
       offsetof(BridgeBehaviorModuleData, m_lateralScaffoldSpeed)},
      {"VerticalScaffoldSpeed", INI::parseVelocityReal, NULL,
       offsetof(BridgeBehaviorModuleData, m_verticalScaffoldSpeed)},
      {"BridgeDieFX", parseFX, NULL, offsetof(BridgeBehaviorModuleData, m_fx)},
      {"BridgeDieOCL", parseOCL, NULL,
       offsetof(BridgeBehaviorModuleData, m_ocl)},
      {0, 0, 0, 0}};
...
static void parseTimeAndLocationInfo(INI *ini, void *instance,
  ...
  // delay value
  ini->parseDurationUnsignedInt(ini, instance, &timeAndLocationInfo->delay,
                                NULL);
...
//-------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
/*static*/ void BridgeBehaviorModuleData::parseFX(INI *ini, void *instance,
                                                  void *store,
                                                  const void *userData) {
  const char *token;
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
/*static*/ void BridgeBehaviorModuleData::parseOCL(INI *ini, void *instance,
                                                   void *store,
                                                   const void *userData) {
  const char *token;
...
BridgeBehavior::BridgeBehavior(Thing *thing, const ModuleData *moduleData)
  ...
  for (i = 0; i < BRIDGE_MAX_TOWERS; ++i) m_towerID[i] = INVALID_ID;

  m_fxResolved = FALSE;
  for (Int bodyState = BODY_PRISTINE; bodyState < BODYDAMAGETYPE_COUNT;
       ++bodyState) {
...
BridgeBehavior::~BridgeBehavior(void) {
  //
  // destroy our four towers if they are still present in the world (they
  // wouldn't be if we are resetting the engine, and those towers were earlier
  // in the destruction list than we were)
  //
  for (Int i = 0; i < BRIDGE_MAX_TOWERS; ++i) {
    ...
    if (tower) TheGameLogic->destroyObject(tower);
...
// ------------------------------------------------------------------------------------------------
/*static */ BridgeBehaviorInterface *
BridgeBehavior::getBridgeBehaviorInterfaceFromObject(Object *obj) {
  // sanity
...
void BridgeBehavior::resolveFX(void) {
  ...
  for (Int bodyState = BODY_PRISTINE; bodyState < BODYDAMAGETYPE_COUNT;
    ...
    for (Int i = 0; i < MAX_BRIDGE_BODY_FX; ++i) {
      ...
      m_damageToOCL[bodyState][i] =
          TheObjectCreationListStore->findObjectCreationList(name.str());
      ...
      name =
          bridgeTemplate->getRepairedToOCLString((BodyDamageType)bodyState, i);
      m_repairToOCL[bodyState][i] =
          TheObjectCreationListStore->findObjectCreationList(name.str());
      if (name.isEmpty() == FALSE && m_repairToOCL[bodyState][i] == NULL)
        DEBUG_CRASH(("OCL list '%s' not found\n", name.str()));

      name =
          bridgeTemplate->getRepairedToFXString((BodyDamageType)bodyState, i);
...
void BridgeBehavior::onDamage(DamageInfo *damageInfo) {
  //
  // get our body info so we now how much damage percent is being done to us ...
  // we need this so that we can propagate the same damage percentage amont the
  ...
  //
  // if the damage didn't come from a bridge tower, then we must propagate this
  // damage to all our towers
  //
    ...
    for (Int i = 0; i < BRIDGE_MAX_TOWERS; i++) {
      ...
      if (tower) {
        ...
        towerDamage.in.m_sourceID =
            getObject()->getID();  // we're now the source
...
void BridgeBehavior::onHealing(DamageInfo *damageInfo) {
  //
  // get our body info so we now how much healing percent is being done to us
  // ... we need this so that we can propagate the same healing percentage amont
  // the towers and the bridge
  //
  BodyModuleInterface *body = getObject()->getBodyModule();
  Real healingPercentage = damageInfo->in.m_amount / body->getMaxHealth();

  //
  // if the healing didn't come from a bridge tower, then we must propagate this
  // healing to all our towers
  //
  Object *source = TheGameLogic->findObjectByID(damageInfo->in.m_sourceID);
  if (source == NULL || source->isKindOf(KINDOF_BRIDGE_TOWER) == FALSE) {
    Object *tower;
    ...
    for (Int i = 0; i < BRIDGE_MAX_TOWERS; i++) {
      ...
      if (tower) {
        BodyModuleInterface *towerBody = tower->getBodyModule();
        tower->attemptHealing(healingPercentage * towerBody->getMaxHealth(),
                              getObject());
...
void BridgeBehavior::getRandomSurfacePosition(TerrainRoadType *bridgeTemplate,
                                              ...
                                              Coord3D *pos) {
  // sanity
  if (bridgeInfo == NULL || pos == NULL) return;

  //
  // pick the spot by finding vectors along the edge of the bridge region,
  // scaling them and then adding them together
  //
  Real scale;
  ...
  // Z component so that effects can be created in a "cube" area on and above
  // the bridge
  //
  pos->z += GameLogicRandomValueReal(
...
void BridgeBehavior::onBodyDamageStateChange(const DamageInfo *damageInfo,
  ...
  //
  // check for coming back from the dead, if our new state is not the rubble
  // state, we can't possibly be dead
  //
  if (newState != BODY_RUBBLE) m_deathFrame = 0;

  // first resolve any fx stuff if we need to
  if (m_fxResolved == FALSE) resolveFX();

  // sanity
  if (m_fxResolved == FALSE) return;

  // sanity
  DEBUG_ASSERTCRASH(
      oldState != newState,
      ("BridgeBehavior::onBodyDamageStateChange - oldState and newState should "
       "be different if this is getting called\n"));
  ...
  DEBUG_ASSERTCRASH(bridgeTemplate,
                    ("BridgeBehavior: Unable to find bridge template '%s' in "
                     "bridge object '%s'\n",
                     bridgeTemplateName, us->getTemplate()->getName().str()));
  ...
  if (gotRepaired) {
    ...
    for (Int i = 0; i < MAX_BRIDGE_BODY_FX; i++)
      doAreaEffects(bridgeTemplate, bridge, m_repairToOCL[newState][i],
                    m_repairToFX[newState][i]);
    ...
    for (Int i = 0; i < MAX_BRIDGE_BODY_FX; i++)
      doAreaEffects(bridgeTemplate, bridge, m_damageToOCL[newState][i],
                    m_damageToFX[newState][i]);

  }  // end else

  // update bridge damage states
  ///@todo this should be re-written, there no need for this looping bridge
  ///examination
  TheTerrainLogic->updateBridgeDamageStates();

  //
  // for the local player, if this bridge has switched from rubble, to usable,
  // or from usable to rubble, we should reflect the change on the radar.  note
  // that we request that the radar queue a refresh sometime in the future
  // because it keeps track of how often we makes requests to do a refresh and
  // doesn't do them too often because it's expensive to refresh the terrain
  //
...
UpdateSleepTime BridgeBehavior::update(void) {
  ...
  if (m_deathFrame != 0) {
    ...
    if (bridge) {
      DEBUG_ASSERTCRASH(bridge,
                        ("BridgeBehavior::update - Unable to find bridge\n"));
      ...
      DEBUG_ASSERTCRASH(bridgeTemplate,
                        ("BridgeBeahvior::getRandomSurfacePosition - "
                         "Encountered a bridge with no template!\n"));
    }
    ...
    for (fxIt = modData->m_fx.begin(); fxIt != modData->m_fx.end(); ++fxIt) {
      ...
      if (deathTime == (*fxIt).timeAndLocationInfo.delay) {
        ...
        //
        // if a bone name is present, we'll use the bone position, otherwise
        // we'll pick a spot somewhere on the bridge surface
        //
        boneName = (*fxIt).timeAndLocationInfo.boneName;
        if (boneName.isEmpty() == FALSE)
          us->getSingleLogicalBonePosition(boneName.str(), &pos, NULL);
        else if (bridge && bridgeTemplate &&
                 bridgeInfo)  // we have valid Terrain data for the bridge
          getRandomSurfacePosition(bridgeTemplate, bridgeInfo, &pos);
        else
    ...
    for (oclIt = modData->m_ocl.begin(); oclIt != modData->m_ocl.end();
         ++oclIt) {
      //
      // we'll launch an ocl list if our death time is equal to exactly the
      // delay we're waiting for to launch the list
      //
      if (deathTime == (*oclIt).timeAndLocationInfo.delay) {
        ...
        if (boneName.isEmpty() == FALSE) {
          //
          // special case for creating an OCL for using the bridge object parent
          // center location why do we have this ... well, apparently the OCL
          // ignores the obj parameter passed in if you give it a position
          // parameter, and we need it to pay attention to that object
          // parameteter so the OCL stuff can inherit the LIKE_EXISTING
...
void BridgeBehavior::onDie(const DamageInfo *damageInfo) {
  ...
  // we need to handle anything that was on top of us now that we've been
  // destroyed
...
void BridgeBehavior::handleObjectsOnBridgeOnDie(void) {
  ...
  Bridge *terrainBridge =
      TheTerrainLogic->findBridgeAt(getObject()->getPosition());
  if (terrainBridge) {
    ...
    //
    // find the lowest Z point of the bridge area ... we will use this to figure
    // out of objects in the bridge area are "on top" of the bridge
    //
    Real lowBridgeZ = bridgePolygon[0].z;
    for (Int i = 0; i < 4; ++i)
      if (bridgePolygon[i].z < lowBridgeZ) lowBridgeZ = bridgePolygon[i].z;

    //
    // given the polygon area, how big is the radius that we need to scan in the
    // world to cover from the center of the bridge (the bridge object position)
    ...
    ObjectIterator *iter = ThePartitionManager->iterateObjectsInRange(
        bridgePos, radius, FROM_CENTER_2D);
    MemoryPoolObjectHolder hold(iter);
    Object *other;
    for (other = iter->first(); other; other = iter->next()) {
      // ingnore some kind of objects
      if (other->isKindOf(KINDOF_BRIDGE) ||
          other->isKindOf(KINDOF_BRIDGE_TOWER))
        continue;

      // ignore airborne objects
      if (other->isAboveTerrain()) continue;

      // ignore objects that were not actually on the bridge
      if (other->getPosition()->z < lowBridgeZ) continue;
      ...
      if (bridgeLayer != other->getLayer()) continue;

      if (other->getLayer() == bridgeLayer) other->setLayer(LAYER_GROUND);
...
// ------------------------------------------------------------------------------------------------
/** Set all the position, angle, and speed data we need to for a single scaffold
 * object */
// ------------------------------------------------------------------------------------------------
void BridgeBehavior::setScaffoldData(Object *obj, Real *angle,
                                     Real *sunkenHeight,
  ...
  scaffoldBehavior =
      BridgeScaffoldBehavior::getBridgeScaffoldBehaviorInterfaceFromObject(obj);
  DEBUG_ASSERTCRASH(scaffoldBehavior,
                    ("Unable to find bridge scaffold behavior interface\n"));
  ...
  //
  // set the speed of the scaffold "animation" which is based on how big of a
  // distance all the scaffold objects have to traverse in order to meet up and
  // be complete in the center of the bridge in an interesting way
  //
  ...
  scaffoldBehavior->setLateralSpeed(
      lateralSpeed *
      (distBuildUpPosToBridgeCenter / distRiseToPosToBridgeCenter));
...
// ------------------------------------------------------------------------------------------------
/** Start the bridge repair scaffolding.  If we already have scaffolding this
 * call is ignored */
// ------------------------------------------------------------------------------------------------
void BridgeBehavior::createScaffolding(void) {
  // if we have scaffolding up already do nothing
  if (m_scaffoldPresent == TRUE) return;
  ...
  TerrainRoadType *bridgeTemplate =
      TheTerrainRoads->findBridge(bridgeTemplateName);
  DEBUG_ASSERTCRASH(bridgeTemplate,
                    ("Unable to find bridge template to create scaffolding\n"));
  ...
  const ThingTemplate *scaffoldTemplate =
      TheThingFactory->findTemplate(scaffoldObjectName);
  ...
  AsciiString scaffoldSupportObjectName =
      bridgeTemplate->getScaffoldSupportObjectName();
  const ThingTemplate *scaffoldSupportTemplate =
      TheThingFactory->findTemplate(scaffoldSupportObjectName);
  ...
  // how much space is going to be between each of the scaffold objects at their
  // final positions
  Real spacing =
      scaffoldTemplate->getTemplateGeometryInfo().getMajorRadius() * 2.0f;
  ...
  Real scaffoldSupportHeight =
      scaffoldSupportTemplate->getTemplateGeometryInfo()
  ...
  //
  // given the area of the bridge, figure out what the start and end points are
  // to create all the scaffold objects at (just in the 2D bridge plane, not
  // thinking about rising the objects up through the ground yet)
  //
  Coord3D leftStart;
  leftStart.x = ((bridgeInfo.fromLeft.x - bridgeInfo.fromRight.x) / 2.0f) +
                bridgeInfo.fromRight.x;
  leftStart.y = ((bridgeInfo.fromLeft.y - bridgeInfo.fromRight.y) / 2.0f) +
                bridgeInfo.fromRight.y;
  leftStart.z = ((bridgeInfo.fromLeft.z - bridgeInfo.fromRight.z) / 2.0f) +
                bridgeInfo.fromRight.z;

  Coord3D rightStart;
  rightStart.x = ((bridgeInfo.toLeft.x - bridgeInfo.toRight.x) / 2.0f) +
                 bridgeInfo.toRight.x;
  rightStart.y = ((bridgeInfo.toLeft.y - bridgeInfo.toRight.y) / 2.0f) +
                 bridgeInfo.toRight.y;
  rightStart.z = ((bridgeInfo.toLeft.z - bridgeInfo.toRight.z) / 2.0f) +
                 bridgeInfo.toRight.z;

  //
  // now that we have the left and right start points, we will compute two
  // angles to use for all the objects that we will create ... objects on the
  // left side of the bridge will be given a 'leftAngle' pointing from
  // 'leftStart' to 'rightStart' and the opposite will be given to objects on
  ...
  //
  // how many of these scaffold objects will take to tile from each of the
  // endpoints to the center area of the bridge
  //
  Real tileDistance = leftVector.length();
  Int numObjects = REAL_TO_INT_CEIL(tileDistance / spacing) + 1;

  //
  // given the number of objects that we need to tile across the whole bridge,
  // we will go through the creation loop ceil( numObjects / 2.0f ) times, and
  // each time through the loop we'll create an object to move from each side of
  // the bridge, except the last object if the number of objects is odd is dead
  // in the center
  //
  Int numIterations = REAL_TO_INT_CEIL(INT_TO_REAL(numObjects) / 2.0f);

  //
  // normalize left and right vectors as it is a vector that goes from our left
  // start position to the destination right start position ... we will multiply
  // this vector by a spacing amount and add to the left start position to find
  // a destination position for a particular scaffold object along the bridge
  // surface
  //
  ...
  for (Int i = 0; i < numIterations; ++i) {
    ...
    //
    // compute position for object moving from the left side, we're adding 0.1
    // here so that all scaffold objects *have* to move some distance ... that
    // way we can just assign the speeds so they line up perfectly and not have
    // to worry about any object reaching a destination before any other object
    //
    ...
    //
    // now that they key positions are calculated, set the rest of the position
    // data and movement speeds for the object
    //
    setScaffoldData(obj, angle, &scaffoldHeight, riseToPos, &destinationPos,
                    center);
    ...
    //
    // create support object layers under the scaffold object, this object
    // mirrors the scaffold object except is on a lower layer
    //
    ...
    while (offset >= 0.0f) {
      ...
      setScaffoldData(obj, angle, &scaffoldSupportHeight, &supportRiseToPos,
                      &supportDestinationPos, &supportBridgeCenter);
      m_scaffoldObjectIDList.push_back(obj->getID());
    ...
    //
    // now create the object from the "right" side of the bridge ... but note
    // that we don't do this on the last iteration through this loop when we
    // have an odd number of objects we're tiling because all the space is
    // already perfectly used up)
    //
    if (scaffoldObjectsCreated < numObjects) {
      ...
      // compute position for object moving from the left side, we're adding 0.1
      // here so that all scaffold objects *have* to move some distance ... that
      // way we can just assign the speeds so they line up perfectly and not
      // have to worry about any object reaching a destination before any other
      // object
      //
      ...
      while (offset >= 0.0f) {
        ...
        supportBridgeCenter.z -= scaffoldSupportHeight;
        obj =
            TheThingFactory->newObject(scaffoldSupportTemplate, us->getTeam());
        setScaffoldData(obj, angle, &scaffoldSupportHeight, &supportRiseToPos,
                        &supportDestinationPos, &supportBridgeCenter);
        m_scaffoldObjectIDList.push_back(obj->getID());
...
// ------------------------------------------------------------------------------------------------
/** Remove the bridge scaffolding.  If we don't have any then this call is
 * ignored */
// ------------------------------------------------------------------------------------------------
void BridgeBehavior::removeScaffolding(void) {
  // if we have no scaffolding, do nothing
  if (m_scaffoldPresent == FALSE) return;

  // go through each object and tell them to reverse course
  Object *obj;
  ObjectIDListIterator it;
  BridgeScaffoldBehaviorInterface *scaffoldBehavior;
  for (it = m_scaffoldObjectIDList.begin(); it != m_scaffoldObjectIDList.end();
       ++it) {
    // get the object
  ...
  // when scaffolding is gone, a bridge can be used again if we're not in a
  // rubble state
  Object *us = getObject();
...
Bool BridgeBehavior::isScaffoldInMotion(void) {
  ...
  // go through the scaffold objects, if any of them are in motion the scaffold
  // is in motion
  ObjectIDListIterator it;
  for (it = m_scaffoldObjectIDList.begin(); it != m_scaffoldObjectIDList.end();
       ++it) {
    ...
    BridgeScaffoldBehaviorInterface *bsbi =
        BridgeScaffoldBehavior::getBridgeScaffoldBehaviorInterfaceFromObject(
            obj);
    if (bsbi == NULL) continue;

    // check in motion
    if (bsbi->getCurrentMotion() != STM_STILL) return TRUE;
...
void BridgeBehavior::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_LOAD) {
    ...
    // sanity
    DEBUG_ASSERTCRASH(bridge,
                      ("BridgeBehavior::xfer - Unable to find bridge\n"));
  ...
  for (Int i = 0; i < BRIDGE_MAX_TOWERS; ++i) xfer->xferObjectID(&m_towerID[i]);
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    ObjectIDListIterator it;
    for (it = m_scaffoldObjectIDList.begin();
