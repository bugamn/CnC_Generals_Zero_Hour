...
// ////////////////////////////////////////////////////////////////// Author:
// Steven Johnson, June 2002 Desc:   Minefield behavior
///////////////////////////////////////////////////////////////////////////////////////////////////
...
//-------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
/*static*/ void MinefieldBehaviorModuleData::buildFieldParse(
    MultiIniFieldParse& p) {
  UpdateModuleData::buildFieldParse(p);

  static const FieldParse dataFieldParse[] = {
      {"DetonationWeapon", INI::parseWeaponTemplate, NULL,
       offsetof(MinefieldBehaviorModuleData, m_detonationWeapon)},
      {"DetonatedBy", INI::parseBitString32, TheRelationshipNames,
       offsetof(MinefieldBehaviorModuleData, m_detonatedBy)},
      {"StopsRegenAfterCreatorDies", INI::parseBool, NULL,
       offsetof(MinefieldBehaviorModuleData, m_stopsRegenAfterCreatorDies)},
      {"Regenerates", INI::parseBool, NULL,
       offsetof(MinefieldBehaviorModuleData, m_regenerates)},
      {"WorkersDetonate", INI::parseBool, NULL,
       offsetof(MinefieldBehaviorModuleData, m_workersDetonate)},
      {"CreatorDeathCheckRate", INI::parseDurationUnsignedInt, NULL,
       offsetof(MinefieldBehaviorModuleData, m_creatorDeathCheckRate)},
      {"ScootFromStartingPointTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(MinefieldBehaviorModuleData, m_scootFromStartingPointTime)},
      {"NumVirtualMines", INI::parseUnsignedInt, NULL,
       offsetof(MinefieldBehaviorModuleData, m_numVirtualMines)},
      {"RepeatDetonateMoveThresh", INI::parseReal, NULL,
       offsetof(MinefieldBehaviorModuleData, m_repeatDetonateMoveThresh)},
      {"DegenPercentPerSecondAfterCreatorDies", INI::parsePercentToReal, NULL,
       offsetof(MinefieldBehaviorModuleData, m_healthPercentToDrainPerSecond)},
      {0, 0, 0, 0}};
...
UpdateSleepTime MinefieldBehavior::calcSleepTime() {
  ...
  if (m_draining) return UPDATE_SLEEP_NONE;

  // if we're scooting we need to update every frame
  if (m_scootFramesLeft > 0) return UPDATE_SLEEP_NONE;

  // if there is anybody in our immulity monitoring we need to update every
  // frame
  for (Int i = 0; i < MAX_IMMUNITY; ++i)
    if (m_immunes[i].id != INVALID_ID) return UPDATE_SLEEP_NONE;

  UnsignedInt sleepTime = FOREVER;
  UnsignedInt now = TheGameLogic->getFrame();
  //
  // sleep until the next death check frame we already have figured outif we
  // care about it (that is, when our creator dies)
  //
...
UpdateSleepTime MinefieldBehavior::update() {
  ...
  if (m_scootFramesLeft > 0) {
    ...
    if (pt.z < ground || m_scootFramesLeft <= 1) pt.z = ground;
  ...
  for (Int i = 0; i < MAX_IMMUNITY; ++i) {
    if (m_immunes[i].id == INVALID_ID) continue;
  ...
  if (now >= m_nextDeathCheckFrame) {
    // check to see if there is an enemy building on me... since enemy buildings
    // can be build on top of me

    // check to see if the building that made me is gone, and whether therefore,
    // I should go
    if (m_regenerates && d->m_stopsRegenAfterCreatorDies) {
      ...
      if (producerID != INVALID_ID) {
        ...
        if (producer == NULL || producer->isEffectivelyDead()) {
          ...
          AutoHealBehavior* ahb =
              (AutoHealBehavior*)obj->findUpdateModule(key_AutoHealBehavior);
          if (ahb) ahb->stopHealing();
        }
      }
    }
  }
  ...
  if (m_draining) {
    ...
    damageInfo.in.m_amount = (obj->getBodyModule()->getMaxHealth() *
                              d->m_healthPercentToDrainPerSecond) /
...
void MinefieldBehavior::detonateOnce(const Coord3D& position) {
  ...
  if (d->m_detonationWeapon) {
    Object* obj = getObject();
    TheWeaponStore->createAndFireTempWeapon(d->m_detonationWeapon, obj,
                                            &position);
  }

  if (m_virtualMinesRemaining > 0) --m_virtualMinesRemaining;
  ...
  if (!m_regenerates && m_virtualMinesRemaining == 0) {
    ...
    if (desired < MIN_HEALTH) desired = MIN_HEALTH;
    ...
    if (amount > 0.0f) {
      ...
      // can't use this, AutoHeal won't work unless we go thru normal damage
      // stuff
...
// ------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void MinefieldBehavior::onCollide(Object* other, const Coord3D* loc,
                                  const Coord3D* normal) {
  if (other == NULL || other->isEffectivelyDead()) return;

  if (m_virtualMinesRemaining == 0) return;
  ...
  // NOTE NOTE NOTE, must always do this check FIRST so that 'collideTime' is
  // updated...
  for (Int i = 0; i < MAX_IMMUNITY; ++i) {
  ...
  if ((d->m_detonatedBy & requiredMask) == 0) return;

  // are we active?
  if (m_scootFramesLeft > 0) return;

  // things that are in the process of clearing mines are immune to mine
  // detonation, even if we aren't the specific mine they are trying to clear.
  // (however, they must have a real mine they area trying to clear... it's
  // possible they could be trying to clear a position where there is no mine,
  // in which case we grant them no immunity, muwahahaha)
  AIUpdateInterface* otherAI = other->getAI();
  if (otherAI && otherAI->isClearingMines() &&
      otherAI->getGoalObject() != NULL) {
    ...
    // of a guy who touches two close-together mines while clearing, then puts
    // up his detector and is blown to smithereens by the other one.)
    for (Int i = 0; i < MAX_IMMUNITY; ++i) {
  ...
  // if we detonated another one nearby, we have to move a little bit to
  // detonate another one.
  Bool found = false;
  for (std::vector<DetonatorInfo>::iterator it = m_detonators.begin();
       it != m_detonators.end(); ++it) {
...
void MinefieldBehavior::onDamage(DamageInfo* damageInfo) {
  ...
  // detonate as many times as neccessary for our virtual mine count to match
  // our health
  BodyModuleInterface* body = getObject()->getBodyModule();
  ...
  for (;;) {
    ...
    Int virtualMinesExpected = damageInfo->in.m_damageType == DAMAGE_HEALING
    ...
    if (m_virtualMinesRemaining < virtualMinesExpected) {
      ...
      if (m_draining && damageInfo->in.m_sourceID == getObject()->getID() &&
          damageInfo->in.m_damageType == DAMAGE_UNRESISTABLE) {
  ...
  if (m_virtualMinesRemaining == 0) {
    // oops, if someone did weapon damage they may have nuked our health to
    // zero, which would be bad if we regen. prevent this. (srj)
...
void MinefieldBehavior::setScootParms(const Coord3D& start,
  ...
  endOnGround.z =
      TheTerrainLogic->getGroundHeight(endOnGround.x, endOnGround.y);
  if (start.z > endOnGround.z) {
    // figure out how long it will take to fall, and replace scoot time with
    // that
    UnsignedInt fallingTime = REAL_TO_INT_CEIL(sqrtf(
        2.0f * (start.z - endOnGround.z) / fabs(TheGlobalData->m_gravity)));
    // we can scoot after we land, but don't want to stop scooting before we
    // land
  ...
  if (scootFromStartingPointTime == 0) {
    ...
    if (dist <= 0.1f && fabs(dz) <= 0.1f) {
      ...
      Real accelMag =
          fabs(2.0f * (dist - scootFromStartingPointSpeed * t) / sqr(t));
      ...
      // we need to wake ourselves up because we could be lying here sleeping
      // forever
      setWakeFrame(obj, calcSleepTime());
    }
  }
...
void MinefieldBehavior::xfer(Xfer* xfer) {
  ...
  // mines remaining
  /// @todo srj -- ensure health, appearance, etc are correct for save/reload!
  /// post-MP!
  ...
  if (maxImmunity != MAX_IMMUNITY) {
    DEBUG_CRASH(
        ("MinefieldBehavior::xfer - MAX_IMMUNITY has changed size, you must "
         "version this code and then you can remove this error message\n"));
  ...
  if (xfer->getXferMode() == XFER_LOAD) m_detonators.clear();
