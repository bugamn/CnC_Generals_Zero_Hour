...
// Desc:   On creation, will set Object's veterancy level if required Science is
// present.
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void VeterancyGainCreateModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"StartingLevel", INI::parseIndexList, TheVeterancyNames,
       offsetof(VeterancyGainCreateModuleData, m_startingLevel)},
      {"ScienceRequired", INI::parseScience, NULL,
       offsetof(VeterancyGainCreateModuleData, m_scienceRequired)},
      {0, 0, 0, 0}};
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
VeterancyGainCreate::VeterancyGainCreate(Thing *thing,
...
void VeterancyGainCreate::onCreate(void) {
  // When produced normally, this Object will ask the Player if the correct
  // Science is known for it to set its level to the given level
  ...
  if (myPlayer && (md->m_scienceRequired == SCIENCE_INVALID ||
    ...
    if (myExp && myExp->isTrainable()) {
      ...
      myExp->setMinVeterancyLevel(
          md->m_startingLevel);  // sVL can override isTrainable, but this
                                 // module should not.
    }
  }
}
