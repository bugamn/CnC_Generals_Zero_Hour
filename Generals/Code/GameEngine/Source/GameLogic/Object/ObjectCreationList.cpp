...
ObjectCreationListStore* TheObjectCreationListStore =
    NULL;  ///< the ObjectCreationList store definition
...
//-------------------------------------------------------------------------------------------------
void ObjectCreationNugget::create(const Object* primary,
                                  const Object* secondary,
                                  UnsignedInt lifetimeFrames) const {
  create(primary, primary ? primary->getPosition() : NULL,
         secondary ? secondary->getPosition() : NULL, lifetimeFrames);
}

//-------------------------------------------------------------------------------------------------
// This one is called only when we have a nugget that doesn't care about
// createOwner.
void ObjectCreationNugget::create(const Object* primaryObj,
                                  const Coord3D* primary,
                                  const Coord3D* secondary, Bool createOwner,
                                  UnsignedInt lifetimeFrames) const {
...
class FireWeaponNugget : public ObjectCreationNugget {
 ...
 public:
  FireWeaponNugget() : m_weapon(NULL) {}

  virtual void create(const Object* primaryObj, const Coord3D* primary,
                      const Coord3D* secondary,
                      UnsignedInt lifetimeFrames = 0) const {
    if (!primaryObj || !primary || !secondary) {
      DEBUG_CRASH(
          ("You must have a primary and secondary source for this effect"));
      return;
    }
  ...
  static void parse(INI* ini, void* instance, void* /*store*/,
    ...
    static const FieldParse myFieldParse[] = {
        {"Weapon", INI::parseWeaponTemplate, NULL,
...
class AttackNugget : public ObjectCreationNugget {
 ...
 public:
  ...
  virtual void create(const Object* primaryObj, const Coord3D* primary,
    ...
    // 1:30 left on the clock, Demo looming, should I de-const all of OCL since
    // this one effect needs the Primary to help make the objects?  Should I
    // rewrite superweapons to completely subsume them into normal special
    // powers, so I can have a spell attack with a global timer and a spinny
    // clock? Should I const cast?  Should I eat them?  No!  I'd rather hurt
    // them stomp them crush them hurt them stomp them while I dance!  Down.
    // Those insects make me dance!  Down.  I'll hurt them while I dance.
  ...
  static void parse(INI* ini, void* instance, void* /*store*/,
                    const void* /*userData*/) {
    static const FieldParse myFieldParse[] = {
        {"NumberOfShots", INI::parseInt, NULL,
         offsetof(AttackNugget, m_numberOfShots)},
        {"WeaponSlot", INI::parseLookupList, TheWeaponSlotTypeNamesLookupList,
         offsetof(AttackNugget, m_weaponSlot)},
        {"DeliveryDecal", RadiusDecalTemplate::parseRadiusDecalTemplate, NULL,
         offsetof(AttackNugget, m_deliveryDecalTemplate)},
        {"DeliveryDecalRadius", INI::parseReal, NULL,
         offsetof(AttackNugget, m_deliveryDecalRadius)},
        {0, 0, 0, 0}};
...
class DeliverPayloadNugget : public ObjectCreationNugget {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(DeliverPayloadNugget,
 ...
 public:
  ...
  virtual void create(const Object* primaryObj, const Coord3D* primary,
    ...
    Team* owner = primaryObj
                      ? primaryObj->getControllingPlayer()->getDefaultTeam()
                      : NULL;

    // What I'm doing for the purposes of the formations is to calculate the
    // relative positions of each member of the formation. To do so, we take the
    // vector from the target location to the lead plane location, normalize it,
    // then rotate it 90 degrees (CW and CCW). When we add the resultant vectors
    // to the initial vectors, we can calculate the delta positions for each
    // plane.
    ...
    for (Int formationIndex = 0; formationIndex < m_formationSize;
         formationIndex++) {
      Coord3D offset;
      offset.zero();
      ...
      // Our target position only applies when using fireweapon and when we have
      // multiple planes, as is the case with the napalm strike. The target
      // position either be somewhere between the moveToPos of the lead plane
      // and that of the relative offset -- determined by the convergenceFactor.
      targetPos.x += offset.x * (1.0f - m_convergenceFactor);
      targetPos.y += offset.y * (1.0f - m_convergenceFactor);

      // first guy in each formation is always spot-on (to keep targeting cursor
      // well-matched)
      ...
      if (createOwner) {
        const ThingTemplate* ttn =
            TheThingFactory->findTemplate(m_transportName);
        ...
        // Adding this nifty flag allows enemy players to target it manually
        // with weapons :)
        transport->setScriptStatus(OBJECT_STATUS_SCRIPT_TARGETABLE);

        if (m_delayDeliveryFramesMax > 0) {
          transport->setDisabledUntil(
              DISABLED_DEFAULT,
              TheGameLogic->getFrame() +
                  GameLogicRandomValue(0, m_delayDeliveryFramesMax));
        }
      } else {
      ...
      SpecialPowerCompletionDie* die =
          transport->findSpecialPowerCompletionDie();
      if (die) {
        if (formationIndex == 0)
      ...
      DeliverPayloadAIUpdate* ai =
          (DeliverPayloadAIUpdate*)transport->findUpdateModule(
      ...
      if (ai) {
        if (m_startAtMaxSpeed && createOwner) {
          ...
          if (physics) {
            ...
            Real maxSpeed = ai->getCurLocomotor()->getMaxSpeedForCondition(
                transport->getBodyModule()->getDamageState());
        ...
        if (formationIndex != 0) data.m_deliveryDecalRadius = 0;
        ai->deliverPayload(&moveToPos, &targetPos, &data);
        if (m_startAtPreferredHeight && createOwner) {
          startPos.z =
              TheTerrainLogic->getGroundHeight(startPos.x, startPos.y) +
              ai->getCurLocomotor()->getPreferredHeight();
          transport->setPosition(&startPos);
        }

        const ThingTemplate* putInContainerTmpl =
            m_putInContainerName.isEmpty()
                ? NULL
                : TheThingFactory->findTemplate(m_putInContainerName);
        for (std::vector<Payload>::const_iterator it = m_payload.begin();
             it != m_payload.end(); ++it) {
          const ThingTemplate* payloadTmpl =
              TheThingFactory->findTemplate(it->m_payloadName);
          for (int i = 0; i < it->m_payloadCount; ++i) {
            ...
            // Notify special power tracking
            SpecialPowerCompletionDie* die =
                payload->findSpecialPowerCompletionDie();
            if (die) {
            ...
            if (putInContainerTmpl) {
              Object* container =
                  TheThingFactory->newObject(putInContainerTmpl, owner);
              container->setPosition(&startPos);
              container->setProducer(transport);

              // Notify special power tracking
              SpecialPowerCompletionDie* die =
                  container->findSpecialPowerCompletionDie();
              if (die) {
                if (formationIndex == 0 && i == 0)
                  die->setCreator(primaryObj->getID());
                else
              ...
              if (container->getContain() &&
                  container->getContain()->isValidContainerFor(payload, true)) {
                container->getContain()->addToContain(payload);
                payload = container;
              } else {
                DEBUG_CRASH(
                    ("DeliverPayload: PutInContainer %s is full, or not valid "
                     "for the payload %s!",
                     m_putInContainerName.str(), it->m_payloadName.str()));
              }
            }

            if (transport->getContain() &&
                transport->getContain()->isValidContainerFor(payload, true)) {
              transport->getContain()->addToContain(payload);
            } else {
              DEBUG_CRASH(
                  ("DeliverPayload: transport %s is full, or not valid for the "
                   "payload %s!",
                   m_transportName.str(), it->m_payloadName.str()));
            }
          }
        }
      } else {
  ...
  static void parsePayload(INI* ini, void* instance, void* store,
                           const void* /*userData*/) {
  ...
  static void parse(INI* ini, void* instance, void* /*store*/,
    ...
    static const FieldParse myFieldParse[] = {
        //***************************************************************
        // OBJECT CREATION LIST SPECIFIC DATA -- once created data no longer
        // needed The transport(s) that carry all the payload items (and initial
        // physics information)
        {"Transport", INI::parseAsciiString, NULL,
         offsetof(DeliverPayloadNugget, m_transportName)},
        {"StartAtPreferredHeight", INI::parseBool, NULL,
         offsetof(DeliverPayloadNugget, m_startAtPreferredHeight)},
        {"StartAtMaxSpeed", INI::parseBool, NULL,
         offsetof(DeliverPayloadNugget, m_startAtMaxSpeed)},

        // For multiple transports, this defines the formation (and convergence
        // if all weapons will hit same target)
        {"FormationSize", INI::parseUnsignedInt, NULL,
         offsetof(DeliverPayloadNugget, m_formationSize)},
        {"FormationSpacing", INI::parseReal, NULL,
         offsetof(DeliverPayloadNugget, m_formationSpacing)},
        {"WeaponConvergenceFactor", INI::parseReal, NULL,
         offsetof(DeliverPayloadNugget, m_convergenceFactor)},
        {"WeaponErrorRadius", INI::parseReal, NULL,
         offsetof(DeliverPayloadNugget, m_errorRadius)},
        {"DelayDeliveryMax", INI::parseDurationUnsignedInt, NULL,
         offsetof(DeliverPayloadNugget, m_delayDeliveryFramesMax)},

        // Payload information (it's all created now and stored inside)
        {"Payload", parsePayload, NULL, 0},
        {"PutInContainer", INI::parseAsciiString, NULL,
         offsetof(DeliverPayloadNugget, m_putInContainerName)},
    ...
    p.add(myFieldParse);
    p.add(DeliverPayloadData::getFieldParse(),
          offsetof(DeliverPayloadNugget, m_data));
 ...
 private:
  ...
  // Specific data needed to create the transport(s), internal payload, and
  // initial physics.
...
//-------------------------------------------------------------------------------------------------
static void calcRandomForce(Real minMag, Real maxMag, Real minPitch,
                            Real maxPitch, Coord3D* force) {
...
class ApplyRandomForceNugget : public ObjectCreationNugget {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(ApplyRandomForceNugget,
 ...
 public:
  ...
  virtual void create(const Object* primary, const Object* secondary,
                      UnsignedInt lifetimeFrames = 0) const {
    if (primary) {
  ...
  static void parse(INI* ini, void* instance, void* /*store*/,
                    const void* /*userData*/) {
    static const FieldParse myFieldParse[] = {
        {"SpinRate", INI::parseAngularVelocityReal, NULL,
         offsetof(ApplyRandomForceNugget, m_spinRate)},
        {"MinForceMagnitude", INI::parseReal, NULL,
         offsetof(ApplyRandomForceNugget, m_minMag)},
        {"MaxForceMagnitude", INI::parseReal, NULL,
         offsetof(ApplyRandomForceNugget, m_maxMag)},
        {"MinForcePitch", INI::parseAngleReal, NULL,
         offsetof(ApplyRandomForceNugget, m_minPitch)},
        {"MaxForcePitch", INI::parseAngleReal, NULL,
         offsetof(ApplyRandomForceNugget, m_maxPitch)},
        {0, 0, 0, 0}};
...
static const char* DebrisDispositionNames[] = {
    "LIKE_EXISTING", "ON_GROUND_ALIGNED", "SEND_IT_FLYING",
    "SEND_IT_UP",    "SEND_IT_OUT",       "RANDOM_FORCE",
    "FLOATING",      "INHERIT_VELOCITY",  "WHIRLING",
};
...
class GenericObjectCreationNugget : public ObjectCreationNugget {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(GenericObjectCreationNugget,
 ...
 public:
  GenericObjectCreationNugget()
      : m_requiresLivePlayer(FALSE),
        ...
        m_fadeFrames(0),
        m_fadeSoundName(
            AsciiString::TheEmptyString),  // Added By Sadullah Nader
        m_particleSysName(
            AsciiString::TheEmptyString),  // Added By Sadullah Nader
        m_putInContainer(
            AsciiString::TheEmptyString),  // Added By Sadullah Nader
  ...
  virtual void create(const Object* primary, const Object* secondary,
                      UnsignedInt lifetimeFrames = 0) const {
    if (primary) {
      if (m_skipIfSignificantlyAirborne &&
          primary->isSignificantlyAboveTerrain())
        return;

      reallyCreate(primary->getPosition(), primary->getTransformMatrix(),
                   primary->getOrientation(), primary, lifetimeFrames);
    } else {
      DEBUG_CRASH(("You must have a primary source for this effect"));
    }
  }
  ...
  static const FieldParse* getCommonFieldParse() {
    static const FieldParse commonFieldParse[] = {
        {"PutInContainer", INI::parseAsciiString, NULL,
         offsetof(GenericObjectCreationNugget, m_putInContainer)},
        {"ParticleSystem", INI::parseAsciiString, NULL,
         offsetof(GenericObjectCreationNugget, m_particleSysName)},
        {"Count", INI::parseInt, NULL,
         offsetof(GenericObjectCreationNugget, m_debrisToGenerate)},
        {"IgnorePrimaryObstacle", INI::parseBool, NULL,
         offsetof(GenericObjectCreationNugget, m_ignorePrimaryObstacle)},
        {"OrientInForceDirection", INI::parseBool, NULL,
         offsetof(GenericObjectCreationNugget, m_orientInForceDirection)},
        {"ExtraBounciness", INI::parseReal, NULL,
         offsetof(GenericObjectCreationNugget, m_extraBounciness)},
        {"ExtraFriction", parseFrictionPerSec, NULL,
         offsetof(GenericObjectCreationNugget, m_extraFriction)},
        {"Offset", INI::parseCoord3D, NULL,
         offsetof(GenericObjectCreationNugget, m_offset)},
        {"Disposition", INI::parseBitString32, DebrisDispositionNames,
         offsetof(GenericObjectCreationNugget, m_disposition)},
        {"DispositionIntensity", INI::parseReal, NULL,
         offsetof(GenericObjectCreationNugget, m_dispositionIntensity)},
        {"SpinRate", INI::parseAngularVelocityReal, NULL,
         offsetof(GenericObjectCreationNugget, m_spinRate)},
        {"YawRate", INI::parseAngularVelocityReal, NULL,
         offsetof(GenericObjectCreationNugget, m_yawRate)},
        {"RollRate", INI::parseAngularVelocityReal, NULL,
         offsetof(GenericObjectCreationNugget, m_rollRate)},
        {"PitchRate", INI::parseAngularVelocityReal, NULL,
         offsetof(GenericObjectCreationNugget, m_pitchRate)},
        {"MinForceMagnitude", INI::parseReal, NULL,
         offsetof(GenericObjectCreationNugget, m_minMag)},
        {"MaxForceMagnitude", INI::parseReal, NULL,
         offsetof(GenericObjectCreationNugget, m_maxMag)},
        {"MinForcePitch", INI::parseAngleReal, NULL,
         offsetof(GenericObjectCreationNugget, m_minPitch)},
        {"MaxForcePitch", INI::parseAngleReal, NULL,
         offsetof(GenericObjectCreationNugget, m_maxPitch)},
        {"MinLifetime", INI::parseDurationUnsignedInt, NULL,
         offsetof(GenericObjectCreationNugget, m_minFrames)},
        {"MaxLifetime", INI::parseDurationUnsignedInt, NULL,
         offsetof(GenericObjectCreationNugget, m_maxFrames)},
        {"SpreadFormation", INI::parseBool, NULL,
         offsetof(GenericObjectCreationNugget, m_spreadFormation)},
        {"MinDistanceAFormation", INI::parseReal, NULL,
         offsetof(GenericObjectCreationNugget, m_minDistanceAFormation)},
        {"MinDistanceBFormation", INI::parseReal, NULL,
         offsetof(GenericObjectCreationNugget, m_minDistanceBFormation)},
        {"MaxDistanceFormation", INI::parseReal, NULL,
         offsetof(GenericObjectCreationNugget, m_maxDistanceFormation)},
        {"FadeIn", INI::parseBool, NULL,
         offsetof(GenericObjectCreationNugget, m_fadeIn)},
        {"FadeOut", INI::parseBool, NULL,
         offsetof(GenericObjectCreationNugget, m_fadeOut)},
        {"FadeTime", INI::parseDurationUnsignedInt, NULL,
         offsetof(GenericObjectCreationNugget, m_fadeFrames)},
        {"FadeSound", INI::parseAsciiString, NULL,
         offsetof(GenericObjectCreationNugget, m_fadeSoundName)},
        {"PreserveLayer", INI::parseBool, NULL,
         offsetof(GenericObjectCreationNugget, m_preserveLayer)},
        {0, 0, 0, 0}};
    return commonFieldParse;
  }

  static void parseObject(INI* ini, void* instance, void* /*store*/,
                          const void* /*userData*/) {
    static const FieldParse myFieldParse[] = {
        {"ContainInsideSourceObject", INI::parseBool, NULL,
         offsetof(GenericObjectCreationNugget, m_containInsideSourceObject)},
        {"ObjectNames", parseDebrisObjectNames, NULL, 0},
        {"ObjectCount", INI::parseInt, NULL,
         offsetof(GenericObjectCreationNugget, m_objectCount)},
        {"InheritsVeterancy", INI::parseBool, NULL,
         offsetof(GenericObjectCreationNugget, m_inheritsVeterancy)},
        {"SkipIfSignificantlyAirborne", INI::parseBool, NULL,
         offsetof(GenericObjectCreationNugget, m_skipIfSignificantlyAirborne)},
        {"InvulnerableTime", INI::parseDurationUnsignedInt, NULL,
         offsetof(GenericObjectCreationNugget, m_invulnerableTime)},
        {"MinHealth", INI::parsePercentToReal, NULL,
         offsetof(GenericObjectCreationNugget, m_minHealth)},
        {"MaxHealth", INI::parsePercentToReal, NULL,
         offsetof(GenericObjectCreationNugget, m_maxHealth)},
        {"RequiresLivePlayer", INI::parseBool, NULL,
         offsetof(GenericObjectCreationNugget, m_requiresLivePlayer)},
        {0, 0, 0, 0}};
    ...
    GenericObjectCreationNugget* nugget =
        newInstance(GenericObjectCreationNugget);
  ...
  static void parseDebris(INI* ini, void* instance, void* /*store*/,
                          const void* /*userData*/) {
    static const FieldParse myFieldParse[] = {
        {"ModelNames", parseDebrisObjectNames, NULL, 0},
        {"Mass", INI::parsePositiveNonZeroReal, NULL,
         offsetof(GenericObjectCreationNugget, m_mass)},
        {"AnimationSet", parseAnimSet, NULL,
         offsetof(GenericObjectCreationNugget, m_animSets)},
        {"FXFinal", INI::parseFXList, NULL,
         offsetof(GenericObjectCreationNugget, m_fxFinal)},
        {"OkToChangeModelColor", INI::parseBool, NULL,
         offsetof(GenericObjectCreationNugget, m_okToChangeModelColor)},
        {"MinLODRequired", INI::parseStaticGameLODLevel, NULL,
         offsetof(GenericObjectCreationNugget, m_minLODRequired)},
        {"Shadow", INI::parseBitString32, TheShadowNames,
         offsetof(GenericObjectCreationNugget, m_shadowType)},
        {"BounceSound", INI::parseAudioEventRTS, NULL,
         offsetof(GenericObjectCreationNugget, m_bounceSound)},
        {0, 0, 0, 0}};
    ...
    DEBUG_ASSERTCRASH(nugget->m_mass > 0.0f,
                      ("Zero masses are not allowed for debris!\n"));
    ((ObjectCreationList*)instance)->addObjectCreationNugget(nugget);
  }
 ...
 protected:
  void doStuffToObj(Object* obj, const AsciiString& modelName,
                    const Coord3D* pos, const Matrix3D* mtx, Real orientation,
                    const Object* sourceObj, UnsignedInt lifetimeFrames) const {
    obj->setProducer(sourceObj);

    static NameKeyType key_LifetimeUpdate = NAMEKEY("LifetimeUpdate");
    LifetimeUpdate* lup =
        (LifetimeUpdate*)obj->findUpdateModule(key_LifetimeUpdate);
    if (lup) {
      if (lifetimeFrames) {
        ...
        // They will both be zero if no lifetime was specified in the OCL.  It
        // could be in the Object so don't mess with it. So the OCL listing will
        // override the Object listing for lifetime, but ONLY if there is one.
    ...
    if (!m_nameAreObjects) {
      for (DrawModule** dm = obj->getDrawable()->getDrawModules(); *dm; ++dm) {
        ...
        if (di) {
          di->setModelName(
              modelName, m_okToChangeModelColor ? obj->getIndicatorColor() : 0,
              m_shadowType);
          if (m_animSets.size() > 0) {
            Int which = GameLogicRandomValue(0, m_animSets.size() - 1);
            di->setAnimNames(m_animSets[which].m_animInitial,
                             m_animSets[which].m_animFlying,
    ...
    if (!m_particleSysName.isEmpty()) {
      ...
      if (tmp) {
        ParticleSystem* sys =
            TheParticleSystemManager->createParticleSystem(tmp);
    ...
    if (m_ignorePrimaryObstacle) {
      ...
      if (p) p->setIgnoreCollisionsWith(sourceObj);
    }

    // set its beginning health
    BodyModuleInterface* body = obj->getBodyModule();
    Real healthPercent = GameLogicRandomValueReal(m_minHealth, m_maxHealth);
    if (body) body->setInitialHealth(healthPercent * 100.0f);

    // If they have a SlavedUpdate, then I have to tell them who their daddy is
    // from now on.
    for (BehaviorModule** update = obj->getBehaviorModules(); *update;
         ++update) {
    ...
    if (m_inheritsVeterancy && sourceObj &&
        obj->getExperienceTracker()->isTrainable()) {
      DEBUG_LOG(("Object %s inherits veterancy level %d from %s\n",
                 obj->getTemplate()->getName().str(),
                 sourceObj->getVeterancyLevel(),
      ...
      // In order to make things easier for the designers, we are going to
      // transfer the unit name to the ejected thing... so the designer can
      // control the pilot with the scripts.
    ...
    if (BitTest(m_disposition, LIKE_EXISTING)) {
      ...
      if (sourceObj && sourceObj->isAboveTerrain()) {
        ...
        if (physics) physics->setAllowToFall(true);
      }
    }
    ...
    if (BitTest(m_disposition, ON_GROUND_ALIGNED)) {
      ...
      PathfindLayerEnum layer =
          TheTerrainLogic->getHighestLayerForDestination(&chunkPos);
      obj->setOrientation(GameLogicRandomValueReal(0.0f, 2 * PI));
      chunkPos.z =
          TheTerrainLogic->getLayerHeight(chunkPos.x, chunkPos.y, layer);
    ...
    if (BitTest(m_disposition, SEND_IT_OUT)) {
      ...
      if (objUp) {
        if (!m_nameAreObjects) objUp->setMass(m_mass);
        ...
        if (m_orientInForceDirection) orientation = atan2(force.y, force.x);
      }
    }
    ...
    if (BitTest(m_disposition, SEND_IT_FLYING | SEND_IT_UP | RANDOM_FORCE)) {
      ...
      if (objUp) {
        ...
        DEBUG_ASSERTCRASH(objUp->getMass() > 0.0f,
                          ("Zero masses are not allowed for obj!\n"));
        ...
        Real spinRate = m_spinRate >= 0.0f
                            ? m_spinRate
                            : (PI / 32.0f) * m_dispositionIntensity;
    ...
    if (BitTest(m_disposition, WHIRLING)) {
      PhysicsBehavior* objUp = obj->getPhysics();
      if (objUp) {
        Real yaw = GameLogicRandomValueReal(-m_dispositionIntensity,
                                            m_dispositionIntensity);
        Real roll = GameLogicRandomValueReal(-m_dispositionIntensity,
                                             m_dispositionIntensity);
        Real pitch = GameLogicRandomValueReal(-m_dispositionIntensity,
                                              m_dispositionIntensity);
    ...
    if (m_containInsideSourceObject) {
      ...
      if (sourceObj->getContain() &&
          sourceObj->getContain()->isValidContainerFor(obj, TRUE)) {
        sourceObj->getContain()->addToContain(obj);

        // Need to hide if they are hidden.
        if (sourceObj->getDrawable() && obj->getDrawable() &&
            sourceObj->getDrawable()->isDrawableEffectivelyHidden())
          obj->getDrawable()->setDrawableHidden(TRUE);
      } else {
        DEBUG_ASSERTCRASH(FALSE,
                          ("A OCL with ContainInsideSourceObject failed the "
                           "contain and is killing the new object."));
  ...
  void reallyCreate(const Coord3D* pos, const Matrix3D* mtx, Real orientation,
                    const Object* sourceObj, UnsignedInt lifetimeFrames) const {
    static const ThingTemplate* debrisTemplate =
        TheThingFactory->findTemplate("GenericDebris");

    if (m_names.size() <= 0) return;

    if (m_requiresLivePlayer &&
        (!sourceObj || !sourceObj->getControllingPlayer()->isPlayerActive()))
      return;  // don't spawn useful objects for dead players.  Avoid the zombie
               // units from Yuri's.

    // Object type debris might need this information to process visual
    // UpgradeModules.
    ...
    if (!m_putInContainer.isEmpty()) {
      const ThingTemplate* containerTmpl =
          TheThingFactory->findTemplate(m_putInContainer);
      if (containerTmpl) {
    ...
    for (Int nn = 0; nn < m_debrisToGenerate; nn++) {
      ...
      else {  // this is using the generic debris type so it's probably safe to
        // remove if requested by the GameLOD manager.
        if (TheGameLODManager->isDebrisSkipped()) continue;
      ...
      if (m_preserveLayer && sourceObj != NULL && container == NULL) {
        ...
        if (layer != LAYER_GROUND) debris->setLayer(layer);
      }

      if (container != NULL && container->getContain() != NULL &&
          container->getContain()->isValidContainerFor(debris, true))
      ...
      if (m_spreadFormation) {
        ...
        fpOptions.minRadius = GameLogicRandomValueReal(m_minDistanceAFormation,
                                                       m_minDistanceBFormation);
        ...
        doStuffToObj(debris, m_names[pick], &resultPos, mtx, orientation,
                     sourceObj, lifetimeFrames);
      } else {
        // do stuff to contained objects too
        doStuffToObj(debris, m_names[pick], pos, mtx, orientation, sourceObj,
                     lifetimeFrames);
      }
    ...
    if (container)
      doStuffToObj(container, AsciiString::TheEmptyString, pos, mtx,
                   orientation, sourceObj, lifetimeFrames);
  }

  static void parseDebrisObjectNames(INI* ini, void* instance, void* store,
                                     const void* /*userData*/) {
    GenericObjectCreationNugget* debrisNugget =
        (GenericObjectCreationNugget*)instance;
    for (const char* debrisName = ini->getNextToken(); debrisName;
 ...
 private:
  ...
  Bool m_containInsideSourceObject;  ///< The created stuff will be added to the
                                     ///< Conatin module of the SourceObject
...
//-------------------------------------------------------------------------------------------------
void ObjectCreationList::create(const Object* primaryObj,
                                const Coord3D* primary,
                                const Coord3D* secondary, Bool createOwner,
                                UnsignedInt lifetimeFrames) const {
  DEBUG_ASSERTCRASH(primaryObj != NULL,
                    ("You should always call OCLs with a non-null primary Obj, "
                     "even for positional calls, to get team ownership right"));
  for (ObjectCreationNuggetVector::const_iterator i = m_nuggets.begin();
       i != m_nuggets.end(); ++i) {
...
//-------------------------------------------------------------------------------------------------
void ObjectCreationList::create(const Object* primary, const Object* secondary,
                                UnsignedInt lifetimeFrames) const {
  DEBUG_ASSERTCRASH(primary != NULL,
                    ("You should always call OCLs with a non-null primary Obj, "
                     "even for positional calls, to get team ownership right"));
  for (ObjectCreationNuggetVector::const_iterator i = m_nuggets.begin();
       i != m_nuggets.end(); ++i) {
...
//-------------------------------------------------------------------------------------------------
ObjectCreationListStore::~ObjectCreationListStore() {
  for (ObjectCreationNuggetVector::iterator i = m_nuggets.begin();
       i != m_nuggets.end(); ++i) {
    if (*i) (*i)->deleteInstance();
  }
  m_nuggets.clear();
}

//-------------------------------------------------------------------------------------------------
const ObjectCreationList* ObjectCreationListStore::findObjectCreationList(
    const char* name) const {
  if (stricmp(name, "None") == 0) return NULL;
...
//-------------------------------------------------------------------------------------------------
void ObjectCreationListStore::addObjectCreationNugget(
    ObjectCreationNugget* nugget) {
  m_nuggets.push_back(nugget);
}

//-------------------------------------------------------------------------------------------------
/*static */ void ObjectCreationListStore::parseObjectCreationListDefinition(
    INI* ini) {
