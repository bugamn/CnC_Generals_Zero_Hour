...
// /////////////////////////////////////////////////////////////////// Author:
// Colin Day, March 2002 Desc:   Damage module capable of launching various
// effects on damage transitions
///////////////////////////////////////////////////////////////////////////////////////////////////
...
static void parseFXLocInfo(INI *ini, void *instance, FXLocInfo *locInfo) {
  ...
  if (stricmp(token, "bone") == 0) {
    ...
    //
    // bones are followed by RandomBone:<Yes|No>, if random bone is yes, the
    // bone name is assumed to be a "base bone name" and we will find all the
    // bones with that prefix when picking an effect position.  If it's no, the
...
//-------------------------------------------------------------------------------------------------
/** In the form of:
 * FXListSlot = <<Bone:BoneName BoneRandom:<Yes|No>> | <Loc: X:x Y:y Z:z>>
 * FXList:FXListName */
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
/** In the form of:
 * OCLSlot = <<Bone:BoneName BoneRandom:<Yes|No>> | <Loc: X:x Y:y Z:z>>
 * OCL:OCLName */
//-------------------------------------------------------------------------------------------------
void TransitionDamageFXModuleData::parseObjectCreationList(
...
//-------------------------------------------------------------------------------------------------
/** In the form of:
 * ParticleSlot = <<Bone:BoneName BoneRandom:<Yes|No>> | <Loc: X:x Y:y Z:z>>
 * PSys:PSysName */
//-------------------------------------------------------------------------------------------------
void TransitionDamageFXModuleData::parseParticleSystem(INI *ini, void *instance,
  ...
  ini->parseParticleSystemTemplate(ini, instance, store,
                                   &info->particleSysTemplate);
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
TransitionDamageFX::~TransitionDamageFX(void) {}  // end ~TransitionDamageFX
...
void TransitionDamageFX::onDelete( void )
{
        ...
        //
        // we would in theory delete any particle systems we have created and
attached, but the
        // particle system will automatically delete itself when the object is
destroyed
        //
...
//-------------------------------------------------------------------------------------------------
/** Given an FXLoc info struct, return the effect position that we are supposed
 * to use. The position is local to to the object */
//-------------------------------------------------------------------------------------------------
static Coord3D getLocalEffectPos(const FXLocInfo *locInfo, Drawable *draw) {
  ...
  if (locInfo->locType == FX_DAMAGE_LOC_TYPE_BONE && draw) {
    if (locInfo->randomBone == FALSE) {
      ...
      Int count = draw->getPristineBonePositions(locInfo->boneName.str(), 0,
                                                 &pos, NULL, 1);

      // sanity, if bone not found revert back to location defined in struct
      // (which is 0,0,0)
      if (count == 0) return locInfo->loc;
      ...
      Int boneCount;
      boneCount = draw->getPristineBonePositions(locInfo->boneName.str(), 1,
                                                 positions, NULL, MAX_BONES);

      // sanity, if bone not found revert back to location defined in struct
      // (which is 0,0,0)
      if (boneCount == 0) return locInfo->loc;
...
void TransitionDamageFX::onBodyDamageStateChange(const DamageInfo *damageInfo,
  ...
  const TransitionDamageFXModuleData *modData =
      getTransitionDamageFXModuleData();
  ...
  //
  // when we are transitioning to a "worse" state we will play a set of effects
  // for that new state to make the transition
  //
  if (IS_CONDITION_WORSE(newState, oldState)) {
    const ParticleSystemTemplate *pSystemT;
    Coord3D pos;

    // if we are restricted by the damage type executing effect, bail out of
    // here
    ...
    for (i = 0; i < DAMAGE_MODULE_MAX_FX; i++) {
      ...
      if (modData->m_fxList[newState][i].fx) {
        if (lastDamageInfo == NULL ||
            getDamageTypeFlag(modData->m_damageFXTypes,
                              lastDamageInfo->in.m_damageType)) {
          pos =
              getLocalEffectPos(&modData->m_fxList[newState][i].locInfo, draw);
      ...
      if (modData->m_OCL[newState][i].ocl) {
        if (lastDamageInfo == NULL ||
            getDamageTypeFlag(modData->m_damageOCLTypes,
                              lastDamageInfo->in.m_damageType)) {
      ...
      if (pSystemT) {
        if (lastDamageInfo == NULL ||
            getDamageTypeFlag(modData->m_damageParticleTypes,
                              lastDamageInfo->in.m_damageType)) {
          // create a new particle system based on the template provided
          ParticleSystem *pSystem =
              TheParticleSystemManager->createParticleSystem(pSystemT);
          if (pSystem) {
            // get the what is the position we're going to playe the effect at
            pos = getLocalEffectPos(
                &modData->m_particleSystem[newState][i].locInfo, draw);

            //
            // set position on system given any bone position provided, the bone
            // position is local to the object and that's what we want for the
            // particle system ... the transormation into world space using the
            // object position is taken care of in the particle system
            ...
            // save the id of this particle system so we can remove it later if
            // it still exists
...
void TransitionDamageFX::xfer(Xfer *xfer) {
  ...
  xfer->xferUser(
      m_particleSystemID,
      sizeof(ParticleSystemID) * BODYDAMAGETYPE_COUNT * DAMAGE_MODULE_MAX_FX);
