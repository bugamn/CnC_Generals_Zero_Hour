...
SimpleObjectIterator::ClumpCompareProc
    SimpleObjectIterator::theClumpCompareProcs[] = {
...
//=============================================================================
SimpleObjectIterator::Clump::Clump() { m_nextClump = NULL; }
...
//=============================================================================
SimpleObjectIterator::~SimpleObjectIterator() { makeEmpty(); }
...
Object *SimpleObjectIterator::nextWithNumeric(Real *num) {
  ...
  if (m_curClump) {
    ...
    if (num) *num = m_curClump->m_numeric;
...
//=============================================================================
void SimpleObjectIterator::reset() { m_curClump = m_firstClump; }
...
void SimpleObjectIterator::sort(IterOrderType order) {
  ...
  if (!cmpProc) return;  // my, that was easy
  ...
  for (Int n = 1;; n *= 2) {
    ...
    while (to_do) {
      ...
      while (to_do_count + subCount > 0) {
        ...
        if (subCount == 0) {
          DEBUG_ASSERTCRASH(to_do_count > 0,
                            ("hmm, expected nonzero to_do_count"));
          tmp = to_do;
    ...
    if (tail) tail->m_nextClump = NULL;

    if (mergeCount <=
        1)  // when we have done just one (or none) swap, we're done
      break;
  }
