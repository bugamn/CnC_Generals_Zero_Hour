...
void ObjectTypes::removeObjectType(const AsciiString &objectType) {
  if (!isInSet(objectType)) {
    DEBUG_CRASH(("Attempted to remove '%s' from '%s', but it wasn't there.",
                 objectType.str(), m_listName.str()));
    return;
  }

  AsciiStringVec::iterator it =
      std::find(m_objectTypes.begin(), m_objectTypes.end(), objectType);

  m_objectTypes.erase(it);
}

//-------------------------------------------------------------------------------------------------
const AsciiString &ObjectTypes::getListName() const { return m_listName; }
...
Bool ObjectTypes::isInSet(const AsciiString &objectType) const {
  return (std::find(m_objectTypes.begin(), m_objectTypes.end(), objectType) !=
          m_objectTypes.end());
}
...
//-------------------------------------------------------------------------------------------------
Int ObjectTypes::prepForPlayerCounting(
    std::vector<const ThingTemplate *> &templates, std::vector<Int> &counts) {
  AsciiStringVecIt it;
...
void ObjectTypes::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    if (m_objectTypes.empty() == FALSE) {
      DEBUG_CRASH(
          ("ObjectTypes::xfer - m_objectTypes vector should be emtpy but is "
           "not!\n"));
