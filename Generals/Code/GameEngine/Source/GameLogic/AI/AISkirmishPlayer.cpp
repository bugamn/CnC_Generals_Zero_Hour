...
AISkirmishPlayer::~AISkirmishPlayer() { clearTeamsInQueue(); }
  ...
  if (m_readyToBuildStructure) {
        ...
        if (bldg == NULL) {
          ...
          if (bldg->getControllingPlayer() == m_player) {
            ...
            if (BitTest(bldg->getStatusBits(),
              ...
              if (bldg->isKindOf(KINDOF_FS_POWER) &&
                  !bldg->isKindOf(KINDOF_CASH_GENERATOR)) {
    ...
    if (bldgPlan && bldgInfo) {
      ...
      if (TheGameLogic->getFrame() - m_frameLastBuildingBuilt < framesToBuild) {
        m_buildDelay = framesToBuild -
                       (TheGameLogic->getFrame() - m_frameLastBuildingBuilt);
        return;
      } else {
 ...
 * of the queue.
 */
Bool AISkirmishPlayer::startTraining(WorkOrder *order, Bool busyOK,
...
void AISkirmishPlayer::buildSpecificAIBuilding(const AsciiString &thingName) {
  ...
  if (foundUnbuilt) {
    ...
    buildingStr.concat(thingName);
    buildingStr.concat(
        "' are already built or queued for build, not queueing.");
    TheScriptEngine->AppendDebugMessage(buildingStr, false);
  } else {
...
Int AISkirmishPlayer::getMyEnemyPlayerIndex(void) {
  ...
  for (playerNdx = 0; playerNdx < ThePlayerList->getPlayerCount();
       playerNdx++) {
    if (ThePlayerList->getNthPlayer(playerNdx)->getPlayerType() ==
        PLAYER_HUMAN) {
      break;
    }
  }
...
void AISkirmishPlayer::acquireEnemy(void) {
  ...
  for (i = 0; i < ThePlayerList->getPlayerCount(); i++) {
    ...
    if (m_player->getRelationship(curPlayer->getDefaultTeam()) == ENEMIES) {
      if (curPlayer->hasAnyObjects() == false)
        continue;  // not much of an enemy.
      // ok, we got an enemy;
      // If a player is out of units, or out of build facilities, we can lower
      // his priority.
      Bool inBadShape =
          !curPlayer->hasAnyUnits() || !curPlayer->hasAnyBuildFacility();
      ...
      Real curDistSqr =
          sqr(enemyPos.x - m_baseCenter.x) + sqr(enemyPos.y - m_baseCenter.y);

      // Fudge for in bad shape.	 If an enemy is crippled, concentrate on
      // the other ones.
      if (inBadShape) {
      ...
      for (k = 0; k < ThePlayerList->getPlayerCount(); k++) {
        ...
        if (somePlayer->isSkirmishAIPlayer() &&
            (somePlayer->getCurrentEnemy() == curPlayer)) {
          // Some ai is already targeting this guy.  Add a distance penalty.
          curDistSqr += (500 * 500);
        }
        if (somePlayer->isSkirmishAIPlayer() &&
            (somePlayer->getCurrentEnemy() == m_player)) {
  ...
  if (bestEnemy != NULL && (bestEnemy != m_currentEnemy)) {
    ...
    msg.concat(
        TheNameKeyGenerator->keyToName(m_currentEnemy->getPlayerNameKey()));
...
/**
        ...
        Front defenses (!flank) are placed starting at the "Center" approach
   path. The first front defense is placed towards th Center path.  Number 2 is
   placed to the left of #1, #3 is placed to the right of #1, #4 is placed to
   the left of #2 and so on.  So it looks like:
   ...
   #9
        ...
        The flank base defenses cover the "Flank" approach, and the "Backdoor"
   approach. They alternate between these two, so the first flank defense covers
   flank, and the second covers backdoor, and continue to alternate.  They cover
   the approach using the same pattern as front above. John A.
...
void AISkirmishPlayer::buildAIBaseDefenseStructure(const AsciiString &thingName,
  ...
  if (tTemplate == NULL) {
    DEBUG_CRASH(("Couldn't find base defense structure '%s' for side %s",
                 thingName.str(), m_player->getSide().str()));
    return;
  }
  do {
    ...
    if (flank) {
      if (m_curFlankBaseDefense & 1) {
        pathLabel.format("%s%d", SKIRMISH_FLANK,
                         m_player->getMpStartIndex() + 1);
      } else {
        pathLabel.format("%s%d", SKIRMISH_BACKDOOR,
                         m_player->getMpStartIndex() + 1);
      }
    } else {
      pathLabel.format("%s%d", SKIRMISH_CENTER,
                       m_player->getMpStartIndex() + 1);
    }

    Coord3D goalPos = m_baseCenter;
    Waypoint *way =
        TheTerrainLogic->getClosestWaypointOnPath(&goalPos, pathLabel);
    if (way) {
    ...
    Real structureRadius =
        tTemplate->getTemplateGeometryInfo().getBoundingCircleRadius();
    ...
    DEBUG_LOG(("Offset is %f  %f, new is %f, %f \n", offset.x, offset.y,
               offset.x * c - offset.y * s, offset.y * c + offset.x * s));
    ...
    canBuild = LBC_OK == TheBuildAssistant->isLocationLegalToBuild(
                             &buildPos, tTemplate, placeAngle,
...
/**
        Checks bridges along a waypoint path.  If any are destroyed, sends a
   dozer to fix, and returns true. If there is no bridge problem, returns false.
        */
Bool AISkirmishPlayer::checkBridges(Object *unit, Waypoint *way) {
  ...
  for (curWay = way; curWay; curWay = curWay->getNext()) {
    if (TheAI->pathfinder()->quickDoesPathExist(locoSet, &unitPos,
                                                curWay->getLocation())) {
      continue;
    }
    ObjectID brokenBridge = INVALID_ID;
    if (TheAI->pathfinder()->findBrokenBridge(
            locoSet, &unitPos, curWay->getLocation(), &brokenBridge)) {
...
/**
        Recruit a specific team, within the specific radius of the home
   position.
        */
 ...
 */
void AISkirmishPlayer::checkReadyTeams(void) { AIPlayer::checkReadyTeams(); }

//----------------------------------------------------------------------------------------------------------
/**
 * See if any queued teams have finished building, or have run out of time.
 */
void AISkirmishPlayer::checkQueuedTeams(void) { AIPlayer::checkQueuedTeams(); }
 ...
 */
void AISkirmishPlayer::update(void) { AIPlayer::update(); }
...
void AISkirmishPlayer::newMap(void) {
  ...
  DEBUG_ASSERTLOG(build != NULL,
                  ("Couldn't find build list for skirmish player.\n"));
 ...
 * Queues up a dozer.
 */
void AISkirmishPlayer::queueDozer(void) { AIPlayer::queueDozer(); }
 ...
 */
void AISkirmishPlayer::computeSuperweaponTarget(
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AISkirmishPlayer::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AISkirmishPlayer::loadPostProcess(void) {}  // end loadPostProcess
