...
void AICommandParmsStorage::store(const AICommandParms &parms) {
  ...
  m_teamName =
      parms.m_team ? parms.m_team->getName() : AsciiString::TheEmptyString;
  ...
  m_path = parms.m_path;  /// @todo srj -- probably need a better way to safely
                          /// save/restore this
}
...
void AICommandParmsStorage::reconstitute(AICommandParms &parms) const {
  ...
  parms.m_path = m_path;  /// @todo srj -- probably need a better way to safely
                          /// save/restore this
}
 ...
 */
static Bool isSamePosition(const Coord3D *ourPos, const Coord3D *prevTargetPos,
                           const Coord3D *curTargetPos) {
  Coord3D diff;
  ...
  Real toleranceSqr =
      (toTarget.x * toTarget.x + toTarget.y * toTarget.y) * TOLERANCE_FACTOR;
 ...
 */
AttackStateMachine::AttackStateMachine(Object *obj, AIAttackState *att,
                                       AsciiString name, Bool follow,
                                       Bool attackingObject,
                                       Bool forceAttacking)
    : StateMachine(obj, name) {
  // we want to use the CONTINUE mode (not NEW) since we already have acquired
  // the target.
  static const StateConditionInfo objectConditionsNormal[] = {
      StateConditionInfo(outOfWeaponRangeObject,
                         AttackStateMachine::CHASE_TARGET, NULL),
      StateConditionInfo(wantToSquishTarget, AttackStateMachine::CHASE_TARGET,
                         NULL),
      StateConditionInfo(cannotPossiblyAttackObject, EXIT_MACHINE_WITH_FAILURE,
                         (void *)ATTACK_CONTINUED_TARGET),
  ...
  static const StateConditionInfo objectConditionsForced[] = {
      StateConditionInfo(outOfWeaponRangeObject,
                         AttackStateMachine::CHASE_TARGET, NULL),
      StateConditionInfo(cannotPossiblyAttackObject, EXIT_MACHINE_WITH_FAILURE,
                         (void *)ATTACK_CONTINUED_TARGET_FORCED),
      StateConditionInfo(wantToSquishTarget, AttackStateMachine::CHASE_TARGET,
                         NULL),
      StateConditionInfo(NULL, NULL, NULL)  // keep last
  };
  ...
  static const StateConditionInfo positionConditions[] = {
      StateConditionInfo(outOfWeaponRangePosition,
                         AttackStateMachine::CHASE_TARGET, NULL),
      StateConditionInfo(NULL, NULL, NULL)  // keep last
  };
  ...
  // The default is Aim rather than Approach so things that cannot move will be
  // able to shoot things that are in range.  Things that cannot move will
  // automatically FAILURE on approach state.
  /*
          This state will succeed when we are aiming a useful weapon at the
     victim, and fail if the victim is dead. (Exception: if the weapon is on a
     turret, we don't leave this state unless we get out of range.)
  */
  defineState(AttackStateMachine::AIM_AT_TARGET,
              newInstance(AIAttackAimAtTargetState)(this, attackingObject,
                                                    forceAttacking),
              AttackStateMachine::FIRE_WEAPON, EXIT_MACHINE_WITH_FAILURE,
              attackingObject ? objectConditions : positionConditions);

  /*
          Note that the fire state succeeds iff it is able to fire... it will
     "fail" if unable to fire. However, it may be unable to fire because the
     target object is already dead.
  */
  ...
  if (obj->isKindOf(KINDOF_IMMOBILE) == FALSE) {
    if (obj->isKindOf(KINDOF_PORTABLE_STRUCTURE) &&
      ...
      static const StateConditionInfo portableStructureChaseConditions[] = {
          StateConditionInfo(inWeaponRangeObject,
                             AttackStateMachine::AIM_AT_TARGET, NULL),
          StateConditionInfo(NULL, NULL, NULL)  // keep last
      };
      ...
      */
      defineState(AttackStateMachine::CHASE_TARGET,
                  newInstance(ContinueState)(this), EXIT_MACHINE_WITH_FAILURE,
                  EXIT_MACHINE_WITH_FAILURE, portableStructureChaseConditions);
    } else if (attackingObject) {
      /*
              This state will pursue a target that is moving away from it.  If
         it is not moving away, it will drop into the AIAttackApproachTarget
         state.
      */
      defineState(AttackStateMachine::CHASE_TARGET,
                  newInstance(AIAttackPursueTargetState)(
                      this, follow, attackingObject, forceAttacking),
                  AttackStateMachine::APPROACH_TARGET,
                  AttackStateMachine::APPROACH_TARGET);

      /*
              This state will succeed when we have a useful weapon within range
         of victim, and fail if the victim is dead
      */
      defineState(AttackStateMachine::APPROACH_TARGET,
                  newInstance(AIAttackApproachTargetState)(
                      this, follow, attackingObject, forceAttacking),
                  AttackStateMachine::AIM_AT_TARGET, EXIT_MACHINE_WITH_FAILURE);
    } else {
      /*
              This state will succeed when we have a useful weapon within range
         of victim, and fail if the victim is dead
    ...
    /*
            This state always instantly fails, so when immobile things
       transition here, we bail.
    */
    defineState(AttackStateMachine::CHASE_TARGET,
                newInstance(FailureState)(this), EXIT_MACHINE_WITH_FAILURE,
...
static Object *findEnemyInContainer(Object *killer, Object *bldg) {
  const ContainedItemsList *items =
      bldg->getContain() ? bldg->getContain()->getContainedItemsList() : NULL;
  if (items) {
    for (ContainedItemsList::const_iterator it = items->begin();
      ...
      // order matters: we want to know if I consider it to be an enemy, not
      // vice versa
...
static Int killEnemiesInContainer(Object *killer, Object *bldg, Int maxToKill) {
  ...
  while (numKilled < maxToKill) {
    ...
    if (enemy) {
      ...
      if (killer) killer->scoreTheKill(enemy);
      enemy->kill();
      ++numKilled;
    } else {
      break;
...
//-----------------------------------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------------------------------
AIRappelState::AIRappelState(StateMachine *machine)
    : State(machine, "AIRappelState") {}

//-----------------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIRappelState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AIRappelState::loadPostProcess(void) {}  // end loadPostProcess
//-----------------------------------------------------------------------------------------------------------
StateReturnType AIRappelState::onEnter() {
  Object *obj = getMachineOwner();
  if (!obj->isKindOf(KINDOF_CAN_RAPPEL)) return STATE_FAILURE;
  ...
  if (bldg == NULL || bldg->isEffectivelyDead() ||
      !bldg->isKindOf(KINDOF_STRUCTURE))
  ...
  PathfindLayerEnum layerAtDest =
      TheTerrainLogic->getHighestLayerForDestination(pos, onlyHealthyBridges);
  ...
  AIUpdateInterface *ai = obj->getAI();
  Real MAX_RAPPEL_RATE =
      fabs(TheGlobalData->m_gravity) * LOGICFRAMES_PER_SECOND * 2.5f;
...
StateReturnType AIRappelState::update() {
  ...
  if (!m_targetIsBldg) {
    // if heading for ground, do this every frame... since jitter at the very
    // start can move us slightly, and on uneven ground it might matter.
  ...
  if (pos->z <= m_destZ) {
    ...
    if (m_targetIsBldg) {
      DEBUG_ASSERTCRASH(
          TheActionManager->canEnterObject(
              obj, bldg, obj->getAI()->getLastCommandSource(), COMBATDROP_INTO),
          ("Hmm, this seems unlikely"));
      // if there are enemies... kill up to two. if we kill two, then we die
      // ourselves, otherwise we enter the bldg.
      ...
      if (numKilled == MAX_TO_KILL) {
        ...
        if (bldg->getContain() &&
            bldg->getContain()->isValidContainerFor(obj, TRUE)) {
          bldg->getContain()->addToContain(obj);
        } else {
          // this can legitimately happen if you drop into a full (or nearly
          // full) building. let's just place the guy on the ground nearby,
          // since it sucks to fall from the top of a building.
          Real exitAngle = bldg->getOrientation();
          // Garrison doesn't have reserveDoor or exitDelay, so if we do
          // nothing, everyone will appear on top of each other and get stuck
          // inside each others' extent (except for the first guy).  So we'll
          ...
          startPosition.z = TheTerrainLogic->getGroundHeight(startPosition.x,
                                                             startPosition.y);
          ...
          Coord3D endPosition;
          Bool foundPosition = ThePartitionManager->findPositionAround(
              &startPosition, &options, &endPosition);
...
/*
        ...
        Do NOT subclass this unless you want ALL of the states this machine
   possesses. If you only want SOME of the states, please make a new
   StateMachine, descended from StateMachine, NOT AIStateMachine. Thank you.
   (srj)

        NOTE NOTE NOTE NOTE NOTE

*/
AIStateMachine::AIStateMachine(Object *obj, AsciiString name)
    : StateMachine(obj, name) {
  DEBUG_ASSERTCRASH(getOwner(), ("An AI State Machine '%s' was constructed "
                                 "without an owner, please tell JKMCD",
                                 name));
  DEBUG_ASSERTCRASH(getOwner()->getAI(),
                    ("An AI State Machine '%s' was constructed without an "
                     "AIUpdateInterface, please tell JKMCD",
                     name));
  ...
  // order matters: first state is the default state.
  defineState(AI_IDLE,
              newInstance(AIIdleState)(this, AIIdleState::LOOK_FOR_TARGETS),
              AI_IDLE, AI_IDLE);
  defineState(AI_MOVE_TO, newInstance(AIMoveToState)(this), AI_IDLE, AI_IDLE);
  defineState(AI_MOVE_OUT_OF_THE_WAY, newInstance(AIMoveOutOfTheWayState)(this),
              AI_IDLE, AI_IDLE);
  defineState(AI_MOVE_AND_TIGHTEN, newInstance(AIMoveAndTightenState)(this),
              AI_IDLE, AI_IDLE);
  defineState(AI_MOVE_AWAY_FROM_REPULSORS,
              newInstance(AIMoveAwayFromRepulsorsState)(this),
              AI_WANDER_IN_PLACE, AI_WANDER_IN_PLACE);
  defineState(AI_WANDER_IN_PLACE, newInstance(AIWanderInPlaceState)(this),
              AI_MOVE_AWAY_FROM_REPULSORS, AI_MOVE_AWAY_FROM_REPULSORS);
  defineState(AI_ATTACK_MOVE_TO, newInstance(AIAttackMoveToState)(this),
              AI_IDLE, AI_IDLE);
  defineState(AI_ATTACKFOLLOW_WAYPOINT_PATH_AS_TEAM,
              newInstance(AIAttackFollowWaypointPathState)(this, true), AI_IDLE,
              AI_IDLE);
  defineState(AI_ATTACKFOLLOW_WAYPOINT_PATH_AS_INDIVIDUALS,
              newInstance(AIAttackFollowWaypointPathState)(this, false),
              AI_IDLE, AI_IDLE);

  defineState(AI_FOLLOW_WAYPOINT_PATH_AS_TEAM,
              newInstance(AIFollowWaypointPathState)(this, true), AI_IDLE,
              AI_IDLE);
  defineState(AI_FOLLOW_WAYPOINT_PATH_AS_INDIVIDUALS,
              newInstance(AIFollowWaypointPathState)(this, false), AI_IDLE,
              AI_IDLE);
  defineState(AI_FOLLOW_WAYPOINT_PATH_AS_TEAM_EXACT,
              newInstance(AIFollowWaypointPathExactState)(this, true), AI_IDLE,
              AI_IDLE);
  defineState(AI_FOLLOW_WAYPOINT_PATH_AS_INDIVIDUALS_EXACT,
              newInstance(AIFollowWaypointPathExactState)(this, false), AI_IDLE,
              AI_IDLE);
  defineState(AI_FOLLOW_PATH, newInstance(AIFollowPathState)(this), AI_IDLE,
              AI_IDLE);
  defineState(AI_FOLLOW_EXITPRODUCTION_PATH,
              newInstance(AIFollowPathState)(this), AI_IDLE, AI_IDLE);
  defineState(AI_MOVE_AND_EVACUATE, newInstance(AIMoveAndEvacuateState)(this),
              AI_IDLE, AI_IDLE);
  defineState(AI_MOVE_AND_EVACUATE_AND_EXIT,
              newInstance(AIMoveAndEvacuateState)(this), AI_MOVE_AND_DELETE,
              AI_MOVE_AND_DELETE);
  defineState(AI_MOVE_AND_DELETE, newInstance(AIMoveAndDeleteState)(this),
              AI_IDLE, AI_IDLE);
  defineState(AI_WAIT, newInstance(AIWaitState)(this), AI_IDLE, AI_IDLE);
  defineState(AI_ATTACK_POSITION,
              newInstance(AIAttackState)(this, false, false, false, NULL),
              AI_IDLE, AI_IDLE);
  defineState(AI_ATTACK_OBJECT,
              newInstance(AIAttackState)(this, false, true, false, NULL),
              AI_IDLE, AI_IDLE);
  defineState(AI_FORCE_ATTACK_OBJECT,
              newInstance(AIAttackState)(this, false, true, true, NULL),
              AI_IDLE, AI_IDLE);

  defineState(AI_ATTACK_AND_FOLLOW_OBJECT,
              newInstance(AIAttackState)(this, true, true, false, NULL),
              AI_IDLE, AI_IDLE);
  defineState(AI_ATTACK_SQUAD, newInstance(AIAttackSquadState)(this, NULL),
              AI_IDLE, AI_IDLE);
  defineState(AI_WANDER, newInstance(AIWanderState)(this), AI_IDLE,
              AI_MOVE_AWAY_FROM_REPULSORS);
  defineState(AI_PANIC, newInstance(AIPanicState)(this), AI_IDLE,
              AI_MOVE_AWAY_FROM_REPULSORS);
  ...
  defineState(AI_GUARD_TUNNEL_NETWORK,
              newInstance(AITunnelNetworkGuardState)(this), AI_IDLE, AI_IDLE);
  defineState(AI_HUNT, newInstance(AIHuntState)(this), AI_IDLE, AI_IDLE);
  defineState(AI_ATTACK_AREA, newInstance(AIAttackAreaState)(this), AI_IDLE,
              AI_IDLE);
  defineState(AI_FACE_OBJECT, newInstance(AIFaceState)(this, true), AI_IDLE,
              AI_IDLE);
  defineState(AI_FACE_POSITION, newInstance(AIFaceState)(this, false), AI_IDLE,
              AI_IDLE);
  defineState(AI_PICK_UP_CRATE, newInstance(AIPickUpCrateState)(this), AI_IDLE,
              AI_IDLE);

  defineState(AI_RAPPEL_INTO, newInstance(AIRappelState)(this), AI_IDLE,
              AI_IDLE);
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIStateMachine::crc(Xfer *xfer) { StateMachine::crc(xfer); }  // end crc
...
void AIStateMachine::xfer(Xfer *xfer) {
  ...
  if (m_temporaryState) {
    ...
    DEBUG_ASSERTCRASH(id != INVALID_STATE_ID,
                      ("State has invalid state id, no really. jba."));
  }
  xfer->xferUnsignedInt(&id);
...

{
  ...
  if (getWantsDebugOutput()) {
    ...
    DEBUG_LOG(("%d '%s' -(TEMP)- '%s' %x exit ", TheGameLogic->getFrame(),
               getOwner()->getTemplate()->getName().str(), getName().str(),
               this));
    if (m_temporaryState) {
  ...
  if (newState) {
    ...
    // If you need to up this check, ok, but 1 minute seems overly long for a
    // temporary state override.  jba.
    DEBUG_ASSERTCRASH(frameLimitCoount <= FRAME_COUNT_MAX,
                      ("Unusually long time to set temporary state."));
...
const Coord3D *AIStateMachine::getGoalPathPosition(Int i) const {
  if (i < 0 || i >= m_goalPath.size()) return NULL;
 ...
 */
const Waypoint *AIStateMachine::getGoalWaypoint() { return m_goalWaypoint; }
...
void AIStateMachine::clear() {
  ...
  if (ai) ai->friend_notifyStateMachineChanged();
}
...
StateReturnType AIStateMachine::setState(StateID newStateID) {
  ...
  AIUpdateInterface *ai = getOwner()->getAI();
  if (ai && oldID != newStateID) ai->friend_notifyStateMachineChanged();
...
//----------------------------------------------------------------------------------------------------------
Squad *AIStateMachine::getGoalSquad(void) { return m_goalSquad; }
...
Bool outOfWeaponRangeObject(State *thisState, void *userData) {
  ...
  if (victim && weapon) {
    ...
    if (containedBy && (containedBy->isKindOf(KINDOF_STRUCTURE) ||
                        !containedBy->isAirborneTarget())) {
      // Contained objects on the ground -- garrisoned buildings for example!
      onGround = true;
    }
    // srj sez: at tiny ranges, isAttackViewBlockedByObstacle() can return false
    // positives, so just skip it for contact weapons
    if (victim && !weapon->isContactWeapon() && onGround &&
        !victim->isSignificantlyAboveTerrain()) {
      viewBlocked = TheAI->pathfinder()->isAttackViewBlockedByObstacle(
          obj, *obj->getPosition(), victim, *victim->getPosition());
    }
    // A weapon with leech range temporarily has unlimited range and is locked
    // onto its target.
    if (!weapon->hasLeechRange() && viewBlocked) {
      // CRCDEBUG_LOG(("outOfWeaponRangeObject() - object %d (%s) view is
      // blocked for attacking %d (%s)\n", 	obj->getID(),
      //obj->getTemplate()->getName().str(), 	victim->getID(),
      //victim->getTemplate()->getName().str()));
      return true;
    }
    if (!weapon->hasLeechRange() && !weapon->isWithinAttackRange(obj, victim)) {
      // CRCDEBUG_LOG(("outOfWeaponRangeObject() - object %d (%s) is out of
      // range for attacking %d (%s)\n", 	obj->getID(),
      //obj->getTemplate()->getName().str(), 	victim->getID(),
      //victim->getTemplate()->getName().str()));
      return true;
    }
...
Bool wantToSquishTarget(State *thisState, void *userData) {
  ...
  if (obj && victim) {
    ...
    if (obj->getAI() &&
        (obj->getAI()->getWhichTurretForCurWeapon() != TURRET_INVALID)) {
      // I can only decide to crush-attack if I am attacking with a turreted
      // weapon.
...
static Bool cannotPossiblyAttackObject(State *thisState, void *userData) {
  ...
  if (obj && victim) {
    ...
    CanAttackResult result = obj->getAbleToAttackSpecificObject(
        attackType, victim, obj->getAI()->getLastCommandSource());
...
//----------------------------------------------------------------------------------------------
AIIdleState::AIIdleState(StateMachine *machine,
                         AIIdleState::AIIdleTargetingType shouldLookForTargets)
    : State(machine, "AIIdleState"),
      m_shouldLookForTargets(shouldLookForTargets ==
                             AIIdleState::LOOK_FOR_TARGETS) {
  m_inited = FALSE;
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIIdleState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AIIdleState::loadPostProcess(void) {}  // end loadPostProcess
//----------------------------------------------------------------------------------------------
/**
 * Stake out our space.
 */
...
StateReturnType AIIdleState::onEnter() {
  ...
  // We could possibly not have ai here if we were constructed this frame.
  // Strange but true. :-<
  ...
  m_initialSleepOffset =
      (UnsignedShort)GameLogicRandomValue(0, IDLE_COUNTDOWN_DELAY);
...
void AIIdleState::doInitIdleState() {
  // Only do it once, but do it in the update cause onEnter for idle is called
  // during object creation.  jba.
...
#define NO_STOP_AND_SLIDE
  if (ai->isIdle() && ai->isDoingGroundMovement()) {
    /*
            ...
            You may be asking yourself, "If I'm in an idle state, how can I be
       doing ground movement?"

            answer from jba:

            If a unit is moving, and you hit stop, it forces it into the idle
       state. Depending where it is, it may be between pathfind grids. This is a
       bad thing. So it "cheat moves", to the nearest grid. Also, for locos the
       "close enough" distance is 1 or so. So it moves the rest of the way to
       it's goal location by cheating.
...
StateReturnType AIIdleState::update() {
  ...
  // This state is used internally some places, so we don't necessarily want to
  // be looking for targets Places that use AI_IDLE internally should set this
  // to false in the constructor. jkmcd
  if (m_shouldLookForTargets && !getMachine()->isLocked()) {
    ...
    if (obj->isKindOf(KINDOF_CAN_BE_REPULSED) && ai->isIdle()) {
      ...
      if (enemy) {
        ...
        // since we just changed the state, it doesn't really matter what we
        // return here.
        return STATE_CONTINUE;
      }
    }
    ...
    if (!obj->isDisabledByType(DISABLED_PARALYZED) &&
      ...
      // mood targeting
      UnsignedInt moodAdjust =
          ai->getMoodMatrixActionAdjustment(MM_Action_Idle);
      if ((moodAdjust & MAA_Affect_Range_IgnoreAll) == 0) {
        ...
        if (enemy) {
          ...
          // weird but true. return state_continue, because if we're here, we're
          // actually an attack state since we just changed the state, it
...
StateReturnType AIDeadState::onEnter() {
  ...
  // How can an object be NULL here? I don't think it actually can, but this
  // check must be here for a reason. - jkmcd
  if (obj) {
    ...
    obj->clearAndSetModelConditionFlags(
        nonDyingStuff, MAKE_MODELCONDITION_MASK(MODELCONDITION_DYING));
...
StateReturnType AIDeadState::update() {
  ...
  // re-mark this every time, just in case our health miraculously heals from 0
  // to nonzero...
  ...
  if (obj->isKindOf(KINDOF_INFANTRY)) {
    ...
    if (phys) {
      // we want 'em to stop, but looks wonky if they stop dead in their tracks
      // in onEnter. this slows 'em down quickly
      const Real FACTOR =
          0.8f;  // 0.8 ^ 30 == 0.012, so they slow to 4% of speed over 1 sec
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIInternalMoveToState::crc(Xfer *xfer) {}  // end crc
...
Bool AIInternalMoveToState::getAdjustsDestination() const {
  ...
  if (obj->testStatus(OBJECT_STATUS_PARACHUTING)) return false;

  const AIUpdateInterface *ai = obj->getAI();
  if (ai && !ai->isAllowedToAdjustDestination()) return false;
...
StateReturnType AIInternalMoveToState::onEnter() {
  ...
  if (ai->getCurLocomotor()) {
    ...
    if (ai->getCurLocomotor()->isUltraAccurate()) {
      setAdjustsDestination(false);  // if we're being ultra accurate, we can't
                                     // adjust the destination. jba.
    }
  }
  m_tryOneMoreRepath =
      true;  // We may try one more repath after the first one is finished.
  ai->friend_startingMove();
  ...
  if (physics && physics->isMotive()) {
    ...
    // This is so if you redirect a moving infantry or other animated model, he
    // doesn't pop into idle for 1 frame.  jba.
    // Determine if we are on a cliff cell... if so, use the climbing model
    // condition instead of moving.
    Int cellX = REAL_TO_INT(obj->getPosition()->x / PATHFIND_CELL_SIZE);
    Int cellY = REAL_TO_INT(obj->getPosition()->y / PATHFIND_CELL_SIZE);

    PathfindCell *cell =
        TheAI->pathfinder()->getCell(obj->getLayer(), cellX, cellY);
    ModelConditionFlagType modelConditionFlag =
        (cell && cell->getType() != PathfindCell::CELL_CLIFF)
  ...
  // Since the worker can go into its attack modelcondition in
  // AIAttack::OnEnter(), it needs to get its moving modelcondition set timely
  // enough to trigger a transition from conditionX into ATTACKING|MOVING...
  // Thanks for reading, MLorenzen,  Jan 2, 2003
  ...
  if (getAdjustsDestination()) {
    ...
    TheAI->pathfinder()->updateGoal(
        obj, &m_goalPosition,
        TheTerrainLogic->getLayerForDestination(&m_goalPosition));
  }
...
void AIInternalMoveToState::startMoveSound(void) {
  ...
  if (objBody && IS_CONDITION_WORSE(objBody->getDamageState(), BODY_DAMAGED)) {
    AudioEventRTS soundEventMoveDamaged =
        *obj->getTemplate()->getSoundMoveStartDamaged();
...
void AIInternalMoveToState::onExit(StateExitType status) {
  ...
  // If this onExit is the result of the state machine being deleted, then there
  // is no AI. (This is why destructors should not do game logic)
  if (ai) {
    ai->friend_endingMove();
    DEBUG_ASSERTLOG(obj->getTeam(),
                    ("AIInternalMoveToState::onExit obj has NULL team.\n"));
    if (obj->getTeam() && ai->isDoingGroundMovement() &&
      ...
      if (dx * dx + dy * dy < PATHFIND_CELL_SIZE_F * PATHFIND_CELL_SIZE_F) {
        // We are doing accurate ground movement, so make sure we end exactly at
        // the goal.
...
StateReturnType AIInternalMoveToState::update() {
  ...
  if (m_waitingForPath) {
    ...
    if (ai->isWaitingForPath()) {
      /// @todo srj -- find a way to sleep for a number of frames here, if
      /// possible
      return STATE_CONTINUE;
    }
    if (thePath == NULL) {
    ...
    if (this->getAdjustsDestination()) {
      TheAI->pathfinder()->updateGoal(obj,
                                      thePath->getLastNode()->getPosition(),
                                      thePath->getLastNode()->getLayer());
    } else {
  ...
  // Totally hacky set of conditions to make col. burton's monkey ass not slide
  // down the cliffs backwards.  This could use some improvement at some point.
  // jba. 31DEC2002
  ...
  if (ai->canComputeQuickPath()) {
    if (obj->isKindOf(KINDOF_PROJECTILE)) {
      m_pathTimestamp =
          0;  // We can compute paths cheap, so don't delay to recompute.  jba.
      forceRecompute = true;
    }
  }

  if (thePath != NULL) ai->setLocomotorGoalPositionOnPath();

  // if our goal has moved, recompute our path
  if (forceRecompute ||
      TheGameLogic->getFrame() - m_pathTimestamp > MIN_REPATH_TIME) {
    if (forceRecompute ||
        !isSamePosition(obj->getPosition(), &m_pathGoalPosition,
                        &m_goalPosition)) {
      // goal moved - repath
  ...
  if (ai->getCurLocomotor() &&
      (onPathDistToGoal < ai->getCurLocomotor()->getCloseEnoughDist())) {
    if (ai->isDoingGroundMovement()) {
      ...
      if (delta.length() > 4 * PATHFIND_CELL_SIZE_F) {
        // DEBUG_LOG(("AIInternalMoveToState Trying to finish early.
        // Continuing...\n"));
    ...
    DEBUG_ASSERTLOG(!getMachine()->getWantsDebugOutput(),
                    ("AIInternalMoveToState::update: reached end of path, "
                     "exiting state with success\n"));
...
class AIAttackMoveStateMachine : public StateMachine {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(AIAttackMoveStateMachine,
...
//-----------------------------------------------------------------------------------------------------------
AIAttackMoveStateMachine::AIAttackMoveStateMachine(Object *owner,
                                                   AsciiString name)
    : StateMachine(owner, name) {
  // order matters: first state is the default state.
  defineState(
      AI_IDLE,
      newInstance(AIIdleState)(this, AIIdleState::DO_NOT_LOOK_FOR_TARGETS),
      AI_IDLE, AI_IDLE);
  defineState(AI_PICK_UP_CRATE, newInstance(AIPickUpCrateState)(this), AI_IDLE,
              AI_IDLE);
  defineState(AI_ATTACK_OBJECT,
...
AIMoveToState::AIMoveToState(StateMachine *machine)
    : m_isMoveTo(true), AIInternalMoveToState(machine, "AIMoveToState") {
  // m_isMoveTo is a boolean that specifies that this thing is ACTUALLY A MOVE
  // TO. child classes should set it false. (We don't have or want RTTI, so...)
}
...
StateReturnType AIMoveToState::onEnter() {
  ...
  if (getMachineGoalObject()) {
    ...
    if (getMachineOwner()->isKindOf(KINDOF_PROJECTILE)) {
      Real halfHeight = getMachineGoalObject()
                            ->getGeometryInfo()
...
StateReturnType AIMoveToState::update() {
  ...
  if (m_isMoveTo && (adjustment & MAA_Action_To_AttackMove))
    ai->aiAttackMoveToPosition(&m_goalPosition, NO_MAX_SHOTS_LIMIT,
                               CMD_FROM_AI);
  ...
  if (goalObj) {
    ...
    Bool gotPhysics =
        obj->getPhysics() != NULL && goalObj->getPhysics() != NULL;
    ...
    // DEBUG_LOG(("update goal pos to %f %f
    // %f\n",m_goalPosition.x,m_goalPosition.y,m_goalPosition.z));
  } else {
    Bool isMissile = obj->isKindOf(KINDOF_PROJECTILE);
    if (isMissile) {
      // When missiles are moving uphill, they need to start up quickly to clear
      // hills.  jba.
      m_goalPosition = *getMachineGoalPosition();
      Real zDelta = m_goalPosition.z - obj->getPosition()->z;
      if (zDelta > 0) {
...
StateReturnType AIMoveAwayFromRepulsorsState::onEnter() {
  ...
  Object *obj = getMachineOwner();
  Object *enemy =
      TheAI->findClosestRepulsor(getMachineOwner(), obj->getVisionRange());
  AIUpdateInterface *ai = getMachineOwner()->getAI();
  if (!enemy || !ai) {
    return STATE_FAILURE;
  }
...
static Bool canPursue(Object *source, Weapon *weapon, Object *victim) {
  /* This state is only used if the target is moving away from us, and has
   * physics. */
...
Bool AIAttackApproachTargetState::computePath() {
  ...
  if (getMachineOwner()->isMobile() == false) return false;

  // CRCDEBUG_LOG(("AIAttackApproachTargetState::computePath - begin for object
  ...
  if (ai->isBlockedAndStuck()) {
    forceRepath = true;
    // Intense logging. jba
    // CRCDEBUG_LOG(("AIAttackApproachTargetState::computePath - stuck,
    // recomputing for object %d\n", getMachineOwner()->getID()));
  }
  ...
  /// @todo Unify recomputation conditions & account for obj ID so everyone
  /// doesnt compute on the same frame (MSB)
  if (!forceRepath &&
      TheGameLogic->getFrame() - m_approachTimestamp < MIN_RECOMPUTE_TIME) {
    // CRCDEBUG_LOG(("AIAttackApproachTargetState::computePath - bailing because
    // of min time for object %d\n", getMachineOwner()->getID()));
    return true;
  }
  ...
  if (getMachineGoalObject()) {
    ...
    if (!forceRepath && isSamePosition(source->getPosition(), &m_prevVictimPos,
      ...
      CRCDEBUG_LOG(
          ("AIAttackApproachTargetState::computePath - bailing because victim "
           "in same place for object %d\n",
           getMachineOwner()->getID()));
      return true;
    }
    ...
    if (!weapon) {
      CRCDEBUG_LOG(
          ("AIAttackApproachTargetState::computePath - bailing because of no "
           "weapon for object %d\n",
           getMachineOwner()->getID()));
      return false;
    }
    ...
    if (weapon->isContactWeapon()) {
      // Weapon is basically a contact weapon, so let the attacker pathfind into
      // the target.
      ai->ignoreObstacle(victim);
      setAdjustsDestination(false);  // We want to run into the target.
      ai->setPathExtraDistance(
          10 * PATHFIND_CELL_SIZE_F);  // We don't want it to slow down.
    }
    ...
    if (ai->isBlockedAndStuck()) {
      ...
      CRCDEBUG_LOG(
          ("AIAttackApproachTargetState::computePath - requestPath() for "
           "object %d\n",
           getMachineOwner()->getID()));
      ai->requestPath(&m_goalPosition, getAdjustsDestination());
    } else {
      ...
      CRCDEBUG_LOG(
          ("AIAttackApproachTargetState::computePath - requestAttackPath() for "
           "object %d\n",
           getMachineOwner()->getID()));
      ai->requestAttackPath(victim->getID(), &pos);
      m_stopIfInRange =
          false;  // we have calculated a position to shoot from, so go there.
    }
    CRCDEBUG_LOG(
        ("AIAttackApproachTargetState::computePath - bailing after repathing "
         "for object %d\n",
         getMachineOwner()->getID()));
    return true;
  } else {
    // goal position.
    setAdjustsDestination(true);
    m_stopIfInRange = false;  // Attack position is used by missiles, and they
                              // hit the position.
    m_goalPosition = *getMachineGoalPosition();
    if (!forceRepath) {
      CRCDEBUG_LOG(
          ("AIAttackApproachTargetState::computePath - bailing because we're "
           "aiming for a fixed position for object %d\n",
           getMachineOwner()->getID()));
    ...
    CRCDEBUG_LOG(
        ("AIAttackApproachTargetState::computePath - bailing after repathing "
         "at a fixed position for object %d\n",
         getMachineOwner()->getID()));
    return true;
  }

  CRCDEBUG_LOG(
      ("AIAttackApproachTargetState::computePath - bailing at end of function "
       "for object %d\n",
       getMachineOwner()->getID()));
  return true;
}
...
StateReturnType AIAttackApproachTargetState::onEnter() {
  ...
  if (source->isKindOf(KINDOF_PROJECTILE)) {
    if (ai->getCurLocomotor()) ai->getCurLocomotor()->setUsePreciseZPos(true);
  }
  ...
  if (victim) {
    ...
    if (weapon->isWithinAttackRange(source, victim)) {
      Bool viewBlocked = false;
      if (source && victim && ai->isDoingGroundMovement() &&
          !victim->isSignificantlyAboveTerrain()) {
        viewBlocked = TheAI->pathfinder()->isAttackViewBlockedByObstacle(
            source, *source->getPosition(), victim, *victim->getPosition());
      }
      if (!viewBlocked) {
        return STATE_SUCCESS;
      }
    ...
    // Check here:  If we are a player, and we got to this state via an ai
    // command (ie we auto-acquired), we don't want to chase the unit.
    // isAllowedToChase is set when we are in a deploy and attack state (troop
    // crawler).
  ...
  // CRCDEBUG_LOG(("AIAttackApproachTargetState::onEnter() - calling
  // computePath() for object %d\n", getMachineOwner()->getID()));
  if (computePath() == false) return STATE_FAILURE;
...
StateReturnType AIAttackApproachTargetState::updateInternal() {
  AIUpdateInterface *ai = getMachineOwner()->getAI();
  // CRCDEBUG_LOG(("AIAttackApproachTargetState::updateInternal() - object
  // %d\n", getMachineOwner()->getID()));
  if (getMachine()->isGoalObjectDestroyed()) {
  ...
  Object *victim = getMachineGoalObject();
  if (victim) {
    if (victim->testStatus(OBJECT_STATUS_STEALTHED) &&
    ...
    if (weapon && weapon->isContactWeapon() &&
        weapon->isWithinAttackRange(source, victim)) {
      return STATE_SUCCESS;
    }
    if (m_stopIfInRange && weapon &&
        weapon->isWithinAttackRange(source, victim)) {
      Bool viewBlocked = false;
      if (victim && ai->isDoingGroundMovement() &&
          !victim->isSignificantlyAboveTerrain()) {
        viewBlocked = TheAI->pathfinder()->isAttackViewBlockedByObstacle(
            source, *source->getPosition(), victim, *victim->getPosition());
      }
    ...
    // find a good spot to shoot from
    // CRCDEBUG_LOG(("AIAttackApproachTargetState::updateInternal() - calling
    // computePath() to victim for object %d\n", getMachineOwner()->getID()));
    if (computePath() == false) return STATE_SUCCESS;
    code = AIInternalMoveToState::update();

    if (code != STATE_CONTINUE) {
      return STATE_SUCCESS;  // Always return state success, as state failure
                             // exits the attack.
    ...
    // find a good spot to shoot from
    // CRCDEBUG_LOG(("AIAttackApproachTargetState::updateInternal() - calling
    // computePath() to position for object %d\n", getMachineOwner()->getID()));
    if (m_stopIfInRange && weapon &&
        weapon->isWithinAttackRange(source, &m_goalPosition)) {
      Bool viewBlocked = false;
      if (ai->isDoingGroundMovement()) {
        viewBlocked = TheAI->pathfinder()->isAttackViewBlockedByObstacle(
            source, *source->getPosition(), NULL, m_goalPosition);
      }
      if (!viewBlocked) {
        return STATE_SUCCESS;
      }
...
StateReturnType AIAttackApproachTargetState::update() {
  ...
  if (m_follow && m_isAttackingObject) {
    ...
    Object *victim = getMachineGoalObject();
    if (source && victim && source->isMobile() &&
        !victim->getTemplate()->isKindOf(KINDOF_IMMOBILE)) {
      if (code != STATE_CONTINUE) {
...
void AIAttackApproachTargetState::onExit(StateExitType status) {
  ...
  if (ai) {
    ...
    // Per JohnA, this state should not be calling ai->destroyPath, because we
    // can have spastic users that click the target repeadedly. This will
    // prevent the unit from stuttering for said spastic users.
    // ai->destroyPath();
  ...
  m_isInitialApproach = false;  // We only want to allow turreted things to fire
                                // at enemies during their first approach
}
...
Bool AIAttackPursueTargetState::computePath() {
  ...
  DEBUG_ASSERTLOG(
      getMachineGoalObject(),
      ("***************************Should only be pursuing objects.  jba"));
  // if we have a goal object, move to it, otherwise fail & continue to
...
StateReturnType AIAttackPursueTargetState::onEnter() {
  // contained by AIAttackState, so no separate timer
  // If we return STATE_SUCCESS or STATE_FAILURE, we proceed to
  // AIAttackApproachTargetState.
  Object *source = getMachineOwner();
  AIUpdateInterface *ai = source->getAI();

  if (source->isKindOf(KINDOF_PROJECTILE)) {
    // CRCDEBUG_LOG(("AIAttackPursueTargetState::onEnter() - is a projectile for
    // object %d (%s)\n", getMachineOwner()->getID(),
    // getMachineOwner()->getTemplate()->getName().str()));
    return STATE_SUCCESS;  // Projectiles go directly to
                           // AIAttackApproachTargetState.
  }

  if (getMachine()->isGoalObjectDestroyed()) {
    // CRCDEBUG_LOG(("AIAttackPursueTargetState::onEnter() - goal object is
    // destroyed for object %d (%s)\n", getMachineOwner()->getID(),
    // getMachineOwner()->getTemplate()->getName().str()));
    return STATE_SUCCESS;  // Already killed victim.
  }
  if (!m_isAttackingObject) {
    // CRCDEBUG_LOG(("AIAttackPursueTargetState::onEnter() - not attacking for
    // object %d (%s)\n", getMachineOwner()->getID(),
    // getMachineOwner()->getTemplate()->getName().str()));
  ...
  setAdjustsDestination(false);
  // Check here:  If we are a player, and we got to this state via an ai command
  // (ie we auto-acquired), we don't want to chase the unit.
  if (source->getControllingPlayer()->getPlayerType() == PLAYER_HUMAN) {
    if (ai->getLastCommandSource() == CMD_FROM_AI) {
      return STATE_SUCCESS;
    }
  ...
  if (victim) {
    ...
    if (!canPursue(source, weapon, victim)) {
      // CRCDEBUG_LOG(("AIAttackPursueTargetState::onEnter() - can't pursue for
      // object %d (%s)\n", getMachineOwner()->getID(),
      // getMachineOwner()->getTemplate()->getName().str()));
      return STATE_SUCCESS;
    }
  } else {
    // CRCDEBUG_LOG(("AIAttackPursueTargetState::onEnter() - no victim for
    // object %d (%s)\n", getMachineOwner()->getID(),
    // getMachineOwner()->getTemplate()->getName().str()));
  ...
  if (tur != TURRET_INVALID) {
    ...
    // CRCDEBUG_LOG(("AIAttackPursueTargetState::onEnter() - no turret for
    // object %d (%s)\n", getMachineOwner()->getID(),
    // getMachineOwner()->getTemplate()->getName().str()));
    return STATE_SUCCESS;  // we only pursue with turrets, as non-turreted
                           // weapons can't fire on the run.
  }
...
StateReturnType AIAttackPursueTargetState::updateInternal() {
  ...
  if (victim) {
    ...
    code = AIInternalMoveToState::update();
    if (code != STATE_CONTINUE) {
      // CRCDEBUG_LOG(("AIAttackPursueTargetState::updateInternal() - failed
      // internal update() for object %d (%s)\n", getMachineOwner()->getID(),
      // getMachineOwner()->getTemplate()->getName().str()));
      return STATE_SUCCESS;  // Always return state success, as state failure
                             // exits the attack.
      // we may need to aim & do another approach if the target moved.  jba.
    ...
    WhichTurretType tur = ai->getWhichTurretForCurWeapon();
    if (tur == TURRET_INVALID) {
      // CRCDEBUG_LOG(("AIAttackPursueTargetState::updateInternal() - no turret
      // for object %d (%s)\n", getMachineOwner()->getID(),
      // getMachineOwner()->getTemplate()->getName().str()));
    ...
    if (!viewBlocked && victim->getPhysics() &&
        weapon->isWithinAttackRange(source, victim)) {
      // If we have a turret, start aiming.
      ai->setTurretTargetObject(tur, victim, m_isForceAttacking);
      //  match speeds;
      m_isInitialApproach = false;
      Real victimSpeed = victim->getPhysics()->getForwardSpeed2D();
      if (weapon->isGoalPosWithinAttackRange(source, source->getPosition(),
      ...
      // source->getPhysics()->getForwardSpeed2D(), victimSpeed));
    } else {
      ai->setDesiredSpeed(FAST_AS_POSSIBLE);
    }
  }
...
StateReturnType AIFollowPathState::onEnter() {
  ...
  if (pos == NULL) return STATE_FAILURE;
  ...
  // Assign this value to the AIUpdateInterface so object's can access this
  // value while determine which waypoints to plot in the waypoint renderer.
...
void AIFollowPathState::onExit(StateExitType status) {
  ...
  if (!ai) return;
  ai->setCanPathThroughUnits(false);
  if (ai->getCurLocomotor()) ai->getCurLocomotor()->setUsePreciseZPos(false);
...
StateReturnType AIFollowPathState::update() {
  ...
  if (status == STATE_SUCCESS || status == STATE_FAILURE) {
    ...
    // value while determine which waypoints to plot in the waypoint renderer.
    ai->friend_setCurrentGoalPathIndex(m_index);
    ai->ignoreObstacleID(INVALID_ID);  // we have exited whatever object we are
                                       // leaving, if any.  jba.
    if (pos == NULL) {
    ...
    if (nextPos) {
      ...
      if (followingPos) offset += 4 * PATHFIND_CELL_SIZE_F;
      ai->setPathExtraDistance(offset);
      // We are in the middle of a path, so don't set the final goal location
      // yet.
      setAdjustsDestination(false);
    } else {
      setAdjustsDestination(m_adjustFinal && (m_adjustFinalOverride ||
                                              ai->isDoingGroundMovement()));
      if (getAdjustsDestination()) {
...
StateReturnType AIMoveAndEvacuateState::onEnter() {
  ...
  getMachine()->lock("AIMoveAndEvacuateState::onEnter");  // This state is not
                                                          // user interruptable.
...
void AIMoveAndEvacuateState::onExit(StateExitType status) {
  ...
  getMachine()->setGoalPosition(
      &m_origin);  // In case we follow with a AIMoveAndDeleteState.
  AIInternalMoveToState::onExit(status);
}
...
//-------------------------------------------------------------------------------------------------
AIAttackMoveToState::AIAttackMoveToState(StateMachine *machine)
    : AIMoveToState(machine) {
#ifdef STATE_MACHINE_DEBUG
  ...
  m_attackMoveMachine = newInstance(AIAttackMoveStateMachine)(
      getMachineOwner(), "AIAttackMoveMachine");
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIAttackMoveToState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AIAttackMoveToState::loadPostProcess(void) {}  // end loadPostProcess
...
AsciiString AIAttackMoveToState::getName() const {
  ...
  if (m_attackMoveMachine)
    name.concat(m_attackMoveMachine->getCurrentStateName());
  else
    name.concat("*NULL m_deployMachine");
  return name;
}
#endif
...
StateReturnType AIAttackMoveToState::update() {
  ...
  if (m_attackMoveMachine->isInIdleState()) {
    ...
    if (nextObjectToAttack != NULL) {
      ...
      shouldRepathThisFrame =
          false;  // we're about to drop out of this function, but this is
                  // semantic emphasis.
  ...
  if (ret != STATE_CONTINUE) {
    ...
    Real distSqr = sqr(owner->getPosition()->x - m_pathGoalPosition.x) +
                   sqr(owner->getPosition()->y - m_pathGoalPosition.y);
...
StateReturnType AIMoveAndDeleteState::update() {
  ...
  if (m_appendGoalPosition) {
    ...
    if (!ai->isWaitingForPath() && ai->getPath()) {
      m_goalPosition.z =
          TheTerrainLogic->getGroundHeight(m_goalPosition.x, m_goalPosition.y);
...
const Waypoint *AIFollowWaypointPathState::getNextWaypoint(void) {
  ...
  getMachine()->setGoalPosition(
      m_currentWaypoint->getLocation());  // THANKS, JOHN
  return nextWay;
#else
  if (!hasNextWaypoint()) {
    m_priorWaypoint = m_currentWaypoint;
...
#define NO_ROTATE_OFFSETS
  ...
  if (!extent.isInRegionNoZ(&m_goalPosition)) {
    ...
    ai->getCurLocomotor()->setAllowInvalidPosition(
        true);                    // allow it to move off the map.
...
StateReturnType AIFollowWaypointPathState::onEnter() {
  ...
  /*	Interesting thought experiment.  Didn't work well. jba
          Real distSqrLimit =
     9*obj->getGeometryInfo().getMajorRadius()*obj->getGeometryInfo().getMajorRadius();
          const Waypoint *way = m_currentWaypoint;
          Bool doPrecise = false;
          while (way) {
                  if (way->getNext()) {
                          Real dx = way->getLocation()->x -
     way->getNext()->getLocation()->x; Real dy = way->getLocation()->y -
     way->getNext()->getLocation()->y; Real distSqr = dx*dx + dy*dy; if (distSqr
     < distSqrLimit) { doPrecise = true;
                          }
                  }
                  way = way->getNext();
          }
...
StateReturnType AIFollowWaypointPathState::update() {
  ...
  if (m_isFollowWaypointPathState && (adjustment & MAA_Action_To_AttackMove)) {
    if (m_moveAsGroup) {
      ai->aiAttackFollowWaypointPathAsTeam(m_currentWaypoint,
                                           NO_MAX_SHOTS_LIMIT, CMD_FROM_AI);
    } else {
      ai->aiAttackFollowWaypointPath(m_currentWaypoint, NO_MAX_SHOTS_LIMIT,
                                     CMD_FROM_AI);
    }
  }
  ...
  if (m_appendGoalPosition) {
    ...
    if (!ai->isWaitingForPath() && ai->getPath()) {
      ...
      thePath->appendNode(&m_goalPosition,
                          LAYER_GROUND);  // waypoints are always on the ground.
      m_appendGoalPosition = false;       // just did it.
    }
  }
  if (m_moveAsGroup &&
      ...
      if (!TheAI->pathfinder()->adjustDestination(obj, ai->getLocomotorSet(),
        ...
        if (m_currentWaypoint) {
          DEBUG_LOG(("Breaking out of follow waypoint path %s of %s\n",
                     m_currentWaypoint->getName().str(),
                     m_currentWaypoint->getPathLabel1().str()));
        }
        return STATE_FAILURE;
      }
    }
  ...
  // We may want to allow ourselves to bail out of this one early. In order to
  // do this, we check and see if we're moving as a group, and then if our team
  // is owned by an AI Skirmish player. If it is, then we compute the group
  // centroid, and see if it is within some distance of the
  if (m_moveAsGroup) {
    if (obj->getControllingPlayer()->isSkirmishAIPlayer()) {
      ...
      if (pos.length() <=
          (numInGroup * TheAI->getAiData()->m_skirmishGroupFudgeValue)) {
  ...
  if (status != STATE_CONTINUE) {
    ...
    Object *obj = getMachineOwner();
    AIUpdateInterface *ai = obj->getAI();
    if (m_priorWaypoint) ai->setPriorWaypointID(m_priorWaypoint->getID());
    if (m_currentWaypoint) ai->setCurrentWaypointID(m_currentWaypoint->getID());
    // LORENZEN ADDED LORENZEN ADDED LORENZEN ADDED
...
StateReturnType AIFollowWaypointPathExactState::onEnter() {
  const Waypoint *currentWaypoint =
      ((AIStateMachine *)getMachine())->getGoalWaypoint();
  AIUpdateInterface *ai = getMachineOwner()->getAI();
  ...
  //			Artist couldn't load his map, and turned out that it was
  //because 			there was a waypoint path that pointed to itself (2 point path).
  //			John said that this code only needs "a" point, not the
  //"last" point. while (m_lastWaypoint && m_lastWaypoint->getLink(0)) {
...
void AIFollowWaypointPathExactState::onExit(StateExitType status) {
  ...
  if (ai) {
    ...
    ai->setCanPathThroughUnits(false);
    ai->getCurLocomotor()->setAllowInvalidPosition(
        false);  // turn off allow it to move off the map.
  }
}
...
AIAttackFollowWaypointPathState::AIAttackFollowWaypointPathState(
    StateMachine *machine, Bool asGroup)
    : AIFollowWaypointPathState(machine, asGroup, false) {
#ifdef STATE_MACHINE_DEBUG
  setName("AIAttackFollowWaypointPathState");
#endif
  m_attackFollowMachine = newInstance(AIAttackMoveStateMachine)(
      getMachineOwner(), "AIAttackFollowMachine");
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIAttackFollowWaypointPathState::crc(Xfer *xfer) {}  // end crc
...
AsciiString AIAttackFollowWaypointPathState::getName() const {
  ...
  if (m_attackFollowMachine)
    name.concat(m_attackFollowMachine->getCurrentStateName());
  else
    name.concat("*NULL m_attackFollowMachine");
  return name;
}
#endif
...
StateReturnType AIAttackFollowWaypointPathState::update() {
  ...
  if (m_attackFollowMachine->isInIdleState()) {
    ...
    {
      nextObjectToAttack =
          ai->getNextMoodTarget(!forceRetargetThisFrame, false);
    }
    if (nextObjectToAttack != NULL) {
...
StateReturnType AIWanderState::onEnter() {
  ...
  AIUpdateInterface *ai = getMachineOwner()->getAI();
  m_priorWaypoint = NULL;
  if (m_currentWaypoint == NULL || ai == NULL) return STATE_FAILURE;
  m_groupOffset.x = m_groupOffset.y = 0;
  ...
  if (curLoco && curLoco->getWanderWidthFactor() > 0.0f) {
    ...
    if (delta < 1) delta = 1;
    m_groupOffset.x =
        GameLogicRandomValue(-delta, delta) * PATHFIND_CELL_SIZE_F;
    m_groupOffset.y =
        GameLogicRandomValue(-delta, delta) * PATHFIND_CELL_SIZE_F;
  }
  m_timer = 0;
...
StateReturnType AIWanderInPlaceState::onEnter() {
  ...
  if (ai->getCurLocomotor()) {
    delta =
        REAL_TO_INT_FLOOR((ai->getCurLocomotor()->getWanderAboutPointRadius() /
...
StateReturnType AIWanderInPlaceState::update() {
  ...
  if (status != STATE_CONTINUE) {
    Int delta = 3;
    if (ai->getCurLocomotor()) {
      delta = REAL_TO_INT_FLOOR(
          (ai->getCurLocomotor()->getWanderAboutPointRadius() /
           PATHFIND_CELL_SIZE_F) +
          0.5f);
    }
    Coord3D offset;
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIAttackAimAtTargetState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AIAttackAimAtTargetState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType AIAttackAimAtTargetState::onEnter() {
  ...
  // Ick.  This was originally a safety to a single line that required victim,
  // and was never meant as an early return to all cases.  We now want to use
  // preattack frames on ground position targets
  ...
  if (containedBy && weapon) {
    ...
    if (victim) {
      inFiringRange =
          contain->attemptBestFirePointPosition(source, weapon, victim);
    } else {
      inFiringRange =
          contain->attemptBestFirePointPosition(source, weapon, targetPos);
    }
  } else if (victim && weapon)
    inFiringRange = weapon->isWithinAttackRange(source, victim);
  else if (weapon)
    inFiringRange = weapon->isWithinAttackRange(
        source,
        targetPos);  // See, I can be attacking the ground.  Perfectly valid.
  else
    return STATE_FAILURE;  // can't happen.

  // add ourself as a targeter BEFORE calling
  // isTemporarilyPreventingAimSuccess().
  if (victimAI) victimAI->addTargeter(source->getID(), true);
  ...
  if (tur != TURRET_INVALID) {
    ...
    // GS moved contact weapon check in here, because Success can never be given
    // to a unit in this state using a turret to attack.  Check out ::update and
    // you will see.
...
StateReturnType AIAttackAimAtTargetState::update() {
  ...
  if (!source->hasAnyWeapon()) return STATE_FAILURE;
  ...
  if (tur != TURRET_INVALID) {
    ...
    if (sourceAI->getTurretTurnRate(tur) != 0.0f) {
      // The Body can never return Success if the weapon is on the turret, or
      // else we end up shooting the current weapon (which is on the turret) in
      // the wrong direction. We always say Continue, so the Turret can do its
      // own Aiming state.
      //			if (m_isAttackingObject &&
      //source->canCrushOrSquish(victim)) { 				return STATE_SUCCESS;
      //			}
      return STATE_CONTINUE;
    }
  ...
  // no else here!
  {
    Real relAngle =
        m_isAttackingObject
            ...
            : ThePartitionManager->getRelativeAngle2D(source,
                                                      getMachineGoalPosition());

    const Real REL_THRESH =
        0.035f;  // about 2 degrees. (getRelativeAngle2D is current only
                 // accurate to about 1.25 degrees)
    ...
    // DEBUG_LOG(("AIM: desired %f, actual %f, delta %f, aimDelta %f, goalpos %f
    // %f\n",rad2deg(obj->getOrientation() +
    // relAngle),rad2deg(obj->getOrientation()),rad2deg(relAngle),rad2deg(aimDelta),victim->getPosition()->x,victim->getPosition()->y));
    if (m_canTurnInPlace) {
      ...
      sourceAI->setLocomotorGoalPositionExplicit(
          m_isAttackingObject ? *victim->getPosition()
                              : *getMachineGoalPosition());
    }
    ...
    if (fabs(relAngle) < aimDelta /*&& !m_preAttackFrames*/) {
      AIUpdateInterface *victimAI = victim ? victim->getAI() : NULL;
      // add ourself as a targeter BEFORE calling
      // isTemporarilyPreventingAimSuccess(). we do this every time thru, just
      // in case we get into a squabble with our turret-ai over whether or not
      // we are a targeter... (srj)
      if (victimAI) victimAI->addTargeter(source->getID(), true);
      Bool preventing =
          victimAI && victimAI->isTemporarilyPreventingAimSuccess();
      if (preventing) {
        return STATE_CONTINUE;
      } else {
        return STATE_SUCCESS;
  ...
  if (source->isDisabledByType(DISABLED_HELD)) {
    ...
    Weapon *weapon = source->getCurrentWeapon();
    const Coord3D *pos =
        m_isAttackingObject ? victim->getPosition() : getMachineGoalPosition();
...
void AIAttackAimAtTargetState::onExit(StateExitType status) {
  ...
  if (m_canTurnInPlace) {
    ...
    if (sourceAI && m_setLocomotor) sourceAI->setLocomotorGoalNone();
  } else {
    // don't do the loco call, or else we will "wiggle"... we already have an
    // appropriate goal
  }
...
StateReturnType AIAttackFireWeaponState::onEnter() {
  ...
  // Passive stuff will approach but not attack, so we check here (after
  ...
  if (victim &&
      obj->getTeam()->getPrototype()->getTemplateInfo()->m_attackCommonTarget) {
...
StateReturnType AIAttackFireWeaponState::update() {
  ...
  if (m_att->isAttackingObject()) {
    ...
    if (!victim || victim->isEffectivelyDead()) return STATE_FAILURE;
  }
  ...
  /**
          this is the weird case where we have multi turrets, and turret 'a'
     wants to fire, but someone has changed the current weapon to be one not on
     him. rather than addressing the situation, we just punt and wait for it to
     clear up on its own.
  */
  ...
  // must adjust the state BEFORE calling fireWeapon, for FX to work
  // correctly...
    ...
    if (continueRange > 0.0f && victim &&
        (victim->isDestroyed() || victim->isEffectivelyDead() ||
         (victim->isKindOf(KINDOF_MINE) &&
          victim->testStatus(OBJECT_STATUS_MASKED)))) {
      const Coord3D *originalVictimPos =
          m_att ? m_att->getOriginalVictimPos() : NULL;
      if (originalVictimPos) {
        // note that it is important to use getLastCommandSource here; this
        // allows dozers that were ordered to clear mines by the human to
        // continue to autoacquire, but not if they were ordered by ai.
        AIUpdateInterface *ai = obj->getAI();
        CommandSourceType lastCmdSource =
            ai ? ai->getLastCommandSource() : CMD_FROM_AI;
        PartitionFilterSamePlayer filterPlayer(victim->getControllingPlayer());
        PartitionFilterSameMapStatus filterMapStatus(obj);
        PartitionFilterPossibleToAttack filterAttack(ATTACK_NEW_TARGET, obj,
                                                     lastCmdSource);
        PartitionFilter *filters[] = {&filterAttack, &filterPlayer,
                                      &filterMapStatus, NULL};
        // note that we look around originalVictimPos, *not* the current
        // victim's pos.
        victim = ThePartitionManager->getClosestObject(
            originalVictimPos, continueRange, FROM_CENTER_2D,
            filters);  // could be null. this is ok.
        if (victim) {
...
//----------------------------------------------------------------------------------------------------------
AIAttackState::AIAttackState(StateMachine *machine, Bool follow,
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIAttackState::crc(Xfer *xfer) {}  // end crc
...
void AIAttackState::xfer(Xfer *xfer) {
  ...
  if (hasMachine && m_attackMachine == NULL) {
    ...
    m_attackMachine = newInstance(AttackStateMachine)(
        getMachineOwner(), this, "AIAttackMachine", m_follow,
        m_isAttackingObject, m_isForceAttacking);
  }
  if (hasMachine) {
    xfer->xferSnapshot(
        m_attackMachine);  ///< state sub-machine for attack behavior
  }
  /* Not saved or loaded - passed in on creation.
  Bool m_follow; Bool
  m_isAttackingObject;
  // if false, attacking position AttackExitConditionsInterface*
  m_attackParameters;					///< these are not owned
  by this, and will not be deleted on destruction Bool
  m_isForceAttacking
  */
}  // end xfer
...
void AIAttackState::loadPostProcess(void) {
  ...
  m_lockedWeaponOnEnter =
      source->isCurWeaponLocked() ? source->getCurrentWeapon() : NULL;
}  // end loadPostProcess
...
AsciiString AIAttackState::getName() const {
  ...
  name.concat("/");
  if (m_attackMachine)
    name.concat(m_attackMachine->getCurrentStateName());
  else
    name.concat("*NULL m_attackMachine");
  return name;
}
#endif
...
void AIAttackState::chooseWeapon() {
  Object *victim = getMachineGoalObject();
  if (m_isAttackingObject && !victim) return;

  Object *source = getMachineOwner();
  AIUpdateInterface *ai = source->getAI();

  if (victim) {
    /*bool found =*/source->chooseBestWeaponForTarget(
        victim, PREFER_MOST_DAMAGE, ai->getLastCommandSource());
    // DEBUG_ASSERTLOG(found, ("unable to autochoose any weapon for
...
void AIAttackState::notifyNewVictimChosen(Object *victim) {
  // do NOT update m_originalVictimPos here. It's a new victim, not the
  // original!
  getMachine()->setGoalObject(victim);
  if (m_attackMachine) m_attackMachine->setGoalObject(victim);
}

//----------------------------------------------------------------------------------------------------------
/**
 * Begin an attack on the machine's goal object.
 * To do this complex behavior, instantiate another state machine as a
 * "sub-machine" of the attack state.
 */
...
StateReturnType AIAttackState::onEnter() {
  ...
  if ((ai->getMoodMatrixActionAdjustment(MM_Action_Attack) & MAA_Action_Ok) ==
      0)
    return STATE_SUCCESS;

  // if we've met the conditions specified by m_attackParameters, we consider
  ...
  // create new state machine for attack behavior
  // CRCDEBUG_LOG(("AIAttackState::onEnter() - constructing state machine for
  // object %d\n", getMachineOwner()->getID()));
  m_attackMachine =
      newInstance(AttackStateMachine)(source, this, "AIAttackMachine", m_follow,
                                      m_isAttackingObject, m_isForceAttacking);
  ...
  if (curWeapon) {
    ...
    // icky special case for ignoring stealth units we might be targeting, that
    // are currently stealthed. (srj)
...
StateReturnType AIAttackState::update() {
  ...
  if (m_isAttackingObject) {
    ...
    if (victim->getTeam() != m_victimTeam) {
      // If, while I have been attacking my victim, it has lost its ability to
      // attack
      //(a recently de-garrisoned building) I should bail here...
      // We are not sure whether the problem occurs here or sometime before, but
      // this is an edge case failsafe for it Steven calls this hack 'greasy,'
      // and I agreesy.-Lorenzen
      AIUpdateInterface *ai = source->getAI();
      if (ai) {
        if ((victim->getStatusBits() & OBJECT_STATUS_CAN_ATTACK) == 0) {
          if (victim->getContain() != NULL) {
            if (victim->getContain()->isGarrisonable() &&
                (victim->getContain()->getContainCount() == 0)) {
              if (source->getRelationship(victim) == NEUTRAL) {
                ...
                ai->notifyVictimIsDead();  // well, not "dead", but longer
                                           // attackable
                return STATE_FAILURE;
              }
            }
      ...
      // The team of the victim has changed since we began attacking it.
      // Evaluate whether or not we should keep attacking it.
      //  order matters: we want to know if I consider it to be an enemy, not
      //  vice versa
    ...
    if (victim != m_attackMachine->getGoalObject()) {
      // Our parent machine has changed the goal.  We need to reset to the new
      // goal.  jba.
      m_attackMachine->setGoalObject(victim);
    }
  }
...
class AIAttackThenIdleStateMachine : public StateMachine {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(AIAttackThenIdleStateMachine,
...
//-----------------------------------------------------------------------------------------------------------
AIAttackThenIdleStateMachine::AIAttackThenIdleStateMachine(Object *owner,
                                                           AsciiString name)
    : StateMachine(owner, name) {
  // order matters: first state is the default state.
  defineState(AI_ATTACK_OBJECT,
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIAttackSquadState::crc(Xfer *xfer) {}  // end crc
...
void AIAttackSquadState::xfer(Xfer *xfer) {
  ...
  if (hasMachine && m_attackSquadMachine == NULL) {
    // create new state machine for attack behavior
    m_attackSquadMachine = newInstance(AIAttackThenIdleStateMachine)(
        getMachineOwner(), "AIAttackMachine");
  }
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AIAttackSquadState::loadPostProcess(void) {}  // end loadPostProcess
...
AsciiString AIAttackSquadState::getName() const {
  ...
  name.concat("/");
  if (m_attackSquadMachine)
    name.concat(m_attackSquadMachine->getCurrentStateName());
  else
    name.concat("*NULL m_attackSquadMachine");
  return name;
}
#endif
...
StateReturnType AIAttackSquadState::update(void) {
  ...
  /*
          ...
          sleeps, we still need to be called every frame.
  */
  StateReturnType attackStatus =
      CONVERT_SLEEP_TO_CONTINUE(m_attackSquadMachine->updateStateMachine());
...
Object *AIAttackSquadState::chooseVictim(void) {
  ...
  GameDifficulty difficulty =
      owner->getControllingPlayer()->getPlayerDifficulty();
  const CommandSourceType cmdSource = ai->getLastCommandSource();

  if (cmdSource == CMD_FROM_PLAYER) {
    // if a player did this, we want to always give them the Seek and obliterate
    // method.
  ...
  switch (difficulty) {
    ...
    case DIFFICULTY_NORMAL: {
      ...
      Object *victim = ThePartitionManager->getClosestObject(
          getMachineOwner(), HUGE_DIST, FROM_CENTER_2D, filters, NULL, NULL);
      return victim;
      break;
    }
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIDockState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AIDockState::loadPostProcess(void) {}  // end loadPostProcess
...
AsciiString AIDockState::getName() const {
  ...
  name.concat("/");
  if (m_dockMachine)
    name.concat(m_dockMachine->getCurrentStateName());
  else
    name.concat("*NULL m_dockMachine");
  return name;
}
#endif
...
StateReturnType AIDockState::onEnter() {
  ...
  // tell the pathfinder to ignore the object we are docking with, so it doesn't
  // block us
  AIUpdateInterface *ai = getMachineOwner()->getAI();
  if (ai) {
...
void AIDockState::onExit(StateExitType status) {
  ...
  if (m_dockMachine) {
    m_dockMachine
        ->halt();  // GS, you have to halt before you delete to do cleanup.
    m_dockMachine->deleteInstance();
    m_dockMachine = NULL;
  } else {
...
StateReturnType AIEnterState::onEnter() {
  ...
  if (goal) {
    if (!TheActionManager->canEnterObject(
            obj, goal, obj->getAI()->getLastCommandSource(), CHECK_CAPACITY))
      return STATE_FAILURE;
...
StateReturnType AIEnterState::update() {
  ...
  Object *goal = getMachineGoalObject();
  if (goal) {
    // if our goal is contained by something else, give up. this is for the
    // following bug:
    ...
    if (goal->getContainedBy() != NULL && goal->isAboveTerrain() &&
        !obj->isAboveTerrain()) {
      return STATE_FAILURE;
    }
    ...
    if (!TheActionManager->canEnterObject(
            obj, goal, obj->getAI()->getLastCommandSource(), CHECK_CAPACITY)) {
      /*
              special-case: if it's an enemy, try attacking it instead. this is
         to address this bug: (srj)

              Bug: Units stop instead of attacking if building they were trying
         to garrison is taken by enemy units first.

              1. any game/any faction
              2. build infantry units that can garrison neutral buildings
              3. have infantry  units garrison a neutral building, before they
         garrison the building have some enemy units garrison it first.

              Expected result: Based on test plan: Instead of just stopping when
         enemy units garrison building first, they should continue and attack
         the building.
      */
      if (obj->getRelationship(goal) == ENEMIES && obj->getAI()) {
        CanAttackResult result = TheActionManager->getCanAttackObject(
            obj, goal, obj->getAI()->getLastCommandSource(), ATTACK_NEW_TARGET);
        if (result == ATTACKRESULT_POSSIBLE ||
            result == ATTACKRESULT_POSSIBLE_AFTER_MOVING) {
          obj->getAI()->aiAttackObject(goal, NO_MAX_SHOTS_LIMIT,
                                       obj->getAI()->getLastCommandSource());
          // weird but true. return state_continue, because if we're here, we're
          // actually an attack state since we just changed the state, it
          // doesn't really matter what we return here.
          return STATE_CONTINUE;
  ...
  if (code == STATE_SUCCESS && goal->isAboveTerrain() &&
      !obj->isAboveTerrain()) {
    code = STATE_CONTINUE;
  }
  ...
  if (code == STATE_SUCCESS) {
    ...
    // srj sez: I don't think we want to restrict this to HELD items. See the
    // intro of GLA02.map for an example of guys-off-the-border-but-not-held who
    // need this check.
    // if( obj->isDisabledByType( DISABLED_HELD ) )
    {
      if (goal) {
        ...
        if (closeEnough) {
          ...
          // This case is primarily to handle transports on the map border for
          // scripted setup. The partition manager doesn't generate collisions
          // in the border area, so we have to add ourselves.  jba.
          ContainModuleInterface *contain = goal->getContain();
          if (contain) {
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIExitState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AIExitState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType AIExitState::update() {
  ...
  if (goal) {
    ...
    // GS.  The goal of unified ExitInterfaces dies a horrible death.  I can't
    // ask Object for the exit,
    //  as removeFromContain is only in the Contain type.  I'm spliting the
    //  names in shame.
    ExitInterface *goalExitInterface =
        goal->getContain() ? goal->getContain()->getContainExitInterface()
                           : NULL;
    if (goalExitInterface == NULL) return STATE_FAILURE;
    ...
    ExitDoorType exitDoor =
        goalExitInterface
            ? goalExitInterface->reserveDoorForExit(obj->getTemplate(), obj)
            : DOOR_NONE_NEEDED;
    if (exitDoor == DOOR_NONE_AVAILABLE) return STATE_FAILURE;

    goalExitInterface->exitObjectViaDoor(obj, exitDoor);
    if (getMachine()->getCurrentStateID() != getID())
      return STATE_CONTINUE;  // Not sucess, because exitViaDoor has changed us
                              // to FollowPath, and if we say Success, our
                              // machine will think FollowPath succeeded
    else
      return STATE_SUCCESS;
  } else {
    return STATE_FAILURE;
...
AsciiString AIGuardState::getName() const {
  ...
  name.concat("/");
  if (m_guardMachine)
    name.concat(m_guardMachine->getCurrentStateName());
  else
    name.concat("*NULL guardMachine");
  return name;
}
#endif
// ------------------------------------------------------------------------------------------------
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIGuardState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AIGuardState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType AIGuardState::onEnter() {
  ...
  switch (ai->getGuardTargetType()) {
    ...
    case GUARDTARGET_OBJECT:
      m_guardMachine->setTargetToGuard(
          TheGameLogic->findObjectByID(ai->getGuardObject()));
      break;
  ...
  // now that essential parameters are set, set the machine's initial state
  if (m_guardMachine->initDefaultState() == STATE_FAILURE) return STATE_FAILURE;
...
StateReturnType AIGuardState::update() {
  // DEBUG_LOG(("AIGuardState frame %d:
  ...
  if (owner->isOutOfAmmo() && !owner->isKindOf(KINDOF_PROJECTILE)) {
    DEBUG_CRASH(
        ("Hmm, this should probably never happen, since this case should be "
         "intercepted by JetAIUpdate\n"));
    return STATE_FAILURE;
  }

  getMachine()->lock("AIGuardState::update");  // We don't want to switch out of
                                               // guard during the update.
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AITunnelNetworkGuardState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AITunnelNetworkGuardState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType AITunnelNetworkGuardState::update() {
  // DEBUG_LOG(("AITunnelNetworkGuardState frame %d:
  // %08lx\n",TheGameLogic->getFrame(),getMachineOwner()));
  ...
  getMachine()->lock(
      "AITunnelNetworkGuardState::update");  // We don't want to switch out of
                                             // guard during the update.
  StateReturnType ret = m_guardMachine->updateStateMachine();
  getMachine()->unlock();
  return ret;
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIHuntState::crc(Xfer *xfer) {}  // end crc
...
void AIHuntState::xfer(Xfer *xfer) {
  ...
  if (hasMachine && m_huntMachine == NULL) {
    ...
    m_huntMachine = newInstance(AIAttackThenIdleStateMachine)(
        getMachineOwner(), "AIAttackThenIdleStateMachine");
  }
  if (hasMachine) {
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AIHuntState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType AIHuntState::onEnter() {
  ...
  // first time thru, use a random amount so that everyone doesn't scan on the
  // same frame, to avoid "spikes".
...
AsciiString AIHuntState::getName() const {
  ...
  name.concat("/");
  if (m_huntMachine)
    name.concat(m_huntMachine->getCurrentStateName());
  else
    name.concat("*NULL huntMachine");
  return name;
}
#endif
...
StateReturnType AIHuntState::update() {
  ...
  if (now >= m_nextEnemyScanTime) {
    ...
    if (teamVictim && info == NULL) {
      ...
      if (victim == NULL && owner->getControllingPlayer() &&
          owner->getControllingPlayer()->getUnitsShouldHunt()) {
        // If we are doing an all hunt, try hunting without the attack priority
        // info. jba.
    ...
    if (owner->getControllingPlayer() &&
        owner->getControllingPlayer()->getUnitsShouldHunt() == FALSE) {
      // If we are not doing an all hunt, then exit hunt state - no more
      // victims.
  ...
  getMachine()->lock(
      "AIHuntState::update");  // The idle state in the sub machine can
                               // sometimes acquire targets. It is important to
                               // not switch out of this state via a sub machine
                               // call. jba.
  /*
          Note the use of CONVERT_SLEEP_TO_CONTINUE; even if the sub-machine
          sleeps, we still need to be called every frame.
  */
  /// @todo srj -- find a way to sleep for a number of frames here, if possible
  StateReturnType ret =
      CONVERT_SLEEP_TO_CONTINUE(m_huntMachine->updateStateMachine());
  getMachine()->unlock();
  return ret;
}
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIAttackAreaState::crc(Xfer *xfer) {}  // end crc
...
void AIAttackAreaState::xfer(Xfer *xfer) {
  ...
  if (hasMachine && m_attackMachine == NULL) {
    // create new state machine for hunt behavior
    m_attackMachine = newInstance(AIAttackThenIdleStateMachine)(
        getMachineOwner(), "AIAttackThenIdleStateMachine");
  }
  if (hasMachine) {
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AIAttackAreaState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType AIAttackAreaState::update() {
  ...
  if (now >= m_nextEnemyScanTime) {
    ...
    // first time thru, add a random amount so that everyone doesn't scan on the
    // same frame, to avoid "spikes". Note that this implementation ensures that
    // this unit checks immediately upon entering this state, then wait a
    // possibly-longer-than-usual time (due to randomness), then settle into a
    // regular schedule.
    m_nextEnemyScanTime = now + ENEMY_SCAN_RATE;

    AIUpdateInterface *ai = owner->getAI();
    if (ai->getAreaToGuard() == NULL) return STATE_FAILURE;
    ...
    // do NOT do line of sight check - we want to find everything
    Object *victim = TheAI->findClosestEnemy(owner, 9999.9f, AI::CAN_ATTACK,
                                             info, &polyFilter);
    m_attackMachine->setGoalObject(victim);
  ...
  getMachine()->lock(
      "AIAttackAreaState::update");  // The idle state in the sub machine can
                                     // sometimes acquire targets. It is
                                     // important to not switch out of this
                                     // state via a sub machine call. jba.
  /*
          Note the use of CONVERT_SLEEP_TO_CONTINUE; even if the sub-machine
          sleeps, we still need to be called every frame.
  */
  /// @todo srj -- find a way to sleep for a number of frames here, if possible
  StateReturnType ret =
      CONVERT_SLEEP_TO_CONTINUE(m_attackMachine->updateStateMachine());
  getMachine()->unlock();
  return ret;
}
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIFaceState::crc(Xfer *xfer) {}  // end crc
...
StateReturnType AIFaceState::update() {
  ...
  const Real REL_THRESH =
      0.035f;  // about 2 degrees. (getRelativeAngle2D is current only accurate
