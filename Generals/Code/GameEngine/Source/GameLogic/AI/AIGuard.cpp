...
static Bool hasAttackedMeAndICanReturnFire(State *thisState,
                                           void * /*userData*/) {
  ...
  // K. It appears we have a valid aggressor. Find it, and determine if we can
  // attack it, etc.
  Object *target =
      TheGameLogic->findObjectByID(bmi->getClearableLastAttacker());
  bmi->clearLastAttacker();

  // We use the clearable last attacker because we should continue attacking the
  // guy. But if he stops attacking us, then we want our timer to kick us off of
  // him and make us go attack other units instead.
  ...
  // This is a quick test on the target. It will be duplicated in
  // getAbleToAttackSpecificObject, but the payoff is worth the duplication.
  if (target->isEffectivelyDead()) {
    return FALSE;
  }

  //@todo: Get this out of here. Move it into the declaration of calling this
  //function, or figure
...
Bool ExitConditions::shouldExit(const StateMachine *machine) const {
  ...
  if (m_conditionsToConsider & ATTACK_ExitIfOutsideRadius) {
    ...
    deltaAggressor.z =
        0;  // BGC - when we search for a target we don't account for Z, so why
            // should we here? changing this fixed a crash where a
            // GLARebelInfantry would be in GuardReturnState, find a target that
            // is within range, then not be able to attack because its actually
            // out of range. then it would look for a new target, get the same
            // one, and proceed in an infinite recursive loop that eventually
            // blew the stack.
...
AIGuardMachine::AIGuardMachine(Object *owner)
  ...
  static const StateConditionInfo attackAggressors[] = {
      StateConditionInfo(hasAttackedMeAndICanReturnFire,
                         AI_GUARD_ATTACK_AGGRESSOR, NULL),
      StateConditionInfo(NULL, NULL, NULL)  // keep last
  };

  // order matters: first state is the default state.
  // srj sez: I made "return" the start state, so that if ordered to guard a
  // position that isn't the unit's current position, it moves to that position
  // first.
  defineState(AI_GUARD_RETURN, newInstance(AIGuardReturnState)(this),
              AI_GUARD_IDLE, AI_GUARD_INNER, attackAggressors);
  defineState(AI_GUARD_IDLE, newInstance(AIGuardIdleState)(this),
              AI_GUARD_INNER, AI_GUARD_RETURN, attackAggressors);
  defineState(AI_GUARD_INNER, newInstance(AIGuardInnerState)(this),
              AI_GUARD_OUTER, AI_GUARD_OUTER);
  defineState(AI_GUARD_OUTER, newInstance(AIGuardOuterState)(this),
              AI_GUARD_GET_CRATE, AI_GUARD_GET_CRATE);
  defineState(AI_GUARD_GET_CRATE, newInstance(AIGuardPickUpCrateState)(this),
              AI_GUARD_RETURN, AI_GUARD_RETURN);
  defineState(AI_GUARD_ATTACK_AGGRESSOR,
              newInstance(AIGuardAttackAggressorState)(this), AI_GUARD_RETURN,
              AI_GUARD_RETURN);
}
...
/*static*/ Real AIGuardMachine::getStdGuardRange(const Object *obj) {
  Real visionRange = TheAI->getAdjustedVisionRangeForObject(
      obj, AI_VISIONFACTOR_OWNERTYPE | AI_VISIONFACTOR_MOOD |
               AI_VISIONFACTOR_GUARDINNER);
...
Bool AIGuardMachine::lookForInnerTarget(void) {
  ...
  Coord3D pos =
      targetToGuard ? *targetToGuard->getPosition() : *getPositionToGuard();

  const PolygonTrigger *area = getAreaToGuard();
  PartitionFilterRelationship f1(owner,
  ...
  //filters, ITER_SORTED_NEAR_TO_FAR); 	MemoryPoolObjectHolder hold(iter);
  //	Object* target = iter->first();
  //
  // srj sez: the above code is stupid and slow. since we only want the closest
  // object, just ask for that; the above has to find ALL objects in range, but
  // we ignore all but the first (closest).
  //
  Object *target = ThePartitionManager->getClosestObject(
      &pos, visionRange, FROM_CENTER_2D, filters);
  if (target) {
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIGuardMachine::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AIGuardMachine::loadPostProcess(void) {}  // end loadPostProcess
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIGuardInnerState::crc(Xfer *xfer) {}  // end crc
...
StateReturnType AIGuardInnerState::onEnter(void) {
  ...
  Coord3D pos = targetToGuard ? *targetToGuard->getPosition()
                              : *getGuardMachine()->getPositionToGuard();
  Object *nemesis =
      TheGameLogic->findObjectByID(getGuardMachine()->getNemesisID());
  if (nemesis == NULL) {
    DEBUG_LOG(("Unexpected NULL nemesis in AIGuardInnerState.\n"));
    return STATE_SUCCESS;
  }
  m_exitConditions.m_center = pos;
  m_exitConditions.m_radiusSqr =
      sqr(AIGuardMachine::getStdGuardRange(getMachineOwner()));
  m_exitConditions.m_conditionsToConsider =
      (ExitConditions::ATTACK_ExitIfOutsideRadius |
       ExitConditions::ATTACK_ExitIfNoUnitFound);

  m_attackState = newInstance(AIAttackState)(getMachine(), false, true, false,
                                             &m_exitConditions);
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIGuardOuterState::crc(Xfer *xfer) {}  // end crc
...
StateReturnType AIGuardOuterState::onEnter(void) {
  ...
  Real range = TheAI->getAdjustedVisionRangeForObject(
      obj, AI_VISIONFACTOR_OWNERTYPE | AI_VISIONFACTOR_MOOD);

  const PolygonTrigger *area = getGuardMachine()->getAreaToGuard();
  if (area) {
    if (range < area->getRadius()) range = area->getRadius();
    area->getCenterPoint(&pos);
  }
  m_exitConditions.m_center = pos;
  ...
  m_exitConditions.m_conditionsToConsider =
      (ExitConditions::ATTACK_ExitIfExpiredDuration |
       ExitConditions::ATTACK_ExitIfOutsideRadius |
       ExitConditions::ATTACK_ExitIfNoUnitFound);
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIGuardReturnState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AIGuardReturnState::loadPostProcess(void) {}  // end loadPostProcess

//--------------------------------------------------------------------------------------
StateReturnType AIGuardReturnState::onEnter(void) {
  UnsignedInt now = TheGameLogic->getFrame();
  m_nextReturnScanTime =
      now +
      GameLogicRandomValue(0, TheAI->getAiData()->m_guardEnemyReturnScanRate);

  // no, no, no, don't do this in onEnter, unless you like really slow maps.
  // (srj)
  //	if (getGuardMachine()->lookForInnerTarget())
  //		return STATE_FAILURE; // early termination because we found a
  //target.

  Object *targetToGuard = getGuardMachine()->findTargetToGuardByID();
  m_goalPosition = targetToGuard ? *targetToGuard->getPosition()
                                 : *getGuardMachine()->getPositionToGuard();
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIGuardIdleState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AIGuardIdleState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType AIGuardIdleState::onEnter(void) {
  ...
  UnsignedInt now = TheGameLogic->getFrame();
  m_nextEnemyScanTime =
      now + GameLogicRandomValue(0, TheAI->getAiData()->m_guardEnemyScanRate);
...
StateReturnType AIGuardIdleState::update(void) {
  // DEBUG_LOG(("AIGuardIdleState frame %d:
  // %08lx\n",TheGameLogic->getFrame(),getMachineOwner()));

  UnsignedInt now = TheGameLogic->getFrame();
  if (now < m_nextEnemyScanTime) return STATE_SLEEP(m_nextEnemyScanTime - now);
...
//----------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
AIGuardPickUpCrateState::AIGuardPickUpCrateState(StateMachine *machine)
    : AIPickUpCrateState(machine) {
#ifdef STATE_MACHINE_DEBUG
...
StateReturnType AIGuardAttackAggressorState::onEnter(void) {
  ...
  if (obj->getBodyModule() &&
      obj->getBodyModule()->getLastDamageInfo()->in.m_sourceID) {
  ...
  m_attackState = newInstance(AIAttackState)(getMachine(), true, true, false,
                                             &m_exitConditions);
  m_attackState->getMachine()->setGoalObject(nemesis);
...
//-------------------------------------------------------------------------------------------------
void AIGuardAttackAggressorState::loadPostProcess() { onEnter(); }
