...
static Object *findBestTunnel(Player *ownerPlayer, const Coord3D *pos) {
  ...
  for (std::list<ObjectID>::const_iterator iter = allTunnels->begin();
    ...
    // For each ID, look it up and change its team.  We all get captured
    // together.
 ...
 */
Bool TunnelNetworkExitConditions::shouldExit(
...
AITNGuardMachine::AITNGuardMachine(Object *owner)
  ...
  static const StateConditionInfo attackAggressors[] = {
      StateConditionInfo(hasAttackedMeAndICanReturnFire,
                         AI_TN_GUARD_ATTACK_AGGRESSOR, NULL),
  ...
  // first.
  defineState(AI_TN_GUARD_RETURN, newInstance(AITNGuardReturnState)(this),
              AI_TN_GUARD_IDLE, AI_TN_GUARD_INNER, attackAggressors);
  defineState(AI_TN_GUARD_IDLE, newInstance(AITNGuardIdleState)(this),
              AI_TN_GUARD_INNER, AI_TN_GUARD_RETURN);
  defineState(AI_TN_GUARD_INNER, newInstance(AITNGuardInnerState)(this),
              AI_TN_GUARD_OUTER, AI_TN_GUARD_OUTER, attackAggressors);
  defineState(AI_TN_GUARD_OUTER, newInstance(AITNGuardOuterState)(this),
              AI_TN_GUARD_GET_CRATE, AI_TN_GUARD_GET_CRATE);
  defineState(AI_TN_GUARD_GET_CRATE,
              newInstance(AITNGuardPickUpCrateState)(this), AI_TN_GUARD_RETURN,
              AI_TN_GUARD_RETURN);
  defineState(AI_TN_GUARD_ATTACK_AGGRESSOR,
              newInstance(AITNGuardAttackAggressorState)(this),
              AI_TN_GUARD_RETURN, AI_TN_GUARD_RETURN);
          ...
          if (body->getLastDamageTimestamp() +
            ...
            if (attacker) {
              ...
              CanAttackResult result =
                  getOwner()->getAbleToAttackSpecificObject(
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AITNGuardMachine::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AITNGuardMachine::loadPostProcess(void) {}  // end loadPostProcess
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AITNGuardInnerState::crc(Xfer *xfer) {}  // end crc
...
static Object *TunnelNetworkScan(Object *owner) {
  ...
  Object *target = ThePartitionManager->getClosestObject(
      owner->getPosition(), visionRange, FROM_CENTER_2D, filters);
  return target;
}
  ...
  if (nemesis == NULL) {
    ...
    if (tunnels) {
      ...
      if (nemesis) {
        ...
        m_exitConditions.m_attackGiveUpFrame =
            TheGameLogic->getFrame() +
            TheAI->getAiData()->m_guardChaseUnitFrames;
        return STATE_CONTINUE;
      }
    }
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AITNGuardOuterState::crc(Xfer *xfer) {}  // end crc
...
StateReturnType AITNGuardReturnState::onEnter(void) {
  ...
  // Scan my tunnels.
  Object *bestTunnel = findBestTunnel(getMachineOwner()->getControllingPlayer(),
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AITNGuardIdleState::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AITNGuardIdleState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType AITNGuardIdleState::update(void) {
  // DEBUG_LOG(("AITNGuardIdleState frame %d:
  ...
  if (getGuardMachine()->lookForInnerTarget()) {
    ...
    if (nemesis == NULL) {
      DEBUG_LOG(
          ("Unexpected NULL nemesis in AITNGuardAttackAggressorState.\n"));
      return STATE_SLEEP(0);
    }
    if (getMachineOwner()->getContainedBy()) {
      Object *bestTunnel =
          findBestTunnel(owner->getControllingPlayer(), nemesis->getPosition());
      ExitInterface *goalExitInterface =
          bestTunnel->getContain()
              ? bestTunnel->getContain()->getContainExitInterface()
              : NULL;
...
//----------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
AITNGuardPickUpCrateState::AITNGuardPickUpCrateState(StateMachine *machine)
...
StateReturnType AITNGuardAttackAggressorState::update(void) {
  if (m_attackState->getMachine()->getCurrentStateID() ==
...
//-------------------------------------------------------------------------------------------------
void AITNGuardAttackAggressorState::loadPostProcess() { onEnter(); }
