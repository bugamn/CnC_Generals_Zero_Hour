...
void TAiData::addFactionBuildList(AISideBuildList *buildList) {
  ...
  while (info) {
    if (buildList->m_side == info->m_side) {
      if (info->m_buildList) info->m_buildList->deleteInstance();
...
AISideBuildList::AISideBuildList(AsciiString side)
    : m_side(side), m_buildList(NULL), m_next(NULL) {}
...
static const FieldParse TheAIFieldParseTable[] = {
    ...
    {"StructureSeconds", INI::parseReal, NULL,
     offsetof(TAiData, m_structureSeconds)},
    ...
    {"ForceIdleMSEC", INI::parseDurationUnsignedInt, NULL,
     offsetof(TAiData, m_forceIdleFramesCount)},
    {"StructuresWealthyRate", INI::parseReal, NULL,
     offsetof(TAiData, m_structuresWealthyMod)},
    {"TeamsWealthyRate", INI::parseReal, NULL,
     offsetof(TAiData, m_teamWealthyMod)},
    {"StructuresPoorRate", INI::parseReal, NULL,
     offsetof(TAiData, m_structuresPoorMod)},
    {"TeamsPoorRate", INI::parseReal, NULL, offsetof(TAiData, m_teamPoorMod)},
    {"TeamResourcesToStart", INI::parseReal, NULL,
     offsetof(TAiData, m_teamResourcesToBuild)},
    {"GuardInnerModifierAI", INI::parseReal, NULL,
     offsetof(TAiData, m_guardInnerModifierAI)},
    {"GuardOuterModifierAI", INI::parseReal, NULL,
     offsetof(TAiData, m_guardOuterModifierAI)},
    {"GuardInnerModifierHuman", INI::parseReal, NULL,
     offsetof(TAiData, m_guardInnerModifierHuman)},
    {"GuardOuterModifierHuman", INI::parseReal, NULL,
     offsetof(TAiData, m_guardOuterModifierHuman)},
    {"GuardChaseUnitsDuration", INI::parseDurationUnsignedInt, NULL,
     offsetof(TAiData, m_guardChaseUnitFrames)},
    {"GuardEnemyScanRate", INI::parseDurationUnsignedInt, NULL,
     offsetof(TAiData, m_guardEnemyScanRate)},
    {"GuardEnemyReturnScanRate", INI::parseDurationUnsignedInt, NULL,
     offsetof(TAiData, m_guardEnemyReturnScanRate)},
    {"SkirmishGroupFudgeDistance", INI::parseReal, NULL,
     offsetof(TAiData, m_skirmishGroupFudgeValue)},

    {"RepulsedDistance", INI::parseReal, NULL,
     offsetof(TAiData, m_repulsedDistance)},
    {"EnableRepulsors", INI::parseBool, NULL,
     offsetof(TAiData, m_enableRepulsors)},

    {"AlertRangeModifier", INI::parseReal, NULL,
     offsetof(TAiData, m_alertRangeModifier)},
    {"AggressiveRangeModifier", INI::parseReal, NULL,
     offsetof(TAiData, m_aggressiveRangeModifier)},

    {"ForceSkirmishAI", INI::parseBool, NULL,
     offsetof(TAiData, m_forceSkirmishAI)},
    {"RotateSkirmishBases", INI::parseBool, NULL,
     offsetof(TAiData, m_rotateSkirmishBases)},

    {"AttackUsesLineOfSight", INI::parseBool, NULL,
     offsetof(TAiData, m_attackUsesLineOfSight)},
    {"AttackIgnoreInsignificantBuildings", INI::parseBool, NULL,
     offsetof(TAiData, m_attackIgnoreInsignificantBuildings)},

    {"AttackPriorityDistanceModifier", INI::parseReal, NULL,
     offsetof(TAiData, m_attackPriorityDistanceModifier)},
    {"MaxRecruitRadius", INI::parseReal, NULL,
     offsetof(TAiData, m_maxRecruitDistance)},
    ...
    {"MinInfantryForGroup", INI::parseInt, NULL,
     offsetof(TAiData, m_minInfantryForGroup)},
    {"MinVehiclesForGroup", INI::parseInt, NULL,
     offsetof(TAiData, m_minVehiclesForGroup)},

    {"MinDistanceForGroup", INI::parseReal, NULL,
     offsetof(TAiData, m_minDistanceForGroup)},
    {"DistanceRequiresGroup", INI::parseReal, NULL,
     offsetof(TAiData, m_distanceRequiresGroup)},
    {"MinClumpDensity", INI::parseReal, NULL,
     offsetof(TAiData, m_minClumpDensity)},

    {"InfantryPathfindDiameter", INI::parseInt, NULL,
     offsetof(TAiData, m_infantryPathfindDiameter)},
    {"VehiclePathfindDiameter", INI::parseInt, NULL,
     offsetof(TAiData, m_vehiclePathfindDiameter)},
    {"RebuildDelayTimeSeconds", INI::parseInt, NULL,
     offsetof(TAiData, m_rebuildDelaySeconds)},
    {"SupplyCenterSafeRadius", INI::parseReal, NULL,
     offsetof(TAiData, m_supplyCenterSafeRadius)},

    {"AIDozerBoredRadiusModifier", INI::parseReal, NULL,
     offsetof(TAiData, m_aiDozerBoredRadiusModifier)},
    {"AICrushesInfantry", INI::parseBool, NULL,
     offsetof(TAiData, m_aiCrushesInfantry)},

    {NULL, NULL, NULL, 0}  // keep this last

};

void AI::parseSideInfo(INI *ini, void *instance, void * /*store*/,
                       const void * /*userData*/) {
  const char *c = ini->getNextToken();
  AsciiString side(c);

  static const FieldParse myFieldParse[] = {
      {"ResourceGatherersEasy", INI::parseInt, NULL,
       offsetof(AISideInfo, m_easy)},
      {"ResourceGatherersNormal", INI::parseInt, NULL,
       offsetof(AISideInfo, m_normal)},
      {"ResourceGatherersHard", INI::parseInt, NULL,
       offsetof(AISideInfo, m_hard)},
      {"BaseDefenseStructure1", INI::parseAsciiString, NULL,
       offsetof(AISideInfo, m_baseDefenseStructure1)},
...
void AI::parseSkillSet(INI *ini, void *instance, void *store,
                       const void * /*userData*/) {
  static const FieldParse myFieldParse[] = {
      {"Science", AI::parseScience, NULL, NULL}, {NULL, NULL, NULL, 0}
      // keep this last
  };
...
void AI::parseScience(INI *ini, void *instance, void * /*store*/,
                      const void * /*userData*/) {
  TSkillSet *skillset = ((TSkillSet *)instance);
  if (skillset->m_numSkills >= MAX_AI_UPGRADES) {
#ifdef DEBUG_CRASHING
    const char *c = ini->getNextToken();
    DEBUG_CRASH(("Too many SCIENCE skills in skillset. Skill = %s, max is %d",
                 c, MAX_AI_UPGRADES));
#endif
    return;
  }
  skillset->m_skills[skillset->m_numSkills] = SCIENCE_INVALID;
  INI::parseScience(ini, instance, skillset->m_skills + skillset->m_numSkills,
                    NULL);
...
void AI::parseSkirmishBuildList(INI *ini, void *instance, void * /*store*/,
                                const void * /*userData*/) {
  const char *c = ini->getNextToken();
 ...
 */
void AI::init(void) { m_nextGroupID = 0; }
...
void AI::parseAiDataDefinition(INI *ini) {
  if (TheAI) {
    ...
    //
    if (ini->getLoadType() == INI_LOAD_CREATE_OVERRIDES) TheAI->newOverride();
...
void AI::destroyGroup(AIGroup *group) {
  std::list<AIGroup *>::iterator i =
      std::find(m_groupList.begin(), m_groupList.end(), group);

  // make sure group is actually in the list
  if (i == m_groupList.end()) return;

  DEBUG_ASSERTCRASH(
      group != NULL,
      ("A NULL group made its way into the AIGroup list.. jkmcd"));
...
class PartitionFilterLiveMapEnemies : public PartitionFilter {
 ...
 public:
  ...
  virtual Bool allow(Object *objOther) {
    ...
    if (objOther->isEffectivelyDead()) return false;

    // this is also way fast (bit test) so do it next.
    if (objOther->isOffMap() != m_obj->isOffMap()) return false;
...
class PartitionFilterWithinAttackRange : public PartitionFilter {
 ...
 public:
  ...
  virtual const char *debugGetName() {
    return "PartitionFilterWithinAttackRange";
  }
#endif
};
 ...
 */
Object *AI::findClosestEnemy(const Object *me, Real range,
                             UnsignedInt qualifiers,
                             const AttackPriorityInfo *info,
                             PartitionFilter *optionalFilter) {
  ...
  // for enemies here)
  // *** This doesn't cut it anymore. Bombtrucks can be disguised as an enemy
  // member meaning we
  //     still want to acquire it -- however this old filter fails because the
  //     unit is stealthed.
  ...
  PartitionFilterPossibleToAttack filterAttack(ATTACK_NEW_TARGET, me,
                                               CMD_FROM_AI);
  ...
  PartitionFilterFreeOfFog filterFogged(
      me->getControllingPlayer()->getPlayerIndex());

  PartitionFilter *filters[16];
  Int numFilters = 0;

  // Important note: the filters are called in order, and once one rejects an
  // object, the remaining ones are not called, so you should endeavor to
  // arrange them such that the most-coarse (ie, the ones that will usually
  // REJECT the most objects) should come first.
  //
  // srj sez: I actually did profiling on USA04 (enabling FILTER_PROFILING in
  // partition mgr) to determine the order of these. some observations:
  //
  // -- filterTeam is BY FAR the best to put first (since most things near you
  // tend to be nonenemies).
  // -- filterLOS tends to reject a very large number, but is computationally
  // expensive
  // -- filterStealth is BY FAR the least common to be useful, so it goes last.
  // GS Fog check used to be inside can attack, so it feels right to be right
  // after it

  filters[numFilters++] = &filterObvious;

  if (!(qualifiers & ATTACK_BUILDINGS)) filters[numFilters++] = &filterBldgs;

  if (qualifiers & WITHIN_ATTACK_RANGE)
    filters[numFilters++] = &filterWithinAttackRange;

  if (qualifiers & CAN_SEE) filters[numFilters++] = &filterLOS;

  if (qualifiers & CAN_ATTACK) filters[numFilters++] = &filterAttack;

  if (qualifiers & UNFOGGED) filters[numFilters++] = &filterFogged;
  ...
  if (info == NULL || info == TheScriptEngine->getDefaultAttackInfo()) {
    ...
    Object *o = ThePartitionManager->getClosestObject(
        me, range, FROM_BOUNDINGSPHERE_2D, filters);
    return o;
  }
  ...
  ObjectIterator *iter = ThePartitionManager->iterateObjectsInRange(
      me, range, FROM_BOUNDINGSPHERE_2D, filters, ITER_SORTED_NEAR_TO_FAR);
  ...
  for (Object *theEnemy = iter->first(); theEnemy; theEnemy = iter->next()) {
    ...
    if (curPriority == 0) continue;  // don't attack 0 priority targets.

    /* check for garrisoned buildings/vehicles & see if a higher priority unit
     * is inside. */
    ...
    if (modPriority < 1) modPriority = 1;
 ...
 */
Object *AI::findClosestAlly(const Object *me, Real range,
                            UnsignedInt qualifiers) {
  // never target buildings (unless they can attack)
  PartitionFilterRejectBuildings filterBldgs(me);
  ...
  PartitionFilterRelationship filterTeam(
      me, PartitionFilterRelationship::ALLOW_ALLIES);
  ...
  return ThePartitionManager->getClosestObject(me, range,
...
Real AI::getAdjustedVisionRangeForObject(const Object *object,
  ...
  if (!ai) {
    DEBUG_CRASH(
        ("Unit without AI ('%s') calling AI::getAdjustedVisionRangeForObject. "
         "Notify jkmcd.",
  ...
  if (object->getContainedBy() != NULL) {
    ...
    if ((factorsToConsider & AI_VISIONFACTOR_MOOD) &&
        ((moodMatrixVal & MM_Controller_Player) == 0)) {
  ...
  if (TheGlobalData->m_debugVisibility) {
    // ICK. This really nasty statement is used so that we only initialize this
    // color once. It should be exactly double the intensity of its targettable
    // brother.
    ...
    for (int i = 0; i < TheGlobalData->m_debugVisibilityTileCount; ++i) {
      pos.Rotate_Z(1.0f * i / TheGlobalData->m_debugVisibilityTileCount * 2 *
                   PI);
      Coord3D coord = {pos.X + object->getPosition()->x,
...
//-----------------------------------------------------------------------------
void TAiData::loadPostProcess(void) {}  // end loadPostProcess
...
void AI::crc(Xfer *xfer) {
  ...
  for (std::list<AIGroup *>::iterator groupIt = m_groupList.begin();
...
//-----------------------------------------------------------------------------
void AI::loadPostProcess(void) {}  // end loadPostProcess
