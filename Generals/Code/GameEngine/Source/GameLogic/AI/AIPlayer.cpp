...
AIPlayer::AIPlayer(Player *p)
    : m_player(p),
      ...
      m_structureTimer(
          2),  // Important - don't start building structures until frame 1.
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
AIPlayer::~AIPlayer() { clearTeamsInQueue(); }
...
void AIPlayer::onStructureProduced(Object *factory, Object *bldg) {
  m_teamDelay =
      0;  // Cause the update queues & selection to happen immediately.
...
// ------------------------------------------------------------------------------------------------
/** See if the building is a supply center, and see how many supply trucks we
 * want. */
// ------------------------------------------------------------------------------------------------
void AIPlayer::checkForSupplyCenter(BuildListInfo *info, Object *bldg) {
  ...
  SupplyCenterDockUpdate *centerModule =
      (SupplyCenterDockUpdate *)bldg->findUpdateModule(key_centerUpdate);
  if (centerModule) {
    ...
    info->setDesiredGatherers(desiredGatherers +
                              1);  // get a freebie with the supply depots.
  }
}
...
void AIPlayer::queueSupplyTruck(void) {
  ...
  for (DLINK_ITERATOR<TeamInQueue> iter = iterate_TeamBuildQueue();
    ...
    for (order = team->m_workOrders; order; order = order->m_next) {
      // GLA dozers (workers) are also resource gatherers, so make sure it isn't
      // a worker. jba.
    ...
    for (DLINK_ITERATOR<Team> iter = (*it)->iterate_TeamInstanceList();
      ...
      for (DLINK_ITERATOR<Object> objIter = team->iterate_TeamMemberList();
        ...
        SupplyTruckAIInterface *supplyTruckAI =
            obj->getAI()->getSupplyTruckAIInterface();
        if (supplyTruckAI) {
          totalHarvesters++;
        }
      }
  ...
  for (BuildListInfo *info = m_player->getBuildList(); info;
    ...
    if (curGatherers >= desiredGatherers) {
      ...
      if (supplyCenter) {
        ...
        Object *supplySource = ThePartitionManager->getClosestObject(
        ...
        if (warehouseModule) {
          Int availableCash = warehouseModule->getBoxesStored() *
                              TheGlobalData->m_baseValuePerSupplyBox;
        ...
        for (it = m_player->getPlayerTeams()->begin();
          ...
          for (DLINK_ITERATOR<Team> iter = (*it)->iterate_TeamInstanceList();
            ...
            for (DLINK_ITERATOR<Object> objIter =
      ...
      /* See if we have any "loose" harvesters (cause my supply center got
       * nuked.) */
      Player::PlayerTeamList::const_iterator it;
      for (it = m_player->getPlayerTeams()->begin();
           it != m_player->getPlayerTeams()->end(); ++it) {
          ...
          for (DLINK_ITERATOR<Object> objIter = team->iterate_TeamMemberList();
            ...
            if (supplyTruckAI) {
              ...
              if (supplyTruckAI->isCurrentlyFerryingSupplies() ||
                ...
                // This thinks he is a gatherer, but doesn't have a preferred
                // dock id.
                Object *center =
        ...
        if (isSupplyTruck) {
          ...
          if (factory) {
            ...
            if (info->getCurrentGatherers() == -1) {
              ...
              startTraining(order, team->m_priorityBuild,
                            team->m_team->getName());
            }
            break;
          }
...
Object *AIPlayer::buildStructureNow(const ThingTemplate *bldgPlan,
  ...
  Object *bldg = TheBuildAssistant->buildObjectNow(
      NULL, bldgPlan, info->getLocation(), info->getAngle(), m_player);
  ...
  if (bldg) {
    ...
    info->setObjectID(bldg->getID());
    info->setObjectTimestamp(TheGameLogic->getFrame() +
                             1);  // has to be non-zero, so just add 1.
    ...
    if (exitInterface) {
      ...
      if (fabs(info->getRallyOffset()->x) > 1.0f ||
          fabs(info->getRallyOffset()->y) > 1.0f) {
        gotOffset;
      }
...
Object *AIPlayer::buildStructureWithDozer(const ThingTemplate *bldgPlan,
  ...
  if (TheBuildAssistant->isLocationLegalToBuild(
          &pos, bldgPlan, angle,
          BuildAssistant::CLEAR_PATH | BuildAssistant::TERRAIN_RESTRICTIONS |
    ...
    for (posOffset = 0; posOffset < limit;
      ...
      Real xPos, yPos;
      yPos = pos.y - offset;
      for (xPos = pos.x - offset; xPos <= pos.x + offset;
      ...
      if (valid) break;
      xPos = pos.x - offset;
      for (yPos = pos.y - offset; yPos <= pos.y + offset;
  ...
  TheTerrainVisual->removeAllBibs();  // isLocationLegalToBuild adds bib
                                      // feedback, turn it off.  jba.
  if (!TheAI->pathfinder()->quickDoesPathExist(
  ...
  if (TheGlobalData->m_debugAI == AI_DEBUG_PATHS) {
    ...
    color.green = 1;
    for (i = 1; i < count; i++) {
      myPos.x = dozer->getPosition()->x +
                (pos.x - dozer->getPosition()->x) * i / count;
      myPos.y = dozer->getPosition()->y +
                (pos.y - dozer->getPosition()->y) * i / count;
      myPos.z = TheTerrainLogic->getGroundHeight(myPos.x, myPos.y) + 0.5f;
  ...
  if (m_readyToBuildStructure) {
    for (BuildListInfo *info = m_player->getBuildList(); info;
      ...
      if (info->getObjectID() == INVALID_ID && info->getObjectTimestamp() > 0) {
        // this object was built at some time, and got destroyed at or near
        // objectTimestamp. Wait a few seconds before initiating a rebuild.
        if (info->getObjectTimestamp() +
                TheAI->getAiData()->m_rebuildDelaySeconds *
                    LOGICFRAMES_PER_SECOND >
            TheGameLogic->getFrame()) {
          continue;
        } else {
          DEBUG_LOG(
              ("Enabling rebuild for %s\n", info->getTemplateName().str()));
      ...
      if (info->isBuildable()) {
        ...
        if (bldg == NULL) {
          ...
          if (bldg) {
            ...
            } else if (m_player->getMoney()->countMoney() >
                       TheAI->getAiData()->m_resourcesWealthy) {
              m_structureTimer =
                  m_structureTimer / TheAI->getAiData()->m_structuresWealthyMod;
            }
            m_frameLastBuildingBuilt = TheGameLogic->getFrame();
          ...
          if (TheGameLogic->getFrame() - m_frameLastBuildingBuilt <
              framesToBuild) {
            m_buildDelay = framesToBuild - (TheGameLogic->getFrame() -
            ...
            if (m_player->getMoney()->countMoney() >= cost) {
              ...
              if (bldg) {
                ...
                m_readyToBuildStructure = false;
                m_structureTimer = TheAI->getAiData()->m_structureSeconds *
                                   LOGICFRAMES_PER_SECOND;
                if (m_player->getMoney()->countMoney() <
                    TheAI->getAiData()->m_resourcesPoor) {
                  m_structureTimer = m_structureTimer /
...
void AIPlayer::aiPreTeamDestroy(const Team *deletedTeam) {
  {
    for (DLINK_ITERATOR<TeamInQueue> iter = iterate_TeamBuildQueue();
      ...
      if (team->m_team == deletedTeam) {
        // The members of the team all got killed before we could finish
        // building the team.
  ...
  {
    for (DLINK_ITERATOR<TeamInQueue> iter = iterate_TeamReadyQueue();
         !iter.done(); iter.advance()) {
      TeamInQueue *team = iter.cur();
      if (team->m_team == deletedTeam) {
        // The members of the team all got killed before we could activate the
        // team.
  ...
  for (it = m_player->getPlayerTeams()->begin();
    ...
    for (DLINK_ITERATOR<Team> iter = (*it)->iterate_TeamInstanceList();
      ...
      for (DLINK_ITERATOR<Object> objIter = team->iterate_TeamMemberList();
        ...
        if (!obj->isKindOf(KINDOF_CASH_GENERATOR) &&
...
Bool AIPlayer::isLocationSafe(const Coord3D *pos, const ThingTemplate *tthing) {
  ...
  // (note that stealthed allies aren't hidden from us, but we're only looking
  // for enemies here)
  ...
  PartitionFilterRejectByKindOf filterDozer(MAKE_KINDOF_MASK(KINDOF_DOZER),
                                            KINDOFMASK_NONE);
  ...
  Object *enemy = ThePartitionManager->getClosestObject(
      pos, radius, FROM_BOUNDINGSPHERE_2D, filters);
  if (enemy != NULL) {
    return false;
  }
  return true;
...
void AIPlayer::computeSuperweaponTarget(const SpecialPowerTemplate *power,
  ...
  for (i = 0; i < xCount; i++) {
    for (j = 0; j < yCount; j++) {
      ...
      pos.z = 0;
      Int curCash =
          getPlayerSuperweaponValue(&pos, playerNdx, 2 * weaponRadius);
      if (curCash > cash) {
        cash = curCash;
        bestPos = pos;
      }
  ...
  veryBestPos.z =
      TheTerrainLogic->getGroundHeight(veryBestPos.x, veryBestPos.y);
  *retPos = veryBestPos;
}
 ...
 */
Int AIPlayer::getPlayerSuperweaponValue(Coord3D *center, Int playerNdx,
                                        Real radius) {
    ...
    for (DLINK_ITERATOR<Team> iter = (*it)->iterate_TeamInstanceList();
      ...
      for (DLINK_ITERATOR<Object> iter = team->iterate_TeamMemberList();
        ...
        if (dx * dx + dy * dy < radSqr) {
          ...
          Real factor =
              1.0f - (dist / (2 * radius));  // 1.0 in center, 0.5 on edges.
          Real value = pObj->getTemplate()->calcCostToBuild(pPlayer);
          if (pObj->isKindOf(KINDOF_COMMANDCENTER)) {
            value = value /
                    10;  // Command centers cannot be killed by any superweapon,
                         // so we don't want to target them as highly. jba.
          }
          if (value > 3000) {
            value =
                value / 10;  // Superweapons can't be killed by superweapons, so
                             // we don't want to value them highly.
          }
          cash += factor * value;
        }
      }
...
// ------------------------------------------------------------------------------------------------
/** Search the computer player's buildings for one that can build the given
 * request and start training the unit. If busyOK is true, it will queue a unit
 * even if one is building.  This lets script invoked teams "push" to the front
 * of the queue. */
// ------------------------------------------------------------------------------------------------
Bool AIPlayer::startTraining(WorkOrder *order, Bool busyOK,
...
// ------------------------------------------------------------------------------------------------
/** Search the computer player's buildings for one that can build the given
 * request. If busyOK is true, it will return a busy factory if there are no
 * idle ones.  This is used for script invoked teams "push" to the front of the
 * queue. */
// ------------------------------------------------------------------------------------------------
...
Bool AIPlayer::isPossibleToBuildTeam(TeamPrototype *proto,
                                     ...
                                     Bool &notEnoughMoney) {
  /* Make sure we have at least one idle factory, and factories for all unit
   * types. */
  Bool anyIdle = false;
  Int cost = 0;
  ...
  for (int i = 0; i < proto->getTemplateInfo()->m_numUnitsInfo; i++) {
    ...
    if (thing) {
      ...
      cost +=
          thingCost * ((unitInfo[i].maxUnits + unitInfo[i].minUnits) / 2.0f);
    }
  }
...
// ------------------------------------------------------------------------------------------------
/** Check if this team is buildable, doesn't exceed maximum limits, meets
 * conditions, and isn't under construction. */
// ------------------------------------------------------------------------------------------------
Bool AIPlayer::isAGoodIdeaToBuildTeam(TeamPrototype *proto) {
  ...
  if (proto->countTeamInstances() >= proto->getTemplateInfo()->m_maxInstances) {
    if (TheGlobalData->m_debugAI) {
      AsciiString str;
      str.format("Team %s not chosen - %d already exist.",
  ...
  if (!isPossibleToBuildTeam(proto, true, needMoney)) {
    if (TheGlobalData->m_debugAI) {
      AsciiString str;
      if (needMoney) {
        str.format("Team %s not chosen - Not enough money.",
                   proto->getName().str());
      } else {
        str.format("Team %s not chosen - Factory/tech missing or busy.",
                   proto->getName().str());
      }
      TheScriptEngine->AppendDebugMessage(str, false);
    }
...
Bool AIPlayer::selectTeamToReinforce(Int minPriority) {
  ...
  Team *curTeam = NULL;
  Int curPriority =
      minPriority;  // Don't reinforce a team unless it is above min priority.
  const ThingTemplate *curThing = NULL;
  for (t = m_player->getPlayerTeams()->begin();
    ...
    if (busy) continue;
    if (proto->getTemplateInfo()->m_automaticallyReinforce &&
        proto->getTemplateInfo()->m_productionPriority > curPriority) {
      ...
      for (DLINK_ITERATOR<Team> iter = proto->iterate_TeamInstanceList();
        ...
        const TCreateUnitsInfo *unitInfo =
            &team->getPrototype()->getTemplateInfo()->m_unitsInfo[0];
        for (int i = 0;
             i < team->getPrototype()->getTemplateInfo()->m_numUnitsInfo; i++) {
  ...
  if (curTeam && curThing) {
    ...
    Object *unit = curTeam->tryToRecruit(
        curThing, &origin, TheAI->getAiData()->m_maxRecruitDistance);
    if (unit) {
...
Bool AIPlayer::selectTeamToBuild(void) {
  ...
  // collect all teams that are possible to build, and are at the highest
  // priority
  ...
  if (hiPri == invalidPri) return false;

  if (TheGlobalData->m_debugAI) {
    TheScriptEngine->AppendDebugMessage("**AI** Selecting team to build",
                                        false);
  }
...
// ------------------------------------------------------------------------------------------------
/** Build a specific team.  If priorityBuild, put at front of queue with
 * priority set. */
// ------------------------------------------------------------------------------------------------
void AIPlayer::buildSpecificAIBuilding(const AsciiString &thingName) {
  //
  AsciiString teamStr =
      "Error : Solo ai doesn't support BuildSpecificBuilding. '";
...
void AIPlayer::buildUpgrade(const AsciiString &upgrade) {
  ...
  if (m_player->hasUpgradeInProduction(curUpgrade)) {
    AsciiString msg =
        TheNameKeyGenerator->keyToName(m_player->getPlayerNameKey());
  ...
  for (const BuildListInfo *info = m_player->getBuildList(); info;
       info = info->getNext()) {
    Object *factory = TheGameLogic->findObjectByID(info->getObjectID());
    if (factory) {
      if (BitTest(factory->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION) ==
          TRUE)
        continue;
      ...
      const CommandSet *commandSet =
          TheControlBar->findCommandSet(factory->getCommandSetString());
      ...
      for (Int j = 0; j < MAX_COMMANDS_PER_SET; j++) {
        ...
        if (commandButton->getUpgradeTemplate()->getUpgradeName() ==
            curUpgrade->getUpgradeName()) {
          canUpgradeHere = true;
        }
      }
...
// ------------------------------------------------------------------------------------------------
/** Build a supply center near a supply source with minimumCash or more
 * resources. */
// ------------------------------------------------------------------------------------------------
void AIPlayer::buildBySupplies(Int minimumCash, const AsciiString &thingName) {
  ...
  if (bestSupplyWarehouse && tTemplate) {
    ...
    if (!tTemplate->isKindOf(KINDOF_CASH_GENERATOR)) {
      ...
      Region2D bounds;
      Int enemyNdx =
          TheScriptEngine->getSkirmishEnemyPlayer()->getPlayerIndex();
      getPlayerStructureBounds(&bounds, enemyNdx);
      offset.x = location.x - (bounds.lo.x + bounds.hi.x) * 0.5f;
      offset.y = location.y - (bounds.lo.y + bounds.hi.y) * 0.5f;
      offset.normalize();
    ...
    if (TheBuildAssistant->isLocationLegalToBuild(
            &location, tTemplate, angle, BuildAssistant::NO_OBJECT_OVERLAP,
            NULL, m_player) != LBC_OK) {
      // Warn.
      AsciiString bldgName = tTemplate->getName();
      bldgName.concat(
          " - buildAISupplyCenter unable to place.  Attempting to adjust "
          "position.");
      TheScriptEngine->AppendDebugMessage(bldgName, false);
      // try to fix.
      Real posOffset;
      // Wiggle it a little :)
      for (posOffset = 0; posOffset < 2 * SUPPLY_CENTER_CLOSE_DIST;
           posOffset += 2 * PATHFIND_CELL_SIZE_F) {
        Real offset = posOffset / 2;
        Real xPos, yPos;
        yPos = location.y - offset;
        for (xPos = location.x - offset; xPos <= location.x + offset;
             xPos += PATHFIND_CELL_SIZE_F) {
          newPos.x = xPos;
          newPos.y = yPos;
          valid = TheBuildAssistant->isLocationLegalToBuild(
                      &newPos, tTemplate, angle,
                      BuildAssistant::CLEAR_PATH |
                          BuildAssistant::TERRAIN_RESTRICTIONS |
                          BuildAssistant::NO_OBJECT_OVERLAP,
                      NULL, m_player) == LBC_OK;
        ...
        for (yPos = location.y - offset; yPos <= location.y + offset;
             yPos += PATHFIND_CELL_SIZE_F) {
          newPos.x = xPos;
          newPos.y = yPos;
          valid = TheBuildAssistant->isLocationLegalToBuild(
...
Object *AIPlayer::findSupplyCenter(Int minimumCash) {
  ...
  if (enemy) {
    ...
    enemyCenter.set((bounds.lo.x + bounds.hi.x) * 0.5f,
                    (bounds.lo.y + bounds.hi.y) * 0.5f, 0);
  }
  ...
  do {
    for (obj = TheGameLogic->getFirstObject(); obj;
      ...
      if (warehouseModule) {
        ...
        Real radius = SUPPLY_CENTER_CLOSE_DIST +
                      obj->getGeometryInfo().getBoundingCircleRadius();
        ...
        Object *supplyCenter = ThePartitionManager->getClosestObject(
            &center, radius, FROM_BOUNDINGSPHERE_2D, filters);
        if (supplyCenter) {
        ...
        if (enemy) {
          ...
          if (distSqr * 0.4 > (dx * dx + dy * dy) * 0.6f) {
            // closer than 60/40 to enemy than to us, probably not a good
...
void AIPlayer::buildAIBaseDefenseStructure(const AsciiString &thingName,
  ...
  //
  AsciiString teamStr =
      "Error : Solo ai doesn't support buildAIBaseDefenseStructure. '";
  TheScriptEngine->AppendDebugMessage(teamStr, false);
}
...
void AIPlayer::repairStructure(ObjectID structure) {
  ...
  enum BodyDamageType structureState =
      structureObj->getBodyModule()->getDamageState();
  ...
  if (m_structuresInQueue == MAX_STRUCTURES_TO_REPAIR) {
    DEBUG_LOG(
        ("Structure repair queue is full, ignoring repair request. JBA\n"));
    return;
  }
...
void AIPlayer::selectSkillset(Int skillset) {
  DEBUG_ASSERTCRASH(
      m_skillsetSelector == INVALID_SKILLSET_SELECTION,
      ("Selecting a skill set (%d) after one has already been chosen (%d) "
       "means some points have been incorrectly spent.\n",
...
void AIPlayer::updateBridgeRepair(void) {
  ...
  enum BodyDamageType bridgeState =
      bridgeObj->getBodyModule()->getDamageState();
...
void AIPlayer::buildSpecificAITeam(TeamPrototype *teamProto,
  ...
  if (teamProto) {
    ...
    const TCreateUnitsInfo *unitInfo =
        &teamProto->getTemplateInfo()->m_unitsInfo[0];
    ...
    if (orders) {
      ...
      if (team->m_team->getPrototype()->getTemplateInfo()->m_executeActions) {
        const Script *script =
            TheScriptEngine->findScriptByName(team->m_team->getPrototype()
                                                  ->getTemplateInfo()
  ...
  if (teamProto) {
    ...
    for (i = 0; i < teamProto->getTemplateInfo()->m_numUnitsInfo; i++) {
      ...
      if (thing) {
        ...
        while (count > 0) {
          Object *unit = theTeam->tryToRecruit(
              thing, &teamProto->getTemplateInfo()->m_homeLocation,
              recruitRadius);
          if (unit) {
            unitsRecruited++;
...
#endif
              ai->aiMoveToPosition(
                  &teamProto->getTemplateInfo()->m_homeLocation, CMD_FROM_AI);
            }
          } else {
            break;
          }
...
void AIPlayer::queueUnits(void) {
  ...
  // For each member of the current team to build, try to find a faction
  ...
  for (DLINK_ITERATOR<TeamInQueue> iter = iterate_TeamBuildQueue();
    ...
    for (WorkOrder *order = team->m_workOrders; order; order = order->m_next) {
      // check if there is a unit on the map that we can steal (recruit) instead
      // of building
      // @todo: Should this try to alter the home location of the recruiting
      // area to the center of the team, or to the home area of this player?
      Coord3D home =
          team->m_team->getPrototype()->getTemplateInfo()->m_homeLocation;
      Bool hasHome = false;
      ...
      while (order->isWaitingToBuild()) {
        Object *unit = team->m_team->tryToRecruit(
...
void AIPlayer::doBaseBuilding(void) {
  if (m_player->getCanBuildBase()) {
    ...
    // This timer is to keep from banging on the logic each frame.  If something
    // interesting happens, like a building is added or a unit finished, the
    // timers are shortcut.
    m_buildDelay--;
    if (m_buildDelay < 1) {
      if (m_readyToBuildStructure) {
...
void AIPlayer::checkReadyTeams(void) {
  ...
  {  // needed to scope iter.  silly ms c++.
    for (DLINK_ITERATOR<TeamInQueue> iter = iterate_TeamReadyQueue();
      ...
      Bool timeExpired = team->m_frameStarted + 60 * LOGICFRAMES_PER_SECOND <
                         TheGameLogic->getFrame();
      Bool allIdle = TRUE;
      Bool anyIdle = FALSE;
      if (team->m_reinforcement) {
      ...
      if (anyIdle &&
          team->m_team->getPrototype()->getTemplateInfo()->m_executeActions) {
        const Script *script =
            TheScriptEngine->findScriptByName(team->m_team->getPrototype()
                                                  ->getTemplateInfo()
                                                  ->m_productionCondition);
        if (script && script->getAction()) {
          // we have a start action.  So don't wait for allIdle as the team may
          // be guarding.
          allIdle = true;
        }
      }
      ...
      if (allIdle) {
        if (!team->m_sentToStartLocation) {
          ...
          /*
          if (team->m_team->getPrototype()->getTemplateInfo()->m_hasHomeLocation
          && !team->m_reinforcement) { AIGroup* theGroup = TheAI->createGroup();
                  if (theGroup) {
                          team->m_team->getTeamAsAIGroup(theGroup);
                          Coord3D destination =
          team->m_team->getPrototype()->getTemplateInfo()->m_homeLocation;
                          theGroup->groupTightenToPosition( &destination, false,
          CMD_FROM_AI ); team->m_frameStarted = TheGameLogic->getFrame();
                          continue;
                  }
          }
          */
        ...
        if (team->m_reinforcement) {
          ...
          // mark our completed team as "active" - this will invoke any OnCreate
          // scripts, etc.
...
void AIPlayer::checkQueuedTeams(void) {
  ...
  {  // needed to scope iter.  silly ms c++.
    for (DLINK_ITERATOR<TeamInQueue> iter = iterate_TeamBuildQueue();
      ...
      Bool anyIdle = false;
      for (DLINK_ITERATOR<Object> iter = team->m_team->iterate_TeamMemberList();
           !iter.done(); iter.advance()) {
        Object *obj = iter.cur();
...
void AIPlayer::doTeamBuilding(void) {
  ...
  if (m_player->getCanBuildUnits()) {
    ...
    if (m_teamDelay < 1) {
      ...
      // Note that this timer gets shortcut when a unit or building is
      // completed.
    }
  }
}
...
void AIPlayer::doUpgradesAndSkills(void) {
  ...
  if (m_player->getSciencePurchasePoints() > 0) {
    ...
    switch (m_skillsetSelector) {
      default:
      case 0:
        skillset = &sideInfo->m_skillSet1;
        break;
      case 1:
        skillset = &sideInfo->m_skillSet2;
        break;
      case 2:
        skillset = &sideInfo->m_skillSet3;
        break;
      case 3:
        skillset = &sideInfo->m_skillSet4;
        break;
      case 4:
        skillset = &sideInfo->m_skillSet5;
        break;
    }
    Int i;
...
void AIPlayer::update(void) {
  ...
  doUpgradesAndSkills();  // See if it's time to build an upgrade or buy a
                          // skill.
...
//----------------------------------------------------------------------------------------------------------
/**
 * Find any things that build stuff & add them to the build list.  Then build
 * any initially built buildings.
 */
void AIPlayer::newMap(void) {
  ...
  for (/* nothing */; info; info = info->getNext()) {
    ...
    if (info->isInitiallyBuilt()) {
      ...
      info->incrementNumRebuilds();  // the initial build in the normal build
                                     // list consumes a rebuild, so add one.
    }
  }
}
...
void AIPlayer::computeCenterAndRadiusOfBase(Coord3D *center, Real *radius) {
  ...
  for (info = m_player->getBuildList(); info; info = info->getNext()) {
    ...
    if (dy < 0) dy = -dy;
    Real bldgRadius =
        bldgPlan->getTemplateGeometryInfo().getBoundingCircleRadius() * 0.4f;
    dx += bldgRadius;
    dy += bldgRadius;
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIPlayer::crc(Xfer *xfer) {}  // end crc
...
void AIPlayer::xfer(Xfer *xfer) {
  ...
  for (DLINK_ITERATOR<TeamInQueue> teamInQueueIt = iterate_TeamBuildQueue();
       teamInQueueIt.done() == FALSE; teamInQueueIt.advance())
    teamBuildQueueCount++;
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    if (getFirstItemIn_TeamBuildQueue() != NULL) {
      DEBUG_CRASH(
          ("AIPlayer::xfer - TeamBuildQueue head is not NULL, you should "
           "delete it or something before loading a new list\n"));
    ...
    // the list was loaded in reverse order, reverse the list so it's in the
  ...
  for (DLINK_ITERATOR<TeamInQueue> teamReadyQueueIt = iterate_TeamReadyQueue();
       teamReadyQueueIt.done() == FALSE; teamReadyQueueIt.advance())
    teamReadyQueueCount++;
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    for (DLINK_ITERATOR<TeamInQueue> teamReadyQueueIt =
             iterate_TeamReadyQueue();
         teamReadyQueueIt.done() == FALSE; teamReadyQueueIt.advance()) {
      // get element
    ...
    if (getFirstItemIn_TeamReadyQueue() != NULL) {
      DEBUG_CRASH(
          ("AIPlayer::xfer - TeamReadyQueue head is not NULL, you should "
           "delete it or something before loading a new list\n"));
      throw SC_INVALID_DATA;
  ...
  xfer->xferUser(m_structuresToRepair,
                 sizeof(ObjectID) * MAX_STRUCTURES_TO_REPAIR);
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AIPlayer::loadPostProcess(void) {}  // end loadPostProcess
...
TeamInQueue::~TeamInQueue() {
  ...
  // If we have a team, activate it.  If it is empty, Team.cpp will remove empty
  // active teams.
...
Bool TeamInQueue::includesADozer() {
  ...
  for (order = m_workOrders; order; order = order->m_next) {
    // GLA dozers (workers) are also resource gatherers, so make sure it isn't a
    // gatherer. jba.
    if (order->m_thing->isKindOf(KINDOF_DOZER) &&
...
void TeamInQueue::disband() {
  Team *newTeam =
      m_team->getPrototype()->getControllingPlayer()->getDefaultTeam();
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void TeamInQueue::crc(Xfer *xfer) {}  // end crc
...
void TeamInQueue::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    if (m_workOrders != NULL) {
      DEBUG_CRASH(
          ("TeamInQueue::xfer - m_workOrders should be NULL but isn't.  "
           "Perhaps you should blow it away before loading\n"));
      throw SC_INVALID_DATA;
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void TeamInQueue::loadPostProcess(void) {}  // end loadPostProcess
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void WorkOrder::crc(Xfer *xfer) {}  // end crc
...
void WorkOrder::xfer(Xfer *xfer) {
  ...
  AsciiString thingTemplateName =
      m_thing ? m_thing->getName() : AsciiString::TheEmptyString;
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void WorkOrder::loadPostProcess(void) {}  // end loadPostProcess
