...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
static StateReturnType frameToSleepTime(UnsignedInt frame1,
 ...
 */
TurretStateMachine::TurretStateMachine(TurretAI* tai, Object* obj,
                                       AsciiString name)
  ...
  defineState(TURRETAI_IDLE, newInstance(TurretAIIdleState)(this),
              TURRETAI_IDLE, TURRETAI_IDLESCAN);
  defineState(TURRETAI_IDLESCAN, newInstance(TurretAIIdleScanState)(this),
              TURRETAI_HOLD, TURRETAI_HOLD);
  defineState(TURRETAI_AIM, newInstance(TurretAIAimTurretState)(this),
              TURRETAI_FIRE, TURRETAI_HOLD);
  defineState(TURRETAI_FIRE, newInstance(AIAttackFireWeaponState)(this, tai),
              TURRETAI_AIM, TURRETAI_AIM, fireConditions);
  defineState(TURRETAI_RECENTER, newInstance(TurretAIRecenterTurretState)(this),
              TURRETAI_IDLE, TURRETAI_IDLE);
  defineState(TURRETAI_HOLD, newInstance(TurretAIHoldTurretState)(this),
...
void TurretStateMachine::clear() {
  ...
  if (turret) turret->friend_notifyStateMachineChanged();
}
...
StateReturnType TurretStateMachine::setState(StateID newStateID) {
  ...
  TurretAI* turret = getTurretAI();
  if (turret && oldID != newStateID) turret->friend_notifyStateMachineChanged();
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void TurretStateMachine::crc(Xfer* xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void TurretStateMachine::loadPostProcess(void) {}  // end loadPostProcess
...
//-------------------------------------------------------------------------------------------------
static void parseTWS(INI* ini, void* /*instance*/, void* store,
                     const void* /*userData*/) {
  UnsignedInt* tws = (UnsignedInt*)store;
  const char* token = ini->getNextToken();
  while (token != NULL) {
    WeaponSlotType wslot =
        (WeaponSlotType)INI::scanIndexList(token, TheWeaponSlotTypeNames);
    *tws |= (1 << wslot);
...
//-------------------------------------------------------------------------------------------------
void TurretAIData::parseTurretSweep(INI* ini, void* instance, void* /*store*/,
                                    const void* userData) {
  ...
  INI::parseAngleReal(ini, instance, &self->m_turretFireAngleSweep[wslot],
                      NULL);
}

//-------------------------------------------------------------------------------------------------
void TurretAIData::parseTurretSweepSpeed(INI* ini, void* instance,
                                         void* /*store*/,
                                         const void* userData) {
  TurretAIData* self = (TurretAIData*)instance;
  WeaponSlotType wslot = (WeaponSlotType)INI::scanIndexList(
...
void TurretAIData::buildFieldParse(MultiIniFieldParse& p) {
  static const FieldParse dataFieldParse[] = {
      {"TurretTurnRate", INI::parseAngularVelocityReal, NULL,
       offsetof(TurretAIData, m_turnRate)},
      {"TurretPitchRate", INI::parseAngularVelocityReal, NULL,
       offsetof(TurretAIData, m_pitchRate)},
      {"NaturalTurretAngle", INI::parseAngleReal, NULL,
       offsetof(TurretAIData, m_naturalTurretAngle)},
      {"NaturalTurretPitch", INI::parseAngleReal, NULL,
       offsetof(TurretAIData, m_naturalTurretPitch)},
      {"FirePitch", INI::parseAngleReal, NULL,
       offsetof(TurretAIData, m_firePitch)},
      {"MinPhysicalPitch", INI::parseAngleReal, NULL,
       offsetof(TurretAIData, m_minPitch)},
      {"GroundUnitPitch", INI::parseAngleReal, NULL,
       offsetof(TurretAIData, m_groundUnitPitch)},
      {"TurretFireAngleSweep", TurretAIData::parseTurretSweep, NULL, NULL},
      {"TurretSweepSpeedModifier", TurretAIData::parseTurretSweepSpeed, NULL,
       NULL},
      {"ControlledWeaponSlots", parseTWS, NULL,
       offsetof(TurretAIData, m_turretWeaponSlots)},
      {"AllowsPitch", INI::parseBool, NULL,
       offsetof(TurretAIData, m_isAllowsPitch)},
#ifdef INTER_TURRET_DELAY
      {"InterTurretDelay", INI::parseDurationUnsignedInt, NULL,
       offsetof(TurretAIData, m_interTurretDelay)},
#endif
      {"MinIdleScanAngle", INI::parseAngleReal, NULL,
       offsetof(TurretAIData, m_minIdleScanAngle)},
      {"MaxIdleScanAngle", INI::parseAngleReal, NULL,
       offsetof(TurretAIData, m_maxIdleScanAngle)},
      {"MinIdleScanInterval", INI::parseDurationUnsignedInt, NULL,
       offsetof(TurretAIData, m_minIdleScanInterval)},
      {"MaxIdleScanInterval", INI::parseDurationUnsignedInt, NULL,
       offsetof(TurretAIData, m_maxIdleScanInterval)},
      {"RecenterTime", INI::parseDurationUnsignedInt, NULL,
       offsetof(TurretAIData, m_recenterTime)},
      {"InitiallyDisabled", INI::parseBool, NULL,
       offsetof(TurretAIData, m_initiallyDisabled)},
      {"FiresWhileTurning", INI::parseBool, NULL,
       offsetof(TurretAIData, m_firesWhileTurning)},
      {0, 0, 0, 0}};
...
//
{
  ...
  m_turretRotOrPitchSound =
      *(m_owner->getTemplate()->getPerUnitSound("TurretMoveLoop"));
}
...
TurretAI::~TurretAI() {
  ...
  if (m_turretStateMachine) m_turretStateMachine->deleteInstance();
}

// ------------------------------------------------------------------------------------------------
/** CRC */
// ------------------------------------------------------------------------------------------------
void TurretAI::crc(Xfer* xfer) {}  // end crc
...
void TurretAI::xfer(Xfer* xfer) {
  ...
  /* These 4 are loaded on creation, and don't change. jba.
          ...
          WhichTurretType
     m_whichTurret; Object*
     m_owner; AudioEventRTS
     m_turretRotOrPitchSound;		///< Sound of turret rotation
          */
...
#undef UNPACK_AND_XFER
  ...
  if (version >= 2) xfer->xferUnsignedInt(&m_sleepUntil);
...
//----------------------------------------------------------------------------------------------------------
Bool TurretAI::friend_turnTowardsAngle(Real desiredAngle, Real rateModifier,
                                       Real relThresh) {
...
//----------------------------------------------------------------------------------------------------------
Real TurretAI::getTurretSweepSpeedModifierForWeaponSlot(
    WeaponSlotType slot) const {
  return m_data->m_turretSweepSpeedModifier[slot];
}

//----------------------------------------------------------------------------------------------------------
void TurretAI::notifyFired() { m_didFire = true; }
...
Bool TurretAI::isTryingToAimAtTarget(const Object* victim) const {
  ...
  Object* obj;
  Coord3D pos;
  return (sid == TURRETAI_AIM &&
          friend_getTurretTarget(obj, pos) == TARGET_OBJECT && obj == victim);
}
...
void TurretAI::setTurretTargetObject(Object* victim, Bool forceAttacking) {
  ...
  if (victim == NULL) {
    ...
    // (note that we never ADD self as targeter here; that is done in the aim
    // state)
...
void TurretAI::setTurretTargetPosition(const Coord3D* pos) {
  ...
  m_turretStateMachine->setGoalObject(NULL);
  if (pos) m_turretStateMachine->setGoalPosition(pos);
...
//----------------------------------------------------------------------------------------------------------
/*
        Note that TurretAI isn't a behavior at all, so doesn't really "sleep";
   however, it does return this information so that its AIUpdate owner can
   decide how long it (the AIUpdate) should sleep. So it behooves you to
   maximize "sleep" potential here! (srj)
*/
...
UpdateSleepTime TurretAI::updateTurretAI() {
  ...
  DEBUG_ASSERTCRASH(!m_enabled || m_turretStateMachine->peekSleepTill() == 0 ||
                        m_turretStateMachine->peekSleepTill() >= m_sleepUntil,
  ...
  // DEBUG_LOG(("updateTurretAI frame %d:
  // %08lx\n",TheGameLogic->getFrame(),getOwner()));
  UpdateSleepTime subMachineSleep = UPDATE_SLEEP_FOREVER;  // assume the best!

  // either we don't care about continuous fire stuff, or we care, but time has
  // elapsed
  ...
  if (m_enabled ||
      m_turretStateMachine->getCurrentStateID() == TURRETAI_RECENTER) {
    m_didFire = false;
    ...
    if (m_didFire) {
      ...
      m_continuousFireExpirationFrame =
          now + ENABLE_SWEEP_FRAME_COUNT;  // so the recent firing will not
                                           // interrupt the moving sound
    }
...
void TurretAI::startRotOrPitchSound() {
  if (!m_turretRotOrPitchSound.isCurrentlyPlaying()) {
    ...
    m_turretRotOrPitchSound.setPlayingHandle(
        TheAudio->addAudioEvent(&m_turretRotOrPitchSound));
  }
}
...
#ifdef INTER_TURRET_DELAY
//----------------------------------------------------------------------------------------------------------
void TurretAI::getOtherTurretWeaponInfo(Int& numSelf, Int& numSelfReloading,
                                        Int& numSelfReady, Int& numOther,
...
Bool TurretAI::friend_isAnyWeaponInRangeOf(const Object* o) const {
  ...
  for (Int i = 0; i < WEAPONSLOT_COUNT; i++) {
    // ignore empty slots.
    const Weapon* w = getOwner()->getWeaponInWeaponSlot((WeaponSlotType)i);
    if (w == NULL || !isWeaponSlotOnTurret((WeaponSlotType)i)) continue;

    if (w->isWithinAttackRange(getOwner(), o)
        // srj sez: not sure if we want to do this or not.
        // jba sez: no, don't do this.  isWithinAttackRange checks terrain los
        // now, and some weapons (like tomahawk) can fire beyond los, so this
        // check is problematical here.
...
void TurretAI::friend_checkForIdleMoodTarget() {
  ...
  if (moodAdjust & MAA_Affect_Range_IgnoreAll) return;
  ...
  if (enemy) {
    ...
    // set this magic flag to indicate that this was an independent
    // attack-of-opportunity, and we should check to see if out-of-range, and if
    // so, nuke it
...
UnsignedInt TurretAI::friend_getInterTurretDelay() {
  if (m_data->m_interTurretDelay == 0) return 0;
  ...
  Int numOtherReloading;
  Int numOtherReady;
  getOtherTurretWeaponInfo(numSelf, numSelfReloading, numSelfReady, numOther,
                           numOtherReloading, numOtherReady);
...
StateReturnType TurretAIAimTurretState::onEnter() {
  // don't set the "turret-rotate" bit here; wait and see if we really turn or
  // not.
#ifdef INTER_TURRET_DELAY
  m_extraDelay = 0;
#endif
...
StateReturnType TurretAIAimTurretState::update() {
  // DEBUG_LOG(("TurretAIAimTurretState frame %d:
  ...
  TurretTargetType targetType =
      turret->friend_getTurretTarget(enemy, enemyPosition);
  Object* enemyForDistanceCheckOnly =
      enemy;  // Note: Do not use this anywhere except for the range check.
  ...
  switch (targetType) {
    ...
    case TARGET_OBJECT: {
      ...
      // if the enemy is gone, or we're out of range, or it changed teams, the
      // attack is over
      ...
      if (ableToAttackTarget) {
        ...
        CanAttackResult result = obj->getAbleToAttackSpecificObject(
            turret->isForceAttacking() ? ATTACK_CONTINUED_TARGET_FORCED
      ...
      // aim turret towards enemy (turret angle is relative to its parent
      // object)
      if (enemy->isKindOf(KINDOF_BRIDGE)) {
        ...
        Real distSqr = ThePartitionManager->getDistanceSquared(
            obj, &info.attackPoint1, FROM_BOUNDINGSPHERE_3D);
        if (distSqr > ThePartitionManager->getDistanceSquared(
                          obj, &info.attackPoint2, FROM_BOUNDINGSPHERE_3D)) {
      ...
      // in case we get into a squabble with our ai over whether or not we are a
      // targeter... (srj)
      if (enemyAI) enemyAI->addTargeter(obj->getID(), true);
  ...
  Real turnSpeedModifier = 1.0f;  // Just like how recentering turns you half
                                  // speed, sweeping can change your turn speed
  ...
  Bool turnAlignedToNemesis =
      turret->friend_turnTowardsAngle(aimAngle, turnSpeedModifier, REL_THRESH);
  ...
  if (turret->isAllowsPitch()) {
    ...
    // Some turrets want to fire at a set pitch, and some want to fire straight
    // to the target
    if (turret->getFirePitch() > 0) {
      desiredPitch = turret->getFirePitch();
    } else {
      // Find the pitch to the target, but unlike Turn, we can't go 360 so bind
      // at 0;
      Coord3D v;
      ThePartitionManager->getVectorTo(obj, &enemyPosition, FROM_CENTER_3D, v);

      // GetVectorTo only takes Object as the first, but we want the angle from
      // our Weapon to the
      //  target, not us to the target.  Raise our side to get the line to make
      //  sense.
      v.z -= obj->getGeometryInfo().getMaxHeightAbovePosition() /
             2;  // I kinda hate our logic/client split.
      // The point to fire from should be intrinsic to the turret, but in
      // reality it is very slow to look it up.
      ...
      if (v.length() > 0)
        ...
        actualPitch =
            0;  // Don't point at NAN, just point at 0 if they are right on us
      ...
      if (turret->getGroundUnitPitch() > 0) {
        ...
        if (adjust) {
          ...
          // As the unit gets closer, reduce the pitch so we don't shoot over
          // him.
  ...
  // For now, we require that we're within range before we can successfully exit
  // the AIM state, and move into the FIRE state.
  if (turnAlignedToNemesis && pitchAlignedToNemesis &&
      ((enemyForDistanceCheckOnly &&
        curWeapon->isWithinAttackRange(obj, enemyForDistanceCheckOnly)) ||
       (!enemyForDistanceCheckOnly &&
        curWeapon->isWithinAttackRange(obj, &enemyPosition)))) {
#ifdef INTER_TURRET_DELAY
    if (m_extraDelay > 0) {
      --m_extraDelay;
    } else {
...
StateReturnType TurretAIRecenterTurretState::update() {
  // DEBUG_LOG(("TurretAIRecenterTurretState frame %d:
  // %08lx\n",TheGameLogic->getFrame(),getTurretAI()->getOwner()));

  TurretAI* turret = getTurretAI();
  Bool angleAligned = turret->friend_turnTowardsAngle(
      turret->getNaturalTurretAngle(), 0.5f, 0.0f);
  Bool pitchAligned =
      turret->friend_turnTowardsPitch(turret->getNaturalTurretPitch(), 0.5f);

  if (angleAligned && pitchAligned) return STATE_SUCCESS;
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void TurretAIIdleState::crc(Xfer* xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void TurretAIIdleState::loadPostProcess(void) {}  // end loadPostProcess

//----------------------------------------------------------------------------------------------------------
void TurretAIIdleState::resetIdleScan() {
  UnsignedInt now = TheGameLogic->getFrame();
  UnsignedInt delay =
      GameLogicRandomValue(getTurretAI()->getMinIdleScanInterval(),
...
StateReturnType TurretAIIdleState::onEnter() {
  ...
  TurretAI* turret = getTurretAI();
  return frameToSleepTime(turret->friend_getNextIdleMoodTargetFrame(),
                          m_nextIdleScan);
}

//----------------------------------------------------------------------------------------------------------
StateReturnType TurretAIIdleState::update() {
  // DEBUG_LOG(("TurretAIIdleState frame %d:
  // %08lx\n",TheGameLogic->getFrame(),getTurretAI()->getOwner()));
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void TurretAIIdleScanState::crc(Xfer* xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void TurretAIIdleScanState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType TurretAIIdleScanState::onEnter() {
  ...
  if (minA == 0.0f && maxA == 0.0f) return STATE_SUCCESS;

  m_desiredAngle = minA + GameLogicRandomValueReal(0, maxA - minA);
  if (GameLogicRandomValue(0, 1) == 0) m_desiredAngle = -m_desiredAngle;
...
StateReturnType TurretAIIdleScanState::update() {
  // DEBUG_LOG(("TurretAIIdleScanState frame %d:
  // %08lx\n",TheGameLogic->getFrame(),getTurretAI()->getOwner()));
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void TurretAIHoldTurretState::crc(Xfer* xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void TurretAIHoldTurretState::loadPostProcess(void) {}  // end loadPostProcess
...
StateReturnType TurretAIHoldTurretState::update() {
  // DEBUG_LOG(("TurretAIHoldTurretState frame %d:
  // %08lx\n",TheGameLogic->getFrame(),getTurretAI()->getOwner()));

  if (TheGameLogic->getFrame() >= m_timestamp) return STATE_SUCCESS;
