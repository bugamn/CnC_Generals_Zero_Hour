 ...
 * an AIUpdate module to not having one... (MSB)
        ...
        NOTE: This comment has been wrong for about ten years now.  Any Object
 can be in an AIGroup
 */
...
AIGroup::~AIGroup() {
  ...
  for (i = m_memberList.begin(); i != m_memberList.end(); /* empty */) {
    ...
    if (member) {
      member->leaveGroup();
      i = m_memberList.begin();  // jump back to the beginning, cause
                                 // ai->leaveGroup will remove this element.
    } else {
 ...
 */
UnsignedInt AIGroup::getID(void) { return m_id; }
...
const VecObjectID &AIGroup::getAllIDs(void) const {
  ...
  for (std::list<Object *>::const_iterator cit = m_memberList.begin();
       cit != m_memberList.end(); ++cit) {
...
Bool AIGroup::isMember(Object *obj) {
  std::list<Object *>::iterator i =
      std::find(m_memberList.begin(), m_memberList.end(), obj);

  if (i == m_memberList.end()) return false;
...
/**
 * Remove any objects that aren't owned by the player, and return true if the
 * group was destroyed due to emptiness
 */
...
Bool AIGroup::getCenter(Coord3D *center) {
  ...
  if (count == 0 && !m_memberList.empty()) {
    /*
            if there are no AIs (eg, the team consists of a faction bldg), we
       can get here.

            This was originally used to offset the centers of objects moving
       (still used for that) and non-ais can't move. So if you have a mix of
       ai's & not ai's, you want just the ais. But it seems reasonable that if
       there are no ai's, it returns the center of the other stuff.  Cause they
...
void AIGroup::recompute(void) {
  ...
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    ...
    if ((*i)->isKindOf(KINDOF_IMMOBILE)) continue;
    ...
    if (ai) {
      //
      // speed is slowest speed ... we won't consider slow speeds from objects
      // that are being penalized by their damage state, if we did the whole
      // group would slow down to that slowest penalized speed, instead only
      // those objects that are penalized will fall out of "formation" ...
      // bummer for them!
      //
      Real maxSpeed = ai->getCurLocomotorSpeed();
      if (m_speed > maxSpeed &&
          IS_CONDITION_BETTER(obj->getBodyModule()->getDamageState(),
 ...
 */
Int AIGroup::getCount(void) { return m_memberListSize; }

/**
 * Returns true if the group has no members
 */
Bool AIGroup::isEmpty(void) { return m_memberList.empty(); }
...
void AIGroup::computeIndividualDestination(Coord3D *dest,
                                           ...
                                           Object *obj, const Coord3D *center,
                                           Bool isFormation) {
  Coord2D v;
  ...
  if (ai && ai->isDoingGroundMovement()) {
    if (isFormation) {
      TheAI->pathfinder()->adjustDestination(obj, ai->getLocomotorSet(), dest,
                                             NULL);
    } else {
      TheAI->pathfinder()->adjustDestination(obj, ai->getLocomotorSet(), dest,
                                             groupDest);
...

{
  ...
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    ...
    // Note - we are getting the closest of ANY type of unit for later testing
    // intentionally. jba.
    ...
    for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
      ...
      if (ai) {
        if (!TheAI->pathfinder()->isLinePassable(
                obj, ai->getLocomotorSet().getValidSurfaces(), obj->getLayer(),
                *obj->getPosition(), center, false, true)) {
          isPassable = false;
        }
      }
    }
  ...
  m_groundPath = TheAI->pathfinder()->findGroundPath(
      &center, pos, PATH_DIAMETER_IN_CELLS, false);
...

{
  ...
  PathNode *node;
  for (node = m_groundPath->getFirstNode(); node;
  ...
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    ...
    if ((*i)->isKindOf(KINDOF_MOB_NEXUS)) {
      ...
      // so the nexus will have a far-away goal position for the mobsters to aim
      // at
    }
    ...
    // If units are closer to the end vector than the start vector, use the end
    // vector.
    Real distToEndSqr;
    Real distToStartSqr;
  ...
  if (useEndVector) {
    ...
    for (theUnit = iter2->first(); theUnit; theUnit = iter2->next()) {
      ...
      // Sort by the dot product of normal.
      iter->insert(theUnit,
                   dx * startVectorNormal.x + dy * startVectorNormal.y);
    }
    iter2->makeEmpty();
  }
  ...
  for (theUnit = iter->first(); theUnit; theUnit = iter->next()) {
    ...
    Int columnDelta =
        1 - (curIndex /
             divisor);  // 1, 0, -1 from left to right across column. jba.
    ...
    Int fiveColumnDelta =
        2 -
        (curIndex /
         divisor);  // 2, 1, 0, -1, -2 from left to right across column. jba.
  ...
  for (group = LOCO_MOVES_FRONT; group >= LOCO_MOVES_BACK; group--) {
    for (theUnit = iter2->first(); theUnit; theUnit = iter2->next()) {
      ...
      Int min5 = 10000;
      for (i = 0; i < 3; i++)
        if (column3[i] < min3) min3 = column3[i];
      for (i = 0; i < 5; i++)
        if (column5[i] < min5) min5 = column5[i];
      Int delta = 10000;
      Int best = -1;
      for (i = 0; i < 3; i++) {
        if (column3[i] == min3) {
  ...
  for (theUnit = iter2->first(); theUnit; theUnit = iter2->next()) {
    ...
    while (node) {
      ...
      for (tmpNode = node->getNextOptimized(); tmpNode;
      ...
      cornerVectorNormal.y =
          nextNode->getPosition()->x - previousNode->getPosition()->x;
      cornerVectorNormal.x =
          -(nextNode->getPosition()->y - previousNode->getPosition()->y);
      ...
      for (tmpNode = previousNode->getNextOptimized();
           tmpNode && tmpNode != node; tmpNode = tmpNode->getNextOptimized()) {
    ...
    TheAI->pathfinder()->adjustDestination(theUnit, ai->getLocomotorSet(),
                                           &dest, NULL);
...

{
  ...
  for (theUnit = iter->first(); theUnit; theUnit = iter->next()) {
    ...
    Int columnDelta =
        1 -
        (curIndex / divisor);  // 0, 1 from left to right across column. jba.
    if (columnDelta == 0) columnDelta = -1;
    divisor = ((unitsToPath + 1) / 3);
    if (divisor < 1) divisor = 1;
    Int threeColumnDelta =
        (curIndex / divisor);  // 0, 1, 2 from left to right across column. jba.
  ...
  for (group = LOCO_MOVES_FRONT; group >= LOCO_MOVES_BACK; group--) {
    for (theUnit = iter2->first(); theUnit; theUnit = iter2->next()) {
      ...
      Int min3 = 10000;
      for (i = 0; i < 3; i += 2)
        if (column2[i] < min2) min2 = column2[i];
      for (i = 0; i < 3; i++)
        if (column3[i] < min3) min3 = column3[i];
      Int delta = 10000;
      Int best = -1;
 ...
 * Move to given position(s)
 */
void AIGroup::groupMoveToPosition(const Coord3D *pos, Bool addWaypoint,
                                  CommandSourceType cmdSource) {
  ...
  std::list<Object *>::iterator i;
  if (!isFormation && cmdSource == CMD_FROM_PLAYER &&
  ...
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    if ((*i)->getAI()) {
      if (!(*i)->getAI()->isDoingGroundMovement()) {
        tightenGroup =
            FALSE;  // Don't tighten aircraft.  It is a bad idea. jba.
        isFormation = false;
      }
    }
  }
  ...
  for (theUnit = iter->first(); theUnit; theUnit = iter->next()) {
    ...
    computeIndividualDestination(&dest, &goalPos, theUnit, &center,
                                 isFormation);

    if (cmdSource == CMD_FROM_PLAYER &&
        BitTest(theUnit->getStatusBits(), OBJECT_STATUS_CAN_STEALTH) &&
        ai->canAutoAcquire()) {
      // When ordering a combat stealth unit to move, there is a single special
      // case we want to handle. When a stealth unit is currently not stealthed
      // and doesn't autoacquire while stealthed, then when the player
      // specifically orders the unit to stop, we want to not autoacquire until
      // he is able to stealth again. Of course, if he's detected, then don't
      // bother trying.
      if (!BitTest(theUnit->getStatusBits(), OBJECT_STATUS_STEALTHED) &&
          !BitTest(theUnit->getStatusBits(), OBJECT_STATUS_DETECTED)) {
        ...
        if (!ai->canAutoAcquireWhileStealthed()) {
          ...
          StealthUpdate *stealth =
              (StealthUpdate *)theUnit->findUpdateModule(key_StealthUpdate);
          if (stealth) {
            // Delay the mood check time (for autoacquire) until after the unit
            // can stealth again.
            UnsignedInt stealthFrames = stealth->getStealthDelay();
            // Skew it a little due to having a large group selected.
            UnsignedInt randomFrames =
                GameLogicRandomValue(0, LOGICFRAMES_PER_SECOND);
            ai->setNextMoodCheckTime(TheGameLogic->getFrame() + stealthFrames +
                                     randomFrames);
          }
        }
      }
...
void AIGroup::groupScatter(CommandSourceType cmdSource) {
  ...
  for (theUnit = iter->first(); theUnit; theUnit = iter->next()) {
    ...
    delta.normalize();
    dest.x +=
        delta.x * 4 * theUnit->getGeometryInfo().getBoundingCircleRadius();
    dest.y +=
        delta.y * 4 * theUnit->getGeometryInfo().getBoundingCircleRadius();
    ai->aiMoveToPosition(&dest, cmdSource);
  }
}
  ...
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    ...
    if (ai) {
      ...
      if (cmdSource == CMD_FROM_PLAYER &&
          ...
          ai->canAutoAcquire()) {
        // When ordering a combat stealth unit to stop, there is a single
        // special case we want to handle. When a stealth unit is currently not
        // stealthed and doesn't autoacquire while stealthed, then when the
        // player specifically orders the unit to stop, we want to not
        // autoacquire until he is able to stealth again. Of course, if he's
        // detected, then don't bother trying.
        if (!BitTest(obj->getStatusBits(), OBJECT_STATUS_STEALTHED) &&
          ...
          if (!ai->canAutoAcquireWhileStealthed()) {
            ...
            if (stealth) {
              // Delay the mood check time (for autoacquire) until after the
              // unit can stealth again.
              UnsignedInt stealthFrames = stealth->getStealthDelay();
              // Skew it a little due to having a large group selected.
              UnsignedInt randomFrames =
                  GameLogicRandomValue(0, LOGICFRAMES_PER_SECOND);
              ai->setNextMoodCheckTime(TheGameLogic->getFrame() +
                                       stealthFrames + randomFrames);
            }
          }
        }
      }
 ...
 * Attack given object
 */
void AIGroup::groupAttackObjectPrivate(Bool forced, Object *victim,
                                       Int maxShotsToFire,
                                       CommandSourceType cmdSource) {
  if (!victim) {
  ...
  for (theUnit = iter->first(); theUnit; theUnit = iter->next()) {
    ...
    if (contain && contain->isPassengerAllowedToFire()) {
      // Loop through each member and order them to attack the same target (if
      // possible)
        ...
        for (ContainedItemsList::const_iterator it = items->begin();
          ...
          CanAttackResult result =
              garrisonedMember->getAbleToAttackSpecificObject(
                  forced ? ATTACK_NEW_TARGET_FORCED : ATTACK_NEW_TARGET, victim,
                  cmdSource);
    ...
    SpawnBehaviorInterface *spawnInterface =
        theUnit->getSpawnBehaviorInterface();
    if (spawnInterface) {
      spawnInterface->orderSlavesToAttackTarget(victim, maxShotsToFire,
                                                cmdSource);
    }
 ...
 */
void AIGroup::groupAttackTeam(const Team *team, Int maxShotsToFire,
                              CommandSourceType cmdSource) {
  if (!team) {
    // Hard to kill em if they're already dead.  jba
    return;
...
void AIGroup::groupAttackPosition(const Coord3D *pos, Int maxShotsToFire,
  ...
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    if (!pos) {
      // If you specify a NULL position, it means you are attacking your own
      // location.
        ...
        for (ContainedItemsList::const_iterator it = items->begin();
          ...
          CanAttackResult result =
              garrisonedMember->getAbleToUseWeaponAgainstTarget(
                  ATTACK_NEW_TARGET, NULL, &attackPos, cmdSource);
          if (result == ATTACKRESULT_POSSIBLE ||
              result == ATTACKRESULT_POSSIBLE_AFTER_MOVING) {
            AIUpdateInterface *memberAI = garrisonedMember->getAI();
            if (memberAI) {
    ...
    if (spawnInterface) {
      spawnInterface->orderSlavesToAttackPosition(&attackPos, maxShotsToFire,
                                                  cmdSource);
    }
  ...
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    ...
    if (ai) {
      if ((*i)->isKindOf(KINDOF_AIRCRAFT) && (*i)->isAirborneTarget()) {
        // Calculate the highest point on the ground to drop off troops (chinook
        // or other air transports)
        Coord3D pos = *((*i)->getPosition());
        PathfindLayerEnum layerAtDest =
            TheTerrainLogic->getHighestLayerForDestination(&pos);
      ...
      // case, simple call the function directly. Special powers work in a
      // similar manner.
      ContainModuleInterface *contain = (*i)->getContain();
      if (contain) {
...
void AIGroup::groupExecuteRailedTransport(CommandSourceType cmdSource) {
  ...
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    ...
    if (ai) ai->aiExecuteRailedTransport(cmdSource);
 ...
 */
void AIGroup::groupGuardPosition(const Coord3D *pos, GuardMode guardMode,
                                 CommandSourceType cmdSource) {
  if (!pos) {
    return;
  }
 ...
 */
void AIGroup::groupGuardObject(Object *objToGuard, GuardMode guardMode,
                               CommandSourceType cmdSource) {
  if (!objToGuard) {
    return;
  }
...
void AIGroup::groupHackInternet(
    CommandSourceType cmdSource)  ///< Begin hacking the internet for free cash
                                  ///< from the heavens.
{
  std::list<Object *>::iterator i;
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
...
void AIGroup::groupCreateFormation(
    CommandSourceType cmdSource)  ///< Create a formation.
{
  Coord3D center;
  Coord2D min;
  Coord2D max;
...
/**
 * The unit(s)/structure will perform it's special power -- special powers
 * triggered by buildings don't use AIUpdateInterfaces!!! No special power uses
 * an AIUpdateInterface immediately, but special abilities, which are derived
 * from special powers do... and are unit triggered. Those do have AI.
 */
void AIGroup::groupDoSpecialPower(UnsignedInt specialPowerID,
  ...
  std::list<Object *>::iterator i;
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    // Special powers do a lot of different things, but the top level stuff
    // doesn't use ai interface code. It finds the special power module and
    // calls it directly for each object.
    Object *object = (*i);
    ...
    if (spTemplate) {
      ...
      if ((spTemplate->getRequiredScience() != SCIENCE_INVALID) &&
        ...
        if (!object->getControllingPlayer()->hasScience(
                spTemplate->getRequiredScience()))
          continue;  // Nice try, smacktard.
      }

      SpecialPowerModuleInterface *mod =
          object->getSpecialPowerModule(spTemplate);
      if (mod) {
        if (TheActionManager->canDoSpecialPower(
                object, spTemplate, CMD_FROM_PLAYER, commandOptions)) {
    ...
    if (spTemplate) {
      ...
      if (mod) {
        if (TheActionManager->canDoSpecialPowerAtLocation(
                object, location, CMD_FROM_PLAYER, spTemplate, objectInWay,
                commandOptions)) {
    ...
    if (spTemplate) {
      ...
      if (mod) {
        if (TheActionManager->canDoSpecialPowerAtObject(
                object, target, CMD_FROM_PLAYER, spTemplate, commandOptions)) {
...
void AIGroup::groupSurrender(const Object *objWeSurrenderedTo, Bool surrender,
                             CommandSourceType cmdSource) {
...
void AIGroup::groupCheer(CommandSourceType cmdSource) {
  ...
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    Object *object = (*i);
    // This allows all special conditions states to reset after a specified
    // delay. Assume only one works at a time (it'll clear any others).
    object->setSpecialModelConditionState(MODELCONDITION_SPECIAL_CHEERING,
 ...
 */
void AIGroup::groupPickUpPrisoner(Object *prisoner,
                                  enum CommandSourceType cmdSource) {
  std::list<Object *>::iterator i;
  Object *obj;
  ...
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    ...
    AIUpdateInterface *ai = obj->getAIUpdateInterface();
    if (ai) ai->aiPickUpPrisoner(prisoner, cmdSource);
 ...
 * Return to prison
 */
void AIGroup::groupReturnToPrison(Object *prison,
                                  enum CommandSourceType cmdSource) {
  std::list<Object *>::iterator i;
  Object *obj;
  ...
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    ...
    AIUpdateInterface *ai = obj->getAIUpdateInterface();
    if (ai) ai->aiReturnPrisoners(prison, cmdSource);
 ...
 * Combat drop
 */
void AIGroup::groupCombatDrop(Object *target, const Coord3D &pos,
                              CommandSourceType cmdSource) {
  std::list<Object *>::iterator i;
  Object *obj;
  ...
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    ...
    // do action
    AIUpdateInterface *ai = obj->getAIUpdateInterface();
    if (ai) ai->aiCombatDrop(target, pos, cmdSource);

  }  // end for, i
}

//-------------------------------------------------------------------------------------
// Used by scripts to issue a command button order - Note that it's possible
// that some commands are not AI commands!
//-------------------------------------------------------------------------------------
 ...
 */
AttitudeType AIGroup::getAttitude(void) const { return AI_PASSIVE; }
...
Bool AIGroup::setWeaponLockForGroup(WeaponSlotType weaponSlot,
  ...
  std::list<Object *>::iterator i;
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    if ((*i)->setWeaponLock(weaponSlot, lockType)) any = true;
  }
  return any;
}
...
// This function loops through the AIGroup setting the weaponset only for those
// units that have the specified weaponset. If a member doesn't have the
// weaponset, nothing happens for that unit.
void AIGroup::setWeaponSetFlag(WeaponSetType wst) {
  std::list<Object *>::iterator i;
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    Object *obj = (*i);
    // First check to see if our object even has the specified weaponset. It's
    // very likely that a selected group won't all have the same weaponset
    // options, so only set it for those members that have it.
    WeaponSetFlags flags;
    flags.set(wst);
    const WeaponTemplateSet *set =
        obj->getTemplate()->findWeaponTemplateSet(flags);
    if (set) {
...
void AIGroup::queueUpgrade(const UpgradeTemplate *upgrade) {
  ...
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    ...
    // There is an extra check for Object type only.  These are the same checks
    // as in ControlCommandProcessing when the message was going out.  We are
    // just revalidating on the way in to stop cheaters.
    if (!TheUpgradeCenter->canAffordUpgrade(thisMember->getControllingPlayer(),
                                            upgrade, FALSE)) {
      continue;
    }
    if (upgrade->getUpgradeType() == UPGRADE_TYPE_OBJECT) {
      if (thisMember->hasUpgrade(upgrade) ||
          !thisMember->affectedByUpgrade(upgrade))
        continue;
    }

    // Ever think to check if this thing can actually build the upgrade to "stop
    // cheaters"?
    if (!thisMember->canProduceUpgrade(upgrade))
      continue;  // They have faked their button; go out of sync. (Cheater will
                 // execute it, non cheater will not execute it.)
    ...
    if (pu->canQueueUpgrade(upgrade) == CANMAKE_QUEUE_FULL)
      continue;  // So we don't charge them for something that we can't build...
                 // happy happy
...
// Returns an object that can perform the special power. Useful for making
// queries on the Action Manager
//------------------------------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void AIGroup::groupOverrideSpecialPowerDestination(
    SpecialPowerType spType, const Coord3D *loc, CommandSourceType cmdSource) {
  std::list<Object *>::iterator i;
  for (i = m_memberList.begin(); i != m_memberList.end(); ++i) {
    Object *object = (*i);
    if (object) {
      SpecialPowerUpdateInterface *spuInterface =
          object->findSpecialPowerWithOverridableDestinationActive(spType);
      if (spuInterface) {
...
void AIGroup::crc(Xfer *xfer) {
  ...
  for (std::list<Object *>::iterator it = m_memberList.begin();
...
//-----------------------------------------------------------------------------
void AIGroup::loadPostProcess(void) {}  // end loadPostProcess
