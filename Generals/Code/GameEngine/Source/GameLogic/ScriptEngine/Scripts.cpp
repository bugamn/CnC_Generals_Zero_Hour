...
// Changing the order or meaning of either of these will require you to update
// the maps in a meaningful way. If there are new entries, add them to the end,
...
const char *ShakeIntensities[] = {"Subtle", "Normal",       "Strong",
                                  "Severe", "Cine_Extreme", "Cine_Insane"};
...
/**
 ScriptList::updateDefaults -  checks for empty script lists, and adds some
 default stuff so you don't get a totally blank screen in the editor.
*/
...
/**
  Ctor.
*/
ScriptList::ScriptList(void) : m_firstGroup(NULL), m_firstScript(NULL) {}

/**
  Dtor.  Deletes any script lists or group lists.  Note that dtors for groups
  and script lists delete the whole list, so don't need to traverse here.
*/
...
void ScriptList::xfer(Xfer *xfer) {
  ...
  for (script = getScript(); script; script = script->getNext()) scriptCount++;
  countVerify = scriptCount;
  xfer->xferUnsignedShort(&scriptCount);
  if (countVerify != scriptCount) {
    DEBUG_CRASH(
        ("ScriptList::xfer - Script list count has changed, attempting to "
         "recover."));
  ...
  if (scriptCount > 0) {
    ...
    if (s_mtScript == NULL)
      s_mtScript = newInstance(
          Script);  // Yes it leaks, but this is unusual recovery only. jba.
    while (scriptCount) {
  ...
  for (scriptGroup = getScriptGroup(); scriptGroup;
       scriptGroup = scriptGroup->getNext())
    scriptGroupCount++;
  countVerify = scriptGroupCount;
  xfer->xferUnsignedShort(&scriptGroupCount);
  if (countVerify != scriptGroupCount) {
    DEBUG_CRASH(
        ("ScriptList::xfer - Script group count has changed, attempting to "
         "recover."));
  ...
  if (scriptGroupCount > 0) {
    ...
    if (s_mtGroup == NULL)
      s_mtGroup = newInstance(
          ScriptGroup);  // Yes it leaks, but this is only for recovery.
...
*/
ScriptList *ScriptList::duplicateAndQualify(
    const AsciiString &qualifier, const AsciiString &playerTemplateName,
...

  {
    ...
    ScriptGroup *dst = NULL;
    while (src) {
      ScriptGroup *tmp = src->duplicateAndQualify(qualifier, playerTemplateName,
                                                  newPlayerName);
...

  {
    ...
    Script *dst = NULL;
    while (src) {
      Script *tmp = src->duplicateAndQualify(qualifier, playerTemplateName,
                                             newPlayerName);
...
Bool ScriptList::ParseScriptsDataChunk(DataChunkInput &file,
  ...
  Int i;
  file.registerParser(AsciiString("ScriptList"), info->label,
                      ScriptList::ParseScriptListDataChunk);
  DEBUG_ASSERTCRASH(s_numInReadList == 0,
                    ("Leftover scripts floating aroung."));
  ...
  if (file.parse(&readInfo)) {
    DEBUG_ASSERTCRASH(readInfo.numLists < MAX_PLAYER_COUNT,
                      ("Read too many, overrun buffer."));
...
void ScriptList::WriteScriptListDataChunk(DataChunkOutput &chunkWriter) {
  /**********SCRIPTS DATA ***********************/
  if (m_firstScript)
    m_firstScript->WriteScriptDataChunk(chunkWriter, m_firstScript);
  if (m_firstGroup)
    m_firstGroup->WriteGroupDataChunk(chunkWriter, m_firstGroup);
}
...
Bool ScriptList::ParseScriptListDataChunk(DataChunkInput &file,
  ...
  pInfo->numLists++;
  file.registerParser(AsciiString("Script"), info->label,
                      Script::ParseScriptFromListDataChunk);
  file.registerParser(AsciiString("ScriptGroup"), info->label,
                      ScriptGroup::ParseGroupDataChunk);
...
/**
  Dtor - The script list deletes the rest of the list, but we have to loop &
  delete sll the script groups in out list.
*/
ScriptGroup::~ScriptGroup(void) {
  if (m_firstScript) {
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void ScriptGroup::crc(Xfer *xfer) {}  // end crc
...
void ScriptGroup::xfer(Xfer *xfer) {
  ...
  if (countVerify != scriptCount) {
    DEBUG_CRASH(
        ("ScriptGroup::xfer - Script list count has changed, attempting to "
         "recover."));
    // throw SC_INVALID_DATA; try to recover. jba.
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void ScriptGroup::loadPostProcess(void) {}  // end loadPostProcess
...
/**
  ScriptGroup::duplicateAndQualify - Creates a full, "deep" copy of ScriptGroup,
        ...
        m_nextGroup is NULL on the copy.
*/
ScriptGroup *ScriptGroup::duplicateAndQualify(
    const AsciiString &qualifier, const AsciiString &playerTemplateName,
    const AsciiString &newPlayerName) const {
  ScriptGroup *pNew = newInstance(ScriptGroup);
...
void ScriptGroup::WriteGroupDataChunk(DataChunkOutput &chunkWriter,
  ...
  while (pGroup) {
    ...
    if (pGroup->m_firstScript)
      Script::WriteScriptDataChunk(chunkWriter, pGroup->m_firstScript);
    chunkWriter.closeDataChunk();
 ...
 *
 */
Bool ScriptGroup::ParseGroupDataChunk(DataChunkInput &file, DataChunkInfo *info,
                                      void *userData) {
  ...
  file.registerParser(AsciiString("Script"), info->label,
                      Script::ParseScriptFromGroupDataChunk);
...
/**
  Dtor - The condition and action deletes the rest of the list, but we have to
  loop & delete all the scripts in out list.
*/
Script::~Script(void) {
  if (m_nextScript) {
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void Script::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void Script::loadPostProcess(void) {}  // end loadPostProcess

/**
  Script::duplicate - Creates a full, "deep" copy of script. Condition list and
  action list is duplicated as well.  Note - just the script, doesn't duplicate
  a list of scripts.  m_nextScript is NULL on the copy.
*/
...
Script *Script::duplicateAndQualify(const AsciiString &qualifier,
  ...
  pNew->m_delayEvaluationSeconds = m_delayEvaluationSeconds;
  if (m_condition) {
    pNew->m_condition = m_condition->duplicateAndQualify(
        qualifier, playerTemplateName, newPlayerName);
  }
  if (m_action) {
    pNew->m_action = m_action->duplicateAndQualify(
        qualifier, playerTemplateName, newPlayerName);
  }
  if (m_actionFalse) {
    pNew->m_actionFalse = m_actionFalse->duplicateAndQualify(
        qualifier, playerTemplateName, newPlayerName);
  }
  return pNew;
}
...
/**
  Script::updateFrom - Copies all the data from pSrc into this.  Any data in
  this is deleted (conditions, actions).  Note that this guts pSrc, and removes
  it's conditions and actions.  Intended for use in an edit dialog, where pSrc
  is a copy edited, and if cancelled discarded, and if not cancelled, updated
  into the real script, then discarded.
*/
...
void Script::WriteScriptDataChunk(DataChunkOutput &chunkWriter,
  ...
  while (pScript) {
    ...
    if (pScript->m_condition)
      OrCondition::WriteOrConditionDataChunk(chunkWriter, pScript->m_condition);
    if (pScript->m_action)
      ScriptAction::WriteActionDataChunk(chunkWriter, pScript->m_action);
    if (pScript->m_actionFalse)
      ScriptAction::WriteActionFalseDataChunk(chunkWriter,
                                              pScript->m_actionFalse);
    chunkWriter.closeDataChunk();
...
/**
 * Script::ParseScriptFromGroupDataChunk - read a script chunk in a script
 *group. Format is the newer CHUNKY format. See
 *ScriptList::WriteScriptListDataChunk for the writer. Input: DataChunkInput
 *
 */
...
/**
 * Script::findPreviousOrCondition - find the OrCondition that immediately
 *proceeds curOr. Input: OrCondition
 *
 */
OrCondition *Script::findPreviousOrCondition(OrCondition *curOr) {
  ...
  DEBUG_CRASH(
      ("Tried to find an OrCondition that doesn't seem to exist (jkmcd)"));
  return NULL;
}
...
OrCondition *OrCondition::duplicateAndQualify(
    const AsciiString &qualifier, const AsciiString &playerTemplateName,
    const AsciiString &newPlayerName) const {
  OrCondition *pNew = newInstance(OrCondition);
  if (m_firstAnd) {
    pNew->m_firstAnd = m_firstAnd->duplicateAndQualify(
        qualifier, playerTemplateName, newPlayerName);
  }
  OrCondition *pLink = m_nextOr;
  OrCondition *pCur = pNew;
  while (pLink) {
    ...
    pCur = pCur->m_nextOr;
    if (pLink->m_firstAnd) {
      pCur->m_firstAnd = pLink->m_firstAnd->duplicateAndQualify(
          qualifier, playerTemplateName, newPlayerName);
    }
    pLink = pLink->m_nextOr;
  }
...
void OrCondition::WriteOrConditionDataChunk(DataChunkOutput &chunkWriter,
  ...
  /**********OR CONDITION DATA ***********************/
  while (pOrCondition) {
    chunkWriter.openDataChunk("OrCondition",
                              K_SCRIPT_OR_CONDITION_DATA_VERSION_1);
    if (pOrCondition->m_firstAnd)
      Condition::WriteConditionDataChunk(chunkWriter, pOrCondition->m_firstAnd);
    chunkWriter.closeDataChunk();
...
Bool OrCondition::ParseOrConditionDataChunk(DataChunkInput &file,
  ...
  file.registerParser(AsciiString("Condition"), info->label,
                      Condition::ParseConditionDataChunk);
  return file.parse(pOrCondition);
}

/**
 * OrCondition::findPreviousCondition - find the condition that immediately
 *proceeds curCond. Format is the newer CHUNKY format. See
 *OrCondition::WriteOrConditionDataChunk for the writer. Input: DataChunkInput
 *
 */
...
Condition::Condition(enum ConditionType type)
    : m_conditionType(type), m_numParms(0) {
  Int i;
  for (i = 0; i < MAX_PARMS; i++) {
    m_parms[i] = NULL;
  }
...
void Condition::setConditionType(enum ConditionType type) {
  ...
  for (i = 0; i < m_numParms; i++) {
    if (m_parms[i]) m_parms[i]->deleteInstance();
    m_parms[i] = NULL;
  }
  m_conditionType = type;
  const ConditionTemplate *pTemplate =
      TheScriptEngine->getConditionTemplate(m_conditionType);
...
Condition *Condition::duplicate(void) const {
  ...
  Condition *pCur = pNew;
  while (pLink) {
    pCur->m_nextAndCondition =
        newInstance(Condition)(pLink->getConditionType());
...
Condition *Condition::duplicateAndQualify(
    const AsciiString &qualifier, const AsciiString &playerTemplateName,
    const AsciiString &newPlayerName) const {
  Condition *pNew = newInstance(Condition)(m_conditionType);
  Int i;
    ...
    for (int j = 0; ParameterChangesVer2[j] != -1; ++j) {
      if (pCondition->m_conditionType == ParameterChangesVer2[j]) {
        pCondition->m_parms[pCondition->m_numParms] =
            newInstance(Parameter)(Parameter::SURFACES_ALLOWED, 3);
...
//-------------------------------------------------------------------------------------------------
Template::Template()
    : m_numUiStrings(0), m_numParameters(0), m_name("(placeholder)") {}
  ...
  switch (m_paramType) {
    ...
    case KIND_OF_PARAM:
      if (m_int >= KINDOF_FIRST && m_int < KINDOF_COUNT)
        uiText.format("Kind is '%s'",
                      KindOfMaskType::getNameFromSingleBit(m_int));
      else
    ...
    case COMPARISON:
      switch (m_int) {
        case LESS_THAN:
          uiText.format("Less Than");
          break;
        case LESS_EQUAL:
          uiText.format("Less Than or Equal");
          break;
        case EQUAL:
          uiText.format("Equal To");
          break;
        case GREATER_EQUAL:
          uiText.format("Greater Than or Equal To");
          break;
        case GREATER:
          uiText.format("Greater Than");
          break;
        case NOT_EQUAL:
          uiText.format("Not Equal To");
          break;
        default:
          DEBUG_CRASH(("Unknown comparison type."));
      }
      break;
    ...
    case RELATION:
      switch (m_int) {
        case REL_ENEMY:
          ...
          break;
        case REL_NEUTRAL:
          uiText.format("Neutral");
          break;
        case REL_FRIEND:
          uiText.format("Friend");
          break;
        default:
          DEBUG_CRASH(("Unknown Relation type."));
      }
      break;
    ...
    case AI_MOOD:
      switch (m_int) {
        case AI_SLEEP:
          ...
          break;
        case AI_PASSIVE:
          uiText.format("Passive");
          break;
        case AI_NORMAL:
          uiText.format("Normal");
          break;
        case AI_ALERT:
          uiText.format("Alert");
          break;
        case AI_AGGRESSIVE:
          uiText.format("Aggressive");
          break;
        default:
          DEBUG_CRASH(("Unknown AI Mood type."));
      }
      break;
    ...
    case RADAR_EVENT_TYPE:
      switch (m_int) {
        ...
        case RADAR_EVENT_INVALID:
          DEBUG_CRASH(("Invalid radar event\n"));
          uiText.format("Construction");
          break;
        ...
        case RADAR_EVENT_UNDER_ATTACK:
          uiText.format("Under Attack");
          break;
        case RADAR_EVENT_INFORMATION:
          uiText.format("Information");
          break;
        case RADAR_EVENT_INFILTRATION:
          uiText.format("Infiltration");
          break;
        default:
          DEBUG_CRASH(("Unknown Radar event type."));
      }
      break;
    ...
    case COLOR:
      uiText.format(" R:%d G:%d B:%d ", (m_int & 0x00ff0000) >> 16,
                    (m_int & 0x0000ff00) >> 8, (m_int & 0x000000ff));
      break;
    ...
    case BOUNDARY:
      uiText.format("Boundary %s",
                    BORDER_COLORS[m_int % BORDER_COLORS_SIZE].m_colorName);
      break;

    case BUILDABLE:
      if (m_int >= BSTATUS_YES && m_int < BSTATUS_NUM_TYPES)
        uiText.format("Buildable (%s)",
                      BuildableStatusNames[m_int - BSTATUS_YES]);
      else
...
Parameter *Parameter::ReadParameter(DataChunkInput &file) {
  ...
  if (pParm->getParameterType() == UPGRADE) {
    // quick hack to make obsolete capture building upgrades switch to the new
    // one. jba.
    ...
    if (!pParm->m_string.isEmpty()) {
      ...
      for (int i = 0; kindofNames[i]; ++i) {
        ...
        if (!pParm->m_string.compareNoCase("CRUSHER")) {
          ...
          found = true;
          DEBUG_CRASH(
              ("Kindof CRUSHER no longer exists -- in order to get your map to "
               "load, it has been switched to OBSTACLE, please call Kris "
               "(x36844).",
               pParm->m_string.str()));
          break;
          ...
          found = true;
          DEBUG_CRASH(
              ("Kindof CRUSHABLE no longer exists -- in order to get your map "
               "to load, it has been switched to OBSTACLE, please call Kris "
               "(x36844).",
               pParm->m_string.str()));
          break;
          ...
          found = true;
          DEBUG_CRASH(
              ("Kindof OVERLAPPABLE no longer exists -- in order to get your "
               "map to load, it has been switched to OBSTACLE, please call "
               "Kris (x36844).",
               pParm->m_string.str()));
          break;
        } else if (!pParm->m_string.compareNoCase("MISSILE")) {
          // MISSILE was split into two kinds -- SMALL_MISSILE and
          // BALLISTIC_MISSILE.
          ...
          DEBUG_CRASH(
              ("Unable to find Kindof SMALL_MISSILE', please call KrisM "
               "(x36844).",
               pParm->m_string.str()));
        }
      }
      if (!found) {
        DEBUG_CRASH(("Unable to find Kindof '%s', please call JKM (x36872).",
                     pParm->m_string.str()));
        throw ERROR_BUG;
      }
    } else {
...
ScriptAction::ScriptAction(enum ScriptActionType type)
    : m_actionType(type), m_numParms(0) {
  Int i;
  for (i = 0; i < MAX_PARMS; i++) {
    m_parms[i] = NULL;
  }
...
void ScriptAction::setActionType(enum ScriptActionType type) {
  ...
  m_actionType = type;
  const ActionTemplate *pTemplate =
      TheScriptEngine->getActionTemplate(m_actionType);
  m_numParms = pTemplate->getNumParameters();
  for (i = 0; i < m_numParms; i++) {
    m_parms[i] = newInstance(Parameter)(pTemplate->getParameterType(i));
  }
...
ScriptAction *ScriptAction::duplicateAndQualify(
    const AsciiString &qualifier, const AsciiString &playerTemplateName,
    const AsciiString &newPlayerName) const {
  ScriptAction *pNew = newInstance(ScriptAction)(m_actionType);
  Int i;
 ...
 *
 */
void ScriptAction::WriteActionDataChunk(DataChunkOutput &chunkWriter,
                                        ScriptAction *pScriptAction) {
  /**********ACTION  DATA ***********************/
  while (pScriptAction) {
 ...
 *
 */
Bool ScriptAction::ParseActionDataChunk(DataChunkInput &file,
                                        DataChunkInfo *info, void *userData) {
  Script *pScript = (Script *)userData;
  ...
  const ActionTemplate *at =
      TheScriptEngine->getActionTemplate(pScriptAction->m_actionType);
  if (at && (at->getName().isEmpty() ||
             (at->getName().compareNoCase("(placeholder)") == 0))) {
  ...
  switch (pScriptAction->getActionType()) {
    ...
    case TEAM_FOLLOW_WAYPOINTS:
      if (pScriptAction->m_numParms == 2) {
        ...
        pScriptAction->m_parms[2] =
            newInstance(Parameter)(Parameter::BOOLEAN, 1);
      }
      break;
    case SKIRMISH_BUILD_BASE_DEFENSE_FRONT:
      if (pScriptAction->m_numParms == 1) {
        ...
        if (flank)
          pScriptAction->m_actionType = SKIRMISH_BUILD_BASE_DEFENSE_FLANK;
      }
      break;
    case NAMED_SET_ATTITUDE:
    case TEAM_SET_ATTITUDE:
      if (pScriptAction->m_numParms >= 2 &&
          pScriptAction->m_parms[1]->getParameterType() == Parameter::INT) {
        pScriptAction->m_parms[1] = newInstance(Parameter)(
            Parameter::AI_MOOD, pScriptAction->m_parms[1]->getInt());
      }
      break;
 ...
 *
 */
void ScriptAction::WriteActionFalseDataChunk(DataChunkOutput &chunkWriter,
                                             ScriptAction *pScriptAction) {
  /**********ACTION  DATA ***********************/
  while (pScriptAction) {
...
/**
 * ScriptAction::ParseActionFalseDataChunk - read a false action chunk in a
 *script list. Format is the newer CHUNKY format. See
 *ScriptAction::WriteActionDataChunk for the writer. Input: DataChunkInput
 *
 */
Bool ScriptAction::ParseActionFalseDataChunk(DataChunkInput &file,
                                             DataChunkInfo *info,
                                             void *userData) {
  Script *pScript = (Script *)userData;
...
const char *TheObjectFlagsNames[] = {
    ...
    "Selectable", "AI Recruitable", "Player Targetable", NULL,
};
