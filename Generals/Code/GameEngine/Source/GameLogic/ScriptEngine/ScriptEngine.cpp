...
// ///////////////////////////////////////////////////////////////////////// The
// game scripting engine.  Interprets scripts. Author: John Ahlquist, Nov. 2001
///////////////////////////////////////////////////////////////////////////////////////////////////
...
static void _appendMessage(const AsciiString &str, Bool isTrueMessage = true,
...
extern void _updateAsciiStringParmsFromSystem(
    ParticleSystemTemplate *particleTemplate);
extern void _updateAsciiStringParmsToSystem(
    ParticleSystemTemplate *particleTemplate);
...
extern void _writeSingleParticleSystem(
    File *out, ParticleSystemTemplate *particleTemplate);
static void _reloadTextures(void);

static HMODULE st_ParticleDLL;
ParticleSystem *st_particleSystem;
Bool st_particleSystemNeedsStopping =
    FALSE;  ///< Set along with st_particleSystem if the particle system has
            ///< infinite life
...
AttackPriorityInfo::AttackPriorityInfo()
    : m_defaultPriority(ATTACK_PRIORITY_DEFAULT), m_priorityMap(NULL) {
  m_name.clear();
}
...
void AttackPriorityInfo::setPriority(const ThingTemplate *tThing,
  ...
  if (m_priorityMap == NULL) {
    m_priorityMap =
        NEW AttackPriorityMap;  // STL type, so impractical to use memorypool
  }
...
void AttackPriorityInfo::dumpPriorityInfo(void) {
  DEBUG_LOG(
      ("Attack priority '%s', default %d\n", m_name.str(), m_defaultPriority));
  if (m_priorityMap == NULL) return;
  for (AttackPriorityMap::const_iterator it = m_priorityMap->begin();
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AttackPriorityInfo::crc(Xfer *xfer) {}  // end crc
...
void AttackPriorityInfo::xfer(Xfer *xfer) {
  ...
  // commented out in that we're not sure it returns the correct number of
  // elements in the map.  To be safe, we're getting the count by manually
  // counting the elements ourselves as we iterate through them
  //
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    if (m_priorityMap) {
      ...
      for (it = m_priorityMap->begin(); it != m_priorityMap->end(); ++it) {
        ...
        DEBUG_ASSERTCRASH(thingTemplateName.isEmpty() == FALSE,
                          ("AttackPriorityInfo::xfer - Writing an empty thing "
                           "template name\n"));
      ...
      // sanity
      DEBUG_ASSERTCRASH(
          count == priorityMapCount,
          ("AttackPriorityInfo::xfer - Mismatch in priority map size.  Size() "
           "method returned '%d' but actual iteration count was '%d'\n",
    ...
    for (UnsignedShort i = 0; i < priorityMapCount; ++i) {
      ...
      if (thingTemplate == NULL) {
        DEBUG_CRASH(
            ("AttackPriorityInfo::xfer - Unable to find thing template '%s'\n",
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AttackPriorityInfo::loadPostProcess(void) {}  // end loadPostProcess
...
ScriptEngine::~ScriptEngine() {
  ...
  if (st_ParticleDLL) {
    FARPROC proc =
        GetProcAddress(st_ParticleDLL, "DestroyParticleSystemDialog");
    if (proc) {
      proc();
    }
...
void ScriptEngine::init(void) {
  ...
  /* Recipe for adding an action:
                  1. In Scripts.h, add an enum element to enum ScriptActionType
     just before NUM_ITEMS.
                  2. Go to the end of this section of templates, and create a
     template.
                  3. Go to ScriptActions.h and add a protected method.
                  4. Go to ScriptActions.cpp, and add your enum to the
                                  switch in ScriptActions::executeAction to call
     your method in 3 above.
  */
  ...
  curTemplate->m_name =
      "[Scripting] Display a crash box (debug/internal builds only).";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::TEXT_STRING;
  curTemplate->m_numUiStrings = 1;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::SET_INFANTRY_LIGHTING_OVERRIDE];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::RESET_INFANTRY_LIGHTING_OVERRIDE];
  curTemplate->m_name = "[Map] Infantry Lighting - Reset.";
  curTemplate->m_numParameters = 0;
  curTemplate->m_numUiStrings = 1;
  curTemplate->m_uiStrings[0] =
      "Reset infantry lighting to the normal setting. 1.0 for the two day "
      "states, 2.0 for the two night states. (Look in GamesData.ini)";
  ...
  curTemplate->m_name =
      "[Team] AI - Increase priority by Success Priority Increase amount.";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate->m_name =
      "[Team] AI - Reduce priority by Failure Priority Decrease amount.";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::SKIRMISH_MOVE_TO_APPROACH_PATH];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::SKIRMISH_BUILD_BASE_DEFENSE_FRONT];
  curTemplate->m_name =
      "[Skirmish Only] Build base defense on front perimeter.";
  curTemplate->m_numParameters = 0;
  curTemplate->m_numUiStrings = 1;
  curTemplate->m_uiStrings[0] =
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::SKIRMISH_BUILD_BASE_DEFENSE_FLANK];
  curTemplate->m_name =
      "[Skirmish Only] Build base defense on flank perimeter.";
  curTemplate->m_numParameters = 0;
  curTemplate->m_numUiStrings = 1;
  curTemplate->m_uiStrings[0] =
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::SKIRMISH_BUILD_STRUCTURE_FRONT];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::SKIRMISH_BUILD_STRUCTURE_FLANK];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::MOVE_CAMERA_ALONG_WAYPOINT_PATH];
  ...
  curTemplate->m_name =
      "[Scripting] Seconds countdown timer -- subtract seconds.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::REAL;
  curTemplate->m_parameters[1] = Parameter::COUNTER;
  curTemplate->m_numUiStrings = 3;
  ...
  curTemplate->m_name =
      "[Camera mod(RM)] Freeze time during the camera movement.";
  curTemplate->m_numParameters = 0;
  curTemplate->m_numUiStrings = 1;
  ...
  curTemplate->m_name =
      "[Camera mod(M)] Freeze camera angle during the camera movement.";
  curTemplate->m_numParameters = 0;
  curTemplate->m_numUiStrings = 1;
  curTemplate->m_uiStrings[0] =
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::CAMERA_MOD_SET_FINAL_SPEED_MULTIPLIER];
  curTemplate->m_name =
      "[Camera mod(RM)] Final visual game time for camera movement.";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::INT;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::CAMERA_MOD_SET_ROLLING_AVERAGE];
  curTemplate->m_name =
      "[Camera mod(M)] Number of frames to average movements.";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::INT;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::CREATE_NAMED_ON_TEAM_AT_WAYPOINT];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::CREATE_UNNAMED_ON_TEAM_AT_WAYPOINT];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::NAMED_APPLY_ATTACK_PRIORITY_SET];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_APPLY_ATTACK_PRIORITY_SET];
  ...
  curTemplate->m_name =
      "[Attack Priority Set] Modify priority for a single unit type.";
  curTemplate->m_numParameters = 3;
  ...
  curTemplate->m_name =
      "[Attack Priority Set] Modify priorities for all of a kind.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::ATTACK_PRIORITY_SET;
  ...
  curTemplate->m_name =
      "[Player] Grant a Science to a given Player (ignoring prerequisites).";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  ...
  curTemplate->m_name =
      "[Unit] Set a specific unit to attack a specific trigger area.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  ...
  curTemplate->m_name =
      "[Unit] Set a specific unit to EXACTLY follow a waypoint path.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  curTemplate->m_parameters[1] = Parameter::WAYPOINT_PATH;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::PLAYER_DISABLE_BASE_CONSTRUCTION];
  curTemplate->m_name =
      "[Player] Set a player to be unable to build buildings.";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_numUiStrings = 2;
  curTemplate->m_uiStrings[0] = "";
  ...
  curTemplate->m_name =
      "[Player] Set a player to be unable to build from a specific building.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::OBJECT_TYPE;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::PLAYER_DISABLE_UNIT_CONSTRUCTION];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::PLAYER_ENABLE_BASE_CONSTRUCTION];
  ...
  curTemplate->m_name =
      "[Player] Set a player to be able to build from a specific building.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::OBJECT_TYPE;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::PLAYER_ENABLE_UNIT_CONSTRUCTION];
  ...
  curTemplate->m_name =
      "[User] Flash a specific unit for a specified amount of time.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  curTemplate->m_parameters[1] = Parameter::INT;
  curTemplate->m_numUiStrings = 3;
  ...
  curTemplate->m_name =
      "[User] Set a specific unit to use a special indicator color.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  ...
  curTemplate->m_name =
      "[User] Flash a specific unit white for a specified amount of time.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  curTemplate->m_parameters[1] = Parameter::INT;
  curTemplate->m_numUiStrings = 3;
  ...
  curTemplate->m_name =
      "[User] Flash a team white for a specified amount of time.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::INT;
  curTemplate->m_numUiStrings = 3;
  ...
  curTemplate->m_name =
      "[Multimedia] Play a sound as though coming from a specific unit.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::TEXT_STRING;
  curTemplate->m_parameters[1] = Parameter::UNIT;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::PLAYER_TRANSFER_OWNERSHIP_PLAYER];
  curTemplate->m_name =
      "[Player] Transfer assets from one player to another player.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::NAMED_TRANSFER_OWNERSHIP_PLAYER];
  curTemplate->m_name =
      "[Player] Transfer a specific unit to the control of a player.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  curTemplate->m_parameters[1] = Parameter::SIDE;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::PLAYER_EXCLUDE_FROM_SCORE_SCREEN];
  ...
  curTemplate->m_name =
      "[Player] Change how a player relates to another player.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::SIDE;
  ...
  curTemplate->m_name =
      "[Map] Reveal the entire map permanently is un-done for a player.";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_numUiStrings = 2;
  curTemplate->m_uiStrings[0] = "Undo the permanent reveal for ";
  curTemplate->m_uiStrings[1] =
      ".  This will mess things up badly if called when there has been no "
      "permanent reveal.";
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_GARRISON_SPECIFIC_BUILDING];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_GARRISON_NEAREST_BUILDING];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::NAMED_GARRISON_SPECIFIC_BUILDING];
  curTemplate->m_name =
      "[Unit] Garrison a specific building with a specific unit.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  curTemplate->m_parameters[1] = Parameter::UNIT;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::NAMED_GARRISON_NEAREST_BUILDING];
  curTemplate->m_name =
      "[Unit] Garrison a nearby building with a specific unit.";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  curTemplate->m_numUiStrings = 2;
  curTemplate->m_uiStrings[0] = "";
  ...
  curTemplate->m_name =
      "[Player] Garrison as many buildings as player has units for.";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_numUiStrings = 2;
  curTemplate->m_uiStrings[0] = "";
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_AVAILABLE_FOR_RECRUITMENT];
  curTemplate->m_name =
      "[Team] Set whether members of a team can be recruited into another "
      "team.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::BOOLEAN;
  ...
  curTemplate->m_name =
      "[Scripting] All idle Supply Trucks attempt to resume supply routes.";
  curTemplate->m_numParameters = 0;
  curTemplate->m_numUiStrings = 1;
  curTemplate->m_uiStrings[0] =
  ...
  curTemplate->m_name =
      "[Camera] Set the audible distance for camera-up shots.";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::REAL;
  curTemplate->m_numUiStrings = 2;
  curTemplate->m_uiStrings[0] =
  ...
  curTemplate->m_name =
      "[Unit] Set unit to be held in place, ignoring Physics, Locomotors, etc.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  curTemplate->m_parameters[1] = Parameter::BOOLEAN;
  curTemplate->m_numUiStrings = 3;
  ...
  curTemplate->m_name =
      "[Team] Set stopping distance for each unit's current locomotor.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::REAL;
  curTemplate->m_numUiStrings = 3;
  ...
  curTemplate->m_name =
      "[Scripting] Special power countdown display -- disable.";
  curTemplate->m_numParameters = 0;
  curTemplate->m_numUiStrings = 1;
  ...
  curTemplate->m_name =
      "[Scripting] Special power countdown display -- enable.";
  curTemplate->m_numParameters = 0;
  curTemplate->m_numUiStrings = 1;
  curTemplate->m_uiStrings[0] = "Enables special power countdown display.";

  curTemplate =
      &m_actionTemplates[ScriptAction::NAMED_HIDE_SPECIAL_POWER_DISPLAY];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::NAMED_SHOW_SPECIAL_POWER_DISPLAY];
  ...
  curTemplate->m_name =
      "[Scripting] Counter -- display an individual counter to the user.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::COUNTER;
  ...
  curTemplate->m_name =
      "[Scripting] Counter -- hides an individual counter from the user.";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::COUNTER;
  curTemplate->m_numUiStrings = 1;
  ...
  curTemplate->m_name =
      "[Scripting] Timer -- hides an individual timer from the user.";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::COUNTER;
  curTemplate->m_numUiStrings = 1;
  curTemplate->m_uiStrings[0] = "Hide ";
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::DISABLE_COUNTDOWN_TIMER_DISPLAY];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::ENABLE_COUNTDOWN_TIMER_DISPLAY];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::NAMED_STOP_SPECIAL_POWER_COUNTDOWN];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::NAMED_START_SPECIAL_POWER_COUNTDOWN];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::NAMED_SET_SPECIAL_POWER_COUNTDOWN];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::NAMED_ADD_SPECIAL_POWER_COUNTDOWN];
  ...
  curTemplate = &m_actionTemplates
                    [ScriptAction::SKIRMISH_FIRE_SPECIAL_POWER_AT_MOST_COST];
  curTemplate->m_name =
      "[Skirmish] Special power -- fire at enemy's highest cost area.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::SPECIAL_POWER;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::NAMED_FIRE_SPECIAL_POWER_AT_WAYPOINT];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::NAMED_FIRE_SPECIAL_POWER_AT_NAMED];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_SET_OVERRIDE_RELATION_TO_TEAM];
  curTemplate->m_name =
      "[Team] Override a team's relationship to another team.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::TEAM;
  curTemplate->m_parameters[2] = Parameter::RELATION;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_REMOVE_OVERRIDE_RELATION_TO_TEAM];
  curTemplate->m_name =
      "[Team] Remove an override to a team's relationship to another team.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::TEAM;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_REMOVE_ALL_OVERRIDE_RELATIONS];
  curTemplate->m_name =
      "[Team] Remove all overrides to team's relationship to teams and/or "
      "players.";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate->m_name =
      "[Unit] Kill all units contained within a specific transport or "
      "structure.";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  //	curTemplate->m_parameters[1] = Parameter::INT;
  ...
  curTemplate = &m_actionTemplates
                    [ScriptAction::NAMED_FIRE_WEAPON_FOLLOWING_WAYPOINT_PATH];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_SET_OVERRIDE_RELATION_TO_PLAYER];
  curTemplate->m_name =
      "[Team] Override a team's relationship to another player.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::SIDE;
  curTemplate->m_parameters[2] = Parameter::RELATION;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_REMOVE_OVERRIDE_RELATION_TO_PLAYER];
  curTemplate->m_name =
      "[Team] Remove an override to a team's relationship to another player.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::SIDE;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::PLAYER_SET_OVERRIDE_RELATION_TO_TEAM];
  curTemplate->m_name =
      "[Player] Override a player's relationship to another team.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::TEAM;
  curTemplate->m_parameters[2] = Parameter::RELATION;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::PLAYER_REMOVE_OVERRIDE_RELATION_TO_TEAM];
  curTemplate->m_name =
      "[Player] Remove an override to a player's relationship to another team.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::TEAM;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::UNIT_EXECUTE_SEQUENTIAL_SCRIPT];
  curTemplate->m_name =
      "[Unit] Set a specific unit to execute a script sequentially.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::UNIT_EXECUTE_SEQUENTIAL_SCRIPT_LOOPING];
  curTemplate->m_name =
      "[Unit] Set a specific unit to execute a looping sequential script.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  curTemplate->m_parameters[1] = Parameter::SCRIPT;
  curTemplate->m_parameters[2] = Parameter::INT;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_EXECUTE_SEQUENTIAL_SCRIPT];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_EXECUTE_SEQUENTIAL_SCRIPT_LOOPING];
  ...
  curTemplate->m_name =
      "[Unit] Set a specific unit to stop executing a sequential script.";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  curTemplate->m_numUiStrings = 2;
  curTemplate->m_uiStrings[0] = "";
  ...
  curTemplate->m_name =
      "[Map] Adjust water height to a new level with damage over time";
  curTemplate->m_numParameters = 4;
  curTemplate->m_parameters[0] = Parameter::TRIGGER_AREA;
  curTemplate->m_parameters[1] = Parameter::REAL;
  curTemplate->m_parameters[2] = Parameter::REAL;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::NAMED_USE_COMMANDBUTTON_ABILITY];
  ...
  curTemplate = &m_actionTemplates
                    [ScriptAction::NAMED_USE_COMMANDBUTTON_ABILITY_ON_NAMED];
  ...
  curTemplate = &m_actionTemplates
                    [ScriptAction::NAMED_USE_COMMANDBUTTON_ABILITY_AT_WAYPOINT];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_USE_COMMANDBUTTON_ABILITY];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_USE_COMMANDBUTTON_ABILITY_ON_NAMED];
  ...
  curTemplate = &m_actionTemplates
                    [ScriptAction::TEAM_USE_COMMANDBUTTON_ABILITY_AT_WAYPOINT];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TECHTREE_MODIFY_BUILDABILITY_OBJECT];
  curTemplate->m_name =
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::UNIT_MOVE_TOWARDS_NEAREST_OBJECT_TYPE];
  curTemplate->m_name =
      "[Unit] Move unit towards the nearest object of a specific type.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  curTemplate->m_parameters[1] = Parameter::OBJECT_TYPE;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_MOVE_TOWARDS_NEAREST_OBJECT_TYPE];
  curTemplate->m_name =
      "[Team] Move team towards the nearest object of a specific type.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::OBJECT_TYPE;
  curTemplate->m_parameters[2] = Parameter::TRIGGER_AREA;
  ...
  curTemplate = &m_actionTemplates
                    [ScriptAction::SKIRMISH_ATTACK_NEAREST_GROUP_WITH_VALUE];
  curTemplate->m_name =
      "[Skirmish] Team attacks nearest group matching value comparison.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  ...
  curTemplate->m_name =
      "[Skirmish] Team performs command ability on most valuable object.";
  curTemplate->m_numParameters = 4;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::COMMANDBUTTON_ALL_ABILITIES;
  curTemplate->m_parameters[2] = Parameter::REAL;
  ...
  curTemplate =
      &m_actionTemplates
          [ScriptAction::SKIRMISH_WAIT_FOR_COMMANDBUTTON_AVAILABLE_ALL];
  curTemplate->m_name =
      "[Skirmish] Delay a sequential script until the specified command "
      "ability is ready - all.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::TEAM;
  ...
  curTemplate->m_name =
      "[Team] Set to continue current action for some number of frames.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::INT;
  curTemplate->m_numUiStrings = 3;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_ALL_USE_COMMANDBUTTON_ON_NAMED];
  ...
  curTemplate =
      &m_actionTemplates
          [ScriptAction::TEAM_ALL_USE_COMMANDBUTTON_ON_NEAREST_ENEMY_UNIT];
  curTemplate->m_name =
      "[Team] Use command ability -- all -- nearest enemy unit";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::COMMANDBUTTON_ALL_ABILITIES;
  curTemplate->m_numUiStrings = 3;
  ...
  curTemplate->m_name =
      "[Team] Use command ability -- all -- nearest enemy garrisoned building.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::COMMANDBUTTON_ALL_ABILITIES;
  curTemplate->m_numUiStrings = 3;
  ...
  curTemplate =
      &m_actionTemplates
          [ScriptAction::TEAM_ALL_USE_COMMANDBUTTON_ON_NEAREST_KINDOF];
  curTemplate->m_name =
      "[Team] Use command ability -- all -- nearest enemy object with kind of.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::COMMANDBUTTON_ALL_ABILITIES;
  ...
  curTemplate =
      &m_actionTemplates
          [ScriptAction::TEAM_ALL_USE_COMMANDBUTTON_ON_NEAREST_ENEMY_BUILDING];
  curTemplate->m_name =
      "[Team] Use command ability -- all -- nearest enemy building.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::COMMANDBUTTON_ALL_ABILITIES;
  curTemplate->m_numUiStrings = 3;
  ...
  curTemplate->m_name =
      "[Team] Use command ability -- all -- nearest enemy building kindof.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::COMMANDBUTTON_ALL_ABILITIES;
  curTemplate->m_parameters[2] = Parameter::KIND_OF_PARAM;
  ...
  curTemplate =
      &m_actionTemplates
          [ScriptAction::TEAM_ALL_USE_COMMANDBUTTON_ON_NEAREST_OBJECTTYPE];
  curTemplate->m_name =
      "[Team] Use command ability -- all -- nearest object type.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::COMMANDBUTTON_ALL_ABILITIES;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_PARTIAL_USE_COMMANDBUTTON];
  ...
  curTemplate = &m_actionTemplates
                    [ScriptAction::TEAM_CAPTURE_NEAREST_UNOWNED_FACTION_UNIT];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::PLAYER_CREATE_TEAM_FROM_CAPTURED_UNITS];
  ...
  curTemplate->m_uiStrings[2] =
      " from units it has captured. (There's nothing quite like being "
      "assaulted by your own captured units!)";

  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_WAIT_FOR_NOT_CONTAINED_ALL];
  curTemplate->m_name =
      "[Team] Delay a sequential script until the team is no longer contained "
      "- all";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_WAIT_FOR_NOT_CONTAINED_PARTIAL];
  curTemplate->m_name =
      "[Team] Delay a sequential script until the team is no longer contained "
      "- partial";
  curTemplate->m_numParameters = 1;
  ...
  curTemplate->m_name =
      "[Team] Set emoticon for duration (-1.0 permanent, otherwise duration in "
      "sec).";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  ...
  curTemplate->m_name =
      "[Unit] Set emoticon for duration (-1.0 permanent, otherwise duration in "
      "sec).";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  curTemplate->m_parameters[1] = Parameter::EMOTICON;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::MAP_REVEAL_PERMANENTLY_AT_WAYPOINT];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::MAP_UNDO_REVEAL_PERMANENTLY_AT_WAYPOINT];
  ...
  curTemplate->m_name =
      "[Scripting] Enable or Disable Occlusion (Drawing Behind Buildings).";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::BOOLEAN;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::UNIT_AFFECT_OBJECT_PANEL_FLAGS];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::TEAM_AFFECT_OBJECT_PANEL_FLAGS];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::SCRIPTING_OVERRIDE_HULK_LIFETIME];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::COMMANDBAR_REMOVE_BUTTON_OBJECTTYPE];
  curTemplate->m_name =
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::COMMANDBAR_ADD_BUTTON_OBJECTTYPE_SLOT];
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::UNIT_SPAWN_NAMED_LOCATION_ORIENTATION];
  curTemplate->m_name =
      "[Unit] Spawn -- named unit on a team at a position with an orientation.";
  curTemplate->m_numParameters = 5;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  curTemplate->m_parameters[1] = Parameter::OBJECT_TYPE;
  curTemplate->m_parameters[2] = Parameter::TEAM;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::PLAYER_AFFECT_RECEIVING_EXPERIENCE];
  curTemplate->m_name =
      "[Player] Change the modifier to generals experience that a player "
      "receives.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::REAL;
  ...
  curTemplate =
      &m_actionTemplates[ScriptAction::CHOOSE_VICTIM_ALWAYS_USES_NORMAL];
  curTemplate->m_name =
      "[Map] Force ChooseVictim to ignore game difficulty and always use "
      "Normal setting.";
  curTemplate->m_numParameters = 1;
  curTemplate->m_parameters[0] = Parameter::BOOLEAN;
  curTemplate->m_numUiStrings = 2;
  curTemplate->m_uiStrings[0] =
      "Force ChooseVictim to ignore game difficulty and always use Normal "
      "setting ";
  curTemplate->m_uiStrings[1] = " (true to enable, false to disable).";
  ...
  /* Recipe for adding a condition:
                  1. In Scripts.h, add an enum element to enum ConditionType
     just before NUM_ITEMS.
                  2. Go to the end of this section of templates, and create a
     template.
                  3. Go to ScriptConditions.h and add a protected method.
                  4. Go to ScriptConditions.cpp, and add your enum to the
                                  switch in ScriptConditions::evaluateCondition
     to call your method in 3 above.
  */
  ...
  curTemplate->m_name =
      "[Player] Player has (comparison) to a number of credits.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::INT;
  curTemplate->m_parameters[1] = Parameter::COMPARISON;
  curTemplate->m_parameters[2] = Parameter::SIDE;
  ...
  curTemplate->m_name =
      "[Unit] Unit has sighted a(n) friendly/neutral/enemy unit belonging to a "
      "side.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  ...
  curTemplate->m_name =
      "[Unit] Unit has sighted a type of unit belonging to a side.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  curTemplate->m_parameters[1] = Parameter::OBJECT_TYPE;
  curTemplate->m_parameters[2] = Parameter::SIDE;
  ...
  curTemplate->m_name =
      "[Player] Player has attempted the mission a number of times.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::COMPARISON;
  curTemplate->m_parameters[2] = Parameter::INT;
  ...
  curTemplate =
      &m_conditionTemplates[Condition::PLAYER_HAS_N_OR_FEWER_BUILDINGS];
  ...
  curTemplate =
      &m_conditionTemplates[Condition::PLAYER_HAS_N_OR_FEWER_FACTION_BUILDINGS];
  curTemplate->m_name =
      "[Player] Player currently owns N or fewer faction buildings.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::INT;
  curTemplate->m_numUiStrings = 3;
  ...
  curTemplate->m_name =
      "[Unit] Unit has reached the end of a specific waypoint path.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::UNIT;
  curTemplate->m_parameters[1] = Parameter::WAYPOINT_PATH;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate->m_name =
      "[Team] Team has reached the end of a specific waypoint path.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::TEAM;
  curTemplate->m_parameters[1] = Parameter::WAYPOINT_PATH;
  curTemplate->m_numUiStrings = 2;
  ...
  curTemplate->m_numUiStrings = 1;
  curTemplate->m_uiStrings[0] =
      "The multiplayer game has ended in victory for the local player and his "
      "allies.";
  ...
  curTemplate->m_numUiStrings = 1;
  curTemplate->m_uiStrings[0] =
      "The multiplayer game has ended in defeat for the local player and his "
      "allies.";
  ...
  curTemplate->m_numUiStrings = 1;
  curTemplate->m_uiStrings[0] =
      "Everything belonging to the local player has been destroyed, but his "
      "allies may or may not have been defeated.";
  ...
  curTemplate =
      &m_conditionTemplates[Condition::PLAYER_TRIGGERED_SPECIAL_POWER];
  ...
  curTemplate = &m_conditionTemplates
                    [Condition::PLAYER_TRIGGERED_SPECIAL_POWER_FROM_NAMED];
  curTemplate->m_name =
      "[Player] Player start using a special power from a named unit.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::SPECIAL_POWER;
  curTemplate->m_parameters[2] = Parameter::UNIT;
  ...
  curTemplate->m_name =
      "[Player] Player is midway through using a special power.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::SPECIAL_POWER;
  curTemplate->m_numUiStrings = 3;
  ...
  curTemplate =
      &m_conditionTemplates[Condition::PLAYER_MIDWAY_SPECIAL_POWER_FROM_NAMED];
  curTemplate->m_name =
      "[Player] Player is midway through using a special power from a named "
      "unit.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::SPECIAL_POWER;
  ...
  curTemplate =
      &m_conditionTemplates[Condition::PLAYER_COMPLETED_SPECIAL_POWER];
  ...
  curTemplate = &m_conditionTemplates
                    [Condition::PLAYER_COMPLETED_SPECIAL_POWER_FROM_NAMED];
  curTemplate->m_name =
      "[Player] Player completed using a special power from a named unit.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::SPECIAL_POWER;
  curTemplate->m_parameters[2] = Parameter::UNIT;
  ...
  curTemplate->m_name =
      "[Player] Player can purchase a particular Science (has all prereqs & "
      "points).";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::SCIENCE;
  ...
  curTemplate =
      &m_conditionTemplates[Condition::PLAYER_HAS_SCIENCEPURCHASEPOINTS];
  curTemplate->m_name =
      "[Player] Player has a certain number of Science Purchase Points "
      "available.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::INT;
  ...
  curTemplate =
      &m_conditionTemplates[Condition::PLAYER_BUILT_UPGRADE_FROM_NAMED];
  ...
  curTemplate =
      &m_conditionTemplates[Condition::PLAYER_DESTROYED_N_BUILDINGS_PLAYER];
  curTemplate->m_name =
      "[Player] Player destroyed N or more of an opponent's buildings.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::INT;
  curTemplate->m_parameters[2] = Parameter::SIDE;
  ...
  curTemplate =
      &m_conditionTemplates
          [Condition::PLAYER_HAS_COMPARISON_UNIT_TYPE_IN_TRIGGER_AREA];
  curTemplate->m_name =
  ...
  curTemplate =
      &m_conditionTemplates
          [Condition::PLAYER_HAS_COMPARISON_UNIT_KIND_IN_TRIGGER_AREA];
  curTemplate->m_name =
      "[Player] Player has (comparison) kind of unit or structure in an area.";
  curTemplate->m_numParameters = 5;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::COMPARISON;
  curTemplate->m_parameters[2] = Parameter::INT;
  ...
  curTemplate->m_name =
      "[Player] Player has (comparison) percent power supply to consumption.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::COMPARISON;
  curTemplate->m_parameters[2] = Parameter::INT;
  ...
  curTemplate =
      &m_conditionTemplates[Condition::PLAYER_EXCESS_POWER_COMPARE_VALUE];
  curTemplate->m_name =
      "[Player] Player has (comparison) kilowatts excess power supply.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::COMPARISON;
  curTemplate->m_parameters[2] = Parameter::INT;
  ...
  curTemplate =
      &m_conditionTemplates[Condition::SKIRMISH_SUPPLIES_VALUE_WITHIN_DISTANCE];
  curTemplate->m_name =
      "[Skirmish Only] Supplies are within specified distance.";
  curTemplate->m_numParameters = 4;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::REAL;
  curTemplate->m_parameters[2] = Parameter::TRIGGER_AREA;
  ...
  curTemplate =
      &m_conditionTemplates[Condition::SKIRMISH_TECH_BUILDING_WITHIN_DISTANCE];
  curTemplate->m_name =
      "[Skirmish Only] Tech building is within specified distance.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::REAL;
  curTemplate->m_parameters[2] = Parameter::TRIGGER_AREA;
  ...
  curTemplate =
      &m_conditionTemplates[Condition::SKIRMISH_COMMAND_BUTTON_READY_ALL];
  ...
  curTemplate =
      &m_conditionTemplates[Condition::SKIRMISH_COMMAND_BUTTON_READY_PARTIAL];
  ...
  curTemplate =
      &m_conditionTemplates[Condition::SKIRMISH_UNOWNED_FACTION_UNIT_EXISTS];
  ...
  curTemplate = &m_conditionTemplates
                    [Condition::SKIRMISH_PLAYER_HAS_PREREQUISITE_TO_BUILD];
  curTemplate->m_name =
      "[Skirmish] Player has prerequisites to build an object type.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::OBJECT_TYPE;
  curTemplate->m_numUiStrings = 3;
  ...
  curTemplate = &m_conditionTemplates
                    [Condition::SKIRMISH_PLAYER_HAS_COMPARISON_GARRISONED];
  curTemplate->m_name =
      "[Skirmish] Player has garrisoned buildings -- comparison.";
  curTemplate->m_numParameters = 3;
  curTemplate->m_parameters[0] = Parameter::SIDE;
  curTemplate->m_parameters[1] = Parameter::COMPARISON;
  curTemplate->m_parameters[2] = Parameter::INT;
  ...
  curTemplate = &m_conditionTemplates
                    [Condition::SKIRMISH_PLAYER_HAS_COMPARISON_CAPTURED_UNITS];
  ...
  curTemplate =
      &m_conditionTemplates[Condition::SKIRMISH_PLAYER_HAS_UNITS_IN_AREA];
  ...
  curTemplate = &m_conditionTemplates
                    [Condition::SKIRMISH_PLAYER_HAS_BEEN_ATTACKED_BY_PLAYER];
  ...
  curTemplate =
      &m_conditionTemplates[Condition::SKIRMISH_PLAYER_IS_OUTSIDE_AREA];
  ...
  curTemplate =
      &m_conditionTemplates[Condition::SKIRMISH_PLAYER_HAS_DISCOVERED_PLAYER];
  ...
  curTemplate->m_name =
      "[Multimedia] Music track has completed some number of times.";
  curTemplate->m_numParameters = 2;
  curTemplate->m_parameters[0] = Parameter::MUSIC;
  curTemplate->m_parameters[1] = Parameter::INT;
  curTemplate->m_numUiStrings = 3;
  ...
  curTemplate->m_uiStrings[2] =
      " times. (NOTE: This can only be used to "
      "start other music. USING THIS SCRIPT IN ANY OTHER WAY WILL CAUSE "
      "REPLAYS TO NOT WORK.)";
...
void ScriptEngine::reset(void) {
  ...
  if (m_numFrames > 1) {
    ...
    DEBUG_LOG(
        ("  Max time to update %.3f miliseconds.\n", m_maxUpdateTime * 1000));
  }
  m_numFrames = 0;
  m_totalUpdateTime = 0;
  m_maxUpdateTime = 0;
  ...
  if (TheSidesList) {
    for (numToDump = 0; numToDump < 10; numToDump++) {
      ...
      for (i = 0; i < TheSidesList->getNumSides(); i++) {
        ...
        ScriptGroup *pGroup;
        for (pGroup = pSL->getScriptGroup(); pGroup;
      ...
      if (maxScript) {
        DEBUG_LOG(
            ("   SCRIPT %s total time %f seconds,\n        evaluated %d times, "
             "avg execution %2.3f msec (Goal less than 0.05)\n",
             maxScript->getName().str(), maxScript->getConditionTime(),
             maxScript->getConditionCount(),
             1000 * maxScript->getConditionTime() /
                 maxScript->getConditionCount()));
  ...
  for (seqScriptIt = m_sequentialScripts.begin();
       seqScriptIt != m_sequentialScripts.end();) {
    cleanupSequentialScript(seqScriptIt, TRUE);
  }

  // clear out all the lists of object types that were in the old map.
  for (AllObjectTypesIt it = m_allObjectTypeLists.begin();
       it != m_allObjectTypeLists.end(); it = m_allObjectTypeLists.begin()) {
    if (*it) {
  ...
  DEBUG_ASSERTCRASH(m_allObjectTypeLists.empty() == TRUE,
                    ("ScriptEngine::reset - m_allObjectTypeLists should be "
                     "empty but is not!\n"));
  ...
  for (i = 0; i < MAX_ATTACK_PRIORITIES; ++i) m_attackPriorityInfo[i].reset();

  // clear out all of our object counts.
  for (i = 0; i < MAX_PLAYER_COUNT; ++i) m_objectCounts[i].clear();
...
void ScriptEngine::update(void) {
  ...
  QueryPerformanceFrequency(
      (LARGE_INTEGER *)&freq64);  // LORENZEN'S NOTE_TO_SELF: USE THIS
  QueryPerformanceCounter(
      (LARGE_INTEGER *)&startTime64);  // LORENZEN'S NOTE_TO_SELF: USE THIS
/* dump out the named objects table.  For extremely intense debug only.  jba. :P
        for (VecNamedRequestsIt it = m_namedObjects.begin(); it !=
   m_namedObjects.end(); ++it) { AsciiString name = it->first; Object * obj =
   it->second; if (obj && obj->getAIUpdateInterface()) DEBUG_LOG(("%s=%x('%s'),
   isDead%d\n", name.str(), obj, obj->getName().str(),
   obj->getAIUpdateInterface()->isDead()));
        }
        DEBUG_LOG(("\n\n"));
*/
#endif
  ...
  if (m_closeWindowTimer > 0) {
    ...
    if (m_closeWindowTimer < 1) {
      TheScriptActions->closeWindows(
          FALSE);  // Close victory or defeat windows.
    }
  }
  if (m_endGameTimer > 0) {
    m_endGameTimer--;
    if (m_endGameTimer < 1) {
      ...
      // TheScriptActions->closeWindows(FALSE); // Close victory or defeat
      // windows.
    }
  }
  _updateFrameNumber();
  if (isTimeFrozenDebug()) {
    st_LastCurrentFrame =
        st_CurrentFrame -
        1;  // Force us to get clean result from CanAppContinue
    return;
  }
...
#ifdef DEBUG_LOGGING
  QueryPerformanceCounter(
      (LARGE_INTEGER *)&endTime64);  // LORENZEN'S NOTE_TO_SELF: USE THIS
  timeToUpdate = ((double)(endTime64 - startTime64) /
                  (double)(freq64));  // LORENZEN'S NOTE_TO_SELF: USE THIS
  m_numFrames++;
...
/** getStats */
//-------------------------------------------------------------------------------------------------
AsciiString ScriptEngine::getStats(Real *curTimePtr, Real *script1Time,
                                   Real *script2Time) {
  *curTimePtr = 0;
  *script1Time = 0;
  *script2Time = 0;
  ...
  if (m_currentPlayer) {
    ...
    if (enemy == NULL) {
      ...
      for (i = 0; i < ThePlayerList->getPlayerCount(); i++) {
        ...
        if (/*enemy->isLocalPlayer() &&*/ enemy->getPlayerType() ==
            PLAYER_HUMAN) {
          return enemy;
        }
        enemy = NULL;
...
//-------------------------------------------------------------------------------------------------
Player *ScriptEngine::getPlayerFromAsciiString(
    const AsciiString &playerString) {
  if (playerString == LOCAL_PLAYER) return ThePlayerList->getLocalPlayer();
...
ObjectTypes *ScriptEngine::getObjectTypes(const AsciiString &objectTypeList) {
  ...
  for (it = m_allObjectTypeLists.begin(); it != m_allObjectTypeLists.end();
       ++it) {
    if ((*it) == NULL) {
...
//-------------------------------------------------------------------------------------------------
/** Given a name, return the associated trigger area, or NULL if one doesn't
exist. Handles skirmish name qualification.  */
//-------------------------------------------------------------------------------------------------
...
Team *ScriptEngine::getTeamNamed(const AsciiString &teamName) {
  if (teamName == THIS_TEAM) {
    if (m_callingTeam) return m_callingTeam;
  ...
  if (theTeamProto->countTeamInstances() > 1) {
    if (warnCount < 10) {
      warnCount++;
      AppendDebugMessage(
          "***Referencing multiple team by unspecific instance:***", false);
...
Object *ScriptEngine::getUnitNamed(const AsciiString &unitName) {
  ...
  for (VecNamedRequestsIt it = m_namedObjects.begin();
...
//-------------------------------------------------------------------------------------------------
/** runScript - Executes a subroutine script, or script group - tests
 * conditions, and executes actions or false actions.  */
//-------------------------------------------------------------------------------------------------
void ScriptEngine::runScript(const AsciiString &scriptName, Team *pThisTeam) {
  ...
  if (pGroup) {
    if (pGroup->isSubroutine()) {
      ...
      AppendDebugMessage(
          "***Attempting to call script that is not a subroutine:***", false);
...
Int ScriptEngine::allocateCounter(const AsciiString &name) {
  ...
  DEBUG_ASSERTCRASH(m_numCounters < MAX_COUNTERS,
                    ("Too many counters, failed to make '%s'.\n", name.str()));
...
void ScriptEngine::createNamedMapReveal(const AsciiString &revealName,
  ...
  for (it = m_namedReveals.begin(); it != m_namedReveals.end(); ++it) {
    if (it->m_revealName == revealName) {
      DEBUG_CRASH(
          ("ScriptEngine::createNamedMapReveal: Attempted to redefine named "
           "Reveal '%s', so I won't change it.\n",
           revealName.str()));
      return;
    }
  }
...
void ScriptEngine::doNamedMapReveal(const AsciiString &revealName) {
  ...
  ThePartitionManager->doShroudReveal(pos.x, pos.y, reveal->m_radiusToReveal,
                                      player->getPlayerMask());
}
...
void ScriptEngine::undoNamedMapReveal(const AsciiString &revealName) {
  ...
  ThePartitionManager->undoShroudReveal(pos.x, pos.y, reveal->m_radiusToReveal,
                                        player->getPlayerMask());
}
...
Int ScriptEngine::allocateFlag(const AsciiString &name) {
  ...
  DEBUG_ASSERTCRASH(m_numFlags < MAX_FLAGS,
                    ("Too many flags, failed to make '%s'..\n", name.str()));
...
Bool ScriptEngine::evaluateCounter(Condition *pCondition) {
  DEBUG_ASSERTCRASH(pCondition->getNumParameters() >= 3,
                    ("Not enough parameters.\n"));
  DEBUG_ASSERTCRASH(pCondition->getConditionType() == Condition::COUNTER,
                    ("Wrong condition.\n"));
  ...
  switch (pCondition->getParameter(1)->getInt()) {
    case Parameter::LESS_THAN:
      return m_counters[counterNdx].value < value;
...
void ScriptEngine::setCounter(ScriptAction *pAction) {
  DEBUG_ASSERTCRASH(pAction->getNumParameters() >= 2,
                    ("Not enough parameters.\n"));
...
void ScriptEngine::setFade(ScriptAction *pAction) {
  ...
  DEBUG_ASSERTCRASH(pAction->getNumParameters() >= 5,
                    ("Not enough parameters.\n"));
...
void ScriptEngine::setSway(ScriptAction *pAction) {
  ...
  if (m_breezeInfo.m_breezePeriod < 1) m_breezeInfo.m_breezePeriod = 1;
...
Bool ScriptEngine::evaluateFlag(Condition *pCondition) {
  DEBUG_ASSERTCRASH(pCondition->getNumParameters() >= 2,
                    ("Not enough parameters.\n"));
  DEBUG_ASSERTCRASH(pCondition->getConditionType() == Condition::FLAG,
                    ("Wrong condition.\n"));
  ...
  for (ListAsciiStringIt it = m_uiInteractions.begin();
       it != m_uiInteractions.end(); ++it) {
    if (it->compare(pCondition->getParameter(0)->getString()) == 0) {
      // just return. This flag will be cleared up at the end of the
      // ScriptEngine::update() call
      return true;
    }
  }
  return false;
...
//-------------------------------------------------------------------------------------------------
/** Returns the named attack info, if non-existent returns default attack
   priority info. Never returns null. */
//-------------------------------------------------------------------------------------------------
...
void ScriptEngine::setPriorityThing(ScriptAction *pAction) {
  DEBUG_ASSERTCRASH(pAction->getNumParameters() >= 3,
                    ("Not enough parameters.\n"));
  ...
  if (!types) {
    ...
    if (thingTemplate == NULL) {
      AppendDebugMessage(
          "***Attempting to set attack priority on an invalid thing:***",
          false);
      AppendDebugMessage(pAction->getParameter(0)->getString(), false);
      return;
    }
    AttackPriorityInfo *info =
        findAttackInfo(pAction->getParameter(0)->getString(), true);
    if (info == NULL) {
      AppendDebugMessage(
          "***Error allocating attack priority set - fix or raise limit. ***",
          false);
      return;
    }
    // DEBUG LOGGING
    ...
    for (Int typeIndex = 0; typeIndex < types->getListSize(); typeIndex++) {
      ...
      const ThingTemplate *thisType =
          TheThingFactory->findTemplate(thisTypeName);
      if (thisType == NULL) {
        AppendDebugMessage(
            "***Attempting to set attack priority on an invalid thing:***",
            false);
      ...
      //			debug.concat(" to ");
      //			char intBuffer[16];
      //			itoa(pAction->getParameter(2)->getInt(),
      //intBuffer, 10); 			debug.concat(intBuffer); 			AppendDebugMessage(debug,
      //false);
      // END DEBUGGING LOGGING
...
void ScriptEngine::setPriorityKind(ScriptAction *pAction) {
  ...
  for (tTemplate = TheThingFactory->firstTemplate(); tTemplate;
...
//-------------------------------------------------------------------------------------------------
/** Removes an object types list from the list owned by the script engine, and
 * then deletes the */
/**	associated item. */
//-------------------------------------------------------------------------------------------------
void ScriptEngine::removeObjectTypes(ObjectTypes *typesToRemove) {
  ...
  AllObjectTypesIt it = std::find(m_allObjectTypeLists.begin(),
                                  m_allObjectTypeLists.end(), typesToRemove);
...
Bool ScriptEngine::evaluateTimer(Condition *pCondition) {
  DEBUG_ASSERTCRASH(pCondition->getNumParameters() >= 1,
                    ("Not enough parameters.\n"));
  DEBUG_ASSERTCRASH(pCondition->getConditionType() == Condition::TIMER_EXPIRED,
                    ("Wrong condition.\n"));
  Int counterNdx = pCondition->getParameter(0)->getInt();
  if (counterNdx == 0) {
    counterNdx = allocateCounter(pCondition->getParameter(0)->getString());
...
/** Starts a timer. */
//-------------------------------------------------------------------------------------------------
void ScriptEngine::setTimer(ScriptAction *pAction, Bool millisecondTimer,
                            Bool random) {
  DEBUG_ASSERTCRASH(pAction->getNumParameters() >= 2,
                    ("Not enough parameters.\n"));
  Int counterNdx = pAction->getParameter(0)->getInt();
  ...
  if (millisecondTimer) {
    ...
    m_counters[counterNdx].value =
        REAL_TO_INT_CEIL(ConvertDurationFromMsecsToFrames(value * 1000));
  } else {
    Int value = pAction->getParameter(1)->getInt();
    if (random) {
...
void ScriptEngine::pauseTimer(ScriptAction *pAction) {
  DEBUG_ASSERTCRASH(pAction->getNumParameters() >= 1,
                    ("Not enough parameters.\n"));
  Int counterNdx = pAction->getParameter(0)->getInt();
  if (counterNdx == 0) {
    counterNdx = allocateCounter(pAction->getParameter(0)->getString());
...
/** adjusts a timer. */
//-------------------------------------------------------------------------------------------------
void ScriptEngine::adjustTimer(ScriptAction *pAction, Bool millisecondTimer,
                               Bool add) {
  DEBUG_ASSERTCRASH(pAction->getNumParameters() >= 2,
                    ("Not enough parameters.\n"));
  Int counterNdx = pAction->getParameter(1)->getInt();
...
//-------------------------------------------------------------------------------------------------
/** Checks to see if any teams are referenced in the conditions, so we can
properly iterate over multiple teams. */
//-------------------------------------------------------------------------------------------------
void ScriptEngine::checkConditionsForTeamNames(Script *pScript) {
  ...
  if (pScript->getDelayEvalSeconds() > 0) {
    ...
    pScript->setFrameToEvaluate(
        GameLogicRandomValue(0, 2 * LOGICFRAMES_PER_SECOND));
  } else {
  ...
  for (pOr = pScript->getOrCondition(); pOr; pOr = pOr->getNextOrCondition()) {
    ...
    for (pCondition = pOr->getFirstAndCondition(); pCondition;
         pCondition = pCondition->getNext()) {
      Int i;
      for (i = 0; i < pCondition->getNumParameters(); i++) {
        if (Parameter::TEAM ==
            pCondition->getParameter(i)->getParameterType()) {
          ...
          if (singleton) {
            singletonTeamName =
                teamName;  // Singleton team - use if it is the only one, but
            ...
            if (multiTeamName.isEmpty()) {
              ...
              AppendDebugMessage(
                  "***WARNING: Script contains multiple non-singleton team "
                  "conditions::***",
                  false);
              AppendDebugMessage(scriptName, false);
              ...
              AppendDebugMessage(teamName, false);
              DEBUG_LOG(
                  ("WARNING: Script '%s' contains multiple non-singleton team "
                   "conditions: %s & %s.\n",
                   scriptName.str(), multiTeamName.str(), teamName.str()));
            }
          }
        }
      }
...
void ScriptEngine::executeScript(Script *pScript) {
  ...
  switch (difficulty) {
    case DIFFICULTY_EASY:
      if (!pScript->isEasy()) return;
      break;
    case DIFFICULTY_NORMAL:
      if (!pScript->isNormal()) return;
      break;
    case DIFFICULTY_HARD:
      if (!pScript->isHard()) return;
      break;
  }
  ...
  if (delaySeconds > 0) {
    pScript->setFrameToEvaluate(TheGameLogic->getFrame() +
  ...
  if (pProto && pProto->countTeamInstances() > 0) {
    // We have a team referred to in the conditions.  Iterate over the instances
    // of the team, applying the script conditions (and possibly actions) to
    // each instance of the team.
    for (DLINK_ITERATOR<Team> iter = pProto->iterate_TeamInstanceList();
  ...
  for (VecNamedRequestsIt it = m_namedObjects.begin();
    ...
    if (it->first == objName) {
      if (it->second == NULL) {
        ...
        newNameForDead.format(
            "Reassigning dead object's name '%s' to object (%d) of type '%s'\n",
            objName.str(), pNewObject->getID(),
            pNewObject->getTemplate()->getName().str());
        ...
        DEBUG_CRASH(
            ("Attempting to assign the name '%s' to object (%d) of type '%s',"
             " but object (%d) of type '%s' already has that name\n",
             objName.str(), pNewObject->getID(),
             pNewObject->getTemplate()->getName().str(), it->second->getID(),
             it->second->getTemplate()->getName().str()));
        return;
      }
    }
...
void ScriptEngine::removeObjectFromCache(Object *pDeadObject) {
  for (VecNamedRequestsIt it = m_namedObjects.begin();
    ...
    if (pDeadObject == (it->second)) {
      it->second = NULL;  // Don't remove it, cause we want to check whether we
                          // ever knew a name later
      break;
    }
  }
}
...
//-------------------------------------------------------------------------------------------------
/** Kris:
                Looks for existing cached object with same name and replaces
   that object point with the supplied one. This is an important feature for
   units that change into something else. Good examples include terrorists
                entering a vehicle to convert it to a carbomb, pilots adding
   veterancy to vehicles, hijackers stealing vehicles, and infantry taking over
   disabled vehicles.
*/
//-------------------------------------------------------------------------------------------------
void ScriptEngine::transferObjectName(const AsciiString &unitName,
  ...
  // John Ahlquist: When transferring an object name, make sure the new object
  // isn't already in 							 the vector. If so, remove it, or it'll end up there twice
  //and cause a crash.
  ...
  // Loop through the cached list and find the string entry. If found, change
  // the object so it's pointing to the new one.
  for (VecNamedRequestsIt it = m_namedObjects.begin();
       it != m_namedObjects.end(); ++it) {
    if (!unitName.compare(it->first)) {
      Object *pOldObj = it->second;
      if (pOldObj) {
        // if you are transferring your name, you should also transfer any
        // custom indicator color you have.
...
//-------------------------------------------------------------------------------------------------
void ScriptEngine::notifyOfTriggeredSpecialPower(
    Int playerIndex, const AsciiString &completedPower, ObjectID sourceObj) {
  m_triggeredSpecialPowers[playerIndex].push_back(
      AsciiStringObjectIDPair(completedPower, sourceObj));
}
...
void ScriptEngine::notifyOfMidwaySpecialPower(Int playerIndex,
                                              ...
                                              ObjectID sourceObj) {
  m_midwaySpecialPowers[playerIndex].push_back(
      AsciiStringObjectIDPair(completedPower, sourceObj));
}
...
/** Notify the script engine that a special power fired */
//-------------------------------------------------------------------------------------------------
void ScriptEngine::notifyOfCompletedSpecialPower(
    Int playerIndex, const AsciiString &completedPower, ObjectID sourceObj) {
  m_finishedSpecialPowers[playerIndex].push_back(
      AsciiStringObjectIDPair(completedPower, sourceObj));
}
...
void ScriptEngine::notifyOfCompletedUpgrade(Int playerIndex,
                                            ...
                                            ObjectID sourceObj) {
  m_completedUpgrades[playerIndex].push_back(
      AsciiStringObjectIDPair(upgrade, sourceObj));
}
...
//-------------------------------------------------------------------------------------------------
/** Notify that a UI button was pressed and some flag should go true, for one
 * frame only. */
//-------------------------------------------------------------------------------------------------
...
Bool ScriptEngine::isVideoComplete(const AsciiString &completedVideo,
                                   Bool removeFromList) {
  ListAsciiStringIt findIt = std::find(m_completedVideo.begin(),
                                       m_completedVideo.end(), completedVideo);
...
Bool ScriptEngine::isSpeechComplete(const AsciiString &testSpeech,
  ...
  for (findIt = m_testingSpeech.begin(); findIt != m_testingSpeech.end();
       ++findIt) {
  ...
  if (findIt == m_testingSpeech.end()) {
    ...
    UnsignedInt frameCount =
        REAL_TO_UNSIGNEDINT(audioLength / MSEC_PER_LOGICFRAME_REAL);
...
Bool ScriptEngine::isAudioComplete(const AsciiString &testAudio,
                                   Bool removeFromList) {
  ListAsciiStringUINTIt findIt;
  for (findIt = m_testingAudio.begin(); findIt != m_testingAudio.end();
       ++findIt) {
...
Bool ScriptEngine::isSpecialPowerTriggered(Int playerIndex,
                                           ...
                                           Bool removeFromList,
                                           ObjectID sourceObj) {
  if (playerIndex < 0 || playerIndex >= MAX_PLAYER_COUNT) return FALSE;

  ListAsciiStringObjectID *specialList =
      &(m_triggeredSpecialPowers[playerIndex]);

  for (ListAsciiStringObjectIDIt findIt = specialList->begin();
       findIt != specialList->end(); ++findIt) {
    AsciiStringObjectIDPair pair = *findIt;
    if (pair.first == completedPower &&
        (sourceObj == INVALID_ID || sourceObj == pair.second)) {
      if (removeFromList) {
...
//-------------------------------------------------------------------------------------------------
/** Determine whether a special power has reached a midpoint (not required for
 * all special powers!) */
//-------------------------------------------------------------------------------------------------
...
Bool ScriptEngine::isSpecialPowerComplete(Int playerIndex,
  ...
  ListAsciiStringObjectID *specialList =
      &(m_finishedSpecialPowers[playerIndex]);
...
Bool ScriptEngine::isUpgradeComplete(Int playerIndex,
                                     const AsciiString &upgrade,
                                     Bool removeFromList, ObjectID sourceObj) {
  if (playerIndex < 0 || playerIndex >= MAX_PLAYER_COUNT) return FALSE;
...
//-------------------------------------------------------------------------------------------------
Bool ScriptEngine::isGeneralChosen(Int playerIndex,
                                   const AsciiString &generalName,
                                   Bool removeFromList, ObjectID sourceObj) {
  if (playerIndex < 0 || playerIndex >= MAX_PLAYER_COUNT) return FALSE;
...
Bool ScriptEngine::isScienceAcquired(Int playerIndex, ScienceType science,
  ...
  for (ScienceVec::iterator it = specialList->begin(); it != specialList->end();
       ++it) {
    if (*it == science) {
      if (removeFromList) {
...
//-------------------------------------------------------------------------------------------------
/** if the object has a specified topple direction, change it to direction.
...
//-------------------------------------------------------------------------------------------------
/** if the object is named and has a specified topple direction, topple adjust
...
//-------------------------------------------------------------------------------------------------
Bool ScriptEngine::evaluateConditions(Script *pScript, Team *thisTeam,
                                      Player *player) {
...
#define COLLECT_CONDITION_EVAL_TIMES
  ...
  for (pCurCondition = pConditionHead; pCurCondition;
       pCurCondition = pCurCondition->getNextOrCondition()) {
    ...
    while (pCondition && andTerm) {
      if (!evaluateCondition(pCondition)) {
        andTerm = false;
        break;  // Short circuit the and evauation - after the first false, we
                // can quit.
      }
      pCondition = pCondition->getNext();
    }
    if (andTerm) {  // The outer list is OR'ed - so any true inner means we are
                    // true.
      testValue = true;
      break;
    }
  ...
  for (pCurAction = pActionHead; pCurAction;
    ...
    switch (pCurAction->getActionType()) {
      ...
      case ScriptAction::SET_ATTACK_PRIORITY_THING:
        setPriorityThing(pCurAction);
        break;
      case ScriptAction::SET_ATTACK_PRIORITY_KIND_OF:
        setPriorityKind(pCurAction);
        break;
      case ScriptAction::SET_DEFAULT_ATTACK_PRIORITY:
        setPriorityDefault(pCurAction);
        break;

      case ScriptAction::NO_OP: /* just break. */;
        break;
    }
  }
}
...
void ScriptEngine::executeScripts(Script *pScriptHead) {
  // Evaluate the scripts.
  Script *pCurScript;
  for (pCurScript = pScriptHead; pCurScript;
       pCurScript = pCurScript->getNext()) {
    if (pCurScript->isSubroutine()) {
      continue;  // Don't execute subroutines, except when called by other
                 // scripts.
    }
...
const ActionTemplate *ScriptEngine::getActionTemplate(Int ndx) {
  DEBUG_ASSERTCRASH(ndx >= 0 && ndx < ScriptAction::NUM_ITEMS,
                    ("Out of range.\n"));
  if (ndx < 0 || ndx >= ScriptAction::NUM_ITEMS) ndx = 0;
  DEBUG_ASSERTCRASH(!m_actionTemplates[ndx].getName().isEmpty(),
                    ("Need to initialize action enum=%d.\n", ndx));
...
const ConditionTemplate *ScriptEngine::getConditionTemplate(Int ndx) {
  ...
  DEBUG_ASSERTCRASH(!m_conditionTemplates[ndx].getName().isEmpty(),
                    ("Need to initialize Condition enum=%d.\n", ndx));
...
void ScriptEngine::appendSequentialScript(
    const SequentialScript *scriptToSequence) {
  ...
  for (it = m_sequentialScripts.begin(); it != m_sequentialScripts.end();
    ...
    if ((scriptToSequence->m_objectID &&
        ...
        (scriptToSequence->m_teamToExecOn &&
         scriptToSequence->m_teamToExecOn == seqScript->m_teamToExecOn)) {
      found = true;
...
void ScriptEngine::removeAllSequentialScripts(Team *team) {
  // this function will remove all pending scripts for this team, so just call
  // it.
...
void ScriptEngine::notifyOfTeamDestruction(Team *teamDestroyed) {
  ...
  if (m_callingTeam == teamDestroyed) m_callingTeam = NULL;
  if (m_conditionTeam == teamDestroyed) m_conditionTeam = NULL;
}
  ...
  for (it = m_sequentialScripts.begin(); it != m_sequentialScripts.end();
    ...
    if (ai || aigroup) {
      if (((ai && (ai->isIdle()) || (aigroup && aigroup->isIdle())) &&
          ...
          (seqScript->m_framesToWait == 0)) {
        // We want to supress messages if we're repeatedly waiting for an event
        // to occur, cause it KILLS our debug framerate.
        ...
        ScriptAction *action =
            seqScript->m_scriptToExecuteSequentially->getAction();
        ...
        if (action) {
          ...
          if (action->getActionType() ==
            ...
            if (!TheScriptConditions->evaluateSkirmishCommandButtonIsReady(
                    NULL, action->getParameter(1), action->getParameter(2),
                    true)) {
            ...
            if (!TheScriptConditions->evaluateSkirmishCommandButtonIsReady(
              ...
              seqScript->m_dontAdvanceInstruction = TRUE;
                     ...
                     ScriptAction::TEAM_WAIT_FOR_NOT_CONTAINED_ALL) {
            if (TheScriptConditions->evaluateTeamIsContained(
                    action->getParameter(0), true)) {
              seqScript->m_dontAdvanceInstruction = TRUE;
            }
          } else if (action->getActionType() ==
                     ScriptAction::TEAM_WAIT_FOR_NOT_CONTAINED_PARTIAL) {
            if (TheScriptConditions->evaluateTeamIsContained(
          ...
          // Check to see if executing our action told us to wait. If so, skip
          // to the next Sequential script
          if (seqScript->m_dontAdvanceInstruction) {
            ++it;
            itAdvanced = true;
            continue;
          ...
          if (ai && ai->isIdle()) {
            // pretend like we've already advanced to allow multiple checks on
            // this object this frame.
            itAdvanced = true;
          } else if (team) {
            // attempt to rebuild the aigroup, as it probably expired during the
            // action execution
...
ScriptEngine::VecSequentialScriptPtrIt ScriptEngine::cleanupSequentialScript(
...
Bool ScriptEngine::hasUnitCompletedSequentialScript(
    Object *object, const AsciiString &sequentialScriptName) {
  return FALSE;
}

Bool ScriptEngine::hasTeamCompletedSequentialScript(
    Team *team, const AsciiString &sequentialScriptName) {
  return FALSE;
}
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void SequentialScript::crc(Xfer *xfer) {}  // end crc
...
void SequentialScript::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_LOAD) {
    ...
    if (teamID != TEAM_ID_INVALID && m_teamToExecOn == NULL) {
      DEBUG_CRASH(
          ("SequentialScript::xfer - Unable to find team by ID (#%d) for "
           "m_teamToExecOn\n",
           teamID));
      throw SC_INVALID_DATA;
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    DEBUG_ASSERTCRASH(
        m_scriptToExecuteSequentially == NULL,
        ("SequentialScript::xfer - m_scripttoExecuteSequentially\n"));

    // find script
    m_scriptToExecuteSequentially =
        const_cast<Script *>(TheScriptEngine->findScriptByName(scriptName));
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void SequentialScript::loadPostProcess(void) {}  // end loadPostProcess
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void SequentialScriptStatus::crc(Xfer *xfer) {}  // end crc
...
// Load post process */
// ------------------------------------------------------------------------------------------------
void SequentialScriptStatus::loadPostProcess(void) {}  // end loadPostProcess
#endif
    ...
    if (m_firstUpdate) {
      ...
      switch (_getEditorBehavior()) {
        ...
        case 0x03: {
          ...
          ParticleSystemTemplate *pTemp = const_cast<ParticleSystemTemplate *>(
              TheParticleSystemManager->findTemplate(particleSystemName));
          if (pTemp) {
        ...
        case 0x07: {
          ...
          /*
             ...
             system list and remove each particle system
                                                  ParticleSystemManager::ParticleSystemList
             particleSysList; ParticleSystemManager::ParticleSystemListIt it;
                                                  while (true)
                                                  {
                                                          // reassign values
             into variables particleSysList =
             TheParticleSystemManager->getAllParticleSystems(); it =
             particleSysList.begin(); if (it == particleSysList.end()) break;

                                                          // check to make sure
             the particle system is valid ParticleSystem *sys = (*it); if (!sys)
                                                                  continue;

                                                          //before removing the
             system, make sure to remove all of its particles individually while
...
//-------------------------------------------------------------------------------------------------
Bool ScriptEngine::isTimeFrozenScript(void) { return m_freezeByScript; }

//-------------------------------------------------------------------------------------------------
/** Freeze time */
//-------------------------------------------------------------------------------------------------
void ScriptEngine::doFreezeTime(void) { m_freezeByScript = TRUE; }

//-------------------------------------------------------------------------------------------------
/** Unfreeze time */
//-------------------------------------------------------------------------------------------------
void ScriptEngine::doUnfreezeTime(void) { m_freezeByScript = FALSE; }

//-------------------------------------------------------------------------------------------------
/** For Debug and Internal builds, returns whether to continue (!pause), for
...
//-------------------------------------------------------------------------------------------------
/** For Debug and Internal builds, returns whether we are running fast (skipping
 * draw) */
//-------------------------------------------------------------------------------------------------
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void ScriptEngine::crc(Xfer *xfer) {}  // end crc
...
static void xferListAsciiString(Xfer *xfer, ListAsciiString *list) {
  // sanity
  DEBUG_ASSERTCRASH(list != NULL,
                    ("xferListAsciiString - Invalid parameters\n"));
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    if (list->empty() == FALSE) {
      DEBUG_CRASH(
          ("xferListAsciiString - list should be empty upon loading but is "
           "not\n"));
      throw SC_INVALID_DATA;
...
static void xferListAsciiStringUINT(Xfer *xfer, ListAsciiStringUINT *list) {
  // sanity
  DEBUG_ASSERTCRASH(list != NULL,
                    ("xferListAsciiStringUINT - Invalid parameters\n"));
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    if (list->empty() == FALSE) {
      DEBUG_CRASH(
          ("xferListAsciiStringUINT - list should be empty upon loading but is "
           "not\n"));
      throw SC_INVALID_DATA;
 ...
 * 1: Initial version */
// ------------------------------------------------------------------------------------------------
static void xferListAsciiStringObjectID(Xfer *xfer,
                                        ListAsciiStringObjectID *list) {
  // sanity
  DEBUG_ASSERTCRASH(list != NULL,
                    ("xferListAsciiStringObjectID - Invalid parameters\n"));
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    if (list->empty() == FALSE) {
      DEBUG_CRASH(
          ("xferListAsciiStringObjectID - list should be empty upon loading "
           "but is not\n"));
      throw SC_INVALID_DATA;
 ...
 * 1: Initial version */
// ------------------------------------------------------------------------------------------------
static void xferListAsciiStringCoord3D(Xfer *xfer,
                                       ListAsciiStringCoord3D *list) {
  // sanity
  DEBUG_ASSERTCRASH(list != NULL,
                    ("xferListAsciiStringCoord3D - Invalid parameters\n"));
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    if (list->empty() == FALSE) {
      DEBUG_CRASH(
          ("xferListAsciiStringCoord3D - list should be empty upon loading but "
           "is not\n"));
      throw SC_INVALID_DATA;
...
void ScriptEngine::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    if (m_sequentialScripts.size() != 0) {
      DEBUG_CRASH(
          ("ScriptEngine::xfer - m_sequentialScripts should be empty but is "
           "not\n"));
      throw SC_INVALID_DATA;
  ...
  if (countersSize > MAX_COUNTERS) {
    DEBUG_CRASH(
        ("ScriptEngine::xfer - MAX_COUNTERS has changed size, need to version "
         "this\n"));
    throw SC_INVALID_DATA;
  ...
  if (flagsSize > MAX_FLAGS) {
    DEBUG_CRASH(
        ("ScriptEngine::xfer - MAX_FLAGS has changed size, need to version "
         "this\n"));
    throw SC_INVALID_DATA;
  ...
  if (attackPriorityInfoSize > MAX_ATTACK_PRIORITIES) {
    DEBUG_CRASH(
        ("ScriptEngine::xfer - MAX_ATTACK_PRIORITIES size has changed, need to "
         "version this\n"));
    throw SC_INVALID_DATA;
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (UnsignedShort i = 0; i < namedObjectsCount; ++i) {
      ...
      if (obj == NULL && objectID != INVALID_ID) {
        DEBUG_CRASH(
            ("ScriptEngine::xfer - Unable to find object by ID for "
             "m_namedObjects\n"));
        throw SC_INVALID_DATA;
  ...
  if (triggeredSpecialPowersSize != MAX_PLAYER_COUNT) {
    DEBUG_CRASH(
        ("ScriptEngine::xfer - MAX_PLAYER_COUNT has changed, "
         "m_triggeredSpecialPowers size is now different and we must version "
         "this\n"));
    throw SC_INVALID_DATA;
  ...
  if (midwaySpecialPowersSize != MAX_PLAYER_COUNT) {
    DEBUG_CRASH(
        ("ScriptEngine::xfer - MAX_PLAYER_COUNT has changed, "
         "m_midwaySpecialPowers size is now different and we must version "
         "this\n"));
    throw SC_INVALID_DATA;
  ...
  if (finishedSpecialPowersSize != MAX_PLAYER_COUNT) {
    DEBUG_CRASH(
        ("ScriptEngine::xfer - MAX_PLAYER_COUNT has changed, "
         "m_finishedSpecialPowers size is now different and we must version "
         "this\n"));
    throw SC_INVALID_DATA;
  ...
  if (completedUpgradesSize != MAX_PLAYER_COUNT) {
    DEBUG_CRASH(
        ("ScriptEngine::xfer - MAX_PLAYER_COUNT has changed, "
         "m_completedUpgrades size is now different and we must version "
         "this\n"));
    throw SC_INVALID_DATA;
  ...
  if (acquiredSciencesSize != MAX_PLAYER_COUNT) {
    DEBUG_CRASH((
        "ScriptEngine::xfer - MAX_PLAYER_COUNT has changed, m_acquiredSciences "
        "size is now different and we must version this\n"));
    throw SC_INVALID_DATA;
  ...
  if (version >= 2) {
    ...
    if (xfer->getXferMode() == XFER_SAVE) {
      ...
      if (m_namedReveals.empty() == FALSE) {
        DEBUG_CRASH(
            ("ScriptEngine::xfer - m_namedReveals should be empty but is "
             "not!\n"));
        throw SC_INVALID_DATA;
    ...
    if (xfer->getXferMode() == XFER_SAVE) {
      ...
      if (m_allObjectTypeLists.empty() == FALSE) {
        DEBUG_CRASH(
            ("ScriptEngine::xfer - m_allObjectTypeLists should be empty but is "
             "not!\n"));
        throw SC_INVALID_DATA;
  ...
  if (xfer->getXferMode() == XFER_LOAD) {
    // We are doing a load.  If there is no fade active, do a black fade in to
    // start.
    if (m_fade == FADE_NONE) {
      m_fade = FADE_MULTIPLY;  // default to a fade in from black.
      m_curFadeFrame = 0;
...
void ScriptEngine::loadPostProcess(void) {
  // Now that we've loaded everything, go through and set them all back in sync
  // with what we currently think they should be.
  TheScriptActions->doEnableOrDisableObjectDifficultyBonuses(
...
void _appendMessage(const AsciiString &str, Bool isTrueMessage,
                    Bool shouldPause) {
  typedef void (*funcptr)(const char *);
...
void _appendAllParticleSystems(void) {
  ...
  ParticleSystemManager::TemplateMap::iterator begin(
      TheParticleSystemManager->beginParticleSystemTemplate());
  ParticleSystemManager::TemplateMap::iterator end(
      TheParticleSystemManager->endParticleSystemTemplate());
  for (; begin != end; ++begin) {
...
void _addUpdatedParticleSystem(AsciiString particleSystemName) {
  ...
  ParticleSystemTemplate *pTemplate =
      TheParticleSystemManager->newTemplate(particleSystemName);
  if (!pTemplate) {
    return;
  }
...
extern void _updateAsciiStringParmsFromSystem(
  ...
  ((funcptr)proc)(0, particleTemplate->m_particleTypeName.str(),
                  NULL);  // PARM_ParticleTypeName
  ((funcptr)proc)(1, particleTemplate->m_slaveSystemName.str(),
                  NULL);  // PARM_SlaveSystemName
  ((funcptr)proc)(2, particleTemplate->m_attachedSystemName.str(),
                  NULL);  // PARM_AttachedSystemName
}

#define BACKUP_FILE_NAME "Data\\INI\\ParticleSystem"
#define BACKUP_EXT "BAK"
static void _writeOutINI(void) {
  // currently, this uses NO intelligence. It blindly iterates through all of
  // the particle system templates and writes out every field that it thinks it
  // should.
  ...
  // save the old file
  File *oldINI = TheFileSystem->openFile("Data\\INI\\ParticleSystem.ini",
                                         File::READ | File::TEXT);
  ...
  File *newINI = TheFileSystem->openFile("Data\\INI\\ParticleSystem.ini",
...
static const std::string F_ISEMITABOVEGROUNDONLY = "IsEmitAboveGroundOnly";
...
static const std::string F_WINDPINGPONGSTARTANGLEMIN =
    "WindPingPongStartAngleMin";
static const std::string F_WINDPINGPONGSTARTANGLEMAX =
...
void _writeSingleParticleSystem(File *out, ParticleSystemTemplate *templ) {
  ...
  // the .append looks REALLY ugly, but this code was written with streams in
  // mind, and so these were all originally << (feed-operator for streams) I
  // might come back and re-write this later, if there are enough complaints.
  // ;-) jkmcd in the meantime, move along...
  std::string thisEntry = "";
  thisEntry.append(HEADER)
      .append(SEP_SPACE)
      .append(templ->getName().str())
      .append(SEP_EOL);
  thisEntry.append(SEP_HEAD)
      .append(F_PRIORITY)
      .append(EQ_WITH_SPACES)
      .append(ParticlePriorityNames[templ->m_priority])
      .append(SEP_EOL);
  thisEntry.append(SEP_HEAD)
      .append(F_ISONESHOT)
      .append(EQ_WITH_SPACES)
      .append((templ->m_isOneShot ? STR_TRUE : STR_FALSE))
      .append(SEP_EOL);
  thisEntry.append(SEP_HEAD)
      .append(F_SHADER)
      .append(EQ_WITH_SPACES)
      .append(ParticleShaderTypeNames[templ->m_shaderType])
      .append(SEP_EOL);
  thisEntry.append(SEP_HEAD)
      .append(F_TYPE)
      .append(EQ_WITH_SPACES)
      .append(ParticleTypeNames[templ->m_particleType])
      .append(SEP_EOL);
  thisEntry.append(SEP_HEAD)
      .append(F_PARTICLENAME)
      .append(EQ_WITH_SPACES)
      .append(templ->m_particleTypeName.str())
      .append(SEP_EOL);
  ...
  if (!templ->m_slaveSystemName.isEmpty()) {
    thisEntry.append(SEP_HEAD)
        .append(F_SLAVESYSTEM)
        .append(EQ_WITH_SPACES)
        .append(templ->m_slaveSystemName.str())
        .append(SEP_EOL);
    sprintf(buff1, FORMAT_STRING_LEADING_STRING, STR_X.c_str(),
            templ->m_slavePosOffset.x);
    sprintf(buff2, FORMAT_STRING_LEADING_STRING, STR_Y.c_str(),
            templ->m_slavePosOffset.y);
    sprintf(buff3, FORMAT_STRING_LEADING_STRING, STR_Z.c_str(),
            templ->m_slavePosOffset.z);
    thisEntry.append(SEP_HEAD)
        .append(F_SLAVEPOS)
        .append(EQ_WITH_SPACES)
        .append(buff1)
  ...
  if (!templ->m_attachedSystemName.isEmpty()) {
    thisEntry.append(SEP_HEAD)
        .append(F_ATTACHED)
        .append(EQ_WITH_SPACES)
        .append(templ->m_attachedSystemName.str())
        .append(SEP_EOL);
  }
  ...
  sprintf(buff1, "%s%d", STR_R.c_str(),
          REAL_TO_INT(templ->m_colorKey[0].color.red * 255 + 0.5));
  sprintf(buff2, "%s%d", STR_G.c_str(),
          REAL_TO_INT(templ->m_colorKey[0].color.green * 255 + 0.5));
  sprintf(buff3, "%s%d", STR_B.c_str(),
          REAL_TO_INT(templ->m_colorKey[0].color.blue * 255 + 0.5));
  ...
  sprintf(buff1, "%s%d", STR_R.c_str(),
          REAL_TO_INT(templ->m_colorKey[1].color.red * 255 + 0.5));
  sprintf(buff2, "%s%d", STR_G.c_str(),
          REAL_TO_INT(templ->m_colorKey[1].color.green * 255 + 0.5));
  sprintf(buff3, "%s%d", STR_B.c_str(),
          REAL_TO_INT(templ->m_colorKey[1].color.blue * 255 + 0.5));
  ...
  sprintf(buff1, "%s%d", STR_R.c_str(),
          REAL_TO_INT(templ->m_colorKey[2].color.red * 255 + 0.5));
  sprintf(buff2, "%s%d", STR_G.c_str(),
          REAL_TO_INT(templ->m_colorKey[2].color.green * 255 + 0.5));
  sprintf(buff3, "%s%d", STR_B.c_str(),
          REAL_TO_INT(templ->m_colorKey[2].color.blue * 255 + 0.5));
  ...
  sprintf(buff1, "%s%d", STR_R.c_str(),
          REAL_TO_INT(templ->m_colorKey[3].color.red * 255 + 0.5));
  sprintf(buff2, "%s%d", STR_G.c_str(),
          REAL_TO_INT(templ->m_colorKey[3].color.green * 255 + 0.5));
  sprintf(buff3, "%s%d", STR_B.c_str(),
          REAL_TO_INT(templ->m_colorKey[3].color.blue * 255 + 0.5));
  ...
  sprintf(buff1, "%s%d", STR_R.c_str(),
          REAL_TO_INT(templ->m_colorKey[4].color.red * 255 + 0.5));
  sprintf(buff2, "%s%d", STR_G.c_str(),
          REAL_TO_INT(templ->m_colorKey[4].color.green * 255 + 0.5));
  sprintf(buff3, "%s%d", STR_B.c_str(),
          REAL_TO_INT(templ->m_colorKey[4].color.blue * 255 + 0.5));
  ...
  sprintf(buff1, "%s%d", STR_R.c_str(),
          REAL_TO_INT(templ->m_colorKey[5].color.red * 255 + 0.5));
  sprintf(buff2, "%s%d", STR_G.c_str(),
          REAL_TO_INT(templ->m_colorKey[5].color.green * 255 + 0.5));
  sprintf(buff3, "%s%d", STR_B.c_str(),
          REAL_TO_INT(templ->m_colorKey[5].color.blue * 255 + 0.5));
  ...
  sprintf(buff1, "%s%d", STR_R.c_str(),
          REAL_TO_INT(templ->m_colorKey[6].color.red * 255 + 0.5));
  sprintf(buff2, "%s%d", STR_G.c_str(),
          REAL_TO_INT(templ->m_colorKey[6].color.green * 255 + 0.5));
  sprintf(buff3, "%s%d", STR_B.c_str(),
          REAL_TO_INT(templ->m_colorKey[6].color.blue * 255 + 0.5));
  ...
  sprintf(buff1, "%s%d", STR_R.c_str(),
          REAL_TO_INT(templ->m_colorKey[7].color.red * 255 + 0.5));
  sprintf(buff2, "%s%d", STR_G.c_str(),
          REAL_TO_INT(templ->m_colorKey[7].color.green * 255 + 0.5));
  sprintf(buff3, "%s%d", STR_B.c_str(),
          REAL_TO_INT(templ->m_colorKey[7].color.blue * 255 + 0.5));
  ...
  sprintf(buff1, FORMAT_STRING_LEADING_STRING, STR_X.c_str(),
          templ->m_driftVelocity.x);
  sprintf(buff2, FORMAT_STRING_LEADING_STRING, STR_Y.c_str(),
          templ->m_driftVelocity.y);
  sprintf(buff3, FORMAT_STRING_LEADING_STRING, STR_Z.c_str(),
          templ->m_driftVelocity.z);
  thisEntry.append(SEP_HEAD)
      .append(F_DRIFTVELOCITY)
      .append(EQ_WITH_SPACES)
      .append(buff1)
      .append(SEP_SPACE)
      .append(buff2)
  ...
  thisEntry.append(SEP_HEAD)
      .append(F_VELOCITYTYPE)
      .append(EQ_WITH_SPACES)
      .append(EmissionVelocityTypeNames[templ->m_emissionVelocityType])
      .append(SEP_EOL);

  if (templ->m_emissionVelocityType == ParticleSystemInfo::ORTHO) {
    sprintf(buff1, FORMAT_STRING,
            templ->m_emissionVelocity.ortho.x.getMinimumValue());
    sprintf(buff2, FORMAT_STRING,
            templ->m_emissionVelocity.ortho.x.getMaximumValue());
    thisEntry.append(SEP_HEAD)
        .append(F_VELORTHOX)
        .append(EQ_WITH_SPACES)
        .append(buff1)
    ...
    sprintf(buff1, FORMAT_STRING,
            templ->m_emissionVelocity.ortho.y.getMinimumValue());
    sprintf(buff2, FORMAT_STRING,
            templ->m_emissionVelocity.ortho.y.getMaximumValue());
    thisEntry.append(SEP_HEAD)
        .append(F_VELORTHOY)
        .append(EQ_WITH_SPACES)
        .append(buff1)
    ...
    sprintf(buff1, FORMAT_STRING,
            templ->m_emissionVelocity.ortho.z.getMinimumValue());
    sprintf(buff2, FORMAT_STRING,
            templ->m_emissionVelocity.ortho.z.getMaximumValue());
    thisEntry.append(SEP_HEAD)
        .append(F_VELORTHOZ)
        .append(EQ_WITH_SPACES)
        .append(buff1)
    ...
    sprintf(buff1, FORMAT_STRING,
            templ->m_emissionVelocity.spherical.speed.getMinimumValue());
    sprintf(buff2, FORMAT_STRING,
            templ->m_emissionVelocity.spherical.speed.getMaximumValue());
    thisEntry.append(SEP_HEAD)
        .append(F_VELSPHERE)
        .append(EQ_WITH_SPACES)
        .append(buff1)
  ...
  } else if (templ->m_emissionVelocityType ==
             ParticleSystemInfo::HEMISPHERICAL) {
    sprintf(buff1, FORMAT_STRING,
            templ->m_emissionVelocity.hemispherical.speed.getMinimumValue());
    sprintf(buff2, FORMAT_STRING,
            templ->m_emissionVelocity.hemispherical.speed.getMaximumValue());
    thisEntry.append(SEP_HEAD)
        .append(F_HEMISPHERE)
        .append(EQ_WITH_SPACES)
        .append(buff1)
    ...
    sprintf(buff1, FORMAT_STRING,
            templ->m_emissionVelocity.cylindrical.radial.getMinimumValue());
    sprintf(buff2, FORMAT_STRING,
            templ->m_emissionVelocity.cylindrical.radial.getMaximumValue());
    thisEntry.append(SEP_HEAD)
        .append(F_VELCYLRAD)
        .append(EQ_WITH_SPACES)
        .append(buff1)
    ...
    sprintf(buff1, FORMAT_STRING,
            templ->m_emissionVelocity.cylindrical.normal.getMinimumValue());
    sprintf(buff2, FORMAT_STRING,
            templ->m_emissionVelocity.cylindrical.normal.getMaximumValue());
    thisEntry.append(SEP_HEAD)
        .append(F_VELCYLNOR)
        .append(EQ_WITH_SPACES)
        .append(buff1)
    ...
    sprintf(buff1, FORMAT_STRING,
            templ->m_emissionVelocity.outward.speed.getMinimumValue());
    sprintf(buff2, FORMAT_STRING,
            templ->m_emissionVelocity.outward.speed.getMaximumValue());
    thisEntry.append(SEP_HEAD)
        .append(F_VELOUTWARD)
        .append(EQ_WITH_SPACES)
        .append(buff1)
    ...
    sprintf(buff1, FORMAT_STRING,
            templ->m_emissionVelocity.outward.otherSpeed.getMinimumValue());
    sprintf(buff2, FORMAT_STRING,
            templ->m_emissionVelocity.outward.otherSpeed.getMaximumValue());
    thisEntry.append(SEP_HEAD)
        .append(F_VELOUTOTHER)
        .append(EQ_WITH_SPACES)
        .append(buff1)
        .append(SEP_SPACE)
        .append(buff2)
  ...
  thisEntry.append(SEP_HEAD)
      .append(F_VOLUMETYPE)
      .append(EQ_WITH_SPACES)
      .append(EmissionVolumeTypeNames[templ->m_emissionVolumeType])
      .append(SEP_EOL);
  ...
  if (templ->m_emissionVolumeType == ParticleSystemInfo::POINT) {
    ...
    sprintf(buff1, FORMAT_STRING_LEADING_STRING, STR_X.c_str(),
            templ->m_emissionVolume.line.start.x);
    sprintf(buff2, FORMAT_STRING_LEADING_STRING, STR_Y.c_str(),
            templ->m_emissionVolume.line.start.y);
    sprintf(buff3, FORMAT_STRING_LEADING_STRING, STR_Z.c_str(),
            templ->m_emissionVolume.line.start.z);
    thisEntry.append(SEP_HEAD)
        .append(F_VOLLINESTART)
        .append(EQ_WITH_SPACES)
        .append(buff1)
        .append(SEP_SPACE)
        .append(buff2)
    ...
    sprintf(buff1, FORMAT_STRING_LEADING_STRING, STR_X.c_str(),
            templ->m_emissionVolume.line.end.x);
    sprintf(buff2, FORMAT_STRING_LEADING_STRING, STR_Y.c_str(),
            templ->m_emissionVolume.line.end.y);
    sprintf(buff3, FORMAT_STRING_LEADING_STRING, STR_Z.c_str(),
            templ->m_emissionVolume.line.end.z);
    thisEntry.append(SEP_HEAD)
        .append(F_VOLLINEEND)
        .append(EQ_WITH_SPACES)
        .append(buff1)
    ...
    sprintf(buff1, FORMAT_STRING_LEADING_STRING, STR_X.c_str(),
            templ->m_emissionVolume.box.halfSize.x);
    sprintf(buff2, FORMAT_STRING_LEADING_STRING, STR_Y.c_str(),
            templ->m_emissionVolume.box.halfSize.y);
    sprintf(buff3, FORMAT_STRING_LEADING_STRING, STR_Z.c_str(),
            templ->m_emissionVolume.box.halfSize.z);
    thisEntry.append(SEP_HEAD)
        .append(F_VOLBOXHALF)
        .append(EQ_WITH_SPACES)
        .append(buff1)
  ...
  thisEntry.append(SEP_HEAD)
      .append(F_ISHOLLOW)
      .append(EQ_WITH_SPACES)
      .append((templ->m_isEmissionVolumeHollow ? STR_TRUE : STR_FALSE))
      .append(SEP_EOL);
  thisEntry.append(SEP_HEAD)
      .append(F_ISXYPLANAR)
      .append(EQ_WITH_SPACES)
      .append((templ->m_isGroundAligned ? STR_TRUE : STR_FALSE))
      .append(SEP_EOL);
  thisEntry.append(SEP_HEAD)
      .append(F_ISEMITABOVEGROUNDONLY)
      .append(EQ_WITH_SPACES)
      .append((templ->m_isEmitAboveGroundOnly ? STR_TRUE : STR_FALSE))
      .append(SEP_EOL);
  thisEntry.append(SEP_HEAD)
      .append(F_ISPARTICLEUPTOWARDSEMITTER)
      .append(EQ_WITH_SPACES)
      .append((templ->m_isParticleUpTowardsEmitter ? STR_TRUE : STR_FALSE))
      .append(SEP_EOL);
  ...
  thisEntry.append(SEP_HEAD)
      .append(F_WINDMOTION)
      .append(EQ_WITH_SPACES)
      .append(WindMotionNames[templ->m_windMotion])
      .append(SEP_EOL);

  sprintf(buff1, "%f", templ->m_windAngleChangeMin);
  thisEntry.append(SEP_HEAD)
      .append(F_WINDANGLECHANGEMIN)
      .append(EQ_WITH_SPACES)
      .append(buff1)
      .append(SEP_EOL);
  sprintf(buff1, "%f", templ->m_windAngleChangeMax);
  thisEntry.append(SEP_HEAD)
      .append(F_WINDANGLECHANGEMAX)
      .append(EQ_WITH_SPACES)
      .append(buff1)
      .append(SEP_EOL);
  ...
  thisEntry.append(SEP_HEAD)
      .append(F_WINDPINGPONGSTARTANGLEMIN)
      .append(EQ_WITH_SPACES)
      .append(buff1)
      .append(SEP_EOL);
  sprintf(buff1, "%f", templ->m_windMotionStartAngleMax);
  thisEntry.append(SEP_HEAD)
      .append(F_WINDPINGPONGSTARTANGLEMAX)
      .append(EQ_WITH_SPACES)
      .append(buff1)
      .append(SEP_EOL);
  ...
  thisEntry.append(SEP_HEAD)
      .append(F_WINDPINGPONGENDANGLEMIN)
      .append(EQ_WITH_SPACES)
      .append(buff1)
      .append(SEP_EOL);
  sprintf(buff1, "%f", templ->m_windMotionEndAngleMax);
  thisEntry.append(SEP_HEAD)
      .append(F_WINDPINGPONGENDANGLEMAX)
      .append(EQ_WITH_SPACES)
      .append(buff1)
      .append(SEP_EOL);
    ...
    if (st_particleSystem) {
      if (st_particleSystem->isSystemForever())
        st_particleSystemNeedsStopping =
            TRUE;  // Only infinite lifetime systems need to be stopped.
      // You can't stop others, because you can't know if they have deleted
      // themselves.  That used to be a tiny memory overwrite, now it is a crash
      // since destroy() now has a function call.

      ParticleSystemTemplate *parentTemp =
          TheParticleSystemManager->findParentTemplate(pTemp->getName(), 0);
      if (parentTemp) {
        ParticleSystem *parentSystem = NULL;
        parentSystem =
            TheParticleSystemManager->createParticleSystem(parentTemp);

        if (parentSystem) {
          ParticleSystem::mergeRelatedParticleSystems(parentSystem,
                                                      st_particleSystem, true);
          parentSystem->stop();
      ...
      pos.z =
          TheTerrainLogic->getGroundHeight(pos.x, pos.y) + 5 * MAP_XY_FACTOR;
...
static void _reloadParticleSystemFromINI(AsciiString particleSystemName) {
  ...
  // force the particle system editor to recognize that the system has changed
  // without its consent.
  ...
  // save the old file
  File *iniFile = TheFileSystem->openFile("Data\\INI\\ParticleSystem.ini",
                                          File::READ | File::TEXT);
  ...
  try {
    // find the entry
    while (!((iniFile->eof()) ||
             INI::isDeclarationOfType("ParticleSystem", particleSystemName,
                                      linebuff))) {
    ...
    {  // copy it to a temp file
      ...
      outTempINI =
          TheFileSystem->openFile("temporary.ini", File::WRITE | File::TEXT);
      if (!outTempINI) {
        throw 0;
      }
...
static void _initVTune() {
  ...
  if (st_vTuneDLL) {
    ...
    DEBUG_ASSERTCRASH(VTPause != NULL && VTResume != NULL,
                      ("VTuneAPI procs not found!\n"));
  } else {
    VTPause = NULL;
    VTResume = NULL;
  }
  ...
  if (TheGlobalData->m_vTune) {
    ...
    if (VTPause) VTPause();
    // only complain about it being missing if they were expecting it to be
    // present
    DEBUG_ASSERTCRASH(st_vTuneDLL != NULL, ("VTuneAPI DLL not found!\n"));
  } else {
    // otherwise enable it.
    st_EnableVTune = true;
    if (VTResume) VTResume();
  }
}

static void _updateVTune() {
  if (!st_vTuneDLL) return;
