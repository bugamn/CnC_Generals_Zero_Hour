...
// ///////////////////////////////////////////////////////////////////////// The
// game scripting conditions.  Evaluates conditions. Author: John Ahlquist, Nov.
// 2001
///////////////////////////////////////////////////////////////////////////////////////////////////
...
class ObjectTypesTemp {
 public:
  ...
  ObjectTypesTemp() : m_types(NULL) { m_types = newInstance(ObjectTypes); }

  ~ObjectTypesTemp() {
    if (m_types) m_types->deleteInstance();
  }
};
...
TransportStatus::~TransportStatus() {
  if (m_nextStatus) m_nextStatus->deleteInstance();
}
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
ScriptConditions::ScriptConditions() {}  // end ScriptConditions
...
/** Init */
//-------------------------------------------------------------------------------------------------
void ScriptConditions::init(void) { reset(); }  // end init
...
//-------------------------------------------------------------------------------------------------
/** Finds the player by the name in the parameter, and if found caches the
player mask in the parameter so we don't have to do a name search.  May return
null if the player doesn't exist.*/
//-------------------------------------------------------------------------------------------------
Player *ScriptConditions::playerFromParam(Parameter *pSideParm) {
  DEBUG_ASSERTCRASH(Parameter::SIDE == pSideParm->getParameterType(),
  ...
  if (mask) {
    ...
    if (pPlayer) {
      // Enemy player can change dynamically, so don't cache the player mask.
      // jba.
  ...
  DEBUG_ASSERTCRASH(pPlayer,
                    ("Couldn't find player %s", pSideParm->getString().str()));
  return pPlayer;
}
...
//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluateAllBuildFacilitiesDestroyed(
    Parameter *pSideParm) {
  Player *pPlayer = playerFromParam(pSideParm);
  if (pPlayer) {
...
Bool ScriptConditions::evaluateIsDestroyed(Parameter *pTeamParm) {
  ...
  // the team that is being considered for the condition.  jba. :)
  if (theTeam) {
...
Bool ScriptConditions::evaluateHasUnits(Parameter *pTeamParm) {
  ...
  // It isn't THIS_TEAM, and doesn't match the THIS_TEAM, so check if any team
  // with this name has units.
...
Bool ScriptConditions::evaluateTeamInsideAreaPartially(
    Parameter *pTeamParm, Parameter *pTriggerAreaParm, Parameter *pTypeParm) {
  Team *theTeam = TheScriptEngine->getTeamNamed(pTeamParm->getString());
  // The team is the team based on the name, and the calling team (if any) and
  // the team that is being considered for the condition.  jba. :)
  AsciiString triggerName = pTriggerAreaParm->getString();
  PolygonTrigger *pTrig = TheScriptEngine->getQualifiedTriggerAreaByName(
      pTriggerAreaParm->getString());

  if (pTrig == NULL) return false;
  if (theTeam) {
    return (theTeam->someInsideSomeOutside(pTrig,
                                           (UnsignedInt)pTypeParm->getInt()) ||
...
//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluatePlayerHasUnitTypeInArea(
    Condition *pCondition, Parameter *pPlayerParm, Parameter *pComparisonParm,
    Parameter *pCountParm, Parameter *pTypeParm, Parameter *pTriggerParm) {
  AsciiString triggerName = pTriggerParm->getString();
  PolygonTrigger *pTrig =
      TheScriptEngine->getQualifiedTriggerAreaByName(pTriggerParm->getString());
  if (pTrig == NULL) return false;
  ...
  if (TheScriptEngine->getFrameObjectCountChanged() + 1 >=
      TheGameLogic->getFrame()) {
    anyChanges = true;  // Objects were added/deleted last frame, so count could
                        // have changed.  jba.
  }
      ...
      for (DLINK_ITERATOR<Object> iter = team->iterate_TeamMemberList();
        ...
        if (types.m_types->isInSet(pObj->getTemplate())) {
          if (pObj->isInside(pTrig)) {
            //
            // dead objects will not be considered, except crates ... they are
            // "dead" cause they have no body and health, but are a class of
            // object we want to trigger this stuff
            //
            if (!(pObj->isEffectivelyDead() || pObj->isKindOf(KINDOF_INERT)) ||
                pObj->isKindOf(KINDOF_CRATE)) {
              count++;
            }
          }
        }
...
//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluatePlayerHasUnitKindInArea(
    Condition *pCondition, Parameter *pPlayerParm, Parameter *pComparisonParm,
...
Bool ScriptConditions::evaluateNamedOutsideArea(
    Parameter *pUnitParm,
    Parameter *pTriggerParm) {  // This is actually NamedUnitInside(...)
...
//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluateTeamInsideAreaEntirely(
    Parameter *pTeamParm, Parameter *pTriggerParm,
    Parameter *pTypeParm) {  // This is actually TeamInside(...)
  Team *theTeam = TheScriptEngine->getTeamNamed(pTeamParm->getString());
  // The team is the team based on the name, and the calling team (if any) and
  // the team that is being considered for the condition.  jba. :)
  AsciiString triggerName = pTriggerParm->getString();
...
Bool ScriptConditions::evaluateNamedAttackedByPlayer(Parameter *pUnitParm,
  ...
  if (lastDamageInfo->in.m_sourcePlayerMask) {
    pPlayer =
        ThePlayerList->getPlayerFromMask(lastDamageInfo->in.m_sourcePlayerMask);
  }
...
Bool ScriptConditions::evaluateBuiltByPlayer(Condition *pCondition,
  ...
  const ThingTemplate *pTemplate =
      TheThingFactory->findTemplate(pTypeParm->getString());
  if (!pTemplate) {
    return false;
  }
  ...
  if (numTemplates > 0) {
    pPlayer->countObjectsByThingTemplate(numTemplates, &(*templates.begin()),
                                         false, &(*counts.begin()));
  } else {
    return 0;
  }
...
Bool ScriptConditions::evaluateUnitHealth(Parameter *pUnitParm,
  ...
  switch (pComparisonParm->getInt()) {
    case Parameter::LESS_THAN:
      return (curPercent < pHealthPercent->getInt());
    case Parameter::LESS_EQUAL:
      return (curPercent <= pHealthPercent->getInt());
    case Parameter::EQUAL:
      return (curPercent == pHealthPercent->getInt());
...
Bool ScriptConditions::evaluatePlayerHasCredits(Parameter *pCreditsParm,
  ...
  if (pPlayer && pPlayer->getMoney()) {
    switch (pComparisonParm->getInt()) {
      case Parameter::LESS_THAN:
        return (pCreditsParm->getInt() < pPlayer->getMoney()->countMoney());
        break;
      case Parameter::LESS_EQUAL:
        return (pCreditsParm->getInt() <= pPlayer->getMoney()->countMoney());
        break;
...
Bool ScriptConditions::evaluateBuildingEntered(Parameter *pPlayerParm,
  ...
  if (!contain) {
    DEBUG_CRASH(
        ("evaluateBuildingEntered script condition -- building doesn't have a "
         "container."));
    return false;
  }
...
Bool ScriptConditions::evaluateEnemySighted(Parameter *pItemParm,
  ...
  PartitionFilterRejectByObjectStatus filterStealth(OBJECT_STATUS_STEALTHED,
                                                    OBJECT_STATUS_DETECTED);

  // and only on-map (or not)
  PartitionFilterSameMapStatus filterMapStatus(theObj);

  PartitionFilter *filters[] = {&filterTeam, &filterAlive, &filterStealth,
...
Bool ScriptConditions::evaluateTypeSighted(Parameter *pItemParm,
  ...
  PartitionFilter *filters[] = {&filterAlive, &filterStealth, &filterMapStatus,
                                NULL};
  ...
  for (Object *them = iter->first(); them; them = iter->next()) {
    if (them->getControllingPlayer() == pPlayer) {
      if (types.m_types->isInSet(them->getTemplate()->getName())) return true;
    }
  }
  return false;
}
...
Bool ScriptConditions::evaluateNamedDiscovered(Parameter *pItemParm,
  ...
  ObjectShroudStatus shroud =
      theObj->getShroudedStatus(pPlayer->getPlayerIndex());
...
Bool ScriptConditions::evaluateTeamDiscovered(Parameter *pTeamParm,
  ...
  for (DLINK_ITERATOR<Object> iter = theTeam->iterate_TeamMemberList();
    ...
    ObjectShroudStatus shroud =
        pObj->getShroudedStatus(pPlayer->getPlayerIndex());
...
//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluatePlayerHasNOrFewerBuildings(
    Parameter *pBuildingCountParm, Parameter *pPlayerParm) {
  Player *pPlayer = playerFromParam(pPlayerParm);
  if (!pPlayer) {
    return false;
  }
...
Bool ScriptConditions::evaluatePlayerHasNOrFewerFactionBuildings(
  ...
  return (pBuildingCountParm->getInt() >=
          pPlayer->countObjects(mask, KINDOFMASK_NONE));
}
...
//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluateNamedReachedWaypointsEnd(
    Parameter *pUnitParm, Parameter *pWaypointPathParm) {
  Object *theObj = TheScriptEngine->getUnitNamed(pUnitParm->getString());
  if (!theObj) {
    return false;
  }
...
Bool ScriptConditions::evaluateTeamReachedWaypointsEnd(
  ...
  // Note - This returns true if any of the team completed the path.  This is as
  // the current implementation tends to do group pathfinding by default, so we
  // trigger when the leader actually thinks that he has reached the end of the
  // waypoint path.
  // int i = 0;
  for (DLINK_ITERATOR<Object> iter = theTeam->iterate_TeamMemberList();
       !iter.done(); iter.advance()) {
...
Bool ScriptConditions::evaluateNamedSelected(Condition *pCondition,
  ...
  if (TheInGameUI->getFrameSelectionChanged() + 1 >= TheGameLogic->getFrame()) {
    anyChanges = true;  // Objects were selected last frame, so count could have
                        // changed.  jba.
  }
  if (!anyChanges) {
    if (pCondition->getCustomData() == -1) return false;
...
Bool ScriptConditions::evaluatePlayerSpecialPowerFromUnitTriggered(
    Parameter *pPlayerParm, Parameter *pSpecialPowerParm,
    Parameter *pUnitParm) {
  Player *pPlayer = playerFromParam(pPlayerParm);
  ...
  if (pUnitParm) {
    ...
    if (!pUnit) {
      // we cared about the source object, but it is dead.  No sense checking
      // anymore, since we don't know it's objectID anymore. :P
      return FALSE;
    }
  ...
  if (pPlayer) {
    return TheScriptEngine->isSpecialPowerTriggered(
        pPlayer->getPlayerIndex(), pSpecialPowerParm->getString(), TRUE,
        sourceID);
  }
  return FALSE;
}
...
Bool ScriptConditions::evaluatePlayerSpecialPowerFromUnitMidway(
  ...
  if (pPlayer) {
    return TheScriptEngine->isSpecialPowerMidway(pPlayer->getPlayerIndex(),
                                                 pSpecialPowerParm->getString(),
                                                 TRUE, sourceID);
  }
  return FALSE;
}
...
Bool ScriptConditions::evaluatePlayerSpecialPowerFromUnitComplete(
  ...
  if (pPlayer) {
    return TheScriptEngine->isSpecialPowerComplete(
        pPlayer->getPlayerIndex(), pSpecialPowerParm->getString(), TRUE,
        sourceID);
  }
  return FALSE;
...
Bool ScriptConditions::evaluateUpgradeFromUnitComplete(Parameter *pPlayerParm,
  ...
  if (pPlayer) {
    return TheScriptEngine->isUpgradeComplete(
        pPlayer->getPlayerIndex(), pUpgradeParm->getString(), TRUE, sourceID);
  }
  return FALSE;
}
...
Bool ScriptConditions::evaluateScienceAcquired(Parameter *pPlayerParm,
  ...
  if (pPlayer) {
    ScienceType science =
        TheScienceStore->getScienceFromInternalName(pScienceParm->getString());
    if (science == SCIENCE_INVALID) return FALSE;
    return TheScriptEngine->isScienceAcquired(pPlayer->getPlayerIndex(),
                                              science, TRUE);
  }
  return FALSE;
}
...
Bool ScriptConditions::evaluateSciencePurchasePoints(
    Parameter *pPlayerParm, Parameter *pSciencePointParm) {
  Player *pPlayer = playerFromParam(pPlayerParm);
  if (pPlayer) {
...
Bool ScriptConditions::evaluateMultiplayerPlayerDefeat(void) {
  return TheVictoryConditions->isLocalDefeat() &&
         !TheVictoryConditions->isLocalAlliedDefeat();
}
...
Bool ScriptConditions::evaluatePlayerUnitCondition(Condition *pCondition,
  ...
  if (numObjs > 0) {
    pPlayer->countObjectsByThingTemplate(numObjs, &(*templates.begin()), false,
                                         &(*counts.begin()));
  ...
  switch (pComparisonParm->getInt()) {
    ...
    default:
      DEBUG_CRASH(
          ("ScriptConditions::evaluatePlayerUnitCondition: Invalid comparison "
           "type. (jkmcd)"));
      break;
  }
  pCondition->setCustomData(-1);  // false.
...
Bool ScriptConditions::evaluatePlayerHasComparisonPercentPower(
    Parameter *pPlayerParm, Parameter *pComparisonParm,
...
//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluatePlayerHasComparisonValueExcessPower(
    Parameter *pPlayerParm, Parameter *pComparisonParm, Parameter *pKWHParm) {
  Player *pPlayer = playerFromParam(pPlayerParm);
  if (!pPlayer) {
    return false;
  }
  Int desiredKilowattExcess = pKWHParm->getInt();
  Int actualKilowats = pPlayer->getEnergy()->getProduction() -
                       pPlayer->getEnergy()->getConsumption();
  switch (pComparisonParm->getInt()) {
    case Parameter::LESS_THAN:
      return (actualKilowats < desiredKilowattExcess);
    case Parameter::LESS_EQUAL:
      return (actualKilowats <= desiredKilowattExcess);
    case Parameter::EQUAL:
      return (actualKilowats == desiredKilowattExcess);
...
//-------------------------------------------------------------------------------------------------
/** evaluateSkirmishSpecialPowerIsReady - does any unit have this special power
 * ready to use? */
//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluateSkirmishSpecialPowerIsReady(
  ...
  const SpecialPowerTemplate *power =
      TheSpecialPowerStore->findSpecialPowerTemplate(pPower->getString());
  if (power == NULL) {
  ...
  for (it = pPlayer->getPlayerTeams()->begin();
    ...
    for (DLINK_ITERATOR<Team> iter = (*it)->iterate_TeamInstanceList();
      ...
      for (DLINK_ITERATOR<Object> iter = team->iterate_TeamMemberList();
        ...
        Object *pObj = iter.cur();
        if (!pObj) continue;
        if (BitTest(pObj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION) ||
            pObj->isDisabled()) {
        ...
        if (mod) {
          ...
          if (mod->getReadyFrame() < nextFrame)
            nextFrame = mod->getReadyFrame();
        }
      }
    }
  }
...
Bool ScriptConditions::evaluatePlayerDestroyedNOrMoreBuildings(
    Parameter *pPlayerParm, Parameter *pNumParm, Parameter *pOpponentParm) {
  Player *pPlayer = playerFromParam(pPlayerParm);
...
Bool ScriptConditions::evaluateUnitHasEmptied(Parameter *pUnitParm) {
  ...
  if (stats->m_frameNumber == frameNum - 1) {
    if (stats->m_unitCount > 0 && numPeeps == 0) {
      // don't actually update the info on this round, because we want to make
      // sure that multiple calls to this in the same frame actually work.
      return true;
    }
  }
...
Bool ScriptConditions::evaluateTeamIsContained(Parameter *pTeamParm,
  ...
  for (DLINK_ITERATOR<Object> iter = pTeam->iterate_TeamMemberList();
    ...
    if (!isContained) {
      // we could still be exiting, in which case we should pretend like we are
      // contained.
...
//-------------------------------------------------------------------------------------------------
// @todo: PERF_EVALUATE Get a perf timer on this. Should we adjust this function
// so that it runs like evaluatePlayerHasUnitKindInArea
// ?
Bool ScriptConditions::evaluateSkirmishValueInArea(
    Condition *pCondition, Parameter *pSkirmishPlayerParm,
    Parameter *pComparisonParm, Parameter *pMoneyParm,
    Parameter *pTriggerParm) {
  ...
  for (it = player->getPlayerTeams()->begin();
  ...
  switch (pComparisonParm->getInt()) {
    case Parameter::LESS_THAN:
      comparison = (totalCost < pMoneyParm->getInt());
      break;
    case Parameter::LESS_EQUAL:
      comparison = (totalCost <= pMoneyParm->getInt());
      break;
    ...
    default:
      DEBUG_CRASH(
          ("ScriptConditions::evaluateSkirmishValueInArea: Invalid comparison "
           "type. (jkmcd)"));
      break;
  }
  pCondition->setCustomData(-1);  // false.
...
Bool ScriptConditions::evaluateSkirmishSuppliesWithinDistancePerimeter(
    Parameter *pSkirmishPlayerParm, Parameter *pDistanceParm,
    Parameter *pLocationParm, Parameter *pValueParm) {
  Player *player = playerFromParam(pSkirmishPlayerParm);
  if (!player) {
    return false;
  }
  ...
  for (Object *them = iter->first(); them; them = iter->next()) {
    ...
    SupplyWarehouseDockUpdate *warehouseModule =
        (SupplyWarehouseDockUpdate *)them->findUpdateModule(
            key_warehouseUpdate);
    if (!warehouseModule) {
      continue;
    }
    ...
    Real value =
        player->getSupplyBoxValue() * warehouseModule->getBoxesStored();
    if (value > maxValue) {
      maxValue = value;
    }
  }
...
// If this is called multiple times per frame, the cost could add up. This will
// infrequently (read: never) change, so it shouldn't be called very often.
// jkmcd
Bool ScriptConditions::
    evaluateSkirmishPlayerTechBuildingWithinDistancePerimeter(
        Condition *pCondition, Parameter *pSkirmishPlayerParm,
        Parameter *pDistanceParm, Parameter *pLocationParm) {
  Player *player = playerFromParam(pSkirmishPlayerParm);
  if (!player) {
    return false;
  }
  ...
  PartitionFilterAcceptByKindOf f1(MAKE_KINDOF_MASK(KINDOF_TECH_BUILDING),
                                   KINDOFMASK_NONE);
  PartitionFilterPlayerAffiliation f2(player, ALLOW_ALLIES, false);
  PartitionFilterPlayer f3(player, false);  // Don't find your own units, as our
                                            // affiliation to self is neutral.
  PartitionFilterOnMap filterMapStatus;

  PartitionFilter *filters[] = {&f1, &f2, &f3, &filterMapStatus, 0};

  Bool comparison = ThePartitionManager->getClosestObject(
                        &center, radius, FROM_CENTER_2D, filters) != NULL;
  pCondition->setCustomData(-1);  // false.
  if (comparison) {
    pCondition->setCustomData(1);  // true.
  }
...
Bool ScriptConditions::evaluateSkirmishCommandButtonIsReady(
    Parameter * /* pSkirmishPlayerParm */, Parameter *pTeamParm,
    Parameter *pCommandButtonParm, Bool allReady) {
  ...
  for (DLINK_ITERATOR<Object> iter = theTeam->iterate_TeamMemberList();
    ...
    if (commandButton->getSpecialPowerTemplate()) {
      if (!pObj->hasSpecialPower(commandButton->getSpecialPowerTemplate()
                                     ->getSpecialPowerType())) {
        continue;
      }
...
Bool ScriptConditions::evaluateSkirmishUnownedFactionUnitComparison(
  ...
  // Note: This looks slow, but in practice it shouldn't be. The neutral player
  // shouldn't really ever have more than one team.
  ...
  switch (pComparisonParm->getInt()) {
    case Parameter::LESS_THAN:
      return numFactionUnits < pCountParm->getInt();
      break;
    case Parameter::LESS_EQUAL:
      return numFactionUnits <= pCountParm->getInt();
      break;
  ...
  DEBUG_CRASH(
      ("ScriptConditions::evaluateSkirmishUnownedFactionUnitComparison: "
       "Invalid comparison type. (jkmcd)"));
  return FALSE;
}

//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluateSkirmishPlayerHasPrereqsToBuild(
    Parameter *pSkirmishPlayerParm, Parameter *pObjectTypeParm) {
  Player *player = playerFromParam(pSkirmishPlayerParm);
  if (!player) {
    return FALSE;
  }
...
Bool ScriptConditions::evaluateSkirmishPlayerHasComparisonGarrisoned(
  ...
  switch (pComparisonParm->getInt()) {
    case Parameter::LESS_THAN:
      return numGarrisonedBuildings < pCountParm->getInt();
      break;
    case Parameter::LESS_EQUAL:
      return numGarrisonedBuildings <= pCountParm->getInt();
      break;
  ...
  DEBUG_CRASH(
      ("ScriptConditions::evaluateSkirmishPlayerHasComparisonGarrisoned: "
       "Invalid comparison type. (jkmcd)"));
  return FALSE;
}
...
Bool ScriptConditions::evaluateSkirmishPlayerHasComparisonCapturedUnits(
  ...
  switch (pComparisonParm->getInt()) {
    case Parameter::LESS_THAN:
      return numCapturedUnits < pCountParm->getInt();
      break;
    case Parameter::LESS_EQUAL:
      return numCapturedUnits <= pCountParm->getInt();
      break;
...
//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluateSkirmishNamedAreaExists(
    Parameter *, Parameter *pTriggerParm) {
  PolygonTrigger *pTrig =
      TheScriptEngine->getQualifiedTriggerAreaByName(pTriggerParm->getString());
  return (pTrig != NULL);
}
...
//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluateSkirmishPlayerHasUnitsInArea(
    Condition *pCondition, Parameter *pSkirmishPlayerParm,
    Parameter *pTriggerParm) {
  Player *player = playerFromParam(pSkirmishPlayerParm);
  if (!player) {
  ...
  Object *obj = ThePartitionManager->getClosestObject(&center, radius,
...
//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluateSkirmishSupplySourceSafe(
    Condition *pCondition, Parameter *pSkirmishPlayerParm,
    Parameter *pMinSupplyAmount) {
...
//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluateSkirmishSupplySourceAttacked(
...
//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluateSkirmishStartPosition(
    Parameter *pSkirmishPlayerParm, Parameter *pStartNdx) {
  Player *player = playerFromParam(pSkirmishPlayerParm);
  if (!player) {
    return FALSE;
  }
...
//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluateSkirmishPlayerHasBeenAttackedByPlayer(
    Parameter *pSkirmishPlayerParm, Parameter *pAttackedByParm) {
  Player *player = playerFromParam(pSkirmishPlayerParm);
  if (!player) {
    return FALSE;
  }
...
//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluateSkirmishPlayerIsOutsideArea(
    Condition *pCondition, Parameter *pSkirmishPlayerParm,
    Parameter *pTriggerParm) {
  // Even though these will be prechecked in the other function, we want to
  // preflight here as well.
  Player *player = playerFromParam(pSkirmishPlayerParm);
  if (!player) {
    return FALSE;
  }
  ...
  return !evaluateSkirmishPlayerHasUnitsInArea(pCondition, pSkirmishPlayerParm,
                                               pTriggerParm);
}
  ...
  for (it = player->getPlayerTeams()->begin();
    ...
    for (DLINK_ITERATOR<Team> iter = (*it)->iterate_TeamInstanceList();
      ...
      for (DLINK_ITERATOR<Object> objIter = team->iterate_TeamMemberList();
        ...
        ObjectShroudStatus shroudStatus =
            obj->getShroudedStatus(discoveredByIndex);
        if (shroudStatus == OBJECTSHROUD_CLEAR ||
            shroudStatus == OBJECTSHROUD_PARTIAL_CLEAR) {
          return TRUE;
        }
      }
    }
...
Bool ScriptConditions::evaluatePlayerLostObjectType(Parameter *pPlayerParm,
  ...
  if (numTemplates > 0) {
    player->countObjectsByThingTemplate(numTemplates, &(*templates.begin()),
                                        true, &(*counts.begin()));
  } else {
    return FALSE;
  }
  ...
  Int sumOfObjs = rts::sum(counts);
  Int currentCount = TheScriptEngine->getObjectCount(player->getPlayerIndex(),
                                                     pTypeParm->getString());

  if (sumOfObjs != currentCount) {
    TheScriptEngine->setObjectCount(player->getPlayerIndex(),
                                    pTypeParm->getString(), sumOfObjs);
  }
...
Bool ScriptConditions::evaluateCondition(Condition *pCondition) {
  switch (pCondition->getConditionType()) {
    default:
      DEBUG_CRASH(
          ("Unknown ScriptCondition type %d", pCondition->getConditionType()));
      return false;
    ...
    case Condition::TEAM_INSIDE_AREA_PARTIALLY:
      return evaluateTeamInsideAreaPartially(pCondition->getParameter(0),
                                             pCondition->getParameter(1),
                                             pCondition->getParameter(2));
    case Condition::NAMED_INSIDE_AREA:
      return evaluateNamedInsideArea(pCondition->getParameter(0),
                                     pCondition->getParameter(1));
    ...
    case Condition::TEAM_STATE_IS:
      return evaluateTeamStateIs(pCondition->getParameter(0),
                                 pCondition->getParameter(1));
    case Condition::TEAM_STATE_IS_NOT:
      return evaluateTeamStateIsNot(pCondition->getParameter(0),
                                    pCondition->getParameter(1));
    case Condition::NAMED_OUTSIDE_AREA:
      return evaluateNamedOutsideArea(pCondition->getParameter(0),
                                      pCondition->getParameter(1));
    case Condition::TEAM_INSIDE_AREA_ENTIRELY:
      return evaluateTeamInsideAreaEntirely(pCondition->getParameter(0),
                                            pCondition->getParameter(1),
                                            pCondition->getParameter(2));
    case Condition::TEAM_OUTSIDE_AREA_ENTIRELY:
      return evaluateTeamOutsideAreaEntirely(pCondition->getParameter(0),
                                             pCondition->getParameter(1),
                                             pCondition->getParameter(2));
    case Condition::NAMED_ATTACKED_BY_OBJECTTYPE:
      return evaluateNamedAttackedByType(pCondition->getParameter(0),
                                         pCondition->getParameter(1));
    case Condition::TEAM_ATTACKED_BY_OBJECTTYPE:
      return evaluateTeamAttackedByType(pCondition->getParameter(0),
                                        pCondition->getParameter(1));
    case Condition::NAMED_ATTACKED_BY_PLAYER:
      return evaluateNamedAttackedByPlayer(pCondition->getParameter(0),
                                           pCondition->getParameter(1));
    case Condition::TEAM_ATTACKED_BY_PLAYER:
      return evaluateTeamAttackedByPlayer(pCondition->getParameter(0),
                                          pCondition->getParameter(1));
    case Condition::BUILT_BY_PLAYER:
      return evaluateBuiltByPlayer(pCondition, pCondition->getParameter(0),
                                   pCondition->getParameter(1));
    ...
    case Condition::PLAYER_HAS_CREDITS:
      return evaluatePlayerHasCredits(pCondition->getParameter(0),
                                      pCondition->getParameter(1),
                                      pCondition->getParameter(2));
    ...
    case Condition::NAMED_DISCOVERED:
      return evaluateNamedDiscovered(pCondition->getParameter(0),
                                     pCondition->getParameter(1));
    case Condition::TEAM_DISCOVERED:
      return evaluateTeamDiscovered(pCondition->getParameter(0),
                                    pCondition->getParameter(1));
    case Condition::BUILDING_ENTERED_BY_PLAYER:
      return evaluateBuildingEntered(pCondition->getParameter(0),
                                     pCondition->getParameter(1));
    case Condition::ENEMY_SIGHTED: {
      Int numParameters = pCondition->getNumParameters();
      DEBUG_ASSERTCRASH(numParameters == 3,
                        ("'Condition: [Unit] Unit has sighted a(n) "
                         "friendly/neutral/enemy unit belonging to a side.' "
                         "has too few parameters. Please fix in WB. (jkmcd)"));

      if (numParameters < 3) {
        return false;
      }

      return evaluateEnemySighted(pCondition->getParameter(0),
                                  pCondition->getParameter(1),
                                  pCondition->getParameter(2));
    }
    case Condition::TYPE_SIGHTED:
      return evaluateTypeSighted(pCondition->getParameter(0),
                                 pCondition->getParameter(1),
                                 pCondition->getParameter(2));
    ...
    case Condition::MISSION_ATTEMPTS:
      return evaluateMissionAttempts(pCondition->getParameter(0),
                                     pCondition->getParameter(1),
                                     pCondition->getParameter(2));
    case Condition::NAMED_OWNED_BY_PLAYER:
      return evaluateNamedOwnedByPlayer(pCondition->getParameter(0),
                                        pCondition->getParameter(1));
    case Condition::TEAM_OWNED_BY_PLAYER:
      return evaluateTeamOwnedByPlayer(pCondition->getParameter(0),
                                       pCondition->getParameter(1));
    case Condition::PLAYER_HAS_N_OR_FEWER_BUILDINGS:
      return evaluatePlayerHasNOrFewerBuildings(pCondition->getParameter(1),
                                                pCondition->getParameter(0));
    case Condition::PLAYER_HAS_N_OR_FEWER_FACTION_BUILDINGS:
      return evaluatePlayerHasNOrFewerFactionBuildings(
          pCondition->getParameter(1), pCondition->getParameter(0));
    ...
    case Condition::NAMED_REACHED_WAYPOINTS_END:
      return evaluateNamedReachedWaypointsEnd(pCondition->getParameter(0),
                                              pCondition->getParameter(1));
    case Condition::TEAM_REACHED_WAYPOINTS_END:
      return evaluateTeamReachedWaypointsEnd(pCondition->getParameter(0),
                                             pCondition->getParameter(1));
    ...
    case Condition::NAMED_ENTERED_AREA:
      return evaluateNamedEnteredArea(pCondition->getParameter(0),
                                      pCondition->getParameter(1));
    case Condition::NAMED_EXITED_AREA:
      return evaluateNamedExitedArea(pCondition->getParameter(0),
                                     pCondition->getParameter(1));
    case Condition::TEAM_ENTERED_AREA_ENTIRELY:
      return evaluateTeamEnteredAreaEntirely(pCondition->getParameter(0),
                                             pCondition->getParameter(1),
                                             pCondition->getParameter(2));
    case Condition::TEAM_ENTERED_AREA_PARTIALLY:
      return evaluateTeamEnteredAreaPartially(pCondition->getParameter(0),
                                              pCondition->getParameter(1),
                                              pCondition->getParameter(2));
    case Condition::TEAM_EXITED_AREA_ENTIRELY:
      return evaluateTeamExitedAreaEntirely(pCondition->getParameter(0),
                                            pCondition->getParameter(1),
                                            pCondition->getParameter(2));
    case Condition::TEAM_EXITED_AREA_PARTIALLY:
      return evaluateTeamExitedAreaPartially(pCondition->getParameter(0),
                                             pCondition->getParameter(1),
                                             pCondition->getParameter(2));
    ...
    case Condition::UNIT_HEALTH:
      return evaluateUnitHealth(pCondition->getParameter(0),
                                pCondition->getParameter(1),
                                pCondition->getParameter(2));
    case Condition::PLAYER_TRIGGERED_SPECIAL_POWER:
      return evaluatePlayerSpecialPowerFromUnitTriggered(
          pCondition->getParameter(0), pCondition->getParameter(1), NULL);
    case Condition::PLAYER_TRIGGERED_SPECIAL_POWER_FROM_NAMED:
      return evaluatePlayerSpecialPowerFromUnitTriggered(
          pCondition->getParameter(0), pCondition->getParameter(1),
          pCondition->getParameter(2));
    case Condition::PLAYER_MIDWAY_SPECIAL_POWER:
      return evaluatePlayerSpecialPowerFromUnitMidway(
          pCondition->getParameter(0), pCondition->getParameter(1), NULL);
    ...
    case Condition::PLAYER_COMPLETED_SPECIAL_POWER:
      return evaluatePlayerSpecialPowerFromUnitComplete(
          pCondition->getParameter(0), pCondition->getParameter(1), NULL);
    ...
    case Condition::PLAYER_ACQUIRED_SCIENCE:
      return evaluateScienceAcquired(pCondition->getParameter(0),
                                     pCondition->getParameter(1));
    case Condition::PLAYER_CAN_PURCHASE_SCIENCE:
      return evaluateCanPurchaseScience(pCondition->getParameter(0),
                                        pCondition->getParameter(1));
    case Condition::PLAYER_HAS_SCIENCEPURCHASEPOINTS:
      return evaluateSciencePurchasePoints(pCondition->getParameter(0),
                                           pCondition->getParameter(1));
    ...
    case Condition::PLAYER_BUILT_UPGRADE:
      return evaluateUpgradeFromUnitComplete(pCondition->getParameter(0),
                                             pCondition->getParameter(1), NULL);
    case Condition::PLAYER_BUILT_UPGRADE_FROM_NAMED:
      return evaluateUpgradeFromUnitComplete(pCondition->getParameter(0),
                                             pCondition->getParameter(1),
                                             pCondition->getParameter(2));
    case Condition::PLAYER_HAS_OBJECT_COMPARISON:
      return evaluatePlayerUnitCondition(
          pCondition, pCondition->getParameter(0), pCondition->getParameter(1),
          pCondition->getParameter(2), pCondition->getParameter(3));
    case Condition::PLAYER_DESTROYED_N_BUILDINGS_PLAYER:
      return evaluatePlayerDestroyedNOrMoreBuildings(
          pCondition->getParameter(0), pCondition->getParameter(1),
          pCondition->getParameter(2));
    case Condition::PLAYER_HAS_COMPARISON_UNIT_TYPE_IN_TRIGGER_AREA: {
      ...
      DEBUG_ASSERTCRASH(
          numParameters == 5,
          ("'Condition: [Player] has (comparison) unit type in an area' has "
           "too few parameters. Please fix in WB. (jkmcd)"));

      if (numParameters < 5) {
        return false;
      }

      return evaluatePlayerHasUnitTypeInArea(
          pCondition, pCondition->getParameter(0), pCondition->getParameter(1),
          pCondition->getParameter(2), pCondition->getParameter(3),
          pCondition->getParameter(4));
    }
    case Condition::PLAYER_HAS_COMPARISON_UNIT_KIND_IN_TRIGGER_AREA: {
      Int numParameters = pCondition->getNumParameters();
      DEBUG_ASSERTCRASH(
          numParameters == 5,
          ("'Condition: [Player] has (comparison) kind of unit or structure in "
           "an area' has too few parameters. Please fix in WB. (jkmcd)"));

      if (numParameters < 5) {
        return false;
      }

      return evaluatePlayerHasUnitKindInArea(
          pCondition, pCondition->getParameter(0), pCondition->getParameter(1),
          pCondition->getParameter(2), pCondition->getParameter(3),
          pCondition->getParameter(4));
    }
    ...
    case Condition::PLAYER_POWER_COMPARE_PERCENT:
      return evaluatePlayerHasComparisonPercentPower(
          pCondition->getParameter(0), pCondition->getParameter(1),
          pCondition->getParameter(2));

    case Condition::PLAYER_EXCESS_POWER_COMPARE_VALUE:
      return evaluatePlayerHasComparisonValueExcessPower(
          pCondition->getParameter(0), pCondition->getParameter(1),
          pCondition->getParameter(2));

    case Condition::SKIRMISH_SPECIAL_POWER_READY:
      return evaluateSkirmishSpecialPowerIsReady(pCondition->getParameter(0),
                                                 pCondition->getParameter(1));

    case Condition::UNIT_HAS_OBJECT_STATUS:
      return evaluateUnitHasObjectStatus(pCondition->getParameter(0),
                                         pCondition->getParameter(1));

    case Condition::TEAM_ALL_HAS_OBJECT_STATUS:
      return evaluateTeamHasObjectStatus(pCondition->getParameter(0),
                                         pCondition->getParameter(1), true);

    case Condition::TEAM_SOME_HAVE_OBJECT_STATUS:
      return evaluateTeamHasObjectStatus(pCondition->getParameter(0),
                                         pCondition->getParameter(1), false);

    case Condition::SKIRMISH_VALUE_IN_AREA:
      return evaluateSkirmishValueInArea(
          pCondition, pCondition->getParameter(0), pCondition->getParameter(1),
          pCondition->getParameter(2), pCondition->getParameter(3));

    case Condition::SKIRMISH_PLAYER_FACTION:
      return evaluateSkirmishPlayerIsFaction(pCondition->getParameter(0),
                                             pCondition->getParameter(1));

    case Condition::SKIRMISH_SUPPLIES_VALUE_WITHIN_DISTANCE:
      return evaluateSkirmishSuppliesWithinDistancePerimeter(
          pCondition->getParameter(0), pCondition->getParameter(1),
          pCondition->getParameter(2), pCondition->getParameter(3));

    case Condition::SKIRMISH_TECH_BUILDING_WITHIN_DISTANCE:
      return evaluateSkirmishPlayerTechBuildingWithinDistancePerimeter(
          pCondition, pCondition->getParameter(0), pCondition->getParameter(1),
          pCondition->getParameter(2));

    case Condition::SKIRMISH_COMMAND_BUTTON_READY_ALL:
      return evaluateSkirmishCommandButtonIsReady(
          pCondition->getParameter(0), pCondition->getParameter(1),
          pCondition->getParameter(2), true);

    case Condition::SKIRMISH_COMMAND_BUTTON_READY_PARTIAL:
      return evaluateSkirmishCommandButtonIsReady(
          pCondition->getParameter(0), pCondition->getParameter(1),
          pCondition->getParameter(2), false);

    case Condition::SKIRMISH_UNOWNED_FACTION_UNIT_EXISTS:
      return evaluateSkirmishUnownedFactionUnitComparison(
          pCondition->getParameter(0), pCondition->getParameter(1),
          pCondition->getParameter(2));

    case Condition::SKIRMISH_PLAYER_HAS_PREREQUISITE_TO_BUILD:
      return evaluateSkirmishPlayerHasPrereqsToBuild(
          pCondition->getParameter(0), pCondition->getParameter(1));

    case Condition::SKIRMISH_PLAYER_HAS_COMPARISON_GARRISONED:
      return evaluateSkirmishPlayerHasComparisonGarrisoned(
          pCondition->getParameter(0), pCondition->getParameter(1),
          pCondition->getParameter(2));

    case Condition::SKIRMISH_PLAYER_HAS_COMPARISON_CAPTURED_UNITS:
      return evaluateSkirmishPlayerHasComparisonCapturedUnits(
          pCondition->getParameter(0), pCondition->getParameter(1),
          pCondition->getParameter(2));

    case Condition::SKIRMISH_NAMED_AREA_EXIST:
      return evaluateSkirmishNamedAreaExists(pCondition->getParameter(0),
                                             pCondition->getParameter(1));

    case Condition::SKIRMISH_PLAYER_HAS_UNITS_IN_AREA:
      return evaluateSkirmishPlayerHasUnitsInArea(
          pCondition, pCondition->getParameter(0), pCondition->getParameter(1));

    case Condition::SKIRMISH_PLAYER_HAS_BEEN_ATTACKED_BY_PLAYER:
      return evaluateSkirmishPlayerHasBeenAttackedByPlayer(
          pCondition->getParameter(0), pCondition->getParameter(1));

    case Condition::SKIRMISH_PLAYER_IS_OUTSIDE_AREA:
      return evaluateSkirmishPlayerIsOutsideArea(
          pCondition, pCondition->getParameter(0), pCondition->getParameter(1));

    case Condition::SKIRMISH_PLAYER_HAS_DISCOVERED_PLAYER:
      return evaluateSkirmishPlayerHasDiscoveredPlayer(
          pCondition->getParameter(0), pCondition->getParameter(1));

    case Condition::MUSIC_TRACK_HAS_COMPLETED:
      return evaluateMusicHasCompleted(pCondition->getParameter(0),
                                       pCondition->getParameter(1));

    case Condition::SUPPLY_SOURCE_SAFE:
      return evaluateSkirmishSupplySourceSafe(
          pCondition, pCondition->getParameter(0), pCondition->getParameter(1));
    ...
    case Condition::START_POSITION_IS:
      return evaluateSkirmishStartPosition(pCondition->getParameter(0),
                                           pCondition->getParameter(1));

    case Condition::PLAYER_LOST_OBJECT_TYPE:
      return evaluatePlayerLostObjectType(pCondition->getParameter(0),
                                          pCondition->getParameter(1));
  }
}
