...
// victory condition specifications Author: Matthew D. Campbell, February 2002
...
class VictoryConditions : public VictoryConditionsInterface {
 public:
  ...
  Bool hasAchievedVictory(
      Player *player);  ///< has a specific player and his allies won?
  Bool hasBeenDefeated(
      Player *player);  ///< has a specific player and his allies lost?
  Bool hasSinglePlayerBeenDefeated(
      Player *player);  ///< has a specific player lost?

  void cachePlayerPtrs(
      void);  ///< players have been created - cache the ones of interest
 ...
 private:
  ...
  Bool m_localPlayerDefeated;  ///< prevents condition from being signaled each
                               ///< frame
  Bool m_singleAllianceRemaining;  ///< prevents condition from being signaled
                                   ///< each frame
  Bool m_isObserver;
};
...
//-------------------------------------------------------------------------------------------------
VictoryConditions::VictoryConditions() { reset(); }

//-------------------------------------------------------------------------------------------------
void VictoryConditions::init(void) { reset(); }
...
void VictoryConditions::update(void) {
  ...
  for (Int i = 0; i < MAX_PLAYER_COUNT; ++i) {
    ...
    if (p && !m_isDefeated[i] && hasSinglePlayerBeenDefeated(p)) {
      ...
      if (TheGameLogic->getFrame() > 1) {
        ...
        TheInGameUI->message("GUI:PlayerHasBeenDefeated",
  ...
  if (!m_localPlayerDefeated && !m_isObserver) {
    ...
    if (hasSinglePlayerBeenDefeated(localPlayer)) {
      if (!m_singleAllianceRemaining) {
        // MessageBoxOk(TheGameText->fetch("GUI:Defeat"),
        // TheGameText->fetch("GUI:LocalDefeat"), NULL);
      }
      m_localPlayerDefeated = true;  // don't check again
      TheRadar->forceOn(TRUE);
      SetInGameChatType(
          INGAME_CHAT_EVERYONE);  // can't chat to allies after death.  Only to
                                  // other observers.
    }
  }
}
...
Bool VictoryConditions::hasBeenDefeated(Player *player) {
  ...
  if (m_singleAllianceRemaining && !hasAchievedVictory(player)) return true;
...
void VictoryConditions::cachePlayerPtrs(void) {
  if (!TheRecorder->isMultiplayer()) return;
  ...
  for (Int i = 0; i < MAX_PLAYER_COUNT; ++i) {
    ...
    DEBUG_LOG(("Checking whether to cache player %d - [%ls], house [%ls]\n", i,
               player ? player->getPlayerDisplayName().str() : L"<NOBODY>",
               (player && player->getPlayerTemplate())
                   ? player->getPlayerTemplate()->getDisplayName().str()
                   : L"<NONE>"));
    if (player && player != ThePlayerList->getNeutralPlayer() &&
        player->getPlayerTemplate() &&
        player->getPlayerTemplate() != civTemplate &&
        !player->isPlayerObserver()) {
      DEBUG_LOG(("Caching player\n"));
      m_players[playerCount] = player;
      if (m_players[playerCount]->isLocalPlayer()) m_localSlotNum = playerCount;
      ++playerCount;
    }
  }
  ...
  if (m_localSlotNum < 0) {
    m_localPlayerDefeated =
        true;  // if we have no local player, don't check for defeat
...
Bool VictoryConditions::isLocalAlliedDefeat(void) {
  if (m_isObserver) return m_singleAllianceRemaining;
