...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
// when you set controlling player or merge teams, we don't always update all
// the upgrade stuff or the indicator color. this allows us to force the
// situation. (srj)
static void updateTeamAndPlayerStuff(Object *obj, void *userData) {
  if (obj) {
    obj->updateUpgradeModules();

    // srj sez: apparently we have to do this too, since
    // Team::setControllingPlayer does not. Might make more sense to do it
    // there, but am scared to at this point.
...
/** Init */
//-------------------------------------------------------------------------------------------------
void ScriptActions::init(void) { reset(); }  // end init
...
void ScriptActions::doQuickVictory(void) {
  ...
  doDisableInput();
  if (TheCampaignManager) TheCampaignManager->SetVictorious(TRUE);
...
void ScriptActions::doSetInfantryLightingOverride(Real setting) {
  DEBUG_ASSERTCRASH(
      (setting == -1.0f) || (setting > 0.0f),
      ("Invalid setting (%d) in Infantry Lighting Override script.", setting));
...
void ScriptActions::doVictory(void) {
  ...
  if (!m_suppressNewWindows) {
    ...
    Bool showObserverWindow = localPlayer->isPlayerObserver() ||
                              TheScriptEngine->hasShownMPLocalDefeatWindow();
    if (showObserverWindow)
      m_messageWindow =
          TheWindowManager->winCreateFromScript("Menus/ObserverQuit.wnd");
    else {
      m_messageWindow =
          TheWindowManager->winCreateFromScript("Menus/Victorious.wnd");
    }
  }
  if (TheCampaignManager) TheCampaignManager->SetVictorious(TRUE);
...
void ScriptActions::doDefeat(void) {
  ...
  if (!m_suppressNewWindows) {
    ...
    if (showObserverWindow)
      ...
      m_messageWindow =
          TheWindowManager->winCreateFromScript("Menus/Defeat.wnd");
    }
  }
  if (TheCampaignManager) TheCampaignManager->SetVictorious(FALSE);
  TheScriptEngine->startEndGameTimer();
}
...
void ScriptActions::doLocalDefeat(void) {
  ...
  if (!m_suppressNewWindows) {
    if (!TheVictoryConditions->amIObserver())
      m_messageWindow =
          TheWindowManager->winCreateFromScript("Menus/LocalDefeat.wnd");
  }
  if (TheCampaignManager) TheCampaignManager->SetVictorious(FALSE);
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::changeObjectPanelFlagForSingleObject(
    Object *obj, const AsciiString &flagToChange, Bool newVal) {
  // Enabled flag
...
void ScriptActions::doDamageTeamMembers(const AsciiString &team, Real amount) {
  ...
  // The team is the team based on the name, and the calling team (if any) and
  // the team that triggered the condition.  jba. :)
  if (theTeam) {
...
void ScriptActions::doCameraFollowNamed(const AsciiString &unit,
  ...
  if (theObj) {
    ...
    if (snapToUnit) TheTacticalView->snapToCameraLock();
...
void ScriptActions::doCreateReinforcements(const AsciiString &team,
  ...
  // Create the transport first (if applicable), so we can determine if it has
  // paradrop capabilities. If so, we'll be doing a lot of things differently!
  ...
  // Check to see if we have a transport, and if our transport has paradrop
  // capabilities. If this is the case, we'll need to create each unit inside
  ...
  if (transport) {
    dp = (DeliverPayloadAIUpdate *)transport->findUpdateModule(
        key_DeliverPayloadAIUpdate);
  }

  // Our tranport has a deliverPayload update module. This means it'll do
  // airborned drops.
  ...
  for (i = 0; i < pInfo->m_numUnitsInfo; i++) {
    ...
    unitTemplate =
        TheThingFactory->findTemplate(pInfo->m_unitsInfo[i].unitThingName);
    Coord3D pos = origin;
  ...
  if (pInfo->m_teamStartsFull) {
    ...
    for (DLINK_ITERATOR<Object> iter = theTeam->iterate_TeamMemberList();
      ...
      if (obj == transport) {
        continue;  // Skip the one we created.  The team loads into the
                   // transports on the team for team starts full. jba
      }
      if (obj->isKindOf(KINDOF_TRANSPORT)) {
        ContainModuleInterface *contain = obj->getContain();
        if (contain) {
          vecOfTransports.push_back(std::make_pair(
              obj->getID(), ((TransportContain *)contain)->getContainMax()));
        } else {
          DEBUG_CRASH(
              ("doCreateReinforcement script -- transport doesn't have contain "
               "to hold guys."));
        }
      } else {
    ...
    PartitionSolver partition(vecOfUnits, vecOfTransports,
                              PREFER_FAST_SOLUTION);
    partition.solve();
  ...
  if (contain) {
    for (DLINK_ITERATOR<Object> iter = theTeam->iterate_TeamMemberList();
      ...
      // Check to see if it's a valid transport for this unit, even if it's
      // full.
      ...
      if (contain && contain->isValidContainerFor(obj, false)) {
        // Now that we know it fits in the transport, check to see if it's full.
        // If it is, then we'll create a new transport.
        ...
        // If our unit is going to be put in another container (such as infantry
        // being contained by a parachute) do so now.
        if (putInContainerTemplate) {
          Object *container =
              TheThingFactory->newObject(putInContainerTemplate, theTeam);
          container->setPosition(&pos);

          // Make sure this is valid.
          if (container->getContain() &&
              container->getContain()->isValidContainerFor(obj, true)) {
            container->getContain()->addToContain(obj);
            obj = container;
          } else {
            DEBUG_CRASH(
                ("doCreateReinforcements: PutInContainer %s is full, or not "
                 "valid for the payload %s!",
                 putInContainerTemplate->getName().str(),
    ...
    if (transport) {
      for (DLINK_ITERATOR<Object> iter = theTeam->iterate_TeamMemberList();
        ...
        if (obj->getTemplate()->isEquivalentTo(transport->getTemplate())) {
          ...
          // If there are any units that aren't transportable, move them to the
          // goal.
...
void ScriptActions::doMoveCameraTo(const AsciiString &waypoint, Real sec,
  ...
  for (Waypoint *way = TheTerrainLogic->getFirstWaypoint(); way;
       way = way->getNext()) {
    if (way->getName() == waypoint) {
      Coord3D destination = *way->getLocation();
      TheTacticalView->moveCameraTo(&destination, sec * 1000,
                                    cameraStutterSec * 1000, true);
      break;
    }
  }
}
...
void ScriptActions::doModCameraMoveToSelection(void) {
  ...
  for (Drawable *d = TheGameClient->firstDrawable(); d;
       d = d->getNextDrawable()) {
    if (d->isSelected()) {
...
void ScriptActions::doRotateCameraTowardObject(const AsciiString &unitName,
                                               Real sec, Real holdSec) {
  const Object *unit = TheScriptEngine->getUnitNamed(unitName);
  if (!unit) return;
  TheTacticalView->rotateCameraTowardObject(unit->getID(), sec * 1000,
                                            holdSec * 1000);
}
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doRotateCameraTowardWaypoint(
    const AsciiString &waypointName, Real sec) {
...
void ScriptActions::doMoveCameraAlongWaypointPath(const AsciiString &waypoint,
  ...
  for (Waypoint *way = TheTerrainLogic->getFirstWaypoint(); way;
       way = way->getNext()) {
    if (way->getName() == waypoint) {
      TheTacticalView->moveCameraAlongWaypointPath(
          way, sec * 1000, cameraStutterSec * 1000, true);
      break;
    }
  }
}
...
void ScriptActions::doCreateObject(const AsciiString &objectName,
                                   ...
                                   const AsciiString &teamName, Coord3D *pos,
                                   Real angle) {
  Object *pOldObj = NULL;
  ...
  if (!theTeam) {
    TheScriptEngine->AppendDebugMessage("***WARNING - Team not found:***",
                                        false);
...
void ScriptActions::doAttack(const AsciiString &attackerName,
  ...
  // sanity
  if (attackingTeam == NULL || victimTeam == NULL) return;
...
void ScriptActions::doNamedAttack(const AsciiString &attackerName,
  ...
  // tell every member of attacking team to attack a random member of victim
  // team

  {
    ...
    if (aiUpdate) {
      /// @todo Teams should have a method that returns the number of members in
      /// the team (MSB)
      attackingObj->leaveGroup();
      aiUpdate->chooseLocomotorSet(LOCOMOTORSET_NORMAL);
      aiUpdate->aiForceAttackObject(victimObj, NO_MAX_SHOTS_LIMIT,
                                    CMD_FROM_SCRIPT);
    }
  }
}
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::updateNamedAttackPrioritySet(
    const AsciiString &unitName, const AsciiString &attackPrioritySet) {
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::updateTeamAttackPrioritySet(
    const AsciiString &teamName, const AsciiString &attackPrioritySet) {
...
void ScriptActions::doNamedAttackArea(const AsciiString &unitName,
  ...
  PolygonTrigger *pTrig =
      TheScriptEngine->getQualifiedTriggerAreaByName(areaName);
  if (!pTrig) {
    return;
  }
  ...
  for (DLINK_ITERATOR<Object> iter = theTeam->iterate_TeamMemberList();
    ...
    if (obj->isKindOf(KINDOF_TRANSPORT)) {
      ...
      if (contain) {
        vecOfTransports.push_back(std::make_pair(
            obj->getID(),
            ((TransportContain *)obj->getContain())->getContainMax()));
      } else {
        DEBUG_CRASH(
            ("doLoadAllTransports script -- transport doesn't have a "
             "container!"));
      }
    } else {
      vecOfUnits.push_back(
          std::make_pair(obj->getID(), obj->getTransportSlotCount()));
    }
  }
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doNamedFollowWaypoints(
  ...
  Waypoint *way =
      TheTerrainLogic->getClosestWaypointOnPath(&pos, waypointPathLabel);
  if (!way) {
    return;
  }

  DEBUG_ASSERTLOG(TheTerrainLogic->isPurposeOfPath(way, waypointPathLabel),
                  ("***Wrong waypoint purpose. Make jba fix this.\n"));
...
void ScriptActions::doTeamFollowSkirmishApproachPath(
  ...
  for (DLINK_ITERATOR<Object> iter = theTeam->iterate_TeamMemberList();
    ...
    pos.x += objPos.x;
    pos.y += objPos.y;
    pos.z += objPos.z;  // Not actually used by getClosestWaypointOnPath, but
                        // hey, might as well be correct.
    count++;
    if (firstUnit == NULL) {
      firstUnit = obj;
    }
  ...
  DEBUG_ASSERTLOG(TheTerrainLogic->isPurposeOfPath(way, pathLabel),
                  ("***Wrong waypoint purpose. Make jba fix this.\n"));
  if (asTeam) {
...
void ScriptActions::doTeamHuntWithCommandButton(const AsciiString &teamName,
  ...
  switch (commandButton->getCommandType()) {
    case GUI_COMMAND_SPECIAL_POWER:
      if (commandButton->getSpecialPowerTemplate()) {
        if (BitTest(commandButton->getOptions(),
  ...
  for (DLINK_ITERATOR<Object> iter = theTeam->iterate_TeamMemberList();
    ...
    const CommandSet *commandSet =
        TheControlBar->findCommandSet(obj->getCommandSetString());
    if (commandSet) {
    ...
    switch (commandButton->getCommandType()) {
      ...
      case GUI_COMMAND_SPECIAL_POWER: {
        static NameKeyType key_CommandButtonHuntUpdate =
            NAMEKEY("CommandButtonHuntUpdate");

        CommandButtonHuntUpdate *huntUpdate =
            (CommandButtonHuntUpdate *)obj->findUpdateModule(
                key_CommandButtonHuntUpdate);
        if (huntUpdate) {
          huntUpdate->setCommandButton(ability);
        } else {
          AsciiString msg =
              "Error - Team hunt with command button - unit type '";
          msg.concat(obj->getTemplate()->getName().str());
          msg.concat(
              "' requires CommandButtonHuntUpdate in .ini definition to hunt "
              "with ");
          msg.concat(ability);
          TheScriptEngine->AppendDebugMessage(msg, false);
        }  // end if
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doPlayerDisableBaseConstruction(
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doPlayerDisableUnitConstruction(
    const AsciiString &playerName) {
  Player *pPlayer = TheScriptEngine->getPlayerFromAsciiString(playerName);
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doPlayerEnableBaseConstruction(
    const AsciiString &playerName) {
  Player *pPlayer = TheScriptEngine->getPlayerFromAsciiString(playerName);
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doPlayerEnableUnitConstruction(
    const AsciiString &playerName) {
  Player *pPlayer = TheScriptEngine->getPlayerFromAsciiString(playerName);
...
void ScriptActions::doTeamIncreasePriority(const AsciiString &teamName) {
  ...
  AsciiString msg;
  msg.format("Team '%s' priority increased to %d for success.", teamName.str(),
             theTeamProto->getTemplateInfo()->m_productionPriority);
  TheScriptEngine->AppendDebugMessage(msg, false);
}
...
void ScriptActions::doTeamDecreasePriority(const AsciiString &teamName) {
  ...
  AsciiString msg;
  msg.format("Team '%s' priority decreased to %d for failure.", teamName.str(),
             theTeamProto->getTemplateInfo()->m_productionPriority);
  TheScriptEngine->AppendDebugMessage(msg, false);
}
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doInGamePopupMessage(const AsciiString &message, Int x,
                                         Int y, Int width, Bool pause) {
...
void ScriptActions::doDisplayCinematicText(const AsciiString &displayText,
  ...
  *c++;  // eat through " - Size:"
  ...
  // get font fold
  Bool bold = FALSE;
  if (fontType.endsWith("[Bold]")) bold = TRUE;

  // phew, now set as new font
  GameFont *font = TheFontLibrary->getFont(
      fontName, TheGlobalLanguageData->adjustFontSize(size), bold);
...
void ScriptActions::doCameoFlash(const AsciiString &name, Int timeInSeconds) {
  ...
  if (button == NULL) {
    DEBUG_CRASH(
        ("ScriptActions::doCameoFlash can't find AsciiString cameoflash"));
  }

  Int frames = LOGICFRAMES_PER_SECOND * timeInSeconds;
  // every time the framecount % 20 == 0,  controlbar:: update will do Cameo
  // Flash
...
/** doNamedFlash */
//-------------------------------------------------------------------------------------------------
void ScriptActions::doNamedFlash(const AsciiString &unitName, Int timeInSeconds,
                                 const RGBColor *color) {
  /** This is called the first time this unit is told by the script to flash.
  timeInSeconds will tell the drawable how long to flash for.  Sets drawable to
  start flashing but only allows drawable's update to call the actual flash
  method */
  ...
  if (timeInSeconds > 0) {
    // set count for drawable, but do not flash, allow drawable update to handle
    // it

    /* The designer specifies how long he wants the unit to flash for.  We will
    convert this number into a count of how many times to call the doNamedFlash
    method */
    Int frames = LOGICFRAMES_PER_SECOND * timeInSeconds;
    // every time the framecount % 20 == 0, drawable::update will call
    // doNamedFlash
    Int count = frames / DRAWABLE_FRAMES_PER_FLASH;
    Color flashy =
        (color == NULL) ? obj->getIndicatorColor() : color->getAsInt();
...
void ScriptActions::doTeamFlash(const AsciiString &teamName, Int timeInSeconds,
                                const RGBColor *color) {
  Team *team = TheScriptEngine->getTeamNamed(teamName);
  if (team == NULL || !team->hasAnyObjects()) return;
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doPlayerTransferAssetsToPlayer(
    const AsciiString &playerSrcName, const AsciiString &playerDstName) {
  Player *pPlayerDest =
      TheScriptEngine->getPlayerFromAsciiString(playerDstName);
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doNamedTransferAssetsToPlayer(
    const AsciiString &unitName, const AsciiString &playerDstName) {
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::excludePlayerFromScoreScreen(
    const AsciiString &playerName) {
  Player *pPlayer = TheScriptEngine->getPlayerFromAsciiString(playerName);
  if (pPlayer == NULL) {
    return;
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::updatePlayerRelationTowardPlayer(
    const AsciiString &playerSrcName, Int relationType,
...
/** doRadarDisable */
//-------------------------------------------------------------------------------------------------
void ScriptActions::doRadarDisable(void) { TheRadar->hide(true); }

//-------------------------------------------------------------------------------------------------
/** doRadarEnable */
//-------------------------------------------------------------------------------------------------
void ScriptActions::doRadarEnable(void) { TheRadar->hide(false); }

//-------------------------------------------------------------------------------------------------
/** doCameraMotionBlurJump - zoom in at the current location, jump to waypoint,
 * and zoom out.*/
//-------------------------------------------------------------------------------------------------
void ScriptActions::doCameraMotionBlurJump(const AsciiString &waypointName,
  ...
  if (TheTacticalView->setViewFilter(FT_VIEW_MOTION_BLUR_FILTER)) {
    passed = TRUE;
    if (saturate) {
      if (!TheTacticalView->setViewFilterMode(
              FM_VIEW_MB_IN_AND_OUT_SATURATE)) {  // failed to set filter so
      ...
      if (!TheTacticalView->setViewFilterMode(
              FM_VIEW_MB_IN_AND_OUT_ALPHA)) {  // failed to set filter so
                                               // restore default state
        TheTacticalView->setViewFilter(FT_NULL_FILTER);
        passed = FALSE;
      };
    }
    if (passed) TheTacticalView->setViewFilterPos(&pos);
  }
  if (!passed) {  // if we failed to apply the filter, we still need to get the
...
void ScriptActions::doCameraMotionBlur(Bool zoomIn, Bool saturate) {
  if (TheTacticalView->setViewFilter(FT_VIEW_MOTION_BLUR_FILTER)) {
    ...
    if (!TheTacticalView->setViewFilterMode(
            mode)) {  // failed to set the filter so restore everything to
                      // normal
      TheTacticalView->setViewFilter(FT_NULL_FILTER);
    }
  }
...
void ScriptActions::doRevealMapAtWaypoint(const AsciiString &waypointName,
  ...
  // A reveal script is a quick look.  That way a Radar Jammer will still
  // function correctly.
  ThePartitionManager->doShroudReveal(positionX, positionY, radiusToReveal,
                                      playerMask);
  ThePartitionManager->undoShroudReveal(positionX, positionY, radiusToReveal,
                                        playerMask);
}
...
void ScriptActions::doShroudMapAtWaypoint(const AsciiString &waypointName,
  ...
  ThePartitionManager->doShroudCover(positionX, positionY, radiusToShroud,
                                     playerMask);
  ThePartitionManager->undoShroudCover(positionX, positionY, radiusToShroud,
                                       playerMask);
}
...
void ScriptActions::doRevealMapEntire(const AsciiString &playerName) {
  ...
  if (player && playerName.isNotEmpty()) {
    DEBUG_LOG(
        ("ScriptActions::doRevealMapEntire() for player named '%ls' in "
         "position %d\n",
         player->getPlayerDisplayName().str(), player->getPlayerIndex()));
    ThePartitionManager->revealMapForPlayer(player->getPlayerIndex());
  } else {
    DEBUG_LOG(
        ("ScriptActions::doRevealMapEntire() - no player, so doing all human "
         "players\n"));
    for (Int i = 0; i < ThePlayerList->getPlayerCount(); ++i) {
...
void ScriptActions::doRevealMapEntirePermanently(
    Bool reveal, const AsciiString &playerName) {
  Player *player = TheScriptEngine->getPlayerFromAsciiString(playerName);
  if (player && playerName.isNotEmpty()) {
    if (reveal)
      ThePartitionManager->revealMapForPlayerPermanently(
          player->getPlayerIndex());
    else
      ThePartitionManager->undoRevealMapForPlayerPermanently(
          player->getPlayerIndex());
  } else {
    for (Int i = 0; i < ThePlayerList->getPlayerCount(); ++i) {
      Player *player = ThePlayerList->getNthPlayer(i);
      if (player->getPlayerType() == PLAYER_HUMAN) {
        DEBUG_LOG(
            ("ScriptActions::doRevealMapEntirePermanently() for player %d\n",
             i));
        if (reveal)
...
void ScriptActions::doCollectNearbyForTeam(const AsciiString &teamName) {
  DEBUG_CRASH(
      ("You would think this has been implemented, but you'd be wrong. "
       "(doCollectNearbyForTeam)"));
}
...
void ScriptActions::doMergeTeamIntoTeam(const AsciiString &teamSrcName,
  ...
  while (!iter.done()) {
    ...
    // this has to be done here, setting the team will screw up the iterator.
    // total bummer dude. jkmcd
    nextObj = iter.cur();
    iter.advance();
...
void ScriptActions::doResumeSupplyTruckingForIdleUnits(
  ...
  if (player && playerName.isNotEmpty()) {
    ...
    for (Int i = 0; i < ThePlayerList->getPlayerCount(); ++i) {
      ...
      if (player->getPlayerType() == PLAYER_HUMAN) {
        DEBUG_LOG(
            ("ScriptActions::doResumeSupplyTruckingForIdleUnits() for player "
             "%d\n",
             i));
        player->setUnitsShouldIdleOrResume(false);
      }
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doAmbientSoundsPause(
    Bool pausing)  // if true, then pause, if false then resume.
{
...
void ScriptActions::doTeamGarrisonSpecificBuilding(
  ...
  if (!theBuilding->getContain()) {
    DEBUG_CRASH(
        ("doTeamGarrisonSpecificBuilding script -- building doesn't have a "
         "container!"));
    return;
  }
...
void ScriptActions::doTeamGarrisonNearestBuilding(const AsciiString &teamName) {
  ...
  PartitionFilterGarrisonableByPlayer f1(theTeam->getControllingPlayer(), true,
                                         CMD_FROM_SCRIPT);
  ...
  ObjectIterator *iter = ThePartitionManager->iterateObjectsInRange(
      leader, REALLY_FAR, FROM_CENTER_3D, filters, ITER_SORTED_NEAR_TO_FAR);
  MemoryPoolObjectHolder hold(iter);

  // here's what we do. Find out how many slots each building has open, and tell
  // each unit individually to garrison a specific building. We won't use the
  // partition solver because we've already done most of the work
...
void ScriptActions::doUnitGarrisonSpecificBuilding(
  ...
  if (!contain) {
    DEBUG_CRASH(
        ("doUnitGarrisonSpecificBuilding script -- building doesn't have a "
         "container"));
    return;
  }
  PlayerMaskType player = theBuilding->getContain()->getPlayerWhoEntered();
...
void ScriptActions::doUnitGarrisonNearestBuilding(const AsciiString &unitName) {
  ...
  PartitionFilterAcceptByKindOf f1(MAKE_KINDOF_MASK(KINDOF_STRUCTURE),
                                   KINDOFMASK_NONE);
  PartitionFilterSameMapStatus filterMapStatus(theUnit);
  PartitionFilter *filters[] = {&f1, &filterMapStatus, NULL};

  ObjectIterator *iter = ThePartitionManager->iterateObjectsInRange(
      theUnit, REALLY_FAR, FROM_CENTER_3D, filters, ITER_SORTED_NEAR_TO_FAR);
  MemoryPoolObjectHolder hold(iter);
  ...
  for (Object *theBuilding = iter->first(); theBuilding;
    ...
    if (!contain) {
      DEBUG_CRASH(
          ("doUnitGarrisonNearestBuilding script -- building doesn't have a "
           "container."));
      continue;
    }
    PlayerMaskType player = theBuilding->getContain()->getPlayerWhoEntered();
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doPlayerGarrisonAllBuildings(
    const AsciiString &playerName) {
  Player *player = TheScriptEngine->getPlayerFromAsciiString(playerName);
...
void ScriptActions::doBlackWhiteMode(Bool startBWMode, Int frames) {
  if (startBWMode) {
    ...
    if ((TheTacticalView->getViewFilterType()) ==
        FT_VIEW_BW_FILTER) {  // mode already set, turn it off
      TheTacticalView->setFadeParameters(frames, -1);
    }
  }
}
...
/** doSkyBox */
//-------------------------------------------------------------------------------------------------
extern void doSkyBoxSet(
    Bool startDraw);  // hack to avoid including globaldata here.
...
/** Freeze time */
//-------------------------------------------------------------------------------------------------
void ScriptActions::doFreezeTime(void) { TheScriptEngine->doFreezeTime(); }

//-------------------------------------------------------------------------------------------------
/** Unfreeze time */
//-------------------------------------------------------------------------------------------------
void ScriptActions::doUnfreezeTime(void) { TheScriptEngine->doUnfreezeTime(); }
...
void ScriptActions::doMilitaryCaption(const AsciiString &briefing,
                                      Int duration) {
#if defined(_DEBUG) || defined(_INTERNAL)
  if (TheGlobalData->m_disableMilitaryCaption) duration = 1;
#endif
...
//-------------------------------------------------------------------------------------------------
/** Set the audible distance for shots in which the camera is up, and is
...
void ScriptActions::doTransferTeamToPlayer(const AsciiString &teamName,
  ...
  // srj sez: ensure that all these guys get the upgrades that belong to the new
  // player
...
void ScriptActions::doDisplayCounter(const AsciiString &counterName,
  ...
  TheInGameUI->addNamedTimer(counterName, TheGameText->fetch(counterText),
                             false);
}
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doNamedStopSpecialPowerCountdown(
    const AsciiString &unit, const AsciiString &specialPower, Bool stop) {
  Object *theObj = TheScriptEngine->getUnitNamed(unit);
  const SpecialPowerTemplate *power =
      TheSpecialPowerStore->findSpecialPowerTemplate(specialPower);
  if (theObj && power) {
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doNamedSetSpecialPowerCountdown(
    const AsciiString &unit, const AsciiString &specialPower, Int seconds) {
  Object *theObj = TheScriptEngine->getUnitNamed(unit);
  const SpecialPowerTemplate *power =
      TheSpecialPowerStore->findSpecialPowerTemplate(specialPower);
  if (theObj && power) {
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doNamedAddSpecialPowerCountdown(
    const AsciiString &unit, const AsciiString &specialPower, Int seconds) {
  Object *theObj = TheScriptEngine->getUnitNamed(unit);
  const SpecialPowerTemplate *power =
      TheSpecialPowerStore->findSpecialPowerTemplate(specialPower);
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doNamedFireSpecialPowerAtWaypoint(
    const AsciiString &unit, const AsciiString &specialPower,
    const AsciiString &waypoint) {
  Object *theObj = TheScriptEngine->getUnitNamed(unit);
  const SpecialPowerTemplate *power =
      TheSpecialPowerStore->findSpecialPowerTemplate(specialPower);
  if (theObj && power) {
    ...
    if (mod) {
      ...
      mod->doSpecialPowerAtLocation(way->getLocation(),
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doNamedFireSpecialPowerAtNamed(
    const AsciiString &unit, const AsciiString &specialPower,
  ...
  if (commandSet) {
    for (Int i = 0; i < MAX_COMMANDS_PER_SET; i++) {
      ...
      if (commandButton) {
        if (!commandButton->getName().isEmpty()) {
          if (commandButton->getName() == ability) {
            theObj->doCommandButtonAtPosition(
                commandButton, pWaypoint->getLocation(), CMD_FROM_SCRIPT);
          }
        }
      }
    }
...
void ScriptActions::doTeamUseCommandButtonAbilityOnNamed(
    const AsciiString &team, const AsciiString &ability,
    const AsciiString &target) {
  Team *theTeam = TheScriptEngine->getTeamNamed(team);
  // The team is the team based on the name, and the calling team (if any) and
  // the team that triggered the condition.  jba. :)
  ...
  theGroup->groupDoCommandButtonAtObject(commandButton, theObj,
                                         CMD_FROM_SCRIPT);
}
...
void ScriptActions::doTeamUseCommandButtonAbilityAtWaypoint(
  ...
  theGroup->groupDoCommandButtonAtPosition(
      commandButton, pWaypoint->getLocation(), CMD_FROM_SCRIPT);
}
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doTeamSetOverrideRelationToTeam(
    const AsciiString &teamName, const AsciiString &otherTeam, Int relation) {
  Team *theTeam = TheScriptEngine->getTeamNamed(teamName);
  Team *theOtherTeam = TheScriptEngine->getTeamNamed(otherTeam);
  if (theTeam && theOtherTeam) {
    theTeam->setOverrideTeamRelationship(theOtherTeam->getID(),
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doTeamRemoveOverrideRelationToTeam(
    const AsciiString &teamName, const AsciiString &otherTeam) {
  Team *theTeam = TheScriptEngine->getTeamNamed(teamName);
  Team *theOtherTeam = TheScriptEngine->getTeamNamed(otherTeam);
  if (theTeam && theOtherTeam) {
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doPlayerRemoveOverrideRelationToTeam(
    const AsciiString &playerName, const AsciiString &otherTeam) {
  Player *thePlayer = ThePlayerList->findPlayerWithNameKey(NAMEKEY(playerName));
  Team *theOtherTeam = TheScriptEngine->getTeamNamed(otherTeam);
  if (thePlayer && theOtherTeam) {
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doTeamSetOverrideRelationToPlayer(
    const AsciiString &teamName, const AsciiString &otherPlayer, Int relation) {
  Team *theTeam = TheScriptEngine->getTeamNamed(teamName);
  Player *theOtherPlayer =
      ThePlayerList->findPlayerWithNameKey(NAMEKEY(otherPlayer));
  if (theTeam && theOtherPlayer) {
    theTeam->setOverridePlayerRelationship(theOtherPlayer->getPlayerIndex(),
                                           (Relationship)relation);
  }
}
...
/** doTeamRemoveOverrideRelationToTeam */
//-------------------------------------------------------------------------------------------------
void ScriptActions::doTeamRemoveOverrideRelationToPlayer(
    const AsciiString &teamName, const AsciiString &otherPlayer) {
  Team *theTeam = TheScriptEngine->getTeamNamed(teamName);
  Player *theOtherPlayer =
      ThePlayerList->findPlayerWithNameKey(NAMEKEY(otherPlayer));
  if (theTeam && theOtherPlayer) {
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doTeamRemoveAllOverrideRelations(
    const AsciiString &teamName) {
  Team *theTeam = TheScriptEngine->getTeamNamed(teamName);
  if (theTeam) {
...
/** doNamedFireWeaponFollowingWaypointPath -- Kris
                Orders unit to fire a waypoint following capable weapon to
   follow a waypoint and attack the final waypoint position. */
//-------------------------------------------------------------------------------------------------
void ScriptActions::doNamedFireWeaponFollowingWaypointPath(
  ...
  // We have to do special checking to make sure our unit even has a waypoint
  // following capable weapon.
  ...
  if (projectile) {
    ...
    DEBUG_ASSERTLOG(TheTerrainLogic->isPurposeOfPath(way, waypointPath),
                    ("***Wrong waypoint purpose. Make jba fix this.\n"));
...
void ScriptActions::doBorderSwitch(Int borderToUse) {
  /*
   *	The observer player always has to have the map completely revealed for
   *him. Border switching doesn't play nice with the permanent map reveal so for
   *the observer player we need to undo the old permanent reveal, switch map
   *borders, and re-reveal the map. BGC
   */
  Int observerPlayerIndex = -1;
  if (ThePlayerList != NULL) {
    Player *observer = ThePlayerList->findPlayerWithNameKey(
        TheNameKeyGenerator->nameToKey("ReplayObserver"));
...
void ScriptActions::doForceObjectSelection(const AsciiString &teamName,
  ...
  for (DLINK_ITERATOR<Object> iter = team->iterate_TeamMemberList();
    ...
    if (obj->getTemplate() && obj->getTemplate()->getName() == objectType) {
      if (bestGuess == NULL ||
          obj->getID() < bestGuess->getID()) {  // lower ID means its newer
        bestGuess = obj;
      }
    }
  }
...
/** doRadarForceEnable */
//-------------------------------------------------------------------------------------------------
void ScriptActions::doRadarForceEnable(void) { TheRadar->forceOn(true); }

//-------------------------------------------------------------------------------------------------
/** doRadarRevertNormal */
//-------------------------------------------------------------------------------------------------
void ScriptActions::doRadarRevertNormal(void) { TheRadar->forceOn(false); }
...
void ScriptActions::doModifyBuildableStatus(const AsciiString &objectType,
  ...
  TheGameLogic->setBuildableStatusOverride(templ,
...
void ScriptActions::doSetWarehouseValue(const AsciiString &warehouseName,
  ...
  SupplyWarehouseDockUpdate *warehouseModule =
      (SupplyWarehouseDockUpdate *)obj->findUpdateModule(warehouseModuleKey);
  if (warehouseModule == NULL) return;
...
void ScriptActions::doMoveUnitTowardsNearest(const AsciiString &unitName,
  ...
  PartitionFilter *filters[] = {&thingsToAccept, &acceptWithin,
                                &filterMapStatus, NULL};

  Object *dest = ThePartitionManager->getClosestObject(
      obj->getPosition(), REALLY_FAR, FROM_CENTER_2D, filters);
  if (!dest) {
    return;
  }
...
void ScriptActions::doUnitReceiveUpgrade(const AsciiString &unitName,
  ...
  DEBUG_ASSERTCRASH(
      obj->affectedByUpgrade(templ),
      ("Design bug: Unit '%s' was given upgrade '%s', but he is unaffected.",
       unitName.str(), upgradeName.str()));
...
void ScriptActions::doSkirmishAttackNearestGroupWithValue(
  ...
  if (comparison == Parameter::GREATER_EQUAL ||
      comparison == Parameter::GREATER) {
    ThePartitionManager->getNearestGroupWithValue(player->getPlayerIndex(),
                                                  ALLOW_ENEMIES, VOT_CashValue,
                                                  &groupLoc, value, true, &loc);
  }

  theGroup->groupAttackMoveToPosition(&loc, NO_MAX_SHOTS_LIMIT,
                                      CMD_FROM_SCRIPT);
}
...
void ScriptActions::doSkirmishCommandButtonOnMostValuable(
  ...
  if (commandButton->getSpecialPowerTemplate()) {
    ...
    srcObj =
        theGroup->getCommandButtonSourceObject(commandButton->getCommandType());
  }
  ...
  PartitionFilterPlayerAffiliation f1(team->getControllingPlayer(),
                                      ALLOW_ENEMIES, true);
  PartitionFilterValidCommandButtonTarget f2(srcObj, commandButton, true,
                                             CMD_FROM_SCRIPT);
  PartitionFilterSameMapStatus filterMapStatus(srcObj);

  PartitionFilter *filters[] = {&f1, &f2, &filterMapStatus, 0};
  // @todo: Should we add the group's radius to the range? Seems like a
  // possibility.
  ...
  if (iter && iter->first()) {
    theGroup->groupDoCommandButtonAtObject(commandButton, iter->first(),
                                           CMD_FROM_SCRIPT);
  }
}
...
void ScriptActions::doTeamUseCommandButtonOnNearestEnemy(
  ...
  PartitionFilter *filters[] = {&f1, &f2, &filterMapStatus, 0};
  Object *obj = ThePartitionManager->getClosestObject(&pos, REALLY_FAR,
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doTeamUseCommandButtonOnNearestGarrisonedBuilding(
    const AsciiString &teamName, const AsciiString &commandAbility) {
  Team *team = TheScriptEngine->getTeamNamed(teamName);
  if (!team) {
    return;
  ...
  PartitionFilterPlayerAffiliation f1(team->getControllingPlayer(),
                                      ALLOW_ENEMIES, true);
  PartitionFilterAcceptByKindOf f2(MAKE_KINDOF_MASK(KINDOF_STRUCTURE),
                                   KINDOFMASK_NONE);
  PartitionFilterGarrisonable f3(true);
  PartitionFilterValidCommandButtonTarget f4(srcObj, commandButton, true,
                                             CMD_FROM_SCRIPT);
  PartitionFilterSameMapStatus filterMapStatus(srcObj);
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doTeamUseCommandButtonOnNearestKindof(
    const AsciiString &teamName, const AsciiString &commandAbility,
    Int kindofBit) {
  Team *team = TheScriptEngine->getTeamNamed(teamName);
  if (!team) {
  ...
  PartitionFilterPlayerAffiliation f1(team->getControllingPlayer(),
                                      ALLOW_ENEMIES, true);
  PartitionFilterAcceptByKindOf f2(MAKE_KINDOF_MASK(kindofBit),
                                   KINDOFMASK_NONE);
  PartitionFilterValidCommandButtonTarget f3(srcObj, commandButton, true,
                                             CMD_FROM_SCRIPT);
  PartitionFilterSameMapStatus filterMapStatus(srcObj);
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doTeamUseCommandButtonOnNearestBuilding(
    const AsciiString &teamName, const AsciiString &commandAbility) {
  Team *team = TheScriptEngine->getTeamNamed(teamName);
  if (!team) {
    return;
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doTeamUseCommandButtonOnNearestBuildingClass(
    const AsciiString &teamName, const AsciiString &commandAbility,
    Int kindofBit) {
  Team *team = TheScriptEngine->getTeamNamed(teamName);
  if (!team) {
  ...
  PartitionFilterAcceptByKindOf f2(MAKE_KINDOF_MASK(KINDOF_STRUCTURE),
                                   KINDOFMASK_NONE);
  PartitionFilterAcceptByKindOf f3(MAKE_KINDOF_MASK(kindofBit),
                                   KINDOFMASK_NONE);
  PartitionFilterValidCommandButtonTarget f4(srcObj, commandButton, true,
                                             CMD_FROM_SCRIPT);
  PartitionFilterSameMapStatus filterMapStatus(srcObj);
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doTeamPartialUseCommandButton(
    Real percentage, const AsciiString &teamName,
  ...
  Int count = 0;
  for (std::vector<Object *>::const_iterator it = objList.begin();
       it != objList.end(); ++it) {
    Object *obj = (*it);
...
void ScriptActions::doMapSetRankLevelLimit(Int level) {
  if (TheGameLogic) TheGameLogic->setRankLevelLimit(level);
}
...
void ScriptActions::doPlayerGrantScience(const AsciiString &playerName,
  ...
  if (!pPlayer) return;
  ScienceType science =
      TheScienceStore->getScienceFromInternalName(scienceName);
  if (science == SCIENCE_INVALID) return;
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doPlayerSetScienceAvailability(
    const AsciiString &playerName, const AsciiString &scienceName,
    const AsciiString &scienceAvailability) {
  Player *player = TheScriptEngine->getPlayerFromAsciiString(playerName);
  if (player) {
    ScienceAvailabilityType saType =
        player->getScienceAvailabilityTypeFromString(scienceAvailability);
...
//-------------------------------------------------------------------------------------------------
// if addObject, we're adding an object to a list. If not addObject, we're
// removing the associated object.
//-------------------------------------------------------------------------------------------------
void ScriptActions::doObjectTypeListMaintenance(const AsciiString &objectList,
                                                const AsciiString &objectType,
                                                Bool addObject) {
  TheScriptEngine->doObjectTypeListMaintenance(objectList, objectType,
                                               addObject);
}

//-------------------------------------------------------------------------------------------------
void ScriptActions::doRevealMapAtWaypointPermanent(
    const AsciiString &waypointName, Real radiusToReveal,
    const AsciiString &playerName, const AsciiString &lookName) {
  TheScriptEngine->createNamedMapReveal(lookName, waypointName, radiusToReveal,
                                        playerName);
  TheScriptEngine->doNamedMapReveal(lookName);
}

//-------------------------------------------------------------------------------------------------
void ScriptActions::doUndoRevealMapAtWaypointPermanent(
...
void ScriptActions::doTeamFaceNamed(const AsciiString &teamName,
  ...
  if (team) {
    ...
    if (faceObj) {
      DLINK_ITERATOR<Object> iter = team->iterate_TeamMemberList();
      for (iter = team->iterate_TeamMemberList(); !iter.done();
           iter.advance()) {
        Object *obj = iter.cur();
        if (obj) {
          AIUpdateInterface *ai = obj->getAI();
...
void ScriptActions::doRemoveCommandBarButton(const AsciiString &buttonName,
  ...
  const CommandSet *cs =
      TheControlBar->findCommandSet(templ->friend_getCommandSetString());
  if (!cs) {
    return;
  }
  ...
  Int slotNum = -1;
  for (Int i = 0; i < MAX_COMMANDS_PER_SET; ++i) {
    if (cs->getCommandButton(i) &&
        (cs->getCommandButton(i)->getName() == buttonName)) {
      slotNum = i;
      break;
    }
  }
  ...
  if (slotNum >= 0) {
    TheGameLogic->setControlBarOverride(templ->friend_getCommandSetString(),
                                        slotNum, NULL);
  }
}

//-------------------------------------------------------------------------------------------------
void ScriptActions::doAddCommandBarButton(const AsciiString &buttonName,
                                          const AsciiString &objectType,
                                          Int slotNum) {
  const ThingTemplate *templ = TheThingFactory->findTemplate(objectType);
  if (!templ) {
  ...
  const CommandButton *commandButton =
      TheControlBar->findCommandButton(buttonName);
  ...
  if (slotNum < 0 || slotNum >= MAX_COMMANDS_PER_SET) return;

  TheGameLogic->setControlBarOverride(templ->friend_getCommandSetString(),
                                      slotNum, commandButton);
}
...
//-------------------------------------------------------------------------------------------------
void ScriptActions::doEnableOrDisableObjectDifficultyBonuses(
    Bool enableBonuses) {
...
void ScriptActions::executeAction(ScriptAction *pAction) {
  switch (pAction->getActionType()) {
    ...
    case ScriptAction::DAMAGE_MEMBERS_OF_TEAM:
      doDamageTeamMembers(pAction->getParameter(0)->getString(),
                          pAction->getParameter(1)->getReal());
      return;
    case ScriptAction::MOVE_TEAM_TO:
      doMoveToWaypoint(pAction->getParameter(0)->getString(),
                       pAction->getParameter(1)->getString());
      return;
    case ScriptAction::MOVE_NAMED_UNIT_TO:
      doNamedMoveToWaypoint(pAction->getParameter(0)->getString(),
                            pAction->getParameter(1)->getString());
      return;
    case ScriptAction::TEAM_SET_STATE:
      doSetTeamState(pAction->getParameter(0)->getString(),
                     pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::CREATE_REINFORCEMENT_TEAM:
      doCreateReinforcements(pAction->getParameter(0)->getString(),
                             pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::AI_PLAYER_BUILD_SUPPLY_CENTER:
      doBuildSupplyCenter(pAction->getParameter(0)->getString(),
                          pAction->getParameter(1)->getString(),
                          pAction->getParameter(2)->getInt());
      return;
    case ScriptAction::AI_PLAYER_BUILD_UPGRADE:
      doBuildUpgrade(pAction->getParameter(0)->getString(),
                     pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::RECRUIT_TEAM:
      doRecruitTeam(pAction->getParameter(0)->getString(),
                    pAction->getParameter(1)->getReal());
      return;
    ...
    case ScriptAction::MOVE_CAMERA_TO:
      doMoveCameraTo(pAction->getParameter(0)->getString(),
                     pAction->getParameter(1)->getReal(),
                     pAction->getParameter(2)->getReal());
      return;
    case ScriptAction::SETUP_CAMERA:
      doSetupCamera(pAction->getParameter(0)->getString(),
                    pAction->getParameter(1)->getReal(),
                    pAction->getParameter(2)->getReal(),
                    pAction->getParameter(3)->getString());
      return;
    case ScriptAction::ZOOM_CAMERA:
      doZoomCamera(pAction->getParameter(0)->getReal(),
                   pAction->getParameter(1)->getReal());
      return;
    case ScriptAction::PITCH_CAMERA:
      doPitchCamera(pAction->getParameter(0)->getReal(),
                    pAction->getParameter(1)->getReal());
      return;
    case ScriptAction::CAMERA_FOLLOW_NAMED:
      doCameraFollowNamed(pAction->getParameter(0)->getString(),
                          (pAction->getParameter(1) &&
                           pAction->getParameter(1)->getInt() != 0));
      return;
    ...
    case ScriptAction::MOVE_CAMERA_ALONG_WAYPOINT_PATH:
      doMoveCameraAlongWaypointPath(pAction->getParameter(0)->getString(),
                                    pAction->getParameter(1)->getReal(),
                                    pAction->getParameter(2)->getReal());
      return;
    case ScriptAction::ROTATE_CAMERA:
      doRotateCamera(pAction->getParameter(0)->getReal(),
                     pAction->getParameter(1)->getReal());
      return;
    case ScriptAction::CAMERA_LOOK_TOWARD_OBJECT:
      doRotateCameraTowardObject(pAction->getParameter(0)->getString(),
                                 pAction->getParameter(1)->getReal(),
                                 pAction->getParameter(2)->getReal());
      return;
    case ScriptAction::CAMERA_LOOK_TOWARD_WAYPOINT:
      doRotateCameraTowardWaypoint(pAction->getParameter(0)->getString(),
                                   pAction->getParameter(1)->getReal());
      return;
    case ScriptAction::RESET_CAMERA:
      doResetCamera(pAction->getParameter(0)->getString(),
                    pAction->getParameter(1)->getReal());
      return;
    ...
    case ScriptAction::CAMERA_MOD_SET_FINAL_SPEED_MULTIPLIER:
      TheTacticalView->cameraModFinalTimeMultiplier(
          pAction->getParameter(0)->getInt());
      return;
    case ScriptAction::CAMERA_MOD_SET_ROLLING_AVERAGE:
      TheTacticalView->cameraModRollingAverage(
          pAction->getParameter(0)->getInt());
      return;
    ...
    case ScriptAction::PLAY_SOUND_EFFECT_AT:
      doPlaySoundEffectAt(pAction->getParameter(0)->getString(),
                          pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::CREATE_OBJECT: {
      ...
      doCreateObject(m_unnamedUnit, pAction->getParameter(0)->getString(),
                     pAction->getParameter(1)->getString(), &pos,
                     pAction->getParameter(3)->getReal());
      return;
    }

    case ScriptAction::TEAM_ATTACK_TEAM:
      doAttack(pAction->getParameter(0)->getString(),
               pAction->getParameter(1)->getString());
      return;
    case ScriptAction::NAMED_ATTACK_NAMED:
      doNamedAttack(pAction->getParameter(0)->getString(),
                    pAction->getParameter(1)->getString());
      return;
    case ScriptAction::CREATE_NAMED_ON_TEAM_AT_WAYPOINT:
      createUnitOnTeamAt(pAction->getParameter(0)->getString(),
                         pAction->getParameter(1)->getString(),
                         pAction->getParameter(2)->getString(),
                         pAction->getParameter(3)->getString());
      return;
    case ScriptAction::CREATE_UNNAMED_ON_TEAM_AT_WAYPOINT:
      createUnitOnTeamAt(AsciiString::TheEmptyString,
    ...
    case ScriptAction::NAMED_APPLY_ATTACK_PRIORITY_SET:
      updateNamedAttackPrioritySet(pAction->getParameter(0)->getString(),
                                   pAction->getParameter(1)->getString());
      return;
    case ScriptAction::TEAM_APPLY_ATTACK_PRIORITY_SET:
      updateTeamAttackPrioritySet(pAction->getParameter(0)->getString(),
                                  pAction->getParameter(1)->getString());
      return;
    case ScriptAction::SET_BASE_CONSTRUCTION_SPEED:
      updateBaseConstructionSpeed(pAction->getParameter(0)->getString(),
                                  pAction->getParameter(1)->getInt());
      return;
    case ScriptAction::NAMED_SET_ATTITUDE:
      updateNamedSetAttitude(pAction->getParameter(0)->getString(),
                             pAction->getParameter(1)->getInt());
      return;
    case ScriptAction::TEAM_SET_ATTITUDE:
      updateTeamSetAttitude(pAction->getParameter(0)->getString(),
                            pAction->getParameter(1)->getInt());
      return;
    case ScriptAction::NAMED_SET_REPULSOR:
      doNamedSetRepulsor(pAction->getParameter(0)->getString(),
                         pAction->getParameter(1)->getInt());
      return;
    case ScriptAction::TEAM_SET_REPULSOR:
      doTeamSetRepulsor(pAction->getParameter(0)->getString(),
                        pAction->getParameter(1)->getInt());
      return;
    case ScriptAction::NAMED_ATTACK_AREA:
      doNamedAttackArea(pAction->getParameter(0)->getString(),
                        pAction->getParameter(1)->getString());
      return;
    case ScriptAction::NAMED_ATTACK_TEAM:
      doNamedAttackTeam(pAction->getParameter(0)->getString(),
                        pAction->getParameter(1)->getString());
      return;
    case ScriptAction::TEAM_ATTACK_AREA:
      doTeamAttackArea(pAction->getParameter(0)->getString(),
                       pAction->getParameter(1)->getString());
      return;
    case ScriptAction::TEAM_ATTACK_NAMED:
      doTeamAttackNamed(pAction->getParameter(0)->getString(),
                        pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::NAMED_ENTER_NAMED:
      doNamedEnterNamed(pAction->getParameter(0)->getString(),
                        pAction->getParameter(1)->getString());
      return;
    case ScriptAction::TEAM_ENTER_NAMED:
      doTeamEnterNamed(pAction->getParameter(0)->getString(),
                       pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::NAMED_FOLLOW_WAYPOINTS:
      doNamedFollowWaypoints(pAction->getParameter(0)->getString(),
                             pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::TEAM_GUARD_POSITION:
      doTeamGuardPosition(pAction->getParameter(0)->getString(),
                          pAction->getParameter(1)->getString());
      return;
    case ScriptAction::TEAM_GUARD_OBJECT:
      doTeamGuardObject(pAction->getParameter(0)->getString(),
                        pAction->getParameter(1)->getString());
      return;
    case ScriptAction::TEAM_GUARD_AREA:
      doTeamGuardArea(pAction->getParameter(0)->getString(),
                      pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::TEAM_HUNT_WITH_COMMAND_BUTTON:
      doTeamHuntWithCommandButton(pAction->getParameter(0)->getString(),
                                  pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::PLAYER_DISABLE_FACTORIES:
      doPlayerDisableFactories(pAction->getParameter(0)->getString(),
                               pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::PLAYER_ENABLE_FACTORIES:
      doPlayerEnableFactories(pAction->getParameter(0)->getString(),
                              pAction->getParameter(1)->getString());
      return;
    case ScriptAction::PLAYER_REPAIR_NAMED_STRUCTURE:
      doPlayerRepairStructure(pAction->getParameter(0)->getString(),
                              pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::NAMED_DAMAGE:
      doNamedDamage(pAction->getParameter(0)->getString(),
                    pAction->getParameter(1)->getInt());
      return;
    ...
    case ScriptAction::TEAM_WANDER:
      doTeamWander(pAction->getParameter(0)->getString(),
                   pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::TEAM_PANIC:
      doTeamPanic(pAction->getParameter(0)->getString(),
                  pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::DISPLAY_CINEMATIC_TEXT:
      doDisplayCinematicText(pAction->getParameter(0)->getString(),
                             pAction->getParameter(1)->getString(),
                             pAction->getParameter(2)->getInt());
      return;
    ...
    {
      const char *MSG =
          "Your Script requested the following message be displayed:\n\n";
    ...
    case ScriptAction::INGAME_POPUP_MESSAGE:
      doInGamePopupMessage(pAction->getParameter(0)->getString(),
                           pAction->getParameter(1)->getInt(),
                           pAction->getParameter(2)->getInt(),
                           pAction->getParameter(3)->getInt(),
                           pAction->getParameter(4)->getInt());
      return;
    case ScriptAction::CAMEO_FLASH:
      doCameoFlash(pAction->getParameter(0)->getString(),
                   pAction->getParameter(1)->getInt());
      return;
    case ScriptAction::NAMED_FLASH:
      doNamedFlash(pAction->getParameter(0)->getString(),
                   pAction->getParameter(1)->getInt(), NULL);
      return;
    case ScriptAction::TEAM_FLASH:
      doTeamFlash(pAction->getParameter(0)->getString(),
                  pAction->getParameter(1)->getInt(), NULL);
      return;
    case ScriptAction::NAMED_FLASH_WHITE: {
      ...
      doNamedFlash(pAction->getParameter(0)->getString(),
                   pAction->getParameter(1)->getInt(), &c);
    }
      return;
    case ScriptAction::NAMED_CUSTOM_COLOR: {
      doNamedCustomColor(pAction->getParameter(0)->getString(),
                         pAction->getParameter(1)->getInt());
    }
      return;
    ...
    case ScriptAction::SOUND_PLAY_NAMED:
      doSoundPlayFromNamed(pAction->getParameter(0)->getString(),
                           pAction->getParameter(1)->getString());
      return;
    case ScriptAction::SPEECH_PLAY:
      doSpeechPlay(pAction->getParameter(0)->getString(),
                   pAction->getParameter(1)->getInt());
      return;
    case ScriptAction::PLAYER_TRANSFER_OWNERSHIP_PLAYER:
      doPlayerTransferAssetsToPlayer(pAction->getParameter(0)->getString(),
                                     pAction->getParameter(1)->getString());
      return;
    case ScriptAction::NAMED_TRANSFER_OWNERSHIP_PLAYER:
      doNamedTransferAssetsToPlayer(pAction->getParameter(0)->getString(),
                                    pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::PLAYER_RELATES_PLAYER:
      updatePlayerRelationTowardPlayer(pAction->getParameter(0)->getString(),
                                       pAction->getParameter(2)->getInt(),
                                       pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::OBJECT_CREATE_RADAR_EVENT:
      doObjectRadarCreateEvent(pAction->getParameter(0)->getString(),
                               pAction->getParameter(1)->getInt());
      return;
    case ScriptAction::TEAM_CREATE_RADAR_EVENT:
      doTeamRadarCreateEvent(pAction->getParameter(0)->getString(),
                             pAction->getParameter(1)->getInt());
      return;
    ...
    case ScriptAction::NAMED_SET_STEALTH_ENABLED:
      doNamedEnableStealth(pAction->getParameter(0)->getString(),
                           pAction->getParameter(1)->getInt());
      return;
    case ScriptAction::TEAM_SET_STEALTH_ENABLED:
      doTeamEnableStealth(pAction->getParameter(0)->getString(),
                          pAction->getParameter(1)->getInt());
      return;
    case ScriptAction::MAP_REVEAL_AT_WAYPOINT:
      doRevealMapAtWaypoint(pAction->getParameter(0)->getString(),
                            pAction->getParameter(1)->getReal(),
                            pAction->getParameter(2)->getString());
      return;
    case ScriptAction::MAP_SHROUD_AT_WAYPOINT:
      doShroudMapAtWaypoint(pAction->getParameter(0)->getString(),
                            pAction->getParameter(1)->getReal(),
                            pAction->getParameter(2)->getString());
      return;
    ...
    case ScriptAction::MAP_REVEAL_ALL_UNDO_PERM:
      doRevealMapEntirePermanently(FALSE,
                                   pAction->getParameter(0)->getString());
      return;
    ...
    case ScriptAction::TEAM_AVAILABLE_FOR_RECRUITMENT:
      doTeamAvailableForRecruitment(pAction->getParameter(0)->getString(),
                                    pAction->getParameter(1)->getInt());
      return;
    ...
    case ScriptAction::TEAM_MERGE_INTO_TEAM:
      doMergeTeamIntoTeam(pAction->getParameter(0)->getString(),
                          pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::MUSIC_SET_TRACK:
      doMusicTrackChange(pAction->getParameter(0)->getString(),
                         pAction->getParameter(1)->getInt(),
                         pAction->getParameter(2)->getInt());
      return;
    case ScriptAction::TEAM_GARRISON_SPECIFIC_BUILDING:
      doTeamGarrisonSpecificBuilding(pAction->getParameter(0)->getString(),
                                     pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::NAMED_GARRISON_SPECIFIC_BUILDING:
      doUnitGarrisonSpecificBuilding(pAction->getParameter(0)->getString(),
                                     pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::CAMERA_BW_MODE_BEGIN:
      doBlackWhiteMode(true, pAction->getParameter(0)
                                 ? pAction->getParameter(0)->getInt()
                                 : 0);
      return;
    case ScriptAction::CAMERA_BW_MODE_END:
      doBlackWhiteMode(false, pAction->getParameter(0)
                                  ? pAction->getParameter(0)->getInt()
                                  : 0);
      return;
    ...
    case ScriptAction::CAMERA_MOTION_BLUR:
      doCameraMotionBlur(pAction->getParameter(0)->getInt(),
                         pAction->getParameter(1)->getInt());
      return;
    case ScriptAction::CAMERA_MOTION_BLUR_JUMP:
      doCameraMotionBlurJump(pAction->getParameter(0)->getString(),
                             pAction->getParameter(1)->getInt());
      return;
    case ScriptAction::CAMERA_MOTION_BLUR_FOLLOW:
      TheTacticalView->setViewFilterMode((enum FilterModes)(
          FM_VIEW_MB_PAN_ALPHA + pAction->getParameter(0)->getInt()));
    ...
    case ScriptAction::SHOW_MILITARY_CAPTION:
      doMilitaryCaption(pAction->getParameter(0)->getString(),
                        pAction->getParameter(1)->getInt());
      return;
    ...
    case ScriptAction::SET_STOPPING_DISTANCE:
      doSetStoppingDistance(pAction->getParameter(0)->getString(),
                            pAction->getParameter(1)->getReal());
      return;
    ...
    case ScriptAction::NAMED_SET_STOPPING_DISTANCE:
      doNamedSetStoppingDistance(pAction->getParameter(0)->getString(),
                                 pAction->getParameter(1)->getReal());
      return;

    case ScriptAction::NAMED_SET_HELD:
      doNamedSetHeld(pAction->getParameter(0)->getString(),
                     pAction->getParameter(1)->getInt());
      return;
    ...
    case ScriptAction::SOUND_SET_VOLUME:
      doAudioSetVolume((AudioAffect)(AudioAffect_Sound | AudioAffect_Sound3D),
                       pAction->getParameter(0)->getReal());
      return;
    ...
    case ScriptAction::TEAM_TRANSFER_TO_PLAYER:
      doTransferTeamToPlayer(pAction->getParameter(0)->getString(),
                             pAction->getParameter(1)->getString());
      return;
    case ScriptAction::PLAYER_SET_MONEY:
      doSetMoney(pAction->getParameter(0)->getString(),
                 pAction->getParameter(1)->getInt());
      return;
    case ScriptAction::PLAYER_GIVE_MONEY:
      doGiveMoney(pAction->getParameter(0)->getString(),
                  pAction->getParameter(1)->getInt());
      return;

    case ScriptAction::DISPLAY_COUNTDOWN_TIMER:
      doDisplayCountdownTimer(pAction->getParameter(0)->getString(),
                              pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::DISPLAY_COUNTER:
      doDisplayCounter(pAction->getParameter(0)->getString(),
                       pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::NAMED_STOP_SPECIAL_POWER_COUNTDOWN:
      doNamedStopSpecialPowerCountdown(pAction->getParameter(0)->getString(),
                                       pAction->getParameter(1)->getString(),
                                       TRUE);
      return;
    ...
    case ScriptAction::NAMED_SET_SPECIAL_POWER_COUNTDOWN:
      doNamedSetSpecialPowerCountdown(pAction->getParameter(0)->getString(),
                                      pAction->getParameter(1)->getString(),
                                      pAction->getParameter(2)->getInt());
      return;

    case ScriptAction::NAMED_ADD_SPECIAL_POWER_COUNTDOWN:
      doNamedAddSpecialPowerCountdown(pAction->getParameter(0)->getString(),
                                      pAction->getParameter(1)->getString(),
                                      pAction->getParameter(2)->getInt());
      return;

    case ScriptAction::NAMED_FIRE_SPECIAL_POWER_AT_WAYPOINT:
      doNamedFireSpecialPowerAtWaypoint(pAction->getParameter(0)->getString(),
                                        pAction->getParameter(1)->getString(),
                                        pAction->getParameter(2)->getString());
      return;
    ...
    case ScriptAction::NAMED_FIRE_SPECIAL_POWER_AT_NAMED:
      doNamedFireSpecialPowerAtNamed(pAction->getParameter(0)->getString(),
                                     pAction->getParameter(1)->getString(),
                                     pAction->getParameter(2)->getString());
      return;
    ...
    case ScriptAction::CAMERA_TETHER_NAMED:
      doCameraTetherNamed(pAction->getParameter(0)->getString(),
                          (Bool)(pAction->getParameter(1)->getInt()),
                          pAction->getParameter(2)->getReal());
      return;
    ...
    case ScriptAction::CAMERA_SET_DEFAULT:
      doCameraSetDefault(pAction->getParameter(0)->getReal(),
                         pAction->getParameter(1)->getReal(),
                         pAction->getParameter(2)->getReal());
      return;
    ...
    case ScriptAction::TEAM_SET_OVERRIDE_RELATION_TO_TEAM:
      doTeamSetOverrideRelationToTeam(
          pAction->getParameter(0)->getString(),  // first team
    ...
    case ScriptAction::PLAYER_SET_OVERRIDE_RELATION_TO_TEAM:
      doPlayerSetOverrideRelationToTeam(
          pAction->getParameter(0)->getString(),  // first player
          pAction->getParameter(1)->getString(),  // second team
          pAction->getParameter(2)->getInt());    // relation (ENEMIES, etc)
      return;
    ...
    case ScriptAction::NAMED_USE_COMMANDBUTTON_ABILITY:
      doNamedUseCommandButtonAbility(pAction->getParameter(0)->getString(),
                                     pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::TEAM_USE_COMMANDBUTTON_ABILITY:
      doTeamUseCommandButtonAbility(pAction->getParameter(0)->getString(),
                                    pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::UNIT_EXECUTE_SEQUENTIAL_SCRIPT:
      doUnitStartSequentialScript(pAction->getParameter(0)->getString(),
                                  pAction->getParameter(1)->getString(), 0);
      return;

    case ScriptAction::UNIT_EXECUTE_SEQUENTIAL_SCRIPT_LOOPING:
      doUnitStartSequentialScript(pAction->getParameter(0)->getString(),
                                  pAction->getParameter(1)->getString(),
                                  pAction->getParameter(2)->getInt() - 1);
      return;
    ...
    case ScriptAction::TEAM_EXECUTE_SEQUENTIAL_SCRIPT:
      doTeamStartSequentialScript(pAction->getParameter(0)->getString(),
                                  pAction->getParameter(1)->getString(), 0);
      return;
    ...
    case ScriptAction::UNIT_GUARD_FOR_FRAMECOUNT:
      doUnitGuardForFramecount(pAction->getParameter(0)->getString(),
                               pAction->getParameter(1)->getInt());
      return;

    case ScriptAction::UNIT_IDLE_FOR_FRAMECOUNT:
      doUnitIdleForFramecount(pAction->getParameter(0)->getString(),
                              pAction->getParameter(1)->getInt());
      return;

    case ScriptAction::TEAM_GUARD_FOR_FRAMECOUNT:
      doTeamIdleForFramecount(pAction->getParameter(0)->getString(),
                              pAction->getParameter(1)->getInt());
      return;
    ...
    case ScriptAction::WATER_CHANGE_HEIGHT:
      doWaterChangeHeight(pAction->getParameter(0)->getString(),
                          pAction->getParameter(1)->getReal());
      return;

    case ScriptAction::WATER_CHANGE_HEIGHT_OVER_TIME:
      doWaterChangeHeightOverTime(pAction->getParameter(0)->getString(),
                                  pAction->getParameter(1)->getReal(),
                                  pAction->getParameter(2)->getReal(),
                                  pAction->getParameter(3)->getReal());
      return;
    ...
    case ScriptAction::TECHTREE_MODIFY_BUILDABILITY_OBJECT:
      doModifyBuildableStatus(pAction->getParameter(0)->getString(),
                              pAction->getParameter(1)->getInt());
      return;

    case ScriptAction::SET_CAVE_INDEX:
      doSetCaveIndex(pAction->getParameter(0)->getString(),
                     pAction->getParameter(1)->getInt());
      return;

    case ScriptAction::WAREHOUSE_SET_VALUE:
      doSetWarehouseValue(pAction->getParameter(0)->getString(),
                          pAction->getParameter(1)->getInt());
      return;
    ...
    case ScriptAction::AUDIO_OVERRIDE_VOLUME_TYPE:
      doSoundOverrideVolume(pAction->getParameter(0)->getString(),
                            pAction->getParameter(1)->getReal());
      return;
    ...
    case ScriptAction::UNIT_MOVE_TOWARDS_NEAREST_OBJECT_TYPE:
      doMoveUnitTowardsNearest(pAction->getParameter(0)->getString(),
                               pAction->getParameter(1)->getString(),
                               pAction->getParameter(2)->getString());
      return;

    case ScriptAction::TEAM_MOVE_TOWARDS_NEAREST_OBJECT_TYPE:
      doMoveTeamTowardsNearest(pAction->getParameter(0)->getString(),
                               pAction->getParameter(1)->getString(),
                               pAction->getParameter(2)->getString());
      return;

    case ScriptAction::NAMED_RECEIVE_UPGRADE:
      doUnitReceiveUpgrade(pAction->getParameter(0)->getString(),
                           pAction->getParameter(1)->getString());
      return;
    ...
    case ScriptAction::SKIRMISH_WAIT_FOR_COMMANDBUTTON_AVAILABLE_ALL:
      ...
      DEBUG_CRASH(
          ("\"[Skirmish] Wait for command button available - all\" should "
           "never be used outside of Sequential scripts. - jkmcd"));
      return;

    case ScriptAction::SKIRMISH_WAIT_FOR_COMMANDBUTTON_AVAILABLE_PARTIAL:
      // We should never get here.
      DEBUG_CRASH(
          ("\"[Skirmish] Wait for command button available - partial\" should "
           "never be used outside of Sequential scripts. - jkmcd"));
      return;
    ...
    case ScriptAction::TEAM_WAIT_FOR_NOT_CONTAINED_ALL:
      // We should never get here.
      DEBUG_CRASH(
          ("\"[Team] Wait for team no longer contained - all\" should never be "
           "used outside of Sequential scripts. - jkmcd"));
      return;

    case ScriptAction::TEAM_WAIT_FOR_NOT_CONTAINED_PARTIAL:
      // We should never get here.
      DEBUG_CRASH(
          ("\"[Team] Wait for team no longer contained - partial\" should "
           "never be used outside of Sequential scripts. - jkmcd"));
      return;

    case ScriptAction::TEAM_SPIN_FOR_FRAMECOUNT:
      doTeamSpinForFramecount(pAction->getParameter(0)->getString(),
                              pAction->getParameter(1)->getInt());
      return;

    case ScriptAction::TEAM_ALL_USE_COMMANDBUTTON_ON_NAMED:
      doTeamUseCommandButtonOnNamed(pAction->getParameter(0)->getString(),
                                    pAction->getParameter(1)->getString(),
                                    pAction->getParameter(2)->getString());
      return;
    ...
    case ScriptAction::TEAM_PARTIAL_USE_COMMANDBUTTON:
      doTeamPartialUseCommandButton(pAction->getParameter(0)->getReal(),
                                    pAction->getParameter(1)->getString(),
                                    pAction->getParameter(2)->getString());
      return;
    ...
    case ScriptAction::PLAYER_CREATE_TEAM_FROM_CAPTURED_UNITS:
      doCreateTeamFromCapturedUnits(pAction->getParameter(0)->getString(),
                                    pAction->getParameter(1)->getString());
      return;
    case ScriptAction::PLAYER_ADD_SKILLPOINTS:
      doPlayerAddSkillPoints(pAction->getParameter(0)->getString(),
                             pAction->getParameter(1)->getInt());
      return;
    case ScriptAction::PLAYER_ADD_RANKLEVEL:
      doPlayerAddRankLevels(pAction->getParameter(0)->getString(),
                            pAction->getParameter(1)->getInt());
      return;
    case ScriptAction::PLAYER_SET_RANKLEVEL:
      doPlayerSetRankLevel(pAction->getParameter(0)->getString(),
                           pAction->getParameter(1)->getInt());
      return;
    ...
    case ScriptAction::PLAYER_GRANT_SCIENCE:
      doPlayerGrantScience(pAction->getParameter(0)->getString(),
                           pAction->getParameter(1)->getString());
      return;
    case ScriptAction::PLAYER_PURCHASE_SCIENCE:
      doPlayerPurchaseScience(pAction->getParameter(0)->getString(),
                              pAction->getParameter(1)->getString());
      return;
    case ScriptAction::PLAYER_SCIENCE_AVAILABILITY:
      doPlayerSetScienceAvailability(pAction->getParameter(0)->getString(),
                                     pAction->getParameter(1)->getString(),
                                     pAction->getParameter(2)->getString());
      return;
    case ScriptAction::TEAM_SET_EMOTICON:
      doTeamEmoticon(pAction->getParameter(0)->getString(),
                     pAction->getParameter(1)->getString(),
                     pAction->getParameter(2)->getReal());
      return;
    case ScriptAction::NAMED_SET_EMOTICON:
      doNamedEmoticon(pAction->getParameter(0)->getString(),
                      pAction->getParameter(1)->getString(),
                      pAction->getParameter(2)->getReal());
      return;
    case ScriptAction::OBJECTLIST_ADDOBJECTTYPE:
      doObjectTypeListMaintenance(pAction->getParameter(0)->getString(),
                                  pAction->getParameter(1)->getString(), TRUE);
      return;
    case ScriptAction::OBJECTLIST_REMOVEOBJECTTYPE:
      doObjectTypeListMaintenance(pAction->getParameter(0)->getString(),
                                  pAction->getParameter(1)->getString(), FALSE);
      return;
    case ScriptAction::MAP_REVEAL_PERMANENTLY_AT_WAYPOINT:
      doRevealMapAtWaypointPermanent(pAction->getParameter(0)->getString(),
                                     pAction->getParameter(1)->getReal(),
                                     pAction->getParameter(2)->getString(),
                                     pAction->getParameter(3)->getString());
      return;
    ...
    case ScriptAction::NAMED_FACE_NAMED:
      doNamedFaceNamed(pAction->getParameter(0)->getString(),
                       pAction->getParameter(1)->getString());
      return;

    case ScriptAction::NAMED_FACE_WAYPOINT:
      doNamedFaceWaypoint(pAction->getParameter(0)->getString(),
                          pAction->getParameter(1)->getString());
      return;

    case ScriptAction::TEAM_FACE_NAMED:
      doTeamFaceNamed(pAction->getParameter(0)->getString(),
                      pAction->getParameter(1)->getString());
      return;

    case ScriptAction::TEAM_FACE_WAYPOINT:
      doTeamFaceWaypoint(pAction->getParameter(0)->getString(),
                         pAction->getParameter(1)->getString());
      return;

    case ScriptAction::UNIT_AFFECT_OBJECT_PANEL_FLAGS:
      doAffectObjectPanelFlagsUnit(pAction->getParameter(0)->getString(),
                                   pAction->getParameter(1)->getString(),
                                   pAction->getParameter(2)->getInt());
      return;

    case ScriptAction::TEAM_AFFECT_OBJECT_PANEL_FLAGS:
      doAffectObjectPanelFlagsTeam(pAction->getParameter(0)->getString(),
                                   pAction->getParameter(1)->getString(),
                                   pAction->getParameter(2)->getInt());
      return;

    case ScriptAction::PLAYER_SELECT_SKILLSET:
      doAffectPlayerSkillset(pAction->getParameter(0)->getString(),
                             pAction->getParameter(1)->getInt());
      return;

    case ScriptAction::COMMANDBAR_REMOVE_BUTTON_OBJECTTYPE:
      doRemoveCommandBarButton(pAction->getParameter(0)->getString(),
                               pAction->getParameter(1)->getString());
      return;

    case ScriptAction::COMMANDBAR_ADD_BUTTON_OBJECTTYPE_SLOT:
      doAddCommandBarButton(pAction->getParameter(0)->getString(),
                            pAction->getParameter(1)->getString(),
                            pAction->getParameter(2)->getInt());
      return;
    ...
    case ScriptAction::UNIT_SPAWN_NAMED_LOCATION_ORIENTATION: {
      ...
      doCreateObject(pAction->getParameter(0)->getString(),
                     pAction->getParameter(1)->getString(),
                     pAction->getParameter(2)->getString(), &pos,
                     pAction->getParameter(4)->getReal());
      return;
    }

    case ScriptAction::PLAYER_AFFECT_RECEIVING_EXPERIENCE:
      doAffectSkillPointsModifier(pAction->getParameter(0)->getString(),
                                  pAction->getParameter(1)->getReal());
      return;

    case ScriptAction::TEAM_GUARD_SUPPLY_CENTER:
      doGuardSupplyCenter(pAction->getParameter(0)->getString(),
                          pAction->getParameter(1)->getInt());
      return;
    ...
    case ScriptAction::RESIZE_VIEW_GUARDBAND:
      doResizeViewGuardband(pAction->getParameter(0)->getReal(),
                            pAction->getParameter(1)->getReal());
      return;
