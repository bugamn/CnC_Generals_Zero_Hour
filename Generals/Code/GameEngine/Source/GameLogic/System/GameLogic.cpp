...
// Note - this is only for gathering timing data!  DO NOT DO THIS IN REGULAR
// CODE!!!  JBA
...
enum {
  ...
  LOAD_PROGRESS_SIDE_POPULATION =
      LOAD_PROGRESS_POST_LOAD_MAP +
      1,  // Increment the next one by at least MAX_SLOTS
  LOAD_PROGRESS_POST_SIDE_LIST_INIT =
      LOAD_PROGRESS_SIDE_POPULATION + 1 + MAX_SLOTS,
  LOAD_PROGRESS_POST_PLAYER_LIST_RESET = LOAD_PROGRESS_POST_SIDE_LIST_INIT + 1,
  LOAD_PROGRESS_POST_SCRIPT_ENGINE_NEW_MAP =
      LOAD_PROGRESS_POST_PLAYER_LIST_RESET + 1,
  LOAD_PROGRESS_POST_VICTORY_CONDITION_SETUP =
      LOAD_PROGRESS_POST_SCRIPT_ENGINE_NEW_MAP + 2,
  LOAD_PROGRESS_POST_VICTORY_CONDITION_SET_VICTORY_CONDITION =
      LOAD_PROGRESS_POST_VICTORY_CONDITION_SETUP + 1,
  LOAD_PROGRESS_POST_GHOST_OBJECT_MANAGER_RESET =
      LOAD_PROGRESS_POST_VICTORY_CONDITION_SET_VICTORY_CONDITION + 1,
  LOAD_PROGRESS_POST_TERRAIN_LOGIC_NEW_MAP =
      LOAD_PROGRESS_POST_GHOST_OBJECT_MANAGER_RESET + 1,
  ...
  LOAD_PROGRESS_LOOP_ALL_THE_FREAKN_OBJECTS =
      LOAD_PROGRESS_POST_BRIDGE_LOAD + 1,
  LOAD_PROGRESS_MAX_ALL_THE_FREAKN_OBJECTS =
      80,  // THE END OF THE BIG ASS PROGRESS PAUSE

  LOAD_PROGRESS_LOOP_INITIAL_NETWORK_BUILDINGS =
      LOAD_PROGRESS_MAX_ALL_THE_FREAKN_OBJECTS +
      1,  // Increment the next one by at least MAX_SLOTS
  LOAD_PROGRESS_POST_INITIAL_NETWORK_BUILDINGS =
      LOAD_PROGRESS_LOOP_INITIAL_NETWORK_BUILDINGS + MAX_SLOTS + 1,

  LOAD_PROGRESS_POST_PRELOAD_ASSETS =
      LOAD_PROGRESS_POST_INITIAL_NETWORK_BUILDINGS + 1,
...
void GameLogic::setDefaults(Bool saveGame) {
...
#endif
  for (std::vector<UpdateModulePtr>::iterator it = m_sleepyUpdates.begin();
       it != m_sleepyUpdates.end(); ++it) {
  ...
  //
  // only reset the next object ID allocater counter when we're not loading a
  // save game. for save games, we read this value out of the save game file and
  // it is important that we preserve it as we load and execute the game
  //
  if (saveGame == FALSE) m_nextObjID = (ObjectID)1;
}
...
Bool GameLogic::isInSinglePlayerGame(void) {
  return (m_gameMode == GAME_SINGLE_PLAYER ||
          (TheRecorder && TheRecorder->getMode() == RECORDERMODETYPE_PLAYBACK &&
           TheRecorder->getGameMode() == GAME_SINGLE_PLAYER));
}
...
void GameLogic::destroyAllObjectsImmediate() {
  ...
  processDestroyList();
  DEBUG_ASSERTCRASH(m_objList == NULL,
...
void GameLogic::init(void) {
  ...
  TheScriptActions =
      NEW ScriptActions;  // Basically, a subsystem of TheScriptEngine.
  TheScriptConditions =
      NEW ScriptConditions;  // Basically, a subsystem of TheScriptEngine.
...
static Object *placeObjectAtPosition(Int slotNum,
                                     AsciiString objectTemplateName,
                                     Coord3D &pos, Player *pPlayer,
  ...
  DEBUG_ASSERTCRASH(obj, ("TheThingFactory didn't give me a valid Object for "
                          "player %d's (%ls) starting building\n",
  ...
  if (obj) {
    ...
    CRCDEBUG_LOG(("Placed an object for %ls at pos (%g,%g,%g)\n",
                  pPlayer->getPlayerDisplayName().str(), pos.x, pos.y, pos.z));
    DUMPCOORD3D(&pos);
    ...
    // srj sez: team should not be null, but could be for ill-formed user maps.
    // so don't crash.
    ...
    if (obj->getAIUpdateInterface() && !obj->isKindOf(KINDOF_IMMOBILE)) {
      ...
      if (TheAI->pathfinder()->adjustDestination(
              obj, obj->getAIUpdateInterface()->getLocomotorSet(), &pos)) {
        DUMPCOORD3D(&pos);
        TheAI->pathfinder()->updateGoal(
            obj, &pos,
            LAYER_GROUND);  // Units always start on the ground for now.  jba.
        obj->setPosition(&pos);
      }
    }
  }
...
static void placeNetworkBuildingsForPlayer(Int slotNum, const GameSlot *pSlot,
  ...
  waypointName.format("Player_%d_Start",
                      startPos + 1);  // start pos waypoints are 1-based

  AsciiString rallyWaypointName;
  rallyWaypointName.format("Player_%d_Rally",
                           startPos + 1);  // start pos waypoints are 1-based

  Waypoint *waypoint = findNamedWaypoint(waypointName);
  Waypoint *rallyWaypoint = findNamedWaypoint(rallyWaypointName);
  DEBUG_ASSERTCRASH(waypoint,
                    ("Player %d has no starting waypoint (Player_%d_Start)\n",
                     slotNum, startPos));
  if (!waypoint) return;
  ...
  DEBUG_ASSERTCRASH(
      !buildingTemplateName.isEmpty(),
      ("no starting building type for player %d (playertemplate %ls)\n",
       slotNum, pTemplate->getDisplayName().str()));
  if (buildingTemplateName.isEmpty()) return;

  DEBUG_LOG(("Placing starting building at waypoint %s\n", waypointName.str()));
  Object *conYard = placeObjectAtPosition(slotNum, buildingTemplateName, pos,
                                          pPlayer, pTemplate);
  ...
  for (Int i = 0; i < MAX_MP_STARTING_UNITS; ++i) {
    ...
    if (objName.isNotEmpty()) {
      ...
      options.minRadius =
          conYard->getGeometryInfo().getBoundingSphereRadius() * 0.7f;
      options.maxRadius =
          conYard->getGeometryInfo().getBoundingSphereRadius() * 1.3f;
      DEBUG_LOG(("Placing starting object %d (%s)\n", i, objName.str()));
      ThePartitionManager->update();
      Bool foundPos =
          ThePartitionManager->findPositionAround(&pos, &options, &objPos);
      if (foundPos) {
        Object *unit =
            placeObjectAtPosition(slotNum, objName, objPos, pPlayer, pTemplate);
        if (unit) {
...
void handleNameChange(MapObject *mapObj) {
  if (!mapObj->getName().compare("AmericaTankLeopard")) {
    ...
    const ThingTemplate *thingTemplate =
        TheThingFactory->findTemplate(mapObj->getName());
...
#define MORE_RANDOM
  ...
  for (i = 0; i < ThePlayerTemplateStore->getPlayerTemplateCount(); ++i) {
    const PlayerTemplate *ptTest =
        ThePlayerTemplateStore->getNthPlayerTemplate(i);
    if (!ptTest || ptTest->getStartingBuilding().isEmpty()) continue;
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    ...
    DEBUG_LOG(
        ("Player %d has playerTemplate index %d\n", i, playerTemplateIdx));
    while (playerTemplateIdx != PLAYERTEMPLATE_OBSERVER &&
           (playerTemplateIdx < 0 ||
            playerTemplateIdx >=
                ThePlayerTemplateStore->getPlayerTemplateCount())) {
      DEBUG_ASSERTCRASH(playerTemplateIdx == -1,
                        ("Non-random bad playerTemplate %d in slot %d\n",
                         playerTemplateIdx, i));
#ifdef MORE_RANDOM
      // our RNG is basically shit -- horribly nonrandom at the start of the
      // sequence. get a few values at random to get rid of the dreck. there's
      // no mathematical basis for this, but empirically, it helps a lot.
...
#else
      playerTemplateIdx = GameLogicRandomValue(
          0, ThePlayerTemplateStore->getPlayerTemplateCount() - 1);
#endif
      const PlayerTemplate *pt =
          ThePlayerTemplateStore->getNthPlayerTemplate(playerTemplateIdx);
      if (!pt || pt->getStartingBuilding().isEmpty()) {
        ...
        DEBUG_LOG(
            ("Setting playerTemplateIdx %d to %d\n", i, playerTemplateIdx));
    ...
    Int colorIdx = slot->getColor();
    if (colorIdx < 0 || colorIdx >= TheMultiplayerSettings->getNumColors()) {
      DEBUG_ASSERTCRASH(colorIdx == -1,
                        ("Non-random bad color %d in slot %d\n", colorIdx, i));
      while (colorIdx == -1) {
        colorIdx =
            GameLogicRandomValue(0, TheMultiplayerSettings->getNumColors() - 1);
        if (game->isColorTaken(colorIdx)) colorIdx = -1;
      }
...
static void populateRandomStartPosition(GameInfo *game) {
  ...
  DEBUG_ASSERTCRASH(
      md, ("Could not find map %s in the mapcache", game->getMap().str()));
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (posIdx < 0 || posIdx >= numPlayers) {
      DEBUG_ASSERTCRASH(
          posIdx == -1,
          ("Non-random bad start position %d in slot %d\n", posIdx, i));
      if (hasStartSpotBeenPicked) {
        ...
        DEBUG_ASSERTCRASH(farthestIndex >= 0,
                          ("Couldn't find a farthest spot!\n"));
        slot->setStartPos(farthestIndex);
        taken[farthestIndex] = TRUE;
      } else {
        // We're the first real spot.  Pick randomly.
        // This while loop shouldn't be neccessary, since we're first.  Why not,
        // though?
        while (posIdx == -1) {
        ...
        DEBUG_LOG(
            ("Setting start position %d to %d (random choice)\n", i, posIdx));
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (slot->getPlayerTemplate() != PLAYERTEMPLATE_OBSERVER) continue;
...
void GameLogic::updateLoadProgress(Int progress) {
  if (m_loadScreen) m_loadScreen->update(progress);
...
void GameLogic::setGameLoading(Bool loading) { m_loadingScene = loading; }

// ------------------------------------------------------------------------------------------------
/** Entry point for starting a new game, the engine is already in clean state at
 * this point and ready to load up with all the data */
// ------------------------------------------------------------------------------------------------
void GameLogic::startNewGame(Bool saveGame) {
  ...
  if (saveGame == FALSE) {
    ...
    //
    // sanity, the pristine map should never start with "Save", otherwise that
    // would be a map from the save directory
    //
    if (TheGameState->isInSaveDirectory(TheGlobalData->m_mapName)) {
      DEBUG_CRASH((
          "FATAL SAVE/LOAD ERROR! - Setting a pristine map name that refers to "
          "a map in the save directory.  The pristine map should always refer "
          "to the ORIGINAL map in the Maps directory, if the pristine map "
          "string is corrupt then map.ini files will not load correctly.\n"));

    }  // end if

    if (m_startNewGame == FALSE) {
#if !defined(_PLAYTEST)
      /// @todo: Here is where we would look at the game mode & play an intro
      /// movie or something.
      ...
      if (m_gameMode == GAME_SINGLE_PLAYER) {
        ...
        if (m_loadScreen) {
          TheWritableGlobalData->m_loadScreenRender =
              TRUE;  ///< mark it so only a few select things are rendered
                     ///< during load
  ...
  TheWritableGlobalData->m_loadScreenRender =
      TRUE;  ///< mark it so only a few select things are rendered during load
  ...
  if (TheNetwork) {
    if (TheLAN) {
      ...
      TheGameInfo = game =
          TheGameSpyGame;  /// @todo: MDC add back in after demo
    }
  } else {
  ...
  if (m_loadScreen) updateLoadProgress(LOAD_PROGRESS_POST_PARTICLE_INI_LOAD);
  ...
  if (game) {
    if (TheGameEngine->isMultiplayerSession() || isSkirmishOrSkirmishReplay) {
      // Saves off any player, and resets the sides to 0 players so we can add
      // the skirmish players.
    ...
    for (int i = 0; i < MAX_SLOTS; ++i) {
      ...
      if (slot->getPlayerTemplate() >= 0)
        pt = ThePlayerTemplateStore->getNthPlayerTemplate(
            slot->getPlayerTemplate());
      else
        pt = ThePlayerTemplateStore->findPlayerTemplate(
            TheNameKeyGenerator->nameToKey("FactionObserver"));
      if (pt) {
      ...
      for (int j = 0; j < MAX_SLOTS; ++j) {
        ...
        if (i == j || !teamSlot->isOccupied()) continue;
      ...
      DEBUG_LOG(("Player %d's teams are: allies=%s, enemies=%s\n", i,
                 alliesString.str(), enemiesString.str()));
      /*

                              Int colorIdx = slot->getColor();
                              if (colorIdx < 0 || colorIdx >=
         TheMultiplayerSettings->getNumColors())
                              {
                                      DEBUG_ASSERTCRASH(colorIdx == -1,
         ("Non-random bad color %d in slot %d\n", colorIdx, i)); while (colorIdx
         == -1)
                                      {
                                              colorIdx = GameLogicRandomValue(0,
         TheMultiplayerSettings->getNumColors()-1); if
         (game->isColorTaken(colorIdx)) colorIdx = -1;
                                      }
                                      DEBUG_LOG(("Setting color %d to %d\n", i,
         colorIdx)); slot->setColor(colorIdx);
                              }
                              */

      d.setInt(TheKey_playerColor,
               TheMultiplayerSettings->getColor(slot->getColor())->getColor());
      d.setInt(
      ...
      d.setBool(
          ...
          slot->isHuman() &&
              (slot->getName().compare(
                   game->getSlot(game->getLocalSlotNum())->getName().str()) ==
               0));
      ...
      */
      ...
      if (slot->isHuman() &&
          game->getSlotNum(slotNameAscii) == game->getLocalSlotNum()) {
        localSlot = i;
      }
  ...
  d.setInt(TheKey_playerNightColor,
           TheMultiplayerSettings->getColor(0)->getNightColor());
  ...
  if (TheGameEngine->isMultiplayerSession() || isSkirmishOrSkirmishReplay) {
    // if there are no other teams (happens for debugging) don't end the game
    // immediately
    Int numTeams = 0;  // this can be higher than expected, but is accurate for
                       // determining 0, 1, 2+
    Int lastTeam = -1;
    if (game) {
      for (int i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (numTeams > 1) {
      ...
      if (theInputStream.open(path)) {
        ...
        file.registerParser(AsciiString("PlayerScriptsList"),
    ...
    {
            AsciiString name;
            name.format("MultiplayerPlayerDefeat"); // name is for the ui in the
    editor Script *pNewScript = newInstance(Script); pNewScript->setName(name);

            Condition *pFalse1 =
    newInstance(Condition)(Condition::MULTIPLAYER_PLAYER_DEFEAT); OrCondition
    *pOr = newInstance(OrCondition); pOr->setFirstAndCondition(pFalse1);
            pNewScript->setOrCondition(pOr);

            ScriptAction *action =
    newInstance(ScriptAction)(ScriptAction::LOCALDEFEAT);
            pNewScript->setAction(action);

            ScriptList *pSL = TheSidesList->getSideInfo(0)->getScriptList();
            pSL->addScript(pNewScript, 0); // adds this as the player's first
    script (index 0)
    }
    ...
    {
            AsciiString name;
            name.format("MultiplayerVictory"); // name is for the ui in the
    editor Script *pNewScript = newInstance( Script );
            pNewScript->setName(name);

            Condition *pFalse1 =
    newInstance(Condition)(Condition::MULTIPLAYER_ALLIED_VICTORY); OrCondition
    *pOr = newInstance(OrCondition); pOr->setFirstAndCondition(pFalse1);
            pNewScript->setOrCondition(pOr);

            ScriptAction *action =
    newInstance(ScriptAction)(ScriptAction::VICTORY);
            pNewScript->setAction(action);
    ...
    {
            AsciiString name;
            name.format("MultiplayerDefeat"); // name is for the ui in the
    editor Script *pNewScript = newInstance(Script); pNewScript->setName(name);

            Condition *pFalse1 =
    newInstance(Condition)(Condition::MULTIPLAYER_ALLIED_DEFEAT); OrCondition
    *pOr = newInstance(OrCondition); pOr->setFirstAndCondition(pFalse1);
            pNewScript->setOrCondition(pOr);

            ScriptAction *action =
    newInstance(ScriptAction)(ScriptAction::DEFEAT);
            pNewScript->setAction(action);
  ...
  // Tell the multiplayer victory condition singleton that the players are
  // created
  ...
  ThePartitionManager->init();
  ThePartitionManager
      ->refreshShroudForLocalPlayer();  // Can't do this until after init, and
                                        // doesn't seem right to do in init

  TheGhostObjectManager->setLocalPlayerIndex(
  ...
  for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
    ...
    if (thingTemplate == NULL) continue;
    ...
    if (!isBridgeLikeObject) continue;

    Team *team = ThePlayerList->getNeutralPlayer()->getDefaultTeam();
    // create new object in the world
    Object *obj = TheThingFactory->newObject(
        thingTemplate, team);  //, OBJECT_STATUS_LOADING_FROM_MAP );
    if (obj) {
      ...
      // Do this after positioning the object, because we may place objects in
      // response to keys in the map object properties. update this object
      // instance with properties from the map object
  ...
  // Note that it is important that the pathfinder be called before the map
  // objects are loaded.
  ...
  if (saveGame == FALSE) {
    for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
      ...
      // Kris: Added this function so that we can rename objects and preserve
      // them. If the patch
      //       entry is missing for a particular unit, it doesn't get added to
      //       the world.
      ...
      //
      // if no template continue, some map objects don't have thing templates
      // like lights (handled in the device).  Objects that are test objects
      // have a test string (designated with *** and the define TEST_STRING) and
      // will have temporary templates created 'on the fly' during a
      ...
      if (thingTemplate->isKindOf(KINDOF_WALK_ON_TOP_OF_WALL))
        isBridgeLikeObject = true;
      if (isBridgeLikeObject) continue;  // bridges have to be added earlier.

      Bool useTrees = TheGlobalData->m_useTrees;
      if (TheRecorder && TheRecorder->isMultiplayer()) {
        useTrees = TRUE;  // Always use trees in multiplayer, cause we want it
                          // to sync properly. jba.
      }
      // don't create trees and shrubs if this is one and we have that option
      // off
      if (thingTemplate->isKindOf(KINDOF_SHRUBBERY) && !useTrees) continue;

      // Get the team information
      DEBUG_ASSERTCRASH(
          pMapObj->getProperties()->getType(TheKey_originalOwner) ==
              Dict::DICT_ASCIISTRING,
      ...
      if (obj) {
        ...
        if (pMapObj->getFlag(FLAG_DRAWS_IN_MIRROR) ||
            obj->isKindOf(KINDOF_CAN_CAST_REFLECTIONS)) {
          Drawable *draw = obj->getDrawable();
          if (draw) draw->setDrawableStatus(DRAWABLE_STATUS_DRAWS_IN_MIRROR);
        }
  ...
  if (game && !saveGame) {
    for (int i = 0; i < MAX_SLOTS; ++i) {
      ...
      if (slot->getPlayerTemplate() == PLAYERTEMPLATE_OBSERVER) {
        ...
        const PlayerTemplate *pt = ThePlayerTemplateStore->findPlayerTemplate(
            TheNameKeyGenerator->nameToKey("FactionObserver"));
        if (pt) {
          for (Int j = 0; j < ThePlayerTemplateStore->getPlayerTemplateCount();
               ++j) {
  ...
  //
  // tell the client to pre-load some assets that we will use such as faction
  // things we will build and various damage states for all the structures on
  // the map so that we don't have big pauses when building those objects or
  // switching to those states
  //
  ...
  if (TheRecorder) TheRecorder->initControls();

  // Note - WorldBuilderDoc.cpp also uses initial camera position, so if
  // changed, update both.  jba Note - We construct the multiplayer start spot
  // name manually here, so change this if you
  //        change TheKey_Player_1_Start etc.  mdc
  AsciiString startingCamName =
      TheNameKeyGenerator->keyToName(TheKey_InitialCameraPosition);
  if (game) {
    ...
    if (slot->isHuman()) {
      ...
      startingCamName.format("Player_%d_Start",
                             startPos + 1);  // start pos waypoints are 1-based
  ...
  if (way) {
    ...
    // Just look somewhere.  lookAt does some terrain specific setup, so it is
    // good to call it.  jba
    Coord3D pos;
    pos.x = 50;
    pos.y = 50;
  ...
  // update partition info - We need to do the initial update so that it can be
  // queried during the first frame.  jba.
  ThePartitionManager->update();
  ...
  if (saveGame == FALSE) ThePlayerList->newMap();
  ...
  if (saveGame == FALSE && isInSinglePlayerGame()) {
    for (Int i = 0; i < MAX_PLAYER_COUNT; ++i) {
      ...
      if (pPlayer && pPlayer->getPlayerType() != PLAYER_HUMAN) pPlayer = NULL;

      if (pPlayer) {
        pPlayer->addSkillPoints(m_rankPointsToAddAtGameStart);
        DEBUG_LOG(
            ("GameLogic::startNewGame() - adding "
             "m_rankPointsToAddAtGameStart==%d to player %d(%ls)\n",
             m_rankPointsToAddAtGameStart, i,
  ...
  if (m_loadScreen) {
    ...
    /*
                    //
                    // delete load screen only when not loading a save game, for
       save games we still
                    // have more work to do and the load screen will be deleted
       elsewhere after
  ...
  if (m_gameMode == GAME_SHELL) {
    ...
    if (m_gameMode == GAME_REPLAY) {
      ThePlayerList->setLocalPlayer(ThePlayerList->findPlayerWithNameKey(
          TheNameKeyGenerator->nameToKey("ReplayObserver")));
      TheRadar->forceOn(TRUE);
      ThePartitionManager->refreshShroudForLocalPlayer();
      TheControlBar->setControlBarSchemeByPlayer(
...
// If defined, the game times various units.
  ...
  fputs(
      "Full,100*ms,NoPart-NoSpawn,,No "
      "Spawn,100*ms,Logic,100*ms,Thing,Model,Kind,Side,DrawCalls All,DrawCalls "
      "NoPart-NoSpawn,DrawCalls NoSpawn\n",
      g_UT_commaLog);
  ...
  // If we are now starting a multiplayer or skirmish game, let us set the local
  // players selectionto be the command center We'll ask the Recorder, so we
  // survive replays
  if (TheRecorder->isMultiplayer()) {
    ...
    for (Int i = 0; i < MAX_PLAYER_COUNT; ++i) {
      ...
      if (player && player->isPlayerActive()) {
        // we need to iterate their objects, and select the first Command Center
        // we find
...
#endif
  TheWritableGlobalData->m_loadScreenRender =
      FALSE;  ///< mark to resume rendering as normal
  ...
  if (TheGameSpyBuddyMessageQueue && TheGameSpyGame && isInInternetGame()) {
    ...
    sprintf(
        req.arg.status.locationString, "%s",
  ...
  // Assume that getting this far means we've successfully entered an online
  // game. Add an additional disconnection to player stats in case he doesn't
  // complete this game. -MW
  if (TheGameSpyInfo) TheGameSpyInfo->updateAdditionalGameSpyDisconnections(1);

}  // end startNewGame

//-----------------------------------------------------------------------------------------
static void findAndSelectCommandCenter(Object *obj, void *alreadyFound) {
  if (!((*(Bool *)alreadyFound)) && obj &&
      obj->isKindOf(KINDOF_COMMANDCENTER)) {
    ((*(Bool *)alreadyFound)) = TRUE;
    TheGameLogic->selectObject(obj, TRUE,
                               obj->getControllingPlayer()->getPlayerMask(),
...
void GameLogic::loadMapINI(AsciiString mapName) {
  ...
  // that has been extracted from a save game file ... in that case we need to
  // get the pristine map name string in order to manipulate and load the right
  // map.ini for that map from it's original location
  //
...
void GameLogic::processDestroyList(void) {
  ...
  for (ObjectPointerListIterator iterator = m_objectsToDestroy.begin();
    ...
    /*
            this looks odd, but is necessary; since erasing a single entry can
       shuffle others in the list (in order to maintain its heap-ness), we must
       do two passes: one to find the updates for this object, another to
       actually erase 'em.

            (in case you're wondering: yes, this is still more efficient than
       just deleting them and rebalancing the entire heap afterwards, at least
       for real-world maps, since an individual rebalance is O(log N) and a full
       rebalance is O(N)... so unless you are deleting the majority of the
       objects in the world every frame, we come out well ahead this way.)
    */
    ...
    for (std::vector<UpdateModulePtr>::iterator it2 = m_sleepyUpdates.begin();
         it2 != m_sleepyUpdates.end(); ++it2) {
    ...
    for (--numSUO; numSUO >= 0; --numSUO) {
      // have to re-get idx each time since each call to erase might change
      // others.
      Int idx = sleepyUpdatesForThisObject[numSUO]->friend_getIndexInLogic();
      DEBUG_ASSERTCRASH(
          m_sleepyUpdates[idx] == sleepyUpdatesForThisObject[numSUO],
          ("Hmm, expected update mismatch here"));
      eraseSleepyUpdate(idx);
      DEBUG_ASSERTCRASH(
          sleepyUpdatesForThisObject[numSUO]->friend_getIndexInLogic() == -1,
          ("Hmm, expected index to be -1 here"));
    }
  ...
  m_objectsToDestroy.clear();  // list full of bad pointers now, clear it.  If
                               // anyone's deletion resulted
  // in the request for a new deletion (sub-object), the new object was added to
...
void GameLogic::processCommandList(CommandList *list) {
...
#ifdef _DEBUG
    DEBUG_ASSERTCRASH(msg != NULL && msg != (GameMessage *)0xdeadbeef,
                      ("bad msg"));
#endif
  ...
  if (m_shouldValidateCRCs && !TheNetwork->sawCRCMismatch()) {
    ...
    if (TheNetwork) {
      for (Int i = 0; i < MAX_SLOTS; ++i) {
        if (TheNetwork->isPlayerConnected(i)) ++numPlayers;
      }
    ...
    if (sawCRCMismatch) {
      ...
      for (std::map<Int, UnsignedInt>::const_iterator crcIt =
               m_cachedCRCs.begin();
...
void GameLogic::selectObject(Object *obj, Bool createNewSelection,
  ...
  if (!obj->isMassSelectable() && !createNewSelection) {
    DEBUG_LOG(
        ("GameLogic::selectObject() - Object attempted to be added to "
         "selection, but isn't mass-selectable.\n"));
    return;
  }
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void GameLogic::deselectObject(Object *obj, PlayerMaskType playerMask,
                               Bool affectClient) {
  if (!obj) {
    return;
  }
  ...
  while (playerMask) {
    ...
    AIGroup *group = NULL;
    CRCGEN_LOG(
        ("Removing a unit from a selected group in "
...
#define SLEEPY_DEBUG
  ...
  // for (i = 0; i < sz; ++i)
  //{
  //	DEBUG_LOG(("u %04d: %08lx
  //%08lx\n",i,m_sleepyUpdates[i],m_sleepyUpdates[i]->friend_getNextCallFrame()));
  // }
  for (i = 0; i < sz; ++i) {
    DEBUG_ASSERTCRASH(m_sleepyUpdates[i]->friend_getIndexInLogic() == i,
                      ("index mismatch: expected %d, got %d\n", i,
                       m_sleepyUpdates[i]->friend_getIndexInLogic()));
...
Int GameLogic::rebalanceParentSleepyUpdate(Int i) {
  ...
  Int parent = ((i + 1) >> 1) - 1;
  while (parent >= 0 &&
         isLowerPriority(m_sleepyUpdates[parent], m_sleepyUpdates[i])) {
...
// version present for clarity. (Yes, this is worth doing.) (srj)
  ...
  UpdateModulePtr *pSZ =
      &m_sleepyUpdates[m_sleepyUpdates.size()];  // yes, this is off the end.

  while (pChild < pSZ) {
    // choose the higher-priority of the two children; we must be higher-pri
    // than that.
  ...
  while (child < sz) {
    ...
    // than that.
    if (child < sz - 1 &&
        isLowerPriority(m_sleepyUpdates[child], m_sleepyUpdates[child + 1]))
      ++child;
...
void GameLogic::pushSleepyUpdate(UpdateModulePtr u) {
  ...
  DEBUG_ASSERTCRASH(u != NULL,
                    ("You may not pass null for sleepy update info"));
...
UpdateModulePtr GameLogic::peekSleepyUpdate() const {
  ...
  DEBUG_ASSERTCRASH(u->friend_getIndexInLogic() == 0,
                    ("index mismatch: expected %d, got %d\n", 0,
                     u->friend_getIndexInLogic()));
  return u;
}
...
void GameLogic::friend_awakenUpdateModule(Object *obj, UpdateModulePtr u,
                                          UnsignedInt whenToWakeUp) {
  // USE_PERF_TIMER(friend_awakenUpdateModule)
  UnsignedInt now = TheGameLogic->getFrame();
  DEBUG_ASSERTCRASH(whenToWakeUp >= now,
                    ("setWakeFrame frame is in the past... are you sure this "
                     "is what you want?"));

  if (u == m_curUpdateModule) {
    DEBUG_CRASH((
        "You should not call setWakeFrame() from inside your update(), because "
        "it will be ignored, in favor of the return code from update.\n"));
    return;
  }
  ...
  if ((now > 0) && (u->friend_getNextCallFrame() == now) &&
      (whenToWakeUp == now + 1)) {
    // subtle but important case: if we already awake, and someone calls
    // setWakeFrame(self, UPDATE_SLEEP_NONE), we don't want to reset our wake
    // frame, since that would prevent us from getting called THIS frame. since
    // UPDATE_SLEEP_NONE really means "wake up as soon as possible", we don't
    // want to change our status if we are already awake. (srj)
    return;
  }
...
#define DO_CIVILIAN
  ...
  const Int FACTOR =
      5;  // run at TIME_FRAMES/FACTOR so we dont' die of boredom.  jba.
  ...
  static double timeAll, timeAllNoAnim, timeNoPart, timeNoSpawn, timeLogic,
      timeLogicNoAnim;
  ...
  if (timeFrames > 0) {
    ...
    if (mode == LOGIC) {
      ...
      drawCallLogic = (float)drawCallTotal /
                      (float)(TIME_FRAMES * 100);  // 100 units for TIME_FRAMES
    ...
    if (mode == ALL) {
      ...
      drawCallAll = (float)drawCallTotal /
                    (float)(TIME_FRAMES * 100);  // 100 units for TIME_FRAMES
    ...
    if (mode == NO_PARTICLES) {
      ...
      drawCallNoPart = (float)drawCallTotal /
                       (float)(TIME_FRAMES * 100);  // 100 units for TIME_FRAMES
    ...
    sprintf(foo,
            ...
            timeAll, (Int)(timeAll * 1000 / TIME_FRAMES), thingName.str(),
            timeNoPart, timeNoSpawn, timeLogic);
    DEBUG_LOG((foo));
    sprintf(
        foo,
        "\nDrawCalls for 100 %s , all %d, noPart %d, noSpawn %d logic %d \n",
        thingName.str(), drawCallAll, drawCallNoPart, drawCallNoSpawn,
        drawCallLogic);
    DEBUG_LOG((foo));
    ...
    if (g_UT_commaLog) {
      ...
      if (mi.getCount() > 0) {
        ...
        const W3DModelDrawModuleData *md =
            mdd ? mdd->getAsW3DModelDrawModuleData() : NULL;
        if (md) {
      ...
      sprintf(foo, "%f,%d,%f,%d,%f,%d,%f,%d,%s,%s,%s,%s,%f,%f,%f\n", timeAll,
              ...
              (Int)(timeLogic * 1000 / TIME_FRAMES), thingName.str(),
              modelName.str(), type.str(), sides[side].str(), drawCallAll,
              drawCallNoPart, drawCallNoSpawn);
  ...
  if (g_UT_startTiming && g_UT_curThing && !g_UT_gotUnit) {
    ...
    while (!g_UT_gotUnit) {
      ...
      if (btt->getName() == "GLAAngryMobRockProjectileObject")
        continue;  // 100 overloads system.
      if (btt->getName() == "ClusterMinesBomb")
        continue;  // 100 overloads system.
      if (btt->getName() == "BlackNapalmFirestormSmall")
        continue;  // 100 overloads system.
      if (btt->getName() == "CabooseFullOfTerrorists")
        continue;  // 100 overloads system.
      if (btt->getName() == "GLAAngryMobMolotovCocktailProjectileObject")
        continue;  // 100 overloads system.
      if (btt->getName().startsWith("Firestorm")) continue;  // 100 crashes
      if (btt->getName().startsWith("Avalanche")) continue;  // 100 crashes
      if (btt->getName().startsWith("InfernoTankShell"))
        continue;  // 100 crashes

      if (btt->getName() == "ChinaArtilleryBarrageShell")
        continue;  // 100 takes really, freaking long. Doesn't crash jba.
      if (btt->getName() == "ChinaTankOverlordBattleBunker")
        continue;  // 100 seems to hang gth.
      if (btt->getName() == "ChinaTankOverlordPropagandaTower")
        continue;  // 100 seems to hang gth.
      if (btt->getName() == "ChinaTankOverlordGattlingCannon")
        continue;  // 100 seems to hang gth.
      ...
      for (i = 0; i < 10; i++) {
        for (j = 0; j < 10; j++) {
          ...
          if (obj) {
            ...
            // Now onCreates were called at the constructor.  This magically
            // created thing needs to be considered as Built for Game specific
            // stuff.
            for (BehaviorModule **m = obj->getBehaviorModules(); *m; ++m) {
              CreateModuleInterface *create = (*m)->getCreate();
              if (!create) continue;
...
void GameLogic::update(void) {
  ...
  Bool freezeTime = TheTacticalView->isTimeFrozen() &&
  ...
  if (freezeTime) {
    if (TheCommandList->containsMessageOfType(
  ...
  // Note - TerrainLogic update needs to happen after ScriptEngine update, but
  // before object updates.  jba. This way changes in bridges are noted in the
  // script engine before being cleared in TerrainLogic->update
  {
    TheTerrainLogic->UPDATE();
  }

  // force CRC calculation, so we can keep a cache of the last N CRCs.  We do
  // this right where the recorder would be getting the CRC anyway, so replays
  // can get the CRCs from the exact instant in time as the original.
  Bool isMPGameOrReplay =
      (TheRecorder && TheRecorder->isMultiplayer() &&
       getGameMode() != GAME_SHELL && getGameMode() != GAME_NONE);
  Bool isSoloGameOrReplay =
      (TheRecorder && !TheRecorder->isMultiplayer() &&
       getGameMode() != GAME_SHELL && getGameMode() != GAME_NONE);
  Bool generateForMP =
      (isMPGameOrReplay && (m_frame % TheGameInfo->getCRCInterval()) == 0);
// #if defined(_DEBUG) || defined(_INTERNAL)
#ifdef DEBUG_CRC
  Bool generateForSolo =
      isSoloGameOrReplay && ((m_frame && (m_frame % 100 == 0)) ||
                             (getFrame() > TheCRCFirstFrameToLog &&
                              getFrame() < TheCRCLastFrameToLog &&
                              ((m_frame % REPLAY_CRC_INTERVAL) == 0)));
#else
  Bool generateForSolo =
      isSoloGameOrReplay && ((m_frame % REPLAY_CRC_INTERVAL) == 0);
#endif  // DEBUG_CRC
  ...
  if (generateForSolo || generateForMP) {
    ...
    if (isMPGameOrReplay) {
      GameMessage *msg =
          TheMessageStream->appendMessage(GameMessage::MSG_LOGIC_CRC);
      msg->appendIntegerArgument(m_CRC);
      msg->appendBooleanArgument(
          (TheRecorder && TheRecorder->getMode() ==
                              RECORDERMODETYPE_PLAYBACK));  // playback CRC
...
#ifdef ALLOW_NONSLEEPY_UPDATES
  {
    for (std::list<UpdateModulePtr>::const_iterator it =
             m_normalUpdates.begin();
         it != m_normalUpdates.end(); ++it) {
      UpdateModulePtr u = *it;
      DisabledMaskType dis = u->friend_getObject()->getDisabledFlags();
      if (!dis.any() ||
          dis.anyIntersectionWith(u->getDisabledTypesToProcess())) {
        ...
        DEBUG_ASSERTCRASH(
            ...
            ("you must return SLEEPNONE from all nonsleepy modules"));
#else
        u->update();
#endif
...

  {
    while (!m_sleepyUpdates.empty()) {
      ...
      UpdateSleepTime sleepLen =
          UPDATE_SLEEP_NONE;  // default, if it is disabled.
      ...
      if (!dis.any() ||
        ...
        // DEBUG_LOG(("calling update %08lx (%d %d)...
        // ",update,update->friend_getNextCallFrame(),update->friend_getNextCallPhase()));
        m_curUpdateModule = u;

        sleepLen = u->update();
        DEBUG_ASSERTCRASH(sleepLen > 0, ("you may not return 0 from update"));
        if (sleepLen < 1) sleepLen = UPDATE_SLEEP_NONE;
...
// ------------------------------------------------------------------------------------------------
Object *GameLogic::getFirstObject(void) { return m_objList; }
...
void GameLogic::registerObject(Object *obj) {
  ...
  for (BehaviorModule **b = obj->getBehaviorModules(); *b; ++b) {
    ...
    // this is legal.
    if (when == 0) u->friend_setNextCallFrame(now);
#endif
    {
      DEBUG_ASSERTCRASH(u->friend_getNextCallFrame() >= now,
                        ("you may not specify a zero initial sleep time for "
                         "sleepy modules (%d %d)",
                         u->friend_getNextCallFrame(), now));
      pushSleepyUpdate(u);
    }
  }
}
...
// ------------------------------------------------------------------------------------------------
/** Mark the object as destroyed, and place on list for deletion at the end of
 * the next update. This is the only interface to destroy objects - objects
...
void GameLogic::destroyObject(Object *obj) {
  ...
  for (BehaviorModule **m = obj->getBehaviorModules(); *m; ++m) {
    ...
    if (destroy) destroy->onDestroy();
  }

  // mark object as destroyed
  obj->setStatus(OBJECT_STATUS_DESTROYED);

  // We desperately need to stop here, or else the destructor of the
  // statemachine will try to do stopping logic, which uses virtual functions
  // and deleted modules, which will crash us.
  ...
  // add to end of destruction list, in case something is being destroyed and
  // trying to destroy subobjects
...
UnsignedInt GameLogic::getCRC(Int mode, AsciiString deepCRCFileName) {
  if (mode != CRC_RECALC) return m_CRC;
  ...
  if (isInGameLogicUpdate()) {
    CRCGEN_LOG(
        ("CRC at start of frame %d is 0x%8.8X\n", m_frame, xferCRC->getCRC()));
  }
  ...
  if (isInGameLogicUpdate()) {
    CRCGEN_LOG(
        ("CRC after AI for frame %d is 0x%8.8X\n", m_frame, xferCRC->getCRC()));
  }

  if (xferCRC->getXferMode() == XFER_SAVE) {
    marker = "MARKER:GameSave";
    xferCRC->xferAsciiString(&marker);
    TheGameState->friend_xferSaveDataForCRC(xferCRC,
...
// ------------------------------------------------------------------------------------------------
Bool GameLogic::isGamePaused(void) { return m_gamePaused; }
...
void GameLogic::setGamePaused(Bool paused, Bool pauseMusic) {
  ...
  // GameUnpaused
  // Set mouse the way it "was" <--- Was counting on right answer being set in
  // Pause.
  if (paused == m_gamePaused) return;

  m_gamePaused = paused;

  AudioAffect audToAffect =
      (AudioAffect)(pauseMusic ? AudioAffect_All
                               : (AudioAffect_All & ~AudioAffect_Music));

  if (paused) {
    // remember the state of the mouse/input so we can return to the same state
    // once we "unpause"
    ...
    if (m_inputEnabledMemory) TheInGameUI->setInputEnabled(TRUE);
...
void GameLogic::processProgress(Int playerId, Int percentage) {
  if (m_loadScreen) m_loadScreen->processProgress(playerId, percentage);
...
void GameLogic::processProgressComplete(Int playerId) {
  if (playerId < 0 || playerId >= MAX_SLOTS) {
    DEBUG_ASSERTCRASH(FALSE, ("GameLogic::processProgressComplete, Invalid "
                              "playerid was passed in %d\n",
                              playerId));
    return;
  }
  if (m_progressComplete[playerId] == TRUE) {
    DEBUG_LOG(
        ("GameLogic::processProgressComplete, playerId %d is marked TRUE "
         "already yet we're trying to mark him as true again\n",
         playerId));
    return;
  }
...
Bool GameLogic::isProgressComplete(void) {
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    if (!m_progressComplete[i]) return FALSE;
  }
  return TRUE;
}
...
void GameLogic::lastHeardFrom(Int playerId) {
  if (playerId < 0 || playerId >= MAX_SLOTS) return;
...
void GameLogic::testTimeOut(void) {
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (m_progressComplete[i]) continue;
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
TerrainLogic *GameLogic::createTerrainLogic(void) { return NEW TerrainLogic; }
...
Bool GameLogic::findBuildableStatusOverride(const ThingTemplate *tt,
  ...
  if (tt) {
    BuildableMap::const_iterator it =
        m_thingTemplateBuildableOverrides.find(tt->getName());
...
Bool GameLogic::findControlBarOverride(
    const AsciiString &commandSetName, Int slot,
...
#ifdef DUMP_PERF_STATS
// ------------------------------------------------------------------------------------------------
void GameLogic::getAIMetricsStatistics(UnsignedInt *numAI,
...
/** Light CRC */
// ------------------------------------------------------------------------------------------------
void GameLogic::crc(Xfer *xfer) {}  // end crc
...
GameLogic::ObjectTOCEntry *GameLogic::findTOCEntryByName(AsciiString name) {
  for (ObjectTOCListIterator it = m_objectTOC.begin(); it != m_objectTOC.end();
       ++it)
...
void GameLogic::xferObjectTOC(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (Object *obj = getFirstObject(); obj; obj = obj->getNextObject()) {
      ...
      if (findTOCEntryByName(templateName) != NULL) continue;
...
void GameLogic::prepareLogicForObjectLoad(void) {
  //
  // this is a band-aid :(
  // when loading from a map file, objects were created for the bridges, towers,
  // walls etc. since we're going to load that data from the save game file we
  // will destroy any of those bridge objects and tower objects so that we can
  // create them from the save data
  //
  Object *obj, *next;
  for (obj = getFirstObject(); obj; obj = next) {
    ...
    if (obj->isKindOf(KINDOF_BRIDGE)) {
      ...
      DEBUG_ASSERTCRASH(
          bridge,
          ("GameLogic::prepareLogicForObjectLoad - Unable to find bridge\n"));
      ...
      DEBUG_ASSERTCRASH(oldObject, ("GameLogic::prepareLogicForObjectLoad - "
                                    "Unable to find old bridge object\n"));
      DEBUG_ASSERTCRASH(
          oldObject == obj,
          ("GameLogic::prepareLogicForObjectLoad - obj != oldObject\n"));

      //
      // destroy the 4 towers that are attached to this old object (they will be
      // loaded from the save game file
      //
      Object *oldTower;
  ...
  // process the destruction of these objects immediately before we proceed with
  // the load process
  processDestroyList();
  ...
  DEBUG_ASSERTCRASH(
      getFirstObject() == NULL,
      ("GameLogic::prepareLogicForObjectLoad - There are still objects loaded "
       "in the engine, but it should be empty (Top is '%s')\n",
 ...
 * 4: Added block markers around object data, no version checking is done and
 *therefore this version breaks compatibility with previous versions. (CBD) 5:
 *Added xfering the BuildAssistant's sell list. 9: Added
 *m_rankPointsToAddAtGameStart, or else on a load game, your RestartGame button
 *will forget your exp
 */
// ------------------------------------------------------------------------------------------------
void GameLogic::xfer(Xfer *xfer) {
  ...
  //
  // note that we do not do the id counter here, we did it in the game state
  // block because it's important to do that part very early in the load process
  //
  // !!!DON'T DO THIS!!! ----> xfer->xferObjectID( &m_nextObjectID ); <----
  // !!!DON'T DO THIS!!!

  //
  // xfer a table of contents that contain thing template and indentifier pairs.
  // this table of contents is good for this save file only as unique numbers
  // are only generated and stored for the actual things that are on this map
  //
  xferObjectTOC(xfer);

  // when loading, we need to clean up bridges and get them ready for a load
  if (xfer->getXferMode() == XFER_LOAD) prepareLogicForObjectLoad();
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (UnsignedInt i = 0; i < objectCount; ++i) {
      ...
      if (tocEntry == NULL) {
        DEBUG_CRASH(
            ("GameLogic::xfer - No TOC entry match for id '%d'\n", tocID));
        throw SC_INVALID_DATA;
      ...
      if (thingTemplate == NULL) {
        DEBUG_CRASH(
            ("GameLogic::xfer - Unrecognized thing template name '%s', "
             "skipping.  ENGINEERS - Are you *sure* it's OK to be ignoring "
             "this object from the save file???  Think hard about it!\n",
             tocEntry->name.str()));
      ...
      // end of block of data (not necessary in a load, but looks symettrically
      // nice)
      xfer->endBlock();
  ...
  // is scoring enabled
  if (version >= 2) xfer->xferBool(&m_isScoringEnabled);
  ...
  if (version >= 3) {
    ...
    for (poly = PolygonTrigger::getFirstPolygonTrigger(); poly;
         poly = poly->getNext())
      triggerCount++;
    ...
    //
    // since the save game loaded should have exactly the same number of polygon
    // triggers in it that we did from loading the map this is just a sanity
    // check here
    //
    if (sanityTriggerCount != triggerCount) {
      DEBUG_CRASH(
          ("GameLogic::xfer - Polygon trigger count mismatch.  Save file has a "
           "count of '%d', but map had '%d' triggers\n",
    ...
    if (xfer->getXferMode() == XFER_SAVE) {
      ...
      for (UnsignedInt i = 0; i < triggerCount; ++i) {
        ...
        if (poly == NULL) {
          DEBUG_CRASH((
              "GameLogic::xfer - Unable to find polygon trigger with id '%d'\n",
              triggerID));
          throw SC_INVALID_DATA;
      ...
      //
      // force a recalculation of the pathfinding cause some of these polygon
      // triggers are water which can move during run-time, and therefore
      // affecting the area that objects can move on. Also, map objects need
      // recalculation. jba.
      //
      TheAI->pathfinder()->newMap();
  ...
  if (version >= 7) {
    if (xfer->getXferMode() == XFER_SAVE) {
      for (BuildableMap::const_iterator it =
               m_thingTemplateBuildableOverrides.begin();
           it != m_thingTemplateBuildableOverrides.end(); ++it) {
      ...
      if (m_thingTemplateBuildableOverrides.empty() == false) {
        DEBUG_CRASH(
            ("GameLogic::xfer - m_thingTemplateBuildableOverrides should be "
             "empty, but is not\n"));
        throw SC_INVALID_DATA;
      }
  ...
  if (version >= 8) {
    ...
    if (xfer->getXferMode() == XFER_SAVE) {
      for (ControlBarOverrideMap::const_iterator it =
               m_controlBarOverrides.begin();
           it != m_controlBarOverrides.end(); ++it) {
        AsciiString name = it->first;
        AsciiString value =
            it->second ? it->second->getName() : AsciiString::TheEmptyString;
        xfer->xferAsciiString(&name);
      ...
      if (m_controlBarOverrides.empty() == false) {
        DEBUG_CRASH(
            ("GameLogic::xfer - m_controlBarOverrides should be empty, but is "
             "not\n"));
        throw SC_INVALID_DATA;
      }
      ...
      for (;;) {
        ...
        if (value.isNotEmpty()) {
          ...
          DEBUG_ASSERTCRASH(button != NULL,
                            ("Could not find button %s\n", value.str()));
        }
...
void GameLogic::loadPostProcess(void) {
  //
  // the act of loading objects can (theoretically) as a side effect create
  // other objects, our m_nextObjID that we maintain to give objects unique ID
  // is also continually climbing higher and higher due to us allocating objects
  // during load (even though those objects have their ids overwritten with data
  // from the file.  To prevent the m_nextObjID from getting un-necessarily high
  // we will set it to the next available id from the objects that are now in
  // the world and actually in use
  //
  m_nextObjID = INVALID_ID;
  Object *obj;
  for (obj = getFirstObject(); obj; obj = obj->getNextObject())
  ...
  // go through all objects, examine each update module and put it on the
  // appropriate update list
  for (obj = getFirstObject(); obj; obj = obj->getNextObject()) {
    ...
    for (BehaviorModule **b = obj->getBehaviorModules(); *b; ++b) {
      ...
      DEBUG_ASSERTCRASH(u->friend_getIndexInLogic() == -1,
                        ("Hmm, expected index to be -1 here"));
