...
CrateSystem::CrateSystem() { m_crateTemplateVector.clear(); }
...
void CrateSystem::init(void) { reset(); }
...
void CrateSystem::parseCrateTemplateDefinition(INI *ini) {
  ...
  if (crateTemplate == NULL) {
    ...
    DEBUG_CRASH(("[LINE: %d in '%s'] Duplicate crate %s found!",
...
CrateTemplate *CrateSystem::newCrateTemplate(AsciiString name) {
  ...
  // new template
  const CrateTemplate *defaultCT =
      findCrateTemplate(AsciiString("DefaultCrate"));
  if (defaultCT) {
    *ct = *defaultCT;
  }
...
CrateTemplate *CrateSystem::newCrateTemplateOverride(
    CrateTemplate *crateToOverride) {
  if (!crateToOverride) {
    return NULL;
  }
...
const FieldParse CrateTemplate::TheCrateTemplateFieldParseTable[] = {
    {"CreationChance", INI::parseReal, NULL,
     offsetof(CrateTemplate, m_creationChance)},
    {"VeterancyLevel", INI::parseIndexList, TheVeterancyNames,
     offsetof(CrateTemplate, m_veterancyLevel)},
    {"KilledByType", KindOfMaskType::parseFromINI, NULL,
     offsetof(CrateTemplate, m_killedByTypeKindof)},
    {"CrateObject", CrateTemplate::parseCrateCreationEntry, NULL, NULL},
    {"KillerScience", INI::parseScience, NULL,
     offsetof(CrateTemplate, m_killerScience)},
    {"OwnedByMaker", INI::parseBool, NULL,
     offsetof(CrateTemplate, m_isOwnedByMaker)},
...
CrateTemplate::~CrateTemplate() { m_possibleCrates.clear(); }

void CrateTemplate::parseCrateCreationEntry(INI *ini, void *instance, void *,
                                            const void *) {
  ...
  Real crateValue;
  if (sscanf(token, "%f", &crateValue) != 1) throw INI_INVALID_DATA;
