...
void CaveSystem::reset() {
  for (std::vector<TunnelTracker *>::iterator iter =
           m_tunnelTrackerVector.begin();
       iter != m_tunnelTrackerVector.end(); iter++) {
    TunnelTracker *currentTracker = *iter;
    if (currentTracker)  // could be NULL, since we don't slide back to fill
                         // deleted entries so offsets don't shift
    {
...
Bool CaveSystem::canSwitchIndexToIndex(Int oldIndex, Int newIndex) {
  // When I grant permission, you need to do it.  ie call Unregister and then
  // re-register with the new number
  ...
  if (m_tunnelTrackerVector.size() > oldIndex) {
    ...
    if (oldTracker && oldTracker->getContainCount() > 0)
      return FALSE;  // You can't switch a connection if one of the two is non
                     // empty
  }
  ...
  // (Remember non-exist is only a valid case because you are going to do the
  // switch now.)
...
void CaveSystem::registerNewCave(Int theIndex) {
  ...
  if (theIndex >= m_tunnelTrackerVector.size()) {
    // You are new and off the edge, so I will fill NULLs up to you and then
    // make a newTracker at that spot
    while (theIndex >= m_tunnelTrackerVector.size())
      m_tunnelTrackerVector.push_back(NULL);

    needToCreate = TRUE;
  } else {
    // else you either exist or have existed, so I will either let things be or
    // re-create that slot
    if (m_tunnelTrackerVector[theIndex] == NULL) needToCreate = TRUE;
  }

  if (needToCreate)  // if true, we new theIndex is the index of a NULL to be
                     // filled
    m_tunnelTrackerVector[theIndex] = newInstance(TunnelTracker);
}

void CaveSystem::unregisterCave(Int theIndex) {
  // Doesn't need to do a thing.  ContainModule logic knows how to say goodbye,
  // and a TunnelTracker knows how to exist while having no entry points.
  theIndex;
}
...
TunnelTracker *CaveSystem::getTunnelTrackerForCaveIndex(Int theIndex) {
  ...
  DEBUG_ASSERTCRASH(
      theTracker != NULL,
      ("No one should be interested in a sub-cave that doesn't exist."));
...
void CaveSystem::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (it = m_tunnelTrackerVector.begin(); it != m_tunnelTrackerVector.end();
         ++it) {
      // xfer data
      tracker = *it;
    ...
    if (m_tunnelTrackerVector.empty() == FALSE) {
      DEBUG_CRASH(
          ("CaveSystem::xfer - m_tunnelTrackerVector should be empty but is "
           "not\n"));
