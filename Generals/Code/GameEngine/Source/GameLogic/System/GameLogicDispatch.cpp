...
// //////////////////////////////////////////////////////////////////// Author:
// Mike Booth, Colin Day Description: Message logic to drive the game play
///////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if (ai) {
    if (theBuildPlan) {
      ...
      for (i = 0; i < thePlanSubjectCount; i++)
        if (thePlanSubject[i] == obj) break;

      if (i == thePlanSubjectCount) thePlanSubject[thePlanSubjectCount++] = obj;

      ai->queueWaypoint(pos);
    } else {
      ai->clearWaypointQueue();
      obj->leaveGroup();
      obj->releaseWeaponLock(
          LOCKED_TEMPORARILY);  // release any temporary locks.
...
static void doSetRallyPoint(Object *obj, const Coord3D &pos) {
  ...
  //
  // we must be able to find a path from the object to the point they have
  // chosen, cause setting a rally point at a invalid location would suck.  To
  // be super nice, we have to make sure that every type of object that can be
  // created from the thing setting the rally point can actually find a path
  // from the thing to the point
  //

  // to see the never-finished code to check all locomotor sets, see past revs
  // of GUICommandTranslator.cpp -MDC

  //
  // for now, just use the basic human locomotor ... and enable the above code
  // when Steven tells me how to get the locomotor sets based on a thing
  // template (CBD)
  //
  ...
  if (TheAI->pathfinder()->quickDoesPathExist(locomotorSet, obj->getPosition(),
                                              &pos) == FALSE) {
    // user feedback
    if (isLocalPlayer) {
  ...
  if (isLocalPlayer) {
    ...
    // mark the UI as dirty so that we re-evaluate the selection and show the
    // rally point
    Drawable *draw = obj->getDrawable();
    if (draw && draw->isSelected()) TheControlBar->markUIDirty();

  }  // end if

  // if this object has a ProductionExitUpdate interface, we are setting a rally
  // point
...
static Object *getSingleObjectFromSelection(
    const AIGroup *currentlySelectedGroup) {
  if (currentlySelectedGroup) {
    const VecObjectID &selectedObjects = currentlySelectedGroup->getAllIDs();
    DEBUG_ASSERTCRASH(selectedObjects.size() == 1,
                      ("Trying to get single object from multiple selection!"));
...
void GameLogic::clearGameData(Bool showScoreScreen) {
  ...
  //	DEBUG_ASSERTCRASH(m_background,("We Couldn't Load
  //Menus/BlankWindow.wnd")); 	m_background->hide(FALSE);
  //	m_background->bringForward();
  // reset the game engine to accept data for a new game
  if (TheStatsCollector) TheStatsCollector->writeFileEnd();
  ...
  if ((!isInShellGame() || !isInGame()) && showScoreScreen) {
    ...
    TheShell->showShell(
        FALSE);  // by passing in false, we don't want to run the Init on the
                 // shell screen we just pushed on
...
// ------------------------------------------------------------------------------------------------
void GameLogic::prepareNewGame(Int gameMode, GameDifficulty diff,
                               Int rankPoints) {
  ...
  DEBUG_LOG(
      ("GameLogic::prepareNewGame() - m_rankPointsToAddAtGameStart = %d\n",
       m_rankPointsToAddAtGameStart));

  // If we're about to start a game, hide the shell.
  if (!TheGameLogic->isInShellGame()) TheShell->hideShell();
...
void GameLogic::logicMessageDispatcher(GameMessage *msg, void *userData) {
  ...
  DEBUG_ASSERTCRASH(thisPlayer, ("logicMessageDispatcher: Processing message "
                                 "from unknown player (player index '%d')\n",
                                 msg->getPlayerIndex()));
  ...
  if (isInGame()) {
    if (msg->getType() >= GameMessage::MSG_BEGIN_NETWORK_MESSAGES &&
      ...
      if (msg->getType() != GameMessage::MSG_LOGIC_CRC &&
          msg->getType() != GameMessage::MSG_SET_REPLAY_CAMERA) {
        currentlySelectedGroup =
            TheAI->createGroup();  // can't do this outside a game - it'll cause
                                   // sync errors galore.
        CRCGEN_LOG(
            ("Creating AIGroup %d in GameLogic::logicMessageDispatcher()\n",
             (currentlySelectedGroup) ? currentlySelectedGroup->getID() : 0));
        thisPlayer->getCurrentSelectionAsAIGroup(currentlySelectedGroup);

        // We can't issue commands to groups that contain units that don't
        // belong the issuing player, so pretend like there's nothing selected.
        // Also, if currentlySelectedGroup is empty, go ahead and delete it, so
        // that we can skip any processing on it.
        if (currentlySelectedGroup->isEmpty()) {
          TheAI->destroyGroup(currentlySelectedGroup);
          currentlySelectedGroup = NULL;
        }

        // If there are any units that the player doesn't own, then remove them
        // from the "currentlySelectedGroup"
        if (currentlySelectedGroup)
          if (currentlySelectedGroup->removeAnyObjectsNotOwnedByPlayer(
                  thisPlayer))
            currentlySelectedGroup = NULL;

        if (TheStatsCollector) TheStatsCollector->collectMsgStats(msg);
      }
    }
  }
  ...
  switch (msgType) {
    ...
    case GameMessage::MSG_NEW_GAME: {
      // DEBUG_ASSERTCRASH(msg->getArgumentCount() == 1 ||
      // msg->getArgumentCount() == 2, ("%d arguments to MSG_NEW_GAME",
    ...
    case GameMessage::MSG_DO_WEAPON: {
      ...
      if (currentlySelectedGroup &&
          currentlySelectedGroup->setWeaponLockForGroup(weaponSlot,
                                                        LOCKED_TEMPORARILY)) {
        currentlySelectedGroup->groupAttackPosition(NULL, maxShotsToFire,
                                                    CMD_FROM_PLAYER);
      }
    ...
    case GameMessage::MSG_COMBATDROP_AT_OBJECT: {
      Object *targetObject =
          TheGameLogic->findObjectByID(msg->getArgument(0)->objectID);

      // issue command for either single object or for selected group
      if (currentlySelectedGroup)
        currentlySelectedGroup->groupCombatDrop(
            targetObject, *targetObject->getPosition(), CMD_FROM_PLAYER);

      /*
                              if( sourceObject && targetObject )
                              {
                                      AIUpdateInterface* sourceAI =
         sourceObject->getAIUpdateInterface(); if (sourceAI)
                                      {
                                              sourceAI->aiCombatDrop(
         targetObject, *targetObject->getPosition(), CMD_FROM_PLAYER );
                                      }
                              }
      */
    ...
    case GameMessage::MSG_COMBATDROP_AT_LOCATION: {
      ...
      if (currentlySelectedGroup)
        currentlySelectedGroup->groupCombatDrop(NULL, targetLoc,
                                                CMD_FROM_PLAYER);
      ...
      /*
         ...
         sourceObject->getAIUpdateInterface(); if (sourceAI)
                                      {
                                              sourceAI->aiCombatDrop( NULL,
         targetLoc, CMD_FROM_PLAYER );
                                      }
                              }
      */
    ...
    case GameMessage::MSG_DO_WEAPON_AT_OBJECT: {
      ...
      if (currentlySelectedGroup) {
        // lock it just till the weapon is empty or the attack is "done"
        if (currentlySelectedGroup->setWeaponLockForGroup(weaponSlot,
                                                          LOCKED_TEMPORARILY))
          currentlySelectedGroup->groupAttackObject(
              targetObject, maxShotsToFire, CMD_FROM_PLAYER);
    ...
    case GameMessage::MSG_DO_WEAPON_AT_LOCATION: {
      ...
      if (currentlySelectedGroup) {
        ...
        if (currentlySelectedGroup->setWeaponLockForGroup(weaponSlot,
                                                          LOCKED_TEMPORARILY))
          currentlySelectedGroup->groupAttackPosition(
              &targetLoc, maxShotsToFire, CMD_FROM_PLAYER);
    ...
    case GameMessage::MSG_DO_SPECIAL_POWER_AT_LOCATION: {
      ...
      if (source != NULL) {
        AIGroup *theGroup = TheAI->createGroup();
        theGroup->add(source);
        theGroup->groupDoSpecialPowerAtLocation(specialPowerID, &targetCoord,
                                                objectInWay, options);
        TheAI->destroyGroup(theGroup);
      } else {
        // Use the selected group!
        if (currentlySelectedGroup) {
          currentlySelectedGroup->groupDoSpecialPowerAtLocation(
              specialPowerID, &targetCoord, objectInWay, options);
        }
      }
      break;
    ...
    case GameMessage::MSG_DO_SPECIAL_POWER_AT_OBJECT: {
      ...
      if (source != NULL) {
        ...
        if (currentlySelectedGroup) {
          currentlySelectedGroup->groupDoSpecialPowerAtObject(specialPowerID,
                                                              target, options);
        }
      }
      break;
    ...
    case GameMessage::MSG_DO_ATTACKMOVETO: {
      ...
      if (currentlySelectedGroup) {
        currentlySelectedGroup->releaseWeaponLockForGroup(
            LOCKED_TEMPORARILY);  // release any temporary locks.
        currentlySelectedGroup->groupAttackMoveToPosition(
            &dest, NO_MAX_SHOTS_LIMIT, CMD_FROM_PLAYER);
      }
    ...
    case GameMessage::MSG_DO_FORCEMOVETO: {
      ...
      if (currentlySelectedGroup) {
        currentlySelectedGroup->releaseWeaponLockForGroup(
            LOCKED_TEMPORARILY);  // release any temporary locks.
        currentlySelectedGroup->groupMoveToPosition(&dest, false,
                                                    CMD_FROM_PLAYER);
      }
    ...
    case GameMessage::MSG_ADD_WAYPOINT: {
      ...
      if (currentlySelectedGroup) {
        ...
        currentlySelectedGroup->releaseWeaponLockForGroup(
            LOCKED_TEMPORARILY);  // release any temporary locks.
        currentlySelectedGroup->groupMoveToPosition(&dest, true,
                                                    CMD_FROM_PLAYER);
      }
    ...
    case GameMessage::MSG_DEBUG_KILL_SELECTION: {
      ...
      if (currentlySelectedGroup) {
        const VecObjectID &selectedObjects =
            currentlySelectedGroup->getAllIDs();
        for (VecObjectID::const_iterator it = selectedObjects.begin();
    ...
    case GameMessage::MSG_DEBUG_HURT_OBJECT: {
      ...
      if (objToHurt) {
        ...
        damageInfo.in.m_amount =
            objToHurt->getBodyModule()->getMaxHealth() / 10.0f;
    ...
    case GameMessage::MSG_DO_SURRENDER: {
      ...
      if (currentlySelectedGroup) {
        Object *objWeSurrenderedTo =
            TheGameLogic->findObjectByID(msg->getArgument(0)->objectID);
        Bool surrender = msg->getArgument(1)->boolean;
        currentlySelectedGroup->groupSurrender(objWeSurrenderedTo, surrender,
                                               CMD_FROM_PLAYER);
      }
      break;
    }
    ...
    case GameMessage::MSG_EXIT: {
      Object *objectWantingToExit =
          TheGameLogic->findObjectByID(msg->getArgument(0)->objectID);
      Object *objectContainingExiter =
          getSingleObjectFromSelection(currentlySelectedGroup);

      // sanity
      if (objectWantingToExit == NULL) break;

      if (objectContainingExiter == NULL) break;

      // sanity, the player must actually control this object
      if (objectWantingToExit->getControllingPlayer() != thisPlayer) break;

      objectWantingToExit->releaseWeaponLock(
          LOCKED_TEMPORARILY);  // release any temporary locks.

      // exit whatever objectWantingToExit is INSIDE of
      AIUpdateInterface *ai = objectWantingToExit->getAIUpdateInterface();
      if (ai) ai->aiExit(objectContainingExiter, CMD_FROM_PLAYER);
      // Just like Enter, Exit needs to know the thing to exit.  This can no
      // longer be assumed because of the Tunnel system. If you do not specify
      // the thing to Exit, it will Exit the thing it thinks it is in.  For a
      // tunnel network, that will be the specific Tunnel it entered.  (Scripts
      // can talk directly to the guy to say Get Out Regardless)
    ...
    case GameMessage::MSG_EVACUATE: {
      ...
      if (currentlySelectedGroup) {
        ...
        // if (hasArgs)
        //	currentlySelectedGroup->groupMoveToAndEvacuate( &pos,
        //CMD_FROM_PLAYER ); else
        ...
        //TheInGameUI->getAllSelectedDrawables(), GameMessage::MSG_EVACUATE );
    ...
    case GameMessage::MSG_INTERNET_HACK: {
      //			ObjectID sourceID = msg->getArgument( 0
      //)->objectID;
      if (currentlySelectedGroup) {
        currentlySelectedGroup->releaseWeaponLockForGroup(
            LOCKED_TEMPORARILY);  // release any temporary locks.
    ...
    case GameMessage::MSG_DO_REPAIR: {
      ...
      // sanity
      if (repairTarget == NULL) break;

      //
      // tell the currently selected group of objects to go repair the target
      // object, note that only one of them will actually go ahead and do the
      // repair
      //
      if (currentlySelectedGroup)
    ...
    case GameMessage::MSG_RESUME_CONSTRUCTION: {
      Object *constructTarget =
          TheGameLogic->findObjectByID(msg->getArgument(0)->objectID);

      // sanity
      if (constructTarget == NULL) break;

      //
      // tell the currently selected group of objects to resume construction on
      // the target object, note that only one of them will go off and resume
      // construction on the target
      //
      if (currentlySelectedGroup)
        currentlySelectedGroup->groupResumeConstruction(constructTarget,
                                                        CMD_FROM_PLAYER);

      // no, this is bad, don't do here, do when POSTING message
      //		pickAndPlayUnitVoiceResponse(
      //TheInGameUI->getAllSelectedDrawables(), msg->getType() );
    ...
    case GameMessage::MSG_DO_SPECIAL_POWER_OVERRIDE_DESTINATION: {
      ...
      if (source != NULL) {
        AIGroup *theGroup = TheAI->createGroup();
        theGroup->add(source);
        theGroup->groupOverrideSpecialPowerDestination(spType, loc,
                                                       CMD_FROM_PLAYER);
        TheAI->destroyGroup(theGroup);
      } else {
        if (currentlySelectedGroup) {
          currentlySelectedGroup->groupOverrideSpecialPowerDestination(
              spType, loc, CMD_FROM_PLAYER);
        }
      }
    }
    ...
    case GameMessage::MSG_DO_ATTACK_OBJECT: {
      ...
      if (enemy) {
        if (currentlySelectedGroup) {
          currentlySelectedGroup->releaseWeaponLockForGroup(
              LOCKED_TEMPORARILY);  // release any temporary locks.
          currentlySelectedGroup->groupAttackObject(enemy, NO_MAX_SHOTS_LIMIT,
                                                    CMD_FROM_PLAYER);
        }
      }
    ...
    case GameMessage::MSG_DO_FORCE_ATTACK_OBJECT: {
      ...
      if (enemy) {
        if (currentlySelectedGroup) {
          currentlySelectedGroup->releaseWeaponLockForGroup(
              LOCKED_TEMPORARILY);  // release any temporary locks.
          currentlySelectedGroup->groupForceAttackObject(
              enemy, NO_MAX_SHOTS_LIMIT, CMD_FROM_PLAYER);
        }
      }
    ...
    case GameMessage::MSG_DO_FORCE_ATTACK_GROUND: {
      ...
      if (currentlySelectedGroup) {
        ...
        //       rangers and scud launchers to reset to primary weapon mode
        //       whenever force attacking while not idle. I fixed this by
        //       enforcing the temporary and permanent modes that are already
        //       set when attempting the new lock. In this case, the temp lock
        //       attempt will fail whenever a permanent lock is in effect, thus
        //       fixing the ranger and scud and allowing the tox tractor to work
        //       as well.
        Bool forceAttackRequiresPrimaryWeapon =
            !currentlySelectedGroup->isIdle();
        if (forceAttackRequiresPrimaryWeapon) {
          currentlySelectedGroup->setWeaponLockForGroup(PRIMARY_WEAPON,
                                                        LOCKED_TEMPORARILY);
          currentlySelectedGroup->groupAttackPosition(pos, NO_MAX_SHOTS_LIMIT,
                                                      CMD_FROM_PLAYER);
    ...
    case GameMessage::MSG_QUEUE_UPGRADE: {
      const UpgradeTemplate *upgradeT = TheUpgradeCenter->findUpgradeByKey(
          (NameKeyType)(msg->getArgument(1)->integer));
      if (!upgradeT)  // sanity
        break;
    ...
    case GameMessage::MSG_CANCEL_UPGRADE: {
      ...
      const UpgradeTemplate *upgradeT = TheUpgradeCenter->findUpgradeByKey(
          (NameKeyType)(msg->getArgument(0)->integer));

      // sanity
      if (producer == NULL || upgradeT == NULL) break;

      // the player must actually control the producer object
      if (producer->getControllingPlayer() != thisPlayer) break;
    ...
    case GameMessage::MSG_QUEUE_UNIT_CREATE: {
      ...
      whatToCreate =
          TheThingFactory->findByTemplateID(msg->getArgument(0)->integer);
      productionID = (ProductionID)msg->getArgument(1)->integer;

      // sanity
      if (producer == NULL || whatToCreate == NULL) break;

      // get the production interface for the producer
      ProductionUpdateInterface *pu = producer->getProductionUpdateInterface();
      if (pu == NULL) {
        DEBUG_ASSERTCRASH(0, ("MSG_QUEUE_UNIT_CREATE: Producer '%s' doesn't "
                              "have a unit production interface\n",
    ...
    case GameMessage::MSG_DOZER_CONSTRUCT_LINE: {
      ...
      if (place == NULL || constructorObject == NULL)
        break;  // These are not crashes, as the object may have died before
                // this message came in

      if (msg->getType() == GameMessage::MSG_DOZER_CONSTRUCT) {
        TheBuildAssistant->buildObjectNow(
            constructorObject, place, &loc, angle,
        ...
        // place the line of structures, the end location being present will
        // make it happen
        TheBuildAssistant->buildObjectLineNow(
            constructorObject, place, &loc, &locEnd, angle,
            constructorObject->getControllingPlayer());
    ...
    case GameMessage::MSG_DOZER_CANCEL_CONSTRUCT: {
      ...
      if (building->getControllingPlayer() != thisPlayer) break;

      // Check to make sure it is actually under construction
      if (!building->testStatus(OBJECT_STATUS_UNDER_CONSTRUCTION)) break;
      ...
      if (!building->testStatus(OBJECT_STATUS_RECONSTRUCTING)) {
        ...
        UnsignedInt amount =
            building->getTemplate()->calcCostToBuild(thisPlayer);
        money->deposit(amount);
      }

      //
      // Destroy the building ... killing the
      // building will automatically cause the dozer also cancel its own
      // building behavior and it will go on its merry way doing other assigned
      // tasks
      //
      building->kill();
    ...
    case GameMessage::MSG_PICK_UP_PRISONER: {
      ...
      if (prisoner) {
        // use selected group
        if (currentlySelectedGroup)
          currentlySelectedGroup->groupPickUpPrisoner(prisoner,
                                                      CMD_FROM_PLAYER);
    ...
    //---------------------------------------------------------------------------------------------
    // No sound does exactly the same logical processing as the usual message.
    // Just double them up.
    ...
    case GameMessage::MSG_CREATE_SELECTED_GROUP: {
      ...
      if (player == NULL) {
        DEBUG_CRASH(
            ("GameLogicDispatch - MSG_CREATE_SELECTED_GROUP had an invalid "
             "player nubmer"));
        break;
      }
      ...
      for (Int i = 1; i < msg->getArgumentCount(); ++i) {
        ...
        TheGameLogic->selectObject(obj, createNewGroup && firstObject,
    ...
    case GameMessage::MSG_PLACE_BEACON: {
      ...
      const ThingTemplate *thing = TheThingFactory->findTemplate(
          thisPlayer->getPlayerTemplate()->getBeaconTemplate());
      if (thing &&
          !TheVictoryConditions->hasSinglePlayerBeenDefeated(thisPlayer)) {
        Int count;
        ...
        Object *object =
            TheThingFactory->newObject(thing, thisPlayer->getDefaultTeam());
        object->setPosition(&pos);
        object->setProducer(NULL);

        if (thisPlayer->getRelationship(
                ThePlayerList->getLocalPlayer()->getDefaultTeam()) == ALLIES ||
            ThePlayerList->getLocalPlayer()->isPlayerObserver()) {
          // tell the user
          UnicodeString s;
          s.format(TheGameText->fetch("GUI:BeaconPlaced"),
          ...
          if (ThePlayerList->getLocalPlayer()->getRelationship(
                  thisPlayer->getDefaultTeam()) == ALLIES)
            TheEva->setShouldPlay(EVA_BeaconDetected);

          TheControlBar
              ->markUIDirty();  // check if we should grey out the button
        } else {
          Int updateCount = 0;
          static NameKeyType nameKeyClientUpdate =
              NAMEKEY("BeaconClientUpdate");
          ClientUpdateModule **clientModules =
              object->getDrawable()->getClientUpdateModules();
          if (clientModules) {
            while (*clientModules) {
          ...
          DEBUG_ASSERTCRASH(
              updateCount == 1,
              ("Saw %d update modules for the beacon!", updateCount));
        }
      } else {
        // tell the user
    ...
    case GameMessage::MSG_REMOVE_BEACON: {
      ...
      thisPlayer->getCurrentSelectionAsAIGroup(
          allSelectedObjects);  // need to act on all objects, so we can hide
                                // teammates' beacons.
        ...
        for (VecObjectID::const_iterator it = selectedObjects.begin();
          ...
          if (beacon) {
            const ThingTemplate *thing =
                TheThingFactory->findTemplate(beacon->getControllingPlayer()
                                                  ->getPlayerTemplate()
                                                  ->getBeaconTemplate());
            if (thing->isEquivalentTo(beacon->getTemplate())) {
              if (beacon->getControllingPlayer() == thisPlayer) {
                TheGameLogic->destroyObject(
                    beacon);  // the owner is telling it to go away.  such is
                              // life.

                TheControlBar->markUIDirty();  // check if we should un-grey out
                                               // the button
                ...
                if (beaconDrawable) {
                  static NameKeyType nameKeyClientUpdate =
                      NAMEKEY("BeaconClientUpdate");
                  ClientUpdateModule **clientModules =
                      beaconDrawable->getClientUpdateModules();
                  if (clientModules) {
                    while (*clientModules) {
                      if ((*clientModules)->getModuleNameKey() ==
                          nameKeyClientUpdate)
                        (*(BeaconClientUpdate **)clientModules)->hideBeacon();
    ...
    case GameMessage::MSG_SELF_DESTRUCT: {
      if (msg->getArgument(0)->boolean) {
        ...
        for (Int i = 0; i < ThePlayerList->getPlayerCount(); ++i) {
          if (i != msg->getPlayerIndex()) {
            ...
            if (thisPlayer->getRelationship(otherPlayer->getDefaultTeam()) ==
                    ALLIES &&
                otherPlayer->getRelationship(thisPlayer->getDefaultTeam()) ==
                    ALLIES) {
              if (TheVictoryConditions->hasSinglePlayerBeenDefeated(
                      otherPlayer))
                continue;

              // a living ally!  hooray!
              otherPlayer->transferAssetsFromThat(thisPlayer);
              thisPlayer->killPlayer();  // just to be safe (and to kill beacons
                                         // etc that don't transfer)
              break;
            }
          }
        }
      ...
      // There is no reason to do any notification here, it now takes place in
      // the victory conditions. bonehead.
      break;
    }
    ...
    case GameMessage::MSG_SET_REPLAY_CAMERA: {
      if (TheRecorder->getMode() == RECORDERMODETYPE_PLAYBACK &&
          TheGlobalData->m_useCameraInReplay &&
          TheControlBar->getObserverLookAtPlayer() == thisPlayer) {
        if (TheTacticalView->isCameraMovementFinished()) {
          ...
          if (!TheLookAtTranslator->hasMouseMovedRecently()) {
            TheMouse->setCursor(
                (Mouse::MouseCursor)(msg->getArgument(4)->integer));
    ...
    case GameMessage::MSG_CREATE_TEAM9: {
      ...
      DEBUG_ASSERTCRASH(player != NULL,
                        ("Could not find player for create team message"));

      if (player == NULL) {
        break;
      }

      player->processCreateTeamGameMessage(
    ...
    case GameMessage::MSG_SELECT_TEAM9: {
      ...
      Player *player = ThePlayerList->getNthPlayer(playerIndex);
      DEBUG_ASSERTCRASH(player != NULL,
                        ("Could not find player for select team message"));

      if (player == NULL) {
        break;
      }

      player->processSelectTeamGameMessage(
          msg->getType() - GameMessage::MSG_SELECT_TEAM0, msg);
      break;
    }
    ...
    case GameMessage::MSG_ADD_TEAM9: {
      ...
      Player *player = ThePlayerList->getNthPlayer(playerIndex);
      DEBUG_ASSERTCRASH(player != NULL,
                        ("Could not find player for add team message"));

      if (player == NULL) {
        break;
      }

      player->processAddTeamGameMessage(
          msg->getType() - GameMessage::MSG_ADD_TEAM0, msg);
      break;
    }
    ...
    case GameMessage::MSG_LOGIC_CRC: {
      if (TheNetwork) {
        ...
        for (Int i = 0; i < MAX_SLOTS; ++i) {
          if (thisPlayer->getPlayerType() == PLAYER_HUMAN &&
              TheNetwork->getPlayerName(i) ==
                  thisPlayer->getPlayerDisplayName()) {
            slotIndex = i;
            break;
          }
        }

        if (slotIndex < 0 || !TheNetwork->isPlayerConnected(slotIndex)) break;
        ...
        m_cachedCRCs[msg->getPlayerIndex()] =
            newCRC;  // to mask problem: = (oldCRC < newCRC)?newCRC:oldCRC;
      } else if (TheRecorder &&
                 TheRecorder->getMode() == RECORDERMODETYPE_PLAYBACK) {
        UnsignedInt newCRC = msg->getArgument(0)->integer;
        // DEBUG_LOG(("Saw CRC of %X from player %d.  Our CRC is %X.  Arg count
        // is %d\n", newCRC, thisPlayer->getPlayerIndex(), getCRC(),
        // msg->getArgumentCount()));

        TheRecorder->handleCRCMessage(newCRC, thisPlayer->getPlayerIndex(),
    ...
    case GameMessage::MSG_PURCHASE_SCIENCE: {
      ...
      // sanity
      if (science == SCIENCE_INVALID || thisPlayer == NULL) break;
  ...
  if (currentlySelectedGroup &&
      TheRecorder->getMode() == RECORDERMODETYPE_PLAYBACK &&
      TheGlobalData->m_useCameraInReplay &&
      TheControlBar->getObserverLookAtPlayer() ==
          thisPlayer /*&& !TheRecorder->isMultiplayer()*/) {
    const VecObjectID &selectedObjects = currentlySelectedGroup->getAllIDs();
    ...
    for (VecObjectID::const_iterator it = selectedObjects.begin();
      ...
      if (obj) {
        Drawable *draw = obj->getDrawable();
        if (draw) TheInGameUI->selectDrawable(draw);
      }
    }
  }
  /**/
