...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
Waypoint::Waypoint(WaypointID id, AsciiString name, const Coord3D *pLoc,
                   AsciiString label1, AsciiString label2, AsciiString label3,
                   Bool biDirectional)
    : m_name(name),
      m_pNext(NULL),
      m_location(*pLoc),
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
Waypoint::~Waypoint() {}  // end ~Waypoint
...
// ------------------------------------------------------------------------------------------------
/** Create a tower object for the bridge of the specified type (and therefore
 * position) */
// ------------------------------------------------------------------------------------------------
Object *Bridge::createTower(Coord3D *worldPos, BridgeTowerType towerType,
  ...
  BridgeBehaviorInterface *bridgeInterface =
      BridgeBehavior::getBridgeBehaviorInterfaceFromObject(bridge);
  DEBUG_ASSERTCRASH(
      bridgeInterface != NULL,
      ("Bridge::createTower - no 'BridgeBehaviorInterface' found\n"));
  if (bridgeInterface) bridgeInterface->setTower(towerType, tower);

  // tie the bridge to us
  BridgeTowerBehaviorInterface *bridgeTowerInterface =
      BridgeTowerBehavior::getBridgeTowerBehaviorInterfaceFromObject(tower);
  DEBUG_ASSERTCRASH(
      bridgeTowerInterface != NULL,
      ("Bridge::createTower - no 'BridgeTowerBehaviorInterface' found\n"));
...
Bridge::Bridge(BridgeInfo &theInfo, Dict *props, AsciiString bridgeTemplateName)
  ...
  // Coord3D fromLeft, fromRight, toLeft, toRight; /// The 4 corners of the
  // rectangle that the bridge covers.
  m_bounds.lo.x = m_bridgeInfo.fromLeft.x;
  m_bounds.lo.y = m_bridgeInfo.fromLeft.y;
  m_bounds.hi = m_bounds.lo;
  if (m_bounds.lo.x > m_bridgeInfo.fromRight.x)
    m_bounds.lo.x = m_bridgeInfo.fromRight.x;
  if (m_bounds.lo.y > m_bridgeInfo.fromRight.y)
    m_bounds.lo.y = m_bridgeInfo.fromRight.y;
  if (m_bounds.hi.x < m_bridgeInfo.fromRight.x)
    m_bounds.hi.x = m_bridgeInfo.fromRight.x;
  if (m_bounds.hi.y < m_bridgeInfo.fromRight.y)
    m_bounds.hi.y = m_bridgeInfo.fromRight.y;
  if (m_bounds.lo.x > m_bridgeInfo.toLeft.x)
    m_bounds.lo.x = m_bridgeInfo.toLeft.x;
  if (m_bounds.lo.y > m_bridgeInfo.toLeft.y)
    m_bounds.lo.y = m_bridgeInfo.toLeft.y;
  if (m_bounds.hi.x < m_bridgeInfo.toLeft.x)
    m_bounds.hi.x = m_bridgeInfo.toLeft.x;
  if (m_bounds.hi.y < m_bridgeInfo.toLeft.y)
    m_bounds.hi.y = m_bridgeInfo.toLeft.y;
  if (m_bounds.lo.x > m_bridgeInfo.toRight.x)
    m_bounds.lo.x = m_bridgeInfo.toRight.x;
  if (m_bounds.lo.y > m_bridgeInfo.toRight.y)
    m_bounds.lo.y = m_bridgeInfo.toRight.y;
  if (m_bounds.hi.x < m_bridgeInfo.toRight.x)
    m_bounds.hi.x = m_bridgeInfo.toRight.x;
  if (m_bounds.hi.y < m_bridgeInfo.toRight.y)
    m_bounds.hi.y = m_bridgeInfo.toRight.y;

  m_bridgeInfo.curDamageState = BODY_PRISTINE;

  static const ThingTemplate *genericBridgeTemplate =
      TheThingFactory->findTemplate("GenericBridge");
  ...
  //
  // we'll say the angle of this object representing the bridge is from the
  ...
  TerrainRoadType *bridgeTemplate =
...
#define no_BRIDGE_TOWERS  // since they aren't destructable, don't need towers.
#if BRIDGE_TOWERS
  // initialize each of the tower positions to that of the bridge info bounding
  // rect
  ...
  for (Int i = 0; i < BRIDGE_MAX_TOWERS; ++i) {
    ...
    towerTemplate =
        TheThingFactory->findTemplate(bridgeTemplate->getTowerObjectName(type));
    if (towerTemplate) {
...
Bridge::Bridge(Object *bridgeObj) {
  ...
  DEBUG_ASSERTLOG(bridgeObj->getGeometryInfo().getGeomType() == GEOMETRY_BOX,
                  ("Bridges need to be rectangles.\n"));
  ...
  m_bridgeInfo.fromLeft.set(pos->x - halfsizeX * c - halfsizeY * s,
                            pos->y + halfsizeY * c - halfsizeX * s, pos->z);
  m_bridgeInfo.toLeft.set(pos->x + halfsizeX * c - halfsizeY * s,
                          pos->y + halfsizeY * c + halfsizeX * s, pos->z);
  m_bridgeInfo.fromRight.set(pos->x - halfsizeX * c + halfsizeY * s,
                             pos->y - halfsizeY * c - halfsizeX * s, pos->z);
  m_bridgeInfo.toRight.set(pos->x + halfsizeX * c + halfsizeY * s,
                           pos->y - halfsizeY * c + halfsizeX * s, pos->z);

  m_bridgeInfo.from.x =
      (m_bridgeInfo.fromLeft.x + m_bridgeInfo.fromRight.x) / 2.0f;
  m_bridgeInfo.from.y =
      (m_bridgeInfo.fromLeft.y + m_bridgeInfo.fromRight.y) / 2.0f;
  m_bridgeInfo.from.z =
      (m_bridgeInfo.fromLeft.z + m_bridgeInfo.fromRight.z) / 2.0f;
...
Bool Bridge::isPointOnBridge(const Coord3D *pLoc) {
  ...
  Vector3 left1(m_bridgeInfo.fromLeft.x, m_bridgeInfo.fromLeft.y,
                m_bridgeInfo.fromLeft.z);
  Vector3 right1(m_bridgeInfo.fromRight.x, m_bridgeInfo.fromRight.y,
                 m_bridgeInfo.fromRight.z);
  Vector3 left2(m_bridgeInfo.toLeft.x, m_bridgeInfo.toLeft.y,
                m_bridgeInfo.toLeft.z);
  Vector3 right2(m_bridgeInfo.toRight.x, m_bridgeInfo.toRight.y,
                 m_bridgeInfo.toRight.z);
...
/*-------------------------------------------------------------------------------------------------
/** Clip a floating point line to the region provided.  The source line runs
from p1 to p2, and is clipped
        * using the clipRegion.
        *
        * Return values:
        *				TRUE  - Line intersects the region
        *				FALSE - Line does not intersect the
region
        */
//-------------------------------------------------------------------------------------------------
Bool LineInRegion(const Coord2D *p1, const Coord2D *p2,
  ...
  if (clipCode1 & clipCode2) return FALSE;
  ...
  // Line is visible
  return (x1 >= clipLeft && x1 <= clipRight && y1 >= clipTop &&
          y1 <= clipBottom && x2 >= clipLeft && x2 <= clipRight &&
          y2 >= clipTop && y2 <= clipBottom);

}  // end LineInRegion

static Bool PointInRegion2D(const Coord3D *pt, const Region2D *clipRegion) {
  return (pt->x >= clipRegion->lo.x && pt->y >= clipRegion->lo.y &&
          pt->x <= clipRegion->hi.x && pt->y <= clipRegion->hi.y);
}
...
//-------------------------------------------------------------------------------------------------
Drawable *Bridge::pickBridge(const Vector3 &from, const Vector3 &to,
                             Vector3 *pos) {
  Vector3 left1(m_bridgeInfo.fromLeft.x, m_bridgeInfo.fromLeft.y,
                m_bridgeInfo.fromLeft.z);
  Vector3 right1(m_bridgeInfo.fromRight.x, m_bridgeInfo.fromRight.y,
...
void Bridge::updateDamageState(void) {
  ...
  if (bridge) {
    ...
    {
      enum BodyDamageType damageState =
          bridge->getBodyModule()->getDamageState();
      ...
      if (damageState != curState) {
        ...
        if (damageState == BODY_RUBBLE) {
          ...
          for (obj = TheGameLogic->getFirstObject(); obj;
            ...
            if (obj->getLayer() == m_layer) {
              ...
              if (TheTerrainLogic->objectInteractsWithBridgeLayer(
                      obj, obj->getLayer(), considerBridgeHealth)) {
                // srj sez: if we use this threshold, then stuff on the bridge
                // apron doesn't die but might sink thru the eyecandy of bridge
                // drbris, looking funny. so now we just indiscriminately kill
                // everything that was on the bridge, regardless of height they
                // might fall.
                // Real deltaHeight = obj->getPosition()->z -
                // TheTerrainLogic->getGroundHeight(obj->getPosition()->x,
                // obj->getPosition()->y); if (deltaHeight>PATHFIND_CELL_SIZE_F
                // * 0.5f)
                {
        ...
        if (curState == BODY_RUBBLE) {
          //
          // we do not set the bridge as usable if scaffolding is up ... the
          // scaffolding code will take care of that
          //
...
/** Init */
//-------------------------------------------------------------------------------------------------
void TerrainLogic::init(void) {}  // end init
...
void TerrainLogic::update(void) {
  ...
  if (m_numWaterToUpdate) {
    ...
    Bool finalTransition, doDamageThisFrame = (TheGameLogic->getFrame() %
                                               LOGICFRAMES_PER_SECOND) == 0;
    ...
    for (Int i = m_numWaterToUpdate - 1; i >= 0; --i) {
      ...
      //
      // check to see if this change per frame will get us to our target height,
      // if so we adjust the changePerFrame to make us be exactly at our target
      // height, and after the change we will remove our entry from this update
      // phase
      //
      ...
      if (finalTransition == TRUE) {
        //
        // make the final water height change, note we do damage on the final
        // transition in all situations by passing a valid damage amount
        //
        setWaterHeight(water, targetHeight, damageAmount, TRUE);

        //
        // remove our water entry from the per frame water list, we're
        // processing this array backwards which makes cleanup easy, we just
        // move everything after our index position up one
        //
        ...
        //
        if (doDamageThisFrame == FALSE) damageAmount = 0.0f;

        //
        // because some water implementation store the height as integers, some
        // changes are too small to keep track of in the actual water data
        // structures so we have to keep track of it outselves
        //
...
void TerrainLogic::newMap(Bool saveGame) {
  ...
  //
  // until we have a real way to specify different water planes in the map, we
  // will check for a special waypoint name that we will put in maps that we
  // want to have a water grid
  /// @todo Mark W, remove this when you have water plane placements in the map
  /// done (Colin)
  //
  Waypoint *waypoint = getWaypointByName("WaveGuide1");
  Bool enable = FALSE;
  if (waypoint) enable = TRUE;
...
void TerrainLogic::enableWaterGrid(Bool enable) {
  ...
  //
  // set the vertex animated water properties, that is, the clamps, the water
  // position, the grid resolution etc ...
  //
  if (enable == TRUE) {
    /** @todo we should have this stuff stored with the map and have a real
    interface for design to edit such things so that people can put gridded
    water in any map without all this hard coded nasty stuff, but this is what
    "they" want for now */

    Int waterSettingIndex = -1;
    for (Int i = 0; i < GlobalData::MAX_WATER_GRID_SETTINGS; i++) {
      if (TheGlobalData->m_mapName.compareNoCase(
              TheGlobalData->m_vertexWaterAvailableMaps[i].str()) == 0) {
        waterSettingIndex = i;
        break;  // exit for i
      ...
      //
      // no exact map name (including path) was found, try to look for a match
      // in just the mapname.map without any path information.  This is
      // necessary for save/load due to the fact that the map
      // Data\CHI01\CHI01.map will turn into Save\CHI01.map when loading the map
      ...
      if (c)
        ...
        strippedCompareMapNameOnly =
            TheGlobalData->m_vertexWaterAvailableMaps[i];

      // now try this compare
      if (strippedMapNameOnly.compareNoCase(strippedCompareMapNameOnly.str()) ==
          0) {
        waterSettingIndex = i;
        break;  // exit for i
    ...
    if (waterSettingIndex == -1) {
      DEBUG_CRASH(
          ("!!!!!! Deformable water won't work because there was no group of "
           "vertex water data defined in GameData.INI for this map name '%s' "
           "!!!!!! (C. Day)\n",
    ...
    TheTerrainVisual->setWaterGridHeightClamps(
        NULL, TheGlobalData->m_vertexWaterHeightClampLow[waterSettingIndex],
        TheGlobalData->m_vertexWaterHeightClampHi[waterSettingIndex]);
    TheTerrainVisual->setWaterTransform(
        NULL, TheGlobalData->m_vertexWaterAngle[waterSettingIndex],
    ...
    TheTerrainVisual->setWaterGridResolution(
        NULL, TheGlobalData->m_vertexWaterXGridCells[waterSettingIndex],
    ...
    TheTerrainVisual->setWaterAttenuationFactors(
        NULL, TheGlobalData->m_vertexWaterAttenuationA[waterSettingIndex],
...
//-------------------------------------------------------------------------------------------------
/** device independent terrain logic load.  If query is true, we are just
loading it to get look at some data rather than running a game, so don't pass
this load to the client. */
//-------------------------------------------------------------------------------------------------
Bool TerrainLogic::loadMap(AsciiString filename, Bool query) {
  ...
  if (theInputStream.open(AsciiString(m_filenameString.str()))) try {
      ChunkInputStream *pStrm = &theInputStream;
      pStrm->absoluteSeek(0);
      DataChunkInput file(pStrm);
      if (file.isValidFileType()) {  // Backwards compatible files aren't valid
                                     // data chunk files.
        // Read the waypoints.
        file.registerParser(AsciiString("WaypointsList"),
      ...
      // Eat the error - legacy files are not valid chunk format (and don't have
      // waypoint info.)
...
/** Reads in the waypoint chunk */
//-------------------------------------------------------------------------------------------------
Bool TerrainLogic::parseWaypointData(DataChunkInput &file, DataChunkInfo *info,
                                     void *userData) {
...
void TerrainLogic::addWaypoint(MapObject *pMapObj) {
  ...
  label1 = pMapObj->getProperties()->getAsciiString(TheKey_waypointPathLabel1,
                                                    &exists);
  label2 = pMapObj->getProperties()->getAsciiString(TheKey_waypointPathLabel2,
                                                    &exists);
  label3 = pMapObj->getProperties()->getAsciiString(TheKey_waypointPathLabel3,
                                                    &exists);
  Bool biDirectional;
  biDirectional = pMapObj->getProperties()->getBool(
      TheKey_waypointPathBiDirectional, &exists);
  DEBUG_ASSERTCRASH(pMapObj->isWaypoint(), ("not a waypoint"));
  Waypoint *pWay = newInstance(Waypoint)(pMapObj->getWaypointID(),
                                         pMapObj->getWaypointName(), &loc,
...
/** default get height for terrain logic */
//-------------------------------------------------------------------------------------------------
Real TerrainLogic::getLayerHeight(Real x, Real y, PathfindLayerEnum layer,
...
//-------------------------------------------------------------------------------------------------
void makeAlignToNormalMatrix(Real angle, const Coord3D &pos,
                             const Coord3D &normal, Matrix3D &mtx) {
  Coord3D x, y, z;
  ...
  */
  ...
  DEBUG_ASSERTCRASH(
      fabs(x.x * z.x + x.y * z.y + x.z * z.z) < 0.0001,
...
//-------------------------------------------------------------------------------------------------
PathfindLayerEnum TerrainLogic::alignOnTerrain(Real angle, const Coord3D &pos,
                                               Bool stickToGround,
                                               Matrix3D &mtx) {
  ...
  if (stickToGround) mtx.Set_Z_Translation(terrainAtPos);
...
//-------------------------------------------------------------------------------------------------
void TerrainLogic::addBridgeToLogic(BridgeInfo *pInfo, Dict *props,
...
Waypoint *TerrainLogic::getWaypointByName(AsciiString name) {
  for (Waypoint *way = m_waypointListHead; way; way = way->getNext())
    if (way->getName() == name) return way;
...
Waypoint *TerrainLogic::getClosestWaypointOnPath(const Coord3D *pos,
  ...
  for (Waypoint *way = m_waypointListHead; way; way = way->getNext()) {
    ...
    if (match) {
      ...
      Real newDistSqr = (curPos.x - pos->x) * (curPos.x - pos->x) +
...
Bridge *TerrainLogic::findBridgeLayerAt(const Coord3D *pLoc,
  ...
  while (pBridge) {
    if (pBridge->getLayer() == layer &&
        (!clip || pBridge->isPointOnBridge(pLoc))) {
      return (pBridge);
    }
    pBridge = pBridge->getNext();
  }
...
//-------------------------------------------------------------------------------------------------
// this is just like getLayerForDestination, but always return the highest layer
// that will be <= z at that point (unlike getLayerForDestination, which will
// return the closest layer)
PathfindLayerEnum TerrainLogic::getHighestLayerForDestination(
    const Coord3D *pos, Bool onlyHealthyBridges) {
  PathfindLayerEnum bestLayer = LAYER_GROUND;
  Real bestDistance =
      pos->z - getGroundHeight(pos->x, pos->y);  // NOT fabs in this case.
  ...
  for (Bridge *pBridge = getFirstBridge(); pBridge != NULL;
       pBridge = pBridge->getNext()) {
    if (onlyHealthyBridges &&
        pBridge->peekBridgeInfo()->curDamageState == BODY_RUBBLE)
      continue;
 ...
 */
//-------------------------------------------------------------------------------------------------
Bool TerrainLogic::objectInteractsWithBridgeLayer(
    Object *obj, Int layer, Bool considerBridgeHealth) const {
...
//-------------------------------------------------------------------------------------------------
Drawable *TerrainLogic::pickBridge(const Vector3 &from, const Vector3 &to,
                                   Vector3 *pos) {
...
void TerrainLogic::deleteBridge(Bridge *bridge) {
  ...
  if (m_bridgeListHead == bridge) {
    ...
    for (Bridge *otherBridge = getFirstBridge(); otherBridge;
      ...
      //
      // if the next bridge is the one in question to delete, set this bridge to
      // point to the next pointer of the bridge we are deleting
      //
  ...
  if (bridgeObj) TheGameLogic->destroyObject(bridgeObj);
...
//-------------------------------------------------------------------------------------------------
/** Returns the ground aligned point on the bounding box closest to the given
 * point*/
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
/** Returns the ground aligned point on the bounding box farthest from the given
 * point*/
//-------------------------------------------------------------------------------------------------
// Lorenzen was here
...
Bool TerrainLogic::isUnderwater(Real x, Real y, Real *waterZ, Real *terrainZ) {
  ...
  if (waterHandle == NULL) return FALSE;

  //
  // if this water handle is a grid water use the grid height function,
  // otherwise look into the polygon trigger
  //
  Real wZ = 0.0f;
  ...
  if (terrainZ) *terrainZ = terrainHeight;
...
const WaterHandle *TerrainLogic::getWaterHandle(Real x, Real y) {
  ...
  if (TheTerrainVisual->getWaterGridHeight(x, y, &meshZ)) {
    //
    // point falls on water grid, return the special handle for the grid water,
    // since we only have one of them and don't yet support multiple gridded
    // water sections
    //
    if (meshZ >= waterZ) {
      waterZ = meshZ;
...
const WaterHandle *TerrainLogic::getWaterHandleByName(AsciiString name) {
  if (name.compare(WATER_GRID) == 0) return &TerrainLogic::m_gridWaterHandle;
...
Real TerrainLogic::getWaterHeight(const WaterHandle *water) {
  ...
  //
  // when querying the water height given a handle, we cannot query gridded
  // water in this way because it's variable across the whole surface of the
  // water
  //
  if (water == &m_gridWaterHandle) {
    DEBUG_CRASH(
        ("TerrainLogic::getWaterHeight( WaterHandle *water ) - water is a grid "
         "handle, cannot make this query\n"));
    return 0.0f;
  ...
  // sanity
  DEBUG_ASSERTCRASH(
      water->m_polygon != NULL,
      ("getWaterHeight: polygon trigger in water handle is NULL\n"));
...
// ------------------------------------------------------------------------------------------------
/** Set the water height.  If the water rises, then any objects that now find
 * themselves underwater will be damaged by the amount provided in the parameter
 * 'damageAmount' */
// ------------------------------------------------------------------------------------------------
void TerrainLogic::setWaterHeight(const WaterHandle *water, Real height,
                                  Real damageAmount, Bool forcePathfindUpdate) {
  // sanity
  if (water == NULL) return;

  //
  // if this is a handle to gridded water simple change the transform to
  // raise/lower the whole water table.  Note: The other meaning this *could*
  // have if we want it to is to leave the water table transform where it is and
  // actually change the height of the water at every grid point
  //
  ...
  //
  // if the water height has risen, we need apply water damage to things that
  ...
  if (damageAmount > 0.0f && height > previousHeight) {
    ...
    ObjectIterator *iter = ThePartitionManager->iterateObjectsInRange(
        &center, maxDist, FROM_CENTER_2D, NULL);
...
void TerrainLogic::changeWaterHeightOverTime(const WaterHandle *water,
  ...
  for (Int i = 0; i < m_numWaterToUpdate; i++) {
    if (m_waterToUpdate[i].waterTable == water) {
      ...
      //
      // process this index over again just to be complete, but we should never
      // find "another" duplicate water entry
      //
      --i;
  ...
  m_waterToUpdate[m_numWaterToUpdate].changePerFrame =
...
void TerrainLogic::findAxisAlignedBoundingRect(const WaterHandle *water,
                                               Region3D *region) {
  // sanity
  if (water == NULL || region == NULL) return;

// setup the lo and high of the region to the *opposite* side of the map plus
// some big number
#define BUFFER 99999.9f  /// just to have extreme regions outside of the map
  ...
  if (water == &m_gridWaterHandle) {
    ...
    // compute the 4 corners of the table according to the grids and grid
    // spacing
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (v.X < region->lo.x) region->lo.x = v.X;
      if (v.X > region->hi.x) region->hi.x = v.X;
      if (v.Y < region->lo.y) region->lo.y = v.Y;
      if (v.Y > region->hi.y) region->hi.y = v.Y;
    ...
    for (Int i = 0; i < numPoints; i++) {
      ...
      if (p->x < region->lo.x) region->lo.x = p->x;
      if (p->x > region->hi.x) region->hi.x = p->x;

      if (p->y < region->lo.y) region->lo.y = p->y;
      if (p->y > region->hi.y) region->hi.y = p->y;

      if (p->z < region->lo.z) region->lo.z = p->z;
      if (p->z > region->hi.z) region->hi.z = p->z;
...
void TerrainLogic::setActiveBoundary(Int newActiveBoundary) {
  ...
  if (newActiveBoundary == m_activeBoundary) {
    // since this is fairly expensive (causes reset of PartitionManager as well
    // as pathfinding), we should probably return
    return;
  }
  ...
  // Can't have any lingering looks persist over the resize, so flush the queue
  // now
  ...
  // Don't do a newMap on the pathfinder - It uses the largest active boundary
  // to start.  jba.
...
void TerrainLogic::flattenTerrain(Object *obj) {
  ...
  switch (obj->getGeometryInfo().getGeomType()) {
    case GEOMETRY_BOX: {
      ...
      Vector3 topLeft(pos->x - halfsizeX * c - halfsizeY * s,
                      pos->y + halfsizeY * c - halfsizeX * s, 0);
      Vector3 topRight(pos->x + halfsizeX * c - halfsizeY * s,
                       pos->y + halfsizeY * c + halfsizeX * s, 0);
      Vector3 bottomRight(pos->x + halfsizeX * c + halfsizeY * s,
                          pos->y - halfsizeY * c + halfsizeX * s, 0);
      Vector3 bottomLeft(pos->x - halfsizeX * c + halfsizeY * s,
                         pos->y - halfsizeY * c - halfsizeX * s, 0);
      ...
      for (i = iMin.x; i <= iMax.x; i++) {
        for (j = 0; j <= iMax.y; j++) {
          ...
          Bool match = false;
          unsigned char flags;
          if (Point_In_Triangle_2D(topLeft, topRight, bottomLeft, testPt, 0, 1,
                                   flags)) {
            match = true;
          }
          if (Point_In_Triangle_2D(topRight, bottomRight, bottomLeft, testPt, 0,
                                   1, flags)) {
            match = true;
          }
          if (match) {
      ...
      Int rawDataHeight =
          REAL_TO_INT_FLOOR(0.5f + avgHeight / MAP_HEIGHT_SCALE);

      // Compare to the height at the building's origin, because setRawMapHeight
      // will only lower, not raise.  jba
      Int centerHeight = REAL_TO_INT_FLOOR(
          TheTerrainLogic->getGroundHeight(pos->x, pos->y) / MAP_HEIGHT_SCALE);
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void TerrainLogic::crc(Xfer *xfer) {}  // end crc
...
void TerrainLogic::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_LOAD) setActiveBoundary(activeBoundary);
  ...
  if (version >= 2) {
    ...
    for (UnsignedInt i = 0; i < m_numWaterToUpdate; ++i) {
      ...
      if (xfer->getXferMode() == XFER_SAVE) {
        ...
        if (poly == NULL) {
          DEBUG_CRASH(
              ("TerrainLogic::xfer - Unable to find polygon trigger for water "
               "table with trigger ID '%d'\n",
               triggerID));
        ...
        if (m_waterToUpdate[i].waterTable == NULL) {
          DEBUG_CRASH(
              ("TerrainLogic::xfer - Polygon trigger to use for water handle "
               "has no water handle!\n"));
          throw SC_INVALID_DATA;
...
void TerrainLogic::loadPostProcess(void) {
  ...
  while (pBridge) {
    pNext = pBridge->getNext();
    Object *obj =
        TheGameLogic->findObjectByID(pBridge->peekBridgeInfo()->bridgeObjectID);
    if (obj == NULL) {
