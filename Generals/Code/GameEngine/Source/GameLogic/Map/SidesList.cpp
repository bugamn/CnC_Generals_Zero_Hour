...
//
// Desc:      Contains the information describing Sides (player, ai, neutral
// etc.)
//						in a scenario, including build
//lists for non-player sides.
//
//-----------------------------------------------------------------------------
...
*/
SidesInfo::SidesInfo(void) : m_pBuildList(NULL), m_scripts(NULL) {}

SidesInfo::SidesInfo(const SidesInfo &thatref)
    : m_pBuildList(NULL), m_scripts(NULL) {
  *this = thatref;
}
...
*/
SidesInfo::~SidesInfo(void) { clear(); }

void SidesInfo::init(const Dict *d) {
  m_pBuildList->deleteInstance();
  m_pBuildList = NULL;
  m_dict.clear();
  if (m_scripts) m_scripts->deleteInstance();
  m_scripts = NULL;
  if (d) m_dict = *d;
}

// ug, I hate having to overload stuff, but this makes it a lot easier to make
// copies safely
SidesInfo &SidesInfo::operator=(const SidesInfo &that) {
  if (this != &that) {
    ...
    for (BuildListInfo *thatBL = that.m_pBuildList; thatBL;
         thatBL = thatBL->getNext()) {
...
void SidesInfo::addToBuildList(BuildListInfo *pBuildList, Int position) {
  DEBUG_ASSERTLOG(pBuildList->getNext() == NULL,
                  ("WARNING***Adding already linked element."));
...
/**
 SidesList - reset -
*/
void SidesList::reset(void) { clear(); }
...
Bool SidesList::ParseSidesDataChunk(DataChunkInput &file, DataChunkInfo *info,
  ...
  if (TheSidesList == NULL) return false;
  ...
  file.registerParser(AsciiString("PlayerScriptsList"), info->label,
...
void SidesList::WriteSidesDataChunk(DataChunkOutput &chunkWriter) {
  ...
  ScriptList::WriteScriptsDataChunk(chunkWriter, scripts,
                                    TheSidesList->getNumSides());
...
SidesInfo *SidesList::findSkirmishSideInfo(AsciiString name,
  ...
  for (int i = 0; i < m_numSkirmishSides; i++) {
    if (m_skirmishSides[i].getDict()->getAsciiString(TheKey_playerName) ==
        name) {
      if (index) *index = i;
 ...
 *
 */
static Bool ParsePlayersDataChunk(DataChunkInput &file, DataChunkInfo *info,
                                  void *userData) {
 ...
 *
 */
static Bool ParseTeamsDataChunk(DataChunkInput &file, DataChunkInfo *info,
                                void *userData) {
    ...
    if (theInputStream.open(path)) {
      ...
      for (i = 0; i < count; i++) {
        ...
        for (j = 0; j < m_numSkirmishSides; j++) {
          AsciiString name = getSkirmishSideInfo(j)->getDict()->getAsciiString(
              TheKey_playerName);
...
Bool SidesList::isPlayerDefaultTeam(TeamsInfo *t) {
  // if our name is "teamfoo" and there is a player named "foo", we are a
  // player-default team.
  ...
  if (tname.startsWith("team")) {
    ...
    for (int j = 0; j < m_numSides; j++) {
      AsciiString pname =
          m_sides[j].getDict()->getAsciiString(TheKey_playerName);
...
void SidesList::addSide(const Dict *d) {
  ...
  if (m_numSides < MAX_PLAYER_COUNT) m_sides[m_numSides++].init(d);
}

void SidesList::addTeam(const Dict *d) { m_teamrec.addTeam(d); }

void SidesList::addSkirmishTeam(const Dict *d) { m_skirmishTeamrec.addTeam(d); }

void SidesList::removeSide(Int i) {
  if (i < 0 || i >= m_numSides || m_numSides <= 1) return;

  for (; i < m_numSides - 1; i++) m_sides[i] = m_sides[i + 1];

  for (; i < MAX_PLAYER_COUNT; i++) m_sides[i].clear();

  --m_numSides;
}

void SidesList::removeTeam(Int i) { m_teamrec.removeTeam(i); }
...
Bool SidesList::validateAllyEnemyList(const AsciiString &tname,
  ...
  while (str.nextToken(&token)) {
    ...
    if (!newstr.isEmpty()) newstr.concat(" ");
    newstr.concat(token);
  }
...
void SidesList::addPlayerByTemplate(AsciiString playerTemplateName) {
  ...
  if (playerTemplateName.isEmpty()) {
    ...
    if (playerName == "PlyrCivilian") isHuman = false;
  }
  ...
  for (i = 0; i < getNumSides(); i++) {
    ...
    if (ti) {
      ...
      if (ti->getDict()->getAsciiString(TheKey_teamOwner) != pname) {
        DEBUG_CRASH(
            ("hmm, team owner mismatch (%s) (%s), this should not normally be "
             "possible",
             ti->getDict()->getAsciiString(TheKey_teamOwner).str(),
             pname.str()));
      ...
      DEBUG_LOG(
          ("*** default team for player %s missing (should not be possible), "
           "adding it...\n",
           tname.str()));
      Dict d;
    ...
    // (note that owners can be teams or players, but allies/enemies can only be
    // teams.)
...
validate_team_names:
  for (i = 0; i < getNumTeams(); i++) {
    ...
    if (findSideInfo(tname)) {
      DEBUG_CRASH(
          ("name %s is duplicate between player and team, removing...\n",
           tname.str()));
      removeTeam(i);
      modified = true;
  ...
  for (i = 0; i < getNumTeams(); i++) {
    ...
    //		if (tdict->getType(NAMEKEY(AsciiString("teamAllies"))) !=
    //Dict::DICT_NONE) 			tdict->remove(NAMEKEY(AsciiString("teamAllies"))); 		if
    //(tdict->getType(NAMEKEY(AsciiString("teamEnemies"))) != Dict::DICT_NONE)
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void SidesList::crc(Xfer *xfer) {}  // end crc
...
void SidesList::xfer(Xfer *xfer) {
  ...
  if (sideCount != getNumSides()) {
    DEBUG_CRASH(
        ("SidesList::xfer - The sides list size has changed, this was not "
         "supposed to happen, you must version this method and figure out how "
         "to translate between old and new versions now\n"));
  ...
  for (Int i = 0; i < sideCount; ++i) {
    ...
    if (scriptListPresent) xfer->xferSnapshot(scriptList);
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void SidesList::loadPostProcess(void) {}  // end loadPostProcess
...
void BuildListInfo::parseStructure(INI *ini, void *instance, void * /*store*/,
                                   const void * /*userData*/) {
  ...
  static const FieldParse myFieldParse[] = {
      {"Name", INI::parseAsciiString, NULL,
       offsetof(BuildListInfo, m_buildingName)},
      {"Location", INI::parseCoord2D, NULL,
       offsetof(BuildListInfo, m_location)},
      ...
      {"InitiallyBuilt", INI::parseBool, NULL,
       offsetof(BuildListInfo, m_isInitiallyBuilt)},
      {"RallyPointOffset", INI::parseCoord2D, NULL,
       offsetof(BuildListInfo, m_rallyPointOffset)},
      {"AutomaticallyBuild", INI::parseBool, NULL,
       offsetof(BuildListInfo, m_automaticallyBuild)},
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void BuildListInfo::crc(Xfer *xfer) {}  // end crc
...
void BuildListInfo::xfer(Xfer *xfer) {
  ...
  xfer->xferUser(m_resourceGatherers,
                 sizeof(ObjectID) * MAX_RESOURCE_GATHERERS);
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void BuildListInfo::loadPostProcess(void) {}  // end loadPostProcess
...
TeamsInfoRec::~TeamsInfoRec() { clear(); }
...
void TeamsInfoRec::clear() {
  ...
  for (i = 0; i < m_numTeamsAllocated; i++) m_teams[i].clear();
...
void TeamsInfoRec::addTeam(const Dict *d) {
  ...
  DEBUG_ASSERTCRASH(m_numTeams < 1024,
                    ("hmm, seems like an awful lot of teams..."));
  if (m_numTeams >= m_numTeamsAllocated) {
    // pool[]ify
    TeamsInfo *nti = NEW
        TeamsInfo[m_numTeamsAllocated + TEAM_ALLOC_CHUNK];  // throws on failure

    Int i;

    for (i = 0; i < m_numTeams; i++) nti[i] = m_teams[i];

    for (; i < m_numTeamsAllocated + TEAM_ALLOC_CHUNK; i++) nti[i].clear();
...
void TeamsInfoRec::removeTeam(Int i) {
  if (i < 0 || i >= m_numTeams || m_numTeams <= 1) return;

  for (; i < m_numTeams - 1; i++) m_teams[i] = m_teams[i + 1];

  for (; i < m_numTeamsAllocated; i++) m_teams[i].clear();
