...
/**
 ...
 NOTE: It is expected that this will only get called in the editor, as in the
 game the poly triggers don't change.
*/
...
PolygonTrigger *PolygonTrigger::getPolygonTriggerByID(Int triggerID) {
  for (PolygonTrigger *poly = PolygonTrigger::getFirstPolygonTrigger(); poly;
       poly = poly->getNext())
    if (poly->getID() == triggerID) return poly;
...
/**
 * PolygonTrigger::ParsePolygonTriggersDataChunk - read a polygon triggers
 *chunk. Format is the newer CHUNKY format. See
 *PolygonTrigger::WritePolygonTriggersDataChunk for the writer. Input:
 *DataChunkInput
 *
 */
...
/**
 * PolygonTrigger::WritePolygonTriggersDataChunk - Writes a Polygon triggers
 *chunk. Format is the newer CHUNKY format. See
 *PolygonTrigger::ParsePolygonTriggersDataChunk for the reader. Input:
 *DataChunkInput
 *
 */
void PolygonTrigger::WritePolygonTriggersDataChunk(
    DataChunkOutput &chunkWriter) {
...
void PolygonTrigger::addPolygonTrigger(PolygonTrigger *pTrigger) {
  for (PolygonTrigger *pTrig = getFirstPolygonTrigger(); pTrig;
       pTrig = pTrig->getNext()) {
    DEBUG_ASSERTCRASH(pTrig != pTrigger,
                      ("Attempting to add trigger already in list."));
...
void PolygonTrigger::removePolygonTrigger(PolygonTrigger *pTrigger) {
  ...
  if (pTrig) {
    if (pPrev) {
      DEBUG_ASSERTCRASH(pTrigger == pPrev->m_nextPolygonTrigger,
                        ("Logic errror.  jba."));
      pPrev->m_nextPolygonTrigger = pTrig->m_nextPolygonTrigger;
    } else {
      DEBUG_ASSERTCRASH(pTrigger == ThePolygonTriggerListPtr,
                        ("Logic errror.  jba."));
...
void PolygonTrigger::getCenterPoint(Coord3D *pOutCoord) const {
  DEBUG_ASSERTCRASH(pOutCoord != NULL,
                    ("pOutCoord was null. Non-Fatal, but shouldn't happen."));
  if (!pOutCoord) {
    return;
  }
...
const WaterHandle *PolygonTrigger::getWaterHandle(void) const {
  if (isWaterArea()) return &m_waterHandle;
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void PolygonTrigger::crc(Xfer *xfer) {}  // end crc
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void PolygonTrigger::loadPostProcess(void) {}  // end loadPostProcess
