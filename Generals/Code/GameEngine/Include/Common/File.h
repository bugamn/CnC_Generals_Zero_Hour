 ...
 * absolutely necessary. Also FS::Open should be used to create File objects and
 *open files.
 */
//===============================
...
class File : public MemoryPoolObject {
 ...
 public:
  enum access {
    ...
    STREAMING = 0x00000100  ///< Do not read this file into a ram file, read it
                            ///< as requested.
  };
 ...
 public:
  ...
  virtual Bool open(const Char *filename,
                    Int access = 0);  ///< Open a file for access
  virtual void close(void);  ///< Close the file !!! File object no longer valid
                             ///< after this call !!!

  virtual Int read(
      void *buffer,
      Int bytes) = 0; /**< Read the specified number of bytes from the file
                          * in to the memory pointed at by buffer. Returns the
                          * number of bytes read. Returns -1 if an error
                          * occured.
                          */
  virtual Int write(
      const void *buffer,
      Int bytes) = 0; /**< Write the specified number of bytes from the
                          *	 memory pointed at by buffer to the file.
                          *Returns the number of bytes written. Returns -1 if an
                          *error occured.
                          */
  virtual Int seek(
      Int bytes,
      seekMode mode =
          CURRENT) = 0; /**< Sets the file position of the next read/write
                            * operation. Returns the new file position as the
                            * number of bytes from the start of the file.
                            *  Returns -1 if an error occured.
                            *
                            *  seekMode determines how the seek is done:
                            *
                            *  START : means seek to the specified number of
                            * bytes from the start of the file CURRENT: means
                            * seek the specified the number of bytes from the
                            * current file position END: means seek the
                            * specified number of bytes back from the end of the
                            * file
                            */
  virtual void nextLine(
      Char *buf = NULL,
      Int bufSize = 0) = 0;  ///< reads until it reaches a new-line character

  virtual Bool scanInt(
      Int &newInt) = 0;  ///< read an integer from the current file position.
  virtual Bool scanReal(Real &newReal) = 0;  ///< read a real number from the
                                             ///< current file position.
  virtual Bool scanString(
      AsciiString
          &newString) = 0;  ///< read a string from the current file position.
  ...
  const char *getName(
      void) const;            ///< Returns a pointer to the name of the file
  Int getAccess(void) const;  ///< Returns file's access flags

  void deleteOnClose(
      void);  ///< Causes the File object to delete itself when it closes
