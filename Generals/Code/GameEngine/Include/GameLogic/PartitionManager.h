...
//						 objects in space, iterate
//objects in specified volumes, 						 regions, by types and other properties.
//
//-----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------------------------
enum ValueOrThreat { VOT_CashValue = 1, VOT_ThreatValue, VOT_NumItems };
...
enum FindPositionFlags {
  ...
  FPF_IGNORE_ALL_OBJECTS =
      0x00000004,  // ignore all objects, positions inside objects are ok
  FPF_IGNORE_ALLY_OR_NEUTRAL_UNITS =
      0x00000008,  // ignore friendly units (requires relationshipObject)
  FPF_IGNORE_ALLY_OR_NEUTRAL_STRUCTURES =
      0x00000010,  // ignore friendly structures (requires relationshipObject)
  FPF_IGNORE_ENEMY_UNITS =
      0x00000020,  // ignore enemy units (requires relationshipObject)
  FPF_IGNORE_ENEMY_STRUCTURES =
      0x00000040,  // ignore enemy structures (requires relationshipObject)
  FPF_USE_HIGHEST_LAYER = 0x00000080,  // examine pos on highest layer at given
                                       // xy (rather than on ground layer)
};
// ----------------------------------------------------------------------------------------------

const Real RANDOM_START_ANGLE =
    -99999.9f;  ///< no start angle (an unlikely number to use for the start
                ///< angle)
...
struct FindPositionOptions {
  ...
  const Object *sourceToPathToDest;  ///< object that must be able to path to
                                     ///< the position chosen
...
//=====================================
/**
        a Plain Old Data structure that is used to get optional results from
   collidesWith().
*/
...
/**
        ...
        It stores pairs of potentially-colliding objects (eliminating
   duplicates) for processing after all partitions are updated.
*/
//=====================================
...
class CellAndObjectIntersection  // not MPO: we allocate these in arrays
{
 private:
  ...
  CellAndObjectIntersection *m_prevCoi,
      *m_nextCoi;  ///< if in use, next/prev in this cell. if not in use,
                   ///< next/prev free in this module.

 public:
  // Note, we allocate these in arrays, thus we must have a default ctor (and
  // NOT descend from MPO)
  ...
  */
  ...
  /**
          make 'this' refer to nothing at all. this involves resetting the
     member variables to null, and removing 'this' from the Cell's list of
     COI's.
  */
...
/**
        This class encapsulates one area interaction with PartitionCells.  The
...
class SightingInfo : public MemoryPoolObject, public Snapshot {
 ...
 public:
  ...
  Real m_howFar;
  PlayerMaskType
      m_forWhom;  // ask not for whom the sighting is masked; it masks for thee

  UnsignedInt m_data;  // Threat and value use as the value.  Sighting uses it
                       // for a Timestamp
...
//=====================================
/**
        We sometimes need to save whether or not an area was fogged or
   permanently revealed through a script. This helps us do so.
*/
//=====================================
enum { STORE_DONTTOUCH = 0, STORE_FOG = 1, STORE_PERMANENTLY_REVEALED = 2 };

struct ShroudStatusStoreRestore {
  std::vector<UnsignedByte> m_foggedOrRevealed[MAX_PLAYER_COUNT];
  Int m_cellsWide;  // m_cellsHigh is computed by m_foggedOrRevealed[0].size() /
                    // m_cellsWide
};
...
class PartitionCell : public Snapshot  // not MPO: allocated in an array
{
 private:
  CellAndObjectIntersection
      *m_firstCoiInCell;  ///< list of COIs in this cell (may be null).
  ...
  Short m_cellX;  ///< x-coord of this cell within the Partition Mgr coords (NOT
                  ///< in world coords)
  Short m_cellY;  ///< y-coord of this cell within the Partition Mgr coords (NOT
                  ///< in world coords)
 ...
 public:
  ...
  inline CellAndObjectIntersection *getFirstCoiInCell() {
    return m_firstCoiInCell;
  }
...
class PartitionData : public MemoryPoolObject {
 ...
 private:
  ...
  GhostObject
      *m_ghostObject;  ///< Temporary object used when real object is gone but
                       ///< player thinks it's there because of fog.
  ...
  Int m_coiArrayCount;  ///< number of COIs allocated (may be more than are in
                        ///< use)
  ...
  ObjectShroudStatus
      m_shroudednessPrevious[MAX_PLAYER_COUNT];  ///< previous frames value of
                                                 ///< m_shroudedness
  Bool m_everSeenByPlayer[MAX_PLAYER_COUNT];  ///< whether this object has ever
                                              ///< been seen by a given player.
  const PartitionCell
      *m_lastCell;  ///< The last cell I thought my center was in.

  /**
          Given a shape's geometry and size parameters, calculate the maximum
     number of COIs that the object could possibly occupy.
  */
  Int calcMaxCoiForShape(GeometryType geom, Real majorRadius, Real minorRadius,
                         Bool isSmall);

  /**
          Given an object's geometry and size parameters, calculate the maximum
     number of COIs that the object could possibly occupy. (This simply extracts
     the information from the object and calls calcMaxCoiForShape.)
  */
  ...
  */
  ...
  /**
                  marks self as touching no cells. (and any previous cells as
     not touching self!)
  */
  void removeAllTouchedCells();

  /**
          this discards all current 'touch' information (via
     removeAllTouchedCells) and recalculates the cells touched by this module,
     based on the object's geometry. this will be called frequently and so needs
     to be as efficient as possible.
  */
  void updateCellsTouched();

  /**
          If you imagine the array of Partition Cells as pixels, then this
     method 'sets' the pixel [cell] at cell coordinate (x, y).
  */
  ...
  */
  ...
  /**
          ...
          center and radius. Note that this is used for both spheres and
     cylinders.
  */
  void doCircleFill(Real centerX, Real centerY, Real radius);

  /**
          fill in the pixels covered by the given rectangular shape with the
     given center, dimensions, and rotation.
  */
  void doRectFill(Real centerX, Real centerY, Real halfsizeX, Real halfsizeY,
                  Real angle);
  ...
  /**
          ...
          filled in to the normal to the surface of 'this' at the collide
     location.
  */
  Bool collidesWith(const PartitionData *that,
 ...
 public:
  ...
  /// mark the given module as being "dirty", needing recalcing during next
  /// update phase.
  // if needToUpdateCells is true, we'll recalc the partition cells it touches
  // and do collision testing. if needToUpdateCells is false, we'll just do the
  // collision testing.
  ...
  inline Int wasSeenByAnyPlayers()
      const  ///< check if a player in the game has seen the object but is now
             ///< looking at fogged version.
  {
    Int i = 0;
  ...
  */
  ...
  void friend_setShroudednessPrevious(
      Int playerIndex,
      ObjectShroudStatus status);  ///< only used to restore state after map
                                   ///< border resizing and/or xfer!
  ObjectShroudStatus friend_getShroudednessPrevious(Int playerIndex) {
    return m_shroudednessPrevious[playerIndex];
  }
  ...
  Int friend_getCoiInUseCount() {
    return m_coiInUseCount;
  }  ///< this is only for use by PartitionManager
  Bool friend_collidesWith(const PartitionData *that,
                           CollideLocAndNormal *cinfo) const {
  ...
  inline Bool isInListDirtyModules(PartitionData *const *pListHead) const {
    ...
    DEBUG_ASSERTCRASH(result == (m_dirtyStatus != NOT_DIRTY),
                      ("dirty flag mismatch"));
    return result;
  }
  inline void prependToDirtyModules(PartitionData **pListHead) {
    ...
    if (*pListHead) (*pListHead)->m_prevDirty = this;
    *pListHead = this;
  }
  void removeFromDirtyModules(PartitionData **pListHead) {
    m_dirtyStatus = NOT_DIRTY;
    if (m_nextDirty) m_nextDirty->m_prevDirty = m_prevDirty;
    if (m_prevDirty)
...
//=====================================
/**
        this is an ABC. PartitionData::iterate allows you to pass multiple
   filters to filter out objects you don't want. it calls this method on the
   objects in question; any that get false returned, are not iterated. Note that
        either modMain or posMain will be null, but not both: the filter must be
   aware of this, and respond appropriately. (ie, the filter may be used for
   filtering against an object, or against a position.)
*/
class PartitionFilter {
 public:
...
class PartitionFilterWouldCollide : public PartitionFilter {
 private:
  ...
  GeometryInfo m_geom;
  Real m_angle;
  Bool m_desiredCollisionResult;  // collision must match this for allow to
                                  // return true
 public:
...
class PartitionFilterRelationship : public PartitionFilter {
 ...
 public:
  enum {
    ALLOW_ALLIES =
        (1 << ALLIES),  ///< allow objects that m_obj considers allies
    ALLOW_ENEMIES =
        (1 << ENEMIES),  ///< allow objects that m_obj considers enemy
    ALLOW_NEUTRAL =
        (1 << NEUTRAL)  ///< allow objects that m_obj considers neutral
  };
  PartitionFilterRelationship(const Object *obj, Int flags)
      : m_obj(obj), m_flags(flags) {}
  virtual Bool allow(Object *objOther);
#if defined(_DEBUG) || defined(_INTERNAL)
...
class PartitionFilterPossibleToAttack : public PartitionFilter {
 ...
 public:
  PartitionFilterPossibleToAttack(AbleToAttackType t, const Object *obj,
                                  CommandSourceType commandSource);
  virtual Bool allow(Object *objOther);
#if defined(_DEBUG) || defined(_INTERNAL)
  virtual const char *debugGetName() {
    return "PartitionFilterPossibleToAttack";
  }
#endif
};
...
class PartitionFilterAcceptByObjectStatus : public PartitionFilter {
 ...
 public:
  PartitionFilterAcceptByObjectStatus(UnsignedInt mustBeSet,
                                      UnsignedInt mustBeClear)
      : m_mustBeSet(mustBeSet), m_mustBeClear(mustBeClear) {}
  virtual Bool allow(Object *objOther);
#if defined(_DEBUG) || defined(_INTERNAL)
  virtual const char *debugGetName() {
    return "PartitionFilterAcceptByObjectStatus";
  }
#endif
};
...
class PartitionFilterRejectByObjectStatus : public PartitionFilter {
 ...
 public:
  PartitionFilterRejectByObjectStatus(UnsignedInt mustBeSet,
                                      UnsignedInt mustBeClear)
      : m_mustBeSet(mustBeSet), m_mustBeClear(mustBeClear) {}
  virtual Bool allow(Object *objOther);
#if defined(_DEBUG) || defined(_INTERNAL)
  virtual const char *debugGetName() {
    return "PartitionFilterRejectByObjectStatus";
  }
#endif
};
...
//=====================================
/**
        Objects that are stealthed and not detected or disguised are accepted or
   rejected based on allow bool.
*/
...
#if defined(_DEBUG) || defined(_INTERNAL)
  virtual const char *debugGetName() {
    return "PartitionFilterStealthedAndUndetected";
  }
#endif
};
...
class PartitionFilterRejectByKindOf : public PartitionFilter {
 ...
 public:
  PartitionFilterRejectByKindOf(const KindOfMaskType &mustBeSet,
                                const KindOfMaskType &mustBeClear)
      : m_mustBeSet(mustBeSet), m_mustBeClear(mustBeClear) {}
  virtual Bool allow(Object *objOther);
#if defined(_DEBUG) || defined(_INTERNAL)
...
#if defined(_DEBUG) || defined(_INTERNAL)
  virtual const char *debugGetName() {
    return "PartitionFilterRejectBuildings";
  }
#endif
};
...
#if defined(_DEBUG) || defined(_INTERNAL)
  virtual const char *debugGetName() {
    return "PartitionFilterInsignificantBuildings";
  }
#endif
};
...
//=====================================
/**
 * Reject all objects outside of the irregularly shaped area defined by the set
 * of points passed in at creation. This is done using the even-odd rule against
 * the points, assuming that the area is closed.
 */
class PartitionFilterIrregularArea : public PartitionFilter {
 ...
 public:
  PartitionFilterIrregularArea(Coord3D *area, Int numPointsInArea)
      : m_area(area), m_numPointsInArea(numPointsInArea) {}
 ...
 * passed in at creation. This is done using the even-odd rule against the
 * points, assuming that the area is closed.
 */
...
//=====================================
/**
 * Allow or reject (based on match) all Objects whose affiliation matches one of
 * those specified by
 */
class PartitionFilterPlayerAffiliation : public PartitionFilter {
 ...
 public:
  // whichAffiliation should use AllowPlayerRelationship flags specified in
  // PlayerList.h
  PartitionFilterPlayerAffiliation(const Player *player,
                                   UnsignedInt whichAffiliation, Bool match)
...
#if defined(_DEBUG) || defined(_INTERNAL)
  virtual const char *debugGetName() {
    return "PartitionFilterPlayerAffiliation";
  }
#endif
};
...
class PartitionFilterGarrisonableByPlayer : public PartitionFilter {
 ...
 public:
  PartitionFilterGarrisonableByPlayer(Player *player, Bool match,
                                      CommandSourceType commandSource)
...
#if defined(_DEBUG) || defined(_INTERNAL)
  virtual const char *debugGetName() {
    return "PartitionFilterGarrisonableByPlayer";
  }
#endif
};
...
//=====================================
/**
 * Accept all objects that can/cannot have object X perform a command ability on
 * them
 */
...
#if defined(_DEBUG) || defined(_INTERNAL)
  virtual const char *debugGetName() {
    return "PartitionFilterValidCommandButtonTarget";
  }
#endif
};
...
//=====================================
/**
        PartitionManager is the singleton class that manages the entire
   partition/collision system. It maintains the set of PartitionCells that
   correspond to the world system, and updates the PartitionDatas as needed
   during update phase.
*/
class PartitionManager : public SubsystemInterface, public Snapshot {
 private:
  ...
  Bool m_updatedSinceLastReset;  ///< Used to force a return of
                                 ///< OBJECTSHROUD_INVALID before update has
                                 ///< been called.

  std::queue<SightingInfo *>
      m_pendingUndoShroudReveals;  ///< Anything can queue up an Undo to happen
                                   ///< later. This is a queue, because "later"
                                   ///< is a constant
  ...
  */
  Object *getClosestObjects(
      ...
      SimpleObjectIterator
          *iter,  // if nonnull, append ALL satisfactory objects to the iterator
                  // (not just the single closest)
  ...
  Int iterateCellsAlongLine(const Coord3D &pos, const Coord3D &posOther,
                            CellAlongLineProc proc, void *userData);

  // note iterateCellsBreadthFirst returns the cell index that made the
  // CellBreadthFirstProc return non-Zero.
  typedef Int (*CellBreadthFirstProc)(PartitionCell *cell, void *userData);
  Int iterateCellsBreadthFirst(const Coord3D *pos, CellBreadthFirstProc proc,
                               void *userData);
  ...
  // These are all friend functions now. They will continue to function as
  // before, but can be passed into the DiscreteCircle::drawCircle function.
  ...
  void processPendingUndoShroudRevealQueue(
      Bool considerTimestamp = TRUE);  ///< keep popping and processing untill
                                       ///< you get to one that is in the future
  void
  resetPendingUndoShroudRevealQueue();  ///< Just delete everything in the queue
 ...
 public:
  ...
  inline Bool getUpdatedSinceLastReset(void) const {
    return m_updatedSinceLastReset;
  }

  void registerObject(Object *object);    ///< add thing to system
  void unRegisterObject(Object *object);  ///< remove thing from system
  void registerGhostObject(
      GhostObject *object);  ///< recreate partition data needed to hold object
                             ///< (only used to restore after PM reset).
  void unRegisterGhostObject(
      GhostObject *object);  ///< release partition data held for ghost object.

  void
  processEntirePendingUndoShroudRevealQueue();  ///< process every pending one
  ...
  // Queueing does not give you control of the timestamp to enforce the queue.
  // I own the delay, you don't.
  void doShroudReveal(Real centerX, Real centerY, Real radius,
                      PlayerMaskType playerMask);
  void undoShroudReveal(Real centerX, Real centerY, Real radius,
                        PlayerMaskType playerMask);
  void queueUndoShroudReveal(Real centerX, Real centerY, Real radius,
                             PlayerMaskType playerMask);

  void doShroudCover(Real centerX, Real centerY, Real radius,
                     PlayerMaskType playerMask);
  void undoShroudCover(Real centerX, Real centerY, Real radius,
                       PlayerMaskType playerMask);

  /// Perform threat map and value map updates.
  void doThreatAffect(Real centerX, Real centerY, Real radius,
                      UnsignedInt threatVal, PlayerMaskType playerMask);
  void undoThreatAffect(Real centerX, Real centerY, Real radius,
                        UnsignedInt threatVal, PlayerMaskType playerMask);
  void doValueAffect(Real centerX, Real centerY, Real radius,
                     UnsignedInt valueVal, PlayerMaskType playerMask);
  void undoValueAffect(Real centerX, Real centerY, Real radius,
                       UnsignedInt valueVal, PlayerMaskType playerMask);
  ...
  // given a distance in world coords, return the number of cells needed to
  // cover that distance (rounding up)
  Int worldToCellDist(Real w);

  Object *getClosestObject(const Object *obj, Real maxDist,
  ...
  void getVectorTo(const Object *obj, const Object *otherObj,
                   DistanceCalculationType dc, Coord3D &vec);
  void getVectorTo(const Object *obj, const Coord3D *pos,
                   DistanceCalculationType dc, Coord3D &vec);

  // just like 'getDistance', but return the dist-sqr, meaning we save a sqrt()
  // call if you don't need it.
  Real getDistanceSquared(const Object *obj, const Object *otherObj,
                          DistanceCalculationType dc, Coord3D *vec = NULL);
  Real getDistanceSquared(const Object *obj, const Coord3D *pos,
                          DistanceCalculationType dc, Coord3D *vec = NULL);

  // just like 'getDistanceSquared', but return the dist-sqr where the obj is at
  // goalPos.
  Real getGoalDistanceSquared(const Object *obj, const Coord3D *goalPos,
...
#ifdef PM_CACHE_TERRAIN_HEIGHT
  // note that the 2d positions aren't guaranteed to be the actual spot within
  // the cell where the terrain is lowest or highest.... just the center of the
  // relevant cell. this function is used for rough-n-quick estimates only.
  Bool estimateTerrainExtremesAlongLine(const Coord3D &startWorld,
                                        const Coord3D &endWorld, Real *minZ,
                                        Real *maxZ, Coord2D *minZPos,
                                        Coord2D *maxZPos);
#endif
  ...
  /**
          ...
          geometry.
  */
  SimpleObjectIterator *iteratePotentialCollisions(const Coord3D *pos,
  ...
  Bool geomCollidesWithGeom(const Coord3D *pos1, const GeometryInfo &geom1,
                            Real angle1, const Coord3D *pos2,
                            const GeometryInfo &geom2, Real angle2) const;
  ...
  Real getCellSize() {
    return m_cellSize;
  }  // only for the use of PartitionData!

  /// return (1.0 / getCellSize); this is used frequently, so we cache it for
  /// efficiency
  Real getCellSizeInv() { return m_cellSizeInv; }

  /**
          return true iff there is clear line-of-sight between the two
     positions. this only takes terrain into account; it does not consider
     objects, units, trees, buildings, etc.
  */
  Bool isClearLineOfSightTerrain(const Object *obj, const Coord3D &objPos,
                                 const Object *other, const Coord3D &otherPos);
  ...
  /** this doesn't change the actual shroud values in logic, just pushes them
          back out to the display... this should generally only be used when the
     local player changes.
          */
  void refreshShroudForLocalPlayer();

  /**
          Shrouded has no absolute meaning.  It only makes sense to say
     "Shrouded for him".
  */
  CellShroudStatus getShroudStatusForPlayer(Int playerIndex, Int x,
                                            Int y) const;
  ...
  void getMostValuableLocation(Int playerIndex, UnsignedInt whichPlayerTypes,
                               ValueOrThreat valType, Coord3D *outLocation);
  void getNearestGroupWithValue(Int playerIndex, UnsignedInt whichPlayerTypes,
                                ValueOrThreat valType,
                                const Coord3D *sourceLocation,
                                Int valueRequired, Bool greaterThan,
                                Coord3D *outLocation);

  // If saveToFog is true, then we are writing STORE_FOG.
  // If saveToFog is false, then we are writing STORE_PERMENANT_REVEAL
  void storeFoggedCells(ShroudStatusStoreRestore &outPartitionStore,
                        Bool storeToFog) const;
  void restoreFoggedCells(const ShroudStatusStoreRestore &inPartitionStore,
                          Bool restoreToFog);
...
inline PartitionCell *PartitionManager::getCellAt(Int x, Int y) {
  return (x < 0 || y < 0 || x >= m_cellCountX || y >= m_cellCountY)
             ? NULL
             : &m_cells[y * m_cellCountX + x];
}
